...
type
  ...
  public
    ...
    function ParsePicture(Attr, Data: string): TRTFElement;
  ...
  public
    ...
    {$IFDEF FMXLIB}
    function AddBitmap(ABmp: TBitmap; Bkg: TAlphaColor = claWhite): string; overload;
    function AddBitmap(ABmp: TBitmap; PictureWidth, PictureHeight: integer; Bkg: TAlphaColor = claWhite): string; overload;
    {$ENDIF}
    ...
    property AnsiEncoding: boolean read FAnsiEncoding write FAnsiEncoding;
...
begin
  ...
  {$IFDEF FNCLIB}
  Result := ch.IsLetter or ch.IsNumber;
  {$ENDIF}

  {$IFNDEF FNCLIB}
  {$IFNDEF DELPHIXE4_LVL}
  Result := Character.IsLetter(ch) or IsNumChar(ch);
  {$ENDIF}
...
{$ENDIF}
  ...
  begin
    ...
    begin
      ...
      begin
        if (CharInStr(s,2 - d) = '/') and (Length(s) > 3) then
        begin
          case UpCase(CharInStr(s,3 - d)) of
          'A':begin
                if Anchor then
                begin
                  {$IFNDEF FMXLIB}
          ...
          'B':begin
                {$IFNDEF FMXLIB}
                if (CharInStr(s,4 - d) <> '>') and (RTFEngine.ForeColor <> clBlack) then
                  RTFEngine.AddForeColor(DefFont.Color)
                else
                {$ENDIF}
          ...
          'S':begin
                TagChar := UpCase(CharInStr(s,4 - d));
          ...
          'U':begin
                if (CharInStr(s,4 - d) <> '>') and (ListIndex > 0) then
                begin
                  RTFEngine.EndBullet;
                  FrstBullet := False;
                end
        ...
        begin
          case Upcase(CharInStr(s,2 - d)) of
          'A':begin
          ...
          'B':begin
                TagChar := Upcase(CharInStr(s,3 - d));
...
begin
  ...
  begin
    ...
    p := PChar(S);
    repeat
      unicode := UTF8CodepointToUnicode(p,i);

      if unicode >= 256 then
        enc := enc + '\u' + IntToStr(unicode)+ 'G'
      else
        if unicode > 0 then
        enc := enc + Chr(unicode);
    ...
    begin
      ...
      if (ord(ch) > 127) and (ord(ch) < 256) and FAnsiEncoding then
        enc := enc + '\''' + IntToHex(ord(ch),2)
      else
      if ord(ch) > 127 then
...
begin
  Result := AddBitmap(ABmp, -1,-1, Bkg);
end;

function TRTFEngine.AddBitmap(ABmp: TBitmap; PictureWidth, PictureHeight: integer; Bkg: TAlphaColor = claWhite): string;
var
  Str: TMemoryStream;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          while pos('\',su) > 0 do
          begin
            if (length(su) > 0) and (CharInStr(su,1) = '\') then
            begin
              delete(su,1,1);
              inc(i);
              if pos('\',su) > 0 then
          ...
          begin
            ...
            begin
              ...
              if (nextrep <> '\') then
              begin
                sco := sco + ansichar(chr(b))+ ansistring(su);
                if l = sl.Count - 1 then
                  sp := true;
              end
              else
...
//            else
            ...
            begin
              ...
              begin
                // unicode char followed by other RTF instruction
                if (l < sl.Count - 1) and (copy(sl[l + 1],1,2) <> '\u') then
                  sp := false
                else
                  sp := true;
              end;
            ...
            begin
              ...
              begin
                ...
                FAttr.refcs := FontTable[0].Charset;
              ...
              begin
                ...
                begin
                  ...
                  FAttr.refcs := FontTable[k].CharSet;
...
begin
  ...
  begin
    ...
    if (docstate = dsFont) and (CharInStr(FRTFObjects[i].RTFType, 1) = 'f') then
    begin
  ...
  begin
    ...
    begin
      ...
      begin
        elem := ParsePicture(FRTFObjects[i].RTFAttr, FRTFObjects[i].RTFData);
        if Assigned(elem) then
          elem.Alignment := FAttr.real;
      end
      else
...
{$ENDIF}
{$ENDIF}
function TRTFDocument.ParsePicture(Attr, Data: string): TRTFElement;
var
  r,su: string;
  i: integer;
  b: byte;
...
begin
  ...
  begin
    ...
    try
      ...
      scalebmp.Width := jpg.Width;
      scalebmp.Height := jpg.Height;
      {$ENDIF}
      {$ENDIF}
      ...
      begin
        ...
        {$IFNDEF LCLLIB}
        {$IFDEF DELPHIXE2_LVL}
        scalebmp.Canvas.Draw(0,0,jpg);

        // higher quality image scaling
        if (scalex <> 100) or (scaley <> 100) then
          ResizeBitmap(scalebmp, elem.Bitmap.Width, elem.Bitmap.Height);

        elem.Bitmap.Canvas.Draw(0,0,scalebmp);
  ...
  begin
    ...
    try
      ...
      begin
        ...
        {$IFDEF DELPHIXE2_LVL}
        scalebmp := TBitmap.Create;
        scalebmp.Width := png.Width;
        scalebmp.Height := png.Height;
        {$ENDIF}
        {$ENDIF}
  ...
  begin
    ...
    begin
      ...
      elem.Bitmap.Canvas.FillRect(Rect(0,0, elem.Bitmap.Width - 1, elem.Bitmap.Height - 1));
      elem.Bitmap.Transparent := false;
      ...
      if (goalx <> -1) and (goaly <> -1) then
      begin
        Elem.PicWidth := Round(goalx / IMAGE_DIVIDER);
        Elem.PicHeight := Round(goaly / IMAGE_DIVIDER);
      end;
