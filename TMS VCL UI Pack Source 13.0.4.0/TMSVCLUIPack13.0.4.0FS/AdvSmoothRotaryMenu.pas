...
const
  ...
  // v1.2.1.2 : Improved : On creation check for enabeld AdvFormStyler
  // v1.2.2.0 : New : BackgroundPicture implemented
  // v1.2.3.0 : New : Transparent property added 
...
type
  ...
  private
    ...
    FBackgroundPictureHeight: Integer;
    FBackgroundPictureWidth: Integer;
    FBackgroundPictureClipMenuRegion: Boolean;
    ...
    procedure SetBackgroundPictureHeight(const Value: Integer);
    procedure SetBackgroundPictureWidth(const Value: Integer);
    procedure SetBackgroundPictureClipMenuRegion(const Value: Boolean);
  ...
  public
    ...
    procedure DrawBackgroundPicture(g: TGPGraphics; R: TGPRectF);
  ...
  published
    ...
    property BackgroundPictureWidth: Integer read FBackgroundPictureWidth write SetBackgroundPictureWidth default -1;
    property BackgroundPictureHeight: Integer read FBackgroundPictureHeight write SetBackgroundPictureHeight default -1;
    property BackgroundPictureClipMenuRegion: Boolean read FBackgroundPictureClipMenuRegion write SetBackgroundPictureClipMenuRegion default True;
...
procedure TAdvSmoothRotaryMenu.BackgroundPictureChanged(Sender: TObject);
begin
  Changed;
end;
...
begin
  ...
  FBackgroundPictureHeight := -1;
  FBackgroundPictureWidth := -1;
  FBackgroundPictureClipMenuRegion := True;
...
procedure TAdvSmoothRotaryMenu.DrawBackgroundPicture(g: TGPGraphics; R: TGPRectF);
var
  dr: TGPRectF;
...
begin
  ...
  begin
    ...
    Path.AddPie(R, 0, 360);
    CircleRgn := TGPRegion.Create(Path);
    try
      if FBackgroundPictureClipMenuRegion then
        g.SetClip(CircleRgn);

      dr := r;
      if FBackgroundPictureWidth >= 0 then
      begin
        dr.Width := FBackgroundPictureWidth;
        dr.X := (r.Width - FBackgroundPictureWidth) / 2;
      end;

      if FBackgroundPictureHeight >= 0 then
      begin
        dr.Height := FBackgroundPictureHeight;
        dr.Y := (r.Height - FBackgroundPictureHeight) / 2;
      end;

      FBackgroundPicture.GDIPDraw(g, dr);
    finally
      if FBackgroundPictureClipMenuRegion then
        g.ResetClip;
      Path.Free;
...
procedure TAdvSmoothRotaryMenu.PaintTransparency;
var
  i: Integer;
  P: TPoint;
begin
...
procedure TAdvSmoothRotaryMenu.SetBackgroundPicture(const Value: TAdvGDIPPicture);
begin
  FBackgroundPicture.Assign(Value);
  Changed;
end;

procedure TAdvSmoothRotaryMenu.SetBackgroundPictureClipMenuRegion(const Value: Boolean);
begin
  if FBackgroundPictureClipMenuRegion <> Value then
  begin
    FBackgroundPictureClipMenuRegion := Value;
    Changed;
  end;
end;

procedure TAdvSmoothRotaryMenu.SetBackgroundPictureHeight(const Value: Integer);
begin
  if FBackgroundPictureHeight <> Value then
  begin
    FBackgroundPictureHeight := Value;
    Changed;
  end;
end;

procedure TAdvSmoothRotaryMenu.SetBackgroundPictureWidth(const Value: Integer);
begin
  if FBackgroundPictureWidth <> Value then
  begin
    FBackgroundPictureWidth := Value;
    Changed;
  end;
end;

procedure TAdvSmoothRotaryMenu.SetTransparent(const Value: Boolean);
begin
...
begin
  ...
  case AStyle of
    ...
    begin
      ...
      DefaultFill.Color := clWhite;
      DefaultFill.ColorTo := clWhite;
      DefaultFill.BorderColor := $00C4C6C8;
      DefaultFill.EndUpdate;

      DefaultHoverFill.BeginUpdate;
      DefaultHoverFill.Assign(DefaultFill);
      DefaultHoverFill.Color := $00F2E1D5;
      DefaultHoverFill.ColorTo := $00F2E1D5;
      DefaultHoverFill.EndUpdate;

      DefaultDownFill.BeginUpdate;
      DefaultDownFill.Assign(DefaultFill);
      DefaultDownFill.Color := $00E3BDA3;
      DefaultDownFill.ColorTo := $00E3BDA3;
      DefaultDownFill.EndUpdate;
    end;
    tsOffice2019Gray:
    begin
      DefaultFill.BeginUpdate;
      DefaultFill.Color := $00B8BBBE;
      DefaultFill.ColorTo := $00B8BBBE;
      DefaultFill.BorderColor := $00666666;
      DefaultFill.EndUpdate;

      DefaultHoverFill.BeginUpdate;
      DefaultHoverFill.Assign(DefaultFill);
      DefaultHoverFill.Color := $00969696;
      DefaultHoverFill.ColorTo := $00969696;
      DefaultHoverFill.EndUpdate;

      DefaultDownFill.BeginUpdate;
      DefaultDownFill.Assign(DefaultFill);
      DefaultDownFill.Color := $00666666;
      DefaultDownFill.ColorTo := $00666666;
      DefaultDownFill.EndUpdate;
    end;
    tsOffice2019Black:
    begin
      DefaultFill.BeginUpdate;
      DefaultFill.Color := $00303132;
      DefaultFill.ColorTo := $00303132;
      DefaultFill.BorderColor := $00ADB0B3;
      DefaultFill.EndUpdate;

      DefaultHoverFill.BeginUpdate;
      DefaultHoverFill.Assign(DefaultFill);
      DefaultHoverFill.Color := $00686868;
      DefaultHoverFill.ColorTo := $00686868;
      DefaultHoverFill.EndUpdate;

      DefaultDownFill.BeginUpdate;
      DefaultDownFill.Assign(DefaultFill);
      DefaultDownFill.Color := $00828282;
      DefaultDownFill.ColorTo := $00828282;
      DefaultDownFill.EndUpdate;
    end;
  end;
...
procedure TAdvSmoothRotaryMenuDialog.SetUIStyle(const Value: TTMSStyle);
begin
...
procedure TAdvSmoothRotaryMenu.SetUIStyle(const Value: TTMSStyle);
begin
  SetComponentStyle(Value);
end;
