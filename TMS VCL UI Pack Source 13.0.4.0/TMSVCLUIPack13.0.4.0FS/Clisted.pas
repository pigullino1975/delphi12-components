...
const
  ...
  // 1.4.3.3 : Improved : Handling of double-quotes in list text
  // 1.4.3.4 : Fixed : Issue with label font size handling in high DPI scenarios
  // 1.4.3.5 : Fixed : Rare issue when destroy happens while dropdown is visible
  // 1.4.3.5 : Fixed : Regression with label font in specific scenario
  // 1.4.4.0 : New : Case-sensitive property added
  // 1.4.4.1 : Fixed : Issue with items with double quotes
  // 1.4.4.2 : Fixed : Issue with TAB handling when dropdown is active
  // 1.4.4.3 : Fixed : Width of dropdown button handling for high DPI
  // 1.4.4.4 : Fixed : Workaround for bug in VCL TCheckListBox for checkbox size & DPI
  // 1.4.4.5 : Improved : LabelFont handling in connection with ParentFont
  // 1.4.4.6 : Fixed : Issue with EditorEnabled = true
  // 1.4.4.7 : Fixed : Issue with AutoDropDownSize on high DPI
...
begin
  inherited CreateWnd;
  SetEditRect;
  ReadOnly := not EditorEnabled;

  FDPIScale := GetDPIScale(FButton, FButton.Canvas);

  if not ParentFont and (LabelCaption <> '') and not FLoaded and not (csDesigning in ComponentState) then
  begin
    FLblUpdate := true;
...
begin
  ...
  FCheckListBox.ChangeScale(CurrentPPI,CurrentPPI,true);
  {$ENDIF}
  ...
  begin
    ...
    begin
      ...
      begin
        if (length(s) > 0) and (s[1] = startchar) then
        begin
...
procedure TCheckListEdit.SetEditorEnabled(const Value: Boolean);
begin
...
begin
  ...
  begin
    if NewStyleControls and Ctl3D and not (BorderStyle = bsNone) then
      FButton.SetBounds(Width - FButton.Width - 4, 0, FButton.Width , Height - Dist)
    else
...
procedure TCheckListEdit.CMFontChanged(var Message: TMessage);
begin
...
begin
  ...
  FDropFont.Size := MulDiv(FDropFont.Size, M, D);

  if Assigned(FCheckListBox) then
  {$IFDEF DELPHIXE10_LVL}
    FCheckListBox.ChangeScale(M, D, isDPIChange);
  {$ENDIF}
  {$IFNDEF DELPHIXE10_LVL}
    FCheckListBox.ChangeScale(M, D);
  {$ENDIF}

  FDropHeight := MulDiv(FDropHeight, M, D);
  FDropWidth := MulDiv(FDropWidth, M, D);
  FButton.Width := Round(17 * FDPIScale);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (Pos(FTextDelimiter, sli) > 0) then
      begin
        s := s + '"' + sli + '"';
      end
      else
...
begin
  ...
  begin
    ...
    FItems.CaseSensitive := FCaseSensitive;

    chklist := TStringList.Create;
    chklist.Assign(FItems);

    for i := 0 to chklist.Count - 1 do
    begin
      ss := chklist.Strings[i];
      if (Pos('"', ss) = 1) and (ss[length(ss)] = '"') then
      begin
        ss := copy(ss,2,length(ss)-2);
        chklist.Strings[i] := ss;
      end;
    end;

    if (Pos('"', sli) = 1) and (sli[length(sli)] = '"') then
      sli := copy(sli, 2, length(sli)-2);

    i := chklist.Indexof(sli);
...
begin
  ...
  begin
    ...
    FLabelFont.Height := Round(FLabelFont.Height * FDPIScale);
...
{$IFNDEF DELPHIXE10_LVL}
procedure TInplaceCheckListBox.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TInplaceCheckListBox.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
end;
...
begin
  ...
  begin
    ...
    LHeight := LHeight + Canvas.TextHeight(Items[i]);
    if LWidth > MaxWidth then
      MaxWidth := LWidth;
  end;

  if LHeight + 2 * GetSystemMetrics(SM_CYBORDER) > Height then
