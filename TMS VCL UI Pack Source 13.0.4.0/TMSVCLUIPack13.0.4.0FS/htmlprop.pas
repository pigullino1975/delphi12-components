...
type
  ...
  public
    { Public declarations }
    {$IFDEF DELPHIXE10_LVL}
    FEmoticonPicker: TAdvEmoticonPickerDropDown;
    procedure EmoticonSelect(Sender: TObject);
    {$ENDIF}
...
begin
  ...
  {$IFDEF DELPHIXE10_LVL}
  FEmoticonPicker := TAdvEmoticonPickerDropDown.Create(ToolBar1);
  FEmoticonPicker.Parent := ToolBar1;
  FEmoticonPicker.Left := 650;
  FEmoticonPicker.ShowHint := true;
  FEmoticonPicker.Hint := 'Insert emoticon';
  FEmoticonPicker.OnSelect := EmoticonSelect;
  {$ENDIF}
end;

procedure THTMLEditor.FormShow(Sender: TObject);
begin
  FProg := true;
  AdvOfficeFontSelector1.ItemIndex := 0;
  AdvOfficeFontSizeSelector1.ItemIndex := 0;
  FProg := false;
end;
...
begin
  ...
  if HasTags('<FONT ','</FONT>') then
    ChangeTagProp('<FONT ','</FONT>','face',AdvOfficeFontSelector1.SelectedFontName)
  else
    InsertTags('<FONT face="'+AdvOfficeFontSelector1.SelectedFontName+'">','</FONT>');
  memo1.setfocus;
end;
...
begin
  ...
  if HasTags('<FONT ','</FONT>') then
    ChangeTagProp('<FONT ','</FONT>',' size', IntToStr(AdvOfficeFontSizeSelector1.SelectedFontSize))
  else
    InsertTags('<FONT size="'+IntToStr(AdvOfficeFontSizeSelector1.SelectedFontSize)+'">','</FONT>');
  memo1.setfocus;
end;
...
begin
  ...
  with Form do
	...
	{$IFDEF DELPHI_UNICODE}
        Edit.Style := TComboBoxStyle.csDropDown;
	{$ENDIF}
      end;
...
{$IFDEF DELPHIXE10_LVL}
procedure Thtmleditor.EmoticonSelect(Sender: TObject);
begin
  if FEmoticonPicker.EmoticonIndex >= 0 then
  begin
    InsertTags(FEmoticonPicker.EmoticonHTML,'');
  end;
end;
{$ENDIF}
