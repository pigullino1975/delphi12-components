...
const
  ...
  // v1.0.0.2 : Fixed: Issue with Path delimiter in OSX
...
uses
  ...
  {$ENDIF}
  {$IFDEF VCLLIB}
  ,ComObj, ActiveX, Messages, ShlObj, Types
  {$ENDIF}
  {$IFNDEF LCLLIB}
  {$IFDEF LINUX}
...
var
  i, cIHeaderSize, mIHeaderSize, EndPos, StartoF: NativeUInt;
  zip, hBmp8bit: Cardinal;
  IconInfo: TIconInfo;
    ...
    begin
      ...
      try
        ...
        try
          ...
          CopyMemory(Pointer(NativeUInt(Pstorage) + StartoF), PcInfo,
            cIHeaderSize);
          CopyMemory(Pointer(NativeUInt(Pstorage) + StartoF + cIHeaderSize),
          ...
          CopyMemory(Pointer(NativeUInt(Pstorage) + sizeof(IconCurHd) +
...
begin
  ...
  {$IFDEF MSWINDOWS}
  fst := 0;
  SHGetFileInfo(PChar(AFile), 0, {%H-}fi, SizeOf(fi), SHGFI_DISPLAYNAME or SHGFI_TYPENAME or SHGFI_ICON or SHGFI_SMALLICON);
...
{$ENDIF}
{$HINTS OFF}
{$IF DEFINED(LCLLIB) AND NOT DEFINED(MSWINDOWS)}
begin
  AList.Add('/');
{$IFEND}
{$HINTS ON}
