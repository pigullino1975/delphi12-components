...
const
  ...
  // v1.1.1.0 : New : ButtonImageStretch and ButtonImageAspectRatio property when using larger images
  // v1.1.2.0 : New : Image scales when Imagelist or PictureContainer is used or it is a SVG
  //          : New : ButtonImageMargin property added
...
type
  ...
  private
    ...
    FChecked: Boolean;
    FAutoCheck: Boolean;
    FButtonImageStretch: Boolean;
    FButtonImageAspectRatio: Boolean;
    FButtonImageMargin: TMargins;
    ...
    procedure SetButtonImageStretch(const Value: Boolean);
    procedure SetButtonImageAspectRatio(const Value: Boolean);
    procedure SetButtonImageMargin(const Value: TMargins);
  protected
    ...
    procedure ButtonImageMarginChanged(Sender: TObject);
  ...
  published
    ...
    property ButtonImageStretch: Boolean read FButtonImageStretch write SetButtonImageStretch;
    property ButtonImageAspectRatio: Boolean read FButtonImageAspectRatio write SetButtonImageAspectRatio default True;
    property ButtonImageMargin: TMargins read FButtonImageMargin write SetButtonImageMargin;
...
procedure TImageTextButtonItem.ButtonImageMarginChanged(Sender: TObject);
begin
  Changed;
end;
...
begin
  ...
  FButtonImageStretch := False;
  FButtonImageAspectRatio := True;
  ...
  FChecked := False;
  FAutoCheck := True;
  FButtonImageMargin := TMargins.Create(nil);
  FButtonImageMargin.Top := 0;
  FButtonImageMargin.Right := 0;
  FButtonImageMargin.Bottom := 0;
  FButtonImageMargin.Left := 0;
  FButtonImageMargin.OnChange := ButtonImageMarginChanged;
...
begin
  ...
  begin
    ...
    if FButtonImageStretch then
      f.PicturePosition := ppStretched
    else
      f.PicturePosition := ppCenterCenter;

    f.PictureAspectRatio := FButtonImageAspectRatio;

    if (FButtonType in [btButton, btPushButton]) and((FDown and Assigned(FButtonImageDown)) or (not FDown and Assigned(FButtonImageUp))) then
    begin
      r.X := r.X + FButtonImageMargin.Left;
      r.Y := r.Y + FButtonImageMargin.Top;
      r.Width := r.Width - FButtonImageMargin.Left - FButtonImageMargin.Right;
      r.Height := r.Height - FButtonImageMargin.Top - FButtonImageMargin.Bottom;
    end;
...
procedure TImageTextButtonItem.DrawImage(g: TGPGraphics; ARect: TGPRectF; ItemAppearance: TItemAppearance);
var
  r: TGPRectF;
  imgr: TGPRectF;
  imgw, imgh: Single;
begin
  ...
  begin
    ...
    if (Assigned(ItemOwner) and (ItemOwner is TCustomItemsContainer) and (Assigned((ItemOwner as TCustomItemsContainer).ImageList) or Assigned((ItemOwner as TCustomItemsContainer).PictureContainer)))
      {$IFDEF DELPHIXE7_LVL} or Image.HasSVG {$ENDIF} then
    begin
      imgw := DPIScale * ImageWidth;
      imgh := DPIScale * ImageHeight;
    end
    else
    begin
      imgw := ImageWidth;
    ...
    imgr := MakeRect(r.X + Spacing, r.Y + (r.Height - imgh) / 2, imgw, imgh);
...
begin
  ...
  begin
    r.X := r.X + imgw + (DPIScale * Spacing);
    r.Width := r.Width - imgw - (DPIScale * Spacing);
  end;
  Result := r;
...
procedure TImageTextButtonItem.SetButtonImageAspectRatio(const Value: Boolean);
begin
  if FButtonImageAspectRatio <> Value then
  begin
    FButtonImageAspectRatio := Value;
    Changed;
  end;
end;

procedure TImageTextButtonItem.SetButtonImageDown(const Value: TAdvGDIPPicture);
begin
...
procedure TImageTextButtonItem.SetButtonImageMargin(const Value: TMargins);
begin
  if FButtonImageMargin <> Value then
  begin
    FButtonImageMargin.Assign(Value);
    Changed;
  end;
end;

procedure TImageTextButtonItem.SetButtonImageStretch(const Value: Boolean);
begin
  if FButtonImageStretch <> Value then
  begin
    FButtonImageStretch := Value;
    Changed;
  end;
end;
...
begin
  ButtonGlow := not (AStyle in  [tsWindows8, tsOffice2019White, tsOffice2019Gray, tsOffice2019Black]);
end;
