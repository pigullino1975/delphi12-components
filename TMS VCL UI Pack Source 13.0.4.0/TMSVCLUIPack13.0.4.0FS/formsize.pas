...
const
  ...
  // 1.4.1.5 : Fixed : Issue when only persisting form size without form position
  // 1.4.2.0 : New : Property FollowMainForm added
  // 1.4.2.1 : Fixed : Compiler warnings for LOpenOnSameFormAsMainForm in versions older than XE7 and deprecated functions
  // 1.4.3.0 : Improved : Handling of persistence with different DPI on monitors
  // 1.4.3.1 : Fixed : Issue with persisting window state in database
  // 1.4.3.2 : Improved : Checks for Application.MainForm in case of use via DLL
  // 1.4.4.0 : New : Event OnError for write errors when persisting to file
  // 1.5.0.0 : New : Events OnLoaded / OnSaved added
...
type
  ...
  TFormSizeErrorEvent = procedure(Sender: TObject; AMessage: string) of object;
  ...
  private
    ...
    FUseHook: boolean;
    FLoaded: boolean;
    FFollowMainForm: boolean;
    FOnError: TFormSizeErrorEvent;
  ...
  protected
    ...
    procedure CorrectARectToMainFormCenter(var aFormRect : TRect; const aMainFormRect : TRect);
    procedure HandleError(AMessage: string); virtual;
  public
    { Public declarations }
  ...
  published
    ...
    property FollowMainForm: boolean read FFollowMainForm write FFollowMainForm default False;
    ...
    property OnError: TFormSizeErrorEvent read FOnError write FOnError;
...
var
  ...
  dpi: single;
  {$IFDEF DELPHIXE7_LVL}
  lOpenOnSameFormAsMainForm: boolean;
  {$ENDIF}
begin
  ...
  hasval := false;
  {$IFDEF DELPHIXE7_LVL}
  lOpenOnSameFormAsMainForm := false;
  {$ENDIF}
      ...
      try
        ...
        begin
          ...
          begin
            ...
            if not FSavePosition then
            begin
              Right := Left + ReadInteger(Key, 'Right', Right);
              Bottom := Top + ReadInteger(Key, 'Bottom', Bottom);
            end
            else
            begin
          ...
          {$IFDEF DELPHIXE7_LVL}
          if FollowMainform then
            lOpenOnSameFormAsMainForm := ReadBool(Key, 'OnMainFormScreen', false)
          else
            lOpenOnSameFormAsMainForm := false;
          {$ENDIF}
        end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if (DataBinding.FieldWindowState <> '') then
            begin
              fld := FDataLink.DataSource.DataSet.FindField(DataBinding.FieldWindowState);
              if Assigned(fld) then
              begin
  ...
  {$IFDEF DELPHIXE7_LVL}
  if lOpenOnSameFormAsMainForm and Assigned(Application) and Assigned(Application.MainForm) and ((Owner as TForm).Handle <> Application.MainForm.Handle) then
    if not IntersectRect(ARect, Application.MainForm.Monitor.BoundsRect) then
      CorrectARectToMainFormCenter(ARect, Application.MainForm.BoundsRect);
  {$ENDIF}
...
//  MoveWindow((Owner as TForm).Handle, ARect.Left, ARect.Top, ARect.Width, ARect.Height, true);
  ...
  begin
    ...
    mon := Screen.MonitorFromPoint(Point(ARect.Left + ((ARect.Right - ARect.Left) div 2),
      ARect.Top + ((ARect.Bottom - ARect.Top) div 2)));
    dpi := mon.PixelsPerInch/96;
    {$ENDIF}

    MoveWindow((Owner as TForm).Handle, ARect.Left, ARect.Top, Round(dpi * (ARect.Right - ARect.Left)), Round(dpi * (ARect.Bottom - ARect.Top)), true);
...
begin
  ...
  ARect := Rect((Owner as TForm).Left, (Owner as TForm).Top, (Owner as TForm).Left + (Owner as TForm).Width,
    (Owner as TForm).Top + (Owner as TForm).Height);
        ...
        try
          ...
          begin
            ...
            begin
              if not FSavePosition then
              begin
                WriteInteger(Key, 'Right', Round((Owner as TForm).Width/dpi));
                WriteInteger(Key, 'Bottom', Round((Owner as TForm).Height/dpi));
              end
              else
              begin
                WriteInteger(Key, 'Right', (Owner as TForm).Left + Round((Owner as TForm).Width/dpi));
                WriteInteger(Key, 'Bottom', (Owner as TForm).Top + Round((Owner as TForm).Height/dpi));
              end;
              WriteBool(Key, 'Maximized', showCmd = SW_SHOWMAXIMIZED);
            end;

            if FollowMainform and Assigned(Application) and Assigned(Application.MainForm) then
              WriteBool(Key, 'OnMainFormScreen', (Owner as TForm).Monitor.Handle = Application.MainForm.Monitor.Handle);
          end;
        except
          on e: Exception do
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if Assigned(fld) then
                fld.AsInteger := Round((Self.Owner as TForm).Width/dpi);
            end;
            if DataBinding.FieldSizeHeight <> '' then
            begin
              fld := FindField(DataBinding.FieldSizeHeight);
              if Assigned(fld) then
                fld.AsInteger := Round((Self.Owner as TForm).Height/dpi);
            end;
            if DataBinding.FieldUser <> '' then
            begin
            ...
            if DataBinding.FieldWindowState <> '' then
            begin
              fld := FindField(DataBinding.FieldWindowState);
              if Assigned(fld) then
                fld.AsBoolean :=  (Placement.showCmd = SW_SHOWMAXIMIZED);
            end;
            Post;
          end;
        end;
  ...
  if Assigned(OnSaved) then
    OnSaved(Self);
end;  { TFormSize.DoSavePlacement }

procedure TFormSize.CorrectARectToMainFormCenter(var aFormRect: TRect; const aMainFormRect: TRect);
{$IFDEF DELPHIXE7_LVL}
var
  lMainFormCenterPoint, lFormCenterPoint : TPoint;
  lDifX, lDifY : integer;
{$ENDIF}
begin
{$IFDEF DELPHIXE7_LVL}
  lMainFormCenterPoint := CenterPoint(aMainFormRect);
  lFormCenterPoint := CenterPoint(aFormRect);

  lDifX := lMainFormCenterPoint.X - lFormCenterPoint.X;
  lDifY := lMainFormCenterPoint.Y - lFormCenterPoint.Y;

  // move TRect
  aFormRect.SetLocation(aFormRect.Left + lDifX, aFormRect.Top + lDifY);
{$ENDIF}
end;

procedure TFormSize.HandleError(AMessage: string);
begin
  if Assigned(OnError) then
    OnError(Self, AMessage);
end;
