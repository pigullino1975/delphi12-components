...
const
  ...
  // 1.8.0.1 : Fixed : Design-time style painting
  // 1.8.1.0 : New : UIStyle property for office look
  //         : Improved : On creation check for enabled AdvFormStyler
  // 1.8.1.1 : Fixed : Issue with auto sizing of hint when HintWidth = -1
  // 1.8.1.2 : Improved : Hint rendering with help and rounded
  // 1.8.2.0 : New : Per monitor support for High DPI
  // 1.8.2.1 : Fixed : Updated Office2019 colors
  // 1.8.2.2 : Improved : Hint width calculation
  ...
  case AStyle of
   ...
   tsOffice2016Black:
      ...
      begin
        ...
        HintColor := clWhite;
        HintColorTo := clWhite;
        HintHelpLineColor := $00ABABAB;
      end;
   tsOffice2019Gray:
      begin
        Rounded := false;
        HintColor := $00F0F0F0;
        HintColorTo := $00F0F0F0;
        HintHelpLineColor := $00ABABAB;
      end;
   tsOffice2019Black:
      begin
        Rounded := false;
        HintColor := $00EBEBEB;
        HintColorTo := $00EBEBEB;
        HintHelpLineColor := $00BEBEBE;
      end;
  end;
end;
...
begin
  ...
  Canvas.Font.Height := Round(FDPIScale * FHint.Font.Height);
  ...
  begin
    ...
    begin
      ...
      if FHint.HintWidth > 0  then
        Rect.Right := Rect.Left + Round(FDPIScale * FHint.HintWidth)
      else if FHint.HintWidth = 0  then
        Rect.Right := Rect.Left + Round(FDPIScale * 200)
      else
        Rect.Right := Rect.Left - Round(FDPIScale * FHint.HintWidth);
      ...
      begin
        ...
        if (FHint.HintWidth = -1) and (AHintInfo.Title <> '') then
        begin
          hr.Right := 10000;
          DrawText(Canvas.Handle,PChar(FHint.FHintInfo.Title), Length(FHint.FHintInfo.Title), hr, dtfmt);
          Rect.Right := Max(Rect.Right, hr.Right + 24 + ImgW);
        end;
