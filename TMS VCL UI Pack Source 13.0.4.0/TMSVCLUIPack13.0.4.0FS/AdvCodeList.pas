...
uses
  ...
  Forms, AdvCodeHint, ImgList, Clipbrd, Dialogs, AdvCodeListDD, ActiveX, AdvStyleIF
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF};
...
const
  ...
  BLD_VER = 4; // Build nr.
  DATE_VER = 'Jun, 2023'; // Month version
  ...
  // 3.1.0.1 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // 3.1.0.2 : Fixed : Issue with font color in combination wwith code block caption
  // 3.1.0.3 : Fixed : Issue with clipped text for large font in code blocks
  // 3.1.0.4 : Fixed : Issue with drawing of empty code list
...
begin
  ...
  begin
    ...
    if Integer(ItemId) > CodeBlocks.Count then
      Exit;
    ...
    begin
      ...
      begin
        ...
        bmp.Canvas.Brush.Style := bsClear;
        bmp.Canvas.RoundRect(rcItem.left + 2, rcItem.Top + 1, rcItem.Right - 1, rcItem.Bottom-1, 8, 8);
      end;
    end;
    ...
    begin
      rcItem := Rect(rcItem.Left + 1, t, rcItem.Right - 1, t + bmp.Height);
      SetTextColor(hDC, 0);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          s := SL[SubIndex];
          DrwR := Rect(ARect.Left + Offset, ARect.Top, ARect.Right, ARect.Top + ARect.Top + th);
