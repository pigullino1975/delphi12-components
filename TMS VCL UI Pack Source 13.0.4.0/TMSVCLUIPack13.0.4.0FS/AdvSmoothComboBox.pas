...
const
  ...
  // v2.4.0.0 : New : Support for Office 2019 styles
  // v2.4.1.0 : New : UIStyle property for TMSStyle
  // v2.4.1.1 : Fixed : UIStyle set to tsCustom after initial Office look
  // v2.4.1.2 : Improved : On creation check for enabled AdvFormStyler
  // v2.4.2.0 : New : Per monitor DPI handling
  //          : Fixed : Dropdown direction with multiple screens
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvSmoothComboBox.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvSmoothComboBox.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;

  FDPIScale := GetDPIScale(Self, Canvas);
  FListBox.SetDPIScale(FDPIScale);
...
begin
  ...
  if FAutoHeight and not ((csLoading in ComponentState) or (csReading in ComponentState)) then
    Height := Round(FDPIScale * ItemAppearance.Height);
...
var
  ...
  procedure DrawArrow(ARect: TRect; AClr: TColor);
  begin
    Canvas.Pen.Color := AClr;
    ...
    begin
      ...
      Canvas.LineTo(ARect.Left + (ARect.Right - ARect.Left) div 2, ARect.Top + (ARect.Bottom - ARect.Top) div 2 + Round(FDPIScale * 1) + Canvas.Pen.Width);
      Canvas.LineTo(ARect.Left + (ARect.Right - ARect.Left) div 2 + Round(FDPIScale * 4) + 1 - (Canvas.Pen.Width - 1), ARect.Top + (ARect.Bottom - ARect.Top) div 2 - Round(FDPIScale * 2) - 1 + (Canvas.Pen.Width - 1));
    end
    else
    begin
      ...
      Canvas.LineTo(ARect.Left + (ARect.Right - ARect.Left) div 2 + Round(FDPIScale * 3) + 1 - (Canvas.Pen.Width - 1), ARect.Top + (ARect.Bottom - ARect.Top) div 2 - Round(FDPIScale * 1) - 1 + (Canvas.Pen.Width - 1) );
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      DrawArrow(GetButtonRectangle,clr);
    end;
  end;
end;
...
begin
  ...
  try
    ...
    if HandleAllocated then
      ...
      GetMonitorInfo(MonitorFromWindow(GetParentWnd, MONITOR_DEFAULTTONEAREST), @MonInfo);
  ...
  if (LstPos.y + Round(FDPIScale * DropDownHeight) > r.Bottom) then
    LstPos.Y := LstPos.Y - Round(FDPIScale * DropDownHeight) - Height + 3;

  if (LstPos.x + Round(FDPIScale * DropDownWidth) > r.right) then
    LstPos.x := LstPos.x - (Round(FDPIScale * DropDownWidth) - Width);
  ...
  FListbox.Width := Round(FDPIScale * DropDownWidth);
  FListBox.Height := Round(FDPIScale * DropDownHeight);
...
begin
  result := Bounds(InsideRect.Right - Round(FDPIScale * ButtonAppearance.Width) - 1, InsideRect.Top, Round(FDPIScale * ButtonAppearance.Width) + 1, InsideRect.Bottom - InsideRect.Top);
end;
...
begin
  result := Bounds(InsideRect.Left, InsideRect.Top, InsideRect.Right - InsideRect.Left - Round(FDPIScale * ButtonAppearance.Width), InsideRect.Bottom - InsideRect.Top);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      r := Bounds(5, 5, Width - 10 - Round(FDPIScale * ButtonAppearance.Width), Height - 10);
...
begin
  ...
  ButtonAppearance.ArrowColor := clBlack;
  ButtonAppearance.ArrowColorHover := clBlack;
  ButtonAppearance.ArrowColorDown := clBlack;
 ...
 case AStyle of
    ...
    tsTerminal:
      ...
      tsOffice2019White:
      begin
        ButtonAppearance.FillHover.Color := $00F2E1D5;
        ButtonAppearance.FillHover.ColorTo := clNone;
        ButtonAppearance.FillHover.ColorMirror := clNone;
        ButtonAppearance.FillHover.ColorMirrorTo := clNone;
        ButtonAppearance.FillHover.BorderColor :=  $00E3BDA3;
        ButtonAppearance.FillHover.GradientMirrorType := gtVertical;

        ButtonAppearance.ArrowColor := $00444648;
        ButtonAppearance.ArrowColorHover := $00444648;
        ButtonAppearance.ArrowColorDown := $00444648;
      end;
      tsOffice2019Gray:
      begin
        ButtonAppearance.FillHover.Color := $00969696;
        ButtonAppearance.FillHover.ColorTo := clNone;
        ButtonAppearance.FillHover.ColorMirror := clNone;
        ButtonAppearance.FillHover.ColorMirrorTo := clNone;
        ButtonAppearance.FillHover.BorderColor :=  $00505050;
        ButtonAppearance.FillHover.GradientMirrorType := gtVertical;

        ButtonAppearance.ArrowColor := $00232425;
        ButtonAppearance.ArrowColorHover := $001E1F20;
        ButtonAppearance.ArrowColorDown := $00232425;

      end;
      tsOffice2019Black:
      begin
        ButtonAppearance.FillHover.Color := $00686868;
        ButtonAppearance.FillHover.ColorTo := clNone;
        ButtonAppearance.FillHover.ColorMirror := clNone;
        ButtonAppearance.FillHover.ColorMirrorTo := clNone;
        ButtonAppearance.FillHover.BorderColor :=  $00B5B5B5;
        ButtonAppearance.FillHover.GradientMirrorType := gtVertical;
...
procedure TAdvSmoothComboBox.SetUIStyle(const Value: TTMSStyle);
begin
