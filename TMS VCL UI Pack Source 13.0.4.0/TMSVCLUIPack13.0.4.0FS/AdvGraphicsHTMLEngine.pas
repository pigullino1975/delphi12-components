...
begin
  ...
  // remove all tags
  while (TAdvUtils.VarPos('<',s,TagPos)>0) do
  begin
    PTP := TagPos;
    if (TAdvUtils.VarPos('>',s,TagPos) > PTP) then
    begin
...
begin
  ...
  if WasHTML and not IsHTML then
    s := TAdvUtils.UnFixMarkup(s, false);
...
var
  ...
  begin
    ...
    for i := ofs to length(aval) do
      begin
        if not ((aval[i{$IFDEF ZEROSTRINGINDEX} -1 {$ENDIF}] >= '0') and (aval[i{$IFDEF ZEROSTRINGINDEX} -1 {$ENDIF}] <= '9')) then
        begin
          Result := i;
          break;
        end;

        if (dohex and not ((aval[i{$IFDEF ZEROSTRINGINDEX} -1 {$ENDIF}] >= 'A') and (aval[i{$IFDEF ZEROSTRINGINDEX} -1 {$ENDIF}] <='F')))
          and (dohex and not ((aval[i{$IFDEF ZEROSTRINGINDEX} -1 {$ENDIF}] >= 'a') and (aval[i{$IFDEF ZEROSTRINGINDEX} -1 {$ENDIF}] <='f'))) then
        begin
          Result := i;
          break;
        end;
...
var
  ...
  OldPenKind: TAdvGraphicsStrokeKind;
  ...
  begin
    ...
    begin
      ...
      begin
        if sub[j{$IFDEF ZEROSTRINGINDEX} -1 {$ENDIF}] = s[i{$IFDEF ZEROSTRINGINDEX} -1 {$ENDIF}] then
        begin
          Result := i;
          Exit;
        end;
      ...
      begin
                  ...
                  begin
                    ...
                    begin
                      ...
                      begin
                        ...
                        begin
                          bmp := TAdvBitmap(PictureContainer.FindBitmap(Prop));
                        end;
                        ...
                        begin
                          ...
                          begin
                            ...
                            begin
                              ...
                              begin
                                AGraphics.DrawBitmap(RectF(Round(fr.left+bmpx),Round(fr.top+bmpy), Round(fr.Left + bmpx) + bmp.Width,Round(fr.top+bmpy) + bmp.Height), bmp);
                      ...
                      if (Pos('BGTOPLEFT', TagProp) > 0) then
                        begin
                          ...
                          begin
                            if not IsBitmapEmpty(bmp) and (bmp.Width > 0) and (bmp.Height > 0) then
                            begin
                              AGraphics.DrawBitmap(RectF(Round(fr.Left), Round(fr.Top + bmpy),Round(fr.Left) + bmp.Width ,Round(fr.Top) + bmp.Height), bmp);
                            end;
                          end;
                      ...
                      if (Pos('BGTOPRIGHT', TagProp) > 0) then
                        begin
                          ...
                          begin
                            if not IsBitmapEmpty(bmp) and (bmp.Width > 0) and (bmp.Height > 0) then
                            begin
                              AGraphics.DrawBitmap(RectF(Round(fr.Right) - bmp.Width, Round(fr.Top + bmpy), Round(fr.Right), Round(fr.Top) + bmp.Height ), bmp);
                            end;
                          end;
          ...
          'I':begin
                ...
                begin
                  ...
                  begin
                    ...
                    if Pos('DATA:',Uppercase(Prop)) > 0 then
                    begin
                      if Pos('BASE64,', Uppercase(Prop)) > 0 then
                      begin
                        Delete(Prop, 1, Pos('BASE64,', Uppercase(Prop)) + 6);
                        bmp := TAdvBitmap.Create;
                        TAdvUtils.LoadBitmapFromBase64(Prop, bmp);
                        bmpcreated := true;
                      end;
                    end;
                      ...
                      begin
                        ...
                        begin
                          ...
                          begin
                            ...
                            begin
                              AGraphics.DrawBitmap(RectF(Round(ir.Left),Round(ir.Top),Round(ir.Left) + imgw,Round(ir.Top) + imgh), bmp);
                            end
                            else
                            if (TagWidth > 0) and (TagHeight = 0) then
                            begin
                              AGraphics.DrawBitmap(RectF(Round(ir.Left),Round(ir.Top),Round(ir.Left) + TagWidth,Round(ir.Top) + TagHeight),bmp);
                            end
                            else
                            begin
                              AGraphics.DrawBitmap(RectF(Round(ir.Left),Round(ir.Top),Round(ir.Left) + bmp.Width, Round(ir.Top) + bmp.Height),bmp);
                            end;
                          end;
          ...
          'L':begin
                ...
                begin
                  ...
                  begin
                    ...
                    fs := AGraphics.Font.Style;
                    AGraphics.Font.Style := [];

                    if Odd(ListIndex) then
                      AGraphics.DrawText(cr, {$IFDEF MSWINDOWS}Chr($2022){$ELSE}'•'{$ENDIF}, False, gtaLeading, gtaCenter, gttNone, 0, -1, -1{$IFNDEF LIMITEDGRAPHICSMODE}, False{$ENDIF})
                    else
        ...
        'F':begin
              ...
              begin
                ...
                if (TAdvUtils.VarPos('STYLE',TagProp,TagPos)>0) then
                begin
                  Prop := Copy(TagProp,TagPos+5,Length(TagProp));
                  Prop := Copy(Prop,Pos('=',Prop)+1,Length(Prop));
                  Prop := Copy(Prop,Pos('"',Prop)+1,Length(Prop));
...
begin
  ...
  blnkKind := AGraphics.Fill.Kind;
  oldPenKind := AGraphics.Stroke.Kind;
  BGColor := gcNull;
  BkColor := gcNull;
  ParaColor := gcNull;
  isPara := False;
  ...
  if Pos('&',s) > 0 then
  begin
    s :=  TAdvUtils.UnFixMarkup(s, true, true);
    repeat
      Foundtag := False;
  ...
  AGraphics.Stroke.Kind := OldPenKind;
  AGraphics.Fill.Kind := blnkKind;
