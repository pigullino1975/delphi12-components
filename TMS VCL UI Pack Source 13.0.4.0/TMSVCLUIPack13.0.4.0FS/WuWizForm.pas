...
begin
  ...
  try
    ctrlres := WebUpdate.StartConnection;
  finally
    Screen.Cursor := crDefault;
  end;

  if (ctrlres = WU_SUCCESS) then
  begin
    Cursor := crDefault;
    if (WebUpdate.UpdateType = ftpUpdate) {$IFDEF DELPHI2007_LVL} or (WebUpdate.UpdateType = sftpUpdate){$ENDIF} then
    ...
    try
      ctrlres := WebUpdate.GetControlFile;
    finally
      Screen.Cursor := crDefault;
    end;
