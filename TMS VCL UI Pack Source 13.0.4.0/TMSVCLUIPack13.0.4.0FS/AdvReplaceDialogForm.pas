...
type
  ...
  private
    ...
    FMemoFindVisible: Boolean;
    FMemoReplaceVisible: Boolean;
...
begin
  ...
  begin
    i := FFindList.IndexOf(FindText);

    if i>0 then
      FFindList.Move(i,0)
    else
      if i<0 then
        FFindList.Insert(0, FindText);

    i := FReplaceList.IndexOf(ReplaceText);
    if i>0 then
      FReplaceList.Move(i,0)
    else
      if i<0 then
        FReplaceList.Insert(0, FindText);
  end;
  FVisible := false;
...
begin
  ...
  frm.FMemoFindVisible := fdovFindMemo in VisibleOptions;
  frm.FMemoReplaceVisible := fdovReplaceMemo in VisibleOptions;
...
//  frm.Memo2.Visible := fdovReplaceMemo in VisibleOptions;
  ...
  DPIScale := GetDPIScale(frm, frm.Canvas);

  if frm.DirGroup.Visible then
  begin
    if DPIScale > 1 then
      frm.Height := Round(236 * DPIScale)
    else
    begin
      DialogUnits := GetAveCharSize(frm.Canvas);
      frm.Height := MulDiv(145, DialogUnits.Y, 8); //236
    end;
  end
  else
  begin
    if DPIScale > 1 then
      frm.Height := Round(185 * DPIScale)
    else
    begin
      DialogUnits := GetAveCharSize(frm.Canvas);
    frm.Height := MulDiv(114, DialogUnits.Y, 8); //185
    end;
  end;
  ...
  AdaptFormHeightToVisibleControls(frm);
...
begin
  ...
  if not More then
  begin
    Label2.Visible := FMemoFindVisible;
    Memo1.Visible := FMemoFindVisible;
    Label4.Visible := FMemoReplaceVisible;;
    Memo2.Visible := FMemoReplaceVisible;
    Height := heightopen;
