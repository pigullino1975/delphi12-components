...
const
  ...
  // 2.3.1.0 : New : Selection colors for Office 2019 styles in THTMLTreeView
  // 2.3.2.0 : New : UIStyle property for Office 2019 look
  //         : Improved : On creation check for enabled AdvFormStyler
  // 2.3.2.1 : Fixed : ParentFont kept true on initialization
  // 2.3.2.2 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // 2.3.2.3 : Fixed : ItemHeight size on per monitor dpi
  // 2.3.2.4 : Fixed : Issue with HTML rendering in combination with per monitor DPI changing
  // 2.4.0.0 : New : Support for Emoji
...
type
  ...
  protected
    ...
    procedure MeasureItemEx(Index: Integer; var Height: Integer; ADPIscale: Single);
...
uses
  Extctrls, ShellApi, CommCtrl,Forms, AdvHTML
  {$IFDEF DELPHIXE2_LVL}
  ,VCL.Themes
  {$ENDIF}
  ;
...
begin
  ...
  HTMLDrawEx(Canvas,Items[Index],r,FImages,0,0,-1,-1,FShadowOffset,False,False,False,(odSelected in State),True,False,not Ellipsis,FFormScaled,
             1.0,URLCol,clNone,clNone,FShadowColor,a,s,fa,xsize,ysize,hoverlink,mouselink,hr,FImageCache,FContainer,0,BidiMode, clHighlight, clHighlightText, [], FDPIScale);
...
procedure THTMLComboBox.MeasureItemEx(Index: Integer; var Height: Integer; ADPIscale: Single);
var
...
begin
  ...
  begin
    ...
    try
      ...
      Canvas.Font.Height := Round(Font.Height * ADPIScale);

      HTMLDrawEx(Canvas,Items[Index],r,FImages,0,0,-1,-1,FShadowOffset,True,True,False,True,True,False,not Ellipsis,
                 FFormScaled,1.0,FURLColor,clNone,clNone,FShadowColor,a,s,fa,xsize,ysize,hoverlink,mouselink,hr,FImageCache,FContainer,0,BidiMode,clHighlight, clHighlightText, [], FDPIScale);
...
begin
  ...
  begin
    ...
    if HTMLDrawEx(Canvas,Items[res],r,FImages,msg.XPos,msg.YPos,-1,-1,FShadowOffset,True,False,False,True,True,False,not Ellipsis,
             FFormScaled,1.0,FURLColor,clNone,clNone,FShadowColor,anchor,stripped,fa,xsize,ysize,hoverlink,mouselink,hr,FImageCache,FContainer,0, BidiMode, clHighlight, clHighlightText, [], FDPIScale) then
    begin
      ReleaseDC(Handle,Canvas.Handle);
      Canvas.Free;
  ...
  begin
    ...
    try
      if HTMLDrawEx(Canvas,Items[res],r,FImages,X,Y,-1,-1,FShadowOffset,True,False,False,True,True,False,not Ellipsis, FFormScaled,
        1.0,FURLColor,clNone,clNone,FShadowColor,a,s,fa,xsize,ysize,hoverlink,mouselink,hr,FImageCache,FContainer,0,BidiMode, clHighlight, clHighlightText, [], FDPIScale) then
      begin
      ...
      begin
        if (Cursor <> crDefault) or (a <> fOldAnchor) then
          if Assigned(FAnchorExit) then
  ...
  begin
    ...
    try
      HTMLDrawEx(Canvas,Items[index],r,FImages,0,0,-1,-1,FShadowOffset,True,True,False,True,True,False,not Ellipsis, FFormScaled,
        1.0,FURLColor,clNone,clNone,FShadowColor,a,s,fa,xsize,ysize,hoverlink,mouselink,hr,FImageCache,FContainer,0,BidiMode,clHighlight, clHighlightText, [], FDPIScale);
    finally
      ReleaseDC(Handle,Canvas.Handle);
      Canvas.Free;
    end;
