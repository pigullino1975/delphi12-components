...
const
  ...
  // v1.6.10.3 : Improved : Transparency for tsRoundRect thumb style
  //           : Improved : Handling of PageSize > 0
  //           : Improved : Position of trackhint
  //           : Fixed : Issue with showing tickmarks when ShowImageAtSteps = false
  // v1.6.10.4 : Fixed : Rare issue with combining parent control hint and trackhint
  //           : Improved : Thumb position precision
  //           : Improved : Thumb paging with mouse
  // v1.7.0.0 : New : Support for Office 2019 styles
  // v1.7.0.1 : Fixed : Design-time style painting
  // v1.7.0.2 : Fixed : Office 2019 style was not only forced on creation
  // v1.7.1.0 : New : TickMarks property ShowFirstLastStep and ShowTextAtSteps
  //          : Fixed : Issue with painting when put on a DoubleBuffered parent control
  // v1.7.2.0 : New : UIStyle property for office look
  //          : Fixed : TrackLabel and TickMark font color changes with dark office style
  //          : Improved : On creation check for enabled AdvFormStyler
  // v1.7.2.1 : Fixed : Consistency in Office 2019 gray style label colors
  // v1.7.2.2 : Fixed : Issue with OnChanged handled for key interaction
  // v1.7.2.3 : Fixed : Issue with Transparent = true on parent controls with gradients
  // v1.7.3.0 : New : Slider.Indent property added to set more indent to allow longer tickmark text
  // v1.7.3.1 : Fixed : Minus button center drawing for vertical orientation
  // v1.7.3.2 : Improved : Made CanMovePositionRight virtual method
  //          : Fixed : Issue with detecting whether position can move left or right
...
type
  ...
  private
    ...
    FShowFirstLastStep: Boolean;
    FShowTextAtSteps: Integer;
    procedure SetColor(const Value: TColor);
    procedure SetPicture(const Value: TGDIPPicture);
    procedure PictureChanged(Sender: TObject);
    ...
    procedure SetShowTextAtSteps(const Value: Integer);
    procedure SetShowFirstLastStep(const Value: Boolean);
  protected
    procedure Changed;
  public
    constructor Create;
  ...
  published
    ...
    property ShowTextAtSteps: Integer read FShowTextAtSteps write SetShowTextAtSteps default 0;
    property ShowFirstLastStep: Boolean read FShowFirstLastStep write SetShowFirstLastStep default false;
  ...
  TTrackbarDir = (tdInit, tdUp, tdDown);
  ...
  protected
    ...
    function CanMovePositionRight(Value: Integer): Boolean; virtual;
...
begin
  ...
  Canvas.Font.Height := Round(ADPIScale * f.Height);
...
begin
  ...
  Result.Left := Result.Left + Round(Slider.Indent * FDPIScale);
  Result.Right := Result.Right - Round(Slider.Indent * FDPIScale);
end;
...
begin
  ...
  begin
    ...
    if (Parent is TWinControl) then
     TWinCtrl(Parent).PaintCtrls(Canvas.Handle, nil);
...
begin
  ...
  begin
    ...
    begin
      ...
      if Transparent and (Parent is TWinControl) then
        Clr := (Parent as TWinControl).Brush.Color;
...
begin
  ...
  if Assigned(FOnDrawTick) or TickMark.ShowText or (Max > Mx) or (TickMark.ShowImageAtSteps > 0) then
    Mx := Max;
    ...
    begin
      ...
      begin
        ...
        begin
          if TickMark.ShowImageAtThumbOnly then
            DefaultDraw := (i = Position);
          if DefaultDraw and (TickMark.ShowImageAtSteps > 1) and not (TickMark.ShowFirstLastStep and ((i = Min) or (i = Mx))) then
            DefaultDraw := ((i mod TickMark.ShowImageAtSteps) = 0);
        end;
        ...
        // Draw text
        if FTickMark.ShowText and (((FTickMark.ShowTextAtSteps = 0) and DefaultDraw) or ((FTickMark.ShowTextAtSteps > 0) and ((i mod TickMark.ShowTextAtSteps) = 0))
          or (TickMark.ShowFirstLastStep and ((i = Min) or (i = Mx)))) then
        begin
          ...
          Canvas.Font.Height := Round(FDPIScale * TickMark.Font.Height);
...
begin
  ...
  R := TrackLabelRect;
  Canvas.Font.Assign(FTrackLabel.Font);
  Canvas.Font.Height := Round(FTrackLabel.Font.Height * FDPIScale);
...
begin
  ...
  case Key of
    vk_Prior:	if FPageSize <> 0 then Position := (FPosition div FPageSize) * FPageSize + FPageSize;
    vk_Next: if FPageSize <> 0 then Position := (FPosition div FPageSize) * FPageSize - FPageSize;
    vk_End:
    begin
...
begin
  ...
  begin
    ...
    if (FPageSize <> 0) then
    begin
      if (X < PtX) and (FPageDir in [tdInit, tdDown]) then
      begin
        Result := (FPosition div FPageSize) * FPageSize - FPageSize;
        FPageDir := tdDown;
      end;
      if (X > PtX) and (FPageDir in [tdInit, tdUp]) then
      begin
        Result := (FPosition div FPageSize) * FPageSize + FPageSize;
        FPageDir := tdUp;
      end;
...
procedure TCustomTrackBar.SetUIStyle(const Value: TTMSStyle);
begin
 ...
 case AStyle of
   ...
   tsOffice2016Black:
      begin
        ...
        Slider.BorderColor := $444444;
        TickMark.Color := Slider.BorderColor;
   ...
   tsOffice2019White:
      begin
        ...
        Thumb.BorderColor := $00ABABAB;
        Thumb.Gradient := ggVertical;
        Thumb.GradientMirror := ggVertical;

        Thumb.ColorHot := $00F2E1D5;
        Thumb.ColorHotTo := $00F2E1D5;
        Thumb.ColorMirrorHot := clNone;
        Thumb.ColorMirrorHotTo := clNone;
        Thumb.BorderColorHot := $00F2E1D5;

        Thumb.ColorDown := $00E3BDA3;
        Thumb.ColorDownTo := $00E3BDA3;
        Thumb.ColorMirrorDown := clNone;
        Thumb.ColorMirrorDownTo := clNone;
        Thumb.BorderColorDown := $00E3BDA3;

        Buttons.Color := clWhite;
        Buttons.ColorTo := clWhite;
        Buttons.ColorMirror := clNone;
        Buttons.ColorMirrorTo := clNone;
        Buttons.BorderColor := $00ABABAB;
        Buttons.Gradient := ggVertical;
        Buttons.GradientMirror := ggVertical;

        Buttons.ColorHot := $00F2E1D5;
        Buttons.ColorHotTo := $00F2E1D5;
        Buttons.ColorMirrorHot := clNone;
        Buttons.ColorMirrorHotTo := clNone;
        Buttons.BorderColorHot := $00F2E1D5;

        Buttons.ColorDown := $00E3BDA3;
        Buttons.ColorDownTo := $00E3BDA3;
        Buttons.ColorMirrorDown := clNone;
        Buttons.ColorMirrorDownTo := clNone;
        Buttons.BorderColorDown := $00E3BDA3;

        Slider.BorderColor := $00ABABAB;
        TickMark.Color := Slider.BorderColor;
        TickMark.Font.Color := $00444648;
        TrackLabel.Font.Color := $00444648;
      end;
   tsOffice2019Gray:
      begin
        Thumb.Color := $00B8BBBE;
        Thumb.ColorTo := $00B8BBBE;
        Thumb.ColorMirror := clNone;
        Thumb.ColorMirrorTo := clNone;
        Thumb.BorderColor := $00808080;
        Thumb.Gradient := ggVertical;
        Thumb.GradientMirror := ggVertical;

        Thumb.ColorHot := $00969696;
        Thumb.ColorHotTo := $00969696;
        Thumb.ColorMirrorHot := clNone;
        Thumb.ColorMirrorHotTo := clNone;
        Thumb.BorderColorHot := $00969696;

        Thumb.ColorDown := $00666666;
        Thumb.ColorDownTo := $00666666;
        Thumb.ColorMirrorDown := clNone;
        Thumb.ColorMirrorDownTo := clNone;
        Thumb.BorderColorDown := $00666666;

        Buttons.Color := $00B8BBBE;
        Buttons.ColorTo := $00B8BBBE;
        Buttons.ColorMirror := clNone;
        Buttons.ColorMirrorTo := clNone;
        Buttons.BorderColor := $00808080;
        Buttons.Gradient := ggVertical;
        Buttons.GradientMirror := ggVertical;

        Buttons.ColorHot := $00969696;
        Buttons.ColorHotTo := $00969696;
        Buttons.ColorMirrorHot := clNone;
        Buttons.ColorMirrorHotTo := clNone;
        Buttons.BorderColorHot := $00969696;

        Buttons.ColorDown := $00666666;
        Buttons.ColorDownTo := $00666666;
        Buttons.ColorMirrorDown := clNone;
        Buttons.ColorMirrorDownTo := clNone;
        Buttons.BorderColorDown := $00666666;

        Slider.BorderColor := $00808080;
        TickMark.Color := Slider.BorderColor;
        TickMark.Font.Color := $00B8BBBE;
        TrackLabel.Font.Color := clWhite;
      end;
   tsOffice2019Black:
      begin
        Thumb.Color := $00303132;
        Thumb.ColorTo := $00303132;
        Thumb.ColorMirror := clNone;
        Thumb.ColorMirrorTo := clNone;
        Thumb.BorderColor := $00686868;
        Thumb.Gradient := ggVertical;
        Thumb.GradientMirror := ggVertical;

        Thumb.ColorHot := $6A6A6A;
        Thumb.ColorHotTo := $6A6A6A;
        Thumb.ColorMirrorHot := clNone;
        Thumb.ColorMirrorHotTo := clNone;
        Thumb.BorderColorHot := $6A6A6A;

        Thumb.ColorDown := $444444;
        Thumb.ColorDownTo := $444444;
        Thumb.ColorMirrorDown := clNone;
        Thumb.ColorMirrorDownTo := clNone;
        Thumb.BorderColorDown := $444444;

        Buttons.Color := $00303132;
        Buttons.ColorTo := $00303132;
        Buttons.ColorMirror := clNone;
        Buttons.ColorMirrorTo := clNone;
        Buttons.BorderColor := $00686868;
        Buttons.Gradient := ggVertical;
        Buttons.GradientMirror := ggVertical;

        Buttons.ColorHot := $00686868;
        Buttons.ColorHotTo := $00686868;
        Buttons.ColorMirrorHot := clNone;
        Buttons.ColorMirrorHotTo := clNone;
        Buttons.BorderColorHot := $00686868;

        Buttons.ColorDown := $005C5E60;
        Buttons.ColorDownTo := $005C5E60;
        Buttons.ColorMirrorDown := clNone;
        Buttons.ColorMirrorDownTo := clNone;
        Buttons.BorderColorDown := $005C5E60;

        Slider.BorderColor := $00686868;
        TickMark.Color := Slider.BorderColor;
        TickMark.Font.Color := clWhite;
        TrackLabel.Font.Color := clWhite;
      end;
...
begin
  ...
  if Assigned(Parent) and Parent.ShowHint then
    Exit;
...
procedure TTrackBarTick.SetShowTextAtSteps(const Value: Integer);
begin
  if (FShowTextAtSteps <> Value) then
  begin
    FShowTextAtSteps := Value;
    Changed;
  end;
end;
...
procedure TTrackBarTick.SetShowFirstLastStep(const Value: Boolean);
begin
  if(FShowFirstLastStep <> Value) then
  begin
    FShowFirstLastStep := Value;
    Changed;
  end;
end;
...
begin
  ...
  begin
    ...
    FIndent := (Source as TTrackBarSlider).Indent;
  end
  else
    inherited;
end;
...
procedure TTrackBarSlider.SetIndent(const Value: integer);
begin
...
begin
  ...
  begin
    case Key of
      vk_Prior: if FPageSize <> 0 then PositionRight := (FPositionRight div FPageSize) * FPageSize + FPageSize;
      vk_Next: if FPageSize <> 0 then PositionRight := (FPositionRight div FPageSize) * FPageSize - FPageSize;
      vk_End:
      begin
        if (FOrientation = trVertical) then
