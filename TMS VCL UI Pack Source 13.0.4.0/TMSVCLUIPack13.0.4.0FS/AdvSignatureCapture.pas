...
uses
  ...
  Generics.Collections, pngimage, jpeg, Dialogs, Forms, AdvGDIP
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF}
  {$IFDEF FREEWARE}
...
const
  ...
  // 1.0.1.1  : Fixed : Issue when switching pen color / width at runtime
  // 1.0.2.0  : New : SaveToBitmap() method added
  // 1.0.3.0  : Improved : Save to image file will not draw clear marker & text
  // 1.1.0.0  : New : OnClear event added
  // 1.1.0.1  : Fixed : Issue when starting drawing from clear
  // 1.1.1.0  : New : Property CompressionQuality & CompressionLevel public properties added
  // 1.1.2.0  : Improved : When Text = empty, draw no line at bottom of signature capture
  // 1.1.3.0  : New : Exposed SignaturePoints as public property
  // 1.1.4.0  : New : Background picture property added to TAdvSignatureCapture
  // 1.1.4.1  : Fixed : Issue with line drawing when start drawing on clear button
  // 1.1.5.0  : New : Anti-aliased signature drawing
...
type
  ...
  TSignaturePointsList =  TList<TStoredPoint>;
  ...
  private
    ...
    FText: string;
    FDrawing: Boolean;
    FSignaturePoints: TSignaturePointsList;
    FBorderColor: TColor;
    ...
    procedure SetBackgroundStretch(const Value: boolean);
  protected
  ...
  public
    ...
    property SignaturePoints: TSignaturePointsList read FSignaturePoints;
    {$IFDEF DELPHIXE_LVL}
    property CompressionQuality: TJPEGQualityRange read FCompressionQuality write FCompressionQuality default 100;
    property CompressionLevel: TCompressionLevel read FCompressionLevel write FCompressionLevel default 9;
    {$ENDIF}
  published
    ...
    property BackgroundStretch: boolean read FBackgroundStretch write SetBackgroundStretch default false;
...
procedure TAdvSignatureCapture.BackgroundChanged(Sender: TObject);
begin
  Invalidate;
end;
...
procedure TAdvSignatureCapture.DoClear;
begin
...
begin
  ...
  begin
    ...
    begin
      gppen := TGPPen.Create(MakeColor(255, drawPoint.PenColor), drawPoint.PenWidth);

      gp.DrawLine(gpPen, origx, origy, drawpoint.X, drawPoint.Y);
...
begin
  ...
  if FClearDown and FDown and not PtInRect(GetClearRect,Point(X,Y)) then
  begin
    FDrawing := true;
    FOrigX := X;
    FOrigY := Y;
  ...
  begin
    ...
    gppen := TGPPen.Create(MakeColor(255, FPen.Color), FPen.Width);
    gp.DrawLine(gpPen, FOrigx, FOrigy, X,Y);
    ...
    newPoint.DrawLine := (FSignaturePoints.Count > 0) and not FNewLine;
    newPoint.PenWidth := FPen.Width;
    newPoint.PenColor := FPen.Color;
...
begin
  ...
  if Assigned(Background) and Assigned(Background.Graphic) and not (Background.Graphic.Empty) then
  begin
    if BackgroundStretch then
      Canvas.StretchDraw(Rect(0,0,Width,Height), Background.Graphic)
    else
      Canvas.Draw(0,0,Background.Graphic);
  end;

  if not FPaintSigOnly and (Text <> '') then
  begin
  ...
  if Assigned(ClearSig.Picture.Graphic) and not ClearSig.Picture.Graphic.Empty and not (ClearSig.Position = pNone) and not FPaintSigOnly then
  begin
    ccr := GetClearRect;
...
procedure TAdvSignatureCapture.SaveToBitmap(ABitmap: TBitmap);
var
  rectangle: TRect;
begin
  Abitmap.Width := Width;
  ...
  ABitmap.Canvas.CopyRect(rectangle, Canvas, rectangle);
...
begin
  ...
  begin
    ...
    {$IFDEF DELPHIXE_LVL}
    (img as TJPEGImage).CompressionQuality := CompressionQuality;
    {$ENDIF}
  end;
  ...
  begin
   ...
   {$IFDEF DELPHIXE_LVL}
   (img as TPngImage).CompressionLevel := CompressionLevel;
   {$ENDIF}
  end;
...
procedure TAdvSignatureCapture.SetBackground(const Value: TPicture);
begin
  FBackground.Assign(Value);
  Invalidate;
end;

procedure TAdvSignatureCapture.SetBackgroundStretch(const Value: boolean);
begin
  if (FBackgroundStretch <> Value) then
  begin
    FBackgroundStretch := Value;
    Invalidate;
  end;
end;
