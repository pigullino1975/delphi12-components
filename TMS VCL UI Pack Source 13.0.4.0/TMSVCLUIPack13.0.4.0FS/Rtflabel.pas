...
const
  ...
  // v1.3.3.0 : Fixed : Per monitor high DPI issue
...
type
  ...
  protected
    ...
    {$ENDIF}
    procedure ChangeFontSize(M,D: Integer);
  public
...
begin
  ...
  if Assigned(frm) then
    ChangeFontSize(96,frm.PixelsPerInch);
  {$ENDIF}
...
procedure TRTFLabel.ChangeFontSize(M, D: Integer);
var
  i: integer;
  sz: integer;
  ms: TStringStream;
begin
  ...
  try
    ...
    for i := 0 to Length(FRichEdit.Lines.Text) - 1 do
    begin
      FRichEdit.SelStart := i;
      FRichEdit.SelLength := 1;
      sz := FRichedit.SelAttributes.Size;
      FRichedit.SelAttributes.Size := MulDiv(sz, M, D);
    end;
...
{$IFNDEF DELPHIXE10_LVL}
procedure TRTFLabel.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TRTFLabel.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
  ...
  {$ENDIF}
  begin
    FRichedit.Font.Size := MulDiv(FRichEdit.Font.Size, M, D);
