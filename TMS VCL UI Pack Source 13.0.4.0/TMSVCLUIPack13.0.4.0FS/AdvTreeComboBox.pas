...
const
  ...
  // 1.2.4.2 : Fixed : Dropdown alignment
  //         : Fixed : Display on wrong position when there is a load delay
  // 1.2.4.3 : Fixed : Issue with selected nodes & switching VCL styles at runtime
  // 1.2.4.4 : Fixed : DPI calculation when parent form is not Scaled
  // 1.2.4.5 : Fixed : Issue with EditorEnabled = false and shortcut keys
  // 1.2.4.6 : Fixed : Issue with setting selected node before control handle is allocated
  // 1.2.4.7 : Fixed : Issue with LabelFont setting at design-time
  // 1.2.5.0 : New : Property FindText added to control whether the node is looked up or not
  // 1.2.6.0 : New : Event OnChanging added
  // 1.2.6.1 : Fixed : Issue with multi-monitor high DPI handling
  // 1.2.6.2 : Fixed : Issue with handling ESC when dropdown is closed
  // 1.2.6.3 : Fixed : Issue with return key on modal forms
  // 1.2.6.4 : Fixed : Paint issue when ShowSelectedImage = true
  // 1.2.6.5 : Fixed : High DPI handling for TreeView dropdown font
  // 1.2.7.0 : Improved : When DropWidth = 0, the dropdown width automatically adapts to the control width
...
type
  ...
  published
    ...
    property FindText: Boolean read FFindText write FFindText default true;
...
{$IFDEF DELPHIXE10_LVL}
procedure TAdvTreeComboBox.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ELSE}
procedure TAdvTreeComboBox.ChangeScale(M, D: Integer);
{$ENDIF}
begin
  inherited;
  fTreeViewFont.Height := MulDiv(FTreeViewFont.Height, M, D);
end;

procedure TAdvTreeComboBox.ClickDefault;
var
  frm: TCustomForm;
  i: integer;
begin
  ...
  begin
    ...
    begin
      if (frm.Controls[i] as TButton).Default then
        (frm.Controls[i] as TButton).Click;
    end;
  end;
end;
...
begin
  if (csDesigning in ComponentState) or (csLoading in ComponentState) and ParentFont then
  begin
...
begin
  ...
  begin
    ...
    if (Text <> '') and (FSelectedNode = nil) then
    begin
      FTreeView.Select(GetNodeByText(FTreeView, Text));
    end;
  end;
end;
...
begin
  ...
  case FDropPosition of
    dpAuto: begin
        ...
        begin //Up
          FdropTreeForm.Left := P.x - d;
          FdropTreeForm.Top := p.y - Round(FDPIScale * FDropHeight);
        end
        else
        begin //Down
          FdropTreeForm.Left := P.x - d;
          FdropTreeForm.Top := p.y + Height - d;
        end;
      end;
    dpDown: begin
        FdropTreeForm.Left := P.x - d;
  ...
  FTreeView.OnChanging := TreeViewChanging; // Please leave this here, otherwise procedure FindtextinNode must be modified
end;
...
procedure TAdvTreeComboBox.TreeViewChanging(Sender: TObject; Node: TTreeNode;
  var AllowChange: Boolean);
begin
  if Assigned(OnChanging) then
    OnChanging(Self, Node, AllowChange);
end;
