...
function HTMLDrawEx(Canvas:TCanvas; s:string; fr:TRect;
                    ...
                    ic: THTMLPictureCache; pc: TPictureContainer; WinHandle: THandle; DPIScale: single; BidiMode : TBidiMode = bdLeftToRight): Boolean;
...
begin
  ...
  if not Calc then
  begin
    LCanvas := TDirect2DCanvas.Create(ACanvas, Rect(0,0,1000,100));
...
function TagReplaceString(const Srch,Repl:string;var Dest:string; DoCase: boolean = false):Boolean;
var
  i: Integer;
begin
  if DoCase then
...
function UnFixMarkupEngine(su:string;SpecialChars: boolean = true):string;
var
  i: integer;
begin
  while Pos('&quot;',su) > 0 do
  ...
  begin
    ...
    begin
      while Pos(string(HTMLSpecialChar[i]),su) > 0 do
      begin
        TagReplacestring(string(HTMLSpecialChar[i]),string(HTMLEncodedChar[i]),su,true);
      end;
    end;
  end;
...
var
  ...
  bgcolor,paracolor,hvrcolor,hvrfntcolor,pencolor,blnkcolor,hifcol,hibcol,ColL,ColB: TColor;
  ...
  OrgPenWidth: Integer;
  OLIST: boolean;
  OListIndexStr,LType,LName: string;
  floatrect: TRect;
  floatpt: TPoint;
  txtfloat,txtfloatbefore: TFloatStyle;
  imgfloat: TFloatStyle;
  scale,lhf: single;
  emh,LineSpacing: integer;
  CFontName,CVal: string;
  CFontColor,CCol: TColor;
  CFontSize,CSize: integer;
  ...
  ///                  cr.left := fr.left + Indent;
                ...
                begin
                  ...
                  begin
                    ...
                    if (CVal <> '') and not Invisible then
                    begin
          ...
          'I':begin
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      if Calc then
                        Indent := Round(Indent * resfactor * 3)
                      else
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    Canvas.Font.Size := Round(IStrToInt(Prop) * scale);
                    {$ENDIF}
                    {$IFDEF DELPHIXE11_LVL}
                    Canvas.Font.Size := Round(IStrToInt(Prop));
                    {$ENDIF}
                  end;
                  end;
                end;
