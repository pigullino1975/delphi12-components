...
const
  ...
  // v1.0.1.0 : Improved : Support for High DPI: Scrollbar height
  // v1.1.0.0 : New : OnItemDblCick, OnDblClick event added
  //          : Fixed : Issue with popup menu over items
...
type
  ...
  TPictureItemList = class(TOwnedCollection)
  private
    { Private declarations }
  ...
  private
    ...
    FOnItemDblClick: TItemClickEvent;
    ...
    procedure SetPictures(const Value: TPictureItemList);
  ...
  published
    ...
    property Pictures: TPictureItemList read FPictures write SetPictures;
    ...
    property OnItemDblClick: TItemClickEvent read FOnItemDblClick write FOnItemDblClick;
    property OnKeyDown;
    property OnKeyPress;
    property OnKeyUp;
...
begin
  ...
  FOffsetX := 0;
  FItemIndex := 0;
  FPictures := TPictureItemList.Create(Self);
  FPictures.OnChange := PicturesChanged;
  FScroll := TScrollBar.Create(Self);
  FScroll.Parent := Self;
  FScroll.Height := Round(15 * GetDPIScale(Self, Canvas));
...
procedure TAdvPicturePane.WMLButtonDblClk(var Message: TWMLButtonDblClk);
var
  X,Y: integer;
  Pic: TPictureItem;
begin
  ...
  begin
    ...
    begin
      ...
      if Assigned(OnItemDblClick) then
        OnItemDblClick(Self, FItemIndex);
    end
    else
    begin
...
begin
  ...
  if not (csDesigning in ComponentState) and CanFocus and (Button = TMouseButton.mbLeft) then
  begin
    SetFocus;
    Pic := ItemAtXY(X, Y);
    if Assigned(Pic) then
...
begin
  inherited;
  FScroll.Height := Round(15 * GetDPIScale(Self, Canvas));
  FScroll.Top := Height - FScroll.Height;
  FScroll.Width := Width;
  Loaded;
end;
...
procedure TAdvPicturePane.SetPictures(const Value: TPictureItemList);
begin
...
begin
  inherited;
  FOwner := TPictureItemList(Collection).Owner as TAdvPicturePane;
...
function TPictureItemList.Add: TPictureItem;
begin
...
constructor TPictureItemList.Create(AOwner: TAdvPicturePane);
begin
  inherited Create(AOwner, TPictureItem);
end;

function TPictureItemList.GetItem(Index: Integer): TPictureItem;
begin
  Result := TPictureItem(inherited GetItem(Index));
end;

function TPictureItemList.Insert(Index: Integer): TPictureItem;
begin
...
procedure TPictureItemList.SetItem(Index: Integer; Value: TPictureItem);
begin
...
procedure TPictureItemList.SetOnChange(const Value: TNotifyEvent);
begin
  FOnChange := Value;
end;

procedure TPictureItemList.Update(Item: TCollectionItem);
begin
  inherited;
