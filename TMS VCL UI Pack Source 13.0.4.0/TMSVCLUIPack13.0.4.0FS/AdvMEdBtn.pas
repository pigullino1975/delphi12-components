...
uses
  ...
  Forms, Graphics, Buttons, Dialogs, Menus, AdvEdit, AEBXPVS, AdvStyleIF
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF}
  ;
...
const
  ...
  // v1.3.1.0 : New : UIStyle property for TMSStyles
  //          : New : Support for high DPI
  //          : Fixed : ParentFont kept true on initialization
  // v1.3.1.1 : Fixed : Issue with button glyph painting
...
{ TAdvMaskEditBtn }
  ...
  TAdvMaskEditBtn = class(TAdvMaskEdit, ITMSStyle)
  private
    FDesignTime: Boolean;
    FDPIScale: Single;
    FUnitSize : integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      Canvas.Brush.Color := TAdvMaskEdit(Owner.Owner).Color;
    end
    else
  ...
  begin
    Canvas.Brush.Color := TAdvMaskEditBtn(Owner.Owner).Color;
    Canvas.Pen.Color := Canvas.Brush.Color;
    Canvas.Rectangle(ClientRect);
    DrawContent;
  end
  else
  if Assigned(Owner) and (Owner is TEditButton) and (TEditButton(Owner).ButtonColor <> clNone) then
  begin
    Canvas.Brush.Color := TEditButton(Owner).ButtonColor;
...
begin
  if (FButton = nil) or (csLoading in ComponentState) then
    Exit;
  if (Owner is TAdvMaskEditBtn) then
    W := Round(TAdvMaskEditBtn(Owner).FDPIScale * FBWidth);
...
procedure TAdvMaskEditBtn.SetUIStyle(const Value: TTMSStyle);
begin
...
procedure TAdvMaskEditBtn.SetComponentStyle(AStyle: TTMSStyle);
begin
  inherited;
  ...
  if (FButton <> nil) then
   begin
    if NewStyleControls and Ctl3D then
      FButton.SetBounds(Width - Round(FDPIScale * FButton.FBWidth) - Dist - Offs, 1 + FlatCorr, Round(FDPIScale * FButton.FBWidth), Height - Dist)
    else
      FButton.SetBounds (Width - Round(FDPIScale * FButton.FBWidth) - Offs, 1, Round(FDPIScale * FButton.FBWidth), Height - 2);
    SetEditRect;
   end;
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvMaskEditBtn.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvMaskEditBtn.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
  FDPIScale := GetDPIScale(Self, FButton.FButton.Canvas);
...
procedure TAdvMaskEditBtn.SetButtonBorderColor(const Value: TColor);
begin
  FButtonBorderColor := Value;
  if Assigned(FButton) then
    FButton.ButtonBorderColor := FButtonBorderColor;
end;

procedure TAdvMaskEditBtn.SetButtonColor(const Value: TColor);
begin
  FButtonColor := Value;
  if Assigned(FButton) then
...
procedure TAdvMaskEditBtn.SetButtonColorDown(const Value: TColor);
begin
...
procedure TAdvMaskEditBtn.SetButtonColorHot(const Value: TColor);
begin
...
procedure TAdvMaskEditBtn.SetButtonTextColor(const Value: TColor);
begin
...
procedure TAdvMaskEditBtn.SetButtonTextColorDown(const Value: TColor);
begin
...
procedure TAdvMaskEditBtn.SetButtonTextColorHot(const Value: TColor);
begin
...
procedure TAdvMaskEditBtn.SetButtonTransparent(const Value: boolean);
begin
