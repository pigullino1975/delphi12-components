...
type
  ...
  private
    ...
    {$IFDEF DELPHIXE4_LVL}
    function SaveTempFile(const DataObj: IDataObject; AIndex: integer; AFileName: string): string;
    function SomeFileContents(const DataObj: IDataObject; var Files: TStringList): integer;
    {$ENDIF}
  ...
  public
    ...
    function HasFileContents: boolean;
  ...
  TDropFormat = (dfText, dfFile, dfCol, dfRTF, dfURL, dfGridCells, dfURI, dfFileContents);
  ...
  public
    constructor Create;
    function DropText(pt:TPoint;s:string): boolean; virtual; abstract;
    ...
    procedure DropFileContents(pt:TPoint); virtual; abstract;
...
var
  ...
  CF_FileGroupDescriptor: integer;
  CF_FileGroupDescriptorW: integer;
...
{$IFDEF DELPHIXE4_LVL}
function TEnumFormats.SomeFileContents(const DataObj: IDataObject; var Files: TStringList): integer;
var
  tFMT  : TFORMATETC;
  tSTGM : TSTGMEDIUM;
...
begin
  ...
  begin
    ...
    begin
      StrPCopy(tFGD.fgd[i].cFileName,FileName);
      Files.Add(FileName);
    end;
  ...
  for i := 0 to Result - 1 do
  begin
    SaveTempFile(DataObj, i, Files[i]);
  end;
end;
{$ENDIF}
...
function TEnumFormats.HasFileContents: boolean;
begin
  Result := HasFormat(CF_FILECONTENTS) and (1<0);
end;
...
begin
  ...
  try
    FOk := (HasText and FAcceptText) or (HasFile and FAcceptFiles) or (HasRTF and RTFAware and FAcceptText)
      or (HasCol and FAcceptCols) or (HasURL and FAcceptURLs) or (HasURI and FAcceptURLs) or (HasGridCells) or (HasFileContents and FAcceptFiles);

    FDropFormats := [];
    if HasText then FDropFormats := [dfText];
    if HasFile then FDropFormats := FDropFormats + [dfFile];
    if HasFileContents then FDropFormats := FDropFormats + [dfFileContents];
  ...
  begin
    ...
    try
      ...
      begin
        ...
        {$IFDEF DELPHIXE4_LVL}
        if HasFileContents then
        begin
          SomeFileContents(DataObj, FFiles);
          DropFiles(pt, FFiles);
        end;
        {$ENDIF}
...
{$IFDEF DELPHIXE4_LVL}
function TEnumFormats.SaveTempFile(const DataObj: IDataObject;
  AIndex: integer; AFileName: string): string;
var
  tFMT  : TFORMATETC;
  tSTGM : TSTGMEDIUM;
  Filename : Widestring;
...
begin
  ...
  begin
    ...
    begin
      ...
      Filename := Path + SysUtils.Format('mail%d.msg',[AIndex]);
    ...
    begin
      if tSTGM.TYMED = TYMED_ISTREAM then
      begin
        OleStream := TOleStream.Create(ISTREAM(tSTGM.stm));
        try
