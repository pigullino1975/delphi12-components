...
const
  ...
  // 3.7.1.1 : Fixed : Issue with runtime TabAppearance.Font changes
  // 3.8.0.0 : New : Support for Office 2019 styles
  //         : Fixed : Button placement
  //         : Fixed : DPI change text height without styler and tab height.
  // 3.8.1.0 : New : Internal Office styler implemented in TAdvOfficeTabSet
  //         : Fixed : Performance of Begin- and Endupdate was broken in 3.8.0.0
  // 3.8.1.1 : Improved : Performance of drawing behavior with large number of tabs
  // 3.8.1.2 : Fixed : TabSettings.TextSelectedBold also works for other styles than Office 2019
  ...
  // 3.8.2.1 : Fixed : Issue with responsiveness of tabs after a hidden tab
  ...
  // 3.8.2.3 : Fixed : Issue with runtime VCL style changing
  // 3.8.2.4 : Fixed : DPI calculation when parent form is not Scaled
  // 3.8.2.5 : Improved : Closed button look in high DPI scenarios
  // 3.8.2.6 : Improved : cblChrome close button appearance in high DPI
  // 3.8.2.7 : Improved : Performance when many tabs are used
...
type
  ...
  TAdvCustomOfficeTabSet = class(TCustomControl, ITMSStyleEx, ITMSTones)
  private
...
procedure DrawVistaTab(Canvas: TCanvas; r: TRect; CFU, CTU, CFB, CTB, PC: TColor; GradientU,GradientB: TGDIPGradient;
   Enabled: Boolean; Shape: TAdvTabShape; Focus: Boolean; Rounding: TTabRounding; RotateLeftRight: Boolean; Direction: TTabPosition; Progress: TTabProgress);
var
  graphics : TGPGraphics;
...
begin
  ...
  begin
    ...
    GlowButtonAppearance := (Source as TCustomAdvOfficeTabSetStyler).GlowButtonAppearance;
  end
  else
    inherited Assign(Source);
end;
...
begin
  ...
  begin
    ...
    if FFirstVisibleTab >= 0 then
      FLastVisibleTab := TabIndex - 1;
    Exit;
  end
  else
  begin
    if (TabIndex < FFirstVisibleTab) or (FFirstVisibleTab = -1) then
      FFirstVisibleTab := TabIndex;
    if FLastVisibleTab < TabIndex then
      FLastVisibleTab  := TabIndex;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              GradColor := BlendColor(FColorHot, TabAppearance.BackGround.Color, FAdvOfficeTabs.Items[TabIndex].FStepHover);
              if (TabAppearance.BackGround.ColorTo <> clNone) then
                GradColorTo := BlendColor(FColorHotTo, TabAppearance.BackGround.ColorTo, FAdvOfficeTabs.Items[TabIndex].FStepHover)
              else
                GradColorTo := BlendColor(FColorHotTo, TabAppearance.BackGround.Color, FAdvOfficeTabs.Items[TabIndex].FStepHover);
              GradColorMirror := BlendColor(FColorMirrorHot, TabAppearance.BackGround.Color, FAdvOfficeTabs.Items[TabIndex].FStepHover);
              GradColorMirrorTo := BlendColor(FColorMirrorHotTo, TabAppearance.BackGround.Color, FAdvOfficeTabs.Items[TabIndex].FStepHover);

              if BorderColorHot <> clNone then
                PenColor := BlendColor(BorderColorHot, TabAppearance.BackGround.Color, FAdvOfficeTabs.Items[TabIndex].FStepHover);
            end;
          end;
        end
        else // Normal draw
      ...
      if Assigned(AdvOfficeTabSetStyler) or FSelfStyler then
      ...
      begin
        ...
        begin
          ...
          try
            ...
            DrawVistaGradient(bmp.Canvas, Rect(0, 0, R.Right - R.Left, R.Bottom-R.Top),GradColor, GradColorTo, GradColorMirror, GradColorMirrorTo, PenColor,
              GradU, GradB, '', Font, Layout, Enabled, False, FAntiAlias, TabRoundEdges, FAdvOfficeTabs.Items[TabIndex].Progress, RotateTabLeftRight, TabPosition);
        ...
        begin
          ...
          try
            ...
            begin
              bmp.Height := (R.Bottom - R.Top);
              if (MultiLine and (TabIndex <> ActiveTabIndex)) or not RotateTabLeftRight then
                bmp.Width := (R.Right - R.Left)
              else
            ...
            begin
              ...
              begin
                bmp.Height := bmp.Height + 1;
                bmp.Width := bmp.Width + 1;
              end;

              R2 := Rect(-4, 0, bmp.Width, bmp.Height);
            end;
  ...
  begin
    ...
    case TabPosition of
      ...
      begin
        ...
        begin
          ...
          else if (ButtonSettings.CloseButton and CloseOnTab and (ActiveTabIndex = TabIndex) and AdvOfficeTabs[TabIndex].ShowClose) and (CloseOnTabPosition = cpLeft) and (TabSettings.Alignment <> taCenter) then
            CapR.Left := CapR.Left + ButtonSettings.RealButtonSize + 4;
        end
        else
        begin
          if (ButtonSettings.CloseButton and CloseOnTab and AdvOfficeTabs[TabIndex].ShowClose) and (CloseOnTabPosition = cpLeft) and (TabSettings.Alignment <> taCenter) then
            CapR.Left := CapR.Left + ButtonSettings.RealButtonSize + 4;
        end;
              ...
              case TabSettings.ImagePosition of
                ...
                begin
                  ...
                  if (ButtonSettings.CloseButton and CloseOnTab and (ActiveTabIndex = TabIndex) and AdvOfficeTabs[TabIndex].ShowClose) and (CloseOnTabPosition = cpRight) then
                    ImgY := ImgY - ButtonSettings.RealButtonSize - 4;
                  end;
                  CapR.Bottom := ImgY;
                end;
              end;
    ...
    begin
      ...
      begin
        ...
        if ButtonSettings.CloseButton and CloseOnTab and TabAppearance.Drawline and AdvOfficeTabs[TabIndex].ShowClose then
        begin
    ...
    begin
      ...
      if ButtonSettings.CloseButton and CloseOnTab and ((TabSettings.Width <> 0) or TabAppearance.Drawline)and ((TabIndex = ActiveTabIndex) or ShowCloseOnNonSelectedTabs) and AdvOfficeTabs[TabIndex].ShowClose then
      begin
        if CloseOnTabPosition = cpRight then
        begin
          case TabPosition of
    ...
    if (ActiveTabIndex = TabIndex) and TabAppearance.DrawLine then
      begin
        if not FAnimating then
        begin
          if (FHotTabIndex = TabIndex) and (not FMouseDown) then
          begin
    ...
    begin
      case TabPosition of
        ...
        begin
          case TabSettings.ImagePosition of
            ipTop: ImgY := Max(TxtR.Top - ImgH, 4);
            ipBottom: ImgY := Min(TxtR.Bottom, CapR.Bottom);
          end;
        end;
        tpLeft:
        begin
    ...
    begin
      ...
      begin
        case TabPosition of
          ...
          begin
            case TabSettings.ImagePosition of
              ipTop: ImgY := Max(TxtR.Top - ImgList.Height, 4);
              ipBottom: ImgY := Min(TxtR.Bottom, CapR.Bottom);
            end;
          end;
          tpLeft:
...
procedure TAdvCustomOfficeTabSet.SetUIStyle(const Value: TTMSStyle);
begin
  FTMSStyle := Value;
...
var
  ...
  AllowChange: Boolean;
begin
  if (TabIndex < FAdvOfficeTabs.Count) and (TabIndex <> ActiveTabIndex) then
  begin
    AllowChange := True;
...
begin
  ...
  TabSettings.DPIScale := FDPIScale;
  ButtonSettings.DPIScale := FDPIScale;
  FTabSettings.Height := MulDiv(FTabSettings.Height,M,D);
...
begin
  ...
  if (R.Left = -1) and (Value < FAdvOfficeTabs.Count) and not (csLoading in ComponentState) then // force activated tab to be visible
  begin
    j := 0;
...
begin
  ...
  if FFirstVisibleTab >= 0 then
    start := FFirstVisibleTab
  else
    start := 0;

  if (FLastVisibleTab >= 0) and (FLastVisibleTab >= FFirstVisibleTab) then
    stop := FLastVisibleTab
  else
    stop := FAdvOfficeTabs.Count - 1;
...
begin
  ...
  case TabPosition of
    ...
    begin
      ...
      begin
        Result.Top := Result.Bottom - FTabSettings.Height - FTabOffSet + Round( 2 * FDPIScale);
    ...
    begin
      ...
      if (Result.Left < 0) and ButtonSettings.FullHeight then
        Result.Left := 0;
      if not ButtonSettings.FullHeight then
      ...
      begin
        if not ButtonSettings.FullHeight then
          ...
          Result.Right := Result.Left + Round(FDPIScale * DEFAULT_TABHEIGHT);
      end;
    end;
  end;
end;
    ...
    begin
      case TabPosition of
        ...
        begin
          ...
          if not ButtonSettings.FullHeight then
          begin
            d:= (Result.Bottom - Result.Top - ButtonSettings.RealButtonSize) div 2;
            Result.Bottom := Result.Bottom - 5 - d;
  ...
  begin
    ...
    case TabPosition of
      ...
      begin
        if not ButtonSettings.FullHeight then
        begin
          Result.Right := Result.Left + 3 + (ButtonSettings.RealButtonSize * 2 + 2 + 3);
          Result.Bottom := Result.Bottom - 5;
          Result.Top := Result.Bottom - ButtonSettings.RealButtonSize;
        end
        else
        begin
          Result.Right := Result.Left + Round(FDPIScale * ButtonSettings.ButtonSize * 2);
        end;
      end;
      tpBottom:
      begin
      ...
      begin
        if not ButtonSettings.FullHeight then
        begin
          Result.Bottom := Result.Top + 3 + (ButtonSettings.RealButtonSize * 2 + 2 + 3);
          Result.Right := Result.Right - 5;
          Result.Left := Result.Right - ButtonSettings.RealButtonSize;
        end
        else
        begin
          Result.Bottom := Result.Top + Round(FDPIScale * ButtonSettings.ButtonSize * 2);
        end;
      end;
      tpRight:
      begin
...
begin
  ...
  begin
    ...
    if (ButtonSettings.CloseButtonLook = cblOffice) and not (csLoading in ComponentState) then
      ButtonSettings.FullHeight := AdvOfficeTabSetStyler.TabAppearance.DrawLine;
    if ButtonSettings.FullHeight then
      ButtonSettings.ButtonSize := 13
    else
      ButtonSettings.ButtonSize := TABBUTTON_SIZE;

    ButtonSettings.FBtnPenColor := AdvOfficeTabSetStyler.GlowButtonAppearance.TextColor;
    ButtonSettings.FBtnColor := AdvOfficeTabSetStyler.GlowButtonAppearance.Color;
    ButtonSettings.FBtnHotColor := AdvOfficeTabSetStyler.GlowButtonAppearance.ColorHot;
    ButtonSettings.FBtnDownColor := AdvOfficeTabSetStyler.GlowButtonAppearance.ColorDown;
  end
  else if FSelfStyler and Assigned(FInternalOfficeTabSetStyler) then
  begin
    TabSettings.Rounding := FInternalOfficeTabSetStyler.TabRounding;
    if (ButtonSettings.CloseButtonLook = cblOffice) and not (csLoading in ComponentState) then
      ButtonSettings.FullHeight := FInternalOfficeTabSetStyler.TabAppearance.DrawLine;
    ...
    ButtonSettings.FBtnPenColor := FInternalOfficeTabSetStyler.GlowButtonAppearance.TextColor;
    ButtonSettings.FBtnColor := FInternalOfficeTabSetStyler.GlowButtonAppearance.Color;
    ButtonSettings.FBtnHotColor := FInternalOfficeTabSetStyler.GlowButtonAppearance.ColorHot;
    ButtonSettings.FBtnDownColor := FInternalOfficeTabSetStyler.GlowButtonAppearance.ColorDown;
  end;
end;
...
begin
  ...
  for I := 0 to AdvOfficeTabs.Count - 1 do
  begin
    AdvOfficeTabs[i].FCachedWidth := -1;
  end;

  if ButtonSettings.CloseButton and (CanShowCloseButton or not CloseOnTab) then
  begin
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if (TabPosition in [tpLeft, tpRight]) and ButtonSettings.FullHeight and not RotateTabLeftRight then
              begin
                FCloseButton.Width := Round(FDPIScale * ButtonSettings.ButtonSize - 2);
                FCloseButton.Height := Max(R.Bottom - R.Top, Round(FDPIScale * DEFAULT_TABHEIGHT));
              end
              else
              begin
  ...
  begin
    ...
    if ButtonSettings.FullHeight then
    begin
      FTabListButton.Rounded := False;
      FTabListButton.Picture := nil;
      FTabListButton.OnDrawButton := OnDrawPageButton;
    end
    else
    begin
      FTabListButton.Rounded := True;
      FTabListButton.OnDrawButton := nil;
      if (TabPosition in [tpLeft, tpRight]) then
          FTabListButton.Picture.Assign(ButtonSettings.FScrollButtonNextPicture)
      else
    ...
    FTabListButton.Enabled := GetVisibleTabCount > 0;
  end
  else
  if (FTabListButton <> nil) then
  begin
...
procedure TAdvCustomOfficeTabSet.AnimateLine;
begin
  if (csDesigning in ComponentState) then
  begin
...
procedure TAdvCustomOfficeTabSet.OnDrawCloseButton(Sender: TObject;
  Canvas: TCanvas; Rect: TRect; State: TGlowButtonState);
var
  s: integer;
begin
...
procedure TAdvCustomOfficeTabSet.OnDrawFirstButton(Sender: TObject;
  Canvas: TCanvas; Rect: TRect; State: TGlowButtonState);
var
  s: integer;
begin
...
procedure TAdvCustomOfficeTabSet.OnDrawLastButton(Sender: TObject;
  Canvas: TCanvas; Rect: TRect; State: TGlowButtonState);
var
  s: integer;
begin
...
procedure TAdvCustomOfficeTabSet.OnDrawNextButton(Sender: TObject;
  Canvas: TCanvas; Rect: TRect; State: TGlowButtonState);
var
  s: integer;
begin
...
procedure TAdvCustomOfficeTabSet.OnDrawPageButton(Sender: TObject;
  Canvas: TCanvas; Rect: TRect; State: TGlowButtonState);
var
  s: integer;
begin
...
procedure TAdvCustomOfficeTabSet.OnDrawPrevButton(Sender: TObject;
  Canvas: TCanvas; Rect: TRect; State: TGlowButtonState);
var
  s: integer;
begin
...
procedure TAdvCustomOfficeTabSet.OnDrawLineTimerTime(Sender: TObject);
begin
  if(FNewLineStart > FCurLineStart) then
  begin
...
begin
  ...
  case TabPosition of
    tpTop, tpBottom:
    begin
      Result := Bounds(R.Left + 5 + w, r.Top + (r.Bottom - R.Top - 15) div 2, 15, 15);
    end;
    tpLeft:
    begin
      if RotateTabLeftRight then
...
begin
  ...
  begin
    ...
    if (TabPosition in [tpLeft, tpRight]) and not RotateTabLeftRight and ButtonSettings.FullHeight then
      RoundedBtnSize := ButtonSettings.RealButtonSize - 2
    else
      RoundedBtnSize := ButtonSettings.RealButtonSize;

    if ButtonSettings.FullHeight and (not Odd(RoundedBtnSize)) then
      Inc(RoundedBtnSize);
    ...
    case TabPosition of
      ...
      begin
        ...
        begin
          ...
          begin
            Result.Right := R.Right - Round(FDPIScale * 2);
            Result.Left := Result.Right - RoundedBtnSize ;
            Result.Top := R.Bottom + gap + Round(FDPIScale * 4);
...
begin
  ...
  if FLastVisibleTab >= 0 then
    Result := FLastVisibleTab
  else
  begin
    for i:= FAdvOfficeTabs.Count -1 downto 0 do
    begin
      ...
      begin
        R := GetTabRect(i);
        if ((TabPosition in [tpTop, tpBottom]) and (R.Left > -1) and (R.Right > -1)) or
            ((TabPosition in [tpLeft,tpRight]) and (R.Top > -1) and (R.Bottom > -1))  then
        begin
          Result := i;
          Break;
        end;
...
procedure TAdvCustomOfficeTabSet.SetComponentStyle(AStyle: TTMSStyle);
var
  tempStyler: TAdvOfficeTabSetOfficeStyler;
begin
  if not Assigned(FOfficeTabSetStyler) then
  begin
    FTMSStyle := AStyle;
    if not (AStyle = AdvStyleIf.tsCustom) then
    begin
      FSelfStyler := True;
      tempStyler := TAdvOfficeTabSetOfficeStyler.Create(Self);
      tempStyler.SetComponentStyle(AStyle);
      FInternalOfficeTabSetStyler.Assign(tempStyler);
      tempStyler.Free;
...
procedure TAdvCustomOfficeTabSet.SetComponentStyleAndAppColor(AStyle: TTMSStyle;
  AppColor: TColor);
var
  tempStyler: TAdvOfficeTabSetOfficeStyler;
begin
...
function TTabSetButtonSettings.RealButtonSize: integer;
begin
...
begin
  ...
  begin
    ...
    if (j >= 0) and Assigned(FAdvOfficeTabs[j])
