...
const
  ...
  // v1.1.0.1 : Fixed : Issue with setting dataset Active false and radio button enable
  // v1.1.1.0 : Improved : TDBAdvOfficeRadioGroup, TDBAdvOfficeCheckGroup take DataSource.AutoEdit in account
  // v1.1.1.1 : Fixed : Issue with editing via setting DataSet.Edit in TDBAdvOfficeRadioGroup, TDBAdvOfficeCheckGroup
  // v1.1.1.2 : Fixed : Issue with state setting for use in TDBCtrlGrid
  // v1.1.1.3 : Fixed : Issue with TDBAdvOfficeRadioGroup when dataset is in Insert mode
  // v1.1.1.4 : Fixed : Issue with DataSource.OnDataChange triggered from TDBAdvOfficeRadioGroup
...
type
  ...
  protected
    ...
    procedure DoIsEnabled(AIndex: integer; var AEnabled: boolean); override;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    function GetVersionNr: Integer; override;
...
begin
  if not Focused and (FDatalink.Field <> nil) then
    UpdateState(GetFieldState);
...
begin
  ...
  if Assigned(DataLink.DataSource) and Assigned(DataLink.DataSource.DataSet) then
    Result := FDataLink.Edit and (DataLink.DataSource.AutoEdit or (DataLink.DataSet.State = dsEdit) or (DataLink.DataSet.State = dsInsert));
end;
...
procedure TDBAdvOfficeRadioGroup.DoIsEnabled(AIndex: integer;
  var AEnabled: boolean);
begin
  inherited DoIsEnabled(AIndex, AEnabled);

  if Assigned(DataSource) and Assigned(DataSource.DataSet) and not DataSource.AutoEdit then
    AEnabled := false or (DataSource.DataSet.State = dsEdit) or (DataSource.DataSet.State = dsInsert);
end;
...
procedure TDBAdvOfficeCheckGroup.DoIsEnabled(AIndex: integer;
  var AEnabled: boolean);
begin
  inherited DoIsEnabled(AIndex, AEnabled);
