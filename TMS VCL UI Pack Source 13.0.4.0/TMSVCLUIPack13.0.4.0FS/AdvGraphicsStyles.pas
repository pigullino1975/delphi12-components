...
uses
  Classes, Graphics, Controls, Forms, AdvTypes, AdvCustomComponent, AdvGraphicsTypes
  {$IFDEF FMXLIB}
  ,FMX.Types, UITypes
  {$ENDIF}
...
type
  ...
  {$IFDEF FNCLIB}
  TAdvGraphicsStylesManagerCanLoadStyleEvent = procedure(Sender: TObject; AStyle: string; AComponent: TComponent; var ACanLoadStyle: Boolean) of object;
  TAdvGraphicsStylesManagerStyleLoadedEvent = procedure(Sender: TObject; AStyle: string; AComponent: TComponent) of object;

  TAdvGraphicsStylesManagerFileArray = array of string;

  TAdvGraphicsStylesManagerComponentArray = array of TComponent;

  IAdvStylesManager = interface
    ['{88852C7F-B7B5-4FFA-BB47-6D95600CB1F3}']
    function GetSubComponentArray: TAdvGraphicsStylesManagerComponentArray;
  end;

  TAdvGraphicsStylesManagerOptions = class(TPersistent)
  private
  ...
  {$ENDIF}
  TAdvGraphicsStylesManager = class(TAdvCustomComponent)
  private
    {$IFDEF WEBLIB}
    FComponents: TAdvGraphicsStylesManagerComponentArray;
    {$ENDIF}
    FStyle: string;
    FStyleResource: string;
    FStyleForm: TCustomForm;
    FOnCanLoadStyle: TAdvGraphicsStylesManagerCanLoadStyleEvent;
    FOnStyleLoaded: TAdvGraphicsStylesManagerStyleLoadedEvent;
    FOptions: TAdvGraphicsStylesManagerOptions;
    procedure SetStyle(const Value: string);
    procedure SetStyleResource(const Value: string);
    procedure SetOptions(const Value: TAdvGraphicsStylesManagerOptions);
  protected
    function GetDocURL: string; override;
    procedure InternalLoadStyleFromJSONValue(AJSONValue: TJSONValue; AComponents: TAdvGraphicsStylesManagerComponentArray);
    procedure InternalLoadStyle(AValue: string; AComponents: TAdvGraphicsStylesManagerComponentArray);
  ...
  public
    ...
    procedure LoadStyleFromText(AText: string; AComponents: TAdvGraphicsStylesManagerComponentArray); overload; virtual;
    procedure LoadStyleFromStream(AStream: TStream; AComponents: TAdvGraphicsStylesManagerComponentArray); overload; virtual;
    procedure LoadStyleFromFile(AFile: string; AComponents: TAdvGraphicsStylesManagerComponentArray); overload; virtual;
    {$IFNDEF WEBLIB}
    procedure LoadStyleFromResource(AResourceName: string; AComponents: TAdvGraphicsStylesManagerComponentArray); overload; virtual;
    ...
    function CombineStyles(AFiles: TAdvGraphicsStylesManagerFileArray): string; virtual;
    {$ELSE}
    procedure LoadStyleFromURL(AURL: string; AComponents: TAdvGraphicsStylesManagerComponentArray); overload; virtual;
    procedure LoadStyleFromURL(AURL: string); overload; virtual;
    {$ENDIF}
    property StyleForm: TCustomForm read FStyleForm write FStyleForm;
  published
    property Options: TAdvGraphicsStylesManagerOptions read FOptions write SetOptions;
    property Style: string read FStyle write SetStyle;
    property StyleResource: string read FStyleResource write SetStyleResource;
    property OnCanLoadStyle: TAdvGraphicsStylesManagerCanLoadStyleEvent read FOnCanLoadStyle write FOnCanLoadStyle;
    property OnStyleLoaded: TAdvGraphicsStylesManagerStyleLoadedEvent read FOnStyleLoaded write FOnStyleLoaded;
  end;
  {$ENDIF}
...
uses
  AdvUtils, SysUtils, AdvPersistence, AdvGraphics
  {$IFDEF FMXLIB}
...
begin
  if Assigned(AScene) and Assigned(AScene.GetObject) and (AScene.GetObject.ClassName <> 'TAdvCustomPopupForm') then
    FScene := AScene
...
function TAdvGraphicsStylesManager.GetDocURL: string;
begin
  Result := TAdvBaseDocURL + 'tmsfnccore/components/ttmsfncstyling';
end;

procedure TAdvGraphicsStylesManager.DoCanLoadStyle(AStyle: string;
...
constructor TAdvGraphicsStylesManager.Create(AOwner: TComponent);
begin
  inherited;
  FOptions := TAdvGraphicsStylesManagerOptions.Create;
end;

destructor TAdvGraphicsStylesManager.Destroy;
begin
  FOptions.Free;
  inherited;
end;

procedure TAdvGraphicsStylesManager.DoStyleLoaded(AStyle: string;
...
procedure TAdvGraphicsStylesManager.InternalLoadStyle(AValue: string; AComponents: TAdvGraphicsStylesManagerComponentArray);
var
  ...
  c: TAdvGraphicsColor;
begin
  v := TAdvUtils.ParseJSON(AValue);
  if Assigned(v) then
  begin
    try
      if Options.AdaptFormColor then
      begin
        fc := TAdvUtils.GetJSONValue(v, 'FormColor');
        if Assigned(fc) then
        begin
          fcs := TAdvUtils.GetJSONProp(v, 'FormColor');
          f := StyleForm;
          ...
          if Assigned(f) then
          begin
            c := TAdvGraphics.HTMLToColor(fcs);
            {$IFDEF FMXLIB}
...
procedure TAdvGraphicsStylesManager.InternalLoadStyleFromJSONValue(
  AJSONValue: TJSONValue; AComponents: TAdvGraphicsStylesManagerComponentArray);
var
  ct: string;
  I, K: Integer;
  f: TCustomForm;
  ...
  st: string;
  b: Boolean;
  sm: IAdvStylesManager;
  arr: TAdvGraphicsStylesManagerComponentArray;
  ...
  begin
    ...
    begin
      if Supports(AComponent, IAdvPersistence, io) then
      begin
        st := TAdvUtils.GetJSONValueAsString(AJSONValue);
        b := True;
        DoCanLoadStyle(st, AComponent, b);
        if b then
        begin
          s := TStringStream.Create(st{$IFDEF WEBLIB}, TEncoding.Ansi{$ENDIF});
          try
...
begin
  ...
  begin
    ct := TAdvUtils.GetJSONProp(AJSONValue, TAdvPersistence.ClassTypeVariable);
    if ct <> '' then
    begin
      for I := f.ComponentCount - 1 downto 0 do
      begin
        ApplyStyleToComponent(f.Components[I]);
        if Supports(f.Components[I], IAdvStylesManager, sm) then
        begin
...
{$IFNDEF WEBLIB}
function TAdvGraphicsStylesManager.CombineStyles(AFiles: TAdvGraphicsStylesManagerFileArray): string;
var
  I: Integer;
  sl: TStringList;
begin
...
function TAdvGraphicsStylesManager.GetStyleFromFile(AFile: string): string;
var
...
function TAdvGraphicsStylesManager.GetStyleFromResource(AResourceName: string): string;
var
  r: TResourceStream;
  ss: {$IFDEF LCLLIB}TStringList{$ELSE}TStringStream{$ENDIF};
begin
  ...
  r := nil;
  try
    r := TAdvUtils.GetResourceStream(AResourceName, HInstance);
    if Assigned(r) then
    begin
      ss := {$IFDEF LCLLIB}TStringList{$ELSE}TStringStream{$ENDIF}.Create;
      try
...
procedure TAdvGraphicsStylesManager.LoadStyleFromResource(AResourceName: string; AComponents: TAdvGraphicsStylesManagerComponentArray);
var
  r: TResourceStream;
begin
  r := nil;
...
procedure TAdvGraphicsStylesManager.LoadStyleFromResource(AResourceName: string);
var
  a: TAdvGraphicsStylesManagerComponentArray;
begin
  SetLength(a, 0);
...
{$IFDEF WEBLIB}
procedure TAdvGraphicsStylesManager.LoadStyleFromURL(AURL: string);
var
  a: TAdvGraphicsStylesManagerComponentArray;
begin
  SetLength(a, 0);
...
procedure TAdvGraphicsStylesManager.LoadStyleFromURL(AURL: string; AComponents: TAdvGraphicsStylesManagerComponentArray);
begin
  LoadStyleFromFile(AURL, AComponents);
end;
{$ENDIF}

procedure TAdvGraphicsStylesManager.LoadStyleFromFile(AFile: string);
var
  a: TAdvGraphicsStylesManagerComponentArray;
begin
  SetLength(a, 0);
...
procedure TAdvGraphicsStylesManager.LoadStyleFromFile(AFile: string; AComponents: TAdvGraphicsStylesManagerComponentArray);
var
  ss: {$IFDEF LCLLIB}TStringList{$ELSE}TStringStream{$ENDIF};
begin
  {$IFNDEF WEBLIB}
...
procedure TAdvGraphicsStylesManager.LoadStyleFromStream(AStream: TStream);
var
  a: TAdvGraphicsStylesManagerComponentArray;
begin
  SetLength(a, 0);
...
procedure TAdvGraphicsStylesManager.LoadStyleFromStream(AStream: TStream; AComponents: TAdvGraphicsStylesManagerComponentArray);
var
  ss: {$IFDEF LCLLIB}TStringList{$ELSE}TStringStream{$ENDIF};
begin
  ss := {$IFDEF LCLLIB}TStringList{$ELSE}TStringStream{$ENDIF}.Create;
...
procedure TAdvGraphicsStylesManager.LoadStyleFromText(AText: string);
var
  a: TAdvGraphicsStylesManagerComponentArray;
begin
  SetLength(a, 0);
...
procedure TAdvGraphicsStylesManager.LoadStyleFromText(AText: string; AComponents: TAdvGraphicsStylesManagerComponentArray);
begin
  InternalLoadStyle(AText, AComponents);
end;

procedure TAdvGraphicsStylesManager.SetStyle(const Value: string);
begin
...
procedure TAdvGraphicsStylesManager.SetOptions(const Value: TAdvGraphicsStylesManagerOptions);
begin
  FOptions.Assign(Value);
end;

procedure TAdvGraphicsStylesManager.SetStyleResource(const Value: string);
begin
...
constructor TAdvGraphicsStylesManagerOptions.Create;
begin
