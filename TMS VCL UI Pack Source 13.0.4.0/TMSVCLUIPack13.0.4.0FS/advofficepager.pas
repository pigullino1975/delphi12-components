...
const
  ...
  // 3.7.1.2 : Fixed : Issue with turning VCL Styles off at runtime
  // 3.8.0.0 : New : Support for Office 2019 styles
  //         : Fixed : DPI change page height
  // 3.8.1.0 : New : Internal Office styler implemented
  //         : Fixed : Issue when resetting VCL styles
  //         : Improved : VCL Style color settings for tabs
  // 3.8.1.1 : Fixed : TabSettings.TextSelectedBold also works for other styles than Office 2019
  //         : Improved : Check on creation if AdvFormStyler is on form and change style
  // 3.8.2.0 : New : Possible to make no page active (ActivePageIndex to -1)
  // 3.8.2.1 : Fixed : Issue with UIStyle property and Office 2003 Blue style
  //         : Fixed : VCL styles check updated to work in 10.4 Sydney
  // 3.8.2.2 : Fixed : Use of AppColor in Office2019 White style with TAdvFormStyler
  // 3.8.2.3 : Fixed : VCL style got cleared when changing monitors with different DPI
  // 3.8.2.4 : Improved : Added check with VCL styles if tab color is different from text color
  // 3.8.2.5 : Fixed : DPI calculation when parent form is not Scaled
  // 3.8.2.6 : Fixed : Issue with tab width when TextSelectedBold and MultiLine
  // 3.8.2.7 : Fixed : Issue with the close button drawing on Office 2019 style with the Chrome look.
  // 3.8.2.8 : Improved : Rendering on Windows 11
  // 3.8.3.0 : Improved : High DPI rendering with Delphi 11, Windows 11
  // 3.8.3.1 : Fixed : Issue with VCL styles initialization
  // 3.8.4.0 : New : TAdvOfficePage.TextAlignment,TAdvOfficePage.TextVerticalAlignment added
  // 3.8.4.1 : Fixed : Issue with VCL styles and high DPI
  // 3.8.4.2 : Fixed : Issue with style change when VCL styles are used in RDP usage
  // 3.8.4.3 : Fixed : High DPI issue when used on frames
  // 3.8.4.4 : Fixed : High DPI issue with VCL styles
  // 3.8.4.5 : Fixed : Issue with use in Win64 environment
  // 3.8.4.6 : Fixed : Issue with destroy for floating pages
  // 3.8.4.7 : Fixed : Initialization issue for high DPI for tab font height
  // 3.8.4.8 : Fixed : Issue with destroy component after dock/undock
  // 3.8.4.9 : Fixed : Issue with tab indexing when setting FreeOnClose = true
...
type
  ...
  published
    ...
    property TextSelectedBold: Boolean read FTextSelectedBold write FTextSelectedBold default False;
  end;
  ...
  published
    ...
    property OnChange: TNotifyEvent read FOnChange write FOnChange;
    property FullHeight: Boolean read FFullHeight write FFullHeight default False;
  end;
  ...
  TVerticalAlignment = (tvaTop, tvaCenter, tvaBottom);
  ...
  published
    ...
    property TextVerticalAlignment: TVerticalAlignment read FTextVerticalAlignment write SetTextVerticalAlignment default tvaCenter;
  ...
  {$ENDIF}
  TAdvOfficePager = class(TCustomControl, ITMSStyleEx, ITMSTones)
  private
    FTones: TColorTones;
    ...
    FMouseDown: Boolean;
    //-- Animate Line
    FCurLineStart: Double;
    FCurLineEnd: Double;
    FNewLineStart: integer;
    FNewLineEnd: integer;
    FLineBottom: integer;
    FDeltaStart: Double;
    FDeltaEnd: Double;
    ...
    procedure OnDrawLineTimerTime(Sender: TObject);
    ...
    procedure OnDrawCloseButton(Sender: TObject; Canvas: TCanvas;
       Rect: TRect; State: TGlowButtonState);
    procedure OnDrawFirstButton(Sender: TObject; Canvas: TCanvas;
       Rect: TRect; State: TGlowButtonState);
    procedure OnDrawLastButton(Sender: TObject; Canvas: TCanvas;
       Rect: TRect; State: TGlowButtonState);
    procedure OnDrawNextButton(Sender: TObject; Canvas: TCanvas;
       Rect: TRect; State: TGlowButtonState);
    procedure OnDrawPrevButton(Sender: TObject; Canvas: TCanvas;
       Rect: TRect; State: TGlowButtonState);
    procedure OnDrawPageButton(Sender: TObject; Canvas: TCanvas;
       Rect: TRect; State: TGlowButtonState);
  ...
  protected
    ...
    procedure Notification(AComponent: TComponent; Operation: TOperation); override;
    procedure SetParent(AParent: TWinControl); override;
    procedure StoreTabFontHeight(Writer: TWriter);
    procedure LoadTabFontHeight(Reader: TReader);
...
{$ENDIF}
        ...
        Canvas.TextOut(R.Left + ((R.Right - R.Left) div 2 -
          ...
          ((R.Bottom - R.Top) div 2) + Canvas.TextWidth(IntToStr(Percentage) + '%') div 2
          , IntToStr(Percentage) + '%');
      finally
        tf.Free;
      end;
...
begin
  ...
  {$IFDEF DELPHIXE_LVL}
  if (bmp is TGDIPPicture) and (bmp as TGDIPPicture).HasSVG then
    (bmp as TGDIPPicture).DrawSVG(graphics, P.X, P.Y, bmp.Width, bmp.Height)
  else
  {$ENDIF}
  begin
    ms := TMemoryStream.Create;
...
begin
  ...
  if Pic.HasSVG then
  begin
    Pic.DrawSVG(graphics, r.Left, r.Top, r.Right - r.Left, r.Bottom - r.Top);
    Exit;
  end;
  {$ENDIF}
    ...
    begin
      case (Direction) of
        ...
        begin
          ...
          p5[0] := MakePoint(R.Left, R.Top);
          p5[1] := MakePoint(R.Left + Rounding, R.Top + Rounding*i);
          p5[2] := MakePoint(R.Right - Rounding, R.Top - Rounding + j);
          p5[3] := MakePoint(R.Right, R.Top + (Rounding * 2) + j);
          Result.AddBezier(P5[0],P5[1], P5[2], P5[3]);
...
procedure TVistaBackground.Init;
begin
  FSteps := 64;
  FColor := clWhite;
  FColorTo := clWhite;
...
begin
  ...
  begin
    ...
    FDrawLine := (Source as TCustomTabAppearance).DrawLine;
    FTextSelectedBold := (Source as TCustomTabAppearance).TextSelectedBold;
...
procedure TCustomTabAppearance.Init;
begin
  ...
  FFont.Name := string(DefFontData.Name);
  FFont.Height := -11;
  FFont.Style := [];
...
begin
  ...
  begin
    TabRounding :=((Source as TCustomAdvOfficePagerStyler).TabRounding);
    ...
    GlowButtonAppearance.Assign((Source as TCustomAdvOfficePagerStyler).GlowButtonAppearance);
...
begin
  ...
  FTextVerticalAlignment := tvaCenter;
...
procedure TAdvOfficePage.CreateWnd;
begin
  inherited;
  if IsWin11 then
...
begin
  ...
  if (FParentPager <> nil) and (FParentPager.FFloatingPages <> nil) then
  begin
    if (FParentPager.FFloatingPages.Count > 0) then
    begin
...
begin
  ...
  begin
    ...
    if pos(#13, FText) = 0 then
    begin
      textstyle := DT_SINGLELINE;

      case FTextAlignment of
      taLeftJustify: textstyle := textstyle or DT_LEFT;
      taCenter: textstyle :=  textstyle or DT_CENTER;
      taRightJustify: textstyle :=  textstyle or DT_RIGHT;
      end;

      case FTextVerticalAlignment of
      tvaTop: textstyle := textstyle or DT_TOP;
      tvaCenter: textstyle := textstyle or DT_VCENTER;
      tvaBottom: textstyle := textstyle or DT_BOTTOM;
      end;

      r := ClientRect;
      OffsetRect(r, 2, 2);
      DrawText(Canvas.Handle, PChar(FText), Length(Text), r, textstyle);
    end
    else
    begin
      textstyle := DT_WORDBREAK or DT_CALCRECT or DT_LEFT or DT_TOP;
      ...
      h := DrawText(Canvas.Handle, PChar(FText), Length(FText), r, textstyle);
      w := r.Right - r.Left;
      ...
      case FTextAlignment of
      taLeftJustify: x := 2;
      taCenter: x := (R.Right - R.Left - w) div 2;
      taRightJustify: x := R.Right - 2 - w;
      end;

      case FTextVerticalAlignment of
      tvaTop: y := 2;
      tvaCenter: y := (R.Bottom - R.Top - h) div 2;
      tvaBottom: y := R.Bottom - 2 - h;
      end;

      textstyle := DT_WORDBREAK or DT_LEFT or DT_TOP;
      r := ClientRect;
      r.Left := x;
      r.Top := y;

      DrawText(Canvas.Handle, PChar(FText), Length(FText), r, textstyle);
    end;
  end;
end;
...
procedure TAdvOfficePage.SetTextAlignment(const Value: TAlignment);
begin
...
procedure TAdvOfficePage.SetTextVerticalAlignment(
...
var
  i: integer;
begin
  FIsAeroVista := IsComCtl6 and IsVista and AeroIsEnabled and not (csDesigning in ComponentState);
...
{$ENDIF}
  ...
  FTMSStyle := tsCustom;
  FInternalStyler := False;
  if FDesignTime then
  begin
    SetUIStyle(GetDefaultStyle(Owner));
  end;
  FSelfStyler := False;
...
begin
  ...
  begin
    ...
    repeat
      p := p.Parent;
    until (p is TCustomForm) or (p is TFrame) or (p is TActiveForm) or not Assigned(p);
    ...
    {$IFDEF DELPHIXE14_LVL}
    frm := GetParentForm(Self, false);
    if Assigned(frm) then
      FDPIScale := frm.PixelsPerInch/96;
    {$ENDIF}
...
begin
  ...
  begin
    ...
    {$ENDIF}
      ct := GetVCLStyleTones;
      if ct.Background.BrushColor <> clNone then
      begin
        SetColorTones(GetVCLStyleTones);
      end;

      FOldVCLStyle := StyleServices.Name;
    {$IFDEF DELPHIXE6_LVL}
    end;
    {$ENDIF}
  end;
...
procedure TAdvOfficePager.LoadTabFontHeight(Reader: TReader);
begin
  if Assigned(FTabAppearance) then
    FTabAppearance.Font.Height := Reader.ReadInteger
  else
    FCurrentOfficePagerStyler.TabAppearance.Font.Height := Reader.ReadInteger;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      SetComponentStyle(FTMSStyle);
    end;
    if (AComponent is TAdvOfficePagerOfficeStyler) or (AComponent is TAdvOfficePagerFantasyStyler) then
    begin
      AdvOfficePagerStyler := nil;
      if not FInternalStyler then
      begin
        SetComponentStyle(FTMSStyle);
      end;
    end;
...
begin
  ...
  begin
    ...
    begin
      if DrawLine then
      begin
        FLineColor := TextColorHot;
        if not FMouseDown then
          FBold := TextSelectedBold;
      end
      else
      begin
        FBold := TextSelectedBold;
      ...
      begin
        if DrawLine and not FMouseDown then
        begin
          GradColor := ColorHot;
        ...
        try
          ...
          begin
            DrawVistaGradient(bmp.Canvas, Rect(0, 0, R.Right - R.Left, R.Bottom-R.Top),GradColor, GradColorTo, GradColorMirror, GradColorMirrorTo, PenColor,
              GradU, GradB, '', Font, Layout, Enabled, False, FAntiAlias, TabRoundEdges, RotateLR, AdvPages[PageIndex].Progress, False, False, TabPos)
          end
          else
          begin
            case TabPosition of
      ...
      begin
        ...
        begin
          DrawVistaGradient(Canvas, R,GradColor, GradColorTo, GradColorMirror, GradColorMirrorTo, PenColor,
           GradU, GradB, '', Font, Layout, Enabled, False, FAntiAlias, TabRoundEdges, RotateLR, AdvPages[PageIndex].Progress, False, MultiLine, TabPos)
        end
        else
        begin
          R3 := R;
      ...
      begin
        case TabPosition of
          ...
          begin
            ...
            begin
              Canvas.Pixels[R.Left, R.Top] := c1;
              Canvas.Pixels[R.Left + 1, R.Top] := c1;
              Canvas.Pixels[R.Left, R.Top + 1] := c1;

              Canvas.Pixels[R.Right - 1, R.Top] := c2;
              Canvas.Pixels[R.Right - 2, R.Top] := c2;
              Canvas.Pixels[R.Right - 1, R.Top + 1] := c2;
            end;
          ...
          begin
            ...
            begin
              Canvas.Pixels[R.Left, R.Bottom-2] := c1;
              Canvas.Pixels[R.Left + 1, R.Bottom-1] := c1;
              Canvas.Pixels[R.Left, R.Bottom - 1] := c1;

              Canvas.Pixels[R.Right - 1, R.Bottom-1] := c2;
              Canvas.Pixels[R.Right - 2, R.Bottom-1] := c2;
              Canvas.Pixels[R.Right - 1, R.Bottom - 2] := c2;
            end;
          ...
          begin
            ...
            begin
              ...
              Canvas.Pixels[R.Left, R.Bottom-1] := c2;
              Canvas.Pixels[R.Left + 1, R.Bottom-1] := c2;
              Canvas.Pixels[R.Left, R.Bottom - 2] := c2;
            end;
          ...
          begin
            if TabRoundEdges then
            begin
              Canvas.Pixels[R.Right, R.Top] := c1;
              Canvas.Pixels[R.Right - 1, R.Top] := c1;
              Canvas.Pixels[R.Right, R.Top + 1] := c1;

              Canvas.Pixels[R.Right, R.Bottom-1] := c2;
              Canvas.Pixels[R.Right - 1, R.Bottom-1] := c2;
              Canvas.Pixels[R.Right, R.Bottom - 2] := c2;
...
begin
  ...
  if IsGlass then
    ...
    case TabPosition of
      tpTop, tpBottom:
      begin
        if not TabAppearance.DrawLine then
          CapR := Rect(R.Left + FTabSettings.LeftMargin, R.Top, R.Right, R.Bottom )
        else
          CapR := Rect(R.Left, R.Top, R.Right, R.Bottom );
    ...
    begin
      ...
      if TabAppearance.DrawLine then
        AAlign := taCenter
      else
      begin
        if (TabSettings.Width <> 0) then
    ...
    begin
      ...
      begin
        ...
        if TabAppearance.DrawLine then
        begin
         CapR2.Right := CapR2.Right - Round(FDPIScale * 5);
        end;

        TxtR.Left := CapR2.Left + ((CapR2.Right - CapR2.Left) - Canvas.TextHeight('gh')) div 2;
        ...
        if ButtonSettings.CloseButton and CloseOnTab and TabAppearance.Drawline and AdvPages[PageIndex].ShowClose then
        begin
          if CloseOnTabPosition = cpRight then
            CapR2.Top := CapR2.Top + ButtonSettings.RealButtonSize;
        end;
        ...
        begin
          ...
          if TabAppearance.DrawLine then
            Canvas.TextOut(CapR.Left + ((CapR2.Right - CapR2.Left) - (Canvas.TextHeight('gh') + 5) ) div 2, CapR.Bottom - (((CapR2.Bottom - CapR2.Top) - (TxtR.Bottom - TxtR.Top)) div 2), DCaption)
          else
        ...
        begin
          ...
          TextOutW(Canvas.Handle, CapR.Left + ((CapR2.Right - CapR2.Left) - Canvas.TextHeight('gh')) div 2, CapR.Bottom - (((CapR2.Bottom - CapR2.Top) - (TxtR.Bottom - TxtR.Top)) div 2), PWideChar(WCaption), Length(WCaption));
        end;
      end;
    end
    else
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Canvas.TextOut(CapR.Right - ((CapR.Right - CapR.Left)-Canvas.TextHeight('gh') - Round(3 * DPIScale)) div 2, CapR2.Top + (((CapR2.Bottom - CapR2.Top) - (TxtR.Bottom - TxtR.Top)) div 2), DCaption);
        end
        else
        begin
          WCaption := TrimTextW(WCaption, CapR, False, nil, Canvas, nil, nil, Ellipsis, TabPosition, TabSettings.WordWrap);
          TextOutW(Canvas.Handle, CapR.Right - ((CapR.Right - CapR.Left) - Canvas.TextHeight('gh')) div 2, CapR2.Top + (((CapR2.Bottom - CapR2.Top) - (TxtR.Bottom - TxtR.Top)) div 2), PWideChar(WCaption), Length(WCaption));
        end;
      end;
    end
    else
    begin
      ...
      begin
        ...
        case TabPosition of
          tpTop:
            CapR2.Bottom := CapR2.Bottom - Round(FDPIScale * 6);
          tpRight:
            CapR2.Left := CapR2.Left + Round(FDPIScale * 6);
          tpLeft:
            CapR2.Right := CapR2.Right - Round(FDPIScale * 5);
        end;
      end
      ...
      if ButtonSettings.CloseButton and CloseOnTab and ((TabSettings.Width <> 0) or TabAppearance.Drawline)and ((PageIndex = ActivePageIndex) or ShowCloseOnNonSelectedTabs) and AdvPages[PageIndex].ShowClose then
      begin
        if CloseOnTabPosition = cpRight then
        begin
          case TabPosition of
            tpTop, tpBottom:
              CapR2.Right := CapR2.Right - ButtonSettings.RealButtonSize;
            tpLeft:
              CapR2.Top := CapR2.Top - ButtonSettings.RealButtonSize;
            tpRight:
              CapR2.Bottom := CapR2.Bottom - ButtonSettings.RealButtonSize;
          end;
        end;
      end;
    ...
    if (ActivePageIndex = PageIndex) and TabAppearance.DrawLine then
      begin
        if not FAnimating then
        begin
          if (HotPageIndex = PageIndex) and (not FMouseDown) then
          begin
            if not RotateTabLeftRight or (TabPosition in [tpBottom, tpTop]) then
            begin
              FNewLineStart := CapR2.Left;
              FNewLineEnd := CapR2.Right;
            end
            else
            begin
              case TabPosition of
                tpLeft:
                begin
                  FNewLineStart := CapR2.Top;
                  FNewLineEnd := CapR.Bottom;
                end;
                tpRight:
                begin
                  FNewLineStart := CapR.Top;
                  FNewLineEnd := CapR2.Bottom;
                end;
              end;
            end;
          end
          ...
          begin
            ...
            begin
              if aa <> aaNone then
              begin
                FNewLineStart := TxtR.Left;
                FNewLineEnd := TxtR.Right;
              end
              else
              begin
                FNewLineStart := CapR2.Left + ((CapR2.Right - CapR2.Left) - (TxtR.Right - TxtR.Left)) div 2;
                FNewLineEnd := CapR2.Right - ((CapR2.Right - CapR2.Left) - (TxtR.Right - TxtR.Left)) div 2;
              end;
              end
            else
            begin
              case TabPosition of
                tpLeft:
                begin
                  FNewLineStart := TxtR.Top - (TxtR.Bottom - TxtR.Top) - (((CapR2.Bottom - CapR2.Top) - (TxtR.Bottom - TxtR.Top)) div 2);
                  FNewLineEnd := CapR.Bottom - ((CapR2.Bottom - CapR2.Top) - (TxtR.Bottom - TxtR.Top)) div 2;
                end;
                tpRight:
                begin
                  if ((AntiAlias = aaNone) or (TabSettings.Width > 0)) then
                  begin
                    FNewLineStart := TxtR.Top - (TxtR.Bottom - TxtR.Top) - (((CapR2.Bottom - CapR2.Top) - (TxtR.Bottom - TxtR.Top)) div 2);
                    FNewLineEnd := CapR.Bottom - ((CapR.Bottom - CapR.Top) - (TxtR.Bottom - TxtR.Top)) div 2;
                  end
                  else
                  begin
                    FNewLineStart := TxtR.Top;
                    FNewLineEnd := TxtR.Bottom;
                  end;
                end;
              end;
            end;
          ...
          if not RotateTabLeftRight or (TabPosition = tpTop) then
          begin
            FLineBottom := CapR2.Bottom;
          end
          else
          begin
            case TabPosition of
              tpBottom:
              begin
                FLineBottom := TxtR.Bottom + Round( 3 * FDPIScale);
              end;
              tpLeft:
              begin
                FLineBottom := TxtR.Right + Round(FDPIScale);
              end;
              tpRight:
              begin
                FLineBottom := CapR2.Left;
              end;
            end;
          end;

          if(FNewLineStart <> FCurLineStart) then
          begin
            AnimateLine;
          end
          else
          begin
            if not RotateTabLeftRight or (TabPosition in [tpBottom, tpTop]) then
            begin
              LineRect := Rect(FNewLineStart, FLineBottom - Round(FDPIScale), FNewLineEnd, FLineBottom + 2 * Round(FDPIScale));
            end
            else
            begin
              case TabPosition of
                tpLeft:
                  LineRect := Rect(FLineBottom - Round(FDPIScale), FNewLineStart, FLineBottom + 2 * Round(FDPIScale), FNewLineEnd);
                tpRight:
                  LineRect := Rect(FLineBottom - 2 * Round(FDPIScale), FNewLineStart, FLineBottom + Round(FDPIScale), FNewLineEnd);
              end;
            end;

            DrawRectangle(Canvas, LineRect, FLineColor);
          end;
        end
        else
        begin
          if not RotateTabLeftRight or (TabPosition in [tpBottom, tpTop]) then
          begin
            LineRect := Rect(Round(FCurLineStart), FLineBottom - Round(FDPIScale), Round(FCurLineEnd), FLineBottom + 2 * Round(FDPIScale));
          end
          else
          begin
            case TabPosition of
              tpLeft:
                LineRect := Rect(FLineBottom - Round(FDPIScale), Round(FCurLineStart), FLineBottom + 2 * Round(FDPIScale), Round(FCurLineEnd));
              tpRight:
                LineRect := Rect(FLineBottom - 2 * Round(FDPIScale), Round(FCurLineStart), FLineBottom + Round(FDPIScale), Round(FCurLineEnd));
            end;
          end;
...
begin
  ...
  begin
    ...
    if not ButtonSettings.FullHeight then
    begin
    ...
    begin
      Clr := MakeColor(255, ButtonSettings.FBtnDownColor);
      BrClr := clNone;
      PClr := clNone;
      PBrClr := clNone;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        Clr := MakeColor(255, ButtonSettings.FBtnHotColor);
        PClr := clNone;
        PBrClr := clNone;
      end;
    end
    ...
    begin
      ...
      begin
        Clr := MakeColor(255, ButtonSettings.FBtnColor);
        PClr := clNone;
        PBrClr := clNone;
      end;
    end;
  ...
  if not ButtonSettings.FullHeight then
    ...
    btnpath := GetInsertButtonPath(R, Shape, 0, RotateTabLeftRight, TabPosition);
  ...
  begin
    g.SetSmoothingMode(SmoothingModeDefault);
    l := Min(R.Right - R.Left, R.Bottom - R.Top) div 2;

    Pen := TGPPEn.Create(MakeColor(255, ButtonSettings.FBtnPenColor), FDPIScale);

    if (TabPosition in [tpTop, tpBottom]) or not RotateTabLeftRight then
    begin
      g.DrawLine(Pen, R.Left + (R.Right - R.Left - l) div 2 + 1, R.Top + (R.Bottom - R.Top) div 2, R.Left + (R.Right - R.Left - l) div 2 + l + 1, R.Top + (R.Bottom - R.Top) div 2);
      g.DrawLine(Pen, R.Left + (R.Right - R.Left) div 2 + 1, R.Top + (R.Bottom - R.Top - l) div 2, R.Left + (R.Right - R.Left) div 2 + 1, R.Top + (R.Bottom - R.Top - l) div 2 + l);
    end
    else
    begin
      g.DrawLine(Pen, R.Left + (R.Right - R.Left - l) div 2 , R.Top + (R.Bottom - R.Top) div 2 + 1, R.Left + (R.Right - R.Left - l) div 2 + l , R.Top + (R.Bottom - R.Top) div 2 + 1);
      g.DrawLine(Pen, R.Left + (R.Right - R.Left) div 2, R.Top + (R.Bottom - R.Top - l) div 2 + 1, R.Left + (R.Right - R.Left) div 2, R.Top + (R.Bottom - R.Top - l) div 2 + l + 1);
    end;
...
begin
  ...
  begin
    ...
    begin
      {$IFDEF DELPHIXE_LVL}
      PostMessage(Self.Handle, WM_OPDESTROYCLOSEBTN, nativeuint(Pointer(AdvPage.FCloseButtonChrome)), 1);
      {$ENDIF}
      {$IFNDEF DELPHIXE_LVL}
...
begin
  ...
  if StyleServices.Enabled {$IFDEF DELPHIXE6_LVL} and (seClient in StyleElements){$ENDIF} then
  begin
    if CheckVCLStylesEnabled(StyleServices, (csDesigning in ComponentState)) and (FOldVCLStyle <> StyleServices.Name) and (StyleServices.Name <> 'Windows') then
    begin
      ct := GetVCLStyleTones;
      if ct.Background.BrushColor <> clNone then
      begin
      ...
      if Assigned(AdvOfficePagerStyler) or FSelfStyler then
        UpdateMe(2)
      else
      begin
        if not Assigned(FPageAppearance) then
          FPageAppearance := TVistaBackground.Create;
        FPageAppearance.Init;
...
procedure TAdvOfficePager.SetUIStyle(const Value: TTMSStyle);
begin
  FTMSStyle := Value;
...
var
  ...
  FOnShow: TNotifyEvent;
begin
  if (PageIndex < FAdvPages.Count) and ((PageIndex <> ActivePageIndex) or (FAdvPages.Count = 1)) then
  begin
    ...
    if Assigned(ActivePage) and (FActivePageIndex > 0) and (Visible or (csDesigning in ComponentState)) then
    begin
    ...
    if (Visible or (csDesigning in ComponentState)) and (FActivePageIndex >= 0) then
    begin
      FOnShow := AdvPages[FActivePageIndex].OnShow;
      AdvPages[FActivePageIndex].OnShow := nil;
...
begin
  ...
  Result := ClientRect;
  if ButtonSettings.FullHeight then
    Spc := 0
  else
    Spc := 3;
  case TabPosition of
    ...
    begin
      ...
      begin
        if not ButtonSettings.FullHeight then
          ...
          Result.Left := Result.Left - (ButtonSettings.RealButtonSize * cnt);
      end;
    ...
    begin
      ...
      begin
        Result.Top := Result.Bottom - FTabSettings.RealHeight - FTabOffSet + Round( 2 * FDPIScale);
        Result.Bottom := Result.Bottom - Round(FDPIScale * 2);
      end;
    ...
    begin
      ...
      begin
        if not ButtonSettings.FullHeight then
          ...
          Result.Top := Result.Top - (ButtonSettings.RealButtonSize * cnt)
      end;
      if (ButtonSettings.PageListButton) then
      ...
      begin
        if not ButtonSettings.FullHeight then
          ...
          Result.Left := Result.Right - Round(FDPIScale * DEFAULT_TABHEIGHT);
      end;
    end;
    tpRight:
    begin
      ...
      begin
        ...
        Result.Left := Result.Right - TabSettings.RealHeight - FTabOffSet + Round(4 * FDPIScale);
...
begin
  ...
  case TabPosition of
    ...
    begin
      ...
      Result.Right := GetButtonsRect.Left;
    ...
    begin
      ...
      Result.Bottom := GetButtonsRect.Top;
      if ButtonSettings.ShowInsertButton then
  ...
  begin
    ...
    if (csDesigning in ComponentState) then  // No fixedtabs at design time
      ...
      begin
        ...
        Canvas.Font.Assign(TabAppearance.Font);
        if TabAppearance.FTextSelectedBold then
		      Canvas.Font.Style := [fsBold];
...
begin
  ...
  begin
    ...
    begin
      ...
      if TabAppearance.TextSelectedBold then
        Canvas.Font.Style := [fsBold];
...
begin
  ...
  VisTabCount := 0;
  FUseMaxSpace := False;
  if ButtonSettings.ScrollButtonsAlways then
    ...
    begin
      case TabPosition of
        ...
        begin
          ...
          begin
            Result.Top := Result.Top + Round(4 * FDPIScale);
          end;
        end;
        tpLeft:
        begin
      ...
      case TabPosition of
        ...
        begin
          ...
          begin
            ...
            begin
              Result.Right := Result.Left + ButtonSettings.RealButtonSize;
              Result.Top := Result.Top + Round(FDPIScale);
            end;
          end;
        end;
        tpLeft:
  ...
  begin
    ...
    case TabPosition of
      ...
      begin
        if not ButtonSettings.FullHeight then
        begin
          Result.Right := Result.Left + 3 + (ButtonSettings.RealButtonSize * cnt + 2 + 3);
          Result.Bottom := Result.Bottom - 5;
          Result.Top := Result.Bottom - ButtonSettings.RealButtonSize;
        end
        else
          Result.Right := Result.Left + (ButtonSettings.RealButtonSize * cnt);
      end;
      tpBottom:
      begin
        if not ButtonSettings.FullHeight then
      ...
      begin
        ...
        begin
          ...
          Result.Bottom := Result.Top + 3 + (ButtonSettings.RealButtonSize * cnt);
      end;
      tpRight:
      begin
...
begin
  ...
  begin
    ...
    RoundedBtnSize := ButtonSettings.RealButtonSize;
    if not Odd(ButtonSettings.RealButtonSize) and FButtonSettings.FullHeight then
        Inc(RoundedBtnSize);
    ...
    case TabPosition of
      ...
      begin
        ...
        if not ButtonSettings.FullHeight then
        begin
          Result.Right := Result.Left + RoundedBtnSize + 6;
          Result.Top := R.Top + 5;
          Result.Bottom := R.Bottom - 8;
        end
        else
        begin
          Result.Right := Result.Left + RoundedBtnSize;
          Result.Top := R.Top;
          Result.Bottom := R.Bottom - Round(FDPIScale * 5);
        end;
      end;
      tpBottom:
      begin
        ...
        begin
          Result.Right := Result.Left + RoundedBtnSize;
          Result.Top := R.Top + Round(FDPIScale * 5);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Result.Bottom := Result.Top + RoundedBtnSize + 6;
          end
          else
          begin
            Result.Left := R.Left;
            Result.Right := R.Right - Round(FDPIScale * 5);
            Result.Top := R.Bottom + gap;
            Result.Bottom := Result.Top + RoundedBtnSize;
          end;
        end
        else
        begin
          ...
          begin
            Result.Right := R.Right - 8;
            Result.Left := Result.Right - RoundedBtnSize - 1;
            Result.Top := R.Bottom + gap;
            Result.Bottom := Result.Top + RoundedBtnSize + 6;
          end
          else
          begin
            Result.Right := R.Right - Round(FDPIScale * 5);
            Result.Left := Result.Right - RoundedBtnSize;
            Result.Top := R.Bottom + gap;
            Result.Bottom := Result.Top + Round(FDPIScale * DEFAULT_TABHEIGHT);
          end;
        end;
      end;
      tpRight:
      begin
        ...
        begin
          ...
          begin
            Result.Left := R.Left + Round(FDPIScale * 5);
        ...
        begin
          ...
          begin
            Result.Left := R.Left + 8;
            Result.Right := Result.Left + RoundedBtnSize + 1;
            Result.Top := R.Bottom + gap;
            Result.Bottom := Result.Top + RoundedBtnSize + 6;
          end
          else
...
begin
  ...
  begin
    ...
    ButtonSettings.FullHeight := AdvOfficePagerStyler.TabAppearance.DrawLine;
    TabSettings.ButtonBorder := not AdvOfficePagerStyler.TabAppearance.DrawLine;
    if ButtonSettings.FullHeight then
      ButtonSettings.ButtonSize := 13
    else
      ButtonSettings.ButtonSize := PAGEBUTTON_SIZE;

    ButtonSettings.FBtnPenColor := AdvOfficePagerStyler.GlowButtonAppearance.TextColor;
    ButtonSettings.FBtnColor := AdvOfficePagerStyler.GlowButtonAppearance.Color;
    ButtonSettings.FBtnHotColor := AdvOfficePagerStyler.GlowButtonAppearance.ColorHot;
    ButtonSettings.FBtnDownColor := AdvOfficePagerStyler.GlowButtonAppearance.ColorDown;
  end
  else if FSelfStyler and Assigned(FInternalOfficePagerStyler) then
  begin
    TabSettings.Rounding := FInternalOfficePagerStyler.TabRounding;

    ButtonSettings.FullHeight := FInternalOfficePagerStyler.TabAppearance.DrawLine;
    TabSettings.ButtonBorder := not FInternalOfficePagerStyler.TabAppearance.DrawLine;
    if ButtonSettings.FullHeight then
      ButtonSettings.ButtonSize := 13
    else
      ButtonSettings.ButtonSize := PAGEBUTTON_SIZE;

    ButtonSettings.FBtnPenColor := FInternalOfficePagerStyler.GlowButtonAppearance.TextColor;
    ButtonSettings.FBtnColor := FInternalOfficePagerStyler.GlowButtonAppearance.Color;
    ButtonSettings.FBtnHotColor := FInternalOfficePagerStyler.GlowButtonAppearance.ColorHot;
    ButtonSettings.FBtnDownColor := FInternalOfficePagerStyler.GlowButtonAppearance.ColorDown;
  end;
end;
...
begin
  ...
  begin
  {$ENDIF}
    if not Assigned(AdvOfficePagerStyler) then
    begin

      {$IFDEF DELPHIXE2_LVL}
      if CheckVCLStylesEnabled(StyleServices, (csDesigning in ComponentState)) {$IFDEF DELPHIXE6_LVL} and (seClient in StyleElements){$ENDIF} then
      begin
        FCurrentOfficePagerStyler.TabAppearance.Font.Height := MulDiv(FCurrentOfficePagerStyler.TabAppearance.Font.Height,M,D);
      end
      else
      {$ENDIF}
        FCurrentOfficePagerStyler.TabAppearance.Font.Height := MulDiv(FCurrentOfficePagerStyler.TabAppearance.Font.Height, M, D);
...
begin
  ...
  FTabAppearance.ColorTo := FTabAppearance.BackGround.Color;
  FTabAppearance.ColorMirror := FTabAppearance.BackGround.Color;
  FTabAppearance.ColorMirrorTo := FTabAppearance.BackGround.Color;

  FTabAppearance.TextColor := ATones.Background.TextColor;
  FTabAppearance.ColorSelected := ATones.Background.BrushColor;
  FTabAppearance.ColorSelectedTo := ATones.Background.BrushColor;
  FTabAppearance.ColorMirrorSelected := ATones.Background.BrushColor;
  FTabAppearance.ColorMirrorSelectedTo := ATones.Background.BrushColor;

  FTabAppearance.BorderColorSelected := ATones.Background.BrushColor;
  if (ATones.Background.TextColor = ATones.Background.BrushColor) then
    FTabAppearance.TextColorSelected := ATones.Selected.BrushColor
  else
    FTabAppearance.TextColorSelected :=  ATones.Background.TextColor;
  FTabAppearance.ColorHot := ATones.Hover.BrushColor;
  FTabAppearance.ColorHotTo :=  ATones.Hover.BrushColor;
  FTabAppearance.ColorMirrorHot :=  ATones.Hover.BrushColor;
  FTabAppearance.ColorMirrorHotTo :=  ATones.Hover.BrushColor;
  ...
  FGlowButtonAppearance.BorderColor := clNone;
...
procedure TAdvOfficePager.SetComponentStyle(AStyle: TTMSStyle);
var
  tempStyler: TAdvOfficePagerOfficeStyler;
begin
  if not Assigned(FOfficePagerStyler) then
  begin
    FTMSStyle := AStyle;
    if not (AStyle = tsCustom) then
    begin
      FInternalStyler := True;
      FSelfStyler := True;
      tempStyler := TAdvOfficePagerOfficeStyler.Create(Self);
      tempStyler.SetComponentStyle(AStyle);
      FInternalOfficePagerStyler.Assign(tempStyler);
      tempStyler.Free;
      FInternalStyler := False;
    end;
  end
...
procedure TAdvOfficePager.SetComponentStyleAndAppColor(AStyle: TTMSStyle;
  AppColor: TColor);
var
  tempStyler: TAdvOfficePagerOfficeStyler;
begin
  ...
  begin
    ...
    FTMSStyle := AStyle;
    tempStyler := TAdvOfficePagerOfficeStyler.Create(Self);
    tempStyler.SetComponentStyleAndAppColor(AStyle, AppColor);
    FInternalOfficePagerStyler.Assign(tempStyler);
    tempStyler.Free;
    FInternalStyler := False;
  end
...
begin
  ...
  begin
    ...
    begin
      for i := 0 to AdvPageCount - 1 do
      begin
        if AdvPages[i].TabIndex >= idx then
          AdvPages[i].TabIndex := AdvPages[i].TabIndex - 1;
      end;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if ButtonSettings.FullHeight then
      begin
        FCloseButtonGlobal.Rounded := False;
        FCloseButtonGlobal.Picture := nil;
        FCloseButtonGlobal.OnDrawButton := OnDrawCloseButton;
      end
      else
      begin
        FCloseButtonGlobal.Rounded := True;
        FCloseButtonGlobal.OnDrawButton := nil;
      ...
      FCloseButtonGlobal.Enabled := GetVisibleTabCount > 0;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                {$IFDEF DELPHIXE_LVL}
                PostMessage(Self.Handle, WM_OPDESTROYCLOSEBTN, nativeuint(Pointer(FCloseButtonChrome)), 1);
                {$ENDIF}
                {$IFNDEF DELPHIXE_LVL}
              ...
              if ButtonSettings.FullHeight then
              begin
                FCloseButton.Rounded := False;
                FCloseButton.Picture := nil;
                FCloseButton.OnDrawButton := OnDrawCloseButton;
              end
              else
              begin
                FCloseButton.Rounded := True;
                FCloseButton.OnDrawButton := nil;
            ...
            begin
              ...
              begin
                {$IFDEF DELPHIXE_LVL}
                PostMessage(Self.Handle, WM_OPDESTROYCLOSEBTN, nativeuint(Pointer(FCloseButton)), 0);
                {$ENDIF}
                {$IFNDEF DELPHIXE_LVL}
              ...
              FCloseButtonChrome.Left := R.Left + ((R.Right - R.Left) - s) div 2;
              FCloseButtonChrome.Top := R.Top + ((R.Bottom - R.Top) - s) div 2;
              FCloseButtonChrome.Width := s;
              FCloseButtonChrome.Height := s;

              FCloseButtonChrome.Tag := I;

              FCloseButtonChrome.Hint := ButtonSettings.CloseButtonHint;
              FCloseButtonChrome.ShowHint := True;
              FCloseButtonChrome.Enabled := GetVisibleTabCount > 0;
  ...
  begin
    ...
    if ButtonSettings.FullHeight then
    begin
      FPageListButton.Rounded := False;
      FPageListButton.Picture := nil;
      FPageListButton.OnDrawButton := OnDrawPageButton;
    end
    else
    begin
      FPageListButton.Rounded := True;
      FPageListButton.OnDrawButton := nil;
      if (TabPosition in [tpLeft, tpRight]) then
          FPageListButton.Picture.Assign(ButtonSettings.FScrollButtonNextPicture)
      else
    ...
    FPageListButton.Enabled := GetVisibleTabCount > 0;
  end
  else
  if (FPageListButton <> nil) then
  begin
  ...
  begin
    ...
    if ButtonSettings.FullHeight then
    begin
      FScrollFirstButton.Rounded := False;
      FScrollFirstButton.Picture := nil;
      FScrollFirstButton.OnDrawButton := OnDrawFirstButton;
    end
    else
    begin
      FScrollFirstButton.Rounded := True;
      FScrollFirstButton.OnDrawButton := nil;
      if (TabPosition in [tpLeft, tpRight]) then
      begin
    ...
    if ButtonSettings.FullHeight then
    begin
      FScrollLastButton.Rounded := False;
      FScrollLastButton.Picture := nil;

      FScrollLastButton.OnDrawButton := OnDrawLastButton;
    end
    else
    begin
      FScrollLastButton.Rounded := True;
      FScrollLastButton.OnDrawButton := nil;
      if (TabPosition in [tpLeft, tpRight]) then
      begin
    ...
    if ButtonSettings.FullHeight then
    begin
      FScrollPrevButton.Rounded := False;
      FScrollPrevButton.Picture := nil;
      FScrollPrevButton.OnDrawButton := OnDrawPrevButton;
    end
    else
    begin
      FScrollPrevButton.Rounded := True;
      FScrollPrevButton.OnDrawButton := nil;
      if (TabPosition in [tpLeft, tpRight]) then
      begin
    ...
    if ButtonSettings.FullHeight then
    begin
      FScrollNextButton.Rounded := False;
      FScrollNextButton.Picture := nil;
      FScrollNextButton.OnDrawButton := OnDrawNextButton;
    end
    else
    begin
      FScrollNextButton.Rounded := True;
      FScrollNextButton.OnDrawButton := nil;
      if (TabPosition in [tpLeft, tpRight]) then
      begin
...
procedure TAdvOfficePager.OnDrawCloseButton(Sender: TObject; Canvas: TCanvas;
  Rect: TRect; State: TGlowButtonState);
var
  s: integer;
begin
  Canvas.Pen.Color := ButtonSettings.FBtnPenColor;
  Canvas.Pen.Width := 1; //Round(FDPIScale);
  if (Rect.Bottom - Rect.Top) < (Rect.Right - Rect.Left) then
    s := (Rect.Bottom - Rect.Top) div 2
  else
    s := (Rect.Right - Rect.Left) div 2;

  Canvas.MoveTo(Rect.Left + (Rect.Right - Rect.Left - s) div 2, Rect.Top + (Rect.Bottom - Rect.Top - s) div 2);
  Canvas.LineTo(Rect.Left +  (Rect.Right - Rect.Left - s) div 2 + s + 1, Rect.Top + (Rect.Bottom - Rect.Top - s) div 2 + s + 1);
  Canvas.MoveTo(Rect.Left + (Rect.Right - Rect.Left - s) div 2 + s, Rect.Top + (Rect.Bottom - Rect.Top - s) div 2);
  Canvas.LineTo(Rect.Left + (Rect.Right - Rect.Left - s) div 2 - 1, Rect.Top + (Rect.Bottom - Rect.Top - s) div 2 + s + 1);

end;

procedure TAdvOfficePager.OnDrawFirstButton(Sender: TObject; Canvas: TCanvas;
  Rect: TRect; State: TGlowButtonState);
var
  s: integer;
begin
  ...
  Canvas.Pen.Width := 1; //Round(FDPIScale);
  if (Rect.Bottom - Rect.Top) < (Rect.Right - Rect.Left) then
    s := (Rect.Bottom - Rect.Top) div 3
  else
    s := (Rect.Right - Rect.Left) div 3;

  case TabPosition of
    tpTop, tpBottom:
    begin
      Canvas.MoveTo(Rect.Left + ((Rect.Right - Rect.Left) - 2 * s) div 2, Rect.Top + (Rect.Bottom - Rect.Top) div 2);
      Canvas.LineTo(Rect.Left + ((Rect.Right - Rect.Left) - 2 * s) div 2 + s, Rect.Top + (Rect.Bottom - Rect.Top) div 2 - s);
      Canvas.MoveTo(Rect.Left + ((Rect.Right - Rect.Left) - 2 * s) div 2, Rect.Top + (Rect.Bottom - Rect.Top) div 2);
      Canvas.LineTo(Rect.Left + ((Rect.Right - Rect.Left) - 2 * s) div 2 + s, Rect.Top + (Rect.Bottom - Rect.Top) div 2 + s);
      Canvas.MoveTo(Rect.Left + ((Rect.Right - Rect.Left) - 2 * s) div 2 + s, Rect.Top + (Rect.Bottom - Rect.Top) div 2);
      Canvas.LineTo(Rect.Left + ((Rect.Right - Rect.Left) - 2 * s) div 2 + 2 * s, Rect.Top + (Rect.Bottom - Rect.Top) div 2 - s);
      Canvas.MoveTo(Rect.Left + ((Rect.Right - Rect.Left) - 2 * s) div 2 + s, Rect.Top + (Rect.Bottom - Rect.Top) div 2);
      Canvas.LineTo(Rect.Left + ((Rect.Right - Rect.Left) - 2 * s) div 2 + 2 * s, Rect.Top + (Rect.Bottom - Rect.Top) div 2 + s);
    end;
    tpLeft, tpRight:
    begin
      Canvas.MoveTo(Rect.Left + (Rect.Right - Rect.Left) div 2, Rect.Top + ((Rect.Bottom - Rect.Top) - 2 * s) div 2);
      Canvas.LineTo(Rect.Left + (Rect.Right - Rect.Left) div 2 - s, Rect.Top + ((Rect.Bottom - Rect.Top) - 2 * s) div 2 + s);
      Canvas.MoveTo(Rect.Left + (Rect.Right - Rect.Left) div 2, Rect.Top + ((Rect.Bottom - Rect.Top) - 2 * s) div 2);
      Canvas.LineTo(Rect.Left + (Rect.Right - Rect.Left) div 2 + s, Rect.Top + ((Rect.Bottom - Rect.Top) - 2 * s) div 2 + s);
      Canvas.MoveTo(Rect.Left + (Rect.Right - Rect.Left) div 2, Rect.Top + ((Rect.Bottom - Rect.Top) - 2 * s) div 2 + s);
      Canvas.LineTo(Rect.Left + (Rect.Right - Rect.Left) div 2 - s, Rect.Top + ((Rect.Bottom - Rect.Top) - 2 * s) div 2  + 2 * s);
      Canvas.MoveTo(Rect.Left + (Rect.Right - Rect.Left) div 2, Rect.Top + ((Rect.Bottom - Rect.Top) - 2 * s) div 2 + s);
      Canvas.LineTo(Rect.Left + (Rect.Right - Rect.Left) div 2 + s, Rect.Top + ((Rect.Bottom - Rect.Top) - 2 * s) div 2 + 2 * s);
    end;
  end;
...
procedure TAdvOfficePager.OnDrawLastButton(Sender: TObject; Canvas: TCanvas;
  Rect: TRect; State: TGlowButtonState);
var
  s: integer;
begin
  ...
  case TabPosition of
    ...
    begin
      Canvas.MoveTo(Rect.Left + ((Rect.Right - Rect.Left) - 2 * s) div 2 + s, Rect.Top + (Rect.Bottom - Rect.Top) div 2);
      Canvas.LineTo(Rect.Left + ((Rect.Right - Rect.Left) - 2 * s) div 2, Rect.Top + (Rect.Bottom - Rect.Top) div 2 - s);
      Canvas.MoveTo(Rect.Left + ((Rect.Right - Rect.Left) - 2 * s) div 2 + s, Rect.Top + (Rect.Bottom - Rect.Top) div 2);
      Canvas.LineTo(Rect.Left + ((Rect.Right - Rect.Left) - 2 * s) div 2, Rect.Top + (Rect.Bottom - Rect.Top) div 2 + s);
      Canvas.MoveTo(Rect.Left + ((Rect.Right - Rect.Left) - 2 * s) div 2 + 2 * s, Rect.Top + (Rect.Bottom - Rect.Top) div 2);
      Canvas.LineTo(Rect.Left + ((Rect.Right - Rect.Left) - 2 * s) div 2 + s, Rect.Top + (Rect.Bottom - Rect.Top) div 2 - s);
      Canvas.MoveTo(Rect.Left + ((Rect.Right - Rect.Left) - 2 * s) div 2 + 2 * s, Rect.Top + (Rect.Bottom - Rect.Top) div 2);
      Canvas.LineTo(Rect.Left + ((Rect.Right - Rect.Left) - 2 * s) div 2 + s, Rect.Top + (Rect.Bottom - Rect.Top) div 2 + s);
    end;
    ...
    begin
      Canvas.MoveTo(Rect.Left + (Rect.Right - Rect.Left) div 2, Rect.Top + ((Rect.Bottom - Rect.Top) - 2 * s) div 2 + s);
      Canvas.LineTo(Rect.Left + (Rect.Right - Rect.Left) div 2 - s, Rect.Top + ((Rect.Bottom - Rect.Top) - 2 * s) div 2);
      Canvas.MoveTo(Rect.Left + (Rect.Right - Rect.Left) div 2, Rect.Top + ((Rect.Bottom - Rect.Top) - 2 * s) div 2 + s);
      Canvas.LineTo(Rect.Left + (Rect.Right - Rect.Left) div 2 + s, Rect.Top + ((Rect.Bottom - Rect.Top) - 2 * s) div 2);
      Canvas.MoveTo(Rect.Left + (Rect.Right - Rect.Left) div 2, Rect.Top + ((Rect.Bottom - Rect.Top) - 2 * s) div 2 + 2 * s);
      Canvas.LineTo(Rect.Left + (Rect.Right - Rect.Left) div 2 - s, Rect.Top + ((Rect.Bottom - Rect.Top) - 2 * s) div 2 + s);
      Canvas.MoveTo(Rect.Left + (Rect.Right - Rect.Left) div 2, Rect.Top + ((Rect.Bottom - Rect.Top) - 2 * s) div 2 + 2 * s);
      Canvas.LineTo(Rect.Left + (Rect.Right - Rect.Left) div 2 + s, Rect.Top + ((Rect.Bottom - Rect.Top) - 2 * s) div 2 + s);
    end;
...
procedure TAdvOfficePager.OnDrawNextButton(Sender: TObject; Canvas: TCanvas;
  Rect: TRect; State: TGlowButtonState);
var
  s: integer;
begin
  ...
  case TabPosition of
    tpTop, tpBottom:
    begin
      Canvas.MoveTo(Rect.Left + ((Rect.Right - Rect.Left) - s) div 2 + s, Rect.Top + (Rect.Bottom - Rect.Top) div 2);
      Canvas.LineTo(Rect.Left + ((Rect.Right - Rect.Left) - s) div 2, Rect.Top + (Rect.Bottom - Rect.Top) div 2 - s);
      Canvas.MoveTo(Rect.Left + ((Rect.Right - Rect.Left) - s) div 2 + s, Rect.Top + (Rect.Bottom - Rect.Top) div 2);
      Canvas.LineTo(Rect.Left + ((Rect.Right - Rect.Left) - s) div 2, Rect.Top + (Rect.Bottom - Rect.Top) div 2 + s);
    end;
    tpLeft, tpRight:
    begin
      Canvas.MoveTo(Rect.Left + (Rect.Right - Rect.Left) div 2, Rect.Top + ((Rect.Bottom - Rect.Top) - s) div 2 + s);
      Canvas.LineTo(Rect.Left + (Rect.Right - Rect.Left) div 2 - s, Rect.Top + ((Rect.Bottom - Rect.Top) - s) div 2 );
      Canvas.MoveTo(Rect.Left + (Rect.Right - Rect.Left) div 2, Rect.Top + ((Rect.Bottom - Rect.Top) - s) div 2 + s);
      Canvas.LineTo(Rect.Left + (Rect.Right - Rect.Left) div 2 + s, Rect.Top + ((Rect.Bottom - Rect.Top) - s) div 2);
    end;
  end;
...
procedure TAdvOfficePager.OnDrawPageButton(Sender: TObject; Canvas: TCanvas;
  Rect: TRect; State: TGlowButtonState);
var
  s: integer;
begin
...
procedure TAdvOfficePager.OnDrawPrevButton(Sender: TObject; Canvas: TCanvas;
  Rect: TRect; State: TGlowButtonState);
var
  s: integer;
begin
  ...
  case TabPosition of
    tpTop, tpBottom:
    begin
      Canvas.MoveTo(Rect.Left + ((Rect.Right - Rect.Left) - s) div 2, (Rect.Bottom - Rect.Top) div 2);
      Canvas.LineTo(Rect.Left + ((Rect.Right - Rect.Left) - s) div 2 + s, (Rect.Bottom - Rect.Top) div 2 - s);
      Canvas.MoveTo(Rect.Left + ((Rect.Right - Rect.Left) - s) div 2, (Rect.Bottom - Rect.Top) div 2);
      Canvas.LineTo(Rect.Left + ((Rect.Right - Rect.Left) - s) div 2 + s, (Rect.Bottom - Rect.Top) div 2 + s);
    end;
    tpLeft, tpRight:
    begin
      Canvas.MoveTo(Rect.Left + (Rect.Right - Rect.Left) div 2, ((Rect.Bottom - Rect.Top) - s) div 2);
      Canvas.LineTo(Rect.Left + (Rect.Right - Rect.Left) div 2 - s, ((Rect.Bottom - Rect.Top) - s) div 2 + s);
      Canvas.MoveTo(Rect.Left + (Rect.Right - Rect.Left) div 2, ((Rect.Bottom - Rect.Top) - s) div 2);
      Canvas.LineTo(Rect.Left + (Rect.Right - Rect.Left) div 2 + s, ((Rect.Bottom - Rect.Top) - s) div 2 + s);
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          {$IFDEF DELPHIXE_LVL}
          PostMessage(Handle, WM_OPDESTROYCLOSEBTN, nativeuint(Pointer(AdvPages[ActTabIndex].FCloseButton)), 0);
          {$ENDIF}
          {$IFNDEF DELPHIXE_LVL}
        ...
        begin
          {$IFDEF DELPHIXE_LVL}
          PostMessage(Handle, WM_OPDESTROYCLOSEBTN, nativeuint(Pointer(AdvPages[ActTabIndex].FCloseButtonChrome)), 1);
          {$ENDIF}
          {$IFNDEF DELPHIXE_LVL}
...
procedure TAdvOfficePager.AnimateLine;
begin

  if (csDesigning in ComponentState) then
  begin
    FCurLineStart := FNewLineStart;
    FCurLineEnd := FNewLineEnd;
    Exit;
  end;

  FAnimating := true;
  FDeltaStart := Abs((FNewLineStart - FCurLineStart) / 5);
  FDeltaEnd := Abs((FNewLineEnd - FCurLineEnd) / 5);

  if not Assigned(FDLTimer) then
  begin
    FDLTimer := TTimer.Create(self);
    FDLTimer.OnTimer := OnDrawLineTimerTime;
    FDLTimer.Interval := 20;
    FDLTimer.Enabled := True;
  end
  else
  begin
    FDLTimer.Enabled := True;
...
procedure TAdvOfficePager.OnDrawLineTimerTime(Sender: TObject);
begin
  if(FNewLineStart > FCurLineStart) then
  begin
    FCurLineStart := FCurLineStart + FDeltaStart;
  end
  else
    FCurLineStart := FCurLineStart - FDeltaStart;

  if(FNewLineEnd > FCurLineEnd) then
  begin
    FCurLineEnd := FCurLineEnd + FDeltaEnd;
  end
  else
    FCurLineEnd := FCurLineEnd - FDeltaEnd;

  if(FCurLineStart - 1 < FNewLineStart) and (FCurLineStart + 1 > FNewLineStart) then
    FCurLineStart := FNewLineStart;
  if(FCurLineEnd - 1 < FNewLineEnd) and (FCurLineEnd + 1 > FNewLineEnd) then
    FCurLineEnd := FNewLineEnd;

  if ((FCurLineStart = FNewLineStart) and (FCurLineEnd = FNewLineEnd)) then
  begin
    FDLTimer.Enabled := False;
    FAnimating := False;
  end;
...
procedure TAdvOfficePager.StoreTabFontHeight(Writer: TWriter);
begin
  if Assigned(FTabAppearance) then
    Writer.WriteInteger(FTabAppearance.Font.Height)
  else
    Writer.WriteInteger(FCurrentOfficePagerStyler.TabAppearance.Font.Height);
end;
...
begin
  ...
  Filer.DefineProperty('TabFontHeight', LoadTabFontHeight, StoreTabFontHeight, True);
end;
...
begin
  ...
  begin
    ...
    if ButtonSettings.FullHeight then
    begin
      FClosedListButton.Rounded := False;
      FClosedListButton.Picture := nil;
      FClosedListButton.OnDrawButton := OnDrawPageButton;
    end
    else
    begin
      FClosedListButton.Rounded := True;
      FClosedListButton.OnDrawButton := nil;
    ...
    if ButtonSettings.FullHeight then
    begin
      FClosedListButton.Width := R.Right - R.Left;
      FClosedListButton.Height := R.Bottom - R.Top;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    if AdvPages[I].Glow and (AdvPages[I].GlowColor <> clNone) then
    begin
      Result := True;
      Break;
    end;
