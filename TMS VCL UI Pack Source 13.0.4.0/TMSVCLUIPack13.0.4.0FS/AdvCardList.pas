...
const
  ...
  // v2.4.1.0 : New : Internal Office styler implemented
  // v2.4.1.1 : Improved : On creation check for enabled AdvFormStyler
  // v2.4.2.0 : Improved : Themed high DPI rendering
  // v2.4.2.1 : Fixed : VCL Styles check updated to work in 10.4 Sydney
  // v2.4.2.2 : Fixed : VCL Styles are not used when seClient is excluded from StyleElements
  // v2.4.2.3 : Improved : Changed Images property to TCustomImageList type
  // v2.5.0.0 : New : Support for Emoji rendering in HTML
  // v2.5.1.0 : New : Event OnCardItemAppearance added for dynamic appearance configuration per card item
  // v2.5.2.0 : New : Made properties OleDropTarget / OleDropSource public properties
  // v2.5.3.0 : New : Default parameter IncludeInvisible: boolean = false for SelectAll method
  // v2.5.4.0 : New : OnCardBeforeEdit event added, to intercept editor before editing starts
...
type
  ...
  TCardItemTemplateEvent = procedure(Sender: TObject; ACard: TAdvCard; AItem: TAdvCardItem; ATemplate: TAdvCardTemplateItem) of object;
  ...
  TCustomAdvCardList = class(TCustomControl, ITMSStyle)
  private
    FScrolling : Boolean;
    ...
    FTMSStyle: TTMSStyle;
    FStyler: TObject;
    FOnCardItemAppearance: TCardItemTemplateEvent;
    FOnCardBeforeEdit: TAdvCardItemEvent;
  ...
  public
    ...
    property OnCardBeforeEdit: TAdvCardItemEvent read FOnCardBeforeEdit write FOnCardBeforeEdit;
    ...
    property OnCardItemAppearance: TCardItemTemplateEvent read FOnCardItemAppearance write FOnCardItemAppearance;
    ...
    procedure SelectAll(IncludeInvisible: boolean = false);
    ...
    property Styler: TObject read FStyler write FStyler;
  published
...
uses
  CommCtrl, AdvCardListStyler, AdvHTML
{$IFDEF DELPHIXE2_LVL}
  ,VCL.Themes
{$ENDIF}
  ;
...
begin
  ...
  begin
    ...
    outputdebugstring('set ole drop target');
...
procedure TCustomAdvCardList.SelectAll(IncludeInvisible: boolean = false);
var
  CardN: Integer;
begin
  ...
  begin
    ...
    for CardN := 0 to FCards.Count - 1 do
      if (FCards.Items[CardN].Filtered and FCards.Items[CardN].Visible) or IncludeInvisible then
        FCards.Items[CardN].Selected := True;
...
begin
  ...
  begin
    ...
    if Assigned(FOnCardBeforeEdit) then
      FOnCardBeforeEdit(Self, CardIndex, ItemIndex);
...
procedure TCustomAdvCardList.SetComponentStyle(AStyle: TTMSStyle);
var
  tempStyler: TAdvCardListStyler;
begin
  ...
  begin
    ...
    begin
      tempStyler := TAdvCardListStyler.Create(Self);
      tempStyler.CardList := Self;
      tempStyler.SetComponentStyle(AStyle);
      tempStyler.Free;

      if (csDesigning in ComponentState) and Assigned(Cards) and (Cards.Count > 3) then
      begin
        Cards[0].FAppearance.Assign(FCardNormalAppearance);
        Cards[1].FAppearance.Assign(FCardHoverAppearance);
        Cards[2].FAppearance.Assign(FCardSelectedAppearance);
        Cards[3].FAppearance.Assign(FCardEditingAppearance);
      end;
    end;
  end;
end;
...
procedure TCustomAdvCardList.SetUIStyle(const Value: TTMSStyle);
begin
  FTMSStyle := Value;
...
var
  S: string;
  R: TRect;
  LItem,TItem: TAdvCardTemplateItem;
  DrawFlags: Cardinal;
  {$IFDEF DELPHIXE2_LVL}
  lDetails: TThemedElementDetails;
  {$ENDIF}
begin
  LItem := CardTemplate.Items[ItemIndex];

  TItem := TAdvCardTemplateItem.Create(nil);
  TItem.Assign(LItem);

  if Assigned(OnCardItemAppearance) then
    OnCardItemAppearance(Self, Card, Card.ItemList[ItemIndex], TItem);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        cr := Rect(r.Left, r.Top, r.Left + sz.cx, r.Top + sz.cy);

        if Item.AsBoolean then
          DrawThemeBackground(HTheme,Canvas.Handle, BP_CHECKBOX,CBS_CHECKEDNORMAL,cr,nil)
        else
         DrawThemeBackground(HTheme,Canvas.Handle, BP_CHECKBOX,CBS_UNCHECKEDNORMAL,cr,nil);
