...
procedure TOutlookGroupedList.AdjustItemsSize;
  ...
  begin
    ...
    begin
      ...
      Inc(FTotalItemsHeight, Round(FDPIScale * FGroupItemHeight));
...
begin
  ...
  if ShowNodes then
  begin
    FGroupItemExpandButtonVertOffset := Round(FDPIScale * FGroupItemHeight) - 7 - tmpGroupFontHeight +
...
begin
  ...
  begin
    ...
    FDPIScale := GetDPIScale(Self, TargetCanvas);

    TargetCanvas.Font.Assign(GroupFont{Self.Font});
    TargetCanvas.Font.Height := Round(GroupFont.Height * FDPIScale);
    ...
    TargetCanvas.Font.Height := Round(FDPIScale * GroupCountFont.Height);
...
begin
  ...
  rc := Rect(0, - VertScrollBarPos, ClientWidth,
    -VertScrollBarPos + Round(FDPIScale * FGroupItemHeight));
  ...
  try
    ...
    begin
      ...
      begin
        Inc(rc.Top, Round(FDPIScale * FGroupItemHeight));
        ...
        rc.Top := rc.Bottom;
        rc.Bottom := rc.Top + Round(FDPIScale * FGroupItemHeight);
      end else
        OffsetRect(rc, 0, Round(FDPIScale * FGroupItemHeight));
      GroupItem := GroupItem.NextSibling;
    end;
  finally
    // draw Selection Rect
  ...
  try
    ...
    begin
      ...
      if (not IsGroupItem(Item)) and (Item.Index < FItemsPerRow) then
        Dec(DisplayRect.Top, Round(FDPIScale * FGroupItemHeight));
