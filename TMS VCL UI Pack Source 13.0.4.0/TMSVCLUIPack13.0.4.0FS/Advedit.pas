...
uses
  ...
  StdCtrls, Mask, AdvEdDD, Types, AdvStyleIF, CommCtrl, AdvToolTip
...
const
  ...
  // v3.5.0.2 : Improved : OnKeyDown handler when keys are handled that shouldn't be allowed
  // v3.5.0.3 : Improved : Left alignment in combination with top aligned label
  // v3.5.1.0 : New : OnOLEDropText event added
  //          : New : File OLE drag & drop support added
  //          : Fixed : Issue with LengthLimit for text entry
  // v3.5.1.1 : Fixed : Issue with Undo handling after paste
  // v3.5.2.0 : Improved : Office black style handling
  // v3.5.2.1 : Fixed : Issue with OnChange triggering when EditType = etFloat or etMoney
  //          : Fixed : DefaultHandling = true with form.KeyPreview = true
  // v3.5.2.2 : Fixed : Issue with Signed and TDBAdvEdit DataSet edit state
  // v3.5.2.3 : Improved : Label uses StyleElements
  // v3.5.2.4 : Fixed : Issue with label font size handling in high DPI scenarios
  // v3.5.2.5 : Fixed : Regression with label font in specific scenario
  // v3.5.2.6 : Fixed : Issue with OnChange handler for setting the .Value property programmatic
  // v3.5.3.0 : New : UIStyle property added and updated style colors
  // v3.5.3.1 : Fixed : Issue with text selection in etMoney edit type in TDBAdvEdit
  // v3.5.3.2 : Fixed : Issue with label font for high DPI
  // v3.5.3.3 : Fixed : Issue with Form.KeyPreview and default handling of ESC & RETURN keys
  //          : Fixed : Issue with ESC handling in TDBAdvEdit
  // v3.5.3.4 : Fixed : Rare issue with high DPI when using ShowError = true
  //          : Fixed : Color consistancy in Office styles
  //          : Fixed : ParentFont kept true on initialization
  // v3.5.3.5 : Fixed : Small issue with control color initialization
  // v3.5.4.0 : New : HexValue property when Edit Type is etHex
  // v3.5.4.1 : Fixed : Issue with DisabledTextColor and drawing Prefix/Suffix
  //          : Fixed : Issue with OnChange for setting values programmatically with FloatValue/IntValue
  // v3.5.4.2 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // v3.5.4.3 : Fixed : Rare issue with using return and OnChange event handler
  // v3.5.4.4 : Improved : Lookup is happening on trimmed text entered so leading spaces are ignored
  // v3.5.4.5 : Fixed : Issue with CanUndo = true and ReadOnly = true
  // v3.5.4.6 : Fixed : Rare issue with etFloat validation after clipboard paste
  // v3.5.4.7 : Fixed : Issue with text positioning when BorderStyle = bsNone
  // v3.5.4.8 : Fixed : Rare regression with updating dataset from edit OnChange event in TDBAdvEdit
  // v3.5.4.9 : Improved : Label font high DPI handling
  // v3.5.4.10: Fixed : Issue with TAdvEdit on TTitleBarPanel in Delphi 10.4
  // v3.5.4.11: Improved : Font handling when ParentFont = true
  // v3.5.5.0 : New : RangeCharacter / RangeCharacterFrequency properties added
  // v3.5.6.0 : New : Property LeadingZeros added
  // v4.0.0.0 : New : Balloon functionality in TAdvEdit, TAdvMaskEdit
  // v4.0.0.1 : Fixed : Regression with AutoSelect = true
  // v4.0.0.2 : Fixed : Rare issue with label font and value error handling
  // v4.0.0.3 : Fixed : Regression due to 4.0.0.2
  // v4.0.1.0 : Improved : AutoValidate for etFloat, etMoney type
  // v4.0.1.1 : Fixed : DPI calculation when parent form is not Scaled
  // v4.0.1.2 : Fixed : Issue with changing LabelFont at runtime
  // v4.0.1.3 : Fixed : Issue with LabelFont and high-DPI on Delphi 11
  // v4.0.2.0 : New : etInt64 type added
  // v4.0.3.0 : New : Padding added in TAdvEdit, TAdvMaskEdit
  // v4.0.3.1 : Fixed : Issue with Padding conflicting with TWinControl padding. Renamed to TextPadding
  // v4.0.3.2 : Improved : Lookup dropdown font high DPI handling
  // v4.0.3.3 : Improved : Label font size initialization at design-time for high DPI
  // v4.0.3.4 : Fixed : Rounding issue with label font height calculation in high DPI scenarios
  // v4.0.3.5 : Improved : LabelFont handling in connection with ParentFont
  // v4.0.3.6 : Fixed : Border rendering when Ctl3D = false
  // v4.0.3.7 : Improved : Optimized triggering of OnValueValidate
  // v4.0.4.0 : Improved : TAdvEdit.Lookup.Font uses parent font settings now
  // v4.0.4.1 : Fixed : Rare issue with OnEnter,OnExit with FocusAlign on frames
  // v4.0.4.2 : Fixed : Issue with LengthLimit for EditType = etMoney
  // v4.0.4.3 : Fixed : Issue with ShowError and focus change
...
type
  ...
  TAdvEditType = (etString, etNumeric, etFloat, etUppercase, etMixedCase, etLowerCase,
    etPassword, etMoney, etRange, etHex, etAlphaNumeric, etValidChars, etInvalidChars, etInt64);
  ...
  TEditOLEDropTextEvent = procedure(Sender: TObject; var AText: string; var Allow: boolean) of object;
  ...
  TCustomAdvEdit = class(TCustomEdit, ITMSStyle)
  private
    ...
    FOnOLEDropText: TEditOLEDropTextEvent;
    FTMSStyle: TTMSStyle;
    FHexValue: longint;
    FHexDigits: Byte;
    FRangeCharacterFrequency: word;
    FRangeCharacter: string;
    FLeadingZeros: boolean;
    FToolTip: TAdvToolTip;
    FToolTipWindow: TAdvToolTipWindow;
    FTextPadding: integer;
    FLabelFontEqual: boolean;
    FLastValidate: string;
    ...
    procedure SetFloatInt(const Value: double);
    procedure SetIntInt(const Value: Integer);
  ...
  protected
    ...
    procedure DoOLEDropText(var AText: string; var Allow: boolean); virtual;
    ...
    property LeadingZeros: boolean read FLeadingZeros write FLeadingZeros default true;
    ...
    property RangeCharacter: string read FRangeCharacter write FRangeCharacter;
    property RangeCharacterFrequency: word read FRangeCharacterFrequency write FRangeCharacterFrequency default 1;
    ...
    property TextPadding: integer read FTextPadding write SetTextPadding default 0;
    property PopupMenu;
    property ReadOnly;
    property ShowHint;
    ...
    property OnOLEDropText: TEditOLEDropTextEvent read FOnOLEDropText write FOnOLEDropText;
    property UIStyle: TTMSStyle read FTMSStyle write SetUIStyle default tsCustom;
    property HexDigits: Byte read FHexDigits write SetHexDigits default 0;
    property HexValue: Integer read GetHexValue write SetHexValue default 0;
    property FloatValueInt: double read GetFloat write SetFloatInt;
    property IntValueInt: Integer read GetInt write SetIntInt;
  public
    { Public declarations }
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    ...
    property LookupList: TListHintWindow read FLookupList;
    ...
    {$ENDIF}
    {$IFNDEF DELPHIXE_LVL}
    property Int64Value: Integer read GetInt write SetInt;
    {$ENDIF}
    ...
    {$IFDEF DELPHIXE_LVL}
    class function HexToInt64(s: string): Int64;
    {$ENDIF}
  end;
  ...
  published
    ...
    property SoftBorder;
    property URLColor;
    property RangeCharacter;
    property RangeCharacterFrequency;
...
{$ENDIF}
  ...
  [ComponentPlatformsAttribute(pidWin32 or pidWin64)]
  {$ENDIF}
  TAdvCustomMaskEdit = class(TCustomMaskEdit, ITMSStyle)
  private
    { Private declarations }
    FLabel: TLabel;
    FAutoFocus: boolean;
...
function NumPos(substr, s: string): Integer;
var
  i: integer;
begin
  Result := 0;
...
{$IFDEF DELPHIXE_LVL}
class function TCustomAdvEdit.HexToInt64(s: string): Int64;
var
  i: Integer;
  r, m: Int64;
...
{$IFDEF DELPHI2007_LVL}
procedure TCustomAdvEdit.HideBalloon;
begin
  SendMessage(Handle, EM_HIDEBALLOONTIP, 0, 0);
end;
{$ENDIF}
...
var
  ...
  function AcceptLeadingZeros(ch: word): boolean;
  var
    su: string;
  begin
    Result := true;
    su := self.Text;
    if (ch = 48) and not LeadingZeros then
    begin
      delete(su, SelStart + 1, SelLength);
      if su = '' then
        Result := false;
...
begin
  ...
  if (SelLength = Length(Text)) and (Text <> '') and (EditType <> etPassword) and (Msg.CharCode <> VK_BACK) and (Msg.CharCode <> VK_RETURN) then
  begin
    FBlockChange := true;
  end;
  ...
  if (FLengthLimit > 0) and (FixedLength(Text) >= FLengthLimit) and
    (SelLength = 0) and (msg.charcode <> vk_back) and (EditType in [etFloat, etMoney]) and (SelStart < DecimalPos) and (msg.charcode <> ord(decimalseparator)) and not AllowMin(chr(msg.CharCode)) then Exit;

  if (FLengthLimit > 0) and (Length(Text) >= FLengthLimit) and
    (SelLength = 0) and (msg.charcode <> vk_back) and not (EditType in [etFloat, etMoney]) then Exit;
  ...
  if (EditType in [etMoney, etNumeric, etInt64, etFloat]) and not FSigned and (msg.charcode = ord('-')) then
  begin
    Exit;
  end;
  ...
  case EditType of
    ...
    etValidChars:
      begin
        if (pos(chr(msg.CharCode), RangeCharacter) > 0) and (NumPos(RangeCharacter, Text) < RangeCharacterFrequency) then
        begin
          SetModified(true);
          inherited;
        end;
    ...
    etNumeric, etInt64:
      ...
      // SelLength := oldSelLength;
        ...
        begin
          if not AcceptLeadingZeros(msg.CharCode) then
            Exit;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              DoEdit;
              if (floatvalueInt <> 0) or (1 > 0) then
              begin
...
begin
  ...
  if not (FEditType = etPassword) and (Uppercase(Parent.ClassName) <> 'TTITLEBARPANEL') then
  begin
...
begin
  if ((csDesigning in ComponentState) and not (csLoading in ComponentState)) and not ShowError and FLabelFontEqual then
  begin
...
procedure TCustomAdvEdit.SetUIStyle(const Value: TTMSStyle);
begin
      ...
      case FEditType of
        ...
        begin
          if not (at in [atNumeric, atHex]) then
            SetHexValue(FHexValue);
        end;
        etNumeric: if (at <> atNumeric) then self.IntValueInt := 0;
        etInt64: if (at <> atNumeric) then self.Int64Value := 0;
        etFloat, etMoney: if not (at in [atFloat, atNumeric]) then self.FloatValueInt := 0.0;
      end;
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FloatvalueInt := self.FloatvalueInt;
      Modified := OldModified;
    end;
  end;

  if (EditType in [etNumeric, etInt64]) and (Self.Text = '') and not FAllowNumericNullValue then
    Text := '0';

  if (EditType in [etFloat, etMoney]) and (Self.Text = '') and not FAllowNumericNullValue  then
    FloatvalueInt := 0.0;

 if not Assigned(OnValueValidate) and AutoValidate then
 begin
    if (EditType in [etNumeric, etInt64]) and ((FMinValue <> 0) or (FMaxValue <> 0)) then
    begin
    ...
    begin
      ...
      begin
        if d < FMinFloatValue then
          FloatValueInt := FMinFloatValue
        else
          if d > FMaxFloatValue then
            FloatValueInt := FMaxFloatValue;
      end;
    end;
  end;
  ...
  begin
    ...
    try
      if Assigned(FLabel) then
        fnt.Assign(FLabel.Font);

      Width := Width - 1;
      Width := Width + 1;

      if Assigned(FLabel) then
        FLabel.Font.Assign(fnt);
    finally
      fnt.Free;
    end;
  end;
...
begin
  ...
  if (FFocusColor <> clNone) and DoValidate(Text) and not IsError then
  begin
...
//    end;
  ...
  begin
    ...
    if not (TestURL and ShowURL) and (FFocusFontColor <> clNone) and not ParentFont then
...
procedure TCustomAdvEdit.SetTextPadding(const Value: integer);
begin
...
{$IFDEF DELPHIXE_LVL}
procedure TCustomAdvEdit.WMContextMenu(var Message: TWMContextMenu);
begin
...
{$IFNDEF DELPHI_UNICODE}
  ...
  content: PChar;
{$ENDIF}
  newstr, cliptxt, valstr, origstr: string;
...
{$ENDIF}
    ...
    begin
      origstr := cliptxt;
      FOnClipboardCanPaste(self, cliptxt, Allow);
      if Allow then
        clipmod := clipmod or (origstr <> cliptxt);
    end;

    if Assigned(FOnClipboardPaste) and Allow then
      FOnClipboardPaste(self, cliptxt, Allow);
    ...
    if (MaxLength > 0) and (length(newstr) > MaxLength) then
    begin
    ...
    case FEditType of
      ...
      etFloat, etMoney:
        begin
          ...
          begin
{$IFNDEF DELPHI_UNICODE}
            if not ((FPrecision = 0) and (Pos(DecimalSeparator, StrPas(PAnsiChar(AnsiString(newstr)))) > 0)) then
{$ENDIF}
{$IFDEF DELPHI_UNICODE}
            if not ((FPrecision = 0) and (Pos(DecimalSeparator, newstr) > 0)) then
{$ENDIF}
              begin
                if not (not Signed and (pos('-', newstr) > 0)) then
                begin
      ...
      try
        ...
        eaRight:
          begin
            ...
            DrawText(Canvas.Handle,PChar(txt),Length(txt),r,dstyle);
          end;
        eaCenter:
          begin
...
begin
  Result := Signed and (EditType in [etFloat,etNumeric,etInt64,etMoney]) and (ch = '-');
end;
...
begin
  ...
  if Signed and (EditType in [etFloat,etNumeric,etInt64,etMoney]) and (pos('-',s) > 0) then
    Result := Result - 1;
end;
...
begin
  ...
  begin
    ...
    case FEditType of
      etFloat: if not (IsType(Value) in [atFloat, atNumeric]) then
        Value := '0';
      etMoney: if not (IsType(Value) in [atFloat, atNumeric]) then
        Value := '0';
      etHex: if not (IsType(Value) in [atHex, atNumeric]) then
        Value := '0';
      etNumeric, etInt64: if not (IsType(Value) in [atNumeric]) then
        Value := '0';
    end;
...
procedure TCustomAdvEdit.ShowValidation(const AText: string);
begin
...
procedure TCustomAdvEdit.HideValidation;
begin
...
procedure TCustomAdvEdit.ShowBalloon(const ATitle, AText: string;
  AType: TBalloonType);
var
  ebt: TEditBalloonTip;
...
begin
  ...
  SendMessage(Handle, EM_SHOWBALLOONTIP, 0, LParam(@ebt));
end;
{$ENDIF}
...
begin
  ...
  d := Trunc(Abs(self.FloatValueInt));
...
begin
  ...
  begin
    ...
    if not FParentFnt or not FontIsEqual(LabelFont, Font) then
    begin
...
begin
  ...
  if not FDesignCreate and not (csLoading in ComponentState)  then
    ParentFont := false;
      ...
      lpTopLeft, lpTopCenter, lpTopRight:
        begin
          if (Align in [alTop, alClient, alRight, alLeft]) then
            ATop := ATop + FLabel.Height + lblmargin;
        end;
    end;
  end;
...
begin
  ...
  case FEditType of
    etHex: if (self.Text <> '') then Result := HexToInt(self.Text);
    etNumeric, etInt64, etFloat: Result := ValStr(self.Text);
    etMoney: Result := ValStr(StripThousandSep(self.Text));
  end;
end;

{$IFDEF DELPHIXE_LVL}
function TCustomAdvEdit.GetInt64: Int64;
begin
  Result := 0;
  case FEditType of
    etHex: if (self.Text <> '') then Result := HexToInt64(self.Text);
    etNumeric, etFloat: Result := ValStr(self.Text);
    etInt64: if (self.Text <> '') then Result := StrToInt64(self.Text);
    etMoney: Result := ValStr(StripThousandSep(self.Text));
  end;
end;
{$ENDIF}
...
procedure TCustomAdvEdit.SetFloatInt(const Value: double);
var
  onch: TNotifyEvent;
begin
  onch := OnChange;
...
begin
  case FEditType of
    ...
    etNumeric, etInt64: self.Text := Inttostr(value);
...
{$IFDEF DELPHIXE_LVL}
procedure TCustomAdvEdit.SetInt64(const Value: Int64);
begin
  case FEditType of
    etHex: self.Text := IntToHex(value, 0);
    etInt64, etNumeric: self.Text := Inttostr(value);
    etFloat: self.Text := Inttostr(value);
    etMoney:
      begin
        self.Text := IntToStr(value);
...
procedure TCustomAdvEdit.SetIntInt(const Value: Integer);
var
  onch: TNotifyEvent;
begin
  onch := OnChange;
...
procedure TCustomAdvEdit.Notification(AComponent: TComponent;
...
procedure TCustomAdvEdit.SetComponentStyle(AStyle: TTMSStyle);
begin
  FTMSStyle := AStyle;
...
begin
  inherited;
  if not (csLoading in ComponentState) and not ShowError and FParentFnt then
  begin
    if Assigned(FLabel) then
    begin
      FLabel.Font.Assign(Font);
...
begin
  ...
  FormPrev := (p as TCustomForm).KeyPreview;
  IsPrev := FormPrev and not DefaultHandling;

  if (Msg.CharCode = VK_ESCAPE) and FCanUndo and not ReadOnly and not IsPrev and Assigned(FLookupList) then
  begin
    ...
    if (Parent is TWinControl) and not FormPrev and FDefaultHandling then
    begin
      PostMessage((Parent as TWinControl).Handle, WM_KEYDOWN, VK_ESCAPE, 0);
      PostMessage((Parent as TWinControl).Handle, WM_KEYUP, VK_ESCAPE, 0);
    end;
  ...
  if (Msg.CharCode = VK_RETURN) and not FormPrev then
  begin
    // Take care of default key handling
    if (Parent is TWinControl) then
    begin
...
begin
  ...
  if not ParentFont and (LabelCaption <> '') and not FLoaded and not (csDesigning in ComponentState) and isScaled then
  begin
    FLblUpdate := true;
...
function TCustomAdvEdit.GetHexValue: Integer;
  var
  s: string;
  r: Integer;
begin
  if EditType = etHex then
  begin
    s := Text;
    r := 0;
    while (length(s) >= 2) do
...
procedure TCustomAdvEdit.SetHexDigits(const Value: Byte);
begin
  FHexDigits := Value;
  SetHexValue(FHexValue);
end;

procedure TCustomAdvEdit.SetHexValue(const Value: Integer);
var
  newtxt: string;
  newval: integer;
begin
  ...
  begin
    ...
    newtxt := IntToHex(newval, HexDigits);
...
procedure TCustomAdvEdit.DoEdit;
begin
  //
end;
...
procedure TCustomAdvEdit.DoOLEDropText(var AText: string; var Allow: boolean);
begin
  if Assigned(OnOLEDropText) then
    OnOLEDropText(Self, AText, Allow);
end;
...
begin
  ...
  if (EditType in [etNumeric, etInt64]) and AllowNumericNullValue and (Value = '') then
    isValid := true
  else
    if (EditType in [etNumeric, etInt64]) and ((FMinValue <> 0) or (FMaxValue <> 0)) then
    begin
      ...
      begin
        if i < FMinValue then
          IntValueInt := FMinValue
        else
          if i > FMaxValue then
            IntValueInt := FMaxValue;
      end;
    end;
...
begin
  ...
  if (EditType = etFloat) then
    Result := FloatValueInt
  else
  if (EditType = etInt64) then
    Result := Int64Value
  else
    Result := Text;
end;
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvCustomMaskEdit.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvCustomMaskEdit.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
...
procedure TAdvCustomMaskEdit.SetTextPadding(const Value: integer);
begin
  if FTextPadding <> Value then
  begin
    FTextPadding := Value;
...
procedure TAdvCustomMaskEdit.SetComponentStyle(AStyle: TTMSStyle);
begin
  FTMSStyle := AStyle;
  ...
  case FTMSStyle of
    ...
    begin
      if Flat and FlatParentColor then
        Color := (Parent as TWinControl).Brush.Color
      else
        Color := clWhite;
      FlatLineColor := clWindowText;
...
procedure TAdvCustomMaskEdit.HideValidation;
begin
  if Assigned(FToolTipWindow) then
  begin
    FToolTipWindow.Free;
...
{$IFDEF DELPHI2007_LVL}
procedure TAdvCustomMaskEdit.HideBalloon;
begin
  SendMessage(Handle, EM_HIDEBALLOONTIP, 0, 0);
end;
{$ENDIF}
...
procedure TAdvCustomMaskEdit.ShowValidation(const AText: string);
begin
  if not Assigned(FToolTipWindow) then
    FToolTipWindow := TAdvToolTipWindow.Create(Self);
...
{$IFDEF DELPHI2007_LVL}
procedure TAdvCustomMaskEdit.ShowBalloon(const ATitle, AText: string;
  AType: TBalloonType);
var
  ebt: TEditBalloonTip;
  title, Text: Widestring;
...
procedure TAdvCustomMaskEdit.SetUIStyle(const Value: TTMSStyle);
begin
  SetComponentStyle(Value);
end;
...
procedure TEditDropTarget.DropFiles(pt: TPoint; Files: TStrings);
var
  s: string;
  AAllow: boolean;
begin
  ...
  begin
    ...
    FAdvEdit.DoOLEDropText(s, AAllow);
