...
uses
  ...
  ComCtrls, ToolWin, Menus, ParamEdit, paramlabel, ParamListbox, paramchklist,
  Buttons, ImgList;
...
type
  ...
  Tparamlisteditor = class(TForm)
    ...
    procedure ParamListBox1KeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
  private
  ...
  TParamListBoxEditor = class(TComponent)
  private
    FParamListBox: TParamListBox;
    FCaption: string;
  ...
  published
    ...
    property ParamListBox: TParamListBox read FParamListBox write FParamListBox;
  end;

  TParamCheckListEditor = class(TComponent)
  private
    FParamCheckList: TParamCheckList;
    FCaption: string;
    FBorderIcons: TBorderIcons;
    FBorderStyle: TFormBorderStyle;
  public
  ...
  published
    ...
    property BorderStyle: TFormBorderStyle read FBorderStyle write FBorderStyle;
    property Caption: string read FCaption write FCaption;
    property ParamCheckList: TParamCheckList read FParamCheckList write FParamCheckList;
  end;
...
procedure Tparamlisteditor.InsertInList;
var
  idx: Integer;
begin
...
begin
  ...
  if (S.Count = 0) or (AnsiCompareText(S[S.Count-1], Temp) <> 0) and (pos('@',Temp) <> 1) then
  begin
    S.Add(Temp);
  end;
  Result := 1;
...
procedure Tparamlisteditor.FormShow(Sender: TObject);
begin
  FontName.ItemIndex := FontName.Items.IndexOf(ParamListBox1.Font.Name);
  FontSize.ItemIndex := FontSize.Items.IndexOf(IntToStr(ParamListBox1.Font.Size));
end;
...
procedure Tparamlisteditor.AddInList;
begin
  paramlistbox1.ItemIndex := paramlistbox1.Items.Add('');
  memo1.Lines.Text := paramlistbox1.Items[paramlistbox1.Itemindex];
end;
...
procedure Tparamlisteditor.DeleteFromList;
var
  idx: Integer;
begin
  idx := paramlistbox1.ItemIndex;
...
procedure Tparamlisteditor.ParamListBox1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
constructor TParamListBoxEditor.Create(AOwner: TComponent);begin
  inherited;
  BorderIcons := [biSystemMenu, biMinimize, biMaximize];
  BorderStyle := bsSizeable;
end;

function TParamListBoxEditor.Edit: TModalResult;var
  pe: TParamListEditor;
  res: TModalResult;
begin
  if not Assigned(ParamListBox) then
    raise Exception.Create('No TParamListBox assigned');

  pe := TParamListEditor.Create(Application);

  pe.ParamListBox1.Items.Assign(ParamListBox.Items);

  pe.DBfields.Visible:=false;

  pe.paramlabel1.Font.Assign(ParamListBox.Font);
  pe.ParamListBox1.Font.Assign(ParamListBox.Font);
  pe.ParamListBox1.Color := ParamListBox.Color;
  pe.ParamListBox1.ItemHeight := ParamListBox.ItemHeight;
  pe.ParamListBox1.Images := ParamListBox.Images;
  pe.ParamListBox1.ParamColor := ParamListBox.ParamColor;
  pe.ParamListBox1.ParamHint := ParamListBox.ParamHint;
  pe.ParamListBox1.ShowHint := ParamListBox.ShowHint;
  pe.ParamListBox1.HoverColor := ParamListBox.HoverColor;
  pe.ParamListBox1.HoverFontColor := ParamListBox.HoverFontColor;
  pe.ParamListBox1.Hover := ParamListBox.Hover;
  pe.ParamListBox1.ShadowColor := ParamListBox.ShadowColor;
  pe.ParamListBox1.ShadowOffset := ParamListBox.ShadowOffset;
  pe.ParamListBox1.LineSpacing := ParamListBox.LineSpacing;
  pe.ParamListBox1.SelectionColor := ParamListBox.SelectionColor;
  pe.ParamListBox1.SelectionFontColor := ParamListBox.SelectionFontColor;
  pe.ParamListBox1.ShowSelection := ParamListBox.ShowSelection;

  pe.BorderIcons := BorderIcons;
  pe.BorderStyle := BorderStyle;
  ...
  if res = mrOk then
  begin
    ParamListBox.Items.Assign(pe.ParamListBox1.Items);
  end;
...
constructor TParamCheckListEditor.Create(AOwner: TComponent);begin
  inherited;
  BorderIcons := [biSystemMenu, biMinimize, biMaximize];
  BorderStyle := bsSizeable;
end;

function TParamCheckListEditor.Edit: TModalResult;var
  pe: TParamListEditor;
  res: TModalResult;
begin
  if not Assigned(ParamCheckList) then
    raise Exception.Create('No TParamCheckList assigned');

  pe := TParamListEditor.Create(Application);

  pe.ParamListBox1.Items.Assign(ParamCheckList.Items);

  pe.DBfields.Visible:=false;

  pe.paramlabel1.Font.Assign(ParamCheckList.Font);
  pe.ParamListBox1.Font.Assign(ParamCheckList.Font);
  pe.ParamListBox1.Color := ParamCheckList.Color;
  pe.ParamListBox1.ItemHeight := ParamCheckList.ItemHeight;
  pe.ParamListBox1.Images := ParamCheckList.Images;
  pe.ParamListBox1.ParamColor := ParamCheckList.ParamColor;
  pe.ParamListBox1.ParamHint := ParamCheckList.ParamHint;
  pe.ParamListBox1.ShowHint := ParamCheckList.ShowHint;
  pe.ParamListBox1.HoverColor := ParamCheckList.HoverColor;
  pe.ParamListBox1.HoverFontColor := ParamCheckList.HoverFontColor;
  pe.ParamListBox1.Hover := ParamCheckList.Hover;
  pe.ParamListBox1.ShadowColor := ParamCheckList.ShadowColor;
  pe.ParamListBox1.ShadowOffset := ParamCheckList.ShadowOffset;
  pe.ParamListBox1.LineSpacing := ParamCheckList.LineSpacing;
  pe.ParamListBox1.SelectionColor := ParamCheckList.SelectionColor;
  pe.ParamListBox1.SelectionFontColor := ParamCheckList.SelectionFontColor;
  pe.ParamListBox1.ShowSelection := ParamCheckList.ShowSelection;
  ...
  if res = mrOk then
  begin
    ParamCheckList.Items.Assign(pe.ParamListBox1.Items);
  end;
