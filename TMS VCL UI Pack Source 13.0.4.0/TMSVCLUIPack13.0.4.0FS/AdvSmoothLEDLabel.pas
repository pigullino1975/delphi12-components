...
const
  ...
  // v1.2.0.0 : New : Caption.LeadingZeros property added
  // v1.2.0.1 : Fixed : Issue with LeadingZeros = false
  // v1.2.0.2 : Fixed : Issue with flicker on high refresh rates
...
type
  ...
  {$ENDIF}
  TAdvSmoothLedLabel = class(TCustomControl, ITMSStyle)
  private
    FCount: integer;
    FPainting: Boolean;
    FDesignTime: Boolean;
...
begin
  ...
  DrawLed(g, MakeRect(0, 0, Width, Height), TFillValueType(Caption.ValueType), Caption.Format, Caption.TimeFormat, Caption.Value,
    Caption.TimeValue, Caption.ColorOff, Caption.ColorOffOpacity, Caption.ColorStart, Caption.ColorEnd, Caption.OpacityStart,
      Caption.OpacityEnd, Caption.GradientType, Caption.Angle, Caption.HatchStyle, Caption.Picture, Caption.LeadingZeros);
  g.Free;
  FPainting := false;
  Inc(FCount);
end;
...
procedure TAdvSmoothLedLabel.WMPaint(var Message: TWMPaint);
var
  DC, MemDC: HDC;
...
begin
  ...
  begin
    ...
    FLeadingZeros := (Source as TAdvSmoothLedLabelCaption).LeadingZeros;
    Changed;
  end;
end;
...
procedure TAdvSmoothLedLabelCaption.SetLeadingZeros(const Value: boolean);
begin
