...
const
  ...
  // v1.4.0.0 : New : Support for Office 2019 styles
  // v1.4.1.0 : New : UIStyle property for TMSStyle
  // v1.4.1.1 : Fixed : UIStyle set to tsCustom after initial Office look
  // v1.4.1.2 : Improved : On creation check for enabled AdvFormStyler
  // v1.4.1.3 : Fixed : Additional fixes for per monitor high dpi
  // v1.4.1.4 : Fixed : Issue with opening AdvSmoothPopUp for second time on different DPI
  // v1.4.1.5 : Fixed : Per monitor high DPI fixes to work in additional uses
  // v1.4.1.6 : Fixed : Per monitor high DPI fixes for Delphi 11
  // v1.4.1.7 : Fixed : Issue with uninitialized rect when Caption is empty
  // v1.4.1.8 : Fixed : Issue with Invalid class typecast in Delphi 11
...
type
  ...
  protected
    ...
    function  GetMonitorPixelsPerInch(p: TPoint) : Integer;
    {$ENDIF}
  ...
  public
    ...
    procedure PopupAt(X, Y: Integer {$IFDEF DELPHIXE10_LVL}; forcePPIofMainForm: boolean = True {$ENDIF});
    procedure PopupAtControl(AControl: TWinControl; Direction: TAdvSmoothPopupDirection {$IFDEF DELPHIXE10_LVL}; forcePPIofMainForm: boolean = True {$ENDIF});
...
begin
  ...
  spc := Round(FDPIScale * 4);
  mgr := Round(FDPIScale * 5);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          fnt := TFont.Create;
          fnt.Assign(ButtonFont);
          fnt.Height := Round(FDPIScale * fnt.Height);
          ft := g.MakeFont(fnt);
  ...
  begin
    ...
    fnt.Height := Round(fnt.Height * FDPIScale);
    ft := g.MakeFont(fnt);
    fnt.Free;
    sf := TGPStringFormat.Create;
...
procedure TAdvSmoothPopup.Loaded;
begin
  inherited;
  FInitPPI := 96;
  ...
  begin
    FInitPPI := (Owner as TCustomForm).PixelsPerInch;
  end;
  {$ENDIF}
end;
...
begin
  Result := Round(FooterHeight * FDPIScale);
end;

function TAdvSmoothPopup.GetHeaderHeight: integer;
begin
  Result := Round(HeaderHeight * FDPIScale);
end;
...
begin
  if AutoSize and Assigned(Control) then
    ...
    Result := Round(Control.Height / Control.ScaleFactor * FDPIScale) + GetMaxMarginSize + GetHeaderHeight + GetFooterHeight
    {$ENDIF}
    {$IFNDEF DELPHIXE12_LVL}
    Result := Round(Control.Height * FDPIScale) + GetMaxMarginSize + GetHeaderHeight + GetFooterHeight
    {$ENDIF}
  else
    Result := Round(Height * FDPIScale) + GetMaxMarginSize;
end;

function TAdvSmoothPopup.GetMaxMarginSize: Integer;
begin
  Result := Max(Round(FDPIScale * ArrowSize * 2), ShadowSize * 2);
end;
...
begin
  if AutoSize and Assigned(Control) then
    {$IFDEF DELPHIXE12_LVL}
    Result := Round(Control.Width / Control.ScaleFactor * FDPIScale) + GetMaxMarginSize
    {$ENDIF}
    {$IFNDEF DELPHIXE12_LVL}
    Result := Round(Control.Width * FDPIScale) + GetMaxMarginSize
    {$ENDIF}
  else
    Result := Round(Width * FDPIScale) + GetMaxMarginSize;
end;
...
{$IFDEF DELPHIXE10_LVL}
function TAdvSmoothPopup.GetMonitorPixelsPerInch(p: TPoint): Integer;
var
  i: Integer;
  monitorRect: TRect;
begin
  ...
  begin
    ...
    if MonitorCount > 1 then
      ...
      begin
        monitorRect := Rect(Monitors[i].Left, Monitors[i].Top,
          Monitors[i].Left + Monitors[i].Width,
          Monitors[i].Top + Monitors[i].Height);
        if monitorRect.Contains(p) then
        begin
          result := Monitors[i].PixelsPerInch;
          break;
        end
      end;
...
begin
  ...
  begin
    ...
    frmC.Show;
    // Width and Height can change when showed on screen - ref. line 2483
    ...
    frmC.SetBounds(Round(ir.X + X), Round(ir.Y + Y), Round(ir.Width) - 1, Round(ir.Height) - 1);

    {$IFDEF DELPHIXE10_LVL}
    {$IFNDEF DELPHIXE14_LVL}
    frmC.ScaleForPPI(Round(FDPIScale * 96));
    {$ENDIF}
    {$IFNDEF DELPHIXE14_LVL}
    control.ScaleForPPI(Round(FDPIScale * 96));
    frmC.Font.Height := MulDiv(frmC.Font.Height, Round(FDPIScale * 96), FInitPPI);
    {$ENDIF}
    {$ENDIF}

    if frmC.Width <> (Round(ir.Width) - 1) then
    begin
      frmC.Width := Round(ir.Width) - 1;
      frmC.Height := Round(ir.Height) - 1;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          bf := TFont.Create;
          bf.Assign(ButtonFont);
          bf.Height := Round(bf.Height * FDPIScale);
          ft := g.MakeFont(bf);
          sf := TGPStringFormat.Create;
...
begin
  ...
  begin
    ...
    if Assigned(Owner) and (Owner is TCustomForm) then
    begin
      fnt.Height := MulDiv(HeaderFont.Height, (Owner as TCustomForm).PixelsPerInch, FInitPPI);
    end
    else
    {$ENDIF}
    fnt.Height := Round(fnt.Height * FDPIScale);
  ...
  begin
    ...
    if Assigned(Owner) and (Owner is TCustomForm) then
    begin
      fnt.Height := MulDiv(FooterFont.Height, (Owner as TCustomForm).PixelsPerInch, FInitPPI);
    end
    else
    {$ENDIF}
    fnt.Height := Round(fnt.Height * FDPIScale);
...
begin
  ...
  // arrow
  arsize := Round(FDPIScale * ArrowSize);
  ah := arsize;
  aw := arsize;
  case ArrowPosition of
    paLeftTop:
      begin
        ...
        arrowposend := MakePoint(arrowpos.X - arsize, arrowpos.Y);
        ah := arsize * 3 / 2;
        aw := arsize;
      end;
    paLeftCenter:
    ...
    paTopLeft:
      begin
        ...
        arrowposend := MakePoint(arrowpos.X, arrowpos.Y - arsize);
        aw := arsize * 3 / 2;
        ah := arsize;
      end;
    paTopCenter:
...
procedure TAdvSmoothPopup.PopupAt(X, Y: Integer {$IFDEF DELPHIXE10_LVL}; forcePPIofMainForm: boolean{$ENDIF});
{$IFDEF DELPHIXE10_LVL}
var
  redo: Boolean;
  nLoopMax: Integer;
  refPixelsPerInch, newPixelsPerInch: Integer;
{$ENDIF}
begin
  FDPIScale := 1;
  ...
  {$IFDEF DELPHIXE12_LVL}
  if (forcePPIofMainForm) and Assigned(Application.MainForm) then
    refPixelsPerInch := Round({$IFNDEF DELPHIXE14_LVL}Application.MainForm.ScaleFactor *{$ENDIF} Application.MainForm.PixelsPerInch);
  {$ENDIF}

  if (refPixelsPerInch = 0) then
    refPixelsPerInch := GetMonitorPixelsPerInch(Rect(X, Y, X + GetWidth, Y + GetHeight).CenterPoint);

  redo := false;
  nLoopMax := 4;
  repeat
    FDPIScale := refPixelsPerInch / 96;
    if not forcePPIofMainForm then
    begin
      // DPIScale affects GetWidth and GetHeight
      newPixelsPerInch := GetMonitorPixelsPerInch(Rect(X, Y, X + GetWidth, Y + GetHeight).CenterPoint);
      redo := (refPixelsPerInch <> newPixelsPerInch);
      if redo then
        refPixelsPerInch := newPixelsPerInch;
      dec(nLoopMax);
    end;
  until (not(redo and (nLoopMax > 0)));
  {$ENDIF}

  DoPopup(X, Y, False);
end;

procedure TAdvSmoothPopup.PopupAtControl(AControl: TWinControl; Direction: TAdvSmoothPopupDirection {$IFDEF DELPHIXE10_LVL}; forcePPIofMainForm: boolean {$ENDIF});
var
  X, Y: Integer;
  p: TPoint;
  {$IFDEF DELPHIXE10_LVL}
  center: TPoint;
  refPixelsPerInch: Integer;
  {$ENDIF}
  {$IFDEF DELPHIXE12_LVL}
  frm: TCustomForm;
  {$ENDIF}
    ...
    begin
      ...
      begin
        ...
        if Assigned(frm) and (frm is TForm) then
          refPixelsPerInch := Round({$IFNDEF DELPHIXE14_LVL}frm.ScaleFactor * {$ENDIF}TForm(frm).PixelsPerInch);
      end;
      {$ENDIF}

      if (refPixelsPerInch = 0) then
      begin
        center := AControl.Parent.ClientToScreen(AControl.BoundsRect.CenterPoint);
        refPixelsPerInch := GetMonitorPixelsPerInch(center);
      end;
          ...
          begin
            ...
            case Direction of
              pdLeftTop, pdRightTop:
                Y := Y + ShadowSize + 5 + AControl.Height - GetHeight;
              pdLeftCenter, pdRightCenter:
                Y := Y + (AControl.Height div 2) - (GetHeight div 2);
          ...
          begin
            ...
            case Direction of
              pdTopLeft, pdBottomLeft:
                X := X + (AControl.Width div 2) - GetWidth - 5 + ShadowSize * 2;
              pdTopCenter, pdBottomCenter:
                X := X + (AControl.Width div 2) - (GetWidth div 2);
              pdTopRight, pdBottomRight:
                X := X + (AControl.Width div 2) + 5 - ShadowSize * 2;
            end;
          end;
      end;
...
begin
  ...
  case AStyle of
      ...
      begin
        ButtonColor := clWhite;
        ButtonHoverColor := $00F2E1D5;
        ButtonDownColor := $00E3BDA3;
        HeaderFont.Color := $00444648;
        FooterFont.Color := $00444648;
        ButtonFont.Color := $00444648;
      end;
      tsOffice2019Gray:
      begin
        ButtonColor := $00D4D4D4;
        ButtonHoverColor := $00969696;
        ButtonDownColor := $00666666;
        HeaderFont.Color := $00232425;
        FooterFont.Color := $00232425;
        ButtonFont.Color := $00232425;
      end;
      tsOffice2019Black:
      begin
        ButtonColor := $00444444;
        ButtonHoverColor := $00686868;
        ButtonDownColor := $00828282;
        HeaderFont.Color := clWhite;
        FooterFont.Color := clWhite;
        ButtonFont.Color := clWhite;
      end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FPrevWidth := Round(FControl.Width / FDPIScale);
      FPrevHeight := Round(FControl.Height / FDPIScale);
...
procedure TAdvSmoothPopup.SetUIStyle(const Value: TTMSStyle);
begin
...
begin
  ...
  begin
    mxs := Max(0, Round(Popup.ArrowSize * Popup.FDPIScale) - Popup.ShadowSize);
    // Width and Height of TAdvSmoothPopupForm can change when showed on screen - ref. line 1028
    {$IFDEF DELPHIXE12_LVL}
    R := MakeRect(mxs, mxs, Round(Width / ScaleFactor) - mxs * 2 - 1, Round(Height / ScaleFactor) - mxs * 2 - 1);
    {$ENDIF}
    {$IFNDEF DELPHIXE12_LVL}
