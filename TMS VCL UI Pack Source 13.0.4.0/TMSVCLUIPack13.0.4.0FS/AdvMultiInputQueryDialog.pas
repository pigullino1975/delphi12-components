...
const
  ...
  // v1.1.1.0 : New : VerticalSpacing property added
  // v1.1.1.1 : Fixed : Issue with Label persistence in QueryValues
  // v1.2.0.0 : New : Title propeerty added
  //          : New : Notes property added
...
type
  ...
  private
    FFormHeight, FFormWidth, FTitleHeight: Integer;
    FForm: TForm;
    FModal: Boolean;
    FPanel: TPanel;
    FTitlePanel: TPanel;
  ...
  protected
    ...
    property VerticalSpacing: Integer read FVerticalSpacing write FVerticalSpacing default 10;
...
begin
  ...
  if (FTitle <> '') or (FNotes <> '') then
  begin
    FTitlePanel := TPanel.Create(Self);
    FTitlePanel.Align := TALign.alTop;
    FTitlePanel.BevelOuter := bvNone;
    FTitlePanel.Ctl3D := false;
    FTitlePanel.AutoSize := true;
    FTitlePanel.Parent := AForm;

    if (FTitle <> '') then
    begin
      l := TLabel.Create(FTitlePanel);
      l.Caption := Title;
      l.AutoSize := true;
      l.Font.Style := [fsBold];
      l.Parent := FTitlePanel;
      l.Margins.Left := 10;
      l.Margins.Right := 10;
      l.Margins.Top := 10;
      l.Margins.Bottom := 10;
      l.AlignWithMargins := true;
      l.Align := TAlign.alTop;
      FTitleHeight := l.Height + 20;
      FTitleLabel := l;
    end;
    ...
    begin
      ...
      l.AlignWithMargins := true;
      l.Align := TAlign.alTop;
//      l.Width := AForm.Width - 150;

      l.Caption := Notes;
      l.AutoSize := true;

      FTitleHeight := FTitleHeight + l.Height + 20;
      FNotesLabel := l;
    end;

    FTitlePanel.Height := FTitleHeight;
  end;
...
begin
  ...
  FValuePanel.Top := FTitleHeight + 10;

  FValuePanel.Align := TAlign.alTop;
  FValuePanel.BevelOuter := bvNone;
  FValuePanel.Ctl3D := false;
  FValuePanel.Parent := AForm;
  ...
  begin
    ...
    y := y + VerticalSpacing + ed.Height;
    if y - VerticalSpacing > maxh then
      maxh := y - VerticalSpacing;
  ...
  FValuePanel.Height := maxh;

  FFormHeight := FTitleHeight + FFormHeight + maxh + (FVerticalSpacing div 2);
...
begin
  ...
  if Assigned(FNotesLabel) then
  begin
    FNotesLabel.AutoSize := false;
    FNotesLabel.AutoSize := true;
  end;

  if Assigned(FTitlePanel) then
  begin
    FTitlePanel.AutoSize := false;
    FTitlePanel.AutoSize := true;

    FForm.ClientHeight := FTitlePanel.Height + FValuePanel.Height + FPanel.Height;
  end;
...
begin
  ...
  begin
    ...
    FDefaultValue := (Source as TAdvMultiInputQueryValue).DefaultValue;
    FLabel := (Source as TAdvMultiInputQueryValue).&Label;
  end;
end;
