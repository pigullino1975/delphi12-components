...
uses
  ...
  Forms, Graphics, Buttons, AdvEdit, Types, AdvStyleIF
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF}
...
const
  ...
  // v1.1.2.10: Fixed : Issue with minus handling
  // v1.1.3.0 : New : ButtonWidth property added
  // v1.1.3.1 : Improved : Button glyph center alignment
  // v1.1.4.0 : New : VCL styles support added
  //          : New : CalculatorLook.BorderColor added
  //          : New : AutoShowCalculator property added
...
{ TDropCalculatorButton }
  ...
  published
    ...
    property ButtonWidth: integer read FButtonWidth write FButtonWidth default 24;
    property ButtonHeight: integer read FButtonHeight write FButtonHeight default 24;
    property ButtonColor: TColor read FButtonColor write FButtonColor default clSilver;
...
{ TAdvMoneyEdit }
  ...
  published
    ...
    property AutoSelect;
    property AutoSize;
    property AutoShowCalculator: boolean read FAutoShowCalculator write FAutoShowCalculator default false;
...
begin
  ...
  bkcol := CalculatorLook.Color;
  brdrcol := CalculatorLook.BorderColor;
...
procedure TAdvMoneyEdit.UpdateButton;
var
  MinHeight: Integer;
  Dist: integer;
  BtnW: integer;
...
begin
  ...
  begin
     ...
     BtnW := Round((FButtonWidth + 1) * FDPIScale);
...
begin
  ...
  case Msg.CharCode of
    ...
    Ord('-'):
    begin
      if not (Signed and IsShift) then
      begin
        domin;
        SelectAll;
        Msg.CharCode := 0;
...
begin
  ...
  if FCalcClosed and (IsNumChar(Key)) and AutoShowCalculator then
    ShowCalculator;
  ...
  '+':doplus;
  '/':dodiv;
  '-':if not (Signed and IsShift) then domin;
  '*':domul;
  '=',#13:doeq;
  '%':doperc;
  end;
...
procedure TAdvMoneyEdit.SetButtonWidth(const Value: integer);
begin
  FButtonWidth := Value;
  Button.Width := Value;
  IndentR := FButtonWidth + 2;
...
begin
  ...
  begin
    ...
    FButtonWidth := TCalculatorLook(Source).ButtonWidth;
    FButtonHeight := TCalculatorLook(Source).ButtonHeight;
    FButtonColor := TCalculatorLook(Source).ButtonColor;
    FColor := TCalculatorLook(Source).Color;
    Font.Assign(TCalculatorLook(Source).Font);
    FFlat := TCalculatorLook(Source).Flat;
    FBorderColor := TCalculatorLook(Source).BOrderColor;
  end;
end;
