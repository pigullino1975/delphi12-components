...
const
  ...
  // 1.9.0.0 : New : Support for Office 2019 styles
  // 1.9.1.0 : Fixed : Design-time style painting
  //         : Improved : Support for per monitor high DPI
  // 1.9.1.1 : Fixed : Office 2019 style was not only forced on creation
  // 1.9.2.0 : New: UIStyle property for office look
  //         : Fixed : Office 2019 Black colors
  //         : Improved : On creation check for enabled AdvFormStyler
  //         : Improved : Drawing of the checkboxes on high DPI
  //         : Fixed : VCL styles check updated to work in 10.4 Sydney
  // 1.9.3.0 : New : Enable use of TVirtualImageList on TAdvOutlookList
  // 1.9.4.0 : New : ReadOnly property added in TAdvOutlookColumn to have read-only checkboxes
...
type
  ...
  olsOffice2019White, olsOffice2019Gray, olsOffice2019Black);
  ...
  protected
    ...
    function HTMLPaint(Canvas:TCanvas; s:string; fr:TRect; FImages:TCustomImageList;
...
procedure DrawCheck(Canvas: TCanvas; R:TRect; State, Enabled: Boolean; Color: TColor; CheckBoxStyle: TCheckBoxStyle; ADPIScale: Single = 1);
var
  DrawState: Integer;
  DrawRect: TRect;
  BMP: TBitmap;
...
begin
  ...
  cbsClassic,cbsFlat:
    begin
      ...
      DrawRect.Left := R.Left + (R.Right - R.Left - Round(ADPIScale * CHECKBOX_SIZE)) div 2;
      DrawRect.Top:= R.Top + (R.Bottom - R.Top - Round(ADPIScale * CHECKBOX_SIZE)) div 2;
      DrawRect.Right := DrawRect.Left + Round(ADPIScale * CHECKBOX_SIZE);
      DrawRect.Bottom := DrawRect.Top + Round(ADPIScale * CHECKBOX_SIZE);
  ...
  cbsBorland:
    begin
      ...
      begin
        ...
        Dec(R.Top);
        Dec(R.Bottom);
        Canvas.MoveTo(R.Left + 2,R.Top + Round(ADPIScale * CHECKBOX_SIZE) div 2 + 1);
        Canvas.LineTo(R.Left + 2,R.Bottom - 1);
        Canvas.MoveTo(R.Left + 3,R.Top + Round(ADPIScale * CHECKBOX_SIZE) div 2);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          ItemCanvas.Font.Height := Round(FDPIScale * ItemCanvas.Font.Height);
          ...
          begin
            ...
            begin
              MoveTo(ColRect.Left + Round(FDPIScale * FColumns.Items[i].Width) -1, ColRect.Top);
              LineTo(ColRect.Left + Round(FDPIScale * FColumns.Items[i].Width) -1, ColRect.Bottom);
            end;
          end;
            ...
            case ColumnType of
              ...
              begin
                ...
                begin
                  ...
                  {$IFDEF DELPHIXE2_LVL}
                  if CheckVCLStylesEnabled(StyleServices, (csDesigning in ComponentState)) {$IFDEF DELPHIXE6_LVL}and (seFont in StyleElements){$ENDIF} then
                  begin
              ...
              begin
                ...
                case FColumns.Items[i].alignment of
                  ...
                  begin
                    ...
                    DrawRect.Right := DrawRect.Left + Round(FDPIScale * CHECKBOX_SIZE);
                  end;
                  taRightJustify:
                  begin
                    DrawRect.Left := ColRect.Right - Round(FDPIScale * CHECKBOX_SIZE);
                    DrawRect.Right := DrawRect.Left + Round(FDPIScale * CHECKBOX_SIZE);
                  end;
                  else
                  begin
                    DrawRect.Left := ColRect.Left + (ColRect.Right - ColRect.Left - Round(FDPIScale * CHECKBOX_SIZE)) div 2;
                    DrawRect.Right := DrawRect.Left + Round(FDPIScale * CHECKBOX_SIZE);
                  end;
                end;
                DrawRect.Top:= ColRect.Top + (ColRect.Bottom - ColRect.Top - Round(FDPIScale * CHECKBOX_SIZE)) div 2;
                DrawRect.Bottom := DrawRect.Top + Round(FDPIScale * CHECKBOX_SIZE);
                ...
                {$ENDIF}
                  DrawCheck(ItemCanvas, DrawRect, StrToInt(S) = 1, true, clBlack, FCheckBoxStyle, FDPIScale);
              end;
            end;
          end;
        end;
...
begin
  ItemHeight := Round(FDPIScale * FItemHeight);

  if FPreviewSettings.Active then
  begin
    ItemHeight := ItemHeight + Round(FDPIScale * FPreviewSettings.Height);
  end;
end;
...
begin
  ...
  begin
    ...
    Result.Top := Result.Top + (Result.Bottom - Result.Top) - Round(FDPIScale * FPreviewSettings.Height);
    Exit;
  end;
  ...
  begin
    ...
    w := Round(FDPIScale * FHeader.SectionWidth[i]);
    ...
    begin
      ...
      begin
        Result.Bottom := Result.Top + (Result.Bottom - Result.Top) - Round(FDPIScale * FPreviewSettings.Height);
        x := x + w;
      end;
    end
    else
...
begin
  ...
  begin
    ...
    try
      ...
      ACanvas.Font.Height := Round(FDPIScale * ACanvas.Font.Height);
      ...
      begin
        ...
        ACanvas.Font.Height := Round(FDPIScale * FPreviewSettings.Font.Height);
      end;
        ...
        case AColItem.ColumnType of
          ...
          begin
            ...
            case AColItem.Alignment of
              ...
              taRightJustify:
              begin
                DrawRect.Left := CellRect.Right - Round(FDPIScale * CHECKBOX_SIZE);
                DrawRect.Right := DrawRect.Left + Round(FDPIScale * CHECKBOX_SIZE);
              end;
              else
              begin
                DrawRect.Left := CellRect.Left + (CellRect.Right - CellRect.Left - Round(FDPIScale * CHECKBOX_SIZE)) div 2;
                DrawRect.Right := DrawRect.Left + Round(FDPIScale * CHECKBOX_SIZE);
              end;
            end;
            //DrawRect.Left := CellRect.Left + (CellRect.Right - CellRect.Left - CHECKBOX_SIZE) div 2;
            DrawRect.Top:= CellRect.Top + (CellRect.Bottom - CellRect.Top - Round(FDPIScale * CHECKBOX_SIZE)) div 2;
            //DrawRect.Right := DrawRect.Left + CHECKBOX_SIZE;
            DrawRect.Bottom := DrawRect.Top + Round(FDPIScale * CHECKBOX_SIZE);
            if PtInRect(DrawRect, Point(X, Y)) and not AColItem.ReadOnly then
            begin
...
procedure TAdvOutlookList.SetImages(const Value: TCustomImageList);
begin
...
begin
 ...
 { if FVAlignment in [tvaCenter,tvaBottom] then
  ...
  begin
    ...
    Canvas.Font.Height := Round(FPreviewSettings.Font.Height * FDPIScale);
  end;
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvOutlookList.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvOutlookList.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
      ...
      begin
        ...
        begin
          ...
          if pt.X < CellRect.Right then
            HintPos.X := HintPos.X + Round(FDPIScale * CHECKBOX_SIZE);
        end;
...
begin
  ...
  begin
    ...
    case AStyle of
      ...
      begin
        c.HeaderColorTo := clWhite;
        c.HeaderColor := clWhite;
        c.HeaderFont.Color := $00777777
      end;
      olsOffice2019Gray:
      begin
        c.HeaderColorTo := $00666666;
        c.HeaderColor := $00666666;
        c.HeaderFont.Color:= $00C6C6C6;
        c.Font.Color := $00262626;
      end;
      olsOffice2019Black:
      begin
        c.HeaderColorTo := $00262626;
        c.HeaderColor := $00262626;
  ...
  case AStyle of
    olsOffice2019White:
    begin
      BorderColor := $00B5ACA5;
      ColumnLineColor := clNone;
      GroupFont.Color := $004B4A49;
      GroupCountFont.Color := $004B4A49;
      Color := clWhite;
      GroupColor := $00F0F0F0;
      GroupSelectionColor := $00F7E6CD;
      GroupSelectionTextColor := $004B4A49;
      SelectionColor := $00F7E6CD;
      SelectionTextColor := $00444444;
      GroupLineColor := $00E1E1E1;
      PreviewSettings.Font.Color := $00606060;
      HeaderBorderColor := $00B5ACA5;
    end;
    olsOffice2019Gray:
    begin
      BorderColor := $00444444;
      ColumnLineColor := clNone;
      GroupFont.Color := clWhite;
      GroupCountFont.Color := clWhite;
      Color := $00D4D4D4;
      GroupColor := $00444444;
      GroupSelectionColor := $00B06700;
      GroupSelectionTextColor := clWhite;
      SelectionColor := $00ECD3B3;
      SelectionTextColor := $004B4B4B;
      GroupLineColor := $00E1E1E1;
      PreviewSettings.Font.Color := $004B4B4B;
      HeaderBorderColor := $00444444;
    end;
    olsOffice2019Black:
    begin
      BorderColor := $00444444;
      ...
      Color := $00262626;
      GroupColor := $00444444;
      GroupSelectionColor := $00666666;
      GroupSelectionTextColor := clWhite;
      SelectionColor := $00424242;
      SelectionTextColor := $00F1F1F1;
      GroupLineColor := $00626262;
      PreviewSettings.Font.Color := $00C1C1C1;
      HeaderBorderColor := $00444444;
    end;
    else
    begin
      ...
      GroupColor := clNone;
      GroupFont.Color := $00B96837;
      GroupCountFont.Color := clBlack;
      GroupLineColor := clBtnFace;
      GroupSelectionColor := clGradientActiveCaption;
      GroupSelectionTextColor := clBlack;
      HeaderBorderColor := clSilver;
      SelectionColor := clGradientActiveCaption;
...
begin
  ...
  begin
    ...
    ReadOnly := (Source as TAdvOutlookColumn).ReadOnly;
  end
  else
    inherited;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        SecRect.Right := SecRect.Left + Round(FOutLookList.FDPIScale * SectionWidth[i]);
        ...
        Canvas.Font.Height := Round(FOutLookList.FDPIScale * HeaderFont.Height);
...
begin
  ...
  begin
    Ofs := Ofs + Round(FOutLookList.FDPIScale * SectionWidth[SectionIndex]);
    Inc(SectionIndex);
  end;
  Dec(SectionIndex);
...
begin
  ...
  for SectionIndex := 0 to X - 1 do
    Offset := Offset + Round(FOutLookList.FDPIScale * SectionWidth[SectionIndex]);

  Result.Left := Offset;
  Result.Right := Offset + Round(FOutLookList.FDPIScale * SectionWidth[X]);
  Result.Top := 0;
