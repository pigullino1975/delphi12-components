...
const
  ...
  // v1.4.7.1 : Fixed : Issue with decimals handling in TAdvCalculatorDropDown
  // v1.4.8.0 : New : OnAfterDropDown event added
  // v1.4.8.1 : Fixed : Issue with drop down height initialization in TAdvOfficeColorPicker
  // v1.5.0.0 : New : Support for Office 2019 styles
  //          : Fixed : Issue with dropdown color for certain dropdown controls due to VCL styles
  // v1.5.0.1 : Fixed : Creation style is changed to default style defined in AdvStyleIF
  // v1.5.0.2 : Fixed : Issue with user dropdown sizing and auto dropdown sizing
  // v1.5.1.0 : Improved : VCL Styles support in TAdvDropDown
  // v1.5.2.0 : New : UIStyle property for office look
  //          : Improved : On creation check for enabled AdvFormStyler
  //          : Improved : Dropdown button width on high DPI
  // v1.5.3.0 : New : StyleElements property exposed
  // v1.5.3.1 : Fixed : Color consistency for edit components in Office 2019 Gray
  // v1.5.3.2 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // v1.5.3.3 : Fixed : Issue with restarting calculation from TAdvCalculatorDropDown
  // v1.5.4.0 : New : EmptyText/EmptyTextFocused in TAdvMemoDropDown
  //          : Improved : Font use on TAdvMemoDropDown in dropdown memo
  // v1.5.4.1 : Improved : High DPI compatibility
  // v1.5.5.0 : Improved : VCL Themes handling
  // v1.5.5.1 : Fixed : Issue with dropdown size on per monitor DPI
  // v1.5.5.2 : Fixed : Regression with ScaleForDPI call
  // v1.5.5.3 : Improved : VCL Styles & high DPI compatibility
  // v1.5.5.4 : Improved : VCL Styles usage in dropdown with scrollbars
  // v1.5.5.5 : Fixed : Issue in TAdvMultiColumnDropDown and ColumnSizeWithDropDown = true
  // v1.5.5.6 : Fixed : Issue with csDropDownList and & character
  // v1.5.5.7 : Fixed : DPI calculation when parent form is not Scaled
  // v1.5.5.8 : Fixed : HighDPI rendering of items in TAdvDetailDropDown
  // v1.5.5.9 : Fixed : Regression with introduced high DPI calculations
  // v1.5.6.0 : Improved : HighDPI drawing in dropdown
  // v1.5.6.1 : Fixed : Issue with LabelMargin persisting
  // v1.5.6.2 : Fixed : Item text color initialization issue
  // v1.5.6.3 : Fixed : Issue with high DPI on dropdown with Delphi 11
  // v1.5.6.4 : Improved : High DPI handling in Delphi 11
  // v1.5.6.5 : Improved : Label font size initialization at design-time for high DPI
  // v1.5.6.6 : Improved : Dropdown control sizing in Delphi 11 for high DPI
  // v1.5.6.7 : Improved : LabelFont handling in connection with ParentFont
  // v1.5.7.0 : New : Property EmoticonSize added in TAdvEmoticonPickerDropDown
  // v1.5.7.1 : Improved : Multi-monitor high DPI handling
  // v1.5.7.2 : Fixed : Issue with incorrect handling for AltGr key
...
type
  ...
  TDrawItemsEvent =  procedure(Sender: TObject; Canvas: TCanvas) of object;
  ...
  public
    ...
    procedure DrawItemBackground(Index: Integer; ACanvas: TCanvas); virtual;
  ...
  private
    ...
    FOnBeforeDrawItems: TDrawItemsEvent;
    FOnAfterDrawItems: TDrawItemsEvent;
  ...
  public
    ...
    property ItemSelectorPanel: TCustomItemSelector read FItemSelectorPanel;
  ...
  published
    ...
    property OnBeforeDrawItems: TDrawItemsEvent read FOnBeforeDrawItems write FOnBeforeDrawItems;
    property OnAfterDrawItems: TDrawItemsEvent read FOnAfterDrawItems write FOnAfterDrawItems;
...
function DrawHTMLEX(Canvas: TCanvas; s: string; R: TRect; Images: TCustomImageList; RealDraw: Boolean; WinHandle: THandle; DPIScale: single): TSize;
...
begin
  ...
  HTMLDrawEx(Canvas, s, r, Images, 0, 0, -1, 0, 1, False, not RealDraw,False,True,True,False, False{Wordwrap},false,'',
             ...
             Integer(Result.cx), Integer(Result.cy), hl, ml, hr, cr, CID, CT, CV, nil, nil, WinHandle, DPIScale);
end;
...
begin
  ...
  Loc.Right := ClientWidth - Round(FDPIScale * FButton.Width) - 3;

  if (BorderStyle = bsNone) and (BevelKind = bkNone) then
  begin
    Loc.Top := Round(2 * FDPIScale);
    Loc.Left := Round(2 * FDPIScale);
  end
  else
  begin
    Loc.Top := Round(1 * FDPIScale);
    Loc.Left := Round(1 * FDPIScale);
  end;

  FEditRect := Loc;

  if not Ctl3D then
    Loc.Left := Loc.Left + Round( 1 * FDPIScale);
...
begin
  ...
  if FDesignTime then
  begin
    SetComponentStyle(GetDefaultStyle(Owner, tsCustom));
  end;
...
begin
  ...
  if Assigned(OnAfterDropDown) then
    OnAfterDropDown(Self);
end;
...
begin
  ...
  begin
    ...
    if (FDropDownWidth > 0) then
      FDropDownForm.Width := Round(FDropDownWidth * FDPIScale)
    else
    begin
      if Assigned(FControl) and FControl.Visible then
      begin
        FDropDownForm.Width := Round(ddscale * FOrigCtrlW); // + DropDownBorderWidth * 2;
      end
      else
      begin
    ...
    begin
      FDropDownForm.Height := Round(FDPIScale * FDropDownHeight);
    end
    else
    begin
      // AutoSize
      ...
      if DropDownHeader.Visible then
        h := h + Round(DropDownHeader.Height * FDPIScale);
      if DropDownFooter.Visible then
        h := h + Round(DropDownFooter.Height * FDPIScale);
      if Assigned(FControl) and FControl.Visible then
      begin
        h := h + Round(ddscale * FOrigCtrlH);
      end;
...
begin
  ...
  *)
  ...
  begin
    FDropDownForm.Height := Round(FDPIScale * FUserDropDownHeight);
  end;

  fw := FDropDownForm.Width;
  fh := FDropDownForm.Height;
  ...
  FDropDownForm.Width := fw;
  FDropDownForm.Height := fh;
  ...
  begin
    FDropDownForm.Width := Round(FDPIScale * FUserDropDownWidth);
  end;
...
begin
  ...
  FUserDropDownWidth := Round(FDropDownForm.Width / FDPIScale);
  FUserDropDownHeight := Round(FDropDownForm.Height / FDPIScale);
...
begin
  ...
  if IsCtrl and (msg.CharCode = 65) and (Msg.KeyData = $00100001) (* avoid AltGr *) then
  begin
    SelectAll;
  end;
...
begin
  inherited;
  if Assigned(FLabel) then
    FDPIScale := GetDPIScale(Self, FLabel.Canvas)
  else
  begin
    ...
    begin
      FDPIScale := GetDPIScale(Self, frm.Canvas);
    end
    else
  ...
  if Assigned(FButton) then
  begin
    FButton.FBWidth := Round(FButtonWidth * FDPIScale);
    ResizeControl;
  end;
...
begin
  ...
  tsOffice2019White:
    begin
        DropDownHeader.Color := clWhite;
        DropDownHeader.ColorTo := clNone;
        DropDownHeader.Font.Color := $005E5E5E;
        DropDownHeader.BorderColor := $00ABABAB;

        DropDownFooter.Color := clWhite;
        DropDownFooter.ColorTo := clNone;
        DropDownFooter.Font.Color := $005E5E5E;
        DropDownFooter.BorderColor := $00ABABAB;

        DropDownColor := clWhite;
        DropDownColorTo := clNone;
        DropDownBorderColor := $00C4C6C8;
    end;
  tsOffice2019Gray:
    begin
        DropDownHeader.Color := $00B8BBBE;
        DropDownHeader.ColorTo := clNone;
        DropDownHeader.Font.Color := $00232425;
        DropDownHeader.BorderColor := $00808080;

        DropDownFooter.Color := $00B8BBBE;
        DropDownFooter.ColorTo := clNone;
        DropDownFooter.Font.Color := $00232425;
        DropDownFooter.BorderColor := $00808080;

        DropDownColor := $00B8BBBE;
        DropDownColorTo := clNone;
        DropDownBorderColor := $00666666;
    end;
  tsOffice2019Black:
    begin
        DropDownHeader.Color := $00444444;
        DropDownHeader.ColorTo := clNone;
        DropDownHeader.Font.Color := clWhite;
        DropDownHeader.BorderColor := $00444648;

        DropDownFooter.Color := $00444444;
        DropDownFooter.ColorTo := clNone;
        DropDownFooter.Font.Color := clWhite;
        DropDownFooter.BorderColor := $00444648;

        DropDownColor := $00444444;
        DropDownColorTo := clNone;
        DropDownBorderColor := $00ADB0B3;;
    end;
  end;
   ...
   case AStyle of
    ...
    tsOffice2007Silver:
      ...
      Begin
        ...
        ButtonAppearance.Font.Color := $A6A6A6;
      end;
      ...
      begin
        ...
        ButtonAppearance.Color := clWhite;
        ButtonAppearance.ColorTo := clNone;
        ButtonAppearance.BorderColor := $00ABABAB;

        ButtonAppearance.ColorHot := $00F2E1D5;
        ButtonAppearance.ColorHotTo := clNone;
        ButtonAppearance.BorderColorHot := $00E3BDA3;

        ButtonAppearance.ColorDown := $00E3BDA3;
        ButtonAppearance.ColorDownTo := clNone;
        ButtonAppearance.BorderColorDown := $00E3BDA3;

        ButtonAppearance.ColorChecked := $00E3BDA3;
        ButtonAppearance.ColorCheckedTo := clNone;
        ButtonAppearance.BorderColorChecked := $00E3BDA3;

        ButtonAppearance.ColorDisabled := clWhite;
        ButtonAppearance.ColorDisabledTo := clNone;
        ButtonAppearance.BorderColorDisabled := $00787775;
        ButtonAppearance.Font.Color := $00444648

      end;

      tsOffice2019Gray:
      Begin
        ButtonAppearance.Color := $00B8BBBE;
        ButtonAppearance.ColorTo := clNone;
        ButtonAppearance.BorderColor := $00808080;

        ButtonAppearance.ColorHot := $00969696;
        ButtonAppearance.ColorHotTo := clNone;
        ButtonAppearance.BorderColorHot := $00505050;

        ButtonAppearance.ColorDown := $00666666;
        ButtonAppearance.ColorDownTo := clNone;
        ButtonAppearance.BorderColorDown := $00666666;

        ButtonAppearance.ColorChecked := $00969696;
        ButtonAppearance.ColorCheckedTo := clNone;
        ButtonAppearance.BorderColorChecked := $00666666;

        ButtonAppearance.ColorDisabled := $00D4D4D4;
        ButtonAppearance.ColorDisabledTo := clNone;
        ButtonAppearance.BorderColorDisabled := $00303030;
        ButtonAppearance.Font.Color := $00232425;
      end;

      tsOffice2019Black:
      Begin
        ButtonAppearance.Color := $00444444;
        ButtonAppearance.ColorTo := $00444444;
        ButtonAppearance.BorderColor := $00686868;

        ButtonAppearance.ColorHot := $00686868;
        ButtonAppearance.ColorHotTo := $00686868;
        ButtonAppearance.BorderColorHot := $00B5B5B5;

        ButtonAppearance.ColorDown := $00828282;
        ButtonAppearance.ColorDownTo := $00828282;
        ButtonAppearance.BorderColorDown := $00828282;

        ButtonAppearance.ColorChecked := $00686868;
        ButtonAppearance.ColorCheckedTo := $00686868;
        ButtonAppearance.BorderColorChecked := $00828282;

        ButtonAppearance.ColorDisabled := $00444444;
        ButtonAppearance.ColorDisabledTo := clnone;
        ButtonAppearance.BorderColorDisabled := $00C4C4C4;

        ButtonAppearance.Font.Color := clWhite;
      end;
  end;
  ...
  case AStyle of
    tsOffice2019White:
    begin
      Button.ButtonColor := clWhite;
      Button.ButtonColorHot := $00F2E1D5;
      Button.ButtonColorDown := $00E3BDA3;
      Button.ButtonTextColor := $00444648;
      Button.ButtonTextColorHot := $00232425;
      Button.ButtonTextColorDown := $00232425;
    end;
    tsOffice2019Gray:
    begin
      Button.ButtonColor := $00B8BBBE;
      Button.ButtonColorHot := $00969696;
      Button.ButtonColorDown := $00666666;
      Button.ButtonTextColor := $00232425;
      Button.ButtonTextColorHot := $00232425;
      Button.ButtonTextColorDown := $00232425;
    end;
    tsOffice2019Black:
    begin
      Button.ButtonColor := $00444444;
      Button.ButtonColorHot := $00686868;
      Button.ButtonColorDown := $00828282;
      Button.ButtonTextColor := clWhite;
      Button.ButtonTextColorHot := clWhite;
      Button.ButtonTextColorDown := clWhite;
    end;
    else
    begin
      Button.ButtonColor := clNone;
      Button.ButtonColorHot := clNone;
      Button.ButtonColorDown := clNone;
      Button.ButtonTextColor := clNone;
      Button.ButtonTextColorHot := clNone;
      Button.ButtonTextColorDown := clNone;
      Button.ButtonBorderColor := clNone;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    if Assigned(FControl) then
    begin
      FOrigCtrlW := Round(FControl.Width / FDPIScale);
      FOrigCtrlH := Round(FControl.Height / FDPIScale);

      if (DropDownWidth = 0) and (FOrigCtrlW < Width) then
       FOrigCtrlW := Round((Width - GetSystemMetrics(SM_CXVSCROLL)) / FDPIScale);
...
procedure TAdvCustomDropDown.SetUIStyle(const Value: TTMSStyle);
begin
...
var
  at: TAutoType;
begin
  if (FPrecision <> value) and (editType in [etFloat, etMoney, etString, etValidChars]) then
  begin
    FPrecision := Value;
    at := IsType(Text);
...
begin
  ...
  if not Assigned(ItemAppearance) or (AStyle = tsCustom)  then
    Exit;
  ...
  tsOffice2019White:
    begin
        ItemAppearance.ColorSelected := $00E3BDA3;
        ItemAppearance.ColorSelectedTo := clNone;
        ItemAppearance.ColorMirrorSelected := clNone;
        ItemAppearance.ColorMirrorSelectedTo := clNone;
        ItemAppearance.BorderColorSelected := $00E3BDA3;

       ItemAppearance.ColorHot := $00F2E1D5;
       ItemAppearance.ColorHotTo := clNone;
       ItemAppearance.ColorMirrorHot := clNone;
       ItemAppearance.ColorMirrorHotTo := clNone;
       ItemAppearance.BorderColorHot :=  $00F2E1D5;
    end;
  tsOffice2019Gray:
    begin

        ItemAppearance.ColorSelected := $00656565;
        ItemAppearance.ColorSelectedTo := clNone;
        ItemAppearance.ColorMirrorSelected := clNone;
        ItemAppearance.ColorMirrorSelectedTo := clNone;
        ItemAppearance.BorderColorSelected := $00656565;

       ItemAppearance.ColorHot := $00969696;
       ItemAppearance.ColorHotTo := clNone;
       ItemAppearance.ColorMirrorHot := clNone;
       ItemAppearance.ColorMirrorHotTo := clNone;
       ItemAppearance.BorderColorHot :=  $00B8BBBE;
    end;
  tsOffice2019Black:
    begin
        ItemAppearance.ColorSelected := $00828282;
        ItemAppearance.ColorSelectedTo := clNone;
        ItemAppearance.ColorMirrorSelected := clNone;
        ItemAppearance.ColorMirrorSelectedTo := clNone;
        ItemAppearance.BorderColorSelected := $00828282;
       ItemAppearance.Font.Color := $00F0F0F0;

       ItemAppearance.ColorHot := $00505050;
       ItemAppearance.ColorHotTo := $00505050;
       ItemAppearance.ColorMirrorHot := clNone;
       ItemAppearance.ColorMirrorHotTo := clNone;
       ItemAppearance.BorderColorHot :=  $00505050;
...
begin
  ...
  begin
    ...
    try
      ...
      R := Rect(2,2, Width - Round(FDPIScale * DropDownButtonWidth) - 1, Height - 2);
      ...
      Canvas.Brush.Style := bsClear;
      DrawText(Canvas.Handle, PChar(Text), -1, R, DT_SINGLELINE or DT_VCENTER or DT_LEFT or DT_NOPREFIX);
      Canvas.Free;
    finally
      ReleaseDC(Handle, DC);
    end;
...
begin
  ...
  if se and CheckVCLStylesEnabled(LStyle,(csDesigning in ComponentState)) then
  begin
    Exit;
  end;
  {$ENDIF}
...
begin
  ...
  FDPIScale := FAdvDropDown.FDPIScale;
  ...
  begin
    ...
    begin
      ...
      FHeader.Parent := Self;
      FHeader.Height := Round(AdvDropDown.DropDownHeader.Height * FDPIScale);
  ...
  begin
    ...
    {$IFDEF DELPHIXE6_LVL}
    FHeader.StyleElements := AdvDropDown.StyleElements;
    {$ENDIF}
  end;
  ...
  begin
    ...
    begin
      ...
      FFooter.Parent := Self;
      FFooter.Height := Round(AdvDropDown.DropDownFooter.Height * FDPIScale);
  ...
  begin
    ...
    {$IFDEF DELPHIXE6_LVL}
    FFooter.StyleElements := AdvDropDown.StyleElements;
    {$ENDIF}
  end;
end;
...
begin
  ...
  begin
    ...
    if TAdvCustomDropDown(Owner.Owner).BorderColor <> clNone then
    begin
  ...
  begin
    if FHot then
      LDetails := LStyle.GetElementDetails(tcDropDownButtonHot)
    else
      LDetails := LStyle.GetElementDetails(tcDropDownButtonNormal);

    if ((FState in [bsDown, bsExclusive]) and not FUp) or dropped then
      LDetails := LStyle.GetElementDetails(tcDropDownButtonPressed);

    LStyle.DrawElement(Canvas.Handle, LDetails, ClientRect);
    Exit;
  end;
...
begin
  ...
  if Assigned(Owner) and Assigned(Owner.Owner) then
    scale := (Owner.Owner as TAdvCustomDropDown).FDPIScale;
  ...
  begin
    ...
    begin
      ...
      HTMLDrawEx(Canvas, s, R1, Images, XPos, YPos,-1,0,1,
                 ...
                 XSize,YSize,ml,hl,hr,CR,NewID,CV,CT,nil{FImageCache},nil{FContainer},self.Handle, scale);
...
begin
  ...
  begin
    ...
    begin
      ...
      YPos := 0;
      Canvas.Font.Assign(Appearance.Font);
      Canvas.Font.Height := Round(Canvas.Font.Height * AdvDropDown.FDPIScale);
...
begin
  ...
  if CheckVCLStylesEnabled(LStyle,(csDesigning in ComponentState)) then
  begin
    LStyle.GetElementColor(LStyle.GetElementDetails(tgCellNormal), ecFillColor, BkColor);
    R := ClientRect;
  ...
  if Assigned(AdvItemSelector.OnBeforeDrawItems) then
    AdvItemSelector.OnBeforeDrawItems(Self, Canvas);

  DrawItems(Canvas);

  if Assigned(AdvItemSelector.OnAfterDrawItems) then
    AdvItemSelector.OnAfterDrawItems(Self, Canvas);
end;

//------------------------------------------------------------------------------
procedure TCustomItemSelector.DrawItemBackground(Index: Integer; ACanvas: TCanvas);
var
...
begin
  ...
  begin
    LStyle.GetElementColor(LStyle.GetElementDetails(tgCellNormal), ecFillColor, BkColor);
    LStyle.GetElementColor(LStyle.GetElementDetails(tgCellNormal), ecTextColor, TxtColor);
  end;
  {$ENDIF}
...
begin
  ...
  begin
    iw := Round(ColorBoxWidth * FDPIScale);
    ih := Round(ColorBoxHeight * FDPIScale);
  end;
  ...
  case ItemLayout of
    ...
    begin
      ...
      begin
        ...
        ts := DrawHTMLEX(aCanvas, Items[Index].Caption, TR, Images, False, Handle, FDPIScale);
        if (ts.cy < (TR.Bottom - TR.Top)) then
          TR.Top := TR.Top + (((TR.Bottom - TR.Top) - ts.cy) div 2);
        //--
        ts := DrawHTMLEX(aCanvas, Items[Index].Caption, TR, Images, True, Handle, FDPIScale);
    ...
    begin
      ...
      begin
        ...
        if (ts.cy < (TR.Bottom - TR.Top)) then
          TR.Top := TR.Top + (((TR.Bottom - TR.Top) - ts.cy) div 2);
        //--
        DrawHTMLEX(aCanvas, Items[Index].Caption, TR, Images, True, Handle, FDPIScale);
      end;
    end;
    ilCaptionTop:
    begin
...
begin
  ...
  begin
    ...
    ts := DrawHTMLEX(Canvas, AdvItemSelector.Items[i].Caption, R, Images, False, Self.Handle, FDPIScale);
    tw := max(tw, Round(ts.cx * FDPIScale));
    th := max(th, ts.cy);
  end; // end for
...
procedure TAdvCustomItemSelector.PaintWindow(DC: HDC);
{$IFDEF DELPHIXE2_LVL}
var
  ACanvas: TCanvas;
  LStyle: TCustomStyleServices;
...
initialization
  {$IFDEF DELPHIXE8_LVL}
  TCustomStyleEngine.RegisterStyleHook(TAdvCustomItemSelector, TScrollingStyleHook);
  {$ENDIF}

finalization
  {$IFDEF DELPHIXE8_LVL}
  TCustomStyleEngine.UnRegisterStyleHook(TAdvCustomItemSelector, TScrollingStyleHook);
  {$ENDIF}
