...
uses
  ...
  Windows,
  {$ENDIF}
  Classes, Types, TypInfo, Variants, Generics.Collections,
  AdvGraphics, AdvPersistence, AdvGraphicsTypes, AdvTypes, AdvScrollControl,
...
const
  ...
  // v1.3.3.0 : New : Event OnItemDblClick added
  // v1.3.3.1 : Fixed : VCL styles check updated to work in 10.4 Sydney

const
  TTMSFNCResponsiveListDocURL = 'https://download.tmssoftware.com/doc/tmsfncuipack/components/ttmsfncresponsivelist/';
...
type
  ...
  {$ENDIF}
  {$ENDIF}
  TAdvResponsiveList = class(TCustomScrollingControl, IAdvPersistence)
  private
    ...
    FDPIScale: single;
    FOnItemDblClick: TResponsiveListItemEvent;
  ...
  protected
    ...
    procedure DoItemDblClick(AItem: TResponsiveListItem); virtual;
  ...
  public
    ...
    procedure SaveSettingsToFile(AFileName: string; AAppearanceOnly: Boolean = False); {$IFDEF FNCLIB} {$IFNDEF LCLLIB} override; {$ENDIF} {$ENDIF}
    procedure LoadSettingsFromFile(AFileName: string); {$IFDEF FNCLIB}{$IFNDEF LCLLIB} override; {$ENDIF} {$ENDIF}
    procedure SaveSettingsToStream(AStream: TStreamEx; AAppearanceOnly: Boolean = False); {$IFDEF FNCLIB} {$IFNDEF LCLLIB} override; {$ENDIF} {$ENDIF}
    procedure LoadSettingsFromStream(AStream: TStreamEx); {$IFDEF FNCLIB} {$IFNDEF LCLLIB} override; {$ENDIF} {$ENDIF}
  ...
  published
    ...
    property OnItemDblClick: TResponsiveListItemEvent read FOnItemDblClick write FOnItemDblClick;
...
procedure TAdvResponsiveList.DoItemDblClick(AItem: TResponsiveListItem);
begin
...
begin
  FItemUp := FindItemAtPoint(Point(X + TopLeft.X,Y + TopLeft.Y));
  if Assigned(FItemUp) then
  begin
    DoItemDblClick(FItemUp);
  end;
end;
...
function TAdvResponsiveList.IsAppearanceProperty(AObject: TObject; APropertyName: string; APropertyType: TTypeKind): Boolean;
var
...
function TAdvResponsiveList.CanSaveProperty(AObject: TObject; APropertyName: string; APropertyType: TTypeKind): Boolean;
begin
...
function TAdvResponsiveList.CanLoadProperty(AObject: TObject; APropertyName: string; APropertyType: TTypeKind): Boolean;
begin
  if (AObject = Self) then
    Result := (TAdvUtils.IndexOfTextInArray(APropertyName, ExcludePropertyList) = -1)
  else
...
procedure TAdvResponsiveList.SaveSettingsToFile(AFileName: string; AAppearanceOnly: Boolean = False);
begin
...
procedure TAdvResponsiveList.LoadSettingsFromFile(AFileName: string);
begin
  BeginUpdate;
...
procedure TAdvResponsiveList.SaveSettingsToStream(AStream: TStreamEx; AAppearanceOnly: Boolean = False);
begin
  FAppearancePersisting := AAppearanceOnly;
...
procedure TAdvResponsiveList.LoadSettingsFromStream(AStream: TStreamEx);
begin
  BeginUpdate;
