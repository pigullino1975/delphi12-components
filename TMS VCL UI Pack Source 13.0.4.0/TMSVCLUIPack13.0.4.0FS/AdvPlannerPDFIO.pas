...
uses
  Classes, AdvPDFLib, Planner, AdvPDFIO, Graphics, Windows,
...
const
  ...
  // v1.0.0.2 : Fixed : Issue with rendering item in top Planner cell
  // v1.0.0.3 : Improved : Drawing of time axis in half day display mode
  // v1.0.0.4 : Improved : Drawing of time axis in half day display mode
...
begin
  ...
  if (APlanner.Mode.PLannerType = plHalfDayPeriod) and Odd(Arow + Offs) then
  begin
    Line1 := '';
    Line2 := '';
  end;
  ...
  begin
    ...
    tr := APDFLib.Graphics.DrawText(Line3, PointF(0, 0), True);
    tr.Width := tr.Width + 1 * AScale;
...
procedure TAdvCustomPlannerPDFIO.DoPDFExport(const APDFLib: TAdvPDFLib; const AExportObject: TAdvPDFIOExportObject; const AExportRect: TRectF);
var
...
var
  ...
  begin
    ...
    begin
      ...
      for pi := 0 to mp - 1 do
      begin
        for ri := 0{FRSaveEx} to r - offr do
        begin
