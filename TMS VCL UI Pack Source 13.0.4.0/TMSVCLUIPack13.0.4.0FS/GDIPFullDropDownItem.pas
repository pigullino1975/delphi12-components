...
const
  ...
  // v1.0.0.2 : Fixed : Design-time style painting
  // v1.0.0.3 : Improved : On creation check for enabled AdvFormStyler
...
type
  ...
  TFullDropDownForm = class(TForm)
  private
    FItem: TFullDropDownItem;
  ...
  private
    ...
    ContainerParent: TFullDropDownForm;
...
begin
  ...
  begin
    ...
    begin
      ContainerParent := TFullDropDownForm.CreateNew(parentf);
      ContainerParent.Container := FContainer;
      ContainerParent.FItem := Self;
...
procedure TFullDropDownForm.CreateParams(var Params: TCreateParams);
const
  CS_DROPSHADOW = $00020000;
begin
  inherited CreateParams(Params);
  if Assigned(FItem) and FItem.DropDownAppearance.Shadow then
    Params.WindowClass.Style := Params.WindowClass.Style or CS_DROPSHADOW;
end;

procedure TFullDropDownForm.Init;
begin
  DoubleBuffered := true;
  Visible := false;
  BorderIcons := [];
...
procedure TFullDropDownForm.WMActivate(var Message: TMessage);
begin
  inherited;
  Message.Result := 1;
end;

procedure TFullDropDownForm.WMMouseActivate(var Message: TMessage);
begin
