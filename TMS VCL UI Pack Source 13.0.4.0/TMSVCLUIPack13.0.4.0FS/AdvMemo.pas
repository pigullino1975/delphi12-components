...
uses
  ...
  AdvFindDialogForm, AdvReplaceDialogForm, Types, ImgList
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF}
  ;
...
const
  ...
  BLD_VER = 1; // Build nr.
  DATE_VER = 'Nov, 2023'; // Month version
  ...
  // 3.6.7.0  : New: StyleElements exposed in TAdvMemo
  //          : Fixed : Issue with RTF copy to clipboard
  // 3.6.7.1  : Improved : Code block display in combination with block indicators in comments
  // 3.6.8.0  : New : HighlightWholeWord public property added
  // 3.7.0.0  : New : Support for Office 2019 styles
  //          : Improved : Key handling for autocompletion list
  //          : Fixed : Issue with URL highlighting for wordwrapped text
  // 3.7.0.1  : Improved : Handling of URLs over multiple lines with wordwrapping
  // 3.7.0.2  : Fixed : Issue with DB when dataset is disabled
  // 3.7.1.0  : New : Multiple hex identifiers (comma separated)
  //          : New : Multiple line comment identifiers (comma separated)
  // 3.7.1.1  : Fixed : Issue with border when gutter is invisible and border is turned off
  //          : Fixed : Issue with saving to file when TrimTrailingSpaces = false
  //          : Fixed : Design-time style painting
  // 3.7.1.2  : Fixed : Office 2019 style was not only forced on creation
  // 3.7.1.3  : Fixed : Gutter line drawing color issue
  // 3.7.1.4  : Improved : Detection of text format on clipboard for Paste
  //          : Fixed : Issue with FindTextCount() when text to find occurs multiple times on single lines
  //          : Fixed : Issue with Ctrl-arrow operations when a syntax styler without any delimiters is attached
  //          : Fixed : Issue with Find & Replace in combination with selection
  // 3.7.2.0  : New : Method ClearErrorsForWord added
  //          : Improved : HTML, JS styler background color handling with VCL styles
  //          : Fixed : Issue with CheckWord() checking entire line
  //          : Fixed : Issue with FindText() with option frWholeWord
  //          : Fixed : Issue with find at end of text with wrap
  //          : Fixed : Active line display in gutter for wordwrapped text
  //          : New : UIStyle property for office look
  //          : New : Changed SelStartX,SelStartY,SelEndX,SelEndY to become read/write properties
  //          : Improved : On creation check for enabled AdvFormStyler
  // 3.7.2.1  : Fixed : Issue with wordwrapped hyperlink click
  //          : Fixed : Repeat find in wordwrapped text
  // 3.7.2.2  : Improved : Find & replace within selected text
  // 3.7.2.3  : Fixed : Issue with Paste after programmatic text insert
  // 3.7.2.4  : Fixed : Issue with irregular autocompletion start tokens
  // 3.7.2.5  : Fixed : Issue with using backspace while autocompletionlist is showing
  //          : Fixed : Issue with Undo for DeleteTextAtXY
  // 3.7.3.0  : New : Public property DragCaret added
  // 3.7.3.1  : Fixed : Issue with repeat of Find()
  // 3.7.3.2  : Fixed : Issue with InsertTextAtXY
  // 3.7.4.0  : New : Functions GetLineIndexWrapped, GetLineIndexWrappedEnd added
  //          : Fixed : VCL styles check updated to work in 10.4 Sydney
  // 3.7.4.1  : Fixed : Issue with TAdvMemoFileSaveAction with MemoSource is used
  // 3.7.4.2  : Fixed : Issue with step by step via TAdvMemoFindReplaceDialog
  // 3.7.4.3  : Fixed : Issue with Undo and wordwrapping
  // 3.7.4.4  : Fixed : Incorrect word-break issue on URL
  //          : Fixed : Issue with multiple line comment definitions
  // 3.7.4.5  : Fixed : Issue with UIStyle value for previous existing TAdvMemo
  // 3.7.4.6  : Improved : AutoCompletion type identifier rendering in high DPI
  //          : Fixed : Issue with Paste into selected text
  // 3.7.4.7  : Fixed : Issue with OnKeyUp() for VK_RETURN key
  // 3.7.4.8  : Fixed : Issue with return value of function AddCodeFolding
  // 3.7.4.9  : Fixed : Issue with code folding when no memo syntax styler is assigned
  // 3.7.4.10 : Fixed : Issue with switching WordWrap at runtime and scrollbars
  // 3.7.4.11 : Fixed : Issue with programmatically turning off line modified state
  // 3.7.5.0  : Improved : Code block highlightingµ
  // 3.7.6.0  : New : SyntaxStyler.AutoBlockEnd property added to auto insert block end
  //          : Fixed : Issue with using MemoSource and code folding
  // 3.7.6.1  : Fixed : Issue with default value property AutoBlockEnd
  //          : Fixed : Rare issue with multiple consecutive comments
  // 3.7.7.0  : New : Event OnAfterPaint added
  // 3.7.8.0  : New : Public read-only properties CharWidth/CharHeight exposed
  // 3.7.9.0  : New : Function GetLastChildOfParentNode(Index: integer);
  //          : New : Function NodeWidth: integer
  //          : Fixed : Rare issue with pressing enter breaking specific keywords
  // 3.7.10.0 : New : Event OnNewLineIndent added
  //          : Fixed : Issue with codefolding lines when no gutter is visible
  // 3.8.0.0  : New : Block brackets support added
  // 3.8.0.1  : Fixed : Issue with function GetLastChildOfParentNode(Index: integer);
  // 3.8.1.0  : New : Public method FindNext on TAdvMemoFindDialog
  ...
  // 3.8.1.1  : Fixed : Issue with OnHintForToken, OnHintForWord and comments
  // 3.8.2.0  : New : Binary identifier setting in syntax styler to highlight also binary formatted numbers
  // 3.8.2.1  : Fixed : Issue with code block completion
  // 3.8.3.0  : New : Exposed OnContextPopup event
  // 3.8.3.1  : Fixed : Issue with getting token at cursor when token is preceeded by specific characters
  // 3.8.4.0  : New : MarkerImageList is now compatible with TVirtualImageList
  // 3.8.4.1  : Fixed : Auto completion handling in specific scenarios
  // 3.8.4.2  : Fixed : Issue with OnContextPopup
  // 3.8.4.3  : Fixed : Issue with search on whole word in specific cases
  // 3.8.4.4  : Fixed : Rare issue with Find and setting memo cursor after reaching end of find
  // 3.8.4.5  : Improved : Find & replace dialog trigging from TAdvMemo
  // 3.8.4.6  : Fixed : Issue with up-search with WholeWordOnly option
  // 3.8.4.7  : Fixed : Issue with copying a single line of text as RTF on clipboard
  //          : Fixed : Issue with URL extraction on multi-line wordwrapped lines
  // 3.8.4.8  : Improved : Changed OnBeforeAutoCompletion to allow to show auto completion list when no character is typed
  // 3.8.5.0  : Improved : SQL syntax styler. New keywords and single-quote values added
  // 3.8.6.0  : New : Property MoveLines added to move lines up/down with Alt-Up/Alt-Down keys
  // 3.8.6.1  : Fixed : Issue with HTML export with system colors
  // 3.8.7.0  : New : Multi start block/end block definitions support added (comma-separated)
  // 3.8.7.1  : Fixed : Memory leak with new multi block definitions
  // 3.8.7.2  : Fixed : Issue with block boundary detection when no syntax styler is used
  // 3.8.8.0  : New : Added FindPrev method to Find & replace dialogs
  // 3.8.8.1  : Fixed : Issue with TAdvMemoFindDialog.CloseDialog when TAdvFindDialog is used
  // 3.8.8.2  : Fixed : Issue with undo and AutoExpand = true
  // 3.9.0.0  : New : Auto formatter for TAdvMemoHTMLStyler
  // 3.9.0.1  : Fixed : Issue with DB update after calling DBAdvMemo.Lines.LoadFromFile()
...
type
  ...
  msOffice2019White, msOffice2019Gray, msOffice2019Black);
  ...
  TAdvMemoNewLineIndent = procedure(Sender: TObject; LineNr: integer; var NewLineIndent: string) of object;
...
//------------------------------------------------------------------------------
  ...
  published
    property MarkerImageList: TCustomImageList read FMarkerImageList write SetMarkerImageList;
...
//------------------------------------------------------------------------------
  ...
  TMemoPaintEvent = procedure(Sender: TObject; ACanvas: TCanvas) of object;
...
//------------------------------------------------------------------------------
  ...
  public
    ...
    property IsBegin: boolean read FIsBegin write FIsBegin;
    property Indent: integer read FIndent write FIndent;
  end;
...
//------------------------------------------------------------------------------
  ...
  private
    ...
    FtmpNoStart, FtmpNo, FtmpNoHex, FTmpNoBin: string;              
    FCaretTime: cardinal;
    ...
    FDesignTime: boolean;
    FDragCaret: boolean;
    FOnAfterPaint: TMemoPaintEvent;
    FOnNewLineIndent: TAdvMemoNewLineIndent;
    FBlockLines: TList;
    FBlockLineColors: TIntList;
    FMoveLines: boolean;
    ...
    procedure SetHighlightWholeWord(const Value: Boolean);
    procedure SetUIStyle(const Value: TTMSStyle);
    function GetMemoLines: TAdvMemoStrings;
    procedure ClearBlockLines;
    procedure SetBlockBrackets(const Value: Boolean);
  protected
    FLetRefresh: Boolean;
    ...
    {$ENDIF}
    function GetActiveStyler: TAdvCustomMemoStyler;
    function HasActiveStyler: boolean;
    procedure InitCompletionList(token: string); virtual;
    ...
    property InternalStyles: TAdvCustomMemoStyler read GetActiveStyler;
    ...
    function IsBlockBoundary(ch: char): boolean; virtual;
    procedure ClearUndoList;
    procedure UndoChange;
    procedure EditModify; virtual;
    function GetCursorEx: TCursor;
    procedure CursorChanged; virtual;
    procedure CalcBlockLines; virtual;
    procedure PaintBlockLines(ACanvas: TCanvas); virtual;
    ...
    property BlockBrackets: Boolean read FBlockBrackets write SetBlockBrackets default False;
    ...
    { Events }
    property OnAfterPaint: TMemoPaintEvent read FOnAfterPaint write FOnAfterPaint;
    ...
    property OnNewLineIndent: TAdvMemoNewLineIndent read FOnNewLineIndent write FOnNewLineIndent;
  ...
  public
    ...
    function GetLineIndexWrapped(Index: Integer): Integer;
    function GetLineIndexWrappedEnd(Index: Integer): Integer;
    ...
    procedure ClearErrorsForWord(const AWord:string);
    ...
    property HighlightWholeWord: Boolean read FHighLightWholeWord write SetHighlightWholeWord;
    property Lines: TAdvMemoStrings read GetMemoLines write SetLines;
    property MoveLines: boolean read FMoveLines write FMoveLines default false;
    property BorderStyle: TBorderStyle read FBorderStyle write SetBorderStyle;
    property Ctl3D: boolean read FCtl3D write SetCtl3D;
    property DragCaret: boolean read FDragCaret write FDragCaret;
    ...
    property SelStartX: Integer read FSelStartX write FSelStartX;
    property SelStartY: Integer read FSelStartY write FSelStartY;
    property SelEndX: Integer read FSelEndX write FSelEndX;
    property SelEndY: Integer read FSelEndY write FSelEndY;
    property URLDelimiters: string read FURLDelimiters write FURLDelimiters;
    property BlockLineColors: TIntList read FBlockLineColors;
    ...
    function GetLastChildOfParentNode(Index: integer): integer;
...
//------------------------------------------------------------------------------
  ...
  protected
    ...
    function PosLineComment(s: string; var cl,ci,ct: integer): integer;
  ...
  public
    ...
    property AutoBlockEnd: boolean read FAutoBlockEnd write FAutoBlockEnd;
    ...
    property BinIdentifier: String read FBinIdentifier write FBinIdentifier; 
  ...
  private
    ...
    FFindDone: boolean;
    FFindDoneCurX,FFindDoneCurY: integer;
    FFindText: string;
...
uses
  ClipBrd, ComObj, ShellAPI, Math, IniFiles, Consts, AdvMDD, ActiveX, StrUtils
...
function VarPosCase(su,s:string;DoCase,DoWholeWord:boolean; var Res, SkipRes:Integer): Integer;

  function CheckWordEnd(ch: char): boolean;
  begin
    Result := Ord(ch) in [13, 32, 9, ord('.'),ord(','),ord('='),ord(';'),ord(':'),ord('?'),ord('!')];
  end;
...
begin
  ...
  if DoWholeWord then
    if Res >= 1 then
    begin
      if (Res > 1) and (s[Res - 1] <> ' ') then
        Res := 0
      else
        if (Res + Length(su) <= Length(s)) and not CheckWordEnd(s[Res + Length(su)]) then
          Res := 0;
    end;
end;

function MultiPos(su, s: string): integer;
var
  cs, csu: string;
  cp,cr: integer;
begin
...
begin
  ...
  begin
    ...
    WriteBoolVal('BlockBrackets',BlockBrackets);
...
begin
  ...
  begin
    ...
    FBlockBrackets := ReadBool(INISection, 'BlockBrackets', FBlockBrackets);
...
begin
  ...
  begin
    if (SelStartY = SelEndY) then // single line RTF copy
    begin
      if SelStartX < SelEndX then
        s := Copy(sl, SelStartX, SelEndX - SelStartX + 1)
      else
        s := Copy(sl, SelEndX, SelStartX - SelEndX + 1);

      InternalLines[SelStartY]  := s;
    end;
  ...
  begin
    ...
    try
      ...
      if (cfRTF in ClipboardFormats) and (rtfstr <> '') then
      begin
...
begin
  ...
  if (FSelStartX = FSelEndX) and (FSelStartY = FSelEndY) then
  begin
    if (CurX <> FSelStartX) and (CurX <> FSelEndX) and (FSelStartX = 0) and (FSelEndX = 0) then
    begin
      FSelStartX := CurX;
      FSelStartY := CurY;
      FSelEndX := CurX;
...
begin
  ...
  OldS := Copy(s, X + 1, NumChar);

  Delete(s, X + 1, NumChar);
  InternalLines[y] := s;

  Undo := TDeleteBufUndo.Create(OldX, OldY, X, Y, OldS);
  Undo.UndoSelStartX := X;
  Undo.UndoSelStartY := Y;
  Undo.UndoSelEndX := X + NumChar;
  Undo.UndoSelEndY := Y;
  InternalUndoList.Add(Undo);
end;
...
begin
  ...
  begin
    ...
    begin
      if AutoExpand then
      begin
        str := StringOfChar(' ', Value - len);
        if InternalLines.Count > 0 then
          InternalLines[CurY] := InternalLines[CurY] + str
        else
          InternalLines.Add(str);

        InternalUndoList.Add(TPasteUndo.Create(OldX, OldY, CurX, CurY, str, false));
      end
      else
      begin
        if InternalLines.Count = 0 then
...
begin
  ...
  if Assigned(OnNewLineIndent) then
    OnNewLineIndent(Self, CurY, Result);
end;
...
{$IFDEF DELPHIXE2_LVL}
  ...
  if CheckVCLStylesEnabled(LStyle,(csDesigning in ComponentState)) {$IFDEF DELPHIXE6_LVL} and (seClient in StyleElements) {$ENDIF} then
  begin
    FUseVCLStyles := True;
...
begin
  ...
  if AutoExpand then
  begin
    if OldX > Length(InternalLines[OldY]) then
      OldX := Length(TrimRight(InternalLines[oldY]));
  end;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                begin
                  LUndo1 := InternalUndoList.Items[InternalUndoList.Count - 1];
                  LUndo2 := InternalUndoList.Items[InternalUndoList.Count - 2];
                  SkipIns := (LUndo1 is TInsertOverWriteUndo) and
                     (LUndo2 is TInsertOverWriteUndo);
                end;
                if not SkipIns then
...
{$ENDIF}
  ...
  begin
    LWord := WordAtCursorPos(p);
    if Length(LWord) > 0 then
    begin
      ClearWordError(CurY, p - 1);
    end;
  end;
  ...
  case Key of
    ...
    VK_UP:
      begin
        ...
        if (ssAlt in Shift) and MoveLines then
        begin
          if CurY > 0 then
          begin
            LWord := Lines[CurY - 1];
...
var
  wrd, newword, blockstart,s, matchend: string;
  ps, pe, d, i,startindex: Integer;
  r: TRect;
  selcopy: boolean;
  countstart, countend, indent: integer;
  sl: TStringList;
  hasstart: boolean;
...
begin
  ...
  if Ord(Key) = 13 then
  begin
    if HasActiveStyler then
    begin
      if GetActiveStyler.AutoBlockEnd then
      begin
        blockstart := uppercase(WordTillCursor);

        sl := TStringList.Create;
        sl.Delimiter := ',';
        sl.CommaText := Uppercase(GetActiveStyler.BlockStart);
        startindex := sl.IndexOf(blockstart);
        hasstart := startindex <> -1;
        sl.Free;
//        if pos(blockstart, Uppercase(GetActiveStyler.BlockStart)) > 0 then
        if hasstart then
        begin
          i := CurY - 1;
          ...
          while (i >= 0)  do
          begin
            s := Trim(Uppercase(InternalLines[i]));

            if pos(s, Uppercase(GetActiveStyler.BlockEnd)) > 0 then
              i := 0
            else
            begin
              if pos(s, Uppercase(GetActiveStyler.BlockStart)) > 0 then
                inc(countstart);
          ...
          begin
            ...
            while (length(s) > 0) and IsDelimiter(s[length(s)]) do
              delete(s, length(s), 1);

            if pos(s, Uppercase(GetActiveStyler.BlockStart)) > 0 then
              i := InternalLines.Count
            else
            begin
              if pos(s, Uppercase(GetActiveStyler.BlockEnd)) > 0 then
                inc(countend);
          ...
          begin
            sl := TStringList.Create;
            sl.Delimiter := ',';
            sl.CommaText := GetActiveStyler.BlockEnd;
            if (sl.Count > 0) and (startindex < sl.Count) then
              matchend := sl.Strings[startindex]
            else
              matchend := sl.Strings[0];
            sl.Free;

            InsertText(#13#13+ StringOfChar(' ', indent)+ matchend + GetActiveStyler.LineDelimiter);
            CurY := CurY - 1;
            CurX := CurX - length(GetActiveStyler.BlockEnd) - length(GetActiveStyler.LineDelimiter)+2;
            Key :=  #0;
          end;
        end;
      end;
...
begin
  ...
  begin
    ...
    begin
      TUndo(FUndoList.Items[FUndoList.Count - 2]).LinkedUndo := true;
    end;
...
begin
  rct := EditorRect;
  ARow := ARow - LineIndexToVisIndex(FTopLine);
  with FCellSize do
...
function TAdvCustomMemo.CharHeight: integer;
begin
  Result := FCellSize.H;
end;

function TAdvCustomMemo.CharWidth: integer;
begin
  Result := FCellSize.W
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (blockcomment = 0) and (not bracket) then
          begin
            ll := MultiPos(InternalStyles.LineComment, s);
...
function TAdvCustomMemo.GetLastChildOfParentNode(Index: integer): integer;
var
  Tlp: TlineProp;
begin
  Tlp := InternalLines.GetLineProp(Index);
  if Assigned(Tlp) and (tlp is TLineProp) then
    Result := tlp.LastChildOfParents
  else
    Result := -1;
end;

function TAdvCustomMemo.GetLineIndexWrapped(Index: Integer): Integer;
var
  i: integer;
begin
  Result := -1;
  ...
  begin
    if GetWrappedLineIndex(i) = Index then
    begin
      Result := i;
      break;
    end;
...
function TAdvCustomMemo.GetLineIndexWrappedEnd(Index: Integer): Integer;
var
  i: integer;
  found: boolean;
begin
...
function TAdvCustomMemo.HasActiveStyler: boolean;
var
  res:TAdvCustomMemoStyler;
begin
  res := GetActiveStyler;
...
function TAdvCustomMemo.GetActiveStyler: TAdvCustomMemoStyler;
begin
  Result := nil;

  if Assigned(MemoSource) then
    Result := MemoSource.SyntaxStyler;
...
begin
  if Value then
    ...
    SetModifiedStateInt(Index,lmUnmodified);
end;
...
procedure TAdvCustomMemo.SetHighlightWholeWord(const Value: Boolean);
begin
  if (FHighlightWholeWord <> Value) then
  begin
    FHighlightWholeWord := Value;
    Invalidate;
  end;
end;
...
var
  ...
  LineSelStart, LineSelEnd, posln, i, ep, XC, ls, dw, gw, hvp, hskip, maxb, hcp: integer;
  urls: TStringList;
  lnstr,hs,cs: string;
  S, S1, S2, S3, SH, SHP: string;
  xSelStartX, xSelStartY, xSelEndX, xSelEndY: integer;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        LineCanvas.MoveTo(mr.Left + dw - dx , 0);
        LineCanvas.LineTo(mr.Left + dw - dx, rct.Bottom - rct.Top);
  ...
  var
    ...
    begin
      if (DrawStyle.index >= 0) and (DrawStyle.index < InternalStyles.FAllStyles.Count) then
      begin
...
//      lastch := #0;
    ...
    begin
      ...
      begin
        ...
        if SegmentStyle.isComment = 0 then
        begin
          if (not SegmentStyle.isBracket) and (MultiPos(InternalStyles.LineComment, s) = tostart) then
          begin
      ...
      if validno then
        if (S[tostart] <> ',') and (AnsiPos(UpCase(S[tostart]), FtmpNoStart) > 0) then
        begin
          if Pos(',', FTmpNoHex) > 0 then
          begin
            numsallowed := FTmpNo;
            SH := FTmpNoHex;

            while length(SH) > 0 do
            begin
              hcp := pos(',', SH);
              if hcp > 0 then
                SHP := Copy(SH,1, hcp - 1)
              ...
              if Pos(SHP, Uppercase(s)) = toStart then
              begin
                numsallowed := FtmpNo + 'ABCDEF';
                toEnd := toEnd + length(SHP);
                break;
              end;
          ...
          if Pos(FTmpNoBin, Uppercase(s))= toStart then  
          begin
            numsallowed := '01';
            toEnd := toEnd + length(ftmpNoBin);
          end                                 
          else
            numsallowed := FTmpNo;
...
begin
  ...
  begin
    ...
    begin
      while (HighlightText <> '') and (VarPosCase(HighlightText, s, HighlightCaseSensitive, HighLightWholeWord, hvp, hskip) > 0) do
      begin
        if hvp > 0 then
        begin
          hs := copy(s, 1, hvp - 1);
        ...
        begin
          hs := copy(s, 1, hskip + Length(HighLightText) - 1);
          DrawSegments(hs, rct1, style);
        end;

        s := copy(s, hskip + Length(HighLightText), Length(s));
      end;
      ...
      begin
        ...
        begin
          ...
          // first word of line where previous line did have unterminated URL
...
{$ENDIF}
  ...
  begin
    ...
    if (Assigned(InternalStyles)) and (UseStyler) then
    begin
      FtmpNoStart := UpperCase(InternalStyles.FNumericChars + InternalStyles.FHexIdentifier + InternalStyles.fBinIdentifier);     
      FtmpNo := UpperCase(InternalStyles.FNumericChars) + 'E';
      FtmpNoHex := Uppercase(InternalStyles.FHexIdentifier);
      FtmpNoBin := Uppercase(InternalStyles.FBinIdentifier);  
    end
    else
    begin
      FtmpNoStart := '';
...
{$IFDEF FREEWARE}
    ...
    Canvas.TextOut(eRect.Left, eRect.Bottom - 14, 'Copyright © 2021 tmssoftware.com');
...
function TAdvCustomMemo.NodeWidth: integer;
begin
  Result := NODE_WIDTH;
end;
...
procedure TAdvCustomMemo.SetUIStyle(const Value: TTMSStyle);
begin
...
begin
  ...
  FBlockLines := TList.Create;
  FBlockLineColors := TIntList.Create;
  FBlockLineColors.Add(integer($00FF8080));
  FBlockLineColors.Add(integer($00FF80FF));
  FBlockLineColors.Add(integer($00A8FFD3));
  FBlockLineColors.Add(integer($00D8B0FF));
  FBlockLineColors.Add(integer($009BBCFF));
  FBlockLineColors.Add(integer($00FFCCCC));
end;
...
begin
  ...
  begin
    ...
    while (ToEn <= PartLen) and (S[ToEn] <> #32) and (not ( (S[ToEn] = ';') {or (S[ToEn] = ':')}))
      and (Assigned(InternalStyles) and  (S[ToEn] <> InternalStyles.FMultiCommentRight)) do
      Inc(ToEn);
    ...
    if HasActiveStyler then
    begin
      for rgn := 0 to GetActiveStyler.RegionDefinitions.Count - 1 do
      begin
        if (UpperCase(PartS) = UpperCase(GetActiveStyler.RegionDefinitions.Items[rgn].Identifier))
          or (UpperCase(PartS) = UpperCase(GetActiveStyler.RegionDefinitions.Items[rgn].RegionStart))
          or (UpperCase(PartS) = UpperCase(GetActiveStyler.RegionDefinitions.Items[rgn].RegionEnd)) then
        begin
          if GetActiveStyler.RegionDefinitions.Items[rgn].ShowComments then
          begin
            Result := True;
            RgnIndex := rgn;
            ToSt := ToEn;
            PartLen := Length(S);
            ToEn := ToSt;
            while (ToEn <= PartLen) and (S[ToEn] <> GetActiveStyler.FMultiCommentRight) do
              Inc(ToEn);
...
var
  ...
  begin
    ...
    begin
      Mc := GetActiveStyler.PosCommentLeft(s,cl, allb, actl);

      //Mc := AnsiPos(InternalStyles.FMultiCommentLeft, s);
      Lc := MultiPos(GetActiveStyler.LineComment, s);

      if (Mc > 0) and (Lc > 0) then
      begin
        if (Mc < Lc) then
        begin
          Rc := GetActiveStyler.PosCommentRight(s,cl, arrb, actr);
          //Rc := AnsiPos(InternalStyles.FMultiCommentRight, s);
          if (Rc > 0) then
          begin
            Delete(s, Mc, Rc - Mc);
      ...
      begin
        ...
        begin
          ...
          toEnd := toStart;
          while (toend <= Len) and (S[toend] <> #32) and (not ( (s[toend] = ';') or (s[toend] = ':')))
            and (s[toend] <> GetActiveStyler.FMultiCommentRight) do
            Inc(toend);

          part := Trim(Copy(S, toStart, toEnd{ - toStart}));

          for l := 0 to GetActiveStyler.RegionDefinitions.Count - 1 do
          begin
            if (UpperCase(part) = UpperCase(GetActiveStyler.RegionDefinitions.Items[l].Identifier))
              or (UpperCase(part) = UpperCase(GetActiveStyler.RegionDefinitions.Items[l].RegionStart))
              or (UpperCase(part) = UpperCase(GetActiveStyler.RegionDefinitions.Items[l].RegionEnd)) then
            begin
              SL.Add(part);
              RgnFound := True;
              Break;
...
begin
  if FCodeFolding.Enabled and HasActiveStyler and UseStyler and (GetActiveStyler.RegionDefinitions.Count > 0) then
  begin
    ...
    for i := 0 to GetActiveStyler.FAllStyles.Count - 1 do
    begin
      if GetActiveStyler.FAllStyles.Items[i].FStyleType <> stSymbol then
        Continue;
      Tempdelimiters := Tempdelimiters + GetActiveStyler.FAllStyles.Items[i].FSymbols;
    end;
    ...
    begin
      ...
      begin
        for r := 0 to GetActiveStyler.RegionDefinitions.Count - 1 do
        begin
          if UpperCase(KL[k]) = UpperCase(GetActiveStyler.RegionDefinitions.Items[r].Identifier) then
          begin
            case GetActiveStyler.RegionDefinitions.Items[r].RegionType of
              rtClosed, rtIgnore:
              begin
                if (UpperCase(GetActiveStyler.RegionDefinitions.Items[r].Identifier) = UpperCase(GetActiveStyler.RegionDefinitions.Items[r].RegionStart)) then
                begin
                  // Add in List
          ...
          if Uppercase(KL[k]) = UpperCase(GetActiveStyler.RegionDefinitions.Items[r].RegionStart) then
          begin
            case GetActiveStyler.RegionDefinitions.Items[r].RegionType of
              rtClosed, rtIgnore:
              begin
          ...
          if Uppercase(KL[k]) = UpperCase(GetActiveStyler.RegionDefinitions.Items[r].RegionEnd) then
          begin
            case GetActiveStyler.RegionDefinitions.Items[r].RegionType of
              rtClosed, rtIgnore:
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      if KL[k] = UpperCase(GetActiveStyler.RegionDefinitions.Items[StrToInt(ClosedRL.Values[ClosedRL.Names[j]])].RegionEnd) then
                      begin
                        RgnFound := True;
    ...
    begin
      if (ClosedRL.Objects[i] <> nil) and (GetActiveStyler.RegionDefinitions.Items[Integer(ClosedRL.Objects[i])-1].RegionType <> rtIgnore) then
      begin
...
begin
  ...
  begin
    ...
    begin
      ...
      TlpSt.ByCode := ByCode;
      TlpSt.LastChildOfParents := EndLineIndex;
...
begin
  ...
  {$ENDIF}
  {$IFDEF DELPHI_UNICODE}
  Result := (ch = #32) or (ch = #39) or (ch = '"') or (ch = ')') or (ch = ';') or (ch='=') or (ch = ']') or (ch='+') or (ch='-') or (ch='^') or (ch='*') or (ch ='/') or (ch = '>') or (ch = '>') or (ch = '(');
  {$ENDIF}
end;
...
//  if TokenSeparator(s[Length(s)]) then
//    Exit;
...
begin
  ...
  for i := CurX downto 1 do
  begin
    tb := IsTokenBoundary(s[i]);
    if (not tb) or (tb and IsBlockBoundary(s[i])) then
      fb := i
    else
      Break;
  end;
...
begin
  ...
  try
    sl.Delimiter := ',';
    LStyler := GetActiveStyler;

    if Assigned(LStyler) then
    begin
      sl.CommaText := LStyler.BlockStart;
...
begin
  ...
  {$ENDIF}
  {$IFDEF DELPHI_UNICODE}
  Result := (ch = #32) or (ch = #39) or (ch = '"') or (ch = ',') or (ch = ';') or (ch = '.') or (ch = '?') or (ch = '!') or (ch = '´') or (ch = '`') or (ch = '<') or (ch = '>') or (ch = '+') or (ch = '-') or (ch = '/') or (ch = '*');
  {$ENDIF}
end;
...
begin
  ...
  Result := (ch = #32) or (ch = '(') or (ch = ')') or (ch = '[') or (ch = ']') or (ch = ',') or (ch = '.')
    or (ch = ':') or (ch = ';') or (ch = '"') or (ch = '''') or (ch = '=') or (ch = '´') or (ch = '`') or (ch = '<') or (ch = '>') or (ch = '+') or (ch = '-') or (ch = '/') or (ch = '*');
  {$ENDIF}
end;
...
procedure TAdvCustomMemo.EditModify;
begin
  //
end;
...
procedure TAdvCustomMemo.ResetUpdate;
begin
...
begin
  FTMSStyle := TTMSStyle(AStyle);
  
  if AStyle = msCustom then
    Exit;
  ...
  msWindowsXP:
    ...
    msOffice2019White:
    begin
      Gutter.LineNumberTextColor := $003B3B3B;
      Gutter.FColor := clWhite;
      Gutter.FColorTo := clWhite;
    ...
    begin
      Gutter.LineNumberTextColor := $003B3B3B;
      Gutter.FColor := $00D4D4D4;
      Gutter.FColorTo := $00D4D4D4;
      BorderColor := $00808080;
    end;
    msOffice2019Black:
    begin
      Gutter.LineNumberTextColor := $00F0F0F0;
      Gutter.FColor := $00444444;
      Gutter.FColorTo := $00444444;
...
begin
  ...
  if Assigned(Memo) then
  begin
    if Memo.TrimTrailingSpaces then
...
begin
  ...
  Font.Height := -13;
  Font.Style := [];
  DoubleBuffered := true; //not IsVista;
...
begin
  ...
  begin
    ...
    begin
      ...
      if ((AcLineNo + FGutter.FLineNumberStart) mod FGutter.LineNumberAt = 0) or (AcLineNo = GetWrappedLineIndex(CurY)) then
      begin
        TmpBrushStyle := Brush.Style;
        Brush.Style := bsClear;
      ...
      begin
        ACanvas.Pen.Color := FGutter.FLineNumberTextColor;
        ACanvas.Pen.Width := 1;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        mres := AnsiPos(AllStyles[i].CommentRight, s);
        if (mres > 0) and (mres < maxres) then
        begin
          ct := i;
...
function TAdvCustomMemoStyler.PosLineComment(s: string; var cl,ci,ct: integer): integer;
begin
  cl := 0;
  ci := -1;
  ct := -1;
  ...
  if LineComment <> '' then
  begin
    Result := AnsiPos(LineComment, s);
    cl := Length(LineComment);
  end;
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if urls.Count = 1 then
              begin
                while cury + 1 < InternalLines.Count do
                begin
                  str := InternalLines[cury + 1];
                  ExtractURL(str, temp);
        ...
        begin
          ...
          if (CurY < InternalLines.Count) and (WordWrap <> wwNone) and (i = urls.Count - 1) then
          begin
            deltax := 1;
            ...
            repeat
              ...
              if cury + deltax < InternalLines.Count then
              begin
                 Tlp := InternalLines.GetLineProp(CurY + deltax);
                 if Assigned(Tlp) and (Tlp is TLineProp) and (WordWrap <> WWNone) and tlp.Wrapped then
                 begin
                   str := InternalLines[cury + deltax];
                   ExtractURL(str, temp);
                   if (temp.Count > 0) and (pos(' ', temp.Strings[0]) = 0) then
                     url := url + temp.Strings[0];
...
var
  ...
  begin
    ...
    begin
      ...
      {$IFDEF DELPHI_UNICODE}
      if (position = 1) and CharInSet(sc[length(SearchStr) + 1],worddelim) then
      begin
        Delete(s, 1, Position + Length(SearchStr));
        lineoffset := lineoffset + Position + Length(SearchStr);
        cl := CompareLineText;
        if cl = 0 then
          Result := 0
        else
          Result := CompareLineText + lineoffset;
        Exit;
      end;
      {$ENDIF}

      {$IFNDEF DELPHI_UNICODE}
      if (position = 1) and (sc[length(SearchStr) + 1] in worddelim) then
      begin
        Delete(s, 1, Position + Length(SearchStr));
        lineoffset := lineoffset + Position + Length(SearchStr);
        cl := CompareLineText;
      ...
      {$IFDEF DELPHI_UNICODE}
      if (CharInSet(sc[position - 1], worddelim)) or (CharInSet(sc[length(SearchStr) + position], worddelim)) then
      begin
        Delete(s, 1, Position + Length(SearchStr));
        lineoffset := lineoffset + Position + Length(SearchStr);
        cl := CompareLineText;
      ...
      {$IFNDEF DELPHI_UNICODE}
      if (sc[position - 1] in worddelim) or (sc[length(SearchStr) + position] in worddelim) then
      begin
        Delete(s, 1, Position + Length(SearchStr));
        lineoffset := lineoffset + Position + Length(SearchStr);
        cl := CompareLineText;
  ...
  begin
    ...
    begin
      repeat
        ...
        if freDown in Options then
        begin
          if Position < Length(s) + Length(SearchStr) then
          begin
            Delete(s,1,Position + Length(SearchStr));

            l := l + Position + Length(SearchStr);
      ...
      until Length(s) < Length(SearchStr);
...
begin
  ...
  begin
    ...
    begin
      if (FSelStartX > FSelEndX) or not (freSelection in Options) then
      begin
        if (freDown in Options) then
          FSelStartX := FSelEndX;
      end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
//          Delete(s, 1, FSelEndX - FSelStartX);
//          cit := FSelEndX;
//          FSelStartX := FSelEndX;
//          CurX := FSelEndX;
        end;
...
var
  rz,oldrz,count: integer;
  sex,sey,dx,rsx,rsy,oldsey: integer;
  firstrz: integer;
begin
  oldrz := -1;
  count := 0;
  ...
  begin
    ...
    repeat
      ...
      if rz = firstrz then
        ...
        firstrz := firstrz + (Length(NewStr) - Length(SearchStr));
...
begin
  ...
  begin
    frm := (AOwner as TCustomForm);
    for i := 0 to frm.ControlCount - 1 do
    begin
      if (frm.Controls[i] is TAdvCustomMemo) then
      begin
        FAdvMemo := (frm.Controls[i] as TAdvCustomMemo);
        break;
      end;
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      if not FFindDialogEx.Visible then
      begin
      ...
      begin
        SetActiveWindow(FFindDialogEx.Form.Handle);
      end;
    end
    else
    begin
...
begin
  ...
  begin
    ...
    if not prev then
    begin
      if (FFindDoneCurX = FAdvMemo.CurX) and (FFindDoneCurY = FAdvMemo.CurY) then
      begin
        FAdvMemo.CurX := 0;
        FAdvMemo.CurY := 0;
        FAdvMemo.FSelStartX := 0;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        FFindDoneCurX := FAdvMemo.CurX;
        FFindDoneCurY := FAdvMemo.CurY;
...
procedure TAdvMemoFindDialog.FindPrev;
begin
  FindDir(true);
end;
...
begin
  ...
  begin
    ...
    begin
      if not FReplaceDialogEx.Visible then
      begin
      ...
      begin
        SetActiveWindow(FReplaceDialogEx.Form.Handle);
      end;
    end
    else
    begin
...
procedure TAdvMemoFindReplaceDialog.FindNext;
begin
  FReplaceDialogEx.Options := FReplaceDialogEx.Options + [AdvReplaceDialogForm.fdoDown];
  Find(Self);
end;

procedure TAdvMemoFindReplaceDialog.FindPrev;
begin
  FReplaceDialogEx.Options := FReplaceDialogEx.Options - [AdvReplaceDialogForm.fdoDown];
  Find(Self);
end;
...
procedure TAdvMemoFindReplaceDialog.ReplaceNext;
begin
  Replace(Self);
end;
...
begin
  ...
  begin
    ...
    // multi replace in selection
    if (FAdvMemo.Selection = '') and (freSelection in Options) then
    begin
      rz := FAdvMemo.FindText(AFindText, Options);
      FAdvMemo.Invalidate;
      if (rz = -1) then
...
procedure TAdvCustomMemo.InitCompletionList(token: string);
var
  i: integer;
  method: string;
  vp: integer;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (pos(token, method) = 1) or (token = '') then
        begin
...
{$ENDIF}
  ...
  begin
    ...
    if token <> '' then
    begin
      st := copy(token, length(token), 1);
      if pos(st, AutoCompletion.StartToken) > 0 then
        token := '';
    end;
...
//check if event handler modified the string, if not then apply std behaviour
  ...
  //so Test(parama : string); becomes Test
      ...
      begin
        if not ((sEntry[i] = '(') or (sEntry[i] = ';') or (sEntry[i] = ' ') or (sEntry[i] = ':') or (sEntry[i] = '.') or (sEntry[i] = '=') or (sEntry[i] = '>') or (sEntry[i] = '<')) then
          s := s + sEntry[i]
        else
          break;
      end;
    ...
    begin
      ...
      begin
        for vp := CurX + 1 to length(s) do
          if (s[vp] = #32) or (s[vp] = '(') or (s[vp] = ')') or (s[vp] = '[') or (s[vp] = ']') or (s[vp] = '.') or (s[vp] = ',') or (s[vp] = '=') or (s[vp] = '+') or (s[vp] = '-') or (s[vp] = '*') or (s[vp] = '/') or (s[vp] = '^') or (s[vp] = '>') or (s[vp] = '<') or (s[vp]= ';') then
          begin
            ae := vp - 1;
            Break;
          end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if TokenSeparator(s[fromX]) {= ' ')} {or (s[fromX] = InternalStyles.FHintParameter.HintClassDelimiter)} then break;
        res := s[fromX] + res;
        dec(fromX);
      end;
    end;
...
procedure TAdvCustomMemo.CalcBlockLines;
var
  lc,i,cl,ci,ct,ee: integer;
  wstart,wend,s: string;
  b: TBlockDef;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (varpos(wstart, Uppercase(s), ee) > 0) then
      begin
        lc := InternalStyles.PosCommentLeft(s,cl,ci,ct);
        if lc = 0 then
          lc := InternalStyles.PosLineComment(s,cl,ci,ct);
        if (lc <> 0) and (lc < ee) then
          ee := -1
        else
        begin
          b := TBLockDef.Create;
      ...
      if (varpos(wend, Uppercase(s), ee) > 0) then
      begin
        lc := InternalStyles.PosCommentLeft(s,cl,ci,ct);
        if lc = 0 then
          lc := InternalStyles.PosLineComment(s,cl,ci,ct);
...
procedure TAdvCustomMemo.PaintBlockLines(ACanvas: TCanvas);
var
  i,j,ofs,cofs,pofs: integer;
  b,c: TBlockDef;
  pt1,pt2: TPoint;
  clr: TColor;
  toline,nw: integer;
begin
  ...
  toline := TopLine + VisibleLineCount;
  if CodeFolding.Enabled then
    nw := NodeWidth
  else
    nw := 0;

  for i := 0 to FBlockLines.Count - 1 do
  begin
    b := FBlockLines.Items[i];
    if b.IsBegin and (b.Line < toline) then
    begin
      ofs := 0;
      cofs := 0;

      for j := i + 1 to FBlockLines.Count - 1 do
      begin
        c := FBlockLines.Items[j];

        if c.IsBegin then inc(cofs);
        if not c.IsBegin and (ofs = cofs) then
        begin
          // end found

          pofs := ofs;
          if (pofs >= FBlockLineColors.Count) and (FBlockLineColors.Count > 0) then
            pofs := pofs mod FBlockLineColors.Count;

          if FBlockLineColors.Count > 0 then
            clr := FBlockLineColors[pofs]
          else
            clr := clRed;
          ...
          pt1 := Point(Gutter.Width + nw + b.Indent * CharWidth - (CharWidth div 2) - 2, (b.Line - TopLine) * CharHeight);
          pt2 := Point(Gutter.Width + nw + b.Indent * CharWidth - (CharWidth div 2) - 2, (c.Line - TopLine + 1) * CharHeight);

          if pt2.Y > Height then
            pt2.Y := Height;

          ACanvas.MoveTo(pt1.X + 4, pt1.Y);
          ACanvas.LineTo(pt1.X, pt1.Y);
          ACanvas.LineTo(pt2.X, pt2.Y);
          ACanvas.LineTo(pt2.X + 4, pt2.Y);

         // outputdebugstring(pchar('draw:'+b.Line.ToString+'-'+c.Line.ToString));
          break;
        end
        else
        if not c.IsBegin then inc(ofs);
      end;
    end;
  end;
end;
...
var
  wac, s, wstart, wend, wstartu, wendu: string;
  i, b, vp1, vp2: Integer;
  flgs, flge, foundblock: Boolean;
  es, ee: integer;
  cl,ci,ct,lc,sli,lastend: integer;
  sle,sls: TStringList;
  ...
  begin
    ...
    begin
      ...
      begin
        Result := varpos1(sl[i], s, vp);
        if Result > 0 then
        begin
          su := sl[i]; //return the matched key
          break;
        end;
      end;
...
begin
  ...
  try
    ...
    for sli := 0 to sls.Count - 1 do
    begin
      wstartu := sls[sli];

      if sle.Count > sli then
        wendu := sle[sli]
      else
        wendu := sle[0];
      ...
      begin
        ...
        begin
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  if (vp1 > 0) and ((vp1 < vp2) or (vp2 = 0)) and ((lc = 0) or (lc > vp1)) then
                    inc(b);

                  if (vp2 > 0) and ((vp2 < vp1) or (vp1 = 0)) and ((lc = 0) or (lc > vp2)) then
                    dec(b);

                  if (vp2 > 0) and (b = ee) and (i < lastend) then
                  begin
                    lc := InternalStyles.PosCommentLeft(s,cl,ci,ct);
                    if lc = 0 then
                      lc := AnsiPos(InternalStyles.FLineComment, s);

                    if (lc = 0) or (lc > vp2 + Length(wend)) then
                    begin
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    if (lc = 0) or (lc > vp2 + length(wstart)) then
                    begin
          ...
          if not foundblock and (not flgs or not flge) then
          begin
            SetBlockMatchStart(-1, -1, -1);
            SetBlockMatchEnd(-1, -1, -1);
          end;

          if flgs and flge then
            foundblock := true;
        end;
      end;
    end;
...
procedure TAdvCustomMemo.ClearErrorsForWord(const AWord: string);
var
  i,j:Integer;
  Tlp:TLineProp;
  ErrStart, ErrEnd:Integer;
...
begin
  ...
  begin
    ...
    if Assigned(Tlp) and Tlp.HasErrorInfo then
    begin
      LLineText := InternalLines.Strings[i-1];

      for j := Tlp.FErrStart.Count downto 1 do
      begin
        ErrStart := Tlp.FErrStart.Items[j - 1] - LeftCol;
        ErrEnd := Tlp.FErrStart.Items[j - 1] + Tlp.FErrLen.Items[j - 1] - LeftCol;
        ...
        if ErrStart <> ErrEnd then
        begin
          if SameText(Copy(LLineText, ErrStart+1, ErrEnd - ErrStart), AWord) then
          begin
            Tlp.FErrStart.Delete(j-1);
            Tlp.FErrLen.Delete(j-1);
          end;
        end;
      end;
    end;
...
begin
  ...
  begin
    r := LineRect(LineIndexToVisIndex(BSOldSelLine));
    r.Bottom := r.Bottom + 2;
    InvalidateRect(Handle, @r, True);
  end;
...
procedure TAdvCustomMemo.SetBlockBrackets(const Value: Boolean);
begin
  if FBlockBrackets <> Value then
  begin
    FBlockBrackets := Value;
    CalcBlockLines;
    Invalidate;
  end;
end;
...
begin
  ...
  begin
    r := LineRect(LineIndexToVisIndex(BEOldSelLine));
    r.Bottom := r.Bottom + 2;
...
begin
  ...
  if (FLastHintPos.X <> newpos.X) and (FLastHintPos.Y <> newPos.Y) then
  begin
    BeginUpdate;
    st := GetUpStyle(newpos.Y + 1);
    iscomment := st.isComment <> 0;
    ResetUpdate;
  end;
  ...
  if not iscomment and (newpos.Y < InternalLines.Count) and Assigned(InternalStyles) then
  begin
    s := Trim(InternalLines[newpos.Y]);

    i := MultiPos(InternalStyles.LineComment, s);
    if i > 0 then
      if  i <= newpos.x then
        iscomment := true;
  end;
  ...
  begin
    if Assigned(FOnHintForWord) and not iscomment then
    begin
    ...
    if Assigned(FOnHintForToken) and not isComment then
    begin
...
procedure TAdvCustomMemo.ClearBlockLines;
var
  i: integer;
  b: TBlockDef;
begin
...
function TAdvCustomMemo.GetMemoLines: TAdvMemoStrings;
begin
  if Assigned(MemoSource) then
    Result := MemoSource.FLines
  else
    Result := FLines;
end;
...
// ttVar, ttProp, ttEvent, ttMethod, ttFunc, ttProc);
    ...
    begin
      ...
      Rect.Left := Rect.Left + Round(IdentifierWidth * FDPIScale);
      delete(s, 1, vp);
    end;
...
begin
  ...
  begin
    if (str[i] = ' ') or (str[i] = '+') or (str[i] = '-') or (str[i] = '*') or ((str[i] = '/') and (ls <> ':')) then
    begin
      vp := i;
      break;
    end;
...
procedure TAdvMarkerList.SetMarkerImageList(IL: TCustomImageList);
begin
  FMarkerImageList := IL;
end;
