...
const
  ...
  // 1.0.4.0 : New : WaitForComplete() extended with parameter OnlyCancelTimer
...
type
  ...
  public
    ...
    procedure WaitForComplete(OnlyCancelTimer: Boolean=false);
  published
    property Align;
    property Anchors;
...
procedure TAdvCircularProgress.WaitForComplete(OnlyCancelTimer: Boolean=false);
begin
  try
    if Assigned(FTimer) then 
      FTimer.Enabled := false;

    if OnlyCancelTimer then 
      Exit;
