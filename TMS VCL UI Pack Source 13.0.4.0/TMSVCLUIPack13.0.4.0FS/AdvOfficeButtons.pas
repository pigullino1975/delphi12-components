...
uses
  ...
  PictureContainer, AdvGroupBox, Types, AdvStyleIF, AdvGDIP
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF}
  ;
...
const
  ...
  // 1.8.2.0 : Improved : Office dark style appearance
  // 1.8.3.0 : Improved : CheckBox & RadioButton drawing in Office dark style
  // 1.8.3.1 : Fixed : Issue with brush not being set for checkbox in black styles
  // 1.8.4.0 : Improved : AutoSizing of TAdvOfficeRadioGroup, TAdvOfficeCheckGroup
...
begin
  ...
  if (UIStyle in [tsOffice2019Black, tsOffice2019Gray, tsOffice2016Black, tsOffice2010Black]) then
  begin
    h := Min( Canvas.TextHeight('gh') - 2, ClientRect.Bottom - ClientRect.Top);
    th := ((ClientRect.Bottom - ClientRect.Top) - h) div 2;

    if UIStyle in [tsOffice2019Black, tsOffice2016Black, tsOffice2010Black] then
      Canvas.Brush.Color := $00303132
    else
      Canvas.Brush.Color := $00B2B2B2;

    Canvas.Pen.Color := Font.Color;
    Canvas.Pen.Width := 1;
    Canvas.Pen.Style := psSolid;

    Canvas.Rectangle(ClientRect.Left, ClientRect.Top + th, ClientRect.Left + h, ClientRect.Top + th + h);

    if Checked then
    begin
      gp := TGPGraphics.Create(Canvas.Handle);
      pen := TGPPen.Create(ColorToARGB(Canvas.Font.Color), 1.7);

      gp.SetSmoothingMode(SmoothingModeAntiAlias);

      gp.DrawLine(pen, ClientRect.Left + h div 4, ClientRect.Top + th + h div 2, ClientRect.Left + h div 2, ClientRect.Top + th + h div 2 + h div 4);
      gp.DrawLine(pen, ClientRect.Left + h div 2, ClientRect.Top + th + h div 2 + h div 4, ClientRect.Left + h, ClientRect.Top + th);
...
begin
  ...
  begin
    ...
    begin
      ...
      r.Bottom := Min(ClientRect.Bottom -2, r.Top + YSize);
...
begin
  ...
  Paint;
  Width := FCtrlWidth + 4;
  if Height < FCtrlHeight then
    Height := FCtrlHeight + 4;
end;
...
function TCustomAdvOfficeRadioButton.DrawRadio: integer;
var
  bmp: TBitmap;
  BT, BL: integer;
  i: integer;
  ...
  rf: TGPRectF;
begin
  Result := 13 + Round(16 * FDPIScale) - 16;
  ...
  begin
    ...
    pen := TGPPen.Create(ColorToARGB(Canvas.Font.Color),1);
    fill := TGPSolidBrush.Create(ColorToARGB(Canvas.Font.Color));

    th := Min(Canvas.TextHeight('gh'), ClientRect.Bottom - ClientRect.Top) - 3;

    Result := th;

    t := ClientRect.Top + (ClientRect.Bottom - ClientRect.Top) div 2 - th div 2;

    rf.X := ClientRect.Left;
    rf.Y := t;
    rf.Width := th;
    rf.Height := th;

    gp.DrawEllipse(pen, rf);

    if Checked then
    begin
      rf.X := ClientRect.Left + th div 4;
      rf.Y := t + th div 4;
      rf.Width := th - (th div 4) * 2;
      rf.Height := th - (th div 4) * 2;

      gp.FillEllipse(fill, rf);
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      cr := r;
      r.Right := $FFFF;
      FCtrlHeight := DrawText(Canvas.Handle, PChar(Text), Length(Text), r, DT_CALCRECT) + 6;
...
begin
  FHot := True;
  if not (UIStyle in [tsOffice2016Black, tsOffice2019Black, tsOffice2013Gray, tsOffice2010Black]) then
    DrawRadio;
  inherited;
end;
...
begin
  ...
  begin
    ...
    if (AutoSize in [gasVertical, gasBoth]) then
      ButtonHeight := Canvas.TextHeight('gh') + 8
    else
...
begin
  FTMSStyle := AStyle;
  DarkCheck := (FTMSStyle in [tsOffice2019Black, tsOffice2019Gray, tsOffice2016Black, tsOffice2010Black]);
