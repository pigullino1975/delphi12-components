...
type
  ...
  procedure DrawSpinButtons(Canvas: TCanvas;Handle: THandle; ARect: TRect; AControlStyle: TControlDrawStyle; AState: TControlState; ATones: TColorTones; DPIScale: single);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              if ((ch = 'I') and ((chn = 'M') or (chn = 'N')) or (ch = 'C')) or
                 ((ch = 'B') and (chn = 'R')) or
                 ((ch = 'H') and (chn = 'R')) then
//                 ((ch = 'C') and (chn = 'O')) then
                 begin
...
begin
  Result := IsType(s, DecimalSeparator, ThousandSeparator);
end;
...
begin
  ...
  begin
    ...
    while ((s2^ <> ' ') or odd(q)) and (s2^ <> NULL) and (odd(q) or ((s2^ <> '&') and (s2^ <> '|') and (s2^ <> ')')))  do
    begin
      if (s2^= '"') then
        inc(q);
  ...
  begin
    ...
    while (s2^ <> ' ') and (s2^ <> NULL) and (odd(q) or ((s2^ <> '&') and (s2^ <> '|') and (s2^ <> ')'))) do
    begin
      if s2^ = '"' then
        inc(q);
...
var
  ...
  pt: array of TPoint;
begin
  ARect.Left := ARect.Right - Round(15 * DPIScale);
...
begin
  ...
  EndFragmentIndex := StartFragmentIndex + N;
