...
uses
  ...
  , ImgList, Messages, AdvXPVS, Types, ShellAPI, ActiveX, SysUtils, CommCtrl
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF}
  ;
...
function HexVal(const s:string): Integer;
var
  l,p     : Integer;
  b1,b2   : Integer;

  function CharToDec (const c : char) : integer;
  begin
    if (c>='a') and (c<='f') then
    begin
      Result := ord(c)-87;
...
begin
  ...
  repeat
    ...
    Result := Result + ((b2 shl 4) + b1) shl (8*p);
    inc(p);
  until l <= 0;
end;
...
function IPos(const su,s:string):Integer;
begin
  Result := Pos(AnsiUpperCase(su),AnsiUpperCase(s));
end;

function IStrToInt(const s: string):Integer;
var
  Err,Res: Integer;
begin
  Val(s,Res,Err);
  ...
  begin
    ...
    for i := ofs to length(aval) do
      begin
        ...
        if (dohex and not ((aval[i] >= 'A') and (aval[i] <='F'))) and (dohex and not ((aval[i] >= 'a') and (aval[i] <='f'))) and
           not ((aval[i] >= '0') and (aval[i] <= '9')) then
        begin
          Result := i;
          break;
        end;
...
const
  ...
  HTMLEncodedChar : array[1..83] of ansistring = ('é','è','ë','ê',
                                             'ó','ò','ö','ô',
                                             'í','ì','ï','î',
                                             'ú','ù','ü','û',
                                             'á','à','ä','â',
  ...
  HTMLSpecialChar : array[1..83] of ansistring = ('&eacute;','&egrave;','&euml;','&ecirc;',
                                             ...
                                             '&iexcl;','&ETH;','&THORN;','&deg;',
                                             '&ntilde;','&Ntilde;','&frac14;','&frac12;',
                                             '&frac34;','&acute;','&micro;','&para;',
                                             '&middot','&sup2;','&sup3;');
...
var
  ...
  LastAnchor,OldAnchorVal,LType,LName,HTitle: string;
  IMGSize: TPoint;
  ...
  DrawStyle: DWord;
  ColL,ColB,CCol: TColor;
  CSize: integer;
  ofsx,newofsx,emh: integer;
  floatrect: TRect;
  floatpt: TPoint;
  ...
  function HTMLDrawLine(Canvas: TCanvas; var s: string;r: TRect; Calc: Boolean;
                        ...
                        XPos,YPos: Integer;var Hotspot,ImageHotSpot: Boolean; OffsetX: integer; var NewOffsetX: integer; var floatrect: TRect; var txtfloat: TFloatStyle; var lb: boolean): string;
  var
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if BaseAlign then
              begin
                SetTextAlign(Canvas.Handle, VTA_BASELINE);
                ExtTextOut(Canvas.Handle, dcr.Left, dcr.bottom, 0, dcr, pchar(su), length(su), 0);
              end
              else
              {$ENDIF}
...
//                              IMGSize.Y := MulDiv(IMGSize.Y, GetDeviceCaps(Canvas.Handle, LOGPIXELSY), 96);
          ...
          'C':begin  //<c type="up" color="red" size="1">
                TagProp := Uppercase(Copy(s,2,pos('>',s) - 1));

                CCol := Canvas.Font.Color;
                CSize := Canvas.Font.Size;
                ...
                begin
                  ...
                  if Prop[1] = '#' then
                    ...
                    CCol := Text2Color(AnsiLowerCase(prop));
                end;

                CFontSize := Canvas.Font.Size;
                ...
                CFontName := Canvas.Font.Name;
                CFontColor := Canvas.Font.Color;
                CFontStyle := Canvas.Font.Style;
                Canvas.Font.Color := CCol;
                Canvas.Font.Size := CSize;
                Canvas.Font.Style := [];
                ...
                begin
                  ...
                  begin
                    ...
                    CVal := chr($6C);
                    OffsetRect(ir,-Canvas.Font.Size div 2,-Canvas.Font.Size div 2);
                  end
                  else
                    CVal := '';
                  ...
                  if CVal <> '' then
                  begin
                    DrawTextEx(Canvas.Handle, PChar(CVal), 1, ir, DrawStyle, nil);
                  end;
                  Canvas.Font.Name := CFontName;
                end;

                cr.Left := cr.Left + Canvas.TextWidth('w');

                Canvas.Font.Color := CFontColor;
                Canvas.Font.Size := CFontSize;
                Canvas.Font.Style := CFontStyle;
              end;
...
//                                IMGSize.y := MulDiv((bmp as THTMLPicture).MaxHeight,GetDeviceCaps(Canvas.Handle,LOGPIXELSY),96);
          ...
          'J':begin
                ...
                begin
                  su := copy(s,4,2);
                  {$IFDEF DELPHIXE10_LVL}
                  emh := DrawEmoticon(Canvas, cr.Left, cr.Top, Canvas.Font.Size, su, calc);
                  {$ENDIF}
                  cr.Left := cr.Left + emh;
                end;
              end;
                  ...
                  begin
                    ...
                    begin
                      ...
                      begin
                        if not bmp.Empty then
                        begin
                          Canvas.Draw(cr.Left, cr.Top + 2, bmp);
          ...
          'P':begin
                //if calc and (YPos > cr.Top + Canvas.TextHeight('gh') ) then
                //  Linebreak := true
                //else
                begin
                  if (VarPos('>',s,TagPos)>0) then
...
//  r.Left := r.Left + 1; {required to add offset for DrawText problem with first capital W letter}
  ...
  begin
    ...
    su := HTMLDrawLine(Canvas,s,r,True,HtmlWidth,HtmlHeight,subh,suph,imgali,Align,PIndent,XPos,YPos,HotSpot,ImageHotSpot,ofsx,newofsx, floatrect, txtfloat, dolb);
    ...
    begin
      ...
      HTMLDrawLine(Canvas,su,dr,CheckHotSpot,HtmlWidth,HtmlHeight,subh,suph,ImgAli,Align,PIndent,XPos,YPos,HotSpot,ImageHotspot,ofsx,newofsx, floatrect,txtfloat,dolb);
