...
uses
  ...
  StdCtrls, ExtCtrls, ImgList, ComCtrls,  Buttons, Types,
...
type
  ...
  TParamTreeEditor = class(TForm)
    ...
    procedure ParamTreeview1KeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
  private
  ...
  public
    ...
    procedure InsertSubItem;
    procedure DeleteItem;
  end;

  TParamTreeViewEditor = class(TComponent)
  private
    FParamTreeView: TParamTreeView;
    FCaption: string;
  ...
  published
    ...
    property ParamTreeView: TParamTreeView read FParamTreeView write FParamTreeView;
  end;
...
procedure Tparamtreeeditor.InsertSubItem;
begin
  if Assigned(ParamTreeview1.Selected) then
  begin
...
procedure Tparamtreeeditor.FormShow(Sender: TObject);
begin
  FontName.ItemIndex := FontName.Items.IndexOf(ParamTreeView1.Font.Name);
  FontSize.ItemIndex := FontSize.Items.IndexOf(IntToStr(ParamTreeView1.Font.Size));
end;
...
procedure Tparamtreeeditor.AddNewItem;
begin
  paramtreeview1.Selected := Paramtreeview1.Items.Add(nil,'');
end;
...
procedure Tparamtreeeditor.DeleteItem;
begin
  if Assigned(paramtreeview1.Selected) then
...
procedure Tparamtreeeditor.ParamTreeview1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = VK_ADD then
    AddNewItem;
...
{ TParamTreeViewEditor }
constructor TParamTreeViewEditor.Create(AOwner: TComponent);begin
  inherited;
  BorderIcons := [biSystemMenu, biMinimize, biMaximize];
  BorderStyle := bsSizeable;
end;

function TParamTreeViewEditor.Edit: TModalResult;var
  pe: TParamTreeEditor;
  res: TModalResult;
begin
  if not Assigned(ParamTreeView) then
    raise Exception.Create('No TParamTreeView assigned');

  pe := TParamTreeEditor.Create(Application);
  pe.ParamTreeView1.Items.Assign(ParamTreeView.Items);

  pe.Paramtreeview1.HideSelection := False;
  pe.DBfields.Visible:=false;

  pe.paramlabel1.Font.Assign(ParamTreeView.Font);
  pe.paramtreeview1.Font.Assign(ParamTreeView.Font);
  pe.paramtreeview1.Color := ParamTreeView.Color;
  pe.paramtreeview1.ItemHeight := ParamTreeView.ItemHeight;
  pe.paramtreeview1.HTMLImages := ParamTreeView.HTMLImages;
  pe.ParamTreeview1.ParamColor := ParamTreeView.ParamColor;
  pe.ParamTreeview1.ParamHint := ParamTreeView.ParamHint;
  pe.ParamTreeview1.ShowHint := ParamTreeView.ShowHint;
  pe.ParamTreeview1.HoverColor := ParamTreeView.HoverColor;
  pe.ParamTreeview1.HoverFontColor := ParamTreeView.HoverFontColor;
  pe.ParamTreeview1.Hover := ParamTreeView.Hover;
  pe.ParamTreeview1.ShadowColor := ParamTreeView.ShadowColor;
  pe.ParamTreeview1.ShadowOffset := ParamTreeView.ShadowOffset;
  pe.ParamTreeview1.LineSpacing := ParamTreeView.LineSpacing;
  pe.ParamTreeview1.SelectionColor := ParamTreeView.SelectionColor;
  pe.ParamTreeview1.SelectionFontColor := ParamTreeView.SelectionFontColor;
  pe.ParamTreeview1.ShowSelection := ParamTreeView.ShowSelection;
  ...
  if res = mrOk then
  begin
    ParamTreeView.Items.Assign(pe.ParamTreeView1.Items);
  end;
