...
const
  ...
  // v1.6.1.0 : Improved : UIStyle property for TMSStyle
  //          : Fixed : Tabwidth and status size for high DPI
  // v1.6.1.1 : Fixed : UIStyle set to tsCustom after initial Office look
  //          : Fixed : Tabwidth on high DPI with use of FWidth;
  // v1.6.1.2 : Improved : On creation check for enabeld AdvFormStyler
  // v1.6.2.0 : New : Possible to make no page active (ActivePageIndex to -1)
  // v1.6.2.1 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // v1.6.2.2 : Fixed : Use of AppColor in Office2019 White style with TAdvFormStyler
  // v1.6.2.3 : Fixed : Vertical font caused memory leak
  // v1.6.3.0 : New : Support for HTML formatted text in tab
  //          : New : ImageList image support in tab
...
type
  ...
  private
    ...
    procedure SetDrawTabLine(const Value: Boolean);
  ...
  public
    ...
    function GetThemeID: String;
  published
    property DrawTabLine: Boolean read FDrawTabLine write SetDrawTabLine default False;
...
var
  R2: TRect;
begin
  Result := Round (FOwner.FDPIScale * FWidth);
  if Result = 0 then
  begin
...
begin
  ...
  begin
    ...
    g := TGPGraphics.Create(Canvas.Handle);
    TabAppearance.Status.Appearance.CalculateSize(g, TabAppearance.Status.Caption, FAdvSmoothTabPager.FDPIScale);
...
begin
  ...
  if Assigned(FAdvSmoothTabPager) and (AStyle = tsOffice2019White) then
    FAdvSmoothTabPager.LineColor := AppColor;
end;
...
begin
  ...
  begin
    ...
    case astyle of
      ...
      begin
        ...
        TabAppearance.FAppearance.SimpleLayoutBorder := True;
        TabAppearance.FAppearance.Rounding := 0;
        TabAppearance.ColorTextSelected := $00444648;
        TabAppearance.ColorTextHot := $00444648;
        TabAppearance.Appearance.Font.Color := $00444648;
        Color := clWhite;
        ColorDown := clWhite;
      ...
      begin
        ...
        TabAppearance.FAppearance.SimpleLayoutBorder := True;
        TabAppearance.FAppearance.Rounding := 0;
        TabAppearance.ColorTextSelected := $00232425;
        TabAppearance.Appearance.Font.Color := $00232425;
        TabAppearance.ColorTextHot := $001E1F20;
        Color := $00B8BBBE;
        ColorDown := $00B8BBBE;
        ColorDisabled := $00B8BBBE;
        ColorHot := $00C4C6C8;
      end;
      tsOffice2019Black:
      begin
        ...
        TabAppearance.ColorTextSelected := $FFFFFF;
        TabAppearance.ColorTextHot := $FFFFFF;
        TabAppearance.Appearance.Font.Color := $FFFFFF;
        Color := $00303132;
        ColorDown := $00303132;
        ColorDisabled := $00303132;
        ColorHot := $00444648;
      end;
    end;
  ...
  case AStyle of
    ...
    tsOffice2019Gray:
      begin
        PageAppearance.BorderColor := $00B8BBBE;
    ...
    tsOffice2019Black:
      begin
        PageAppearance.BorderColor := $00303132;
...
procedure TAdvSmoothTabPager.AnimateLine;
begin
...
procedure TAdvSmoothTabPager.OnDrawLineTimerTime(Sender: TObject);
begin
...
begin
  ...
  begin
    ...
    begin
      hs := Round(FDPIScale * HorizontalSpacing);
      vs := Round(FDPIScale * VerticalSpacing);
    end;
    ...
    begin
      ...
      begin
        if FDrawTabLine and not FMouseDown then
          FBold := True;

        if FDrawTabLine and (FHotPageIndex = PageIndex) and not FMouseDown then
        begin
          tc := FLineColor;
          c := ColorHot;
        end
    ...
    begin
      ...
      try
        ...
        if Assigned(FImages) and (AdvSmoothTabPages[PageIndex].ImageIndex >= 0) then
          tw := tw + 4 + FImages.Width;

        if FDrawTabLine then
        begin
          if righttoleft then
            textr := Rect(R.Left + Round(FDPIScale * hs), r.Top + Round(FDPIScale * vs), r.Right - Round(FDPIScale * hs) - Round(FDPIScale * 9), r.Bottom - Round(FDPIScale * vs))
          else
            textr := Rect(R.Left + Round(FDPIScale * hs) + Round(FDPIScale * 9), r.Top + Round(FDPIScale * vs), r.Right - Round(FDPIScale * hs), r.Bottom - Round(FDPIScale * vs));
        end
        else
    ...
    ishtml := pos('</',AdvSmoothTabPages[PageIndex].Caption) > 0;

    if ishtml then
      capstr := ''
    else
      capstr := AdvSmoothTabPages[PageIndex].Caption;

    dr := r;

    FAppearance.ImageList := FImages;
    FAppearance.ImageIndex := AdvSmoothTabPages[PageIndex].ImageIndex;

    if FDrawTabLine then
    begin
      if FBold then
        FAppearance.Font.Style := [fsBold]
      else
        FAppearance.Font.Style := [];

      FAppearance.Draw(g, capstr, R.Left - hs, R.Top - vs, R.Right - R.Left + (hs * 2), R.Bottom - R.Top + (vs * 2) - Round(FDPIScale * 9), hs, vs,
      ...
      begin
        ...
        HTMLDrawGDIP(g, Canvas.Font, AdvSmoothTabPages[PageIndex].Caption, dr,
                    FImages,
                    dr.Left - hs, r.Top - vs ,-1,-1,2,
                    false,true,false,false,false,false,false, FDPIScale,
                    clBlue,clNone,clNone,clGray,
                    anv, sv, fa,
                    xs,ys,hl,ml,
                    hr,nil, nil, 4, FDPIScale);

        OffsetRect(dr,xs div 2,0);

        HTMLDrawGDIP(g, Canvas.Font, AdvSmoothTabPages[PageIndex].Caption, dr,
                    FImages,
                    dr.Left - hs, r.Top - vs ,-1,-1,2,
                    false,false,false,false,false,false,false, FDPIScale,
                    clBlue,clNone,clNone,clGray,
                    anv, sv, fa,
                    xs,ys,hl,ml,
                    hr,nil, nil, 4, FDPIScale);
    ...
    begin
      FAppearance.Draw(g, capstr, R.Left - hs, R.Top - vs, R.Right - R.Left + (hs * 2), R.Bottom - R.Top + (vs * 2), hs, vs,
        c, ColorDown, bc, tc, Shadow, d, Bevel, vertical, righttoleft, roundingtype, Picture, th, tw, true, picidx, picname, TextRenderingHintClearTypeGridFit);
    ...
    with Status do
    begin
      Appearance.CalculateSize(g, Status.Caption, FDPIScale);
    ...
    if TabStop and Ffocused and (ActivePageIndex = PageIndex) and ShowFocus and not FDrawTabLine then
      DrawFocus(g, MakeRect(R.Left - hs, R.Top - vs, R.Right - R.Left + (hs * 2), R.Bottom - R.Top + (vs * 2)), Appearance.Rounding, roundingtype);

    if (ActivePageIndex = PageIndex) and FDrawTabLine then
    begin
      if not FAnimating then
      begin
        if (FHotPageIndex = PageIndex) and (not FMouseDown) then
        begin
          if not vertical then
          begin
            FNewLineStart := R.Left;
            FNewLineEnd := R.Right;
          end
          else
          begin
            FNewLineStart := R.Top;
            FNewLineEnd := R.Bottom;
          end;
        end
        else
        begin
          if not Vertical then
          begin
            FNewLineStart := R.Left + (R.Right - R.Left - tw) div 2;
            FNewLineEnd := R.Right - (R.Right - R.Left - tw) div 2;
          end
          else
          begin
            FNewLineStart := R.Top  + (R.Bottom - R.Top - tw) div 2;
            FNewLineEnd := R.Bottom - (R.Bottom - R.Top - tw) div 2;
          end;
        end;

        if not vertical then
        begin
          FLineBottom := R.Bottom - Round(FDPIScale * 4);
        end
        else
        begin
          if righttoleft then //Left
            FLineBottom := R.Right - Round(FDPIScale * 4)
          else
            FLineBottom := R.Left + Round(FDPIScale * 8);
        end;
        ...
        begin
          if not vertical then
          begin
            LineRect := Rect(FNewLineStart, FLineBottom - Round(FDPIScale * 4), FNewLineEnd, FLineBottom);
          end
          else
          begin

            LineRect := Rect(FLineBottom - Round(FDPIScale * 4), FNewLineStart, FLineBottom, FNewLineEnd);
          end;
      ...
      begin
        if not vertical then
        begin
          LineRect := Rect(Round(FCurLineStart), FLineBottom -  Round(FDPIScale * 4), Round(FCurLineEnd), FLineBottom);
        end
        else
        begin
          LineRect := Rect(FLineBottom - Round(FDPIScale * 4), Round(FCurLineStart), FLineBottom, Round(FCurLineEnd));
        end;
...
procedure TAdvSmoothTabPager.SetUIStyle(const Value: TTMSStyle);
begin
  ...
  case AStyle of
    ...
    tsOffice2007Silver:
     ...
     tsWindows7:
      ...
      begin
        Fill.Color := $00B8BBBE;
        Fill.ColorTo := $00B8BBBE;
        Fill.ColorMirror := clNone;
        Fill.ColorMirrorTo := clNone;
        Fill.BorderColor := $00B8BBBE;
        Fill.GradientMirrorType := gtNone;
        FDrawTabLine := True;
        FLineColor := $001E1F20;
      end;
      ...
      begin
        Fill.Color := $00303132;
        Fill.ColorTo := $00303132;
        Fill.ColorMirror := clNone;
        Fill.ColorMirrorTo := clNone;
        Fill.BorderColor := $00303132;
        Fill.GradientMirrorType := gtNone;
        FDrawTabLine := True;
        FLineColor := $00FFFFFF;
      end;
...
procedure TAdvSmoothTabPager.SetDrawTabLine(const Value: Boolean);
var
  I: Integer;
begin
  FDrawTabLine := Value;

  for I := 0 to AdvSmoothTabPageCount - 1 do
  begin
    AdvSmoothTabPages[i].TabAppearance.FAppearance.Font.Style := [];
  end;
end;
...
procedure TAdvSmoothTabPager.SetImages(const Value: TCustomImageList);
begin
  FImages := Value;
end;
...
var
  ...
  AllowChange: Boolean;
begin
  if (PageIndex < FAdvSmoothTabPages.Count) and (PageIndex <> ActivePageIndex) then
  begin
    AllowChange := True;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          TbW := TbW + Round(FDPIScale * 2);

          if Assigned(FImages) and (AdvSmoothTabPages[i].ImageIndex >= 0) then
            TbW := TbW + FImages.Width;
