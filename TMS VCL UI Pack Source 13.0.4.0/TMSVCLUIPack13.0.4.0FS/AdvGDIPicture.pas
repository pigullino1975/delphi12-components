...
const
  ...
  // v1.4.5.1 : Fixed : Issue with EmptyText background color
...
type
  ...
  TGDIPImageFileType = (gtJPEG,gtBMP,gtPNG,gtGIF,gtEMF,gtAll,gtICO,gtSVG);
...
begin
  ...
  begin
    ...
    h := DrawText(ACanvas.Handle, PChar(EmptyText), Length(EmptyText),r,DT_CENTER or DT_WORDBREAK or DT_CALCRECT);
...
begin
  ...
  if gtSVG in ImageTypes then
  begin
    fltr := AppendStr(fltr,'Scalable Vector Graphics (*.svg)|*.svg','|');
    ext := AppendStr(ext,'*.svg',';');
  end;
