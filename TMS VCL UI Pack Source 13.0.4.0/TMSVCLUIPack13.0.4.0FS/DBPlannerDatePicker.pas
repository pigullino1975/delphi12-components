...
const
  ...
  //         : New : Fixed : Issue with setting DataSource.Enabled = false
  // 1.6.0.1 : Fixed : Design-time style painting
  // 1.6.1.0 : New : UIStyle property for office look
  //         : Improved : On creation check for enabled AdvFormStyler
  // 1.6.2.0 : New : Per monitor support for high DPI
...
begin
  ...
  FPlannerCalendar.Width := Round(FDPIScale * FOrigWidth);
  FPlannerCalendar.Height := Round(FDPIScale * FOrigHeight);
...
procedure TDBPlannerDatePicker.CreateWnd;
begin
  inherited;
  if FDesignTime then
  begin
...
procedure TDBPlannerDatePicker.SetUIStyle(const Value: TTMSStyle);
begin
...
begin
  ...
  ReadOnly := not FDataLink.DataSource.Enabled;

  if not FDataLink.DataSource.Enabled then
  begin
    FInternalChange := true;
    Text := '';
    FInternalChange := false;
