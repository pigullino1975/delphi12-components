...
procedure SetWin11(AWin11: Boolean);
...
\**************************************************************************)
  ...
  public
    ...
    function MakeFontEx(AFont: TFont): TGPFont; overload;
    function MakeFontEx(AFont: TFont; ASize: integer): TGPFont; overload;
  ...
  {$IFDEF DELPHIXE_LVL}
  TAdvGDIPPicture = class(TTMSFNCSVGBitmap)
  {$ELSE}
  ...
  public
    ...
    {$IFDEF DELPHIXE_LVL}
    procedure DrawSVG(Graphics: TGPGraphics; X, Y, W, H: Single); virtual;
    {$ENDIF}
...
{$IFDEF DELPHIXE_LVL}
uses
  AdvUtils, AdvGraphicsSVGEngine, Types;
{$ENDIF}
...
begin
  ...
  Result := ((verinfo.dwMajorVersion = 6) or  (verinfo.dwMajorVersion = 10)) and (verinfo.dwBuildNumber >= 22000);
end;
...
begin
  ...
  else if Assigned(frm) and (frm is TCustomForm) then
  begin
      FDPI := TCustomForm(frm).Monitor.PixelsPerInch;
  end
  else
  {$ENDIF}
  begin
...
function TGPGraphics.MakeFontEx(AFont: TFont): TGPFont;
begin
  Result := MakeFontEx(AFont, AFont.Size);
end;

function TGPGraphics.MakeFontEx(AFont: TFont; ASize: integer): TGPFont;
var
  fontFamily: TGPFontFamily;
  fs: Integer;
  b: Boolean;
...
begin
  ...
  begin
    b := False;
    if FCachedFont.GetSize <> ASize then
      b := True;
    ...
    begin
      ...
      b := FCachedFont.GetStyle <> fs;
    end;

    if not b then
    begin
      fontFamily := TGPFontFamily.Create;
      FCachedFont.GetFamily(fontfamily);
      fontFamily.GetFamilyName(n);
      b := n <> AFont.Name;
      fontFamily.Free;
    end;
  ...
  FCachedFont := TGPFont.Create(fontFamily, ASize , fs, UnitPoint);
  Result := FCachedFont;
...
begin
  ...
  begin
    ...
    begin
      {$IFDEF DELPHIXE_LVL}
      if TAdvGDIPPicture(Source).HasSVG then
...
{$IFDEF DELPHIXE_LVL}
procedure TAdvGDIPPicture.DrawSVG(Graphics: TGPGraphics; X, Y, W, H: Single);
var
  ca: TCanvas;
begin
  if not HasSVG then
  ...
  try
    ...
    TAdvGraphicsSVGImport(SVG).Draw(ca, RectF(X, Y, X + W, Y + H), True);
...
begin
  ...
  if HasSVG then
  begin
    DrawSVG(Graphics, Rect.Left, Rect.Top, Rect.Right - Rect.Left, Rect.Bottom - Rect.Top);
    Exit;
  end;
  {$ENDIF}
...
begin
  ...
  if HasSVG then
  begin
    DrawSVG(Graphics, Rect.X, Rect.Y, Rect.Width, Rect.Height);
    Exit;
  end;
  {$ENDIF}
...
begin
  try
    ...
    {$IFDEF DELPHIXE_LVL}
    s := TAdvUtils.GetImageType(FDatastream);
    if s = '' then
    begin
      FDatastream.Clear;
...
begin
  ...
  begin
    ...
    begin
      {$IFDEF DELPHIXE_LVL}
      s := TAdvUtils.GetImageType(Stream);
      if s = '' then
