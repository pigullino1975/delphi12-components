...
const
  ...
  // 1.4.3.0 : New : Dropdown arrow color can be set with ArrowColor and ArrowColorDisabled
  // 1.4.4.0 : Improved : Exposed Align property for TAdvColorSelector
  // 1.4.5.0 : New : function SelectedPenWidth() added for TAdvPenWidthSelector
  //         : Improved : High DPI handling for multimonitor
...
type
  ...
  protected
    ...
    property ArrowColorDisabled: TColor read FArrowColorDisabled write FArrowColorDisabled default clGray;
  ...
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    function SelectedPenWidth(DPI: integer = 96): single;
    property AppearanceStyle;
  published
    property AutoThemeAdapt;
    property BorderColor;
      ...
      begin
        ...
        begin
          ...
          if Enabled then
            ...
            DrawArrow(AP, ArrowColorDisabled);
        end;
      end;
    end
...
begin
  ...
  // calculate size AFTER visible to get property DPI
  if Assigned(FSelectorPanel) then
  ...
  w := Round(FDropDownWindow.Width);
  h := Round(FDropDownWindow.Height);
  ...
  if Assigned(FSelectorPanel) then
   ...
   FDropDownWindow.ClientWidth := w;
  if Assigned(FSelectorPanel) then
    FSelectorPanel.Height := h;
  FDropDownWindow.ClientHeight := h;
...
function TAdvPenWidthSelector.SelectedPenWidth(DPI: integer = 96): single;
begin
  Result := 0;
