...
uses Windows, Classes, StdCtrls, ExtCtrls, Controls, Messages, SysUtils,
  Forms, Graphics, Buttons, Grids, DB, Dialogs, Math, ImgList, AdvStyleIF
  , Variants, ALXPVS, Types, AdvToolTip
  {$IFDEF DELPHIXE2_LVL}
...
const
  ...
  // 1.9.6.1 : Fixed : Issue with font handling for runtime created column with high DPI
  // 1.9.6.2 : Fixed : Issue in state with DataSource.Enabled = false
  // 1.9.6.3 : Fixed : Regression with handling DataSource.Enabled for case when no DataSource is used
  // 1.9.6.4 : Fixed : Issue with label color when setting Enabled = false at design-time
  // 1.9.6.5 : Improved : Precision with high DPI calculation
  // 1.9.6.6 : Fixed : Issue with setting control disabled and changing dataset enabled state
  // 1.9.7.0 : New : BorderColor property
  // 1.9.7.1 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // 1.9.7.2 : Fixed : Issue with EditorEnabled = false
  // 1.9.7.3 : Fixed : Issue with activating dataset when control window handle is not created
  // 1.9.7.4 : Fixed : Issue with dataset activation
  //         : Fixed : Issue with paste from clipboard
  // 1.9.7.5 : Fixed : Issue Enabling when no DataSource is assigned
  // 1.9.7.6 : Fixed : Issue with dropdown initialization when DataSource is not assigned
  // 1.9.7.7 : Fixed : Issue with dataset update from clipboard paste
  // 1.9.7.8 : Fixed : Issue with setting .Text property in code during form create
  // 1.9.8.0 : New : TAdvFormStyler compatibility added and UIStyle property exposed
  // 2.0.0.0 : New : ShowValidation() method added
  // 2.0.0.1 : Fixed : Issue with setting Enabled at runtime
  // 2.0.0.2 : Improved : Flexibility to open dropdown on controls with different owner than parent
  // 2.0.0.3 : Fixed : DPI calculation when parent form is not Scaled
  // 2.0.1.0 : New : Image columns use alignment property for rendering image
  // 2.0.2.0 : New : Setting SelectionColor, SelectionTextColor = clNone allows to always use specified custom color
  // 2.0.2.1 : Improved : LabelFont handling in connection with ParentFont
  // 2.0.2.2 : Fixed : Show empty text when dataset is not active
  // 2.0.2.3 : Fixed : Issue with LabelFont handling in Delphi 11 with high DPI
  // 2.0.2.5 : Fixed : Issue with use on TAdvControlDropDown and use of Enter key
  // 2.0.2.6 : Fixed : Issue with load from list source on first dropdown
  // 2.0.2.7 : Fixed : Issue with multiple instances using the same ListSource on the form
  // 2.0.2.8 : Fixed : Issue with ListSource lookup on first keypress
  // 2.0.2.9 : Fixed : Issue with ListSource refresh when ListSource dataset changes
  // 2.0.2.10: Fixed : Update Text property from ListSource change adapted
  // 2.0.2.11: Fixed : OnKeyPress event not triggered for return key
  // 2.0.2.12: Fixed : Issue with pasting text when DataSource is not used
  // 2.0.2.13: Fixed : Showing selected item in dropdown after opening dropdown
  // 2.0.2.14: Fixed : Issue with HoverColor set related to underlying VCL bug
  // 2.0.2.15: Fixed : Issue after paste in TAdvDBLookupComboBox with only ListSource
...
type
  ...
  {$ENDIF}
  TAdvDBLookupComboBox = class(TCustomEdit, ITMSStyle)
  private
    FTMSStyle: TTMSStyle;
  ...
  protected
    ...
    {$ENDIF}
    property DataSetEnabled: boolean read FDataSetEnabled write SetDatasetEnabled;
  public
   {$IFDEF TMSDEBUG}
    procedure DebugTest;
   {$ENDIF}
  ...
  published
    ...
    property LookupMethod: TLookupMethod read FLookupMethod write FLookupMethod default lmNormal;
...
begin
  ...
  begin
    if  not Enabled then
      l := LStyle.GetElementDetails(tcDropDownButtonDisabled)
    else
    begin
      if Down then
        l := LStyle.GetElementDetails(tcDropDownButtonPressed)
      else
       if Hover then
         l := LStyle.GetElementDetails(tcDropDownButtonHot)
       else
         l := LStyle.GetElementDetails(tcDropDownButtonNormal);
    end;
...
begin
  ...
  FDesignCreate := (csDesigning in ComponentState) and not
...
begin
  ...
  DPIScale := CalculateDPIScale(FLabel);
...
begin
  OldValue := FControlEnabled and FDataSetEnabled;

  FControlEnabled := Value;

  NewValue := FControlEnabled and FDataSetEnabled;
  ...
  begin
    ...
    if Assigned(FLabel) then
      if not FLabelAlwaysEnabled then
        FLabel.Enabled := FControlEnabled and FDataSetEnabled;
  end;
end;
...
begin
  ...
  if AlwaysRefreshDropDownList or FRefreshForce then
  begin
    FRefreshForce := false;
    LoadFromListSource;
    if FAllFields.Count = 0 then
...
procedure TAdvDBLookupComboBox.ShowValidation(const AText: string);
begin
...
begin
  ...
  begin
    for i := 0 to FStringGrid.ColCount -1 do
    begin
      Columns[i].Width := Round(FStringGrid.ColWidths[i] / CalculateDPIScale(Self));
    end;
  end;
...
procedure TAdvDBLookupComboBox.HideValidation;
begin
...
begin
  ...
  Loc.Right := ClientWidth - Round(CalculateDPIScale(FButton) * FButton.Width) - 3;
...
begin
  ...
  DPIScale := CalculateDPIScale(FButton);
...
procedure TAdvDBLookupComboBox.WMPaint(var Message: TWMPaint);
begin
  inherited;
  DrawBorders;
end;
...
begin
  ...
  if Assigned(ListSource) and (FAllfields.Count = 0) and (msg.CharCode in [Ord('A')..Ord('Z'), Ord('1')..Ord('9')]) and not (GetKeyState(VK_CONTROL) and $8000 = $8000) then
  begin
    s := Text;
    LoadFromListSource;
    if not Assigned(DataSource) then
...
procedure TAdvDBLookupComboBox.CMFontChanged(var Message: TMessage);
begin
  if ((csDesigning in ComponentState) or (csLoading in ComponentState)) and FLabelFontEqual then
  begin
    FLabelFont.Assign(Font);
    if Assigned(FLabel) then
...
begin
  ...
  begin
    ...
    txt := AnsiUpperCase(FCurrentSearch);
    fndmatch := -1;
...
procedure TAdvDBLookupComboBox.SetUIStyle(const Value: TTMSStyle);
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      FStringGrid.Canvas.Font.Height := Round(DPIScale * FColumns.Items[ACol].TitleFont.Height);
    end
  else
  begin
    FStringGrid.Canvas.Font.Assign(Self.Font);
      // do not scale, form font is auto scaled
  ...
  begin
    ...
    taLeftJustify: FImages.Draw(FStringGrid.Canvas, rect.Left, rect.Top, i);
    taCenter: FImages.Draw(FStringGrid.Canvas, rect.Left + Max(0, (rect.Right - rect.Left - FImages.Width) div 2), rect.Top, i);
    taRightJustify: FImages.Draw(FStringGrid.Canvas, rect.Right - FImages.Width, rect.Top, i);
    end;
  end
  else
  begin
...
procedure TAdvDBLookupComboBox.SetKeyField(const Value: string);
begin
...
begin
  ...
  begin
    FAccept := True;
    HideGridList;
    if not ReturnIsTab and Visible then
      SetFocus;
    Exit;
  end;
...
procedure TAdvDBLookupComboBox.SetDatasetEnabled(const Value: boolean);
begin
  FDataSetEnabled := Value;
  SetEnabledEx(FControlEnabled);
end;
...
begin
  if not EditorEnabled and not (Key in [VK_LEFT,VK_RIGHT,VK_HOME,VK_END,VK_UP,VK_DOWN,VK_PRIOR,VK_NEXT,VK_CONTROL]) then
    Key := 0;
...
begin
  ...
  if (Key = #$16) and (GetKeyState(VK_CONTROL) and $8000 = $8000) then
  begin
    inherited;
    Exit;
  end;
...
procedure TAdvDBLookupComboBox.SetBorderColor(const Value: TColor);
begin
...
procedure TAdvDBLookupComboBox.SetComponentStyle(AStyle: TTMSStyle);
begin
  FTMSStyle := AStyle;
...
procedure TAdvDBLookupComboBox.DrawBorders;
var
  DC: HDC;
begin
...
procedure TAdvDBLookupComboBox.DrawControlBorder(DC: HDC);
var
  ARect:TRect;
  BtnFaceBrush: HBRUSH;
begin
  if (FBorderColor <> clNone) and not FHasFocus then
  begin
    BtnFaceBrush := CreateSolidBrush(ColorToRGB(clRed));
...
begin
  ...
  FParent := TWinControl(self.Owner);
  Params.WndParent := FParent.Handle;
...
begin
  ...
  if Assigned(FGrid) and Assigned(FGrid.DataSource) and Assigned(DataSet) then
  begin
    with FGrid do
    begin
      ...
      FDataSetEnabled := DataSet.Active and DataSource.Enabled;
      SetEnabledEx(FControlEnabled);
  ...
  begin
    FGrid.UpdateText(FGrid.GridToString);
    FGrid.SetEnabledEx(true);
    FGrid.FButton.Enabled := true;
  end;
end;
...
begin
  ...
  if Assigned(FGrid) and Assigned(DataSet) and Assigned(FGrid.DataSource) then
  begin
...
begin
  ...
  begin
    ...
    begin
      FGrid.FRefreshForce := true;
    end;

    if Assigned(DataSource) then
      FGrid.DataSetEnabled := not ReadOnly and DataSource.Enabled and CheckDataset
    else
      FGrid.DataSetEnabled := not ReadOnly;

    if not HandleAllocated then
      Exit;

    if Assigned(DataSource) and not DataSource.Enabled then
      SetInternalText('');
