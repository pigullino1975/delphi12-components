...
type
  ...
  private
    ...
    procedure SetGridEndRow(const Value: integer);
    procedure SetXlsStartCol(const Value: integer);
    procedure SetXlsStartRow(const Value: integer);
    procedure SetXlsEndCol(const Value: integer);
    procedure SetXlsEndRow(const Value: integer);
    ...
    function CurrentGridRowCount: integer;
    function CurrentGridColCount: integer;
  protected
  ...
  published
   { Published declarations }
    ...
    property GridEndRow: integer read FGridEndRow write SetGridEndRow default -1;
    ...
    property XlsEndRow: integer read FXlsEndRow write SetXlsEndRow default -1;
    property XlsEndCol: integer read FXlsEndCol write SetXlsEndCol default -1;
...
begin
  ...
  try
    ...
    if FAutoResizeGrid then
    begin
      if (Workbook.MaxRow - XlsStartRow + 1 + GridStartRow + EXR > CurrentGrid.RowCount) and (XlsEndRow = -1) then
        CurrentGrid.RowCount := Workbook.MaxRow - XlsStartRow + 1 + GridStartRow + EXR;

      if (Workbook.MaxCol - XlsStartCol + 1 + GridStartCol + EXC > CurrentGrid.ColCount) and (XlsEndCol = -1) then
    ...
    if (XlsEndRow > XlsStartRow) then
    begin
      MaxR := XlsEndRow;
      if FAutoResizeGrid then
        if (MaxR - XlsStartRow + 1 + GridStartRow + EXR > CurrentGrid.RowCount) then
          CurrentGrid.RowCount := MaxR - XlsStartRow + 1 + GridStartRow + EXR;
    end;

    if (XlsEndCol > XlsStartCol) then
    begin
      MaxC := XlsEndCol;
      if FAutoResizeGrid then
        if (MaxC - XlsStartCol + 1 + GridStartCol + EXC > CurrentGrid.ColCount) then
          CurrentGrid.ColCount := MaxC - XlsStartCol + 1 + GridStartCol + EXC;
    end;
...
begin
  ...
  CurrentGrid.ColCount := CurrentGridColCount + HiddenColCount;
  try
    GridRowCount := CurrentGridRowCount + HiddenRowCount;
    if Options.FExportHiddenRows then
      CurrentGrid.RowCount := CurrentGridRowCount + HiddenRowCount;
    try
      ...
      //Export data
      for rg := GridStartRow to CurrentGridRowCount - 1 do
      begin
        ...
        begin
          ...
          case InsertInSheet of
             ...
             begin
               ...
               GridRowCount := CurrentGridRowCount;
             ...
             begin
               ...
               if cols > 0 then Workbook.InsertAndCopyCols(Max_Columns + 1, Max_Columns + 1, XlsStartCol + dc, cols ,true);
             end;
          end; //case.
        end else
        begin
...
function TAdvGridExcelIO.CurrentGridRowCount;
begin
  if (GridEndRow > GridStartRow) and (GridEndRow <= CurrentGrid.RowCount) then
   Result := GridEndRow + 1
 else
   Result := CurrentGrid.RowCount;
end;

function TAdvGridExcelIO.CurrentGridColCount;
begin
  if (GridEndCol > GridStartCol) and (GridEndCol <= CurrentGrid.ColCount) then
   Result := GridEndCol + 1
 else
   Result := CurrentGrid.ColCount;
end;
...
procedure TAdvGridExcelIO.SetGridEndCol(const Value: integer);
begin
  if (Value > -1) and (Value > FGridStartCol) then FGridEndCol := Value else FGridEndCol := -1;
end;

procedure TAdvGridExcelIO.SetGridEndRow(const Value: integer);
begin
  if (Value > -1) and (Value > FGridStartRow) then FGridEndRow := Value else FGridEndRow := -1;
end;
...
procedure TAdvGridExcelIO.SetXlsEndCol(const Value: integer);
begin
  if (Value >= -1) and (Value > FXlsStartCol) then FXlsEndCol := Value else FXlsEndCol := -1;
end;

procedure TAdvGridExcelIO.SetXlsEndRow(const Value: integer);
begin
  if (Value >= -1) and (Value > FXlsStartRow) then FXlsEndRow := Value else FXlsEndRow := -1;
end;
