...
const
  ...
  // 1.0.2.0 : Fixed : Issues with per monitor high DPI for non native dialog
...
type
  ...
  private
    ...
    procedure CreateTaskButtons(TaskDialog: TCustomAdvMetroTaskDialog);
    procedure UpdateTaskButtons(TaskDialog: TCustomAdvMetroTaskDialog);
  protected
...
//  TAdvGlowButton(Result).Font.Height := -13;
end;
...
begin
  ...
  TAdvMessageForm(Result).Appearance.CaptionFont.Height := -13;
end;
...
var
  DialogUnits: TPoint;
  ButtonGroupWidth, IconTextWidth, {IconTextHeight,} X, Y, ALeft: Integer;
  IconID: string;
  TextRect: TRect;
  Msg: string;
...
begin
  ...
  Y := FVertMargin + Round(FDPIScale * METROCAPTIONHEIGHT);

  CreateTaskButtons(TaskDialog);
  UpdateTaskButtons(TaskDialog);

  // Instruction
  Canvas.Font.Height := Round(-15 * FDPIScale);
...
//    if IconTextHeight < Round(32 * FDPIScale) then
//      IconTextHeight := Round(32 * FDPIScale);
  end;

  if ((IconID <> '') or not (TaskDialog.CustomIcon.Empty)) then
  begin
    ...
    begin
      ...
      SetBounds(FHorzMargin, Y, Round(48 * FDPIScale), Round(48 * FDPIScale));
    end;
  end;
  ...
  begin
    ...
    WordWrap := True;
    Caption := Msg;
    Font.Height := Round(-15 * FDPIScale);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Parent := Self;
        Visible := False;
        SetBounds(FHorzMargin, Y, Round(16 * FDPIScale), Round(16 * FDPIScale));
      end;
    end;
  end;
  ...
  itComboEdit, itComboList:
    begin
      ...
      FInputCombo.SetBounds(ALeft, Y + Round(20 * FDPIScale), ClientWidth - ALeft, Round(20 * FDPIScale));
    end;
  itDate:
    begin
  ...
  itMemo:
    begin
      ...
      if UseRightToLeftAlignment then
        ALeft := Self.ClientWidth - ALeft - Width;
      FInputMemo.SetBounds(ALeft, Y + Round(20 * FDPIScale), ClientWidth - ALeft, Round(60 * FDPIScale));
    end;
  itCustom:
    begin
      ...
      begin
        ...
        if TaskDialog.InputControl.Width + ALeft > self.Width then
          FCalcWidth := TaskDialog.InputControl.Width + ALeft + ALeft;
      end;
    end;
  end;
  ...
  FCalcWidth := Max(FCalcWidth, Round(FDPIScale * FMinFormWidth));
...
procedure TAdvMessageForm.CreateTaskButtons(
  TaskDialog: TCustomAdvMetroTaskDialog);
var
  ...
  begin
    ...
    begin
      ...
      7: if (cbNo in Buttons) then
           DefaultButton := cbNo;
      IDCLOSE: if (cbClose in Buttons) then
...
procedure TAdvMessageForm.UpdateTaskButtons(
  TaskDialog: TCustomAdvMetroTaskDialog);
var
  DialogUnits: TPoint;
...
begin
  ...
  DialogUnits := GetAveCharSize(Canvas);
  ButtonWidth := Round(FDPIScale * MulDiv(mcButtonWidth, DialogUnits.X, 4));
  ButtonHeight := Round(FDPIScale * MulDiv(mcButtonHeight, DialogUnits.Y, 8));
  ButtonSpacing := Round(FDPIScale * MulDiv(mcButtonSpacing, DialogUnits.X, 4));

  Canvas.Font.Height := Round(-13 * FDPIScale) - 2;
  ...
  begin
    ...
    TAdvGlowButton(FcmBtnList.Items[i]).Font.Height := Round(-13 * FDPIScale);
    TextRect := Rect(0,0,0,0);
    Windows.DrawText( Canvas.Handle,
          PChar(TAdvGlowButton(FcmBtnList.Items[i]).Caption), -1,
          TextRect, DT_CALCRECT or DT_LEFT or DT_SINGLELINE or
          DrawTextBiDiModeFlagsReadingOnly);
    bw := TextRect.Right - TextRect.Left + Round(FDPIScale * 17);
    if bw > ButtonWidth then
      ButtonWidth := bw;

    TAdvGlowButton(FcmBtnList.Items[i]).Width := Max(Round(60 * FDPIScale), bw);
    TAdvGlowButton(FcmBtnList.Items[i]).Height := ButtonHeight;
    FcmBtnGroupWidth := FcmBtnGroupWidth + Max(Round(60 * FDPIScale), bw) + ButtonSpacing;
  end;
  ...
  begin
    ...
    begin
      TAdvGlowButton(FcsBtnList.Items[i]).SetColorTones(TaskDialog.FColorTones);
      TAdvGlowButton(FcsBtnList.Items[i]).Font.Height := Round(-13 * FDPIScale);

      TextRect := Rect(0,0,0,0);
      Windows.DrawText( Canvas.Handle,
          PChar(TAdvGlowButton(FcsBtnList.Items[i]).Caption), -1,
          TextRect, DT_CALCRECT or DT_LEFT or DT_SINGLELINE or
          DrawTextBiDiModeFlagsReadingOnly);
      bw := TextRect.Right - TextRect.Left + Round(FDPIScale * 17);
      ...
      TAdvGlowButton(FcsBtnList.Items[i]).Width := Max(Round(FDPIScale * 60), bw);
      TAdvGlowButton(FcsBtnList.Items[i]).Height := ButtonHeight;
      FCsBtnGroupWidth := FCsBtnGroupWidth + Max(Round(FDPIScale * 60), bw) + ButtonSpacing;
    end;
  end
  else
  begin
    ...
    begin
      ...
      bw := 0;
      bh := 0;
      GetMultiLineTextSize(Canvas, TButton(FcsBtnList.Items[i]).Caption, Hf, Pf, DrawTextBiDiModeFlagsReadingOnly, bw, bh);
      ...
      if not (doCommandLinksNoIcon in TaskDialog.Options) then
        FCalcWidth := Max(FCalcWidth, n + FHorzMargin * 2 + FHorzSpacing + Round(32 * FDPIScale))
      else
        FCalcWidth := Max(FCalcWidth, n + FHorzMargin);

      TTaskDialogButton(FcsBtnList.Items[i]).Font.Assign(pf);
      TTaskDialogButton(FcsBtnList.Items[i]).Font.Color := TaskDialog.FColorTones.Selected.BrushColor;
      TTaskDialogButton(FcsBtnList.Items[i]).HeadingFont.Assign(hf);
      TTaskDialogButton(FcsBtnList.Items[i]).HeadingFont.Color := TaskDialog.FColorTones.Selected.BrushColor;
      TTaskDialogButton(FcsBtnList.Items[i]).BorderColorDown := TaskDialog.FColorTones.Selected.BrushColor;
      TTaskDialogButton(FcsBtnList.Items[i]).BorderColorHot := TaskDialog.FColorTones.Selected.BrushColor;
      ...
      TTaskDialogButton(FcsBtnList.Items[i]).BackColor := TaskDialog.FColorTones.Background.BrushColor;
    end;
    hf.Free;
    pf.Free;
  end;
...
begin
  ...
  DialogUnits := GetAveCharSize(Canvas);
  HorzMargin := Round(FDPIScale * MulDiv(mcHorzMargin, DialogUnits.X, 4));
  VertMargin := Round(FDPIScale * MulDiv(mcVertMargin, DialogUnits.Y, 8));
  VertSpacing := Round(FDPIScale * MulDiv(mcVertSpacing, DialogUnits.Y, 8));
  ButtonSpacing := Round(FDPIScale * MulDiv(mcButtonSpacing, DialogUnits.X, 4));
  ...
  begin
    ...
    Y1 := 0;
    r := GetContentRect;
    r := Rect(r.Left, Y, R.Right, Y + Round(26 * FDPIScale));
  ...
  begin
    ...
    begin
      if Assigned(FIcon) then
        TTaskDialogButton(FcsBtnList.Items[i]).Left := FHorzParaMargin;
  ...
  for i := 0 to FcmBtnList.Count-1 do
  begin
    FCmBtnGroupWidth := FCmBtnGroupWidth + TButton(FcmBtnList.Items[i]).Width;
  end;
