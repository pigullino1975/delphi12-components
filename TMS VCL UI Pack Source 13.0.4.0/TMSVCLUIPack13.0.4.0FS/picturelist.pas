...
uses
  ...
  StdCtrls, AdvStyleIF, GDIPicture
  {$IFDEF FREEWARE}
  , TMSTrial
  {$ENDIF}
  ;
...
type
  ...
  TThumbnailSizing = (tsShrink, tsStretch, tsBoth, tsNone);
  ...
  private
    ...
    FInSizing: Boolean;
    FThumbnailSizing: TThumbnailSizing;
  ...
  published
    ...
    property ThumbnailSizing: TThumbnailSizing read FThumbnailSizing write FThumbnailSizing default tsShrink;
    property Visible;
    property OnClick;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (rx > 1) or (ry > 1) and (ThumbnailSizing in [tsShrink, tsBoth]) then
        begin
          if rx > ry then
        ...
        begin
          if  (rx < 1) or (ry < 1) and (ThumbnailSizing in [tsStretch, tsBoth]) then
          begin
            if rx > ry then
              DrawRect := Rect(0,0,Width,Trunc(Pic.Height/rx))
            else
        ...
        DrawRect.Right := Round(DrawRect.Right * LScale);
        DrawRect.Bottom := Round(DrawRect.Bottom * LScale);

        {Center the drawing rectangle}
        OffsetRect(DrawRect,ARect.Left + (Width - DrawRect.Right + DrawRect.Left) div 2,
                            ARect.Top + (Height - DrawRect.Bottom + DrawRect.Top) div 2);
...
var
  ...
  begin
    ...
    if (Ext = '.JPG') or (Ext = '.JPEG') or (Ext = '.GIF') or
       (Ext = '.BMP') or (Ext = '.ICO') or (Ext = '.WMF') or (Ext = '.SVG') then
