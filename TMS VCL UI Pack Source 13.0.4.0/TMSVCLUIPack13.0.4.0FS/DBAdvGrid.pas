...
const
  ...
  // 2.4.6.4  : Improved : VCL Styles issue with predefined column settings
  // 2.4.6.5  : Fixed : Issue with font handling in high DPI when moving columns
  // 2.4.6.6  : Fixed : Rare issue with using datasource before grid handle is created
  // 2.5.0.0  : New : StyleElements property added in TGridColumnItem
  // 2.5.0.1  : Fixed : Issue with high DPI and runtime font changes
  // 2.5.0.2  : Fixed : Regression with AutoCreateColumns and high DPI font handling
  // 2.5.0.3  : Fixed : Issue with Column[].Font handling
  // 2.5.0.4  : Improved : All fixed column cells can now be used to set the active DB record upon click
  // 2.5.0.5  : Improved : Use of Columns[x].ButtonWidth/Columns[x].ButtonCaption
  // 2.5.0.6  : Improved : High DPI handling for column fonts
  //          : Improved : Runtime VCL style switching handling
  // 2.5.0.7  : Improved : Changed Column[x].ButtonCaption setter to allow dynamic button caption config
  // 2.5.0.8  : Fixed : Issue with group header / summary color when using merged header/summary
  // 2.5.1.0  : New : AllowBlank added to TDBGridColumnItem
  // 2.5.1.1  : Fixed : Issue with scrolling in grid when grid.UseRecNo = true
  // 2.5.1.2  : Fixed : Display issue with cancel after edit
  // 2.5.1.3  : Fixed : Issue with using lookup combobox of non-numeric type
  // 2.5.1.4  : Fixed : Issue with EditPostMode = epRow and Navigation.AdvanceOnEnter = true
  // 2.5.1.5  : Fixed : Regression with Navigation.AdvanceOnEnter = true
  // 2.5.1.6  : Fixed : Issue with 10.4.2 breaking change in runtime font size handling
  // 2.5.1.7  : Fixed : Issue with 10.4.2 high DPI breaking change with frame handling
  // 2.5.1.8  : Fixed : Issue with ShowGroupCount for grouping
  // 2.5.1.9  : Fixed : Issue with OnGetDisplText() when grid.PageMode = false
  // 2.5.1.10 : Fixed : High DPI font handling regression in Delphi versions 10.x
  // 2.5.1.11 : Fixed : Issue with cell objects and font size handling in high DPI
  // 2.5.1.12 : Fixed : Default font initialization for header fonts
  // 2.5.1.13 : Fixed : Issue with read-only cells method called with column index -1
  // 2.5.1.14 : Fixed : Issue with KeepHorizScroll and mouse wheel handling
  // 2.5.1.15 : Fixed : Issue with ungrouping & searchfooter
  // 2.5.1.16 : Fixed : Issue with grouping & column property handling for group header rows
  // 2.5.1.17 : Improved : TDBAdvGrid design-time column font initialization
  // 2.5.1.18 : Fixed : Floating footer font size for calculated values on high DPI
  // 2.5.1.19 : Fixed : Delphi 11.2 issue with column resizing in multi-monitor situation
  // 2.5.1.20 : Fixed : Issue with floating footer & color banding
  // 2.5.1.21 : Fixed : Issue with OriginalCellValue access in specific circumstances
...
type
  ...
  protected
    ...
    function GetEditorNone(ACol,ARow: Integer; var AEditor: TEditorType): boolean; override;
   ...
   // procedure GetCellReadOnly(ACol,ARow: Integer;var IsReadOnly: Boolean); override;
    ...
    procedure LoadLookupList(fld: TField; list: TStrings; AllowBlank: Boolean);
    {$IFDEF DELPHI2006_LVL}
    procedure LoadWideLookupList(fld: TField; list: TWideStrings; AllowBlank: Boolean);
    {$ENDIF}
    function GetLookupKey(k: TObject): string;
  ...
  public
    ...
    property ActiveCellGraphic: TCellGraphic read FCellGraphic;
...
//    FFont.Height := Round(FFont.Height / GetGrid.DPIScale);
    FHeaderFont.Assign(TDBGridColumnItem(Source).HeaderFont);
//    FHeaderFont.Height := Round(FHeaderFont.Height / GetGrid.DPIScale);
    FHeaderAlignment := TDBGridColumnItem(source).HeaderAlignment;
    FPassword := TDBGridColumnItem(Source).Password;
    FPrintBorderPen.Assign(TDBGridColumnItem(Source).PrintBorderPen);
    FPrintBorders := TDBGridColumnItem(Source).PrintBorders;
...
begin
  ...
  if Assigned(FDataLink) {and HandleAllocated} then
  begin
...
//FDataLink.OnDataChange := nil;
  ...
  begin
    if FEmptyDataSet and Assigned(OnAutoInsertRow) and not (DataLink.DataSet.State  in [dsEdit, dsInsert]) then
    begin
      OnAutoInsertRow(Self, FixedRows)
    end
    else
...
begin
  ...
  if (TempState = dsInsert) and (FDataLink.DataSet.State = dsBrowse) then
  begin
    FEditEnding := True;
    InvalidateEditor;
    FEditEnding := False;
  ...
  if (FDataLink.DataSet.State = dsBrowse) and EditMode then
  begin
    HideInplaceEdit;
  end;
...
begin
  ...
  begin
    ...
    begin
      if idx >= 0 then
      begin
        o := ComboBox.Items.Objects[idx];
        if o = nil then
          FDataLink.DataSet.FieldByName(Field.KeyFields).Clear
        else
          FDataLink.DataSet.FieldByName(Field.KeyFields).AsString := GetLookupKey(o);
      end;
    end
    else
    begin
      ...
      if (Editor = edComboList) and (idx >= 0) then
      begin
        if Field.DataType in [ftSmallint, ftInteger, ftWord, ftLargeInt {$IFDEF DELPHIXE_LVL}, ftLongWord, ftShortInt{$ENDIF}] then
        begin
...
begin
  c := ACol;
  if (GroupColumn >= 0) and (ACol >= GroupColumn) and not IsNode(ARow) then
    inc(c);
...
var
  ...
  begin
    ...
    if not UseVCLStyles then
      Exit;
    if (ACol >= FColumnCollection.Count) or (ACol < 0) then
      Exit;
    {$IFDEF DELPHIXE6_LVL}
    if not (seClient in TDBGridColumnItem(FColumnCollection.Items[ACol]).StyleElements) then
      Exit;
    {$ENDIF}
    Result := false;
  end;
  ...
  begin
    ...
    if (ACol >= FColumnCollection.Count) or (ACol < 0) then
      Exit;
    {$IFDEF DELPHIXE6_LVL}
    if not (seFont in TDBGridColumnItem(FColumnCollection.Items[ACol]).StyleElements) then
      Exit;
    {$ENDIF}
    Result := false;
  end;
...
begin
  ...
  if (Groupcolumn <> -1) and (ACol >= GroupColumn) and not IsNode(ARow) then
  begin
    RCol := ACol + 1;
    nh := 1;
  end;

  if (ACol >= 0) and (FColumnCollection.Count > ACol) then
  begin
    if (ACol >= FixedCols) and
      (ACol < ColCount - FixedRightCols + nh) and
      (ARow < RowCount - FixedFooters) and not HasCellProperties(ACol,ARow) then
    begin
      fontcol := Font.Color;

      if (RCol >= 0) and (FColumnCollection.Count > RCol) then
      begin

        if UseColumnColorStyle then
        begin
          if TDBGridColumnItem(FColumnCollection.Items[RCol]).Fixed then
            ABrush.Color := FixedColor
          else
            if not (TDBGridColumnItem(FColumnCollection.Items[RCol]).ShowBands and Bands.Active) and (ARow >= FixedRows) then
              ABrush.Color := TDBGridColumnItem(FColumnCollection.Items[RCol]).Color;
        end;

        if ARow < FixedRows then
        begin
          if Assigned(TDBGridColumnItem(FColumnCollection.Items[RCol]).HeaderFont) then
          begin
            AFont.Assign(TDBGridColumnItem(FColumnCollection.Items[RCol]).HeaderFont);
            AFont.Height := Round(AFont.Height/DPIScale);
          end;
        end
        else
        begin
          if Assigned(TDBGridColumnItem(FColumnCollection.Items[RCol]).Font) then
          begin
            AFont.Assign(TDBGridColumnItem(FColumnCollection.Items[RCol]).Font);
            AFont.Height := Round(AFont.Height/DPIScale);
          end;
        end;

        if not UseColumnColorStyle then
          AFont.Color := fontcol;
      end;
    end;
  end;
...
function TDBAdvGrid.GetEditorNone(ACol,ARow: Integer; var AEditor: TEditorType): boolean;
begin
  Result := false;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          allowBlank := (not fld.Required) and Columns[ACol].AllowBlank;
          {$IFDEF DELPHI2006_LVL}
          {$IFNDEF DELPHI_UNICODE}
          ...
          begin
            ...
            {$IFNDEF DELPHI_UNICODE}
            if isWide then
              LoadWideLookupList(fld, slw, allowBlank)
            else
            {$ENDIF}
            {$ENDIF}
              LoadLookupList(fld, sl, allowBlank);
           end;
        end
        else if not FShowDefinedFields then
        begin
          ...
          begin
            ...
            allowBlank := (not fld.Required) and Columns[RCol].AllowBlank;
            {$IFDEF DELPHI2006_LVL}
            {$IFNDEF DELPHI_UNICODE}
            isWide := fld.DataType = ftWideString;
            {$ENDIF}
        ...
        begin
          ...
          begin
            ...
            begin
              BtnEdit.ButtonWidth := Columns[RCol].ButtonWidth;
              BtnEdit.ButtonCaption := Columns[RCol].ButtonCaption;
            end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (FEditPostMode in [epCell,epRow]) then
      begin
        ...
        try
          ...
          begin
            ...
            begin
              ...
              begin
                o := ComboBox.Items.Objects[ComboBox.ItemIndex];
                if o = nil then
                  FDataLink.DataSet.FieldByName(aField.KeyFields).Clear
                else
                  FDataLink.DataSet.FieldByName(aField.KeyFields).AsString := GetLookupKey(o);
              end;
            end
            else
            begin
        ...
        try
          if ShouldUpdateData and (EditPostMode = epCell) then
          begin
            FDataLink.DataSet.Post;
          end;
        except
          ...
          begin
            ...
            begin
              ...
              if aField.DataType = ftWideString then
                o := UniCombo.Items.Objects[UniCombo.ItemIndex]
              else
              {$ENDIF}
                o := ComboBox.Items.Objects[ComboBox.ItemIndex];
              if o = nil then
  ...
  with FDataLink do
    ...
    begin
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              Row := MinI(VisibleRowCount + FixedRows + 1, RowCount - 1);
      ...
      begin                                                            // Change to avoid StackOverFlow
        ...
        else if (FOldState in [dsBrowse, dsInactive]) and (FDataLink.DataSet.State = dsBrowse) and not FInternalCall
...
{$IFDEF DELPHIXE10_LVL}
procedure TDBAdvGrid.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
{$IFNDEF DELPHIXE10_LVL}
...
begin
  ...
  begin
    ...
    begin
      delta := -FDataLink.DataSet.RecNo + 1;
      FDataLink.DataSet.First;
      Result := delta;
      Exit;
    end;
...
begin
  ...
  if (ACol >= Columns.Count) or (ACol < 0) then
    Exit;
...
procedure TDBAdvGrid.LoadLookupList(fld: TField; list: TStrings; AllowBlank: boolean);
var
  s, k: string;
  i: Integer;
begin
...
{$IFDEF DELPHI2006_LVL}
procedure TDBAdvGrid.LoadWideLookupList(fld: TField; list: TWideStrings; AllowBlank: boolean);
var
  s, k: widestring;
  i: Integer;
begin
...
function TDBAdvGrid.GetLookupKey(k: TObject): string;
var
  j: Integer;
begin
  Result := '';
  ...
  begin
    if FLookupKeys.Objects[j - 1] = k then
    begin
...
procedure TDBAdvGrid.GetDefaultProps(ACol, ARow: Integer; AFont: TFont;
  ABrush: TBrush; var AColorTo, AMirrorColor, AMirrorColorTo: TColor;
...
begin
  ...
  begin
    ...
    if Bands.Active and Columns[ACol].ShowBands and not IsFixed(ACol,ARow) then
    begin
      ABrush.Color := clNone;
      AColorTo := clNone;
...
begin
  ...
  if (R <> Row) and (R >= 0) and PageMode and (LastRowClicked or (ShowDBIndicator and (FixedCols > 0) and (C < FixedCols))) and not FRowHeightChanged then
  begin
    if Assigned(DataSource) then
      if Assigned(DataSource.Dataset) then
      begin
...
begin
  ...
  if Assigned(FDataLink) then
  begin
    if FDataLink.Active and Assigned(FDataLink.Dataset) and FDataLink.DataSet.Active  then
...
begin
  ...
  if Navigation.AdvanceOnEnter and (FDataLink.DataSet.State in [dsBrowse, dsEdit, dsInsert]) and (not Navigation.AdvanceInsert or (Col < ColCount - 1)) and (EditPostMode = epCell) then
  begin
    if Key = VK_RETURN then
    begin
      ch := #13;
