...
uses
  Windows, Classes, StdCtrls, ExtCtrls, Controls, Messages, SysUtils, AdvStyleIF,
...
const
  ...
  // v1.3.4.0 : New : Button colors can be set and updated in office 2019 styles
  //          : Fixed : Support for High DPI of new dropdown arrow and high dpi resize on focus issue.
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvSmoothSpeedButton.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvSmoothSpeedButton.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
...
begin
  ...
  else if (Owner.Owner is TAdvSmoothEditBtn) and (TAdvSmoothEditBtn(Owner.Owner).ButtonColor <> clNone) then
  begin
    Canvas.Brush.Color := TAdvSmoothEditBtn(Owner.Owner).ButtonColor;

    if FHot then
      Canvas.Brush.Color := TAdvSmoothEditBtn(Owner.Owner).ButtonColorHot;

    if (FState in [bsDown, bsExclusive]) and not FUp then
      Canvas.Brush.Color := TAdvSmoothEditBtn(Owner.Owner).ButtonColorDown;
    ...
    if TAdvSmoothEditBtn(Owner.Owner).ButtonBorderColor <> clNone then
      Canvas.Pen.Color :=  TAdvSmoothEditBtn(Owner.Owner).ButtonBorderColor
    else if TAdvSmoothEditBtn(Owner.Owner).BorderColor <> clNone then
      Canvas.Pen.Color :=  TAdvSmoothEditBtn(Owner.Owner).BorderColor
    else
    ...
    Canvas.Brush.Color := TAdvSmoothEditBtn(Owner.Owner).ButtonTextColor;

    if FHot then
    begin
      Canvas.Brush.Color := TAdvSmoothEditBtn(Owner.Owner).ButtonTextColorHot;
    end;

    if ((FState in [bsDown, bsExclusive]) and not FUp) then
    begin
      Canvas.Brush.Color := TAdvSmoothEditBtn(Owner.Owner).ButtonTextColorDown;
    end;
...
begin
  ...
  Loc.Right := ClientWidth - Round((FButton.Width) * FDPIScale) - FUnitSize - 2;
...
begin
  ...
  if (FButton <> nil) then
   begin
     cBtnW := Round(FButton.FBWidth * FDPIScale);
...
begin
  inherited;
  Button.PaintTo(DC, X + Width - Round(Button.Width * FDPIScale) - 2 - BorderWidth, Y + BorderWidth + 2 );
end;

procedure TAdvSmoothEditBtn.PaintTo(Canvas: TCanvas; X: Integer; Y: Integer);
begin
  inherited;
  Button.PaintTo(Canvas, X + Width - Round(Button.Width * FDPIScale) - 2 - BorderWidth, Y + BorderWidth + 2 );
end;
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvSmoothEditBtn.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvSmoothEditBtn.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
