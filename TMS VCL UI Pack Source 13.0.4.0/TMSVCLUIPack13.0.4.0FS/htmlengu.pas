                ...
                if TagChar = '>' then  // <B> tag
                  ...
                  begin
                    ...
                    begin
                      ...
                      if (Pos(widestring('BACKGROUND'),TagProp) > 0) and not Calc then
                      begin
                        Prop := Copy(TagProp,Pos(widestring('BACKGROUND'),TagProp)+10,Length(TagProp));
                        ...
                        if (Pos(widestring('://'),Prop) > 0) and Assigned(ic) then
                        begin
                      ...
                      if (Pos(widestring('BGCOLOR'),TagProp)>0) then
                      begin
                        Prop := Copy(TagProp,Pos(widestring('BGCOLOR'),TagProp) + 7,Length(TagProp));
                        Prop := Copy(Prop,Pos('"',Prop) + 1,Length(Prop));
                        Prop := Copy(Prop,1,Pos('"',Prop) - 1);
                        if not Calc then
                        begin
                          if Pos(widestring('CL'),Prop) > 0 then
          ...
          'I':begin
                ...
                begin
                  ...
                  begin
                    ...
                    Prop := Copy(TagProp,Pos(widestring('SRC'),TagProp) + 4,Length(TagProp));
                    Prop := Copy(Prop,Pos('"',Prop) + 1,Length(Prop));
                    Prop := Copy(Prop,1,Pos('"',Prop) - 1);

                    if (Pos(widestring('ALT'),TagProp) > 0) and (AltImg = ImgIdx) then
                    begin
                      Prop := Copy(TagProp,Pos(widestring('ALT'),TagProp) + 4,Length(TagProp));
                      Prop := Copy(Prop,Pos(widestring('"'),Prop) + 1,Length(Prop));
                      Prop := Copy(Prop,1,Pos(widestring('"'),Prop) - 1);
                    end;

                    TagWidth := 0;
                    TagHeight := 0;

                    if Pos(widestring('WIDTH'),TagProp) > 0 then
                    begin
                      Tagp := Copy(TagProp,Pos(widestring('WIDTH'),TagProp) + 6,Length(TagProp));
                      Tagp := Copy(Tagp,Pos(widestring('"'),tagp) + 1,Length(Tagp));
                      Tagp := Copy(Tagp,1,Pos(widestring('"'),tagp) - 1);
                      Val(Tagp,TagWidth,Err);
                    end;

                    if Pos(widestring('HEIGHT'),TagProp) > 0 then
                    begin
                      Tagp := Copy(TagProp,ipos(widestring('HEIGHT'),TagProp) + 7,Length(TagProp));
                      Tagp := Copy(Tagp,pos(widestring('"'),Tagp) + 1,Length(Tagp));
                      Tagp := Copy(Tagp,1,pos(widestring('"'),Tagp) - 1);
                    ...
                    if Pos(widestring('ALIGN'),TagProp) > 0 then
                    begin
                      if Pos(widestring('"TOP'),TagProp) > 0 then
                      begin
                        ImgAli := h - WideTextHeight(Canvas,'gh');
                      end
                      else
                      begin
                        if Pos(widestring('"MIDDLE'),TagProp) > 0 then
          ...
          'P':begin
                ...
                begin
                  ...
                  begin
                    ...
                    if Pos(widestring('RIGHT'),Prop) > 0 then Align := taRightJustify;
                    if Pos(widestring('LEFT'),Prop) > 0 then Align := taLeftJustify;
                    if Pos(widestring('CENTER'),Prop) > 0 then Align := taCenter;
                  end;
...
begin
  ...
  tclose := '>';
  tstop := '</';
  while Pos(topen+tag+tclose,widestring(Uppercase(s))) > 0 do
    s := StringReplace(s,'<'+tag+'>','',[rfIgnoreCase]);
  while Pos(tstop+tag+tclose,widestring(Uppercase(s))) > 0 do
    s := StringReplace(s,'</'+tag+'>','',[rfIgnoreCase]);
  Result := s;
end;
