...
const
  ...
  // v2.1.1.0 : New : Bitmap accessor for TPictureItem
...
type
  ...
  {$IFDEF DELPHIXE_LVL}
  THTMLPicture = class(TTMSFNCSVGBitmap)
  {$ELSE}
  ...
  public
    ...
    {$IFDEF DELPHIXE_LVL}
    procedure DrawSVG(ACanvas: TCanvas; X, Y, W, H: Single); virtual;
    {$ENDIF}
  ...
  public
    ...
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
    function Bitmap: TPicture;
    function Container: TPictureContainer;
  published
...
begin
  ...
  begin
    ...
    begin
      ...
      {$IFDEF DELPHIXE_LVL}
      if THTMLPicture(Source).HasSVG then
...
{$IFDEF DELPHIXE_LVL}
procedure THTMLPicture.DrawSVG(ACanvas: TCanvas; X, Y, W, H: Single);
begin
  if not HasSVG then
    Exit;

  TAdvGraphicsSVGImport(SVG).Draw(ACanvas, RectF(X, Y, X + W, Y + H), True);
end;
{$ENDIF}
...
function TPictureItem.Bitmap: TPicture;
begin
  if Assigned(Container.FPicture) then
  begin
    Container.FPicture.Free;
    Container.FPicture := nil;
  end;

  Container.FPicture := TPicture.Create;
  Result := Container.FPicture;
  Result.Assign(Picture as TGraphic);
end;

function TPictureItem.Container: TPictureContainer;
begin
  Result := TPictureContainer(TPictureCollection(Collection).FOwner);
end;
