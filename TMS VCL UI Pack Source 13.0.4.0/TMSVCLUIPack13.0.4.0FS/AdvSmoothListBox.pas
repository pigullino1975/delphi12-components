...
const
  ...
  // v3.0.1.2 : Fixed : Issue with GetBottomIndex in LayoutMode = lblBubble
  // v3.1.0.0 : New : Support for Office 2019 styles
  //          : Fixed : On Draw Event issue with vertical spacing
  // v3.1.1.0 : Improved : UIStyle property for TMSStyle
  // v3.1.1.1 : Fixed : UIStyle set to tsCustom after initial Office look
  // v3.1.2.0 : New : Support for per monitor DPI
  //          : Improved : On creation check for enabled AdvFormStyler
  // v3.1.2.1 : Improved : function added to change DPI scale from TAdvSmoothComboBox
  // v3.1.2.2 : Fixed : Issue with font size set in runtime on high DPI
  // v3.1.2.3 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // v3.1.2.4 : Fixed : Issue resizing in TAdvSmoothListboxItem when notes were changed to an empty text and autosize was enabled
  // v3.1.2.5 : Fixed : Caption width not correct on high DPI greater than 200 per cent
  // v3.1.3.0 : New : Property DragScroll: boolean added
  // v3.1.3.1 : Fixed : Issue with LookupBar initialization
  // v3.1.4.0 : New : OnFilterEnter and OnFilterExit event added
  //          : New : TAdvSmoothFilter Font property introduced
  // v3.1.4.1 : Fixed : Bottom line of border on last item was not visible
  // v3.1.4.2 : Fixed : Issue with bottom line drawing on ScrollToItem
  // v3.1.4.3 : Improved : Performance for adding HTML formatted items
  // v3.1.4.4 : Fixed : Issue with initializing time when using VCL styles
  // v3.1.5.0 : New : Property Item.NotesLineSpacing added
  // v3.1.5.1 : Improved : Added Key in TAdvSmoothListBoxItem.CopySettings()
  // v3.1.5.2 : Improved : Font handling when form.Scaled = false
  // v3.1.5.3 : Fixed : Small GDI+ based text rendering issue
  // v3.1.5.4 : Fixed : Issue with handling URL's longer than control width
  // v3.1.5.5 : Improved : Text rect calculation for auto sizing items
...
type
  ...
  private
    ...
    procedure SetNotesLineSpacing(const Value: integer);
  protected
    FSelected: Boolean;
    procedure Changed;
    procedure ChangedVisual;
  ...
  published
    ...
    property NotesLineSpacing: integer read FNotesLineSpacing write SetNotesLineSpacing default 2;
  ...
  private
    ...
    FDPIScale: single;
    FDragScroll: boolean;
    FOnFilterEnter: TNotifyEvent;
    FOnFilterExit: TNotifyEvent;
  ...
  public
    ...
    procedure DoFilterExit(Sender: TObject);
  ...
  published
    ...
    property OnFilterEnter: TNotifyEvent read FOnFilterEnter write FOnFilterEnter;
    property OnFilterExit: TNotifyEvent read FOnFilterExit write FOnFilterExit;
...
uses
  CommCtrl, ShellApi, GDIPHTMLEngine;
...
begin
  ...
  begin
    ...
    begin
      res := YToItem((InsideRect.Right - InsideRect.Left) div 2, GetDisplayRect.Bottom - ItemAppearance.VerticalSpacing, false, false);
    end;
...
begin
  case LookupBar.Position of
    pLeft:   Result := Bounds(InsideRect.Left + Round(FDPIScale * LookupBar.GetWidth), InsideRect.Top + Round(FDPIScale * Header.GetHeight) + Round(FDPIScale * Filter.GetHeight), InsideRect.Right - InsideRect.Left - GetShadowOffset,
    InsideRect.Bottom - InsideRect.Top - Round(FDPIScale * Footer.GetHeight) - Round(FDPIScale * Header.GetHeight) - Round(FDPIScale * Filter.GetHeight) - GetShadowOffset);
    pRight:   Result := Bounds(InsideRect.Left, InsideRect.Top + Round(FDPIScale * Header.GetHeight) + Round(FDPIScale * Filter.GetHeight), InsideRect.Right - Round(FDPIScale * LookupBar.GetWidth) - InsideRect.Left - GetShadowOffset,
    InsideRect.Bottom - InsideRect.Top - Round(FDPIScale * Footer.GetHeight) - Round(FDPIScale * Header.GetHeight) - GetShadowOffset - Round(FDPIScale * Filter.GetHeight));
  end;
end;
...
begin
  Result := Round(FDPIScale * Itemappearance.Height);
...
function TAdvSmoothListBox.GetItemBorderWidth: integer;
begin
  Result := Max(ItemAppearance.Fill.BorderWidth, Max(ItemAppearance.FillSelected.BorderWidth, ItemAppearance.FillSelectedAlternate.BorderWidth));
end;
...
begin
  ...
  begin
    ...
    begin
      h := ((Self.Height - Round(FDPIScale * Header.GetHeight) - Round(FDPIScale * Footer.GetHeight) - GetShadowOffset - Round(FDPIScale * Filter.GetHeight)));
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            FScPosTo := ItemRect.Top - Round(FDPIScale * Header.GetHeight) - sh - Round(FDPIScale * Filter.GetHeight);
...
begin
  ...
  Result := Result - Height + Round(FDPIScale * Footer.GetHeight) + GetShadowOffset + GetItemBorderWidth;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if header then
        r := Bounds(rc.Left, rc.Top, rc.Right - rc.Left - GetShadowOffset, Round(FDPIScale * Height))
      else
        r := Bounds(rc.Left, rc.Bottom - Round(FDPIScale * Height) - GetShadowOffset, rc.Right - rc.Left - GetShadowOffset, Round(FDPIScale * Height));
...
begin
  ...
  begin
    ...
    begin
      rc := InsideRect;
      if header then
        Result := Bounds(rc.Left, rc.Top, rc.Right - rc.Left - GetShadowOffset, Round(FDPIScale * Height))
      else
        Result := Bounds(rc.Left, rc.Bottom - Round(FDPIScale * Height) - GetShadowOffset, rc.Right - rc.Left - GetShadowOffset, Round(FDPIScale * Height));
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    try
      ...
      bmp.Canvas.Font.Height := Round(FDPIScale * Sections.Font.Height);
    ...
    thsection := Round(FDPIScale * Sections.Height);

  if LookupBarVisible and (not LookupBar.OnTop) then
  begin
    twLookup := Round(FDPIScale * LookupBar.GetWidth);
    FLookUpSize := twlookup;
  end;

  prevrect := Rect(0, GetShadowOffset, 0, Round(FDPIScale * Header.GetHeight) + Round(FDPIScale * Filter.GetHeight));
...
begin
  ...
  begin
    ...
    begin
      ...
      if c then
      begin
        r := Bounds(ItemRect.Right, ItemRect.Top + (ItemRect.Bottom - ItemRect.Top - Round(FDPIScale * ItemAppearance.DeleteButtonHeight)) div 2, Round(FDPIScale * ItemAppearance.DeleteButtonWidth), Round(FDPIScale * ItemAppearance.DeleteButtonHeight));
...
begin
  ...
  begin
    ...
    begin
      ...
      if c then
      begin
		nSpacedItemRect := FDisplayList.GetItem(i).ItemRect;
	    nSpacedItemRect.Top := nSpacedItemRect.Top - ItemAppearance.VerticalSpacing;
        
		if PtInRect(nSpacedItemRect, Point(X, Y + GetPosition)) and check then
        begin
          Result := i;
          Break;
        end;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                if ItemRect.Top - GetPosition > Height - Round(FDPIScale * Footer.GetHeight) - (ItemRect.Bottom-ItemRect.Top) then
                  FScPosTo := ItemRect.Bottom - (Height - Round(FDPIScale * Footer.GetHeight))
                else if ItemRect.Top < GetPosition + Round(FDPIScale * Header.GetHeight) + Round(FDPIScale * Filter.GetHeight) then
                  FScPosTo := ItemRect.Top - Round(FDPIScale * Header.GetHeight) - sh - Round(FDPIScale * Filter.GetHeight);
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                else if ItemRect.Top < GetPosition + Header.GetHeight + Round(FDPIScale * Filter.GetHeight) + sh then
                  FScPosTo := ItemRect.Top - Header.GetHeight - sh - Round(FDPIScale * Filter.GetHeight);
      ...
      begin
        ...
        begin
          ...
          if Sections.Visible then          
            ...
            FScPosTo := ItemRect.Top - Round(FDPIScale * Header.GetHeight) - Round(FDPIScale * Filter.GetHeight);
...
begin
  ...
  begin
    ...
    if it <> -1 then
    begin
      isdragx := not ((X < FDragX + DRAGMARGIN) and (X > FDragX - DRAGMARGIN));
      isdragy := not ((Y < FDragY + DRAGMARGIN) and (Y > FDragY - DRAGMARGIN));

      FDragging := FDisplayList.GetItem(it).DisplayItem.Selected and EnableDragging;
      FDragging := FDragging and (isdragx or (isdragy and not FDragScroll));
    end
    else
      FDragging := False;
...
//            BitBlt(di.Canvas.Handle,0,0,r.Right, r.Bottom - getPosition, Self.Canvas.Handle, r.Left, r.Top - GetPosition, SRCCopy);
      ...
      begin
        mindrag := Self.Top + Self.Parent.ClientOrigin.Y + Round(FDPIScale * Header.GetHeight) + Round(FDPIScale * Filter.GetHeight);
        maxdrag := Self.Top + Self.Parent.ClientOrigin.Y + self.Height - GetShadowOffset - Round(FDPIScale * Footer.GetHeight) - FDragItemForm.Height;
    ...
    begin
      ...
      if ((FDragY < Y - CLICKMARGIN) or (FDragY > Y + CLICKMARGIN)) and FDragScroll then
      begin
        ScrollIndicator.FAnimateOpacity := ScrollIndicator.Opacity;
        FSp := 4;
        FHoveredItemIndex := -1;
          ...
          case CategoryType of
            ...
            begin
              ...
              if LookupBar.AutoSize then
                l := 1 + Round((Y - Round(FDPIScale * Header.GetHeight) - Round(FDPIScale * Filter.GetHeight) - (th / 2)) / (InsideRect.Bottom - InsideRect.Top - Round(FDPIScale * Footer.GetHeight) - Round(FDPIScale * Header.GetHeight) - Round(FDPIScale * Filter.GetHeight)) * stop)
              else
              begin
                l := 1 + Round((Y - Round(FDPIScale * Header.GetHeight) - Round(FDPIScale * Filter.GetHeight) - ((th + Round(FDPIScale * LookupBar.Spacing)) / 2)) / (th + Round(FDPIScale * LookupBar.Spacing)));
              end;
              ...
              begin
                ...
                begin
                  ...
                  if Assigned(aDispItem) then
                  begin
                    FScPosTo := aDispItem.ItemRect.Top - Round(FDPIScale * Header.GetHeight) - Round(FDPIScale * Filter.GetHeight);
                    FCurrentScPos := FScPosTo;
                  end;
                end;
            ...
            begin
              ...
              if LookupBar.AutoSize then
                l := Round((Y - Round(FDPIScale * Header.GetHeight) - Round(FDPIScale * Filter.GetHeight) - (th / 2)) / (InsideRect.Bottom - InsideRect.Top - Round(FDPIScale * Footer.GetHeight) - Round(FDPIScale * Header.GetHeight) - Round(FDPIScale * Filter.GetHeight)) * stop)
              else
                l := Round((Y - Round(FDPIScale * Header.GetHeight) - Round(FDPIScale * Filter.GetHeight) - ((th + Round(FDPIScale * LookupBar.Spacing)) / 2)) / (th + Round(FDPIScale * LookupBar.Spacing)));
...
begin
  ...
  begin
    ...
    rgn := CreateRectRgn(
      InsideRect.Left - org.X, InsideRect.Top + Round(FDPIScale * Header.GetHeight) + Round(FDPIScale * Filter.GetHeight) - org.Y,
      InsideRect.Right + 1 - org.X, InsideRect.Bottom - Round(FDPIScale * Footer.GetHeight) - GetShadowOffset + 1 - org.Y);
...
begin
  ...
  begin
    ...
    r := Rect(InsideRect.Left, Round(FDPIScale * Header.GetHeight) + Round(FDPIScale * Filter.GetHeight), InsideRect.Right, InsideRect.Bottom - Round(FDPIScale * Footer.GetHeight) - Round(FDPIScale * Header.GetHeight) - Round(FDPIScale * Filter.GetHeight));
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if ItemRect.Top - GetPosition > Height - Round(FDPIScale * Footer.GetHeight) - (ItemRect.Bottom-ItemRect.Top) then
              FScPosTo := ItemRect.Bottom - (Height - Round(FDPIScale * Footer.GetHeight))+ GetItemBorderWidth
            else if ItemRect.Top < GetPosition + Round(FDPIScale * Header.GetHeight) + Round(FDPIScale * Filter.GetHeight) then
              FScPosTo := ItemRect.Top - Round(FDPIScale * Header.GetHeight) - sh - Round(FDPIScale * Filter.GetHeight) + GetItemBorderWidth;
  ...
  case AStyle of
   ...
   tsOffice2019White:
      begin
        ...
        Header.Fill.Color := clWhite;
        Header.Fill.ColorTo := clNone;
        Header.Font.Color := $00444648;
        Header.Fill.BorderColor := $00C4C6C8;
        Header.Fill.GradientType := gtVertical;
        Header.Fill.ColorMirror := clNone;

        Filter.Fill.Color := clWhite;
        Filter.Fill.GradientType := gtSolid;
        Filter.Fill.BorderColor := $00C4C6C8;

        Footer.Fill.Color := clWhite;
        Footer.Fill.ColorTo := clNone;
        Footer.Font.Color := $00444648;
        Footer.Fill.BorderColor := $00C4C6C8;
        Footer.Fill.GradientType := gtVertical;
        Footer.Fill.ColorMirror := clNone;
        ...
        ItemAppearance.FillDisabled.ColorMirror := clNone;
        ItemAppearance.FillDisabled.ColorMirrorTo := clNone;
        ItemAppearance.FillDisabled.BorderColor := $00D4D4D4;
        ItemAppearance.FillDisabled.GradientMirrorType := gtVertical;

        ItemAppearance.FillSelected.Color := $00F2E1D5;
        ItemAppearance.FillSelected.ColorTo := clNone;
        ItemAppearance.FillSelected.ColorMirror := clNone;
        ItemAppearance.FillSelected.ColorMirrorTo := clNone;
        ItemAppearance.FillSelected.BorderColor := $00F2E1D5;
        ItemAppearance.FillSelected.GradientMirrorType := gtVertical;
        ItemAppearance.FillSelected.Glow:= gmNone;
        ItemAppearance.FillSelected.GlowGradientColor:= clNone;

        FLookUpBar.Color := clWhite;
        FLookUpBar.ColorTo := clNone;
        FLookUpBar.Font.Color := clBlack;
        FLookUpBar.DisabledFont.Color := clSilver;
      end;
    tsOffice2019Gray:
      begin
        ItemAppearance.Fill.Glow := gmNone;
        ...
        Header.Fill.Color := $00B8BBBE;
        Header.Fill.ColorTo := clNone;
        Header.Font.Color := $00232425;
        Header.Fill.BorderColor := $00666666;
        Header.Fill.GradientType := gtVertical;
        Header.Fill.ColorMirror := clNone;

        Filter.Fill.Color := $00B8BBBE;
        Filter.Fill.GradientType := gtSolid;
        Filter.Fill.BorderColor := $00666666;

        Footer.Fill.Color := $00B8BBBE;
        Footer.Fill.ColorTo := clNone;
        Footer.Font.Color := $00232425;
        Footer.Fill.BorderColor := $00666666;
        Footer.Fill.GradientType := gtVertical;
        Footer.Fill.ColorMirror := clNone;

        ItemAppearance.Fill.Color := $00B8BBBE;
        ItemAppearance.Fill.ColorTo := clNone;
        ItemAppearance.Fill.ColorMirror := clNone;
        ItemAppearance.Fill.ColorMirrorTo := clNone;
        ItemAppearance.Fill.BorderColor := $00B8BBBE;
        ItemAppearance.Fill.GradientMirrorType := gtVertical;

        ItemAppearance.FillDisabled.Color := $00B8BBBE;
        ItemAppearance.FillDisabled.ColorTo := clNone;
        ItemAppearance.FillDisabled.ColorMirror := clNone;
        ItemAppearance.FillDisabled.ColorMirrorTo := clNone;
        ItemAppearance.FillDisabled.BorderColor := $00D4D4D4;
        ...
        ItemAppearance.FillSelected.Color := $00969696;
        ItemAppearance.FillSelected.ColorTo := clNone;
        ItemAppearance.FillSelected.ColorMirror := clNone;
        ItemAppearance.FillSelected.ColorMirrorTo := clNone;
        ItemAppearance.FillSelected.BorderColor := $00969696;
        ItemAppearance.FillSelected.GradientMirrorType := gtVertical;
        ItemAppearance.FillSelected.Glow:= gmNone;
        ItemAppearance.FillSelected.GlowGradientColor:= clNone;

        FLookUpBar.Color := $00666666;
        FLookUpBar.ColorTo := Color;
        FLookUpBar.Font.Color := clBlack;
        FLookUpBar.DisabledFont.Color := clSilver;
      end;
    tsOffice2019Black:
      begin
        ...
        Header.Fill.Color := $00303132;
        Header.Fill.ColorTo := clNone;
        Header.Font.Color := clWhite;
        Header.Fill.BorderColor := $00ADB0B3;
        Header.Fill.GradientType := gtVertical;
        Header.Fill.ColorMirror := clNone;

        Filter.Fill.Color := $00303132;
        Filter.Fill.GradientType := gtSolid;
        Filter.Fill.BorderColor := $00ADB0B3;

        Footer.Fill.Color := $00303132;
        Footer.Fill.ColorTo := clNone;
        Footer.Font.Color := clWhite;
        Footer.Fill.BorderColor := $00ADB0B3;
        Footer.Fill.GradientType := gtVertical;
        Footer.Fill.ColorMirror := clNone;

        ItemAppearance.Fill.Color := $00303132;
        ItemAppearance.Fill.ColorTo := clNone;
        ItemAppearance.Fill.ColorMirror := clNone;
        ItemAppearance.Fill.ColorMirrorTo := clNone;
        ItemAppearance.Fill.BorderColor := $00303132;;
        ItemAppearance.Fill.GradientMirrorType := gtVertical;


        ItemAppearance.FillDisabled.Color := $00444444;
        ItemAppearance.FillDisabled.ColorTo := clNone;
        ItemAppearance.FillDisabled.ColorMirror := clNone;
        ItemAppearance.FillDisabled.ColorMirrorTo := clNone;
        ItemAppearance.FillDisabled.BorderColor := $00D4D4D4;
        ...
        ItemAppearance.FillSelected.Color := $00505050;
        ItemAppearance.FillSelected.ColorTo := clNone;
        ItemAppearance.FillSelected.ColorMirror := clNone;
        ItemAppearance.FillSelected.ColorMirrorTo := clNone;
        ItemAppearance.FillSelected.BorderColor := $00505050;
        ItemAppearance.FillSelected.GradientMirrorType := gtVertical;
        ItemAppearance.FillSelected.Glow:= gmNone;
        ItemAppearance.FillSelected.GlowGradientColor:= clNone;
      ...
      FLookUpBar.Color := $00444444;
      FLookUpBar.ColorTo := Color;
      FLookUpBar.Font.Color := $00F0F0F0;
      FLookUpBar.DisabledFont.Color := clSilver;
    end;
...
procedure TAdvSmoothListBox.SetDPIScale(ADPIScale: single);
begin
  FDPIScale := ADPIScale;
end;
...
procedure TAdvSmoothListBox.SetUIStyle(const Value: TTMSStyle);
begin
...
begin
  ...
  begin
    ...
    if Item.Splitter then
      h := Round(FDPIScale * SplitterHeight)
    else
      h := ht;

    hs := Round(FDPIScale * ItemAppearance.HorizontalSpacing);
    vs := Round(FDPIScale * ItemAppearance.VerticalSpacing);
    ...
    begin
      ...
      begin
        ItemRect := Rect(InsideRect.Left + left + hs + Item.Indent, prevrect.Bottom + vs,
          InsideRect.Right - right - GetShadowOffset - hs - Round(FDPIScale * ItemAppearance.DeleteButtonWidth),
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        FCurrentControl.Top := Round(FDPIScale * Header.GetHeight) + Round(FDPIScale * Filter.GetHeight);
        FCurrentControl.Width := Width;
        FCurrentControl.Height := Height - Round(FDPIScale * Header.GetHeight) - Round(FDPIScale * Footer.GetHeight) - Round(FDPIScale * Filter.GetHeight);
...
begin
  ...
  begin
    ...
    captionf.Height := Round(FDPIScale * captionf.Height);
    notesf.Height := Round(FDPIScale * notesf.Height);
    infof.Height := Round(FDPIScale * infof.Height);
    ...
    begin
      ...
      f := TGPFont.Create(ff, -infof.Height, fs, UnitPixel);
      ...
      begin
        ...
        gppointf := MakePoint(crl - Round(FDPIScale * ScrollIndicator.GetWidth) - wr, crt);
      end
      else
      begin
        crt := ir.Y;
      ...
      finfor := Makerect(gppointf.X - Round(FDPIScale * FGraphicRightMargin) - 5, gppointf.Y + 2, sri.Width + 2, sri.Height);
    ...
    begin
      ...
      f := TGPFont.Create(ff, -captionf.Height, fs, UnitPixel);
      b := TGPSolidBrush.Create(ColorToARGB(captionf.Color));

      wl := 0;
      if FGraphicLeftType <> gtNone then
        wl := Round(FDPIScale * FGraphicLeftWidth) + Round(FDPIScale * GraphicLeftMargin);

      wr := 0;
      if FGraphicRightType <> gtNone then
        wr := Round(FDPIScale * FGraphicRightWidth) + Round(FDPIScale * GraphicRightMargin);

      UseHTML := (Pos('</', c) > 0) or (Pos('/>', c) > 0) or (Pos('<BR>',uppercase(c)) > 0);

      if UseHTML then
      begin
        capr := Bounds(round(ir.x), Round(ir.y), round(ir.Width - Round(FDPIScale * CaptionMargin) - Round(FDPIScale * LookupBar.GetWidth) - wl - wr), 10000);
        ...
        g.MeasureString(c, length(c), f, MakeRect(ir.X, ir.Y, ir.Width - Round(FDPIScale * CaptionMargin) - Round(FDPIScale * LookupBar.GetWidth) - wl - wr, 10000), sf, src);
      ...
      begin
        ...
        gppointf := MakePoint(crl + Round(FDPIScale * FMargin) + wl, crt);
      end
      else
      begin
        crt := ir.Y;
      ...
      fcaptionr := MakeRect(gppointf.X + Round(FDPIScale * GraphicLeftMargin), gppointf.Y, src.Width + Round(FDPIScale * 3) - Round(FDPIScale * CaptionMarginRight), src.Height);
    ...
    begin
      ...
      ff := TGPFontFamily.Create(notesf.Name);
      if (ff.Status in [FontFamilyNotFound, FontStyleNotFound]) then
      begin
        ff.Free;
        ff := TGPFontFamily.Create('Arial');
      ...
      fs := 0;
      if (fsBold in notesf.Style) then
        fs := fs + 1;
      if (fsItalic in notesf.Style) then
        fs := fs + 2;
      if (fsUnderline in notesf.Style) then
        fs := fs + 4;
      if fsStrikeOut in notesf.Style then
        fs := fs + 8;

      f := TGPFont.Create(ff, -notesf.Height, fs, UnitPixel);
      ...
      sizegl := 0;
      if FGraphicLeftType <> gtNone then
        sizegl := Round(FDPIScale * FGraphicLeftMargin) + Round(FDPIScale * FGraphicLeftWidth);

      sizegr := 0;
      if FGraphicRightType <> gtNone then
        sizegr := Round(FDPIScale * FGraphicRightMargin) + Round(FDPIScale * FGraphicRightWidth);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            g.MeasureString(n+'w', Length(n)+1, f, MakeRect(htmlr.Left, htmlr.Top, htmlr.Right - htmlr.Left, 10000), fr);
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvSmoothListBox.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvSmoothListBox.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
  FDPIScale := GetDPIScale(Self, Canvas);

  Header.Font.Height := MulDiv(Header.Font.Height, M, D);
  Footer.Font.Height := MulDiv(Footer.Font.Height, M, D);

//  InitDisplayList;
  CalculateRects;
  Filter.UpdateFilter;
  FScPosTo := MulDiv(FScPosTo, M, D);
  FCurrentScPos := FScPosTo;
end;
...
procedure TAdvSmoothListBox.DoFilterEnter(Sender: TObject);
begin
  if Assigned(OnFilterEnter) then
    OnFilterEnter(Sender);
end;

procedure TAdvSmoothListBox.DoFilterExit(Sender: TObject);
begin
  if Assigned(OnFilterExit) then
    OnFilterExit(Sender);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      g.SetSmoothingMode(SmoothingModeAntiAlias);
      rc := InsideRect;
      Fill.Fill(g, MakeRect(rc.Left, rc.Top + Round(FDPIScale * Header.GetHeight), rc.Right - rc.Left - GetShadowOffset, Round(FDPIScale * Height)));
      g.Free;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if header then
        fillr := fill.Fill(g, MakeRect(rc.Left, rc.Top, rc.Right - rc.Left - GetShadowOffset, Round(FDPIScale * Height)))
      else
        fillr := fill.Fill(g, MakeRect(rc.Left, rc.Bottom - Round(FDPIScale * Height) - GetShadowOffset, rc.Right - rc.Left - GetShadowOffset, Round(FDPIScale * Height)));
...
begin
      ...
      case Kind of
        ...
        begin
          ...
          begin
            ...
            begin
              Floating := ItemRect.Top <= GetPosition + Round(FDPIScale * Header.GetHeight) + Round(FDPIScale * Filter.GetHeight);
              if Floating then
              begin
  ...
  begin
    ...
    begin
      ...
      pos := GetPosition + Round(FDPIScale * Header.GetHeight) + Round(FDPIScale * Filter.GetHeight);

      if (nextfloatitem <> nil) then
        if nextfloatitem.ItemRect.Top - GetPosition <= diff + Round(FDPIScale * Header.GetHeight) + Round(FDPIScale * Filter.GetHeight) then
...
var
  ...
  customch: String;
  I: integer;
  LookupFont, LookupDisabledFont: TFont;
  f, fd: TGPFont;
  sf: TGPStringFormat;
...
begin
  ...
  begin
    if LookupBarVisible then
    begin
      LookupFont := TFont.Create;
      LookupFont.Assign(Font);
      LookupFont.Height := Round(FDPIScale * LookupFont.Height);

      ca := TCanvas.Create;
      ca.Handle := Canvas.Handle;
      ca.Font.Assign(LookupFont);

      tw := 0;
      maxtextw := GetMaximumCustomTextWidth(ca);
      case CategoryType of
        alphanumeric: tw := ca.TextWidth('W') + Round(FDPIScale * FMargin);
        custom: tw := maxtextw;
      end;
      ...
      p.Fillpath := false;
      case Position of
        pRight: rf := MakeRect(InsideRect.Right - tw - GetShadowOffset, InsideRect.Top + Round(FDPIScale * Header.GetHeight) + Round(FDPIScale * Filter.GetHeight), tw, InsideRect.Bottom - Round(FDPIScale * Footer.GetHeight) - 1 - GetShadowOffset);
        pLeft: rf := MakeRect(InsideRect.Left, InsideRect.Top + Round(FDPIScale * Header.GetHeight) + Round(FDPIScale * Filter.GetHeight), InsideRect.Left + tw, InsideRect.Bottom - Round(FDPIScale * Footer.GetHeight) - 1 - GetShadowOffset);
      end;
      ...
      LookupDisabledFont := TFont.Create;
      LookupDisabledFont.Assign(DisabledFont);
      LookupDisabledFont.Height := Round(FDPIScale * LookupDisabledFont.Height);

      f := g.MakeFont(LookupFont);
      fd := g.MakeFont(LookupDisabledFont);

      LookupFont.Free;
      LookupDisabledFont.Free;

      y := 0;
      if not AutoSize then
         y := InsideRect.Top + Round(FDPIScale * Header.GetHeight) + Round(FDPIScale * Filter.GetHeight);
      ...
      case CategoryType of
        ...
        begin
          ...
          begin
            ...
            x := 0;
            case Position of
              pLeft: x := InsideRect.Left + (Round(FDPIScale * FMargin) div 2);
              pRight: x := InsideRect.Right - GetShadowOffset - (tw + Round(FDPIScale * FMargin) div 2);
            end;

            if AutoSize then
            begin
              pos := InsideRect.Top + Round(FDPIScale * Header.GetHeight) + Round(FDPIScale * Filter.GetHeight);
              y := pos + Round(((InsideRect.Bottom - pos - GetShadowOffset - Round(FDPIScale * Footer.GetHeight)) / (stop - 1)) * (I - 1));
            end;
            ...
            if not AutoSize then
              y := y + th + Round(FDPIScale * LookupBar.Spacing)
          end;
        end;
        custom:
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                pos := InsideRect.Top + Round(FDPIScale * Header.GetHeight) + Round(FDPIScale * Filter.GetHeight);
                y := pos + Round(((InsideRect.Bottom - pos - GetShadowOffset - Round(FDPIScale * Footer.GetHeight)) / stop) * I);
              end;
...
begin
  ...
  begin
    w := Round(FDPIScale * LookupBar.GetWidth);
    ...
    if (GetFullHeight > 0) and (Self.Height > 0) then
      h := ((Self.Height - Round(FDPIScale * Header.GetHeight) - Round(FDPIScale * Filter.GetHeight) - Round(FDPIScale * Footer.GetHeight) - GetShadowOffset) / (GetFullHeight + Height + GetShadowOffset)) * Self.Height;

    pos := 0;

    if (GetPosition > 0) and (GetFullHeight > 0) then
      pos := (Self.Height - Round(FDPIScale * Header.GetHeight) - Round(FDPIScale * Filter.GetHeight) - h - Round(FDPIScale * Footer.GetHeight) - GetShadowOffset) / GetFullHeight * GetPosition;

    pos := pos + Round(FDPIScale * Header.GetHeight) + Round(FDPIScale * Filter.GetHeight);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        SectionFont.Assign(FFont);
        SectionFont.Height := Round(FDPIScale * SectionFont.Height);

        ff := TGPFontFamily.Create(SectionFont.Name);
        if (ff.Status in [FontFamilyNotFound, FontStyleNotFound]) then
        begin
          ff.Free;
          ff := TGPFontFamily.Create('Arial');
        ...
        fs := 0;
        if (fsBold in SectionFont.Style) then
          fs := fs + 1;
        if (fsItalic in SectionFont.Style) then
          fs := fs + 2;
        if (fsUnderline in SectionFont.Style) then
          fs := fs + 4;
        if fsStrikeOut in SectionFont.Style then
          fs := fs + 8;

        sf := TGPStringFormat.Create;
        f := TGPFont.Create(ff, -SectionFont.Height, fs, UnitPixel);
...
begin
  ...
  begin
    ...
    FNotesLineSpacing := (Source as TAdvSmoothListBoxItem).NotesLineSpacing;
...
procedure TAdvSmoothListBoxItem.ChangedVisual;
begin
  if Assigned(FOwner) and (FOwner.FUpdateCount = 0) then
  begin
    if Assigned(FOwner.FCurrentControl) and not FOwner.FAnimatingdetail then
...
begin
  ...
  begin
    ...
    FKey := (Source as TAdvSmoothListBoxItem).Key;
    FEnabled := (Source as TAdvSmoothListBoxItem).Enabled;
    FChecked := (Source as TAdvSmoothListBoxItem).Checked;
    FCaptionAlignment := (Source as TAdvSmoothListBoxItem).CaptionAlignment;
...
var
  ...
  sb: TGPSolidBrush;
  isSel: boolean;
  captionf, notesf, infof, delbtnf: TFont;
...
begin
  ...
  begin
    ...
    if FDeleteButton and DeleteButtonVisible then
      r.Right := r.Right + Round(FDPIScale * ItemAppearance.DeleteButtonWidth);
    ...
    begin
      ...
      begin
        HTMLDrawGDIP(g, notesf, n, fhtmlrvar, FImages, 0, 0, -1, -1, NotesShadowOffset, False, False, False, False,
          False, False, True, 1.0, NotesURLColor, clNone, clNone, NotesShadowColor, a, s, k, XSize, YSize, l, m, hr, nil, FContainer, NotesLineSpacing);
      end
      else
      begin
        f := g.MakeFont(notesf);
    ...
    begin
      ca := TCanvas.Create;
      ca.Font.Height := Round(FDPIScale * -11);
      h := g.GetHDC;
      ca.Handle := h;
      ...
      if showgraphic then
      begin
        fgrLeft := Bounds(R.Left + Round(FDPIScale * GraphicLeftMargin), r.Top - GetPosition + (R.Bottom - R.Top - Round(FDPIScale * FGraphicLeftHeight)) div 2,
          Round(FDPIScale * FGraphicLeftWidth), Round(FDPIScale * FGraphicLeftHeight));

        frameleftr := Bounds(fgrLeft.Left + (fgrLeft.Right - fgrLeft.Left - Round(FDPIScale * 17)) div 2, fgrLeft.Top + (fgrLeft.Bottom - fgrLeft.Top - Round(FDPIScale * 17)) div 2, Round(FDPIScale * 17), Round(FDPIScale * 17));
        ...
        case GraphicLeftType of
          gtCheckBox:
            ...
            begin
              fgrLeft := Bounds(R.Left + Round(FDPIScale * GraphicLeftMargin), r.Top - GetPosition + (R.Bottom - R.Top - Round(FDPIScale * 13)) div 2,
                Round(FDPIScale * 13), Round(FDPIScale * 13));
          ...
          begin
            ...
            bx := fgrLeft.Left;
            by := fgrLeft.Top;
            bw := Round(FDPIScale * GraphicLeftWidth);
            bh := Round(FDPIScale * GraphicLeftHeight);
            ...
            FOwner.FItemAppearance.FButtonAppearance.Draw(gpic, ButtonCaption, bx, by, bw, bh, 0, 0,
              ...
              rtBoth, FGraphicLeft, 0, 0, true, GraphicLeftIndex, GraphicLeftName, TextRenderingHintSystemDefault, FDPIScale);
      ...
      if showgraphic then
      begin
        fgrRight := Bounds(R.Right - Round(FDPIScale * GraphicRightMargin) - Round(FDPIScale * FGraphicRightWidth) - Round(FDPIScale * ScrollIndicator.GetWidth),
          r.Top - GetPosition + (R.Bottom - R.Top - Round(FDPIScale * FGraphicRightHeight)) div 2, Round(FDPIScale * FGraphicRightWidth), Round(FDPIScale * FGraphicRightHeight));

        framerightr := Bounds(fgrRight.Left + (fgrRight.Right - fgrRight.Left - 17) div 2, fgrRight.Top + (fgrRight.Bottom - fgrRight.Top - Round(FDPIScale * 17)) div 2, Round(FDPIScale * 17), Round(FDPIScale * 17));
        ...
        case GraphicRightType of
          gtCheckBox:
            if FIsWinXP and IsThemeActive then
            begin
              fgrRight := Bounds(R.Right - Round(FDPIScale * GraphicRightMargin) - Round(FDPIScale * 13) - Round(FDPIScale * ScrollIndicator.GetWidth),
              r.Top - GetPosition + (R.Bottom - R.Top - Round(FDPIScale * 13)) div 2, Round(FDPIScale * 13), Round(FDPIScale * 13));
              htheme := OpenThemeData(Handle,'button');
          ...
          begin
            ...
            bx := fgrRight.Left;
            by := fgrRight.Top;
            bw := Round(FDPIScale * GraphicRightWidth);
            bh := Round(FDPIScale * GraphicRightHeight);
            ...
            FOwner.FItemAppearance.FButtonAppearance.Draw(gpic, ButtonCaption, bx, by, bw, bh, 0, 0,
              ...
              rtBoth, FGraphicRight, 0, 0, true, GraphicLeftIndex, GraphicLeftName, TextRenderingHintSystemDefault, FDPIScale);
      ...
      begin
        if ProgressPosition <> plCustom then
          GetTextPosition(x, y, MakeRect(R.Left, R.Top, R.Right - R.Left, R.Bottom - R.Top),
            Round(FDPIScale * ProgressWidth) + (Round(FDPIScale * ProgressMargin * 2)), Round(FDPIScale * ProgressHeight) + (Round(FDPIScale * ProgressMargin * 2)), ProgressPosition)
        else
        begin
          x := Round(FDPIScale * ProgressLeft);
          y := Round(FDPIScale * ProgressTop);
        end;

        pr := Bounds(R.Left + x + Round(FDPIScale * ProgressMargin), R.Top - GetPosition + y + Round(FDPIScale * ProgressMargin), Round(FDPIScale * ProgressWidth), Round(FDPIScale * ProgressHeight));
        Draw(g, pr, ProgressMinimum, ProgressMaximum, ProgressValue, pbdHorizontal, FOwner.FDPIScale);
      end;
    end;

    if FDeleteButton and DeleteButtonVisible then
    begin
      delr := MakeRect(r.Right - Round(FDPIScale * ItemAppearance.DeleteButtonWidth) - Round(FDPIScale * 5) - 2, r.Top - GetPosition +
        (r.Bottom - r.Top - Round(FDPIScale * ItemAppearance.DeleteButtonHeight)) / 2, Round(FDPIScale * ItemAppearance.DeleteButtonWidth) + 2,
          Round(FDPIScale * ItemAppearance.DeleteButtonHeight));
      ...
      delbtnf := TFont.Create;
      delbtnf.Assign(ItemAppearance.DeleteButtonFont);
      delbtnf.Height := Round(FDPIScale * delbtnf.Height);

      ftg := g.MakeFont(delbtnf);
...
begin
  ...
  begin
    if (Value.Height <> FCaptionFont.Height) and not (csDesigning in  FOwner.ComponentState) then
    begin
      FCaptionFont.Assign(Value);
      FCaptionFont.Height := Round(Value.Height / FOwner.FDPIScale);
    end
    else
      FCaptionFont.Assign(Value);
    FontChanged(Self);
...
begin
  ...
  begin
    if (Value.Height <> FCaptionSelectedFont.Height) and not (csDesigning in  FOwner.ComponentState) then
    begin
      FCaptionSelectedFont.Assign(Value);
      FCaptionSelectedFont.Height := Round(FCaptionSelectedFont.Height / FOwner.FDPIScale);
   end
    else
      FCaptionSelectedFont.Assign(Value);
    FontChanged(Self);
...
begin
  ...
  begin
    if (Value.Height <> FInfoFont.Height) and not (csDesigning in  FOwner.ComponentState) then
    begin
      FInfoFont.Assign(Value);
      FInfoFont.Height := Round(Value.Height / FOwner.FDPIScale);
    end
    else
      FInfoFont.Assign(Value);
    FontChanged(Self);
...
begin
  ...
  begin
    if (Value.Height <> FInfoSelectedFont.Height) and not (csDesigning in  FOwner.ComponentState) then
    begin
      FInfoSelectedFont.Assign(Value);
      FInfoSelectedFont.Height := Round(Value.Height / FOwner.FDPIScale);
    end
    else
      FInfoSelectedFont.Assign(Value);
    FontChanged(Self);
...
begin
  ...
  begin
    if (Value.Height <> FNotesFont.Height) and not (csDesigning in  FOwner.ComponentState) then
    begin
      FNotesFont.Assign(value);
      FNotesFont.Height := Round(Value.Height / FOwner.FDPIScale);
    end
    else
      FNotesFont.Assign(value);
    Changed;
...
procedure TAdvSmoothListBoxItem.SetNotesLineSpacing(const Value: integer);
begin
  if (FNotesLineSpacing <> Value) then
  begin
    FNotesLineSpacing := Value;
    Changed;
  end;
end;
...
begin
  ...
  begin
    if (Value.Height <> FNotesSelectedFont.Height) and not (csDesigning in  FOwner.ComponentState) then
    begin
      FNotesSelectedFont.Assign(Value);
      FNotesSelectedFont.Height := Round(Value.Height / FOwner.FDPIScale);
    end
    else
      FNotesSelectedFont.Assign(Value);
    FontChanged(Self);
...
begin
  ...
  begin
    ...
    case Fowner.CategoryType of
      alphanumeric: Result := bmp.Canvas.TextWidth('W') + Round(Fowner.FDPIScale * FMargin);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              if ((s[1] >= '0') and (s[1] <= '9')) or ((s[1] >= 'a') and (s[1] <= 'z')) or ((s[1] >= 'A') and (s[1] <= 'Z')) then
              begin
...
begin
  ...
  TCustomEditProtected(FEdit).OnEnter := EditEnter;
  TCustomEditProtected(FEdit).OnExit := EditExit;
  FAutoFilter := true;
...
procedure TAdvSmoothListBoxFilter.EditEnter(Sender: TObject);
begin
  if Visible and Assigned(FEdit) and Assigned(FOwner) then
  begin
    FOwner.DoFilterEnter(FOwner);
  end;
end;

procedure TAdvSmoothListBoxFilter.EditExit(Sender: TObject);
begin
  if Visible and Assigned(FEdit) and Assigned(FOwner) then
  begin
    FOwner.DoFilterExit(FOwner);
  end;
end;
...
function TAdvSmoothListBoxFilter.GetFont: TFont;
begin
  Result := TCustomEditProtected(FEdit).Font
end;
...
procedure TAdvSmoothListBoxFilter.SetFont(const Value: TFont);
begin
    if Assigned(FEdit) then
      TCustomEditProtected(FEdit).Font.Assign(Value);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FEdit.Left := rc.Left + Round(p.FDPIScale * 5);
      FEdit.Top := rc.Top + Round(p.FDPIScale * p.Header.GetHeight) + (Round(p.FDPIScale * p.Filter.GetHeight) - Round(p.FDPIScale * FEditHeight)) div 2;
      FEdit.Height := Round(p.FDPIScale * FEditHeight);

      if (not AutoFilter) then
        FEdit.Width := rc.Right - rc.Left - p.GetShadowOffset - FButton.Width - Round(p.FDPIScale * 15)
      else
        FEdit.Width := rc.Right - rc.Left - p.GetShadowOffset - Round(p.FDPIScale * 10);
      ...
      FButton.Left := FEdit.Left + FEdit.Width + Round(p.FDPIScale * 5);
      FButton.Top := rc.Top + Round(p.FDPIScale * p.Header.GetHeight) + (Round(p.FDPIScale * p.Filter.GetHeight) - FButton.Height) div 2;
      ...
      FShowButton.Left := rc.Left + Round(p.FDPIScale * 3);
      FShowButton.Top := rc.Top - FShowButton.Height - Round(p.FDPIScale * 3) + Round(p.FDPIScale * p.Header.GetHeight);
