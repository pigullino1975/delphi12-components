...
uses
  Forms, Windows, Types, SysUtils, Classes, Graphics, Menus, Controls, AdvGDIP,
  AdvStyleIF, ImgList
...
const
  ...
  // 2.6.6.1 : Improved : Notes rendering on high DPI
  // 2.7.0.0 : New : Support for Office 2019 styles
  // 2.7.1.0 : New : Internal style implemented for AdvMainMenu and AdvPopUpMenu
  // 2.7.1.1 : Improved : On creation check for enabled AdvFormStyler
  // 2.7.1.2 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // 2.7.1.3 : Fixed : Design-time issue with copy & paste of TAdvPopupMenu
  // 2.7.1.4 : Improved : Rendering of TAdvPopupMenu with menu items with break
  // 2.7.1.5 : Fixed : Issue with high DPI when menu system font is used
  // 2.7.1.6 : Fixed : Drawing issue with menu item text with shortcuts
  // 2.7.1.7 : Fixed : Rendering issue with BidiMode right to left
  // 2.7.1.8 : Fixed : Issue with using TAdvMenuStyler on data module
  // 2.7.1.9 : Improved : Submenu item triangle drawing for high DPI
  // 2.7.1.10: Fixed : Regression with subitem triangle drawing
  // 2.7.1.11: Fixed : Issue with menu font size when custom menu styler is used in high DPI
  // 2.7.1.12: Improved : Do not draw menu border when MenuBorderColor = clNone
...
type
  ...
  TPopupControlPosition = (pcpBottomLeft, pcpBottomRight, pcbTopLeft, pcpTopRight, pcpCenter);
...
type
  ...
  [ComponentPlatformsAttribute(pidWin32 or pidWin64)]
  {$ENDIF}
  TAdvMainMenu = class(TMainMenu, ITMSStyle, IAdvMenu , IUnknown)
  private
  ...
  published
    ...
    property DisabledImages: TCustomImageList read FDisabledImages write FDisabledImages;
    property MenuStyler: TCustomAdvMenuStyler read GetMenuStyler write SetMenuStyler;
  ...
  [ComponentPlatformsAttribute(pidWin32 or pidWin64)]
  {$ENDIF}
  TAdvPopupMenu = class(TPopupMenu, ITMSStyle, IAdvMenu , IUnknown)
  private
    { Private declarations }
    FMenuStyler: TCustomAdvMenuStyler;
    FInternalMenuStyler: TCustomAdvMenuStyler;
  ...
  public
    ...
    procedure PopupAtControl(AControl: TControl; Position: TPopupControlPosition = pcpCenter; DeltaX: integer = 0; DeltaY: integer = 0);
  ...
  published
    ...
    property UIStyle: TTMSStyle read FTMSStyle write SetUIStyle;
  end;
...
function AfterLastMenuBreak(MenuItem: TMenuItem): Boolean;
...
uses
  ActnList, AdvMenuUtil, Messages, Dialogs, AdvMenuStylers;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if Assigned(AdvMenu) and Assigned(AdvMenu.MenuStyler) then
          ACanvas.Font.Height := Round(ACanvas.Font.Height * FDPIScale)
        else
          ACanvas.Font.Height := Round(ACanvas.Font.Height);
      end;
...
begin
  ...
  if AfterLastMenuBreak(MenuItem) then
    ARect.Right := ARect.Right + TriangleSize;
...
begin
  ...
  frm := nil;
  if (AdvMenu.Self.Owner is TCustomForm) then
    frm := (AdvMenu.Self.Owner as TCustomForm);

  if Assigned(frm) and (frm.HandleAllocated) then
  begin
    if GetMenuItemRect(frm.Handle, AdvMenu.Self.Handle, MenuItem.MenuIndex, dr) then
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if AdvMenu.BiDiMode = bdRightToLeft then
...
var
  ...
  begin
    ...
    begin
      ...
      if not Assigned(AdvMenu.MenuStyler) then
      begin
        ...
        {$ENDIF}
        R := ItemRect;
        R.Left := ARect.Right - ACanvas.TextWidth('> ');
        DrawText(ACanvas.Handle, '>', 1, R, DT_SINGLELINE or DT_VCENTER);
      end
      else
...
{$ENDIF}
      ...
      if AdvMenu.BidiMode = bdLeftToRight then
        ARect.Right := ARect.Left + origw;
...
begin
  if MenuItem.GetParentMenu is TAdvMainMenu then
    if (csDestroying in (MenuItem.GetParentMenu).ComponentState) then
      Exit;
...
begin
  ...
  {$ENDIF}
    ...
    begin
      ...
      begin
       ...
       if not AfterLastMenuBreak(MenuItem.Parent) then
        begin
  ...
  if AfterLastMenuBreak(MenuItem.Parent) then
    Inc(WindowRect.Right, TriangleSize);
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if MenuBorderColor = clNone then
          Brush.Style := bsClear;
...
begin
  ...
  {$IFDEF DELPHIXE2_LVL}
    ...
    vclstyles := CheckVCLStylesEnabled(LStyle,(csDesigning in AdvMenu.Self.ComponentState));
  {$ENDIF}
...
begin
  ...
  with MenuItem do
  begin
    for I := MenuIndex to Parent.Count - 1 do
    begin
      PMI := Parent.Items[I];
      if not PMI.Visible then
        Continue;
...
begin
  ...
  begin
    SetUIStyle(GetDefaultStyle(AOwner));
  end;
end;
...
procedure TAdvPopupMenu.SetUIStyle(const Value: TTMSStyle);
begin
  FTMSStyle := Value;
...
procedure TAdvPopupMenu.SetBidiMode(const Value: TBidiMode);
begin
  BidiMode := Value;
end;

procedure TAdvPopupMenu.SetComponentStyle(AStyle: TTMSSTyle);
var
  tempStyler : TAdvMenuOfficeStyler;
begin
  
  if not Assigned(FMenuStyler) then
  begin
    FTMSStyle := AStyle;
    if not (AStyle = tsCustom) then
    begin
      tempStyler := TAdvMenuOfficeStyler.Create(Self);
      tempStyler.SetComponentStyle(AStyle);
      FCurrentMenuStyler := tempStyler;
    end;
  end;
end;
...
procedure TAdvPopupMenu.PopupAtControl(AControl: TControl; Position: TPopupControlPosition = pcpCenter; DeltaX: integer = 0; DeltaY: integer = 0);
var
  pt: TPoint;
begin
...
procedure TAdvMainMenu.SetUIStyle(const Value: TTMSStyle);
begin
  FTMSStyle := Value;
  SetComponentStyle(Value);
end;
...
procedure TAdvMainMenu.SetBidiMode(const Value: TBidiMode);
begin
  BidiMode := Value;
end;

procedure TAdvMainMenu.SetComponentStyle(AStyle: TTMSStyle);
var
  tempStyler : TAdvMenuOfficeStyler;
begin
  if not Assigned(FMenuStyler) then
...
function TAdvMainMenu.GetBidiMode: TBidiMode;
begin
  Result := BidiMode;
end;
...
function TAdvPopupMenu.GetBidiMode: TBidiMode;
begin
  Result := BidiMode;
end;
