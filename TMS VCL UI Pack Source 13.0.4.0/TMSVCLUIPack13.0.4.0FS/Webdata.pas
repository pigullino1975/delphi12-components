...
const
  ...
  // 1.4.1.2 : Fixed : Issue with scanning image refs using HTTPS
  // 1.4.1.3 : Fixed : Issue with retrieval of image references on HTTPS
  // 1.4.1.4 : Fixed : Issue with executing loops with multiple HTTP errors
...
var
  ...
  function OpenInternetconnection: boolean;
  begin
    Result := false;
    // http
    if FProxy = '' then
...
begin
  ...
  if not OpenInternetConnection then
    Exit;
