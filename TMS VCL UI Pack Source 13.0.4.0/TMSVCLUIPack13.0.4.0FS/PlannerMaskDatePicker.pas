{***********************************************************************}
{ TPlannerMaskDatePicker component                                      }
...
uses
  ...
  AdvMEdBtn, PlannerCal, MaskUtils, Variants, AdvStyleIF;
...
const
  ...
  // 1.5.2.0 : New : UIStyle property for TMSStyles
  //         : New : Support for high DPI
  // 1.6.0.0 : New : Property ButtonTransparent

type
  {$IFDEF DELPHIXE2_LVL}
  [ComponentPlatformsAttribute(pidWin32 or pidWin64)]
  {$ENDIF}
  TPlannerMaskDatePicker = class(TAdvMaskEditBtn, ITMSStyle)
  private
...
begin
  ...
  DPIScale := GetDPIScale(Self , FPlannerParent.Canvas);
  FPlannerCalendar.DPIScale := DPIScale;

  FPlannerCalendar.Width := Round(FOrigWidth * DPIScale);
  FPlannerCalendar.Height := Round(FOrigHeight * DPIScale);
  ...
  FPlannerCalendar.Font.Height := Round(FOrigFontHeight * DPIScale);
...
procedure TPlannerMaskDatePicker.EndUpdate;
begin
...
procedure TPlannerMaskDatePicker.BeginUpdate;
begin
...
procedure TPlannerMaskDatePicker.SetUIStyle(const Value: TTMSStyle);
begin
...
procedure TPlannerMaskDatePicker.SetComponentStyle(AStyle: TTMSStyle);
begin
  inherited;
  FTMSStyle := AStyle;
  Calendar.SetComponentStyle(AStyle);
end;
