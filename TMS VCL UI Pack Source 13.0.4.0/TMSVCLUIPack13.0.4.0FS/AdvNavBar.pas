...
const
  ...
  // 2.3.0.0 : New : Support for Office 2019 styles
  //         : Fixed : High DPI for popup indicator
  //         : New : SVG drawing of thumbnails in fixed tab
  // 2.3.0.1 : Fixed : Issue with drawing panel captions for panels hidden
  // 2.3.0.2 : Fixed : Panel height on high DPI
  //         : Improved : On creation check for enabled AdvFormStyler
  // 2.3.0.3 : Fixed : Issue with color initialization of panels
  // 2.3.0.4 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // 2.3.0.5 : Fixed : Default panel text color for esCustom style
  // 2.3.0.6 : Fixed : Issue with DefaultTabPosition = tpTop and ShowShortCutBar = false and drawing of first tab
...
begin
  ...
  begin
    ...
    begin
      ...
      if FAdvNavBar.FDesignTime or (FAdvNavBar.Style = esCustom) then
      begin
        Color := FAdvNavBar.Color;
...
begin
  if FDesignTime then
    Result := TAdvNavBarPanel.Create(GetParentForm(Self))
  else
...
begin
  ...
  if (ShowShortCutBar = false) and (DefaultTabPosition = tpTop) and (TabR.Top = 0) then
    isVis := true;
  ...
  if (TabR.Left = 0) and (TabR.Right = 0) and (Index <> FActiveTabIndex) then
    Exit;
...
begin
  ...
  begin
    {$IFDEF DELPHIXE12_LVL}
    b := (FImages is TVirtualImageList) and Assigned((FImages as TVirtualImageList).ImageCollection);
    if b then
    begin
      col := (FImages as TVirtualImageList).ImageCollection;
      DR := TabR;
      InflateRect(DR, -4, -4);
      col.Draw(Canvas, DR, TAdvNavBarPanel(FAdvNavBarPanel[Index]).ImageIndex, True);
    end
    else
    {$ENDIF}
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if FUSeVCLStyles then
        sb := TGPSolidBrush.Create(MakeColor(255, SectionFont.Color))
      else
        sb := TGPSolidBrush.Create(MakeColor(255, Font.Color));

      pts[0].X := R.Left + (R.Right - R.Left) div 2 - Round(FDPIScale * 3);
      pts[0].Y := R.Bottom - Round(FDPIScale * 10);
      pts[1].X := R.Left + (R.Right - R.Left) div 2 + Round(FDPIScale * 3);
      pts[1].Y := R.Bottom - Round(FDPIScale * 10);
      pts[2].X := R.Left + (R.Right - R.Left) div 2 ;
      pts[2].Y := R.Bottom - Round(FDPIScale * 7);
    ...
    begin
      ...
      begin
        ...
        begin
          Canvas.TextOut(R.Right - GetIndicatorWidth + Round(FDPIScale * 6), R.Top + 6, '>');
          Canvas.TextOut(R.Right - GetIndicatorWidth + Round(FDPIScale * 9), R.Top + 6, '>');
        end;         *)
        ...
        size := (R.Right - R.Left) div 5;

        Canvas.MoveTo(R.Left + (R.Right - R.Left) div 2 - size, R.Top + Round(FDPIScale * 6));
        Canvas.LineTo(R.Left + (R.Right - R.Left) div 2, R.Top + Round(FDPIScale * 6) + size);
        Canvas.LineTo(R.Left + (R.Right - R.Left) div 2 - size - 1, R.Top + Round(FDPIScale * 6) + size * 2 + 1);
        Canvas.MoveTo(R.Left + (R.Right - R.Left) div 2 , R.Top + Round(FDPIScale * 6));
        Canvas.LineTo(R.Left + (R.Right - R.Left) div 2 + size, R.Top + Round(FDPIScale * 6) + size);
        Canvas.LineTo(R.Left + (R.Right - R.Left) div 2 - 1, R.Top + Round(FDPIScale * 6) + size * 2 + 1);
      end;
    end;
  end;
end;
...
begin
  ...
  begin
    if IsOffice2010 then
      Result := Round(FDPIScale * IndicatorWidthOffice2010)
    else
      Result := Round(FDPIScale * IndicatorWidth);
  end
  else
  begin
    if IsOffice2010 then
    ...
    esWindowsXP:
      ...
      esOffice2019White:
        begin
          ...
          CaptionFont.Color := $00444648;
          Font.Color := $00444648;

          ActiveColor := $00FAECE3;
          ActiveColorTo := $00FAECE3;

          ActiveMirrorColor := clNone;
          ActiveMirrorColorTo := clNone;
          ActiveTextColor := $003B3B3B;
          ...
          DownTabColor := $00B0784A;
          DownTabColorTo := $00B0784A;
          DownTabMirrorColor := clNone;
          DownTabMirrorColorTo := clNone;

          DownTextColor := $00444648;
          HoverTextColor := $00444648;
          DefaultTextColor := $00444648;


          HoverTabColor := $00FAECE3;
          HoverTabColorTo := $00FAECE3;
          HoverTabMirrorColor := clNone;
          HoverTabMirrorColorTo := clNone;

          CollapsedHoverColor := $00FAECE3;
          CollapsedHoverColorTo := $00FAECE3;
          CollapsedDownColor := clNone;
          CollapsedDownColorTo := clNone;

          SplitterColor := clWhite;
          SplitterColorTo := clWhite;
          SplitterBorderColor := clNone;


          BorderColor := $00A3A3A3;

          SectionColor := $00A3A3A3;
          SectionColorTo := $00A3A3A3;
      ...
      esOffice2019Gray:
        begin
          Color := $00B8BBBE;
          CaptionColor := $00B8BBBE;
          CaptionColorTo := $00B8BBBE;

          CaptionFont.Color := $00232425;
          Font.Color := $00232425;

          ActiveColor := $00939597;
          ActiveColorTo := $00939597;

          ActiveMirrorColor := clNone;
          ActiveMirrorColorTo := clNone;
          ActiveTextColor := $003B3B3B;

          DefaultTabColor := $00B8BBBE;
          DefaultTabColorTo := $00B8BBBE;

          DefaultTabMirrorColor := clNone;
          DefaultTabMirrorColorTo := clNone;

          DownTabColor := $0086888A;
          DownTabColorTo := $0086888A;
          DownTabMirrorColor := clNone;
          DownTabMirrorColorTo := clNone;

          DownTextColor := $00232425;
          HoverTextColor := $001E1F20;
          DefaultTextColor := $00232425;


          HoverTabColor := $00939597;
          HoverTabColorTo := $00939597;
          HoverTabMirrorColor := clNone;
          HoverTabMirrorColorTo := clNone;

          CollapsedHoverColor := $00939597;
          CollapsedHoverColorTo := $00939597;
          CollapsedDownColor := clNone;
          CollapsedDownColorTo := clNone;

          SplitterColor := $00B8BBBE;
          SplitterColorTo := $00B8BBBE;
          SplitterBorderColor := clNone;


          BorderColor := $00505050;

          SectionColor := $00505050;
          SectionColorTo := $00505050;
      ...
      esOffice2019Black:
        begin
          Color := $00303132;
          CaptionColor := $00303132;
          CaptionColorTo := $00303132;

          CaptionFont.Color := clWhite;
          Font.Color := clWhite;

          ActiveColor := $00444648;
          ActiveColorTo := $00444648;

          ActiveMirrorColor := clNone;
          ActiveMirrorColorTo := clNone;
          ActiveTextColor := clWhite;

          DefaultTabColor := $00303132;
          DefaultTabColorTo := $00303132;

          DefaultTabMirrorColor := clNone;
          DefaultTabMirrorColorTo := clNone;

          DownTabColor := $005C5E60;
          DownTabColorTo := $005C5E60;
          DownTabMirrorColor := clNone;
          DownTabMirrorColorTo := clNone;
          ...
          HoverTabColor := $00444648;
          HoverTabColorTo := $00444648;
          HoverTabMirrorColor := clNone;
          HoverTabMirrorColorTo := clNone;

          CollapsedHoverColor := $00444648;
          CollapsedHoverColorTo := $00444648;
          CollapsedDownColor := clNone;
          CollapsedDownColorTo := clNone;

          SplitterColor := $00303132;
          SplitterColorTo := $00303132;
          SplitterBorderColor := clNone;


          BorderColor := $00B5B5B5;

          SectionColor := $00B5B5B5;
          SectionColorTo := $00B5B5B5;
...
begin
  ...
  if FDesignTime then
  begin
//    AddPanel.Name := Name + '_1';
//    AddPanel.Name := Name + '_2';
  end;
end;
...
initialization
  Classes.RegisterClass(TAdvNavBarPanel);
