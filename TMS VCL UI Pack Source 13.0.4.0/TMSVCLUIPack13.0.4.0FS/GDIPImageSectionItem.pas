...
const
  ...
  // 1.0.0.1 : Improved : Rendering on high DPI
  // 1.0.0.2 : Fixed : Image scales when Imagelist or PictureContainer is used or it is a SVG
  // 1.0.0.3 : Fixed : Issue with CaptionSize and highDPI
...
begin
  ...
  begin
    ...
    imgr := MakeRect(ARect.X + DPIScale * Spacing, ARect.Y + (ARect.Height - imgh) / 2, imgw, imgh);
  ...
  begin
    ...
    begin
      ...
      if CaptionSize <> -1 then
        f := TGPFont.Create(ff, Round(CaptionSize * fscale), fs, UnitPoint)
      else
        f := TGPFont.Create(ff, Round(ft.Size * Trunc(fscale * 10)/10), fs, UnitPoint);
    ...
    begin
      ...
      if DescriptionSize <> -1 then
        f := TGPFont.Create(ff, Round(fscale * DescriptionSize), fs, UnitPoint)
      else
        f := TGPFont.Create(ff, Round(fscale * ft.Size), fs, UnitPoint);
