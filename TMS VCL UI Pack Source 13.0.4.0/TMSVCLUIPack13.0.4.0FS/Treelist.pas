...
uses
  ...
  StdCtrls, Comctrls, ExtCtrls, CommCtrl, Types, AdvStyleIF;
...
const
  ...
  // 1.2.0.0 : New : Color banding added
  //         : New : Row line & row line color added
  //         : New : Property FullRow added
  // 1.2.0.1 : Fixed : Memory leak with bands
  // 1.2.1.0 : Improved : Behavior on high DPI
...
type
  ...
  TTreeListBands = class(TPersistent)
  private
  ...
  private
    ...
    procedure SetBands(const Value: TTreeListBands);
  ...
  published
    ...
    property Bands: TTreeListBands read FBands write SetBands;
...
procedure TTreeList.SetFullRow(const Value: boolean);
begin
  FFullRow := Value;
end;
...
procedure TTreeList.SetBands(const Value: TTreeListBands);
begin
  FBands.Assign(Value);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FHeader.Top := Round(ATop - (FHeaderSettings.Height - 2)/FDPIScale);
      FHeader.Left := Round(ALeft/FDPIScale);
      FHeader.Width := Round(AWidth/FDPIScale);
      FHeader.Height := FHeaderSettings.Height;
      {$ELSE}
...
procedure TTreeList.BandsChanged(Sender: TObject);
begin
  Invalidate;
end;

{$IFDEF DELPHIXE10_LVL}
procedure TTreeList.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ELSE}
procedure TTreeList.ChangeScale(M, D: Integer);
{$ENDIF}
var
  i: integer;
begin
  ...
  begin
    Columns[i].Font.Height := MulDiv(Columns[i].Font.Height, M, D);
    Columns[i].Width := MulDiv(Columns[i].Width, M, D);
  end;

  HeaderSettings.Font.Height := MulDiv(HeaderSettings.Font.Height, M, D);
  HeaderSettings.Height := MulDiv(HeaderSettings.Height, M, D);

  ItemHeight := MulDiv(ItemHeight, M, D);

  if HeaderSettings.Visible then
  begin
    FHeader.Left := MulDiv(Left, D, M);
    FHeader.Top := MulDiv(Top, D, M) - FHeader.Height;
    FHeader.Width := MulDiv(Width, D, M);
  end;
end;
...
var
  ...
  defdraw: boolean;
  scpos,ItemsRight: integer;
  paintimg: boolean;
  TmpItem: TTVItem;
  code: longbool;
  i,cnt,cntvis: integer;
    ...
    CDDS_ITEMPOSTPAINT:
      begin
        ...
        begin
          ...
          begin
            ItemsRight := ItemsRight + TColumnItem(FColumnCollection.Items[i]).Width;
          end;
        end;
...
begin
  ...
  begin
    ...
    for i := 0 to FColumnCollection.Count - 1 do
    begin
      xp := xp + TColumnItem(FColumnCollection.Items[i]).Width;
...
procedure TTreeList.SetRowLineColor(const Value: TColor);
begin
...
procedure TTreeList.SetRowLines(const Value: boolean);
begin
...
procedure TTreeListBands.Assign(Source: TPersistent);
begin
  if (Source is TTreeListBands) then
  begin
    FActive := (Source as TTreeListBands).Active;
    FPrimaryColor := (Source as TTreeListBands).PrimaryColor;
    FSecondaryColor := (Source as TTreeListBands).SecondaryColor;
  end;
end;

procedure TTreeListBands.Changed;
begin
  if Assigned(OnChange) then
    OnChange(Self);
end;

constructor TTreeListBands.Create;
begin
  inherited;
  FActive := false;
...
procedure TTreeListBands.SetActive(const Value: boolean);
begin
...
procedure TTreeListBands.SetPrimaryColor(const Value: TColor);
begin
...
procedure TTreeListBands.SetSecondaryColor(const Value: TColor);
begin
