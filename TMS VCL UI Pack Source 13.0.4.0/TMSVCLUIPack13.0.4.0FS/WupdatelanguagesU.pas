...
type
  ...
  [ComponentPlatformsAttribute(pidWin32 or pidWin64)]
  {$ENDIF}
  TWebUpdateWizardArabic = class(TWebUpdateWizardLanguage)
  public
    constructor Create(AOwner: TComponent); override;
  end;
  {$ENDIF}
...
constructor TWebUpdateWizardArabic.Create(AOwner: TComponent);
begin
  inherited;
  Welcome := 'يرجى الضغط على بدء لتفحص وجود تحديثات للبرنامج';
  StartButton := 'بدء';
  NextButton := 'التالي';
  ExitButton := 'خروج';
  CancelButton := 'الغاء';
  RestartButton := 'اعادة التشغيل';
  GetUpdateButton := 'تحديث';
  NewVersionFound := 'يوجد تحديث جديد';
  NewVersion := 'تحديث جديد';
  NoNewVersionAvail := 'لا يوجد تحديث جديد';
  NewVersionAvail := 'يوجد اصدار جديد';
  CurrentVersion := 'الاصدار الحالي';
  NoFilesFound := 'لم يتم العثور على ملفات للتحديث';
  NoUpdateOnServer := 'لا يوجد تحديث على المخدم...';
  CannotConnect := 'فشل الاتصال بمخدم التحديث';
  WhatsNew := 'مالجديد ؟';
  License := 'اتفاقية الاستخدام';
  AcceptLicense := 'أوافق';
  NotAcceptLicense := 'لا أوافق';
  ComponentsAvail := 'التحديثات المتوفرة';
  DownloadingFiles := 'تحميل الملفات';
  CurrentProgress := 'تقدم الملف الحالي';
  TotalProgress := 'التقدم الكلي';
  UpdateComplete := 'تم الانتهاء من التحديث ...';
  RestartInfo := 'اضغط اعادة التشغيل لتشغيل النسخة المحدثة';
  WhatsNewPopup := 'عرض ضمن المفكرة';
  LicensePopup := 'عرض ضمن المفكرة';
  FailedDownload := 'حدث خطأ أثناء تحميل التحديثات';
end;
