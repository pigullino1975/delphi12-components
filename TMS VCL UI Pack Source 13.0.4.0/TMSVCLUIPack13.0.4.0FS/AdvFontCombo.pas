...
const
  ...
  // 1.6.0.1 : Fixed : Design-time style painting
  // 1.6.1.0 : New : ArrowColor property for dropdown button
  //         : Improved : On creation check for enabled AdvFormStyler
  // 1.6.1.1 : Improved : Combobox dropup via button click
  // 1.6.1.2 : Improved : High DPI handling for TAdvFontSelector, TAdvOfficeComboBox
...
type
  ...
  TAdvCustomComboBtn = class(TPersistent)
  private
  ...
  protected
    ...
    property HotArrowColor: TColor read FHotArrowColor write SetHotArrowColor default clBlack;
    property DownArrowColor: TColor read FDownArrowColor write SetDownArrowColor default clWhite;
  ...
  TAdvComboBtn = class(TAdvCustomComboBtn)
  published
    property ArrowColor;
    property HotArrowColor;
    property DownArrowColor;
    property Color;
    property ColorTo;
    property ColorHot;
    property ColorHotTo;
  ...
  protected
    ...
    property DeactiveClose: boolean read FDeactiveClose write FDeactiveClose;
...
var
  ...
  SecondDown: Boolean;
begin
  if (TMessage(msg).LParam = 0) and (TMessage(msg).WParam = 0) then
  begin
    FDeactiveClose := false;
    Exit;
  end;
  ...
  if FDeactiveClose and PtInRect(GetButtonRect, point(msg.xpos, msg.ypos)) then
  begin
    FDeactiveClose := false;
    Exit;
  end;
    ...
    esOffice2016Black:
      ...
      esOffice2019White:
        begin
          ...
          FButton.ColorDown := $00E3BDA3;
          FButton.ColorDownTo := $00E3BDA3;

          FButton.ColorHot := $00F2E1D5;
          FButton.ColorHotTo := $00F2E1D5;

          FSelectionColor := $00F2D5C2;
          FSelectionColorTo := $00F2D5C2;
          FSelectionTextColor := $003B3B3B;

          BorderColor := $00A3A3A3;
          BorderHotColor := $00E3BDA3;
          Color := clWhite;
          Font.Color := clWindowText;
          FButton.ArrowColor := clBlack;
        end;
      esOffice2019Gray:
        begin
          FButton.Color := $00D4D4D4;
          FButton.ColorTo := $00D4D4D4;

          FButton.ColorDown := $00666666;
          FButton.ColorDownTo := $00666666;

          FButton.ColorHot := $00969696;
          FButton.ColorHotTo := $00969696;

          FSelectionColor := $00969696;
          FSelectionColorTo := $00969696;
          FSelectionTextColor := $003B3B3B;
      ...
      esOffice2019Black:
        begin
          FButton.Color := $00444444;
          FButton.ColorTo := $00444444;

          FButton.ColorDown := $00828282;
          FButton.ColorDownTo := $00828282;

          FButton.ColorHot := $00686868;
          FButton.ColorHotTo := $00686868;

          FSelectionColor := $00505050;
          FSelectionColorTo := $00505050;
          FSelectionTextColor := clWhite;
...
procedure TAdvCustomComboBox.SetUIStyle(const Value: TTMSStyle);
begin
...
procedure TAdvCustomComboBtn.Assign(Source: TPersistent);
begin
...
procedure TAdvCustomComboBtn.Change;
begin
...
constructor TAdvCustomComboBtn.Create;
begin
  inherited;
  FColor := clBtnFace;
  FColorTo := clBtnFace;
...
destructor TAdvCustomComboBtn.Destroy;
begin
  FGlyph.Free;
  FGlyphHot.Free;
  FGlyphDown.Free;
...
procedure TAdvCustomComboBtn.SetArrowColor(const Value: TColor);
begin
...
procedure TAdvCustomComboBtn.SetColor(const Value: TColor);
begin
...
procedure TAdvCustomComboBtn.SetColorTo(const Value: TColor);
begin
...
procedure TAdvCustomComboBtn.SetDownArrowColor(const Value: TColor);
begin
...
procedure TAdvCustomComboBtn.SetColorDown(const Value: TColor);
begin
...
procedure TAdvCustomComboBtn.SetColorDownTo(const Value: TColor);
begin
...
procedure TAdvCustomComboBtn.SetGlyphDown(const Value: TBitMap);
begin
...
procedure TAdvCustomComboBtn.SetGlyph(const Value: TBitMap);
begin
  FGlyph.Assign(Value);
  Change;
end;
...
procedure TAdvCustomComboBtn.SetHotArrowColor(const Value: TColor);
begin
...
procedure TAdvCustomComboBtn.SetColorHot(const Value: TColor);
begin
...
procedure TAdvCustomComboBtn.SetColorHotTo(const Value: TColor);
begin
...
procedure TAdvCustomComboBtn.SetGlyphHot(const Value: TBitMap);
begin
...
procedure TAdvCustomComboBtn.SetWidth(const Value: integer);
begin
...
begin
  ...
  begin
    ...
    begin
      if (Owner is TAdvCustomComboBox) then
        (Owner as TAdvCustomComboBox).DeactiveClose := true;
    end;

    if HideOnDeActivate then
      Hide;

    PostMessage((Owner as TAdvCustomComboBox).Handle, WM_LBUTTONDOWN, 0, 0);
  end
  else
  begin
    ...
    begin
      if (Owner is TAdvCustomComboBox) then
        (Owner as TAdvCustomComboBox).DeactiveClose := false;
    end;
...
//   TAdvFontType = (aftBitmap, aftTrueType, aftPostScript, aftPrinter);
  ...
  FDPIScale := GetDPIScale(Self,nil);
  TListBox(Control).Canvas.Font.Height := Round(FDPIScale *   TListBox(Control).Canvas.Font.Height);
