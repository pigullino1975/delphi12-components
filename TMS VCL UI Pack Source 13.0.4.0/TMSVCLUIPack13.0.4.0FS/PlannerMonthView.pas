...
uses
  ...
  ComCtrls, Dialogs, ExtCtrls, Printers, AdvStyleIF, Planner, Forms, ImgList
  , PlanXPVS
{$IFDEF USEIMAGE}
  , AdvImage
{$ENDIF}
...
const
  ...
  BLD_VER = 0; // Build nr.
  DATE_VER = 'Apr, 2022'; // Month version
  ...
  // 3.2.0.0 : New : Support for Office 2019 styles
  // 3.2.0.1 : Fixed : Design-time creation style is now retrieved from AdvStyleIF
  // 3.2.1.0 : New : UIStyle property for office look
  //         : Improved : On creation check for enabled AdvFormStyler
  // 3.2.1.1 : Fixed : Issue with clipping for HTML formatted text
  // 3.2.1.2 : Improved : Item Post not performed on item dblclk
  // 3.2.1.3 : Fixed : Issue with OnItemEdited event
  // 3.2.1.4 : Fixed : Issue with synchronizing selected DB record
  //         : Fixed : Issue with showing balloons on overlapping items
  // 3.2.2.0 : New : XYToItem() added
  // 3.2.3.0 : New : Added support for virtual imagelists
...
type
  ...
  pmsOffice2019White, pmsOffice2019Gray, pmsOffice2019Black);
...
{$ENDIF}
    ...
    FPlannerImages: TCustomImageList;
...
{$ENDIF}
    ...
    property PlannerImages: TCustomImageList read FPlannerImages write SetImages;
    property PopupMenu;
...
begin
  ...
  if Done and ((NewStartTime <> OldStartTime) or (NewEndTime <> OldEndTime)) then
  begin
    if Assigned((Collection as TPlannerMonthViewItems).PlannerMonthView) then
...
begin
  ...
  case AStyle of
    ...
    tsOffice2019White: Style := pmsOffice2019White;
    tsOffice2019Gray: Style := pmsOffice2019Gray;
    tsOffice2019Black: Style := pmsOffice2019Black;
  end;
end;
...
begin
  ...
  pmsOffice2003Classic:
    ...
    begin
      ...
      InversBkColor := $00F0F0F0;
      InversBkColorTo := clNone;
      InversColor := $00666666;
      SelectColor := $00FAECE3;
      FocusColor := $00F2E1D5;

      TodayColor := $00FAECE3;
      TodayColorTo := $00FAECE3;
      ...
      DefaultItem.ColorTo := clWhite;
      DefaultItem.Color := clWhite;
      DefaultItem.BorderColor :=  $00ABABAB;
      DefaultItem.SelectColorTo := $00F2E1D5;
      DefaultItem.SelectColor := $00F2E1D5;
      DefaultItem.CaptionBkgTo := $00F2E1D5;
      DefaultItem.CaptionBkg := $00F2E1D5;
      DefaultItem.TrackColor := $00E3BDA3;
      DefaultItem.TrackSelectColor := $00E3BDA3;
      DefaultItem.CaptionBkgDirection := Planner.gdHorizontal;
      DefaultItem.ColorDirection := Planner.gdVertical;
      DefaultItem.Shadow := False;

      CaptionFont.Color := $00444648;
      CaptionColor := $00E3BDA3;
      CaptionColorTo := $00E3BDA3;
      DayFont.Color := $00444648;
      ShadowColor := clNone;
    end;
    pmsOffice2019Gray:
    begin
      MonthGradientStartColor := $00F0F0F0;
      MonthGradientEndColor := $00F0F0F0;
      InactiveColor := $00D4D4D4;
      InversBkColor := $00D4D4D4;
      InversBkColorTo := clNone;
      InversColor := $00444444;
      SelectColor := $00939597;
      FocusColor := $00969696;

      TodayColor := $00939597;
      TodayColorTo := $00939597;
      ...
      DefaultItem.ColorTo := $00B8BBBE;
      DefaultItem.Color := $00B8BBBE;
      DefaultItem.BorderColor :=  $00808080;
      DefaultItem.SelectColorTo := $00969696;
      DefaultItem.SelectColor := $00969696;
      DefaultItem.CaptionBkgTo := $00969696;
      DefaultItem.CaptionBkg := $00969696;
      DefaultItem.TrackColor := $00666666;
      DefaultItem.TrackSelectColor := $00666666;
      DefaultItem.CaptionBkgDirection := Planner.gdHorizontal;
      DefaultItem.ColorDirection := Planner.gdVertical;
      DefaultItem.Shadow := False;

      CaptionFont.Color := $00232425;
      CaptionColor := $0086888A;
      CaptionColorTo := $0086888A;
      DayFont.Color := $00232425;
      ShadowColor := clNone;
    end;
    pmsOffice2019Black:
    begin
      MonthGradientStartColor := $00252525;
      MonthGradientEndColor := $00252525;
      InactiveColor := $00252525;
      InversBkColor := $00252525;
      InversBkColorTo := clNone;
      InversColor := $00F1F1F1;
      SelectColor := $00444648;
      FocusColor := $00686868;

      TodayColor := $00444648;
      TodayColorTo := $00444648;
      ...
      DefaultItem.ColorTo := $003B3B3B;
      DefaultItem.Color := $003B3B3B;
      DefaultItem.BorderColor :=  $00686868;
      DefaultItem.SelectColorTo := $00505050;
      DefaultItem.SelectColor := $00505050;
      DefaultItem.CaptionBkgTo := $00505050;
      DefaultItem.CaptionBkg := $00505050;
      DefaultItem.TrackColor := $00828282;
      DefaultItem.TrackSelectColor := $00828282;
      DefaultItem.CaptionBkgDirection := Planner.gdHorizontal;
      DefaultItem.ColorDirection := Planner.gdVertical;
      DefaultItem.Shadow := False;

      CaptionFont.Color := clWhite;
      CaptionColor := $005C5E60;
      CaptionColorTo := $005C5E60;
      DayFont.Color := clWhite;
      ShadowColor := clNone;
    end;
  end;
...
function TPlannerMonthView.XYToItem(X, Y: integer): TPlannerItem;
var
  dt: TDateTime;
begin
  DateAtXY(X,Y,dt);
  Result := Items.FindItemAtDate(dt,X,Y);
end;
...
begin
  ...
  if (dt <> FToolTipPos) or (FToolTipItem <> APlannerItem) then
  begin
  ...
  begin
    ...
    begin
      ...
      if HTMLDrawEx(Canvas, APlannerItem.NotesText, R,
        ...
        , cor, CID, CV, CT, nil,
        FPlanner.PictureContainer, Handle, 1
      ) then
    ...
    begin
      ...
      begin
        ...
        if HTMLDrawEx(Canvas, APlannerItem.NotesText, tr,
          ...
          , 1.0, FPlanner.URLColor, clNone, clNone, clGray, Anchor,
          StrippedHTMLString, FocusAnchor, XSize, YSize, ml, hl, hr
          , cor, CID, CV, CT, nil, FPlanner.PictureContainer, Handle, 1) then
          begin
            FLastAnchor := '';
            if Assigned(FOnItemAnchorClick) then
...
procedure TPlannerMonthView.SetImages(const Value: TCustomImageList);
begin
  ...
  begin
    ...
    begin
      if pos('</',FPrintOptions.Header.Text) > 0 then
        ...
        , cr, CID, CV, CT, FImageCache, FContainer, Handle, 1
        )
      else
    ...
    case APlannerItem.InplaceEdit of
      peMaskEdit, peEdit:
        begin
          ...
          TPlannerEx(FPlanner).SetEditMode(True);
    ...
    begin
      ...
      begin
        ...
        if Assigned(FPlannerMonthView) then
          FPlannerMonthView.ItemUnSelected(Items[i]);
      end;
    end;
