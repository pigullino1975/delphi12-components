...
type
  ...
  TCustomPaintedForm = class(TCustomForm)
  {$IFDEF WEBLIB}
  private
    FPaintBox: TPaintBox;
  ...
  TAdvCustomPopupForm = class(TCustomPaintedForm)
  private
  ...
  public
    ...
    property FillColor: TAdvGraphicsColor read FFillColor write FFillColor default gcNull;
    property StrokeColor: TAdvGraphicsColor read FStrokeColor write FStrokeColor default gcNull;
  end;
...
begin
  ...
  begin
    ...
    begin
      if (Screen.Forms[I] is TAdvCustomPopupForm) then
      begin
        o := (Screen.Forms[I] as TAdvCustomPopupForm).Popup.Owner;
        if Assigned(o) and (o is TControl) then
        begin
          f := TAdvUtils.GetParentForm((o as TControl).Parent);
    ...
    begin
      if (Screen.CustomForms[I] is TAdvCustomPopupForm) then
      begin
        o := (Screen.CustomForms[I] as TAdvCustomPopupForm).Popup.Owner;
        f := nil;
        if Assigned(o) and (o is TControl) then
        begin
          if o.HasParent then
...
begin
  ...
  begin
    ...
    if Assigned(p) and (p is TWinControl) then
      ...
      FContentControl.Parent := TAdvUtils.GetParentForm(Self)
    {$ENDIF}
  end;
...
begin
  ...
  try
    try
      ...
      TAdvUtils.ScaleForCurrentDPI(FPopupForm);
...
function TAdvCustomPopup.GetDocURL: string;
begin
...
procedure TCustomPaintedForm.DoPaint(Sender: TObject);
begin
  Paint;
end;

procedure TCustomPaintedForm.Paint;
begin
  inherited;
...
procedure TCustomPaintedForm.CreateCanvas;
begin
...
function TCustomPaintedForm.GetCanvas: TCanvas;
begin
  CreateCanvas;
...
begin
  ...
  try
    ...
    BeginUpdate;
    {$IFDEF FMXMOBILE}
    if Assigned(FOwner) and (FOwner is TPopup) and ((FOwner as TPopup).Owner is TAdvCustomPopup) then
      if not ((FOwner as TPopup).Owner as TAdvCustomPopup).StaysOpen then
    {$ENDIF}
...
begin
  ...
  begin
    p := FOwner as TPopup;
    if p.Owner is TAdvCustomPopup then
      Result := p.Owner as TAdvCustomPopup;
  end;
  {$ENDIF}
end;
