...
function TAdvUpDownEdit.GetValue: double;
begin
  {$IFDEF DELPHIXE_LVL}
...
procedure TAdvUpDownEdit.SetValue(const Value: double);
var
  p: string;
begin
  if UpdownType = udInteger then
  begin
    Text := IntToStr(Round(Value));
  end
  else
  begin
    p := inTToStr(Precision);
    Text := Format('%.'+p+'f',[Value]);
  end;
end;
