...
type
  ...
  protected
    ...
    property BtnIns: Boolean read FBtnIns write FBtnIns;
    property BtnDel: Boolean read FBtnDel write FBtnDel;
  public
    constructor Create(Collection: TCollection); override;
    destructor Destroy; override;
  ...
  private
    ...
    FOnAfterLast: TNotifyEvent;
    FOnBeforeDelete: TNotifyEvent;
    FOnBeforePrior: TNotifyEvent;
    FOnBeforeFirst: TNotifyEvent;
    FOnAfterCancel: TNotifyEvent;
    FOnBeforeInsert: TNotifyEvent;
    FOnBeforeNext: TNotifyEvent;
    FOnBeforeLast: TNotifyEvent;
    FOnAfterDelete: TNotifyEvent;
    FOnAfterPrior: TNotifyEvent;
    FOnAfterFirst: TNotifyEvent;
    FOnAfterInsert: TNotifyEvent;
    FOnAfterNext: TNotifyEvent;
  ...
  protected
    ...
    procedure DoBeforeFirst; virtual;
    procedure DoBeforeLast; virtual;
    procedure DoBeforeNext; virtual;
    procedure DoBeforePrior; virtual;
    procedure DoBeforeInsert; virtual;
    procedure DoBeforeDelete; virtual;
    procedure DoBeforeCancel; virtual;
    procedure DoBeforeEdit; virtual;
    procedure DoBeforePost; virtual;
    procedure DoAfterFirst; virtual;
    procedure DoAfterLast; virtual;
    procedure DoAfterNext; virtual;
    procedure DoAfterPrior; virtual;
  ...
  published
    ...
    property OnBeforeFirst: TNotifyEvent read FOnBeforeFirst write FOnBeforeFirst;
    property OnBeforeLast: TNotifyEvent read FOnBeforeLast write FOnBeforeLast;
    property OnBeforeNext: TNotifyEvent read FOnBeforeNext write FOnBeforeNext;
    property OnBeforePrior: TNotifyEvent read FOnBeforePrior write FOnBeforePrior;
    ...
    property OnAfterFirst: TNotifyEvent read FOnAfterFirst write FOnAfterFirst;
    property OnAfterLast: TNotifyEvent read FOnAfterLast write FOnAfterLast;
    property OnAfterNext: TNotifyEvent read FOnAfterNext write FOnAfterNext;
    property OnAfterPrior: TNotifyEvent read FOnAfterPrior write FOnAfterPrior;
...
procedure TDBInspectorBar.DoAfterCancel;
begin
  if Assigned(OnAfterCancel) then
    OnAfterCancel(Self);
end;

procedure TDBInspectorBar.DoAfterDelete;
begin
  if Assigned(OnAfterDelete) then
    OnAfterDelete(Self);
end;

procedure TDBInspectorBar.DoAfterEdit;
begin
  if Assigned(OnAfterEdit) then
    OnAfterEdit(Self);
end;

procedure TDBInspectorBar.DoAfterFirst;
begin
  if Assigned(OnAfterFirst) then
    OnAfterFirst(Self);
end;

procedure TDBInspectorBar.DoAfterInsert;
begin
  if Assigned(OnAfterInsert) then
    OnAfterInsert(Self);
end;

procedure TDBInspectorBar.DoAfterLast;
begin
  if Assigned(OnAfterLast) then
    OnAfterLast(Self);
end;

procedure TDBInspectorBar.DoAfterNext;
begin
  if Assigned(OnAfterNext) then
    OnAfterNext(Self);
end;

procedure TDBInspectorBar.DoAfterPost;
begin
  if Assigned(OnAfterPost) then
    OnAfterPost(Self);
end;

procedure TDBInspectorBar.DoAfterPrior;
begin
  if Assigned(OnAfterPrior) then
    OnAfterPrior(Self);
end;

procedure TDBInspectorBar.DoBeforeCancel;
begin
  if Assigned(OnBeforeCancel) then
    OnBeforeCancel(Self);
end;

procedure TDBInspectorBar.DoBeforeDelete;
begin
  if Assigned(OnBeforeDelete) then
    OnBeforeDelete(Self);
end;

procedure TDBInspectorBar.DoBeforeEdit;
begin
  if Assigned(OnBeforeEdit) then
    OnBeforeEdit(Self);
end;

procedure TDBInspectorBar.DoBeforeFirst;
begin
  if Assigned(OnBeforeFirst) then
    OnBeforeFirst(Self);
end;

procedure TDBInspectorBar.DoBeforeInsert;
begin
  if Assigned(OnBeforeInsert) then
    OnBeforeInsert(Self);
end;

procedure TDBInspectorBar.DoBeforeLast;
begin
  if Assigned(OnBeforeLast) then
    OnBeforeLast(Self);
end;

procedure TDBInspectorBar.DoBeforeNext;
begin
  if Assigned(OnBeforeNext) then
    OnBeforeNext(Self);
end;

procedure TDBInspectorBar.DoBeforePost;
begin
  if Assigned(OnBeforePost) then
    OnBeforePost(Self);
end;

procedure TDBInspectorBar.DoBeforePrior;
begin
  if Assigned(OnBeforePrior) then
    OnBeforePrior(Self);
end;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        nbInsert: BtnEnabled := DBPanel.InsEnabled;
        nbDelete: BtnEnabled := DBPanel.DelEnabled;
        nbEdit: BtnEnabled := DBPanel.EditEnabled;
        nbPost: BtnEnabled := DBPanel.PostEnabled;
        nbCancel: BtnEnabled := DBPanel.PostEnabled;
        nbRefresh: BtnEnabled := DBPanel.DelEnabled;
        else
          BtnEnabled := False;
        end;
  ...
  begin
    ...
    begin
      if (x > Indent) and (x < Indent + Navigator.ButtonSize + 3) then
        ...
        nbInsert: if DBPanel.InsEnabled then
        begin
          DoBeforeInsert;
          DBPanel.DataSource.DataSet.Insert;
          DoAfterInsert;
        end;
        nbDelete: if DBPanel.DelEnabled then
        begin
          DoBeforeDelete;
        ...
        nbRefresh: if DBPanel.DelEnabled then
...
begin
  if (InspectorItem.PropertyType = ptCustom) and (InspectorItem.EditLink = nil) then
...
function TDBInspectorPanel.InsEnabled: Boolean;
begin
  Result := False;
  if not Assigned(DataSource) then
    Exit;
  ...
  Result := BtnIns and DataSource.DataSet.CanModify;
end;

function TDBInspectorPanel.DelEnabled: Boolean;
begin
  Result := False;
  if not Assigned(DataSource) then
    Exit;
  ...
  Result := BtnDel and DataSource.DataSet.CanModify;
end;
...
begin
  ...
  FPanel.BtnDel := Active and DataSet.CanModify and
    not (DataSet.BOF and DataSet.EOF);
  FPanel.BtnIns := Active and DataSet.CanModify;
  FPanel.InspectorBar.Invalidate;
end;
...
begin
  ...
  begin
    ...
    FPanel.BtnDel := CanModify and not (DataSet.BOF and DataSet.EOF);
    FPanel.BtnIns := CanModify;
