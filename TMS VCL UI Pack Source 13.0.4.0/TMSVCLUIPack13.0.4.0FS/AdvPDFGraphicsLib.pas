...
type
  ...
  {$IFDEF WEBLIB}
  TAdvPDFGraphicsLibList = class(TList)
  private
  ...
  {$ENDIF}
  {$IFNDEF WEBLIB}
  TAdvPDFGraphicsLibList = class(TList<TObject>);
  {$ENDIF}

  TAdvPDFGraphicsLibFactoryService = class(TInterfacedObject, IAdvPDFGraphicsLibService, IAdvPDFGraphicsLibGeneralService)
  private
...
begin
  ...
  begin
    if TAdvPDFLibPlatformServices.Current.SupportsPlatformService(IAdvPDFGraphicsLibService, IInterface(PDFGraphicsLibService)) then
    begin
  ...
  {$ENDIF}
  begin
    if TAdvPDFLibPlatformServices.Current.SupportsPlatformService(IAdvPDFGraphicsLibGeneralService, IInterface(PDFGraphicsLibServiceGeneral)) then
    begin
...
{$IFDEF WEBLIB}
function TAdvPDFGraphicsLibList.GetItem(Index: Integer): TObject;
begin
  Result := TObject(inherited Items[Index]);
end;

procedure TAdvPDFGraphicsLibList.SetItem(Index: Integer; const Value: TObject);
begin
