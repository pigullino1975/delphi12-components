...
const
  ...
  // 2.5.0.5 : Fixed : Disabled text color in Office 2016 white style
  // 2.6.0.0 : New : Office 2019 styles added (change dropdown arrow design)
  //         : New : StyleElements property added
  //		     : Fixed : Dropdown width now changes on high DPI
  //         : Fixed : Issue with down state while mouse leave
  // 2.6.0.1 : Fixed : Use of TextColorChecked when working with groups
  //         : Fixed : Design-time style painting
  //         : Fixed : Issue with turning off VCL Styles
  // 2.6.0.2 : Fixed : Office 2019 style was not only forced on creation
  //         : Fixed : Design-time set of Font.Color
  //         : Fixed : Design-time copy & paste handling
  // 2.6.0.3 : Fixed : Issue with switching between VCL styles & TMS styles at runtime
  // 2.6.1.0 : New : UIStyle property for office look
  //         : Improved : On creation check for enabled AdvFormStyler
  // 2.6.1.1 : Fixed : Regression with copy & paste control at design-time
  // 2.7.0.0 : New : DropDownWidth property added
  //         : Fixed : Regression with copy & paste control at design-time
  // 2.7.0.1 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // 2.7.0.2 : Fixed : Glyph location in left and right layout with notes longer than caption
  // 2.7.0.3 : Fixed : Added check to forced paint when ButtonSizeState was set
  // 2.7.0.4 : Fixed : Regression with using Notes rectangle when no Notes are used
  // 2.7.0.5 : Fixed : Issue with setting Default at runtime
  // 2.7.0.6 : Fixed : Issue with MarginHorz setting for blGlyphRightAdjusted layout
  // 2.7.0.7 : Fixed : Issue with RDP reconnect when no VCL styles are used
  // 2.7.0.8 : Fixed : Issue with DropDownWidth handling
  // 2.7.1.0 : New : PictureWidth, PictureHeight properties to control size of SVG on button
  // 2.7.2.0 : New : With PictureWidth = 0, PictureHeight = 0, picture automatically sizes
  // 2.7.2.1 : Fixed : Issue with transparent icon drawing in hover state
  // 2.7.2.2 : Fixed : Regression with hover state painting
  // 2.7.2.3 : Fixed : Issue with text down color in combination with VCL styles
  // 2.7.2.4 : Fixed : Issue with Notes font & high DPI
  // 2.7.2.5 : Improved : High DPI handling with Delphi 11
  // 2.7.2.6 : Fixed : Issue with setting Appearance.TranspHover = true
  // 2.7.3.0 : New : Property AutoSizeDirection added
  // 2.7.3.1 : Fixed : Issue with ParentFont = true and font handling in high DPI
  // 2.7.3.2 : Fixed : Hover drawing of dropdown button in specific modes
  // 2.7.3.3 : Fixed : Issue with VCL Style Windows10 on Delphi 11
  // 2.7.3.4 : Fixed : Issue with hovering background color in specific Office styles on toolbar
  // 2.7.3.5 : Fixed : Issue with drawinbg disabled imagelist images on Win64
  // 2.8.0.0 : New : ShowPip, PipColor properties added to show small notification pip on button
  //         : Fixed : Issue with autosizing for Layout blGlyphTopAdjusted
...
type
  ...
  protected
    procedure Changed;
    property TranspOffice: boolean read FTranspOffice write FTranspOffice;
  public
    constructor Create;
    procedure Init;
  ...
  TAutoSizeDirection = (asVertical, asHorizontal);
  ...
  private
    ...
    FSpacing: Integer;
    FAutoSize: Boolean;
    FAutoSizeDirection: TAutoSizeDirection;
    FWordWrap: Boolean;
    FDoAutoSize: Boolean;
    FFirstPaint: Boolean;
    FMarginVert: integer;
    ...
    procedure SetIsRotated(const Value: boolean);
    procedure SetAutoSizeDirection(const Value: TAutoSizeDirection);
    procedure SetPipColor(const Value: TColor);
    procedure SetShowPip(const Value: Boolean);
  protected
    FHot: Boolean;
    ...
    property DropDownWidth: integer read FDropDownWidth write SetDropDownWidth default DropDownSectWidth;
    ...
    function GetDropDownBtnWidth: integer; virtual;
    ...
    {$ENDIF}
    {$ENDIF}
    property IsDesignTime: boolean read FDesignTime write FDesignTime;
  public
    ...
    property IsRotated: boolean read FIsRotated write SetIsRotated;
  ...
  published
    ...
    property AutoSizeDirection: TAutoSizeDirection read FAutoSizeDirection write SetAutoSizeDirection default asVertical;
    ...
    property PictureHeight: integer read FPictureHeight write SetPictureHeight default -1;
    property PictureWidth: integer read FPictureWidth write SetPictureWidth default -1;
    property PipColor: TColor read FPipColor write SetPipColor default clRed;
    property PopupMenu;
    ...
    property ShowPip: Boolean read FShowPip write SetShowPip default false;
...
procedure DrawOpenRoundRectTop(graphics: TGPGraphics; PC:TColor; X,Y,Width,Height,Radius: integer);
var
  path:TGPGraphicsPath;
  gppen:TGPPen;
begin
  ...
  if IsWin7 then
    path.AddLine(X, Y + Height - 2, X + Width , Y + Height - 2)
  else
...
procedure DrawOpenRoundRectBottom(graphics: TGPGraphics; PC: TColor; X,Y,Width,Height,Radius: integer;Hot: boolean);
var
  path:TGPGraphicsPath;
  gppen:TGPPen;
begin
  ...
  path.AddLine(X, Y + height - Radius , X , Y);
  ...
  begin
    ...
    // 3D color effect
    gppen := tgppen.Create(ColorToARGB(BrightnessColor(clwhite,-10)),1);
    path.AddLine(X, Y + 2, X + Width, Y - 2);
    graphics.DrawPath(gppen, path);
    gppen.Free;
    path.Free;
  end;
...
//  i, s: integer;
  ...
  ArDPI: single;
begin
  ArDPI := GetDPIScale(AControl, ACanvas);
  FArrSize := Round(ArDPI * 2);
  ArP.X := ArP.X + 1 + Round(ArDPI) - FArrSize div 2;
  ArP.Y := ArP.Y + 1 - FArrSize div 2;

  ACanvas.Pen.Color := ArClr;

  g := TGPGraphics.Create(ACanvas.Handle);
  pen := TGPPen.Create(MakeColor(255, ArClr), ArDPI + 0.4 * ArDPI);
  path := TGPGraphicsPath.Create;
  ...
  ddDown:
    begin
      Path.AddLine(ArP.X - (ArDPI * 3), ArP.Y - (ArDPI * 2), ArP.X, ArP.Y + ArDPI);
      Path.AddLine(ArP.X, ArP.Y + ArDPI, ArP.X + (ArDPI * 3) , ArP.Y - (ArDPI * 2));
  ...
  ddRight:
    begin
      Path.AddLine(ArP.X - (ArDPI * 2), ArP.Y - (ArDPI * 3), ArP.X + ArDPI, ArP.Y );
      Path.AddLine(ArP.X + ArDPI, ArP.Y, ArP.X - (ArDPI * 2) , ArP.Y + (ArDPI * 3));
      {* for I := 0 to FArrSize - 1 do
      begin
        ACanvas.MoveTo(ArP.X + I, ArP.Y + I);
  ...
  ddUp:
    begin
      Path.AddLine(ArP.X - (ArDPI * 3), ArP.Y + ArDPI , ArP.X, ArP.Y - (ArDPI * 2));
      Path.AddLine(ArP.X,  ArP.Y - (ArDPI * 2), ArP.X + (ArDPI * 3), ArP.Y + ArDPI);
      {* for I := 0 to FArrSize - 1 do
      begin
        s := FArrSize - 1 - I;
...
begin
  ...
  if Pic.HasSVG then
  begin
    Pic.DrawSVG(graphics, P.X, P.Y, SmartMin(mw,Pic.Width), SmartMin(mh,Pic.Height));
    Exit;
  end;
  {$ENDIF}
...
function DrawVistaButton(Owner: TControl; Canvas: TCanvas; r: TRect; CFU, CTU, CFB, CTB, PC, PCF: TColor;
   ...
   Trimming: TStringTrimming; Notes: TStringList; NotesFont: TFont;Checked, Down, TranspHover, GDIPArrow: boolean; MargH,MargV: integer; NoDefBorder: boolean; AlwaysAccel: boolean; Frm: TCustomForm; ADropDownWidth: integer; Rotated: boolean; PipColor: TColor): TSize;
var
  graphics : TGPGraphics;
  w,h,h2,h2d: Integer;
  fontFamily,nfontFamily: TGPFontFamily;
  font,nfont: TGPFont;
  rectf, mrectf, outrect: TGPRectF;
  stringFormat: TGPStringFormat;
  solidBrush,nsolidBrush,pipBrush: TGPSolidBrush;
  x1,y1,x2,y2: single;
  fs,nfs: integer;
  sizerect: TGPRectF;
  noterect: TGPRectF;
  ImgX, ImgY, ImgW, ImgH, DrwX,DrwY: Integer;
  BtnR, DwR: TRect;
  BR1,BR2: TRect;
  DR1,DR2: TRect;
  AP: TPoint;
...
begin
  ...
  FScale := GetDPIScale(frm, frm.Canvas);

  FDropDownSectWidth := Round(ADropDownWidth * FScale);
...
//    Layout := blGlyphLeftAdjusted;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if not TranspHover or DropDownSplit then
        begin
  ...
  begin
    ...
    bpLeft:
      begin
        if not Rotated then
          DrawOpenRoundRectLeft(Graphics, PC, r.Left, r.Top, r.Right - 1, r.Bottom - 1, Radius)
        else
          DrawOpenRoundRectTop(Graphics, PC, r.Left, r.Top, r.Right - 1, r.Bottom - 1, Radius);
      end;
    bpRight:
      begin
        if not Rotated then
          DrawOpenRoundRectRight(Graphics, PC, r.Left, r.Top, r.Right - 1, r.Bottom - 1, Radius, Hot or Checked)
        else
          DrawOpenRoundRectBottom(Graphics, PC, r.Left, r.Top, r.Right - 1, r.Bottom - 1, Radius, Hot or Checked);
      end;
    bpMiddle:
      begin
        DrawOpenRoundRectMiddle(Graphics, PC, r.Left, r.Top, r.Right - 1, r.Bottom - 1, Radius, Hot or Checked)
      end;
    end;
  end;
  ...
  if (Layout = blGlyphRightAdjusted) and not AutoSize then
  begin
    r.Right := r.Right - MargH;
    BtnR.Right := BtnR.Right - MargH;
  end;

  if (Layout = blGlyphTopAdjusted) and not AutoSize then
    r.Top := r.Top + MargV;

  if (Layout = blGlyphBottomAdjusted) and not AutoSize then
  begin
    r.Bottom := r.Bottom - MargV;
    BtnR.Bottom := BtnR.Bottom - MargV;
  end;
  ...
  begin
    ...
    if (ForcePicSize.CX > 0) and (ForcePicSize.CY > 0) then
    begin
      ImgW := Round(ForcePicSize.CX * FScale);
      ImgH := Round(ForcePicSize.CY * FScale);
    end
    else
    begin
      ImgW := Picture.Width;
  ...
  begin
    ...
    begin
      ...
      case Layout of
        ...
        begin
          Result.cx := Max(ImgW + Spacing, Spacing + Round(Max(noterect.Width , sizerect.Width)));
          Result.cy := ImgH + {Spacing +} Round(sizerect.Height);
        end;
      end;
    end;
...
//      if (ImgW > 0) then
        ...
        case Layout of
          ...
          begin
            ...
            begin
              ...
              x2 := w - 2 - ImgW;
              if Notes.Text <> '' then
                ImgX := round(Min(noterect.X, sizerect.X) - ImgW div 2)
              else
          ...
          begin
            ...
            begin
              if Notes.Text <> '' then
                ImgX := round(X2 - Max(noterect.Width, sizerect.width))
              else
              ...
              if Notes.Text <> '' then
                ImgX := ImgX + round(Max(noterect.Width, sizerect.width)) + 4
              else
    ...
    if PipColor <> clNone then
    begin
      graphics.MeasureString(Caption, Length(Caption), font, rectf, stringformat, outrect);
      pipBrush := TGPSolidBrush.Create(ColorToARGB(PipColor));
      graphics.FillEllipse(pipBrush, outrect.X + outrect.Width - 3 * FScale, outrect.Y - 3 * FScale, 6* FScale, 6* FScale);
      pipBrush.Free;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            DrwX := px;
            DrwY := py;
            Canvas.StretchDraw(Rect(DrwX, DrwY, px + ForcePicSize.CX, py + ForcePicSize.CY), Glyph);
          end
          else
            Canvas.StretchDraw(Rect(DrwX, DrwY, ImgX + ForcePicSize.CX, ImgY + ForcePicSize.CY), Glyph);
        end
        else
        begin
          if (Caption = '') and (WideCaption = '') then
          begin
            DrwX := r.Left + Max(0, (w - ImgW) div 2);
            DrwY := r.Top + Max(0, (h - ImgH) div 2);
            Canvas.Draw(DrwX, DrwY, Glyph)
          end
          else
            Canvas.Draw(DrwX, DrwY, Glyph);
        end;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              DrawStretchPicture(graphics, Canvas, Rect(DrwX, DrwY, px + ForcePicSize.CX, py + ForcePicSize.CY), Picture);
            end
            else
            begin
              //Canvas.StretchDraw(Rect(ImgX, ImgY, ImgX + ForcePicSize.CX, ImgY + ForcePicSize.CY), Picture);
              DrawStretchPicture(graphics, Canvas, Rect(DrwX, DrwY, DrwX + ForcePicSize.CX, DrwY + ForcePicSize.CY), Picture);
            end;
          end
          else
          begin
            ...
            begin
              ...
              if not Assigned(Icon) then
                DrawGDIPImage(graphics, Point(DrwX, DrwY), Picture, w, w)
              else
              begin
                DrawIconEx(Canvas.Handle, DrwX, DrwY, Icon.Handle, ImgW, ImgH, 0, 0, DI_NORMAL);
              end;
            end
            else
              DrawGDIPImage(graphics, Point(DrwX, DrwY), Picture, w, h);
          end;
        end
        else
          if (ImageIndex <> -1) and Assigned(Images) then
          begin
            if (Caption = '') and (WideCaption = '') then
            begin
              DrwX := r.Left + Max(0, (w - Images.Width) div 2);
              DrwY := r.Top + Max(0, (h - Images.Height) div 2);
              ...
              if Assigned(Owner) and Assigned(Owner.Parent) and (Owner.Parent.ClassName = 'TAdvQuickAccessToolBar') and (GDIPArrow or (IsGlass and IsWin7)) then
                DrawGDIPImageFromImageList(graphics, Point(DrwX, DrwY), Images, ImageIndex, EnabledImage)
              else
              begin
                {$IFDEF DELPHI2007_LVL}
                if not EnabledImage and not Assigned(DisabledImages) and IsVista and TMSDISABLEWITHCOLORSATURATION then
                begin
                  DrawDisabledImage(Canvas.Handle, Images, ImageIndex, DrwX, DrwY)
                end
                else
                {$ENDIF}
                begin
                  if not EnabledImage and Assigned(DisabledImages) then
                    DisabledImages.Draw(Canvas, DrwX, DrwY, ImageIndex, true)
                  else
                    Images.Draw(Canvas, DrwX, DrwY, ImageIndex, EnabledImage);
                end;
              end;
            end
            else
            begin
              if Assigned(Owner) and Assigned(Owner.Parent) and (Owner.Parent.ClassName = 'TAdvQuickAccessToolBar') and (GDIPArrow or (IsGlass and IsWin7)) then
                DrawGDIPImageFromImageList(graphics, Point(DrwX, DrwY), Images, ImageIndex, True{EnabledImage})
              else
              begin
                {$IFDEF DELPHI2007_LVL}
                if not EnabledImage and not Assigned(DisabledImages) and IsVista and TMSDISABLEWITHCOLORSATURATION then
      ...
      if (Caption = '') and (PipColor <> clNone) then
      begin
        pipBrush := TGPSolidBrush.Create(ColorToARGB(PipColor));
        graphics.FillEllipse(pipBrush, DrwX + ImgW - 3 * FScale, DrwY - 3 * FScale, 6* FScale, 6* FScale);
        pipBrush.Free;
      end;
    end;
...
begin
  ...
  FGotDownKey := false;
  FDropDownWidth := DropDownSectWidth;
...
begin
  ...
  begin
    ...
    // this is the default streamed value
    Rounded := true;
    FTMSStyle := tsCustom;
  end;

  {$IFDEF DELPHIXE2_LVL}
  FUsingStyles := CheckVCLStylesEnabled(StyleServices, (csDesigning in ComponentState));
  {$ENDIF}
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ct.Selected.TextColor := ct.Hover.TextColor;
        SetColorTones(ct);
      end;
    end;
  end;
...
begin
  ...
  FDropDownSectWidth := GetDropDownBtnWidth;
...
begin
  ...
  if Parent is TAdvCustomToolBar then
    LTranspHover := false
  else
    LTranspHover := Appearance.FTranspHover;
  ...
  try
    if FTransparent and (LTranspHover or (not FMouseEnter or StaticButton)) then
    begin
      // TRANSPARENCY CODE
      R := ClientRect;
  ...
  //            PenColor := BlendColor(BorderColorDown, BorderColorChecked, FStepPush);
      ...
      if ((Style = bsCheck) and (FState = absDown)) or ((FState = absExclusive) and (FGroupIndex <> 0)) then
      ...
      begin
        szrect := drrect;
        if AutoSizeDirection = asVertical then
          szrect.Bottom := szrect.Top + 4096
        else
          szrect.Right := szrect.Right + 4096;

        if Assigned(Picture) and not Picture.Empty and (PictureHeight > 0) and (PictureWidth > 0) then
        begin
          PicSize.cx := PictureWidth;
          PicSize.cy := PictureHeight;
        end;

        if Assigned(Picture) and not Picture.Empty and (PictureHeight = 0) and (PictureWidth = 0)  then
        begin
          PicSize.cy := (Height - PICGAP);

          Picture.GetImageSizes;
          if Picture.Height > 0 then
             PicSize.cx := Round((Height - PICGAP) / Picture.Height * Picture.Width)
          else
        ...
        sz := DrawVistaButton(Self, Canvas, drrect, GradColor, GradColorTo, GradColorMirror,GradColorMirrorTo,
          ...
          FStaticButton or Transparent and not ((FMouseEnter and not LTranspHover) or DrawFocusedHot or (State = absDown)), FMouseEnter, Position, DropDownSplit, CanDrawBorder,
          FOverlappedText, FWordWrap, True, FRounded, FDropDownDirection,
          FSpacing, FTrimming, FNotes, FNotesFont, FDownChecked, FDroppedDown, TranspOffice and Transparent, InternalIsOnGlassQAT, MarginHorz, MarginVert, Appearance.NoBorderDefault, AlwaysShowAccel, GetParentForm(Self), DropDownWidth, IsRotated, PipCo);
      ...
      if Assigned(Picture.Graphic) and not Picture.Empty and (PictureHeight = 0) and (PictureWidth = 0)  then
      begin
        PicSize.cy := (Height - PICGAP);
      ...
      sz := DrawVistaButton(Self, Canvas, drrect, GradColor, GradColorTo, GradColorMirror, GradColorMirrorTo,
        PenColor, PenColorFocused, GradU, GradB, DCaption, DWideCaption, FDefaultCaptionDrawing, AFont, ImgList, FDisabledImages, ImageIndex, EnabledImg, Layout, FDropDownButton {and (Style <> bsCheck)},
        DrawDwLn, Enabled, DrawFocused, DropDownPosition, Pic, FIcon, PicSize, AntiAlias, FDefaultPicDrawing, bmp, BD, FStaticButton or Transparent and not ((FMouseEnter and not LTranspHover) or DrawFocusedHot or (State = absDown)), FMouseEnter, Position, DropDownSplit, CanDrawBorder, FOverlappedText, FWordWrap,
        False, FRounded, FDropDownDirection, FSpacing, FTrimming, FNotes, FNotesFont, FDownChecked, FDroppedDown, TranspOffice and Transparent, InternalIsOnGlassQAT, MarginHorz,MarginVert,Appearance.NoBorderDefault, AlwaysShowAccel, GetParentForm(Self), DropDownWidth, IsRotated, PipCo);
      DrawGlyphCaption;
...
procedure TAdvCustomGlowButton.SetIsRotated(const Value: boolean);
begin
  FIsRotated := Value;
end;
...
procedure TAdvCustomGlowButton.SetAutoSizeDirection(
  const Value: TAutoSizeDirection);
begin
  if (FAutoSizeDirection <> Value) then
  begin
    FAutoSizeDirection := Value;
    PerformResize;
  end;
end;
...
procedure TAdvCustomGlowButton.SetShowPip(const Value: Boolean);
begin
  if (FShowPip <> Value) then
  begin
    FShowPip := Value;
    Invalidate;
  end;
...
begin
  ...
  begin
    if Assigned(Parent) and (Parent is TAdvToolBar) and Assigned((Parent as TAdvToolBar).ToolBarStyler) then
    begin
      Appearance.Assign((Parent as TAdvToolBar).ToolBarStyler.GlowButtonAppearance);
    end
    else
    begin
      if FUsingStyles then
      begin
        FUsingStyles := false;
        Appearance.Init;
...
procedure TAdvCustomGlowButton.SetUIStyle(const Value: TTMSStyle);
begin
...
procedure TAdvCustomGlowButton.SetPictureHeight(const Value: integer);
begin
...
procedure TAdvCustomGlowButton.SetPictureWidth(const Value: integer);
begin
...
procedure TAdvCustomGlowButton.SetPipColor(const Value: TColor);
begin
  if (FPipColor <> Value) then
  begin
    FPipColor := Value;
    Invalidate;
  end;
...
procedure TAdvCustomGlowButton.SetDropDownWidth(const Value: integer);
begin
...
begin
  ...
  Appearance.ColorMirrorTo := ATones.Background.BrushColor;
  Appearance.ColorHot := ATones.Hover.BrushColor;
  Appearance.ColorHotTo := ATones.Hover.BrushColor;
  Appearance.ColorMirrorHot := ATones.Hover.BrushColor;
  Appearance.ColorMirrorHotTo := ATones.Hover.BrushColor;
  Appearance.BorderColorHot := ATones.Hover.BorderColor;
  Appearance.ColorDown := ATones.Selected.BrushColor;
  Appearance.ColorDownTo := ATones.Selected.BrushColor;
  Appearance.ColorMirrorDown := ATones.Selected.BrushColor;
  Appearance.ColorMirrorDownTo := ATones.Selected.BrushColor;
  ...
  Appearance.ColorCheckedTo := ATones.Selected.BrushColor;
  Appearance.ColorMirrorChecked := ATones.Selected.BrushColor;
  Appearance.ColorMirrorCheckedTo := ATones.Selected.BrushColor;
  ...
  Appearance.TextColorChecked := ATones.Selected.TextColor;
  Appearance.TextColor := ATones.Background.TextColor;

  if ATones.Disabled.TextColor = ATones.Disabled.BrushColor then
    Appearance.TextColorDisabled := clGray
  else
    Appearance.TextColorDisabled := ATones.Disabled.TextColor;
  ...
  begin
    ...
    if not FLoaded then
      Font.Height := Round(-11 * FScaleFactor);
    {$ENDIF}
...
begin
  ...
  begin
    {$IFDEF DELPHIXE14_LVL}
    if not FLoaded and not (csLoading in ComponentState) then
      Font.Height := Round(-11 * FScaleFactor);
    {$ENDIF}
    Font.Color := clWindowText;
  end;
  ...
  Appearance.TranspOffice := (AStyle in [tsOffice2010Blue, tsOffice2010Silver, tsOffice2010Black,
  ...
  case AStyle of
    ...
    tsWhidbey:
      ...
      begin
        ...
        Appearance.BorderColor := $00ADB0B3;
        Appearance.Gradient := ggVertical;
        Appearance.GradientMirror := ggVertical;

        Appearance.ColorHot := $00FAECE3;
        Appearance.ColorHotTo := $00FAECE3;
        Appearance.ColorMirrorHot := $00FAECE3;
        Appearance.ColorMirrorHotTo := $00FAECE3;
        Appearance.BorderColorHot := $00B0784A;
        Appearance.GradientHot := ggVertical;
        Appearance.GradientMirrorHot := ggVertical;

        Appearance.ColorDown := $00B0784A;
        Appearance.ColorDownTo := $00B0784A;
        Appearance.ColorMirrorDown := $00B0784A;
        Appearance.ColorMirrorDownTo := $00B0784A;
        Appearance.BorderColorDown := $00B0784A;
        Appearance.GradientDown := ggVertical;
        Appearance.GradientMirrorDown := ggVertical;

        Appearance.ColorChecked := $00D1B9A5;
        Appearance.ColorCheckedTo := $00D1B9A5;
        Appearance.ColorMirrorChecked := $00D1B9A5;
        Appearance.ColorMirrorCheckedTo := $00D1B9A5;
        Appearance.BorderColorChecked := $00D1B9A5;
        Appearance.GradientChecked := ggVertical;
        Appearance.GradientMirrorChecked := ggVertical;
        ...
        Appearance.BorderColorCheckedHot := $00B0784A;

        // Ensure ParentFont remains true
        // Font.Color := $00444648;
        Appearance.TextColorChecked := $00393A3B;
        Appearance.TextColorHot := $00232425;
        Appearance.TextColorDown := $00232425;
        Appearance.TextColorDisabled := $00D4D4D4;
      end;

      tsOffice2019Gray:
      Begin
        Appearance.Color := $00B8BBBE;
        Appearance.ColorTo := $00B8BBBE;
        Appearance.ColorMirror := $00B8BBBE;
        Appearance.ColorMirrorTo := $00B8BBBE;
        Appearance.BorderColor := $00444648;
        Appearance.Gradient := ggVertical;
        Appearance.GradientMirror := ggVertical;

        Appearance.ColorHot := $00939597;
        Appearance.ColorHotTo := $00939597;
        Appearance.ColorMirrorHot := $00939597;
        Appearance.ColorMirrorHotTo := $00939597;
        Appearance.BorderColorHot := $0086888A;
        Appearance.GradientHot := ggVertical;
        Appearance.GradientMirrorHot := ggVertical;

        Appearance.ColorDown := $0086888A;
        Appearance.ColorDownTo := $0086888A;
        Appearance.ColorMirrorDown := $0086888A;
        Appearance.ColorMirrorDownTo := $0086888A;
        Appearance.BorderColorDown := $0086888A;
        Appearance.GradientDown := ggVertical;
        Appearance.GradientMirrorDown := ggVertical;

        Appearance.ColorChecked := $00939597;
        Appearance.ColorCheckedTo := $00939597;
        Appearance.ColorMirrorChecked := $00939597;
        Appearance.ColorMirrorCheckedTo := $00939597;
        Appearance.BorderColorChecked := $00939597;
        Appearance.GradientChecked := ggVertical;
        Appearance.GradientMirrorChecked := ggVertical;

        Appearance.ColorDisabled := $00B8BBBE;
        Appearance.ColorDisabledTo := $00B8BBBE;
        Appearance.ColorMirrorDisabled := $00B8BBBE;
        Appearance.ColorMirrorDisabledTo := $00B8BBBE;
        Appearance.BorderColorDisabled := $00D4D4D4;

        Appearance.BorderColorCheckedHot := $005C5E60;

        Font.Color := $00232425;
        Appearance.TextColorChecked := $00232425;
        Appearance.TextColorHot := $00232425;
        Appearance.TextColorDown := $00232425;
        Appearance.TextColorDisabled := $00D4D4D4;
      End;
      ...
      tsOffice2019Black:
      Begin
        Appearance.Color := $00303132;
        Appearance.ColorTo := $00303132;
        Appearance.ColorMirror := $00303132;
        Appearance.ColorMirrorTo := $00303132;
        Appearance.BorderColor := $00666666;
        Appearance.Gradient := ggVertical;
        Appearance.GradientMirror := ggVertical;

        Appearance.ColorHot := $00444648;
        Appearance.ColorHotTo := $00444648;
        Appearance.ColorMirrorHot := $00444648;
        Appearance.ColorMirrorHotTo := $00444648;
        Appearance.BorderColorHot := $005C5E60;
        Appearance.GradientHot := ggVertical;
        Appearance.GradientMirrorHot := ggVertical;

        Appearance.ColorDown := $005C5E60;
        Appearance.ColorDownTo := $005C5E60;
        Appearance.ColorMirrorDown := $005C5E60;
        Appearance.ColorMirrorDownTo := $005C5E60;
        Appearance.BorderColorDown := $005C5E60;
        Appearance.GradientDown := ggVertical;
        Appearance.GradientMirrorDown := ggVertical;

        Appearance.ColorChecked := $00444648;
        Appearance.ColorCheckedTo := $00444648;
        Appearance.ColorMirrorChecked := $00444648;
        Appearance.ColorMirrorCheckedTo := $00444648;
        Appearance.BorderColorChecked := $00444648;
        Appearance.GradientChecked := ggVertical;
        Appearance.GradientMirrorChecked := ggVertical;

        Appearance.ColorDisabled := $00303132;
        Appearance.ColorDisabledTo := $00303132;
        Appearance.ColorMirrorDisabled := $00303132;
        Appearance.ColorMirrorDisabledTo := $00303132;
        Appearance.BorderColorDisabled := $00505050;

        Appearance.BorderColorCheckedHot := $0086888A;

        Font.Color := $00FFFFFF;
        Appearance.TextColorChecked := $00FFFFFF;
        Appearance.TextColorHot := $00FFFFFF;
        Appearance.TextColorDown := $00FFFFFF;
        Appearance.TextColorDisabled := $00505050;
      end;
  end;
...
procedure TAdvCustomGlowButton.SetButtonSizeState(const Value: TButtonSizeState);
begin
...
procedure TAdvCustomGlowButton.SetMaxButtonSizeState(const Value: TButtonSizeState);
begin
  ...
  with Appearance do
    Result := DrawVistaButton(Self, Canvas,ClientRect,FColor, FColorTo, FColorMirror, FColorMirrorTo,
      BorderColor, BorderColorFocused, Gradient, GradientMirror, DCaption, DWideCaption, FDefaultCaptionDrawing, Font, ImgList, FDisabledImages, ImageIndex, EnabledImg, LayOt, FDropDownButton,
        DrawDwLn, Enabled, DrawFocused, DpDwPosition, Pic, FIcon, PicSize, AntiAlias, FDefaultPicDrawing, bmp, BD, FStaticButton or Transparent and not ((FMouseEnter and not TranspHover) or DrawFocusedHot or (State = absDown)), FMouseEnter, Position, DropDownSplit, CanDrawBorder,
        FOverlappedText, FWordWrap, True, FRounded, FDropDownDirection, FSpacing, FTrimming, FNotes, FNotesFont, FDownChecked, FDroppedDown, TranspOffice and Transparent, InternalIsOnGlassQAT,MarginHorz,MarginVert,NoBorderDefault, AlwaysShowAccel, GetParentForm(Self), DropDownWidth, IsRotated, clNone);
...
function TAdvCustomGlowButton.GetDropDownBtnWidth: integer;
begin
  Result := Round(DropDownWidth * CalcDPIScale(Canvas));
end;
...
procedure TGlowButtonAppearance.Init;
begin
  Color := clWhite;
  ColorTo := clWhite;
...
begin
  ...
  begin
    ...
    TranspOffice := (Source as TGlowButtonAppearance).TranspOffice;
