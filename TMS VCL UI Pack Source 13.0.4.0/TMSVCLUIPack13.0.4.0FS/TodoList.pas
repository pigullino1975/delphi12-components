...
{$ENDIF}
  ...
  BLD_VER = 0; // Build nr.
  DATE_VER = 'Oct, 2022'; // Month version
  ...
  // 1.5.6.0 : New : Per monitor high DPI support added
  // 1.6.0.0 : New : Support for Office 2019 styles
  // 1.6.0.1 : Improved : High DPI handling for header resizing
  //         : Improved : High DPI handling for inplace editors
  // 1.6.0.2 : Improved : High DPI handling for completion interaction
  //         : Fixed : Design-time style painting
  // 1.6.0.3 : Improved : On creation check for enabled AdvFormStyler
  // 1.6.0.4 : Fixed : Issue with rendering on high DPI
  // 1.6.1.0 : New : HeaderFlat: boolean property added
  // 1.6.2.0 : New : Support for TVirtualImageList added
...
type
  ...
  TTodoListStyle = (esOffice2003Blue, esOffice2003Silver, esOffice2003Olive, esOffice2003Classic, esOffice2007Luna, esOffice2007Obsidian, esWindowsXP, esWhidbey, esCustom, esOffice2007Silver,
                    ...
                    esOffice2019White, esOffice2019Gray, esOffice2019Black);
...
{$ENDIF}
    ...
    FPriorityImageList: TCustomImageList;
...
//    procedure WMPaint(var Message: TWMPaint); message WM_PAINT;
    ...
    procedure ColItemRect(ColIdx, ItemIdx: Integer; var R: TRect; AScale: single);
  ...
  public
    ...
    procedure EndUpdate;
    property EditorInteger: TInplaceSpinEdit read IntegerEditor;
    property EditorString: TInplaceEdit read StringEditor;
    property EditorNumeric: TInplaceEdit read FNumericOnlyEditor;
    property EditorMemo: TInplaceMemo read StringListEditor;
{$IFDEF USE_PLANNERDATEPICKER}
    property EditorPlannerDate: TPlannerDatePicker read PlannerDateEditor;
{$ENDIF}
    property EditorDate: TTodoDateTimePicker read DefaultDateEditor;
    property EditorFloat: TInplaceFloatSpinEdit read FFloatEditor;
    property EditorPriority: TInplaceODListBox read PriorityEditor;
    property EditorStatus: TInplaceListBox read StatusEditor;
  published
...
{$ENDIF}
    ...
    procedure SetHeaderFlat(const Value: boolean);
...
{$ENDIF}
    ...
    property HeaderFlat: boolean read GetHeaderFlat write SetHeaderFlat;
...
procedure DrawGauge(Canvas: TCanvas; R: TRect; Position: Integer; Settings: TGaugeSettings; ADPIScale: single = 1);
...
procedure DrawGauge(Canvas: TCanvas; R: TRect; Position: Integer;
  Settings: TGaugeSettings; ADPIScale: single = 1);
var
  RectL: TRect;
  RectM: TRect;
  RectR: TRect;
...
{$ENDIF}
        ...
        Canvas.Font.Height := Round(ADPIScale * tf.Height);
...
begin
  ...
  begin
    ...
    R.Bottom := R.Top + Round(FDPIScale * FOwner.ItemHeight) - 1;
    ...
    begin
      R.Right := R.Left + Round(FDPIScale * TodoColumns.Items[ColIdx].Width);
      ...
      begin
        ...
        r.Right := Round(FDPIScale * GetControlWidth);
      end;
...
procedure TTodoListBox.ColItemRect(ColIdx, ItemIdx: Integer; var R: TRect; AScale: single);
var
  j: Integer;
begin
  SendMessage(Handle, LB_GETITEMRECT, ItemIdx, LParam(@R));

  R.Bottom := R.Top + Round(AScale * FOwner.ItemHeight) - 1;
  ...
  begin
    R.Right := R.Left + Round(AScale * TodoColumns.Items[j].Width);
...
var
    ...
    case Alignment of
      taLeftJustify:
        begin
          ...
          DrawRect.Right := DrawRect.Left + Round(FDPIScale * CHECKBOXSIZE);
          DrawRect.Bottom := DrawRect.Top + Round(FDPIScale * CHECKBOXSIZE);
        end;
      taCenter:
        begin
          DrawRect.Left := R.Left + ((R.Right - R.Left - Round(FDPIScale * CHECKBOXSIZE)) shr 1) + 1;
          DrawRect.Top := R.Top + 1;
          DrawRect.Right := DrawRect.Left + Round(FDPIScale * CHECKBOXSIZE);
          DrawRect.Bottom := DrawRect.Top + Round(FDPIScale * CHECKBOXSIZE);
        end;
      taRightJustify:
        begin
          DrawRect.Left := R.Right - Round(FDPIScale * CHECKBOXSIZE) - 1;
          DrawRect.Top := R.Top + 1;
          DrawRect.Right := DrawRect.Left + Round(FDPIScale * CHECKBOXSIZE);
          DrawRect.Bottom := DrawRect.Top + Round(FDPIScale * CHECKBOXSIZE);
        end;
  ...
  begin
    ...
    begin
      dy := Max(0, (DrawRect.Bottom - DrawRect.Top - HandleGlyph.Height) div 2);
      ACanvas.Draw(DrawRect.Left, DrawRect.Top + dy, HandleGlyph)
    end
    else
    begin
      Bmp := TBitmap.Create;
      try
        ...
        Bmp.TransparentMode := tmAuto;
        Bmp.Transparent := True;
        dy := Max(0, (DrawRect.Bottom - DrawRect.Top - bmp.Height) div 2);
        ACanvas.Draw(DrawRect.Left, DrawRect.Top + dy, Bmp);
        Bmp.FreeImage;
      finally
        Bmp.Free;
      end;
  ...
  begin
    ...
    begin
      ...
      DrawGauge(ACanvas, R, Completion, Settings, FDPIScale);
    end;
  end;
...
begin
  ...
  IHeight := Round(FDPIScale * FOwner.ItemHeight);

  SetBkMode(ACanvas.Handle, TRANSPARENT);

  for Col := 1 to TodoColumns.Count do
  begin
    r.Right := r.Left + Round(FDPIScale * TodoColumns.Items[Col - 1].Width);
    ...
    begin
      ...
      ACanvas.Font.Height := Round(FCompletionFont.Height * FDPIScale);
    end
    else
    begin
      ...
      begin
        ...
        ACanvas.Font.Height := Round(FPriorityFont.Height * FDPIScale);
      end
      else
      begin
        ACanvas.Font.Assign(TodoColumns.Items[Col - 1].Font);
        ACanvas.Font.Height := Round(TodoColumns.Items[Col - 1].Font.Height * FDPIScale);
      end;
    end;
    ...
    begin
      R2 := BRect;
      R2.Left := r.Left;
      R2.Bottom := Rect.Top + Round(FDPIScale * FOwner.FItemHeight) - 1;
      ...
      begin
        ...
        ACanvas.Rectangle(r.Left, Rect.Top, Rect.Right, Rect.Top + Round(FDPIScale * FOwner.FItemHeight));
    end
    else
    begin
      R2 := r;
      ...
      begin
        ...
        ACanvas.Rectangle(r.Left, Rect.Top, r.Right, Rect.Top + Round(FDPIScale * FOwner.FItemHeight));
    end;
    ...
    case TodoColumns.Items[Col - 1].TodoData of
      ...
      tdSubject:
        begin
          ...
          DrawTextEx(ACanvas.Handle, Pchar(su), Length(su), r, Align or DT_END_ELLIPSIS or DT_SINGLELINE or DT_VCENTER, nil);
        end;
      tdResource:
        begin
      ...
      tdImage:
        begin
          ...
          begin
            dy := Max(0, r.Bottom - r.Top - FImages.Height) div 2;
            FImages.Draw(ACanvas, r.Left, r.Top + dy, TodoItems.Items[Index].ImageIndex);
          end;
        end;
      ...
      tdPriority:
        begin
          IIndent := 0;
          dy := Max(0, r.Bottom - r.Top - FPriorityImageList.Height) div 2;
          ...
          case TodoItems.Items[Index].Priority of
            tpLowest: FPriorityImageList.Draw(ACanvas, r.Left + IIndent, r.Top + dy, 4, True);
            tpLow: FPriorityImageList.Draw(ACanvas, r.Left + IIndent, r.Top + dy, 3, True);
            tpNormal: FPriorityImageList.Draw(ACanvas, r.Left + IIndent, r.Top + dy, 2, True);
            tpHigh: FPriorityImageList.Draw(ACanvas, r.Left + IIndent, r.Top + dy, 1, True);
            tpHighest: FPriorityImageList.Draw(ACanvas, r.Left + IIndent, r.Top + dy, 0, True);
          end;
          ...
          if FShowPriorityText then
            DrawTextEx(ACanvas.Handle, Pchar(FOwner.PriorityToString(TodoItems.Items[Index].Priority)), Length(FOwner.PriorityToString(TodoItems.Items[Index].Priority)), r3, Align or DT_END_ELLIPSIS or DT_SINGLELINE or DT_VCENTER, nil);
        end;
      tdCategory:
        begin
  ...
  begin
    ...
    ACanvas.Font.Height := Round(FPreviewFont.Height * FDPIScale);
  ...
  begin
    ...
    begin
      ...
      begin
        r.Left := r.Left + Round(FTodoColumns.Items[Col - 1].Width * FDPIScale);
...
var
  ...
  Canvas: TCanvas;
begin
  Height := Round(FDPIScale * 40);
  if (Index >= 0) then
  begin
    Canvas := TCanvas.Create;
    Canvas.Handle := GetDC(Handle);
    Res := Canvas.TextHeight('gh') + Round(FDPIScale * 4); {some overlap on fonts}
    ReleaseDC(Handle, Canvas.Handle);
    Canvas.Free;
    SendMessage(Handle, CB_SETITEMHEIGHT, Index, Res);
  end
  else
    Res := Round(FDPIScale * 20);
...
procedure TTodoListBox.SetImages(const Value: TCustomImageList);
begin
  FImages := Value;
  Invalidate;
end;
...
begin
  ...
  begin
    Result := Result + Round(TodoColumns.Items[i].Width * FDPIScale);
  end;
end;
...
var
  ...
  begin
    ...
    if (((Pos(Delim, S) = 0) and (Pos('"', S) > 0))) then
    begin
      S := '"' + S + '"';
      dblquotes := true;
    end;
...
begin
  ...
  if AOwner.FDPIScale > 1 then
    FFont.Height := Round(FFont.Height / AOwner.FDPIScale);
...
procedure TTodoHeader.SetImageList(const Value: TCustomImageList);
begin
  FImageList := Value;
  Invalidate;
end;
...
begin
  ...
  begin
    ...
    repeat
      ...
      begin
        w := Round(SectionWidth[SectionIndex]);
      ...
      begin
        ...
        pr.Bottom := pr.Top + Round(sc * FTextHeight);
...
begin
  ...
  begin
    ...
    FInplaceEdit.Text := Sections[FEditSection];
...
{ TCustomTodoList }
{$IFNDEF DELPHIXE10_LVL}
procedure TCustomTodoList.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TCustomTodoList.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
  ...
  FTodoListBox.FDPIScale := FDPIScale;
  FTodoHeader.FDPIScale := FDPIScale;
...
begin
  ...
  begin
    ...
    begin
      ...
      SectionWidth[Col - 1] := Round(sc * FTodoListBox.TodoColumns.Items[Col - 1].Width);
    end;
  end;
...
function TCustomTodoList.GetImages: TCustomImageList;
begin
...
begin
  ...
  begin
    FTodoListBox.TodoColumns.Items[SectionIdx].Width := Round(SectionWidth / FDPIScale);
  end;
end;
...
procedure TCustomTodoList.SetImages(const Value: TCustomImageList);
begin
...
function TCustomTodoList.GetHeaderFlat: boolean;
begin
  Result := FTodoHeader.Flat;
end;
...
procedure TCustomTodoList.SetHeaderFlat(const Value: boolean);
begin
  FTodoHeader.Flat := Value;
end;
...
procedure TCustomTodoList.SetHeaderImages(const Value: TCustomImageList);
begin
...
function TCustomTodoList.GetHeaderImages: TCustomImageList;
begin
...
begin
  ...
  esOffice2019White:
    begin
      HeaderActiveColor := $00FAECE3;
      HeaderActiveColorTo := $00FAECE3;
      HeaderColor := clWhite;
      HeaderColorTo := clWhite;

      SelectionColor := $00F2E1D5;
      SelectionColorTo := $00F2E1D5;

      HeaderFont.Color := $00444648;
      Font.Color := $00444648;
      SelectionFontColor := $003B3B3B
    end;
  esOffice2019Gray:
    begin
      HeaderActiveColor := $00939597;
      HeaderActiveColorTo := $00939597;
      HeaderColor := $00B8BBBE;
      HeaderColorTo := $00B8BBBE;

      SelectionColor := $00969696;
      SelectionColorTo := $00969696;

      HeaderFont.Color := $00232425;
      Font.Color := $00232425;
      SelectionFontColor := $003B3B3B;
    end;
  esOffice2019Black:
    begin
      HeaderActiveColor := $005C5E60;
      HeaderActiveColorTo := $005C5E60;
      HeaderColor := $003B3B3B;
      HeaderColorTo := $003B3B3B;

      SelectionColor := $00505050;
      SelectionColorTo := $00505050;
...
begin
  ...
  begin
    ...
    d := Rect.Bottom - Rect.Top - ImageList.Height;
    if d > 0 then
      d := d div 2
    else
      d := 0;

    ImageList.Draw(Canvas, Rect.Left, Rect.Top + d + 1, 4 - Index, True);
  end;
end;
...
procedure SetFontButKeepFontColor(Destination, Source: TFont; ADPIScale: single = 1);
var SaveColor: TColor;
begin
  SaveColor := Destination.Color;
  Destination.Assign(Source);
  Destination.Height := Round(ADPIScale * Source.Height);
...
begin
  if Editor = StatusEditor then
    SetFontButKeepFontColor(StatusEditor.Font, Font, FDPIScale)
  else if Editor = PriorityEditor then
    SetFontButKeepFontColor(PriorityEditor.Font, Font, FDPIScale)
{$IFDEF USE_PLANNERDATEPICKER}
  else if Editor = PlannerDateEditor then
    SetFontButKeepFontColor(PlannerDateEditor.Font, Font, FDPIScale)
{$ENDIF}
  else if Editor = DefaultDateEditor then
    SetFontButKeepFontColor(DefaultDateEditor.Font, Font, FDPIScale)
  else if Editor = IntegerEditor then
    SetFontButKeepFontColor(IntegerEditor.Font, Font, FDPIScale)
  else if Editor = StringListEditor then
    SetFontButKeepFontColor(StringListEditor.Font, Font, FDPIScale)
  else if Editor = StringEditor then
    SetFontButKeepFontColor(StringEditor.Font, Font, FDPIScale);
end;


procedure SetControlRect(Control: TControl; Rect: TRect; ADPIScale: single = 1);
begin
  Control.Top := Rect.Top;
  Control.Left := Rect.Left;
  Control.Width := Rect.Right - Rect.Left;
  Control.Height := Round(ADPIScale * (Rect.Bottom - Rect.Top));
end;
...
begin
  ...
  // Put the editor parent in the correct position
    ...
    {$IFDEF DELPHI9_LVL}
    SetControlRect(EditorParent, Rect(0,0,0,0), FDPIScale);
    {$ELSE}
    SetControlRect(EditorParent, R, FDPIScale);
    {$ENDIF}
  end
  else
  begin
    ActiveEditor.Parent := Self;
    SetControlRect(ActiveEditor, R, FDPIScale);
  end;
...
begin
  ...
  ColItemRect(Column, Index, R, FDPIScale);
  ...
  case TodoData of
    tdSubject:
      begin
        ...
        StringEditor.Font.Height := Round(FDPIScale * TodoColumns.Items[Column].Font.Height);
    ...
    tdNotes:
      ...
      begin
        ...
        StringListEditor.Font.Height := Round(FDPIScale * PreviewFont.Height);
        OldIdx := Index;
      ...
      begin
        ...
        StringListEditor.Font.Height := Round(FDPIScale * TodoColumns.Items[Column].Font.Height);
      end;
    ...
    tdTotalTime:
      begin
        ...
        FFloatEditor.Font.Height := Round(FDPIScale * TodoColumns.Items[Column].Font.Height);
...
{$ENDIF}
          if EditedItem.DueDate <> 0 then
            ...
            DefaultDateEditor.Font.Height := Round(FDPIScale * TodoColumns.Items[Column].Font.Height);
...
{$ENDIF}
    ...
    tdPriority:
      begin
        ...
        PriorityEditor.ItemHeight := ItemHeight;

        R.Bottom := R.Top + (ItemHeight * (Integer(High(TTodoPriority)) + 1));
        ...
        PriorityEditor.Font.Height := Round(FDPIScale * TodoColumns.Items[Column].Font.Height);
    ...
    tdStatus:
      begin
        ...
        R.Bottom := R.Top + (ItemHeight * (Integer(High(TTodoStatus)) + 1));
        StatusEditor.ItemHeight := ItemHeight;
        ...
        StatusEditor.Font.Height := Round(FDPIScale * TodoColumns.Items[Column].Font.Height);
        StatusEditor.Ctl3D := False;
      end;
    ...
    tdCompletion:
      ...
      begin
        ...
        begin
          if (X < R.Right - 1) then
          begin
            NewCompletion := 100 * Round((X - R.Left) * FDPIScale) div (R.Right - Round(FDPIScale * R.Left) - 2);
        ...
        begin
          ...
          FNumericOnlyEditor.Font.Height := Round(FDPIScale * TodoColumns.Items[Column].Font.Height);
          R.Top := R.Top + 1;
          R.Bottom := R.Bottom - 1;
      ...
      begin
        ...
        IntegerEditor.Font.Height := Round(FDPIScale * TodoColumns.Items[Column].Font.Height);
      end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      for Col := 1 to FTodoColumns.Count - 1 do
      begin
        cw := Round(FDPIScale * FTodoColumns.Items[Col - 1].Width);
        x := x + cw;
