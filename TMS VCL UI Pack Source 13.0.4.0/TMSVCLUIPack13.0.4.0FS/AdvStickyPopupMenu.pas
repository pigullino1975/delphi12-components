...
uses
  ...
  SysUtils, Math, ImgList, Menus, AdvMenus, AtbXPVS, ActnList, Types, AdvStyleIF
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF}
  ;
...
const
  ...
  // v1.6.2.0 : New : High DPI per monitor handling implemented
  //          : Fixed : With Styler Font is used when item is not selected
  // v1.6.2.1 : Fixed : Issue with high DPI when menu item controls are used
  // v1.6.2.2 : Fixed : Issue with Visible property from Action not being updated at runtime
...
function GetTextSize(Canvas: TCanvas; WinCtrl: TWinControl; Text: string; AFont: TFont; ADPIScale: single = 1): TSize;
var
  ACanvas: TCanvas;
  R: TRect;
begin
  ...
  ACanvas.Font.Height := Round(ADPIScale * AFont.Height);
...
procedure DrawContent(Canvas: TCanvas; Caption: string; R: TRect; Images: TCustomImageList; ImageIndex, OffsetX: Integer; Enabled, DrwImg: Boolean; ADPIScale: Single = 1);
var
  bmp: TBitmap;
begin
  if not Assigned(Canvas) then
  ...
  R.Left := R.Left + Round(ADPIScale * OffSetX);
  ...
  begin
    ...
    R.Left := R.Left + Images.Width + Round(ADPIScale * IMG_GAP);
  end;
...
procedure DrawCheck(WinCtrl: TWinControl; Canvas: TCanvas; R:TRect; State, Enabled: Boolean; ControlStyle: TAdvControlStyle; ADPIScale: Single = 1);
var
  DrawState: Integer;
  DrawRect: TRect;
  BMP: TBitmap;
...
begin
  ...
  csClassic, csFlat:
    begin
      ...
      DrawRect.Left := R.Left + (R.Right - R.Left - Round(ADPIScale * CHECK_SIZE)) div 2;
      DrawRect.Top:= R.Top + (R.Bottom - R.Top - Round(ADPIScale * CHECK_SIZE)) div 2;
      DrawRect.Right := DrawRect.Left + Round(ADPIScale * CHECK_SIZE);
      DrawRect.Bottom := DrawRect.Top + Round(ADPIScale * CHECK_SIZE);
  ...
  csTheme:
    begin
      ...
      begin
        ...
        r := Rect(R.Left, R.Top, R.Left + Round(ADPIScale * CHECK_SIZE), R.Top + Round(ADPIScale * CHECK_SIZE));
...
procedure DrawRadio(WinCtrl: TWinControl; Canvas: TCanvas; ControlStyle: TAdvControlStyle; R:TRect; Num, Idx: Integer; dir, dis: Boolean;
   sl: TStrings; Selected:boolean; ACol,ARow: integer; SelectionColor: TColor; ADPIScale: Single = 1);
var
  DrawState: Integer;
  DrawRect: TRect;
  DrawNum: Integer;
  ...
  begin
    ...
    csClassic, csFlat:
      begin
        ...
        if dir then
        begin
          DrawRect.Left := DrawOfs + R.Left + 2 + (DrawNum-1) * Round(ADPIScale * RADIO_SIZE);
          DrawRect.Top := R.Top + (R.Bottom - R.Top - Round(ADPIScale * RADIO_SIZE)) div 2;

          if s <> '' then
          begin
            Canvas.TextOut(DrawRect.Left + Round(ADPIScale * RADIO_SIZE), DrawRect.Top - Round(ADPIScale * 2), s);
        ...
        begin
          ...
          begin
            ...
            DrawRect.Top := R.Top + Round(ADPIScale * 2) + (DrawNum - 1) * th;
            Canvas.TextOut(DrawRect.Left + Round(ADPIScale * RADIO_SIZE) + Round(ADPIScale * 4),DrawRect.Top - Round(ADPIScale * 2), s);
          end
          else
          begin
            DrawRect.Left := R.Left + (R.Right - R.Left - Round(ADPIScale * RADIO_SIZE)) div 2;
            DrawRect.Top := R.Top + Round(ADPIScale * 2) + (DrawNum - 1) * th;
          end;
        end;
        ...
        if ControlStyle = csFlat then
        begin
          DrawRect.Right := DrawRect.Left + Round(ADPIScale * RADIO_SIZE) + 2;
          DrawRect.Bottom := DrawRect.Top + Round(ADPIScale * RADIO_SIZE) + Round(ADPIScale * 2);
        end
        else
        begin
          DrawRect.Right := DrawRect.Left + Round(ADPIScale * RADIO_SIZE);
          DrawRect.Bottom := DrawRect.Top + Round(ADPIScale * RADIO_SIZE);
        end;
    ...
    csTMS, csWinXP, csGlyph:
      begin
        ...
        if dir then
        begin
          DrawRect.Left := DrawOfs + R.Left + 2 + (DrawNum - 1) * Round(ADPIScale * 16);
          DrawRect.Top := R.Top + (R.Bottom - R.Top - Round(ADPIScale * 16)) div 2;

          if s <> '' then
          begin
            Canvas.Textout(DrawRect.Left + Round(ADPIScale * 16), DrawRect.Top + Round(ADPIScale * 1), s);
            DrawOfs := DrawOfs + Canvas.TextWidth(s);
          end
        end
        else
        begin
          th := Max(Round(ADPIScale * 16), Canvas.TextHeight('gh'));
          if s <> '' then
          begin
            DrawRect.Left := R.Left + 2;
            DrawRect.Top := R.Top + Round(ADPIScale * 2) + (DrawNum - 1) * th;
          ...
          begin
            DrawRect.Left := R.Left + (R.Right - R.Left - Round(ADPIScale * 16)) div 2;
            DrawRect.Top := R.Top + Round(ADPIScale * 2) + (DrawNum - 1) * th;
          end;
        end;
    ...
    csBorland:
      begin
        ...
        begin
          ...
          if s <> '' then
          begin
           Canvas.Textout(DrawRect.Left + Round(ADPIScale * 16), DrawRect.Top - Round(ADPIScale * 2), s);
           DrawOfs := DrawOfs + Canvas.TextWidth(s);
          end
        end
        else
        begin
          ...
          begin
            DrawRect.Left := R.Left + 2;
            DrawRect.Top := R.Top + Round(ADPIScale * 2) + (DrawNum - 1) * th;
            Canvas.Textout(DrawRect.Left + Round(ADPIScale * 16) + 2, Drawrect.Top - Round(ADPIScale * 2), s);
          end
          else
          begin
            DrawRect.Left := R.Left + (R.Right - R.Left - Round(ADPIScale * 16)) div 2;
        ...
        Canvas.Polygon([Point(DrawRect.Left + Round(ADPIScale * 2), DrawRect.Top + Round(ADPIScale * 8)),
                        Point(DrawRect.Left + Round(ADPIScale * 8), DrawRect.Top + Round(ADPIScale * 2)),
                        Point(DrawRect.Left + Round(ADPIScale * 14), DrawRect.Top + Round(ADPIScale * 8)),
                        Point(DrawRect.Left + Round(ADPIScale * 8), DrawRect.Top + Round(ADPIScale * 14))]);
        ...
        Canvas.MoveTo(DrawRect.Left + Round(ADPIScale * 8), DrawRect.Top + Round(ADPIScale * 14));
        Canvas.LineTo(DrawRect.Left + Round(ADPIScale * 2), DrawRect.Top + Round(ADPIScale * 8));
        Canvas.LineTo(DrawRect.Left + Round(ADPIScale * 8), DrawRect.Top + Round(ADPIScale * 2));
        ...
        Canvas.LineTo(DrawRect.Left + Round(ADPIScale * 14), DrawRect.Top + Round(ADPIScale * 8));
        Canvas.LineTo(DrawRect.Left + Round(ADPIScale * 8), DrawRect.Top + Round(ADPIScale * 14));
      end;
    csTheme:
      begin
        if True{FIsWinXP} then
        begin
          ...
          DrawRect.Right := DrawRect.Left + Round(ADPIScale * 16);
          DrawRect.Bottom := DrawRect.Top + Round(ADPIScale * 16);
...
procedure TStickyMenuItemActionLink.SetVisible(Value: Boolean);
begin
  if IsVisibleLinked then
  begin
    TStickyMenuItem(FClient).Visible := Value;
...
var
  ...
  ADPIScale: single;
begin
  ADPIScale := TButtonBar(Collection).FOwner.FStickyMenuPanel.FDPIScale;

  R := TButtonBar(Collection).FOwner.FStickyMenuPanel.GetClientRectEx;
  Result := R.Left + Round(ADPIScale * TButtonBar(Collection).FOwner.FItemMarginX);
  if (Index - 1 >= 0) then
    Result := TButtonBar(Collection).Items[Index - 1].Left + TButtonBar(Collection).Items[Index - 1].Width + Round(ADPIScale * BTN_SPACE);
end;
...
begin
  AFont := TFont.Create;
  AFont.Assign(TButtonBar(Collection).FOwner.FCurrentStyler.ButtonAppearance.CaptionFont);

  ADPIScale := TButtonBar(Collection).FOwner.FStickyMenuPanel.FDPIScale;
  if not Assigned(TButtonBar(Collection).FOwner.Styler) then
  begin
    AFont.Height := Round(AFont.Height * 96 * ADPIScale / TButtonBar(Collection).FOwner.FCurrentStyler.ButtonAppearance.CaptionFont.PixelsPerInch);
    ADPIScale := 1;
  end;

  Result := BTNOFFSET_X * 2 + GetTextSize(TButtonBar(Collection).FOwner.FStickyMenuPanel.Canvas, TButtonBar(Collection).FOwner.FStickyMenuPanel, Caption, AFont, ADPIScale).cx;
...
begin
  ...
  bh := GetButtonBarHeight + Round(FDPIScale * 2);
  if not ButtonBarVisible then
    bh := Round(FDPIScale * 2);

  // assign default font
  fnt := TFont.Create;
  try
    AdvStickyPopupMenu.FStickyMenuWindow.Font.Assign(fnt);
  finally
    fnt.Free;
  end;

  // scale font
  AdvStickyPopupMenu.FStickyMenuWindow.Font.Height := Round(AdvStickyPopupMenu.FStickyMenuWindow.Font.Height * FDPIScale);
  ...
  begin
    ...
    begin
      ...
      Ctrl.ScaleForPPI(Round(96 * FDPIScale));
      {$ENDIF}
      ...
      Ctrl.Align := alNone;
      Ctrl.Left := R.Left + Round(FDPIScale * AdvStickyPopupMenu.FItemOffSetX);
      Ctrl.Top := R.Bottom + Round(FDPIScale * 1);
    end;
  end;
  ...
  Width := Max(w, Round(FDPIScale * MIN_MENUWIDTH));
  Height := Max(h, Round(FDPIScale * MIN_MENUHEIGHT));
...
begin
  ...
  begin
    R.Top := TControl(MenuItems[Index].Control).Top + Round(FDPIScale * AdvStickyPopupMenu.FItemOffSetY) + Round(FDPIScale * AdvStickyPopupMenu.FItemMarginY) + Round(FDPIScale * 12);
    DrawContent(Canvas, '', R, AdvStickyPopupMenu.ItemImages, MenuItems[Index].ImageIndex, Round(FDPIScale * AdvStickyPopupMenu.FItemOffSetX), MenuItems[Index].Enabled, true, FDPIScale);
    Exit;
  end;
  ...
  begin
    ...
    begin
      ind := getIConBarRect.Right + Round(FDPIScale * 2) ;
      Canvas.MoveTo(ind , R.Top + Round(FDPIScale * 2));
      Canvas.LineTo(R.Right, R.Top + Round(FDPIScale * 2));
      Canvas.Pen.Color := clWhite;
      Canvas.MoveTo(ind, R.Top + Round(FDPIScale * 3));
      Canvas.LineTo(R.Right, R.Top + Round(FDPIScale * 3));
    end;
  end
  else
  begin
    ...
    begin
      ...
      begin
        ...
        CaptionClr := Styler.Font.Color;
      end;

      if not MenuItems[Index].Enabled then
        CaptionClr := clGray;
      ...
      Rc := GetIconBarRect;
      R1.Left := Rc.Right + Round(FDPIScale * AdvStickyPopupMenu.FItemMarginX);
      ...
      begin
        if Assigned(MenuItems[Index].OnDrawItem) then
          ...
          DrawGradientBackGround(Canvas, Clr, ClrTo, ClrMirror, ClrMirrorTo, BrClr, 60, R1, GradientDirection, Round(FDPIScale * ROUND_RADIUS));

        R2 := R;
        R2.Right := Rc.Right;
        R2.Left := R2.Left + Round(FDPIScale * AdvStickyPopupMenu.FItemOffSetX);
        R2.Top := R2.Top + Round(FDPIScale * AdvStickyPopupMenu.FItemOffSetY);
        ...
        if (MenuItems[Index].Style = isCheckBox) then
          DrawCheck(Self, Canvas, R2, MenuITems[Index].Checked, MenuItems[Index].Enabled, CtrlStyle, FDPIScale)
        else if (MenuItems[Index].Style = isRadioButton)then
          DrawRadio(Self, Canvas, CtrlStyle, R2, 1, b, true, false, nil, MenuItems[Index].Checked, 0, 0, clBlue, FDPIScale);

        if (Index = FMenuItemIndexHot) then
          Canvas.Font.Assign(Styler.SelectedItem.Font)
        else
          Canvas.Font.Assign(Styler.Font);
        ...
        begin
          R3 := R1;
          R3.Right := R3.Left + Round(FDPIScale * AdvStickyPopupMenu.FItemOffSetX) + AdvStickyPopupMenu.ItemImages.Width + Round(FDPIScale * IMG_GAP);
        ...
        if not Assigned(MenuITems[Index].OnDrawItem) then
          DrawContent(Canvas, MenuItems[Index].Caption, R1, AdvStickyPopupMenu.ItemImages, MenuItems[Index].ImageIndex, Round(FDPIScale * AdvStickyPopupMenu.FItemOffSetX), MenuItems[Index].Enabled, DrwImg, FDPIScale)
        else if not Assigned(MenuITems[Index].OnDrawImage) then
          DrawContent(Canvas, '', R1, AdvStickyPopupMenu.ItemImages, MenuItems[Index].ImageIndex, Round(FDPIScale * AdvStickyPopupMenu.FItemOffSetX), MenuItems[Index].Enabled, DrwImg, FDPIScale);
      end
      else
      begin
        if Assigned(MenuItems[Index].OnDrawItem) then
          ...
          DrawGradientBackGround(Canvas, Clr, ClrTo, ClrMirror, ClrMirrorTo, BrClr, 60, R, GradientDirection, Round(FDPIScale * ROUND_RADIUS));

        if Assigned(MenuItems[Index].OnDrawImage) and Assigned(AdvStickyPopupMenu.ItemImages) and (MenuITems[Index].ImageIndex >= 0) then
        begin
          R3 := R;
          R3.Right := R3.Left + Round(FDPIScale * AdvStickyPopupMenu.FItemOffSetX) + AdvStickyPopupMenu.ItemImages.Width + IMG_GAP;
          MenuItems[Index].OnDrawImage(MenuItems[Index], Canvas, R3, Index = FMenuItemIndexHot);
        end
        else
          DrawContent(Canvas, '', R, AdvStickyPopupMenu.ItemImages, MenuItems[Index].ImageIndex, Round(FDPIScale * AdvStickyPopupMenu.FItemOffSetX), MenuItems[Index].Enabled, DrwImg, FDPIScale);

        Canvas.Font.Assign(Styler.SelectedItem.Font);
        Canvas.Font.Height := Round(FDPIScale * Canvas.Font.Height);

        Canvas.Font.Color := CaptionClr;
//        R1.Left := R1.Left + Round(FDPIScale * AdvStickyPopupMenu.FItemOffSetX);
        if not Assigned(MenuITems[Index].OnDrawItem) then
          DrawContent(Canvas, MenuItems[Index].Caption, R1, nil, -1, Round(FDPIScale * AdvStickyPopupMenu.FItemOffSetX), MenuItems[Index].Enabled, DrwImg, FDPIScale);
      end;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    Result.Left := Result.Left + Round(FDPIScale * 1);
    Result.Right := Result.Right - Round(FDPIScale * 1);
    Result.Bottom := Result.Bottom - Round(FDPIScale * 1);
...
begin
  ...
  if not Assigned(FAdvStickyPopupMenu) then
    Exit;
  Result := Max(Result, GetTextSize(Canvas, Self, 'gh', Styler.SelectedItem.Font, FDPIScale).cy + AdvStickyPopupMenu.FItemOffSetY * 2);
...
begin
  ...
  begin
    ...
    R := GetItemsRect;
    R.Top := R.Top + Round(FDPIScale * AdvStickyPopupMenu.FItemMarginY);
    R.Left := R.Left + Round(FDPIScale * AdvStickyPopupMenu.FItemMarginX);
    R.Right := R.Right - Round(FDPIScale * AdvStickyPopupMenu.FItemMarginX);
    for i:= 0 to Index do
    begin
      if not MenuItems[i].Visible then
        Continue;
      ...
      if IsSeparatorItem(MenuItems[i]) then
        Result.Bottom := R.Top + Round(FDPIScale * SEPARATOR_SIZE)
      else if (MenuItems[i].Style = isControlItem) and Assigned(MenuItems[i].Control) and (MenuItems[i].Control is TControl) then
          Result.Bottom := R.Top
      else
        Result.Bottom := R.Top + GetItemHeight;
      R.Top := Result.Bottom + Round(FDPIScale * ITEM_SPACE);
      ...
      begin
        ItemWithCtrlRect.Bottom := ItemWithCtrlRect.Bottom + TControl(MenuItems[i].Control).Height + Round(FDPIScale * 2);
        R.Top := ItemWithCtrlRect.Bottom + Round(FDPIScale * ITEM_SPACE);
      end;
...
begin
  Result := Round(FDPIScale * AdvStickyPopupMenu.FCurrentStyler.SideBar.Size);
end;
...
begin
  ...
  if (ButtonBar.Count > 0) then
    Result := ButtonBar[ButtonBar.Count - 1].Left + ButtonBar[ButtonBar.Count - 1].Width + Round(FDPIScale * BTN_SPACE);
end;
...
begin
  ...
  begin
    ...
    if (MenuItems[i].Style = isControlItem) and Assigned(MenuItems[i].Control) and (MenuItems[i].Control is TControl) then
      ...
      w := Round(FDPIScale * AdvStickyPopupMenu.FItemOffSetX) * 2 + GetTextSize(Canvas, Self, MenuItems[i].Caption, Styler.SelectedItem.Font, FDPIScale).cx;
    ...
    if Assigned(MenuItems[i].Control) and (MenuItems[i].Control is TControl) then
    begin
      w := Max(w, Round(FDPIScale * AdvStickyPopupMenu.FItemOffSetX) * 2 + Round({FDPIScale *} TControl(MenuItems[i].Control).Width));
    end;
  ...
  ItemWidth := ItemWidth + Round(FDPIScale * AdvStickyPopupMenu.FItemMarginX) * 2 + Round(FDPIScale * 35);
end;
...
begin
  ...
  begin
    ...
    Rc := Rect(0, 0, 0, 0);
    R := GetItemRect(i, Rc);
    Result := Result + (Rc.Bottom - Rc.Top) + Round(FDPIScale * ITEM_SPACE);
  end;
  Result := Result + Round(FDPIScale * AdvStickyPopupMenu.FItemMarginY);
end;
...
begin
  ...
  begin
    ...
    DrawGradientBackGround(Canvas, Clr, ClrTo, clNone, clNone, BrClr, 60, R, GradientDirection, Round(FDPIScale * 2));
    DrawContent(Canvas, ButtonBar[Index].Caption, R, AdvStickyPopupMenu.ButtonImages, ButtonBar[Index].ImageIndex, BTNOFFSET_X, ButtonBar[Index].Enabled, True{DrawImage}, FDPIScale);
  end;
end;
...
begin
  ...
  begin
    ...
    Result.Top := Result.Top + Round(FDPIScale * 2);
...
begin
  Result := GetClientRectEx;
  Result.Left := Result.Left + Round (FDPIScale * 1);
  Result.Right := Result.Left + Round (FDPIScale * Styler.IconBar.Size);
...
begin
  ...
  begin
    ...
    FStickyMenuPanel.FDPIScale := GetDPIScale(FStickyMenuPanel, FStickyMenuPanel.Canvas);
...
begin
  ...
  begin
    ...
    {$IFDEF DELPHIXE10_LVL}
    mon := Screen.MonitorFromPoint(TPoint.Create(FStickyMenuWindow.Left, FStickyMenuWindow.Top));
    DPIScale := mon.PixelsPerInch / 96;
    {$ENDIF}
    {$IFNDEF DELPHIXE10_LVL}
    DPIScale := GetDPIScale(FStickyMenuWindow, FStickyMenuWindow.Canvas);
    {$ENDIF}
    if (DPIScale <> FStickyMenuPanel.FDPIScale) then
    begin
      FStickyMenuPanel.FDPIScale := DPIScale;
      FStickyMenuWindow.UpdateSize;
    end;
