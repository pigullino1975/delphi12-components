...
const
  ...
  // 1.1.4.0 : New : TextVertAlign property added
  // 1.1.5.0 : Improved : HTML detection in Caption
...
type
  ...
  TTextVertAlignment = (ttvaCenter, ttvaTop, ttvaBottom);
  ...
  private
    ...
    FTextVertAlign: TTextVertAlignment;
  ...
  protected
    ...
    property TextVertAlign: TTextVertAlignment read FTextVertAlign write FTextVertAlign default ttvaCenter;
...
begin
  ...
  begin
    ...
    if (pos('</', Caption) > 0) or (pos('<I', Uppercase(Caption)) > 0) then
    begin
      ys := 0;

      if (TextVertAlign in [ttvaCenter, ttvaBottom]) then
      begin
      ...
      case TextVertAlign of
      ttvaCenter: dr.Top := dr.Top + (dr.Bottom - dr.Top - ys) div 2;
      ttvaBottom: dr.Top := dr.Bottom - ys;
      end;
    ...
    begin
      if Layout = tlPicBackground then
      begin
        case TextVertAlign of
        ttvaTop: DTATTR := DTATTR and not DT_VCENTER or DT_TOP;
        ttvaBottom: DTATTR := DTATTR and not DT_VCENTER or DT_BOTTOM;
        end;
      end;
