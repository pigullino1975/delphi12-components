...
uses
  ...
  AdvGlowButton, AdvRichEditorIO, AdvStyleIF;
...
uses
  SysUtils, ExtDlgs, Dialogs, PNGImage, JPEG, Math, Windows, Controls;
...
begin
  ...
  if Compact then
  begin
    if Assigned(CompactWindow) and (CompactWindow.ControlCount > 0) then
    begin
      if CompactWindow.Controls[0] is TAdvCustomToolBar then
        ctrl := CompactWindow.Controls[0] as TAdvCustomToolBar;
    end;
  end;

  for i := 0 to ctrl.ControlCount - 1 do
  begin
    if (ctrl.Controls[i].Tag = ID) and (ctrl.Controls[i] is TAdvCustomGlowButton) then
    begin
      Result := ctrl.Controls[i] as TAdvCustomGlowButton;
    end;
  end;
end;
