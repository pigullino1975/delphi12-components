...
const
  ...
  // 1.5.1.1 : Fixed : Issue with FileVersionInt function
  // 1.5.2.0 : New : Detection of Windows Server 2019
  // 1.6.0.0 : New : Returns different editions of Windows 10 versions
  // 1.6.0.1 : Fixed : Issue with use on Windows XP
  // 1.6.1.0 : New : Support for Windows 11 added
  // 1.6.1.1 : Fixed : Windows version retrieval for non Unicode Delphi versions
...
type
  ...
  _OSVERSIONINFOEXA = record
    ...
    dwBuildNumber: DWORD;
    dwPlatformId: DWORD;
    szCSDVersion: array[0..127] of Char; { Maintenance UnicodeString for PSS usage }
...
{$IFNDEF DELPHI_UNICODE}
type
  pfnRtlGetVersion = function(var _OSVERSIONINFOEXA): DWORD; stdcall;
{$ENDIF}
type
...
var
  ...
  majorVer, minorVer, buildVer: Cardinal;
  osVerStr: string;
  spmajorVer, spminorVer, edition: Cardinal;
{$IFDEF DELPHI_UNICODE}
  ver: RTL_OSVERSIONINFOEXW;
{$ENDIF}
{$IFNDEF DELPHI_UNICODE}
  ver: _OSVERSIONINFOEXA;
{$ENDIF}
  ...
  var
    DLLHandle: THandle;
    GetProdInfo: function(majVer, minVer, spmajVer, spminVer: dword; var edition: dword): boolean cdecl stdcall;
  begin
    Result := 'Windows 10';
    ...
    begin
      @GetProdInfo :=  GetProcAddress(DLLHandle, 'GetProductInfo');

      GetProdInfo(majorVer, minorVer, spmajorVer, spminorVer, edition);
...
begin
  ...
  begin
    ...
    osVerStr := ' (' + IntToStr(majorVer) + '.' + IntToStr(minorVer) + '.' + IntToStr(buildVer) + ')';
      ...
      VER_PLATFORM_WIN32_NT: { Windows NT/2000 }
        begin
          ...
          begin
            ...
            begin
              if osVerInfo.wProductType = VER_NT_WORKSTATION then
                Result := GetWindows10Edition + OsVerStr
              else
              begin
          ...
          begin
            if osVerInfo.wProductType = VER_NT_WORKSTATION then
            begin
              if osVerInfo.dwBuildNumber >= 22000 then
                Result := 'Windows 11'
              else
                Result := 'Windows 10';
            end
