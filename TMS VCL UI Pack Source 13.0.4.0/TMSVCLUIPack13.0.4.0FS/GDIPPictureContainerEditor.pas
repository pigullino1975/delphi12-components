...
type
  TGDIPPictureContainerEditor = class(TForm)
    ...
    procedure FileListKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure Savetofle1Click(Sender: TObject);
...
var
  ...
  ARegistry: TRegistry;
begin
  // Store in the following key : Ordinateur\HKEY_CURRENT_USER\Software\tmssoftware\TMS VCL UI Pack
  ARegistry := TRegistry.Create;
  try
    // Read previously opened directory
    folder := FPictureFolder;

    ARegistry.RootKey := HKEY_CURRENT_USER;
    ARegistry.OpenKey('Software\tmssoftware\TMS VCL UI Pack\GDIPPictureContainerEditor', True);
    {$IFDEF DELPHIXE_LVL}
    if SelectDirectory(ARegistry.ReadString(cstFolderKey), '', folder, [sdNewUI, sdShowEdit]) then
    {$ENDIF}
    {$IFNDEF DELPHIXE_LVL}
    if SelectDirectory(ARegistry.ReadString(cstFolderKey), '', folder) then
    {$ENDIF}
    begin
      FPictureFolder := Folder;
      // Write it back to registry
      ARegistry.WriteString(cstFolderKey, Folder);
...
procedure TGDIPPictureContainerEditor.FormClose(Sender: TObject;
...
begin
  ...
  try
    ...
    ARegistry.RootKey := HKEY_CURRENT_USER;
    ARegistry.OpenKey('Software\tmssoftware\TMS VCL UI Pack\GDIPPictureContainerEditor', True);
    if FPictureFolder <> '' then
      ARegistry.WriteString(cstFolderKey, FPictureFolder);
  finally
    ARegistry.Free;
  end;
...
var
  ...
  PictDir: string;
begin
  // Store in the following key : HKCU\Software\tmssoftware\TMS VCL UI Pack
  ARegistry := TRegistry.Create;
  try
    // Read previously opened directory
    ARegistry.RootKey := HKEY_CURRENT_USER;
    ARegistry.OpenKey('Software\tmssoftware\TMS VCL UI Pack\GDIPPictureContainerEditor', True);
    PictDir := ARegistry.ReadString(cstFolderKey);
  finally
    ARegistry.Free;
  end;

  if Length(PictDir) = 0 then
    PictDir := PathToMyPictures;

  FPictureFolder := PictDir;

  PopulateDirectory(AddBackSlash(PictDir)+'*.*');
...
begin
  outputdebugstring(pchar(AFolder));
  Screen.Cursor := crHourGlass;
  ...
  begin
    ...
    ImageList.AddImageLocationsFromFolder(ExtractFilePath(AFolder)+'\*.svg', true);
  end;
  ...
  8: ImageList.AddImageLocationsFromFolder(ExtractFilePath(AFolder)+'\*.svg', true);
  end;
...
procedure TGDIPPictureContainerEditor.PopupMenu1Popup(Sender: TObject);
begin
  Savetofle1.Enabled := (PopupMenu1.PopupComponent = FileList) and (FileList.SelectedItemIndex > -1);
end;

procedure TGDIPPictureContainerEditor.Savetofle1Click(Sender: TObject);
var
  ARegistry: TRegistry;
  tmpS: string;
begin
  // Store in the following key : HKCU\HKEY_CURRENT_USER\Software\tmssoftware\TMS VCL UI Pack
  ARegistry := TRegistry.Create;
  try
    // Read previously opened directory
    ARegistry.RootKey := HKEY_CURRENT_USER;
    ARegistry.OpenKey('Software\tmssoftware\TMS VCL UI Pack\GDIPPictureContainerEditor', True);
    SaveDialog1.InitialDir := ARegistry.ReadString(cstFolderKey);
    {$IFDEF DELPHIXE_LVL}
    tmpS := ExtractFileExt(FileList.Items[FileList.SelectedItemIndex].Image.FileName);
    {$ENDIF}
    {$IFNDEF DELPHIXE_LVL}
    tmpS := ExtractFileExt(FileList.Items[FileList.SelectedItemIndex].FileName);
    {$ENDIF}
    if Length(tmpS) > 0 then
      SaveDialog1.DefaultExt := Copy(tmpS, 2, Length(tmpS));

    {$IFDEF DELPHIXE_LVL}
    SaveDialog1.Filename   := AddBackSlash(SaveDialog1.InitialDir)
                                    + FileList.Items[FileList.SelectedItemIndex].Image.FileName;
    {$ENDIF}
    {$IFNDEF DELPHIXE_LVL}
    SaveDialog1.Filename   := AddBackSlash(SaveDialog1.InitialDir)
                                    + FileList.Items[FileList.SelectedItemIndex].FileName;
    {$ENDIF}

    if SaveDialog1.Execute then
    begin
      // Write it back to registry
      ARegistry.WriteString(cstFolderKey, ExtractFilePath(SaveDialog1.FileName));
      FileList.Items[FileList.SelectedItemIndex].Image.SaveToFile(SaveDialog1.FileName);
    end;
  finally
    ARegistry.Free;
  end;
