...
const
  ...
  // v1.1.0.1 : Fixed : VCL Styles check updated to work in 10.4 Sydney
  // v1.1.0.2 : Fixed : Background color issue in Delphi versions prior XE2
  // v1.1.1.0 : New : BeginUpdate/EndUpdate methods added
  //          : Fixed : Issue with AutoSize = true and adding words
  // v1.1.2.0 : New : Property HoverCursor added
...
procedure TAdvWordCloud.BeginUpdate;
begin
  inc(FUpdateCount);
end;
    ...
    begin
      ...
      dsSelected:
        begin
          ...
          {$ENDIF}
          begin
            LDetails := LStyle.GetElementDetails(tgGradientCellNormal);
            if LStyle.GetElementColor(LDetails, ecBorderColor, clr) then
              clrbrdr := clr
            else
              clrbrdr := LStyle.GetStyleColor(scBorder);
          ...
          {$ENDIF}
          begin
            LDetails := LStyle.GetElementDetails(tgGradientCellSelected);
            if LStyle.GetElementColor(LDetails, ecGradientColor1, clr) then
              clrfrom := clr
            else
              clrfrom := LStyle.GetStyleColor(scButtonFocused);
            if LStyle.GetElementColor(LDetails, ecGradientColor2, clr) then
              clrto := clr
            else
              clrto := clrfrom;
          end;
...
procedure TAdvWordCloud.EndUpdate;
begin
  if FUpdateCount > 0 then
    dec(FUpdateCount);
end;
...
procedure TAdvWordCloud.WMSetCursor(var Msg: TWMSetCursor);
var
  idx: integer;
  P: TPoint;
begin
  ...
  begin
    ...
    idx := ValueAtXY(p.X,p.Y);

    if idx = -1 then
      Windows.SetCursor(Screen.Cursors[Cursor])
    else
      Windows.SetCursor(Screen.Cursors[HoverCursor]);
  end;
end;
