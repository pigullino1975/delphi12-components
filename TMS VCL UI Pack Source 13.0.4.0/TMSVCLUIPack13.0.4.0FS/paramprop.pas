...
type
  ...
  TParamLabelEditor = class(TComponent)
  private
    FParamLabel: TParamLabel;
    FCaption: string;
  ...
  published
    ...
    property ParamLabel: TParamLabel read FParamLabel write FParamLabel;
  end;
...
procedure Tparameditor.FormShow(Sender: TObject);
begin
  FontName.ItemIndex := FontName.Items.IndexOf(ParamLabel1.Font.Name);
  FontSize.ItemIndex := FontSize.Items.IndexOf(IntToStr(ParamLabel1.Font.Size));
end;
...
constructor TParamLabelEditor.Create(AOwner: TComponent);
begin
  inherited;
...
function TParamLabelEditor.Edit: TModalResult;
var
  pe: TParamEditor;
  res: TModalResult;
begin
  if not Assigned(ParamLabel) then
    raise Exception.Create('No TParamLabel assigned');

  pe := TParamEditor.Create(Application);
  pe.ParamLabel1.HTMLText.Text := ParamLabel.HTMLText.Text;

//  pe.ParamLabel1.HideSelection := False;
  pe.DBfields.Visible:=false;

  pe.paramlabel1.Font.Assign(ParamLabel.Font);
  pe.paramLabel1.Font.Assign(ParamLabel.Font);
  pe.paramLabel1.Color := ParamLabel.Color;
//  pe.paramLabel1.ItemHeight := ParamLabel.ItemHeight;
  pe.paramLabel1.Images := ParamLabel.Images;
  pe.ParamLabel1.ParamColor := ParamLabel.ParamColor;
  pe.ParamLabel1.ParamHint := ParamLabel.ParamHint;
  pe.ParamLabel1.ShowHint := ParamLabel.ShowHint;
  pe.ParamLabel1.HoverColor := ParamLabel.HoverColor;
  pe.ParamLabel1.HoverFontColor := ParamLabel.HoverFontColor;
  pe.ParamLabel1.Hover := ParamLabel.Hover;
  pe.ParamLabel1.ShadowColor := ParamLabel.ShadowColor;
  pe.ParamLabel1.ShadowOffset := ParamLabel.ShadowOffset;
  pe.ParamLabel1.LineSpacing := ParamLabel.LineSpacing;
  ...
  if res = mrOk then
  begin
    ParamLabel.HTMLText.Text := pe.ParamLabel1.HTMLText.Text;
  end;
