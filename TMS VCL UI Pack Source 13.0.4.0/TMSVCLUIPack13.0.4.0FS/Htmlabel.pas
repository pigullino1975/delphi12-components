...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, AdvStyleIF,
...
// v2.2.2.0 : New : UIStyle property for TMSStyles
//          : Fixed : ParentFont kept true on initialization
// v2.2.2.1 : Fixed : VCL styles check updated to work in 10.4 Sydney
// v2.3.0.0 : New : Emoji support
// v2.3.0.1 : Fixed : Issue with excluding font from StyleElement with VCL Styles
// v2.4.0.0 : New : Padding property added
// v2.4.0.1 : Fixed : Handling of Padding in auto sized label
// v2.4.1.0 : New : Draw text with common base alignment when control.Linespacing = -1
// v2.4.2.0 : Improved : HTML drawing performance
//          : New : Support for long HEX character notation
//          : New : Support for arrows in HTML formatted text
//          : Improved : Color names in HTML attributes do not longer need 'cl' prefix
// v2.4.2.1 : Fixed : Issue with rendering HTML chars expressed in hex values
...
type
  ...
  {$ENDIF}
  THTMLabel = class(TCustomLabel, ITMSStyle)
  private
    ...
    {$IFDEF DELPHIXE_LVL}
    procedure SetPadding(APadding: TPadding);
    {$ENDIF}
  protected
    ...
    {$ENDIF}
    {$IFDEF DELPHIXE_LVL}
    procedure DoPaddingChanged(Sender: TObject);
    {$ENDIF}
  public
    { Public declarations }
...
begin
  ...
  FPadding.OnChange := DoPaddingChanged;
  {$ENDIF}
...
{$IFDEF DELPHIXE_LVL}
procedure THTMLabel.SetPadding(APadding: TPadding);
begin
  FPadding.Assign(APadding);
end;

procedure THTMLabel.DoPaddingChanged(Sender: TObject);
begin
  Invalidate;
end;
{$ENDIF}
...
procedure THTMLabel.SetUIStyle(const Value: TTMSStyle);
begin
...
procedure THTMLabel.SetComponentStyle(AStyle: TTMSStyle);
begin
  FTMSStyle := AStyle;
