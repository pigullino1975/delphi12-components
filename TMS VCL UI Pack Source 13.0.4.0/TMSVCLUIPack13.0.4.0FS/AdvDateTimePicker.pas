...
uses
  ...
  Dialogs, StdCtrls, ComCtrls, CommCtrl, ExtCtrls, AdvStyleIF, AdvToolTip
  {$IFDEF DELPHIXE2_LVL}
...
const
  ...
  // v1.3.5.2 : Fixed : Issue with label caption font with high DPI
  // v1.3.5.3 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // v1.3.5.4 : Fixed : Issue with border drawing on Delphi 7
  // v1.3.5.5 : Fixed : Drawing issue when runtime themes are disabled
  // v1.3.6.0 : Improved : AutoTab between parts of date entry
  // v1.3.6.1 : Fixed : DPI calculation when parent form is not Scaled
  // v1.3.6.2 : Fixed : Issue with AutoTab and moving focus between 2 TAdvDateTimePicker controls
  // v1.3.6.3 : Fixed : Issue with invoke dataset edit during runtime creation / setting visible
  // v1.3.6.4 : Fixed : Issue with timepicker position on high DPI
  // v1.3.6.5 : Improved : Label font size initialization at design-time for high DPI
  // v1.3.6.6 : Fixed : Rare issue with AutoTab = true and focus handling
...
//  Params.Style := Params.Style + DTS_APPCANPARSE ;
end;
...
begin
  ...
  if AutoTab and (Message.Msg = WM_SETFOCUS) and not DroppedDown and not Focused then
  begin
...
begin
  ...
  begin
    ...
    inherited;
    FLast2Keys := FLast2Keys + Key;
    FLast4Keys := FLast4Keys + Key;
...
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvDateTimePicker.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
...
begin
  ...
  begin
    ...
    begin
      DropW := Round(DROPDOWNBTN_WIDTH * FDPIScale);
...
var
  ...
  function ZeroStr(w: word): string;
  begin
    Result := inttostr(w);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (Length(FLast2Keys) = 2) and ((FLast2Keys = ZeroStr(Fd)) or (FLast2Keys = ZeroStr(Fm))) then
        begin
          FLast4Keys := '';
          PostMessage(Handle, WM_KEYDOWN, VK_RIGHT, 0)
        end;

        if (Length(FLast4Keys) = 4) and (FLast4Keys = ZeroStr(Fy)) then
        begin
          if Kind = dkDateTime then
          begin
            TabToNext;
          end;
        end;
      ...
      if Length(FLast2Keys) = 2 then
      begin
        FLast2Keys := '';
      end;
      if Length(FLast4Keys) = 4 then
      begin
        FLast4Keys := '';
      end;
...
procedure TAdvDateTimePicker.ShowValidation(const AText: string);
begin
...
procedure TAdvDateTimePicker.Notification(AComponent: TComponent;
...
procedure TAdvDateTimePicker.HideValidation;
begin
...
begin
  ...
  begin
    {$IFNDEF DELPHIXE10_LVL}
    FLabelFont.Height := Round(FLabelFont.Height * CalculateDPIScale(Self));
    {$ENDIF}
  end;
