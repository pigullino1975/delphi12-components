...
uses
  ...
  SysUtils, AdvEdit, AdvEddd, Messages, ImgList, AdvStyleIF;
...
const
  ...
  // 1.7.0.2 : Fixed : Issue with OnKeyDown & ESC key
  // 1.8.0.0 : New : WantTab property to shift through the items with the TAB key
  //         : New : OnAutoResize event added
  // 1.8.0.1 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // 1.8.0.2 : Fixed : Inplace editor vertical positioning
  // 1.9.0.0 : New : OnValueSelect/OnValueUnSelect events added
  // 1.9.0.1 : Fixed : Issue with use on parent controls with Ctl3D = false
  // 1.9.0.3 : Improved : Behavior on high DPI
  // 1.9.0.4 : Fixed : Issue with OnContextPopup when the inplace editor is active
  // 1.9.0.5 : Improved : Behavior on high DPI for inplace editor
  // 1.9.0.6 : Fixed : Lookup high DPI issue on older Delphi versions
  // 1.9.0.7 : Fixed : Issue with caret and AutoSize = false
  // 1.9.1.0 : New : Event OnValuePaste added
  // 1.9.1.1 : Fixed : Issue with handling paste of multiline text
  // 1.9.2.0 : Improved : LookupPopup.Font setting uses ParentFont
...
type
  ...
  TValueEvent = procedure(Sender: TObject; Value: TAdvListValue) of object;
  ...
  TValuePasteEvent = procedure(Sender: TObject; var Value: string; var Allow: boolean) of object;
  ...
  TAdvListEditor = class;

  TAdvListEdit = class(TAdvEdit)
  private
    FAdvListEditor: TAdvListEditor;
  ...
  private
    ...
    FWantTab: Boolean;
    FOnValueSelect: TValueEvent;
    FOnValueUnSelect: TValueEvent;
    FBlockSelect: boolean;
    FOnValuePaste: TValuePasteEvent;
    {$IFDEF DELPHIXE2_LVL}
  ...
  protected
    ...
    procedure DoAutoSize(AutoSizing: boolean); virtual;
    ...
    procedure DoValueSelect(AValue: TAdvListValue); virtual;
    procedure DoValueUnSelect(AValue: TAdvListValue); virtual;
    ...
    procedure EditPasteValue(Sender: TObject; var AValue: string; var Allow: boolean);
//    procedure DialogKey(var Key: Word; Shift: TShiftState); override;
  public
    ...
    property Edit: TAdvListEdit read FEdit;
  ...
  published
    ...
    property OnValueSelect: TValueEvent read FOnValueSelect write FOnValueSelect;
    property OnValuePaste: TValuePasteEvent read FOnValuePaste write FOnValuePaste;
    property OnValueUnSelect: TValueEvent read FOnValueUnSelect write FOnValueUnSelect;
...
{$IFDEF DELPHIXE10_LVL}
procedure TAdvListEditor.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ELSE}
procedure TAdvListEditor.ChangeScale(M, D: Integer);
{$ENDIF}
begin
  inherited;
  ...
  begin
    ...
    LookupPopup.Font.Height := MulDiv(LookupPopup.Font.Height, M, D);
    {$ENDIF}
    FEdit.Font.Size := MulDiv(FEdit.Font.Size, M, D);
  end;
end;
...
procedure TAdvListEditor.CMParentFontChanged(var Message: TMessage);
begin
  if not (csLoading in ComponentState) and ParentFont then
  begin
    FLookupPopup.Font.Assign(Font);
  end;
end;
...
begin
  ...
  DoubleBuffered := true;
  FEdit := TAdvListEdit.Create(Self);
  ...
  FEdit.OnClipboardCanPaste := EditPasteValue;
  FEdit.Width := 2;
  FEdit.Height := 14;
  FEdit.Left := 1;
...
procedure TAdvListEditor.DoAutoSize(AutoSizing: boolean);
var
  oldHeight: Integer;
  scale: single;
begin
  ...
  FEdit.Height := Canvas.TextHeight('gh')+ Round(3 * scale);

  if AutoSizing and (Height <> oldHeight) and Assigned(FOnAutoResize) then
    FOnAutoResize(Self);
end;
...
procedure TAdvListEditor.DoValueSelect(AValue: TAdvListValue);
begin
  if Assigned(OnValueSelect) then
    OnValueSelect(Self, AValue);
end;

procedure TAdvListEditor.DoValueUnSelect(AValue: TAdvListValue);
begin
  if Assigned(OnValueUnSelect) then
    OnValueUnSelect(Self, AValue);
end;
  ...
  try
    ...
    try
      ...
      begin
        ...
        gp.FillEllipse(gpsbrush, x , y, w - 4, w - 4);
        gpsBrush.Free;
        ...
        gp.DrawLine(gpPen, x - d, y -d, x + d, y + d);
...
begin
  ...
  FEdit.Width := Canvas.TextWidth(Value) + Round(EDITMARGIN * scale);
end;

procedure TAdvListEditor.EditContextPopup(Sender: TObject; MousePos: TPoint;
  var Handled: Boolean);
begin
...
begin
  ...
  begin
    ...
    FEdit.Width := Canvas.TextWidth(EditText) + Round(EDITMARGIN * scale);
    UpdateEditPos(FEditPos);
    Invalidate;
  end;
end;
...
procedure TAdvListEditor.EditPasteValue(Sender: TObject; var AValue: string;
  var Allow: boolean);
begin
  if Assigned(OnValuePaste) then
    OnValuePaste(Sender, AValue, Allow);
end;
...
begin
  ...
  begin
    ...
    Result := FEdit.Width + Round(EDITMARGIN * scale);
  end;
end;
...
begin
  ...
  FEdit.Width := Max(10, Canvas.TextWidth(s) + Round(EDITMARGIN * scale));
  FEdit.SetEditText(s);
  UpdateEditPos(Index);
  FEdit.PopupMenu := PopupMenu;
  FEdit.SelectAll;
...
{$IFDEF DELPHIXE2_LVL}
  ...
  FUseVCLStyles := CheckVCLStylesEnabled(LStyle,(csDesigning in ComponentState));
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if CallSelectionChangeEvent and not FBlockSelect then
            DoValueSelect(Values[FItemIndex]);
        end;
    ...
    begin
      if not FBlockSelect then
      begin

        if (FItemIndex >= 0) and (FItemIndex < Values.Count) then
          DoValueSelect(Values[FItemIndex]);

        if (oldindex >= 0) and (oldindex <> Values.Count) then
          DoValueUnSelect(Values[oldindex]);
      end;
    end;
    Invalidate;
...
begin
  ...
  if not Ctl3D then
  begin
    if (FEdit.Width <= Round(10 * scale)) then
      FEdit.Width := Round(16 * scale)
    else
      FEdit.Width := FEdit.Width + 4;
  end;
...
procedure TAdvListEdit.WMGetDlgCode(var Message: TMessage);
var
  isShift: Boolean;
begin
  if Assigned(FAdvListEditor) and FAdvListEditor.WantTab and (FAdvListEditor.ItemIndex >= 0) then
  begin
    isShift := (GetKeyState(VK_SHIFT) and $8000 = $8000);
    
    if (not isShift and (FAdvListEditor.ItemIndex < FAdvListEditor.FList.Count -1)) or (isShift and (FAdvListEditor.ItemIndex > 0)) then
