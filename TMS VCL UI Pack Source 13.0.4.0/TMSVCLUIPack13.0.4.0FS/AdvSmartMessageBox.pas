...
const
  ...
  // 1.6.0.2 : Improved : On creation check for enabled AdvFormStyler
  //         : Fixed : SetComponentStyle changes the color settings of the existing messages
  // 1.6.0.3 : Fixed : Rare issue with threading
...
begin
  ...
  case AStyle of
    ...
    tsOffice2019White:
    begin
      DefaultMessage.Rounded := False;
      DefaultMessage.Color := clWhite;
      DefaultMessage.ColorTo := clWhite;
      DefaultMessage.BorderColor := $00C4C6C8;
    end;
    tsOffice2019Gray:
    begin
      DefaultMessage.Rounded := False;
      DefaultMessage.Color := $00B8BBBE;
      DefaultMessage.ColorTo := $00B8BBBE;
      DefaultMessage.BorderColor := $00666666;
    end;
    tsOffice2019Black:
    begin
      DefaultMessage.Rounded := False;
      DefaultMessage.Font.Color := clWhite;
      DefaultMessage.Color := $00303132;
      DefaultMessage.ColorTo := $00303132;
      DefaultMessage.BorderColor := $00ADB0B3;
    end;
  end;
  ...
  begin
    case AStyle of
      tsOffice2003Blue:
      begin
        Messages[i].Rounded := True;
        Messages[i].Color := $FADAC4;
        Messages[i].ColorTo := $F5BFA0;
        Messages[i].BorderColor := $962D00;
      end;
      tsOffice2003Silver:
      begin
        Messages[i].Rounded := True;
        Messages[i].Color := $F7F3F3;
        Messages[i].ColorTo := $E6D8D8;
        Messages[i].BorderColor := $947C7C;
      end;
      tsOffice2003Olive:
      begin
        Messages[i].Rounded := True;
        Messages[i].Color := $E4F1F2;
        Messages[i].ColorTo := $AADADA;
        Messages[i].BorderColor := $588060;
      end;
      tsOffice2003Classic:
      begin
        Messages[i].Rounded := True;
        Messages[i].Color := clWhite;
        Messages[i].ColorTo := $C9D1D5;
        Messages[i].BorderColor := clGray;
      end;
      tsOffice2007Luna:
      begin
        Messages[i].Rounded := True;
        Messages[i].Rounded := True;
        Messages[i].Color := $FFF4E3;
        Messages[i].ColorTo := $EDD9C8;
        Messages[i].BorderColor := $FFD2AF;
      end;
      tsOffice2007Obsidian:
      begin
        Messages[i].Rounded := True;
        Messages[i].Color := $F1F0E6;
        Messages[i].ColorTo := $C6BCB5;
        Messages[i].BorderColor := $5C534C;
      end;
      tsOffice2007Silver:
      begin
        Messages[i].Rounded := True;
        Messages[i].Color := $F8F7F6;
        Messages[i].ColorTo := $E8E0DB;
        Messages[i].BorderColor := $74706F;
      end;
      tsWindowsXP:
      begin
        Messages[i].Rounded := True;
        Messages[i].Color := clWhite;
        Messages[i].ColorTo := clBtnFace;
        Messages[i].BorderColor := clBlack;
      end;
      tsWindowsVista:
      begin
        Messages[i].Rounded := True;
        Messages[i].Color := $FDF8F1;
        Messages[i].ColorTo := $FCEFD5;
        Messages[i].BorderColor := $FDDE99;
      end;
      tsWindows7:
      begin
        Messages[i].Rounded := True;
        Messages[i].Color := $FCEBDC;
        Messages[i].ColorTo := $FCDBC1;
        Messages[i].BorderColor := $CEA27D;
      end;
      tsTerminal:
      begin
        Messages[i].Rounded := True;
        Messages[i].Color := clBtnFace;
        Messages[i].ColorTo := clBtnFace;
        Messages[i].BorderColor := clGray;
      end;
      tsWhidbey:
      begin
        Messages[i].Rounded := True;
        Messages[i].Color := $F5F9FA;
        Messages[i].ColorTo := $A8C0C0;
        Messages[i].BorderColor := $7E9898;
      end;
      tsOffice2010Blue:
      begin
        Messages[i].Rounded := True;
        Messages[i].Color := $FDF6EF;
        Messages[i].ColorTo := $F0DAC7;
        Messages[i].BorderColor := $C7B29F;
      end;
      tsOffice2010Silver:
      begin
        Messages[i].Rounded := True;
        Messages[i].Color := $FFFFFF;
        Messages[i].ColorTo := $EDE5E0;
        Messages[i].BorderColor := $D2CDC8;
      end;
      tsOffice2010Black:
      begin
        Messages[i].Rounded := True;
        Messages[i].Color := $BFBFBF;
        Messages[i].ColorTo := $919191;
        Messages[i].BorderColor := $6D6D6D;
      end;
      tsWindows8, tsWindows10:
      begin
        Messages[i].Rounded := True;
        Messages[i].Color := $F7F6F5;
        Messages[i].ColorTo := $F7F6F5;
        Messages[i].BorderColor := $E4E3E2;
      end;
      tsOffice2013White:
      begin
        Messages[i].Rounded := True;
        Messages[i].Color := clWhite;
        Messages[i].ColorTo := clWhite;
        Messages[i].BorderColor := $D4D4D4;
      end;
      tsOffice2013LightGray:
      begin
        Messages[i].Rounded := True;
        Messages[i].Color := $F6F6F6;
        Messages[i].ColorTo := $F6F6F6;
        Messages[i].BorderColor := $C6C6C6;
      end;
      tsOffice2013Gray:
      begin
        Messages[i].Rounded := True;
        Messages[i].Color := $E5E5E5;
        Messages[i].ColorTo := $E5E5E5;
        Messages[i].BorderColor := $ABABAB;
      end;
      tsOffice2016White:
      begin
        Messages[i].Rounded := True;
      ...
      begin
        Messages[i].Rounded := True;
        Messages[i].Color := $B2B2B2;
        Messages[i].ColorTo := $B2B2B2;
        Messages[i].BorderColor := $444444;
      end;
      tsOffice2016Black:
      begin
        Messages[i].Rounded := True;
        Messages[i].Color := $6A6A6A;
        Messages[i].ColorTo := $6A6A6A;
        Messages[i].BorderColor := $363636;
      end;
      tsOffice2019White:
      begin
        Messages[i].Rounded := False;
        Messages[i].Color := clWhite;
        Messages[i].ColorTo := clWhite;
        Messages[i].BorderColor := $00C4C6C8;
      end;
      tsOffice2019Gray:
      begin
        Messages[i].Rounded := False;
        Messages[i].Color := $00B8BBBE;
        Messages[i].ColorTo := $00B8BBBE;
        Messages[i].BorderColor := $00666666;
      end;
      tsOffice2019Black:
      begin
        Messages[i].Rounded := False;
        Messages[i].Font.Color := clWhite;
        Messages[i].Color := $00303132;
        Messages[i].ColorTo := $00303132;
        Messages[i].BorderColor := $00ADB0B3;
      end;
    end;
  end;
end;
