...
const
  ...
  // 1.4.0.1 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // 1.5.0.0 : New : Password editor type added
  //         : New : Support for virtual image list added
  // 1.5.0.1 : Improved : Date/Time Picker positioning on high DPI
  // 1.5.0.2 : Fixed : Issue with repaint after control resize
  // 1.5.0.3 : Fixed : Issue with closing custom editor from application
  // 1.5.0.4 : Fixed : Rare issue when closing form from inplace editor active
  // 1.5.0.5 : Fixed : Issue with customizing parameter hints via OnParamHint
  // 1.5.0.6 : Improved : Initialization of design-time editor
  //         : Improved : Handling of ESC / Enter key handling on modal dialogs
  // 1.5.0.7 : Fixed : Issue with inplace editor size calculation in high DPI situations
...
type
  ...
  private
    ...
    procedure CMChildKey(var Message: TCMDialogKey); message CM_CHILDKEY;
...
uses
  ...
  VCL.Themes,
  {$ENDIF}
  CommCtrl, ShellApi, Forms,  ShlObj, ActiveX, Variants;
...
procedure TParamListBox.CMChildKey(var Message: TCMDialogKey);
begin
  if (Message.CharCode = VK_RETURN) and FIsEditing then
  begin
    Message.CharCode := 0;
    Message.Result := 1;
    // stop editing
  ...
  if (Message.CharCode = VK_ESCAPE) and FIsEditing then
  begin
    Message.CharCode := 0;
    Message.Result := 1;
    SendMessage(GetFocus, WM_KEYDOWN, VK_ESCAPE, 0);
  end;
    ...
    begin
      ...
      begin
        ...
        if Assigned(FOnParamHint) then
          FOnParamHint(Self, res, href, Anchor,CanShow);
...
procedure TParamListBox.SetImages(Value: TCustomImagelist);
begin
  FImages := Value;
  ReMeasure;
end;
...
procedure TParamListBox.WMPaint(var Message: TWMPaint);
var
  DC: HDC;
begin
  inherited;
  ...
  begin
    ...
    try
      DrawFocusRect(DC, Rect(1,1,Width-1,ItemHeight - 1));
    finally
      ReleaseDC(Handle,DC);
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FParamDatePicker.Width := Max(Round(96 * dpiscale),hr.Right - hr.Left);
    ...
    begin
      ...
      FParamDatePicker.Width := Max(Round(64 * dpiscale),hr.Right - hr.Left);
    ...
    begin
      ...
      cw := Max(Round(16 * dpiscale),hr.Right - hr.Left) + Round(dpiscale * 24);

      FParamSpinEdit.AutoSize := EditAutoSize;
    ...
    begin
      ...
      FParamEdit.Top := pt.Y - 2;
      FParamEdit.Left := pt.X;
      FParamEdit.Width := Max(Round(16 * dpiscale),hr.Right - hr.Left) + Round(16 * dpiscale);

      if c = 'PASSWORD' then
        FParamEdit.PasswordChar := '*'
      else
        FParamEdit.PasswordChar := #0;
...
procedure TParamListBox.WMSetFocus(var Message: TMessage);
begin
  inherited;
  FIsEditing := false;
end;
...
procedure TParamListBox.WMSysKeyDown(var Message: TWMSysKeyDown);
begin
