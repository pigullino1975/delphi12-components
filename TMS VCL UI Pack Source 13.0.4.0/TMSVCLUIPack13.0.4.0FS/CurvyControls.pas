...
uses
  ...
  Math, GDIPFill, AdvGDIP, uxTheme, Menus, ImgList, Types, AdvStyleIF
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF}
  ;
...
const
  ...
  // v1.2.2.0 : New : TCurvyPanel.BorderWidth property added
  // v1.2.2.1 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // v1.2.2.2 : Fixed : Issue with use of TCurvyPanel on a TAdvTabSheet
  // v1.2.3.0 : New : OnClick event added in TCurvyPanel
  //          : New : Caption added in TCurvyPanel
  // v1.2.4.0 : New : WordWrap added in TCurvyPanel
  //          : New : PopupMenu added in TCurvyPanel
  // v1.2.4.1 : Improved : Rendering of TCurvyComboBox in style csDropDownList
...
begin
  ...
  begin
    ...
    begin
      cr := dr;
      h := DrawText(Canvas.Handle, PChar(Caption), Length(Caption), cr, DT_CENTER or DT_WORDBREAK or DT_CALCRECT);
      if h < dr.Bottom - dr.Top then
      begin
        dr.Top := ((dr.Bottom - dr.Top) - h) div 2;
      end;
      DrawText(Canvas.Handle, PChar(Caption), Length(Caption), dr, DT_CENTER or DT_WORDBREAK);
    end
    else
    begin
      DrawText(Canvas.Handle, PChar(Caption), Length(Caption), dr, DT_CENTER or DT_VCENTER or DT_SINGLELINE);
    end;
  end;
end;
...
procedure TCurvyPanel.SetBorderWidth(const Value: integer);
begin
...
procedure TCurvyPanel.SetCaption(const Value: TCaption);
begin
...
procedure TCurvyPanel.SetWordWrap(const Value: boolean);
begin
...
procedure TCurvyCombo.CMMouseEnter(var Message: TMessage);
begin
  inherited;
  if Style = csDropDownList then
    Color := $FBF1E5;
...
procedure TCurvyCombo.CMMouseLeave(var Message: TMessage);
begin
  inherited;
  if (Style = csDropDownList) then
  begin
...
begin
  FCombo.Style := Value;
  if FCombo.Style = csDropDown then
    Self.Color := clWindow
  else
    Self.Color := $E0E0E0;
