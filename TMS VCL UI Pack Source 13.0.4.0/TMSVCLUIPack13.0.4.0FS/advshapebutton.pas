...
const
  ...
  // 6.2.1.6 : Fixed : High DPI handling for Frame in TAdvShapeButton
  // 6.2.1.7 : Improved : Changes made for TAdvToolBarPager internal Office styler
  // 6.2.1.8 : Fixed : Issue with using Font for caption drawing
...
begin
  ...
  begin
    with Appearance do
    begin
      if (FShape in [bsRectangle, bsCustom]) then
      begin
        FShape := bsOrb;
      end;
...
begin
  ...
  if (Parent is TAdvToolBarPager) then
  begin
    if IsWin7 or IsWin10 or FMenuFrame.ITones then
    begin
...
procedure TAdvCustomShapeButton.SetComponentStyleAndAppColor(AStyle: TTMSStyle;
  AAppColor: TColor);
begin
  if (Parent is TAdvToolBarPager) and ((Parent as TAdvToolBarPager).ToolBarStyler = nil) then
  begin
    //
  end
  else
