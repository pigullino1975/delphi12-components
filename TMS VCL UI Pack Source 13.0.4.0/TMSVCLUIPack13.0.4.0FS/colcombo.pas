...
uses
  ...
  Forms, Dialogs, Types, ImgList, AdvStyleIF, ExtCtrls
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF}
  ;
...
const
  ...
  // v1.7.1.0 : Improved : Changed TImageList property to TCustomImageList for compatibility with TVirtualImageList
  // v1.7.1.1 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // v1.7.1.2 : Fixed : Issue with high DPI
  // v1.7.1.3 : Fixed : DPI calculation when parent form is not Scaled
  // v1.7.1.4 : Improved : LabelFont handling in connection with ParentFont
  // v1.7.2.0 : New : LookupBkColor, LookupTextColor, LookupUnderline properties added
  // v1.7.2.1 : Fixed : Issue with default LookupBkColor, LookupTextColor values
...
type
  ...
  private
    ...
    FLookupIncr: Boolean;
    FLookupDelay : Integer;
    //FLastLookup : NativeUInt;
    FSortColumn: Integer;
    FSortedEx: Boolean;
    FDropped: Boolean;
    ...
    FLookupTextColor: TColor;
    FLookupUnderline: boolean;
    FLookupBkColor: TColor;
    FLookup: string;
  ...
  published
    ...
    property LookupDelay : Integer read FLookupDelay write FLookupDelay default 0;
    property LookupColumn: Integer read FLookupColumn write FLookupColumn;
    property LookupBkColor: TColor read FLookupBkColor write FLookupBkColor default clNone;
    property LookupTextColor: TColor read FLookupTextColor write FLookupTextColor default clNone;
    property LookupUnderline: boolean read FLookupUnderline write FLookupUnderline default false;
...
begin
  ...
  dpi := CalculateDPIScale(Self);

  // ignore special character flookup if lenght = 1
  if length(flookup) = 1 then
  begin
    if ord(flookup[1]) <= 13 then
    begin
      flookup := '';
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      Canvas.Font.Height := Round(Canvas.Font.Height * DPI);
    end;
    ...
    begin
      ...
      begin
        ...
        if (isedit and FShowLookup) and (col = FLookupColumn + 1) and ((Length(flookup) > 0) and (flookup <> chr(8))) then
        begin
          ...
          if FLookupUnderline then
            Canvas.Font.Style := [fsUnderline];

          if FLookupBkColor <> clNone then
            Canvas.Brush.Color := FLookupBkColor;
          if FLookupTextColor <> clNone then
            Canvas.Font.Color := FLookupTextColor;
...
procedure TColumnComboBox.TimerEvent(Sender : TObject);
begin
...
begin
  ...
  FTimer.Enabled := false;
  FLookupDelay := 0;
//  FTimer.Interval := FLookupDelay;
  FTimer.OnTimer := TimerEvent;
  FLookupBkColor := clNone;
  FLookupTextColor := clNone;
end;
...
procedure TColumnComboBox.SetImages(const Value: TCustomImageList);
begin
  FImages := Value;
  Invalidate;
end;
    ...
    begin
      ...
      if (s <> '') then
        ...
        begin
          ...
          FLookup := copy(s,1,length(Flookup));
          Invalidate;
          Msg.Result := 1;
          Msg.CharCode := 0;
          Exit;
    ...
    if (s <> '') then
      ...
      begin
        ...
        begin
          ...
          Msg.Result := 1;
          Msg.CharCode := 0;
          if FLookupDelay > 0 then FTimer.Enabled := true
          else
          begin
            if Assigned(OnChange) then
            begin
