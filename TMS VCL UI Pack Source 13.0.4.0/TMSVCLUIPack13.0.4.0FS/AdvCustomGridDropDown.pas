...
const
  ...
  // 6.2.0.0 : New : Support for Office 2019 styles
  // 6.2.0.1 : Fixed : Design-time style painting
  // 6.2.1.0 : New : UIStyle property for office look
  //         : Improved : On creation check for enabled AdvFormStyler
  // 6.2.1.1 : Fixed : ParentFont kept true on initialization
  // 6.2.1.2 : Fixed : Issue with checkbox click
  // 6.2.1.3 : Fixed : Issue with mousewheel handling on closed TAdvGridDropDown
  // 6.2.1.4 : Fixed : Issue with keyboard lookup
  // 6.2.1.5 : Fixed : Issue with OnSelect from keyboard selection
  // 6.2.1.6 : Fixed : Issue with dropdown when drop down grid is empty
  // 6.2.1.7 : Improved : Lookup in TAdvGridDropDown
  // 6.2.1.8 : Fixed : Issue with column width synchronization during startup
  // 6.2.1.9 : Fixed : Issue with checkbox in dropdown
  // 6.2.2.0 : New : Event OnGridSelectCell event added
  // 6.2.2.1 : Fixed : Issue with scrolling while list is not visible
  // 6.2.2.2 : Improved : Behavior on high-DPI
  // 6.2.2.3 : Fixed : Initialization of grid row heights with high-DPI
  // 6.2.3.0 : New : MouseWheel property added to disable mouse wheel selection when dropdown is closed
  // 6.2.3.1 : Fixed : Issue with setting grid.FixedRows = 0
...
type
  ...
  TMouseWheelAction = (mwSelect, mwNone);
  ...
  private
    ...
    FNodeCell: Boolean;
    FTMSStyle: TTMSStyle;
    FOnGridSelectCell: TSelectCellEvent;
    FMouseWheel: TMouseWheelAction;
  ...
  published
    ...
    property MouseWheel: TMouseWheelAction read FMouseWheel write FMouseWheel default mwSelect;
...
{$ENDIF}
    ...
    property OnGridSelectCell: TSelectCellEvent read FOnGridSelectCell write FOnGridSelectCell;
...
uses
  StdCtrls, BaseGrid, AdvGrid, AdvUtil, Math;
...
begin
  ...
  FMouseWheel := mwSelect;
  FLookupIndex := -1;
  FFirstDropDown := false;
  FSelRow := 0;
...
begin
  ...
  begin
    ...
    InternalGrid.ControlLook.NoDisabledCheckRadioLook := true;
    InternalGrid.ControlLook.NoDisabledButtonLook := true;
    InternalGrid.ControlLook.CheckAlwaysActive := true;
    InternalGrid.ControlLook.RadioAlwaysActive := true;
    ...
    InternalGrid.SetComponentStyle(GetComponentStyle);
    {
     ...
     // make sure checkboxes & radiobuttons react
     if FGridEditable then
       InternalGrid.Options := InternalGrid.Options + [goEditing];
    ...
    begin
      ...
      if InternalGrid.RowCount > 1 then
        InternalGrid.FixedRows := 1;

      InternalGrid.Populate;
      idx := GetRowFromItemIndex(ItemIdx);
      if (idx >= InternalGrid.FixedRows) and (idx < InternalGrid.RowCount) and (FLookupIndex = -1) then
        InternalGrid.Row := idx
      else
      begin
        idx := Max(InternalGrid.FixedRows, GetRowFromItemIndex(FLookupIndex));
        if idx < InternalGrid.RowCount then
          InternalGrid.Row := idx;
      end;
    end
    else
...
procedure TCustomAdvGridDropDown.DoSelect;
begin
...
begin
  ...
  begin
    ...
    begin
      ItemIndex := ItemIndex - 1;
      if FAdvColGrid.Row > FAdvColGrid.FixedRows then
        FAdvColGrid.Row := FAdvColGrid.Row - 1;
      FSelRow := FAdvColGrid.Row;
    end;
  end;
...
begin
  ...
  case AStyle of
    ...
    tsTerminal:
      ...
      tsOffice2019White:
      begin
        FixedLineColor := $00ABABAB;
        LineColor := $00D4D4D4;
        HeaderColor := clWhite;
        HeaderColorTo := clNone;
        SelectionColorStyle := scCustom;
        SelectionColor := $00F9ECDE;
        SelectionColorTo := $00F9ECDE;
        HeaderFont.Color := $00444444;
        SelectionTextColor := $00393A3B;
      end;
      tsOffice2019Gray:
      begin
        FixedLineColor := $00999999;
        LineColor := $00D4D4D4;
        HeaderColor := $00666666;
        HeaderColorTo := clNone;
        SelectionColorStyle := scCustom;
        SelectionColor := $0086888A;
        SelectionColorTo := $0086888A;
        HeaderFont.Color := clWhite;
        SelectionTextColor := clBlack;
      end;
      tsOffice2019Black:
      begin
        FixedLineColor := $00999999;
        LineColor := $00D4D4D4;
        HeaderColor := $00262626;
        HeaderColorTo := clNone;
        SelectionColorStyle := scCustom;
        SelectionColor := $00B1B1B1;
        SelectionColorTo := $00B1B1B1;
        HeaderFont.Color := $00DADADA;
        SelectionTextColor := clWhite;
      end;
  end;
end;
...
procedure TCustomAdvGridDropDown.SetUIStyle(const Value: TTMSStyle);
begin
...
begin
  ...
  begin
    ...
    if (FSelRow <> ARow) and Assigned(OnGridSelectCell) then
    begin
      OnGridSelectCell(Self, ACol, ARow, CanSelect);
    end;
...
begin
  ...
  begin
    ...
    AFont.Height := Round(AFont.Height * (FAdvColGrid as TAdvColGrid).GridDPIScale);
  end;
...
begin
  ...
  begin
    ...
    begin
      Res := Text;
      if (SelLength > 0) and (FLookupEntry <> '') then
      begin
...
begin
  ...
  begin
    ...
    begin
      L := '';
      if Items[i].Text.Count > FLookupColumn then
        L := Items[i].Text.Strings[FLookupColumn];
  ...
  begin
    if Assigned(FAdvColGrid) then
    begin
      if Assigned(OnBeforeDropDown) and not FFirstDropDown then
      begin
...
procedure TCustomAdvGridDropDown.CreateWnd;
begin
  inherited;
  if FDesignTime then
    SetComponentStyle(tsCustom);
...
function TAdvColGrid.GridDPIScale: single;
begin
  Result := DPIScale;
end;
...
begin
  ...
  begin
    ...
    DefaultRowHeight := Round(AdvDropDown.DropDownRowHeight * FDPIScale);
    RowHeights[0] := Round(AdvDropDown.HeaderHeight * FDPIScale);
