...
const
  ...
  // 1.5.0.2 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // 1.6.0.0 : New : Property AutoHeight added
  // 1.6.0.1 : Improved : Label top positioning on form
...
type
  ...
  protected
    ...
    procedure LabelSizeChange(Sender:TObject; NewWidth, NewHeight : Integer);
  public
    { Public declarations }
...
var
  HTMLabel: THTMLabel;
  DialogUnits: TPoint;
  ButtonTop, BW, NBW: Integer;
  OffsX, OffsY: Integer;
  FDPIScale: single;
begin
  InitVCLStyle(True);
  FDialogForm := THTMLDialogForm.CreateNew(Application);
  FDialogForm.FOwner := Self;
  ...
  with FDialogForm do
    try
      if Assigned(Self.Owner) and (Self.Owner is TControl) then
        ...
        FDPIScale := GetDPIScale(FDialogForm, FDialogForm.Canvas);
      ...
      FButtonWidth := MulDiv(50, DialogUnits.X, 4);
      FButtonHeight := MulDiv(14, DialogUnits.Y, 8);
      ButtonTop := ClientHeight - FButtonHeight -4;

      HTMLabel := THTMLabel.Create(FDialogForm);
      HTMLabel.PictureContainer := self.PictureContainer;
      HTMLabel.SetDPIScale(FDPIScale);

      FDialogForm.FHTMLabel := HTMLabel;

      with HTMLabel do
      begin
        OnNewSize := LabelSizeChange;
        Parent := FDialogForm;
        Font.Assign(FFont);
        Canvas.Font.Assign(Font);
        Font.Height := Round(FDPIScale * Font.Height);
        OffsX := MulDiv(8, DialogUnits.X, 4);
        OffsY := 4; // MulDiv(8, DialogUnits.Y, 8);
      ...
      with TButton.Create(FDialogForm) do
      begin
        Font.Assign(FFont);
        ...
        if NBW < FButtonWidth then
          NBW := FButtonWidth;

        SetBounds(FDialogForm.ClientWidth - (NBW + 10), ButtonTop, NBW, FButtonHeight);
      end;
      ...
      begin
        ...
        SetBounds(FDialogForm.ClientWidth - BW - (NBW + 10), ButtonTop, NBW, FButtonHeight);
      end;
...
procedure THTMLDialog.LabelSizeChange(Sender: TObject; NewWidth,
  NewHeight: Integer);
begin
  if AutoHeight then
  begin
    FDialogForm.ClientHeight := NewHeight + FButtonHeight + 16;
    // recalculate position
    // FDialogForm.Position := Position;
  end;
end;
