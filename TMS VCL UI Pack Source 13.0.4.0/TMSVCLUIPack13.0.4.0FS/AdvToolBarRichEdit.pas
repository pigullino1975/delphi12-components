...
uses
  ...
  StdActns, ExtActns, RichEdit, AdvStyleIF;
...
begin
  ...
  atb := AddButton(HInstance,TRichEditAlignLeft, bsCheck, 'TMSRETBBALIGNLEFT','Align left');
  atb.OfficeHint.Title := Hints.AlignLeftTitle;
  atb.OfficeHint.Notes.Text := Hints.AlignLeftContent;
  atb.Tag := integer(btAlignLeft);

  atb := AddButton(HInstance,TRichEditAlignCenter, bscheck, 'TMSRETBBALIGNCENTER','Align center');
  ...
  atb := AddButton(HInstance,TRichEditAlignRight, bsCheck, 'TMSRETBBALIGNRIGHT','Align right');
  ...
  atb := AddButton(HInstance,TRichEditBullets, bsCheck, 'TMSRETBBOL','Numbered');
...
begin
  ...
  atb := AddButton(HInstance,nil, bsButton, 'TMSRETBBOPEN','Open file');
  ...
  atb := AddButton(HInstance,nil, bsButton, 'TMSRETBBSAVE','Save');
  ...
  atb := AddButton(HInstance,TEditCut, bsButton, 'TMSRETBBCUT', 'Cut to clipboard');
  atb.OfficeHint.Title := FHints.CutTitle;
  atb.OfficeHint.Notes.Text := FHints.CutContent;
  atb.Tag := integer(btCut);

  atb := AddButton(HInstance,TEditCopy, bsButton, 'TMSRETBBCOPY', 'Copy to clipboard');
  atb.OfficeHint.Title := FHints.CopyTitle;
  atb.OfficeHint.Notes.Text := FHints.CopyContent;
  atb.Tag := integer(btCopy);

  atb := AddButton(HInstance,TEditPaste, bsButton, 'TMSRETBBPASTE', 'Paste from clipboard');
  ...
  atb := AddButton(HInstance,TEditUndo, bsButton, 'TMSRETBBUNDO', 'Undo');
...
begin
  ...
  atb := AddButton(HInstance, TEditPaste, bsButton, 'TMSRETBBPASTELARGE', 'Paste from clipboard');
  atb.Tag := 1;
  atb.OfficeHint.Title := FHints.PasteTitle;
  atb.OfficeHint.Notes.Text := FHints.PasteContent;
  atb.Caption := 'Paste';
...
begin
  ...
  AutoPositionControls := false;
  ToolBarState := tsFixed;
  FCopy.Top := Round(DPIScale * 24);
  FCopy.Left := Round(DPIScale * 41);
end;
...
//  fss.OnExit := ExitFontSize;
  ...
  atb := AddButton(HInstance, TRichEditBold, bsButton, 'TMSRETBBBOLD', 'Bold');
  ...
  atb := AddButton(HInstance, TRichEditItalic, bsButton, 'TMSRETBBITALIC', 'Italic');
  ...
  atb := AddButton(HInstance, TRichEditUnderline, bsButton, 'TMSRETBBUNDERLINE', 'Underline');
  ...
  atb := AddButton(HInstance, TRichEditStrikeout, bsButton, 'TMSRETBBSTRIKE', 'Strikeout');
...
begin
  ...
  begin
    ...
    begin
      cbSize := SizeOf(Format);
      (Target as TRichedit).Perform(EM_GETCHARFORMAT, SCF_SELECTION, LParam(@Format));
...
function TRichEditFontNameAction.ExecuteAction(Action: TBasicAction): Boolean;
begin
  Result := inherited;
end;
...
begin
  ...
  // get selection color here and update
  if Enabled and (Target as TRichEdit).Focused then
  begin
...
function TRichEditFontSizeAction.ExecuteAction(Action: TBasicAction): Boolean;
begin
  Result := inherited;
end;
