...
begin
  if not ((Key = #13) and ReturnIsTab) and not (Key = #27) then
    if not EditCanMOdify then
      Exit;
...
begin
  ...
  if not ((Message.CharCode = 13) and ReturnIsTab) and not (Message.CharCode = 27) and (GetKeyState(VK_CONTROL) and $8000 = 0) then
  begin
    FIsEditing := true;
    if not EditCanModify then
    begin
