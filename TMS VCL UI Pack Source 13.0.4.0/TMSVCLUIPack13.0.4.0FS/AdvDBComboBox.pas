...
const
  ...
  // 1.0.6.0 : New : BorderColor property added
...
procedure TAdvDBComboBox.DrawBorders;
var
  DC: HDC;
begin
...
procedure TAdvDBComboBox.DrawControlBorder(DC: HDC);
var
  ARect: TRect;
  BtnFaceBrush: HBRUSH;
begin
  if (FBorderColor <> clNone) and not (FHasFocus and (FocusBorderColor <> clNone)) then
  begin
  ...
  begin
    if FHasFocus then
    begin
      BtnFaceBrush := CreateSolidBrush(ColorToRGB(FocusBorderColor));
      GetWindowRect(Handle, ARect);
      OffsetRect(ARect, -ARect.Left, -ARect.Top);
      FrameRect(DC, ARect, BtnFaceBrush);
      DeleteObject(BtnFaceBrush);
...
procedure TAdvDBComboBox.DataUpdate(Sender: TObject);
begin
...
procedure TAdvDBComboBox.WMKillFocus(var Message: TWMKillFocus);
begin
  inherited;
...
procedure TAdvDBComboBox.WMNCPaint(var Message: TMessage);
begin
  inherited;
  if (FBorderColor <> clNone) or (FocusBorderColor <> clNone) then
    DrawBorders;
end;

procedure TAdvDBComboBox.WMPaint(var Message: TWMPaint);
begin
  inherited;
  if (FBorderColor <> clNone) or (FocusBorderColor <> clNone) then
    DrawBorders;
...
procedure TAdvDBComboBox.WMSetFocus(var Message: TWMSetFocus);
begin
  inherited;
...
procedure TAdvDBComboBox.SetBorderColor(const Value: TColor);
begin
