...
uses
  ...
  Menus, StdCtrls, FileCtrl, ShellApi, CommCtrl, AdvStyleIF
  {$IFDEF DELPHIXE2_LVL}
...
const
  ...
  // v1.3.0.4 : Fixed : Drawing of checkboxes when scrolling on Win10
  // v1.3.1.0 : Improved : High DPI drawing of sys icons & per monitor DPI handling
  // v1.3.2.0 : Improved : Item height handling for newer operating systems & high DPI
...
type
  ...
  published
    ...
    property ItemHeight default 20;
    property Version: string read GetVersion write SetVersion;
  end;
...
begin
  ...
  begin
    ...
    ItemHeight := Canvas.TextHeight('gh') + 6;
  end;
end;
...
begin
  ...
  begin
    ...
    if ShowGlyphs then
    begin
      offset := Round(17 * GetDPIScale(Self, Canvas));
      sz := Round(16 * GetDPIScale(Self, Canvas));
      ...
      begin
        ...
        hicon := ImageList_GetIcon(syshandle,sfi.iICon, ILD_TRANSPARENT);

        DrawIconEx(canvas.Handle, rect.Left, rect.Top, hicon, sz, sz, 0, 0, DI_NORMAL);
...
begin
  ...
  begin
    FillRect(Rect);
    bmpWidth  := Round(16 * GetDPIScale(Self, Canvas));
    ...
    begin
      ...
      if fWinNT then
       begin
        ...
        DrawIconEx(canvas.Handle, rect.Left + diroffset, rect.Top, hicon, bmpWidth, bmpWidth, 0, 0, DI_NORMAL);
...
{$IFDEF DELPHIXE10_LVL}
procedure TDirectoryListBoxEx.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ELSE}
procedure TDirectoryListBoxEx.ChangeScale(M, D: Integer);
{$ENDIF}
begin
  inherited;
...
procedure TDirectoryListBoxEx.CNMeasureItem(var Message: TWMMeasureItem);
begin
  with Message.MeasureItemStruct^ do
  begin
    Canvas.Font.Assign(Font);
...
begin
  with Message.MeasureItemStruct^ do
  begin
    itemHeight := Round(16 * GetDPIScale(Self, Canvas));
  end;
end;
