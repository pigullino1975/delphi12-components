...
uses
  ...
  ,web, AdvUtils
  {$ENDIF}
  ,AdvPDFGraphicsLib, AdvPDFCoreLibBase, AdvCustomComponent, AdvGraphicsTypes,
  AdvPDFRichTextLib, AdvTypes, PictureContainer;

const
  TAdvPDFLibDocURL = 'https://download.tmssoftware.com/download/manuals/AdvPDFLibraryDevGuide.pdf';
...
type
  ...
  TAdvCustomPDFLib = class(TAdvCustomComponent)
  private
  ...
  {$IFDEF WEBLIB}
  TAdvPDFLibList = class(TList)
  private
    function GetItem(Index: Integer): IAdvCustomPDFLib;
    procedure SetItem(Index: Integer; const Value: IAdvCustomPDFLib);
  public
    property Items[Index: Integer]: IAdvCustomPDFLib read GetItem write SetItem; default;
  end;
  {$ENDIF}
  {$IFNDEF WEBLIB}
  TAdvPDFLibList = class(TList<IAdvCustomPDFLib>);
  {$ENDIF}

  TAdvPDFLibFactoryService = class(TInterfacedObject, IAdvPDFLibService, IAdvPDFLibGeneralService)
  private
...
type
  ...
  private
    ...
    FURL: string;
  protected
    procedure DoFontFileLoaded(const AFile: TAdvUtilsFile);
  public
    procedure Load;
    property Loaded: Boolean read FLoaded;
    property Data: TAdvUtilsFile read FData;
...
function GetFontCacheItem(AIndex: Integer): TAdvPDFLibFontCacheItem;
{$ENDIF}
...
{$IFDEF WEBLIB}
type
  TAdvPDFLibFontCache = class(TObjectList)
  private
    function GetItem(Index: Integer): TAdvPDFLibFontCacheItem;
    procedure SetItem(Index: Integer; const Value: TAdvPDFLibFontCacheItem);
  public
    property Items[Index: Integer]: TAdvPDFLibFontCacheItem read GetItem write SetItem; default;
  end;

var
  FFontCache: TAdvPDFLibFontCache;

procedure TAdvPDFLibFontCacheItem.DoFontFileLoaded(const AFile: TAdvUtilsFile);
var
  I: Integer;
begin
  FData := AFile;
...
procedure TAdvPDFLibFontCacheItem.Load;
var
  u: string;
begin
  if Assigned(Blob) then
    TAdvUtils.LoadFile(Blob, @DoFontFileLoaded)
  else
  begin
    u := URL;
    asm
      ...
      xhr.onload = function(e) {
        if (this.status == 200) {
          pas["WEBLib.AdvUtils"].TAdvUtils.LoadFile(this.response,rtl.createCallback(me,"DoFontFileLoaded"));
        }
      };
      xhr.send();
    end;
...
var
  fci: TAdvPDFLibFontCacheItem;
begin
  if not Assigned(FFontCache) then
    FFontCache := TAdvPDFLibFontCache.Create;

  fci := TAdvPDFLibFontCacheItem.Create;
  fci.FURL := AFontURL;
  fci.Load;
  FFontCache.Add(fci);
end;
...
function TAdvCustomPDFLib.GetDocURL: string;
begin
...
{$IFDEF WEBLIB}
function TAdvCustomPDFLib.GetFullFontEmbedding: Boolean;
begin
  Result := True;
  if Assigned(FPDFLib) then
...
function TAdvCustomPDFLib.GetInstance: NativeUInt;
begin
  Result := HInstance;
end;
...
constructor TAdvCustomPDFLib.Create(AOwner: TComponent);
begin
  inherited;
  Initialize(True);
end;
...
procedure TAdvCustomPDFLib.Initialize(AUseNativePDFImplementation: Boolean);
var
...
begin
  ...
  begin
    if TAdvPDFLibPlatformServices.Current.SupportsPlatformService(IAdvPDFLibService, IInterface(PDFLibService)) then
      FPDFLib := PDFLibService.CreatePDFLib;
  end
  else
  {$ENDIF}
  begin
    if TAdvPDFLibPlatformServices.Current.SupportsPlatformService(IAdvPDFLibGeneralService, IInterface(PDFLibServiceGeneral)) then
...
{$IFDEF WEBLIB}
procedure TAdvCustomPDFLib.SetFullFontEmbedding(const Value: Boolean);
begin
  if Assigned(FPDFLib) then
...
{$IFDEF WEBLIB}
function TAdvPDFLibList.GetItem(Index: Integer): IAdvCustomPDFLib;
begin
  Result := IAdvCustomPDFLib(inherited Items[Index]);
end;

procedure TAdvPDFLibList.SetItem(Index: Integer; const Value: IAdvCustomPDFLib);
begin
  inherited Items[Index] := Value;
end;

function TAdvPDFLibFontCache.GetItem(Index: Integer): TAdvPDFLibFontCacheItem;
begin
  Result := TAdvPDFLibFontCacheItem(inherited Items[Index]);
end;

procedure TAdvPDFLibFontCache.SetItem(Index: Integer; const Value: TAdvPDFLibFontCacheItem);
begin
  inherited Items[Index] := Value;
end;
{$ENDIF}
