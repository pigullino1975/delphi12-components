...
uses
  ...
  InspEdits, AdvStyleIF, InspXPVS, InspDD, InspImg, JPEG, Types, ImgList
...
const
  ...
  // 1.10.0.0 : New : Support for Office 2019 styles
  // 1.10.0.1 : Fixed : Issue with updating Item.FloatValue
  //          : Fixed : Design-time style painting
  // 1.10.0.2 : Fixed : Issue with mouse wheel scrolling in imMultiPanelActive mode
  //          : Improved : Item.ValueFont also used for ptCustom property type
  //          : Fixed : Issue with OnItemClick in value area for imMultiPanelActive mode
  // 1.10.1.0 : New : Property AutoDropDown: boolean added
  // 1.10.1.1 : Fixed : Regression with default tree node drawing
  //          : Improved : On creation check for enabled AdvFormStyler
  // 1.10.1.2 : Fixed : Issue with gradient panel background color when using VCL styles
  // 1.10.2.0 : Improved : Themed high DPI rendering
  // 1.11.0.0 : New : Color property for TInspectorItem
  // 1.11.0.1 : Fixed : Issue with ptCustom property type and display of value
  // 1.11.0.2 : Fixed : Potential issue with font picker dialog causing focus leave on InspectorBar
  //          : Fixed : VCL styles check updated to work in 10.4 Sydney
  // 1.11.0.3 : Fixed : Issue with ptCustom font value for drawing
  // 1.11.1.0 : Improved : Support for editing blob memo fields with TMemoInspectorEditLink
  //          : Improved : Checkbox versus text drawing in psProperties mode
  // 1.12.0.0 : New : OnBefore*, OnAfter* events added for navigator in TDBInspectorBar
  // 1.12.0.1 : Fixed : Text drawing transparency in help panel
  // 1.12.1.0 : New : Property DropDownButtonWidth added
  // 1.12.1.1 : Improved : High DPI handling for InspectorBar combobox property types
  //          : Improved : High DPI handling for InspectorBar font property types
  // 1.12.1.2 : Improved : MultiPanel mode with VCL styles
  // 1.12.2.0 : New : TVirtualImageList support added
  // 1.12.2.1 : Fixed : Issue with Panel & Item Font scaling in high DPI
  // 1.12.2.2 : Fixed : Issue with property type panel value editing in high DPI
  // 1.12.2.3 : Fixed : Item.CaptionFont handling in multimonitor high DPI situation
  // 1.12.3.0 : Improved : Now uses Color/ColorTo for background color drawing when no panels are in the InspectorBar
  // 1.13.0.0 : New : ptStringList property editor
  // 1.13.0.1 : Fixed : Issue with font picker in high DPI
...
type
  ...
  TPropertyType = (ptInteger, ptFloat, ptText, ptColor, ptFont, ptBoolean, ptValues,
    ptIntSpin, ptTextButton, ptPropButton, ptDate, ptTime, ptCustom, ptValuesList, ptFixedColor, ptButton, ptPassword, ptPicture, ptStringList);
  ...
  public
    ...
    property StringsValue: TStrings read FStringsValue write FStringsValue;
  ...
  private
    ...
    function GetItemByName(AName: string): TInspectorItem;
    procedure SetDropDownButtonWidth(const Value: integer);
  protected
  ...
  published
    ...
    property AutoAdvance: Boolean read FAutoAdvance write FAutoAdvance default false;
    ...
    property DragKind;
    property DragCursor;
    property DropDownButtonWidth: integer read FDropDownButtonWidth write SetDropDownButtonWidth default 18;
...
procedure TInspectorItem.SetColor(const Value: TColor);
begin
  FColor := Value;
  Changed;
end;
...
begin
  ...
  begin
    case ocGraphic of
      ...
      ocgCross:
        begin
          Canvas.StretchDraw(Rect(pt.X + 4, pt.Y + 4, pt.X + 4 + Round(9 * FDPIScale), pt.Y + 4 + Round(9 * FDPIScale)), FNodeCloseGlyph);
        end;
      ocgTriangle:
        begin
  ...
  begin
    case ocGraphic of
      ...
      ocgCross:
        begin
          Canvas.StretchDraw(Rect(pt.X + 4, pt.Y + 4, pt.X + 4 + Round(9 * FDPIScale), pt.Y + 4 + Round(9 * FDPIScale)), FNodeOpenGlyph);
        end;
      ocgTriangle:
        begin
          Canvas.Brush.Color := clBlack;
...
//---------  End DRAW -------------
  ...
  begin
    ...
    begin
      {$IFDEF DELPHIXE2_LVL}
      clr := StyleServices.GetStyleColor(TStyleColor.scGenericBackground);
      {$ENDIF}
    end;
    if clr = clNone then
      clr := clWindow;
  ...
  case Panel.Style of
    ...
    psProperties:
      begin
        ...
        begin
          if not Item.InspectorPanel.WordWrap then
            DrawText(Canvas.Handle, PChar(Text), Length(Text), R, DT_LEFT or DT_SINGLELINE or DT_VCENTER or DT_END_ELLIPSIS)
          else
        ...
        if ((Item.PropertyType in [ptTextButton, ptPropButton, ptFont, ptStringList]) or (IEButton = ebMore))
        ...
        begin
          ...
          begin
            ...
            DrwRect := Rect(R.Right - Round(FDropDownButtonWidth * FDPIScale), R.Top - 1, R.Right + 2, R.Bottom - 2);
            ...
            CloseThemeData(HTheme);
            DrwRect := Rect(R.Right - Round((FDropDownButtonWidth - 2) * FDPIScale), R.Top + 2, R.Right, R.Bottom);
          end
          else
          begin
            DrawFrameControl(Canvas.Handle, Rect(R.Right - Round(FDropDownButtonWidth * FDPIScale), R.Top, R.Right + 2, R.Bottom - 2), DFC_BUTTON, DFCS_BUTTONPUSH or DFCS_FLAT);
            DrwRect := Rect(R.Right - Round((FDropDownButtonWidth - 2) * FDPIScale), R.Top + 2, R.Right + 2, R.Bottom);
          end;

          SetBKMode(Canvas.Handle, TRANSPARENT);
          DrwRect.Left := DrwRect.Left - 2;
          DrawText(Canvas.Handle, '...', 3, DrwRect, DT_CENTER);
          R.Right := R.Right - Round((FDropDownButtonWidth - 2) * FDPIScale);
        end;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              GetThemePartSize(HTheme, Canvas.Handle, BP_CHECKBOX, RBS_CHECKEDNORMAL, nil, TS_DRAW, sz);
              DrwRect := Rect(R.Left, R.Top + (R.Bottom - R.Top - sz.cy) div 2, R.Left + sz.cx, R.Top + (R.Bottom - R.Top - sz.cy) div 2 + sz.cy);
        ...
        begin
          ...
          begin
            ...
            FontValue := Item.TextValue;
            if Assigned(FOnItemValue) then
              FOnItemValue(self, Panel, Item, FontValue);

            DrawText(Canvas.Handle, PChar(FontValue), Length(FontValue), R, DTSTYLE or DT_END_ELLIPSIS);
          end
          else
...
begin
  ...
  case InspectorItem.PropertyType of
    ...
    ptTextButton, ptPropButton, ptStringList:
      begin
        ...
        if InspectorItem.PropertyType = ptStringList then
          FInspectorEditBtn.Text := '(TStrings)'
        else
        ...
        FInspectorEditBtn.ButtonWidth := FDropDownButtonWidth + 2;
        FInspectorEditBtn.ButtonCaption := '...';
    ...
    ptValues, ptValuesList:
      begin
        ...
        {$IFNDEF DELPHIXE11_LVL}
        FInspectorCombo.Font.Height := Round(FInspectorCombo.Font.Height * FDPIScale);
        {$ENDIF}
        ...
        FInspectorCombo.DroppedDown := FAutoDropDown;
        FEditing := True;
        FEditType := itCombo;
      end;
    ptColor, ptFixedColor:
      begin
        ...
        FInspectorColorCombo.DroppedDown := FAutoDropDown;
        FEditing := True;
    ...
    ptDate, ptTime:
      begin
        ...
        //FInspectorDateTimePicker.DroppedDown := FAutoDropDown;
        FEditing := True;
        FEditType := itPicker;
      end;
  end;
...
var
  ...
  D: Double;
begin
  if not Assigned(FEditItem) then
    Exit;
  ...
  case FEditItem.PropertyType of
    ...
    ptFloat, ptText, ptPassword:
      begin
        ...
        val(FInspectorEdit.Text, D, E);
        if E = 0 then
          FInspectorEdit.InspectorItem.FloatValue := D;
...
begin
  ...
  case InspectorItem.PropertyType of
    ...
    ptTextButton, ptFont, ptPropButton, ptStringList:
...
var
  FD: TFontDialog;
  AItem: TInspectorItem;
  SD: TStringListDialog;
begin
  FMouseDown := False;
  AItem := FEditItem;

  if FEditItem.PropertyType = ptStringList then
  begin
    SD := TStringListDialog.Create(self);
    try
      SD.FMemo.Lines.Text := AItem.TextValue;
      if SD.ShowModal = mrOK then
      begin
        AItem.TextValue := SD.FMemo.Lines.Text;
        StopEdit(AItem);
      end;
    finally
      sd.Free;
  ...
  begin
    ...
    begin
      ...
      AItem.FontValue.Height := Round(AItem.FontValue.Height/FDPIScale);
...
begin
  ...
  for i := 0 to Panels.Count - 1 do
  begin
    Panels[i].CaptionFont.Size := MulDiv(Panels[i].CaptionFont.Size, M, D);
    Panels[i].Font.Size := MulDiv(Panels[i].Font.Size, M, D);
    Panels[i].ModifiedFont.Size := MulDiv(Panels[i].ModifiedFont.Size, M, D);

    for j := 0 to Panels[i].Items.Count - 1 do
    begin
      Panels[i].Items[j].FontValue.Size := MulDiv( Panels[i].Items[j].FontValue.Size , M, D);
  ...
  begin
    ...
    {$ENDIF}
      ...
      begin
        ...
        Panels[i].GradientStart := StyleServices.GetStyleColor(scGenericGradientBase);
        Panels[i].GradientEnd := StyleServices.GetStyleColor(scGenericGradientEnd);
      end;
...
begin
  case FStyle of
    ...
    esOffice2019White:
    begin
      Panel.Font.Color := $00444648;
      Panel.GradientDirection := gdVertical;
      Panel.GradientStart := clWhite;
      Panel.GradientEnd := clWhite;
      Panel.GradientSteps := 128;
      Panel.Background := pbGradient;
      Panel.HoverIconColor := $00F2E1D5;
      Panel.HoverIconColorTo := $00F2E1D5;
      Panel.HoverIconDownColor := $00E3BDA3;
      Panel.HoverIconDownColorTo := $00E3BDA3;
      Panel.HoverIconBorderColor := $00F2E1D5;
      Panel.SelectColor := $00F2E1D5;
      Panel.SelectColorTo := $00F2E1D5;
      Panel.SelectDownColor := $00E3BDA3;
      Panel.SelectDownColorTo := $00E3BDA3;
      Panel.SelectFontColor := $003B3B3B;
    end;
    esOffice2019Gray:
    begin
      Panel.Font.Color := $00232425;
      Panel.GradientDirection := gdVertical;
      Panel.GradientStart := $00B8BBBE;
      Panel.GradientEnd := $00B8BBBE;
      Panel.GradientSteps := 128;
      Panel.Background := pbGradient;
      Panel.HoverIconColor := $00969696;
      Panel.HoverIconColorTo := $00969696;
      Panel.HoverIconDownColor :=$00666666;
      Panel.HoverIconDownColorTo := $00666666;
      Panel.HoverIconBorderColor := $00969696;
      Panel.SelectColor := $00969696;
      Panel.SelectColorTo := $00969696;
      Panel.SelectDownColor := $00666666;
      Panel.SelectDownColorTo := $00666666;
      Panel.SelectFontColor := $003B3B3B;
    end;
    esOffice2019Black:
    begin
      Panel.Font.Color := clWhite;
      Panel.GradientDirection := gdVertical;
      Panel.GradientStart := $003B3B3B;
      Panel.GradientEnd := $003B3B3B;
      Panel.GradientSteps := 128;
      Panel.Background := pbGradient;
      Panel.HoverIconColor := $00686868;
      Panel.HoverIconColorTo := $00686868;
      Panel.HoverIconDownColor := $00828282;
      Panel.HoverIconDownColorTo := $00828282;
      Panel.HoverIconBorderColor := $00686868;
      Panel.SelectColor := $00505050;
      Panel.SelectColorTo := $00505050;
      Panel.SelectDownColor := $00828282;
      Panel.SelectDownColorTo := $00828282;
      Panel.SelectFontColor := clWhite;
    end;
  end;
  ...
  begin
    case FStyle of
     ...
     esTerminal:
        ...
        esOffice2019White:
        begin
          PanelCaption.Color := $00FAECE3;
          PanelCaption.ColorTo := $00FAECE3;
          PanelCaption.ActiveFont.Color := $00444648;
          Color := PanelCaption.Color;
          for i := 0 to Panels.Count - 1 do
            SetPanelStyle( esOffice2019White, Panels[i]);
        ...
        esOffice2019Gray:
        begin
          PanelCaption.Color := $00939597;
          PanelCaption.ColorTo := $00939597;
          PanelCaption.ActiveFont.Color := $00232425;
          Color := PanelCaption.Color;
          for i := 0 to Panels.Count - 1 do
            SetPanelStyle( esOffice2019Gray, Panels[i]);
        ...
        esOffice2019Black:
        begin
          PanelCaption.Color := $00444648;
          PanelCaption.ColorTo := $00444648;
          PanelCaption.ActiveFont.Color := clWhite;
          Color := PanelCaption.Color;
          for i := 0 to Panels.Count - 1 do
            SetPanelStyle( esOffice2019Black, Panels[i]);
...
procedure TInspectorBar.SetDropDownButtonWidth(const Value: integer);
begin
  if (FDropDownButtonWidth <> Value) then
  begin
    FDropDownButtonWidth := Value;
    Invalidate;
  end;
end;
...
constructor TStringListDialog.Create(AOwner: TComponent);
begin
