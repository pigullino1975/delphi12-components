...
const
  ...
  // 1.4.0.0 : New : Methods LoadFromFile(), LoadFromStream() added
  //         : New : Function IsEmpty added
  //         : New : Property NoCache added             
  //         : New : Support to assign from TGifImage added   
...
type
  ...
  public
    ...
    property NoCache: Boolean read FNoCache write FNoCache default true;
...
begin
  ...
  begin
    ...
    begin
      TGifImage(Source).SaveToStream(FDataStream);
...
procedure TWebPicture.LoadFromStream(Stream: TStream);
  ...
  function IsIdenticalStreams(Stream1, Stream2: TStream): boolean;
  const
    Block_Size = 4096;
  var
...
begin
  ...
  begin
    if (not FNoCache) and IsIdenticalStreams(Stream, FDataStream) then 
      Exit;
...
begin
  ...
  FIHttp := InternetOpenURL(fISession,pchar(furl),nil,0, fOptions, 0);
...
procedure TWebImage.LoadFromFile(const FileName: string);
begin
  FURL := '';
  If Assigned(FWebPicture) then
    FWebPicture.LoadFromFile(FileName);
end;

procedure TWebImage.LoadFromStream(Stream: TStream);
begin
  FURL := '';
  If Assigned(FWebPicture) then
    FWebPicture.LoadFromStream(Stream);
end;

function TWebImage.IsEmpty: Boolean;
begin
  Result := (not Assigned(FWebPicture)) or FWebPicture.FIsEmpty;
end;
