...
type
  ...
  protected
    ...
    property DarkCheck: boolean read FDarkCheck write FDarkCheck;
...
begin
  ...
  if Result then
  begin
    // prevent themed drawing on Win7
...
//    DrawThemed := false;
  ...
  if DarkCheck or (UIStyle in [tsOffice2019Black, tsOffice2019Gray, tsOffice2016Black, tsOffice2010Black]) then
  begin
    ...
    if CheckBox.State = cbChecked then
      Canvas.TextOut(r.Left, r.Top, chr($78))
    else
      Canvas.TextOut(r.Left, r.Top, chr($6F));
...
begin
  ...
  begin
    Canvas.Font.Assign(Font);
    cw := Canvas.TextWidth(Caption) + 20;
...
begin
  ...
  begin
    ...
    if FGroupBox.DarkCheck or (FGroupBox.UIStyle in [tsOffice2019Black, tsOffice2019Gray, tsOffice2016Black, tsOffice2010Black]) then
      FGroupBox.Invalidate;
  end;
end;
