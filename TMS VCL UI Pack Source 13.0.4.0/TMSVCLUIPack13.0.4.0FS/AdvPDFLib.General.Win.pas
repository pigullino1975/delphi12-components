...
function TAdvGeneralPDFLibFontInitializer.GetGlyphIDs: TAdvPDFGraphicsLibUsedFontCharArray;
begin
  Result := nil;
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          {$IFNDEF LCLLIB}
          {$IF COMPILERVERSION >= 22}
          ec := pointer(NativeInt(@ft^.fmt) + SizeOf(TAdvGeneralPDFLibFontCmapFormat4));
          sc := pointer(NativeInt(ec) + ft^.sgX2 + 2);
          ida := pointer(NativeInt(sc) + ft^.sgX2);
          idr := Pointer(NativeInt(ida) + ft^.sgX2);
          gia := Pointer(NativeInt(idr) + ft^.sgX2);
          {$ELSE}
