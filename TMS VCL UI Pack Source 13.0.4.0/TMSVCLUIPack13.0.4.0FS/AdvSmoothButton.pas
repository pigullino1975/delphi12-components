...
const
  ...
  // v2.2.0.0 : New : Support for Office 2019 styles
  // v2.2.1.0 : New : UIStyle property for TMSStyle
  // v2.2.1.1 : Fixed : UIStyle set to tsCustom after initial Office look
  // v2.2.1.2 : Improved : On Creation check for enabled AdvFormStyler
  // v2.2.1.3 : Fixed : ParentFont kept true on initialization
  // v2.2.1.4 : Fixed : Issue with SimpleLayout property persistence when copying TAdvSmoothButton
  // v2.2.2.0 : New : BiDiMode added
  // v2.2.3.0 : New : Property ColorDown made public
  // v2.2.3.1 : New : OnRightClick event implemented and behavior blocked in TAdvSmoothButton
...
begin
  ...
  Appearance.Font.Height := MulDiv(FAppearance.Font.Height, M, D);
...
procedure TAdvSmoothButton.DoRightClick;
begin
...
begin
  ...
  if (Button = mbRight) and FRightButton then
  begin
    DoRightClick;
  end
  else if Button = mbLeft then
...
//  if FTransparent and not FMouseEnter then
  ...
  begin
    ...
    FAppearance.Draw(g, Caption, 0, 0, Width, Height, vs, hs, c, FColorDown, BevelColor, fc,
      Shadow, FDown and FMouseInside, Bevel, false, BiDiMode = bdRightToLeft, rtBoth, Picture, 0, 0, true, picidx, picname, g.GetTextRenderingHint);
...
begin
  ...
  case astyle of
    ...
    begin
      ...
      Color := clWhite;
      FColorDown := $00E3BDA3;
      Appearance.Font.Color := $00232425;
    end;
    tsOffice2019Gray:
    begin
      Appearance.SimpleLayout := True;
...
procedure TAdvSmoothButton.SetUIStyle(const Value: TTMSStyle);
begin
