...
const
  ...
  // v1.3.0.1 : Improved : On creation check for enabled AdvFormStyler
  // v1.3.0.2 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // v1.3.0.3 : Fixed : Height set when MinimumPanelHeight used in runtime creation
  // v1.3.1.0 : New : Event OnExpandCollapsChange event added
...
type
  ...
  TAdvSmoothExpanderPanelExpandCollapseChange = procedure(Sender: TObject; AExpanded: boolean; var DefaultProcessing: boolean) of object;
  ...
  private
    ...
    FExpanderArrowColor: TColor;
    FOnExpandCollapseChange: TAdvSmoothExpanderPanelExpandCollapseChange;
    ...
    procedure SetExpandedInt(const Value: Boolean; Internal: boolean);
    ...
    procedure SetExpandedState(const Value: boolean);
  protected
    procedure Changed;
  ...
  public
    ...
    property ExpandedState: boolean read GetExpandedState write SetExpandedState;
  published
    ...
    property OnExpandCollapseChange: TAdvSmoothExpanderPanelExpandCollapseChange read FOnExpandCollapseChange write FOnExpandCollapseChange;
  end;
...
begin
  ...
  begin
    ...
    //DRAW SIGN
    p := TGPPen.Create(MakeColor(255, FExpanderArrowColor), 1);
...
function TAdvSmoothExpanderPanel.GetExpandedState: boolean;
begin
  Result := FExpanded;
end;
...
begin
  inherited;
  if Key = VK_F4 then
    SetExpandedInt(not Expanded, true);
end;
...
begin
  ...
  case AStyle of
    ...
    begin
      FFlatStyle := True;
      ExpanderColor := clWhite;
      ExpanderHoverColor := $00FAECE3;
      ExpanderDownColor := $00B0784A;
      ExpanderBorderColor := $00C4C6C8;
    end;
    tsOffice2019Gray:
    begin
      FFlatStyle := True;
      ExpanderColor := $00B8BBBE;
      ExpanderHoverColor := $00939597;
      ExpanderDownColor := $0086888A;
      ExpanderBorderColor := $00666666;
    end;
    tsOffice2019Black:
    begin
      FFlatStyle := True;
      ExpanderColor := $00303132;
      ExpanderHoverColor := $00444648;
      ExpanderDownColor := $005C5E60;
      ExpanderBorderColor := $00ADB0B3;
      FExpanderArrowColor := clWhite;
    end;
  else
  begin
...
begin
  SetExpandedInt(Value, False);
end;

procedure TAdvSmoothExpanderPanel.SetExpandedInt(const Value: Boolean; Internal: boolean);
var
  Allow: boolean;
begin
  ...
  begin
    ...
    begin
      ...
      if Assigned(OnExpandCollapseChange) and Internal then
        OnExpandCollapseChange(Self, FExpanded, Allow);
...
procedure TAdvSmoothExpanderPanel.SetExpandedState(const Value: boolean);
begin
  FExpanded := Value;
end;
...
begin
  ...
  begin
    ...
    begin
      Height := Value;
      FExpHeightTo := Max(FExpHeightTo, FMinimumPanelHeight);
    end;
