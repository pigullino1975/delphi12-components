...
type
  ...
  public
    ...
    procedure Draw(g: TGPGraphics; Caption: String; x, y, Width, Height, VerticalSpacing,
      ...
      RoundingType: TFillRoundingType; APicture: TAdvGDIPPicture; TextW, TextH: integer; WW: boolean; AImageIndex: Integer; APictureName: String; ATextRendering: TTextRenderingHint = TextRenderingHintSystemDefault; ADPIScale: Single = 1);
  ...
  published
    ...
    property FocusColor: TColor read FFocusColor write SetFocusColor default clBlack;
    property SimpleLayout: Boolean read FSimpleLayout write SetSimpleLayout;
  ...
  protected
    ...
    procedure DrawValue(g: TGPGraphics; r: TRect; min, max, position: double; Direction: TGDIPProgressDirection; ADPIScale: Single = 1);
  ...
  public
    ...
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
    procedure Draw(g: TGPGraphics; r: TRect; min, max, position: Double; Direction: TGDIPProgressDirection; ADPIScale: Single = 1);
    {$IFDEF DIRECT2D}
...
var
  ...
  Angle: Integer; HatchStyle:THatchStyle; Picture: TAdvGDIPPicture; LeadingZeros: boolean = true);
...
procedure DrawDigit(g: TGPGraphics; number: Integer; position: TGPPointF; Width, Height: integer;  b: TGPBrush; l: integer; ColorOff: TColor; ColorOffOpacity: Byte; Enabled: boolean);
type
...
procedure DrawLed(g: TGPGraphics; r: TGPRectF; ValueType: TFillValueType; Format: String; TimeFormat: String; Value: Double; TimeValue: TDateTime;
  ColorOff: TColor; ColorOffOpacity: Byte; ColorStart, ColorEnd: TColor; OpacityStart, OpacityEnd: Byte; GradientType: TAdvGradientType;
  Angle: Integer; HatchStyle:THatchStyle; Picture: TAdvGDIPPicture; LeadingZeros: boolean);
var
  num: String;
  i: Integer;
  shift: Double;
...
begin
  ...
  begin
    ...
    begin
      numch := num[i];
      if numch = '.' then
        DrawDigit(g, -3, MakePoint((drect.X + shift), drect.Y), Round(DrecT.Width), Round(drect.Height), textb, total, ColorOff, ColorOffOpacity,true)
      else
      begin
        case numch of
        ':':
          begin
            DrawDigit(g, -2, MakePoint((drect.X + shift), drect.Y), Round(DrecT.Width), Round(drect.Height), textb, total, ColorOff, ColorOffOpacity,true);
          end;
        '-':
          begin
            DrawDigit(g, -1, MakePoint((drect.X + shift), drect.Y), Round(DrecT.Width), Round(drect.Height), textb, total, ColorOff, ColorOffOpacity,true);
        ...
        '0'..'9':
          begin
            if (numch <> '0') then
              isleading := true;
            if (i < length(num)) and (num[i + 1] = '.') then
              isleading := true;


            DrawDigit(g, ord(numch) - ord('0') , MakePoint((drect.X + shift), drect.Y), Round(DrecT.Width), Round(drect.Height), textb, total, ColorOff, ColorOffOpacity, isleading);
            shift := shift + (Drect.Width / total);
          end;
        end;
      end;
...
begin
  ...
  begin
    ...
    try
      ...
      (bmp as TTMSFNCSVGBitmap).Draw(ca, Rect(P.X, P.Y, P.X + bmp.Width, p.Y + bmp.Height));
...
procedure TGDIPProgress.Draw(g: TGPGraphics; r: TRect; min, max, position: Double; Direction: TGDIPProgressDirection; ADPIScale: Single = 1);
var
  bg: TGPRectF;
begin
  if (BackGroundFill.BorderWidth = 0) or (BackGroundFill.BorderColor = clNone) then
  ...
  if ValueVisible then
    DrawValue(g, r, min, max, position, Direction, ADPIScale);
...
procedure TGDIPProgress.DrawValue(g: TGPGraphics; r: TRect; min, max, position: double; Direction: TGDIPProgressDirection; ADPIScale: Single = 1);
var
  f: TGPFont;
  sf: TGPStringFormat;
  b: TGPSolidBrush;
...
begin
  ...
  ft := TFont.Create;
  ft.Assign(Font);
  ft.Height := Round(ADPIScale * ft.Height);

  GetTextSize(g, r, s, ft, sizerect);
  tw := sizerect.Width;
  th := sizerect.Height;
  bn := GetInsideRectF(r);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if ADPIScale > 1 then
        begin
          ca.Font.Height := Round(Font.Height * ADPIScale);
        end;
