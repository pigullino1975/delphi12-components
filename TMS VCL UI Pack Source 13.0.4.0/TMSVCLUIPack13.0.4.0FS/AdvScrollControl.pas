...
uses
  ...
  Controls, SysUtils, Types, TypInfo
  {$IFDEF VCLLIB}
  , uxTheme, Windows
  {$ENDIF}
  {$IFDEF DELPHIXE5_LVL}
  ...
  {$IFDEF FNCLIB}
  {$IFDEF VCLLIB}
  , VCL.TMSFNCTypes, VCL.TMSFNCPersistence, VCL.TMSFNCStyles, VCL.TMSFNCUtils
  {$ENDIF}
  {$IFDEF FMXLIB}
  , FMX.TMSFNCTypes, FMX.TMSFNCPersistence, FMX.TMSFNCStyles, FMX.TMSFNCUtils
  {$ENDIF}
  {$ENDIF}
  ;

const
   TTMSFNCScrollControlDocURL = 'https://download.tmssoftware.com/doc/tmsfncuipack/components/ttmsfncuipack/';
...
type
  ...
  TCustomScrollingControl = class(TScrollingWinControl {$IFDEF FNCLIB} ,ITMSFNCProductInfo, ITMSFNCPersistence {$ENDIF})
  private
    {$IFDEF FNCLIB}
  ...
  protected
    ...
    function CanLoadProperty(AObject: TObject; APropertyName: string; APropertyType: TTypeKind): Boolean; virtual;
    {$ENDIF}
...
procedure TCustomScrollingControl.BeginUpdate;
begin
//
end;
...
function TCustomScrollingControl.IsAppearanceProperty(AObject: TObject; APropertyName: string; APropertyType: TypInfo.TTypeKind): Boolean;
var
...
function TCustomScrollingControl.CanSaveProperty(AObject: TObject; APropertyName: string; APropertyType: TypInfo.TTypeKind): Boolean;
begin
...
function TCustomScrollingControl.CanLoadProperty(AObject: TObject; APropertyName: string; APropertyType: TypInfo.TTypeKind): Boolean;
begin
  if (AObject = Self) then
    Result := (TTMSFNCUtils.IndexOfTextInArray(APropertyName, ExcludePropertyList) = -1)
  else
...
procedure TCustomScrollingControl.SaveSettingsToFile(AFileName: string; AAppearanceOnly: Boolean = False);
begin
...
procedure TCustomScrollingControl.LoadSettingsFromFile(AFileName: string);
begin
  BeginUpdate;
...
procedure TCustomScrollingControl.SaveSettingsToStream(AStream: TStreamEx; AAppearanceOnly: Boolean = False);
begin
  FAppearancePersisting := AAppearanceOnly;
...
procedure TCustomScrollingControl.LoadSettingsFromStream(AStream: TStreamEx);
begin
  BeginUpdate;
...
procedure TCustomScrollingControl.UpdateScrollBars;
begin
//
end;
...
procedure TCustomScrollingControl.WMLButtonDblClk(
...
procedure TCustomScrollingControl.EndUpdate;
begin
