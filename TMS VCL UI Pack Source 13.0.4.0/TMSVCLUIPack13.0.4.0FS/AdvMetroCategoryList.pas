...
const
  ...
  // 1.2.0.0 : New : VCL Styles support added
  // 1.2.0.1 : Fixed : VCL styles check updated to work in 10.4 Sydney
...
begin
  ...
  {$IFDEF DELPHIXE2_LVL}
  DoStyles := CheckVCLStylesEnabled(StyleServices, (csDesigning in ComponentState));
  {$IFDEF DELPHIXE6_LVL}
  DoStyles := DoStyles and (seClient in StyleElements);
  {$ENDIF}
  {$ENDIF}
...
begin
  ...
  begin
    LDetails := StyleServices.GetElementDetails(tgFixedCellNormal);
    if StyleServices.GetElementColor(LDetails, ecTextColor, clr) then
