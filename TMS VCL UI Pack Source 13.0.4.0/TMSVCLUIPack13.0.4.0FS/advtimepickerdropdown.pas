...
uses
  Classes, Windows, Graphics, Controls, Messages, AdvDropDown, ExtCtrls, SysUtils, Math, Dialogs,
  ComCtrls, AdvStyleIF, Types
  {$IFDEF TMSGDIPLUS}
  , AdvGDIP, ActiveX
  {$ENDIF}
  {$IFDEF DELPHIXE2_LVL}
...
const
  ...
  // v1.3.0.0 : New : Support for Office 2019 styles
  // v1.3.1.0 : New : UIStyle property for office look
  //          : Improved : On creation check for enabled AdvFormStyler
  // v1.3.1.1 : Fixed : ParentFont kept true on initialization
  // v1.3.1.2 : Fixed : Issue with Resolution handling
  // v1.3.2.0 : New : Keyboard support added in TAdvWatch
  // v1.3.2.1 : Fixed : Issue with DropDownHeight and ShowTimePicker = true
  // v1.3.2.2 : Improved : Error handling when invalid time was entered
...
begin
  ...
  {$IFDEF DELPHIXE_LVL}
  if (bmp is TTMSFNCSVGBitmap) and (bmp as TTMSFNCSVGBitmap).HasSVG then
  begin
    ca := TCanvas.Create;
    try
      ca.Handle := graphics.GetHDC;
      (bmp as TTMSFNCSVGBitmap).Draw(ca, R);
      graphics.ReleaseHDC(ca.Handle);
    finally
      ca.Free;
    end;
  end
...
begin
  ...
  begin
    ...
    DropDownHeight :=170 + FDateTimePicker.Height + 1;
  end
...
procedure TAdvTimePickerDropDown.CreateWnd;
begin
  inherited;
  if FDesignTime then
    FTMSStyle := tsCustom;
...
procedure TAdvTimePickerDropDown.DoShowDropDown;
begin
  inherited;
  FAdvWatch.SetFocus;
end;
...
begin
  ...
  case AStyle of
    ...
    tsOffice2019White:
    begin
      WatchAppearance.HourFont.Color := $00444648;
      WatchAppearance.AMPMFont.Color := $00444648;
      Font.Color := $00444648;

      WatchAppearance.AMPMBorderColor := clWhite;
      WatchAppearance.AMPMColor := clWhite;
      WatchAppearance.BackgroundColor := clWhite;
      WatchAppearance.BackgroundColorTo := clWhite;
      WatchAppearance.BorderColor := $00C4C6C8;
      WatchAppearance.CenterPointBorderColor := $00E3BDA3;
      WatchAppearance.CenterPointColor := $00E3BDA3;
      WatchAppearance.HourColor := $00B0784A;
      WatchAppearance.MinuteColor := $00444648;
      WatchAppearance.HourPointerShadow := clNone;
      WatchAppearance.HourPointer := $00E3BDA3;
      WatchAppearance.MinutePointerShadow := clNone;
      WatchAppearance.MinutePointer := $00F2E1D5;
      WatchAppearance.SecondPointer := $00FAECE3;
    end;
    tsOffice2019Gray:
    begin
      WatchAppearance.HourFont.Color := $00232425;
      WatchAppearance.AMPMFont.Color := $00232425;

      Color := $00D4D4D4;
      Font.Color := $00232425;

      WatchAppearance.AMPMBorderColor := $00B8BBBE;
      WatchAppearance.AMPMColor := $00B8BBBE;
      WatchAppearance.BackgroundColor := $00B8BBBE;
      WatchAppearance.BackgroundColorTo := $00B8BBBE;
      WatchAppearance.BorderColor := $00666666;
      WatchAppearance.CenterPointBorderColor := $00666666;
      WatchAppearance.CenterPointColor := $00666666;
      WatchAppearance.HourColor := $0086888A;
      WatchAppearance.MinuteColor := $00232425;
      WatchAppearance.HourPointerShadow := clNone;
      WatchAppearance.HourPointer := $00666666;
      WatchAppearance.MinutePointerShadow := clNone;
      WatchAppearance.MinutePointer := $00969696;
      WatchAppearance.SecondPointer := $00939597;
    end;
    tsOffice2019Black:
    begin
       WatchAppearance.HourFont.Color := clWhite;
      WatchAppearance.AMPMFont.Color := clWhite;

      Color := $00444444;
      Font.Color := clWhite;
      WatchAppearance.AMPMBorderColor := $00303132;
      WatchAppearance.AMPMColor := $00303132;
      WatchAppearance.BackgroundColor := $00303132;
      WatchAppearance.BackgroundColorTo := $00303132;
      WatchAppearance.BorderColor := $00ADB0B3;
      WatchAppearance.CenterPointBorderColor := $00828282;
      WatchAppearance.CenterPointColor := $00828282;
      WatchAppearance.HourColor := clWhite;
      WatchAppearance.MinuteColor := clWhite;
      WatchAppearance.HourPointerShadow := clNone;
      WatchAppearance.HourPointer := $00828282;
      WatchAppearance.MinutePointerShadow := clNone;
      WatchAppearance.MinutePointer := $00686868;
      WatchAppearance.SecondPointer := $00444648;
    end;
  end;
end;
...
procedure TAdvTimePickerDropDown.SetUIStyle(const Value: TTMSStyle);
begin
...
procedure TAdvWatch.KeyDown(var Key: Word; Shift: TShiftState);
var
  d: integer;
  h,m,s,s100: word;
  orgtime: TDateTime;
begin
  ...
  VK_NUMPAD0..VK_NUMPAD9: // 0..9
    begin
      ...
      Time := EncodeTime(Key - VK_NUMPAD0,m,s,s100);
    end;
  VK_HOME:
    begin
      DecodeTime(Time, h,m,s,s100);
  ...
  VK_END:
    begin
      DecodeTime(Time, h,m,s,s100);
      Time := EncodeTime(6,0,0,0);
    end;
  end;

  if orgtime <> Time then
    if Assigned(FOnChange) then
      FOnChange(Self);
end;
