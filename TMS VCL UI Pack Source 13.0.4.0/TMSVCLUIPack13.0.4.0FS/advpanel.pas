...
const
  ...
  // v2.5.9.0 : New : Styles for Office 2019 has been added
  // v2.5.9.1 : Fixed : Caption visibility issues
  // v2.5.10.0 : New : Internal Office styler implemented
  // v2.5.10.1 : Improved : Drawing with VCL Styles
  //           : Fixed : Caption was reintroduced with office 2019 internal styler
  //           : Fixed : UIStyle is set to tsCustom after initial office look
  // v2.5.10.2 : Improved : On creation check for enabled AdvFormStyler
  // v2.5.10.3 : Fixed : ParentFont kept true on initialization
  // v2.5.10.4 : Fixed : Issue with clearing caption glyphs when changing the style
  // v2.5.10.5 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // v2.5.10.6 : Fixed : Issue with style initialization
  // v2.5.10.7 : Fixed : Issue with Background image size when starting on high DPI screen
  // v2.5.11.0 : New : Method MoveToGroup(), CopyToGroup() added to move panels between groups
  // v2.6.0.0  : New : Support for Emoji rendering in HTML
  // v2.6.1.0  : New : Uses Padding for rendering & autosize
  //           : New : Uses child controls for autosize
  //           : Fixed : Issue with triggering OnCaptionClick for click on min/max button
  // v2.6.2.0  : New : Public function GetCaptionSize: TSize added
  // v2.6.3.0  : Improved : Rendering on high DPI with Delphi 11
  // v2.6.3.1  : Fixed : Small color issue in Office2013 light gray style
  // v2.6.3.2  : Fixed : Rare alignment issue in Delphi 11 during form loading
  // v2.6.3.3  : Fixed : Memory leak for specific caption drawing
  // v2.6.3.4  : Fixed : Issue with caption font for HTML text on high DPI
...
type
  ...
  public
    ...
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
    procedure AssignFromStyler(Source: TPersistent);
  published
  ...
  TCustomAdvPanel = class(TCustomPanel, ITMSStyleEx, ITMSTones, ITMSOfficeHint)
  private
    FText: string;
    FImage:TAdvImage;
  ...
  public
    ...
    procedure CopyToGroup(FromIndex: integer; TargetGroup: TAdvPanelGroup);
    procedure MoveToGroup(FromIndex: integer; TargetGroup: TAdvPanelGroup);
...
uses
  ShellApi, Commctrl, Math, AdvHTML
{$IFDEF DELPHIXE2_LVL}
  , VCL.Themes
{$ENDIF}
  ;
...
var
  r: TRect;
  GW: Integer;
  DueMinMax,ClickCollaps: Boolean;
...
begin
  ...
  begin
    ...
    begin
      ...
      if not ClickCollaps and Assigned(FOnCaptionClick) then
  ...
  begin
    ...
    begin
      ...
      HTMLDrawEx(ACanvas,FCaption.Text,r,fImages,0,0,-1,FCaptionHoverHyperLink,FShadowOffset,false,false,false,false,false,fHover,false, FFormScaled,
        1.0, FURLColor,FHoverColor,FHoverFontColor,FShadowColor,a,s,fa,xsize,ysize,hl,ml,hr,FImageCache,FContainer,FLineSpacing,bdLeftToRight, clNone, clNone, [], FDPIScale);
    end
    else if FCaption.Position = cpLeft then
    begin
...
begin
  ...
  begin
    FStyler := nil;
    if not FCustomStyler then //Only update style if styler component is removed
    begin
...
begin
  ...
  FImage.SetLogPixX(Round(FDPIScale * 96));
  FImage.SetLogPixY(Round(FDPIScale * 96));

  if FPosition.Save then
    LoadPosition;
...
function TCustomAdvPanel.GetCaptionSize: TSize;
var
  xsize,ysize: integer;
  r,hr: TRect;
  a,s,fa:string;
...
begin
  ...
  Canvas.Font.Assign(Caption.Font);

  HTMLDrawEx(Canvas,Caption.Text,r,FImages,0,0,-1,FHoverHyperLink,fShadowOffset,true,false,false,false,false,fHover,true,FFormScaled,
    1.0, FURLColor,FHoverColor,FHoverFontColor,FShadowColor,a,s,fa,xsize,ysize,hl,ml,hr,FImageCache,FContainer,FLineSpacing,bdLeftToRight, clNone, clNone, [], FDPIScale);
...
begin
  inherited;
  if Assigned(Caption) and ParentFont and not (csDesigning in ComponentState) then
  begin
...
begin
  ...
  {$IFDEF DELPHIXE_LVL}
  XSize := XSize + Padding.Left + Padding.Right;
  YSize := YSize + Padding.Top + Padding.Bottom;
  {$ENDIF}
...
procedure TCustomAdvPanel.SetComponentStyle(AStyle: TTMSStyle);
var
  tempStyler: TAdvPanelStyler;
begin
  if not Assigned(FStyler) then
  begin
    FTMSStyle := AStyle;
    if not (AStyle = tsCustom) then
    begin
      FCustomStyler := True;
      tempStyler := TAdvPanelStyler.Create(Self);
      tempStyler.FSettings.Font.Height := Round(Font.Height / FDPIScale);
      tempStyler.FSettings.Caption.Font.Height := Round(Caption.Font.Height / FDPIScale);
      SetStyler(tempStyler);
      tempStyler.SetComponentStyle(AStyle);
      AssignStyle(tempStyler.Settings);
      tempStyler.Free;
      FCustomStyler := False;
    end;
  end;
...
procedure TCustomAdvPanel.SetComponentStyleAndAppColor(AStyle: TTMSStyle;
  AppColor: TColor);
var
  tempStyler: TAdvPanelStyler;
begin
...
begin
  ...
  Caption.AssignFromStyler(Settings.Caption);
  ...
  begin
    ...
    Caption.Font.Height := Round(Caption.Font.Height * FDPIScale);
  end
  else
    Caption.Font.Assign(Font);
...
procedure TCustomAdvPanel.SetUIStyle(const Value: TTMSSTyle);
begin
  FTMSStyle := Value;
...
procedure TPanelCaption.AssignFromStyler(Source: TPersistent);
begin
  ButtonPosition := (Source as TPanelCaption).ButtonPosition;
  Color := (Source as TPanelCaption).Color;
  ColorTo := (Source as TPanelCaption).ColorTo;
...
procedure TAdvPanelGroup.CopyToGroup(FromIndex: integer;
  TargetGroup: TAdvPanelGroup);
var
  pnl: TCustomAdvPanel;
begin
  if Index < PanelCount  then
  begin
    pnl := TargetGroup.AddPanel;
    pnl.Assign(Panels[FromIndex]);
    pnl.Caption.Visible := Panels[FromIndex].Caption.Visible;
    pnl.Caption.Position := Panels[FromIndex].Caption.Position;
    TargetGroup.ArrangePanels;
  end;
end;
...
procedure TAdvPanelGroup.MoveToGroup(FromIndex: integer;
  TargetGroup: TAdvPanelGroup);
var
  pnl: TCustomAdvPanel;
begin
...
begin
  ...
  case AStyle of
    ...
    tsOffice2019White: ApStyle := psOffice2019White;
    tsOffice2019Gray: ApStyle := psOffice2019Gray;
    tsOffice2019Black: ApStyle := psOffice2019Black;
  end;
...
begin
  ...
  psOffice2019Gray:
    ...
    begin
      ...
      StatusBar.BorderStyle := bsSingle;
      StatusBar.BevelInner := false;
      StatusBar.Color := $00808080;
      StatusBar.ColorTo := clNone;
      StatusBar.GradientDirection := gdVertical;
      StatusBar.Font.Color := $001E1F20;
    end;
...
begin
  ...
  FHeight := 18;
  FFont := TFont.Create;
  FFont.Name := string(DefFontData.Name); // make sure to use a Truetype font
  FFont.Height := -11;
  FFont.OnChange := FontChanged;
end;
