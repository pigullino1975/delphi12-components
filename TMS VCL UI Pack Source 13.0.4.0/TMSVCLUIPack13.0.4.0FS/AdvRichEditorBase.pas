...
type
  {$IFDEF FMXLIB}
  {$IFDEF FNCLIB}
  TPicture = TTMSFNCBitmap;
  TGDIPPicture = TTMSFNCBitmap;
  {$ELSE}
  ...
  public
    ...
    property FontCharset: integer read FFontCharset write FFontCharset;
  ...
  TTabPersistElement = class(TREElement)
  private
    FTabs: string;
  published
  ...
  TTabPersistElementClass = class of TTabPersistElement;
  ...
  public
    procedure Assign(Source: TPersistent); override;
    property Color: TColor read FColor write FColor;
    property EndOfLine: boolean read FEndOfLine write FEndOfLine;
  ...
  public
    procedure AutoComplete(AWord: string; var APrediction: string);
  end;

  TRichEditorAutoCompletion = class(TPersistent)
  private
    FEnabled: boolean;
    FTextColor: TColor;
    FWords: TWordList;
    FCaseSensitive: boolean;
    FNumChars: integer;
    procedure SetWords(const Value: TWordList);
  ...
  published
    ...
    property NumChars: integer read FNumChars write FNumChars default 3;
    property TextColor: TColor read FTextColor write FTextColor default clSilver;
    property Words: TWordList read FWords write SetWords;
  end;
  ...
  TTabPosition = class(TCollectionItem)
  private
    FIndent: integer;
  ...
  TTabPositions = class(TOwnedCollection)
  private
    FOnChange: TNotifyEvent;
    function GetItem(Index: integer): TTabPosition;
    procedure SetItem(Index: integer; const Value: TTabPosition);
  ...
  public
    ...
    function Add: TTabPosition; overload;
    function Add(Indent: integer): TTabPosition; overload;
    function Insert(Index: Integer): TTabPosition;
    property Items[Index: integer]: TTabPosition read GetItem write SetItem; default;
  ...
  private
    ...
    FModifyIndex, FLastModifyIndex: integer;
    {$IFNDEF FMXLIB}
    ...
    FAutoCompletion: TRichEditorAutoCompletion;
    ...
    FLastInsertElement: TREElement;
    FTabPositions: TTabPositions;
    FOnRulerUpdate: TNotifyEvent;
    FRTFAnsiEncoding: boolean;
    FBlockDBUpdate: boolean;
    FZoomFactor: double;
    ...
    procedure SetTabPositions(const Value: TTabPositions);
    procedure SetAutoCompletion(const Value: TRichEditorAutoCompletion);
    function GetAsText: string;
    procedure SetAsText(const Value: string);
  protected
    ...
    function GetDefaultFontCharset: integer;
    function GetLineHeight(AElement: TREElement): integer; virtual; abstract;
    function GetNamedPicture(AElement: TNamedPictureElement): TGDIPPicture; virtual;

    // Calculation functions
    procedure CalcLine(ACanvas: TCanvas; X,Y,FromElementIndex, FromWordIndex, MaxLineWidth: integer; var LineWidth, LineHeight, ToElementIndex, ToWordIndex, Descent, BaseLine, {%H-}Indent: integer; var Alignment: TAlignment);
    ...
    function GetContentAsHTML(UseSelection: boolean; ImgPath: string = ''; PlainHTML: boolean = false; BodyClass: string = ''; Head: string = ''; Target: string = ''): string;
    function GetRTEContent: string;
    procedure SetRTEContent(const Value: string);
    procedure SetIsMerged(const Value: boolean);
    procedure SetZoomFactor(const Value: double); virtual;
    ...
    function GetSize(VertSizeOnly: boolean;FromIndex: integer): TSize;
    function GetLineWidth: integer; virtual;
    function GetSelLengthProp: integer;
    function GetSelStartProp: integer;

    function SkipWords(const Value: string; Count: integer; var CharsSkipped: integer): string;
    ...
    procedure MoveToCaret; virtual;
    procedure UpdateElementXY; virtual;
    ...
    procedure TabPositionsChanged(Sender: TObject); virtual;
    ...
    class function GetFontCharset(AFont: TFont): integer;
    class procedure SetFontCharset(AFont: TFont; ACharset: integer);
    ...
    procedure GetWordsAtCaret(var WordBefore: string; var WordsBefore:string; var WordsAfter: string);
    ...
    function TabAtX(x: integer): integer;
    ...
    procedure UpdateSizeEx(FromIndex: integer);
    ...
    procedure InsertFromStreamInt(const AStream: TStream; f: double; Clipboard: boolean);
    ...
    property BlockDBUpdate: boolean read FBlockDBUpdate write FBlockDBUpdate;
    ...
    property WantReturn: boolean read FWantReturn write FWantReturn;
    property BulletSpacing: integer read FBulletSpacing write FBulletSpacing;
    property LastInsertElement: TREElement read FLastInsertElement write FLastInsertElement;
    property AutoCompletion: TRichEditorAutoCompletion read FAutoCompletion write SetAutoCompletion;
    procedure ClearLastElement;
    procedure ClearSelectionInt(RestoreCaret: boolean = false);
  public
    ...
    function InsertImage(AStream: TStream; AWidth: integer = 0; AHeight: integer = 0): TPictureElement; overload;
    ...
    function SelectURLAtCaret: string;
    ...
    function SelectError(ErrorSelection: TErrorSelection): TREElement;
    ...
    function SetSelectionColor(AColor: TColor): boolean;
    procedure SetSelectionBkColor(AColor: TColor);
    function SetSelectionBold(DoBold: boolean): boolean;
    function SetSelectionItalic(DoItalic: boolean): boolean;
    function SetSelectionUnderline(DoUnderline: boolean): boolean;
    function SetSelectionStrikeOut(DoStrikeOut: boolean): boolean;
    ...
    function SetSelectionSubscript(DoSubScript: boolean): boolean;
    function SetSelectionSuperscript(DoSuperScript: boolean): boolean;
    procedure SetSelectionIndent(AIndent: integer);
    procedure SetSelectionIndentPos(AIndentPos: integer);
    procedure SetSelectionRightIndent(AIndent: integer);
    ...
    function SetSelectionFontName(AName: string): boolean;
    function SetSelectionFontSize(ASize: integer): boolean;
    function SetSelectionFontCharset(ACharset: integer): boolean;
    ...
    function GetSelLength(ASelection: TSelection): integer;
    function GetSelStart(ASelection: TSelection): integer;

    property SelStart: integer read GetSelStartProp write SetSelStart;
    property SelLength: integer read GetSelLengthProp write SetSelLength;
    ...
    property RTFAnsiEncoding: boolean read FRTFAnsiEncoding write FRTFAnsiEncoding;
    ...
    property OnRulerUpdate: TNotifyEvent read FOnRulerUpdate write FOnRulerUpdate;
    ...
    property TabPositions: TTabPositions read FTabPositions write SetTabPositions;
...
function CharIsLowSurrugate(ch: char): boolean;
begin
  {$IFDEF DELPHIXE13_LVL}
  {$IFDEF FMXLIB}
  Result := ch.IsLowSurrogate;
  {$ELSE}
  Result := false;
  {$ENDIF}
  {$ELSE}
...
begin
  ...
  begin
    FColor := ColorToRGB((Source as TTextElement).Color);
    FTextColor := ColorToRGB((Source as TTextElement).TextColor);
    ...
    FIndent := (Source as TTextElement).Indent;
  end;
end;
...
begin
  ...
  begin
    ...
    FFontCharset := (Source as TREElement).FontCharset;
  end;
end;
...
function TREElement.GetLeftIndent: integer;
begin
  Result := Indent and $FFFF;
end;

function TREElement.GetRightIndent: integer;
begin
  Result := (Indent and $FFFF0000) shr 16;
end;
...
procedure TREElement.SetLeftIndent(const Value: integer);
begin
  Indent := (Indent and $FFFF00000) + (Value and $FFFF);
end;

procedure TREElement.SetRightIndent(const Value: integer);
begin
  Indent := (Indent and $FFFF) + ((Value and $FFFF) shl 16);
end;
...
begin
  ...
  // remove superfluous empty element again
  el := FContext.Content.Items[idx + 1];
  el.Free;
  FContext.Content.Delete(idx+1);
...
var
  ...
  begin
    ...
    begin
      ...
      el.FontCharset := GetDefaultFontCharset;
    end;
    Result := el;
  end;
...
begin
  Result := true;
  if Assigned(OnModified) and (FUpdateCount = 0) then
    OnModified(Self);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ch := CharInStr(s, Caret.CharIndex);
        su := Ord(ch);
      ...
      begin
        ...
        if pel is TTabPersistElement then
        begin
          Caret.Element := NextElement(el);
          Caret.CharIndex := 0;
        end
        else
    ...
    begin
      ...
      begin
        if Caret.Element.LeftIndent > 0 then
          Caret.Element.LeftIndent := 0;
      end;
    end
    else
...
procedure TAdvRichEditorBase.CalcLine(ACanvas: TCanvas; X,Y, FromElementIndex, FromWordIndex, MaxLineWidth: integer;
...
begin
  ...
  firstwrd := FromWordIndex = 0;

// TDebug.Write('Calc line: '+ el.ClassName + ':' + el.Text);

  if (FromWordIndex = 0) and (Y <> -1) and (el.Alignment = taLeftJustify) then
  begin
    el.XY := pt;
  end;
  ...
  begin
    ...
    nel.XY := Point(nel.XY.X + Round(nel.LeftIndent * DPIRatio), nel.XY.Y);
    if nel.Alignment = taLeftJustify then
      pt.X := Round(nel.LeftIndent * DPIratio) + Offsetx;
  end;
  ...
  LineHeight := sz.cy;
  LeftIndent := OffsetX + Round(el.LeftIndent * DPIratio);
  ...
  if (LineWidth >= MaxLineWidth - Round(el.RightIndent * DPIRatio)) then
  begin
  ...
  while (LineWidth < MaxLineWidth - Round(el.RightIndent * DPIRatio)) do
  begin
    if (el is TLineBreakElement) then
    begin
      el.XY := pt;
      ...
      begin
        ...
        if (sz.cy > LineHeight) and not (PreviousElement(el) is TLineBreakElement) then
          LineHeight := sz.cy;
    ...
    begin
      pt.X := offsetx + Round(el.LeftIndent * DPIratio);
      el.XY := pt;
    end;
    ...
    begin
      el.XY := pt;
      el.XY := Point(offsetx + Round(el.LeftIndent * DPIratio), pt.Y);
      ...
      LineWidth := Round(el.LeftIndent * DPIratio) + Offsetx;
    ...
    begin
      el.XY := pt;
      el.XY := Point(el.XY.X + Round(el.LeftIndent * DPIratio), pt.Y);

      //sz.cx := Round(el.Size.cx * DPIratio);
      //sz.cy := Round(el.Size.cy * DPIratio);

      sz.cx := Round(el.Size.cx * FZoomFactor);
      sz.cy := Round(el.Size.cy * FZoomFactor);
    ...
    begin
      ...
      tx := TabAtX(LineWidth);
      (el as TTabElement).TabWidth := tx - LineWidth;
      LineWidth := tx;
    ...
    begin
      ...
      if (LineWidth + sz.cx < MaxLineWidth - Round(el.RightIndent * DPIRatio)) or ((Offsetx + sz.cx > MaxLineWidth - Round(el.RightIndent * DPIRatio)) and (FromWordIndex = ToWordIndex) and (sz.cx > GetClientWidth - Round(el.RightIndent * DPIRatio))) then
      begin
        LineWidth := LineWidth + sz.cx;
        pt.X := pt.X + sz.cx;
        inc(ToWordIndex);
      ...
      begin
        if (s = '') and (FromWordIndex = ToWordIndex) and (LeftIndent + el.LeftIndent + sz.cx >= MaxLineWidth - Round(el.RightIndent * DPIRatio)) then
        begin
          LineWidth := LineWidth + sz.cx;
        ...
        begin
          ...
          begin
            ...
            if firstwrd then
            begin
              // LineWidth := LineWidth + sz.cx + 4;
              // compensate for extra word to be rendered on next line
              if (ToElementIndex > 1) then
              begin
                el := FContext.Content.Items[ToElementIndex];
                if el is TTextElement then
                begin
                  dec(ToElementIndex);
                  ToWordIndex := LastWord;
                end;
              end
//              else
//                inc(ToWordIndex);
...
var
  ...
  IsSel, IsInError, IsDef, Error: boolean;
  carX: integer;
begin
  ...
  KEY_RIGHT:
    begin
      ...
      // force immediate repaint for immediate caret recalculation
      Repaint;

      if Caret.XY.X - TopLeft.X >= GetClientWidth - offsetx then
        ScrollHorz(Caret.XY.X - GetClientWidth + offsetx);

      if Caret.XY.Y + Caret.LH - TopLeft.Y >= GetClientHeight - offsety then
        ScrollVert(Caret.XY.Y + Caret.LH - GetClientHeight + offsety);
    end;
  KEY_LEFT:
    begin
      UpdateElementXY;
      ...
      if Caret.XY.Y - offsety < TopLeft.Y then
        ScrollVert(Caret.XY.Y - offsety);
    end;
  KEY_PRIOR:
    begin
      UpdateElementXY;
  ...
  VK_B:
    begin
      ...
      begin
        IsDef := SetSelectionBold(not IsSelectionBold);
        DoChanged;
      end;
    end;
  VK_I:
    begin
      if AllowFormatShortCuts then
      begin
        IsDef := SetSelectionItalic(not IsSelectionItalic);
        DoChanged;
      end;
    end;
  VK_U:
    begin
      if AllowFormatShortCuts then
      begin
        IsDef := SetSelectionUnderline(not IsSelectionUnderline);
        DoChanged;
      end;
    end;
  VK_L:
...
begin
  ClearSelectionInt(True);
end;

procedure TAdvRichEditorBase.ClearSelectionInt(RestoreCaret: boolean = false);
var
  el: TREElement;
  idx: integer;
begin
...
procedure TAdvRichEditorBase.ClearLastElement;
begin
  FLastElement := nil;
end;
...
begin
  ...
  FTabPositions := TTabPositions.Create(Self);
  FAutoCompletion := TRichEditorAutoCompletion.Create;
  FTabPositions.OnChange := TabPositionsChanged;
  FSelectionElements := TElements.Create;
  FFindPos := TCaret.Create;
  FSelectionType := stWord;
  FLineSpacing := 0;
  FRTFAnsiEncoding := false;
    ...
    begin
      ...
      begin
        ...
        begin
          if Length(s) > 0 then
          begin
            ch := CharInStr(s, Caret.CharIndex + 1);
            su := Ord(ch);
            if su and $D800 = $D800 then
              su := 2
            else
      ...
      if (Caret.Element is TLineBreakElement) or (Caret.Element is TTabElement) then
      begin
        el := NextElement(Caret.Element);
...
begin
  ...
  if Assigned(OnRulerUpdate) then
    OnRulerUpdate(Self);
end;
...
begin
  ...
  begin
    ...
    nel.XY := Point(X + nel.LeftIndent + SP.X, Y + SP.Y);
  end;

  //LeftIndent := OffsetX + Round(nel.Indent * DPIratio);

  X := X + Round(nel.LeftIndent * DPIratio);
  ...
  while (LineWidth < MaxLineWidth - Round(el.RightIndent * DPIRatio) - SP.X) do
  begin
    el.Selected := false;
    ...
    if (el is TBulletStart)  then
    begin
      x := orgx + Round(el.LeftIndent * DPIratio);
      if (el = Caret.Element) then
      begin
        if Caret.CharIndex = 0 then
          CalcCaret(ACanvas,el, x, y + SP.Y, LineHeight, Descent, BaseLine, ' ', 0, DoDraw)
    ...
    if (el is TLineBreakElement) then
    begin
      if (el.Selected) and not HideSelection and ((Selection.FromElement <> el) or (Selection.ToElement <> el)) and DoDraw then
      ...
      begin
        ...
        begin
          ...
          taLeftJustify: cx := OffsetX + el.LeftIndent;
          taCenter: cx := (MaxLineWidth - el.RightIndent) div 2;
          taRightJustify: cx := MaxLineWidth - el.RightIndent;
          else
            cx := 0;
          end;
      ...
      el.XYE := Point(SP.X + el.LeftIndent, Y + SP.Y + LineHeight);

      if (el is TLineElement) then
      begin
        DrawElement(ACanvas, el, SP.X, Y, MaxLineWidth - Round(el.RightIndent * DPIRatio), LineHeight, Descent, BaseLine, '', 0, 0);
      end;
    ...
    begin
      ...
      DrawElement(ACanvas, el, x - sz.cx - 2 - SP.X, y, MaxLineWidth - Round(el.RightIndent * DPIRatio), LineHeight, Descent, BaseLine, '', CharsSkipped, 0);
    end;
    ...
    begin
      ...
      sz.cx := Round(el.Size.cx * DPIratio * FZoomFactor);
      sz.cy := Round(el.Size.cy * DPIratio * FZoomFactor);

      (el as TCustomGraphicElement).DPIratio := DPIratio;

      if DoDraw then
        DrawElement(ACanvas, el, x - SP.X, y, MaxLineWidth - Round(el.RightIndent * DPIRatio), LineHeight, Descent, BaseLine, '', CharsSkipped, 0);
...
//      TDebug.Write(el.XYE);
    ...
    begin
      ...
      if (x + sz.cx < MaxLineWidth - Round(el.RightIndent * DPIRatio) - SP.X) or ((x + sz.cx >= MaxLineWidth - Round(el.RightIndent * DPIRatio) - SP.X) and (FromWordIndex = ToWordIndex) and firstwrd {and (LeftIndent + el.Indent + x + sz.cx >= MaxLineWidth)}) then
      begin
        if DoDraw then
        begin
          if s <> '' then
            DrawElement(ACanvas, el, x - SP.X, y, MaxLineWidth - Round(el.RightIndent * DPIRatio), LineHeight, Descent, BaseLine, su + ' ', CharsSkipped, 1)
          else
            DrawElement(ACanvas, el, x - SP.X, y, MaxLineWidth - Round(el.RightIndent * DPIRatio), LineHeight, Descent, BaseLine, su, CharsSkipped, 0);
        end;
  ...
  begin
    if Caret.EndOfLine and (dskip = 0) then
     Exit;
...
begin
  ...
  begin
    ...
    begin
      dot := Pos('.', Value);
      if (dot > amp) and (dot >= Length(TrimRight(Value)) - 3) and (dot < Length(TrimRight(Value)) - 2) then
        Result := true;
    end;
  end;
end;
...
procedure TAdvRichEditorBase.ResetUndoRedo;
begin
  ClearContext;
end;
...
function TAdvRichEditorBase.GetSelectionCount: integer;
begin
  GetSelectionElements;
  Result := FSelectionElements.Count;
end;
...
begin
  ...
  if Assigned(Caret.Element) then
    Result := Caret.Element.LeftIndent;
end;

function TAdvRichEditorBase.GetSelectionRightIndent: integer;
begin
  Result := 0;

  if Assigned(Caret.Element) then
    Result := Caret.Element.RightIndent;
end;
...
function TAdvRichEditorBase.GetSelLengthProp: integer;
begin
...
function TAdvRichEditorBase.GetSelStartProp: integer;
begin
...
function TAdvRichEditorBase.GetSize(VertSizeOnly: boolean; FromIndex: integer): TSize;
var
  x,y,i: integer;
...
begin
  ...
  for i := FromIndex to FContext.Content.Count - 1 do
  begin
  ...
  FElementIndex := FromIndex;

  if (FromIndex > 0) and (FContext.Content.Count > 0) then
  begin
    y := OffsetY + FContext.Content.Items[FromIndex].XYE.Y;
    x := FContext.Content.Items[FromIndex].XYE.X;
  end;
  ...
  begin
    CalcLine(Canvas, X, Y, FElementIndex, WordIndex, GetLineWidth, LW, LH, ToElementIndex, ToWordIndex, Descent, Baseline, Indent, FAlign);

    y := y + LH;
    if LW > MLW then
      MLW := LW;

    for i := FElementIndex to ToElementIndex do
    begin
      if i < FContext.Content.Count then
        FContext.Content[i].LH := LH;
    end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          (Caret.Element as TTextElement).Text := s;
          CaretToSelection;
          LastInsertElement := Caret.Element;
        ...
        begin
          ...
          begin
            Caret.CharIndex := Caret.CharIndex + Length(Value) - 1;
          end;
...
begin
  Result := InsertImage(AStream, 0, 0);
end;

function TAdvRichEditorBase.InsertImage(AStream: TStream; AWidth: integer = 0; AHeight: integer = 0): TPictureElement;
var
  lb: TPictureElement;
begin
  if HasSelection then
...
begin
  Result := (ch = ' ') or (ch = ';') or (ch = '.') or (ch = '-') {or (ch = ':')} or (ch = ',')  or (ch = ')') or (ch = '(') or (ch = '[') or (ch = ']') or (ch = '}') or (ch = '{');
end;

function TAdvRichEditorBase.IsDelimiter(ch: string): boolean;
begin
  Result := (ch = ' ') or (ch = ';') or (ch = '.')  or (ch = '-') {or (ch = ':')} or (ch = ',')  or (ch = ')') or (ch = '(') or (ch = '[') or (ch = ']') or (ch = '}') or (ch = '{')  {or (ch = '''') } or (ch = '"');
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if el.LeftIndent >= BULLETINDENT then
            el.LeftIndent := Max(0, el.LeftIndent - BULLETINDENT);
        end;
    ...
    begin
      ...
      buls.LeftIndent := BULLETINDENT;
      buls.StartIndent := el.LeftIndent;
  ...
  begin
    ...
    begin
      ...
      el := FContext.Content.Items[i];
      bulind := Max(0, el.LeftIndent - el.BulletIndent);
      break;
    end;
    dec(i);
  end;
  ...
  begin
    FContext.Content.Items[i].LeftIndent := bulind;
...
begin
  ...
  begin
    ...
    begin
      ...
      AIndent := (FContext.Content.Items[i] as TBulletElement).LeftIndent;
      Result := false;
      break;
    end;
...
begin
  InsertFromStreamInt(AStream, f, false);
end;

procedure TAdvRichEditorBase.InsertFromStreamInt(const AStream: TStream; f: double; Clipboard: boolean);
var
  es: TElementSaver;
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        try
          ...
          if esc.ElementClass = 'TTextElement' then
            ...
            if (esc.ElementClass = 'TTabPersistElement') then
            begin
              el := TREElement(TTabPersistElementClass(Cref).Create);
              tp := TTabPersistElement(el);
            end
          else
            el := TREElement(TREElementClass(Cref).Create);
      ...
      if Assigned(tp) then
      begin
        TabPositions.AsText := tp.Tabs;

        if Assigned(OnRulerUpdate) then
          OnRulerUpdate(Self);
      end;

      // delete TTabPersistElement that is just used for persistence
      if (FContext.Content.Count > 0) and (FContext.Content[0] is TTabPersistElement) then
      begin

        if (Caret.Element = FContext.Content[0]) then
        begin
          if FContext.Content.Count > 1 then
            Caret.Element := FContext.Content[1]
          else
            Caret.Element := nil;
        end;

        FContext.Content[0].Free;
        FContext.Content.Delete(0);
      end;
    end;
  end;
...
begin
  ...
  begin
    ...
    Classes.RegisterClass(TTabPersistElement);
  end;
end;
...
begin
  ...
  AState.Tags := Tags;
  AState.ColorC := ColorToRGB(Color);
  AState.DefaultFont.Assign(Font);
  AState.TextColorC := ColorToRGB(GetDefaultFontColor);
...
begin
  ...
  InsertFromStreamInt(AStream, f, true);
end;
...
begin
  ...
  tp := TTabPersistElement.Create;
  tp.Tabs := TabPositions.AsText;
  ...
  try
    ...
    try
      if not ((FContext.Content.Count > 0) and (FContext.Content[0] is TTabPersistElement)) then
      begin
        // write tab positions
        esc.ElementClass := tp.ClassName;
        AStream.WriteComponent(esc);
        es.SaveElement := tp;
...
begin
  ...
  begin
    CalcLine(Canvas,0, -1, FElementIndex, WordIndex, MaxLineWidth, LW, LH, ToElementIndex, ToWordIndex, Descent, BaseLine, Indent, FAlign);
...
begin
  ...
  begin
    ...
    begin
      if (Caret.Element is TTextElement) then
      begin
        ch := CharInStr((Caret.Element as TTextElement).Text, Caret.CharIndex + 1);
        su := Ord(ch);
        if su and $D800 = $D800 then
          Caret.CharIndex := Caret.CharIndex + 1;
      end;
...
begin
  ...
  begin
    ...
    if elindent = 0 then
      fromel.LeftIndent := BULIND + startindent;
...
begin
{$IFDEF FMXLIB}
  Result := inherited IsFocused or AlwaysFocus;
{$ENDIF}
{$IFNDEF FMXLIB}
...
class function TAdvRichEditorBase.GetFontCharset(AFont: TFont): integer;
begin
{$IFDEF FMXLIB}
  Result := 0;
{$ENDIF}
...
class procedure TAdvRichEditorBase.SetFontCharset(AFont: TFont; ACharset: integer);
begin
{$IFNDEF FMXLIB}
...
function TAdvRichEditorBase.GetDefaultFontCharset: integer;
begin
  Result := GetFontCharset(GetDefaultFont);
end;
...
begin
  ...
  if PageWidth = 0 then
  begin
    Result := Round(Max(FTextWidth, GetClientWidth - OffsetX + PageMargin.Left));
  end
  else
  begin
    Result := PageWidth;
  ...
  Result := Result - PageMargin.RightIndent;
end;
...
function TAdvRichEditorBase.GetNamedPicture(AElement: TNamedPictureElement): TGDIPPicture;
begin
  Result := nil;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        y := FContext.Content.Items[FElementIndex].XY.Y;
        x := FContext.Content.Items[FElementIndex].XY.X;
        inc(FElementIndex);
      end;
    end;

    // if current element was drawn already, start from know element start xy
    if (FElementIndex < FContext.Content.Count) and (FContext.Content.Items[FElementIndex].XY.Y <> 0) then
    begin
      y := FContext.Content.Items[FElementIndex].XY.Y;
      x := FContext.Content.Items[FElementIndex].XY.X;
    end;
    ...
    while (FElementIndex < FContext.Content.Count) do
    begin
      CalcLine(ACanvas, x, y, FElementIndex, WordIndex, MaxLineWidth, LW, LH, ToElementIndex, ToWordIndex, Descent, BaseLine, Indent, FAlign);
    ...
    begin
      sz := GetTextSize(ACanvas, nil, TextForHeight);
      Descent := GetTextDescent(ACanvas, nil);
      LH := sz.cy;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (Caret.Element is  TLineBreakElement) or (Caret.Element is TGraphicElement) then
      begin
        Caret.CharIndex := 0;
      end;
...
function TAdvRichEditorBase.TabAtX(x: integer): integer;
var
  d,i,m: integer;
begin
  d := x div TabSize;
  ...
  begin
    ...
    begin
      if (TabPositions.Items[i].Indent + PageMargin.Left > x) and (TabPositions.Items[i].Indent +  PageMargin.Left < m)  then
      begin
        m := TabPositions.Items[i].Indent + PageMargin.Left;
      end;
    end;
...
procedure TAdvRichEditorBase.TabPositionsChanged(Sender: TObject);
begin
  Refresh;
end;

{$IFDEF VCLLIB}
procedure TAdvRichEditorBase.MeasureGetSize;
var
  t: dword;
begin
  t := gettickcount;
...
function TAdvRichEditorBase.GetAsText: string;
var
  sel: TSelection;
begin
  Result := '';
...
begin
  ...
  begin
    ...
    if FContext.Content.Items[i] is TLineBreakElement then
      ...
      su := FContext.Content.Items[i].DisplayText;
  ...
  s := FContext.Content.Items[i].DisplayText;
...
procedure TAdvRichEditorBase.SetZoomFactor(const Value: double);
begin
  FZoomFactor := Value;
end;
...
begin
    ...
    begin
      ...
      repeat
        ...
        begin
          ...
          s := nel.DisplayText;
          if (nel is TTextElement) and (nel.Alignment = orgalign) then
          begin
  ...
  begin
    ...
    Caret.CharIndex := idxto;
    Caret.Element := elto;
  end;
end;

procedure TAdvRichEditorBase.GetWordsAtCaret(var WordBefore: string; var WordsBefore,
  WordsAfter: string);
var
  AElement: TREElement;
  el,nel,clickel: TREElement;
  ...
  function CheckEOL(ch: string): boolean;
  begin
    Result := IsDelimiter(ch) and not IsSpace(ch);
  end;
...
begin
  ...
  AElement := Caret.Element;
  AIndex := Caret.CharIndex;
  ...
  begin
    ...
    begin
      ...
      repeat
        while (idx > 0) and not CheckEOL(StrInStr(s,idx)) do
        begin
          dec(idx);
        end;

        WordsBefore := Copy(s, idx + 1, scanidx - idx) + WordsBefore;

        if (idx = 0) and (elidx > 0) and not CheckEOL(Copy(s,idx,1)) then
        begin
          dec(elidx);
          ...
          if (nel is TTextElement) and (nel.Alignment = orgalign) then
          begin
            if CheckEOL(Copy(s,Length(s),1)) then
              found := true
            else
            begin
              idx := Length(s);
      ...
      repeat
        while (idx <= Length(s)) and not CheckEOL(Copy(s,idx,1)) do
          inc(idx);

        WordsAfter := WordsAfter + Copy(s, scanidx, idx - (clickidx + 1));
  ...
  begin
    ...
    begin
      if not IsSpace(Copy(s,idx,1)) then
        WordBefore := Copy(s,idx,1) + WordBefore
      else
        Break;
    end;
  end;
...
function TAdvRichEditorBase.SelectURLAtCaret: string;
var
  ci: integer;
begin
  ci := 0;
  Result := GetWordAtCaret(Caret.Element, Caret.CharIndex, ci, True, True);
end;
...
procedure TAdvRichEditorBase.SetAsText(const Value: string);
begin
  Clear;
  AddMultiLineText(Value);
end;

procedure TAdvRichEditorBase.SetAutoCompletion(const Value: TRichEditorAutoCompletion);
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      firstline := (el is TLineBreakElement);
      ...
      while Assigned(el) and ((el is TTextElement) or firstline or (el is TCustomGraphicElement)) do
      begin
        el.Alignment := AAlignment;
        el := PreviousElement(el);
        firstline := false;
...
function TAdvRichEditorBase.SkipWords(const Value: string; Count: integer; var CharsSkipped: integer): string;
var
  len, vp, i: integer;
begin
  if Value = '' then
  ...
  begin
    ...
    Dec(vp);
    CharsSkipped := vp;
    Result := Copy(Value, CharsSkipped + 1, len);
  end;
end;
...
procedure TAdvRichEditorBase.UpdateElementXY;
begin
  if (FModifyIndex <> FLastModifyIndex) then
  begin
    FLastModifyIndex := FModifyIndex;
    // force element XY, XYE recalculation
    Clip := false;
    Repaint;
    Clip := true;
  end;
...
procedure TAdvRichEditorBase.UpdateSizeEx(FromIndex: integer);
var
  sz: TSize;
  cond: boolean;
begin
  ...
  if SetRange(sz) or cond then
  begin
    SetRange(GetSize(False,FromIndex));
  end;
  FUpdateCount := 0;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if xy.X + sz.cx >= GetLineWidth - Round(el.RightIndent * DPIRatio) then
      begin
        xy.X := el.LeftIndent + OffsetX;
        ...
        begin
          ...
          begin
            ...
            if cws + szs.cx >= GetLineWidth - Round(el.RightIndent * DPIRatio) then
            begin
              ss := '';
              linecross := true;
            end
          ...
          if not linecross and Assigned(nel) then
            cws := cws + GetLineWidth - Round(el.RightIndent * DPIRatio) - nel.XY.X;

          case el.Alignment of
          taRightJustify: xy.X := OffsetX + Round(GetLineWidth - (el.RightIndent * DPIRatio) - cws - 1);
          taCenter: xy.X := OffsetX + Round(GetLineWidth - (el.RightIndent * DPIRatio) - cws - 1) div 2;
          end;
        end;
    ...
    begin
      ...
      begin
        ...
        begin
          // is after last word, so put caret as last char
          if not Assigned(nel) and (X > xy.X + sz.cx) then
          begin
            CX := xy.X + sz.cx;
            idx := idx + length(s);
          end
      ...
      begin
        cw := GetCharInWord(el, s, X - xy.X + SP.X);
...
begin
  ...
  if Assigned(el) and (el is TBulletEnd) then
    el := PreviousElement(el);
...
procedure TAdvRichEditorBase.MoveToCaret;
begin
  // force full caret recalculation
  Clip := false;
  UpdateSize;
  Repaint;
  Clip := true;
...
function TAdvRichEditorBase.SetSelectionSubscript(DoSubScript: boolean): boolean;
var
  i: integer;
begin
  Result := false;
...
function TAdvRichEditorBase.SetSelectionSuperscript(DoSuperScript: boolean): boolean;
var
  i: integer;
begin
  Result := false;
...
function TAdvRichEditorBase.SetSelectionBold(DoBold: boolean): boolean;
var
  i: integer;
begin
  Result := false;
...
function TAdvRichEditorBase.SetSelectionStrikeOut(DoStrikeOut: boolean): boolean;
var
  i: integer;
begin
  Result := false;
...
function TAdvRichEditorBase.SetSelectionColor(AColor: TColor): boolean;
var
  i,cnt: integer;
  clr: TColor;
begin
...
function TAdvRichEditorBase.SetSelectionFontCharset(ACharset: integer): boolean;
var
  i: integer;
  foundbul: boolean;
begin
  ...
  begin
    ...
    begin
      for i := 0 to FSelectionElements.Count - 1 do
      begin
        SetFontCharset(FSelectionElements[i].Font, ACharset);
      end;
...
function TAdvRichEditorBase.SetSelectionFontName(AName: string): boolean;
var
  i: integer;
  foundbul: boolean;
begin
...
function TAdvRichEditorBase.SetSelectionFontSize(ASize: integer): boolean;
var
  i: integer;
  foundbul: boolean;
...
begin
  ...
  begin
    ...
    begin
      ...
      BulIndent := BULLETINDENT + fel.LeftIndent;

      inds := TBulletStart.Create;
      inds.Font.Assign(fel.Font);
      inds.Alignment := fel.Alignment;
      inds.LeftIndent := BulIndent;
      inds.StartIndent := fel.LeftIndent;
      ...
      bul.LeftIndent := BULLETINDENT + fel.LeftIndent;
      ...
      begin
        FSelectionElements.Items[j].LeftIndent := BulIndent;
      ...
      begin
        ...
        begin
          ...
          bul.BulletFont := AFontName;
          bul.LeftIndent := BulIndent; //BULLETINDENT + nel.Indent;
          bul.Spacing := BulletSpacing;
...
function TAdvRichEditorBase.SelectError(ErrorSelection: TErrorSelection): TREElement;
...
procedure TAdvRichEditorBase.SetSelectionRightIndent(AIndent: integer);
var
  i: integer;
  el,pel: TREElement;
begin
  ...
  begin
    ...
    begin
      ...
      if not (el is TBulletEnd) then
        el.LeftIndent := Max(el.BulletIndent,el.LeftIndent + AIndent);
    end;
  end;
...
procedure TAdvRichEditorBase.SetSelectionIndentPos(AIndentPos: integer);
var
  i: integer;
  el,pel: TREElement;
begin
  ...
  begin
    ...
    begin
      ...
      if el is TBulletStart then
        (el as TBulletStart).StartIndent := Max(0, AIndentPos);

      if not (el is TBulletEnd) then
        el.LeftIndent := Max(el.BulletIndent, AIndentPos);
    end;
  end;
...
function TAdvRichEditorBase.SetSelectionItalic(DoItalic: boolean): boolean;
var
  i: integer;
begin
  Result := false;
...
function TAdvRichEditorBase.SetSelectionUnderline(DoUnderline: boolean): boolean;
var
  i: integer;
begin
  Result := false;
...
procedure TAdvRichEditorBase.SetTabPositions(const Value: TTabPositions);
begin
...
begin
  ...
  begin
    ...
    TMSLogger.Info(inttostr(i)+'|'+el.ClassName+':'+al+':'+inttostr(el.LeftIndent)+' ['+inttostr(el.XY.X)+','+inttostr(el.XY.Y)+'] '+el.Text);
    inc(i);
  end;
  {$ENDIF}
end;
...
{$IFDEF TMSDEBUG}
  ...
  begin
    ...
    outputdebugstring(pchar(inttostr(i)+'|'+el.ClassName+':'+al+':'+inttostr(el.LeftIndent)+' ['+inttostr(el.XY.X)+','+inttostr(el.XY.Y)+'] {'+GetFontName(el.Font)+FontStyleAsString(el.FontStyle)+'-' + inttostr(el.GetFontSize)+'} |' + txt + '| -clr:'+inttohex(el.Color,6) +'|' + el.Indent.ToString+'|'));
...
function TAdvRichEditorBase.GetContentAsHTML(UseSelection: boolean; ImgPath: string = ''; PlainHTML: boolean = false; BodyClass: string = ''; Head: string = ''; Target: string = ''): string;
var
  sl: TStringList;
  i,curindent: integer;
...
begin
  ...
  begin
    ...
    BodyClassStr := '';
    if BodyClass <> '' then
      BodyClassStr := 'class="'+ BodyClass+'" ';

    if Head <> '' then
    sl.Add(Head);

    sl.Add('<BODY '+ BodyClassStr+ 'style=''font-family:"'+ GetFontName(Font)+'";font-style:'+ fstyle+';font-weight:'+fweight+';font-size:'+inttostr(GetFontSize(Font))+'pt''>');
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        if Target = '' then
          ...
          s := s + '<A href="'+ StringReplace(el.URL, ' ', '%20',[rfReplaceAll]) +'" target="'+ Target+ '">';
        isaurl := true;
      end;
    end;
    ...
    if el.LeftIndent <> curindent then
    begin
      if (curindent > 0) then
        s := s + '</DIV>';

      curindent := el.LeftIndent;
      if (curindent > 0) then
      begin
        if fntclose or isfntclr then
        begin
      ...
      begin
        ...
        igFile:
          begin
            ...
            if (imgpath <> '') and (imgpath <> '.\') then
              ...
              s := s + '<IMG src="' + imgname + '"' + picsz + imgtag + '>';
        ...
        igInline:
          begin
            ...
            s := s + preamble + EncodePicture(pic.Picture) + '"' + picsz + imgtag+ '>'
          end;
        igID:
          begin
            s := s + '<IMG src="cid:image' + inttostr(imgidx) + '"' + imgtag+ '">';
  ...
  try
    ...
    begin
      ...
      begin
        ...
        bulStart.LeftIndent := BULLETINDENT;
        bulidx := 0;
        isbul := true;
        Continue;
      end;
      ...
      begin
        ...
        if doc.ElementTable.Items[i].PicWidth <> -1 then
          picel.PictureWidth := doc.ElementTable.Items[i].PicWidth;
        if doc.ElementTable.Items[i].PicHeight <> -1 then
          picel.PictureHeight := doc.ElementTable.Items[i].PicHeight;

        picel.Alignment := doc.ElementTable.Items[i].Alignment;
      ...
      begin
        ...
        fcsn := doc.ElementTable.Items[i].FontCharset;

        if (fcsn <> fcs) then
        begin
          SetSelectionFontCharset(fcsn);
          fcs := fcsn;
        end;
...
//          if FSelectionElements.Count > 0 then
        ...
        if (ind <> doc.ElementTable.Items[i].Indent) and not (pel is TBulletElement) and not (pel is TBulletStart) then
        begin
        ...
        begin
          ...
          if (fn = '') and (doc.FontTable.Count > 0) then
            fn := doc.FontTable[0].Name;
          SetSelectionFontName(fn);
        end;
        ...
        begin
          ...
          begin
            //Caret.Element := nil;
            if isBul then
              txtEl := AddText(doc.ElementTable.Items[i].Text)
            else
...
//            if bulidx > 0 then
//              txtEl.LeftIndent := BULLETINDENT;
          end
          else
          begin
            re := InsertLineBreak(false);
            re.FontSize := doc.ElementTable.Items[i].FontSize;
            pel := PreviousElement(re);
            if Assigned(pel) then
              re.Alignment := pel.Alignment;
//            if bulidx > 0 then
//              re.LeftIndent := BULLETINDENT;
          end;
        end;
...
begin
  ...
  FRTFEngine.AnsiEncoding := FRTFAnsiEncoding;
  ...
  begin
    ...
    if (curindent <> el.LeftIndent)  then
    begin
      curindent := el.LeftIndent;
      if not (el is TBulletEnd) then
      begin
    ...
    begin
      ...
      {$ENDIF}
      {$IFDEF FMXLIB}
      FRTFEngine.AddBitmap((el as TPictureElement).Picture, (el as TPictureElement).PictureWidth, (el as TPictureElement).PictureHeight, Color);
      {$ENDIF}
    end;
...
begin
  ...
  begin
    ...
    FEndOfLine := (Source as TCaret).EndOfLine;
...
begin
  ...
  begin
    ...
    FRight := (Source as TPageMargin).Right;
    FLeft := (Source as TPageMargin).Left;
  end;
end;
...
function TPageMargin.RightIndent: integer;
begin
...
procedure TPageMargin.SetLeft(const Value: integer);
begin
...
procedure TPageMargin.SetRight(const Value: integer);
begin
...
begin
  if (FVertical <> Value) and (Value >= 0) then
  begin
    FVertical := Value;
    Changed;
  end;
...
procedure TTabPosition.Assign(Source: TPersistent);
begin
  if (Source is TTabPosition) then
  begin
    FIndent := (Source as TTabPosition).Indent;
  end;
end;

constructor TTabPosition.Create(Collection: TCollection);
begin
  inherited;
  FIndent := 0;
end;

procedure TTabPosition.SetIndent(const Value: integer);
begin
  if (FIndent <> Value) then
  begin
    FIndent := Value;
    (Collection as TTabPositions).Update(Self);
  end;
end;
...
function TTabPositions.Add: TTabPosition;
begin
  Result := TTabPosition(inherited Add);
end;

function TTabPositions.Add(Indent: integer): TTabPosition;
begin
  Result := Add;
  Result.Indent := Indent;
end;

constructor TTabPositions.Create(AOwner: TComponent);
begin
  inherited Create(Aowner, TTabPosition);
end;

function TTabPositions.GetAsText: string;
var
  sl: TStringList;
  i: integer;
begin
  ...
  try
    ...
    begin
      sl.Add(inttostr(Items[i].Indent));
    end;
    Result := sl.CommaText;
  finally
    sl.Free;
...
function TTabPositions.GetItem(Index: integer): TTabPosition;
begin
  Result := TTabPosition(inherited Items[Index]);
end;

function TTabPositions.Insert(Index: Integer): TTabPosition;
begin
  Result := TTabPosition(inherited Insert(Index));
end;

procedure TTabPositions.SetAsText(const Value: string);
var
  sl: TStringList;
  i,ind,e: integer;
begin
  ...
  try
    ...
    begin
      val(sl.Strings[i], ind, e);
      if (e = 0) and (ind > 0) then
        Add(ind);
    end;
  finally
    sl.Free;
...
procedure TTabPositions.SetItem(Index: integer; const Value: TTabPosition);
begin
  inherited Items[Index] := Value;
end;

procedure TTabPositions.Update(Item: TCollectionItem);
begin
  if (UpdateCount = 0) and Assigned(OnChange) then
    OnChange(Self);
end;

{ TRichEditorAutoCompletion }

constructor TRichEditorAutoCompletion.Create;
begin
  inherited;
  FEnabled := false;
  FTextColor := clSilver;
  FWords := TWordList.Create;
  FWords.Sorted := true;
...
destructor TRichEditorAutoCompletion.Destroy;
begin
  FWords.Free;
  inherited;
end;

procedure TRichEditorAutoCompletion.SetCaseSensitive(const Value: boolean);
begin
  FCaseSensitive := Value;
  FWords.CaseSensitive := Value;
end;

procedure TRichEditorAutoCompletion.SetWords(const Value: TWordList);
begin
  FWords.Assign(Value);
end;
...
procedure TWordList.AutoComplete(AWord: string; var APrediction: string);
var
  L, H, I, C: Integer;
  W,SW: string;
begin
  ...
  begin
    ...
    if pos(W, Uppercase(SW)) = 1 then
    begin
      APrediction := Strings[I];
      break;
    end
    else
    begin
      C := CompareStrings(SW, W);
