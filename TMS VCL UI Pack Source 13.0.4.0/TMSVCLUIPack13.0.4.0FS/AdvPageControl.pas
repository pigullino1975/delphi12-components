...
uses
  Messages, Windows, Types, SysUtils, Classes, Controls, Dialogs,
...
const
  ...
  // 2.0.2.1 : Fixed : Drawing of border when TabVisible = false on all tabs
  // 2.0.2.2 : Fixed : Issue with accelerator keys and OnChanging event
  // 2.0.2.3 : Fixed : Issue with OnChange event trigger sequence when FreeOnClose is used
  // 2.0.2.4 : Fixed : Issue with focus handling in invisible tab sheets
  // 2.0.2.5 : Fixed : Regression with setting TabVisible = false
  // 2.0.3.0 : New : OnMouseLeave, OnMouseEnter events exposed
  // 2.0.3.1 : Fixed : Issue with tab borders in combination with hidden tabs
  // 2.0.3.2 : Fixed : Issue with active font for right & left aligned tabs on high DPI
  // 2.0.4.0 : Improved : High DPI handling for Delphi 11
  // 2.0.4.1 : Improved : Use of TabMargin on tabs with & without image
  // 2.0.4.2 : Fixed : Issue with page control border when none of tabsheets has TabVisible = true
  // 2.0.5.0 : New : Support for VCL styles added
...
type
  ...
  protected
    ...
    procedure SetColorTones(ct: TColorTones);
...
begin
  ...
  {$IFDEF DELPHIXE10_LVL}
  FActiveFont.Name := 'Segoe UI';
  {$ELSE}
  FActiveFont.Name := 'Tahoma';
  {$ENDIF}
  FActiveFont.Height := -11;
...
begin
  ...
  if isDPIChange then
  {$ENDIF}
    ActiveFont.Size := MulDiv(ActiveFont.Size, M, D);
end;
...
begin
  ...
  begin
    ...
    // always needs an image via which variable size is set when LeftMargin/RightMargin are used
    if (TabMargin.LeftMargin <> 0) or (TabMargin.RightMargin <> 0) then
      Result := 0;
  end;
end;
...
{$IFDEF DELPHIXE2_LVL}
procedure TAdvPageControl.CMStyleChanged(var Message: TMessage);
var
  ct: TColorTones;
begin
  inherited;
...
//              tf.Height := Round(tf.Height * FDPIScale);
        ...
        begin
          ...
          if (Images <> nil) and (TAdvTabSheet(FPages[TabIndex]).ImageIndexDummy >= 0) and not Assigned(FOnDrawTab) then
          begin
            imgY := TextY;
            ...
            if (TabPosition = tpLeft) then
              Images.Draw(Canvas, TextX + 1, TextY - 14, TAdvTabSheet(FPages[TabIndex]).ImageIndexDummy)
            else
              if (TabPosition = tpRight) then
                Images.Draw(Canvas, TextX - 18, TextY, TAdvTabSheet(FPages[TabIndex]).ImageIndexDummy)
              else
                Images.Draw(Canvas, TextX - 2, ImgY, TAdvTabSheet(FPages[TabIndex]).ImageIndexDummy);
...
procedure TAdvPageControl.CMFontChanged(var Message: TMessage);
begin
  if (csDesigning in ComponentState) then
...
procedure TAdvPageControl.SetColorTones(ct: TColorTones);
var
  i: integer;
begin
  DefaultTabColor := ct.Background.BrushColor;
  DefaultTextColor := ct.Background.TextColor;
  DefaultTabColorTo := clNone;

  Brush.Color := ct.Background.BrushColor;
  Brush.Style := bsSolid;

  ActiveColor := ct.Selected.BrushColor;
  ActiveColorTo := clNone;
  ActiveFont.Color := ct.Selected.TextColor;

  Color := ct.Background.BrushColor;

  TabSheetBorderColor := ct.Background.BrushColor;

  TabBackGroundColor := ct.Background.BrushColor;

  TabBorderColor := ct.Background.BorderColor;

  for i := 0 to PageCount - 1 do
  begin
    Pages[i].Color := ct.Background.BrushColor;
    Pages[i].ColorTo := clNone;
  end;
end;
