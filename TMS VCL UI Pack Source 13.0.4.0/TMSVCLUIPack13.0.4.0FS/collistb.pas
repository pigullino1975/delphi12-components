...
const
  ...
  // v1.3.2.0 : Improved : Changed TImageList property to TCustomImageList for compatibility with TVirtualImageList
  // v1.3.2.1 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // v1.3.3.0 : Improved ; Automatic design-time created column
  // v1.3.3.1 : Fixed : Issue with updating item images at runtime
  // v1.3.4.0 : New : Event OnDrawItemProp added
...
type
  ...
  TColumnListBoxDrawItemPropEvent = procedure(Sender: TObject;AIndex, ACol: integer; ACanvas: TCanvas; ARect: TRect; AState: TOwnerDrawState) of object;
  ...
  private
    ...
    FDPIScale: single;
    FOnDrawItemProp: TColumnListBoxDrawItemPropEvent;
  ...
  protected
    ...
    procedure DoDrawItemProp(AIndex, ACol: integer; ACanvas: TCanvas; ARect: TRect; AState: TOwnerDrawState); virtual;
  ...
  published
    ...
    property OnDragOver;
    property OnDrawItem;
    property OnDrawItemProp: TColumnListBoxDrawItemPropEvent read FOnDrawItemProp write FOnDrawItemProp;
    property OnEndDrag;
    property OnEnter;
    property OnExit;
    property OnKeyDown;
...
{$IFDEF DELPHIXE2_LVL}
  VCL.Themes,
{$ENDIF}
  ExtCtrls, ShellApi, Commctrl, AdvStyleIF;
...
begin
  ...
  begin
    ...
    DoDrawItemProp(Index, Col-1, Canvas, dr, State);
...
procedure TColumnListBox.SetImages(const Value: TCustomImageList);
begin
  FImages := Value;
  Invalidate;
end;
...
procedure TColumnListBox.DoDrawItemProp(AIndex, ACol: integer; ACanvas: TCanvas;
  ARect: TRect; AState: TOwnerDrawState);
begin
  if Assigned(OnDrawItemProp) then
    OnDrawItemProp(Self, AIndex, ACol, ACanvas, ARect, AState);
end;
...
procedure TColumnListBox.Clear;
begin
  ListBoxItems.Clear;
  inherited;
end;
...
begin
  ...
  begin
    ...
    if not (csDesigning in ComponentState) then BuildItems;
    Invalidate;
  end;
end;
