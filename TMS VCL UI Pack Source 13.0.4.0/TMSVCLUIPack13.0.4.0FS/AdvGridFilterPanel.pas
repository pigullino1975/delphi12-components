...
const
  ...
  // v1.0.6.0 : New : OnGetEditorType event added
  // v1.0.6.1 : Improved : High DPI handling
  // v1.0.7.0 : New : Exposed Form in TAdvGridFilterDialog
  // v1.0.7.1 : Fixed : Issue with restoring the filter logic operations

type
  TAdvGridFilterPanel = class;

  TAllowColumnFilter = procedure(Sender: TObject; AColumn: integer; var Allow: boolean) of object;
  ...
  private
    ...
    FOnAllowColumnFilter: TAllowColumnFilter;
  ...
  published
    ...
    property OnAllowColumnFilter: TAllowColumnFilter read FOnAllowColumnFilter write FOnAllowColumnFilter;
  end;
  ...
  protected
    ...
    procedure DoAllowColumnFilter(Sender: TObject; AColumn: integer; var Allow: boolean);
    procedure Notification(AComponent: TComponent; AOperation: TOperation); override;
...
begin
  ...
  begin
    ...
    if Assigned(OnAllowColumnFilter) then
      OnAllowColumnFilter(Self, i, Allow);
...
procedure TAdvGridFilterPanel.DoGetEditorType(AColumn: integer; var AEditor: TColumnEditor);
var
  ct: TColumnType;
begin
  AEditor := ceText;
  ...
  case ct of
  ctNumeric: AEditor := ceNumeric;
  ctFloat: AEditor := ceNumeric;
  ctAlpha: AEditor := ceText;
  ctDate: AEditor := ceDate;
  ctTime: AEditor := ceTime;
  ctBoolean: AEditor := ceBoolean;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (LC + 1 < F.Count) and (F.Items[LC+1].Operation = foAND) then
      begin
        combo.ItemIndex := 0;
      end
      else
...
procedure TAdvGridFilterDialog.DoAllowColumnFilter(Sender: TObject;
  AColumn: integer; var Allow: boolean);
begin
  if Assigned(OnAllowColumnFilter) then
    OnAllowColumnFilter(Self, AColumn, Allow);
end;

procedure TAdvGridFilterDialog.DoGetEditor(Sender: TObject; AColumn: integer;
...
begin
  ...
  ((Form as TAdvGridFilterForm).FilterPanel as TAdvGridFilterPanel).OnGetEditorType := DoGetEditor;
  ((Form as TAdvGridFilterForm).FilterPanel as TAdvGridFilterPanel).OnAllowColumnFilter := DoAllowColumnFilter;
end;
