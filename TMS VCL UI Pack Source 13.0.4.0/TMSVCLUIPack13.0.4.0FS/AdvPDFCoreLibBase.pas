...
uses
  Classes, Types, AdvGraphics, PictureContainer, AdvGraphicsTypes, Graphics, AdvTypes
  {$IFDEF WEBLIB}
  ,Contnrs
  {$ENDIF}
  {$IFNDEF LCLWEBLIB}
...
const
  ...
  {$IFDEF WEBLIB}
  PDFPageFontGlyphRef = '{PDFPAGEFONTGLYPHREFERENCE%s%s}';
  {$ENDIF}
...
type
  ...
  TAdvPDFPlatformServicesService = class
  private
  ...
  {$IFDEF WEBLIB}
  TAdvPDFPlatformServicesList = class(TObjectList)
  private
    function GetItem(Index: Integer): TAdvPDFPlatformServicesService;
    procedure SetItem(Index: Integer; const Value: TAdvPDFPlatformServicesService);
  public
    property Items[Index: Integer]: TAdvPDFPlatformServicesService read GetItem write SetItem; default;
  {$ENDIF}
  {$IFNDEF WEBLIB}
  TAdvPDFPlatformServicesList = class(TObjectList<TAdvPDFPlatformServicesService>)
  {$ENDIF}
  private
  ...
  TAdvPDFLibPlatformServices = class
  private
    FServicesList: TAdvPDFPlatformServicesList;
    class var FCurrent: TAdvPDFLibPlatformServices;
...
{$ENDIF}
  public
    ...
    class function Current: TAdvPDFLibPlatformServices;
  end;

  {$IFDEF WEBLIB}
  TAdvPDFGraphicsLibUsedFontCharArray = class(TList)
  private
    function GetItem(Index: Integer): Integer;
    procedure SetItem(Index: Integer; const Value: Integer);
  public
    property Items[Index: Integer]: Integer read GetItem write SetItem; default;
    function SubSetCount(ALow, AHigh: Integer): Integer;
    function HeadSetCount(AHigh: Integer): Integer;
  end;
  ...
  private
    FDestStream: TMemoryStream;
  public
    constructor Create(compressionLevel: TCompressionLevel; dest: TMemoryStream);
    function CopyFrom(const Source: TStream; Count: Int64): Int64;
  end;
  {$ENDIF}
  {$IFNDEF WEBLIB}
  TAdvPDFGraphicsLibUsedFontCharArray = class(TList<Integer>);
  {$ENDIF}
  ...
  TAdvPDFGraphicsLibFontCharWidth = record
    w: word;
    g: word;
    i: Integer;
  end;

  TAdvPDFGraphicsLibFontCharWidths = array of TAdvPDFGraphicsLibFontCharWidth;
  ...
  TAdvPDFGraphicsLibOutputWriterNotifyGoToEvent = procedure(Sender: TObject; ARect: TRectF; ADestination: UnicodeString) of object;
  TAdvPDFGraphicsLibOutputWriterNotifyUnicodeEvent = procedure(Sender: TObject; AValue: UnicodeString) of object;
  TAdvPDFGraphicsLibOutputWriterNotifyBitmapEvent = procedure(Sender: TObject; AValue: TAdvBitmapHelperClass; AImageType: TAdvPDFGraphicsLibImageType; AQuality: Single; ABackgroundColor: TAdvGraphicsColor; var ABitmapReference: string) of object;
  ...
  {$IFDEF WEBLIB}
  TAdvPDFGraphicsLibOutputWriterStreams = class(TObjectList)
  private
    function GetItem(Index: Integer): TStream;
    procedure SetItem(Index: Integer; const Value: TStream);
  public
    property Items[Index: Integer]: TStream read GetItem write SetItem; default;
  end;
  {$ENDIF}
  {$IFNDEF WEBLIB}
  TAdvPDFGraphicsLibOutputWriterStreams = class(TObjectList<TStream>);
  {$ENDIF}
  ...
  private
    ...
    FFontGlyphRefName: String;
    FOnNotifyGoTo: TAdvPDFGraphicsLibOutputWriterNotifyGoToEvent;
  ...
  public
    ...
    procedure NotifyGoTo(ARect: TRectF; ADestination: UnicodeString);
    ...
    procedure NotifyBitmap(AValue: TAdvBitmapHelperClass; AImageType: TAdvPDFGraphicsLibImageType; AQuality: Single; ABackgroundColor: TAdvGraphicsColor; var ABitmapReference: string);
    ...
    property FontGlyphRefName: String read FFontGlyphRefName write FFontGlyphRefName;
    ...
    property OnNotifyGoTo: TAdvPDFGraphicsLibOutputWriterNotifyGoToEvent read FOnNotifyGoTo write FOnNotifyGoTo;
...
function ImageFromBitmap(ABitmap: TAdvBitmapHelperClass): UIImage; overload;
function ImageFromBitmap(ABitmap: TAdvBitmapHelperClass; ASize: Single): UIImage; overload;
...
function ImageFromBitmap(ABitmap: TAdvBitmapHelperClass): NSImage; overload;
function ImageFromBitmap(ABitmap: TAdvBitmapHelperClass; ASize: Single): NSImage; overload;
...
{$IFDEF ANDROID}
  ...
  ['{315993F5-88BB-4995-B03A-CD015291E56D}']
    function obtain(source: JCharSequence; start: Integer; end_: Integer; paint: JTextPaint; width: Integer): JStaticLayout_Builder; cdecl;
  end;
  [JavaSignature('android/text/StaticLayout$Builder')]
  JStaticLayout_Builder = interface(JObject)
  ['{5BC7CCB0-B90E-4794-90F0-EBA126BFB04D}']
    function build: JStaticLayout; cdecl;
    function setMaxLines(maxLines: Integer): JStaticLayout_Builder; cdecl;
    function setAlignment(alignment: JLayout_Alignment): JStaticLayout_Builder; cdecl;
    function setIncludePad(includePad: Boolean): JStaticLayout_Builder; cdecl;
    function setLineSpacing(spacingAdd: Single; spacingMult: Single): JStaticLayout_Builder; cdecl;
  end;
  TJStaticLayout_Builder = class(TJavaGenericImport<JStaticLayout_BuilderClass, JStaticLayout_Builder>) end;
...
function ImageFromBitmap(ABitmap: TAdvBitmapHelperClass): JBitmap;
{$ENDIF}
...
function CreateStringStream(AValue: string = ''): TAdvPDFGraphicsLibOutputWriterStream;
begin
  Result := TAdvPDFGraphicsLibOutputWriterStream.Create(AValue{$IFDEF WEBLIB}, TEncoding.Ansi{$ENDIF});
end;
...
function ImageFromBitmap(ABitmap: TAdvBitmapHelperClass; ASize: Single): UIImage;
  type
    TBArray = TArray<Byte>;
...
function ImageFromBitmap(ABitmap: TAdvBitmapHelperClass): UIImage;
var
  ms: TMemoryStream;
  dt: NSData;
begin
...
function ImageFromBitmap(ABitmap: TAdvBitmapHelperClass; ASize: Single): NSImage;
  type
...
function ImageFromBitmap(ABitmap: TAdvBitmapHelperClass): NSImage;
var
  ms: TMemoryStream;
  dt: NSData;
begin
  ...
  begin
    ...
    begin
      if (TFontStyle.fsBold in AFont.Style) and (TFontStyle.fsItalic in AFont.Style) then
        ATextPaint.setTypeface(TJTypeface.JavaClass.create(TJTypeFace.JavaClass.MONOSPACE, TJTypeface.JavaClass.BOLD_ITALIC))
      else if TFontStyle.fsBold in AFont.Style then
        ATextPaint.setTypeface(TJTypeface.JavaClass.create(TJTypeFace.JavaClass.MONOSPACE, TJTypeface.JavaClass.BOLD))
      else if TFontStyle.fsBold in AFont.Style then
        ATextPaint.setTypeface(TJTypeface.JavaClass.create(TJTypeFace.JavaClass.MONOSPACE, TJTypeface.JavaClass.ITALIC))
      else
        ATextPaint.setTypeface(TJTypeface.JavaClass.create(TJTypeFace.JavaClass.MONOSPACE, TJTypeface.JavaClass.NORMAL));
    end
    else
    begin
      if (TFontStyle.fsBold in AFont.Style) and (TFontStyle.fsItalic in AFont.Style) then
...
{$IFDEF WEBLIB}
function TCompressionStream.CopyFrom(const Source: TStream; Count: Int64): Int64;
var
  b: TBytes;
begin
  if Assigned(FDestStream) then
  begin
    FDestStream.CopyFrom(Source, Count);
    b := FDestStream.Bytes;
    if CanCompress then
    begin
      asm
    ...
    FDestStream.Clear;
    FDestStream.Position := 0;
    FDestStream.Write(b, Length(b));
  end;
end;

constructor TCompressionStream.Create(compressionLevel: TCompressionLevel; dest: TMemoryStream);
begin
  FDestStream := dest;
end;
{$ENDIF}
...
{ TAdvPDFLibPlatformServices }

procedure TAdvPDFLibPlatformServices.AddPlatformService(const AServiceGUID: TGUID; const AService: IInterface);
var
  LService: IInterface;
begin
  ...
  begin
    if Supports(AService, AServiceGUID, LService) then
      FServicesList.Add(TAdvPDFPlatformServicesService.Create(GUIDToString(AServiceGUID), AService));
  end;
end;

constructor TAdvPDFLibPlatformServices.Create;
begin
  inherited;
  FServicesList := TAdvPDFPlatformServicesList.Create;
end;

destructor TAdvPDFLibPlatformServices.Destroy;
begin
...
{$IFNDEF AUTOREFCOUNT}
class procedure TAdvPDFLibPlatformServices.ReleaseCurrent;
begin
  FreeAndNil(FCurrent);
...
class function TAdvPDFLibPlatformServices.Current: TAdvPDFLibPlatformServices;
begin
  if (FCurrent = nil) and not FCurrentReleased then
    FCurrent := TAdvPDFLibPlatformServices.Create;
  Result := FCurrent;
end;

function TAdvPDFLibPlatformServices.GetPlatformService(const AServiceGUID: TGUID): IInterface;
var
  k: IInterface;
begin
...
procedure TAdvPDFLibPlatformServices.RemovePlatformService(const AServiceGUID: TGUID);
begin
  FServicesList.RemoveByGUID(GUIDToString(AServiceGUID));
end;

function TAdvPDFLibPlatformServices.SupportsPlatformService(const AServiceGUID: TGUID;
...
function TAdvPDFLibPlatformServices.SupportsPlatformService(const AServiceGUID: TGUID): Boolean;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      {$IFDEF WEBLIB}
      if FontEmbedding then
        Result := Result + Format(PDFPageFontGlyphRef, [IntToStr(v), FontGlyphRefName])
      else
      {$ENDIF}
      begin
...
begin
  ...
  i := 1;
  {$ENDIF}
  s[0 + i] := HexChars[v shr 4];
  s[1 + i] := HexChars[v and $F];
  s[2 + i] := HexChars[aWordValue shr 4];
  s[3 + i] := HexChars[aWordValue and $F];
  Result := s;
end;
...
function TAdvPDFGraphicsLibOutputWriter.FinishContentStream(AReference: String; ACompress: Boolean; AAdditionalFlags: String = ''): TStringStream;
var
  ls: Integer;
begin
  Result := nil;
  ...
  begin
    if ACompress and CanCompress then
      Result := CompressStream(FContentStream)
    else
    begin
      Result := CreateStringStream;
      Result.CopyFrom(FContentStream, 0);
    end;
...
begin
  ...
  begin
    ...
    {$ENDIF}
    {$IFNDEF LCLLIB}
    FFontUsedCharArray.Sort(TListSortCompare(@CompareVal));
    {$ENDIF}
    {$ENDIF}
  end;
end;
...
procedure TAdvPDFGraphicsLibOutputWriter.NotifyBitmap(AValue: TAdvBitmapHelperClass; AImageType: TAdvPDFGraphicsLibImageType; AQuality: Single; ABackgroundColor: TAdvGraphicsColor; var ABitmapReference: string);
begin
...
procedure TAdvPDFGraphicsLibOutputWriter.NotifyGoTo(ARect: TRectF; ADestination: UnicodeString);
begin
  if Assigned(OnNotifyGoTo) then
    OnNotifyGoTo(Self, ARect, ADestination);
end;
...
begin
  {$IFDEF WEBLIB}
  raise Exception.Create('Write Buffer in Stream');
  {$ELSE}
  if Assigned(FContentStream) then
...
procedure TAdvPDFGraphicsLibOutputWriter.WriteMatrix(m11, m12, m21, m22, dx, dy: Double);
begin
  WriteString(ConvertFloatToString(m11) + ' ' +
              ConvertFloatToString(m12) + ' ' +
              ConvertFloatToString(m21) + ' ' +
              ConvertFloatToString(m22) + ' ' +
              ConvertFloatToString(dx) + ' ' +
              ConvertFloatToString(dy) + ' cm' + PDFLB);
end;
...
begin
  WriteString(ConvertFloatToString(ARect.Left) + ' ' + ConvertFloatToString(ARect.Top) + ' ' + ConvertFloatToString(ARect.Right - ARect.Left) + ' ' + ConvertFloatToString(ARect.Bottom - ARect.Top) + ' re' + PDFLB);
end;
...
procedure TAdvPDFGraphicsLibOutputWriter.WriteTextMatrix(m11, m12, m21, m22, dx, dy: Double);
begin
  WriteString(ConvertFloatToString(m11) + ' ' +
              ConvertFloatToString(m12) + ' ' +
              ConvertFloatToString(m21) + ' ' +
              ConvertFloatToString(m22) + ' ' +
              ConvertFloatToString(dx) + ' ' +
              ConvertFloatToString(dy) + ' Tm' + PDFLB);
end;
...
function FastLocateWordSorted(P: TWordDynArray; R: Integer; V: word): Integer;
var
  L, cmp: Integer;
begin
  if R < 0 then
...
begin
  Result := FastLocateWordSorted(v, c - 1, aValue);
  if Result < 0 then
    Exit;
...
{ TAdvPDFPlatformServicesService }

constructor TAdvPDFPlatformServicesService.Create(AGUID: string;
...
{ TAdvPDFPlatformServicesList }

function TAdvPDFPlatformServicesList.ContainsKey(AGUID: string): Boolean;
var
  I: Integer;
begin
  Result := False;
...
function TAdvPDFPlatformServicesList.GetValue(AGUID: string): IInterface;
var
  I: Integer;
begin
  Result := nil;
...
procedure TAdvPDFPlatformServicesList.RemoveByGUID(AGUID: string);
var
  I: Integer;
begin
...
{$IFDEF WEBLIB}
function TAdvPDFPlatformServicesList.GetItem(Index: Integer): TAdvPDFPlatformServicesService;
begin
  Result := TAdvPDFPlatformServicesService(inherited Items[Index]);
end;

procedure TAdvPDFPlatformServicesList.SetItem(Index: Integer; const Value: TAdvPDFPlatformServicesService);
begin
  inherited Items[Index] := Value;
end;

function TAdvPDFGraphicsLibUsedFontCharArray.GetItem(Index: Integer): Integer;
begin
  Result := Integer(inherited Items[Index]);
end;

procedure TAdvPDFGraphicsLibUsedFontCharArray.SetItem(Index: Integer; const Value: Integer);
begin
  inherited Items[Index] := Value;
end;

function TAdvPDFGraphicsLibUsedFontCharArray.HeadSetCount(AHigh: Integer): Integer;
var
  c, I: Integer;
begin
  c := 0;
...
function TAdvPDFGraphicsLibUsedFontCharArray.SubSetCount(ALow, AHigh: Integer): Integer;
var
  c, I: Integer;
begin
  c := 0;
  for I := 0 to Count - 1 do
  begin
    if (Items[I] >= ALow) and (Items [I] < AHigh) then
      Inc(c);
  end;
...
function TAdvPDFGraphicsLibOutputWriterStreams.GetItem(Index: Integer): TStream;
begin
  Result := TStream(inherited Items[Index]);
end;

procedure TAdvPDFGraphicsLibOutputWriterStreams.SetItem(Index: Integer; const Value: TStream);
begin
  inherited Items[Index] := Value;
end;
{$ENDIF}
...
initialization
begin
  TAdvPDFLibPlatformServices.FCurrentReleased := False;
end;
...
begin
{$IFNDEF AUTOREFCOUNT}
  TAdvPDFLibPlatformServices.ReleaseCurrent;
{$ENDIF}
end;
{$ENDIF}
