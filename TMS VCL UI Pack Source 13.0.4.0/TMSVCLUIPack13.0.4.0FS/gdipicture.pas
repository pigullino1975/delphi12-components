...
type
  ...
  {$IFDEF DELPHIXE_LVL}
  TGDIPPicture = class(TTMSFNCSVGBitmap)
  {$ELSE}
...
uses
  Math
  {$IFDEF DELPHIXE_LVL}
  ,AdvUtils, AdvGraphics, AdvGraphicsSVGEngine, Types
  {$ENDIF}
  ;
...
begin
  ...
  begin
    ...
    begin
      ...
      if TGDIPPicture(Source).HasSVG then
...
{$IFDEF DELPHIXE_LVL}
procedure TGDIPPicture.DrawSVG(Graphics: TGPGraphics; X, Y, W, H: Single);
var
  ca: TCanvas;
begin
  if not HasSVG then
...
begin
  ...
  if HasSVG then
  begin
    DrawSVG(Graphics, X, Y, Width, Height);
    Exit;
  end;
  {$ENDIF}
