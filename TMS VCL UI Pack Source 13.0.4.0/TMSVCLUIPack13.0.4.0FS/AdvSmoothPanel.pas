...
const
  ...
  // v1.7.1.2 : Improved : On creation check for enabled AdvFormStyler
  // v1.7.1.3 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // v1.7.1.4 : Improved : Transparent drawing
  // v1.7.1.5 : Fixed : Issue with high DPI in Delphi 11
  // v1.7.1.6 : Improved : Centered text drawing for non wordwrapped text
...
type
  ...
  protected
    ...
    {$ENDIF}
    procedure SetFTMSStyle(Value: TTMSStyle);
  public
...
begin
  ...
  FCaption.Font.Height := MulDiv(FCaption.Font.Height, M, D);
  {$ENDIF}
end;
...
begin
  ...
  case AStyle of
   ...
   tsOffice2016White:
    ...
    tsOffice2019White:
      begin
        Caption.ColorStart := $00444648;
        Caption.ColorEnd := $00444648;
        Caption.LineColor := $00C4C6C8;
    ...
    tsOffice2019Gray:
      begin
        Caption.ColorStart := $00232425;
        Caption.ColorEnd := $00232425;
        Caption.LineColor := $00666666;
...
procedure TAdvSmoothPanel.SetFTMSStyle(Value: TTMSStyle);
begin
  FTMSStyle := Value;
end;
...
procedure TAdvSmoothPanel.SetUIStyle(const Value: TTMSStyle);
begin
  ...
  begin
    ...
    begin
      ...
      if (Parent is TWinControl) then
        (TWinCtrl(Parent)).PaintCtrls(DC,Self);
      RestoreDC(DC, i);
...
begin
  ...
  i := 0;
  c := -1;
  // Find index for Self in control list
  While (i < ControlCount) and (c < 0) do
  begin
    if First = Controls[i] then
      c := i;
    inc(i);
  end;
  ...
  begin
    ...
    begin
      with TWinControl(Controls[I]) do
        if Visible and RectVisible(DC, Rect(Left, Top, Left + Width, Top + Height)) // Visible under control?
