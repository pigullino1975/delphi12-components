...
const
  ...
  // v1.6.1.1 : Improved : Caption.Font handling for per monitor high DPI
  // v1.6.1.2 : Fixed : Caption shadow high DPI handling for Delphi 11
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvSmoothLabel.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvSmoothLabel.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
  ...
  {$ENDIF}
  begin
    FCaption.Font.Size := MulDiv(Caption.Font.Size, M, D);
    FCaptionShadow.Font.Size := MulDiv(CaptionShadow.Font.Size, M, D);
  end;
end;
