...
uses
  ...
  Forms, Dialogs, ExtCtrls, Math, AdvStyleIF
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF}
  ;
...
const
  ...
  // 1.3.1.1 : Improved : Small change to handle case when setting Max to lower value than Position
  // 1.3.2.0 : New : UIStyle property for TMSStyles
  // 1.3.2.1 : Improved : Painting performance
  // 1.3.2.2 : Fixed : Issue with updating large ranges
  // 1.3.2.3 : Fixed : Rare rendering issue in specific ranges
...
type
  ...
  {$ENDIF}
  TAdvProgressBar = class(TGraphicControl, ITMSStyle)
  private
...
procedure TAdvProgressBar.SetComponentStyle(AStyle: TTMSStyle);
begin
  FTMSStyle := AStyle;
  ...
  begin
    case AStyle of
      ...
      begin
        Rounded := True;
        ShowGradient := True;
        BackgroundColor := $00FFD2AF;
        BorderColor := $00C0C0C0;
      ...
      begin
        Rounded := True;
        ShowGradient := True;
        BackgroundColor := $00FDF8F1;
        BorderColor := $00FDDE99;
        Font.Color := clWindowText;
      end;
      tsWindows7:
      begin
      ...
      begin
        Rounded := True;
        ShowGradient := True;
        BackgroundColor := $00F0DAC7;
        BorderColor := $00C7B29F;
        Font.Color := clWindowText;
      end;
      tsOffice2010Silver:
      begin
      ...
      begin
        Rounded := True;
        ShowGradient := True;
        BackgroundColor := $00919191;
        BorderColor := $00D7D7D6;
        Font.Color := clWindowText;
      end;
      tsOffice2013White:
      begin
      ...
      begin
        Rounded := True;
        ShowGradient := True;
        BackgroundColor := $00F6F6F6;
        BorderColor := $00C6C6C6;
        Font.Color := clWindowText;
      end;
      tsOffice2013Gray:
      begin
        Rounded := True;
        ShowGradient := True;
        BackgroundColor := $00E5E5E5;
        BorderColor := $00ABABAB;
        Font.Color := clWindowText;
      end;
      tsWindows10, tsWindows8:
      begin
        Rounded := False;
        ShowGradient := True;
        BackgroundColor := $00F7F6F5;
        BorderColor := $00E4E3E2;
        Font.Color := clWindowText;
      end;
      tsOffice2016White:
      begin
    ...
    if (Level0Color = clLime) or (Level0Color = $0000AA00) then
    begin
      if AStyle in [tsOffice2016Black, tsOffice2019Black] then
      begin
        Level0Color := $0000AA00;
        Level0ColorTo := $0030CC30;
      end
      else
      begin
        Level0Color := clLime;
        Level0ColorTo := $00E1FFE1;
      end;
    end;

    if (Level1Color = clYellow) or (Level1Color =  $0000B1B1) then
    begin
      if AStyle in [tsOffice2016Black, tsOffice2019Black] then
      begin
        Level1Color := $0000B1B1;
        Level1ColorTo := $0030BBBB;
      end
      else
      begin
    ...
    if (Level2Color = $0053A9FF) or (Level2Color =  $00007CF9) then
    begin
      if AStyle in [tsOffice2016Black, tsOffice2019Black] then
      begin
        Level2Color := $00007CF9;
        Level2ColorTo := $006FB7FF;
      end
      else
      begin
    ...
    if (Level3Color = clRed) or (Level3Color =  $000000DD) then
    begin
      if AStyle in [tsOffice2016Black, tsOffice2019Black] then
      begin
        Level3Color := $000000DD;
        Level3ColorTo := $004040FF;
      end
      else
      begin
        Level3Color := clRed;
        Level3ColorTo := $00CACAFF;
      end;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    if FOldPos <> NewPos then
      if (FMaxIncrement = 0) or (FPosition mod FMaxIncrement = 0) or
        (FOldBlockPos <> FPosition div FMaxIncrement) or (FPosition = Max) or (FPosition = Min) then
      begin
        Paint;
        FOldPos := NewPos;
      end;
...
procedure TAdvProgressBar.SetUIStyle(const Value: TTMSStyle);
begin
