...
interface
  ...
  // 1.7.4.4 : Fixed : Issue with header text height in high dpi with HeaderOwnerDraw
  // 1.7.4.5 : Fixed : High DPI issue with OwnerDraw = true
  // 1.8.0.0 : New : Support for Emoji in HTML rendering
  // 1.8.1.0 : New : IAdvDataBinderGrid interface added to TAdvListView
  // 1.8.1.1 : Fixed : Issue when doing custom sort in Win64
  // 1.8.1.2 : Fixed : Regression with sorting on Win64
  // 1.8.1.3 : Fixed : Issue with declaration tagNMHeader for Delphi 11
  // 1.8.1.4 : Improved : Handling of checkboxes in high DPI
  // 1.8.1.5 : Fixed : Issue with getting filter in 64bit
  // 1.8.2.0 : New : Support for TVirtualImageList for HeaderImages
  // 1.9.0.0 : New : Automatic sort indicator drawing for custom header
  //         : New : Vertical centered header text drawing
...
uses
  ...
  AdvStyleIF, Commctrl, Forms, PictureContainer, Dialogs, Printers, Comobj,
  ActiveX, Types, Variants, AdvUtil, ImgList
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF}
  {$IFDEF FREEWARE}
...
const
  ...
  BLD_VER = 0; // Build nr.
  DATE_VER = 'Aug, 2023'; // Month version
...
{$ENDIF}
  ...
  {$ENDIF}
  TAdvListView = class(TListView, IAdvDataBinderGrid)
  private
    FTempHeaderFont: TFont;
...
{$ENDIF}
  ...
  public
    ...
    procedure DrawHeaderItem(DrawItemStruct: TDrawItemStruct); virtual;
...
{$ENDIF}
    ...
    property HeaderIndent: integer read FHeaderIndent write SetHeaderIndent default 0;
...
uses
  INIFiles, Clipbrd, Registry, ShellApi, AlvUtil, RichEdit, AdvHTML;
...
function __CustomSort(Item1, Item2: TListItem; Data: NativeInt): Integer; stdcall;
{$ENDIF}
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          case VAlignment of
            vtaCenter: if (r.Bottom - r.Top > round(CHECKBOX_SIZE * FDPIScale)) then
                r.Top := r.Top + ((r.Bottom - r.Top) - round(CHECKBOX_SIZE * FDPIScale)) shr 1;
            vtaBottom: r.Top := r.Bottom - round(CHECKBOX_SIZE * FDPIScale);
          end;

          r.Bottom := r.Top + round(CHECKBOX_SIZE * FDPIScale);
          r.Left := r.Left + round(COLUMN_SPACING * FDPIScale);
          r.Right := r.Left + round(CHECKBOX_SIZE * FDPIScale);
          ...
          TmpRct.Left := TmpRct.Left + round((CHECKBOX_SIZE + 2 * COLUMN_SPACING) * FDPIScale);
        end;
      end;
        ...
        begin
          ...
          begin
            ...
            if CheckBoxes and (i = -1) then
              SmallImages.Draw(FCanvas, Rect.Left + round((CHECKBOX_SIZE + 2 * COLUMN_SPACING) * FDPIScale), ih, im)
            else
              SmallImages.Draw(FCanvas, Rect.Left + round(COLUMN_SPACING * FDPIScale), ih, im);

            iw := Smallimages.Width + round(COLUMN_SPACING * FDPIScale);
          end;
        end;
      end;
...
{$ENDIF}
  ...
  FHeaderFont := TFont.Create;
  FHeaderFont.Height := -11;
  FDummyFont := TFont.Create;
  FDummyFont.Height := -11;
  FTempHeaderFont := TFont.Create;
...
begin
  ...
  begin
    FTempHeaderFont.Assign(FHeaderFont);
    FTempHeaderFont.Height := Round(FDPIScale * FHeaderFont.Height);

    hHeader := SendMessage(self.Handle, LVM_GETHEADER, 0, 0);
    SendMessage(hHeader, WM_SETFONT, WParam(FTempHeaderFont.Handle), MakeLParam(WORD(TRUE), 0));
...
procedure TAdvListView.DataBeginUpdate;
begin
  BeginUpdate;
end;

procedure TAdvListView.DataEndUpdate;
begin
  EndUpdate;
end;
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvListView.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvListView.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
...
procedure TAdvListView.ClearData;
begin
  Items.Clear;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      rcItem.Left := rcItem.Left + HeaderIndent;
      ...
      Canvas.Font.Height := Round(FDPIScale * FHeaderFont.Height);
      ...
      begin
        ...
        begin
          vcenter := 0;
          if SortDownGlyph.Height < (rcItem.Bottom - rcItem.Top) then
            vcenter := 1 + (rcItem.Bottom - rcItem.Top - SortDownGlyph.Height) shr 1;
...
procedure TAdvListView.SetHeaderImages(AValue: TCustomImageList);
begin
...
procedure TAdvListView.SetHeaderIndent(const Value: integer);
var
  hHeader: THandle;
begin
  if (FHeaderIndent <> Value) then
  begin
    FHeaderIndent := Value;
    hHeader := SendMessage(Handle, LVM_GETHEADER, 0, 0);
    InvalidateRect(hHeader,nil,true);
...
function TAdvListView.GetDataRowCount: Integer;
begin
  Result := Items.Count;
end;
...
procedure TAdvListView.SetDataColumnCount(AValue: Integer);
begin
  ViewStyle := vsReport;

  Columns.Clear;
  while Columns.Count < AValue do
    Columns.Add;
end;

procedure TAdvListView.SetDataHeader(AColumn: Integer; AValue: string);
begin
  Columns.Items[AColumn].Caption := AValue;
end;

procedure TAdvListView.SetDataRowCount(AValue: Integer);
begin
  while Items.Count < AValue do
    Items.Add;
end;

procedure TAdvListView.SetDataValue(AColumn, ARow: Integer; AValue: string);
begin
  if (AColumn = 0) then
    Items[ARow].Caption := AValue
  else
  begin
    while Items[ARow].SubItems.Count < AColumn do
      Items[ARow].SubItems.Add('');
    Items[ARow].SubItems[AColumn - 1] := AValue;
  end;
end;
