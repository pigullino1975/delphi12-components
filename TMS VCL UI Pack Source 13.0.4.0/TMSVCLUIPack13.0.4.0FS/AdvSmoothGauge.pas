...
const
  ...
  // v2.5.1.0 : New : UIStyle property for TMSStyle
  // v2.5.1.1 : Fixed : UIStyle set to tsCustom after initial Office look
  // v2.5.1.2 : Improved : On creation check for enabled AdvFormStyler
  // v2.5.1.3 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // v2.5.1.4 : Fixed : High DPI issue with section margins.
  // v2.5.1.5 : Improved : StyleElements published for VCL styles
  // v2.5.2.0 : New : Visible property to show or hide the extra needles
...
type
  ...
  protected
    {$IFDEF DELPHIXE6_LVL}
    procedure SetStyleElements(const Value: TStyleElements); {$IFDEF DELPHIXE11_LVL} override; {$ENDIF}
    {$ENDIF}
    ...
    procedure DefaultDockImage(DragDockObject: TDragDockObject; Erase: Boolean);
      override;
  public
...
procedure TAdvSmoothGauge.DefaultDockImage(DragDockObject: TDragDockObject;
  Erase: Boolean);
begin
  inherited;
...
begin
  ...
  FMetroStyle := False;
  Needle.OuterCenterColorTo := clBlue;
  InnerCircle.Gloss.Color := clWhite;
  ...
  tsTerminal:
    ...
    begin
        Arc.Color := $00F2E1D5;
        InnerCircle.Color := clWhite;
        OuterCircle.Color := $00C4C6C8;
        Arc.Threshold.Color := $00B0784A;
        Digit.BackGroundColor := clWhite;
        Digit.Color := $003B3B3B;
        DivisionColor := $003B3B3B;
        SubDivisionColor := $00444648;
        Needle.Color := $00444648;
        Needle.InnerCenterColor := $00444648;
        Needle.InnerCenterColorTo := $00444648;
        Needle.OuterCenterColor := clWhite;
        Needle.OuterCenterColorTo := clNone;
        Needle.ShineColor := clNone;
        ValueFont.Color := $00444648;
    end;
    tsOffice2019Gray:
    begin
        Arc.Color := $00969696;
        InnerCircle.Color := $00B8BBBE;
        InnerCircle.Gloss.Color := clNone;
        OuterCircle.Color := $00505050;
        Arc.Threshold.Color := $00666666;
        Digit.BackGroundColor := $00B8BBBE;
        Digit.Color := $00232425;
        DivisionColor := $00232425;
        SubDivisionColor := $003B3B3B;
        Needle.Color := $003B3B3B;
        Needle.InnerCenterColor := $003B3B3B;
        Needle.InnerCenterColorTo := $003B3B3B;
        Needle.OuterCenterColor := $00B8BBBE;
        Needle.OuterCenterColorTo := clNone;
        Needle.ShineColor := clNone;
        ValueFont.Color := $003B3B3B;
    end;
    tsOffice2019Black:
    begin
        Arc.Color := $00505050;
        InnerCircle.Color := $00303132;
        InnerCircle.Gloss.Color := clNone;
        OuterCircle.Color := $00303132;
        Arc.Threshold.Color := $00B5B5B5;
        Digit.BackGroundColor := $00303132;
        Digit.Color := clWhite;
        DivisionColor := $00444648;
        SubDivisionColor := $00444648;
        Needle.Color := $00444648;
        Needle.InnerCenterColor := $00444648;
        Needle.InnerCenterColorTo := $00444648;
        Needle.OuterCenterColor := $00303132;
        Needle.OuterCenterColorTo := clNone;
        Needle.ShineColor := clNone;
        ValueFont.Color := clWhite;
    end;
...
{$IFDEF DELPHIXE6_LVL}
procedure TAdvSmoothGauge.SetStyleElements(const Value: TStyleElements);
begin
...
procedure TAdvSmoothGauge.SetUIStyle(const Value: TTMSStyle);
begin
...
begin
  ...
  if {$IFDEF DELPHIXE6_LVL} (seClient in StyleElements) and {$ENDIF} CheckVCLStylesEnabled(StyleServices,(csDesigning in ComponentState)) then
  begin
...
begin
  ...
  begin
    if ExtraNeedles[I].Visible then
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        innermg := Round(DPIScale * InnerMargin);
        outermg := Round(DPIScale * OuterMargin);
...
procedure TAdvSmoothGaugeNeedleItem.SetVisible(const Value: Boolean);
begin
  if FVisible <> Value then
  begin
    FVisible := Value;
