...
uses
  Windows, Classes, Types, StdCtrls, ExtCtrls, Controls, Messages, SysUtils, AdvStyleIF,
  Forms, Graphics, Buttons, Dialogs, Menus, AdvEdit, AEBXPVS, GDIPicture, ImgList
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF}
  ;
...
const
  ...
  // v1.5.2.1 : Fixed : Issue with LabelFont high DPI handling in specific circumstances
  // v1.5.2.2 : Fixed : Issue with high DPI handling
  // v1.5.2.3 : Fixed : Issue with ShowPassOnClick and mouse capture
  // v1.5.2.4 : Fixed : Issue with ButtonAlign = baLeft and EditAlign = eaRight
  // v1.5.3.0 : New : UIStyle property for office look
  // v1.5.3.1 : Fixed : Issue with ESC handling in TDBAdvEditBtn
  // v1.6.0.0 : New : GlyphIndex property added to support imagelist images for button
  // v1.6.1.0 : New : Button colors of TAdvEditButton can now be set and changes with new Office styles
  //          : New : High DPI drawing of new dropdown arrow
  //          : Fixed : ParentFont kept true on initialization
  // v1.6.2.0 : New : HexValue property when Edit Type is etHex
  // v1.7.0.0 : New : Balloon functionality added
  // v1.7.1.0 : Improved : VCL styles handling for button in TAdvEditBtn
  // v1.7.1.1 : Fixed : Issue with button background painting
  // v1.7.1.2 : Improved : ButtonWidth handling with high DPI
  // v1.7.1.3 : Fixed : Issue with setting StyleElements = [] for button
...
{ TCustomAdvEditBtn }
  ...
  TCustomAdvEditBtn = class(TCustomAdvEdit, ITMSStyle)
  private
    FDesignTime: Boolean;
    FUnitSize : integer;
    FButton: TEditButton;
    ...
    procedure SetButtonColorHot(const Value: TColor);
    procedure SetButtonTextColor(const Value: TColor);
    procedure SetButtonTextColorDown(const Value: TColor);
    procedure SetButtonTextColorHot(const Value: TColor);
  protected
    ...
    property ButtonColorHot: TColor read FButtonColorHot write SetButtonColorHot default clNone;
    property ButtonColorDown: TColor read FButtonColorDown write SetButtonColorDown default clNone;
    property ButtonTextColor: TColor read FButtonTextColor write SetButtonTextColor default clNone;
    property ButtonTextColorHot: TColor read FButtonTextColorHot write SetButtonTextColorHot default clNone;
    property ButtonTextColorDown: TColor read FButtonTextColorDown write SetButtonTextColorDown default clNone;
  public
...
procedure TAdvSpeedButton.SetGlyphIndex(const Value: integer);
begin
...
procedure TAdvSpeedButton.SetImages(const Value: TCustomImageList);
begin
  FImages := Value;
  Invalidate;
end;
...
{$IFDEF DELPHIXE10_LVL}
procedure TAdvSpeedButton.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ELSE}
procedure TAdvSpeedButton.ChangeScale(M, D: Integer);
{$ENDIF}
begin
  inherited;
...
begin
  if Assigned(Owner) and Assigned(Owner.Owner) and (Owner.Owner is TCustomAdvEditBtn) then
  begin
...
begin
  ...
  DoStyle := Assigned(LStyle);

  {$IFDEF DELPHIXE6_LVL}
  if Assigned(Parent) and not (seClient in Parent.StyleElements) then
    DoStyle := false;
  {$ENDIF}

  if CheckVCLStylesEnabled(LStyle,(csDesigning in ComponentState)) and DoStyle then
  begin
    r := ClientRect;
    InflateRect(r,1,1);

    if not TCustomAdvEditBtn(Owner.Owner).Enabled or TCustomAdvEditBtn(Owner.Owner).ReadOnly then
      LDetails := StyleServices.GetElementDetails(TThemedButton.tbPushButtonDisabled)
    else
    begin
      if (FState in [bsDown, bsExclusive]) and not FUp then
        LDetails := StyleServices.GetElementDetails(TThemedButton.tbPushButtonPressed)
      else
        if FHot then
          LDetails := StyleServices.GetElementDetails(TThemedButton.tbPushButtonHot)
        else
          LDetails := StyleServices.GetElementDetails(TThemedButton.tbPushButtonNormal);
    end;

    StyleServices.DrawElement(Canvas.Handle, LDetails, r);
    FntColor := StyleServices.GetStyleFontColor(TStyleFont.sfButtonTextNormal);
...
procedure TEditButton.SetGlyphIndex(const Value: integer);
begin
  FButton.GlyphIndex := Value;
  FGlyphIndex := Value;
end;

procedure TEditButton.SetImages(const Value: TCustomImageList);
begin
  FImages := Value;
...
procedure TCustomAdvEditBtn.Notification(AComponent: TComponent;
...
procedure TCustomAdvEditBtn.SetGlyphIndex(const Value: integer);
begin
  FGlyphIndex := Value;
  FButton.GlyphIndex := Value;
end;

procedure TCustomAdvEditBtn.SetImages(const Value: TCustomImageList);
begin
  FImages := Value;
  FButton.Images := Value;
end;
...
procedure TCustomAdvEditBtn.SetUIStyle(const Value: TTMSStyle);
begin
...
procedure TCustomAdvEditBtn.SetComponentStyle(AStyle: TTMSStyle);
begin
  inherited;
...
begin
  ...
  begin
    EditType := etString;
    FIsPass := true;
    SetCapture((Sender as TEditButton).Handle);
  end;
...
procedure TCustomAdvEditBtn.SetButtonColor(const Value: TColor);
begin
  FButtonColor := Value;
  if Assigned(FButton) then
    FButton.ButtonColor := FButtonColor;
end;

procedure TCustomAdvEditBtn.SetButtonColorDown(const Value: TColor);
begin
  FButtonColorDown := Value;
  if Assigned(FButton) then
    FButton.ButtonColorDown := FButtonColorDown;
end;

procedure TCustomAdvEditBtn.SetButtonColorHot(const Value: TColor);
begin
  FButtonColorHot := Value;
  if Assigned(FButton) then
    FButton.ButtonColorHot := FButtonColorHot;
end;
...
procedure TCustomAdvEditBtn.SetButtonRect;
var
...
procedure TCustomAdvEditBtn.SetButtonTextColor(const Value: TColor);
begin
  FButtonTextColor := Value;
  if Assigned(FButton) then
    FButton.ButtonTextColor := FButtonTextColor;
end;

procedure TCustomAdvEditBtn.SetButtonTextColorDown(const Value: TColor);
begin
  FButtonTextColorDown := Value;
  if Assigned(FButton) then
    FButton.ButtonTextColorDown := FButtonTextColorDown;
end;

procedure TCustomAdvEditBtn.SetButtonTextColorHot(const Value: TColor);
begin
  FButtonTextColorHot := Value;
  if Assigned(FButton) then
    FButton.ButtonTextColorHot := FButtonTextColorHot;
end;
