...
uses
  ...
  PictureContainer, ImgList, BtnXPVS , Types, AdvGroupBox
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF}
  ;
...
const
  ...
  // 1.8.0.0  : New : THTMLRadioGroup, THTMLCheckGroup descend from TAdvGroupBox now
  //          : New : Property DisabledShadow added to THTMLRadioGroup, THTMLCheckGroup
  // 1.8.0.1  : Improved : Positioning of controls in THTMLRadioGroup, THTMLCheckGroup when caption is empty
  // 1.8.1.0  : New : Property CheckedAsString added in THTMLCheckGroup
  // 2.0.0.0  : New : Support for HTML Emoji rendering
  // 2.0.0.1  : Fixed : DPI calculation when parent form is not Scaled
  // 2.1.0.0  : New : AutoSize property added for THTMLRadioButton, THTMLCheckBox
...
type
 ...
 protected
  ...
  TCustomHTMLRadioGroup = class(TAdvGroupbox)
  private
    FButtons: TList;
    FItems: TStrings;
    FItemIndex: Integer;
  ...
  TCustomHTMLCheckGroup = class(TAdvGroupBox)
  private
    FButtons: TList;
    FItems: TStrings;
    FColumns: Integer;
    ...
    procedure SetVersion(const Value: string);
    procedure SetDisabledShadow(const Value: boolean);
    function GetCheckedAsString: string;
    procedure SetCheckedAsString(const Value: string);
  protected
    procedure Loaded; override;
    procedure CreateWnd; override;
    procedure ReadState(Reader: TReader); override;
  ...
  public
    ...
    property CheckedAsString: string read GetCheckedAsString write SetCheckedAsString;
...
procedure TCustomHTMLCheckBox.DoAutoSize;
var
  R: TRect;
  a,s,fa: string;
  XSize,YSize: integer;
...
begin
  ...
  R := Rect(0,0,$FFFF,$FFFF);

  HTMLDrawEx(Canvas,Text,R,FImages,0,0,-1,-1,FShadowOffset,False,False,False,False,False,False,not FEllipsis,FFormScaled,1.0,FURLColor,
          clNone,clNone,FShadowColor,a,s,fa,XSize,YSize,HyperLinks,MouseLink,hr,FImageCache,FContainer,0,BidiMode);

  Width := Round(28 * FScaleFactor) + XSize;
  Height := Round(12 * FScaleFactor) + YSize;
end;
...
procedure TCustomHTMLCheckBox.SetAutoSizeEx(const Value: boolean);
begin
...
procedure THTMLRadioButton.DoAutoSize;
var
  R: TRect;
  a,s,fa: string;
  XSize,YSize: integer;
...
procedure THTMLRadioButton.SetAutoSizeEx(const Value: boolean);
begin
  if (FAutoSize <> Value) then
  begin
    FAutoSize := Value;
...
procedure TCustomHTMLRadioGroup.CreateWnd;
begin
  inherited;
...
begin
  ...
  begin
    ...
    if (Caption = '') then
      Metrics.tmHeight := 6; // only border height to take in account
    ...
    if (AutoSize in [gasVertical, gasBoth]) then
      ButtonHeight := Canvas.TextHeight('gh') + 6
    else
    ...
    try
      for I := 0 to FButtons.Count - 1 do
        ...
        begin
          ...
          DisabledShadow := Self.DisabledShadow;
...
procedure TCustomHTMLRadioGroup.AutoSizeControls;
var
  i: integer;
begin
  inherited;
  ...
  for i := 0 to FButtons.Count - 1 do
  begin
    THTMLGroupButton(FButtons[i]).Ellipsis := true;
    THTMLGroupButton(FButtons[i]).DoAutoSize;
  end;
end;
...
procedure TCustomHTMLRadioGroup.SetDisabledShadow(const Value: boolean);
begin
  if (FDisabledShadow <> Value) then
  begin
    FDisabledShadow := Value;
...
procedure TCustomHTMLCheckGroup.CreateWnd;
begin
  inherited;
  if AutoSize <> gasNone then
  begin
...
procedure TCustomHTMLCheckGroup.AutoSizeControls;
var
  i: integer;
begin
  inherited;
...
procedure TCustomHTMLCheckGroup.SetDisabledShadow(const Value: boolean);
begin
  if (FDisabledShadow <> Value) then
  begin
    FDisabledShadow := Value;
...
function TCustomHTMLCheckGroup.GetCheckedAsString: string;
var
  i: integer;
begin
  Result := '';
...
procedure TCustomHTMLCheckGroup.SetCheckedAsString(const Value: string);
var
  i: integer;
begin
  for i := 1 to Length(Value) do
  begin
    Checked[i - 1] := Value[i] = '1';
  end;
end;
