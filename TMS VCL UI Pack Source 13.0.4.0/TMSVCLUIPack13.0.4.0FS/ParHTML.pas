...
var
  ...
  var
    su,Res,TagProp,UTagProp,Prop,Tagp,LineText:string;
    cr,ir: TRect;
      ...
      begin
        ...
        begin
          ...
          'A':begin
                ...
                UTagProp := Uppercase(TagProp);

                if pos('CLASS',UTagProp) > 0 then
                begin
                  Prop := Copy(UTagProp,Pos('CLASS',UTagProp)+5,Length(UTagProp));
                  Prop := Copy(Prop,Pos('"',Prop)+1,Length(prop));
                  Prop := Copy(Prop,1,Pos('"',Prop)-1);
                  IsPassword := Prop = 'PASSWORD';
                end;

                if pos('HREF',UTagProp) > 0 then
                begin
                  Prop := Copy(TagProp,Pos('HREF',UTagProp)+4,Length(UTagProp));
                  Prop := Copy(Prop,Pos('"',Prop)+1,Length(prop));
                  Prop := Copy(Prop,1,Pos('"',Prop)-1);
                  LastAnchor := Prop;
                end;
...
procedure TPopupEdit.CMDialogKey(var Message: TCMDialogKey);
begin
...
procedure TPopupListBox.CNKeyDown(var Message: TWMKeydown);
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if Assigned(OnUpdate) then
            OnUpdate(Self,Param,TimeToStr(Time));
        end;
      end;
      Visible := false;
      if not (csDestroying in Parent.ComponentState) then
