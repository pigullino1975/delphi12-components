...
const
  ...
  // v1.4.1.1 : Fixed : High dpi issues
...
uses
  CommCtrl, ShellApi, GDIPHTMLEngine, Dialogs;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        x := Round(FHorizontalMargin * DPIScale) + S * w;
        y := Round(FVerticalMargin * DPIScale) + J * h;
...
{$IFDEF DELPHIXE14_LVL}
procedure TAdvSmoothExpanderButtonPanel.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  inherited;

  if M <> D then
  begin
    FButtonAppearance.Font.Height := MulDiv(FButtonAppearance.Font.Height, M, D);
    FButtonStatus.Appearance.Font.Height := MulDiv(FButtonStatus.Appearance.Font.Height, M, D);
  end;
end;
{$ENDIF}
...
begin
  ...
  FButtonAppearance.Font.Height := -11;
...
begin
  if Rows = -1 then
    Result := Round(GetDPIScale(Self, Self.Canvas) * ButtonHeight)
  else
    Result := (InsideRect.Bottom - Round(GetDPIScale(Self, Self.Canvas) * ButtonVerticalMargin) ) div Rows;
end;

function TAdvSmoothExpanderButtonPanel.GetButtonWidth: integer;
begin
  if Columns = -1 then
    Result := Round(GetDPIScale(Self, Self.Canvas) * ButtonWidth)
  else
    Result := (InsideRect.Right - Round(GetDPIScale(Self, Self.Canvas) * ButtonHorizontalMargin * 2)) div Columns;
end;

function TAdvSmoothExpanderButtonPanel.GetMaxButtons: integer;
begin
  Result := (InsideRect.Right - Round(GetDPIScale(Self, Self.Canvas) * ButtonHorizontalMargin)) div GetButtonWidth;
end;
...
begin
  ...
  if Assigned(FCache)  then
    FCache.Free;
  FCache := TGPBitmap.Create(Width, Height - Round(GetDPIScale(Self, Self.Canvas) * ButtonVerticalMargin));
  FValidCache := false;
end;
