...
type
  ...
  TGetEditorTypeEvent = procedure(Sender: TObject; ACol: integer; var AEditorType: TColumnEditor) of object;
  ...
  private
    ...
    FRestoreButtonGlyph: TBitmap;
    FApplyButtonGlyph: TBitmap;
    FClearButtonGlyph: TBitmap;
    ...
    procedure SetApplyButtonGlyph(const Value: TBitmap);
    procedure SetClearButtonGlyph(const Value: TBitmap);
    procedure SetRestoreButtonGlyph(const Value: TBitmap);
  protected
    procedure DoChange;
  public
  ...
  published
    ...
    property ApplyButtonGlyph: TBitmap read FApplyButtonGlyph write SetApplyButtonGlyph;
    ...
    property ClearButtonGlyph: TBitmap read FClearButtonGlyph write SetClearButtonGlyph;
    ...
    property RestoreButtonGlyph: TBitmap read FRestoreButtonGlyph write SetRestoreButtonGlyph;
  ...
  private
    ...
    FFooterAddClearButton: TBitBtn;
    FFooterAddFilterButton: TBitBtn;
    FFooterRestoreFilterButton: TBitBtn;
    FLabelFont: TFont;
    FTitleFont: TFont;
  ...
  protected
    ...
    procedure DoGetEditorType(AColumn: integer; var AEditor: TColumnEditor); virtual;
    function GetEditorType(AColumn: integer): TColumnEditor; virtual;
  ...
  public
    ...
    property FooterAddClearButton: TBitBtn read FFooterAddClearButton write FFooterAddClearButton;
    property FooterAddFilterButton: TBitBtn read FFooterAddFilterButton write FFooterAddFilterButton;
    property FooterRestoreFilterButton: TBitBtn read FFooterRestoreFilterButton write FFooterRestoreFilterButton;
...
{$IFDEF DELPHIXE10_LVL}
procedure TAdvCustomFilterPanel.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ELSE}
procedure TAdvCustomFilterPanel.ChangeScale(M, D: Integer);
{$ENDIF}
begin
  inherited;
  ...
  begin
    ...
    FHeader.Height := MulDiv(FHeader.Height, M, D);
    FFooter.Height := MulDiv(FFooter.Height, M, D);
  end;
end;
...
begin
  ...
  FooterAddFilterButton := TBitBtn.Create(Self);
  ...
  FooterAddFilterButton.Width := 96;

  FooterAddClearButton := TBitBtn.Create(Self);
  ...
  FooterAddClearButton.Width := 96;

  FooterRestoreFilterButton := TBitBtn.Create(Self);
  ...
  FooterRestoreFilterButton.Width := 96;
end;
...
begin
  ...
  FHeader.Height := Round(30 * FDPISCale);
  FFooter.Height := Round(30 * FDPIScale);

  FScrollBox.Align := alClient;

  Setcustomcontrolheight;

  FooterAddFilterButton.Width := Round(FooterAddFilterButton.Width * FDPIScale);
  FooterAddClearButton.Width := Round(FooterAddClearButton.Width * FDPIScale);
  FooterRestoreFilterButton.Width := Round(FooterRestoreFilterButton.Width * FDPIScale);
...
begin
  ...
  begin
    if Footer.Controls[Y] is TBitBtn then
    begin
      B:= TBitBtn(Footer.Controls[Y]);

      if B.Tag = tagAdd then
      begin
        B.Caption := FFilterUI.ApplyButton;
        B.Glyph.Assign(FFilterUI.ApplyButtonGlyph);
        if not FFilterUI.ApplyButtonGlyph.Empty then
          B.Width := Round(128 * FDPIScale)
        else
          B.Width := Round(96 * FDPIScale);
      end;

      if B.Tag = tagClear then
      begin
        B.Caption := FFilterUI.ClearButton;
        B.Glyph.Assign(FFilterUI.ClearButtonGlyph);
        if not FFilterUI.ClearButtonGlyph.Empty then
          B.Width := Round(128 * FDPIScale)
        else
          B.Width := Round(96 * FDPIScale);
      end;
      ...
      begin
        ...
        B.Glyph.Assign(FFilterUI.RestoreButtonGlyph);
        if not FFilterUI.RestoreButtonGlyph.Empty then
          B.Width := Round(128 * FDPIScale)
        else
          B.Width := Round(96 * FDPIScale);
      end;
...
procedure TAdvCustomFilterPanel.DoGetEditorType(AColumn: integer;
  var AEditor: TColumnEditor);
begin
  if Assigned(OnGetEditorType) then
    OnGetEditorType(Self, AColumn, AEditor);
end;
...
function TAdvCustomFilterPanel.GetEditorType(AColumn: integer): TColumnEditor;
begin
  DoGetEditorType(AColumn, Result);
end;
...
begin
  ...
  begin
    ...
    // positions
    ACB.Top := Round((containerY * FDPIScale * Position) - (20 * FDPIScale));
...
begin
  ...
  FOperationSmallerThen := 'Smaller than';
  FOperationLargerThen := 'Larger than';
  ...
  FApplyButtonGlyph := TBitmap.Create;
  FRestoreButtonGlyph := TBitmap.Create;
  FClearButtonGlyph := TBitmap.Create;
end;

destructor TFilterUI.Destroy;
begin
  FApplyButtonGlyph.Free;
  FRestoreButtonGlyph.Free;
  FClearButtonGlyph.Free;
...
procedure TFilterUI.SetApplyButtonGlyph(const Value: TBitmap);
begin
  FApplyButtonGlyph.Assign(Value);
  DoChange;
end;
...
procedure TFilterUI.SetClearButtonGlyph(const Value: TBitmap);
begin
  FClearButtonGlyph.Assign(Value);
  DoChange;
end;
...
procedure TFilterUI.SetRestoreButtonGlyph(const Value: TBitmap);
begin
  FRestoreButtonGlyph.Assign(Value);
  DoChange;
end;
