...
const
  ...
  // v1.0.8.1 : Fixed : English language initialization issue
  // v1.0.8.2 : Fixed : Issue with space in text field for filter operations
  // v1.0.9.0 : New : OnGetEditorType event added
  //          : New : OnAllowColumnFilter event added
  // v1.0.9.1 : Fixed : Issue with date based filtering
  // v1.0.9.2 : Fixed : Issue with SQL Date separator quote handling
  // v1.0.9.3 : Fixed : Issue with boolean fields in SQL command
  // v1.0.9.4 : Fixed : Issue with boolean filters for non-English Delphi
  // v1.0.9.5 : Fixed : Issue with destroying dataset while filter is active
  // v1.0.9.6 : Fixed : Issue with handling field names with spaces
  // v1.1.0.0 : New : Support for images in filter action buttons
  //          : Improved : High DPI handling for filter action buttons
  // v1.1.0.1 : Fixed : Issue with spaces in fieldname in Access when used with TAdvDBFilterPanel
  // v1.1.0.2 : Fixed : Issue in filter restore with fieldname containing spaces
  // v1.1.1.0 : New : Property SQLFalse,SQLTrue to customize default boolean field values for queries
  // v1.1.1.1 : Fixed : Issue with specific conditions for fields with & without spaces
  // v1.1.1.2 : Fixed : Issue with restore filder for specific conditions
  // v1.1.2.0 : New : Property SQLFalse,SQLTrue added in TAdvDBFilterDialog
...
type
  ...
  TAllowDBColumnFilter = procedure(Sender: TObject; AColumn: integer; AField:TField; var Allow: boolean) of object;
  ...
  private
    ...
    FOnAllowColumnFilter: TAllowDBColumnFilter;
    FSQLTrue: string;
    FSQLFalse: string;
  ...
  protected
    ...
    procedure DoGetEditorType(AColumn: integer; var AEditor: TColumnEditor); override;
  ...
  published
    ...
    property SQLTrue: string read FSQLTrue write FSQLTrue;
    property SQLFalse: string read FSQLFalse write FSQLFalse;
    ...
    property OnAllowColumnFilter: TAllowDBColumnFilter read FOnAllowColumnFilter write FOnAllowColumnFilter;
  end;
  ...
  protected
    ...
    procedure DoGetEditor(Sender: TObject; AColumn: integer; var AEditor: TColumnEditor);
    procedure DoAllowColumnFilter(Sender: TObject; AColumn: integer; AField: TField; var Allow: boolean);
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    procedure ResetFilter;
...
procedure TAdvDBFilterPanel.Notification(AComponent: TComponent;
...
begin
  ...
  begin
    ...
    if Assigned(DataSource) and (AComponent = DataSource.DataSet) then
    begin
      AddClear(Self);
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Assigned(OnAllowColumnFilter) then
         OnAllowColumnFilter(Self, i, DataSource.DataSet.Fields[i], Allow);
...
procedure TAdvDBFilterPanel.DoGetEditorType(AColumn: integer; var AEditor: TColumnEditor);
var
  T: TFieldType;
begin
  AEditor := ceText;
  ...
  inherited DoGetEditorType(AColumn, AEditor);
end;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                begin
                  if AnsiLowerCase(StringReplace(TFilterOperationItem(FQueryParams.Items[loopcount]).Column, ' ', '', [rfReplaceAll])) = AnsiLowerCase(StringReplace(combo2.Items[Y],' ','',[rfReplaceAll])) then
                  begin
                  combo2.ItemIndex := Y;
            ...
            begin
              ...
              begin
                ...
                if fltr_expr = '=' then
                begin
                  if (TFilterOperationItem(FQueryParams.Items[loopcount]).Value = FSQLTrue) or (TFilterOperationItem(FQueryParams.Items[loopcount]).Value = FSQLFalse) then
              ...
              begin
                ...
                case celltype of
                  ...
                  TFieldType.ftFixedWideChar, TFieldType.ftWideMemo, TFieldType.ftLongWord:
                  begin
                    FV := Trim(TFilterOperationItem(FQueryParams.Items[loopCount]).Value);
                    AddEdit(C, loopcount, FV);
                  end;
                  TFieldType.ftDate, TFieldType.ftDateTime, TFieldType.ftTimeStamp, TFieldType.ftOraTimeStamp:
                  begin
                  ...
                  begin
                    try
                      ...
                      b := Uppercase(FV) = Uppercase(FSQLTrue);
                    end;
                    AddBool(C, b);
                  end;
                end;
...
begin
  ...
  ((Result as TAdvFilterForm).FilterPanel as TAdvDBFilterPanel).SQLFalse := FSQLFalse;
  ((Result as TAdvFilterForm).FilterPanel as TAdvDBFilterPanel).SQLTrue := FSQLTrue;
  ...
  ((Result as TAdvFilterForm).FilterPanel as TAdvDBFilterPanel).OnGetEditorType := DoGetEditor;
  ((Result as TAdvFilterForm).FilterPanel as TAdvDBFilterPanel).OnAllowColumnFilter := DoAllowColumnFilter;
end;
...
procedure TAdvDBFilterDialog.DoAllowColumnFilter(Sender: TObject;
  AColumn: integer; AField: TField; var Allow: boolean);
begin
  if Assigned(OnAllowColumnFilter) then
    OnAllowColumnFilter(Self, AColumn, AField, Allow);
end;
...
procedure TAdvDBFilterDialog.DoGetEditor(Sender: TObject; AColumn: integer;
