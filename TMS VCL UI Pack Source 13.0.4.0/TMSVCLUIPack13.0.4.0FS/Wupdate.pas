...
const
  ...
  // 2.6.0.0 : New : Support for ZIP files
  // 2.6.0.1 : Fixed : Issue with function NewVersionAvailable when sFTP based update is used
  // 2.6.0.2 : Fixed : Issue with signing in spawned updater
  // 2.6.0.3 : Fixed : Issue with SFTP connection to a subfolder
  // 2.6.1.0 : New : SYSWOW64 folder prefix support added
  // 2.6.1.1 : Fixed : Issue with closing SFTP sessions
  // 2.6.1.2 : Fixed : Issue with SFTP connections in Delphi2007
  // 2.6.2.0 : New : Support with ADR tag for the AppData\Roaming target folder
  // 2.6.3.0 : Improved : Included ignoring weak certificates when certificate checking is disabled
  // 2.6.3.1 : Fixed : Issue in URLToFile function when request parameters are used
  //         : Improved : Using subdomain with CertCheck = ccDisabled
  // 2.6.4.0 : Improved : More extensive status information during SFTP connection establishing
  // 2.6.5.0 : Improved : High DPI handling for What's New dialog & Progress form
  // 2.6.5.1 : Improved : OnDownloadedEULA event handle to allow easier custom handling
...
type
  ...
  TWebUpdateType = (ftpUpdate,httpUpdate,fileUpdate{$IFDEF DELPHI2007_LVL},sftpUpdate{$ENDIF});
  ...
  private
    ...
    {$IFDEF DELPHI2007_LVL}
    function EncodePath(APath: WideString; CP: Word): AnsiString;
    {$ENDIF}
  protected
    ...
    function StartupSession: Boolean;
    {$ENDIF}
    {$IFDEF DELPHIXE2_LVL}
    function ZIPExtract(AZipFile, ATargetDir: string): Integer;
    {$ENDIF}
  public
   ...
   published
    ...
    {$IFDEF DELPHIXE2_LVL}
    property ExtractZIP: Boolean read FExtractZIP write FExtractZIP default false;
    {$ENDIF}
...
const
  READBUFFERSIZE = 4096;
  SECURITY_FLAG_IGNORE_WEAK_SIGNATURE = $00010000;
...
function GetAppDataRoaming: string;
var
  r: Bool;
...
begin
  ...
  if not r then
    raise Exception.Create('Could not find Application Data folder location.');
  Result := path;
end;
...
{$IFDEF DELPHIXE2_LVL}
function TWebUpdate.ZIPExtract(AZipFile, ATargetDir: string): Integer;
var
  zf: TZipFile;
begin
  zf := TZipFile.Create;
  try
    try
      zf.Open(AZipFile, zmRead);
      zf.ExtractAll(ATargetDir);
      zf.Close;
      Result := 0;
    except
      Result := -1;
...
{$IFDEF DELPHI2007_LVL}
function DynaLink_SocketConnect(S: TSocket; name: Pointer; namelen: Integer): Integer;
var
  WS2DLL: THandle;
  Socket_Connect:function(S: TSocket; name: Pointer; namelen: Integer): Integer; stdcall;
begin
  Result := -1;
  WS2DLL := LoadLibrary('ws2_32.dll');
  if WS2DLL > 0 then
  begin
    @Socket_Connect := GetProcAddress(WS2DLL,'connect');
    if Assigned(Socket_Connect) then
    begin
      Result := Socket_Connect(S,name, namelen);
    end;
    FreeLibrary(WS2DLL);
  end;
end;

function DynaLink_SocketAddrInfo(pNodeName, pServiceName: PAnsiChar; const pHints: PAddrInfo; var ppResult: PAddrInfo): Integer;
var
  WS2DLL: THandle;
  Socket_GetAddrInfo:function(pNodeName, pServiceName: PAnsiChar; const pHints: PAddrInfo; var ppResult: PAddrInfo): Integer; stdcall;
begin
  Result := -1;
  WS2DLL := LoadLibrary('ws2_32.dll');
  if WS2DLL > 0 then
  begin
    @Socket_GetAddrInfo := GetProcAddress(WS2DLL,'getaddrinfo');
    if Assigned(Socket_GetAddrInfo) then
    begin
      Result := Socket_GetAddrInfo(pNodeName, pServiceName, pHints, ppResult);
    end;
    FreeLibrary(WS2DLL);
  end;
end;

procedure DynaLink_SocketFreeAddrInfo(ai: PAddrInfo);
var
  WS2DLL: THandle;
  Socket_FreeAddrInfo:procedure(ai: PAddrInfo); stdcall;
begin
  WS2DLL := LoadLibrary('ws2_32.dll');
  if WS2DLL > 0 then
  begin
    @Socket_FreeAddrInfo := GetProcAddress(WS2DLL,'freeaddrinfo');
    if Assigned(Socket_FreeAddrInfo) then
    begin
...
{$IFDEF DELPHI2007_LVL}
function TWebUpdate.CreateSocket: Integer;
var
  FSocketBuffLen: Integer;
  WSData: TWSAData;
begin
  ...
  begin
    DoStatus(980, 'Invalid winsock version', WebUpdateInformation, ErrConnectError);
    Exit;
  end;
...
{$IFDEF DELPHI2007_LVL}
function TWebUpdate.EncodePath(APath: WideString; CP: Word): AnsiString;
var
  L: Integer;
begin
  Result := '';
  L := WideCharToMultiByte(CP, 0, PWideChar(APath), -1, nil, 0, nil, nil);
  if L > 1 then
  begin
    SetLength(Result, L - 1);
    WideCharToMultiByte(CP, 0, PWideChar(APath), -1, PAnsiChar(Result), L - 1, nil, nil)
  end;
end;
{$ENDIF}
...
begin
  ...
  begin
    ...
    if prefix = 'SYSWOW64' then
    begin
      GetWindowsDirectory(buf, sizeof(buf));
      s := StrPas(buf) + '\SysWOW64';
    end
    else
      if prefix = 'APP' then
...
begin
  ...
  begin
     ...
     begin
        CustomProcess(fn);
        if Uncompress and (Pos('.CAB',Uppercase(fn)) = 0) {$IFDEF DELPHIXE2_LVL}and (Pos('.ZIP', UpperCase(fn)) = 0){$ENDIF} then
        ...
        {$IFDEF DELPHIXE2_LVL}
        if Uncompress and (Pos('.ZIP',Uppercase(fn)) <> 0) then
        begin
          DoStatus(965,fn,WebUpdateInformation,0);
          ZIPExtract(fn,tgt);
          if not FKeepIntermediateFiles then
  ...
  {$IFDEF DELPHI2007_LVL}
  if FUpdateType = sftpUpdate then
  begin
    hintfile := nil;
    if (FFTPDirectory <> '') then
    begin
      ...
      if FSFtp <> nil then
      begin
        DirHandle := libssh2_sftp_opendir(FSFTP, PAnsiChar(EncodePath(s, FCodePage)));
    ...
    if libssh2_sftp_stat(FSFTP, PAnsiChar(EncodePath(fullp, FCodePage)), attr) = 0 then
    begin
      lpdword := attr.filesize; //file size
      FileHandle := libssh2_sftp_open(FSFTP, PAnsiChar(EncodePath(fullp, FCodePage)), LIBSSH2_FXF_READ, 0);
  ...
  else{$ENDIF} if FUpdateType = FTPUpdate then
  begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          // Add desired flags
          OpenFlags := OpenFlags or SECURITY_FLAG_IGNORE_UNKNOWN_CA or SECURITY_FLAG_IGNORE_CERT_CN_INVALID or SECURITY_FLAG_IGNORE_CERT_DATE_INVALID or SECURITY_FLAG_IGNORE_WEAK_SIGNATURE;
          // Set new flags
    ...
    if FCertCheck = ccDisable then
      OpenFlags := OpenFlags or INTERNET_FLAG_IGNORE_CERT_DATE_INVALID or INTERNET_FLAG_IGNORE_CERT_CN_INVALID or SECURITY_FLAG_IGNORE_UNKNOWN_CA or SECURITY_FLAG_IGNORE_WEAK_SIGNATURE;
...
retryafterlogin:
  ...
  if ({$IFDEF DELPHI2007_LVL}(FUpdateType <> sftpUpdate) and{$ENDIF}(hintfile = nil){$IFDEF DELPHI2007_LVL}) or ((FUpdateType = sftpUpdate) and (FileHandle = nil){$ENDIF}) then
  begin
    Error;
  ...
  {$IFDEF DELPHI2007_LVL}trns := 0;{$ENDIF}
  FSize := 0;
  ...
  begin
    ...
    GetMem(fsbuf, bufsize);
    try
      repeat
        Application.ProcessMessages;
        R := libssh2_sftp_read(FileHandle, fsbuf, bufsize);
        if R > 0 then
        begin
          N := FS.Write(fsbuf^, R);
          if N > 0 then
          ...
          HandleProgress(fn, trns, lpdword);
  ...
  begin
    ...
    if UnCompress and (UpperCase(ExtractFileExt(fn)) <> '.CAB') {$IFDEF DELPHIXE2_LVL}and (UpperCase(ExtractFileExt(fn)) <> '.ZIP'){$ENDIF} then
      ExpandFile(ansistring(fn));
    ...
    {$IFDEF DELPHIXE2_LVL}
    if UnCompress and (UpperCase(ExtractFileExt(fn)) = '.ZIP') then
    begin
      ZIPExtract(fn,tgt);
      if not FKeepIntermediateFiles then
        DeleteFile(pchar(fn));
...
begin
  ...
  if Assigned(Application.MainForm) and Assigned(Application.MainForm.Canvas) then
    LDPIScale := CalcDPIScale(Application.MainForm.Canvas,Application.MainForm);

  form := TForm.Create(Application);
  form.Position := poScreenCenter;

  form.Width := Round(400 * LDPIScale);
  form.Height := Round(400 * LDPIScale);
  form.Caption := Caption;
  form.BorderStyle := bsDialog;
  ...
  begin
    ...
    begin
      ...
      memo.Height := form.ClientRect.Bottom - Round(60 * LDPIScale);
  ...
  ok.Parent := form;
  ok.Caption := okbtn;
  ok.Top := ctrlheight + ctrltop + Round(10 * LDPIScale);
  ok.Width := Round(ok.Width * LDPIScale);
  ok.Left := ctrlWidth + ctrlleft - ok.Width - ok.Width - 10;
  ok.Height := Round(ok.Height * LDPIScale);
  ok.TabOrder := 0;
  ok.ModalResult := mrOk;
  ...
  cancel.Top := ctrlheight + ctrltop + Round(10 * LDPIScale);
  cancel.Left := ctrlwidth + ctrlleft - ok.Width;
  cancel.Height := Round(cancel.Height * LDPIScale);
  cancel.Width := Round(cancel.Width * LDPIScale);
...
begin
  ...
  begin
    ...
    try
      ...
      // no positive handling via dialog and no RTF, so event handler could update EULA
      if (Res = mrCancel) and not RTF then
      begin
        ms.Clear;
        sl.SaveToStream(ms);
      end;
    ...
    // was positive handled via custom dialog
    if (Res = mrOk) then
      Exit;
  end;
...
begin
  ...
  {$IFDEF FREEWARE}
  ShowMessage('Starting application update with'#13#10'TWebUpdate © 1998-2020 by tmssoftware.com');
  {$ENDIF}
...
{$IFDEF DELPHI2007_LVL}
function TWebUpdate.StartupSession: Boolean;
var
  S, R, I, limit: Integer;
  auth: Boolean;
  err: string;
...
begin
  ...
  if (Authenticate = waAlways) or ((Authenticate = waAuto) and ((FUserID = '') or (FPassword = ''))) then
  begin
    if not GetLogin('Connect to '+URLToDomain(url),szUser, szPassword) then
    begin
      DoStatus(956,'Authentication cancelled',0,0);
  ...
  if R = 0 then
  begin
    libssh2_session_set_blocking(FSession, 1); //set to blocking

    UserAuthList := libssh2_userauth_list(FSession, PAnsiChar(AnsiString(FUserID)), Length(AnsiString(FUserID)));
    auth := libssh2_userauth_authenticated(FSession) > 0;
    if not auth then
    begin
      if not Assigned(UserAuthList) then
      begin
        DoStatus(980, 'Could not get auth list', WebUpdateInformation, ErrConnectError);
        Exit;
      end;
    ...
    Auth_Repeat : //auth repeat
      ...
      begin
        if err = '' then
        begin
          if (R <> 0) and Assigned(FSession) then //we are connected
          begin
            I := 0;
        ...
        DoStatus(980, err, WebUpdateInformation, ErrConnectError);
        Exit;
      end;

      Inc(I);

      sUserName := AnsiString(FUserID);
      pUserName := PAnsiChar(sUserName);
      sPassword := AnsiString(FPassword);
      pPassword := PAnsiChar(sPassword);
      try
        S := libssh2_userauth_password(FSession, pUserName, pPassword);
        auth := S = 0;
        if not auth then
        begin
          I := 0;
...
begin
  ...
  begin
    ...
    begin
      DoStatus(980, 'Cannot open socket', WebUpdateInformation, ErrConnectError);
      Exit;
    end;
    ...
    begin
      DoStatus(980, 'Unable to initialize libssh2.dll', WebUpdateInformation, ErrConnectError);
      Exit;
    end;
    ...
    abst.SelfPtr := Self;
    abst.Extra := nil;
    FSession := libssh2_session_init_ex(nil, nil, nil, @abst);

    //Startup session
    if not StartupSession then
    begin
      DoStatus(980, 'Cannot start SSH session', WebUpdateInformation, ErrConnectError);
      Exit;
    end;
    ...
    if FSFTP <> nil then
    begin
      DoStatus(986, 'Succesfull SFTP session', WebUpdateInformation, 0);
      Result := WU_SUCCESS
    end
    else
      DoStatus(980, 'Could not initialize SFTP session', WebUpdateInformation, ErrConnectError);
  end
  else
  {$ENDIF}
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        try
          libssh2_session_disconnect(FSession, PAnsiChar('Going to shutdown. Bye.'));
        finally
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Compressed := IniFile.ReadInteger(filekey,'compressed',0) <> 0;
    ...
    begin
      ...
      begin
        ...
        if (UpperCase(ExtractFileExt(UFName)) <> '.CAB') and
           ...
           {$IFDEF DELPHIXE2_LVL}
           (UpperCase(ExtractFileExt(UFName)) <> '.ZIP') and
           {$ENDIF}
           (FUseWinTempDir) and
        ...
        if (((UpperCase(ExtractFileExt(UFName)) <> '.CAB') and FExtractCAB)
          {$IFDEF DELPHIXE2_LVL} or ((UpperCase(ExtractFileExt(UFName)) <> '.ZIP') and FExtractZIP) {$ENDIF}) and
          FFileList.Items[i - 1].Compressed and
          not inAppComps then
        begin
        ...
        {$IFDEF DELPHIXE2_LVL}
        if (UpperCase(ExtractFileExt(UFName)) = '.ZIP') and
          FExtractZIP and FFileList.Items[i - 1].Compressed and
          not inAppComps then
        begin
          if ZIPExtract(AddBackSlash(tgtdir) + UFName, FFileList.Items[i - 1].TargetDir) = -1 then
            DoStatus(947,AddBackSlash(tgtdir) + UFName,WebUpdateZIPError,0)
          else
          begin
            if not FKeepIntermediateFiles then
              DeleteFile(PChar(AddBackSlash(tgtdir) + UFName));
...
begin
  ...
  begin
    ...
    if (FUpdateType = ftpUpdate) {$IFDEF DELPHI2007_LVL}or (FUpdateType = sftpUpdate){$ENDIF} then
    begin
      if Host = '' then
    ...
    if (FHintConnect <> nil) {$IFDEF DELPHI2007_LVL}or (FSFTP <> nil){$ENDIF} or (FUpdateType = fileUpdate) then
    begin
...
begin
  ...
  begin
    ...
    *)
    {$IFDEF USEUAC}
    ShellExecute(0,'open',pchar(wintempdir + 'upd.exe'),pchar(commandline),nil,SW_SHOW);
    {$ENDIF}
...
{$IFDEF DELPHI2007_LVL}
function TWebUpdate.ConnectSocket: Boolean;
var
  connRes: Integer;
  P: PAddrInfo;
  hints: addrinfo;
begin
  ...
  if DynaLink_SocketAddrInfo(PAnsiChar(AnsiString(FHost)), PAnsiChar(AnsiString(IntToStr(FPort))), @hints, P) <> 0 then
  begin
    DoStatus(980, SysErrorMessage(WSAGetLastError), WebUpdateInformation, ErrConnectError);
    Exit;
  end;

  while P <> nil do
  begin
    connRes := DynaLink_SocketConnect(FSocket, P^.ai_addr, P^.ai_addrlen);
    if connRes <> -1 then
      break;
    P := P^.ai_next;
  end;
...
begin
  ...
  {$ENDIF}
  {$IFDEF USEUAC}
  srcname := wintempdir + 'wusetup.cab';
  AssignFile(binfile,srcname);
  Rewrite(binfile);
  Blockwrite(binfile,ptr^,ressize);
  Closefile(binfile);
  dstname := wintempdir + 'upd.exe';
  CABExtract(srcname, wintempdir);
  {$IFDEF DELPHI_UNICODE}
  DeleteFile(pwidechar(string(srcname)));
  {$ENDIF}
  {$IFNDEF DELPHI_UNICODE}
...
begin
  ...
  Width := Round(300 *  LDPIScale);
  Height := Round(110 * LDPIScale);
end;
...
begin
  ...
  FProgressBar.Width := ClientWidth - 20;
  {$IFDEF DELPHIXE_LVL}
