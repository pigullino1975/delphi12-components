...
uses
  ...
  AdvFileNameEdit, AdvDirectoryEdit, Dialogs, LUCombo, PlannerDatePicker;
...
// Lookup column combo box editor class
  ...
  [ComponentPlatformsAttribute(pidWin32 or pidWin64)]
  {$ENDIF}
  TPlannerCalendarEditLink = class(TEditLink)
  private
    FEdit: TPlannerDatePicker;
    procedure PlannerDatePicker1DaySelect(Sender: TObject; SelDate: TDateTime);
  protected
    procedure EditExit(Sender:TObject);
  public
    procedure CreateEditor(aParent:TWinControl); override;
...
procedure TPlannerCalendarEditLink.CreateEditor(aParent: TWinControl);
begin
  inherited;
  FEdit := TPlannerDatePicker.Create(Grid);
  FEdit.BorderStyle := bsNone;
  FEdit.OnKeydown := EditKeyDown;
  FEdit.Width := 0;
  FEdit.Height := 0;
  ...
  WantKeyLeftRight := true;
  WantKeyHomeEnd := true;
  FEdit.Calendar.ShowToday := True;
  FEdit.Calendar.ShowGotoToday := True;
  FEdit.Calendar.StartDay := 1;
  FEdit.Calendar.AutoThemeAdapt := True;

  FEdit.OnDaySelect := PlannerDatePicker1DaySelect;
  FEdit.OnExit := EditExit;
end;

procedure TPlannerCalendarEditLink.DestroyEditor;
begin
  FreeAndNil(fEdit);
  inherited;
end;
...
function TPlannerCalendarEditLink.GetEditControl: TWinControl;
begin
   Result := fEdit;
end;

function TPlannerCalendarEditLink.GetEditorValue: string;
begin
  Result := fEdit.Text;
end;

procedure TPlannerCalendarEditLink.PlannerDatePicker1DaySelect(
  Sender: TObject; SelDate: TDateTime);
begin
  HideEditor;
end;

procedure TPlannerCalendarEditLink.SetEditorValue(s: string);
begin
  FEdit.UIStyle := grid.UIStyle;
  FEdit.Text := s;
end;
