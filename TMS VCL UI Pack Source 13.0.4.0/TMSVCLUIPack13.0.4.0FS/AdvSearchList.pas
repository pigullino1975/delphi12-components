...
uses
  Classes, TypInfo, Generics.Collections, AdvPersistence,
...
const
  ...
  TTMSFNCSearchListDocURL = 'https://download.tmssoftware.com/doc/tmsfncuipack/components/ttmsfncsearchlist/';
  ...
  // v1.0.10.1: Fixed : Issue with OnDblClick event triggering
  // v1.1.10.2: Improved : Tab key handling
  // v1.1.11.0: New : UIStyle property for TMSStyles
  // v1.1.11.1: Fixed : VCL styles check updated to work in 10.4 Sydney
  // v1.1.11.2: Fixed : Selection color intialization with VCL Styles when seFont = false in StyleElements
  // v1.1.11.3: Fixed : Issue with WheelIncrement < 0
...
type
  ...
  TAdvCustomSearchList = class(TCustomScrollingControl, IAdvPersistence {$IFNDEF FNCLIB}, ITMSStyle {$ENDIF})
  private
  ...
  protected
    ...
    function CanSaveProperty(AObject: TObject; APropertyName: string; APropertyType: TTypeKind): Boolean; {$IFDEF FNCLIB} override; {$ENDIF}
    function CanLoadProperty(AObject: TObject; APropertyName: string; APropertyType: TTypeKind): Boolean; {$IFDEF FNCLIB} override; {$ENDIF}
    function IsAppearanceProperty({%H-}AObject: TObject; {%H-}APropertyName: string; {%H-}APropertyType: TTypeKind): Boolean;  {$IFDEF FNCLIB} override; {$ENDIF}
    ...
    function CategoryWithID(id: integer): TCategoryItem;
  ...
  public
    ...
    procedure SaveSettingsToFile(AFileName: string; AAppearanceOnly: Boolean = False);  {$IFDEF FNCLIB} override; {$ENDIF}
    procedure LoadSettingsFromFile(AFileName: string);  {$IFDEF FNCLIB} override; {$ENDIF}
    procedure SaveSettingsToStream(AStream: TStreamEx; AAppearanceOnly: Boolean = False);  {$IFDEF FNCLIB} override; {$ENDIF}
    procedure LoadSettingsFromStream(AStream: TStreamEx);  {$IFDEF FNCLIB} override; {$ENDIF}
    ...
    function SelectPrevPageItem(PageSize: integer): boolean;
    function SelectNextPageItem(PageSize: integer): boolean;
...
begin
  ...
  tag := AnsiUppercase(tag); // an upper-case version of a tag used in the rest of the function
  hs := AnsiUppercase(s); // an upper-case version of the searched text
...
function TAdvCustomSearchList.CategoryWithID(id: integer): TCategoryItem;
var
  i: integer;
begin
  Result := nil;
...
begin
  ...
  begin
    SelectNextPageItem(WheelIncrement);
    ScrollToItem(ItemIndex);
  end
  else
  begin
    SelectPrevPageItem(WheelIncrement);
    ScrollToItem(ItemIndex);
...
begin
  ...
  begin
    cat := CategoryWithID(AItem.CategoryID);

    if (AItem.CategoryID >= 0) and (AItem.CategoryID < Categories.Count) and Assigned(Cat) and Cat.Filter then
    begin
      Result := false;
      Exit;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        fis := AnsiUppercase(AItem.Captions[col]);
      end;
    ...
    begin
      ...
      begin
        fs := FilterCondition.FTextUppercase;
        fis := AnsiUppercase(AItem.Descriptions[col]);
      end;
...
begin
  ...
  KEY_PRIOR:
    begin
      if SelectPrevPageItem(+4) then
        ScrollToItem(ItemIndex);
    end;
  KEY_NEXT:
    begin
      if SelectNextPageItem(+4) then
        ScrollToItem(ItemIndex);
    end;
  end;
end;
  ...
  KEY_TAB:
    begin
     ...
     {$ENDIF}
      ...
      if Assigned(OnKeyUp) then // ... tab key to be handled in TTMSFNCSearchDropDown.SearchListKeyUp in this case
        OnKeyUp(Self, Key, {$IFDEF FMXLIB} KeyChar, {$ENDIF} Shift);
    end;
  end;
end;
...
function TAdvCustomSearchList.IsAppearanceProperty(AObject: TObject; APropertyName: string; APropertyType: TTypeKind): Boolean;
var
...
function TAdvCustomSearchList.CanSaveProperty(AObject: TObject; APropertyName: string; APropertyType: TTypeKind): Boolean;
begin
...
function TAdvCustomSearchList.CanLoadProperty(AObject: TObject; APropertyName: string; APropertyType: TTypeKind): Boolean;
begin
  if (AObject = Self) then
    Result := (TAdvUtils.IndexOfTextInArray(APropertyName, ExcludePropertyList) = -1)
  else
...
procedure TAdvCustomSearchList.SaveSettingsToFile(AFileName: string; AAppearanceOnly: Boolean = False);
begin
...
procedure TAdvCustomSearchList.LoadSettingsFromFile(AFileName: string);
begin
  BeginUpdate;
...
procedure TAdvCustomSearchList.SaveSettingsToStream(AStream: TStreamEx; AAppearanceOnly: Boolean = False);
begin
  FAppearancePersisting := AAppearanceOnly;
...
procedure TAdvCustomSearchList.LoadSettingsFromStream(AStream: TStreamEx);
begin
  BeginUpdate;
...
begin
  ...
  try
          ...
          begin
            ...
            {$IFNDEF FNCLIB}
              ...
              begin
                b := TAdvBitmap.Create;
                try
                  b.Assign(Items[li].Columns[j].Picture);
                  grb.DrawBitmap(ConvertToRectf(r), b);
                finally
                  b.Free;
                end;
              end;
    ...
    try
      b.Assign(grb.Bitmap);
      gr.DrawBitmap(RectF(0,0,Width,Height), b);
    finally
      b.Free;
    end;
...
{$IFDEF FNCLIB}
{$IFDEF ANDROIDSUPPORT}
procedure TAdvCustomSearchList.AddFocus;
begin

end;

procedure TAdvCustomSearchList.RemoveFocus;
begin
...
{$IFNDEF FNCLIB}
procedure TAdvCustomSearchList.SetComponentStyle(AStyle: TTMSStyle);
var
  ...
  begin
    case AStyle of
      ...
      begin
        BorderWidth := 0;
        Color := clWhite;
        ItemFontColor := $00444648;
        Appearance.SelectionColor := $00F2D5C2;
        Appearance.SelectionTextColor := $003B3B3B;
      ...
      begin
        BorderWidth := 0;
        Color := $00B8BBBE;
        ItemFontColor := $00232425;
        Appearance.SelectionColor := $00969696;
        Appearance.SelectionTextColor := $003B3B3B;
      ...
      begin
        BorderWidth := 0;
        Color := $00444444;
        ItemFontColor := clWhite;
        Appearance.SelectionColor := $00505050;
        Appearance.SelectionTextColor := clWhite;
    ...
    begin
      ...
      begin
        Items[i].Columns[j].TextColor := ItemFontColor;
      end;
    end;
  end;
end;
...
function TAdvCustomSearchList.SelectPrevPageItem(PageSize: integer): boolean;
var
  d,i: integer;
begin
  Result := false;
  ...
  if (ItemIndex > 0) or (PageSize < 0) then
  begin
    d := ItemIndex;
    i := Max(1, ItemIndex - PageSize);
    if i >= Rows then
      i := Rows;
...
function TAdvCustomSearchList.SelectNextPageItem(PageSize: integer): boolean;
var
  d,i: integer;
begin
  Result := false;
  ...
  if (ItemIndex < Rows - 1) or (PageSize < 0) then
  begin
    d := ItemIndex;
    i := Min(ItemIndex + PageSize, Rows - 2);
    if i < 0 then
      i := -1;
...
{$IFNDEF FNCLIB}
procedure TAdvCustomSearchList.SetUIStyle(const Value: TTMSStyle);
begin
...
begin
  FText := Value;
  FTextUppercase := AnsiUpperCase(FText);
end;
