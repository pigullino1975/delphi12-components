...
uses
  ...
  ParXPVS, Contnrs, Types, AdvStyleIF, ImgList
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF}
  ;
...
const
  ...
  // 1.5.0.2 : Fixed : Issue with closing custom editor from application
  //         : Fixed : Issue when using in 64bit app
...
uses
  ...
  VCL.Themes,
  {$ENDIF}
  Consts, Shellapi, ShlObj, ActiveX, CommCtrl, Variants;
...
procedure TParamCheckList.CMChildKey(var Message: TCMChildKey);
begin
...
procedure TParamCheckList.CNSysKeyDown(var Message: TWMKeyDown);
begin
  outputdebugstring('sys key');
  inherited;
end;
...
var
  obj: TParamCheckListDataWrapper;
begin
  if Checked <> GetChecked(Index) then
  begin
    obj := TParamCheckListDataWrapper(GetWrapper(Index));
...
procedure TParamCheckList.SetImageList(const Value: TCustomImageList);
begin
  FImages := Value;
  Invalidate;
end;
...
procedure TParamCheckList.WMSetFocus(var Message: TMessage);
begin
  inherited;
  FIsEditing := false;
end;

procedure TParamCheckList.WMSysKeyDown(var Message: TWMSysKeyDown);
begin
  outputdebugstring('sys key');
