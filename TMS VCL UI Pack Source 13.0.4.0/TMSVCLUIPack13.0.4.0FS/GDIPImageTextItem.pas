...
const
  ...
  //v1.0.1.0 : New : Support for per monitor DPI Handling
  //v1.0.1.1 : Fixed : Image was incorrectly scaled on high DPI
  //v1.0.1.2 : Fixed : Image scales when Imagelist or PictureContainer is used or it is a SVG
...
procedure TImageTextItem.DrawImage(g: TGPGraphics; ARect: TGPRectF; ItemAppearance: TItemAppearance);
var
  r: TGPRectF;
  imgr: TGPRectF;
  imgw, imgh: single;
begin
  ...
  begin
    ...
    imgr := MakeRect(r.X + Round(DPIScale * Spacing), r.Y + (r.Height - imgh) / 2, imgw, imgh);
...
begin
  ...
  begin
    r.X := r.X + imgw + Round(ADPI * Spacing);
    r.Width := r.Width - imgw + Round(ADPI * Spacing);
  end;
  Result := r;
  ...
  begin
    ...
    begin
      ...
      if CaptionSize <> -1 then
        f := TGPFont.Create(ff, Round(CaptionSize * ADPI), fs , UnitPoint)
      else
        f := TGPFont.Create(ff, Round(ft.Size * ADPI), fs , UnitPoint);
      ...
      textr := MakeRect(r.X + Round(ADPI * CaptionMargin.Left), r.Y + Round(ADPI * CaptionMargin.Top), r.Width - Round(ADPI * CaptionMargin.Right) - Round(ADPI * CaptionMargin.Left),
        r.Height - Round(ADPI * CaptionMargin.Bottom) - Round(ADPI * CaptionMargin.Top));
