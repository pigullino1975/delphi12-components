...
const
  ...
  // 1.2.0.0   : New : Support for Office 2019 styles
  // 1.2.0.1   : Fixed : Design-time style painting
  // 1.2.1.0   : New : UIStyle property for office look
  //           : Improved : On creation check for enabled AdvFormStyler
  // 1.3.0.0   : New : Support for use of TVirtualImageList for use in high-DPI scenarios
  // 1.3.0.1   : Fixed : Issue with prior button image index assignment
...
type
  ...
  TImageIndexes = class(TPersistent)
  private
    FSearchIndex: integer;
    FCancelIndex: integer;
    FPostIndex: integer;
    FSetBookMarkIndex: integer;
    ...
    FLastIndex: integer;
    FPriorIndex: integer;
    FGotoBookMarkIndex: integer;
    FRefreshIndex: integer;
    FOnChange: TNotifyEvent;
    procedure SetCancelIndex(const Value: integer);
    procedure SetDeleteIndex(const Value: integer);
    procedure SetEditIndex(const Value: integer);
    procedure SetFirstIndex(const Value: integer);
    procedure SetGotoBookMarkIndex(const Value: integer);
    procedure SetInsertIndex(const Value: integer);
    procedure SetLastIndex(const Value: integer);
    procedure SetNextIndex(const Value: integer);
    procedure SetPostIndex(const Value: integer);
    procedure SetPriorIndex(const Value: integer);
    procedure SetSearchIndex(const Value: integer);
    procedure SetSetBookMarkIndex(const Value: integer);
    procedure SetRefreshIndex(const Value: integer);
  protected
    procedure Changed;
  ...
  published
    ...
    property PriorIndex: integer read FPriorIndex write SetPriorIndex default -1;
    property NextIndex: integer read FNextIndex write SetNextIndex default -1;
    property LastIndex: integer read FLastIndex write SetLastIndex default -1;
    property InsertIndex: integer read FInsertIndex write SetInsertIndex default -1;
    property DeleteIndex: integer read FDeleteIndex write SetDeleteIndex default -1;
    property EditIndex: integer read FEditIndex write SetEditIndex default -1;
    property PostIndex: integer read FPostIndex write SetPostIndex default -1;
    property CancelIndex: integer read FCancelIndex write SetCancelIndex default -1;
    property RefreshIndex: integer read FRefreshIndex write SetRefreshIndex default -1;
    property SearchIndex: integer read FSearchIndex write SetSearchIndex default -1;
    property SetBookmarkIndex: integer read FSetBookMarkIndex write SetSetBookMarkIndex default -1;
    property GotoBookMarkIndex: integer read FGotoBookMarkIndex write SetGotoBookMarkIndex default -1;
  end;
  ...
  private
    ...
    procedure SetImageIndexes(const Value: TImageIndexes);
  protected
    ...
    procedure ImageIndexesChanged(Sender: TObject);
  ...
  published
    ...
    property ImageIndexes: TImageIndexes read FImageIndexes write SetImageIndexes;
...
begin
  ...
  if (csDesigning in ComponentState) and not
    ...
    Style := GetDefaultStyle(AOwner);
  FStyle := tsCustom;
  FImageIndexes := TImageIndexes.Create;
  FImageIndexes.OnChange := ImageIndexesChanged;
...
procedure TDBAdvGlowNavigator.ImageIndexesChanged(Sender: TObject);
begin
  FButtons[nbFirst].ImageIndex := FImageIndexes.FirstIndex;
  FButtons[nbPrior].ImageIndex := FImageIndexes.PriorIndex;
  FButtons[nbNext].ImageIndex := FImageIndexes.NextIndex;
  FButtons[nbLast].ImageIndex := FImageIndexes.LastIndex;
  FButtons[nbInsert].ImageIndex := FImageIndexes.InsertIndex;
  FButtons[nbDelete].ImageIndex := FImageIndexes.DeleteIndex;
  FButtons[nbEdit].ImageIndex := FImageIndexes.EditIndex;

  FButtons[nbPost].ImageIndex := FImageIndexes.PostIndex;
  FButtons[nbCancel].ImageIndex := FImageIndexes.CancelIndex;
  FButtons[nbRefresh].ImageIndex := FImageIndexes.RefreshIndex;
  FButtons[nbSearch].ImageIndex := FImageIndexes.SearchIndex;
  FButtons[nbSetBookmark].ImageIndex := FImageIndexes.SetBookmarkIndex;
  FButtons[nbGotoBookmark].ImageIndex := FImageIndexes.GotoBookMarkIndex;

  InitButtonImages;
end;

procedure TDBAdvGlowNavigator.InitButtonImages;
var
  I: TAdvNavigateBtn;
...
begin
  ...
  begin
    ...
    begin
      ...
      Btn.GlyphHot.Assign(nil);
      Btn.GlyphDisabled.Assign(nil);
      Btn.HotPicture.Assign(nil);
    ...
    begin
      ...
      if (FindResource(HInstance,PChar(ResName), RT_RCDATA) <> 0) then
        Btn.HotPicture.LoadFromResourceName(Hinstance,ResName)
      else
...
procedure TDBAdvGlowNavigator.SetImageIndexes(const Value: TImageIndexes);
begin
...
procedure TDBAdvGlowNavigator.SetImages(const Value: TCustomImageList);
var
  I: TAdvNavigateBtn;
begin
  FImages := Value;
   ...
   // Windows10
    ...
    // Office2019White
      ...
      begin
        ...
        ColorHot := $00F2E1D5;
        ColorHotTo := clNone;
        BorderColorHot := $00F2E1D5;
        ColorDown := $00E3BDA3;
        ColorDownTo := clNone;
        BorderColorDown := $00E3BDA3;
        Transparent := True;
      end;
    // Office2019Gray
      22:
      begin
        Color := $00B8BBBE;
        ColorTo := clNone;
        BorderColor := $00666666;
        ColorHot := $00969696;
        ColorHotTo := clNone;
        BorderColorHot := $00969696;
        ColorDown := $00666666;
        ColorDownTo := clNone;
        BorderColorDown := $00666666;
        Transparent := True;
      end;
    // Office2019Black
      23:
      begin
        Color := $00303132;
        ColorTo := clNone;
        BorderColor := $00ADB0B3;
        ColorHot := $00686868;
        ColorHotTo := clNone;
        BorderColorHot := $00686868;
        ColorDown := $00828282;
        ColorDownTo := clNone;
        BorderColorDown := $00828282;
        Transparent := True;
      end;
    end;
  end;
...
procedure TImageIndexes.Assign(Source: TPersistent);
begin
  if (Source is TImageIndexes) then
  begin
    FCancelIndex := (Source as TImageIndexes).CancelIndex;
    FSearchIndex := (Source as TImageIndexes).SearchIndex;
    FPostIndex := (Source as TImageIndexes).PostIndex;
    FSetBookMarkIndex := (Source as TImageIndexes).SetBookmarkIndex;
    FDeleteIndex := (Source as TImageIndexes).DeleteIndex;
    FFirstIndex := (Source as TImageIndexes).FirstIndex;
    FEditIndex := (Source as TImageIndexes).EditIndex;
    FInsertIndex := (Source as TImageIndexes).InsertIndex;
    FNextIndex := (Source as TImageIndexes).NextIndex;
    FLastIndex := (Source as TImageIndexes).LastIndex;
    FPriorIndex := (Source as TImageIndexes).PriorIndex;
    FGotoBookMarkIndex := (Source as TImageIndexes).GotoBookMarkIndex;
    FRefreshIndex := (Source as TImageIndexes).RefreshIndex;
  end;
end;

procedure TImageIndexes.Changed;
begin
  if Assigned(OnChange) then
    OnChange(Self);
end;

constructor TImageIndexes.Create;
begin
  inherited Create;
  FCancelIndex := -1;
  FSearchIndex := -1;
  ...
  FLastIndex := -1;
  FPriorIndex := -1;
  FGotoBookMarkIndex := -1;
  FRefreshIndex := -1;
end;


procedure TImageIndexes.SetCancelIndex(const Value: integer);
begin
  FCancelIndex := Value;
  Changed;
end;

procedure TImageIndexes.SetDeleteIndex(const Value: integer);
begin
  FDeleteIndex := Value;
  Changed;
end;

procedure TImageIndexes.SetEditIndex(const Value: integer);
begin
  FEditIndex := Value;
  Changed;
end;

procedure TImageIndexes.SetFirstIndex(const Value: integer);
begin
  FFirstIndex := Value;
  Changed;
end;

procedure TImageIndexes.SetGotoBookMarkIndex(const Value: integer);
begin
  FGotoBookMarkIndex := Value;
  Changed;
end;

procedure TImageIndexes.SetInsertIndex(const Value: integer);
begin
  FInsertIndex := Value;
  Changed;
end;

procedure TImageIndexes.SetLastIndex(const Value: integer);
begin
  FLastIndex := Value;
  Changed;
end;

procedure TImageIndexes.SetNextIndex(const Value: integer);
begin
  FNextIndex := Value;
  Changed;
end;

procedure TImageIndexes.SetPostIndex(const Value: integer);
begin
  FPostIndex := Value;
  Changed;
end;

procedure TImageIndexes.SetPriorIndex(const Value: integer);
begin
  FPriorIndex := Value;
  Changed;
end;

procedure TImageIndexes.SetRefreshIndex(const Value: integer);
begin
  FRefreshIndex := Value;
  Changed;
end;

procedure TImageIndexes.SetSearchIndex(const Value: integer);
begin
  FSearchIndex := Value;
  Changed;
end;

procedure TImageIndexes.SetSetBookMarkIndex(const Value: integer);
begin
  FSetBookMarkIndex := Value;
  Changed;
end;
