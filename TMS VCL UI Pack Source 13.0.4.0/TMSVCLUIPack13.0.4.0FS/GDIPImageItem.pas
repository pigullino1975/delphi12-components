...
const
  ...
  //v1.0.1.0 : New : Per monitor DPI support
  //v1.0.1.1 : Fixed : Discription font size and runtime image size on high DPI
...
begin
  ...
  begin
    case ImageTextLayout of
      ilLeftRight: imgr := MakeRect(ARect.X + Round(ADPIScale * ImageMargin.Left), ARect.Y + (ARect.Height - Round(ADPIScale * ImageHeight)) / 2, Round(ADPIScale * ImageWidth), Round(ADPIScale * ImageHeight));
      ilRightLeft: imgr := MakeRect(ARect.X + ARect.Width - Round(ADPIScale * ImageMargin.Left) - Round(ADPIScale * ImageWidth), ARect.Y + (ARect.Height - Round(ADPIScale * ImageHeight)) / 2, Round(ADPIScale * ImageWidth), Round(ADPIScale * ImageHeight));
      ilTopBottom: imgr := MakeRect(ARect.X + (ARect.Width - Round(ADPIScale * ImageWidth)) / 2, ARect.Y + Round(ADPIScale * ImageMargin.Top), Round(ADPIScale * ImageWidth), Round(ADPIScale * ImageHeight));
      ilBottomTop: imgr := MakeRect(ARect.X + (ARect.Width - Round(ADPIScale * ImageWidth)) / 2, ARect.Y + ARect.Height - Round(ADPIScale * ImageMargin.Bottom) - Round(ADPIScale * ImageHeight) , Round(ADPIScale * ImageWidth), Round(ADPIScale * ImageHeight));
    end;
...
begin
  ...
  begin
    case ImageTextLayout of
      ilLeftRight:
      begin
        imgr := MakeRect(r.X + Round(DPIScale * ImageMargin.Left), r.Y + (r.Height - Round(DPIScale * ImageHeight)) / 2, Round(DPIScale * ImageWidth), Round(DPIScale * ImageHeight));
        r.X := r.X + imgr.Width;
      ...
      ilRightLeft:
      begin
        imgr := MakeRect(r.X + r.Width - Round(DPIScale * ImageMargin.Left) - ImageWidth, r.Y + (r.Height - ImageHeight) / 2, ImageWidth, ImageHeight);
        r.Width := r.Width - imgr.Width;
      end;
      ilTopBottom:
      begin
        imgr := MakeRect(r.X + (r.Width - Round(DPIScale * ImageWidth)) / 2, r.Y + Round(DPIScale * ImageMargin.Top) , Round(DPIScale * ImageWidth), Round(DPIScale * ImageHeight));
        r.Y := r.Y + imgr.Height;
        r.Height := r.Height - imgr.Height;
      end;
      ilBottomTop:
      begin
        imgr := MakeRect(r.X + (ARect.Width - ImageWidth) / 2, r.Y + r.Height - Round(DPIScale * ImageMargin.Bottom) - ImageHeight , ImageWidth, ImageHeight);
        r.Height := r.Height - imgr.Height;
      end;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      if DescriptionSize <> -1 then
        f := TGPFont.Create(ff, Round(DPIScale * DescriptionSize), fs, UnitPoint)
      else
        f := TGPFont.Create(ff, Round(DPIScale * ft.Size), fs, UnitPoint);
