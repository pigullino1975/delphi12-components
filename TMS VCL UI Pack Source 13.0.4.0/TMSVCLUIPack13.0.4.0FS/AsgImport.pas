...
type
  ...
  private
    ...
    {$ENDIF}
  protected
    procedure FormFileProgress(Sender: TObject; progress: SmallInt);
  public
    ...
    property OnFileProgress: TGridProgressEvent read FOnFileProgress write FOnFileProgress;
  end;
  ...
  protected
    ...
    procedure DlgFileProgress(Sender: TObject; progress: smallint);
  public
    constructor Create(AOwner: TComponent); override;
...
begin
  ...
  GridPreview.OnFileProgress := FormFileProgress;
...
begin
  ...
  with FForm do
    try
      ...
      FForm.OnFileProgress := DlgFileProgress;
...
procedure TAdvGridImportDialog.DlgFileProgress(Sender: TObject;
  progress: smallint);
begin
  if Assigned(OnFileProgress) then
    OnFileProgress(Self, progress);
end;
...
procedure TAsgImportForm.FormFileProgress(Sender: TObject; progress: SmallInt);
begin
  if Assigned(OnFileProgress) then
    OnFileProgress(Self, progress);
end;
