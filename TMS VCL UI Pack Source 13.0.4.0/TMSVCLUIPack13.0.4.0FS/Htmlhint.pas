...
uses
  ...
  PictureContainer, ImgList, Types, AppEvnts, AdvStyleIF
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF}
  ;
...
const
  ...
  // v1.5.1.1 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // v1.6.0.0 : New : Support for Emoji rendering in HTML
  // v1.6.0.1 : Fixed : Issue with VCL styles based rendering
  // v1.6.0.2 : Fixed : Shortcoming with certain VCL styles for background color
  // v1.6.0.3 : Fixed : Issue with linebreaks in HTML hint text
...
begin
  ...
  begin
    ...
    if StyleServices.GetElementColor(lDetails, ecFillColor, clr) and (clr <> clNone) then
    begin
  ...
  Caption := StringReplace(Caption,#13,'',[rfReplaceAll]);
...
begin
  Caption := StringReplace(AHint, #13, '',[rfReplaceAll]);
  ...
  begin
    ...
    HTMLDrawEx(Canvas,Caption,Rect,FHint.FImages,0,0,-1,-1,1,False,True,False,False,True,False,not FHint.Ellipsis,true,
