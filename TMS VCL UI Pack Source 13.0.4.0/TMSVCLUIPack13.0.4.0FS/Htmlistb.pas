...
const
  ...
  // 2.4.1.1 : Fixed : Design-time style painting
  // 2.4.2.0 : New : UIStyle property for Office 2019 look
  //         : Improved : On creation check for enabled AdvFormStyler
  // 2.4.2.1 : Fixed : ParentFont kept true on initialization
  // 2.4.2.2 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // 2.4.2.3 : Fixed : Issue with MaxHorizontalExtent calculation
  // 2.4.2.4 : Fixed : Issue with DPI change on runtime created forms
  // 2.5.0.0 : New : Emoji support
  // 2.5.0.1 : Improved : Added flag WS_EX_COMPOSITED in Windows params
  // 2.5.1.0 : Improved : Selection & Font drawing when seFont, seClient not set in StyleElements
  // 2.5.2.0 : New : Method ScrollnView added
...
type
  ...
  public
    ...
    procedure ScrollInView(AIndex: integer);
  published
...
uses
  CommCtrl,ShellApi,Forms, AdvHTML
  {$IFDEF DELPHIXE2_LVL}
  ,VCL.Themes
  {$ENDIF}
  ;
...
procedure THTMListBox.CreateParams(var Params: TCreateParams);
begin
  inherited;
...
procedure THTMListBox.ScrollInView(AIndex: integer);
begin
  if (AIndex >= 0) and (AIndex < Items.Count) then
  begin
    // Calculate the TopIndex to make the item visible
    TopIndex := AIndex;
  end;
end;
