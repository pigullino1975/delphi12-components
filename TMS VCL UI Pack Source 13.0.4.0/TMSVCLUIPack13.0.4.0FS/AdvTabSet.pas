...
const
  ...
  // 1.7.3.2 : Fixed : DPI calculation when parent form is not Scaled
  // 1.7.4.0 : Improved : High DPI handling for Delphi 11
  // 1.7.4.1 : Fixed : Issue with incorrect OnClick triggering during add/remove tabs
  // 1.7.4.2 : Fixed : Small potential issue with Win64
...
type
  ...
  protected
    ...
    property LowerSelected: integer read FLowerSelected write SetLowerSelected default 0;
    property TabHeight: Integer read FOwnerDrawHeight write SetTabHeight default 20;
    property TabIndex: Integer read FTabIndex write SetTabIndex default -1;
    property UnselectedColor: TColor read FUnselectedColor write SetUnselectedColor default clBtnFace;
...
begin
  inherited;
  if Assigned(TTabCollection(Collection).AdvTabSet) and (csDesigning in TTabCollection(Collection).AdvTabSet.ComponentState)
    and not (csLoading in TTabCollection(Collection).AdvTabSet.ComponentState) then
    FCaption:= 'TabSheet';
  ...
  TTabCollection(Collection).AdvTabSet.FProgChange := true;
  TTabCollection(Collection).AdvTabSet.TabIndex := 0;
  TTabCollection(Collection).AdvTabSet.FProgChange := false;
...
begin
  ...
  begin
    ...
    begin
      {$IFDEF DELPHIXE_LVL}
      TTabCollection(Collection).AdvTabSet.DuplicateTabs.Objects[i] := pointer(nativeuint(TTabCollection(Collection).AdvTabSet.DuplicateTabs.Objects[i])-1);
      {$ENDIF}
      {$IFNDEF DELPHIXE_LVL}
...
begin
  ...
  FActiveFont.Name := string(DefFontData.Name);
...
begin
  ...
  {$ENDIF}
  begin
    FActiveFont.Size := MulDiv(FActiveFont.Size, M, D);
    FScroller.Height := MulDiv(FScroller.Height, M, D);
    FScroller.Width := MulDiv(FScroller.Width, M, D);
  end;
end;
...
//    RealItemIndex := integer(FDuplicateTabs.Objects[Tab + FirstIndex]);
    ...
    begin
      ...
      begin
        ...
        TextR.Left := TextR.Left + FTabMargin.LeftMargin;
        if isSelected then
          TextR.Left:= TextR.Left + FLowerSelected;
      end
      else // pRight
      begin
    ...
    if not IsSelected then
      MemBitmap.Canvas.Font.Color := FAdvTabs.Items[RealItemIndex].TextColor;
