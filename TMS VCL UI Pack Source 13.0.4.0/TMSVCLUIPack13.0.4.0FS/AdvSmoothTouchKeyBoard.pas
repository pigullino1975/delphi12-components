...
const
  ...
  // v1.9.0.0 : New : Support for Office 2019 styles
  // v1.9.0.1 : Improved : high DPI support
  // v1.9.1.0 : New: Property UIStyle added for office look
  //          : Improved: On creation check for enabled AdvFormStyler
  // v1.9.2.0 : New : Per monitor high DPI support
  // v1.9.2.1 : Improved : TAdvSmoothPopupTouchKeyboard Font property published
  // v1.9.2.2 : Fixed : Issue with high DPI support in Dephi 11 on low DPI Form
  // v1.9.3.0 : New : Methods PostChar/PostString added to post custom keys to the input control
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvSmoothTouchKeyBoard.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvSmoothTouchKeyBoard.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  FChangingScale := True;
...
begin
  ...
  fs := TGPFont.Create(ffs, Round(FDPIScale * SmallFont.Size), fss, UnitPoint);

  for I := 0 to Keys.Count - 1 do
  begin
    r := MakeRect(Round(FDPIScale * Keys[I].X), Round(FDPIScale * Keys[I].Y), Round(FDPIScale * Keys[I].Width), Round(FDPIScale * Keys[I].Height));
...
begin
  ...
  fhorz := Width / Round(FDPIScale * FOriginalW);
  fvert := Height / Round(FDPIScale * FOriginalH);
...
begin
  inherited;
  FOriginalW := Round(Width / FDPIScale);
  FOriginalH := Round(Height / FDPIScale);
...
procedure TAdvSmoothTouchKeyBoard.PostChar(ch: char);
begin
  SendMessage(KeybdInputHandle, WM_CHAR, Ord(ch),0);
end;
...
procedure TAdvSmoothTouchKeyBoard.PostString(s: string);
var
  i: integer;
begin
...
begin
  ...
  for I := 0 to Keys.Count - 1 do
  begin
    if PtInRect(Bounds(Round(FDPIScale * Keys[I].X),Round(FDPIScale * Keys[I].Y), Round(FDPIScale * Keys[I].Width), Round(FDPIScale * Keys[I].Height)), Point(X, y))  then
    begin
      result := I;
      break;
    end;
...
var
  ...
  imgw, imgh: Integer;
begin
  VMargin := Round(FOwner.FDPIScale * 5);
  HMargin := Round(FOwner.FDPIScale *10);
  offset := 0;
  curDistance := Round(FOwner.FDPIScale * FOwner.KeyDistance);

  if not FKeyNormalPosition then
    offset := Round(FOwner.FDPIScale * FOwner.KeyDownOffset);
...
begin
  ...
  FOrigKbdW := FKbd.Width;
  FOrigKbdH := FKbd.Height;
...
begin
  ...
  begin
    FDPIScale := GetDPIScale(FOwnerform, FOwnerform.Canvas);
    FKbd.FDPIScale := FDPIScale;

    if ShowCaption then
    begin
      FFrm.Width := (Round(FDPIScale * FOrigKbdW) + Round(FDPIScale * GetSystemMetrics(SM_CXDLGFRAME) * 2)) + Round(FDPIScale * FKbd.KeyDistance);
      FFrm.Height := (Round(FDPIScale * FOrigKbdH) + Round(FDPIScale * GetSystemMetrics(SM_CYDLGFRAME)) * 2 + Round(FDPIScale * GetSystemMetrics(SM_CYSMCAPTION))) + Round(FDPIScale * FKbd.KeyDistance);
    end
    else
    begin
      FFrm.Width := Round(FDPIScale * FOrigKbdW);
      FFrm.Height := Round(FDPIScale * FOrigKbdH);
    end;
  end;
end;
...
procedure TAdvSmoothPopupTouchKeyBoard.SetFont(const Value: TFont);
begin
  FKbd.Font.Assign(Value);
end;
...
function TAdvSmoothPopupTouchKeyBoard.GetFont: TFont;
begin
  Result := FKbd.Font;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (pt.X < 0) and (Screen.MonitorCount = 1) then
        pt.X := 0;

      if (pt.Y < 0) and (Screen.MonitorCount = 1) then
        pt.Y := 0;
