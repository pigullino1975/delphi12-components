...
begin
  ...
  if not (Msg.CharCode in [VK_UP, VK_DOWN, VK_HOME, VK_END, VK_PRIOR, VK_NEXT])  then
    inherited;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          LStyle.GetElementColor(LStyle.GetElementDetails(teEditTextNormal), ecTextColor, TxtColor);
        ...
        if pos('</',s) > 0 then
        begin
          DrawHTMLEX(Canvas,s,R1,nil,true,Handle, 1);
        end
        else
...
begin
  ...
  // force calculation to take in account other loaded control settings
  ss := FColorSelectionStyle;
  SetColorSelectionStyle(succ(ss));
  SetColorSelectionStyle(ss);
end;
