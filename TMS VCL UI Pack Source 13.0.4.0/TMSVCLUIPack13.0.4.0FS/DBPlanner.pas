...
const
  ...
  BLD_VER = 0; // Build nr.
  DATE_VER = 'Apr, 2021'; // Month version
  ...
  // v3.4.6.0 : New : TDBMultiMonthSource.HeaderDateFormat property added
  // v3.4.6.1 : Fixed : Issue with initialization Active state in TItemSource
...
type
  ...
  published
    property AutoHeaderUpdate: Boolean read FAutoHeaderUpdate write FAutoHeaderUpdate default False;
    property HeaderDateFormat: string read FHeaderDateFormat write FHeaderDateFormat;
...
begin
  ...
  if Assigned(DataSet) then
    ...
    begin
      ...
      begin
        fld := DataSet.FieldByName(FDBItemSource.KeyField);
        if Assigned(fld) then
          FDBItemSource.ItemChanged(fld.AsString)
        else
          raise Exception.Create('Keyfield "'+FDBItemSource.KeyField+'" not found!');
      end;
    end;
end;
...
begin
  ...
  begin
    ...
    begin
      if HeaderDateFormat <> '' then
      begin
        dt := EncodeDate(y,j,1);
        FPlanner.Header.Captions.Strings[i] := FormatDateTime(HeaderDateFormat,dt);
      end
      else
      begin
        {$IFNDEF DELPHIXE_LVL}
