...
uses
  Windows, Classes, StdCtrls, ExtCtrls, Controls, Messages, SysUtils, CommCtrl,
  Forms, Graphics, Menus, Buttons, Mask, Clipbrd, ASXPVS, Types, AdvStyleIF,
  AdvToolTip, Math
  {$IFDEF DELPHIXE2_LVL}
  , System.UITypes
  {$ENDIF}
...
const
  ...
  // 1.9.0.2 : Fixed : Issue with OnClipboardPaste handler
  //         : Improved : Handling of clipboard when clipboard contains multiple lines
  // 1.9.0.3 : Improved : Handling of decimal separator with clipboard
  // 1.9.1.0 : Improved : Office black style handling
  // 1.9.1.1 : Improved : Label uses StyleElements
  // 1.9.2.0 : New : UIStyle property for office look
  //         : Improved : On creation check for enabled AdvFormStyler
  //         : Fixed : Office 2019 colors updated
  // 1.9.2.1 : Fixed : Issue with OnChange event handler when typing text in spinedit with selection
  // 1.9.2.2 : Fixed : Regression with OnChange for button click press
  // 1.9.2.3 : Fixed : Spin button strange issue with Windows XXX VCL styles
  // 1.9.2.4 : Fixed : Issue with label font high DPI handling when ParentFont = true
  // 1.9.3.0 : New : Button colors can now be set and changes with new Office styles 
  //         : Fixed : ParentFont kept true on initialization
  // 1.9.3.1 : Fixed : OnChange event handler for programmatic changes
  // 1.9.3.2 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // 1.9.3.3 : Fixed : Issue with runtime color initialization
  // 1.9.3.4 : Fixed : Issue with VCL style color retrieval for Windows XX styles
  // 1.9.3.5 : Improved : Behavior when keying '-' on selected signed value
  // 1.9.3.6 : Improved : Rendering with VCL Styles of Windows 10 * type
  // 1.9.3.7 : Fixed : Issue with toggling Enabled property at runtime
  // 2.0.0.0 : New : Balloon tip functionality added
  // 2.0.0.1 : Fixed : Issue with up/down button repainting with VCL styles
  // 2.0.0.2 : Fixed : Issue with changing Enabled at design-time
  // 2.0.0.3 : Fixed : DPI calculation when parent form is not Scaled
  // 2.0.0.4 : Fixed : VCL styles and disabled spin buttons
  // 2.0.0.5 : Fixed : Issue with ReadOnly and PgUp/PgDn
  // 2.0.0.6 : Fixed : Issue with OnChange and PgUp/PgDn
  // 2.0.0.7 : Fixed : Issue with IncrementSmart/DecrementSmart and OnChange event
  // 2.0.0.8 : Fixed : TDBAdvSpinEdit handling for IncrementSmart/DecrementSmart in specific circumstances
  // 2.0.1.0 : New : Padding added
  //         : New : Prefix added
  // 2.0.1.1 : Fixed : Issue with multi-monitor high DPI handling
  // 2.0.1.2 : Improved : Label font size initialization at design-time for high DPI
  // 2.0.2.0 : New : UpButton, DownButton as public properties on TAdvSpinEdit.Button exposed
  // 2.0.2.1 : Fixed : Issue with LabelFont handling when ParentFont = false
  // 2.0.2.2 : New : function IsInvalid: boolean added to verify current input against set min/max
  // 2.0.2.3 : Fixed : Use Precision when setting the FloatValue programmatically
  // 2.0.2.4 : Fixed : Issue with hex editing in TDBAdvSpinEdit
  // 2.0.2.5 : Fixed : Issue with Color when Enabled = false at design-time
  // 2.0.2.6 : Fixed : Issue with OnChange triggered twice for SpinType = sptFloat
...
{ TAdvSpinButton }
  ...
  public
    ...
    property UpButton: TAdvTimerSpeedButton read FUpButton;
    property DownButton: TAdvTimerSpeedButton read FDownButton;
  published
    property Align;
    property Anchors;
...
{ TAdvSpinEdit }
  ...
  {$ENDIF}
  TAdvSpinEdit = class(TCustomMaskEdit, ITMSStyle)
  private
    FFlat: Boolean;
    ...
    procedure SetValueInt(NewValue: LongInt);
...
procedure TAdvSpinEdit.Notification(AComponent: TComponent;
...
function TAdvSpinEdit.GetColorEx: TColor;
begin
...
begin
  ...
  case Key of
  VK_UP:if not (FIncrementSmart and (sl=0) and (fSpinType in [sptFloat,sptNormal])) then UpClick(Self) else
    begin
      if DoEdit then
        IncSmart;
      if Text <> oldVal then
  ...
  VK_DOWN:if not (FIncrementSmart and (sl=0) and (fSpinType in [sptFloat,sptNormal])) then DownClick(self) else
    begin
      if DoEdit then
        DecSmart;
      if Text <> oldVal then
  ...
  VK_NEXT:
    if not ReadOnly and DoEdit then
    begin
      if not (FIncrementSmart and (sl=0)) then
        case fSpinType of
        sptNormal: SetValueInt(Value - FIncrementPage);
  ...
  VK_DELETE: if not fEditorEnabled or ((fSpinType=sptHex) and (selstart<2)) then
    key := 0;
...
function TAdvSpinEdit.IsInValid: boolean;
begin
  Result := true;

  case fSpinType of
  sptNormal: Result := CheckValue(Value) <> Value;
  sptFloat: Result := CheckFloatValue(FloatValue) <> FloatValue;
  sptTime: Result := CheckDateValue(TimeValue) <> TimeValue;
  sptDate: Result := CheckDateValue(DateValue) <> DateValue;
  sptHex: Result := CheckValue(HexValue) <> HexValue;
  end;
end;
...
begin
  ...
  loc.Left := loc.Left + Padding;

  if (Prefix <> '') and (HandleAllocated) then
  begin
    ACanvas := TCanvas.Create;
    ACanvas.Handle := GetDC(Handle);
    ACanvas.Font.Assign(Font);

    loc.Left := loc.Left + ACanvas.TextWidth(Prefix) + 2;
...
procedure TAdvSpinEdit.SetComponentStyle(AStyle: TTMSStyle);
begin
  inherited;
...
procedure TAdvSpinEdit.SetButtonColors;
begin
  FButton.ButtonColor := ButtonColor;
  FButton.ButtonColorHot := ButtonColorHot;
  FButton.ButtonColorDown := ButtonColorDown;
  FButton.ButtonTextColor := ButtonTextColor;
  FButton.ButtonTextColorHot := ButtonTextColorHot;
  FButton.ButtonTextColorDown := ButtonTextColorDown;
end;
...
function TAdvSpinEdit.DoEdit: boolean;
begin
  Result := true;
end;
...
procedure TAdvSpinEdit.DoChange;
begin
...
begin
  ...
  // in case the value pasted doesn't have the expected system decimal & thousand separator
  if (Pos(ThousandSeparator, newstr) > 0) and (Pos(DecimalSeparator, newstr) = 0) then
  begin
    newstr := StringReplace(newstr, ThousandSeparator, DecimalSeparator, [rfReplaceAll]);
  end;

  if pos(#13, newstr) > 0 then
    newstr := Copy(newstr, 1, Pos(#13, newstr) - 1);
...
begin
  ...
  begin
    ...
    if FPrecision = 0 then
      Text := Format('%.0f', [checkFloatValue(NewValue)])
    else
...
procedure TAdvSpinEdit.SetValueInt(NewValue: LongInt);
var
  onch: TNotifyEvent;
begin
  onch := OnChange;
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvSpinEdit.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvSpinEdit.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
...
procedure TAdvSpinEdit.SetPadding(const Value: integer);
begin
...
procedure TAdvSpinEdit.SetPrefix(const Value: string);
begin
...
begin
  ...
  begin
    ...
    sptNormal: SetValueInt(GetValue);
    sptTime: TimeValue := GetTimeValue;
    sptDate: DateValue := GetDateValue;
    sptHex: HexValue := GetHexValue;
    end;
...
begin
  ...
  begin
    case fSpinType of
    sptNormal: SetValueInt(Value - Increment);
    sptFloat:  FloatValue := FloatValue - IncrementFloat;
    end;
    Exit;
  end;

  if (sp = Length(Text)) then
    sp := -1;
  ...
  begin
    incd := Exp10(ss-sp-1);
    case fSpinType of
    sptNormal: SetValueInt(Value - incd);
...
procedure TAdvSpinEdit.HideValidation;
begin
...
{$IFDEF DELPHI2007_LVL}
procedure TAdvSpinEdit.HideBalloon;
begin
...
procedure TAdvSpinEdit.ShowValidation(const AText: string);
begin
...
{$IFDEF DELPHI2007_LVL}
procedure TAdvSpinEdit.ShowBalloon(const ATitle, AText: string;
  AType: TBalloonType);
var
  ebt: TEditBalloonTip;
...
begin
 ...
 if (FCaretPos.x <> -1) and (FCaretPos.y <> -1) then
  ...
  begin
    ...
    th := Max(0, (Loc.Bottom - Loc.Top - th) div 2);

    if BorderStyle = bsSingle then
      th := th + 1;

    ACanvas.TextOut(2, th,Prefix);
    ReleaseDC(Handle, ACanvas.Handle);
    ACanvas.Free;
  end;
end;
...
procedure TAdvSpinEdit.CMVisibleChanged(var Msg: TMessage);
begin
  inherited;
  if Assigned(FLabel) then
...
begin
  ...
  if (SelLength = Length(Text)) and (Text <> '') and (Msg.CharCode <> VK_BACK) then
  begin
    FBlockChange := true;
  end;
...
{$IFDEF DELPHIXE2_LVL}
  ...
  begin
    ...
    stylebkg := pos('Windows', LStyle.Name) = 0;

    // Windows * styles have issues with color retrieval

    if stylebkg then
    begin
      LStyle.GetElementColor(LStyle.GetElementDetails(teBackgroundNormal), ecFillColor, clr);
      TAdvSpinButton(Owner).ButtonColor := clr;

      LStyle.GetElementColor(LStyle.GetElementDetails(tbPushButtonHot), ecFillColor, clr);
      TAdvSpinButton(Owner).ButtonColorHot := clr;

      LStyle.GetElementColor(LStyle.GetElementDetails(tbPushButtonPressed), ecFillColor, clr);
      TAdvSpinButton(Owner).ButtonColorDown := clr;
    end
    else
    begin
      clr := clWhite;
      ...
      clr := clWhite;
      LStyle.GetElementColor(LStyle.GetElementDetails(tgClassicCellSelected), ecFillColor, clr);
...
{$ENDIF}
  ...
  begin
    ...
    if TAdvSpinButton(Owner).ButtonTextColor <> clNone then
      arrowcolor := TAdvSpinButton(Owner).ButtonTextColor;

    if (TAdvSpinButton(Owner).ButtonTextColorHot <> clNone) and FHasMouse then
      arrowcolor := TAdvSpinButton(Owner).ButtonTextColorHot;

    if (TAdvSpinButton(Owner).ButtonTextColorDown <> clNone) and (FState = bsDown) then
      arrowcolor := TAdvSpinButton(Owner).ButtonTextColorDown;
...
begin
  ...
  FHasMouse := True;
  Invalidate;
  (Owner as TAdvSpinButton).Width := (Owner as TAdvSpinButton).Width + 1;
  (Owner as TAdvSpinButton).Width := (Owner as TAdvSpinButton).Width - 1;
end;
