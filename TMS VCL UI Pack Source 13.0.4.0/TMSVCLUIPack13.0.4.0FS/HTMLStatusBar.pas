...
const
  ...
  // 1.5.2.2 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // 1.6.0.0 : New : Support for rendering Emoji in HTML
  // 1.6.1.0 : Improved : Changed interface to use TImageCollection as well as ImageLists
  // 1.6.1.1 : Fixed : Issue with system font and high DPI
  // 1.6.1.1 : Improved : High dpi support for Delphi 11
...
procedure THTMLStatusPanel.ChangeDPIScale(M, D: Integer);
begin
  HTMLOffsetX := MulDiv(HTMLOffsetX, M, D);
  HTMLOffsetY := MulDiv(HTMLOffsetY, M, D);
...
procedure THTMLStatusPanel.SetAnimationImages(const Value: TCustomImageList);
begin
...
begin
  ...
  Canvas.Font.Height := Round(-11 * FDPIScale);
...
begin
  if FUseSystemFont and (GetDPIScale(Self, nil) = 1) then // goes wrong when high DPI is enabled
...
begin
  ...
  begin
    FPanels[I].ChangeDPIScale(M, D);
  end;

  UpdatePanels(True, True);
end;

procedure THTMLStatusBar.SetImages(const Value: TCustomImageList);
begin
  FImages := Value;
  Invalidate;
end;
