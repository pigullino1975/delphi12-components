...
type
  ...
  protected
    ...
    function GetFontCombo: TComboBox;
    function GetFontSizeCombo: TComboBox;
  public
...
uses
  Dialogs, AdvRichEditorIO, Forms, ExtDlgs, Windows, StrUtils;

const
  BTNSIZE = 24;

type
  TRichEditorEx = class(TAdvRichEditor);
...
begin
  ...
  begin
    ...
    TRichEditorEx(RichEditor).DoChanged;
  end;
end;
...
begin
  ...
  begin
    ...
    if Assigned(cb) then
    begin
      cb.ItemIndex := cb.Items.IndexOf(Trim(FRichEditor.Font.Name));
    end;

    cb := GetFontSizeCombo;
    if Assigned(cb) then
    begin
      s := inttostr(FRichEditor.Font.Size);
      cb.ItemIndex := cb.Items.IndexOf(s);
    end;
  end;
end;

function TAdvRichEditorFormatButtonBar.GetFontCombo: TComboBox;
var
  c: integer;
begin
  Result := nil;
...
function TAdvRichEditorFormatButtonBar.GetFontSizeCombo: TComboBox;
var
  c: integer;
begin
  Result := nil;
