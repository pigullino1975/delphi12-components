...
type
  ...
  private
    ...
    procedure SetGridFixedLineWidth(const Value: integer);
  ...
  protected
    ...
    function ControlDPIScale: single; virtual;
  ...
  published
    ...
    property GridFixedLineWidth: integer read FGridFixedLineWidth write SetGridFixedLineWidth default 1;
    ...
    property HoverRowCells: THoverRowCells read FHoverRowCells write FHoverRowCells default [hcNormal, hcSelected];
...
begin
  ...
  if ((ActiveRowShow) and (ARow = Row) and not IsFixed(ACol,ARow) and
  ...
  begin
    if FGridFixedLineWidth > 0 then
      ACanvas.Pen.Color := FGridFixedLineColor;
    ACanvas.Pen.Width := FGridFixedLineWidth;
    GLW := (FGridFixedLineWidth + 1) shr 1;
  end
  else
  begin
...
begin
  if FGridFixedLineColor <> Value then
  begin
...
procedure TBaseGrid.SetGridFixedLineWidth(const Value: integer);
begin
  if (FGridFixedLineWidth <> Value) then
  begin
    FGridFixedLineWidth := Value;
    Invalidate;
  end;
end;
...
begin
  ...
  if BidiMode = bdRightToLeft then
    ARect := Rect(x - ColWidths[c],y,x,y + RowHeights[r])
  else
  ...
  begin
    if RowCount > VisibleRowCount then
      OffsetRect(ARect, -GetSystemMetrics(SM_CXVSCROLL) - 4,0);
  end;
...
begin
  ...
  cp.FFontHeight := FDefFont.Height;  //Round(FDefFont.Height * ControlDPIScale);
...
begin
  ...
  begin
    ...
    cp.VAlignment := VA;
    cp.WordWrap := WW;
    cp.FFontName := AFont.Name;
    cp.FFontHeight := AFont.Height;
    cp.FFontStyle := AFont.Style;
    cp.FFontColor := AFont.Color;
...
function TBaseGrid.ControlDPIScale: single;
begin
  Result := 1;
end;
