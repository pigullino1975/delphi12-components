...
uses
  ...
  PictureContainer,  AdvGDIP, GDIPicture, AdvHintInfo, AdvStyleIF, Types, ImgList
  {$IFDEF DELPHIXE2_LVL}
...
const
  ...
  // 1.6.4.0 : Improved : Improvements for per monitor high DPI
  // 1.7.0.0 : New : VCL Styles support
  //         : New : StyleElements property exposed
  // 1.7.1.0 : New : Internal Office styler implemented
  // 1.7.2.0 : New : Support for using TVirtualImageList imagelists added
  // 1.7.2.1 : Fixed : UseSystemFont property wasn't stored in form file when true
  // 1.7.2.2 : Improved : On creation check for enabled AdvFormStyler
  // 1.7.3.0 : New : SeperatorColor added in TVistaBackground to show seperation in white styles
  //         : Fixed : Issue with panel width on high DPI
  // 1.7.3.1 : Fixed : Issue with retrieving correct panel index on high DPI
  // 1.7.3.2 : Improved : Use in combination with a TVirtualImageList on high DPI
  // 1.7.3.3 : Fixed : Issue with StretchPanel size on high DPI.
  // 1.7.3.4 : Fixed : DPI calculation when parent form is not Scaled
  // 1.7.3.5 : Fixed : Issue with Appearance.TextStyle for psText panel style
...
{ TAdvOfficeStatusBar }
  ...
  private
    ...
    FAnimationImages: TCustomImageList;
    ...
    procedure SetAnimationImages(const Value: TCustomImageList);
  ...
  published
    ...
    property AnimationImages: TCustomImageList read FAnimationImages write SetAnimationImages;
  ...
  {$ENDIF}
  TAdvOfficeStatusBar = class(TCustomControl, ITMSStyleEx, ITMSTones)
  private
  ...
  published
    ...
    property UseSystemFont: Boolean read FUseSystemFont write SetUseSystemFont nodefault;
    property Visible;
    property OnClick;
  ...
  published
    ...
    property SeperatorColor: TColor read FSeperatorColor write SetSeperatorColor default clWhite;
...
uses
  Math, CommCtrl, Forms, ShellAPI, StdActns, ComCtrls,
...
begin
  ...
  begin
    ...
    begin
      ...
      fillchar(sizerect,0,sizeof(sizerect));
      ...
      // handle case when incorrect result from MeasureString comes
      if (sizerect.X < maxsz) and (sizerect.Y < maxsz) and (sizerect.Width <maxsz) and (sizerect.Height < maxsz) and
        (sizerect.X >=0) and (sizerect.Y >= 0) and (sizerect.Width >= 0) and (sizerect.Height >= 0) then
      begin
        Result := Rect(Round(sizerect.X), Round(sizerect.Y), Round(sizerect.X + sizerect.Width), Round(sizerect.Y + sizerect.Height));
      end
      else
        Result := Rect(0,0,200,20);
...
procedure TAdvOfficeStatusPanel.SetAnimationImages(const Value: TCustomImageList);
begin
...
var
  ...
  Clr, ClrTo, ClrMirror, ClrMirrorTo, SpcClr: TColor;
...
begin
  ...
  begin
    ...
    SpcClr := PanelAppearance.SeperatorColor;
    ...
    begin
      ...
      if LnClrTo <> clNone then
        DrawGradient(Canvas, LnClrTo, SpcClr, 40, R2, False)
      else
        DrawGradient(Canvas, SpcClr, SpcClr, 40, R2, False);

      R2 := Rect(R.Left + 1, R.Top + y, R.Left + 2, R.Bottom );
      if LnClrTo <> clNone then
        DrawGradient(Canvas, SpcClr, LnClrTo, 40, R2, False)
      else
        DrawGradient(Canvas, SpcClr, SpcClr, 40, R2, False);
...
procedure TAdvOfficeStatusBar.SetUIStyle(const Value: TTMSStyle);
begin
  FTMSStyle := Value;
...
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvOfficeStatusBar.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
...
procedure TAdvOfficeStatusBar.SetDisabledImages(const Value: TCustomImageList);
begin
...
procedure TAdvOfficeStatusBar.SetImages(const Value: TCustomImageList);
begin
  FImages := Value;
  AutoSizePanels;
  Invalidate;
...
function TAdvOfficeStatusBar.UseVCLStyle: boolean;
begin
...
procedure TAdvOfficeStatusBar.SetComponentStyle(AStyle: TTMSStyle);
var
  tempStyler : TAdvOfficeStatusBarOfficeStyler;
begin
  if not Assigned(FOfficeStatusBarStyler) then
  begin
    FTMSStyle := AStyle;
    if not (AStyle = tsCustom) then
    begin
      tempStyler := TAdvOfficeStatusBarOfficeStyler.Create(Self);
      tempStyler.SetComponentStyle(AStyle);
      FInternalOfficeStatusBarStyler.Assign(tempStyler);
      tempStyler.Free;
    end;
  end;
end;
...
procedure TAdvOfficeStatusBar.SetComponentStyleAndAppColor(AStyle: TTMSStyle;
  AppColor: TColor);
var
  tempStyler : TAdvOfficeStatusBarOfficeStyler;
begin
...
begin
  ...
  begin
    ...
    FSeperatorColor := (Source as TVistaBackground).SeperatorColor;
  end
  else
    inherited Assign(Source);
end;
...
procedure TVistaBackground.SetSeperatorColor(const Value: TColor);
begin
