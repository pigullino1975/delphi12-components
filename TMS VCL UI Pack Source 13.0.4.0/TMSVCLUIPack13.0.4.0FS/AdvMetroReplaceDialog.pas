...
const
  ...
  // 1.0.1.0 : New : VCL styles compatibility
  // 1.0.1.1 : Improved : Caption supports high DPI
  // 1.0.1.2 : Fixed : VCL styles check updated to work in 10.4 Sydney
...
begin
  ...
  Constraints.MinHeight := Round(FDPIScale * DEFAULT_CAPTIONHEIGHT);
  Constraints.MinWidth := Round(FDPIScale * 100);
...
begin
  Result := Round(FDPIScale * DEFAULT_CAPTIONHEIGHT);
end;
...
begin
  ...
  try
    ...
    bmp.Canvas.Font.Height := Round(FDPIScale * bmp.Canvas.Font.Height);
...
{$IFDEF DELPHIXE2_LVL}
  ...
  begin
    ...
    if LStyle.GetElementColor(LDetails, ecTextColor, clr) and (clr <> clNone) then
      Appearance.CaptionFont.Color := clr
    else
      Appearance.CaptionFont.Color := ATones.Selected.TextColor;

    if (LStyle.GetStyleColor(TStyleColor.scButtonPressed) <> clNone) then
      Appearance.CaptionActiveColor := LStyle.GetStyleColor(TStyleColor.scButtonPressed)
    else
