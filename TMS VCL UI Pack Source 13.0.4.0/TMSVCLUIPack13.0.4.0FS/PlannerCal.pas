...
const
  ...
  // 2.4.0.0 : New : Support for Office 2019 styles
  // 2.4.0.1 : Fixed : Design-time style painting
  // 2.4.1.0 : New : UIStyle property for office look
  // 2.4.1.1 : New : Functions added for TAdvBadgeCalendar and DateToRect was changed for dates not shown
  // 2.4.1.2 : Fixed : paint issue with the height of header background
  // 2.4.1.3 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // 2.4.1.4 : Fixed : Issue with TDBPlannerCalendar and event OnEventInsert
  // 2.4.2.0 : Improved : Drawing with large DayFont size
  //         : Improved : Vertical division of weeks
  // 2.4.2.1 : Fixed : Issue with ShowGoToToday
  // 2.4.2.2 : Fixed : Issue with ShowWeeks = true
  // 2.4.2.3 : Fixed : Issue with months with days in 6 weeks
  // 2.4.3.0 : New : Support for drawing overlapped events added
  // 2.4.4.0 : New : Methods ChangeMonth(), ChangeYear() added in TPlannerCalendarGroup
  // 2.4.4.1 : Fixed : Issue with font coloring for days outside displayed month
...
type
  ...
  TPlannerCalendarStyle = (psWindowsXP, psOffice2000, psOffice2003Blue, psOffice2003Olive, psOffice2003Silver, psOffice2003Classic, psFlat, psAvantGarde, psWhidbey,
  psOffice2007Luna, psOffice2007Obsidian, psCustom, psOffice2007Silver, psWindowsVista, psWindows7, psTerminal,
  psOffice2010Blue, psOffice2010Silver, psOffice2010Black, psWindows8,
  ...
  public
    ...
    procedure ChangeMonth(const Offset: Integer);
    procedure ChangeYear(const Offset: Integer);
    procedure SetComponentStyle(AStyle: TTMSStyle);
    function GetComponentStyle: TTMSStyle;
    property Date: TDateTime read GetDate write SetDate;
    property Style: TPlannerCalendarStyle read FStyle write SetStyle;
...
procedure TPlannerCalendar.SetUIStyle(const Value: TTMSStyle);
begin
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                if int(Events[k].Date) = d then
                begin
                  if Assigned(Events[k]) then
                    PaintEventMarker(R, Events[k].Color, Events[k].ColorPM, Events[k].Shape)
                  else
                    PaintEventMarker(R, FEventMarkerColor, FEventMarkerColor, FEventMarkerShape);

                  if Assigned(Events[k]) then
                    FCanvas.Font.Color := Events[k].FontColor;
                end;
              end;
            ...
            begin
              ...
              if FUsedVCLStylesTextColor then
                ...
                FCanvas.Font.Color := InversColor;
            end;
...
begin
  ...
  FCanvas.TextOut(xoffset + Round(FDPIScale * 2), dy * 8 - 4, FormatDateTime(TodayFormat,Now));
end;
...
begin
  ...
  {$IFDEF DELPHIXE7_LVL}
  if (R.Top - yoffset < 0) or (R.Left - xoffset < 0) or (R.Bottom > Top + Height) or (R.Right > Left + Width) then
    Result := TRect.Create(-1, -1, -1, -1)
  else
  {$ENDIF}
    Result := r;
end;
...
procedure TPlannerCalendar.MonthChange;
begin
  //Used for AdvBadgeCalendar
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if FDPIScale > 1 then
        wfh := FCanvas.TextHeight('gh') - Round(FDPIScale * 4)
      else
...
begin
  ...
  psAvantGarde:
    ...
    begin
      ...
      GradientEndColor := clWhite;
      GradientDirection := gdVertical;
      MonthGradientStartColor := $00FAECE3;
      MonthGradientEndColor := $00FAECE3;
      MonthGradientDirection := gdVertical;
      InactiveColor := $00D4D4D4;
      DateSelectColor:= $00F2E1D5;
      DateSelectBorderColor:= $00F2E1D5;
      DateHoverColor := $00F2E1D5;
      DateDownColor := $00E3BDA3;
      SelectFontColor := $003B3B3B;
      Line3D := false;
    end;
    psOffice2019Gray:
    begin
      GradientStartColor := $00B8BBBE;
      GradientEndColor := $00B8BBBE;
      GradientDirection := gdVertical;
      MonthGradientStartColor := $00939597;
      MonthGradientEndColor := $00939597;
      MonthGradientDirection := gdVertical;
      InactiveColor := $00D4D4D4;
      DateSelectColor:= $00969696;
      DateSelectBorderColor:= $00969696;
      DateHoverColor := $00969696;
      DateDownColor := $00666666;
      SelectFontColor := $003B3B3B;
      Line3D := false;

      CaptionTextColor:= $00232425;
      DayFont.Color := $00232425;
      TextColor:= $00232425;
    end;
    psOffice2019Black:
    begin
      GradientStartColor := $003B3B3B;
      GradientEndColor := $003B3B3B;
      GradientDirection := gdVertical;
      MonthGradientStartColor := $00444648;
      MonthGradientEndColor := $00444648;
      MonthGradientDirection := gdVertical;
      InactiveColor := $00D4D4D4;
      DateSelectColor:= $00505050;
      DateSelectBorderColor:= $00505050;
      DateHoverColor := $00686868;
      DateDownColor := $00828282;
      SelectFontColor := clWhite;
      Line3D := false;
...
procedure TPlannerCalendar.YearChange;
begin
  //Used in AdvBadgeCalendar
end;
...
procedure TPlannerCalendarGroup.CreateWnd;
begin
  inherited;
  if FDesignTime then
  begin
...
procedure TPlannerCalendarGroup.SetUIStyle(const Value: TTMSStyle);
begin
  SetComponentStyle(Value);
end;
...
procedure TPlannerCalendarGroup.ChangeMonth(const Offset: Integer);
var
  Cal: TPlannerCalendar;
  OrigDate: TDateTime;
begin
  if (FCalendars <> nil) and (FCalendars.Count > 0) then
  begin
    Cal := TPlannerCalendar(FCalendars[0]);
    OrigDate := Cal.SelDate;
    Cal.ChangeMonth(Offset);
    Cal.DiffCheck(OrigDate, Cal.SelDate);
  end;
end;
...
procedure TPlannerCalendarGroup.ChangeYear(const Offset: Integer);
var
  Cal: TPlannerCalendar;
  OrigDate: TDateTime;
begin
