...
const
  ...
  // v1.5.1.1 : Fixed : Issue with drawing long text
  // v1.5.1.2 : Fixed : Issue with dynamic VCL style switching
  //          : Improved : StateImage position for multiline nodes
  // v1.5.1.3 : Fixed : Issue with VCL style color initialization
  // v1.5.2.0 : New : Selection colors for Office 2019 styles
  // v1.5.3.0 : New : UIStyle property for Office 2019 look
  //          : Improved : On creation check for enabled AdvFormStyler
  // v1.5.3.1 : Fixed : ParentFont kept true on initialization
  // v1.5.3.2 : Fixed : VCL styles check updated to work in 10.4 Sydney
  // v1.6.0.0 : New : Support for rendering Emoji in HTML
  //          : Fixed : Issue with text width calculation for hint when node images are used
  // v1.6.0.1 : Improved : Text rendering in combination with VCL styles use
  // v1.6.0.2 : Fixed : Issue with selection colors when turning VCL styles on/off
  // v1.6.0.3 : Improved : Behavior with ItemHeight in multimonitor with different DPI situation
...
{$IFDEF DELPHIXE2_LVL}
  VCL.Themes,
{$ENDIF}
  Commctrl, ShellApi, SysUtils, AdvHTML;
...
var
  ...
  canvas: TCanvas;
  a,s,fa: string;
  xsize,ysize,ml,hl,imgs,dt: Integer;
  tn: TTreenode;
  r,cr,hr,nr: TRect;
    ...
    CDDS_ITEMPOSTPAINT:
      begin
        ...
        begin
          ...
          begin
            dt := (r.Bottom - r.Top - StateImages.Height) div 2;

            if hasChk then
            begin
              StateImages.Draw(Canvas,r.Left + 2, r.Top  - 2 + dt, tn.StateIndex);
              r.Left := r.Left + StateImages.Width + 6;
            end
            else
            begin
              StateImages.Draw(Canvas,r.Left + 0, r.Top  - 2 + dt, tn.StateIndex);
              r.Left := r.Left + StateImages.Width + 6;
            end;
          end;
...
begin
  ...
  begin
    ...
    {$ENDIF}
    {$IFDEF DELPHIXE12_LVL}
    TreeView_SetItemHeight(Handle, Round(FDPIScale * FItemHeight));
    {$ENDIF}
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      idx := SendMessage(Handle, LB_ITEMFROMPOINT, 0, makelparam(hintinfo.cursorpos.x, hintinfo.cursorpos.y));
      ...
      begin
        ...
        begin
          ...
          if Node.ImageIndex >= 0 then
            widthText := widthText + Round(16 * FDPIScale);
...
{$IFDEF DELPHIXE2_LVL}
  ...
  begin
    ...
    {$ENDIF}
      ...
      SelectionColors.ColorMirrorFrom := clr;
      SelectionColors.ColorMirrorTo := clr;
  ...
  begin
    ...
    begin
      ...
      SelectionColors.BorderColor := clHighlight;
