...
uses
  Classes, AdvPDFLib, AdvRichEditorBase, AdvPDFIO, AdvTypes, AdvGraphicsTypes,
  Types, GDIPicture;
...
const
  ...
  // v1.0.0.2 : Fixed: Issue with font left indent
...
uses
  AdvUtils, PictureContainer, Forms, SysUtils, AdvGraphics, Graphics
  {$HINTS OFF}
...
procedure TAdvCustomRichEditorPDFIO.DoPDFExport(const APDFLib: TAdvPDFLib; const AExportObject: TAdvPDFIOExportObject; const AExportRect: TRectF);
begin
...
var
  ...
  namedpicel: TNamedPictureElement;
  r: TRectF;
  s: String;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        ft := '<font indent="'+IntToStr(el.LeftIndent)+'"';
      ...
      else if (el is TNamedPictureElement) then
      begin
        if Options.ExportImages then
        begin
          namedpicel := el as TNamedPictureElement;

          pic := TAdvRichEditorBaseEx(ARichEditor).GetNamedPicture(namedpicel);

          if Assigned(pic) then
          begin
            pici := IntToStr(bmpc.Items.Count);
            img := '<img src="' + pici + '" width="' + IntToStr(namedpicel.Width) + '" height="' + IntToStr(namedpicel.Height) + '"';
            case el.Alignment of
            taLeftJustify: img := img + ' align="left"';
            taRightJustify: img := img + ' align="right"';
            taCenter: img := img + ' align="center"';
      ...
      begin
        ...
        ft := '<font bullet="" indent="'+IntToStr(el.BulletIndent)+'"';
        case el.Alignment of
          taLeftJustify: ft := ft + ' align="left"';
          taRightJustify: ft := ft + ' align="right"';
          taCenter: ft := ft + ' align="center"';
