...
const
  ...
  // v1.5.0.1 : Fixed : Indicator placement on resize and size on high DPI
...
{$IFNDEF DELPHIXE10_LVL}
procedure TAdvSmoothMenu.ChangeScale(M, D: Integer);
{$ENDIF}
{$IFDEF DELPHIXE10_LVL}
procedure TAdvSmoothMenu.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ENDIF}
begin
  inherited;
...
begin
  ...
  begin
    ...
    begin
      FCurrentPos := FDPIScale * (FCaptionRect.X + (FCaptionRect.Width / 2));
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    y := Round(FDPIScale * (ItemAppearance.Splitter.Height + 2));

    size := Round(FDPIScale * Appearance.IndicatorSize);
...
begin
  ...
  begin
    g := TGPGraphics.Create(Canvas.Handle);
    p := TGPPen.Create(MakeColor(Appearance.LineOpacity, Appearance.LineColor), FDPIScale * 0.1);

    bw := 0;
    if (Appearance.Fill.BorderColor <> clNone) then
      bw := Appearance.Fill.BorderWidth;

    g.DrawLine(p, bw, Round(FDPIScale * (ItemAppearance.Splitter.Height + 2)) , Width - (2 * bw)  - GetShadowOffset, Round(FDPIScale * (ItemAppearance.Splitter.Height + 2)));
    p.Free;
    g.Free;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      f := TGPFont.Create(ff, cf.Size * FDPIScale , fs, UnitPoint);
      ...
      if i = FHoveredItem then
      begin
        FCurrentIndPos := FItemRect.X + FItemRect.Width / 2;
      end;
...
procedure TAdvSmoothMenu.Loaded;
begin
  inherited;
  FHoveredItem := FSelectedItem;

  FCurrentPos := Round(FDPIScale * FCurrentPos);
  FPosTo := FCurrentPos;
end;
...
begin
  inherited;
  InitItemRects;
  FCurrentPos := FCurrentIndPos;
  FPosTo := FCurrentIndPos;
  DrawIndicator;
  Changed;
end;
...
begin
  ...
  ItemAppearance.Splitter.Visible := True;
  ...
  case AStyle of
    ...
    tsOffice2019White:
      begin
        Appearance.Fill.BorderColor := $00C4C6C8;
        Appearance.Fill.Color := clWhite;
        Appearance.Fill.ColorTo := clWhite;
        Appearance.Fill.ColorMirror := clWhite;
        Appearance.Fill.ColorMirrorTo := clWhite;
        ...
        Appearance.IndicatorColor := $00E39B20;
        Appearance.LineColor := $00E39B20;

        ItemAppearance.GlowColor := clWhite;
        ItemAppearance.GlowColorHover := clWhite;
        ItemAppearance.GlowColorSelected := clWhite;
        ItemAppearance.Splitter.Visible := False;

        ItemAppearance.Font.Color := $00444648;
        ItemAppearance.HoverFont.Color := $00444648;
        ItemAppearance.SelectedFont.Color := $00444648;
        ItemAppearance.NotesFont.Color := $003B3B3B;
      end;
    tsOffice2019Gray:
      begin
        Appearance.Fill.BorderColor := $00666666;
        Appearance.Fill.Color := $00B8BBBE;
        Appearance.Fill.ColorTo := $00B8BBBE;
        Appearance.Fill.ColorMirror := $00B8BBBE;
        Appearance.Fill.ColorMirrorTo := $00B8BBBE;
        Appearance.Fill.GradientMirrorType := gtVertical;

        Appearance.IndicatorColor := $001E1F20;
        Appearance.LineColor := $001E1F20;

        ItemAppearance.GlowColor := $00B8BBBE;
        ItemAppearance.GlowColorHover := $00B8BBBE;
        ItemAppearance.GlowColorSelected := $00B8BBBE;
        ItemAppearance.Splitter.Visible := False;

        ItemAppearance.Font.Color := $00232425;
        ItemAppearance.HoverFont.Color := $001E1F20;
        ItemAppearance.SelectedFont.Color := $00232425;
        ItemAppearance.NotesFont.Color := $003B3B3B;
      end;

    tsOffice2019Black:
      begin
        Appearance.Fill.BorderColor := $00ADB0B3;
        Appearance.Fill.Color := $00303132;
        Appearance.Fill.ColorTo := $00303132;
        Appearance.Fill.ColorMirror := $00303132;
        Appearance.Fill.ColorMirrorTo := $00303132;
        Appearance.Fill.GradientMirrorType := gtVertical;

        Appearance.IndicatorColor := clWhite;
        Appearance.LineColor := clWhite;

        ItemAppearance.GlowColor := $00303132;
        ItemAppearance.GlowColorHover := $00303132;
        ItemAppearance.GlowColorSelected := $00303132;
        ItemAppearance.Splitter.Visible := False;

        ItemAppearance.Font.Color := clWhite;
        ItemAppearance.HoverFont.Color := clWhite;
        ItemAppearance.SelectedFont.Color := $FFFFFF;
        ItemAppearance.NotesFont.Color := $F0F0F0;
      end;
  end;
end;
...
procedure TAdvSmoothMenu.SetUIStyle(const Value: TTMSStyle);
begin
...
begin
  ...
  begin
    InitItemRects;
    FCurrentPos := (Items[0].FCaptionRect.X + Items[0].FCaptionRect.Width / 2) * FDPIScale;
    FPosTo := FCurrentPos;
    Changed;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      g.DrawString(FCaption, Length(FCaption), f, MakeRect(FCaptionRect.X, Round(FCaptionRect.Y * FDPIScale), FCaptionRect.Width, Round(FCaptionRect.Height * FDPIScale)), sf, b);
    ...
    ir := MakeRect(ClientRect.Left + 2, FDPISCale * ItemAppearance.Splitter.Height + 4, ClientRect.Right - ClientRect.Left,
      ClientRect.Bottom - FDPIScale * ItemAppearance.Splitter.Height - 4);
    ...
    begin
      ...
      picr := Bounds(Round(ClientRect.Left + FCurrentTextPos), Round(FDPIScale * ItemAppearance.Splitter.Height + 4), Round(ir.Width), Round(ir.Height));

      if FPicturePosition <> ppCustom then
        DrawPicture(g, FCurrentTextOpc, Picture, FPicturePosition, pw, ph, picr, picr.Left, picr.Top)
      else
       DrawPicture(g, FCurrentTextOpc, Picture, FPicturePosition, pw, ph, picr, Round(FcurrentTextPos) + Round(FDPIScale * PictureLeft), Round(FDPIScale * PictureTop));
      ...
      begin
        ...
        begin
          ...
          sf := TGPStringFormat.Create;
          f := TGPFont.Create(ff, FNotesFont.Size * FDPIScale, fs, UnitPoint);
        ...
        begin
          x := Round(FDPIScale * NotesLeft);
          y := Round(FDPIScale * NotesTop);
        end;
    ...
    begin
      ...
      begin
        ...
        rsplit := MakeRect(FItemRect.X + FItemRect.Width, FItemRect.Y + (FItemRect.Height / 2) - (FDPIScale * Height / 2), 1, FDPIScale * Height / 2);
        ...
        rsplit := MakeRect(FItemRect.X + FItemRect.Width, FItemRect.Y + (FItemRect.Height / 2), 1, Height * FDPIScale / 2);
