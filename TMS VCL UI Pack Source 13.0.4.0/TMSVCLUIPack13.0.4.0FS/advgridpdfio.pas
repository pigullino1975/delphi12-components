...
const
  ...
  // v1.0.0.10: Fixed: Retrieve display cells instead of cells for PDF export
  // v1.0.1.0 : New : Public property PrintRow: integer added to know row being printed from event handlers
  // v1.0.1.1 : Fixed : Issue with page end and multirow cells
  // v1.0.1.2 : Fixed : Issue calculating fit to page width
  // v1.0.1.3 : Improved : edge detection and drawing
  // v1.0.1.4 : Fixed : Issue with export to PDF and hidden columns
  // v1.0.1.5 : Fixed : Issue with export when last column is a hidden column
  // v1.0.1.6 : Fixed : Issue with export with multiple hidden columns
...
type
  ...
  protected
    ...
    procedure DoPDFGridExport(const APDFLib: TAdvPDFLib; const AGrid: TAdvStringGrid; const ASelection: TAdvGridPDFIOCellRange; const ASkipSaveBounds: Boolean); virtual;
    ...
    property PrintRow: integer read FPrintRow;
  public
...
uses
  ...
  Grids, AdvPDFCoreLibBase, AdvGraphicsPDFEngine
  {$HINTS OFF}
...
begin
  ...
  try
    ...
    bc := clBlack;
    ww := AGrid.WordWrap;
    RCol := AGrid.DisplColIndex(ACol);
    AGrid.GetVisualProperties(RCol, ARow, st, False, False, True, b, cto, cm, cmto, ft, ha, va, ww, gd);
    ...
    begin
      ...
      if AGrid.IsRotated(ACol,ARow, AAngle) then
      begin
        g := TAdvGraphicsPDFEngine.Create(APDFLib);
        try
          tr := RectF(tr.Left, tr.Top, tr.Left + tr.Height, tr.Top + tr.Width);
          g.DrawText(tr, s, False, gtaTrailing, gtaCenter, gttNone, -AAngle);
        finally
          g.Free;
        end;
      end
    ...
    case AGrid.CellTypes[ACol, ARow] of
      ...
      begin
        ...
        begin
          ...
          if (AGrid.CellNode.NodeType = cnLeaf) and Options.ExportImages then
          begin
          ...
          if (AGrid.CellNode.NodeType = cnXP) and Options.ExportImages then
          begin
          ...
          if (AGrid.CellNode.NodeType = cnGlyph) and
             ...
             (not AGrid.CellNode.ContractGlyph.Empty) and Options.ExportImages then
          begin
            cr := r;
...
procedure TAdvCustomGridPDFIO.DoPDFExport(const APDFLib: TAdvPDFLib; const AExportObject: TAdvPDFIOExportObject; const AExportRect: TRectF);
var
...
begin
  ...
  for I := 0 to Length(pgb) - 1 do
    DoPDFGridExport(APDFLib, g, pgb[I], Length(pgb) > 1);
end;

procedure TAdvCustomGridPDFIO.DOPDFGridExport(const APDFLib: TAdvPDFLib; const AGrid: TAdvStringGrid; const ASelection: TAdvGridPDFIOCellRange; const ASkipSaveBounds: Boolean);
var
  c, r: Integer;
  cellstr: String;
  ...
  begin
    ...
    begin
      ...
      if w > 0 then
        Result := (APDFLib.MediaBox.Width - Options.Margins.Left - Options.Margins.Right) / (w + 2);
    end;
  end;
...
begin
  ...
  while c <= Min(CurrentGrid.AllColCount - 1, ASelection.EndCol) do
  begin
    FPrintRow := r;
    ...
    begin
      ...
      begin
        ...
        if chk then
        begin
          cellstr := CurrentGrid.DisplCells[bc, br];
        ...
        begin
          FSaveYBounds := h - Options.Margins.Bottom;
          Break;
        end;
    ...
    begin
      FSaveXBounds := w - Options.Margins.Right;
      ...
      if (c < Min(CurrentGrid.AllColCount - 1, ASelection.EndCol + 1)) then
      begin
        NewPage(APDFLib, CurrentGrid);
        x := Options.Margins.Left;
        y := Options.Margins.Top;
