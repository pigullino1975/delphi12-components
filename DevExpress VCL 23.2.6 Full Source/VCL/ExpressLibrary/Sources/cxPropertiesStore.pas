...
const
  dxThisUnitName = 'cxPropertiesStore';
...
begin
  ...
  begin
    ...
    begin
      TForm(Owner).OnCreate := FOnCreateHandler;
      TForm(Owner).OnDestroy := FOnDestroyHandler;
    end
    else if Owner is TDataModule then
    begin
      TDataModule(Owner).OnCreate := FOnCreateHandler;
      TDataModule(Owner).OnDestroy := FOnDestroyHandler;
    end;
  end;
  FComponents.Free;
  inherited Destroy;
