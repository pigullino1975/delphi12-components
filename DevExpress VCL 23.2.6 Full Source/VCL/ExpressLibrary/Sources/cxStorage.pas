...
type
  ...
  strict private
    ...
    procedure GetAllPublishedProperties(AProperties: TStrings); overload;
    procedure GetAllPublishedProperties(AProperties: TStrings; ATypeKinds: TTypeKinds); overload;
    procedure GetChildren(AChildren: TStringList);
...
uses
  Controls, cxControls, dxTypeInfo, dxDPIAwareUtils;

const
  dxThisUnitName = 'cxStorage';
...
begin
  GetAllPublishedProperties(AProperties, [tkClass]);
end;

procedure TcxStorage.GetAllPublishedProperties(AProperties: TStrings);
begin
  GetAllPublishedProperties(AProperties, tkAny - [tkMethod]);
end;

procedure TcxStorage.GetAllPublishedProperties(AProperties: TStrings; ATypeKinds: TTypeKinds);
var
  ALocalProperties: TStrings;
begin
  ALocalProperties := AProperties;
  TdxTypeInfo.EnumProperties(FStoredObject.ClassInfo, ATypeKinds,
    procedure(APropInfo: PPropInfo; var ABreak: Boolean)
    begin
      ALocalProperties.Add(TdxTypeInfo.GetPropertyName(APropInfo));
    end);
end;
