...
uses
  ...
  DesignIntf, DesignEditors, DesignConst, DesignMenus, PicEdit, Dialogs,
...
type
  ...
  TdxSkinsRequiresUnitsProc = procedure (AProc: TGetStrProc);
  ...
  strict private
    ...
    FOpenPictureEditor: Boolean;

    FTempDesigner: IDesigner;
    FTempOriginalImage: TObject;

    procedure CheckDesigningComponent(const AObjectName: string);
    procedure CheckDesigningComponents(ADesigner: IDesigner; AOriginalImage: TObject);
    procedure SetPicture(Value: TPicture);
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    function Execute(ADesigner: IDesigner; AOriginalImage: TObject): Boolean;

    property GraphicClass: TGraphicClass read FGraphicClass write FGraphicClass;
    property Picture: TPicture read FPicture write SetPicture;
    property OpenPictureEditor: Boolean read FOpenPictureEditor write FOpenPictureEditor;
  end;
  ...
  protected
    ...
    function GraphicToString(AGraphic: TGraphic): string;
  ...
  protected
    function CreateOpenDialog(AOwner: TComponent): TOpenDialog; virtual;
  ...
  { TdxPanelSelectionEditor }

  TdxPanelSelectionEditor = class(TSelectionEditor)
  public
    procedure RequiresUnits(Proc: TGetStrProc); override;
  end;
...
var
  ...
  FdxSkinsRequiresUnits: TdxSkinsRequiresUnitsProc;
...
uses
  VCLEditors, ColnEdit, Types, Math, ImgList, ToolsApi,
  dxTypeInfo, cxGraphics, cxPropEditors, cxLocalization, dxCore, cxGeometry,
  cxComponentCollectionEditor, dxScreenTipRepositoryEditor, dxGalleryDesigner, ExtDlgs,
  Variants, dxDPIAwareUtils, dxSmartImage, dxPictureEditor, cxCustomCanvas, dxPanel;

const
  dxThisUnitName = 'cxLibraryReg';
...
type
  ...
  strict private
    ...
    function AllowChangeRenderMode: Boolean;
    function AreSkinsAvailable: Boolean;
    function AreSkinsSupported: Boolean;
    procedure GetPropProc(const Prop: IProperty);
  public
    procedure GetProperties(Proc: TGetPropProc); override;
  end;
  ...
  TcxScrollBarSelectionEditor = class(TdxSkinsBaseSelectionEditor)
  public
    procedure RequiresUnits(Proc: TGetStrProc); override;
  end;
...
procedure dxSkinsRequiresUnits(AProc: TGetStrProc);
begin
  if Assigned(FdxSkinsRequiresUnits) then
    FdxSkinsRequiresUnits(AProc);
end;

procedure AddMethodParam(var P: PAnsiChar; AParamFlags: TParamFlags; const AParamName, AParamTypeName: ShortString);
var
...
procedure TcxContainerStyleAssignedValuesProperty.PopulateVisiblePropertiesList(AList: TStringList);

  procedure PopulateStyleValueList(AStyle: TcxContainerStyle; AStyleValueList: TStringList);
  var
    AStyleValue: TcxContainerStyleValue;
    AStyleValueIndex: Integer;
  begin
    TdxTypeInfo.EnumProperties(AStyle.ClassInfo,
      procedure(APropInfo: PPropInfo; var ABreak: Boolean)
      var
        APropertyName: string;
      begin
        APropertyName := TdxTypeInfo.GetPropertyName(APropInfo);
        if not AStyle.GetStyleValue(APropertyName, AStyleValue) then
          Exit;
        if (AStyle.Container = nil) and ((AStyle.State = csNormal) or AStyle.IsExtendedStylePropertyPublished(APropertyName)) or
          (AStyle.Container <> nil) and AStyle.Container.IsStylePropertyPublished(APropertyName, AStyle.State <> csNormal) then
        begin
          AStyleValueIndex := AStyleValueList.IndexOf(APropertyName);
          if AStyleValueIndex = -1 then
            AStyleValueList.AddObject(APropertyName, TObject(1))
          else
...
begin
  if SameText(Prop.GetName, 'SkinName') and not (AreSkinsAvailable and AreSkinsSupported) then
    Exit;
  if SameText(Prop.GetName, 'RenderMode') and not AllowChangeRenderMode then
    Exit;
  FProc(Prop);
end;

function TcxLookAndFeelProperty.AllowChangeRenderMode: Boolean;
begin
  Result := cxIsCustomCanvasSupported(GetComponent(0));
end;

function TcxLookAndFeelProperty.AreSkinsAvailable: Boolean;
begin
  Result := cxLookAndFeelPaintersManager.GetPainter(lfsSkin) <> nil;
end;

function TcxLookAndFeelProperty.AreSkinsSupported: Boolean;
begin
  Result := ClassSupportsSkinProc(GetComponent(0));
end;
...
function TdxPictureEditor.Execute(ADesigner: IDesigner; AOriginalImage: TObject): Boolean;
var
  AAdvancedPictureDialog: IdxAdvancedPictureEditor;
begin
  CheckDesigningComponents(ADesigner, AOriginalImage);
  if Supports(dxIconLibraryIntf, IdxAdvancedPictureEditor, AAdvancedPictureDialog) then
    Result := AAdvancedPictureDialog.Execute(Picture, GraphicClass, FImportList, FPreferSmartImage, OpenPictureEditor)
  else
    Result := dxExecutePictureEditor(Picture, GraphicClass, FImportList, FPreferSmartImage);
end;

procedure TdxPictureEditor.CheckDesigningComponent(const AObjectName: string);

  procedure CheckAddToImportList(AComponent: TComponent);
  begin
    TdxTypeInfo.EnumProperties(AComponent.ClassInfo,
      procedure(APropInfo: PPropInfo; var ABreak: Boolean)
      var
        ACanAdd: Boolean;
        AObject: TObject;
      begin
        ...
        AObject := GetObjectProp(AComponent, APropInfo);
        if (AObject = nil) or (AObject = FTempOriginalImage) then
          Exit;

        if AObject.ClassType = TBitmap then
          ACanAdd := not TBitmap(AObject).Empty
        else if AObject.ClassType = TdxSmartGlyph then
          ACanAdd := not TdxSmartGlyph(AObject).Empty
        else if AObject.ClassType = TPicture then
          ACanAdd := TPicture(AObject).Graphic <> nil
        else
          ACanAdd := False;

        if ACanAdd then
          FImportList.AddObject(AObjectName + '.' + GetPropName(APropInfo), AObject);
      end);
  end;

  function IsDxComponent(AComponent: TComponent): Boolean;
  begin
    Result := (AComponent is TcxCustomComponent) or (AComponent is TcxCustomControl);
  end;

var
  AComponent: TComponent;
begin
  AComponent := FTempDesigner.GetComponent(AObjectName);
  if AComponent <> nil then
    CheckAddToImportList(AComponent);
  if not FPreferSmartImage then
    FPreferSmartImage := IsDxComponent(AComponent);
end;

procedure TdxPictureEditor.CheckDesigningComponents(ADesigner: IDesigner; AOriginalImage: TObject);
begin
  FPreferSmartImage := False;
  FTempOriginalImage := AOriginalImage;
  FTempDesigner := ADesigner;
  FTempDesigner.GetComponentNames(GetTypeData(TComponent.ClassInfo), CheckDesigningComponent);
  FTempDesigner := nil;
  FTempOriginalImage := nil;
  FImportList.Sort;
end;
...
begin
  ...
  try
    ...
    if APictureEditor.Execute(Designer, GetImage) then
    begin
...
begin
  Result := GraphicToString(GetGraphic);
end;
...
function TcxGraphicProperty.GraphicToString(AGraphic: TGraphic): string;
begin
  if IsGlyphAssigned(AGraphic) then
    Result := '(' + string(AGraphic.ClassName) + ')'
  else
    Result := 'None';
end;
...
function TcxPictureProperty.GetGraphicClass: TGraphicClass;
begin
  Result := TGraphic;
end;
...
var
  ADialog: TOpenDialog;
begin
  ADialog := CreateOpenDialog(Application);
...
function TcxFilenameProperty.CreateOpenDialog(AOwner: TComponent): TOpenDialog;
begin
...
begin
  dxSkinsRequiresUnits(Proc);
end;

{ TdxPanelSelectionEditor }

procedure TdxPanelSelectionEditor.RequiresUnits(Proc: TGetStrProc);
begin
  inherited;
  Proc('cxGeometry');
  Proc('dxFramedControl');
end;
...
{$IF DEFINED(DXREGISTERPNGIMAGE) AND DEFINED(DXREGISTERPNGIMAGEINDESIGNTIME)}
  TdxSmartImageCodecsRepository.RegisterInVCL := True;
{$ELSE}
  TdxSmartImageCodecsRepository.RegisterInVCL := False;
{$IFEND}
  ...
  RegisterSelectionEditor(TdxPanel, TdxPanelSelectionEditor);
  ...
  RegisterComponents(dxLibraryProductPage, [TdxPanel]);
  ...
  RegisterCustomModule(TdxForm, TCustomModule);
end;
