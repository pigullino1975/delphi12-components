...
uses
  Types, StrUtils, Graphics, Math, dxTypeInfo, cxGraphics, dxCustomTree, dxShapeReaders, dxShapeTransformations,
  dxShapeBrushes;

const
  dxThisUnitName = 'dxShapePrimitives';
...
begin
  ...
  begin
    if AColorName[1] = '#' then
      Result := TdxAlphaColors.FromHexCode(Copy(AColorName, 2, Length(AColorName) - 1))
    else
...
function dxStrToPointF(const S: string): TdxPointF;
  ...
  function GetCoordinate(const AString: string; AFirstCoordinate: Boolean): Single;
  var
    ASeparatorPosition: Integer;
  begin
    ASeparatorPosition := PosEx(CoordinateSeparator, AString);
    if AFirstCoordinate then
...
var
  ...
  I: Integer;
begin
  if TdxTypeInfo.TryGetPropList(ABaseShape.ClassInfo, AShapePropList, AShapePropCount) then
    try
...
procedure TdxShapeObjectReader.ReadAttribute(ABaseShape: TdxShapeObject; APropList: PPropList; APropCount: Integer; AAttribute: TdxXMLNodeAttribute);
  ...
  begin
    ...
    for I := 0 to APropCount - 1 do
      if SameText(TdxTypeInfo.GetPropertyName(APropList[I]), APropName) then
      begin
        Result := I;
        Break;
      end;
