...
uses
  ...
  cxLookAndFeelPainters, cxCustomCanvas;
...
type
  ...
  public
    ...
    function DrawImage(DC: HDC; const R: TRect): Boolean; overload;
    function DrawImage(ACanvas: TcxCustomCanvas; const R: TRect): Boolean; overload;
    procedure Finalize;
    //
  ...
  private
    function GetItems(Index: TdxListIndex): TdxFadingElement;
  public
    procedure Clear; override;
    property Items[Index: TdxListIndex]: TdxFadingElement read GetItems; default;
  end;
  ...
  public
    ...
    class procedure CorrectAlphaChannel(AColors: PRGBQuad; AColorCount: Integer); overload;
  end;
  ...
  public
    ...
    function DrawFadeImage(AObject: TObject; DC: HDC; const R: TRect): Boolean; overload;
    function DrawFadeImage(AObject: TObject; ACanvas: TcxCustomCanvas; const R: TRect): Boolean; overload;
...
begin
  ...
  try
    AAlpha2 := MulDiv(Stage, FixedPointOne, AnimationFrameCount);
    AAlpha1 := FixedPointOne - AAlpha2;
    ...
    begin
      ATempColor^.rgbBlue := (AColor1^.rgbBlue * AAlpha1 + AColor2^.rgbBlue * AAlpha2) shr FixedPointBits;
      ATempColor^.rgbGreen := (AColor1^.rgbGreen * AAlpha1 + AColor2^.rgbGreen * AAlpha2) shr FixedPointBits;
      ATempColor^.rgbRed :=  (AColor1^.rgbRed * AAlpha1 + AColor2^.rgbRed * AAlpha2) shr FixedPointBits;
      ATempColor^.rgbReserved := (AColor1^.rgbReserved * AAlpha1 + AColor2^.rgbReserved * AAlpha2) shr FixedPointBits;
      Inc(ATempColor);
      Inc(AColor1);
      Inc(AColor2);
    end;
...
function TdxFadingElement.DrawImage(ACanvas: TcxCustomCanvas;
  const R: TRect): Boolean;
begin
  Result := Assigned(WorkImage);
  if Result then
    ACanvas.DrawImage(WorkImage, R);
end;
...
function TdxFadingList.GetItems(Index: TdxListIndex): TdxFadingElement;
begin
...
begin
  if Length(AColors) > 0 then
    CorrectAlphaChannel(@AColors[0], Length(AColors));
end;

class procedure TdxFadingHelper.CorrectAlphaChannel(AColors: PRGBQuad; AColorCount: Integer);
begin
  while AColorCount > 0 do
  begin
    if (AColors^.rgbReserved = 0) and (AColors^.rgbBlue + AColors^.rgbGreen + AColors^.rgbRed > 0) then
      AColors^.rgbReserved := MaxByte;
    Dec(AColorCount);
    Inc(AColors);
  end;
end;
...
begin
  ...
  FTimer := cxCreateTimer(DoTimer, dxFadeTimerInterval, False);
...
function TdxFader.DrawFadeImage(AObject: TObject; ACanvas: TcxCustomCanvas; const R: TRect): Boolean;
var
  AFadingElement: TdxFadingElement;
begin
  Result := Find(AObject, AFadingElement) and AFadingElement.DrawImage(ACanvas, R);
end;
