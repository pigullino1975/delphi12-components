...
type
  ...
  public
    ...
    function ToDateTime(const ADateTime: TcxDateTime): TDateTime; overload; override;
  end;
  ...
  public
    ...
    function TryGetMonthNumber(AYear: Integer; const S: string; const AFormat: TFormatSettings; out AValue: Cardinal): Boolean; override;
  end;
  ...
  public
    ...
    function TryGetDayNumber(const S: string; out AValue: Cardinal): Boolean; override;
    function TryGetMonthNumber(AYear: Integer; const S: string; const AFormat: TFormatSettings; out AValue: Cardinal): Boolean; override;
    function TryGetYearNumber(const S: string; out AValue: Cardinal): Boolean; override;
  end;
...
uses
  Classes, DateUtils, dxCore, dxCoreClasses;

const
  dxThisUnitName = 'dxCalendarUtils';
...
function TcxGregorianCalendarTable.ToDateTime(const ADateTime: TcxDateTime): TDateTime;
begin
  with ADateTime do
...
function TcxJapaneseCalendarTable.ToDateTime(const ADateTime: TcxDateTime): TDateTime;
var
  AYear: Cardinal;
begin
  with ADateTime do
...
function TcxHijriCalendarTable.ToDateTime(const ADateTime: TcxDateTime): TDateTime;
var
  I: Integer;
begin
  Result := 227013;
...
function TcxHijriCalendarTable.TryGetMonthNumber(
  AYear: Integer; const S: string; const AFormat: TFormatSettings; out AValue: Cardinal): Boolean;
var
  I: Integer;
begin
  for I := 1 to 12 do
  ...
  Result := inherited TryGetMonthNumber(AYear, S, AFormat, AValue);
end;
...
function TcxHebrewCalendarTable.ToDateTime(const ADateTime: TcxDateTime): TDateTime;
var
  AYear, ADays: Integer;
  ALunarDate: TcxDate;
...
function TcxHebrewCalendarTable.TryGetDayNumber(const S: string; out AValue: Cardinal): Boolean;
begin
  AValue := HebrewNumber(S);
  if AValue > 0 then
    Result := True
  else
    Result := inherited TryGetDayNumber(S, AValue);
end;

function TcxHebrewCalendarTable.TryGetMonthNumber(AYear: Integer;
  const S: string; const AFormat: TFormatSettings; out AValue: Cardinal): Boolean;
var
  I: Integer;
begin
  AValue := HebrewNumber(S);
  if IsValidMonth(AYear, AValue) then
    Exit(True);
  for I := 1 to 13 do
  begin
    if AnsiCompareText(S, cxGetLocalMonthName(AYear, I, Self)) = 0 then
...
function TcxHebrewCalendarTable.TryGetYearNumber(const S: string; out AValue: Cardinal): Boolean;
begin
  AValue := HebrewNumber(S);
  if AValue <> 0 then
  begin
    ...
    Result := inherited TryGetYearNumber(S, AValue);
end;
