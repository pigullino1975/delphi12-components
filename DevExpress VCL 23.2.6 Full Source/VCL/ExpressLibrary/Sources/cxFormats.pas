...
uses
  Types, Windows, Messages, SysUtils, Classes,
  dxCore, cxDateUtils, cxClasses, cxControls;
...
type
  ...
  public
    ...
    property Items[AIndex: Integer]: TcxDateTimeFormatItem read GetItems; // for internal use
  ...
  private
    ...
    FDateEditShowZeroTime: Boolean;
    FFirstWeekOfYear: TcxFirstWeekOfYear;
    FIsStandardMaskValid: Boolean;
    FList: TList;
    ...
    function GetCurrencyFormat: string; overload;
    ...
    class function GetDateTimeFormatItemStandardMaskZoneLength(const AItem: TcxDateTimeFormatItem): Integer;
    function GetStartOfWeek: TDay;
    function InternalGetDateTimeEditRegExprMask(AFormatInfo: TcxDateTimeFormatInfo; AMaskKind: TcxDateTimeEditMaskKind): string;
    function InternalGetDateTimeEditStandardMask(AFormatInfo: TcxDateTimeFormatInfo; AMaskKind: TcxDateTimeEditMaskKind): string;
    function InternalGetMaskedDateEditFormat(AFormatInfo: TcxDateTimeFormatInfo): string;
    ...
    procedure SetDateEditShowZeroTime(Value: Boolean);
  ...
  public
    ...
    class function GetDateTimeFormatItemStandardMaskInfo(
      const AFormatInfo: TcxDateTimeFormatInfo; APos: Integer; out AItemInfo: TcxDateTimeFormatItemInfo): Boolean;
    function GetDateTimeStandardMaskStringLength(const AFormatInfo: TcxDateTimeFormatInfo): Integer;
    ...
    class function GetCurrencyFormat(const AFormatSettings: TFormatSettings): string; overload; static; // for internal use
    ...
    property DateEditShowZeroTime: Boolean read FDateEditShowZeroTime write SetDateEditShowZeroTime;
    property DateFormatInfo: TcxDateTimeFormatInfo read FDateFormatInfo; // for internal use
    property DateTimeFormatInfo: TcxDateTimeFormatInfo read FDateTimeFormatInfo; // for internal use
    property FirstWeekOfYear: TcxFirstWeekOfYear read FFirstWeekOfYear write SetFirstWeekOfYear;
    property IsStandardMaskValid: Boolean read FIsStandardMaskValid; // for internal use
    ...
    property TimeFormatInfo: TcxDateTimeFormatInfo read FTimeFormatInfo; // for internal use
  ...
  protected
    function ToFormatSettings: TFormatSettings;
  public
...
function cxFormatControllerAssigned: Boolean;
function cxFormatController: TcxFormatController;
function GetCharString(C: Char; ACount: Integer): string; inline; 
...
uses
  Forms, DateUtils, Math, dxCalendarUtils, StrUtils;

const
  dxThisUnitName = 'cxFormats';
...
var
  ...
  begin
    ...
    if (AItemKind <> dtikDay) or (Length(AItemData) <= 2) then
  ...
  begin
    if (AFormatInfo.ItemCount = 0) or (AFormatInfo.Items[AFormatInfo.ItemCount - 1].Kind <> dtikString) then
  ...
  function GetCount(const AFormat: string; var AStartIndex: Integer; AStartSymbol: Char): Integer;
  begin
    Result := 1;
      ...
      case AToken of
        'E', 'Y':
          begin
            ...
            AddFormatItem(dtikYear, IfThen(ACount <= 2, 'YY', 'YYYY'));
          end;
        'G':
          begin
            Result := False;
        ...
        'M':
          begin
            ACount := Min(GetCount(AFormat, I, AStartSymbol), 4);
            AddFormatItem(dtikMonth, DupeString('M', ACount));
          end;
        'D':
          begin
            ACount := GetCount(AFormat, I, AStartSymbol);
            case ACount of
              1..4:
                AddFormatItem(dtikDay, DupeString('D', ACount));
              5:
        ...
        'H':
          begin
            ACount := Min(GetCount(AFormat, I, AStartSymbol), 2);
            AddFormatItem(dtikHour, DupeString('H', ACount));
            AThereIsHourItem := True;
          end;
        'N':
          begin
            ACount := Min(GetCount(AFormat, I, AStartSymbol), 2);
            AddFormatItem(dtikMin, DupeString('N', ACount));
          end;
        'S':
          begin
            ACount := Min(GetCount(AFormat, I, AStartSymbol), 2);
            AddFormatItem(dtikSec, DupeString('S', ACount));
          end;
        'T':
          begin
            ACount := GetCount(AFormat, I, AStartSymbol);
        ...
        'Z':
          begin
            ACount := Min(GetCount(AFormat, I, AStartSymbol), 3);
            AddFormatItem(dtikMSec, DupeString('Z', ACount));
          end;
        'A':
          begin
            ...
            begin
              ...
              if SameText(Copy(AFormat, I - 1, 3), 'A/P') then
              begin
                ...
                if SameText(Copy(AFormat, I - 1, 4), 'AMPM') then
                begin
                  ...
                  if SameText(Copy(AFormat, I - 1, 3), 'AAA') then
                  begin
...
procedure TcxFormatController.CalculateDateEditMasks(AUseSmartInputWhenRegExpr: Boolean);

  procedure ValidateStandardMask(AFormatInfo: TcxDateTimeFormatInfo);
  var
    I, J, L: Integer;
    AItem: TcxDateTimeFormatItem;
  begin
    FIsStandardMaskValid := True;
    for I := 0 to AFormatInfo.ItemCount - 1 do
    begin
      AItem := AFormatInfo.Items[I];
      if (AItem.Kind = dtikMonth) and (Length(AItem.Data) > 2) then
      begin
        L := Length(dxFormatSettings.ShortMonthNames[1]);
        for J := 2 to 12 do
          if L <> Length(dxFormatSettings.ShortMonthNames[J]) then
          begin
            FIsStandardMaskValid := False;
            Break;
          end;
          Break;
      end;
...
begin
  ...
  GetDateTimeFormatInfo(GetDateTimeDisplayFormat(dtmkDateTime), FDateTimeFormatInfo);

  FDateEditMask := InternalGetDateTimeEditStandardMask(FDateFormatInfo, dtmkDate);
  if not FAssignedStandardDateEditMask then
  begin
    FStandardDateEditMask := FDateEditMask;
    ValidateStandardMask(FDateFormatInfo);
  end
  else
    FIsStandardMaskValid := True;
  ...
  begin
    FRegExprDateTimeEditMask := InternalGetDateTimeEditRegExprMask(FDateFormatInfo, dtmkDate);
  ...
  if not FAssignedStandardDateTimeEditMask then
    FStandardDateTimeEditMask := InternalGetDateTimeEditStandardMask(FDateTimeFormatInfo, dtmkDateTime);
...
class function TcxFormatController.GetCurrencyFormat(
  const AFormatSettings: TFormatSettings): string;

  function GetPositiveCurrencyFormat(const AFormat, ACurrStr: string): string;
  begin
    case AFormatSettings.CurrencyFormat of
  ...
  begin
    case AFormatSettings.NegCurrFormat of
...
begin
  if AFormatSettings.CurrencyDecimals > 0 then
    Result := DupeString('0', AFormatSettings.CurrencyDecimals)
  else
    Result := '';
  Result := ',0.' + Result;
  ACurrStr := '';
  for I := 1 to Length(AFormatSettings.CurrencyString) do
  begin
    C := AFormatSettings.CurrencyString[I];
    if (C = ',') or (C = '.') then
      ACurrStr := ACurrStr + '''' + C + ''''
    else
      ACurrStr := ACurrStr + C;
...
begin
  AFormatSettings := dxFormatSettings.ToFormatSettings;
  Result := GetCurrencyFormat(AFormatSettings)
end;
...
var
  ...
  AExistFirst: Boolean;
begin
  AFormat := dxFormatSettings.ShortDateFormat;
  Result := '';
  ...
  begin
    ...
    begin
      AExistFirst := True;
      if (1 < I) and (AFormat[I - 1] = 'd') then
        AExistFirst := False;
      if (I < Length(AFormat)) and (AFormat[I + 1] = 'd') then
        AExistFirst := False;
      if AExistFirst then
        Result := Result + 'd';
    end;
    ...
    begin
      AExistFirst := True;
      if (1 < I) and (AFormat[I - 1] = 'M') then
        AExistFirst := False;
      if (I < Length(AFormat)) and (AFormat[I + 1] = 'M') then
        AExistFirst := False;
      if AExistFirst then
        Result := Result + 'M';
    end;
...
function TcxFormatController.GetDateTimeDisplayFormat(AMaskKind: TcxDateTimeEditMaskKind): string;
begin
  case AMaskKind of
    dtmkDate:
...
class function TcxFormatController.GetDateTimeFormatItemStandardMaskZoneLength(const AItem: TcxDateTimeFormatItem): Integer;
begin
  case AItem.Kind of
    dtikString:
      Result := Length(AItem.Data);
    dtikYear:
      Result := IfThen(Length(AItem.Data) = 2, 2, 4);
    dtikMonth, dtikDay:
      Result := IfThen(Length(AItem.Data) < 3, 2, 3);
    ...
    dtikTimeSuffix:
      begin
        if UpperCase(AItem.Data) = 'A/P' then
          ...
          if UpperCase(AItem.Data) = 'AM/PM' then
            Result := 2
          else
          begin
...
function TcxFormatController.InternalGetDateTimeEditRegExprMask(
  ...
  procedure ProcessDateItem(var S: string; const AFormatItem: TcxDateTimeFormatItem);
  const
  ...
  procedure ProcessTimeItem(var S: string; const AFormatItem: TcxDateTimeFormatItem);
  begin
    with AFormatItem do
      case Kind of
        dtikString:
        ...
        dtikTimeSuffix:
          begin
            if UpperCase(Data) = 'A/P' then
              ...
              if UpperCase(Data) = 'AM/PM' then
                S := S + '(AM|PM)?'
              else
                if (dxFormatSettings.TimeAMString <> '') or (dxFormatSettings.TimePMString <> '') then
                begin
                  S := S + '(''';
...
begin
  ...
  begin
    ...
    case AItem.Kind of
      dtikString:
        for J := 1 to Length(AItem.Data) do
          AddChar(Result, AItem.Data[J]);
      dtikYear:
      ...
      dtikTimeSuffix:
        begin
          if UpperCase(AItem.Data) = 'A/P' then
            ...
            begin
              J := Max(Length(dxFormatSettings.TimePMString), Length(dxFormatSettings.TimeAMString));
              Result := Result + DupeString('c', J);
            end;
        end;
      dtikDateSeparator:
        Result := Result + '/';
...
function TcxFormatController.InternalGetMaskedDateEditFormat(AFormatInfo: TcxDateTimeFormatInfo): string;
var
  I: Integer;
begin
  Result := '';
  for I := 0 to AFormatInfo.ItemCount - 1 do
    with AFormatInfo.Items[I] do
      case Kind of
        ...
        dtikMonth:
          Result := Result + IfThen(Length(Data) < 3, 'mm', 'mmm');
        dtikDay:
          Result := Result + IfThen(Length(Data) < 3, 'dd', 'ddd');
        dtikHour:
          Result := Result + 'hh';
        dtikMin:
          Result := Result + 'nn';
...
function TcxFormatController.IsMessageOnlyWindow: Boolean;
begin
  Result := False;
end;
...
function cxFormatControllerAssigned: Boolean;
begin
  Result := FcxFormatController <> nil;
end;

function cxFormatController: TcxFormatController;
begin
  if (FcxFormatController = nil) and not FUnitIsFinalized then
...
procedure TcxFormatController.SetDateEditShowZeroTime(Value: Boolean);
begin
  if FDateEditShowZeroTime <> Value then
  begin
    FDateEditShowZeroTime := Value;
    FormatChanged;
  end;
end;
...
function TdxFormatSettings.ToFormatSettings: TFormatSettings;
var
  I: Integer;
begin
...
finalization
  ...
  dxFreeGlobalObject(FcxFormatController);
