...
type
  ...
  IcxDesignHelper2 = interface
  ['{B5E39C64-543A-4AED-B81D-DA59317FB6E9}']
    procedure ClearSelection(AOwner: TComponent);
    procedure GetSelection(AOwner: TComponent; AList: TList);
    procedure SetSelection(AOwner: TComponent; AList: TList);
  end;
  ...
  TdxTimerProc = procedure (AWnd: HWnd; AMsg: UINT; ATimerID: UINT_PTR; ASysTime: DWORD); stdcall;
  ...
  private
    FOwnObjects: Boolean;
    function GetItem(Index: TdxListIndex): TObject;
  protected
  ...
  public
    ...
    property Items[Index: TdxListIndex]: TObject read GetItem; default;
  ...
  private
    function GetItem(Index: TdxListIndex): TObject;
    procedure SetItem(Index: TdxListIndex; Value: TObject);
  public
    property Items[Index: TdxListIndex]: TObject read GetItem write SetItem; default;
  end;
  ...
  TcxComponentCollectionSortCompareFunc = reference to function (const AItem1, AItem2: TcxComponentCollectionItem): Integer;
  ...
  protected
    ...
    procedure UpdateFormPosition(AForm: TForm);
    procedure UpdateFormSize(AForm: TForm);
  public
...
procedure RenameComponents(ACaller, AOwner: TComponent; ANewName: TComponentName;
  const AOldName: TComponentName; AComponentCount: Integer; AGetComponent: TcxGetComponent);
...
procedure cxRegisterVclClasses(AClasses: array of TPersistentClass);
...
function dxSetTimer(AWindowHandle: THandle; ATimerID: TdxNativeUInt; AInterval: Cardinal): Boolean; overload;
function dxSetTimer(AInterval: Cardinal; ATimerProcedure: TdxTimerProc): TdxNativeUInt; overload;
function dxSetTimer(AWindowHandle: THandle; ATimerID: TdxNativeUInt; AInterval: Cardinal; ATimerProcedure: TdxTimerProc): TdxNativeUInt; overload;
function dxKillTimer(AWindowHandle: HWND; var ATimerID: TdxNativeUInt): Boolean;

procedure dxStreamInsertData(AStream: TStream; AData: TMemoryStream;
  const AInsertPosition: Int64; AProgressHelper: TcxCustomProgressCalculationHelper = nil);

implementation

uses
  RTLConsts, Math, Graphics, dxTypeInfo, cxGraphics, cxControls, dxHashUtils, dxHash, dxDPIAwareUtils, dxThreading,
  dxForms;

const
  dxThisUnitName = 'cxClasses';
...
begin
  if AObject <> nil then
    Result := ObjectLinkController.AddLink(AObject)
  else
    Result := nil;
end;
...
begin
  ...
  begin
    if FUnitIsFinalized then
      ...
      ObjectLinkController.RemoveLink(ALink);
  end;
end;

procedure cxClearObjectLinks(AObject: TObject);
begin
  if not FUnitIsFinalized and (FObjectLinkController <> nil) then
...
function cxIsDestroying(AComponent: TComponent; ARecursive: Boolean = False): Boolean;
begin
  Result := (AComponent <> nil) and ((csDestroying in AComponent.ComponentState) or ARecursive and (cxIsDestroying(AComponent.Owner) or (AComponent is TControl) and (TControl(AComponent).Parent <> AComponent.Owner) and cxIsDestroying(TControl(AComponent).Parent)));
end;
...
function cxStrCharLength(const AStr: string; AIndex: Integer = 1): Integer;
begin
  Result := TdxNativeUInt(CharNext(GetPChar(AStr, AIndex))) - TdxNativeUInt(GetPChar(AStr, AIndex));
end;

function cxNextCharPos(const AStr: string; AIndex: Integer): Integer;
begin
  Result := TdxNativeUInt(CharNext(GetPChar(AStr, AIndex))) - TdxNativeUInt(GetPChar(AStr, 1)) + 1;
end;

function cxPrevCharPos(const AStr: string; AIndex: Integer): Integer;
begin
  Result := TdxNativeUInt(CharPrev(GetPChar(AStr, 1), GetPChar(AStr, AIndex))) - TdxNativeUInt(GetPChar(AStr, 1)) + 1;
end;
...
destructor TcxCustomComponent.Destroy;
begin
...
begin
  if CanSetTimer then
    TimerOn := dxSetTimer(cxTimerWindow.Handle, FEventID, FInterval);
end;
...
begin
  ...
  if dxGetProcessDpiAwareness = pdaUnaware then 
...
function TcxObjectList.GetItem(Index: TdxListIndex): TObject;
begin
...
function TcxOpenList.GetItem(Index: TdxListIndex): TObject;
begin
  Result := TObject(inherited Items[Index]);
end;

procedure TcxOpenList.SetItem(Index: TdxListIndex; Value: TObject);
begin
...
procedure TcxComponentCollection.CustomSort(const ACompare: TcxComponentCollectionSortCompareFunc);
var
  AWasChanges: Boolean;
begin
  AWasChanges := False;
  FItems.SortList(
    function (AItem, AItem2: Pointer): Integer
    begin
      Result := ACompare(TcxComponentCollectionItem(AItem), TcxComponentCollectionItem(AItem2));
      AWasChanges := AWasChanges or (Result > 0);
    end);
  if AWasChanges then
    Changed;
end;
...
begin
  AName := '';
  TdxTypeInfo.EnumProperties(AObject.ClassInfo, [tkClass],
    procedure(APropInfo: PPropInfo; var ABreak: Boolean)
    begin
      ABreak := APropInfo.PropType^ = ASubobject.ClassInfo;
      if ABreak then
        AName := TdxTypeInfo.GetPropertyName(APropInfo);
    end);
  Result := AName;
end;
...
function CreateUniqueName(AOwnerForm, AOwnerComponent, AComponent: TComponent;
  ...
  var
    I: Integer;
  begin
    if AComponent.FindComponent(AName) <> nil then
      Exit(False);
    for I := 0 to AComponent.ComponentCount - 1 do
    begin
      if not IsUnique(AComponent.Components[I], AName) then
        Exit(False);
    end;
    Result := True;
  end;
...
begin
  ...
  begin
    Result := cxGenerateComponentName(AOwnerComponent, AComponent.ClassName, APrefixName, ASuffixName, I);
...
begin
  ...
  for I := Low(AClasses) to High(AClasses) do
    GroupDescendentsWith(AClasses[I], TControl);
  for I := Low(AClasses) to High(AClasses) do
...
function dxSetTimer(AWindowHandle: THandle; ATimerID: TdxNativeUInt; AInterval: Cardinal): Boolean;
begin
  Result := dxSetTimer(AWindowHandle, ATimerID, AInterval, nil) <> 0;
end;

function dxSetTimer(AInterval: Cardinal; ATimerProcedure: TdxTimerProc): TdxNativeUInt;
begin
  Result := dxSetTimer(0, 0, AInterval, ATimerProcedure);
end;

function dxSetTimer(AWindowHandle: THandle; ATimerID: TdxNativeUInt; AInterval: Cardinal; ATimerProcedure: TdxTimerProc): TdxNativeUInt;
begin
  Result := Windows.SetTimer(AWindowHandle, ATimerID, AInterval, @ATimerProcedure);
end;
...
begin
  ...
  if Result then
  begin
    KillTimer(AWindowHandle, ATimerID);
    ATimerID := 0;
  end;
end;
...
begin
  UpdateFormPosition(AForm);
  UpdateFormSize(AForm);
...
procedure TcxDialogMetricsInfo.UpdateFormPosition(AForm: TForm);
begin
  AForm.DefaultMonitor := dmDesktop;
  if not FMaximized and (AForm.WindowState = wsMaximized) then
    AForm.WindowState := wsNormal;
  AForm.SetBounds(FLeft, FTop, AForm.Width, AForm.Height);
end;

procedure TcxDialogMetricsInfo.UpdateFormSize(AForm: TForm);
var
  IClientSize: IcxDialogMetricsClientSize;
  AClientSize: TSize;
  M, D: Integer;
...
begin
  Result := dxWaitForSyncObject(Handle, TimeOut);
end;
...
procedure TcxComponentList.DoNotify(AItem: TComponent; AAction: TListNotification);
  ...
  function ConvertNotification(ANotification: TListNotification): TcxComponentCollectionNotification;
  begin
...
begin
  ...
  Update(AItem, ConvertNotification(AAction));
end;
...
finalization
  ...
  dxFreeGlobalObject(FObjectLinkController);
