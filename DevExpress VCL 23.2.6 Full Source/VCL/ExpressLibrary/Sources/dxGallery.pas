...
uses
  ...
  dxCoreClasses, dxCore, cxClasses, dxGDIPlusClasses, cxGraphics, cxAccessibility;
...
type
  ...
  TdxGalleryGroupEvent = procedure(Sender: TObject; AGroup: TdxCustomGalleryGroup) of object;
  ...
  { TdxGalleryGroupCustomAccessibilityHelper }

  TdxGalleryGroupCustomAccessibilityHelper = class(TcxAccessibilityHelper) // for internal use
  protected
    function GetGroup: TdxCustomGalleryGroup; virtual; abstract;
    ...
    property Group: TdxCustomGalleryGroup read GetGroup;
  end;
  ...
  protected
    ...
    function GetCollectionFromParent(AParent: TComponent): TcxComponentCollection; override;
    function GetGallery: IdxGallery2; inline;
...
uses
  System.UITypes,
  Types, StrUtils, Controls, Variants, cxGeometry;

const
  dxThisUnitName = 'dxGallery';
...
procedure TdxCustomGalleryGroupHeader.SetHeight(AValue: Integer);
begin
...
function TdxGalleryGroupCustomAccessibilityHelper.ChildIsSimpleElement(AIndex: Integer): Boolean;
begin
  Result := True;
end;

function TdxGalleryGroupCustomAccessibilityHelper.GetChildCount: Integer;
begin
  Result := Group.Items.Count;
end;

function TdxGalleryGroupCustomAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  if AChildID = 0 then
    Result := Group.Caption
  else
    Result := Group.Items[AChildID - 1].Caption;
end;

function TdxGalleryGroupCustomAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  if AChildID = 0 then
...
function TdxGalleryGroupCustomAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  if (Parent.States[cxAccessibleObjectSelfID] and cxSTATE_SYSTEM_INVISIBLE <> 0) or not Group.Visible then
...
function TdxGalleryGroupCustomAccessibilityHelper.GetSupportedProperties(
  AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := [aopLocation, aopSelect];
  if AChildID > 0 then
    Result := Result + [aopFocus];
end;
...
begin
  Changed((AItem = nil) or (AAction <> ccnChanged));
end;
...
begin
  ...
  begin
    case ItemCheckMode of
      ...
      icmSingleRadio:
        begin
          CheckItem(GetCheckedItem, False);
          CheckItem(AItem, True);
        end;
      icmMultiple:
...
procedure TdxCustomGallery.GroupsChangeHandler(Sender: TObject;
  AItem: TcxComponentCollectionItem; AAction: TcxComponentCollectionNotification);
begin
  Changed(gctHard);
end;
