...
type
  ...
  public
    ...
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
    procedure AssignFont(ASourceFont: TFont; ASourceDPI: Integer); overload;
    procedure AssignFont(ASourceFont: TFont; ASourceScaleFactor: TdxScaleFactor); overload;
...
uses
  Controls, TypInfo, IniFiles, dxCore, dxDPIAwareUtils, Forms;

const
  dxThisUnitName = 'cxStyles';
...
begin
  ...
  begin
    ...
    begin
      AScaleFactor := FStyleRepository.ScaleFactor;
      if not (csDestroying in ComponentState) then
        ChangeScale(AScaleFactor.Denominator, AScaleFactor.Numerator);
      FStyleRepository.RemoveItem(Self);
      FStyleRepository := nil;
    end;
    if Value <> nil then
    begin
      FStyleRepository := Value;
      FStyleRepository.AddItem(Self);
      AScaleFactor := FStyleRepository.ScaleFactor;
      if not (csReading in ComponentState) then
        ChangeScale(AScaleFactor.Numerator, AScaleFactor.Denominator);
    end;
  end;
end;
...
{$IFDEF DELPHI110}
  if AOwner is TCustomForm then
    FFont.Height := dxGetFontHeightForDefaultDPI(FFont.Size, TCustomForm(AOwner).PixelsPerInch);
{$ELSE}
...
procedure TcxStyle.AssignFont(ASourceFont: TFont; ASourceDPI: Integer);
var
  AStyleDPI: Integer;
begin
  if Assigned(StyleRepository) then
    AStyleDPI := StyleRepository.PixelsPerInch
  else
    AStyleDPI := dxDefaultDPI;
  dxAssignFont(Font, ASourceFont, AStyleDPI, ASourceDPI)
end;

procedure TcxStyle.AssignFont(ASourceFont: TFont; ASourceScaleFactor: TdxScaleFactor);
var
  AStyleScaleFactor: TdxScaleFactor;
begin
  if Assigned(StyleRepository) then
    AStyleScaleFactor := IdxScaleFactor(StyleRepository).Value
  else
    AStyleScaleFactor := dxDefaultScaleFactor;
  dxAssignFont(Font, ASourceFont, AStyleScaleFactor, ASourceScaleFactor)
end;
...
begin
  Result := AStyle.Font;
  if AStyle.StyleRepository <> nil then
  begin
