...
type
  ...
  private
    ...
    function IsFaxDevice(AAttributes: DWORD): Boolean;
  public
    constructor Create(ADriver, ADevice, APort: PChar; AAttributes: Integer);
    ...
    property IsFax: Boolean read FIsFax;
  end;
...
begin
  ...
  TdxImageDrawer.DrawImage(ACanvas, cxRectOffset(cxRectSetSize(ARect, AImageList.Width, AImageList.Height), AOffset, AOffset),
...
constructor TdxPrintDeviceInfo.Create(ADriver, ADevice, APort: PChar; AAttributes: Integer);
begin
  inherited Create;
  FDriver := StrNew(ADriver);
  FDevice := StrNew(ADevice);
  FPort := StrNew(APort);
  FIsDefault := dxIsDefaultPrinter(ADevice);
  FIsNetwork := IsNetworkDevice(AAttributes);
  FIsFax := IsFaxDevice(AAttributes);
end;
...
function TdxPrintDeviceInfo.IsFaxDevice(AAttributes: DWORD): Boolean;
begin
  Result := AAttributes and PRINTER_ATTRIBUTE_FAX = PRINTER_ATTRIBUTE_FAX;
end;

function TdxPrintDeviceInfo.IsNetworkDevice(AAttributes: DWORD): Boolean;
begin
...
function TdxPrintDevice.GetPrinters: TStrings;
  ...
  begin
    ...
    begin
      PrintDeviceInfo := TdxPrintDeviceInfo.Create('', pPrinterName, '', Attributes);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        PrintDeviceInfo := TdxPrintDeviceInfo.Create(nil, pPrinterName, PortName, Attributes);
