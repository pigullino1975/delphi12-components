...
uses
  ...
  cxGraphics, dxPSCore, dxBkGnd, cxClasses, cxDrawTextUtils, cxGeometry, dxGDIPlusClasses, dxGDIPlusAPI,
...
type
  ...
  public
    ...
    procedure Combine(const R: TcxRegion; AOperation: TcxRegionOperation; const XForm: TXForm);
  ...
  public
    ...
    // Clipping
    function CombineClipRgn(ARegion: TcxRegionHandle; AOperation: TcxRegionOperation): Boolean;
    ...
    procedure DrawPath(APath: TdxGPPath; const R: TRect;
      AColor: TColor = clNone; AStrokeWidth: Double = 0; ABackgroundColor: TColor = clNone);
    ...
    procedure PolyBezier(APoints: array of TPoint; AColor: TColor = clNone; AStrokeWidth: Double = 0;
...
uses
  Types, Math, Forms, Dialogs, ShellApi, dxCore, dxCoreGraphics, cxControls, cxCustomCanvas,
  dxShellDialogs, dxPSPDFExportDialog, dxPSPDFMetaFileParser, dxPSEdgePatterns, dxPSPDFStrings;

const
  dxThisUnitName = 'dxPSPDFExport';
...
type
  ...
  public
    ...
    function CalculateTextWidth(const AText: string): Single; overload;
    function CalculateTextWidth(const AText: PChar; ATextLength: Integer): Single; overload;
    function CalculateTotalTextHeight: Integer;
    procedure CalculateEndEllipsis(DC: HDC; const R: TRect);
    //
...
function InternalExportToPDF(AStream: TStream; AReportLink: TBasedxReportLink;
  ...
  out AOpenDocumentAfterExport: Boolean; const AStreamFileName: string = ''): Boolean;
...
begin
  ...
  try
    Result := AReportLink.CanExportToPDF and SetupOptions(AProvider.Options) and DoBeforeExportToPDF(AProvider.Options);
    if Result then
    begin
      AProvider.Build;
      AProvider.SaveToStream(AStream);
    end;
    AOpenDocumentAfterExport := AProvider.Options.OpenDocumentAfterExport;
  finally
    AProvider.Free;
  end;
end;
...
begin
  ...
  Result.cx := Round(PDFCanvas.PageContent.TextWidth(AText, ATextLength));
end;
...
begin
  Result := False;
  if dxPSShowPDFSettingsDialog(AReportLink.PDFExportOptions) then
  begin
    ...
    try
      ...
      Result := AResult and dxPSExportToPDFFile(
        ChangeFileExt(ASaveDialog.FileName, sdxPDFExt),
        AReportLink, False, AReportLink.PDFExportOptions);
    finally
      ASaveDialog.Free;
    end;
  end;
...
function dxPSExportToPDFFile(const AFileName: string; AReportLink: TBasedxReportLink;
  AShowSettingsDialog: Boolean = True; ASettings: TdxPSPDFReportExportOptions = nil): Boolean;
var
  AOpenDocumentAfterExport: Boolean;
  AStream: TFileStream;
begin
  Result := False;
  try
    ...
    try
      Result := InternalExportToPDF(AStream, AReportLink,
        AShowSettingsDialog, ASettings, AOpenDocumentAfterExport, AFileName);
    finally
      AStream.Free;
    end;
  finally
    if Result then
    begin
      if AOpenDocumentAfterExport then
...
var
  AOpenDocumentAfterExport: Boolean;
begin
  Result := InternalExportToPDF(AStream, AReportLink, AShowSettingsDialog, ASettings, AOpenDocumentAfterExport);
end;
...
begin
  ...
  begin
    ...
    if TextParams.EndEllipsis and (TextRowCount > 0) then
      CalculateEndEllipsis(PDFCanvas.Canvas.Handle, ABounds);
    Result := CalculateTextRect(Result);
  end;
end;

function TdxPSPDFExportCustomTextHelper.CalculateTextWidth(const AText: string): Single;
begin
  Result := CalculateTextWidth(PChar(AText), Length(AText));
end;

function TdxPSPDFExportCustomTextHelper.CalculateTextWidth(const AText: PChar; ATextLength: Integer): Single;
begin
  Result := PDFCanvas.PageContent.TextWidth(AText, ATextLength);
end;

procedure TdxPSPDFExportCustomTextHelper.CalculateEndEllipsis(DC: HDC; const R: TRect);
var
  ACharNumber: Integer;
  AHeight: Integer;
  ARow: PcxTextRow;
  ...
  AWidth: Integer;
begin
  FHasEndEllipsis := False;
  AHeight := cxRectHeight(R);
  AWidth := cxRectWidth(R);

  while TextRowCount > 1 do
  begin
    if CalculateTotalTextHeight <= AHeight then
      Break;
    FHasEndEllipsis := True;
    Dec(FRows.Count);
  end;

  ARow := LastTextRow;
  FHasEndEllipsis := FHasEndEllipsis or (ARow^.TextExtents.cx > AWidth);
  if FHasEndEllipsis then
  begin
    FTextParams.EndEllipsisWidth := Round(CalculateTextWidth(cxEndEllipsis));
    Dec(AWidth, TextParams.EndEllipsisWidth);
    if AWidth > 0 then
    begin
      if not GetTextExtentExPointW(DC, ARow.Text, ARow.TextLength, AWidth, @ACharNumber, nil, ASize) then
        ACharNumber := 0;
      while ACharNumber > 0 do
      begin
        ARowWidth := CalculateTextWidth(ARow.Text, ACharNumber);
        if ARowWidth <= AWidth then
        begin
          ARow^.TextLength := ACharNumber;
          ARow^.TextExtents.cx := Round(ARowWidth);
          Break;
        end;
        Dec(ACharNumber);
      end;
...
begin
  ...
  begin
    ARow := TextRow[I];
    ARow^.TextExtents.cx := Min(ARow^.TextExtents.cx, Round(CalculateTextWidth(TextRowText[I])));
  end;

  AExtraSpace := CalculateTotalTextHeight - S.cy;
  if AExtraSpace > 0 then
  begin
    ALineSpacing := AExtraSpace / TextRowCount;
    if ALineSpacing > TextParams.RowHeight * 0.3 then
      Exit;
...
begin
  ...
  APDFFile.DocumentInfo.Created := ReportLink.ReportDocument.CreationDate;
  APDFFile.SecurityOptions.Assign(Options.SecurityOptions);
  APDFFile.SignatureOptions.Assign(Options.SignatureOptions);
end;
...
begin
  ...
  TBasedxReportLinkAccess(ReportLink).UpdatePrintDate;
...
procedure TdxPSPDFCanvasClipRegion.Combine(const R: TcxRegion; AOperation: TcxRegionOperation; const XForm: TXForm);
begin
  GetActualRegion(XForm).Combine(R, AOperation, False);
end;
...
begin
  ...
  ASize := cxSizeScale(FCanvasSize, 1 / XForm.eM11);
  Result := TdxPSPDFCanvasRegion.Create(0, 0, ASize.cx, ASize.cy);
  Result.XForm := XForm;
  Add(Result);
end;
...
begin
  ...
  FWorldTransform := cxIdentityMatrix;
end;
...
procedure TdxPSPDFReportRenderCanvas.DrawPath(APath: TdxGPPath; const R: TRect;
  AColor: TColor = clNone; AStrokeWidth: Double = 0; ABackgroundColor: TColor = clNone);
var
  I, APointCount: Integer;
  APathPoints: TdxGpPointFDynArray;
  APointTypes: TBytes;
  APointType: Byte;
  AClose: Boolean;
begin
  if IsRectVisible(R) and ((AColor <> clNone) or (ABackgroundColor <> clNone)) then
  begin
    PreparePageContent;
    try
      APointCount := APath.GetPointCount;
      APath.GetPoints(APathPoints, APointCount);
      APath.GetPointTypes(APointTypes, APointCount);
      AClose := False;
      I := 0;
      while I < APointCount do
      begin
        APointType := APointTypes[I];
        case APointType and $0F of
          Ord(TdxGpPathPointType.PathPointTypeStart):
            PageContent.MoveTo(APathPoints[I].X, APathPoints[I].Y);
          Ord(TdxGpPathPointType.PathPointTypeLine):
            PageContent.LineTo(APathPoints[I].X, APathPoints[I].Y);
          Ord(TdxGpPathPointType.PathPointTypeBezier):
            if I + 2 < APointCount then
            begin
              PageContent.Bezier(TdxPointF(APathPoints[I]), TdxPointF(APathPoints[I + 1]), TdxPointF(APathPoints[I + 2]));
              Inc(I, 2);
              APointType := APointTypes[I];
            end;
        end;
        if (APointType and $F0) = Ord(TdxGpPathPointType.PathPointTypeCloseSubpath) then
          AClose := True;
        Inc(I);
      end;
      if ABackgroundColor <> clNone then
        PageContent.FillPolygon(AColor, ABackgroundColor)
      else
        PageContent.Stroke(AColor, AStrokeWidth, AClose);
    finally
...
begin
  ...
  begin
    DrawTextLine(AText, CalculateTextOffset(dxRectF(ATextRect), ADirection),
      AClipRect, ATextWidth, ATextColor, cxRotationAngleToAngle[ADirection], ABreakExtraSpacing, ANoClip);
  end;
end;
  ...
  begin
    ...
    begin
      ...
      PageContent.DrawFrame(cxRectInflate(R, -ALineThickness, -ALineThickness),
        ATopLeftColor, ARightBottomColor, ALineThickness, cxBordersAll);
      ...
      begin
        ...
        PageContent.DrawFrame(R, ATopLeftColor, ARightBottomColor, ALineThickness, cxBordersAll);
      end;
    end;
  end;
...
begin
  ...
  begin
    PreparePageContent;
    try
      PageContent.DrawFrame(R, ATopLeftColor, ARightBottomColor, ABorderWidth, ABorders);
    finally
      UnpreparePageContent;
    end;
  end;
...
begin
  ...
  begin
    PreparePageContent;
    try
      PageContent.Polyline(APoints, AColor, ALineWidth);
    finally
      UnpreparePageContent;
    end;
  end;
...
begin
  ...
  try
    AMatrix.Rotate(cxRotationAngleToAngle[AAngle], GetPivotPoint(ARect, AAngle));
...
function TdxPSPDFReportRenderCanvas.CombineClipRgn(ARegion: TcxRegionHandle; AOperation: TcxRegionOperation): Boolean;
var
...
begin
  ...
  try
    ...
    ClipRegion.Combine(ATempRegion, AOperation, WorldTransform);
    Result := True;
  finally
    ATempRegion.Free;
  end;
...
begin
  Result := CombineClipRgn(ARegion, roIntersect);
end;
...
procedure TdxPSPDFReportRenderCanvas.PolyBezier(APoints: array of TPoint; AColor: TColor; AStrokeWidth: Double;
  ABackgroundColor: TColor);
var
  AIndex, APointCount: Integer;
begin
  if Length(APoints) > 1 then
  begin
    PreparePageContent;
    try
      PageContent.MoveTo(APoints[0]);
      AIndex := 1;
      APointCount := Length(APoints);
      while AIndex + 1 < APointCount do
      begin
        PageContent.Bezier(
          dxPointF(APoints[AIndex]),
          dxPointF(APoints[AIndex + 1]),
          dxPointF(APoints[IfThen(AIndex + 2 > APointCount, 0, AIndex + 2)]));
        Inc(AIndex, 3);
      end;
      if ABackgroundColor <> clNone then
        PageContent.FillPolygon(clNone, ABackgroundColor)
      else
        PageContent.Stroke(AColor, AStrokeWidth);
    finally
      UnpreparePageContent;
    end;
  end;
...
begin
  ...
  dxPDFExportMetaFile(Self, FCustomDrawMetaFile, FCustomDrawBounds);
...
procedure TdxPSPDFReportRenderCanvas.PreparePageContent;
  ...
  begin
    ...
    ATransform := cxIdentityMatrix;
    for AIndex := ClipRegion.Count - 1 downto 0 do
      ATransform := TXForm.Combine(ATransform, TdxGPMatrix.Invert(ConvertMatrix(ClipRegion[AIndex].XForm)));
