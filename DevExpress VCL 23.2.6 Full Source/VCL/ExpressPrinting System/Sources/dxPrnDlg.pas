...
uses
  ...
  dxLayoutLookAndFeels, dxLayoutControlAdapters, dxLayoutcxEditAdapters, cxImageList, cxCustomListBox, cxImageComboBox;
...
uses
  Variants, WinSpool, Math, Types, cxGeometry, dxShellDialogs, dxPrintingStrs, dxPSPopupMan, dxPSRes, dxPSEngn, dxPSEvnt,
  dxPSUtl, dxPSImgs;

const
  dxThisUnitName = 'dxPrnDlg';
...
begin
  ...
  try
    ...
    begin
      ...
      AImageItem.Value := I;
      with TdxPrintDeviceInfo(APrinters.Objects[I]) do
      begin
        if IsFax then
          AImageItem.ImageIndex := 2
        else
          AImageItem.ImageIndex := IfThen(IsDefault, 1, 0);
      end;
    end;
