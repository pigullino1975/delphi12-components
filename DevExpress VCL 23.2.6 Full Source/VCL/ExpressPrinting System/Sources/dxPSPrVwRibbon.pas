...
type
  ...
  TdxRibbonPrintPreviewForm = class(TdxRibbonForm,
    ...
    procedure cbxPredefinedZoomButtonClick(Sender: TObject; AButtonIndex: Integer);
...
function TdxRibbonPSPopupMenuBuilder.BuildPopup(const AControl: TControl; 
  ...
  begin
    ...
    Result := FBarManager.AddItem(BarItemClass) as TdxBarItem;
...
begin
  ...
  try
    ...
    begin
      ilSmallImages.AddImagesFromResZipStream(HInstance, IDB_DXPSPREVIEW_TOOLBARS_SMALLIMAGELIST, 'SVG');
      ilLargeImages.AddImagesFromResZipStream(HInstance, IDB_DXPSPREVIEW_RIBBON_IMAGELIST, 'SVG');
    end
    else
    begin
...
begin
  ...
  (cbxPredefinedZoom.Properties as TcxComboBoxProperties).DropDownRows := Preview.PredefinedZooms.Count;
  cbxPredefinedZoom.Properties.BeginUpdate;
  try
    for I := 0 to Preview.PredefinedZooms.Count - 1 do
      (cbxPredefinedZoom.Properties as TcxComboBoxProperties).Items.Add(Preview.PredefinedZooms[I]);
    cbxPredefinedZoom.Properties.OnButtonClick := cbxPredefinedZoomButtonClick;
    AButton := cbxPredefinedZoom.Properties.Buttons.Add;
    AButton.Kind := bkEllipsis;
    AButton.Default := False;
  finally
    cbxPredefinedZoom.Properties.EndUpdate;
  end;
...
begin
  ...
  cbxPredefinedZoom.EditValue := IntToStr(ZoomFactor) + PercentSymbol;
  if Assigned(ReportLink) then
    (seActivePage.Properties as TcxSpinEditProperties).MaxValue := ReportLink.PageCount;
...
begin
  ...
  try
    ...
    dxBarScaleToFit.Visible := (ReportLink <> nil) and (ReportLink.SupportsScaling);
    ...
    dxBarFormat.Visible := bbLargePageSetup.ActuallyVisible or bbFormatSubItem.ActuallyVisible or rgiBackgroundColor.ActuallyVisible;
    ...
    begin
      (seActivePage.Properties as TcxSpinEditProperties).MinValue := 1;
      if ReportLink <> nil then
      begin
        (seActivePage.Properties as TcxSpinEditProperties).MaxValue := ReportLink.PageCount;
...
procedure TdxRibbonPrintPreviewForm.cbxPredefinedZoomButtonClick(Sender: TObject; AButtonIndex: Integer);
begin
...
begin
  ...
  begin
    ABarButton := BarDesignController.AddInternalItem(TdxBarButton, dxBarManager, Self, nil) as TdxBarButton;
...
begin
  ...
  begin
    ...
    V := Max(V, Round((seActivePage.Properties as TcxSpinEditProperties).MinValue));
    V := Min(V, Round((seActivePage.Properties as TcxSpinEditProperties).MaxValue));
...
begin
  ...
  UpdateControls;
  cbxPredefinedZoom.EditValue := AddPercentageChar(IntToStr(ZoomFactor));
end;
