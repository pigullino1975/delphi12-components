...
uses
  ...
  dxPSReportRenderCanvas, cxGeometry, cxGraphics, dxLayoutCommon, dxPSGlbl, dxGDIPlusClasses, dxTreeView;
...
type
  ...
  private
    ...
    function GetTextOutTextFlag: Integer;
    function GetTextSize: TSize;
  protected
    function GetDTFormat: DWORD; override;

    function CanDrawByFormattedText: Boolean; virtual;
    procedure DoDrawByFormattedText(ACanvas: TdxPSReportRenderCustomCanvas); virtual;

    property Item: TdxCustomLayoutItem read GetItem;
  public
    procedure DrawText(ACanvas: TdxPSReportRenderCustomCanvas); override;
    function MeasureTextHeight(ACanvas: TdxPSReportRenderCustomCanvas): Integer; override;
    function MeasureTextWidth(ACanvas: TdxPSReportRenderCustomCanvas): Integer; override;
  ...
  protected
    function CanDrawByFormattedText: Boolean; override;
  ...
  { TdxReportLayoutCheckBoxItem }

  TdxReportLayoutCheckBoxItem = class(TdxReportLayoutLabeledItem)
  private
    FCheckBoxCell: TdxReportCellCheckImage;
  protected
    procedure Initialize(AReportLink: TBasedxReportLink); override;
  end;

  { TdxReportLayoutRadioButtonItem }

  TdxReportLayoutRadioButtonItem = class(TdxReportLayoutLabeledItem)
  private
    FRadioButtonCell: TdxReportCellRadio;
  protected
    procedure Initialize(AReportLink: TBasedxReportLink); override;
  end;
  ...
  { TdxPSLayoutCheckBoxItemProducer }

  TdxPSLayoutCheckBoxItemProducer = class(TdxPSLayoutCustomLabeledItemProducer)
  protected
    class function ReportItemClass: TdxReportCellClass; override;
  public
    class function LayoutItemClass: TdxCustomLayoutItemClass; override;
  ...
  { TdxPSLayoutRadioButtonItemProducer }

  TdxPSLayoutRadioButtonItemProducer = class(TdxPSLayoutCustomLabeledItemProducer)
  protected
    class function ReportItemClass: TdxReportCellClass; override;
  public
    class function LayoutItemClass: TdxCustomLayoutItemClass; override;
  ...
  protected
    ...
    procedure AddHiddenItem(ATreeView: TdxTreeViewControl; AParent: TdxTreeViewNode; AnItem: TdxCustomLayoutItem);
    procedure AddItem(ATreeView: TdxTreeViewControl; AParent: TdxTreeViewNode; AnItem: TdxCustomLayoutItem);
    function IsComponentEditable(AComponent: TComponent): Boolean; override;
    procedure LoadControlsTree(ATreeView: TdxTreeViewControl); override;
    procedure LoadHiddenControlsTree(ATreeView: TdxTreeViewControl); override;
  ...
  protected
    ...
    function IsBoldNode(ANode: TdxTreeViewNode): Boolean; override;
  public
...
uses
  ...
  dxPSUtl, dxLayoutControlAdapters, cxControls, ImgList, Types, dxCore, cxLookAndFeels, cxLookAndFeelPainters,
  dxFormattedText, dxPScxCommon, dxCustomTree;

const
  dxThisUnitName = 'dxPSdxLCLnk';
...
procedure TdxReportLayoutCaptionTextCell.DrawText(ACanvas: TdxPSReportRenderCustomCanvas);
begin
  if CanDrawByFormattedText then
    DoDrawByFormattedText(ACanvas)
  else
    inherited DrawText(ACanvas);
end;

function TdxReportLayoutCaptionTextCell.MeasureTextHeight(ACanvas: TdxPSReportRenderCustomCanvas): Integer;
begin 
  if CanDrawByFormattedText then
    Result := GetTextSize.cy
  else
    Result := inherited MeasureTextHeight(ACanvas);
end;

function TdxReportLayoutCaptionTextCell.MeasureTextWidth(ACanvas: TdxPSReportRenderCustomCanvas): Integer;
begin 
  if CanDrawByFormattedText then
    Result := GetTextSize.cx
  else
    Result := inherited MeasureTextWidth(ACanvas);
end;

function TdxReportLayoutCaptionTextCell.MeasureHeight(ACanvas: TdxPSReportRenderCustomCanvas): Integer;
begin 
  Result := inherited MeasureHeight(ACanvas);
  if CanDrawByFormattedText then
    Result := Result + cxMarginsHeight(Indents.Rect);
end;

function TdxReportLayoutCaptionTextCell.MeasureWidth(ACanvas: TdxPSReportRenderCustomCanvas): Integer;
begin 
  Result := inherited MeasureWidth(ACanvas);
  if CanDrawByFormattedText then
    Result := Result + cxMarginsWidth(Indents.Rect);
end;
...
function TdxReportLayoutCaptionTextCell.CanDrawByFormattedText: Boolean;
begin
  Result := TdxCustomLayoutItemCaptionOptionsAccess(Item.CaptionOptions).FormattedText.HasFormatting;
end;

procedure TdxReportLayoutCaptionTextCell.DoDrawByFormattedText(ACanvas: TdxPSReportRenderCustomCanvas);

  procedure CalculateTextLayout(AFormattedText: TdxFormattedText; var ARect: TRect; AZoomFactor: Single);
  begin 
    TdxFormattedTextConverters.Import(AFormattedText, Text);
    ARect := cxRectContent(BoundsRect, Indents.Rect);
    Font.Size := MulDiv(Font.Size, PixelsDenominator, PixelsNumerator);
    try
      AFormattedText.CalculateLayout(cxScreenCanvas.Canvas, Font, ARect, GetTextOutTextFlag, nil, AZoomFactor);
    finally
      Font.Size := MulDiv(Font.Size, PixelsNumerator, PixelsDenominator);
    end;
    cxScreenCanvas.Dormant;
  end;

var
  APainter: TdxPSFormattedTextPainter;
...
begin 
  ...
  try
    ...
    CalculateTextLayout(AFormattedText, ARect, PixelsNumerator/PixelsDenominator);
    APainter := TdxPSFormattedTextPainter.Create(ACanvas);
    try
      ACanvas.SaveState;
      try
        AFormattedText.Draw(APainter, ARect.TopLeft);
      finally
...
function TdxReportLayoutCaptionTextCell.GetItem: TdxCustomLayoutItem;
begin
  Result := (Parent as TdxReportCustomLayoutCaptionCell).Item;
end;

function TdxReportLayoutCaptionTextCell.GetTextOutTextFlag: Integer;
const
  NoClipToFlag: array[Boolean] of Integer = (0, CXTO_NOCLIP);
  CharbreakToFlag: array[Boolean] of Integer = (0, CXTO_CHARBREAK);
  EndEllipsysToFlag: array[Boolean] of Integer = (0, CXTO_END_ELLIPSIS);
  MultilineToFlag: array[Boolean] of Integer = (0, CXTO_WORDBREAK);
  AlignmentsHorzToFlag: array[TcxTextAlignX] of Integer = (CXTO_LEFT, CXTO_CENTER_HORIZONTALLY, CXTO_RIGHT,
    CXTO_JUSTIFY_HORIZONTALLY, CXTO_DISTRIBUTE_HORIZONTALLY);
  AlignmentsVertToFlag: array[TcxTextAlignY] of Integer = (CXTO_TOP, CXTO_CENTER_VERTICALLY, CXTO_BOTTOM,
    CXTO_DISTRIBUTE_VERTICALLY);
begin 
  Result := AlignmentsHorzToFlag[TextAlignX] or AlignmentsVertToFlag[TextAlignY] or EndEllipsysToFlag[EndEllipsis] or
    MultilineToFlag[Multiline] or CharbreakToFlag[BreakByChars] or NoClipToFlag[NoClip];
end;

function TdxReportLayoutCaptionTextCell.GetTextSize: TSize;
var
  AFormattedText: TdxFormattedText;
  ARect: TRect;
begin 
  ...
  try
    ...
    TdxFormattedTextConverters.Import(AFormattedText, Text);
    ARect := cxRectContent(BoundsRect, Indents.Rect);
    AFormattedText.CalculateLayout(cxScreenCanvas.Canvas, Font, ARect, GetTextOutTextFlag or CXTO_NOCLIP);
    cxScreenCanvas.Dormant;
...
begin
  ...
  if Result.CanDrawByFormattedText then
    Result.Text := TdxCustomLayoutItemCaptionViewInfoAccess(ViewInfo.CaptionViewInfo).FormattedText.GetDisplayText
  else
  ...
  Result.EndEllipsis := TdxLayoutLabeledItemCustomCaptionOptionsAccess(CaptionOptions).EllipsisPosition <> epNone;
end;
...
function TdxReportLayoutGroupCaptionTextCell.CanDrawByFormattedText: Boolean;
begin
  Result := False; 
end;
...
function TdxReportLayoutSeparatorCaptionTextCell.CanDrawByFormattedText: Boolean;
begin
  Result := False; 
end;
...
procedure TdxReportLayoutCheckBoxItem.Initialize(AReportLink: TBasedxReportLink);
var
  AItem: TdxLayoutCheckBoxItem;
  ACheckGlyph: TdxSmartGlyph;
begin
  inherited Initialize(AReportLink);
  AItem := (ViewInfo as TdxLayoutCheckBoxItemViewInfo).Item;
  FCheckBoxCell := TdxReportCellCheckImage.Create(Self);
  FCheckBoxCell.BoundsRect :=
    cxRectOffset((ViewInfo as TdxLayoutCheckBoxItemViewInfo).CheckAreaBounds, ViewInfo.Bounds.TopLeft, False);
  FCheckBoxCell.ButtonEdgeStyle := cbesSingle;
  FCheckBoxCell.CellSides := [];
  FCheckBoxCell.Checked := AItem.State <> cbsUnchecked;
  FCheckBoxCell.Enabled := AItem.Enabled and (AItem.State <> cbsGrayed);
  ACheckGlyph := AItem.CheckBoxOptions.Glyph;
  if not ACheckGlyph.Empty then
  begin
    FCheckBoxCell.Glyph.Assign(ACheckGlyph);
    FCheckBoxCell.GlyphCount := AItem.CheckBoxOptions.GlyphCount;
  end;
end;

{ TdxReportLayoutRadioButtonItem }

procedure TdxReportLayoutRadioButtonItem.Initialize(AReportLink: TBasedxReportLink);
var
  AItem: TdxLayoutRadioButtonItem;
begin
  inherited Initialize(AReportLink);
  AItem := (ViewInfo as TdxLayoutRadioButtonItemViewInfo).Item;
  FRadioButtonCell := TdxReportCellRadio.Create(Self);
  FRadioButtonCell.BoundsRect :=
    cxRectOffset((ViewInfo as TdxLayoutRadioButtonItemViewInfo).CheckAreaBounds, ViewInfo.Bounds.TopLeft, False);
  FRadioButtonCell.ButtonEdgeStyle := cbesSingle;
  FRadioButtonCell.CellSides := [];
  FRadioButtonCell.Checked := AItem.Checked;
  FRadioButtonCell.Enabled := AItem.Enabled;
end;
...
class function TdxPSLayoutCheckBoxItemProducer.LayoutItemClass: TdxCustomLayoutItemClass;
begin
  Result := TdxLayoutCheckBoxItem;
end;

class function TdxPSLayoutCheckBoxItemProducer.ReportItemClass: TdxReportCellClass;
begin
  Result := TdxReportLayoutCheckBoxItem;
end;

{ TdxPSLayoutRadioButtonItemProducer }

class function TdxPSLayoutRadioButtonItemProducer.LayoutItemClass: TdxCustomLayoutItemClass;
begin
  Result := TdxLayoutRadioButtonItem;
end;

class function TdxPSLayoutRadioButtonItemProducer.ReportItemClass: TdxReportCellClass;
begin
  Result := TdxReportLayoutRadioButtonItem;
end;
...
procedure TdxLayoutControlReportLink.AddHiddenItem(ATreeView: TdxTreeViewControl;
  AParent: TdxTreeViewNode; AnItem: TdxCustomLayoutItem);
var
  Index, I: Integer;
begin
...
procedure TdxLayoutControlReportLink.AddItem(ATreeView: TdxTreeViewControl;
  AParent: TdxTreeViewNode; AnItem: TdxCustomLayoutItem);
var
  Index, I: Integer;
begin
...
procedure TdxLayoutControlReportLink.LoadControlsTree(ATreeView: TdxTreeViewControl);
begin
...
procedure TdxLayoutControlReportLink.LoadHiddenControlsTree(ATreeView: TdxTreeViewControl);
begin
  ATreeView.Items.Clear;
  if Container <> nil then
...
function TdxPSLayoutControlDesignWindow.IsBoldNode(ANode: TdxTreeViewNode): Boolean;
begin
...
begin
  ...
  TdxPSLayoutCheckBoxItemProducer.Register;
  TdxPSLayoutRadioButtonItemProducer.Register;
...
begin
  ...
  TdxReportLayoutCheckBoxItem.Register;
  TdxReportLayoutRadioButtonItem.Register;
...
begin
  ...
  TdxPSLayoutRadioButtonItemProducer.Unregister;
  TdxPSLayoutCheckBoxItemProducer.Unregister;
...
begin
  ...
  TdxReportLayoutRadioButtonItem.UnRegister;
  TdxReportLayoutCheckBoxItem.UnRegister;
