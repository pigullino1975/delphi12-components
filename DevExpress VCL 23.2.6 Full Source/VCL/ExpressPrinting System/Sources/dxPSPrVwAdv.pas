...
uses
  ...
  cxBarEditItem, cxSpinEdit, cxDropDownEdit, dxForms;

type
  TdxfmPreviewWdxBar = class(TdxForm, IdxPSPreviewWindowDialog)
  ...
  private
    ...
    procedure SetPredefinedZoomValue(AValue: Integer);
...
procedure TdxfmPreviewWdxBar.LoadStrings;
  ...
  procedure LocalizeImageComboBoxItem(const AItem: TcxImageComboBoxItem; AResId: Pointer);
  begin
    AItem.Description := cxGetResourceString(AResId);
    AItem.Value := AItem.Description;
  end;
...
//  bbFilePrintSetup.Caption := cxGetResourceString(@sdxMenuFilePrintSetup);
  ...
  LocalizeImageComboBoxItem(TcxImageComboBoxProperties(cbxPredefinedZoom.Properties).Items[8], @sdxPageWidth);
  LocalizeImageComboBoxItem(TcxImageComboBoxProperties(cbxPredefinedZoom.Properties).Items[9], @sdxWholePage);
  LocalizeImageComboBoxItem(TcxImageComboBoxProperties(cbxPredefinedZoom.Properties).Items[10], @sdxTwoPages);
  LocalizeImageComboBoxItem(TcxImageComboBoxProperties(cbxPredefinedZoom.Properties).Items[11], @sdxFourPages);
  LocalizeImageComboBoxItem(TcxImageComboBoxProperties(cbxPredefinedZoom.Properties).Items[12], @sdxWidenToSourceWidth);
...
procedure TdxfmPreviewWdxBar.SetPredefinedZoomValue(AValue: Integer);
var
  AProperties: TcxImageComboBoxProperties;
begin
  AProperties := cbxPredefinedZoom.Properties as TcxImageComboBoxProperties;
  AProperties.DefaultDescription := AddPercentageChar(IntToStr(Preview.ZoomFactor));
  cbxPredefinedZoom.EditValue := AProperties.DefaultDescription;
end;
...
begin
  ...
  Preview.UpdateControls;
  SetPredefinedZoomValue(Preview.ZoomFactor);
end;
