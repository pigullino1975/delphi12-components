...
uses
  ...
  dxCore, dxCoreClasses, cxGraphics, cxClasses, cxControls, cxStyles, cxLookAndFeels, cxGeometry, cxContainer, cxLookAndFeelPainters,
...
uses
  ...
  cxSchedulerStrs, cxSchedulerActionsStrs, dxCoreGraphics, dxMessageDialog;

const
  dxThisUnitName = 'dxPScxSchedulerLnk';
...
procedure TcxSchedulerReportDetailsAgendaEventCell.CreateMessageCell(AProperties: TcxCustomEditProperties);
begin
  if Scheduler.ViewAgenda.ShowMessages then
...
begin
  ...
  try
    TdxImageDrawer.DrawImage(cxPaintCanvas, cxRectCenter(ABounds, StylePreviewImages.Width, StylePreviewImages.Height), nil,
...
begin
  ...
  begin
    dxMessageBox(Handle,
      cxGetResourceString(@sdxBadDatePrintRange),
      cxGetResourceString(@sdxReportDesignerCaption),
...
function TcxfmSchedulerReportLinkDesignWindow.CheckPrintStyleSettings: Boolean;
  ...
  begin
    dxMessageBox(Handle,
      cxGetResourceString(@sdxBadTimePrintRange),
      cxGetResourceString(@sdxReportDesignerCaption),
      MB_ICONINFORMATION or MB_OK);
  end;
...
begin
  AParams := AViewParams;
  if AParams.Color = clWhite then
    AParams.TextColor := clBlack
  else
    if (AParams.Color = clWindow) or (AParams.Color = ColorToRgb(clWindow)) then
      AParams.TextColor := clWindowText;
  ReportLink.SetCellViewParams(Self, AParams);
  for I := 0 to DataItemCount - 1 do
    ReportLink.SetCellViewParams(DataItems[I], AParams);
end;
...
begin
  Result := CreateStringCell(GetCaption, FHasDetailInfo, cdidSchedulerEventCaption);
  if HasDetailInfo then
    Result.TextAlignY := taTop;
  if not FHasDetailInfo and (IsHeaderEvent and not HasImages) then
...
procedure TcxSchedulerReportEventModernCell.CorrectCaptionRectForMessage(
  ACanvas: TdxPSReportRenderCustomCanvas; var ARect: TRect);
begin
  inherited CorrectCaptionRectForMessage(ACanvas, ARect);
end;
...
procedure TcxSchedulerReportTimeLineCellsProducer.GetPageResources(AResources: TdxFastObjectList);
var
  I: Integer;
begin
  AResources.Clear;
