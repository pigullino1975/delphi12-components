...
type
  ...
  public
    ...
    function GetDrawingBoundsInLayoutUnits(const ABounds: TRect): TRect; virtual;
...
const
  dxThisUnitName = 'dxPSRichEditControlLnk.DocumentExporter';
...
begin
  ...
  begin
    Bounds := TRect.CreateSize(FPageCell.BoundsRect.Size);
...
var
  ABounds, ABoundingRectInLayoutUnits, ABoundingRect: TRect;
...
begin
  ABoundingRectInLayoutUnits := GetDrawingBoundsInLayoutUnits(TdxRectangleUtils.BoundingRectangle(ABox.Bounds, ATransform));
  ABoundingRect := DocumentModel.LayoutUnitConverter.LayoutUnitsToDocuments(ABoundingRectInLayoutUnits);
  Result := TdxOfficeImage.CreateSize(Max(ABoundingRect.Width, 1), Max(ABoundingRect.Height, 1));
  Result.SetResolution(300, 300);
  AGraphics := TdxGraphics.CreateFromImage(Result);
  try
    AGraphics.PageUnit := guDocument;
    AModifier := TdxGraphicsToLayoutUnitsModifier.Create(AGraphics, DocumentModel.LayoutUnitConverter);
    try
      APainter := TdxGdiPainter.Create(AGraphics, DocumentModel.LayoutUnitConverter);
      try
        AExporter := TdxGraphicsDocumentLayoutExporter.Create(DocumentModel, APainter,
          TdxWinFormsGraphicsDocumentLayoutExporterAdapter.Create, Bounds, TextColors);
        try
          AExporter.PieceTable := PieceTable;
          ABounds := AExporter.GetDrawingBounds(ABox.Bounds);
          AGraphics.TranslateWorldTransform(
            -ABounds.X + (ABoundingRectInLayoutUnits.Width - ABounds.Width) / 2,
            -ABounds.Y + (ABoundingRectInLayoutUnits.Height - ABounds.Height) / 2);
          if Safe.Cast(ABox, TdxFloatingObjectBox, AFloatingObjectBox) then
            AExporter.ExportRotatedContent(AFloatingObjectBox);
          if Safe.Cast(ABox, TdxParagraphFrameBox, AParagraphFrameBox) then
...
begin
  ...
  begin
    ATextViewInfo := FMeasurer.TextViewInfoCache.TryGetTextViewInfo(AText, AFontInfo);
...
function TdxPrintingDocumentExporter.GetDrawingBoundsInLayoutUnits(const ABounds: TRect): TRect;
begin
  Result := inherited GetDrawingBounds(ABounds);
end;

function TdxPrintingDocumentExporter.GetDrawingBounds(const ABounds: TRect): TRect;
begin
  Result := DocumentModel.LayoutUnitConverter.LayoutUnitsToDocuments(ABounds);
end;
