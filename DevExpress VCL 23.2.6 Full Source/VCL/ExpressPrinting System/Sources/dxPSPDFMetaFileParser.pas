...
uses
  ...
  dxCore, cxGeometry, cxClasses, cxGraphics, cxDrawTextUtils, dxBkGnd, dxGDIPlusClasses,
  dxPSCore, dxPSPDFExportCore, dxPSPDFExport, dxPSGlbl, dxPSFillPatterns, dxPSReportRenderCanvas;

type

  { TdxPSPDFMetaFileGdiObjectItem }

  TdxPSPDFMetaFileGdiObjectItem = class
  strict private
    FColor: TColor;
    FHandle: THandle;
  ...
  private
    function GetItem(Index: TdxListIndex): TdxPSPDFMetaFileGdiObjectItem;
  protected
    procedure Notify(Ptr: Pointer; Action: TListNotification); override;
  public
    function CreateBrush(const ABrushInfo: TLogBrush32): HBRUSH;
    function CreateFont(const AFontInfo: TExtLogFontW): HFONT;
    function CreatePen(const APenInfo: TLogPen): HPEN;
    function CreatePenEx(const APenInfo: TExtLogPen32): HPEN;
    function FindObject(AHandle: THandle; var AObject: TdxPSPDFMetaFileGdiObjectItem): Boolean;
    //
    property Items[Index: TdxListIndex]: TdxPSPDFMetaFileGdiObjectItem read GetItem;
  end;

  { TdxPSPDFMetaFileCanvasState }

  TdxPSPDFMetaFileCanvasState = class
  strict private
  ...
  public
    ...
    property TextAlign: Cardinal read FTextAlign write FTextAlign;
  ...
  TdxPSPDFMetaFileTextInfo = class
  strict private
    FFont: TFont;
    FOptions: Integer;
    FText: string;
    ...
    function ConvertTextOrigin(const P: TPoint; const ABox: TRect; ATextWidth, ATextHeight, AAlignMode: Integer): TPoint;
    function GetNoClip: Boolean;
  public
    constructor Create;
    destructor Destroy; override;
    function Concatenate(AInfo: TdxPSPDFMetaFileTextInfo): Boolean;
    procedure Initialize(const R: PEMRExtTextOut; AIsUnicode: Boolean; AFont: TFont; ATextAlign: Integer);
    //
  ...
  TdxPSPDFMetaFileExportProvider = class
  strict private
  ...
  protected
    ...
    function CreateRegion(ARgnData: PRgnData): TcxRegion;
    ...
    function ConvertPoints(P: PByte; ACount: Integer; AIsSmallPoints: Boolean;
      AReservedStartPointCount: Integer = 0): TPoints;
    ...
    procedure EndOutput;
    //
    procedure EmfBeginPath(const ARecord: PEMRBeginPath);
    procedure EmfAbortPath(const ARecord: PEMRAbortPath);
    procedure EmfEndPath(const ARecord: PEMREndPath);
    procedure EmfFillPath(const ARecord: PEMRFillPath);
    procedure EmfStrokePath(const ARecord: PEMRStrokePath);

    procedure EmfAlphaBlend(const ARecord: PEMRAlphaBlend);
    procedure EmfBitBlt(const ARecord: PEMRBitBlt);
    procedure EmfCloseFigure(const ARecord: PEMRCloseFigure);
    ...
    procedure EmfMoveTo(const ARecord: PEMRMoveToEx);
    procedure EmfPaintRgn(const ARecord: EMRPaintRgn);
    procedure EmfPie(const ARecord: EMRPie; ATable: PHandleTable);
    procedure EmfPolyBezier(const ARecord: PEMRPolyBezier; ASmallPoints: Boolean);
    procedure EmfPolyBezierTo(const ARecord: PEMRPolyBezierTo; ASmallPoints: Boolean);
    ...
    procedure EmfPolylineTo(const P: PEMRPolyLineTo; ASmallPoints: Boolean);
    ...
    procedure EmfSelectClipPath(const ARecord: PEMRSelectClipPath);
    ...
    procedure EmfSetTextAlign(const ARecord: PEMRSetTextAlign; ATable: PHandleTable);
    ...
    property HasFill: Boolean read GetHasFill;
...
uses
  dxCoreGraphics, Math, dxPSTrueTypeFont, dxPSUtl, dxGDIPlusAPI;

const
  dxThisUnitName = 'dxPSPDFMetaFileParser';
...
function TdxPSPDFMetaFileExportProvider.ConvertPoints(P: PByte; ACount: Integer; AIsSmallPoints: Boolean;
  AReservedStartPointCount: Integer = 0): TPoints;
var
  I: Integer;
begin
  Inc(ACount, AReservedStartPointCount);
  SetLength(Result, ACount);
  for I := AReservedStartPointCount to ACount - 1 do
  begin
...
var
  ARegionData: PRgnData;
begin
  Result := nil;
  if cxGetRegionData(ARegion, ARegionData) then
  try
    Result := CreateRegion(ARegionData)
  finally
    FreeMem(ARegionData);
  end;
...
function TdxPSPDFMetaFileExportProvider.CreateRegion(ARgnData: PRgnData): TcxRegion;
var
  ARgnRect: PRect;
  I: Integer;
begin
...
procedure TdxPSPDFMetaFileExportProvider.EmfAbortPath(const ARecord: PEMRAbortPath);
begin
  if FPath <> nil then
    FPath.Reset;
  Dec(FPathLevel);
...
procedure TdxPSPDFMetaFileExportProvider.EmfBeginPath(const ARecord: PEMRBeginPath);
begin
  Inc(FPathLevel);
  if FPathLevel = 1 then
  begin
    FreeAndNil(FPath);
...
procedure TdxPSPDFMetaFileExportProvider.EmfCloseFigure(const ARecord: PEMRCloseFigure);
begin
  if FPath <> nil then
  begin
    FPath.FigureFinish;
...
procedure TdxPSPDFMetaFileExportProvider.EmfSetTextAlign(const ARecord: PEMRSetTextAlign; ATable: PHandleTable);
begin
  State.TextAlign := ARecord.iMode;
end;
...
begin
  ...
  begin
    ...
    AInfo.Initialize(ARecord, AIsUnicode, PDFCanvas.Font, State.TextAlign);
...
begin
  ...
  begin
    ...
    try
      if HasFill then
        PDFCanvas.FillEllipse(R, BrushColor, State.TextColor, nil, Brush);
...
procedure TdxPSPDFMetaFileExportProvider.EmfEndPath(const ARecord: PEMREndPath);
begin
  Dec(FPathLevel);
end;
...
procedure TdxPSPDFMetaFileExportProvider.EmfFillPath(const ARecord: PEMRFillPath);
begin
  if (FPath <> nil) and (FPath.GetPointCount > 0) then
  begin
    BeginOutput;
    try
      PDFCanvas.DrawPath(FPath, ARecord^.rclBounds, clNone, 0, BrushColor);
    finally
      EndOutput;
    end;
  end;
...
begin
  ...
  begin
    ARegion := CreateRegion(@ARecord.RgnData[0]);
    try
      BeginOutput;
      try
...
var
  ATargetPoint: TPoint;
begin
  ATargetPoint := ConvertPoint(ARecord.ptl);
  if IsInPath then
    FPath.AddLine(FMoveToPoint, ATargetPoint)
  else
  begin
    BeginOutput;
    try
      PDFCanvas.Polyline([FMoveToPoint, ATargetPoint], Pen.Color, PenWidth);
    finally
      EndOutput;
    end;
  end;
  FMoveToPoint := ATargetPoint;
end;
...
procedure TdxPSPDFMetaFileExportProvider.EmfMaskBlt(const ARecord: PEMRMaskBlt);
  ...
  begin
    ...
    try
      ...
      PDFCanvas.ClipRegion.Combine(AMaskRegion, roIntersect, GetActualWorldTransform);
    finally
      AMaskRegion.Free;
    end;
  end;
...
procedure TdxPSPDFMetaFileExportProvider.EmfMoveTo(const ARecord: PEMRMoveToEx);
begin
  FMoveToPoint := ConvertPoint(PEMRMoveToEx(ARecord)^.ptl);
  if IsInPath then
    FPath.FigureStart;
end;
...
begin
  ...
  begin
    ...
    try
      BeginOutput;
      try
        PDFCanvas.FillRegion(ARegion.Handle, BrushColor, clNone, nil, Brush);
      finally
        EndOutput;
      end;
    finally
...
var
  APoints: TPoints;
begin
  APoints := ConvertPoints(@P^.aptl[0], P^.cptl, ASmallPoints);
  if IsInPath then
    FPath.AddPolyline(APoints)
  else
  begin
    BeginOutput;
    try
      PDFCanvas.Polyline(APoints, Pen.Color, PenWidth);
    finally
      EndOutput;
    end;
  end;
...
procedure TdxPSPDFMetaFileExportProvider.EmfPolylineTo(const P: PEMRPolyLineTo; ASmallPoints: Boolean);
var
  APoints: TPoints;
begin
  APoints := ConvertPoints(@P^.aptl[0], P^.cptl, ASmallPoints, 1);
  APoints[0] := FMoveToPoint;
  if IsInPath then
    FPath.AddPolyline(APoints)
  else
  begin
  ...
  FMoveToPoint := APoints[High(APoints)];
end;

procedure TdxPSPDFMetaFileExportProvider.EmfPolyBezier(const ARecord: PEMRPolyBezier; ASmallPoints: Boolean);
var
  APoints: TPoints;
  AIndex: Integer;
begin
  APoints := ConvertPoints(@ARecord^.aptl[0], ARecord^.cptl, ASmallPoints);
  if IsInPath and (BrushColor = clNone) then
  begin
    AIndex := 0;
    while AIndex + 3 < Length(APoints) do
    begin
      FPath.AddBezier(
        dxPointF(APoints[AIndex]),
        dxPointF(APoints[AIndex + 1]),
        dxPointF(APoints[AIndex + 2]),
        dxPointF(APoints[AIndex + 3]));
      Inc(AIndex, 4);
    end;
  end
  else
  begin
    BeginOutput;
    try
      PDFCanvas.PolyBezier(APoints, Pen.Color, PenWidth, BrushColor);
    finally
      EndOutput;
    end;
  end;
...
procedure TdxPSPDFMetaFileExportProvider.EmfPolyBezierTo(const ARecord: PEMRPolyBezierTo; ASmallPoints: Boolean);
var
  APoints: TPoints;
  AIndex: Integer;
begin
  ...
  begin
    AIndex := 0;
    while AIndex + 2 < Length(APoints) do
    begin
      FPath.AddBezier(
        dxPointF(FMoveToPoint),
        dxPointF(APoints[AIndex]),
        dxPointF(APoints[AIndex + 1]),
        dxPointF(APoints[AIndex + 2]));
      FMoveToPoint := APoints[AIndex + 2];
      Inc(AIndex, 3);
    end;
  end
//TODO:
...
//    BeginOutput;
//    try
//      PDFCanvas.Bezier(APoints, Pen.Color, PenWidth);
//    finally
//      EndOutput;
//    end;
//  end;
  ...
  begin
    ...
    begin
      if APolygon^.aPolyCounts[I] > 1 then
        FPath.AddPolygon(ConvertPoints(ACursor, APolygon^.aPolyCounts[I], ASmallPoints));
  ...
  begin
    ...
    try
      ...
      try
        ...
        PageContent.FillPolygon(Pen.Color, BrushColor, State.PolyFillMode);
      finally
...
begin
  ...
  begin
    for I := 0 to ACount - 1 do
    begin
      FPath.AddPolyline(ConvertPoints(ACursor, APolyline^.aPolyCounts[I], ASmallPoints));
  ...
  begin
    ...
    try
      ...
      try
        for I := 0 to ACount - 1 do
        begin
          PageContent.Polyline(ConvertPoints(ACursor, APolyline^.aPolyCounts[I], ASmallPoints), Pen.Color, PenWidth);
          Inc(ACursor, APolyline^.aPolyCounts[I] * PointSizeMap[ASmallPoints]);
        end;
      finally
        PDFCanvas.UnpreparePageContent;
...
begin
  ...
  begin
    BeginOutput;
    try
      PDFCanvas.Pie(ConvertRect(ARecord.rclBox), ConvertPoint(ARecord.ptlStart), ConvertPoint(ARecord.ptlEnd), BrushColor);
    finally
      EndOutput;
    end;
  end;
...
var
  APoints: TPoints;
begin
  APoints := ConvertPoints(@APolygon^.aptl[0], APolygon^.cptl, ASmallPoints);
  if IsInPath then
    FPath.AddPolygon(APoints)
  else
  begin
    BeginOutput;
    try
      PDFCanvas.Polygon(APoints, Pen.Color, BrushColor, PenWidth, State.PolyFillMode);
    finally
      EndOutput;
    end;
  end;
...
begin
  ...
  begin
    ...
    try
      if HasFill then
        PDFCanvas.FillRectEx(R, BrushColor, nil, Brush);
      if (BrushColor <> Pen.Color) and (Pen.Width <> 0) then
...
begin
  ...
  if IsInPath then
    FPath.AddRoundRect(R, ACorners.cx, ACorners.cy)
  else
  begin
    BeginOutput;
    try
      if HasFill then
        PDFCanvas.FillRoundRect(R, ACorners.cx, ACorners.cy, BrushColor, State.TextColor, nil, Brush);
...
procedure TdxPSPDFMetaFileExportProvider.EmfSelectClipPath(const ARecord: PEMRSelectClipPath);
var
  ARegion: TcxRegion;
begin
  if FPath <> nil then
  begin
    ARegion := TcxRegion.Create(FPath.GetGdiRegionHandle(dxGPMeasureCanvas));
    try
      ARegion.Offset(cxPointInvert(BaseWindowOrg));
      PDFCanvas.ClipRegion := FBaseClipRegion.Clone;
      PDFCanvas.ClipRegion.Combine(ARegion, roIntersect, FBaseWorldTransform);
    finally
      ARegion.Free;
    end;
  end;
...
var
  R: TRect;
begin
  R := ConvertRect(Bounds(ARecord.ptlPixel.X, ARecord.ptlPixel.Y, 1, 1));
  if IsInPath then
    FPath.AddRect(R)
  else
  begin
    BeginOutput;
    try
      PDFCanvas.FillRect(R, ARecord.crColor);
    finally
      EndOutput;
    end;
  end;
...
begin
  if not IsInPath and (ARecord^.cbBitsSrc <> 0) and CheckROP(ARecord^.dwRop) then
  begin
    DrawBitmap(
...
procedure TdxPSPDFMetaFileExportProvider.EmfStrokePath(const ARecord: PEMRStrokePath);
begin
  if (FPath <> nil) and (FPath.GetPointCount > 0) then
  begin
    BeginOutput;
    try
      PDFCanvas.DrawPath(FPath, ARecord^.rclBounds, Pen.Color, PenWidth);
    finally
      EndOutput;
    end;
  end;
...
function TdxPSPDFMetaFileExportProvider.IsInPath: Boolean;
begin
  Result := FPathLevel > 0;
end;
...
function TdxPSPDFMetaFileExportProvider.GetBrushColor: TColor;
begin
...
function TdxPSPDFMetaFileExportProvider.GetHasFill: Boolean;
begin
  Result := (BrushColor <> clNone) or (Brush.Style > bsClear);
end;
...
begin
  ...
  case ARecord^.iType of
    EMR_BEGINPATH:
      EmfBeginPath(PEMRBeginPath(ARecord));
    EMR_ABORTPATH:
      EmfAbortPath(PEMRAbortPath(ARecord));
    EMR_ENDPATH:
      EmfEndPath(PEMREndPath(ARecord));
    EMR_FILLPATH:
      EmfFillPath(PEMRFillPath(ARecord));
    EMR_STROKEPATH:
      EmfStrokePath(PEMRStrokePath(ARecord));
    ...
    EMR_POLYLINETO, EMR_POLYLINETO16:
      EmfPolyLineTo(PEMRPolyLineTo(ARecord), ARecord^.iType = EMR_POLYLINETO16);
    EMR_POLYBEZIER, EMR_POLYBEZIER16:
      EmfPolyBezier(PEMRPolyBezier(ARecord), ARecord^.iType = EMR_POLYBEZIER16);
    EMR_POLYBEZIERTO, EMR_POLYBEZIERTO16:
      EmfPolyBezierTo(PEMRPolyBezierTo(ARecord), ARecord^.iType = EMR_POLYBEZIERTO16);
    EMR_SETPIXELV:
      EmfSetPixel(PEMRSetPixelV(ARecord)^);
    EMR_MOVETOEX:
      EmfMoveTo(PEMRMoveToEx(ARecord));
    EMR_LINETO:
    ...
    EMR_SELECTCLIPPATH:
      EmfSelectClipPath(PEMRSelectClipPath(ARecord));
...
function TdxPSPDFMetaFileGdiObjectList.CreateBrush(
  const ABrushInfo: TLogBrush32): HBRUSH;
var
  ABrush: TBrush;
...
function TdxPSPDFMetaFileGdiObjectList.CreatePenEx(
  const APenInfo: TExtLogPen32): HPEN;
var
  APen: TPen;
begin
  APen := TPen.Create;
...
function TdxPSPDFMetaFileGdiObjectList.GetItem(Index: TdxListIndex): TdxPSPDFMetaFileGdiObjectItem;
begin
...
procedure TdxPSPDFMetaFileTextInfo.Initialize(const R: PEMRExtTextOut; AIsUnicode: Boolean; AFont: TFont; ATextAlign: Integer);
...
begin
  ...
  if (TextBounds.Right < 0) and (TextBounds.Bottom < 0) then
    FTextBounds := Bounds(TextOrigin.X, TextOrigin.Y, TextWidth, AFontHeight)
  else
    if FFont.Orientation = 0 then
    begin
      FTextOrigin := ConvertTextOrigin(FTextOrigin, TextBounds, FTextWidth, AFontHeight, ATextAlign);
      FTextBounds := cxRectSetOrigin(TextBounds, TextOrigin);
      FTextBounds.Bottom := TextBounds.Bottom + AFontHeight;
    end;
end;
...
function TdxPSPDFMetaFileTextInfo.ConvertTextOrigin(
  const P: TPoint; const ABox: TRect; ATextWidth, ATextHeight, AAlignMode: Integer): TPoint;
begin
  Result := P;
  if AAlignMode and TA_RIGHT = TA_RIGHT then
    Dec(Result.X, ATextWidth);
  if AAlignMode and TA_CENTER = TA_CENTER then
    Dec(Result.X, ATextWidth div 2);
  if (AAlignMode and TA_BOTTOM = TA_BOTTOM) or (AAlignMode and TA_CENTER = TA_CENTER) then
    Result.Y := ABox.Top; 
end;
