...
uses
  ...
  cxGraphics, dxTreeView, cxGroupBox, dxPSReportRenderCanvas, cxGeometry, cxLookAndFeels,
  ...
  Contnrs, dxGenerics, cxCustomListBox, dxTypeHelpers, dxPanel;
...
type
  ...
  TdxPSCustomContainerItemDefinitionList = class(TdxObjectList<TdxPSCustomContainerItemDefinition>);
  ...
  TdxPSdxCustomPanelProducer = class(TdxPSCustomContainerProducer)
  protected
    procedure InitializeItem(AnItem: TdxReportVisualItem); override;
    function ItemClass: TdxReportVisualItemClass; override;
    function ObjectExpandHeight: Boolean; override;
    function ObjectExpandWidth: Boolean; override;
  public
    function Control: TdxCustomPanel; reintroduce; overload;
    class function ControlClass: TControlClass; override;
    class function HasNativeSupportForBorders: Boolean; override;
  end;
  ...
  strict private
    ...
    FDefinitions: TdxPSCustomContainerItemDefinitionList; 
    FDelimitersHorz: TList;
    FDelimitersVert: TList;
  ...
  protected
    ...
    procedure AddControl(ATreeView: TdxTreeViewControl; AParent: TdxTreeViewNode; AControl: TControl);
    procedure AddHiddenControl(ATreeView: TdxTreeViewControl; AParent: TdxTreeViewNode; AControl: TControl);
    function AddNode(ATreeView: TdxTreeViewControl; AParent: TdxTreeViewNode; AComponent: TComponent; AChecked: Boolean): TdxTreeViewNode;
    ...
    procedure LoadControlsTree(ATreeView: TdxTreeViewControl); virtual;
    procedure LoadHiddenControlsTree(ATreeView: TdxTreeViewControl); virtual;
  ...
  TdxfmCustomContainerDesignWindow = class(TStandarddxReportLinkDesignWindow)
    ...
    tvControls: TdxTreeViewControl;
    tvHiddenControls: TdxTreeViewControl;
    ...
    procedure TreeViewChange(Sender: TObject; Node: TdxTreeViewNode);
    procedure TreeViewCheckAllChildrenClick(Sender: TObject);
    procedure TreeViewCheckAllClick(Sender: TObject);
    procedure TreeViewCustomDrawItem(Sender: TCustomTreeView; Node: TdxTreeViewNode; State: TCustomDrawState; var DefaultDraw: Boolean);
    ...
    procedure TreeViewNodeStateChanged(Sender: TdxCustomTreeView; ANode: TdxTreeViewNode);
    procedure tvControlsCustomDrawNode(Sender: TdxCustomTreeView; ACanvas: TcxCanvas; ANodeViewInfo: TdxTreeViewNodeViewInfo; var AHandled: Boolean);
    procedure TreeViewControlsEditing(Sender: TdxCustomTreeView; ANode: TdxTreeViewNode; var Allow: Boolean);
  private
    FAreHiddenControlsChanged: Boolean;
    FLastActiveTab: Integer;
    FIsTreeViewProcessing: Boolean;
    ...
    procedure DoCheckAllChildren(ANode: TdxTreeViewNode; AChecked: Boolean);
  ...
  protected
    ...
    function GetActiveTreeView: TdxTreeViewControl; virtual;
    procedure InitializeControlsTree; virtual;
    procedure InitializeHiddenControlsTree; virtual;
    function IsBoldNode(ANode: TdxTreeViewNode): Boolean; virtual;
    ...
    procedure TreeView_CheckAllChildren(ANode: TdxTreeViewNode; AChecked: Boolean);
    procedure TreeView_EndUpdate;
    procedure TreeView_FreeNodeObjects(ATreeView: TdxTreeViewControl);
    function TreeView_HasCheckedChildren(ANode: TdxTreeViewNode): Boolean;
    function TreeView_HasUncheckedChildren(ANode: TdxTreeViewNode): Boolean;
    function TreeView_GetNodeObject(ANode: TdxTreeViewNode): TdxNodeObject;
    function TreeView_IsNodeEditable(ANode: TdxTreeViewNode): Boolean;
    procedure TreeView_NormalizeNode(ANode: TdxTreeViewNode);
    procedure TreeView_SetNodeState(ANode: TdxTreeViewNode; AState: TcxCheckBoxState);
    procedure TreeView_UpdateNodesState(ANode: TdxTreeViewNode);

    property ActiveTreeView: TdxTreeViewControl read GetActiveTreeView;
...
uses
  SysUtils, Math, dxCore, dxPSGlbl, dxPSUtl, dxPSRes, dxPSPopupMan, dxPSImgs, dxCustomTree;

const
  dxThisUnitName = 'dxPSContainerLnk';

const
  StandardContainerCount = 14;
  StandardContainers: array[0..StandardContainerCount - 1] of TWinControlClass =
    (TPanel, TScrollBox, TTabControl, TTabSheet, TPageControl, TTabPage,
     TTabbedNotebook, TPage, TNotebook, TCustomForm, TCustomFrame, TcxGroupBox, TcxScrollBox, TdxPanel);
...
begin
  ...
  begin
    ...
    if not Assigned(Result) then
      Assert(False, Component.ClassType.ClassName);
...
begin
  Result := TdxReportCellDpiAwareGraphic(AItem).CreateImage(TMetafile);
  try
...
begin
  ...
  TdxReportCellDpiAwareGraphic(AItem).CellSides := [];
  TdxReportCellDpiAwareGraphic(AItem).ImageTransparent := False;
...
function TdxPSWinControlAsMetafileProducer.CreateControlImage(
  AItem: TdxReportVisualItem): TGraphic;
var
...
{ TdxPSdxCustomPanelProducer }

function TdxPSdxCustomPanelProducer.Control: TdxCustomPanel;
begin
  Result := inherited Control as TdxCustomPanel;
end;

class function TdxPSdxCustomPanelProducer.ControlClass: TControlClass;
begin
  Result := TdxCustomPanel;
end;

class function TdxPSdxCustomPanelProducer.HasNativeSupportForBorders: Boolean;
begin
  Result := True;
end;

procedure TdxPSdxCustomPanelProducer.InitializeItem(AnItem: TdxReportVisualItem);
var
  AFrameBorders: TdxCellSides;
  TheItem: TdxReportCellString;
begin
  inherited;
  TheItem := TdxReportCellString(AnItem);
  if Control.Frame.Visible then
  begin
    TheItem.BorderClass := TdxPSCellUltraFlatBorder;
    AFrameBorders := TdxCellSides(Control.Frame.Borders);
    TheItem.CellSides := AFrameBorders;
  end
  else
    TheItem.CellSides := [];
end;

function TdxPSdxCustomPanelProducer.ItemClass: TdxReportVisualItemClass;
begin
  Result := TdxReportCellString;
end;

function TdxPSdxCustomPanelProducer.ObjectExpandHeight: Boolean;
begin
  if Control <> RootContainer then
    Result := True
  else
...
function TdxPSdxCustomPanelProducer.ObjectExpandWidth: Boolean;
begin
  if Control <> RootContainer then
    Result := True
  else
...
var
  AProducer: TdxPSCustomContainerItemProducer;
begin
  FCurrentControl := AControl;
  try
    AProducer := Producers[AControl];
    if AParentHost = nil then
      AParentHost := Host;
    Result := AProducer.ItemClass.Create(AParentHost);
    CreateItemDefinition(AControl, Result);
    AProducer.InitializeItem(Result);
...
begin
  ...
  FDefinitions := TdxPSCustomContainerItemDefinitionList.Create;
  FDelimitersHorz := TList.Create;
  FDelimitersVert := TList.Create;
...
begin
  ...
  begin
    ...
    for I := 0 to FDefinitions.Count - 1 do
      if FDefinitions[I].Component = AComponent then
      begin
        FDefinitions.Delete(I);
        Break;
      end;
  end;
end;
...
function TdxCustomContainerReportLink.SupportsGapsBetweenCells: Boolean;
begin
  Result := True;
end;
...
begin
  ADefinition.Component.FreeNotification(Self);
...
procedure TdxCustomContainerReportLink.AddControl(ATreeView: TdxTreeViewControl; AParent: TdxTreeViewNode; AControl: TControl);
var
  Index, I: Integer;
  Control: TControl;
begin
...
procedure TdxCustomContainerReportLink.AddHiddenControl(ATreeView: TdxTreeViewControl; AParent: TdxTreeViewNode; AControl: TControl);
var
  Index, I: Integer;
  Control: TControl;
begin
...
function TdxCustomContainerReportLink.AddNode(ATreeView: TdxTreeViewControl; AParent: TdxTreeViewNode; AComponent: TComponent; AChecked: Boolean): TdxTreeViewNode;
...
begin
  ...
  Result.CheckState := NodeObject.State;
end;

function TdxCustomContainerReportLink.CreateNodeObject(AComponent: TComponent; AChecked: Boolean): TdxNodeObject;
const
...
procedure TdxCustomContainerReportLink.LoadControlsTree(ATreeView: TdxTreeViewControl);
begin
...
procedure TdxCustomContainerReportLink.LoadHiddenControlsTree(ATreeView: TdxTreeViewControl);
begin
  ATreeView.Items.Clear;
  if Container <> nil then
...
begin
  ...
  dxLoadIconFromResourceEx(imgSize, IDB_DXPSGROUPICON_SIZE);
end;
...
begin
  ...
  Root := tvControls.Root;
  tvControls.Enabled := Root <> nil;
  btnControlsCheckAll.Enabled := (Root <> nil) and (Root.CheckState <> cbsChecked);
  btnControlsExpandAll.Enabled := (Root <> nil) and (Root.Count <> 0);

  Root := tvHiddenControls.Root;
  tvHiddenControls.Enabled := Root <> nil;
  btnHiddenControlsCheckAll.Enabled := (Root <> nil) and (Root.CheckState <> cbsChecked);
  btnHiddenControlsExpandAll.Enabled := (Root <> nil) and (Root.Count <> 0);
  tshHiddenControls.Visible := IsDesigning;
end;

function TdxfmCustomContainerDesignWindow.GetActiveTreeView: TdxTreeViewControl;
begin
...
var
  Root: TdxTreeViewNode;
begin
  tvControls.BeginUpdate;
  try
    FIsTreeViewProcessing := True;
    ...
    FIsTreeViewProcessing := False;
    tvControls.EndUpdate;
  end;
end;
...
begin
  ...
  try
    ...
    FIsTreeViewProcessing := False;
    tvHiddenControls.EndUpdate;
  end;
end;

function TdxfmCustomContainerDesignWindow.IsBoldNode(ANode: TdxTreeViewNode): Boolean;
begin
...
procedure TdxfmCustomContainerDesignWindow.TreeView_CheckAllChildren(ANode: TdxTreeViewNode;
  AChecked: Boolean);

  procedure DoCheckAllChildren(ANode: TdxTreeViewNode);
  const
    StateMap: array[Boolean, Boolean] of TcxCheckBoxState = ((cbsGrayed, cbsChecked), (cbsUnchecked, cbsChecked));
  var
    I: Integer;
...
procedure TdxfmCustomContainerDesignWindow.TreeView_FreeNodeObjects(ATreeView: TdxTreeViewControl);

  procedure FreeNodeObject(ANode: TdxTreeViewNode);
  var
    I: Integer;
  begin
...
function TdxfmCustomContainerDesignWindow.TreeView_GetNodeObject(ANode: TdxTreeViewNode): TdxNodeObject;
begin
  Result := TdxNodeObject(ANode.Data);
end;

function TdxfmCustomContainerDesignWindow.TreeView_HasCheckedChildren(ANode: TdxTreeViewNode): Boolean;
var
  I: Integer;
  Node: TdxTreeViewNode;
begin
  ...
  begin
    Node := ANode[I];
    if TreeView_GetNodeObject(Node).State <> cbsUnchecked then
      Exit;
...
function TdxfmCustomContainerDesignWindow.TreeView_HasUncheckedChildren(ANode: TdxTreeViewNode): Boolean;
var
  I: Integer;
  Node: TdxTreeViewNode;
begin
  ...
  begin
    Node := ANode[I];
    if TreeView_GetNodeObject(Node).State <> cbsChecked then
      Exit;
...
function TdxfmCustomContainerDesignWindow.TreeView_IsNodeEditable(ANode: TdxTreeViewNode): Boolean;
begin
  Result := (ANode <> nil) and ReportLink.IsComponentEditable(TreeView_GetNodeObject(ANode).Component);
end;

procedure TdxfmCustomContainerDesignWindow.TreeView_NormalizeNode(ANode: TdxTreeViewNode);

  procedure DoUpdateNodeState(ANode: TdxTreeViewNode);
  var
    NodeObject: TdxNodeObject;
  begin
    NodeObject := TreeView_GetNodeObject(ANode);
    if Assigned(NodeObject) and (NodeObject.State = cbsChecked) and TreeView_HasUncheckedChildren(ANode) then
      NodeObject.State := cbsGrayed;
  end;
...
procedure TdxfmCustomContainerDesignWindow.TreeView_SetNodeState(ANode: TdxTreeViewNode; AState: TcxCheckBoxState);
var
  NodeObject: TdxNodeObject;
  Component: TComponent;
begin
  NodeObject := TreeView_GetNodeObject(ANode);
  ANode.CheckState := AState;
...
procedure TdxfmCustomContainerDesignWindow.TreeView_UpdateNodesState(ANode: TdxTreeViewNode);
  ...
  begin
    if Assigned(ANode.Data) then
      ...
      begin
        ...
        ANode.CheckState := TreeView_GetNodeObject(ANode).State;
      end;
  end;
...
procedure TdxfmCustomContainerDesignWindow.tvControlsCustomDrawNode(Sender: TdxCustomTreeView; ACanvas: TcxCanvas;
  ANodeViewInfo: TdxTreeViewNodeViewInfo; var AHandled: Boolean);
var
  Node: TdxTreeViewNode;
  ANodeViewInfoAccess: TdxTreeViewNodeViewInfoAccess absolute ANodeViewInfo;
  ARegularFontColor,
  ADisabledFontColor: TColor;
  ...
  begin
    ...
    R := ANodeViewInfoAccess.TextRect;
    ACanvas.Font.Style := ACanvas.Font.Style - [fsBold];
    R.Width := ACanvas.TextWidth(S);
    if Sender.UseRightToLeftAlignment then
      R.X := ANodeViewInfoAccess.TextRect.Left - R.Width
    else
      R.X := ANodeViewInfoAccess.TextRect.Right;
    AFlags := CXTO_SINGLELINE;
    if Sender.UseRightToLeftAlignment then
      AFlags := AFlags or CXTO_RIGHT;
    if TdxTreeViewControl(Sender).OptionsView.ShowEndEllipsis then
      AFlags := AFlags or CXTO_END_ELLIPSIS;
    cxTextOut(ACanvas.Canvas, S, R, AFlags, ACanvas.Font, 0, 0, 0, ADisabledFontColor);
  end;

begin
  Node := ANodeViewInfo.Node;
  if (TreeView_GetNodeObject(Node) = nil) then
    Exit;
  ...
  if IsBoldNode(Node) then
    ...
    ACanvas.Font.Style := ACanvas.Font.Style - [fsBold];
  ARegularFontColor := ANodeViewInfo.LookAndFeelPainter.GetTreeViewNodeTextColor(ANodeViewInfoAccess.GetState);
  ADisabledFontColor := ANodeViewInfo.LookAndFeelPainter.GetTreeViewNodeTextColor(ANodeViewInfoAccess.GetState + [dxtnsDisabled]);

  if not TreeView_IsNodeEditable(Node) then
    ANodeViewInfoAccess.FTextColor := ADisabledFontColor
  else
    ANodeViewInfoAccess.FTextColor := ARegularFontColor;
  TdxTreeViewNodeViewInfoAccess(ANodeViewInfo).AdjustTextRect(ACanvas.Font);
  ANodeViewInfo.DefaultDraw(ACanvas);
  if TBasedxReportLinkAccess(ReportLink).IsDesigning then
    DrawNodeExtraText;
  AHandled := True;
end;

procedure TdxfmCustomContainerDesignWindow.TreeViewControlsEditing(Sender: TdxCustomTreeView; ANode: TdxTreeViewNode; var Allow: Boolean);
begin
  Allow := False;
end;

procedure TdxfmCustomContainerDesignWindow.TreeViewNodeStateChanged(Sender: TdxCustomTreeView; ANode: TdxTreeViewNode);
begin
  if FIsTreeViewProcessing then Exit;
  try
    FIsTreeViewProcessing := True;
    TreeView_GetNodeObject(ANode).State := ANode.CheckState;
    TreeView_CheckAllChildren(ANode, ANode.CheckState = cbsChecked);
    if Assigned(ANode.Parent) and Assigned(ANode.Parent.Data) then
      TreeView_UpdateNodesState(ANode.Parent);
  finally
    FIsTreeViewProcessing := False;
  end;
...
procedure TdxfmCustomContainerDesignWindow.DoCheckAllChildren(ANode: TdxTreeViewNode; AChecked: Boolean);
begin
  if ANode <> nil then
  begin
    ANode.TreeView.BeginUpdate;
    try
      TreeView_CheckAllChildren(ANode, AChecked);
    finally
      ANode.TreeView.EndUpdate;
    end;
...
begin
  ...
  begin
    Pt := Point(X, Y);
    if TdxTreeViewControl(Sender).GetNodeAtPos(Pt, Node) then
    begin
      Node.Focused := True;
      Node.Selected := True;
...
begin
  DoCheckAllChildren(ActiveTreeView.Root, True);
end;
...
procedure TdxfmCustomContainerDesignWindow.TreeViewCustomDrawItem(
  Sender: TCustomTreeView; Node: TdxTreeViewNode; State: TCustomDrawState;
...
begin
  ...
  TdxPSdxCustomPanelProducer.Register;
