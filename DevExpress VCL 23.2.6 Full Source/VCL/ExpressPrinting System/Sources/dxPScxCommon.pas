...
uses
  ...
  dxFormattedLabel, dxFormattedText, dxTextViewInfoCache, dxCoreGraphics;
...
type
  ...
  { TdxPSFormattedTextPainter }

  TdxPSFormattedTextPainter = class(TdxFormattedTextCustomPainter)
  strict private
    FFont: TFont;
    FPSCanvas: TdxPSReportRenderCustomCanvas;
  public
  ...
  { TdxReportCellFormattedLabel }

  TdxReportCellFormattedLabel = class(TdxReportCellPreviewText)
  strict private
  ...
  { TdxPSFormattedLabelDataMap }

  TdxPSFormattedLabelDataMap = class(TdxPSCustomDataMap)
  protected
    class procedure InitializeItem(AnItem: TAbstractdxReportCellData; AProperties: TcxCustomEditProperties;
      const AValue: TcxEditValue; const ACellParams: IdxPSCellParams; var AViewParams: TdxReportItemViewParams;
      AnIsPreview: Boolean = False; ARecordIndex: Integer = 0; AOwner: TObject = nil); override;
  ...
  protected
    ...
    function GetControlActivePalette: IdxColorPalette;
...
function cxTextEditProperties_GetDisplayText(AProperties: TcxCustomTextEditProperties; const AValue: TcxEditValue; AInplace: Boolean): string;
...
uses
  ...
  cxDataUtils, dxPSUtl, dxPSEngn, dxPSImgs, dxExtCtrls, dxDPIAwareUtils, cxEditUtils, StrUtils;

const
  dxThisUnitName = 'dxPScxCommon';
...
{ TextEditProperties Helpers }

function cxTextEditProperties_GetDisplayText(AProperties: TcxCustomTextEditProperties; const AValue: TcxEditValue; AInplace: Boolean): string;
var
  APasswordChar: Char;
begin
  Result := AProperties.GetDisplayText(AValue, AInplace, AInplace);
  if AProperties.EchoMode = eemPassword then
  begin
    APasswordChar := AProperties.PasswordChar;
    if APasswordChar = #0 then
      APasswordChar := cxTextEditDefaultPasswordChar;
    Result := DupeString(APasswordChar, Length(Result));
  end;
end;
...
class function TdxPSCustomDataMap.GetText(AProperties: TcxCustomEditProperties; const AValue: TcxEditValue): string;
begin
  Result := AProperties.GetDisplayText(AValue, True);
end;
  
class function TdxPSCustomDataMap.HasText(AProperties: TcxCustomEditProperties; const AValue: TcxEditValue): Boolean;
begin
...
class function TdxPSTextDataMap.GetText(AProperties: TcxCustomEditProperties; const AValue: TcxEditValue): string;
begin
  Result := cxTextEditProperties_GetDisplayText(TcxCustomTextEditProperties(AProperties), AValue, True);
end;
...
class procedure TdxPSToggleSwitchDataMap.InitializeItem(AnItem: TAbstractdxReportCellData;
  ...
  begin
    ...
    try
      AViewData := TdxCustomToggleSwitchViewData(AProperties.CreateViewData(DefaultEditStyleController.Style, True, dxDefaultScaleFactor));
      try
...
begin
  ...
  begin
    TdxReportCellSparklineImage(AnItem).ViewData := AProperties.CreateViewData(DefaultEditStyleController.Style, True, dxDefaultScaleFactor);
...
begin
  AViewData := AProperties.CreateViewData(DefaultEditStyleController.Style, True, dxDefaultScaleFactor) as TdxTokenEditViewData;
  try
...
constructor TdxPSFormattedTextPainter.Create(ACanvas: TdxPSReportRenderCustomCanvas);
begin
  inherited Create;
  FPSCanvas := ACanvas;
  FFont := TFont.Create;
...
destructor TdxPSFormattedTextPainter.Destroy;
begin
  FreeAndNil(FFont);
  inherited;
end;

procedure TdxPSFormattedTextPainter.ExtTextOut(const ABounds: TRect; AFontHandle: THandle; AFontColor: TColor;
  const AText: string; AGlyphs: PdxWordArray; AGlyphCount: Integer; AGlyphWidths: PdxIntegerArray);
begin
  FFont.Handle := CreateFontIndirect(dxGetFontData(AFontHandle));
  FFont.Color := AFontColor;
  FPSCanvas.ExtTextOut(ABounds, FFont, AText, @AGlyphs[0], AGlyphCount, @AGlyphWidths[0]);
end;

procedure TdxPSFormattedTextPainter.FillRect(const ARect: TRect; AColor: TColor);
begin
  FPSCanvas.FillRect(ARect, AColor);
end;

procedure TdxPSFormattedTextPainter.IntersectClipRect(ARect: TRect);
begin
  FPSCanvas.IntersectClipRgn(ARect);
end;

procedure TdxPSFormattedTextPainter.Restore;
begin
  //do nothing;
end;

procedure TdxPSFormattedTextPainter.Store;
begin
  //do nothing;
end;
...
procedure TdxReportCellFormattedLabel.Assign(Source: TPersistent);
begin
  inherited;
  if Source is TdxReportCellFormattedLabel then
    SingleLine := TdxReportCellFormattedLabel(Source).SingleLine;
end;

procedure TdxReportCellFormattedLabel.DrawText(ACanvas: TdxPSReportRenderCustomCanvas);
var
...
begin
  ...
  try
    AFormattedText.MeasureTrailingSpaces := MeasureTrailingSpaces;
...
function TdxReportCellFormattedLabel.MeasureTextHeight(ACanvas: TdxPSReportRenderCustomCanvas): Integer;
begin
  Result := GetTextSize.cy;
end;

function TdxReportCellFormattedLabel.MeasureTextWidth(ACanvas: TdxPSReportRenderCustomCanvas): Integer;
begin
  Result := GetTextSize.cx;
end;

function TdxReportCellFormattedLabel.MeasureHeight(ACanvas: TdxPSReportRenderCustomCanvas): Integer;
begin
  Result := inherited MeasureHeight(ACanvas) + cxMarginsHeight(Indents.Rect);
end;

function TdxReportCellFormattedLabel.MeasureWidth(ACanvas: TdxPSReportRenderCustomCanvas): Integer;
begin
  Result := inherited MeasureWidth(ACanvas) + cxMarginsWidth(Indents.Rect);
end;

procedure TdxReportCellFormattedLabel.ReadData(AReader: TdxPSDataReader);
begin
  inherited;
  FSingleLine := AReader.ReadBoolean;
end;

procedure TdxReportCellFormattedLabel.WriteData(AWriter: TdxPSDataWriter);
begin
  inherited;
  AWriter.WriteBoolean(FSingleLine);
end;

function TdxReportCellFormattedLabel.GetTextOutTextFlag: Integer;
const
  NoClipToFlag: array[Boolean] of Integer = (0, CXTO_NOCLIP);
  CharbreakToFlag: array[Boolean] of Integer = (0, CXTO_CHARBREAK);
  SingleLineToFlag: array[Boolean] of Integer = (0, CXTO_SINGLELINE);
...
begin
  Result := AlignmentsHorzToFlag[TextAlignX] or AlignmentsVertToFlag[TextAlignY] or EndEllipsysToFlag[EndEllipsis] or
    MultilineToFlag[Multiline] or SingleLineToFlag[SingleLine] or CharbreakToFlag[BreakByChars] or NoClipToFlag[NoClip];
end;

procedure TdxReportCellFormattedLabel.CalculateTextLayout(AFormattedText: TdxFormattedText; var ARect: TRect; AZoomFactor: Single);
begin
...
function TdxReportCellFormattedLabel.GetTextSize: TSize;
var
  AFormattedText: TdxFormattedText;
  ARect: TRect;
begin
...
class procedure TdxPSFormattedLabelDataMap.InitializeItem(AnItem: TAbstractdxReportCellData; AProperties: TcxCustomEditProperties;
  const AValue: TcxEditValue; const ACellParams: IdxPSCellParams; var AViewParams: TdxReportItemViewParams;
  AnIsPreview: Boolean = False; ARecordIndex: Integer = 0; AOwner: TObject = nil);
const
  AlignmentHorzToTextAlignX: array[TcxEditHorzAlignment] of TcxTextAlignX = (taLeft, taRight, taCenterX);
  AlignmentVertToTextAlignY: array[TcxEditVertAlignment] of TcxTextAlignY = (taTop, taBottom, taCenterY);
var
  AFormattedLabelProperties: TdxFormattedLabelProperties;
  AItem: TdxReportCellFormattedLabel;
begin
  if AProperties is TdxFormattedLabelProperties then
  begin
    AFormattedLabelProperties := TdxFormattedLabelProperties(AProperties);
    AItem := AnItem as TdxReportCellFormattedLabel;
    AItem.TextAlignX := AlignmentHorzToTextAlignX[AFormattedLabelProperties.Alignment.Horz];
    AItem.TextAlignY := AlignmentVertToTextAlignY[AFormattedLabelProperties.Alignment.Vert];
    AItem.Multiline := AFormattedLabelProperties.WordWrap;
    AItem.EndEllipsis := AFormattedLabelProperties.ShowEndEllipsis;
    AItem.Indents.Rect := Rect(3, 3, 3, 3);
    AItem.Text := VarToStr(AValue);
  end;
end;

class function TdxPSFormattedLabelDataMap.ItemClass(AProperties: TcxCustomEditProperties; AnIsPreview: Boolean = False): TdxReportCellDataClass;
begin
  Result := TdxReportCellFormattedLabel;
end;

class function TdxPSFormattedLabelDataMap.PropertiesClass: TcxCustomEditPropertiesClass;
begin
...
function TdxCustomcxControlReportLink.GetControlActivePalette: IdxColorPalette;
begin
  Result := nil;
  if (Component is TcxControl) then
    Result := TcxControlAccess(Component).LookAndFeelPainter.ActiveColorPalette;
end;
...
begin
  ...
  TdxPSFormattedLabelDataMap.Register;
end;
...
begin
  ...
  TdxReportCellFormattedLabel.Register;
end;
...
begin
  TdxReportCellFormattedLabel.Unregister;
