...
type
  ...
  public
    ...
    function GetCaptionAreaContentHeight: Integer; override;
...
uses
  ...
  cxGeometry, dxOffice11, Math, dxCore;

const
  dxThisUnitName = 'dxDockControlXPView';
...
function TdxDockControlXPPainter.GetCaptionAreaContentHeight: Integer;
begin
  Result := Max(inherited GetCaptionAreaContentHeight, TdxTextMeasurer.TextLineHeight(GetFont));
end;
...
var
  ATheme: TdxTheme;
begin
  ATheme := OpenTheme(totToolBar, ScaleFactor);
  if ATheme <> 0 then
  begin
  ...
  begin
    ...
    begin
      ...
      try
        with ARect do
        begin
          ACanvas.ExcludeClipRect(Rect(Left, Top, Left + 2, Top + 2));
          ACanvas.ExcludeClipRect(Rect(Right - 2, Top, Right, Top + 2));
          ACanvas.ExcludeClipRect(Rect(Right - 2, Bottom - 2, Right, Bottom));
          ACanvas.ExcludeClipRect(Rect(Left, Bottom - 2, Left + 2, Bottom));
        end;
        dxDrawThemeBackground(ACanvas.Handle, totExplorerBar, ScaleFactor, ARect, EBP_NORMALGROUPBACKGROUND, 0);
      finally
    ...
    dxDrawThemeBackground(ACanvas.Handle, totButton, ScaleFactor, ARect, BP_GROUPBOX, 0);
  end
  else
    inherited;
end;
...
begin
  if not dxDrawThemeBackground(ACanvas.Handle, totToolBar, ScaleFactor, ARect, BP_PUSHBUTTON, BtnStateToXPBtnState(AState)) then
...
begin
  if not dxDrawThemeBackground(ACanvas.Handle, totTab, ScaleFactor, ARect, TABP_BODY, 0) then
    inherited;
end;
...
begin
  if not dxDrawThemeBackground(ACanvas.Handle, totWindow, ScaleFactor, ARect, WP_SMALLCLOSEBUTTON, StateMap[AState]) then
...
begin
  ...
  begin
    ...
    try
      ...
      try
        cxStretchBlt(ARotatedBitmap.Canvas.Handle, ABitmap.Canvas.Handle, ARotatedBitmap.ClientRect, ABitmap.ClientRect, SRCCOPY);
...
begin
  ATheme := OpenTheme(totWindow, ScaleFactor);
  if ATheme <> 0 then
  begin
    if IsSwitched then
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        TdxImageDrawer.DrawImage(ACanvas, R, nil, DockControl.Images, DockControl.ImageIndex, True, GetCaptionColorPalette, ScaleFactor);
...
begin
  if not dxDrawThemeBackground(ACanvas.Handle, totSpin, ScaleFactor, ARect,
    ArrowButtonPartIdMap[AArrow], ArrowButtonStateIdMap[AArrow, AState])
  then
