...
type
  ...
  TdxDockingControllerVS2005Helper = class;
  ...
  strict private
    ...
    FHelper: TdxDockingControllerVS2005Helper;
  ...
  protected
    function AddZoneViewInfo(AKind: TdxDockingTargetSelectorZoneKind; const ABounds: TRect): TdxDockingTargetSelectorZoneViewInfo;
  ...
  public
    constructor Create(AHelper: TdxDockingControllerVS2005Helper); reintroduce; virtual;
    destructor Destroy; override;
...
const
  dxThisUnitName = 'dxDockStyleVS2005';
...
constructor TdxDockingCustomTargetSelector.Create(AHelper: TdxDockingControllerVS2005Helper);
begin
  inherited Create;
  FHelper := AHelper;
...
function TdxDockingCustomTargetSelector.AddZoneViewInfo(
  AKind: TdxDockingTargetSelectorZoneKind; const ABounds: TRect): TdxDockingTargetSelectorZoneViewInfo;
  ...
  function CanDocking(AZone: TdxZone; ADockControl: TdxCustomDockControl): Boolean;
  begin
    Result := (AZone <> nil) and AZone.CanDock(ADockControl) and
      FHelper.CanDocking(ADockControl, AZone.Owner, AZone, cxRectCenter(ABounds));
  end;
...
begin
  ...
  if not CanDocking(AZone, dxDockingController.DockingDockControl) then
    AZone := nil;
...
begin
  ...
  AddZoneViewInfo(tszClient, cxRectCenter(ARect, TargetZoneSize[tszClient]));
end;
...
begin
  ...
  AZoneViewInfo := AddZoneViewInfo(tszLeft, PlaceZone(ARect, TargetZoneSize[tszLeft]));
  ARect.Left := AZoneViewInfo.Bounds.Right + CalculateVerticalRowMaxSize;

  AddZoneViewInfo(tszRight, PlaceZone(ARect, TargetZoneSize[tszRight]));
end;
...
begin
  ...
  AZoneViewInfo := AddZoneViewInfo(tszTop, PlaceZone(ARect, TargetZoneSize[tszTop]));
  ARect.Top := AZoneViewInfo.Bounds.Bottom + CalculateHorizontalRowMaxSize;

  AddZoneViewInfo(tszBottom, PlaceZone(ARect, TargetZoneSize[tszBottom]));
end;
...
var
  AZoneViewInfo: TdxDockingTargetSelectorZoneViewInfo;
begin
  AZoneViewInfo := AddZoneViewInfo(ZoneKind, cxRect(TargetZoneSize[ZoneKind]));
...
begin
  Result := TargetSelectorsClasses[AType].Create(Self);
end;
...
function TdxDockingControllerVS2005Helper.GetTargetSelector(AType: TdxDockingType): TdxDockingCustomTargetSelector;
begin
