...
uses
  ...
  cxLibraryConsts, dxHooks, dxAnimation, cxButtonEdit, cxEdit, dxMessages, cxCustomCanvas, Generics.Collections;
...
type
  ...
  strict private
    function GetItem(AIndex: TdxListIndex): TdxTileControlCustomCellViewInfo; inline;
  public
    function CalculateHitTest(AHitTest: TdxTileControlHitTest): Boolean; virtual;
    procedure Draw(ACanvas: TcxCustomCanvas); virtual;
    procedure DrawWithoutClipping(ACanvas: TcxCustomCanvas);
    procedure FlushCache;
    procedure Offset(dX, dY: Integer);
    procedure RefreshState; virtual;

    property Items[AIndex: TdxListIndex]: TdxTileControlCustomCellViewInfo read GetItem; default; // for internal use
  end;
  ...
  public
    procedure Draw(ACanvas: TcxCustomCanvas); virtual; abstract;
  ...
  protected
    procedure DrawContent(ACanvas: TcxCustomCanvas; const R: TRect); virtual;
  ...
  private
    FCacheContent: TcxCanvasBasedImage;
    FCacheText: TcxCanvasBasedImage;
    ...
    function GetItemHotTrackMode: TdxTileControlItemHotTrackMode;
  ...
  protected
    ...
    procedure DoDrawContent(ACanvas: TcxCustomCanvas; const R: TRect); virtual;
    procedure DrawChecked(ACanvas: TcxCustomCanvas); virtual;
    procedure DrawContent(ACanvas: TcxCustomCanvas; const R: TRect); override;
    procedure DrawFocusRect(ACanvas: TcxCustomCanvas); virtual;
    procedure DrawFrameRect(ACanvas: TcxCustomCanvas; AColor: TColor; AIsOuter: Boolean; AColorAlpha: Byte = 255); virtual;
    procedure DrawHighlightRect(ACanvas: TcxCustomCanvas); overload;
    procedure DrawHighlightRect(ACanvas: TcxCustomCanvas; const R: TRect; const AHighlightColor: TdxAlphaColor); overload; virtual;
    procedure DrawHottrackRect(ACanvas: TcxCustomCanvas); virtual;
    procedure DrawSelection(ACanvas: TcxCustomCanvas); virtual;
    ...
    property ItemHotTrackMode: TdxTileControlItemHotTrackMode read GetItemHotTrackMode;
  ...
  public
    ...
    property Painter: TdxTileControlPainter read GetPainter; // for internal use
    property Position: TdxLayoutItemPosition read FPosition write FPosition;
    property Selected: Boolean read GetSelected;
    property ViewData: TdxTileControlCustomItemViewData read GetViewData write SetViewData; // for internal use
  end;
  ...
  private
    ...
    function GetTileViewInfo: TdxTileControlViewInfo;
  ...
  protected
    ...
    property TileViewInfo: TdxTileControlViewInfo read GetTileViewInfo;
  public
    procedure Draw(ACanvas: TcxCustomCanvas); override;
  ...
  protected
    ...
    procedure DoCalculate; override;
    procedure DoCalculateFont; virtual;
    procedure DoCalculateGlyphBounds; virtual;
    procedure DoCalculateTextBounds; virtual;
    procedure DoDraw(ACanvas: TcxCustomCanvas); override;
    procedure DoRightToLeftConversion(const AClientBounds: TRect); override;
    function GetActualCanvas: TcxCustomCanvas; virtual;
    function GetHeight: Integer; override;
    function GetHitTest(AHitTest: TdxTileControlHitTest): Boolean; override;
    ...
    property ActualCanvas: TcxCustomCanvas read GetActualCanvas;
    property TileControl: TdxCustomTileControl read GetTileControl;
  ...
  public
    constructor Create(ATitle: TdxTileControlTitle);
    destructor Destroy; override;
  ...
  { IdxTileControlGalleryImage }

  IdxTileControlGalleryImage = interface
  ['{30F2D0B4-274F-4699-AA28-8ED97FBF6116}']
    procedure Draw(ACanvas: TcxCustomCanvas; const ATargetRect, ASourceRect: TRect);
  end;

  { TdxTileControlGalleryImage }

  TdxTileControlGalleryImage = class(TInterfacedObject, IdxTileControlGalleryImage)
  strict private
    FCache: TcxCanvasBasedImage;
    FImage: TdxGpFastDIB;
  public
    constructor Create(AImage: TdxGpFastDIB);
    destructor Destroy; override;
    // IdxTileControlGalleryImage
    procedure Draw(ACanvas: TcxCustomCanvas; const ATargetRect, ASourceRect: TRect);
  end;

  { TdxTileControlGalleryCellImage }

  TdxTileControlGalleryCellImage = class
  strict private
    FImage: IdxTileControlGalleryImage;
    FSourceRect: TRect;
  public
    constructor Create(AImage: IdxTileControlGalleryImage; const ASourceRect: TRect);
    procedure Draw(ACanvas: TcxCustomCanvas; const R: TRect);
  end;
  ...
  strict private
    ...
    FCurrentImage: TdxTileControlGalleryCellImage;
    FItem: TdxTileControlItem;
    FTileViewInfo: TdxTileControlViewInfo;

    function GetMiddle: Integer; inline;
  public
    constructor Create(AItem: TdxTileControlItem; ACell: TdxTileControlItemGalleryCell); reintroduce; virtual;
    destructor Destroy; override;
    procedure Draw(ACanvas: TcxCustomCanvas; const R: TRect);
    procedure RefreshAnimatedObject; override;
    ...
    property Item: TdxTileControlItem read FItem;
    property Middle: Integer read GetMiddle;
  end;
  ...
  protected
    ...
    ClipRect2: TRect;
    ColumnAfter: Integer;
    ColumnBefore: Integer;
    GroupCaption: TdxTileControlGroupCaption;
    GroupOriginAfter: TPoint;
    GroupOriginBefore: TPoint;
  ...
  public
    ...
    procedure Draw(ACanvas: TcxCustomCanvas; const AProgress: Integer);
  ...
  protected
    ...
    procedure DrawItems(ACanvas: TcxCustomCanvas); virtual;
  ...
  public
    ...
    property ImmediateFinish: Boolean read FImmediateFinish write FImmediateFinish;
  end;
  ...
  private
    function GetItem(Index: TdxListIndex): TdxTileControlItem;
  protected
    procedure SortByIndexInGroupBeforeDrag;
  public
    property Items[Index: TdxListIndex]: TdxTileControlItem read GetItem; default;
  end;
  ...
  protected
    ...
    procedure UpdatePosition(AWinPosInfo: THandle);
  ...
  public
    ...
    function ReleaseImage: TdxGpFastDIB;
    procedure Scale;

    property CallBackProc: TNotifyEvent read FCallBackProc;
    property Image: TdxGpFastDIB read FImage;
  ...
  protected
    ...
    procedure GroupDestroyed(AGroup: TdxTileControlGroup);
    ...
    property ItemsContentAnimationLockCount: Integer read FItemsContentAnimationLockCount;
  ...
  public
    ...
    property PressedScrollButton: TdxTileControlScrollButtonViewInfo read FPressedScrollButton write SetPressedScrollButton; // for internal use
  end;
  ...
  private
    ...
    function GetTileControl: TdxCustomTileControl; inline;
  ...
  protected
    ...
    function IsImageListUsed: Boolean;
  ...
  strict private
    FClientRect: TRect;
    FContentLayer: TdxFastDIB;
    FImageBounds: TRect;
    FIsDirty: Boolean;
    FItem: TdxTileControlCustomItem;
    FTextBounds: array[0..3] of TRect;
    FTextLayer: TdxFastDIB;

    function GetBounds: TRect;
    function GetContentLayer: TdxFastDIB; inline;
    function GetGalleryCellController: TdxTileControlGalleryCellController;
    function GetIsActive: Boolean;
    function GetPainter: TdxTileControlPainter;
    function GetTextBounds(AIndex: Integer): TRect;
    function GetTextLayer: TdxFastDIB; inline;
  ...
  protected
    ...
    procedure DrawItemBackground(ACanvas: TcxCustomCanvas); virtual;
    procedure DrawItemGlyph(ACanvas: TcxCustomCanvas); virtual;
    procedure DrawItemText(ACanvas: TcxCustomCanvas; const ATextBounds, AClientRect: TRect; AText: TdxTileControlItemText); virtual;
    procedure DrawItemTexts(ACanvas: TcxCustomCanvas; const AOrigin: TPoint; const AClipRect: TRect); virtual;
    procedure RebuildLayers; virtual;
  ...
  public
    ...
    property ContentLayer: TdxFastDIB read GetContentLayer;
    ...
    property TextLayer: TdxFastDIB read GetTextLayer;
  ...
  protected
    ...
    function DrawTextOnImage: Boolean; virtual;
  ...
  strict private
    ...
    FCurrentImage: TdxTileControlGalleryCellImage;
    FNextImage: TdxTileControlGalleryCellImage;
    FRow: Byte;
    FTileItem: TdxTileControlItem;

    function GetAnimation: TdxTileControlGalleryCellAnimation;
    procedure SetNextImage(AValue: TdxTileControlGalleryCellImage);
  protected
    procedure DoDraw(ACanvas: TcxCustomCanvas); override;
    ...
    function JumpToNextFrame: TdxTileControlGalleryCellImage;
  ...
  public
    constructor Create(ATileItem: TdxTileControlItem;
      const AController: TdxTileControlGalleryCellController;
      const ARow, AColumn: Byte; const ABounds: TRect); virtual;
    destructor Destroy; override;

    property Column: Byte read FColumn;
    property CurrentImage: TdxTileControlGalleryCellImage read FCurrentImage;
    property NextImage: TdxTileControlGalleryCellImage read FNextImage write SetNextImage;
  ...
  strict private
    ...
    function GetItem(AIndex: TdxListIndex): TdxTileControlItemGalleryCell; inline;
  public
    function CalculateHitTest(AHitTest: TdxTileControlHitTest): Boolean; override;

    property Items[Index: TdxListIndex]: TdxTileControlItemGalleryCell read GetItem; default;
  ...
  public
    ...
    procedure Draw(ACanvas: TcxCustomCanvas; const AOrigin: TPoint); virtual;
  ...
  protected
    ...
    function DrawTextOnImage: Boolean; override;
    ...
    function IsRealMostBottom: Boolean;
    function IsRealMostLeft: Boolean;
    function IsRealMostRight: Boolean;
    function IsRealMostTop: Boolean;
  ...
  public
    ...
    property TileControl: TdxCustomTileControl read GetTileControl;
    property ViewInfo: TdxTileControlItemViewInfo read FViewInfo; // for internal use
  published
  ...
  protected
    procedure CalculateBounds; virtual;
    procedure CalculateHeight; virtual;
    procedure DoCalculate; override;
    procedure DoCalculateFont; virtual;
    procedure DoDraw(ACanvas: TcxCustomCanvas); override;
    procedure DrawContent(ACanvas: TcxCustomCanvas; const R: TRect); override;
  ...
  strict private
    function GetItem(AIndex: Integer): TdxTileControlGroup; inline;
    procedure SetItem(AIndex: Integer; const AValue: TdxTileControlGroup); inline;
  protected
    procedure ChangeScale(M, D: Integer); override;
  ...
  public
    ...
    procedure UpdateEditorPositions;
  ...
  private
    ...
    procedure DrawDesignTimeBorder(ACanvas: TcxCustomCanvas);
    function GetContentBounds: TRect;
    function GetTitleHeight: Integer;
    function GetTitleViewInfo: TdxTileControlTitleViewInfo; inline;
  ...
  protected
    ...
    function CalculateFrameSelectionClipRect(const R: TRect; AFrameSize: Integer): TRect;
    procedure DoCalculate; virtual;
    ...
    procedure GroupDestroyed(AGroup: TdxTileControlGroup);
    procedure HideScrollButtons;
    function IsContentCenteredHorz: Boolean;
    function IsContentCenteredVert: Boolean;
    function IsContentNotCenteredHorz: Boolean;
    function IsContentNotCenteredVert: Boolean;
    function IsScrollAvailable: Boolean; overload;
    function IsScrollAvailable(ADirection: TcxArrowDirection): Boolean; overload;
    procedure ItemDestroyed(AItem: TdxTileControlItem);
    procedure MakeVisible(const ABounds: TRect; AIsMostLeft, AIsMostTop, AIsMostRight, AIsMostBottom: Boolean);
  ...
  public
    ...
    procedure Clear;
    procedure Draw(ACanvas: TcxCustomCanvas); override;
    procedure DrawBackground(ACanvas: TcxCustomCanvas);
    procedure FlushCache; override;
    procedure RefreshState; override;

    property Cells: TdxTileControlCells read FCells write FCells; // for internal use
    ...
    property TitleViewInfo: TdxTileControlTitleViewInfo read GetTitleViewInfo; // for internal use
  ...
  strict private
    FBackgroundCache: TcxCanvasBasedImage;
    ...
    function GetTextOutFlags(AAlignment: TAlignment; AVertAlignment: TcxAlignmentVert; AWordWrap: Boolean): Integer;
    function GetUserSkins: Boolean;
  protected
    procedure DrawDefaultItemBackground(ACanvas: TcxCustomCanvas; const R: TRect); virtual;
    procedure DrawDefaultItemCheck(ACanvas: TcxCustomCanvas; const R: TRect); overload; virtual;
    procedure DrawDefaultItemCheck(ACanvas: TcxCustomCanvas; const R: TRect;
      ABackgroundColor, ACheckMarkColor: TColor; AAlpha: Byte = MaxByte); overload; virtual;
    procedure DrawDefaultItemCheck(ACanvas: TdxGPCanvas; const R: TRect;
      ABackgroundColor, ACheckMarkColor: TdxAlphaColor; AScaleFactor: TdxScaleFactor); overload; virtual;
    procedure DrawHighlightRect(ACanvas: TcxCustomCanvas; const R: TRect;
      AHighlightColor: TdxAlphaColor; ABackgroundColor: TColor = clNone); virtual;
    procedure DrawItemCheck(ACanvas: TcxCustomCanvas; const R: TRect); virtual;
    procedure DrawStyleRect(ACanvas: TcxCustomCanvas; const R: TRect; AStyle: TdxTileControlCustomStyle;
      ATextureOffsetX, ATextureOffsetY: Integer; ATextureCache: PcxCanvasBasedImage = nil);
    procedure DrawStyleRectFrame(ACanvas: TcxCustomCanvas; const R: TRect; AStyle: TdxTileControlCustomStyle);
    procedure DrawStyleTexture(ACanvas: TcxCustomCanvas; R: TRect; AStyle: TdxTileControlCustomStyle;
      ATextureOffsetX, ATextureOffsetY: Integer; ATextureCache: PcxCanvasBasedImage = nil);
  ...
  public
    ...
    procedure FlushCache;
    //
    procedure DrawBackground(ACanvas: TcxCustomCanvas; const R: TRect); virtual;
    procedure DrawDesignTimeBorder(ACanvas: TcxCustomCanvas; const R: TRect; ASelected: Boolean); virtual;
    procedure DrawGlyph(ACanvas: TcxCustomCanvas; const R: TRect; AGlyph: TdxSmartImage); overload; virtual;
    procedure DrawGlyph(ACanvas: TcxCustomCanvas; const R: TRect; AGlyph: TdxSmartImage;
      AImages: TCustomImageList; AImageIndex: TcxImageIndex; ADrawMode: TcxImageDrawMode = idmNormal); overload; virtual;
    procedure DrawGlyph(ACanvas: TcxCustomCanvas; const R: TRect; AState: TcxButtonState;
      AGlyph: TdxSkinImage; AGlyphType: TImageType; AColor: TColor; AIsRTL: Boolean = False); overload; virtual;
    procedure DrawItemBackground(ACanvas: TcxCustomCanvas; const R: TRect; AStyle: TdxTileControlCustomStyle); virtual;
    procedure DrawItemPlace(ACanvas: TcxCustomCanvas; const R: TRect); virtual;
    procedure DrawPageTab(ACanvas: TcxCustomCanvas; const R: TRect; AState: TcxButtonState); virtual;
    procedure DrawText(ACanvas: TcxCustomCanvas; const R: TRect; const AText: string; AFont: TFont; AColor: TColor;
      AAlignment: TAlignment = taLeftJustify; AVertAlignment: TcxAlignmentVert = vaCenter;
      AWordWrap: Boolean = False; APaintOnGlass: Boolean = False); virtual;
    procedure DrawVirtualGroup(ACanvas: TcxCustomCanvas; const R: TRect); virtual;
    //
    function MeasureTextSize(ACanvas: TcxCustomCanvas; const AText: string; AFont: TFont;
      AAlignment: TAlignment; AWordWrap: Boolean; AMaxWidth, AMaxHeight: Integer): TSize; virtual;
    //
    ...
    property SkinInfo: TdxSkinInfo read FSkinInfo;  // for internal use
    property TileControl: TdxCustomTileControl read FTileControl;
  ...
  public
    ...
    property State: TcxButtonState read FState write SetState;
    property ViewInfo: TdxTileControlViewInfo read GetViewInfo; // for internal use
  end;
  ...
  protected
    ...
    procedure DoCalculate; override;
    procedure DoDraw(ACanvas: TcxCustomCanvas); override;
    procedure DoDrawScrollTabsButtons(ACanvas: TcxCustomCanvas); virtual;
    function GetActualCanvas: TcxCustomCanvas; override;
    function GetHeight: Integer; override;
    function GetHitTest(AHitTest: TdxTileControlHitTest): Boolean; override;
  ...
  protected
    ...
    function GetTextureSize: TSize; virtual;
    procedure DoCalculate; override;
    procedure DoRightToLeftConversion(const AClientBounds: TRect); override;
  ...
  protected
    ...
    function GetTexture: TdxSkinImage; override;
    function GetTextureSize: TSize; override;
  ...
  TdxTileControlChange = (tccLayout, tccSelection, tccItems, tccTitle, tccStyle);
  ...
  private
    ...
    FOnGroupRemoveWithLastItem: TdxTileControlGroupRemoveWithLastItemEvent;
  ...
  protected
    ...
    procedure AfterActiveDetailChanging(AActiveDetail, ANewDetail: TdxTileControlDetailSite); virtual;
    procedure BeforeActiveDetailChanging(AActiveDetail, ANewDetail: TdxTileControlDetailSite); virtual;
    ...
    function CanRemoveGroupIfEmpty(AGroup: TdxTileControlGroup): Boolean; virtual;
    ...
    procedure DoCustomPaint(ACanvas: TcxCustomCanvas); virtual;
    procedure DoPaint; override;
    ...
    property OnGroupRemoveWithLastItem: TdxTileControlGroupRemoveWithLastItemEvent read FOnGroupRemoveWithLastItem write FOnGroupRemoveWithLastItem;
  ...
  public
    ...
    property Controller: TdxTileControlController read FController;
    property DragAndDropObject; // doc'd    //# it is used in sample for T970822
...
uses
  dxOffice11, RTLConsts, Variants, cxTextEdit, dxDPIAwareUtils, dxTypeHelpers;

const
  dxThisUnitName = 'dxCustomTileControl';
...
type
  ...
  private
    function GetItem(AIndex: TdxListIndex): TdxDetailControlInfo;
  public
    ...
    property Items[AIndex: TdxListIndex]: TdxDetailControlInfo read GetItem;
  end;
...
function TdxDetailControlsInfo.GetItem(AIndex: TdxListIndex): TdxDetailControlInfo;
begin
...
constructor TdxTileControlGalleryImage.Create(AImage: TdxGpFastDIB);
begin
  FImage := AImage;
end;

destructor TdxTileControlGalleryImage.Destroy;
begin
  FreeAndNil(FCache);
  FreeAndNil(FImage);
  inherited;
...
procedure TdxTileControlGalleryImage.Draw(ACanvas: TcxCustomCanvas; const ATargetRect, ASourceRect: TRect);
begin
  ACanvas.DrawBitmap(FImage, ATargetRect, ASourceRect, afPremultiplied, @FCache);
end;

{ TdxTileControlGalleryCellImage }

constructor TdxTileControlGalleryCellImage.Create(AImage: IdxTileControlGalleryImage; const ASourceRect: TRect);
begin
  FImage := AImage;
  FSourceRect := ASourceRect;
end;

procedure TdxTileControlGalleryCellImage.Draw(ACanvas: TcxCustomCanvas; const R: TRect);
begin
  FImage.Draw(ACanvas, R, FSourceRect);
end;
...
constructor TdxTileControlItemGalleryCell.Create(ATileItem: TdxTileControlItem;
  const AController: TdxTileControlGalleryCellController;
  const ARow, AColumn: Byte; const ABounds: TRect);
begin
  inherited Create;
  FRow := ARow;
  FColumn := AColumn;
...
procedure TdxTileControlItemGalleryCell.DoDraw(ACanvas: TcxCustomCanvas);
begin
  if IsAnimating then
    Animation.Draw(ACanvas, Bounds)
  else
  begin
    ACanvas.FillRect(Bounds, GetEmptyCellBasicColor);
    if CurrentImage <> nil then
      CurrentImage.Draw(ACanvas, Bounds);
  end;
end;
...
begin
  Result := cxGetActualColor(TileItem.Style.GradientBeginColor, dxTileItemShadowColor);
...
function TdxTileControlItemGalleryCell.IsAnimating: Boolean;
begin
  Result := (Animation <> nil) and (Animation.Cell = Self);
end;

function TdxTileControlItemGalleryCell.JumpToNextFrame: TdxTileControlGalleryCellImage;
begin
  Result := FCurrentImage;
  FCurrentImage := FNextImage;
  FNextImage := nil;
end;

procedure TdxTileControlItemGalleryCell.SetNextImage(AValue: TdxTileControlGalleryCellImage);
begin
  if FNextImage <> AValue then
  begin
    FreeAndNil(FNextImage);
    FNextImage := AValue;
  end;
...
procedure TdxTileControlGroupCaptionViewInfo.CalculateHeight;
var
  AText: string;
  ATextHeight: Integer;
begin
  ...
  AText := Caption.Text;
  if TileControl.OptionsBehavior.GroupRenaming and (AText = '') then
    AText := 'I';
  ATextHeight := Painter.MeasureTextSize(TileControl.ActualCanvas, AText, Font, Caption.Alignment, False, 0, 0).cy;
...
procedure TdxTileControlGroupCaptionViewInfo.DoCalculateFont;
begin
...
procedure TdxTileControlGroupCaptionViewInfo.DoDraw(ACanvas: TcxCustomCanvas);
begin
  if Group.Visible and (Group <> TileControl.DraggedGroup) and not IsAnimatedOnDragDrop then
    DrawContent(ACanvas, TextBounds);
end;

procedure TdxTileControlGroupCaptionViewInfo.DrawContent(ACanvas: TcxCustomCanvas; const R: TRect);
begin
  if Caption.Color <> clDefault then
    ACanvas.FillRect(R, Caption.Color);
  if ACanvas.RectVisible(R) and (Caption.Text <> '') then
    Painter.DrawText(ACanvas, R, Caption.Text, Font, TextColor, Caption.Alignment, vaBottom);
end;
...
begin
  ...
  if not Group.Enabled then
    Result := dxGetLighterColor(Result, 50);
end;
...
begin
  ...
  FColor := clDefault;
  FFont := dxCreateFontForDefaultDPI(dxTileControlDefaultGroupCaptionFontSize);
...
procedure TdxTileControlItemCollection.ForEach(AProc: TdxTileControlForEachItemProc; const AData: Pointer; AVisibleOnly: Boolean = True);
var
  I: Integer;
begin
  for I := 0 to Count - 1 do
  begin
    if not AVisibleOnly or Items[I].ActuallyVisible then
      AProc(Items[I], AData);
  end;
end;
...
begin
  if AnsiUpperCase(AClassName) = AnsiUpperCase(ItemClass.ClassName) then
  begin
...
function TdxTileControlGalleryCells.GetItem(AIndex: TdxListIndex): TdxTileControlItemGalleryCell;
begin
  Result := inherited Items[AIndex] as TdxTileControlItemGalleryCell;
end;
...
begin
  FPreparedCell := PreparedCells[Random(PreparedCells.Count)];
end;

procedure TdxTileControlGalleryCellController.Draw(ACanvas: TcxCustomCanvas; const AOrigin: TPoint);
var
  APrevImageStretchQuality: TcxCanvasImageStretchQuality;
begin
  APrevImageStretchQuality := ACanvas.ImageStretchQuality;
  try
    ACanvas.ImageStretchQuality := isqStandard;
    if cxPointIsNull(AOrigin) then
      GalleryCells.DrawWithoutClipping(ACanvas)
    else
    begin
      GalleryCells.Offset(AOrigin.X, AOrigin.Y);
      GalleryCells.DrawWithoutClipping(ACanvas);
      GalleryCells.Offset(-AOrigin.X, -AOrigin.Y);
    end;
  finally
    ACanvas.ImageStretchQuality := APrevImageStretchQuality;
  end;
end;
...
var
  ACell: TdxTileControlItemGalleryCell;
  AImage: IdxTileControlGalleryImage;
  AOrigin: TPoint;
  ATask: TdxThreadScalerTaskItem absolute Sender;
  I: Integer;
begin
  ATask.ImageItem.RemoveFreeNotification(TileItem);
  AImage := TdxTileControlGalleryImage.Create(ATask.ReleaseImage);

  AOrigin := PreparedCells[0].Bounds.TopLeft;
  for I := 0 to PreparedCells.Count - 1 do
  begin
    ACell := PreparedCells[I];
    ACell.NextImage := TdxTileControlGalleryCellImage.Create(AImage, cxRectOffset(ACell.Bounds, AOrigin, False));
  end;
...
begin
  if IsEmptyImage(FImageIndex) then
  begin
    FPreparedCell := PreparedCells[0];
    FPreparedCell.NextImage := nil;
  end
  else
  begin
...
begin
  ...
  inherited Create(
    SourceViewData.ContentLayer, DestViewData.ContentLayer,
    dxTileControlFramesAnimationTime, Item.AnimationMode,
    ateAccelerateDecelerate, -1, False, False);
...
begin
  FCell := ACell;
  FItem := AItem;
  FCurrentImage := ACell.JumpToNextFrame;
  FTileViewInfo := AItem.TileControl.ViewInfo;
...
procedure TdxTileControlGalleryCellAnimation.Draw(ACanvas: TcxCustomCanvas; const R: TRect);
var
  AActualHeight: Integer;
  AHeight: Integer;
  AImage: TdxTileControlGalleryCellImage;
  AMiddle: Integer;
  ARect: TRect;
begin
  AMiddle := Middle;
  AHeight := cxRectHeight(R);
  AActualHeight := MulDiv(AHeight, Abs(AMiddle - Position), AMiddle);
  ARect := cxRectSetHeight(cxRectOffsetVert(R, (AHeight - AActualHeight) div 2), AActualHeight);
  ACanvas.FillRect(ARect, Cell.GetEmptyCellBasicColor);

  if Position < AMiddle then
    AImage := FCurrentImage
  else
    AImage := Cell.CurrentImage;

  if AImage <> nil then
    AImage.Draw(ACanvas, R);
end;
...
function TdxTileControlGalleryCellAnimation.GetMiddle: Integer;
begin
  Result := Length div 2;
end;
...
procedure TdxTileControlGroupCollection.UpdateEditorPositions;
var
  AEditorCount: Integer;
...
begin
  ...
  for I := 0 to Count - 1 do
  begin
    if Items[I].IsCaptionEditorPresent then
      Inc(AEditorCount);
  end;

  if AEditorCount > 0 then
  begin
    AHandle := BeginDeferWindowPos(AEditorCount);
    for I := 0 to Count - 1 do
    begin
      AGroup := Items[I];
      if AGroup.IsCaptionEditorPresent then
        AGroup.CaptionEditor.UpdatePosition(AHandle);
    end;
...
procedure TdxTileControlGroupCollection.SetItem(AIndex: Integer; const AValue: TdxTileControlGroup);
begin
  inherited SetItem(AIndex, AValue);
end;

procedure TdxTileControlGroupCollection.Update(
...
begin
  RemoveItems;
  TileControl.Controller.GroupDestroyed(Self);
  FreeAndNil(FCaption);
  FreeAndNil(FItems);
...
begin
  if (AVirtualGroup = VirtualGroupAfter) and
      not (((GroupLayout = glHorizontal) and IsRealMostRight) or ((GroupLayout = glVertical) and IsRealMostBottom))then
    Result := Indent
  else
    Result := 0;
end;
...
function TdxTileControlGroup.IsRealMostBottom: Boolean;
var
  I, ABottom: Integer;
begin
  ...
  begin
    Result := True;
    ABottom := ViewInfo.Bounds.Bottom;
    for I := 0 to TileControl.Groups.Count - 1 do
      if TileControl.Groups[I].Visible then
      begin
        Result := TileControl.Groups[I].ViewInfo.Bounds.Bottom <= ABottom;
        if not Result then
          Exit;
      end;
  end;
...
function TdxTileControlGroup.IsRealMostLeft: Boolean;
begin
  if GroupLayout = glVertical then
    Result := True
  else
...
function TdxTileControlGroup.IsRealMostRight: Boolean;
var
  I, AOutside: Integer;
begin
  ...
  begin
    Result := True;
    if not TileControl.UseRightToLeftAlignment then
      AOutside := ViewInfo.Bounds.Right
    else
      AOutside := ViewInfo.Bounds.Left;
    for I := 0 to TileControl.Groups.Count - 1 do
      if TileControl.Groups[I].Visible then
      begin
        if not TileControl.UseRightToLeftAlignment then
          Result := TileControl.Groups[I].ViewInfo.Bounds.Right <= AOutside
        else
          Result := TileControl.Groups[I].ViewInfo.Bounds.Left >= AOutside;
        if not Result then
          Exit;
      end;
  end;
...
function TdxTileControlGroup.IsRealMostTop: Boolean;
begin
  if GroupLayout = glHorizontal then
    Result := True
  else
...
begin
  ...
  TileControl.ViewInfo.MakeVisible(ViewInfo.Bounds, IsRealMostLeft, IsRealMostTop, IsRealMostRight, IsRealMostBottom);
  Invalidate;
end;
...
begin
  ...
  ATileControl := TileControl;
  ATileControl.BeginUpdate;
  try
    if FItems.Remove(AItem) >= 0 then
    begin
      AItem.FGroup := nil;
      if not((csDestroying in ComponentState) or ATileControl.IsDesigning or ATileControl.IsDestroying or ATileControl.IsRestoring) and
         not TileControl.IsItemDragged and ATileControl.CanRemoveGroupIfEmpty(Self) then
        ATileControl.RemoveGroupIfEmpty(Self);
      if not FIsDeleted then
        GroupChanged;
    end;
  finally
...
begin
  ...
  ViewInfo.Painter.DrawBackground(DragImage.Canvas, cxRectUnion(TileControl.ClientRect, DragCell.Bounds));
...
procedure TdxTileControlAnimatedDragAndDropCustomCellViewInfo.DrawContent(ACanvas: TcxCustomCanvas; const R: TRect);
begin
  // do nothing
end;
...
begin
  FreeAndNil(FPosition);
  FreeAndNil(FCacheContent);
  FreeAndNil(FCacheText);
  inherited Destroy;
end;
...
procedure TdxTileControlItemViewInfo.DoDraw(ACanvas: TcxCustomCanvas);

  function CanDrawItemCheckOnPullGesture(out AAlpha: Byte): Boolean;
  var
    AItem: TdxTileControlDragDropItem;
  begin
    AItem := TileControl.DragAndDropObject as TdxTileControlDragDropItem;
    Result := AItem.State = tcddsPulling;
    if Result then
    begin
      if AItem.IsPullDownGesture then
        AAlpha := MaxByte
      else
        AAlpha := MulDiv(150, AItem.PullProgress, 100);
    end;
  end;
...
var
  AAlpha: Byte;
begin
  if (Group = nil) or Group.IsDragged or not ACanvas.RectVisible(GetActualVisualBounds) then
    Exit;
  ...
  try
    ...
    begin
      ...
      if CanDrawItemCheckOnPullGesture(AAlpha) then
        Painter.DrawDefaultItemCheck(ACanvas, Bounds, clNone, Painter.GroupCaptionDefaultTextColor, AAlpha);
      Exit;
    end;

    if IsAnimatedOnDragDrop then
      Exit;
    ...
    if Item.CanFocused and Hottracked and (ItemHotTrackMode = tcihtmHighlight) then
      DrawHighlightRect(ACanvas);
  finally
  ...
  begin
    ...
    if Hottracked and (ItemHotTrackMode <> tcihtmHighlight) then
      DrawHottrackRect(ACanvas);
  end;
end;

procedure TdxTileControlItemViewInfo.DoDrawContent(ACanvas: TcxCustomCanvas; const R: TRect);
begin
  if ContentAnimation is TdxTileControlFramesAnimation then
    TdxTileControlFramesAnimation(ContentAnimation).Draw(ACanvas, R)
  else
    if Item.CanGalleryAnimation then
      GalleryCellController.Draw(ACanvas, R.TopLeft)
    else
      ACanvas.DrawBitmap(ViewData.ContentLayer, R, afIgnored, @FCacheContent);

  if ViewData.TextLayer <> nil then
    ACanvas.DrawBitmap(ViewData.TextLayer, R, afPremultiplied, @FCacheText);
end;
...
procedure TdxTileControlItemViewInfo.DrawChecked(ACanvas: TcxCustomCanvas);
begin
  Painter.DrawItemCheck(ACanvas, Bounds);
end;

procedure TdxTileControlItemViewInfo.DrawContent(ACanvas: TcxCustomCanvas; const R: TRect);
var
  AMatrix: TXForm;
  APrevImageStretchQuality: TcxCanvasImageStretchQuality;
begin
  ...
  if cxSizeIsEqual(R, Bounds) then
    DoDrawContent(ACanvas, R)
  else
  begin
    ...
    try
      APrevImageStretchQuality := ACanvas.ImageStretchQuality;
      try
        ACanvas.ImageStretchQuality := isqHigh;
        AMatrix := TXForm.CreateScaleMatrix(cxRectWidth(R) / cxRectWidth(Bounds), cxRectHeight(R) / cxRectHeight(Bounds));
        AMatrix.eDx := R.Left - Bounds.Left * AMatrix.eM11;
        AMatrix.eDy := R.Top - Bounds.Top * AMatrix.eM22;
        ACanvas.ModifyWorldTransform(AMatrix);
        DoDrawContent(ACanvas, Bounds);
      finally
        ACanvas.ImageStretchQuality := APrevImageStretchQuality;
      end;
    finally
  ...
  if not Item.Style.IsEmpty and (Item.ItemType <> tcitGallery) then
    Painter.DrawStyleRectFrame(ACanvas, R, Item.Style);
  if not Item.IsEnabled then
    DrawHighlightRect(ACanvas, R, dxTileItemDisabledMaskColor);
end;

procedure TdxTileControlItemViewInfo.DrawFocusRect(ACanvas: TcxCustomCanvas);
begin
  DrawFrameRect(ACanvas, Painter.GetSelectionFocusedColor(Item), TileControl.OptionsBehavior.ItemFocusMode = tcifmOuterFrame);
end;

procedure TdxTileControlItemViewInfo.DrawFrameRect(ACanvas: TcxCustomCanvas; AColor: TColor; AIsOuter: Boolean; AColorAlpha: Byte = 255);
var
  ABounds: TRect;
  AFrameSize: Integer;
begin
  ACanvas.SaveClipRegion;
  try
    ABounds := GetActualVisualBounds;
    AFrameSize := GetActualFocusFrameSize;
    ACanvas.IntersectClipRect(TileControl.ViewInfo.CalculateFrameSelectionClipRect(ABounds, AFrameSize));
    if AIsOuter then
      AFrameSize := ScaleFactor.Apply(2);
    ACanvas.FrameRect(ABounds, dxColorToAlphaColor(AColor, AColorAlpha), AFrameSize);
  finally
    ACanvas.RestoreClipRegion;
  end;
end;

procedure TdxTileControlItemViewInfo.DrawHighlightRect(ACanvas: TcxCustomCanvas);
begin
  DrawHighlightRect(ACanvas, GetHighlightBounds(Bounds), TileControl.OptionsBehavior.ItemHotTrackHighlightColor);
end;

procedure TdxTileControlItemViewInfo.DrawHighlightRect(ACanvas: TcxCustomCanvas; const R: TRect; const AHighlightColor: TdxAlphaColor);
begin
  TileControl.Painter.DrawHighlightRect(ACanvas, R, AHighlightColor, Item.Style.GradientBeginColor);
end;

procedure TdxTileControlItemViewInfo.DrawHottrackRect(ACanvas: TcxCustomCanvas);
begin
...
procedure TdxTileControlItemViewInfo.DrawSelection(ACanvas: TcxCustomCanvas);
var
  A160B487D707F6C1D211F191136191B0473681B5D7762736F: TRect;
begin
  A160B487D707F6C1D211F191136191B0473681B5D7762736F := cxRectInflate(Bounds, InflateDelta);
  ACanvas.FillRect(A160B487D707F6C1D211F191136191B0473681B5D7762736F, Painter.SelectionSelectedColor, 127);
  ACanvas.Rectangle(A160B487D707F6C1D211F191136191B0473681B5D7762736F, clNone, clRed, psDashDot);
end;

function TdxTileControlItemViewInfo.GetActualFocusFrameSize: Integer;
begin
  Result := 0;
  if Item = Controller.HottrackedItem then
    case TileControl.OptionsBehavior.ItemHotTrackMode of
      tcihtmOuterFrame:
  ...
  if Focused and (TileControl.OptionsBehavior.ItemFocusMode = tcifmOuterFrame) then
...
begin
  Result := cxRectInflate(Bounds, GetActualFocusFrameSize);
end;

function TdxTileControlItemViewInfo.GetDragItemPlaceBounds: TRect;
begin
  Result := cxRectInflate(Bounds, -dxTileControlInflateAnimationStepsCount);
end;

function TdxTileControlItemViewInfo.GetBaseDrawBounds: TRect;
begin
  Result := cxRectInflate(inherited GetBaseDrawBounds, InflateDelta);
end;
...
procedure TdxTileControlItemViewInfo.Draw(ACanvas: TcxCustomCanvas);
begin
  if Visible then
    DoDraw(ACanvas);
end;

procedure TdxTileControlItemViewInfo.FlushCache;
begin
  inherited;
  FreeAndNil(FCacheContent);
  FreeAndNil(FCacheText);
...
function TdxTileControlItemViewInfo.GetItemHotTrackMode: TdxTileControlItemHotTrackMode;
begin
  Result := TileControl.OptionsBehavior.ItemHotTrackMode;
end;
...
function TdxTileControlCells.GetItem(AIndex: TdxListIndex): TdxTileControlCustomCellViewInfo;
begin
  Result := TdxTileControlCustomCellViewInfo(inherited Items[AIndex]);
end;

procedure TdxTileControlCells.Draw(ACanvas: TcxCustomCanvas);
var
  I: Integer;
begin
  for I := 0 to Count - 1 do
...
procedure TdxTileControlCells.FlushCache;
var
  I: Integer;
begin
  for I := 0 to Count - 1 do
...
procedure TdxTileControlCells.Offset(dX, dY: Integer);
var
  I: Integer;
begin
  for I := 0 to Count - 1 do
...
procedure TdxTileControlCells.DrawWithoutClipping(ACanvas: TcxCustomCanvas);
var
  AItem: TdxTileControlCustomCellViewInfo;
  I: Integer;
begin
  for I := 0 to Count - 1 do
  begin
    AItem := Items[I];
    if AItem.Clipped then
    begin
      AItem.FClipped := False;
      AItem.DoDraw(ACanvas);
      AItem.FClipped := True;
    end
    else
      AItem.DoDraw(ACanvas);
...
begin
  ...
  begin
    AItemHalfIndent := TileViewInfo.ItemHalfIndent;
...
begin
  ...
  begin
    ...
    repeat
      ...
      if (ARow > 0) and (AItem.ViewInfo.GetLastOccupiedRow(ARow) >= 2 * TileViewInfo.RowCount) then
      begin
        ARow := 0;
        ASmallColumnCountInLogicalRow := SmallColumnCountInLogicalRow;
...
procedure TdxTileControlGroupViewInfo.DoDraw(ACanvas: TcxCustomCanvas);
begin
  if not cxRectIsNull(FNewGroupDrawingBounds) then
    ...
    begin
      if (Group.Collection <> nil) and ((TileViewInfo.DragDropChanges = nil) or not TileViewInfo.DragDropChanges.DragObjectFinished) then
...
begin
  ABounds.Bottom := Max(ABounds.Bottom, TileViewInfo.ClientBounds.Bottom);
end;

procedure TdxTileControlGroupViewInfo.ExpandBoundsToLeftControl(var ABounds: TRect);
begin
  ABounds.Left := Min(ABounds.Left, TileViewInfo.ClientBounds.Left);
end;

procedure TdxTileControlGroupViewInfo.ExpandBoundsToRightControl(var ABounds: TRect);
begin
  ABounds.Right := Max(ABounds.Right, TileViewInfo.ClientBounds.Right);
end;

procedure TdxTileControlGroupViewInfo.ExpandBoundsToTopControl(var ABounds: TRect);
begin
  ABounds.Top := Min(ABounds.Top, TileViewInfo.ClientBounds.Top);
end;
...
procedure TdxTileControlGroupViewInfo.Draw(ACanvas: TcxCustomCanvas);
begin
  inherited;

  if TileControl.IsDesigning and not Group.IsDragged and cxRectIsNull(FNewGroupDrawingBounds) then
  begin
    ACanvas.SaveClipRegion;
    try
      ACanvas.IntersectClipRect(TileViewInfo.CalculateFrameSelectionClipRect(Bounds, GetBorderWidth));
      Painter.DrawDesignTimeBorder(ACanvas, Bounds, Selected);
    finally
      ACanvas.RestoreClipRegion;
    end;
  end;
...
begin
  ...
  begin      // left and top can't any more to change!
    ...
    Result.Right := Max(ABounds.Right, Result.Right);
    Result.Bottom := Max(ABounds.Bottom, Result.Bottom);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      Result := TileViewInfo.TilesArea.Left - TileViewInfo.LeftScrollPos - GetBorderWidth;
      if not TileControl.OptionsView.FixedIndentHorz then
        Inc(Result, TileControl.OptionsView.IndentHorz);
      if TileViewInfo.IsFixedContentLeftSide then
        Result := Max(Result, TileViewInfo.VisibleGroupsOrigin.X);
    end;
  end;
end;
...
begin
  ...
  if GroupLayout = glHorizontal then
    Result := Result * TileViewInfo.RowCount
  else
    if RowCount > 0 then
...
begin
  ...
  if Odd(ColumnCount) then
    Inc(Result, TileViewInfo.ItemSmallWidth + AItemIndent);
end;

function TdxTileControlGroupViewInfo.GetTileViewInfo: TdxTileControlViewInfo;
begin
  Result := TileControl.ViewInfo;
end;
...
begin
  ...
  if GroupLayout = glHorizontal then
  begin
    Result := TileViewInfo.TilesArea.Top - TileViewInfo.TopScrollPos - GetBorderWidth;
    if not TileControl.OptionsView.FixedIndentVert then
      Inc(Result, TileControl.OptionsView.IndentVert);
    if TileViewInfo.IsFixedContentTopSide then
      Result := Max(Result, TileViewInfo.VisibleGroupsOrigin.Y);
  end
  else
  begin
    APrevGroup := Group.Groups.GetPrevVisibleGroup(Group);
...
constructor TdxTileControlTitleViewInfo.Create(ATitle: TdxTileControlTitle);
begin
  inherited Create;
  FTitle := ATitle;
  FFont := TFont.Create;
...
procedure TdxTileControlTitleViewInfo.DoCalculateFont;
begin
...
begin
  ...
  begin
    FGlyphBounds := Rect(Bounds.Right - Title.Glyph.Width, Bounds.Bottom - Title.Glyph.Height, Bounds.Right, Bounds.Bottom);
...
begin
  ...
  if Title.Text <> '' then
  begin
    FTextBounds := MeasureTextBounds(FTextBounds);
    FTextBounds := cxRectSetTop(FTextBounds, Bounds.Top + (cxRectHeight(Bounds) - cxRectHeight(FTextBounds)) div 2);
  end;
end;

procedure TdxTileControlTitleViewInfo.DoDraw(ACanvas: TcxCustomCanvas);
begin
  if Title.Color <> clDefault then
    ACanvas.FillRect(Bounds, Title.Color);
  if Title.Text <> '' then
    Painter.DrawText(ACanvas, FTextBounds, Title.Text, Font, TextColor, Title.TextAlignHorz, Title.TextAlignVert, Title.WordWrap);
  if not Title.Glyph.Empty then
    Painter.DrawGlyph(ACanvas, GlyphBounds, Title.Glyph);
end;

function TdxTileControlTitleViewInfo.GetActualCanvas: TcxCustomCanvas;
begin
  Result := TileControl.ActualCanvas;
end;

function TdxTileControlTitleViewInfo.GetHeight: Integer;
begin
  if Title.Glyph.Empty then
    Result := inherited GetHeight
  else
    Result := Title.Glyph.Height + 2 * Title.IndentVert;

  if Title.Text <> '' then
    Result := Max(Result, cxRectHeight(MeasureTextBounds(cxNullRect)) + 2 * Title.IndentVert);
end;
...
begin
  if Title.Text <> '' then
  begin
    Result := cxRectSetSize(AVisibleBounds, Painter.MeasureTextSize(ActualCanvas, Title.Text,
      Font, Title.TextAlignHorz, Title.WordWrap, IfThen(Title.WordWrap, cxRectWidth(AVisibleBounds)), 0));
  end
  else
    Result := cxRectSetHeight(AVisibleBounds, 0);
end;
...
begin
  Result := TitleViewInfo.GetHitTest(AHitTest) or Cells.CalculateHitTest(AHitTest);
end;
...
begin
  if AGroup.Visible and
    (((GroupLayout = glHorizontal) and AGroup.IsRealMostLeft) or ((GroupLayout = glVertical) and AGroup.IsRealMostTop))then
  begin
...
begin
  ...
  begin
    ...
    TitleViewInfo.DoRightToLeftConversion(ClientBounds);
  end;
end;
...
function TdxTileControlViewInfo.CalculateFrameSelectionClipRect(const R: TRect; AFrameSize: Integer): TRect;
begin
  Result := R;
  if R.Left + AFrameSize < TilesArea.Left then
    Result.Left := TilesArea.Left;
  if (R.Right - AFrameSize - 1 >= TilesArea.Right) or TileControl.IsScrollBarActive(sbVertical) and (R.Right >= TileControl.GetVScrollBarBounds.Left - 1) then
    Result.Right := TilesArea.Right;
  if R.Top + AFrameSize < TilesArea.Top then
    Result.Top := TilesArea.Top;
  if (R.Bottom - AFrameSize - 1 >= TilesArea.Bottom) or TileControl.IsScrollBarActive(sbHorizontal) and (R.Bottom >= TileControl.GetHScrollBarBounds.Top - 1) then
    Result.Bottom := TilesArea.Bottom;
end;
...
begin
  ...
  if IsContentNotCenteredHorz or (VirtualGroups.Count = 0) or (FContentRight > AAreaRight) then Exit;
...
begin
  ...
  if IsContentNotCenteredVert or (VirtualGroups.Count = 0) or (FContentBottom > AControlBottom) then Exit;
...
begin
  if TitleViewInfo.Visible then
    Result := TitleViewInfo.Bounds.Bottom
  else
    Result := 0;
end;

function TdxTileControlViewInfo.GetTitleViewInfo: TdxTileControlTitleViewInfo;
begin
...
procedure TdxTileControlViewInfo.GroupDestroyed(AGroup: TdxTileControlGroup);
begin
  Cells.Remove(AGroup.ViewInfo);
  Cells.Remove(AGroup.VirtualGroupBefore);
  Cells.Remove(AGroup.VirtualGroupAfter);
  Cells.Remove(AGroup.Caption.ViewInfo);
end;
...
procedure TdxTileControlViewInfo.ItemDestroyed(AItem: TdxTileControlItem);
begin
  Cells.Remove(AItem.ViewInfo);
end;

procedure TdxTileControlViewInfo.MakeVisible(
  const ABounds: TRect; AIsMostLeft, AIsMostTop, AIsMostRight, AIsMostBottom: Boolean);
var
  ALeftPos, ATopPos, AGap: Integer;
begin
  AGap := ItemHalfIndent + TileControl.GetBorderSize * 2;

  ALeftPos := LeftScrollPos;
  if AIsMostLeft then
    ALeftPos := 0
  else
    if AIsMostRight then
      ALeftPos := FContentWidth - HorzScrollPage
    else
      if not TileControl.UseRightToLeftAlignment then
      begin
        if ABounds.Left < TilesArea.Left then
          Dec(ALeftPos, TilesArea.Left - ABounds.Left + AGap)
        else
          if ABounds.Right > TilesArea.Right then
            Inc(ALeftPos, ABounds.Right - TilesArea.Right + AGap);
      end
      else
      begin
        if ABounds.Left < TilesArea.Left then
          Inc(ALeftPos, TilesArea.Left - ABounds.Left + AGap)
        else
          if ABounds.Right > TilesArea.Right then
            Dec(ALeftPos, ABounds.Right - TilesArea.Right + AGap);
      end;

  ATopPos := TopScrollPos;
  if AIsMostTop then
    ATopPos := 0
  else
    if AIsMostBottom then
      ATopPos := FContentHeight - VertScrollPage
    else
      if ABounds.Top < TilesArea.Top then
        Dec(ATopPos, TilesArea.Top - ABounds.Top + AGap)
      else
        if ABounds.Bottom > TilesArea.Bottom then
          Inc(ATopPos, ABounds.Bottom - TilesArea.Bottom + AGap);

  if (LeftScrollPos <> ALeftPos) or (TopScrollPos <> ATopPos) then
    Scroll(ALeftPos - LeftScrollPos, ATopPos - TopScrollPos);
end;
...
begin
  ...
  begin
    ...
    if AGroup.Visible then
    begin
      AGroup.Caption.ViewInfo.CalculateHeight;
      AMaxHeight := Max(AMaxHeight, AGroup.Caption.Height);
    end;
  end;

  if (AMaxHeight > 0) and (GroupLayout = glHorizontal) then
    for I := 0 to Groups.Count - 1 do
    begin
      AGroup := Groups[I];
      if AGroup.Visible then
        AGroup.Caption.Height := AMaxHeight;
    end;
end;
...
procedure TdxTileControlViewInfo.Draw(ACanvas: TcxCustomCanvas);
begin
  ...
  Groups.UpdateEditorPositions;
end;

procedure TdxTileControlViewInfo.DrawBackground(ACanvas: TcxCustomCanvas);
begin
  Painter.DrawBackground(ACanvas, TileControl.ClientRect);
end;

procedure TdxTileControlViewInfo.FlushCache;
begin
  TitleViewInfo.FlushCache;
  Cells.FlushCache;
end;

procedure TdxTileControlViewInfo.DrawDesignTimeBorder(ACanvas: TcxCustomCanvas);
begin
  Painter.DrawDesignTimeBorder(ACanvas, Bounds, TileControl.Controller.IsObjectSelected(TileControl));
end;
...
function TdxTileControlViewInfo.IsContentCenteredHorz: Boolean;
begin
  Result := TileControl.OptionsView.CenterContentHorz and
    (ContentWidth <= HorzScrollPage) and (VirtualGroups.Count > 0);
end;

function TdxTileControlViewInfo.IsContentCenteredVert: Boolean;
begin
  Result := TileControl.OptionsView.CenterContentVert and
    (ContentHeight <= VertScrollPage) and (VirtualGroups.Count > 0);
end;

function TdxTileControlViewInfo.IsContentNotCenteredHorz: Boolean;
begin
  Result := not IsContentCenteredHorz or IsFixedContentLeftSide;
end;

function TdxTileControlViewInfo.IsContentNotCenteredVert: Boolean;
begin
  Result := not IsContentCenteredVert or IsFixedContentTopSide;
end;
...
begin
  inherited DoPaint;
  TileControl.Painter.DrawBackground(ActualCanvas, ClientBounds);
  TitleViewInfo.Draw(ActualCanvas);
end;
...
function TdxTileControlCheckedItems.GetItem(Index: TdxListIndex): TdxTileControlItem;
begin
...
begin
  inherited Create(AOwner);
  FFont := dxCreateFontForDefaultDPI(dxTileControlDefaultTitleFontSize);
  FFont.Color := clDefault;
  FFont.Name := DefaultFontName;
  FFont.OnChange := FontChanged;
...
begin
  Result := TdxTileControlTitleViewInfo.Create(Self);
end;
...
procedure TdxTileControlActionBars.ImagesChanged(Sender: TObject);
begin
  Changed;
end;

procedure TdxTileControlActionBars.ImagesFreeNotification(AComponent: TComponent);
begin
  Images := nil;
end;
...
procedure TdxTileControlActionBars.SetImages(AValue: TCustomImageList);
begin
...
begin
  inherited Create(AOwner.TileControl.Title);
  FOwner := AOwner;
...
var
  ABounds: TRect;
begin
  ABounds := cxRect(BackButtonViewInfo.MeasureSize);
...
procedure TdxTileControlDetailSiteTitleViewInfo.CalculateTabs;
  ...
  begin
    ...
    begin
      ACell := TdxTileControlPageTabCellViewInfo(Tabs.List[I]);
      ACellSize := ACell.MeasureSize;
      ABounds := cxRectSetBottom(R, R.Bottom, Min(cxRectHeight(R), ACellSize.cy));
      ABounds := cxRectSetWidth(ABounds, AOffset, ACellSize.cx);
...
begin
  ...
  begin
    ...
    FTabsBounds := cxRectSetXPos(Bounds,
      ...
      Min(GlyphBounds.Left, Bounds.Right - TileControl.OptionsView.IndentHorz));
...
procedure TdxTileControlDetailSiteTitleViewInfo.DoDraw(ACanvas: TcxCustomCanvas);
begin
  ACanvas.SaveClipRegion;
  ...
  begin
    ACanvas.SaveClipRegion;
    try
      ACanvas.IntersectClipRect(TabsBounds);
      Tabs.Draw(ACanvas);
    finally
      ACanvas.RestoreClipRegion;
    end;
...
procedure TdxTileControlDetailSiteTitleViewInfo.DoDrawScrollTabsButtons(ACanvas: TcxCustomCanvas);
begin
...
function TdxTileControlDetailSiteTitleViewInfo.GetActualCanvas: TcxCustomCanvas;
begin
  Result := FOwner.ActualCanvas;
end;

function TdxTileControlDetailSiteTitleViewInfo.GetHeight: Integer;
begin
  Result := Max(inherited GetHeight, BackButtonViewInfo.MeasureSize.cy + 2 * Title.IndentVert);
end;
...
begin
  ...
  Result.Bottom := Max(Result.Bottom, Result.Top + BackButtonViewInfo.MeasureSize.cy + Title.IndentVert * 2);
end;
...
procedure TdxTileControlCustomActionBarViewInfo.CalculateItems(ABounds: TRect);
  ...
  procedure CheckBoundaryValues(const AGlyphBounds: TRect; var ATop, ABottom: Integer);
  begin
    ATop := Min(ATop, AGlyphBounds.Top);
    ABottom := Max(ABottom, AGlyphBounds.Bottom);
  end;
...
var
  ...
  AItemWidth: Integer;
  I, ATop, ABottom, AHeight: Integer;
begin
  ...
  ABottom := MinInt;
  for I := 0 to Cells.Count - 1 do
    CheckBoundaryValues(TdxTileControlActionBarCustomItemViewInfo(Cells.Items[I]).FGlyphBounds, ATop, ABottom);
  ...
  begin
    ...
    begin
      ...
      AItemWidth := AItemViewInfo.MeasureSize.cx;
      ...
      CheckBoundaryValues(AItemViewInfo.FGlyphBounds, ATop, ABottom);
  ...
  for I := 0 to Cells.Count - 1 do
  begin
    AItemViewInfo := TdxTileControlActionBarCustomItemViewInfo(Cells.Items[I]);
    if (AItemViewInfo.FGlyphBounds.Top <> ATop) or (AItemViewInfo.FGlyphBounds.Bottom <> ABottom) then
    begin
      AItemViewInfo.FGlyphBounds.Top := ATop;
      AItemViewInfo.FGlyphBounds.Bottom := ABottom;
      AHeight := cxRectHeight(AItemViewInfo.FGlyphRect);
      AItemViewInfo.FGlyphRect.Top := cxRectCenter(AItemViewInfo.FGlyphBounds).Y - AHeight div 2;
      AItemViewInfo.FGlyphRect.Bottom := AItemViewInfo.FGlyphRect.Top + AHeight;
    end;
  end;
end;
...
procedure TdxTileControlCustomActionBarViewInfo.Draw(ACanvas: TcxCustomCanvas);
begin
  DrawBackground(ACanvas);
  Cells.Draw(ACanvas);
end;

procedure TdxTileControlCustomActionBarViewInfo.DrawBackground(ACanvas: TcxCustomCanvas);
begin
  ACanvas.FillRect(Bounds, cxGetActualColor(ActionBars.Color, Painter.ActionBarDefaultBackgroundColor));
end;
...
begin
  Result := 0;
  for I := 0 to Cells.Count - 1 do
    Result := Max(Result, TdxTileControlCustomButtonViewInfo(Cells[I]).MeasureSize.cy);
  if Result > 0 then
...
begin
  ...
  begin
    ...
    ImageIndex := ABarItem.ImageIndex;
...
function TdxTileControlActionBarItem.GetImages: TCustomImageList;
begin
  if Collection.ParentComponent is TdxCustomTileControl then
    Result := TdxCustomTileControl(Collection.ParentComponent).ActionBars.Images
  else
    Result := nil;
end;

function TdxTileControlActionBarItem.IsImageListUsed: Boolean;
var
...
procedure TdxTileControlActionBarItem.SetImageIndex(AValue: TcxImageIndex);
begin
...
function TdxTileControlCustomButtonViewInfo.MeasureSize: TSize;
begin
  Result := ScaleFactor.Apply(Texture.Size);
end;
...
procedure TdxTileControlDetailSiteBackButtonViewInfo.Draw(ACanvas: TcxCustomCanvas);
begin
  Owner.TileControl.Painter.DrawGlyph(ACanvas, Bounds, State, Texture, TextureType,
    Owner.TitleViewInfo.TextColor, Owner.TileControl.ViewInfo.UseRightToLeftAlignment);
end;
...
procedure TdxTileControlScrollButtonViewInfo.DoDraw(ACanvas: TcxCustomCanvas);
begin
...
procedure TdxTileControlDetailSiteTabsScrollButtonViewInfo.DoDraw(ACanvas: TcxCustomCanvas);
begin
...
procedure TdxCustomTileControl.BeforeDestruction;
begin
...
procedure TdxCustomTileControl.AfterActiveDetailChanging(AActiveDetail, ANewDetail: TdxTileControlDetailSite);
begin
  ...
  begin
    ...
    if cxCopyDirectCanvasContentToGdiCanvas(ActualCanvas, Canvas) then
...
procedure TdxCustomTileControl.BeforeActiveDetailChanging(AActiveDetail, ANewDetail: TdxTileControlDetailSite);
  ...
  function CreateImage(AValue: TdxTileControlDetailSite): TdxFastDIB;
  var
    AArea: TRect;
  begin
    AArea := ViewInfo.DetailSiteArea;
    Result := TdxFastDIB.Create(AArea, True);

    cxPaintCanvas.BeginPaint(Result.DC);
    try
      if AValue <> nil then
        cxPaintTo(AValue, cxPaintCanvas, cxNullPoint, Rect(0, 0, AValue.Width, AValue.Height))
      else
      begin
        cxPaintCanvas.WindowOrg := AArea.TopLeft;
        ViewInfo.Draw(cxPaintCanvas);
        cxPaintCanvas.WindowOrg := cxNullPoint;
      end;
    finally
...
var
  ...
  AFinishImage: TdxFastDIB;
  AStartImage: TdxFastDIB;
begin
  ...
  if not ViewInfo.DetailSiteArea.IsEmpty then
  begin
    ...
    FAnimation := TdxImageAnimationTransition.Create(AStartImage, AFinishImage, AActualInterval, GetAnimationMode, ateLinear, -1, True, False);
...
procedure TdxCustomTileControl.InitScrollBarsParameters;
  ...
  procedure InitScrollBarParameters(AKind: TScrollBarKind; ASize, APage, APosition: Integer);
  var
    AMin: Integer;
  begin
    if Controller.IsHandScrolling(AKind) then
      AMin := Min(0, APosition)
    else
      AMin := 0;

    if (ActiveDetail <> nil) and not DetailsAnimationInProcess then
    begin
      ASize := 0;
      AMin := 0;
    end;

    SetScrollBarInfo(AKind, AMin, ASize + AMin, GetScrollStep, APage, APosition, OptionsBehavior.ScrollMode = smScrollbars, True);
  end;

begin
  if ([csLoading, csDestroying, csUpdating, csReading] * ComponentState = []) and (ViewInfo <> nil) then
  begin
    InitScrollBarParameters(sbHorizontal, ViewInfo.ContentWidth - 1, ViewInfo.HorzScrollPage, ViewInfo.LeftScrollPos);
    InitScrollBarParameters(sbVertical, ViewInfo.ContentHeight - 1, ViewInfo.VertScrollPage, ViewInfo.TopScrollPos);
  end;
end;
...
procedure TdxCustomTileControl.DoCustomPaint(ACanvas: TcxCustomCanvas);
begin
  if Animation <> nil then
    Animation.Draw(ACanvas, ViewInfo.DetailSiteArea)
  else
    if ActiveDetail = nil then
...
begin
  ADetail.BorderStyle := BorderStyle;
  ADetail.Align := alClient;
  ADetail.BringToFront;
end;

procedure TdxCustomTileControl.CheckTitleChanges;
begin
  ViewInfo.TitleViewInfo.FlushCache;
end;
...
begin
  if not IsLocked and (Changes <> []) then
    ValidateChanges;
end;
...
function TdxCustomTileControl.CanRemoveGroupIfEmpty(AGroup: TdxTileControlGroup): Boolean;
begin
  Result := (AGroup <> nil) and (AGroup.ItemCount = 0);
  if Result then
  begin
    Result := FDeleteItemInProcess;
    if Assigned(OnGroupRemoveWithLastItem) then
      OnGroupRemoveWithLastItem(Self, AGroup, Result);
  end;
end;
...
procedure TdxCustomTileControl.FlushCache;
var
...
procedure TdxCustomTileControl.CreateCanvasBasedResources;
begin
  inherited CreateCanvasBasedResources;
  FController.StartAnimations;
end;

procedure TdxCustomTileControl.FreeCanvasBasedResources;
begin
  FController.StopAnimations;
  FlushCache;
...
procedure TdxCustomTileControl.LookAndFeelChanged(Sender: TcxLookAndFeel; AChangedValues: TcxLookAndFeelValues);
begin
  inherited LookAndFeelChanged(Sender, AChangedValues);
  AddChanges([tccItems, tccStyle]);
  Painter.ValidatePainterData;
  if ActiveDetail <> nil then
    ActiveDetail.Invalidate;
  Invalidate;
...
function TdxCustomTileControl.IsMouseWheelHandleNeeded(Shift: TShiftState; WheelDelta: Integer; MousePos: TPoint): Boolean;
begin
  Result := True;
end;
...
begin
  ...
  begin
    if AComponent = FImages then
      Images := nil;
    if not IsDestroying and (AComponent = ActiveDetail) then
    begin
      FActiveDetail := nil;
      Repaint;
    end;
...
begin
  ...
  begin
    ...
    BeforeActiveDetailChanging(FActiveDetail, AValue);
    try
      if ActiveDetail <> nil then
      begin
        SendMessage(Handle, WM_SETREDRAW, 0, 0);
      ...
      AfterActiveDetailChanging(FActiveDetail, AValue);
    end;
    if APrevItem <> nil then
...
begin
  ...
  FBackButton.Stretch := smNoResize;
  ...
  FCustomButton.Stretch := smNoResize;
end;
...
begin
  ...
  begin
    ...
    ABackButtonBounds := cxRectSetWidth(ABackButtonBounds, ABackButtonViewInfo.MeasureSize.cx);
...
begin
  ...
  FItemsContentAnimationLockCount := 2;
...
var
  ...
  AOldItem: TdxTileControlItem;
begin
  if (FHottrackedItem = AValue) or TileControl.IsDesigning then
    Exit;
  if (AValue <> nil) and not AValue.IsGroupEnabled then
    AValue := nil;
  if AValue = FHottrackedItem then
    Exit;
...
begin
  if FDesignPopupMenuCaller is TdxTileControlGroup then
    Result := TdxTileControlGroup(FDesignPopupMenuCaller)
  else
...
begin
  case TdxTileControlItemSize(AItemSizeIndex) of
    tcisSmall:
...
begin
  if (AnimationItems.IndexOf(AItem) < 0) and (AItem.CanGalleryAnimation or (AItem.ViewInfo.ViewData <> AItem.ActiveFrame.ViewData)) then
  begin
...
procedure TdxTileControlController.GroupDestroyed(AGroup: TdxTileControlGroup);
begin
  ViewInfo.GroupDestroyed(AGroup);
end;
...
begin
  ViewInfo.Calculate;
  cxCopyDirectCanvasContentToGdiCanvas(TileControl.ActualCanvas, TileControl.Canvas);
  for I := 0 to TileControl.Groups.Count - 1 do
  begin
    AGroup := TileControl.Groups[I];
...
begin
  inherited DoCalculate;
  ASize := GetTextureSize;
  FGlyphBounds := cxRectSetHeight(cxRectCenterHorizontally(Bounds, ASize.cx), ASize.cy);
  FGlyphRect := FGlyphBounds;
end;

procedure TdxTileControlActionBarCustomItemViewInfo.DrawGlyph(ACanvas: TcxCustomCanvas);
begin
  Painter.DrawGlyph(ACanvas, FGlyphRect, State, Texture, TextureType, TextColor);
end;

procedure TdxTileControlActionBarCustomItemViewInfo.Draw(ACanvas: TcxCustomCanvas);
begin
  DrawGlyph(ACanvas);
  if Text <> '' then
    Painter.DrawText(ACanvas, TextBounds, Text, Font, TextColor, taCenter, vaCenter);
end;
...
function TdxTileControlActionBarCustomItemViewInfo.MeasureSize: TSize;
var
  ATextSize: TSize;
begin
  ...
  if Text <> '' then
  begin
    ATextSize := Painter.MeasureTextSize(cxMeasureCanvas, Text, Font, taCenter, False, 0, 0);
    Result.cx := Max(Result.cx, ATextSize.cx);
    Result.cy := Result.cy + ATextSize.cy + 2 * ScaleFactor.Apply(cxTextOffset);
  end;
end;
...
function TdxTileControlActionBarCustomItemViewInfo.GetTextureSize: TSize;
begin
  Result := ScaleFactor.Apply(Texture.Size);
end;
...
function TdxTileControlActionBarItemViewInfo.GetTextureSize: TSize;
begin
  if Item.IsImageListUsed then
    Result := dxGetImageSize(Item.GetImages, ScaleFactor)
  else
    Result := inherited GetTextureSize;
end;
...
procedure TdxTileControlActionBarItemViewInfo.DrawGlyph(ACanvas: TcxCustomCanvas);
const
  DrawModeMap: array[Boolean] of TcxImageDrawMode = (idmNormal, idmDingy);
var
  R: TRect;
begin
  if Item.IsImageListUsed then
  begin
    ...
    Painter.DrawGlyph(ACanvas, R, Item.Glyph, Item.GetImages, Item.ImageIndex, DrawModeMap[State <> cxbsNormal]);
  end
  else
...
begin
  FreeAndNil(FContentLayer);
  FreeAndNil(FTextLayer);
  inherited Destroy;
end;

procedure TdxTileControlCustomItemViewData.AdjustObjectBounds(const ABounds: TRect;
  var AObjectBounds: TRect; AAlign: TdxTileItemInnerObjectAlignment; AIndentHorz, AIndentVert: Integer);
begin
  ...
  if AAlign in [oaTopLeft, oaTopCenter, oaTopRight] then
    ...
    begin
      AObjectBounds := cxRectSetTop(AObjectBounds, (ABounds.Top + ABounds.Bottom - cxRectHeight(AObjectBounds)) div 2);
       AIndentVert := 0;
    end
    else
    begin
...
procedure TdxTileControlCustomItemViewData.CalculateImageAndTextLayout(const ABounds: TRect);
  ...
  begin
    ...
    if (ANum = 0) and (not AGlyph.Image.Empty or Item.Glyph.IsImageListUsed) and (AImageAlign <> itaNone) then
    begin
...
var
  ...
  AGlyph: TdxTileControlItemCustomGlyph;
  APainter: TdxTileControlPainter;
...
begin
  AGlyph := Item.Glyph;
  APainter := Painter;
  ACanvas := cxMeasureCanvas;//APainter.TileControl.ActualCanvas;
  FImageBounds := cxRectScaleSize(AGlyph.GetClientRect(ABounds), dxDefaultDPI, AGlyph.Image.SourceDPI);
  for I := 0 to 3 do
  begin
    FTextBounds[I] := cxNullRect;
    AText := Item.GetText(I);
    if AText.HasValue then
    begin
      ATextBounds := cxRectInflate(GetTextBounds(AText, I, AGlyph), -APainter.ScaleFactor.Apply(cxTextSpace));
      ATextBounds := cxRectSetSize(ATextBounds, APainter.MeasureTextSize(ACanvas, AText.GetActualValue,
        AText.GetActualFont, AText.Alignment, AText.WordWrap, cxRectWidth(ATextBounds), cxRectHeight(ATextBounds)));
      AdjustObjectBounds(ABounds, ATextBounds, AText.GetActualAlign, AText.IndentHorz, AText.IndentVert);
      FTextBounds[I] := ATextBounds;
    end;
  end;
...
procedure TdxTileControlCustomItemViewData.DrawItemBackground(ACanvas: TcxCustomCanvas);
var
...
procedure TdxTileControlCustomItemViewData.DrawItemGlyph(ACanvas: TcxCustomCanvas);
var
  AGlyph: TdxTileControlItemCustomGlyph;
begin
  AGlyph := Item.Glyph;
  Painter.DrawGlyph(ACanvas, ImageBounds, AGlyph.Image, AGlyph.GetImages, AGlyph.ImageIndex);
end;

procedure TdxTileControlCustomItemViewData.DrawItemText(
  ACanvas: TcxCustomCanvas; const ATextBounds, AClientRect: TRect; AText: TdxTileControlItemText);
var
  AActualText: string;
  AClipNeeded: Boolean;
  AClipRect: TRect;
begin
  if cxRectIntersect(AClipRect, AClientRect, ATextBounds) then
  begin
    AActualText := AText.GetActualValue;
    if AActualText = '' then
      Exit;

    AClipNeeded := not cxRectIsEqual(AClipRect, ATextBounds);
    if AClipNeeded then
    begin
      ACanvas.SaveClipRegion;
    ...
    begin
      ACanvas.FillRect(ATextBounds, cxGetActualColor(
        AText.GetActualColor, Painter.DefaultItemTextBackgroundColor), MaxByte);
    end;

    Painter.DrawText(ACanvas, ATextBounds, AActualText, AText.GetActualFont,
      cxGetActualColor(AText.GetActualFontColor, Painter.DefaultItemTextColor), 
      AText.Alignment, vaTop, AText.WordWrap, not Item.DrawTextOnImage);
...
procedure TdxTileControlCustomItemViewData.DrawItemTexts(
  ACanvas: TcxCustomCanvas; const AOrigin: TPoint; const AClipRect: TRect);
var
  I: Integer;
begin
  for I := 0 to 3 do
    DrawItemText(ACanvas, cxRectOffset(FTextBounds[I], AOrigin), AClipRect, Item.GetText(I));
end;

procedure TdxTileControlCustomItemViewData.RebuildLayers;

  procedure ValidateLayer(var ALayer: TdxFastDIB);
  begin
    if ALayer <> nil then
      ALayer.Resize(Bounds)
    else
      ALayer := TdxFastDIB.Create(Bounds);
  end;

  procedure ValidateContentLayer;
  begin
    ValidateLayer(FContentLayer);
    cxPaintCanvas.BeginPaint(FContentLayer.DC);
    try
      cxPaintCanvas.FillRect(FContentLayer.ClientRect, clWhite);
      if TileItem.CanGalleryAnimation then
        GalleryCellController.Draw(cxPaintCanvas, cxNullPoint)
      else
      begin
        DrawItemBackground(cxPaintCanvas);
        DrawItemGlyph(cxPaintCanvas);
      end;
      if Item.DrawTextOnImage then
        DrawItemTexts(cxPaintCanvas, cxNullPoint, FClientRect);
    finally
      cxPaintCanvas.EndPaint;
    end;
  end;

  procedure ValidateTextLayer;
  begin
    if not Item.DrawTextOnImage then
    begin
      ValidateLayer(FTextLayer);
      FTextLayer.Clear;
      cxPaintCanvas.BeginPaint(FTextLayer.DC);
      try
        DrawItemTexts(cxPaintCanvas, cxNullPoint, FClientRect);
      finally
        cxPaintCanvas.EndPaint;
...
begin
  FClientRect := cxRectContent(TileItem.ViewInfo.GetHighlightBounds(Bounds), Item.Margins.Margin);

  CalculateImageAndTextLayout(FClientRect);
  ValidateContentLayer;
  ValidateTextLayer;

  if IsActive then
  begin
    TileItem.ViewInfo.Calculated := False;
    TileItem.ViewInfo.FlushCache;
  end;
end;
...
function TdxTileControlCustomItemViewData.GetContentLayer: TdxFastDIB;
begin
  IsDirty := False;
  Result := FContentLayer;
end;
...
function TdxTileControlCustomItemViewData.GetTextLayer: TdxFastDIB;
begin
  IsDirty := False;
  Result := FTextLayer;
end;
...
begin
  ...
  FCountInfoFont := dxCreateFontForDefaultDPI(dxTileControlDefaultGroupCaptionFontSize);
...
procedure TdxTileControlDragDropItem.DoCollapseBeginGroupWhenItemIsLast;
  ...
  function CanHideBeginGroup: Boolean;
  begin
    Result := True;
    if Assigned(TileControl.OnGroupRemoveWithLastItem) then
      TileControl.OnGroupRemoveWithLastItem(TileControl, BeginGroup, Result);
  end;

begin
  if TileControl.IsDesigning or CanHideBeginGroup then
  begin
...
begin
  Result := not (((GroupLayout = glHorizontal) and BeginGroup.IsRealMostRight) or
                 ((GroupLayout = glVertical) and BeginGroup.IsRealMostBottom));
  if not Result then
    Exit;
  if GroupLayout = glVertical then
...
begin
  ...
  try
    ...
    begin
      AGroup := TileControl.Groups[I];
      AAllow := True;
      AVisible := AGroup.ItemCount > 0;
      if AGroup.Visible and (not AVisible) and Assigned(TileControl.OnGroupRemoveWithLastItem) then
        TileControl.OnGroupRemoveWithLastItem(TileControl, AGroup, AAllow);
      if AAllow then
...
begin
  ...
  if ((Self = Group.VirtualGroupBefore) and Group.IsRealMostLeft) or
     ((Self = Group.VirtualGroupAfter) and Group.IsRealMostRight) then
  ...
  FBasisWidth := AWidth;
  if TileControl.ViewInfo.IsFixedContentLeftSide and (Self = Group.VirtualGroupBefore) and Group.IsRealMostLeft then
...
begin
  ...
  if ((Self = Group.VirtualGroupBefore) and Group.IsRealMostTop) or
     ((Self = Group.VirtualGroupAfter) and Group.IsRealMostBottom) then
  ...
  if TileControl.ViewInfo.IsFixedContentTopSide and (Self = Group.VirtualGroupBefore) and Group.IsRealMostTop then
...
procedure TdxTileControlVirtualGroupViewInfo.DoDraw(ACanvas: TcxCustomCanvas);
begin
...
begin
  ...
  if Self = Group.VirtualGroupBefore then
    ...
    if Group.IsRealMostBottom then
...
begin
  ...
  if TileControl.OptionsView.FixedIndentHorz and (GroupLayout = glVertical) then
...
begin
  RestoreDetailControlPrevParent;
  DetailControl := nil;
...
procedure TdxTileControlCustomViewInfo.FlushCache;
begin
  // do nothing
end;
...
begin
  FVisibleBounds := GetVisibleBounds;
  FVisible := cxRectIntersect(FClipRect, VisibleBounds, Bounds);
  FClipped := not cxRectIsEqual(FClipRect, Bounds);
end;
...
procedure TdxTileControlCustomCellViewInfo.DoDraw(ACanvas: TcxCustomCanvas);
begin
  // do nothing
end;

procedure TdxTileControlCustomCellViewInfo.Draw(ACanvas: TcxCustomCanvas);
begin
  if Visible then
  begin
    if Clipped then
...
begin
  ...
  if not (avColor in AssignedValues) then
    Result := TileControl.Painter.DefaultItemTextBackgroundColor;
end;
...
begin
  Result := Value;
  if (Result = '') and TileControl.IsDesigning and (FDefaultAlign = oaTopLeft) then
...
function TdxTileControlItemText.GetTileControl: TdxCustomTileControl;
begin
  Result := Owner.TileItem.TileControl;
end;
...
begin
  if ((TileControl.ActiveDetail = nil) or (TileControl.ActiveDetail.ActiveControl <> DetailOptions.DetailControl)) and
      TileControl.CanDeactivateDetail(TileControl.ActiveDetail) then
...
begin
  Result := not(csDestroying in ComponentState) and not(csDestroying in TileControl.ComponentState) and
    (ItemType = tcitGallery) and (GalleryCellController <> nil);
end;
...
function TdxTileControlItem.DrawTextOnImage: Boolean;
begin
  Result := (ItemType <> tcitGallery) and inherited DrawTextOnImage;
end;
...
begin
  Result := ViewData;
  if TileControl.IsDesigning or (ActiveFrame = nil) or (ItemType = tcitGallery) then
    Exit;
...
begin
  ...
  if not (csDestroying in ComponentState) then
    ViewInfo.ViewData.RebuildLayers;
...
begin
  if not Visible or TileControl.IsLocked then Exit;
  TileControl.ViewInfo.MakeVisible(ViewInfo.GetActualVisualBounds,
    IsRealMostLeft, IsRealMostTop, IsRealMostRight, IsRealMostBottom);
  Invalidate;
end;
...
function TdxTileControlItem.IsRealMostBottom: Boolean;
var
  I, ABottom: Integer;
begin
  Result := (Group <> nil) and Group.IsRealMostBottom;
  if Result then
  begin
    ABottom := ViewInfo.Bounds.Bottom;
    for I := 0 to Group.ItemCount - 1 do
    begin
      Result := Group.Items[I].ViewInfo.Bounds.Bottom <= ABottom;
      if not Result then
        Exit;
    end;
  end;
...
function TdxTileControlItem.IsRealMostLeft: Boolean;
begin
  Result := (Group <> nil) and Group.IsRealMostLeft and (ViewInfo.Position.Column = 0);
end;

function TdxTileControlItem.IsRealMostRight: Boolean;
var
  I, AOutside: Integer;
begin
  Result := (Group <> nil) and Group.IsRealMostRight;
  if Result then
  begin
    if not TileControl.UseRightToLeftAlignment then
      AOutside := ViewInfo.Bounds.Right
    ...
    begin
      if not TileControl.UseRightToLeftAlignment then
        Result := Group.Items[I].ViewInfo.Bounds.Right <= AOutside
      else
        Result := Group.Items[I].ViewInfo.Bounds.Left >= AOutside;
      if not Result then
        Exit;
    end;
  end;
...
function TdxTileControlItem.IsRealMostTop: Boolean;
begin
  Result := (Group <> nil) and Group.IsRealMostTop and (ViewInfo.Position.Row = 0);
end;
...
begin
  ...
  begin
    ...
    if not TileControl.IsDesigning then
    begin
      if not DragAndDropObject.ImmediateFinish and ((ItemsInfo.Count > 0) or DragObjectFinished or DragFinished) then
...
procedure TdxTileControlDragAndDropChanges.DrawItems(ACanvas: TcxCustomCanvas);
var
  I: Integer;
begin
  for I := 0 to Count - 1 do
...
procedure TdxTileControlDragAndDropChanges.PopulateItemsInfo(ABeforeUpdate: Boolean);
var
  I: Integer;
...
begin
  Result := (TileControl.DragAndDropState = ddsNone) and (DragObject <> nil) or DragFinished;
end;
...
var
  ...
  ARectSize: TSize;
begin
  AImageSize := dxGetImageSize(Image, GetImages, ImageIndex, Owner.TileItem.TileControl.ScaleFactor);
  if (AImageSize.cx > 0) and (AImageSize.cy > 0) then
  begin
    ARectSize := cxSize(ABounds);
    if Mode = ifmStretch then
      AImageSize := ARectSize
    else
      if Mode in [ifmProportionalStretch, ifmFit] then
      begin
        AAspectRatio := Min(ARectSize.cx / AImageSize.cx, ARectSize.cy / AImageSize.cy);
        if (Mode = ifmProportionalStretch) or (AAspectRatio < 1) then
          AImageSize := cxSize(Trunc(AImageSize.cx * AAspectRatio), Trunc(AImageSize.cy * AAspectRatio));
      end;
  end;
  Result := cxRectCenter(ABounds, AImageSize);
end;
...
begin
  Result := (Align <> oaDefault) or not Image.Empty or IsImageListUsed or (IndentHorz <> 0) or (IndentVert <> 0);
end;

function TdxTileControlItemCustomGlyph.IsImageListUsed: Boolean;
var
  AImages: TCustomImageList;
begin
  AImages := GetImages;
...
procedure TdxTileControlGroupCaptionInplaceEdit.UpdatePosition(AWinPosInfo: THandle);
var
  AActualCaptionPos: TPoint;
begin
  AActualCaptionPos := GetCaptionTextBounds.TopLeft;
  DeferWindowPos(AWinPosInfo, Handle, 0,
    IniBoundsPos.X + AActualCaptionPos.X - IniCaptionPos.X,
    IniBoundsPos.Y + AActualCaptionPos.Y - IniCaptionPos.Y,
    Width, Height, SWP_NOSIZE or SWP_NOZORDER);
end;
...
function TdxThreadScalerTaskItem.ReleaseImage: TdxGpFastDIB;
begin
  Result := FImage;
  FImage := nil;
end;
...
procedure TdxThreadScalerTaskItem.Scale;
  ...
  function CreateSmartImage(AGraphic: TGraphic): TdxSmartImage;
  begin
    Result := TdxSmartImage.Create;
    if AGraphic is TdxGPImage then
      Result.Assign(AGraphic)
    else
      LoadGraphicToImage(AGraphic, Result);
  end;

  function GetImageRect(AGraphic: TGraphic): TRect;
  const
    ModeMap: array[Boolean] of TcxImageFitMode = (ifmFit, ifmFill);
  begin
    Result := cxGetImageRect(cxRect(FOutputSize), Size(AGraphic.Width, AGraphic.Height),
      ModeMap[(FOutputSize.cx < AGraphic.Width) and (FOutputSize.cy < AGraphic.Height)]);

    // Clean Rounding Error
    if cxRectWidth(Result) - FOutputSize.cx = -1 then
      Inc(Result.Right);
    if cxRectHeight(Result) - FOutputSize.cy = -1 then
      Inc(Result.Bottom);
  end;
...
var
  ...
  AImageCanvas: TdxGPCanvas;
begin
  AImage := CreateSmartImage(FImageItem.Picture.Graphic);
  try
    FImage := TdxGpFastDIB.Create(FOutputSize.cx, FOutputSize.cy, True);
    AImageCanvas := FImage.CreateCanvas;
    try
      AImage.StretchDraw(AImageCanvas, GetImageRect(AImage), AImage.ClientRect, nil);
    finally
      AImageCanvas.Free;
    end;
  finally
...
function TdxTileControlCustomItem.CreateText(const AAlign: TdxTileItemInnerObjectAlignment): TdxTileControlItemText;
begin
...
function TdxTileControlCustomItem.DrawTextOnImage: Boolean;
begin
  Result := OptionsAnimate.AnimateText;
end;
...
begin
  ...
  if Accepted then
    cxRecalculateAutoSize(TileControl);
end;
...
procedure TdxTileControlDragAndDropChangesCellInfo.Draw(ACanvas: TcxCustomCanvas; const AProgress: Integer);

  procedure DoDraw(AClipRect: TRect; const ASource, ADest: TRect);
  begin
    if cxRectIntersect(AClipRect, AClipRect, ViewInfo.TilesArea) then
    begin
      ACanvas.SaveClipRegion;
      try
        ACanvas.IntersectClipRect(AClipRect);
...
begin
  ...
  DoDraw(ClipRect1, ASource, ADest);
  DoDraw(ClipRect2, ASource, ADest);
end;
...
begin
  TileControl.InvalidateRect(ClipRect1, False);
  TileControl.InvalidateRect(ClipRect2, False);
end;
...
begin
  ...
  IsSimpleMoving := (GroupCaption <> nil) or (Item <> nil) and (RowBefore = AInfo.RowAfter) or (ColumnBefore = AInfo.ColumnAfter);
  if IsSimpleMoving then
  begin
    ClipRect1 := cxRectUnion(BoundsBefore, BoundsAfter);
    ClipRect2 := cxRectSetSize(ClipRect1, 0, 0);
  end
  else
  begin
    ClipRect1 := BoundsBefore;
    ClipRect2 := BoundsAfter;
  end;
...
destructor TdxTileControlPainter.Destroy;
begin
  FlushCache;
  inherited;
end;

procedure TdxTileControlPainter.FlushCache;
begin
  FreeAndNil(FBackgroundCache);
end;

procedure TdxTileControlPainter.DrawBackground(ACanvas: TcxCustomCanvas; const R: TRect);
var
  ATextureOffsetX: Integer;
  ATextureOffsetY: Integer;
begin
  if TileControl.Transparent and (ACanvas is TcxCanvas) then
  begin
    DrawTransparentBackground(TcxCanvas(ACanvas), R);
    Exit;
  end;
  ...
  begin
    ...
    SkinInfo.TileControlBackground.Draw(ACanvas, R, ScaleFactor);
    DrawStyleTexture(ACanvas, R, TileControl.Style, ATextureOffsetX, ATextureOffsetY, @FBackgroundCache);
  end
  else
  begin
    if TileControl.Style.IsEmpty then
      LookAndFeelPainter.DrawPanelContent(ACanvas, R);
    DrawStyleRect(ACanvas, R, TileControl.Style, ATextureOffsetX, ATextureOffsetY, @FBackgroundCache);
  end;
end;

procedure TdxTileControlPainter.DrawDesignTimeBorder(ACanvas: TcxCustomCanvas; const R: TRect; ASelected: Boolean);
const
  ColorMap: array [Boolean] of TColor = (dxTileVirtualGroupBackgroundColor, clBlue);
  StyleMap: array [Boolean] of TPenStyle = (psDash, psDashDot);
begin
  ACanvas.Rectangle(R, clNone, ColorMap[ASelected], StyleMap[ASelected]);
end;

procedure TdxTileControlPainter.DrawGlyph(ACanvas: TcxCustomCanvas; const R: TRect; AGlyph: TdxSmartImage);
begin
  ACanvas.DrawNativeObject(R,
    TcxCanvasBasedResourceCacheKey.Create(AGlyph, cxSize(R), 0, 0, ScaleFactor.TargetDPI),
    procedure (ACanvas: TcxGdiBasedCanvas; const R: TRect)
    begin
      TdxImageDrawer.DrawImage(TcxCanvas(ACanvas), R, AGlyph, ifmNormal, nil, ScaleFactor);
    end);
end;

procedure TdxTileControlPainter.DrawGlyph(ACanvas: TcxCustomCanvas; const R: TRect; AGlyph: TdxSmartImage;
  AImages: TCustomImageList; AImageIndex: TcxImageIndex; ADrawMode: TcxImageDrawMode = idmNormal);
var
  ACacheKey: TcxCanvasBasedResourceCacheKey;
begin
  ACacheKey := TcxCanvasBasedResourceCacheKey.Create(AGlyph, cxSize(R), Ord(ADrawMode), -1, ScaleFactor.TargetDPI);
  if not IsGlyphAssigned(AGlyph) then
  begin
    ACacheKey.Owner := AImages;
    ACacheKey.Part := AImageIndex;
  end;

  ACanvas.DrawNativeObject(R, ACacheKey,
    procedure (ACanvas: TcxGdiBasedCanvas; const R: TRect)
    begin
      TdxImageDrawer.DrawImage(TcxCanvas(ACanvas), R, AGlyph, AImages,
        AImageIndex, ifmStretch, ADrawMode, False, nil, ScaleFactor, True);
    end);
end;

procedure TdxTileControlPainter.DrawGlyph(ACanvas: TcxCustomCanvas; const R: TRect;
  AState: TcxButtonState; AGlyph: TdxSkinImage; AGlyphType: TImageType; AColor: TColor; AIsRTL: Boolean = False);
const
...
begin
  ...
  begin
    ...
    try
      ...
      ASkinImage.RightToLeftDependentDraw(ACanvas, R, ScaleFactor, AIsRTL, StateMap[AState]);
    finally
      ASkinImage.Free;
    end;
  end
  else
    AGlyph.Draw(ACanvas, R, ScaleFactor, StateMap[AState]);
end;

procedure TdxTileControlPainter.DrawDefaultItemBackground(ACanvas: TcxCustomCanvas; const R: TRect);
begin
  if UseSkins then
    SkinInfo.TileControlItem.Draw(ACanvas, R, ScaleFactor)
  else
    ACanvas.FillRectByGradient(R,
      dxColorToAlphaColor(dxTileItemShadowColor),
      dxColorToAlphaColor(dxTileItemLightColor),
      LinearGradientModeForwardDiagonal);
end;

procedure TdxTileControlPainter.DrawDefaultItemCheck(ACanvas: TcxCustomCanvas; const R: TRect);
begin
  DrawDefaultItemCheck(ACanvas, R, CheckedItemFrameColor, CheckedItemCheckMarkColor);
end;

procedure TdxTileControlPainter.DrawDefaultItemCheck(ACanvas: TcxCustomCanvas;
  const R: TRect; ABackgroundColor, ACheckMarkColor: TColor; AAlpha: Byte = MaxByte);
begin
  ACanvas.DrawNativeObject(R,
    TcxCanvasBasedResourceCacheKey.Create(Self, cxSize(R), ABackgroundColor, ACheckMarkColor, ScaleFactor.TargetDPI),
    procedure (ACanvas: TdxGPCanvas; const R: TRect)
    begin
      DrawDefaultItemCheck(ACanvas, R,
        dxColorToAlphaColor(ABackgroundColor, AAlpha),
        dxColorToAlphaColor(ACheckMarkColor, AAlpha), ScaleFactor);
    end);
end;

procedure TdxTileControlPainter.DrawDefaultItemCheck(ACanvas: TdxGPCanvas; const R: TRect;
  ABackgroundColor, ACheckMarkColor: TdxAlphaColor; AScaleFactor: TdxScaleFactor);

  function CreateRegion: TdxGPRegion;
  var
  ...
  begin
    ...
    Result.CombineRegionRect(cxRectInflate(R, -AScaleFactor.Apply(dxTileCheckedItemFrameBorderSize)), gmExclude);
    Result.CombineRegionRegion(CreatePolygonRgn(APoints[0], Length(APoints), ALTERNATE), gmUnion, True);
  end;
  ...
  procedure DrawCheckMark(ACanvas: TdxGPCanvas; AColor: TdxAlphaColor);
  var
  ...
  begin
    ...
    APrevSmoothingMode := ACanvas.SmoothingMode;
    try
      ACanvas.SmoothingMode := smAntiAlias;
      ACanvas.Polyline(APoints, AColor, AScaleFactor.Apply(dxTileCheckedItemCheckMarkPenSize));
    finally
      ACanvas.SmoothingMode := APrevSmoothingMode;
    end;
  end;
...
begin
  ...
  try
    ...
    try
      ACanvas.SetClipRegion(ARegion, gmReplace);
      ACanvas.FillRectangle(R, ABackgroundColor);
      if ACheckMarkColor <> TdxAlphaColors.Empty then
        DrawCheckMark(ACanvas, ACheckMarkColor);
    finally
      ARegion.Free;
    end;
  finally
...
procedure TdxTileControlPainter.DrawHighlightRect(ACanvas: TcxCustomCanvas;
  const R: TRect; AHighlightColor: TdxAlphaColor; ABackgroundColor: TColor = clNone);
...
begin
  ...
  ACanvas.FillRect(R, AColor, AAlpha);
end;

procedure TdxTileControlPainter.DrawItemCheck(ACanvas: TcxCustomCanvas; const R: TRect);
begin
  if UseSkins then
    SkinInfo.TileControlItemCheck.Draw(ACanvas, R, ScaleFactor)
  else
    DrawDefaultItemCheck(ACanvas, R);
end;

procedure TdxTileControlPainter.DrawStyleRect(ACanvas: TcxCustomCanvas; const R: TRect;
  AStyle: TdxTileControlCustomStyle; ATextureOffsetX, ATextureOffsetY: Integer;
  ATextureCache: PcxCanvasBasedImage = nil);
begin
  ACanvas.FillRectByGradient(R,
    dxColorToAlphaColor(cxGetActualColor(AStyle.GradientBeginColor, LookAndFeelPainter.DefaultContentColor)),
    dxColorToAlphaColor(AStyle.GradientEndColor),
    dxSkinsGradientModeMap[AStyle.Gradient]);
  DrawStyleTexture(ACanvas, R, AStyle, ATextureOffsetX, ATextureOffsetY, ATextureCache);
end;

procedure TdxTileControlPainter.DrawStyleRectFrame(ACanvas: TcxCustomCanvas; const R: TRect; AStyle: TdxTileControlCustomStyle);
begin
  if AStyle.BorderColor <> clNone then
    ACanvas.FrameRect(R, dxColorToAlphaColor(cxGetActualColor(AStyle.BorderColor, clBtnShadow)), 1, cxBordersAll);
end;

procedure TdxTileControlPainter.DrawStyleTexture(ACanvas: TcxCustomCanvas; R: TRect;
  AStyle: TdxTileControlCustomStyle; ATextureOffsetX, ATextureOffsetY: Integer; ATextureCache: PcxCanvasBasedImage);
var
  APrevQuality: TcxCanvasImageStretchQuality;
begin
  if AStyle.Texture.Empty then
    Exit;

  if AStyle.Stretch <> smStretch then
  begin
    Dec(R.Left, ATextureOffsetX);
    Dec(R.Top, ATextureOffsetY);
  end;

  if AStyle.Stretch = smNoResize then
    R := cxRectCenter(R, AStyle.Texture.Width, AStyle.Texture.Height);

  case AStyle.Stretch of
    smTile:
      ACanvas.FillRect(R, AStyle.Texture, ATextureCache);

    smStretch:
      begin
        APrevQuality := ACanvas.ImageStretchQuality;
        try
          ACanvas.ImageStretchQuality := isqHigh;
          ACanvas.DrawImage(AStyle.Texture, R, ATextureCache);
        finally
          ACanvas.ImageStretchQuality := APrevQuality;
        end;
      end;
...
procedure TdxTileControlPainter.DrawItemBackground(ACanvas: TcxCustomCanvas; const R: TRect; AStyle: TdxTileControlCustomStyle);
begin
  if AStyle.IsEmpty then
    DrawDefaultItemBackground(ACanvas, R)
  else
    DrawStyleRect(ACanvas, R, AStyle, 0, 0);
end;

procedure TdxTileControlPainter.DrawItemPlace(ACanvas: TcxCustomCanvas; const R: TRect);
begin
  if UseSkins then
    SkinInfo.TileControlVirtualGroup.Draw(ACanvas, R, ScaleFactor)
  else
    ACanvas.FillRect(R, dxTileItemPlaceColor);
end;

procedure TdxTileControlPainter.DrawPageTab(ACanvas: TcxCustomCanvas; const R: TRect; AState: TcxButtonState);
const
...
begin
  if UseSkins then
    SkinInfo.TileControlTabHeader.Draw(ACanvas, R, ScaleFactor, 0, ButtonStateToSkinState[AState]);
end;

procedure TdxTileControlPainter.DrawText(ACanvas: TcxCustomCanvas; const R: TRect; const AText: string; 
  AFont: TFont; AColor: TColor; AAlignment: TAlignment = taLeftJustify; AVertAlignment: TcxAlignmentVert = vaCenter; 
  AWordWrap: Boolean = False; APaintOnGlass: Boolean = False);
var
  ALayout: TcxCanvasBasedTextLayout;
begin
  if ACanvas.RectVisible(R) then
  begin
    ALayout := ACanvas.CreateTextLayout;
    try
      ALayout.SetColor(AColor);
      ALayout.SetFont(AFont);
      ALayout.SetFlags(GetTextOutFlags(AAlignment, AVertAlignment, AWordWrap));
      ALayout.SetLayoutConstraints(R);
      ALayout.SetPaintOnGlass(APaintOnGlass);
      ALayout.SetText(AText);
      ALayout.Draw(R);
    finally
      ALayout.Free;
    end;
...
procedure TdxTileControlPainter.DrawVirtualGroup(ACanvas: TcxCustomCanvas; const R: TRect);
begin
  if UseSkins then
    SkinInfo.TileControlVirtualGroup.Draw(ACanvas, R, ScaleFactor)
  else
...
function TdxTileControlPainter.MeasureTextSize(ACanvas: TcxCustomCanvas; const AText: string;
  AFont: TFont; AAlignment: TAlignment; AWordWrap: Boolean; AMaxWidth, AMaxHeight: Integer): TSize;
var
  ALayout: TcxCanvasBasedTextLayout;
begin
  ALayout := ACanvas.CreateTextLayout;
  try
    ALayout.SetFont(AFont);
    ALayout.SetFlags(GetTextOutFlags(AAlignment, vaTop, AWordWrap));
    ALayout.SetText(AText);
    ALayout.SetLayoutConstraints(AMaxWidth, AMaxHeight, 0);
    Result := ALayout.MeasureSize;
  finally
    ALayout.Free;
  end;
end;
...
begin
  case AState of
    ...
    cxbsDisabled:
      Result := TileControl.Title.TabsDisabledTextColor;
  else
...
function TdxTileControlPainter.GetScaleFactor: TdxScaleFactor;
begin
  Result := TileControl.ScaleFactor;
end;

function TdxTileControlPainter.GetTextOutFlags(AAlignment: TAlignment; AVertAlignment: TcxAlignmentVert; AWordWrap: Boolean): Integer;
const
  HorzAlign: array[Boolean, TAlignment] of Integer = (
    (CXTO_LEFT, CXTO_RIGHT, CXTO_CENTER_HORIZONTALLY),
    (CXTO_RIGHT, CXTO_LEFT, CXTO_CENTER_HORIZONTALLY)
  );
  VertAlign: array[TcxAlignmentVert] of Integer = (
    CXTO_TOP, CXTO_BOTTOM, CXTO_CENTER_VERTICALLY
  );
begin
  Result := CXTO_EXPANDTABS or CXTO_EDITCONTROL or CXTO_END_ELLIPSIS;
  Result := Result or HorzAlign[TileControl.UseRightToLeftAlignment, AAlignment] or VertAlign[AVertAlignment];
  if TileControl.UseRightToLeftReading then
...
procedure TdxTileControlPageTabCellViewInfo.DoDraw(ACanvas: TcxCustomCanvas);
begin
  Painter.DrawPageTab(ACanvas, Bounds, State);
  Painter.DrawText(ACanvas, cxRectContent(Bounds, GetContentOffsets), 
    Caption, Font, Painter.PageTabTextColor[State], taCenter, vaCenter);
end;
...
begin
  ...
  begin
    if Item.GetText(I).HasValue then
      Exit(Item.GetText(I).GetActualValue);
  end;
...
function TdxTileControlPageTabCellViewInfo.MeasureSize: TSize;
begin
  Result := Painter.MeasureTextSize(Owner.ActualCanvas, Caption, Font, taCenter, False, 0, 0);
  Result := cxSizeAddMargins(Result, GetContentOffsets);
