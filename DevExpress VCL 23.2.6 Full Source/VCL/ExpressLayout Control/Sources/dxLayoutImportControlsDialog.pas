...
unit dxLayoutImportControlsDialog;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, StdCtrls, cxClasses, dxLayoutControl, cxControls, cxStyles, dxForms,
  ...
  dxLayoutControlAdapters, dxCore;
...
type
  ...
  private
    ...
    function CanDeleteControl: Boolean;
    procedure SetAlignments;
  public
  ...
  private
    function GetItem(Index: TdxListIndex): TdxAutoLayoutItem;
  public
    ...
    property Items[Index: TdxListIndex]: TdxAutoLayoutItem read GetItem; default;
  end;
  ...
  public
    ...
    procedure Extract(AIndex: TdxListIndex);
  ...
  private
    ...
    FConvertCheckBoxes: Boolean;
    FConvertRadioButtons: Boolean;
  protected
  ...
  public
    ...
    property ConvertCheckBoxes: Boolean read FConvertCheckBoxes write FConvertCheckBoxes default False;
    property ConvertPageControls: Boolean read FConvertPageControls write FConvertPageControls default True;
    property ConvertRadioButtons: Boolean read FConvertRadioButtons write FConvertRadioButtons default False;
  ...
  { TfmImportControlsDialog }

  TfmImportControlsDialog = class(TdxForm)
    ...
    cbUseLabeledItems: TdxLayoutCheckBoxItem;
    cbConvertPageControls: TdxLayoutCheckBoxItem;
    cbAssociate: TdxLayoutCheckBoxItem;
    cbConvertCheckBoxes: TdxLayoutCheckBoxItem;
    cbConvertRadioButtons: TdxLayoutCheckBoxItem;
    procedure cbAssociateClick(Sender: TObject);
    procedure cbConvertCheckBoxesClick(Sender: TObject);
    procedure cbConvertRadioButtonsClick(Sender: TObject);
    procedure FormCreate(Sender: TObject);
  public
    function GetImportContainer: TWinControl;
    procedure PopulateContainersList(ALayoutControl: TdxCustomLayoutControl; ARootContainer: TWinControl);
  end;
...
uses
  cxGeometry, Math, dxLayoutConvertControlsDialog, dxBevel;

const
  dxThisUnitName = 'dxLayoutImportControlsDialog';
...
type
  ...
  private
    ...
    function GetItem(Index: TdxListIndex): TdxSideItem;
  protected
  ...
  public
    ...
    property Items[Index: TdxListIndex]: TdxSideItem read GetItem; default;
  end;
...
function TdxSideItemList.GetItem(Index: TdxListIndex): TdxSideItem;
begin
...
begin
  ...
  if (FAutoLayout.ConvertCheckBoxes and ((Control is TCheckBox) or (Control is TcxCheckBox))) or
    (FAutoLayout.UseLabeledItems and IsStandaloneLabel and ((Control is TLabel) or (Control is TcxLabel))) or
    (FAutoLayout.ConvertRadioButtons and (Control is TRadioButton)) then
    Result := TdxLayoutControlEmbeddedControlItemConversionHelper.Convert(TdxLayoutItem(Result));
end;
...
begin
  Result := ((Control is TBevel) and (TBevel(Control).Shape in [bsTopLine, bsBottomLine, bsLeftLine, bsRightLine])) or
    ((Control is TdxBevel) and (TdxBevel(Control).Shape in [dxbsLineTop, dxbsLineBottom, dxbsLineLeft, dxbsLineRight,
    dxbsLineCenteredHorz, dxbsLineCenteredVert]));
end;
...
begin
  if Safe.Cast(LayoutControlItem, TdxLayoutItem, ALayoutItem) then
  begin
    if Labeled then
    begin
      ALayoutItem.Caption := Caption;
      ALayoutItem.CaptionOptions.Layout := CaptionLayout;
    end
    else
      ALayoutItem.ShowCaption := False;
  end;
  if ANeedSetAligns then
  begin
    SetAlignments;
    if CanDeleteControl then
      FreeAndNil(FControl);
  end;
end;

function TdxAutoLayoutItem.CanDeleteControl: Boolean;
begin
  Result :=
    not (csAncestor in FControl.ComponentState) and (
    IsGroup or
    IsSplitter or
    IsSeparator or
    (IsPageControl and FAutoLayout.ConvertPageControls));
end;

procedure TdxAutoLayoutItem.SetAlignments;
begin
...
function TdxAutoLayoutItemList.GetItem(Index: TdxListIndex): TdxAutoLayoutItem;
begin
...
procedure TdxAutoLayoutOwnedItemList.Extract(AIndex: TdxListIndex);
begin
  Items[AIndex].Free;
  Delete(AIndex);
end;
...
function TdxAutoLayoutOwnedItemList.GetItem(Index: TdxListIndex): TdxAutoLayoutItem;
begin
  Result := TdxAutoLayoutItem(inherited Items[Index]);
end;
...
var
  ...
  var
    AUngroupedItems: TdxAutoLayoutItemList;
    I: Integer;
    AGroupBounds: TRect;
  begin
    AUngroupedItems := TdxAutoLayoutItemList.Create;
    try
      for I := 0 to ItemCount - 1 do
        AUngroupedItems.Add(Items[I]);
      ConvertGroup(AUngroupedItems, ASide, APreviousGroupBounds, AGroupBounds);
    finally
      AUngroupedItems.Free;
    end;
  end;
...
begin
  Result := (AControl <> Owner.Control) and not IsParent(AControl, Owner.Control) and
  ...
  for I := ItemCount - 1 downto 0 do
    ...
    begin
      ...
      begin
        ...
        if AItem <> nil then
        begin
          ALabel := Items[I].Control;
          AItem.SetLabelInfo(ALabel);
          ALabel.Free;
          Items.Extract(I);
        end;
      end;
...
begin
  ...
  FConvertCheckBoxes := False;
  FConvertRadioButtons := False;
end;
...
procedure ImportLayout(ALayoutControl: TdxLayoutControl; AContainer: TWinControl;
  AConvertPageControls, ASmartFindFocusControl, AUseLabeledItems, AConvertCheckBoxes, AConvertRadioButtons: Boolean;
...
begin
  ...
  try
    ...
    AAutoLayout.ConvertCheckBoxes := AConvertCheckBoxes;
    AAutoLayout.ConvertRadioButtons := AConvertRadioButtons;
...
var
  AImportForm: TfmImportControlsDialog;
begin
  if ALayoutControl = nil then Exit;
  AImportForm := TfmImportControlsDialog.Create(nil);
  try
    ...
    if ADefaultImport or (AImportForm.ShowModal = mrOk) then
      ImportLayout(ALayoutControl, AImportForm.GetImportContainer,
        AImportForm.cbConvertPageControls.Checked, AImportForm.cbAssociate.Checked,
        AImportForm.cbUseLabeledItems.Checked, AImportForm.cbConvertCheckBoxes.Checked,
        AImportForm.cbConvertRadioButtons.Checked, AImportForm.seDeltaX.Value, AImportForm.seDeltaY.Value);
  finally
    AImportForm.Free;
  end;
end;
...
function TfmImportControlsDialog.GetImportContainer: TWinControl;
begin
...
procedure TfmImportControlsDialog.PopulateContainersList(ALayoutControl: TdxCustomLayoutControl; ARootContainer: TWinControl);
  ...
  begin
    Result := (AContainer <> ALayoutControl) and
      ((ARootContainer = AContainer) or not IsParent(AContainer, ALayoutControl)) and
...
begin
  InternalPopulateContainersList(ARootContainer, cbContainers.Properties.Items, 0);
...
procedure TfmImportControlsDialog.cbAssociateClick(Sender: TObject);
begin
  seDeltaX.Enabled := cbAssociate.Checked;
  seDeltaY.Enabled := cbAssociate.Checked;
end;

procedure TfmImportControlsDialog.cbConvertCheckBoxesClick(Sender: TObject);
begin
end;

procedure TfmImportControlsDialog.cbConvertRadioButtonsClick(Sender: TObject);
begin
end;

procedure TfmImportControlsDialog.FormCreate(Sender: TObject);
begin
  seDeltaX.Value := 48;
  seDeltaY.Value := 16;
end;
