...
const
  dxThisUnitName = 'dxLayoutPainters';
...
procedure TdxLayoutGroupCaptionStandardPainter.DoDrawText(ACanvas: TcxCanvas);
  ...
  var
    AFlags: TTextFormatFlags;
    R: TRect;
  begin
    AFlags := TTextFormatFlags(cxFlagsToDTFlags(TdxLayoutGroupCaptionViewInfoAccess(ViewInfo).CalculateTextFlags(ACanvas)));
    R := ARect;
    cxStyleServices.DrawText(ADC, cxStyleServices.GetElementDetails(AElementMap[ViewInfo.Enabled]),
      TdxCustomLayoutItemCaptionViewInfoAccess(ViewInfo).FormattedText.GetDisplayText, R, AFlags);
  end;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        TdxLayoutGroupCaptionViewInfoAccess(ViewInfo).PrepareCanvas(ABitmap.cxCanvas, True);
        ...
        TdxImageDrawer.DrawUncachedImage(ACanvas.Handle, ATextRect, ATextRect, ABitmap, nil, -1, idmNormal);
      finally
        ABitmap.Free;
      end;
    end;
...
begin
  Result := (ViewInfo.TextColor = clDefault) and not TdxCustomLayoutItemCaptionViewInfoAccess(ViewInfo).IsTextFormatted
    and cxIsVCLThemesEnabled;
end;
...
begin
  AGroupViewInfo := GroupViewInfo(ViewInfo);
  AScaleFactor := (AGroupViewInfo.Group as IdxScaleFactor).Value;
  ...
  LookAndFeelPainter.DrawGroupBoxScaledFrame(ACanvas, AFrameBounds, ViewInfo.Enabled, ACaptionPosition, AScaleFactor);
  ...
  begin
    ...
    LookAndFeelPainter.DrawGroupBoxScaledCaption(ACanvas, ACaptionRect, cxNullRect, ACaptionPosition, AScaleFactor);
    ...
    begin
      ...
      try
        ...
        LookAndFeelPainter.DrawGroupBoxScaledCaption(ACanvas, ACaptionRect, ACaptionRect, ACaptionPosition, AScaleFactor);
      finally
