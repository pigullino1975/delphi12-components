...
uses
  ...
  cxLookAndFeels, cxLibraryConsts, cxLookAndFeelPainters, cxEdit, cxPC, dxFluentDesignFormInterfaces,
...
type
  ...
  {$ENDIF}
    ...
    property Painter: TdxLayoutControlPainter read FPainter; // for internal use
    property ViewInfo: TdxLayoutControlViewInfo read FViewInfo; // for internal use
  ...
  public
    ...
    property ItemsViewInfo: TdxLayoutGroupViewInfo read GetItemsViewInfo; // for internal use
  ...
  public
    ...
    procedure AddLayout(ALayout: TdxCustomLayoutControl); virtual;
    procedure RemoveLayout(ALayout: TdxCustomLayoutControl); virtual;

    property Count: Integer read GetCount;
    property Items[AIndex: Integer]: TdxCustomLayoutControl read GetItem;
  end;
  TdxDesignCustomizationHelperClass = class of TdxDesignCustomizationHelper;

var
  dxLayoutDesignTimeSelectionHelperClass: TdxLayoutRunTimeSelectionHelperClass;
  dxLayoutDesignCustomizationHelperClass: TdxDesignCustomizationHelperClass;
...
uses
  ...
  UxTheme, Themes,
  dxGDIPlusClasses, dxHooks, dxCoreGraphics, cxContainer, dxOffice11, dxShadowWindow, dxThreading,
  dxLayoutControlAdapters, dxLayoutCustomizeForm, dxLayoutStrs, dxLayoutDragAndDrop, dxDPIAwareUtils, dxCustomFluentDesignForm;

const
  dxThisUnitName = 'dxLayoutControl';
...
type
  ...
  TcxBitmapAccess = class(TcxBitmap);
  ...
  TdxLayoutControlCustomCustomizeFormAccess = class(TdxLayoutControlCustomCustomizeForm);
...
procedure TdxLayoutShadowPart.DrawShadow(ACanvas: TdxGpCanvas);
begin
  inherited;
  ACanvas.Rectangle(cxRectInflate(FOwnerRect, 1, 1), $BEBEBE, clNone, 1, psSolid, 60, 255);
end;
...
begin
  if (FDesignCustomizationHelper = nil) and not FUnitIsFinalized then
    FDesignCustomizationHelper := dxLayoutDesignCustomizationHelperClass.Create;
...
procedure TdxDesignCustomizationHelper.AddLayout(ALayout: TdxCustomLayoutControl);
begin
  if FLayoutControls.IndexOf(ALayout) = -1 then
    FLayoutControls.Add(ALayout);
end;

procedure TdxDesignCustomizationHelper.RemoveLayout(ALayout: TdxCustomLayoutControl);
begin
  FLayoutControls.Extract(ALayout);
end;
...
begin
  if IsDesigning and Supports(FindRootDesigner(AItem), IDesignerHook, ADesignerHook) then
    AItem.Name := ADesignerHook.UniqueName(AItem.ClassName)
  else
    inherited;
end;
...
const
  VisibilityChangeMap: array[Boolean] of TdxVisibilityType = (vtPartially, vtFully);
begin
  inherited;
...
begin
  ...
  ControlStyle := ControlStyle + [csAcceptsControls, csActionClient, csOpaque];
...
begin
  ...
  Customization := False;
  if dxDesignCustomizationHelper <> nil then
...
begin
  if not Container.IsCustomization and Container.IsChildKey(Message) and
      (Message.Sender.Perform(WM_GETDLGCODE, 0, 0) and DLGC_WANTTAB = 0) and
      Container.FocusController.SelectNext(Message.Sender, Message.CharCode) then
    Message.Result := 1
  else
    inherited;
end;
...
procedure TdxCustomLayoutControl.CMControlChange(var Message: TCMControlChange);
  ...
  function GetExpandedGroup(AGroup: TdxCustomLayoutGroup): TdxCustomLayoutGroup;
  begin
    if (AGroup = nil) or AGroup.Expanded then
      Result := AGroup
    else
      Result := GetExpandedGroup(AGroup.Parent);
  end;
...
begin
  ...
  if not (IsLoading or IsDestroying) and Message.Inserting and not IsInternalControl(AControl) and
    not (csAncestor in AControl.ComponentState) and (FindItem(AControl) = nil) and
    (not IsDesigning or not Container.IsUpdateLocked) then 
  begin
    if HandleAllocated then
    begin
      P := AControl.BoundsRect.TopLeft;
      AGroup := GetExpandedGroup(GetHitTest(P).GetGroupForInsert);
      if AGroup = nil then
        AGroup := Items;
    end
    else
    ...
    Inc(FLockModifiedCount);
    try
      if TdxLayoutItemAccess(AGroup).IsViewInfoValid then
      begin
        AIndex := AGroup.ViewInfo.GetInsertionPos(P);
        AGroup.CreateItemForControl(AControl).VisibleIndex := AIndex;
      end
      else
        AGroup.CreateItemForControl(AControl);
    finally
      Dec(FLockModifiedCount);
    end;
  end;
end;
...
begin
  dxMessagesController.KillMessages(Handle, DXM_LAYOUT_BUILDSELECTIONLAYER);
...
begin
  ...
  begin
    ...
    if CanAutoWidth then
      NewWidth := ContentBounds.Width + GetBorderSizes.Width;
    if CanAutoHeight then
      NewHeight := ContentBounds.Height + GetBorderSizes.Height;
  end
  else
    Result := False
end;
...
procedure TdxCustomLayoutControl.CreateWindowHandle(const Params: TCreateParams);
begin
  inherited;
...
function TdxCustomLayoutControl.FluentDesignContainerControlCanBeOpaque: Boolean;
begin
...
procedure TdxCustomLayoutControl.InitiateAction;
begin
  inherited InitiateAction;
  TdxCustomLayoutGroupAccess(Items).InitiateAction;
end;
...
procedure TdxCustomLayoutControl.Modified;
begin
  if FLockModifiedCount = 0 then
    inherited;
end;
...
begin
  if not IsLoading and not IsDestroying then
    ...
    begin
      ...
      if TcxBitmapAccess(FLockedStateImage).IsDormant then
...
procedure TdxCustomLayoutControl.ScrollPosChanged(const AOffset: TPoint);
  ...
  procedure HideUnscrolledControls;
  begin
    ShowWindow(HScrollBar.Control.Handle, SW_HIDE);
    ShowWindow(VScrollBar.Control.Handle, SW_HIDE);
    ShowWindow(SizeGrip.Handle, SW_HIDE);
  end;

  procedure ShowUnscrolledControls;
  begin
    if HScrollBar.Visible then
      ShowWindow(HScrollBar.Control.Handle, SW_SHOW);
    if VScrollBar.Visible then
      ShowWindow(VScrollBar.Control.Handle, SW_SHOW);
    if SizeGrip.Visible then
      ShowWindow(SizeGrip.Handle, SW_SHOW);
  end;
...
{ T1008108
  ...
  begin
    Container.PlaceControls(Container.ViewInfo.RootViewInfo, True, SWP_NOREDRAW or SWP_NOSIZE);
    UpdateScrollBars;
    cxRedrawWindow(Handle, RDW_ALLCHILDREN or RDW_INVALIDATE);
    PostBuildSelectionLayer;
  end
  else
}
  begin
    if not IsPopupScrollBars then
      HideUnscrolledControls;
    try
      dxPrepareActiveControlForScrolling(GetParentForm(Self));
      R := Container.ViewInfo.ClientBounds;
      Windows.ScrollWindowEx(Handle, AOffset.X, AOffset.Y, nil, @R, 0, nil, SW_INVALIDATE or SW_ERASE or SW_SCROLLCHILDREN);
      Container.PlaceControls(Container.ViewInfo.RootViewInfo, False);
    finally
      UpdateScrollBars;
      if not IsPopupScrollBars then
        ShowUnscrolledControls; 
    end;
    UpdateWindow(Handle);
  end;
  if Container.IsCustomization and (Container.ViewInfo.SelectionLayer <> nil) and Container.ViewInfo.SelectionLayer.HasIntersection then
    PostBuildSelectionLayer; 
end;
...
function TdxCustomLayoutControl.CanStartDragAndDrop(Button: TMouseButton; Shift: TShiftState; X, Y: Integer): Boolean;
begin
  Result := ([ssCtrl, ssShift] * Shift = []) and inherited;
end;
...
begin
  if HandleAllocated and (Container <> nil) and (Container.MasterContainer = nil) and
    (Container.CustomizeForm <> nil) and (TdxLayoutControlCustomCustomizeFormAccess(Container.CustomizeForm).LockCount = 0) then 
...
procedure TdxCustomLayoutControl.AcrylicHostControlChanged(
  AValue: IdxAcrylicHostControl; AHasAcrylicParent: Boolean);
begin
  LayoutChanged;
end;
...
begin
  Result := (TdxLayoutDragAndDropState.DragAndDropObject = nil) and (TdxLayoutDragAndDropState.SizingDragAndDropObject = nil) and not FIsPopupShown;
end;
...
procedure TdxLayoutControlPainter.DrawContent(ACanvas: TcxCanvas);
  ...
  begin
    if Control.Container.IsDesignSelectorsVisible then
      TdxLayoutItemSelectorHelper.DrawDesignSelectorRect(ACanvas.Handle, Control.GetDesignSelectorRect, Control.Container.IsComponentSelected(Control));
  end;
...
initialization
  ...
  dxLayoutDesignCustomizationHelperClass := TdxDesignCustomizationHelper;
