...
const
  dxThisUnitName = 'dxLayoutLookAndFeels';
...
begin
  ...
  begin
    with TdxLayoutLookAndFeelCaptionOptions(Source) do
    begin
      Self.Font := Font;
...
var
  AIndent: Integer;
begin
  if cxRectIsNull(GroupViewInfo(AViewInfo).FFrameBounds) then
  begin
    Result := GroupViewInfo(AViewInfo).Bounds;
    ...
    GroupViewInfo(AViewInfo).FFrameBounds := Result;
  end
  else
    Result := GroupViewInfo(AViewInfo).FFrameBounds;
end;
...
begin
  ...
  Unregister(Sender as TFont);
  dxCallNotify((Sender as TFont).OnChange, Sender);
end;
...
begin
  ...
  if AIndex <> -1 then
  begin
    Items[AIndex].Font.OnChange := Items[AIndex].PrevFontChangeHandler;
    Delete(AIndex);
  end;
end;
...
begin
  Result := LookAndFeel.LookAndFeelPainter.DefaultLabelTextColorEx(esckNormal);
end;

function TdxLayoutCxLookAndFeelItemCaptionOptions.GetDefaultTextDisabledColor: TColor;
begin
  Result := LookAndFeel.LookAndFeelPainter.DefaultLabelTextColorEx(esckDisabled);
end;
...
begin
  dxFreeGlobalObject(dxLayoutDefaultLookAndFeel);
  ...
  dxFreeGlobalObject(dxLayoutLookAndFeelDefs);
  dxFreeGlobalObject(dxLayoutTextMetrics);
end;
