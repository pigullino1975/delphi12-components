...
uses
  ...
  cxLibraryReg, dxSortedFieldsEditor;

const
  dxThisUnitName = 'dxmdsreg';
...
{ TdxSortedFieldsPropertyEditor }
type
  TdxSortedFieldsPropertyEditor = class(TStringProperty)
  public
    procedure Edit; override;
    function GetAttributes: TPropertyAttributes; override;
...
procedure TdxSortedFieldsPropertyEditor.Edit;
begin
  EditSortedFields(GetComponent(0) as TdxMemData);
end;

function TdxSortedFieldsPropertyEditor.GetAttributes: TPropertyAttributes;
begin
  Result := [paDialog];
end;

function TdxSortedFieldsPropertyEditor.GetValue: string;
begin
  Result := (GetComponent(0) as TdxMemData).SortedFields;
end;
...
begin
  ...
  RegisterPropertyEditor(TypeInfo(string), TdxMemData, 'SortedFields',
     TdxSortedFieldsPropertyEditor);

  HideClassProperties(TdxMemData, ['SortedField']);
end;
