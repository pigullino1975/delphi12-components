...
uses
  ...
  dxSkinsCore, dxSkinInfo, cxLookAndFeelPainters, dxCoreClasses, cxControls, dxNavBarOverflowPanel, dxCoreGraphics;
...
type
  ...
  protected
    ...
    function TryCreateSkinInfo(var AData: TdxSkinInfo): Boolean;
  ...
  protected
    ...
    // Drawing
    function InternalCreateOverflowPainter: TdxNavBarOverflowPanelPainter; override;
  ...
  { TdxNavBarSkinBasedOverflowPanelPainter }

  TdxNavBarSkinBasedOverflowPanelPainter = class(TdxNavBarOverflowPanelPainter)
  private
    FHelper: TdxNavBarSkinBasedPainterHelper;
  protected
    ...
    function GetItemColorPalette(AItem: TdxNavBarOverflowPanelViewInfoItem): IdxColorPalette; override;

    function GetItemElement: TdxSkinElement; virtual;
    function IsActiveGroup(APanel: TdxNavBarCustomOverflowPanelViewInfo; AItemIndex: Integer): Boolean;

    function GetPartElementState(APanel: TdxNavBarCustomOverflowPanelViewInfo; const APart: TdxNavBarPart): TdxSkinElementState;
    function GetSkinElementState(APanel: TdxNavBarCustomOverflowPanelViewInfo; AIndex: Integer): TdxSkinElementState;

    property Helper: TdxNavBarSkinBasedPainterHelper read FHelper;
  public
    constructor Create(AHelper: TdxNavBarSkinBasedPainterHelper); reintroduce;
    ...
    procedure DrawPopupMenuItem(APanel: TdxNavBarCustomOverflowPanelViewInfo; ACanvas: TCanvas; ARect: TRect;
      AImageList: TCustomImageList; AImageIndex: Integer; AText: String; AState: TdxNavBarObjectStates); override;
  end;
...
function NavBarObjectStateToSkinState(AState: TdxNavBarObjectStates): TdxSkinElementState; overload;
function NavBarObjectStateToSkinState(AState: TdxNavBarNavPanePartState): TdxSkinElementState; overload;

implementation

uses
  Math, dxTypeHelpers, dxNavBarGraphics, dxDPIAwareUtils;

const
  dxThisUnitName = 'dxNavBarSkinBasedViews';
...
type
  ...
  TdxNavBarNavigationPaneViewInfoAccess = class(TdxNavBarNavigationPaneViewInfo);
  TdxNavBarViewInfoAccess = class(TdxNavBarViewInfo);
  TdxNavBarPopupControlViewInfoAccess = class(TdxNavBarPopupControlViewInfo);
...
function NavBarObjectStateToSkinState(AState: TdxNavBarNavPanePartState): TdxSkinElementState;
const
...
function TdxNavBarSkinBasedPainterHelper.TryCreateSkinInfo(var AData: TdxSkinInfo): Boolean;
begin
...
function TdxNavBarSkinBasedPainterHelper.NavBarChildGroup: TdxSkinElement;
begin
  Result := nil;
end;
...
begin
  ...
  if AElement <> nil then
    NavBar.DefaultStyles.NavigationPaneHeader.Font.Height := dxGetFontHeight(AElement.Value, ScaleFactor);
end;
...
begin
  Result := NavBarObjectStateToSkinState(GetNavPanePartState(APart));
end;
...
function TdxNavBarSkinBasedNavPanePainter.InternalCreateOverflowPainter: TdxNavBarOverflowPanelPainter;
begin
  Result := TdxNavBarSkinBasedOverflowPanelPainter.Create(FSkinBasedPainterHelper);
end;
...
begin
  if not DrawSkinElement(FSkinBasedPainterHelper.NavPaneItemPanel, Canvas, ARect, ScaleFactor,
    0, NavBarObjectStateToSkinState(AState)) then
    inherited;
end;
...
begin
  ...
  begin
    ...
    try
//      ACanvas.ExcludeClipRect(AViewInfo.ClientRect);  //#Ch remove because of draw distortion when RightToLeft element transform in some skins (e.g. Office2007)
      AElement.RightToLeftDependentDraw(ACanvas, AViewInfo.Rect, ScaleFactor, TdxNavBarPopupControlViewInfoAccess(AViewInfo).ViewInfo.GetExpandDirection = dirLeft);
    finally
...
begin
  TdxCustomNavBarAccess(NavBar).PerformWithAnimationLocking(
    procedure
    begin
      NavBar.AssignDefaultStyles;
      NavBar.InvalidateAll(doRecreate);
...
function TdxNavBarSkinBasedExplorerBarPainter.GetGroupColorPalette(AGroupViewInfo: TdxNavBarGroupViewInfo): IdxColorPalette;
begin
  if FSkinBasedPainterHelper.NavBarGroupButtonCaption <> nil then
    Result := FSkinBasedPainterHelper.NavBarGroupButtonCaption.GetGlyphColorPalette(NavBarObjectStateToSkinState(AGroupViewInfo.State))
  else
    Result := nil;
end;

function TdxNavBarSkinBasedExplorerBarPainter.GetItemColorPalette(ALinkViewInfo: TdxNavBarLinkViewInfo): IdxColorPalette;
begin
...
procedure TdxNavBarSkinBasedExplorerBarGroupViewInfo.DoOffsetContent;
begin
  with TdxNavBarSkinBasedExplorerBarPainter(Painter) do
  begin
    if FSkinBasedPainterHelper.NavBarGroupButtonCaption <> nil then
    begin
      if not FImageRect.IsEmpty then
        OffsetRect(FImageRect, FSkinBasedPainterHelper.NavBarGroupButtonCaption.ContentOffset.Left, 0);
      Inc(FCaptionTextRect.Left, FSkinBasedPainterHelper.NavBarGroupButtonCaption.ContentOffset.Left);
    end;
  end;
end;
...
begin
  with TdxNavBarSkinBasedExplorerBarPainter(Painter) do
    if IsCaptionVisible and not IsRectEmpty(FImageRect) then
      DoOffsetContent;
end;

{ TdxNavBarSkinBasedOverflowPanelPainter }

constructor TdxNavBarSkinBasedOverflowPanelPainter.Create(AHelper: TdxNavBarSkinBasedPainterHelper);
begin
  inherited Create;
  FHelper := AHelper;
end;

procedure TdxNavBarSkinBasedOverflowPanelPainter.DrawBackground(ACanvas: TCanvas;
  APanel: TdxNavBarCustomOverflowPanelViewInfo);
begin
  if not DrawSkinElement(FHelper.NavPaneOverflowPanel, ACanvas, APanel.Rect, APanel.ScaleFactor) then
    inherited DrawBackground(ACanvas, APanel);
end;

procedure TdxNavBarSkinBasedOverflowPanelPainter.DrawSign(ACanvas: TCanvas;
  APanel: TdxNavBarCustomOverflowPanelViewInfo);
begin
  if not DrawSkinElement(FHelper.NavPaneOverflowPanelSign, ACanvas, APanel.SignRect,
    APanel.ScaleFactor, 0, GetPartElementState(APanel, dxNavBarPart(nbOverflowPanelSign)), False,
    APanel.NavBar.UseRightToLeftAlignment)
  then
    inherited DrawSign(ACanvas, APanel);
end;

procedure TdxNavBarSkinBasedOverflowPanelPainter.DrawPopupMenuItem(APanel: TdxNavBarCustomOverflowPanelViewInfo;
...
var
  ...
  begin
    APopupMenu := FHelper.NavPanePopupMenu;
    ASelected := FHelper.NavPanePopupMenuSelection;
    ASeparator := FHelper.NavPanePopupMenuSeparator;
    ASideStrip := FHelper.NavPanePopupMenuSideStrip;
  ...
  begin
    ...
    R := cxRectInflate(R, -APanel.PopupMenuImageIndent);
    if APanel.NavBar.UseRightToLeftAlignment then
  ...
  begin
    ...
    R.Left := AImageAreaWidth + APanel.PopupMenuTextIndent;
    ...
    if sDisabled in AState then
    begin
      FHelper.GetSkinPainterData(ASkinInfo); 
      ACanvas.Font.Color := ASkinInfo.BarDisabledTextColor.Value;
    end
    else
      ACanvas.Font.Color := APopupMenu.TextColor;


    cxDrawText(ACanvas, AText, R, APanel.NavBar.DrawTextBiDiModeFlags(DT_LEFT or DT_VCENTER or DT_SINGLELINE));
  end;
  ...
  begin
    ...
    if APanel.NavBar.UseRightToLeftAlignment then
      R := TdxRightToLeftLayoutConverter.ConvertRect(R, ARect);
    ASideStrip.RightToLeftDependentDraw(ABuf.Canvas.Handle, R, dxSystemScaleFactor, APanel.NavBar.UseRightToLeftAlignment);
  end;

begin
  if not InitializeElements then
    inherited DrawPopupMenuItem(APanel, ACanvas, ARect, AImageList, AImageIndex, AText, AState)
  else
  begin
    AImageAreaWidth := 2 * APanel.PopupMenuImageIndent + TdxNavBarViewInfoAccess(APanel.NavBar.ViewInfo).GetSmallImageWidth;
    ...
    try
      ...
      begin
        ...
        DrawImage(ABuf.Canvas, sActive in AState, AImageAreaWidth);
...
procedure TdxNavBarSkinBasedOverflowPanelPainter.DrawOverflowPanelItemBackground(ACanvas: TCanvas;
  APanel: TdxNavBarCustomOverflowPanelViewInfo; AItemIndex: Integer; AState: TdxNavBarPartState; const ARect: TRect);
var
  AElement: TdxSkinElement;
begin
  AElement := GetItemElement;
  if AElement <> nil then
    AElement.Draw(ACanvas.Handle, ARect, APanel.ScaleFactor, Byte(IsActiveGroup(APanel, AItemIndex)),
      NavBarObjectStateToSkinState(AState))
  else
    inherited DrawOverflowPanelItemBackground(ACanvas, APanel, AItemIndex, AState, ARect);
end;

function TdxNavBarSkinBasedOverflowPanelPainter.GetItemColorPalette(AItem: TdxNavBarOverflowPanelViewInfoItem): IdxColorPalette;
var
  AElement: TdxSkinElement;
begin
  AElement := GetItemElement;
  if AElement <> nil then
    ...
    Result := inherited GetItemColorPalette(AItem);
end;

function TdxNavBarSkinBasedOverflowPanelPainter.GetItemElement: TdxSkinElement;
begin
  Result := FHelper.NavPaneOverflowPanelItem;
end;

function TdxNavBarSkinBasedOverflowPanelPainter.IsActiveGroup(APanel: TdxNavBarCustomOverflowPanelViewInfo;
  AItemIndex: Integer): Boolean;
begin
  Result := InRange(AItemIndex, 0, APanel.ItemCount - 1) and (APanel.Items[AItemIndex].Group = APanel.NavBar.ActiveGroup)
end;

function TdxNavBarSkinBasedOverflowPanelPainter.GetPartElementState(APanel: TdxNavBarCustomOverflowPanelViewInfo;
  const APart: TdxNavBarPart): TdxSkinElementState;
begin
  Result := NavBarObjectStateToSkinState(TdxNavBarControllerAccess(APanel.NavBar.Controller).GetPartState(APart));
end;

function TdxNavBarSkinBasedOverflowPanelPainter.GetSkinElementState(APanel: TdxNavBarCustomOverflowPanelViewInfo;
  AIndex: Integer): TdxSkinElementState;
begin
  Result := GetPartElementState(APanel, dxNavBarPart(nbOverflowPanelItem, AIndex));
  case Result of
    esChecked:
      Result := esNormal;
    esHotCheck:
