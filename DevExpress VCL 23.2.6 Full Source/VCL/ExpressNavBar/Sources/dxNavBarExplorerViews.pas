...
uses
  ...
  cxClasses, cxControls, cxGeometry, dxTypeHelpers;
...
type
  ...
  protected
    procedure CorrectGroupPositions; virtual;
    ...
    function GetScrollContentHeight: Integer; virtual;
...
uses
  Types, CommCtrl, ImgList, Math, cxGraphics, dxOffice11, dxNavBarViewsFact, dxNavBarConsts, dxNavBarGraphics, dxCore;

const
  dxThisUnitName = 'dxNavBarExplorerViews';
...
procedure TdxNavBarExplorerBarViewInfo.CorrectGroupPositions;
var
  I: Integer;
begin
  for I := 0 to GroupCount - 1 do
...
begin
  ...
  begin
    AHeight := GetScrollContentHeight;
    AClientHeight := cxRectHeight(GroupsArea);
    if (AHeight > AClientHeight) and (AHeight > 0) then
    begin
      ...
      if TdxNavBarAccess(NavBar).IsScrollBarUseClientArea then
      begin
        if NavBar.UseRightToLeftScrollBar then
          ALeft := ClientRect.Left
        else
          ALeft := ClientRect.Right - NavBar.ScrollBar.Width;
        R := cxRectBounds(ALeft, GroupsArea.Top, NavBar.ScrollBar.Width, AClientHeight);
...
function TdxNavBarExplorerBarViewInfo.GetScrollContentHeight: Integer;
begin
  Result := Groups[GroupCount - 1].Rect.Bottom + GetGroupEdges.Y - HeaderRect.Bottom;
end;
...
begin
  ...
  begin
    ...
    begin
      ADisplacement := 0;
      if ATop or (ALinkViewInfo.Rect.Top < GroupsArea.Top) then
        ADisplacement := ALinkViewInfo.Rect.Top - GroupsArea.Top - 2
      else
        if ALinkViewInfo.Rect.Bottom > GroupsArea.Bottom then
          ADisplacement := ALinkViewInfo.Rect.Bottom - GroupsArea.Bottom;
      NavBar.ScrollBar.UpdatePosition(NavBar.ScrollBar.Position + ADisplacement, TdxNavBarAccess(NavBar).IsScrollAnimationEnabled);
    end;
  end;
end;
...
begin
  ...
  begin
    ADisplacement := 0;
    if ATop or (AGroupViewInfo.CaptionRect.Top < GroupsArea.Top) then
      ADisplacement := AGroupViewInfo.Rect.Top - GroupsArea.Top - 2
    else
    begin
      AMakeVisibleBottomEdge := AGroupViewInfo.CaptionRect.Bottom;
      if AMakeVisibleBottomEdge > GroupsArea.Bottom then
        ADisplacement := AMakeVisibleBottomEdge - GroupsArea.Bottom;
    end;
    NavBar.ScrollBar.UpdatePosition(NavBar.ScrollBar.Position + ADisplacement, TdxNavBarAccess(NavBar).IsScrollAnimationEnabled);
  end;
end;
...
begin
  ...
  begin
    ...
    if AGroupViewInfo <> nil then
      if (cxRectHeight(ClientRect) - cxRectHeight(HeaderRect) < cxRectHeight(AGroupViewInfo.Rect)) or
        (AGroupViewInfo.Rect.Top - GroupsArea.Top - NavBar.ScrollBar.Position < GetSpaceBetweenGroups div 2) then
      begin
        ADisplacement := AGroupViewInfo.Rect.Top - GroupsArea.Top - GetSpaceBetweenGroups div 2;
        NavBar.ScrollBar.Position := ADisplacement;
      end
      else
      begin
        AMakeVisibleBottomEdge := AGroupViewInfo.Rect.Bottom - GroupsArea.Top + GetSpaceBetweenGroups div 2;
        if AMakeVisibleBottomEdge - NavBar.ScrollBar.Position > ClientRect.Bottom then
        begin
          ADisplacement := AMakeVisibleBottomEdge - ClientRect.Bottom;
          NavBar.ScrollBar.Position := ADisplacement;
        end;
      end;
    NavBar.Groups.JustExpandedGroup := nil;
...
begin
  if GetCaptureControl = NavBar then
