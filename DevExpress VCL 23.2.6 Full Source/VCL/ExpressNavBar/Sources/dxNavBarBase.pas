...
type
  ...
  TdxNavBarChangeEvent = procedure(Sender: TObject; AType: TdxNavBarChangeType) of object; // for internal use
  ...
  IdxNavBarAccessibilityHelper = interface(IcxAccessibilityHelper) // for internal use
  ...
  TdxNavBarCustomAccessibilityHelper = class(TcxAccessibilityHelper, IdxNavBarAccessibilityHelper) // for internal use
  strict private
  ...
  protected
    ...
    function InternalGetNextObjectForNavigation(ADirection: TcxDirection): IdxNavBarAccessibilityHelper; virtual;
    ...
    function IsScrollable(out AInvisiblePartHeight, AChildMinTopScreenBound: Integer): Boolean; virtual;
    procedure MakeVisible; virtual;
    procedure SetIsPressed(Value: Boolean);

    property AssociatedObject: TdxNavBarCustomAccessibilityHelper read GetAssociatedObject;
    property LocalId: Integer read FLocalId;
    property OwnerObjectControl: TWinControl read FOwnerObjectControl;
    property Tag: Integer read FTag write FTag;
  public
    constructor Create(AOwnerObject: TObject; AOwnerObjectControl: TWinControl); reintroduce; virtual;
    destructor Destroy; override;
...
procedure NavBarAccessibleObjectOwnerObjectDestroyed(var AAccessibleObject: IdxNavBarAccessibilityHelper);
function NavBarGetAccessibilityHelper(AObject: TObject): IdxNavBarAccessibilityHelper;
function NavBarGetFocusableAccessibleObjectAtPos(
  ARootAccessibleObject: IdxNavBarAccessibilityHelper; const P: TPoint): IdxNavBarAccessibilityHelper;
...
procedure NavBarAccessibleObjectOwnerObjectDestroyed(var AAccessibleObject: IdxNavBarAccessibilityHelper);
begin
  cxAccessibilityHelperOwnerObjectDestroyed(AAccessibleObject);
end;
...
function GetNearestFocusableAccessibleObject(AFocusableAccessibleObjectList: TList;
  ACurrentObject: TdxNavBarCustomAccessibilityHelper;
  var ACurrentObjectScreenBounds: TRect;
...
begin
  ...
  begin
    ...
    AObjectScreenBounds := AObject.GetScreenBounds(cxAccessibleObjectSelfID);
    OffsetRect(AObjectScreenBounds, 0, AObject.Tag);
    AMinDistance := Min(AMinDistance, GetDistance(ACurrentObjectScreenBounds, AObjectScreenBounds, ADirection));
  end;
  I := 0;
  ...
  if AFocusableAccessibleObjectList.Count = 0 then
    ...
    begin
      ReplaceContainerWithItsFocusableChildren(AFocusableAccessibleObjectList, 0, ACurrentObject, ACurrentObjectScreenBounds);
...
procedure ReplaceContainerWithItsFocusableChildren(AFocusableAccessibleObjectList: TList; AContainerIndex: Integer;
  ACurrentObject: TdxNavBarCustomAccessibilityHelper; var ACurrentObjectScreenBounds: TRect);
var
  ...
  AObject: TdxNavBarCustomAccessibilityHelper;
begin
  AContainer := TdxNavBarCustomAccessibilityHelper(AFocusableAccessibleObjectList[AContainerIndex]);
  ...
  if (ACurrentObject <> nil) and AContainer.IsScrollable(AContainerInvisiblePartHeight, AContainerChildMinTopScreenBound) then
  begin
    for I := 0 to AFocusableAccessibleObjectList.Count - 1 do
    begin
      AObject := TdxNavBarCustomAccessibilityHelper(AFocusableAccessibleObjectList[I]);
    ...
    AContainerChildTopBoundOffset := AContainerScreenBounds.Top - AContainerChildMinTopScreenBound + AContainer.Tag;
  end
  else
...
//
  ...
  FLocalId := TdxCustomNavBarAccess(OwnerObjectControl).FAccessibleObjects.Count + 1;
  TdxCustomNavBarAccess(OwnerObjectControl).FAccessibleObjects.Add(Self);
...
begin
  ...
  TdxCustomNavBarAccess(OwnerObjectControl).AccessibleObjectOwnerObjectDestroyedNotification(Self);
end;

procedure TdxNavBarCustomAccessibilityHelper.DoDefaultAction(AChildID: TcxAccessibleSimpleChildElementID);
begin
  if CanFocus(False) then
    TdxCustomNavBarAccess(OwnerObjectControl).FocusedAccessibleObject := Self;
end;

function TdxNavBarCustomAccessibilityHelper.GetChild(AIndex: Integer): TcxAccessibilityHelper;
begin
...
function TdxNavBarCustomAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
var
  AHandle: HWND;
begin
...
// IdxNavBarAccessibilityHelper
procedure TdxNavBarCustomAccessibilityHelper.AttachChild(AChild: IdxNavBarAccessibilityHelper);
begin
// Requires
...
function TdxNavBarCustomAccessibilityHelper.CanFocus(AFocusingByDefault: Boolean): Boolean;
...
begin
  Result := not (csDestroying in OwnerObjectControl.ComponentState) and
    (States[cxAccessibleObjectSelfID] and (cxSTATE_SYSTEM_INVISIBLE or cxSTATE_SYSTEM_UNAVAILABLE) = 0) and
    (not IsContainer and (not AFocusingByDefault or CanBeFocusedByDefault) or IsContainer and HasFocusableChild);
end;

procedure TdxNavBarCustomAccessibilityHelper.DetachChild(AChild: IdxNavBarAccessibilityHelper);
begin
// Requires
  Assert(IsContainer and (AChild <> nil));
//
...
procedure TdxNavBarCustomAccessibilityHelper.FocusedChanged(AMakeVisible: Boolean);
begin
  TdxCustomNavBar(OwnerObjectControl).InvalidateAll(doRecalc);
  if IsFocused then
  begin
    if AMakeVisible then
      if NeedMakeVisible then
        MakeVisible
      else
        TdxCustomNavBarAccess(OwnerObjectControl).StopScrollAnimation;
    NotifyWinEvent(EVENT_OBJECT_FOCUS, OwnerObjectControl.Handle, FLocalId, CHILDID_SELF);
  end
  else
    SetIsPressed(False);
end;
...
function TdxNavBarCustomAccessibilityHelper.GetNextObjectForNavigation(ADirection: TcxDirection): IdxNavBarAccessibilityHelper;
begin
// Requires
  Assert((Parent = nil) and (ADirection in [dirNone, dirDown]) or (Parent <> nil) and (ADirection <> dirNone) and not IsContainer);
...
var
  AClipBounds, AGroupsAreaClipBounds: TRect;
begin
  AClipBounds := cxGetWindowRect(TdxCustomNavBar(OwnerObjectControl));
  IntersectRect(AClipBounds, AClipBounds, GetClipBounds);
  AGroupsAreaClipBounds := TdxNavBarViewInfoAccess(TdxCustomNavBar(OwnerObjectControl).ViewInfo).GroupsArea;
  AGroupsAreaClipBounds := cxRectOffset(AGroupsAreaClipBounds, OwnerObjectControl.ClientOrigin);
  IntersectRect(AClipBounds, AClipBounds, AGroupsAreaClipBounds);
  Result := not cxRectContain(AClipBounds, GetScreenBounds(cxAccessibleObjectSelfID));
end;
...
begin
  ...
  Result := GetNearestFocusableAccessibleObject(GetRootHelper as TdxNavBarCustomAccessibilityHelper,
    ACurrentObject, ACurrentObjectScreenBounds, ADirection);
end;

function TdxNavBarCustomAccessibilityHelper.IsChild(AChild: TdxNavBarCustomAccessibilityHelper): Boolean;
begin
// Requires
  Assert(AChild <> nil);
//
...
function TdxNavBarCustomAccessibilityHelper.IsScrollable(out AInvisiblePartHeight, AChildMinTopScreenBound: Integer): Boolean;
begin
  Result := False;
end;
...
function TdxNavBarCustomAccessibilityHelper.InternalGetChild(AIndex: Integer): TdxNavBarCustomAccessibilityHelper;
begin
