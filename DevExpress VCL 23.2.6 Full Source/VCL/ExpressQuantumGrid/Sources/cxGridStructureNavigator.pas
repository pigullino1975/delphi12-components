...
uses
  ...
  cxGridCommon, cxGrid, cxGridLevel, cxGridCustomView, cxViewEditor, dxCore;
...
type
  ...
  private
    function GetItem(Index: TdxListIndex): TcxGridStructureControlRowViewInfo;
  public
    property Items[Index: TdxListIndex]: TcxGridStructureControlRowViewInfo read GetItem; default;
  end;
...
const
  dxThisUnitName = 'cxGridStructureNavigator';
...
begin
  ...
  for I := 0 to cxGridRegisteredViews.Count - 1 do
    if (Pos(SGridDBViewPrefix, cxGridRegisteredViews.Descriptions[I]) <> 1) and
       (Pos(SGridServerModeViewPrefix, cxGridRegisteredViews.Descriptions[I]) <> 1) and
       (Pos(SGridEMFViewPrefix, cxGridRegisteredViews.Descriptions[I]) <> 1) then
    begin
...
function FillEMFRegisteredViewsMenu(AMenu: TMenuItem; AOnClick: TNotifyEvent;
  ACheckAddSeparator: Boolean): Boolean;
var
  I: Integer;
begin
  Result := False;
  for I := 0 to cxGridRegisteredViews.Count - 1 do
    if Pos(SGridEMFViewPrefix, cxGridRegisteredViews.Descriptions[I]) = 1 then
    begin
      if not Result and ACheckAddSeparator then
        AMenu.Add(CreateMenuItem(AMenu.Owner, '-'));
      AMenu.Add(CreateMenuItem(AMenu.Owner, cxGridRegisteredViews.Descriptions[I],
...
begin
  ...
  ACheckAddSeparator := FillServerModeRegisteredViewsMenu(AMenu, AOnClick, ACheckAddSeparator);
  FillEMFRegisteredViewsMenu(AMenu, AOnClick, ACheckAddSeparator);
end;
...
function TcxGridStructureControlRowsViewInfo.GetItem(Index: TdxListIndex): TcxGridStructureControlRowViewInfo;
begin
...
begin
  Result := StructureControl.ActiveCanvas;
end;
