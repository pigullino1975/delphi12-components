...
const
  dxThisUnitName = 'cxGridGroupRowPopupMenuItems';
...
begin
  Result := (AHitTest is TcxGridRecordHitTest) and not TcxGridRecordHitTest(AHitTest).GridRecord.IsData;
  if Result then
  begin
    GroupRow := TcxGridGroupRow(TcxGridRecordHitTest(AHitTest).GridRecord);
    Result := inherited CanProcess(AHitTest);
  end;
end;
