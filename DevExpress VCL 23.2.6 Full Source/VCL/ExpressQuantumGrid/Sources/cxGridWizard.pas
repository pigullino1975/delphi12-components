...
uses
  cxGridWizardCommonWizardModePage, cxGridWizardCommonSelectViewPage,
  cxGridWizardCommonFinishPage, cxDWMApi, cxGeometry, Math, dxMessageDialog;

const
  dxThisUnitName = 'cxGridWizard';
...
begin
  ...
  if FIsNextLevelNeeded then
    FIsNextLevelNeeded := dxMessageDlg(cxGetResourceString(@scxgwCommonCreateDetailLevelQuery), mtConfirmation, [mbYes, mbNo], 0) = mrYes;
end;
...
begin
  if ModalResult = mrCancel then
    CanClose := dxMessageDlg(cxGetResourceString(@scxgwCommonCloseQuery), mtWarning, [mbYes, mbNo], 0) = mrYes;
end;
