...
{$ENDIF}
  ...
  cxGridLevel,
  cxGridViewLayoutEditor, dxCore;

const
  dxThisUnitName = 'cxViewEditor';
...
begin
  if (FView = Value) and UpdateLocked then
    Exit;
  FView := Value;
...
procedure TcxViewEditor.LockUpdate;
begin
  Inc(FLockUpdate);
end;
...
procedure TcxViewEditor.UnlockUpdate;
begin
  Dec(FLockUpdate);
end;
...
function TcxViewEditor.GetUpdateLocked: Boolean;
begin
...
begin
  ...
  try
    ...
    begin
      ACanAddComponent := CanAddComponent(DBGridView.Owner);
      if ACanAddComponent then
