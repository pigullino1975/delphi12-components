...
type
  ...
  protected
    ...
    property SeriesImages: TcxImageList read FSeriesImages;
  public
  ...
  TcxGridChartCustomizationController = class(TcxGridCustomizationController)
  strict private
    function GetViewInfo: TcxGridChartViewInfo;
  protected
...
//  function GetTransparentValues: Boolean; override;
  ...
  private
    ...
    FItemOffset: Integer;
    FItems: TList;
    FItemKeyHeight: Integer;
    FItemKeyWidth: Integer;
...
//  TcxGridChartEmptyPointLineStyle = (eplsDefault, eplsNone, eplsSolid, eplsDash, eplsDot, eplsDashDot, eplsDashDotDot);
  ...
  protected
    ...
    procedure SetGridView(Value: TcxGridChartView); virtual;
  ...
  protected
    ...
    function HasNullGroupValue: Boolean;
  ...
  public
    constructor Create(AOwner: TPersistent); override;
    procedure Assign(Source: TPersistent); override;
    procedure GetValueParams(AValueIndex: Integer; AVaryColorsByCategory: Boolean; out AParams: TcxViewParams); virtual;
    property Series: TcxGridChartSeries read GetSeries;
  published
    property Values: TcxStyle index ssValues read GetValue write SetValue;
    property OnGetValueStyle: TcxGridChartSeriesGetValueStyleEvent read FOnGetValueStyle write SetOnGetValueStyle;
  end;
  ...
  protected
    ...
    function GetVisibleValue(AIndex: Integer): Variant; override;
    function GetVisibleValueCount: Integer; override;
    procedure SetGridView(Value: TcxGridChartView); override;
    procedure SetValue(AIndex: Integer; const Value: Variant); override;
    procedure SetValueCount(Value: Integer); override;
  ...
  private
    function GetItem(Index: TdxListIndex): TcxGridChartItem;
    procedure SetItem(Index: TdxListIndex; Value: TcxGridChartItem);
  public
    property Items[Index: TdxListIndex]: TcxGridChartItem read GetItem write SetItem; default;
  end;
...
*)
  ...
  protected
    ...
    function UseOptimalHeightWhenDetail: Boolean; override;
...
procedure cxGridChartDiagramImages_Add(const AResourceName: string; AResType: PChar = RT_BITMAP);
...
{$ENDIF}
  ...
  cxEditDataRegisteredRepositoryItems, cxGrid, cxGridLevel, cxGridStrs, cxContainer,
  cxTextEdit, cxLibraryConsts, cxGeometry, DateUtils, dxDPIAwareUtils, dxCoreGraphics, RTLConsts;

const
  dxThisUnitName = 'cxGridChartView';
...
var
  FcxGridChartDiagramImagesInitialized: Boolean;

const
  DiagramNamesCount = 8;
  DiagramNames_SVG: array [1..DiagramNamesCount] of string = (
    'CXGRIDCHARTAREADIAGRAM',
    'CXGRIDCHARTBARDIAGRAM',
    'CXGRIDCHARTCOLUMNDIAGRAM',
    'CXGRIDCHARTLINEDIAGRAM',
    'CXGRIDCHARTPIEDIAGRAM',
    'CXGRIDCHARTSTACKEDAREADIAGRAM',
    'CXGRIDCHARTSTACKEDBARDIAGRAM',
    'CXGRIDCHARTSTACKEDCOLUMNDIAGRAM'
    );
  DiagramNames_BMP: array [1..DiagramNamesCount] of string = (
    ...
    'CXGRIDCHARTSTACKEDAREADIAGRAMBITMAP',
    'CXGRIDCHARTSTACKEDBARDIAGRAMBITMAP',
    'CXGRIDCHARTSTACKEDCOLUMNDIAGRAMBITMAP'
    );
...
begin
  ...
  begin
    ...
    try
      ...
      cxGridChartDiagramImages.Add(ABitmap, nil);
    finally
      ABitmap.Free;
    end;
  end
  ...
  begin
    ...
    try
      AImage.LoadFromResource(HInstance, AResourceName, AResType);
      cxGridChartDiagramImages.Add(AImage);
    finally
      AImage.Free;
    end;
  end;
...
procedure cxGridChartDiagramDefaultImagesInitialize;
var
  i: Integer;
begin
  if FcxGridChartDiagramImagesInitialized then
    Exit;
  for I := 1 to DiagramNamesCount do
  begin
    if dxUseVectorIcons then
      cxGridChartDiagramImages_Add(DiagramNames_SVG[i], 'SVG')
    else
      cxGridChartDiagramImages_Add(DiagramNames_BMP[i], RT_BITMAP);
  end;
  FcxGridChartDiagramImagesInitialized := True;
end;
...
begin
  ...
  FSeriesImages := TcxImageList.Create(nil);
...
begin
  ...
  SeriesImages.Width := ScaleFactor.Apply(16);
  SeriesImages.Height := SeriesImages.Width;
  SeriesImages.SourceDPI := ScaleFactor.TargetDPI;
...
procedure TcxGridChartSortBySeriesComboBox.DrawItem(AControl: TcxCustomComboBox;
  ...
  begin
    Result := ARect;
    AKeySize := LookAndFeelPainter.ScaledCheckButtonSize(ScaleFactor).cy;
    InflateRect(Result, -ScaleFactor.Apply(cxTextOffset + KeyOffset),
     -(Result.Bottom - Result.Top - AKeySize) div 2);
...
procedure TcxGridChartDataGroupsListBox.UpdateItemHeight;
begin
  ItemHeight := 2 * (LookAndFeelPainter.ChartToolBoxDataLevelInfoBorderSize + ScaleFactor.Apply(cxTextOffset)) +
    TdxTextMeasurer.TextLineHeight(Font);
end;
...
{ TcxGridChartCustomizationController }

procedure TcxGridChartCustomizationController.CustomizationChanged;
begin
  ViewInfo.ToolBoxViewInfo.InvalidateCustomizeButton;
  inherited CustomizationChanged;
end;

function TcxGridChartCustomizationController.GetFormDefaultWidth: Integer;
begin
  Result := cxGridChartCustomizationFormDefaultWidth;
end;

function TcxGridChartCustomizationController.GetViewInfo: TcxGridChartViewInfo;
begin
  Result := TcxGridChartViewInfo(inherited ViewInfo);
end;
...
function TcxGridChartController.CreateCustomizationController: TcxCustomGridCustomizationController;
begin
  Result := TcxGridChartCustomizationController.Create(GridView);
end;
...
begin
  ...
  for I := 0 to ViewInfo.Count - 1 do
  begin
    AText := ViewInfo.Captions[I];
    ABounds := ViewInfo.CaptionBounds[I];
    if not ViewInfo.IsVertical or not cxRectIntersect(ABounds, APrevBounds) and (AText <> '') then
    begin
      Canvas.DrawText(AText, ABounds, ATextAttributes);
...
begin
  if ViewInfo.ImageIndex <> -1 then
    TdxImageDrawer.DrawImage(Canvas, ViewInfo.ImageBounds, nil, cxGridChartDiagramImages, ViewInfo.ImageIndex, True, nil, ScaleFactor);
end;
...
begin
  if not FcxGridChartDiagramImagesInitialized then
    cxGridChartDiagramDefaultImagesInitialize;
...
var
  I: Integer;
begin
  if FItemKeyWidth > 0 then
    Exit(FItemKeyWidth);
  for I := 0 to Count - 1 do
    FItemKeyWidth := Max(FItemKeyWidth, Items[I].CalculateLegendKeyWidth);
   Result := FItemKeyWidth;
end;
...
var
  I: Integer;
begin
  if FItemKeyHeight > 0 then
    Exit(FItemKeyHeight);
  for I := 0 to Count - 1 do
    FItemKeyHeight := Max(FItemKeyHeight, Items[I].CalculateLegendKeyHeight);
  Result := FItemKeyHeight;
end;
...
begin
  ...
  begin
    CalculateParams;
    FCaptionHeight := TdxTextMeasurer.TextLineHeight(Params.Font);
  end;
...
procedure TcxGridChartDiagram.RefreshStyleCache;
begin
  if FStyles <> nil then
...
begin
  ...
  try
    Result.DefaultValuesProvider := DefaultValuesProvider;
  finally
...
begin
  ...
  if FDataBinding = nil then
    FDataBinding := GridView.GetItemDataBindingClass.Create(GridView, IsValue, GetDefaultValueTypeClass);
end;
...
function TcxGridChartDataGroup.HasNullGroupValue: Boolean;
var
  I: Integer;
begin
  Result := False;
  if VisibleIndex < 0 then
    Exit;
  for I := 0 to VisibleValueCount - 1 do
  begin
    Result := VarIsNull(VisibleValues[I]);
    if Result then
      Exit;
  end;
end;
...
begin
  Result := not VarIsEmpty(FActiveValue) and (not VarIsNull(FActiveValue) or HasNullGroupValue);
end;
...
procedure TcxGridChartSeries.RefreshStyleCache;
begin
  if FStyles <> nil then
    FStyles.RefreshCache;
end;
...
function TcxGridOpenChartItemList.GetItem(Index: TdxListIndex): TcxGridChartItem;
begin
  Result := TcxGridChartItem(inherited Items[Index]);
end;

procedure TcxGridOpenChartItemList.SetItem(Index: TdxListIndex; Value: TcxGridChartItem);
begin
...
procedure TcxGridChartView.DoRefreshStylesCache;
var
  I: Integer;
begin
  inherited DoRefreshStylesCache;
  for I := 0 to DiagramCount - 1 do
    Diagrams[I].RefreshStyleCache;
  for I := 0 to SeriesCount - 1 do
    Series[I].RefreshStyleCache;
end;
...
function TcxGridChartView.UseOptimalHeightWhenDetail: Boolean;
begin
  Result := True;
end;
...
procedure TcxGridChartView.InitializeResources;
begin
  if not FcxGridChartDiagramImagesInitialized then
    cxGridChartDiagramDefaultImagesInitialize;
end;
...
begin
  ...
  dxRegisterCursor(crcxGridMagnifier, HInstance, 'CXGRIDCHARTMAGNIFIERCURSOR');
  dxRegisterCursor(crcxGridChartDrag, HInstance, 'CXGRIDCHARTDRAGCURSOR');
end;
...
begin
  ...
  dxUnregisterCursor(crcxGridMagnifier);
  dxUnregisterCursor(crcxGridChartDrag);
