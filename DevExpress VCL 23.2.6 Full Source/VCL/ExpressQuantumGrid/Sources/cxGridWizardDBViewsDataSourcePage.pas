...
uses
  ...
  cxGridCustomView, cxGridDBDataDefinitions, dxBevel, cxGroupBox, dxLayoutLookAndFeels, cxLabel, cxDBData;
...
type
  ...
  TcxGridWizardDBViewsDataSourcePageFrame = class(TcxGridWizardCustomPageFrame)
    ...
    cbKeyField: TcxComboBox;
    lciKeyField: TdxLayoutItem;
    cbMasterKeyField: TcxComboBox;
    lciMasterKeyField: TdxLayoutItem;
    cbDetailKeyField: TcxComboBox;
  ...
  protected
    ...
    procedure PopulateDataSetFields(ADataSource: TDataSource; AComboBox: TcxComboBox);
...
const
  dxThisUnitName = 'cxGridWizardDBViewsDataSourcePage';
...
begin
  LoadPageContent;
  cbKeyField.Text := DataController.KeyFieldNames;
  cbDetailKeyField.Text := DataController.DetailKeyFieldNames;
...
begin
  ...
  Result := (AHelperClass <> nil) and AHelperClass.CanBeMasterView and (AView <> SelectedGridView) and
    (AHelperClass.GetGridViewType = Helper.GetGridViewType);
...
procedure TcxGridWizardDBViewsDataSourcePageFrame.PopulateDataSetFields(ADataSource: TDataSource; AComboBox: TcxComboBox);
var
  I: Integer;
begin
  ...
  try
    ...
    if ADataSource <> nil then
      for I := 0 to ADataSource.DataSet.FieldCount - 1 do
        AComboBox.Properties.Items.Add(ADataSource.DataSet.Fields[I].FieldName);
  finally
...
begin
  ...
  cbMasterKeyField.Text := DataController.MasterKeyFieldNames;
  if cbMasterKeyField.Text = '' then
    cbMasterKeyField.Text := AMasterGridViewDataController.KeyFieldNames;
end;
