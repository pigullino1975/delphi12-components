...
uses
  ...
  dxServerModeData, cxEdit, cxGridCustomTableView, dxCore;

type
  TcxGridWizardGridViewType = (gvtUnbound, gvtDB, gvtServerMode, gvtEMF);
  ...
  public
    ...
    property Glyph: TdxSmartImage read FGlyph write FGlyph;
  ...
  private
    function GetItem(Index: TdxListIndex): TcxGridWizardHelperInfo;
  public
    ...
    property Items[Index: TdxListIndex]: TcxGridWizardHelperInfo read GetItem; default;
  end;
...
const
  dxThisUnitName = 'cxGridWizardCustomHelper';
...
begin
  if Pos('DB', AGridViewClass.ClassName) > 0 then
    ...
    if Pos('ServerMode', AGridViewClass.ClassName) > 0 then
      ...
      if Pos('EMF', AGridViewClass.ClassName) > 0 then
        Result := gvtEMF
      else
        Result := gvtUnbound;
end;
...
begin
  ...
  try
    Glyph.LoadFromResource(AInstance, UpperCase(GridViewClass.ClassName), RT_BITMAP);
  except
    on EResNotFound do
    try
      Glyph.LoadFromResource(AInstance, UpperCase(GridViewClass.ClassName), RT_RCDATA);
    except
      on EResNotFound do;
    end;
  end;
...
function TcxGridWizardHelperInfoList.GetItem(Index: TdxListIndex): TcxGridWizardHelperInfo;
begin
