...
uses
  ...
  dxCore, cxGraphics, cxControls, cxLookAndFeels, cxCheckBox, cxButtons, dxTreeView,
...
type
  ...
  TcxGridViewLayoutCustomizationForm = class(TdxLayoutControlCustomizeForm,
    ...
    lgGridViewContainer: TdxLayoutGroup;
  ...
  protected
    ...
    procedure Initialize(AController: TcxCustomGridController); reintroduce; overload;
  ...
  public
    ...
    procedure Initialize(AGridView: TcxCustomGridView); reintroduce; overload;
...
uses
  cxGeometry, cxGridStrs, dxMessageDialog;

const
  dxThisUnitName = 'cxGridViewLayoutCustomizationForm';
...
procedure TcxGridViewLayoutCustomizationForm.Initialize(AGridView: TcxCustomGridView);
begin
  FGridView := AGridView;
  CreateControls;
...
begin
  Initialize(AController.GridView);
end;
...
begin
  if Parent <> nil then
    ...
    if (GridView <> nil) and (GridView.Control <> nil) then
...
procedure TcxGridViewLayoutCustomizationForm.RefreshLayoutLookAndFeel;
  ...
  function GetGridViewGroupPadding(ASide: TdxLayoutSide): Integer;
  begin
    Result := 1 - llfMain.GetGroupBorderWidth(lgGridViewContainer.Container, sdLeft, sdTop, False, True);
  end;
...
begin
  ...
  lgGridViewContainer.Padding.Left := GetGridViewGroupPadding(sdLeft);
  lgGridViewContainer.Padding.Right := GetGridViewGroupPadding(sdRight);
  lgGridViewContainer.Padding.Top := GetGridViewGroupPadding(sdTop);
  lgGridViewContainer.Padding.Bottom := GetGridViewGroupPadding(sdBottom);
end;
...
begin
  ...
  begin
    AChoice := dxMessageBox(Handle,
      cxGetResourceString(@scxGridLayoutViewCustomizeWarningDialogMessage),
      cxGetResourceString(@scxGridLayoutViewCustomizeWarningDialogCaption),
