...
uses
  Windows, Classes, DB, cxGridCustomView, dxCoreClasses,
...
type
  ...
  protected
    ...
    function IsDestroying: Boolean; override;
    // IcxCustomGridDataController
    ...
    function CanIgnoreTimeForFiltering(AItemLink: TObject): Boolean; override;
...
uses
  ...
  cxEditDataRegisteredRepositoryItems, cxEditDBRegisteredRepositoryItems,
  cxStorage, cxGridCommon, cxGridLevel, Controls, Forms, cxDBLookupComboBox, dxDPIAwareUtils, cxGeometry,
  cxDataControllerSpreadSheetExpressionProvider, RTLConsts, dxCore;

const
  dxThisUnitName = 'cxGridDBDataDefinitions';
...
function TcxGridDBDataFilterCriteriaItem.IsItemLinkSupportsMultiThreading: Boolean;
var
  AProperties: TcxCustomEditProperties;
begin
  AProperties := TcxCustomGridTableItem(ItemLink).GetProperties;
  Result := TcxPropertiesAccess(AProperties).SupportsMultiThreading(CompareByDisplayValue);
end;
...
function TcxGridDBDataController.IsDestroying: Boolean;
begin
...
function TcxGridDBDataController.CanIgnoreTimeForFiltering(AItemLink: TObject): Boolean;
begin
  Result := TcxCustomGridTableItemAccess.CanIgnoreTimeForFiltering(TcxCustomGridTableItem(AItemLink));
end;
...
function TcxGridDBDataController.CreateExpressionProvider: TcxDataCustomExpressionProvider;
begin
