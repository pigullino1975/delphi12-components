...
uses
  ...
  dxLayoutContainer, dxLayoutControl, dxLayoutControlAdapters, cxContainer, cxEdit, cxListBox, dxCoreGraphics,
  cxCustomListBox, dxLayoutcxEditAdapters;
...
type
  ...
  protected
    FColumnsListSuffixOffset: Integer;
    procedure AddColumn(AItem: TcxCustomGridTableItem);
    ...
    procedure DrawListBoxItem(AListBox: TcxListBox; ACanvas: TcxCanvas; AIndex: Integer; R: TRect); overload; virtual;
    procedure DrawListBoxItem(AListBox: TcxListBox; ACanvas: TcxCanvas; R: TRect; const APrefix: string;
      const ASuffix: string = ''; ASuffixPos: Integer = 0); overload; virtual;
...
uses
  cxDesignWindows, dxCore, dxMessageDialog;

const
  dxThisUnitName = 'cxCustomTableViewEditor';
...
constructor TcxCustomTableViewEditor.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  LBColumns.DoubleBuffered := True;
  LBFooterSummary.DoubleBuffered := True;
  LBDefaultGroupsSummary.DoubleBuffered := True;
  LBSummaryGroups.DoubleBuffered := True;
  LBGroupSummaryItems.DoubleBuffered := True;
  LBUnlinkedColumns.DoubleBuffered := True;
  LBLinkedColumns.DoubleBuffered := True;
end;
...
procedure TcxCustomTableViewEditor.AddColumn(AItem: TcxCustomGridTableItem);
var
  S: string;
begin
  FColumnsListSuffixOffset := 0;
  S := AItem.Name;
  if AItem.RepositoryItem <> nil then
    S := S + ' (' + AItem.RepositoryItem.Name + ')'; // TODO: description
  LBColumns.ItemIndex := LBColumns.Items.AddObject(S, AItem);
end;

procedure TcxCustomTableViewEditor.CalculateListBoxItemHeight(AListBox: TListBox);
begin
  AListBox.ItemHeight := ScaleFactor.Apply(2) + TdxTextMeasurer.TextLineHeight(AListBox.Font);
end;
...
begin
  ...
  try
    ...
    AListBox.ItemIndex := AListBox.Items.IndexOfObject(AItem);
    if AListBox.ItemIndex = (AListBox.Count - 1) then
      AListBox.TopIndex := AListBox.TopIndex + 1;
  finally
  ...
  try
    ...
    try
      ...
      if AListBox.ItemIndex >= 0 then
        AListBox.Selected[AListBox.ItemIndex] := True;
...
begin
  DrawListBoxItem(AListBox, ACanvas, R, AListBox.Items[AIndex]);
end;

procedure TcxCustomTableViewEditor.DrawListBoxItem(AListBox: TcxListBox; ACanvas: TcxCanvas; R: TRect;
  const APrefix: string; const ASuffix: string = ''; ASuffixPos: Integer = 0);
begin
  ACanvas.FillRect(R);
  if APrefix <> '' then
    ACanvas.TextOut(R.Left + ScaleFactor.Apply(2), R.Top, APrefix);
  if ASuffix <> '' then
    ACanvas.TextOut(ASuffixPos, R.Top, ASuffix);
end;
...
begin
  Result := Format('You are going to retrieve all fields. Delete existing %ss?', [LowerCase(GetViewItemName(View))]);
end;
...
begin
  if View.ItemCount > 0 then
    case dxMessageDlg(GetDeleteItemsCaption, mtConfirmation, [mbYes, mbNo, mbCancel], 0) of
      mrYes:
        View.ClearItems;
      mrCancel:
        Exit;
