...
const
  dxThisUnitName = 'cxGridWizardTableViewOptionsInterfaceElementsPage';
...
begin
  ...
  if not (Helper.GetGridViewType in [gvtServerMode]) then
...
begin
  ...
  lciNewItemRow.Visible := not (Helper.GetGridViewType in [gvtServerMode]);
  ...
  chbInplaceEditForm.Checked := AView.OptionsBehavior.EditMode <> emInplace;
