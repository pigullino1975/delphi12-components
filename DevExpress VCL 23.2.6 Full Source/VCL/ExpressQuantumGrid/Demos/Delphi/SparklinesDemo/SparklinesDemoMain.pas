...
const
  ...
  AName: array[0..2] of string = ('miArea',  'miLine', 'miBar');
begin
  ASeries := TdxLookupSparklineProperties(cxgCarOrdersTableViewPayments.Properties).Series;
  cxgCarOrdersTableViewPayments.Editing := False;
  ASeries[0].SeriesType := AType[TComponent(Sender).Tag];
  ASeries[1].SeriesType := AType[TComponent(Sender).Tag];
  //
  MenuItemSetChecked('miArea', ASeries[0].SeriesType = stArea);
  MenuItemSetChecked('miLine', ASeries[0].SeriesType = stLine);
  MenuItemSetChecked('miBar', ASeries[0].SeriesType = stBar);
end;
