...
uses
  ...
  cxScrollBar, dxFading, dxCoreClasses, dxCore, dxScrollAnimation;

type
  TcxCustomScrollBox = class;
  TcxScrollBoxScrollBarOptions = class;

  TcxScrollBoxScrollAnimationHelper = class(TdxScrollAnimationHelper)
  strict private
    FOwner: TcxScrollBoxScrollBarOptions;
  protected
    ...
    property Owner: TcxScrollBoxScrollBarOptions read FOwner;
  public
    constructor Create(AOwner: TcxScrollBoxScrollBarOptions);
  end;
  ...
  private
    ...
    FPosition: Integer;
    FRange: Integer;
    FScrollAnimationHelper: TcxScrollBoxScrollAnimationHelper;
    ...
    procedure DoSetPosition(Value: Integer);
...
const
  dxThisUnitName = 'cxScrollBox';

type
  TcxControlCustomScrollBarsAccess = class(TcxControlCustomScrollBars);

{ TcxScrollBoxScrollAnimationHelper }

constructor TcxScrollBoxScrollAnimationHelper.Create(AOwner: TcxScrollBoxScrollBarOptions);
begin
  inherited Create;
  FOwner := AOwner;
end;

function TcxScrollBoxScrollAnimationHelper.GetOwnerPosition: Integer;
begin
  Result := Owner.Position;
end;

procedure TcxScrollBoxScrollAnimationHelper.UpdateOwnerPosition(
  AValue: Integer);
begin
  Owner.DoSetPosition(AValue);
end;

procedure TcxScrollBoxScrollAnimationHelper.CheckPosition(var AValue: Integer);
begin
  Owner.CheckPosition(AValue);
end;
...
begin
  ...
  FVisible := True;
  FIncrement := 8;
  FScrollAnimationHelper := TcxScrollBoxScrollAnimationHelper.Create(Self);
end;

destructor TcxScrollBoxScrollBarOptions.Destroy;
begin
...
var
  ...
  I: Integer;
begin
  if AutoScroll and not ScrollBox.IsScaleChanging then
  begin
    ARange := 0;
    AAlignMargin := 0;
...
procedure TcxScrollBoxScrollBarOptions.DoSetPosition(Value: Integer);
var
  ADelta: Integer;
begin
...
begin
  ...
  if FScrollAnimationHelper.IsAnimationActive and (ACode in [scTrack]) then
    FScrollAnimationHelper.StopAnimation(False);
  if ScrollBox.IsScrollAnimationEnabled and (ACode in [scLineUp..scPageDown]) then
    FScrollAnimationHelper.SetPosition(APosition)
  else
...
procedure TcxScrollBoxScrollBarOptions.ChangeScale(M, D: Integer);
begin
  FPosition := MulDiv(FPosition, M, D);
  FRange := MulDiv(FRange, M, D);
...
procedure TcxCustomScrollBoxViewInfo.DrawBackground(ACanvas: TcxCanvas; const ARect: TRect);
begin
  Painter.DrawScrollBoxBackground(ACanvas, ARect, ScrollBox.Color);
end;
...
var
  APrevParentBidiModeValue: Boolean;
begin
  inherited;
  APrevParentBidiModeValue := ParentBiDiMode;
  BiDiMode := FLoadedBiDiMode;
  ParentBiDiMode := APrevParentBidiModeValue;
  UpdateScrollBars;
end;
...
procedure TcxCustomScrollBox.ChangeScaleEx(M: Integer; D: Integer; isDpiChange: Boolean);
begin
  HorzScrollBar.ChangeScale(M, D);
  VertScrollBar.ChangeScale(M, D);
  inherited;
end;
...
procedure TcxCustomScrollBox.DrawOpaqueBackground(ACanvas: TcxCanvas; const ARect: TRect);
begin
  ViewInfo.DrawBackground(ACanvas, ARect);
end;
...
function TcxCustomScrollBox.HasBackground: Boolean;
begin
  Result := True;
end;
...
begin
  ...
  if not IsDesigning and (APrevUseRightToLeftAlignment <> UseRightToLeftAlignment) then
    for I := 0 to ControlCount - 1 do
    begin
...
begin
  inherited;
  R := cxRectSetNullOrigin(Message.CalcSize_Params^.rgrc[0]);
  ViewInfo.Calculate(R);
  R := cxRectOffset(ViewInfo.ClientRect, Message.CalcSize_Params^.rgrc[0].TopLeft);
