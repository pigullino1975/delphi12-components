...
type
  ...
  public
    constructor Create(AProperties: TcxCustomEditProperties; AStyle: TcxCustomEditStyle; AIsInplace: Boolean); override;
...
constructor TdxCustomZoomTrackBarViewData.Create(AProperties: TcxCustomEditProperties; AStyle: TcxCustomEditStyle; AIsInplace: Boolean);
begin
  inherited Create(AProperties, AStyle, AIsInplace);
  FAlwaysShowTickArea := True;
end;
...
begin
  ...
  APixelsPerPositionFirstRange := (ATrackRect.Width - AViewInfo.ThumbSize) / (2 * GetMinCenterPositionDiff);
  APixelsPerPositionSecondRange := (ATrackRect.Width - AViewInfo.ThumbSize) / (2 * GetCenterPositionMaxDiff);
...
function TdxCustomZoomTrackBarViewInfo.GetPositionAfterJump(X, Y: Integer): Integer;
  ...
  begin
    Result := GetCurrentMousePos(X, Y) < TrackRect.Left + TrackRect.Width div 2;
  end;
