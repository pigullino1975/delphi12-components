...
uses
  Variants, Clipbrd, Math, dxCoreGraphics, cxEditConsts, cxEditUtils, cxScrollBar, dxMessages, dxDPIAwareUtils,
  dxThreading;
...
begin
  if Painter.SupportsListBoxPadding and not IsTouchScrollUIMode and (Edit <> nil) then
  begin
    ABackPadding := Painter.GetListBoxBackgroundPadding(IsInplace, dxDefaultScaleFactor);

    if TcxCustomEditAccess(Edit).IsScrollBarActive(sbHorizontal) then
    begin
      AScrollPadding := Painter.GetListBoxScrollBarPadding(IsInplace, sbHorizontal, nil);
      Dec(APadding.Bottom, AScrollPadding.Bottom);
    end;
    if TcxCustomEditAccess(Edit).IsScrollBarActive(sbVertical) then
    begin
      AScrollPadding := Painter.GetListBoxScrollBarPadding(IsInplace, sbVertical, nil);
      APadding.Right := ABackPadding.Right - AScrollPadding.Right - 1; 
    end;
  end
  else
    inherited;
...
begin
  ...
  begin
    APadding := AViewInfo.Painter.GetListBoxBackgroundPadding(AViewInfo.IsInplace, AViewInfo.ScaleFactor);
...
begin
  ...
  try
    ...
    AInnerEditBounds := cxRectInflate(AInnerContentBounds, cxContainerMaxBorderWidth);
