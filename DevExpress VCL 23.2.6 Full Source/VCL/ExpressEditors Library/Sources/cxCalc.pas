...
uses
  ...
  cxTextEdit, cxFilterControlUtils, dxFading, cxAccessibility;
...
type
  ...
  { TdxCalculatorAccessibilityHelper }

  TdxCalculatorAccessibilityHelper = class(TcxAccessibilityHelper) // for internal use
  strict private
    function GetButtons(AIndex: Integer): TcxCalcEditButtonViewInfo;
    function GetCalculator: TcxCustomCalculator;
  protected
    ...
    property Buttons[AIndex: Integer]: TcxCalcEditButtonViewInfo read GetButtons;
    property Calculator: TcxCustomCalculator read GetCalculator;
  public
  ...
  protected
    ...
    property Buttons: TcxCalcEditButtonsViewInfo read FButtons;
  ...
  private
    ...
    function GetRealPrecision: Byte;
  ...
  protected
    ...
    property AssignedValues: TcxCalcEditPropertiesValues read GetAssignedValues write SetAssignedValues;
  public
    ...
    property Precision: Byte read GetPrecision write SetPrecision stored IsPrecisionStored;
    property QuickClose: Boolean read FQuickClose write SetQuickClose default False;
    property ScientificFormat: Boolean read FScientificFormat write SetScientificFormat default False;
...
function TdxCalculatorAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  if AChildID = 0 then
    Result := Calculator.Bounds
  else
    Result := Calculator.Buttons[TcxCalcButtonKind(AChildID - 1)].Bounds;
  Result := cxRectSetOrigin(Result, Calculator.ClientToScreen(Result.TopLeft));
end;

function TdxCalculatorAccessibilityHelper.ChildIsSimpleElement(AIndex: Integer): Boolean;
begin
  Result := True;
end;

procedure TdxCalculatorAccessibilityHelper.DoDefaultAction(AChildID: TcxAccessibleSimpleChildElementID);
begin
  if AChildID > 0 then
    Calculator.ButtonClick(TcxCalcButtonKind(AChildID - 1));
end;

function TdxCalculatorAccessibilityHelper.GetChildCount: Integer;
begin
  Result := Integer(High(TcxCalcButtonKind)) + 1;
end;

function TdxCalculatorAccessibilityHelper.GetDefaultActionDescription(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := inherited;
  if AChildID > 0 then
    Result := 'Press';
...
function TdxCalculatorAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := inherited;
  if AChildID > 0 then
    Result := Buttons[AChildID - 1].Text;
end;

function TdxCalculatorAccessibilityHelper.GetOwnerObjectWindow: HWND;
begin
  Result := Calculator.Handle;
end;

function TdxCalculatorAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := inherited;
  if AChildID > 0 then
...
function TdxCalculatorAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxSTATE_SYSTEM_FOCUSABLE;
  if AChildID > 0 then
  begin
    if Buttons[AChildID - 1].IsDefault then
      Result := Result or cxSTATE_SYSTEM_FOCUSED;
    if Buttons[AChildID - 1].State = cxbsDisabled then
      Result := Result or cxSTATE_SYSTEM_UNAVAILABLE;
  end;
end;

function TdxCalculatorAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := [aopFocus, aopLocation, aopDefaultAction]
end;

function TdxCalculatorAccessibilityHelper.IsExtended: Boolean;
begin
  Result := True;
end;

function TdxCalculatorAccessibilityHelper.GetButtons(AIndex: Integer): TcxCalcEditButtonViewInfo;
begin
  Result := Calculator.Buttons[TcxCalcButtonKind(AIndex)];
end;

function TdxCalculatorAccessibilityHelper.GetCalculator: TcxCustomCalculator;
begin
  Result := TcxCustomCalculator(OwnerObject);
end;
...
procedure TcxCustomCalculator.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  AButtonKind : TcxCalcButtonKind;
begin
  if not (csDesigning in ComponentState) and (CanFocus or (GetParentForm(Self) = nil)) and not IsPopupControl then
    SetFocus;

  AButtonKind := GetButtonKindAt(X, Y);
  if (Button = mbLeft) and (AButtonKind <> cbNone) and not FButtons[AButtonKind].Grayed then
  begin
    MouseCapture := True;
    FTracking := True;
    FDownButton := AButtonKind;
    TrackButton(X, Y);
  end;
...
procedure TcxCustomCalculator.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  AWasPressed: Boolean;
begin
  AWasPressed := (FDownButton <> cbNone) and FButtons[FDownButton].Down;
  StopTracking;
  if (Button = mbLeft) and AWasPressed then
...
var
  ANewButton, AOldButton: TcxCalcButtonKind;
begin
  inherited KeyDown(Key, Shift);
  AOldButton := FPressedButton;
  ANewButton := GetButtonKindKey(Key, Shift);
  if (ANewButton <> cbNone) and (AOldButton <> ANewButton) then
  begin
    DoButtonUp(AOldButton);
    FPressedButton := ANewButton;
...
begin
  ...
  case Key of
    ...
    AOldButton := FPressedButton;
    ANewButton := GetButtonKindChar(Key);
    if (ANewButton <> cbNone) and (AOldButton <> ANewButton) then
    begin
      DoButtonUp(AOldButton);
      FPressedButton := ANewButton;
  ...
  begin
    ...
    try
      if not AButtonInfo.FadingHelper.DrawImage(ACanvas.Handle, AButtonInfo.Bounds) then
        GetPainter.DrawScaledButton(ACanvas, AButtonInfo.Bounds, AButtonInfo.State, AButtonInfo.IsDefault, ScaleFactor);
    ...
    begin
     ABitmap := TcxBitmap.CreateSize(AButton.Bounds);
      try
        ...
        if GetPainter.SupportsNativeFocusRect then
...
procedure TcxCustomCalculator.WMGetObject(var Message: TMessage);
var
  AObjectID: Cardinal;
begin
...
procedure TcxCustomCalculator.CalcSize(AWidth, AHeight : Integer);
  ...
  var
    AOffset: Integer;
  begin
    AOffset := MulDiv(ABtnHeight, cxMinCalcYOfs, cxMinCalcBtnHeight);
    Result := (AOffset + ABtnHeight) * 5 + AOffset;
  end;

var
  h, ANearHeight, d, AMinHeight: Integer;
begin
  if AutoFontSize then
  begin
    ...
    ANearHeight := Canvas.FontHeight(Font) * 2;
  FCalcBtnHeight     := ANearHeight;
...
function TcxCustomCalculator.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin
  if FIAccessibilityHelper = nil then
    FIAccessibilityHelper := TdxCalculatorAccessibilityHelper.Create(Self);
...
begin
  ...
  case Key of
    ...
    VK_DELETE:
      Result := cbCancel;
    Ord('C'){VK_C}:
      if not (ssCtrl in Shift) then Result := cbClear;
    Ord('P'){VK_P}:
...
begin
  if AssignedValues.Precision then
    ...
    if DefaultValuesProvider <> nil then
      Result := DefaultValuesProvider.DefaultPrecision
    else
...
function TcxCustomCalcEditProperties.GetRealPrecision: Byte;
begin
  if Precision = 0 then
    Result := cxDefCalcPrecision
  else
...
procedure TcxCustomCalcEditProperties.SetAssignedValues(Value: TcxCalcEditPropertiesValues);
begin
...
function TcxCustomCalcEditProperties.HasDigitGrouping(AIsDisplayValueSynchronizing: Boolean): Boolean;
begin
...
begin
  if not AEditFocused and not AssignedValues.DisplayFormat and
    (DefaultValuesProvider <> nil) and  DefaultValuesProvider.IsDisplayFormatDefined(True) then
      Result := evsText
  else
    Result := evsValue;
end;
...
function TcxCustomCalcEditProperties.IsEditValueNumeric: Boolean;
begin
  Result := True;
end;
  ...
  if VarIsSoftNull(AEditValue) then
    ...
    begin
      if ScientificFormat then
        ...
        AFormat := ffGeneral;
      S := FloatToStrF(AEditValue, AFormat, GetRealPrecision, 0);
...
begin
  Result := ((not ActiveProperties.ReadOnly) or ActiveProperties.AllowDropDownWhenReadOnly) and DataBinding.IsDataAvailable; 
end;
...
begin
  if Text = cxGetResourceString(@scxSCalcError) then
    InternalEditValue := 0;
...
begin
  ...
  begin
    ...
    FCalculator.Precision := ActiveProperties.GetRealPrecision;
...
function TcxCustomCalcEdit.InternalPrepareEditValue(const ADisplayValue: string; out EditValue: TcxEditValue): Boolean;
var
  AValue: Extended;
  S: string;
begin
...
begin
  Result := [fcoEqual, fcoNotEqual, fcoLess, fcoLessEqual, fcoGreater, fcoGreaterEqual, fcoBlanks, fcoNonBlanks];
...
procedure TcxCalcEditButtonFadingHelper.GetFadingImages(out AFadeOutImage, AFadeInImage: TcxBitmap);
  ...
  begin
    ...
    Painter.DrawScaledButton(Result.cxCanvas, Result.ClientRect, AState, ViewInfo.IsDefault, ViewInfo.Owner.ScaleFactor);
  end;
