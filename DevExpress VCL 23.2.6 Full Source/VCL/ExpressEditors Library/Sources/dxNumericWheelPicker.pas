...
uses
  ...
  cxFilterControlUtils, dxWheelPicker, cxAccessibility;
...
type
  ...
  { TdxNumericWheelPickerWheelAccessibilityHelper }

  TdxNumericWheelPickerWheelAccessibilityHelper = class(TdxCustomWheelPickerWheelAccessibilityHelper) // for internal use
  protected
  ...
  protected
    ...
    function NeedProcessKey(AKey: Word): Boolean; virtual;
  ...
  strict private
    {$IFDEF DELPHIXE8}
    function GetFontHeight(AViewInfo: TdxCustomNumericWheelPickerViewInfo): Integer;
    {$ELSE}
...
uses
  ...
  cxControls, cxFormats;

const
  dxThisUnitName = 'dxNumericWheelPicker';
...
begin
  dxGPPaintCanvas.BeginPaint(cxMeasureCanvas.Handle, cxNullRect);
  try
...
begin
  ...
  {$IFDEF DELPHIXE8}
  FFont.Height := AViewInfo.FFontHeight;
  {$ELSE}
...
function TdxNumericWheelPickerWheelAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  if AChildID > 0 then
    Result := TdxCustomNumericWheelPickerItemViewInfo(
      TdxCustomNumericWheelPickerWheelViewInfo(ViewInfo).Items[AChildID - 1]).FText;
end;

{ TdxCustomNumericWheelPickerWheelViewInfo }

function TdxCustomNumericWheelPickerWheelViewInfo.GetAccessibilityHelperClass: TdxCustomWheelPickerWheelAccessibilityHelperClass;
begin
  Result := TdxNumericWheelPickerWheelAccessibilityHelper;
end;
...
function TdxCustomNumericWheelPickerWheelViewInfo.NeedProcessKey(AKey: Word): Boolean;
begin
...
begin
  ...
  begin
    ...
    {$IFDEF DELPHIXE8}
    FFontHeight := AViewInfo.FFontHeight;
    {$ELSE}
...
begin
  inherited;
  {$IFDEF DELPHIXE8}
  (AViewInfo as TdxCustomNumericWheelPickerViewInfo).FFontHeight := GetFontHeight(AViewInfo as TdxCustomNumericWheelPickerViewInfo);
  {$ELSE}
  ...
  try
    ...
    {$IFDEF DELPHIXE8}
    AFont.Height := (AViewInfo as TdxCustomNumericWheelPickerViewInfo).FFontHeight;
    {$ELSE}
...
begin
  ...
  {$IFDEF DELPHIXE8}
  AWheelPickerViewInfo.FFontHeight := GetFontHeight(AWheelPickerViewInfo);
  {$ELSE}
...
function TdxCustomNumericWheelPickerViewData.GetFontHeight(
  AViewInfo: TdxCustomNumericWheelPickerViewInfo): Integer;
begin
  if AViewInfo.FIsSmallItemSize then
    Result := Style.Font.Height
  else
    Result := Trunc(Style.Font.Height * dxWheelPickerFontSizeFactor);
end;
