...
uses
  ...
  cxLookAndFeels, cxLookAndFeelPainters, cxGeometry, cxAccessibility;
...
type
  ...
  protected
    ...
    class function GetTextIndent(APainter: TcxCustomLookAndFeelPainter; AScaleFactor: TdxScaleFactor): Integer; static;
  ...
  { TdxCustomColorComboBoxListBoxAccessibilityHelper }

  TcxCustomColorComboBox = class;

  TdxCustomColorComboBoxListBoxAccessibilityHelper = class(TdxCustomComboBoxListBoxAccessibilityHelper)
  private
    function GetColorComboBox: TcxCustomColorComboBox;
  protected
    function GetChildCount: Integer; override;
    function GetName(AChildID: TcxAccessibleSimpleChildElementID): string; override;

    property ColorComboBox: TcxCustomColorComboBox read GetColorComboBox;
  end;
  ...
  protected
    ...
    function GetItemNonTextWidth(AIndex: Integer): Integer; override;
...
const
  dxThisUnitName = 'cxColorComboBox';
...
function TcxColorComboBoxItems.InsertColor(Index: Integer; const AColor: TColor; const ADescription: string): TcxColorComboBoxItem;
begin
  Result := nil;
  if FindColorItem(AColor) = nil then
  begin
...
begin
  ...
  begin
    ...
    if Painter.SupportsNativeFocusRect then
...
procedure TcxCustomColorComboBoxViewData.DisplayValueToDrawValue(const ADisplayValue: TcxEditValue; AViewInfo: TcxCustomEditViewInfo);
var
...
procedure TcxCustomColorComboBoxViewData.EditValueToDrawValue(const AEditValue: TcxEditValue; AViewInfo: TcxCustomEditViewInfo);
var
...
class function TcxCustomColorComboBoxViewData.GetTextIndent(APainter: TcxCustomLookAndFeelPainter; AScaleFactor: TdxScaleFactor): Integer;
begin
  if APainter.SupportsListBoxPadding then
    Result := APainter.GetEditorGlyphIndent(False, False, AScaleFactor)
  else
    Result := AScaleFactor.Apply(TdxVisualRefinements.Padding.Left div 2);
end;
...
begin
  ...
  if AProperties.ShowDescriptions then
    Inc(Result.cx, ScaleFactor.Apply(AProperties.ColorBoxWidth, AProperties.ScaleFactor) +
      GetTextIndent(Painter, AProperties.ScaleFactor) +
      6);
  Assert(Painter <> nil);
end;
...
function TdxCustomColorComboBoxListBoxAccessibilityHelper.GetChildCount: Integer;
begin
  Result := inherited + ColorComboBox.ActiveProperties.MRUColors.Count;
end;

function TdxCustomColorComboBoxListBoxAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  if AChildID > 0 then
  begin
    if AChildID <= ColorComboBox.Properties.MRUColors.Count then
      Result := ColorComboBox.Properties.MRUColors[AChildID - 1].Description
    else
      Result := ListBox.Items[AChildID - ColorComboBox.Properties.MRUColors.Count - 1];
  end
  else
    Result := '';
end;

function TdxCustomColorComboBoxListBoxAccessibilityHelper.GetColorComboBox: TcxCustomColorComboBox;
begin
  Result := TcxCustomColorComboBox(TcxCustomColorComboBoxListBox(ListBox).Edit);
end;
...
begin
  ...
  begin
    AMinItemHeight := AProperties.ScaleFactor.Apply(16);
    Inc(AMinItemHeight, FItemPadding.Height);
    Result := Max(AMinItemHeight, inherited GetItemHeight);
  end;
...
procedure TcxCustomColorComboBoxListBox.CalculatePadding;
begin
  ...
  FElementIndent := TcxCustomColorComboBoxViewData.GetTextIndent(Painter, ScaleFactor);
end;

function TcxCustomColorComboBoxListBox.GetDefaultItemPadding: TdxPadding;
begin
  if SupportsListBoxSkinPadding then
...
function TcxCustomColorComboBoxListBox.GetItemNonTextWidth(AIndex: Integer): Integer;
begin
  Result := FItemPadding.Width + FBackgroundPadding.Width + FElementIndent;
end;

function TcxCustomColorComboBoxListBox.GetRealStyle: TListBoxStyle;
begin
  Result := TListBoxStyle.lbOwnerDrawVariable;
end;

function TcxCustomColorComboBoxListBox.IsBufferedItemPaint: Boolean;
begin
  Result := True;
end;
...
procedure TcxCustomColorComboBoxListBox.DrawItemContent(AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
var
...
begin
  ...
  begin
    ...
    begin
      Dec(ATextRect.Right, AViewInfo.ColorBoxWidth + FElementIndent);
      AColorBoxRect.Left := ATextRect.Right;
    end
    else
    begin
      AColorBoxRect.Right := AColorBoxRect.Left + AViewInfo.ColorBoxWidth;
      ATextRect.Left := AColorBoxRect.Right + FElementIndent;
    end;
  ...
  TcxColorComboBoxHelper.DrawColorBox(Canvas, AColorBoxRect, Edit.ActiveProperties.ColorBoxFrameColor,
    Edit.ActiveProperties.GetColorByIndex(Edit.LookupData.GetLookupItemIndexFromFilteredItemIndex(AIndex)),
    Canvas.Brush.Color, cxdcColor, Edit.ScaleFactor);
end;

function TcxCustomColorComboBoxListBox.GetAccessibilityHelperClass: TdxCustomInnerListBoxAccessibilityHelperClass;
begin
  Result := TdxCustomColorComboBoxListBoxAccessibilityHelper;
end;
...
var
  AIndex: Integer;
begin
  if (Edit.Text <> '') or Edit.IsLoading then
    inherited
  else
  begin
    if (Edit as TcxCustomColorComboBox).EditModeSetting then
      Exit;
    AIndex := ActiveProperties.GetIndexByColor((Edit as TcxCustomColorComboBox).ColorValue);
    InternalSetCurrentKey(AIndex);
  end;
end;
...
function TcxCustomColorComboBoxProperties.IsDisplayValueValid(var ADisplayValue: TcxEditValue; AEditFocused: Boolean): Boolean;
begin
  Result := True;
end;

procedure TcxCustomColorComboBoxProperties.PrepareDisplayValue(const AEditValue: TcxEditValue; var ADisplayValue: TcxEditValue; AEditFocused: Boolean);
var
  AValue: TColor;
  AValueText: string;
begin
  TranslateValues(AEditValue, AValue, AValueText);
  ADisplayValue := AValueText;
end;
...
begin
  InternalGetColorComboBoxDisplayValue(IndexByValue(AEditValue), AEditValue, AColor, ADescription, AColorFound);
end;

procedure TcxCustomColorComboBoxProperties.PrepareColorList(APrepareList: TcxColorPrepareList; ASaveCustom, ASaveMRU: Boolean);
begin
  LockUpdate(True);
  try
...
procedure TcxCustomColorComboBoxProperties.DoChanged;
begin
  inherited DoChanged;
  if Buttons.Count > 1 then
    Buttons[1].Enabled := (not ReadOnly) or AllowDropDownWhenReadOnly;
end;
...
function TcxCustomColorComboBoxProperties.GetItemCount: Integer;
begin
  Result := Items.Count;
end;
...
procedure TcxCustomColorComboBoxProperties.InternalPrepareColorList(AColorValues: array of TColor; AColorNames: array of string);
var
  I: Integer;
...
procedure TcxCustomColorComboBox.PrepareEditValue(
  const ADisplayValue: TcxEditValue; out AEditValue: TcxEditValue; AEditFocused: Boolean);
var
  AColor: TColor;
  AIndex: Integer;
begin
  ...
  AEditValue := LookupKeyToEditValue(AIndex);
end;
...
begin
  if IsReadOnly and (not ActiveProperties.AllowDropDownWhenReadOnly) and (ActiveProperties.GetButtonIndexByVisibleIndex(AButtonVisibleIndex) in [0, 1]) then
    Exit;
  inherited DoButtonClick(AButtonVisibleIndex);
  if ActiveProperties.GetButtonIndexByVisibleIndex(AButtonVisibleIndex) = 1 then
...
begin
  ...
  if (APrevKey = VK_RETURN) and not (ssAlt in Shift) and (ItemIndex = -1) and ActiveProperties.StrToColor(Text, AColor) then
  begin
    if not IsReadOnly then
...
procedure TcxCustomColorComboBox.PrepareColorList(APrepareList: TcxColorPrepareList; ASaveCustom, ASaveMRU: Boolean);
var
...
function TcxColorComboBox.SupportsItemIndex: Boolean;
begin
  Result := True;
end;
...
function TcxColorComboBoxProperties.FindValueByItemIndex(AItemIndex: Integer;
  out AValue: Variant): Boolean;
begin
  Result := InRange(AItemIndex, 0, Items.Count - 1);
  if Result then
    AValue := Items[AItemIndex].Color;
end;

function TcxColorComboBoxProperties.SupportsItemIndex: Boolean;
begin
  Result := True;
end;
