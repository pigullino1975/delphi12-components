...
uses
  ...
  cxControls, cxContainer, ImgList, cxEdit, dxAnimation, dxGallery, cxGeometry, cxFormats, cxAccessibility;
...
type
  ...
  { TdxCustomWheelPickerWheelAccessibilityHelper }

  TdxCustomWheelPickerWheelAccessibilityHelper = class(TdxEditChildAccessibilityHelper) // for internal use
  strict private
    function GetViewInfo: TdxCustomWheelPickerWheelViewInfo;
  protected
    ...
    property ViewInfo: TdxCustomWheelPickerWheelViewInfo read GetViewInfo;
  public
    function GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect; override;
  end;

  TdxCustomWheelPickerWheelAccessibilityHelperClass = class of TdxCustomWheelPickerWheelAccessibilityHelper;
  ...
  protected
    ...
    function GetAccessibilityHelperClass: TdxCustomWheelPickerWheelAccessibilityHelperClass; virtual;
  ...
  TdxCustomWheelPickerProperties = class(TcxCustomEditProperties, IcxFormatControllerListener)
  private
    FCyclic: Boolean;
    FLineCount: Integer;
 ...
 { TdxWheelPickerAccessibilityHelper }

  TdxCustomWheelPicker = class;

  TdxWheelPickerAccessibilityHelper = class(TdxEditAccessibilityHelper) // for internal use
  private
    function GetWheelPicker: TdxCustomWheelPicker;
    function GetViewInfo: TdxCustomWheelPickerViewInfo;
  protected
    function GetChildCount: Integer; override;
    function GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer; override;

    property WheelPicker: TdxCustomWheelPicker read GetWheelPicker;
  ...
  { TdxWheelPickerWheelAccessibilityHelper }

  TdxWheelPickerWheelAccessibilityHelper = class(TdxCustomWheelPickerWheelAccessibilityHelper) // for internal use
  protected
  ...
  protected
    ...
    function GetAccessibilityHelperClass: TdxCustomWheelPickerWheelAccessibilityHelperClass; override;
...
uses
  SysUtils, Math, Variants, dxThemeConsts, dxCoreGraphics, cxDrawTextUtils;

const
  dxThisUnitName = 'dxWheelPicker';
...
function TdxCustomWheelPickerWheelAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  if AChildID = 0 then
...
function TdxCustomWheelPickerWheelAccessibilityHelper.ChildIsSimpleElement(AIndex: Integer): Boolean;
begin
  Result := True;
end;

procedure TdxCustomWheelPickerWheelAccessibilityHelper.DoDefaultAction(AChildID: TcxAccessibleSimpleChildElementID);
begin
  if (AChildID > 0) and (ViewInfo.ItemIndex <> ViewInfo.Items[AChildID - 1].FIndex) then
  begin
    ViewInfo.ItemIndex := ViewInfo.Items[AChildID - 1].FIndex;
    TdxWheelPickerAccessibilityHelper(Parent).WheelPicker.InternalSetSelectedValue(ViewInfo.WheelPickerViewInfo.GetInternalValue);
  end;
end;

function TdxCustomWheelPickerWheelAccessibilityHelper.GetChildCount: Integer;
begin
  Result := ViewInfo.ItemCount;
end;

function TdxCustomWheelPickerWheelAccessibilityHelper.GetDefaultActionDescription(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := 'Click';
end;

function TdxCustomWheelPickerWheelAccessibilityHelper.GetParent: TcxAccessibilityHelper;
begin
  if ViewInfo.WheelPickerViewInfo.Edit <> nil then
    Result := TdxCustomWheelPicker(ViewInfo.WheelPickerViewInfo.Edit).IAccessibilityHelper.GetHelper
  else
    Result := nil;
end;

function TdxCustomWheelPickerWheelAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  if AChildID = 0 then
...
function TdxCustomWheelPickerWheelAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxSTATE_SYSTEM_FOCUSABLE or cxSTATE_SYSTEM_SELECTABLE;
  if ViewInfo.GetState = cxbsDisabled then
    Result := Result or cxSTATE_SYSTEM_UNAVAILABLE;
  if ViewInfo.IsSelected then
    Result := Result or cxSTATE_SYSTEM_SELECTED;
  if (AChildID > 0) and (ViewInfo.Items[AChildID - 1].GetState = cxbsPressed) then
    Result := Result or cxSTATE_SYSTEM_FOCUSED;
end;

function TdxCustomWheelPickerWheelAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := [aopLocation, aopSelect, aopFocus, aopDefaultAction];
end;

function TdxCustomWheelPickerWheelAccessibilityHelper.IsExtended: Boolean;
begin
  Result := True;
end;

function TdxCustomWheelPickerWheelAccessibilityHelper.GetViewInfo: TdxCustomWheelPickerWheelViewInfo;
begin
  Result := TdxCustomWheelPickerWheelViewInfo(OwnerObject);
end;
...
function TdxCustomWheelPickerWheelViewInfo.GetAccessibilityHelperClass: TdxCustomWheelPickerWheelAccessibilityHelperClass;
begin
  Result := TdxCustomWheelPickerWheelAccessibilityHelper;
end;
...
function TdxCustomWheelPickerWheelViewInfo.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin
  ...
  begin
    ...
    begin
      ...
      if AWheelItem.Text <> '' then
        AMaxTextHeight := Max(AMaxTextHeight, TdxTextMeasurer.TextLineHeight(Style.Font));
    end;
...
begin
  ...
  begin
    ...
    MinContentSize.cy := MinContentSize.cy + (TdxTextMeasurer.TextLineHeight(Style.Font) +
...
constructor TdxCustomWheelPickerProperties.Create(AOwner: TPersistent);
begin
  inherited Create(AOwner);
  cxFormatController.AddListener(Self);
end;

destructor TdxCustomWheelPickerProperties.Destroy;
begin
...
procedure TdxCustomWheelPickerProperties.FormatChanged;
begin
  Changed;
end;
...
function TdxWheelPickerAccessibilityHelper.GetChildCount: Integer;
var
  I: Integer;
begin
  inherited;
  for I := 0 to ViewInfo.FWheels.Count - 1 do
    AddAccessibilityChild(TdxCustomWheelPickerWheelViewInfo(ViewInfo.FWheels[I]).IAccessibilityHelper);
  Result := AccessibilityChildren.Count;
end;

function TdxWheelPickerAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxROLE_SYSTEM_LIST;
end;

function TdxWheelPickerAccessibilityHelper.GetWheelPicker: TdxCustomWheelPicker;
begin
  Result := TdxCustomWheelPicker(Edit);
end;

function TdxWheelPickerAccessibilityHelper.GetViewInfo: TdxCustomWheelPickerViewInfo;
begin
  Result := TdxCustomWheelPickerViewInfo(inherited ViewInfo);
end;
...
begin
  ...
  Invalidate;
  if Focused and (ViewInfo.SelectedWheel <> nil) then
    NotifyWinEvent(EVENT_OBJECT_FOCUS, Handle,
      TdxCustomWheelPickerWheelAccessibilityHelper(ViewInfo.SelectedWheel.IAccessibilityHelper.GetHelper).FLocalId, CHILDID_SELF);
end;

function TdxCustomWheelPicker.GetAccessibilityHelperClass: TdxEditAccessibilityHelperClass;
begin
  Result := TdxWheelPickerAccessibilityHelper;
end;
...
begin
  ...
  begin
    ...
    InternalSetSelectedValue(ViewInfo.GetInternalValue);
    if HandleAllocated and (ViewInfo.SelectedWheel <> nil) then
      NotifyWinEvent(EVENT_OBJECT_FOCUS, Handle,
        TdxCustomWheelPickerWheelAccessibilityHelper(ViewInfo.SelectedWheel.IAccessibilityHelper.GetHelper).FLocalId, CHILDID_SELF);
    Invalidate;
  end;
...
begin
  TdxImageDrawer.DrawImage(ACanvas, FImageRect, FGlyph, WheelPickerViewInfo.FImages, FImageIndex,
...
begin
  ...
  begin
    ...
    ARect := cxRect(Result);
    TdxTextMeasurer.TextRectDT(ARect, FText, WheelPickerViewInfo.Font, DT_SINGLELINE);
...
function TdxWheelPickerWheelAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  if AChildID > 0 then
    Result := TdxWheelPickerItemViewInfo(ViewInfo.Items[AChildID - 1]).FText;
end;

{ TdxWheelPickerWheelViewInfo }

function TdxWheelPickerWheelViewInfo.GetAccessibilityHelperClass: TdxCustomWheelPickerWheelAccessibilityHelperClass;
begin
  Result := TdxWheelPickerWheelAccessibilityHelper;
end;
...
procedure TdxCustomWheelPickerViewData.AdjustPadding(var APadding: TdxPadding);
begin
