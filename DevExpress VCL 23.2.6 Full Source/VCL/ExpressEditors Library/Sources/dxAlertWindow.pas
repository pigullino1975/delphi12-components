...
uses
  ...
  Graphics, cxGeometry, Messages, cxLookAndFeelPainters, Math, dxFormattedText,
  cxDrawTextUtils, ImgList, dxCoreClasses, cxLookAndFeels, Menus, dxForms, dxCore;
...
uses
  Types, cxEditConsts, ShellAPI, dxDPIAwareUtils, dxCoreGraphics,
  dxCustomHint, dxFormattedTextConverterBBCode, dxFormattedTextConverterRTF;

const
  dxThisUnitName = 'dxAlertWindow';
...
begin
  ...
  begin
    ARect := Rect(0, 0, AWidth, MAXWORD);
    if CalculateFormattedText(ARect, ATextSize) then
      Result := ATextSize.cy
    else
    begin
