...
type
  ...
  strict private
    ...
    procedure AfterFilterChangeHandler(ASender: TObject);
    procedure BeforeFilterChangeHandler(ASender: TObject);
  ...
  public
    ...
    property ViewInfo: TdxFilterPopupWindowViewInfo read GetViewInfo; // for internal use
  end;
...
uses
  Types, SysUtils, Graphics, Math, Forms, cxDrawTextUtils, cxEdit, cxEditConsts, dxCoreGraphics, dxTypeHelpers;

const
  dxThisUnitName = 'dxFilterPopupWindow';
...
const
  ...
  dxFilterPopupWindowSysPanelButtonMinWidth = 88;
...
begin
  AValue := IfThen(AValue > 0, AValue, GetDesktopWorkArea(Popup.OwnerScreenBounds.TopLeft).Width);
...
begin
  Result := TdxTextMeasurer.TextLineHeight(FilterPopup.Font) + 2 * Painter.ScaledButtonTextOffset(ScaleFactor) + 2 * Painter.ButtonBorderSize;
...
begin
  Result := ScaleFactor.Apply(dxFilterPopupWindowSysPanelButtonMinWidth);
end;
...
procedure TdxFilterPopupWindow.Deactivate;
begin
...
begin
  ...
  FilterValueContainer.OnAfterFilterChange := AfterFilterChangeHandler;
  FilterValueContainer.OnBeforeFilterChange := BeforeFilterChangeHandler;
...
procedure TdxFilterPopupWindow.AfterFilterChangeHandler(ASender: TObject);
begin
  LockDeactivate(False);
end;

procedure TdxFilterPopupWindow.BeforeFilterChangeHandler(ASender: TObject);
begin
  LockDeactivate(True);
end;
