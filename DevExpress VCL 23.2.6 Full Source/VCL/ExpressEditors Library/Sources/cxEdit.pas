...
uses
  System.UITypes,
  Types, Windows, Messages, Variants, Forms, ComCtrls, Classes, Controls, Graphics, Generics.Defaults, Generics.Collections,
...
uses
  ...
  dxThemeConsts, cxDWMApi, Math, cxLibraryConsts, dxHash, dxTypeHelpers;
...
begin
  ...
  AInfo.EditPropertClassName := AEditProperties.ClassName;
...
begin
  for AInfo in FEditPropertyIcons do
    if AInfo.EditPropertClassName = AName then
    begin
...
begin
  inherited;
  UseSkinnedColors := UseSkinnedColors and not IsInplace;
end;
...
function TcxCustomEditViewInfo.ShouldUseStyleBackgroundColor(AStyle: TcxContainerStyle): Boolean;
begin
...
begin
  if AUsePainterPadding then
    Result := Painter.GetEditorPadding(IsInplace, nil)
  else
...
begin
  Result := (Painter <> nil) and Painter.SupportsEditorPadding(IsInplace);
end;

function TcxCustomEditViewInfo.SupportsRoundedContainerBorders: Boolean;
begin
  Result := Painter.SupportsRoundedContainerBorders(-1, IsInplace); 
end;
...
procedure TcxCustomEditViewData.AdjustClientExtentByPadding(var AClientExtent: TRect; ACanvas: TcxCanvas; AViewInfo: TcxCustomEditViewInfo);
  ...
  begin
    ...
    AActualPadding.InflatePadding(AClientExtent, ScaleFactor, UseRightToLeftAlignment);
  end;
...
begin
  ...
  AViewInfo.UseStyleBackgroundColor := AViewInfo.ShouldUseStyleBackgroundColor(Style);
end;
...
begin
  ...
  Windows.ValidateRect(Edit.Parent.Handle, ABoundsForValidate);
end;
...
begin
  ...
  Result.Properties.ScaleFactor.Assign(ScaleFactor);
...
procedure TcxCustomEdit.InternalSetEditValue(const Value: TcxEditValue; AValidateEditValue: Boolean);
var
