...
type
  ...
  protected
    ...
    function GetScaledFontSize(const ABounds: TRect; AFont: TFont; const ACodeText: string): Integer; virtual; // for internal use;
  ...
  protected
    ...
    function GetScaledFontSize(const ABounds: TRect; AFont: TFont; const ACodeText: string): Integer; override;
...
uses
  Math,
  cxDrawTextUtils, dxCoreGraphics;

const
  dxThisUnitName = 'dxBarCodeUtils';
...
function TdxCustomBarCodeGenerator.Paint(APainter: TdxBarCodeCustomPainter; const ABounds: TRect;
  ...
  begin
    ...
    if Symbology.ShowText and (Symbology.FitMode = ifmStretch) then
      Result.cy := cxRectHeight(ABounds) - TdxTextMeasurer.TextLineHeight(AScaledFont) - (GetBottomOffset + GetTopOffset) * Result.cx;
  end;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        {$ELSE}
        AScaledFont.Size := GetScaledFontSize(ABounds, AScaledFont, ACodeText);
        {$ENDIF}
...
function TdxCustomBarCodeGenerator.GetScaledFontSize(const ABounds: TRect;
  AFont: TFont; const ACodeText: string): Integer;
begin
  Result := Trunc(AFont.Size * ScaleX(cxRectSize(ABounds), GetInternalSize(ACodeText, AFont)));
end;
...
begin
  ...
  try
    ...
    {$IFDEF DELPHIXE8}
    AAdjustFontSizeHelper.CalculateHeight(AAvailableWidth, AText);
    AFont.Height := AAdjustFontSizeHelper.Font.Height;
    {$ELSE}
...
function TdxBarCodeEAN13Generator.GetScaledFontSize(const ABounds: TRect;
  AFont: TFont; const ACodeText: string): Integer;
begin
  Result := Trunc(Symbology.ModuleWidth * ScaleX(cxRectSize(ABounds), GetInternalSize(ACodeText, AFont)) * 8.3);
end;
...
function TdxBarCodeEAN8Generator.GetScaledFontSize(const ABounds: TRect;
  AFont: TFont; const ACodeText: string): Integer;
begin
  Result := Trunc(Symbology.ModuleWidth * ScaleX(cxRectSize(ABounds), GetInternalSize(ACodeText, AFont)) * 8.3);
end;
...
function TdxBarCodeUPCAGenerator.GetScaledFontSize(const ABounds: TRect;
  AFont: TFont; const ACodeText: string): Integer;
begin
  Result := Trunc(Symbology.ModuleWidth * ScaleX(cxRectSize(ABounds), GetInternalSize(ACodeText, AFont)) * 8.3);
end;
...
function TdxBarCodeUPCEGenerator.GetScaledFontSize(const ABounds: TRect;
  AFont: TFont; const ACodeText: string): Integer;
begin
  Result := Trunc(Symbology.ModuleWidth * ScaleX(cxRectSize(ABounds), GetInternalSize(ACodeText, AFont)) * 8.3);
end;
...
begin
  Result.cx := GetCodeWidth(ACodeText);
  if Symbology.ShowText then
    Result.cy := DefaultBarHeight + TdxTextMeasurer.TextLineHeight(AFont)
  else
    Result.cy := DefaultBarHeight;
