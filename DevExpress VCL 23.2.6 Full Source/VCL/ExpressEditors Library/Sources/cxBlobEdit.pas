...
uses
  ...
  cxLookAndFeelPainters, dxDPIAwareUtils, dxCoreGraphics, dxSkinInfo;

const
  dxThisUnitName = 'cxBlobEdit';
  ...
  begin
    ...
    begin
      ...
      try
        ...
        if Painter.GetPainterData(AData) then
          APalette := AData.Skin.ActiveColorPalette;
        TdxImageDrawer.DrawImage(ACanvas, IconRect, nil, GetBlobImages, ImageIndex, ifmStretch, EnabledImageDrawModeMap[Enabled], False, APalette);
      finally
...
begin
  if (BlobEditKind = bekAuto) and not((DefaultValuesProvider <> nil) and not DefaultValuesProvider.IsDataAvailable) then
  begin
    LockUpdate(True);
    try
      if DefaultValuesProvider <> nil then
        BlobEditKind := ABlobEditKindCorrectionA[DefaultValuesProvider.DefaultBlobKind]
      else
...
function KeyDownToModalResult(var Key: Word; Shift: TShiftState; AReturnClosed, AAlwaysSaveData: Boolean): TModalResult;
begin
  Result := mrNone;
  ...
  begin
    ...
    if (Key = VK_RETURN) and (AReturnClosed or (ssCtrl in Shift)) then
      Result := mrOk;
...
begin
  ...
  if not IsDestroying and HasPopupWindow and (StorageMemo <> nil) then
    StorageMemo.Style.Color := ViewInfo.BackgroundColor;
end;
...
procedure TcxCustomBlobEdit.CreatePopupControls;
  ...
  begin
    ...
    AButtonHeight := MulDiv(TdxTextMeasurer.TextLineHeight(AFont), 20, 13);
  ...
  case ActiveProperties.BlobEditKind of
    ...
    bekMemo:
      begin
        ...
        StorageMemo.Style.Color := ViewInfo.BackgroundColor;
        StorageMemo.Style.TextColor := VisibleFontColor;
        InitControls;
      end;
  end;
end;
...
begin
  ...
  try
    imgBlobImages := TcxImageList.CreateSize(16, 13);
    for I := Low(ImageNames) to High(ImageNames) do
    begin
      AImage.LoadFromResource(HInstance, ImageNames[I], 'SVG');
      GetBlobImages.Add(AImage, ImageNames[I]);
    end;
  finally
    AImage.Free;
  end;
