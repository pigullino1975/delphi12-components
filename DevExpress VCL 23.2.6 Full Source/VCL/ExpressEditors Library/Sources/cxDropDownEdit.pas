...
uses
  System.UITypes,
  Windows, Messages, Variants, Classes, Controls, Graphics, Forms, SysUtils, StdCtrls, MultiMon,
  ...
  cxTextEdit, cxVariants, cxFilterControlUtils, dxIncrementalFiltering, cxGeometry, cxAccessibility;

const
  dxApplyFormTransparencyToPopupWindows: Boolean = True;
...
type
  ...
  IcxItemIndexHandler = interface // for internal use //# kp: for descendants of TcxCustomEdit
    ['{F740558D-2F86-4623-AA65-33EEA3FBC665}']
    function GetItemIndex: Integer;
    procedure SetItemIndex(AValue: Integer);
    function SupportsItemIndex: Boolean;
    property ItemIndex: Integer read GetItemIndex write SetItemIndex;
  end;

 { IcxValueByItemIndexProvider }

  IcxValueByItemIndexProvider  = interface // for internal use  //# kp: for descendants of TcxCustomEditProperties
    ['{42C0ACEE-B627-41B4-A32B-146357221CCF}']
    function GetItemCount: Integer;
    function FindValueByItemIndex(AItemIndex: Integer; out AValue: Variant): Boolean;
    function SupportsItemIndex: Boolean;
  ...
  public
    ...
    property ViewInfo: TcxCustomEditPopupWindowViewInfo read GetViewInfo; // for internal use
    property OnClosed;
  end;
  ...
  private
    FAllowDropDownWhenReadOnly: Boolean;
    ...
    procedure SetAllowDropDownWhenReadOnly(const AValue: Boolean);
  ...
  protected
    ...
    function DefaultAllowDropDownWhenReadOnly: Boolean; virtual;
  ...
  public
    ...
    // !!!
    property AllowDropDownWhenReadOnly: Boolean read FAllowDropDownWhenReadOnly write SetAllowDropDownWhenReadOnly default True;
  ...
  { TdxCustomDropDownInnerInnerEditAccessibilityHelper }

  TdxCustomDropDownInnerInnerEditAccessibilityHelper = class(TdxCustomInnerTextEditAccessibilityHelper) // for internal use
  protected
  ...
  protected
    // IdxScrollParentWindowNotify
    procedure ScrollParentWindowNotify;
    procedure IdxScrollParentWindowNotify.Notify = ScrollParentWindowNotify;

    function GetAccessibilityHelperClass: TdxCustomInnerTextEditAccessibilityHelperClass; override;

    property Container: TcxCustomDropDownEdit read GetContainer;
  // TODO CLX
  end;

  { TdxCustomDropDownExpandCollapseProvider }

  TdxCustomDropDownExpandCollapseProvider = class(TdxExpandCollapseProvider) // for internal use
  strict private
    FDropDownEdit: TcxCustomDropDownEdit;
  protected
    procedure Collapse; override;
    procedure Expand; override;
    function GetExpandCollapseState: Integer; override;
  public
    constructor Create(ADropDownEdit: TcxCustomDropDownEdit);
  end;

  { TdxCustomDropDownEditAccessibilityHelper }

  TdxCustomDropDownEditAccessibilityHelper = class(TdxCustomTextEditAccessibilityHelper) // for internal use
  private
    FExpandCollapseProvider: TdxCustomDropDownExpandCollapseProvider;

    function GetDropDownEdit: TcxCustomDropDownEdit;
  protected
    procedure DoButtonClick(AVisibleIndex: Integer); override;
    function GetButtonName(AIndex: Integer): string; override;
    function GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer; override;
    function GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer; override;
  ...
  protected
    ...
    procedure HidePopupWindowOnScroll; override;
  ...
  public
    ...
    function DropDownAllowed: Boolean; // for internal use
  ...
  { TcxCustomComboBoxListBoxAccessibilityHelper }

  TcxCustomComboBoxListBox = class;

  TdxCustomComboBoxListBoxAccessibilityHelper = class(TdxCustomEditListBoxAccessibilityHelper) // for internal use
  protected
  ...
  protected
    ...
    procedure MouseMove(Shift: TShiftState; X, Y: Integer); override;
    function NeedApplyVerticalPaddings: Boolean; override;
  ...
  protected
    procedure AdjustStandaloneEditTextOffsets(var AContentBounds: TRect); override;
    function IsComboBoxStyle: Boolean; override;
    function IsOwnerDrawing: Boolean;
  public
    OwnerDrawItemIndex: Integer;

    constructor Create(AProperties: TcxCustomEditProperties;
      AStyle: TcxCustomEditStyle; AIsInplace: Boolean); override;
    procedure Calculate(ACanvas: TcxCanvas; const ABounds: TRect; const P: TPoint;
      Button: TcxMouseButton; Shift: TShiftState; AViewInfo: TcxCustomEditViewInfo;
      AIsMouseEvent: Boolean); override;
  ...
  protected
    // IcxValueByItemIndexProvider
    function SupportsItemIndex: Boolean; virtual;
    function FindValueByItemIndex(AItemIndex: Integer; out AValue: Variant): Boolean; virtual;
  ...
  { TdxCustomComboBoxInnerEditAccessibilityHelper }

  TdxCustomComboBoxInnerEditAccessibilityHelper = class(TdxCustomDropDownInnerInnerEditAccessibilityHelper) // for internal use
  protected
    function GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer; override;
  end;
  ...
  protected
    // IcxItemIndexHandler
    function IcxItemIndexHandler.GetItemIndex = GetHandlerItemIndex;
    procedure IcxItemIndexHandler.SetItemIndex = SetHandlerItemIndex;
    function GetHandlerItemIndex: Integer;
    procedure SetHandlerItemIndex(AValue: Integer);
    function SupportsItemIndex: Boolean; virtual;
    ...
    procedure DoOnDrawItem(ACanvas: TcxCanvas; AIndex: Integer; const ARect: TRect; AState: TOwnerDrawState);
    procedure DoOnMeasureItem(AIndex: Integer; ACanvas: TcxCanvas; var AHeight: Integer);
  ...
  public
    ...
    property ItemIndex; 
    property ItemObject;
    property Properties: TcxCustomComboBoxProperties read GetProperties write SetProperties;
...
type
  ...
  { TcxInplaceComboBoxCustomDrawHelper }

  TcxInplaceComboBoxCustomDrawHelper = class(TcxCustomComboBox)
  strict private
    class var FInstance: TcxInplaceComboBoxCustomDrawHelper;
  public
    class procedure CustomDrawItem(AProperties: TcxCustomComboBoxProperties;
...
class procedure TcxInplaceComboBoxCustomDrawHelper.CustomDrawItem(
  AProperties: TcxCustomComboBoxProperties; ACanvas: TcxCanvas;
  AIndex: Integer; const ARect: TRect; AState: TOwnerDrawState);
begin
  if FInstance = nil then
    FInstance := TcxInplaceComboBoxCustomDrawHelper.Create(nil, True);

  ExchangePointers(FInstance.FProperties, AProperties);
  try
    FInstance.DoOnDrawItem(ACanvas, AIndex, ARect, AState);
  finally
    ExchangePointers(FInstance.FProperties, AProperties);
  end;
end;

class procedure TcxInplaceComboBoxCustomDrawHelper.Finalize;
begin
...
begin
  ...
  ADesktopArea := GetDesktopWorkArea(AEditBounds);
...
begin
  ...
  FAllowDropDownWhenReadOnly := DefaultAllowDropDownWhenReadOnly;
...
begin
  ...
  if AProperties is TcxCustomDropDownEditProperties then
    ...
    begin
      Self.AllowDropDownWhenReadOnly := AllowDropDownWhenReadOnly;
  ...
  try
    ...
    try
      ...
      if Buttons.Count > 0 then
        Buttons[0].Enabled := (not ReadOnly) or AllowDropDownWhenReadOnly;
    finally
      Buttons.EndUpdate;
    end;
  finally
...
begin
  if (Buttons.Count <> 0) and Buttons[0].Visible then
    Result := 0
  else
    Result := -1;
end;

function TcxCustomDropDownEditProperties.DefaultAllowDropDownWhenReadOnly: Boolean;
begin
  Result := True;
end;
...
procedure TcxCustomDropDownEditProperties.SetAllowDropDownWhenReadOnly(
  const AValue: Boolean);
begin
  if AValue <> FAllowDropDownWhenReadOnly then
  begin
    FAllowDropDownWhenReadOnly := AValue;
    Changed;
  end;
end;
...
function TdxCustomDropDownInnerInnerEditAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  if not InnerTextEdit.Focused then
    Result := cxROLE_SYSTEM_STATICTEXT
  else
    Result := inherited;
end;

function TdxCustomDropDownInnerInnerEditAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := inherited;
  if TcxCustomDropDownEdit(CustomTextEdit).DroppedDown then
    Result := Result xor cxSTATE_SYSTEM_FOCUSED;
end;

{ TcxCustomDropDownInnerEdit }

procedure TcxCustomDropDownInnerEdit.ScrollParentWindowNotify;
begin
  Container.DroppedDown := False;
end;

function TcxCustomDropDownInnerEdit.GetAccessibilityHelperClass: TdxCustomInnerTextEditAccessibilityHelperClass;
begin
  Result := TdxCustomDropDownInnerInnerEditAccessibilityHelper;
end;
...
{ TdxCustomDropDownRangeValueProvider }

constructor TdxCustomDropDownExpandCollapseProvider.Create(ADropDownEdit: TcxCustomDropDownEdit);
begin
  FDropDownEdit := ADropDownEdit;
end;

procedure TdxCustomDropDownExpandCollapseProvider.Collapse;
begin
  FDropDownEdit.DroppedDown := False;
end;

procedure TdxCustomDropDownExpandCollapseProvider.Expand;
begin
  FDropDownEdit.DroppedDown := True;
end;

function TdxCustomDropDownExpandCollapseProvider.GetExpandCollapseState: Integer;
begin
  Result := Integer(FDropDownEdit.DroppedDown);
end;

{ TdxCustomDropDownEditAccessibilityHelper }

constructor TdxCustomDropDownEditAccessibilityHelper.Create(AOwnerObject: TObject);
begin
  inherited;
  FExpandCollapseProvider := TdxCustomDropDownExpandCollapseProvider.Create(TcxCustomDropDownEdit(Edit));
end;

destructor TdxCustomDropDownEditAccessibilityHelper.Destroy;
begin
  FreeAndNil(FExpandCollapseProvider);
  inherited;
end;

procedure TdxCustomDropDownEditAccessibilityHelper.DoButtonClick(AVisibleIndex: Integer);
begin
  if AVisibleIndex = DropDownEdit.ActiveProperties.DropDownButtonVisibleIndex then
    DropDownEdit.DroppedDown := True;
  inherited;
end;

function TdxCustomDropDownEditAccessibilityHelper.GetButtonName(AIndex: Integer): string;
begin
  if AIndex = 0 then
  begin
    if DropDownEdit.DroppedDown then
      Result := 'Close'
    else
      Result := 'Open';
  end;
...
function TdxCustomDropDownEditAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxROLE_SYSTEM_COMBOBOX;
end;

function TdxCustomDropDownEditAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  if DropDownEdit.DroppedDown then
    Result := inherited or cxSTATE_SYSTEM_EXPANDED
  else
    Result := inherited or cxSTATE_SYSTEM_COLLAPSED;
end;

function TdxCustomDropDownEditAccessibilityHelper.GetValue(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := VarToStr(DropDownEdit.InnerEdit.EditValue);
end;

function TdxCustomDropDownEditAccessibilityHelper.IsSupportedPattern(APatternID: Integer;
  out AProvider: IUnknown; AChildID: TcxAccessibleSimpleChildElementID = 0): Boolean;
begin
...
function TdxCustomDropDownEditAccessibilityHelper.GetDropDownEdit: TcxCustomDropDownEdit;
begin
...
begin
  ...
  if IsTextChar(Key) and ActiveProperties.ImmediateDropDownWhenKeyPressed and
    ...
    DroppedDown := True;
  if not SendActivationKey(Key) and DropDownAllowed and (GetPopupFocusedControl <> nil) then
...
begin
  if (Key in [VK_UP, VK_DOWN]) and (ssAlt in Shift) or (Key = VK_F4) and not (ssAlt in Shift) then
  begin
    if HasPopupWindow then
      CloseUp(crClose)
    else
    ...
    if (Key in [VK_RETURN, VK_ESCAPE]) and not(ssAlt in Shift) and HasPopupWindow then
    begin
      FIsPopupWindowJustClosed := True;
      if (Key = VK_RETURN) and DoEditing then
        CloseUp(crEnter)
      else
        CloseUp(crCancel);
...
begin
  ...
  begin
    ...
    if IsPopupWindowCaptureFocus and (Key <> dxVKReturn) and DropDownAllowed and (GetPopupFocusedControl <> nil) then
    begin
      PostMessage(PopupWindow.Handle, DXM_POPUPCONTROLKEY, WPARAM(Key), 0);
      Key := #0;
    end;
...
procedure TcxCustomDropDownEdit.HidePopupWindowOnScroll;
begin
  DroppedDown := False;
end;
...
function TcxCustomDropDownEdit.GetAccessibilityHelperClass: TdxEditAccessibilityHelperClass;
begin
  Result := TdxCustomDropDownEditAccessibilityHelper;
end;
...
var
  AProperties: TcxCustomDropDownEditProperties;
begin
  AProperties := ActiveProperties;
  FPopupSizeChanged := False;
  TcxCustomDropDownEditData(EditData).Initialized := False;
  TcxCustomDropDownEditData(EditData).Width := AProperties.ScaleFactor.Revert(AProperties.PopupWidth);
  TcxCustomDropDownEditData(EditData).Height := AProperties.ScaleFactor.Revert(AProperties.PopupHeight);
end;
...
begin
  Result := not(ActiveProperties.ImmediateDropDownWhenKeyPressed and IsPopupWindowCaptureFocus and (Key <> dxVKReturn));
end;
  ...
  if RepositoryItem <> nil then
    with ActiveProperties do
      if Assigned(OnCloseUp) then
        OnCloseUp(Self);
  if HandleAllocated then
    NotifyWinEvent(UIA_ExpandCollapseExpandCollapseStatePropertyId, Handle, OBJID_CLIENT, CHILDID_SELF);
end;
...
begin
  ...
  if IsWinXPOrLater and dxApplyFormTransparencyToPopupWindows then // W2K bug
  begin
...
begin
  AProperties := ActiveProperties;
  if AProperties.PopupSizeable then
    if IsInplace then
    begin
      if EditData <> nil then
      begin
        TcxCustomDropDownEditData(EditData).Initialized := True;
        TcxCustomDropDownEditData(EditData).Height := PopupWindow.ScaleFactor.Revert(PopupWindow.Height);
        TcxCustomDropDownEditData(EditData).Width := PopupWindow.ScaleFactor.Revert(PopupWindow.Width);
      end
    end
    else
    begin
      AProperties.PopupWidth := AProperties.ScaleFactor.Apply(PopupWindow.Width, PopupWindow.ScaleFactor);
      AProperties.PopupHeight := AProperties.ScaleFactor.Apply(PopupWindow.Height, PopupWindow.ScaleFactor);
    end;
end;
...
function TcxCustomDropDownEdit.DropDownAllowed: Boolean;
begin
  Result := CanDropDown and ((not ActiveProperties.ReadOnly) or ActiveProperties.AllowDropDownWhenReadOnly);
end;
...
begin
  if IsInplace and ActiveProperties.PopupSizeable then
    Result := ScaleFactor.Apply(TcxCustomDropDownEditData(EditData).Height)
  else
...
begin
  if IsInplace and ActiveProperties.PopupSizeable then
    Result := ScaleFactor.Apply(TcxCustomDropDownEditData(EditData).Width)
  else
...
procedure TdxCustomComboBoxListBoxAccessibilityHelper.DoDefaultAction(AChildID: TcxAccessibleSimpleChildElementID);
begin
  if AChildID > 0 then
  begin
    TcxCustomComboBoxListBox(ListBox).Edit.ItemIndex := AChildID - 1;
    ListBox.ItemIndex := AChildID - 1;
  end;
end;

procedure TdxCustomComboBoxListBoxAccessibilityHelper.DoSelect(AFlags: Integer; AChildID: TcxAccessibleSimpleChildElementID);
begin
  ListBox.ItemIndex := AChildID - 1;
end;
...
function TcxCustomComboBoxListBox.GetDefaultItemHeight: Integer;
begin
...
function TcxCustomComboBoxListBox.NeedApplyVerticalPaddings: Boolean;
begin
  Result := not Edit.IsOnMeasureItemEventAssigned
            and (Edit.ActiveProperties.ItemHeight = 0)
            and inherited NeedApplyVerticalPaddings;
end;
...
function TcxCustomComboBoxListBox.GetAccessibilityHelperClass: TdxCustomInnerListBoxAccessibilityHelperClass;
begin
  Result := TdxCustomInnerListBoxAccessibilityHelper;
end;

function TcxCustomComboBoxListBox.DoCustomDrawItem(AIndex: Integer; const ARect: TRect; AState: TOwnerDrawState): Boolean;
begin
...
procedure TcxComboBoxListBox.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  inherited;
  FMousePressed := Button = mbLeft;
end;

procedure TcxComboBoxListBox.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  AItemIndex: Integer;
begin
  inherited MouseUp(Button, Shift, X, Y);

  if (Button = mbLeft) and FMousePressed then
  try
...
begin
  ...
  AExtraPadding := TcxContainerAccess(ListContainer).GetBorderExtent;
  Inc(Result.cy, AExtraPadding.Top + AExtraPadding.Bottom);
  if UsePreferredWidth then
  begin
    Result.cx := AWidth;
    List.SetScrollWidth(0);
  end
  else
  begin
    AScrollWidth := GetScrollWidth;
    if AScrollWidth > List.ClientWidth then 
      List.SetScrollWidth(AScrollWidth)
    else
      List.SetScrollWidth(0);
    Result.cx := AScrollWidth + (AExtraPadding.Left + AExtraPadding.Right);
...
function TcxComboBoxLookupData.GetScrollWidth: Integer;
var
  AItemWidth: Integer;
  I: Integer;
begin
...
function TcxComboBoxLookupData.UsePreferredWidth: Boolean;
begin
  Result := False;
end;
...
constructor TcxCustomComboBoxViewData.Create(
  AProperties: TcxCustomEditProperties; AStyle: TcxCustomEditStyle; AIsInplace: Boolean);
begin
  inherited Create(AProperties, AStyle, AIsInplace);
  OwnerDrawItemIndex := -1;
end;
...
begin
  ...
  begin
    if IsOwnerDrawing then
      OwnerDrawItemIndex := TcxCustomComboBox(Edit).ItemIndex
    else
      OwnerDrawItemIndex := -1;
  end;

  TcxCustomComboBoxViewInfo(AViewInfo).ItemIndex := OwnerDrawItemIndex;
  TcxCustomComboBoxViewInfo(AViewInfo).IsOwnerDrawing := OwnerDrawItemIndex >= 0;
end;
...
begin
  ...
  if IsOwnerDrawing then
    OwnerDrawItemIndex := Properties.LookupItems.IndexOf(VarToStr(AEditValue))
  else
    OwnerDrawItemIndex := -1;
end;

procedure TcxCustomComboBoxViewData.AdjustStandaloneEditTextOffsets(var AContentBounds: TRect);
begin
...
function TcxCustomComboBoxViewData.IsOwnerDrawing: Boolean;
begin
  Result := (Properties.DropDownListStyle = lsFixedList) and Assigned(Properties.OnDrawItem);
end;
...
begin
  ...
  if FEdit <> nil then
    ...
    TcxInplaceComboBoxCustomDrawHelper.CustomDrawItem(
      TcxCustomComboBoxProperties(EditProperties), ACanvas, ItemIndex, ARect, AState);
end;
...
function TcxCustomComboBoxProperties.SupportsItemIndex: Boolean;
begin
  Result := False;
end;

function TcxCustomComboBoxProperties.FindValueByItemIndex(AItemIndex: Integer; out AValue: Variant): Boolean;
begin
  Result := False;
end;

function TcxCustomComboBoxProperties.GetItemCount: Integer;
begin
  Result := Items.Count;
end;
...
function TdxCustomComboBoxInnerEditAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  if (EditProperties is TcxCustomComboBoxProperties) and
    (TcxCustomComboBoxProperties(EditProperties).DropDownListStyle <> lsEditList) then
      Result := cxROLE_SYSTEM_STATICTEXT
  else
    Result := inherited;
end;
...
function TcxCustomComboBoxInnerEdit.GetAccessibilityHelperClass: TdxCustomInnerTextEditAccessibilityHelperClass;
begin
  Result := TdxCustomComboBoxInnerEditAccessibilityHelper;
end;
...
function TcxCustomComboBox.GetHandlerItemIndex: Integer;
begin
  Result := ItemIndex;
end;

procedure TcxCustomComboBox.SetHandlerItemIndex(AValue: Integer);
begin
  ItemIndex := AValue;
end;

function TcxCustomComboBox.SupportsItemIndex: Boolean;
begin
  Result := False;
end;
...
begin
  Result := Assigned(Properties.OnDrawItem) or Assigned(ActiveProperties.OnDrawItem);
end;
...
function TcxComboBox.SupportsItemIndex: Boolean;
begin
  Result := True;
end;
...
function TcxComboBoxProperties.FindValueByItemIndex(AItemIndex: Integer;
...
function TcxComboBoxProperties.SupportsItemIndex: Boolean;
begin
  Result := True;
end;
