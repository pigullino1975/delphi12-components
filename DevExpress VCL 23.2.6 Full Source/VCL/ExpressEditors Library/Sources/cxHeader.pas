...
uses
  ...
  cxEdit, cxEditUtils, cxExtEditConsts, cxExtEditUtils, dxCoreClasses;
...
type
  ...
  public
    ...
    function HasAutoSize: Boolean; // for internal use
...
procedure DrawScaledHeaderSection(AHeaderHandle: HWND; AIndex: Integer; ACanvas: TcxCanvas; ALookAndFeel: TcxLookAndFeel;
  AScaleFactor: TdxScaleFactor; AImages: TCustomImageList = nil; ABorders: TcxBorders = cxBordersAll);
...
uses
  Forms, Types, Math, dxThemeConsts, dxThemeManager, dxUxTheme, dxDPIAwareUtils, dxCoreGraphics;

const
  dxThisUnitName = 'cxHeader';
...
procedure DrawScaledHeaderSection(AHeaderHandle: HWND; AIndex: Integer; ACanvas: TcxCanvas;
  ALookAndFeel: TcxLookAndFeel; AScaleFactor: TdxScaleFactor; AImages: TCustomImageList = nil; ABorders: TcxBorders = cxBordersAll);
const
  ImageTextOffset = 2;
  SortingMarkOffset = 4;
...
begin
  ...
  try
    ...
    ALookAndFeel.Painter.DrawScaledHeaderControlSection(ACanvas, AHeaderItemInfo.Rect, ATextRect, ANeighbors,
      ABorders, AHeaderItemInfo.State, ATextAlignment, vaCenter, False, True,
    ...
    if IsImageAssigned(AImages, AHeaderItemInfo.ImageIndex) then
      TdxImageDrawer.DrawImage(ACanvas, AImageRect, nil, AImages, AHeaderItemInfo.ImageIndex, ifmNormal, idmNormal, False, nil, AScaleFactor);
...
begin
  ...
  Result := Max(TdxTextMeasurer.TextLineHeight(Canvas.Font), dxGetImageSize(Images, ScaleFactor).cy) +
...
begin
  ...
  begin
    ...
    begin
      ...
      if LookAndFeel.Painter.IncludeTopBorderToSectionHeaderForLightBorders then
        Include(ABorders, bTop);
      if SectionIndex = 0 then
        Exclude(ABorders, bLeft);
      if (SectionIndex = FSections.Count - 1) and FSections.HasAutoSize then
        Exclude(ABorders, bRight);
      DrawScaledHeaderSection(Handle, SectionIndex, Canvas, LookAndFeel, ScaleFactor, Images, ABorders);
    end;
  end;
end;
...
function TcxHeaderSections.HasAutoSize: Boolean;
var
  I: Integer;
begin
  Result := True;
