...
type
  ...
  private
    ...
    FIsStored: Boolean;
  {$IFDEF DELPHI120}
    FOwnerDataItemCount: Integer;
    FOwnerDataSelectedItemIndex: Integer;
  {$ENDIF}
    FSelectionStream: TMemoryStream;
  ...
  TcxListViewContainerStyle = class(TcxContainerStyle)
  protected
  ...
  protected
    ...
    function HandleInnerControlGestures: Boolean; override;
    ...
    function UseBorderedBackground: Boolean; override;
...
function TcxListViewContainer.HandleInnerControlGestures: Boolean;
begin
  Result := True;
end;
...
function TcxListViewContainer.UseBorderedBackground: Boolean;
begin
  Result := False;
end;
...
function TcxListViewContainer.GetStyleClass: TcxContainerStyleClass;
begin
  Result := TcxListViewContainerStyle;
end;
...
begin
  inherited Create(AOwner);
{$IFDEF DELPHI120}
  FOwnerDataItemCount := -1;
  FOwnerDataSelectedItemIndex := -1;
{$ENDIF}
  FCanvas := TcxCanvas.Create(inherited Canvas);
  BorderStyle := bsNone;
  ControlStyle := ControlStyle + [csDoubleClicks, csOverrideStylePaint];
  ParentFont := True;
...
procedure TcxBaseInnerListView.SaveSelection;
var
  i: Integer;
begin
  if OwnerData then Exit;
  if FSelectionStream = nil then
    FSelectionStream := TMemoryStream.Create
  else
    FSelectionStream.Size := 0;
  for i := 0 to Items.Count - 1 do
  begin
    if Items[i].Selected then
      FSelectionStream.Write(i, SizeOf(i));
  end;
  FSelectionStream.Position := 0;
end;
...
{$IFDEF DELPHI120}
  if OwnerData then
  begin
    if (FOwnerDataItemCount <> -1) and (Items.Count = 0) then
      Items.Count := FOwnerDataItemCount;
    if (FOwnerDataSelectedItemIndex <> -1) and (FOwnerDataSelectedItemIndex < Items.Count)  then
      Selected := Items[FOwnerDataSelectedItemIndex];
  end;
  FOwnerDataItemCount := -1;
  FOwnerDataSelectedItemIndex := -1;
{$ENDIF}
  if MultiSelect and (FSelectionStream <> nil) then
    RestoreSelection;
end;
...
begin
  ...
  {$IFDEF DELPHI120}
    ...
    if OwnerData then
    begin
      FOwnerDataItemCount := Items.Count;
      var ASelectedItem: TListItem := Selected;
      if Assigned(ASelectedItem) then
        FOwnerDataSelectedItemIndex := ASelectedItem.Index;
      Items.Count := 0;
    end;
  {$ENDIF}
  end;
...
procedure TcxBaseInnerListView.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TcxBaseInnerListView.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TcxBaseInnerListView.DrawHeader;
  ...
  function HasAutosizeColumn: Boolean;
  var
    I: Integer;
  begin
    Result := True;
...
begin
  ...
  ALookAndFeelPainter := GetLookAndFeelPainter;
  Canvas.Brush.Color := ALookAndFeelPainter.DefaultHeaderColor;
  Canvas.Font := Font;
  Canvas.Font.Color := ALookAndFeelPainter.DefaultHeaderTextColor;

  if not TdxVisualRefinements.LightBorders then
  begin
    for I := 0 to Columns.Count do
      DrawScaledHeaderSection(FHeaderHandle, I, Canvas, ALookAndFeel, AScaleFactor, SmallImages);
  end
  else
  begin
    AIncludeTopBorder := ALookAndFeelPainter.IncludeTopBorderToSectionHeaderForLightBorders;
    for I := 0 to Columns.Count do
    begin
      ABorders := [bLeft, bRight, bBottom];
      if AIncludeTopBorder then
        Include(ABorders, bTop);
      if I = 0 then
        Exclude(ABorders, bLeft);
      if (I = Columns.Count - 1) and HasAutosizeColumn then
        Exclude(ABorders, bRight);
      DrawScaledHeaderSection(FHeaderHandle, I, Canvas, ALookAndFeel, AScaleFactor, SmallImages, ABorders);
    end;
  end;
end;
...
procedure TcxBaseInnerListView.RestoreSelection;
var
  AOnChangeSave: TLVChangeEvent;
  AOnSelectItemSave: TLVSelectItemEvent;
  Index: Integer;
begin
  if not OwnerData and (FSelectionStream <> nil) then
  begin
    Items.BeginUpdate;
    AOnChangeSave := OnChange;
    OnChange := nil;
    AOnSelectItemSave := OnSelectItem;
    OnSelectItem := nil;
    try
      while (FSelectionStream.Position < FSelectionStream.Size) do
      begin
        FSelectionStream.ReadBuffer(Index, SizeOf(Index));
        if Index < Items.Count then
          Items[Index].Selected := True;
      end;
    finally
      Items.EndUpdate;
      OnChange := AOnChangeSave;
      OnSelectItem := AOnSelectItemSave;
    end;
  end;
  FSelectionStream.Free;
  FSelectionStream := nil;
end;
...
procedure TcxBaseInnerListView.CMRecreateWnd(var Message: TMessage);
var
  AHasHandle: Boolean;
begin
...
{ TcxListViewContainerStyle }

function TcxListViewContainerStyle.DefaultTransparentBorder: Boolean;
begin
  if not TdxVisualRefinements.LightBorders then
