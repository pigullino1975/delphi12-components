...
{$ENDIF}
  ...
  dxBarCode, dxBarCodeUtils, dxFormattedText, cxCustomCanvas;
...
{$ENDIF}
  ...
  TcxButtonSelectionEditor = class(TdxSkinsBaseSelectionEditor)
  public
    procedure RequiresUnits(Proc: TGetStrProc); override;
  end;
  ...
  { TcxImageComponentEditor }

  TcxImageComponentEditor = class(TcxEditorsLibraryCXControlComponentEditor)
  protected
    function InternalGetVerb(AIndex: Integer): string; override;
    function InternalGetVerbCount: Integer; override;
    procedure InternalExecuteVerb(AIndex: Integer); override;
...
{ TdxWheelPickerWheelsPropertyEditor }
  ...
  { TdxFormattedLabelFormattedTextPropertyEditor }

  TdxFormattedLabelFormattedTextPropertyEditor = class(TStringProperty)
  public
    procedure Edit; override;
    function GetAttributes: TPropertyAttributes; override;
    function GetValue: string; override;
  ...
  { TdxFormattedHintPropertyEditor }

  TdxFormattedHintPropertyEditor = class(TStringProperty)
  public
    procedure Edit; override;
    function GetAttributes: TPropertyAttributes; override;
  end;

  { TdxFormattedLabelComponentEditor }

  TdxFormattedLabelComponentEditor = class(TcxEditComponentEditor)
  protected
    function HasEditor: Boolean; override;
    procedure RunEditor; override;
  public
  ...
  { TdxFormattedTextPropertyEditor }

  TdxFormattedTextPropertyEditor = class(TStringProperty)
  private
    function UseFormattedTextEditor(var AIntf: IdxFormattedTextOwner): Boolean;
  public
    procedure Edit; override;
    function GetAttributes: TPropertyAttributes; override;
    function GetValue: string; override;
  ...
  { TdxHatchStylesProperty }

  TdxHatchStylesProperty = class(TEnumProperty,
    ICustomPropertyDrawing,
    ICustomPropertyListDrawing,
    ICustomPropertyDrawing80)
  protected
    function DrawValue(AValue: TdxFillOptionsHatchStyle; ACanvas: TCanvas; const ARect: TRect): TRect;
  public
    function GetAttributes: TPropertyAttributes; override;
    procedure PropDrawName(ACanvas: TCanvas; const ARect: TRect; ASelected: Boolean);
    procedure PropDrawValue(ACanvas: TCanvas; const ARect: TRect; ASelected: Boolean);
...
uses
  SysUtils, dxTypeInfo, cxButtons, cxClasses, cxControls, cxEditMaskEditor, dxGDIPlusClasses, dxGDIPlusAPI,
  cxEditRepositoryEditor, cxImage, cxImageComboBox, cxListBox, cxMaskEdit, dxTypeHelpers,
  ...
  cxCustomData, cxGraphics, cxGeometry, Math, dxColorDialog, dxAlertWindowDesignTester,
  dxActivityIndicator, dxWheelPicker, dxWheelPickerItemIndexesEditor, dxSparkline, dxTokenEdit, dxFormattedLabel,
  dxFormattedTextConverterRTF, dxFormattedTextConverterBBCode, dxFormattedLabelCaptionEditor, dxFormattedHintEditor, dxPictureEditor;

const
  dxThisUnitName = 'cxEditPropEditors';
...
procedure TcxImageComponentEditor.Edit;
var
  APictureEditor: TdxPictureEditor;
  APicture: TPicture;
begin
  APicture := (Component as TcxCustomImage).Picture;
  APictureEditor := TdxPictureEditor.Create(nil);
  try
    APictureEditor.GraphicClass := TdxSmartImage;
    APictureEditor.Picture.Assign(APicture);
    APictureEditor.OpenPictureEditor := True;
    if APictureEditor.Execute(Designer, APicture.Graphic) then
    begin
      APicture.Assign(APictureEditor.Picture.Graphic);
      Designer.Modified;
    end;
  finally
...
procedure TcxImageComponentEditor.InternalExecuteVerb(AIndex: Integer);
begin
  if AIndex = 0 then
    Edit
  else
...
function TcxImageComponentEditor.InternalGetVerb(AIndex: Integer): string;
begin
  if AIndex = 0 then
    Result := 'Edit...'
  else
...
function TcxImageComponentEditor.InternalGetVerbCount: Integer;
begin
  Result := inherited InternalGetVerbCount + 1;
end;
...
begin
  ...
  AValue := '';
  TdxTypeInfo.EnumProperties(GetPropType, [tkUnknown..tkDynArray],
    procedure(APropInfo: PPropInfo; var ABreak: Boolean)
    var
      APropertyName: string;
    begin
      APropertyName := TdxTypeInfo.GetPropertyName(APropInfo);
      if Boolean(GetOrdProp(AAssignedValues, APropInfo)) and IsVisibleProperty(APropertyName) then
      begin
        if AValue <> '' then
          AValue := AValue + ',';
        AValue := AValue + APropertyName;
      end;
    end);
...
begin
  Result := TdxAlphaColors.GetPropertyValue(GetOrdValue);
end;

procedure TdxAlphaColorPropertyEditor.GetValues(Proc: TGetStrProc);
begin
  TdxAlphaColors.GetPropertyValues(Proc);
end;

procedure TdxAlphaColorPropertyEditor.SetValue(const Value: string);
begin
  SetOrdValue(TdxAlphaColors.SetPropertyValue(Value));
end;
...
begin
  ...
  DrawPreview(ACanvas, APreviewRect, TdxAlphaColors.FromString(Value), ASelected);
...
begin
  ...
  try
    ...
    cxDrawTransparencyCheckerboard(ACanvas.Handle, R, cxTransparencyCheckerboardSizeSmall);
...
procedure TdxFormattedLabelFormattedTextPropertyEditor.Edit;
begin
  if dxShowFormattedLabelCaptionEditor(GetComponent(0) as TdxFormattedLabel) then
    Designer.Modified;
end;

function TdxFormattedLabelFormattedTextPropertyEditor.GetAttributes: TPropertyAttributes;
begin
  Result := [paDialog];
end;

function TdxFormattedLabelFormattedTextPropertyEditor.GetValue: string;
begin
  Result := (GetComponent(0) as TdxFormattedLabel).Caption;
end;

{ TdxFormattedLabelComponentEditor }

procedure TdxFormattedLabelComponentEditor.Edit;
begin
  RunEditor;
end;

function TdxFormattedLabelComponentEditor.HasEditor: Boolean;
begin
  Result := True;
end;

procedure TdxFormattedLabelComponentEditor.RunEditor;
begin
  if dxShowFormattedLabelCaptionEditor(Component as TdxFormattedLabel) then
    Designer.Modified;
end;

{ TdxFormattedTextPropertyEditor }

procedure TdxFormattedTextPropertyEditor.Edit;
var
  AIntf: IdxFormattedTextOwner;
  AText: string;
begin
  if UseFormattedTextEditor(AIntf) then
  begin
    AText := AIntf.GetFormattedText;
    dxShowFormattedTextEditor(AText, AIntf.GetFormattedTextFont);
    AIntf.SetFormattedText(AText);
  end
  else
    inherited Edit;
end;

function TdxFormattedTextPropertyEditor.GetAttributes: TPropertyAttributes;
var
  AIntf: IdxFormattedTextOwner;
begin
  if UseFormattedTextEditor(AIntf) then
...
function TdxFormattedTextPropertyEditor.GetValue: string;
var
  AIntf: IdxFormattedTextOwner;
begin
  if UseFormattedTextEditor(AIntf) then
    Result := AIntf.GetFormattedText
  else
    Result := inherited;
end;

function TdxFormattedTextPropertyEditor.UseFormattedTextEditor(var AIntf: IdxFormattedTextOwner): Boolean;
begin
  Result := Supports(GetComponent(0), IdxFormattedTextOwner, AIntf) and AIntf.GetUseFormattedText;
end;

{ TdxHatchStylesProperty }

procedure TdxHatchStylesProperty.ListDrawValue(const Value: string;
...
begin
  ...
  R := DrawValue(TdxFillOptionsHatchStyle(I), ACanvas, ARect);
  DefaultPropertyListDrawValue(Value, ACanvas, R, ASelected);
end;

procedure TdxHatchStylesProperty.ListMeasureHeight(const Value: string;
  ACanvas: TCanvas; var AHeight: Integer);
begin
// do nothing
end;

procedure TdxHatchStylesProperty.ListMeasureWidth(const Value: string;
  ACanvas: TCanvas; var AWidth: Integer);
begin
// do nothing
end;

function TdxHatchStylesProperty.DrawValue(AValue: TdxFillOptionsHatchStyle;
  ACanvas: TCanvas; const ARect: TRect): TRect;
var
  R: TRect;
...
begin
  R := ARect;
  R.Width := R.Height;
  R := cxRectInflate(R, -2, -2);

  ABitmap := TcxBitmap.CreateSize(R);
  try
    ABrush := ABitmap.cxCanvas.CreateBrush(TdxGPHatchBrush.Create(TdxAlphaColors.White, TdxAlphaColors.Black, TdxGpHatchStyle(AValue)), ooOwned);
    try
...
function TdxHatchStylesProperty.GetAttributes: TPropertyAttributes;
begin
  Result := [paMultiSelect, paValueList, paRevertable];
end;

procedure TdxHatchStylesProperty.PropDrawName(ACanvas: TCanvas;
  const ARect: TRect; ASelected: Boolean);
begin
  DefaultPropertyDrawName(Self, ACanvas, ARect);
end;

function TdxHatchStylesProperty.PropDrawNameRect(const ARect: TRect): TRect;
begin
  Result := ARect;
end;

procedure TdxHatchStylesProperty.PropDrawValue(ACanvas: TCanvas;
  const ARect: TRect; ASelected: Boolean);
var
  R: TRect;
begin
  if GetVisualValue <> '' then
    R := DrawValue(TdxFillOptionsHatchStyle(GetOrdValue), ACanvas, ARect)
  else
    DefaultPropertyDrawValue(Self, ACanvas, ARect);
end;

function TdxHatchStylesProperty.PropDrawValueRect(const ARect: TRect): TRect;
begin
  Result := cxRectSetWidth(ARect, cxRectHeight(ARect));
end;
...
procedure TdxFormattedHintPropertyEditor.Edit;
var
  aHintText: string;
begin
  aHintText := GetValue;
  if dxShowFormattedHintEditor(aHintText, TComponent(GetComponent(0))) then
    SetStrValue(aHintText);
end;

function TdxFormattedHintPropertyEditor.GetAttributes: TPropertyAttributes;
begin
