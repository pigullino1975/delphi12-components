...
uses
  System.UITypes,
  Windows, Messages, SysUtils, Classes, Controls, Graphics, Forms, Themes, Generics.Defaults, Generics.Collections,
  dxCore, dxMessages, cxControls, cxGraphics, cxClasses, cxLookAndFeels, dxFluentDesignFormInterfaces, cxContainer,
...
type
  ...
  protected
    function CalculateBorderColor(AStyle: TcxContainerStyle; AEnabled, AIsDesigning: Boolean; AContainer: TcxContainer = nil): TColor; override;
  ...
  protected
    function CalculateReadOnly: Boolean; override;
  ...
  public
    ...
    function IgnorePainterBorderSettings: Boolean; override;
  ...
  protected
    ...
    procedure GetEditMetrics(AAutoHeight: Boolean; ACanvas: TcxCanvas; out AMetrics: TcxEditMetrics); virtual;
    function GetCaptionRectExtent: TRect; virtual;
    function GetBiDiModeDependentAlignment(AAlignment: TLeftRight): TLeftRight;
    function GetButtonSize: TSize; virtual;
  public
    procedure Calculate(ACanvas: TcxCanvas; const ABounds: TRect; const P: TPoint;
      Button: TcxMouseButton; Shift: TShiftState; AViewInfo: TcxCustomEditViewInfo;
      AIsMouseEvent: Boolean); override;
  ...
  TcxGroupBoxStyle = class(TcxEditStyle)
  protected
    function UsePainterBorderSettings: Boolean; override;
  end;
  ...
  private
    ...
    FContentExtentsUpdating: Boolean;
  ...
  protected
    FCaptionFont: TFont;
    FTextRect: TdxCachedRect;

    procedure AdjustClientRect(var Rect: TRect); override;
    procedure CalculateViewInfo(P: TPoint; Button: TcxMouseButton; Shift: TShiftState; AIsMouseEvent: Boolean); overload; override;
    ...
    procedure ShortRefreshContainerOnWindowPosChanging; override;
  ...
  public
    ...
    property Alignment: TcxCaptionAlignment read FAlignment write SetAlignment default alTopLeft;
  ...
  protected
    ...
    function NeedUpdateWindowRegion: Boolean; override;
...
{$ENDIF}
  ...
  dxThemeConsts, cxEditUtils, dxOffice11, dxThemeManager, dxCoreGraphics,
  dxUxTheme, cxDrawTextUtils, cxGeometry, dxCustomFluentDesignForm, dxHash;

const
  dxThisUnitName = 'cxGroupBox';
...
function TcxGroupBoxViewInfo.CalculateBorderColor(
  AStyle: TcxContainerStyle; AEnabled, AIsDesigning: Boolean; AContainer: TcxContainer = nil): TColor;
begin
  if TcxCustomEditStyle(AStyle).BorderStyle in [ebsUltraFlat, ebsOffice11] then
  begin
    if AEnabled then
      Result := GetEditBorderHighlightColor(TcxCustomEditStyle(AStyle).BorderStyle = ebsOffice11)
    else
      Result := clBtnShadow;
  end
  else
    Result := TcxCustomEditStyle(AStyle).BorderColor;
end;
    ...
    begin
      ...
      begin
        if HasNonClientArea then
          Painter.DrawGroupBoxBackground(ACanvas, Bounds, ACaptionRect);
        Exit;
      end;
      AdjustRectForBordersNone(ACaptionRect, ACaptionPosition);
    end;
    Painter.DrawGroupBoxScaledCaption(ACanvas, ACaptionRect, GetCaptionContentRect, ACaptionPosition, ScaleFactor);
  end;
...
begin
  ...
  try
    ...
    if (Edit.Caption = '') or Edit.DoCustomDrawCaption(ACanvas, CaptionRect, Painter) or not RectVisible(ACanvas.Handle, CaptionRect) then
      Exit;
...
begin
  ...
  AHash := GetHash(AFlags);

  if TcxCustomGroupBox(Edit).FTextRect.Hash <> AHash then
  begin
    Result := ARect;
    cxDrawText(ACanvas.Handle, Edit.Caption, Result, AFlags or DT_CALCRECT);
    TcxCustomGroupBox(Edit).FTextRect.Hash := AHash;
    TcxCustomGroupBox(Edit).FTextRect.Rect := Result;
  end
  else
    Result := TcxCustomGroupBox(Edit).FTextRect.Rect;
end;
...
begin
  ...
  if (not IsRectEmpty(FocusRect)) and Painter.SupportsNativeFocusRect then
...
begin
  ...
  cxDrawText(ACanvas, Edit.Caption, TextRect, AFlags, clDefault, ARotationAngleMap[CaptionPosition = cxgpLeft]);
end;
...
begin
  ...
  if not IsPanelStyle then
    case CaptionPosition of
      cxgpTop:
        Result.Top := TdxTextMeasurer.TextLineHeight(ACanvas.Handle) div 2;
      cxgpBottom:
        Dec(Result.Bottom, TdxTextMeasurer.TextLineHeight(ACanvas.Handle) div 2);
      cxgpLeft:
        Result.Left := TdxTextMeasurer.TextLineHeight(ACanvas.Handle) div 2;
      cxgpRight:
        Dec(Result.Right, TdxTextMeasurer.TextLineHeight(ACanvas.Handle) div 2);
    end;
end;
...
begin
  ...
  try
    if Edit.IsTransparent then
      ...
      if Edit.IsNativeBackground and IsThemeBackgroundPartiallyTransparent(OpenTheme(totButton), BP_GROUPBOX, cxNativeState[Enabled]) then
      begin
...
begin
  if NativeStyle or (IsPanelStyle and Assigned(Edit.LookAndFeelPainter) and
    (Edit.LookAndFeelPainter.LookAndFeelStyle = lfsOffice11)) then
...
begin
  ...
  if not RectVisible(ACanvas.Handle, AContentRect) or Edit.DoCustomDrawContentBackground(ACanvas, AContentRect, Painter) then
    Exit;

  if IsPanelStyle then
  begin
    if Edit.IsTransparentBackground then
      DrawParentBackground(ACanvas, AContentRect)
    else
      Painter.DrawPanelContent(ACanvas, AContentRect);
    if BorderStyle <> ebsNone then
      Painter.DrawPanelScaledBorders(ACanvas, AContentRect, ScaleFactor);
  end
  else
    if BorderStyle = ebsNone then
    ...
    begin
      ...
      if Painter.IsGroupBoxTransparent(ACaptionPos) then
        DrawParentBackground(ACanvas, ControlRect);
      ...
      Painter.DrawGroupBoxScaledContent(ACanvas, AContentRect, ACaptionPos, ScaleFactor, Edges);
    end;
end;
...
begin
  ...
  begin
    AHeaderSideBorderOffset := TdxTextMeasurer.TextLineHeight(Edit.FCaptionFont) div 2 - 1 + cxEditMaxBorderWidth;
    case CaptionPosition of
      cxgpTop:
...
function TcxGroupBoxViewData.GetClientExtent(
  ...
  begin
    if Style.LookAndFeel.SkinPainter = nil then
      Result := TdxTextMeasurer.TextLineHeight(Edit.FCaptionFont)
    else
    begin
...
function TcxGroupBoxViewData.IgnorePainterBorderSettings: Boolean;
begin
  Result := True;
end;
...
function TcxGroupBoxViewData.CalculateReadOnly: Boolean;
begin
...
begin
  Result := ViewInfo.GetGlyphRect(EditViewInfo.GetGlyphSize, EditViewInfo.ItemAlignment, False);
end;
...
begin
  ...
  AGlyphRect := AButtonViewInfo.GetGlyphRect(GetGlyphSize, ItemAlignment, IsDBEditPaintCopyDrawing);
...
begin
  ...
  ADefaultButtonHeight := Max(ACanvas.TextHeight('Zg'), GetButtonSize.cy) + ScaleFactor.Apply(ASizeCorrection.cy);
...
procedure TcxButtonGroupViewData.CalculateButtonPositions(ACanvas: TcxCanvas; AViewInfo: TcxCustomEditViewInfo);
var
...
begin
  ...
  if (TcxGroupBoxViewInfo(AViewInfo).ItemAlignment = taRightJustify) xor UseRightToLeftAlignment then
  begin
    AEditMetrics.ColumnOffset := Max(-AEditMetrics.ClientWidthCorrection, AEditMetrics.ColumnOffset);
    AButtonWidth := (cxRectWidth(AViewInfo.Bounds) - AClientExtent.Left - AClientExtent.Right +
      AEditMetrics.ClientWidthCorrection * 2 - AEditMetrics.ColumnOffset * (AColumnsCount - 1)) div AColumnsCount;
  end
  else
    AButtonWidth := (cxRectWidth(AViewInfo.Bounds) - AClientExtent.Left - AClientExtent.Right +
  ...
  if IsInplace and (GetEditSize(ACanvas, Null, cxDefaultEditSizeProperties).cy > AClientHeight) then
  begin
    ATopOffset := AViewInfo.Bounds.Top + AClientExtent.Top;
    AButtonHeight := Max(ACanvas.TextHeight('Zg'), GetButtonSize.cy) + ScaleFactor.Apply(GetEditContentSizeCorrection.cy);
  end
  else
  begin
...
procedure TcxButtonGroupViewData.CalculateButtonViewInfos(AViewInfo: TcxCustomEditViewInfo);
  ...
  const
    ButtonInplaceStyleMap: array[TcxLookAndFeelKind] of TcxEditButtonStyle = (btsFlat, bts3D, btsUltraFlat, btsOffice11);
    ButtonStyleMap: array [TcxEditBorderStyle] of TcxEditButtonStyle = (
      bts3D, btsFlat, btsFlat, btsFlat, bts3D, btsUltraFlat, btsOffice11
    );
  begin
    if IsInplace then
      Result := ButtonInplaceStyleMap[Style.LookAndFeel.Kind]
    else
      case Style.BorderStyle of
        ...
        Result := ButtonStyleMap[AViewInfo.BorderStyle];
      end;
  end;
...
begin
  ...
  begin
    AButtonViewInfo := TcxGroupBoxButtonViewInfo(AViewInfo.ButtonsInfo[I]);
    AButtonViewInfo.HasBackground := AViewInfo.HasBackground;
    AButtonViewInfo.Data.Style := AButtonStyle;
    AButtonViewInfo.Caption := Properties.FItems[I].Caption;
    AButtonViewInfo.Enabled := Properties.FItems[I].Enabled;
    AButtonViewInfo.TextColor := AViewInfo.TextColor;
    if IsInplace and not AButtonViewInfo.Enabled then
      AButtonViewInfo.TextColor := TcxContainerStyleAccess(Style).DefaultDisabledTextColor;
    AButtonViewInfo.Column := I div AButtonsPerColumn;
    AButtonViewInfo.Row := I mod AButtonsPerColumn;
  end;
end;
...
procedure TcxButtonGroupViewData.GetEditMetrics(AAutoHeight: Boolean; ACanvas: TcxCanvas; out AMetrics: TcxEditMetrics);
begin
  AMetrics.ClientLeftBoundCorrection := ScaleFactor.Apply(6 - Integer(IsInplace) * 5);
  AMetrics.ClientWidthCorrection := -AMetrics.ClientLeftBoundCorrection;
//  AMetrics.ColumnOffset := ScaleFactor.Apply(6);
//  AMetrics.ColumnOffset := ScaleFactor.Apply(0);
...
function TcxButtonGroupViewData.GetBiDiModeDependentAlignment(AAlignment: TLeftRight): TLeftRight;
begin
  Result := AAlignment;
  if UseRightToLeftAlignment then
...
function TcxButtonGroupViewData.GetButtonSize: TSize;
begin
  Result := Style.LookAndFeel.Painter.ScaledCheckButtonSize(ScaleFactor);
end;
...
class function TcxCustomGroupBoxProperties.GetStyleClass: TcxCustomEditStyleClass;
begin
  Result := TcxGroupBoxStyle;
end;
...
procedure TcxCustomButtonGroupProperties.SetItemAlignment(Value: TLeftRight);
begin
...
begin
  if HandleAllocated and not (IsScaleChanging or FContentExtentsUpdating) then
  begin
    FContentExtentsUpdating := True;
    try
      if IsNonClientAreaSupported then
      begin
        dxRecalculateNonClientPart(Handle);
        cxRedrawWindow(Handle, GetControlRect(Self), True, True);
      end;
      Realign;
    finally
      FContentExtentsUpdating := False;
    end;
  end;
end;
...
var
  APrevInternalWindowRegionSetting: Boolean;
begin
  if IsNonClientAreaSupported then
  begin
    APrevInternalWindowRegionSetting := FInternalWindowRegionSetting;
    try
      FInternalWindowRegionSetting := True;
      UpdateContentExtents;
    finally
      FInternalWindowRegionSetting := APrevInternalWindowRegionSetting;
    end;
  end;
end;
...
procedure TcxCustomGroupBox.WMNCHitTest(var Message: TWMNCHitTest);
begin
...
begin
  ...
  if IsLoading and not ViewInfo.FCalculated then
    CalculateViewInfo(cxInvalidPoint, False);
  UpdateContentExtents;
end;
...
procedure TcxCustomGroupBox.CreateWindowHandle(const Params: TCreateParams);
begin
  inherited;
...
function TcxCustomGroupBox.FluentDesignContainerControlCanBeOpaque: Boolean;
begin
  Result := True;
end;
...
function TcxCustomGroupBox.GetBackgroundStyle: TcxControlBackgroundStyle;
begin
...
function TcxCustomGroupBox.NeedAccessibilityHelper: Boolean;
begin
  Result := False;
end;
...
procedure TcxCustomGroupBox.ShortRefreshContainerOnWindowPosChanging;
begin
  inherited ShortRefreshContainerOnWindowPosChanging;
  if HasNonClientArea and not FInternalWindowRegionSetting then
...
var
  ACaptionIndents: TRect;
  ACaptionPos: TcxGroupBoxCaptionPosition;
  ACaptionRect: TRect;
  ASavedOrg: TPoint;
begin
  ACaptionIndents := ViewInfo.CaptionRectIndent;
  if UseRightToLeftAlignment then
    ACaptionIndents := TdxRightToLeftLayoutConverter.ConvertOffsets(ACaptionIndents);

  ACaptionRect := ViewInfo.CaptionRect;
  ACaptionPos := ViewInfo.GetBiDiModeCaptionPosition;
  case ACaptionPos of
    cxgpLeft:
      ACaptionRect.Right := ACaptionRect.Left + ACaptionIndents.Left;
    cxgpRight:
      ACaptionRect.Left := ACaptionRect.Right - ACaptionIndents.Right;
    cxgpTop:
      ACaptionRect.Bottom := ACaptionRect.Top + ACaptionIndents.Top;
    cxgpBottom:
      ACaptionRect.Top := ACaptionRect.Bottom - ACaptionIndents.Bottom;
  end;

  ASavedOrg := ACanvas.MoveWindowOrg(ACaptionIndents.TopLeft);
  try
    ACanvas.IntersectClipRect(ACaptionRect);
    if ViewInfo.Painter.IsGroupBoxTransparent(ACaptionPos) then
      ViewInfo.DrawParentBackground(ACanvas, ACaptionRect);
...
procedure TcxCustomGroupBox.ResetCache;
begin
  FTextRect.Hash := 0;
end;
...
function TcxCustomButtonGroup.NeedUpdateWindowRegion: Boolean;
begin
  Result := True;
end;
...
{B38193
  ...
  begin
    AButton := TWinControlAccess(FButtons[I]);
    AButton.Color := ViewInfo.BackgroundColor;
  end;
end;
...
function TcxGroupBoxStyle.UsePainterBorderSettings: Boolean;
begin
  Result := False;
end;
