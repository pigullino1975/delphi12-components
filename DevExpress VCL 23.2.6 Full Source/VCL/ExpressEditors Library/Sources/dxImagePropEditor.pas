...
const
  dxThisUnitName = 'dxImagePropEditor';
...
begin
  if ((Key < '0') or (Key > '9')) and (Key <> dxVKBack) and ((Key <> '-') or ((Edit2.Text <> '') and (Edit2.Text <> Edit2.SelText))) then
  begin
    Key := #0;
    MessageBeep(0);
  end;
...
var
  ...
  St: string;
begin
  if (ListBox.Items.Count > 0) and (Edit1.Text <> ListBox.Items[ListBox.ItemIndex]) then
  begin
    St := Edit1.Text;
...
procedure TfrmdxImagePropEditor.ListBoxDragDrop(Sender, Source: TObject; X, Y: Integer);
Var
 p : TPoint;
 Item, Item1 : Integer;
 flag : Boolean;
...
begin
  if (ListBox.ItemIndex > -1) and (ListBox.Items.Count > 0) and (Edit3.Text <> ListBox.Values[ListBox.ItemIndex]) then
    ListBox.Values[ListBox.ItemIndex] := Edit3.Text;
end;

procedure TfrmdxImagePropEditor.SpinImageChange(Sender: TObject; ItemIndex: Integer);
begin
  if (ListBox.ItemIndex > -1) and (ListBox.Items.Count > 0) then
  begin
