...
uses
  ...
  cxEdit, cxExtEditConsts, cxExtEditUtils, cxLookAndFeelPainters, cxGeometry, dxThemeConsts, cxAccessibility,
...
type
  ...
  protected
    ...
    function TryScaleThumbAndTrack(AViewInfo: TcxCustomTrackBarViewInfo): Boolean; override;
  ...
  { TdxRangeTrackBarAccessibilityHelper }

  TdxRangeTrackBarAccessibilityHelper = class(TdxTrackBarAccessibilityHelper) // for internal use
  strict private
    function GetViewInfo: TdxCustomRangeTrackBarViewInfo;
  protected
...
const
  dxThisUnitName = 'dxRangeTrackBar';
...
begin
  Result := Integer(ARange and $FFFFFFFF);
end;

function GetMaxRange(ARange: Int64): Integer;
begin
  Result := Integer(ARange shr 32);
end;

{ TdxRangeTrackBarAccessibilityHelper }

function TdxRangeTrackBarAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  if AChildID >= FirstSimpleChildIndex then
    case AChildID - FirstSimpleChildIndex of
      0:
        Result := ViewInfo.LeftThumb.Bounds;
      1:
        Result := ViewInfo.RightThumb.Bounds;
      2:
        begin
          Result := ViewInfo.TrackRect;
          Result.Right := ViewInfo.LeftThumb.Bounds.Left;
        end;
      3:
        begin
          Result := ViewInfo.TrackRect;
          Result.Left := ViewInfo.LeftThumb.Bounds.Right;
          Result.Right := ViewInfo.RightThumb.Bounds.Left;
        end;
      4:
        begin
          Result := ViewInfo.TrackRect;
          Result.Left := ViewInfo.RightThumb.Bounds.Right;
        end;
      5: Result := ViewInfo.LeftButtonRect;
      6: Result := ViewInfo.RightButtonRect;
    else
      Result := cxNullRect;
    end
  else
...
function TdxRangeTrackBarAccessibilityHelper.GetChildCount: Integer;
begin
  Result := inherited + 5;
...
function TdxRangeTrackBarAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  if AChildID >= FirstSimpleChildIndex then
  begin
    if AChildID - FirstSimpleChildIndex > 1 then
...
function TdxRangeTrackBarAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := inherited;
  if AChildID = 0 then
...
function TdxRangeTrackBarAccessibilityHelper.GetValue(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := Format('%d - %d', [TdxCustomRangeTrackBar(TrackBar).Range.Min, TdxCustomRangeTrackBar(TrackBar).Range.Max]);
end;

function TdxRangeTrackBarAccessibilityHelper.IsSupportedPattern(APatternID: Integer; out AProvider: IInterface;
...
function TdxRangeTrackBarAccessibilityHelper.GetViewInfo: TdxCustomRangeTrackBarViewInfo;
begin
  Result := TdxCustomRangeTrackBarViewInfo(inherited ViewInfo);
end;
...
function TdxCustomRangeTrackBar.GetAccessibilityHelperClass: TdxEditAccessibilityHelperClass;
begin
  Result := TdxRangeTrackBarAccessibilityHelper;
end;
...
procedure TdxCustomRangeTrackBar.SetProperties(const Value: TdxCustomRangeTrackBarProperties);
begin
...
procedure TdxCustomRangeTrackBarProperties.CalculateRangeByEditValue(AValue: Variant;
  AEditValueFormat: TdxRangeTrackBarEditValueFormat; ADelimiter: Char; out AMin, AMax: Integer);
var
  ARange: Int64;
begin
...
procedure TdxCustomRangeTrackBarProperties.CalculateRangeByEditValue(AValue: Variant; out AMin, AMax: Integer);
begin
...
procedure TdxCustomRangeTrackBarProperties.DoAssign(AProperties: TcxCustomEditProperties);
var
...
procedure TdxCustomRangeTrackBarProperties.DoDrawRangeMinThumb(Sender: TObject; ACanvas: TcxCanvas; const ARect: TRect);
begin
  if Assigned(OnDrawRangeMinThumb) then
    OnDrawRangeMinThumb(Self, ACanvas, ARect);
end;

procedure TdxCustomRangeTrackBarProperties.DoDrawRangeMaxThumb(Sender: TObject; ACanvas: TcxCanvas; const ARect: TRect);
begin
...
procedure TdxCustomRangeTrackBarProperties.PrepareDisplayValue(const AEditValue: TcxEditValue;
...
procedure TdxCustomRangeTrackBarProperties.SetEditValueFormat(const Value: TdxRangeTrackBarEditValueFormat);
begin
...
procedure TdxCustomRangeTrackBarProperties.SetShowSelection(const Value: Boolean);
begin
...
procedure TdxCustomRangeTrackBarProperties.SetOnDrawMinThumb(const Value: TcxDrawThumbEvent);
begin
  OnDrawThumb := Value;
end;
...
procedure TdxRangeTrackBar.SetProperties(const Value: TdxRangeTrackBarProperties);
begin
  inherited Properties := Value;
end;
...
procedure TdxCustomRangeTrackBarViewInfo.DrawCustomThumb(ACanvas: TcxCanvas);
begin
...
procedure TdxCustomRangeTrackBarViewInfo.DrawLeftThumb(ACanvas: TcxCanvas; const R: TRect; AState: Integer);
begin
...
procedure TdxCustomRangeTrackBarViewInfo.DrawRightThumb(ACanvas: TcxCanvas; const R: TRect; AState: Integer);
begin
...
function TdxCustomRangeTrackBarViewInfo.IsPtToTheLeftFromThumb(AThumb: TcxTrackBarThumbViewInfo; X, Y: Integer): Boolean;
begin
...
function TdxCustomRangeTrackBarViewInfo.IsPtToTheRightFromThumb(AThumb: TcxTrackBarThumbViewInfo; X, Y: Integer): Boolean;
begin
  if AThumb = FRightThumb then
...
function TdxCustomRangeTrackBarViewInfo.IsTickText(ATickValue: Integer): Boolean;
begin
...
procedure TdxCustomRangeTrackBarViewInfo.UpdateValue(AValue: TcxEditValue);
begin
...
procedure TdxRangeTrackBarLeftThumbFadingHelper.DrawThumb(ACanvas: TcxCanvas; const R: TRect; AState: Integer);
begin
...
procedure TdxRangeTrackBarRightThumbFadingHelper.DrawThumb(ACanvas: TcxCanvas; const R: TRect; AState: Integer);
begin
...
procedure TdxRangeTrackBarController.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  ANewPosition: Integer;
...
function TdxRangeTrackBarController.GetPosition(AThumb: TcxTrackBarThumbViewInfo): Integer;
begin
...
procedure TdxRangeTrackBarController.SetPosition(AValue: Integer; AThumb: TcxTrackBarThumbViewInfo);
begin
  if AThumb = ViewInfo.GetMinThumb then
...
procedure TdxRangeTrackBarViewData.CalculateTrackBarViewInfoProperties(AViewInfo: TcxCustomEditViewInfo);
var
...
begin
  ...
  if ARangeTrackBarViewInfo.UseSkins then
  begin
    ASize := ARangeTrackBarViewInfo.Painter.RangeTrackBarScaledLeftThumbSize(
      ARangeTrackBarViewInfo.Orientation = tboHorizontal, TrackBarTicksToTicksAlign[ARangeTrackBarViewInfo.TickMarks], ScaleFactor);
    ARangeTrackBarViewInfo.FLeftThumbWidth := ASize.Width;
    ASize := ARangeTrackBarViewInfo.Painter.RangeTrackBarScaledRightThumbSize(
      ARangeTrackBarViewInfo.Orientation = tboHorizontal, TrackBarTicksToTicksAlign[ARangeTrackBarViewInfo.TickMarks], ScaleFactor);
    ARangeTrackBarViewInfo.FRightThumbWidth := ASize.Width;
    ARangeTrackBarViewInfo.ThumbSize := ARangeTrackBarViewInfo.FLeftThumbWidth + ARangeTrackBarViewInfo.FRightThumbWidth - 1;
    ARangeTrackBarViewInfo.ThumbLargeSize := ASize.Height;
  end
  else
  begin
    ARangeTrackBarViewInfo.FLeftThumbWidth := ThumbHalfSize(ARangeTrackBarViewInfo) + 1;
    ARangeTrackBarViewInfo.FRightThumbWidth := ARangeTrackBarViewInfo.FLeftThumbWidth;
  end;
end;

procedure TdxRangeTrackBarViewData.CalculateThumbRect(ACanvas: TcxCanvas; AViewInfo: TcxCustomTrackBarViewInfo);
var
  ARangeTrackBarViewInfo: TdxCustomRangeTrackBarViewInfo;
  ...
  var
    R: TRect;
  begin
    R.Top := ARangeTrackBarViewInfo.TrackRect.Top +
      (ARangeTrackBarViewInfo.TrackSize - ARangeTrackBarViewInfo.ThumbLargeSize) div 2;
    R.Bottom := R.Top + ARangeTrackBarViewInfo.ThumbLargeSize;

    R.Right := ARangeTrackBarViewInfo.GetCoordinate(ARangeTrackBarViewInfo.LeftThumb.Position) + 1;
    R.Left := R.Right - ARangeTrackBarViewInfo.FLeftThumbWidth;
    ARangeTrackBarViewInfo.LeftThumb.Bounds := ARangeTrackBarViewInfo.DrawingThumbRectToRealThumbRect(ACanvas, R);
    ARangeTrackBarViewInfo.LeftThumb.FullRect := R;

    ARangeTrackBarViewInfo.ThumbRect := ARangeTrackBarViewInfo.LeftThumb.FullRect;
    ARangeTrackBarViewInfo.VisibleThumbRect := ARangeTrackBarViewInfo.LeftThumb.Bounds;

    R.Left := ARangeTrackBarViewInfo.GetCoordinate(ARangeTrackBarViewInfo.RightThumb.Position);
    R.Right := R.Left + ARangeTrackBarViewInfo.FRightThumbWidth;
    ARangeTrackBarViewInfo.RightThumb.Bounds := ARangeTrackBarViewInfo.DrawingThumbRectToRealThumbRect(ACanvas, R);
    ARangeTrackBarViewInfo.RightThumb.FullRect := R;
  end;
...
procedure TdxRangeTrackBarViewData.UpdateSelection(AViewInfo: TcxCustomTrackBarViewInfo);
var
  ARangeTrackBarViewInfo: TdxCustomRangeTrackBarViewInfo;
begin
  ARangeTrackBarViewInfo := AViewInfo as TdxCustomRangeTrackBarViewInfo;
...
procedure TdxRangeTrackBarViewData.UpdateValue(AViewInfo: TcxCustomTrackBarViewInfo; AValue: TcxEditValue);
begin
...
function TdxRangeTrackBarViewData.TryScaleThumbAndTrack(AViewInfo: TcxCustomTrackBarViewInfo): Boolean;
var
  ARangeTrackBarViewInfo: TdxCustomRangeTrackBarViewInfo;
  AThumbSizeBefore: Integer;
begin
  AThumbSizeBefore := AViewInfo.ThumbSize;
  Result := inherited TryScaleThumbAndTrack(AViewInfo);
  if Result then
  begin
    ARangeTrackBarViewInfo := AViewInfo as TdxCustomRangeTrackBarViewInfo;
    Dec(ARangeTrackBarViewInfo.FLeftThumbWidth, (AThumbSizeBefore - AViewInfo.ThumbSize) div 2);
    ARangeTrackBarViewInfo.FRightThumbWidth := ARangeTrackBarViewInfo.FLeftThumbWidth;
  end;
end;
