...
uses
  ...
  dxFormattedText, cxDrawTextUtils;
...
type
  ...
  protected
    ...
    property HintStyleController: TcxCustomHintStyleController read BaseGetHintStyleController write BaseSetHintStyleController;
  ...
  protected
    ...
    FFormattedCaptionText: TdxFormattedText;
...
uses
  ...
  dxFormattedTextConverterBBCode, dxFormattedTextConverterRTF,
  dxCoreGraphics, cxEditUtils, cxExtEditUtils, dxDPIAwareUtils, dxCore;

const
  dxThisUnitName = 'cxHint';
...
begin
  ...
  FCaptionFont.PixelsPerInch := dxDefaultDPI;
  FIcon := TIcon.Create;
...
begin
  ...
  FFormattedCaptionText := TdxFormattedText.Create;
end;

destructor TcxHintViewInfo.Destroy;
begin
  FreeAndNil(FFormattedHintText);
  FreeAndNil(FFormattedCaptionText);
  inherited;
end;
...
begin
  ...
  ARegion := CreateRoundRectRgn(R.Left, R.Top, R.Right, R.Bottom, RoundRadius, RoundRadius);
...
procedure TcxHintViewInfo.Paint(ACanvas: TcxCanvas);
  ...
  begin
    ...
    begin
      ...
      AScaledIconHandle := CopyImage(Owner.Icon.Handle, IMAGE_ICON, IconSize, IconSize, LR_COPYFROMRESOURCE);
      try
        DrawIconEx(ACanvas.Handle, R.Left, R.Top, AScaledIconHandle, IconSize, IconSize, 0, 0, DI_NORMAL);
      finally
        DestroyIcon(AScaledIconHandle);
      end;
    end;
  end;

  procedure InternalDrawHintText(ABounds: TRect; const AText: string; AFormattedText: TdxFormattedText; AFont: TFont; AColor: TColor; AFlags: Cardinal);
  begin
    ACanvas.Font.Assign(AFont);
    if AColor <> clDefault then
      ACanvas.Font.Color := AColor;

    if AFormattedText.HasFormatting then
      AFormattedText.Draw(ACanvas.Canvas, ABounds.TopLeft, False)
    else
    begin
  ...
  begin
    ...
    begin
      ...
      if Owner.InternalUseRightToLeftAlignment then
        R := TdxRightToLeftLayoutConverter.ConvertRect(R, FHintWindowRect);
      InternalDrawHintText(R, Caption, FFormattedCaptionText, CaptionFont, FPainter.ScreenTipGetTitleTextColor, DT_WORDBREAK or DT_EDITCONTROL or DT_NOPREFIX or DT_VCENTER);
    end;

    InternalDrawHintText(FHintBounds, Hint, FFormattedHintText, HintFont, FPainter.ScreenTipGetDescriptionTextColor, DT_WORDBREAK or DT_EDITCONTROL or DT_NOPREFIX);
...
procedure TcxHintViewInfo.CalculateTextsBounds;
  ...
  function CalculateFormattedText(const AText: string; AFormattedText: TdxFormattedText; AFont: TFont; ATextColor: TColor): Boolean;
  var
    PrevColor: TColor;
  begin
    PrevColor := AFont.Color;
    try
      AFont.Color := ATextColor;
      AFormattedText.Import(AText, AFont);
      Result := AFormattedText.HasFormatting;
      if Result then
        AFormattedText.CalculateLayout(FOwner.Canvas, AFont, Rect(0, 0, GetMaxCaptionWidth, 1),
          CXTO_NOCLIP or CXTO_WORDBREAK or CXTO_CALCRECT or CXTO_CHARBREAK, ScaleFactor);
    finally
      AFont.Color := PrevColor;
    end;
  end;
...
begin
  ...
  begin
    if CalculateFormattedText(Caption, FFormattedCaptionText, CaptionFont, FPainter.ScreenTipGetTitleTextColor) then
      FCaptionBounds := TRect.CreateSize(FFormattedCaptionText.TextSize)
    else
    begin
  ...
  begin
    if CalculateFormattedText(Hint, FFormattedHintText, HintFont, FPainter.ScreenTipGetDescriptionTextColor) then
      FHintBounds := TRect.CreateSize(FFormattedHintText.TextSize)
    else
...
begin
  cxGetTextRect(ARect, AText, AFont, DT_WORDBREAK or DT_EDITCONTROL or DT_NOPREFIX);
end;
...
begin
  if Owner.Rounded then
    Result :=  ScaleFactor.Apply(Owner.RoundRadius)
  else
    Result := 0;
end;
...
procedure TcxHintWindow.LoadHintStyleData(const AHintData: IcxHint);
  ...
  procedure LoadFont(ATargetFont, AHintDataFont, ADefaultFont: TFont);
  begin
    if AHintDataFont <> nil then
      ATargetFont.Assign(AHintDataFont)
    else
      ATargetFont.Assign(ADefaultFont);

    ATargetFont.Height := ScaleFactor.Apply(ATargetFont.Height, ATargetFont.PixelsPerInch, dxDefaultDPI);
  end;
...
begin
  ...
  try
    LoadFont(Font, AHintData.GetHintFont, ADefaultFont);
    LoadFont(FCaptionFont, AHintData.GetHintCaptionFont, ADefaultFont);
  finally
    ADefaultFont.Free;
  end;
end;
...
var
  R: TRect;
  ASaveAbsolutePosition: Boolean;
begin
  inherited ChangeScale(M, D);
  ScaleFactor.Change(M, D);
  ASaveAbsolutePosition := AbsolutePosition;
  try
    AbsolutePosition := True;
    FCaptionFont.Height := MulDiv(FCaptionFont.Height, M, D);
    R := CalcHintRect(Screen.Width, Hint, nil);
    OffsetRect(R, Left, Top);
  finally
    AbsolutePosition := ASaveAbsolutePosition;
  end;
  ActivateHint(R, Hint);
end;
...
begin
  ...
  if StandardHint or (CallOutPosition = cxbpNone) then
  begin
