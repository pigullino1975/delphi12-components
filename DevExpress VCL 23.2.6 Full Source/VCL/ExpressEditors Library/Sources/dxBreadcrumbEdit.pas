...
uses
  System.UITypes,
  Types, Windows, SysUtils, Classes, Messages, Graphics, Forms, Controls,
  ImgList, Menus, Generics.Defaults, Generics.Collections,
  ...
  cxDropDownEdit, cxTextEdit, dxAutoCompleteWindow, cxGeometry, cxAccessibility;
...
type
  ...
  ['{58BB7C9A-21D2-4E8C-B95C-E20BC3768E28}']
    ...
    procedure InitPathEdit(AEdit: TcxCustomEdit);
  ...
  { TdxBreadcrumbEditViewItemAccessibilityHelper }

  TdxBreadcrumbEditViewItem = class;

  TdxBreadcrumbEditViewItemAccessibilityHelper = class(TcxAccessibilityHelper) // for internal use
  strict private
    function GetViewItem: TdxBreadcrumbEditViewItem;
  protected
    FLocalId: Integer;
    ...
    property ViewItem: TdxBreadcrumbEditViewItem read GetViewItem;
  public
  ...
  TdxBreadcrumbEditViewItemAccessibilityHelperClass = class of TdxBreadcrumbEditViewItemAccessibilityHelper;
  ...
  protected
    ...
    function GetAccessibilityHelperClass: TdxBreadcrumbEditViewItemAccessibilityHelperClass; virtual;
    ...
    function IntersectsWithProgressArea(out AIntersection: TRect): Boolean; virtual;
    // IdxFadingObject
  ...
  public
    ...
    property Controller: TdxBreadcrumbEditController read GetController; // for internal use
  ...
  { TdxBreadcrumbEditNodeAccessibilityHelper }

  TdxBreadcrumbEditNodeViewItem = class;

  TdxBreadcrumbEditNodeAccessibilityHelper = class(TdxBreadcrumbEditViewItemAccessibilityHelper) // for internal use
  strict private
    function GetNodeViewItem: TdxBreadcrumbEditNodeViewItem;
  protected
    ...
    property NodeViewItem: TdxBreadcrumbEditNodeViewItem read GetNodeViewItem;
  public
    function GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect; override;
  end;
  ...
  protected
    ...
    function CalculateAutoHeight: Integer; override;
    function GetAccessibilityHelperClass: TdxBreadcrumbEditViewItemAccessibilityHelperClass; override;
    ...
    procedure DrawBackground(ACanvas: TcxCanvas); override;
    procedure DrawContent(ACanvas: TcxCanvas); override;
    procedure DrawDelimiterGlyph(ACanvas: TcxCanvas; AColor: TColor = clDefault); virtual;
    procedure DrawTexT(ACanvas: TcxCanvas; const ATextRect: TRect; ATextColor: TColor);
  public
  ...
  protected
    procedure DrawDelimiterGlyph(ACanvas: TcxCanvas; AColor: TColor = clDefault); override;
  public
  ...
  strict private
    ...
    FRecentPathsAddToSuggestions: Boolean;
  ...
  protected
    property RecentPathsAddToSuggestions: Boolean read FRecentPathsAddToSuggestions write FRecentPathsAddToSuggestions;
  public
  ...
  public
    Point: TPoint;
    ViewItem: TdxBreadcrumbEditViewItem; // for internal use
    ViewItemPart: Integer;
    procedure Reset; virtual;
  end;

  { TdxBreadcrumbEditPartCustomAccessibilityHelper }

  TdxBreadcrumbEditPartCustomAccessibilityHelper = class(TcxAccessibilityHelper) // for internal use
  strict private
    function GetViewInfo: TdxBreadcrumbEditPartCustomViewInfo;
  protected
    FLocalId: Integer;
    ...
    property ViewInfo: TdxBreadcrumbEditPartCustomViewInfo read GetViewInfo;
  public
    constructor Create(AOwnerObject: TObject); override;
  ...
  TdxBreadcrumbEditPartCustomAccessibilityHelperClass = class of TdxBreadcrumbEditPartCustomAccessibilityHelper;
  ...
  protected
    ...
    function GetAccessibilityHelperClass: TdxBreadcrumbEditPartCustomAccessibilityHelperClass; virtual;
  ...
  public
    ...
    //
    property Bounds: TRect read FBounds;
    property Control: IdxBreadcrumbEdit read GetControl; // for internal use
    property Owner: TdxBreadcrumbEditViewInfo read FOwner; // for internal use
    property Painter: TcxCustomLookAndFeelPainter read GetPainter;
    property Properties: TdxCustomBreadcrumbEditProperties read GetProperties;
    property ScaleFactor: TdxScaleFactor read GetScaleFactor;
    property UseRightToLeftAlignment: Boolean read GetUseRightToLeftAlignment;
    property ViewItems[Index: Integer]: TdxBreadcrumbEditViewItem read GetViewItem; // for internal use
  ...
  { TdxBreadcrumbPathEditorPartAccessibilityHelper }

  TdxBreadcrumbEditNodesAreaPartViewInfo = class;

  TdxBreadcrumbPathEditorPartAccessibilityHelper = class(TdxBreadcrumbEditPartCustomAccessibilityHelper) // for internal use
  strict private
    function GetViewInfo: TdxBreadcrumbEditNodesAreaPartViewInfo;
  protected
    function GetName(AChildID: TcxAccessibleSimpleChildElementID): string; override;

    property ViewInfo: TdxBreadcrumbEditNodesAreaPartViewInfo read GetViewInfo;
  public
    function GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect; override;
  end;

  { TdxBreadcrumbEditNodesAreaPartAccessibilityHelper }

  TdxBreadcrumbEditNodesAreaPartAccessibilityHelper = class(TdxBreadcrumbEditPartCustomAccessibilityHelper) // for internal use
  strict private
    function GetViewInfo: TdxBreadcrumbEditNodesAreaPartViewInfo;
  protected
    function ChildIsSimpleElement(AIndex: Integer): Boolean; override;
  ...
  private
    ...
    FIPathEditorPartAccessibilityHelper: IcxAccessibilityHelper;

    function GetDropDownButtonWidth: Integer;
    function GetFirstViewItem: TdxBreadcrumbEditNodeViewItem;
    function GetIPathEditorPartAccessibilityHelper: IcxAccessibilityHelper;
  ...
  protected
    ...
    function GetAccessibilityHelperClass: TdxBreadcrumbEditPartCustomAccessibilityHelperClass; override;
    ...
    property IPathEditorPartAccessibilityHelper: IcxAccessibilityHelper read GetIPathEditorPartAccessibilityHelper;
  public
    ...
    //
    property DropDownButton: TdxBreadcrumbEditViewItem read FDropDownButton; // for internal use
    property FirstViewItem: TdxBreadcrumbEditNodeViewItem read GetFirstViewItem; // for internal use
    property ImageRect: TRect read FImageRect;
    property PathEditorRect: TRect read GetPathEditorRect;
    property SelectedNodeViewItem: TdxBreadcrumbEditNodeViewItem read GetSelectedNodeViewItem; // for internal use
    property ViewItems[Index: Integer]: TdxBreadcrumbEditNodeViewItem read GetViewItem; // for internal use
  end;
  ...
  { TdxBreadcrumbEditAccessibilityHelper }

  TdxBreadcrumbEditAccessibilityHelper = class(TcxAccessibilityHelper) // for internal use
  strict private
  ...
  public
    ...
    property Bounds: TRect read FBounds;
    property ButtonsViewInfo: TdxBreadcrumbEditButtonsPartViewInfo read FButtonsViewInfo; // for internal use
    property ContentRect: TRect read GetContentRect;
    property Control: IdxBreadcrumbEdit read FControl; // for internal use
    property HitTestInfo: TdxBreadcrumbEditHitTestInfo read FHitTestInfo; // for internal use
    property NodesAreaViewInfo: TdxBreadcrumbEditNodesAreaPartViewInfo read FNodesAreaViewInfo; // for internal use
    property Painter: TcxCustomLookAndFeelPainter read GetPainter;
    property ProgressBarViewInfo: TdxBreadcrumbEditProgressBarPartViewInfo read FProgressBarViewInfo; // for internal use
    property Properties: TdxCustomBreadcrumbEditProperties read GetProperties;
  ...
  { TdxBreadcrumbEditListBoxAccessibilityHelper }

  TdxBreadcrumbEditListBoxAccessibilityHelper = class(TdxCustomListBoxAccessibilityHelper) // for internal use
  protected
    procedure DoDefaultAction(AChildID: TcxAccessibleSimpleChildElementID); override;
    function GetChildCount: Integer; override;
    function GetName(AChildID: TcxAccessibleSimpleChildElementID): string; override;
  ...
  protected
    ...
    function GetAccessibilityHelperClass: TdxCustomListBoxAccessibilityHelperClass; override;
    function GetHasSeparator(AItem: TdxCustomListBoxItem): Boolean; override;
    function GetItemsAreaRect: TRect; override;
    function IsDefaultItem(AItem: TdxCustomListBoxItem): Boolean; override;
  ...
  public
    ...
    //
    property ActiveProperties: TdxBreadcrumbEditPathInplaceEditorProperties read GetActiveProperties; // for internal use
    property AutoCompleteSuggestions: TStringList read FAutoCompleteSuggestions write SetAutoCompleteSuggestions;
    property LookupData: TdxBreadcrumbEditPathEditorLookupData read GetLookupData; // for internal use
    property Properties: TdxBreadcrumbEditPathInplaceEditorProperties read GetProperties; // for internal use
    //
  ...
  strict private
    ...
    procedure PathEditorInitPopupHandler(Sender: TObject);
  ...
  protected
    ...
    procedure CheckRecentPathsLoaded; virtual;
    function CreatePathEditor: TdxBreadcrumbEditPathEditor; virtual;
    procedure DoInitPathEdit(AEdit: TcxCustomEdit); virtual;
  ...
  public
    ...
    procedure DropDown;
    procedure OpenPathEditor; virtual;
    procedure ReleasePathEditor; virtual;
    procedure UpdatePathEditorBounds;
    //
    property Control: IdxBreadcrumbEdit read GetControl; // for internal use
    property Controller: TdxBreadcrumbEditController read FController; // for internal use
    property IsFocused: Boolean read GetIsFocused;
    property PathEditor: TdxBreadcrumbEditPathEditor read FPathEditor; // for internal use
    ...
    property ViewInfo: TdxBreadcrumbEditViewInfo read GetViewInfo; // for internal use
  end;
  ...
  protected
    ...
    function ShouldUpdateSelectedNode(ANode: TdxBreadcrumbEditNode): Boolean; virtual;
  ...
  public
    ...
    procedure SelectPathViaDropDownMenu(AItem: TdxBreadcrumbEditNode); virtual;
    //
    ...
    property HitTestInfo: TdxBreadcrumbEditHitTestInfo read GetHitTestInfo; // for internal use
    property IsFocused: Boolean read GetIsFocused;
    property PathEditingController: TdxBreadcrumbEditPathEditingController read FPathEditingController; // for internal use
    ...
    property ViewInfo: TdxBreadcrumbEditViewInfo read FViewInfo; // for internal use
  end;
  ...
  {$ENDIF}
    ...
    FOnPopulateRecentPaths: TNotifyEvent;
  ...
  {$ENDIF}
    ...
    procedure PopulateRecentPaths; virtual;
    procedure ProcessButtonClick(AButton: TdxBreadcrumbEditButton); virtual;
    procedure SelectionChanged; virtual;
    //
    ...
    property OnInitPathEdit: TNotifyEvent read FOnInitPathEdit write FOnInitPathEdit;
    ...
    property OnPopulateRecentPaths: TNotifyEvent read FOnPopulateRecentPaths write FOnPopulateRecentPaths;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
...
uses
  Math, StdCtrls, cxEditConsts, Variants, StrUtils, dxCoreGraphics, dxTypeHelpers;

const
  dxThisUnitName = 'dxBreadcrumbEdit';
...
constructor TdxBreadcrumbEditViewItemAccessibilityHelper.Create(AOwnerObject: TObject);
begin
  inherited;
  FLocalId := ViewItem.Owner.Owner.FAccessibleObjects.Count + 1;
  ViewItem.Owner.Owner.FAccessibleObjects.Add(Self);
end;

function TdxBreadcrumbEditViewItemAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  Result := ViewItem.Bounds;
  Result := cxRectSetOrigin(Result, ViewItem.Owner.Control.GetContainer.ClientToScreen(Result.TopLeft));
end;

procedure TdxBreadcrumbEditViewItemAccessibilityHelper.DoDefaultAction(AChildID: TcxAccessibleSimpleChildElementID);
begin
  ViewItem.Click;
end;

function TdxBreadcrumbEditViewItemAccessibilityHelper.GetDefaultActionDescription(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := 'Click';
end;

function TdxBreadcrumbEditViewItemAccessibilityHelper.GetLocalId(AChildID: TcxAccessibleSimpleChildElementID = 0): Integer;
begin
  Result := FLocalId;
end;

function TdxBreadcrumbEditViewItemAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := ViewItem.HintText;
end;

function TdxBreadcrumbEditViewItemAccessibilityHelper.GetParent: TcxAccessibilityHelper;
begin
  Result := TdxBreadcrumbEditPartCustomViewInfo(ViewItem.Owner).IAccessibilityHelper.GetHelper;
end;

function TdxBreadcrumbEditViewItemAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxROLE_SYSTEM_PUSHBUTTON;
end;

function TdxBreadcrumbEditViewItemAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxSTATE_SYSTEM_NORMAL;
  if ViewItem.State = dxbcbsHot then
    Result := cxSTATE_SYSTEM_HOTTRACKED;
  if (AChildID = 0) and (ViewItem.State = dxbcbsFocused) then
    Result := Result or cxSTATE_SYSTEM_FOCUSED;
end;

function TdxBreadcrumbEditViewItemAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := [aopFocus, aopLocation, aopDefaultAction];
end;

function TdxBreadcrumbEditViewItemAccessibilityHelper.IsExtended: Boolean;
begin
  Result := True;
end;

procedure TdxBreadcrumbEditViewItemAccessibilityHelper.OwnerObjectDestroyed;
begin
  if (ViewItem.Owner <> nil) and (ViewItem.Owner.Owner.FAccessibleObjects <> nil) then
    ViewItem.Owner.Owner.FAccessibleObjects[FLocalId - 1] := nil;
  inherited;
end;

function TdxBreadcrumbEditViewItemAccessibilityHelper.GetViewItem: TdxBreadcrumbEditViewItem;
begin
  Result := TdxBreadcrumbEditViewItem(OwnerObject);
end;
...
function TdxBreadcrumbEditViewItem.GetAccessibilityHelperClass: TdxBreadcrumbEditViewItemAccessibilityHelperClass;
begin
  Result := TdxBreadcrumbEditViewItemAccessibilityHelper;
end;
...
function TdxBreadcrumbEditViewItem.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin
...
function TdxBreadcrumbEditViewItem.IntersectsWithProgressArea(out AIntersection: TRect): Boolean;
begin
  Result := cxRectIntersect(AIntersection, Bounds, Owner.Owner.ProgressBarViewInfo.ChunkRect);
end;
...
begin
  Result := (Controller.FocusedViewItem = Self) and (Controller.PressedViewItem = nil) and not FIsDropTarget;
end;

function TdxBreadcrumbEditViewItem.GetIsPressed: Boolean;
begin
  Result := (Controller.FocusedViewItem = Self) and (Controller.PressedViewItem = Self) and not FIsDropTarget;
end;
...
begin
  TdxImageDrawer.DrawUncachedImage(ACanvas.Handle,
...
function TdxBreadcrumbEditNodeAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  if AChildID = 0 then
    Result := NodeViewItem.Bounds
  else
    Result := NodeViewItem.DelimiterRect;
  Result := cxRectSetOrigin(Result, NodeViewItem.Owner.Control.GetContainer.ClientToScreen(Result.TopLeft));
end;

function TdxBreadcrumbEditNodeAccessibilityHelper.ChildIsSimpleElement(AIndex: Integer): Boolean;
begin
  Result := True;
end;

procedure TdxBreadcrumbEditNodeAccessibilityHelper.DoDefaultAction(AChildID: TcxAccessibleSimpleChildElementID);
begin
  if AChildID = 0 then
    ViewItem.Click(nvipControl)
  else
    NodeViewItem.Controller.ShowNodeDropDownMenu(NodeViewItem);
end;

function TdxBreadcrumbEditNodeAccessibilityHelper.GetDefaultActionDescription(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := 'Open';
end;

function TdxBreadcrumbEditNodeAccessibilityHelper.GetChildCount: Integer;
begin
  Result := 1;
end;

function TdxBreadcrumbEditNodeAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  if AChildID = 0 then
    Result := NodeViewItem.Node.ActualDisplayName
  else
    Result := 'Open';
end;

function TdxBreadcrumbEditNodeAccessibilityHelper.GetParent: TcxAccessibilityHelper;
begin
  Result := TdxBreadcrumbEditNodesAreaPartViewInfo(NodeViewItem.Owner).IPathEditorPartAccessibilityHelper.GetHelper;
end;

function TdxBreadcrumbEditNodeAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  if AChildID = 0 then
...
function TdxBreadcrumbEditNodeAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := inherited;
  if (AChildID > 0) and cxRectIsEmpty(NodeViewItem.ButtonRect) and (NodeViewItem.DelimiterState = dxbcbsFocused) then
    Result := Result or cxSTATE_SYSTEM_FOCUSED;
end;

function TdxBreadcrumbEditNodeAccessibilityHelper.GetNodeViewItem: TdxBreadcrumbEditNodeViewItem;
begin
  Result := TdxBreadcrumbEditNodeViewItem(inherited ViewItem);
end;
...
function TdxBreadcrumbEditNodeViewItem.GetAccessibilityHelperClass: TdxBreadcrumbEditViewItemAccessibilityHelperClass;
begin
  Result := TdxBreadcrumbEditNodeAccessibilityHelper;
end;
...
var
  AHighligtedArea: TRect;
  AHighligtedAreaTextColor: TColor;  
  ATextRect: TRect;
begin
  ATextRect := cxRectContent(ButtonRect, Painter.BreadcrumbEditScaledNodeTextOffsets(ScaleFactor));

  AHighligtedAreaTextColor := Painter.BreadcrumbEditProgressChunkTextColor;
  if cxColorIsValid(AHighligtedAreaTextColor) and (State = dxbcbsNormal) and IntersectsWithProgressArea(AHighligtedArea) then
  begin
    ACanvas.SaveClipRegion;
    try
      ACanvas.IntersectClipRect(AHighligtedArea);
      DrawDelimiterGlyph(ACanvas, AHighligtedAreaTextColor);
      DrawTexT(ACanvas, ATextRect, AHighligtedAreaTextColor);
    finally
    ...
    ACanvas.SaveClipRegion;
    try
      ACanvas.ExcludeClipRect(AHighligtedArea);
      DrawDelimiterGlyph(ACanvas);
      DrawTexT(ACanvas, ATextRect, GetTextColor);
    finally
      ACanvas.RestoreClipRegion;
    end;
  end
...
procedure TdxBreadcrumbEditNodeViewItem.DrawDelimiterGlyph(ACanvas: TcxCanvas; AColor: TColor);
begin
  Painter.DrawBreadcrumbEditScaledNodeDelimiterGlyph(ACanvas, DelimiterRect, DelimiterState, ScaleFactor, AColor);
end;

procedure TdxBreadcrumbEditNodeViewItem.DrawTexT(ACanvas: TcxCanvas; const ATextRect: TRect; ATextColor: TColor);
const
  AlignMap: array[Boolean] of TAlignment = (taLeftJustify, taCenter);
begin
  if ACanvas.RectVisible(ATextRect) then
  begin
    ACanvas.Font := Font;
    ACanvas.Font.Color := ATextColor;
    ACanvas.DrawTexT(Node.ActualDisplayName, ATextRect, AlignMap[cxRectWidth(ATextRect) > FTextSize.cx], vaCenter, False, True);
  end;
end;
...
procedure TdxBreadcrumbEditRootNodeViewItem.DrawDelimiterGlyph(ACanvas: TcxCanvas; AColor: TColor = clDefault);
begin
  if IsNextNodeHidden then
    Painter.DrawBreadcrumbEditScaledNodeMoreButtonGlyph(ACanvas, DelimiterRect, DelimiterState, ScaleFactor, AColor)
  else
    inherited;
end;
...
begin
  ...
  FRecentPathsAddToSuggestions := True;
...
begin
  ...
  begin
    ...
    RecentPathsAddToSuggestions := TdxBreadcrumbEditPathEditorProperties(Source).RecentPathsAddToSuggestions;
...
constructor TdxBreadcrumbEditPartCustomAccessibilityHelper.Create(AOwnerObject: TObject);
begin
  inherited;
  FLocalId := ViewInfo.Owner.FAccessibleObjects.Count + 1;
  ViewInfo.Owner.FAccessibleObjects.Add(Self);
end;

function TdxBreadcrumbEditPartCustomAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  Result := ViewInfo.Bounds;
  Result := cxRectSetOrigin(Result, ViewInfo.Control.GetContainer.ClientToScreen(Result.TopLeft));
end;

function TdxBreadcrumbEditPartCustomAccessibilityHelper.ChildIsSimpleElement(AIndex: Integer): Boolean;
begin
  Result := False;
end;

function TdxBreadcrumbEditPartCustomAccessibilityHelper.GetChild(AIndex: Integer): TcxAccessibilityHelper;
begin
  Result := ViewInfo.ViewItems[AIndex].IAccessibilityHelper.GetHelper;
end;

function TdxBreadcrumbEditPartCustomAccessibilityHelper.GetChildCount: Integer;
begin
  Result := ViewInfo.FViewItemList.Count;
end;

function TdxBreadcrumbEditPartCustomAccessibilityHelper.GetParent: TcxAccessibilityHelper;
begin
  Result := ViewInfo.Owner.IAccessibilityHelper.GetHelper;
end;

function TdxBreadcrumbEditPartCustomAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxROLE_SYSTEM_TOOLBAR;
end;

function TdxBreadcrumbEditPartCustomAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxSTATE_SYSTEM_FOCUSABLE;
end;

function TdxBreadcrumbEditPartCustomAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := [aopFocus, aopLocation];
end;

function TdxBreadcrumbEditPartCustomAccessibilityHelper.IsExtended: Boolean;
begin
  Result := True;
end;

procedure TdxBreadcrumbEditPartCustomAccessibilityHelper.OwnerObjectDestroyed;
begin
  if (ViewInfo.Owner <> nil) and (ViewInfo.Owner.FAccessibleObjects <> nil) then
    ViewInfo.Owner.FAccessibleObjects[FLocalId - 1] := nil;
  inherited;
end;

function TdxBreadcrumbEditPartCustomAccessibilityHelper.GetViewInfo: TdxBreadcrumbEditPartCustomViewInfo;
begin
  Result := TdxBreadcrumbEditNodesAreaPartViewInfo(OwnerObject);
end;
...
function TdxBreadcrumbEditPartCustomViewInfo.GetAccessibilityHelperClass: TdxBreadcrumbEditPartCustomAccessibilityHelperClass;
begin
  Result := TdxBreadcrumbEditPartCustomAccessibilityHelper;
end;
...
function TdxBreadcrumbEditPartCustomViewInfo.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin
  if FIAccessibilityHelper = nil then
    FIAccessibilityHelper := GetAccessibilityHelperClass.Create(Self);
  Result := FIAccessibilityHelper;
...
function TdxBreadcrumbPathEditorPartAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  Result := cxRectSetWidth(ViewInfo.Bounds, cxRectWidth(ViewInfo.Bounds) - cxRectWidth(ViewInfo.DropDownButton.Bounds));
  Result := cxRectSetOrigin(Result, ViewInfo.Control.GetContainer.ClientToScreen(Result.TopLeft));
end;

function TdxBreadcrumbPathEditorPartAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := ViewInfo.SelectedNodeViewItem.Node.Path;
end;

function TdxBreadcrumbPathEditorPartAccessibilityHelper.GetViewInfo: TdxBreadcrumbEditNodesAreaPartViewInfo;
begin
  Result := TdxBreadcrumbEditNodesAreaPartViewInfo(inherited ViewInfo);
end;

{ TdxBreadcrumbEditNodesAreaPartAccessibilityHelper }

function TdxBreadcrumbEditNodesAreaPartAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  if AChildID = 0 then
    Result := ViewInfo.Bounds
  else
    Result := ViewInfo.DropDownButton.Bounds;

  Result := cxRectSetOrigin(Result, ViewInfo.Control.GetContainer.ClientToScreen(Result.TopLeft));
end;

function TdxBreadcrumbEditNodesAreaPartAccessibilityHelper.ChildIsSimpleElement(AIndex: Integer): Boolean;
begin
  Result := AIndex = 1;
end;

procedure TdxBreadcrumbEditNodesAreaPartAccessibilityHelper.DoDefaultAction(AChildID: TcxAccessibleSimpleChildElementID);
begin
  if AChildID = 2 then
    ViewInfo.DropDownButton.Click(nvipControl);
end;

function TdxBreadcrumbEditNodesAreaPartAccessibilityHelper.GetChild(AIndex: Integer): TcxAccessibilityHelper;
begin
  Result := ViewInfo.IPathEditorPartAccessibilityHelper.GetHelper;
end;

function TdxBreadcrumbEditNodesAreaPartAccessibilityHelper.GetChildCount: Integer;
begin
  Result := 1;
  if ViewInfo.DropDownButton.Visible then
    Inc(Result);
end;

function TdxBreadcrumbEditNodesAreaPartAccessibilityHelper.GetDefaultActionDescription(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  if AChildID = 2 then
    Result := 'Click';
end;

function TdxBreadcrumbEditNodesAreaPartAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  if AChildID = 0 then
    Result := ViewInfo.SelectedNodeViewItem.Node.Path
  else
  begin
    Result := ViewInfo.DropDownButton.HintText;
    if Result = '' then
      Result := 'Open';
  end;
end;

function TdxBreadcrumbEditNodesAreaPartAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  if AChildID = 0 then
    Result := cxROLE_SYSTEM_INDICATOR
  else
    Result := cxROLE_SYSTEM_PUSHBUTTON;
end;

function TdxBreadcrumbEditNodesAreaPartAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxSTATE_SYSTEM_NORMAL;
  if (AChildID > 0) and ViewInfo.DropDownButton.GetIsHot then
    Result := Result or cxSTATE_SYSTEM_HOTTRACKED;
end;

function TdxBreadcrumbEditNodesAreaPartAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := [aopFocus, aopLocation];
  if AChildID = 0 then
    Result := Result + [aopValue];
  if AChildID = 2 then
    Result := Result + [aopDefaultAction];
end;

function TdxBreadcrumbEditNodesAreaPartAccessibilityHelper.GetValue(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := FloatToStr(ViewInfo.Owner.ProgressBarViewInfo.Progress);
end;

function TdxBreadcrumbEditNodesAreaPartAccessibilityHelper.GetViewInfo: TdxBreadcrumbEditNodesAreaPartViewInfo;
begin
  Result := TdxBreadcrumbEditNodesAreaPartViewInfo(OwnerObject);
end;
...
begin
  cxAccessibilityHelperOwnerObjectDestroyed(FIPathEditorPartAccessibilityHelper);
...
function TdxBreadcrumbEditNodesAreaPartViewInfo.GetAccessibilityHelperClass: TdxBreadcrumbEditPartCustomAccessibilityHelperClass;
begin
  Result := TdxBreadcrumbEditNodesAreaPartAccessibilityHelper;
end;
...
begin
  if IsImageAssigned(Properties.Images, AImageIndex) then
    TdxImageDrawer.DrawUncachedImage(ACanvas.Handle, ARect, ARect, nil, Properties.Images,
...
function TdxBreadcrumbEditNodesAreaPartViewInfo.GetIPathEditorPartAccessibilityHelper: IcxAccessibilityHelper;
begin
  if FIPathEditorPartAccessibilityHelper = nil then
    FIPathEditorPartAccessibilityHelper := TdxBreadcrumbPathEditorPartAccessibilityHelper.Create(Self);
  Result := FIPathEditorPartAccessibilityHelper;
end;
...
constructor TdxBreadcrumbEditAccessibilityHelper.Create(AOwnerObject: TObject);
begin
  inherited;
  FAccessibilityChildren := TList.Create;
end;

destructor TdxBreadcrumbEditAccessibilityHelper.Destroy;
begin
  FreeAndNil(FAccessibilityChildren);
  inherited;
end;

function TdxBreadcrumbEditAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  if AChildID = 0 then
    Result := ViewInfo.Bounds
  else
    Result := TdxBreadcrumbEditButtonViewItem(ViewInfo.ButtonsViewInfo.FViewItemList[AChildID - FAccessibilityChildren.Count - 1]).Bounds;

  Result := cxRectSetOrigin(Result, ViewInfo.Control.GetContainer.ClientToScreen(Result.TopLeft));
end;

function TdxBreadcrumbEditAccessibilityHelper.ChildIsSimpleElement(AIndex: Integer): Boolean;
begin
  Result := AIndex >= FAccessibilityChildren.Count;
end;

function TdxBreadcrumbEditAccessibilityHelper.GetChild(AIndex: Integer): TcxAccessibilityHelper;
begin
  Result := FAccessibilityChildren[AIndex];
end;

function TdxBreadcrumbEditAccessibilityHelper.GetChildCount: Integer;
begin
  FAccessibilityChildren.Clear;
  if (ViewInfo.Control.GetController.PathEditingController.PathEditor <> nil) then
    FAccessibilityChildren.Add(ViewInfo.Control.GetController.PathEditingController.PathEditor.IAccessibilityHelper.GetHelper);
  FAccessibilityChildren.Add(ViewInfo.NodesAreaViewInfo.IAccessibilityHelper.GetHelper);
  Result := ViewInfo.ButtonsViewInfo.FViewItemList.Count + FAccessibilityChildren.Count;
end;

function TdxBreadcrumbEditAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  if AChildID = 0 then
    Result := inherited
  else
    Result := TdxBreadcrumbEditButtonViewItem(ViewInfo.ButtonsViewInfo.FViewItemList[AChildID - FAccessibilityChildren.Count - 1]).HintText;
end;

function TdxBreadcrumbEditAccessibilityHelper.GetOwnerObjectWindow: HWND;
begin
  Result := ViewInfo.Control.GetContainer.Handle;
end;

function TdxBreadcrumbEditAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  if AChildID = 0 then
...
function TdxBreadcrumbEditAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  if AChildID = 0 then
    Result := cxSTATE_SYSTEM_READONLY or cxSTATE_SYSTEM_FOCUSABLE
  else
    Result := cxSTATE_SYSTEM_NORMAL;
end;

function TdxBreadcrumbEditAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := [aopFocus, aopLocation];
end;

function TdxBreadcrumbEditAccessibilityHelper.IsExtended: Boolean;
begin
  Result := True;
end;

function TdxBreadcrumbEditAccessibilityHelper.GetViewInfo: TdxBreadcrumbEditViewInfo;
begin
  Result := TdxBreadcrumbEditViewInfo(OwnerObject);
end;
...
function TdxBreadcrumbEditViewInfo.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin
  if FIAccessibilityHelper = nil then
    FIAccessibilityHelper := TdxBreadcrumbEditAccessibilityHelper.Create(Self);
  Result := FIAccessibilityHelper;
end;
...
function TdxBreadcrumbEditListBoxAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  case AChildID of
    0:
      Result := ListBox.Bounds;
    1:
      Result := TdxBreadcrumbEditDropDownInnerListBox(ListBox).RootItemRect;
  else
    Result := ListBox.ItemRect(AChildID - 2);
  end;
  Result := cxRectSetOrigin(Result, ListBox.ClientToScreen(Result.TopLeft));
end;

function TdxBreadcrumbEditListBoxAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;

  function IsRootItemFocused: Boolean;
  begin
    Result := (AChildID = 1) and (TdxBreadcrumbEditDropDownInnerListBox(ListBox).RootItemState = cxbsHot);
  end;

begin
  Result := cxSTATE_SYSTEM_FOCUSABLE or cxSTATE_SYSTEM_SELECTABLE;
  if (AChildID > 0) and ((ListBox.ItemIndex = AChildID - 2) or IsRootItemFocused)then
    Result := Result or cxSTATE_SYSTEM_FOCUSED or cxSTATE_SYSTEM_SELECTED;
end;

procedure TdxBreadcrumbEditListBoxAccessibilityHelper.DoDefaultAction(AChildID: TcxAccessibleSimpleChildElementID);
begin
  if AChildID = 1 then
  begin
    ListBox.ItemIndex := -1;
    TdxBreadcrumbEditDropDownInnerListBox(ListBox).RootItemState := cxbsPressed;
  end
  else
    inherited DoDefaultAction(AChildID - 1);
end;

function TdxBreadcrumbEditListBoxAccessibilityHelper.GetChildCount: Integer;
begin
  Result := inherited + 1;
end;

function TdxBreadcrumbEditListBoxAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  case AChildID of
    0:
      Result := '';
    1:
      Result := TdxBreadcrumbEditDropDownInnerListBox(ListBox).RootItem.Caption;
  else
    Result := ListBox.Items[AChildID - 2].Caption;
  end;
end;
...
function TdxBreadcrumbEditDropDownInnerListBox.CalculateItemHeight: Integer;
begin
  Result := Max(
    ImageSize.cy + cxMarginsHeight(ImageOffsets),
    cxMarginsHeight(TextOffsets) + TdxTextMeasurer.TextLineHeight(Font, Font.Style + [fsBold]));
end;
...
function TdxBreadcrumbEditDropDownInnerListBox.GetAccessibilityHelperClass: TdxCustomListBoxAccessibilityHelperClass;
begin
  Result := TdxBreadcrumbEditListBoxAccessibilityHelper;
end;
...
function TdxBreadcrumbEditDropDownInnerListBox.IsDefaultItem(AItem: TdxCustomListBoxItem): Boolean;
begin
  if AItem <> RootItem then
    Result := inherited IsDefaultItem(AItem)
  else
...
begin
  ...
  if Images <> nil then
    TdxImageDrawer.DrawUncachedImage(ACanvas.Handle, ImageRect, ImageRect, nil, Images, ImageIndex, idmNormal);
end;
...
procedure TdxBreadcrumbEditPathEditorViewInfo.UpdateStyle(AStyle: TcxContainerStyle);
begin
  inherited;
...
procedure TdxBreadcrumbEditPathEditorListBox.DrawItemContent(AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
var
  AImageRect: TRect;
  AText: string;
begin
  ...
  begin
    AImageRect := GetImageRect(ARect);
    if Images <> nil then
      TdxImageDrawer.DrawUncachedImage(Canvas.Handle, AImageRect, AImageRect, nil, Images, ImageIndex[AIndex], idmNormal);
    if UseRightToLeftAlignment then
      ARect.Right := AImageRect.Left
    else
      ARect.Left := AImageRect.Right;

    AText := GetItem(AIndex);
    ARect := cxRectInflate(ARect, -cxTextOffset);
    DrawText(Canvas.Handle, PChar(AText), Length(AText), ARect,
      DrawTextBiDiModeFlags(DT_SINGLELINE or DT_VCENTER or DT_NOPREFIX));
  end;
end;

function TdxBreadcrumbEditPathEditorListBox.GetDefaultItemHeight: Integer;
begin
  Result := inherited GetDefaultItemHeight;
  if Images <> nil then
    Result := Max(Result, 2 * dxBreadcrumbEditImageBorder + dxGetImageSize(Images, ScaleFactor).cy);
end;

function TdxBreadcrumbEditPathEditorListBox.GetDefaultItemPadding: TdxPadding;
begin
...
function TdxBreadcrumbEditPathEditingController.CreatePathEditor: TdxBreadcrumbEditPathEditor;
begin
  Result := TdxBreadcrumbEditPathEditor.Create(nil, True);
end;
...
procedure TdxBreadcrumbEditPathEditingController.CheckRecentPathsLoaded;
var
  AEvents: IdxBreadcrumbEditEvents;
begin
  if RecentPaths.Count = 0 then
  begin
    if Supports(Control, IdxBreadcrumbEditEvents, AEvents) then
    begin
      AEvents.PopulateRecentPaths;
      CheckRecentPathsCount;
    end;
  end;
end;
...
procedure TdxBreadcrumbEditPathEditingController.DoInitPathEdit(AEdit: TcxCustomEdit);
var
  AEvents: IdxBreadcrumbEditEvents;
begin
  if Supports(Control, IdxBreadcrumbEditEvents, AEvents) then
    AEvents.InitPathEdit(AEdit);
end;
...
procedure TdxBreadcrumbEditPathEditingController.PathEditorInitPopupHandler(Sender: TObject);
begin
...
var
  ANewPath: string;
begin
  if not VarIsNull(PathEditor.EditingValue) then
  begin
    ANewPath := PathEditor.EditingValue;
...
procedure TdxBreadcrumbEditPathEditingController.PopulateSuggestions(const APath: string; ASuggestions: TStringList);
  ...
  var
    I: Integer;
  begin
    CheckRecentPathsLoaded;
    for I := 0 to Properties.RecentPaths.Count - 1 do
      AddSuggestion(APath, Properties.RecentPaths[I].Path, ASuggestions);
  end;
...
begin
  if APath <> '' then
  begin
    if Properties.RecentPathsEnabled and Properties.RecentPathsAddToSuggestions then
...
begin
  ...
  PathEditor.Properties.OnInitPopup := PathEditorInitPopupHandler;
end;
...
function TdxBreadcrumbEditController.ShouldUpdateSelectedNode(ANode: TdxBreadcrumbEditNode): Boolean;
begin
  Result := ANode <> FSelected;
end;
...
begin
  ...
  begin
    ANodeViewItem.Node.LoadChildren;
...
begin
  ...
  ViewInfo.UpdateViewItemStates;
  if FocusedViewItem <> nil then
    NotifyWinEvent(EVENT_OBJECT_FOCUS, Control.GetContainer.Handle,
      TdxBreadcrumbEditViewItemAccessibilityHelper(FocusedViewItem.IAccessibilityHelper.GetHelper).FLocalId, CHILDID_SELF);
end;
...
begin
  ...
  begin
    if ShouldUpdateSelectedNode(AValue) then
    begin
      DoBeforeSelect;
      FSelected := AValue;
      DoAfterSelect;
...
procedure TdxCustomBreadcrumbEdit.InitPathEdit(AEdit: TcxCustomEdit);
begin
  if Assigned(OnInitPathEdit) then
    OnInitPathEdit(Self);
end;
...
procedure TdxCustomBreadcrumbEdit.PopulateRecentPaths;
begin
  if Assigned(OnPopulateRecentPaths) then
    OnPopulateRecentPaths(Self);
end;
...
procedure TdxCustomBreadcrumbEdit.WMGetObject(var Message: TMessage);
var
  AObjectID: Cardinal;
begin
...
function TdxCustomBreadcrumbEdit.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin
