...
type
  ...
  TfmCloudStorageDemoForm = class(TfmBaseForm)
    ...
    lvPermissions: TcxListView;
    acCreateSharedLink: TAction;
    dxLayoutLabeledItem1: TdxLayoutLabeledItem;
    dxLayoutAutoCreatedGroup1: TdxLayoutAutoCreatedGroup;
    dxLayoutGroup3: TdxLayoutGroup;
    cxButton6: TcxButton;
    dxLayoutItem8: TdxLayoutItem;
    acDeletePermission: TAction;
    ...
    procedure csMainItemPermissionsLoaded(Sender: TObject; const AItem: TdxCloudStorageItem);
    procedure csMainItemPermissionCreated(Sender: TObject; const APermission: TdxCloudStorageItemPermission);
    procedure csMainItemPermissionDeleted(Sender: TObject; const APermission: TdxCloudStorageItemPermission);
    procedure lvMainSelectItem(Sender: TObject; Item: TListItem;
      Selected: Boolean);
    procedure acCreateSharedLinkExecute(Sender: TObject);
    procedure acDeletePermissionExecute(Sender: TObject);
  private
    ...
    procedure UpdatePermissionList;
...
uses
  ...
  dxCloudStorageGoogleDriveProvider, DemoUtils, dxWinInet, Registry,
...
begin
  ...
  begin
    ...
    if (tvMain.Selected = nil) or (tvMain.Selected.Data <> AItem) then
...
procedure TfmCloudStorageDemoForm.lvMainSelectItem(Sender: TObject;
...
procedure TfmCloudStorageDemoForm.UpdatePermissionList;
var
  APermissions: TdxCloudStorageItemPermissions;
  I: Integer;
begin
  ...
  try
    ...
    if lvMain.Selected.Data <> nil then
    begin
      APermissions := TdxCloudStorageItem(lvMain.Selected.Data).Permissions;
      if APermissions.IsLoaded then
      begin
        for I := 0 to APermissions.Count - 1 do
          with lvPermissions.Items.Add do
          begin
            Data := APermissions[I];
            Caption := APermissions[I].ID;
            SubItems.Add(BoolToStr(APermissions[I].ReadOnly, True));
            SubItems.Add(APermissions[I].Link);
          end;
      end;
    end;
  finally
...
procedure TfmCloudStorageDemoForm.acCreateSharedLinkExecute(Sender: TObject);
begin
  if (lvMain.Selected = nil) or
      not ((TObject(lvMain.Selected.Data) is TdxCloudStorageFile) or (TObject(lvMain.Selected.Data) is TdxCloudStorageFolder)) then
    Exit;
  TdxCloudStorageItem(lvMain.Selected.Data).Permissions.CreateSharedLink;
end;
...
procedure TfmCloudStorageDemoForm.acDeletePermissionExecute(Sender: TObject);
begin
  if lvPermissions.Selected = nil then
    Exit;
...
begin
  ...
  acCreateSharedLink.Enabled := (lvMain.Selected <> nil) and (ATopFolder <> nil) and ATopFolder.IsRoot;
  acDeletePermission.Enabled := acCreateSharedLink.Enabled and (lvPermissions.Selected <> nil);
end;
...
procedure TfmCloudStorageDemoForm.csMainItemPermissionsLoaded(Sender: TObject;
  const AItem: TdxCloudStorageItem);
begin
  if (lvMain.Selected = nil) or (AItem = lvMain.Selected.Data) then
    UpdatePermissionList;
end;

procedure TfmCloudStorageDemoForm.csMainItemPermissionCreated(Sender: TObject; const APermission: TdxCloudStorageItemPermission);
begin
  if (lvMain.Selected = nil) or (APermission.Owner = lvMain.Selected.Data) then
    UpdatePermissionList;
end;

procedure TfmCloudStorageDemoForm.csMainItemPermissionDeleted(Sender: TObject; const APermission: TdxCloudStorageItemPermission);
begin
  if (lvMain.Selected = nil) or (APermission.Owner = lvMain.Selected.Data) then
    UpdatePermissionList;
end;
...
initialization
  TdxWebFileTransferManager.DefaultBlockSize := 8 * 1024 * 1024;
