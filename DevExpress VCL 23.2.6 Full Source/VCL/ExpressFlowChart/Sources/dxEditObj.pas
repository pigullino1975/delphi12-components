...
uses
  ...
  cxSpinEdit, dxGalleryControl, dxGallery, cxFontNameComboBox, cxImageList;
...
type
  ...
  TFEditObject = class(TdxForm)
    ...
    procedure cbFontPropertiesEditValueChanged(Sender: TObject);
  private
    FFontButton: TcxEditButton;

    procedure AddFontButton;
    procedure cbFontButtonClickHandler(Sender: TObject; AButtonIndex: Integer);
    //
...
begin
  ...
  FontDialog.Font.Assign(AObject.Font);
  cbFont.FontName := FontDialog.Font.Name;
...
procedure TFEditObject.AddFontButton;
begin
  FFontButton := cbFont.Properties.Buttons.Add;
  cbFont.Properties.OnButtonClick := cbFontButtonClickHandler;
  FFontButton.Kind := bkGlyph;
  cbFont.Properties.Images := ilImages;
  FFontButton.ImageIndex := 0;
end;
...
begin
  ...
  Label4.CaptionOptions.Text := cxGetResourceString(@sdxFlowChartConnectionEditorTextFontHint);
...
begin
  ...
  if IsModified(cbFont) then
    AObject.Font.Assign(FontDialog.Font);
...
procedure TFEditObject.cbFontButtonClickHandler(Sender: TObject; AButtonIndex: Integer);
var
  AFont: TFont;
begin
  if FFontButton.Index = AButtonIndex then
  begin
    AFont := TFont.Create;
    try
...
procedure TFEditObject.cbFontPropertiesEditValueChanged(Sender: TObject);
begin
  FontDialog.Font.Name := cbFont.FontName;
end;
