...
uses
  ...
  dxFlowChartColorPicker, dxShellDialogs, dxDPIAwareUtils;
...
type
  ...
  protected
    ...
    property AddingObject: TdxFcObject read FAddingObject;
  ...
  TdxFlowChartDesigner = class(TdxFlowChartCustomDesigner,
      IdxFlowChartEditorOptions,
    ...
    rgiBackgroundThemeColor: TdxRibbonGalleryItem;
    rgiObjectBackgroundColor: TdxRibbonGalleryItem;
    rgiObjectBackgroundThemeColor: TdxRibbonGalleryItem;
    ...
    iStandardShapeIcons: TcxImageList;
  ...
  protected
    ...
    procedure UpdateDockingCaption;
...
const
  dxThisUnitName = 'dxFlowChartDesigner';
...
procedure DrawHelpedColorLine(ScaleFactor: TdxScaleFactor; AGlyph: TdxSmartGlyph; AColor: TColor;
...
begin
  ...
  try
    ASize := ScaleFactor.Apply(ASourceSize);
    ...
    try
      ...
      ALineWidth := Round(ScaleFactor.ApplyF(13.0));
      ALineHeight := Round(ScaleFactor.ApplyF(2.0));
      ...
      try
        ...
        ALineBounds := cxRectBounds(Round(ScaleFactor.ApplyF(1.0)), Round(ScaleFactor.ApplyF(13.0)), ALineWidth, ALineHeight);
...
type
  ...
  strict private
    ...
    function GetViewInfo: TdxGalleryControlViewInfo;
...
function TdxGalleryControlHelper.GetViewInfo: TdxGalleryControlViewInfo;
begin
  ...
  try
    ...
    begin
      if AShapeType = fcsAdvanced then
        FAddingObject := CreateObject(0, 0, 0, 0, AAdvancedShape)
      else
        FAddingObject := CreateObject(0, 0, 0, 0, AShapeType);
      AddingObject.ShapeColor := dxDefaultShapeColor;
      AddingObject.BkColor := dxDefaultShapeBkColor;
      Result := AddingObject;
    end);
  finally
    EndTransactChange;
...
var
  AMemo: TcxMemo;
begin
  if Safe.Cast(Sender, TcxMemo, AMemo) then
...
var
  DesignerSessionData: TDesignerSessionData;

function TdxFlowChartDesigner.GetInfoData: Pointer;
begin
  DesignerSessionData.AntiAliasing := acAntialiasing.Checked;
  DesignerSessionData.ShowGrid := acGridLines.Checked;
  DesignerSessionData.SnapToGrid := acSnapToGrid.Checked;
  Result := @DesignerSessionData;
end;

function TdxFlowChartDesigner.GetInfoDataSize: Integer;
begin
  Result := SizeOf(TDesignerSessionData);
end;

procedure TdxFlowChartDesigner.SetInfoData(AData: Pointer);
begin
  Move(AData^, DesignerSessionData, GetInfoDataSize);
  acAntialiasing.Checked := DesignerSessionData.AntiAliasing;
  acAntialiasingExecute(acAntialiasing);

  acGridLines.Checked := DesignerSessionData.ShowGrid;
  acGridLinesExecute(acGridLines);

  acSnapToGrid.Checked := DesignerSessionData.SnapToGrid;
  acSnapToGridExecute(acSnapToGrid);
end;
...
begin
  ...
  AMenuItem.ImageIndex := Ord(AShapeType) - 1;
  AMenuItem.LargeImageIndex := Ord(AShapeType) -1;
  AMenuItem.OnClick := ToolsStandardShapeItemClickHandler;
  if AMenuItem.Down then
    AMenuItem.Click;
end;

procedure TdxFlowChartDesigner.AddStandardShapeIcon(AShapeType: TdxFcShapeType);
var
  AGlyph: TdxSmartGlyph;
begin
  AGlyph := GetStandardShapeGlyph(AShapeType, ScaleFactor.Apply(cxSize(dxToolStandardShapesGlyphSize)));
  try
    iStandardShapeIcons.Add(AGlyph);
  finally
    AGlyph.Free;
  end;
end;

procedure TdxFlowChartDesigner.AfterConstruction;
begin
  inherited;
...
begin
  Result := True;
  AMsgRes := dxMessageDlg(cxGetResourceString(@sdxFlowChartEditorNewConfirmation), mtConfirmation, [mbYes, mbNo, mbCancel], 0);
  case AMsgRes of
    mrYes:
      begin
        acSave.Execute;
...
begin
  bpmStencils.ItemLinks.BeginUpdate;
  try
    ...
    begin
      ...
      if not AStencil.HasVisibleShapes then
        Continue;
      AMenuButton := TdxBarButton(bpmStencils.ItemLinks.AddButton.Item);
...
procedure TdxFlowChartDesigner.BuildStandardShapeIcons;
begin
  iStandardShapeIcons.BeginUpdate;
  try
    iStandardShapeIcons.Clear;
    iStandardShapeIcons.SourceDPI := ScaleFactor.TargetDPI;
    iStandardShapeIcons.SetSize(ScaleFactor.Apply(cxSize(dxToolStandardShapesGlyphSize)));
    ForEachStandardShape(AddStandardShapeIcon);
  finally
    iStandardShapeIcons.EndUpdate;
  end;
end;
...
begin
  ...
  ChangeSelectedChartItems(
    ...
    var
      ...
      AObject: TdxFcObject;
    begin
      if Safe.Cast(AItem, TdxFcObject, AObject) then
        AObject.ShapeStyle := APenStyle;
      if Safe.Cast(AItem, TdxFcConnection, AConnection) then
...
var
  ...
  ACanvas: TdxGpCanvas;
begin
  AGlyph := TdxSmartGlyph.CreateSize(gcShapes.OptionsView.Item.Image.Size.Size.Scale(2));
  AGlyph.SourceDPI := AGlyph.SourceDPI * 2;
  try
...
procedure TdxFlowChartDesigner.ScaleFactorChanged(M, D: Integer);
begin
...
begin
  DrawHelpedColorLine(ScaleFactor, bbBackgroundColor.Glyph, FBackgroundColorPicker.Color, ilSmallIcons, acBackgroundColor.ImageIndex);
end;

procedure TdxFlowChartDesigner.UpdateLineColorImage;
begin
  DrawHelpedColorLine(ScaleFactor, btnLineColor.OptionsImage.Glyph, FLineColorPicker.Color,
    ilSmallIcons, btnLineColor.OptionsImage.ImageIndex);
end;

procedure TdxFlowChartDesigner.UpdateDestArrowColorImage;
begin
  DrawHelpedColorLine(ScaleFactor, btnDestArrowColor.OptionsImage.Glyph, FDestArrowColorPicker.Color,
    ilSmallIcons, btnDestArrowColor.OptionsImage.ImageIndex);
end;

procedure TdxFlowChartDesigner.UpdateDockingCaption;
begin
  dpShapes.Caption := cxGetResourceString(@sdxFlowChartEditorShapes);
  dpProperties.Caption := cxGetResourceString(@sdxFlowChartEditorProperties);

  gciMoreShapes.Caption := cxGetResourceString(@sdxFlowChart_More_Shapes);
end;

procedure TdxFlowChartDesigner.UpdateFontColorImage;
begin
  DrawHelpedColorLine(ScaleFactor, bbFontColor.Glyph, FFontColorPicker.Color, ilSmallIcons, acFontColor.ImageIndex);
end;
...
begin
  ...
  try
    ...
    for I := 0 to bpmStencils.ItemLinks.Count - 1 do
    begin
      AMenuButton := TdxBarButton(bpmStencils.ItemLinks[I].Item);
...
begin
  DrawHelpedColorLine(ScaleFactor, btnObjectBkColor.OptionsImage.Glyph, FObjectBkColorPicker.Color, ilSmallIcons,
...
begin
  ...
  acBackgroundColor.Caption := cxGetResourceString(@sdxFlowChartEditorEditBackground);
  ...
  acCloseAndApplyChanges.Caption := cxGetResourceString(@sdxFlowChartEditorEditCloseAndApplyChanges);
  acConnector.Caption := cxGetResourceString(@sdxFlowChartEditorEditConnector);
  ...
  acStrokeColor.Caption := cxGetResourceString(@sdxFlowChartEditorEditStroke);
  ...
  acSnapToGrid.Caption := cxGetResourceString(@sdxFlowChartEditorDesignSnapToGrid);
  bsiConnectors.Caption := cxGetResourceString(@sdxFlowChartEditorDesignConnectors);
  acApplyLayeredLayout.Caption := cxGetResourceString(@sdxFlowChartEditorDesignLayeredLayout);
  ...
  acGridLines.Caption := cxGetResourceString(@sdxFlowChartEditorViewGridLines);
...
begin
  DrawHelpedColorLine(ScaleFactor, btnSourceArrowColor.OptionsImage.Glyph, FSourceArrowColorPicker.Color,
...
begin
  ...
  begin
    if Safe.Cast(AGroup[I], TdxCustomLayoutGroup, AChildGroup) then
      IterateGroupControls(AChildGroup, AAction)
    else
      if Safe.Cast(AGroup[I], TdxLayoutItem, AItem) then
      begin
        if Safe.Cast(AItem.Control, TWinControl, AControl) then
          AAction(AControl);
      end;
  end;
end;
...
begin
  ...
  IterateGroupControls(lgCommon,
    ...
    begin
      ...
      begin
        if Safe.Cast(AControl, TcxCustomEdit, AEdit) then
...
begin
  DrawHelpedColorLine(ScaleFactor, bbStrokeColor.Glyph, FStrokeColorPicker.Color, ilSmallIcons, acStrokeColor.ImageIndex);
end;
...
begin
  ...
  begin
    P := GetMouseCursorPos;
    FDownPoint := Chart.ScreenToClient(P);
...
begin
  if FUpdatingPropertiesControls then
    Exit;
  if Safe.Cast(Sender, TcxCustomEdit, AEdit) and AEdit.ModifiedAfterEnter then
  begin
...
var
  AAction: TAction;
begin
  if Safe.Cast(Sender, TAction, AAction) then
  begin
...
begin
  if not ((ModalResult = mrCancel) and FModified) then
    Exit;
  AMsgRes := dxMessageDlg(cxGetResourceString(@sdxFlowChartEditorExitConfirmation), mtConfirmation, [mbYes, mbNo, mbCancel], 0);
  case AMsgRes of
    mrYes:
      if Mode = dmChartDesigner then
...
begin
  ...
  FBackgroundColorPicker := TdxFlowChartColorPickerController.Create(ppmBackgroundColor,
    rgiBackgroundColor, rgiBackgroundThemeColor);
  ...
  FObjectBkColorPicker := TdxFlowChartColorPickerController.Create(
    ppmObjectBkColor, rgiObjectBackgroundColor, rgiObjectBackgroundThemeColor);
...
begin
  if gcMoreShapes.ViewInfo.FindItemViewInfo(gciMoreShapes, AViewInfo) then
  begin
    ARect := dxMapWindowRect(gcMoreShapes.Handle, 0, AViewInfo.Bounds);
    bpmStencils.Popup(ARect.Right, ARect.Top);
  end;
end;
...
begin
  ...
  dxRegisterCursor(crFlChartZoomIn, HInstance, 'ZOOMIN');
  dxRegisterCursor(crFlChartZoomOut, HInstance, 'ZOOMOUT');
end;

procedure Finalize;
begin
  dxUnregisterCursor(crFlChartZoomIn);
  dxUnregisterCursor(crFlChartZoomIn);
