...
const
  dxThisUnitName = 'dxFlowChartShapeDescriptions.Xml';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TUnitCollection.Initialize', SysInit.HInstance);{$ENDIF}
  FDefault := TUnitCollection.CreateDefault;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TUnitCollection.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TUnitCollection.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TUnitCollection.Finalize', SysInit.HInstance);{$ENDIF}
  FDefault.Free;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TUnitCollection.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
procedure TdxShapeXmlDescription.Validate;

  procedure ErrorBadGeometry;
  const
    SBadGeometry = 'Each geometry must include the Start segment followed by at least one Line or Arc segment.';
  begin
    raise EdxFlowChartShapeParserError.CreateFmt('The "%s" custom shape error. %s', [Caption, SBadGeometry]);
  end;

  procedure ErrorIdMissed;
  begin
    raise EdxFlowChartShapeParserError.Create('The custom shape error. The "ID" attribute is missed.');
  end;
...
begin
  if ID = '' then
    ErrorIdMissed;
  if (FSegments.Count < 2) or (FSegments[0].ClassType <> TStart) then
    ErrorBadGeometry;
  for I := 0 to FSegments.Count - 2 do
    if (FSegments[I].ClassType = TStart) and (FSegments[I + 1].ClassType = TStart) then
      ErrorBadGeometry;
end;
