...
uses
  System.UITypes,
  Types, SysUtils, Classes, Controls, Generics.Defaults, Generics.Collections, dxCoreClasses;
...
type
  ...
  TdxGuideController = class // for internal use
  public type
  ...
  public
    ...
    property Bounds: TdxRectList read FBounds;
  end;

  { TdxGuidesViewData }

  TdxGuidesViewData = class // for internal use
  strict private
    FCheckSizes: Boolean;
    FControl: TControl;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxGuideController.TGuideList.Initialize', SysInit.HInstance);{$ENDIF}
  FComparer := TComparer<TGuide>.Construct(
    function(const Left, Right: TGuide): Integer
    begin
      Result := Left.Value - Right.Value;
    end);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxGuideController.TGuideList.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxGuideController.TGuideList.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxGuideController.TGuideList.Finalize', SysInit.HInstance);{$ENDIF}
  FComparer := nil;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxGuideController.TGuideList.Finalize', SysInit.HInstance);{$ENDIF}
end;
