...
const
  dxThisUnitName = 'dxRightAngleRoutingStrategy';
...
begin
  ...
  if (ATarget.HasValue and ATarget.Value.Contains(Result)) and (ASource <> ATarget) then
    Exit(APoint);
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxRightAngleRoutingStrategy.TOpenSet.Initialize', SysInit.HInstance);{$ENDIF}
  ...
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxRightAngleRoutingStrategy.TOpenSet.Initialize', SysInit.HInstance);{$ENDIF}
end;
