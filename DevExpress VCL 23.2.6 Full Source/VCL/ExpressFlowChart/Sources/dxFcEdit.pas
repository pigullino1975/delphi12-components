...
uses
  ...
  cxImageList, cxClasses, dxColorDialog, cxGeometry, dxCoreClasses, dxShellDialogs;
...
procedure TBufferItem.SetObject(Value : TObject);
begin
  FObject := Value;
end;

procedure TBufferItem.SetObjectType(Value : TItemType);
begin
  FObjectType := Value;
end;
...
var
  AColor: TColor;
begin
  AColor := dxColorToAlphaColor(TPanel(Sender).Color);
...
begin
  ...
  if (hTest * [htOnConnection, htOnConLabel,htOnArrowSrc,htOnArrowDst]) <> [] then
    ...
    begin
      FStore := False;
      Res := ConnectEditor(Chart, Chart.SelectedConnections[0]); 
      FStore := True;
      if Res then
        ChartChange(Chart, nil);
      ChartSelected(Chart, nil);
...
begin
  for I := 0 to Buf.Count - 1 do
    if TBufferItem(Buf.Items[i]).ObjType = iConnection then
      TdxFcConnection(TBufferItem(Buf.Items[i]).Obj).Free;
  for i := 0 to Buf.Count - 1 do
    if TBufferItem(Buf.Items[i]).ObjType = iObject then
      TdxFcObject(TBufferItem(Buf.Items[i]).Obj).Free;
  while Buf.Count > 0 do
  begin
    TBufferItem(Buf.Items[0]).Free;
    Buf.Delete(0);
  end;
end;
  ...
  try
    ...
    for i := 0 to Buf.Count - 1 do
      ...
      begin
        ...
        AObj.Assign(TdxFcObject(TBufferItem(Buf.Items[i]).Obj));
    ...
    for i := 0 to Buf.Count - 1 do
      ...
      begin
        ...
        ACon.AssignPattern(TdxFcConnection(TBufferItem(Buf.Items[i]).Obj));
        ...
        for j := 0 to Buf.Count - 1 do
          if TBufferItem(Buf.Items[j]).ObjType = iObject then
          begin
            inc(Index);
            if TdxFcObject(TBufferItem(Buf.Items[j]).Obj) = TdxFcConnection(TBufferItem(Buf.Items[i]).Obj).ObjectSource then
              SObj := TdxFcObject(List.Items[Index]);
            if TdxFcObject(TBufferItem(Buf.Items[j]).Obj) = TdxFcConnection(TBufferItem(Buf.Items[i]).Obj).ObjectDest then
              DObj := TdxFcObject(List.Items[Index]);
          end;
        ACon.SetObjectSource(SObj, TdxFcConnection(TBufferItem(Buf.Items[i]).Obj).PointSource);
        ACon.SetObjectDest(DObj, TdxFcConnection(TBufferItem(Buf.Items[i]).Obj).PointDest);
        ACon.Selected := True;
      end;
  finally
    List.Free;
...
begin
  ...
  begin
    P := GetMouseCursorPos;
    DownPoint := Chart.ScreenToClient(P);
  ...
  begin
    if btnCreateObject.Down then
    begin
      P := Chart.ScreenToClient(GetMouseCursorPos);
