...
uses
  cxGeometry, Forms, Math, dxTypeHelpers;

const
  dxThisUnitName = 'dxLines';
...
class function TdxFcLinesHelper.LineCenter(
  ...
  begin
    ...
    if AIndex = ACount - 2 then
      ...
      ANewLength := PartialLength(AOldLength + Result, AIndex + 1, ACenter);

    if (Result + AOldLength >= ANewLength) and (ANewLength + Result >= AOldLength) then
    begin
      ACenter.X := (ANewLength + Result - AOldLength) shr 1;
      ACenter.Y := Result;
      LastIndex := AIndex;
      PartLength(ALineType, APoints, AIndex, ACount, @ACenter);
    end;
    Inc(Result, ANewLength);
  end;
...
begin
  Result := Round(Src.DistanceToSegment(PS, PE));
end;
