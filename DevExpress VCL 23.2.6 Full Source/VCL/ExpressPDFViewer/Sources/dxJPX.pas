...
uses
  Types, SysUtils, Classes, Generics.Defaults, Generics.Collections, dxCore, dxCoreClasses, cxVariants;
...
type
  ...
  { IdxJPXTileDataAction }

  IdxJPXTileDataAction = interface
  ['{3AD28C1C-75FA-4705-A814-37FB68E09641}']
    procedure Process(ATile: TdxJPXTile);
  end;
  ...
  protected
    ...
    property Tiles: TObjectList<TdxJPXTileData> read GetTiles;
  public
    constructor Create;
    destructor Destroy; override;

    class function Decode(const AData: TBytes; out AComponentCount: Integer): TBytes;
    class function DecodeImage(const AData: TBytes): TdxJPXImage; static;

    function CreateTile(AIndex: Integer): TdxJPXTile;

    property CodingStyleComponents: TdxJPXCodingStyleComponents read FCodingStyleComponents write FCodingStyleComponents;
    property NumXTiles: Integer read FNumXTiles;
    property NumYTiles: Integer read FNumYTiles;
  ...
  { TdxJPXTilePacketData }

  TdxJPXTilePacketData = class(TInterfacedObject, IdxJPXTileDataAction)
  strict private
    FStream: TStream;
  public
  ...
  { TdxJPXTileQuantizationParametersData }

  TdxJPXTileQuantizationParametersData = class(TInterfacedObject, IdxJPXTileDataAction)
  strict private
    FComponentIndex: Integer;
    FQuantizationComponentParameters: TdxJPXQuantizationComponentParameters;
  public
    constructor Create(AComponentIndex: Integer; AQuantizationComponentParameters: TdxJPXQuantizationComponentParameters);
    procedure Process(ATile: TdxJPXTile);
  end;
  ...
  TdxJPXTileData = class
  strict private
    FPackets: TList<IdxJPXTileDataAction>;
  public
    constructor Create;
    destructor Destroy; override;
    procedure AppendPacket(AStream: TStream);
    procedure AppendQuantizationParameters(AComponentIndex: Integer; AParameters: TdxJPXQuantizationComponentParameters);
    procedure Process(ATile: TdxJPXTile);
  end;
  ...
  strict private
    FColorTransformation: TdxJPXColorTransformation;
    ...
    function GetColorTransformation: TdxJPXColorTransformation;
  ...
  public
    ...
    property ColorTransformation: TdxJPXColorTransformation read GetColorTransformation;
  end;
  ...
  public
    constructor Create(ASubBand: TdxJPXSubBand; ACodeBlockWidth, ACodeBlockHeight, X, Y, AWidth, AHeight, AIndex: Integer);
    destructor Destroy; override;
  ...
  protected
    ...
    procedure TransformColor(V, H: Integer; const ADestination: TBytes; ADestinationOffset: Integer); virtual; abstract;
    //
  ...
  public
    ...
    destructor Destroy; override;
    //
    procedure Transform(var AResult: TBytes; AStartOffset, ARowWidth: Integer);
    procedure TransformLine(var AResult: TBytes; AStartOffset, V: Integer);
  end;
  ...
  protected
    procedure TransformColor(V, H: Integer; const ADestination: TBytes; ADestinationOffset: Integer); override;
  end;
  ...
  protected
    procedure Transform(const AData: TBytes; Y0, Y1, Y2: Single; ADestinationOffset: Integer); virtual;
    procedure TransformColor(V, H: Integer; const ADestination: TBytes; ADestinationOffset: Integer); override;
  public
  ...
  protected
    procedure Transform(const AData: TBytes; Y0, Y1, Y2: Single; ADestinationOffset: Integer); override;
  end;
...
uses
  Math, cxGeometry, dxPDFUtils;

const
  dxThisUnitName = 'dxJPX';
...
var
  AImage: TdxJPXImage;
  AGridWidth, ARowSize, I: Integer;
  ATile: TdxJPXTile;
  AColorTransformation: TdxJPXColorTransformation;
begin
  AImage := DecodeImage(AData);
  try
    AGridWidth := AImage.Size.GridWidth;
    AComponentCount := Length(AImage.CodingStyleComponents);
    ARowSize := AComponentCount * AGridWidth;
    SetLength(Result, ARowSize * AImage.Size.GridHeight);
    for I := 0 to AImage.TileCount - 1 do
    begin
      ATile := AImage.CreateTile(I);
      try
        AColorTransformation := TdxJPXColorTransformation.Parse(ATile);
        try
          AColorTransformation.Transform(Result, AComponentCount * (ATile.X0 + ATile.Y0 * AGridWidth), AGridWidth);
        finally
          AColorTransformation.Free;
        end;
      finally
        ATile.Free;
      end;
    end;
    AComponentCount := Length(AImage.Size.Components);
  finally
    AImage.Free;
  end;
end;

class function TdxJPXImage.DecodeImage(const AData: TBytes): TdxJPXImage;

  procedure InternalDecode(AImage: TdxJPXImage; AReader: TdxBigEndianStreamReader);
  var
  ...
  begin
    ...
    if AIsCodeStream then
      TdxJPXContiguousCodeStreamBox.PopulateImage(AReader, AReader.Length, AImage)
    else
      while not AReader.IsEOF do
        TdxJPXCustomBox.ReadImage(AReader, AImage);
  end;
...
begin
  ...
  try
    Result := TdxJPXImage.Create;
    try
      AReader := TdxBigEndianStreamReader.Create(AStream);
      try
        InternalDecode(Result, AReader);
      finally
        AReader.Free;
      end;
    except
...
function TdxJPXImage.CreateTile(AIndex: Integer): TdxJPXTile;
begin
  Result := TdxJPXTile.Create(Self, AIndex);
  Tiles[AIndex].Process(Result);
end;
...
function TdxJPXImage.GetTileCount: Integer;
begin
  Result := FNumXTiles * FNumYTiles;
end;

function TdxJPXImage.GetTiles: TObjectList<TdxJPXTileData>;
var
  I: Integer;
begin
  if FTiles = nil then
  begin
    FTiles := TObjectList<TdxJPXTileData>.Create;
    for I := 0 to FNumXTiles * FNumYTiles - 1 do
      FTiles.Add(TdxJPXTileData.Create);
  end;
  Result := FTiles;
end;
...
begin
  ...
  FNumXTiles := Ceil((FSize.GridWidth - Max(FSize.TileHorizontalOffset, FSize.GridHorizontalOffset)) / FSize.TileWidth);
  FNumYTiles := Ceil((FSize.GridHeight - Max(FSize.TileVerticalOffset, FSize.GridVerticalOffset)) / FSize.TileHeight);
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxJPXCustomBox.Create', SysInit.HInstance);{$ENDIF}
  ...
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxJPXCustomBox.Create', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxJPXCustomBox.Destroy;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxJPXCustomBox.Destroy', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FDictionary);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxJPXCustomBox.Destroy', SysInit.HInstance);{$ENDIF}
end;
...
class procedure TdxJPXContiguousCodeStreamBox.PopulateImage(AReader: TdxBigEndianStreamReader; ALength: Integer;
  ...
  procedure AddMarker(AList: TdxFastObjectList; AMarkerID: Byte);
  var
...
var
  ...
  ATempList: TdxFastObjectList;
begin
  AEndPosition := AReader.Position + ALength;
  ADefaultCodingStyleMarker := nil;
  ADefaultQuantizationMarker := nil;
  ATempList := TdxFastObjectList.Create;
  try
    ...
    begin
      ...
      begin
        ...
        if ACurrentMarker is TdxJPXCodeStreamEndMarker then
          Break;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxJPXCustomMarker.Create', SysInit.HInstance);{$ENDIF}
  ...
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxJPXCustomMarker.Create', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxJPXCustomMarker.Destroy;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxJPXCustomMarker.Destroy', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FDictionary);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxJPXCustomMarker.Destroy', SysInit.HInstance);{$ENDIF}
end;
...
procedure TdxJPXStartOfTilePartMarker.PopulateImage(AReader: TdxBigEndianStreamReader; AImage: TdxJPXImage;
  ...
  begin
    ...
    AImage.Tiles[ATile].AppendPacket(TBytesStream.Create(AData));
  end;
...
begin
  ...
  if IsValidIntegerValue(ATileIndex) then
    AImage.Tiles[ATileIndex].AppendQuantizationParameters(AComponent,
...
constructor TdxJPXTilePacketData.Create(AStream: TStream);
begin
  inherited Create;
  FStream := AStream;
end;

destructor TdxJPXTilePacketData.Destroy;
begin
  FreeAndNil(FStream);
  inherited Destroy;
end;

procedure TdxJPXTilePacketData.Process(ATile: TdxJPXTile);
begin
  while FStream.Position < FStream.Size do
    ATile.AppendPacket(FStream);
  FreeAndNil(FStream);
end;

{ TdxJPXTileQuantizationParametersData }

constructor TdxJPXTileQuantizationParametersData.Create(AComponentIndex: Integer; AQuantizationComponentParameters: TdxJPXQuantizationComponentParameters);
begin
  inherited Create;
  FComponentIndex := AComponentIndex;
  FQuantizationComponentParameters := AQuantizationComponentParameters;
end;

procedure TdxJPXTileQuantizationParametersData.Process(ATile: TdxJPXTile);
begin
  ATile.Components[FComponentIndex].QuantizationParameters := FQuantizationComponentParameters;
end;

{ TdxJPXTileData }

constructor TdxJPXTileData.Create;
begin
  inherited Create;
  FPackets := TList<IdxJPXTileDataAction>.Create;
end;

destructor TdxJPXTileData.Destroy;
begin
  FreeAndNil(FPackets);
  inherited Destroy;
end;

procedure TdxJPXTileData.AppendPacket(AStream: TStream);
begin
  FPackets.Add(TdxJPXTilePacketData.Create(AStream));
end;

procedure TdxJPXTileData.AppendQuantizationParameters(AComponentIndex: Integer;
  AParameters: TdxJPXQuantizationComponentParameters);
begin
  FPackets.Add(TdxJPXTileQuantizationParametersData.Create(AComponentIndex, AParameters));
end;

procedure TdxJPXTileData.Process(ATile: TdxJPXTile);
var
  APacket: IdxJPXTileDataAction;
begin
  for APacket in FPackets do
    APacket.Process(ATile);
  FPackets.Clear;
end;
...
begin
  inherited Create;
  FImage := AImage;
  P := ATileIndex mod AImage.NumXTiles;
  Q := Floor(ATileIndex / AImage.NumXTiles);
...
begin
  FreeAndNil(FColorTransformation);
...
function TdxJPXTile.GetColorTransformation: TdxJPXColorTransformation;
begin
  if FColorTransformation = nil then
    FColorTransformation := TdxJPXColorTransformation.Parse(Self);
  Result := FColorTransformation;
end;
...
constructor TdxJPXPrecinct.Create(ASubBand: TdxJPXSubBand; ACodeBlockWidth, ACodeBlockHeight, X, Y, AWidth,
  AHeight, AIndex: Integer);
var
  ACodeBlockCount: Integer;
begin
  inherited Create;
  FIndex := AIndex;
  Inc(X, ASubBand.X0 div AWidth);
  Inc(Y, ASubBand.Y0 div AHeight);
  X0 := Max(ASubBand.X0, X * AWidth);
  Y0 := Max(ASubBand.Y0, Y * AHeight);
  X1 := Min(ASubBand.X1, (X + 1) * AWidth);
  Y1 := Min(ASubBand.Y1, (Y + 1) * AHeight);
  FCodeBlockHorizontalCount := Ceil(Width / ACodeBlockWidth);
  FCodeBlockVerticalCount := Ceil(Height / ACodeBlockHeight);
  ACodeBlockCount := FCodeBlockHorizontalCount * FCodeBlockVerticalCount;
  if ACodeBlockCount <> 0 then
  begin
...
var
  ACodeBlock: TdxJPXCodeBlock;
  APrecinct: TdxJPXPrecinct;
  APrecinctNumber, I, J: Integer;
begin
  APrecinctNumber := 0;
  for I := 0 to FPrecinctHighCount - 1 do
    for J := 0 to FPrecinctWideCount - 1 do
    begin
      APrecinct := TdxJPXPrecinct.Create(ASubBand, CodeBlockWidth, CodeBlockHeight, J, I, FPrecinctWidth, FPrecinctHeight, APrecinctNumber);
      Inc(APrecinctNumber);
      for ACodeBlock in ASubBand.CodeBlocks do
        if (ACodeBlock.X0 >= APrecinct.X0) and (ACodeBlock.X1 <= APrecinct.X1) and (ACodeBlock.Y0 >= APrecinct.Y0) and
          (ACodeBlock.Y1 <= APrecinct.Y1) then
          APrecinct.CodeBlocks.Add(ACodeBlock);
...
begin
  TdxPDFTask.ForEach(0, FTileComponents.Count,
    procedure(AIndex : Integer)
    begin
      AddTileComponentData(AIndex);
    end);
  Result := FData;
end;
...
procedure TdxJPXColorTransformation.Transform(var AResult: TBytes; AStartOffset, ARowWidth: Integer);
var
...
begin
  ...
  begin
    ...
    begin
      TransformColor(V, H, AResult, ADestOffset);
...
procedure TdxJPXColorTransformation.TransformLine(var AResult: TBytes; AStartOffset, V: Integer);
var
  AComponentCount, ADestinationOffset, H: Integer;
begin
  AComponentCount := Length(FComponentsData);
  ADestinationOffset := AStartOffset;
  for H := 0 to FWidth - 1 do
  begin
    TransformColor(V, H, AResult, ADestinationOffset);
    Inc(ADestinationOffset, AComponentCount);
  end;
end;
...
procedure TdxJPXGrayColorTransformation.TransformColor(V, H: Integer; const ADestination: TBytes;
  ADestinationOffset: Integer);
begin
  ADestination[ADestinationOffset] := Normalize(ComponentsData[0].Data[V * Width + H] + Shift[0]);
end;
...
begin
  ...
  FFirstComponentsData := AComponentData[0].Data;
  FSecondComponentData := AComponentData[1].Data;
  FThirdComponentData := AComponentData[2].Data;
end;

procedure TdxJPXMultipleComponentTransformation.TransformColor(V, H: Integer; const ADestination: TBytes;
  ADestinationOffset: Integer);
var
  I, AIndex, AOffset: Integer;
begin
  AIndex := V * Width + H;
  Transform(ADestination, FFirstComponentsData[AIndex], FSecondComponentData[AIndex], FThirdComponentData[AIndex], ADestinationOffset);
  AOffset := ADestinationOffset + 3;
  for I := 3 to Length(ComponentsData) - 1 do
  begin
    ADestination[AOffset] := Normalize(ComponentsData[I].Data[AIndex] + Shift[I]);
    Inc(AOffset);
  end;
end;
...
procedure TdxJPXMultipleComponentTransformation.Transform(const AData: TBytes; Y0, Y1, Y2: Single;
  ADestinationOffset: Integer);
var
  AShift, AOffset: Integer;
begin
  AShift := Shift[0];
  AOffset := ADestinationOffset;
  AData[AOffset] := Normalize(Y0 + AShift);
  AData[AOffset + 1] := Normalize(Y1 + AShift);
  AData[AOffset + 2] := Normalize(Y2 + AShift);
end;

{ TdxJPXReversibleColorTransformation }

procedure TdxJPXReversibleColorTransformation.Transform(const AData: TBytes; Y0, Y1, Y2: Single; ADestinationOffset: Integer);
var
  I1, I0, I2: Single;
begin
  I1 := Y0 - (Y1 + Y2) / 4;
  I0 := I1 + Y2;
  I2 := I1 + Y1;
  inherited Transform(AData, I0, I1, I2, ADestinationOffset);
end;
