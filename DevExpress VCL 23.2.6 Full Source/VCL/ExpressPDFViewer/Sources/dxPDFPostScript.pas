...
type
  ...
  strict private
    ...
    function GetItem(Index: Integer): TdxPDFPostScriptDictionaryEntry;
    function InternalContainsKey(const AKey: string; out AEntryIndex: Integer): Boolean;
  public
    ...
    property Items[Index: Integer]: TdxPDFPostScriptDictionaryEntry read GetItem; default;
  ...
  { TdxPDFPostScriptCopyOperator }

  TdxPDFPostScriptCopyOperator = class(TdxPDFPostScriptOperator)
  protected
    class function GetName: string; override;
  public
    procedure Execute(AInterpreter: TdxPDFPostScriptInterpreter); override;
  ...
  { TdxPDFPostScriptCviOperator }

  TdxPDFPostScriptCviOperator = class(TdxPDFPostScriptOperator)
  protected
    class function GetName: string; override;
  public
    procedure Execute(AInterpreter: TdxPDFPostScriptInterpreter); override;
  ...
  { TdxPDFPostScriptDivOperator }

  TdxPDFPostScriptDivOperator = class(TdxPDFPostScriptOperator)
  protected
    class function GetName: string; override;
  public
    procedure Execute(AInterpreter: TdxPDFPostScriptInterpreter); override;
  ...
  { TdxPDFPostScriptDTransformOperator }

  TdxPDFPostScriptDTransformOperator = class(TdxPDFPostScriptOperator)
  protected
    class function GetName: string; override;
  public
    procedure Execute(AInterpreter: TdxPDFPostScriptInterpreter); override;
  ...
  { TdxPDFPostScriptNotOperator }

  TdxPDFPostScriptNotOperator = class(TdxPDFPostScriptOperator)
  protected
    class function GetName: string; override;
  public
    procedure Execute(AInterpreter: TdxPDFPostScriptInterpreter); override;
  ...
  { TdxPDFPostScriptExpOperator }

  TdxPDFPostScriptExpOperator = class(TdxPDFPostScriptOperator)
  protected
    class function GetName: string; override;
  public
    procedure Execute(AInterpreter: TdxPDFPostScriptInterpreter); override;
  ...
  { TdxPDFPostScriptExecOperator }

  TdxPDFPostScriptExecOperator = class(TdxPDFPostScriptOperator)
  protected
    class function GetName: string; override;
  public
    procedure Execute(AInterpreter: TdxPDFPostScriptInterpreter); override;
...
uses
  Math, Variants, dxFontFile, dxPDFType1Font, dxPDFUtils, dxCore;

const
  dxThisUnitName = 'dxPDFPostScript';
...
function TdxPDFPostScriptDictionary.GetItem(Index: Integer): TdxPDFPostScriptDictionaryEntry;
begin
  Result := FItems[Index] as TdxPDFPostScriptDictionaryEntry;
end;

function TdxPDFPostScriptDictionary.InternalContainsKey(const AKey: string; out AEntryIndex: Integer): Boolean;
var
  I: Integer;
begin
  Result := False;
...
var
  I: Integer;
begin
  Result := InternalContainsKey(AKey, I);
end;
...
begin
  Result := TdxPDFUtils.ArrayToMatrix(GetObject(AKey) as TdxPDFArray).XForm;
end;
...
procedure TdxPDFPostScriptDictionary.Remove(const AKey: string);
var
  AEntryIndex: Integer;
begin
  if InternalContainsKey(AKey, AEntryIndex) then
    FItems.Delete(AEntryIndex);
end;
...
begin
  ...
  begin
    case AMode of
      2:
        AIsValidByte := (current = Zero) or (current = One);
      8:
        AIsValidByte := (current >= Zero) and (current <= Seven);
      16:
  ...
  begin
    ...
    begin
      if AObject is TdxPDFType1FontClassicFontProgram then
        AInterpreter.Stack.Push(TdxPDFType1FontClassicFontProgram(AObject).CharStrings)
      else
        TdxPDFUtils.RaiseTestException;
    end
    else
...
procedure TdxPDFPostScriptCopyOperator.Execute(AInterpreter: TdxPDFPostScriptInterpreter);
var
  AObjectCount: Integer;
  AOperand: TObject;
  AStack: TdxPDFPostScriptStack;
  AValues: TdxPDFReferencedObjects;
  I, J: Integer;
begin
  AStack := AInterpreter.Stack;
  AOperand := AStack.Pop;
  if AOperand is TdxPDFInteger then
  begin
    AObjectCount := TdxPDFInteger(AOperand).Value;
    AValues := TdxPDFReferencedObjects.Create;
    try
      for I := 0 to AObjectCount - 1 do
        AValues.Add(AStack.Pop);
      for J := 0 to 2 - 1 do
        for I := AObjectCount - 1 downto 0 do
...
class function TdxPDFPostScriptCopyOperator.GetName: string;
begin
  Result := 'copy';
end;
...
procedure TdxPDFPostScriptCviOperator.Execute(AInterpreter: TdxPDFPostScriptInterpreter);
begin
  AInterpreter.Stack.Push(TdxPDFDouble.Create(TdxPDFUtils.ConvertToInt(AInterpreter.Stack.Pop as TdxPDFBase)));
end;

class function TdxPDFPostScriptCviOperator.GetName: string;
begin
  Result := 'cvi'
end;
...
procedure TdxPDFPostScriptDivOperator.Execute(AInterpreter: TdxPDFPostScriptInterpreter);
var
  AOperand1, AOperand2: TObject;
begin
  AOperand2 := AInterpreter.Stack.Pop;
...
class function TdxPDFPostScriptDivOperator.GetName: string;
begin
  Result := 'div';
end;
...
procedure TdxPDFPostScriptDTransformOperator.Execute(AInterpreter: TdxPDFPostScriptInterpreter);
var
  ADX, ADY: TdxPDFBase;
begin
  ADY := AInterpreter.Stack.Pop as TdxPDFBase;
  ADX := AInterpreter.Stack.Pop as TdxPDFBase;
  AInterpreter.Stack.Push(TdxPDFDouble.Create(TdxPDFUtils.ConvertToDouble(ADX)));
  AInterpreter.Stack.Push(TdxPDFDouble.Create(TdxPDFUtils.ConvertToDouble(ADY)));
end;

class function TdxPDFPostScriptDTransformOperator.GetName: string;
begin
  Result := 'dtransform';
end;
...
procedure TdxPDFPostScriptNotOperator.Execute(AInterpreter: TdxPDFPostScriptInterpreter);
var
  AOperand: TdxPDFBase;
begin
  AOperand := AInterpreter.Stack.Pop as TdxPDFBase;
  if AOperand is TdxPDFBoolean then
    AInterpreter.Stack.Push(TdxPDFBoolean.Create(not TdxPDFBoolean(AOperand).Value))
  else
    if AOperand is TdxPDFInteger then
      AInterpreter.Stack.Push(TdxPDFBoolean.Create(not TdxPDFInteger(AOperand).Value));
end;

class function TdxPDFPostScriptNotOperator.GetName: string;
begin
  Result := 'not';
end;
...
procedure TdxPDFPostScriptExpOperator.Execute(AInterpreter: TdxPDFPostScriptInterpreter);
var
  AOperand, AExponent: TdxPDFBase;
begin
  AExponent := AInterpreter.Stack.Pop as TdxPDFBase;
  AOperand := AInterpreter.Stack.Pop as TdxPDFBase;
  if (AOperand is TdxPDFNumericObject) and (AExponent is TdxPDFNumericObject) then
    try
      AInterpreter.Stack.Push(TdxPDFDouble.Create(Power(
        TdxPDFNumericObjectAccess(AOperand).GetValue, TdxPDFNumericObjectAccess(AExponent).GetValue)));
    except
    end;
end;

class function TdxPDFPostScriptExpOperator.GetName: string;
begin
  Result := 'exp';
end;
...
procedure TdxPDFPostScriptExecOperator.Execute(AInterpreter: TdxPDFPostScriptInterpreter);
var
  AProc: TdxPDFReferencedObject;
begin
  AProc := AInterpreter.Stack.Pop;
  if AProc is TdxPDFArray then
    AInterpreter.Execute(TdxPDFArray(AProc))
end;

class function TdxPDFPostScriptExecOperator.GetName: string;
begin
  Result := 'exec';
end;
...
initialization
  ...
  dxPDFPostScriptOperatorFactory.RegisterOperator(TdxPDFPostScriptCopyOperator);
  dxPDFPostScriptOperatorFactory.RegisterOperator(TdxPDFPostScriptCurrentDictionaryOperator);
  dxPDFPostScriptOperatorFactory.RegisterOperator(TdxPDFPostScriptCviOperator);
  dxPDFPostScriptOperatorFactory.RegisterOperator(TdxPDFPostScriptDivOperator);
  dxPDFPostScriptOperatorFactory.RegisterOperator(TdxPDFPostScriptDTransformOperator);
  dxPDFPostScriptOperatorFactory.RegisterOperator(TdxPDFPostScriptNotOperator);
  dxPDFPostScriptOperatorFactory.RegisterOperator(TdxPDFPostScriptExpOperator);
  dxPDFPostScriptOperatorFactory.RegisterOperator(TdxPDFPostScriptExecOperator);
