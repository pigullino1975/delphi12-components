...
uses
  Types, Classes, Controls, dxActions, dxPDFViewer, dxPrinting, Windows;
...
uses
  Dialogs, dxPDFViewerActionsStrs, dxCustomPreview, dxShellDialogs, Forms;

const
  dxThisUnitName = 'dxPDFViewerActions';
...
begin
  ...
  try
    if Application.ActiveFormHandle <> 0 then
      Result := AOpenDialog.Execute(Application.ActiveFormHandle)
    else
...
begin
  ...
  FDefaultImageNameInIconLibrary := 'Actions\RotateClockwise.png';
end;
...
begin
  ...
  FDefaultImageNameInIconLibrary := 'Actions\RotateCounterclockwise.png';
end;
