...
unit dxPDFParser; // for internal use
...
uses
  SysUtils, Classes, Generics.Defaults, Generics.Collections, Math, dxHashUtils, dxPDFStreamFilter, dxPDFBase, dxPDFTypes,
  dxPDFUtils;
...
type
  ...
  strict private
    ...
    FBooleanTrueToken: TdxPDFTokenDescription;
    FBooleanFalseToken: TdxPDFTokenDescription;
    FNullToken: TdxPDFTokenDescription;

    procedure SetCurrentPosition(const AValue: Int64);
    procedure SetData(const AValue: TBytes);

    function CreateBooleanObject(AToken: TdxPDFTokenDescription; AValue: Boolean): TdxPDFBoolean;
    function GetName: string;
    function DoReadArray(out ACompleted: Boolean): TdxPDFArray; inline;
    function DoReadBooleanObject(AToken: TdxPDFTokenDescription; ADefaultValue: Boolean;
  ...
  protected
    ...
    function CreateStream(ADictionary: TdxPDFDictionary; const AData: TBytes): TdxPDFStream;
    ...
    function GetEncryptionInfo: IdxPDFEncryptionInfo; virtual;
    ...
    function IsHexDigit: Boolean; inline;
    function IsPeriod: Boolean; inline;
    ...
    function ReadReal(AInitialValue: Integer; AIsNegative: Boolean): TdxPDFNumericObject;
  ...
  public
    ...
    property Position: Int64 read FCurrentPosition;
  end;
  ...
  strict private
    FEndStreamToken: TdxPDFTokenDescription;
    FStreamToken: TdxPDFTokenDescription;
    ...
    procedure SkipStreamLeadingWhiteSpace;
    function GetStreamData(ALength: Integer): TBytes;
  private
    FRepository: TdxPDFCustomRepository;
  protected
    function CreateDictionary: TdxPDFDictionary; override;
    function GetEncryptionInfo: IdxPDFEncryptionInfo; override;
    ...
    function TryReadKnownObject: Boolean; virtual;
    function ReadHexadecimalString(AIsHexStrSeparatedByWhiteSpaces: Boolean = False): TdxPDFString; virtual;

    function DoReadData(AToken: TdxPDFTokenDescription; AIgnoreWhiteSpace: Boolean = False;
      AEndOfDataToken: TdxPDFTokenDescription = nil; AHasWhiteSpaceBeforeToken: Boolean = True): TBytes; overload;
    function DoReadData(ALength: Integer; AToken: TdxPDFTokenDescription): TBytes; overload;
  ...
  private
    FCheckObjToken: TdxPDFTokenDescription;
    FEndObjToken: TdxPDFTokenDescription;
    FObjToken: TdxPDFTokenDescription;
    FStream: TStream;
  protected
  ...
  public
    ...
    function IsValidReference(AReference: TdxPDFReference): Boolean;
    function ReadToken(AToken: TdxPDFTokenDescription): Boolean;
    ...
    function TryReadObjectHeader(AOffset: Int64; out ANumber, AGeneration: Integer): Boolean;
    procedure SkipSpaces(out ASymbol: Byte); overload;
  end;

  TdxPDFCrossReferenceIndexDescription = record // for internal use
  public
    StartNumber: Integer;
    Count: Integer;
  end;

  TdxPDFCrossReferenceIndexDescriptionList = class(TList<TdxPDFCrossReferenceIndexDescription>);
  ...
  strict private
    ...
    FIsNegativePositionOffset: Boolean;
    FStartPosition: Int64;
    FStartXRefObjectToken: TdxPDFTokenDescription;

    function ReadValue(const AData: TBytes; AWeight: Integer; var APosition: Int64): Integer;
    procedure Decode(const AData: TBytes; AIndexes: TdxPDFCrossReferenceIndexDescriptionList;
    ...
    procedure PopulateIndexes(ATrailer: TdxPDFDictionary; AIndexes: TdxPDFCrossReferenceIndexDescriptionList);
  protected
    procedure InitializeTokens; override;
    procedure FinalizeTokens; override;
  public
    ...
    function FindToken(AToken: TdxPDFTokenDescription): Boolean; overload;
    ...
    function TryFindObjectOffset(ANumber, AGeneration: Integer; out AOffset: Int64): Boolean;
    procedure CalculateHeaderPosition;
    procedure FindObjects(AProc: TProc<Integer>); overload;
    procedure FindObjects; overload;
...
uses
  Variants, Character, StrUtils, Windows, dxCore, dxStringHelper, dxPDFCore;

const
  dxThisUnitName = 'dxPDFParser';
...
function TdxPDFBaseParser.CreateStream(ADictionary: TdxPDFDictionary; const AData: TBytes): TdxPDFStream;
begin
  Result := TdxPDFStream.Create(AData, ADictionary, GetEncryptionInfo);
end;
...
function TdxPDFBaseParser.GetEncryptionInfo: IdxPDFEncryptionInfo;
begin
  Result := nil;
end;
...
begin
  FBooleanTrueToken := TdxPDFTokenDescription.Create(TdxPDFUtils.StrToByteArray('true'));
  FBooleanFalseToken := TdxPDFTokenDescription.Create(TdxPDFUtils.StrToByteArray('false'));
  FNullToken := TdxPDFTokenDescription.Create(TdxPDFUtils.StrToByteArray('null'));
end;
...
begin
  case Current of
    TdxPDFDefinedSymbols.Minus:
      begin
        AIsNegative := True;
        if not ReadNext or (not IsDigit and not IsPeriod and ((Current <> TdxPDFDefinedSymbols.Minus) or not ReadNext)) then
          Exit(nil)
      end;
  ...
  AValue := 0;
  if IsDigit then
    for I := CurrentPosition to FDataLength - 1 do
    begin
  ...
  if IsPeriod then
    Result := ReadReal(AValue, AIsNegative)
  else
    Result := TdxPDFInteger.Create(IfThen(AIsNegative, -1, 1) * AValue);
end;
      ...
      if IsDigit then
        ...
        case Current of
          ...
          TdxPDFDefinedSymbols.StartArray:
            begin
              ACurrentPosition := CurrentPosition;
              Result := DoReadArray(ACompletedArray);
              if not ACompletedArray then
              begin
                dxPDFFreeObject(Result);
                Result := nil;
                CurrentPosition := ACurrentPosition + 1;
              end;
            end;
          TdxPDFDefinedSymbols.EndArray, TdxPDFDefinedSymbols.EndString:
...
function TdxPDFBaseParser.IsPeriod: Boolean;
begin
  Result := TdxPDFUtils.IsPeriod(FCurrent);
end;
...
begin
  Data := AData;
  Result := DoReadArray(ACompleted);
end;

function TdxPDFBaseParser.ReadInteger: Integer;
var
  AObject: TdxPDFBase;
begin
  AObject := DoReadNumber;
  try
...
function TdxPDFBaseParser.ReadReal(AInitialValue: Integer; AIsNegative: Boolean): TdxPDFNumericObject;

  function CheckResultValue(AValue: Single): Single;
  const
    SingleMaxValue: Single = 340282346638528859811704183484516925440.0;
    SingleMinValue: Single = -340282346638528859811704183484516925440.0;
  begin
    if (AValue > SingleMaxValue) or (AValue < SingleMinValue) then
      Result := 0
    else
      Result := dxPDFRoundFloat(AValue);
   end;

const
  SignMap: array[Boolean] of Integer = (1, -1);
var
  AValue, AFraction, APow: Single;
...
begin
  ...
  begin
    ...
    AValue := AValue + AFraction;
    if Current = TdxPDFDefinedSymbols.Minus then
    begin
      while ReadNext and (IsDigit or IsPeriod or
        (Current = TdxPDFDefinedSymbols.Minus) or (Current = TdxPDFDefinedSymbols.Plus)) do;
      Exit(TdxPDFDouble.Create(SignMap[AIsNegative] * AValue));
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      if (AExponent = nil) or (AExponent <> nil) and (AExponent.ObjectType <> otInteger) then
        ...
        try
          if TdxPDFInteger(AExponent).Value > 38 then
            AValue := 0
          else
            AValue := CheckResultValue(SignMap[AIsNegative] * AValue * Power(10, TdxPDFInteger(AExponent).Value));
  ...
  Result := TdxPDFDouble.Create(CheckResultValue(SignMap[AIsNegative] * AValue));
end;
...
function TdxPDFBaseParser.CreateBooleanObject(AToken: TdxPDFTokenDescription; AValue: Boolean): TdxPDFBoolean;
begin
  Result := nil;
...
function TdxPDFBaseParser.DoReadArray(out ACompleted: Boolean): TdxPDFArray;
var
...
function TdxPDFBaseParser.DoReadBooleanObject(AToken: TdxPDFTokenDescription; ADefaultValue: Boolean;
...
begin
  Result := TdxPDFDocumentRepository(Repository).CreateDictionary;
end;

function TdxPDFStructureParser.GetEncryptionInfo: IdxPDFEncryptionInfo;
begin
  if not Supports(TdxPDFDocumentRepository(Repository).EncryptionInfo, IdxPDFEncryptionInfo, Result) then
    Result := inherited GetEncryptionInfo;
end;
...
var
  ...
  APosition: Int64;
begin
  ATempNumeric := Safe<TdxPDFNumericObject>.Cast(DoReadNumber);
  if (ATempNumeric <> nil) and (ATempNumeric.ObjectType = otInteger) then
  begin
...
begin
  ...
  FStreamToken := TdxPDFTokenDescription.Create(TdxPDFUtils.StrToByteArray('stream'));
  FEndStreamToken := TdxPDFTokenDescription.Create(TdxPDFUtils.StrToByteArray('endstream'));
end;
...
function TdxPDFStructureParser.TryReadKnownObject: Boolean;
begin
  Result := True;
end;
...
function TdxPDFStructureParser.DoReadData(AToken: TdxPDFTokenDescription; AIgnoreWhiteSpace: Boolean = False;
  AEndOfDataToken: TdxPDFTokenDescription = nil; AHasWhiteSpaceBeforeToken: Boolean = True): TBytes;

  function ReadRange(ACount: Integer; out ARange: TBytes): Integer;
  begin
    if CurrentPosition + ACount > DataLength then
      Result := DataLength - CurrentPosition
    else
      Result := ACount;
    SetLength(ARange, Result);
    TdxPDFUtils.CopyData(Data, CurrentPosition, ARange, 0, Result);
  end;

  function FindTokenOffset(const AToken, ABuffer: TBytes; AOffset, ALimit: Integer): Integer;
  var
    I, AState, ATokenLength: Integer;
    ACurrent: Byte;
  begin
    AState := 0;
    ATokenLength := Length(AToken);
    for I := AOffset to ALimit - 1 do
    begin
      ACurrent := ABuffer[I];
      if ACurrent = AToken[AState] then
      begin
        Inc(AState);
        if AState = ATokenLength then
          Exit(I + 1);
      end
      else
        if ACurrent = AToken[0] then
          AState := 1;
    end;
    Result := -1;
  end;
...
var
  I, AActualSize, AEndMarkerPosition, ATokenLength, ALength, AOffset, ALimit: Integer;
  AData: TBytes;
begin
    ...
    begin
      ...
      begin
        if AIgnoreWhiteSpace then
          Break;
        SetLength(AData, 10);
        AActualSize := ReadRange(10, AData);
        if FindTokenOffset(AToken.Sequence, AData, 0, AActualSize) = -1 then
        begin
          if AEndOfDataToken <> nil then
          begin
            AOffset := Max(0, Length(Result) - 15);
            ALimit := Min(Length(Result), AOffset + 15);
            AOffset := FindTokenOffset(AEndOfDataToken.Sequence, Result, AOffset, ALimit);
            if AOffset <> -1 then
            begin
              if AOffset < Length(Result) then
                TdxPDFUtils.DeleteData(AOffset, Length(Result) - AOffset, Result);
              Exit;
            end;
          end;
          if TryReadKnownObject then
            Break;
        end;
      end;
      AEndMarkerPosition := 0;
    end;
    if not ReadNext and ((AEndMarkerPosition = ATokenLength) or IsEOF) then
    begin
      TdxPDFUtils.AddByte(Current, Result);
      Break;
    end;
  end;
  ALength := Length(Result) - ATokenLength - 1 - Ord(AHasWhiteSpaceBeforeToken);
  SetLength(Result, Max(ALength, 0));
end;

function TdxPDFStructureParser.DoReadData(ALength: Integer; AToken: TdxPDFTokenDescription): TBytes;
begin
  ...
  begin
    ...
    CurrentPosition := CurrentPosition + ALength + Length(AToken.Sequence) + 1;
    SkipSpaces;
  end;
end;
...
begin
  ...
  begin
    ...
    FRepository := AValue as TdxPDFDocumentRepository;
  end;
end;
...
begin
  ...
  begin
    ...
    if ALengthObject <> nil then
      ...
      ALength := dxPDFInvalidValue;
    Result := CreateStream(ADictionary, GetStreamData(ALength));
  end
  else
    Result := nil;
end;
...
begin
  Result := CreateStream(nil, GetStreamData(ALength));
end;
...
procedure TdxPDFStructureParser.SkipStreamLeadingWhiteSpace;
begin
...
function TdxPDFStructureParser.GetStreamData(ALength: Integer): TBytes;

  function ReadToEndStreamToken: TBytes;
  var
    L: Int64;
    AData: TBytes;
  begin
    L := 0;
    try
      AData := DoReadData(FEndStreamToken, True);
      L := Length(AData);
    finally
      Result := Copy(AData, 0, L);
      SetLength(AData, 0);
    end;
  end;
...
function TdxPDFObjectParser.IsValidReference(AReference: TdxPDFReference): Boolean;
var
  ANumber, AGeneration: Integer;
begin
  Result := CanReadNumber(AReference.Offset) and TryReadObjectHeader(AReference.Offset, ANumber, AGeneration) and
    (ANumber = AReference.Number) and (AGeneration = AReference.Generation);
end;

function TdxPDFDocumentParser.ReadCrossReferences(AOffset: Integer): Boolean;

  procedure FindXRefStartPositionBeforeOffset;
  var
    APosition, ADelta: Int64;
    ACrossReferencesToken: TdxPDFTokenDescription;
  begin
    ACrossReferencesToken := TdxPDFTokenDescription.Create(TdxPDFUtils.StrToByteArray('xref'));
    try
      ADelta := 0;
  ...
  procedure FindXRefStartPositionAfterOffset;
  var
    APosition, ADelta: Int64;
    ACrossReferencesToken: TdxPDFTokenDescription;
  begin
    ...
    try
      ADelta := 0;
      if AOffset + StartXRefOffset < FStream.Size then
        while not Result and (FStream.Position < AOffset + StartXRefOffset) do
        begin
          FStream.Position := FStream.Position + 1;
          APosition := FStream.Position;
          ...
          if not Result then
            ...
            ADelta := FStream.Position - AOffset - Length(ACrossReferencesToken.Sequence);
        end;
      FIsNegativePositionOffset := False;
      FPositionOffset := FPositionOffset + ADelta;
    finally
      ACrossReferencesToken.Free;
    end;
...
begin
  FStream.Position := AOffset;
  ACrossReferencesToken := TdxPDFTokenDescription.Create('xref');
  try
    Result := ReadToken(ACrossReferencesToken);
    FindXRefStartPositionBeforeOffset;
    if not Result then
      FindXRefStartPositionAfterOffset;
    while Result do
    begin
      Result := True;
      APosition := FStream.Position;
...
begin
  Result := TdxPDFUtils.ReadLine(FStream);
end;

function TdxPDFObjectParser.TryReadObjectHeader(AOffset: Int64; out ANumber, AGeneration: Integer): Boolean;
var
  AObjToken: TdxPDFTokenDescription;
begin
  SaveCurrentPosition;
  AObjToken := TdxPDFTokenDescription.Create(TdxPDFUtils.StrToByteArray('obj'));
  try
    FStream.Position := AOffset;
    try
      ANumber := ReadNumber;
      AGeneration := ReadNumber;
      Result := ReadToken(AObjToken);
    except
      Result := False;
    end;
  finally
...
begin
  AToken := TdxPDFTokenDescription.Create(ATokenDescription);
  try
...
function TdxPDFDocumentParser.FindToken(AToken: TdxPDFTokenDescription): Boolean;
begin
  Result := False;
  while not IsEOF do
  begin
...
function TdxPDFDocumentParser.TryFindObjectOffset(ANumber, AGeneration: Integer; out AOffset: Int64): Boolean;

  function FindReference(const AReferenceAsString, ABuffer: string; out AReferenceOffset: Int64): Boolean;
  begin
    AOffset := PosEx(AReferenceAsString, ABuffer) - 1;
    Result := AReferenceOffset > 0;
  end;
...
var
  ABuffer: AnsiString;
  ABufferAsString, APattern: string;
  AReferenceAsString1, AReferenceAsString2: string;
  AActualReadCount, ABufferOffset, AReferenceAsStringLength: Integer;
begin
  Result := False;
  if FStream = nil then
    Exit;
  APattern := IntToStr(ANumber) + ' ' + IntToStr(AGeneration) + ' obj';
  AReferenceAsString1 := #10 + APattern;
  AReferenceAsString2 := #13 + APattern;
  AReferenceAsStringLength := Length(AReferenceAsString1);
  dxTestCheck(AReferenceAsStringLength = Length(AReferenceAsString2), '');

  SaveCurrentPosition;
  try
    ABufferOffset := 0;
    FStream.Position := ABufferOffset;
    SetLength(ABuffer, BufferSize);
    while not IsEOF do
    begin
      AActualReadCount := FStream.Read(ABuffer[1], BufferSize);
      ABufferAsString := dxAnsiStringToString(ABuffer);
      Result := FindReference(AReferenceAsString1, ABufferAsString, AOffset) or FindReference(AReferenceAsString2, ABufferAsString, AOffset);
      if not Result then
      begin
        Inc(ABufferOffset, AActualReadCount - AReferenceAsStringLength);
        if IsEOF then
          Break;
        FStream.Position := FStream.Position - AReferenceAsStringLength;
      end
      else
      begin
        AOffset := ABufferOffset + AOffset + 1;
        Break;
      end;
    end;
  finally
...
var
  AData: TBytes;
  ASymbol: Byte;
  ACrossReferencesStartToken, ATrailerToken: TdxPDFTokenDescription;
begin
  SetLength(Result, 0);
  ATrailerToken := TdxPDFTokenDescription.Create(TdxPDFKeywords.Trailer);
  try
    begin
      ACrossReferencesStartToken := TdxPDFTokenDescription.Create(TdxPDFKeywords.StartXRef);
      try
        while not IsEOF do
        begin
          ASymbol := ReadByte;
...
procedure TdxPDFDocumentParser.FindObjects(AProc: TProc<Integer>);
var
  AData: Byte;
  AStartXRefPosition, APosition: Int64;
  AEOFToken, AEndObjToken, AObjToken, ACrossReferencesToken: TdxPDFTokenDescription;
begin
  FStream.Position := 0;

  ACrossReferencesToken := TdxPDFTokenDescription.Create(TdxPDFUtils.StrToByteArray('xref'));
  AObjToken := TdxPDFTokenDescription.Create(TdxPDFUtils.StrToByteArray('obj'));
  AEndObjToken := TdxPDFTokenDescription.Create(TdxPDFUtils.StrToByteArray('endobj'));
  AEOFToken := TdxPDFTokenDescription.Create(TdxPDFUtils.StrToByteArray('%%EOF'));
        ...
        begin
          ...
          begin
            AStartXRefPosition := FStream.Position;
              if not FindToken(AEOFToken) then
              Break
            else
              Continue;
          end
...
var
  AActualOffset: Integer;
  ATrailerData: TBytes;
  ATrailerToken: TdxPDFTokenDescription;
begin
  AActualOffset := Max(AOffset - FPositionOffset, 0);
  if ReadCrossReferences(AActualOffset) then
  begin
    ...
    ReadTrailerStream(AOffset, ATrailer);
end;
...
begin
  inherited InitializeTokens;
  FCheckObjToken := TdxPDFTokenDescription.Create(TdxPDFUtils.StrToByteArray(' obj'));
  FObjToken := TdxPDFTokenDescription.Create(TdxPDFUtils.StrToByteArray('obj'));
  FEndObjToken := TdxPDFTokenDescription.Create(TdxPDFUtils.StrToByteArray('endobj'));
end;

procedure TdxPDFObjectParser.FinalizeTokens;
begin
  FreeAndNil(FEndObjToken);
  FreeAndNil(FObjToken);
...
begin
  inherited InitializeTokens;
  FIsNegativePositionOffset := True;
  FStartXRefObjectToken := TdxPDFTokenDescription.Create(TdxPDFUtils.StrToByteArray(TdxPDFKeywords.StartXRef));
end;
...
begin
  ...
  ANumber := ReadNumber;
  AGeneration := ReadNumber;
  if ReadToken(FObjToken) then
  begin
    FCheckObjToken.Reset;
    FEndObjToken.Reset;
    ANested := False;
    SetLength(Result, 0);
    while not IsEOF do
    begin
      ...
      ASymbol := Byte(ANext);
      TdxPDFUtils.AddByte(ASymbol, Result);
      if FEndObjToken.Compare(ASymbol) then
        if ANested then
        begin
          FEndObjToken.Reset;
          ANested := False;
        ...
        begin
          ATokenLength := Length(FEndObjToken.Sequence);
          TdxPDFUtils.DeleteData(Length(Result) - ATokenLength, ATokenLength, Result);
          Break;
        end;
      if FCheckObjToken.Compare(ASymbol) then
      begin
...
function TdxPDFObjectParser.ReadToken(AToken: TdxPDFTokenDescription): Boolean;
var
  I: Integer;
  ASymbol: Byte;
begin
...
begin
  ...
  begin
    AOffset := ReadObjectParameter(dxcXReferenceObjectOffsetDimension) - IfThen(FIsNegativePositionOffset, FPositionOffset);
    AGeneration := ReadObjectParameter(dxcXReferenceObjectGenerationDimension);
    ANext := ReadByte;
    if (AStartNumber = 1) and (I = 0) and (AOffset = 0) and (AGeneration = 65535) and (ANext = Byte('f')) then
      Dec(ANumber);
    case ANext of
      Byte('n'):
...
begin
  Result := dxPDFInvalidValue;
  FStartXRefObjectToken.Reset;
  repeat
    if FStartXRefObjectToken.Compare(Current) then
...
var
  ...
  AEOFToken: TdxPDFTokenDescription;
...
begin
  ...
  AEOFToken := TdxPDFTokenDescription.Create(TdxPDFUtils.StrToByteArray('%%EOF'));
  try
    AStreamSize := FStream.Size;
    FStream.Position := IfThen(AStreamSize > StartXRefOffset, AStreamSize - StartXRefOffset, AStreamSize);
    AOffset := -1;
...
var
  ATypeSize, AOffsetSize, AGenerationSize: Integer;
  AIndexes: TdxPDFCrossReferenceIndexDescriptionList;
  AObject: TdxPDFBase;
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        AIndexes := TdxPDFCrossReferenceIndexDescriptionList.Create;
        try
...
procedure TdxPDFDocumentParser.Decode(const AData: TBytes; AIndexes: TdxPDFCrossReferenceIndexDescriptionList;
  ATypeSize, AOffsetSize, AGenerationSize: Integer);
var
  APosition: Int64;
