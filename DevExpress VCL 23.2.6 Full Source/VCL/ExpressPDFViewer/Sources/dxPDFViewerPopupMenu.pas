...
uses
  ...
  dxPDFCore, dxPDFViewer, cxLookAndFeels;
...
type
 ...
 { TdxPDFViewerPagePopupMenu }
  ...
  protected
    ...
    function GetRotatePagesCaption: Pointer; virtual;
...
uses
  ...
  dxPDFViewerDialogsStrs, dxPDFViewerAnnotations;

const
  dxThisUnitName = 'dxPDFViewerPopupMenu';

{$R dxPDFViewerPopupMenu.res}
{$R dxPDFViewerPopupMenu_svg.res}
...
type
  ...
  TdxPDFViewerDocumentHitTestAccess = class(TdxPDFViewerDocumentHitTest);
  ...
  begin
    ...
    try
      if dxUseVectorIcons then
        ...
        AImage.LoadFromResource(HInstance, AResourceName, 'PNG');
      Result := ImageList.Add(AImage);
    finally
      AImage.Free;
    end;
  end;
...
begin
  Result := TdxPDFViewerDocumentHitTestAccess(Viewer.HitTest).GetAttachment;
end;
...
begin
  ...
  AddMenuItem(GetRotatePagesCaption, RotatePagesID);
  AddSeparator;
  if TdxPDFViewerAccess(Viewer).CanPrint then
  begin
    AddMenuItem(GetPrintPagesCaption, PrintPagesCommandID);
...
begin
  case ACommand of
    ...
    RotatePagesID:
      TdxPDFViewerAccess(Viewer).Thumbnails.RotatePages;
  end;
end;
...
function TdxPDFViewerThumbnailsPopupMenu.GetRotatePagesCaption: Pointer;
begin
  Result := @sdxPDFViewerThumbnailPopupMenuRotatePages;
end;
