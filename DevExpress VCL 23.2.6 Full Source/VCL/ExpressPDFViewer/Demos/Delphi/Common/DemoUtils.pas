...
procedure HandleLookAndFeelChangeCommand(ASender: TObject; ALookAndFeelController: TcxLookAndFeelController);
begin
  case (ASender as TComponent).Tag of
    0..3:
      begin
...
begin
  Result := AItem.Action as TAction;
end;
...
begin
{$IFDEF EXPRESSBARS}
  for I := 0 to (AParent as TdxBarSubItem).ItemLinks.Count - 1 do
  begin
    AComponent := TdxBarSubItem(AParent).ItemLinks[I].Item;
{$ELSE}
  for I := 0 to (AParent as TMenuItem).Count - 1 do
  begin
  ...
  begin
    ...
    begin
      if AMenuItem.Count = 0 then
        ABarItem := FBarManager.AddButton
      else
      begin
        ABarItem := FBarManager.AddSubItem;
      ...
      while Count > 0 do
        ...
        begin
          ...
          if Parent = nil then
            ...
            AItemLink := (ABarItem as TdxBarSubItem).ItemLinks.Add;
