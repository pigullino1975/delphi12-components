...
const
  dxThisUnitName = 'dxRichEdit.View.Simple';
...
type
  ...
  strict private
    function GetView: TdxSimpleView; inline;
  protected
...
begin
  Result := Max(VisibleHeight, Round(ViewPortBounds.Height / View.ZoomFactor));
end;
...
begin
  ...
  AMaxWidth := TdxMaxWidthCalculator.GetMaxWidth(APage);
...
begin
  ...
  try
    ...
    AWidth := TdxMaxWidthCalculator.GetMaxWidth(DocumentLayout.Pages[0]);
    AHeight := DocumentLayout.Pages[0].Bounds.Height;
    ACount := DocumentLayout.Pages.Count;
    for I := 1 to ACount - 1 do
    begin
      AWidth := Max(TdxMaxWidthCalculator.GetMaxWidth(DocumentLayout.Pages[I]), AWidth);
