...
unit dxRichEdit.Import.OpenXML.DestinationHeaderFooter; // for internal use
...
const
  dxThisUnitName = 'dxRichEdit.Import.OpenXML.DestinationHeaderFooter';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxHeaderFooterDestination<T>.Initialize', SysInit.HInstance);{$ENDIF}
  FHandlerTable := CreateElementHandlerTable;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxHeaderFooterDestination<T>.Initialize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxHeaderFooterDestination<T>.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FHandlerTable);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxHeaderFooterDestination<T>.Finalize', SysInit.HInstance);{$ENDIF}
end;
