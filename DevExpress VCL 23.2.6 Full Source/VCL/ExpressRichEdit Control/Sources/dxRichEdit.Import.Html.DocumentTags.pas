...
unit dxRichEdit.Import.Html.DocumentTags; // for internal use
...
type
  ...
  protected
    ...
    procedure FindNamescpaceInAttribute;
  public
...
uses
  SysUtils, Classes, RegularExpressions, StrUtils,
...
const
  dxThisUnitName = 'dxRichEdit.Import.Html.DocumentTags';
...
procedure TdxHtmlTag.FindKeywordInAttributeTable;
begin
  inherited FindKeywordInAttributeTable;
  FindNamescpaceInAttribute;
end;

procedure TdxHtmlTag.FindNamescpaceInAttribute;
var
  AAttribute: TdxAttribute;
  I, ANamespaceDelimiterIndex: Integer;
  ANamespace: string;
begin
  for I := 0 to Tag.Attributes.Count - 1 do
  begin
    AAttribute := Tag.Attributes[I];
    ANamespaceDelimiterIndex := TdxStringHelper.IndexOf(AAttribute.Name, 'XMLNS:');
    if ANamespaceDelimiterIndex >= 0 then
    begin
      ANamespace := TdxStringHelper.Substring(AAttribute.Name, ANamespaceDelimiterIndex + Length('XMLNS:'));
      TdxHtmlImporter(Importer).HtmlNamespaces.AddOrSetValue(ANamespace, AAttribute.Value);
    end;
  end;
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxLinkTag.Initialize', SysInit.HInstance);{$ENDIF}
  FAttributeTable := AddAttributes;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxLinkTag.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxLinkTag.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxLinkTag.Finalize', SysInit.HInstance);{$ENDIF}
  FAttributeTable.Free;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxLinkTag.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  ...
  begin
    ...
    AStream := AStreamService.GetStream(AUri, 0);
    if AStream <> nil then
    try
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxMetaTag.Initialize', SysInit.HInstance);{$ENDIF}
  FAttributeTable := AddAttributes;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxMetaTag.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxMetaTag.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxMetaTag.Finalize', SysInit.HInstance);{$ENDIF}
  FAttributeTable.Free;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxMetaTag.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxBodyTag.Initialize', SysInit.HInstance);{$ENDIF}
  FAttributeTable := AddAttributes;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxBodyTag.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxBodyTag.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxBodyTag.Finalize', SysInit.HInstance);{$ENDIF}
  FAttributeTable.Free;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxBodyTag.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
function GetOfficeTranslatorTable: TdxNamespaceTagInfos;
begin
  Result := TdxNamespaceTagInfos.Create;
  Result.Add('P', TdxNamespaceTagInfo.Create(TdxHtmlTagNameID.Paragraph, TdxOfficeParagraphTag));
end;


initialization
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.InitializationStarted(dxThisUnitName, SysInit.HInstance);{$ENDIF}
  TdxNamespaceTag.PopulateNamespaceTranslatorTable('urn:schemas-microsoft-com:office:office', GetOfficeTranslatorTable);
