...
type
  ...
  TdxRichEditControlPadding = class(TcxOffsets);
  ...
  TdxDefaultCommandUIState = class(TInterfacedObject, IdxCommandUIState) // for internal use
  private
    FIsEnabled: Boolean;
    FIsChecked: Boolean;
    FIsVisible: Boolean;
  ...
  private
    function GetActivePieceTable: TdxPieceTable; inline;
    function GetActiveViewType: TdxRichEditViewType; inline;
    function GetDocumentModel: TdxDocumentModel; inline;
    function GetInnerControl: IdxInnerControl; inline;
    function GetOptions: TdxRichEditControlOptionsBase; inline;
  protected
  ...
  TdxMouseCursorCalculator = record
  private
  ...
  public
    ...
    function Calculate(AHitTestResult: TdxRichEditHitTestResult; const APhysicalPoint: TPoint): TCursor;
  ...
  ['{496D044E-9CE8-4C97-ADB4-BDE4215693C2}']
    ...
    function CreateCopySelectionManager: TdxCopySelectionManager; // for internal use
  ...
  IdxRichEditControl = interface(IdxBatchUpdatable) 
  ...
  strict private
    ...
    FRichEditDocumentFormats: TdxRichEditDocumentFormats;
  private
  ...
  protected
    ...
    property RichEditDocumentFormats: TdxRichEditDocumentFormats read FRichEditDocumentFormats write FRichEditDocumentFormats;
  public
  ...
  public
    ...
    function GetInputPosition: TdxInputPosition; virtual; // for internal use
    function TryGetInputPosition: TdxInputPosition; virtual; // for internal use
  ...
  TdxExplicitCaretPosition = class(TdxCaretPosition) // for internal use
  strict private
    FLogPosition: TdxDocumentLogPosition;
  protected
    function GetLogPosition: Integer; override;
  ...
  TdxExplicitHeaderFooterCaretPosition = class(TdxHeaderFooterCaretPosition) // for internal use
  strict private
    FLogPosition: TdxDocumentLogPosition;
  protected
    function GetLogPosition: TdxDocumentLogPosition; override;
  ...
  public
    ...
    property IsBlinking: Boolean read FBlinking write FBlinking;
  ...
  TdxDocumentSelectionLayout = class(TcxIUnknownObject, IdxSelectionLayoutItem) // for internal use
  protected
  ...
  public
    ...
    function CreateDocumentLayoutExporter(APainter: TdxPainter; AAdapter: TdxGraphicsDocumentLayoutExporterAdapter;
      APageViewInfo: TdxPageViewInfo; const ABounds: TRect): TdxDocumentLayoutExporter; virtual; abstract; // for internal use
  ...
  TdxScrollBarAdapter = class abstract (TcxIUnknownObject, IdxBatchUpdateHandler, IdxBatchUpdatable)
  private
  ...
  public
    constructor Create(ADocumentServer: TObject);
...
const
  dxThisUnitName = 'dxRichEdit.View.Core';

type

  TdxInnerRichEditControlAccess = class(TdxInnerRichEditControl);
  TdxInnerRichEditDocumentServerAccess = class(TdxInnerRichEditDocumentServer);
  ...
  protected
    ...
    procedure TryToAddTextBoxFrameSelectionLayout(AStart: TdxDocumentLayoutPosition; AEnd: TdxDocumentLayoutPosition);
  public
...
var
  APieceTable: TdxPieceTable;
  ANormalizedStart, ANormalizedEnd: PdxDocumentModelPosition;
  ALastParagraph: TdxParagraph;
begin
  APieceTable := ASelectionItem.PieceTable;
  Result := TdxSelectionItem.Create(APieceTable);
  ANormalizedStart := ASelectionItem.Interval.NormalizedStart;
  ANormalizedEnd := ASelectionItem.Interval.NormalizedEnd;
  Result.Start := APieceTable.Paragraphs[ANormalizedStart.ParagraphIndex].LogPosition;
  ALastParagraph := APieceTable.Paragraphs[ANormalizedEnd.ParagraphIndex];
  if (ANormalizedStart.ParagraphIndex <> ALastParagraph.Index) and (ALastParagraph.LogPosition = ANormalizedEnd.LogPosition) then
    ALastParagraph := APieceTable.Paragraphs[ALastParagraph.Index - 1];
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        Result := TdxRectangularObjectSelectionLayout.CreateResizableLayout(View, ABox, ASelection.NormalizedStart, PieceTable);
...
begin
  ...
  begin
    ...
    begin
      ...
      Result := TdxRectangularObjectSelectionLayout.CreateResizableLayout(View, AInlinePicture, ALogPos, PieceTable);
    end
  end;
end;
...
var
  ...
  ABox: TdxFloatingObjectBox;
  AResult: TdxRectangularObjectSelectionLayout;
begin
  if Safe.Cast(DocumentModel.ActivePieceTable.ContentType, TdxTextBoxContentType, ATextBoxContentType) then
  begin
    ...
    begin
      ...
      begin
        ...
        AResult := TdxRectangularObjectSelectionLayout.CreateResizableLayout(View, ABox, ALogPosition, TdxPieceTable(ATextBoxContentType.PieceTable));
...
begin
  ...
  APainter.ClipBounds := GetNewClientBounds(AActualClipBounds.DeflateToTRect, AOldClipBounds);
...
begin
  ...
  FGraphics.PageUnit := TdxGraphicUnit.guPixel;
...
begin
  ...
  AHalfOfHorizontalPageGap := PageViewInfoGenerator.HorizontalPageGap div 2;
...
begin
  ...
  Result := (ABox.DocumentLayout <> nil) and Safe.Cast(ARun.Content, TdxTextBoxFloatingObjectContent, AContent) and
...
procedure TdxRichEditView.ControlResizeDelegate;
begin
...
begin
  ...
  begin
    ...
    if Safe.Cast(ALastRow, TdxTableCellRow, ATableCellRow) then
    begin
...
begin
  ...
  AComparable := TdxBoxLogPositionComparable.Create(DocumentFormatter.DocumentModel.MainPieceTable, FSecondaryLayoutStart.LogPosition);
  if not TdxAlgorithms1<TdxBoxBase>.BinarySearch(APages, AComparable.BoxAndLogPositionCompare, AFirstPageIndex) then
    AFirstPageIndex := 0;

  AComparable := TdxBoxLogPositionComparable.Create(DocumentFormatter.DocumentModel.MainPieceTable, FSecondaryLayoutEnd.LogPosition);
  if not TdxAlgorithms1<TdxBoxBase>.BinarySearch(APages, AComparable.BoxAndLogPositionCompare, ALastPageIndex) then
...
constructor TdxCopySelectionManager.Create(ADocumentServer: TObject);
begin
  inherited Create;
  FDocumentServer := ADocumentServer{$IFDEF DEBUG} as TdxInnerRichEditDocumentServer{$ENDIF};
...
begin
  TdxInnerRichEditDocumentServerAccess(FDocumentServer).OnAfterExport(Sender);
end;

procedure TdxCopySelectionManager.DoBeforeExport(Sender: TObject; E: TdxBeforeExportEventArgs);
begin
  TdxInnerRichEditDocumentServerAccess(FDocumentServer).OnBeforeExport(Sender, E);
end;

procedure TdxCopySelectionManager.DoCalculateDocumentVariable(Sender: TObject; E: TdxCalculateDocumentVariableEventArgs);
begin
  TdxInnerRichEditDocumentServerAccess(FDocumentServer).OnCalculateDocumentVariable(Sender, E);
end;
