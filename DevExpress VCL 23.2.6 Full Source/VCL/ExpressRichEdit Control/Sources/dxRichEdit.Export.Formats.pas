...
unit dxRichEdit.Export.Formats; // for internal use
...
const
  dxThisUnitName = 'dxRichEdit.Export.Formats';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxExportFileFormats.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FExportFileFormats);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxExportFileFormats.Finalize', SysInit.HInstance);{$ENDIF}
end;
