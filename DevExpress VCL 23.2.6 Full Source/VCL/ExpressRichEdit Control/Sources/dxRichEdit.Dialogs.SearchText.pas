...
const
  dxThisUnitName = 'dxRichEdit.Dialogs.SearchText';
...
begin
  Result := dxMessageBox(AMessage, Application.Title, AFlags);
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxRichEditSearchTextDialogForm.Finalize', SysInit.HInstance);{$ENDIF}
  FSearchStrings.Free;
  FReplaceStrings.Free;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxRichEditSearchTextDialogForm.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxRichEditSearchTextDialogForm.Initialize', SysInit.HInstance);{$ENDIF}
  FSearchStrings := TStringList.Create;
  FReplaceStrings := TStringList.Create;
  FFormPreviousLocation.Init(MinInt, MinInt);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxRichEditSearchTextDialogForm.Initialize', SysInit.HInstance);{$ENDIF}
end;
