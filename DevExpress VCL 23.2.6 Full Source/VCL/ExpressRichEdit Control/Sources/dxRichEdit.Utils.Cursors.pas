...
uses
  System.UITypes,
  cxLibraryConsts, dxDPIAwareUtils,
  dxRichEdit.Utils.Types, dxCore;

const
  dxThisUnitName = 'dxRichEdit.Utils.Cursors';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxRichEditCursors.Finalize', SysInit.HInstance);{$ENDIF}
  TdxCursorManager.Unregister(crcxRichEditControlBeginRotate);
  TdxCursorManager.Unregister(crcxRichEditControlIBeamItalic);
  TdxCursorManager.Unregister(crcxRichEditControlResizeColumn);
  TdxCursorManager.Unregister(crcxRichEditControlResizeRow);
  TdxCursorManager.Unregister(crcxRichEditControlReverseArrow);
  TdxCursorManager.Unregister(crcxRichEditControlRotate);
  TdxCursorManager.Unregister(crcxRichEditControlSelectColumn);
  TdxCursorManager.Unregister(crcxRichEditControlSelectTableCell);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxRichEditCursors.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxRichEditCursors.Initialize', SysInit.HInstance);{$ENDIF}
  TdxCursorManager.Register(crcxRichEditControlBeginRotate, HInstance, 'DXRICHEDITCONTROLBEGINROTATE');
  TdxCursorManager.Register(crcxRichEditControlIBeamItalic, HInstance, 'DXRICHEDITCONTROLIBEAMITALIC');
  TdxCursorManager.Register(crcxRichEditControlResizeColumn, HInstance, 'DXRICHEDITCONTROLRESIZECOLUMN');
  TdxCursorManager.Register(crcxRichEditControlResizeRow, HInstance, 'DXRICHEDITCONTROLRESIZEROW');
  TdxCursorManager.Register(crcxRichEditControlReverseArrow, HInstance, 'DXRICHEDITCONTROLREVERSEARROW');
  TdxCursorManager.Register(crcxRichEditControlRotate, HInstance, 'DXRICHEDITCONTROLROTATE');
  TdxCursorManager.Register(crcxRichEditControlSelectColumn, HInstance, 'DXRICHEDITCONTROLSELECTCOLUMN');
  TdxCursorManager.Register(crcxRichEditControlSelectTableCell, HInstance, 'DXRICHEDITCONTROLSELECTTABLECELL');
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxRichEditCursors.Initialize', SysInit.HInstance);{$ENDIF}
end;
