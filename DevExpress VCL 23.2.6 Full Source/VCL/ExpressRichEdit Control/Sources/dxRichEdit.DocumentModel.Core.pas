...
type
  ...
  TdxCommentViewInfoList = TList<TdxCommentViewInfo>; // for internal use
  TdxCommentBoxCalculator = class(TObject); // for internal use
  TdxCommentColorer = class(TObject); // for internal use
  TdxCommentSizeCalculator = class(TObject);
  TdxComment = class(TObject); // for internal use
  TdxCommentList = class(TList); // for internal use
  TdxCommentCollection = class(TObject); // for internal use

  TdxCustomMarkCollection = class(TObject);
  IdxHoverLayoutItem = class(TObject); // for internal use
  TdxCustomMarkBoxCalculator = class(TObject); // for internal use
  ...
  TdxFormatterPosition = record
    ...
    procedure Init(ARunIndex: TdxRunIndex; AOffset, ABoxIndex: Integer); inline;
  ...
  TdxRunCollection = class // for internal use
  strict private
    FList: TdxFastObjectList;
    function GetItem(Index: TdxRunIndex): TdxRunBase; inline;
  ...
  public
    ...
    function First: TdxRunBase; inline;
    function Last: TdxRunBase; inline;
  ...
  TdxParagraphBase = class abstract 
  strict private
    FLength: Integer;
  ...
  TdxParagraphBaseCollection = class abstract // for internal use
  strict protected
  ...
  PdxDocumentModelPosition = ^TdxDocumentModelPosition; // for internal use
  ...
  public
    ...
    class function MoveBackward(const APos: TdxDocumentModelPosition): TdxDocumentModelPosition; static; inline;
    class procedure MoveBackwardCore(var APos: TdxDocumentModelPosition); static;
    class function MoveForward(const APos: TdxDocumentModelPosition): TdxDocumentModelPosition; static; inline;
  ...
  TdxPositionConverter = class sealed // for internal use
  public
  ...
  TdxSectionPropertyModifierBase = class abstract // for internal use
  public
  ...
  TdxHistoryItem = class abstract // for internal use
  private
    FPieceTable: TdxCustomPieceTable;
  ...
  TdxCompositeHistoryItem = class(TdxHistoryItem) // for internal use
  private
  ...
  TdxNotificationIdGenerator = class // for internal use
  public const
    EmptyId = 0;
  private
    FLastId: Integer;
  ...
  TdxDocumentHistory = class // for internal use
  public const
  ...
  TdxEmptyNotificationIdGenerator = class(TdxNotificationIdGenerator) // for internal use
  public
  ...
  TdxEmptyHistory = class(TdxDocumentHistory) // for internal use
  strict private
  ...
  TdxDisabledHistory = class(TdxEmptyHistory) // for internal use
  strict private
  ...
  TdxHistoryTransaction = class // for internal use
  private
  ...
  TdxWebSettings = class // for internal use
  strict private
    FLeftMargin: Integer;
    FTopMargin: Integer;
  ...
  TdxCustomDocumentCache = class abstract // for internal use
  public
  ...
  TdxCustomDocumentModelDeferredChanges = class abstract(TcxIUnknownObject) // for internal use
  strict private
    FDocumentModel: TdxCustomDocumentModel;
  ...
  TdxCustomSection = class abstract // for internal use
  strict private
    FDocumentModel: TdxCustomDocumentModel;
  ...
  TdxCustomSectionCollection = class(TdxObjectList<TdxCustomSection>); // for internal use
  ...
  strict private
    ...
    FCurrentFontCache: TdxFontCache; 
    function GetEnableFieldNames: Boolean;
    function GetFontCache: TdxFontCache; inline;
  ...
  TdxContentTypeBase = class abstract // for internal use
  strict private
...
const
  dxThisUnitName = 'dxRichEdit.DocumentModel.Core';

type

  { TdxSectionAndLogPositionComparable }

  TdxSectionAndLogPositionComparable = record
  strict private
...
class procedure TdxDocumentModelPosition.MoveBackwardCore(var APos: TdxDocumentModelPosition);
var
  ARun: TdxRunBase;
begin
...
function TdxIndexBasedObject.QueryInterface(const IID: TGUID; out Obj): HResult;
begin
...
function TdxIndexBasedObject._AddRef: Integer;
begin
  Result := -1;
end;

function TdxIndexBasedObject._Release: Integer;
begin
  Result := -1;
end;
...
function TdxRunBase.QueryInterface(const IID: TGUID; out Obj): HResult;
begin
  if GetInterface(IID, Obj) then
    Result := S_OK
  else
...
function TdxRunBase._AddRef: Integer;
begin
  Result := -1;
end;

function TdxRunBase._Release: Integer;
begin
  Result := -1;
end;
...
var
  ...
  ARuns: PdxPointerList;
  I: Integer;
begin
  AStartIndex := FirstRunIndex;
  AEndIndex := LastRunIndex;
  ARuns := PieceTable.Runs.List.List;
...
function TdxParagraphBase.QueryInterface(const IID: TGUID; out Obj): HResult;
begin
  if GetInterface(IID, Obj) then
    Result := S_OK
  else
...
function TdxParagraphBase._AddRef: Integer;
begin
  Result := -1;
end;

function TdxParagraphBase._Release: Integer;
begin
  Result := -1;
end;
...
begin
  ...
  if not TdxAlgorithms1<TdxCustomSection>.BinarySearch(Sections, APredicate.CompareTo, Result) then
  begin
...
begin
  if FCurrentFontCache = nil then
    FCurrentFontCache := TdxFontCacheManager.GetFontCache(LayoutUnit, Dpi);
  Result := FCurrentFontCache;
end;
