...
type
 ...
 { TdxFindCommand }
  ...
  protected
    ...
    class function GetTableCellsCommandClass: TdxRichEditCommandClass; virtual; abstract;
...
uses
  Math, RTLConsts, Dialogs, IOUtils, DB,
...
const
  dxThisUnitName = 'dxRichEdit.Commands.Dialogs';
...
begin
  ACommandState := TdxTabsFormCommandUIState(AState);
  Result := TdxPieceTableModifiers.CreateTabFormattingInfoModifier(ACommandState.Value, ACommandState.DefaultTabWidth);
end;
...
begin
  ...
  if Safe.Cast(TObject(AState), TdxTabsFormCommandUIState, ACommandState) then
  begin
...
begin
  AValueBasedState := AState as IdxValueBasedCommandUIState<TdxPageSetupInfo>;
  Result := TdxPieceTableModifiers.CreateSectionPageSetupModifier(AValueBasedState.Value);
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxChangeSectionPaperKindCommand.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FDefaultPaperKindList);
  FreeAndNil(FFullPaperKindList);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxChangeSectionPaperKindCommand.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxChangeSectionPaperKindCommand.Initialize', SysInit.HInstance);{$ENDIF}
  FDefaultPaperKindList := CreateDefaultPaperKindList;
  FFullPaperKindList := CreateFullPaperKindList;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxChangeSectionPaperKindCommand.Initialize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  Result := TdxPieceTableModifiers.CreateSectionPageOrientationLandscapeModifier(False);
end;
...
function TdxSetLandscapePageOrientationCommand.CreateModifier(
  const AState: IdxCommandUIState): TdxSectionPropertyModifier<Boolean>;
begin
  Result := TdxPieceTableModifiers.CreateSectionPageOrientationLandscapeModifier(True);
end;
...
var
  ...
  AModifier: TdxRunPropertyModifier<string>;
  ARun: TdxTextRunBase;
...
begin
  ...
  begin
    AModifier := TdxPieceTableModifiers.CreateRunFontNamePropertyModifier(AValueState.Value.FontName);
    try
...
begin
  Result := TdxSplitTableCellsCommand.TCommandUIState.Create;
end;

procedure TdxShowSplitTableCellsFormCommand.ForceExecute(const AState: IdxCommandUIState);
var
  AValueBasedState: TdxSplitTableCellsCommand.TCommandUIState;
begin
  CheckExecutedAtUIThread;
  NotifyBeginCommandExecution(AState);
  try
    AValueBasedState := AState as TdxSplitTableCellsCommand.TCommandUIState;
    AValueBasedState.Value := GetSplitTableCellsParameters;

    RichEditControl.ShowSplitTableCellsForm(AValueBasedState.Value, ShowSplitTableCellsFormCallback, AValueBasedState);
  finally
    NotifyEndCommandExecution(AState);
  end;
end;
...
var
  ...
  AState: TdxSplitTableCellsCommand.TCommandUIState;
begin
  ...
  try
    ...
    try
      ...
      try
        ...
        begin
          AState := TdxSplitTableCellsCommand.TCommandUIState.Create;
...
begin
  AValueBasedState := AState as IdxValueBasedCommandUIState<TdxColumnsInfoUI>;
  Result := TdxPieceTableModifiers.CreateSectionColumnsSetupModifier(AValueBasedState.Value);
end;
...
begin
  ...
  try
    if Safe.Cast(ASourceStyle, TdxParagraphStyle, AParagraphStyleTo) and
       Safe.Cast(ATargetStyle, TdxParagraphStyle, AParagraphStyleFrom)
    then
      ModifyParagraphStyle(AParagraphStyleTo, AParagraphStyleFrom)
    else
      if Safe.Cast(ASourceStyle, TdxCharacterStyle, ACharacterStyleTo) and
         Safe.Cast(ATargetStyle, TdxCharacterStyle, ACharacterStyleFrom)
      then
...
begin
  AValueBasedState := AState as IdxValueBasedCommandUIState<TdxLineNumberingInfo>;
  Result := TdxPieceTableModifiers.CreateSectionLineNumberingModifier(AValueBasedState.Value);
end;
...
begin
  ...
  try
    AFileSaveDialog := TdxSaveFileDialog.Create(nil);
    try
      ...
      if Application.ActiveFormHandle <> 0 then
        AResult := AFileSaveDialog.Execute(Application.ActiveFormHandle)
      else
        AResult := AFileSaveDialog.Execute;
...
begin
  ...
  AOptions.HeaderFooterLinkToPrevious := False;
