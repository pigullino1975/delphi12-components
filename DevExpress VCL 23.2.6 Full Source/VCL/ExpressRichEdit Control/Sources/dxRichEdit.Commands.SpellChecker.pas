...
const
  dxThisUnitName = 'dxRichEdit.Commands.SpellChecker';
...
begin
  ...
  try
    ...
    try
      ...
      ALogPosition := AInterval.Start;
      AController.ReplaceWord(AStart, AEnd, Suggestion);

      DocumentModel.Selection.Start := ALogPosition + Length(Suggestion);
...
begin
  if Safe.Cast(TObject(AState), TdxDefaultValueBasedCommandUIState<string>, AValueState) then
  begin
