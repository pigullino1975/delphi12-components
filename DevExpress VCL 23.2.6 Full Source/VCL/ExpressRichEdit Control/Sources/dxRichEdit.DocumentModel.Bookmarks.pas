...
unit dxRichEdit.DocumentModel.Bookmarks; // for internal use
...
type
  ...
  strict private
    function GetItem(Index: TdxListIndex): TdxBookmarkBase; inline;
  public
    property Items[Index: TdxListIndex]: TdxBookmarkBase read GetItem; default;
  end;
  ...
  strict private
    function GetItem(Index: TdxListIndex): TdxBookmark;
  public
    property Items[Index: TdxListIndex]: TdxBookmark read GetItem; default;
  end;
  ...
  public
    ...
    procedure MoveNext;
    procedure Reset;
    class function CreateVisitableDocumentIntervalStartBoundaryFactory: TdxVisitableDocumentIntervalStartBoundaryFactory; static;
    class function CreateVisitableDocumentIntervalEndBoundaryFactory: TdxVisitableDocumentIntervalEndBoundaryFactory; static;
  ...
  strict private
    function GetItem(Index: TdxListIndex): TdxRangePermission; inline;
  public
    property Items[Index: TdxListIndex]: TdxRangePermission read GetItem; default;
  end;
...
const
  dxThisUnitName = 'dxRichEdit.DocumentModel.Bookmarks';
...
function TdxBookmarkBaseList.GetItem(Index: TdxListIndex): TdxBookmarkBase;
begin
...
var
  C: Char;
begin
  if (System.Length(AName) > 0) and not AName[1].IsLetter then
     Exit(False);
  for C in AName do
    if not C.IsLetterOrDigit and (C <> '_') then
      Exit(False);
  Result := True;
end;
...
function TdxBookmarkList.GetItem(Index: TdxListIndex): TdxBookmark;
begin
...
begin
  ...
  if ACount <= 1 then
    Exit;
  AOwnsItems := OwnsObjects;
  OwnsObjects := False;
  try
    QuickSort(AComparer, AIndex, AIndex + ACount - 1);
  finally
    OwnsObjects := AOwnsItems;
  end;
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxVisitableDocumentIntervalBoundaryIterator.Initialize', SysInit.HInstance);{$ENDIF}
  FComparer := TComparer.Create;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxVisitableDocumentIntervalBoundaryIterator.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxVisitableDocumentIntervalBoundaryIterator.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxVisitableDocumentIntervalBoundaryIterator.Finalize', SysInit.HInstance);{$ENDIF}
  FComparer.Free;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxVisitableDocumentIntervalBoundaryIterator.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
class function TdxVisitableDocumentIntervalBoundaryIterator.CreateVisitableDocumentIntervalStartBoundaryFactory: TdxVisitableDocumentIntervalStartBoundaryFactory;
begin
   Result := TdxVisitableDocumentIntervalStartBoundaryFactory.Create;
end;

class function TdxVisitableDocumentIntervalBoundaryIterator.CreateVisitableDocumentIntervalEndBoundaryFactory: TdxVisitableDocumentIntervalEndBoundaryFactory;
begin
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxRangePermission.Initialize', SysInit.HInstance);{$ENDIF}
  FBoundaryComparer := TBoundaryComparer.Create;
  FBoundaryOrderComparer := TBoundaryOrderComparer.Create;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxRangePermission.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxRangePermission.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxRangePermission.Finalize', SysInit.HInstance);{$ENDIF}
  FBoundaryComparer.Free;
  FBoundaryOrderComparer.Free;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxRangePermission.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
function TdxRangePermissionList.GetItem(Index: TdxListIndex): TdxRangePermission;
begin
...
var
  ARangePermission: TdxRangePermission;
begin
  if not Safe.Cast(AInterval, TdxRangePermission, ARangePermission) then
    Exit(False);
...
begin
  ...
  begin
    ...
    if not TdxAlgorithms1<TdxBoxBase>.BinarySearch(ABoxes, AComparable.CompareTo, ABoxIndex) then
    begin
