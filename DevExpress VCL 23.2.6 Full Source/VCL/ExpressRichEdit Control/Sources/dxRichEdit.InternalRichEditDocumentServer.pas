...
unit dxRichEdit.InternalRichEditDocumentServer; // for internal use
...
const
  dxThisUnitName = 'dxRichEdit.InternalRichEditDocumentServer';
...
begin
  ...
  if Safe.Cast(AObject, TdxInternalRichEditDocumentServer, Result) then
    Exit;
...
procedure TdxInternalRichEditDocumentServer.AddDocumentClosingHandler(const AHandler: TdxCloseQueryEvent);
begin
  if InnerServer <> nil then
    InnerServer.DocumentClosing.Add(AHandler);
end;

procedure TdxInternalRichEditDocumentServer.RemoveDocumentClosingHandler(const AHandler: TdxCloseQueryEvent);
begin
  if InnerServer <> nil then
...
var
  ABatchUpdatable: IdxBatchUpdatable;
begin
  ABatchUpdatable := InnerServer as IdxBatchUpdatable;
  Result := ABatchUpdatable.BatchUpdateHelper;
end;
