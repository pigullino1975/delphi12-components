...
unit dxRichEdit.DocumentModel.NotesRange; // for internal use
...
type
  ...
  protected
    ...
    procedure InheritRowProcessingFlags(ARun: TdxTextRunBase); override;
  public
...
const
  dxThisUnitName = 'dxRichEdit.DocumentModel.NotesRange';
...
procedure TdxLayoutDependentTextRun.InheritRowProcessingFlags(ARun: TdxTextRunBase);
begin
...
var
  ACounter: TdxCounter;
  ALogPosition: TdxDocumentLogPosition;
  ATypedFormatter: TdxParagraphBoxFormatter absolute AFormatter;
begin
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxFootNoteNumberResultFormatting.Initialize', SysInit.HInstance);{$ENDIF}
  FInstance := TdxFootNoteNumberResultFormatting.Create;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxFootNoteNumberResultFormatting.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxFootNoteNumberResultFormatting.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxFootNoteNumberResultFormatting.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FInstance);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxFootNoteNumberResultFormatting.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxEndNoteNumberResultFormatting.Initialize', SysInit.HInstance);{$ENDIF}
  FInstance := TdxEndNoteNumberResultFormatting.Create;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxEndNoteNumberResultFormatting.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxEndNoteNumberResultFormatting.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxEndNoteNumberResultFormatting.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FInstance);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxEndNoteNumberResultFormatting.Finalize', SysInit.HInstance);{$ENDIF}
end;
