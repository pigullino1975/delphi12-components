...
type
  ...
  protected
    ...
    procedure GeneratePagesToEnsureScrollingSuccessfully(ADelta: Int64); virtual;
    procedure UpdateUIStateCore(const AState: IdxCommandUIState); override;
  ...
  protected
    procedure GeneratePagesToEnsureScrollingSuccessfully(ADelta: Int64); override;
  ...
  private
    ...
    procedure SetCurrentPageRow(const Value: TdxPageViewInfoRow);
  protected
  ...
  public
    ...
    function CalculateNextPosition(ACaretPosition: TdxCaretPosition): TdxDocumentModelPosition; // for internal use
    ...
    function ObtainTargetRowFromCurrentPageViewInfoAndCaretX(APageViewInfo: TdxPageViewInfo;
      ACellViewInfoFromPreviousTableViewInfo: TdxTableCellViewInfo; var ACaretX: Integer): TdxRow;
  ...
  TdxExtendKeyboardSelectionHorizontalDirectionCalculator = record
  strict private
...
const
  dxThisUnitName = 'dxRichEdit.Commands';
...
begin
  ...
  try
    ...
    GeneratePagesToEnsureScrollingSuccessfully(ADelta * Sign(AOffset));
...
procedure TdxScrollVerticallyByPhysicalOffsetCommandBase.GeneratePagesToEnsureScrollingSuccessfully(ADelta: Int64);
begin
end;
...
begin
  ...
  TdxPageViewInfo.Release(APageViewInfo);
end;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          begin
            TdxPageViewInfo.AddReference(APageViewInfo);
            Exit(APageViewInfo);
          end;
        end;
      end;
...
destructor TdxNextCaretPositionVerticalDirectionCalculator.Destroy;
begin
  CurrentPageRow := nil;
  inherited Destroy;
end;
...
begin
  ...
  if TdxAlgorithms1<TdxTableCellViewInfo>.BinarySearch(ACells, APredicate.CompareTo, AIndex) then
    Exit(ACells[AIndex]);
...
begin
  ...
  try
    ...
    try
      ...
      ABoxIndex := AHitTestCalculator.FastHitTestIndexCore(ARow.Boxes, AComparable.CompareTo, False);
      Assert(ABoxIndex >= 0);
...
begin
  ...
  if ShouldObtainTargetPageViewInfo(ARowIndex, APageRows.Count) then
    GenerateTargetPageViewInfoRow
  else
    CurrentPageRow := ObtainExistingTargetPageViewInfoRow(APageRows, ARowIndex);
  if CurrentPageRow <> nil then
  begin
    Result := CurrentPageRow.GetPageAtPoint(Point(APhysicalCaretX, 0), False);
    Assert(Result <> nil);
  end
  else
    Result := nil;
...
function TdxNextCaretPositionVerticalDirectionCalculator.ObtainTargetRowFromCurrentPageViewInfoAndCaretX(
  APageViewInfo: TdxPageViewInfo;
  ACellViewInfoFromPreviousTableViewInfo: TdxTableCellViewInfo;
...
begin
  ...
  if ACellViewInfoFromPreviousTableViewInfo <> nil then
    ATargetTable := ACellViewInfoFromPreviousTableViewInfo.TableViewInfo.Table
  else
...
procedure TdxNextCaretPositionVerticalDirectionCalculator.SetCurrentPageRow(const Value: TdxPageViewInfoRow);
begin
...
procedure TdxScrollVerticallyByPhysicalOffsetEnsurePageGenerationCommand.GeneratePagesToEnsureScrollingSuccessfully(
  ADelta: Int64);
var
...
begin
  ...
  if Supports(AState, IdxValueBasedCommandUIState<TdxAlphaColor>, AValueBasedState) then
    AValueBasedState.Value := DocumentModel.DocumentProperties.PageBackColor;
end;
