...
type
  ...
  TdxDragContentMouseHandlerStateCalculator = record
  strict private
  ...
  public
    ...
    procedure OnInternalDragStart;
  end;

  { TdxDragFloatingObjectMouseHandlerStateCalculator }

  TdxDragFloatingObjectMouseHandlerStateCalculator = record
  private
    FAnchor: TdxFloatingObjectAnchorRun;
    FLogicalPointOffset: TPoint;
  public
    class function Create: TdxDragFloatingObjectMouseHandlerStateCalculator; static;
    procedure Init(AAnchor: TdxFloatingObjectAnchorRun; const AOffset: TPoint);
    function CanDropTo(APoint: TdxRichEditHitTestResult): Boolean;
  end;
...
uses
  Windows, Math, RTLConsts, cxGeometry, cxControls, dxTypeHelpers, cxLibraryConsts,
...
const
  dxThisUnitName = 'dxRichEdit.Control.Mouse';
...
type
  ...
  private
    ...
    procedure ExtendMultiSelection(AIterator: TdxPieceTableIterator; const ACurrentModelPosition: TdxDocumentModelPosition);
  ...
  protected
    ...
    function CreateDragState(const ADataObject: IDataObject): TdxRichEditMouseCustomState;
    function CreateDragContentState(AHitTestResult: TdxRichEditHitTestResult): TdxRichEditMouseCustomState;
    function CreateExternalDragState: TdxRichEditMouseCustomState;
    function CreateInternalDragState: TdxRichEditMouseCustomState;
    function IsExternalDrag(const ADataObject: IDataObject): Boolean;

    procedure ExtendSelection(AIterator: TdxPieceTableIterator);
    procedure SwitchStateToDragState(AHitTestResult: TdxRichEditHitTestResult; ADragState: TdxRichEditMouseCustomState);

    function HandleHotZoneMouseDown(AHitTestResult: TdxRichEditHitTestResult): Boolean;
    function HandleHotZoneMouseDownCore(AHitTestResult: TdxRichEditHitTestResult; AHotZone: TdxHotZone): Boolean;
  public
    ...
    procedure BeginResizeTableRowState(AHitTestResult: TdxRichEditHitTestResult; ATableRow: TdxTableRowViewInfoBase);
    procedure BeginResizeTableCellsHorizontallyState(AHitTestResult: TdxRichEditHitTestResult; AColumn: TdxVirtualTableColumn);
    procedure BeginTableColumnsSelectionState(AHitTestResult: TdxRichEditHitTestResult);
    procedure BeginTableCellsSelectionState(AHitTestResult: TdxRichEditHitTestResult);

    function CreatePlaceCaretToPhysicalPointCommand(AShift: Boolean): TdxPlaceCaretToPhysicalPointCommand;
    ...
    function IsMouseDownInSelection(ALogPosition: TdxDocumentLogPosition): Boolean;
  protected
    ...
    function ActivateFloatingObjectTextBox(ARun: TdxFloatingObjectAnchorRun; AHitTestResult: TdxRichEditHitTestResult): Boolean;
...
begin
  ...
  TdxReferencedObject.AddReference(FPageViewInfo);
  FBounds := ABounds;
end;
...
begin
  ...
  Controller.ActiveObject := CalculateActiveObject(AHitTestResult);
end;
...
destructor TdxRectangleVisualFeedbackValue.Destroy;
begin
  TdxReferencedObject.Release(FPageViewInfo);
  inherited Destroy;
end;
...
begin
  ...
  if not Safe.Cast(AHitTestResult.FloatingObjectBox.GetRun(AHitTestResult.FloatingObjectBox.PieceTable), TdxFloatingObjectAnchorRun, ARun) then
    Exit(False);
...
class function TdxDragFloatingObjectMouseHandlerStateCalculator.Create: TdxDragFloatingObjectMouseHandlerStateCalculator;
begin
  Result := Default(TdxDragFloatingObjectMouseHandlerStateCalculator);
end;
...
begin
  ...
  if Safe.Cast(ARun.Content, TdxTextBoxFloatingObjectContent, ATextBoxContent) then
  begin
    Control.BeginUpdate;
    try
...
begin
  ...
  begin
    ...
    begin
      ...
      Result := Safe.Cast(ARun.Content, TdxTextBoxFloatingObjectContent, AContent) and (AContent.TextBox.PieceTable <> ActivePieceTable);
    end
    else
