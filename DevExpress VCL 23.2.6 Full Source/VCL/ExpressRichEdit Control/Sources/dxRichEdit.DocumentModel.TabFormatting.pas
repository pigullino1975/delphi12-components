...
unit dxRichEdit.DocumentModel.TabFormatting; // for internal use
...
type
  ...
  TdxTabFormattingInfoWithoutSort = class(TdxTabFormattingInfo, IdxBatchUpdatable, IdxBatchUpdateHandler)
  private
...
uses
  RTLConsts,
  dxRichEdit.DocumentModel.Cache.Simple, dxCore;

const
  dxThisUnitName = 'dxRichEdit.DocumentModel.TabFormatting';
...
var
  TabInfoComparer: TdxTabInfoComparer;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxTabFormattingInfo.Finalize', SysInit.HInstance);{$ENDIF}
  TabInfoComparer.Free;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxTabFormattingInfo.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxTabFormattingInfo.Initialize', SysInit.HInstance);{$ENDIF}
  TabInfoComparer := TdxTabInfoComparer.Create;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxTabFormattingInfo.Initialize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  FTabInfos.Sort(TabInfoComparer);
end;
