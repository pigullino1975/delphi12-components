...
type
  ...
  ['{BBD4B8A8-C6F1-4FCA-971C-6A4963BE20CB}']
    function GetStream(const AUri: string; ATimeout: Cardinal): TStream;
  ...
  ['{C060940C-6100-48B7-8DA3-B2624C996CD8}']
    function AsyncGetStream(const AUri: string; ATimeout: Cardinal; const ACancelStatus: TdxTaskCancelCallback): TStream;
  end;
...
const
  dxThisUnitName = 'dxRichEdit.Utils.UriStreamService';
...
function TdxUriStreamService.GetStream(const AUri: string; ATimeout: Cardinal): TStream;
begin
  Result := AsyncGetStream(AUri, ATimeout, nil);
end;

function TdxUriStreamService.AsyncGetStream(const AUri: string; ATimeout: Cardinal; const ACancelStatus: TdxTaskCancelCallback): TStream;
var
  ACount, I: Integer;
  ...
  begin
    try
      ...
      if Supports(Providers[I], IdxUriStreamProvider2, AProvider2) then
        Result := AProvider2.AsyncGetStream(AUri, ATimeout, ACancelStatus)
      else
        Result := Providers[I].GetStream(AUri, ATimeout);
      if Result <> nil then
        Break;
    except
      Result := nil;
...
function TdxLocalUriStreamProvider.GetStream(const AUri: string; ATimeout: Cardinal): TStream;
var
  AUriRecord: TdxUri;
  AFile: string;
begin
  ...
  if AUriRecord.IsFileScheme then
    AFile := AUriRecord.LocalPath
  else
    if AUriRecord.IsRelativeUri then
      AFile := AUri
    else
      AFile := '';
  if TFile.Exists(AFile) then
    try
      Result := TdxMemoryStream.Create(AFile);
      Result.Position := 0;
    except
      FreeAndNil(Result);
    end;
...
function TdxWebUriStreamProvider.GetStream(const AUri: string; ATimeout: Cardinal): TStream;
begin
  Result := AsyncGetStream(AUri, ATimeout, nil);
end;

function TdxWebUriStreamProvider.AsyncGetStream(const AUri: string; ATimeout: Cardinal; const ACancelStatus: TdxTaskCancelCallback): TStream;
...
begin
  ...
  begin
    try
      ...
      if TdxHttpHelper.GetStream(Application.Title, AFile, '', Result, nil, ATimeout) and
...
function TdxDataStringUriStreamProvider.GetStream(const AUri: string; ATimeout: Cardinal): TStream;
...
begin
  ...
  try
    ARegex := TRegEx.Create(TDataStringUriPattern.ImageDataStringPattern, [roNotEmpty, roSingleLine]);
...
begin
  ...
  AUri := TdxURI.Create(Result);
  if AUri.IsAbsoluteUri then
    Result := AUri.AbsoluteUri
  else
    Result := AUri.LocalPath;
end;
