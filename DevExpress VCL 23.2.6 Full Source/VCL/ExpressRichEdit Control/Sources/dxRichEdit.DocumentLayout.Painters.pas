...
type
  ...
  TdxCustomMarkVisualInfo = class // for internal use
  private
    FBounds: TRect;
  ...
  TdxCustomMarkExporter = class(TcxIUnknownObject, IdxCustomMarkExporter) // for internal use
  private
  ...
  TdxOfficeSelectionPainter = class abstract (TcxIUnknownObject) // for internal use
  strict private
    class var
  ...
  TdxRichEditSelectionPainter = class abstract(TdxOfficeSelectionPainter,
    IdxSelectionPainter,
    IdxHotZoneVisitor,
    IdxRectangularObjectHotZoneVisitor) // for internal use
  public const
    SelectionColor: TdxAlphaColor = $7F5097E3; 
  strict private
    class var
...
{$R 'dxRichEdit.DocumentModel.Images_svg.res'}
...
const
  dxThisUnitName = 'dxRichEdit.DocumentLayout.Painters';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxOfficeSelectionPainter.Initialize', SysInit.HInstance);{$ENDIF}
  ...
  FShapeBorderWidth := 1;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxOfficeSelectionPainter.Initialize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  if dxUseVectorIcons then
    AStream := TResourceStream.Create(HInstance, 'DXRECANCHOR', 'SVG')
  else
