...
type
  ...
  private
    ...
    function CalculateLevelIndentAdjustment(const ALevel: IdxListLevel): Integer;
  ...
  protected
    procedure SetRecoveredFormat(const AFormat: string);
    procedure ParsePreparedFormat(const AFormat: string);
...
uses
  ...
  cxControls,
  dxCore, dxCoreClasses, dxTypeHelpers, dxDocumentLayoutUnitConverter,
...
const
  dxThisUnitName = 'dxRichEdit.Dialogs.NumberingHelper';
...
begin
  ...
  AAdjustment := CalculateLevelIndentAdjustment(ALevel);
...
function TdxNumberingListBoxHelper.CalculateLevelIndentAdjustment(const ALevel: IdxListLevel): Integer;
begin
...
var
  ...
  ALength: Integer;
  AUnderlineModifier: TdxRunPropertyModifier<TdxUnderlineType>;
  AUnderlineColorModifier: TdxRunPropertyModifier<TdxAlphaColor>;
begin
  ...
  AUnderlineModifier := TdxPieceTableModifiers.CreateRunFontUnderlineTypeModifier(TdxUnderlineType.ThickSingle);
  try
    APieceTable.ApplyCharacterFormatting(AFrom, ALength, AUnderlineModifier);
    AUnderlineColorModifier := TdxPieceTableModifiers.CreateRunUnderlineColorModifier(TdxAlphaColors.BtnShadow); 
    try
...
procedure TdxNumberingListFormHelper.ApplyChanges(ANewList: TdxAbstractNumberingList);
begin
  ApplyChanges(ANewList, TdxNumberingListApplyScope.ToSelectedText, TdxNumberingType.Bullet, NumberingListIndexListIndexNotSet);
end;
...
begin
  ...
  begin
    ...
    if AIndexOfParagraphInListBeforeCurrent >= ParagraphIndexMinValue then
      ...
      ANewListIndex := NumberingListIndexListIndexNotSet;
...
begin
  ...
  if (FOwner.RestartNumbering or ASelectedParagraphsHaveDifferentNumberingLists or not AAreThereParagraphsInList) then
    ...
    begin
      AParagraph := AFirstSelectedParagraph.PieceTable.Paragraphs[AIndexOfParagraphInListBeforeCurrent];
      APrevParagraphInDifferentListStartIndexes := AFirstSelectedParagraph.PieceTable.GetRangeListCounters(AParagraph);
  ...
  try
    ...
    begin
      ...
      if AIndexOfParagraphInListBeforeCurrent >= ParagraphIndexMinValue then
        ...
        ANewNumbListIndex := NumberingListIndexListIndexNotSet;
...
procedure TdxDisplayFormatHelper.SetRecoveredFormat(const AFormat: string);
begin
  FStringBuilder.Append('0');
  FlushAsFormat;
  FStringBuilder.Append(AFormat);
  FlushAsText;
end;

procedure TdxDisplayFormatHelper.ParsePreparedFormat(const AFormat: string);
var
  P: PChar;
begin
  P := PChar(AFormat);
...
var
  APreparedFormat: string;
begin
  APreparedFormat := Format(AFormat, [#$00'0', #$00'1', #$00'2', #$00'3', #$00'4', #$00'5', #$00'6', #$00'7', #$00'8']);
  FStringBuilder.Clear;
  FControl.BeginUpdate;
  try
    ...
    try
      if APreparedFormat = AFormat then 
        SetRecoveredFormat(AFormat)
      else
        ParsePreparedFormat(APreparedFormat);
      Update;
    finally
...
function TdxNumberingListDataSet.GetFieldData(Field: TField; var Buffer: TValueBuffer): Boolean;
var
  AValue: string;
