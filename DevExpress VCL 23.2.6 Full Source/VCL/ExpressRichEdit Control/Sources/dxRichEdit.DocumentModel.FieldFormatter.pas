...
unit dxRichEdit.DocumentModel.FieldFormatter; // for internal use
...
uses
  SysUtils, Classes, Generics.Defaults, Generics.Collections, Rtti, Contnrs,
...
uses
  RTLConsts, Character, Variants, Math,
  cxVariants,
...
const
  dxThisUnitName = 'dxRichEdit.DocumentModel.FieldFormatter';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxFieldFormatter.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FCultureUpdater);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxFieldFormatter.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxGeneralNumberFormatter.Initialize', SysInit.HInstance);{$ENDIF}
  FNumberFormatKeywords := CreateNumberFormatKeywordTable;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxGeneralNumberFormatter.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxGeneralNumberFormatter.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxGeneralNumberFormatter.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FNumberFormatKeywords);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxGeneralNumberFormatter.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  ...
  AConverter := TdxOrdinalBasedNumberConverter.GetUpperLatinLetterNumberConverter;
  try
...
begin
  AIntegerValue := ValidateValueForConvertingToLatinChars(AValue);
  AConverter := TdxOrdinalBasedNumberConverter.GetLowerLatinLetterNumberConverter;
  try
    Result := AConverter.ConvertNumber(AIntegerValue);
  finally
    AConverter.Free;
...
begin
  AIntegerValue := ConvertToShortInteger(AValue);
  AConverter := TdxOrdinalBasedNumberConverter.GetUpperRomanNumberConverterClassic;
  try
    Result := AConverter.ConvertNumber(AIntegerValue);
  finally
    AConverter.Free;
...
begin
  AIntegerValue := ConvertToShortInteger(AValue);
  AConverter := TdxOrdinalBasedNumberConverter.GetLowerRomanNumberConverterClassic;
  try
    Result := AConverter.ConvertNumber(AIntegerValue);
  finally
    AConverter.Free;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxGeneralStringFormatter.Initialize', SysInit.HInstance);{$ENDIF}
  FStringFormatKeywords := CreateStringFormatKeywordsTable;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxGeneralStringFormatter.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxGeneralStringFormatter.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxGeneralStringFormatter.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FStringFormatKeywords);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxGeneralStringFormatter.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
var
  ...
  AIndex: Integer;
begin
  ABuilder := TStringBuilder.Create(AValue.AsString.ToLower);
  try
    AIndex := 0;
    while AIndex < ABuilder.Length do
    begin
      ABuilder[AIndex] := ABuilder[AIndex].ToUpper;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxGeneralDocumentModelStringFormatter.Initialize', SysInit.HInstance);{$ENDIF}
  FStringFormatKeywords := CreateStringFormatKeywordsTable;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxGeneralDocumentModelStringFormatter.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxGeneralDocumentModelStringFormatter.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxGeneralDocumentModelStringFormatter.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FStringFormatKeywords);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxGeneralDocumentModelStringFormatter.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  ...
  try
    ...
    begin
      ...
      ATextBuffer[APosition] := ATextBuffer[APosition].ToUpper;
      APos := AIterator.MoveForward(APos);
    end;
    Result := '';
  finally
...
begin
  ...
  for I := 0 to ACount - 1 do
    ATextBuffer[I] := ATextBuffer[I].ToLower;
  Result := '';
end;
...
begin
  ...
  ACount := ATextBuffer.Length;
  for I := 0 to ACount - 1 do
    ATextBuffer[I] := ATextBuffer[I].ToUpper;
  Result := '';
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxNumberToTraditionalZodiacIdeographsConverter.Initialize', SysInit.HInstance);{$ENDIF}
  ...
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxNumberToTraditionalZodiacIdeographsConverter.Initialize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxNumberToDigitalIdeographsConverter.Initialize', SysInit.HInstance);{$ENDIF}
  FDigitalIdeographs := TArray<Char>.Create(#$3007, #$4E00, #$4E8C, #$4E09, #$56DB,
    #$4E94, #$516D, #$4E03, #$516B, #$4E5D);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxNumberToDigitalIdeographsConverter.Initialize', SysInit.HInstance);{$ENDIF}
end;
