...
unit dxRichEdit.DocumentModel.Fields.InfixNotationParser; // for internal use
...
type
  ...
  strict private
    ...
    class procedure ProcessSubstructionOperation(AOperands: TStack<Double>); static;
...
uses
  ...
  dxCharacters,
  dxRichEdit.DocumentModel.FieldFormatter, dxCore;

const
  dxThisUnitName = 'dxRichEdit.DocumentModel.Fields.InfixNotationParser';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxInfixNotationParserStateBase.Initialize', SysInit.HInstance);{$ENDIF}
  FDelimiters := CreateDelimitersTable;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxInfixNotationParserStateBase.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxInfixNotationParserStateBase.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxInfixNotationParserStateBase.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FDelimiters);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxInfixNotationParserStateBase.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  ...
  if ACh.IsDigit or (ACh = ADecimalSeparator) then
  begin
...
begin
  ...
  if ANotation[AIndex].IsDigit then
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxMathematicalCalculator.Initialize', SysInit.HInstance);{$ENDIF}
  ...
  Operations[TdxOperationType.Subtraction] := ProcessSubstructionOperation;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxMathematicalCalculator.Initialize', SysInit.HInstance);{$ENDIF}
end;
...
class procedure TdxMathematicalCalculator.ProcessSubstructionOperation(AOperands: TStack<Double>);
var
