...
type
  ...
  TdxHyperlinkInfoCollection = class(TEnumerable<Integer>) // for internal use
  strict private
  ...
  TdxHyperlinkInstructionBuilder = class // for internal use
  ...
  TdxHyperlinkUriHelper = class // for internal use
  ...
  TdxHyperlinkCalculator = class // for internal use
  strict private type
    TdxRecord = record
      Start: TdxRunIndex;
      &End: TdxRunIndex;
...
const
  dxThisUnitName = 'dxRichEdit.DocumentModel.Hyperlink';

{ TdxHyperlinkInfo }

class constructor TdxHyperlinkInfo.Initialize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxHyperlinkInfo.Initialize', SysInit.HInstance);{$ENDIF}
  FEmpty := TdxHyperlinkInfo.Create;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxHyperlinkInfo.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxHyperlinkInfo.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxHyperlinkInfo.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FEmpty);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxHyperlinkInfo.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
var
  ...
  I: Integer;
begin
  if Length(Trim(ABaseUri)) = 0 then
    Exit(AUri);
  AParts := TdxStringList.Create;
  try
