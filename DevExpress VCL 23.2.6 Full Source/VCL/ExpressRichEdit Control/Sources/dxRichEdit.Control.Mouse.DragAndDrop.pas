...
type
  ...
  protected
    ...
    procedure UpdateTableViewInfoController(const APhysicalPoint: TPoint; AHitTestResult: TdxRichEditHitTestResult);
  public
    procedure DoDragOver(var Args: TdxDragEventArgs); override;
    procedure DoDragLeave; override;
  end;
...
const
  dxThisUnitName = 'dxRichEdit.Control.Mouse.DragAndDrop';
...
procedure TdxCancellableDragMouseHandlerStateBase.SetPageViewInfo(
  const Value: TdxPageViewInfo);
begin
...
destructor TdxCancellableDragMouseHandlerStateBase.Destroy;
begin
  PageViewInfo := nil;
  inherited Destroy;
end;
...
begin
  AManager := Control.InnerControl.CreateCopySelectionManager;
  try
    ASelection := DocumentModel.Selection;
...
begin
  ...
  begin
    ...
    if Safe.Cast(FRun.Content, TdxPictureFloatingObjectContent, AContent) then
...
begin
  ...
  PageViewInfo := Control.InnerControl.ActiveView.GetPageViewInfoFromPoint(APoint, False);
...
begin
  if FCurrentTopLeftCorner.IsEqual(Unassigned) or (PageViewInfo = nil) or (FCurrentHitTestResult = nil) then
    Exit(nil);
...
begin
  if FCurrentTopLeftCorner.IsEqual(Unassigned) or (PageViewInfo = nil) or (FCurrentHitTestResult = nil) then
    Exit;
  ShowVisualFeedbackCore(TRect.CreateSize(FCurrentTopLeftCorner, FInitialShapeBounds.Size), PageViewInfo, FImage);
end;
...
begin
  ...
  HideVisualFeedbackCore(TRect.Null, PageViewInfo);
end;
