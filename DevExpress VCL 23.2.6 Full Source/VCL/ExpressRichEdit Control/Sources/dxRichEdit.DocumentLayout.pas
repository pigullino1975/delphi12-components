...
uses
  ...
  dxCoreGraphics, cxGeometry, dxTypeHelpers,
...
type
  ...
  TdxDocumentLayoutExporter = class abstract (TcxIUnknownObject,
    ...
    IdxDocumentLayoutExporter) // for internal use
  strict private
    ...
    FBackgroundLayers: TdxFastObjectList; 
  protected
  ...
  public
    ...
    procedure ExportFloatingObjectPicture(ABox: TdxFloatingObjectBox; AImage: TdxOfficeImage); virtual;
  ...
  TdxBoundedDocumentLayoutExporter = class abstract(TdxDocumentLayoutExporter,
    ...
    IdxUnderlinePainter,
    IdxStrikeoutPainter,
    IdxCharacterLinePainter) // for internal use
  private
    FBounds: TRect;
  ...
  TdxGraphicsDocumentLayoutExporterWhitespaceStrategy = class abstract // for internal use
  private
  ...
  TdxGraphicsDocumentLayoutExporterAdapter = class abstract // for internal use
  public
    ...
    procedure ExportFloatingObjectPicture(AExporter: TdxGraphicsDocumentLayoutExporter; ABox: TdxFloatingObjectBox;
      AImage: TdxOfficeImage); virtual; abstract;
  end;

  { TdxMarkBoxExporterBase }

  TdxMarkBoxExporterBase = class abstract // for internal use
  public const
    PaddingsWidth = 60;
    SpacingWidth = 6;
  strict private
  ...
  TdxGraphicsDocumentLayoutExporter = class(TdxBoundedDocumentLayoutExporter) // for internal use
  protected type
  ...
  protected
    ...
    function GetActualColor(AColor: TdxAlphaColor): TdxAlphaColor;
    ...
    procedure ExportTextBoxCore(ABox: TdxBox; const ATextBounds: TRect; const AText: string); overload;
    procedure ExportSpaceBoxCore(ABox: TdxBox; const ATextBounds: TRect; const AText: string);
    ...
    function GetBorderPainter(ACorner: TdxCornerViewInfoBase): TdxTableCornerPainter;
  ...
  public
    ...
    procedure ExportFloatingObjectPicture(ABox: TdxFloatingObjectBox; AImage: TdxOfficeImage); override;
  ...
  TdxScreenOptimizedGraphicsDocumentLayoutExporter = class(TdxGraphicsDocumentLayoutExporter) // for internal use
  strict private
    FPixel: Integer;
  ...
  public
    ...
    function ShouldOptimizeBox(ABox: TdxBox): Boolean;
    ...
    procedure ExportNumberingListBox(ABox: TdxNumberingListBox); override;
    procedure ExportLineNumberBox(ABox: TdxLineNumberBox); override;
    procedure ExportBoxOptimized(ABox: TdxBox);
  ...
  TdxScreenOptimizedGraphicsDocumentLayoutExporterNoLineNumbers = class(TdxScreenOptimizedGraphicsDocumentLayoutExporter) // for internal use
  public
    procedure ExportLineNumberBox(ABox: TdxLineNumberBox); override;
  end;
  ...
  public
    ...
    procedure ExportFloatingObjectPicture(AExporter: TdxGraphicsDocumentLayoutExporter; ABox: TdxFloatingObjectBox;
      AImage: TdxOfficeImage); override;
    ...
    procedure DrawLineBreakArrow(AExporter: TdxGraphicsDocumentLayoutExporter; const AGlyphBounds: TRect; AForeColor: TdxAlphaColor);
  ...
  TdxCharacterBoxLevelDocumentLayoutExporter = class(TdxDocumentLayoutExporter) // for internal use
  private
  ...
  TdxGraphicsPainterWrapper = class(TcxIUnknownObject, IdxPainterWrapper) // for internal use
  private
    FPainter: TdxPainter;
  ...
  TdxGraphicsDocumentLayoutExporterTableBorder = record
  private
    FPainter: TdxPainter;
    FHorizontalLinePainter: TdxRichEditPatternLinePainter;
    FVerticalLinePainter: TdxRichEditPatternLinePainter;
    FDocumentModel: TdxDocumentModel;
    FBounds: TRect;
    function GetOffset: TPoint; inline;
    function GetBorderPainter(ABorder: TdxBorderInfo; AConverter: TdxDocumentModelUnitToLayoutUnitConverter;
      APainterWrapper: TdxGraphicsPainterWrapper): TdxTableBorderPainter; overload;  
    function GetTableBorderLine(ABorderLineStyle: TdxBorderLineStyle; ADocumentModel: TdxDocumentModel): TdxUnderline; overload;
  ...
  public
    ...
    procedure ExportTableBorder(ABorder: TdxBorderInfo; const ADrawingBounds: TRect;
      AConverter: TdxDocumentModelUnitToLayoutUnitConverter; const AViewInfo: TdxTableBorderViewInfoBase);
...
const
  dxThisUnitName = 'dxRichEdit.DocumentLayout';
...
begin
  ...
  FBackgroundLayers := TdxFastObjectList.Create; 
  PushBackgroundLayer;
end;
...
begin
  if Safe.Cast(ARow, TdxTableCellRow, ACellRow) then
  begin
...
procedure TdxDocumentLayoutExporter.ExportFloatingObjectPicture(ABox: TdxFloatingObjectBox; AImage: TdxOfficeImage);
begin
end;
...
procedure TdxDocumentLayoutExporter.RestorePieceTable;
begin
...
begin
  ...
  try
    ...
    if Safe.Cast(ARun.Content, TdxPictureFloatingObjectContent, APictureContent) then
    begin
      ExportFloatingObjectPicture(ABox, APictureContent.Image.Image);
      Exit(True);
    end;
  finally
    PieceTable := APreviousPieceTable;
  end;
  if (ABox.DocumentLayout <> nil) and Safe.Cast(ARun.Content, TdxTextBoxFloatingObjectContent, ATextBoxContent) then
...
begin
  ...
  begin
    AColor := TdxBackgroundLayer(FBackgroundLayers[ALayerIndex]).GetBackColor(ABounds);
    Dec(ALayerIndex);
  end;
...
begin
  TdxBackgroundLayer(FBackgroundLayers[FBackgroundLayers.Count - 1]).SetBackColor(AColor, ABounds);
end;
...
begin
  Result := not ABoxBounds.IsEmpty and (Painter.HasTransform or FBounds.IntersectsWith(ABoxBounds));
end;
...
function TdxGraphicsDocumentLayoutExporter.IsValidBounds(const ABoxBounds: TRect): Boolean;
begin
  Result := not ABoxBounds.Size.IsZero and (Painter.HasTransform or Painter.ClipBounds.IntersectsWith(ABoxBounds));
end;
...
begin
  ...
  Result := Painter.TryPushRotationTransform(ACenter, DocumentModel.GetBoxEffectiveRotationAngleInDegrees(ABox));
  Painter.PushSmoothingMode(Result);
end;
...
procedure TdxGraphicsDocumentLayoutExporter.ExportFloatingObjectPicture(ABox: TdxFloatingObjectBox; AImage: TdxOfficeImage);
begin
  FAdapter.ExportFloatingObjectPicture(Self, ABox, AImage);
end;
...
begin
  ...
  begin
    AText := GetBoxText(ABox);
    ExportTextBoxCore(ABox, ATextBounds, AText);
    if Safe.Cast(ABox, TdxNumberingListBoxWithSeparator, ANumberingListBoxWithSeparator) then
...
procedure TdxWinFormsGraphicsDocumentLayoutExporterAdapter.ExportFloatingObjectPicture(
  AExporter: TdxGraphicsDocumentLayoutExporter; ABox: TdxFloatingObjectBox; AImage: TdxOfficeImage);
var
  AImgBounds: TRect;
begin
  ...
  begin
    if (AImgBounds.Width = 0) or (AImgBounds.Height = 0) then
      Exit;
    AExporter.Painter.DrawImage(AImage, AImgBounds);
...
begin
  if Safe.Cast(ANumberingListBoxWithSeparator.SeparatorBox, TdxTabSpaceBox, ATabSpaceBox) and not ATabSpaceBox.Bounds.IsEmpty then 
...
procedure TdxCharacterBoxLevelDocumentLayoutExporter.ExportInlinePictureBox(ABox: TdxInlinePictureBox);
begin
  ExportSingleCharacterBox(ABox);
end;
