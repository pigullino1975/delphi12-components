...
unit dxRichEdit.DocumentModel.Fields.SequenceField; // for internal use
...
const
  dxThisUnitName = 'dxRichEdit.DocumentModel.Fields.SequenceField';

type
  { TdxSequentialCounterItemAndFieldComparable }

  TdxSequentialCounterItemAndFieldComparable = record
  strict private
    FField: TdxField;
  public
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxSequenceField.Initialize', SysInit.HInstance);{$ENDIF}
  FSwitchesWithArgument := CreateSwitchesWithArgument(['r', 's']);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxSequenceField.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxSequenceField.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxSequenceField.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FSwitchesWithArgument);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxSequenceField.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  ...
  if TdxAlgorithms<TdxSequentialCounterItem>.BinarySearch(FValues, AComparable.CompareTo, AIndex) then
...
begin
  ...
  try
    ...
    try
      ...
      try
        if not Safe.Cast(ATempField, TdxSequenceField, ACalculatedField) then
          Exit(AValue);
