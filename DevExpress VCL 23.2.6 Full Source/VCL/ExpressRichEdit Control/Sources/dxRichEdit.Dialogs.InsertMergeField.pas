...
const
  dxThisUnitName = 'dxRichEdit.Dialogs.InsertMergeField';
...
begin
  if Safe.Cast(Control.InnerControl.CreateCommand(TdxRichEditCommandId.InsertMailMergeField), TdxInsertMergeFieldCommand, ACommand) then
  try
    ACommand.FieldArgument := TdxMergeFieldName(lbMergeFields.ItemObject).Name;
    ACommand.Execute;
  finally
    ACommand.Free;
  end;
