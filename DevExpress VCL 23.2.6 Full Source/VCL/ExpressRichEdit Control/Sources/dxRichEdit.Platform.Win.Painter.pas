...
unit dxRichEdit.Platform.Win.Painter; // for internal use
...
type
  ...
  TdxDrawStringProc = procedure (AHdc: THandle; const AText: string; AFontInfo: TdxFontInfo; const ABounds: TRect; AStringFormat: TdxGPStringFormat) of object;
  ...
  public
    ...
    procedure PushPixelOffsetMode(AHighQuality: Boolean); virtual; abstract;
  ...
  public
    ...
    procedure PushPixelOffsetMode(AHighQuality: Boolean); override;
    procedure PopPixelOffsetMode; override;
  end;

  { TdxGdiPlusPainter }

  TdxGdiPlusPainter = class(TdxPainter)
  strict private
  ...
  protected
    ...
    class function CalculateGdiPlusYCorrection(AFontInfo: TdxFontInfo): Integer;
    function GetDpiY: Integer; override;
  ...
  protected
    ...
    procedure DefaultDrawStringImplementation(AHdc: THandle; const AText: string; AFontInfo: TdxFontInfo;
      const ABounds: TRect; AStringFormat: TdxGPStringFormat); virtual;
    procedure SpacesPlaceholdersDrawStringImplementation(AHdc: THandle; const AText: string; AFontInfo: TdxFontInfo;
      const ABounds: TRect; AStringFormat: TdxGPStringFormat);
    procedure DrawStringImpl(const AText: string; AFontInfo: TdxFontInfo; AForeColor: TdxAlphaColor;
      const ABounds: TRect; AStringFormat: TdxGPStringFormat; AImpl: TdxDrawStringProc);
  ...
  public
    ...
    procedure DrawStringCore(AHdc: THandle; const AText: string; const ABounds: TRect; ATextInfo: TdxTextViewInfo;
      AFontInfo: TdxFontInfo); virtual;
...
const
  dxThisUnitName = 'dxRichEdit.Platform.Win.Painter';

{$EXCESSPRECISION OFF}

function GetDrawingColor(AColor: TdxAlphaColor): TdxAlphaColor; inline;
begin
...
begin
  ...
  begin
    APoints[0].X := Int(APoints[0].X + 0.5);
    APoints[0].Y := Int(APoints[0].Y + 0.5);
...
begin
  ...
  for I := Length(AHeights) downto 1 do
    if AHeights[I - 1] <> 0 then
      APoints[I].Y := Math.Max(Int(APoints[I].Y - APoints[0].Y + 0.5), 1)
    else
      APoints[I].Y := 0;
  APoints[0].Y := 0;
...
begin
  ...
  for I := Length(AWidths) downto 1 do
    if AWidths[I - 1] <> 0 then
      APoints[I].X := Math.Max(Int(APoints[I].X - APoints[0].X + 0.5), 1)
    else
      APoints[I].X := 0;
  APoints[0].X := 0;
  TransformToLayoutUnits(APoints);
...
procedure TdxEmptyPainter.PushPixelOffsetMode(AHighQuality: Boolean);
begin
end;
...
class function TdxGdiPlusPainter.CreateStringFormat: TdxGPStringFormat;
begin
...
function TdxGdiPlusPainter.CorrectTextDrawingBounds(AFontInfo: TdxFontInfo; const ATextBounds: TRect): TRect;
begin
  Result := ATextBounds;
  Result.Offset(0, CalculateGdiPlusYCorrection(AFontInfo));
end;

class function TdxGdiPlusPainter.CalculateGdiPlusYCorrection(AFontInfo: TdxFontInfo): Integer;
begin
  Result := AFontInfo._Free;
  if not AFontInfo.IsCJKFont then 
    Dec(Result, AFontInfo.DrawingOffset);
end;
...
var
  AGdiFontInfo: TdxFontInfo absolute AFontInfo;
...
procedure TdxGdiPlusPainter.PushPixelOffsetMode(AHighQuality: Boolean);
begin
...
begin
  if HasTransform then
    ...
    DrawStringImpl(AText, TdxFontInfo(AFontInfo), TextForeColor, ABounds, StringFormat, SpacesPlaceholdersDrawStringImplementation);
end;
...
procedure TdxGdiPainter.DefaultDrawStringImplementation(AHdc: THandle; const AText: string;
  AFontInfo: TdxFontInfo; const ABounds: TRect; AStringFormat: TdxGPStringFormat);
begin
...
procedure TdxGdiPainter.DrawStringImpl(const AText: string; AFontInfo: TdxFontInfo; AForeColor: TdxAlphaColor;
  const ABounds: TRect; AStringFormat: TdxGPStringFormat; AImpl: TdxDrawStringProc);
var
  AHdc: THandle;
  ABoundsRect: TRect;
...
begin
  ...
  try
    ...
    ADrawBounds := ABounds;
    ADrawBounds.Offset(0, AFontInfo.AscentAndFree);
    APrevValue := SetTextAlign(AHdc, TA_BASELINE or TA_LEFT);
    ExtTextOut(AHdc, ADrawBounds.Left, ADrawBounds.Top, ETO_IGNORELANGUAGE, ABounds, AText, Length(AText), nil);
...
begin
  ...
  APlacement := GetCharacterPlacement(AHdc, PChar(AText), ALength, 0, AGcpResults, GCP_USEKERNING or GCP_LIGATE);
  if (APlacement = 0) and (ALength > 0) then
  begin
...
begin
  ATextInfo := Measurer.TextViewInfoCache.TryGetTextViewInfo(AText, AFontInfo);
...
procedure TdxRichEditGdiPainter.DrawStringCore(AHdc: THandle; const AText: string; const ABounds: TRect;
  ATextInfo: TdxTextViewInfo; AFontInfo: TdxFontInfo);
var
  Y, APrevAlign: Integer;
begin
  Y := ABounds.Y;
  Inc(Y, AFontInfo.AscentAndFree);
  APrevAlign := SetTextAlign(AHdc, TA_BASELINE or TA_LEFT);
  try
    ExtTextOut(AHdc, ABounds.Left, Y,
      ETO_GLYPH_INDEX or ETO_IGNORELANGUAGE,
      @ABounds, @ATextInfo.Glyphs[0], ATextInfo.GlyphCount, @ATextInfo.CharacterWidths[0]);
  finally
    SetTextAlign(AHdc, APrevAlign);
  end;
end;
