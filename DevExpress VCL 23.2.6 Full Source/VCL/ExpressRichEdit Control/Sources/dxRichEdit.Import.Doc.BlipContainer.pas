...
{                                                                    }
{********************************************************************}
unit dxRichEdit.Import.Doc.BlipContainer; // for internal use
...
type
  ...
  strict private const
    FirstBlipTypeCode = TdxBlipTypeCodes.BlipEmf;
    LastBlipTypeCode = TdxBlipTypeCodes.BlipJpeg2;
  strict private class var
    TypeCodeTranslationTable: array[FirstBlipTypeCode..LastBlipTypeCode] of TdxBlipBaseClass;
    ImageFormatTranslationTable: array[TdxOfficeImageFormat] of TdxBlipBaseClass;
  strict private
  ...
  protected
    ...
    function PixelsToEmu(APixels: Integer): Integer;
  public
...
uses
  System.ZLib,
  Math, Types, Contnrs, dxZIPUtils, dxTypeHelpers, dxEncoding,
  dxRichEdit.Import.Doc.OfficeArtContent, dxRichEdit.Utils.Exceptions;

const
  dxThisUnitName = 'dxRichEdit.Import.Doc.BlipContainer';
  ...
  begin
    ...
    try
      ...
      try
        ...
        if (MetafileHeader.HeightInEmus <> 0) or (MetafileHeader.WidthInEmus <> 0) then
        begin
          AClipWidth := FMetafileHeader.WidthInHundredthsOfMillimeter;
          AClipHeight := FMetafileHeader.HeightInHundredthsOfMillimeter;
        end
        else
        begin
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxBlipFactory.Initialize', SysInit.HInstance);{$ENDIF}
  TypeCodeTranslationTable[TdxBlipTypeCodes.BlipEmf] := TdxBlipEmf;
  TypeCodeTranslationTable[TdxBlipTypeCodes.BlipWmf] := TdxBlipWmf;
  TypeCodeTranslationTable[TdxBlipTypeCodes.BlipPict] := TdxBlipPict;
  TypeCodeTranslationTable[TdxBlipTypeCodes.BlipJpeg] := TdxBlipJpeg;
  TypeCodeTranslationTable[TdxBlipTypeCodes.BlipPng] := TdxBlipPng;
  TypeCodeTranslationTable[TdxBlipTypeCodes.BlipDib] := TdxBlipDib;
  TypeCodeTranslationTable[TdxBlipTypeCodes.BlipTiff] := TdxBlipTiff;
  TypeCodeTranslationTable[TdxBlipTypeCodes.BlipJpeg2] := TdxBlipJpeg;

  ImageFormatTranslationTable[TdxOfficeImageFormat.Bmp] := TdxBlipDib;
  ImageFormatTranslationTable[TdxOfficeImageFormat.MemoryBmp] := TdxBlipDib;
  ImageFormatTranslationTable[TdxOfficeImageFormat.Emf] := TdxBlipEmf;
  ImageFormatTranslationTable[TdxOfficeImageFormat.Jpeg] := TdxBlipJpeg;
  ImageFormatTranslationTable[TdxOfficeImageFormat.Png] := TdxBlipPng;
  ImageFormatTranslationTable[TdxOfficeImageFormat.Gif] := TdxBlipPng;
  ImageFormatTranslationTable[TdxOfficeImageFormat.Tiff] := TdxBlipTiff;
  ImageFormatTranslationTable[TdxOfficeImageFormat.Wmf] := TdxBlipWmf;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxBlipFactory.Initialize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  ...
  case ATypeCode of
    TdxBlipTypeCodes.FileBlipStoreEntry:
      Result := TdxFileBlipStoreEntry.Create(AReader, AEmbeddedReader, AHeader, AImageCreator);
    Low(TypeCodeTranslationTable) .. High(TypeCodeTranslationTable):
      begin
        ABlipClass := TypeCodeTranslationTable[ATypeCode];
        if ABlipClass <> nil then
...
var
  ABlipClass: TdxBlipBaseClass;
begin
  ABlipClass := ImageFormatTranslationTable[AImage.RawFormat];
  if ABlipClass <> nil then
...
begin
  ...
  Result.Top := 0;
  Result.Bottom := AHeightInPixels;
  Result.HeightInEmus := PixelsToEmu(AHeightInPixels);
  Result.WidthInEmus := PixelsToEmu(AWidthInPixels);
end;

function TdxBlipMetaFile.PixelsToEmu(APixels: Integer): Integer;
const
  PixelInEmu = 9525;
begin
  Result := APixels * PixelInEmu;
end;
