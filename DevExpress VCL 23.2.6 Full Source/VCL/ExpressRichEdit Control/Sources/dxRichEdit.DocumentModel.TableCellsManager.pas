...
unit dxRichEdit.DocumentModel.TableCellsManager; // for internal use
...
type
  ...
  protected
    ...
    function GetSubTree(AStart, AEnd: TdxParagraphIndex; AFromNestedLevel: Integer): TdxTableCellNode;
    procedure RemoveCellNode(ATargetCellNode, ASourceCellNode: TdxTableCellNode; AStart, AEnd: TdxParagraphIndex); 
  ...
  {$ENDREGION}
    ...
    procedure OnRunUnmerged(AParagraphIndex: TdxParagraphIndex; ARunIndex: TdxRunIndex; ADeltaRunLength: Integer); overload;
    procedure OnRunMerged(AParagraphIndex: TdxParagraphIndex; ARunIndex: TdxRunIndex; ADeltaRunLength: Integer); overload;
    procedure OnRunJoined(AParagraphIndex: TdxParagraphIndex; AJoinedRunIndex: TdxRunIndex; ASplitOffset, ATailRunLength: Integer); overload;
    procedure OnRunSplit(AParagraphIndex: TdxParagraphIndex; ARunIndex: TdxRunIndex; ASplitOffset: Integer); overload;
    procedure OnRunRemoved(AParagraphIndex: TdxParagraphIndex; ARunIndex: TdxRunIndex; ALength, AHistoryNotificationId: Integer); overload;
    procedure OnRunInserted(AParagraphIndex: TdxParagraphIndex; ANewRunIndex: TdxRunIndex; ALength, AHistoryNotificationId: Integer); overload;
    ...
    procedure OnParagraphMerged(ASectionIndex: TdxSectionIndex; AParagraphIndex: TdxParagraphIndex; ARunIndex: TdxRunIndex;
      AHistoryNotificationId: Integer); overload;
    procedure OnParagraphInserted(ASectionIndex: TdxSectionIndex; AParagraphIndex: TdxParagraphIndex; ARunIndex: TdxRunIndex;
      ACell: TdxCustomTableCell; AIsParagraphMerged: Boolean; AActualParagraphIndex: TdxParagraphIndex; AHistoryNotificationId: Integer); overload;
  ...
  public
    ...
    destructor Destroy; override;
    function IsInCell(AParagraph: TdxParagraphBase): Boolean;
    function GetCell(AParagraph: TdxParagraphBase): TdxTableCell;
    procedure Clear;
    procedure ResetCachedTableLayoutInfo(AFrom, ATo: TdxParagraphIndex);
    function GetCellByNestingLevel(AParagraphIndex: TdxParagraphIndex; ANestingLevel: Integer): TdxTableCell;
    function GetCellsByParagraphIndex(AParagraphIndex: TdxParagraphIndex; ANestedLevel: Integer): TdxTableCellList;
...
const
  dxThisUnitName = 'dxRichEdit.DocumentModel.TableCellsManager';

type
  { TdxTableCellNodeAndCellComparer }

  TdxTableCellNodeAndCellComparer = record
  strict private
  ...
  TdxTableCellNodeAndParagraphIndexComparer = record
  strict private
...
function TdxTableCellNodeAndParagraphIndexComparer.CompareTo(const AOther: TdxTableCellNode): Integer;
begin
...
begin
  ...
  AChildIndex := ChildNodes.BinarySearch(AComparer.CompareTo);
...
begin
  ...
  AIndex := ChildNodes.BinarySearch(AComparer.CompareTo);
  if AIndex < 0 then
    Exit(ACellFromPreviousLevel);
...
begin
  ...
  AStartIndex := ChildNodes.BinarySearch(AComparer.CompareTo);

  if AStartIndex < 0 then
  begin
    AComparer := TdxTableCellNodeAndParagraphIndexComparer.Create(AEnd);
    AEndIndex := ChildNodes.BinarySearch(AComparer.CompareTo);
    if AEndIndex < 0 then
    begin
...
begin
  ...
  ACellIndex := ChildNodes.BinarySearch(AComparer.CompareTo);
...
begin
  ...
  begin
    AChildNodes[I].Cell.Table.RecalcNestedLevel;
...
begin
  ...
  AFromIndex := ChildNodes.BinarySearch(AComparer.CompareTo);
  ...
  AToIndex := ChildNodes.BinarySearch(AComparer.CompareTo);
  if AToIndex >= 0 then
  begin
