...
unit dxRichEdit.DocumentModel.PieceTable; // for internal use
...
uses
  System.UITypes,
  Types, Windows, SysUtils, Classes, Generics.Defaults, Generics.Collections,
  Graphics, Controls,
...
type
  ...
  TdxCalculateDocumentVariableEventRouter = record
  strict private
  ...
  TdxExtendedDocumentProperties = record
  private
    FPages: Integer;
    procedure SetPages(APages: Integer);
  public
    class function Create: TdxExtendedDocumentProperties; static;
  ...
  public
    ...
    procedure UnsubscribeMailMergeDataControllerEvents;
  ...
  { TdxRangeEditableChecker }

  TdxRangeEditableChecker = class
  strict private
    FPieceTable: TdxPieceTable;
    function GetDocumentModel: TdxDocumentModel;
  protected
  ...
  public
    constructor Create(APieceTable: TdxPieceTable);
    function CanEditRange(AStart, AEnd: TdxDocumentLogPosition): Boolean;
    function CheckPermissionsForDocument(APermissions: TdxRangePermissionCollection; AStart, AEnd: TdxDocumentLogPosition): Boolean;
    function CheckPermissionsCore(APermissions: TdxRangePermissionCollection; AStart, AEnd: TdxDocumentLogPosition; const ACheck: TFunc<TdxRangePermission, TdxDocumentLogPosition, TdxDocumentLogPosition, Boolean>): Boolean;
    function CheckInsidePermission(ARangePermission: TdxRangePermission; AStart, AEnd: TdxDocumentLogPosition): Boolean;
    function CheckIntersectsWithPermission(ARangePermission: TdxRangePermission; AStart, AEnd: TdxDocumentLogPosition): Boolean;
  end;
  ...
  strict private
    ...
    FRangeEditableChecker: TdxRangeEditableChecker;
  ...
  protected
    ...
    property RangeEditableChecker: TdxRangeEditableChecker read FRangeEditableChecker;
  public
    ...
    procedure InsertFootNoteRunCore(AParagraphIndex: TdxParagraphIndex; ALogPosition: TdxDocumentLogPosition;
      AFormatting: TdxFieldResultFormatting; ANoteIndex: Integer; AInserter: TdxObjectInserter);
    ...
    function CreateCustomMarkCore(APosition: TdxDocumentLogPosition; AUserData: TObject): TdxCustomMark;
    ...
    function JoinTableWithPrevious(ATable: TdxTable): TdxTable;
  ...
  strict private
  private
    function GetItem(Index: Integer): TdxParagraph; inline;
  public
    function First: TdxParagraph; reintroduce; inline;
    function Last: TdxParagraph; reintroduce; inline;
  ...
  strict private
    ...
    FTableSelectionStructure: TdxSelectedTableStructureBase;
    ...
    procedure SetTableSelectionStructure(AValue: TdxSelectedTableStructureBase);
  protected
  ...
  public
    ...
    property SelectedCells: TdxSelectedTableStructureBase read FTableSelectionStructure write SetTableSelectionStructure;
    property PieceTable: TdxPieceTable read GetPieceTable;
...
const
  dxThisUnitName = 'dxRichEdit.DocumentModel.PieceTable';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxDocumentModel.Initialize', SysInit.HInstance);{$ENDIF}
  FDefaultUnderlineRepository := TdxUnderlineRepository.Create;
  FDefaultStrikeoutRepository := TdxStrikeoutRepository.Create;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxDocumentModel.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxDocumentModel.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxDocumentModel.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FDefaultUnderlineRepository);
  FreeAndNil(FDefaultStrikeoutRepository);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxDocumentModel.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  ...
  begin
    UnsubscribeMailMergeDataControllerEvents;
    FreeAndNil(FMailMergeDataController);
  end;
...
begin
  if Safe.Cast(Selection.PieceTable.ContentType, TdxSectionHeaderFooterBase, AHeaderFooter) then
  begin
...
begin
  TdxAlgorithms1<TdxCustomSection>.BinarySearch(Sections,
    function (const ASection: TdxCustomSection): Integer
    begin
      Result := ASection.LastParagraphIndex - AParagraphIndex
    end, Result);
end;
...
procedure TdxDocumentModel.UnsubscribeMailMergeDataControllerEvents;
begin
...
constructor TdxRangeEditableChecker.Create(APieceTable: TdxPieceTable);
begin
  FPieceTable := APieceTable;
end;

function TdxRangeEditableChecker.GetDocumentModel: TdxDocumentModel;
begin
  Result := FPieceTable.DocumentModel;
end;

function TdxRangeEditableChecker.CanEditRange(AStart, AEnd: TdxDocumentLogPosition): Boolean;
begin
  if not DocumentModel.IsDocumentProtectionEnabled then
    Exit(True);
  Result := CheckPermissionsForDocument(FPieceTable.RangePermissions, AStart, AEnd);
end;

function TdxRangeEditableChecker.CheckPermissionsForDocument(APermissions: TdxRangePermissionCollection;
  AStart, AEnd: TdxDocumentLogPosition): Boolean;
begin
  Result := CheckPermissionsCore(APermissions, AStart, AEnd, CheckInsidePermission);
end;

function TdxRangeEditableChecker.CheckPermissionsCore(APermissions: TdxRangePermissionCollection;
  AStart, AEnd: TdxDocumentLogPosition; const ACheck: TFunc<TdxRangePermission, TdxDocumentLogPosition, TdxDocumentLogPosition, Boolean>): Boolean;
var
  I: Integer;
begin
  for I := 0 to APermissions.Count - 1 do
    if ACheck(APermissions[I], AStart, AEnd) then
      Exit(True);
  Result := False;
end;

function TdxRangeEditableChecker.CheckInsidePermission(ARangePermission: TdxRangePermission;
  AStart, AEnd: TdxDocumentLogPosition): Boolean;
begin
  Result := ARangePermission.Contains(AStart, Min(AEnd, FPieceTable.DocumentEndLogPosition)) and
    FPieceTable.IsPermissionGranted(ARangePermission);
end;

function TdxRangeEditableChecker.CheckIntersectsWithPermission(ARangePermission: TdxRangePermission;
  AStart, AEnd: TdxDocumentLogPosition): Boolean;
begin
  Result := ARangePermission.IntersectsWith(AStart, AEnd) and FPieceTable.IsPermissionGranted(ARangePermission);
end;
...
begin
  ...
  FRangeEditableChecker := TdxRangeEditableChecker.Create(Self);
  ...
  FFootNoteRunInserter := TdxPieceTableModifiers.CreateFootNoteRunInserter(Self);
  FEndNoteRunInserter := TdxPieceTableModifiers.CreateEndNoteRunInserter(Self);
...
begin
  ...
  FreeAndNil(FRangeEditableChecker);
...
function TdxPieceTable.CreateCustomMarkCore(APosition: TdxDocumentLogPosition;
...
begin
  ...
  begin
    AParagraph.ResetNumberingListIndex(NumberingListIndexListIndexNotSet);
...
begin
  if Safe.Cast(ARun, TdxInlinePictureRun, AInlinePictureRun) then
...
begin
  Result := CanEditRange(AStart, AStart + ALength);
end;

function TdxPieceTable.CanEditRange(AStart, AEnd: TdxDocumentLogPosition): Boolean;
begin
  Result := RangeEditableChecker.CanEditRange(AStart, AEnd);
end;
...
begin
  Result := TdxPieceTableModifiers.CreateTextInserter(Self);
end;
...
begin
  ...
  for I := 0 to Runs.Count - 1 do
  begin
    if Safe.Cast(Runs[I], TdxFloatingObjectAnchorRun, AAnchorRun) then
...
var
  AInserter: TdxPieceTableObjectInserter;
begin
  TextBuffer.Append(TdxCharacters.FloatingObjectMark);
  AInserter := TdxPieceTableModifiers.CreateFloatingObjectAnchorInserter(Self);
  try
...
var
  AInserter: TdxSimpleTextInserter;
begin
  TextBuffer.Append(ASymbol);
  AInserter := TdxPieceTableModifiers.CreateFieldSymbolResultInserter(Self);
  try
...
var
  AInserter: TdxParagraphInserter;
begin
  TextBuffer.Append(TdxCharacters.SectionMark);
  AInserter := TdxPieceTableModifiers.CreateSectionInserter(Self);
  try
...
begin
  ...
  try
    ...
    try
      ...
      begin
        if AParagraph.GetOwnNumberingListIndex = NumberingListIndexListIndexNotSet then
        begin
...
var
  AModifier: TdxRunPropertyModifier<TdxAlphaColor>;
begin
  AModifier := TdxPieceTableModifiers.CreateRunBackColorModifier(ABackColor);
  try
...
var
  AModifier: TdxRunPropertyModifier<TFont>;
begin
  AModifier := TdxPieceTableModifiers.CreateRunFontModifier(AFont);
  try
    ApplyCharacterFormatting(ALogPositionStart, ALength, AModifier);
  finally
    AModifier.Free;
...
var
  AModifier: TdxRunPropertyModifier<TdxAlphaColor>;
begin
  AModifier := TdxPieceTableModifiers.CreateRunForeColorModifier(AForeColor);
  try
    ApplyCharacterFormatting(ALogPositionStart, ALength, AModifier);
  finally
    AModifier.Free;
...
var
  ATypedInserter: TdxFootNoteRunInserterBase;
begin
  ATypedInserter := TdxFootNoteRunInserterBase(AInserter);
  Assert((AInserter.ClassName = 'TdxFootNoteRunInserter') or (AInserter.ClassName = 'TdxEndNoteRunInserter'));
  TextBuffer.Append('#');
...
function TdxPieceTable.JoinTableWithPrevious(ATable: TdxTable): TdxTable;
var
...
begin
  ...
  if AFirstParagraphInTableIndex = 0 then
    Exit(ATable);
  APreviousParagraph := Paragraphs[AFirstParagraphInTableIndex - 1];
  ACell := APreviousParagraph.GetCell;
  if (ACell = nil) or (ATable.NestedLevel <> ACell.Table.NestedLevel) then
    Exit(ATable);
  ATopTable := ACell.Table;
  JoinTables(ATopTable, ATable);
  Result := ATopTable;
end;
...
begin
  ...
  if IsInNonStyleList or (GetOwnNumberingListIndex = NumberingListIndexNoNumberingList) then
    ...
    if (AResultParagraph.PieceTable.PreCalculatedNumberingListTexts <> nil) and
        (GetOwnNumberingListIndex = NumberingListIndexListIndexNotSet) and (ParagraphStyle.GetNumberingListIndex >= NumberingListIndexMinValue) then
...
begin
  ...
  Result := ANumList.Levels[GetListLevelIndex].ListLevelProperties.Format <> TdxRichEditNumberingFormat.None; 
end;
...
class function TdxExtendedDocumentProperties.Create: TdxExtendedDocumentProperties;
begin
  Result := Default(TdxExtendedDocumentProperties);
end;
...
procedure TdxSelection.SetTableSelectionStructure(AValue: TdxSelectedTableStructureBase);
begin
  if FTableSelectionStructure <> AValue then
  begin
    TdxSelectedTableStructureBase.Release(FTableSelectionStructure);
    FTableSelectionStructure := AValue;
    TdxSelectedTableStructureBase.AddReference(FTableSelectionStructure);
  end;
end;
...
var
  ASelectedCells: TdxSelectedCellsCollection;
begin
  if Safe.Cast(SelectedCells, TdxSelectedCellsCollection, ASelectedCells) and ASelectedCells.IsNotEmpty then
...
begin
  ...
  if (AServer <> nil) and Safe.Cast(Sender, TdxDocumentModel, AModel) then
...
var
  ALeftCopiedField, ARightCopiedField: TdxField;
begin
  ARightCopiedField := SourcePieceTable.FindFieldByRunIndex(ARunInfo.&End.RunIndex);
  if ARightCopiedField <> nil then
    ChangeRunInfo(ARunInfo, ARightCopiedField);
...
begin
  if not TdxAlgorithms1<T>.BinarySearch(InnerList,
    function (const AOther: T): Integer
    begin
      if APosition < AOther.Start then
        Exit(1);
      if APosition >= AOther.&End then
        Exit(-1);
      Result := 0;
    end, Result) then
    Result := not Result;
...
begin
  if not TdxAlgorithms1<T>.BinarySearch(InnerList,
    function (const AOther: T): Integer
    begin
      if AEnd <= AOther.Start then
        Exit(1);
      if AStart >= AOther.&End then
        Exit(-1);
      Result := 0;
    end, Result) then
    Result := not Result;
...
var
  APieceTable: TdxPieceTable;
begin
  Result := not Safe.Cast(PieceTable, TdxPieceTable, APieceTable) or APieceTable.CanEditRange(AStart.LogPosition, AEnd.LogPosition);
end;
