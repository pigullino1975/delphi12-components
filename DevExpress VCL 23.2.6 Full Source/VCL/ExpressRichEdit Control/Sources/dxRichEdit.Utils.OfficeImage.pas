...
{$ENDREGION}
  ...
  protected
    ...
    function GetTargetPlaceHolder(AOwner: TdxImageCache; APixelTargetWidth, APixelTargetHeight: Integer): TdxOfficeImage;
  ...
  public
    constructor Create(AOwner: TdxImageCache; const AServiceProvider: IdxServiceProvider; AUriList: TArray<string>;
      APixelTargetWidth, APixelTargetHeight: Integer; AAsyncImageLoading: Boolean; ALoadingTimeout: Cardinal = 0); reintroduce; overload;
    constructor Create(AImage: TdxUriOfficeImage; APixelTargetWidth, APixelTargetHeight: Integer); reintroduce; overload;
    destructor Destroy; override;
...
{$R dxRichEdit.Utils.OfficeImage_svg.res}
...
const
  dxThisUnitName = 'dxRichEdit.Utils.OfficeImage';
...
type
  ...
  public
    constructor Create(const AUriService: IdxUriStreamService;
      const AUpdater: IdxUriOfficeImageUpdater; const AUri: TArray<string>; ATimeout: Cardinal; AEndTaskEvent: TSimpleEvent);
    destructor Destroy; override;
    ...
    try
      ...
      begin
        ...
        if AUriService2 <> nil then
          AStream := AUriService2.AsyncGetStream(AUri, FTimeout, ACancelStatus)
        else
          AStream := FUriService.GetStream(AUri, FTimeout);
        if AStream <> nil then
          Break;
...
begin
  ...
  if Safe.Cast(ASmartImage, TdxOfficeImage, AImage) then
  begin
...
begin
  ...
  try
    ...
    try
      ...
      TdxMetafileHelper.DeleteMetafileHandle(AEmf);
    end;
  finally
    AMetafile.Free;
  end;
...
function TdxOfficeImage.IsExportSupported(ARawFormat: TdxOfficeImageFormat): Boolean;
begin
  Result := True;
end;
...
var
  ABmp: GPImage;
begin
  GdipCreateBitmapFromScan0(10, 10, 0, TdxGpPixelFormat(APixelFormat), nil, ABmp);
  try
...
begin
  ...
  begin
    ...
    TdxMetafileHelper.DeleteMetafileHandle(AHmf);
  end
  else
  begin
...
constructor TdxUriOfficeImage.Create(AOwner: TdxImageCache;
  ...
  APixelTargetWidth, APixelTargetHeight: Integer; AAsyncImageLoading: Boolean; ALoadingTimeout: Cardinal);
var
  I: Integer;
  AUri: string;
  AImage: TdxOfficeImage;
  ...
  AStream: TStream;
begin
  AImage := GetTargetPlaceHolder(AOwner, APixelTargetWidth, APixelTargetHeight);
  ...
  begin
    ...
    begin
      ...
      AThreadPoolService.AddTask(TdxUriOfficeImageLoadingTask.Create(AService, FUpdater, FUriList, ALoadingTimeout, FEndLoadingTaskEvent));
    end
    else
    begin
      AStream := nil;
      ...
      begin
        AUri := FUriList[I];
        AStream := AService.GetStream(Uri, ALoadingTimeout);
        if AStream <> nil then
          Break;
      end;
      try
...
constructor TdxUriOfficeImage.Create(AImage: TdxUriOfficeImage; APixelTargetWidth, APixelTargetHeight: Integer);
begin
  if AImage.IsLoaded then
    inherited Create(AImage.Owner, AImage.Image)
  else
    inherited Create(AImage.Owner, GetTargetPlaceHolder(AImage.Owner, APixelTargetWidth, APixelTargetHeight));
  FPixelTargetHeight := APixelTargetHeight;
  FPixelTargetWidth := APixelTargetWidth;
  FUriList := AImage.FUriList;
  Uri := FUriList[0];
  FUpdater := AImage.FUpdater;
  FUpdater.Add(Self);
  if AImage.IsLoaded then
    Update(AImage.Image);
end;
...
begin
  ...
  if dxUseVectorIcons then
    Result.LoadFromResource(HInstance, 'DXRICHEDITIMAGEPLACEHOLDER', 'SVG')
  else
...
function TdxUriOfficeImage.GetTargetPlaceHolder(AOwner: TdxImageCache; APixelTargetWidth, APixelTargetHeight: Integer): TdxOfficeImage;
var
  AHasImage: Boolean;
  ACrc32: Cardinal;
begin
  AHasImage := False;
  Result := CreatePlaceHolder(APixelTargetWidth, APixelTargetHeight);
  try
    ACrc32 := Result.CalculateCrc32;
    AHasImage := AOwner.GetImage(ACrc32) <> nil;
    if not AHasImage then
      AOwner.AddImage(Result, ACrc32);
  finally
    if AHasImage then
      Result.Free;
  end;
  Result := AOwner.GetImage(ACrc32);
end;
