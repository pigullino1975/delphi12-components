...
const
  dxThisUnitName = 'dxSymbolListBox';
...
begin
  ...
  if (Focused) and LookAndFeelPainter.SupportsNativeFocusRect then
...
begin
  FUnicodeChars.Clear;
  AFontCache := TdxFontCacheManager.GetFontCache(TdxDocumentLayoutUnit.Document,
    TdxDocumentLayoutUnitDocumentConverter.DefaultDpi);
  AFontCharacterSet := AFontCache.GetFontCharacterSet(AFontName);
  ...
  begin
    ...
    begin
      ...
      AIsCharCategoryControl := AChar.IsControl;
      AIsCharCategoryPrivateUse := AChar.GetUnicodeCategory = TUnicodeCategory.ucPrivateUse;
...
begin
  ...
  if TdxFontCache.SystemTrueTypeFonts.TryGetValue(Value.ToUpper, AFontInfo) then
  begin
