...
unit dxRichEdit.InnerControl.DrawingStrategy; // for internal use
...
const
  dxThisUnitName = 'dxRichEdit.InnerControl.DrawingStrategy';
...
begin
  if Safe.Cast(Measurer, TdxGdiBoxMeasurerLockHdc, AMeasurer) then
    AMeasurer.ReleaseCachedHdc;

  FreeAndNil(FHdcGraphicsModifier);
  FreeAndNil(FHdcMeasureGraphics);
  inherited Destroy;
end;
