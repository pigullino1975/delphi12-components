...
type
  ...
  public
    ...
    property DeferredUpdateControl: Boolean read FDeferredUpdateControl write FDeferredUpdateControl;
...
const
  dxThisUnitName = 'dxRichEdit.Dialogs.TableProperties';
...
begin
  Dec(FLockCount);
  if (FLockCount = 0) and FDeferredUpdateControl then
    DoPropertiesChanged;
end;
...
procedure TdxCustomTableControlHelper<T>.UpdateCheckbox;
var
  ACheck: TdxNullableBoolean;
begin
...
var
  ...
  AActualWidthUnitType: TdxWidthUnitType;
begin
  UnsubscribeEvents;
  try
    ...
    if not AWidthType.IsNull and (AWidthType.Value = TdxWidthUnitType.FiftiethsOfPercent) then
      AActualWidthUnitType := AWidthType.Value
    else
      AActualWidthUnitType := TdxWidthUnitType.ModelUnits;
    IsValueInPercent := AActualWidthUnitType = TdxWidthUnitType.FiftiethsOfPercent;
    SetValueToEditor(Width);
    ValueType := AActualWidthUnitType;
  finally
    SubscribeEvents;
  end;
end;
