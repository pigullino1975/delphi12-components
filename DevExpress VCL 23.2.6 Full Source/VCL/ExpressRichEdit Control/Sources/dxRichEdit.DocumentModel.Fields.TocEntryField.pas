...
unit dxRichEdit.DocumentModel.Fields.TocEntryField; // for internal use
...
const
  dxThisUnitName = 'dxRichEdit.DocumentModel.Fields.TocEntryField';

{ TdxTocEntryField }

class constructor TdxTocEntryField.Initialize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxTocEntryField.Initialize', SysInit.HInstance);{$ENDIF}
  FSwitchesWithArgument := CreateSwitchesWithArgument(['f', 'l']);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxTocEntryField.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxTocEntryField.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxTocEntryField.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FSwitchesWithArgument);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxTocEntryField.Finalize', SysInit.HInstance);{$ENDIF}
end;
