...
unit dxRichEdit.Import.Html.TextTags; // for internal use
...
uses
  ...
  dxRichEdit.DocumentModel.PieceTable, dxCore;

const
  dxThisUnitName = 'dxRichEdit.Import.Html.TextTags';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxFontTag.Initialize', SysInit.HInstance);{$ENDIF}
  FAttributeTable := AddAttributes;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxFontTag.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxFontTag.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxFontTag.Finalize', SysInit.HInstance);{$ENDIF}
  FAttributeTable.Free;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxFontTag.Finalize', SysInit.HInstance);{$ENDIF}
end;
