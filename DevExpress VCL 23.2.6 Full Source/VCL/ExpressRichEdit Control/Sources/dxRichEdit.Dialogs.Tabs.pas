...
type
  ...
  private
    ...
    function FindTabStopWithClosePosition(ATabFormattingInfo: TdxTabFormattingInfo; ANewTabStopPosition: Integer): TdxNullableValue<TdxTabInfo>;
  ...
  protected const
    ...
    MinimumAllowableTabStopPositionDistance = 4;
  protected
    FIsClearAllTabStopPositions: Boolean;
    ...
    procedure UpdateLabelRemoteTabs;
    procedure UpdateSetAndClearButtons;
    procedure UpdatePosition;
    procedure UpdateRadioGroupAlignment;
    procedure UpdateRadioGroupLeader;
...
const
  dxThisUnitName = 'dxRichEdit.Dialogs.Tabs';
...
begin
  ...
  FIsClearAllTabStopPositions := True;
	UpdateForm;
end;
...
begin
  if ModalResult <> mrOk then
    Exit;
  seDefaultTabStops.ValidateEdit(True);
  btnSetClick(Sender);
  if not DoValidateStopPosition then
    CanClose := False
  else
...
function TdxRichEditTabsDialogForm.FindTabStopWithClosePosition(ATabFormattingInfo: TdxTabFormattingInfo; ANewTabStopPosition: Integer): TdxNullableValue<TdxTabInfo>;
var
  I, ACount: Integer;
  ADelta: Integer;
  ATabInfo: TdxTabInfo;
begin
  ...
  begin
    ATabInfo := ATabFormattingInfo[I];
    ADelta := Abs(ATabInfo.Position - ANewTabStopPosition);
		if ADelta <= MinimumAllowableTabStopPositionDistance then
      Exit(ATabInfo);
  end;
...
begin
  ...
  if S = '' then
    Exit(True);
  teTabStopPosition.ValidateEdit(True);
...
begin
  ...
  UpdateSetAndClearButtons;
  UpdateRadioGroupAlignment;
  UpdateRadioGroupLeader;
  UpdateLabelRemoteTabs;
  SetValueToEditor(seDefaultTabStops, Controller.DefaultTabWidth);
end;

procedure TdxRichEditTabsDialogForm.UpdateLabelRemoteTabs;
var
  ...
  I: Integer;
begin
  if FIsClearAllTabStopPositions then
...
procedure TdxRichEditTabsDialogForm.UpdateRadioGroupAlignment;
var
  ATabInfo: TdxNullableValue<TdxTabInfo>;
begin
  ATabInfo := GetEditedTabInfo;
...
procedure TdxRichEditTabsDialogForm.UpdateRadioGroupLeader;
var
  ATabInfo: TdxNullableValue<TdxTabInfo>;
begin
  ATabInfo := GetEditedTabInfo;
