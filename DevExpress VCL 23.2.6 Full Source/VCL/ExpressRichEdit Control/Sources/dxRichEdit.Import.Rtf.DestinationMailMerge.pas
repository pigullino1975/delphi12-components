...
unit dxRichEdit.Import.Rtf.DestinationMailMerge; // for internal use
...
uses
  Contnrs, dxGenerics,
  dxStringHelper, dxCore;

const
  dxThisUnitName = 'dxRichEdit.Import.Rtf.DestinationMailMerge';

{ TdxMailMergeDestination }

class constructor TdxMailMergeDestination.Initialize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxMailMergeDestination.Initialize', SysInit.HInstance);{$ENDIF}
  FKeywordHT := CreateKeywordTable;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxMailMergeDestination.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxMailMergeDestination.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxMailMergeDestination.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FKeywordHT);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxMailMergeDestination.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxStringPropertyBaseDestination.Initialize', SysInit.HInstance);{$ENDIF}
  FControlCharHT := CreateControlCharTable;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxStringPropertyBaseDestination.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxStringPropertyBaseDestination.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxStringPropertyBaseDestination.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FControlCharHT);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxStringPropertyBaseDestination.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxFieldMapDataDestination.Initialize', SysInit.HInstance);{$ENDIF}
  FKeywordHT := CreateKeywordTable;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxFieldMapDataDestination.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxFieldMapDataDestination.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxFieldMapDataDestination.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FKeywordHT);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxFieldMapDataDestination.Finalize', SysInit.HInstance);{$ENDIF}
end;
