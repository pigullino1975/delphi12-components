...
type
  ...
  ['{8F910E69-A9F9-4106-B398-322ED3398F63}']
    ...
    function CreateSpaceBoxHitTestManager(ABox: TdxBox): TdxBoxHitTestCustomManager;
  ...
  {$REGION 'IdxBoxHitTestCustomCalculator'}
    ...
    function CreateSpaceBoxHitTestManager(ABox: TdxBox): TdxBoxHitTestCustomManager; virtual; abstract;
  ...
  protected
    function GetHasSpaceBox: Boolean; virtual; 
  ...
  public
    ...
    property HasSpaceBox: Boolean read GetHasSpaceBox;
  end;
  ...
  strict private
    function GetItem(Index: TdxListIndex): TdxBox; inline;
  protected
    procedure Grow; override;
  public
    property Items[Index: TdxListIndex]: TdxBox read GetItem;
  end;
  ...
  public
    ...
    procedure RegisterSuccessfulItemHitTest(ACalculator: TdxBoxHitTestCustomCalculator; AItem: TdxBox); virtual; abstract;
  ...
  TdxHiddenTextUnderlineBox = class // for internal use
  strict private
    FStart: Integer;
    FEnd: Integer;
  ...
  TdxHiddenTextUnderlineBoxCollection = class(TdxObjectList<TdxHiddenTextUnderlineBox>); // for internal use
  ...
  TdxUnderlineBox = class(TdxReferencedObject) // for internal use
  strict private
  ...
  TdxUnderlineBoxCollectionBase = class abstract(TdxReferencedObjectList<TdxUnderlineBox>) // for internal use
  public
    procedure MoveVertically(ADeltaY: Integer);
  end;

  TdxUnderlineBoxCollection = class sealed(TdxUnderlineBoxCollectionBase); // for internal use

  { TdxErrorBox }

  TdxErrorBox = class sealed(TdxUnderlineBox) // for internal use
  strict private
  ...
  TdxErrorBoxCollection = class sealed(TdxUnderlineBoxCollectionBase) // for internal use
  strict private
    function GetItem(Index: TdxListIndex): TdxErrorBox;
  public
    property Items[Index: TdxListIndex]: TdxErrorBox read GetItem; default;
  end;
  ...
  TdxSpecialTextBox = class(TdxTextBox) // for internal use
  public
    class function CreateBox: TdxBox; override;
    procedure ExportTo(const AExporter: IdxSimpleDocumentLayoutExporter); override;
  end;
  TdxSpecialTextBoxCollection = class(TdxReferencedObjectList<TdxSpecialTextBox>); // for internal use

  { TdxSimpleRowExtendedBoxes }

  TdxSimpleRowExtendedBoxes = class // for internal use
  strict private
  ...
  public
    ...
    function IsLineBreak: Boolean; override;
    function IsNotWhiteSpaceBox: Boolean; override;
    function IsTableCellRow: Boolean; virtual;
    function IsVisible: Boolean; override;
  ...
  TdxSingleSpaceBox = class(TdxSingleCharacterMarkBox) // for internal use
  strict private
    FMinWidth: Integer;
    function GetBox: TdxBox; inline;
  protected
    function GetMinWidth: Integer; override;
    procedure SetMinWidth(const Value: Integer); override;
    function GetMarkCharacter: Char; override;
    function GetHasSpaceBox: Boolean; override;
  public
    procedure ExportTo(const AExporter: IdxSimpleDocumentLayoutExporter); override;
    function CreateHitTestManager(const ACalculator: IdxBoxHitTestCustomCalculator): TdxBoxHitTestCustomManager; override;
    class function CreateBox: TdxBox; override;
  ...
  TdxCharacterMarkBox = class(TdxMultiPositionBox) // for internal use
  protected
    FTightBounds: TRect;
    function GetMarkCharacter: Char; virtual; abstract;
  ...
  TdxSpaceBoxa = class(TdxCharacterMarkBox) // for internal use
  strict private
    FMinWidth: Integer;
  protected
  ...
  public
    ...
    property Items[Index: TdxListIndex]: TdxBox read GetItem; default;
  end;

  { TdxSimpleParagraphBoxCollection }

  TdxSimpleParagraphBoxCollection = class // for internal use
  strict private
    FBoxCollection: TdxBoxCollection;
    FParagraphStartRunIndex: TdxRunIndex;
    function GetItem(Index: Integer): TdxBox; inline;
  ...
  TdxParagraphAlignmentCalculatorBase = class // for internal use
  ...
  TdxBoxMeasurer = class(TdxCustomBoxMeasurer, IdxObjectMeasurer) // for internal use
  strict private
  ...
  public
    ...
    function MeasureText(const AText: string; AFontInfo: TdxFontInfo): TSize; override;
    ...
    function MeasureCharactersBounds(const AText: string; AFontInfo: TdxFontInfo; const ABounds: TRect): TArray<TRect>; overload; virtual; abstract;
  ...
  TdxVisitableDocumentIntervalBox = class abstract // for internal use
  strict private
  ...
  TdxVisitableDocumentIntervalBoxCollection = class(TdxObjectList<TdxVisitableDocumentIntervalBox>); // for internal use
...
const
  dxThisUnitName = 'dxRichEdit.DocumentModel.Boxes.Simple';
...
begin
  ...
  FBox := ABox;
  TdxBoxBase.AddReference(FBox);
end;

destructor TdxBoxHitTestCustomManager.Destroy;
begin
  TdxBoxBase.Release(FBox);
  inherited Destroy;
end;
...
procedure TdxBox.SetMinWidth(const AValue: Integer);
begin
// do nothing
end;
...
function TdxBox.GetMinWidth: Integer;
begin
  Result := 0;
end;
...
function TdxBox.GetHasSpaceBox: Boolean;
begin
  Result := False;
end;
...
function TdxBoxList.GetItem(Index: TdxListIndex): TdxBox;
begin
{$IFDEF DEBUG}
...
begin
  ...
  TdxAlgorithms1<TdxBoxBase>.BinarySearch(Self, AComparable.CompareTo, Result);
end;
...
begin
  ...
  TdxAlgorithms1<TdxBoxBase>.BinarySearch(Self, AComparable.BoxAndLogPositionCompare, Result);
end;
...
function TdxBoxCollectionBase<T>.GetItem(Index: TdxListIndex): T;
begin
...
function TdxErrorBoxCollection.GetItem(Index: TdxListIndex): TdxErrorBox;
begin
...
function TdxSimpleRow.IsTableCellRow: Boolean;
begin
  Result := False;
end;
...
begin
  ...
  AFontCache := ADocumentModel.FontCache;
  if not AFontCache.ShouldUseDefaultFontToDrawInvisibleCharacter(Result, MarkCharacter) then
  begin
    ARun := TdxTextRunBase(GetRun(APieceTable));
    AFontIndex := ARun.CalculateFontIndexCore(ARun.DocumentModel.DefaultCharacterProperties.FontName);
    Result := AFontCache[AFontIndex];
  end;
end;
...
begin
  Result := ACalculator.CreateSpaceBoxHitTestManager(Self.Box);
end;
...
function TdxSingleSpaceBox.IsSpaceBox: Boolean;
begin
  Result := True;
end;
...
function TdxSingleSpaceBox.GetHasSpaceBox: Boolean;
begin
  Result := True;
end;
...
procedure TdxCharacterBoxCollection.RegisterSuccessfulItemHitTest(ACalculator: TdxBoxHitTestCustomCalculator; AItem: TdxBox);
begin
...
function TdxSpaceBoxa.IsSpaceBox: Boolean;
begin
  Result := True;
end;
...
function TdxSpaceBoxa.GetHasSpaceBox: Boolean;
begin
  Result := True;
end;
...
function TdxBoxCollection.GetItem(Index: TdxListIndex): TdxBox;
begin
{$IFDEF DEBUG}
  if Cardinal(Index) >= Cardinal(Count) then
    Error(@SListIndexError, Index);
...
procedure TdxBoxCollection.RegisterSuccessfulItemHitTest(ACalculator: TdxBoxHitTestCustomCalculator; AItem: TdxBox);
begin
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxParagraphAlignmentCalculatorBase.Initialize', SysInit.HInstance);{$ENDIF}
  ...
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxParagraphAlignmentCalculatorBase.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxParagraphAlignmentCalculatorBase.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxParagraphAlignmentCalculatorBase.Finalize', SysInit.HInstance);{$ENDIF}
  ...
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxParagraphAlignmentCalculatorBase.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  ...
  begin
    ABox := ABoxes[I];
    R := ABox.Bounds;
    if ABox.GetHasSpaceBox then
      R.Width := ABox.GetMinWidth;
    R.MoveToLeft(ALeft);
    Inc(ALeft, R.Width);
    ABox.Bounds := R;
  end;
...
begin
  ...
  begin
    ABox := ABoxes[I];
    if ABox.GetHasSpaceBox then
      Inc(ATotalWidth, ABox.GetMinWidth)
    else
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          Inc(ATotalWidth, ABox.GetMinWidth);
          Inc(ATotalSpaceWidth, ABox.GetMinWidth);
        end
        else
          Inc(ATotalWidth, ABox.Bounds.Width);
      end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if not ABoxes[I].HasSpaceBox then
        ALastNonSpace := I;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      T := AFreeSpace * ABox.GetMinWidth + ARemainder;
      ADelta := T div ATotalSpaceWidth;
      ARemainder := T mod ATotalSpaceWidth;
      R := cxRectSetWidth(R, ABox.GetMinWidth + ADelta);
    end;
    Inc(ALeft, R.Width);
    ABox.Bounds := R;
  end;
