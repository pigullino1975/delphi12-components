...
unit dxRichEdit.Import.Html; // for internal use
{$I cxVer.inc}
...
type
  ...
  strict private
    ...
    FHtmlNamespaces: TdxStringsDictionary;
    FUseRawContent: Boolean;
    FIsAdditionalLineBreakBefore: Boolean;
    FKeepLineBreaksInParagraphEnd: Boolean;
  ...
  protected
    ...
    class function NamespaceKeywordTag(AImporter: TdxHtmlImporter): TdxTagBase; static;

    function GetHtmlKeywordTable: TdxHtmlKeywordTranslatorTable; inline;
    function GetSpecialSymbolTable: TdxHtmlSpecialSymbolTable; inline;
    ...
    function UseRawContent: Boolean; inline;
    ...
    property IsAdditionalLineBreakBefore: Boolean read FIsAdditionalLineBreakBefore  write FIsAdditionalLineBreakBefore;
  public
    ...
    class function GetDefaultEncoding(AStream: TStream): TEncoding; static;
    procedure AppendInlineImage(AImage: TdxOfficeImageReference; AScaleX, AScaleY: Single;
      const ADesiredSize: TSize); overload; virtual;
    ...
    function ApplyEncoding(const AContentText: string): string;
    ...
    property HtmlNamespaces: TdxStringsDictionary read FHtmlNamespaces;
...
type
  ...
  strict private
    ...
    FOpenTag: TdxReferencedObjectContainer<TdxTag>;
  ...
  protected
    ...
    function ProcessTableTag(ATag: TdxTag): Boolean;
  ...
  public
    ...
    procedure Process(AHtmlElement: TdxHtmlElement);
  ...
  protected
    ...
    procedure MoveLastTableOutputPosToEnd;
...
const
  dxThisUnitName = 'dxRichEdit.Import.Html';
...
begin
  ...
  for I := 1 to Length(ARawText) do
    if not ARawText[I].IsWhiteSpace then
      Exit(False);
  Result := True;
end;
...
begin
  ...
  if Corrector.State.CurrentOutput <> CurrentOutput then
    ...
    Corrector.MoveLastTableOutputPosToEnd;
end;
...
procedure TdxHtmlCorrector.MoveLastTableOutputPosToEnd;
begin
  if (FLastOpenedTableState <> nil) and (FLastOpenedTableState.InnerOutput <> nil) then
    FLastOpenedTableState.OnEndCell(FLastOpenedTableState.InnerOutput.Count);
end;
...
begin
  ...
  FHtmlNamespaces := TdxStringsDictionary.Create;
end;

destructor TdxHtmlImporter.Destroy;
begin
  FreeAndNil(FHtmlNamespaces);
  FreeAndNil(FEmptyHyperlinks);
  FreeAndNil(FMarkTable);
  FreeAndNil(FParser);
  FreeAndNil(FUsedNumberingLists);
  FreeAndNil(FProcessBookmarks);
  FreeAndNil(FTablesImportHelper);
  FreeAndNil(FStyleTagCollection);
  FreeAndNil(FTagsStack);
  FreeAndNil(FUniqueUriBasedImages);
  FreeAndNil(FTagCollection);
  FreeAndNil(FProcessHyperlink);
  Element := nil;
  inherited Destroy;
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxHtmlImporter.Initialize', SysInit.HInstance);{$ENDIF}
  FHtmlKeywordTable := CreateHtmlKeywordTable;
  FSpecialSymbolTable := CreateHtmlSpecialSymbolTable;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxHtmlImporter.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxHtmlImporter.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxHtmlImporter.Finalize', SysInit.HInstance);{$ENDIF}
  FHtmlKeywordTable.Free;
  FSpecialSymbolTable.Free;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxHtmlImporter.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  ...
  Result.Add(TdxHtmlTagNameID.Namespace,     NamespaceKeywordTag);
end;
...
class function TdxHtmlImporter.NamespaceKeywordTag(AImporter: TdxHtmlImporter): TdxTagBase;
begin
  Result := TdxNamespaceTag.Create(AImporter);
end;
...
procedure TdxHtmlImporter.ProcessIsEmptyLine;
var
  AParagraphsAllowed, AOldValue: Boolean;
  ACharacterCount, ADefaultSpacingAfter: Integer;
begin
  AParagraphsAllowed := DocumentModel.DocumentCapabilities.ParagraphsAllowed;
  if ((IsEmptyLine and not IsEmptyParagraph) and AParagraphsAllowed) and not FKeepLineBreaksInParagraphEnd then
  begin
    ACharacterCount := 1;
    if IsAdditionalLineBreakBefore then
    begin
      ADefaultSpacingAfter := DocumentModel.UnitConverter.TwipsToModelUnits(240);
      PieceTable.Paragraphs[Position.ParagraphIndex].ParagraphProperties.SpacingAfter := ADefaultSpacingAfter;
      Inc(ACharacterCount);
      IsAdditionalLineBreakBefore := False;
    end;
    Position.LogPosition := Position.LogPosition - ACharacterCount;
    AOldValue := DocumentModel.ForceNotifyStructureChanged;
    DocumentModel.ForceNotifyStructureChanged := True;
    PieceTable.DeleteContent(Position.LogPosition, ACharacterCount, False);
    IsEmptyLine := False;
...
var
  ...
  I: Integer;
begin
  if FEmptyHyperlinks.Count = 0 then
    Exit;
  PieceTable.UpdateIntervals;
  AOldValue := DocumentModel.ForceNotifyStructureChanged;
  DocumentModel.ForceNotifyStructureChanged := True;
  for I := FEmptyHyperlinks.Count - 1 downto 0 do
    PieceTable.DeleteFieldWithoutResult(FEmptyHyperlinks.List[I]);
  DocumentModel.ForceNotifyStructureChanged := AOldValue;
end;
...
begin
  ...
  for I := 0 to ACount - 1 do
    FStyleTagCollection[I].Selector.InvalidateSpecificity;
  ACssElementComparsion :=
    function (const AElement1, AElement2: TdxCssElement): Integer
    var
      ASpecificity1, ASpecificity2: Integer;
    begin
      ASpecificity1 := AElement1.Selector.Specificity;
      ASpecificity2 := AElement2.Selector.Specificity;
      if ASpecificity1 <> ASpecificity2 then
        Result := ASpecificity1 - ASpecificity2
      else
...
begin
  ...
  begin
    ...
    if ATag.Tag is TdxTableTag then
      FUseRawContent := False;
...
var
  ARecalculateHasPreformattedTagInStack, AHasTableTagBeforePreformattedTag: Boolean;
  I: Integer;
begin
  ARecalculateHasPreformattedTagInStack := FHasPreformattedTagInStack and ((TagsStack[AIndex].Tag is TdxPreformattedTag)
    or (TagsStack[AIndex].Tag is TdxTableTag));
  ...
  begin
    ...
    AHasTableTagBeforePreformattedTag := False;
    for I := TagsStack.Count - 1 downto 0 do
    begin
      if (TagsStack[I].Tag is TdxTableTag) and not AHasTableTagBeforePreformattedTag then
        AHasTableTagBeforePreformattedTag := True
      else
    ...
    FUseRawContent := FHasPreformattedTagInStack and not AHasTableTagBeforePreformattedTag;
  end;
end;
...
begin
  AHtmlTag := TdxTag(FElement);
  case AHtmlTag.NameID of
    TdxHtmlTagNameID.Unknown:
      Exit;
    TdxHtmlTagNameID.Namespace:
      if not TdxNamespaceTag.NamespaceContains(Self, AHtmlTag.RawText) then
        Exit;
  end;
  if HtmlKeywordTable.TryGetValue(AHtmlTag.NameID, ATranslator) then
  begin
...
class function TdxHtmlImporter.GetDefaultEncoding(AStream: TStream): TEncoding;
var
  ABuffer: TBytes;
  ASize, AOffset: Int64;
begin
...
begin
  ...
  begin
    if not ((FTag <> nil) and FTag.DecodeStringContent(AContentText, Result)) then
      Result := ReplaceSpecialSymbols(ApplyEncoding(AContentText))
  end
  else
...
begin
  ...
  case Element.ElementType of
    TdxHtmlElementType.OpenTag:
      OpenProcess(ATag);
    TdxHtmlElementType.CloseTag:
      CloseProcess(ATag);
    else
      EmptyProcess(ATag);
  end;
...
begin
  if FUniqueUriBasedImages.TryGetValue(AUri, AResult) then
    Result := TdxUriOfficeImage.Create(AResult, APixelTargetWidth, APixelTargetHeight)
  else
    Result := TdxUriOfficeImage(CreateUriBasedRichEditImageCore(AUri, APixelTargetWidth, APixelTargetHeight).Clone);
end;
...
begin
  ...
  Result := TdxUriOfficeImage.Create(DocumentModel.ImageCache,
    DocumentModel, AUriList, APixelTargetWidth, APixelTargetHeight, Options.AsyncImageLoading, Options.ExternalResourceTimeout);
...
begin
  ...
  IsAdditionalLineBreakBefore := IsEmptyLine;
  IsEmptyLine := False;
end;

function TdxHtmlImporter.ApplyEncoding(const AContentText: string): string;
begin
  Result := TdxHtmlCodePageDecoder.ApplyEncoding(AContentText, Encoding);
end;
...
begin
  APartBefore := TdxStringHelper.Substring(AHtmlContent, 0, AStartFragent);
  APartAfter := TdxStringHelper.Substring(AHtmlContent, AEndFragment);
  if not APartBefore.EndsWith(StartFragmentComment) or not APartAfter.StartsWith(EndFragmentComment) then
    Exit(AHtmlContent);
  AMiddlePart := TdxStringHelper.Substring(AHtmlContent, AStartFragent, AEndFragment - AStartFragent);
  Result := RemoveFragmentMarks(APartBefore) + StartFragmentComment + RemoveFragmentMarks(AMiddlePart) +
    EndFragmentComment + RemoveFragmentMarks(APartAfter);
end;
