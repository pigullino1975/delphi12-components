...
unit dxRichEdit.Import.Html.CSSParser; // for internal use
...
type
  ...
  protected
    ...
    class procedure CssMsoSpacerunKeyword(ACssProperties: TdxCssProperties; const APropertiesValue: TArray<string>); static;
...
const
  dxThisUnitName = 'dxRichEdit.Import.Html.CSSParser';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxCssParser.Initialize', SysInit.HInstance);{$ENDIF}
  FAcssKeywordTable := CreateCssKeywordTable;
  FHtmlColors := CreateHtmlColors;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxCssParser.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxCssParser.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxCssParser.Finalize', SysInit.HInstance);{$ENDIF}
  FAcssKeywordTable.Free;
  FHtmlColors.Free;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxCssParser.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  ...
  Result.Add(ConvertKeyToUpper('mso-spacerun'), CssMsoSpacerunKeyword);
end;
...
begin
  ...
  Result := TdxFontCache.SystemTrueTypeFonts.ContainsKey(UpperCase(AFontName));
...
class procedure TdxCssParser.CssMsoSpacerunKeyword(ACssProperties: TdxCssProperties;
  const APropertiesValue: TArray<string>);
var
  AStyleValue: string;
begin
  if Length(APropertiesValue) > 0 then
    AStyleValue := APropertiesValue[0]
  else
    AStyleValue := 'yes';
  ACssProperties.RawProperties.AddOrSetValue('mso-spacerun', AStyleValue);
end;
