...
unit dxRichEdit.Platform.Win.Control; // for internal use
...
const
  dxThisUnitName = 'dxRichEdit.Platform.Win.Control';
...
begin
  ...
  begin
    ...
    dxBufferedPaintControl(Self, True);
end;
...
var
  ...
  AMousePos: TPoint;
begin
  if CanProcessScrollEvents and IsGestureHelperMessage(Message) then
    Exit;
  ALink := cxAddObjectLink(Self);
  try
    if ((Message.Msg = WM_LBUTTONDOWN) or (Message.Msg = WM_LBUTTONDBLCLK)) and not Dragging then
    begin
      if HandleAllocated and ((Width > 32768) or (Height > 32768)) then
        AMousePos := ScreenToClient(GetMouseCursorPos)
      else
        AMousePos := SmallPointToPoint(TWMMouse(Message).Pos);
      if (IsDesigning and GetDesignHitTest(AMousePos.X, AMousePos.Y, [ssLeft]) or
