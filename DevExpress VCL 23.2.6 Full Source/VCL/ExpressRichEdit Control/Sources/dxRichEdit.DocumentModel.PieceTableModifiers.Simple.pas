...
unit dxRichEdit.DocumentModel.PieceTableModifiers.Simple; // for internal use
...
type
  ...
  strict private
    ...
    procedure InsertInlinePictureRun(AParagraph: TdxParagraphBase; AWhere: TdxRunIndex; AStartIndex: Integer;
      AForceVisible: Boolean);
  protected
    function CreateRunInsertedHistoryItem(APieceTable: TdxSimplePieceTable): TdxInlinePictureRunInsertedHistoryItem; inline;
  public
  ...
  strict private
    FHistoryItemClass: TdxRichEditHistoryItemClass;
    function CreateHistoryItem: TdxMarkRunInsertedHistoryItemBase; inline;
  protected
    constructor Create(APieceTable: TdxCustomPieceTable; AHistoryItemClass: TdxRichEditHistoryItemClass); reintroduce;
  public
    class function CreateFieldResultEndRunInserter(APieceTable: TdxCustomPieceTable): TdxFieldRunInserterBase; static;
    class function CreateFieldCodeStartRunInserter(APieceTable: TdxCustomPieceTable): TdxFieldRunInserterBase; static;
    class function CreateFieldCodeEndRunInserter(APieceTable: TdxCustomPieceTable): TdxFieldRunInserterBase; static;
  ...
  public
    constructor Create(AStyleIndex: Integer; AResetProperties: Boolean = True); overload;
    class function CreateReplaceRunCharacterStyleProperties(AStyleIndex: Integer;
      const ASourceProperties: TdxMergedCharacterProperties): TdxRunCharacterStyleModifier; static;
    class function CreateRunCharacterStyleKeepOldStyleProperties(AStyleIndex: Integer; AApplyDefaultHyperlinkStyle: Boolean;
      const AIgnoredOptions: TdxUsedCharacterFormattingOptions = []): TdxRunCharacterStyleModifier; static;
...
function CreateSeparatorTextRunInserter(APieceTable: TdxCustomPieceTable): TdxSimplePieceTableObjectInserter;
...
const
  dxThisUnitName = 'dxRichEdit.DocumentModel.PieceTableModifiers.Simple';
...
type
  ...
  private
    ...
    function LeaveUseInStyleProperties(AStyle: TdxCharacterStyle;
      const AProperties: TdxMergedCharacterProperties): TdxMergedCharacterProperties;
  public
    constructor Create(AStyleIndex: Integer; const ASourceProperties: TdxMergedCharacterProperties);
    procedure ModifyTextRun(ARun: TdxTextRunBase; ARunIndex: TdxRunIndex); override;
  end;
...
constructor TdxFieldRunInserterBase.Create(APieceTable: TdxCustomPieceTable; AHistoryItemClass: TdxRichEditHistoryItemClass);
begin
  inherited Create(APieceTable);
  FHistoryItemClass := AHistoryItemClass;
end;

class function TdxFieldRunInserterBase.CreateFieldCodeEndRunInserter(APieceTable: TdxCustomPieceTable): TdxFieldRunInserterBase;
begin
  Result := TdxFieldRunInserterBase.Create(APieceTable, TdxFieldCodeEndRunInsertedHistoryItem);
end;

class function TdxFieldRunInserterBase.CreateFieldCodeStartRunInserter(APieceTable: TdxCustomPieceTable): TdxFieldRunInserterBase;
begin
  Result := TdxFieldRunInserterBase.Create(APieceTable, TdxFieldCodeStartRunInsertedHistoryItem);
end;

class function TdxFieldRunInserterBase.CreateFieldResultEndRunInserter(APieceTable: TdxCustomPieceTable): TdxFieldRunInserterBase;
begin
  Result := TdxFieldRunInserterBase.Create(APieceTable, TdxFieldResultEndRunInsertedHistoryItem);
end;

function TdxFieldRunInserterBase.CreateHistoryItem: TdxMarkRunInsertedHistoryItemBase;
begin
  Result := TdxMarkRunInsertedHistoryItemBase(FHistoryItemClass.Create(PieceTable));
end;
...
class function TdxRunCharacterStyleModifier.CreateReplaceRunCharacterStyleProperties(AStyleIndex: Integer;
  const ASourceProperties: TdxMergedCharacterProperties): TdxRunCharacterStyleModifier;
begin
  Result := TdxReplaceRunCharacterStylePropertiesModifier.Create(AStyleIndex, ASourceProperties);
end;

class function TdxRunCharacterStyleModifier.CreateRunCharacterStyleKeepOldStyleProperties(AStyleIndex: Integer;
  AApplyDefaultHyperlinkStyle: Boolean;
  const AIgnoredOptions: TdxUsedCharacterFormattingOptions): TdxRunCharacterStyleModifier;
begin
  Result := TdxRunCharacterStyleKeepOldStylePropertiesModifier.Create(AStyleIndex, AApplyDefaultHyperlinkStyle,
    AIgnoredOptions);
end;
...
constructor TdxReplaceRunCharacterStylePropertiesModifier.Create(AStyleIndex: Integer;
  const ASourceProperties: TdxMergedCharacterProperties);
begin
...
begin
  ...
  try
    ...
    try
      ...
      begin
        AStyleProperties := ADocumentModel.CharacterStyles[NewValue].GetMergedCharacterProperties;
        try
          AStyleOptions := AStyleProperties.Options.Value - FIgnoredOptions;
        finally
          AStyleProperties.Free;
        end;
...
function CreateSeparatorTextRunInserter(APieceTable: TdxCustomPieceTable): TdxSimplePieceTableObjectInserter;
begin
  Result := TdxSeparatorTextRunInserter.Create(APieceTable);
end;
