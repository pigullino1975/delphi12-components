...
const
  dxThisUnitName = 'dxRichEdit.Api.NativeDocument';
...
begin
  ...
  AManager := TdxCopySelectionManager.Create(ADocument.DocumentServer);
  try
...
var
  ...
  ALastRun, APreviousToLastRun: TdxTextRunBase;
  AParagraphs: TdxParagraphCollection;
  ALastParagraph, APreviousToLastParagraph: TdxParagraph;
begin
  ARuns := APieceTable.Runs;
  ALastRun := ARuns.Last;
  APreviousToLastRun := ARuns[ARuns.Count - 2];
  ALastRun.CharacterProperties.CopyFrom(APreviousToLastRun.CharacterProperties);
  ALastRun.CharacterStyleIndex := APreviousToLastRun.CharacterStyleIndex;

  AParagraphs := APieceTable.Paragraphs;
  ALastParagraph := AParagraphs.Last;
  APreviousToLastParagraph := AParagraphs[AParagraphs.Count - 2];
  ALastParagraph.ParagraphProperties.CopyFrom(APreviousToLastParagraph.ParagraphProperties);
  ALastParagraph.ParagraphStyleIndex := APreviousToLastParagraph.ParagraphStyleIndex;
end;
...
begin
  ...
  try
    ...
    Result := TdxNativeProperties.GetNativeParagraphProperties(Self, AFrom, ALength);
  finally
    AList.Free;
  end;
  DocumentModel.BeginUpdate;
...
begin
  CheckValid;
  TdxNativeProperties.ApplyRichEditParagraphProperties(AProperties);
  DocumentModel.EndUpdate;
end;
...
begin
  ...
  ALength := AEnd - AStart;
  Result := TdxNativeProperties.GetNativeCharacterProperties(Self, AStart, ALength);
end;

procedure TdxNativeSubDocument.EndUpdateCharacters(const AProperties: IdxRichEditCharacterProperties);
begin
  CheckValid;
  TdxNativeProperties.ApplyRichEditCharacterProperties(AProperties);
  DocumentModel.EndUpdate;
end;
...
begin
  Result := TdxCopySelectionManager.Create(DocumentServer);
...
begin
  ...
  if ADirection = TdxRichEditSearchDirection.Forward then
    Result := TdxNativeSearchResultBase.CreateNativeSearchResultForward(Self, ATextToFind, AOptions, ARange)
  else
    Result := TdxNativeSearchResultBase.CreateNativeSearchResultBackward(Self, ATextToFind, AOptions, ARange);
end;
...
function TdxNativeDocument.GetDocBytes: TBytes;
begin
  Result := InternalAPI.DocBytes;
end;
...
procedure TdxNativeDocument.SetDocBytes(const Value: TBytes);
begin
  UnsubscribeInternalAPIEventsCore;
  try
    InternalAPI.DocBytes := Value;
  finally
    SubscribeInternalAPIEventsCore;
  end;
end;
...
begin
  ...
  FCharacterStyles := TdxNativeStyleCollections.GetCharacterStyleCollection(Self);
  FParagraphStyles := TdxNativeStyleCollections.GetParagraphStyleCollection(Self);
  FTableStyles := TdxNativeStyleCollections.GetTableStyles(Self);

  FDefaultParagraphProperties := TdxNativeProperties.GetDefaultParagraphProperties(Self, DocumentModel.DefaultParagraphProperties);
  FDefaultCharacterProperties := TdxNativeProperties.GetDefaultCharacterProperties(DocumentModel.DefaultCharacterProperties);
  FDefaultTableProperties := TdxNativeProperties.GetDefaultTableProperties(Self, DocumentModel.DefaultTableProperties);
...
begin
  ...
  if AImportManagerService = nil then
    TdxRichEditExceptions.ThrowInvalidOperationException('Could not find service: IdxDocumentImportManagerService');
...
begin
  ...
  try
    if ASelection = nil then
      Exit;
    AManager := TdxCopySelectionManager.Create(DocumentServer);
    try
...
begin
  ...
  if Safe.Cast(ASubDocument.PieceTable.ContentType, TdxSectionHeaderFooterBase, AHeaderFooter) then
