...
unit dxRichEdit.Export.Rtf; // for internal use
...
{$ENDREGION}
  ...
  protected
    procedure ExportCharacterProperties(const ACharacterProperties: TdxMergedCharacterProperties);
  ...
  protected
    ...
    function AddLevelNumber(const AFormatString: string; I: Integer): Integer;
    function AddLevelNumberCore(const AFormatString: string; I: Integer): Integer;
    class function DoubleBrackets(const AFormatString: string; I: Integer): Boolean;
...
const
  dxThisUnitName = 'dxRichEdit.Export.Rtf';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxRtfContentExporter.Finalize', SysInit.HInstance);{$ENDIF}
  ...
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxRtfContentExporter.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxRtfContentExporter.Initialize', SysInit.HInstance);{$ENDIF}
  ...
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxRtfContentExporter.Initialize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxRtfBuilder.Initialize', SysInit.HInstance);{$ENDIF}
  FSpecialMarks := CreateSpecialMarksTable;
  FSpecialPCDataMarks := CreateSpecialPCDataMarksTable;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxRtfBuilder.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxRtfBuilder.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxRtfBuilder.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FSpecialPCDataMarks);
  FreeAndNil(FSpecialMarks);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxRtfBuilder.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
function TdxRtfNumberingListExporter.AddLevelNumber(const AFormatString: string; I: Integer): Integer;
begin
  if DoubleBrackets(AFormatString, I) then
    Exit(AddEscapedChar(AFormatString[I], I) + 1);
  if AFormatString[I] = '\' then
    Exit(AddEscapedChar(AFormatString[I], I));

  Result := AddLevelNumberCore(AFormatString, I);
end;

function TdxRtfNumberingListExporter.AddLevelNumberCore(const AFormatString: string; I: Integer): Integer;
var
  AValue: string;
  AIndex: Integer;
  ACh: Char;
begin
  AValue := '';
  AIndex := TdxStringHelper.IndexOfAny(AFormatString, ['s', ':'], I);
  AValue := TdxStringHelper.Substring(AFormatString, I, AIndex - I);
  ...
  if AFormatString[AIndex + 1] = ':' then
    Inc(Result);
end;

class function TdxRtfNumberingListExporter.DoubleBrackets(const AFormatString: string; I: Integer): Boolean;
begin
  Result := ((AFormatString[I] = '%') and (AFormatString[I + 1] = '%')) or
    ((AFormatString[I] = '}') and (AFormatString[I + 1] = '}')) or
    ((AFormatString[I] = '{') and (AFormatString[I + 1] = '{'));
end;
...
begin
  Result := TdxListLevelDestination.NumberingFormatsIndexOf(ANumberingFormat);
  if Result < 0 then
    Result := 0;
end;
...
begin
  ...
  AFormatString := StringReplace(ADisplayFormatString, '{', '{{', [rfReplaceAll]);
  AFormatString := StringReplace(AFormatString, '}', '}}', [rfReplaceAll]);
  ACount := Length(AFormatString);
  I := 1;
  while I <= ACount do
  begin
    ACh := AFormatString[I];
    if not RtfBuilder.IsSpecialSymbol(ACh) and (ACh <> '%') then
      ...
      I := AddLevelNumber(AFormatString, I);
  end;
end;
...
procedure TdxRtfStyleExporter.ExportCharacterProperties(
  const ACharacterProperties: TdxMergedCharacterProperties);
var
