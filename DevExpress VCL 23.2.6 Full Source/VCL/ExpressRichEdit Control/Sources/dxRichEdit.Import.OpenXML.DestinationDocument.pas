...
unit dxRichEdit.Import.OpenXML.DestinationDocument; // for internal use
...
uses
  ...
  dxRichEdit.Import.OpenXML.DestinationBody, dxCore;

const
  dxThisUnitName = 'dxRichEdit.Import.OpenXML.DestinationDocument';

{ TdxDocumentDestination }

class constructor TdxDocumentDestination.Initialize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxDocumentDestination.Initialize', SysInit.HInstance);{$ENDIF}
  FHandlerTable := CreateElementHandlerTable;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxDocumentDestination.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxDocumentDestination.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxDocumentDestination.Finalize', SysInit.HInstance);{$ENDIF}
  FHandlerTable.Free;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxDocumentDestination.Finalize', SysInit.HInstance);{$ENDIF}
end;
