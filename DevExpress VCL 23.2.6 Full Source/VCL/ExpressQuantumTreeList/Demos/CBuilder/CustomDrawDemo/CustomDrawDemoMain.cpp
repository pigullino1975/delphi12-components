...
void TCustomDrawDemoMainForm::DrawRectWithBorders(TcxCanvas *ACanvas,
  Cxgeometry::TcxBorders ABorders, TColor AColor, const TRect &ARect)
{
...
/* remove/add the closing slash on this line to disable/enable the following code*/
  ...
  if (AViewInfo->Button){
    ...
    ACanvas->FrameRect(ARect, clBtnText, 1, Cxgeometry::TcxBorders()<<bLeft<<bTop<<bRight<<bBottom, False);
  ...
  const TRect &R)
{
   ...
   ACanvas->FrameRect(cxRectInflate(R, -1, -1), clBtnText, 1, Cxgeometry::TcxBorders()<<bLeft<<bTop<<bRight<<bBottom, False);
   return Result;
}
//---------------------------------------------------------------------------
