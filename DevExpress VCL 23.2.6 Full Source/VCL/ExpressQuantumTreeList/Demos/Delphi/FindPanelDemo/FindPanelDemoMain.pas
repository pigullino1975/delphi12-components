...
type
  TfrmMain = class(TDemoBasicMainForm)
    ...
    cbShowPrevAndNextButtons: TcxCheckBox;
    lbBehavior: TcxLabel;
...
procedure TfrmMain.cbeLayoutPropertiesEditValueChanged(Sender: TObject);
begin
  if cbeLayout.ItemIndex = 1 then
    TreeList.FindPanel.Layout := fplCompact
  else
    TreeList.FindPanel.Layout := fplDefault;
end;
...
begin
  ...
  TreeList.DataController.FindCriteria.Text := 'cali +manager';
