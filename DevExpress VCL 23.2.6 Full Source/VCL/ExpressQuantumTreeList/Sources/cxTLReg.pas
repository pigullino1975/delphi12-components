...
uses
  dxCoreReg, cxLibraryReg, cxEditPropEditors, dxCoreClasses, Math;

const
  dxThisUnitName = 'cxTLReg';
...
begin
  ...
  begin
    ...
    if AOperations.SupportItemsEditor then
      ...
      if AOperations.SupportCreateAllItems {and (not IsLinkable)} then
      begin
        ...
        begin
          ...
          FItems.AddObject('(no data linked)', TObject(-2));
      end;
...
begin
  if InRange(Index, 0, FItems.Count - 1) and (FItems.Objects[Index] = TObject(-2)) then
    AItem.Enabled := False;
...
begin
  ...
  RegisterPropertyEditor(TypeInfo(TcxTreeListColumn), TcxTreeListOptionsBehavior,
    'IncSearchItem', TcxTreeListColumnProperty);
