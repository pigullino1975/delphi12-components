...
uses
  Windows, SysUtils, Messages, Classes, cxClasses, cxCustomPivotGrid, cxGridChartView, Variants, Forms,
  cxControls, cxVariants, dxCore;
...
type
  ...
  strict private
    function GetItem(AIndex: TdxListIndex): TcxPivotGridChartCellInfo;
  public
    property Items[Index: TdxListIndex]: TcxPivotGridChartCellInfo read GetItem; default;
  end;
  ...
  public
    ...
    procedure AddCell(const AValue: Variant; ADataField: TcxPivotGridField; ARow, AColumn: TcxPivotGridViewDataItem);

    property Cells: TcxPivotGridChartCellInfosList read FCells;
    property Columns: TStringList read FColumns;
    property DataFields: TList read FDataFields;
  ...
  strict private
    ...
    function ValidateValueForSummary(var AValue: Variant): Boolean;
    function GetDataFieldFromViewData(ADataBuilder: TcxPivotGridDataBuilder; AItem: TcxPivotGridViewDataItem): TcxPivotGridField;
    function GetDataFieldValue(ACol, ARow: TcxPivotGridViewDataItem; AViewData: TcxPivotGridViewData;
      ACell: TcxPivotGridCrossCellSummary; var ADataField: TcxPivotGridField; var AValue: Variant): Boolean;
...
const
  dxThisUnitName = 'cxPivotGridChartConnection';
...
function TcxPivotGridChartCellInfosList.GetItem(AIndex: TdxListIndex): TcxPivotGridChartCellInfo;
begin
  ...
  begin
    ...
    for J := 0 to AViewData.RowCount - 1 do
      ...
      begin
        ...
        if GetDataFieldValue(AColumn, ARow, AViewData, AViewData.Cells[J, I], ADataField, AValue) then
        begin
          if SourceForCategories = sfcRows then
            AStorage.AddCell(AValue, ADataField, AColumn, ARow)
          else
            AStorage.AddCell(AValue, ADataField, ARow, AColumn);
        end;
      end;
  end;
end;
  ...
  try
    ...
    try
      ...
      begin
        ...
        ASeries.ValueCaptionFormat := TcxPivotGridField(AStorage.DataFields[I]).DisplayFormat
      end;
    finally
      AHelper.Free;
    end;
...
function TcxPivotGridChartConnection.ValidateValueForSummary(var AValue: Variant): Boolean;
begin
  Result := VarIsNumericEx(AValue) or VarIsDate(AValue) or VarIsNull(AValue);
  if not Result then
    AValue := Null;
end;
...
begin
  ...
  begin
    ...
    Result := ValidateValueForSummary(AValue);
  end;
end;
...
procedure TcxPivotGridChartConnectionStorage.AddCell(
  const AValue: Variant; ADataField: TcxPivotGridField; ARow, AColumn: TcxPivotGridViewDataItem);
begin
  if ARow.Tag = 0 then
  begin
    ARow.Tag := Rows.AddObject(GetDataItemDisplayText(ARow), ARow) + 1;
    DataFields.Add(ADataField);
  end;
