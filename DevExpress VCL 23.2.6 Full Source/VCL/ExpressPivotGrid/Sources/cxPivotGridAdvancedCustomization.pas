...
uses
  ...
  cxCustomPivotGrid, cxPivotGridCustomization, cxGraphics, dxCoreGraphics, cxControls, cxLookAndFeels,
  cxLookAndFeelPainters, cxContainer, cxEdit, cxGroupBox, cxButtons, cxCheckBox, cxLabel,
  cxSplitter, dxGDIPlusClasses, cxImageList, cxImage, dxMessages, dxTypeHelpers;
...
type
  ...
  TcxPivotGridAdvancedCustomizationForm = class(TcxPivotGridCustomCustomizationForm)
    ...
    ilLayoutChooser_svg: TcxImageList;
    imgFilterArea_svg: TcxImage;
    imgColumnArea_svg: TcxImage;
    imgDataAdea_svg: TcxImage;
    imgRowArea_svg: TcxImage;
...
const
  dxThisUnitName = 'cxPivotGridAdvancedCustomization';
...
constructor TcxPivotGridCustomizationFormLayoutChooser.Create(AFieldChooser: TcxPivotGridAdvancedCustomizationForm);
begin
  inherited Create;
...
begin
  ...
  if dxUseVectorIcons then
    PopupMenu.Images := FieldChooser.ilLayoutChooser_svg
  else
...
begin
  ...
  imgColumnArea_svg.Enabled := False;
  imgDataAdea_svg.Enabled := False;
  imgFilterArea_svg.Enabled := False;
  imgRowArea_svg.Enabled := False;

  imgColumnArea.Visible := not dxUseVectorIcons;
  imgDataAdea.Visible := not dxUseVectorIcons;
  imgFilterArea.Visible := not dxUseVectorIcons;
  imgRowArea.Visible := not dxUseVectorIcons;

  imgColumnArea_svg.Visible := dxUseVectorIcons;
  imgDataAdea_svg.Visible := dxUseVectorIcons;
  imgFilterArea_svg.Visible := dxUseVectorIcons;
  imgRowArea_svg.Visible := dxUseVectorIcons;

  if dxUseVectorIcons then
    btnLayoutMode.OptionsImage.Images := ilLayoutChooser_svg
  else
    btnLayoutMode.OptionsImage.Images := ilLayoutChooser;
end;
...
procedure TcxPivotGridAdvancedCustomizationForm.InternalDisableAlign(AControl: TWinControl);
var
  I: Integer;
begin
...
procedure TcxPivotGridAdvancedCustomizationForm.InternalEnableAlign(AControl: TWinControl);
var
  I: Integer;
begin
...
procedure TcxPivotGridAdvancedCustomizationForm.AlignChildren(AGroup: TcxGroupBox; AColumnCount, ARowCount: Integer);
  ...
  begin
    ...
    begin
      ...
      ALabel.Left := TdxVisualRefinements.Padding.Left;
      ALabel.Width := AImage.Left - ScaleFactor.Apply(4) - TdxVisualRefinements.Padding.Left;
    end
    else
    begin
      AImage.Left := 0;
      ALabel.Left := AImage.Width + ScaleFactor.Apply(4) + TdxVisualRefinements.Padding.Left;
    end;
...
begin
  ...
  AlignImagesAndCaption(imgColumnArea_svg, lbColumnArea, gbColumnArea);
  AlignImagesAndCaption(imgDataAdea_svg, lbDataArea, gbDataArea);
  AlignImagesAndCaption(imgFilterArea_svg, lbFilterArea, gbFilterArea);
  AlignImagesAndCaption(imgRowArea_svg, lbRowArea, gbRowArea);
end;
...
procedure TcxPivotGridAdvancedCustomizationForm.LayoutChooserExecute(Sender: TObject);
begin
  Layout := TcxPivotGridCustomizationFormLayout(TComponent(Sender).Tag);
end;

procedure TcxPivotGridAdvancedCustomizationForm.btnLayoutModeClick(Sender: TObject);
...
procedure TcxPivotGridAdvancedCustomizationForm.cbDeferLayoutUpdateClick(Sender: TObject);
begin
...
procedure TcxPivotGridAdvancedCustomizationForm.btnUpdateClick(Sender: TObject);
begin
