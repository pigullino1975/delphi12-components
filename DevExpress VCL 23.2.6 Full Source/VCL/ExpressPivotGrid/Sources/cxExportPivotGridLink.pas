...
const
  dxThisUnitName = 'cxExportPivotGridLink';
...
type
  ...
  TcxPivotGridFieldDataBindingAccess = class(TcxPivotGridFieldDataBinding);
  ...
  public
    constructor Create(APivotGrid: TcxCustomPivotGrid; AExportType: Integer;
      AStream: TStream; AHandler: TObject); virtual;
    destructor Destroy; override;
  ...
  protected
    ...
    procedure FreeAndNilContent(var AContent: TdxPivotGridExportContentItem);
    procedure FreeAndNilGroup(var AGroup: TdxPivotGridExportGroupItem);
...
begin
  Result := TcxPivotGridFieldDataBindingAccess(APivotField.DataBinding).IsCurrencyField;
end;
...
constructor TcxPivotGridExportHelper.Create(APivotGrid: TcxCustomPivotGrid;
  AExportType: Integer; AStream: TStream; AHandler: TObject);
...
var
  ...
  APivotGridExport: TcxPivotGridExportHelper;
...
begin
  ...
  try
    APivotGridExport := TcxPivotGridExportHelper.Create(APivotGrid, AExportType, AFileStream, AHandler);
    try
      APivotGridExport.Expand := AExpand;
      APivotGridExport.FIsNativeFormat :=  AUseNativeFormat;
      if cxSupports(APivotGridExport.Provider, IcxExportProviderEncoding, AEncodingIntf) then
        AEncodingIntf.SetEncoding(AEncoding);
      if cxSupports(APivotGridExport.Provider, IcxExportWithSeparators, AIntf) and (Length(ASeparators) > 0) then
      begin
        for I := Low(ASeparators) to High(ASeparators) do
          AIntf.AddSeparator(ASeparators[I]);
      end;
      APivotGridExport.ExportPivotGrid;
    finally
      APivotGridExport.Free;
    end;
  finally
    AFileStream.Free;
  end;
...
begin
  if Group.Field.CustomTotals.Count = 0 then
   ...
   Result := Format(Result, [Group.DisplayText])
end;
...
begin
  dxChangeHandle(ContentStyles[False], nil);
  dxChangeHandle(ContentStyles[True], nil);
  FreeAndNil(Groups);
  FreeAndNilGroup(Columns);
  FreeAndNilGroup(Rows);
  FreeAndNilContent(ColumnsContent);
  FreeAndNilContent(RowsContent);
  inherited Destroy;
end;
...
begin
  View.FreezePanes(ColumnMaxLevel + 1, RowMaxLevel);
...
procedure TcxPivotGridDataExportHelper.CalculateGroupsFinish;
  ...
  procedure CalculateGroupFinish(AGroup: TdxPivotGridExportGroupItem);
  var
...
begin
  for I := 0 to Groups.Count - 1 do
    CalculateGroupFinish(TdxPivotGridExportGroupItem(Groups[I]));
end;
...
type
  TRemoveNextListItem = reference to function(AListItem: TObject): TObject;

procedure FreeAndNilLinkedList(var AList: TObject; ARemoveItemProc: TRemoveNextListItem); inline;
var
  AItem, ANext: TObject;
begin
  AItem := AList;
...
procedure TcxPivotGridDataExportHelper.FreeAndNilContent(var AContent: TdxPivotGridExportContentItem);
begin
  FreeAndNilLinkedList(TObject(AContent),
    function(AItem: TObject): TObject
    begin
      Result := TdxPivotGridExportContentItem(AItem).Next;
      TdxPivotGridExportContentItem(AItem).Next := nil;
    end);
end;

procedure TcxPivotGridDataExportHelper.FreeAndNilGroup(var AGroup: TdxPivotGridExportGroupItem);
begin
  FreeAndNilLinkedList(TObject(AGroup),
    function(AItem: TObject): TObject
    begin
      Result := TdxPivotGridExportGroupItem(AItem).Next;
      TdxPivotGridExportGroupItem(AItem).Next := nil;
    end);
end;
...
begin
  ...
  try
    ...
    if (AProducer.Fields.Count = 0) and (AProducer.DataFieldsIndex >= 0) then
      Inc(RowMaxLevel);
  finally
    AProducer.Free;
  end;
...
begin
  ...
  begin
    ...
    dxChangeHandle(ContentStyles[AIsTotal], SpreadSheet.CellStyles.AddStyle(AStyle));
  end;
