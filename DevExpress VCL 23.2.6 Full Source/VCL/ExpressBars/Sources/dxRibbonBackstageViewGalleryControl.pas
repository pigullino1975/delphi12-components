...
uses
  ...
  dxRibbonSkins, dxBar, dxBarStrs, dxMessages, cxAccessibility, cxCustomCanvas;
...
type      
  ...
  TdxRibbonBackstageViewCustomGalleryItemAccessibilityHelper = class(TdxBarAccessibilityHelper) // for internal use
  strict private
    ...
    function GetGalleryItemViewInfo: TdxRibbonBackstageViewGalleryItemViewInfo;
  protected
    function DoGetKeyTip: string; virtual; abstract;
    function DoGetKeyTipBasePoint: TPoint; virtual; abstract;
    procedure DoHandleKeyTip; virtual; abstract;
    ...
    property GalleryItemViewInfo: TdxRibbonBackstageViewGalleryItemViewInfo read GetGalleryItemViewInfo;
  end;

  { TdxRibbonBackstageViewGalleryItemAccessibilityHelper }

  TdxRibbonBackstageViewGalleryItemAccessibilityHelper = class(TdxRibbonBackstageViewCustomGalleryItemAccessibilityHelper) // for internal use
  protected
    function DoGetKeyTip: string; override;
    function DoGetKeyTipBasePoint: TPoint; override;
    procedure DoHandleKeyTip; override;
  ...
  TdxRibbonBackstageViewGalleryItemPinAccessibilityHelper = class(TdxRibbonBackstageViewCustomGalleryItemAccessibilityHelper) // for internal use
  protected
    function DoGetKeyTip: string; override;
    function DoGetKeyTipBasePoint: TPoint; override;
    procedure DoHandleKeyTip; override;
  ...
  public
    ...
    procedure CalculateTextAreaSizeLimitedByRowCount(ARowCount: Integer); override;
    procedure CalculateTextAreaSizeLimitedByWidth(AMaxWidth, AMaxRowCount: Integer); override;
  ...
  TdxRibbonBackstageViewGalleryItemsAccessibilityHelper = class(TdxBarAccessibilityHelper) // for internal use
  strict private
    ...
    function GetGalleryGroupViewInfo: TdxRibbonBackstageViewGalleryGroupViewInfo;
  protected
    function GetBarManager: TdxBarManager; override;
    ...
    property GalleryGroupViewInfo: TdxRibbonBackstageViewGalleryGroupViewInfo read GetGalleryGroupViewInfo;
  public
  ...
  protected
    ...
    function CreateItemViewInfo(AItem: TdxGalleryControlItem): TdxGalleryItemViewInfo; override;
    procedure DrawContent(ACanvas: TcxCustomCanvas); override;
    procedure DrawSeparator(ACanvas: TcxCustomCanvas); virtual;
  public
    procedure Calculate(AType: TdxChangeType; const ABounds: TRect); override;
  ...
  protected
    function CreateGroupViewInfo(AGroup: TdxGalleryControlGroup): TdxGalleryGroupViewInfo; override;
  ...
  strict private
    ...
    function GetPartCount(AItem: TdxRibbonBackstageViewGalleryItem): Integer;
    function GetPinMode: TdxRibbonBackstageViewGalleryItemPinMode;
  ...
  strict private
    function GetcxCanvas(ACanvas: TcxCustomCanvas): TcxCanvas;
  ...
  public
    procedure DrawBackground(ACanvas: TcxCustomCanvas; const ABounds: TRect); override;
    procedure DrawBorder(ACanvas: TcxCustomCanvas; const ABounds: TRect); virtual;
    function GetBorderSize: Integer; virtual;
    // Group
    procedure DrawGroupHeader(ACanvas: TcxCustomCanvas; AViewInfo: TdxGalleryGroupViewInfo); override;
    function GetGroupHeaderContentOffsets: TRect; override;
    // Item
    procedure DrawItem(ACanvas: TcxCustomCanvas; AViewInfo: TdxGalleryItemViewInfo); override;
    procedure DrawItemPin(ACanvas: TcxCustomCanvas; AViewInfo: TdxRibbonBackstageViewGalleryItemViewInfo); virtual;
    procedure DrawItemSelection(ACanvas: TcxCustomCanvas; AViewInfo: TdxGalleryItemViewInfo); override;
    // Pin
    function GetPinButtonSize: TSize; virtual;
    function GetPinTagSize: TSize; virtual;

    // Pinned Items Separator
    procedure DrawPinnedItemSeparator(ACanvas: TcxCustomCanvas; const R: TRect); virtual;
  ...
  TdxRibbonBackstageViewGalleryControlAccessibilityHelper = class(TdxBarAccessibilityHelper) // for internal use
  private
    function GetGalleryControl: TdxCustomRibbonBackstageViewGalleryControl;
    function GetTabSheetButtonControlAccessibilityHelper: TdxBarAccessibilityHelper;
  protected
...
const
  dxThisUnitName = 'dxRibbonBackstageViewGalleryControl';
...
begin
  ...
  AKeyTipInfo.BasePoint := GalleryControl.ClientToScreen(DoGetKeyTipBasePoint);
...
function TdxRibbonBackstageViewCustomGalleryItemAccessibilityHelper.GetGalleryControl: TdxCustomRibbonBackstageViewGalleryControl;
begin
  Result := TdxCustomRibbonBackstageViewGalleryControl(GalleryItem.GalleryControl);
end;
...
function TdxRibbonBackstageViewCustomGalleryItemAccessibilityHelper.GetGalleryItemViewInfo: TdxRibbonBackstageViewGalleryItemViewInfo;
begin
  if not GalleryControl.ViewInfo.FindItemViewInfo(GalleryItem, TdxGalleryItemViewInfo(Result)) then
...
var
  ...
  AViewInfo: TdxRibbonBackstageViewGalleryItemViewInfo;
begin
  AViewInfo := GalleryItemViewInfo;
  if AViewInfo = nil then
    Exit(cxInvalidPoint);

  AGlyphRect := AViewInfo.GlyphRect;
...
procedure TdxRibbonBackstageViewGalleryItemAccessibilityHelper.DoHandleKeyTip;
begin
  GalleryItem.Checked := not GalleryItem.Checked;
end;

procedure TdxRibbonBackstageViewGalleryItemAccessibilityHelper.DoSetKeyTipAlignment(
  var AHorzAlign: TAlignment; var AVertAlign: TcxAlignmentVert);
begin
...
begin
  ...
  R := AViewInfo.Parts[bvgipPin];
  if GalleryControl.UseRightToLeftAlignment then
    Result.X := R.Right
  else
    Result.X := R.Left;
  if GalleryControl.OptionsView.Item.PinMode = bgipmTag then
    Result.Y := AViewInfo.Parts[bvgipPin].Top
  else
    Result.Y := cxRectCenter(GalleryItemViewInfo.Parts[bvgipPin]).Y - AViewInfo.PinSize.cy div 2;
end;

procedure TdxRibbonBackstageViewGalleryItemPinAccessibilityHelper.DoHandleKeyTip;
begin
...
procedure TdxRibbonBackstageViewGalleryItemViewInfo.CalculateTextAreaSizeLimitedByRowCount(ARowCount: Integer);
begin
  inherited CalculateTextAreaSizeLimitedByRowCount(ARowCount);
  CheckTextAreaSize;
end;

procedure TdxRibbonBackstageViewGalleryItemViewInfo.CalculateTextAreaSizeLimitedByWidth(AMaxWidth, AMaxRowCount: Integer);
begin
  if PinMode = bgipmTag then
    Dec(AMaxWidth, PinSize.cx);
  inherited CalculateTextAreaSizeLimitedByWidth(AMaxWidth, AMaxRowCount);
  CheckTextAreaSize;
end;
...
function TdxRibbonBackstageViewGalleryItemsAccessibilityHelper.ChildIsSimpleElement(AIndex: Integer): Boolean;
begin
  Result := False;
end;
...
function TdxRibbonBackstageViewGalleryItemsAccessibilityHelper.GetGalleryGroupViewInfo: TdxRibbonBackstageViewGalleryGroupViewInfo;
begin
  if not GalleryControl.ViewInfo.FindGroupViewInfo(GalleryGroup, TdxGalleryGroupViewInfo(Result)) then
    raise EdxException.Create('');
end;
...
procedure TdxRibbonBackstageViewGalleryGroupViewInfo.CalculatePinnableItemsPlace(AItemRect: TRect; AType: TdxChangeType);
  ...
  procedure PlaceItems(APinState: Boolean; const R: TRect; var ACellIndex: Integer; var ALastItem: TdxRibbonBackstageViewGalleryItemViewInfo);
  var
  ...
  begin
    ...
    begin
      AItemViewInfo := TdxRibbonBackstageViewGalleryItemViewInfo(Items[I]);
      if AItemViewInfo.Item.Pinned = APinState then
      begin
        PlaceItem(AItemViewInfo, AType, R, ACellIndex);
        ALastItem := AItemViewInfo;
        Inc(ACellIndex);
      end;
    end;
  end;
...
var
  ACellIndex: Integer;
  ALastItem: TdxRibbonBackstageViewGalleryItemViewInfo;
begin
  ALastItem := nil;
  ACellIndex := 0;
  ...
  begin
    ...
    FSeparatorRect := cxRectSetTop(AItemRect,
      ALastItem.Bounds.Bottom + ContentOffsetGroups.Bottom,
  ...
  begin
    FItemsRect.Bottom := ALastItem.Bounds.Bottom;
    FRowCount := ALastItem.CellPositionInGroup.Y + 1;
  end;
end;

function TdxRibbonBackstageViewGalleryGroupViewInfo.CreateItemViewInfo(AItem: TdxGalleryControlItem): TdxGalleryItemViewInfo;
begin
  Result := TdxRibbonBackstageViewGalleryItemViewInfo.Create(GalleryControl, AItem);
end;

procedure TdxRibbonBackstageViewGalleryGroupViewInfo.DrawContent(ACanvas: TcxCustomCanvas);
begin
  inherited;
  DrawSeparator(ACanvas);
end;

procedure TdxRibbonBackstageViewGalleryGroupViewInfo.DrawSeparator(ACanvas: TcxCustomCanvas);
begin
...
function TdxRibbonBackstageViewGalleryControlViewInfo.CreateGroupViewInfo(AGroup: TdxGalleryControlGroup): TdxGalleryGroupViewInfo;
begin
  Result := TdxRibbonBackstageViewGalleryGroupViewInfo.Create(GalleryControl, AGroup);
end;
...
begin
  if (AItem <> nil) and ViewInfo.FindItemViewInfo(AItem, AItemViewInfo) then
  begin
    ProcessItemPartClick(
      TdxRibbonBackstageViewGalleryItem(AItem),
      TdxRibbonBackstageViewGalleryItemViewInfo(AItemViewInfo).GetPartAtPos(Point(X, Y)));
  end;
end;
...
procedure TdxRibbonBackstageViewGalleryControlController.ProcessNavigationKey(AKey: Word; AShift: TShiftState);
  ...
  begin
    Result := (AItem <> nil) and (APartIndex >= 0) and (APartIndex < GetPartCount(AItem));
    if Result then
    begin
    ...
    VK_LEFT, VK_UP:
      ...
      begin
        ...
        begin
          ...
          if AItem <> nil then
            TrySelectItem(AItem, GetPartCount(AItem) - 1);
        end;
      end;
  end;
end;
...
var
  AItemViewInfo: TdxRibbonBackstageViewGalleryItemViewInfo;
begin
  if Owner.ViewInfo.GetItemAtPos(P, TdxGalleryItemViewInfo(AItemViewInfo)) and AItemViewInfo.Item.Enabled then
  begin
    MouseHoveredItem := AItemViewInfo.Item;
    MouseHoveredItemPartIndex := AItemViewInfo.GetPartAtPos(P);
  end
  else
...
begin
  if AItem <> nil then
    APartIndex := Max(-1, Min(APartIndex, GetPartCount(AItem) - 1))
  else
    APartIndex := -1;
end;
...
function TdxRibbonBackstageViewGalleryControlController.GetPartCount(AItem: TdxRibbonBackstageViewGalleryItem): Integer;
begin
  if PinMode = bgipmNone then
    Result := 1
  else
    Result := 2;
end;
...
procedure TdxRibbonBackstageViewGalleryControlPainter.DrawBackground(ACanvas: TcxCustomCanvas; const ABounds: TRect);
begin
  if Skin <> nil then
    Skin.DrawBackground(GetcxCanvas(ACanvas).Handle, ABounds, DXBAR_BACKSTAGEVIEW_GALLERYCONTROL)
  else
    inherited;
end;

procedure TdxRibbonBackstageViewGalleryControlPainter.DrawBorder(ACanvas: TcxCustomCanvas; const ABounds: TRect);
begin
  if Skin <> nil then
...
procedure TdxRibbonBackstageViewGalleryControlPainter.DrawGroupHeader(ACanvas: TcxCustomCanvas; AViewInfo: TdxGalleryGroupViewInfo);
begin
  if Skin <> nil then
  begin
    Skin.DrawBackground(GetcxCanvas(ACanvas).Handle, AViewInfo.CaptionRect, DXBAR_BACKSTAGEVIEW_GALLERYCONTROL_GROUPHEADER);
...
procedure TdxRibbonBackstageViewGalleryControlPainter.DrawItem(ACanvas: TcxCustomCanvas; AViewInfo: TdxGalleryItemViewInfo);
var
  AItemViewInfo: TdxRibbonBackstageViewGalleryItemViewInfo;
begin
...
procedure TdxRibbonBackstageViewGalleryControlPainter.DrawItemPin(
  ACanvas: TcxCustomCanvas; AViewInfo: TdxRibbonBackstageViewGalleryItemViewInfo);
const
...
begin
  ...
  begin
    ...
    Skin.DrawBackground(GetcxCanvas(ACanvas).Handle, AViewInfo.Parts[bvgipPin],
  ...
  begin
    ...
    try
      ...
      LookAndFeelPainter.DrawPin(GetcxCanvas(ACanvas),
...
procedure TdxRibbonBackstageViewGalleryControlPainter.DrawItemSelection(ACanvas: TcxCustomCanvas; AViewInfo: TdxGalleryItemViewInfo);

  procedure DrawPartSelection(const R: TRect; const AState: TdxGalleryItemViewState; APartId: Integer);
  begin
    if Skin <> nil then
      Skin.DrawBackground(GetcxCanvas(ACanvas).Handle, R, APartId, GetPartState(AState))
    else
...
begin
  ...
  begin
    DrawPartSelection(AItemViewInfo.Parts[bvgipItem],
      AItemViewInfo.PartState[bvgipItem], DXBAR_BACKSTAGEVIEW_GALLERYCONTROL_ITEM);
    DrawPartSelection(AItemViewInfo.Parts[bvgipPin],
      AItemViewInfo.PartState[bvgipPin], DXBAR_BACKSTAGEVIEW_GALLERYCONTROL_ITEMPINBUTTON);
  end
  else
    DrawPartSelection(AItemViewInfo.Bounds, AItemViewInfo.State, DXBAR_BACKSTAGEVIEW_GALLERYCONTROL_ITEM);
end;
...
procedure TdxRibbonBackstageViewGalleryControlPainter.DrawPinnedItemSeparator(ACanvas: TcxCustomCanvas; const R: TRect);
begin
  if Skin <> nil then
    Skin.DrawBackground(GetcxCanvas(ACanvas).Handle, R, DXBAR_BACKSTAGEVIEW_GALLERYCONTROL_SEPARATOR)
  else
    LookAndFeelPainter.DrawBevelShape(GetcxCanvas(ACanvas), R, dxbsLineCenteredVert, dxbsLowered);
end;
...
function TdxRibbonBackstageViewGalleryControlPainter.GetcxCanvas(ACanvas: TcxCustomCanvas): TcxCanvas;
begin
  Result := ACanvas as TcxCanvas;
end;
...
var
  ATabSheetButtonControlAccessibilityHelper: TdxRibbonBackstageViewTabSheetButtonControlAccessibilityHelperAccess;
begin
  ATabSheetButtonControlAccessibilityHelper :=
    TdxRibbonBackstageViewTabSheetButtonControlAccessibilityHelperAccess(GetTabSheetButtonControlAccessibilityHelper);
  if ATabSheetButtonControlAccessibilityHelper <> nil then
    ATabSheetButtonControlAccessibilityHelper.AddKeyTipChild(Self);
end;
...
begin
  ATabSheetButtonControlAccessibilityHelper :=
    TdxRibbonBackstageViewTabSheetButtonControlAccessibilityHelperAccess(GetTabSheetButtonControlAccessibilityHelper);
  if ATabSheetButtonControlAccessibilityHelper <> nil then
    ATabSheetButtonControlAccessibilityHelper.DeleteKeyTipChild(Self);
end;
...
begin
  if GalleryControl.HandleAllocated then
    Result := GalleryControl.Handle
  else
    Result := 0;
end;
...
function TdxRibbonBackstageViewGalleryControlAccessibilityHelper.GetState(
  AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  if GetTabSheetButtonControlAccessibilityHelper <> nil then
    Result := GetTabSheetButtonControlAccessibilityHelper.States[cxAccessibleObjectSelfID]
  else
    Result := inherited GetState(AChildID);
  if not GalleryControl.Visible or (GetTabSheetButtonControlAccessibilityHelper = nil) then
    Result := Result or cxSTATE_SYSTEM_INVISIBLE;
end;

procedure TdxRibbonBackstageViewGalleryControlAccessibilityHelper.Select(ASetFocus: Boolean);
begin
  BarNavigationController.SelectedObject := Self;
  BarNavigationController.SelectedObjectParent := GetTabSheetButtonControlAccessibilityHelper;
end;

function TdxRibbonBackstageViewGalleryControlAccessibilityHelper.ChildIsSimpleElement(AIndex: Integer): Boolean;
begin
  Result := False;
end;
...
function TdxRibbonBackstageViewGalleryControlAccessibilityHelper.GetSupportedProperties(
...
function TdxRibbonBackstageViewGalleryControlAccessibilityHelper.GetParentForKeyTip: TdxBarAccessibilityHelper;
begin
  Result := GetTabSheetButtonControlAccessibilityHelper;
end;
...
function TdxRibbonBackstageViewGalleryControlAccessibilityHelper.GetTabSheetButtonControlAccessibilityHelper: TdxBarAccessibilityHelper;
var
  ...
  begin
    ...
    begin
      ...
      for I := 0 to AItemLinks.Count - 1 do
        ...
        begin
          Result := AItemLinks[I].Control.IAccessibilityHelper.GetBarHelper;
          Break;
        end;
    end;
  end;
...
procedure TdxCustomRibbonBackstageViewGalleryControl.DrawBorder(ACanvas: TcxCustomCanvas);
begin
...
begin
  ...
  begin
    if ViewInfo.GetItemAtPos(Message.HintInfo.CursorPos, TdxGalleryItemViewInfo(AItemViewInfo)) and AItemViewInfo.Item.Enabled then
    begin
      Message.HintInfo.CursorRect := AItemViewInfo.Bounds;
      Message.HintInfo.HintStr := AItemViewInfo.GetPartHint(AItemViewInfo.GetPartAtPos(Message.HintInfo.CursorPos));
      Message.Result := 0;
    end;
  end;
end;
