...
uses
  ...
  dxGDIPlusClasses, dxFluentDesignFormInterfaces;
...
type
  ...
  TdxRibbonBackstageViewBackButtonViewInfo = class;
  ...
  { TdxRibbonBackstageViewBackButtonAccessibleHelper }

  TdxRibbonBackstageViewBackButtonAccessibleHelper = class(TdxBarAccessibilityHelper)
  strict private
    function GetBackButtonViewInfo: TdxRibbonBackstageViewBackButtonViewInfo;
  protected
  ...
  public
    ...
    property BackButtonViewInfo: TdxRibbonBackstageViewBackButtonViewInfo read GetBackButtonViewInfo;
  end;

  { TdxRibbonBackstageViewMenuBarAccessibilityHelper }

  TdxRibbonBackstageViewMenuBarAccessibilityHelper = class(TdxRibbonBarControlAccessibilityHelper) // for internal use
  strict private
  ...
  protected
    procedure DoBarGetFocus(ASelectedItem: TdxBarItemControl; AHideAll: Boolean); override;
  ...
  TdxRibbonBackstageViewMenuDockControlAccessibilityHelper = class(TdxDockControlAccessibilityHelper) // for internal use
  private
    function GetDockControl: TdxRibbonBackstageViewMenuDockControl;
  protected
    ...
    property DockControl: TdxRibbonBackstageViewMenuDockControl read GetDockControl;
  end;
  ...
  strict private
    function GetItem(Index: TdxListIndex): TdxRibbonBackstageViewTabSheet;
  public
    property Items[Index: TdxListIndex]: TdxRibbonBackstageViewTabSheet read GetItem; default;
  end;
  ...
  TdxRibbonBackstageViewTabSheetButtonControlAccessibilityHelper = class(TdxBarButtonControlAccessibilityHelper) // for internal use
  strict private
    FKeyTipChilds: TInterfaceList;
    FKeyTipWindowsManager: IdxBarKeyTipWindowsManager;

    function GetTabSheet: TdxRibbonBackstageViewTabSheet;
  protected
    procedure AddKeyTipChild(AChild: IdxBarAccessibilityHelper);
    procedure DeleteKeyTipChild(AChild: IdxBarAccessibilityHelper);
  ...
  strict private
    function GetItem(Index: TdxListIndex): TdxRibbonBackstageViewTabSheetButton;
  public
    ...
    //
    property Items[Index: TdxListIndex]: TdxRibbonBackstageViewTabSheetButton read GetItem; default;
  end;
  ...
  strict private
    function GetItem(Index: TdxListIndex): TdxRibbonBackstageViewMenuBarButton;
  public
    procedure RefreshInfo;
    //
    property Items[Index: TdxListIndex]: TdxRibbonBackstageViewMenuBarButton read GetItem; default;
  end;
  ...
  { TdxRibbonCustomBackstageViewAccessibilityHelper }

  TdxRibbonBackstageViewAccessibilityHelper = class(TdxBarAccessibilityHelper) // for internal use
  strict private
    function GetBackstageView: TdxRibbonCustomBackstageView;
    function GetBackstageViewViewInfo: TdxRibbonBackstageViewViewInfo;
  protected
    ...
    property BackstageView: TdxRibbonCustomBackstageView read GetBackstageView;
    property BackstageViewViewInfo: TdxRibbonBackstageViewViewInfo read GetBackstageViewViewInfo;
  public
    function GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect; override;
  end;
  ...
  strict private
    ...
    FBackButtonIAccessibleHelper: IdxBarAccessibilityHelper;
    ...
    function GetBarManager: TdxBarManager;
    function GetBackButtonIAccessibleHelper: IdxBarAccessibilityHelper;
    function GetIAccessibilityHelper: IdxBarAccessibilityHelper;
    function GetIsBarManagerValid: Boolean;
  ...
  protected
    ...
    procedure WndProc(var Message: TMessage); override;
    //
    property BackButtonIAccessibleHelper: IdxBarAccessibilityHelper read GetBackButtonIAccessibleHelper;
    property BarManager: TdxBarManager read GetBarManager;
...
uses
  cxDrawTextUtils, dxBarSkinConsts, dxFading, dxOffice11, dxRibbonForm, dxCustomFluentDesignForm, dxForms, dxDPIAwareUtils;

const
  dxThisUnitName = 'dxRibbonBackstageView';
...
function TdxRibbonBackstageViewBackButtonAccessibleHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  Result := BackButtonViewInfo.Bounds;
  Result := cxRectSetOrigin(Result, TdxRibbonBackstageView(FOwnerObject).Ribbon.ClientToScreen(Result.TopLeft));
end;

procedure TdxRibbonBackstageViewBackButtonAccessibleHelper.DoDefaultAction(AChildID: TcxAccessibleSimpleChildElementID);
begin
  BackButtonViewInfo.Click;
end;

function TdxRibbonBackstageViewBackButtonAccessibleHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := 'Back';
end;

function TdxRibbonBackstageViewBackButtonAccessibleHelper.GetParent: TcxAccessibilityHelper;
begin
  Result := TdxRibbonCustomBackstageView(FOwnerObject).IAccessibilityHelper.GetHelper;
end;

function TdxRibbonBackstageViewBackButtonAccessibleHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxROLE_SYSTEM_PUSHBUTTON;
end;

function TdxRibbonBackstageViewBackButtonAccessibleHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := Parent.States[cxAccessibleObjectSelfID];
end;

function TdxRibbonBackstageViewBackButtonAccessibleHelper.GetSupportedProperties(
  AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := [aopLocation, aopDefaultAction];
end;

function TdxRibbonBackstageViewBackButtonAccessibleHelper.GetAssignedKeyTip: string;
begin
  Result := '';
end;

function TdxRibbonBackstageViewBackButtonAccessibleHelper.GetDefaultKeyTip: string;
begin
  Result := '';
end;

function TdxRibbonBackstageViewBackButtonAccessibleHelper.GetBackButtonViewInfo: TdxRibbonBackstageViewBackButtonViewInfo;
begin
  Result := TdxRibbonBackstageView(FOwnerObject).NonClientViewInfo.BackButtonViewInfo;
end;
...
function TdxRibbonBackstageViewMenuBarAccessibilityHelper.GetParent: TcxAccessibilityHelper;
begin
  Result := BackstageView.MenuViewInfo.DockControl.IAccessibilityHelper.GetBarHelper;
end;

function TdxRibbonBackstageViewMenuBarAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := BackstageView.Ribbon.ApplicationButton.Text;
  if (Result = '') and (BackstageView.Ribbon.ApplicationButton.ScreenTip <> nil) then
    Result := BackstageView.Ribbon.ApplicationButton.ScreenTip.Header.Text;
end;

function TdxRibbonBackstageViewMenuBarAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
...
procedure TdxRibbonBackstageViewMenuBarControl.DoBarGetFocus(ASelectedItem: TdxBarItemControl; AHideAll: Boolean);
begin
  if ASelectedItem = nil then
    ASelectedItem := BackstageView.ViewInfo.ActiveTabItemControl;
  inherited DoBarGetFocus(ASelectedItem, AHideAll);
end;
...
begin
  if BackstageView.DoMouseWheel(KeysToShiftState(Message.Keys), Message.WheelDelta, SmallPointToPoint(Message.Pos)) then
    Message.Result := 1
  else
    inherited;
end;
...
{ TdxRibbonBackstageViewMenuDockControlAccessibilityHelper }

function TdxRibbonBackstageViewMenuDockControlAccessibilityHelper.GetChild(AIndex: Integer): TcxAccessibilityHelper;
begin
  Result := DockControl.BackstageView.ViewInfo.MenuBarControl.IAccessibilityHelper.GetBarHelper;
end;

function TdxRibbonBackstageViewMenuDockControlAccessibilityHelper.GetChildCount: Integer;
begin
  Result := 1;
end;

function TdxRibbonBackstageViewMenuDockControlAccessibilityHelper.GetChildIndex(AChild: TcxAccessibilityHelper): Integer;
begin
  Result := 1;
end;

function TdxRibbonBackstageViewMenuDockControlAccessibilityHelper.GetParent: TcxAccessibilityHelper;
begin
  Result := DockControl.BackstageView.IAccessibilityHelper.GetBarHelper;
end;

function TdxRibbonBackstageViewMenuDockControlAccessibilityHelper.GetDockControl: TdxRibbonBackstageViewMenuDockControl;
begin
  Result := TdxRibbonBackstageViewMenuDockControl(OwnerObject);
end;
...
function TdxRibbonBackstageViewMenuDockControl.GetAccessibilityHelperClass: TdxBarAccessibilityHelperClass;
begin
  Result := TdxRibbonBackstageViewMenuDockControlAccessibilityHelper;
end;
...
var
  ...
  AImagesEnabled: TdxDefaultBoolean;
  AImageSize: TSize;
...
begin
  ...
  begin
    ...
    AImages := GetImages(Enabled, AImagesEnabled);
    TdxImageDrawer.DrawUncachedImage(ADrawParams.Canvas.Handle, AImageRect, AImageRect,
      Glyph, AImages, ImageIndex, idmNormal, True, 0, clDefault, True,
      MenuPainter.GetGlyphColorPalette(Self,
        ADrawParams.PaintType, ADrawParams.DrawSelected, ADrawParams.Downed,
        ADrawParams.DrawDowned, ADrawParams.DroppedDown, False, AImagesEnabled = bTrue));
...
begin
  Result := [cpText];
  if (BackstageView.RibbonStyle = rs2019) or HasImage then
...
function TdxRibbonBackstageViewTabSheets.GetItem(Index: TdxListIndex): TdxRibbonBackstageViewTabSheet;
begin
...
begin
  ...
  FKeyTipChilds := TInterfaceList.Create;
end;

destructor TdxRibbonBackstageViewTabSheetButtonControlAccessibilityHelper.Destroy;
begin
  FreeAndNil(FKeyTipChilds);
  inherited Destroy;
end;

procedure TdxRibbonBackstageViewTabSheetButtonControlAccessibilityHelper.AddKeyTipChild(AChild: IdxBarAccessibilityHelper);
begin
  if FKeyTipChilds.IndexOf(AChild) = -1 then
    FKeyTipChilds.Add(AChild);
end;

procedure TdxRibbonBackstageViewTabSheetButtonControlAccessibilityHelper.DeleteKeyTipChild(AChild: IdxBarAccessibilityHelper);
begin
  FKeyTipChilds.Remove(AChild);
end;

function TdxRibbonBackstageViewTabSheetButtonControlAccessibilityHelper.GetRole(
...
procedure TdxRibbonBackstageViewTabSheetButtonControlAccessibilityHelper.DoGetKeyTipsData(AKeyTipsData: TList);
var
  AAccessibilityHelper: TdxBarAccessibilityHelper;
  I: Integer;
begin
  if Visible then
    for I := 0 to FKeyTipChilds.Count - 1 do
    begin
      AAccessibilityHelper := IdxBarAccessibilityHelper(FKeyTipChilds[I]).GetBarHelper;
      if TdxBarAccessibilityHelperAccess(AAccessibilityHelper).Selectable then
        TdxBarAccessibilityHelperAccess(AAccessibilityHelper).GetKeyTipData(AKeyTipsData)
      else
        if TdxBarAccessibilityHelperAccess(AAccessibilityHelper).IsParentForKeyTip then
          TdxBarAccessibilityHelperAccess(AAccessibilityHelper).DoGetKeyTipsData(AKeyTipsData);
    end;
end;
...
function TdxRibbonBackstageViewTabSheetButtonList.GetItem(Index: TdxListIndex): TdxRibbonBackstageViewTabSheetButton;
begin
...
function TdxRibbonBackstageViewMenuButtonList.GetItem(Index: TdxListIndex): TdxRibbonBackstageViewMenuBarButton;
begin
...
begin
  ...
  begin
    AItemLink := BarDesignController.AddInternalItem(ItemLinks, TdxRibbonBackstageViewMenuBarButton, BarManager, MenuButtonList);
    (AItemLink.Item as TdxRibbonBackstageViewMenuBarButton).MenuButton := AButton;
...
begin
  ...
  begin
    AItemLink := BarDesignController.AddInternalItem(ItemLinks, TdxRibbonBackstageViewTabSheetButton, BarManager, TabButtonList);
    (AItemLink.Item as TdxRibbonBackstageViewTabSheetButton).Tab := ATab;
...
function TdxRibbonBackstageViewMenuViewInfo.GetIAccessibilityHelper: IdxBarAccessibilityHelper;
begin
...
function TdxRibbonBackstageViewViewInfo.CanProcessMouseWheel: Boolean;
begin
...
begin
  Result := CanProcessMouseWheel;
  if Result then
    ScrollPositionY := ScrollPositionY + Signs[ALineDown] * Mouse.WheelScrollLines * dxBackstageViewScrollLineSize;
end;
...
begin
  BarNavigationController.ChangeSelectedObject(False,
    BackstageView.MenuViewInfo.IAccessibilityHelper.GetBarHelper.GetFirstSelectableObject);
end;

procedure TdxRibbonBackstageViewApplicationMenuHelper.ShowKeyTips;
begin
  BarNavigationController.SetKeyTipsShowingState(BackstageView.MenuViewInfo.IAccessibilityHelper, '');
...
function TdxRibbonBackstageViewApplicationMenuHelper.IsHiding: Boolean;
begin
  Result := (BackstageView <> nil) and BackstageView.IsHiding;
end;
...
begin
  ...
  begin
    AFormData := RibbonFormCaptionHelper.FormPaintData;
    FFormBounds := dxMapWindowRect(Ribbon.Parent.Handle, BackstageView.Handle, AFormData.GetBounds, False);
    AOffset := dxMapWindowPoint(Ribbon.Handle, BackstageView.Handle, cxNullPoint, False);
    FCornerRadius := AFormData.GetWindowCornersRadius;
...
function TdxRibbonBackstageViewNonClientViewInfo.CreateWindowRegion: TcxRegionHandle;
  ...
  procedure ExcludeRegion(var ARegion: TcxRegionHandle; ASourceRegion: TcxRegionHandle);
  begin
    if ARegion = 0 then
      ARegion := CreateRectRgnIndirect(Bounds);
    CombineRgn(ARegion, ARegion, ASourceRegion, RGN_DIFF);
    DeleteObject(ASourceRegion);
  end;

  function CreateRoundedFrameRegion(ACornerRadius: Integer): TcxRegionHandle;
  begin
    Result := TdxFormHelper.CreateRoundedRegion(FFormBounds, ACornerRadius);
    ExcludeRegion(Result, TdxFormHelper.CreateRoundedRegion(cxRectInflate(FFormBounds, -1), ACornerRadius - 1));
  end;
...
begin
  ...
  begin
    if (CornerRadius > 0) and not IsWin11OrLater then
      ExcludeRegion(Result, CreateRoundedFrameRegion(CornerRadius));
    if Painter.CanShowTabAreaToolbar then
      ExcludeRegion(Result, CreateRectRgnIndirect(TabAreaToolbarArea));
    ExcludeRegion(Result, CreateRectRgnIndirect(BorderIconsArea));
  end;
end;
...
begin
  ...
  begin
    if cxRectContain(ContentRect, MenuBarHeaderRect) then
...
begin
  ...
  begin
    FFonts[AFontIndex] := dxCreateFontForDefaultDPI;
...
function TdxRibbonBackstageViewAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  Result := BackstageViewViewInfo.Bounds;
  Result.Top := -BackstageView.NonClientViewInfo.ClientOffsets.Top;
  Result := cxRectOffset(Result, BackstageView.ClientToScreen(BackstageViewViewInfo.Bounds.TopLeft));
end;

function TdxRibbonBackstageViewAccessibilityHelper.ChildIsSimpleElement(AIndex: Integer): Boolean;
begin
  Result := False;
end;

function TdxRibbonBackstageViewAccessibilityHelper.GetChild(AIndex: Integer): TcxAccessibilityHelper;
begin
  if AIndex = 0 then
    Result := BackstageViewViewInfo.MenuViewInfo.DockControl.IAccessibilityHelper.GetBarHelper
  else
    Result := BackstageView.BackButtonIAccessibleHelper.GetBarHelper;
end;

function TdxRibbonBackstageViewAccessibilityHelper.GetChildCount: Integer;
begin
  Result := 2;
end;

function TdxRibbonBackstageViewAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := 'Backstage view';
end;

function TdxRibbonBackstageViewAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxROLE_SYSTEM_PANE;
end;

function TdxRibbonBackstageViewAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  if not BackstageView.Visible then
    Result := cxSTATE_SYSTEM_INVISIBLE
  else
    Result := cxSTATE_SYSTEM_NORMAL;
end;

function TdxRibbonBackstageViewAccessibilityHelper.GetSupportedProperties(
  AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := [aopLocation];
end;

function TdxRibbonBackstageViewAccessibilityHelper.GetAssignedKeyTip: string;
begin
  Result := '';
end;

function TdxRibbonBackstageViewAccessibilityHelper.GetDefaultKeyTip: string;
begin
  Result := '';
end;

function TdxRibbonBackstageViewAccessibilityHelper.GetBackstageView: TdxRibbonCustomBackstageView;
begin
  Result := TdxRibbonCustomBackstageView(OwnerObject);
end;

function TdxRibbonBackstageViewAccessibilityHelper.GetBackstageViewViewInfo: TdxRibbonBackstageViewViewInfo;
begin
  Result := BackstageView.ViewInfo;
end;
...
begin
  ...
  BarAccessibilityHelperOwnerObjectDestroyed(FBackButtonIAccessibleHelper);
  FActiveTab := nil;
  DeleteAllTabs;
  Ribbon := nil;
  FreeAndNil(FTabs);
...
begin
  Result := Enabled and Visible and ViewInfo.CanProcessMouseWheel;
end;

function TdxRibbonCustomBackstageView.DoMouseWheel(Shift: TShiftState; WheelDelta: Integer; MousePos: TPoint): Boolean;
var
  AControl: TWinControl;
  AScrollInfo: TScrollInfo;
begin
  if not CanProcessMouseWheel then
    Exit(False);

  if not cxPointIsEqual(MousePos, cxInvalidPoint) then
  begin
    AControl := FindControl(WindowFromPoint(MousePos));
    if (AControl <> nil) and (AControl <> Self) then
    begin
      AScrollInfo.cbSize := SizeOf(AScrollInfo);
      AScrollInfo.fMask := SIF_RANGE or SIF_PAGE;
      if GetScrollInfo(AControl.Handle, SB_VERT, AScrollInfo) and (AScrollInfo.nMax > AScrollInfo.nMin + Integer(AScrollInfo.nPage)) then
        Exit(False);
      if TWinControlAccess(AControl).DoMouseWheel(Shift, WheelDelta, MousePos) then
        Exit(True);
    end;
  end;
...
function TdxRibbonCustomBackstageView.IsHiding: Boolean;
begin
  Result := not Visible and FIsInMessageLoop;
end;
    ...
    try
      ...
      try
        ...
        try
          ...
          BringToFront;
          TdxHybridScrollbarManagers.HideAll;
          TdxTouchScrollUIModeManager.HideActive;
          Show;
          SetFocus;
          FIsInMessageLoop := True;
          PopupMessageLoop(AForm);
        finally
          if AObjectLink.Ref <> nil then
          begin
            FIsInMessageLoop := False;
...
begin
  ...
  begin
    ...
    begin
      ...
      // Is top border drawn by ribbon?
      if cxGetWindowRect(Ribbon).Top = cxGetWindowRect(Parent).Top then
        Inc(R.Top);
...
procedure TdxRibbonCustomBackstageView.CreateWindowHandle(const Params: TCreateParams);
begin
  inherited;
...
begin
  ...
  if ActiveTab <> ANewActiveTab then
    ActiveTab := nil;
  Changed([rbvcStruct]);
end;
...
function TdxRibbonCustomBackstageView.FluentDesignContainerControlCanBeOpaque: Boolean;
begin
  Result := True;
end;
...
procedure TdxRibbonCustomBackstageView.WMGetObject(var Message: TMessage);
var
  AObjectID: Cardinal;
begin
...
function TdxRibbonCustomBackstageView.GetBackButtonIAccessibleHelper: IdxBarAccessibilityHelper;
begin
  if FBackButtonIAccessibleHelper = nil then
    FBackButtonIAccessibleHelper := TdxRibbonBackstageViewBackButtonAccessibleHelper.Create(Self);
  Result := FBackButtonIAccessibleHelper;
end;

function TdxRibbonCustomBackstageView.GetIAccessibilityHelper: IdxBarAccessibilityHelper;
begin
  if FIAccessibilityHelper = nil then
    FIAccessibilityHelper := TdxRibbonBackstageViewAccessibilityHelper.Create(Self);
