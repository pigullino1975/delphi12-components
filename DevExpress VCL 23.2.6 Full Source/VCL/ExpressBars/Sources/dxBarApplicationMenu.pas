...
uses
  dxBarStrs, Math, dxCoreGraphics;

const
  dxThisUnitName = 'dxBarApplicationMenu';
...
type
  ...
  TdxBarExtraPaneDockControlAccessibilityHelper = class(TdxDockControlAccessibilityHelper) // for internal use
  protected
  ...
  TdxBarExtraPaneControlAccessibilityHelper = class(TdxBarControlAccessibilityHelper) // for internal use
  private
  ...
  TdxBarApplicationMenuControlAccessibilityHelper = class(TdxBarSubMenuControlAccessibilityHelper) // for internal use
  private
    function GetBarControl: TdxBarApplicationMenuControl;
  ...
  TdxBarExtraPanePinnedButtonControlAccessibilityHelper = class(TdxBarButtonControlAccessibilityHelper) // for internal use
  private
...
begin
  ...
  FListItem := BarDesignController.AddInternalItem(TdxBarExtraPaneListItem, FApplicationMenu.BarManager, nil, FApplicationMenu.FInternalItems) as TdxBarExtraPaneListItem;
...
constructor TdxBarApplicationMenuButton.Create(ACollection: TCollection);
begin
  inherited Create(ACollection);
  FInternalItem := BarDesignController.AddInternalItem(TdxBarApplicationMenuButtonItem, Collection.FApplicationMenu.BarManager, nil, Collection.FApplicationMenu.FInternalItems) as TdxBarApplicationMenuButtonItem;
end;
...
procedure TdxBarCustomApplicationMenu.CreateSubClasses;
begin
  inherited;
...
begin
  inherited;
  FExtraPane.FListItem.BarManager := Value;
  for I :=  0 to FButtons.Count  - 1  do
    FButtons[I].FInternalItem.FBarManager := Value;
end;
...
begin
  Result := cxRectHeight(ItemsRect) div Painter.GetButtonHeight(Painter.GetSmallIconSize(ScaleFactor), GetTextSize, EditTextSize, ScaleFactor);
end;
...
begin
  ...
  begin
    ...
    AFrameSizes.Bottom := Max(AFrameSizes.Bottom,
      TdxTextMeasurer.TextLineHeight(Font) + 2 * IndentBetweenTextAndBorders +
