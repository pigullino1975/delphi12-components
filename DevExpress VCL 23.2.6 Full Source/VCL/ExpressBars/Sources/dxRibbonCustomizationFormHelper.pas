...
type
  ...
  private
    ...
    function GetItem(AIndex: TdxListIndex): TdxRibbonCustomizationFormMergedTabInfo;
  protected
    function AddTabInfo(ATab: TdxRibbonTab): TdxRibbonCustomizationFormMergedTabInfo;

    property Items[Index: TdxListIndex]: TdxRibbonCustomizationFormMergedTabInfo read GetItem; default;
  ...
  protected
    ...
    procedure ConvertItemNodeToNew(ANode: TcxTreeListNode); virtual;
  ...
  protected
    ...
    procedure LoadTabsOrderFormIni(AContextNode: TcxTreeListNode; ASource: TCustomIniFile);
...
const
  dxThisUnitName = 'dxRibbonCustomizationFormHelper';
...
begin
  ...
  begin
    ...
    TdxImageDrawer.DrawImage(ACanvas, AImageRect, AGlyph, Item.BarManager.Images, Item.ImageIndex, ifmFit, idmNormal, True, nil, AScaleFactor);
  end;
end;
...
function TdxRibbonCustomizationFormMergedTabsList.GetItem(AIndex: TdxListIndex): TdxRibbonCustomizationFormMergedTabInfo;
begin
...
begin
  ...
  ARect := cxRectCenterVertically(ARect, TdxTextMeasurer.TextLineHeight(ACanvas.Handle));
...
function FindItemProc(ANode: TcxTreeListNode; AData: Pointer): Boolean;
begin
  Result := (TObject(ANode.Data) is TdxRibbonCustomizationFormItemNodeData) and
    (TdxRibbonCustomizationFormItemNodeData(ANode.Data).Item = TdxBarItem(AData));
end;
...
procedure TdxCustomRibbonCustomizationFormHelper.PopulateAllItems(ASource: TdxBarManager;
  ...
  procedure DoPopulateItem(AItem: TdxBarItem; const ACheckForDuplicates: Boolean = False);
  begin
    if ACheckForDuplicates and
      ((AItem.BarManager = ASource) or (CommandsTreeList.Find(AItem, nil, False, True, FindItemProc) <> nil)) then
      Exit;
    if not AOnlyMissingItems or not IsRibbonContainsItem(AItem) then
      PopulateItemNodeContent(CommandsTreeList.Add, AItem, nil);
  end;

  procedure DoPopulateItems(ABarManager: TdxBarManager; const ACheckForDuplicates: Boolean = False);
  var
    I: Integer;
  begin
    for I := 0 to ABarManager.ItemCount - 1 do
      DoPopulateItem(ABarManager.Items[I], ACheckForDuplicates);
  end;
...
begin
  ...
  DoPopulateItems(ASource);
  for I := 0 to ASource.Bars.Count - 1 do
  begin
    ABar := TdxBarAccess(ASource.Bars[I]);
    if ABar.MergeData.CreatedByMerging or ABar.IsMerged then
      for J := 0 to ABar.ItemLinks.Count - 1 do
        DoPopulateItems(ABar.ItemLinks[J].Item.BarManager, True);
  end;
end;
...
begin
  ...
  ATextRect := cxRectCenterVertically(AViewInfo.BoundsRect, TdxTextMeasurer.TextLineHeight(ACanvas.Handle));
...
begin
  ...
  if (ATargetTreeList = CommandsTreeList) or AIsReseting then
    LoadTabsOrderFormIni(Result, FSavedState);
...
procedure TdxRibbonCustomizationFormHelper.LoadTabsOrderFormIni(AContextNode: TcxTreeListNode; ASource: TCustomIniFile);

  function GetRibbonIniSection: string;
  var
    AOwner: TComponent;
  begin 
    Result := Ribbon.Name;
    ...
    Result := TdxBarManagerAccess(Ribbon.BarManager).GetBaseIniSection + Result;
  end;

  function GetTabNode(const ATabName: string; AOwnerNode: TcxTreeListNode): TcxTreeListNode;
  var
    I: Integer;
  begin
    Result := nil;
    for I := 0 to AOwnerNode.Count - 1 do
      if TdxRibbonTab(AOwnerNode[I].Data).Name = ATabName then
      begin
        Result := AOwnerNode[I];
        Break;
      end;
...
var
  AContextCaption, ARibbonSection, ATabSection: string;
  ATabNode: TcxTreeListNode;
  AIndex, I: Integer;
begin
  ARibbonSection := GetRibbonIniSection;
  if AContextNode.Data = nil then
    AContextCaption := ''
  else
    AContextCaption := TdxRibbonContext(AContextNode.Data).Caption;
  AIndex := 0;
  for I := 0 to ASource.ReadInteger(ARibbonSection, 'TabCount', 0) - 1 do
  begin
    ATabSection := ARibbonSection + '.Tab' + IntToStr(I); 
    if ASource.ReadString(ATabSection, 'ContextCaption', '') = AContextCaption then
    begin
      ATabNode := GetTabNode(ASource.ReadString(ATabSection, 'Name', ''), AContextNode);
      if ATabNode <> nil then
      begin
        ATabNode.MoveTo(ATabNode.Parent[AIndex], tlamInsert);
        Inc(AIndex);
      end;
    end;
  end;
