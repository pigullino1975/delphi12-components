...
type
  ...
  TdxBarDBEnableType = (dxdbtCanModify, dxdbtNotEOF, dxdbtNotBOF, dxdbtHasRecords, dxdbtIsModified, dxdbtIsNotModified);
  ...
  TdxBarDBNavButtonType = (dxbnFirst, dxbnPrior, dxbnNext, dxbnLast,
    dxbnInsert, dxbnDelete, dxbnEdit, dxbnPost, dxbnCancel, dxbnRefresh, dxbnAppend);
  ...
  published
    ...
    property VisibleButtons: TdxBarDBNavButtonTypes read FVisibleButtons write SetVisibleButtons default [];
  end;
...
uses
  Windows, SysUtils, Forms, dxBarDBNavStrs, Math, StrUtils, Graphics, dxMessageDialog, cxNavigator;

const
  dxThisUnitName = 'dxBarDBNav';
...
const
  PResStrs: array[TdxBarDBNavButtonType] of Pointer = (@dxSBAR_BTNCAPTION_FIRST,
    ...
    @dxSBAR_BTNCAPTION_POST, @dxSBAR_BTNCAPTION_CANCEL, @dxSBAR_BTNCAPTION_REFRESH, @dxSBAR_BTNCAPTION_APPEND);
begin
...
const
  dxBarNames: array[TdxBarDBNavButtonType] of string =
    ('First', 'Prev', 'Next', 'Last', 'Insert', 'Delete', 'Edit', 'Post', 'Cancel', 'Refresh', 'Append');
  dxNavigatorImageIndex: array [TdxBarDBNavButtonType] of Integer =
    (0, 2, 3, 5, 6, 8, 9, 10, 11, 12, 7);
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        NavigatorImages.GetImage(dxNavigatorImageIndex[FNavButton], Glyph);
      end;
      ...
      case FNavButton of
        ...
        dxbnDelete:
          if not BarDBNavigator.ConfirmDelete or
            (dxMessageBox(cxGetResourceString(@dxSBAR_DELETERECORD),
              Application.Title, MB_ICONQUESTION or MB_YESNO) = ID_YES)
          then
            Delete;
        dxbnEdit: Edit;
        dxbnPost: Post;
...
begin
  ...
  if Buttons[dxbnAppend] <> nil then
    Buttons[dxbnAppend].Enabled := CanModify;
...
begin
  ...
  begin
    ...
    if not (csLoading in ComponentState) and (BarManager <> nil) then
      ...
      begin
        ...
        begin
          AButton := BarManager.AddItem(TdxBarDBNavButton) as TdxBarDBNavButton;
          Buttons[I] := AButton;
          AButton.BarDBNavigator := Self;
  ...
  if (FVisibleButtons = []) and (BarManager <> nil) and not (csDestroying in BarManager.ComponentState) then
  begin
    AIndex := BarManager.Categories.IndexOf(FCategoryName);
