...
uses
  System.UITypes,
  Types, Windows, RTLConsts, dxBar, Classes, cxClasses, dxCoreClasses, dxCore, Graphics, cxGraphics, dxRibbonGallery, dxActions,
...
type
  ...
  { TdxRibbonColorGalleryInternalItem }

  TdxRibbonColorGalleryInternalItem = class(TdxRibbonGalleryItem)                   //for internal use
  protected
    function GetGalleryGroupsClass: TdxRibbonGalleryGroupsClass; override;
  end;

  { TdxRibbonColorGalleryGroups }

  TdxRibbonColorGalleryGroups = class(TdxRibbonGalleryGroups)                       // for internal use
  protected
    function GetGroupViewInfoClass: TdxRibbonGalleryGroupViewInfoClass; override;
  end;

  { TdxRibbonColorGalleryGroupViewInfo }

  TdxRibbonColorGalleryGroupViewInfo = class(TdxRibbonGalleryGroupViewInfo)         // for internal use
  protected
    function GetItemViewInfoClass: TdxRibbonGalleryGroupItemViewInfoClass; override;
  end;

  { TdxRibbonColorGalleryGroupItemViewInfo }

  TdxRibbonColorGalleryGroupItemViewInfo = class(TdxRibbonGalleryGroupItemViewInfo) // for internal use
  protected
...
begin
  FAutoColorButton := BarDesignController.AddInternalItem(TdxBarButton, BarManager, BarManager, FInternalItems) as TdxBarButton;
  ...
  FColorGallery := BarDesignController.AddInternalItem(TdxRibbonColorGalleryInternalItem, BarManager, BarManager, FInternalItems) as TdxRibbonColorGalleryInternalItem;
  ...
  FMoreColorsButton := BarDesignController.AddInternalItem(TdxBarButton, BarManager, BarManager, FInternalItems) as TdxBarButton;
...
begin
  ...
  try
    ...
    begin
      ...
      TdxColorDialogHelper.AddToCustomColors(CustomColors, ADialog.Color, False);
...
function TdxRibbonColorGalleryInternalItem.GetGalleryGroupsClass: TdxRibbonGalleryGroupsClass;
begin
  Result := TdxRibbonColorGalleryGroups;
end;

{ TdxRibbonColorGalleryGroups }

function TdxRibbonColorGalleryGroups.GetGroupViewInfoClass: TdxRibbonGalleryGroupViewInfoClass;
begin
  Result := TdxRibbonColorGalleryGroupViewInfo;
end;

{ TdxRibbonColorGalleryGroupViewInfo }

function TdxRibbonColorGalleryGroupViewInfo.GetItemViewInfoClass: TdxRibbonGalleryGroupItemViewInfoClass;
begin
  Result := TdxRibbonColorGalleryGroupItemViewInfo;
end;

{ TdxRibbonColorGalleryGroupItemViewInfo }

function TdxRibbonColorGalleryGroupItemViewInfo.GetContentOffset: TdxPadding;
begin
  Result := Painter.LookAndFeelPainter.GetRibbonColorGalleryContentOffsets(ScaleFactor);
  TdxRibbonGalleryBaseOptionsAccess(Options).CheckRemoveItemPadding(Result);
end;

initialization
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.InitializationStarted(dxThisUnitName, SysInit.HInstance);{$ENDIF}
  dxBarRegisterItem(TdxRibbonColorGalleryItem, TdxRibbonColorGalleryItemControl, True);
  dxBarRegisterItem(TdxRibbonColorGalleryInternalItem, TdxRibbonGalleryControl, True);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.InitializationFinished(dxThisUnitName, SysInit.HInstance);{$ENDIF}
finalization
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.FinalizationStarted(dxThisUnitName, SysInit.HInstance);{$ENDIF}
  dxBarUnregisterItem(TdxRibbonColorGalleryInternalItem);
