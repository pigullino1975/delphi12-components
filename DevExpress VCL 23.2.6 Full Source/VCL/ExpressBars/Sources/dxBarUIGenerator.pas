...
const
  dxThisUnitName = 'dxBarUIGenerator';
...
begin
  inherited;
  TcxFontNameComboBoxProperties(TcxBarEditItem(AItem).Properties).FontPreview.Visible := False;
  TcxFontNameComboBoxProperties(TcxBarEditItem(AItem).Properties).FontTypes := [cxftTTF];
end;
...
begin
  ...
  if Result = nil then
  begin
    Result := TdxBarManagerAccess(BarManager).CreateItem(ABarItemInfo.GetBarItemClass, BarManager.Owner, nil);
