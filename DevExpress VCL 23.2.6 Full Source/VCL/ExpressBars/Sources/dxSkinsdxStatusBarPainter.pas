...
uses
  Types, cxGeometry, dxDPIAwareUtils, dxCore;

const
  dxThisUnitName = 'dxSkinsdxStatusBarPainter';
...
begin
  ...
  if GetSkinInfo(AStatusBar, ASkinInfo) and (ASkinInfo.SizeGrip <> nil) then
    ASkinInfo.SizeGrip.RightToLeftDependentDraw(ACanvas.Handle, R, dxGetScaleFactor(AStatusBar), ACanvas.UseRightToLeftAlignment)
  else
