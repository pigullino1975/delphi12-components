...
uses
  ...
  dxBarSkin, dxBarSkinConsts, cxLookAndFeelPainters, dxRibbonSkins, dxRibbonSkins2013, cxLookAndFeels;
...
const
  dxThisUnitName = 'dxRibbonSkins2016';
    ...
    if AIcon <> rbdiClose then
      case AState of
        rbisHot, rbisHotInactive:
          AColor := TdxColorUtils.ChangeLightness(GetAccentColor, 0.07);
        rbisPressed:
          AColor := TdxColorUtils.ChangeLightness(GetAccentColor, -0.09);
      end
    else
      case AState of
        rbisHot, rbisHotInactive:
...
procedure TdxColorfulRibbon2016Skin.DrawQuickAccessToolbarMarkArrow(DC: HDC; const R: TRect; AState: Integer;
...
procedure TdxColorfulRibbon2016Skin.DrawQuickAccessToolbarMarkTruncated(DC: HDC; const R: TRect; AState: Integer;
  APlace: TdxQATPosition);
begin
  if APlace = qatpBelow then
    inherited
...
begin
  if LowColors then
    inherited DrawQuickAccessToolbarSmallButton(DC, R, AState, APlace)
  else
  begin
...
begin
  if LowColors then
    ...
    DrawQuickAccessToolbarSmallButtonDropButtonMainPart(DC, R, AState, APlace);
end;
...
procedure TdxColorfulRibbon2016Skin.DrawStatusBar(DC: HDC; const R: TRect; AIsLeftSideRaised, AIsRightSideRaised: Boolean);
begin
  if LowColors then
    inherited
  else
  ...
  case APart of
    ...
    rspTabHeaderText:
      case AState of
        ...
        DXBAR_DISABLED:
          Result := TdxColorUtils.MultiplyLightness(GetAccentColor, 1.5);
      else
        Result := $F9F9F9;
      end;
...
begin
  DrawQuickAccessToolbarArrowDown(DC, R, AState, qatpAbove);
end;
...
begin
  DrawQuickAccessToolbarMarkArrow(DC, R, AState, qatpAbove);
end;

procedure TdxColorfulRibbon2016Skin.DrawTabAreaMarkTruncated(DC: HDC; const R: TRect; AState: Integer);
begin
  DrawQuickAccessToolbarMarkTruncated(DC, R, AState, qatpAbove);
end;
...
begin
  case AState of
    DXBAR_HOT:
      Result := GetMinimizeRibbonButtonGlyphColor(AState);
    DXBAR_PRESSED:
      Result := TdxColorUtils.ChangeLightness(GetMinimizeRibbonButtonGlyphColor(AState), -0.05);
  else
    Result := inherited;
  end;
end;
