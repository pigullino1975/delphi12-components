...
uses
  ...
  dxSpreadSheetStyles, dxSpreadSheetConditionalFormatting, dxSpreadSheetPrinting, dxGenerics;
...
type
  ...
  public
    ...
    procedure StoreData(AStream: TStream);
  end;
  ...
  { TdxSpreadSheetHistoryAbstractChangeCellStyleCommand }

  TdxSpreadSheetHistoryAbstractChangeCellStyleCommand = class(TdxSpreadSheetHistoryCustomCommand)
  strict private
    FHandle: TdxHashTableItem;
  protected
    function GetStyleHandle: TdxSpreadSheetCellStyleHandle; virtual; abstract;
    procedure SetStyleHandle(AValue: TdxSpreadSheetCellStyleHandle); virtual; abstract;
    procedure Store(AStyleHandle: TdxSpreadSheetCellStyleHandle); virtual;
    procedure Restore;
  public
    destructor Destroy; override;
    procedure Redo; override;
  ...
  TdxSpreadSheetHistoryChangeCellStyleCommand = class(TdxSpreadSheetHistoryAbstractChangeCellStyleCommand)
  protected
    FOwner: TObject;
    FPoint: TPoint;

    function GetStyleHandle: TdxSpreadSheetCellStyleHandle; override;
    procedure SetStyleHandle(AValue: TdxSpreadSheetCellStyleHandle); override;
  public
  ...
  { TdxSpreadSheetHistoryChangeDefaultCellStyleCommand }

  TdxSpreadSheetHistoryChangeDefaultCellStyleCommand = class(TdxSpreadSheetHistoryAbstractChangeCellStyleCommand)
  protected
    function GetStyleHandle: TdxSpreadSheetCellStyleHandle; override;
    procedure SetStyleHandle(AValue: TdxSpreadSheetCellStyleHandle); override;
    procedure Store(AStyleHandle: TdxSpreadSheetCellStyleHandle); override;
  public
  ...
  protected
    FIndexes: TdxIntegerList;

    procedure RedoUndo; virtual; abstract;
    procedure StoreIndexes(AIndexes: TdxIntegerList; AGroup: TdxSpreadSheetTableItemGroup);
  public
  ...
  strict private
    function GetDefinedNames: TdxSpreadSheetDefinedNames;
  protected
    FCaption: string;
    FComment: string;
    FIndex: Integer;
  ...
  public
    constructor Create(const ADefinedName: TdxSpreadSheetDefinedName);
    class function ActionClass: TdxSpreadSheetHistoryActionClass; override;
    procedure Redo; override;
    procedure Undo; override;
  end;

  { TdxSpreadSheetHistoryChangeDefinedNameCommand }

  TdxSpreadSheetHistoryChangeDefinedNameCommand = class(TdxSpreadSheetHistoryCreateDefinedNameCommand)
  public
    NewCaption: string;
    NewComment: string;
    NewReference: string;
    NewScope: TdxSpreadSheetCustomView;
  ...
  { TdxSpreadSheetHistoryDeleteDefinedNameCommand }

  TdxSpreadSheetHistoryDeleteDefinedNameCommand = class(TdxSpreadSheetHistoryCreateDefinedNameCommand)
  public
    class function ActionClass: TdxSpreadSheetHistoryActionClass; override;
    procedure Redo; override;
    procedure Undo; override;
...
const
  dxThisUnitName = 'dxSpreadSheetCoreHistory';
...
destructor TdxBufferStorage.Destroy;
begin
  FreeMemory(FBuffer);
  inherited;
end;

function TdxBufferStorage.GetDataStream: TStream;
begin
  Result := TMemoryStream.Create;
  WriteBufferToStream(Result, FBuffer, FSize);
  Result.Position := 0;
end;

procedure TdxBufferStorage.StoreData(AStream: TStream);
begin
  FSize := AStream.Size;
  FBuffer := ReallocMemory(FBuffer, FSize);
  AStream.Position := 0;
  ReadBufferFromStream(AStream, FBuffer, FSize);
end;
...
begin
  ...
  try
    AWriter := TcxWriter.Create(ADataStream, dxSpreadSheetBinaryFormatVersion);
    try
      TdxSpreadSheetCellAccess(ACell).SaveToStream(AWriter);
    finally
      AWriter.Free;
    end;
    FData.StoreData(ADataStream);
  finally
    ADataStream.Free;
  end;
end;
...
var
  ...
  AReader: TcxReader;
begin
  AReader := TcxReader.Create(FData.GetDataStream, dxSpreadSheetBinaryFormatVersion);
  try
    ACell := Cell;
    Store(ACell);
...
destructor TdxSpreadSheetHistoryAbstractChangeCellStyleCommand.Destroy;
begin
  dxChangeHandle(FHandle, nil);
  inherited Destroy;
end;

procedure TdxSpreadSheetHistoryAbstractChangeCellStyleCommand.Redo;
begin
  Restore;
end;

procedure TdxSpreadSheetHistoryAbstractChangeCellStyleCommand.Undo;
begin
  Restore;
end;

procedure TdxSpreadSheetHistoryAbstractChangeCellStyleCommand.Store(AStyleHandle: TdxSpreadSheetCellStyleHandle);
begin
  dxChangeHandle(FHandle, AStyleHandle);
end;

procedure TdxSpreadSheetHistoryAbstractChangeCellStyleCommand.Restore;
var
  AHandle: TdxHashTableItem;
begin
  AHandle := nil;
  dxChangeHandle(AHandle, FHandle);
  try
    Store(GetStyleHandle);
    SetStyleHandle(TdxSpreadSheetCellStyleHandle(AHandle));
  finally
...
constructor TdxSpreadSheetHistoryChangeDefaultCellStyleCommand.Create(ASpreadSheet: TdxCustomSpreadSheet);
begin
  inherited Create;
  Store(ASpreadSheet.DefaultCellStyle.Handle);
end;

class function TdxSpreadSheetHistoryChangeDefaultCellStyleCommand.ActionClass: TdxSpreadSheetHistoryActionClass;
begin
  Result := TdxSpreadSheetHistoryEditCellAction;
end;

function TdxSpreadSheetHistoryChangeDefaultCellStyleCommand.GetStyleHandle: TdxSpreadSheetCellStyleHandle;
begin
  Result := View.SpreadSheet.DefaultCellStyle.Handle;
end;

procedure TdxSpreadSheetHistoryChangeDefaultCellStyleCommand.SetStyleHandle(AValue: TdxSpreadSheetCellStyleHandle);
begin
  View.SpreadSheet.DefaultCellStyle.Handle := AValue;
end;

procedure TdxSpreadSheetHistoryChangeDefaultCellStyleCommand.Store(AStyleHandle: TdxSpreadSheetCellStyleHandle);
begin
  inherited Store(AStyleHandle.Clone);
end;
...
begin
  dxChangeHandle(FStyleHandle, AValue);
end;
...
begin
  ...
  FIndexes := TdxIntegerList.Create;
...
procedure TdxSpreadSheetHistoryCustomGroupCommand.StoreIndexes(AIndexes: TdxIntegerList; AGroup: TdxSpreadSheetTableItemGroup);
var
  AIndex: Integer;
begin
...
begin
  inherited Create(AGroup);
  FPrevIndexes := TdxIntegerList.Create;
end;
...
constructor TdxSpreadSheetHistoryCreateDefinedNameCommand.Create(const ADefinedName: TdxSpreadSheetDefinedName);
begin
  FCaption := ADefinedName.Caption;
  FComment := ADefinedName.Comment;
  FReference := ADefinedName.Reference;
  FScope := ADefinedName.Scope;
  FIndex := ADefinedName.Index;
end;
...
begin
  DefinedNames.BeginUpdate;
  try
    ADefinedName := DefinedNames.Add(FCaption, FReference, FScope);
    ADefinedName.Comment := FComment;
    if ADefinedName.Index <> FIndex then
      TdxSpreadSheetDefinedNamesAccess(DefinedNames).ItemList.Move(ADefinedName.Index, FIndex);
  finally
    DefinedNames.EndUpdate;
  end;
end;

procedure TdxSpreadSheetHistoryCreateDefinedNameCommand.Undo;
begin
  DefinedNames.GetItemByName(FCaption, FScope).Free;
end;

function TdxSpreadSheetHistoryCreateDefinedNameCommand.GetDefinedNames: TdxSpreadSheetDefinedNames;
begin
  Result := View.SpreadSheet.DefinedNames;
end;

{ TdxSpreadSheetHistoryChangeDefinedNameCommand }

class function TdxSpreadSheetHistoryChangeDefinedNameCommand.ActionClass: TdxSpreadSheetHistoryActionClass;
begin
  Result := TdxSpreadSheetHistoryChangeDefinedNameAction;
end;

procedure TdxSpreadSheetHistoryChangeDefinedNameCommand.Redo;
var
  AName: TdxSpreadSheetDefinedName;
begin
  AName := DefinedNames.GetItemByName(NewCaption, NewScope);
  if AName <> nil then
  begin
    DefinedNames.BeginUpdate;
    try
      AName.Caption := FCaption;
      AName.Comment := FComment;
      AName.Reference := FReference;
      AName.Scope := FScope;
    finally
      DefinedNames.EndUpdate;
    end;
    ExchangePointers(NewScope, FScope);
    ExchangeStrings(NewCaption, FCaption);
    ExchangeStrings(NewReference, FReference);
    ExchangeStrings(NewComment, FComment);
  end;
end;

procedure TdxSpreadSheetHistoryChangeDefinedNameCommand.Undo;
begin
  Redo;
end;
...
procedure TdxSpreadSheetHistoryDeleteDefinedNameCommand.Redo;
begin
  inherited Undo;
end;

procedure TdxSpreadSheetHistoryDeleteDefinedNameCommand.Undo;
begin
  inherited Redo;
end;

class function TdxSpreadSheetHistoryDeleteDefinedNameCommand.ActionClass: TdxSpreadSheetHistoryActionClass;
begin
  Result := TdxSpreadSheetHistoryDeleteDefinedNameAction;
end;
...
begin
  ...
  try
    ProcessCore(FArea, FModification, FIsHorizontal);
  finally
    Free;
  end;
end;
