...
type
  ...
  public
    class function GetDefaultPosition(ACell: TdxSpreadSheetCell): TRect; static; // for internal use only
  ...
  protected
    ...
    FLinePoint1: TPoint;
    FLinePoint2: TPoint;
    FMarkPosition: TLeftRight;

    procedure AdjustAbsoluteBounds; virtual;
    procedure CalculateArrow; virtual;
    procedure CalculateBounds; override;
    procedure CalculateLinePoints(const ACellBounds, AContainerBounds: TRect; AMarkPosition: TLeftRight); virtual;
  ...
  public
    ...
    property LinePoint1: TPoint read FLinePoint1;
    property LinePoint2: TPoint read FLinePoint2;
  end;
  ...
  protected
    ...
    //
    property CellBounds: TRect read GetCellBounds;
  public
  ...
  strict private
    FAlignToCells: Boolean;
    FKeepAspectUsingCornerHandles: Boolean;
    FKeepCenterPointer: Boolean;
  ...
  protected
    ...
    procedure AdjustDeltasToAlignToCells(var ADeltaX, ADeltaY: Single);
    procedure AdjustDeltasToKeepAspectRatio(var ADeltaX, ADeltaY: Single);
    ...
    procedure UpdateResizeSettings;

  {$WARNINGS OFF}
    property AlignToCells: Boolean read FAlignToCells;
    property KeepAspectUsingCornerHandles: Boolean read FKeepAspectUsingCornerHandles;
    property KeepCenterPointer: Boolean read FKeepCenterPointer;
  {$WARNINGS ON}
  public
...
uses
  Math, SysUtils, RTLConsts, cxControls, dxHashUtils, dxTypeHelpers, dxGDIPlusAPI,
  dxSpreadSheetCoreHistory, dxSpreadSheetStrs, dxSpreadSheetCoreStrs;

const
  dxThisUnitName = 'dxSpreadSheetContainers';
...
begin
  if dxChangeHandle(FImageHandle, AValue) then
    Changed;
end;
...
begin
  if Source is TdxSpreadSheetCustomTextBox then
  begin
    AlignHorz := TdxSpreadSheetCustomTextBox(Source).AlignHorz;
    AlignVert := TdxSpreadSheetCustomTextBox(Source).AlignVert;
    AutoSize := TdxSpreadSheetCustomTextBox(Source).AutoSize;
    ContentOffsets := TdxSpreadSheetCustomTextBox(Source).ContentOffsets;
    Text := TdxSpreadSheetCustomTextBox(Source).Text;
    Font := TdxSpreadSheetCustomTextBox(Source).Font;
    WordWrap := TdxSpreadSheetCustomTextBox(Source).WordWrap;
  end;
end;
...
begin
  if dxChangeHandle(FText, AValue) then
    Changed;
end;
...
procedure TdxSpreadSheetTextBoxViewInfo.RightToLeftConversion(const AClientBounds: TRect);
begin
  StringFormat.FormatFlags := StringFormat.FormatFlags or Integer(StringFormatFlagsDirectionRightToLeft);
end;
...
begin
  ...
  try
    ...
    ABounds := ValidateBounds(ABounds);
...
procedure TdxSpreadSheetCustomTextBoxContainerCalculator.UpdateAnchors(const ABounds: TRect);
  ...
  begin
    ...
    begin
      BeginAutoSize;
      try
        UpdateAnchors(ValidateBounds(cxRectCenter(ABounds, CalculateAutoSize(cxRectWidth(ABounds)))));
      finally
        EndAutoSize;
      end;
    end
...
begin
  ...
  try
    ...
    Result := cxSizeAddMargins(cxSize(AViewInfo.TextBoxViewInfo.MeasureSize(AWidth).Ceil), TextBox.ContentOffsets);
  finally
    AViewInfo.Free;
  end;
end;
...
procedure TdxSpreadSheetCustomTextBoxContainerViewInfo.RightToLeftConversion(const AClientBounds: TRect);
begin
  inherited;
  TextBoxViewInfo.RightToLeftConversion(AClientBounds);
end;
...
class function TdxSpreadSheetCommentContainer.GetDefaultPosition(ACell: TdxSpreadSheetCell): TRect;
begin
  Result := TdxSpreadSheetTableViewAccess(ACell.View).GetAbsoluteCellBounds(ACell.RowIndex, ACell.ColumnIndex, True);
...
procedure TdxSpreadSheetCommentContainerConnectionViewInfo.Offset(X, Y: Integer);
var
  I: Integer;
begin
  inherited;
  FLinePoint1 := cxPointOffset(FLinePoint1, X, Y);
  FLinePoint2 := cxPointOffset(FLinePoint2, X, Y);
  for I := Low(FArrowPoints) to High(FArrowPoints) do
    FArrowPoints[I] := cxPointOffset(FArrowPoints[I], X, Y);
end;

procedure TdxSpreadSheetCommentContainerConnectionViewInfo.RightToLeftConversion(const AClientBounds: TRect);
begin
  inherited;
  FMarkPosition := TdxRightToLeftLayoutConverter.ConvertAlignment(FMarkPosition);
  CalculateBounds;
end;
...
begin
  CalculateLinePoints(ContainerViewInfo.CellBounds, ContainerViewInfo.ContentBounds.DeflateToTRect, FMarkPosition);
...
procedure TdxSpreadSheetCommentContainerConnectionViewInfo.CalculateLinePoints(
  const ACellBounds, AContainerBounds: TRect; AMarkPosition: TLeftRight);
begin
  if AMarkPosition = taLeftJustify then
    FLinePoint1 := ACellBounds.TopLeft
  else
    FLinePoint1 := Point(ACellBounds.Right, ACellBounds.Top);

  if AContainerBounds.Left >= LinePoint1.X then
    FLinePoint2 := AContainerBounds.TopLeft
  else
    if AContainerBounds.Bottom < ACellBounds.Top then
      FLinePoint2 := Point(AContainerBounds.Right, AContainerBounds.Bottom)
    else
      FLinePoint2 := Point(AContainerBounds.Right, AContainerBounds.Top);

  if cxRectPtIn(AContainerBounds, LinePoint1) and cxRectPtIn(AContainerBounds, LinePoint2) then
    FLinePoint2 := FLinePoint1;
end;
...
procedure TdxSpreadSheetCommentContainerConnectionViewInfo.DoCalculate;
begin
  FMarkPosition := taRightJustify;
  inherited;
end;
...
function TdxSpreadSheetCommentContainerViewInfo.GetCellBounds: TRect;
var
...
begin
  ...
  begin
    ...
    Result := AView.ViewInfo.GetAreaBounds(AView.MergedCells.ExpandArea(ACell.ColumnIndex, ACell.RowIndex));
  end
  else
    Result := cxNullRect;
end;
...
begin
  if not Containers.Find(FContainer, Result) then
    raise EListError.CreateFmt(SListIndexError, [-1]);
end;
...
procedure TdxSpreadSheetContainerResizeDragAndDropObject.AdjustDeltasToAlignToCells(var ADeltaX, ADeltaY: Single);
var
  dX, dY: Integer;
  R: TdxRectF;
begin
...
procedure TdxSpreadSheetContainerResizeDragAndDropObject.AdjustDeltasToKeepAspectRatio(var ADeltaX, ADeltaY: Single);

  function CalculateDelta(AFlip, AIsFarSide: Boolean; AWidth, AOriginalWidth, AOriginalHeight, AAspectRatio: Single): Single;
  const
    Map: array[Boolean] of Single = (-1, 1);
  var
    ADeltaWidth: Single;
  begin
    ADeltaWidth := Abs(AWidth) - AOriginalWidth;
    if KeepCenterPointer then
      ADeltaWidth := ADeltaWidth * 0.5;
    if AFlip and not KeepCenterPointer then
      Result := -Map[AIsFarSide] * (2 * AOriginalHeight + ADeltaWidth * AAspectRatio)
    else
      Result :=  Map[AIsFarSide] * ADeltaWidth * AAspectRatio;
  end;
...
begin
  ...
  AAspectRatio := Bounds.Height / Bounds.Width;
  ATarget := CalculateContentBounds(ADeltaX, ADeltaY);

  if AAspectRatio * Abs(ATarget.Width) > Abs(ATarget.Height) then
    ADeltaY := CalculateDelta(ATarget.Height < 0, Marker in [smBottomLeft, smBottomRight], ATarget.Width, Bounds.Width, Bounds.Height, AAspectRatio)
  else
    ADeltaX := CalculateDelta(ATarget.Width < 0, Marker in [smBottomRight, smTopRight], ATarget.Height, Bounds.Height, Bounds.Width, 1 / AAspectRatio);
end;
...
begin
  ...
  FKeepAspectUsingCornerHandles := TdxSpreadSheetContainerAccess(Container).KeepAspectUsingCornerHandles;
...
begin
  ...
  if AlignToCells then
    AdjustDeltasToAlignToCells(ADeltaX, ADeltaY);

  if KeepAspectUsingCornerHandles then
  begin
    if Marker in [smTopLeft, smTopRight, smBottomRight, smBottomLeft] then
      AdjustDeltasToKeepAspectRatio(ADeltaX, ADeltaY);
  end;
...
procedure TdxSpreadSheetContainerResizeDragAndDropObject.UpdateResizeSettings;
begin
  FAlignToCells := IsAltPressed;
  FKeepCenterPointer := IsControlPressed;
end;
...
begin
  ...
  UpdateViewInfoBounds(R.InflateToTRect);
end;
