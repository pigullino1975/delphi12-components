...
const
  dxSpreadSheetBinaryFormatVersion = 16;
...
type
  ...
  strict private
    FCellStyles: TdxSpreadSheetCellStyleHandleList;
  ...
  protected
    ...
    //
    property CellStyles: TdxSpreadSheetCellStyleHandleList read FCellStyles;
...
begin
  ...
  FCellStyles := TdxSpreadSheetCellStyleHandleList.Create;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        AStyleHandle := Reader.ReadInteger;
        if AStyleHandle <> dxSpreadSheetBinaryDefaultItem then
          ACell.StyleHandle := CellStyles[AStyleHandle];
...
begin
  ...
  begin
    ...
    AStyleHandle := Reader.ReadInteger;
    if AStyleHandle <> dxSpreadSheetBinaryDefaultItem then
      AItem.Style.Handle := CellStyles[AStyleHandle];
...
begin
  ...
  begin
    ...
    if Reader.Version > 12 then
      AView.Options.RightToLeftLayout := TdxDefaultBoolean(Reader.ReadInteger);
  end;
...
procedure TdxSpreadSheetBinaryReader.ReadMetadata;
begin
  if Reader.Version >= 15 then
  begin
    SpreadSheet.OptionsMetadata.CreatedBy := Reader.ReadWideString;
    SpreadSheet.OptionsMetadata.Description := Reader.ReadWideString;
    SpreadSheet.OptionsMetadata.Keywords := Reader.ReadWideString;
    SpreadSheet.OptionsMetadata.LastModifiedBy := Reader.ReadWideString;
    SpreadSheet.OptionsMetadata.Subject := Reader.ReadWideString;
    SpreadSheet.OptionsMetadata.Title := Reader.ReadWideString;
    SpreadSheet.OptionsMetadata.Created := Reader.ReadDateTime;
    SpreadSheet.OptionsMetadata.LastModified := Reader.ReadDateTime;
    SpreadSheet.OptionsMetadata.LastPrinted := Reader.ReadDateTime;
  end;
end;
...
begin
  ...
  for I := 0 to CellStyles.Count - 1 do
    CellStyles[I].SaveToStream(Writer);
end;
...
procedure TdxSpreadSheetBinaryWriter.WriteMetadata;
begin
  Writer.WriteWideString(SpreadSheet.OptionsMetadata.CreatedBy);
  Writer.WriteWideString(SpreadSheet.OptionsMetadata.Description);
  Writer.WriteWideString(SpreadSheet.OptionsMetadata.Keywords);
  Writer.WriteWideString(SpreadSheet.OptionsMetadata.LastModifiedBy);
  Writer.WriteWideString(SpreadSheet.OptionsMetadata.Subject);
  Writer.WriteWideString(SpreadSheet.OptionsMetadata.Title);
  Writer.WriteDateTime(SpreadSheet.OptionsMetadata.Created);
  Writer.WriteDateTime(SpreadSheet.OptionsMetadata.LastModified);
  Writer.WriteDateTime(SpreadSheet.OptionsMetadata.LastPrinted);
end;
...
begin
  ...
  while ARow <> nil do
  begin
    Writer.WriteInteger(ARow.Index);
    ...
    begin
      ...
      TdxSpreadSheetCellAccess(ACell).SaveToStream(Writer);
      ACell := ACell.Next;
      Inc(ACellCount);
    end;
  ...
  TdxDynamicItemListAccess(AItems).ForEach(
    ...
    begin
      ...
      if not AItem.Style.IsDefault then
        ...
        Writer.WriteInteger(dxSpreadSheetBinaryDefaultItem);
...
begin
  ...
  Writer.WriteInteger(Ord(AView.Options.RightToLeftLayout));
