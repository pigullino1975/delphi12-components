...
uses
  SysUtils, Math, StrUtils, cxGraphics, dxStringHelper, dxCoreGraphics;

const
  dxThisUnitName = 'dxSpreadSheetPrinting';
...
procedure TdxSpreadSheetTableViewOptionsPrintRect.DoChanged;
begin
  Assigned := True;
  inherited;
end;
...
begin
  ...
  begin
    FScaleMode := Value;
    if FScaleMode = oppsmFitToPage then
    begin
      FFitToHeight := Max(FFitToHeight, 1);
      FFitToWidth := Max(FFitToWidth, 1);
    end;
    Changed;
  end;
end;
...
begin
  if FFont <> nil then
    FFont.Color := TdxAlphaColors.FromHexCode(Copy(S, AIndex, 6));
  Inc(AIndex, 6);
  Result := '';
end;
