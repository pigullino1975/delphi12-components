...
uses
  System.UITypes,
  Types, Windows, Classes, Graphics, dxGDIPlusClasses, dxCore, dxCoreGraphics, dxCoreClasses,
  dxSpreadSheetCore, dxSpreadSheetPrinting, dxSpreadSheetTypes, dxHashUtils;
...
const
  dxThisUnitName = 'dxSpreadSheetFormatUtils';
...
begin
  ...
  try
    if AView.OptionsPrint.Source.ColumnsToRepeat.Assigned then
      ABuffer.Append(TdxSpreadSheetDefinedNameHelper.BuildReference(AView, AView.OptionsPrint.Source.ColumnsToRepeat.Rect));
    if AView.OptionsPrint.Source.RowsToRepeat.Assigned then
    begin
      if ABuffer.Length > 0 then
        ABuffer.Append(',');
      ABuffer.Append(TdxSpreadSheetDefinedNameHelper.BuildReference(AView, AView.OptionsPrint.Source.RowsToRepeat.Rect));
    end;
