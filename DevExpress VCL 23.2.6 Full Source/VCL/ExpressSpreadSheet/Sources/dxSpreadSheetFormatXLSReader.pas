...
uses
  ...
  dxSpreadSheetProtection, dxSpreadSheetCoreStyles, dxSpreadSheetStyles, dxGenerics;
...
type
  ...
  public
    ...
    property Parts: TdxIntegerList read FParts;
  ...
  { TdxSpreadSheetXLSCustomMSODrawingReader }

  TdxSpreadSheetXLSCustomMSODrawingReader = class
  strict private
    FOwner: TdxSpreadSheetXLSReader;
  protected
    property Owner: TdxSpreadSheetXLSReader read FOwner;
  public
    ...
    function CreateContainer(AContainerType: Integer = -1): TdxSpreadSheetContainer; virtual; abstract;
  ...
  strict private
    ...
    FContainers: TdxMap<Word, TdxSpreadSheetContainer>;
    ...
    FMSODrawingReader: TdxSpreadSheetXLSCustomMSODrawingReader;
    FNames: TStringList;
    ...
    FSheetOffset: TdxIntegerList;
    FStrings: TdxHashTableItemList;
  ...
  protected
    ...
    procedure ConvertContainer(AContainer: TdxSpreadSheetContainer;
      ATargetContainerClass: TdxSpreadSheetContainerClass; var ATargetContainer); overload;
    procedure ConvertContainer(AContainer: TdxSpreadSheetContainer;
      AMinContainerClass, ATargetContainerClass: TdxSpreadSheetContainerClass; var ATargetContainer); overload;
    ...
    procedure ReadMetadata(AStream: TStream);
  ...
  public
    ...
    property Containers: TdxMap<Word, TdxSpreadSheetContainer> read FContainers;
    ...
    property SheetOffset: TdxIntegerList read FSheetOffset;
    property Strings: TdxHashTableItemList read FStrings;
...
const
  dxThisUnitName = 'dxSpreadSheetFormatXLSReader';
...
constructor TdxSpreadSheetXLSCustomMSODrawingReader.Create(AOwner: TdxSpreadSheetXLSReader);
begin
  FOwner := AOwner;
end;
...
begin
  ...
  FSheetOffset := TdxIntegerList.Create;
  FStrings := TdxHashTableItemList.Create;
  ...
  FData := Document.StreamByName(sdxXLSWorkbookStream);
  ...
  FContainers := TdxMap<Word, TdxSpreadSheetContainer>.Create;
  ThemeHelper := TdxSpreadSheetXLSReaderThemeHelper.Create;
  FMSODrawingReader := TdxSpreadSheetMSODrawingReader.Create(Self);
...
begin
  ...
  ReadMetadata(Document.StreamByName(sdxXLSSummaryInformationStream));
  ResolveHyperlinks;
  SpreadSheet.ActiveSheetIndex := ActiveTabIndex;
  SpreadSheet.Password := FPassword;
end;

procedure TdxSpreadSheetXLSReader.ConvertContainer(AContainer: TdxSpreadSheetContainer;
  ATargetContainerClass: TdxSpreadSheetContainerClass; var ATargetContainer);
begin
  ConvertContainer(AContainer, ATargetContainerClass, ATargetContainerClass, ATargetContainer);
end;

procedure TdxSpreadSheetXLSReader.ConvertContainer(AContainer: TdxSpreadSheetContainer;
  AMinContainerClass, ATargetContainerClass: TdxSpreadSheetContainerClass; var ATargetContainer);
var
  AContainerID: Word;
begin
  if AContainer.InheritsFrom(AMinContainerClass) then
    TObject(ATargetContainer) := AContainer
  else
    try
      CurrentSheet.Containers.Add(ATargetContainerClass, ATargetContainer);
      if Containers.TryGetKey(AContainer, AContainerID) then
      begin
        Containers.DeleteByKey(AContainerID);
        Containers.Add(AContainerID, TdxSpreadSheetContainer(ATargetContainer));
      end;
      TdxSpreadSheetContainer(ATargetContainer).Assign(AContainer);
    finally
      AContainer.Free;
    end;
end;
...
begin
  ...
  begin
    ...
    AFont.Handle := AddFont(AFontHandle);
  end;
...
var
  ...
  ASharedStringNew: TdxSpreadSheetSharedString;
  I: Integer;
begin
  for I := 0 to Strings.Count - 1 do
  begin
    ASharedString := Strings[I];
    ...
    ASharedStringNew := StringTable.Add(ASharedString);
    if ASharedStringNew <> ASharedString then
    begin
      Strings.List[I] := nil; 
      Strings[I] := ASharedStringNew;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    if ABrush.ForegroundColor = $40 then
      ABrush.ForegroundColor := clBlack
    else
...
begin
  ...
  RegisterReader(brcNameCmt, ReadNameComment);
  ...
  RegisterReader(brcMSODRAWINGGROUP, ReadMSODrawing);
...
const
  BlockInfoWordSize = 3;
var
  AIndex, ARow, AStartColumn, AEndColumn, ADataCount, AAvailableCount: Integer;
begin
  ARow := Reader.Words[0];
  AStartColumn := Reader.Words[1];
  AEndColumn := Reader.Words[RecordReader.Size div 2 - 1];
  ADataCount := AEndColumn - AStartColumn + 1;
  AAvailableCount := RecordReader.Size div 2 - BlockInfoWordSize;
  if ADataCount > AAvailableCount then
    ADataCount := AAvailableCount;
  for AIndex := 0 to ADataCount - 1 do
...
var
  AAuthorLength: Word;
  AColumn: Word;
  ACommentContainer: TdxSpreadSheetCommentContainerAccess;
...
begin
  ...
  begin
    ConvertContainer(AContainer, TdxSpreadSheetCommentContainer, ACommentContainer);
    ACommentContainer.Cell := CurrentSheet.CreateCell(ARow, AColumn);
    ACommentContainer.Visible := AOptions and $2 <> 0;
    if not AreAnchorsAssigned(ACommentContainer) then
    begin
      ACommentContainer.AnchorType := catAbsolute;
      ACommentContainer.Calculator.UpdateAnchors(TdxSpreadSheetCommentContainer.GetDefaultPosition(ACommentContainer.Cell));
    end;
...
procedure TdxSpreadSheetXLSReader.ReadMetadata(AStream: TStream);
var
  AMetadata: TdxSpreadSheetOptionsMetadata;
  APropertySet: TdxOLEDocumentPropertySet;
  ASection: TdxOLEDocumentPropertySetSection;
begin
  if AStream <> nil then
  begin
    APropertySet := TdxOLEDocumentPropertySet.Create;
    try
      APropertySet.LoadFromStream(AStream);
      if APropertySet.Find(FMTID_SummaryInformation, ASection) then
      begin
        AMetadata := SpreadSheet.OptionsMetadata;
        AMetadata.Created := ASection.Get(PID_CREATE_DTM, 0.0);
        AMetadata.CreatedBy := ASection.Get(PID_AUTHOR, EmptyStr);
        AMetadata.Description := ASection.Get(PID_COMMENTS, EmptyStr);
        AMetadata.Keywords := ASection.Get(PID_KEYWORDS, EmptyStr);
        AMetadata.LastModifiedBy := ASection.Get(PID_LASTAUTHOR, EmptyStr);
        AMetadata.LastPrinted := ASection.Get(PID_LASTPRINTED, 0.0);
        AMetadata.LastModified := ASection.Get(PID_LASTSAVE_DTM, 0.0);
        AMetadata.Subject := ASection.Get(PID_SUBJECT, EmptyStr);
        AMetadata.Title := ASection.Get(PID_TITLE, EmptyStr);
      end;
    finally
      APropertySet.Free;
    end;
...
begin
  ...
  AContainerId := Reader.ReadWord;
  if AType in AllowedObjects then
  begin
    if FMSODrawingReader.HasData then
      Containers.Add(AContainerId, FMSODrawingReader.CreateContainer(AType))
    else
      if CurrentSheet.Containers.Count > 0 then
      begin
        CurrentSheet.Containers.Add(TdxSpreadSheetContainerClass(CurrentSheet.Containers.Last.ClassType), AContainer);
        AContainer.Assign(CurrentSheet.Containers.Last);
        Containers.Add(AContainerId, AContainer);
      end;
  end;
end;
...
var
  AAlignFlags: Word;
  AContainer: TdxSpreadSheetCustomTextBoxContainerAccess;
  ARun: TdxSpreadSheetFormattedSharedStringRun;
  ARunCount: Word;
  ATextLength: Word;
begin
  ...
  ConvertContainer(CurrentSheet.Containers.Last, TdxSpreadSheetCustomTextBoxContainer, TdxSpreadSheetTextBoxContainer, AContainer);
  ...
  try
    ...
    if ATextLength <= 0 then
      AContainer.TextBox.TextAsString := ''
    else
      if ARunCount > 0 then
      begin
        AContainer.TextBox.Text := ReadFormattedString(ATextLength, ARunCount);
        ARun := TdxSpreadSheetFormattedSharedString(AContainer.TextBox.Text).Runs[0];
        if ARun.FontHandle <> nil then
          AContainer.TextBox.Font.Handle := ARun.FontHandle;
      end
      else
...
procedure TdxSpreadSheetXLSReader.ReadNameComment;
var
  AComment: string;
  ACommentLength: Word;
  ADefinedName: TdxSpreadSheetDefinedName;
...
begin
  ...
  Reader.ReadWord; 
  Reader.ReadInt64; 
  ANameLength := Reader.ReadWord;
  ACommentLength := Reader.ReadWord;
  AName := Reader.XLS_ReadString(Reader.ReadByte, ANameLength);
  AComment := Reader.XLS_ReadString(Reader.ReadByte, ACommentLength);

  ADefinedName := SpreadSheet.DefinedNames.GetItemByName(AName, nil);
  if ADefinedName <> nil then
    ADefinedName.Comment := AComment;
end;
...
begin
  ...
  for AIndex := 0 to Strings.Count - 1 do
    Strings[AIndex] := Reader.XLS_ReadSharedString;
end;
...
begin
  ...
  CurrentSheet.Options.RightToLeftLayout := TdxDefaultBoolean(AOptions and $40 = $40);
