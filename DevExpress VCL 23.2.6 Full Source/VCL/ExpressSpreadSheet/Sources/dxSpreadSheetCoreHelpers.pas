...
type
  ...
  public
    class procedure Calculate(ACell: TdxSpreadSheetCell);
    class procedure MergeBorders(ACurrRowCell, APrevRowCell, ANextRowCell: TdxSpreadSheetCell;
      ACellStyles: TdxSpreadSheetCellStyles); overload;
    class procedure MergeBorders(ACell, ALeftCell, ATopCell, ARightCell, ABottomCell: TdxSpreadSheetCell;
      ACellStyles: TdxSpreadSheetCellStyles; ATakeWeightIntoAccount: Boolean); overload;
    class procedure MergeBorders(var ACellBorders: TdxSpreadSheetBordersHandle;
      ALeftCell, ATopCell, ARightCell, ABottomCell: TdxSpreadSheetCell; ATakeWeightIntoAccount: Boolean); overload;
    class procedure MergeBorders(AView: TdxSpreadSheetTableView); overload;
  end;
  ...
  public
    procedure Process(AArea: TRect); overload;
    procedure ProcessCore(const AArea: TRect; AModification: TdxSpreadSheetCellsModification; AIsHorizontal: Boolean); // for history, for internal use
    //
  ...
  protected
    ...
    procedure CalculateStyle(ANewStyle, ALeftSourceStyle, ARightSourceStyle: TdxSpreadSheetCellStyle;
      AIsFirst, AIsLast: Boolean); virtual;
    procedure CalculateStyleForNewCells(ATarget, ALeftSource, ARightSource: TdxSpreadSheetTableItem;
      AOppositeItems: TdxSpreadSheetTableItems; AStartIndex, AFinishIndex: Integer; AIsFirst, AIsLast: Boolean);
  ...
  { TdxSpreadSheetTableViewMergedCellStyleHelper }

  TdxSpreadSheetTableViewMergedCellStyleHelper = class(TdxSpreadSheetCustomTableViewHelper)
  strict private
    ...
    FUseActiveCellAsPattern: Boolean;
  ...
  public
    constructor Create(AMergeCell: TdxSpreadSheetMergedCell; AUseActiveCellAsPattern: Boolean); reintroduce;
    class procedure Calculate(AMergeCell: TdxSpreadSheetMergedCell; AUseActiveCellAsPattern: Boolean = False);
  end;
  ...
  TdxSpreadSheetTableViewEnumCellStylesHelper = class
  strict private type
  {$REGION 'Private Types'}
    TCache = class
    strict private
      FCrossStyles: TObjectDictionary<TdxSpreadSheetCellStyleHandle, TdxSpreadSheetCellStyleHandleList>;
      FStyles: TdxSpreadSheetCellStyleHandleList;
    public
      constructor Create;
      destructor Destroy; override;
  ...
  strict private
    ...
    procedure EnumColumnStyles(AArea: TRect; AProc: TdxSpreadSheetCellStyleEnumProcRef);
    procedure EnumDefaultCellsStyles(const R: TRect; AProc: TdxSpreadSheetCellStyleEnumProcRef);
    procedure EnumRowStyles(AArea: TRect; AProc: TdxSpreadSheetCellStyleEnumProcRef);
  public
    constructor Create(ASheet: TdxSpreadSheetTableView); virtual;
    destructor Destroy; override;
    procedure PrepareToSave(AArea: TRect);
    procedure ProcessArea(AArea: TRect; AProc: TdxSpreadSheetCellStyleEnumProcRef);
    //
  ...
  public
    class function BuildReference(AView: TdxSpreadSheetCustomView; const AArea: TRect): string;
    ...
    class function GetHeaders(AView: TdxSpreadSheetTableView; const AArea: TRect): TcxBorders; overload;
    class function GetHeaders(AView: TdxSpreadSheetTableView; const AAreas: TdxRectList): TcxBorders; overload;
    ...
    class function ReplaceIllegalChars(const ACaption: string; ASpreadSheet: TdxCustomSpreadSheet): string;
  end;
...
function dxSpreadSheetGetNeighbor(const ARow, AColumn: Integer;
  ASide: TcxBorder; out ANeighborRow, ANeighborColumn: Integer): Boolean; inline;
procedure dxSpreadSheetMergeBorderStyle(ASide1: TcxBorder; AStyle1: TdxSpreadSheetCellStyleHandle;
  AStyle2: TdxSpreadSheetCellStyleHandle; var AColor: TColor; var AStyle: TdxSpreadSheetCellBorderStyle); overload;
procedure dxSpreadSheetMergeBorderStyle(ASide1: TcxBorder; AStyle1: TdxSpreadSheetBordersHandle;
...
uses
  ...
  dxSpreadSheetConditionalFormatting, dxSpreadSheetCoreStrs, dxSpreadSheetPrinting, dxStringHelper, dxMessageDialog;

const
  dxThisUnitName = 'dxSpreadSheetCoreHelpers';
...
begin
  ...
  Result :=
    InRange(ANeighborRow, 0, dxSpreadSheetMaxRowIndex) and
    InRange(ANeighborColumn, 0, dxSpreadSheetMaxColumnIndex);
end;

procedure dxSpreadSheetMergeBorderStyle(ASide1: TcxBorder; AStyle1: TdxSpreadSheetCellStyleHandle;
  AStyle2: TdxSpreadSheetCellStyleHandle; var AColor: TColor; var AStyle: TdxSpreadSheetCellBorderStyle);
begin
  dxSpreadSheetMergeBorderStyle(ASide1, AStyle1.Borders, AStyle2.Borders, AColor, AStyle);
...
var
  ...
  ASide2: TcxBorder;
begin
  ASide2 := NeighborBorderSide[ASide1];
  ABorderStyle1 := AStyle1.BorderStyle[ASide1];
  ABorderStyle2 := AStyle2.BorderStyle[ASide2];

  ADelta :=
    IfThen(not AStyle1.BorderIsAuto[ASide1], dxSpreadSheetBorderStyleWeights[ABorderStyle1]) -
    IfThen(not AStyle2.BorderIsAuto[ASide2], dxSpreadSheetBorderStyleWeights[ABorderStyle2]);

  if (ADelta < 0) or (ADelta = 0) and (AStyle1.BorderColor[ASide1] = clDefault) then
  begin
    AColor := AStyle2.BorderColor[ASide2];
    AStyle := ABorderStyle2;
  end
  else
  begin
    AColor := AStyle1.BorderColor[ASide1];
...
begin
  BeginUpdate;
  try
    AView := TdxSpreadSheetTableViewAccess(View);
    Merge(AColumn.Style, ARow.Style);
    for ASide := Low(TcxBorder) to High(TcxBorder) do
    begin
      if dxSpreadSheetGetNeighbor(ARow.Index, AColumn.Index, ASide, ANeighborRow, ANeighborCol) then
      begin
        dxSpreadSheetMergeBorderStyle(ASide, Handle, AView.GetCellStyleHandle(ANeighborRow, ANeighborCol), ABorderColor, ABorderStyle);
        Borders[ASide].Assign(ABorderStyle, ABorderColor);
      end;
    end;
  finally
    EndUpdate;
...
class procedure TdxSpreadSheetCellStyleHelper.Calculate(ACell: TdxSpreadSheetCell);
var
  ABordersHandle: TdxSpreadSheetBordersHandle;
  ACellStyles: TdxSpreadSheetCellStyles;
  AColumnStyle: TdxSpreadSheetCellStyleHandle;
  ARowStyle: TdxSpreadSheetCellStyleHandle;
  AStyleHandle: TdxSpreadSheetCellStyleHandle;
  AView: TdxSpreadSheetTableView;
begin
  AView := ACell.View;
  ACellStyles := AView.CellStyles;
  AColumnStyle := ACell.Column.Style.Handle;
  ARowStyle := ACell.Row.Style.Handle;

  AStyleHandle := ACell.StyleHandle.Clone;
  try
    if ARowStyle.IsDefault then
      AStyleHandle.Assign(AColumnStyle)
    else
      AStyleHandle.Assign(ARowStyle);

    ABordersHandle := AStyleHandle.Borders.Clone;
    ABordersHandle.Assign(AColumnStyle.Borders, bLeft);
    ABordersHandle.Assign(AColumnStyle.Borders, bRight);
    ABordersHandle.Assign(ARowStyle.Borders, bBottom);
    ABordersHandle.Assign(ARowStyle.Borders, bTop);
    MergeBorders(ABordersHandle,
      AView.Cells[ACell.RowIndex, ACell.ColumnIndex - 1],
      AView.Cells[ACell.RowIndex - 1, ACell.ColumnIndex],
      AView.Cells[ACell.RowIndex, ACell.ColumnIndex + 1],
      AView.Cells[ACell.RowIndex + 1, ACell.ColumnIndex],
      False);

    AStyleHandle.Borders := ACellStyles.Borders.AddBorders(ABordersHandle);
  finally
    ACell.StyleHandle := ACellStyles.AddStyle(AStyleHandle);
  end;
end;

class procedure TdxSpreadSheetCellStyleHelper.MergeBorders(
  ACurrRowCell, APrevRowCell, ANextRowCell: TdxSpreadSheetCell; ACellStyles: TdxSpreadSheetCellStyles);
var
  ABottomCell: TdxSpreadSheetCell;
  ACurrCell: TdxSpreadSheetCell;
  ALeftCell: TdxSpreadSheetCell;
  ANextCell: TdxSpreadSheetCell;
  APrevCell: TdxSpreadSheetCell;
  ARightCell: TdxSpreadSheetCell;
  ATopCell: TdxSpreadSheetCell;
begin
  APrevCell := nil;
  ACurrCell := ACurrRowCell;
  while ACurrCell <> nil do
  begin
    ANextCell := ACurrCell.Next;
    while (ANextRowCell <> nil) and (ANextRowCell.Index < ACurrCell.Index) do
      ANextRowCell := ANextRowCell.Next;
    while (APrevRowCell <> nil) and (APrevRowCell.Index < ACurrCell.Index) do
      APrevRowCell := APrevRowCell.Next;

    if (APrevCell <> nil) and (APrevCell.Index + 1 = ACurrCell.Index) then
      ALeftCell := APrevCell
    else
      ALeftCell := nil;

    if (ANextCell <> nil) and (ANextCell.Index - 1 = ACurrCell.Index) then
      ARightCell := ANextCell
    else
      ARightCell := nil;

    if (ANextRowCell <> nil) and (ANextRowCell.Index = ACurrCell.Index) then
      ABottomCell := ANextRowCell
    else
      ABottomCell := nil;

    if (APrevRowCell <> nil) and (APrevRowCell.Index = ACurrCell.Index) then
      ATopCell := APrevRowCell
    else
      ATopCell := nil;

    MergeBorders(ACurrCell, ALeftCell, ATopCell, ARightCell, ABottomCell, ACellStyles, True);
...
class procedure TdxSpreadSheetCellStyleHelper.MergeBorders(ACell, ALeftCell, ATopCell, ARightCell,
  ABottomCell: TdxSpreadSheetCell; ACellStyles: TdxSpreadSheetCellStyles; ATakeWeightIntoAccount: Boolean);
var
  ABorderHandle: TdxSpreadSheetBordersHandle;
  AStyleHandle: TdxSpreadSheetCellStyleHandle;
begin
  if ACell <> nil then
  begin
    ABorderHandle := ACell.StyleHandle.Borders;
    try
      MergeBorders(ABorderHandle, ALeftCell, ATopCell, ARightCell, ABottomCell, ATakeWeightIntoAccount);
    finally
      if ACell.StyleHandle.Borders <> ABorderHandle then
      begin
        AStyleHandle := ACell.StyleHandle.Clone;
        try
          AStyleHandle.Borders := ACellStyles.Borders.AddBorders(ABorderHandle);
        finally
          ACell.StyleHandle := ACellStyles.AddStyle(AStyleHandle);
        end;
      end;
...
class procedure TdxSpreadSheetCellStyleHelper.MergeBorders(var ACellBorders: TdxSpreadSheetBordersHandle;
  ALeftCell, ATopCell, ARightCell, ABottomCell: TdxSpreadSheetCell; ATakeWeightIntoAccount: Boolean);

  procedure MergeBorder(var ATarget: TdxSpreadSheetBordersHandle; ASource: TdxSpreadSheetBordersHandle; ABorder: TcxBorder);
  const
    OppositeBordersMap: array[TcxBorder] of TcxBorder = (bRight, bBottom, bLeft, bTop);
  var
    AOppositeBorder: TcxBorder;
  begin
    AOppositeBorder := OppositeBordersMap[ABorder];

    if ATakeWeightIntoAccount then
    begin
      if dxSpreadSheetBorderStyleWeights[ASource.BorderStyle[AOppositeBorder]] <
         dxSpreadSheetBorderStyleWeights[ATarget.BorderStyle[ABorder]]
      then
        Exit;
    end;

    if not ATarget.IsEqual(ASource, ABorder, AOppositeBorder) then
    begin
      if ATarget.RefCount > 0 then // is it original handle?
        ATarget := ATarget.Clone;
      ATarget.BorderColor[ABorder] := ASource.BorderColor[AOppositeBorder];
      ATarget.BorderStyle[ABorder] := ASource.BorderStyle[AOppositeBorder];
      ATarget.BorderIsAuto[ABorder] := True;
    end;
  end;

begin
  if ATopCell <> nil then
    MergeBorder(ACellBorders, ATopCell.StyleHandle.Borders, bTop);
  if ALeftCell <> nil then
    MergeBorder(ACellBorders, ALeftCell.StyleHandle.Borders, bLeft);
  if ARightCell <> nil then
    MergeBorder(ACellBorders, ARightCell.StyleHandle.Borders, bRight);
  if ABottomCell <> nil then
    MergeBorder(ACellBorders, ABottomCell.StyleHandle.Borders, bBottom);
end;

class procedure TdxSpreadSheetCellStyleHelper.MergeBorders(AView: TdxSpreadSheetTableView);
var
  ACellStyles: TdxSpreadSheetCellStyles;
  ACurrRow: TdxSpreadSheetTableItem;
  ACurrRowCell: TdxSpreadSheetCell;
  ANextRow: TdxSpreadSheetTableItem;
  ANextRowCell: TdxSpreadSheetCell;
  APrevRow: TdxSpreadSheetTableItem;
  APrevRowCell: TdxSpreadSheetCell;
begin
  AView.BeginUpdate;
  try
    APrevRow := nil;
    ACurrRow := AView.Rows.First;
    ACellStyles := AView.CellStyles;
    while ACurrRow <> nil do
    begin
      ANextRow := ACurrRow.Next;

      if ACurrRow <> nil then
        ACurrRowCell := TdxSpreadSheetTableRow(ACurrRow).FirstCell
      else
        ACurrRowCell := nil;

      if (APrevRow <> nil) and (APrevRow.Index + 1 = ACurrRow.Index) then
        APrevRowCell := TdxSpreadSheetTableRow(APrevRow).FirstCell
      else
        APrevRowCell := nil;

      if (ANextRow <> nil) and (ANextRow.Index - 1 = ACurrRow.Index) then
        ANextRowCell := TdxSpreadSheetTableRow(ANextRow).FirstCell
      else
        ANextRowCell := nil;

      if ACurrRowCell <> nil then
        MergeBorders(ACurrRowCell, APrevRowCell, ANextRowCell, ACellStyles);
...
begin
  ...
  try
    ...
    TdxSpreadSheetTableRowsAccess(View.Rows).ForEachCell(SourceArea,
      procedure (ACell: TdxSpreadSheetCell)
      begin
        TdxSpreadSheetCellAccess(ACell).Move(
          ACell.RowIndex - SourceArea.Top + TargetArea.Top,
          ACell.Index - SourceArea.Left + TargetArea.Left);
      end,
      TargetArea.Top <= SourceArea.Top, TargetArea.Left <= SourceArea.Left);
...
procedure TdxSpreadSheetTableViewCellsModificationHelper.ProcessCore(const AArea: TRect;
  ...
  if HasAffectedMergedCells then
    case MergedCellsConflictResolution of
      mccrShowConfirmation:
        if dxMessageDlg(cxGetResourceString(@sdxUnmergeCellsConfirmation), mtConfirmation, [mbOK, mbCancel], 0) = mrOk then
...
begin
  ...
  TdxSpreadSheetTableRowsAccess(Rows).ForEach(
    ...
    var
      ACells: TdxSpreadSheetTableRowCells;
    begin
      ACells := TdxSpreadSheetTableRowAccess(AItem).FCells;
      if ACells <> nil then
        ShiftIndexes(ACells, AStartFromIndex, ADelta);
    end,
...
begin
  ...
  TdxSpreadSheetTableRowsAccess(Rows).ForEach(
    ...
    var
      ...
      ANextCell: TdxSpreadSheetCell;
      ARow: TdxSpreadSheetTableRow absolute AItem;
...
procedure TdxSpreadSheetTableViewInsertCellsHelper.CalculateStyle(
  ANewStyle, ALeftSourceStyle, ARightSourceStyle: TdxSpreadSheetCellStyle; AIsFirst, AIsLast: Boolean);

  function HasBorder(AStyle: TdxSpreadSheetCellStyle; ABorder: TcxBorder): Boolean;
  begin
    Result := (AStyle <> nil) and (AStyle.Borders[ABorder].Style <> sscbsDefault);
  end;

var
  AHasLeftBorder: Boolean;
  AHasRightBorder: Boolean;
  ALeftSide: TcxBorder;
...
begin
  ...
  try
    ...
    AHasLeftBorder := HasBorder(ALeftSourceStyle, ARightSide);
    AHasRightBorder := HasBorder(ARightSourceStyle, ALeftSide);

    if not (AHasLeftBorder and AHasRightBorder) then
      ANewStyle.Handle.Borders := SpreadSheet.CellStyles.DefaultStyle.Borders;
    if AHasLeftBorder and AIsFirst then
      ANewStyle.Borders[ALeftSide].Assign(ALeftSourceStyle.Borders[ARightSide]);
    if AHasRightBorder and AIsLast then
...
procedure TdxSpreadSheetTableViewInsertCellsHelper.CalculateStyleForNewCells(
  ATarget, ALeftSource, ARightSource: TdxSpreadSheetTableItem; AOppositeItems: TdxSpreadSheetTableItems;
  AStartIndex, AFinishIndex: Integer; AIsFirst, AIsLast: Boolean);
var
...
begin
  ...
  begin
    ...
    if (ALeftSourceStyle <> nil) or (ARightSourceStyle <> nil) then
      CalculateStyle(ATarget.CreateCell(I).Style, ALeftSourceStyle, ARightSourceStyle, AIsFirst, AIsLast);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      AIsFirst := I = AArea.Left;
      AIsLast := I = AArea.Right;
      if Modification in [cmShiftColumns, cmShiftRows] then
      begin
        CalculateStyle(AItem.Style, GetStyle(ALeftSourceItem), GetStyle(ARightSourceItem), AIsFirst, AIsLast);
        if (ALeftSourceItem <> nil) and not ALeftSourceItem.DefaultSize then
          AItem.Size := ALeftSourceItem.CustomSize;
      end;
      CalculateStyleForNewCells(AItem, ALeftSourceItem,
        ARightSourceItem, AOppositeItems, AArea.Top, AArea.Bottom, AIsFirst, AIsLast);
    end;
  end;
end;
...
constructor TdxSpreadSheetTableViewMergedCellStyleHelper.Create(
  AMergeCell: TdxSpreadSheetMergedCell; AUseActiveCellAsPattern: Boolean);
begin
  inherited Create(AMergeCell.Owner.View);
  FMergeCell := AMergeCell;
  FUseActiveCellAsPattern := AUseActiveCellAsPattern;
end;

class procedure TdxSpreadSheetTableViewMergedCellStyleHelper.Calculate(
  AMergeCell: TdxSpreadSheetMergedCell; AUseActiveCellAsPattern: Boolean = False);
begin
  with Create(AMergeCell, AUseActiveCellAsPattern) do
  try
    DoCalculate;
  finally
    Free;
...
procedure TdxSpreadSheetTableViewMergedCellStyleHelper.CalculateCellValues;
begin
  FNonBlankCell := nil;
...
procedure TdxSpreadSheetTableViewMergedCellStyleHelper.CalculateBorder(ABorder: TcxBorder; AItem: TdxSpreadSheetTableItem;
  AStartIndex, AFinishIndex: Integer; out ABorderColor: TColor; out ABorderStyle: TdxSpreadSheetCellBorderStyle);
...
procedure TdxSpreadSheetTableViewMergedCellStyleHelper.CalculateCellStyles;
var
  AArea: TRect;
  ABorder: TcxBorder;
  AHasStyle: Boolean;
  AStylePattern: TdxSpreadSheetCellStyle;
  AStylePatternArea: TRect;
begin
  ...
  if FUseActiveCellAsPattern then
    AStylePatternArea := cxRectBounds(AArea.TopLeft, 0, 0)
  else
    AStylePatternArea := AArea;

  CalculateBorder(bBottom, Rows[AStylePatternArea.Bottom], AStylePatternArea.Left,
    AStylePatternArea.Right, FBorderColors[bBottom], FBorderStyles[bBottom]);
  CalculateBorder(bLeft, Columns[AStylePatternArea.Left], AStylePatternArea.Top,
    AStylePatternArea.Bottom, FBorderColors[bLeft], FBorderStyles[bLeft]);
  CalculateBorder(bRight, Columns[AStylePatternArea.Right], AStylePatternArea.Top,
    AStylePatternArea.Bottom, FBorderColors[bRight], FBorderStyles[bRight]);
  CalculateBorder(bTop, Rows[AStylePatternArea.Top], AStylePatternArea.Left,
    AStylePatternArea.Right, FBorderColors[bTop], FBorderStyles[bTop]);
...
procedure TdxSpreadSheetTableViewMergedCellStyleHelper.CalculateCellStylesInCustomArea(
...
begin
  ...
  begin
    ...
    begin
      ...
      ACellStyle := ACell.Style;

      ACellStyle.BeginUpdate;
      try
        ACellStyle.Assign(AStylePattern);

        if AIsTopRow then
        begin
          ACellStyle.Borders[bTop].Color := FBorderColors[bTop];
          ACellStyle.Borders[bTop].Style := FBorderStyles[bTop];
        end
        else
          ACellStyle.Borders[bTop].Reset;

        if AIsBottomRow then
        begin
          ACellStyle.Borders[bBottom].Color := FBorderColors[bBottom];
          ACellStyle.Borders[bBottom].Style := FBorderStyles[bBottom];
        end
        else
          ACellStyle.Borders[bBottom].Reset;

        if AColumnIndex = AArea.Left then
        begin
          ACellStyle.Borders[bLeft].Color := FBorderColors[bLeft];
          ACellStyle.Borders[bLeft].Style := FBorderStyles[bLeft];
        end
        else
          ACellStyle.Borders[bLeft].Reset;

        if AColumnIndex = AArea.Right then
        begin
          ACellStyle.Borders[bRight].Color := FBorderColors[bRight];
          ACellStyle.Borders[bRight].Style := FBorderStyles[bRight];
        end
        else
          ACellStyle.Borders[bRight].Reset;
      finally
...
procedure TdxSpreadSheetTableViewMergedCellStyleHelper.CalculateCellStylesInEntireColumnOrRow(
...
procedure TdxSpreadSheetTableViewMergedCellStyleHelper.DoCalculate;
begin
  CalculateCellValues;
  CalculateCellStyles;
...
procedure TdxSpreadSheetTableViewMergedCellStyleHelper.SetupActiveCellValue;
var
...
function TdxSpreadSheetTableViewMergedCellStyleHelper.GetActiveCell: TdxSpreadSheetCell;
begin
  Result := MergeCell.ActiveCell;
end;

function TdxSpreadSheetTableViewMergedCellStyleHelper.GetDefaultStyle: TdxSpreadSheetCellStyle;
begin
  Result := SpreadSheet.DefaultCellStyle;
end;

function TdxSpreadSheetTableViewMergedCellStyleHelper.GetStylePattern: TdxSpreadSheetCellStyle;
begin
...
type
  TSelection = (sCells, sColumns, sRows, sAll);
const
  SelectionMap: array[Boolean, Boolean] of TSelection = ((sCells, sRows), (sColumns, sAll));
var
  AColumnIndex: Integer;
...
begin
  ...
  case SelectionMap[dxSpreadSheetIsEntireColumn(AArea), dxSpreadSheetIsEntireRow(AArea)] of
    sColumns:
      for AColumnIndex := AArea.Left to AArea.Right do
...
procedure TdxSpreadSheetTableViewEnumCellStylesHelper.ProcessArea(AArea: TRect; AProc: TdxSpreadSheetCellStyleEnumProcRef);
begin
  FCache.Clear;
  try
    AArea := dxSpreadSheetGetRealArea(AArea);

    if dxSpreadSheetIsEntireArea(AArea) then
    begin
      Inc(TdxSpreadSheetTableViewAccess(Sheet).FMergeCellStylesLockCount);
      try
        AProc(Sheet.SpreadSheet.DefaultCellStyle, -1, -1, AArea);
        EnumColumnStyles(AArea, AProc);
        EnumRowStyles(AArea, AProc);
      finally
    ...
    Sheet.ForEachCell(AArea,
      procedure (ACell: TdxSpreadSheetCell)
      begin
        AProc(ACell.Style, ACell.RowIndex, ACell.ColumnIndex, AArea);
      end);

    if EnumDefaultStyles then
      EnumDefaultCellsStyles(AArea, AProc);
  finally
    FCache.Clear;
  end;
end;
...
procedure TdxSpreadSheetTableViewEnumCellStylesHelper.EnumColumnStyles(
  AArea: TRect; AProc: TdxSpreadSheetCellStyleEnumProcRef);
var
  AItem: TdxSpreadSheetTableItem;
begin
  AItem := Sheet.Columns.First;
  while (AItem <> nil) and (AItem.Index < AArea.Left) do
    AItem := AItem.Next;
  while (AItem <> nil) and (AItem.Index <= AArea.Right) do
  begin
    AProc(AItem.Style, -1, AItem.Index, AArea);
    AItem := AItem.Next;
  end;
end;

procedure TdxSpreadSheetTableViewEnumCellStylesHelper.EnumDefaultCellsStyles(
  const R: TRect; AProc: TdxSpreadSheetCellStyleEnumProcRef);
var
  ACellCount: Integer;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        if ARow.Cells[AColumn.Index] <> nil then
          ...
          begin
            ...
            begin
              AStyle.Calculate(TdxSpreadSheetTableRow(ARow), AColumn);
              AProc(AStyle, ARow.Index, AColumn.Index, R);
            end;
            Inc(ACellCount);
          end;
      ...
      if ACellCount <> dxSpreadSheetAreaWidth(R) then
      begin
        if FCache.Add(ARow.Style.Handle) then
          AProc(ARow.Style, ARow.Index, -1, R);
      end;
      ARow := ARow.Next;
      Inc(ARowCount);
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        AProc(AColumn.Style, -1, AColumn.Index, R);
        Inc(ACellCount);
      end;
      AColumn := TdxSpreadSheetTableColumnAccess(AColumn.Next);
    end;
    if ACellCount <> dxSpreadSheetAreaWidth(R) then
      AProc(Sheet.SpreadSheet.DefaultCellStyle, -1, -1, R);
  end;
end;
...
begin
  ACount := 0;
  AItem := Sheet.Rows.First;
  while (AItem <> nil) and (AItem.Index < AArea.Top) do
    AItem := AItem.Next;
  while (AItem <> nil) and (AItem.Index <= AArea.Bottom) do
  begin
    AProc(AItem.Style, AItem.Index, -1, AArea);
    FHasEntireRowWithDefaultStyle := FHasEntireRowWithDefaultStyle or AItem.Style.IsDefault;
    AItem := AItem.Next;
    Inc(ACount);
  end;
  FHasEntireRowWithDefaultStyle := FHasEntireRowWithDefaultStyle or (ACount <> dxSpreadSheetAreaHeight(AArea));
end;
...
begin
  inherited Create;
  FStyles := TdxSpreadSheetCellStyleHandleList.Create;
  FCrossStyles := TObjectDictionary<TdxSpreadSheetCellStyleHandle, TdxSpreadSheetCellStyleHandleList>.Create([doOwnsValues]);
end;
...
var
  AList: TdxSpreadSheetCellStyleHandleList;
begin
  ...
  begin
    AList := TdxSpreadSheetCellStyleHandleList.Create;
    AList.Add(AColumnStyle);
...
var
  ACell: TdxSpreadSheetCell;
  AColumnStyle: TdxSpreadSheetCellStyleHandle;
  AContainers: TdxSpreadSheetContainers;
  ANextCell: TdxSpreadSheetCell;
  ARow: TdxSpreadSheetTableItem;
  ARowStyle: TdxSpreadSheetCellStyleHandle;
begin
  AContainers := View.Containers;
  ARow := View.Rows.First;
  while ARow <> nil do
  begin
    ARowStyle := ARow.Style.Handle;
    ACell := TdxSpreadSheetTableRowAccess(ARow).FirstCell;
    while ACell <> nil do
    begin
      ANextCell := ACell.Next;
      ...
      begin
        AColumnStyle := ACell.Column.Style.Handle;
        if (ACell.StyleHandle = AColumnStyle) and ARowStyle.IsDefault or
           (ACell.StyleHandle = ARowStyle) and (not ARowStyle.IsDefault or AColumnStyle.IsDefault)
        then
          if not (ACell.IsMerged or AContainers.IsCellUsed(ACell)) then
          begin
...
var
  AItem: TdxSpreadSheetTableItem;
  ANextItem: TdxSpreadSheetTableItem;
begin
  AItem := TdxSpreadSheetTableItem(AItems.Last);
  while AItem <> nil do
  begin
    ANextItem := TdxSpreadSheetTableItem(AItem.Prev);
    if AItem.DefaultSize and AItem.Visible and AItem.Style.IsDefault then
    begin
      if not TdxSpreadSheetTableItemAccess(AItem).HasCells then
        AItem.Free;
    end;
    AItem := ANextItem;
  end;
...
procedure TdxSpreadSheetPasteCellDataFromStreamHelper.RestoreCells(AView: TdxSpreadSheetTableView;
  AStyles: TdxSpreadSheetCellStyleHandleList; const AArea: TRect; AOptions: TdxSpreadSheetClipboardPasteOptions);
var
...
class function TdxSpreadSheetDefinedNameHelper.BuildReference(AView: TdxSpreadSheetCustomView; const AArea: TRect): string;
begin
  Result := dxReferenceToString(AArea, False, 
    [croAbsoluteColumn, croAbsoluteRow, croSheetName], AView.Caption);
end;
...
class function TdxSpreadSheetDefinedNameHelper.GetHeaders(AView: TdxSpreadSheetTableView; const AArea: TRect): TcxBorders;

  function Check(ADefinedNames: TdxSpreadSheetDefinedNamesAccess; ARow, AColumn: Integer): Boolean;
  var
    ACell: TdxSpreadSheetCell;
  begin
    ACell := AView.Cells[ARow, AColumn];
    Result := (ACell <> nil) and (ACell.DataType = cdtString) and (
      ADefinedNames.IsCaptionValid(ACell.AsString) or
      ADefinedNames.IsCaptionValid(ACell.AsString + ADefinedNames.AutoNameSuffix) or
      ADefinedNames.IsCaptionValid(ReplaceIllegalChars(ACell.AsString, ADefinedNames.SpreadSheet)));
  end;

var
  ADefinedNames: TdxSpreadSheetDefinedNamesAccess;
  AAreaHeight: Integer;
  AAreaWidth: Integer;
begin
  Result := [];
  ADefinedNames := TdxSpreadSheetDefinedNamesAccess(AView.SpreadSheet.DefinedNames);
  AAreaWidth := dxSpreadSheetAreaWidth(AArea);
  AAreaHeight := dxSpreadSheetAreaHeight(AArea);

  if (AAreaWidth <= 1) and (AAreaHeight <= 1) then
    Exit;

  if AAreaHeight > 1 then
  begin
    if Check(ADefinedNames, AArea.Top, AArea.Left + Ord(AAreaWidth > 1)) then
      Include(Result, bTop)
    else
      if Check(ADefinedNames, AArea.Bottom, AArea.Left + Ord(AAreaWidth > 1)) then
        Include(Result, bBottom);
  end;

  if AAreaWidth > 1 then
  begin
    if Check(ADefinedNames, AArea.Top + Ord(AAreaHeight > 1), AArea.Left) then
    begin
      if not (bBottom in Result) or (AArea.Top + 1 <> AArea.Bottom) then
        Include(Result, bLeft);
    end
    else
      if Check(ADefinedNames, AArea.Bottom - Ord(AAreaHeight > 1), AArea.Right) then
      begin
        if not (bTop in Result) or (AArea.Top <> AArea.Bottom - 1) then
          Include(Result, bRight);
      end;
  end;
end;

class function TdxSpreadSheetDefinedNameHelper.GetHeaders(AView: TdxSpreadSheetTableView; const AAreas: TdxRectList): TcxBorders;
var
  I: Integer;
begin
  ...
  Result := GetHeaders(AView, AAreas[0]);
  for I := 1 to AAreas.Count - 1 do
  begin
    Result := Result * GetHeaders(AView, AAreas[I]);
    if Result = [] then
      Break;
  end;
end;
...
class function TdxSpreadSheetDefinedNameHelper.ReplaceIllegalChars(const ACaption: string; ASpreadSheet: TdxCustomSpreadSheet): string;
var
  ABreakChars: string;
...
begin
  ...
  if ALength > 0 then
  begin
    ABreakChars := TdxSpreadSheetAccess(ASpreadSheet).FormatSettings.BreakChars;
    ABuffer := TdxStringBuilderManager.Get(ALength);
    try
      for I := 1 to ALength do
      begin
        ASymbol := ACaption[I];
        if Pos(ASymbol, ABreakChars) > 0 then
          ABuffer.Append('_')
        else
          ABuffer.Append(ASymbol);
      end;
