...
type
  ...
  public
    ...
    class procedure Insert(const AFormula: string; AEdit: TcxCustomEdit; AAddParenthesis: Boolean = True);
  end;
...
uses
  ...
  cxGeometry, dxHashUtils, cxFormats, dxCore;

const
  dxThisUnitName = 'dxSpreadSheetFormulasHelpers';
...
begin
  if (AChar = dxLeftParenthesis) or (AChar = dxLeftArrayParenthesis) or (AChar = dxFormatSettings.ListSeparator) then
    Exit(True);
  if dxCharInSet(AChar, [#13, #10]) then
    Exit(True);
...
class procedure TdxModifyFormulasHelper.Insert(const AFormula: string; AEdit: TcxCustomEdit; AAddParenthesis: Boolean);
var
...
begin
  ...
  begin
    ...
    if AEdit is TcxCustomRichEdit then
      AText := GetAdjustedRichEditText(TcxCustomRichEdit(AEdit))
    else if AEdit is TcxCustomMemo then
    ...
    begin
      System.Insert(dxDefaultOperations[opEQ], AText, ACursorPosition);
      ACursorPosition := Length(dxDefaultOperations[opEQ]);
    end;
    ...
    System.Insert(AFormula, AText, ACursorPosition + 1);
    Inc(ACursorPosition, Length(AFormula));

    if AAddParenthesis then
    begin
      System.Insert(dxLeftParenthesis + dxRightParenthesis, AText, ACursorPosition + 1);
