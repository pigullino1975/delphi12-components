...
uses
  ...
  dxCustomTree, dxForms, dxSpreadSheetStyles, dxSpreadSheetCoreStyles, dxSpreadSheetCoreFormulas, dxSpreadSheetCoreReferences,
  dxSpreadSheetConditionalFormatting, cxLookAndFeels;
...
type
  ...
  protected
    ...
    ViewInfo: TdxSpreadSheetCellViewInfo;
  ...
  { TdxSpreadSheetReportSections }

  TdxSpreadSheetReportSections = class(TObjectList<TdxSpreadSheetReportSection>)
  public
    procedure Draw(ACanvas: TcxCanvas);
    function Find(const AName: string; out ASection: TdxSpreadSheetReportSection): Boolean;
    function FindByCell(const ARow, AColumn: Integer; out ASection: TdxSpreadSheetReportSection): Boolean;
    function InitHitTest(AHitTest: TdxSpreadSheetCustomHitTest): Boolean;
 ...
 { TdxSpreadSheetReportDetails }
  ...
  protected
    ...
    function CheckSection(AType: TdxSpreadSheetReportSectionType; var ASection: TdxSpreadSheetReportSection): Boolean;
    procedure DoBuildReport; virtual;
    procedure DoBuildDetailLevel(AInfo: TdxSpreadSheetReportLevelBuildInfo); virtual;
    procedure DoBuildDetailSection(AInfo: TdxSpreadSheetReportLevelBuildInfo; var ARowIndex: Integer); virtual;
  ...
  strict private
    ...
    FSections: TdxSpreadSheetReportSections;
  ...
  protected
    ...
    procedure DragOverCore(const P: TPoint; ADragObject: TDragObject; var AAccept: Boolean);
    ...
    property Filter: IcxFilterControl read GetFilter implements IcxFilterControl;
    property Sections: TdxSpreadSheetReportSections read FSections;
  ...
  published
    ...
    property OnPageControlNewTabButtonClick;
    property OnPageControlContextPopup;
    property OnPageControlTabStartDrag;
    property OnPageControlTabDragAndDrop;
    property OnPageControlTabEndDrag;
    ...
    property OnTableViewContextPopup;
  end;

  { TdxSpreadSheetReportTableView }

  TdxSpreadSheetReportTableViewInfo = class;

  TdxSpreadSheetReportTableView = class(TdxSpreadSheetTableView)
  strict private
    function GetViewInfo: TdxSpreadSheetReportTableViewInfo; inline;
  protected
  ...
  strict private
    ...
    function GetSections: TdxSpreadSheetReportSections;
    function GetView: TdxSpreadSheetReportTableView;
  protected
    FDrawBaseSelection: Boolean;
    FDragDropAreaCell: TdxSpreadSheetReportDragDropFieldAreaCell;

    function CreateCellViewInfo(ACell: TdxSpreadSheetCell): TdxSpreadSheetTableViewCellViewInfo; override;
    function CreateMergedCellViewInfo(AMergedCell: TdxSpreadSheetMergedCell): TdxSpreadSheetTableViewMergedCellViewInfo; override;
    procedure CalculateCore; override;
    procedure CalculateHitTest(AHitTest: TdxSpreadSheetCustomHitTest); override;
    procedure CalculateSection(ASection: TdxSpreadSheetReportSection); virtual;
    procedure DrawSelection(ACanvas: TcxCanvas); override;
    procedure RightToLeftConversion(const AClientBounds: TRect); override;
    procedure UpdateDragDropAreaCell(ARowIndex, AColumnIndex: Integer; AAccepted: Boolean); virtual;

    property Designer: TdxSpreadSheetReportDesigner read GetDesigner;
    property HitTest: TdxSpreadSheetTableViewHitTest read GetHitTest;
    property Sections: TdxSpreadSheetReportSections read GetSections;
  ...
  { TdxSpreadSheetReportCellStyleViewInfo }

  TdxSpreadSheetReportCellStyleViewInfo = class(TdxSpreadSheetConditionalFormattingStyleViewInfo)
  strict private
    FDataCell: TdxSpreadSheetTableViewCustomDataCellViewInfo;
    FDatabaseGlyphBounds: TRect;

    function GetDatabaseGlyph: TdxSmartImage;
    function GetDesigner: TdxSpreadSheetReportDesigner;
  protected
    function GetSupportedDrawingStages: TdxSpreadSheetDrawingStages; override;
  public
    constructor Create(ADataCell: TdxSpreadSheetTableViewCustomDataCellViewInfo); reintroduce; virtual;
    procedure Calculate(const ABounds, AContentBounds: TRect); override;
    procedure Draw(ACanvas: TcxCanvas; AStage: TdxSpreadSheetDrawingStage); override;
    procedure Offset(const AOffset: TPoint); override;
    //
    property DatabaseGlyph: TdxSmartImage read GetDatabaseGlyph;
    property Designer: TdxSpreadSheetReportDesigner read GetDesigner;
  end;
  ...
  protected
    function CreateStyleViewInfo: TdxSpreadSheetConditionalFormattingStyleViewInfo; override;
  end;

  { TdxSpreadSheetReportMergedCellViewInfo }

  TdxSpreadSheetReportMergedCellViewInfo = class(TdxSpreadSheetTableViewMergedCellViewInfo)
  protected
    function CreateStyleViewInfo: TdxSpreadSheetConditionalFormattingStyleViewInfo; override;
  end;
...
resourcestring
  ...
  sdxEmptySheet = 'Empty Sheet';
  sdxNoDataInfoText = '< NO DATA TO DISPLAY >';
...
{$R dxSpreadSheetReportDesigner.res}
{$R dxSpreadSheetReportDesigner_svg.res}

uses
  dxSpreadSheetPopupMenu, dxSpreadSheetCoreFormulasTokens, dxDPIAwareUtils, dxSpreadSheetCoreHelpers, StrUtils;

const
  dxThisUnitName = 'dxSpreadSheetReportDesigner';
...
type
  ...
  TdxSpreadSheetCellViewInfoAccess = class(TdxSpreadSheetCellViewInfo);
  ...
  TdxSpreadSheetReportSizingArea = (saNone, saLeft, saTop, saRight, saBottom, saBottomRight, saBottomLeft);
  ...
  { TdxSpreadSheetReportSectionsNameController }

  TdxSpreadSheetReportSectionsNameController = class
  strict private
    class var FNames: TDictionary<string, TdxSpreadSheetReportSectionType>;
  public
    class function IsSectionName(AName: TdxSpreadSheetDefinedName;
      var AType: TdxSpreadSheetReportSectionType; var AIndex: Integer): Boolean;
    class function TryGetType(const AName: string;
      var AType: TdxSpreadSheetReportSectionType; var AIndex: Integer): Boolean;
  ...
  protected
    FSizingArea: TdxSpreadSheetReportSizingArea;
    FSourceArea: TRect;
    ...
    property Designer: TdxSpreadSheetReportDesigner read GetDesigner;
    property Section: TdxSpreadSheetReportSection read FSection;
    property ViewInfo: TdxSpreadSheetReportTableViewInfo read GetViewInfo;
  public
  ...
  public
    constructor Create(ASection: TdxSpreadSheetReportSection; AView: TdxSpreadSheetCustomView); reintroduce;
    procedure Draw(ACanvas: TcxCanvas; AStage: TdxSpreadSheetDrawingStage); override;
  ...
  protected
    ...
    procedure DoChange;
  public
    constructor Create(ASection: TdxSpreadSheetReportSection; const AName: string; const ABounds: TRect; ALevel: Integer = -1);
...
var
  FDatabaseGlyph: TdxSmartGlyph;
  FFieldChooserBounds: TDictionary<TdxSpreadSheetReportDesigner, TRect>;
...
begin
  ...
  AToken.EnumReferences(
    procedure (const AArea: TRect; AView: TObject; AToken: TdxSpreadSheetFormulaToken)
    begin
      ARange := AArea;
    end);
  Result := ARange;
...
function GetDatabaseGlyph: TdxSmartGlyph;
const
  dxReportDataBase = 'DXREPORTDATABASE';
begin
  if FDatabaseGlyph = nil then
  begin
    FDatabaseGlyph := TdxSmartGlyph.Create;
    if dxUseVectorIcons then
    begin
      FDatabaseGlyph.LoadFromResource(HInstance, dxReportDataBase, 'SVG');
      FDatabaseGlyph.SourceDPI := MulDiv(dxDefaultDPI, FDatabaseGlyph.Width, 10);
    end
    else
    begin
      FDatabaseGlyph.LoadFromResource(HInstance, dxReportDataBase, RT_RCDATA);
      FDatabaseGlyph.Resize(10, 10);
    end;
  end;
  Result := FDatabaseGlyph;
end;
  ...
  begin
    ...
    begin
      ...
      if AToken is TdxSpreadSheetFormulaReferenceToken then
        TdxSpreadSheetFormulaReferenceTokenAccess(AToken).ExtractReference(ASheet, APictureInfo.Range);
...
class procedure TdxSpreadSheetReportSectionsNameController.Register;
begin
  FNames := TDictionary<string, TdxSpreadSheetReportSectionType>.Create;
  FNames.Add(sanHeaderRange, rstHeader);
  FNames.Add(sanFooterRange, rstFooter);
  FNames.Add(sanDetailRange, rstDetail);
  FNames.Add(sanDetailLevelRange, rstDetailLevel);
  FNames.Add(sanGroupHeaderRange, rstGroupHeader);
  FNames.Add(sanGroupFooterRange, rstGroupFooter);
end;

class procedure TdxSpreadSheetReportSectionsNameController.Unregister;
begin
  FreeAndNil(FNames);
end;

class function TdxSpreadSheetReportSectionsNameController.IsSectionName(
  AName: TdxSpreadSheetDefinedName; var AType: TdxSpreadSheetReportSectionType; var AIndex: Integer): Boolean;
begin
  Result := TryGetType(AName.Caption, AType, AIndex);
end;

class function TdxSpreadSheetReportSectionsNameController.TryGetType(
  const AName: string; var AType: TdxSpreadSheetReportSectionType; var AIndex: Integer): Boolean;
var
  ACandidate: string;
  I: Integer;
begin
  ...
  Result := FNames.TryGetValue(UpperCase(ACandidate), AType);
end;
...
begin
  ...
  begin
    FSection := TdxSpreadSheetReportSectionViewInfo(View.HitTest.HitObject).Section;
    FSizingArea := TdxSpreadSheetReportSizingArea(View.HitTest.HitObjectData);
    FSourceArea := Section.Bounds;
  end
  else
    Abort;
end;
...
begin
  ...
  Accepted := HitTest.HitAtCell;
  if Accepted then
  begin
    UpdateSectionBounds(
      TdxSpreadSheetTableViewCellViewInfo(HitTest.HitObject).Row,
      TdxSpreadSheetTableViewCellViewInfo(HitTest.HitObject).Column);
  end;
...
begin
  ...
  begin
    ...
    ViewInfo.CalculateSection(Section);
    View.Invalidate;
  end;
end;
...
begin
  case FSizingArea of
    saLeft:
     UpdateSectionBounds(MakeBounds(AColumn, FSourceArea.Top, FSourceArea.Right, FSourceArea.Bottom));
    saTop:
     UpdateSectionBounds(MakeBounds(FSourceArea.Left, ARow, FSourceArea.Right, FSourceArea.Bottom));
    saRight:
     UpdateSectionBounds(MakeBounds(FSourceArea.Left, FSourceArea.Top, AColumn, FSourceArea.Bottom));
    saBottom:
     UpdateSectionBounds(MakeBounds(FSourceArea.Left, FSourceArea.Top, FSourceArea.Right, ARow));
    saBottomRight:
     UpdateSectionBounds(MakeBounds(FSourceArea.Left, FSourceArea.Top, AColumn, ARow));
    saBottomLeft:
     UpdateSectionBounds(MakeBounds(AColumn, FSourceArea.Top, FSourceArea.Right, ARow));
  end;
end;
...
constructor TdxSpreadSheetReportSectionViewInfo.Create(ASection: TdxSpreadSheetReportSection; AView: TdxSpreadSheetCustomView);
begin
  inherited Create(AView);
  FSection := ASection;
end;

procedure TdxSpreadSheetReportSectionViewInfo.DoCalculate;
begin
  inherited;
  Corner := coBottomRight;
  FText := Section.ToString;
  FTextBounds := cxRectInflate(cxRect(MeasureSize), cxTextOffset * 2);
  FTextBounds := cxRectSetRight(FTextBounds, Bounds.Right + 1);
  if Section.SectionType in [rstFooter, rstGroupFooter] then
    FTextBounds := cxRectSetBottom(FTextBounds, Bounds.Bottom + 1)
  else
    FTextBounds := cxRectSetTop(FTextBounds, Bounds.Top - cxTextOffset);
end;
...
begin
  ...
  APrevPPI := dxSpreadSheetPrepareCanvas(ACanvas, Font);
  try
    AColor := dxGetLighterColor(ViewInfo.SelectionParams.Color, 60);

    ACanvas.SaveClipRegion;
    try
      TdxSpreadSheetSelectionHelper.Draw(ACanvas, Bounds, AColor, ViewInfo.ContentParams.Color, [ssseFrame], [Corner]);
    finally
      ACanvas.RestoreClipRegion;
    end;

    ACanvas.FillRect(TextBounds, AColor);
    ACanvas.Font.Color := ViewInfo.ContentParams.Color;
    ACanvas.DrawTexT(Text, TextBounds, taCenter, vaCenter, False, False);

    TdxSpreadSheetSelectionHelper.Draw(ACanvas, Bounds, AColor, ViewInfo.ContentParams.Color, [ssseCorners], [Corner]);
  finally
...
const
  CursorMap: array[TdxSpreadSheetReportSizingArea] of TCursor = (
    crDefault, crSizeWE, crSizeNS, crSizeWE, crSizeNS, crSizeNWSE, crSizeNESW
  );
begin
  if AHitTest.HitObject = Self then
    Result := CursorMap[TdxSpreadSheetReportSizingArea(AHitTest.HitObjectData)]
  else
...
function TdxSpreadSheetReportSectionViewInfo.InitHitTest(AHitTest: TdxSpreadSheetCustomHitTest): Boolean;
  ...
  function GetBorderBounds(ABorder: TcxBorder): TRect;
  begin
    Result := cxRectInflate(Bounds, HitTestZoneSize div 2);
    if ViewInfo.IsRightToLeftLayout then
      ABorder := TdxRightToLeftLayoutConverter.ConvertBorder(ABorder);
    case ABorder of
      bLeft:
...
const
  BorderToArea: array[TcxBorder] of TdxSpreadSheetReportSizingArea = (saLeft, saTop, saRight, saBottom);
  CornerToArea: array[TdxCorner] of TdxSpreadSheetReportSizingArea = (saBottomLeft, saBottomRight, saBottomLeft, saBottomRight);
var
  ABorder: TcxBorder;
  ACornerBounds: TRect;
begin
  ...
  if Result then
  begin
    ACornerBounds := TdxSpreadSheetSelectionHelper.GetCornerBounds(Bounds, Corner);
    ...
    if PtInRect(ACornerBounds, AHitTest.ActualHitPoint) then
      SetupHitTest(CornerToArea[Corner])
    else
      for ABorder := Low(TcxBorder) to High(TcxBorder) do
        if PtInRect(GetBorderBounds(ABorder), AHitTest.ActualHitPoint) then
        begin
          SetupHitTest(BorderToArea[ABorder]);
          Break;
        end;
  end;
end;

function TdxSpreadSheetReportSectionViewInfo.MeasureSize: TSize;
var
  APrevPPI: Integer;
begin
  APrevPPI := dxSpreadSheetPrepareCanvas(cxScreenCanvas, Font);
  try
    Result := cxScreenCanvas.TextExtent(FText);
  finally
    dxSpreadSheetUnprepareCanvas(cxScreenCanvas, APrevPPI);
    cxScreenCanvas.Dormant;
  end;
end;

procedure TdxSpreadSheetReportSectionViewInfo.Offset(X, Y: Integer);
begin
  inherited;
  FTextBounds := cxRectOffset(FTextBounds, X, Y);
end;

procedure TdxSpreadSheetReportSectionViewInfo.RightToLeftConversion(const AClientBounds: TRect);
begin
  inherited;
  FTextBounds := TdxRightToLeftLayoutConverter.ConvertRect(FTextBounds, AClientBounds);
end;

function TdxSpreadSheetReportSectionViewInfo.GetFont: TFont;
begin
  Result := TdxSpreadSheetFontAccess(SpreadSheet.DefaultCellStyle.Font).Handle.GraphicObject;
end;
...
begin
  if Visible then
    InvalidateRect(cxRectInflate(Bounds, 5));
end;
...
constructor TdxSpreadSheetDesignerChangeSectionCommand.Create(
  ASection: TdxSpreadSheetReportSection; const AName: string; const ABounds: TRect; ALevel: Integer = -1);
begin
  Level := ALevel;
  ...
  begin
    ...
    TdxSpreadSheetReportSectionsNameController.TryGetType(AName, SectionType, ALevel);
end;
...
procedure TdxSpreadSheetReportSections.Draw(ACanvas: TcxCanvas);
var
  I: Integer;
begin
  for I := 0 to Count - 1 do
    Items[I].ViewInfo.Draw(ACanvas, dsSecond);
end;

function TdxSpreadSheetReportSections.Find(const AName: string; out ASection: TdxSpreadSheetReportSection): Boolean;
var
  I: Integer;
begin
  Result := False;
  ...
  for I := 0 to Count - 1 do
  begin
    Result := SameText(Items[I].Name.Caption, AName);
    if Result then
    begin
      ASection := Items[I];
      Break;
...
function TdxSpreadSheetReportSections.FindByCell(const ARow, AColumn: Integer; out ASection: TdxSpreadSheetReportSection): Boolean;
var
  AItem: TdxSpreadSheetReportSection;
  I: Integer;
begin
  ASection := nil;
  for I := 0 to Count - 1 do
  begin
    AItem := Items[I];
    if dxSpreadSheetContains(AItem.Bounds, ARow, AColumn) then
    begin
      if (ASection = nil) or (AItem.Index > ASection.Index) then
        ASection := AItem;
    end;
  end;
  Result := ASection <> nil;
...
function TdxSpreadSheetReportSections.InitHitTest(AHitTest: TdxSpreadSheetCustomHitTest): Boolean;
var
  I: Integer;
begin
  for I := 0 to Count - 1 do
  begin
    if TdxSpreadSheetCellViewInfoAccess(Items[I].ViewInfo).InitHitTest(AHitTest) then
      Exit(True);
  end;
  Result := False;
end;

procedure TdxSpreadSheetReportSections.RightToLeftConversion(const AClientBounds: TRect);
var
  I: Integer;
begin
  for I := 0 to Count - 1 do
    Items[I].ViewInfo.RightToLeftConversion(AClientBounds);
end;
...
constructor TdxSpreadSheetReportSection.Create(AName: TdxSpreadSheetDefinedName; AType: TdxSpreadSheetReportSectionType; AIndex: Integer);
begin
  SectionType := AType;
  Index := AIndex;
  Name := AName;
  ...
  ViewInfo := TdxSpreadSheetReportSectionViewInfo.Create(Self, AName.SpreadSheet.ActiveSheetAsTable);
end;
...
function TdxSpreadSheetReportSection.ToString: string;
begin
  Result := cxGetResourceString(AreaCaptions[SectionType]) + IfThen(Index >= 0, IntToStr(Index));
end;
...
begin
  ...
  if Owner.Site <> nil then
    ...
    if FFieldChooserBounds.TryGetValue(Designer, R) then
      BoundsRect := R
    else
    begin
...
begin
  FFieldChooserBounds.AddOrSetValue(Designer, BoundsRect);
  inherited Destroy;
end;
...
begin
  FFieldChooserBounds.AddOrSetValue(Designer, Form.BoundsRect);
...
var
  R: TRect;
begin
  if (FForm <> nil) and FFieldChooserBounds.TryGetValue(Designer, R) then
    Form.BoundsRect := R;
...
var
  AAccepted: Boolean;
begin
  if FDragObject <> AValue then
  begin
    FreeAndNil(FDragObject);
    FDragObject := AValue;
    Designer.DragOverCore(Designer.GetMouseCursorClientPos, FDragObject, AAccepted);
    if AValue = nil then
      Designer.Invalidate;
  end;
end;
...
begin
  if (FSite <> AValue) and CanAssignedSite(AValue) then
  begin
    if FSite <> nil then
    begin
...
procedure TdxSpreadSheetReportBuilder.PostProcessFormulas;
  ...
  begin
    if (Area.Left = Area.Right) and (Area.Top = Area.Bottom) then
      Result := TdxSpreadSheetFormulaReferenceToken.Create(
        TdxSpreadSheetReference.Create(Area.Top, False),
        TdxSpreadSheetReference.Create(Area.Left, False))
    else
      Result := TdxSpreadSheetFormulaAreaReferenceToken.Create(
        TdxSpreadSheetReference.Create(Area.Top, False),
        TdxSpreadSheetReference.Create(Area.Left, False),
        TdxSpreadSheetReference.Create(Area.Bottom, False),
        TdxSpreadSheetReference.Create(Area.Right, False));
  end;
...
begin
  Destination.AddSheet(cxGetResourceString(@sdxEmptySheet));
  Destination.ActiveSheetAsTable.CreateCell(0, 0).SetText(cxGetResourceString(@sdxNoDataInfoText));
...
begin
  Result := Sections.Find(ActuallySectionName(AType, AIndex), ASection);
end;

function TdxSpreadSheetReportDesigner.FindSectionByCell(const ARow, AColumn: Integer;
  var ASection: TdxSpreadSheetReportSection): Boolean;
begin
  Result := Sections.FindByCell(ARow, AColumn, ASection);
end;
...
begin
  ID := AIndex;
  if TdxSpreadSheetReportSectionsNameController.TryGetType(AName, AType, AIndex) then
  begin
...
begin
  ...
  FSections := TdxSpreadSheetReportSections.Create;
...
procedure TdxSpreadSheetReportDesigner.DragOver(Source: TObject; X, Y: Integer; State: TDragState; var Accept: Boolean);
var
...
begin
  ...
  begin
    ...
    if Accept then
      DragOverCore(Point(X, Y), TdxSpreadSheetReportFieldDragObject(Source), Accept);
  end;
end;

procedure TdxSpreadSheetReportDesigner.DragOverCore(const P: TPoint; ADragObject: TDragObject; var AAccept: Boolean);
var
  ACellReference: TPoint;
  ACellViewInfo: TdxSpreadSheetTableViewCellViewInfo;
  ATableView: TdxSpreadSheetReportTableView;
begin
  ATableView := TdxSpreadSheetReportTableView(ActiveSheetAsTable);
  ATableView.HitTest.Calculate(P);
  AAccept := (ADragObject <> nil) and (FieldChooser.DragObject = ADragObject) and ATableView.HitTest.HitAtCell;

  if AAccept then
  begin
    ACellViewInfo := ATableView.HitTest.HitObject as TdxSpreadSheetTableViewCellViewInfo;
    ACellReference.X := ACellViewInfo.Column;
    ACellReference.Y := ACellViewInfo.Row;
  end
  else
    ACellReference := cxInvalidPoint;

  ATableView.ViewInfo.UpdateDragDropAreaCell(ACellReference.Y, ACellReference.X, AAccept);
end;
...
begin
  ...
  try
    ...
    for I := 0 to DefinedNames.Count - 1 do
      if TdxSpreadSheetReportSectionsNameController.IsSectionName(DefinedNames[I], AType, AIndex) then
      begin
...
function TdxSpreadSheetReportTableView.GetViewInfo: TdxSpreadSheetReportTableViewInfo;
begin
  Result := TdxSpreadSheetReportTableViewInfo(inherited ViewInfo);
end;
...
begin
  inherited Create(AView);
  FDragDropAreaCell := TdxSpreadSheetReportDragDropFieldAreaCell.Create(AView);
end;

destructor TdxSpreadSheetReportTableViewInfo.Destroy;
begin
  FreeAndNil(FDragDropAreaCell);
  inherited Destroy;
end;

function TdxSpreadSheetReportTableViewInfo.CanDrawCellSelection: Boolean;
begin
  FDrawBaseSelection := inherited CanDrawCellSelection;
  Result := True;
end;
...
function TdxSpreadSheetReportTableViewInfo.CreateMergedCellViewInfo(AMergedCell: TdxSpreadSheetMergedCell): TdxSpreadSheetTableViewMergedCellViewInfo;
begin
  Result := TdxSpreadSheetReportMergedCellViewInfo.Create(Self, AMergedCell);
end;

procedure TdxSpreadSheetReportTableViewInfo.CalculateCore;
var
  I: Integer;
begin
  inherited CalculateCore;
  for I := 0 to Sections.Count - 1 do
    CalculateSection(Sections[I]);
end;

procedure TdxSpreadSheetReportTableViewInfo.CalculateHitTest(AHitTest: TdxSpreadSheetCustomHitTest);
begin
  if not Sections.InitHitTest(AHitTest) then
    inherited CalculateHitTest(AHitTest);
end;

procedure TdxSpreadSheetReportTableViewInfo.CalculateSection(ASection: TdxSpreadSheetReportSection);
var
  AViewInfo: TdxSpreadSheetCellViewInfo;
begin
  AViewInfo := ASection.ViewInfo;
  AViewInfo.SetBounds(GetAreaBounds(ASection.Bounds), Bounds);
  AViewInfo.Recalculate;
  if IsRightToLeftLayout then
    AViewInfo.RightToLeftConversion(AViewInfo.Bounds);
end;

procedure TdxSpreadSheetReportTableViewInfo.DrawSelection(ACanvas: TcxCanvas);
begin
  if FDrawBaseSelection then
    inherited DrawSelection(ACanvas);

  ACanvas.SaveClipRegion;
  try
    if FDragDropAreaCell.Visible then
      FDragDropAreaCell.Draw(ACanvas, dsSecond);
    if Designer.Options.DesignView then
      Sections.Draw(ACanvas);
  finally
    ACanvas.RestoreClipRegion;
  end;
end;

procedure TdxSpreadSheetReportTableViewInfo.RightToLeftConversion(const AClientBounds: TRect);
begin
  inherited;
  Sections.RightToLeftConversion(AClientBounds);
end;

procedure TdxSpreadSheetReportTableViewInfo.UpdateDragDropAreaCell(ARowIndex, AColumnIndex: Integer; AAccepted: Boolean);
var
  AArea: TRect;
  AVisibleChanged: Boolean;
begin
  AVisibleChanged := FDragDropAreaCell.Visible <> AAccepted;
  FDragDropAreaCell.FVisible := AAccepted;
  if AAccepted then
  begin
    AArea := View.MergedCells.CheckCell(ARowIndex, AColumnIndex);
    if FDragDropAreaCell.Visible and not AVisibleChanged then
      FDragDropAreaCell.Invalidate;
    FDragDropAreaCell.SetBounds(GetAreaBounds(AArea), Bounds);
    if FDragDropAreaCell.Visible then
      FDragDropAreaCell.Invalidate;
  end
  else
...
function TdxSpreadSheetReportTableViewInfo.GetSections: TdxSpreadSheetReportSections;
begin
  Result := Designer.Sections;
end;
...
constructor TdxSpreadSheetReportCellStyleViewInfo.Create(ADataCell: TdxSpreadSheetTableViewCustomDataCellViewInfo);
begin
  inherited Create(ADataCell.Style, dxDefaultScaleFactor);
  FDataCell := ADataCell;
end;

procedure TdxSpreadSheetReportCellStyleViewInfo.Calculate(const ABounds, AContentBounds: TRect);
var
  ACell: TdxSpreadSheetCell;
  AGlyphSize: TSize;
begin
  ...
  FDatabaseGlyphBounds := cxNullRect;
  if Designer.Options.DesignView then
  begin
    ACell := FDataCell.Cell;
    if (ACell <> nil) and (ACell.DisplayText <> '') and Designer.IsDBFormula(ACell.AsFormula) then
    begin
      AGlyphSize := dxGetImageSize(DatabaseGlyph, ScaleFactor);
      FDatabaseGlyphBounds := cxRectSetTop(AContentBounds, AContentBounds.Top + cxTextOffset, AGlyphSize.Height);
      if UseRightToLeftLayout then
        FDatabaseGlyphBounds := cxRectSetLeft(FDatabaseGlyphBounds, FDatabaseGlyphBounds.Left + cxTextOffset, AGlyphSize.Width)
      else
        FDatabaseGlyphBounds := cxRectSetRight(FDatabaseGlyphBounds, FDatabaseGlyphBounds.Right - cxTextOffset, AGlyphSize.Width);
    end;
  end;
end;

procedure TdxSpreadSheetReportCellStyleViewInfo.Draw(ACanvas: TcxCanvas; AStage: TdxSpreadSheetDrawingStage);
begin
  inherited;

  if (AStage = dsSecond) and not cxRectIsEmpty(FDatabaseGlyphBounds) then
    DatabaseGlyph.StretchDraw(ACanvas.Handle, FDatabaseGlyphBounds);
end;

procedure TdxSpreadSheetReportCellStyleViewInfo.Offset(const AOffset: TPoint);
begin
  inherited;
  FDatabaseGlyphBounds := cxRectOffset(FDatabaseGlyphBounds, AOffset);
end;

function TdxSpreadSheetReportCellStyleViewInfo.GetSupportedDrawingStages: TdxSpreadSheetDrawingStages;
begin
  Result := inherited GetSupportedDrawingStages;
  if not cxRectIsEmpty(FDatabaseGlyphBounds) then
    Include(Result, dsSecond);
end;

function TdxSpreadSheetReportCellStyleViewInfo.GetDatabaseGlyph: TdxSmartImage;
begin
  Result := dxSpreadSheetReportDesigner.GetDatabaseGlyph;
end;

function TdxSpreadSheetReportCellStyleViewInfo.GetDesigner: TdxSpreadSheetReportDesigner;
begin
  Result := TdxSpreadSheetReportDesigner(FDataCell.SpreadSheet);
end;

{ TdxSpreadSheetReportCellViewInfo }

function TdxSpreadSheetReportCellViewInfo.CreateStyleViewInfo: TdxSpreadSheetConditionalFormattingStyleViewInfo;
begin
  Result := TdxSpreadSheetReportCellStyleViewInfo.Create(Self);
end;

{ TdxSpreadSheetReportMergedCellViewInfo }

function TdxSpreadSheetReportMergedCellViewInfo.CreateStyleViewInfo: TdxSpreadSheetConditionalFormattingStyleViewInfo;
begin
  Result := TdxSpreadSheetReportCellStyleViewInfo.Create(Self);
end;
...
begin
  ...
  AProduct.Add('sdxEmptySheet', @sdxEmptySheet);
  AProduct.Add('sdxNoDataInfoText', @sdxNoDataInfoText);
end;

procedure RegisterAssistants;
var
  ARepository: TdxSpreadSheetFunctionsRepositoryAccess;
begin
  FFieldChooserBounds := TDictionary<TdxSpreadSheetReportDesigner, TRect>.Create;

  ARepository := TdxSpreadSheetFunctionsRepositoryAccess(dxSpreadSheetFunctionsRepository);
  ARepository.InternalAdd(@sfnField, fnField, fpiField, frkValue, 255, ftDatabase);
  ARepository.InternalAdd(@sfnParameter, fnParameters, fpiField, frkValue, 255, ftDatabase);
  ARepository.InternalAdd(@sfnFieldPicture, fnFieldPicture, fpiFieldPicture, frkValue, 255, ftDatabase);
  ARepository.InternalAdd(@sfnRange, fnRange, fpiRange, frkValue, 255, ftDatabase);

  RegisterClasses([TdxSpreadSheetReportDesigner, TdxSpreadSheetReportTableView, TdxSpreadSheetReportDetail]);
  TdxSpreadSheetReportSectionsNameController.Register;
...
begin
  TdxSpreadSheetReportSectionsNameController.Unregister;
  FreeAndNil(FFieldChooserBounds);
  FreeAndNil(FDatabaseGlyph);
