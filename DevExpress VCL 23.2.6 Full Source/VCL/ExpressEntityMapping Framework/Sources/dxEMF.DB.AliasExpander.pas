...
unit dxEMF.DB.AliasExpander; // for internal use
...
type
  ...
  strict private
    ...
    class function GetFalse: TdxExpandedCriteriaHolder; static;
    class function GetTrue: TdxExpandedCriteriaHolder; static;
  public
    ...
    class property FalseCriteria: TdxExpandedCriteriaHolder read GetFalse;
    class property TrueCriteria: TdxExpandedCriteriaHolder read GetTrue;
  end;
...
const
  dxThisUnitName = 'dxEMF.DB.AliasExpander';
...
begin
  ...
  begin
    ...
    APropertyName := AOperand.PropertyName.TrimRight(['!']);
    if APropertyName.StartsWith('^.') then
    begin
      APropertyName := APropertyName.Substring(2);
    ...
    begin
      ...
      if APointLastIndex >= 0 then
        AMemberName := APropertyName.Substring(0, APointLastIndex)
      else
      ...
      if (((AReferenceMember <> nil) and (AReferenceMember.ReferenceType = AParentEntityInfo)) and
        ...
        (AParentEntityInfo.KeyProperty.Member.MemberName = APropertyName.Substring(APointLastIndex + 1)))) then
...
begin
  ...
  while APropertyName.StartsWith('^.') do
  begin
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxConstantDescriptor.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FInstance);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxConstantDescriptor.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
class constructor TdxExpandedCriteriaHolder.Create;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(dxThisUnitName, 'TdxExpandedCriteriaHolder.Create', SysInit.HInstance);{$ENDIF}
  ...
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(dxThisUnitName, 'TdxExpandedCriteriaHolder.Create', SysInit.HInstance);{$ENDIF}
end;
...
class function TdxExpandedCriteriaHolder.GetTrue: TdxExpandedCriteriaHolder;
begin
  Result := FTrue;
end;
...
class function TdxExpandedCriteriaHolder.GetFalse: TdxExpandedCriteriaHolder;
begin
  Result := FFalse;
end;
...
begin
  ...
  begin
    ...
    if AOperand.PropertyName.EndsWith(TdxEMFPropertyDescriptor.ReferenceAsObjectTail) then 
      AOperand := TdxOperandProperty.Create(AOperand.PropertyName.Substring(0, Length(AOperand.PropertyName) - Length(TdxEMFPropertyDescriptor.ReferenceAsObjectTail)));
  Result := AOperand;
end;
