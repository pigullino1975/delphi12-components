...
type
 ...
 { TdxBaseFormatterHelper }
  ...
  public
    class function ToValue(AField: TField; AMemberInfo: TdxMappingMemberInfo): TValue; overload;
    class function ToValue(const AValue: Variant; AMemberInfo: TdxMappingMemberInfo): TValue; overload;
...
begin
  ...
  begin
    ...
    if ASerializedObject <> nil then
      if VarIsNull(AValues[0]) then
        AMemberInfo.Serializer.Clear(ASerializedObject)
      else
      begin
        ALength := VarArrayHighBound(AValues[0], 1);
    ...
    AMemberInfo.SetValue(AObject, ToValue(AValues[0], AMemberInfo));
end;
...
class function TdxFieldValueConverter.ToValue(const AValue: Variant; AMemberInfo: TdxMappingMemberInfo): TValue;
var
  S: string;
  ABytes: TBytes;
begin
  ...
  begin
    ABytes := AValue;
    Exit(AMemberInfo.ConvertArray(ABytes));
  end
  else
    if AMemberInfo.IsType(TypeInfo(TGUID)) then
      Exit(TValue.From<TGUID>(dxStringToGUID(AValue)))
    else
      case AMemberInfo.FieldType of
        TFieldType.ftSmallint, TFieldType.ftInteger, TFieldType.ftWord, TFieldType.ftShortint, TFieldType.ftByte:
