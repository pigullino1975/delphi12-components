...
const
  dxThisUnitName = 'dxEMF.Core.DBHelper';
...
var
  ...
  AResult: TdxArray<TdxDBColumn>;
begin
  if AMemberInfo.ReferenceType = nil then
    AWorkMemberInfo := AMemberInfo
  else
  begin
    if AMemberInfo.ReferenceType.KeyProperty.IsCompositeKey then
    begin
      NotImplemented; 
      AResult := TdxArray<TdxDBColumn>.Create(Length(AMemberInfo.ReferenceType.KeyProperty.SubMembers));
      for AWorkMemberInfo in AMemberInfo.ReferenceType.KeyProperty.SubMembers do
        AResult.AddRange(CreateColumn(AWorkMemberInfo, ''));
...
var
  ...
  AMemberInfo: TdxMappingMemberInfo;
begin
  AResult := TdxArray<TdxDBColumn>.Create(AEntityInfo.KeyAttributes.Count * 2);
