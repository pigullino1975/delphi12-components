...
//    destructor Destroy; override;
  ...
  strict private
    ...
    function GetSortProperties: TdxQuerySortingCollection; inline;
    function GetGroupProperties: TdxCriteriaOperatorList; inline;
  protected
    procedure CollectJoinNodesAndCriteriaInternal(ANodes: TdxJoinNodeList; ACriteria: TdxCriteriaOperatorList); override;
  public
    destructor Destroy; override;
  ...
  strict private
    FSQLText: string;
    FParameters: TArray<IdxOperandValue>;
  ...
  public
    ...
    constructor Create(const ASQLText: string; const AParameters: TArray<IdxOperandValue>; const AParameterNames: TArray<string>;
    ...
    destructor Destroy; override;
    procedure AppendParameter(const AName: string; const AParameter: IdxOperandValue);
    ...
    property Parameters: TArray<IdxOperandValue> read FParameters;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxQueryParameterCollection.Destroy', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FEmpty);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxQueryParameterCollection.Destroy', SysInit.HInstance);{$ENDIF}
end;
...
var
  ...
  ATable: TdxDBTable;
begin
  ATablesNames := TdxArray<string>.Create(Length(AStatements));
...
constructor TdxQuery.Create(const ASQLText: string; const AParameters: TArray<IdxOperandValue>;
  const AParameterNames: TArray<string>; ASkipSelectedRecords: Integer; ATopSelectedRecords: Integer);
begin
  inherited Create;
  FConstantValues := nil;
  FOperandIndexes := nil;
...
constructor TdxQuery.Create(const ASQLText: string; AParameters: TdxQueryParameterCollection;
  const AParameterNames: TArray<string>; ASkipSelectedRecords, ATopSelectedRecords: Integer);
begin
  Create(ASQLText, AParameters.ToArray, AParameterNames, ASkipSelectedRecords, ATopSelectedRecords);
end;
...
procedure TdxQuery.AppendParameter(const AName: string; const AParameter: IdxOperandValue);
var
  I: Integer;
begin
  I := Length(FParameterNames);
  SetLength(FParameterNames, I + 1);
  SetLength(FParameters, I + 1);
  FParameterNames[I] := AName;
  FParameters[I] := AParameter;
  FParametersBuf.Add(AName, AParameter);
end;
