...
type
  ...
  strict private
    ...
    class function GetNull: TdxCriteriaToStringVisitResult; static;
  public
    ...
    class property Null: TdxCriteriaToStringVisitResult read GetNull; 
  end;
  ...
  public
    ...
    class function CriterionEquals(const ALeftOperator, ARightOperator: IdxCriteriaOperator): Boolean; static;
  end;
  ...
  public
    ...
    procedure Insert(AIndex: Integer; const AItem: IdxCriteriaOperator);
 ...
 { TdxOperandValue }
  ...
  public
    ...
    constructor Create(AJoinType: TClass; const ACondition: IdxCriteriaOperator; AType: TdxAggregateFunctionType;
      const AAggregatedExpression: IdxCriteriaOperator); overload;
  ...
  public
    ...
    function Add(const AProperty: IdxCriteriaOperator; ASortDirection: TdxSortDirection = TdxSortDirection.Ascending): Integer; overload;
    function Add(const APropertyName: string; ASortDirection: TdxSortDirection = TdxSortDirection.Ascending): Integer; overload;
    procedure Clear;
  ...
  public
    class function Accept<T>(const ACriteriaOperator: IdxCriteriaOperator; const AVisitor: IdxCriteriaVisitor<T>): T; overload;
    class function Accept<T>(const ACriteriaOperator: IdxCriteriaOperator; const AVisitor: IdxClientCriteriaVisitor<T>): T; overload;
...
uses
  Math, TypInfo, TimeSpan, StrUtils, Variants,
  dxCore, dxCoreClasses, dxStringHelper, dxHash,
  dxEMF.Utils.CriteriaParser,
  dxEMF.DB.Query;

const
  dxThisUnitName = 'dxEMF.DB.Criteria';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(dxThisUnitName, 'TdxCriteriaToStringVisitResult.Create', SysInit.HInstance);{$ENDIF}
  FNull := TdxCriteriaToStringVisitResult.Create(NullCriteriaResult, TdxCriteriaPriorityClass.Atom);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(dxThisUnitName, 'TdxCriteriaToStringVisitResult.Create', SysInit.HInstance);{$ENDIF}
end;
...
class function TdxCriteriaToStringVisitResult.GetNull: TdxCriteriaToStringVisitResult;
begin
  Result := FNull;
end;
      ...
      begin
        ...
        begin
          if AValue.IsSingle then
            ...
            D := AValue.AsExtended;
          Result := FixNonFixedText(FloatToStr(D, dxInvariantFormatSettings), AIsLegacy, AValue); //ToString('r', CultureInfo.InvariantCulture)
        end;
      end;
...
begin
  ...
  if (AVal.Value.IsNullOrEmpty) and not (AVal is TdxConstantValue) then
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxCriteriaToBasicStyleParameterlessProcessor.Initialize', SysInit.HInstance);{$ENDIF}
  FInstance := TdxCriteriaToBasicStyleParameterlessProcessor.Create;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxCriteriaToBasicStyleParameterlessProcessor.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxCriteriaToBasicStyleParameterlessProcessor.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxCriteriaToBasicStyleParameterlessProcessor.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FInstance);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxCriteriaToBasicStyleParameterlessProcessor.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
class function TdxCriteriaOperator.CriterionEquals(const ALeftOperator, ARightOperator: IdxCriteriaOperator): Boolean;
begin
  if ALeftOperator = ARightOperator then
    Exit(True);
  if ALeftOperator = nil then
    Exit(False);
  Result := TdxCriteriaOperator(ALeftOperator).Equals(TdxCriteriaOperator(ARightOperator));
end;
...
constructor TdxCriteriaOperatorCollection.Create(const AOperands: array of IdxCriteriaOperator);
begin
  Create(Length(AOperands));
  AddRange(AOperands);
end;
...
function TdxCriteriaOperatorCollection.Equals(Obj: TObject): Boolean;
var
...
begin
  if Self = Obj then
    Exit(True);
  AAnother := Safe<TdxCriteriaOperatorCollection>.Cast(Obj);
...
procedure TdxCriteriaOperatorCollection.Insert(AIndex: Integer; const AItem: IdxCriteriaOperator);
begin
...
function TdxOperandValue.Equals(Obj: TObject): Boolean;
var
  AOperandValue: TdxOperandValue absolute Obj;
begin
...
function TdxOperandParameter.Equals(Obj: TObject): Boolean;
begin
  Result := inherited Equals(Obj) and (ParameterName = (TdxOperandParameter(Obj)).ParameterName);
end;
...
function TdxOperandProperty.Equals(Obj: TObject): Boolean;
begin
  if Obj = nil then
    Exit(False);
  if ClassType <> Obj.ClassType then
    Exit(False);
  Result := PropertyName = TdxOperandProperty(Obj).PropertyName;
end;
...
constructor TdxConstantValue.Create;
begin
  Create(TValue.FromVariant(Null));
end;
...
function TdxBinaryOperator.Equals(Obj: TObject): Boolean;
begin
  if Obj = nil then
    Exit(False);
  if ClassType <> Obj.ClassType then
    Exit(False);
  Result := (OperatorType = TdxBinaryOperator(Obj).OperatorType) and
    (LeftOperand as TdxCriteriaOperator).Equals(TdxBinaryOperator(Obj).LeftOperand as TdxCriteriaOperator) and
    (RightOperand as TdxCriteriaOperator).Equals(TdxBinaryOperator(Obj).RightOperand as TdxCriteriaOperator);
end;
...
function TdxUnaryOperator.Equals(Obj: TObject): Boolean;
begin
  if Obj = nil then
    Exit(False);
  if ClassInfo <> Obj.ClassInfo then
    Exit(False);
  Result := (OperatorType = TdxUnaryOperator(Obj).OperatorType) and (Operand as TdxCriteriaOperator).Equals(TdxUnaryOperator(Obj).Operand as TdxCriteriaOperator);
end;
...
function TdxBetweenOperator.Equals(Obj: TObject): Boolean;
begin
  if Obj = nil then
    Exit(False);
  if ClassType <> Obj.ClassType then
    Exit(False);
  Result := (TestExpression as TdxCriteriaOperator).Equals(TdxBetweenOperator(Obj).TestExpression as TdxCriteriaOperator) and
    (BeginExpression as TdxCriteriaOperator).Equals(TdxBetweenOperator(Obj).BeginExpression as TdxCriteriaOperator) and
    (EndExpression as TdxCriteriaOperator).Equals(TdxBetweenOperator(Obj).EndExpression as TdxCriteriaOperator);
end;
...
function TdxInOperator.Equals(Obj: TObject): Boolean;
var
...
begin
  ...
  ARightOperands := TdxInOperator(Obj).Operands;
  if Operands.Count <> ARightOperands.Count then
    Exit(False);
  if not (LeftOperand as TdxCriteriaOperator).Equals(TdxInOperator(Obj).LeftOperand as TdxCriteriaOperator) then
    Exit(False);
...
function TdxAggregateOperand.Equals(Obj: TObject): Boolean;
var
  AAnother: TdxAggregateOperand;
begin
  AAnother := Safe<TdxAggregateOperand>.Cast(Obj);
  if AAnother = nil then
    Result := False
  else
...
constructor TdxJoinOperand.Create(AJoinType: TClass; const ACondition: IdxCriteriaOperator);
begin
  Assert(AJoinType <> nil);
  Create(dxGetQualifiedClassName(AJoinType), ACondition);
end;

constructor TdxJoinOperand.Create(AJoinType: TClass; const ACondition: IdxCriteriaOperator;
  AType: TdxAggregateFunctionType; const AAggregatedExpression: IdxCriteriaOperator);
begin
  Assert(AJoinType <> nil);
  Create(dxGetQualifiedClassName(AJoinType), ACondition, AType, AAggregatedExpression);
end;


function TdxJoinOperand.Equals(Obj: TObject): Boolean;
var
  AAnother: TdxJoinOperand;
begin
  AAnother := Safe<TdxJoinOperand>.Cast(Obj);
  if AAnother = nil then
    Exit(False);
...
function TdxGroupOperator.Equals(Obj: TObject): Boolean;
var
  I: Integer;
begin
  if Obj = nil then
  ...
  if ClassType <> Obj.ClassType then
    Exit(False);
  if OperatorType <> TdxGroupOperator(Obj).OperatorType then
    Exit(False);
  if Operands.Count <> (TdxGroupOperator(Obj)).Operands.Count then
    Exit(False);
  for I := 0 to Operands.Count - 1 do
    if not (Operands[I] as TdxCriteriaOperator).Equals(TdxGroupOperator(Obj).Operands[I] as TdxCriteriaOperator) then
      Exit(False);
  Result := True;
end;
...
function TdxFunctionOperator.Equals(Obj: TObject): Boolean;
var
  I: Integer;
begin
  if Obj = nil then
  ...
  if ClassType <> Obj.ClassType then
    Exit(False);
  if OperatorType <> TdxFunctionOperator(Obj).OperatorType then
    Exit(False);
  if Operands.Count <> (TdxFunctionOperator(Obj)).Operands.Count then
    Exit(False);
  for I := 0 to Operands.Count - 1 do
    if not (Operands[I] as TdxCriteriaOperator).Equals(TdxFunctionOperator(Obj).Operands[I] as TdxCriteriaOperator) then
      Exit(False);
  Result := True;
end;
...
function TdxSortByExpression.Equals(Obj: TObject): Boolean;
var
  ASortByExpression: TdxSortByExpression;
begin
  ASortByExpression := Safe<TdxSortByExpression>.Cast(Obj);
  if ASortByExpression = nil then
    Exit(False);
  Result := TdxCriteriaOperator.CriterionEquals(FProperty, ASortByExpression.FProperty) and
    (FSortDirection = ASortByExpression.FSortDirection);
end;
...
function TdxSortByExpression.ToString: string;
begin
  Result := PropertyName + IfThen(FSortDirection = TdxSortDirection.Descending, ' DESC');
end;
...
function TdxSortByExpressions.Add(const APropertyName: string; ASortDirection: TdxSortDirection): Integer;
begin
  Result := Add(TdxSortByExpression.Create(APropertyName, ASortDirection));
end;

function TdxSortByExpressions.Add(const AProperty: IdxCriteriaOperator; ASortDirection: TdxSortDirection): Integer;
begin
  Result := Add(TdxSortByExpression.Create(AProperty, ASortDirection));
end;
...
function TdxSortByExpressions.Equals(Obj: TObject): Boolean;
var
  AAnother: TdxSortByExpressions;
  I: Integer;
begin
  if Self = Obj then
    Exit(True);
  AAnother := Safe<TdxSortByExpressions>.Cast(Obj);
  if AAnother = nil then
    Exit(False);
  if Count <> AAnother.Count then
    Exit(False);
  for I := 0 to Count - 1 do
    if not (Self[I] as TdxSortByExpression).Equals(AAnother[I] as TdxSortByExpression) then
      Exit(False);
  Result := True;
end;
...
function TdxSortByExpressions.ToString: string;
var
  AResult: TStringBuilder;
  AExpression: IdxSortByExpression;
  I: Integer;
begin
  AResult := TStringBuilder.Create;
  try
    ...
    begin
      ...
      if AResult.Length > 0 then
        AResult.Append(', ');
      AResult.Append((AExpression as TdxSortByExpression).ToString);
    end;
    Result := AResult.ToString;
  finally
    AResult.Free;
...
var
  AFunctionOperand: TdxOperandValue;
  AName: string;
begin
  if AFunc = nil then
    Exit(False);
  ...
  if AFunc.Operands.Count <> 3 then
    Exit(False);
  AFunctionOperand := Safe<TdxOperandValue>.Cast(AFunc.Operands[0] as TdxCriteriaOperator);
  if AFunctionOperand = nil then
    Exit(False);
  if AFunctionOperand.Value.IsString then
    AName := AFunctionOperand.Value.AsString
  else
    AName := '';
  Result := IsName(AName);
end;
...
function TdxQueryOperand.Equals(Obj: TObject): Boolean;
var
  AAnother: TdxQueryOperand;
begin
  AAnother := Safe<TdxQueryOperand>.Cast(Obj);
  if AAnother = nil then
    Exit(False);
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxAcceptor.Create', SysInit.HInstance);{$ENDIF}
  ...
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxAcceptor.Create', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxAcceptor.Destroy;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxAcceptor.Destroy', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FCriteria);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxAcceptor.Destroy', SysInit.HInstance);{$ENDIF}
end;
...
class function TdxAcceptor.Accept<T>(const ACriteriaOperator: IdxCriteriaOperator;
  const AVisitor: IdxCriteriaVisitor<T>): T;
begin
  case GetCriteriaType(ACriteriaOperator) of
    TCriteria.TBetweenOperator:
      Result := AVisitor.Visit(IdxBetweenOperator(ACriteriaOperator));
    TCriteria.TBinaryOperator:
      Result := AVisitor.Visit(IdxBinaryOperator(ACriteriaOperator));
    TCriteria.TUnaryOperator:
      Result := AVisitor.Visit(IdxUnaryOperator(ACriteriaOperator));
    TCriteria.TInOperator:
      Result := AVisitor.Visit(IdxInOperator(ACriteriaOperator));
    TCriteria.TGroupOperator:
      Result := AVisitor.Visit(IdxGroupOperator(ACriteriaOperator));
    TCriteria.TOperandValue:
      Result := AVisitor.Visit(IdxOperandValue(ACriteriaOperator));
    TCriteria.TFunctionOperator:
      Result := AVisitor.Visit(IdxFunctionOperator(ACriteriaOperator));
    else
...
class function TdxAcceptor.Accept<T>(const ACriteriaOperator: IdxCriteriaOperator;
  const AVisitor: IdxClientCriteriaVisitor<T>): T;
begin
  case GetCriteriaType(ACriteriaOperator) of
    TCriteria.TBetweenOperator:
      Result := AVisitor.Visit(IdxBetweenOperator(ACriteriaOperator));
    ...
    TCriteria.TOperandProperty:
      Result := AVisitor.Visit(IdxOperandProperty(ACriteriaOperator));
    TCriteria.TAggregateOperand:
      Result := AVisitor.Visit(IdxAggregateOperand(ACriteriaOperator));
    TCriteria.TJoinOperand:
      Result := AVisitor.Visit(IdxJoinOperand(ACriteriaOperator));
    else
      raise EArgumentException.Create('');
  end;
end;
