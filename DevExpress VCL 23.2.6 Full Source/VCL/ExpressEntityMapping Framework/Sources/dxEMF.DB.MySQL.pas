...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxMySQLConnectionProvider.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FConnectionParameters);
  FreeAndNil(FDBTypes);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxMySQLConnectionProvider.Finalize', SysInit.HInstance);{$ENDIF}
end;
