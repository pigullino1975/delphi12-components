...
type
  ...
  protected const
    Ownerships: array [Boolean] of TDictionaryOwnerships = ([], [doOwnsValues]);
  protected
    procedure FreePool; virtual; abstract;
  public
  ...
  strict private
    ...
    FOnCreatePool: TNotifyEvent;
  protected
    function CreatePool(AEntityInfo: TdxEntityInfo): TdxCustomEntityPool;
    procedure FreePools;
    procedure RemovePool(AClass: TClass);
  public
    constructor Create(AOwnsObjects: Boolean = True);
    destructor Destroy; override;
    ...
    function GetEnumerator: TEnumerator<TdxCustomEntityPool>; inline;
    function GetPool(AClass: TClass): TdxCustomEntityPool;
    function IsNewObject(AObject: TObject): Boolean;
    property OnCreatePool: TNotifyEvent read FOnCreatePool write FOnCreatePool;
  end;
...
const
  dxThisUnitName = 'dxEMF.Core.Pool';
...
constructor TdxCustomEntityPool<T>.Create(AOwnsObjects: Boolean = True);
begin
  inherited Create;
  FObjectPool := TObjectDictionary<T, TObject>.Create(Ownerships[AOwnsObjects], GetComparer);
...
procedure TdxCustomEntityPool<T>.FreePool;
begin
  FObjectKeys.Clear;
  FObjectPool.Clear;
end;
...
constructor TdxEntityPoolClasses.Create(AOwnsObjects: Boolean = True);
begin
  inherited Create;
...
function TdxEntityPoolClasses.CreatePool(AEntityInfo: TdxEntityInfo): TdxCustomEntityPool;
begin
  if not AEntityInfo.KeyProperty.IsCompositeKey then
    case AEntityInfo.KeyProperty.Member.TypeKind of
      tkInteger:
        Exit(TdxIntegerKeyEntityPool.Create(FOwnsObjects));
      tkRecord:
        if AEntityInfo.KeyProperty.Member.RttiType.Handle = TypeInfo(TGUID) then
          Exit(TdxGUIDKeyEntityPool.Create(FOwnsObjects));
      tkChar, tkWChar, tkEnumeration, tkInt64:
        Exit(TdxInt64KeyEntityPool.Create(FOwnsObjects));
      tkUString:
        Exit(TdxStringKeyEntityPool.Create(FOwnsObjects));
    end;
  Result := TdxEntityPool.Create(FOwnsObjects);
end;
...
procedure TdxEntityPoolClasses.FreePools;
var
  APool: TdxCustomEntityPool;
begin
  for APool in FPool.Values do
    APool.FreePool;
end;

function TdxEntityPoolClasses.GetEnumerator: TEnumerator<TdxCustomEntityPool>;
begin
  Result := FPool.Values.GetEnumerator;
end;
...
begin
  ...
  begin
    ...
    if Assigned(FOnCreatePool) then
      FOnCreatePool(Result);
...
procedure TdxEntityPoolClasses.RemovePool(AClass: TClass);
begin
  FPool.Remove(AClass);
end;
