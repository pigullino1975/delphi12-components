...
const
  dxThisUnitName = 'dxEMF.Core.Helpers';
...
begin
  ...
  if (AOptimisticLock <> nil) and AOptimisticLock.GetValue(AObject).IsNullOrEmpty then
    ...
    try
      ...
      begin
        ...
        begin
          ...
          if AKeyValue.IsNullOrEmpty or (AKeyValue.AsGUID = TGUID.Empty) then
...
begin
  ...
  begin
    ...
    begin
      ...
      if AReferred.IsNullOrEmpty then
        Continue;
...
var
  ANewObjects: TList<TObject>;
  AOldObjects, AResult: TdxArray<TObject>;
  AObjects: TArray<TObject>;
  AObject: TObject;
begin
  ANewObjects := TList<TObject>.Create;
  try
    AObjects := TdxEMFCustomSessionAccess(ASession).GetObjectsToSave;
    AOldObjects := TdxArray<TObject>.Create(Length(AObjects));
    AResult := TdxArray<TObject>.Create(Length(AObjects));
    ...
    AResult.AddRange(AOldObjects.ToArray);
