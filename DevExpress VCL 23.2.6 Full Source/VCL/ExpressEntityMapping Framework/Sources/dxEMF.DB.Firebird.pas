...
const
  dxThisUnitName = 'dxEMF.DB.Firebird';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxFirebirdConnectionProvider.Destroy', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FConnectionParameters);
  FreeAndNil(FDBTypes);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxFirebirdConnectionProvider.Destroy', SysInit.HInstance);{$ENDIF}
end;
