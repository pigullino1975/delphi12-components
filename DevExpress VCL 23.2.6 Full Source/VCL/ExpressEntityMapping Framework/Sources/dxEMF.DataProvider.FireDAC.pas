...
const
  dxThisUnitName = 'dxEMF.DataProvider.FireDAC';
...
var
  ...
  AParameter: TFDParam;
begin
  for I := 0 to Length(Query.Parameters) - 1 do
  begin
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxFireDACConnectionVendor.Destroy', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FDriverNameAliases);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxFireDACConnectionVendor.Destroy', SysInit.HInstance);{$ENDIF}
end;
...
begin
  ...
  for I := 0 to Length(AQuery.Parameters) - 1 do
  begin
