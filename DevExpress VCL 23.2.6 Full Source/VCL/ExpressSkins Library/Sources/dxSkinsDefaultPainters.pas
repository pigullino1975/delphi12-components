...
uses
  ...
  dxSkinsLookAndFeelPainter, dxSkinsStrs, dxSkinInfo;
...
type
  ...
  TdxSkinsUserSkinPainter = class(TdxAdvancedSkinLookAndFeelPainter)
  protected
  ...
  { TdxSkinsUserSkinLoader }

  TdxSkinsUserSkinLoader = class(TInterfacedObject, IdxUserSkinLoader)
  protected
    //IdxUserSkinLoader
...
function dxSkinsUserSkinLoadFromResource(AInstance: THandle; const AResourceName: string; const ASkinName: string = ''): Boolean; 
...
function dxSkinsUserSkinPopulateSkinNames(AStream: TStream; AList: TStrings; ADisplayNames: Boolean = False): Boolean; overload; 
function dxSkinsUserSkinPopulateSkinNames(const AFileName: string; AList: TStrings; ADisplayNames: Boolean = False): Boolean; overload; 
implementation

uses
  dxCore, cxLookAndFeels, Math;

const
  dxThisUnitName = 'dxSkinsDefaultPainters';
...
function dxSkinsGetUserSkin(out ASkin: TdxSkin; out APainter: TcxCustomLookAndFeelPainter): Boolean; overload;
var
  ASkinInfo: TdxSkinLookAndFeelPainterInfo;
begin
  Result := cxLookAndFeelPaintersManager.GetPainter(sdxSkinsUserSkinName, APainter);
  if not Result then
    cxLookAndFeelPaintersManager.Register(TdxSkinsUserSkinPainter.Create(sdxDefaultUserSkinData, HInstance));
  Result := cxLookAndFeelPaintersManager.GetPainter(sdxSkinsUserSkinName, APainter) and APainter.GetPainterData(ASkinInfo);
  if Result then
    ASkin := ASkinInfo.Skin;
end;

function dxSkinsGetUserSkin(out ASkin: TdxSkin): Boolean; overload;
var
  APainter: TcxCustomLookAndFeelPainter;
begin
  Result := dxSkinsGetUserSkin(ASkin, APainter);
end;
...
class procedure TdxSkinsUserSkinLoader.Initialize;
begin
  dxIUserSkinLoader := TdxSkinsUserSkinLoader.Create;
end;

class procedure TdxSkinsUserSkinLoader.Finalize;
begin
  dxIUserSkinLoader := nil;
end;

function TdxSkinsUserSkinLoader.GetUserSkinName: string;
begin
  Result := sdxSkinsUserSkinName;
end;

function TdxSkinsUserSkinLoader.LoadFromFile(const AFileName, ASkinName: string): Boolean;
begin
  Result := dxSkinsUserSkinLoadFromFile(AFileName, ASkinName);
end;

function TdxSkinsUserSkinLoader.LoadFromResource(AInstance: THandle; const AResourceName,
  ASkinName: string): Boolean;
begin
  Result := dxSkinsUserSkinLoadFromResource(AInstance, AResourceName, ASkinName);
end;

function TdxSkinsUserSkinLoader.LoadFromStream(AStream: TStream; const ASkinName: string): Boolean;
begin
  AStream.Position := 0;
  Result := dxSkinsUserSkinLoadFromStream(AStream, ASkinName);
end;
...
initialization
  ...
  TdxSkinsUserSkinLoader.Initialize;

finalization
  TdxSkinsUserSkinLoader.Finalize;
