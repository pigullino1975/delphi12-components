...
type
  ...
  private
    function GetItem(Index: TdxListIndex): TdxHunspellFlags; inline;
  public
    property Items[Index: TdxListIndex]: TdxHunspellFlags read GetItem; default;
  end;
...
const
  dxThisUnitName = 'dxHunspellWords';
...
function TdxHunspellFlagsList.GetItem(Index: TdxListIndex): TdxHunspellFlags;
begin
...
begin
  Result := False;
  GetMem(ALineBuffer, MaxLineLength);
  try
    AReader := TdxHunspellReader.Create(AStream, CodePage);
    try
      ALineCursor := ALineBuffer;
      if not AReader.GetNextLine(ALineBuffer^, MaxLineLength) then
        Exit;
      if not AllocateHashTable(ALineCursor) then
        Exit;
      while AReader.GetNextLine(ALineBuffer^, MaxLineLength) do
