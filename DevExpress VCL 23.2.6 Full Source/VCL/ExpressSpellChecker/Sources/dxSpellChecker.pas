...
type
  ...
  strict private
    function GetItem(Index: TdxListIndex): TdxSpellCheckerSuggestion;
  public
    ...
    property Items[Index: TdxListIndex]: TdxSpellCheckerSuggestion read GetItem; default;
  end;
  ...
  strict private
    ...
    function GetItem(Index: TdxListIndex): TdxSpellCheckerSuggestionCacheItem;
  ...
  public
    ...
    property Items[Index: TdxListIndex]: TdxSpellCheckerSuggestionCacheItem read GetItem; default;
  end;
  ...
  TdxSpellCheckerSentence = record
    MisspelledWordPositionFinish: IdxSpellCheckerPosition; // for internal use
    MisspelledWordPositionInText: Integer;
    MisspelledWordPositionStart: IdxSpellCheckerPosition; // for internal use
    TextPositionFinish: IdxSpellCheckerPosition; // for internal use
    TextPositionStart: IdxSpellCheckerPosition; // for internal use
    Text: string;
  end;
  ...
  protected
    ...
    function InternalCheckWord(var AWord: string): Boolean; virtual;
  ...
  public
    ...
    property Controller: IdxSpellCheckTextController read FController; // for internal use
  ...
  public
    ...
    procedure DrawMisspellings(AControl: TWinControl; DC: HDC = 0); virtual; abstract;
  ...
  public
    ...
    property Rules: TdxSpellCheckerRules read FRules; // for internal use
...
uses
  ...
  cxContainer, dxSpellCheckerAutoCorrect, dxSpellCheckerBaseForm, dxXMLDoc, dxMessageDialog;

const
  dxThisUnitName = 'dxSpellChecker';
...
function TdxSpellCheckerSuggestionList.GetItem(
  Index: TdxListIndex): TdxSpellCheckerSuggestion;
begin
...
begin
  dxShowMessage(cxGetResourceString(@sdxSpellCheckerSpellingComplete));
end;
...
procedure TdxCustomSpellChecker.DrawMisspellings(AControl: TWinControl; DC: HDC = 0);
begin
  CheckAsYouTypeManager.DrawMisspellings(AControl, DC);
end;
...
begin
  ...
  begin
    if InternalProcessWord(AWord) then
      Continue;
    if not InternalCheckWord(AWord) then
      Exit(True);
    PrevWord := AWord;
...
function TdxSpellCheckerAbstractCheckMode.InternalCheckWord(var AWord: string): Boolean;
var
  AWordLength: Integer;
  AWordWithoutLastChar: string;
begin
  FLastCode := CheckWord(AWord);
  AWordLength := Length(AWord);

  // Remove the last non-alphanumeric symbol (.')
  if (LastCode <> scNoError) and (AWordLength >= 2) then
  begin
  ...
  if (LastCode = scMisspelled) and (AWordLength >= 1) then
  begin
...
function TdxSpellCheckerSuggestionCache.GetItem(Index: TdxListIndex): TdxSpellCheckerSuggestionCacheItem;
begin
...
function TdxSpellCheckerDialogCheckMode.ValidateSpellingBounds: Boolean;
  ...
  begin
    Result := dxMessageDlg(cxGetResourceString(@sdxSpellCheckerSelectionCheckIsFinished), mtInformation, [mbYes, mbNo], 0) = mrYes;
  end;
