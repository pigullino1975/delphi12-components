...
uses
  ...
  dxSpellCheckerTextParsers, cxGraphics, dxDPIAwareUtils;
...
type
  ...
  protected
    ...
    property IsMultiLine: Boolean read GetIsMultiLine;
    property MeasureDC: HDC read FMeasureDC;
...
uses
  dxCore, SysUtils, cxControls, cxContainer, cxEdit, Math, cxGeometry, dxDrawRichTextUtils, cxMemo, dxMessages, dxCoreGraphics;

const
  dxThisUnitName = 'dxSpellCheckerAdapters';
...
begin
  Result := cxGetWindowText(EditorHandle);
end;
...
procedure TdxSpellCheckerCustomEditAdapter.UpdateText;
begin
  if EditorHandleAllocated then
    FText := GetText;
end;
...
begin
  UpdateText;
  ATextController := AController as TdxSpellCheckerTextController;
...
destructor TdxSpellCheckerCxTextEditAdapter.Destroy;
begin
  DeleteDC(FMeasureDC);
  inherited Destroy;
end;
...
begin
  ...
  if AStartLineIndex = AFinishLineIndex then
    ACallback(GetUnderlineRect(AStartIndex, AFinishIndex - AStartIndex))
  else
    ...
    begin
      if I = AStartLineIndex then
        ...
        ACharIndex := GetFirstCharIndexInLine(I);

      if I = AFinishLineIndex then
        AWordPartLength := AFinishIndex - ACharIndex
      else
        AWordPartLength := GetLineLength(I) - ACharIndex + GetFirstCharIndexInLine(I);
...
begin
  ...
  GetTextExtentPointW(MeasureDC, @FText[APosition + 1], ALength, ATextSize);
  Result.Right := Result.Left + ATextSize.cx;
end;
...
var
  ATextSize: TSize;
begin
  if GetTextExtentPointW(MeasureDC, PWideChar(dxMeasurePattern), Length(dxMeasurePattern), ATextSize) then
    Result := ATextSize.cy
  else
    Result := -1;
end;
...
begin
  SelectObject(MeasureDC, SendMessage(EditorHandle, WM_GETFONT, 0, 0));
...
begin
  ...
  try
    if dxGetCurrentScaleFactor(Edit, ANumerator, ADenominator) then 
      AScaleFactor.Assign(ANumerator, ADenominator)
    else
      AScaleFactor.Assign(dxSystemScaleFactor);
    ARect := cxRectSetNullOrigin(TextAreaRect);
    ACharIndex := GetFirstCharIndexInLine(ALineIndex);
    Result := dxDrawRichEdit(MeasureDC, ARect, EditorHandle, AScaleFactor,
      ACharIndex, ACharIndex + GetLineLength(ALineIndex), True, not (IsMultiLine and Edit.WordWrap));
  finally
    AScaleFactor.Free;
  end;
end;
