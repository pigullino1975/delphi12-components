...
uses
  ...
  dxLayoutControlAdapters, dxLayoutControl, dxLayoutLookAndFeels, cxCustomListBox;
...
uses
  dxSpellCheckerStrs, dxSpellCheckerDialogs, dxSpellCheckerRules, dxMessageDialog;

const
  dxThisUnitName = 'dxSpellCheckerOutlookDialog';
...
begin
  ...
  begin
    if not btnSuggest.Enabled and not FHasSuggestions then
      if dxMessageDlg(cxGetResourceString(@sdxSpellCheckerConfirmUseUnknownWord), mtWarning, [mbYes, mbNo], 0) <> mrYes then
      begin
...
procedure TfmSpellCheckerOutlookForm.FormCreate(Sender: TObject);
begin
  teMisspelledWord.ParentColor := teMisspelledWord.Style.LookAndFeel.SkinPainter = nil;
end;
