...
begin
  ...
  Assert(AHighSurr.IsHighSurrogate);
  if Char.IsSurrogatePair(AHighSurr, ALowSurr) then
  begin
    ASurrogatePair := True;
...
begin
  ...
  begin
    ...
    begin
      ...
      ANumberOfBytes := TEncoding.UTF8.GetBytes(string(APStr), Succ(I), ACount, ABytes, 0); 
      if ANumberOfBytes = 0 then
        raise TdxUriFormatException.Create('net_uri_BadString');

      Inc(I, ACount - 1);

      for J := 0 to ANumberOfBytes - 1 do
        EscapeAsciiChar(Char(ABytes[J]), Result, ADestPos);
...
begin
  ...
  begin
    AIsHighSurr := AUnescapedChars[J].IsHighSurrogate;
...
begin
  Result := CharInSet(ACh, dxSpaceCharacters);
end;
