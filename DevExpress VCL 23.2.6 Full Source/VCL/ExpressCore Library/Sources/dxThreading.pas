...
type
  ...
  private
    ...
    FCompareDelegate: TdxListSortCompareDelegate;
    FCount: Integer;
  ...
  public
    ...
    procedure SortList(AList: PdxPointerList; ACount: Integer; const ACompareDelegate: TdxListSortCompareDelegate); overload;
    procedure SortList(AList: TdxFastList; const ACompareDelegate: TdxListSortCompareDelegate); overload;
  end;
...
{$REGION 'Tasks Dispatcher'}
  ...
  protected
    ...
    function GetCanceled: Boolean; virtual;
  public
  ...
  public
    ...
    procedure AddTask(const AProc: TThreadMethod); overload;
  ...
  {$REGION 'Protected Types'}
    ...
    TTaskProcWrapper = class(TdxTask)
    strict private
      FProc: TdxTaskProc;
    protected
      procedure Execute; override;
    ...
    strict private
      ...
      FCompleteSyncMode: TdxThreadMethodCallMode;
      FTask: IdxTask;
  ...
  public
    ...
    function Run(ATask: TdxTask; ACompleteEvent: TThreadMethod;
      ACompleteEventCallMode: TdxThreadMethodCallMode; ATaskAutoFree: Boolean = True): THandle; overload;
    function RunInCurrentThread(ATask: TdxTask; ATaskAutoFree: Boolean = True): THandle; overload;
    function RunInCurrentThread(const ATask: IdxTask): THandle; overload;
...
procedure dxCallThreadMethod(AMethod: TThreadMethod; AMode: TdxThreadMethodCallMode);
procedure dxCallThreadProcedure(AProcedure: TThreadProcedure; AMode: TdxThreadMethodCallMode);

function dxIsMainThread: Boolean;
function dxWaitForSyncObject(AHandle: THandle; ATimeOut: Cardinal): TWaitResult;
implementation

uses
  dxCore, Math, Forms, dxMessages, dxStringHelper;

const
  dxThisUnitName = 'dxThreading';
...
function dxWaitForSyncObject(AHandle: THandle; ATimeOut: Cardinal): TWaitResult;
const
  MaxWaitTime = TdxTaskDispatcher.CpuUsageMonitorUpdateInterval;
var
  ...
  begin
    ...
    begin
      ...
      case AWaitResult of
        ...
        WAIT_TIMEOUT:
          if ATimeOut >= MaxWaitTime then
          begin
            if dxTasksDispatcher <> nil then
              dxTasksDispatcher.UpdateCpuUsage;
          end;
      end;
...
procedure TdxMultithreadedSort.Sort(AList: TdxFastList; const ACompareFunc: TListSortCompare);
begin
  Sort(AList.List, AList.Count, ACompareFunc);
end;

procedure TdxMultithreadedSort.SortList(AList: PdxPointerList; ACount: Integer;
  const ACompareDelegate: TdxListSortCompareDelegate);
begin
  if Lock then
  try
    FCompareFunc := nil;
    FCompareClassFunc := nil;
    FCompareDelegate := ACompareDelegate;
    Initialize(ACount);
    SortListChunks(AList);
    MergeChunks(AList);
  finally
    ...
    dxInternalQuickSortList2(AList, ACount, ACompareDelegate);
end;

procedure TdxMultithreadedSort.SortList(AList: TdxFastList; const ACompareDelegate: TdxListSortCompareDelegate);
begin
  SortList(AList.List, AList.Count, ACompareDelegate);
end;
...
procedure MergeWithDelegate(AList, ATemp: PdxPointerList;
  ALowBound, ADivider, AHiBound: Integer; const ACompareDelegate: TdxListSortCompareDelegate);
var
  AIndexA, AIndexB, ATempSize, ACount: Integer;
  ADest, AItemA, AItemB, AHighA, AHighB: PPointer;
begin
  ...
  while (TdxNativeUInt(AItemA) <= TdxNativeUInt(AHighA)) and
    (TdxNativeUInt(AItemB) <= TdxNativeUInt(AHighB)) do
  begin
    if ACompareDelegate(AItemA^, AItemB^) < 0 then
    begin
      ADest^ := AItemA^;
      Inc(AItemA);
      Inc(AIndexA);
...
begin
  with AMergeWorkItem^ do
    ...
    else if Assigned(FCompareClassFunc) then
      Merge(List, Temp, StartIndex, MiddleIndex, EndIndex, FCompareClassFunc)
    else
      MergeWithDelegate(List, Temp, StartIndex, MiddleIndex, EndIndex, FCompareDelegate)
end;
...
begin
  with ASortWorkItem^ do
    if Assigned(FCompareFunc) then
      ...
      dxInternalQuickSortList2(@List[StartIndex], (EndIndex - StartIndex) + 1, FCompareDelegate)
end;
...
begin
  dxCallThreadMethod(FOnComplete, FOnCompleteMode);
end;
...
procedure TdxTask.ReleaseResources;
begin
  FEvent := nil;
end;
...
begin
  AddTask(TdxTaskDispatcher.TTaskProcWrapper.Create(AProc));
end;
...
procedure TdxTaskGroup.AddTask(const AProc: TThreadMethod);
begin
  AddTask(TdxTaskDispatcher.TProcWrapper.Create(AProc));
end;

procedure TdxTaskGroup.Cancel;
var
  I: Integer;
begin
  if TaskCount > 1 then
  begin
    for I := 0 to TaskCount - 1 do
      dxTasksDispatcher.Cancel(Tasks[I].Handle);
  end
  else
    if TaskCount > 0 then
      InterlockedExchange(Tasks[0].FCanceled, 1);
end;
...
begin
  ...
  begin
    ...
    for I := 0 to TaskCount - 1 do
      dxTasksDispatcher.Run(Tasks[I], CompleteHandler, tmcmAsync, False);
    dxWaitForSyncObject(FEvent.Handle, INFINITE);
  end
  else
    if TaskCount > 0 then
      Tasks[0].Execute;
...
begin
  Result := dxWaitForSyncObject(FHandle, ATimeOut);
end;
...
begin
  Result := Run(TTaskProcWrapper.Create(AProc));
end;
...
begin
  Result := Run(TTaskProcWrapper.Create(AProc), ACompleteEvent, ACompleteEventCallMode);
end;
...
function TdxTaskDispatcher.Run(ATask: TdxTask; ACompleteEvent: TThreadMethod;
  ACompleteEventCallMode: TdxThreadMethodCallMode; ATaskAutoFree: Boolean = True): THandle;
begin
  EnterCriticalSection(FLock);
  try
    ATask.FOnComplete := ACompleteEvent;
    ATask.FOnCompleteMode := ACompleteEventCallMode;
    ATask.FAutoFree := ATaskAutoFree;
    Result := ATask.Handle;
    FTasks.Add(ATask);
  finally
...
function TdxTaskDispatcher.RunInCurrentThread(ATask: TdxTask; ATaskAutoFree: Boolean = True): THandle;
begin
  Result := 0;
  try
    try
    ...
    if ATaskAutoFree then
      ATask.Free;
  end;
end;

function TdxTaskDispatcher.RunInCurrentThread(const ATask: IdxTask): THandle;
begin
  Result := RunInCurrentThread(TTaskWrapper.Create(ATask));
end;
...
function TdxTaskDispatcher.ToString: string;
var
  ABuffer: TStringBuilder;
begin
  ABuffer := TdxStringBuilderManager.Get(64);
  try
    ABuffer.Append('Tasks: ').Append(FActiveTasks.Count);
...
begin
  ...
  begin
    ...
    EnterCriticalSection(FLock);
    try
      ATask := TdxTask(ATaskHandle);
      AIndex := FTasks.IndexOf(ATask);
      if AIndex >= 0 then
      begin
        ATask.FCanceled := 1;
        ATask.Complete;
        APrevOwnsObjects := FTasks.OwnsObjects;
        try
          FTasks.OwnsObjects := ATask.FAutoFree;
          FTasks.Delete(AIndex)
        finally
          FTasks.OwnsObjects := APrevOwnsObjects;
        end;
        Exit(wrSignaled);
      end;
...
procedure TdxTaskDispatcher.UpdateCpuUsage;
begin
{$IFDEF DELPHI101BERLIN}
  if UseCpuUsageMonitor then
    HandlerCpuUsageMonitor(nil);
{$ENDIF}
end;
...
begin
  AEvent := ATask.FEvent;
  AFreeOnTerminate := ATask.FAutoFree;
  try
    ...
    try
      try
        ...
        ATask.ReleaseResources; // before complete
        ATask.Complete;
      end;
    except
...
begin
  ...
  FCpuUsageLogIndex := Min(FCpuUsageLogIndex + 1, CpuUsageMonitorLogSize);

  if FCpuUsageLogIndex >= CpuUsageMonitorLogSize then
  begin
    AAverageCpuUsage := 0;
    for I := Low(FCpuUsageLog) to High(FCpuUsageLog) do
      Inc(AAverageCpuUsage, FCpuUsageLog[I]);
    FAverageCpuUsage := AAverageCpuUsage div Length(FCpuUsageLog);
    ...
    else if FAverageCpuUsage >= CpuUsageTooHigh then
      Dec(ANumberOfActiveTasks)
    else if FAverageCpuUsage <= CpuUsageLow then
...
{ TdxTaskDispatcher.TProcWrapper }

constructor TdxTaskDispatcher.TProcWrapper.Create(AProc: TThreadMethod);
begin
  inherited Create;
  FProc := AProc;
end;
...
{ TdxTaskDispatcher.TTaskProcWrapper }

constructor TdxTaskDispatcher.TTaskProcWrapper.Create(AProc: TdxTaskProc);
begin
  inherited Create;
  FProc := AProc;
end;

procedure TdxTaskDispatcher.TTaskProcWrapper.Execute;
begin
  if Assigned(FProc) then
    FProc(GetCanceled);
end;
...
begin
  ...
  if dxIsMainThread then
    FCompleteSyncMode := tmcmSync
  else
    FCompleteSyncMode := tmcmAsync;
end;

procedure TdxTaskDispatcher.TTaskWrapper.Complete;
begin
  dxCallThreadMethod(SyncComplete, FCompleteSyncMode);
end;
...
begin
  try
    ...
    FCompletedStatus := TdxTaskCompletedStatus.Fail;
    raise;
  end;
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxUIThreadSyncService.Initialize', SysInit.HInstance);{$ENDIF}
  ...
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxUIThreadSyncService.Initialize', SysInit.HInstance);{$ENDIF}
end;

{$HINTS OFF}
class destructor TdxUIThreadSyncService.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxUIThreadSyncService.Finalize', SysInit.HInstance);{$ENDIF}
  FShutdown := True;
  DeleteAllListeners;
  DeallocateHWnd(FHandle);
  DeleteCriticalSection(FLock);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxUIThreadSyncService.Finalize', SysInit.HInstance);{$ENDIF}
end;
{$HINTS ON}
