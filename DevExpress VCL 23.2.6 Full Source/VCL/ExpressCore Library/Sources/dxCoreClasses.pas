...
type
  TdxComponentNotificationProc = procedure (AComponent: TComponent; AOperation: TOperation) of object;
  ...
  TcxLockableComponent = class(TComponent)
  strict private
    FHasChanges: Boolean;
    FLockCount: Integer;
  protected
  ...
  private
    ...
    function IsFreeNotifiesExists: Boolean;
  protected
    procedure Notification(AComponent: TComponent; Operation: TOperation); override;
    procedure Check; overload;
  public
    constructor Create(AMaster: TObject); reintroduce;
    procedure AddSender(ASender: TComponent);
    procedure RemoveSender(ASender: TComponent);
    class procedure Check(var AFreeNotificator: TcxFreeNotificator); overload;
  ...
  TdxListSortCompareDelegate = reference to function (Item1, Item2: Pointer): Integer;
  TdxListDirection = (ldFromBeginning, ldFromEnd);
  TdxStringCompareProc = function(const AValue1, AValue2: string): Integer;
  ...
  public
    ...
    procedure AddRange(AList: TdxFastList); inline;
    ...
    procedure Insert(AIndex: Integer; AList: TdxFastList); overload; deprecated;
...
{ TdxFastThreadList }
  ...
  TdxNullableByte = TdxNullableValue<Byte>;
  TdxNullableShortInt = TdxNullableValue<ShortInt>;
  TdxNullableSmallInt = TdxNullableValue<SmallInt>;
  TdxNullableWord = TdxNullableValue<Word>;
  TdxNullableInteger = TdxNullableValue<Integer>;
  TdxNullableInt64 = TdxNullableValue<Int64>;
  TdxNullableSingle = TdxNullableValue<Single>;
  TdxNullableDouble = TdxNullableValue<Double>;
  TdxNullableCurrency = TdxNullableValue<Currency>;
  TdxNullableDateTime = TdxNullableValue<TDateTime>;
  TdxNullableDate = TdxNullableValue<TDate>;
  TdxNullableTime = TdxNullableValue<TTime>;
  TdxNullableGUID = TdxNullableValue<TGUID>;
  TdxNullableRect = TdxNullableValue<TRect>;

  { TdxCustomComparer }

  TdxCustomComparer<T> = class(TInterfacedObject, IEqualityComparer<T>, IComparer<T>)
  public
    function Compare(const Left, Right: T): Integer; virtual; abstract;
    function Equals(const ALeft, ARight: T): Boolean; reintroduce; virtual; abstract;
    function GetHashCode(const AValue: T): Integer; reintroduce; virtual; abstract;
  end;

  { TdxStringComparer }

  TdxStringComparer = class
  private class var
    FOrdinal: TdxCustomComparer<string>;
  protected
    class procedure Finalize;
  public
    class function Ordinal: TdxCustomComparer<string>;
  end;

  { TdxIStringComparer }

  TdxIStringComparer = class
  private class var
    FOrdinal: TdxCustomComparer<string>;
  protected
    class procedure Finalize;
  ...
  { TdxOrdinalStringComparer }

  TdxOrdinalStringComparer = class(TdxCustomComparer<string>)
  public
  ...
  TdxOrdinalIStringComparer = class(TdxOrdinalStringComparer)
  public
  ...
  { TdxReadOnlySubStream }

  TdxReadOnlySubStream = class(TStream)
  strict private
    FOffset: Int64;
    FPosition: Int64;
    FSize: Int64;
  ...
  public
    constructor Create(ASource: TStream; const AOffset, ASize: Int64; ASourceOwnership: TStreamOwnership = soReference); reintroduce;
    destructor Destroy; override;
  ...
  { TdxHGlobalReadOnlyStream }

  TdxHGlobalReadOnlyStream = class(TCustomMemoryStream)
  strict private
    FData: THandle;
  ...
  TdxBufferedStream = class(TStream)
  strict private
    FBuffer: PByte;
    ...
    procedure BufferFinalize;
    procedure BufferInitialize(ABufferSize: Integer);
    procedure BufferNextBlock;
  protected
    function GetSize: Int64; override;
    procedure SetSize(const NewSize: Int64); override;
    //
    property SourceStream: TStream read FSource;
  public
    constructor Create(const ASource: TStream; AOwnership: TStreamOwnership; ABufferSize: Integer); overload;
    constructor Create(const AFileName: string; AAccessMode: Word; ABufferSize: Integer); overload;
    destructor Destroy; override;
    function Read(var Buffer; Count: Longint): Longint; override;
    function Seek(const Offset: Int64; Origin: TSeekOrigin): Int64; override;
    function Write(const Buffer; Count: Integer): Integer; override;
...
procedure dxInternalQuickSortList2(ASortList: PdxPointerList; ACount: Integer;
  const ACompareDelegate: TdxListSortCompareDelegate); // for internal use
...
const
  dxThisUnitName = 'dxCoreClasses';

const
  sdxErrorCannotModifyReadOnlyStream = 'You cannot modify read-only stream';
...
procedure InsertionSort(ASortList: PdxPointerList; ALast: Integer; const ACompareFunc: TdxListSortCompareDelegate); overload;
var
  I, J: Integer;
  ATemp: Pointer;
begin;
...
procedure ModifiedQuickSort(ASortList: PdxPointerList; ARight: Integer; const ACompareFunc: TdxListSortCompareDelegate); overload;
var
  I, J: Integer;
...
procedure dxInternalQuickSortList2(ASortList: PdxPointerList; ACount: Integer; const ACompareDelegate: TdxListSortCompareDelegate);
begin;
  if (ASortList <> nil) and (ACount > 1) then
  begin;
    Dec(ACount);
    if ACount > dxInsertionSortLast then
      ModifiedQuickSort(ASortList, ACount, ACompareDelegate);
    InsertionSort(ASortList, ACount, ACompareDelegate);
  end;
end;
...
function TcxLockablePersistent.IsLocked: Boolean;
begin
  Result := FLockCount <> 0;
end;
...
begin
  Dec(FLockCount);
  if not IsLocked and FHasChanges then
    Changed;
end;

{ TcxLockableComponent }

procedure TcxLockableComponent.Assign(Source: TPersistent);
begin
  BeginUpdate;
  try
...
procedure TcxLockableComponent.BeginUpdate;
begin
  Inc(FLockCount);
end;

procedure TcxLockableComponent.CancelUpdate;
begin
  Dec(FLockCount);
  if FLockCount = 0 then
    FHasChanges := False;
...
procedure TcxLockableComponent.Changed;
begin
  if FLockCount = 0 then
  begin
    FHasChanges := False;
...
procedure TcxLockableComponent.EndUpdate;
begin
  Dec(FLockCount);
...
constructor TcxFreeNotificator.Create(AMaster: TObject);
begin
  FMaster := AMaster;
  if AMaster is TComponent then
    inherited Create(TComponent(AMaster))
  else
...
class procedure TcxFreeNotificator.Check(var AFreeNotificator: TcxFreeNotificator);
begin
  if (AFreeNotificator <> nil) and not AFreeNotificator.IsFreeNotifiesExists then
    FreeAndNil(AFreeNotificator);
end;

function TcxFreeNotificator.IsFreeNotifiesExists: Boolean;
var
  AFreeNotifies: PPointer;
begin
  AFreeNotifies := ShiftPointer(@DesignInfo, -SizeOf(Pointer));
  Result := AFreeNotifies^ <> nil;
end;
...
procedure TcxFreeNotificator.Check;
var
  AMaster: IdxFreeNotify;
begin
  if Supports(FMaster, IdxFreeNotify, AMaster) and not IsFreeNotifiesExists then
    AMaster.DestroyFreeNotificator;
end;
...
begin
  ...
  ALinkedObject.Prev := nil;
  ALinkedObject.Next := nil;
end;
...
procedure TdxFastList.AddRange(AList: TdxFastList);
begin
  InsertRange(Count, AList);
end;
...
var
  ATempItem: Pointer;
begin
  if Cardinal(AIndex1) >= Cardinal(FCount) then
    TList.Error(@SListIndexError, AIndex1);
  if Cardinal(AIndex2) >= Cardinal(FCount) then
...
begin
  InsertRange(AIndex, AList);
end;

procedure TdxFastList.InsertRange(AIndex: Integer; AList: TdxFastList);
var
  ANewCount: Integer;
begin
  if Cardinal(AIndex) > Cardinal(FCount) then
...
begin
  ...
  begin
    if Cardinal(ANewIndex) >= Cardinal(Count) then
...
procedure TdxFastList.SortList(const ACompare: TdxListSortCompareDelegate; AMultiThreaded: Boolean = False);
begin
  if Count > 1 then
{$IFDEF MSWINDOWS}
    if AMultiThreaded and dxCanUseMultiThreading and (Count > dxMaxUserWorkItems) then
      dxSortHelper.SortList(List, Count, ACompare)
    else
{$ENDIF}
      dxInternalQuickSortList2(List, Count, ACompare);
end;
...
constructor TdxFastThreadList.Create;
begin
  inherited Create;
...
destructor TdxFastThreadList.Destroy;
begin
  LockList;
  try
    FList.Free;
...
procedure TdxFastThreadList.Add(Item: Pointer);
begin
  LockList;
  try
    FList.Add(Item)
...
procedure TdxFastThreadList.Clear;
begin
  LockList;
  try
    FList.Clear;
...
function TdxFastThreadList.LockList: TdxFastList;
begin
  TMonitor.Enter(FLock);
  Result := FList;
end;

procedure TdxFastThreadList.Remove(Item: Pointer);
begin
  LockList;
  try
    FList.Remove(Item);
...
procedure TdxFastThreadList.UnlockList;
begin
  TMonitor.Exit(FLock);
end;

function TdxFastThreadList.GetCount;
begin
  LockList;
  try
    Result := FList.Count;
...
class function TdxStringComparer.Ordinal: TdxCustomComparer<string>;
begin
...
class function TdxIStringComparer.Ordinal: TdxCustomComparer<string>;
begin
  if FOrdinal = nil then
  begin
...
begin
  Result := inherited Compare(AnsiLowerCase(Left), AnsiLowerCase(Right));
end;

function TdxOrdinalIStringComparer.Equals(const ALeft, ARight: string): Boolean;
begin
  Result := (Length(ALeft) = Length(ARight)) and inherited Equals(AnsiLowerCase(ALeft), AnsiLowerCase(ARight));
end;

function TdxOrdinalIStringComparer.GetHashCode(const AValue: string): Integer;
begin
  Result := inherited GetHashCode(AnsiLowerCase(AValue));
end;
...
{$IFDEF DELPHI110}
function TdxMemoryStream.Realloc(var NewCapacity: NativeInt): Pointer;
{$ELSE}
function TdxMemoryStream.Realloc(var NewCapacity: Longint): Pointer;
{$ENDIF}
const
  MemoryDelta = $4000; 
begin
...
constructor TdxReadOnlySubStream.Create(ASource: TStream; const AOffset, ASize: Int64; ASourceOwnership: TStreamOwnership = soReference);
var
  ARealSize: Int64;
begin
  FSource := ASource;
  FSourceOwnership := ASourceOwnership;
  ARealSize := FSource.Size;
  FOffset := Min(AOffset, ARealSize);
  FSize := Min(ASize, ARealSize - FOffset);
end;

destructor TdxReadOnlySubStream.Destroy;
begin
...
function TdxReadOnlySubStream.GetSize: Int64;
begin
  Result := FSize;
end;

function TdxReadOnlySubStream.Read(var Buffer; Count: Longint): Longint;
begin
  TMonitor.Enter(FSource);
  try
    FSource.Position := FPosition + FOffset;
    Result := FSource.Read(Buffer, Min(Count, FSize - FPosition));
    if Result > 0 then
      Inc(FPosition, Result);
  finally
    TMonitor.Exit(FSource);
  end;
end;

function TdxReadOnlySubStream.Seek(const Offset: Int64; Origin: TSeekOrigin): Int64;
begin
  case Origin of
    soCurrent:
  ...
  Result := EnsureRange(Result, 0, Size);
  FPosition := Result;
end;

procedure TdxReadOnlySubStream.SetSize(const NewSize: Int64);
begin
  raise EInvalidOperation.Create(sdxErrorCannotModifyReadOnlyStream);
end;

function TdxReadOnlySubStream.Write(const Buffer; Count: Longint): Longint;
begin
  raise EInvalidOperation.Create(sdxErrorCannotModifyReadOnlyStream);
end;
...
constructor TdxHGlobalReadOnlyStream.Create(AData: THandle; ADataOwnership: TStreamOwnership);
begin
  FData := AData;
...
destructor TdxHGlobalReadOnlyStream.Destroy;
begin
  GlobalUnlock(FData);
...
procedure TdxHGlobalReadOnlyStream.SetSize(const NewSize: Int64);
begin
  raise EInvalidOperation.Create(sdxErrorCannotModifyReadOnlyStream);
end;

function TdxHGlobalReadOnlyStream.Write(const Buffer; Count: Integer): Integer;
begin
  raise EInvalidOperation.Create(sdxErrorCannotModifyReadOnlyStream);
end;
...
constructor TdxBufferedStream.Create(const ASource: TStream; AOwnership: TStreamOwnership; ABufferSize: Integer);
begin
  inherited Create;
  FSource := ASource;
  FSourceOwnership := AOwnership;
  BufferInitialize(ABufferSize);
end;

constructor TdxBufferedStream.Create(const AFileName: string; AAccessMode: Word; ABufferSize: Integer);
begin
  Create(TFileStream.Create(AFileName, AAccessMode), soOwned, ABufferSize);
end;

destructor TdxBufferedStream.Destroy;
begin
  BufferFinalize;
  if FSourceOwnership = soOwned then
    FreeAndNil(FSource);
...
function TdxBufferedStream.Read(Buffer: TBytes; Offset, Count: Longint): Longint;
begin
  Result := Read(Buffer[Offset], Count);
end;

function TdxBufferedStream.Read(var Buffer; Count: Longint): Longint;
  ...
  begin
    while ACount > 0 do
    begin
      if (FBufferPosition < FBufferAllocationBase) or (FBufferPosition >= FBufferUsed + FBufferAllocationBase) then
      begin
        BufferNextBlock;
      ...
      ASize := Min(ACount, FBufferUsed - AOffset);
      Move(PByteArray(FBuffer)^[AOffset], ADest^, ASize);
...
function TdxBufferedStream.Write(const Buffer; Count: Integer): Integer;
  ...
  begin
    while ACount > 0 do
    begin
      if (FBufferPosition < FBufferAllocationBase) or (FBufferPosition >= FBufferSize + FBufferAllocationBase) then
        BufferNextBlock;
      AOffset := FBufferPosition - FBufferAllocationBase;
      ASize := Min(ACount, FBufferSize - AOffset);
      Move(ASource^, PByteArray(FBuffer)^[AOffset], ASize);
      FBufferUsed := Max(FBufferUsed, AOffset + ASize);
...
function TdxBufferedStream.Seek(const Offset: Int64; Origin: TSeekOrigin): Int64;
begin
  case Origin of
    soCurrent:
  ...
  FBufferPosition := Min(Max(Result, 0), Size);
  Result := FBufferPosition;
end;

function TdxBufferedStream.Write(const Buffer: TBytes; Offset, Count: Longint): Longint;
begin
  Result := Write(Buffer[Offset], Count);
end;

function TdxBufferedStream.GetSize: Int64;
begin
  Result := FSource.Size;
  if FBufferModified then
    Result := Max(Result, FBufferAllocationBase + FBufferUsed);
end;

procedure TdxBufferedStream.SetSize(const NewSize: Int64);
begin
  FSource.Size := NewSize;
end;

procedure TdxBufferedStream.BufferCheckSaveChanges;
begin
  if FBufferModified then
  begin
    FBufferModified := False;
    FSource.Seek(FBufferAllocationBase, soBeginning);
    FSource.WriteBuffer(FBuffer^, FBufferUsed);
  end;
end;

procedure TdxBufferedStream.BufferFinalize;
begin
...
procedure TdxBufferedStream.BufferInitialize(ABufferSize: Integer);
begin
  FBufferUsed := 0;
...
procedure TdxBufferedStream.BufferNextBlock;
begin
  BufferCheckSaveChanges;
  FBufferAllocationBase := FBufferPosition;
  FSource.Seek(FBufferAllocationBase, soBeginning);
  FBufferUsed := FSource.Read(FBuffer^, FBufferSize);
  FBufferModified := False;
end;

{ TdxRectList }

procedure TdxRectList.Offset(X, Y: Integer);
var
  I: Integer;
begin
  for I := 1 to Count - 1 do
    Items[I] := cxRectOffset(Items[I], X, Y);
end;
