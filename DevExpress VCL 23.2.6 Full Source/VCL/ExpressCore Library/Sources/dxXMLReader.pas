...
uses
  Classes, SysUtils, Generics.Defaults, Generics.Collections, dxCore, dxCoreClasses, dxStringHelper, dxGenerics, dxXMLClasses;
...
type
  ...
  public
    ...
    function GetAttributeAsDefaultBoolean(const AAttribute: string): TdxDefaultBoolean;
...
uses
  Math, StrUtils, Windows, dxXMLDoc;

const
  dxThisUnitName = 'dxXMLReader';
...
type
  ...
  strict private
    ...
    FNameTable: TdxXmlNameTable;
    FScopeId: Integer;
    FHashTable: TdxStringIntegerDictionary;
...
{$ENDREGION}
  private
    ...
    function GetAttributeWithoutPrefix(const AName: string): TdxNodeData;
    function GetAttributeWithNamespace(const AName, ANamespaceURI: string): TdxNodeData;
    function GetAttributeWithPrefix(const ALocalName, APrefix: string): TdxNodeData; overload;
    function GetAttributeWithPrefix(const AName: string): TdxNodeData; overload;
...
begin
  ...
  begin
    ...
    begin
      while (AByteCount > 0) and (UnicodeFromLocaleChars(TMBCSEncoding(Self).CodePage, MB_ERR_INVALID_CHARS, PAnsiChar(@ABytes[AByteIndex]), AByteCount, nil, 0) = 0) do
        Dec(AByteCount);
    end;
...
begin
  ...
  if FUseHashTable then
    ...
    begin
      ...
      FHashTable := TdxStringIntegerDictionary.Create(FLastDecl);
...
function TdxXmlReader.GetActualValue: string;
begin
  Result := Value;
  if XmlSpace <> TdxXmlSpace.Preserve then
...
function TdxXmlReader.GetAttribute(const AAttribute: AnsiString): string;
begin
  Result := GetAttribute(dxAnsiStringToString(AAttribute));
end;

function TdxXmlReader.GetAttribute(const AAttribute: string): string;
begin
  if not TryGetAttribute(AAttribute, Result) then
    Result := EmptyStr;
end;

function TdxXmlReader.GetAttributeAsBoolean(const AAttribute: AnsiString; const ADefaultValue: Boolean = False): Boolean;
begin
  Result := GetAttributeAsBoolean(dxAnsiStringToString(AAttribute), ADefaultValue);
end;

function TdxXmlReader.GetAttribute(const APrefix, ALocalName, ANamespaceURI: string): string;
begin
  if not TryGetAttribute(APrefix, ALocalName, ANamespaceURI, Result) then
    Result := EmptyStr;
end;

function TdxXmlReader.GetAttributeAsBoolean(const AAttribute: string; const ADefaultValue: Boolean = False): Boolean;
var
  AValue: string;
begin
  if TryGetAttribute(AAttribute, AValue) then
    Result := TdxXMLHelper.DecodeBoolean(AValue)
  else
    Result := ADefaultValue;
end;

function TdxXmlReader.GetAttributeAsDefaultBoolean(const AAttribute: string): TdxDefaultBoolean;
var
  AValue: string;
begin
  if TryGetAttribute(AAttribute, AValue) then
    Result := dxBooleanToDefaultBoolean(TdxXMLHelper.DecodeBoolean(AValue))
  else
    Result := bDefault;
end;

function TdxXmlReader.GetAttributeAsInt64(const AAttribute: string; const ADefaultValue: Int64 = 0): Int64;
begin
  Result := StrToInt64Def(GetAttribute(AAttribute), ADefaultValue);
end;

function TdxXmlReader.GetAttributeAsInteger(const AAttribute: string; ADefaultValue: Integer = 0): Integer;
begin
  Result := StrToIntDef(GetAttribute(AAttribute), ADefaultValue);
end;

function TdxXmlReader.GetAttributeAsSingle(const AAttribute: string; ADefaultValue: Single = 0): Single;
begin
  Result := StrToFloatDef(GetAttribute(AAttribute), ADefaultValue, dxInvariantFormatSettings);
end;
...
var
  AOther: TdxNodeData;
begin
  if Safe.Cast(AObject, TdxNodeData, AOther) then
  begin
...
function TdxXmlTextReaderImpl.TryGetAttribute(const AAttribute: string; out AValue: string): Boolean;
var
  AAttrData: TdxNodeData;
begin
  if TdxStringHelper.IndexOf(AAttribute, ':') < 0 then
...
function TdxXmlTextReaderImpl.TryGetAttribute(const APrefix, ALocalName, ANamespaceURI: string; out AValue: string): Boolean;
var
  AAttrData: TdxNodeData;
begin
  if ANamespaceURI <> '' then
...
function TdxXmlTextReaderImpl.GetProgress: Integer;
begin
...
function TdxXmlTextReaderImpl.GetAttributeWithoutPrefix(const AName: string): TdxNodeData;
begin
  Result := GetAttributeWithPrefix(AName, '');
end;

function TdxXmlTextReaderImpl.GetAttributeWithPrefix(const AName: string): TdxNodeData;
var
  ANameValue: string;
  ANode: TdxNodeData;
  I: Integer;
...
function TdxXmlTextReaderImpl.GetAttributeWithNamespace(const AName, ANamespaceURI: string): TdxNodeData;
var
  ALocalName: string;
  ANamespace: string;
  ANode: TdxNodeData;
...
function TdxXmlTextReaderImpl.GetAttributeWithPrefix(const ALocalName, APrefix: string): TdxNodeData;
var
  ANameValue: string;
  ANode: TdxNodeData;
  APrefixValue: string;
...
begin
  ...
  repeat
    ...
    if APosition = FParsingState.CharsUsed then
      goto LblReadData;
    if (((TdxXmlCharType.CharProperties[AChars[APosition]] and TdxXmlCharType.NCNameSC) <> 0) or (AChars[APosition] = ':')) then
      ThrowTagMismatch(AStartTagNode);

    if AChars[APosition] <> '>' then
    repeat
      ATmpCh := AChars[APosition];
      if not TdxXmlCharType.IsWhiteSpace(ATmpCh) then
        Break;
      Inc(APosition);
      case ATmpCh of
        #$000A:
