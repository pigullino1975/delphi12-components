...
type
  ...
  protected
    ...
    procedure ReleaseItem(AItem: TdxDynamicListItem); virtual;
  ...
  TdxHashTableRow = class(TdxDynamicItemList)
  protected
    procedure ReleaseItem(Item: TdxDynamicListItem); override;
  end;
  ...
  protected
    FCount: Integer;
    FTable: array of TdxHashTableRow;
  ...
  strict private
    ...
    procedure SetHashTable(AValue: TdxHashTable); inline;
  protected
    ...
    procedure CalculateHash; overload;
    procedure CalculateHash(var AHash: Integer); overload; virtual; abstract;
    function DoIsEqual(const AItem: TdxHashTableItem): Boolean; virtual; abstract;

    property HashTable: TdxHashTable read FHashTable write SetHashTable;
  public
    destructor Destroy; override;
    procedure AddRef;
  ...
  { TdxHashTableItemList }

  TdxHashTableItemList = class(TList)
  protected
    procedure Notify(Ptr: Pointer; Action: TListNotification); override;
  public
    function FirstOrNil: Pointer;
  end;

function dxChangeHandle(var ACurrentValue; ANewValue: TdxHashTableItem): Boolean;
implementation

const
  dxThisUnitName = 'dxHashUtils';

function dxChangeHandle(var ACurrentValue; ANewValue: TdxHashTableItem): Boolean;
var
  APrevValue: TdxHashTableItem;
begin
  Result := TdxHashTableItem(ACurrentValue) <> ANewValue;
  if Result then
  begin
    APrevValue := TdxHashTableItem(ACurrentValue);
    TdxHashTableItem(ACurrentValue) := ANewValue;
    if TdxHashTableItem(ACurrentValue) <> nil then
      TdxHashTableItem(ACurrentValue).AddRef;
...
procedure TdxDynamicItemList.ReleaseItem(AItem: TdxDynamicListItem);
begin
  AItem.Free;
end;
...
var
  ...
  AItems: TdxHashTableRow;
begin
  AItemObject := TdxHashTableItem(AItem);
  if AItemObject.Index = 0 then
    AItemObject.CalculateHash;

  AIndex := AItemObject.Key mod dxHashTableSize;
  AItems := FTable[AIndex];
  if AItems = nil then
  begin
    AItems := TdxHashTableRow.Create;
  ...
  TdxHashTableItem(AItem).HashTable := Self;
end;
...
procedure TdxHashTableItem.SetHashTable(AValue: TdxHashTable);
begin
  if FHashTable <> AValue then
  begin
    if (FHashTable <> nil) and (AValue <> nil) then
      raise EInvalidOperation.Create('Hash table is already set');

    FHashTable := AValue;
    if (FHashTable <> nil) and (FRefCount > 0) then
      Inc(FHashTable.FCount, FRefCount);
  end;
end;

{ TdxHashTableRow }

procedure TdxHashTableRow.ReleaseItem(Item: TdxDynamicListItem);
var
  AItem: TdxHashTableItem absolute Item;
begin
...
function TdxHashTableItemList.FirstOrNil: Pointer;
begin
  if Count > 0 then
    Result := List[0]
  else
...
procedure TdxHashTableItemList.Notify(Ptr: Pointer; Action: TListNotification);
begin
  if Action = lnAdded then
    TdxHashTableItem(Ptr).AddRef
  else
    if Action = lnDeleted then
      TdxHashTableItem(Ptr).Release;
end;
