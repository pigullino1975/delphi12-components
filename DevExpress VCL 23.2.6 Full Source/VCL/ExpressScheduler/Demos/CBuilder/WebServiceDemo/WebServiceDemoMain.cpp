...
TWebServiceDemoMainForm *WebServiceDemoMainForm;
	...
	#define REG_KEY "Software\\Microsoft\\Internet Explorer\\Main\\FeatureControl\\FEATURE_BROWSER_EMULATION"
#else
	#define REG_KEY "SOFTWARE\\Wow6432Node\\Microsoft\\Internet Explorer\\Main\\FeatureControl\\FEATURE_BROWSER_EMULATION"
#endif
//---------------------------------------------------------------------------
		...
		: TDemoBasicMainForm(Owner)
{
  ...
  if (ARegistry->OpenKey(REG_KEY, true))
...
{
	if (AProviderClass == __classid(TcxSchedulerWebServiceStorageGoogleProvider))
		return __classid(TdxGoogleAPIOAuth2AuthorizationAgent);
	else
		return __classid(TdxMicrosoftGraphAPIOAuth2AuthorizationAgent);
}
//---------------------------------------------------------------------------
