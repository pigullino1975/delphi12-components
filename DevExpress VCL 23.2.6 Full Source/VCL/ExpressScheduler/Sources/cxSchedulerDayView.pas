...
type
  ...
  TcxSchedulerAllDayContainerGestureScrollHelper = class(TcxIUnknownObject, IdxGestureClient, IdxGestureClient2)
  private
  ...
  strict private
    FAboveOfTimeLineRect: TRect;
    FBelowOfTimeLineRect: TRect;
  private
    FRealBoundsLeft: Integer;
    FRealBorderColor: TColor;
    FTimeLineShift: Integer;
  protected
    ...
    function GetTimeLineShift: Integer; override;
    function GetViewStyle: TcxSchedulerViewStyle; override;
    function IsTimeLineVisible: Boolean; override;
  ...
  protected
    ...
    procedure AddMoreEventsButton(AColumnIndex: Integer; const AContainerBounds: TRect; const ARowIndex: Integer; AIsTop: Boolean; AEvent: TcxSchedulerEvent);
    ...
    procedure CheckEventRowsVisibility(AColumnIndex: Integer; const AContainerBounds: TRect; AEvent: TcxSchedulerControlEvent; AStartRow, AFinishRow: Integer);
    procedure Clear; override;
    procedure ClearEventCells; override;
    procedure ClearHeaderEventCells;
    ...
    procedure DoIntradayEventCellsVerticalScroll(APositionCount: Integer);
    ...
    function IsHeaderEventCell(ACell: TcxSchedulerEventCellViewInfo): Boolean;
  ...
  protected
    ...
    property TimeLineShift: Integer read FTimeLineShift;
  public
  ...
  TcxCalculateEventLayout = class(TdxFastList)
  private
    FDate: TDateTime;
    FLeft: Integer;
    FRowHeight: Integer;
...
const
  dxThisUnitName = 'cxSchedulerDayView';
...
type
  ...
  TcxCustomResourceViewAdapterAccess = class(TcxCustomResourceViewAdapter);
  TcxSchedulerTimeRulerCellAccess = class(TcxSchedulerTimeRulerCellViewInfo);

  { TcxSchedulerDayViewEventCellContainerInfo }

  TcxSchedulerDayViewEventCellContainerInfo = class
  public
    ContainerIndex: Integer;
    FinishRow: Integer;
    StartRow: Integer;

    constructor Create(AContainerIndex: Integer; AInfo: TcxEventLayoutInfo);
  end;
...
constructor TcxSchedulerDayViewEventCellContainerInfo.Create(AContainerIndex: Integer; AInfo: TcxEventLayoutInfo);
begin
  ContainerIndex := AContainerIndex;
  StartRow := AInfo.StartRow;
  FinishRow := AInfo.StopRow + 1;
end;
...
begin
  ...
  if (StartRow = StopRow) and (Abs(Finish - Int(Event.Start + 1)) <= 1/24/60/60) then
  begin
    FTimeLineShift := ATop - StartRow * ARowHeight;
    FDisplayRect := cxRectOffsetVert(FDisplayRect, -FTimeLineShift);
  end;
end;

function TcxEventModernLayoutInfo.GetFinishCoordinate(AScale: Integer): Integer;
begin
  Result := Round(dxTimeOf(Max(Event.Finish, Min(dxDateOf(Event.Start + 1), Event.Start + AScale / cxMinutesPerDay))) /
    MinuteToTime);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ViewInfo.DoIntradayEventCellsVerticalScroll(ViewInfo.FTopIndex - DH);
...
procedure TcxSchedulerDayView.UnInitScrollbars;
begin
  FAllDayScrollBar.UnInitControl;
end;
...
constructor TcxSchedulerAllDayContainerGestureScrollHelper.Create(AView: TcxSchedulerDayView);
begin
  inherited Create;
  FView := AView;
end;
...
function TcxSchedulerAllDayContainerGestureScrollHelper.UseRightToLeftScrollbar: Boolean;
begin
  Result := FView.Scheduler.UseRightToLeftScrollBar;
end;
...
function TcxSchedulerDayViewEventCellModernViewInfo.GetTimeLineShift: Integer;
begin
  Result := FTimeLineShift;
end;

procedure TcxSchedulerDayViewEventCellModernViewInfo.DrawSelection;
const
  ABorders: array[Boolean] of TcxBorders = ([bLeft, bTop, bBottom], [bTop, bRight, bBottom]);
var
  R, R1: TRect;
  ARgn: TcxRegion;
begin
  ...
  try
    ...
    try
      ...
      if not cxRectIsNull(FAboveOfTimeLineRect) then
        ARgn.Combine(FAboveOfTimeLineRect, roSubtract);
      if not cxRectIsNull(FBelowOfTimeLineRect) then
        ARgn.Combine(FBelowOfTimeLineRect, roSubtract);
      ...
      R1 := FTimeLineRect;         
      R1.Top := Max(R1.Top, SelectionBorderRect.Top + SelectionBorderSize);
      R1.Bottom := Min(R1.Bottom, SelectionBorderRect.Bottom - SelectionBorderSize);
      if not UseRightToLeftAlignment then
      begin
        R.Left := Bounds.Left - (SelectionBorderSize - 1);
        R1.Left := R1.Right - SelectionBorderSize - 1;
      end
      else
      begin
        R.Right := Bounds.Right + (SelectionBorderSize - 1);
        R1.Right := R1.Left + SelectionBorderSize + 1;
      end;
      ARgn.Combine(R1, roSubtract);
      Canvas.SetClipRegion(ARgn, roSet, False);
      Canvas.FrameRect(R, GetSelectedBorderColor, SelectionBorderSize);

      R1 := FTimeLineRect;         
      R1.Top := Max(SelectionBorderRect.Top, R1.Top - SelectionBorderSize);
      R1.Bottom := Min(SelectionBorderRect.Bottom, R1.Bottom + SelectionBorderSize);
      if not UseRightToLeftAlignment then
      begin
        R1.Left := SelectionBorderRect.Left;
        R1.Right := Bounds.Left;
      end
      else
      begin
        R1.Left := Bounds.Right;
        R1.Right := SelectionBorderRect.Right;
      end;
      Canvas.FrameRect(R1, GetSelectedBorderColor, SelectionBorderSize, ABorders[UseRightToLeftAlignment]);
    finally
      ARgn.Free;
    end;
  finally
...
begin
  ...
  FAboveOfTimeLineRect := cxNullRect;
  FBelowOfTimeLineRect := cxNullRect;
  if Selected then
  begin
    R := SelectionBorderRect;
    if not UseRightToLeftAlignment then
      R.Right := TimeLineRect.Right - SelectionBorderSize
    else
      R.Left := TimeLineRect.Left + SelectionBorderSize;

    if TimeLineRect.Bottom + Integer(not ActualBorderColorIsNone) < Bounds.Bottom then
    begin
      FBelowOfTimeLineRect := R;
      FBelowOfTimeLineRect.Top := TimeLineRect.Bottom + Integer(not ActualBorderColorIsNone) * SelectionBorderSize;
      AClipRgn.Combine(FBelowOfTimeLineRect, roSubtract);
    end;

    if TimeLineRect.Top - Integer(not ActualBorderColorIsNone) > Bounds.Top then
    begin
      FAboveOfTimeLineRect := R;
      FAboveOfTimeLineRect.Bottom := TimeLineRect.Top - Integer(not ActualBorderColorIsNone) * SelectionBorderSize;
      AClipRgn.Combine(FAboveOfTimeLineRect, roSubtract);
    end;
  end;
end;
...
var
  APositionCount, AAllDayScrollPos: Integer;
begin
  if (TcxCustomSchedulerAccess(Scheduler).MouseWheelScrollingKind <> mwskNone) and
     NeedAllDayScrollBar and cxRectPtIn(FAllDayAreaBounds, View.ScreenToClient(GetMouseCursorPos)) then
  begin
    AAllDayScrollPos := FAllDayTopIndex;
    AllDayScroll(AScrollCode, AAllDayScrollPos);
  end
  else
  begin
    ...
    begin
      DayView.LockEventLayoutCalculating;
      try
        APositionCount := FTopIndex - AScrollPos;
        FTopIndex := AScrollPos;
        DoIntradayEventCellsVerticalScroll(APositionCount);
        DayView.LayoutChanged;
        AScrollPos := FTopIndex;
      finally
        DayView.UnlockEventLayoutCalculating;
      end;
    end;
  end;
end;
...
procedure TcxSchedulerDayViewViewInfo.AddMoreEventsButton(AColumnIndex: Integer;
  const AContainerBounds: TRect;
  const ARowIndex: Integer; AIsTop: Boolean; AEvent: TcxSchedulerEvent);

  function GetButton: TcxSchedulerMoreEventsButtonViewInfo;
  var
    I: Integer;
  begin
    for I := 0 to FButtons.Count - 1 do
      if FButtons[I] is TcxSchedulerMoreEventsButtonViewInfo then
      begin
        Result := TcxSchedulerMoreEventsButtonViewInfo(FButtons[I]);
        if Result.IsDown = not AIsTop then
        begin
          if (ViewStyle = TcxSchedulerViewStyle.svsClassic) or (AColumnIndex = Result.ColumnIndex) then
            Exit;
        end;
      end;
    Result := nil;
...
begin
  ...
  AButton := GetButton;
  if AButton = nil then
    AddButton(AColumnIndex, R, ARowIndex / FRowCount, not AIsTop, AEvent)
  else
  begin
    ADateTime := ARowIndex / FRowCount;
    if AIsTop then
    begin
      if AButton.DateTime > ADateTime  then
      begin
        AButton.DateTime := ADateTime;
        AButton.Event := AEvent;
      end;
    end
    else
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        R := AColumnRect;
        CalculateNavigationButtonHeight(R);
        if not IsNavigationButtonsCollapsed then
        begin
          Y := cxHalfCoordinate(R.Bottom + R.Top);
          Inc(R.Top, FHeaderContainerContentOffset);
          if Y - NavigationButtonHeight div 2 < R.Top then
          begin
            R.Bottom := R.Top + NavigationButtonHeight;
            CalculateNavigationButtonHeight(R);
          end;
        end;
      end;
...
begin
  ...
  begin
    ...
    CheckEventRowsVisibility(AIndex, AContainer.Bounds, AInfo.Event, AInfo.StartRow, AInfo.StopRow + 1);
    ...
    if ViewStyle = svsClassic then
      AEventCell := AddEventCell(AViewData)
    else
    begin
      AEventCell := AddEventCell(AViewData, False);
     (AEventCell as TcxSchedulerDayViewEventCellModernViewInfo).FTimeLineHeight := (AInfo as TcxEventModernLayoutInfo).RealHeight;
     (AEventCell as TcxSchedulerDayViewEventCellModernViewInfo).FTimeLineShift := (AInfo as TcxEventModernLayoutInfo).TimeLineShift;
      TcxSchedulerEventCellViewInfoAccess(AEventCell).Calculate;
    end;
    TcxSchedulerEventCellViewInfoAccess(AEventCell).FContainerInfo := TcxSchedulerDayViewEventCellContainerInfo.Create(AIndex, AInfo);
  end;
end;
...
var
  ...
  ACommonCell, ARulerCell, AFirstRulerCell: TcxSchedulerTimeRulerCellViewInfo;
  ABiasDelta, AAdditionalMinutesOffset: LongInt;

  function AddRulerCell(ARegularHour, ADisplayedHour: Integer): TcxSchedulerTimeRulerCellViewInfo;
  begin
    CreateCellInstance(TimeRulerCellClass,
      cxRectSetTop(R, R.Bottom * ARegularHour - AOffset), ABounds, AViewParams, UseRightToLeftAlignment, Result);
    Result.ShowMinutes := DayView.TimeRulerMinutes;
    TcxSchedulerTimeRulerCellAccess(Result).FAdditionalTimeZoneMinutesOffset := AAdditionalMinutesOffset;
    Result.Calculate(ADisplayedHour, LinePerHour, FCurrentTimeZone, FAdditionalTimeZone, LargeFont,
      FAdditionalTimeZoneBiasDelta, SelectedDays);
    Result.LastVisibleHour := not IsHourVisible(ARegularHour + 1);
    if AAdditionalMinutesOffset <> 0 then
    begin
      TcxSchedulerTimeRulerCellAccess(Result).FBounds[False] :=
        cxRectOffsetVert(Result.Bounds[False], Round(cxRectHeight(Result.Bounds[False]) * AAdditionalMinutesOffset/60));
      TcxSchedulerTimeRulerCellAccess(Result).CheckActiveClipping;
    end;
    FTimeRulerCells.Add(Result);
  end;
...
begin
  ...
  Dec(R.Left, Byte(UseRightToLeftAlignment));
  R.Right := FHeaderOffsetLeft;
  ...
  ABiasDelta := IfThen(FAdditionalTimeZone >= 0,
    TdxTimeZoneHelper.TimeZoneInfo(FCurrentTimeZone).TZI.Bias - TdxTimeZoneHelper.TimeZoneInfo(FAdditionalTimeZone).TZI.Bias, 0);
  if ABiasDelta mod 60 = 0 then
    AAdditionalMinutesOffset := 0
  else
    if ABiasDelta < 0 then
      AAdditionalMinutesOffset := IfThen(ABiasDelta mod 60 <> 0, -(60 - Abs(ABiasDelta) mod 60), 0)
    else
      AAdditionalMinutesOffset := IfThen(ABiasDelta mod 60 <> 0, -Abs(ABiasDelta) mod 60, 0);
  AFirstRulerCell := nil;
  AHour := 0;
  I := 0;
  while I < FDayRowCount do
  begin
    if (AAdditionalMinutesOffset <> 0) or IsRowVisible(I) then
    begin
      AHour := I div LinePerHour;
      ARulerCell := AddRulerCell(AHour, AHour);
      if (AAdditionalMinutesOffset <> 0) and (I = 0) then
        AFirstRulerCell := ARulerCell;
      I := (AHour + 1) * LinePerHour;
    end
    else
      Inc(I);
  end;
  if AFirstRulerCell <> nil then
    AddRulerCell(AHour + 1, AFirstRulerCell.Hour);
end;
...
procedure TcxSchedulerDayViewViewInfo.ClearEventCells;
begin
  if IsEventCellLayoutCalculatingLocked then
    ClearHeaderEventCells
  else
    inherited ClearEventCells;
end;

procedure TcxSchedulerDayViewViewInfo.ClearHeaderEventCells;
var
  I: Integer;
begin
  for I := EventCells.Count - 1 downto 0 do
    if IsHeaderEventCell(EventCells[I]) then
      EventCells.Delete(I);
end;
...
procedure TcxSchedulerDayViewViewInfo.CheckEventRowsVisibility(AColumnIndex: Integer; const AContainerBounds: TRect;
  AEvent: TcxSchedulerControlEvent; AStartRow, AFinishRow: Integer);
begin
  if Adapter.IsPrinting then Exit;
  if AStartRow < FTopIndex then
    AddMoreEventsButton(AColumnIndex, AContainerBounds, AStartRow, True, AEvent);
  if (AFinishRow > (FTopIndex + VisibleRowCount)) then
  begin
    if AFinishRow <= FRowCount then
      AddMoreEventsButton(AColumnIndex, AContainerBounds, AFinishRow, False, AEvent)
    else
      if AStartRow > (FTopIndex + VisibleRowCount) then
        AddMoreEventsButton(AColumnIndex, AContainerBounds, AStartRow, False, AEvent);
  end;
end;
...
procedure TcxSchedulerDayViewViewInfo.DoIntradayEventCellsVerticalScroll(APositionCount: Integer);
var
  I: Integer;
  AEventCell: TcxSchedulerEventCellViewInfoAccess;
begin
  for I := 0 to EventCells.Count - 1 do
  begin
    AEventCell := TcxSchedulerEventCellViewInfoAccess(EventCells[I]);
    if not IsHeaderEventCell(AEventCell) then
      AEventCell.Scroll(0, APositionCount * ContentLineHeight);
  end;
end;
...
function TcxSchedulerDayViewViewInfo.IsHeaderEventCell(ACell: TcxSchedulerEventCellViewInfo): Boolean;
begin
  Result := not DayView.ShowAllDayEventsInContentArea and ACell.Event.IsAllDayOrLonger;
end;
...
procedure TcxSchedulerDayViewViewInfo.CustomProcessPreviouslyCalculatedEvent(AEventCell: TcxSchedulerEventCellViewInfo);
var
  AContainerInfo: TcxSchedulerDayViewEventCellContainerInfo;
  AContainerBounds: TRect;
begin
  AContainerInfo :=
    TcxSchedulerDayViewEventCellContainerInfo((TcxSchedulerEventCellViewInfoAccess(AEventCell)).FContainerInfo);
  AContainerBounds := Containers[AContainerInfo.ContainerIndex].Bounds;
  CheckEventRowsVisibility(AContainerInfo.ContainerIndex, AContainerBounds, AEventCell.Event,
    AContainerInfo.StartRow, AContainerInfo.FinishRow);
...
procedure TcxSchedulerDayViewViewInfo.RecreateBuilder;
begin
  Builder.Free;
  Builder := TcxSchedulerEventLayoutBuilder.Create(ViewStyle);
end;
...
begin
  ...
  begin
    {$IFDEF DELPHIXE8}
    FLargeFont.Height := AFont.Height * 2
    {$ELSE}
  ...
  begin
    {$IFDEF DELPHIXE8}
    FLargeFont.Height := (AFont.Height * 3) div 2;
    {$ELSE}
...
begin
  ...
  APositionCount := ATopIndex - ViewInfo.TopIndex;
  if APositionCount <> 0 then
    ViewInfo.DoIntradayEventCellsVerticalScroll(APositionCount);
...
begin
  if IsResourceNavigation(AGoToNext, AColumn, ATime) then
  begin
    DayView.UnlockEventLayoutCalculating;
...
begin
  ...
  begin
    ...
    begin
      ...
      ViewInfo.FTopIndex := Min(ViewInfo.FTopIndex + ViewInfo.VisibleRowCount, ViewInfo.FRowCount - 1 - ViewInfo.VisibleRowCount)
    end
    else
    begin
...
begin
  CreateNewEventItems(rpmiNewEvent in FItems, rpmiNewAllDayEvent in FItems,
    rpmiNewReccuringEvent in FItems, Ord(rpmiNewEvent), Ord(rpmiNewAllDayEvent), Ord(rpmiNewReccuringEvent)); 
