...
uses
  ...
  cxContainer, cxLookAndFeelPainters, cxDateUtils, cxSchedulerCustomControls, cxSchedulerCustomResourceView, dxGenerics,
  cxSchedulerUtils, cxSchedulerStorage, cxSchedulerStrs, cxEdit, dxTouch, dxCoreClasses, cxSchedulerDayView, dxTypeHelpers;
...
type
  ...
  protected
    ...
    procedure SelectTopInfoCell(ANeedRefresh: Boolean);
  ...
  private
    ...
    procedure SetShowMessages(AValue: Boolean);
  ...
  protected
    ...
    function GetLastAvailableDate: TDateTime; override;
    ...
    procedure InitEventBySelectedTime(AEvent: TcxSchedulerEvent; AllDay: Boolean;
      ARecurrence: Boolean; AInplaceEditing: Boolean); override;
    procedure InitScrollBarsParameters; override;
    function IsAllDaySelection: Boolean; override;
  ...
  published
    ...
    property ShowMessages: Boolean read FShowMessages write SetShowMessages default True;
  ...
  public
    procedure GetPageResources(AResources: TdxFastObjectList); override;
  end;
  ...
  protected
    ...
    procedure OffsetRects(const DX, DY: Integer); override;
  ...
  protected
    ...
    function GetLastVisibleCell: TcxSchedulerCustomViewInfoItem;
...
uses
  RTLConsts,
  dxCoreGraphics, cxDrawTextUtils;

const
  dxThisUnitName = 'cxSchedulerAgendaView';
...
begin
  ...
  ARect.Offset(0, AHalfArea - AHalfRect);
end;
...
begin
  ...
  begin
    ...
    AHeaderHeight := ViewInfo.GetDayHeader(ACell).Bounds.Height;
...
procedure TcxSchedulerAgendaViewController.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
  ...
  function IsRightClickOnContent: Boolean;
  begin
    Result := (Button = mbRight) and HitTest.HitAtContent;
  end;
  ...
  begin
    ...
    if IsRightClickOnContent then
    begin
      if FreeDaysSelection.IndexOf(ViewInfo.GetCellDate(HitTest.ContentCell)) < 0 then
      begin
        ClearFreeDaysSelection([]);
    ...
    begin
      AIsFreeDay := True;
      FKeepEventsSelection := not IsRightClickOnContent and (ssShift in Shift);
  ...
  begin
    if IsRightClickOnContent then
      SelectFreeDayCell(ADate, [ssShift], True)
    else
  ...
  function CreateEventDates: TList<TDate>;
  var
    I, ADay, ADay1, ADay2: Integer;
  begin
    Result := TList<TDate>.Create;
    for I := 0 to EventSelection.Count - 1 do
    begin
      ADay1 := Trunc(EventSelection[I].Start);
      ADay2 := Trunc(EventSelection[I].Finish);
      if EventSelection[I].Finish - ADay2 < 1e-7  then
        Dec(ADay2);
      for ADay := ADay1 to ADay2 do
        if Result.IndexOf(ADay) < 0 then
          Result.Add(ADay);
    end;
  end;

  function CreateJointRange(ARange1, ARange2: TList<TDate>): TdxIntegerList;
  var
    I, AValue: Integer;
  begin
    Result := TdxIntegerList.Create;
    for I := 0 to ARange1.Count - 1 do
      Result.Add(Trunc(ARange1[I]));
    for I := 0 to ARange2.Count - 1 do
    begin
      AValue := Trunc(ARange2[I]);
      if Result.IndexOf(AValue) < 0 then
        Result.Add(AValue);
    end;
  end;

  procedure ExpandSelectionRange(var AStart, AFinish: Integer; AJointRange: TdxIntegerList);
  var
    I: Integer;
  begin
    if AJointRange.IndexOf(AStart) < 0 then
      AJointRange.Add(AStart);
    if (AFinish <> AStart) and (AJointRange.IndexOf(AFinish) < 0) then
      AJointRange.Add(AFinish);
    AJointRange.Sort;

    for I := AJointRange.IndexOf(AStart) - 1 downto 0 do
      if AStart - AJointRange[I] = 1 then
        Dec(AStart)
      else
        Break;
    for I := AJointRange.IndexOf(AFinish) + 1 to AJointRange.Count - 1 do
      if AJointRange[I] - AFinish = 1 then
        Inc(AFinish)
      else
        Break;
  end;

  procedure RemoveOutOfRangeEventsSelection(AStart, AFinish: Integer);
  var
    I: Integer;
    ANeedRefresh: Boolean;
  begin
    ANeedRefresh := False;
    for I := EventSelection.Count - 1 downto 0 do
      if not InRange(Trunc(EventSelection[I].Start), AStart, AFinish) then
      begin
        EventSelection.Add(EventSelection[I], [ssCtrl]);
        ANeedRefresh := True;
      end;
    if ANeedRefresh then
    begin
      AgendaView.LockEventLayoutCalculating;
      AgendaView.Refresh;
      AgendaView.UnlockEventLayoutCalculating;
    end;
  end;

  procedure CheckSelectionParams(const ADate: TDateTime);
  var
    AStart, AFinish: Integer;
    AEventDates: TList<TDate>;
    AJointRange: TdxIntegerList;
  begin
    AStart := Trunc(ADate);
    AFinish := Trunc(ADate);
    FreeDaysSelection.Sort;
    if FreeDaysSelection.Count > 0 then
    begin
      AStart := Min(AStart, Trunc(FreeDaysSelection[0]));
      AFinish := Max(AFinish, Trunc(FreeDaysSelection[FreeDaysSelection.Count - 1]));
    end;

    AEventDates := CreateEventDates;
    try
      AJointRange := CreateJointRange(FreeDaysSelection, AEventDates);
      try
        ExpandSelectionRange(AStart, AFinish, AJointRange);
        TcxCustomSchedulerAccess(Scheduler).ReplaceSelParams(AStart, AFinish, Scheduler.SelResource);
      finally
        AJointRange.Free;
      end;
      RemoveOutOfRangeEventsSelection(AStart, AFinish);
    finally
      AEventDates.Free;
    end;
  end;
...
var
  ADate: TDateTime;
  AIsFreeDay, AIsInfoCell, ANeedUpdate, ANeedUnselect: Boolean;
...
begin
  ...
  begin
    if FreeDaysSelection.Count > 0 then
      CheckSelectionParams(FreeDaysSelection[0])
    else
      if EventSelection.Count > 0 then
        CheckSelectionParams(Trunc(EventSelection[0].Start));
    ADate := HitTest.Time;
    ANeedUpdate := (FFocusedCell is TcxSchedulerAgendaViewFreeDayCellViewInfo) and
...
procedure TcxSchedulerAgendaViewController.SelectTopInfoCell(ANeedRefresh: Boolean);
var
  ACell: TcxSchedulerCustomViewInfoItem;
begin
  ACell := ViewInfo.GetTopVisibleCell;
  SelectCell(ACell, [], ANeedRefresh);
  FocusedCell := ACell;
end;
...
begin
  ...
  TcxCustomSchedulerAccess(Scheduler).ReplaceSelParams(ADate, ADate, Scheduler.SelResource);
end;
...
begin
  ...
  begin
    ...
    FShowMessages := TcxSchedulerAgendaView(Source).ShowMessages;
...
function TcxSchedulerAgendaView.GetLastAvailableDate: TDateTime;
begin
...
function TcxSchedulerAgendaView.GetLastVisibleDate: TDateTime;
var
  ACell: TcxSchedulerCustomViewInfoItem;
begin
  Result := GetFirstVisibleDate;
  ACell := ViewInfo.GetLastVisibleCell;
  if ACell <> nil then
    if ACell is TcxSchedulerAgendaViewFreeDayCellViewInfo then
      Result := TcxSchedulerAgendaViewFreeDayCellViewInfo(ACell).DayHeader.DateTime
    else
      Result := Int(TcxSchedulerAgendaViewEventCellViewInfo(ACell).FDateTime);
end;
...
procedure TcxSchedulerAgendaView.InitEventBySelectedTime(AEvent: TcxSchedulerEvent; AllDay: Boolean;
...
begin
  ...
  AEvent.AllDayEvent := AllDay or IsAllDaySelection;
  if not AEvent.AllDayEvent then
  begin
    AStart := AStart + cxTime8AM;
    AFinish := AStart + cxHalfHour;
  end;
  AEvent.Start := AStart;
...
function TcxSchedulerAgendaView.IsAllDaySelection: Boolean;
begin
  Result := dxDateOf(Scheduler.SelFinish - Scheduler.SelStart) > 1;
end;
...
function TcxSchedulerAgendaView.IsShowResources: Boolean;
begin
...
procedure TcxSchedulerAgendaView.SetShowMessages(AValue: Boolean);
begin
  if AValue <> FShowMessages then
  begin
...
var
  ...
  AMax, DY: Integer;
begin
  FTextRect.Deflate(ScaleFactor.Apply(dxDPITextOffset), 0);
  ...
  begin
    AlignVert := vaTop;     
    FTextRect.Deflate(ScaleFactor.Apply(dxDPITextOffset), 0);
    FTextRect.Offset(0, ScaleFactor.Apply(dxDPIDoubleTextOffset));
    FYearTextRect := TextRect;
    ...
    FTextRect.Offset(0, FBounds.Top - FTextRect.Top);
    ...
    FDayNameRect := FYearTextRect;
    FDayNameRect.Offset(0, AgendaViewInfo.DayHeaderLineHeight + ScaleFactor.Apply(dxDPITextOffset));
  ...
  begin
    ...
    FTextRect.Offset(0, DY);
    FYearTextRect.Offset(0, DY);
...
begin
  Result := (Orientation = dhoHorizontal) and
    (cxTextWidth(AgendaViewInfo.GetCurrentFont, FDisplayText) < TextRect.Width);
end;

function TcxSchedulerAgendaViewDayHeaderCellViewInfo.ConvertDateToDisplayText(AType: Integer = 0): Integer;
var
  ADisplayTextWithoutYear: string;
  AYearPosition: Integer;
begin
  ...
  ADisplayTextWithoutYear := FDisplayText;
  AYearPosition := Pos(FYearText, FDisplayText);
  if AYearPosition > 0 then
    Delete(ADisplayTextWithoutYear, AYearPosition, Length(FYearText));
  FDayNum := IntToStr(DayOf(DateTime));
  if Pos('0' + FDayNum, ADisplayTextWithoutYear) > 0 then
    FDayNum := '0' + FDayNum;
  FMonthName := TcxSchedulerDateTimeHelper.MonthToStrNominative(DateTime);
  if AYearPosition = 0 then
...
begin
  if AgendaView.DayHeaderOrientation = dhoVertical then
  begin
    dH := DayHeader.Bounds.Height - Bounds.Height;
...
procedure TcxSchedulerAgendaViewAdapter.GetPageResources(AResources: TdxFastObjectList);
var
  ACount, I: Integer;
begin
...
begin
  FEventTimeRect := AgendaViewInfo.EventCellStateRectPattern;
  FEventTimeRect.Top := Bounds.Top + 1;
  FEventTimeRect.Bottom := Bounds.Bottom - 2;
  FTimeLineRect := FEventTimeRect;
  FTimeLineRect.Offset(1, 0);
end;
...
begin
  ...
  if FLabelRect.Bottom < FTimeAreaRect.Bottom then
    cxRectCenterVerticallyRelativelyOfArea(FLabelRect, FTimeAreaRect);
end;
...
begin
  ...
  begin
    FLocationRect := AgendaViewInfo.EventCellLocationRectPattern;
    FLocationRect.Top := FTimeAreaRect.Top;
    FLocationRect.Bottom := FTimeAreaRect.Bottom;
  end;
end;
...
procedure TcxSchedulerAgendaViewEventCellViewInfo.CalculateTextItemsLayout;
  ...
  begin
    Result.Init(Bounds.Left, Bounds.Top + cxEventBorderWidth,
      (Bounds.Left + Bounds.Right - ScaleFactor.Apply(cxTextOffset)) div 2,
...
begin
  ...
  begin
    ...
    FResourceRect.Bottom := FTimeAreaRect.Bottom;
  end;
...
begin
  AProperties.InitValues(0, -1, AMessageRect.Width);
  ASize := AEditViewData.GetEditSize(Canvas, Message, AProperties);
  AMessageRect.Bottom := AMessageRect.Top + ASize.Height;
end;
...
begin
  ...
  FTimeAreaRect := AgendaViewInfo.EventCellTimeAreaPattern;
  FTimeAreaRect.Top := FContentTop;
  FTimeAreaRect.Bottom := FContentTop + Min(AgendaViewInfo.FEventCellBoundsPattern.Height, cxTextHeight(Canvas.Font));
  if ViewData.ShowTimeAsClock then
...
begin
  AClockSize.Width := TcxSchedulerPainterHelper.IconsWidth(ScaleFactor);
  AClockSize.Height := TcxSchedulerPainterHelper.IconsHeight(ScaleFactor);
  ...
  case FTimeAreaArrows of
    avtaNone:
    begin
      FStartRect.InitSize(FTimeAreaRect.TopLeft, AClockSize);
      FFinishRect := FStartRect;
      FFinishRect.Offset(FStartRect.Right - FStartRect.Left + 2 * ScaleFactor.Apply(cxTextOffset), 0);
    ...
    avtaContinueArrowStart:
    begin
      FStartContinueArrowRect.InitSize(FTimeAreaRect.TopLeft, ASizeStart);
      FFinishRect.InitSize(FTimeAreaRect.TopLeft, AClockSize);
      FFinishRect.Offset(FStartContinueArrowRect.Right - FStartContinueArrowRect.Left + 2 * ScaleFactor.Apply(cxTextOffset), 0);
    ...
    begin
      FStartRect.InitSize(FTimeAreaRect.TopLeft, AClockSize);
      FEndContinueArrowRect.InitSize(FStartRect.TopLeft, ASizeEnd);
      FEndContinueArrowRect.Offset(FStartRect.Right - FStartRect.Left + 2 * ScaleFactor.Apply(cxTextOffset), 0);
      cxRectCenterVerticallyRelativelyOfArea(FStartRect, FTimeAreaRect);
    ...
    begin
      FStartContinueArrowRect.InitSize(FTimeAreaRect.TopLeft, ASizeStart);
      FStartRect.InitSize(FTimeAreaRect.TopLeft, AClockSize);
      FStartRect.Offset(FStartContinueArrowRect.Right - FStartContinueArrowRect.Left + 2 * ScaleFactor.Apply(cxTextOffset), 0);
      FEndContinueArrowRect.InitSize(FStartRect.TopLeft, ASizeEnd);
      FEndContinueArrowRect.Offset(FStartRect.Right - FStartRect.Left + 2 * ScaleFactor.Apply(cxTextOffset), 0);
      cxRectCenterVerticallyRelativelyOfArea(FStartRect, FTimeAreaRect);
      cxRectCenterVerticallyRelativelyOfArea(FStartContinueArrowRect, FTimeAreaRect);
...
begin
  ...
  case FTimeAreaArrows of
    avtaContinueArrowsBoth:
    begin
      FStartContinueArrowRect.InitSize(FTimeTextRect.TopLeft, ASizeStart);
      ...
      FEndContinueArrowRect.InitSize(FTimeTextRect.Right, FTimeTextRect.Top, ASizeEnd);
      cxRectCenterVerticallyRelativelyOfArea(FStartContinueArrowRect, FTimeAreaRect);
      cxRectCenterVerticallyRelativelyOfArea(FEndContinueArrowRect, FTimeAreaRect);
    end;
    avtaContinueArrowStart:
    begin
      FStartContinueArrowRect.InitSize(FTimeTextRect.TopLeft, ASizeStart);
      FTimeTextRect.Left := FStartContinueArrowRect.Right + ScaleFactor.Apply(dxDPITextOffset);
      FTimeTextRect.Width := cxTextWidth(Canvas.Font, FStartText) + ScaleFactor.Apply(dxDPITextOffset);
      cxRectCenterVerticallyRelativelyOfArea(FStartContinueArrowRect, FTimeAreaRect);
    end;
    avtaContinueArrowEnd:
    begin
      FTimeTextRect.Width := cxTextWidth(Canvas.Font, FStartText) + ScaleFactor.Apply(dxDPIDoubleTextOffset);
      FEndContinueArrowRect.InitSize(FTimeTextRect.Right, FTimeTextRect.Top, ASizeEnd);
      cxRectCenterVerticallyRelativelyOfArea(FEndContinueArrowRect, FTimeAreaRect);
    end;
  end;
...
begin
  ...
  if IsResourcePresent then
    Result := TcxSchedulerControlEventAccess(Event).GetResourcesDisplayNames;
end;
...
begin
  ...
  begin
    dH := AgendaView.Scheduler.OptionsView.EventHeight - Bounds.Height;
    Inc(FClipRect.Bottom, dH);
    Inc(FEventTimeRect.Bottom, dH);
    Inc(FBounds.Bottom, dH);
    FCaptionRect.Bottom := FBounds.Bottom;
  end;
  ViewData.Bounds.Bottom := Bounds.Bottom;
end;
...
begin
  if AgendaView.ShowLocations and (Event.Location <> '') and (LocationRect.Width > 0) then
...
var
  ...
  AStyle: TcxStyle;
begin
  if IsResourcePresent and (FResourceRect.Width > 0) then
  begin
...
procedure TcxSchedulerAgendaViewEventCellViewInfo.OffsetRects(const DX, DY: Integer);
begin
  inherited OffsetRects(DX, DY);
...
begin
  ...
  if DayHeaderOrientation = dhoHorizontal then
    Result.FVisibleRect.Top := ADayHeader.Bounds.Height;
  Result.Calculate;
...
begin
  ...
  begin
    ...
    AViewData.AutoHeight := AgendaView.ShowMessages;
  ...
  if AIsOnlyEventInDay and (DayHeaderOrientation = dhoVertical) then
    ABounds.Bottom := ABounds.Top + ADayHeader.Bounds.Height;
  AViewData := CreateEventViewData(AEvent, cxRectOffsetVert(ABounds, FNextTop),
    GetEventStart(AEvent, ADate), GetEventFinish(AEvent, ADate), nil, ADate);
  AViewData.AutoHeight := AgendaView.ShowMessages;
  AViewData.BorderColor := LookAndFeelPainter.DefaultGridLineColor;
  if DayHeaderOrientation = dhoHorizontal then
    AViewData.VisibleRect.Top := ADayHeader.Bounds.Height;
...
begin
  FDayHeaderBoundsPattern.InitSize(Bounds.Left, 0, Bounds.Width, DayHeaderDefaultHeight);
...
begin
  FEventCellBoundsPattern.InitSize(Bounds.Left, 0, Bounds.Width, CellDefaultHeight);
  if DayHeaderOrientation = dhoVertical then
...
begin
  FEventCellStateRectPattern := FEventCellBoundsPattern;
  FEventCellStateRectPattern.Width := ScaleFactor.Apply(cxTimeLineWidth) - ScaleFactor.Apply(2);
end;
...
begin
  ...
  AWidth := Max(TcxSchedulerPainterHelper.IconsWidth(ScaleFactor) * 3 + ScaleFactor.Apply(dxDPIFourfoldTextOffset),
    ...
    [APattern, APattern])) + ScaleFactor.Apply(dxDPIDoubleTextOffset), AllDayCaptionWidth + ASize1.Width + ASize2.Width));
  FEventCellTimeAreaPattern.InitSize(EventCellStateRectPattern.Right + ScaleFactor.Apply(5), FEventCellBoundsPattern.Top,
    AWidth, FEventCellLineHeight);
end;
...
begin
  ...
  FEventCellLabelRectPattern.InitSize(EventCellTimeAreaPattern.Right + ScaleFactor.Apply(10), FEventCellBoundsPattern.Top, ASize);
  if ASize.Height < FEventCellTimeAreaPattern.Height then
    FEventCellLabelRectPattern.Offset(0, FEventCellTimeAreaPattern.Height div 2 - ASize.Height div 2);
end;
...
begin
  AOffsetHorz := ScaleFactor.Apply(dxDPITripleTextOffset);
  FEventCellCaptionRectPattern.Init(EventCellLabelRectPattern.Right + AOffsetHorz * 2, FEventCellBoundsPattern.Top,
    FEventCellBoundsPattern.Right, FEventCellTimeAreaPattern.Bottom);
  ...
  begin
    ...
    if AgendaView.ShowLocations then
    begin
      FEventCellLocationRectPattern.Init(ALeft2, FEventCellBoundsPattern.Top, ALeft2 + ASmallWidth, FEventCellTimeAreaPattern.Bottom);
    ...
    if AgendaView.ShowResources then
    begin
      FEventCellResourceRectPattern.Init(ALeft3, FEventCellBoundsPattern.Top, FEventCellBoundsPattern.Right, FEventCellTimeAreaPattern.Bottom);
...
begin
  ...
  begin
    ...
    if DayHeaderOrientation = dhoHorizontal then
      Inc(Result, GetDayHeader(ABegCell).Bounds.Height);
  end;
end;
...
function TcxSchedulerAgendaViewViewInfo.GetLastVisibleCell: TcxSchedulerCustomViewInfoItem;
var
  I: Integer;
  ACell: TcxSchedulerCustomViewInfoItem;
begin
  Result := nil;
  for I := InfoCells.Count - 1 downto 0 do
  begin
    ACell := InfoCell[I];
...
begin
  ...
  begin
    AVisibleTop := 0;
    if DayHeaderOrientation = dhoHorizontal then
      AVisibleTop := GetDayHeader(ACell).Bounds.Height;
...
begin
  ...
  if DayHeaderOrientation = dhoHorizontal then
  begin
    AHeaderHeight := GetDayHeader(ACell).Bounds.Height;
...
begin
  ...
  ADayContentBounds := TopVisibleDayHeader.Bounds;
  ADayContentBounds.Height := GetDayContentHeight(ADate);
  AHeaderHeight := TopVisibleDayHeader.Bounds.Height;
...
begin
  ...
  begin
    ...
    AStart := ADate - AgendaView.GetVisibleDaysRange;
    AFinish := ADate + 3 * AgendaView.GetVisibleDaysRange;
    if Events.Count > 0 then
    begin
      AStart := Max(Events.Start, AStart);
      AFinish := Min(Events.Finish, AFinish);
    end;
...
begin
  ...
  begin
    ...
    AHeader.DoScroll(0, -(AHeader.Bounds.Top + AHeader.Bounds.Height - TopVisibleDayHeader.Bounds.Top));
  end;
end;
