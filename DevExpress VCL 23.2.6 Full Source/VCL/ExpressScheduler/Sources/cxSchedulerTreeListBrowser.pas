...
uses
  ...
  Contnrs, SysUtils, Types, RTLConsts, cxInplaceContainer, dxCore, cxControls, cxGraphics, cxGeometry,
  ...
  cxScrollBar, cxSchedulerCustomResourceView, cxSchedulerTimeGridView, cxTL, cxTLData,
  cxCustomData, cxVariants, dxMessages, dxCoreClasses, dxCoreGraphics;
...
type
  ...
  protected
    ...
    procedure CorrectIndicatorWidth;
...
const
  dxThisUnitName = 'cxSchedulerTreeListBrowser';

type
  TcxSchedulerControlEventAccess = class(TcxSchedulerControlEvent);
  TcxSchedulerAccess = class(TcxCustomScheduler);
...
procedure TcxSchedulerTreeListBrowser.ChangeScaleEx(M, D: Integer; isDpiChange: Boolean);
begin
  inherited;
  CorrectIndicatorWidth;
end;

procedure TcxSchedulerTreeListBrowser.CorrectIndicatorWidth;
begin
  if Painter.Painter.ApplyEditorAdvancedMode then
    OptionsView.IndicatorWidth := Max(dxGetImageSize(cxIndicatorImages, ScaleFactor).Width,
      cxTextWidth(Font, '99 ') + cxMarginsWidth(Painter.Painter.HeaderContentOffsets(ScaleFactor)))
  else
    OptionsView.IndicatorWidth := ScaleFactor.Apply(20);
end;
...
function TcxSchedulerTreeListBrowser.GetMouseWheelScrollingKind: TcxMouseWheelScrollingKind;
begin
  if IsVerticalScrollBarVisible then
    Result := mwskVertical
  else
...
begin
  ...
  begin
    AViewInfo.Painter.DrawScaledIndicatorItem(ACanvas, AViewInfo.BoundsRect, AViewInfo.Kind,
      AViewInfo.ViewParams.Color, ScaleFactor, AViewInfo.IsRightToLeftConverted, TcxTreeListIndicatorAccess(AViewInfo).DrawBackgroundHandler);
...
procedure TcxSchedulerTreeListBrowser.FontChanged;
begin
  inherited;
  CorrectIndicatorWidth;
end;
...
function TcxSchedulerTreeListBrowser.IsMouseWheelHandleNeeded(Shift: TShiftState; WheelDelta: Integer; MousePos: TPoint): Boolean;
begin
  Result := (MouseWheelScrollingKind = mwskVertical) and IsVerticalScrollBarVisible or
    (MouseWheelScrollingKind = mwskHorizontal) and IsScrollBarActive(sbHorizontal) ;
end;

function TcxSchedulerTreeListBrowser.IsVerticalScrollBarVisible: Boolean;
begin
  Result := (TcxSchedulerViewAccess(View).ViewInfo.ResourceViewShiftList.Count > 0) or
    TcxSchedulerAccess(TcxSchedulerViewAccess(View).Scheduler).VScrollBarVisible;
end;
...
procedure TcxSchedulerTreeListBrowser.SetStyle(AValue: TcxShedulerTreeBrowserStyle); 
begin
