...
uses
  ...
  Controls, Menus, Types, DateUtils, Generics.Collections, Generics.Defaults,
  dxCore, dxCoreClasses, cxClasses, cxControls, cxGraphics, cxStyles, cxGeometry, cxLookAndFeelPainters,
...
type
  ...
  published
    ...
    property MonthHeaderPopupMenu: TcxSchedulerMonthHeaderPopupMenu read FMonthHeaderPopupMenu write SetMonthHeaderPopupMenu; // for internal use
...
uses
  cxFormats, dxCoreGraphics, cxDrawTextUtils;

const
  dxThisUnitName = 'cxSchedulerYearView';
...
begin
  for I := APlace.ColStart to APlace.ColFinish do
    ...
    begin
      AddButton(I, DaysCells[I].Bounds, DaysCells[I].TimeStart, True, APlace.Event);
...
begin
  ...
  ABuilder := TcxSchedulerEventLayoutBuilder.Create(TcxSchedulerViewStyle.svsClassic); 
  try
...
procedure TcxSchedulerYearViewViewInfo.ClearButtons;
var
  I: Integer;
begin
...
procedure TcxSchedulerYearViewViewInfo.ClearEventCells;
begin
  if not IsEventCellLayoutCalculatingLocked then
...
procedure TcxSchedulerYearViewViewInfo.CustomProcessPreviouslyCalculatedEvent(AEventCell: TcxSchedulerEventCellViewInfo);
begin
...
begin
  ...
  {$IFDEF DELPHIXE8}
  FContentSmallFont.Height := Round(FContentFont.Height * 2 / 3);
  {$ELSE}
...
function TcxSchedulerYearViewNavigation.ContentCell(
  AIndex: Integer): TcxSchedulerYearViewContentCellViewInfo;
begin
  Result := TcxSchedulerYearViewContentCellViewInfo(ViewInfo.ContentCells[AIndex]);
end;
...
function TcxSchedulerYearViewContentCellViewInfo.GetTextLeftForSmallFont: Integer;
begin
  Result := Max(FTextRect.Left,
    FTextRect.Right - PainterHelper.TextWidth(FSmallTextFont, DisplayText)) - 2 * ScaleFactor.Apply(cxTextOffset);
end;
...
function TcxSchedulerYearView.InternalMouseWheel(Shift: TShiftState; WheelDelta: Integer; MousePos: TPoint): Boolean;
begin
  Result := True;
  if WheelDelta < 0 then
    Scheduler.ScrollContent(dirDown)
  else
    if WheelDelta > 0 then
      Scheduler.ScrollContent(dirUp)
  else
    Result := False;
end;
...
begin
  CreateNewEventItems(mhpmiNewEvent in FItems, mhpmiNewAllDayEvent in FItems,
    mhpmiNewReccuringEvent in FItems, Ord(mhpmiNewEvent), Ord(mhpmiNewAllDayEvent), Ord(mhpmiNewReccuringEvent)); 
