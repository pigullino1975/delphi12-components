...
type
  ...
  protected
    ...
    function AddDayCellEventPlace(ABuilder: TcxSchedulerEventLayoutBuilder; AEvent: TcxSchedulerControlEvent;
      var AIndex: Integer; AStartDay: TcxSchedulerContentCellViewInfo): Boolean;
    procedure AddMoreEventsButton(AColumnIndex: Integer; const AContainerBounds: TRect; const ADateTime: TDateTime;
      AIsTop: Boolean; AEvent: TcxSchedulerEvent);
    ...
    procedure CalculateEventsViewInfo(ABuilder: TcxSchedulerEventLayoutBuilder); virtual;
    ...
    procedure CheckDaysTimeEvent(AEventInfo: TcxSchedulerEventCellViewInfo; AColStart, AColFinish: Integer);
    ...
    function IsCellEvent(AEvent: TcxSchedulerControlEvent; ACell: TcxSchedulerContentCellViewInfo): Boolean;
    function IsClassicIntradayEventCell(AEvent: TcxSchedulerControlEvent): Boolean; inline;
    function IsSingleLine(ADay1, ADay2: TcxSchedulerContentCellViewInfo): Boolean;
    function IsTransparentEventCell(AEvent: TcxSchedulerControlEvent): Boolean; inline;
...
uses
  dxCoreGraphics, cxSchedulerDateNavigator;

const
  dxThisUnitName = 'cxSchedulerWeekView';
...
type
  ...
  { TcxSchedulerWeekViewEventPlaceInfo }

  TcxSchedulerWeekViewEventPlaceInfo = class
  public
    ColStart: Integer;
    ColFinish: Integer;

    constructor Create(APlace: TcxSchedulerEventPlace);
  end;
...
constructor TcxSchedulerWeekViewEventPlaceInfo.Create(APlace: TcxSchedulerEventPlace);
begin
  ColStart := APlace.ColStart;
  ColFinish := APlace.ColFinish;
end;
...
begin
  ...
  begin
    if IsScrollResource(AGoToNext) then
    begin
      View.UnlockEventLayoutCalculating;
      Exit;
    end;
...
begin
  ...
  if IsHeaderEvent or (DateOf(Event.Finish) <> DateOf(Event.Start)) then
    InitHitTestForHorzEvent(AHitTest)
  else
    if (ShowStartTime and (FStartRect.Left <= AHitTest.HitX) and (AHitTest.HitX <= FStartRect.Right)) or
      (ShowFinishTime and (FFinishRect.Left <= AHitTest.HitX) and (AHitTest.HitX <= FFinishRect.Right)) then
...
begin
  ...
  begin
    ...
    ACell.Transparent := IsTransparentEventCell(AEvent);
    if IsClassicIntradayEventCell(AEvent) then
      ACell.Borders := [];
    TcxSchedulerEventCellViewInfoAccess(ACell).FContainerInfo := TcxSchedulerWeekViewEventPlaceInfo.Create(APlace);
    CheckDaysTimeEvent(ACell, APlace.ColStart, APlace.ColFinish);
  end
  else
    ...
    begin
       ...
       begin
         AddMoreEventsButton(I, DayCells[I].Bounds, DayCells[I].TimeStart, True, AEvent);
         DayCells[I].LineCount := MaxInt;
       end;
    end;
end;

function TcxSchedulerWeekViewViewInfo.AddDayCellEventPlace(ABuilder: TcxSchedulerEventLayoutBuilder;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        AddMoreEventsButton(I, ADay.Bounds, ADay.TimeStart, True, AEvent);
        AIndex := I + 1;
        Break;
      end;
    end
  ...
  ABuilder.AddEventPlace(AEvent, AStartCol, AFinishCol);
  AStartDay.LineCount := AStartDay.LineCount + 1;
end;

procedure TcxSchedulerWeekViewViewInfo.AddMoreEventsButton(AColumnIndex: Integer; const AContainerBounds: TRect;
  const ADateTime: TDateTime; AIsTop: Boolean; AEvent: TcxSchedulerEvent);
  ...
  begin
    for I := 0 to FButtons.Count - 1 do
      ...
      begin
        ...
        if (AColumnIndex = Result.ColumnIndex) and (ADateTime = Result.DateTime) then
          Exit;
      end;
    Result := nil;
  end;
...
begin
  if GetButton = nil then
    AddButton(AColumnIndex, AContainerBounds, ADateTime, AIsTop, AEvent);
end;
...
procedure TcxSchedulerWeekViewViewInfo.CalculateEventsViewInfo(ABuilder: TcxSchedulerEventLayoutBuilder);
var
  I, J: Integer;
  AEvent: TcxSchedulerControlEvent;
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if not AddDayCellEventPlace(ABuilder, AEvent, J, DayCells[J]) and (ResourceCount = 0) then
          Break;
      end
      else
        Inc(J);
  ...
  for I := 0 to ABuilder.EventPlaceCount - 1 do
    AddDayCellEvent(ABuilder.EventPlaces[I]);
end;
...
procedure TcxSchedulerWeekViewViewInfo.CheckDaysTimeEvent(AEventInfo: TcxSchedulerEventCellViewInfo;
...
begin
  if (AEventInfo.Event.State <> 0) and (AEventInfo.Event.IsAllDayOrLonger) then
    for I := AColStart to AColFinish do
    begin
      ADay := DayCells[I];
...
procedure TcxSchedulerWeekViewViewInfo.ClearButtons;
var
  I: Integer;
begin
...
procedure TcxSchedulerWeekViewViewInfo.ClearEventCells;
begin
  if not IsEventCellLayoutCalculatingLocked then
...
procedure TcxSchedulerWeekViewViewInfo.CustomProcessPreviouslyCalculatedEvent(AEventCell: TcxSchedulerEventCellViewInfo);
var
  APlaceInfo: TcxSchedulerWeekViewEventPlaceInfo;
begin
  TcxSchedulerEventCellViewInfoAccess(AEventCell).ViewData.ViewParams := StylesAdapter.GetEventParams(AEventCell.Event);
  AEventCell.Transparent := IsTransparentEventCell(AEventCell.Event);
  APlaceInfo := TcxSchedulerWeekViewEventPlaceInfo(TcxSchedulerEventCellViewInfoAccess(AEventCell).FContainerInfo);
  CheckDaysTimeEvent(AEventCell, APlaceInfo.ColStart, APlaceInfo.ColFinish);
end;
...
begin
  ABuilder := TcxSchedulerEventLayoutBuilder.Create(ViewStyle);
  try
    View.CheckWorkDays;
    FPrintWeekEnds := True;
    ...
    CalculateEventsViewInfo(ABuilder);
    CheckHeaderBorders;
...
function TcxSchedulerWeekViewViewInfo.IsClassicIntradayEventCell(AEvent: TcxSchedulerControlEvent): Boolean;
begin
  Result := (ViewStyle = svsClassic) and not AEvent.IsAllDayOrLonger;
end;

function TcxSchedulerWeekViewViewInfo.IsSingleLine(ADay1, ADay2: TcxSchedulerContentCellViewInfo): Boolean;
begin
  Result := (ADay1 <> nil) and (ADay2 <> nil) and (ADay1.Bounds.Top = ADay2.Bounds.Top);
end;

function TcxSchedulerWeekViewViewInfo.IsTransparentEventCell(AEvent: TcxSchedulerControlEvent): Boolean;
begin
  Result := IsClassicIntradayEventCell(AEvent) and not(AEvent.Selected or Styles.IsEventStyleAssigned(AEvent));
end;
