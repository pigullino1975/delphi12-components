...
uses
  ...
  SysUtils, Types, DateUtils, Menus, Generics.Defaults, Generics.Collections,
  ...
  cxDrawTextUtils, dxOffice11, cxScrollBar, dxGenerics, cxVariants, dxTypeHelpers;
...
type
  ...
  public
    ...
    property Splitter: TcxSchedulerTimeGridViewTreeBrowserSplitter read FSplitter; // for internal use
  ...
  protected
    ...
    function RoundTime(const ADateTime: TDateTime): TDateTime; inline;
    procedure ValidateMidnightForward(var ADateTime: TDateTime); inline;
    function ValidateStartTime(const ADateTime: TDateTime): TDateTime; inline;
    function ValidateTime(var ADateTime: TDateTime; AGoForward: Boolean; AInc: Integer = 1): Boolean; inline;
    procedure ValidateUnits(var AMajorUnit, AMinorUnit: TcxSchedulerTimeGridScaleUnit); inline;
  public
  ...
  protected
    ...
    procedure CorrectCaptionBottom;
    procedure DoDrawAsProgress; override;
    ...
    function GetBoundsAsProgress: TRect; override;
  ...
  TcxSchedulerForEachForEventUniqueResourceIDsFunc = reference to procedure(AResourceIndex: Integer);

  TcxSchedulerTimeGridViewViewInfo = class(TcxSchedulerCustomResourceViewViewInfo)
  private
    FEventUniqueResourceIDsMap: TObjectDictionary<TcxSchedulerEvent, TdxVariantList>;
    FResourceMap: TdxNamedObjectDictionary<TdxIntegerList>;
    ...
    procedure ForEachEventUniqueResourceIDs(AEvent: TcxSchedulerControlEvent;
      AShowEventsWithoutResource, AHasNullResourceID: Boolean; const AFunc: TcxSchedulerForEachForEventUniqueResourceIDsFunc);
    function GetEventUniqueResourceIDs(AEvent: TcxSchedulerControlEvent;
      ANeedAddNull: Boolean = False): TdxVariantList;
    function GetGroupCount: Integer; inline;
    function GetGroupLineCount(AIndex: Integer): Integer; inline;
    function GetResourceHeight: Integer; inline;
    function GetResourceViewShift(Index: Integer): TcxSchedulerTimeGridResourceScroll;
    function GetTimeGridView: TcxSchedulerTimeGridView; inline;
    function GetVisibleLineCount(AResourceIndex: Integer): Integer; inline;
    function HasNullResourceID: Boolean;
  ...
  protected
    ...
    FWorkTimeOnly: Boolean;
    FWorkDaysOnly: Boolean;
    function AddContentCell(const ARect: TRect; const AStart: TDateTime;
      const AFinish: TDateTime; AResourceIndex: Integer): TcxSchedulerContentCellViewInfo; override;
    ...
    procedure AddEventsForCalculation(ABuilder: TcxSchedulerEventLayoutBuilder);
    ...
    procedure ClearEventCells; override;
    ...
    function CreateResourceMap: TdxNamedObjectDictionary<TdxIntegerList>;
    procedure CustomProcessPreviouslyCalculatedEvent(AEventCell: TcxSchedulerEventCellViewInfo); override;
    ...
    function DoSetResourceHasVisibleEvent(AEvent: TcxSchedulerControlEvent): Boolean; override;
    ...
    function IsMinorUnitBreak(AColumn: Integer): Boolean; inline;
    ...
    function NeedEventBorderAlignment(AIsMilestone: Boolean): Boolean;
...
uses
  Variants, RTLConsts,
  dxCoreGraphics, cxFormats;

const
  dxThisUnitName = 'cxSchedulerTimeGridView';
...
function cxCompareEventsByStart(AEvent1, AEvent2: TcxSchedulerEvent): Integer;
begin
  Result := Sign(AEvent1.Start - AEvent2.Start);
end;
...
begin
  ...
  begin
    ...
    AllDayEvent := (dxTimeOf(Event.Finish) = 0) and (dxTimeOf(ATime) = 0) and ((Event.Finish - ATime) >= 1);
  ...
  begin
    ...
    AllDayEvent := (dxTimeOf(Event.Start) = 0) and (dxTimeOf(ATime) = 0) and ((ATime - Event.Start) >= 1);
    if AllDayEvent <> Event.AllDayEvent then
    begin
      if Event.AllDayEvent then
      begin
...
procedure TcxSchedulerTimeGridViewController.BeforeMouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TcxSchedulerTimeGridViewController.InitTimer(AllowStart: Boolean; AScrollCode: TScrollCode);
begin
...
procedure TcxSchedulerTimeGridViewController.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TcxSchedulerTimeGridViewController.MouseMove(Shift: TShiftState; X, Y: Integer);
begin
...
procedure TcxSchedulerTimeGridViewController.SyncEventSelection(AEvent: TcxSchedulerControlEvent);
var
...
begin
  ...
  begin
    ...
    TcxSchedulerTimeGridViewNavigation(Navigation).ReplaceSelParams(AStart, AFinish, AResourceItem);
  end
  else
...
procedure TcxSchedulerTimeGridViewNavigation.KeyDown(var AKey: Word; AShift: TShiftState);
const
...
function TcxSchedulerTimeGridViewStyles.GetMajorScaleParams(const ADateTime: TDateTime): TcxViewParams;
var
  AStyle: TcxStyle;
begin
  AStyle := nil;
...
function TcxSchedulerTimeGridViewStyles.GetMinorScaleParams(const ADateTime: TDateTime): TcxViewParams;
var
  AStyle: TcxStyle;
begin
  AStyle := nil;
...
function TcxSchedulerTimeGridViewStyles.GetSelectionBarParams(const ADateTime: TDateTime): TcxViewParams;
begin
...
procedure TcxSchedulerTimeGridViewStyles.GetDefaultViewParams(Index: Integer; AData: TObject; out AParams: TcxViewParams);
var
...
function TcxSchedulerTimeGridViewStyles.GetSelectionBarParamsEx(const ADateTime: TDateTime; ASelected: Boolean): TcxViewParams;
var
  AStyle: TcxStyle;
begin
  AStyle := nil;
...
function TcxSchedulerTimeGridViewStyles.IsTimeSelected(const ADateTime: TDateTime): Boolean;
begin
...
constructor TcxSchedulerTimeGridViewScales.Create(AOwner: TcxSchedulerTimeGridView);
begin
  FOwner := AOwner;
  Major := True;
...
destructor TcxSchedulerTimeGridViewTreeBrowser.Destroy;
begin
...
procedure TcxSchedulerTimeGridViewTreeBrowser.DoPaint;
begin
  Canvas.Brush.Color := LookAndFeelPainter.DefaultSchedulerControlColor;
...
procedure TcxSchedulerTimeGridViewTreeBrowser.SetView(AValue: TcxSchedulerTimeGridView);
begin
  FView := AValue;
...
function TcxSchedulerTimeGridView.GetControlEventLineStart(AEvent: TcxSchedulerControlEvent): Integer;
var
  I: Integer;
begin
...
function TcxSchedulerTimeGridView.GetEditWithSingleLineEditor(AEvent: TcxSchedulerControlEvent): Boolean;
begin
...
procedure TcxSchedulerTimeGridView.ValidateSelectionFinishTime(var ADateTime: TDateTime);
var
  Y, M, D: Word;
begin
...
procedure TcxSchedulerTimeGridView.GetPropertyValue(const AName: string; var AValue: Variant);
begin
...
procedure TcxSchedulerTimeGridView.UnInitScrollbars;
var
  I: Integer;
begin
  for I := 0 to ViewInfo.ResourceViewShiftList.Count - 1 do
    ViewInfo.ResourceViewShift[I].UnInitScrollBars;
end;
...
procedure TcxSchedulerTimeGridView.SetScales(AValue: TcxSchedulerTimeGridViewScales);
begin
...
procedure TcxSchedulerTimeGridView.SetStyles(AValue: TcxSchedulerTimeGridViewStyles);
begin
  FStyles.Assign(AValue);
  Changed;
end;

procedure TcxSchedulerTimeGridView.SetTreeBrowser(AValue: TcxSchedulerTimeGridViewTreeBrowser);
begin
...
begin
  ...
  R.InitSize(0, 0, AIconsWidth, TcxSchedulerPainterHelper.IconsHeight(ScaleFactor));
  AVertOffset := GetImagesVerticalOffset(R.Bottom, True);
  ASpaceWidth := CaptionRect.Width - (ACaptionWidth + AImagesWidth + cxEventImagesOffset);
...
begin
  Result := Max(ScaleFactor.Apply(cxEventImagesOffset), (Bounds.Height - 2 * cxEventBorderWidth - AImageHeight) div 2);
  if AIsAbsolute then
...
procedure TcxSchedulerTimeGridViewEventCellCustomViewInfo.InitHitTest(AHitTest: TcxSchedulerCustomResourceViewHitTest);
begin
...
begin
  ...
  if FCaptionRect.IsZero and FLocationRect.IsZero then
...
var
  ...
  begin
    ...
    if not FEndContinueCaptionRect.IsEmpty then
      Result.Right := FEndContinueCaptionRect.Left - ATextOffset
    else
...
begin
  ...
  begin
    ...
    InternalCorrectStartContinueArrowAttributeRect(FStartContinueArrowRect);
    InternalCorrectStartContinueArrowAttributeRect(FStartContinueCaptionRect);
    ACaptionRect := GetInitialCaptionRect;
    ...
    if (ShowStartTime or ShowFinishTime) and (AFullCaptionWidth > ACaptionRect.Width) and not GetForceShowClockInHeaderEvent then
    begin
    ...
    if (ACaptionOnlyWidth <= ACaptionRect.Width) or ((EventStart >= ContentStart) and (EventFinish <= ContentFinish)) then
      Break;
  end;
...
var
  AImagesRect, R: TRect;
  AImages: TcxSchedulerEventImagesAccess;
  AHeight, ATextOffset: Integer;

  procedure CalculateImagesLayout;
  begin
     AImagesRect := cxRectInflate(GetAvailableBounds, -ATextOffset, -ATextOffset);
     ...
     Inc(AImagesRect.Left, ATextOffset);
     AImagesRect.Bottom := AImagesRect.Top + AImages.TotalVisibleHeight;
     Inc(FCaptionRect.Left, AImages.ItemWidth + ATextOffset);
...
begin
  ATextOffset := ScaleFactor.Apply(cxTextOffset);
  AImages := TcxSchedulerEventImagesAccess(Images);
  Result := AImages.ItemHeight + 2 * ATextOffset;
  ...
  begin
    ...
    Result := AImagesRect.Height + 2 * ATextOffset;
  end;
  R := CaptionRect;
  if IsHeaderEvent then
  begin
...
var
  AImagesRight, ADelta, ACaptionMinWidth, AImagesWidth, AImagesOffset: Integer;
  AImagesRect: TRect;
  AImages: TcxSchedulerEventImagesAccess;
begin
  AImagesOffset := ScaleFactor.Apply(cxEventImagesOffset);
  AImages := TcxSchedulerEventImagesAccess(Images);
  FCaptionRect := GetNonDetailEventPossibleCaptionRect;
  ...
  if (AImages.ForceVisibleWidth > 0) and
     (AImagesRect.Left + AImages.ForceVisibleWidth + AImagesOffset > CaptionRect.Right - cxTextWidth(Canvas.Font, '0')) then
  begin
  ...
  begin
    AImagesRight := AImagesRect.Left + AImages.ForceVisibleWidth + AImagesOffset;
    ADelta := AImages.TotalVisibleWidth - AImages.ForceVisibleWidth;
    ACaptionMinWidth := GetCaptionMinWidth;
    while ADelta > 0 do
    begin
      Inc(AImagesRight, AImages.ItemWidth + AImagesOffset);
      if AImagesRight >= CaptionRect.Right - ACaptionMinWidth then
        Break;
      Dec(ADelta, AImages.ItemWidth + AImagesOffset);
    end;
    ...
    FCaptionRect.Left := Max(AImagesRect.Right +
      IfThen(AImagesWidth > 0, AImagesOffset), GetMessageRectLeft + GetMessageSeparatorLeftIndent);
...
begin
  ...
  begin
    ...
    if ViewData.AutoHeight and (ViewData.EventHeight <> 0) then
    begin
      FLocationRect.Height := GetLocationAutoHeight(FLocationRect);
      if FLocationRect.Height > 0 then
        FLocationRect.Height := FLocationRect.Height + 2 * (ScaleFactor.Apply(cxTextOffset) + ScaleFactor.Apply(1));
    end;
  end
  else
  begin
...
begin
  FCaptionRect.Top := Bounds.Top + ScaleFactor.Apply(cxEventImagesOffset);
  FCaptionRect.Bottom := Bounds.Bottom - ScaleFactor.Apply(cxEventImagesOffset);
end;

procedure TcxSchedulerTimeGridViewEventCellModernViewInfo.CorrectCaptionBottom;
var
  AImagesOffset: Integer;
begin
  if FShowMessage and not FCaptionRect.IsZero and (ViewData.EventHeight <> 0) then
  begin
    FCaptionRect.Bottom := FMessageRect.Top;
    AImagesOffset := Max((FBounds.Height - ((FMessageRect.Top - FCaptionRect.Top) + ScaleFactor.Apply(1))) div 2, 0);
    if Images.Layout <> eilVertical then
    begin
      TcxSchedulerEventImagesAccess(Images).Offset(0, -AImagesOffset);
      if TcxSchedulerEventImagesAccess(Images).VisibleImageCount > 0 then
        FCaptionRect.Bottom := FCaptionRect.Bottom - ScaleFactor.Apply(cxTextOffset);
    end
    else
      if TcxSchedulerEventImagesAccess(Images).VisibleImageCount = 1 then
        TcxSchedulerEventImagesAccess(Images).Offset(0, -AImagesOffset);
    if ShowStartTime and not FStartRect.IsEmpty then
    begin
      FStartRect.Top := FCaptionRect.Top;
      FStartRect.Bottom := FCaptionRect.Bottom;
    end;
    if ShowFinishTime and not FFinishRect.IsEmpty then
    begin
      FFinishRect.Top := FCaptionRect.Top;
      FFinishRect.Bottom := FCaptionRect.Bottom;
    end;
    if not FStartContinueArrowRect.IsEmpty then
    begin
      FStartContinueArrowRect.Top := FCaptionRect.Top;
      FStartContinueArrowRect.Bottom := FCaptionRect.Bottom;
    end;
    if not FStartContinueCaptionRect.IsEmpty then
    begin
      FStartContinueCaptionRect.Top := FCaptionRect.Top;
      FStartContinueCaptionRect.Bottom := FCaptionRect.Bottom;
    end;
    if not FEndContinueArrowRect.IsEmpty then
    begin
      FEndContinueArrowRect.Top := FCaptionRect.Top;
      FEndContinueArrowRect.Bottom := FCaptionRect.Bottom;
    end;
    if not FEndContinueCaptionRect.IsEmpty then
    begin
      FEndContinueCaptionRect.Top := FCaptionRect.Top;
      FEndContinueCaptionRect.Bottom := FCaptionRect.Bottom;
    end;
    CalculateLocationRect;
    if FCaptionAndLocationCanPlaceOneLineOnly then
      FLocationRect.Bottom := FCaptionRect.Bottom;
  end;
end;

procedure TcxSchedulerTimeGridViewEventCellModernViewInfo.DoDrawAsProgress;
var
  R: TRect;
  ACurrentClipRegion: TcxRegion;
begin
  Canvas.SaveClipRegion;
  try
    ACurrentClipRegion := Canvas.GetClipRegion;
    R := ACurrentClipRegion.BoundsRect;
    if not UseRightToLeftAlignment then
      R.Right := R.Left + 1
    else
      R.Left := R.Right - 1;
    ACurrentClipRegion.Combine(R, roSubtract);
    Canvas.SetClipRegion(ACurrentClipRegion, roSet);
    inherited;
  finally
...
function TcxSchedulerTimeGridViewEventCellModernViewInfo.GetBoundsAsProgress: TRect;
begin
  Result := inherited GetBoundsAsProgress;
  if not UseRightToLeftAlignment then
...
begin
  Result := TdxTextMeasurer.TextWidthTO(Canvas.Font, ViewData.Caption, GetCaptionFontStyle);
  if FLocation <> '' then
...
procedure TcxSchedulerTimeGridViewViewInfo.CalculateHitTest(AHitTest: TcxSchedulerCustomResourceViewHitTest);
begin
  if not FTimeLineCells.CalculateHitTest(AHitTest) then
    inherited CalculateHitTest(AHitTest);
end;

procedure TcxSchedulerTimeGridViewViewInfo.ChangeResourceShift(AResourceIndex, ADelta: Integer);
begin
...
function TcxSchedulerTimeGridViewViewInfo.AddContentCell(const ARect: TRect; const AStart: TDateTime;
  const AFinish: TDateTime; AResourceIndex: Integer): TcxSchedulerContentCellViewInfo;

  function GetResourceViewInfo: TcxSchedulerResourceViewInfo;
  begin
    if AResourceIndex = -1 then
      Result := nil
    else
      Result := Resources[AResourceIndex];
  end;

  function GetResourceItem: TcxSchedulerStorageResourceItem;
  var
    AViewInfo: TcxSchedulerResourceViewInfo;
  begin
    AViewInfo := GetResourceViewInfo;
    if AViewInfo = nil then
      Result := nil
    else
      Result := AViewInfo.ResourceItem;
  end;
...
begin
  CreateCellInstance(ContentCellClass, ARect,
    StylesAdapter.GetContentParams(AStart, True, GetResourceItem), UseRightToLeftAlignment, Result);
  Result.SetTime(AStart, AFinish);
  AssignResourceID(Result, AResourceIndex);
  ContentCells.Add(Result);
  AContentCell := TcxSchedulerTimeGridViewContentCellViewInfo(Result);
  AContentCell.FContentParams := AContentCell.ViewParams;
  if not View.IsWorkTime(GetResourceItem, AStart) then
    AContentCell.FViewParams.Color := PainterHelper.GetContentColor(AContentCell.FViewParams.Color, False, ViewStyle);
  if not HiddenSelection and IsTimeSelected(AContentCell.TimeStart, GetResourceViewInfo) then
    AContentCell.FViewParams := GetSelectionParams(AContentCell.FViewParams);
end;
...
procedure TcxSchedulerTimeGridViewViewInfo.AddEventForCalculation(
  ...
  var
    L, H: Integer;
    mid, cmp: Integer;
    AStart, AFinish, AEventStart: TDateTime;
  begin
    Result := 0;
    ...
    begin
      mid := L + (H - L) shr 1;
      AStart := TcxSchedulerTimeGridSelectionBarCell(FTimeLineCells[mid]).TimeStart;
      AFinish := TcxSchedulerTimeGridSelectionBarCell(FTimeLineCells[mid]).TimeFinish;
      if AEventStart < AStart then
        cmp := 1
      else
        if AEventStart >= AFinish then
          cmp := -1
        else
          cmp := 0;
      if cmp < 0 then
...
begin
  ...
  begin
    AColumnIndex := FindColumnIndex;
    while AColumnIndex < FColumnCount do
    begin
      if IsColumnEvent(AEvent, AColumnIndex) then
      begin
        CalculateEventPosition(AEvent, AColumnIndex, AStartCol, AFinishCol, AStart, AFinish);
...
procedure TcxSchedulerTimeGridViewViewInfo.AddEventsForCalculation(ABuilder: TcxSchedulerEventLayoutBuilder);
var
  AEventIndex: Integer;
  I: Integer;
  AShowEventsWithoutResource: Boolean;
  AEvent: TcxSchedulerControlEvent;
  AItem: TcxSchedulerStorageResourceItem;
  AHasNullResourceID: Boolean;
begin
  AShowEventsWithoutResource := ShowEventsWithoutResource;
  AHasNullResourceID := HasNullResourceID;
  FResourceMap := CreateResourceMap;
  FEventUniqueResourceIDsMap := TObjectDictionary<TcxSchedulerEvent, TdxVariantList>.Create([doOwnsValues]);
  try
    for AEventIndex := 0 to Events.AbsoluteItemsList.Count - 1 do
    begin
      AEvent := TcxSchedulerControlEvent(Events.AbsoluteItemsList[AEventIndex]);
      if not IsEventVisible(AEvent) then
        Continue;
      TcxSchedulerControlEventAccess(AEvent).VisibleIndex := -1;

      if ResourceCount = 0 then
        AddEventForCalculation(ABuilder, AEvent, nil, 0)
      else
      begin
        if AShowEventsWithoutResource and VarIsNull(AEvent.ResourceID) then
        begin
          for I := 0 to ResourceCount - 1 do
          begin
            AItem := Resources[I].ResourceItem;
            AddEventForCalculation(ABuilder, AEvent, AItem, I);
          end;
        end
        else
        begin
          ForEachEventUniqueResourceIDs(AEvent, AShowEventsWithoutResource, AHasNullResourceID,
            procedure(AResourceIndex: Integer)
            begin
              AddEventForCalculation(ABuilder, AEvent, Resources[AResourceIndex].ResourceItem,
                AResourceIndex);
            end);
        end;
      end;
    ...
    FreeAndNil(FEventUniqueResourceIDsMap);
    FreeAndNil(FResourceMap);
  end;
end;

function TcxSchedulerTimeGridViewViewInfo.AddEventViewInfo(APlace: TcxSchedulerEventPlace; AResourceIndex: Integer;
  const AStart, AFinish: TDateTime): TcxSchedulerEventCellViewInfo;
var
  I: Integer;
...
begin
  AVisibleBounds.Init(Bounds.Left + ResourceHeaderWidth, ScalesHeight, Bounds.Right, Bounds.Bottom);
  ...
  begin
    ...
    AResource := nil;
  ABounds.Init(APlace.ColStart, AVisibleBounds.Top + IndentBetweenLines,
    APlace.ColFinish, AVisibleBounds.Top + IndentBetweenLines + FContentLineHeight);
  if Adapter.IsPrinting then
    Inc(ABounds.Left);
  AViewData := CreateEventViewData(TcxSchedulerControlEvent(APlace.Event), ABounds, AStart, AFinish, AResource);
  AViewData.VisibleRect := AVisibleBounds;
  AViewData.EventHeight := View.Scheduler.OptionsView.EventHeight;
...
procedure TcxSchedulerTimeGridViewViewInfo.AddMajorScaleCell(ABounds: TRect; const AStart, AFinish: TDateTime);
var
...
begin
  ...
  AddTimeLineItem(TcxSchedulerTimeGridMajorScaleCell, ABounds, 0, AStart, AFinish, View.Styles.GetMajorScaleParams(AStart), AItem);
  AItem.FDisplayText := View.GetMajorUnitDisplayText(AStart, CheckFinishTime(AFinish, Scales.MajorUnit), FMajorTextType);
  AItem.FBorders := [bBottom];
  AItem.FRightIndent := FSeparatorWidth;
end;

procedure TcxSchedulerTimeGridViewViewInfo.AddMajorSeparator(var ALeft: Integer);
var
  ARect: TRect;
  ACell: TcxSchedulerMajorSeparatorCellViewInfo;
begin
  Dec(ALeft);
  ARect.Init(ALeft, Bounds.Top, ALeft + FSeparatorWidth, Bounds.Bottom);
...
var
  ...
  begin
    ...
    if (ATextType <> sttShort) and
      (cxTextWidth(ACell.Font, S) > (ACell.Bounds.Width - cxTextOffset * 2)) then
    begin
...
begin
  ...
  repeat
    ...
    begin
      ACell := TcxSchedulerTimeGridMinorScaleCell(TimeLineCells[I]);
...
begin
  ...
  begin
    ...
    begin
      ACell := TcxSchedulerTimeGridSelectionBarCell(TimeLineCells[J]);
      if not ACell.Visible then Continue;
      R.Left := ACell.Bounds.Left;
      R.Right := ACell.Bounds.Right;
      AContentCell := TcxSchedulerTimeGridViewContentCellViewInfo(
        AddContentCell(R, ACell.TimeStart, ACell.TimeFinish, AResourceIndex));
      AColor := AContentCell.FContentParams.Color;
      AContentCell.Borders := [bRight];
      AContentCell.SetBorderColor(AColor, IsMinorUnitBreak(J + 1), View.IsWorkTime(AResource, ACell.TimeFinish));
    end;
  end;
end;
...
var
  ARoundedEventStart, ARoundedEventFinish, AFirst, ALast: TDateTime;
  AEventFinish, AEventStart: TDateTime;
  ...
  begin
    ...
    if DateOf(TimeBuilder.WorkStart) = 0 then
      AInc := 0
    else
      AInc := 1;

    while (AEventFinish > ALast) or ((AEventStart = ALast) and View.SnapEventsToTimeSlots) do
    begin
      AStartValue := ALast;
      APrevFinish := ALast;
      ...
      if AEventFinish < AFirst then Break;
      ...
      if (ARoundedEventFinish < ALast) and (ARoundedEventFinish > AFirst) and TimeBuilder.IsPeriodChanged(APrevFinish, AFirst) then
      begin
      ...
      //
      if ARoundedEventFinish > AFirst then
      begin
        AFinish := ALast;
        if (ARoundedEventFinish > ALast) and TimeBuilder.IsPeriodChanged(AStartValue, ALast) then
        begin
          Inc(AFinishX, FSeparatorWidth - 1);
          Inc(AFinishCellRight, FSeparatorWidth - 1);
        end;
    ...
    if not View.IsSnapEventsToTimeSlots and (ARoundedEventFinish <= AActualLast) then
    begin
      AFinishX := GetActualPos(AFinishX - FColumnWidth, ARoundedEventFinish,
  ...
  begin
    AFirst := AStart;
    ALast := AFirst;
    if not View.IsSnapEventsToTimeSlots and (ARoundedEventStart >= AFirst) then
    begin
      AStartX := GetActualPos(AStartX, ARoundedEventStart, AFirst, TimeBuilder.Inc(AFirst));
      Exit;
    end;
    ABreak := False;
    while (AEventStart < AFirst) and not ABreak do
    begin
      ALast := AFirst;
      ...
      if (ARoundedEventStart <= AFirst) or ((ARoundedEventStart > AFirst) and (ARoundedEventStart < ALast) and
        not TimeBuilder.IsPeriodChanged(AFirst, ARoundedEventStart)) then
      begin
        if AIsWorkDays then
        begin
          ...
          AStart := AFirst;
        if not TimeBuilder.IsPeriodChanged(AFirst, ALast) or (ARoundedEventStart < TimeBuilder.CalculateDateTimeDateTimeWithoutCheckWorkDays(AFirst, 1)) then
          AStartX := AStartX - FColumnWidth;
      end;
      if not View.IsSnapEventsToTimeSlots and (ARoundedEventStart >= AFirst) and not AIsWorkDays and not ACorrected then
        AStartX := GetActualPos(AStartX, ARoundedEventStart, AFirst, ALast);
      if TimeBuilder.IsPeriodChanged(AFirst, ALast) and (ARoundedEventStart <= AFirst) and not AIsWorkDays then
...
begin
  AStartScaleCell := TcxSchedulerTimeGridSelectionBarCell(FTimeLineCells[AColIndex]);
  AIsMilestone := IsMilestone(AEvent);
  AStartX := AStartScaleCell.Bounds.Left;
  if NeedEventBorderAlignment(AIsMilestone) and (AStartX > 0) then
    Dec(AStartX);
  AFinishX := AStartX;
  ...
  AEventStart := AEvent.Start;
  AEventFinish := AEvent.Finish;
  ARoundedEventStart := DateTimeHelper.RoundTime(AEventStart);
  ARoundedEventFinish := DateTimeHelper.RoundTime(AEventFinish);
...
begin
  ...
  begin
    ...
    FEventUniqueResourceIDsMap := TObjectDictionary<TcxSchedulerEvent, TdxVariantList>.Create([doOwnsValues]);
    try
      ProcessPreviouslyCalculatedEvents;
    finally
      FreeAndNil(FEventUniqueResourceIDsMap);
      FreeAndNil(FResourceMap);
    end;
  ...
  try
    ABuilder.Clear;
    AddEventsForCalculation(ABuilder);
    ABuilder.Calculate;
    CalculateResourceViewShifts(ABuilder);
    SaveEventsLineStart(ABuilder);
    for I := 0 to ABuilder.EventPlaceCount - 1 do
    begin
      Realign(ABuilder.EventPlaces[I]);
      TcxSchedulerControlEventAccess(ABuilder.EventPlaces[I].Event).VisibleIndex := I;
    end;
    for I := EventCells.Count - 1 downto 0 do
      CheckEventVisibility(EventCells[I], I);
  finally
    ABuilder.Free;
  end;
end;
...
begin
  ...
  with Bounds do
    FScalesBounds.Init(Left + FResourceHeaderWidth, Top, Right, Top + FScalesHeight);
...
procedure TcxSchedulerTimeGridViewViewInfo.CalculateResourceHeadersAutoHeight(AWidth: Integer);
begin
...
procedure TcxSchedulerTimeGridViewViewInfo.CalculateResourceTopBottom(AResourceIndex: Integer; var ABounds: TRect);
var
  H: Integer;
begin
  H := GetResourceHeight;
...
procedure TcxSchedulerTimeGridViewViewInfo.CheckEventVisibility(ACell: TcxSchedulerEventCellViewInfo; AIndex: Integer);
begin
  if ACell.Hidden then
    FEventCells.Delete(AIndex);
end;

function TcxSchedulerTimeGridViewViewInfo.CheckFinishTime(const ATime: TDateTime; AUnit: TcxSchedulerTimeGridScaleUnit): TDateTime;
begin
  Result := ATime;
...
procedure TcxSchedulerTimeGridViewViewInfo.ClearButtons;
var
  I: Integer;
begin
  if not IsEventCellLayoutCalculatingLocked then
    ...
    if UseRightToLeftAlignment then
      for I := 0 to Buttons.Count - 1 do
        TcxSchedulerCustomViewInfoItemAccess(Buttons[I]).AllowRightToLeftConversion := False;
end;

procedure TcxSchedulerTimeGridViewViewInfo.ClearEventCells;
begin
  if not IsEventCellLayoutCalculatingLocked then
    inherited ClearEventCells;
end;
...
begin
  Result := TcxSchedulerEventLayoutBuilder.Create(svsClassic); 
end;

function TcxSchedulerTimeGridViewViewInfo.CreateResourceMap: TdxNamedObjectDictionary<TdxIntegerList>;
var
  I: Integer;
  AList: TdxIntegerList;
  AValue: string;
begin
  Result := TdxNamedObjectDictionary<TdxIntegerList>.Create(True);
  for I := 0 to ResourceCount - 1 do
  begin
    if VarIsEmpty(Resources[I].ResourceID) then
      Continue;
    AValue := VarToStr(Resources[I].ResourceID);
    if Result.TryGetValue(AValue, AList) then
      AList.Add(I)
    else
    begin
      AList := TdxIntegerList.Create;
      AList.Add(I);
      Result.Add(AValue, AList);
    end;
  end;
end;

procedure TcxSchedulerTimeGridViewViewInfo.CustomProcessPreviouslyCalculatedEvent(AEventCell: TcxSchedulerEventCellViewInfo);
begin
  TcxSchedulerEventCellViewInfoAccess(AEventCell).ViewData.ViewParams := StylesAdapter.GetEventParams(AEventCell.Event);
end;
...
begin
  ...
  try
    if not View.Bounds.IsEmpty then
    begin
...
procedure TcxSchedulerTimeGridViewViewInfo.DoContentNavigationButtonClick(Sender: TcxSchedulerContentNavigationButtonViewInfo);
begin
  View.VisibleStart := View.VisibleStart + Sender.Interval;
  inherited DoContentNavigationButtonClick(Sender);
end;

procedure TcxSchedulerTimeGridViewViewInfo.DoMoreEventsButtonClick(Sender: TcxSchedulerMoreEventsButtonViewInfo);
var
...
begin
  ResourceViewShift[(Sender as TcxSchedulerTimeGridScrollBarWrapper).ResourceIndex].Shift := -ScrollPos;
  ScrollPos := -ResourceViewShift[(Sender as TcxSchedulerTimeGridScrollBarWrapper).ResourceIndex].Shift;
  View.Changed;
end;

function TcxSchedulerTimeGridViewViewInfo.DoSetResourceHasVisibleEvent(AEvent: TcxSchedulerControlEvent): Boolean;
var
  AResourceFound: Boolean;
begin
  AResourceFound := False;
  ForEachEventUniqueResourceIDs(AEvent, False, True,
    procedure(AResourceIndex: Integer)
    begin
...
function TcxSchedulerTimeGridViewViewInfo.IsColumnEvent(AEvent: TcxSchedulerControlEvent; AColumnIndex: Integer): Boolean;
var
...
begin
  ...
  if (AEvent.Duration < 1) and View.WorkTimeOnly and
    ((dxTimeOf(AEventFinish) < View.WorkStart) and (dxTimeOf(AEventStart) > View.WorkFinish)) then
    Exit;
  AStart := TcxSchedulerTimeGridSelectionBarCell(FTimeLineCells[AColumnIndex]).TimeStart;
  AFinish := TcxSchedulerTimeGridSelectionBarCell(FTimeLineCells[AColumnIndex]).TimeFinish;
  if Scales.MinorUnit = suDay then
    Result := AEvent.IsDayEvent(Trunc(AStart))
  else
    if Scales.MinorUnit > suDay then
      Result := ((AEventFinish > AStart) and (AEventStart < AFinish)) or ((AEventFinish = AStart) and (AEvent.Duration = 0))
    else
      if AEventFinish = AEventStart then
        Result := (AEventStart >= AStart) and (AEventStart < AFinish)
      else
      begin
        if AEventFinish > AStart then
          Result := {(AEventStart <= AStart) or
            (AEventFinish <= AFinish) todo: } AEventStart < AFinish
        else
          if AEventFinish = AStart then
            Result := AEventStart = AStart;
      end;
end;

function TcxSchedulerTimeGridViewViewInfo.IsEventVisible(AEvent: TcxSchedulerControlEvent): Boolean;
var
  I: Integer;
begin
  Result := True;
...
function TcxSchedulerTimeGridViewViewInfo.IsMinorUnitBreak(AColumn: Integer): Boolean;
  ...
  function IsHourEqual(const ATime1, ATime2: TDateTime): Boolean; inline;
  begin
...
function TcxSchedulerTimeGridViewViewInfo.GetEventClipRect(AEventViewInfo: TcxSchedulerEventCellViewInfo): TRect;
var
  I: Integer;
  AResourceInfo: TcxSchedulerResourceViewInfo;
begin
  AResourceInfo := AEventViewInfo.ResourceInfo;
  Result.Init(Bounds.Left + ResourceHeaderWidth, ScalesHeight, Bounds.Right, Bounds.Bottom);
...
const
  AResult: array[Boolean] of TcxSchedulerMoreEventsButtonViewInfoClass =
    (TcxSchedulerTimeGridMoreEventsButtonViewInfo, TcxSchedulerTimeGridMoreEventsModernButtonViewInfo);
begin
...
begin
  ...
  begin
    ...
    if ResourceHeaders.RotateCaptions then
...
function TcxSchedulerTimeGridViewViewInfo.NeedEventBorderAlignment(AIsMilestone: Boolean): Boolean;
begin
  Result := (ViewStyle = svsModern) and not AIsMilestone;
end;

function TcxSchedulerTimeGridViewViewInfo.MeasureFontHeight(AStyle: TcxStyle; AHeight: Integer; Borders: TcxBorders): Integer;
begin
  Result := AHeight;
  if Result = -1 then
...
procedure TcxSchedulerTimeGridViewViewInfo.Realign(APlace: TcxSchedulerEventPlace);
  ...
  function GetButton(AColumnIndex: Integer; IsDown: Boolean): TcxSchedulerMoreEventsButtonViewInfo;
  begin
    Result := GetResourceViewShift(
      Integer(APlace.Resource)).FMoreEventButtons[AColumnIndex * 2 + IfThen(IsDown, 1)];
  end;
  ...
  begin
    ...
    if (AColumnIndex < 0) or
      (AColumnIndex * 2 >= Length(GetResourceViewShift(Integer(APlace.Resource)).FMoreEventButtons)) then
      Exit;
    ADelta := 0;
    ...
    begin
      ...
      if AButton = nil then
      begin
        AButton := AddButton(AColumnIndex, R, ADateTime, ADownDirection, APlace.Event) as TcxSchedulerTimeGridMoreEventsButtonViewInfo;
...
var
  AEventRect, AButtonRect, AEventClipRect: TRect;
...
begin
  ...
  AnEventLineIndex := APlace.LineStart + GetResourceViewShift(Integer(APlace.Resource)).FShift - FLineOffset;
  ...
  begin
    ...
    AEventClipRect := GetEventClipRect(ACell);
    if NeedEventBorderAlignment(AViewData.IsMilestone) then
      Dec(AEventClipRect.Left);
    while AEventRect.Right > AButtonRect.Left do
    begin
      if not Adapter.IsPrinting and ((AButtonRect.Left >= AEventClipRect.Left) and
        (AButtonRect.Right <= AEventClipRect.Right)) then
        AddMoreEventButton(AnEventLineIndex, ALineCount, AButtonRect, ACell.Event.Start,
          (AViewData.VisibleRect.Bottom - AViewData.VisibleRect.Top) div 2 >= GetMoreButtonHeight);
...
procedure TcxSchedulerTimeGridViewViewInfo.ReturnVisibleInterval(var AStart, AEnd: TDateTime);
begin
...
begin
  ...
  FTimeBuilder.CalculateActualStart(FActualStartTime, View.FVisibleStart, FFirstVisibleIndex);
  if not Adapter.IsPrinting then
  begin
    SelectedDays.Clear;
...
procedure TcxSchedulerTimeGridViewViewInfo.CalculateResourceViewShifts(ABuilder: TcxSchedulerEventLayoutBuilder);
  ...
  begin
    ...
    for I := 0 to ABuilder.EventPlaceCount - 1 do
      ...
      begin
        Result := TcxSchedulerEventCellViewInfo(ABuilder.EventPlaces[I].Data).EventViewData.VisibleRect;
        ADimension := Result.Right - Result.Left;
        Result.Left := View.Left;
        Result.Right := Result.Left + ADimension + IfThen(ResourceHeaderCells.Count > 0, ResourceHeaderHeight, 0);
  ...
  begin
    ...
    begin
      ...
      GetResourceViewShift(AResourceIndex).SetEventMaxLine(ABuilder.EventPlaces[I].LineStart);
    end;
  end;
...
procedure TcxSchedulerTimeGridViewViewInfo.ForEachEventUniqueResourceIDs(AEvent: TcxSchedulerControlEvent;
  AShowEventsWithoutResource, AHasNullResourceID: Boolean;
  const AFunc: TcxSchedulerForEachForEventUniqueResourceIDsFunc);
var
  AResourceIndex: Integer;
  AIDs: TdxVariantList;
  AID: Variant;
  AIndexes: TdxIntegerList;
  I: Integer;
  AIsIDEmpty: Boolean;
begin
  AIDs := GetEventUniqueResourceIDs(AEvent, AHasNullResourceID);
  for AID in AIDs do
  begin
    AIsIDEmpty := VarIsEmpty(AID) or VarIsNull(AID);
    if FResourceMap.TryGetValue(VarToStr(AID), AIndexes) then
      for I := 0 to AIndexes.Count - 1 do
      begin
        AResourceIndex := AIndexes[I];
        if not AIsIDEmpty or AEvent.IsResourceEvent(Resources[AResourceIndex].ResourceItem, AShowEventsWithoutResource) then
          AFunc(AResourceIndex);
      end;
  end;
end;

function TcxSchedulerTimeGridViewViewInfo.GetEventUniqueResourceIDs(AEvent: TcxSchedulerControlEvent; ANeedAddNull: Boolean = False): TdxVariantList;
var
  I: Integer;
begin
  if FEventUniqueResourceIDsMap.TryGetValue(AEvent, Result) then
    Exit;

  Result := TdxVariantList.Create;
  if ANeedAddNull then
    Result.Add(Null);
  for I := 0 to AEvent.ResourceIDCount - 1 do
    if Result.IndexOf(AEvent.ResourceIDs[I]) = -1 then
      Result.Add(AEvent.ResourceIDs[I]);
  FEventUniqueResourceIDsMap.Add(AEvent, Result);
end;
...
function TcxSchedulerTimeGridViewViewInfo.GetGroupLineCount(AIndex: Integer): Integer;
begin
...
function TcxSchedulerTimeGridViewViewInfo.GetResourceViewShift(Index: Integer): TcxSchedulerTimeGridResourceScroll;
begin
  if ResourceCount = 0 then
    Index := 0;
...
function TcxSchedulerTimeGridViewViewInfo.HasNullResourceID: Boolean;
var
  I: Integer;
begin
  Result := True;
  for I := 0 to ResourceCount - 1 do
    if VarIsNull(Resources[I].ResourceID) then
      Exit;
  Result := False;
end;
...
const
  Horz: array[TAlignment] of Integer = (CXTO_LEFT, CXTO_RIGHT, CXTO_CENTER_HORIZONTALLY);
...
begin
  ...
  if (Bounds.Left < ClipRect.Left) and (W >= FTextBounds.Width) then
  begin
    FAlignHorz := taRightJustify;
    FHideDisplayText := (FTextBounds.Width / W) < 0.5;
  end;
end;
...
procedure TcxSchedulerTimeGridMajorScaleCell.InitHitTest(AHitTest: TcxSchedulerCustomResourceViewHitTest);
begin
  inherited InitHitTest(AHitTest);
...
procedure TcxSchedulerTimeGridSelectionBarCell.CalculateTimeLineParams(ANeedShowCurrentTime: Boolean);
var
...
begin
  ...
  begin
    ALeft := Bounds.Left + Trunc((Bounds.Right - Bounds.Left) * (ACurrentTime - TimeStart) / (TimeFinish - TimeStart));
    FTimeLineRect.Init(ALeft - 1, Bounds.Top, ALeft + 1, ViewHeight);
  end;
end;
...
procedure TcxSchedulerMajorSeparatorCellViewInfo.InitHitTest(AHitTest: TcxSchedulerCustomResourceViewHitTest);
begin
  inherited InitHitTest(AHitTest);
...
function TcxSchedulerTimeGridMoreEventsModernButtonViewInfo.IsModern: Boolean;
begin
  Result := True;
end;
...
function TcxSchedulerTimeBuilder.IsPeriodChanged(const AFirst, ANext: TDateTime): Boolean;
begin
  Result := False;
  case MajorUnit of
    suDay:
...
procedure TcxSchedulerTimeBuilder.Initialize(AView: TcxSchedulerTimeGridView; AWorkTimeOnly, AWorkDaysOnly: Boolean);
begin
...
function TcxSchedulerTimeBuilder.TimeCorrected(var AStart, AFinish: TDateTime; AGoForward: Boolean = True): Boolean;
begin
...
function TcxSchedulerTimeBuilder.CalculateDateTime(const ADateTime: TDateTime; AInc: Integer): TDateTime;
begin
...
begin
  ...
  CheckTime := (dxTimeOf(WorkStart) > 0) or (dxTimeOf(WorkFinish) < AMidnight) and (MinorUnit = suHour);
  WorkFinish := RoundTime(AlignTime(WorkFinish));
  WorkStart := RoundTime(AlignTime(WorkStart));
  CheckMidnight := AIsWorkTimeOnly and CheckTime and (Abs(AMidnight - WorkFinish) <= SecondToTime);
  if CheckMidnight then
    WorkFinish := AMidnight;
end;

procedure TcxSchedulerTimeBuilder.CalculateWorkDays(AIsWorkDaysOnly: Boolean; var AWorkDays: TDays);
const
...
procedure TcxSchedulerTimeBuilder.ValidateMidnightForward(var ADateTime: TDateTime);
const
...
function TcxSchedulerTimeBuilder.ValidateStartTime(const ADateTime: TDateTime): TDateTime;
begin
  Result := ADateTime;
  if CheckTime then
  begin
...
begin
  ...
  Result := (ARoundedDateTime > WorkFinish) or (ARoundedDateTime < WorkStart);
  if Result then
  begin
    ...
    begin
      ...
      begin
        ...
        ADateTime := RoundTime(Trunc(ADateTime) - AInc + Trunc(WorkFinish * AMinorUnitsPerDay) / AMinorUnitsPerDay)
      end
      else
...
procedure TcxSchedulerTimeBuilder.ValidateUnits(var AMajorUnit, AMinorUnit: TcxSchedulerTimeGridScaleUnit);
var
...
procedure TcxSchedulerTimeBuilder.ValidateVisibleStart(var ADateTime: TDateTime);
begin
  case MinorUnit of
    suHour:
    begin
      ADateTime := Trunc(ADateTime) + (Round(dxTimeOf(ADateTime) / MinuteToTime) div TimeScale * TimeScale) *
        MinuteToTime;
    end;
    suDay:
...
procedure TcxSchedulerTimeGridResourceScroll.UnInitScrollBars;
begin
  FScrollBar.UnInitControl;
end;
...
begin
  ...
  begin
    if (FScrollBar <> nil) and (Abs(AValue) > ScrollBar.Max - ScrollBar.PageSize + 1) then
      FShift := -(ScrollBar.Max - ScrollBar.PageSize + 1)
    else
      FShift := AValue;
  end;
end;
