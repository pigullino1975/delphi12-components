...
type
  ...
  private
    ...
    function ConvertToLocalDateTime(ADateTime: TDateTime): TDateTime;
    function ConvertFromLocalDateTime(ADateTime: TDateTime): TDateTime;
    procedure FillDays;
...
const
  dxThisUnitName = 'cxSchedulerRecurrenceEditor';
...
begin
  FLocked := True;
  AStart := ConvertFromLocalDateTime(Event.Start);
  AFinish := ConvertFromLocalDateTime(Event.Finish);
...
var
  AStart, AFinish: TDateTime;
begin
  AStart := ConvertToLocalDateTime(deStart.Date + teStart.Time);
  AFinish := ConvertToLocalDateTime(AStart + GetCurrentDuration);
  //correct event time
  with AInfo.Event do
  begin
...
function TcxSchedulerRecurrenceEventEditorForm.ConvertFromLocalDateTime(ADateTime: TDateTime): TDateTime;
begin
  Result := TdxTimeZoneHelper.ConvertTimeZoneToTimeZone(ADateTime, TdxTimeZoneHelper.CurrentTimeZone, TimeZone, TimeZoneDaylightSaving);
end;

function TcxSchedulerRecurrenceEventEditorForm.ConvertToLocalDateTime(ADateTime: TDateTime): TDateTime;
begin
  Result := TdxTimeZoneHelper.ConvertTimeZoneToTimeZone(ADateTime, TimeZone, TdxTimeZoneHelper.CurrentTimeZone, TimeZoneDaylightSaving);
end;
