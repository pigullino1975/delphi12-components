...
uses
  Types, dxShellDialogs, cxSchedulerStrs, cxSchedulerDialogs, Math, cxSchedulerUtils, cxCustomData;

const
  dxThisUnitName = 'cxSchedulerHolidaysEditor';
...
var
  ...
  AResult: Boolean;
begin
  AExportDialog := TdxSaveFileDialog.Create(Self);
  try
    AExportDialog.Filter := '(*.hol)|*.hol';
    AExportDialog.DefaultExt := '*.hol';
    if Application.ActiveFormHandle <> 0 then
      AResult := AExportDialog.Execute(Application.ActiveFormHandle)
    else
      AResult := AExportDialog.Execute;
...
begin
  AImportDialog := TdxOpenFileDialog.Create(Self);
  try
    AImportDialog.Filter := '(*.hol)|*.hol';
    if AImportDialog.Execute(Handle) then
    begin
      CheckButtonsState(False);
      Cursor := crHourGlass;
      try
