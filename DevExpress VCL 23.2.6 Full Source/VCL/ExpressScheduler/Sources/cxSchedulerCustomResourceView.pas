...
uses
  ...
  DateUtils, Variants, Math, ImgList, Types, Generics.Defaults, Generics.Collections,
  dxCoreClasses, cxClasses, cxGraphics, dxCoreGraphics,
  ...
  cxDateUtils, cxEditUtils, cxSchedulerStrs, cxLookAndFeels, dxGenerics;
...
type
  ...
  TcxSchedulerViewInfoCellList = class(TdxFastObjectList)
  private
  ...
  TcxSchedulerCustomViewInfoItem = class
  private
    FAllowRightToLeftConversion: Boolean;
  ...
  protected
    ...
    // class item settings
    property AllowRightToLeftConversion: Boolean read FAllowRightToLeftConversion write FAllowRightToLeftConversion;
  ...
  protected
    ...
    FAdditionalTimeZoneMinutesOffset: Integer;
 ...
 { TcxSchedulerBackgroundSlotCellViewInfo }
  ...
  public
    ...
    EventHeight: Integer;
    ExternalPainter: TcxSchedulerExternalPainter; // for internal use
  ...
  private
    ...
    function GetActualBorderColorIsNone: Boolean; inline;
  ...
  protected
    ...
    FCaptionRect: TRect;
    FContainerInfo: TObject;
    FDetailCaptionVertOffset: Integer;
    FEditViewInfo: TcxCustomEditViewInfo;
    FEndContinueCaption: string;
    FEndContinueCaptionRect: TRect;
    ...
    FStartContinueCaption: string;
    FStartContinueCaptionRect: TRect;
    ...
    procedure OffsetRects(const DX, DY: Integer); virtual;
    procedure RecalculateVisibility;
    ...
    function GetBoundsAsProgress: TRect; virtual;
    ...
    function GetTimeLineShift: Integer; virtual;
    ...
    procedure DoDraw; override;
    procedure DoDrawAsProgress; virtual;
    ...
    function HasSizingHandlers: Boolean;
    procedure Initialize; virtual;
    procedure InitializeSizingHandlers;
    ...
    property ActualBorderColorIsNone: Boolean read GetActualBorderColorIsNone;
  ...
  public
    ...
    property EventViewData: TcxSchedulerEventViewData read ViewData; // for internal use
  ...
  protected
    ...
    procedure DoDraw; override;
    procedure InitHitTest(AHitTest: TcxSchedulerCustomResourceViewHitTest); override;
    function IsModern: Boolean; virtual;

    property HitTest: TcxSchedulerCustomResourceViewHitTest read FHitTest;
  ...
  protected
    function IsModern: Boolean; override;
  end;
  ...
  private
    ...
    FPainter: TcxCustomLookAndFeelPainter;
    function GetPainterHelper: TcxSchedulerPainterHelperClass;
    procedure SetPainter(const Value: TcxCustomLookAndFeelPainter);
  protected
  ...
  public
    ...
    procedure DrawSchedulerScaledEventGlyph(ACanvas: TcxCanvas; const AGlyphRect: TRect; AImageList: TCustomImageList;
      AImageIndex: TcxImageIndex; AEnabled: Boolean; AScaleFactor: TdxScaleFactor); virtual;
    ...
    procedure AdjustEditStyleViewParams(var AViewParams: TcxViewParams); virtual;
    ...
    function GetGlyphColorPalette(AEnabled: Boolean): IdxColorPalette; virtual;
    ...
    property Painter: TcxCustomLookAndFeelPainter read FPainter write SetPainter;
    property CommonPainter: IcxSchedulerCommonViewItemsPainter read FCommonPainter write FCommonPainter; // for internal use
  end;
  ...
  protected
    ...
    function GetEditStyleViewParams(AEvent: TcxSchedulerControlEvent): TcxViewParams; override;
  ...
  public
    ...
    procedure GetPageResources(AResources: TdxFastObjectList); virtual;
  ...
  private
    ...
    FIsNavigationButtonsCollapsed: Boolean;
    FIsNavigationButtonsHeightCalculated: Boolean;
    FCalculationCounter: Integer;
    ...
    FNavigationButtonHeight: Integer;
    ...
    function GetHiddenSelection: Boolean; inline;
    function GetMinResourceHeight: Integer;
    function GetResource(AIndex: Integer): TcxSchedulerResourceViewInfo; inline;
    function GetResourceCaption(AIndex: Integer): string;
    function GetResourceCount: Integer; inline;
    ...
    function GetView: TcxSchedulerCustomResourceView; inline;
    function HasMoreEventsButton(AResourceIndex: Integer): Boolean;
  ...
  protected
    ...
    function AddButton(AColumnIndex: Integer; ABounds: TRect; const ADateTime: TDateTime;
    ...
    procedure CalculateNavigationButtonHeight(const AColumnRect: TRect);
    procedure CalculateNavigationButtonParams(const AColumnRect: TRect; AKind: TcxSchedulerContentNavigationButtonKind;
    ...
    procedure ClearEventCells; virtual;
    procedure ClearNavigationButtons;
    ...
    procedure DoRightToLeftConversion(const AClientBounds: TRect); virtual;
    function DoSetResourceHasVisibleEvent(AEvent: TcxSchedulerControlEvent): Boolean; virtual;
    ...
    function HasEventForResource(AResourceIndex: Integer): Boolean; virtual;
    ...
    function IsTimeSelected(ATime: TDateTime; AResource: TObject): Boolean;
    ...
    procedure ProcessPreviouslyCalculatedEvents; virtual;
    procedure CustomProcessPreviouslyCalculatedEvent(AEventCell: TcxSchedulerEventCellViewInfo); virtual;
    procedure ResourceHeadersChanged;
    ...
    property IsNavigationButtonsCollapsed: Boolean read FIsNavigationButtonsCollapsed;
    property IsNavigationButtonsHeightCalculated: Boolean read FIsNavigationButtonsHeightCalculated;
    ...
    property NavigationButtonHeight: Integer read FNavigationButtonHeight;
  ...
  public
    ...
    property ContentNavigationButton: TcxSchedulerContentNavigationButtonViewInfo read FContentNavigationButton; // for internal use
 ...
 { TcxSchedulerCustomViewPainter }
  ...
  TcxSchedulerEventLayoutBuilder = class
  strict private
    FEventPlaces: TdxObjectSortedList<TcxSchedulerEventPlace>;
    FViewStyle: TcxSchedulerViewStyle;

    function GetEventPlace(AIndex: Integer): TcxSchedulerEventPlace; inline;
    function GetEventPlaceCount: Integer; inline;
  protected
    function CreateComparer: IComparer<TcxSchedulerEventPlace>; virtual;

    property ViewStyle: TcxSchedulerViewStyle read FViewStyle;
  public
    constructor Create(const AViewStyle: TcxSchedulerViewStyle); virtual;
    destructor Destroy; override;
...
uses
  RTLConsts, cxDrawTextUtils, cxLibraryConsts, cxVariants, cxSchedulerDateNavigator,
  dxCore, cxFormats, dxDPIAwareUtils, dxOffice11,
  dxTypeHelpers;

const
  dxThisUnitName = 'cxSchedulerCustomResourceView';
...
type
  ...
  TcxSchedulerResourceNavigatorAccess = class(TcxSchedulerResourceNavigator);
...
type
  { TcxSchedulerEventPlaceComparer }

  TcxSchedulerEventPlaceComparer = class(TInterfacedObject, IComparer<TcxSchedulerEventPlace>)
  strict private
    FViewStyle: TcxSchedulerViewStyle;
  protected
    function Compare(const Left, Right: TcxSchedulerEventPlace): Integer;
  public
    constructor Create(AViewStyle: TcxSchedulerViewStyle);
  end;

  { TcxSchedulerOccupiedArea }

  TcxSchedulerOccupiedArea = class
  private
    FBounds: TRect;

    function LeftOf(const ABounds: TRect): Boolean; inline;
    function RightOf(const ABounds: TRect): Boolean; inline;
    function TopOf(const ABounds: TRect): Boolean; inline;
    function BottomOf(const ABounds: TRect): Boolean; inline;
    function CanMergeHorizontal(const ABounds: TRect): Boolean; inline;
    function CanMergeVertical(const ABounds: TRect): Boolean; inline;
  public
    constructor Create(const ABounds: TRect);
    function Merge(const ABounds: TRect): Boolean; overload;
    function Merge(AOccupiedArea: TcxSchedulerOccupiedArea): Boolean; overload;

    property Bounds: TRect read FBounds;
  end;

  { TcxSchedulerOccupiedAreaManager }

  TcxSchedulerOccupiedAreaManager = class
  private
    FAreas: TdxFastObjectList;

    function GetItem(AIndex: Integer): TcxSchedulerOccupiedArea; inline;
  protected
    function MergeRecursively(var AArea: TcxSchedulerOccupiedArea): Boolean;
  public
    constructor Create;
    destructor Destroy; override;
    function AddArea(const ABounds: TRect): TcxSchedulerOccupiedArea;
    function GetFreeBounds(const AEventPlaceBounds: TRect): TRect;

    property Items[Index: Integer]: TcxSchedulerOccupiedArea read GetItem;
  end;
...
begin
  if AEventPlace1 = AEventPlace2 then
    Exit(0);
    ...
    if AEventPlace1.ColStart > AEventPlace2.ColStart then
      ...
      if AEventPlace1.ColFinish > AEventPlace2.ColFinish then
        ...
        begin
          if AAllDay1 then
          begin
            if Int(AEvent1.Start) < Int(AEvent2.Start) then  
              Result := -1
            else
            if Int(AEvent1.Start) > Int(AEvent2.Start) then
              Result := 1
            else
              if Int(AEvent1.Finish) > Int(AEvent2.Finish) then
                Result := -1
              else
                if Int(AEvent1.Finish) < Int(AEvent2.Finish) then
                  Result := 1
...
constructor TcxSchedulerEventPlaceComparer.Create(AViewStyle: TcxSchedulerViewStyle);
begin
  inherited Create;
  FViewStyle := AViewStyle;
end;

function TcxSchedulerEventPlaceComparer.Compare(const Left, Right: TcxSchedulerEventPlace): Integer;
begin
  if FViewStyle = svsClassic then
    Result := cxCompareEventPlaces(Left, Right)
  else
    Result := cxCompareEventModernPlaces(Left, Right);
end;

{ TcxSchedulerOccupiedArea }

constructor TcxSchedulerOccupiedArea.Create(const ABounds: TRect);
begin
  FBounds := ABounds;
end;

function TcxSchedulerOccupiedArea.LeftOf(const ABounds: TRect): Boolean;
begin
  Result := FBounds.Right + 1 = ABounds.Left;
end;

function TcxSchedulerOccupiedArea.RightOf(const ABounds: TRect): Boolean;
begin
  Result := FBounds.Left - 1 = ABounds.Right;
end;

function TcxSchedulerOccupiedArea.TopOf(const ABounds: TRect): Boolean;
begin
  Result := FBounds.Bottom + 1 = ABounds.Top;
end;

function TcxSchedulerOccupiedArea.BottomOf(const ABounds: TRect): Boolean;
begin
  Result := FBounds.Top - 1 = ABounds.Bottom;
end;

function TcxSchedulerOccupiedArea.CanMergeHorizontal(const ABounds: TRect): Boolean;
begin
  Result := (FBounds.Top = ABounds.Top) and (FBounds.Bottom = ABounds.Bottom) and (RightOf(ABounds) or LeftOf(ABounds));
end;

function TcxSchedulerOccupiedArea.CanMergeVertical(const ABounds: TRect): Boolean;
begin
  Result := (FBounds.Left = ABounds.Left) and (FBounds.Right = ABounds.Right) and (BottomOf(ABounds) or TopOf(ABounds));
end;

function TcxSchedulerOccupiedArea.Merge(const ABounds: TRect): Boolean;
begin
...
function TcxSchedulerOccupiedArea.Merge(AOccupiedArea: TcxSchedulerOccupiedArea): Boolean;
begin
...
constructor TcxSchedulerOccupiedAreaManager.Create;
begin
  inherited Create;
  FAreas := TdxFastObjectList.Create;
end;

destructor TcxSchedulerOccupiedAreaManager.Destroy;
begin
  FAreas.Free;
  inherited Destroy;
end;

function TcxSchedulerOccupiedAreaManager.AddArea(const ABounds: TRect): TcxSchedulerOccupiedArea;
var
  I: Integer;
  AOldArea: TcxSchedulerOccupiedArea;
begin
  Result := nil;
  ...
  if Result = nil then
  begin
    Result := TcxSchedulerOccupiedArea.Create(ABounds);
    FAreas.Add(Result);
  end
  else
...
function TcxSchedulerOccupiedAreaManager.MergeRecursively(var AArea: TcxSchedulerOccupiedArea): Boolean;
var
  I: Integer;
  AOldArea: TcxSchedulerOccupiedArea;
begin
...
function TcxSchedulerOccupiedAreaManager.GetItem(AIndex: Integer): TcxSchedulerOccupiedArea;
begin
  Result := TcxSchedulerOccupiedArea(FAreas[AIndex]);
end;

function TcxSchedulerOccupiedAreaManager.GetFreeBounds(const AEventPlaceBounds: TRect): TRect;
var
  I: Integer;
  AOccupiedArea: TcxSchedulerOccupiedArea;
begin
  Result := AEventPlaceBounds;
  for I := 0 to FAreas.Count - 1 do
  begin
...
begin
  for I := 0 to Count - 1 do
    if Items[I].AllowRightToLeftConversion then
...
begin
  ...
  begin
    AItem := Items[I];
    if AItem.FVisible and not AItem.ActiveBounds.IsEmpty and ACanvas.RectVisible(AItem.ActiveBounds) then
    begin
...
begin
  ...
  FAllowRightToLeftConversion := AUseRightToLeftAlignment;
...
begin
  ...
  FVisible := not FClipRect.IsEmpty;
...
procedure TcxSchedulerCustomViewInfoItem.MakeCacheDirty;
begin
  FCache.SetDirty;
end;
...
begin
  ...
  FTextRect := FBounds;
  FTextRect.Deflate(ScaleFactor.Apply(cxTextOffset), 0);
end;
...
//    Canvas.TextExtent(DisplayText, FTextRect, GetTextOutcxFlags);
    FTextRect.Right := FTextRect.Left + Resource.FTextRect.Width;
    FTextRect.Bottom := FTextRect.Top + Resource.FTextRect.Height;
  end
  else
    FTextRect := cxNullRect;
  ATextSize := FTextRect.Size;
  ATopLeft := cxPointOffset(ABounds.CenterPoint, ScaleFactor.Apply(1), -ScaleFactor.Apply(1) + Byte(RotateHeader));
...
begin
  ...
  try
    ...
    begin
      if RotateText then
      begin
        AImageBounds.InitSize(0, 0, FImageRect.Height, FImageRect.Width);
        ...
        try
          ...
          TdxImageDrawer.DrawImage(ABitmap.cxCanvas, AImageBounds, nil, Images, ImageIndex, ifmNormal, idmNormal, True, nil, ScaleFactor);
          ABitmap.Rotate(raMinus90);
        ...
        TdxImageDrawer.DrawImage(ACanvas, FImageRect, nil, Images, ImageIndex, ifmNormal, idmNormal, True, nil, ScaleFactor);
    end;
  finally
...
begin
  AAvailableWidth := FTextRect.Width;
  if AAvailableWidth <= 0 then
    Exit(AType);
  AAvailableWidth := AAvailableWidth - ScaleFactor.Apply(cxTextOffset);
  Result := AType - 1;
  repeat
    Inc(Result);
    FDisplayText := DateTimeHelper.DayToStr(FDateTime, Result, Compressed);
  until (Result = DateTimeHelper.DayToStrFormatCount - 1) or (TdxTextMeasurer.TextWidthTO(Font, DisplayText) <= AAvailableWidth);
end;
...
begin
  ...
  if AAvailableWidth <= 0 then
    Exit(AType);
  AAvailableWidth := AAvailableWidth - ScaleFactor.Apply((cxTimeLineWidth * 3) div 2 + cxTextOffset);
  Result := AType - 1;
  case ActualModernStyleDisplayMode of
    hdmDefault:
      ...
      until (Result = 3) or (TdxTextMeasurer.TextWidthTO(Font, DisplayText) <= AAvailableWidth);
  else
    repeat
      Inc(Result);
      ADate := AnsiUpperCase(DateTimeHelper.DayToStr(FDateTime, Result, Compressed));
      ADay := '';
      if Pos('DDD', AnsiUpperCase(DateTimeHelper.GetDayMonthFormat(Result, False))) = 0 then
    ...
    until (Result = DateTimeHelper.DayToStrFormatCount - 1) or
      (TdxTextMeasurer.TextWidthTO(Font, DisplayText) <= AAvailableWidth);
  end;
end;
...
begin
  ...
  until (Result = 3) or (TdxTextMeasurer.TextWidthTO(Font, DisplayText) <= FTextRect.Width);
...
begin
  ...
  if FVisible or (FAdditionalTimeZoneMinutesOffset <> 0) then
...
begin
  ...
  begin
    FBounds[True].Left := inherited Bounds.CenterPoint.X;
...
procedure TcxSchedulerTimeRulerCellViewInfo.CheckActiveClipping;
var
  ABounds: TRect;
begin
  ABounds := ActiveBounds;
  FClipRect := CalculateClipRect(ABounds, VisibleRect);
  FVisible := not FClipRect.IsEmpty;
  FHasClipping := FVisible and not cxRectIsEqual(FClipRect, ABounds);
end;
...
begin
  LH := inherited Bounds.Height div LineCount;
  R := ABounds;
  cxRightToLeftDependentDraw(Canvas.Handle, ABounds, Canvas.UseRightToLeftAlignment,
    procedure
    begin
...
function TcxSchedulerTimeRulerCellViewInfo.GetActiveBounds: TRect;
begin
  Result := inherited GetActiveBounds;
  Result.Top := Min(Bounds[False].Top, Bounds[True].Top);
  Result.Bottom := Max(Bounds[False].Bottom, Bounds[True].Bottom);
end;
...
begin
  if ALineCount = 1 then
    ...
    Result := TcxSchedulerPainterHelper.TextWidth(AFont1, ' 24') +
      TdxTextMeasurer.TextWidthTO(AFont2, 'pm') + AScaleFactor.Apply(cxTextOffset);
end;
...
begin
  ...
  if Hour >= 0 then
  begin
    LH := inherited Bounds.Height div FLineCount;
...
procedure TcxSchedulerTimeRulerModernCellViewInfo.DrawTimeZone(const ABounds: TRect; const AText: string; AIsCurrent: Boolean);
  ...
  begin
    ...
    if not Is24HourTimeFormat then
      Result := TdxTextMeasurer.TextWidthTO(Font, TcxSchedulerDateTimeHelper.GetMeridiem(True, True)) + ScaleFactor.Apply(cxTextOffset);
    if ShowMinutes then
...
var
  ...
  st: string;
begin
  LH := ABounds.Height div LineCount;
  R := ABounds;
  cxRightToLeftDependentDraw(Canvas.Handle, ABounds, Canvas.UseRightToLeftAlignment,
    procedure
    begin
...
begin
  if not Is24HourTimeFormat or AShowMinutes then
    Result := TcxSchedulerPainterHelper.TextWidth(AFont2, ' 24') + TdxTextMeasurer.TextWidthTO(AFont1, 'PM') + AScaleFactor.Apply(cxTextOffset)
  else
...
begin
  ...
  begin
    ...
    begin
      ...
      if InRange(Bounds[True].Top, FVisibleRect.Top, FVisibleRect.Top + Bounds[True].Height - 1) then
...
begin
  ...
  if TdxTextMeasurer.TextWidthTO(Font, Result) >= (FTextRect.Width div 2) then
...
const
  LeftBorders: array[Boolean] of TcxBorders = ([bLeft], [bRight]);
begin
  ...
  if HasLeftBorder and not ActualBorderColorIsNone then
    DrawFrame(Bounds, LeftBorders[UseRightToLeftAlignment], BorderColor, BorderSize);
...
begin
  ...
  ShiftRect(FStartContinueCaptionRect, X, Y);
  ShiftRect(FEndContinueArrowRect, X, Y);
  ShiftRect(FEndContinueCaptionRect, X, Y);
...
procedure TcxSchedulerEventCellViewInfo.AssignEditStyle(AEditStyle: TcxCustomEditStyle);
var
...
begin
  ...
  AEditViewData := AProperties.CreateViewData(ViewData.EditStyle, True, ScaleFactor);
  try
...
begin
  ...
  if not IsDrawBySkin and not TimeLineRect.IsZero then
...
function TcxSchedulerEventCellViewInfo.CalculateAutoLayoutImagesAutoHeight(AVisibleImages: TList): Integer;

  function GetEventTextsHeight(const AWidth: Integer): Integer;
  var
    R: TRect;
  begin
    R.InitSize(0, 0, AWidth, MaxInt);
...
begin
  ASpaceWidth := GetAvailableBounds.Width - 2 * ScaleFactor.Apply(cxTextOffset);
...
begin
  ...
  begin
    ...
    Inc(FStartContinueArrowRect.Left, ScaleFactor.Apply(cxTextOffset));
    ...
    begin
      FStartContinueCaption := cxGetResourceString(@scxContinueFrom) + ' ' +
        FormatDateTime(GetContinueArrowCaptionDateFormat, EventStart);
      FStartContinueCaptionRect := FStartContinueArrowRect;
      FStartContinueCaptionRect.Left := FStartContinueArrowRect.Right + ScaleFactor.Apply(cxTextOffset);
      FStartContinueCaptionRect.Right := FStartContinueCaptionRect.Left +
        TdxTextMeasurer.TextWidthTO(Canvas.Font, FStartContinueCaption);
    end;
  end;
  if NeedHeaderEventFinishContinueArrow then
  begin
    FEndContinueArrowRect := GetTimeLineRectBaseBounds;
    Dec(FEndContinueArrowRect.Right, ScaleFactor.Apply(cxTextOffset));
    ...
    if ACompressing = 0 then
    begin
      FEndContinueCaption := cxGetResourceString(@scxContinueTo) + ' ' +
        FormatDateTime(GetContinueArrowCaptionDateFormat, IfThen(TimeOf(EventFinish) > 0, EventFinish, EventFinish - 1));
      FEndContinueCaptionRect := FEndContinueArrowRect;
      FEndContinueCaptionRect.Right := FEndContinueArrowRect.Left - ScaleFactor.Apply(cxTextOffset);
      FEndContinueCaptionRect.Left := FEndContinueCaptionRect.Right -
        TdxTextMeasurer.TextWidthTO(Canvas.Font, FEndContinueCaption);
    end;
  end;
end;
...
var
  ...
  AMessageTop, AImagesWidth, ATextOffset: Integer;
  AImagesRect, R: TRect;
begin
  ATextOffset := ScaleFactor.Apply(cxTextOffset);
  Result := Images.ItemHeight + ATextOffset;
  ASpaceWidth := Bounds.Width - 2 * ATextOffset;
  ACaptionWidth := TdxTextMeasurer.TextWidthTO(ViewParams.Font, Caption);
  AImagesRect := cxRectInflate(Bounds, -ATextOffset, -ATextOffset);
  ...
  begin
    ...
    AMessageTop := Bounds.Top + ATextOffset + ScaleFactor.Apply(cxEventImagesGap) + Result;
  end
  else
  begin
    ARightLimit := 0;
    ...
    begin
      ...
      begin
        ACaptionWidth := R.Width;
        Dec(AImagesRect.Right, ACaptionWidth);
        ARightLimit := Bounds.Right - ATextOffset - R.Width;
      end;
    end;
    ...
    Result := Max(Result, ACaptionHeight + 2 * ATextOffset);
...
begin
  ...
  begin
    ...
    if FCaptionAndLocationCanPlaceOneLineOnly then
      ACaptionHeight := Max(ACaptionHeight,
        IfThen((FLocation <> '') and (FLocationRect.Height > 0), FLocationRect.Height,
          ACaptionHeight + 2 * (ScaleFactor.Apply(cxTextOffset) + ScaleFactor.Apply(1))))
    else
      ACaptionHeight := Max(CaptionRect.Height, ACaptionHeight + FLocationRect.Height);
    AHeight := Max(AHeight, ACaptionHeight);
    if FShowMessage then
    begin
      AMessageTop := FCaptionRect.Top + ACaptionHeight - ScaleFactor.Apply(1);
      if FCaptionAndLocationCanPlaceOneLineOnly and (Images.Layout = eilVertical) and
        ((ViewData.EventHeight <> 0) or (FLocation = '')) then
          Inc(AMessageTop, ScaleFactor.Apply(1));
      FMessageRect.Init(R.Left - ScaleFactor.Apply(2), AMessageTop, Bounds.Right - ScaleFactor.Apply(1), 0);
...
begin
  ...
  begin
    ...
    if (AImagesRect.Width > Images.ItemWidth + ScaleFactor.Apply(cxEventImagesGap)) or
...
begin
  AFullWidth := TdxTextMeasurer.TextWidthTO(Canvas.Font, ViewData.Caption);
  ACaptionOnlyWidth := TdxTextMeasurer.TextWidthTO(Canvas.Font, Event.Caption);
end;

procedure TcxSchedulerEventCellViewInfo.CalculateHeaderEventLayout;
var
  ACaptionRect: TRect;
  ATextOffset, ALeft, AWidth, AImagesWidth, AFullCaptionWidth, ACaptionOnlyWidth, AVertOffset, ACompressing: Integer;
...
begin
  ...
  begin
    ...
    if (ACaptionOnlyWidth > ACaptionRect.Width) and (ACompressing < 2) then
      Continue;
    ...
    if (ShowStartTime or ShowFinishTime) and
      (AFullCaptionWidth + Images.TotalVisibleWidth > ACaptionRect.Width) and
    ...
    if (ACaptionOnlyWidth <= ACaptionRect.Width) or
...
begin
  Result := TdxTextMeasurer.TextWidthTO(ViewParams.Font, ViewData.Caption);
end;
...
var
  ...
  AVertOffset: Integer;
begin
  R.InitSize(0, 0, TcxSchedulerPainterHelper.IconsWidth(ScaleFactor),
...
var
  ...
  R: TRect;
begin
  ASpaceWidth := ABounds.Width - AImagesWidth;
  with Canvas.TextExtent('00:00') do
    R.InitSize(ABounds.Left, 0, CX, CY);
  if not Is24HourTimeFormat then
    R.Width := GetTimeTextMaxWidth;
  AWidth := R.Width;
  if ASpaceWidth >= AWidth then
  begin
    FStartRect.Init(ALeft, FCaptionRect.Top, ALeft + AWidth, FCaptionRect.Bottom);
...
begin
  ...
  if not ActualBorderColorIsNone and (ATop + AHeight >= Bounds.Bottom) then
    Dec(ATop, ATop + AHeight - Bounds.Bottom + 1);
...
begin
  Result := (ARect.Width >= EventImages.Width + ScaleFactor.Apply(cxEventImagesGap)) and ViewData.ShowTimeAsClock;
end;
...
begin
  ...
  begin
    ...
    if ShowStartTime then
      AWidth := L + ScaleFactor.Apply(cxTextOffset) * 2;
    if ShowFinishTime then
      Inc(AWidth, L + ScaleFactor.Apply(cxTextOffset) * 2);
    if Length(Caption) > 0 then
      Inc(AWidth, 2 * TdxTextMeasurer.TextWidthTO(Canvas.Font, Caption[1]));
    Result := ARect.Width > AWidth;
  end;
end;
...
begin
  ...
  DoRightToLeftConversionWithChecking(FEndContinueCaptionRect, AClientBounds);
  ...
  DoRightToLeftConversionWithChecking(FStartContinueCaptionRect, AClientBounds);
  ...
  DoRightToLeftConversionWithChecking(ViewData.Bounds, AClientBounds);
...
begin
  Result := inherited GetActiveBounds;
  if not TimeLineRect.IsZero then
    if UseRightToLeftAlignment then
...
function TcxSchedulerEventCellViewInfo.GetBoundsAsProgress: TRect;
begin
  Result := Bounds;
end;
...
begin
  ...
  if FStartContinueArrowRect.IsEmpty and FEndContinueArrowRect.IsEmpty then
  ...
  begin
    ...
    if not FStartContinueCaptionRect.IsEmpty then
      Result.Left := FStartContinueCaptionRect.Right + AOffset
    else
      if not FStartContinueArrowRect.IsEmpty then
        Result.Left := FStartContinueArrowRect.Right + AOffset
      else
        Result.Left := ABounds.Left + AOffset;

    if not FEndContinueCaptionRect.IsEmpty then
      Result.Right := FEndContinueCaptionRect.Left - AOffset
    else
      if not FEndContinueArrowRect.IsEmpty then
...
begin
  ...
  if FShowMessage then
  begin
    FMessageRect.Init(GetMessageRectLeft, AMessageTop - ScaleFactor.Apply(1), Bounds.Right - ScaleFactor.Apply(1), 0);
    AMessageHeight := CalculateMessageHeight(FMessageRect);
    FMessageRect.Bottom := FMessageRect.Top + AMessageHeight;
...
begin
  Result := cxRectSetLeft(cxRectInflate(ACaptionRect, -ScaleFactor.Apply(1), -ScaleFactor.Apply(1)), 0,
    ACaptionRect.Width - (ATextWidth + ScaleFactor.Apply(cxTextOffset)));
end;
...
begin
  ...
  begin
    ...
    if UseRightToLeftAlignment and not AllowRightToLeftConversion and ((Result.Left <> 0) or (Result.Right <> 0)) then
...
begin
  Result := FBounds.Top + GetTimeLineShift;
  if not ActualBorderColorIsNone then
    Inc(Result, BorderSize);
end;

function TcxSchedulerEventCellViewInfo.GetTimeLineShift: Integer;
begin
  Result := 0;
end;
...
begin
  ...
  FStartContinueCaption := '';
  FEndContinueCaption := '';
  FStartContinueCaptionRect := cxNullRect;
  FEndContinueCaptionRect := cxNullRect;
end;
...
begin
  ...
  if AShowItem then
  begin
    if ((APos + ABounds.Width) <= FBounds.CenterPoint.X) then
    begin
...
begin
  FCaptionRect := cxRectInflate(GetAvailableBounds,
    -(ScaleFactor.Apply(cxTextOffset) + ScaleFactor.Apply(1)), -FDetailCaptionVertOffset);
end;
...
begin
  Result := (ViewStyle = svsModern) or
    (not Event.AllDayEvent and not IsDetailInfo and (FinishRect.IsEmpty or StartRect.IsEmpty));
end;
...
procedure TcxSchedulerEventCellViewInfo.DoDrawAsProgress;
begin
...
begin
  if FCaptionRect.IsEmpty or ((Caption = '') and (Event.Location = '')) then Exit;
  Canvas.Brush.Style := bsClear;
...
begin
  ...
  if FStartContinueCaption <> '' then
    cxTextOut(Canvas.Canvas, FStartContinueCaption, FStartContinueCaptionRect, GetCaptionFlags,
      0, 0, Canvas.Font, clNone, clNone, 0, 0, 0, ViewParams.TextColor);
  if not FEndContinueArrowRect.IsEmpty then
    DoDrawEndContinueArrow;
  if FEndContinueCaption <> '' then
    cxTextOut(Canvas.Canvas, FEndContinueCaption, FEndContinueCaptionRect, GetCaptionFlags,
      0, 0, Canvas.Font, clNone, clNone, 0, 0, 0, ViewParams.TextColor);
end;
  ...
  begin
    ...
    for I := 0 to Images.Count - 1 do
      if Images.Items[I].Visible and not FHidden then
        ExternalPainter.DrawSchedulerScaledEventGlyph(Self.Canvas, Images.Items[I].Bounds, Images.Items[I].Images,
          Images.Items[I].ImageIndex, Enabled, ScaleFactor);
...
begin
  if (FLocation <> '') and (LocationRect.Width > 0) then
...
procedure TcxSchedulerEventCellViewInfo.OffsetRects(const DX, DY: Integer);
begin
  OffsetRect(ViewData.Bounds, DX, DY);
  ...
  ShiftRect(FSelectionBorderRect, DX, DY);
  ShiftRect(FSizingHandlerRect1, DX, DY);
  ShiftRect(FSizingHandlerRect2, DX, DY);
  Images.Offset(DX, DY);
end;

procedure TcxSchedulerEventCellViewInfo.Scroll(const DX, DY: Integer);
begin
  OffsetRects(DX, DY);
  RecalculateVisibility;
end;

procedure TcxSchedulerEventCellViewInfo.RecalculateVisibility;
var
  ABounds: TRect;
begin
  FreeAndNil(FClipRgn);
...
begin
  if not FSizingHandlerRect1.IsZero then
    AClipRgn.Combine(FSizingHandlerRect1, roSubtract);
  if not FSizingHandlerRect2.IsZero then
...
begin
  ...
  if (Caption <> '') and not CaptionRect.IsEmpty then
...
function TcxSchedulerEventCellViewInfo.GetEnabled: Boolean;
begin
...
begin
  ...
  if Result and not Hidden and ShowTimeLine and Selected then
    ABreak := not PtInRect(CaptionRect, AHitTest.HitPoint) and not PtInRect(MessageRect, AHitTest.HitPoint)
  else
    ABreak := False;
end;
...
begin
  Result.Init(Bounds.Left + ScaleFactor.Apply(cxEventImagesOffset) + cxEventBorderWidth - Byte(ViewData.ShowTimeLine),
...
begin
  ...
  if not FIsDetailCaption then
    Result := Max(Result, (Bounds.Height - 2 * cxEventBorderWidth - AImageHeight) div 2);
  if AIsAbsolute then
...
function TcxSchedulerEventCellViewInfo.GetMessageRect(const ACaptionRect: TRect; AHasImages: Boolean): TRect;
var
  ACaptionBottom: Integer;
  R: TRect;
begin
  if not ACaptionRect.IsEmpty then
  begin
...
begin
  Result := ALeftBorderRTL[UseRightToLeftAlignment and AllowRightToLeftConversion and (FCalculationCounter = 0)] in Borders;
end;
...
begin
  Result := ARightBorderRTL[UseRightToLeftAlignment and AllowRightToLeftConversion and (FCalculationCounter = 0)] in Borders;
end;
...
function TcxSchedulerEventCellViewInfo.HasSizingHandlers: Boolean;
begin
  Result := not(FSizingHandlerRect1.IsZero and FSizingHandlerRect2.IsZero);
end;
...
procedure TcxSchedulerEventCellViewInfo.InitializeSizingHandlers;
begin
...
procedure TcxSchedulerEventCellViewInfo.InitHitTest(AHitTest: TcxSchedulerCustomResourceViewHitTest);
begin
...
begin
  if not R.IsEmpty then OffsetRect(R, X, Y);
end;
...
begin
  AEditViewData := ViewData.EditProperties.CreateViewData(ViewData.EditStyle, True, ScaleFactor);
  try
    AssignEditStyle(ViewData.EditStyle);
    AEditViewData.ContentOffset := cxSimpleRect;
    ASize.InitValues(0, -ScaleFactor.Apply(1), R.Width);
...
function TcxSchedulerEventCellViewInfo.GetActualBorderColorIsNone: Boolean;
begin
  Result := ViewData.BorderColor = clNone;
end;
...
begin
  Result := R;
  if not LocationRect.IsZero and (LocationRect.Top > CaptionRect.Top) then
...
begin
  if not IsModern then
    ExternalPainter.DrawSchedulerScaledEventGlyph(Canvas, Bounds, MoreEventButtonGlyphs, Byte(not IsDown), True, ScaleFactor)
  else
...
function TcxSchedulerMoreEventsButtonViewInfo.IsModern: Boolean;
begin
  Result := False;
end;
...
function TcxSchedulerMoreEventsModernButtonViewInfo.IsModern: Boolean;
begin
  Result := True;
end;
...
begin
  ...
  Y := Round(ABounds.Top + (ANow * ABounds.Height) / HourToTime);
  Dec(Y, 3);
  R := ABounds;
  Dec(R.Right);
...
procedure TcxSchedulerExternalPainter.SetPainter(const Value: TcxCustomLookAndFeelPainter);
begin
...
procedure TcxSchedulerExternalPainter.DrawEventAsProgress(AViewInfo: TcxSchedulerEventCellViewInfo);
var
  ABounds, AContent, AProgressRect: TRect;
begin
  with AViewInfo do
  begin
    ABounds := GetBoundsAsProgress;
    AContent := cxRectContent(ABounds, Painter.SchedulerEventProgressOffsets);
    AProgressRect := AContent;
    if not AViewInfo.UseRightToLeftAlignment then
      AProgressRect.Right := AProgressRect.Left + Trunc(ViewData.TaskComplete / 100 * AProgressRect.Width)
    else
      AProgressRect.Left := AProgressRect.Right - Trunc(ViewData.TaskComplete / 100 * AProgressRect.Width);

    Painter.DrawSchedulerScaledEventProgress(Canvas, ABounds, AProgressRect, ViewParams, Transparent, ScaleFactor);
    Canvas.Brush.Style := bsClear;
    Canvas.Font.Color := Painter.ProgressBarTextColor;
    DrawEventAsProgressText(AViewInfo, AContent, AProgressRect, IntToStr(ViewData.TaskComplete) + '%');
  end;
end;
...
begin
  Painter.DrawProgressBarText(AViewInfo.Canvas, AText, AContent, AProgressRect, cxNullRect, cxNullRect);
end;
...
procedure TcxSchedulerExternalPainter.DrawSchedulerScaledEventGlyph(ACanvas: TcxCanvas; const AGlyphRect: TRect;
  AImageList: TCustomImageList; AImageIndex: TcxImageIndex; AEnabled: Boolean; AScaleFactor: TdxScaleFactor);
begin
  TdxImageDrawer.DrawImage(ACanvas, AGlyphRect, nil, AImageList, AImageIndex, AEnabled, GetGlyphColorPalette(AEnabled), AScaleFactor);
end;
...
procedure TcxSchedulerExternalPainter.AdjustEditStyleViewParams(var AViewParams: TcxViewParams);
begin
  // do nothing
end;

procedure TcxSchedulerExternalPainter.FlushCache;
begin
  // do nothing
end;
...
var
  ...
  ASourceHSL: TdxHSL;
begin
  AEventHSL := TdxColorUtils.ColorToHSL(AEvent.GetTimeLineBorderColor);
  ASourceHSL := TdxColorUtils.ColorToHSL(ASource);
  ...
  Result := TdxColorUtils.HSLToColor(ASourceHSL);
end;
...
begin
  ...
  if (AViewInfo <> nil) and (AViewInfo.ViewStyle = svsModern) then
    Result.Init(0, ABorderSize - 1, 0, ABorderSize - 1)
  else
    Result.Init(ABorderSize, ABorderSize, ABorderSize, ABorderSize);
end;

function TcxSchedulerExternalPainter.GetGlyphColorPalette(AEnabled: Boolean): IdxColorPalette;
begin
  Result := nil;
end;
...
constructor TcxSchedulerCustomResourceView.Create(AOwner: TcxCustomScheduler);
begin
...
begin
  ...
  for I := 0 to ViewInfo.EventCells.Count - 1 do
    if ViewInfo.EventCells[I].Event = AEvent then
    begin
      Result := True;
      AViewInfo := ViewInfo.EventCells[I];
      if (AResource = nil) or (AViewInfo.ResourceItem = AResource) then
        if (ADate = NullDate) or ((dxDateOf(AViewInfo.ContentStart) = ADate) or (AViewInfo.IsHeaderEvent and
          (ADate >= dxDateOf(AViewInfo.ContentStart)) and (ADate < dxDateOf(AViewInfo.ContentFinish)))) then Break;
    end;
end;
...
function TcxSchedulerCustomResourceView.GetEditStyleViewParams(AEvent: TcxSchedulerControlEvent): TcxViewParams;
begin
  Result := inherited GetEditStyleViewParams(AEvent);
  if ExternalPainter <> nil then
    ExternalPainter.AdjustEditStyleViewParams(Result);
end;
...
function TcxSchedulerCustomResourceView.GetEventVisibility(AEvent: TcxSchedulerControlEvent): Boolean;
var
  AViewInfo: TcxSchedulerEventCellViewInfo;
begin
  Result := FindEventViewInfo(AEvent, NullDate, nil, AViewInfo) and AViewInfo.Visible;
end;
...
begin
  Result := Assigned(FOnCustomDrawGroupSeparator) or ExternalPainter.HasCustomDrawGroupSeparator;
end;

function TcxSchedulerCustomResourceView.HasCustomDrawResourceHeader: Boolean;
begin
  Result := Assigned(FOnCustomDrawResourceHeader) or ExternalPainter.HasCustomDrawResourceHeader;
end;
...
begin
  TcxSchedulerResourceNavigatorAccess(ResourceNavigator).UpdateViewData;
...
begin
  ...
  Result := ViewInfo.GetFontHeight(StylesAdapter.GetEventParams(nil)) + 3 * ViewInfo.ScaleFactor.Apply(cxTextOffset) +
    TdxPadding(ViewInfo.LookAndFeelPainter.GetEditorPadding(True, ViewInfo.ScaleFactor)).Height;
...
begin
  ...
  AItem.FBounds.InitSize(ALeft, ATop, AItem.Width, AItem.Height);
end;
...
begin
  Result := not FDirty and (Image.Width = AItem.Width) and (Image.Height = AItem.Height);
end;

procedure TcxSchedulerCachedImage.SetDirty;
begin
  FDirty := True;
end;
...
begin
  if not AItem.Bounds.IsEmpty then
  begin
...
begin
  ...
  FResources := TdxFastObjectList.Create;
...
function TcxSchedulerCustomResourceViewViewInfo.AddButton(
  AColumnIndex: Integer; ABounds: TRect; const ADateTime: TDateTime;
...
function TcxSchedulerCustomResourceViewViewInfo.AddEventCell(
  ...
  function GetResourceItem: TcxSchedulerStorageResourceItem;
  begin
    if AViewData.Resource = nil then
      Result := nil
    else
      Result := AViewData.Resource.ResourceItem;
  end;

var
  ACanResize: Boolean;
begin
  ACanResize := Scheduler.EventOperations.Sizing and TcxSchedulerControlEventAccess(AViewData.Event).CanResize(GetResourceItem);
  Result := CreateEventCellViewInfo(AViewData, ACanResize);
...
begin
  ...
  Dec(FCalculationCounter);
  if UseRightToLeftAlignment and (FCalculationCounter = 0) then
  begin
    DoRightToLeftConversion(Bounds);
    for I := 0 to EventCells.Count - 1 do
      EventCells[I].AllowRightToLeftConversion := UseRightToLeftAlignment;
  end;
end;
...
begin
  AGroupingKind := GetGroupingKind;
  Result := (((AGroupingKind = gkNone) or (AResourceIndex < 0))
    and (HasVisibleEvents or HasMoreEventsButton(-1))) or
    ((AGroupingKind <> gkNone) and (AResourceIndex >= 0) and
    (HasEventForResource(AResourceIndex) or
...
procedure TcxSchedulerCustomResourceViewViewInfo.CalculateNavigationButtonHeight(const AColumnRect: TRect);
begin
  if IsNavigationButtonsVertical then
    FIsNavigationButtonsCollapsed := Round(ScaleFactor.ApplyF(ContentNavigationButtonHeight * 1.5)) >= GetCorrectColumnHeight(AColumnRect)
  else
    FIsNavigationButtonsCollapsed := Round(ScaleFactor.ApplyF(ContentNavigationButtonHeight * 1.5)) >= GetCorrectColumnHeight(cxRectRotate(AColumnRect));
  if IsNavigationButtonsCollapsed then
    FNavigationButtonHeight := ScaleFactor.Apply(ContentNavigationButtonReducedHeight)
  else
    FNavigationButtonHeight := ScaleFactor.Apply(ContentNavigationButtonHeight);
  FIsNavigationButtonsHeightCalculated := True;
end;

procedure TcxSchedulerCustomResourceViewViewInfo.CalculateNavigationButtonParams(const AColumnRect: TRect;
...
var
  ...
  AInternalButtonWidth: Integer;
begin
  if not IsNavigationButtonsHeightCalculated then
    CalculateNavigationButtonHeight(AColumnRect);
  if LookAndFeelPainter.ApplyEditorAdvancedMode then
  begin
    LookAndFeelPainter.SchedulerNavigationButtonSizes(AKind = nbkNext, ABorders, AArrowSize, AHasTextArea, ScaleFactor, IsNavigationButtonsVertical);
    AInternalButtonWidth := ScaleFactor.Apply(Max(ContentNavigationButtonWidth, cxTextHeight(Styles.GetBackgroundParams.Font) + ABorders.Right + ABorders.Left));
  end
  else
    AInternalButtonWidth := ScaleFactor.Apply(ContentNavigationButtonWidth);
  if IsNavigationButtonsVertical then
  begin
    AMiddle := (AColumnRect.Top + AColumnRect.Bottom) div 2;
    case AKind of
      nbkPrevious: FButtonBounds.Init(AColumnRect.Left, AMiddle - FNavigationButtonHeight div 2,
        AColumnRect.Left + AInternalButtonWidth, AMiddle + FNavigationButtonHeight div 2);
      nbkNext: FButtonBounds.Init(AColumnRect.Right - AInternalButtonWidth, AMiddle - FNavigationButtonHeight div 2,
        AColumnRect.Right, AMiddle + FNavigationButtonHeight div 2);
    else
  ...
  begin
    ...
    case AKind of
      nbkPrevious: FButtonBounds.Init(AMiddle - FNavigationButtonHeight div 2, AColumnRect.Top,
        AMiddle + FNavigationButtonHeight div 2, AColumnRect.Top + AInternalButtonWidth);
      nbkNext: FButtonBounds.Init(AMiddle - FNavigationButtonHeight div 2, AColumnRect.Bottom - AInternalButtonWidth,
        AMiddle + FNavigationButtonHeight div 2, AColumnRect.Bottom);
    else
      FButtonBounds := cxNullRect;
    end;
  end;
  ...
  LookAndFeelPainter.CalculateSchedulerNavigationButtonRects(
    AKind = nbkNext, IsNavigationButtonsCollapsed, cxTextExtent(Styles.GetBackgroundParams.Font,
...
procedure TcxSchedulerCustomResourceViewViewInfo.CalculateResourceHeadersAutoHeight(AWidth: Integer);
  ...
  begin
    ...
    AImagesSize := Size(0, 0);
    AHasImage := UseResourceImages and (AResource.ActualImageIndex >= 0);
    if AHasImage then
    begin
      AImagesSize := GetResourceImagesSize;
      if ResourceHeaders.ImagePosition in [ipLeft, ipRight] then
        AWidth := Max(0, AWidth - (AImagesSize.cx + ScaleFactor.Apply(cxTextOffset * 2)));
    end;
    ...
    begin
      if (ResourceHeaders.ImagePosition in [ipTop, ipBottom]) then
        Inc(Result, AImagesSize.cy + 4 * ScaleFactor.Apply(cxTextOffset))
      else
        Result := Max(Result, AImagesSize.cy + 3 * ScaleFactor.Apply(cxTextOffset));
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      if AHasImage and (ResourceHeaders.ImagePosition in [ipTop, ipBottom]) then
        Dec(R.Bottom, GetResourceImagesSize.cy + 3 * ATextOffset);
      ...
      Result := R.Right - R.Left + 3 * ATextOffset;
      SetResourceTextRect(AResource, R);
      if AHasImage and (ResourceHeaders.ImagePosition in [ipTop, ipBottom]) then
        Result := Max(Result, ResourceImages.Width + 4 * ATextOffset);
    end;
    if AHasImage and (ResourceHeaders.ImagePosition in [ipLeft, ipRight]) then
      Inc(Result, ResourceImages.Width + 4 * ATextOffset);
  end;

begin
  ATextOffset := ScaleFactor.Apply(cxTextOffset);
  AWidth := GetResourceHeaderWidth - 3 * ATextOffset;
...
begin
  if not IsEventCellLayoutCalculatingLocked then
    FResources.Clear;
...
procedure TcxSchedulerCustomResourceViewViewInfo.ClearButtons;
begin
  Buttons.Clear;
end;

procedure TcxSchedulerCustomResourceViewViewInfo.ClearEventCells;
begin
  EventCells.Clear;
end;

procedure TcxSchedulerCustomResourceViewViewInfo.ClearNavigationButtons;
begin
  NavigationButtons.Clear;
  FIsNavigationButtonsHeightCalculated := False;
  FIsNavigationButtonsCollapsed := False;
end;
...
function TcxSchedulerCustomResourceViewViewInfo.DoSetResourceHasVisibleEvent(AEvent: TcxSchedulerControlEvent): Boolean;
var
  AResourceIndex: Integer;
begin
  Result := False;
...
begin
  ...
  AIsHorzSplitterMoving := not AHorzSplitterMovingRect.IsEmpty;
  AIsVertSplitterMoving := not AVertSplitterMovingRect.IsEmpty;
...
var
  I: Integer;
begin
  if IsEventCellLayoutCalculatingLocked then
    Exit;
...
function TcxSchedulerCustomResourceViewViewInfo.HasEventForResource(AResourceIndex: Integer): Boolean;
begin
  Result := Resources[AResourceIndex].VisibleEventCount > 0;
  if not Result then
    Result := HasMoreEventsButton(AResourceIndex);
end;
...
begin
  DrawCommonBorders := LookAndFeelPainter.HeaderBorders([nLeft..nBottom]) = cxBordersAll;
  APrevCell := nil;
  ...
  begin
    ...
    begin
      Include(ACell.FNeighbors, nTop);
      Include(ACell.FNeighbors, nLeft);
    end;
...
procedure TcxSchedulerCustomResourceViewViewInfo.ProcessPreviouslyCalculatedEvents;
var
  I: Integer;
...
begin
  ...
  begin
    ...
    AEventCell.AllowRightToLeftConversion := False;
    SetResourceHasVisibleEvent(AEventCell.Event);

    if AEventCell.Selected then
    begin
      if not UseRightToLeftAlignment then
        AEventCell.CalculateSelectionBorderParams
      else
      begin
        AEventCell.DoRightToLeftConversion(Bounds);
        AEventCell.CalculateSelectionBorderParams;
        AEventCell.DoRightToLeftConversion(Bounds);
      end;
      AEventCell.RecalculateVisibility;
    end
    else
    if AEventCell.HasSizingHandlers then
    begin
      AEventCell.InitializeSizingHandlers;
      AEventCell.RecalculateVisibility;
    end;

    CustomProcessPreviouslyCalculatedEvent(AEventCell);
  end
end;

procedure TcxSchedulerCustomResourceViewViewInfo.CustomProcessPreviouslyCalculatedEvent(AEventCell: TcxSchedulerEventCellViewInfo);
begin
//
end;

procedure TcxSchedulerCustomResourceViewViewInfo.ResourceHeadersChanged;
var
  I: Integer;
  AItem: TcxSchedulerHeaderCellViewInfo;
begin
  for I := 0 to ResourceHeaderCells.Count - 1 do
  begin
    AItem := ResourceHeaderCells.Items[I] as TcxSchedulerHeaderCellViewInfo;
    if CanCacheResourceHeader(AItem) then
      AItem.MakeCacheDirty;
  end;
end;
...
begin
  if not DoSetResourceHasVisibleEvent(AEvent) then
...
function TcxSchedulerCustomResourceViewViewInfo.GetUseRightToLeftAlignment: Boolean;
begin
  Result := inherited GetUseRightToLeftAlignment and not Adapter.IsPrinting;
end;

procedure TcxSchedulerCustomResourceViewViewInfo.ProcessDateToDisplayText(ArrangeByType: Boolean = False);
var
  I: Integer;
  AType: Integer;
begin
  DateTimeHelper.UpdateDayMonthFormat;
  DateTimeHelper.LockUpdateDayMonthFormat(True);
  try
    AType := 0;
    for I := 0 to FDayHeaderCells.Count - 1 do
      AType := Max(TcxSchedulerDayHeaderCellViewInfo(FDayHeaderCells[I]).ConvertDateToDisplayText, AType);
    if ArrangeByType and (AType > 0) then
    begin
      for I := 0 to FDayHeaderCells.Count - 1 do
        TcxSchedulerDayHeaderCellViewInfo(FDayHeaderCells[I]).ConvertDateToDisplayText(AType);
    end;
  finally
    DateTimeHelper.LockUpdateDayMonthFormat(False);
  end;
end;
...
begin
  Result := CanSelected and (AResource = FSelResource) and not HideSelection;
  if Result then
  begin
    ATime := DateTimeHelper.RoundTime(ATime);
    Result := (ATime >= SelStart) and (ATime <= SelFinish);
  end;
end;
...
function TcxSchedulerCustomResourceViewViewInfo.HasMoreEventsButton(AResourceIndex: Integer): Boolean;
...
begin
  ...
  for I := 0 to Buttons.Count - 1 do
    ...
    begin
      AButton := TcxSchedulerMoreEventsButtonViewInfo(Buttons[I]);
      if (AResourceIndex = -1) or
          AButton.Event.IsSharedWithResource(Resources[AResourceIndex].ResourceID) or
          IsButtonWithoutResources(AButton) then
        Exit;
    end;
  Result := False;
end;
...
begin
  if HitAtResource then
    Result := Scheduler.EventOperations.Sizing and TcxSchedulerControlEventAccess(AEvent).CanResize(Resource)
  else
...
constructor TcxSchedulerEventLayoutBuilder.Create(const AViewStyle: TcxSchedulerViewStyle);
var
  AComparer: IComparer<TcxSchedulerEventPlace>;
begin
  inherited Create;
  FViewStyle := AViewStyle;
  AComparer := CreateComparer;
  FEventPlaces := TdxObjectSortedList<TcxSchedulerEventPlace>.Create(AComparer);
end;
...
begin
  ...
  Result.Resource := AResource;
  AIndex := FEventPlaces.BinarySearch(Result);
  if AIndex >= 0 then
  begin
    Inc(AIndex);
    while AIndex < FEventPlaces.Count do
    begin
      if FEventPlaces.Comparer.Compare(FEventPlaces[AIndex], Result) <> 0 then
        Break;
      Inc(AIndex);
    end;
  end
    ...
    AIndex := not AIndex;
  FEventPlaces.Insert(AIndex, Result);
end;
...
var
  ...
  ABounds: TRect;
  I: Integer;
  AAreaManagers: TObjectDictionary<Integer, TcxSchedulerOccupiedAreaManager>;
  AManager: TcxSchedulerOccupiedAreaManager;
begin
  if EventPlaceCount = 0  then
    Exit;
  AAreaManagers := TObjectDictionary<Integer, TcxSchedulerOccupiedAreaManager>.Create([doOwnsValues]);
  try
    for I := 0 to EventPlaceCount - 1 do
    begin
      APlace := EventPlaces[I];
      if not AAreaManagers.TryGetValue(Integer(APlace.Resource), AManager) then
      begin
        AManager := TcxSchedulerOccupiedAreaManager.Create;
        AAreaManagers.Add(Integer(APlace.Resource), AManager);
      end;
      ABounds := AManager.GetFreeBounds(cxRect(APlace.ColStart, APlace.LineStart, APlace.ColFinish, APlace.LineFinish));
      APlace.LineStart := ABounds.Top;
      APlace.LineFinish := ABounds.Bottom;
...
function TcxSchedulerEventLayoutBuilder.CreateComparer: IComparer<TcxSchedulerEventPlace>;
begin
  Result := TcxSchedulerEventPlaceComparer.Create(ViewStyle);
end;

function TcxSchedulerEventLayoutBuilder.GetEventPlace(AIndex: Integer): TcxSchedulerEventPlace;
begin
  Result := FEventPlaces[AIndex];
end;
