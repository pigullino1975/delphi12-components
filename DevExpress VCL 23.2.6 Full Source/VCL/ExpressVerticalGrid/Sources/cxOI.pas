...
uses
  ...
  TypInfo, RTLConsts,
  dxCore, dxCoreClasses, dxCoreGraphics, dxMessages, cxGraphics, cxClasses, cxControls, cxLookAndFeels,
...
type
  ...
  protected
    ...
    procedure ProcessPropertyReadOnly(var AAttributes: TcxPropertyAttributes);
  ...
  { TcxAlphaColorProperty }

  TcxAlphaColorProperty = class(TcxIntegerProperty)
  public
  ...
  TcxSkinPaletteNameProperty = class(TcxStringProperty)
  public
    function GetAttributes: TcxPropertyAttributes; override;
    procedure GetValues(Proc: TGetStrProc); override;
  end;
...
uses
  ...
  dxTypeInfo, cxFormats, cxContainer, cxCustomData, cxGeometry, cxLookAndFeelPainters, cxDateUtils, dxShellDialogs,
  cxTextEdit, cxDropDownEdit, cxCalendar, cxSpinEdit, cxTimeEdit, cxButtonEdit, dxSkinInfo, dxSkinsStrs,
  cxColorComboBox, cxOIStringsEd, cxOIPictureEd, cxOICollectionEd, cxVGridConsts, dxGDIPlusClasses, dxColorDialog;
...
function HasProperty(AClass: TClass; const APropertyName: string): Boolean;
begin
  Result := TdxTypeInfo.HasProperty(AClass, APropertyName);
end;
...
begin
  ...
  ProcessPropertyReadOnly(Result);
end;
...
procedure TcxPropertyEditor.ProcessPropertyReadOnly(var AAttributes: TcxPropertyAttributes);
begin
  if not Assigned(GetPropInfo^.SetProc) then
    Include(AAttributes, ipaReadOnly);
end;
...
function TcxPropertyEditor.IsDefaultValue: Boolean;
  ...
  var
    AIsDefaultValue: Boolean;
  begin
    AIsDefaultValue := True;
    // Go through each of the properties on the object
    TdxTypeInfo.EnumProperties(AnObject.ClassInfo,
      procedure(PropInfo: PPropInfo; var ABreak: Boolean)
      begin
        ABreak := (PropInfo = nil);
        if not IsDefaultPropertyValue(AnObject, PropInfo, GetLookupInfo) then
        begin
          AIsDefaultValue := False;
          ABreak := True;
        end;
      end);
    Result := AIsDefaultValue;
  end;
...
begin
  Result := [ipaMultiSelect, ipaSortList, ipaRevertable, ipaValueList];
  ProcessPropertyReadOnly(Result);
end;
...
begin
  Result := [ipaMultiSelect, ipaSortList, ipaValueList, ipaRevertable];
  ProcessPropertyReadOnly(Result);
end;
...
var
  S1, S2: TcxIntegerSet;
  HasStoredProc, ProcAsInt: TdxNativeUInt;
begin
  Result := inherited IsDefaultValue;
  if not Result then
  begin
    ProcAsInt := TdxNativeUInt(PPropInfo(GetPropInfo)^.StoredProc);
    HasStoredProc := ProcAsInt and not TdxNativeUInt($FF);
...
begin
  OpenDialog := TdxOpenFileDialog.Create(Application);
  with OpenDialog do
  try
    Filename := GetValue;
...
procedure TcxAlphaColorProperty.Edit;
var
  AColor: TdxAlphaColor;
begin
  AColor := GetOrdValue;
  if TdxGlobalColorDialog.Execute(AColor) then
  begin
    SetOrdValue(AColor);
    PostChangedNotification;
  end;
...
function TcxAlphaColorProperty.GetAttributes: TcxPropertyAttributes;
begin
  Result := [ipaMultiSelect, ipaDialog];
  ProcessPropertyReadOnly(Result);
end;

function TcxAlphaColorProperty.GetValue: string;
begin
  Result := TdxAlphaColors.ToString(GetOrdValue, True);
end;

procedure TcxAlphaColorProperty.SetValue(const Value: string);
begin
  SetOrdValue(TdxAlphaColors.FromString(Value));
end;
...
begin
  Result := inherited GetAttributes + [ipaAutoUpdate];
end;
...
procedure TcxVariantProperty.GetProperties(AOwner: TComponent;
  Proc: TcxGetPropEditProc);
begin
  if not (ipaReadOnly in GetAttributes) then
...
{ TcxSkinPaletteNameProperty }

function TcxSkinPaletteNameProperty.GetAttributes: TcxPropertyAttributes;
begin
  Result := inherited GetAttributes - [ipaReadOnly] + [ipaValueList];
end;

procedure TcxSkinPaletteNameProperty.GetValues(Proc: TGetStrProc);
var
  AData: TdxSkinInfo;
  APalettes: TStringList;
  I: Integer;
begin
  if (RootLookAndFeel.Painter.LookAndFeelStyle = lfsSkin)
    and RootLookAndFeel.Painter.GetPainterData(AData) and (AData.Skin.ColorPalettes.Count > 0) then
  begin
    APalettes := TStringList.Create;
    try
      for I := 0 to AData.Skin.ColorPalettes.Count - 1 do
        APalettes.Add(AData.Skin.ColorPalettes[I].Name);
      APalettes.Sort;
...
initialization
  ...
  cxRegisterPropertyEditor(TypeInfo(TdxAlphaColor), nil, '', TcxAlphaColorProperty);
  ...
  cxRegisterEditPropertiesClass(TcxAlphaColorProperty, TcxButtonEditProperties);
  ...
  cxRegisterPropertyEditor(TypeInfo(string), nil, 'SkinPaletteName', TcxSkinPaletteNameProperty);
