...
uses
  SysUtils, Math, cxControls, cxGeometry, dxCoreGraphics, cxEditUtils, Forms,
  cxLookAndFeels;

const
  dxThisUnitName = 'cxVGridViewInfo';
...
begin
  if not VerticalGrid.UseLightBorders then
    ...
    Result := ViewBandWidth - ViewHeaderWidth - DividerWidth;
end;
...
var
  I: Integer;
begin
  for I := Scroller.LeftVisibleBand to Scroller.BandsInfo.Count - 1 do
    CreateBand(Scroller.BandsInfo[I].BandHeight, 40);
  UpdateBandsWidth;
end;
...
var
  ...
  ARowRect: TRect;
begin
  ARowRect := ARowViewInfo.RowRect;
  ALeft := ARowRect.Left + ViewHeaderWidth + DividerWidth +
    AValueIndex * (ViewValueWidth + cxSetValue(RecordsInterval = 0, VertLineWidth, RecordsInterval + 2 * VertLineWidth));
  Result := cxRectBounds(ALeft + Scroller.PixelScrollRecordOffset, ARowRect.Top, ViewValueWidth, ARowRect.Bottom - ARowRect.Top);
end;
      ...
      begin
        ...
        begin
          if (HorzLineWidth > 0) and not ALast then
            LinesInfo.Add(ALeft, ATop + AHeight, RecordsInterval, HorzLineWidth, BandBorderColor);
        end;
        ...
        if VertLineWidth > 0 then
          LinesInfo.Add(R.Right, R.Top, VertLineWidth, AHeight + HorzLineWidth, BandBorderColor);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if I = 0 then
            Inc(AHeight, cxRectHeight(ARowInfo.RowRect))
          else
            Inc(AHeight, cxRectHeight(ARowInfo.RowRect) + HorzLineWidth);
        end;
      end
      ...
      begin
        ...
        if not VerticalGrid.UseLightBorders then
          ...
          ALeft := ABandRect.Left + VertLineWidth + DividerWidth +
            ViewHeaderWidth + ViewValueWidth + Scroller.PixelScrollRecordOffset
      end;
    end;
  ...
  begin
    if (VertLineWidth > 0) then
      LinesInfo.Add(R.Left, R.Top - HorzLineWidth, VertLineWidth, R.Bottom - R.Top,
        BandBorderColor);
  end;
  ...
  begin
    if ALeft + ASize.cx > R.Right then
      ASize.cx := R.Right - ALeft;
...
begin
  ...
  if not VerticalGrid.UseLightBorders then
    ...
    AWidth := ViewHeaderWidth + VertLineWidth + DividerWidth + ViewValueWidth;
  FCommonWidth := AWidth;
...
function TcxMultiRecordViewInfo.GetBandSizeableRect(const ABandRect: TRect): TRect;
var
  ALeft: Integer;
begin
  if VerticalGrid.LayoutStyle <> lsMultiRecordView then
    ...
    begin
      ...
      begin
        if not VerticalGrid.UseLightBorders then
        begin
          ALeft := ABandRect.Left + ViewHeaderWidth + DividerWidth + ViewValueWidth + VertLineWidth + Scroller.PixelScrollRecordOffset;
          Result := cxRect(ALeft - 1, Top, ALeft + 2, Bottom);
        end
        else
        begin
          ALeft := ABandRect.Left + ViewHeaderWidth + DividerWidth + ViewValueWidth + Scroller.PixelScrollRecordOffset;
          Result := cxRect(ALeft - 1, Top, ALeft + 2 + VertLineWidth, Bottom);
        end;
      end
      else
      begin
        if not VerticalGrid.UseLightBorders then
        begin
          ALeft := ABandRect.Right - ViewHeaderWidth - DividerWidth - ViewValueWidth - VertLineWidth - Scroller.PixelScrollRecordOffset;
          Result := cxRect(ALeft - 2, Top, ALeft + 1, Bottom);
        end
        else
        begin
          ALeft := ABandRect.Right - ViewHeaderWidth - DividerWidth - ViewValueWidth - Scroller.PixelScrollRecordOffset;
          Result := cxRect(ALeft - 2 - VertLineWidth, Top, ALeft + 1, Bottom);
        end;
      end
    end;
end;
...
begin
  if not (inherited VerticalGrid).UseLightBorders then
    Result := cxRectWidth(ClientRect) - ViewHeaderWidth - DividerWidth - VertLineWidth
  else
    Result := cxRectWidth(ClientRect) - ViewHeaderWidth - DividerWidth;
end;
