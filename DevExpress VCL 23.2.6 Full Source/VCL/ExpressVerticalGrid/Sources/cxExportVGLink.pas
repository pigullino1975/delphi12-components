...
type
  ...
  private
    function GetIndent(Index: TdxListIndex): TcxRowIndentsInfo;
  public
    procedure Clear; override;
    property Indents[Index: TdxListIndex]: TcxRowIndentsInfo read GetIndent; default;
  end;
  ...
  private
    function GetItem(Index: TdxListIndex): TRowCaptionCellInfo;
  public
    function AddCaption(AColumn: Integer; AWidth: Integer; AStyleIndex: Integer; const ACaption: string): Integer; overload;
    procedure Clear; override;
    property Items[Index: TdxListIndex]: TRowCaptionCellInfo read GetItem; default;
  end;
...
function TcxRowsIndents.GetIndent(Index: TdxListIndex): TcxRowIndentsInfo;
begin
...
function TcxRowCellsInfo.GetItem(Index: TdxListIndex): TRowCaptionCellInfo;
begin
...
var
  ...
  AVerticalGridExport: TcxVerticalGridExportHelper;
...
begin
  ...
  try
    AVerticalGridExport := TcxVerticalGridExportHelper.Create(AVerticalGrid, AExportType, AFileStream, AHandler);
    try
      AVerticalGridExport.FExpand := AExpand;
      if AExpand then
        AVerticalGrid.FullExpand;
      AVerticalGrid.FocusedRow := nil;
      AVerticalGridExport.FRecordsPerBand := Max(ARecordPerBand, 1);
      AVerticalGridExport.FIsNativeFormat :=  AUseNativeFormat;
      if cxSupports(AVerticalGridExport.Provider, IcxExportProviderEncoding, AEncodingIntf) then
        AEncodingIntf.SetEncoding(AEncoding);
      if cxSupports(AVerticalGridExport.Provider, IcxExportWithSeparators, AIntf) and (Length(ASeparators) > 0) then
      begin
        for I := Low(ASeparators) to High(ASeparators) do
          AIntf.AddSeparator(ASeparators[I]);
      end;
      AVerticalGridExport.DoCalculateTableMap;
      AVerticalGridExport.DoWriteCells;
    finally
      AVerticalGridExport.Free;
    end;
  finally
    AFileStream.Free;
  end;
