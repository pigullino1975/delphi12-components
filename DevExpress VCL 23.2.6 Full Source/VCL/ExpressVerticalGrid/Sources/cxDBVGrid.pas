...
uses
  SysUtils, cxGraphics, cxControls, cxLookAndFeelPainters, cxEditDataRegisteredRepositoryItems, cxEditDBRegisteredRepositoryItems,
  Types, cxGeometry, cxVGridViewInfo, cxClasses, dxCore;

const
  dxThisUnitName = 'cxDBVGrid';
...
function TcxDBVerticalGridDataController.CreateExpressionProvider: TcxDataCustomExpressionProvider;
begin
  Result := VerticalGrid.CreateExpressionProvider;
end;

procedure TcxDBVerticalGridDataController.FilterChanged;
begin
  inherited FilterChanged;
  VerticalGrid.FilterChanged;
end;
...
function TcxDBVerticalGridDataController.GetFilterDisplayText(ARecordIndex, AItemIndex: Integer): string;
var
  AItem: TcxCustomInplaceEditContainerAccess;
  AProperties: TcxCustomEditProperties;
  AValue: Variant;
begin
  AItem := TcxCustomInplaceEditContainerAccess(GetItem(AItemIndex));
  AProperties := AItem.PropertiesValue;
  AValue := GetInternalDisplayText(ARecordIndex, AItemIndex);
...
function TcxDBVerticalGridItemDataBinding.GetDefaultValueTypeClass: TcxValueTypeClass;
begin
  Result := nil;
end;
...
function TcxDBVerticalGridItemDataBinding.IsValueTypeChangeable: Boolean;
begin
  Result := Field = nil;
end;

function TcxDBVerticalGridItemDataBinding.IsValueTypeStored: Boolean;
begin
  Result := Field = nil;
end;
...
begin
  if Field = nil then
    inherited ValueTypeClassChanged
  else
