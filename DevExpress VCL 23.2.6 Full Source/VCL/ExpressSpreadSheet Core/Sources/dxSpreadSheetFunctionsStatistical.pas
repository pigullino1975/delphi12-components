...
unit dxSpreadSheetFunctionsStatistical; // for internal use
...
const
  dxThisUnitName = 'dxSpreadSheetFunctionsStatistical';
...
type
  ...
  TdxReferenceAccess = class(TdxSpreadSheetFormulaReferenceToken);
...
function dxGetDensityStandardNormDist(X: Extended): Extended;
begin
  Result := 0.3989422804014326779 * Exp(-X * X / 2);
end;

function dxGetStandardNormDist(X: Extended): Extended;
const
  APointCount = 10;
...
begin
  ...
  begin
    ...
    Result := ALaplaceTable[ANearestTableArgumentNumber] +
      dxGetIntegral(ANearestTableArgumentNumber * AStep, AAbsX, @dxGetDensityStandardNormDist);
  end;
  if X < 0 then
    Result := 1 - Result;
end;
...
begin
  if not ACumulative then
    Result := dxGetDensityStandardNormDist((X - AMean)/AStdDev) / AStdDev
  else
    Result := dxGetStandardNormDist((X - AMean)/AStdDev);
end;

function dxGetStandardNormInv(X: Extended): Extended;
...
var
  ...
  ARange: TdxSpreadSheetFormulaReferenceToken;
  AValue: Variant;
...
begin
  ...
  begin
    dRow := ACellReference.RowIndex - TdxSpreadSheetFormulaReferenceToken(AData.ConditionRange).ActualRow;
    dColumn := ACellReference.ColumnIndex - TdxSpreadSheetFormulaReferenceToken(AData.ConditionRange).ActualColumn;
    for I := 0 to AConditions.Count - 1 do
    begin
      ACondition := AConditions[I];
      ARange := TdxSpreadSheetFormulaReferenceToken(ACondition.Range);
      ARange.GetCellValueAsOrdinal(ARange.View, ARange.ActualRow + dRow, ARange.ActualColumn + dColumn, AValue, AErrorCode);
...
function cbSumIF(const AValue: Variant; ACanConvertStrToNumber: Boolean;
  ...
  function GetActualRow(AReference: TdxSpreadSheetFormulaReferenceToken): Integer;
  begin
  ...
  function GetActualColumn(AReference: TdxSpreadSheetFormulaReferenceToken): Integer;
  begin
...
var
  ...
  AConditionRange: TdxSpreadSheetFormulaReferenceToken;
  AEnumData: TdxSpreadSheetEnumValuesWithCondition;
  AResultRange: TdxSpreadSheetFormulaReferenceToken;
...
begin
  ...
  AConditionRange := TdxSpreadSheetFormulaReferenceToken(AEnumData.ConditionRange);
  AResultRange := TdxSpreadSheetFormulaReferenceToken(AEnumData.ResultRange);
...
begin
  ...
  begin
    ...
    ANormsInv := -dxGetStandardNormInv(Probability);
...
begin
  ...
  EndY := 1;
  Dichotomy := 1 / (9 * alpha);
  NormSInv := 1 - Dichotomy - dxGetStandardNormInv(Probability) * Sqrt(Dichotomy);
...
var
  ...
  ARangeStorage: TdxSpreadSheetFormulaResult;
begin
  ARangeStorage := TdxSpreadSheetFormulaResult.Create(Sender.Owner);
  try
    ...
    if not Odd(AParamCount + APriorRequiredParamCount) then
      ...
      begin
        ...
        begin
          ...
          if not dxSpreadSheetIsReferenceToken(AConditionRange, ARangeStorage) then
...
procedure dxInitializeEnumData(var AData: TdxSpreadSheetEnumValuesWithCondition); inline;
begin
  AData.ResultValue := 0;
  AData.ErrorCode := ecNone;
  AData.CountValues := 0;
  AData.ConditionValue := Null;
  AData.Operation := opEQ;
end;

procedure dxCustomSumIfCalculate(Sender: TdxSpreadSheetFormulaResult; const AParams: TdxSpreadSheetFormulaToken;
  const AIFS, ACalculateAverage: Boolean);

  function ExtractBaseRanges(var ARangeStorage: TdxSpreadSheetFormulaResult;
    var ARange, AResultRange: TdxSpreadSheetFormulaToken; var ARowShift, AColumnShift: Integer): Boolean;
  begin
    Result := True;
    if AIFS then
    begin
    ...
    if Result then
      Result := dxSpreadSheetIsReferenceToken(ARange, ARangeStorage) and dxSpreadSheetIsReferenceToken(AResultRange, ARangeStorage)
        and Sender.Validate
    else
      Sender.SetError(ecNA);
...
var
  ...
  ATmpRes: TdxSpreadSheetFormulaResult;
  ARow, AColumn, ARowShift, AColumnShift: Integer;
  ARangeStorage: TdxSpreadSheetFormulaResult;
begin
  ...
  try
    ...
    try
      ...
      begin
          ...
          AColumnShift := 0;
          if not AIFS then
            ExtractBaseRanges(ARangeStorage, AData.ConditionRange, AData.ResultRange, ARowShift, AColumnShift)
          else
            if ExtractBaseRanges(ARangeStorage, AData.ConditionRange, AData.ResultRange, ARowShift, AColumnShift) then
              dxExtractCustomIfsAdditionalConditions(Sender, AParams, AData, 3);
          if Sender.Validate then
          begin
            ADimension := AData.ConditionRange.GetDimension(AData.ErrorCode);
            if AData.ErrorCode <> ecNone then
              Sender.SetError(AData.ErrorCode);
          end;
          if Sender.Validate then
          begin
            ...
            for ARow := 0 to ADimension.RowCount - 1 do
              ...
              begin
                ...
                dxInitializeEnumData(AData);
                ATmpRes := TdxFormulaAccess(Sender.Owner).Calculate(AData.ConditionRange);
                try
                  ATmpRes.LastItem.GetValueAsArrayItem(ARow, AColumn, AData.ConditionValue, AData.ErrorCode);
                finally
                  ATmpRes.Free;
                end;
                if AData.ErrorCode = ecNone then
                begin
                  if VarIsNull(AData.ConditionValue) or VarIsEmpty(AData.ConditionValue) then
                    AData.ConditionValue := 0;
                  dxSpreadSheetExtractConditionParams(VarToStr(AData.ConditionValue), Sender.Owner.FormatSettings,
                    AData.ConditionValue, AData.Operation);
                end;

                AParentToken := TdxSpreadSheetFormulaToken.Create;
                TdxSpreadSheetFormulaToken.AddChild(AResult, AParentToken);
                if AData.ErrorCode <> ecNone then
                  TdxSpreadSheetFormulaToken.AddChild(AParentToken, TdxSpreadSheetFormulaErrorValueToken.Create(AData.ErrorCode))
                else
                begin
                  ...
                  if AData.ErrorCode <> ecNone then
                    ...
                    if not ACalculateAverage then
                      TdxSpreadSheetFormulaToken.AddChild(AParentToken, dxSpreadSheetCreateResultToken(AData.ResultValue, AData.ErrorCode))
                    else
                      if AData.ErrorCode <> ecNone then
                        TdxSpreadSheetFormulaToken.AddChild(AParentToken, TdxSpreadSheetFormulaErrorValueToken.Create(AData.ErrorCode))
                      else
                        if AData.CountValues = 0 then
                          TdxSpreadSheetFormulaToken.AddChild(AParentToken, TdxSpreadSheetFormulaErrorValueToken.Create(ecDivByZero))
                        else
                          TdxSpreadSheetFormulaToken.AddChild(AParentToken,
                            dxSpreadSheetCreateResultToken(AData.ResultValue / AData.CountValues, AData.ErrorCode));
                end;
              end;
          end;
      end
...
procedure fnAverageIF(Sender: TdxSpreadSheetFormulaResult; const AParams: TdxSpreadSheetFormulaToken);
begin
  dxCustomSumIfCalculate(Sender, AParams, False, True);
end;

procedure fnAverageIFS(Sender: TdxSpreadSheetFormulaResult; const AParams: TdxSpreadSheetFormulaToken);
begin
  dxCustomSumIfCalculate(Sender, AParams, True, True);
end;
...
procedure dxRelationshipBetweenTwoDatasets(Sender: TdxSpreadSheetFormulaResult; AParamY, AParamX: TdxSpreadSheetFormulaToken;
  ...
  procedure PopulateVariantArray(var AArray: Variant; const ADimension: TdxSpreadSheetFormulaTokenDimension; const AParams: TdxSpreadSheetFormulaToken);
    ...
    begin
      Result := ((AParams is TdxSpreadSheetFormulaAreaReferenceToken) and dxIsNumberOrDateTime(AValue)) or
...
var
  ...
  AErrorCode: TdxSpreadSheetFormulaErrorCode;
  AParamStorage: TdxSpreadSheetFormulaResult;
begin
  AParamStorage := TdxSpreadSheetFormulaResult.Create(Sender.Owner);
  try
    dxSpreadSheetIsReferenceToken(AParamY, AParamStorage);
    ADimensionY := AParamY.GetDimension(AErrorCode);
    if AErrorCode = ecNone then
    begin
      dxSpreadSheetIsReferenceToken(AParamX, AParamStorage);
...
var
  AData: TdxSpreadSheetEnumValuesWithCondition;
  ACriteriaToken, AParentToken: TdxSpreadSheetFormulaToken;
  ADimension: TdxSpreadSheetFormulaTokenDimension;
  AResult: TdxSpreadSheetFormulaArrayToken;
  ARow, AColumn: Integer;
  ATmpRes, ARangeStorage: TdxSpreadSheetFormulaResult;
begin
   AData := TdxSpreadSheetEnumValuesWithCondition.Create;
   try
     if Sender.Owner.IsArrayFormula then
     begin
        ...
        ACriteriaToken := AParams.Next.FirstChild;
        if ACriteriaToken = nil then
          AData.ErrorCode := ecValue;
        if AData.ErrorCode = ecNone then
          ADimension := ACriteriaToken.GetDimension(AData.ErrorCode);
        if AData.ErrorCode <> ecNone then
          Sender.SetError(AData.ErrorCode)
        else
          if AIFS then
        ...
        begin
          ...
          for ARow := 0 to ADimension.RowCount - 1 do
            ...
            begin
              ...
              dxInitializeEnumData(AData);
              ATmpRes := TdxFormulaAccess(Sender.Owner).Calculate(ACriteriaToken);
              try
                ATmpRes.LastItem.GetValueAsArrayItem(ARow, AColumn, AData.ConditionValue, AData.ErrorCode);
              finally
                ATmpRes.Free;
     ...
     begin
       ...
       try
         ...
         begin
           AData.ConditionRange := AParams.FirstChild;
           if not dxSpreadSheetIsReferenceToken(AData.ConditionRange, ARangeStorage) then
             Sender.SetError(ecNA);
         end;
...
begin
  if X >= 9 then
    ...
    if X <= -9 then
      ...
      Result := dxGetStandardNormDist(X) - 0.5;
end;
...
begin
  if Sender.ExtractNumericParameter(AProbability, AParams) and Sender.ExtractNumericParameter(AMean, AParams, 1) and
    ...
    if not((AProbability > 0) and (AProbability < 1) and (AStdDev > 0)) then
      ...
      Sender.AddValue(AMean + AStdDev * dxGetStandardNormInv(AProbability));
end;
...
begin
  if Sender.ExtractNumericParameter(X, AParams) then
    Sender.AddValue(dxGetStandardNormDist(X));
end;
...
begin
  if Sender.ExtractNumericParameter(X, AParams) then
    if not((X > 0) and (X < 1)) then
      ...
      Sender.AddValue(dxGetStandardNormInv(X));
end;
...
function dxGetCustomVar(Sender: TdxSpreadSheetFormulaResult; const AParams: TdxSpreadSheetFormulaToken;
  AIsStandard, AWithText, AErrorIfNonReferenceToken, AIgnoreHiddenRows, AForSubTotal: Boolean): Extended;
var
  AData: TdxSpreadSheetEnumValues;
  AAverage: Extended;
  ACount: Integer;
...
begin
  ...
  try
    ...
    if Sender.Validate then
    begin
      Dec(ACount, Integer(AIsStandard));
      if ACount > 0 then
...
procedure dxCustomVar(Sender: TdxSpreadSheetFormulaResult; const AParams: TdxSpreadSheetFormulaToken;
  const AIsStandard, AWithText, AErrorIfNonReferenceToken, AIgnoreHiddenRows, AForSubTotal: Boolean);
var
  AResult: Extended;
begin
  AResult := dxGetCustomVar(Sender, AParams, AIsStandard, AWithText, AErrorIfNonReferenceToken, AIgnoreHiddenRows, AForSubTotal);
  if Sender.Validate then
    Sender.AddValue(AResult);
end;
...
procedure fnSumIF(Sender: TdxSpreadSheetFormulaResult; const AParams: TdxSpreadSheetFormulaToken);
begin
  dxCustomSumIfCalculate(Sender, AParams, False, False);
end;

procedure fnSumIFS(Sender: TdxSpreadSheetFormulaResult; const AParams: TdxSpreadSheetFormulaToken);
begin
  dxCustomSumIfCalculate(Sender, AParams, True, False);
end;

procedure fnSumProduct(Sender: TdxSpreadSheetFormulaResult; const AParams: TdxSpreadSheetFormulaToken);

  function CheckParamsDimension(APreparedParams: TdxFastObjectList; var ARowCount, AColumnCount: Integer): Boolean;
  var
    I: Integer;
    ADimension, ACurrentDimension: TdxSpreadSheetFormulaTokenDimension;
    AErrorCode: TdxSpreadSheetFormulaErrorCode;
  begin
    ADimension := TdxSpreadSheetFormulaResult(APreparedParams.Items[0]).LastItem.GetDimension(AErrorCode);
    ARowCount := ADimension.RowCount;
    AColumnCount := ADimension.ColumnCount;
    for I := 1 to APreparedParams.Count - 1 do
    begin
      ACurrentDimension := TdxSpreadSheetFormulaResult(APreparedParams.Items[I]).LastItem.GetDimension(AErrorCode);
      if ADimension <> ACurrentDimension then
      begin
        Sender.SetError(ecValue);
        Break;
      end;
  ...
  function PrepareParams: TdxFastObjectList;
  var
    AParam: TdxSpreadSheetFormulaToken;
  ...
  begin
    ...
    AParam := AParams;
    while (AParam <> nil) and Sender.Validate do
    begin
      AParamResult := TdxFormulaAccess(AParam.Owner).CalculateAsArrayParam(AParam.FirstChild);
      Result.Add(AParamResult);
      Sender.SetError(AParamResult.ErrorCode);
      AParam := AParam.Next;
    end;
  end;

var
  AParamsCount, AParamIndex: Integer;
  APreparedParams: TdxFastObjectList;
...
begin
  ...
  APreparedParams := PrepareParams;
  try
    if Sender.Validate and CheckParamsDimension(APreparedParams, ARowCount, AColumnCount) then
    begin
      AResult := 0;
      ...
      begin
        ...
        begin
          ...
          AParamIndex := 0;
          while (AParamIndex < AParamsCount) and (AErrorCode = ecNone) and (ASubResult <> 0)  do
          begin
            AResultToken := TdxSpreadSheetFormulaResult(APreparedParams.Items[AParamIndex]).LastItem;
            AResultToken.GetValueAsArrayItem(ARow, AColumn, AValue, AErrorCode);
            if AErrorCode = ecNone then
              if dxIsNumberOrDateTime(AValue) then
              begin
                if dxIsDateTime(AValue) and (Sender.FormatSettings.DateTimeSystem <> dts1904) and (AValue <= 60) then
                  AValue := AValue - 1;
              ...
              begin
                ASubResult := 0;
                if not((AResultToken is TdxSpreadSheetFormulaArrayToken) and dxIsLogical(AValue)) then
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          AArrayX.GetValueAsArrayItem(ARow, AColumn, X, AErrorCode);
          if AErrorCode = ecNone then
          begin
            if CheckValue(X) then
            begin
              ExtractIndexes(GetCompositeIndex(ADimensionX, ARow, AColumn), ADimensionY, ARowY, AColumnY);
              AArrayY.GetValueAsArrayItem(ARowY, AColumnY, Y, AErrorCode);
...
begin
  ARepository.InternalAdd(@sfnAveDev, fnAveDev, fpiAveDev, frkNonArrayValue, 269, ftStatistical, @sfnAveDevDescription);
  ARepository.InternalAdd(@sfnAverage, fnAverage, fpiAverage, frkNonArrayValue, 5, ftStatistical, @sfnAverageDescription);
  ARepository.InternalAdd(@sfnAverageA, fnAverageA, fpiAverageA, frkNonArrayValue, 361, ftStatistical, @sfnAverageADescription);
  ARepository.InternalAdd(@sfnAverageIF, fnAverageIF, fpiAverageIF, frkNonArrayValue, 255, ftStatistical, @sfnAverageIFDescription, 0, False);
  ARepository.InternalAdd(@sfnAverageIFS, fnAverageIFS, fpiAverageIFS, frkNonArrayValue, 255, ftStatistical, @sfnAverageIFSDescription, 0, False);
  ARepository.InternalAdd(@sfnBeta_Dist, fnBeta_Dist, fpiBeta_Dist, frkValue, 255, ftStatistical, @sfnBeta_DistDescription, 1);
  ARepository.InternalAdd(@sfnBeta_Inv, fnBeta_Inv, fpiBeta_Inv, frkValue, 255, ftStatistical, @sfnBeta_InvDescription, 1);
  ARepository.InternalAdd(@sfnBinom_Dist, fnBinom_Dist, fpiBinom_Dist, frkValue, 255, ftStatistical, @sfnBinom_DistDescription, 1);
  ARepository.InternalAdd(@sfnBinom_Dist_Range, fnBinom_Dist_Range, fpiBinom_Dist_Range, frkValue, 255, ftStatistical, @sfnBinom_Dist_RangeDescription, 1);
  ARepository.InternalAdd(@sfnBinom_Inv, nil, nil, frkValue, 255, ftStatistical, @sfnBinom_InvDescription, 1);
  ARepository.InternalAdd(@sfnChiSQ_Dist, fnChiSQ_Dist, fpiChiSQ_Dist, frkValue, 255, ftStatistical, @sfnChiSQ_DistDescription, 1);
  ARepository.InternalAdd(@sfnChiSQ_Dist_RT, fnChiSQ_Dist_RT, fpiChiSQ_Dist_RT, frkValue, 255, ftStatistical, @sfnChiSQ_Dist_RTDescription, 1);
  ARepository.InternalAdd(@sfnChiSQ_Inv, fnChiSQ_Inv, fpiChiSQ_Inv, frkValue, 255, ftStatistical, @sfnChiSQ_InvDescription, 1);
  ARepository.InternalAdd(@sfnChiSQ_Inv_RT, fnChiSQ_Inv_RT, fpiChiSQ_Inv_RT, frkValue, 255, ftStatistical, @sfnChiSQ_Inv_RTDescription, 1);
  ARepository.InternalAdd(@sfnChiSQ_Test, nil, nil, frkValue, 255, ftStatistical, @sfnChiSQ_TestDescription, 1);
  ARepository.InternalAdd(@sfnConfidence_Norm, nil, nil, frkValue, 255, ftStatistical, @sfnConfidence_NormDescription, 1);
  ARepository.InternalAdd(@sfnConfidence_T, nil, nil, frkValue, 255, ftStatistical, @sfnConfidence_TDescription, 1);
  ARepository.InternalAdd(@sfnCorrel, fnCorrel, fpiCorrel, frkNonArrayValue, 307, ftStatistical, @sfnCorrelDescription, 0, False);
  ARepository.InternalAdd(@sfnCovariance_P, fnCovariance_P, fpiCovariance_P, frkNonArrayValue, 255, ftStatistical, @sfnCovariance_PDescription, 1, False);
  ARepository.InternalAdd(@sfnCovariance_S, fnCovariance_S, fpiCovariance_S, frkNonArrayValue, 255, ftStatistical, @sfnCovariance_SDescription, 1, False);
  ARepository.InternalAdd(@sfnCount, fnCount, fpiCount, frkNonArrayValue, 0, ftStatistical, @sfnCountDescription);
  ARepository.InternalAdd(@sfnCountA, fnCountA, fpiCountA, frkNonArrayValue, 169, ftStatistical, @sfnCountADescription);
  ARepository.InternalAdd(@sfnCountBlank, fnCountBlank, fpiCountBlank, frkNonArrayValue, 347, ftStatistical, @sfnCountBlankDescription);
  ARepository.InternalAdd(@sfnCountIF, fnCountIF, fpiCountIF, frkNonArrayValue, 346, ftStatistical, @sfnCountIFDescription, 0, False);
  ARepository.InternalAdd(@sfnCountIFS, fnCountIFS, fpiCountIFS, frkNonArrayValue, 255, ftStatistical, @sfnCountIFSDescription, 0, False);
  ARepository.InternalAdd(@sfnDevSQ, fnDevSQ, fpiDevSQ, frkNonArrayValue, 318, ftStatistical, @sfnDevSQDescription);
  ARepository.InternalAdd(@sfnExpon_Dist, fnExpon_Dist, fpiExpon_Dist, frkValue, 255, ftStatistical, @sfnExpon_DistDescription, 1);
  ARepository.InternalAdd(@sfnF_Dist, nil, nil, frkValue, 255, ftStatistical, @sfnF_DistDescription, 1);
  ARepository.InternalAdd(@sfnF_Dist_RT, nil, nil, frkValue, 255, ftStatistical, @sfnF_Dist_RTDescription, 1);
  ARepository.InternalAdd(@sfnF_Inv, nil, nil, frkValue, 255, ftStatistical, @sfnF_InvDescription, 1);
  ARepository.InternalAdd(@sfnF_Inv_RT, nil, nil, frkValue, 255, ftStatistical, @sfnF_Inv_RTDescription, 1);
  ARepository.InternalAdd(@sfnF_Test, nil, nil, frkValue, 255, ftStatistical, @sfnF_TestDescription, 1);
  ARepository.InternalAdd(@sfnFisher, nil, nil, frkValue, 283, ftStatistical, @sfnFisherDescription);
  ARepository.InternalAdd(@sfnFisherInv, nil, nil, frkValue, 284, ftStatistical, @sfnFisherInvDescription);
  ARepository.InternalAdd(@sfnForecast, fnForecast, fpiForecast, frkValue, 309, ftStatistical, @sfnForecastDescription, 0, False);
  ARepository.InternalAdd(@sfnFrequency, nil, nil, frkValue, 252, ftStatistical, @sfnFrequencyDescription);
  ARepository.InternalAdd(@sfnGamma, fnGamma, fpiGamma, frkValue, 255, ftStatistical, @sfnGammaDescription, 1);
  ARepository.InternalAdd(@sfnGamma_Dist, fnGamma_Dist, fpiGamma_Dist, frkValue, 255, ftStatistical, @sfnGamma_DistDescription, 1);
  ARepository.InternalAdd(@sfnGamma_Inv, fnGamma_Inv, fpiGamma_Inv, frkValue, 255, ftStatistical, @sfnGamma_InvDescription, 1);
  ARepository.InternalAdd(@sfnGammaLn, fnGammaLn, fpiGammaLn, frkValue, 271, ftStatistical, @sfnGammaLnDescription);
  ARepository.InternalAdd(@sfnGammaLn_Precise, fnGammaLn_Precise, fpiGammaLn_Precise, frkValue, 255, ftStatistical, @sfnGammaLn_PreciseDescription, 1);
  ARepository.InternalAdd(@sfnGauss, fnGauss, fpiGauss, frkValue, 255, ftStatistical, @sfnGaussDescription, 1);
  ARepository.InternalAdd(@sfnGeomean, fnGeomean, fpiGeomean, frkNonArrayValue, 319, ftStatistical, @sfnGeomeanDescription);
  ARepository.InternalAdd(@sfnGrowth, nil, nil, frkValue, 52, ftStatistical, @sfnGrowthDescription);
  ARepository.InternalAdd(@sfnHarmean, nil, nil, frkValue, 320, ftStatistical, @sfnHarmeanDescription);
  ARepository.InternalAdd(@sfnHypgeom_Dist, fnHypgeom_Dist, fpiHypgeom_Dist, frkValue, 255, ftStatistical, @sfnHypgeom_DistDescription, 1);
  ARepository.InternalAdd(@sfnIntercept, fnIntercept, fpiIntercept, frkNonArrayValue, 311, ftStatistical, @sfnInterceptDescription, 0, False);
  ARepository.InternalAdd(@sfnKurt, nil, nil, frkValue, 322, ftStatistical, @sfnKurtDescription);
  ARepository.InternalAdd(@sfnLarge, fnLarge, fpiLarge, frkValue, 325, ftStatistical, @sfnLargeDescription, 0, False);
  ARepository.InternalAdd(@sfnLinest, nil, nil, frkValue, 49, ftStatistical, @sfnLinestDescription);
  ARepository.InternalAdd(@sfnLogest, nil, nil, frkValue, 51, ftStatistical, @sfnLogestDescription);
  ARepository.InternalAdd(@sfnLogNorm_Dist, nil, nil, frkValue, 255, ftStatistical, @sfnLogNorm_DistDescription, 1);
  ARepository.InternalAdd(@sfnLogNorm_Inv, nil, nil, frkValue, 255, ftStatistical, @sfnLogNorm_InvDescription, 1);
  ARepository.InternalAdd(@sfnMax, fnMax, fpiMax, frkNonArrayValue, 7, ftStatistical, @sfnMaxDescription);
  ARepository.InternalAdd(@sfnMaxA, fnMaxA, fpiMaxA, frkNonArrayValue, 362, ftStatistical, @sfnMaxADescription);
  ARepository.InternalAdd(@sfnMedian, fnMedian, fpiMedian, frkNonArrayValue, 227, ftStatistical, @sfnMedianDescription);
  ARepository.InternalAdd(@sfnMin, fnMin, fpiMin, frkNonArrayValue, 6, ftStatistical, @sfnMinDescription);
  ARepository.InternalAdd(@sfnMinA, fnMinA, fpiMinA, frkNonArrayValue, 363, ftStatistical, @sfnMinADescription);
  ARepository.InternalAdd(@sfnMode_Mult, fnMode_Mult, fpiMode_Mult, frkArray, 255, ftStatistical, @sfnMode_MultDescription, 1, False);
  ARepository.InternalAdd(@sfnMode_SNGL, fnMode_Sngl, fpiMode_Sngl, frkNonArrayValue, 255, ftStatistical, @sfnMode_SNGLDescription, 1);
  ARepository.InternalAdd(@sfnNegBinom_Dist, nil, nil, frkValue, 255, ftStatistical, @sfnNegBinom_DistDescription, 1);
  ARepository.InternalAdd(@sfnNorm_Dist, fnNorm_Dist, fpiNorm_Dist, frkValue, 255, ftStatistical, @sfnNorm_DistDescription, 1);
  ARepository.InternalAdd(@sfnNorm_Inv, fnNorm_Inv, fpiNorm_Inv, frkValue, 255, ftStatistical, @sfnNorm_InvDescription, 1);
  ARepository.InternalAdd(@sfnNorm_S_Dist, fnNorm_S_Dist, fpiNorm_S_Dist, frkValue, 255, ftStatistical, @sfnNorm_S_DistDescription, 1);
  ARepository.InternalAdd(@sfnNorm_S_Inv, fnNorm_S_Inv, fpiNorm_S_Inv, frkValue, 255, ftStatistical, @sfnNorm_S_InvDescription, 1);
  ARepository.InternalAdd(@sfnPearson, fnPearson, fpiPearson, frkNonArrayValue, 312, ftStatistical, @sfnPearsonDescription, 0, False);
  ARepository.InternalAdd(@sfnPercentile_Exc, fnPercentile_Exc, fpiPercentile_Exc, frkValue, 255, ftStatistical, @sfnPercentile_ExcDescription, 1, False);
  ARepository.InternalAdd(@sfnPercentile_Inc, fnPercentile_Inc, fpiPercentile_Inc, frkValue, 255, ftStatistical, @sfnPercentile_IncDescription, 1, False);
  ARepository.InternalAdd(@sfnPercentRank_Exc, nil, nil, frkValue, 255, ftStatistical, @sfnPercentRank_ExcDescription, 1);
  ARepository.InternalAdd(@sfnPercentRank_Inc, nil, nil, frkValue, 255, ftStatistical, @sfnPercentRank_IncDescription, 1);
  ARepository.InternalAdd(@sfnPermut, fnPermut, fpiPermut, frkValue, 299, ftStatistical, @sfnPermutDescription);
  ARepository.InternalAdd(@sfnPermutationA, nil, nil, frkValue, 255, ftStatistical, @sfnPermutationADescription, 1);
  ARepository.InternalAdd(@sfnPHI, nil, nil, frkValue, 255, ftStatistical, @sfnPHIDescription, 1);
  ARepository.InternalAdd(@sfnPoisson_Dist, fnPoisson_Dist, fpiPoisson_Dist, frkValue, 255, ftStatistical, @sfnPoisson_DistDescription, 1);
  ARepository.InternalAdd(@sfnProb, nil, nil, frkValue, 317, ftStatistical, @sfnProbDescription);
  ARepository.InternalAdd(@sfnQuartile_Exc, fnQuartile_Exc, fpiQuartile_Exc, frkValue, 255, ftStatistical, @sfnQuartile_ExcDescription, 1, False);
  ARepository.InternalAdd(@sfnQuartile_Inc, fnQuartile_Inc, fpiQuartile_Inc, frkValue, 255, ftStatistical, @sfnQuartile_IncDescription, 1, False);
  ARepository.InternalAdd(@sfnRank_Avg, fnRank_Avg, fpiRank_Avg, frkValue, 255, ftStatistical, @sfnRank_AvgDescription, 1, False);
  ARepository.InternalAdd(@sfnRank_Eq, fnRank_Eq, fpiRank_Eq, frkValue, 255, ftStatistical, @sfnRank_EqDescription, 1, False);
  ARepository.InternalAdd(@sfnRSQ, fnRSQ, fpiRSQ, frkNonArrayValue, 313, ftStatistical, @sfnRSQDescription, 0, False);
  ARepository.InternalAdd(@sfnSkew, fnSkew, fpiSkew, frkNonArrayValue, 323, ftStatistical, @sfnSkewDescription);
  ARepository.InternalAdd(@sfnSkew_P, fnSkew_P, fpiSkew_P, frkNonArrayValue, 255, ftStatistical, @sfnSkew_PDescription, 1);
  ARepository.InternalAdd(@sfnSlope, fnSlope, fpiSlope, frkNonArrayValue, 315, ftStatistical, @sfnSlopeDescription, 0, False);
  ARepository.InternalAdd(@sfnSmall, fnSmall, fpiSmall, frkValue, 326, ftStatistical, @sfnSmallDescription, 0, False);
  ARepository.InternalAdd(@sfnStandardize, fnStandardize, fpiStandardize, frkValue, 297, ftStatistical, @sfnStandardizeDescription);
  ARepository.InternalAdd(@sfnStDev_P, fnStDev_P, fpiStDev_P, frkNonArrayValue, 255, ftStatistical, @sfnStDev_PDescription, 1);
  ARepository.InternalAdd(@sfnStDev_S, fnStDev_S, fpiStDev_S, frkNonArrayValue, 255, ftStatistical, @sfnStDev_SDescription, 1);
  ARepository.InternalAdd(@sfnStDevA, fnStDevA, fpiStDevA, frkNonArrayValue, 366, ftStatistical, @sfnStDevADescription);
  ARepository.InternalAdd(@sfnStDevPA, fnStDevPA, fpiStDevPA, frkNonArrayValue, 364, ftStatistical, @sfnStDevPADescription);
  ARepository.InternalAdd(@sfnSTEYX, fnSTEYX, fpiSTEYX, frkNonArrayValue, 314, ftStatistical, @sfnSTEYXDescription, 0, False);
  ARepository.InternalAdd(@sfnT_Dist, fnT_Dist, fpiT_Dist, frkValue, 255, ftStatistical, @sfnT_DistDescription, 1);
  ARepository.InternalAdd(@sfnT_Dist_2T, fnT_Dist_2T, fpiT_Dist_2T, frkValue, 255, ftStatistical, @sfnT_Dist_2TDescription, 1);
  ARepository.InternalAdd(@sfnT_Dist_RT, fnT_Dist_RT, fpiT_Dist_RT, frkValue, 255, ftStatistical, @sfnT_Dist_RTDescription, 1);
  ARepository.InternalAdd(@sfnT_Inv, fnT_Inv, fpiT_Inv, frkValue, 255, ftStatistical, @sfnT_InvDescription, 1);
  ARepository.InternalAdd(@sfnT_Inv_2T, fnT_Inv_2T, fpiT_Inv_2T, frkValue, 255, ftStatistical, @sfnT_Inv_2TDescription, 1);
  ARepository.InternalAdd(@sfnT_Test, nil, nil, frkValue, 255, ftStatistical, @sfnT_TestDescription, 1);
  ARepository.InternalAdd(@sfnTrend, nil, nil, frkValue, 50, ftStatistical, @sfnTrendDescription);
  ARepository.InternalAdd(@sfnTrimMean, nil, nil, frkValue, 331, ftStatistical, @sfnTrimMeanDescription);
  ARepository.InternalAdd(@sfnVar_P, fnVar_P, fpiVar_P, frkNonArrayValue, 255, ftStatistical, @sfnVar_PDescription, 1);
  ARepository.InternalAdd(@sfnVar_S, fnVar_S, fpiVar_S, frkNonArrayValue, 255, ftStatistical, @sfnVar_SDescription, 1);
  ARepository.InternalAdd(@sfnVarA, fnVarA, fpiVarA, frkNonArrayValue, 367, ftStatistical, @sfnVarADescription);
  ARepository.InternalAdd(@sfnVarPA, fnVarPA, fpiVarPA, frkNonArrayValue, 365, ftStatistical, @sfnVarPADescription);
  ARepository.InternalAdd(@sfnWeibull_Dist, fnWeibull_Dist, fpiWeibull_Dist, frkValue, 255, ftStatistical, @sfnWeibull_DistDescription, 1);
  ARepository.InternalAdd(@sfnZ_Test, nil, nil, frkValue, 255, ftStatistical, @sfnZ_TestDescription, 1);
end;
