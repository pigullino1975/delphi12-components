...
type
  ...
  TdxSpreadSheetFormulaEnumReferenceTokensProc = reference to procedure (const AArea: TRect; AView: TObject; AToken: TdxSpreadSheetFormulaToken);
  ...
  strict private
    ...
    function GetItem(AIndex: TdxListIndex): TdxSpreadSheetCustomFormula;
  protected
    FMaxIterationCount: Integer;
    ...
    function IsPartOfArrayFormula(AView: TObject; const ARow, AColumn: Integer;
      var AFormulaArea: TRect; AFormula: PdxSpreadSheetCustomFormula = nil): TdxSpreadSheetArrayFormulaPart;
    ...
    property MaxIterationCount: Integer read FMaxIterationCount;
  ...
  public
    ...
    property Items[Index: TdxListIndex]: TdxSpreadSheetCustomFormula read GetItem; default;
  end;
  ...
  strict private
    ...
    FCachedResultValueIsValid: Boolean;
    FCalculatingSubResult: TdxSpreadSheetFormulaResult;
    FErrorCode: TdxSpreadSheetFormulaErrorCode;
    FErrorIndex: Integer;
    FIsArrayFormula: Boolean;
    FIteration: Integer;
    FNeedCalculateParamsAsArray: Boolean;
    FSourceText: string;

    function BeginIteration(out AState: Integer): Boolean;
    function EndIteration(const AState: Integer): Boolean;
  ...
  protected
    ...
    procedure CacheResultValue(const AValue: TdxSpreadSheetFormulaResult); overload;
    procedure CacheResultValue(const AValue: Variant); overload;
    ...
    function CalculateAsArrayParam(AExpression: TdxSpreadSheetFormulaToken): TdxSpreadSheetFormulaResult;
    procedure CalculateResultValue; virtual;
    ...
    property CachedResultValue: Variant read FCachedResultValue;
    property CachedResultValueIsValid: Boolean read FCachedResultValueIsValid;
    property NeedCalculateParamsAsArray: Boolean read FNeedCalculateParamsAsArray;
    property ValueAsText: string read GetValueAsText;
  public
    destructor Destroy; override;
    procedure AfterConstruction; override;
    procedure BeforeDestruction; override;
    procedure EnumReferences(AProc: TdxSpreadSheetFormulaEnumReferencesProc; AProcessDefinedNames: Boolean = False); overload;
  ...
  protected
    ...
    // to String
    procedure ToString(AStack: TStringList); reintroduce; virtual;
    ...
    function Cacheable: Boolean; virtual;
    ...
    function SupportsIsect: Boolean; virtual;
  ...
  public
    ...
    procedure EnumReferences(AProc: TdxSpreadSheetFormulaEnumReferenceTokensProc; AProcessDefinedNames: Boolean = False); virtual;
    ...
    function GetValueAsText(out AText: string): Boolean; virtual;
    ...
    // Utilities
    class function Add(APrev, AToken: TdxSpreadSheetFormulaToken): TdxSpreadSheetFormulaToken; inline;
    class procedure AddChild(AParent, AToken: TdxSpreadSheetFormulaToken);
    class function Append(APrev, AToken: TdxSpreadSheetFormulaToken): TdxSpreadSheetFormulaToken; inline;
    class procedure DestroyTokens(var ATokens: TdxSpreadSheetFormulaToken);
    class function FindToken(ASourcePosition: Integer; ATokens: TdxSpreadSheetFormulaToken): TdxSpreadSheetFormulaToken;
    class procedure Remove(var AList: TdxSpreadSheetFormulaToken; AToken: TdxSpreadSheetFormulaToken); inline;
    class procedure Replace(var ASource: TdxSpreadSheetFormulaToken; const ADest: TdxSpreadSheetFormulaToken); overload;
    class procedure Replace(var AList, ASource: TdxSpreadSheetFormulaToken; const ADest: TdxSpreadSheetFormulaToken); overload;
    ...
    property View: TObject read GetView;
    //
    property SourceFinish: Word read FSourceFinish write FSourceFinish;
    property SourceStart: Word read FSourceStart write FSourceStart;
  end;
  ...
  TdxSpreadSheet3DReferenceCustomLinkClass = class of TdxSpreadSheet3DReferenceCustomLink;
  TdxSpreadSheet3DReferenceCustomLink = class
  strict private
    FLink: IdxSpreadSheetObjectLink;

    function GetActualData: TObject;
  ...
  public
    ...
    destructor Destroy; override;
    function Clone: TdxSpreadSheet3DReferenceCustomLink; virtual;

    property ActualData: TObject read GetActualData;
  ...
  public
    ...
    function Clone: TdxSpreadSheet3DReferenceCustomLink; override;
    //
  ...
  protected
    ...
    function HasSimpleValue: Boolean;
  public
    ...
    procedure AddValue(const AValue: TDateTime); overload; inline;
    ...
    function ExtractStringParameter(const AParams: TdxSpreadSheetFormulaToken; AIndex: Integer = 0): string; inline;
...
procedure dxSpreadSheetCalculateExpression(AExpression: TdxSpreadSheetFormulaToken;
  var AResult: TdxSpreadSheetFormulaResult; const AForceConvertNullValueToZero: Boolean = False);
function dxSpreadSheetExpressionToText(AExpression: TdxSpreadSheetFormulaToken): string; overload;
procedure dxSpreadSheetExpressionToText(ABuffer: TStringBuilder; AExpression: TdxSpreadSheetFormulaToken); overload;
...
function dxSpreadSheetParseCellReference(AReference: string;
  AController: TdxSpreadSheetCustomFormulaController; out AScope: TObject; out AArea: TRect): Boolean;
...
const
  dxThisUnitName = 'dxSpreadSheetCoreFormulas';

type
  TResultAccess = class(TdxSpreadSheetFormulaResult);
  TTokenAccess = class(TdxSpreadSheetFormulaToken);
  TReferenceTokenAccess = class(TdxSpreadSheetFormulaReferenceToken);

  TdxSpreadSheetFakeFormula = class(TdxSpreadSheetCustomFormula)
  strict private
    FController: TdxSpreadSheetCustomFormulaController;
  protected
    function GetController: TdxSpreadSheetCustomFormulaController; override;
  public
    constructor Create(AController: TdxSpreadSheetCustomFormulaController);
  end;
...
function dxSpreadSheetExpressionToText(AExpression: TdxSpreadSheetFormulaToken): string;
var
...
begin
  ...
  try
    dxSpreadSheetExpressionToText(ABuffer, AExpression);
...
procedure dxSpreadSheetExpressionToText(ABuffer: TStringBuilder; AExpression: TdxSpreadSheetFormulaToken);
var
  AStack: TStringList;
  I: Integer;
begin
  if AExpression <> nil then
  begin
    AStack := TStringList.Create;
    try
      AStack.Capacity := 8;
      while AExpression <> nil do
      begin
        AExpression.ToString(AStack);
        AExpression := AExpression.Next;
      end;
      for I := 0 to AStack.Count - 1 do
        ABuffer.Append(AStack[I]);
    finally
      AStack.Free;
    end;
  end;
...
function dxSpreadSheetIsReferenceToken(var AToken: TdxSpreadSheetFormulaToken; var AResult: TdxSpreadSheetFormulaResult): Boolean;
begin
  if not (AToken is TdxSpreadSheetFormulaReferenceToken) then
  begin
...
function dxSpreadSheetParseCellReference(AReference: string;
  AController: TdxSpreadSheetCustomFormulaController; out AScope: TObject; out AArea: TRect): Boolean;
var
  AFormula: TdxSpreadSheetFakeFormula;
...
begin
  ...
  AFormula := TdxSpreadSheetFakeFormula.Create(AController);
  try
    AParser := AController.CreateParser as TdxSpreadSheetCustomFormulaParser;
    try
      if not AParser.ParseFormula(dxSpreadSheetFormulaIncludeEqualSymbol(AReference), AFormula) then
        Exit(False);
    finally
      AParser.Free;
    end;

    if AFormula.FTokens = nil then
      Exit(False);
    if AFormula.FTokens.Child <> nil then
      Exit(False);
    if AFormula.FTokens is TdxSpreadSheetFormulaReferenceToken then
      Exit(TReferenceTokenAccess(AFormula.FTokens).ExtractReference(AScope, AArea));
    Result := False;
  finally
    AFormula.Free;
  end;
...
{ TdxSpreadSheetFakeFormula }

constructor TdxSpreadSheetFakeFormula.Create(AController: TdxSpreadSheetCustomFormulaController);
begin
  inherited Create;
  FController := AController;
end;

function TdxSpreadSheetFakeFormula.GetController: TdxSpreadSheetCustomFormulaController;
begin
  Result := FController;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      if not AFormula.IsArrayFormula and
        (IsPartOfArrayFormula(AFormula.View, AFormula.AnchorRow, AFormula.AnchorColumn, AFormulaArea) = afpSlaveCell) then
      begin
        AFormula.Free;
        Continue;
      end;
  ...
  try
    ...
    begin
      if ABuffer.Length > 0 then
        ...
        ABuffer.Append(cxGetResourceString(@sdxErrorCircularPathPrefix));

      ABuffer.Append(APath.ToString);
      APath := APath.Next;
    end;
    Result := ABuffer.ToString;
  finally
...
function TdxSpreadSheetCustomFormulaController.IsPartOfArrayFormula(
  AView: TObject; const ARow, AColumn: Integer;
  var AFormulaArea: TRect; AFormula: PdxSpreadSheetCustomFormula): TdxSpreadSheetArrayFormulaPart;
var
...
begin
  Result := afpNone;
  AFormulaArea := cxInvalidRect;
  if (FArrayFormulas.Count > 0) and FArrayFormulas.TryGetValue(AView, AList) then
    for I := 0 to AList.Count - 1 do
    begin
      AItem := TdxSpreadSheetCustomFormula(AList.List[I]);
      if dxSpreadSheetContains(AItem.ArrayFormulaArea, ARow, AColumn) then
      begin
        AFormulaArea := AItem.ArrayFormulaArea;
    ...
    case ACell.DataType of
      ...
      cdtFormula:
        begin
          ...
          AddPath(ARow, AColumn, AFormula.View);
          try
            ...
            RemovePath(ARow, AColumn, AFormula.View);
          end;
        end;
...
function TdxSpreadSheetCustomFormulaController.GetItem(AIndex: TdxListIndex): TdxSpreadSheetCustomFormula;
begin
...
procedure TdxSpreadSheetCustomFormula.CacheResultValue(const AValue: TdxSpreadSheetFormulaResult);
begin
  FCachedResultValueIsValid := AValue.HasSimpleValue;
  if FCachedResultValueIsValid then
    FCachedResultValue := AValue.Value;
end;

procedure TdxSpreadSheetCustomFormula.CacheResultValue(const AValue: Variant);
begin
  FCachedResultValue := AValue;
  FCachedResultValueIsValid := True;
end;
...
begin
  FCachedResultValueIsValid := False;
  FIteration := 0;
...
function TdxSpreadSheetCustomFormula.CalculateAsArrayParam(AExpression: TdxSpreadSheetFormulaToken): TdxSpreadSheetFormulaResult;
begin
  FNeedCalculateParamsAsArray := True;
  try
    Result := Calculate(AExpression);
  finally
    FNeedCalculateParamsAsArray := False;
  end;
end;

procedure TdxSpreadSheetCustomFormula.CalculateResultValue;
var
  ACanCalculate: Boolean;
  AState: Integer;
  ATempResult: TdxSpreadSheetFormulaResult;
begin
  ACanCalculate := BeginIteration(AState);
  try
    ATempResult := nil;
    try
      if ACanCalculate then
      ...
      CacheResultValue(ATempResult);
      FreeAndNil(FResultValue);    
      FResultValue := ATempResult;
    end;
  finally
    if EndIteration(AState) then
      UpdateSlaveCells;
  end;
end;
...
var
  ...
  AState: Integer;
begin
  if CachedResultValueIsValid then
    Exit(CachedResultValue);

  if IsCalculationInProcess then
  begin
...
function TdxSpreadSheetCustomFormula.GetValueAsText: string;
begin
  if FResultValue <> nil then
    Result := FResultValue.ValueAsText
  else
...
function TdxSpreadSheetCustomFormula.BeginIteration(out AState: Integer): Boolean;
begin
  AState := Iteration;
  Inc(FIteration);
  Result := (Iteration = 1) or (Iteration < Controller.MaxIterationCount);
  if not Result and (Controller.MaxIterationCount = 1) then
  begin
    Controller.CircularReference := True;
    if FResultValue <> nil then
      FResultValue.HasCircularReferences := True;
    Controller.CircularReferencePaths.Add(TdxSpreadSheetReferencePath.Create(AnchorRow, AnchorColumn, View));
  end;
end;

function TdxSpreadSheetCustomFormula.EndIteration(const AState: Integer): Boolean;
begin
  Result := AState = 0;
  if Result then
    FIteration := 0;
...
begin
  Result := FErrorCode;
  if (Result = ecNone) and (FResultValue <> nil) then
    Result := FResultValue.ErrorCode;
end;
...
begin
  if ErrorIndex <> 0 then
    Exit(dxSpreadSheetFormulaIncludeEqualSymbol(SourceText));
  if FTokens = nil then
    Exit(FormatSettings.Operations[opEQ]);

  ABuffer := TdxStringBuilderManager.Get;
  try
    ABuffer.Append(FormatSettings.Operations[opEQ]);
    dxSpreadSheetExpressionToText(ABuffer, FTokens);
    Result := ABuffer.ToString;
  finally
    TdxStringBuilderManager.Release(ABuffer);
  end;
...
function TdxSpreadSheetFormulaToken.Cacheable: Boolean;
begin
  Result := not IsEnumeration;
end;
...
var
  ...
  AFormulaArea: TRect;
begin
  if dxSpreadSheetIsValidCellReference(ARow, AColumn) and Supports(AView, IdxSpreadSheetViewData, AViewData) then
  begin
    AErrorCode := ecNone;
    if Controller.IsPartOfArrayFormula(AView, ARow, AColumn, AFormulaArea, @AArrayFormula) <> afpNone then
...
begin
  ...
  if VarIsStr(ACellValue) then
  begin
    ACell := GetDataProvider(AView).GetCellData(ARow, AColumn);
    if ((ACell = nil) or (ACell.DataType <> cdtString)) and dxTryStrToOrdinal(ACellValue, AValue, Owner.FormatSettings) then
      Exit;
  end;
  AValue := ACellValue;
end;
...
function TdxSpreadSheetFormulaToken.SupportsIsect: Boolean;
begin
  Result := False;
end;
...
function TdxSpreadSheetFormulaToken.GetValueAsText(out AText: string): Boolean;
begin
  Result := False;
end;
...
class function TdxSpreadSheetFormulaToken.Add(APrev, AToken: TdxSpreadSheetFormulaToken): TdxSpreadSheetFormulaToken;
begin
  Result := AToken;
  if APrev = nil then
    Exit;
...
class function TdxSpreadSheetFormulaToken.Append(APrev, AToken: TdxSpreadSheetFormulaToken): TdxSpreadSheetFormulaToken;
begin
  if APrev <> nil then
  begin
...
class function TdxSpreadSheetFormulaToken.FindToken(
  ASourcePosition: Integer; ATokens: TdxSpreadSheetFormulaToken): TdxSpreadSheetFormulaToken;
begin
  Result := nil;
  while ATokens <> nil do
  begin
    if (ASourcePosition >= ATokens.SourceStart) and (ASourcePosition <= ATokens.SourceFinish) or
       (ASourcePosition  = ATokens.SourceStart) and (ATokens.SourceStart > ATokens.SourceFinish) 
    then
      begin
        Result := FindToken(ASourcePosition, ATokens.FirstChild);
        if Result = nil then
          Result := ATokens;
        Break;
      end;

    ATokens := ATokens.Next;
  end;
end;

class procedure TdxSpreadSheetFormulaToken.Remove(var AList: TdxSpreadSheetFormulaToken; AToken: TdxSpreadSheetFormulaToken);
begin
  if AToken = AList then
  begin
    if AToken.Prev <> nil then
      AList := AToken.Prev
    else
      AList := AToken.Next;
  end;
...
class procedure TdxSpreadSheetFormulaToken.Replace(
  var AList, ASource: TdxSpreadSheetFormulaToken; const ADest: TdxSpreadSheetFormulaToken);
begin
  if AList = ASource then
    AList := ADest;
  Replace(ASource, ADest);
end;
...
procedure TdxSpreadSheetFormulaToken.ToString(AStack: TStringList);
begin
  AStack.Add(dxSpreadSheetExpressionToText(FChildFirst));
end;
...
function TdxSpreadSheet3DReferenceCustomLink.Clone: TdxSpreadSheet3DReferenceCustomLink;
begin
  Result := TdxSpreadSheet3DReferenceCustomLinkClass(ClassType).Create(nil);
  Result.FLink := FLink;
end;

function TdxSpreadSheet3DReferenceCustomLink.GetActualData: TObject;
begin
  Result := Data;
  if not TdxSpreadSheetInvalidObject.IsLive(Result) then
    Result := nil;
end;
...
begin
  if AValue <> nil then
  begin
    if not Supports(AValue, IdxSpreadSheetObjectLink, FLink) then
      raise EdxSpreadSheetError.Create('IdxSpreadSheetObjectLink is not supported');
  end
  else
    FLink := nil;
end;
...
function TdxSpreadSheet3DReferenceCustomExternalLink.Clone: TdxSpreadSheet3DReferenceCustomLink;
begin
  Result := inherited;
  TdxSpreadSheet3DReferenceCustomExternalLink(Result).FName := Name;
end;
...
begin
{$IFDEF DEBUG}
  if (AValue.Owner <> nil) and (AValue.Owner <> Owner) then
...
procedure TdxSpreadSheetFormulaResult.AddValue(const AValue: TDateTime);
begin
  AddTemporary(TdxSpreadSheetFormulaDateTimeValueToken.Create(AValue));
end;
  ...
  if VarIsStr(AValue) and not ACanConvertStr then
    ...
    if dxSpreadSheetIsNullValue(AValue) then
      ...
      begin
        ...
        begin
          if not VarIsNumericEx(AValue) and not VarIsDate(AValue) then
          begin
            if dxTryStrToDateTime(AValue, ADate, FormatSettings) or dxIsShortDate(AValue, ADate, FormatSettings.Data) then
            begin
...
begin
  ...
  if Validate then
    FErrorCode := ecValue;
  Result := dxSpreadSheetErrorCodeToString(FErrorCode);
end;

function TdxSpreadSheetFormulaResult.GetValueAsText: string;
begin
  if Validate and (Values.Count > 0)then
  begin
    if LastItem.GetValueAsText(Result) then
      Exit;
  end;
  Result := VarToStr(Value);
end;

function TdxSpreadSheetFormulaResult.HasSimpleValue: Boolean;
begin
  Result := (Values.Count = 0) or not TdxSpreadSheetFormulaToken(Values.Last).IsEnumeration;
end;
...
function TdxSpreadSheetFormulaResult.ExtractStringParameter(
  const AParams: TdxSpreadSheetFormulaToken; AIndex: Integer = 0): string;
var
  ACanConvertStrToNumber: Boolean;
  AValue: Variant;
begin
  if ExtractParameter(AValue, ACanConvertStrToNumber, AParams, AIndex) then
