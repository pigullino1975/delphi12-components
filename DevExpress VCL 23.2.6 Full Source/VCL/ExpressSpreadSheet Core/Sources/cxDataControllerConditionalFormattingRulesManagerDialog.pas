...
const
  dxThisUnitName = 'cxDataControllerConditionalFormattingRulesManagerDialog';
...
begin
  ...
  begin
    ...
    begin
      with TcxDataControllerConditionalFormattingFormulaItemReference(AToken) do
    ...
    begin
      ...
      if AArea.Left <> AArea.Right then
        for I := 0 to ConditionalFormatting.DataController.ItemCount - 1 do
          if ConditionalFormatting.IsItemVisible(I) then
          begin
            AArea := TRect.Create(I, 0, I, MaxInt);
            Break;
          end;
        if AArea.Left <> AArea.Right then
...
begin
  ...
  for I := 0 to ConditionalFormatting.DataController.ItemCount - 1 do
    if ConditionalFormatting.IsItemVisible(I) then
    begin
      AProperties.Items.AddObject(ConditionalFormatting.GetItemDisplayName(I), TObject(I));
      cbDisplayMode.Properties.Items.AddObject(ConditionalFormatting.GetItemDisplayName(I), TObject(I));
    end;
...
begin
  ...
  begin
    AItemIndex := Integer(cbDisplayMode.Properties.Items.Objects[cbDisplayMode.ItemIndex]);
...
begin
  ...
  AItemIndex := Integer(AComboBox.Properties.Items.Objects[AComboBox.ItemIndex]);
