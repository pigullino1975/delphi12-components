...
type
  ...
  protected
    procedure AreasChangeHandler(Sender: TObject); override;
  ...
  protected
    ...
    function GetStopName(AStop: TdxSpreadSheetConditionalFormattingRuleCustomScaleStop): string; override;
    function IsRightToLeftDirection: Boolean; overload; virtual;
    function IsRightToLeftDirection(ACanvas: TcxCanvas): Boolean; overload; virtual;
...
const
  dxThisUnitName = 'dxSpreadSheetConditionalFormattingRules';
...
begin
  ...
  begin
    if ABordersStyles[ASide] <> sscbsDefault then
      dxSpreadSheetDrawBorder(ACanvas, R, Style.Borders[ASide].Color, ABackgroundColor, ASide, ABordersStyles);
  end;
...
procedure TdxSpreadSheetConditionalFormattingRuleExpression.AreasChangeHandler(Sender: TObject);
begin
...
function TdxSpreadSheetConditionalFormattingRuleCellIs.CanApplyCore(
  ...
  function InnerCompare(const V1, V2: Variant): TVariantRelationship;
  const
  ...
  begin
    try
      V1Str := VarIsStr(V1);
      V2Str := VarIsStr(V2);
      if V1Str and V2Str then
        Result := Map[Sign(dxSpreadSheetCompareText(V1, V2))]
      else
        if V1Str then
        begin
          if TryStrToFloat(V1, AValue) then
            Result := VarCompareValue(AValue, V2)
          else
            Result := vrNotEqual;
        end
        else
          if V2Str then
          begin
            if TryStrToFloat(V2, AValue) then
              Result := VarCompareValue(V1, AValue)
            else
              Result := vrNotEqual;
          end
          else
            Result := VarCompareValue(V1, V2);
    except
      Result := vrNotEqual;
    end;
  end;

  function InRange(const AValue, AMinValue, AMaxValue: Variant): Boolean;
  begin
    Result :=
      (InnerCompare(AValue, AMinValue) in [vrGreaterThan, vrEqual]) and
      (InnerCompare(AValue, AMaxValue) in [vrLessThan, vrEqual]);
  end;
...
begin
  ...
  case ComparisonOperator of
    cicoGreaterThan:
      Result := InnerCompare(AValue, AFormulaValue1) = vrGreaterThan;
    cicoLessThan:
      Result := InnerCompare(AValue, AFormulaValue1) = vrLessThan;
    cicoGreaterThanOrEqual:
      Result := InnerCompare(AValue, AFormulaValue1) in [vrGreaterThan, vrEqual];
    cicoLessThanOrEqual:
      Result := InnerCompare(AValue, AFormulaValue1) in [vrLessThan, vrEqual];
    cicoEqual:
      Result := InnerCompare(AValue, AFormulaValue1) = vrEqual;
    cicoNotEqual:
      Result := InnerCompare(AValue, AFormulaValue1) <> vrEqual;
  else // cicoBetween cicoNotBetween
    Result := (ComparisonOperator = cicoNotBetween) xor (
      InRange(AValue, AFormulaValue1, AFormulaValue2) or
      InRange(AValue, AFormulaValue2, AFormulaValue1));
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      dxSpreadSheetParseReference(dxSpreadSheetTextValueAsString(AReference, dxStringMarkChar), AFormula);
    end;
  end
  else
    inherited;
...
var
  ARank: Integer;
begin
  if (ACell = nil) or ACell.IsEmpty or not ACell.IsNumericValue then
    Exit(False);
...
begin
  ...
  if Style.FillMode = dbfmGradient then
    AColor2 := clWhite;
  if IsRightToLeftDirection(ACanvas) then
...
function TdxSpreadSheetConditionalFormattingRuleDataBar.IsRightToLeftDirection(ACanvas: TcxCanvas): Boolean;
begin
  Result := (Style.Direction = dbdRightToLeft) or (Style.Direction = dbdAuto) and ACanvas.UseRightToLeftAlignment;
end;
