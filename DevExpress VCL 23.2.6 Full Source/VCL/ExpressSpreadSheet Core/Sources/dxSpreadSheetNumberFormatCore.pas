...
uses
  System.UITypes,
  Windows, Types, SysUtils, Graphics, Math, StrUtils, Variants, Generics.Collections, Generics.Defaults,
  dxCore, cxGraphics, dxSpreadSheetCoreStrs, dxSpreadSheetUtils, dxSpreadSheetTypes, dxSpreadSheetClasses;

type
  TdxNumberFormatResult = class;

  { TdxNumberFormatCodeAbstractBuilder }

  TdxNumberFormatCodeAbstractBuilder = class
  protected
    function GetFormatSettings: TFormatSettings; virtual; abstract;
  public
    procedure Append(const V: Integer); overload;
    procedure Append(const V: string); overload; virtual; abstract;
    procedure AppendDesignator(ID: Integer); virtual; abstract;
    function MustBeEscaped(AChar: Char): Boolean; virtual; abstract;
  ...
  public
    procedure BuildCode(const ABuilder: TdxNumberFormatCodeAbstractBuilder); virtual; abstract;
  ...
  public
    procedure BuildCode(const ABuilder: TdxNumberFormatCodeAbstractBuilder); virtual;
  ...
  public
    procedure BuildCode(const ABuilder: TdxNumberFormatCodeAbstractBuilder);
  ...
  public
    ...
    procedure BuildCode(const ABuilder: TdxNumberFormatCodeAbstractBuilder); override;
...
//----------------------------------------------------------------------------------------------------------------------
  ...
  protected
    procedure BuildCodeCore(const ABuilder: TdxNumberFormatCodeAbstractBuilder); virtual; abstract;
  public
    procedure BuildCode(const ABuilder: TdxNumberFormatCodeAbstractBuilder); override; final;
    function Designator: Integer; override;
  end;
  ...
  strict private
    ...
    FColorIndex: Integer;
  protected
    procedure BuildCodeCore(const ABuilder: TdxNumberFormatCodeAbstractBuilder); override;
  public
    constructor Create(AColor: TColor; AIndex: Integer = -1);
    procedure Format(const AValue: Variant; AValueType: TdxSpreadSheetCellDataType;
      const AFormatSettings: TdxSpreadSheetCustomFormatSettings; AResult: TdxNumberFormatResult); override;
    //
  ...
  TdxNumberFormatElementAsterisk = class(TdxNumberFormatElementTextBase)
  public
    function Designator: Integer; override;
  end;
...
//----------------------------------------------------------------------------------------------------------------------
  ...
  { TdxDateTimeSystemFullDateTimeNumberFormat }

  TdxDateTimeSystemFullDateTimeNumberFormat = class(TdxDateTimeSystemDateTimeNumberFormat)
  protected
    function GetFormatString(const AData: TFormatSettings): string; override;
  end;
  ...
  protected
    ...
    FDisplayFactorPower: Double;
    FGrouping: Boolean;
    FIntegerCount: Integer;
    FIsNegativePart: Boolean;
    FPercentCount: Integer;

    procedure BuildCodeCore(const ABuilder: TdxNumberFormatCodeAbstractBuilder; AEndIndex: Integer);
  ...
  public
    ...
    procedure Format(const AValue: Variant; AValueType: TdxSpreadSheetCellDataType;
      const AFormatSettings: TdxSpreadSheetCustomFormatSettings; AResult: TdxNumberFormatResult); override;
    //
    property FractionSeparatorIndex: Integer read FDecimalCount;
  end;
...
uses
  dxSpreadSheetNumberFormatParser, dxSpreadSheetNumberFormat, Character;

const
  dxThisUnitName = 'dxSpreadSheetNumberFormatCore';

{ TdxNumberFormatCodeAbstractBuilder }

procedure TdxNumberFormatCodeAbstractBuilder.Append(const V: Integer);
begin
  Append(IntToStr(V));
end;
...
procedure TdxNumberFormatElement.BuildCode(const ABuilder: TdxNumberFormatCodeAbstractBuilder);
begin
  ABuilder.AppendDesignator(Designator);
end;

{ TdxNumberFormatElementList }

procedure TdxNumberFormatElementList.BuildCode(const ABuilder: TdxNumberFormatCodeAbstractBuilder);
var
  I: Integer;
begin
  for I := 0 to Count - 1 do
    Items[I].BuildCode(ABuilder);
end;
...
procedure TdxSimpleNumberFormat.BuildCode(const ABuilder: TdxNumberFormatCodeAbstractBuilder);
begin
  FList.BuildCode(ABuilder);
end;
...
procedure TdxSimpleNumberFormatList.BuildCode(const ABuilder: TdxNumberFormatCodeAbstractBuilder);
var
  I: Integer;
begin
  for I := 0 to Count - 1 do
  begin
    if I > 0 then
      ABuilder.AppendDesignator(TdxNumberFormatDesignator.EndOfPart);
    Items[I].BuildCode(ABuilder);
  end;
end;
...
begin
  Result := TdxNumberFormatDesignator.General;
end;
...
procedure TdxNumberFormatElementCondition.BuildCode(const ABuilder: TdxNumberFormatCodeAbstractBuilder);
begin
  ABuilder.Append('[');
  BuildCodeCore(ABuilder);
  ABuilder.Append(']');
end;
...
constructor TdxNumberFormatElementColor.Create(AColor: TColor; AIndex: Integer = -1);
begin
  inherited Create;
  FColor := AColor;
  FColorIndex := AIndex;
...
procedure TdxNumberFormatElementColor.BuildCodeCore(const ABuilder: TdxNumberFormatCodeAbstractBuilder);
var
  AName: string;
begin
  ...
  begin
    ...
    ABuilder.Append(FColorIndex + 1);
  end
  else
    if cxNameByColor(Color, AName) and (AName <> '') then
      ABuilder.Append(dxUpperCase(AName[1]) + Copy(AName, 2, MaxInt))
    else
      raise EInvalidArgument.CreateFmt('Unsupported color (%d)', [Color]);
end;
...
procedure TdxNumberFormatElementTextBase.BuildCode(const ABuilder: TdxNumberFormatCodeAbstractBuilder);
begin
  inherited;
  ABuilder.Append(FText);
end;
...
procedure TdxNumberFormatElementBackslashedText.BuildCode(const ABuilder: TdxNumberFormatCodeAbstractBuilder);
var
  ATextLength: Integer;
begin
  ATextLength := Length(Text);
  if (ATextLength = 0) or (ATextLength = 1) and ABuilder.MustBeEscaped(Text[1]) then
    ABuilder.AppendDesignator(TdxNumberFormatDesignator.Backslash);
...
procedure TdxNumberFormatElementDisplayLocale.BuildCodeCore(const ABuilder: TdxNumberFormatCodeAbstractBuilder);
begin
  ABuilder.Append('$');
  ABuilder.Append(FCurrency);
  if LocaleId > 0 then
  begin
    ABuilder.Append('-');
    ABuilder.Append(IntToHex(LocaleId, 1));
  end;
end;
...
procedure TdxNumberFormatElementDateBase.BuildCode(const ABuilder: TdxNumberFormatCodeAbstractBuilder);
var
  I: Integer;
begin
  for I := 0 to Count - 1 do
...
procedure TdxNumberFormatElementTimeBase.BuildCode(const ABuilder: TdxNumberFormatCodeAbstractBuilder);
begin
  if FElapsed then
  begin
    ABuilder.Append('[');
...
procedure TdxNumberFormatElementAmPm.BuildCode(const ABuilder: TdxNumberFormatCodeAbstractBuilder);
begin
  ABuilder.Append(GetSuffix(True));
  ABuilder.Append('/'); // do not localize
  ABuilder.Append(GetSuffix(False));
end;
...
procedure TdxNumberFormatElementMilliseconds.BuildCode(const ABuilder: TdxNumberFormatCodeAbstractBuilder);
begin
  ABuilder.AppendDesignator(TdxNumberFormatDesignator.DecimalSeparator);
  inherited;
end;
...
procedure TdxNumberFormatElementExprCondition.BuildCodeCore(const ABuilder: TdxNumberFormatCodeAbstractBuilder);
begin
  ABuilder.Append(Expression);
end;
...
procedure TdxNumberFormatElementUnderline.BuildCode(const ABuilder: TdxNumberFormatCodeAbstractBuilder);
begin
  ABuilder.AppendDesignator(Designator);
  ABuilder.Append(Text);
end;
...
procedure TdxNumberFormatElementUnderline.Format(const AValue: Variant; AValueType: TdxSpreadSheetCellDataType;
  const AFormatSettings: TdxSpreadSheetCustomFormatSettings; AResult: TdxNumberFormatResult);
begin
  AResult.Append(' ');
  AResult.IsText := bTrue;
...
procedure TdxNumberFormatElementQuotedText.BuildCode(const ABuilder: TdxNumberFormatCodeAbstractBuilder);
begin
  inherited;
  ABuilder.AppendDesignator(Designator);
end;
...
procedure TdxNumberFormatElementInvariantYear.Format(const AValue: Variant; AValueType: TdxSpreadSheetCellDataType;
  const AFormatSettings: TdxSpreadSheetCustomFormatSettings; AResult: TdxNumberFormatResult);
begin
  FormatDateTime('YYYY', AValue, AFormatSettings, AResult);
end;
...
begin
  if not dxIsNumericType(AValueType) then
    ...
    begin
      ...
      if dxIsValidDateTime(AValue) then
      begin
...
procedure TdxDateTimeSystemDateTimeNumberFormat.BuildCode(const ABuilder: TdxNumberFormatCodeAbstractBuilder);
begin
  if List.Count = 0 then
  begin
    FormatTemplate := GetFormatString(ABuilder.FormatSettings);
    Formatter.BuildCode(ABuilder);
  end
  else
    inherited;
end;
...
function TdxDateTimeSystemFullDateTimeNumberFormat.GetFormatString(const AData: TFormatSettings): string;
begin
  Result := AData.ShortDateFormat + ' ' + AData.ShortTimeFormat;
end;
...
procedure TdxConditionalNumberFormat.BuildCode(const ABuilder: TdxNumberFormatCodeAbstractBuilder);
begin
  Parts.BuildCode(ABuilder);
end;
...
begin
  ...
  FDisplayFactor := ADisplayFactor;
  FDisplayFactorPower := Power(10, 3 * ADisplayFactor);
  FGrouping := AGrouping;
...
procedure TdxSimpleNumericNumberFormat.BuildCode(const ABuilder: TdxNumberFormatCodeAbstractBuilder);
begin
  BuildCodeCore(ABuilder, List.Count);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if FDisplayFactorPower > 1 then
        AExtendedValue := AExtendedValue / FDisplayFactorPower;
...
procedure TdxSimpleNumericNumberFormat.BuildCodeCore(const ABuilder: TdxNumberFormatCodeAbstractBuilder; AEndIndex: Integer);
var
  ADigitCount: Integer;
...
begin
  AIndex := 0;
  ADigitCount := FIntegerCount;
  if FGrouping then
  begin
    while ADigitCount > 0 do
    begin
      AElement := List[AIndex];
      AElement.BuildCode(ABuilder);
      if AElement.IsDigit then
      begin
        Dec(ADigitCount);
        if (ADigitCount mod 3 = 0) and (ADigitCount <> 0) then
          ABuilder.AppendDesignator(TdxNumberFormatDesignator.GroupSeparator);
      end;
      Inc(AIndex);
    end;
  end;

  if FDisplayFactor > 0 then
  begin
    Inc(ADigitCount, FDecimalCount);
    while ADigitCount > 0 do
    begin
      AElement := List[AIndex];
      AElement.BuildCode(ABuilder);
    ...
    for I := 0 to FDisplayFactor - 1 do
      ABuilder.AppendDesignator(TdxNumberFormatDesignator.GroupSeparator);
  end;

  while AIndex < AEndIndex do
  begin
    List[AIndex].BuildCode(ABuilder);
    Inc(AIndex);
  end;
end;
...
procedure TdxNumericNumberFormatExponent.BuildCode(const ABuilder: TdxNumberFormatCodeAbstractBuilder);
var
  I: Integer;
begin
  BuildCodeCore(ABuilder, FExpIndex);
  ABuilder.Append('E');
  ABuilder.Append(IfThen(FExplicitSign, '+', '-'));
  for I := FExpIndex + 1 to List.Count - 1 do
    List[I].BuildCode(ABuilder);
end;
...
procedure TdxNumericFractionNumberFormat.BuildCode(const ABuilder: TdxNumberFormatCodeAbstractBuilder);
var
  I: Integer;
begin
  BuildCodeCore(ABuilder, FractionSeparatorIndex);
  ABuilder.Append('/'); // do not localize
  for I := FractionSeparatorIndex to List.Count - 1 do
    List[I].BuildCode(ABuilder);
end;
...
procedure TdxExplicitNumericFractionNumberFormat.BuildCode(const ABuilder: TdxNumberFormatCodeAbstractBuilder);
var
  I: Integer;
begin
  BuildCodeCore(ABuilder, FractionSeparatorIndex);
  ABuilder.Append('/'); // do not localize
  for I := FractionSeparatorIndex + 1 to FDivisorIndex do
    List[I].BuildCode(ABuilder);
  ABuilder.Append(ExplicitDivisor);
  for I := FDivisorIndex + 1 to List.Count - 1 do
    List[I].BuildCode(ABuilder);
end;
