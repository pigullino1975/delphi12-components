...
{                                                                    }
{           Copyright (c) 2012-2024 Developer Express Inc.           }
{           ALL RIGHTS RESERVED                                      }
{                                                                    }
...
type                                 
  ...
  public
    ...
    property Controller: TdxWizardControlController read GetController; // for internal use
    property Painter: TdxWizardControlCustomPainter read GetPainter; // for internal use
  ...
  private
    function GetItem(Index: TdxListIndex): TdxWizardControlCellViewInfo;
  public
    ...
    property Items[Index: TdxListIndex]: TdxWizardControlCellViewInfo read GetItem; default; // for internal use
  end;
  ...
  public
    ...
    //
    property CellList: TdxWizardControlCellViewInfoList read FCellList; // for internal use
  end;
  ...
  public
    ...
    property Painter: TdxWizardControlCustomPainter read FPainter; // for internal use
    //
    property CommandAreaViewInfo: TdxWizardControlCustomCommandAreaViewInfo read FCommandAreaViewInfo; // for internal use
    property HeaderViewInfo: TdxWizardControlCellViewInfo read FHeaderViewInfo; // for internal use
    property TitleViewInfo: TdxWizardControlCustomTitleViewInfo read FTitleViewInfo; // for internal use
    property WatermarkViewInfo: TdxWizardControlWatermarkViewInfo read FWatermarkViewInfo; // for internal use
    //
  ...
  public
    ...
    property FocusedCell: TdxWizardControlCellViewInfo read FFocusedCell write SetFocusedCell; // for internal use
    property HitTest: TdxWizardControlHitTest read FHitTest;
    property HotCell: TdxWizardControlCellViewInfo read FHotCell write SetHotCell; // for internal use
    property PressedCell: TdxWizardControlCellViewInfo read FPressedCell write SetPressedCell; // for internal use
    property ViewInfo: TdxWizardControlViewInfo read GetViewInfo; // for internal use
  end;
...
const
  dxThisUnitName = 'dxCustomWizardControl';
...
procedure ResetFont(AOwner: TdxWizardControlCustomPersistent; AFont: TFont);
begin
  cxResetFont(AFont, dxGetScaleFactor(AOwner));
...
function TdxWizardControlCellViewInfoList.GetItem(Index: TdxListIndex): TdxWizardControlCellViewInfo;
begin
...
begin
  ...
  begin
    FTitleRect := cxRectSetHeight(TitleRect, TdxTextMeasurer.TextLineHeight(TitleFont));
...
begin
  ...
  try
    ...
    begin
      ...
      cxDrawText(ACanvas, Header.Description, DescriptionRect, AFormat or DT_WORDBREAK);
    end;
  finally
...
begin
  if Header <> nil then
  begin
    Result := TdxTextMeasurer.TextLineHeight(TitleFont);
    if DescriptionIsVisible then
      Inc(Result, 2 * TdxTextMeasurer.TextLineHeight(DescriptionFont) + ScaleFactor.Apply(cxTextSpace));
    if GlyphIsVisible then
...
begin
  ...
  begin
    if not Button.Glyph.Empty then
      ...
      AGlyphBitmap := nil;
    try
      TdxImageDrawer.DrawUncachedImage(ACanvas.Handle, cxRectOffset(GlyphRect, AOffset, not WizardControl.UseRightToLeftAlignment),
  ...
  try
    ...
    if (IsFocused) and Painter.LookAndFeelPainter.SupportsNativeFocusRect then
...
function TdxWizardControlButtonViewInfo.MeasureHeight: Integer;
  ...
  begin
    if Button.GlyphAlignment in [blGlyphLeft, blGlyphRight] then
      Result := Max(GlyphSize.cy, TdxTextMeasurer.TextLineHeight(Font)) + cxMarginsHeight(Margins)
    else
    begin
      Result := TdxTextMeasurer.TextLineHeight(Font) + cxMarginsHeight(Margins);
...
begin
  Result := TdxTextMeasurer.TextLineHeight(Font) + ScaleFactor.Apply(cxTextOffset) * 2;
end;
...
begin
  if IsHyperlinkMode then
    ...
    if InfoPanel.Enabled then
      Result := cxGetActualColor(InfoPanel.Font.Color, Painter.CommandAreaTextColor)
    else
  ...
  begin
    ...
    try
      ...
      if CheckResetValue(wchvTitleFont, AValue) then
        ResetFont(Self, TitleFont);
      if CheckResetValue(wchvDescriptionFont, AValue) then
        ResetFont(Self, DescriptionFont);
...
begin
  if Animation <> nil then
    Animation.Draw(ACanvas, AnimatedRect)
end;
  ...
  if (Self = Message.Sender) and Controller.ProcessChildKey(Message.CharCode, KeyboardStateToShiftState) then
    ...
    if (Message.Sender = Self) or (Message.Sender is TdxWizardControlCustomPage) or
      ...
      case Message.CharCode of
        ...
        VK_ESCAPE:
          Message.Result := Ord(CanProcessSpecialKey and Buttons.Cancel.Enabled and Buttons.Cancel.Visible
            and DoButtonClick(wcbkCancel));
      end;
