...
uses
  ...
  Generics.Collections, Generics.Defaults, cxLookAndFeels, dxForms;
...
type
  ...
  ['{79EB28B8-227E-4C23-BC59-7CF4311789FC}']
    ...
    function UseNavigationControlCompatibleBorders: Boolean;
  end;
  ...
  protected
    ...
    FWindowCornersPainter: TdxFormCornersPainter;
    ...
    procedure DrawArrow(DC: HDC; const R: TRect; AArrowDirection: TcxArrowDirection; AColor: TColor); virtual;
    ...
    function GetFormCornerColor(AActive: Boolean; ACorner: TdxCorner): TdxAlphaColor; virtual;
    ...
    property FormPaintData: IdxRibbonFormPaintData read FFormPaintData;
  ...
  public
    ...
    function AreBordersThin: Boolean; virtual;
    ...
    procedure DrawFormBorders(DC: HDC; const ABounds, ABordersWidth: TRect; AIsActive, AIsRectangular: Boolean); virtual;
    procedure DrawFormCaption(DC: HDC; const R: TRect); virtual;
    procedure DrawFormCorners(DC: HDC; const R: TRect; ACorners: TdxCorners); virtual;
    ...
    procedure DrawMenuMark(DC: HDC; const R: TRect; AState: Integer); virtual;
    ...
    procedure DrawQATButton(DC: HDC; const R: TRect; AState: Integer; APlace: TdxQATPosition); virtual;
    ...
    function GetTabPanelHorizontalMargins: TRect; virtual;
    ...
    // QAT
    procedure AdjustQuickAccessToolbarVertical(var ABounds: TRect; APlace: TdxQATPosition; ADontUseAero: Boolean); virtual;
    procedure DrawQuickAccessToolbar(DC: HDC; const R: TRect;
      APlace: TdxQATPosition; AHasApplicationButton, AIsActive, ADontUseAero, AIsRibbonMinimized: Boolean); virtual;
    procedure DrawQuickAccessToolbarArrowDown(DC: HDC; const R: TRect; AState: Integer; APlace: TdxQATPosition); virtual;
    procedure DrawQuickAccessToolbarDefaultGlyph(DC: HDC; const R: TRect; APlace: TdxQATPosition); virtual;
    procedure DrawQuickAccessToolbarGroupButton(DC: HDC; const R: TRect; APlace: TdxQATPosition; AIsActive: Boolean; AState: Integer); virtual;
    procedure DrawQuickAccessToolbarMarkArrow(DC: HDC; const R: TRect; AState: Integer; APlace: TdxQATPosition); virtual;
    procedure DrawQuickAccessToolbarMarkTruncated(DC: HDC; const R: TRect; AState: Integer; APlace: TdxQATPosition); virtual;
    procedure DrawQuickAccessToolbarPopup(DC: HDC; const R: TRect); virtual;
    procedure DrawQuickAccessToolbarSmallButton(DC: HDC; const R: TRect; AState: Integer; APlace: TdxQATPosition); virtual;
    procedure DrawQuickAccessToolbarSmallButtonDropButtonArrowPart(DC: HDC; const R: TRect; AState: Integer; APlace: TdxQATPosition); virtual;
    procedure DrawQuickAccessToolbarSmallButtonDropButtonMainPart(DC: HDC; const R: TRect; AState: Integer; APlace: TdxQATPosition); virtual;
    function GetQuickAccessToolbarColorPalette(AState: Integer; APlace: TdxQATPosition): IdxColorPalette; virtual;
    ...
    // StatusBar
    procedure DrawStatusBar(DC: HDC; const R: TRect; AIsLeftSideRaised, AIsRightSideRaised: Boolean); virtual;
    procedure DrawStatusBarGripBackground(DC: HDC; const AStatusBarBounds, R: TRect); virtual;
    procedure DrawStatusBarPanel(DC: HDC; const AStatusBarBounds, R: TRect; AIsLowered: Boolean); virtual;
    ...
    procedure DrawGalleryFilterBandBackground(DC: HDC; const R: TRect; AState: Integer); virtual;
    ...
    property UseExternalBorders: Boolean read FUseExternalBorders write FUseExternalBorders;
...
function IsRectangularFormBottom(const AFormData: IdxRibbonFormPaintData): Boolean; inline;

implementation

uses
  Types, dxOffice11, Math, cxDWMApi, cxControls, dxTypeHelpers;

const
  dxThisUnitName = 'dxRibbonSkins';
...
function IsRectangularFormBottom(const AFormData: IdxRibbonFormPaintData): Boolean;
begin
  Result :=
    (AFormData <> nil) and (AFormData.GetWindowCorners <= fcRectangular) and (
      (AFormData.GetFormBorderStyle in [bsDialog, bsSingle, bsToolWindow]) or
      (AFormData.GetStyle = fsMDIChild) or (AFormData.GetState = wsMinimized) or
      (AFormData.GetWindowCornersRadius = 0)
    );
end;
...
begin
  ...
  FWindowCornersPainter := TdxFormCornersPainter.Create(GetFormCornerColor);
  UpdateBitsPerPixel;
end;

destructor TdxCustomRibbonSkin.Destroy;
begin
  FreeAndNil(FWindowCornersPainter);
...
begin
  ...
  begin
    ...
    FFormPaintData := PaintData.GetRibbonFormPaintData;
  end
  else
  begin
...
begin
  ...
  begin
    AOffsetSize.cx := 0;
    AOffsetSize.cy := R.Height;
  end
  else
  begin
    AOffsetSize.cx := R.Width;
    AOffsetSize.cy := 0;
  end;
...
begin
  Result := ScaleFactor.Apply(TSize.Create(42));
end;
...
function TdxCustomRibbonSkin.AreBordersThin: Boolean;
begin
  Result := False;
end;
...
begin
  DrawFrameControl(DC, TRect.Create(R.Left + 1, R.Top + 2, R.Right - 1, R.Bottom),
    DFC_CAPTION, Buttons[AIcon] or Pushes[AState = rbisPressed]);
end;

procedure TdxCustomRibbonSkin.DrawFormBorders(DC: HDC;
  const ABounds, ABordersWidth: TRect; AIsActive, AIsRectangular: Boolean);
var
  ASaveIndex: Integer;
begin
  ...
  try
    ...
    InternalDrawFormBorders(DC, ABounds, AIsActive, AIsRectangular);
  finally
...
begin
  ...
  try
    AData := FormPaintData;
    if AData.GetState <> wsMaximized then
    begin
      InternalDrawFormBorders(DC, ARect, AData.GetIsActive, IsRectangularFormBottom(AData));
      ATopBorderWidth := GetWindowBordersWidth(PaintData.IsStatusBarVisible).Left;
      ARect := cxRectInflate(ARect, 0, -ATopBorderWidth, 0, 0);
    end;
    FillRectByColor(DC, ARect, ColorMap[AData.GetIsActive]);
  finally
    RestoreDC(DC, ASaveIndex);
  end;
end;

procedure TdxCustomRibbonSkin.DrawFormCorners(DC: HDC; const R: TRect; ACorners: TdxCorners);
var
  AData: IdxRibbonFormPaintData;
begin
  ...
  begin
    AData := FormPaintData;
    if (AData <> nil) and (AData.GetWindowCorners >= fcRounded) and not IsZoomed(AData.GetHandle) then
      FWindowCornersPainter.Draw(DC, R, ACorners, AData.GetIsActive, AData.GetWindowCornersRadius);
  end;
end;
...
begin
  ...
  try
    ...
    AArrowRect := cxRectOffset(cxRectCenter(R, TSize.Create(AArrowSize)), IfThen(not UseRightToLeftAlignment, 1, 2), 1);
    if Odd(AArrowSize) <> Odd(R.Width) then
...
const
  ...
  Pushes: array[Boolean] of Integer = (0, DFCS_PUSHED);
begin
  DrawFrameControl(DC, TRect.Create(R.Left + 1, R.Top + 3, R.Right - 1, R.Bottom - 3),
...
begin
  if not cxUseSkins then
    DrawArrow(DC, R, adRight, GetPartColor(DXBAR_ARROWDOWN))
  else
    DrawArrow(DC, R, adRight, GetPartColor(DXBAR_MENUARROWRIGHT));
end;
...
procedure TdxCustomRibbonSkin.DrawMenuMark(DC: HDC; const R: TRect; AState: Integer);
begin
  if LowColors then
...
procedure TdxCustomRibbonSkin.DrawQATButton(DC: HDC; const R: TRect; AState: Integer; APlace: TdxQATPosition);
begin
  DrawSmallButton(DC, R, AState);
end;
...
begin
  H := R.Height div 7;
  DrawPart(DC, TRect.Create(R.Left + 3, R.Top + H * 3, R.Right - 3, R.Bottom - H * 2), AState);
end;
...
begin
  ...
  begin
    H := R.Height div 7;
    DrawPart(DC, TRect.Create(R.Left + H + 1, R.Top + H * 3, R.Right - H + 1, R.Bottom - H * 2), AState);
  end
  else
...
var
  R1: TRect;
begin
  R1.Init(R.Left + 4, R.Top, R.Right - 4, R.Top + 1);
...
function TdxCustomRibbonSkin.GetTabPanelHorizontalMargins: TRect;
begin
  Result.Empty;
end;
...
procedure TdxCustomRibbonSkin.AdjustQuickAccessToolbarVertical(
  var ABounds: TRect; APlace: TdxQATPosition; ADontUseAero: Boolean);
begin
  if APlace = qatpInCaption then
  begin
...
procedure TdxCustomRibbonSkin.DrawQuickAccessToolbar(DC: HDC; const R: TRect;
  ...
  var
    AInactive: Boolean;
  begin
    if (APlace = qatpInCaption) and IsCompositionEnabled and not ADontUseAero then
    begin
    ...
    begin
      AInactive := (APlace = qatpInCaption) and not AIsActive;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        APath.AddLine(ALeft, ARect.Bottom, ALeft, ARect.Bottom - (ARect.Height div 2));
        APath.AddArc(ALeft - 19, ARect.Top, 20, ARect.Height * 2, -45, -43);
      ...
      begin
        ...
        APath.AddArc(ARect.Right - ALeft * 2, ARect.Top, ALeft * 2, ARect.Height, -90, 180);
...
begin
  ...
  begin
    AWidth := R.Height div 2;
    if R.Width >= 2 * AWidth then
    begin
      GetQuickAccessToolBarParts(ALeftPart, ARightPart);
      R1 := cxRectSetWidth(R, Parts[ALeftPart].Size.cx);
      AdjustQuickAccessToolbarVertical(R1, APlace, ADontUseAero);
      ALeftPartRect := R1;
      R1.Left := R1.Right;
...
procedure TdxCustomRibbonSkin.DrawQuickAccessToolbarArrowDown(DC: HDC; const R: TRect; AState: Integer;
  APlace: TdxQATPosition);
begin
  DrawArrowDown(DC, R, AState);
end;

procedure TdxCustomRibbonSkin.DrawQuickAccessToolbarDefaultGlyph(DC: HDC; const R: TRect; APlace: TdxQATPosition);
begin
  FillRectByColor(DC, R, clBtnFace)
end;

procedure TdxCustomRibbonSkin.DrawQuickAccessToolbarGroupButton(DC: HDC; const R: TRect;
  APlace: TdxQATPosition; AIsActive: Boolean; AState: Integer);
begin
  if (APlace <> qatpAbove) and not AIsActive then
...
procedure TdxCustomRibbonSkin.DrawQuickAccessToolbarMarkArrow(
  DC: HDC; const R: TRect; AState: Integer; APlace: TdxQATPosition);
begin
...
function TdxCustomRibbonSkin.GetQuickAccessToolbarColorPalette(AState: Integer; APlace: TdxQATPosition): IdxColorPalette;
begin
  if APlace = qatpBelow then
...
procedure TdxCustomRibbonSkin.DrawQuickAccessToolbarSmallButton(DC: HDC; const R: TRect; AState: Integer;
  APlace: TdxQATPosition);
begin
  DrawQATButton(DC, R, AState, APlace);
end;
...
procedure TdxCustomRibbonSkin.DrawStatusBar(DC: HDC; const R: TRect; AIsLeftSideRaised, AIsRightSideRaised: Boolean);
begin
  FillRectByColor(DC, R, clBtnFace);
end;

procedure TdxCustomRibbonSkin.DrawStatusBarGripBackground(DC: HDC; const AStatusBarBounds, R: TRect);
begin
  FillRectByColor(DC, R, clBtnFace);
end;

procedure TdxCustomRibbonSkin.DrawStatusBarPanel(DC: HDC; const AStatusBarBounds, R: TRect; AIsLowered: Boolean);
begin
  FillRectByColor(DC, R, clBtnFace);
end;
...
begin
  FillRectByColor(DC, R, clBtnFace);
  FillRectByColor(DC, TRect.Create(R.Left, R.Top + 1, R.Left + 1, R.Bottom - 1), clBtnShadow);
end;
...
begin
  FillRectByColor(DC, R, clBtnFace);
  FillRectByColor(DC, TRect.Create(R.Left, R.Top, R.Left + 1, R.Bottom - 1), clBtnShadow);
end;
...
procedure TdxCustomRibbonSkin.DrawGalleryFilterBandBackground(DC: HDC; const R: TRect; AState: Integer);
begin
  FillRectByColor(DC, R, clWindow);
end;
...
begin
  case APart of
    ...
    DXRIBBON_QAT_GROUPBUTTON, DXBAR_SMALLBUTTON:
      Result := ScaleFactor.Apply(TRect.Create(3));
    DXBAR_LARGEBUTTON:
      Result := ScaleFactor.Apply(TRect.Create(5, 3, 5, 3));
    DXBAR_APPLICATIONBUTTON:
      Result := ScaleFactor.Apply(TRect.Create(3, 5, 4, 0));
    DXBAR_APPLICATIONMENUCONTENT:
      Result := ScaleFactor.Apply(TRect.Create(4, 16, 4, 29));
    DXBAR_APPLICATIONBUTTONICONOFFSET:
      Result := ScaleFactor.Apply(TRect.Create(8, 8, 10, 10));
    DXBAR_TOOLBARINPOPUP, DXBAR_TOOLBAR:
      Result := TRect.Create(2, 2, 2, 1);
    DXBAR_RIBBONTABGROUP, DXBAR_RIBBONCONTEXTTABGROUP:
      Result := TRect.Create(3, 3, 3, 4);
    DXBAR_QUICKACCESSTOOLBAR:
      Result := TRect.Create(2);
    DXBAR_BACKSTAGEVIEW_MENUBAR_ITEM:
      Result := ScaleFactor.Apply(TRect.Create(10, 0, 0, 0));
    DXBAR_BACKSTAGEVIEW_MENUBAR_TAB:
      Result := ScaleFactor.Apply(TRect.Create(21, 12, 11, 12));
    DXBAR_BACKSTAGEVIEW_MENUBAR:
      Result := ScaleFactor.Apply(TRect.Create(6, 5, 6, 6));
  else
    Result := cxNullRect;
  end;
end;
...
begin
  FWindowCornersPainter.FlushCache;
end;
...
begin
  FQATGlassAtTopLeft[True] := LoadScaledElement(cxRectBounds(0, 353, 16, 26), TRect.Create(0, 2, 2, 2), rspQATNonClientLeft1Vista);
  FQATGlassAtTopLeft[False] := LoadScaledElement(cxRectBounds(34, 353, 4, 26), TRect.Create(3, 2, 0, 1), rspQATNonClientLeft2Vista);
  FQATGlassAtTopRight := LoadScaledElement(cxRectBounds(16, 353, 18, 26), TRect.Create(0, 7, 15, 7), rspQATNonClientRightVista);
...
begin
  FQATAtBottom := AddPart3x3(ABitmap, cxRectBounds(13, 209, 10, 26), TRect.Create(3), rspQATAtBottom);
  LoadScaledElement(FQATAtTopLeft[True], cxRectBounds(93, 113, 15, 26), TRect.Create(13, 5, 0, 5), rspQATNonClientLeft1Active);
  LoadScaledElement(FQATAtTopLeft[False], cxRectBounds(125, 113, 7, 26), TRect.Create(2, 5, 0, 5), rspQATNonClientLeft2Active);
  LoadScaledElement(FQATAtTopRight, cxRectBounds(106, 113, 18, 26), TRect.Create(0, 5, 13, 5), rspQATNonClientRightActive);
end;
...
begin
  ...
  ARect.Right := DropDownGalleryVerticalSizeGripBitmapSize.cx *
    ARect.Height div DropDownGalleryVerticalSizeGripBitmapSize.cy;
  OffsetRect(ARect, (R.Width - ARect.Width) div 2, 0);
  if LowColors then
...
function TdxCustomRibbonSkin.GetFormCornerColor(AActive: Boolean; ACorner: TdxCorner): TdxAlphaColor;
begin
...
begin
  dxFreeGlobalObject(FSkinsManager);
end;


initialization

  dxUnitsLoader.AddUnit(SysInit.HInstance, dxThisUnitName, nil, DestroySkins);

finalization

  dxUnitsLoader.RemoveUnit(SysInit.HInstance, dxThisUnitName, DestroySkins);
