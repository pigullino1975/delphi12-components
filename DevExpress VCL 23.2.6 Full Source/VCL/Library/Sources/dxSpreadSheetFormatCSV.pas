...
type
  ...
  public
    ...
    procedure Reset;
    //
    property AlignColumns: Boolean read FAlignColumns write FAlignColumns;
...
const
  dxThisUnitName = 'dxSpreadSheetFormatCSV';
...
begin
  ...
  begin
    AlignColumns := TdxSpreadSheetCSVFormatExportSettings(Source).AlignColumns;
...
begin
  ...
  begin
    ...
    AEncoding := nil;
    ASize := TEncoding.GetBufferEncoding(ABuffer, AEncoding, dxSpreadSheetCSVFormatSettings.Encoding);
...
begin
  ...
  Inc(FRowIndex);
  FColumnIndex := -1;
  Include(TdxCustomSpreadSheetAccess(View.SpreadSheet).FState, sssReadingCells);
end;

procedure TdxSpreadSheetCSVReaderCustomParser.DoRowEnd;
begin
  inherited DoRowEnd;
  Exclude(TdxCustomSpreadSheetAccess(View.SpreadSheet).FState, sssReadingCells);
  FRow := nil;
end;
...
begin
  ...
  FSettings := TdxSpreadSheetCSVFormatSettings.Create;
  FSettings.Assign(dxSpreadSheetCSVFormatSettings);
  FAlignColumns := FSettings.Export.AlignColumns;
end;
...
function TdxSpreadSheetCSVWriter.EncodeValue(ACell: TdxSpreadSheetCell): string;
  ...
  begin
    if Settings.Export.UseDisplayValues then
      Result := ACell.DisplayValueDataType = cdtString
    else
