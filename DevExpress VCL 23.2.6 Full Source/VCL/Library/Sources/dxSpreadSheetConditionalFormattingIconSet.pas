...
{$R dxSpreadSheetConditionalFormattingIconSet_svg.res}

interface

uses
  Types, Classes, Generics.Defaults, Generics.Collections, cxGraphics, Graphics, dxGenerics, cxLookAndFeels;
...
uses
  SysUtils, dxGDIPlusClasses, cxGeometry, Math, dxCore;

const
  dxThisUnitName = 'dxSpreadSheetConditionalFormattingIconSet';
...
begin
  inherited Create;
  FName := AName;
  FIconIndexes := TdxIntegerList.Create;
end;
...
begin
  ...
  try
    ...
    begin
      AResourceStream := TResourceStream.Create(HInstance, 'DXSPREADSHEET_CONDITIONAL_FORMATTING_ICONSET_SVG', RT_RCDATA);
      try
        FIcons.AddImagesFromZipStream(AResourceStream);
      finally
