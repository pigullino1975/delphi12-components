...
uses
  System.UITypes,
  System.ZLib,
  Windows, Types, Classes, SysUtils, Dialogs, Math, Graphics, Generics.Defaults, Generics.Collections,
  dxCore, dxCoreGraphics, cxClasses, cxVariants, dxGDIPlusClasses, dxHash, dxHashUtils, cxGeometry,
  dxSpreadSheetFormatXLS, dxSpreadSheetFormatXLSTypes, dxSpreadSheetCore, dxSpreadSheetFormulas, dxSpreadSheetTypes,
  dxSpreadSheetFunctions, dxSpreadSheetFormatXLSFormulas, dxSpreadSheetFormatUtils, dxSpreadSheetUtils,
  dxSpreadSheetClasses, dxSpreadSheetContainers, dxSpreadSheetHyperlinks, dxSpreadSheetFormatXLSWriter,
  dxSpreadSheetFormatXLSReader;
...
type
  ...
  TdxSpreadSheetMSODrawingReader = class(TdxSpreadSheetXLSCustomMSODrawingReader)
  strict private
    ...
    function ComplexDataToString(const AComplexData: array of Byte): string;
  ...
  protected
    procedure ApplyPropertyValues(AContainer: TdxSpreadSheetContainer);
  ...
  public
    constructor Create(AOwner: TdxSpreadSheetXLSReader); override;
    destructor Destroy; override;
    function CreateContainer(AContainerType: Integer = -1): TdxSpreadSheetContainer; override;
  ...
  protected
    ...
    procedure WriteProperty_DrawingFillShadeColors(const AValue: TdxXLSColorRefArray; ADataWriter: TcxWriter);
    ...
    procedure WriteProperty_Hyperlink(AHyperlink: TdxSpreadSheetHyperlink; ADataWriter: TcxWriter);
    procedure WriteProperty_Text(ID: Word; const AName: UnicodeString; ADataWriter: TcxWriter);
...
uses
  dxSpreadSheetFormatXLSXReaderDrawing, dxSmartImage, dxSpreadSheetCoreStyles, dxGenerics;

const
  dxThisUnitName = 'dxSpreadSheetFormatXLSDrawing';
...
function TdxSpreadSheetMSODrawingReader.CreateContainer(AContainerType: Integer = -1): TdxSpreadSheetContainer;
var
  AContainerClass: TdxSpreadSheetContainerClass;
begin
  if not HasData then
    raise EInvalidOperation.Create('MSODrawing is not ready');

  if AContainerType < 0 then
  begin
    if GetGraphic(FBlipID) <> nil then
      AContainerType := oPicture
    else if FShapeType = 202 then
      AContainerType := oText
    else
      AContainerType := oRectangle;
  end;

  case AContainerType of
    oPicture:
      AContainerClass := TdxSpreadSheetPictureContainer;
    oComment:
      AContainerClass := TdxSpreadSheetCommentContainer;
    oText:
  ...
  Owner.CurrentSheet.Containers.Add(AContainerClass, Result);
  ApplyPropertyValues(Result);
end;

function TdxSpreadSheetMSODrawingReader.HasData: Boolean;
begin
  Result := FHasData;
end;
...
begin
  ...
  begin
    ...
    if Assigned(ARecordReader) and ARecordReader then
      FHasData := True
    else
...
procedure TdxSpreadSheetMSODrawingReader.ApplyPropertyValues(AContainer: TdxSpreadSheetContainer);
...
var
  ...
  AShape: TdxSpreadSheetShape;
  I: Integer;
begin
  AContainer.Visible := not FHidden;
  ...
  if AContainer is TdxSpreadSheetPictureContainer then
    TdxSpreadSheetPictureContainer(AContainer).Picture.Image := GetGraphic(FBlipID);

  if AContainer is TdxSpreadSheetTextBoxContainer then
  begin
    TdxSpreadSheetTextBoxContainer(AContainer).TextBox.ContentOffsets := FTextPadding;
    TdxSpreadSheetTextBoxContainer(AContainer).TextBox.WordWrap := FWrapText;
  end;

  if AContainer is TdxSpreadSheetShapeContainer then
  begin
    AShape := TdxSpreadSheetShapeContainer(AContainer).Shape;
    if HasShape then
      AShape.ShapeType := TdxSpreadSheetShapeType(IFThen(FShapeType > 3, 1, FShapeType) - 1);

    AShape.Pen.Style := FPenStyle;
    AShape.Pen.Width := FPenWidth;
    ApplyBrushColor(AShape.Pen.Brush, FPenColor);

    if HasShape or HasBlip then
      ApplyBrushColor(AShape.Brush, FBrushColor)
    else
      ApplyBrushColor(AShape.Brush, TdxAlphaColors.Default);

    if FBrushColors.Count > 0 then
    begin
      AShape.Brush.GradientPoints.Add(0, FBrushColor);
      for I := 0 to FBrushColors.Count - 1 do
        AShape.Brush.GradientPoints.Add(Min(1, FBrushColors[I].Position / (MAXWORD + 1)), MakeAlphaColor(FBrushColors[I].Color));
      AShape.Brush.GradientPoints.Add(1, FBrushBottomColor);
      AShape.Brush.Style := gpbsGradient;
      AShape.Brush.GradientMode := FBrushGradientMode;
      if FBrushGradientModeInverseOrder then
        AShape.Brush.GradientPoints.InvertOrder;
    end;

    if HasBlip then
    begin
      dxSpreadSheetInitializeBrushPattern(AShape.Brush, GetGraphic(FFillBlipID),
    ...
    begin
      if AShape.Brush.Color = TdxAlphaColors.Default then
        AShape.Brush.Color := dxMakeAlphaColor(ColorToRgb(clInfoBk));
      if AShape.Pen.Brush.Color = TdxAlphaColors.Default then
        AShape.Pen.Brush.Color := TdxAlphaColors.Black;
    end
    else
    begin
      if AShape.Brush.Color = TdxAlphaColors.Default then
        AShape.Brush.Color := TdxAlphaColors.Empty;
      if AShape.Pen.Brush.Color = TdxAlphaColors.Default then
        AShape.Pen.Brush.Color := TdxAlphaColors.Empty;
    end;
  end;
...
function TdxSpreadSheetMSODrawingReader.ComplexDataToString(const AComplexData: array of Byte): string;
var
  ACharCount: Integer;
begin
  ACharCount := Length(AComplexData) div 2 - 1;
  if ACharCount > 0 then
  begin
    SetLength(Result, ACharCount);
    Move(AComplexData[0], Result[1], ACharCount * 2);
  end
  else
    Result := '';
end;
...
procedure TdxSpreadSheetMSODrawingReader.Property_DrawingShapeName(
  const AValue: Integer; const AComplexData: array of Byte);
begin
  FName := ComplexDataToString(AComplexData);
end;
...
begin
  if FHyperlink <> nil then
    FHyperlink.ScreenTip := ComplexDataToString(AComplexData);
end;
    ...
    begin
      ...
      try
        ...
        try
          ...
          WriteBlipStoryEntry;
          Writer.WriteRaw(ImageBinaryData);
        finally
...
begin
  ...
  try
    ...
    AWriter.WriteRaw(AHash[0], SizeOf(AHash));
...
begin
  ...
  //
  Writer.WriteRaw(AEntry, SizeOf(AEntry));
...
begin
  ...
  try
    ...
    Writer.WriteRaw(AData, SizeOf(AData));
...
var
  ...
  AComplexData: TMemoryStream;
  AComplexDataWriter: TcxWriter;
...
begin
  ...
  AHasName := Container.Name <> '';

  ARecord := MakeRecordHeader(3, 5 +
    Byte(IsShape) + Byte(BlipID >= 0) + Byte(AHasGradient) * 2 + 5 * Byte(IsTextBox) +
    Byte(AHyperlink <> nil) + Byte((AHyperlink <> nil) and (AHyperlink.ScreenTip <> '')) +
    Byte(AHasName), oaPropertiesTable);

  AComplexData := TMemoryStream.Create;
  AComplexDataWriter := TcxWriter.Create(AComplexData);
  try
    if IsTextBox then
    ...
    begin
      ...
      WriteProperty_DrawingFillShadeColors(AColors, AComplexDataWriter);
    end;
    ...
    if AHasName then
      WriteProperty_Text($C380, Container.Name, AComplexDataWriter);
    if AHyperlink <> nil then
      WriteProperty_Hyperlink(AHyperlink, AComplexDataWriter);

    Writer.WriteRaw(AComplexData);
  finally
    AComplexDataWriter.Free;
    AComplexData.Free;
  end;
  EndRecord(ARecord);
end;
...
procedure TdxSpreadSheetMSODrawingWriter.WriteProperty_DrawingFillShadeColors(
  const AValue: TdxXLSColorRefArray; ADataWriter: TcxWriter);
var
  I: Integer;
begin
  MakeProperty($c197, Length(AValue) * SizeOf(TdxXLSColorRef) + 6);

  ADataWriter.WriteWord(Length(AValue));
  ADataWriter.WriteWord(Length(AValue) + 1);
  ADataWriter.WriteWord(SizeOf(TdxXLSColorRef));
  for I := 0 to Length(AValue) - 1 do
  begin
    ADataWriter.WriteInteger(AValue[I].Color);
    ADataWriter.WriteInteger(AValue[I].Position);
  end;
end;
...
procedure TdxSpreadSheetMSODrawingWriter.WriteProperty_Hyperlink(AHyperlink: TdxSpreadSheetHyperlink; ADataWriter: TcxWriter);
var
  AHelper: TdxBIFFHyperlinkHelper;
  APosition: Int64;
begin
  APosition := ADataWriter.Stream.Position;
  AHelper := TdxBIFFHyperlinkHelper.Create(AHyperlink);
  try
    AHelper.SaveToStream(ADataWriter.Stream);
  finally
    AHelper.Free;
  end;
  MakeProperty($C382, ADataWriter.Stream.Position - APosition);
  WriteProperty_Text($C38D, AHyperlink.ScreenTip, ADataWriter);
end;

procedure TdxSpreadSheetMSODrawingWriter.WriteProperty_Text(ID: Word; const AName: string; ADataWriter: TcxWriter);
var
  ABytes: TBytes;
  ALength: Integer;
begin
  ABytes := TEncoding.Unicode.GetBytes(AName);
  ALength := Length(ABytes);
  if ALength > 0 then
  begin
    MakeProperty(ID, ALength + 2);
    ADataWriter.WriteRaw(ABytes[0], ALength);
    ADataWriter.WriteWord(0);
  end;
end;
