...
unit dxRichEdit.DocumentModel.PieceTable.InternalAPI; // for internal use
...
const
  dxThisUnitName = 'dxRichEdit.DocumentModel.PieceTable.InternalAPI';
...
procedure TdxInternalAPI.SetDocBytes(const AValue: TArray<Byte>);
begin
  if FCachedDocContent.HasValue and (StringOf(FCachedDocContent.Value) = StringOf(AValue)) then
    Exit;

  if AValue = nil then
    Text := ''
  else
    SetDocumentContent<TArray<Byte>>(AValue, SetDocumentDocContent);
end;
...
function TdxInternalAPI.GetDocBytes: TArray<Byte>;
begin
  if FCachedDocContent.IsNull then
    FCachedDocContent := GetDocumentDocContent;
  Result := FCachedDocContent.Value;
end;
