...
unit dxRichEdit.Import.Rtf.DestinationPieceTable; // for internal use
...
const
  dxThisUnitName = 'dxRichEdit.Import.Rtf.DestinationPieceTable';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxDestinationPieceTable.Initialize', SysInit.HInstance);{$ENDIF}
  FKeywordHT := CreateKeywordHT;
  FControlCharHT := CreateControlCharHT;
  FShadingPatterns := CreateShadingPatterns;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxDestinationPieceTable.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxDestinationPieceTable.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxDestinationPieceTable.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FShadingPatterns);
  FreeAndNil(FControlCharHT);
  FreeAndNil(FKeywordHT);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxDestinationPieceTable.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  ...
  if AOwnNumberingListIndex = AParentNumberingListIndex then
    Result := NumberingListIndexListIndexNotSet
  else
    if (AOwnNumberingListIndex = NumberingListIndexNoNumberingList) or (AOwnNumberingListIndex = NumberingListIndexListIndexNotSet) then
    begin
...
begin
  ...
  begin
    if AOwnNumberingListIndex = NumberingListIndexNoNumberingList then
      AOwnNumberingListIndex := NumberingListIndexListIndexNotSet;
...
begin
  ...
  begin
    AImporter.Position.ParagraphFormattingInfo.NumberingListIndex := NumberingListIndexListIndexNotSet;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxStringValueDestination.Initialize', SysInit.HInstance);{$ENDIF}
  FKeywordHT := CreateKeywordHT;
  FControlCharHT := CreateControlCharHT;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxStringValueDestination.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxStringValueDestination.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxStringValueDestination.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FControlCharHT);
  FreeAndNil(FKeywordHT);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxStringValueDestination.Finalize', SysInit.HInstance);{$ENDIF}
end;
