...
type
  ...
  public
    ...
    property InnerTreeView: TcxCustomInnerTreeView read GetInnerTreeView; // for internal use
  end;
...
{$IFDEF DELPHIBERLIN}
procedure TcxBaseInnerTreeView.ChangeScale(M, D: Integer; isDpiChange: Boolean);
{$ELSE}
procedure TcxBaseInnerTreeView.ChangeScale(M, D: Integer);
{$ENDIF}
begin
  if (M <> D){$IFDEF DELPHIBERLIN} or isDpiChange{$ENDIF} then
  begin
    inherited;
    {$IFNDEF DELPHI103}
...
function TcxTreeViewContainer.HandleInnerControlGestures: Boolean;
begin
  Result := True;
end;
...
begin
  ...
  if FInnerTreeView = nil then
    Exit;
  AOnChange := FInnerTreeView.OnChange;
  AOnChanging := FInnerTreeView.OnChanging;
  try
    FInnerTreeView.OnChange := nil;
    FInnerTreeView.OnChanging := nil;
    FInnerTreeView.RecreateWnd; 
  finally
    FInnerTreeView.OnChange := AOnChange;
    FInnerTreeView.OnChanging := AOnChanging;
  end;
end;
...
function TcxTreeViewContainer.UseBorderedBackground: Boolean;
begin
  Result := False;
end;
