...
type
  ...
  protected
    ...
    procedure DoCompleteUpload; virtual;
    procedure DoStartUpload; virtual;
    procedure DoUpload; virtual;
  ...
  TdxWebFileTransferManager = class
  public const
    DefaultBlockSize: Cardinal = 64 * 1024;
  strict private
  ...
  protected
    ...
    procedure DoError(const AErrorObject: TObject); virtual;
  ...
  public
    ...
    procedure RunDownloadFileTask(const ATask: TdxWebFileDownloadTask; AUseCurrentThread: TdxDefaultBoolean = bDefault);
    procedure RunUploadFileTask(const ATask: TdxWebFileUploadTask; AUseCurrentThread: TdxDefaultBoolean = bDefault);
  end;
...
const
  dxThisUnitName = 'dxWebFileTransferManager';
...
procedure TdxWebFileTransferTask.DoError(AObject: TObject);
begin
  Synchronize(
    procedure
    begin
...
procedure TdxWebFileTransferTask.SetFileSize(const Value: Int64);
begin
  FFileSize := Value;
end;
...
begin
  ...
  try
    if TdxHttpHelper.OpenRequestURL(ASession, Uri, Header, ARequest) then
    try
      SetFileSize(TdxHttpRequest.GetContentLength(ARequest));

      DoStartDownloadFile;
      if ACancelStatus then
        Exit(TdxTaskCompletedStatus.Cancelled);

      SetLength(ABuffer, TdxWebFileTransferManager.DefaultBlockSize);
      try
        ...
        begin
          ...
          if ABytesRead = 0 then
            Break;
          WriteBuffer(ABuffer[0], ABytesRead);
        end;
...
begin
  ...
  if TdxHttpHelper.OpenSession(UserAgent, ASession) then
  try
    if TdxHttpHelper.OpenConnection(ASession, ServerName, AConnection) then
    try
      if TdxHttpHelper.OpenRequest(AConnection, Verb, ObjectName, ARequest) then
      try
        DoStartUpload;
        if ACancelStatus then
          Exit(TdxTaskCompletedStatus.Cancelled);
...
procedure TdxWebFileTransferManager.DoError(const AErrorObject: TObject);
begin
// do nothing
end;
...
begin
  FTaskManager.ForEach(
    procedure(ATask: TdxWebTask)
    begin
...
procedure TdxWebFileTransferManager.RunDownloadFileTask(const ATask: TdxWebFileDownloadTask; AUseCurrentThread: TdxDefaultBoolean = bDefault);
begin
  FTaskManager.RunTask(ATask, AUseCurrentThread);
end;

procedure TdxWebFileTransferManager.RunUploadFileTask(const ATask: TdxWebFileUploadTask; AUseCurrentThread: TdxDefaultBoolean = bDefault);
begin
  FTaskManager.RunTask(ATask, AUseCurrentThread);
end;
