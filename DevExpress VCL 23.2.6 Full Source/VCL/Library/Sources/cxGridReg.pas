...
{$ENDIF}
  Dialogs, cxGridImportDialog, cxControls, dxCoreGraphics,
  ...
  ColnEdit, cxGridInplaceEditForm, cxFilter, TreeIntf, cxGridViewLayoutContainer, cxGridRowLayout,
  cxDataControllerConditionalFormatting, cxDesignWindows;

const
  dxThisUnitName = 'cxGridReg';
...
type
  ...
  TcxCustomGridColumnAccess = class(TcxCustomGridColumn);
...
{ TcxGridTableRowLayoutEventsProperty }

type
  TcxGridTableRowLayoutEventsProperty = class(TcxNestedEventProperty)
  protected
    function GetInstance: TPersistent; override;
  end;

function TcxGridTableRowLayoutEventsProperty.GetInstance: TPersistent;
begin
  Result := TcxGridTableView(GetComponent(0)).RowLayout;
end;
...
type
  ...
  { TGridColumnCustomLayoutItemProperty }

  TGridColumnCustomLayoutItemProperty = class(TClassProperty)
  private
    function GetColumn: TcxCustomGridColumn;
    function GetLayoutItem: TcxGridRowLayoutItem;
  protected
    function IsLayoutItemAvailable: Boolean; virtual;
  public
    function GetAttributes: TPropertyAttributes; override;
    function GetValue: string; override;

    property Column: TcxCustomGridColumn read GetColumn;
    property LayoutItem: TcxGridRowLayoutItem read GetLayoutItem;
  end;

  { TGridColumnLayoutItemProperty }

  TGridColumnLayoutItemProperty = class(TGridColumnCustomLayoutItemProperty)
  protected
    function IsLayoutItemAvailable: Boolean; override;
  end;

  { TGridColumnRowLayoutItemProperty }

  TGridColumnRowLayoutItemProperty = class(TGridColumnCustomLayoutItemProperty)
  protected
    function IsLayoutItemAvailable: Boolean; override;
  end;
  ...
  { TGridItemVisibleForExpressionEditorPropertyEditor }

  TGridItemVisibleForExpressionEditorPropertyEditor = class(TdxDefaultBooleanPropertyEditor)
  protected
    function GetDefaultValue: Boolean; override;
  end;

  { TGridColumnVisibleForRowLayoutPropertyEditor }

  TGridColumnVisibleForRowLayoutPropertyEditor = class(TdxDefaultBooleanPropertyEditor)
  protected
    function GetDefaultValue: Boolean; override;
  end;

  { TGridItemDataBindingExpressionProperty }

  TGridItemDataBindingExpressionProperty = class(TStringProperty)
  private
...
var
  ...
  AIsEditFormUseDefaultLayout, AIsRowUseDefaultLayout: Boolean;
begin
  if (ASelection.Count <> 1) or not (ASelection[0] is TcxGridColumn) then
    Exit;
  AColumn := TcxGridColumn(ASelection[0]);
  AIsRowUseDefaultLayout := AColumn.GridView.RowLayout.UseDefaultLayout;
  AIsEditFormUseDefaultLayout := AColumn.GridView.EditForm.UseDefaultLayout;
  if not AIsEditFormUseDefaultLayout and not AIsRowUseDefaultLayout then
    Exit;
  ...
  begin
    ...
    if AProperty = nil then
      Continue;
    AName := AProperty.GetName;
    if AIsEditFormUseDefaultLayout and (AName = 'LayoutItem') or
      AIsRowUseDefaultLayout and (AName = 'RowLayoutItem') then
...
function TGridColumnCustomLayoutItemProperty.GetAttributes: TPropertyAttributes;
begin
...
function TGridColumnCustomLayoutItemProperty.GetValue: string;
begin
  if IsLayoutItemAvailable then
    Result := inherited GetValue
  else
...
function TGridColumnCustomLayoutItemProperty.IsLayoutItemAvailable: Boolean;
begin
  Result := LayoutItem <> nil;
end;

function TGridColumnCustomLayoutItemProperty.GetColumn: TcxCustomGridColumn;
begin
  Result := TcxCustomGridColumn(LayoutItem.GridViewItem);
end;

function TGridColumnCustomLayoutItemProperty.GetLayoutItem: TcxGridRowLayoutItem;
begin
  if TComponent(GetOrdValue) is TcxGridRowLayoutItem then
    Result := TcxGridRowLayoutItem(GetOrdValue)
  else
    Result := nil;
end;
...
begin
  Result := inherited IsLayoutItemAvailable and not Column.GridView.EditForm.UseDefaultLayout;
end;

{ TGridColumnRowLayoutItemProperty }

function TGridColumnRowLayoutItemProperty.IsLayoutItemAvailable: Boolean;
begin
  Result := inherited IsLayoutItemAvailable and not Column.GridView.RowLayout.UseDefaultLayout;
end;
...
begin
  Result := TcxCustomGridColumnAccess(GetComponent(0)).IsDefaultVisibleForEditForm
end;

{ TGridItemVisibleForExpressionEditorPropertyEditor }

function TGridItemVisibleForExpressionEditorPropertyEditor.GetDefaultValue: Boolean;
begin
  Result := TcxCustomGridTableItemAccess(GetComponent(0)).IsDefaultVisibleForExpressionEditor;
end;

{ TGridColumnVisibleForRowLayoutPropertyEditor }

function TGridColumnVisibleForRowLayoutPropertyEditor.GetDefaultValue: Boolean;
begin
  Result := TcxCustomGridColumnAccess(GetComponent(0)).IsDefaultVisibleForRowLayout;
end;

{ TGridItemDataBindingExpressionProperty }

procedure TGridItemDataBindingExpressionProperty.Edit;
begin
  Item.ShowExpressionEditor;
end;

function TGridItemDataBindingExpressionProperty.GetAttributes: TPropertyAttributes;
begin
  Result := inherited GetAttributes;
  if PropCount = 1 then
    Result := [paDialog];
...
function TGridItemDataBindingExpressionProperty.GetItem: TcxCustomGridTableItem;
begin
  Result := TcxGridItemDataBinding(GetComponent(0)).Item;
end;
...
begin
  ...
  RegisterSelectionEditor(TcxGridColumn, TcxGridColumnLayoutItemPropertyFilter);
...
{$ENDIF}
  ...
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxCustomGrid, 'RootLevelStylesEvents', TcxGridRootLevelStylesEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxGridLevel, 'StylesEvents', TcxGridLevelStylesEventsProperty);
  RegisterPropertyEditor(TypeInfo(TcxCustomGridView), TcxGridLevel, 'GridView', TcxGridLevelGridViewProperty);
  RegisterPropertyEditor(TypeInfo(TcxImageIndex), TcxGridLevel, 'ImageIndex', TcxGridLevelImageIndexProperty);
  RegisterPropertyEditor(TypeInfo(TcxImageComboBoxItems), TcxImageComboBoxProperties, 'Items', TcxImageComboBoxItemsProperty);
  RegisterPropertyEditor(TypeInfo(TcxDataGroupSummaryItems), TcxDataSummary, '', TcxDataGroupSummaryItemsPropertyEditor);

  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxCustomGridView, 'DataControllerEvents', TcxGridDataControllerEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxCustomGridView, 'StylesEvents', TcxGridViewStylesEventsProperty);
  RegisterPropertyEditor(TypeInfo(TComponent), TcxCustomGridView, 'PopupMenu', TcxControlPopupMenuProperty);

  RegisterPropertyEditor(TypeInfo(TcxGridFindPanelOptions), TcxCustomGridTableView, 'FindPanel', TGridFindPanelOptionsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxCustomGridTableView, 'NavigatorEvents', TcxGridViewNavigatorEventsProperty);
  RegisterPropertyEditor(TypeInfo(TdxDefaultBoolean), TcxCustomGridTableItem, 'VisibleForExpressionEditor', TGridItemVisibleForExpressionEditorPropertyEditor);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxCustomGridTableItem, 'PropertiesEvents', TcxGridTableItemPropertiesEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxCustomGridTableItem, 'StylesEvents', TcxGridTableItemStylesEventsProperty);
  RegisterPropertyEditor(TypeInfo(TcxCustomEditProperties), TcxCustomGridTableItem, 'Properties', TcxCustomGridTableItemPropertiesProperty);
  RegisterPropertyEditor(TypeInfo(TcxCustomGridTableItem), TcxGridPreview, 'Column', TcxGridPreviewColumnProperty);
  RegisterPropertyEditor(TypeInfo(TcxCustomGridTableItem), TcxGridTableSummaryItem, 'Column', TcxGridTableSummaryItemColumnProperty);
  RegisterPropertyEditor(TypeInfo(TcxCustomGridTableItem), TcxGridTableSummaryGroupItemLink, 'Column', TcxGridTableSummaryItemColumnProperty);
  RegisterPropertyEditor(TypeInfo(TcxCustomGridTableItem), TcxCustomGridTableOptionsBehavior, 'IncSearchItem', TcxGridIncSearchItemProperty);
  RegisterPropertyEditor(TypeInfo(string), TcxGridItemDataBinding, 'ValueType', TcxValueTypeProperty);
  RegisterPropertyEditor(TypeInfo(string), TcxGridItemDataBinding, 'Expression', TGridItemDataBindingExpressionProperty);

  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxGridTableView, 'RowLayoutEvents', TcxGridTableRowLayoutEventsProperty);
  RegisterPropertyEditor(TypeInfo(TcxGridInplaceEditFormLayoutItem), TcxGridColumn, 'LayoutItem', TGridColumnLayoutItemProperty);
  RegisterPropertyEditor(TypeInfo(TcxGridTableRowLayoutItem), TcxGridColumn, 'RowLayoutItem', TGridColumnRowLayoutItemProperty);
  ...
  RegisterPropertyEditor(TypeInfo(TdxDefaultBoolean), TcxGridColumn, 'VisibleForRowLayout', TGridColumnVisibleForRowLayoutPropertyEditor);
  ...
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxGridBand, 'StylesEvents', TcxGridBandStylesEventsProperty);
  RegisterPropertyEditor(TypeInfo(Integer), TcxGridBandPosition, 'BandIndex', TcxGridBandBandIndexProperty);
  RegisterPropertyEditor(TypeInfo(Integer), TcxGridBandedColumnPosition, 'BandIndex', TcxGridBandedColumnBandIndexProperty);

  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxGridChartView, 'CategoriesEvents', TcxGridChartCategoriesEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxGridChartView, 'DiagramAreaEvents', TcxGridChartAreaDiagramEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxGridChartView, 'DiagramBarEvents', TcxGridChartBarDiagramEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxGridChartView, 'DiagramStackedBarEvents', TcxGridChartStackedBarDiagramEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxGridChartView, 'DiagramStackedColumnEvents', TcxGridChartStackedColumnDiagramEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxGridChartView, 'DiagramStackedAreaEvents', TcxGridChartStackedAreaDiagramEventsProperty);
  ...
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxGridChartView, 'DiagramLineEvents', TcxGridChartLineDiagramEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxGridChartView, 'DiagramPieEvents', TcxGridChartPieDiagramEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxGridChartSeries, 'StylesEvents', TcxGridChartSeriesStylesEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxGridChartHistogram, 'ValuesEvents', TcxGridChartHistogramValuesEventsProperty);
...
{$IFNDEF NONDB}
  ...
  RegisterPropertyEditor(TypeInfo(TcxCustomGridTableItem), TcxGridDBTableSummaryItem, 'Column', TcxGridTableSummaryItemColumnProperty);
  ...
  RegisterEditPropertiesIcon(TcxExtLookupComboBoxProperties, 'TcxExtLookupComboBox', Hinstance);
...
{$ENDIF}
  ...
  RegisterNoIcon([
    TcxGridInplaceEditFormLayoutItem, TcxGridInplaceEditFormGroup,
    TcxGridTableRowLayoutItem, TcxGridTableRowLayoutGroup]);
  ...
  RegisterPropertyEditor(TypeInfo(TcxGridColumnFixedKind), TcxGridBandedColumn, 'FixedKind', nil);
  RegisterPropertyEditor(TypeInfo(TColor), TcxGridBandedTableOptionsView, 'FixedColumnSeparatorColor', nil);
  RegisterPropertyEditor(TypeInfo(Integer), TcxGridBandedTableOptionsView, 'FixedColumnSeparatorWidth', nil);
  RegisterPropertyEditor(TypeInfo(TdxAlphaColor), TcxGridBandedTableOptionsView, 'FixedColumnHighlightColor', nil);
  RegisterPropertyEditor(TypeInfo(Boolean), TcxGridBandedTableOptionsView, 'HighlightFixedColumns', nil);
  HideClassProperties(TcxGridInplaceEditFormLayoutItem, ['LayoutLookAndFeel', 'Name']);
  HideClassProperties(TcxGridTableRowLayoutItem, ['LayoutLookAndFeel', 'Name']);

  dxHideClassesFromStructureView([TcxGridInplaceEditFormLayoutItem, TcxGridInplaceEditFormGroup,
    TcxGridTableRowLayoutItem, TcxGridTableRowLayoutGroup]);
