...
uses
  Types, Classes, SysUtils, TypInfo, Generics.Defaults, Generics.Collections, SyncObjs, RTTI, DB,
  dxEMF.Types,
  dxEMF.DB.Model,
  dxEMF.Utils,
  dxEMF.Metadata,
...
{$ENDIF}
  ...
  protected
    ...
    function GetCollectionProperties(const Index: TCollectionProperty): Boolean; inline;
    procedure SetCollectionProperties(const Index: TCollectionProperty; const Value: Boolean); inline;
  ...
  public const
    DoNotSetAssociatedMemberToNullWhenRemovingDeletedObjectFromAssociationList: Boolean = False;
  strict private
  ...
  protected
    ...
    property FieldsCache: TArray<TFieldCache> read FFieldsCache write FFieldsCache;
    property KeyFieldsCache: TArray<TFieldCache> read FKeyFieldsCache write FKeyFieldsCache;
  public
  ...
  strict private
    ...
    FBatchUpdateCount: Integer;
    FBatchLoadedPoolClasses: TdxEntityPoolClasses;
    FClients: TList<TObject>;
    FConnectEvents: TList<Pointer>;
    FDeletedObjects: TObjectDictionary<TObject, Integer>;
    ...
    procedure CreatePoolEvent(ASender: TObject);
    procedure EntityInfoNotification(ASender: TObject; AConnecting: Boolean);
    procedure UnregisterEntityInfoNotification;
  ...
  protected
    ...
    procedure BeginBatchLoad; inline;
    procedure EndBatchLoad; inline;
    function FindBatch(AClass: TClass; const AKeys: TArray<TValue>): TObject;
    ...
    function CanNullifyAssociatedMember(AAssociatedMember: TdxMappingMemberInfo; AObjectRemoved: TObject): Boolean;
    ...
    procedure DetachLinkedDataSources;
    ...
    function CreateEntity(AEntityInfo: TdxEntityInfo): TObject;
    ...
    procedure RegisterClient(AClient: TObject; AEvent: TConnectChangeEvent = nil); virtual;
    procedure SendConnectEvent(AConnecting: Boolean);
    procedure UnRegisterClient(AClient: TObject); virtual;
    ...
    property DeletedObjects: TObjectDictionary<TObject, Integer> read FDeletedObjects;
  ...
  public
    ...
    function Find<T: class>(const AQuery: IdxQueryable<T>): T; overload;
    ...
    function Find(const AQuery: IdxQueryable): TObject; overload;
  ...
  strict private
    ...
    FSessions: TList<TdxEMFCustomSession>;
    procedure DetachSessions;
  ...
  protected
    ...
    procedure ConnectionStateChange(ASender: TObject; AConnecting: Boolean);
    ...
    procedure CheckSQLProvider; virtual;
    function GetActualDBEngineType: TdxDBEngine;
    procedure RecreateProviderSQL;

    procedure Notification(AComponent: TComponent; Operation: TOperation); override;
    procedure RegisterSession(ASession: TdxEMFCustomSession);
    procedure UnregisterSession(ASession: TdxEMFCustomSession);
    ...
    function GetDBSchema(ASortByName: Boolean = False): TObjectList<TdxDBTable>;
    ...
    property QueryDataPool: TdxQueryDataPool read FQueryDataPool;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
...
procedure TdxEMFCustomCollection.DoFetchAll;
begin
  if IsLoaded then
    Exit;
  if FSession = nil then
  ...
  begin
    FSession.BeginBatchLoad;
    try
      FetchAll;
    finally
      FSession.EndBatchLoad;
    end;
  end;
end;
...
procedure TdxDataSetLoader.EndLoad;
begin
end;
...
procedure TdxDataSetLoader.CloseAndFreeDataSet;
begin
  if FOwnDataSet then
    FreeAndNil(FDataSet)
  else
...
function TdxDataSetLoader.GetFieldDefs: TFieldDefs;
begin
...
begin
  ...
  FPoolClasses.OnCreatePool := CreatePoolEvent;
  ...
  FDeletedObjects := TObjectDictionary<TObject, Integer>.Create([doOwnsKeys]);
end;
...
begin
  ...
  FreeAndNil(FBatchLoadedPoolClasses);
  inherited Destroy;
...
function TdxEMFCustomSession.CreateEntity(AEntityInfo: TdxEntityInfo): TObject;
begin
  DoCreateObject(AEntityInfo.ClassAttributes.PersistentClass, Result);
  if Result <> nil then
    Exit;
  Result := AEntityInfo.CreateObject;
end;
...
var
  AEMFCollection: TdxEMFCustomCollection;
begin
  for AEMFCollection in FLinkedCollections.Keys.ToArray do
    AEMFCollection.DetachSession();
end;

procedure TdxEMFCustomSession.DetachLinkedDataSources;
var
  I: Integer;
begin
  for I := FClients.Count - 1 downto 0 do
    if FClients[I] is TdxCustomEMFDataSource then
      TdxCustomEMFDataSource(FClients[I]).Session := nil;
end;
...
procedure TdxEMFCustomSession.BeginBatchLoad;
begin
  if FBatchUpdateCount = 0 then
  begin
    TMonitor.Enter(Self);
    try
      if FBatchLoadedPoolClasses = nil then
        FBatchLoadedPoolClasses := TdxEntityPoolClasses.Create(False);
    finally
      TMonitor.Exit(Self);
    end;
  end;
  TInterlocked.Increment(FBatchUpdateCount); 
end;
  ...
  try
    ...
    try
      ...
      if (AStatements.Count <> 0) and (AContext.Session.Options.LockingOption = TdxLockingOption.None) then
        for AStatement in AStatements.ToArray do
          AStatement.AffectedRecordCount := 0;
      AContext.Statements := AStatements.ToArray;
    finally
...
var
  AObjectsToFireSaved: TdxArray<TObject>;
  AObject: TObject;
begin
  Result := nil;
  ...
  try
    ...
    AObjectsToFireSaved := TdxArray<TObject>.Create(FObjectsMarkedSaved.Count);
...
function TdxEMFCustomSession.CanNullifyAssociatedMember(AAssociatedMember: TdxMappingMemberInfo;
  AObjectRemoved: TObject): Boolean;
begin
  if AAssociatedMember.IsKeyOrInKey then
    Exit(False);
  if TdxReferencedCollectionHelper.DoNotSetAssociatedMemberToNullWhenRemovingDeletedObjectFromAssociationList then
    if IsObjectToDelete(AObjectRemoved) then
      Exit(False);
  Result := True;
end;
...
begin
  ...
  CommitChanges(AFullListForDelete, AProcessedListForSave.ToArray);
end;
...
var
  ...
  ALoadDataQuery: TdxArray<TdxObjectsQuery>;
  ARequiresResorting: TArray<Boolean>;
  I, ASkipPhysicallySelected, ATopPhysicallySelected, ACount: Integer;
...
begin
  if AQueries = nil then
    raise EArgumentNilException.Create('');
  ACount := Length(AQueries);
  ALoadDataQuery := TdxArray<TdxObjectsQuery>.Create(ACount);
  SetLength(AResult, ACount);
  SetLength(AExpanded, ACount);
  SetLength(ARequiresResorting, ACount);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if ARefObject.IsNullOrEmpty then
            Continue;
        ...
        begin
          ...
          if ACollection = nil then
            Continue;
           if Safe.Cast(ACollection.Helper, TdxReferencedCollectionHelper, AReferencedCollectionHelper) then
...
procedure TdxEMFCustomSession.RegisterClient(AClient: TObject; AEvent: TConnectChangeEvent = nil);
begin
  FClients.Add(AClient);
  FConnectEvents.Add(TMethod(AEvent).Code);
end;
...
begin
  DeletedObjects.ExtractPair(AObject);
  AppendObject(AObject);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if AInterface.QueryInterface(IdxList, AAssociatedCollection) = 0 then
...
procedure TdxEMFCustomSession.DropIdentityMap;
begin
  FLoadingObjects.Clear;
  if FBatchLoadedPoolClasses <> nil then
    TdxEntityPoolClassesAccess(FBatchLoadedPoolClasses).FreePools;
  TdxEntityPoolClassesAccess(FPoolClasses).FreePools;
  DataProvider.QueryDataPool.Clear;
end;

procedure TdxEMFCustomSession.EndBatchLoad;
begin
  TInterlocked.Decrement(FBatchUpdateCount);
  if FBatchUpdateCount = 0 then
  begin
    TMonitor.Enter(Self);
    FreeAndNil(FBatchLoadedPoolClasses);
...
function TdxEMFCustomSession.IsReady: Boolean;
begin
  CheckConnection;
  Result := DataProvider.Connection.Connected;
end;

procedure TdxEMFCustomSession.EntityInfoNotification(ASender: TObject; AConnecting: Boolean);
var
  AEntityInfo: TdxEntityInfo;
  AEntityClass: TClass;
  ACollection: TdxEMFCustomCollection;
begin
  AEntityInfo := ASender as TdxEntityInfo;
  AEntityClass := AEntityInfo.ClassAttributes.PersistentClass;
  for ACollection in FLinkedCollections.Keys.ToArray do
    if ACollection.CollectionElementClass = AEntityClass then
    begin
      ACollection.Clear;
      FLinkedCollections.Remove(ACollection);
    end;
  TdxEntityPoolClassesAccess(FPoolClasses).RemovePool(AEntityClass);
end;
...
procedure TdxEMFCustomSession.SendConnectEvent(AConnecting: Boolean);
var
  I: Integer;
...
begin
  for I := 0 to FClients.Count - 1 do
    ...
    begin
      TMethod(AConnectEvent).Code := FConnectEvents[I];
      TMethod(AConnectEvent).Data := FClients[I];
      AConnectEvent(Self, AConnecting);
    end;
end;
...
begin
  ...
  begin
    FDataProvider.UnregisterSession(Self);
    FDataProvider.RemoveFreeNotification(Self);
  end;
  FDataProvider := Value;
  if FDataProvider <> nil then
  begin
    FDataProvider.RegisterSession(Self);
...
procedure TdxEMFCustomSession.UnRegisterClient(AClient: TObject);
var
  AIndex: Integer;
begin
  AIndex := FClients.IndexOf(AClient);
  if AIndex <> -1 then
  begin
    FClients.Delete(AIndex);
    FConnectEvents.Delete(AIndex);
  end;
end;
...
procedure TdxEMFCustomSession.UnregisterEntityInfoNotification;
var
  AEntityPool: TdxCustomEntityPool;
begin
  for AEntityPool in FPoolClasses do
    TdxEntityManager.UnRegisterEntityClient(AEntityPool.EntityInfo, Self);
end;
...
begin
  Detach(AObject);
  if UseCache then
    if not FDeletedObjects.ContainsKey(AObject) then
      FDeletedObjects.Add(AObject, 0);
end;
...
procedure TdxEMFCustomSession.CreatePoolEvent(ASender: TObject);
begin
  TdxEntityManager.RegisterEntityClient((ASender as TdxCustomEntityPool).EntityInfo, Self, EntityInfoNotification);
end;
...
function CheckEmpty(const AKeys: TArray<TValue>): Boolean; inline;
var
  I: Integer;
begin
  for I := 0 to Length(AKeys) - 1 do
   if not AKeys[I].IsNullOrEmpty then
     Exit(False);
  Result := True;
end;

function TdxEMFCustomSession.FindBatch(AClass: TClass; const AKeys: TArray<TValue>): TObject;
begin
  if CheckEmpty(AKeys) then
    Exit(nil);
  if FBatchLoadedPoolClasses = nil then
    Result := Find(AClass, AKeys)
  else
  begin
    Result := FBatchLoadedPoolClasses.FindObjectByKey(AClass, AKeys);
    if Result = nil then
    begin
      Result := Find(AClass, AKeys);
      if Result <> nil then
        FBatchLoadedPoolClasses.AppendObject(AKeys, Result);
    end;
  end;
end;
...
function TdxEMFCustomSession.Find(const AQuery: IdxQueryable): TObject;
var
  ALinqQuery: IdxLinqQueryBuilder;
begin
  ALinqQuery := AQuery as IdxLinqQueryBuilder;
  Result := GetObjects(ALinqQuery.FromClause.FromClass, ALinqQuery.WhereClause, ALinqQuery.OrderByClause,
    ALinqQuery.SkipSelectedRecords, 1).First;
end;

function TdxEMFCustomSession.Find<T>(const AQuery: IdxQueryable<T>): T;
var
  ALinqQuery: IdxLinqQueryBuilder;
begin
  ALinqQuery := AQuery as IdxLinqQueryBuilder;
  ...
  Result := GetObjects<T>(ALinqQuery.WhereClause, ALinqQuery.OrderByClause,
    ALinqQuery.SkipSelectedRecords, 1).First;
end;
...
begin
  ...
  FSessions := TList<TdxEMFCustomSession>.Create;
end;
...
procedure TdxEMFCustomDataProvider.DetachSessions;
var
  ASession: TdxEMFCustomSession;
begin
  for ASession in FSessions do
    ASession.DataProvider := nil;
end;
...
function TdxEMFCustomDataProvider.GetDBSchema(ASortByName: Boolean = False): TObjectList<TdxDBTable>;
var
  ATables: TStringList;
  ADBTable: TdxDBTable;
  I: Integer;
...
procedure TdxEMFCustomDataProvider.UnregisterSession(ASession: TdxEMFCustomSession);
begin
...
procedure TdxEMFCustomDataProvider.CheckSQLProvider;
begin
...
procedure TdxEMFCustomDataProvider.ConnectionStateChange(ASender: TObject; AConnecting: Boolean);
var
  ASession: TdxEMFCustomSession;
begin
  if not AConnecting then
    for ASession in FSessions do
      ASession.SendConnectEvent(AConnecting);
end;
...
begin
  ...
  begin
    ConnectionStateChange(FConnection, False);
    FConnection.RemoveFreeNotification(Self);
    TCustomConnectionAccess(FConnection).UnregisterClient(Self);
  end;
  FConnection := Value;
  if FConnection <> nil then
  begin
    FConnection.FreeNotification(Self);
    TCustomConnectionAccess(FConnection).RegisterClient(Self, ConnectionStateChange)
  end;
...
procedure TdxEMFCustomDataProvider.RegisterSession(ASession: TdxEMFCustomSession);
begin
  if not FSessions.Contains(ASession) then
        ...
        if AMemberInfo.IsKey then
          begin
            if AMemberInfo.IsReference then
              AKeyFieldsCache.Add(TFieldCache.Create(AMemberInfo.ReferenceType.KeyProperty.Member, AFields))
            else
...
begin
  ...
  for I := 0 to Length(FFieldsCache) - 1 do
  begin
    AMemberInfo := FFieldsCache[I].Key;
    AFields := FFieldsCache[I].Value;
    if not (AMemberInfo.IsWritable or AMemberInfo.IsSerialize) then
      Continue;
    if AMemberInfo.IsReference then
    begin
      AKeys := TdxFieldValueConverter.ToValues(AFields, AMemberInfo.ReferenceType.KeyProperty.Member);
      ARefObject := ASession.FindBatch(AMemberInfo.ReferenceType.ClassAttributes.PersistentClass, AKeys);
    ...
    begin
      if AFields <> nil then
        if Assigned(AMemberInfo.OnSetter) then
          AMemberInfo.OnSetter(AInstance, AFields[0].Value)
        else
          TdxFieldValueConverter.SetValue(AInstance, AFields, AMemberInfo);
    end;
  end;
end;
