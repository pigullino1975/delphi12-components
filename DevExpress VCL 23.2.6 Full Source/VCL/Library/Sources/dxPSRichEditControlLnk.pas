...
uses
  ...
  cxGeometry,
  dxCore, dxPSCore, dxPSGlbl, dxPrnPg, dxPrnDlg, dxPSRes, dxBase, dxPSFillPatterns, dxPSReportRenderCanvas, dxCustomPreview, dxPreVw, dxPSPrVw,
...
type
  ...
  protected
    ...
    function UseReportLinkDelimiters: Boolean; override;
  public
...
const
  dxThisUnitName = 'dxPSRichEditControlLnk';
...
function TcxRichEditControlReportLinkRenderInfo.UseReportLinkDelimiters: Boolean;
begin
  Result := True;
end;
...
function TdxRichEditControlReportLink.PrintDialog(var APrintDlgData: TdxPrintDlgData): Boolean;
var
  APagedView: TdxPageBasedRichEditView;
  APageCount: Integer;
begin
  if not ComponentPrinter.PreviewExists then
  begin
    Active := True;
    if Safe.Cast(Control.ActiveView, TdxPageBasedRichEditView, APagedView) then
    begin
      APageCount := APagedView.PageCount;
      APrintDlgData.DialogData.MaxRange := IfThen(APageCount > 0, APageCount, -1);
      APrintDlgData.DialogData.MinRange := Ord(APageCount > 0);
      APrintDlgData.DialogData.PageCount := APageCount;
      if APageCount > 0 then
        APrintDlgData.DialogData.Pages := '1-' + IntToStr(APageCount);
      CurrentPage := EnsureRange(APagedView.CurrentPageIndex + 1, 1, APageCount);
    end
    else
    begin
      APrintDlgData.DialogData.Pages := '';
      APrintDlgData.OptionsEnabled := APrintDlgData.OptionsVisible - [pdoCurrentPage, pdoSelection, pdoPageRange];
      CurrentPage := 1;
    end;
  end;
  Result := inherited PrintDialog(APrintDlgData);
end;
