...
uses
  Types, Windows, SysUtils, Classes, Controls, Graphics, Forms, StdCtrls, Messages, Generics.Defaults, Generics.Collections,
  dxCore, cxClasses, dxCoreClasses, cxControls, cxContainer, cxGraphics, cxLookAndFeels, cxLookAndFeelPainters,
  cxDataUtils, cxDataStorage, cxCustomData, cxData, cxEdit, cxEditRepositoryItems, cxAccessibility, cxGeometry;
...
type
  ...
  public
    ...
    function CreateEditViewData(AProperties: TcxCustomEditProperties; AScaleFactor: TdxScaleFactor): TcxCustomEditViewData;
  ...
  private
    function GetItem(Index: TdxListIndex): TcxLookupGridColumnViewInfo;
  public
    property Items[Index: TdxListIndex]: TcxLookupGridColumnViewInfo read GetItem; default; // for internal use
  end;

  // rows

  { TdxLookupGridCellAccessibilityHelper }

  TcxLookupGridCellViewInfo = class;
  TdxLookupGridRowAccessibilityHelper = class;

  TdxLookupGridCellAccessibilityHelper = class(TcxAccessibilityHelper) // for internal use
  strict private
    FRow: TdxLookupGridRowAccessibilityHelper;

    function GetViewInfo: TcxLookupGridCellViewInfo;
  protected
    FLocalId: Integer;
    ...
    property Row: TdxLookupGridRowAccessibilityHelper read FRow;
    property ViewInfo: TcxLookupGridCellViewInfo read GetViewInfo;
  public
    constructor Create(AOwnerObject: TObject; ARow: TdxLookupGridRowAccessibilityHelper); reintroduce; virtual;
  ...
  { TdxLookupGridRowAccessibilityHelper }

  TdxLookupGridAccessibilityHelper = class;
  TcxLookupGridRowViewInfo = class;

  TdxLookupGridRowAccessibilityHelper = class(TcxAccessibilityHelper) // for internal use
  strict private
    FGrid: TdxLookupGridAccessibilityHelper;
    FIAccessibilityChildren: TInterfaceList;

    function GetViewInfo: TcxLookupGridRowViewInfo;
  protected
    ...
    property Grid: TdxLookupGridAccessibilityHelper read FGrid;
    property ViewInfo: TcxLookupGridRowViewInfo read GetViewInfo;
  public
    constructor Create(AOwnerObject: TObject; AGrid: TdxLookupGridAccessibilityHelper); reintroduce; virtual;
    destructor Destroy; override;
  ...
  private
    function GetItem(Index: TdxListIndex): TcxLookupGridCellViewInfo;
  protected
    FIAccessibilityHelper: IcxAccessibilityHelper;

    function AddCell(AIndex: Integer; const AInitBounds: TRect; AIsFocused: Boolean): TcxLookupGridCellViewInfo;
    procedure PopulateIAccessibilityChildren(IAccessibilityChildren: TInterfaceList);
  public
    Borders: TcxBorders;
    Bounds: TRect;
    ContentBounds: TRect;
    ...
    property Items[Index: TdxListIndex]: TcxLookupGridCellViewInfo read GetItem; default; // for internal use
  end;

  TcxLookupGridRowsViewInfo = class(TcxObjectList)
  private
    function GetItem(Index: TdxListIndex): TcxLookupGridRowViewInfo;
  public
    function FindByRowIndex(ARowIndex: Integer): TcxLookupGridRowViewInfo;
    property Items[Index: TdxListIndex]: TcxLookupGridRowViewInfo read GetItem; default; // for internal use
  end;

  { TdxLookupGridAccessibilityHelper }

  TcxLookupGridViewInfo = class;

  TdxLookupGridAccessibilityHelper = class(TcxAccessibilityHelper) // for internal use
  strict private
    FIAccessibilityChildren: TInterfaceList;

    function GetColumnIndex(AChildID: TcxAccessibleSimpleChildElementID): Integer;
  ...
  public
    ...
    property Columns: TcxLookupGridColumnsViewInfo read FColumns; // for internal use
    property EmptyAreaColor: TColor read GetEmptyAreaColor;
    property Grid: TcxCustomLookupGrid read FGrid; // for internal use
    ...
    property Rows: TcxLookupGridRowsViewInfo read FRows; // for internal use
  ...
  public
    ...
    property Grid: TcxCustomLookupGrid read GetGrid; // for internal use
  ...
  public
    ...
    property Painter: TcxLookupGridPainter read FPainter; // for internal use
    property RowCount: Integer read GetRowCount;
    ...
    property ViewInfo: TcxLookupGridViewInfo read FViewInfo; // for internal use
...
uses
  Math, Variants, cxEditRegisteredRepositoryItems, cxEditDataRegisteredRepositoryItems, cxTextEdit, dxDPIAwareUtils;

const
  dxThisUnitName = 'cxLookupGrid';
...
function TcxLookupGridColumnViewInfo.CreateEditViewData(AProperties: TcxCustomEditProperties; AScaleFactor: TdxScaleFactor): TcxCustomEditViewData;
begin
  FEditViewData := AProperties.CreateViewData(FStyle, True, AScaleFactor);
...
function TcxLookupGridColumnsViewInfo.GetItem(Index: TdxListIndex): TcxLookupGridColumnViewInfo;
begin
...
constructor TdxLookupGridCellAccessibilityHelper.Create(AOwnerObject: TObject; ARow: TdxLookupGridRowAccessibilityHelper);
begin
  inherited Create(AOwnerObject);
  FRow := ARow;
  FLocalId := Row.Grid.ViewInfo.FAccessibleObjects.Count + 1;
  Row.Grid.ViewInfo.FAccessibleObjects.Add(Self);
end;

function TdxLookupGridCellAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  Result := cxRectSetOrigin(ViewInfo.Bounds, Row.Grid.Grid.ClientToScreen(ViewInfo.Bounds.TopLeft));
end;

procedure TdxLookupGridCellAccessibilityHelper.DoDefaultAction(AChildID: TcxAccessibleSimpleChildElementID);
begin
  Row.Grid.Grid.FocusedRowIndex := Row.ViewInfo.RowIndex;
  Row.Grid.Grid.DoCloseUp(True);
end;

function TdxLookupGridCellAccessibilityHelper.GetDefaultActionDescription(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := 'Double click';
end;

function TdxLookupGridCellAccessibilityHelper.GetLocalId(AChildID: TcxAccessibleSimpleChildElementID = 0): Integer;
begin
  Result := FLocalId;
end;

function TdxLookupGridCellAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := 'Row ' + IntToStr(Row.ViewInfo.RowIndex + 1) + ', Column ' + IntToStr(ViewInfo.Index + 1)
end;

function TdxLookupGridCellAccessibilityHelper.GetParent: TcxAccessibilityHelper;
begin
  Result := FRow;
end;

function TdxLookupGridCellAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxROLE_SYSTEM_CELL
end;

function TdxLookupGridCellAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxSTATE_SYSTEM_SELECTABLE or cxSTATE_SYSTEM_FOCUSABLE;
  if Row.IsSelected then
    Result := Result or cxSTATE_SYSTEM_SELECTED or cxSTATE_SYSTEM_FOCUSED;
end;

function TdxLookupGridCellAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := [aopFocus, aopLocation, aopValue, aopDefaultAction];
end;

function TdxLookupGridCellAccessibilityHelper.GetValue(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := TcxCustomTextEditViewInfo(ViewInfo.EditViewInfo).Text;
end;

function TdxLookupGridCellAccessibilityHelper.IsExtended: Boolean;
begin
  Result := True;
end;

function TdxLookupGridCellAccessibilityHelper.GetViewInfo: TcxLookupGridCellViewInfo;
begin
  Result := TcxLookupGridCellViewInfo(OwnerObject);
end;
...
constructor TdxLookupGridRowAccessibilityHelper.Create(AOwnerObject: TObject; AGrid: TdxLookupGridAccessibilityHelper);
begin
  inherited Create(AOwnerObject);
...
destructor TdxLookupGridRowAccessibilityHelper.Destroy;
begin
  FreeAndNil(FIAccessibilityChildren);
  inherited;
end;

function TdxLookupGridRowAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  Result := cxRectSetOrigin(ViewInfo.Bounds, Grid.Grid.ClientToScreen(ViewInfo.Bounds.TopLeft));
end;

function TdxLookupGridRowAccessibilityHelper.ChildIsSimpleElement(AIndex: Integer): Boolean;
begin
  Result := False;
end;

function TdxLookupGridRowAccessibilityHelper.GetChild(AIndex: Integer): TcxAccessibilityHelper;
begin
  Result := IcxAccessibilityHelper(FIAccessibilityChildren[AIndex]).GetHelper;
end;

function TdxLookupGridRowAccessibilityHelper.GetChildCount: Integer;
begin
  ViewInfo.PopulateIAccessibilityChildren(FIAccessibilityChildren);
  Result := FIAccessibilityChildren.Count;
end;

function TdxLookupGridRowAccessibilityHelper.GetParent: TcxAccessibilityHelper;
begin
  Result := FGrid;
end;

function TdxLookupGridRowAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxROLE_SYSTEM_ROW;
end;

function TdxLookupGridRowAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxSTATE_SYSTEM_FOCUSABLE;
  if Grid.Grid.DataController.IsRowSelected(ViewInfo.RowIndex) then
    Result := Result or cxSTATE_SYSTEM_SELECTED;
end;

function TdxLookupGridRowAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := [aopFocus, aopLocation];
end;

function TdxLookupGridRowAccessibilityHelper.IsExtended: Boolean;
begin
  Result := True;
end;

function TdxLookupGridRowAccessibilityHelper.IsSelected: Boolean;
begin
  Result := States[0] and cxSTATE_SYSTEM_SELECTED <> 0;
end;

function TdxLookupGridRowAccessibilityHelper.GetViewInfo: TcxLookupGridRowViewInfo;
begin
  Result := TcxLookupGridRowViewInfo(OwnerObject);
end;

{ TcxLookupGridRowViewInfo }

destructor TcxLookupGridRowViewInfo.Destroy;
begin
  cxAccessibilityHelperOwnerObjectDestroyed(FIAccessibilityHelper);
  inherited;
end;
...
procedure TcxLookupGridRowViewInfo.PopulateIAccessibilityChildren(IAccessibilityChildren: TInterfaceList);
var
  I: Integer;
begin
  ...
  begin
    if Items[I].FIAccessibilityHelper = nil then
      Items[I].FIAccessibilityHelper :=
        TdxLookupGridCellAccessibilityHelper.Create(Items[I], TdxLookupGridRowAccessibilityHelper(FIAccessibilityHelper.GetHelper));
    IAccessibilityChildren.Add(Items[I].FIAccessibilityHelper);
  end;
end;

function TcxLookupGridRowViewInfo.GetItem(Index: TdxListIndex): TcxLookupGridCellViewInfo;
begin
...
function TcxLookupGridRowsViewInfo.GetItem(Index: TdxListIndex): TcxLookupGridRowViewInfo;
begin
...
constructor TdxLookupGridAccessibilityHelper.Create(AOwnerObject: TObject);
begin
  inherited;
  FIAccessibilityChildren := TInterfaceList.Create;
end;

destructor TdxLookupGridAccessibilityHelper.Destroy;
begin
  FreeAndNil(FIAccessibilityChildren);
  inherited;
end;

function TdxLookupGridAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  if AChildID > FirstSimpleChildIndex then
    Result := ViewInfo.Columns[GetColumnIndex(AChildID)].Bounds
  else
    Result := Grid.Bounds;

  Result := cxRectSetOrigin(Result, Grid.ClientToScreen(Result.TopLeft));
end;

function TdxLookupGridAccessibilityHelper.ChildIsSimpleElement(AIndex: Integer): Boolean;
begin
  Result := AIndex >= FirstSimpleChildIndex;
end;

function TdxLookupGridAccessibilityHelper.GetChild(AIndex: Integer): TcxAccessibilityHelper;
begin
  Result := IcxAccessibilityHelper(FIAccessibilityChildren[AIndex]).GetHelper;
end;

function TdxLookupGridAccessibilityHelper.GetChildCount: Integer;
begin
  ViewInfo.PopulateIAccessibilityChildren(FIAccessibilityChildren);
  Result := FIAccessibilityChildren.Count + ViewInfo.Columns.Count;
end;

function TdxLookupGridAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  if AChildID > 0 then
    Result := ViewInfo.Columns[GetColumnIndex(AChildID)].Text
  else
    Result := 'Grid';
end;

function TdxLookupGridAccessibilityHelper.GetOwnerObjectWindow: HWND;
begin
  Result := Grid.Handle;
end;

function TdxLookupGridAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  if AChildID = 0 then
    Result := cxROLE_SYSTEM_TABLE
  else
    Result := cxROLE_SYSTEM_COLUMNHEADER;
end;

function TdxLookupGridAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxSTATE_SYSTEM_FOCUSABLE;
end;

function TdxLookupGridAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := [aopFocus, aopLocation];
end;

function TdxLookupGridAccessibilityHelper.IsExtended: Boolean;
begin
  Result := True;
end;

procedure TdxLookupGridAccessibilityHelper.FocusRowChanged;
var
  I: Integer;
  ARowAccessibilityHelper: TdxLookupGridRowAccessibilityHelper;
begin
  ViewInfo.PopulateIAccessibilityChildren(FIAccessibilityChildren);
  for I := 0 to FIAccessibilityChildren.Count - 1 do
  begin
    ARowAccessibilityHelper := TdxLookupGridRowAccessibilityHelper(FIAccessibilityChildren[I]);
    if ARowAccessibilityHelper.IsSelected then
      if TdxLookupGridRowAccessibilityHelper(FIAccessibilityChildren[I]).ChildCount > 0 then
      begin
        NotifyWinEvent(EVENT_OBJECT_FOCUS, GetOwnerObjectWindow,
          TdxLookupGridCellAccessibilityHelper(ARowAccessibilityHelper.Childs[0]).FLocalId, CHILDID_SELF);
        Break;
      end;
  end;
end;

function TdxLookupGridAccessibilityHelper.GetGrid: TcxCustomLookupGrid;
begin
  Result := ViewInfo.Grid;
end;

function TdxLookupGridAccessibilityHelper.GetColumnIndex(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := AChildID - FirstSimpleChildIndex - 1;
end;

function TdxLookupGridAccessibilityHelper.GetFirstSimpleChildIndex: Integer;
begin
  Result := FIAccessibilityChildren.Count;
end;

function TdxLookupGridAccessibilityHelper.GetViewInfo: TcxLookupGridViewInfo;
begin
  Result := TcxLookupGridViewInfo(OwnerObject);
end;
...
begin
  ...
  begin
    LockUpdate(True);
    try
      DefaultValuesProvider := AColumn.DefaultValuesProvider;
    finally
      LockUpdate(False);
    end;
  end;
  Result := AColumnViewInfo.CreateEditViewData(AColumn.Properties, Grid.ScaleFactor);
end;
...
begin
  Result := Grid.Painter.LFPainterClass.DefaultGridLineColor;
end;
...
procedure TcxLookupGridViewInfo.PopulateIAccessibilityChildren(IAccessibilityChildren: TInterfaceList);
var
  I: Integer;
begin
  IAccessibilityChildren.Clear;
  ...
  begin
    if Rows[I].FIAccessibilityHelper = nil then
      Rows[I].FIAccessibilityHelper :=
        TdxLookupGridRowAccessibilityHelper.Create(Rows[I], TdxLookupGridAccessibilityHelper(IAccessibilityHelper.GetHelper));
    IAccessibilityChildren.Add(Rows[I].FIAccessibilityHelper);
  end;
end;
...
function TcxLookupGridViewInfo.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin
  if FIAccessibilityHelper = nil then
    FIAccessibilityHelper := TdxLookupGridAccessibilityHelper.Create(Self);
...
begin
  ...
  if ARowViewInfo.IsFocused and (Grid.Focused or Grid.IsPopupControl) and Grid.LookAndFeelPainter.SupportsNativeFocusRect then
...
function TcxCustomLookupGrid.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin
...
begin
  ...
  begin
    ...
    if dxIsGdiScaledMode and not IsDoubleBufferedNeeded then
      R := ClientBounds
    else
      R := ARowViewInfo.Bounds;
...
begin
  ...
  if HandleAllocated then
    TdxLookupGridAccessibilityHelper(IAccessibilityHelper.GetHelper).FocusRowChanged;
end;
...
procedure TcxCustomLookupGrid.ScrollTimerHandler(Sender: TObject);
  ...
  begin
    FocusedRowIndex := Max(Min(Value, RowCount - 1),  0);
    MakeFocusedRowVisible;
    SyncSelected(False);
  end;
...
procedure TcxCustomLookupGrid.WMGetObject(var Message: TMessage);
var
  AObjectID: Cardinal;
begin
