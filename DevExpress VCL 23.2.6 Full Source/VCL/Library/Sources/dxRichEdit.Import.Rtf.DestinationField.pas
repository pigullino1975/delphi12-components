...
unit dxRichEdit.Import.Rtf.DestinationField; // for internal use
...
uses
  ...
  dxRichEdit.DocumentModel.History.FieldHistory, dxCore;

const
  dxThisUnitName = 'dxRichEdit.Import.Rtf.DestinationField';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxFieldDestination.Initialize', SysInit.HInstance);{$ENDIF}
  FKeywordHT := CreateFieldKeywords;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxFieldDestination.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxFieldDestination.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxFieldDestination.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FKeywordHT);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxFieldDestination.Finalize', SysInit.HInstance);{$ENDIF}
end;
