...
type
  ...
  public
    constructor Create(AScheduler: TcxCustomScheduler; const AStart, AFinish: TDateTime;
      AExportType: Integer; AStream: TStream; AHandler: TObject = nil); virtual;
...
const
  dxThisUnitName = 'cxExportSchedulerLink';
...
var
  ...
  ASchedulerExport: TcxSchedulerExportHelper;
...
begin
  ...
  try
    ASchedulerExport := SchedulerExportHelpers[AsTable].Create(AScheduler, AStart, AFinish, AExportType, AFileStream, AHandler);
    try
      ASchedulerExport.FHeader := AHeader;
      if Supports(ASchedulerExport.Provider, IcxExportProviderEncoding, AEncodingIntf) then
        AEncodingIntf.SetEncoding(AEncoding);
      if Supports(ASchedulerExport.Provider, IcxExportWithSeparators, AIntf) and (Length(ASeparators) > 0) then
      begin
        for I := Low(ASeparators) to High(ASeparators) do
          AIntf.AddSeparator(ASeparators[I]);
      end;
      ASchedulerExport.DoExport;
    finally
      ASchedulerExport.Free;
    end;
  finally
    AFileStream.Free;
  end;
