...
uses
  ...
  Forms, Menus, ImgList, IniFiles, Contnrs, AppEvnts, MultiMon,
...
type
  ...
  private
    function GetItem(Index: TdxListIndex): TdxZone;
  public
    ...
    //
    property Items[Index: TdxListIndex]: TdxZone read GetItem; default;
  end;
  ...
  TdxCustomDockControlButton = class(TcxIUnknownObject,
  ...
  protected
    ...
    function GetDockingTargetControlAtPos(const pt: TPoint): TdxCustomDockControl; virtual;
  ...
  protected
    ...
    function GetMarginFar(Index: Integer): Integer; virtual; abstract;
    function GetMarginNear(Index: Integer): Integer; virtual; abstract;
    ...
    property MarginFar[Index: Integer]: Integer read GetMarginFar;
    property MarginNear[Index: Integer]: Integer read GetMarginNear;
  ...
  protected
    ...
    function GetMarginFar(Index: Integer): Integer; override;
    function GetMarginNear(Index: Integer): Integer; override;
  ...
  strict private
    function GetItem(Index: TdxListIndex): TdxDockSiteHideBarButtonSection;
  public
    function First: TdxDockSiteHideBarButtonSection;
    function Last: TdxDockSiteHideBarButtonSection;
    //
    property Items[Index: TdxListIndex]: TdxDockSiteHideBarButtonSection read GetItem; default; // for internal use
  end;
  ...
  public
    ...
    property DockControl: TdxCustomDockControl read FDockControl;
    property Sections: TdxDockSiteHideBarButtonSectionList read FSections; // for internal use
  end;
  ...
  strict private
    function GetItem(AIndex: TdxListIndex): TdxDockSiteHideBarButton;
  public
    ...
    procedure Scroll(dX, dY: Integer);
    //
    property Items[Index: TdxListIndex]: TdxDockSiteHideBarButton read GetItem; default;
  end;
  ...
  protected
    ...
    function GetCaptionAreaContentHeight: Integer; virtual;
    ...
    function GetHideBarButtonTextColor(AControl: TdxCustomDockControl): TColor; virtual;
    function GetHideBarButtonTextIndent: Integer; virtual;
...
uses
  ...
  dxSkinsdxDockControlPainter, Themes;

const
  dxThisUnitName = 'dxDockControl';
...
begin
  Result := cxSizeMax(GetCaptionButtonDefaultGlyphSize, dxGetImageSize(DockControl.CustomCaptionButtons.Images, ScaleFactor));
  Result := cxSizeAddMargins(Result, GetCaptionButtonBorderWidths);
...
procedure TdxDockControlPainter.DrawHideBarButtonSection(ACanvas: TcxCanvas;
  ...
  begin
    ...
    begin
      AImageSize := GetDefaultImageSize(APosition);
      if APosition in [ahpLeft, ahpRight] then
      begin
        Inc(R.Top, GetHideBarHorizInterval);
        AImageRect := cxRectSetHeight(R, AImageSize);
        R.Top := AImageRect.Bottom;
      end
      else
      begin
        Inc(R.Left, GetHideBarHorizInterval);
...
begin
  ...
  begin
    TdxImageDrawer.DrawImage(ACanvas, cxRectCenter(ARect, GetImageWidth, GetImageHeight), nil, AControl.Images,
...
procedure TdxDockControlPainter.DrawHideBarButtonText(ACanvas: TcxCanvas;
  ...
  procedure DrawTextCore(ACanvas: TCanvas; R: TRect);
  begin
    ACanvas.Brush.Style := bsClear;
    ACanvas.Font := GetHideBarButtonFont;
    ACanvas.Font.Color := cxGetActualColor(GetHideBarButtonTextColor(AControl), ACanvas.Font.Color);
    cxDrawText(ACanvas.Handle, AControl.Caption, R, DT_SINGLELINE or DT_CENTER or DT_VCENTER or DT_END_ELLIPSIS);
  end;

var
  ABitmap: TcxBitmap;
begin
  ARect := cxRectInflate(ARect, -GetHideBarButtonTextIndent);
  case APosition of
    ahpTop, ahpBottom:
      DrawTextCore(ACanvas.Canvas, ARect);
    ...
    ahpLeft, ahpRight:
      begin
        ...
        try
          ...
          DrawTextCore(ABitmap.Canvas, ABitmap.ClientRect);
...
function TdxDockControlPainter.GetCaptionAreaContentHeight: Integer;
begin
  Result := GetCaptionButtonSize.cy;
end;

function TdxDockControlPainter.GetCaptionAreaHeight: Integer;
begin
  Result := GetCaptionAreaContentHeight + cxMarginsHeight(GetCaptionContentOffsets(DockControl.IsCaptionVertical));
  Result := Max(Result, GetCaptionHeight);
end;
...
function TdxDockControlPainter.GetHideBarButtonTextColor(AControl: TdxCustomDockControl): TColor;
begin
  Result := clDefault;
end;

function TdxDockControlPainter.GetHideBarButtonTextIndent: Integer;
begin
  Result := ScaleFactor.Apply(2);
end;
...
procedure ShowMovementTimerProc(Wnd: HWnd; Msg: UINT; TimerID: UINT_PTR; SysTime: DWORD); stdcall;
var
  AControl: TdxDockSite;
begin
  AControl := FindControl(Wnd) as TdxDockSite;
...
procedure TdxDockSite.DoShowMovement;
  ...
  begin
    dxKillTimer(WindowHandle, FMovingTimerID);
    FMovingControlHideBar := nil;
...
begin
  ...
  if MovingControlHideBar.CheckShowingFinish then
    ...
    if FMovingTimerID = 0 then
      FMovingTimerID := dxSetTimer(Handle, 1, ControllerAutoHideMovingInterval, ShowMovementTimerProc);
end;

procedure HideMovementTimerProc(Wnd: HWnd; Msg: UINT; TimerID: UINT_PTR; SysTime: DWORD); stdcall;
var
  AControl: TdxDockSite;
begin
  AControl := FindControl(Wnd) as TdxDockSite;
...
begin
  ...
  if MovingControlHideBar.CheckHidingFinish then
    ...
    if FMovingTimerID = 0 then
      FMovingTimerID := dxSetTimer(Handle, 1, ControllerAutoHideMovingInterval, HideMovementTimerProc);
end;
...
procedure AutoHideTimerProc(Wnd: HWnd; Msg: UINT; TimerID: UINT_PTR; SysTime: DWORD); stdcall;
var
  AControl: TdxDockSite;
begin
  AControl := FindControl(Wnd) as TdxDockSite;
...
begin
  dxKillTimer(WindowHandle, FHidingTimerID);
  if not IsDestroying and (FHidingTimerID = 0) and (ShowingControl <> nil) then
    FHidingTimerID := dxSetTimer(Handle, 2, ControllerAutoHideInterval, AutoHideTimerProc)
end;

procedure AutoShowTimerProc(Wnd: HWnd; Msg: UINT; TimerID: UINT_PTR; SysTime: DWORD); stdcall;
var
  AControl: TdxDockSite;
begin
  AControl := FindControl(Wnd) as TdxDockSite;
...
begin
  if not IsDestroying and (FShowingTimerID = 0) then
    FShowingTimerID := dxSetTimer(Handle, 3, ControllerAutoShowInterval, AutoShowTimerProc)
end;
...
begin
  dxKillTimer(WindowHandle, FShowingTimerID);
  ASubControl := nil;
  AControl := GetControlAtPos(ScreenToClient(GetMouseCursorPos), ASubControl);
...
begin
  ...
  case AMsg.message of
    ...
    WM_SETFOCUS:
      if (dxDockingController.FActiveDockControlLockCount = 0) and Application.Active then
      begin
        AActiveDockControl := dxDockingController.GetDockControlForWindow(AMsg.hwnd);
        if AActiveDockControl <> nil then
        begin
          if AActiveDockControl.HandleAllocated and (AActiveDockControl <> dxDockingController.FActivatingDockControl) then
            dxDockingController.ActiveDockControl := AActiveDockControl;
        end
        else
          if (AControl is TCustomForm) and IsMDIForm(TCustomForm(AControl)) then 
    ...
    WM_KILLFOCUS:
      ...
      begin
        ...
        if (AParentDockControl <> nil) and
           IsWindowEnabledEx(AParentDockControl.Handle) and 
...
var
  AManager: TdxDockingManager;
begin
  if FindManager(AForm, AManager)
    and (AManager.Images <> nil) and not (csDestroying in AManager.Images.ComponentState) then
...
begin
  ...
  try
    ...
    begin
      ...
      if ControlNeedUpdate(ADockControl, AForm) then
      begin
        if ADockControl.ManagerFont and not ADockControl.IsDestroying then
          ADockControl.AssignManagerFont;
      end;
    end;
    CalculateControls(AForm);
  finally
...
begin
  ...
  begin
    ...
    try
      APrevLayoutMode := SetLayout(ADC, LAYOUT_LTR);
      try
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if IsParentForFloatDockSite(AParentForm, AControl) and (AControl.FloatForm <> nil) then
...
function TdxDockSiteHideBarButtonsList.GetItem(AIndex: TdxListIndex): TdxDockSiteHideBarButton;
begin
...
function TdxDockSiteHideBarButtonSectionList.GetItem(Index: TdxListIndex): TdxDockSiteHideBarButtonSection;
begin
...
procedure TdxCustomDockControlButton.MouseLeave;
begin
...
procedure TdxCustomDockControlButton.UpdateHotState;
var
  ABounds: TRect;
begin
  ABounds.TopLeft := DockControl.WindowToClient(Bounds.TopLeft);
  ABounds.BottomRight := DockControl.WindowToClient(Bounds.BottomRight);
  BeginMouseTracking(Controller.DockControl, ABounds, Self);
end;
...
begin
  ...
  TdxImageDrawer.DrawImage(ACanvas, Bounds, nil, Images, Button.ImageIndex,
...
begin
  Assert(AOwner <> nil, sdxInvalidZoneOwner);
  FOwner := AOwner;
  FWidth := AWidth;
  FKind := AKind;
end;
...
function TdxFloatDockSite.IsFontStored: Boolean;
begin
  Result := True;
end;
...
begin
  ...
  {$IFDEF DELPHI110}
    FloatForm.PixelsPerInch := TCustomForm(Owner).PixelsPerInch;
  {$ENDIF}
...
procedure TdxSideContainerDockSite.AdjustChildrenBoundsComplexMode(AControl: TdxCustomDockControl);
  ...
  function GetActualContainerSize: Integer;
  var
    I: Integer;
  begin
    ...
    begin
      if IsValidChild(Children[I]) then
      begin
        Dec(Result, MarginFar[I]);
        Dec(Result, MarginNear[I]);
      end;
    end;
  end;
...
begin
  ...
  AContainerSize := GetActualContainerSize;
...
function TdxZoneList.GetItem(Index: TdxListIndex): TdxZone;
begin
...
function TdxVertContainerDockSite.GetMarginFar(Index: Integer): Integer;
var
  AChild: TdxCustomDockControl;
begin
  AChild := Children[Index];
  if AChild.AlignWithMargins then
    Result := AChild.Margins.Bottom
  else
    Result := 0;
end;

function TdxVertContainerDockSite.GetMarginNear(Index: Integer): Integer;
var
  AChild: TdxCustomDockControl;
begin
  AChild := Children[Index];
  if AChild.AlignWithMargins then
    Result := AChild.Margins.Top
  else
    Result := 0;
end;
...
function TdxHorizContainerDockSite.GetMarginFar(Index: Integer): Integer;
var
  AChild: TdxCustomDockControl;
begin
  AChild := Children[Index];
  if AChild.AlignWithMargins then
    Result := AChild.Margins.Right
  else
    Result := 0;
end;

function TdxHorizContainerDockSite.GetMarginNear(Index: Integer): Integer;
var
  AChild: TdxCustomDockControl;
begin
  AChild := Children[Index];
  if AChild.AlignWithMargins then
    Result := AChild.Margins.Left
  else
    Result := 0;
end;
...
begin
  ASideContainer := SideContainer;
  if (ASideContainer <> nil) and (SideContainerItem = Self) and ASideContainer.IsValidChild(Self) and not (dcisInternalResizing in FInternalState) then
  begin
    Include(FInternalState, dcisInternalResizing);
    try
      if IsAlignDisabled or IsUpdateLayoutLocked or ASideContainer.IsAdjustBoundsLocked or ASideContainer.IsUpdateLayoutLocked then
      begin
        ASideContainer.SetChildBounds(Self, AWidth, AHeight);
        DoSetBounds;
      end
      else
      begin
        ASideContainer.DoChildResize(Self,
          (ASideContainer.GetDimension(AWidth - Width, AHeight - Height)),
          (ASideContainer.GetDimension(Left - ALeft, Top - ATop) = 0) and Assigned(ASideContainer.GetNextValidChild(DockIndex)));
      end;
    finally
...
begin
  ...
  try
    ...
    begin
      ...
      AWidth := ASite.FloatForm.ScaleFactor.Apply(OriginalWidth, ScaleFactor);
      AHeight := ASite.FloatForm.ScaleFactor.Apply(OriginalHeight, ScaleFactor);
...
begin
  ...
  if ParentDockControl <> nil then
  begin
    FStoredPosition.OriginalHeight := ParentDockControl.ScaleFactor.Apply(OriginalHeight, ParentDockControl.ScaleFactor);
    FStoredPosition.OriginalWidth := ParentDockControl.ScaleFactor.Apply(OriginalWidth, ParentDockControl.ScaleFactor);
  end
  else
  begin
...
begin
  ...
  begin
    ...
    case AParentSite.Align of
      alLeft, alRight:
        FOriginalWidth := AParentSite.ScaleFactor.Apply(FOriginalWidth, dxSystemScaleFactor);
      alTop, alBottom:
        FOriginalHeight := AParentSite.ScaleFactor.Apply(FOriginalHeight, dxSystemScaleFactor);
      alClient: ;
    else
      FOriginalWidth := AParentSite.ScaleFactor.Apply(FOriginalWidth, dxSystemScaleFactor);
      FOriginalHeight := AParentSite.ScaleFactor.Apply(FOriginalHeight, dxSystemScaleFactor);
...
procedure TdxCustomDockControl.ConstrainedResize(var MinWidth, MinHeight, MaxWidth, MaxHeight: Integer);
begin
...
begin
  ...
  begin
    if IsOwner(PopupParent.Handle, WindowHandle) then
...
begin
  ...
  begin
    ...
    begin
      BeginUpdateNC(ResizingStyle <> drsUpdate);
      try
        ParentDockControl.NCChanged;
        ResizingSourceZone.DoResize(AStartPoint, AFinishPoint);
      finally
...
begin
  FOriginalWidth := ScaleFactor.Apply(AControl.OriginalWidth, AControl.ScaleFactor);
  FOriginalHeight := ScaleFactor.Apply(AControl.OriginalHeight, AControl.ScaleFactor);
  SetSize(FOriginalWidth, FOriginalHeight);
end;
...
procedure TdxCustomDockControl.AssignManagerFont;
var
  AManager: TdxDockingManager;
begin
  if Controller.FindManager(ParentForm, AManager) then
    dxAssignFont(Font, AManager.Font, ScaleFactor, AManager.ScaleFactor)
  else
    dxAssignFont(Font, Controller.FFont, ScaleFactor, dxSystemScaleFactor);
...
begin
  if FNeedDrawBackground then
  begin
    DoDrawClient(Canvas, ClientRect);
    FNeedDrawBackground := False;
  end;
  DrawDesignRect;
end;
...
begin
  if HandleAllocated and not IsInternalDestroying then
  begin
    AFlags := RDW_FRAME or RDW_INVALIDATE;
    if AWithChildren then
      AFlags := AFlags or RDW_ALLCHILDREN or RDW_ERASE;
    if Controller.IsResizing and (Controller.ResizingDockControl.ResizingStyle = drsUpdate) then
...
procedure TdxCustomDockControl.WMPaint(var Message: TWMPaint);
begin
  FNeedDrawBackground := Message.DC = 0;
  inherited;
end;
...
begin
   if Handle <> WindowFromDC(Message.DC) then
   begin
    cxPaintCanvas.BeginPaint(Message.DC);
    try
...
begin
  ...
  begin
    AZone := GetResizeZoneAtPos(GetMouseCursorPos);
    if AZone <> nil then
    begin
      case AZone.Direction of
        zdHorizontal:
...
begin
  ...
  begin
    ...
    {$IFDEF DELPHI110}
      ...
      begin
        FDesignPPI := Designer.DesignPPI(Self);
        ScaleForPPI(FDesignPPI);
      end;
    {$ENDIF}
    end
    else
...
begin
  ...
  dxFreeGlobalObject(FDockingPaintersManager);
  dxFreeGlobalObject(FDockingController);
end;
