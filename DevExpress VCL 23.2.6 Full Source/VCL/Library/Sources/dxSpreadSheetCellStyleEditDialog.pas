...
{$ENDIF}
  ...
  dxSpreadSheetCoreStyles, cxCustomListBox;
...
type
  ...
  protected
    ...
    procedure UpdateBordersPreview;
...
const
  dxThisUnitName = 'dxSpreadSheetCellStyleEditDialog';
...
begin
  ...
  FontLoader.PopulateFonts(Self, FontLoaderCompleteHandler, lbFontNames.Items, [cxftTTF]);
...
begin
  ...
  try
    ...
    lciMergeCells.Enabled := Controller.CanMergeAreas and (Controller.HasMultipleCellsAreaAtHorz or Controller.HasMultipleCellsAreaAtVert);
...
begin
  ...
  TdxColorGalleryAccess(cgFill).ShowGroupCaption := False;
...
procedure TdxSpreadSheetCellStyleEditDialogForm.UpdateBordersPreview;
begin
  BordersPreview.BiDiMode := BidiMode;
...
procedure TdxSpreadSheetCellStyleEditDialogForm.FontLoaderCompleteHandler(Sender: TObject);
begin
...
begin
  ...
  if (AIndex = AControl.ItemIndex) and (not AControl.Focused) and RootLookAndFeel.Painter.SupportsNativeFocusRect then
