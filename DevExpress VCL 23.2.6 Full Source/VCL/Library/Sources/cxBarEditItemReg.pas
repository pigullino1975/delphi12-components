...
uses
  DesignConst, DesignEditors, DesignIntf, Variants, SysUtils,
  Classes, TypInfo, cxBarEditItem, cxBarEditItemValueEditor, cxEdit,
  cxPropEditors, dxBarStrs, dxCore, cxClasses, cxDropDownEdit, dxBar;

const
  dxThisUnitName = 'cxBarEditItemReg';
...
type
  ...
  { TdxBarItemPropertyFilter }

  TdxBarItemPropertyFilter = class(TSelectionEditor, ISelectionPropertyFilter)
  protected
  ...
  TcxBarEditItemSelectionEditor = class(TdxBarItemPropertyFilter)
  private
    FProc: TGetStrProc;
...
procedure TcxBarEditItemSelectionEditor.RequiresUnits(Proc: TGetStrProc);
begin
  FProc := Proc;
...
procedure TdxBarItemPropertyFilter.FilterProperties(
  const ASelection: IDesignerSelections; const ASelectionProperties: IInterfaceList);

  function IndexOfItemIndexProperty: Integer;
  var
    AProperty: IProperty;
  begin
    for Result := 0 to ASelectionProperties.Count - 1 do
    begin
      AProperty := ASelectionProperties[Result] as IProperty;
      if (AProperty <> nil) and (AProperty.GetName = 'ItemIndex') then
        Exit;
    end;
    Result := -1;
  end;
...
begin
  ...
  begin
    if (ASelection[I] is TcxCustomBarEditItem) then
    begin
      if Supports(TcxCustomBarEditItem(ASelection[I]).Properties, IcxValueByItemIndexProvider, AProvider) and
        AProvider.SupportsItemIndex then
          Continue;
      AIndex := IndexOfItemIndexProperty;
      if AIndex >= 0 then
        ASelectionProperties.Delete(AIndex);
      Break;
    end;
  end;
end;
...
begin
  ...
  RegisterSelectionEditor(TdxBarItem, TdxBarItemPropertyFilter);
