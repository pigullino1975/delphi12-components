...
type
  TdxHookType = (htKeyboard, htMouse, htWndProc, htGetMessage, htCBT, htWndProcRet, htMouseLL);
...
function dxSystemMouseLLHook(Code: Integer; wParam: WParam; lParam: LParam): LRESULT; stdcall;
begin
  Result := dxHookController.FSystemHooks[htMouseLL].ProcessHookProcs(Code, wParam, lParam);
end;
...
const
  HooksData: TdxSystemHooksData = (
    (Proc: dxSystemKeyboardHook;   ID: WH_KEYBOARD;       ThreadID: -1), // htKeyboard
    (Proc: dxSystemMouseHook;      ID: WH_MOUSE;          ThreadID: -1), // htMouse
    (Proc: dxSystemWndProcHook;    ID: WH_CALLWNDPROC;    ThreadID: -1), // htWndProc
    (Proc: dxSystemGetMessageHook; ID: WH_GETMESSAGE;     ThreadID: -1), // htGetMessage
    (Proc: dxSystemCBTHook;        ID: WH_CBT;            ThreadID: -1), // htWCBT
    (Proc: dxSystemWndProcRetHook; ID: WH_CALLWNDPROCRET; ThreadID: -1), // htWndProcRet
    (Proc: dxSystemMouseLLHook;    ID: WH_MOUSE_LL;       ThreadID: 0)); // htMouseLL
...
begin
  Assert(FHandle = 0);
  AThreadID := HooksData[FHookType].ThreadID;
  if AThreadID = -1 then
    AThreadID := GetCurrentThreadId;
  FHandle := SetWindowsHookEx(HooksData[FHookType].ID, HooksData[FHookType].Proc, 0, AThreadID);
end;
...
finalization
  ...
  dxFreeGlobalObject(FdxHookController);
