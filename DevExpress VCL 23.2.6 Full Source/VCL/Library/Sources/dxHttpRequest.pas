...
const
  dxThisUnitName = 'dxHttpRequest';

{ TdxHttpRequest }

class constructor TdxHttpRequest.Initialize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxHttpRequest.Initialize', SysInit.HInstance);{$ENDIF}
  FHandlers := TList<TdxHttpRequestHandlerClass>.Create;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxHttpRequest.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxHttpRequest.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxHttpRequest.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FHandlers);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxHttpRequest.Finalize', SysInit.HInstance);{$ENDIF}
end;
