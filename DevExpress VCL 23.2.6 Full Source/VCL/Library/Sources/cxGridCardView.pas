...
uses
  Types, Windows, Variants, Classes, Graphics, Controls, ImgList, Forms, StdCtrls,
  ...
  cxDataStorage, cxCustomData, cxGridCustomLayoutView, dxCoreClasses, cxListBox;
...
type
  ...
  TcxGridCardExpandButtonPainter = class(TcxGridCustomLayoutRecordExpandButtonPainter)
  private
  ...
  TcxGridCardPainter = class(TcxGridCustomLayoutRecordPainter)
  private
  ...
  protected
    ...
    class function GetCellHeight(ATextHeight: Integer;
      ALookAndFeelPainter: TcxCustomLookAndFeelPainter; AScaleFactor, ABordersScaleFator: TdxScaleFactor): Integer; override;
  ...
  TcxGridCardExpandButtonViewInfo = class(TcxGridCustomLayoutRecordExpandButtonViewInfo)
  strict private
    function GetRecordViewInfo: TcxGridCardViewInfo;
  protected
    function GetPainterClass: TcxCustomGridCellPainterClass; override;
  ...
  private
    ...
    function GetExpandButtonViewInfo: TcxGridCardExpandButtonViewInfo;
  ...
  public
    ...
    property ExpandButtonViewInfo: TcxGridCardExpandButtonViewInfo read GetExpandButtonViewInfo;
    property GridView: TcxGridCardView read GetGridView;
    property GridRecord: TcxGridCard read GetGridRecord;
  ...
  private
    ...
    function GetRowFilteredItemsListShowFilteredItemsOnly: Boolean;
    ...
    procedure SetRowFilteredItemsListShowFilteredItemsOnly(AValue: Boolean);
  ...
  published
    ...
    property RowFilteredItemsListShowFilteredItemsOnly: Boolean read GetRowFilteredItemsListShowFilteredItemsOnly
      write SetRowFilteredItemsListShowFilteredItemsOnly default True;
...
uses
  SysUtils, Math, cxScrollBar, cxGridStrs, dxCoreGraphics, cxLookAndFeels, dxTypeHelpers;

const
  dxThisUnitName = 'cxGridCardView';
...
procedure TcxGridCardRowsListBox.UpdateItemHeight;
begin
  ItemHeight := TcxGridCardRowCaptionViewInfo.GetCellHeight(TdxTextMeasurer.TextLineHeight(Font), LookAndFeelPainter,
    ScaleFactor, GridView.BordersScaleFactor);
end;
...
begin
  ViewInfo.LookAndFeelPainter.DrawScaledGroupExpandButton(Canvas,
    ViewInfo.Bounds, ViewInfo.RecordViewInfo.Expanded, ViewInfo.ButtonState, ScaleFactor);
end;
...
begin
  AScaleFactor := ARow.GridView.ScaleFactor;
  ADefaultCaptionHeight := GetCellHeight(ACanvas.FontHeight(AFont), ARow.GridView.LookAndFeelPainter,
    AScaleFactor, ARow.GridView.BordersScaleFactor);
  dxAdjustToTouchableSize(ADefaultCaptionHeight, AScaleFactor);
...
class function TcxGridCardRowCaptionViewInfo.GetCellHeight(ATextHeight: Integer;
  ALookAndFeelPainter: TcxCustomLookAndFeelPainter; AScaleFactor, ABordersScaleFator: TdxScaleFactor): Integer;
begin
  Result := ATextHeight;
  Inc(Result, 2 * AScaleFactor.Apply(cxGridCellTextOffset));
  Inc(Result, TdxVisualRefinements.HeaderPadding.ScaledHeight(AScaleFactor));
end;
...
function TcxGridCardRowCaptionViewInfo.GetTextWidthWithOffset: Integer;
begin
  Result := TextWidth;
  Inc(Result, 2 * ScaleFactor.Apply(cxGridCellTextOffset));
  Inc(Result, TdxVisualRefinements.HeaderPadding.ScaledWidth(ScaleFactor));
end;

function TcxGridCardRowCaptionViewInfo.GetTextAreaBounds: TRect;
begin
  Result := inherited GetTextAreaBounds;
  TdxVisualRefinements.HeaderPadding.Deflate(Result, ScaleFactor);
  if (FilterButtonViewInfo <> nil) and FilterButtonViewInfo.OccupiesSpace then
...
var
  R1, R2: TRect;
begin
  inherited GetEditViewDataContentOffsets(R);
...
function TcxGridCardExpandButtonViewInfo.GetRecordViewInfo: TcxGridCardViewInfo;
begin
  Result := TcxGridCardViewInfo(inherited RecordViewInfo);
end;
...
function TcxGridCardViewInfo.GetExpandButtonViewInfo: TcxGridCardExpandButtonViewInfo;
begin
  Result := TcxGridCardExpandButtonViewInfo(inherited ExpandButtonViewInfo);
end;
...
function TcxGridCardViewInfo.GetExpandButtonOffset: Integer;
begin
  Result := CardExpandButtonOffset;
end;
...
function TcxGridCardViewInfo.GetExpandButtonViewInfoClass: TcxGridCustomLayoutViewRecordExpandButtonViewInfoClass;
begin
...
begin
  ...
  begin
    ...
    if Result = nil then
    begin
      Result := ExpandButtonViewInfo.GetHitTest(P);
      if Result = nil then
      begin
        Result := inherited GetHitTest(P);
        if Result <> nil then
...
function TcxGridCardViewFiltering.GetRowFilteredItemsListShowFilteredItemsOnly: Boolean;
begin
...
procedure TcxGridCardViewFiltering.SetRowFilteredItemsListShowFilteredItemsOnly(AValue: Boolean);
begin
