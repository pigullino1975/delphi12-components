...
uses
  ...
  dxPSUtl, dxPSRes, dxPSAutoHFTextMnuBld, dxPSfmAutoHFTextFmt, dxBase, dxPrintingStrs,
  dxPSHFLibrary, Math, cxFormats, dxMessageDialog;

const
  dxThisUnitName = 'dxPgsDlg';
...
begin
  ADialog := dxCreateMessageDialog(Message, mtWarning, mbYesNoCancel);
  try
    ADialog.FindButton(mbYes).Caption := cxGetResourceString(@sdxBtnFix);
    ADialog.FindButton(mbNo).Caption := cxGetResourceString(@sdxBtnRestoreOriginal);
    ADialog.FindButton(mbCancel).Caption := cxGetResourceString(@sdxBtnClose);
...
var
  ADialog: TdxMessageDialogForm;
begin
  ADialog := dxCreateMessageDialog(Message, mtWarning, [mbYes, mbIgnore]);
  try
    ADialog.FindButton(mbYes).Caption := cxGetResourceString(@sdxBtnFix);
    ADialog.AlignButtons;
    Result := ADialog.ShowModal;
...
begin
  ...
  TdxImageDrawer.DrawUncachedImage(AListBox.Canvas.Handle, AImageRect, AImageRect, AStyle.StyleGlyph, AImages, AStyle.ImageIndex, idmNormal, True);
...
procedure TdxfmPageSetupDialog.SetupDialog(const APageSetupDlgData: TdxPageSetupDlgData);
  ...
  begin
    ...
    pnlStyleName.Enabled := pnlStyleName.Visible and (not FSavePrintStyle.BuiltIn or IsDesignTime);
...
procedure TdxfmPageSetupDialog.DrawBackgroundBitmapContent(ACanvas: TcxCanvas; R: TRect; ABackground: TdxBackground);
  ...
  procedure DrawText(ACanvas: TcxCanvas; const S: string; const R: TRect; APainter: TcxCustomLookAndFeelPainter);
  begin
    ACanvas.FillRect(R, APainter.DefaultContentColor);
    ACanvas.Brush.Style := bsClear;
    ACanvas.Font.Color := APainter.ButtonSymbolColor(cxbsDisabled);
    ACanvas.DrawTexT(S, R, DT_SINGLELINE or DT_CENTER or DT_VCENTER);
  end;

  procedure DrawByBrush(ACanvas: TcxCanvas; B: HBRUSH; R: TRect; APainter: TcxCustomLookAndFeelPainter);
  begin
    ACanvas.FrameRect(R, APainter.GetContainerBorderColor(False));
    InflateRect(R, -1, -1);
    FillRect(ACanvas.Handle, R, B);
    DeleteObject(B);
  end;

  procedure DrawBackground(ACanvas: TcxCanvas; R: TRect; APainter: TcxCustomLookAndFeelPainter);
  var
    PrevColor: TColor;
  begin
    ACanvas.FrameRect(R, APainter.DefaultContentColor);
    InflateRect(R, -1, -1);
    case ABackground.Mode of
      bmNone:
        DrawText(ACanvas, '[' + DropAmpersand(cxGetResourceString(@sdxBtnNoFill)) + ']', R, APainter);

      bmPicture:
        DrawText(ACanvas, '[' + DropAmpersand(cxGetResourceString(@sdxPicture)) + ']', R, APainter);

      bmBrushBitmap:
        DrawByBrush(ACanvas, CreatePatternBrush(TBitmap(ABackground.Picture).Handle), R, APainter);

      bmBrush:
        begin
          PrevColor := SetBkColor(ACanvas.Handle, ColorToRGB(ABackground.BkColor));
          DrawByBrush(ACanvas, CreateSolidBrush(ColorToRGB(ABackground.Brush.Color)), R, APainter);
...
var
  ...
  ARect: TRect;
begin
  APainter := dxPSEngine.DialogsLookAndFeel.Painter;
  ...
  try
    ...
    DrawBackground(ACanvas, ARect, APainter);
    ...
    APainter.DrawArrow(ACanvas, ARect, adDown, APainter.ButtonSymbolColor(cxbsNormal));
  finally
...
begin
  TcxCustomInnerListBoxAccess(lbxPaperType.InnerListBox).DrawItemBackground(AIndex, ARect, AState);
  InflateRect(ARect, -ScaleFactor.Apply(2), -ScaleFactor.Apply(1));
  TdxImageDrawer.DrawImage(ACanvas, cxRectSetWidth(ARect, ScaleFactor.Apply(ilPaperTypes.Width)), nil, ilPapers,
  ...
  TcxCustomInnerListBoxAccess(lbxPaperType.InnerListBox).DrawItemContent(AIndex, ARect, AState);
end;
...
begin
  ...
  TdxImageDrawer.DrawImage(ACanvas, cxRectSetWidth(R, ScaleFactor.Apply(ilBins.Width)), nil, ilBins,
