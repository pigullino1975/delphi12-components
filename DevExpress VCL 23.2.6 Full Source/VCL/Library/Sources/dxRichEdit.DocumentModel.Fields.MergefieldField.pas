...
unit dxRichEdit.DocumentModel.Fields.MergefieldField; // for internal use
...
const
  dxThisUnitName = 'dxRichEdit.DocumentModel.Fields.MergefieldField';

{ TdxMergefieldField }

class constructor TdxMergefieldField.Initialize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxMergefieldField.Initialize', SysInit.HInstance);{$ENDIF}
  FSwitchesWithArgument := CreateSwitchesWithArgument(['b', 'f']);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxMergefieldField.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxMergefieldField.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxMergefieldField.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FSwitchesWithArgument);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxMergefieldField.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  ...
  if AValue.IsEmpty or (not AValue.IsObject and (VarIsNull(AValue.AsVariant) or VarIsEmpty(AValue.AsVariant))) then
    Exit(GetNullValue);
