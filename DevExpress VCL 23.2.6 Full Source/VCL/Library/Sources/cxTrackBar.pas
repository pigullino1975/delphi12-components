...
uses
  Types, Variants, Windows, Classes,
  Controls, Forms, Graphics, Messages, SysUtils, Math, Generics.Defaults, Generics.Collections,
  dxCore, dxCoreClasses, cxClasses, cxVariants, cxGraphics, cxLookAndFeels, cxLookAndFeelPainters,
  cxControls, cxContainer, cxEdit, cxTextEdit, cxExtEditConsts, dxCustomHint,
  cxFilterControlUtils, cxAccessibility;

const
  dxTrackBarDefaultThumbHeight = 11;
  dxTrackBarDefaultThumbWidth = 6;
  dxTrackBarDefaultTrackSize = 5;
  dxTrackBarAutoThumbHeight = 20;
  dxTrackBarAutoThumbWidth = 10;
  dxTrackBarAutoTrackSize = 10;
...
type
  ...
  IcxTrackBarPostProvider = interface //for internal use only
  ['{F61E036A-10F7-486E-9A28-9B22084626AA}']
    procedure PostPosition;
  end;
  ...
  public
    ...
    //
    property ViewInfo: TcxCustomTrackBarViewInfo read FViewInfo; // for internal use
  end;
  ...
  protected
    ...
    function GetTextTopRect(ATickPosition, ATextWidth, ATextHeight, AOffset: Integer): TRect; virtual;
    function GetTextBottomRect(ATickPosition, ATextWidth, ATextHeight, AOffset: Integer): TRect; virtual;
  ...
  protected
    ...
    function GetTextTopRect(ATickPosition, ATextWidth, ATextHeight, AOffset: Integer): TRect; override;
    function GetTextBottomRect(ATickPosition, ATextWidth, ATextHeight, AOffset: Integer): TRect; override;
  ...
  public
    ...
    property FadingHelper: TcxTrackBarThumbFadingHelper read FFadingHelper; // for internal use
  end;
  ...
  protected
    ...
    function GetCalculatedPosition(APosition: Integer; AIsInc: Boolean; AIsLineSize: Boolean;
      ANeedRound: Boolean = False): Integer; virtual;
    ...
    function IsTextOrientationEqualTrackOrientation: Boolean;
    ...
    function GetTextBottomRect(ATickPosition, ATextWidth, ATextHeight, AOffset: Integer): TRect;
    function GetTextTopRect(ATickPosition, ATextWidth, ATextHeight, AOffset: Integer): TRect;
  ...
  public
    ...
    PixelsPerPositionStep: TdxDouble; // for internal use
    ...
    property ButtonLeftFadingHelper: TcxTrackBarLeftButtonFadingHelper read FButtonLeftFadingHelper; // for internal use
    property ButtonLeftState: TcxButtonState read FButtonLeftState write SetButtonLeftState;
    property ButtonRightFadingHelper: TcxTrackBarRightButtonFadingHelper read FButtonRightFadingHelper; // for internal use
    property ButtonRightState: TcxButtonState read FButtonRightState write SetButtonRightState;
    property HotThumb: TcxTrackBarThumbViewInfo read FHotThumb write FHotThumb; // for internal use
    ...
    property OrientationHelper: TcxHorizontalTrackBarViewInfoHelper read FOrientationHelper; // for internal use
    property Position: Integer read GetPosition write SetPosition;
    property PositionHintHelper: TcxTrackBarPositionHintHelper read FPositionHintHelper; // for internal use
    property PressedThumb: TcxTrackBarThumbViewInfo read FPressedThumb write FPressedThumb; // for internal use
  ...
  private
    procedure FitRectToBounds(var ARect: TRect; const ABounds: TRect);
    procedure GetOnGetThumbRect(out AValue: TcxGetThumbRectEvent);
    function GetProperties: TcxCustomTrackBarProperties;
  protected
    FAlwaysShowTickArea: Boolean;
    FCustomThumbRect: TRect;
    FTopLeftTickAreaSize: Integer;
    FBottomRightTickAreaSize: Integer;
    FTrackZoneSize: Integer;
    ...
    procedure CalculateCustomTrackBarRects(ACanvas: TcxCanvas; AViewInfo: TcxCustomTrackBarViewInfo); virtual;
    ...
    function CalculateTickAreaSize(ACanvas: TcxCanvas; AViewInfo: TcxCustomTrackBarViewInfo; ALeftTop: Boolean): Integer; virtual;
    ...
    procedure CalculateTrackZoneRect(ACanvas: TcxCanvas; AViewInfo: TcxCustomTrackBarViewInfo); virtual;
    procedure DoOnGetThumbRect(var ARect: TRect);
    function GetRectOutOfBoundsDelta(const ARect, ABounds: TRect): Integer;
    ...
    function TryScaleThumbAndTrack(AViewInfo: TcxCustomTrackBarViewInfo): Boolean; virtual;
  ...
  public
    ...
    function IsPaddingAllowedForInplace: Boolean; override;
    function IsPaddingAllowedForStandalone: Boolean; override;
    function IsTopLeftAlignment: Boolean;
    function IsBottomRightAlignment: Boolean;
  ...
  protected
    ...
    function CreateAlignment: TcxEditAlignment; override;
  ...
  public
    ...
    property ThumbHeight: Integer read FThumbHeight write SetThumbHeight default dxTrackBarDefaultThumbHeight;
    ...
    property ThumbWidth: Integer read FThumbWidth write SetThumbWidth default dxTrackBarDefaultThumbWidth;
    ...
    property TrackSize: Integer read FTrackSize write SetTrackSize default dxTrackBarDefaultTrackSize;
  ...
  protected
    ...
    procedure StopSliding;

    property StartSlidingPosition: Integer read FStartSlidingPosition;
  ...
  { TdxTrackBarRangeValueProvider }

  TdxTrackBarRangeValueProvider = class(TdxRangeValueProvider) // for internal use
  private
    FTrackBar: TcxCustomTrackBar;
  protected
    function GetIsReadOnly: Boolean; override;
    function GetMaximum: Double; override;
    function GetMinimum: Double; override;
    function GetSmallChange: Double; override;
    function GetValue: Double; override;
    procedure SetValue(AValue: Double); override;
  public
    constructor Create(ATrackBar: TcxCustomTrackBar);
  end;

  { TdxTrackBarAccessibilityHelper }

  TdxTrackBarAccessibilityHelper = class(TdxEditAccessibilityHelper) // for internal use
  private
    FRangeValueProvider: TdxTrackBarRangeValueProvider;

    function GetFirstSimpleChildIndex: Integer;
    function GetTrackBar: TcxCustomTrackBar;
    function GetViewInfo: TcxCustomTrackBarViewInfo;
  protected
    ...
    property FirstSimpleChildIndex: Integer read GetFirstSimpleChildIndex;
    property TrackBar: TcxCustomTrackBar read GetTrackBar;
    property ViewInfo: TcxCustomTrackBarViewInfo read GetViewInfo;
  public
...
uses
  cxGeometry, dxThemeConsts, dxThemeManager, dxUxTheme, dxCoreGraphics, dxTypeHelpers,
  cxEditConsts, cxEditPaintUtils, cxEditUtils, cxExtEditUtils, cxSpinEdit;

const
  dxThisUnitName = 'cxTrackBar';

const
  TrackAndTickIndent = 2;
  TickAndTextIndent = 2;
  FromBorderIndent = 7;
  ButtonIndent = 5;
  PositionHintIndent = 10;
  MaxRealTrackZoneSize = 22;
...
begin
  AThumbSize := ACustomRect.Width;
  AThumbLargeSize := ACustomRect.Height;
end;
...
begin
  ACanvas.Line(
    ATickPosition, GetThumbRect.Bottom + TrackAndTickIndent,
    ATickPosition, GetThumbRect.Bottom + TrackAndTickIndent + ATickSize);
end;
...
begin
  ...
  if AIsHintMultiLine then
    Result.Right := Result.Left + R.Width div 2
  else
...
function TcxHorizontalTrackBarViewInfoHelper.GetTextTopRect(ATickPosition, ATextWidth, ATextHeight, AOffset: Integer): TRect;
begin
  Result.InitSize(ATickPosition - ATextWidth div 2, GetThumbRect.Top - AOffset - TrackAndTickIndent - ATextHeight, ATextWidth, ATextHeight);
end;

function TcxHorizontalTrackBarViewInfoHelper.GetTextBottomRect(ATickPosition, ATextWidth, ATextHeight, AOffset: Integer): TRect;
begin
  Result.InitSize(ATickPosition - ATextWidth div 2, GetThumbRect.Bottom + AOffset + TrackAndTickIndent, ATextWidth, ATextHeight);
end;
...
begin
  AThumbSize := ACustomRect.Height;
  AThumbLargeSize := ACustomRect.Width;
end;

procedure TcxVerticalTrackBarViewInfoHelper.DrawTickAsLineBottomRight(ACanvas: TcxCanvas; ATickPosition, ATickSize: Integer);
begin
  ACanvas.Line(
    GetThumbRect.Right + TrackAndTickIndent, ATickPosition,
    GetThumbRect.Right + TrackAndTickIndent + ATickSize, ATickPosition);
end;
...
function TcxVerticalTrackBarViewInfoHelper.GetTextTopRect(ATickPosition, ATextWidth, ATextHeight, AOffset: Integer): TRect;
begin
  Result.InitSize(GetThumbRect.Left - TrackAndTickIndent - ATextWidth - AOffset, ATickPosition - ATextHeight div 2, ATextWidth, ATextHeight);
end;

function TcxVerticalTrackBarViewInfoHelper.GetTextBottomRect(ATickPosition, ATextWidth, ATextHeight, AOffset: Integer): TRect;
begin
  Result.InitSize(GetThumbRect.Right + AOffset + TrackAndTickIndent, ATickPosition - ATextHeight div 2, ATextWidth, ATextHeight);
end;
...
begin
  ...
  try
    ARect := GetLeftButtonRect;
    ARect.NormalizeRect;
    ACanvas.IntersectClipRect(ARect);
    if not ButtonLeftFadingHelper.DrawImage(ACanvas.Handle, ARect) then
      DrawButtonLeft(ACanvas, ARect, ButtonLeftState);
  finally
    ACanvas.RestoreClipRegion;
  end;
end;

procedure TcxCustomTrackBarViewInfo.DrawButtonLeft(ACanvas: TcxCanvas; const R: TRect; AState: TcxButtonState);
begin
...
begin
  ACanvas.SaveClipRegion;
  try
    ARect := GetRightButtonRect;
    ARect.NormalizeRect;
    ACanvas.IntersectClipRect(ARect);
    if not ButtonRightFadingHelper.DrawImage(ACanvas.Handle, ARect) then
      DrawButtonRight(ACanvas, ARect, ButtonRightState);
  finally
    ACanvas.RestoreClipRegion;
  end;
end;

procedure TcxCustomTrackBarViewInfo.DrawButtonRight(ACanvas: TcxCanvas; const R: TRect; AState: TcxButtonState);
begin
  if ReverseDirection then
    LookAndFeel.Painter.DrawScaledZoomOutButton(ACanvas, R, AState, ScaleFactor)
  else
...
procedure TcxCustomTrackBarViewInfo.DrawTickAsText(ACanvas: TcxCanvas; AIndex, ATickPosition: Integer);
  ...
  const
    RotationAngle: array [TcxTrackBarTextOrientation] of TcxRotationAngle = (ra0, raPlus90);
  begin
    if ACanvas.RectFullyVisible(ARect) then
    begin
      cxDrawText(ACanvas, ACaption, ARect, 0, clDefault, RotationAngle[TextOrientation]);
...
var
  ACaption: string;
  ATextSize: TSize;
  AOffset, ATextWidth, ATextHeight: Integer;
begin
  ...
  begin
    ATextWidth := ATextSize.Height;
    ATextHeight := ATextSize.Width;
  end
  else
  begin
    ATextWidth := ATextSize.Width;
    ATextHeight := ATextSize.Height;
  end;

  AOffset := GetTextOffset;

  if TickType = tbttTicksAndNumbers then
    AOffset := AOffset + TickAndTextIndent + TickSize;

  if TickMarks in [cxtmTopLeft, cxtmBoth] then
    InternalDrawTickText(GetTextTopRect(ATickPosition, ATextWidth, ATextHeight, AOffset), ACaption);
  if TickMarks in [cxtmBottomRight, cxtmBoth] then
    InternalDrawTickText(GetTextBottomRect(ATickPosition, ATextWidth, ATextHeight, AOffset), ACaption);
end;
...
begin
  Result := AThumbRect;
  LookAndFeel.Painter.CorrectThumbRect(ACanvas, Result, Orientation = tboHorizontal,
    TrackBarTicksToTicksAlign[TickMarks], ScaleFactor);
end;
...
function TcxCustomTrackBarViewInfo.IsTextOrientationEqualTrackOrientation: Boolean;
const
  OrientationMap: array [TcxTrackBarTextOrientation] of TcxTrackBarOrientation = (tboHorizontal, tboVertical);
begin
  Result := OrientationMap[TextOrientation] = Orientation
end;
...
function TcxCustomTrackBarViewInfo.IsPtToTheLeftFromThumb(AThumb: TcxTrackBarThumbViewInfo; X, Y: Integer): Boolean;
begin
  Result := GetCurrentMousePos(X, Y) < VisibleThumbRect.Left;
end;

function TcxCustomTrackBarViewInfo.IsPtToTheRightFromThumb(AThumb: TcxTrackBarThumbViewInfo; X, Y: Integer): Boolean;
begin
...
function TcxCustomTrackBarViewInfo.GetPositionAfterJump(X, Y: Integer): Integer;
  ...
  begin
    Result := GetCurrentMousePos(X, Y) - TrackRect.Left - VisibleThumbRect.Width div 2
  end;
...
function TcxCustomTrackBarViewInfo.GetTextTopRect(ATickPosition, ATextWidth, ATextHeight, AOffset: Integer): TRect;
begin
  Result := OrientationHelper.GetTextTopRect(ATickPosition, ATextWidth, ATextHeight, AOffset);
end;

function TcxCustomTrackBarViewInfo.GetTextBottomRect(ATickPosition, ATextWidth, ATextHeight, AOffset: Integer): TRect;
begin
  Result := OrientationHelper.GetTextBottomRect(ATickPosition, ATextWidth, ATextHeight, AOffset);
end;

function TcxCustomTrackBarViewInfo.GetTextOffset: Integer;
begin
  Result := IfThen(IsTextOrientationEqualTrackOrientation, 0, 1);
end;
...
function TcxCustomTrackBarViewInfo.GetNearestThumb(X, Y: Integer): TcxTrackBarThumbViewInfo;
var
  AThumb: TcxTrackBarThumbViewInfo;
...
function TcxCustomTrackBarViewInfo.GetThumbFromPos(const APoint: TPoint): TcxTrackBarThumbViewInfo;
var
  AThumb: TcxTrackBarThumbViewInfo;
begin
  Result := nil;
...
begin
  ...
  begin
    ...
    Max := Properties.Max;
    if Orientation = tboHorizontal then
      ALength := ClientRect.Width
    else
      ALength := ClientRect.Height;
    Frequency := Math.Max(Properties.Frequency, (Max - Min) div Math.Max(1, ALength));
    ...
    if AViewInfo.UseSkins then
      TrackSize := Painter.TrackBarScaledTrackSize(ScaleFactor)
    else
      TrackSize := ScaleFactor.Apply(IfThen(Properties.AutoSize, dxTrackBarAutoTrackSize, Properties.TrackSize));

    if (Properties.ThumbType = cxttCustom) and IsOnGetThumbRectEventAssigned then
    begin
      DoOnGetThumbRect(FCustomThumbRect);
      CalculateCustomThumbSize(FCustomThumbRect, AThumbSize, AThumbLargeSize);
      ThumbWidth := AThumbSize;
      ThumbHeight := AThumbLargeSize;
    end
    else
      if AViewInfo.UseSkins then
      begin
        APainterThumbSize := Painter.TrackBarScaledThumbSize(True, ScaleFactor);
        ThumbWidth := APainterThumbSize.Width;
        ThumbHeight := APainterThumbSize.Height;
      end
      else
      begin
        ThumbWidth := IfThen(Properties.AutoSize, dxTrackBarAutoThumbWidth, Properties.ThumbWidth);
        if Painter.LookAndFeelStyle = lfsNative then
          ThumbWidth := ThumbWidth + 1;
        ThumbWidth := ScaleFactor.Apply(ThumbWidth);

        ThumbHeight := IfThen(Properties.AutoSize, dxTrackBarAutoThumbHeight, Properties.ThumbHeight);
        if Painter.LookAndFeelStyle = lfsNative then
          ThumbHeight := ThumbHeight + 1;
        ThumbHeight := ScaleFactor.Apply(ThumbHeight);
      end;
    ThumbSize := ThumbWidth;
    ThumbLargeSize := ThumbHeight;
    ...
    if Properties.ShowTicks or FAlwaysShowTickArea then
      TickSize := ScaleFactor.Apply(Properties.TickSize)
    else
      TickSize := 0;
    PopulateMajorTicks;
    PopulateMinorTicks;
...
begin
  ...
  AViewInfo.LeftButtonRect.Right := AViewInfo.TrackZoneRect.Left + AViewInfo.GetLeftButtonSize.Width;
  AViewInfo.LeftButtonRect.Top := AViewInfo.TrackZoneRect.Top +
    (AViewInfo.TrackZoneRect.Height - AViewInfo.GetLeftButtonSize.Height) div 2;
  AViewInfo.LeftButtonRect.Bottom := AViewInfo.LeftButtonRect.Top + AViewInfo.GetLeftButtonSize.Height;
  FitRectToBounds(AViewInfo.LeftButtonRect, AViewInfo.TrackBarRect);
end;

procedure TcxCustomTrackBarViewData.CalculateButtonRightRect(AViewInfo: TcxCustomTrackBarViewInfo);
begin
  AViewInfo.RightButtonRect.Left := AViewInfo.TrackZoneRect.Right - AViewInfo.GetRightButtonSize.Width;
  if not AViewInfo.UseSkins then
    Inc(AViewInfo.RightButtonRect.Left);
  AViewInfo.RightButtonRect.Right := AViewInfo.RightButtonRect.Left + AViewInfo.GetRightButtonSize.Width;
  AViewInfo.RightButtonRect.Top := AViewInfo.TrackZoneRect.Top +
    (AViewInfo.TrackZoneRect.Height - AViewInfo.GetRightButtonSize.Height) div 2;
  AViewInfo.RightButtonRect.Bottom := AViewInfo.RightButtonRect.Top + AViewInfo.GetRightButtonSize.Height;
  FitRectToBounds(AViewInfo.RightButtonRect, AViewInfo.TrackBarRect);
end;
...
begin
  AViewInfo.PixelsPerPositionStep.Numerator := AViewInfo.TrackRect.Width - AViewInfo.ThumbSize;
  AViewInfo.PixelsPerPositionStep.Denominator := GetMinMaxDiff;
end;

function TcxCustomTrackBarViewData.CalculateTickAreaSize(
  ACanvas: TcxCanvas; AViewInfo: TcxCustomTrackBarViewInfo; ALeftTop: Boolean): Integer;

  procedure CorrectTickSize(ATextHeight: Integer);
  var
    ATrackBarHeight: Integer;
    APartOfIndent: Integer;
    AMultiplier: Integer;
  begin
    AMultiplier := IfThen(Properties.TickMarks = cxtmBoth, 2, 1);
    ATrackBarHeight := AViewInfo.TrackBarRect.Height;
    if ATrackBarHeight - AMultiplier * Result < MaxRealTrackZoneSize then
    begin
      APartOfIndent := TrackAndTickIndent + TickAndTextIndent + ATextHeight;
      AViewInfo.TickSize := Max((ATrackBarHeight - MaxRealTrackZoneSize - AMultiplier * APartOfIndent) div AMultiplier, 0);
      Result := AViewInfo.TickSize + APartOfIndent;
    end;
  end;
...
begin
  Result := 0;
  ATextHeight := 0;
  AIsCorrectSide := (Properties.ShowTicks or FAlwaysShowTickArea) and
  ...
  begin
    if (Properties.TickType in [tbttTicks, tbttTicksAndNumbers]) or FAlwaysShowTickArea then
      Result := AViewInfo.TickSize + IfThen(AViewInfo.TickSize > 0, TrackAndTickIndent);
    ...
    begin
      ...
      if AViewInfo.IsTextOrientationEqualTrackOrientation then
      ...
      if Properties.TickType = tbttTicksAndNumbers then
        Result := Result + ATextHeight
      else
        Result := ATextHeight;
      Result := Result + TickAndTextIndent + AViewInfo.GetTextOffset;
    end;
  end;
  if AViewInfo.TrackBarBorderWidth = 0 then
    Inc(Result, 1);

  if AIsCorrectSide and (Properties.TickType = tbttTicksAndNumbers) and
    ((not IsInplace and not TcxCustomTrackBar(Edit).AutoSize) or (IsInplace and Properties.AutoSize)) then
...
procedure TcxCustomTrackBarViewData.CalculateThumbRect(ACanvas: TcxCanvas; AViewInfo: TcxCustomTrackBarViewInfo);

  procedure InternalCalculateThumbRect;
  begin
    ...
    AViewInfo.ThumbRect.Top := AViewInfo.TrackRect.Top + (AViewInfo.TrackSize - AViewInfo.ThumbLargeSize) div 2;
...
begin
  InternalCalculateThumbRect;
  if TryScaleThumbAndTrack(AViewInfo) then
    InternalCalculateThumbRect;
  AViewInfo.Thumb[0].Bounds := AViewInfo.VisibleThumbRect;
  AViewInfo.Thumb[0].FullRect := AViewInfo.ThumbRect;
end;

procedure TcxCustomTrackBarViewData.CalculateTrackBarRect(AViewInfo: TcxCustomTrackBarViewInfo);
begin
  AViewInfo.RealTrackBarRect := AViewInfo.GetClientRect;
  AViewInfo.TrackBarRect := AViewInfo.RealTrackBarRect;
  AViewInfo.TrackBarBorderWidth := Min(AViewInfo.TrackBarBorderWidth, AViewInfo.TrackBarRect.Width div 2);
  AViewInfo.TrackBarBorderWidth := Min(AViewInfo.TrackBarBorderWidth, AViewInfo.TrackBarRect.Height div 2);
end;
...
procedure TcxCustomTrackBarViewData.CalculateTrackRect(AViewInfo: TcxCustomTrackBarViewInfo);
  ...
  begin
    AViewInfo.TrackRect.Left := AViewInfo.TrackRect.Left + AViewInfo.GetLeftButtonSize.Width + ScaleFactor.Apply(ButtonIndent);
    AViewInfo.TrackRect.Right := AViewInfo.TrackRect.Right - AViewInfo.GetRightButtonSize.Width - ScaleFactor.Apply(ButtonIndent);
  end;

begin
  AViewInfo.TrackRect.Left := AViewInfo.TrackZoneRect.Left;
  AViewInfo.TrackRect.Right := AViewInfo.TrackZoneRect.Right;
  AViewInfo.TrackRect.Top := AViewInfo.TrackZoneRect.Top + (AViewInfo.TrackZoneRect.Height - AViewInfo.TrackSize) div 2;
  AViewInfo.TrackRect.Bottom := AViewInfo.TrackRect.Top + AViewInfo.TrackSize;
  if Properties.ShowChangeButtons then
    AdjustTrackRect;
end;

procedure TcxCustomTrackBarViewData.CalculateTrackZoneRect(ACanvas: TcxCanvas; AViewInfo: TcxCustomTrackBarViewInfo);
begin
  FTopLeftTickAreaSize := CalculateTickAreaSize(ACanvas, AViewInfo, True);
  FBottomRightTickAreaSize := CalculateTickAreaSize(ACanvas, AViewInfo, False);
  if not IsInplace and TcxCustomTrackBar(Edit).AutoSize then
    FTrackZoneSize := AViewInfo.ThumbLargeSize
  else
  begin
    FTrackZoneSize := AViewInfo.TrackBarRect.Height - FTopLeftTickAreaSize - FBottomRightTickAreaSize;
    FTrackZoneSize := Min(Max(FTrackZoneSize, 9), ScaleFactor.Apply(MaxRealTrackZoneSize));
  end;
  if FCustomThumbRect.Height > 0 then
    FTrackZoneSize := FCustomThumbRect.Height;

  if IsTopLeftAlignment then
      AViewInfo.TrackZoneRect.Top := AViewInfo.TrackBarRect.Top + FTopLeftTickAreaSize
  else
    if IsBottomRightAlignment then
      AViewInfo.TrackZoneRect.Top := AViewInfo.TrackBarRect.Bottom - FBottomRightTickAreaSize - FTrackZoneSize
    else
      AViewInfo.TrackZoneRect.Top := AViewInfo.TrackBarRect.Top +
        (((AViewInfo.TrackBarRect.Height - FTopLeftTickAreaSize - FBottomRightTickAreaSize) - FTrackZoneSize) div 2) +
        FTopLeftTickAreaSize;

  AViewInfo.TrackZoneRect.Bottom := AViewInfo.TrackZoneRect.Top + FTrackZoneSize;
...
procedure TcxCustomTrackBarViewData.FitRectToBounds(var ARect: TRect; const ABounds: TRect);
var
  ADelta: Integer;
begin
  ADelta := GetRectOutOfBoundsDelta(ARect, ABounds);
  if ADelta < 0 then
    ARect.Inflate(ADelta);
end;
...
function TcxCustomTrackBarViewData.GetRectOutOfBoundsDelta(const ARect, ABounds: TRect): Integer;
begin
  Result := Min(ARect.Left - ABounds.Left, Min(ARect.Top - ABounds.Top,
    Min(ABounds.Bottom - ARect.Bottom, ABounds.Right - ARect.Right)));
end;
...
begin
  ...
  if IsInplace then
  begin
    ASize.Width := TcxCustomTrackBarViewInfo(AViewInfo).GetThumbRect.Width;
    ASize.Height := GetTextEditContentSize(ACanvas, Self, dxMeasurePattern, 0, AEditSizeProperties, 0, False).Height;
    if Properties.Orientation = tboHorizontal then
    begin
      Result.Width := Result.Width + ASize.Width;
      Result.Height := Result.Height + ASize.Height;
    end
    else
    begin
      Result.Width := Result.Width + ASize.Height;
      Result.Height := Result.Height + ASize.Width;
    end;
  end
  else
    Calculate(ACanvas, AViewInfo.Bounds, cxNullPoint, cxmbNone, [], AViewInfo, False);
end;

function TcxCustomTrackBarViewData.InternalGetEditContentSize(ACanvas: TcxCanvas; const AEditValue: TcxEditValue;
...
begin
  ...
  begin
    Result := cxNullSize;
    ASize := FTopLeftTickAreaSize + FTrackZoneSize + FBottomRightTickAreaSize;
    if Properties.Orientation = tboHorizontal then
      Result.Height := ASize
    else
      Result.Width := ASize;
...
function TcxCustomTrackBarViewData.IsBottomRightAlignment: Boolean;
begin
  Result := (Properties.Orientation = tboVertical) and (Properties.Alignment.Horz = taRightJustify) or
    (Properties.Orientation = tboHorizontal) and (Properties.Alignment.Vert = taBottomJustify);
end;
...
function TcxCustomTrackBarViewData.IsPaddingAllowedForInplace: Boolean;
begin
  Result := False;
end;

function TcxCustomTrackBarViewData.IsPaddingAllowedForStandalone: Boolean;
begin
  Result := False;
end;
...
function TcxCustomTrackBarViewData.IsTopLeftAlignment: Boolean;
begin
  Result := (Properties.Orientation = tboVertical) and (HorzAlignment = taLeftJustify) or
    (Properties.Orientation = tboHorizontal) and (VertAlignment = taTopJustify);
end;

function TcxCustomTrackBarViewData.TryScaleThumbAndTrack(AViewInfo: TcxCustomTrackBarViewInfo): Boolean;
var
  ADelta, AThumbLargeSizeBefore: Integer;
  ADeltaPercent: Single;
begin
  Result := False;
  if not Properties.AutoSize then
    Exit;
  ADelta := GetRectOutOfBoundsDelta(AViewInfo.ThumbRect, AViewInfo.TrackZoneRect);
  if ADelta < 0 then
  begin
    AThumbLargeSizeBefore := AViewInfo.ThumbLargeSize;
    AViewInfo.ThumbLargeSize := Max(AViewInfo.ThumbLargeSize + ADelta * 2, dxTrackBarDefaultThumbHeight);
    ADeltaPercent := Min(AViewInfo.ThumbLargeSize * 100 / AThumbLargeSizeBefore / 100, 0.9);
    AViewInfo.ThumbSize := Max(Round(AViewInfo.ThumbSize * ADeltaPercent), dxTrackBarDefaultThumbWidth);

    if not AViewInfo.UseSkins then
    begin
      if AViewInfo.ThumbSize = dxTrackBarDefaultThumbWidth then
        AViewInfo.TrackSize := dxTrackBarDefaultTrackSize
      else
        AViewInfo.TrackSize := AViewInfo.ThumbSize;
...
procedure TcxCustomTrackBarViewData.UpdateValue(AViewInfo: TcxCustomTrackBarViewInfo; AValue: TcxEditValue);
begin
...
begin
  ...
  FTrackSize := dxTrackBarDefaultTrackSize;
  ...
  FTickSize := 3;
  FThumbHeight := dxTrackBarDefaultThumbHeight;
  FThumbWidth := dxTrackBarDefaultThumbWidth;
  FThumbColor := clBtnFace;
  FThumbHighlightColor := clSilver;
  FThumbStep := cxtsNormal;
end;

function TcxCustomTrackBarProperties.CreateAlignment: TcxEditAlignment;
begin
  Result := TcxEditAlignment.Create(Self, GetDefaultHorzAlignment, GetDefaultVertAlignment);
end;
...
function TcxCustomTrackBarProperties.GetDefaultHorzAlignment: TAlignment;
begin
  Result := taCenter;
end;

function TcxCustomTrackBarProperties.GetDefaultVertAlignment: TcxEditVertAlignment;
begin
  Result := taVCenter;
end;
...
begin
  Result := [esoAlwaysHotTrack, esoEditing, esoFiltering, esoSorting, esoTransparency];
end;
...
function TcxCustomTrackBarProperties.IsEditValueValid(var EditValue: TcxEditValue; AEditFocused: Boolean): Boolean;
begin
...
function TcxCustomTrackBarProperties.EditValueToPosition(const AEditValue: TcxEditValue): Integer;
begin
...
var
  ASource: TcxCustomTrackBarProperties absolute AProperties;
begin
  inherited DoAssign(AProperties);
  if AProperties is TcxCustomTrackBarProperties then
  begin
    AutoSize := ASource.AutoSize;
    BorderWidth := ASource.BorderWidth;
    ReverseDirection := ASource.ReverseDirection;
    Frequency := ASource.Frequency;
    LineSize := ASource.LineSize;
    Min := ASource.Min;
    Max := ASource.Max;
    Orientation := ASource.Orientation;
    TextOrientation := ASource.TextOrientation;
    PageSize := ASource.PageSize;
    SelectionStart := ASource.SelectionStart;
    SelectionEnd := ASource.SelectionEnd;
    SelectionColor := ASource.SelectionColor;
    ShowChangeButtons := ASource.ShowChangeButtons;
    ShowTicks := ASource.ShowTicks;
    ThumbStep := ASource.ThumbStep;
    ThumbType := ASource.ThumbType;
    ShowPositionHint := ASource.ShowPositionHint;
    ShowTrack := ASource.ShowTrack;
    TickColor := ASource.TickColor;
    TickType := ASource.TickType;
    TickMarks := ASource.TickMarks;
    TrackColor := ASource.TrackColor;
    TickSize := ASource.TickSize;
    TrackSize := ASource.TrackSize;
    ThumbHeight := ASource.ThumbHeight;
    ThumbWidth := ASource.ThumbWidth;
    ThumbColor := ASource.ThumbColor;
    ThumbHighlightColor := ASource.ThumbHighlightColor;
    OnGetTickLabel := ASource.OnGetTickLabel;
    OnGetPositionHint := ASource.OnGetPositionHint;
    OnGetThumbRect := ASource.OnGetThumbRect;
    OnDrawThumb := ASource.OnDrawThumb;
  end;
end;
...
begin
  ...
  begin
    ...
    if AOwnerControl.ActiveProperties.Orientation = tboHorizontal then
      ARect := cxRectSetTop(ARect, ATopLeft.Y - ARect.Height - PositionHintIndent)
    else
      ARect := cxRectSetLeft(ARect, ATopLeft.X - ARect.Width - PositionHintIndent);
  end;
end;
...
procedure TcxTrackBarPositionHintHelper.PositionHintHidingTimerHandler(Sender: TObject);
begin
  FPositionHintHidingTimer.Enabled := False;
  CancelHint;
end;
...
begin
  ...
  if IsSliding and (ssLeft in KeyboardStateToShiftState) then
...
function TcxTrackBarController.IsSliding: Boolean;
begin
  Result := tbmpSliding in ViewInfo.MouseStates;
end;

procedure TcxTrackBarController.StartSliding(APosition: Integer);
begin
  Include(ViewInfo.MouseStates, tbmpSliding);
  FStartSlidingPosition := TrackBar.GetPosition;
  StoredPosition := APosition;
end;

procedure TcxTrackBarController.StopSliding;
var
  APostProvider: IcxTrackBarPostProvider;
begin
  Exclude(ViewInfo.MouseStates, tbmpSliding);
  if (TrackBar.GetPosition <> StartSlidingPosition) and Supports(TrackBar, IcxTrackBarPostProvider, APostProvider) then
    APostProvider.PostPosition;
  FStartSlidingPosition := 0;
  StoredPosition := 0;
end;
...
{ TdxTrackBarRangeValueProvider }
constructor TdxTrackBarRangeValueProvider.Create(ATrackBar: TcxCustomTrackBar);
begin
  FTrackBar := ATrackBar;
end;

function TdxTrackBarRangeValueProvider.GetIsReadOnly: Boolean;
begin
  Result := FTrackBar.Properties.ReadOnly;
end;

function TdxTrackBarRangeValueProvider.GetMaximum: Double;
begin
  Result := FTrackBar.Properties.Max;
end;

function TdxTrackBarRangeValueProvider.GetMinimum: Double;
begin
  Result := FTrackBar.Properties.Min;
end;

function TdxTrackBarRangeValueProvider.GetSmallChange: Double;
begin
  Result := FTrackBar.Properties.PageSize;
end;

function TdxTrackBarRangeValueProvider.GetValue: Double;
begin
  Result := FTrackBar.Position;
end;

procedure TdxTrackBarRangeValueProvider.SetValue(AValue: Double);
begin
  FTrackBar.InternalSetPosition(Round(AValue));
end;

{ TdxTrackBarAccessibilityHelper }

constructor TdxTrackBarAccessibilityHelper.Create(AOwnerObject: TObject);
begin
  inherited;
  FRangeValueProvider := TdxTrackBarRangeValueProvider.Create(TrackBar);
end;

destructor TdxTrackBarAccessibilityHelper.Destroy;
begin
  FreeAndNil(FRangeValueProvider);
  inherited;
end;

function TdxTrackBarAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  if AChildID >= FirstSimpleChildIndex then
    case AChildID - FirstSimpleChildIndex of
      0: Result := ViewInfo.ThumbRect;
      1:
        begin
          Result := ViewInfo.TrackRect;
          Result.Right := ViewInfo.ThumbRect.Left;
        end;
      2:
        begin
          Result := ViewInfo.TrackRect;
          Result.Left := ViewInfo.ThumbRect.Right;
        end;
      3: Result := ViewInfo.LeftButtonRect;
      4: Result := ViewInfo.RightButtonRect;
    else
      Result := cxNullRect;
    end
  else
    Result := ViewInfo.Bounds;
  Result := cxRectSetOrigin(Result, Edit.ClientToScreen(Result.TopLeft))
end;

function TdxTrackBarAccessibilityHelper.ChildIsSimpleElement(AIndex: Integer): Boolean;
begin
  Result := AIndex >= FirstSimpleChildIndex - 1;
end;

function TdxTrackBarAccessibilityHelper.GetChildCount: Integer;
begin
  Result := inherited + 3;
  if TrackBar.Properties.ShowChangeButtons then
    Result := Result + 2;
end;

function TdxTrackBarAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  if AChildID >= FirstSimpleChildIndex then
  begin
    if AChildID - FirstSimpleChildIndex > 0 then
      Result := cxROLE_SYSTEM_PUSHBUTTON
    else
      Result := cxROLE_SYSTEM_INDICATOR;
  end
  else
    Result := cxROLE_SYSTEM_SLIDER;
end;

function TdxTrackBarAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  if AChildID >= FirstSimpleChildIndex then
...
function TdxTrackBarAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  if AChildID = 0 then
    Result := inherited
  else
...
function TdxTrackBarAccessibilityHelper.IsSupportedPattern(APatternID: Integer; out AProvider: IUnknown;
  AChildID: TcxAccessibleSimpleChildElementID = 0): Boolean;
begin
  Result := (APatternID = dxUIA_RangeValuePatternId) and (AChildID = 0);
  if Result then
    AProvider := FRangeValueProvider;
end;

function TdxTrackBarAccessibilityHelper.GetFirstSimpleChildIndex: Integer;
begin
  Result := TrackBar.Properties.Buttons.Count + 1;
end;

function TdxTrackBarAccessibilityHelper.GetTrackBar: TcxCustomTrackBar;
begin
  Result := TcxCustomTrackBar(inherited Edit);
end;

function TdxTrackBarAccessibilityHelper.GetViewInfo: TcxCustomTrackBarViewInfo;
begin
  Result := TcxCustomTrackBarViewInfo(inherited ViewInfo);
end;

{ TcxCustomTrackBar }

function TcxCustomTrackBar.CanAutoHeight: Boolean;
begin
  Result := Properties.Orientation = tboHorizontal;
end;

function TcxCustomTrackBar.CanAutoWidth: Boolean;
begin
  Result := Properties.Orientation = tboVertical;
end;
...
function TcxCustomTrackBar.GetAccessibilityHelperClass: TdxEditAccessibilityHelperClass;
begin
  Result := TdxTrackBarAccessibilityHelper;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      if not FController.IsSliding then
        PostPosition;
...
procedure TcxCustomTrackBar.PostPosition;
begin
  BeginUserAction;
  try
...
begin
  Result := ActiveProperties.FixPosition(ActiveProperties.EditValueToPosition(FEditValue));
end;
...
procedure TcxTrackBarLeftButtonFadingHelper.DrawButton(ACanvas: TcxCanvas; const R: TRect; AState: TcxButtonState);
begin
...
procedure TcxTrackBarRightButtonFadingHelper.DrawButton(ACanvas: TcxCanvas; const R: TRect; AState: TcxButtonState);
begin
...
constructor TcxTrackBarThumbViewInfo.Create(AViewInfo: TcxCustomTrackBarViewInfo);
begin
  inherited Create;
  FViewInfo := AViewInfo;
