...
const
  dxThisUnitName = 'dxRichEdit.Commands.FloatingObject';
...
begin
  ...
  if AEnd.LogPosition - AStart.LogPosition <> 1 then
    Exit;
  if Safe.Cast(ActivePieceTable.Runs[AStart.RunIndex], TdxFloatingObjectAnchorRun, ARun) then
...
begin
  ...
  try
    Result := Safe.Cast(ActivePieceTable.Runs[DocumentModel.Selection.Interval.NormalizedStart.RunIndex], TdxFloatingObjectAnchorRun, ARun);
    if Result then
...
begin
  Result := TdxPieceTableModifiers.CreateFloatingObjectRunTextWrapTypeModifier(TextWrapType);
end;
...
begin
  ...
  if Safe.Cast(ActivePieceTable.Runs[DocumentModel.Selection.Interval.NormalizedStart.RunIndex], TdxFloatingObjectAnchorRun, ARun) then 
  begin
    AModifier := CreateModifier(AState);
    try
...
begin
  Result := TdxPieceTableModifiers.CreateFloatingObjectRunIsBehindDocTextWrapTypeNoneModifier(IsBehindDoc);
end;
...
begin
  AModifier := TdxPieceTableModifiers.CreateFloatingObjectRunHorizontalPositionAlignmentModifier(HorizontalPositionAlignment);
  try
    Result := ChangeFloatingObjectProperty(AModifier, AStart, AEnd, AState);
  finally
    AModifier.Free;
  end;
  AModifier := TdxPieceTableModifiers.CreateFloatingObjectRunVerticalPositionAlignmentModifier(VerticalPositionAlignment);
  try
    Result := Result + ChangeFloatingObjectProperty(AModifier, AStart, AEnd, AState);
  finally
    AModifier.Free;
  end;
  AModifier := TdxPieceTableModifiers.CreateFloatingObjectRunHorizontalPositionTypeModifier(HorizontalPositionType);
  try
    Result := Result + ChangeFloatingObjectProperty(AModifier, AStart, AEnd, AState);
  finally
    AModifier.Free;
  end;
  AModifier := TdxPieceTableModifiers.CreateFloatingObjectRunVerticalPositionTypeModifier(VerticalPositionType);
  try
    Result := Result + ChangeFloatingObjectProperty(AModifier, AStart, AEnd, AState);
  finally
    AModifier.Free;
...
begin
  Result := cxGetResourceString(@sdxRichEditCommandChangeFloatingObjectFillColorMenuCaption);
end;

class function TdxChangeFloatingObjectFillColorCommand.GetDescription: string;
begin
  Result := cxGetResourceString(@sdxRichEditCommandChangeFloatingObjectFillColorDescription);
end;
...
begin
  ...
  Result := TdxPieceTableModifiers.CreateFloatingObjectRunFillColorModifier(AValueBasedState.Value);
end;
...
begin
  Result := cxGetResourceString(@sdxRichEditCommandChangeFloatingObjectOutlineColorMenuCaption);
end;

class function TdxChangeFloatingObjectOutlineColorCommand.GetDescription: string;
begin
  Result := cxGetResourceString(@sdxRichEditCommandChangeFloatingObjectOutlineColorDescription);
end;
...
begin
  if not Supports(AState, IdxValueBasedCommandUIState<TdxAlphaColor>, AValueBasedState) then
    TdxRichEditExceptions.ThrowInternalException;
  Result := TdxPieceTableModifiers.CreateFloatingObjectRunOutlineColorModifier(AValueBasedState.Value);
end;
...
begin
  Result := cxGetResourceString(@sdxRichEditCommandChangeFloatingObjectOutlineWidthMenuCaption);
end;

class function TdxChangeFloatingObjectOutlineWidthCommand.GetDescription: string;
begin
  Result := cxGetResourceString(@sdxRichEditCommandChangeFloatingObjectOutlineWidthDescription);
end;
...
begin
  if not Supports(AState, IdxValueBasedCommandUIState<Integer>, AValueBasedState) then
    TdxRichEditExceptions.ThrowInternalException;
  Result := TdxPieceTableModifiers.CreateFloatingObjectRunOutlineWidthAndColorModifier(AValueBasedState.Value);
end;
