...
uses
  ...
  cxLookAndFeels, dxGallery, cxLookAndFeelPainters, cxCustomCanvas;
...
type
  ...
  private
    ...
    procedure SetShowGroupCaption(AValue: Boolean);
  ...
  protected
    ...
    property ShowGroupCaption: Boolean read FShowGroupCaption write SetShowGroupCaption;
...
uses
  Controls, dxOffice11, Variants, cxEditConsts, Math, cxGeometry, dxDPIAwareUtils, dxTypeHelpers;

const
  dxThisUnitName = 'dxColorGallery';
...
procedure TdxColorGalleryPainter.DrawItemSelection(ACanvas: TcxCustomCanvas; AViewInfo: TdxGalleryItemViewInfo);
begin
...
begin
  inherited;
  FMarginSize := Floor(ScaleFactor.ApplyF(DefaultMarginSize));
...
procedure TdxCustomColorGallery.SetShowGroupCaption(AValue: Boolean);
begin
  if FShowGroupCaption <> AValue then
  begin
    FShowGroupCaption := AValue;
    BuildColorPalette;
  end;
end;
...
begin
  ...
  Result.ShowCaption := ShowGroupCaption;
end;
...
begin
  ...
  try
    ...
    case ColorPalette of
      cpOffice:
        begin
          ContentOffsetItems.Margin := TRect.Create(FMarginSize, 0, FMarginSize, 0);
          ContentOffsetGroups.Margin := TRect.Create(0, FMarginSize, 0, FMarginSize);
          PopulateWithOfficeColors;
        end;

      cpExtended:
        begin
          ContentOffsetItems.Margin := TRect.Create(FMarginSize, FMarginSize, FMarginSize, FMarginSize);
