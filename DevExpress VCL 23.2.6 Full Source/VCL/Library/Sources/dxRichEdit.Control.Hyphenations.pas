...
const
  dxThisUnitName = 'dxRichEdit.Control.Hyphenations';
...
begin
  ...
  begin
    if not APattern[I].IsDigit then
      Inc(Result);
  end;
end;
...
begin
  ...
  try
    ...
    for I := 1 to ACount do
    begin
      ANumericValue := Trunc(APattern[I].GetNumericValue);
...
begin
  Result := TdxIntegerList.Create;
  AWord := AOriginalWord.ToLower;
  ALength := Length(AWord);
  AStartOfWord := 1;
  for I := 1 to ALength do
  begin
    if AWord[I].IsLetter then
      Continue;
