...
begin
  if Assigned(FTouchHelpers) then
...
begin
  if AScrollKind = sbHorizontal then
    Inc(FOverPan.X, IfThen(IsRightToLeft, - ADelta, ADelta))
  else
...
function TdxGestureHelper.IsRightToLeft: Boolean;
var
  AGestureClient2: IdxGestureClient2;
begin
  Result := Supports(FGestureClient, IdxGestureClient2, AGestureClient2) and AGestureClient2.UseRightToLeftScrollbar;
end;
...
finalization
  ...
  dxFreeGlobalObject(FTouchHelpers);
