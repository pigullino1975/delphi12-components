...
type
  ...
  protected
    ...
    procedure Lock;
    procedure Unlock;
    procedure SetHSL(AH: Double; ASa: Double; AL: Double);
  public
  ...
  private
    function GetItem(AIndex: TdxListIndex): TdxColorPickerCustomViewInfoCell;
  public
    ...
    procedure Draw(ACanvas: TcxCanvas);
    //
    property Items[AIndex: TdxListIndex]: TdxColorPickerCustomViewInfoCell read GetItem; default;
  end;
  ...
  { TdxColorPickerLightnessEditViewInfoCell }

  TdxColorPickerLightnessEditViewInfoCell = class(TdxColorPickerCustomSpinEditViewInfoCell)
  protected
    procedure EditValueChanged; override;
    function GetCaption: string; override;
    procedure UpdateEditValue; override;
...
procedure TdxColorPickerColorInfo.Lock;
begin
  Inc(FLockCount);
end;

procedure TdxColorPickerColorInfo.Unlock;
begin
  Dec(FLockCount);
  if (FLockCount = 0) and FNotifyChanged then
    Changed;
end;
...
begin
  Result := TdxColorUtils.HSLToColor(FHSL);
end;
...
begin
  ...
  begin
    FHSL := TdxColorUtils.AlphaColorToHSL(AValue, FAlpha);
    Changed;
  end;
end;
...
procedure TdxColorPickerColorInfo.SetHSL(AH, ASa, AL: Double);
begin
  Lock;
  try
    H := AH;
...
function TdxColorPickerCustomViewInfoCells.GetItem(AIndex: TdxListIndex): TdxColorPickerCustomViewInfoCell;
begin
...
begin
  FlushContentCache;
  FContentCache := TcxBitmap32.CreateSize(ContentBounds, True);
...
begin
  cxDrawTransparencyCheckerboard(ACanvas, Rect(0, 0, AWidth, AHeight), cxTransparencyCheckerboardSizeSmall);
  dxGpFillRectByGradient(ACanvas.Handle, Rect(0, 0, AWidth, AHeight), clNone, ColorInfo.Color, LinearGradientModeVertical, 0);
end;
...
begin
  ...
  Y := Min(cxRectHeight(ContentBounds), Max(0, Y - ContentBounds.Top));
  ColorInfo.SetHSL(X / cxRectWidth(ContentBounds), 1 - Y / cxRectHeight(ContentBounds), ColorInfo.L);
...
begin
  ...
  begin
    dxGpFillRectByGradient(
      ...
      TdxColorUtils.HSLToColor(I / AWidth, 1, 0.5),
      TdxColorUtils.HSLToColor(I / AWidth, 0, 0.5),
...
var
  AColor: TColor;
begin
  AColor := TdxColorUtils.HSLToColor(ColorInfo.H, ColorInfo.S, 0.5);
...
begin
  ...
  ANewS := X / cxRectWidth(ContentBounds);
  ColorInfo.SetHSL(ColorInfo.H, ANewS, (1 - ANewS * 0.5) * (1 - Y / cxRectHeight(ContentBounds)));
  CursorPosition := Point(X, Y);
end;
...
begin
  ...
  begin
    dxGpFillRectByGradient(ACanvas.Handle, Rect(0, I, AWidth, I + 1),
      TdxColorUtils.HSLToColor(ColorInfo.H, 0, 1 - I / AHeight),
      TdxColorUtils.HSLToColor(ColorInfo.H, 1, 0.5 * (1 - I / AHeight)),
...
begin
  ...
  begin
    dxGpFillRectByGradient(
      ...
      TdxColorUtils.HSLToColor(I / AHeight, 1, 0.5),
      TdxColorUtils.HSLToColor(I / AHeight, 1, 0.5),
      LinearGradientModeVertical);
  end;
end;
...
begin
  ...
  cxDrawTransparencyCheckerboard(ACanvas, R, cxTransparencyCheckerboardSizeSmall);
...
begin
  ...
  for I := 0 to Length(AllowedChars) - 1 do
    Result := Max(Result, cxMeasureCanvas.TextWidth(AllowedChars[I]));
...
procedure TdxColorPickerLightnessEditViewInfoCell.EditValueChanged;
begin
  ColorInfo.L := EditValue / 255;
end;

function TdxColorPickerLightnessEditViewInfoCell.GetCaption: string;
begin
  Result := cxGetResourceString(@sdxColorPickerLightnessLabel);
end;

procedure TdxColorPickerLightnessEditViewInfoCell.UpdateEditValue;
begin
...
begin
  ...
  CreateCell(TdxColorPickerLightnessEditViewInfoCell);
end;
...
begin
  ColorInfo.AlphaColor := TdxAlphaColors.FromHexCode(Edit.EditingValue, HexNotation = cphnDelphi);
end;
...
begin
  if not Edit.Focused then
    Edit.EditValue := TdxAlphaColors.ToHexCode(ColorInfo.AlphaColor, Control.OptionsView.AllowEditAlpha, HexNotation = cphnDelphi);
end;
...
procedure TdxCustomColorPicker.DrawOpaqueBackground(ACanvas: TcxCanvas; const ARect: TRect);
begin
