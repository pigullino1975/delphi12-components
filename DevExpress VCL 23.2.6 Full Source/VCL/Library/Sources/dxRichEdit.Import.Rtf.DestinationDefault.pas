...
unit dxRichEdit.Import.Rtf.DestinationDefault; // for internal use
...
type
  ...
  private
    ...
    class procedure DocumentProtectionLevelHandler(AImporter: TdxRtfImporter; AParameterValue: Integer; AHasParameter: Boolean); static;
    ...
    class procedure EnforceDocumentProtectionHandler(AImporter: TdxRtfImporter; AParameterValue: Integer; AHasParameter: Boolean); static;
    ...
    class procedure ReadOnlyProtectionHandler(AImporter: TdxRtfImporter; AParameterValue: Integer; AHasParameter: Boolean); static;
...
uses
  ...
  dxRichEdit.Types, dxCore;

const
  dxThisUnitName = 'dxRichEdit.Import.Rtf.DestinationDefault';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxDefaultDestination.Initialize', SysInit.HInstance);{$ENDIF}
  FKeywordHT := CreateKeywordHT;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxDefaultDestination.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxDefaultDestination.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxDefaultDestination.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FKeywordHT);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxDefaultDestination.Finalize', SysInit.HInstance);{$ENDIF}
end;
