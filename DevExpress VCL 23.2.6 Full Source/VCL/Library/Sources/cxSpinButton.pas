...
uses
  ...
  dxCore, cxClasses, cxContainer, cxControls, cxEdit, cxExtEditConsts, dxMessages,
...
type
  ...
  private
    ...
    procedure DXMAdjustSize(var Message: TMessage); message DXM_ADJUSTSIZE;
  ...
  protected
    ...
    function HandleMouseWheel(Shift: TShiftState): Boolean; override;
...
begin
  ...
  begin
    case AMessage.Msg of
      WM_KEYDOWN, WM_KEYUP:
        PostMessage(Handle, AMessage.Msg, AMessage.WParam, AMessage.LParam);
      WM_MOUSEWHEEL:
        Perform(CM_MOUSEWHEEL, AMessage.WParam, AMessage.LParam);
    end;
...
function TcxCustomSpinButton.HandleMouseWheel(Shift: TShiftState): Boolean;
begin
  Result := not IsDesigning and (not IsInplace or ActiveProperties.UseMouseWheel or (ssCtrl in Shift));
end;
...
procedure TcxCustomSpinButton.AdjustHeight;
begin
  if not FIsHeightAdjusting and (Associate <> nil) then
  begin
    FIsHeightAdjusting := True;
    try
      if Associate.Showing then
        Height := Associate.Height
      else
        PostMessage(Handle, DXM_ADJUSTSIZE, 0, 0);
    finally
      FIsHeightAdjusting := False;
    end;
  end;
end;

procedure TcxCustomSpinButton.CMShowingChanged(var Message: TMessage);
begin
  inherited;
  AdjustHeight;
end;

procedure TcxCustomSpinButton.DXMAdjustSize(var Message: TMessage);
begin
  Height := Associate.Height;
end;

procedure TcxCustomSpinButton.DXMScaleChanged(var Message: TMessage);
begin
  inherited;
  AdjustHeight;
end;
