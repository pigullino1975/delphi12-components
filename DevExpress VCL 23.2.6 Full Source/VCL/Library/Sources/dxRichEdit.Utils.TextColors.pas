...
const
  dxThisUnitName = 'dxRichEdit.Utils.TextColors';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxTextColors.Initialize', SysInit.HInstance);{$ENDIF}
  Defaults := FromSkinColors(TdxAlphaColors.Window, TdxAlphaColors.Black);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxTextColors.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxTextColors.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxTextColors.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(Defaults);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxTextColors.Finalize', SysInit.HInstance);{$ENDIF}
end;
