...
uses
  ...
  dxCore, dxCoreClasses, cxGraphics, cxClasses, cxGeometry,
  dxMapControlTypes, dxMapControlHttpRequest, dxRESTService, dxBingMapRESTService,
...
const
  dxThisUnitName = 'dxMapControlBingMapInformationProviders';
...
procedure TdxMapControlBingMapInformationProvider.SetConnectionProtocol(const Value: TdxMapControlConnectionProtocol);
begin
  if FConnectionProtocol <> Value then
  begin
    FConnectionProtocol := Value;
    FBingService.ConnectionProtocol := FConnectionProtocol;
  end;
end;
