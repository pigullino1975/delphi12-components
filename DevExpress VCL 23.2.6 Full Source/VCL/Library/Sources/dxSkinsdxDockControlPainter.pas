...
type
  ...
  protected
    ...
    function GetHideBarButtonTextColor(AControl: TdxCustomDockControl): TColor; override;
    function GetHideBarButtonTextIndent: Integer; override;
...
uses
  cxPCPainters, dxSkinsStrs, dxCore;

const
  dxThisUnitName = 'dxSkinsdxDockControlPainter';
...
var
  ASize: TSize;
begin
  ASize := HeaderLineSize;
  Result := cxRect(ASize.cy, ASize.cy, ASize.cy, ASize.cy);
end;
...
var
  AInfo: TdxSkinInfo;
  AViewInfo: TcxTabViewInfo;
begin
  Result := clDefault;
  if GetSkinPainterData(AInfo) then
  begin
    AViewInfo := ViewInfo.TabsViewInfo[ATabVisibleIndex];
    if not AViewInfo.Enabled then
      Result := dxSkinColorSelect(True, AInfo.DockControlTabTextColorDisabled, AInfo.DockControlTabTextColor)
    else
      if AViewInfo.IsMainTab then
        Result := dxSkinColorSelect(AViewInfo.IsHotTrack, AInfo.DockControlTabTextColorActiveHot, AInfo.DockControlTabTextColorActive)
      else
        Result := dxSkinColorSelect(AViewInfo.IsHotTrack, AInfo.DockControlTabTextColorHot, AInfo.DockControlTabTextColor);
  end;
...
begin
  ...
  begin
    FPanelBackgroundCache.DrawEx(ACanvas.Handle, ABorder, ARect, ScaleFactor);
    ACanvas.SaveClipRegion;
    try
...
function TdxDockControlSkinPainter.GetHideBarButtonTextColor(AControl: TdxCustomDockControl): TColor;
var
  AColor: TdxSkinColor;
  ASkinInfo: TdxSkinInfo;
begin
  ...
  if GetSkinPainterData(ASkinInfo) then
  begin
    AColor := ASkinInfo.DockControlHideBarTextColor[IsWorkingControl(AControl)];
    if AColor <> nil then
      Result := AColor.Value;
  end;
end;

function TdxDockControlSkinPainter.GetHideBarButtonTextIndent: Integer;
begin
