...
const
  dxThisUnitName = 'dxRichEdit.Api.Paragraphs';
...
begin
  CheckValid;
  Result := TdxNativeStyleCollections.GetStyle(Document.MainDocument.ParagraphStyles, InnerParagraph.ParagraphStyle);
end;
...
begin
  ...
  try
    Result := TdxNativeProperties.CreateTabInfoCollection(Document, ATabs);
  finally
    ATabs.Free;
  end;
end;
...
begin
  ...
  ATabInfo := TdxNativeProperties.CreateModelTabInfoCollection(Document, ATabs);
  try
...
begin
  if FCharacterProperties = nil then
    FCharacterProperties := TdxNativeProperties.GetNativeSimpleCharacterProperties(InnerListLevel.CharacterProperties);
...
begin
  if FParagraphProperties = nil then
    FParagraphProperties := TdxNativeProperties.GetNativeSimpleParagraphProperties(FDocument, InnerListLevel.ParagraphProperties);
  Result := FParagraphProperties;
end;

function TdxNativeListLevel.GetParagraphStyle: IdxRichEditParagraphStyle;
begin
  Result := TdxNativeStyleCollections.GetStyle(FDocument.MainDocument.ParagraphStyles, InnerListLevel.ParagraphStyle);
end;
