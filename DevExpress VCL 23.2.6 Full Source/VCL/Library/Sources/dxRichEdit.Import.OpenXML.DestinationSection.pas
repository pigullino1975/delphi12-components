...
unit dxRichEdit.Import.OpenXML.DestinationSection; // for internal use
...
const
  dxThisUnitName = 'dxRichEdit.Import.OpenXML.DestinationSection';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxSectionDestination.Initialize', SysInit.HInstance);{$ENDIF}
  FHandlerTable := CreateElementHandlerTable;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxSectionDestination.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxSectionDestination.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxSectionDestination.Finalize', SysInit.HInstance);{$ENDIF}
  FHandlerTable.Free;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxSectionDestination.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxColumnsDestination.Initialize', SysInit.HInstance);{$ENDIF}
  FHandlerTable := CreateElementHandlerTable;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxColumnsDestination.Initialize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxColumnsDestination.Finalize', SysInit.HInstance);{$ENDIF}
  FHandlerTable.Free;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxColumnsDestination.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
var
  AClones: TdxColumnInfoCollection;
  I: Integer;
begin
  ...
  begin
    ...
    begin
      AClones := TdxColumnInfoCollection.Create;
      try
        for I := 0 to FColumnInfos.Count - 1 do
          AClones.Add(FColumnInfos[I].Clone);
        Columns.SetColumns(AClones);
      finally
        AClones.Free;
      end;
    end
...
begin
  ...
  try
    ...
    Assert(not FColumnInfos.OwnsObjects, 'Error adding to GC');
    FColumnInfos.Add(AColumnInfo);
  finally
    Importer.AddToGarbageCollector(AColumnInfo);
  end;
end;
