...
type
  ...
  public
    ...
    property AlwaysShowBlanksAndLiterals: Boolean read FAlwaysShowBlanksAndLiterals write SetAlwaysShowBlanksAndLiterals default False;
  ...
  public
    ...
    property Properties: TcxCustomMaskEditProperties read GetProperties write SetProperties;
  end;
...
constructor TcxMaskEditCustomMode.Create(AEdit: TcxCustomMaskEdit; AProperties: TcxCustomMaskEditProperties);
begin
  inherited Create;
  FEdit := AEdit;
  if FEdit = nil then
...
constructor TcxMaskEditStandardMode.Create(AEdit: TcxCustomMaskEdit; AProperties: TcxCustomMaskEditProperties);
begin
...
begin
  ...
  FEdit.SelStart := 0;
  if (FMask.Count > 0) and (FMask.Items[FEdit.SelStart] is TcxStandardMaskLiteralItem) then
    PressRight;
end;
...
begin
  Result := False;
  if (Length(FEdit.EditText) <= 0) or (FEdit.SelStart >= Length(FEdit.EditText)) then
    Exit;
...
function TcxMaskEditStandardMode.ProcessSelText(const Value: string; out ANewText: string;
  out ANewSelStart: Integer): Boolean;
var
  ALength: Integer;
  I, J: Integer;
  ASymbol: Char;
...
function TcxMaskEditStandardMode.GetUpdatedText(const AText: string; AMatchForBlanksAndLiterals: Boolean = True): string;
begin
  Result := AText;
...
constructor TcxMaskEditRegExprMode.Create(AEdit: TcxCustomMaskEdit; AProperties: TcxCustomMaskEditProperties);
begin
  inherited Create(AEdit, AProperties);
    ...
    try
      ...
      begin
        ...
        for I := 0 to E.Errors.Count - 1 do
          AStr := AStr + dxCRLF + E.Errors[I].FullMessage;
...
begin
  ...
  begin
    if FEdit.SelLength > 0 then
    begin
      if (FEdit.CursorPos = FEdit.SelStart) and not FEdit.FShiftOn then
      begin
        for I := 0 to FEdit.SelLength - 1 do
          GetTailFirstChar;
        Exit;
        ...
        if (FEdit.CursorPos = FEdit.SelStart + FEdit.SelLength) and not FEdit.FShiftOn then
          Exit;
    end;
...
function TcxMaskEditRegExprMode.GetUpdatedText(const AText: string; AMatchForBlanksAndLiterals: Boolean = True): string;
begin
  Result := AText; 
end;
...
function TcxMaskEditRegExprMode.CompileRegExpr(ARegExpr: TcxRegExpr): Boolean;
begin
...
begin
  Result := FMouseAction or
    (FEdit.Text <> '') and (FHead = '') and ((FHead + FTail <> FEdit.Text) or (FEdit.CursorPos <> Length(FHead)));
end;
...
var
  I: Integer;
begin
  if FEdit.SelStart = Length(FHead) then  
  begin
    ...
    if (FEdit.SelStart + FEdit.SelLength) = Length(FHead) then 
    begin
...
constructor TcxMaskEditRegExprExMode.Create(AEdit: TcxCustomMaskEdit; AProperties: TcxCustomMaskEditProperties);
begin
  inherited Create(AEdit, AProperties);
...
function TcxCustomMaskEditProperties.IsEditValueValid(var AEditValue: TcxEditValue; AEditFocused: Boolean): Boolean;
begin
  Result := IsMasked or inherited IsEditValueValid(AEditValue, AEditFocused);
end;

function TcxCustomMaskEditProperties.IsValueFormattedByProperties: Boolean;
begin
  Result := not EmptyMask(FEditMask) and ((DefaultValuesProvider = nil) or
    (DefaultValuesProvider.DefaultDisplayFormat = '') and not DefaultValuesProvider.IsOnGetTextAssigned and
    ((FMaskKind <> emkStandard) or (FEditMask <> DefaultValuesProvider.DefaultEditMask)));
end;
...
var
  AText: string;
begin
  if IsMasked and not (not AEditFocused and (DefaultValuesProvider <> nil) and
  ...
  begin
    ...
    begin
      if DefaultValuesProvider <> nil then
        Result := DefaultValuesProvider.DefaultEditMask
      else
        Result := '';
    end
    else
...
function TcxCustomMaskEditProperties.SupportsMultiThreading(AUseDisplayValue: Boolean): Boolean;
begin
  Result := inherited SupportsMultiThreading(AUseDisplayValue) and not (AUseDisplayValue and
    IsMasked and (MaskKind in [emkRegExpr, emkRegExprEx]));
end;
...
begin
  if not ActiveProperties.IsMasked then
    ...
    if (Key = dxVKTab) or (Key = dxVKEscape) or not ValidateKeyPress(Key) then
      Key := #0
    else
    begin
      ...
      begin
        ...
        begin
          if dxCharInSet(Key, [dxVKCopy, dxVKPaste, dxVKCut, dxVKSelectAll]) then
            inherited
          else
            if (Key = dxVKBack) and not Mode.PressBackSpace or (Key <> dxVKBack) and not Mode.PressSymbol(Key) then
              Key := #0;
        end;
      end;
    end;
...
procedure TcxCustomMaskEdit.InternalSetValue(const AValue: string);
begin
...
procedure TcxCustomMaskEdit.SetEditText(const AValue: TCaption);
var
  AEditText: string;
begin
...
procedure TcxCustomMaskEdit.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  if ActiveProperties.IsMasked then
...
procedure TcxCustomMaskEdit.PrepareEditValue(const ADisplayValue: TcxEditValue; out AEditValue: TcxEditValue; AEditFocused: Boolean);
begin
  if ActiveProperties.IsMasked then
    if not AEditFocused and (Mode is TcxMaskEditStandardMode) and
      (TcxMaskEditStandardMode(Mode).FMask.SaveLiteralCharacters or not ActiveProperties.AlwaysShowBlanksAndLiterals) then
        inherited PrepareEditValue(ADisplayValue, AEditValue, AEditFocused)
    else
      AEditValue := Mode.GetUpdatedText(ADisplayValue)
  else
    inherited PrepareEditValue(ADisplayValue, AEditValue, AEditFocused);
end;
...
function TcxCustomMaskEdit.IsCharValidForPos(var AChar: Char; APos: Integer): Boolean;
begin
  Result := True;
end;
...
begin
  ...
  if not TcxCustomMaskEditProperties(AProperties).IsMasked and (AValueTypeClass <> nil) and AValueTypeClass.IsString then
...
class procedure TcxFilterMaskEditHelper.InitializeProperties(AProperties, AEditProperties: TcxCustomEditProperties; AHasButtons: Boolean);
begin
  inherited InitializeProperties(AProperties, AEditProperties, AHasButtons);
  with TcxCustomMaskEditProperties(AProperties) do
  begin
    if TcxCustomMaskEditProperties(AEditProperties).MaskKind = emkRegExpr then
