...
uses
  ...
  ComCtrls, ExtCtrls, Menus, Buttons, ActnList, ToolWin, ImgList, Contnrs, StdCtrls,
  dxCore, cxControls, cxClasses, dxMessages, dxBar, cxLookAndFeelPainters, cxLookAndFeels,
  cxImageList, dxForms, cxGeometry, cxListBox;
...
type
  ...
  public
    ...
    class function CalculateListBoxItemHeight(AListBox: TcxListBox): Integer; virtual;
    class function GetListBoxPadding(AListBox: TcxListBox): TdxPadding; virtual;
  ...
  public
    constructor Create(AOwner: TdxBarCustomCustomizationForm; AListBox: TcxListBox); virtual;
    destructor Destroy; override;
  ...
  strict private
    ...
    procedure DoListDrawItem(AListBox: TcxListBox; ACanvas: TcxCanvas; AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
  ...
  public
    constructor Create(AOwner: TdxBarCustomCustomizationForm; AListBox: TcxListBox); override;
  ...
  strict private
    ...
    procedure LAllDrawItem(AListBox: TcxListBox; ACanvas: TcxCanvas; AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
  protected
  ...
  strict private
    ...
    procedure lbGroupsDrawItem(AListBox: TcxListBox; ACanvas:TcxCanvas; AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
    procedure lbGroupsKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
    procedure lbGroupsMeasureItem(AListBox: TcxListBox; Index: Integer; var Height: Integer);
  protected
    procedure WndProc(var Message: TMessage); override;
  public
    constructor Create(AOwner: TdxBarCustomCustomizationForm; AListBox: TcxListBox); override;
  ...
  strict private
    ...
    procedure lbGroupItemsDrawItem(AListBox: TcxListBox; ACanvas: TcxCanvas; AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
    procedure lbGroupItemsKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
    procedure lbGroupItemsMeasureItem(AListBox: TcxListBox; AIndex: Integer; var Height: Integer);
  protected
  ...
  private
    ...
    FAlreadySynchronous: TcxListBox;
  ...
  protected
    ...
    procedure ClearObjects(AListBox: TcxListBox);
    function CreateSelectionList: TcxComponentList;
    procedure CreateParams(var Params: TCreateParams); override;
    procedure DeleteSelectedObjects(AListBox: TcxListBox;  ADeleteProc: TNotifyEvent = nil; ASynchronizeDesigner: Boolean = True);
    procedure DoShowHelp; virtual;
    function GetAllCommandsListBox: TcxListBox; virtual; abstract;
    function GetBarItemsBackgroundColor: TColor; virtual;
    function GetBarList(Index: Integer): TdxBar; virtual;
    function GetBarListBox: TcxListBox; virtual; abstract;
    function GetCategoriesList: TcxListBox; virtual; abstract;
    ...
    function GetExclusiveObject(AListBox: TcxListBox): TObject;
    function GetGroupItemsListBox: TcxListBox; virtual; abstract;
    function GetGroupsListBox: TcxListBox; virtual; abstract;
    function GetItemsListBox: TcxListBox; virtual; abstract;
    function GetNextSelectedObject(AListBox: TcxListBox): TObject;
    function GetObjectFromListBox(AListBox: TcxListBox; AIndex: Integer): TObject;
    function GetPainterClass: TdxBarCustomizationFormPainterClass; virtual;
    function GetSelCount(AListBox: TcxListBox): Integer;
    function GetShowCommandsWithShortCut: Boolean; virtual; abstract;
    procedure GetSelection(AListBox: TcxListBox; AList: TList);
    function GetVisibleItemsCount(AListBox: TcxListBox): Integer;
    ...
    procedure MoveItems(AListBox: TcxListBox; ABarComponentList: TdxBarComponentList; ADirection: Integer);
    procedure SetSelection(AListBox: TcxListBox; AList: TList); overload;
    procedure SetSelection(AListBox: TcxListBox; AObject: TObject); overload;

    procedure DeferredCallSynchronizationListBox(AListBox: TcxListBox);
    procedure SynchronizeListBox(AListBox: TcxListBox; AChangedObject: TObject = nil; AAction: TcxComponentCollectionNotification = ccnChanged);
    procedure SynchronizeListBoxes; virtual; abstract;
    procedure SynchronizeListBoxSelection(AListBox: TcxListBox);
    procedure UpdateTopIndex(AListBox: TcxListBox);
    ...
    procedure SynchronizeDesigner(AListBox: TcxListBox); overload;

    procedure DeleteGroupItem(AGroupItem: TObject);
    procedure GroupStructureChange;

    procedure UpdateCommonEvents(AListBox: TcxListBox;
    ...
    property AllCommandsListBox: TcxListBox read GetAllCommandsListBox;
    property AlreadySynchronous: TcxListBox read FAlreadySynchronous;
    ...
    property BarListBox: TcxListBox read GetBarListBox;
    ...
    property GroupItemsListBox: TcxListBox read GetGroupItemsListBox;
    property GroupItemsListBoxHelper: TdxBarCustomizationGroupItemsListBoxHelper read FGroupItemsListBoxHelper;
    property GroupsListBox: TcxListBox read GetGroupsListBox;
    ...
    property ItemsListBox: TcxListBox read GetItemsListBox;
  ...
  public
    ...
    property CategoriesList: TcxListBox read GetCategoriesList;
...
const
  dxThisUnitName = 'dxBarCustomCustomizationForm';
...
constructor TdxBarCustomizationFormAllCommandsListHelper.Create(AOwner: TdxBarCustomCustomizationForm; AListBox: TcxListBox);
begin
  ...
  AListBox.ItemHeight := TdxTextMeasurer.TextLineHeight(AListBox.Canvas.Handle) + ScaleFactor.Apply(2);
end;
...
begin
  ...
  ACanvas := TcxListBoxAccess(ListBox).Canvas.Canvas;
  ACanvas.Font := ListBox.Style.Font;
...
procedure TdxBarCustomizationFormAllCommandsListHelper.LAllDrawItem(
  AListBox: TcxListBox; ACanvas: TcxCanvas; AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);

  procedure DrawSeparator(AColor: TColor; APos, ATop, ABottom: Integer);
  begin
  ...
  procedure DrawColumn(const AText: string; ATextWidth: Integer; var AColumnPos: Integer; ANeedDrawSeparator, ALastRow: Boolean);
  begin
    ACanvas.TextOut(AColumnPos + AllCommandsIndent, ARect.Top, AText);
    Inc(AColumnPos, AllCommandsIndent + ATextWidth + AllCommandsIndent);
    if ANeedDrawSeparator then
    begin
      DrawSeparator(ACanvas.Font.Color, AColumnPos, ARect.Top, ARect.Bottom);
      if ALastRow then
        DrawSeparator(clWindowText, AColumnPos, ARect.Bottom, AListBox.ClientHeight);
    end;
  end;
...
begin
  ...
  W := 0;
  ALastRow := AIndex = AListBox.Items.Count - 1;
  with AllCommandList[AIndex] do
  begin
    DrawColumn(Name, FAllCommandsNameWidth, W, True, ALastRow);
    DrawColumn(Caption, FAllCommandsCaptionWidth, W, True, ALastRow);
    DrawColumn(IntToStr(LinkCount) + ' link(s)', FAllCommandsLinksCountWidth, W, True, ALastRow);
    DrawColumn(ShortCutToText(ShortCut), FAllCommandsShortCutWidth, W, False, ALastRow);
  end;
end;
...
constructor TdxBarCustomizationFormGroupsListHelper.Create(AOwner: TdxBarCustomCustomizationForm; AListBox: TcxListBox);
begin
  inherited Create(AOwner, AListBox);
...
begin
  ...
  cxExtTextOut(ACanvas.Handle, AListBox.Items[AIndex], Point(ARect.Left + 2, ARect.Top + 1), ARect, ETO_OPAQUE);
end;
...
procedure TdxBarCustomizationFormGroupsListHelper.lbGroupsMeasureItem(AListBox: TcxListBox; Index: Integer; var Height: Integer);
begin
  Height := TdxTextMeasurer.TextLineHeight(AListBox.Style.Font) + 2;
end;

{ TdxBarCustomizationGroupItemsListBoxHelper }

constructor TdxBarCustomizationGroupItemsListBoxHelper.Create(AOwner: TdxBarCustomCustomizationForm; AListBox: TcxListBox);
begin
  inherited Create(AOwner, AListBox);
...
procedure TdxBarCustomizationGroupItemsListBoxHelper.lbGroupItemsDrawItem(
  AListBox: TcxListBox; ACanvas: TcxCanvas; AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
begin
  if AListBox.Items.Objects[AIndex] is TdxBarGroup then
    ACanvas.Font.Style := ACanvas.Font.Style + [fsBold];

  cxExtTextOut(ACanvas.Handle, AListBox.Items[AIndex],
    Point(ARect.Left + Self.ScaleFactor.Apply(2),
    ARect.Top + Self.ScaleFactor.Apply(1)), ARect, ETO_OPAQUE);
end;
...
procedure TdxBarCustomizationGroupItemsListBoxHelper.lbGroupItemsMeasureItem(
  AListBox: TcxListBox; AIndex: Integer; var Height: Integer);
begin
  Height := TdxTextMeasurer.TextLineHeight(AListBox.Style.Font) + ScaleFactor.Apply(2);
end;
    ...
    cxRightToLeftDependentDraw(ACanvas.Handle, ARect, UseRightToLeftAlignment(AItem),
      ...
      begin
        DrawSubItemArrow(ACanvas,
          ...
          AArrowRect.Top + AArrowRect.Height div 2 - ScaleFactor.Apply(3), False);
      end
    );
end;
...
begin
  ...
  TdxCustomBarEditAccess(AItem).DrawCustomizingImageContent(ACanvas, AItemRect, ASelected, dxGetScaleFactorForCanvas(ACanvas));
end;
...
class function TdxBarCustomizationFormPainter.CalculateListBoxItemHeight(AListBox: TcxListBox): Integer;
var
  APadding: TdxPadding;
begin
  APadding := GetListBoxPadding(AListBox);
  Result := TdxTextMeasurer.TextLineHeight(AListBox.Style.Font) + APadding.Height;
end;

class function TdxBarCustomizationFormPainter.GetListBoxPadding(AListBox: TcxListBox): TdxPadding;
begin
  Result := TdxPadding.Create(TcxListBoxAccess(AListBox).ScaleFactor.Apply(3));
end;
...
begin
  ACanvas.Brush.Color := BrushColors(False, AItem);
  ACanvas.FillRect(R);
  dxDrawThemeBackground(ACanvas.Handle, totToolBar, ScaleFactor, R, TP_BUTTON, TS_HOT);
end;
...
class procedure TdxBarCustomizationFormXPPainter.DrawComboBoxButton(
  ACanvas: TCanvas; AItem: TdxBarItem; ARect: TRect; ASelected: Boolean);
begin
  dxDrawThemeBackground(ACanvas.Handle, totComboBox, ScaleFactor, ARect, CP_DROPDOWNBUTTON, CBXS_NORMAL);
end;

class procedure TdxBarCustomizationFormXPPainter.DrawFocusedRect(ACanvas: TCanvas; ARect: TRect; AItem: TdxBarItem);
var
  IsGlyph: Boolean;
begin
  with TdxBarItemAccess(AItem) do
...
class procedure TdxBarCustomizationFormXPPainter.DrawEditEdge(
  ACanvas: TCanvas; var AItemRect: TRect; AItem: TdxCustomBarEdit; ASelected: Boolean);
begin
  ACanvas.FrameRect(AItemRect);
  InflateRect(AItemRect, -1, -1);
...
begin
  ...
  InflateRect(AItemRect, 1, 1);
  dxDrawThemeBackground(ACanvas.Handle, totEdit, ScaleFactor, AItemRect, EP_EDITTEXT, ETS_NORMAL);
...
begin
  ...
  BarListBox.Style.Color := EditBackgroundColor;
  BarListBox.Style.Font.Color := EditTextColor;
  CategoriesList.Style.Color := EditBackgroundColor;
  CategoriesList.Style.Font.Color := EditTextColor;
  ItemsListBox.Style.Color := BarItemsBackgroundColor;
end;

procedure TdxBarCustomCustomizationForm.UpdateListBoxItemHeight;
begin
  BarListBox.ItemHeight := BarListBoxHelper.CalculateItemHeight;
  CategoriesList.ItemHeight := CategoryListBoxHelper.CalculateItemHeight;
end;
...
var
  ...
  R: TRect;
begin
  if not dxSystemInfo.UseMouseWheelRouting then
  begin
    ...
    begin
      ...
      if (AControl is TcxInnerListBox) and PtInRect(R, P) and
        not (ActiveControl is TcxInnerListBox) then
      begin
...
begin
  ...
  SynchronizeListBox(TcxListBox(Message.WParam));
end;
...
begin
  ...
  begin
    if FDisabledWindows <> nil then
      raise EdxException.Create('TdxBarCustomCustomizationForm.EnableWindows fails');
...
function TdxBarCustomCustomizationForm.GetExclusiveObject(AListBox: TcxListBox): TObject;
var
  AIndex: Integer;
begin
  ...
  begin
    ...
    begin
      if SendMessage(AListBox.InnerListBox.Handle, LB_GETSELITEMS, 1, LPARAM(@AIndex)) <> 1 then
        AIndex := -1;
    end
    else
      AIndex := -1
...
function TdxBarCustomCustomizationForm.GetNextSelectedObject(AListBox: TcxListBox): TObject;
var
...
function TdxBarCustomCustomizationForm.GetObjectFromListBox(
  AListBox: TcxListBox; AIndex: Integer): TObject;
begin
...
procedure TdxBarCustomCustomizationForm.GetSelection(AListBox: TcxListBox; AList: TList);
var
  I: Integer;
begin
...
procedure TdxBarCustomCustomizationForm.SynchronizeDesigner(AListBox: TcxListBox);
var
...
procedure TdxBarCustomCustomizationForm.UpdateCommonEvents(AListBox: TcxListBox;
  AAddAction, ADeleteAction, AMoveUpAction, AMoveDownAction: TAction;
  ADeletePermissiveProc: TdxBarPermissiveProc = nil);
var
  I: Integer;
...
procedure TdxBarCustomCustomizationForm.DeleteSelectedObjects(
  AListBox: TcxListBox; ADeleteProc: TNotifyEvent = nil; ASynchronizeDesigner: Boolean = True);
var
  I: Integer;
  ASelectedObjects: TObjectList;
  ANextObject: TObject;
...
procedure TdxBarCustomCustomizationForm.ClearObjects(AListBox: TcxListBox);
var
  I: Integer;
  ANextObject: TObject;
begin
...
function TdxBarCustomCustomizationForm.GetSelCount(AListBox: TcxListBox): Integer;
begin
  if AListBox.MultiSelect then
...
function TdxBarCustomCustomizationForm.GetVisibleItemsCount(AListBox: TcxListBox): Integer;
begin
...
procedure TdxBarCustomCustomizationForm.MoveItems(AListBox: TcxListBox;
...
procedure TdxBarCustomCustomizationForm.SetSelection(AListBox: TcxListBox; AList: TList);
var
  I, AIndex: Integer;
begin
  AListBox.Items.BeginUpdate;
...
procedure TdxBarCustomCustomizationForm.SetSelection(AListBox: TcxListBox; AObject: TObject);
var
  ASelectedObjects: TList;
begin
  ASelectedObjects := TList.Create;
...
procedure TdxBarCustomCustomizationForm.DeferredCallSynchronizationListBox(AListBox: TcxListBox);
begin
  PostMessage(Handle, DXM_BAR_LB_DEFERREDCALLSYNCHRONIZATION, WPARAM(AListBox), 0);
end;

procedure TdxBarCustomCustomizationForm.SynchronizeListBox(AListBox: TcxListBox;
  AChangedObject: TObject = nil; AAction: TcxComponentCollectionNotification = ccnChanged);
var
  AObjectIndex, APrevTopIndex: Integer;
  AIsSelected: Boolean;
begin
  if BarManager.IsDestroying then
    Exit;
  AListBox.Items.BeginUpdate;
  try
...
procedure TdxBarCustomCustomizationForm.SynchronizeListBoxSelection(AListBox: TcxListBox);
var
  AList: TList;
begin
...
procedure TdxBarCustomCustomizationForm.UpdateTopIndex(AListBox: TcxListBox);
...
function TdxBarCustomizationFormListBoxHelper.CalculateItemHeight: Integer;
begin
  Result := PainterClass.CalculateListBoxItemHeight(ListBox);
end;

constructor TdxBarCustomizationFormListBoxHelper.Create(AOwner: TdxBarCustomCustomizationForm; AListBox: TcxListBox);
begin
  FOwner := AOwner;
  FListBox := AListBox;
...
constructor TdxBarCustomizationFormBarListHelper.Create(AOwner: TdxBarCustomCustomizationForm; AListBox: TcxListBox);
begin
  inherited Create(AOwner, AListBox);
  ListBox.ListStyle := lbOwnerDrawFixed;
  ...
  ListBox.ItemHeight := CalculateItemHeight;
...
var
  ...
  ABar: TdxBarAccess;
begin
  if Owner.BarList[AIndex] = nil then 
    Exit;
  R1 := ARect;
  if AListBox.UseRightToLeftAlignment then
    R1.Left := R1.Right - cxRectHeight(ARect)
  else
    R1.Right := R1.Left + cxRectHeight(ARect);
  ACanvas.Canvas.FillRect(ARect);
  ABar := TdxBarAccess(Owner.BarList[AIndex]);
  PainterClass.DrawCheckBox(ACanvas.Canvas, R1, ABar.Visible, ABar.CanClose);
  if AListBox.UseRightToLeftAlignment then
    Dec(ARect.Right, cxRectHeight(ARect))
  else
    Inc(ARect.Left, cxRectHeight(ARect));
  if AIndex < AListBox.Items.Count then
  begin
    if AListBox.UseRightToLeftAlignment then
      Dec(ARect.Right, ScaleFactor.Apply(2))
    else
      Inc(ARect.Left, ScaleFactor.Apply(2));

    cxDrawText(ACanvas.Handle, AListBox.Items[AIndex], ARect,
      AListBox.DrawTextBiDiModeFlags(DT_SINGLELINE or DT_VCENTER or DT_NOPREFIX));
  end;
end;
...
constructor TdxBarCustomizationFormCategoryListHelper.Create(AOwner: TdxBarCustomCustomizationForm; AListBox: TcxListBox);
begin
  inherited Create(AOwner, AListBox);
  ListBox.ListStyle := lbOwnerDrawFixed;
...
constructor TdxBarCustomizationFormItemsListHelper.Create(
  AOwner: TdxBarCustomCustomizationForm; AListBox: TcxListBox);
begin
  inherited Create(AOwner, AListBox);
  ListBox.MultiSelect := BarManager.Designing;
...
procedure TdxBarCustomCustomizationForm.WMDestroy(var Message: TWMDestroy);
begin
  EnableWindows(True);
  inherited;
end;

procedure TdxBarCustomCustomizationForm.WMNCDestroy(var Message: TWMNCDestroy);
begin
  EnableWindows(True);
  inherited;
end;
