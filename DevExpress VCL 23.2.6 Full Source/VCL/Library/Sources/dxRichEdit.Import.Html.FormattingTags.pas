...
unit dxRichEdit.Import.Html.FormattingTags; // for internal use
...
type
  ...
  public
    constructor Create(AImporter: TdxCustomHtmlImporter; AFontSize: Integer; AOutlineLevel: Integer); reintroduce; virtual;
  end;
  ...
  { TdxOfficeParagraphTag }

  TdxOfficeParagraphTag = class(TdxParagraphTagBase)
  protected
    procedure ApplyTagProperties; override;
  public
    procedure AppendContent(const AText: string; AUseRawText: Boolean); override;
    function DecodeStringContent(const AContentText: string; out ADecodeContentText: string): Boolean; override;
  end;
...
const
  dxThisUnitName = 'dxRichEdit.Import.Html.FormattingTags';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxParagraphTagBase.Initialize', SysInit.HInstance);{$ENDIF}
  FAttributeTable := AddAttributes;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxParagraphTagBase.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxParagraphTagBase.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxParagraphTagBase.Finalize', SysInit.HInstance);{$ENDIF}
  FAttributeTable.Free;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxParagraphTagBase.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
procedure TdxOfficeParagraphTag.AppendContent(const AText: string; AUseRawText: Boolean);
begin
  if AText = TdxCharacters.ParagraphMark then
    Importer.AppendParagraph
  else
    inherited AppendContent(AText, AUseRawText);
end;

procedure TdxOfficeParagraphTag.ApplyTagProperties;
begin
end;

function TdxOfficeParagraphTag.DecodeStringContent(const AContentText: string; out ADecodeContentText: string): Boolean;
begin
  if AContentText = '&nbsp;' then
  begin
    ADecodeContentText := TdxCharacters.ParagraphMark;
    Result := True
  end
  else
    Result := False;
...
begin
  ...
  Importer.Position.ParagraphFormatting.CopyFrom(DocumentModel.DefaultParagraphProperties.Info);
...
function TdxSpanTag.DecodeStringContent(const AContentText: string; out ADecodeContentText: string): Boolean;
var
  AText, AStyleValue: string;
begin
  if FStyleAttributeProperties.HasRawProperties and
    FStyleAttributeProperties.RawProperties.TryGetValue('mso-spacerun', AStyleValue) then
    begin
      AText := TdxHtmlImporterAccess(Importer).ApplyEncoding(AContentText);
      if SameText(AStyleValue, 'yes') then
        ADecodeContentText := SysUtils.StringReplace(SysUtils.StringReplace(AText, '&nbsp;', ' ', [rfReplaceAll, rfIgnoreCase]),
          TdxCharacters.NonBreakingSpace, ' ', [rfReplaceAll])
      else
        ADecodeContentText := TdxHtmlImporterAccess(Importer).ReplaceSpecialSymbols(
          SysUtils.StringReplace(AText, ' ', TdxCharacters.NonBreakingSpace, [rfReplaceAll, rfIgnoreCase]));
      Result := True;
    end
  else
    Result := False;
