...
unit dxRichEdit.Import; // for internal use
...
const
  dxThisUnitName = 'dxRichEdit.Import';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxShadingHelper.Initialize', SysInit.HInstance);{$ENDIF}
  FPatternMultipliers := CreatePatternMultiplierDictionary;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxShadingHelper.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxShadingHelper.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxShadingHelper.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FPatternMultipliers);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxShadingHelper.Finalize', SysInit.HInstance);{$ENDIF}
end;
