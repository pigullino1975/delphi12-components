...
unit dxRichEdit.Import.Html.HTMLParser; // for internal use
...
const
  dxThisUnitName = 'dxRichEdit.Import.Html.HTMLParser';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxHtmlParser.Initialize', SysInit.HInstance);{$ENDIF}
  FHtmlTagNameIDTable := CreateHtmlTagNameTable;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxHtmlParser.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxHtmlParser.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxHtmlParser.Finalize', SysInit.HInstance);{$ENDIF}
  FHtmlTagNameIDTable.Free;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxHtmlParser.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  case ACh of
    ...
    '/':
      ...
      if not ACh.IsLetter and (ACh <> '?') then
      begin
        ParserState.ChangeState(TdxState.ReadContent);
        ToggleReadContentState(ACh);
      end
...
begin
  if HtmlTagNameIDTable.TryGetValue(AName, Result) then
    Exit
  else
    if TdxStringHelper.Contains(AName, ':') then
      Result := TdxHtmlTagNameID.Namespace
    else
