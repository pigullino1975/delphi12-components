...
unit dxRichEdit.Export.Html.Utils; // for internal use
...
type
  ...
  strict private
    class var
      FUnitHT: TdxEnumeratedDictionary<TdxGraphicUnit, string>;
  ...
  strict private
    class function CreateUnitTable: TdxEnumeratedDictionary<TdxGraphicUnit, string>; static;
  ...
  public
    ...
    class function FontSizeToString(ASize: Single; AUnit: TdxGraphicUnit): string; overload; static;
    class function FontSizeToStringInPixels(ASize: Single; AUnit: TdxGraphicUnit): string; static;
  end;
...
const
  dxThisUnitName = 'dxRichEdit.Export.Html.Utils';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxHtmlEntities.Initialize', SysInit.HInstance);{$ENDIF}
  ...
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxHtmlEntities.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxHtmlEntities.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxHtmlEntities.Finalize', SysInit.HInstance);{$ENDIF}
  FLock.Free;
  FEntitiesLookup.Free;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxHtmlEntities.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxHtmlConvert.Initialize', SysInit.HInstance);{$ENDIF}
  FUnitHT := CreateUnitTable;
  FHtmlAlignHT := CreateHorizontalAlignmentTable;
  FHtmlVAlignHT := CreateVerticalAlignmentTable;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxHtmlConvert.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxHtmlConvert.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxHtmlConvert.Finalize', SysInit.HInstance);{$ENDIF}
  FUnitHT.Free;
  FHtmlAlignHT.Free;
  FHtmlVAlignHT.Free;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxHtmlConvert.Finalize', SysInit.HInstance);{$ENDIF}
end;

class function TdxHtmlConvert.CreateUnitTable: TdxEnumeratedDictionary<TdxGraphicUnit, string>;
begin
  Result := TdxEnumeratedDictionary<TdxGraphicUnit, string>.Create;
  Result.Add(TdxGraphicUnit.guInch, 'in');
  Result.Add(TdxGraphicUnit.guMillimeter, 'mm');
  Result.Add(TdxGraphicUnit.guPixel, 'px');
  Result.Add(TdxGraphicUnit.guPoint, 'pt');
end;
...
class function TdxHtmlConvert.FontSizeToString(ASize: Single; AUnit: TdxGraphicUnit): string;
var
  AUnitType: string;
begin
  if (AUnit = TdxGraphicUnit.guDocument) or not not FUnitHT.TryGetValue(AUnit, AUnitType) then
    AUnitType := 'pt';

  if AUnitType = 'px' then
    Result := IntToStr(Round(ASize)) + AUnitType
  else
    Result := FloatToStr(SimpleRoundTo(ASize, -1), TdxCultureInfo.InvariantCulture.FormatSettings) + AUnitType;
end;

class function TdxHtmlConvert.FontSizeToStringInPixels(ASize: Single; AUnit: TdxGraphicUnit): string;
//var
