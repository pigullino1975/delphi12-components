...
type
  ...
  strict private type
    ...
    TCommandPool = class(TdxObjectMRUCache<string, TdxCustomDBCommand>)
    private
      const
        MaxPoolSize = 12;
    public
  ...
  protected
    ...
    function TryCastVariant(const AValue: Variant; AExpectedType: TdxDBColumnType = TdxDBColumnType.Unknown): Variant; virtual;
  ...
  public
    ...
    function GetParameterName(const AParameter: IdxOperandValue; AIndex: Integer; var ACreateParameter: Boolean): string; virtual;
    procedure GetTableSchema(ATable: TdxDBTable; ACheckIndexes, ACheckForeignKeys: Boolean); virtual; abstract;
...
const
  dxThisUnitName = 'dxEMF.DB.SQLConnectionProvider';
...
begin
  ...
  AListParameters := ConnectionString.Split([';']);
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxSQLConnectionProvider.Create', SysInit.HInstance);{$ENDIF}
  DefaultStringSize := 100;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxSQLConnectionProvider.Create', SysInit.HInstance);{$ENDIF}
end;
...
var
  ...
  ASchemaName: string;
begin
  ADot := ATableName.LastIndexOf('.');
  if ADot > 0 then
  begin
    AStart := ATableName.IndexOf('.');
...
function TdxSQLConnectionProvider.TryCastVariant(const AValue: Variant; AExpectedType: TdxDBColumnType): Variant;
begin
  Result := AValue;
end;
...
function TdxSQLConnectionProvider.GetParameterName(const AParameter: IdxOperandValue; AIndex: Integer;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxSQLConnectionProviderFactory.Destroy', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FProviderSQLs);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxSQLConnectionProviderFactory.Destroy', SysInit.HInstance);{$ENDIF}
end;
