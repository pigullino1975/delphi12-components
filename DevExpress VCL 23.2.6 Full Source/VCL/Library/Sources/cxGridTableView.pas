...
uses
  Types, Variants, Windows, Messages, Classes, Graphics, Controls, ImgList, Forms, Generics.Collections, Generics.Defaults,
  ...
  cxData, cxDataStorage, cxFilter, Menus, dxCoreClasses, cxGridInplaceEditForm, dxUIElementPopupWindow, cxFindPanel,
...
type
  ...
  protected
    ...
    procedure CalculateDynamicFixedItems(ALeft, ATop, ACheckedkLeft: Integer); virtual;
    function CalculateHeight: Integer; override;
...
begin
  Result := inherited CanProcessMultiSelect(AHitTest, AShift) and
    ...
    (not CellMultiSelect or (ssLeft in AShift) or
     not Safe.Cast(AHitTest.ViewInfo, TcxGridTableDataCellViewInfo, ACell) or
...
procedure TcxGridHeaderViewInfo.CalculateDynamicFixedItems(ALeft, ATop, ACheckedkLeft: Integer);
var
  I, AWidth: Integer;
  AItem: TcxGridColumnHeaderViewInfo;
begin
  AItem := FirstItem[fkNone];
  Inc(ACheckedkLeft, IfThen(bLeft in AItem.HiddenBorders, 0, AItem.BorderWidth[bLeft]));
  for I := FirstItemIndex[fkNone] to LastItemIndex[fkNone] do
  begin
    AItem := Items[I];
    if (AItem.Column.FixedKind = fkLeftDynamic) and (AItem.ClientBounds.Left < ACheckedkLeft) then
    begin
      ...
      begin
        Dec(ACheckedkLeft, Min(AWidth, AItem.BorderSize[bRight]));
        CalculateSeparatorBounds(fkLeftDynamic, ACheckedkLeft, ATop, GridViewInfo.FixedSeparatorWidth);
...
begin
  ...
  begin
    ...
    if IsRightToLeftConverted then
      Result := ACell.Bounds.Right >= AFinishBound
    else
      Result := ACell.Bounds.Left <= AFinishBound;
  end;
end;
...
begin
  Result := Options.ShowGroupValuesWithImages and
    Safe.Cast(GetProperties, TcxImageComboBoxProperties, AProperties) and (AProperties.Images <> nil);
end;
...
initialization
  ...
  Classes.RegisterClasses([TcxGridColumn, TcxGridTableViewStyleSheet, TcxGridTableRowLayoutItem,
