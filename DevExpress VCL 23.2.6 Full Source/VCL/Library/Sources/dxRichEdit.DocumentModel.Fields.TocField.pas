...
unit dxRichEdit.DocumentModel.Fields.TocField; // for internal use
...
uses
  Math, Windows, RTLConsts, Character, Rtti,
  dxCore,
  dxRichEdit.Strs,
...
const
  dxThisUnitName = 'dxRichEdit.DocumentModel.Fields.TocField';
...
type
  ...
  protected
    ...
    procedure SetTocSearchRange;
    procedure SetDefaultTocSearchRange;
    ...
    procedure ProcessField(AField: TdxField);
    procedure ProcessCalculatedField(AField: TdxCalculatedFieldBase;
      AScanner: TdxFieldScanner; ADocumentField: TdxField);
    procedure InitializeField(AField: TdxCalculatedFieldBase; AScanner: TdxFieldScanner);
    procedure ProcessTocEntryField(AEntry: TdxTocEntryField);
    procedure ProcessSequenceField(ASequence: TdxSequenceField; ADocumentField: TdxField);
    ...
    function LookupEntireParagraphBookmark(AParagraph: TdxParagraph; AActualParagraphPosition: TdxDocumentLogPosition): TdxBookmark;
    function LookupZeroLengthBookmarkAtLogPosition(APos: TdxDocumentLogPosition): TdxBookmark;
    function LookupExactBookmark(APos: TdxDocumentLogPosition; ALen: Integer): TdxBookmark;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxTocField.Initialize', SysInit.HInstance);{$ENDIF}
  FSwitchesWithArgument := CreateSwitchesWithArgument(['o', 'n', 't', 'l', 'f', 'a', 'b', 'c', 'd', 'p', 's']);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxTocField.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxTocField.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxTocField.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FSwitchesWithArgument);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxTocField.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  Result := ACh.IsWhiteSpace or CharInSet(ACh, [TdxCharacters.NonBreakingSpace,
