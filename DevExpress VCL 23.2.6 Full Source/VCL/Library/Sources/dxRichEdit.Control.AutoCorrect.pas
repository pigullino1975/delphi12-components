...
const
  dxThisUnitName = 'dxRichEdit.Control.AutoCorrect';
...
begin
  if Safe.Cast(AInfo.DocumentServer, TdxInnerRichEditControl, AControl) and not AControl.IsDisposed then
  begin
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxUrlAutoCorrectProvider.Initialize', SysInit.HInstance);{$ENDIF}
  ...
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxUrlAutoCorrectProvider.Initialize', SysInit.HInstance);{$ENDIF}
end;
