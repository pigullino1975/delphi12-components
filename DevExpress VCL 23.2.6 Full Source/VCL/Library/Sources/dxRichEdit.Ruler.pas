...
unit dxRichEdit.Ruler; // for internal use
...
type
  ...
  strict private
    ...
    FTextSize: TSize;
    FControl: TdxRulerControlBase;
    FActiveAreaCollection: TdxRectFList;
    FDisplayActiveAreaCollection: TdxRectFList;
    FSpaceAreaCollection: TdxRectFList;
    FDisplaySpaceAreaCollection: TdxRectFList;
    FClientBounds: TRect;
  ...
  public
    ...
    property SpaceAreaCollection: TdxRectFList read FSpaceAreaCollection;
    property ActiveAreaCollection: TdxRectFList read FActiveAreaCollection;
    ...
    property DisplaySpaceAreaCollection: TdxRectFList read FDisplaySpaceAreaCollection;
    property DisplayActiveAreaCollection: TdxRectFList read FDisplayActiveAreaCollection;
...
const
  dxThisUnitName = 'dxRichEdit.Ruler';

type
  { TdxTickMarkPositionComparable }

  TdxTickMarkPositionComparable = record
  strict private
    FOrientation: TdxOrientation;
    FPos: Integer;
  public
...
begin
  if Safe.Cast(State, TdxDragAndDropMouseHandlerState, ADragState) then
    ADragState.Cancel;
end;
...
begin
  ...
  if not TdxAlgorithms1<TdxRulerTickmark>.BinarySearch(ATickmarks, AComparer.CompareTo, AIndex) then
  begin
...
begin
  ...
  FActiveAreaCollection := TdxRectFList.Create;
  FDisplayActiveAreaCollection := TdxRectFList.Create;
  FSpaceAreaCollection := TdxRectFList.Create;
  FDisplaySpaceAreaCollection := TdxRectFList.Create;
...
var
  AUnitConverter: TdxUnitConverter;
...
begin
  AUnitConverter := Control.DocumentModel.InternalAPI.UnitConverters[Control.RichEditControl.InnerControl.UIUnit];
  AToLayoutUnitConverter := Control.DocumentModel.ToDocumentLayoutUnitConverter;
  AStep := CalculateTickStep(AUnitConverter);
  ...
  begin
    ...
    AValue := Abs(Round(AUnitConverter.FromUnits(AValueInModelUnits))); 
...
begin
  ...
  IsReady := True;
  APageClientBounds := ACaretPosition.PageViewInfo.ClientBounds;
  APageClientBounds.Y := 0;
  if AControl.RichEditControl.InnerControl.ActiveViewType = TdxRichEditViewType.PrintLayout then
    APageClientBounds.Height := Round(ToDocumentLayoutUnitConverter.ToLayoutUnits(Min(MaxRulerDimensionInModelUnits, SectionProperties.PageHeight)) * AControl.ZoomFactor)
  else
    APageClientBounds.Height := Round(ToDocumentLayoutUnitConverter.ToLayoutUnits(Min(MaxRulerDimensionInModelUnits, SectionProperties.PageHeight - SectionProperties.TopMargin)) * AControl.ZoomFactor);
...
begin
  ...
  AActiveViewPageCalculator := AControl.InnerControl.Formatter.DocumentFormatter.Controller.PageController.PageBoundsCalculator;
  AActiveViewPageClientBounds := CalculatePageClientBounds(AActiveViewPageCalculator, SectionProperties);
  ADeltaY := AActiveViewPageClientBounds.Top - APrintLayoutViewPageClientBounds.Top - Ceil(Control.Painter.PaddingTop / Control.ZoomFactor);
...
begin
  ...
  begin
    ABounds := TRect.CreateSize(Trunc(ActiveAreaCollection[0].Left), Trunc(FTableVerticalPositions[I] * Control.ZoomFactor),
      Trunc(ActiveAreaCollection[0].Width), Control.DocumentModel.LayoutUnitConverter.PixelsToLayoutUnits(MSO365ZoneHeight));
