...
uses
  ...
  cxGraphics, dxGDIPlusClasses, cxGeometry, cxControls, cxLookAndFeels, cxLookAndFeelPainters, cxClasses, cxContainer,
...
type
  ...
  private
    ...
    function CalculateIndicatorRect(AViewInfo: TdxCustomToggleSwitchViewInfo;
      AIndicatorSize: TSize; AAlign: TLeftRight): TRect;
  protected
  ...
  public
    ...
    function GetTextGap(AIsInplace, ANativeStyle: Boolean; AScaleFactor: TdxScaleFactor): Integer; override;
    function GetContentOffset(AIsCaptionVisible: Boolean): TRect; override;
  ...
  protected
    ...
    class function IsOldStyleSupported: Boolean; override;
    class function GetContentOffset(AIsInplace, AIsCaptionVisible: Boolean; AItemAlignment: TLeftRight; AScaleFactor: TdxScaleFactor): TRect; override;
    class function GetTextGap(AIsInplace, ANativeStyle: Boolean; AScaleFactor: TdxScaleFactor): Integer; override;
...
uses
  Types, Variants, Math,
  dxCore, cxFilterControlUtils, dxCoreGraphics;

const
  dxThisUnitName = 'dxToggleSwitch';
...
function TdxCustomToggleSwitchViewData.GetTextGap(AIsInplace, ANativeStyle: Boolean; AScaleFactor: TdxScaleFactor): Integer;
begin
  Result := TdxCustomToggleSwitchViewInfo.GetTextGap(AIsInplace, ANativeStyle, AScaleFactor);
end;

function TdxCustomToggleSwitchViewData.GetContentOffset(AIsCaptionVisible: Boolean): TRect;
begin
  Result := TdxCustomToggleSwitchViewInfo.GetContentOffset(IsInplace, AIsCaptionVisible, Alignment, ScaleFactor);
end;
...
begin
  ...
  Inc(Result.cx, ASize1.cx + AOffset.Left + AOffset.Right);
end;
...
begin
  ...
  begin
    case AViewInfo.StateIndicatorPosition of
      sipInside:
        begin
          Result := cxRectCenterVertically(AViewInfo.CheckBoxRect, AIndicatorSize.cy);
          Result := cxRectSetWidth(Result, AIndicatorSize.cx);
      ...
      sipOutside:
        begin
          Result := cxRectCenterVertically(AViewInfo.ClientRect, AIndicatorSize.cy);
          Result := cxRectSetWidth(Result, AIndicatorSize.cx);
          if AAlign = taLeftJustify then
...
begin
  ...
  begin
    ...
    if AViewInfo.IsTextEnabled and (AViewInfo.Text <> '') then
      ATextGap := AViewInfo.GetTextGap(IsInplace, NativeStyle, ScaleFactor)
    else
      ATextGap := 0;
...
begin
  case AViewInfo.StateIndicatorKind of
    sikText:
      begin
        AViewInfo.OffIndicatorSize.cx := Max(cxTextWidth(AViewInfo.Font, AViewInfo.OffText),
          cxTextWidth(AViewInfo.Font, AViewInfo.OffText));
        AViewInfo.OnIndicatorSize.cx := Max(cxTextWidth(AViewInfo.Font, AViewInfo.OnText),
          cxTextWidth(AViewInfo.Font, AViewInfo.OnText));
        AViewInfo.OffIndicatorSize.cy := Max(TdxTextMeasurer.TextLineHeight(AViewInfo.Font),
          TdxTextMeasurer.TextLineHeight(AViewInfo.Font));
        AViewInfo.OnIndicatorSize.cy := AViewInfo.OffIndicatorSize.cy;
      end;
    sikGlyph:
      begin
...
begin
  case AViewInfo.StateIndicatorPosition of
    sipOutside:
      Result := AViewInfo.GetTextGap(IsInplace, NativeStyle, ScaleFactor);
    sipInside:
...
class function TdxCustomToggleSwitchViewInfo.IsOldStyleSupported: Boolean;
begin
  Result := False;
end;

class function TdxCustomToggleSwitchViewInfo.GetContentOffset(AIsInplace, AIsCaptionVisible: Boolean; AItemAlignment: TLeftRight; AScaleFactor: TdxScaleFactor): TRect;
begin
  if AIsInplace then
    Result := Rect(1, 1, 1, 1)
  else
...
class function TdxCustomToggleSwitchViewInfo.GetTextGap(AIsInplace, ANativeStyle: Boolean; AScaleFactor: TdxScaleFactor): Integer;
begin
  Result := AScaleFactor.Apply(dxToggleSwitchTextOffset)
end;
...
begin
  ...
  Painter.DrawScaledToggleSwitch(ACanvas, ToggleSwitchBounds, ThumbState, Focused, ThumbBounds, ScaleFactor);
  ...
  if (not IsRectEmpty(FocusRect)) and Painter.SupportsNativeFocusRect(TdxPaintPartID.ToggleSwitch) then
...
begin
  if not cxRectIsNull(OffIndicatorRect) and NeedShowOffIndicator then
    Painter.DrawToggleSwitchStateIndicator(ACanvas, OffIndicatorRect, OffText, Font);

  if not cxRectIsNull(OnIndicatorRect) and NeedShowOnIndicator then
    Painter.DrawToggleSwitchStateIndicator(ACanvas, OnIndicatorRect, OnText, Font);
end;

procedure TdxCustomToggleSwitchViewInfo.DrawThumb(ACanvas: TcxCanvas; AClientRect: TRect; AState: TcxButtonState);
begin
  Painter.DrawScaledToggleSwitchThumb(ACanvas, AClientRect, AState,  State = cbsChecked, Focused, ScaleFactor);
end;
