...
type
  ...
  protected
    ...
    function TryCastVariant(const AValue: Variant; AExpectedType: TdxDBColumnType = TdxDBColumnType.Unknown): Variant; override;
...
const
  dxThisUnitName = 'dxEMF.DB.SQLite';
  ...
  try
    ...
    begin
      ...
      ATable.ForeignKeys.Add(TdxDBForeignKey.Create([APrimaryKeyColumns[0]],
        ADataSet.Fields[2].AsString.Trim(['"']), [AForeignKeyColumns[0]]));
      ADataSet.Next;
    end;
  finally
...
function TdxSQLiteConnectionProvider.TryCastVariant(const AValue: Variant; AExpectedType: TdxDBColumnType): Variant;
begin
  Result := AValue;
end;
...
begin
  ...
  if Supports(AParameter, IdxConstantValue) and not AParameter.Value.IsNullOrEmpty then
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxSQLiteConnectionProvider.Destroy', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FDBTypes);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxSQLiteConnectionProvider.Destroy', SysInit.HInstance);{$ENDIF}
end;
            ...
            if ALikeIndex > 0 then
              Exit(Format('((%0:s like %2:s) And (Substr(%0:s, 1, Length(%1:s)) = (%1:s)))',
                [AProcessParameter(ASecondOperator), AProcessParameter(ASecondOperand),
                 ProcessParameter(AOperandString.Substring(0, ALikeIndex) + '%')]));
        end;
