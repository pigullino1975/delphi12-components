...
uses
  dxCore, cxCustomData, cxGridCustomView, cxGridTableView, cxGridMenuOperations, Classes, StrUtils;
...
uses
  SysUtils, cxClasses, cxLookAndFeels, cxGridCustomTableView, cxGridPopupMenuConsts, cxGridStrs;

const
  dxThisUnitName = 'cxGridDataRowPopupMenuItems';
...
begin
  Result := IfThen(dxUseVectorIcons, 'GRID_DATAROWFIXEDONTOP', AImageName + '1');
end;
...
begin
  Result := IfThen(dxUseVectorIcons, 'GRID_DATAROWFIXEDONBOTTOM', AImageName + '3');
end;
...
begin
  Result := IfThen(dxUseVectorIcons, 'GRID_DATAROWFIXEDNONE', AImageName + '2');
end;
