...
var
  ADisplayBias: TDateTime;
begin
  if OriginalEventType = etPattern then
    ADisplayBias := -Storage.TimeBias + BiasTime - DateTimeHelper.CurrentTimeZoneBias
  else
    ADisplayBias := 0;
  if (Event.Pattern <> nil) and not DeleteExceptions then
  begin
    Event.Pattern.RecurrenceInfo.DisplayTimeBias := ADisplayBias;
    Result := cxGetRecurrenceDescriptionStringProc(Event.Pattern.RecurrenceInfo, True);
  end
  else
  begin
    Event.RecurrenceInfo.DisplayTimeBias := ADisplayBias;
