...
procedure cxExportTLToHTMLStream(AStream: TStream; ATreeList: TcxCustomTreeList; AExpand: Boolean = True;
...
procedure cxExportTLToCSVStream(AStream: TStream; ATreeList: TcxCustomTreeList;
  AExpand: Boolean = True; ASaveAll: Boolean = True; const ASeparator: Char = ',';
...
procedure cxExportTLToTextStream(AStream: TStream; ATreeList: TcxCustomTreeList; AExpand: Boolean;
  ASaveAll: Boolean = True; AHandler: TObject = nil; AEncoding: TEncoding = nil); overload;
...
procedure cxExportTLToTextStream(AStream: TStream; ATreeList: TcxCustomTreeList; AExpand: Boolean = True;
  ASaveAll: Boolean = True; const ASeparator: string = ''; const ABeginString: string = ''; const AEndString: string = '';
...
procedure cxExportTLToExcelStream(AStream: TStream; ATreeList: TcxCustomTreeList; AExpand: Boolean = True;
  ASaveAll: Boolean = True; AUseNativeFormat: Boolean = True; AHandler: TObject = nil);
...
procedure cxExportTLToXLSXStream(const AStream: TStream; ATreeList: TcxCustomTreeList;
  AExpand: Boolean = True; ASaveAll: Boolean = True; AUseNativeFormat: Boolean = True;
...
procedure cxExportTLToFile(AFileName: string; ATreeList: TcxCustomTreeList; AExportType: Integer;
  ...
  AHandler: TObject = nil; AEncoding: TEncoding = nil);
procedure cxExportTLToStream(AStream: TStream; ATreeList: TcxCustomTreeList; AExportType: Integer;
  AExpand, ASaveAll, AUseNativeFormat: Boolean; const ASeparators: array of string;
  AHandler: TObject = nil; AEncoding: TEncoding = nil);
...
type
  ...
  public
    constructor Create(ATreeList: TcxCustomTreeList; AExportType: Integer;
      AStream: TStream; AHandler: TObject = nil); virtual;
...
constructor TcxTreeListExportHelper.Create(ATreeList: TcxCustomTreeList;
  AExportType: Integer; AStream: TStream; AHandler: TObject = nil);
...
begin
  ...
  TcxExport.Provider(AExportType, AStream).GetInterface(IcxExportProvider, FProvider);
  ...
  begin
    ...
    begin
      ...
      if IsNativeFormatProperties(AEditCell.Properties) then
        Provider.AddValue(AExportVisualItem, AEditCell.Node.Values[AEditCell.Column.ItemIndex],
          AEditCell.Properties, GetDisplayFormat(AEditCell.Properties), GetDisplayFormatType(AEditCell.Properties))
      else
...
procedure cxExportTLToHTMLStream(AStream: TStream; ATreeList: TcxCustomTreeList; AExpand: Boolean = True;
  ASaveAll: Boolean = True; AHandler: TObject = nil);
begin
  cxExportTLToStream(AStream, ATreeList, cxExportToHtml, AExpand, ASaveAll, False, [], AHandler);
end;
...
procedure cxExportTLToExcelStream(AStream: TStream; ATreeList: TcxCustomTreeList; AExpand: Boolean = True;
  ASaveAll: Boolean = True; AUseNativeFormat: Boolean = True; AHandler: TObject = nil);
begin
  cxExportTLToStream(AStream, ATreeList, cxExportToExcel, AExpand, ASaveAll, AUseNativeFormat, [], AHandler);
end;
...
procedure cxExportTLToXLSXStream(const AStream: TStream; ATreeList: TcxCustomTreeList;
  ...
  AHandler: TObject = nil);
begin
  cxExportTLToStream(AStream, ATreeList, cxExportToXlsx, AExpand, ASaveAll, AUseNativeFormat, [], AHandler);
end;
...
procedure cxExportTLToCSVStream(AStream: TStream; ATreeList: TcxCustomTreeList;
  ...
  AHandler: TObject = nil; AEncoding: TEncoding = nil);
begin
  cxExportTLToStream(AStream, ATreeList, cxExportToCSV, AExpand,
    ASaveAll, False, [ASeparator], AHandler, AEncoding);
end;
...
procedure cxExportTLToTextStream(AStream: TStream; ATreeList: TcxCustomTreeList; AExpand: Boolean;
  ASaveAll: Boolean = True; AHandler: TObject = nil; AEncoding: TEncoding = nil);
begin
  cxExportTLToTextStream(AStream, ATreeList, AExpand, ASaveAll, '', '', '', AHandler, AEncoding);
end;
...
procedure cxExportTLToTextStream(AStream: TStream; ATreeList: TcxCustomTreeList; AExpand: Boolean = True;
  ...
  AHandler: TObject = nil; AEncoding: TEncoding = nil);
begin
  cxExportTLToStream(AStream, ATreeList, cxExportToText, AExpand, ASaveAll, False,
    [ASeparator, ABeginString, AEndString], AHandler, AEncoding);
end;
...
procedure DoExportTLToStream(AStream: TStream; ATreeList: TcxCustomTreeList; AExportType: Integer;
  AExpand, ASaveAll, AUseNativeFormat: Boolean; const ASeparators: array of string;
  AHandler: TObject = nil; AEncoding: TEncoding = nil);
var
  ATreeListExport: TcxTreeListExportHelper;
  AEncodingIntf: IcxExportProviderEncoding;
  AIntf: IcxExportWithSeparators;
  I: Integer;
begin
  ATreeListExport := TcxTreeListExportHelper.Create(ATreeList, AExportType, AStream, AHandler);
  try
    ATreeListExport.Expand := AExpand;
    ATreeListExport.IsNativeFormat := AUseNativeFormat;
    ATreeListExport.SaveAll := ASaveAll;
    if cxTLExportLink.Supports(ATreeListExport.Provider, IcxExportProviderEncoding, AEncodingIntf) then
      AEncodingIntf.SetEncoding(AEncoding);
    if cxTLExportLink.Supports(ATreeListExport.Provider, IcxExportWithSeparators, AIntf) and (Length(ASeparators) > 0) then
    begin
      for I := Low(ASeparators) to High(ASeparators) do
        AIntf.AddSeparator(ASeparators[I]);
    end;
    ATreeListExport.DoExport;
  finally
    ATreeListExport.Free;
  end;
end;
...
begin
  ...
  try
    DoExportTLToStream(AFileStream, ATreeList, AExportType, AExpand, ASaveAll, AUseNativeFormat, ASeparators, AHandler, AEncoding);
  finally
    AFileStream.Free;
  end;
end;
...
begin
  if not ATreeList.Visible then
    cxTreeListError(scxExportNotVisibleControl);
  DoExportTLToStream(AStream, ATreeList, AExportType, AExpand, ASaveAll, AUseNativeFormat, ASeparators, AHandler, AEncoding);
end;
