...
const
  dxThisUnitName = 'dxEMF.DB.MSAccess';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxAccessConnectionProvider.Destroy', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FConnectionParameters);
  FreeAndNil(FDBTypes);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxAccessConnectionProvider.Destroy', SysInit.HInstance);{$ENDIF}
end;
...
begin
  Result := (ASourceString.StartsWith('PARA')) or (TRegex.Match(ASourceString, 'Forms]?!', [TRegexOption.roIgnoreCase]).Success);
end;
