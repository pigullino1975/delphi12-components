...
type
  ...
  private
    FBandsListSuffixOffset: Integer;
  ...
  protected
    procedure DrawListBoxItem(AListBox: TcxListBox; ACanvas: TcxCanvas; AIndex: Integer;
      R: TRect; AParentBand: TcxGridBand; var ASuffixOffset: Integer); reintroduce;
...
uses
  cxDesignWindows, Math, dxCore;

const
  dxThisUnitName = 'cxBandedTableViewEditor';
...
constructor TcxBandedTableViewEditor.Create(AOwner: TComponent);
begin
...
procedure TcxBandedTableViewEditor.DrawListBoxItem(AListBox: TcxListBox; ACanvas: TcxCanvas;
  AIndex: Integer; R: TRect; AParentBand: TcxGridBand; var ASuffixOffset: Integer);
...
begin
  ...
  begin
    if ASuffixOffset = 0 then
      ASuffixOffset := R.Left + GetParentBandInfoOffset;
    inherited DrawListBoxItem(AListBox, ACanvas, R, AListBox.Items[AIndex], GetParentBandInfoText, ASuffixOffset);
  end
  else
...
begin
  FBandsListSuffixOffset := 0;
  ListBoxLoadCollection(LBBands.InnerListBox, Bands);
  if LBBands.ItemIndex = LBBands.Count - 1 then
    LBBands.TopIndex := LBBands.TopIndex + 1;
end;
...
begin
  DrawListBoxItem(AControl, ACanvas, AIndex, ARect, TcxGridBandedColumn(View.Items[AIndex]).Position.Band, FColumnsListSuffixOffset);
end;
...
procedure TcxBandedTableViewEditor.LBBandsDrawItem(
  AControl: TcxListBox; ACanvas: TcxCanvas; AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
begin
  DrawListBoxItem(AControl, ACanvas, AIndex, ARect, Bands[AIndex].ParentBand, FBandsListSuffixOffset);
end;
