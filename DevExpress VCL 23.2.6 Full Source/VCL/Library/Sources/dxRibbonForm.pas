...
uses
  ...
  cxDWMApi, dxAnimation, dxCoreClasses, dxCustomFluentDesignForm, cxGeometry, cxLookAndFeels,
  dxFluentDesignFormInterfaces, cxAccessibility, dxGDIPlusClasses;
...
type
  ...
  { TdxRibbonFormAccessibilityHelper }

  TdxRibbonFormAccessibilityHelper = class(TcxAccessibilityHelper) // for internal use
  strict private
    function GetRibbonForm: TdxCustomRibbonForm;
  protected
    ...
    function GetHitTest(AScreenX: Integer; AScreenY: Integer; out AChildIndex: Integer): TcxAccessibleObjectHitTest; override;
  ...
  TdxCustomRibbonForm = class(TdxCustomFluentDesignForm,
  ...
  strict private
    ...
    function AreBordersThinForRoundedWindowCorners: Boolean;
    ...
    function CreateWindowRegion: HRGN;
    ...
    procedure InternalDrawNonClientArea(ADC: HDC; ADrawCaption, ADrawBorder: Boolean);
    ...
    procedure SetFormCorners(const Value: TdxFormCorners);
    ...
    procedure SetShowFormShadow(const Value: TdxDefaultBoolean);
  ...
  protected
    ...
    function IsNavigationControlVisible: Boolean;
    ...
    function NeedShowFormShadow: Boolean;
    ...
    procedure UpdateFormCorners;
    procedure UpdateFormShadow;
    procedure UpdateNonClientArea;
    procedure UpdateSkins;
    procedure UpdateSystemMenu;
    procedure UpdateWindowRegion; overload;
    procedure UpdateWindowRegion(AIsMaximized: Boolean); overload;
    ...
    // IdxRibbonFormPaintData
    function AreRoundedWindowCornersSupported: Boolean;
    ...
    function GetWindowCorners: TdxFormCorners;
    function GetWindowCornersRadius: Integer;
  ...
  public
    ...
    procedure ResetWindowRegion; // for internal use
    procedure ScaleForPPI(ATargetDPI: Integer; AWindowRect: PRect = nil); override;
    procedure SetRedraw(ARedraw: Boolean); // for internal use
    procedure UpdateBorders; // for internal use
    procedure UpdateColorScheme; virtual; // for internal use
    function UseNavigationControlCompatibleBorders: Boolean; // for internal use

    property DisableAero: Boolean read FDisableAero write SetDisableAero default False;
    property IAccessibilityHelper: IcxAccessibilityHelper read GetIAccessibilityHelper; // for internal use
    property IsActive: Boolean read FIsActive;
    property PrevActiveControl: TWinControl read FPrevActiveControl write SetPrevActiveControl; // for internal use
    ...
    property RibbonNonClientHelper: TdxRibbonFormCaptionHelper read FRibbonNonClientHelper write SetRibbonNonClientHelper; // for internal use
  published
...
uses
  ...
  dxRibbon, dxStatusBar, dxHooks, cxContainer, dxDPIAwareUtils, dxCoreGraphics, dxAcrylicEffect,
  cxLookAndFeelPainters, dxForms, dxThreading, dxTypeHelpers;

const
  dxThisUnitName = 'dxRibbonForm';
...
procedure DrawAcrylicNCPart(DC: HDC; const R: TRect; AMakeOpaque: Boolean; AProc: TDrawOnAcrylicProc);
var
  ACachedDC: HDC;
...
begin
  ...
  try
    ...
    if AMakeOpaque then
      BufferedPaintSetAlpha(APaintBuffer, @R, 255);
...
function TdxRibbonFormAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  Result := TdxCustomRibbonForm(OwnerObject).BoundsRect;
end;

function TdxRibbonFormAccessibilityHelper.ChildIsSimpleElement(AIndex: Integer): Boolean;
begin
  Result := False;
end;

function TdxRibbonFormAccessibilityHelper.GetChild(AIndex: Integer): TcxAccessibilityHelper;
begin
  Result := TdxCustomRibbon(RibbonForm.RibbonControl).BorderIconsAreaIAccessibilityHelper.GetHelper;
end;

function TdxRibbonFormAccessibilityHelper.GetChildCount: Integer;
begin
  if TdxCustomRibbonAccess(RibbonForm.RibbonControl).FormCaptionHelper <> nil then
    Result := 1
  else
    Result := 0;
end;

function TdxRibbonFormAccessibilityHelper.GetHitTest(AScreenX, AScreenY: Integer;
  out AChildIndex: Integer): TcxAccessibleObjectHitTest;
begin
  Result := aohtNone;
  if cxRectPtIn(GetChild(0).GetScreenBounds(cxAccessibleObjectSelfID), Point(AScreenX, AScreenY)) then
  begin
    AChildIndex := 0;
    Result := aohtChild;
  end;
...
function TdxRibbonFormAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := RibbonForm.Caption;
end;

function TdxRibbonFormAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxROLE_SYSTEM_WINDOW;
end;

function TdxRibbonFormAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxSTATE_SYSTEM_NORMAL;
end;

function TdxRibbonFormAccessibilityHelper.GetSupportedProperties(
  AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := [aopLocation];
end;

function TdxRibbonFormAccessibilityHelper.GetRibbonForm: TdxCustomRibbonForm;
begin
  Result := TdxCustomRibbonForm(OwnerObject);
end;
...
begin
  Result := DisableAero or (ParentWindow <> 0) or dxIsGdiScaledMode or
    IsWin10v1809OrLater and (Self <> Application.MainForm) and HandleAllocated and IsIconic(Handle); 
end;
...
function TdxCustomRibbonForm.GetWindowBordersWidth: TRect;
begin
  if UseNavigationControlCompatibleBorders then
    Result := cxSimpleRect
  else
    if UseSkin then
    begin
      with RibbonNonClientHelper.RibbonHelper do
        Result := GetSkin.GetWindowBordersWidth(HasStatusBar);
    end
    else
      Result := cxNullRect;
end;

function TdxCustomRibbonForm.GetWindowCorners: TdxFormCorners;
begin
  Result := fcRectangular;
  if AreRoundedWindowCornersSupported then
  begin
    Result := FormCorners;
...
function TdxCustomRibbonForm.GetWindowCornersRadius: Integer;
var
...
begin
  ...
  if AreRoundedWindowCornersSupported then
  begin
    ACorners := GetWindowCorners;
    if ACorners >= fcRounded then
      Result := ScaleFactor.Apply(dxStandardFormCornerRadius[ACorners])
    else
      if UseSkin and RibbonNonClientHelper.RibbonHelper.GetSkin.UseRoundedWindowCorners then
        Result := 4;
  end;
end;

function TdxCustomRibbonForm.AreRoundedWindowCornersSupported: Boolean;
var
  AIntf: IdxRibbonFormStatusBar;
begin
  Result := UseSkin and not IsUseAeroNCPaint and (IsWin11OrLater or not UseNavigationControlCompatibleBorders and
    (not AreBordersThinForRoundedWindowCorners or GetStatusBar(AIntf) or RibbonNonClientHelper.RibbonHelper.GetSkin.UseRoundedWindowCorners));
end;

procedure TdxCustomRibbonForm.NavigationControlChanged(AChanges: TdxFluentDesignNavigationControlChangeTypes);
begin
  if nchtVisibility in AChanges then
  begin
    UpdateColorScheme;
    FullUpdate;
  end;
...
begin
  ...
  if IsNavigationControlVisible then
  begin
    Rect.Left := NavigationControl.BoundsRect.Right;
    if UseNavigationControlCompatibleBorders then
      Inc(Rect.Left);
  end;
end;
...
begin
  ...
  try
    if IsAcrylic or UseNavigationControlCompatibleBorders then
    begin
      R := cxRectOffset(ClientRect, cxGetClientOffset(Handle));
      ExcludeClipRect(DC, R.Left, R.Top, R.Right, R.Bottom);
    end;
    if IsAcrylic then
    begin
      DrawAcrylicNCPart(DC, GetBounds, not UseNavigationControlCompatibleBorders,
        procedure (ADC: HDC; const R: TRect)
        begin
          InternalDrawNonClientArea(ADC, ADrawCaption, not IsZoomed(Handle));
        end );
    end
    else
      InternalDrawNonClientArea(DC, ADrawCaption, not IsZoomed(Handle));
  finally
    ReleaseDC(Handle, DC);
  end;
end;
...
function TdxCustomRibbonForm.IsAcrylicSupported: Boolean;
begin
  Result := inherited IsAcrylicSupported and (RootLookAndFeel.ActiveStyle = lfsSkin);
end;

function TdxCustomRibbonForm.IsNavigationControlVisible: Boolean;
begin
  Result := IsNavigationControlValid and NavigationControl.Visible;
end;
...
procedure TdxCustomRibbonForm.LookAndFeelChanged;
begin
  inherited;
  UpdateColorScheme;
end;
...
procedure TdxCustomRibbonForm.NavigationControlAssigned;
begin
  if NavigationControl <> nil then
  begin
    NavigationControl.Align := alNone;
    NavigationControl.Top := 0;
    NavigationControl.Left := 0;
    NavigationControl.Height := ClientHeight;
    NavigationControl.Anchors := [akLeft, akTop, akBottom];
  end;
  if UseSkin then
  begin
    UpdateColorScheme;
...
procedure TdxCustomRibbonForm.SetAcrylic(AValue: Boolean);
begin
...
begin
  ...
  try
    ...
    begin
      AControl := Controls[I];
      if (AControl <> NavigationControl) and (AControl.Align in [alNone, alCustom]) then
      begin
        ...
        begin
          ...
          AControl.Top := AControl.Top + ADelta;
      end;
    end;
  finally
    EnableAlign;
...
procedure TdxCustomRibbonForm.UpdateWindowRegion;
begin
...
begin
  ...
  begin
    if not AIsMaximized then
      ARgn := CreateWindowRegion
    else
      if FormStyle = fsMDIChild then
        ARgn := 0
      else
        ARgn := CreateRectRgnIndirect(cxRectContent(cxGetWindowBounds(Handle), GetDefaultWindowBordersWidth(Handle)));
...
function TdxCustomRibbonForm.GetStatusBar(out AStatusBar: IdxRibbonFormStatusBar): Boolean;
begin
  Result := (RibbonNonClientHelper <> nil) and RibbonNonClientHelper.RibbonHelper.GetStatusBar(AStatusBar);
end;

function TdxCustomRibbonForm.NeedShowFormShadow: Boolean;
var
  AShowShadow: TdxDefaultBoolean;
begin
  Result := False;
  if not BufferedShowing and UseSkin and not IsUseAeroNCPaint then
  begin
    AShowShadow := ShowFormShadow;
    if AShowShadow = bDefault then
      AShowShadow := dxSkinShowFormShadow;
    if AShowShadow = bDefault then
      AShowShadow := dxBooleanToDefaultBoolean(cxMarginsWidth(GetWindowBordersWidth) <= 2 * dxShadowWindowThinBorderSize);
    Result := AShowShadow = bTrue;
  end;
end;
...
function TdxCustomRibbonForm.AreBordersThinForRoundedWindowCorners: Boolean;
var
  ABorders: TRect;
begin
  ABorders := GetWindowBordersWidth;
  Result := Min(ABorders.Left, ABorders.Right) < TdxFormHelper.GetMinBorderWidth(ScaleFactor.Apply(dxStandardFormCornerRadius[fcRounded]));
end;
...
procedure TdxCustomRibbonForm.AfterResize(APrevRibbonVisible: Boolean; APrevRibbonHeight: Integer; AIsZoomed: Boolean);
  ...
  begin
    ...
    begin
      if AIsZoomed or IsWin10v1809OrLater then 
        ResetWindowRegion;
...
var
  H: Integer;
begin
  if GetWindowCornersRadius > 0 then
    H := dxGetSystemMetrics(SM_CYCAPTION, ScaleFactor)
  else
    H := FSizingBorders.cy;

  CalculateRegion(FCornerRegions[0],  FSizingBorders.cx,  FSizingBorders.cy, cxRect(0, 0, H, H));
  CalculateRegion(FCornerRegions[1], -FSizingBorders.cx,  FSizingBorders.cy, cxRect(Width - H, 0, Width, H));
  CalculateRegion(FCornerRegions[2], -FSizingBorders.cx, -FSizingBorders.cy, cxRect(Width - H, Height - H, Width, Height));
  CalculateRegion(FCornerRegions[3],  FSizingBorders.cx, -FSizingBorders.cy, cxRect(0, Height - H, H, Height));
end;
...
procedure TdxCustomRibbonForm.InternalDrawNonClientArea(ADC: HDC; ADrawCaption, ADrawBorder: Boolean);
var
  R: TRect;
begin
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          R := GetBounds;
          dxGPPaintCanvas.BeginPaint(ADC, R);
          dxGPPaintCanvas.FillRectangle(R, GetNavigationControl.GetBackgroundColor);
          dxGPPaintCanvas.FillRectangle(R, GetNavigationControl.GetBorderColor);
...
begin
  FShadow.CornerRadius := GetWindowCornersRadius;
  FShadow.ResizeOwnerWindowUsingShadow := AllowResize;
...
procedure TdxCustomRibbonForm.SetShowFormShadow(const Value: TdxDefaultBoolean);
begin
  if FShowFormShadow <> Value then
  begin
    FShowFormShadow := Value;
    UpdateFormShadow;
  end;
end;
...
procedure TdxCustomRibbonForm.SetFormCorners(const Value: TdxFormCorners);
begin
  if FFormCorners <> Value then
  begin
    FFormCorners := Value;
    UpdateColorScheme;
    FullUpdate;
...
begin
  ...
  if GetStatusBar(AStatusBar) then
    AStatusBar.UpdateColorScheme;
end;

procedure TdxCustomRibbonForm.UpdateFormCorners;
begin
  if IsWin11OrLater and HandleAllocated then
  begin
    if IsZoomed(Handle) then 
      dxSetFormCorners(Handle, fcRectangular)
    else
      dxSetFormCorners(Handle, GetWindowCorners);

    TdxUIThreadSyncService.EnqueueInvokeInUIThread(Self, FShadow.Refresh);
  end;
end;

procedure TdxCustomRibbonForm.UpdateFormShadow;
begin
  FShadow.UpdateBounds;
  FShadow.UpdateVisibility;
end;

function TdxCustomRibbonForm.UseNavigationControlCompatibleBorders: Boolean;
begin
  Result := UseSkin and IsNavigationControlVisible and GetNavigationControl.GetAcrylicEnabled;
end;
...
var
  R, R1: TRect;
begin
  if UseSkin and (IsUseAeroNCPaint or cxIsDrawToMemory(Message) or not DoubleBuffered and not IsAcrylic) then
  begin
    R := ClientRect;
    //reduce flickering
    ...
    if UseNavigationControlCompatibleBorders then
    begin
      R1 := NavigationControl.BoundsRect;
      R1.Left := R1.Right;
      Inc(R1.Right);
      dxGPPaintCanvas.BeginPaint(Message.DC, R1);
      dxGPPaintCanvas.FillRectangle(R1, GetNavigationControl.GetBackgroundColor);
      dxGPPaintCanvas.FillRectangle(R1, GetNavigationControl.GetBorderColor);
      dxGPPaintCanvas.EndPaint;
      ExcludeClipRect(Message.DC, R1.Left, R1.Top, R1.Right, R1.Bottom);
    end;
...
procedure TdxCustomRibbonForm.WMGetObject(var Message: TMessage);
var
  AObjectID: Cardinal;
begin
  if (RibbonControl <> nil) and (TdxCustomRibbonAccess(RibbonControl).FormCaptionHelper <> nil) and
    not TdxRibbonFormAccessibilityHelper(IAccessibilityHelper.GetHelper).Lock and CanReturnAccessibleObject(Message) then
  begin
    AObjectID := Cardinal(Message.LParam);
    if AObjectID = OBJID_CLIENT then
    begin
      TdxRibbonFormAccessibilityHelper(IAccessibilityHelper.GetHelper).SetInternalChild(GetHandle);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          PostMessage(Handle, WM_SIZE, 0, 0);
          if (BackgroundBlur = TdxFluentDesignFormBackgroundBlur.None) and EnableAcrylicEffects then
            TdxUIThreadSyncService.EnqueueInvokeInUIThread(Self,
              procedure
              begin
                if (BackgroundBlur = TdxFluentDesignFormBackgroundBlur.None) and IsAcrylic and Active then
                begin
                  ApplyAcrylicAccent(False, 0);
                  ApplyAcrylicAccent(True, GetCurrentAcrylicColor(True));
                end;
              end
              );
        end;
...
begin
  ...
  if UseSkin then
  begin
    FSizingBorders.cx := dxGetSystemMetrics(SM_CXSIZEFRAME, ScaleFactor);
    FSizingBorders.cy := dxGetSystemMetrics(SM_CYSIZEFRAME, ScaleFactor);
  end;
...
begin
  if not UseSkin then
  begin
    if (Message.Msg = WM_NCACTIVATE) and IsAcrylic then
    begin
...
function TdxCustomRibbonForm.CreateWindowRegion: HRGN;
var
  ARadius: Integer;
...
begin
  Result := 0;
  if HandleAllocated and GetWindowRect(Handle, AWindowRect) then
  begin
    AWidth := cxRectWidth(AWindowRect);
    AHeight := cxRectHeight(AWindowRect);
    ARadius := GetWindowCornersRadius;
    if (ARadius > 0) and not (IsWin11OrLater and (GetWindowCorners >= fcRounded)) then
    begin
      Result := TdxFormHelper.CreateRoundedRegion(cxRect(0, 0, AWidth, AHeight), ARadius);
      if IsRectangularFormBottom(Self) then
        dxCombineRectRegion(Result, 0, 2 * ARadius, AWidth + 1, AHeight + 1, RGN_OR)
    end
    else
...
begin
  ...
  begin
    if not IsUseAeroNCPaint and IsNormalWindowState then
      SetRegion(CreateWindowRegion, True);
    RecalculateCaptionArea;
  end;
end;
...
function TdxCustomRibbonForm.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin
  if FIAccessibilityHelper = nil then
    FIAccessibilityHelper := TdxRibbonFormAccessibilityHelper.Create(Self);
...
procedure TdxRibbonFormShadowWindow.DrawShadow(ACanvas: TdxGPCanvas);
var
  ABottomPartHeight: Integer;
  ARect: TRect;
begin
  if IsRectangularFormBottom(OwnerWindow) and (CornerRadius > 0) then
  begin
    ABottomPartHeight := 2 * CornerRadius + 1;

    ACanvas.SaveClipRegion;
    try
      ARect := ClientRect;
      ARect.Bottom := ARect.Bottom - ABottomPartHeight;
      ACanvas.SetClipRect(ARect, gmIntersect);
    ...
    try
      ARect := ClientRect;
      ARect.Top := ARect.Bottom - ABottomPartHeight;
      ACanvas.SetClipRect(ARect, gmIntersect);
      DrawShadowCore(ACanvas, 0);
    finally
      ACanvas.RestoreClipRegion;
    end;
  end
...
begin
  Result := inherited CalculateVisibility and OwnerWindow.NeedShowFormShadow;
end;
...
begin
  AControl := nil;
  if RibbonForm.GetStatusBar(AIntf) then
