...
unit dxRichEdit.Import.Rtf.DestinationInfo; // for internal use
...
type
  ...
  private
    class procedure LegacyPasswordHashHandler(AImporter: TdxRtfImporter; AParameterValue: Integer; AHasParameter: Boolean); static;
    class procedure PasswordHashHandler(AImporter: TdxRtfImporter; AParameterValue: Integer; AHasParameter: Boolean); static;
  protected
...
uses
  dxHash,
  dxRichEdit.DocumentModel.ProtectionFormatting, dxCore;

const
  dxThisUnitName = 'dxRichEdit.Import.Rtf.DestinationInfo';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxInfoDestination.Initialize', SysInit.HInstance);{$ENDIF}
  FKeywordHT := CreateKeywordHT;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxInfoDestination.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxInfoDestination.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxInfoDestination.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FKeywordHT);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxInfoDestination.Finalize', SysInit.HInstance);{$ENDIF}
end;
