...
{$ENDIF}
  ...
  cxMemo, cxButtonEdit, dxCoreClasses, dxSpreadSheetReferenceEditDialog;
...
type
  ...
  TdxSpreadSheetPageSetupDialogForm = class(TdxSpreadSheetReferenceEditDialogForm)
    btnCancel: TcxButton;
  ...
  protected
    ...
    procedure PopulateHeaderFooterTemplates(AStrings: TStrings); overload; virtual;
    procedure PopulateHeaderFooterTemplates; overload; virtual;
    ...
    procedure SelectionChanged(Sender: TdxSpreadSheetCustomView); override;
    ...
    function TryStringToReference(const S: string; out AView: TdxSpreadSheetTableView; out AArea: TRect): Boolean; overload; override;
    function TryStringToReference(const S: string; AArea: TdxSpreadSheetTableViewOptionsPrintRect): Boolean; reintroduce; overload;
    function ValidateReference(const S: string): Boolean; override;

    function FlipTemplate(const S: string): string;
    procedure FlipTemplates(AComboBox: TcxComboBox);
...
uses
  dxSpreadSheetDialogStrs, dxCore, dxSpreadSheetUtils, dxSpreadSheetCoreReferences, StrUtils, Math,
  dxPrintUtils, dxTypeHelpers, cxFormats, dxPrinting, dxSpreadSheetPageSetupHeaderFooterDialog,
  dxCoreGraphics, cxVariants, dxSpreadSheetCoreHistory;

const
  dxThisUnitName = 'dxSpreadSheetPageSetupDialog';
...
const
  BuildInTemplates: array[0..8] of string = (
    '%0:sPage &P%0:s',
    '%0:sPage &P of &N%0:s',
    '%0:s&A%0:s',
...
begin
  ...
  begin
    SpreadSheet.History.BeginAction(TdxSpreadSheetHistoryChangePrintingOptionsAction);
    try
      SpreadSheet.History.AddCommand(TdxSpreadSheetHistoryChangePrintingOptionsCommand.Create);
      Options.BeginUpdate;
      try
        Save;
      finally
...
begin
  if Sender = ActiveSheet then
    beAreaSelector.Text := AreaToString(ActiveSheet.Selection.Area);
end;
...
begin
  ...
  if Length(AParts) >= 3 then
    SetHeaderFooterText(AParts[IfThen(IsRightToLeft, 2, 0)], AParts[1], AParts[IfThen(IsRightToLeft, 0, 2)], AText)
  else
...
procedure TdxSpreadSheetPageSetupDialogForm.DrawHeaderFooterPreview(ACanvas: TCanvas;
  ...
  begin
    Result := cxTextWidth(ACanvas.Font, TdxSpreadSheetHeaderFooterMacroPreview.Evaluate(S, ActiveSheet));
  end;
  ...
  begin
    ...
    ACanvas.DrawTexT(TdxSpreadSheetHeaderFooterMacroPreview.Evaluate(S, ActiveSheet),
...
begin
  ...
  try
    ...
    try
      ...
      DrawSection(cxPaintCanvas, ARect, IfThen(IsRightToLeft, AText.RightSection, AText.LeftSection), taLeftJustify);
      ...
      DrawSection(cxPaintCanvas, ARect, IfThen(IsRightToLeft, AText.LeftSection, AText.RightSection), taRightJustify);
    finally
      ACalculator.Free;
    end;
  finally
...
procedure TdxSpreadSheetPageSetupDialogForm.PopulateHeaderFooterTemplates;
begin
...
begin
  ...
  try
    AStrings.Clear;
    AStrings.Add('');
    for I := Low(BuildInTemplates) to High(BuildInTemplates) do
    begin
      ATemplate := Format(BuildInTemplates[I], [SectionsDelimiter, AUserName]);
      if IsRightToLeft then
        ATemplate := FlipTemplate(ATemplate);
      AStrings.Add(ATemplate);
    end;
  finally
    AStrings.EndUpdate;
  end;
...
begin
  TryStringToReference(tePrintArea.Text, Options.Source.Area);
  TryStringToReference(teRowsToRepeat.Text, Options.Source.RowsToRepeat);
  TryStringToReference(teColumnsToRepeat.Text, Options.Source.ColumnsToRepeat);
...
function TdxSpreadSheetPageSetupDialogForm.TryStringToReference(
  const S: string; out AView: TdxSpreadSheetTableView; out AArea: TRect): Boolean;
begin
  Result := inherited TryStringToReference(S, AView, AArea) and (AView = Sheet);
end;

function TdxSpreadSheetPageSetupDialogForm.TryStringToReference(
  const S: string; AArea: TdxSpreadSheetTableViewOptionsPrintRect): Boolean;
var
  AAreaRect: TRect;
...
begin
  ...
  if Result then
  begin
    AArea.Rect := AAreaRect;
    AArea.Assigned := True;
  end;
end;

function TdxSpreadSheetPageSetupDialogForm.ValidateReference(const S: string): Boolean;
var
  AArea: TdxSpreadSheetTableViewOptionsPrintRect;
begin
  AArea := TdxSpreadSheetTableViewOptionsPrintRect.Create(nil);
  try
    Result := TryStringToReference(S, AArea);
  finally
    AArea.Free;
  end;
end;

function TdxSpreadSheetPageSetupDialogForm.FlipTemplate(const S: string): string;
var
  AParts: TStringDynArray;
begin
  AParts := SplitString(S, SectionsDelimiter);
  if Length(AParts) = 3 then
    Result := AParts[2] + SectionsDelimiter + AParts[1] + SectionsDelimiter + AParts[0]
  else
    Result := S;
end;

procedure TdxSpreadSheetPageSetupDialogForm.FlipTemplates(AComboBox: TcxComboBox);
var
  ASavedIndex: Integer;
  I: Integer;
begin
  ...
  try
    for I := 0 to AComboBox.Properties.Items.Count - 1 do
      AComboBox.Properties.Items[I] := FlipTemplate(AComboBox.Properties.Items[I]);
  finally
    AComboBox.Properties.Items.EndUpdate;
    AComboBox.ItemIndex := ASavedIndex;
  end;
end;

procedure TdxSpreadSheetPageSetupDialogForm.CMBiDiModeChanged(var Message: TMessage);
begin
  inherited;
  FlipTemplates(cbHeaderTemplates);
  FlipTemplates(cbFooterTemplates);
end;

function TdxSpreadSheetPageSetupDialogForm.GetOptions: TdxSpreadSheetTableViewOptionsPrint;
begin
  Result := ActiveSheet.OptionsPrint;
end;
...
begin
  ...
  try
    ...
    if ShowPageHeaderFooterSetupDialog(ActiveSheet, AOptions) then
    begin
...
begin
  ...
  try
    TdxImageDrawer.DrawImage(cxPaintCanvas, TPaintBox(Sender).ClientRect, nil,
...
begin
  ...
  StartEditing(AEdit, TdxSpreadSheetTableViewSelectionMode(AEdit.Tag));
end;

procedure TdxSpreadSheetPageSetupDialogForm.tePrintAreaExit(Sender: TObject);
begin
  FinishEditing(Sender as TcxButtonEdit);
end;
