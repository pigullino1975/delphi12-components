...
uses
  ...
  cxEdit, cxEditConsts, ImgList, dxZoomTrackBar, cxTrackBar, dxGDIPlusApi, dxGDIPlusClasses, dxScrollAnimation;
...
type
  ...
  public
    ...
    destructor Destroy; override;
    procedure DrawBorderedBackground(ACanvas: TcxCanvas; const R: TRect); override;
    function GetUnscaledPadding(AUsePainterPadding: Boolean): TdxPadding; override;
    function SupportsBorders: Boolean; override;
    function SupportsPadding: Boolean; override; // for internal use //#lcm: WXI
    //
    property ZoomTrackBarViewInfo: TdxCustomZoomTrackBarViewInfo read FZoomTrackBarViewInfo; // for internal use
  end;
  ...
  { TcxImageCustomScrollAnimationHelper }

  TcxImageCustomScrollAnimationHelper = class(TdxScrollAnimationHelper)
  strict private
    FOwner: TcxCustomImage;
  protected
    procedure DoUpdateOwnerPosition(AValue: Integer); virtual; abstract;
    procedure UpdateOwnerPosition(AValue: Integer); override;
    property Owner: TcxCustomImage read FOwner;
  public
    constructor Create(AOwner: TcxCustomImage); virtual;
  end;

  { TcxImageVertScrollAnimationHelper }

  TcxImageVertScrollAnimationHelper = class(TcxImageCustomScrollAnimationHelper)
  private
  protected
    procedure CheckPosition(var AValue: Integer); override;
    procedure DoUpdateOwnerPosition(AValue: Integer); override;
    function GetOwnerPosition: Integer; override;
  end;

  { TcxImageHorzScrollAnimationHelper }

  TcxImageHorzScrollAnimationHelper = class(TcxImageCustomScrollAnimationHelper)
  protected
    procedure CheckPosition(var AValue: Integer); override;
    procedure DoUpdateOwnerPosition(AValue: Integer); override;
    function GetOwnerPosition: Integer; override;
  ...
  private
    ...
    FHorzScrollAnimationHelper: TcxImageHorzScrollAnimationHelper;
    FInternalPopupMenu: TPopupMenu;
    FIsDialogShowed: Boolean;
    FLastDragPos: TPoint;
    FOnGetGraphicClass: TcxImageEditGraphicClassEvent;
    FPicture: TPicture;
    FVertScrollAnimationHelper: TcxImageVertScrollAnimationHelper;
    FZoomPercent: Integer;
  ...
  public
    ...
    property ViewInfo: TcxImageViewInfo read GetViewInfo; // for internal use
  end;
...
uses
  Math, cxEditUtils, dxCameraControl, dxCameraDialog, dxSmartImage, dxShellDialogs;

const
  dxThisUnitName = 'cxImage';
    ...
    begin
      ...
      begin
        if dxUseVectorIcons then
          AImage.LoadFromResource(HInstance, GetResourceName(APopupMenuItem), 'SVG')
        else
          AImage.LoadFromResource(HInstance, GetResourceName(APopupMenuItem), RT_BITMAP);
        TcxImageList(FPopupMenuImageList).Add(AImage);
      end;
    end;
  finally
    AImage.Free;
...
constructor TcxImageCustomScrollAnimationHelper.Create(AOwner: TcxCustomImage);
begin
  inherited Create;
  FOwner := AOwner;
end;

procedure TcxImageCustomScrollAnimationHelper.UpdateOwnerPosition(AValue: Integer);
begin
  Owner.UpdateScrollBars;
  DoUpdateOwnerPosition(AValue);
  Owner.Update;
  Owner.Invalidate;
end;
...
procedure TcxImageVertScrollAnimationHelper.CheckPosition(var AValue: Integer);
begin
  AValue := Max(Min(AValue, Owner.GetContentSize.cy - cxSize(Owner.ClientBounds).cy), 0);
end;

procedure TcxImageVertScrollAnimationHelper.DoUpdateOwnerPosition(
  AValue: Integer);
begin
  Owner.SetTopPos(AValue);
end;

function TcxImageVertScrollAnimationHelper.GetOwnerPosition: Integer;
begin
  Result := Owner.GetTopPos;
end;

{ TcxImageHorzScrollAnimationHelper }

procedure TcxImageHorzScrollAnimationHelper.CheckPosition(var AValue: Integer);
begin
  AValue := Max(Min(AValue, Owner.GetContentSize.cx - cxSize(Owner.ClientBounds).cx), 0);
end;

procedure TcxImageHorzScrollAnimationHelper.DoUpdateOwnerPosition(AValue: Integer);
begin
  Owner.SetLeftPos(AValue);
end;

function TcxImageHorzScrollAnimationHelper.GetOwnerPosition: Integer;
begin
  Result := Owner.GetLeftPos;
end;
...
begin
  ...
  FHorzScrollAnimationHelper := TcxImageHorzScrollAnimationHelper.Create(Self);
  FVertScrollAnimationHelper := TcxImageVertScrollAnimationHelper.Create(Self);
...
begin
  ...
  begin
    dx := FLastDragPos.X - X;
    dy := FLastDragPos.Y - Y;
    if FVertScrollAnimationHelper.IsAnimationActive then
      FVertScrollAnimationHelper.StopAnimation(False);
    if FHorzScrollAnimationHelper.IsAnimationActive then
      FHorzScrollAnimationHelper.StopAnimation(False);
...
var
  AScrollAnimationHelper: TcxImageCustomScrollAnimationHelper;
  APos, ADelta: Integer;
begin
  if AScrollBarKind = sbHorizontal then
    AScrollAnimationHelper := FHorzScrollAnimationHelper
  else
    AScrollAnimationHelper := FVertScrollAnimationHelper;
  if AScrollAnimationHelper.IsAnimationActive and (AScrollCode = scTrack) then
    AScrollAnimationHelper.StopAnimation(False);

  if IsScrollAnimationEnabled and (AScrollCode in [scLineUp..scPageDown]) then
  begin
    if AScrollAnimationHelper.IsAnimationActive then
      APos := AScrollAnimationHelper.TargetPosition
    else
      APos := AScrollAnimationHelper.GetOwnerPosition;
    ADelta := 0;
    case AScrollCode of
      scLineUp:
        ADelta := - ScrollStep;
      scLineDown:
        ADelta := ScrollStep;
      scPageUp:
        ADelta := - GetPageScrollStep;
      scPageDown:
        ADelta := GetPageScrollStep;
    end;
    AScrollAnimationHelper.SetPosition(APos + ADelta);
    AScrollPos := AScrollAnimationHelper.GetOwnerPosition;
  end
  else
...
var
  ADialog: TdxOpenPictureDialog;
begin
  if not CanModify then
    Exit;
  ADialog := TdxOpenPictureDialog.Create(nil);
  try
    FIsDialogShowed := True;
    ADialog.Filter := GetDialogFilter;
    if ExecuteEmbeddedDialog(ADialog, Handle) and DoEditing then
    begin
...
var
  ADialog: TdxSavePictureDialog;
begin
  if (FPicture = nil) or (FPicture.Graphic = nil) then
    Exit;
  ADialog := TdxSavePictureDialog.Create(Application);
  FIsDialogShowed := True;
  try
    if ActiveProperties.CustomFilter <> '' then
...
function TcxImageViewInfo.GetUnscaledPadding(AUsePainterPadding: Boolean): TdxPadding;
begin
  if AUsePainterPadding and not IsInplace and not Transparent and (BorderStyle <> ebsNone) then
    Result := Painter.GetImageEditorPadding(IsInplace, nil)
  else
    Result := TdxPadding.Null;
end;

procedure TcxImageViewInfo.DrawBorderedBackground(ACanvas: TcxCanvas; const R: TRect);
var
  [unsafe] APalette: IdxColorPalette;
begin
  GetBackgroundStyleColorPalette(APalette);
  Painter.DrawScaledImageEditorBackground(ACanvas, R, IsInplace, GetDrawReadOnly, BackgroundColor, BorderState,
    ScaleFactor, APalette);
end;
...
begin
  ...
  if not Painter.SupportsImageEditorPadding then
  begin
...
function TcxImageViewInfo.SupportsBorders: Boolean;
begin
  Result := not SupportsPadding;
end;

function TcxImageViewInfo.SupportsPadding: Boolean;
begin
  Result := (Painter <> nil) and Painter.SupportsImageEditorPadding;
end;
...
begin
  ...
  if AImageViewInfo.Center or AProperties.Stretch then
    AImageViewInfo.TopLeft := TPoint.Null;
end;
...
begin
  with TcxImageViewInfo(AViewInfo) do
    ...
    begin
      ...
      if (TcxCustomImageProperties(Properties).GraphicTransparency <> gtDefault) and (Picture.Graphic <> nil) then
        Picture.Graphic.Transparent := TcxCustomImageProperties(Properties).GraphicTransparency = gtTransparent;
    end
    else
...
procedure TcxImageViewData.CalculateZoomTrackBar(ACanvas: TcxCanvas; const ABounds: TRect; const P: TPoint;
...
begin
  ARect := cxRectCenterHorizontally(ABounds, Min(ScaleFactor.Apply(200), ABounds.Width));
  ARect := cxRectSetHeight(ARect, ScaleFactor.Apply(31));
  ARect := cxRectSetTop(ARect, Min(ABounds.Height - ScaleFactor.Apply(40), Round(7 / 8 * ABounds.Height)));
