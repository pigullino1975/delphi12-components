...
type
  ...
  protected
    function BarBrushColor: TColor; virtual;
  ...
  TdxBarInPlaceSubItemEvent = procedure(Sender: TdxBarInPlaceSubItem; Link: TdxBarItemLink) of object;
  ...
  TdxBarInPlaceSubItemControlAccessibilityHelper = class(TdxBarSubItemControlAccessibilityHelper) // for internal use
  protected
...
uses
  Math, ActiveX, Printers, dxBarStrs, dxCoreGraphics, dxGDIPlusClasses, dxSkinInfo,  cxLookAndFeelPainters,
  cxClasses, cxGraphics, cxFormats, cxLookAndFeels, dxDPIAwareUtils, cxFontNameComboBox;

const
  dxThisUnitName = 'dxBarExtItems';
  ...
  begin
    if (cpIcon in DrawParams.ViewStructure) then
      if DrawParams.PaintType <> ptMenu then
        ...
        Result := Painter.SubMenuControlGetItemTextIndent(DrawParams, Painter) + GetCaptionWidth + 4
    else
...
procedure TdxBarFontNameCombo.DrawItem(APainter: TdxBarPainter; AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
var
  AImageSize: TSize;
  ...
  if Assigned(OnDrawItem) or (AIndex = -1) then
    ...
    begin
      if Integer(Items.Objects[AIndex]) and TRUETYPE_FONTTYPE <> 0 then
        ACurrentGlyph := FTrueTypeFontGlyph
      else
        ACurrentGlyph := FNonTrueTypeFontGlyph;
      AImageSize := dxGetImageSize(FTrueTypeFontGlyph, TBarManagerAccess(BarManager).ScaleFactor);
      R.Left := Left;
      R.Top := (Top + Bottom - AImageSize.cy) div 2;
      R.Size := AImageSize;
      TdxImageDrawer.DrawUncachedImage(Handle, R, ARect, ACurrentGlyph, nil, -1, idmNormal, False, Brush.Handle, clNone, False, APainter.LookAndFeelPainter.DefaultContentGlyphColorPalette(cxbsNormal));
      S := Items[AIndex];
...
begin
  FontLoader.PopulateFonts(Self, nil, Items, [cxftTTF..cxftSymbol]);
end;

procedure TdxBarFontNameCombo.MeasureItemWidth(AIndex: Integer; var AWidth: Integer);
begin
  inherited;
  Inc(AWidth, dxGetImageSize(FTrueTypeFontGlyph, TBarManagerAccess(BarManager).ScaleFactor).cx + 1);
end;
...
procedure AMonthListBoxTimerProc(Wnd: HWnd; Msg: UINT; TimerID: UINT_PTR; SysTime: DWORD); stdcall;
var
  AControl: TAMonthListBox;
  Year, Month, Day: Word;
begin
  AControl := TAMonthListBox(FindControl(Wnd));
  DecodeDate(AControl.TopDate, Year, Month, Day);
  dxChangeMonth(Year, Month, 2 * Integer(TimerID > 5) - 1);
  AControl.TopDate := EncodeDate(Year, Month, 1);
end;
...
begin
  ...
  begin
    ...
    ANewTimerId := Sign + Delta div 12;
    if Abs(ANewTimerId) > 4 then
      ANewTimerId := Sign * 4;
    ANewTimerId := ANewTimerId + 5;
    if (FTimer = 0) or (ANewTimerId <> FTimerId) then
    begin
      FreeTimer;
      FTimerId := ANewTimerId;
      FTimer := dxSetTimer(Handle, FTimerId, Times[Abs(FTimerId - 5)], AMonthListBoxTimerProc);
    end;
  end;    
end;
...
procedure ADateNavigatorTimerProc(Wnd: HWnd; Msg: UINT; TimerID: UINT_PTR; SysTime: DWORD); stdcall;
var
  AControl: TdxBarDateNavigator;
  P: TPoint;
begin
  AControl := TdxBarDateNavigator(FindControl(Wnd));
  P := AControl.ScreenToClient(GetMouseCursorPos);
  case TimerID of
    1: if PtInRect(AControl.GetLeftArrowRect, P) then AControl.StepToPast;
    2: if PtInRect(AControl.GetRightArrowRect, P) then AControl.StepToFuture;
  end;
end;
  ...
  begin
    ...
    begin  // show month's list box
      ...
      begin  // shift by month to past
        StepToPast;
        if FTimer = 0 then
          FTimer := dxSetTimer(Handle, 1, ADateNavigatorTime, ADateNavigatorTimerProc);
      end
      else
        ...
        begin  // shift by month to future
          StepToFuture;
          if FTimer = 0 then
            FTimer := dxSetTimer(Handle, 2, ADateNavigatorTime, ADateNavigatorTimerProc);
        end;
  end;
end;
...
procedure TdxBarTreeViewCombo.SetBarManager(Value: TdxBarManager);
begin
  inherited;
...
begin
  inherited;
  AKeySet := [dxVKBack, dxVKCopy, dxVKPaste, dxVKCut, '0'..'9'];
  if FMinValue < 0 then
    Include(AKeySet, '-');
    ...
    case Msg of
          ...
          begin
            ...
            try
              ...
              if (ALinkSelf.Ref <> nil) and (GetCapture = Handle) then
                FTimerID := dxSetTimer(Handle, 1, GetDoubleClickTime - 100, nil);
            finally
              cxRemoveObjectLink(ALinkSelf);
            end;
          end;
      ...
      WM_TIMER:
        case wParam of
          1: begin
               dxKillTimer(Handle, FTimerID);
               FTimerID := dxSetTimer(Handle, 2, 100, nil);
             end;
...
function TdxBarControlContainerControl.GetControlDPI: Integer;
  ...
  function GetCurrentDPIEx(AComponent: TComponent): Integer;
  var
    M, D: Integer;
  begin
    if dxGetCurrentScaleFactor(AComponent, M, D) then
      Result := MulDiv(dxDefaultDPI, M, D)
    else
    begin
      if FLastKnownControlDPI <> 0 then
        Result := FLastKnownControlDPI
      else
        Result := dxDefaultDPI;
    end;
  end;
...
begin
  if Supports(Control, IdxScaleFactor, AIntf) then
    ...
    if (Place <> nil) and FPlacedControl then
      ...
      Result := GetCurrentDPIEx(Control);
  FLastKnownControlDPI := Result;
end;
...
begin
  ...
  begin
    ...
    try
      Place.ForceScaleForPPI(ScaleFactor.Apply(dxDefaultDPI));
...
function TdxBarProgressControl.BarBrushColor: TColor;
begin
