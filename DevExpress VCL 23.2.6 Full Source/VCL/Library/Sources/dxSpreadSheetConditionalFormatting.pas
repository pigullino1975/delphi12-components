...
type
  ...
  public
    ...
    function CalculateStyle(const AStyle: TdxSpreadSheetCellDisplayStyle; ARow, AColumn: Integer; ACell: IdxSpreadSheetCellData): Boolean; // for internal use
    procedure Clear;
  ...
  protected
    procedure AreasChangeHandler(Sender: TObject); virtual;
  ...
  strict private
    ...
    function GetCellCountInAreas: Integer;
  ...
  protected
    ...
    function GetSupportedDrawingStages: TdxSpreadSheetDrawingStages; virtual;
  public
    ...
    property SupportedDrawingStages: TdxSpreadSheetDrawingStages read GetSupportedDrawingStages;
    property UseRightToLeftLayout: Boolean read FUseRightToLeftLayout write FUseRightToLeftLayout;
  end;
...
const
  dxThisUnitName = 'dxSpreadSheetConditionalFormatting';
...
begin
  ...
  begin
    FDuplicates := TdxVariantSet.Create(Min(GetCellCountInAreas, MaxInitialCapacity));
    PopulateDuplicateValues;
  end;
  Result := not FDuplicates.Contains(AValue);
end;
...
function TdxSpreadSheetConditionalFormattingAreaInfo.GetCellCountInAreas: Integer;
var
  I: Integer;
  R: TRect;
begin
  ...
  for I := 0 to Areas.Count - 1 do
  begin
    R := dxSpreadSheetGetRealArea(Areas[I]);
    Inc(Result, dxSpreadSheetAreaWidth(R) * dxSpreadSheetAreaHeight(R));
  end;
  Result := Max(Result, 0);
end;
...
var
  ATempList: TdxVariantSet;
begin
  ATempList := TdxVariantSet.Create(Min(GetCellCountInAreas, MaxInitialCapacity));
  try
    EnumCells(
      procedure (const ACell: IdxSpreadSheetCellData)
      var
      ...
      begin
        ...
        begin
          if ATempList.Contains(AValue) then
            FDuplicates.Include(AValue)
          else
            ATempList.Include(AValue);
        end;
      end);
  finally
    ATempList.Free;
...
begin
  ...
  begin
    ...
    begin
      FDataBarAxisBounds.Right := GetPointPosition(DataBar.AxisPosition);
      FDataBarAxisBounds.Left := FDataBarAxisBounds.Right - AxisThickness;
    end
    else
    begin
      FDataBarAxisBounds.Left := GetPointPosition(DataBar.AxisPosition);
      FDataBarAxisBounds.Right := FDataBarAxisBounds.Left + AxisThickness;
    end;
...
begin
  ...
  begin
    ...
    if UseRightToLeftLayout then
    begin
      FIconBounds := cxRectSetRight(R, R.Right, AImageSetSize.cx);
      FIconBounds := cxRectCenterVertically(FIconBounds, AImageSetSize.cy);
      R.Right := FIconBounds.Left - cxTextSpace;
    end
    else
    begin
...
begin
  ...
  if AStage = dsSecond then
    TdxImageDrawer.DrawImage(ACanvas, IconBounds, nil, ConditionalFormattingIconSet.Icons, Style.IconIndex, True, nil, ScaleFactor);
end;
...
function TdxSpreadSheetConditionalFormattingStyleViewInfo.GetSupportedDrawingStages: TdxSpreadSheetDrawingStages;
begin
  Result := [];
  if not cxRectIsEmpty(DataBarBounds) then
    Include(Result, dsFirst);
  if not cxRectIsEmpty(IconBounds) then
