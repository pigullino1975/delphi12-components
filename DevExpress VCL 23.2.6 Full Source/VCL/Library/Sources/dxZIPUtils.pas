...
unit dxZIPUtils; // for internal use
...
uses
  System.ZLib,
  Windows, Classes, SysUtils, RTLConsts, dxCore, dxCoreClasses, cxClasses, AnsiStrings, Generics.Defaults, Generics.Collections;
...
type
  ...
  public
    class function AbsoluteFileName(const ACurrentPath, AFileName: AnsiString): AnsiString; overload; static;
    class function AbsoluteFileName(const ACurrentPath, AFileName: string): string; overload; static;
    class function DecodePath(const APath: AnsiString): AnsiString; overload; static;
    class function DecodePath(const APath: string): string; overload; static;
    class function EncodePath(const APath: AnsiString): AnsiString; overload; static;
    class function EncodePath(const APath: string): string; overload; static;
    class function ExcludeRootPathDelimiter(const APath: AnsiString): AnsiString; overload; static;
    class function ExcludeRootPathDelimiter(const APath: string): string; overload; static;
    class function ExcludeTrailingPathDelimiter(const APath: AnsiString): AnsiString; overload;static;
    class function ExcludeTrailingPathDelimiter(const APath: string): string; overload; static;
    class function ExpandFileName(const AFileName: AnsiString): AnsiString; overload; static;
    class function ExpandFileName(const AFileName: string): string; overload; static;
    class function ExtractFilePath(const AFileName: AnsiString): AnsiString; overload; static;
    class function ExtractFilePath(const AFileName: string): string; overload; static;
    class function IsAbsoluteFileName(const AFileName: AnsiString): Boolean; overload; static;
    class function IsAbsoluteFileName(const AFileName: string): Boolean; overload; static;
    class function RelativePath(const ACurrentPath, AFileName: AnsiString): AnsiString; overload; static;
    class function RelativePath(const ACurrentPath, AFileName: string): string; overload; static;
  end;
...
begin
  ...
  begin
    TMemoryStreamAccess(ATargetStream).Capacity :=
      Max(TMemoryStreamAccess(ATargetStream).Capacity, ATargetPosition + AEntry.OriginalSize);
  end;
...
begin
  ...
  begin
    dxTestCheck(AFreeStream, 'The AFreeStream argument must set to True for the TdxCompressedStream instance');
...
class function TdxZIPPathHelper.AbsoluteFileName(const ACurrentPath, AFileName: string): string;
begin
  if IsAbsoluteFileName(AFileName) then
    Result := Copy(AFileName, 2, MaxInt)
  else if AFileName <> '' then
...
class function TdxZIPPathHelper.DecodePath(const APath: string): string;
begin
...
class function TdxZIPPathHelper.EncodePath(const APath: string): string;
begin
  Result := dxReplacePathDelimiter(APath, PathDelim, dxUnixPathDelim);
end;
...
class function TdxZIPPathHelper.ExcludeRootPathDelimiter(const APath: string): string;
begin
  Result := APath;
  if (Result <> '') and (Result[1] = dxUnixPathDelim) then
    Delete(Result, 1, 1);
...
class function TdxZIPPathHelper.ExcludeTrailingPathDelimiter(const APath: string): string;
var
  AIndex: Integer;
begin
  Result := APath;
...
class function TdxZIPPathHelper.ExpandFileName(const AFileName: string): string;
begin
  Result := dxAnsiStringToString(ExpandFileName(dxStringToAnsiString(AFileName)));
end;
...
class function TdxZIPPathHelper.ExtractFilePath(const AFileName: string): string;
var
  AIndex: Integer;
begin
  AIndex := LastDelimiter(dxUnixPathDelim + DriveDelim, AFileName);
  Result := Copy(AFileName, 1, AIndex);
end;
...
class function TdxZIPPathHelper.IsAbsoluteFileName(const AFileName: string): Boolean;
begin
  Result := (AFileName <> '') and (AFileName[1] = dxUnixPathDelim);
end;

class function TdxZIPPathHelper.RelativePath(const ACurrentPath, AFileName: string): string;
begin
...
class function TdxDeflateHelper.Compress(const AData: TBytes): TBytes;
var
  ACompressionStream: TCompressionStream;
  AOutput: TBytesStream;
begin
  AOutput := TBytesStream.Create;
  try
    ACompressionStream := TCompressionStream.Create(AOutput);
    try
...
constructor TdxDeflateHelper.Create;
begin
  inherited Create;
  FStream := TMemoryStream.Create;
end;

destructor TdxDeflateHelper.Destroy;
begin
  FreeAndNil(FStream);
  inherited Destroy;
end;

function TdxDeflateHelper.GetCompressedData: TBytes;
begin
  Result := Compress(GetRawData);
end;

function TdxDeflateHelper.GetRawData: TBytes;
var
  ASavedPosition, AStreamSize: Int64;
begin
  AStreamSize := FStream.Size;
  SetLength(Result, AStreamSize);
  ASavedPosition := FStream.Position;
  FStream.Position := 0;
  try
    FStream.ReadBuffer(Result[0], AStreamSize);
  finally
    FStream.Position := ASavedPosition;
  end;
end;

procedure TdxDeflateHelper.WriteByte(AValue: Byte);
begin
  FStream.Write(AValue, SizeOf(Byte));
end;

procedure TdxDeflateHelper.WriteData(const AData: TBytes);
begin
