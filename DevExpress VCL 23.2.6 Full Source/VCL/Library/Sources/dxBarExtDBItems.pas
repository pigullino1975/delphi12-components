...
uses
  Variants, cxClasses, dxBarStrs, cxControls, dxCoreGraphics;

const
  dxThisUnitName = 'dxBarExtDBItems';
...
begin
  ...
  begin
    ...
    try
      KeySave := FKeyValue;
      if not VarIsNull(FKeyValue) and FListLink.Active and FListLink.DataSet.Locate(FKeyFieldName, FKeyValue, []) then
      begin
        Result := True;
        FKeyValue := KeySave;
      end;
...
begin
  if FBarPopupLookup <> nil then
    FBarPopupLookup.UpdateListFields;
end;

procedure TdxBarPopupLookupLink.DataSetChanged;
begin
  if FBarPopupLookup <> nil then
    FBarPopupLookup.ListLinkDataChanged;
end;
...
begin
  ...
  begin
    ...
    if Selected and TdxBarPainterAccess(Painter).GetLookAndFeelPainter.SupportsNativeFocusRect then
...
begin
  ...
  begin
    ...
    FTimerActive := dxSetTimer(Handle, 1, Interval);
  end;
end;
