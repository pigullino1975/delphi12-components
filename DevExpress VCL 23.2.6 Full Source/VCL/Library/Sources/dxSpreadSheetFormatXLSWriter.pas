...
uses
  ...
  dxSpreadSheetStyles, dxGenerics;
...
type
  ...
  strict private
    FAllStyles: TdxSpreadSheetCellStyleHandleList;
    FBoundSheets: TdxObjectIntegerDictionary;
    ...
    FContainers: TdxObjectIntegerDictionary;
    ...
    FDrawings: TdxObjectIntegerDictionary;
    FEncryptor: TdxXLSAbstractEncryptor;
    FExtSSTBuckets: TList<TdxXLSWriterSSTBucketInfo>;
    FFontID: Integer;
    FFonts: TdxObjectIntegerDictionary;
    FFormatIndex: Integer;
    FFormats: TdxObjectIntegerDictionary;
    FMetadata: TMemoryStream;
    ...
    FSST: TdxObjectIntegerDictionary;
    FStyleID: Integer;
    FStyles: TdxObjectIntegerDictionary;
  ...
  protected
    ...
    procedure WriteMetadata(AStream: TMemoryStream);
    ...
    procedure WriteNameComment(AName: TdxSpreadSheetDefinedName);
    ...
    function WriteEmptyCellBlock(ARowIndex: Integer; var ACell: TdxSpreadSheetCell): Boolean;
    ...
    //
    property AllStyles: TdxSpreadSheetCellStyleHandleList read FAllStyles;
    ...
    property ExtSSTBuckets: TList<TdxXLSWriterSSTBucketInfo> read FExtSSTBuckets;
 ...
 public
    ...
    property BoundSheets: TdxObjectIntegerDictionary read FBoundSheets;
    ...
    property Containers: TdxObjectIntegerDictionary read FContainers;
    property CurrentSheet: TdxSpreadSheetTableView read FCurrentSheet;
    property Document: TdxOLEDocument read FDocument;
    property Drawings: TdxObjectIntegerDictionary read FDrawings;
    property Fonts: TdxObjectIntegerDictionary read FFonts;
    property Formats: TdxObjectIntegerDictionary read FFormats;
    ...
    property SST: TdxObjectIntegerDictionary read FSST;
    property Styles: TdxObjectIntegerDictionary read FStyles;
...
uses
  ...
  dxSpreadSheetFormatXLS, dxSpreadSheetCoreFormulas, dxSpreadSheetConditionalFormatting, dxSpreadSheetCoreStrs;

const
  dxThisUnitName = 'dxSpreadSheetFormatXLSWriter';
...
begin
  ...
  FAllStyles := TdxSpreadSheetCellStyleHandleList.Create();
  FAllStyles.Capacity := dxMaxXFStyles;
  FWorkbookData := FDocument.CreateStream(ARoot, sdxXLSWorkbookStream) as TMemoryStream;
  if AOwner.OptionsMetadata.HasData then
    FMetadata := FDocument.CreateStream(ARoot, sdxXLSSummaryInformationStream) as TMemoryStream;
  FDocument.CreateDirEntry('', ET_EMPTY, ARoot);
  FDocument.CreateDirEntry('', ET_EMPTY, ARoot);
  FExtSSTBuckets := TList<TdxXLSWriterSSTBucketInfo>.Create;
  FBoundSheets := TdxObjectIntegerDictionary.Create;
  FFonts:= TdxObjectIntegerDictionary.Create(SpreadSheet.CellStyles.Fonts.Count + 1);
  FFormats := TdxObjectIntegerDictionary.Create;
  ...
  FContainers := TdxObjectIntegerDictionary.Create;
  ...
  FDrawings := TdxObjectIntegerDictionary.Create;
  FPictures := TList<TdxSpreadSheetContainer>.Create;
  FShapes := TList<TdxSpreadSheetContainer>.Create;
  FSST := TdxObjectIntegerDictionary.Create;
  FStyles := TdxObjectIntegerDictionary.Create;
...
begin
  ...
  begin
    ...
    while ARow <> nil do
    begin
      Inc(FTotalObjectsCount, GetRowCellCount(TdxSpreadSheetRowAccess(ARow).FCells) + 1);
...
procedure TdxSpreadSheetXLSWriter.WriteMetadata(AStream: TMemoryStream);
var
...
begin
  ...
  try
    ...
    ASection := APropertySet.Add(FMTID_SummaryInformation);
    ASection.Add(PID_CODEPAGE, GetACP); // must be first
    if AMetadata.Created > 0 then
      ASection.Add(PID_CREATE_DTM, AMetadata.Created);
    if AMetadata.CreatedBy <> '' then
      ASection.Add(PID_AUTHOR, AMetadata.CreatedBy);
    if AMetadata.Description <> '' then
      ASection.Add(PID_COMMENTS, AMetadata.Description);
    if AMetadata.Keywords <> '' then
      ASection.Add(PID_KEYWORDS, AMetadata.Keywords);
    if AMetadata.LastModifiedBy <> '' then
      ASection.Add(PID_LASTAUTHOR, AMetadata.LastModifiedBy);
    if AMetadata.LastPrinted > 0 then
      ASection.Add(PID_LASTPRINTED, AMetadata.LastPrinted);
    if AMetadata.LastModified > 0 then
      ASection.Add(PID_LASTSAVE_DTM, AMetadata.LastModified);
    if AMetadata.Subject <> '' then
      ASection.Add(PID_SUBJECT, AMetadata.Subject);
    ASection.Add(PID_TITLE, AMetadata.Title);

    APropertySet.SaveToStream(AStream);
  finally
    APropertySet.Free;
  end;
end;
...
procedure TdxSpreadSheetXLSWriter.WriteNameComment(AName: TdxSpreadSheetDefinedName);
begin
  if AName.Comment <> '' then
  begin
    RecordWriter.BeginWrite(brcNameCmt);
    try
      Writer.WriteWord(brcNameCmt);
      Writer.WriteWord(0); 
      Writer.WriteInt64(0); 

      Writer.WriteWord(Length(AName.Caption));
      Writer.WriteWord(Length(AName.Comment));
      Writer.WriteByte(1); 
      Writer.WriteBlock(AName.Caption[1], Length(AName.Caption) * 2);
      Writer.WriteByte(1); 
      Writer.WriteBlock(AName.Comment[1], Length(AName.Comment) * 2);
    finally
      RecordWriter.EndWrite(WorkbookData);
    end;
  end;
...
begin
  ...
  for I := 0 to SpreadSheet.DefinedNames.Count - 1 do
    WriteNameComment(SpreadSheet.DefinedNames[I]);
end;
...
var
  I: Integer;
  AInfo: TdxXLSWriterSSTBucketInfo;
begin
  ...
  for I := 0 to FExtSSTBuckets.Count - 1 do
  begin
    AInfo := FExtSSTBuckets[I];
...
const
  ...
  begin
    if AStyle.Borders.BorderIsAuto[ASide] then
      ABorderStyle := sscbsDefault
    else
      ABorderStyle := AStyle.Borders.BorderStyle[ASide];

    Result := BorderLineStyleMap[ABorderStyle];
  end;
  ...
  begin
    if AStyle.Borders.BorderIsAuto[ASide] then
      ...
      AColor := AStyle.Borders.BorderColor[ASide];

    Result := GetColorIndex(AColor, $40, $40);
  end;
...
begin
  ...
  if AController.IsPartOfArrayFormula(CurrentSheet, ARowIndex, ACell.ColumnIndex, AFormulaArea) = afpSlaveCell then
    Exit;
...
begin
  ...
  begin
    ...
    while ACell <> nil do
    begin
      if not WriteEmptyCellBlock(ARow.Index, ACell) then
      begin
...
procedure TdxSpreadSheetXLSWriter.WriteColumnFormatInformation(AStartColumn, AFinishColumn: TdxSpreadSheetTableColumn);
var
  AGroup: TdxSpreadSheetTableItemGroup;
  AOptions: Word;
begin
  if AStartColumn.Index > AFinishColumn.Index then
    Exit;
...
function TdxSpreadSheetXLSWriter.WriteEmptyCellBlock(ARowIndex: Integer;
  var ACell: TdxSpreadSheetCell): Boolean;
var
  ABlockSize, ALastIndex, AStartIndex: Integer;
begin
  Result := False;
  ...
  if (ACell.Next = nil) or (ACell.Next.DataType <> cdtBlank) or (ACell.Next.Index > ACell.Index + 1) then
    Exit;

  Result := True;
  ABlockSize := 0;
  AStartIndex := ACell.Index;
  ALastIndex := AStartIndex;
  RecordWriter.BeginWrite(brcMulBlank);
  try
    Writer.WriteWord(ARowIndex);
    Writer.WriteWord(AStartIndex);
    while (ACell <> nil) and (ACell.DataType = cdtBlank) and
      (ACell.Index = AStartIndex + ABlockSize) and (ABlockSize < 256) do
    begin
      Writer.WriteWord(Styles[ACell.StyleHandle]);
      ALastIndex := ACell.Index;
      ACell := ACell.Next;
      Inc(ABlockSize);
    end;
    Writer.WriteWord(ALastIndex);
  finally
    RecordWriter.EndWrite(WorkbookData);
  end;
...
begin
  ...
  begin
    ...
    if AFontHandle <> nil then
      AFontIndex := Owner.Fonts[AFontHandle]
    else
      AFontIndex := 0;

    PWord(@Result[I * 4])^ := ARun.StartIndex - 1;
    PWord(@Result[I * 4 + 2])^ := AFontIndex;;
