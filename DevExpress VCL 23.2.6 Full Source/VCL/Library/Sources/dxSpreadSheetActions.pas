...
uses
  ...
  dxSpreadSheetPrinting, dxSpreadSheetFunctions, dxSpreadSheetProtection;
...
type
  ...
  protected
    ...
    procedure InsertToEditingFormula(const AText: string; AIsFunction: Boolean);
    ...
    function IsActiveTableProtected: Boolean; inline;
  ...
  protected
    ...
    function FindGalleryGroupItem(AGroup: TdxCustomGalleryGroup; const AActionIndex: Variant): TdxCustomGalleryItem; virtual;
    ...
    procedure PopulateGalleryInfo(AInfo: IdxActionGalleryInfo); virtual;
  ...
  TdxSpreadSheetPageMarginsGallery = class(TdxSpreadSheetCustomGalleryAction, IdxActionGalleryWithDescriptionTabbedText)
  protected type
  ...
  { TdxSpreadSheetShowDefinedNameManager }

  TdxSpreadSheetShowDefinedNameManager = class(TdxSpreadSheetAction)
  protected
    procedure DoExecute; override;
    function IsEnabled: Boolean; override;
  public
  ...
  { TdxSpreadSheetCreateDefinedName }

  TdxSpreadSheetCreateDefinedName = class(TdxSpreadSheetAction)
  protected
    procedure DoExecute; override;
    function IsEnabled: Boolean; override;
  public
  ...
  { TdxSpreadSheetCreateDefinedNamesFromSelection }

  TdxSpreadSheetCreateDefinedNamesFromSelection = class(TdxSpreadSheetAction)
  protected
    procedure DoExecute; override;
    function IsEnabled: Boolean; override;
  public
  ...
  { TdxSpreadSheetUseDefinedNameInFormula }

  TdxSpreadSheetUseDefinedNameInFormula = class(TdxSpreadSheetCustomGalleryAction,
    IdxSpreadSheetListener,
    IdxSpreadSheetListener2)
  protected
    FNeedUpdateGallery: Boolean;

    procedure DoAfterSetControl; override;
    procedure DoBeforeSetControl; override;
    procedure DoPopulateGallery(AGroup: TdxCustomGalleryGroup);
    procedure DoUpdateState; override;
    function IsEnabled: Boolean; override;
...
uses
  Dialogs, ExtDlgs, dxGDIPlusClasses, dxShellDialogs, dxSpreadSheetActionsStrs, dxSpreadSheetTypes,
  dxSpreadSheetFormatCellsDialog, dxSpreadSheetClasses, dxSpreadSheetStrs, dxSpreadSheetUnhideSheetDialog,
  dxSpreadSheetFindAndReplaceDialog, dxSpreadSheetEditHyperlinkDialog, dxSpreadSheetCoreStrs, dxSpreadSheetCoreHistory,
  dxSpreadSheetUtils, dxSpreadSheetPageSetupDialog, dxPrintUtils, dxMeasurementUnits, dxSpreadSheetDialogStrs, dxTypeHelpers,
  dxSpreadSheetFormulasHelpers, cxEdit, Variants, dxSpreadSheetInplaceEdit, dxSpreadSheetFunctionsStrs,
  dxSpreadSheetDefinedNameManagerDialog, Menus, dxSpreadSheetDefinedNameEditDialog,
  dxSpreadSheetDefinedNamesCreateFromSelectionDialog, dxMessageDialog, Forms;

const
  dxThisUnitName = 'dxSpreadSheetActions';
...
procedure TdxSpreadSheetAction.InsertToEditingFormula(const AText: string; AIsFunction: Boolean);
var
  AEdit: TcxCustomEdit;
...
begin
  ...
  begin
    ...
    if not TdxSpreadSheetTableViewEditingController.GetActualInplaceEdit(ActiveTable, TdxSpreadSheetCustomInplaceEdit(AEdit)) then
    begin
    ...
    if AEdit <> nil then
    begin
      TdxModifyFormulasHelper.Insert(AText, AEdit, AIsFunction);
...
function TdxSpreadSheetAction.IsActiveTableProtected: Boolean;
begin
  Result := (ActiveTable <> nil) and ActiveTable.OptionsProtection.&Protected;
end;
...
begin
  Result := inherited IsEnabled and (not IsActiveTableProtected or
...
begin
  ...
  FDefaultImageNameInIconLibrary := 'RichEdit\BordersAll.png';
end;
...
begin
  ...
  FDefaultImageNameInIconLibrary := 'RichEdit\BorderBottom.png';
end;
...
begin
  ...
  FDefaultImageNameInIconLibrary := 'RichEdit\BorderLeft.png';
end;
...
begin
  ...
  FDefaultImageNameInIconLibrary := 'RichEdit\BorderNone.png';
end;
...
begin
  ...
  FDefaultImageNameInIconLibrary := 'RichEdit\BordersOutside.png';
end;
...
begin
  ...
  FDefaultImageNameInIconLibrary := 'RichEdit\BorderRight.png';
end;
...
begin
  ...
  FDefaultImageNameInIconLibrary := 'RichEdit\BorderTop.png';
end;
...
begin
  ...
  FDefaultImageNameInIconLibrary := 'RichEdit\AlignBottomCenter.png';
...
begin
  ...
  FDefaultImageNameInIconLibrary := 'RichEdit\AlignMiddleCenter.png';
...
begin
  ...
  FDefaultImageNameInIconLibrary := 'RichEdit\AlignTopCenter.png';
...
begin
  if ActiveSelection.Count <> 1 then
    raise EdxSpreadSheetError.Create(cxGetResourceString(@sdxErrorInvalidSelection));
  ActiveTable.DeleteColumns(ActiveSelection.Area.Left, cxRectWidth(ActiveSelection.Area) + 1);
end;
...
begin
  if ActiveSelection.Count <> 1 then
    raise EdxSpreadSheetError.Create(cxGetResourceString(@sdxErrorInvalidSelection));
  ActiveTable.DeleteRows(ActiveSelection.Area.Top, cxRectHeight(ActiveSelection.Area) + 1);
end;
...
begin
  ActiveTable.HideSelectedColumns;
end;
...
begin
  ActiveTable.UnhideSelectedColumns;
end;
...
begin
  ActiveTable.UnhideSelectedRows;
end;
...
begin
  ...
  FDefaultImageNameInIconLibrary := 'RichEdit\FontColor.png';
end;
...
begin
  ...
  try
    ...
    if Application.ActiveFormHandle <> 0 then
      AResult := AOpenDialog.Execute(Application.ActiveFormHandle)
    else
      AResult := AOpenDialog.Execute;
    if AResult then
...
begin
  ...
  FDefaultImageNameInIconLibrary := 'RichEdit\Hyperlink.png';
  FIsContainerSelectionSupported := True;
end;
...
var
  AOpenPictureDialog: TdxOpenPictureDialog;
begin
  ActiveTable.BeginUpdate;
  try
    AOpenPictureDialog := TdxOpenPictureDialog.Create(nil);
    try
      AOpenPictureDialog.Title := cxGetResourceString(@sdxSpreadSheetActionInsertPictureDialogTitle);
...
begin
  ...
  FDefaultImageNameInIconLibrary := 'RichEdit\Header.png';
end;
...
procedure TdxSpreadSheetCustomGalleryAction.Assign(Source: TPersistent);
begin
  inherited;
  if Source is TdxSpreadSheetCustomGalleryAction then
    FGalleryGroup := TdxSpreadSheetCustomGalleryAction(Source).FGalleryGroup;
end;
...
begin
  Result := nil;
  AOwner := Owner;
  while AOwner is TWinControl do
  begin
...
function TdxSpreadSheetCustomGalleryAction.FindGalleryGroupItem(
  AGroup: TdxCustomGalleryGroup; const AActionIndex: Variant): TdxCustomGalleryItem;
var
  I: Integer;
begin
  Result := nil;
...
procedure TdxSpreadSheetCustomGalleryAction.DefineProperties(Filer: TFiler);
begin
  inherited;
  Filer.DefineProperty('GalleryGroup', ReadGalleryGroup, WriteGalleryGroup, FGalleryGroup <> nil);
end;

procedure TdxSpreadSheetCustomGalleryAction.Notification(AComponent: TComponent; Operation: TOperation);
begin
  inherited;
...
procedure TdxSpreadSheetCustomGalleryAction.PopulateGalleryInfo(AInfo: IdxActionGalleryInfo);
begin
...
procedure TdxSpreadSheetCustomGalleryAction.ReadGalleryGroup(Reader: TReader);
begin
  FGalleryGroup := FindGalleryGroup(Reader.ReadIdent);
end;

procedure TdxSpreadSheetCustomGalleryAction.WriteGalleryGroup(Writer: TWriter);
begin
...
begin
  ...
  if (Control <> nil) and (Control.ActiveSheetAsTable <> nil) then
    Result := Max(Control.ActiveSheetAsTable.OptionsPrint.Page.Paper.SizeID, Result);
end;

function TdxSpreadSheetPaperSizeGallery.GetPaperDescription(const AHeight, AWidth: Integer): string;
var
...
procedure TdxSpreadSheetPaperSizeGallery.UpdateGalleryContentResourceStrings;

  procedure UpdateGalleryGroupItemResourceStrings(APaper: TdxPaper);
  var
    AItem: TdxCustomGalleryItem;
  begin
    AItem := FindGalleryGroupItem(FGalleryGroup, APaper.DMPaper);
    if AItem <> nil then
    begin
      AItem.Caption := APaper.ShortName;
      AItem.Description := GetPaperDescription(APaper.Width, APaper.Height);
    end;
  end;
...
var
  I: Integer;
begin
  if FGalleryGroup <> nil then
    for I := 0 to TdxPrintingDefaults.Papers.Count - 1 do
      UpdateGalleryGroupItemResourceStrings(TdxPrintingDefaults.Papers[I]);
end;
...
var
  ...
  I: Integer;
begin
  inherited PopulateGalleryInfo(AInfo);
...
begin
  ...
  if (Control <> nil) and (Control.ActiveSheetAsTable <> nil) then
  begin
    AMargins := Control.ActiveSheetAsTable.OptionsPrint.Page.Margins;
...
begin
  ...
  for i := Low(AMarginsSet) to High(AMarginsSet)  do
    AMarginsSet[i] := AUnitsConverter.FromInch(AMarginsSet[i]);
  Result :=
    Format('%s:'#9'%2.2f %s'#9'%s:'#9'%2.2f %s', [GetMarginsPartCaption(mpTop), AMarginsSet[mpTop], AUnitsName,
      GetMarginsPartCaption(mpBottom), AMarginsSet[mpBottom], AUnitsName]) + #13#10 +
    Format('%s:'#9'%2.2f %s'#9'%s:'#9'%2.2f %s', [GetMarginsPartCaption(mpLeft), AMarginsSet[mpLeft], AUnitsName,
      GetMarginsPartCaption(mpRight), AMarginsSet[mpRight], AUnitsName]) + #13#10 +
    Format('%s:'#9'%2.2f %s'#9'%s:'#9'%2.2f %s', [GetMarginsPartCaption(mpHeader), AMarginsSet[mpHeader],
...
begin
{$WARNINGS OFF}
  InsertToEditingFormula(cxGetResourceString(AValue), True); 
{$WARNINGS ON}
end;
...
begin
  Result := oppoDefault;
  if (Control <> nil) and (Control.ActiveSheetAsTable <> nil) then
    Result := Control.ActiveSheetAsTable.OptionsPrint.Page.Orientation;
...
constructor TdxSpreadSheetShowDefinedNameManager.Create(AOwner: TComponent);
begin
  inherited;
  FDefaultCaptionResString := @sdxSpreadSheetActionShowDefinedNameManagerCaption;
  FDefaultImageNameInIconLibrary := 'SpreadSheet\NameManager.png';
end;

procedure TdxSpreadSheetShowDefinedNameManager.DoExecute;
begin
  ShowDefinedNameManagerDialog(Control);
end;

function TdxSpreadSheetShowDefinedNameManager.IsEnabled: Boolean;
begin
  Result := (ActiveTable <> nil) and not ActiveTable.IsEditing;
end;

{ TdxSpreadSheetCreateDefinedName }

constructor TdxSpreadSheetCreateDefinedName.Create(AOwner: TComponent);
begin
  inherited;
  FDefaultCaptionResString := @sdxSpreadSheetActionCreateDefinedNameCaption;
  FDefaultImageNameInIconLibrary := 'SpreadSheet\DefineName.png';
end;

procedure TdxSpreadSheetCreateDefinedName.DoExecute;
begin
  ShowDefinedNameEditDialog(Control);
end;

function TdxSpreadSheetCreateDefinedName.IsEnabled: Boolean;
begin
  Result := (ActiveTable <> nil) and not (ActiveTable.IsEditing or IsActiveTableProtected);
end;

{ TdxSpreadSheetCreateDefinedNamesFromSelection }

constructor TdxSpreadSheetCreateDefinedNamesFromSelection.Create(AOwner: TComponent);
begin
  inherited;
  FDefaultCaptionResString := @sdxSpreadSheetActionCreateDefinedNamesFromSelectionCaption;
  FDefaultImageNameInIconLibrary := 'SpreadSheet\DefinedNameCreateFromSelection.png';
end;

procedure TdxSpreadSheetCreateDefinedNamesFromSelection.DoExecute;
begin
  ShowCreateDefinedNamesFromSelectionDialog(ActiveTable);
end;

function TdxSpreadSheetCreateDefinedNamesFromSelection.IsEnabled: Boolean;
begin
  Result := inherited IsEnabled and not (ActiveTable.IsEditing or IsActiveTableProtected);
end;

{ TdxSpreadSheetUseDefinedNameInFormula }

constructor TdxSpreadSheetUseDefinedNameInFormula.Create(AOwner: TComponent);
begin
  inherited;
  FDefaultCaptionResString := @sdxSpreadSheetActionUseDefinedNameInFormulaCaption;
  FDefaultImageNameInIconLibrary := 'SpreadSheet\DefinedNameUseInFormula.png';
  FNeedUpdateGallery := True;
end;

procedure TdxSpreadSheetUseDefinedNameInFormula.DoAfterSetControl;
begin
  inherited;
  FNeedUpdateGallery := True;
  if Control <> nil then
...
procedure TdxSpreadSheetUseDefinedNameInFormula.DoBeforeSetControl;
begin
  if Control <> nil then
    Control.RemoveListener(Self);
  inherited;
end;

procedure TdxSpreadSheetUseDefinedNameInFormula.DoPopulateGallery(AGroup: TdxCustomGalleryGroup);
var
  AActiveView: TdxSpreadSheetCustomView;
  ADefinedName: TdxSpreadSheetDefinedName;
  AIndex: Integer;
  AItem: TdxCustomGalleryItemAccess;
begin
  ...
  try
    ...
    if Control <> nil then
    begin
      AActiveView := ActiveTable;
      for AIndex := 0 to Control.DefinedNames.Count - 1 do
      begin
        ADefinedName := Control.DefinedNames[AIndex];
        if (ADefinedName.Scope = nil) or (ADefinedName.Scope = AActiveView) then
        begin
          AItem := TdxCustomGalleryItemAccess(AGroup.Items.Add);
          AItem.Caption := ADefinedName.Caption;
          AItem.FActionIndex := AIndex;
        end;
      end;
    end;
  finally
...
procedure TdxSpreadSheetUseDefinedNameInFormula.DoUpdateState;
begin
  if FNeedUpdateGallery then
  begin
    if FGalleryGroup <> nil then
      DoPopulateGallery(FGalleryGroup);
    FNeedUpdateGallery := False;
  end;
  inherited;
end;

function TdxSpreadSheetUseDefinedNameInFormula.IsEnabled: Boolean;
begin
  Result := (FGalleryGroup <> nil) and (FGalleryGroup.ItemCount > 0) and inherited IsEnabled and not IsActiveTableProtected;
end;

function TdxSpreadSheetUseDefinedNameInFormula.GetValue: Variant;
begin
  Result := -1;
end;

procedure TdxSpreadSheetUseDefinedNameInFormula.SetValue(const AValue: Variant);
begin
  InsertToEditingFormula(Control.DefinedNames[AValue].Caption, False);
end;

procedure TdxSpreadSheetUseDefinedNameInFormula.ActiveSheetChanged(Sender: TdxCustomSpreadSheet);
begin
  FNeedUpdateGallery := True;
end;

procedure TdxSpreadSheetUseDefinedNameInFormula.DataChanged(Sender: TdxCustomSpreadSheet);
begin
  // do nothing
end;

procedure TdxSpreadSheetUseDefinedNameInFormula.DefinedNamesChanged(Sender: TdxCustomSpreadSheet);
begin
  FNeedUpdateGallery := True;
end;
