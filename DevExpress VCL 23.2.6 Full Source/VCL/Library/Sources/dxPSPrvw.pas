...
{$IFDEF OLEDRAGANDDROP}
  ActiveX,
{$ENDIF}
  dxCore, dxCoreGraphics, cxClasses, dxPSRes,
...
begin
  ...
  LookAndFeelPainter.DrawPanelContent(Canvas, R);
  InflateRect(R, -2, -2);
...
begin
  ...
  LookAndFeelPainter.DrawPanelContent(Canvas, ClientRect);
end;
...
begin
  ...
  Pt := FPreview.ScreenToClient(GetMouseCursorPos);
...
begin
  if CanPageSetup and PtInRect(StatusBar.PanelRect(7), GetMouseCursorPos) then
    DoPageSetupReport(0);
end;
...
begin
  if not IsDestroying and (Value <> (ThumbnailsPane.Width <> 0)) then
  begin
    if Value then
    begin
...
begin
  ...
  begin
    Dialog := dxPSCreateOpenReportDialog(nil);
    with Dialog do
    try
      if Execute then 
...
begin
  if ReportLink <> nil then
  begin
    ADialog := dxPSCreateSaveReportDialog(nil);
    try
...
begin
  Result := IsVisible(pvoPrint) and IsEnabled(peoPrint) and
    ...
    (ReportLink <> nil) and (ReportLink.PageCount > 0) and
    (dxPrintDevice.Printers.Count > 0) and not IsPrinting;
end;
...
begin
  Result := (ReportLink <> nil) and ReportLink.CanExportToPDF and (ReportLink.PageCount > 0) and not IsPrinting;
end;
