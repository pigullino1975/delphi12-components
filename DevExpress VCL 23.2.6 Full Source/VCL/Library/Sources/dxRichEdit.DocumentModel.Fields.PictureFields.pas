...
unit dxRichEdit.DocumentModel.Fields.PictureFields; // for internal use
...
uses
  RTLConsts, IOUtils, Rtti, Forms,
  dxRichEdit.DocumentModel.PieceTable,
  dxRichEdit.DocumentModel.FloatingObjectRange,
  dxRichEdit.DocumentModel.InlineObjectRange;

const
  dxThisUnitName = 'dxRichEdit.DocumentModel.Fields.PictureFields';

{ TdxIncludePictureField }

class constructor TdxIncludePictureField.Initialize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxIncludePictureField.Initialize', SysInit.HInstance);{$ENDIF}
  FSwitchesWithArgument := CreateSwitchesWithArgument(['c']);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxIncludePictureField.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxIncludePictureField.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxIncludePictureField.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FSwitchesWithArgument);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxIncludePictureField.Finalize', SysInit.HInstance);{$ENDIF}
end;
