...
const
  dxThisUnitName = 'dxEMF.Linq';

function Linq: TdxLinqFrom;
begin
  Result := Default(TdxLinqFrom);
end;
...
begin
  Result := FOwnerEnumerator.GetCurrent;
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxEntityMetadata.TCollectionExpression.Destroy', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FAggregateFunctionTypes);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxEntityMetadata.TCollectionExpression.Destroy', SysInit.HInstance);{$ENDIF}
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxLinqExpressionFactory.Create', SysInit.HInstance);{$ENDIF}
  FDataContexts := TList<PTypeInfo>.Create;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxLinqExpressionFactory.Create', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxLinqExpressionFactory.Destroy;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxLinqExpressionFactory.Destroy', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FExpressions);
  FreeAndNil(FDataContexts);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxLinqExpressionFactory.Destroy', SysInit.HInstance);{$ENDIF}
end;
