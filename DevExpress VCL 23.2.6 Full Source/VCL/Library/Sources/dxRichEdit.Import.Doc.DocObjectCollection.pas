...
{                                                                    }
{********************************************************************}
unit dxRichEdit.Import.Doc.DocObjectCollection; // for internal use
...
type
  ...
  strict private
    ...
    function CalculateScaleFactor(AFinalSize, AInitialSize: Integer): Word;
...
const
  dxThisUnitName = 'dxRichEdit.Import.Doc.DocObjectCollection';
...
begin
  inherited Create;
  AActualSize := ARun.ActualSize;
  ASizeInTwips := ARun.Image.SizeInTwips;
  FHorizontalScaleFactor := CalculateScaleFactor(AActualSize.cx, ASizeInTwips.cx);
  FVerticalScaleFactor := CalculateScaleFactor(AActualSize.cy, ASizeInTwips.cy);
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxPictureDescriptor.Initialize', SysInit.HInstance);{$ENDIF}
  FEmptyBorder := TdxBorderDescriptor97.Create;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxPictureDescriptor.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxPictureDescriptor.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxPictureDescriptor.Finalize', SysInit.HInstance);{$ENDIF}
  FEmptyBorder.Free;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxPictureDescriptor.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
function TdxPictureDescriptor.CalculateScaleFactor(AFinalSize, AInitialSize: Integer): Word;
begin
  if AInitialSize <= 0 then
    Exit(0);
  Result := Min(32767, Trunc(AFinalSize / AInitialSize * 1000));
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxDocObjectFactory.Initialize', SysInit.HInstance);{$ENDIF}
  FInstance := TdxDocObjectFactory.Create;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxDocObjectFactory.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxDocObjectFactory.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxDocObjectFactory.Finalize', SysInit.HInstance);{$ENDIF}
  FInstance.Free;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxDocObjectFactory.Finalize', SysInit.HInstance);{$ENDIF}
end;
