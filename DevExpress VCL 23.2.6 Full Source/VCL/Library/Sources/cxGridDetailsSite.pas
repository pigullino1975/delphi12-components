...
type
  ...
  public
    IsAssigned: Boolean;
    IsTopBottomBoundsAssigned: Boolean;
...
uses
  Types, SysUtils, Math, Forms, Menus, cxPCPainters, cxGrid, cxGridCustomTableView;

const
  dxThisUnitName = 'cxGridDetailsSite';
...
function TcxCustomGridDetailsSiteTabsViewInfo.GetBordersScaleFactor: TdxScaleFactor;
begin
  if ActiveLevel <> nil then
    Result := ActiveLevel.GridView.BordersScaleFactor
  else
    Result := ScaleFactor;
end;
...
begin
  ...
  if not TabsViewInfo.Painter.HasTabPressedState then
    CheckActiveLevel;
end;
...
begin
  ...
  if TabsViewInfo.Painter.HasTabPressedState then
    CheckActiveLevel;
end;
...
begin
  ...
  AInfo.IsAssigned := TabsViewInfo <> nil;
  if AInfo.IsAssigned then
  begin
    AInfo.FirstTabVisibleIndex := TabsViewInfo.FirstTabVisibleIndex;
    AInfo.Bounds.Left := TabsViewInfo.Bounds.Left;
    AInfo.Bounds.Right := TabsViewInfo.Bounds.Right;
    AInfo.IsTopBottomBoundsAssigned := FFullyVisible;
    if AInfo.IsTopBottomBoundsAssigned then
    begin
      AInfo.Bounds.Top := TabsViewInfo.Bounds.Top;
      AInfo.Bounds.Bottom := TabsViewInfo.Bounds.Bottom;
    end;
  end;
end;
...
begin
  if (TabsViewInfo <> nil) and AInfo.IsAssigned then
  begin
    if AInfo.IsTopBottomBoundsAssigned then
    begin
      TabsViewInfo.Bounds.Top := AInfo.Bounds.Top;
      TabsViewInfo.Bounds.Bottom := AInfo.Bounds.Bottom;
    end;
    TabsViewInfo.Bounds.Left := AInfo.Bounds.Left;
    TabsViewInfo.Bounds.Right := AInfo.Bounds.Right;
