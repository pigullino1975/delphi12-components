...
type
  ...
  private
    FUseSVGIcons: Boolean;
    procedure SetUseSVGIcons(const Value: Boolean);
  protected
    procedure AddImage(AAction: TAction; const AImageFileName: string; out ASmallImageIndex, ALargeImageIndex: Integer); overload;
    function AddImage(ATargetImageList: TCustomImageList; AImageFileName: string): Integer; overload;
    ...
    function GetCorrectedImageName(const AImageFileName: string): string;
  ...
  public
    ...
    property UseSVGIcons: Boolean read FUseSVGIcons write SetUseSVGIcons;
  end;
  ...
  strict private
    class function GetActualImageFileName(const AImageFileName, AImageSet: string;
      out AFileName: string; AWidth: Integer = 0; AHeight: Integer = 0): Boolean;
  public
...
uses
  SysUtils, StrUtils, Math, cxControls, cxGraphics;

const
  dxThisUnitName = 'dxUIGenerator';
...
function TdxUIGeneratorAdapter.AddImage(ATargetImageList: TCustomImageList; AImageFileName: string): Integer;
begin
  Result := TdxUIGeneratorHelper.AddImage(ATargetImageList, GetCorrectedImageName(AImageFileName), ImageSet);
end;
...
function TdxUIGeneratorAdapter.GetCorrectedImageName(const AImageFileName: string): string;
begin
  if not UseSVGIcons or (AImageFileName = '') then
    Result := AImageFileName
  else
  begin
    if EndsText('_16x16.png', AImageFileName) then
      Result := StringReplace(AImageFileName, '_16x16.png', '.svg', [])
    else
      Result := ChangeFileExt(AImageFileName, '.svg');
  end;
end;
...
procedure TdxUIGeneratorAdapter.SetUseSVGIcons(const Value: Boolean);
begin
  if FUseSVGIcons <> Value then
  begin
    if Value then
...
var
  ...
  ABitmap: TBitmap;
  APictureExists: Boolean;
begin
  Result := -1;
  if ATargetImageList <> nil then
  begin
    if EndsText('.svg', AImageFileName) then
      APictureExists := GetActualImageFileName(AImageFileName, AImageSet, AFileName, 0, 0)
    else
      APictureExists := GetActualImageFileName(AImageFileName, AImageSet, AFileName, ATargetImageList.Width, ATargetImageList.Height);
    if APictureExists then
    begin
      AImage := TdxSmartImage.Create;
      try
        AImage.LoadFromFile(AFileName);
        if ATargetImageList is TcxImageList then
          Result := TcxImageListAccess(ATargetImageList).AddCore(
            AImage, nil, clNone, '', TdxIconLibraryFileName.ToRelative(AFileName))
        else
        begin
...
var
  AFileName: string;
begin
  if GetActualImageFileName(AImageFileName, AImageSet, AFileName) then
    ATarget.LoadFromFile(AFileName)
  else
    ATarget.Clear;
end;

class function TdxUIGeneratorHelper.GetActualImageFileName(const AImageFileName, AImageSet: string;
  out AFileName: string; AWidth: Integer = 0; AHeight: Integer = 0): Boolean;
begin
  if (AImageFileName <> '') and (AImageSet <> '') then
  begin
    AFileName := TdxIconLibraryFileName.Build(AImageSet, ExtractFileDir(AImageFileName), ExtractFileName(AImageFileName), AWidth, AHeight);
