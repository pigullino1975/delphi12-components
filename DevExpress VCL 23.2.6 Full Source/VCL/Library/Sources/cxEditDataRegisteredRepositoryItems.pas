...
uses
  SysUtils, dxCore, cxClasses, cxDataStorage, cxEdit, cxEditRegisteredRepositoryItems,
...
const
  dxThisUnitName = 'cxEditDataRegisteredRepositoryItems';
...
{$IFNDEF NONDB}
  GetDefaultEditDataRepositoryItems.RegisterItem(TcxSQLTimeStampValueType,
    cxEditRegisteredItemsStandardVersion, GetDefaultEditRepository.CreateItem(TcxEditRepositoryDateItem));
{$ENDIF}
end;
...
function GetClassID(AClass: TClass): TdxNativeInt;
begin
  Result := TdxNativeInt(AClass);
end;
...
finalization
  ...
  dxFreeGlobalObject(DefaultDataEditRepositoryItems);
