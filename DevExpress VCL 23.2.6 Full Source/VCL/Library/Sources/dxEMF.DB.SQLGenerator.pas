...
type
  ...
  strict private
    ...
    FQueryParameterNames: TdxArray<string>;
...
const
  dxThisUnitName = 'dxEMF.DB.SQLGenerator';
...
type
  ...
  strict private
    ...
    FNodeAliasCache: TdxStringsDictionary;
  public
    constructor Create;
    destructor Destroy; override;
...
begin
  inherited Create;
  FNodeAliasCache := TdxStringsDictionary.Create;
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxOuterApplyInfo.Destroy', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FComparer);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxOuterApplyInfo.Destroy', SysInit.HInstance);{$ENDIF}
end;
...
begin
  ...
  if not AConsolidateParameter.Value.IsNullOrEmpty then
  begin
...
begin
  ...
  FQueryParameterNames := TdxArray<string>.Create(16);
end;
...
begin
  ...
  try
    ...
    begin
      ASp := ASortProperties.List[I];
      J := 0;
      while J < I do
      begin
        if (ASortProperties.List[J].&Property as TdxCriteriaOperator).Equals(ASp.&Property as TdxCriteriaOperator) then
          Break;
        Inc(J);
      end;
      if J < I then
...
begin
  ...
  try
    ...
    begin
      ...
      ANames.Append(Process(Root.Operands[I]));
