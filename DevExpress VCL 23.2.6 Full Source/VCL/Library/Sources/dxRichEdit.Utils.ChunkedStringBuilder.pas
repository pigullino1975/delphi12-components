...
type
  ...
  protected
    type
      ...
      TChunks = class(TdxObjectList<TChunk>)
      private
        function GetItem(AIndex: TdxListIndex): TChunk; inline;
      public
        property Items[AIndex: TdxListIndex]: TChunk read GetItem; default;
      end;
...
const
  dxThisUnitName = 'dxRichEdit.Utils.ChunkedStringBuilder';
...
{ TdxChunkedStringBuilder.TChunks }

function TdxChunkedStringBuilder.TChunks.GetItem(AIndex: TdxListIndex): TChunk;
begin
{$IFDEF DEBUG}
...
begin
  ...
  if ASpace >= ACount then
    AChunk.Append(@AValue[AStartIndex + Low(string)], ACount)
  else
  begin
    AIndex := AStartIndex;
    if ASpace > 0 then
    begin
      AChunk.Append(@AValue[AStartIndex + Low(string)], ASpace);
      Inc(AIndex, ASpace);
    end;
    AEndIndex := AStartIndex + ACount;
    while AIndex < AEndIndex do
    begin
      ALength := Min(AEndIndex - AIndex, MaxChunkSize);
      AChunk := TChunk.Create(MaxChunkSize);
      AChunk.Append(@AValue[AIndex + Low(string)], ALength);
      FChunks.Add(AChunk);
      Inc(AIndex, ALength);
    end;
  end;
