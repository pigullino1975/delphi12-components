...
uses
  ...
  Data.Win.ADODB,
  DB, cxFilter, dxServerModeClasses, dxServerModeData;
...
const
  dxThisUnitName = 'dxServerModeADODataSource';

type
  TADOConnectionAccess = class(TADOConnection);
  TdxServerModeCustomSQLAdapterAccess = class(TdxServerModeCustomSQLAdapter);
  ...
  try
    ...
    if (AParams <> nil) and (AParams.Count > 0) then
      ...
      begin
        ...
        if (AParams[I].DataType = ftWideMemo) or (paLong in AParameter.Attributes) then
        begin
          AMaxStringSize := TdxServerModeCustomSQLAdapterAccess(DataSource.SQLAdapter).GetMaximumStringSize;
          if (AParameter.Size > AMaxStringSize) or (Length(VarToStr(AParams[I].Value)) > AMaxStringSize) then
          begin
            AParameter.ParameterObject.Type_ := adVarWChar;
