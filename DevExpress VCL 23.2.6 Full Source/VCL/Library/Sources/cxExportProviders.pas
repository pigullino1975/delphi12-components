...
type
  ...
  protected
    ...
    FSharedImages: TDictionary<TSharedImageKey, TObject>;
    ...
    function CellAutoHeight: Boolean; virtual;
    ...
    procedure SetRowHeight(const ARow, AHeight: Integer); virtual;
    ...
    //
    function TryGetSharedImageHandle(AImageList: TObject; AImageIndex: Integer; const AImageSize: TSize; var AHandle: TObject): Boolean; inline;
 ...
 { TcxExportToCSVProvider }
  ...
  protected
    function CellAutoHeight: Boolean; override;
    procedure Commit(AProgressHelper: TcxCustomProgressCalculationHelper; AHandler: TObject); override;
  public
    constructor Create(AStream: TStream); override;
    class function ExportName: string; override;
  ...
  protected
    ...
    function CreateSpreadSheet: TdxSpreadSheet; virtual;
...
{$IFNDEF NONDB}
function dxPrepareBcdForExport(const AValue: Variant; AIsCurrency: Boolean): Variant;
{$ENDIF}
implementation

uses
  Math, StrUtils, DateUtils, cxExportStrs, dxSpreadSheetFormatXLSX, dxGDIPlusClasses, dxDPIAwareUtils,
  dxSpreadSheetCoreFormulas,
  dxSpreadSheetNumberFormat,
  dxSpreadSheetStyles;

const
  dxThisUnitName = 'cxExportProviders';
...
begin
  ABcdAsSting := BcdToStr(VarToBcd(AValue));
  Result := ABcdAsSting;
  if AIsCurrency and TryStrToCurr(ABcdAsSting, ACurrencyValue) then
    Result := ACurrencyValue
  else
  begin
    Val(ABcdAsSting, AInt64Value, AError);
    if AError = 0 then
      Result := AInt64Value
    else
      if TryStrToFloat(ABcdAsSting, AFloatValue) then
        Result := AFloatValue
  end;
end;
{$ENDIF}
...
constructor TdxSpreadSheetBasedExporterTableRow.Create(AOwner: TdxDynamicItemList; AIndex: Integer);
begin
...
constructor TcxExportProvider.Create(AStream: TStream);
begin
  ...
  FSpreadSheet.OptionsView.CellAutoHeight := False;
  ...
  FSharedImages := TDictionary<TSharedImageKey, TObject>.Create;
end;
...
begin
  ...
  try
    ARow := TdxSpreadSheetTableRowAccess(TableView.Rows.First);
    while ARow <> nil do
    begin
      if ARow.Visible then
      begin
        ACell := TdxSpreadSheetCellAccess(TdxSpreadSheetTableRowAccess(ARow).FirstCell);
        while ACell <> nil do
        begin
...
var
  AImageKey: TSharedImageKey;
begin
  AImageKey.ImageList := AImageList;
  AImageKey.ImageIndex :=  AImageIndex;
  AImageKey.ImageSize := TdxSpreadSheetSharedImageHandle(AHandle).Image.Size;
  FSharedImages.AddOrSetValue(AImageKey, AHandle);
end;
...
begin
  ...
  AImageSize := cxSize(AImages.Width, AImages.Height);
  if not TryGetSharedImageHandle(AImages, AImageIndex, AImageSize, TObject(AHandle)) then
  begin
    ABitmap := TcxBitmap.CreateSize(AImageSize.cx, AImageSize.cy, pf24bit);
    try
...
function TcxExportProvider.CellAutoHeight: Boolean;
begin
  Result := False;
end;
...
begin
  ...
  try
    FSpreadSheet.OptionsView.CellAutoHeight := CellAutoHeight;
    FSpreadSheet.SaveToStream(Stream, GetFormat);
  finally
...
{$IFNDEF NONDB}
  ...
  if TVarData(AValue).VType = VarFMTBcd then
    ACell.AsVariant := dxPrepareBcdForExport(AValue, True)
  else
{$ENDIF}
...
procedure TcxExportProvider.SetGridLines(AValue: Boolean);
begin
  SpreadSheet.OptionsView.GridLines := AValue;
end;
...
function TcxExportProvider.TryGetSharedImageHandle(AImageList: TObject; AImageIndex: Integer;
  const AImageSize: TSize; var AHandle: TObject): Boolean;
var
  AImageKey: TSharedImageKey;
begin
  AImageKey.ImageList := AImageList;
  AImageKey.ImageIndex :=  AImageIndex;
  AImageKey.ImageSize := AImageSize;
  Result := FSharedImages.TryGetValue(AImageKey, AHandle);
end;
...
begin
  ...
  AFontHandle.Initialize(AStyle.FontName, AStyle.FontCharset, AStyle.FontColor, AStyle.FontSize, fpDefault, AStyle.FontStyle);
...
constructor TcxExportToHTMLProvider.Create(AStream: TStream);
begin
  inherited Create(AStream);
...
function TcxExportToHTMLProvider.CellAutoHeight: Boolean;
begin
  Result := True;
end;
...
begin
  ...
  try
    ...
    try
      ...
      ADefaultSize := TableView.Rows.DefaultSize;
      TableView.Rows.DefaultSize := 1;
      while ARow <> nil do
      begin
        ASize := ARow.Size;
        ARow.ApplyBestFit;
        ARow.Size := Max(ARow.Size, ASize);
        ARow := ARow.Next;
      end;
      TableView.Rows.DefaultSize := ADefaultSize;
    finally
      TableView.EndUpdate;
    end;
...
constructor TcxExportToXMLProvider.Create(AStream: TStream);
begin
  inherited Create(AStream);
  FDefaultAlignVert := ssavCenter;
end;
    ...
    try
      ...
      try
        ...
        try
          ...
          if Supports(FHandler, IcxExportBeforeSave, AIntf) then
            AIntf.OnBeforeSave(FSpreadSheet);
...
function TdxCustomDataExport.CreateSpreadSheet: TdxSpreadSheet;
begin
  Result := TdxSpreadSheet.Create(nil);
end;
