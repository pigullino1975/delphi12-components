...
type
  ...
  private
    ...
    function GetHolidayTextColor: TColor;
  ...
  public
    ...
    property Bounds;
    property Calendar: TcxCustomCalendarTable read GetCalendar; // for internal use
...
uses
  DateUtils, cxGeometry, Forms, dxCoreGraphics, cxFormats, cxLibraryConsts;

const
  dxThisUnitName = 'cxSchedulerDateNavigator';
...
begin
  ...
  FTimer := cxCreateTimer(TimerProc, cxDateStartNavigatorTime);
...
var
  ...
  ARealNeighbors: TcxNeighbors;
begin
  FHeaderParams := AViewInfo.ViewParams;
  APainter := TcxCustomLookAndFeelPainterAccess(LookAndFeelPainter);
  if LookAndFeelPainter.LookAndFeelStyle = lfsSkin then
  begin
    ARealNeighbors := AViewInfo.GetRealNeighbors;
    LookAndFeelPainter.DrawScaledHeader(Canvas, AViewInfo.Bounds, AViewInfo.Bounds, ARealNeighbors,
      LookAndFeelPainter.SchedulerHeaderBorders(ARealNeighbors), cxbsNormal, taCenter, vaCenter, False, False, AViewInfo.Text,
  ...
  begin
    ...
    APainter.DrawContent(Canvas, ABounds, ABounds, cxbsNormal,
...
begin
  ...
  begin
    ...
    {$IFDEF DELPHIXE8}
    Font.Height := MulDiv(AViewInfo.ViewParams.Font.Height, 2, 3);
    {$ELSE}
...
function TcxSchedulerDateNavigatorPainter.GetHolidayTextColor: TColor;
begin
  Result := DateNavigator.HolidayColor;
  if Result = cxSchedulerDefaultHolidayColor then
    Result := cxGetActualColor(LookAndFeelPainter.DefaultDateNavigatorHolydayTextColor, Result);
end;
...
constructor TcxSchedulerDateNavigatorMonthHeaderViewInfo.Create(AOwner: TcxSchedulerDateNavigatorMonthViewInfo);
begin
  inherited Create;
  FOwner := AOwner;
end;

destructor TcxSchedulerDateNavigatorMonthHeaderViewInfo.Destroy;
begin
...
begin
  ...
  begin
    ...
    if Holiday and DateNavigator.ShowDatesContainingHolidaysInColor then
      FViewParams.TextColor := TcxSchedulerDateNavigatorPainter(DateNavigator.Painter).GetHolidayTextColor
    else
      if DateNavigator.IsDateToday(Date) then
         FViewParams.TextColor :=  cxGetActualColor(DateNavigator.LookAndFeelPainter.DefaultDateNavigatorTodayTextColor(Selected), FViewParams.TextColor)
      else
        if FGrayed then
          FViewParams.TextColor := DateNavigator.LookAndFeelPainter.DefaultDateNavigatorInactiveTextColor
        else
          if not Selected and cxIsWeekendDay(Date) then
            FViewParams.TextColor := cxGetActualColor(DateNavigator.LookAndFeelPainter.DefaultDateNavigatorWeekendTextColor, FViewParams.TextColor);
  end;
  Inc(FDayCount);
end;
