...
unit dxBingMapImageryDataService; // for internal use
...
uses
  ...
  dxMapControlHttpRequest, dxBingMapRESTService, dxRESTService, dxBingMapRESTServiceStrs;
...
type
  ...
  public
    ...
    property ConnectionProtocol: TdxMapControlConnectionProtocol read FConnectionProtocol write FConnectionProtocol;
  end;
...
const
  dxThisUnitName = 'dxBingMapImageryDataService';

{ TdxBingMapImageryDataService }

function TdxBingMapImageryDataService.GetAddress: string;
begin
  Result := dxBingImageryMetadataServiceBaseAddress + '/';
end;
...
begin
  ...
  (Params as TdxBingMapRESTServiceImageryParams).ConnectionProtocol := ConnectionProtocol;
  Execute;
end;
...
function TdxBingMapRESTServiceImageryParams.DoAsString: string;
begin
  Result := inherited DoAsString;
  if dxMapControlGetActualConnectionProtocol(ConnectionProtocol) = cpHTTPS then
    AddParamBefore(Result, dxBingImageryMetadataServiceUriSchemeParam);
end;
