...
uses
  Types, SysUtils, Messages, Forms, dxPSUtl, dxCore, Math, cxGraphics;

const
  dxThisUnitName = 'dxPSRELnk';
...
var
  ...
  APageSize: TSize;
begin
  APageSize := cxSize(RenderInfo.PageSize);
  AMetafile := TMetafile.Create;
  try
    AMetafile.Width := MulDiv(APageSize.cx, UnitsPerInch, PixelsPerInch);
    AMetafile.Height := MulDiv(APageSize.cy, UnitsPerInch, PixelsPerInch);
    ...
    AMetafileSize := cxSize(AMetafile.Width, AMetafile.Height);
    AMetafileSize := cxSizeScale(AMetafileSize, UnitsPerInch, PixelsPerInch);

    if (AMetafileSize.cx > APageSize.cx) or (AMetafileSize.cy > APageSize.cy) then
    begin
      Canvas.SaveClipRgn;
      Canvas.IntersectClipRgn(cxRect(APageSize));
      Canvas.DrawPicture(AMetafile, cxRect(AMetafileSize), ppmStretch, 1, 1);
      Canvas.RestoreClipRgn;
    end
    else
      Canvas.DrawPicture(AMetafile, cxRect(APageSize), ppmStretch, 1, 1);
  finally
    AMetafile.Free;
  end;
end;
