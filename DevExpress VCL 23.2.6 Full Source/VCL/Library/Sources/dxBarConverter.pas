...
uses
  SysUtils, Controls, Forms, dxBarCustomCustomizationForm, dxBarStrs;

const
  dxThisUnitName = 'dxBarConverter';
...
begin
  ...
  if ABarPopupMenu.BarManager <> nil then
  begin
    ABarPopupMenu.BarManager.BeginUpdate;
    try
      ConvertItems(ABarPopupMenu.BarManager, ABarPopupMenu.ItemLinks, APopupMenu.Items, AddCategory(ABarPopupMenu.BarManager, APopupMenu.Name));
    finally
      ABarPopupMenu.BarManager.EndUpdate;
    end;
  end
  else
    dxBarMessageBox(dxSBAR_NOBARMANAGER, MB_ICONERROR or MB_OK);
end;
