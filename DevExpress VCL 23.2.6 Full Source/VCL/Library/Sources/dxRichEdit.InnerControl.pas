...
unit dxRichEdit.InnerControl; // for internal use
...
uses
  System.UITypes,
  Types, Classes, SysUtils, Graphics, Controls, Generics.Defaults, Generics.Collections, Rtti, Forms,
...
type
  ...
  ['{06330B38-247B-48C7-9296-42FE3CE94C12}']
    ...
    procedure ApplyFont(ACharacterProperties: TdxCharacterProperties; AFont: TFont);
    function ShouldApplyForeColor: Boolean;
    function ShouldApplyFont: Boolean;
  ...
  TdxCancelEventHandler = TdxMulticastMethod<TdxCloseQueryEvent>;
  ...
  strict private
    ...
    procedure AddDocumentClosingHandler(const AHandler: TdxCloseQueryEvent);
    procedure RemoveDocumentClosingHandler(const AHandler: TdxCloseQueryEvent);
    ...
    function TryLoadDocument(AStream: TStream; ADocumentFormat: TdxRichEditDocumentFormat;
      const ASourceUri: string): Boolean;
  protected
    // IdxServiceProvider
    ...
    function LoadDocumentCore(AStream: TStream; ADocumentFormat: TdxRichEditDocumentFormat; const ASourceUri: string): Boolean; overload; virtual;
...
//    IdxGestureStateIndicator
  ...
  protected
    ...
    procedure ApplyFontAndForeColorCore(ACharacterProperties: TdxCharacterProperties); override;
...
uses
  ...
  dxTypeHelpers, dxSpellCheckerCore, dxThreading, dxOLECryptoContainer,
...
const
  dxThisUnitName = 'dxRichEdit.InnerControl';
...
begin
  ...
  AControl := Safe<TdxInnerRichEditControl>.Cast(Server);
...
begin
  ...
  AService := AContainer.GetService(IdxDocumentLayoutService) as TdxDocumentServerLayoutService;
...
begin
  ...
  begin
    ...
    begin
      AState := AFormattingController.RowsController.TablesController.State as TdxTablesControllerTableState;
...
procedure TdxInnerRichEditDocumentServer.AddDocumentClosingHandler(const AHandler: TdxCloseQueryEvent);
begin
  FOnDocumentClosing.Add(AHandler);
end;

procedure TdxInnerRichEditDocumentServer.RemoveDocumentClosingHandler(const AHandler: TdxCloseQueryEvent);
begin
...
function TdxInnerRichEditDocumentServer.GetDefaultCharacterProperties: TdxCharacterProperties;
begin
  if DocumentModel = nil then
    Exit(nil);
...
procedure TdxInnerRichEditDocumentServer.ApplyFontAndForeColorCore(ACharacterProperties: TdxCharacterProperties);
begin
end;
...
function TdxInnerRichEditDocumentServer.LoadDocumentCore(AStream: TStream; ADocumentFormat: TdxRichEditDocumentFormat;
  const ASourceUri: string): Boolean;
var
  AImportManagerService: IdxDocumentImportManagerService;
  AFormatDetector: IdxDocumentFormatRating;
  ARatings: TArray<TdxRichEditDocumentFormatRating>;
  I: Integer;
begin
  Result := False;
  ...
  begin
    ...
    DocumentModel.DocumentSaveOptions.CurrentFormat := ADocumentFormat;
    Exit(True);
  end;
  if ADocumentFormat <> TdxRichEditDocumentFormat.Undefined then
    if TryLoadDocument(AStream, ADocumentFormat, ASourceUri) then
      Exit(True);
  if Supports(AImportManagerService, IdxDocumentFormatRating, AFormatDetector) then
  begin
    ADocumentFormat := AFormatDetector.DetectFormat(AStream);
    if TryLoadDocument(AStream, ADocumentFormat, ASourceUri) then
      Exit(True);
  end;
  ARatings := AFormatDetector.CalculateDocumentFormatRating(AStream, ADocumentFormat);
  for I := 0 to Length(ARatings) - 1 do
    if TryLoadDocument(AStream, ARatings[I].DocumentFormat, ASourceUri) then
      Exit(True);
end;
...
function TdxInnerRichEditDocumentServer.TryLoadDocument(AStream: TStream; ADocumentFormat: TdxRichEditDocumentFormat;
  const ASourceUri: string): Boolean;
var
  ASuccess: Boolean;
  APosition: Integer;
begin
  ...
  if ASuccess then
    ...
    AStream.Seek(APosition, soFromBeginning);
  Result := ASuccess;
end;
...
begin
  ...
  TdxSpellCheckerInstance.RegisterListener(OnSpellCheckerChanged);
end;
...
begin
  TdxSpellCheckerInstance.UnregisterListener(OnSpellCheckerChanged);
  DisposeBackgroundFormatter; 
  DisposeViews;
...
function TdxInnerRichEditControl.GetFont: TFont;
begin
  Result := Owner.Font;
end;
...
procedure TdxInnerRichEditControl.ResetSpellChecker;
begin
  if DocumentModel = nil then
    Exit;
  DocumentModel.BeginUpdate;
...
begin
  if ShouldApplyForeColor then
    ACharacterProperties.ForeColor := TdxAlphaColors.FromColor(Font.Color);
  if ShouldApplyFont then
    ApplyFont(ACharacterProperties, Font);
end;

function TdxInnerRichEditControl.ShouldApplyForeColor: Boolean;
begin
  Result := Owner.ShouldApplyForeColor;
end;

function TdxInnerRichEditControl.ShouldApplyFont: Boolean;
begin
  Result := Owner.ShouldApplyFont;
end;

procedure TdxInnerRichEditControl.ApplyFont(ACharacterProperties: TdxCharacterProperties; AFont: TFont);
begin
  Owner.ApplyFont(ACharacterProperties, AFont);
end;
    ...
    begin
      ...
      begin
        ...
        if AShape.TextBox = nil then
          Continue;
        ADocument := AShape.TextBox.Document as TdxNativeSubDocumentBase;
