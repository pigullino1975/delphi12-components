...
uses
  Types, Variants, Windows, Classes, Controls, Forms, Graphics, Messages, SysUtils, ImgList,
  ...
  cxAccessibility, cxLookAndFeels;
...
type
  ...
  { TdxRatingControlRangeValueProvider }

  TdxCustomRatingControl = class;

  TdxRatingControlRangeValueProvider = class(TdxRangeValueProvider) // for internal use
  private
    FRatingControl: TdxCustomRatingControl;
  protected
  ...
  public
    constructor Create(ARatingControl: TdxCustomRatingControl);
  end;

  { TdxRatingControlAccessibilityHelper }

  TdxRatingControlAccessibilityHelper = class(TdxEditAccessibilityHelper) // for internal use
  private
    FRangeValueProvider: TdxRatingControlRangeValueProvider;

    function GetRatingControl: TdxCustomRatingControl;
  ...
  protected
    ...
    property RatingControl: TdxCustomRatingControl read GetRatingControl;
...
uses
  ...
  dxCoreGraphics, RTLConsts;

const
  dxThisUnitName = 'dxRatingControl';
...
begin
  if (not cxRectIsEmpty(FocusRect)) and Painter.SupportsNativeFocusRect then
...
procedure TdxCustomRatingControlViewData.AdjustPadding(var APadding: TdxPadding);
begin
  inherited AdjustPadding(APadding);
  if not IsInplace then
  begin
    if Properties.Orientation = orVertical then
...
constructor TdxRatingControlRangeValueProvider.Create(ARatingControl: TdxCustomRatingControl);
begin
  FRatingControl := ARatingControl;
end;

function TdxRatingControlRangeValueProvider.GetIsReadOnly: Boolean;
begin
  Result := FRatingControl.Properties.ReadOnly;
end;

function TdxRatingControlRangeValueProvider.GetMaximum: Double;
begin
  Result := FRatingControl.Properties.MaxValue;
end;

function TdxRatingControlRangeValueProvider.GetMinimum: Double;
begin
  Result := FRatingControl.Properties.MinValue;
end;

function TdxRatingControlRangeValueProvider.GetSmallChange: Double;
begin
  Result := FRatingControl.Properties.Step;
end;

function TdxRatingControlRangeValueProvider.GetValue: Double;
begin
  Result := FRatingControl.Rating;
end;

procedure TdxRatingControlRangeValueProvider.SetValue(AValue: Double);
begin
  FRatingControl.InternalSetRating(AValue);
end;

{ TdxRatingControlAccessibilityHelper }

constructor TdxRatingControlAccessibilityHelper.Create(AOwnerObject: TObject);
begin
  inherited;
  FRangeValueProvider := TdxRatingControlRangeValueProvider.Create(TdxCustomRatingControl(Edit));
end;

destructor TdxRatingControlAccessibilityHelper.Destroy;
begin
  FreeAndNil(FRangeValueProvider);
  inherited;
end;

function TdxRatingControlAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  Result := cxRectSetOrigin(ViewInfo.Bounds, Edit.ClientToScreen(ViewInfo.Bounds.TopLeft))
end;

function TdxRatingControlAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxROLE_SYSTEM_SLIDER;
end;

function TdxRatingControlAccessibilityHelper.IsSupportedPattern(APatternID: Integer; out AProvider: IUnknown;
...
function TdxRatingControlAccessibilityHelper.GetRatingControl: TdxCustomRatingControl;
begin
  Result := TdxCustomRatingControl(Edit);
end;

function TdxRatingControlAccessibilityHelper.GetViewInfo: TdxCustomRatingControlViewInfo;
begin
...
function TdxCustomRatingControl.GetAccessibilityHelperClass: TdxEditAccessibilityHelperClass;
begin
  Result := TdxRatingControlAccessibilityHelper;
end;
