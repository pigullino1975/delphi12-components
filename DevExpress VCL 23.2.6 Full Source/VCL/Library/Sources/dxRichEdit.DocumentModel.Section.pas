...
unit dxRichEdit.DocumentModel.Section; // for internal use
...
type
  ...
  protected
    ...
    function RaiseRequestSectionIndex: TdxSectionIndex;
    function CalculateMainPieceTableStartRunIndex(ARunIndex: TdxRunIndex): TdxRunIndex;
    function CalculateMainPieceTableEndRunIndex(ARunIndex: TdxRunIndex): TdxRunIndex;
    procedure OverrideType(AType: TdxHeaderFooterType);
  ...
  strict private
    FIndices: array[0..2] of TdxSectionIndex;
  protected
    ...
    procedure SubscribeObjectEvents(AObj: TObj);
    procedure UnsubscribeObjectEvents(AObj: TObj);
    procedure OnRequestSectionIndex(ASender: TObject; AArgs: TdxRequestSectionIndexEventArgs);
    function GetValidUniqueObjectIndices: TdxIntegerList;
    procedure PerformLinkToPrevious(AType: TdxHeaderFooterType; ALink: Boolean; const ALinkAction: TdxLinkObjectDelegate);
    procedure PerformLinkToNext(AType: TdxHeaderFooterType; ALink: Boolean; const ALinkAction: TdxLinkObjectDelegate);
    ...
    procedure ChangeObjectIndex(AType: TdxHeaderFooterType; ANewIndex: TdxSectionIndex);
  ...
  public
    ...
    procedure CopyFrom(ASource: TdxSectionHeadersFooters<TObj>); overload;
    procedure CopyFrom(ASourceSection: TdxSection); overload;
  ...
  protected
    ...
    function GetPreviousSection: TdxSection;
    function GetNextSection: TdxSection;
    function GetPreviousSectionHeader(AHeader: TdxSectionFooter): TdxSectionHeader;
    function GetPreviousSectionFooter(AFooter: TdxSectionFooter): TdxSectionFooter;
  public
    ...
    procedure CopyFromCore(const ASection: TdxSection);
    function Copy(CopyManager: TObject): TdxSection;  
    function GetActualColumnsCount: Integer;
    function GetCorrespondingHeader(AFooter: TdxSectionFooter): TdxSectionHeader;
    function GetCorrespondingFooter(AHeader: TdxSectionHeader): TdxSectionFooter;
    procedure Reset;
  ...
  private
    function GetItem(Index: TdxListIndex): TdxSection;
    procedure SetItem(Index: TdxListIndex; const Value: TdxSection);
  public
    function First: TdxSection; reintroduce;
    function Last: TdxSection; reintroduce;

    property Items[Index: TdxListIndex]: TdxSection read GetItem write SetItem; default;
  end;
  ...
  strict private
    ...
    FSection: TdxSection;
    FSpace: Integer;
    procedure CopyColumnsToSection(ASection: TdxSection);
  public
    constructor Create(ASection: TdxSection); overload;
...
uses
  RTLConsts, Contnrs,
  cxGeometry, cxClasses, dxHash, dxHashUtils,
...
const
  dxThisUnitName = 'dxRichEdit.DocumentModel.Section';
...
function TdxSectionCollection.GetItem(Index: TdxListIndex): TdxSection;
begin
  Result := TdxSection(inherited Items[Index]);
end;

procedure TdxSectionCollection.SetItem(Index: TdxListIndex; const Value: TdxSection);
begin
...
begin
  ...
  FTopMargin := ASectionProperties.TopMargin;
  FBottomMargin := ASectionProperties.BottomMargin;
...
begin
  ...
  CopyColumnsToSection(ASection);
  ASection.Page.Width := PageWidth;
  ASection.Page.Height := PageHeight;
end;

procedure TdxSectionProperties.CopyColumnsToSection(ASection: TdxSection);
var
  I: Integer;
  AColumns: TdxColumnInfoCollection;
begin
  AColumns := TdxColumnInfoCollection.Create;
  try
    AColumns.Capacity := ColumnInfoCollection.Count;
    for I := 0 to ColumnInfoCollection.Count - 1 do
      AColumns.Add(ColumnInfoCollection[I].Clone); 
    ASection.Columns.BeginUpdate;
    try
      ASection.Columns.SetColumns(AColumns);
      ASection.Columns.ColumnCount := ColumnCount;
      ASection.Columns.EqualWidthColumns := EqualWidthColumns;
      ASection.Columns.Space := Space;
    finally
      ASection.Columns.EndUpdate;
    end;
  finally
    AColumns.Free;
  end;
