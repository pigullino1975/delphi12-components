...
type
  ...
  ['{E910C359-6427-4F89-9338-086F25A715E2}']
    ...
    function GetMember(AIndex: Integer): TdxMappingMemberInfo;
  ...
  public
    ...
    function GetMember(AIndex: Integer): TdxMappingMemberInfo; overload;
  ...
  protected
    ...
    function GetSubMembers(AMember: TdxMappingMemberInfo; const APrefix: string): TdxMemberInfoList;
...
const
  dxThisUnitName = 'dxEMF.DB.Generator';
...
begin
  if AValue.IsNullOrEmpty or not AValue.IsObject then
    Exit(nil);
...
begin
  ...
  begin
    Result := TList<TdxModificationStatement>.Create;
          ACriteria := TdxInOperator.Create(AEntityInfo.KeyProperty.MemberName, AKeys);
          ...
          try
            ...
            try
              ...
              try
                for AStatement in ADeletes do
                  AStatement.AffectedRecordCount := AKeyCount;
                Result.AddRange(ADeletes);
              finally
                ADeletes.Free;
              end;
...
var
  AProps: TdxArray<IdxCriteriaOperator>;
  AMic: IdxCriteriaOperator;
  ARes: IdxCriteriaOperator;
  ANode: IdxPropertyAlias;
  ACount, I: Integer;
begin
  AProps := TdxArray<IdxCriteriaOperator>.Create(AProperties.Count);
...
begin
  ...
  try
    ...
    begin
      ...
      Result.Add(TdxModificationStatement(inherited GenerateSQL(ACriteria)));
    end;
  finally
    AList.Free;
  end;
...
begin
  ...
  if AMember.IsKey and AMember.IsIdentity then
    AValue := AObject
  else
...
begin
  ...
  FSubMembers := ASubMembers.ToArray;
  FPrefix := APrefix;
end;
...
begin
  Create(ANode, AMember);
  FSubMembers := AMember.SubMembers;
end;
...
function TdxPropertyAlias.GetMember(AIndex: Integer): TdxMappingMemberInfo;
begin
...
begin
  ...
  begin
    if FLeftCriteriaList.Count > 0 then
      Exit(False);
    FResult := FPathStack.ToArray;
    TArray.Reverse<TdxPathInfo>(FResult);
    Exit(True);
  end;
...
var
  ...
  ACriteria: IdxCriteriaOperator;
  A160B4E45452547325B31101A100414046E75590366706473: IdxCriteriaOperator;
begin
  ADefaultMembers := TdxPropertyAlias.Empty;
  for ACriteria in AOperands do
  begin
    A160B4E45452547325B31101A100414046E75590366706473 := Process(ACriteria);
    AValues.Add(A160B4E45452547325B31101A100414046E75590366706473);
    Supports(A160B4E45452547325B31101A100414046E75590366706473, IdxPropertyAlias, ADefaultMembers);
  end;
...
function TdxBaseQueryGenerator.CreateBinaryGroup(const ASubMembers: IdxPropertyAlias; const ALeftOperator: IdxCriteriaOperator;
  const ALeftMembers: IdxPropertyAlias; const ARightOperator: IdxCriteriaOperator; const ARightMembers: IdxPropertyAlias;
  AOperatorType: TdxBinaryOperatorType): IdxCriteriaOperator;
var
...
begin
  ...
  begin
    if ATargetMember.ReferenceType <> nil then
      ATargetMember := ATargetMember.ReferenceType.KeyProperty;
    AList := AValue.AsType<TArray<TValue>>;
    AIndex := 0;
    for AMemberInfo in ATargetMember.SubMembers do
    begin
      if AMemberInfo.IsPersistent then
      begin
        if AMemberInfo = AMember then
        begin
          AValue := AList[AIndex];
          Break;
        end;
...
var
  ACondition, AMiCondition: IdxCriteriaOperator;
  AMemberInfo: TdxMappingMemberInfo;
  ALeftMemberMappingPrefix: string;
begin
  if Length(ARightMember.SubMembers) = 0 then
    Result := TdxBinaryOperator.Create(CreateOperand(ALeftMember, ALeftAlias), CreateOperand(ARightMember, ARightAlias),
      TdxBinaryOperatorType.Equal)
  else
  begin
    ACondition := nil;
    for AMemberInfo in ARightMember.SubMembers do
    begin
      if AMemberInfo.IsPersistent then
      begin
        if ALeftMember = ARightMember then
          ALeftMemberMappingPrefix := ''
        else
          ALeftMemberMappingPrefix := ALeftMember.MappingField;
        AMiCondition := TdxBinaryOperator.Create(
          CreateOperand(ALeftMemberMappingPrefix + AMemberInfo.MappingField, ALeftAlias),
          CreateOperand(AMemberInfo, ARightAlias),
          TdxBinaryOperatorType.Equal);
        ACondition := TdxGroupOperator.And(ACondition, AMiCondition);
      end;
    end;
    Result := ACondition;
  end;
...
function TdxBaseQueryGenerator.GetSubMembers(AMember: TdxMappingMemberInfo; const APrefix: string): TdxMemberInfoList;
var
  ACount, I: Integer;
  AMemberInfo: TdxMappingMemberInfo;
begin
  Result := TdxMemberInfoList.Create;
  ACount := Length(AMember.SubMembers);
  Result.Capacity := ACount;
  for I := 0 to ACount - 1 do
  begin
    AMemberInfo := AMember.SubMembers[I];
    if AMemberInfo.IsPersistent then
    begin
      if (AMemberInfo.ReferenceType <> nil) and (Length(AMemberInfo.ReferenceType.KeyProperty.SubMembers) > 0) then
        raise ENotSupportedException.CreateFmt('MetaData_ReferenceTooComplex', [AMember.Owner.FullName, AMember.MemberName]);
...
begin
  if AAlias.Member = nil then
    Result := AParam
  else
    if Supports(AParam, IdxOperandValue, AOperandValue) then
      Result := GetConstParameter(AAlias.GetMember(AIndex), AOperandValue, AAlias.Member)
    else
      if Supports(AParam, IdxPropertyAlias, AParamPropertyAlias) then
        Result := GetQueryOperandFromAlias(AParamPropertyAlias, AAlias, AIndex)
      else
        Result := AParam;
end;
...
var
  ...
  ADefaultMembers, ALeftMembers, ARightMembers: IdxPropertyAlias;
...
begin
  ...
  try
    ...
    try
      ...
      if ALeftOperator.QueryInterface(IdxPropertyAlias, ALeftMembers) = 0 then
        ADefaultMembers := ALeftMembers;
      if ADefaultMembers.Count > 1 then
        Result := BuildInGroup(ADefaultMembers, AValues, ALeftOperator)
      else
      begin
...
begin
  Result := IdxPropertyAlias(ExecuteWithPropertyNameDiving(AProperty.PropertyName,
    ...
    begin
      if APropertyPath.StartsWith('^.') then
      begin
        ALength := Length(FPropertyName);
        I := APropertyPath.LastIndexOf('^.') div 2 + 1;
        if I >= ALength then
          APropertyPath := APropertyPath.Remove(0, ALength * 2)
        else
        begin
