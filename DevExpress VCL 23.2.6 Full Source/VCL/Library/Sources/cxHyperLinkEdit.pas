...
type
  ...
  protected
    ...
    function CalculateTextColor(AStyle: TcxContainerStyle; AEnabled: Boolean; AContainer: TcxContainer = nil): TColor; override;
...
uses
  ShellAPI, Variants, Forms, cxGeometry, cxVariants, cxClasses, cxLibraryConsts,
  dxCoreGraphics, cxLookAndFeelPainters, cxDrawTextUtils;

const
  dxThisUnitName = 'cxHyperLinkEdit';
...
function TcxHyperLinkEditViewInfo.CalculateTextColor(AStyle: TcxContainerStyle; AEnabled: Boolean; AContainer: TcxContainer): TColor;
begin
  if IsInplace then
    Result := inherited CalculateTextColor(AStyle, AEnabled, AContainer)
  else
    Result := GetHyperLinkTextColor;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      ATextWidth := TdxTextMeasurer.TextWidthTO(TcxCustomTextEditAccess(Edit).Font, TcxCustomTextEditAccess(Edit).InnerTextEdit.EditValue);
