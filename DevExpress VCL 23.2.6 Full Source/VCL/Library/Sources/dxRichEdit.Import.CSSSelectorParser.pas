...
unit dxRichEdit.Import.CSSSelectorParser; // for internal use
...
type
 ...
 { TdxAttributePattern }
  ...
  strict private
    ...
    function GetSpecificity: Integer;
  protected
    function CalculateSpecificityCore(ASpecificity: Integer): Integer; virtual;
  public
    constructor Create;
    destructor Destroy; override;
    procedure InvalidateSpecificity;

    property Elements: TdxSelectorElementList read FElements;
    property Specificity: Integer read GetSpecificity;
  end;
...
uses
  Contnrs, Math, Character, dxCore;

const
  dxThisUnitName = 'dxRichEdit.Import.CSSSelectorParser';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxAttributePattern.Initialize', SysInit.HInstance);{$ENDIF}
  ...
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxAttributePattern.Initialize', SysInit.HInstance);{$ENDIF}
end;
...
function TdxSelector.GetSpecificity: Integer;
begin
  if FSpecificity = -1 then
    FSpecificity := CalculateSpecificityCore(FSpecificity);
  Result := FSpecificity;
end;

procedure TdxSelector.InvalidateSpecificity;
begin
  FSpecificity := -1;
end;

function TdxSelector.CalculateSpecificityCore(ASpecificity: Integer): Integer;
var
