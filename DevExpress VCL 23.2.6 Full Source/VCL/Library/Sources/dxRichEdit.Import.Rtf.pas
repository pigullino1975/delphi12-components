...
unit dxRichEdit.Import.Rtf; // for internal use
...
type
  ...
  protected
    ...
    procedure LinkParagraphStyleWithNumberingLists(AStyle: TdxParagraphStyle);
    ...
    class function CheckSignature(AStream: TStream): Boolean;static;
...
const
  dxThisUnitName = 'dxRichEdit.Import.Rtf';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxRichEditRtfDestinationBase.Initialize', SysInit.HInstance);{$ENDIF}
  FDefaultKeywordHT := CreateDefaultKeywordHT;
  FDefaultControlCharHT := CreateDefaultControlCharHT;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxRichEditRtfDestinationBase.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxRichEditRtfDestinationBase.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxRichEditRtfDestinationBase.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FDefaultControlCharHT);
  FreeAndNil(FDefaultKeywordHT);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxRichEditRtfDestinationBase.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  ...
  FCurrentOldMultiLevelListIndex := NumberingListIndexListIndexNotSet;
  FCurrentOldSimpleListIndex := NumberingListIndexListIndexNotSet;
end;
...
var
  AColumnCollection: TdxColumnInfoCollection;
  I: Integer;
begin
...
class function TdxRtfImporter.CheckSignature(AStream: TStream): Boolean;
var
