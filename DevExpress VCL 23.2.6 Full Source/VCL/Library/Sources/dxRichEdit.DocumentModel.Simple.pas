...
unit dxRichEdit.DocumentModel.Simple; // for internal use
...
type
  ...
  TdxRichEditHistoryItemClass = class of TdxRichEditHistoryItem;
  ...
  strict private
    ...
    function GetMergedCharacterFormatting: TdxCharacterFormattingInfo; inline;
    function GetMergedCharacterFormattingCacheIndex: Integer; inline;
    function GetParagraph: TdxSimpleParagraph; inline;
    function GetPieceTable: TdxSimplePieceTable; inline;
  ...
  protected
    ...
    procedure InheritRowProcessingFlags(ARun: TdxTextRunBase); virtual;
  ...
  public
    ...
    function GetNonEmptyText(ABuffer: TdxChunkedStringBuilder): string; inline;
  ...
  strict private
    function GetItem(Index: TdxRunIndex): TdxTextRunBase; inline;
  ...
  strict private
    function GetItem(Index: Integer): TdxSimpleParagraph; inline;
  public
    function First: TdxSimpleParagraph; reintroduce; inline;
    function Last: TdxSimpleParagraph; reintroduce; inline;
  ...
  strict private
    ...
    function GetParagraphs: TdxSimpleParagraphCollection; inline;
    function GetRuns: TdxTextRunCollection; inline;
...
const
  dxThisUnitName = 'dxRichEdit.DocumentModel.Simple';
...
begin
  ...
  InheritRowProcessingFlags(ARun);
...
procedure TdxTextRunBase.InheritRowProcessingFlags(ARun: TdxTextRunBase);
begin
...
begin
  Result := TdxTextRunBase(List.First);
end;

function TdxTextRunCollection.Last: TdxTextRunBase;
begin
  Result := TdxTextRunBase(List.Last);
end;
...
begin
  Result := TdxTextRunBase(List{$IFDEF RELEASE}.List{$ENDIF}[Index]);
end;
...
begin
  ...
  FNumberingListIndex := NumberingListIndexListIndexNotSet;
...
begin
  ...
  if (ARunInfo.Start.RunIndex < 0) or (ARunInfo.&End.RunIndex < ARunInfo.Start.RunIndex) then
    Exit;
  AModifier := TdxRunCharacterStyleModifier.CreateReplaceRunCharacterStyleProperties(AStyleIndex, ASource);
  try
...
var
  AInserter: TdxFieldRunInserterBase;
begin
  FTextBuffer.Append(AFieldResultEndRunMark);
  AInserter := TdxFieldRunInserterBase.CreateFieldResultEndRunInserter(Self);
  try
...
begin
  FTextBuffer.Append('{');
  AInserter := TdxFieldRunInserterBase.CreateFieldCodeStartRunInserter(Self);
  try
    Result := InsertObjectCore(AInserter, AParagraphIndex, ALogPosition, AForceVisible);
  finally
    AInserter.Free;
...
begin
  FTextBuffer.Append('}');
  AInserter := TdxFieldRunInserterBase.CreateFieldCodeEndRunInserter(Self);
  try
    Result := InsertObjectCore(AInserter, AParagraphIndex, ALogPosition, AForceVisible);
  finally
    AInserter.Free;
...
begin
  ...
  if AStyleIndex < 0 then
    Exit;
  AModifier := TdxRunCharacterStyleModifier.CreateRunCharacterStyleKeepOldStyleProperties(AStyleIndex, AApplyDefaultHyperlinkStyle);
  try
    ChangeCharacterStyle(ARunInfo, AModifier);
  finally
    AModifier.Free;
...
var
  AInserter: TdxSimplePieceTableObjectInserter;
begin
  TextBuffer.Append(TdxSeparatorTextRun.SeparatorText);
  AInserter := CreateSeparatorTextRunInserter(Self);
  try
...
begin
  ACount := Fields.Count;
  if not TdxAlgorithms1<TdxField>.BinarySearch(Fields.InnerList,
