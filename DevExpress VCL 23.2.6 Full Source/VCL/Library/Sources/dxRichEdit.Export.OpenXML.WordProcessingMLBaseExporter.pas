...
unit dxRichEdit.Export.OpenXML.WordProcessingMLBaseExporter; // for internal use
...
const
  dxThisUnitName = 'dxRichEdit.Export.OpenXML.WordProcessingMLBaseExporter';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxWordProcessingMLBaseExporter.Initialize', SysInit.HInstance);{$ENDIF}
  ...
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxWordProcessingMLBaseExporter.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxWordProcessingMLBaseExporter.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxWordProcessingMLBaseExporter.Finalize', SysInit.HInstance);{$ENDIF}
  ...
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxWordProcessingMLBaseExporter.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  ...
  try
    ...
    if AStyle.ParagraphProperties.Info.Options.Value <> TdxParagraphFormattingOptions.MaskUseNone then
      ExportStyleParagraphProperties(AStyle.ParagraphProperties, AStyle.Tabs.Info, NumberingListIndexListIndexNotSet, 0, NumberingListIndexListIndexNotSet);
  ...
  try
    ...
    begin
      ...
      try
        ...
        WriteWpStringAttr('val', TdxXmlConvert.EncodeName(AVariables[AName].ToString));
      finally
        WriteWpEndElement;
      end;
    end;
