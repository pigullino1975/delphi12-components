...
function IsShadowDrawingNeeded(AViewData: TcxCustomEditViewData): Boolean; deprecated;
  ...
  begin
    with AViewInfo do
      ...
      begin
        ...
        begin
          ATheme := OpenTheme(totComboBox, ScaleFactor);
      ...
      begin
        R := EditViewInfo.Painter.GetEditorButtonsPadding(ScaleFactor);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        with EditViewInfo.Painter.GetEditorButtonPadding(ScaleFactor) do
          Result := Left + Right + AButtonTotalBorderExtent;
        if Data.Kind = bkText then
          Inc(Result, ACanvas.TextWidth(Data.VisibleCaption));
        Result := Max(Result, dxGetSystemMetrics(SM_CYHSCROLL, ScaleFactor));
      end;
...
begin
  ...
  ConvertFlag(AFlags, Result, CXTO_END_ELLIPSIS, cxShowEndEllipsis);
  ConvertFlag(AFlags, Result, CXTO_NOCLIP, cxDontClip);
  ConvertFlag(AFlags, Result, CXTO_RTLREADING, cxRtlReading);
end;
...
begin
  ...
  ConvertFlag(AFlags, Result, cxShowEndEllipsis, CXTO_END_ELLIPSIS);
  ConvertFlag(AFlags, Result, cxDontClip, CXTO_NOCLIP);
  ConvertFlag(AFlags, Result, cxRtlReading, CXTO_RTLREADING);
end;
...
procedure DrawCustomEditBackground(ACanvas: TcxCanvas; AViewInfo: TcxCustomEditViewInfo; ACanDrawBackground: Boolean);
  ...
  begin
    ...
    begin
      ...
      if AButtonViewInfo.Data.BackgroundPartiallyTransparent then
        FillRect(ACanvas.Handle, AButtonViewInfo.Bounds, ABackgroundBrush);
    end;
  end;

  procedure FillEditBorderRect(ABackgroundBrush: TBrushHandle);
  begin
    FillRect(ACanvas.Handle, AViewInfo.BorderRect, ABackgroundBrush);
  end;

  procedure FillContentOffsetRegion(ABackgroundBrush: TBrushHandle);
  begin
    if AViewInfo.HasContentOffsets then
    begin
  ...
  begin
    ...
    begin
      AViewInfo.Painter.DrawContainerBorderedBackground(ACanvas, R,
        TcxContainerBorderStyle(AViewInfo.BorderStyle),
        ABorderWidth, AViewInfo.BorderColor, AViewInfo.Edges, AViewInfo.BorderState);
    end;
  end;
  ...
  begin
    ...
    if not AViewInfo.SupportsBorders then
    begin
      ...
      begin
        if not AViewInfo.Transparent then
          ...
          begin
            FillRect(ACanvas.Handle, AViewInfo.Bounds, ABackgroundBrush);
            AViewInfo.DrawBorderedBackground(ACanvas, R);
          end
          else
          begin
        ...
        if not AViewInfo.Transparent and ACanDrawBackground then
          AViewInfo.DrawBorderedBackground(ACanvas, R);
      Exit;
    end;
...
begin
  ...
  begin
    ABackgroundBrush := GetSolidBrush(AViewInfo.BackgroundColor);
