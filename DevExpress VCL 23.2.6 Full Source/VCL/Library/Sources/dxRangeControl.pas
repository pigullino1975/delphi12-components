...
uses
  ...
  cxEdit, cxExtEditConsts, cxExtEditUtils, cxLookAndFeelPainters, cxGeometry, dxThemeConsts, cxCustomCanvas,
  cxFilterControlUtils, cxGraphics, cxLookAndFeels, cxTextEdit, cxVariants, Math, dxCustomHint, dxTypeHelpers,
  dxAnimation, Generics.Collections, Generics.Defaults, cxAccessibility;
...
type
  ...
  public
    ...
    function GetHitTest(AHitTest: TdxCustomRangeControlHitTest): Boolean; virtual;
    ...
    property ViewInfo: TdxCustomRangeControlViewInfo read FViewInfo; // for internal use
  end;
  ...
  public
    ...
    procedure CalculateBounds(const ABounds: TRect); override;
    procedure Paint(ACanvas: TcxCanvas); override;
    property MaxZoomGrip: TdxRangeControlZoomScrollBarZoomGripViewInfo read FMaxZoomGrip; // for internal use
    property MinZoomGrip: TdxRangeControlZoomScrollBarZoomGripViewInfo read FMinZoomGrip; // for internal use
    property SelectionScrollThumb: TdxRangeControlZoomScrollBarSelectionScrollThumbViewInfo read FSelectionScrollThumb; // for internal use
    property ViewportScrollThumb: TdxRangeControlZoomScrollBarViewportScrollThumbViewInfo read FViewportScrollThumb; // for internal use
  end;
  ...
  public
    ...
    destructor Destroy; override;
    procedure CalculateBounds(const ABounds: TRect); override;
    function GetHitTest(AHitTest: TdxCustomRangeControlHitTest): Boolean; override;
    procedure Paint(ACanvas: TcxCanvas); override;
    property MaxSelectionThumb: TdxRangeControlClientAreaSelectionThumbViewInfo read FMaxSelectionThumb; // for internal use
    property MinSelectionThumb: TdxRangeControlClientAreaSelectionThumbViewInfo read FMinSelectionThumb; // for internal use
    property SelectedRange: TdxRangeControlClientAreaSelectedRangeViewInfo read FSelectedRange; // for internal use
    property SelectedRangeTarget: TdxRangeControlClientAreaSelectedRangeTargetViewInfo read FSelectedRangeTarget; // for internal use
  end;
  ...
  public
    ...
    procedure CalculateBounds(const ABounds: TRect); override;
    property Properties: TdxRangeControlCustomClientProperties read GetProperties;
    property Content: TdxRangeControlContentViewInfo read FContent; // for internal use
    property Ruler: TdxRangeControlRulerViewInfo read FRuler; // for internal use
  end;
  ...
  public
    ...
    property ClientArea: TdxRangeControlClientAreaViewInfo read FClientArea; // for internal use
    property ClientAreaNormalFactor: Double read FClientAreaNormalFactor;
    property ClientInfo: TdxRangeControlCustomClientViewInfo read FClientInfo; // for internal use
    property ErrorInfo: TdxRangeControlErrorInfo read FErrorInfo; // for internal use
    property Ruler: TdxRangeControlRulerViewInfo read GetRuler; // for internal use
    property ZoomScrollBar: TdxRangeControlZoomScrollBarViewInfo read FZoomScrollBar; // for internal use
  ...
  { TdxRangeControlAccessibilityHelper }

  TdxRangeControlAccessibilityHelper = class(TcxAccessibilityHelper) // for internal use
  strict private
    function GetRangeControl: TdxCustomRangeControl;
  protected
...
function TdxRangeControlClientAreaViewInfo.GetHitTest(AHitTest: TdxCustomRangeControlHitTest): Boolean;
begin
  Result := FMinSelectionThumb.GetHitTest(AHitTest) or
    FMaxSelectionThumb.GetHitTest(AHitTest) or inherited GetHitTest(AHitTest);
end;
...
begin
  Result := TdxTextMeasurer.TextLineHeight(ViewInfo.GetRulerLabelFont) + ScaleFactor.Apply(7);
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxRangeControlDateTimeAutoFormatHelper.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FDayFormats);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxRangeControlDateTimeAutoFormatHelper.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxRangeControlDateTimeAutoFormatHelper.Initialize', SysInit.HInstance);{$ENDIF}
  FDayFormats := TStringList.Create;
  InitializeDayFormats;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxRangeControlDateTimeAutoFormatHelper.Initialize', SysInit.HInstance);{$ENDIF}
end;
...
function TdxRangeControlAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  Result := cxRectSetOrigin(RangeControl.Bounds, RangeControl.ClientToScreen(RangeControl.Bounds.TopLeft))
end;

function TdxRangeControlAccessibilityHelper.GetOwnerObjectWindow: HWND;
begin
  Result := RangeControl.Handle;
end;

function TdxRangeControlAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxROLE_SYSTEM_SLIDER;
end;

function TdxRangeControlAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxSTATE_SYSTEM_FOCUSABLE;
  if not RangeControl.Enabled then
    Result := Result or cxSTATE_SYSTEM_UNAVAILABLE;
  if RangeControl.Focused then
    Result := Result or cxSTATE_SYSTEM_FOCUSED;
  if not RangeControl.Visible then
    Result := Result or cxSTATE_SYSTEM_INVISIBLE;
end;

function TdxRangeControlAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := [aopFocus, aopLocation, aopValue];
end;

function TdxRangeControlAccessibilityHelper.GetValue(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := Format('%s - %s', [VarToStr(RangeControl.SelectedRangeMinValue), VarToStr(RangeControl.SelectedRangeMaxValue)]);
end;

function TdxRangeControlAccessibilityHelper.IsExtended: Boolean;
begin
  Result := True;
end;

function TdxRangeControlAccessibilityHelper.GetRangeControl: TdxCustomRangeControl;
begin
  Result := TdxCustomRangeControl(OwnerObject);
end;
...
procedure TdxCustomRangeControl.DrawBorder(ACanvas: TcxCustomCanvas);
begin
  ACanvas.FrameRect(Bounds, cxGetActualColor(Style.BorderColor, LookAndFeelPainter.GetRangeControlBorderColor));
end;
...
function TdxCustomRangeControl.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin
  if FIAccessibilityHelper = nil then
    FIAccessibilityHelper := TdxRangeControlAccessibilityHelper.Create(Self);
...
procedure TdxCustomRangeControl.WMGetObject(var Message: TMessage);
var
  AObjectID: Cardinal;
begin
