...
type
  ...
  { TdxCloudStorageMicrosoftOneDriveProviderCreateSharedLinkTask }

  TdxCloudStorageMicrosoftOneDriveProviderCreateSharedLinkTask = class(TdxCloudStorageProviderCreateSharedLinkCustomTask)
  strict private
  ...
  { TdxCloudStorageMicrosoftOneDriveProviderInviteUsersTask }

  TdxCloudStorageMicrosoftOneDriveProviderInviteUsersTask = class(TdxCloudStorageProviderInviteUsersCustomTask)
  strict private
    function GetResultObject(const ACancelStatus: TdxTaskCancelCallback; out AResult: TdxJSONObject): TdxTaskCompletedStatus;
    function GetRequestObject: TdxJSONObject;
  protected
  ...
  { TdxCloudStorageMicrosoftOneDriveProviderDeletePermissionTask }

  TdxCloudStorageMicrosoftOneDriveProviderDeletePermissionTask = class(TdxCloudStorageProviderDeletePermissionTask)
  strict private
  ...
  { TdxCloudStorageMicrosoftOneDriveProviderFetchPermissionsTask }

  TdxCloudStorageMicrosoftOneDriveProviderFetchPermissionsTask = class(TdxCloudStorageProviderFetchPermissionsCustomTask)
  strict private
    function GetUri: string;
  protected
    function DoRun(const ACancelStatus: TdxTaskCancelCallback): TdxTaskCompletedStatus; override;
  ...
  protected
    ...
    function CreateCreateSharedLinkTask(AItem: TdxCloudStorageItem;
      AReadOnly: Boolean; AScope: TdxCloudStorageItemPermissionScope;
      ACallback: TdxCloudStoragePermissionCallback = nil): TdxCloudStorageProviderCreateSharedLinkCustomTask; override;
    ...
    function CreateInviteUsersTask(AItem: TdxCloudStorageItem; const AUsers: TArray<string>; AReadOnly: Boolean;
      ARequireSignIn: Boolean; ASendInvitation: Boolean; const AMessage: string; ACallback: TdxCloudStoragePermissionCallback = nil): TdxCloudStorageProviderInviteUsersCustomTask; override;
...
const
  dxThisUnitName = 'dxCloudStorageMicrosoftOneDriveProvider';
...
var
  ...
  AArray: TdxJSONArray;
begin
  Result := AValue.HasParam('id');
  if not Result then
    Exit;
  AData.ID := AValue.GetParamValue('id');
  AData.Link := AValue.GetChildParamValue('link', 'webUrl');
  AData.ReadOnly := False;
  if AValue.GetValue('roles').IsArray then
  begin
    AArray := TdxJSONArray(AValue.GetValue('roles'));
    for I := 0 to AArray.Count - 1 do
    begin
      if AArray.Items[I].Value = 'read' then
        AData.ReadOnly := True;
    end;
  end;
  AData.InheritedFrom := AValue.HasParam('inheritedFrom');
  AData.InheritedFromDriveID := AValue.GetChildParamValue('inheritedFrom', 'driveId');
  AData.InheritedFromID := AValue.GetChildParamValue('inheritedFrom', 'id');
  AData.InheritedFromName := AValue.GetChildParamValue('inheritedFrom', 'name');
  AData.User := AValue.GetChildParamValue('invitation', 'email');
end;

{ TdxCloudStorageMicrosoftOneDriveProviderCreateSharedLinkTask }

function TdxCloudStorageMicrosoftOneDriveProviderCreateSharedLinkTask.DoRun(
...
function TdxCloudStorageMicrosoftOneDriveProviderCreateSharedLinkTask.GetProvider: TdxCloudStorageMicrosoftOneDriveProvider;
begin
  Result := TdxCloudStorageMicrosoftOneDriveProvider(inherited Provider);
end;

function TdxCloudStorageMicrosoftOneDriveProviderCreateSharedLinkTask.GetResultObject(const ACancelStatus: TdxTaskCancelCallback; out AResult: TdxJSONObject): TdxTaskCompletedStatus;
var
...
begin
  ...
  try
    ...
    AResult := TdxHttpHelper.PostRequest(UserAgent, TdxCloudStorageMicrosoftOneDriveProvider.FServerName,
      Format('v1.0/me/drive/items/%s/createLink', [Item.ID]), Header, AObject);
    Result := TdxTaskCompletedStatus.Success;
  finally
    AObject.Free;
  end;
...
function TdxCloudStorageMicrosoftOneDriveProviderCreateSharedLinkTask.GetRequestObject: TdxJSONObject;
begin
  Result := TdxJSONObject.Create;
  if ReadOnly then
    Result.AddPair('type', 'view')
  else
    Result.AddPair('type', 'edit');
  if Scope = TdxCloudStorageItemPermissionScope.Anonymous then
    Result.AddPair('scope', 'anonymous')
  else
    Result.AddPair('scope', 'organization');
end;

{ TdxCloudStorageMicrosoftOneDriveProviderInviteUsersTask }

function TdxCloudStorageMicrosoftOneDriveProviderInviteUsersTask.DoRun(
  const ACancelStatus: TdxTaskCancelCallback): TdxTaskCompletedStatus;
var
  AResult: TdxJSONObject;
...
begin
  ...
  try
    ...
    begin
      Result := TdxTaskCompletedStatus.Fail;
      if (AResult = nil) or not AResult.GetValue('value').IsArray then
        Exit;
      AValues := TdxJSONArray(AResult.GetValue('value'));
      for I := 0 to AValues.Count - 1 do
      begin
        if ACancelStatus then
          Exit(TdxTaskCompletedStatus.Cancelled);
        if ValueToPermissionData(TdxJSONObject(AValues.Items[I]), AData) then
          Result := TdxTaskCompletedStatus.Success
        else
          DoError(AResult);
        Items.Add(AData);
...
function TdxCloudStorageMicrosoftOneDriveProviderInviteUsersTask.GetRequestObject: TdxJSONObject;
var
  ARoles: TdxJSONArray;
  ARecipients: TdxJSONArray;
  I: Integer;
  AUser: TdxJSONObject;
begin
  Result := TdxJSONObject.Create;
  ...
  Result.AddPair('requireSignIn', TdxJSONValue.CreateBooleanValue(RequireSignIn));
  Result.AddPair('sendInvitation', TdxJSONValue.CreateBooleanValue(SendInvitation));
  ARoles := TdxJSONArray.Create;
  if ReadOnly then
    ARoles.Add('read')
  else
    ARoles.Add('write');
  Result.AddPair('roles', ARoles);
  ARecipients := TdxJSONArray.Create;
  for I := 0 to Length(Users) - 1 do
  begin
    AUser := TdxJSONObject.Create;
    AUser.AddPair('email', Users[I]);
    ARecipients.AddElement(AUser);
  end;
  Result.AddPair('recipients', ARecipients);
end;

function TdxCloudStorageMicrosoftOneDriveProviderInviteUsersTask.GetResultObject(
...
begin
  ...
  try
    UpdateRequestParams;
    AResult := TdxHttpHelper.PostRequest(UserAgent, TdxCloudStorageMicrosoftOneDriveProvider.FServerName,
      Format('v1.0/me/drive/items/%s/invite', [Item.ID]), Header, AObject);
    Result := TdxTaskCompletedStatus.Success;
  finally
    AObject.Free;
  end;
...
function TdxCloudStorageMicrosoftOneDriveProviderDeletePermissionTask.DoRun(
  const ACancelStatus: TdxTaskCancelCallback): TdxTaskCompletedStatus;
var
...
begin
  Result := TdxTaskCompletedStatus.Fail;
  UpdateRequestParams;
  AResponse := TdxHttpHelper.DeleteRequest(UserAgent, TdxCloudStorageMicrosoftOneDriveProvider.FServerName,
    GetUri, Header, nil);
  try
...
function TdxCloudStorageMicrosoftOneDriveProviderDeletePermissionTask.GetUri: string;
begin
  if (Item is TdxCloudStorageFolder) and TdxCloudStorageFolder(Item).IsRoot then
    Result := Format('v1.0/me/drive/root/permissions/%s', [Permission.ID])
  else
    if Item.DriveID <> '' then
      Result := Format('v1.0/drives/%s/items/%s/permissions/%s', [Item.DriveID, Item.ID, Permission.ID])
    else
      Result := Format('v1.0/me/drive/items/%s/permissions/%s', [Item.ID, Permission.ID]);
end;
...
begin
  ...
  begin
    ...
    AObject := TdxHttpHelper.GetRequest(UserAgent, AUri, Header);
    try
      if (AObject = nil) or not AObject.HasParam('value') or
        (not IsTextFullValid(AObject.GetParamValue('@odata.context'), 'https:\/\/graph\.microsoft\.com\/v1\.0\/\$metadata#Collection\((microsoft\.graph\.)?driveItem\)')) then
      begin
        if AObject <> nil then
          DoError(AObject);
        Exit(TdxTaskCompletedStatus.Fail);
...
function TdxCloudStorageMicrosoftOneDriveProviderFetchPermissionsTask.DoRun(
  const ACancelStatus: TdxTaskCancelCallback): TdxTaskCompletedStatus;
var
  AData: TdxCloudStorageItemPermissionData;
  AResponse: TdxJSONObject;
...
begin
  ...
  if ACancelStatus then
    Exit(TdxTaskCompletedStatus.Cancelled);
  AResponse := TdxHttpHelper.GetRequest(UserAgent, GetUri, Header);
  try
    if not AResponse.HasParam('value') or not AResponse.GetValue('value').IsArray then
    begin
      DoError(AResponse);
      Exit;
    end;
    AValues := TdxJSONArray(AResponse.GetValue('value'));
    for I := 0 to AValues.Count - 1 do
    begin
      if ACancelStatus then
        Exit(TdxTaskCompletedStatus.Cancelled);
...
function TdxCloudStorageMicrosoftOneDriveProviderFetchPermissionsTask.GetUri: string;
begin
  if (Item is TdxCloudStorageFolder) and TdxCloudStorageFolder(Item).IsRoot then
    Result := 'https://graph.microsoft.com/v1.0/me/drive/root/permissions'
  else
    if Item.DriveID <> '' then
      Result := Format('https://graph.microsoft.com/v1.0/drives/%s/items/%s/permissions', [Item.DriveID, Item.ID])
    else
      Result := Format('https://graph.microsoft.com/v1.0/me/drive/items/%s/permissions', [Item.ID]);
end;
...
var
  ...
  AUri: string;
  AParams: TBytes;
  ASession, AConnection, ARequest: TdxInternet;
  ALocation: string;
  AObject: TdxJSONObject;
begin
  Result := TdxTaskCompletedStatus.Fail;
  ...
  try
    ...
    if TdxHttpHelper.OpenSession(UserAgent, ASession) then
    try
      if TdxHttpHelper.OpenConnection(ASession, TdxCloudStorageMicrosoftOneDriveProvider.FServerName, AConnection) then
      try
        if TdxHttpHelper.OpenRequest(AConnection, 'POST', AUri, ARequest) then
        try
          if not HTTPSendRequest(ARequest, PChar(Header), Length(Header), @AParams[0], Length(AParams)) then
            Exit;
          if ACancelStatus then
            Exit(TdxTaskCompletedStatus.Cancelled);
          ALocation := TdxHttpRequest.GetResponseCustomHeader(ARequest, 'Location');
          if ALocation = '' then
            Exit;
          while True do
          begin
          ...
          TdxHttpHelper.CloseRequest(ARequest);
        end;
      finally
        TdxHttpHelper.CloseConnection(AConnection);
      end;
    finally
      TdxHttpHelper.CloseSession(ASession);
    end;
  finally
    ARequestObject.Free;
  end;
...
//      Result := 'https://graph.microsoft.com/v1.0/me/drive/deletedItems';
  else
    Result := '';
  end;
end;
...
begin
  ...
  try
    ...
    if TdxHttpHelper.OpenSession(UserAgent, ASession) then
    try
      if TdxHttpHelper.OpenConnection(ASession, AUriRecord.Host, AConnection) then
      try
        if TdxHttpHelper.OpenRequest(AConnection, 'PUT', AObjectName, ARequest) then
        try
          Position := 0;
...
begin
  inherited Create(AProvider, AFile.ID,
    Format('v1.0/drives/%s/items/%s/content', [AFile.DriveID, AFile.ID]), AStream, nil, AProgressCallback);
  FFile := AFile;
...
begin
  ...
  try
    inherited Create(AProvider, AFileName,
      Format('v1.0/drives/%s/items/%s:/%s:/content', [AParent.DriveID, AParent.ID, TdxUri.EscapeDataString(ExtractFileName(AFileName))]), AStream, ACallback, AProgressCallback);
  finally
    AStream.Free;
  end;
  FParent := AParent;
...
function TdxCloudStorageMicrosoftOneDriveFileTransferManager.CreateUpdateFileStreamTask(AFile: TdxCloudStorageFile;
  AStream: TStream; AProgressCallback: TdxCloudStorageFileProgressCallback = nil): TdxCloudStorageUploadTask;
begin
  Result := TdxCloudStorageMicrosoftOneDriveUpdateFileStreamTask.Create(Provider, AFile, AStream, AProgressCallback);
end;

function TdxCloudStorageMicrosoftOneDriveFileTransferManager.CreateUploadFileTask(
  AParent: TdxCloudStorageFolder; const AFileName: string;
  ACallback: TdxCloudStorageFileCallback = nil;
  AProgressCallback: TdxCloudStorageFileProgressCallback = nil): TdxCloudStorageUploadTask;
begin
  Result := TdxCloudStorageMicrosoftOneDriveUploadFileTask.Create(Provider, AParent, AFileName, ACallback, AProgressCallback);
end;
...
function TdxCloudStorageMicrosoftOneDriveProvider.CreateCreateFolderTask(AParent: TdxCloudStorageFolder;
  const AName: string; ACallback: TdxCloudStorageFolderCallback = nil): TdxCloudStorageProviderCreateFolderTask;
begin
  Result := TdxCloudStorageMicrosoftOneDriveProviderCreateFolderTask.Create(Self, AParent, AName, ACallback);
end;

function TdxCloudStorageMicrosoftOneDriveProvider.CreateCreateSharedLinkTask(AItem: TdxCloudStorageItem;
  AReadOnly: Boolean; AScope: TdxCloudStorageItemPermissionScope;
  ACallback: TdxCloudStoragePermissionCallback = nil): TdxCloudStorageProviderCreateSharedLinkCustomTask;
begin
  Result := TdxCloudStorageMicrosoftOneDriveProviderCreateSharedLinkTask.Create(Self, AItem, AReadOnly, AScope, ACallback);
end;

function TdxCloudStorageMicrosoftOneDriveProvider.CreateDeletePermissionTask(
  APermission: TdxCloudStorageItemPermission): TdxCloudStorageProviderDeletePermissionTask;
begin
  Result := TdxCloudStorageMicrosoftOneDriveProviderDeletePermissionTask.Create(Self, APermission);
end;
...
function TdxCloudStorageMicrosoftOneDriveProvider.CreateFetchPermissionsTask(AItem: TdxCloudStorageItem): TdxCloudStorageProviderFetchPermissionsCustomTask;
begin
  Result := TdxCloudStorageMicrosoftOneDriveProviderFetchPermissionsTask.Create(Self, AItem);
end;

function TdxCloudStorageMicrosoftOneDriveProvider.CreateInviteUsersTask(
  AItem: TdxCloudStorageItem; const AUsers: System.TArray<System.string>;
  AReadOnly, ARequireSignIn, ASendInvitation: Boolean;
  const AMessage: string;
  ACallback: TdxCloudStoragePermissionCallback = nil): TdxCloudStorageProviderInviteUsersCustomTask;
begin
  Result := TdxCloudStorageMicrosoftOneDriveProviderInviteUsersTask.Create(Self, AItem, AUsers, AReadOnly,
