...
const
  dxThisUnitName = 'dxEMF.DB.Oracle';
          ...
          begin
            ...
            if ALikeIndex > 0 then
              Exit(Format('((%0:s like %2:s) And (Instr(%0:s, %1:s) = 1))',
                ...
                AProcessParameter(TdxConstantValue.Create(AOperandString.Substring(0, ALikeIndex) + '%'))]));
        end;
...
begin
  ...
  try
    try
      ACount := GetScalar(AQuery1);
      Result := not VarIsNull(ACount) and (ACount = 1);
      if Result then
      begin
        ACount := GetScalar(AQuery2);
        Result := not VarIsNull(ACount) and (ACount = 1);
      end;
    except
      Result := False;
  ...
  try
    ...
    begin
      ...
      if not ADataSet.Fields[6].IsNull then
        ADataDefault := ADataSet.Fields[6].Value
      else
        ADataDefault := Null;
