...
uses
  ...
  {$IFDEF DELPHI120}dxMediaUtils,{$ENDIF}
...
const
  dxThisUnitName = 'dxCameraControl';
...
begin
  ScaleFactor.Assign(FCameraControl.ScaleFactor);
  InitSettingsPage;
...
begin
  ...
  AItemHeight := TdxTextMeasurer.TextLineHeight(Font) * 2;
...
begin
  ...
  begin
    AItem := CreateItem('', cxSizeToString(FCameraControl.Resolutions[I]));
...
procedure TdxCameraControlSettingsToolbar.Reinitialize;
begin
...
begin
  ...
  begin
    ...
    FColumnCount := Ceil(Sqrt(FItems.Count * TdxTextMeasurer.TextLineHeight(Font) * 2 * Result.cx) / Result.cx);
  end;
  Result.cx := Max(Result.cx, cxTextWidth(Font, Items[0].FCaption) + ScaleFactor.Apply(5));
  Result := cxSize(Result.cx * FColumnCount, (Ceil((FItems.Count - 1) / FColumnCount) + 1) * TdxTextMeasurer.TextLineHeight(Font) * 2);
end;
...
const
  dxCameraStateToCameraControlState: array[TdxCameraState] of TdxCameraControlState = (ccsInitializing, ccsRunning, ccsDeviceIsBusy);
var
...
procedure TdxCustomCameraControl.Paint;
  ...
  function NeedSmoothResize(const AFrameRect: TRect): Boolean;
  begin
    Result := (cxRectWidth(ClientRect) < cxRectWidth(AFrameRect)) or (cxRectHeight(ClientRect) < cxRectHeight(AFrameRect));
  end;
...
begin
  ...
  case State of
    ccsRunning:
      TdxImageDrawer.DrawImage(Canvas, ClientRect, FCamera.PreviewBitmap, FFitMode, nil, nil, NeedSmoothResize(FCamera.PreviewBitmap.ClientRect));
    ccsPaused:
      TdxImageDrawer.DrawImage(Canvas, ClientRect, CapturedBitmap, FFitMode, nil, nil, NeedSmoothResize(CapturedBitmap.ClientRect));
    else
