...
uses
  ...
  dxCore, dxCoreClasses, cxClasses, dxFormattedText, cxDrawTextUtils,
...
type
  ...
  IcxHintableObject2 = interface(IcxHintableObject) // for internal use
  ['{2D6C9CAB-443E-4F72-A3A6-1467489580D4}']
  ...
  protected
    ...
    procedure UpdateScaleFactorFromPos(const P: TPoint);
...
//    property HintStyle: TcxCustomHintStyle read FHintStyle write SetHintStyle;
  ...
  strict private
    ...
    FScaleFactor: TdxScaleFactor;
    FText: string;
    FFormattedHintText: TdxFormattedText;
  ...
  private
    FActivatingHintObject: TObject;
    FActivatingHintIntf: IcxHintableObject2;
  ...
  protected
    ...
    property HintAreaBounds: TRect read FHintAreaBounds;
    property HintShowPause: Integer read FHintShowPause write FHintShowPause;
  public
    constructor Create;
    destructor Destroy; override;
    procedure ActivateHint(const AHintableObject: IcxHintableObject2);
    procedure CancelHint; virtual;
    procedure HideHint;
    procedure MouseDown; virtual;
    procedure ResetLastHintElement; virtual;
    ...
    procedure UnsubscribeHintObject(AHintObject: TObject); // for internal use
...
uses
  Math, dxDPIAwareUtils, cxControls, cxContainer;

const
  dxThisUnitName = 'dxCustomHint';

const
  dxMaxHintTextLength = 32*1024;
...
function cxProcessControlHintInfo(out AHintInfo: THintInfo;
  ...
  begin
    ...
    if P <> nil then
      ...
      begin
        Start := P;
        while not dxCharInSet(P^, dxBreakLineCharacters) do
          P := StrNextChar(P);
...
begin
  ...
  begin
    if not (csDesigning in AController.ComponentState) then
    begin
      if FControllerList.Count = 0 then
...
procedure TcxHintController.UnRegisterHintStyleController(AController: TcxCustomHintStyleController);
  ...
  function HasNotDesignTimeController: Boolean;
  var
    I: Integer;
  begin
    Result := False;
    for I := 0 to FControllerList.Count - 1 do
      if not (csDesigning in TcxHintController(FControllerList[I]).ComponentState) then
        Exit(True);
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if not HasNotDesignTimeController then
        UnInitGlobalParameters;
    end
    else
      FControllerList.Remove(AController);
  end;
end;
...
procedure TcxBaseHintWindow.ScaleForPPI(const PPI: Integer);
begin
  ChangeScale(PPI, ScaleFactor.TargetDPI);
  ScaleFactor.Assign(PPI, dxDefaultDPI);
end;

procedure TcxBaseHintWindow.UpdateScaleFactorFromPos(const P: TPoint);
var
  ATargetDPI: Integer;
begin
  ATargetDPI := dxGetMonitorDPI(P);
  if ATargetDPI <> ScaleFactor.TargetDPI then
...
procedure TcxCustomHintWindow.ShowHint(X, Y: Integer; ACaption, AHint: string;
  AMaxWidth: Integer);
begin
  UpdateScaleFactorFromPos(Point(X, Y));
end;
...
begin
  ...
  FPos := ACursorPos;
  FFormattedHintText := TdxFormattedText.Create;
end;

destructor TdxHintViewInfoHelper.Destroy;
begin
  FreeAndNil(FScaleFactor);
  FreeAndNil(FFormattedHintText);
  inherited;
end;
...
begin
  ...
  FFormattedHintText.Import(FText, ACanvas.Font);
  if FText <> '' then
  begin
    if FFormattedHintText.HasFormatting then
    begin
      FFormattedHintText.CalculateLayout(ACanvas, ACanvas.Font, Rect(0, 0, 1, 1), CXTO_NOCLIP, ScaleFactor);
      ASize := FFormattedHintText.TextSize;
    end
    else
...
begin
  R := GetTextRect;
  if FFormattedHintText.HasFormatting then
    FFormattedHintText.Draw(ACanvas, R.TopLeft, False)
  else
  begin
...
constructor TcxCustomHintHelper.Create;
begin
  inherited Create;
  FHintShowPause := 900;
end;
...
procedure TcxCustomHintHelper.ActivateHint(const AHintableObject: IcxHintableObject2);
var
  AHintObject: TObject;
begin
  if AHintableObject <> nil then
    AHintObject := AHintableObject.GetHintObject
  else
    AHintObject := nil;
  if AHintObject = FActivatingHintObject then
    Exit;
  if AHintObject = FHintableObject then
    Exit;
  if AHintObject = FLastHintableObject then
    Exit;
  FLastHintableObject := nil;
  InternalCancelHint;
  if (AHintableObject <> nil) and (AHintableObject.GetHintText <> '') then
  begin
    FActivatingHintIntf := AHintableObject;
    FActivatingHintObject := AHintObject;
    FHintableObject := AHintObject;
    BeginMouseTracking(GetHintWinControl, cxNullRect, Self);
    StartShowHintTimer;
  end;
end;

procedure TcxCustomHintHelper.CancelShowHint;
begin
  FActivatingHintObject := nil;
  FActivatingHintIntf := nil;
...
procedure TcxCustomHintHelper.HintShowTimerHandler(Sender: TObject);
begin
  if FActivatingHintIntf <> nil then
  begin
    ShowHint(
      FActivatingHintIntf.GetHintAreaBounds,
      FActivatingHintIntf.GetHintTextBounds,
      FActivatingHintIntf.GetHintText,
      FActivatingHintIntf.IsHintMultiLine,
      FActivatingHintIntf.GetHintObject,
      FActivatingHintIntf.GetHintFont);
  end;
  CancelShowHint;
end;

procedure TcxCustomHintHelper.InternalCancelHint;
begin
  CancelShowHint;
  HideHint;
...
{$IFDEF DELPHI104SYDNEY}
  var
    AOldFont: TFont;
    AOnScreenHintFontChangeHandler: TNotifyEvent;
{$ENDIF}

  procedure PrepareHintFont;
  begin
    if Assigned(AFont) then
    begin
      FHintWindow.Canvas.Font := AFont;
{$IFDEF DELPHI104SYDNEY}
      AOldFont := TFont.Create;
      AOldFont.Assign(Screen.HintFont);
      AOnScreenHintFontChangeHandler := Screen.HintFont.OnChange;
      Screen.HintFont.OnChange := nil;
      Screen.HintFont.Assign(AFont);
      if Screen.ActiveCustomForm <> nil then
        Screen.HintFont.Height := MulDiv(Screen.HintFont.Height, Screen.PixelsPerInch, Screen.ActiveCustomForm.CurrentPPI);
{$ENDIF}
    end;
  end;

  procedure UnprepareHintFont;
  begin
{$IFDEF DELPHI104SYDNEY}
    if Assigned(AFont) then
    begin
      Screen.HintFont.Assign(AOldFont);
      Screen.HintFont.OnChange := AOnScreenHintFontChangeHandler;
      FreeAndNil(AOldFont);
    end;
{$ENDIF}
  end;
...
begin
  if not CanShowHint then
    Exit;
  S := Copy(AText, 1, dxMaxHintTextLength);
  if (AHintObject = FLastHintableObject) then
  begin
  ...
  if (FHintWindow = nil) or (FHintWindow.ClassType <> GetHintWindowClass) then
    ...
    begin
      if (FLastHintText = S) and cxRectIsEqual(FHintTextRect, ATextRect) then
        Exit;
      EndMouseTracking(Self);
    end;
  PrepareHintFont;
  try
    ...
    AHintWindowRect := GetHintWindowRect(AHintAreaBounds, ATextRect, S, AIsHintMultiLine);
    if cxProcessControlHintInfo(AHintInfo, FHintWindow, GetOwnerWinControl, S,
...
procedure TcxCustomHintHelper.UnsubscribeHintObject(AHintObject: TObject);
begin
  if FHintableObject = AHintObject then
    CancelHint
  else if FActivatingHintObject = AHintObject then
    CancelShowHint;
end;
...
procedure TcxCustomHintHelper.StartShowHintTimer;
begin
  StopHintCheckerTimers;
  FreeAndNil(FHintShowTimer);
  FHintShowTimer := cxCreateTimer(HintShowTimerHandler, HintShowPause);
end;
