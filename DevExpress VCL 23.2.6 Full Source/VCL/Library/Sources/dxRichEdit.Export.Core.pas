...
unit dxRichEdit.Export.Core; // for internal use
...
type
  ...
  protected
    ...
    function CreateSaveFileDialog(AFilters: TdxFileDialogFilterCollection; ACurrentFilterIndex: Integer): TSaveDialog; virtual;
    ...
    procedure SetDirectoryName(ADialog: TSaveDialog; const ADirectoryName: string);
    procedure SetFileName(ADialog: TSaveDialog; const AFileName: string);
    function ShowSaveFileDialog(ADialog: TSaveDialog; const AParent: TWinControl): Boolean; virtual;
  public
...
const
  dxThisUnitName = 'dxRichEdit.Export.Core';
...
begin
  ...
  if (AEncryptionPropertiesContainer <> nil) and (AEncryptionPropertiesContainer.EncryptionProperties.Password <> '') and not AExporter.SupportsEncryption then
    if dxMessageBox(cxGetResourceString(@sdxRichEditConfirmSaveDocumentWithPasswordProtection), Application.Title, MB_YESNO or MB_ICONWARNING) <> ID_YES then
      Abort;
...
function TdxExportHelper.ShowSaveFileDialog(ADialog: TSaveDialog;
...
function TdxExportHelper.CreateSaveFileDialog(AFilters: TdxFileDialogFilterCollection; ACurrentFilterIndex: Integer): TSaveDialog;
var
  AFileName, ADirectoryName: string;
begin
  Result := TdxSaveFileDialog.Create(nil);
  Result.Filter := CreateFilterString(AFilters);
  Result.Options  := Result.Options - [ofFileMustExist, ofNoDereferenceLinks, ofNoValidate] + [ofPathMustExist, ofOverwritePrompt];
...
function TdxExportHelper.GetFileStorage(ADialog: TSaveDialog): string;
begin
  Result := ADialog.FileName;
end;

function TdxExportHelper.GetFileName(ADialog: TSaveDialog): string;
begin
  Result := ADialog.FileName;
end;

procedure TdxExportHelper.SetFileName(ADialog: TSaveDialog; const AFileName: string);
begin
  ADialog.FileName := AFileName;
end;

procedure TdxExportHelper.SetDirectoryName(ADialog: TSaveDialog; const ADirectoryName: string);
begin
