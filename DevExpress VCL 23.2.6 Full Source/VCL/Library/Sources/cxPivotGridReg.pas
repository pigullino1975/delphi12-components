...
type
  ...
  TcxPivotGridFieldGroupIntervalAdvancedProperty = class(TIntegerProperty)
  public
    function GetAttributes: TPropertyAttributes; override;
    function GetValue: string; override;
    procedure GetValues(Proc: TGetStrProc); override;
...
function TcxPivotGridFieldGroupIntervalAdvancedProperty.GetAttributes: TPropertyAttributes;
begin
  Result := inherited GetAttributes - [paSubProperties, paReadOnly] + [paValueList];
end;

function TcxPivotGridFieldGroupIntervalAdvancedProperty.GetValue: string;
begin
  if TcxPivotGridFieldAccess(GetComponent(0)).GroupIntervalItem <> nil then
    Result := TcxPivotGridFieldAccess(GetComponent(0)).GroupIntervalItem.GetName
  else
    Result := 'Default'
end;

procedure TcxPivotGridFieldGroupIntervalAdvancedProperty.GetValues(Proc: TGetStrProc);
var
  I: Integer;
begin
  Proc('Default');
  for I := 0 to TcxPivotGridAdvancedDataGroupIntervalRepository.GetCount - 1 do
    Proc(TcxPivotGridAdvancedDataGroupIntervalRepository.GetItem(I).GetName);
end;

procedure TcxPivotGridFieldGroupIntervalAdvancedProperty.SetValue(const Value: string);
var
  AItem: TcxPivotGridAdvancedDataGroupIntervalItem;
begin
  AItem := TcxPivotGridAdvancedDataGroupIntervalRepository.GetItemByName(Value);
  if AItem <> nil then
    TcxPivotGridField(GetComponent(0)).GroupIntervalAdvanced := AItem.ID
  else
    TcxPivotGridField(GetComponent(0)).GroupIntervalAdvanced := -1;
end;
...
begin
  ...
  RegisterPropertyEditor(TypeInfo(Integer), TcxPivotGridField,
    'GroupIntervalAdvanced', TcxPivotGridFieldGroupIntervalAdvancedProperty);
