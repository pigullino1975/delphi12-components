...
type
  ...
  public type
    TAction = class sealed // for internal use
    public const
  ...
  public const
    DefaultExternalResourceTimeoutDefaultValue = 5000;
    DefaultTableCellSpacingDefaultValue = 15; 
    DefaultTableCellMarginDefaultValue  = 15; 
  public type
    TAction = class sealed // for internal use
    public const
      ...
      ExternalResourceTimeout = TdxRichEditOptionsAction.ExternalResourceTimeout;
  ...
  strict private
    ...
    FExternalResourceTimeout: Cardinal;
    ...
    procedure SetExternalResourceTimeout(const AValue: Cardinal);
  ...
  published
    ...
    property ExternalResourceTimeout: Cardinal read FExternalResourceTimeout write SetExternalResourceTimeout default 5000;
    property AutoDetectEncoding: Boolean read FAutoDetectEncoding write SetAutoDetectEncoding default True;
  ...
  published
    ...
    property ScaleEmbeddedImages: Boolean read FScaleEmbeddedImages write FScaleEmbeddedImages default False;
    property TabMarker: string read FTabMarker write SetTabMarker stored IsTabMarkerStored;
...
const
  dxThisUnitName = 'dxRichEdit.Options';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxRtfDocumentImporterOptions.Initialize', SysInit.HInstance);{$ENDIF}
  ...
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxRtfDocumentImporterOptions.Initialize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  inherited CopyFrom(AValue);
  if Safe.Cast(AValue, TdxXmlBasedDocumentImporterOptions, AOptions) then
...
procedure TdxHtmlDocumentImporterOptions.SetExternalResourceTimeout(const AValue: Cardinal);
begin
  if FExternalResourceTimeout = AValue then
    Exit;
  FExternalResourceTimeout := AValue;
  DoChanged(TAction.ExternalResourceTimeout);
end;
...
begin
  inherited CopyFrom(AValue);
  if Safe.Cast(AValue, TdxHtmlDocumentImporterOptions, AOptions) then
  begin
    DefaultAsyncImageLoading := AOptions.DefaultAsyncImageLoading;
    AsyncImageLoading := AOptions.AsyncImageLoading;
    ExternalResourceTimeout := AOptions.ExternalResourceTimeout;
    AutoDetectEncoding := AOptions.AutoDetectEncoding;
...
begin
  ...
  ExternalResourceTimeout := DefaultExternalResourceTimeoutDefaultValue;
  DefaultTableCellSpacing := DefaultTableCellSpacingDefaultValue;
  FDefaultTableCellMargin := DefaultTableCellMarginDefaultValue;
...
begin
  inherited CopyFrom(AValue);
  if Safe.Cast(AValue, TdxDocDocumentImporterOptions, AOptions) then
  begin
...
begin
  ...
  begin
    ...
    ScaleEmbeddedImages := AOptions.ScaleEmbeddedImages;
  end;
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxBookmarkOptions.Initialize', SysInit.HInstance);{$ENDIF}
  FDefaultColor := TdxAlphaColors.FromArgb(127, 127, 127);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxBookmarkOptions.Initialize', SysInit.HInstance);{$ENDIF}
end;
