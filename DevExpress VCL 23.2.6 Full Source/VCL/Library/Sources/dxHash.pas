...
type
  ...
  public
    ...
    class function Calculate(const S: TStream): TBytes; overload;
    ...
    class function ToString(const AHash: TBytes): string; reintroduce; static;
...
class function TdxHashAlgorithm.Calculate(const S: TStream): TBytes;
var
...
begin
  AAlgorithm := Create;
  try
    APrevPosition := S.Position;
    try
      S.Position := 0;
      SetLength(AData, MaxWord);
      repeat
        ACount := S.Read(AData, Length(AData));
        if ACount > 0 then
          AAlgorithm.Add(AData, 0, ACount);
      until ACount = 0;
      Result := AAlgorithm.GetHash;
    finally
      S.Position := APrevPosition;
    end;
  finally
    AAlgorithm.Free;
  end;
...
class function TdxHashAlgorithm.ToString(const AHash: TBytes): string;
var
  I: Integer;
  S: TStringBuilder;
begin
  S := TStringBuilder.Create(Length(AHash) * 2);
  try
    for I := 0 to Length(AHash) - 1 do
      S.Append(IntToHex(AHash[I], 2));
    Result := S.ToString;
  finally
    S.Free;
  end;
