...
const
  dxThisUnitName = 'dxPSfmReportProperties';
...
begin                 
  ...
  GetDateFormat(DefaultLCID, 0, @SystemTime, PChar(DateFormat), @Buffer, Length(Buffer));
  edCreationDate.Text := Buffer;
  
  TimeFormat := GetLocaleStr(DefaultLCID, LOCALE_STIMEFORMAT, dxFormatSettings.LongTimeFormat);
  GetTimeFormat(DefaultLCID, 0, @SystemTime, PChar(TimeFormat), @Buffer, Length(Buffer));
