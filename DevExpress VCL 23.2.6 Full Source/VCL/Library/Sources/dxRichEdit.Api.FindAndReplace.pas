...
type
  ...
  public
    ...
    property PieceTable: TdxPieceTable read GetPieceTable; // for internal use
  ...
  public
    ...
    class function CreateForwardProvider(APieceTable: TdxPieceTable; AParameters: TdxSearchParameters; AContext: TdxSearchContext): TdxTextSearchProvider; static;
    class function CreateBackwardProvider(APieceTable: TdxPieceTable; AParameters: TdxSearchParameters; AContext: TdxSearchContext): TdxTextSearchProvider; static;
  ...
  public
    ...
    destructor Destroy; override;
    class function CreateNativeSearchResultBackward(ADocument: TdxNativeSubDocument;
      const AFindText: string; ASearchOptions: TdxRichEditSearchOptions;
      const ARange: IdxRichEditDocumentRange): TdxNativeSearchResultBase; static;
    class function CreateNativeSearchResultForward(ADocument: TdxNativeSubDocument;
      const AFindText: string; ASearchOptions: TdxRichEditSearchOptions;
      const ARange: IdxRichEditDocumentRange): TdxNativeSearchResultBase; static;
  ...
  protected
    function Match(const ARange: IdxRichEditDocumentRange): IdxRichEditRegexSearchMatch;
    function Matches(const ARange: IdxRichEditDocumentRange): TArray<IdxRichEditDocumentRange>;
    ...
    class function CreatePosition(const APos: TdxDocumentModelPosition; AOffset: Integer): TdxDocumentLogPosition; static;
    function CreateMatch(const AOffset: TdxDocumentModelPosition; const AMatch: TMatch): IdxRichEditRegexSearchMatch;
    function CreateRegexSearch: TdxBufferedRegexSearchBase;
  ...
  protected
    function CreateRegexSearch(ADocument: TdxNativeSubDocument; const ARegex: TRegEx): TdxNativeRegexSearch;
    property MaxGuaranteedSearchResultLength: Integer read FMaxGuaranteedSearchResultLength;
  public
    constructor Create(ADocument: TdxNativeSubDocument;
...
uses
  RTLConsts, Contnrs, RegularExpressionsCore, Math, Character,
...
const
  dxThisUnitName = 'dxRichEdit.Api.FindAndReplace';
...
type
  ...
  strict private
    ...
    function GetSearchParameters: TdxSearchParameters; inline;
    function GetSearchContext: TdxSearchContext; inline;
...
class function TdxTextSearchProvider.CreateBackwardProvider(APieceTable: TdxPieceTable; AParameters: TdxSearchParameters; AContext: TdxSearchContext): TdxTextSearchProvider;
begin
  Result := TdxTextSearchBackwardProvider.Create(APieceTable, AParameters, AContext);
end;

class function TdxTextSearchProvider.CreateForwardProvider(APieceTable: TdxPieceTable; AParameters: TdxSearchParameters; AContext: TdxSearchContext): TdxTextSearchProvider;
begin
  Result := TdxTextSearchForwardProvider.Create(APieceTable, AParameters, AContext);
end;
...
begin
  ...
  begin
    PopulateBadCharacterShiftTable(Pattern.ToLower, Result);
    PopulateBadCharacterShiftTable(Pattern.ToUpper, Result);
  end;
end;
...
begin
  ...
  if (APrevChar <> #0) and APrevChar.IsLetterOrDigit then
    Exit(False);

  ANextChar := GetNextCharacter(ARunInfo.&End);
  if (ANextChar <> #0) and ANextChar.IsLetterOrDigit then
    Result := False
  else
    Result := True;
end;
...
class function TdxNativeSearchResultBase.CreateNativeSearchResultBackward(ADocument: TdxNativeSubDocument;
      const AFindText: string; ASearchOptions: TdxRichEditSearchOptions;
      const ARange: IdxRichEditDocumentRange): TdxNativeSearchResultBase;
begin
  Result := TdxNativeSearchResultBackward.Create(ADocument, AFindText, ASearchOptions, ARange);
end;

class function TdxNativeSearchResultBase.CreateNativeSearchResultForward(ADocument: TdxNativeSubDocument;
      const AFindText: string; ASearchOptions: TdxRichEditSearchOptions;
      const ARange: IdxRichEditDocumentRange): TdxNativeSearchResultBase;
begin
  Result := TdxNativeSearchResultForward.Create(ADocument, AFindText, ASearchOptions, ARange);
end;
...
begin
  if ARange.Length = 0 then
    Exit(nil);
  ARegexSearch := CreateRegexSearch as TdxBufferedRegexSearchForward;
  try
...
function TdxNativeRegexSearch.CreateRegexSearch: TdxBufferedRegexSearchBase;
begin
