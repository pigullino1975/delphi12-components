...
uses
  ...
  dxNavBarSkinBasedViews, dxSkinsCore, dxNavBarCustomPainters, dxCoreGraphics, dxNavBarOverflowPanel;
...
const
  dxThisUnitName = 'dxSkinsdxNavBarPainter';
...
begin
  ...
  begin
    ...
    Canvas.FillRect(ViewInfo.ClientRect);
  end;
end;
...
function TdxNavBarSkinNavPanePainter.GetGroupColorPalette(AGroupViewInfo: TdxNavBarGroupViewInfo): IdxColorPalette;
begin
...
begin
  ...
  begin
    Result := dxSkinGetElementSize(AElement, ScaleFactor);
    Result := cxSizeAddPadding(Result, ScaleFactor.Apply(AElement.ContentOffset.Rect));
  end
  else
    Result := inherited GetGroupCaptionSignSize;
end;
...
begin
  if not DrawSkinElement(FSkinBasedPainterHelper.NavBarBackground, Canvas, ViewInfo.ClientRect, ScaleFactor) then
    inherited;
end;
