...
uses
  ...
  cxEditConsts, cxFilterControlUtils, cxDateUtils, dxWheelPicker, dxNumericWheelPicker, cxAccessibility;
...
type
  ...
  protected
    ...
    property LowerText: string read FLowerText;
  public
    constructor Create(AWheelPickerViewInfo: TdxCustomWheelPickerViewInfo); override;
    destructor Destroy; override;
  end;

  { TdxDateTimeWheelPickerWheelAccessibilityHelper }

  TdxDateTimeWheelPickerWheelAccessibilityHelper = class(TdxNumericWheelPickerWheelAccessibilityHelper) // for internal use
  protected
  ...
  protected
    ...
    function NeedProcessKey(AKey: Word): Boolean; override;
    ...
    procedure SynchronizeDateTime(var ADateTime: TcxDateTime; var ATimeSuffix: Integer);
  end;
...
const
  dxThisUnitName = 'dxDateTimeWheelPicker';
...
function TdxDateTimeWheelPickerWheelAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
var
  AItemViewInfo: TdxDateTimeWheelPickerItemViewInfo;
begin
  Result := inherited;
  if AChildID > 0 then
  begin
    AItemViewInfo := TdxDateTimeWheelPickerItemViewInfo(TdxDateTimeWheelPickerWheelViewInfo(ViewInfo).Items[AChildID - 1]);
    if AItemViewInfo.HasLowerText then
      Result := Result + AItemViewInfo.LowerText;
  end;
end;

{ TdxDateTimeWheelPickerWheelViewInfo }

function TdxDateTimeWheelPickerWheelViewInfo.GetAccessibilityHelperClass: TdxCustomWheelPickerWheelAccessibilityHelperClass;
begin
  Result := TdxDateTimeWheelPickerWheelAccessibilityHelper;
end;
...
procedure TdxDateTimeWheelPickerWheelViewInfo.SynchronizeDateTime(var ADateTime: TcxDateTime; var ATimeSuffix: Integer);
begin
  case FKind of
    dtikSec:
...
function TdxDateTimeWheelPickerWheelViewInfo.NeedProcessKey(AKey: Word): Boolean;
begin
...
begin
  for I := 0 to FWheels.Count - 1 do
    (FWheels[I] as TdxDateTimeWheelPickerWheelViewInfo).SynchronizeDateTime(FDateTime, ATimeSuffix);
...
begin
  if AssignedValues.MaxDate and (MaxDate <> NullDate) and (not AssignedValues.MinDate or (MinDate < MaxDate)) then
    Result := MaxDate
  else
    Result := MaxDateTime;
end;
...
begin
  if AssignedValues.MinDate and (MinDate <> NullDate) and (not AssignedValues.MaxDate or (MinDate < MaxDate))  then
    Result := MinDate
  else
    Result := MinDateTime;
end;
