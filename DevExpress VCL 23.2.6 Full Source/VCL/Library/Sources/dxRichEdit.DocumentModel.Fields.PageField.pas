...
unit dxRichEdit.DocumentModel.Fields.PageField; // for internal use
...
uses
  ...
  dxRichEdit.DocumentModel.NumberingFormatting, dxCore;

const
  dxThisUnitName = 'dxRichEdit.DocumentModel.Fields.PageField';

{ TdxPageFieldResultFormatting }

class constructor TdxPageFieldResultFormatting.Initialize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxPageFieldResultFormatting.Initialize', SysInit.HInstance);{$ENDIF}
  FEmptyGenericFormatting := TArray<string>.Create();
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxPageFieldResultFormatting.Initialize', SysInit.HInstance);{$ENDIF}
end;
