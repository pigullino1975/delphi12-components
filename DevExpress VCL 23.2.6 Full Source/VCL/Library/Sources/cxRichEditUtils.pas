...
type
  ...
  TcxRichEditTableCellParams = record
    ...
    crBrdrLeft: TColor;     // Left border color (\clbrdrl\brdrcf)
    crBrdrTop: TColor;      // Top border color (\clbrdrt\brdrcf)
    crBrdrRight: TColor;    // Right border color (\clbrdrr\brdrcf)
    crBrdrBottom: TColor;   // Bottom border color (\clbrdrb\brdrcf)
    crBackPat: TColor;      // Background color (\clcbpat)
    crForePat: TColor;      // Foreground color (\clcfpat)
  end;
...
const
  dxThisUnitName = 'cxRichEditUtils';
...
function TcxTextAttributes2.GetColor(Index: Integer): TColor;
  ...
  procedure GetColorParams(out ATextColor, ABackgroundColor: TColor);
  var
    AFormat: CHARFORMAT2;
  begin
    GetAttributes(AFormat);
    ATextColor := AFormat.crTextColor;
    ABackgroundColor := AFormat.crBackColor;
  end;
