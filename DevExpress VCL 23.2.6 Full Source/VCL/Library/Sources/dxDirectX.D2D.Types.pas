...
uses
  ActiveX, Windows, SysUtils, WinCodec, DxgiFormat, D2D1, dxCore;

type
  PIUnknown = ^IUnknown;
  TdxLargeInteger = Int64;
...
type
  ...
  TDXGIFrameStatistics = record
    ...
    SyncQPCTime: TdxLargeInteger;
    SyncGPUTime: TdxLargeInteger;
  end;
...
type
  ...
  ['{2411e7e1-12ac-4ccf-bd14-9798e8534dc0}']
    ...
    function CheckInterfaceSupport(const InterfaceName: TGUID; out pUMDVersion: TdxLargeInteger): HResult; stdcall;
  end;
...
type
  ...
  ['{a2296057-ea42-4099-983b-539fb6505426}']
    ...
    function CopyFromBitmap(const destPoint: PD2D1Point2U; bitmap: ID2D1Bitmap; const srcRect: PD2D1RectU): HResult; stdcall;
    function CopyFromRenderTarget(const destPoint: PD2D1Point2U; renderTarget: ID2D1RenderTarget; const srcRect: PD2D1RectU): HResult; stdcall;
    function CopyFromMemory(const dstRect: PD2D1RectU; srcData: Pointer; pitch: UINT32): HResult; stdcall;
  end;
...
function D2D1CreateFactoryNew(factoryType: D2D1_FACTORY_TYPE;
  const riid: TGUID; pFactoryOptions: PD2D1FactoryOptions; out ppIFactory): HRESULT;
function D3D11CreateDevice(pAdapter: IDXGIAdapter; DriverType: TD3DDriveType; Software: HMODULE; Flags: UINT;
  pFeatureLevels: PD3DFeatureLevel; FeatureLevels: UINT; SDKVersion: UINT; out ppDevice: ID3D11Device;
  pFeatureLevel: PD3DFeatureLevel; out ppImmediateContext: ID3D11DeviceContext): HRESULT;
...
type
  ...
  TDCompositionFrameStatistics = record
    lastFrameTime: TdxLargeInteger;
    currentCompositionRate: TDXGIRational;
    currentTime: TdxLargeInteger;
    timeFrequency: TdxLargeInteger;
    nextEstimatedFrameTime: TdxLargeInteger;
  end;
...
function D2D1Factory1: ID2D1Factory1;
function DWriteFactory: IDWriteFactory;

function CheckNeedRecreateContext(const AErrorCode: HRESULT): Boolean; overload;
function CheckNeedSwitchToGdiRenderMode(const AErrorCode: HRESULT = S_OK): Boolean; overload;
function CheckNeedSwitchToGdiRenderMode(const AException: Exception): Boolean; overload;
function IsDirectD2Available: Boolean;
implementation
...
const
  dxThisUnitName = 'dxDirectX.D2D.Types';
...
var
  FAreDirectXLibrariesLoaded: Boolean = False;
  FD2D1CreateFactory: TD2D1CreateFactoryFunc = nil;
  FD2D1Library: THandle = 0;
  FD3D11CreateDevice: TD3D11CreateDeviceFunc = nil;
  FD3D11Library: THandle = 0;
  FIsDirect2DAvailable: TdxDefaultBoolean = bDefault;
  FSharedD2D1Factory1: ID2D1Factory1 = nil;
  FSharedDirectWriteFactory: IDWriteFactory = nil;

procedure CheckAreDirectXLibrariesLoaded;
begin
  if not FAreDirectXLibrariesLoaded then
  begin
    FAreDirectXLibrariesLoaded := True;
...
procedure ReleaseDirectXLibraries;
begin
  FIsDirect2DAvailable := bFalse;
  FSharedDirectWriteFactory := nil;
  FSharedD2D1Factory1 := nil;
...
function D2D1CreateFactoryNew(factoryType: D2D1_FACTORY_TYPE;
  const riid: TGUID; pFactoryOptions: PD2D1FactoryOptions; out ppIFactory): HRESULT;
begin
  CheckAreDirectXLibrariesLoaded;
  if Assigned(FD2D1CreateFactory) then
    Result := FD2D1CreateFactory(factoryType, riid, pFactoryOptions, ppIFactory)
  else
    Result := E_NOTIMPL;
end;
...
begin
  ...
  if Assigned(FD3D11CreateDevice) then
    Result := FD3D11CreateDevice(pAdapter, DriverType, Software, Flags, pFeatureLevels,
      FeatureLevels, SDKVersion, ppDevice, pFeatureLevel, ppImmediateContext)
  else
    Result := E_NOTIMPL;
end;

function CheckNeedRecreateContext(const AErrorCode: HRESULT): Boolean;
begin
  Result := CheckNeedSwitchToGdiRenderMode(AErrorCode) or
...
function CheckNeedSwitchToGdiRenderMode(const AErrorCode: HRESULT = S_OK): Boolean; overload;
begin
  Result := (AErrorCode = DXGI_ERROR_UNSUPPORTED) or dxIsGdiScaledMode;
  if Result then
    FIsDirect2DAvailable := bFalse;
end;

function CheckNeedSwitchToGdiRenderMode(const AException: Exception): Boolean;
begin
  Result := (AException is EAccessViolation) or (AException is EOutOfMemory) or dxIsGdiScaledMode;
  if Result then
    FIsDirect2DAvailable := bFalse;
end;
...
function IsDirectD2Available: Boolean;
begin
  if FIsDirect2DAvailable = bDefault then
    FIsDirect2DAvailable := dxBooleanToDefaultBoolean(not dxIsGdiScaledMode and (D2D1Factory1 <> nil) and (DWriteFactory <> nil));
  Result := FIsDirect2DAvailable = bTrue;
end;
...
var
  AFactory: IUnknown;
begin
  if FSharedD2D1Factory1 = nil then
  begin
    if Failed(D2D1CreateFactoryNew(D2D1_FACTORY_TYPE_SINGLE_THREADED, ID2D1Factory1, nil, AFactory)) then
      AFactory := nil;
    if InterlockedCompareExchangePointer(Pointer(FSharedD2D1Factory1), Pointer(AFactory), nil) = nil then
    begin
      if AFactory <> nil then
        AFactory._AddRef;
    end;
  end;
  Result := FSharedD2D1Factory1;
end;
...
var
  AFactory: IUnknown;
begin
  if FSharedDirectWriteFactory = nil then
  begin
    if Failed(DWriteCreateFactory(DWRITE_FACTORY_TYPE_SHARED, IDWriteFactory, AFactory)) then
      AFactory := nil;
    if InterlockedCompareExchangePointer(Pointer(FSharedDirectWriteFactory), Pointer(AFactory), nil) = nil then
    begin
      if AFactory <> nil then
        AFactory._AddRef;
    end;
  end;
  Result := FSharedDirectWriteFactory;
end;
