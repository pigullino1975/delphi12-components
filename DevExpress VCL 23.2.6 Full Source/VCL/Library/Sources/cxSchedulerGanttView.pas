...
uses
  ...
  dxCore, dxCoreClasses, cxScrollBar, cxLookAndFeelPainters, cxSchedulerCustomControls, cxSchedulerStorage,
...
type
  ...
  protected
    function CreateComparer: IComparer<TcxSchedulerEventPlace>; override;
  public
  ...
  private
    ...
    FVisibleEventEnumerationList: TdxFastObjectList;
  ...
  protected
    ...
    procedure CheckVisibleEventCounter(AEvent: TcxSchedulerControlEvent);
    ...
    function HasEventForResource(AResourceIndex: Integer): Boolean; override;
...
const
  dxThisUnitName = 'cxSchedulerGanttView';
...
type
  ...
  { TcxSchedulerGanttViewEventPlaceComparer }

  TcxSchedulerGanttViewEventPlaceComparer = class(TInterfacedObject, IComparer<TcxSchedulerEventPlace>)
  protected
...
function TcxSchedulerGanttViewEventPlaceComparer.Compare(const Left, Right: TcxSchedulerEventPlace): Integer;
begin
  Result := cxGanttViewCompareEventPlaces(Left, Right);
end;
...
function TcxSchedulerGanttViewEventLayoutBuilder.CreateComparer: IComparer<TcxSchedulerEventPlace>;
begin
  Result := TcxSchedulerGanttViewEventPlaceComparer.Create;
end;
...
begin
  ...
  begin
    AItem := ViewInfo.TimeLineCells[I];
...
begin
  ...
  FVisibleEventEnumerationList := TdxFastObjectList.Create;
...
procedure TcxSchedulerGanttViewViewInfo.CheckVisibleEventCounter(AEvent: TcxSchedulerControlEvent);
...
begin
  if (AEvent.Start < FLastVisibleTime) and (AEvent.Finish > View.VisibleStart) then
...
begin
  ...
  CheckVisibleEventCounter(AEvent);
end;

function TcxSchedulerGanttViewViewInfo.CreateLayoutBuilder: TcxSchedulerEventLayoutBuilder;
begin
  Result := TcxSchedulerGanttViewEventLayoutBuilder.Create(ViewStyle);
end;
...
procedure TcxSchedulerGanttViewViewInfo.CustomProcessPreviouslyCalculatedEvent(AEventCell: TcxSchedulerEventCellViewInfo);
begin
  CheckVisibleEventCounter(AEventCell.Event);
  inherited CustomProcessPreviouslyCalculatedEvent(AEventCell);
end;
...
function TcxSchedulerGanttViewViewInfo.HasEventForResource(AResourceIndex: Integer): Boolean;
var
  I: Integer;
  AResourceID: Variant;
begin
  ...
  I := 0;
  while I < ResourceCount do
  begin
    if VarEquals(VisibleEventEnumerationList[I].ResourceID, AResourceID) then
      Exit(VisibleEventEnumerationList[I].VisibleEventCount > 0);
    Inc(I);
  end;
  Result := inherited HasEventForResource(AResourceIndex);
end;
...
begin
  ...
  begin
    for I := 0 to Links.Count - 1 do
       TcxSchedulerGanttViewLinkViewInfo(Links[I]).AllowRightToLeftConversion := False;
    Exit;
  end;
