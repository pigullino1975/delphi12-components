...
uses
  ...
  cxDataStorage, cxDataUtils, cxEdit, cxDropDownEdit, cxTextEdit, cxFilterControlUtils, dxCoreGraphics;
...
type
  ...
  protected
    function GetFilteredItemImageIndex(Index: Integer): Integer;
    function GetListBoxClass: TcxCustomEditListBoxClass; override;
    function GetItem(Index: Integer): string; override;
    function GetItemCount: Integer; override;
    function GetScrollWidth: Integer; override;
    function UsePreferredWidth: Boolean; override;
  ...
  public
    ...
    function GetImageColorPalette: IdxColorPalette;
  end;
  ...
  protected
    ...
    procedure DeleteLinks; override;
...
uses
  ...
  cxLookAndFeelPainters, cxDWMApi, cxLookAndFeels, dxTypeHelpers;

const
  dxThisUnitName = 'cxImageComboBox';
...
function TcxImageComboBoxListBox.GetDefaultItemHeight: Integer;
var
  AImageSize: TSize;
begin
  AImageSize := dxGetImageSize(GetImages, ScaleFactor);
  if not cxSizeIsEmpty(AImageSize) then
    Result := AImageSize.cy + ScaleFactor.Apply(2)
  else
    Result := 0;
  Result := Max(TdxTextMeasurer.TextLineHeight(Canvas.Handle) + ScaleFactor.Apply(2), Result);
  Inc(Result, FItemPadding.Height);
  dxAdjustToTouchableSize(Result, ScaleFactor);
end;

function TcxImageComboBoxListBox.GetItemHeight(AIndex: Integer): Integer;
var
  AData: string;
  AImages: TCustomImageList;
  AImageIndex: Integer;
...
begin
  ...
  Dec(W, FItemPadding.Width);
  AImageSize := dxGetImageSize(GetImages, ScaleFactor);
  if not cxSizeIsEmpty(AImageSize) then
  begin
    AImages := GetImages;
    AImageIndex := Edit.LookupData.GetFilteredItemImageIndex(AIndex);
  ...
  begin
    ...
    H := Max(TdxTextMeasurer.TextLineHeight(Canvas.Handle) + ScaleFactor.Apply(2), H);

  Inc(H, FItemPadding.Height);
  dxAdjustToTouchableSize(H, ScaleFactor);
  if (AIndex >= 0) and Edit.IsOnMeasureItemEventAssigned then
    Edit.DoOnMeasureItem(AIndex, Canvas, H);
  Result := H;
end;
...
begin
  ...
  begin
    Result := Canvas.TextWidth(Edit.LookupData.GetFilteredItem(AIndex));
    AImageSize := dxGetImageSize(GetImages, ScaleFactor);

    if cxSizeIsEmpty(AImageSize) or (Edit.LookupData.GetFilteredItemImageIndex(AIndex) < 0) then
      Inc(Result, FElementIndent)
    else
      Inc(Result, FElementIndent * 2 + AImageSize.cx); 
    Inc(Result, FItemPadding.Width);
  end;
end;

procedure TcxImageComboBoxListBox.CalculatePadding;
begin
  inherited CalculatePadding;
  FElementIndent := Painter.GetEditorGlyphIndent(False, False, ScaleFactor, TdxPaintPartID.ImageComboBox);
  if SupportsListBoxSkinPadding then
    FItemPadding.Left := Painter.GetEditorGlyphIndent(True, False, ScaleFactor, TdxPaintPartID.ImageComboBox)
  else
    if UseCustomPadding then
      Inc(FItemPadding.Left, Painter.GetEditorGlyphIndent(True, False, ScaleFactor, TdxPaintPartID.ImageComboBox))
    else
      FItemPadding := TRect.Create(Painter.GetEditorGlyphIndent(True, False, ScaleFactor, TdxPaintPartID.ImageComboBox), 0, 0, 0);
end;

procedure TcxImageComboBoxListBox.DrawItemContent(AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
var
  AText: string;
  AImages: TCustomImageList;
  AImageIndex: Integer;
begin
  if (AIndex > -1) and (AIndex < Edit.LookupData.GetFilteredItemCount) then
  begin
    ...
    if IsImageAssigned(AImages, AImageIndex) then
    begin
      TdxImageDrawer.DrawImage(Canvas, GetImageRect(AImages, ARect), nil, AImages,
        AImageIndex, ifmNormal, EnabledImageDrawModeMap[Enabled], False,
        TcxImageComboBoxViewInfo(Edit.ViewInfo).GetImageColorPalette, ScaleFactor);
    end;
    if not IsRectEmpty(ARect) then
      if IsHighlightSearchText then
        DrawItemText(AText, ARect, AState)
      else
        cxDrawText(Canvas, AText, ARect, GetDrawTextFlags);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Result.Right := Result.Left + AImageSize.cx;
        R.Left := Result.Right + FElementIndent;
      end
      else
      begin
        Result.Left := Result.Right - AImageSize.cx;
        R.Right := Result.Left - FElementIndent;
      end;
...
function TcxImageComboBoxListBox.GetRealStyle: TListBoxStyle;
begin
  if Edit.IsOnMeasureItemEventAssigned or Properties.MultiLineText then
...
function TcxImageComboBoxLookupData.GetFilteredItemImageIndex(Index: Integer): Integer;
var
  AItemIndex: Integer;
begin
  AItemIndex := GetLookupItemIndexFromFilteredItemIndex(Index);
  if (AItemIndex > -1) and (AItemIndex < Items.Count) then
    Result := Items[AItemIndex].ImageIndex
  else
    Result := -1;
end;
...
function TcxImageComboBoxLookupData.GetScrollWidth: Integer;
var
  AItemWidth: Integer;
  I: Integer;
begin
  if ActiveProperties.MultiLineText then
    Result := 0
  else
  begin
    Result := 0;
...
function TcxImageComboBoxLookupData.UsePreferredWidth: Boolean;
begin
  Result := ActiveProperties.MultiLineText;
end;
...
begin
  ...
  begin
    ...
    try
      ...
      TdxImageDrawer.DrawImage(ACanvas, ImageRect, nil, Images, ImageIndex, ifmNormal,
        EnabledImageDrawModeMap[Enabled], False, GetImageColorPalette, ScaleFactor);
    finally
...
function TcxImageComboBoxViewInfo.GetImageColorPalette: IdxColorPalette;
const
  StateMap: array[Boolean] of TcxEditButtonState = (ebsDisabled, ebsNormal);
begin
  Result := GetButtonColorPalette(StateMap[Enabled]);
end;
...
procedure TcxCustomImageComboBoxProperties.DeleteLinks;
begin
  inherited DeleteLinks;
  if Assigned(Images) and Assigned(FImagesChangeLink) then
    Images.UnRegisterChanges(FImagesChangeLink);
  if Assigned(LargeImages) and Assigned(FLargeImagesChangeLink) then
    LargeImages.UnRegisterChanges(FLargeImagesChangeLink);
end;
...
begin
  ...
  AChanged := (Sender = FImages) or (Sender = FLargeImages);
...
function TcxCustomImageComboBoxProperties.GetItemCount: Integer;
begin
  Result := Items.Count;
end;
...
function TcxImageComboBox.SupportsItemIndex: Boolean;
begin
  Result := True;
end;
...
function TcxImageComboBoxProperties.FindValueByItemIndex(AItemIndex: Integer;
  out AValue: Variant): Boolean;
begin
   Result := InRange(AItemIndex, 0, Items.Count - 1);
  if Result then
    AValue := Items[AItemIndex].Value;
end;

function TcxImageComboBoxProperties.SupportsItemIndex: Boolean;
begin
  Result := True;
end;
