...
type
  ...
  TdxRibbonRadialMenuItemAccessibilityHelper = class;
  ...
  protected
    ...
    function NeedAssignBarManagerOnInitialize: Boolean; override;
  ...
  private
    ...
    procedure ClearItemIAccessibilityHelpers;
    function GetItemIAccessibilityHelper(AIndex: Integer): IdxBarAccessibilityHelper;
    procedure DestroyItemIAccessibilityHelpers;
  ...
  protected
    ...
    property ItemIAccessibilityHelpers: TInterfaceList read FItemIAccessibilityHelpers;
    property ItemIAccessibilityHelper[AIndex: Integer]: IdxBarAccessibilityHelper read GetItemIAccessibilityHelper;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
  ...
  TdxRibbonRadialMenuAccessibilityHelper = class(TCustomdxBarControlAccessibilityHelper) // for internal use
  private
  ...
  protected
    ...
    function GetItemAccessibilityHelper(ASectorIndex: Integer): TdxRibbonRadialMenuItemAccessibilityHelper;
  ...
  { TdxRibbonRadialMenuItemAccessibilityHelper }

  TdxRibbonRadialMenuItemAccessibilityHelper = class(TdxBarItemLinkAccessibilityHelper) // for internal use
  strict private
    function GetBarControl: TdxRibbonRadialMenuControl;
  ...
  protected
    ...
    property BarControl: TdxRibbonRadialMenuControl read GetBarControl;
    property SectorIndex: Integer read GetSectorIndex;
  public
...
const
  dxThisUnitName = 'dxRibbonRadialMenu';
...
begin
  if (ItemLinks.BarControl <> nil) or (BarManager = nil) then 
    Exit;
...
function TdxRibbonCustomRadialMenu.NeedAssignBarManagerOnInitialize: Boolean;
begin
  Result := True;
end;
...
begin
  ...
  ACoreItem := BarDesignController.AddInternalItem(FInternalItemLinksOwner.ItemLinks, TdxBarButton, nil, FInternalItems).Item as TdxBarButton;
...
begin
  ...
  ActivateRootLevel;
  DestroyItemIAccessibilityHelpers;
...
begin
  if NotHandleMouseMove or IsAnimationInProcess then
    Exit;
  SetHotItem(APoint);
end;
...
function TdxRibbonRadialMenuControl.GetItemRect(ASectorIndex: Integer): TRect;
const
...
procedure TdxRibbonRadialMenuControl.DrawRadialItemContent(ACanvas: TcxCanvas; AGPGraphics: TdxGPGraphics; AItem: TdxBarItem; ARect: TRect);
  ...
  begin
    ...
    ATextHeight := TdxTextMeasurer.TextLineHeight(ACanvas.Font);
    if FAnimationStep > AnimationTurn1 then
      AAnimationImageOffset := ATextHeight - MulDiv(ATextHeight, FAnimationStep - AnimationTurn1, AnimationTurn3 - AnimationTurn1)
    else
      AAnimationImageOffset := ATextHeight;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        TdxImageDrawer.DrawImage(ATempBitmap.cxCanvas, ATempBitmap.ClientRect,
...
begin
  ...
  try
    ...
    begin
      ...
      TdxImageDrawer.DrawImage(ACanvas, AGlyphRect, GetCurrentCoreImage, ifmNormal, FColorPalette, ScaleFactor);
    end;
  finally
    AGPGraphics.Free;
  end;
...
function TdxRibbonRadialMenuControl.HandleNavigationKey(var AKey: Word): Boolean;
  ...
  var
    ...
    AItem: TdxBarItem;
    AAccessibilityItem: TdxRibbonRadialMenuItemAccessibilityHelper;
  begin
    ...
    try
      ...
      begin
        AItem := GetVisibleItem(ASectorIndex);
        if (AItem <> nil) and (AItem.Enabled or (GetOwnedLinks(AItem) <> nil)) then
      ...
      begin
        SetHotItem(htCore);
        AAccessibilityItem := TdxRibbonRadialMenuItemAccessibilityHelper(ItemIAccessibilityHelper[0].GetBarHelper);
      end
      else
      begin
        SetHotItem(Integer(ANavigationList[ANavigationIndex]));
        AAccessibilityItem := TdxRibbonRadialMenuAccessibilityHelper(
          IAccessibilityHelper.GetBarHelper).GetItemAccessibilityHelper(Integer(ANavigationList[ANavigationIndex]));
      end;
      NotifyWinEvent(EVENT_OBJECT_FOCUS, AAccessibilityItem.OwnerObjectWindow, AAccessibilityItem.LocalId, CHILDID_SELF);
    finally
...
begin
  ClearItemIAccessibilityHelpers;
  FInternalVisibleLinks.Clear;
  FItemIAccessibilityHelpers.Add(TdxRibbonRadialMenuItemAccessibilityHelper.Create(CoreItemLink));
  ...
  begin
    ...
    FItemIAccessibilityHelpers.Add(TdxRibbonRadialMenuItemAccessibilityHelper.Create(GetCurrentItemLinks.VisibleItems[I]));
    if (FInternalVisibleLinks.Count > 8) or (FInternalVisibleLinks.Count = 8) and (I < GetCurrentItemLinks.VisibleItemCount - 1) then
    begin
      AMoreButtonsLink := BarDesignController.AddInternalItem(FInternalItemLinksOwner.ItemLinks, TdxBarSubItem, Owner, FInternalItems);
      AMoreButtonsItem := AMoreButtonsLink.Item as TdxBarSubItem;
      ...
      FItemIAccessibilityHelpers.Add(TdxRibbonRadialMenuItemAccessibilityHelper.Create(AMoreButtonsLink));
      Break;
    end;
  end;
end;
...
procedure TdxRibbonRadialMenuControl.ClearItemIAccessibilityHelpers;
var
  I: Integer;
  AIAccessibilityHelper: IcxAccessibilityHelper;
begin
  if FItemIAccessibilityHelpers <> nil then
  begin
    for I := 0 to FItemIAccessibilityHelpers.Count - 1 do
    begin
      Supports(FItemIAccessibilityHelpers[I], IdxBarAccessibilityHelper, AIAccessibilityHelper);
      AIAccessibilityHelper.OwnerObjectDestroyed;
    end;
    FItemIAccessibilityHelpers.Clear;
  end;
end;

function TdxRibbonRadialMenuControl.GetItemIAccessibilityHelper(AIndex: Integer): IdxBarAccessibilityHelper;
begin
  Supports(FItemIAccessibilityHelpers[AIndex], IdxBarAccessibilityHelper, Result);
end;

procedure TdxRibbonRadialMenuControl.DestroyItemIAccessibilityHelpers;
begin
  ClearItemIAccessibilityHelpers;
...
begin
  Result := BarControl.ItemIAccessibilityHelper[AIndex].GetHelper;
end;

function TdxRibbonRadialMenuAccessibilityHelper.GetChildCount: Integer;
begin
  Result := BarControl.ItemIAccessibilityHelpers.Count;
end;

function TdxRibbonRadialMenuAccessibilityHelper.GetChildIndex(AChild: TcxAccessibilityHelper): Integer;
begin
  Result := BarControl.FItemIAccessibilityHelpers.IndexOf(Self);
end;

function TdxRibbonRadialMenuAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
...
function TdxRibbonRadialMenuAccessibilityHelper.GetItemAccessibilityHelper(
  ASectorIndex: Integer): TdxRibbonRadialMenuItemAccessibilityHelper;

  function GetHelper(AIndex: Integer): TdxRibbonRadialMenuItemAccessibilityHelper;
  begin
    Result := TdxRibbonRadialMenuItemAccessibilityHelper(BarControl.ItemIAccessibilityHelper[AIndex].GetBarHelper);
  end;
...
begin
  Result := nil;
  for I := 0 to BarControl.ItemIAccessibilityHelpers.Count - 1 do
    if BarControl.GetVisibleItem(ASectorIndex) = GetHelper(I).ItemLink.Item then
    begin
      Result := GetHelper(I);
      Break;
    end;
...
function TdxRibbonRadialMenuItemAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  if SectorIndex = -1 then
    Result := BarControl.FCoreRingRect
  else
    Result := BarControl.GetItemRect(SectorIndex);
  Result := cxRectSetOrigin(Result, BarControl.ClientToScreen(Result.TopLeft))
end;

procedure TdxRibbonRadialMenuItemAccessibilityHelper.DoDefaultAction(AChildID: TcxAccessibleSimpleChildElementID);
begin
  if GetOwnedLinks(Item) <> nil then
  begin
    if ItemLink = BarControl.CoreItemLink then
      BarControl.InternalCoreClick
    else
      BarControl.InternalClick(Item);
  end
  else
    TdxRibbonRadialMenuAccessibilityHelper(Parent).DoClickItem(Self);
end;

function TdxRibbonRadialMenuItemAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  if GetOwnedLinks(Item) <> nil then
...
function TdxRibbonRadialMenuItemAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := Parent.States[cxAccessibleObjectSelfID];
  if (Result and cxSTATE_SYSTEM_INVISIBLE = 0) and not ItemLink.Visible then
...
function TdxRibbonRadialMenuItemAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := inherited;
  if GetOwnedLinks(Item) <> nil then
    Result := Result + [aopSelect];
end;

function TdxRibbonRadialMenuItemAccessibilityHelper.GetOwnerObjectWindow: HWND;
begin
  Result := Parent.OwnerObjectWindow;
end;

function TdxRibbonRadialMenuItemAccessibilityHelper.GetBarControl: TdxRibbonRadialMenuControl;
begin
  Result := TdxRibbonRadialMenuAccessibilityHelper(Parent).BarControl;
end;

function TdxRibbonRadialMenuItemAccessibilityHelper.GetSectorIndex: Integer;
begin
  if ItemLink = BarControl.CoreItemLink then
    Result := -1
  else
    Result := TdxRibbonRadialMenuAccessibilityHelper(Parent).GetChildSectorIndex(Self);
end;

function TdxRibbonRadialMenuItemAccessibilityHelper.IsSelected: Boolean;
begin
  Result := (ItemLink = BarControl.CoreItemLink) and (BarControl.FHitTest.SectorIndex = -1) or
    (BarControl.GetVisibleItem(BarControl.FHitTest.SectorIndex) = ItemLink.Item);
end;
