...
const
  dxThisUnitName = 'dxRichEdit.Dialogs.NumberingFormController';
...
var
  ...
  I: Integer;
begin
  APrevNumbListIndex := NumberingListIndexListIndexNotSet;
  for I := 0 to Paragraphs.Count - 1 do
  begin
    AParagraph := Paragraphs[I];
    if not AParagraph.IsInList then
      Continue;
    if APrevNumbListIndex = NumberingListIndexListIndexNotSet then
    begin
...
procedure TdxNumberingListFormController.ModifyNumberingListLevel(ATargetList: TdxNumberingList; ALevelIndex: Integer);
var
  ...
  ASourceListLevel, ATargetListLevel: TdxAbstractListLevel;
begin
  ASourceListLevel := NewAbstractList.Levels[ALevelIndex];
  ATargetListLevel := ATargetList.Levels[ALevelIndex];
  ACharacterFormattingInfo := ASourceListLevel.CharacterProperties.Info.Info;
  ACharacterFormattingOptions := ASourceListLevel.CharacterProperties.Info.Options;
  ACharacterMergedProperties := TdxMergedCharacterProperties.Create(ACharacterFormattingInfo, ACharacterFormattingOptions);
  try
    ATargetListLevel.CharacterProperties.CopyFrom(ACharacterMergedProperties);
  finally
    ACharacterMergedProperties.Free;
  end;
  AParagraphFormattingInfo := ASourceListLevel.ParagraphProperties.Info.Info;
  AParagraphFormattingOptions := ASourceListLevel.ParagraphProperties.Info.Options;
  AParagraphMergedProperties := TdxMergedParagraphProperties.Create(AParagraphFormattingInfo, AParagraphFormattingOptions);
  try
    ATargetListLevel.ParagraphProperties.CopyFrom(AParagraphMergedProperties);
  finally
    AParagraphMergedProperties.Free;
  end;
  ATargetListLevel.ListLevelProperties.CopyFrom(ASourceListLevel.ListLevelProperties.Info);
end;
