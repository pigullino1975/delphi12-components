...
uses
  ...
  cxFilterControlUtils, cxGraphics, cxTextEdit, cxVariants, cxLookAndFeelPainters, cxAccessibility;
...
type
  ...
  private
    ...
    function GetActualProgressChunkRect: TRect;
  ...
  public
    procedure Calculate(ACanvas: TcxCanvas; const ABounds: TRect; const P: TPoint;
      Button: TcxMouseButton; Shift: TShiftState; ACustomViewInfo: TcxCustomEditViewInfo; AIsMouseEvent: Boolean); override;
  ...
  { TdxProgressBarRangeValueProvider }

  TdxProgressBarRangeValueProvider = class(TdxRangeValueProvider) // for internal use
  private
    FProgressBar: TcxCustomProgressBar;
  protected
  ...
  public
    constructor Create(AProgressBar: TcxCustomProgressBar);
  end;

  { TdxProgressBarAccessibilityHelper }

  TdxProgressBarAccessibilityHelper = class(TdxEditAccessibilityHelper) // for internal use
  private
    FRangeValueProvider: TdxProgressBarRangeValueProvider;
  protected
...
uses
  ...
  dxUxTheme, cxGeometry, dxOffice11, cxLookAndFeels, dxCoreClasses, dxCoreGraphics, dxTypeHelpers;

const
  dxThisUnitName = 'cxProgressBar';
...
procedure CalculateCustomProgressBarViewInfo(AViewData: TcxCustomProgressBarViewData; AViewInfo: TcxCustomProgressBarViewInfo);
...
begin
  ...
  ABarRect := BarRect;
  AProgressBarRect := GetActualProgressChunkRect;
  AOverloadBarRect := OverloadBarRect;
  APeakBarRect := PeakBarRect;

  if UseRightToLeftAlignment then
  begin
    AOverloadBarRect := TdxRightToLeftLayoutConverter.ConvertRect(AOverloadBarRect, ABarRect);
    AProgressBarRect := TdxRightToLeftLayoutConverter.ConvertRect(AProgressBarRect, ABarRect);
    APeakBarRect := TdxRightToLeftLayoutConverter.ConvertRect(APeakBarRect, ABarRect);
  end;
  ...
  try
    ...
    Painter.DrawProgressBarText(ACanvas, GetDrawText, ABarRect, AProgressBarRect,
      AOverloadBarRect, APeakBarRect, IfThen(SolidTextColor, TextColor, clDefault),
      FTextOrientation = cxorVertical);
  finally
...
begin
  ...
  OffsetRect(BarContentRect, DX, DY);
...
begin
  ...
  begin
    if UseSkins and (BarStyle = cxbsSolid) then
...
begin
  ...
  try
    ...
    ABarRect := cxRectSetNullOrigin(BarRect);
    ...
    FUsualBitmap.cxCanvas.WindowOrg := BarRect.TopLeft;
    DrawText(FUsualBitmap.cxCanvas);
    FUsualBitmap.cxCanvas.WindowOrg := cxNullPoint;
...
begin
  CreatePainterBitmap;
  if IsInplace then
    cxEditFillRect(FPainterBitmap.cxCanvas, cxRectSetNullOrigin(Bounds), BackgroundColor)
  else
  ...
  FPainterBitmap.cxCanvas.SaveState;
  try
    AVertical := Orientation = cxorVertical;
    FPainterBitmap.cxCanvas.WindowOrg := Bounds.TopLeft;
    Painter.DrawProgressBarPart(FPainterBitmap.cxCanvas, BarRect, BarRect, AVertical, ScaleFactor, pbpBackground);
    FPainterBitmap.cxCanvas.IntersectClipRect(BarContentRect);
    Painter.DrawProgressBarPart(FPainterBitmap.cxCanvas, GetActualProgressChunkRect, BarContentRect, AVertical, ScaleFactor, pbpProgressChunk);
    if FRealShowOverload then
      Painter.DrawProgressBarPart(FPainterBitmap.cxCanvas, OverloadBarRect, BarContentRect, AVertical, ScaleFactor, pbpOverloadBar);
    if FRealShowPeak then
      Painter.DrawProgressBarPart(FPainterBitmap.cxCanvas, PeakBarRect, BarContentRect, AVertical, ScaleFactor, pbpPeakBar);
    DrawText(FPainterBitmap.cxCanvas);
  finally
    FPainterBitmap.cxCanvas.RestoreState;
  end;
...
procedure TcxCustomProgressBarViewInfo.CalcDrawingParams(
  out ADrawProgressBarRect, ADrawOverloadBarRect, ADrawPeakBarRect, ADrawAnimationBarRect, ASolidRect: TRect;
  out ALEDsWidth: Integer);
begin
...
begin
  ...
  ATheme := OpenTheme(totProgress, ScaleFactor);
...
function TcxCustomProgressBarViewInfo.GetActualProgressChunkRect: TRect;
begin
  if FMarquee and not IsInplace then
    Result := GetCorrectAnimationBarRect
  else
    Result := ProgressBarRect;
end;
...
begin
  ...
  begin
    if NativeStyle then
      dxDrawThemeBackground(ACanvas.Handle, totProgress, ScaleFactor, ABounds, BarThemeTypeMap[FOrientation], 1)
    else
...
begin
  ...
  begin
    ...
    begin
      ...
      if FOrientation = cxorHorizontal then
        FillRectByGradient(ABarRect, FBeginColor, FEndColor, True)
      else
        FillRectByGradient(ABarRect, FEndColor, FBeginColor, False);
    ...
    begin
      ...
      R.Top := ABarRect.Top;
      if FOrientation = cxorHorizontal then
        FillRectByGradient(R, FOverloadBeginColor, FOverloadEndColor, True)
      else
        FillRectByGradient(R, FOverloadEndColor, FOverloadBeginColor, False);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      with ASolidRect do
      begin
        FillRectByGradient(Rect(Left, Top, Left + ABorderExtPathWidth, Bottom),
          ABorderBeginColor, ABorderEndColor, True);
        FillRectByGradient(Rect(Right - ABorderExtPathWidth, Top, Right, Bottom),
          ABorderEndColor, ABorderBeginColor, True);

        FillRectByGradient(Rect(Left + ABorderExtPathWidth, Top, Left + ABorderWidth,
          Bottom), ABorderEndColor, ASolidColor, True);
        FillRectByGradient(Rect(Right - ABorderWidth, Top, Right - ABorderExtPathWidth, Bottom),
          ASolidColor, ABorderEndColor, True);

        FillRectByGradient(Rect(Left, Top, Right, Top + ScaleFactor.Apply(cxAnimationBarTopPartWidth)),
    ...
    begin
      ...
      with ASolidRect do
      begin
        FillRectByGradient(Rect(Left, Top, Right, Top + ABorderExtPathWidth),
          ABorderBeginColor, ABorderEndColor, False);
        FillRectByGradient(Rect(Left, Bottom - ABorderExtPathWidth, ASolidRect.Right, Bottom),
          ABorderEndColor, ABorderBeginColor, False);

        FillRectByGradient(Rect(Left, Top + ABorderExtPathWidth, Right,
          Top + ABorderWidth), ABorderEndColor, ASolidColor, False);
        FillRectByGradient(Rect(Left, Bottom - ABorderWidth, Right, Bottom - ABorderExtPathWidth),
          ASolidColor, ABorderEndColor, False);

        FillRectByGradient(Rect(Left, Top, Left + ScaleFactor.Apply(cxAnimationBarTopPartWidth),
...
procedure TcxCustomProgressBarViewData.Calculate(ACanvas: TcxCanvas; const ABounds: TRect; const P: TPoint;
  Button: TcxMouseButton; Shift: TShiftState; ACustomViewInfo: TcxCustomEditViewInfo; AIsMouseEvent: Boolean);
var
  ABarRect: TRect;
  ABitmap: TBitmap;
  ADelta: Integer;
  ...
  AProgressBarRect: TRect;
  ARect: TRect;
  AViewInfo: TcxCustomProgressBarViewInfo absolute ACustomViewInfo;
begin
  inherited Calculate(ACanvas, ABounds, P, Button, Shift, AViewInfo, AIsMouseEvent);
  if (ABounds.Bottom = cxMaxRectSize) or (ABounds.Right = cxMaxRectSize) then
    Exit; // B94428

  CalculateViewInfo(AViewInfo, AIsMouseEvent);
  CalculateViewInfoProperties(AViewInfo);
  AViewInfo.NativeStyle := AreVisualStylesMustBeUsed(NativeStyle, totProgress);
  ADelta := AViewInfo.GetDrawDelta;
  AOverloadBarRect := cxNullRect;
  APeakBarRect := cxNullRect;
  ...
  begin
    ...
    ABarRect := ABounds;
    if IsInplace then
      ARect := cxRectInflate(ARect, -(AViewInfo.BorderWidth + ADelta))
    else
      if Properties.BarStyle in [cxbsAnimation, cxbsAnimationLEDs] then
        ARect := cxRectInflate(ARect, -(AViewInfo.BorderWidth + 1))
      else
        ARect := cxRectTransform(ARect,
           (AViewInfo.BorderWidth + ADelta + 1), (AViewInfo.BorderWidth + ADelta + 1),
          -(AViewInfo.BorderWidth + ADelta), -(AViewInfo.BorderWidth + ADelta));
  end
  else
  begin
    if IsInplace then
      ABarRect := cxRectInflate(ABounds, -ADelta)
    else
      ABarRect := AViewInfo.BorderRect;

    if AViewInfo.UseSkins and (AViewInfo.BarStyle = cxbsSolid) then
      ARect := cxRectContent(AViewInfo.BorderRect, AViewInfo.Painter.ProgressBarBorderSize(AViewInfo.Orientation = cxorVertical))
    else
      ARect := cxRectInflate(AViewInfo.BorderRect, -AViewInfo.BorderWidth);
  ...
  CalculateCustomProgressBarViewInfo(Self, AViewInfo);

  if not Properties.Marquee then
  begin
    if AViewInfo.FOrientation = cxorHorizontal then
      AProgressBarRect.Right := ARect.Left + CalculateDelta(AViewInfo.Position - AViewInfo.Min, cxRectWidth(ARect), AViewInfo.MaxMinDiff)
    else
      AProgressBarRect.Top := ARect.Bottom - CalculateDelta((AViewInfo.Position - AViewInfo.Min), cxRectHeight(ARect), AViewInfo.MaxMinDiff);
  end;

  AViewInfo.FRealShowOverload := False;
  if not Properties.Marquee and Properties.ShowOverload and not AViewInfo.NativeStyle and (AViewInfo.Position >= AViewInfo.OverloadValue) then
  begin
    AViewInfo.FRealShowOverload := True;
    AOverloadBarRect := AProgressBarRect;
    if AViewInfo.FOrientation = cxorHorizontal then
    begin
      AOverloadBarRect.Left := ARect.Left + CalculateDelta(AViewInfo.RelativeOverloadValue, cxRectWidth(ARect), AViewInfo.MaxMinDiff);
      AOverloadBarRect.Right := Math.Min(AOverloadBarRect.Right, ARect.Right);
      AProgressBarRect.Right := AOverloadBarRect.Left;
    end
    else
    begin
      AOverloadBarRect.Top := AOverloadBarRect.Bottom - CalculateDelta(AViewInfo.RelativePosition, cxRectHeight(ARect), AViewInfo.MaxMinDiff);
      AOverloadBarRect.Bottom := AOverloadBarRect.Bottom - CalculateDelta(AViewInfo.RelativeOverloadValue, cxRectHeight(ARect), AViewInfo.MaxMinDiff);
      AOverloadBarRect.Bottom := Math.Max(AOverloadBarRect.Bottom, ARect.Top);
      AProgressBarRect.Top := AOverloadBarRect.Bottom;
    end;
  end;

  AViewInfo.FRealShowPeak := AViewInfo.ShowPeak and not Properties.Marquee;
  if AViewInfo.FRealShowPeak then
  begin
    APeakBarRect := AProgressBarRect;
    if AViewInfo.FOrientation = cxorHorizontal then
    begin
      APeakBarRect.Left := ARect.Left + CalculateDelta(AViewInfo.RelativePeakValue, cxRectWidth(ARect), AViewInfo.MaxMinDiff);
      APeakBarRect.Left := Math.Min(APeakBarRect.Left, ARect.Right - AViewInfo.PeakSize);
      APeakBarRect.Right := APeakBarRect.Left + AViewInfo.PeakSize;
    end
    else
    begin
      APeakBarRect.Bottom := ARect.Bottom - CalculateDelta(AViewInfo.RelativePeakValue, cxRectHeight(ARect), AViewInfo.MaxMinDiff);
      APeakBarRect.Bottom := Math.Max(APeakBarRect.Bottom, ARect.Top + AViewInfo.PeakSize);
      APeakBarRect.Top := APeakBarRect.Bottom - AViewInfo.PeakSize;
    end;
  end;

  if Properties.IsForegroundImageChanged or not IsGlyphAssigned(AViewInfo.ForegroundImage) then
  begin
    AViewInfo.ForegroundImage.Assign(Properties.ForegroundImage);
    Properties.IsForegroundImageChanged := False;
    if Properties.TransparentImage then
    begin
      ABitmap := TBitmap.Create;
      try
        AViewInfo.ForegroundImage.Transparent := True;
        ABitmap.Width := AViewInfo.ForegroundImage.Width;
        ABitmap.Height := AViewInfo.ForegroundImage.Height;
        ABitmap.Canvas.Brush.Color := AViewInfo.BackgroundColor;
        ABitmap.Canvas.FillRect(ABitmap.Canvas.ClipRect);
        ABitmap.Canvas.Draw(0, 0, AViewInfo.ForegroundImage);
        AViewInfo.ForegroundImage.Assign(ABitmap);
      finally
        ABitmap.Free;
      end;
    end;
  ...
  if Properties.IsPositionChanged and not AIsMouseEvent and
    cxRectIsEqual(AProgressBarRect, AViewInfo.ProgressBarRect) and
    cxRectIsEqual(ABarRect, AViewInfo.BarRect) and
    (not AViewInfo.FRealShowOverload or cxRectIsEqual(AOverloadBarRect, AViewInfo.OverloadBarRect)) and
    (not AViewInfo.FRealShowPeak or cxRectIsEqual(APeakBarRect, AViewInfo.PeakBarRect))
  then
    AViewInfo.FIsBoundsChanged := False
  else
  begin
    AViewInfo.BarContentRect := ARect;
    AViewInfo.ProgressBarRect := AProgressBarRect;
    if not cxRectIsEqual(AViewInfo.BarRect, ABarRect) then
    begin
      AViewInfo.BarRect := ABarRect;
      AViewInfo.FIsBarRectChanged := True;
    end;
    AViewInfo.OverloadBarRect := AOverloadBarRect;
    AViewInfo.PeakBarRect := APeakBarRect;
    AViewInfo.FIsBoundsChanged := True;
  end;
...
begin
  ...
  try
    ...
    with TcxCustomProgressBarViewInfo(AViewInfo) do
    begin
      ASize1 := cxRectSize(ProgressBarRect);

      if not(IsInplace or AreVisualStylesMustBeUsed(AViewInfo.NativeStyle, totButton)) then
      begin
        ASize1.cx := ASize1.cx + 4;
        ASize1.cy := ASize1.cy + 4;
      end;
      ...
      if Orientation = cxorHorizontal then
      begin
        ASize1.cx := ASize1.cx + ASize2.cx + 3;
        ASize1.cy := ASize2.cy;
      end
      else
      begin
        ASize1.cx := ASize2.cy;
        ASize1.cy := ASize1.cy + AEditSizeProperties.Height + 3;
      end;
    end;
...
constructor TdxProgressBarRangeValueProvider.Create(AProgressBar: TcxCustomProgressBar);
begin
  FProgressBar := AProgressBar;
end;

function TdxProgressBarRangeValueProvider.GetIsReadOnly: Boolean;
begin
  Result := True;
end;

function TdxProgressBarRangeValueProvider.GetMaximum: Double;
begin
  if FProgressBar.Properties.ShowTextStyle = cxtsPercent then
    Result := 100
  else
    Result := FProgressBar.Properties.Max;
end;

function TdxProgressBarRangeValueProvider.GetMinimum: Double;
begin
  if FProgressBar.Properties.ShowTextStyle = cxtsPercent then
    Result := 0
  else
    Result := FProgressBar.Properties.Min;
end;

function TdxProgressBarRangeValueProvider.GetValue: Double;
begin
  if FProgressBar.Properties.ShowTextStyle = cxtsPercent then
    Result := 100 * FProgressBar.Position / (FProgressBar.Properties.Max - FProgressBar.Properties.Min)
  else
...
constructor TdxProgressBarAccessibilityHelper.Create(AOwnerObject: TObject);
begin
  inherited;
  FRangeValueProvider := TdxProgressBarRangeValueProvider.Create(TcxCustomProgressBar(Edit));
end;

destructor TdxProgressBarAccessibilityHelper.Destroy;
begin
  FreeAndNil(FRangeValueProvider);
  inherited;
end;

function TdxProgressBarAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxROLE_SYSTEM_PROGRESSBAR;
end;

function TdxProgressBarAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxSTATE_SYSTEM_FOCUSABLE or cxSTATE_SYSTEM_READONLY;
end;

function TdxProgressBarAccessibilityHelper.GetValue(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := TcxCustomProgressBar(Edit).ViewInfo.GetDrawText;
end;

function TdxProgressBarAccessibilityHelper.IsSupportedPattern(APatternID: Integer; out AProvider: IUnknown;
  AChildID: TcxAccessibleSimpleChildElementID = 0): Boolean;
begin
  Result := APatternID = dxUIA_RangeValuePatternId;
  if Result then
...
function TcxCustomProgressBar.GetAccessibilityHelperClass: TdxEditAccessibilityHelperClass;
begin
  Result := TdxProgressBarAccessibilityHelper;
end;
...
function TcxCustomProgressBar.CanAutoHeight: Boolean;
begin
  Result := ActiveProperties.Orientation = cxorHorizontal;
end;

function TcxCustomProgressBar.CanAutoWidth: Boolean;
begin
  Result := ActiveProperties.Orientation = cxorVertical;
end;
