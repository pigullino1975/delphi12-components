...
{$REGION 'internal types'}
    ...
    TEMFFieldDef = class(TFieldDef)
    public
      MemberInfo: TdxMappingMemberInfo;
    end;

    TEMFFieldDefs = class(TFieldDefs)
    protected
    ...
    public
      ...
      procedure GetBookmark(AValue: TBookmark); inline;
      procedure SetBookmark(AValue: TBookmark); inline;
      class function BookmarkToObject(AValue: TBookmark): TObject; static; inline;
      class procedure ObjectToBookmark(AValue: TObject; ABookmark: TBookmark); static; inline;
    end;
...
{$ENDREGION}
  strict private
    ...
    function GetDataContext: TdxEMFCustomDataContext; 
  ...
  protected
    ...
    procedure InternalSetToRecord(ABuffer: TRecBuf); override;
    function IsCursorOpen: Boolean; override;
    function GetCanModify: Boolean; override;
    function GetRecordCount: Integer; override;
    procedure GetBookmarkData(ABuffer: TRecBuf; AData: TBookmark); override;
    function GetBookmarkFlag(ABuffer: TRecBuf): TBookmarkFlag; override;
    procedure SetBookmarkFlag(ABuffer: TRecordBuffer; AValue: TBookmarkFlag); override;
    procedure SetBookmarkData(ABuffer: TRecBuf; AData: TBookmark); override;
  ...
  public
    ...
    function GetFieldData(AField: TField; var Buffer: TValueBuffer): Boolean; override;
  ...
  public
    ...
    procedure AssignData<T: class>(const AData: TEnumerable<T>); overload;
    property Criteria; 
...
class function TdxEMFCustomDataSet.TRecInfo.BookmarkToObject(AValue: TBookmark): TObject;
begin
  Move(AValue[0], Result, SizeOf(TObject));
end;

class procedure TdxEMFCustomDataSet.TRecInfo.ObjectToBookmark(AValue: TObject; ABookmark: TBookmark);
begin
...
procedure TdxEMFCustomDataSet.TRecInfo.GetBookmark(AValue: TBookmark);
begin
  ObjectToBookmark(TheObject, AValue);
end;

procedure TdxEMFCustomDataSet.TRecInfo.SetBookmark(AValue: TBookmark);
begin
...
{ TdxEMFCustomDataSet.TEMFFieldDefs }

function TdxEMFCustomDataSet.TEMFFieldDefs.GetFieldDefClass: TFieldDefClass;
begin
  Result := TEMFFieldDef;
end;
...
begin
  TdxCustomEMFDataSourceAccess(FEMFDataSource).CheckEntityInfo;
end;
...
procedure TdxEMFCustomDataSet.GetBookmarkData(ABuffer: TRecBuf;
  AData: TBookmark);
begin
  GetBufferRecInfo(Pointer(ABuffer))^.GetBookmark(AData);
end;

function TdxEMFCustomDataSet.GetBookmarkFlag(ABuffer: TRecBuf): TBookmarkFlag;
begin
...
function TdxEMFCustomDataSet.GetFieldData(AField: TField; var Buffer: TValueBuffer): Boolean;
var
  ARecBuf: TRecordBuffer;
begin
  if not GetActiveRecordBuffer(ARecBuf) then
...
function TdxEMFCustomDataSet.GetFieldDefsClass: TFieldDefsClass;
begin
  Result := TEMFFieldDefs;
end;
...
procedure TdxEMFCustomDataSet.InternalAddRecord(Buffer: TRecordBuffer;
  Append: Boolean);
begin
  if Append then
...
procedure TdxEMFCustomDataSet.InternalGotoBookmark(ABookmark: TBookmark);
var
  AIndex: Integer;
begin
  AIndex := FData.IndexOf(TRecInfo.BookmarkToObject(ABookmark));
...
begin
  ...
  begin
    ...
    begin
      AFieldDef := FieldDefs.AddFieldDef as TEMFFieldDef;
      AFieldDef.MemberInfo := AMemberInfo;
...
begin
  ...
  begin
    ...
    AHasTrackingChanges := not TdxEMFCustomSessionAccess(Session).TrackingChanges;
    if AHasTrackingChanges then
...
procedure TdxEMFCustomDataSet.InternalSetToRecord(ABuffer: TRecBuf);
begin
...
procedure TdxEMFCustomDataSet.DataEvent(AEvent: TDataEvent; AInfo: NativeInt);
begin
...
procedure TdxEMFCustomDataSet.SetBookmarkData(ABuffer: TRecBuf;
  AData: TBookmark);
begin
...
procedure TdxEMFCustomDataSet.SetFieldProps(Field: TField; FieldDef: TFieldDef);
var
  AEMFFieldDef: TEMFFieldDef absolute FieldDef;
  AMemberInfo: TdxMappingMemberInfo;
begin
  inherited SetFieldProps(Field, FieldDef);
  if (FieldDef is TEMFFieldDef) and (AEMFFieldDef.MemberInfo <> nil) then
  begin
    AMemberInfo := AEMFFieldDef.MemberInfo;
    if AMemberInfo.DisplayLabel.HasValue then
      Field.DisplayLabel := AMemberInfo.DisplayLabel;
    if AMemberInfo.DisplayWidth.HasValue then
      Field.DisplayWidth := AMemberInfo.DisplayWidth;
    if AMemberInfo.EditMask.HasValue then
      Field.EditMask := AMemberInfo.EditMask;
    if Field is TNumericField then
    begin
      if AMemberInfo.DisplayFormat.HasValue then
        TNumericField(Field).DisplayFormat := AMemberInfo.DisplayFormat;
      if AMemberInfo.EditFormat.HasValue then
        TNumericField(Field).EditFormat := AMemberInfo.EditFormat;
      if Field is TIntegerField then
      begin
        if AMemberInfo.MaxValue.HasValue then
          TIntegerField(Field).MaxValue := Trunc(AMemberInfo.MaxValue.Value);
        if AMemberInfo.MinValue.HasValue then
          TIntegerField(Field).MinValue := Trunc(AMemberInfo.MinValue.Value);
      end
      else
      if Field is TLongWordField then
      begin
        if AMemberInfo.MaxValue.HasValue then
          TLongWordField(Field).MaxValue := Trunc(AMemberInfo.MaxValue.Value);
        if AMemberInfo.MinValue.HasValue then
          TLongWordField(Field).MinValue := Trunc(AMemberInfo.MinValue.Value);
      end
      else
      if Field is TLargeintField then
      begin
        if AMemberInfo.MaxValue.HasValue then
          TLargeintField(Field).MaxValue := Trunc(AMemberInfo.MaxValue.Value);
        if AMemberInfo.MinValue.HasValue then
          TLargeintField(Field).MinValue := Trunc(AMemberInfo.MinValue.Value);
      end
      else
      if Field is TFloatField then
      begin
        if AMemberInfo.MaxValue.HasValue then
          TFloatField(Field).MaxValue := AMemberInfo.MaxValue;
        if AMemberInfo.MinValue.HasValue then
          TFloatField(Field).MinValue := AMemberInfo.MinValue;
        if AMemberInfo.Precision.HasValue then
          TFloatField(Field).Precision := AMemberInfo.Precision;
      end
      else
      if Field is TSingleField then
      begin
        if AMemberInfo.MaxValue.HasValue then
          TSingleField(Field).MaxValue := AMemberInfo.MaxValue;
        if AMemberInfo.MinValue.HasValue then
          TSingleField(Field).MinValue := AMemberInfo.MinValue;
        if AMemberInfo.Precision.HasValue then
          TSingleField(Field).Precision := AMemberInfo.Precision;
      end
      else
      if Field is TExtendedField then
      begin
        if AMemberInfo.MaxValue.HasValue then
          TExtendedField(Field).MaxValue := AMemberInfo.MaxValue;
        if AMemberInfo.MinValue.HasValue then
          TExtendedField(Field).MinValue := AMemberInfo.MinValue;
        if AMemberInfo.Precision.HasValue then
          TExtendedField(Field).Precision := AMemberInfo.Precision;
      end
      else
      if Field is TBCDField then
      begin
        if AMemberInfo.MaxValue.HasValue then
          TBCDField(Field).MaxValue := AMemberInfo.MaxValue;
        if AMemberInfo.MinValue.HasValue then
          TBCDField(Field).MinValue := AMemberInfo.MinValue;
        if AMemberInfo.Precision.HasValue then
          TBCDField(Field).Precision := AMemberInfo.Precision;
      end
      else
      if Field is TFMTBCDField then
      begin
        if AMemberInfo.MaxValue.HasValue then
          TFMTBCDField(Field).MaxValue := FloatToStr(AMemberInfo.MaxValue);
        if AMemberInfo.MinValue.HasValue then
          TFMTBCDField(Field).MinValue := FloatToStr(AMemberInfo.MinValue);
        if AMemberInfo.Precision.HasValue then
          TFMTBCDField(Field).Precision := AMemberInfo.Precision;
      end
    end
    else
    ...
    begin
      if AMemberInfo.DisplayFormat.HasValue then
        TDateTimeField(Field).DisplayFormat := AMemberInfo.DisplayFormat;
    end
    else
    if Field is TSQLTimeStampField then
    begin
      if AMemberInfo.DisplayFormat.HasValue then
        TSQLTimeStampField(Field).DisplayFormat := AMemberInfo.DisplayFormat;
    end
  end;
end;
...
begin
  ...
  begin
    ...
    case AFieldInfo.Field.DataType of
      ...
      ftBoolean:
        if PWord(AFieldInfo.GetFieldBuffer(AActiveBuffer))^ = 0 then
          AData := False
        else
          AData := True;
      ftFloat, ftCurrency:
...
begin
  if HasLoader then
    TdxEMFCustomCollectionAccess(FInternalCollection).DoFetchAll;
end;
...
procedure TdxEMFTableDataSource.ActiveChanged(AActive: Boolean);
begin
end;
...
procedure TdxEMFDataSet.AssignData<T>(const AData: TEnumerable<T>);
var
  AItem: T;
begin
  Close;
...
procedure TdxEMFCustomQueryDataSet.DataEvent(AEvent: TDataEvent; AInfo: NativeInt);
begin
  if (AEvent = deParentScroll) and (DataSetField = nil) then
  begin
    CheckBrowseMode;
