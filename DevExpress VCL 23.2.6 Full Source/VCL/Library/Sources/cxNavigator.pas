...
uses
  System.UITypes,
  Types, Messages, Windows, Classes, Forms, Controls, Graphics, ImgList, StdCtrls, SysUtils,
  dxCore, dxCoreClasses, cxClasses, cxContainer, cxControls, cxFilter, cxGraphics, cxGeometry, cxAccessibility,
...
type
  ...
  IcxNavigatorOwner3 = interface // for internal use
  ...
  public
    ...
    property Navigator: IcxNavigatorOwner read GetNavigator; // for internal use
  ...
  TcxNavigatorChildComponent = class(TcxInterfacedPersistent)
  strict private
  ...
  TcxNavigatorInfoPanelClickEvent = procedure (Sender: TcxCustomNavigatorInfoPanel; Button: TMouseButton) of object;

  TcxCustomNavigatorInfoPanel = class(TcxNavigatorChildComponent, IdxLocalizerListener)
  strict private
    FDisplayMask: string;
  ...
  protected
    ...
    function IsInplaceNavigator: Boolean; virtual;
  ...
  public
    ...
    property OnGetIRecordPosition: TcxInfoPanelGetIRecordPosition read FOnGetIRecordPosition write FOnGetIRecordPosition;
  published
  ...
  public
    ...
    //
    property Owner: TcxNavigatorViewInfo read FOwner; // for internal use
  ...
  public
    ...
    property Items[Index: Integer]: TcxNavigatorButtonViewInfo read GetItem; default; // for internal use
    property Size: TSize read FSize;
  end;
  ...
  { TdxNavigatorAccessibilityHelper }

  TdxNavigatorAccessibilityHelper = class(TcxAccessibilityHelper)
  strict private
    function GetViewInfo: TcxNavigatorViewInfo;
  protected
    ...
    property ViewInfo: TcxNavigatorViewInfo read GetViewInfo;
  public
  ...
  strict private
    ...
    FNavigatorOwner3: IcxNavigatorOwner3;
    FPressedButtonViewInfo: TcxNavigatorButtonViewInfo;
    FUseFullControlWidth: Boolean;
  ...
  protected
    ...
    procedure AddAdornerTargetElements(AList: TStrings); virtual;
    procedure CalculateBounds(const AInfoPanelText: string; AInfoPanelWidth, AWidthDelta, AHeight, AButtonIndent: Integer);
    ...
    procedure PaintBackground(ACanvas: TcxCanvas; const ABounds: TRect); virtual;
    ...
    property NavigatorOwner3: IcxNavigatorOwner3 read FNavigatorOwner3;
    property PressedButtonViewInfo: TcxNavigatorButtonViewInfo read FPressedButtonViewInfo write FPressedButtonViewInfo;
    property ScaleFactor: TdxScaleFactor read GetScaleFactor;
    property UseFullControlWidth: Boolean read FUseFullControlWidth write FUseFullControlWidth;
  public
    ...
    property Buttons: TcxNavigatorButtonsViewInfo read FButtons; // for internal use
  end;
  ...
  private
    ...
    function GetUseFullWidth: Boolean;
  ...
  protected
    ...
    function IsInplaceNavigator: Boolean; override;
...
{$R cxNavigator_svg.res}

uses
  Dialogs, Math, RTLConsts,
  dxThemeConsts, dxUxTheme, cxLibraryConsts, dxCoreGraphics,
  cxEditPaintUtils, cxEditUtils, cxEditConsts, dxDPIAwareUtils, dxTypeHelpers, dxMessageDialog;

const
  dxThisUnitName = 'cxNavigator';
...
begin
  ...
  begin
    ...
    AResourceStream := TResourceStream.Create(HInstance, 'CXNAVIGATORBUTTONS_SVG', RT_RCDATA);
    try
      FNavigatorImages.AddImagesFromZipStream(AResourceStream);
    finally
...
destructor TcxCustomNavigatorInfoPanel.Destroy;
begin
...
begin
  FFormatString := cxGetNavigatorFormatString(GetDisplayMask, FIsNeedRecordIndex,
    FIsNeedRecordCount);
end;
...
function TcxCustomNavigatorInfoPanel.GetDisplayMask: string;
begin
  if IsDisplayMaskStored then
    Result := FDisplayMask
  else
    Result := cxGetResourceString(@cxNavigatorInfoPanelDefaultDisplayMask);
end;

function TcxCustomNavigatorInfoPanel.IsInplaceNavigator: Boolean;
begin
  Result := True;
end;
...
procedure TcxCustomNavigatorInfoPanel.TranslationChanged;
begin
  BuildFormatString;
  Changed(nctSize, not IsInplaceNavigator);
end;
...
begin
  ...
  begin
    if Self = Owner.Buttons.First then
      Dec(R.Left);
    if Self = Owner.Buttons.Last then
      Inc(R.Right);
    Dec(R.Top);
    Inc(R.Bottom);
  end;
...
begin
  ...
  if not dxFader.DrawFadeImage(Self, ACanvas.Handle, ABounds) then
    DrawContent(ACanvas, ABounds, AState);
  ...
  begin
    ...
    DrawGlyph(ACanvas, AImageRect, AState);
  end;

  if NeedDrawFocusRect then
    ACanvas.DrawFocusRect(cxRectInflate(ABounds, -2));
end;

procedure TcxNavigatorButtonViewInfo.DrawContent(ACanvas: TcxCanvas; const R: TRect; AState: TcxButtonState);
begin
  Painter.DrawNavigatorScaledButton(ACanvas, R, AState, clWindow, Button.ScaleFactor);
end;

procedure TcxNavigatorButtonViewInfo.DrawGlyph(ACanvas: TcxCanvas; const R: TRect; AState: TcxButtonState);
var
  AGlyphRect: TRect;
begin
  AGlyphRect := R;
  cxRightToLeftDependentDraw(ACanvas.Handle, AGlyphRect,
    InRange(Button.DefaultIndex, NBDI_FIRST, NBDI_LAST) and Owner.IsRightToLeftConverted,
    procedure
    begin
      Painter.DrawNavigatorScaledButtonGlyph(ACanvas, Button.InternalImages,
        Button.InternalImageIndex, AGlyphRect, AState <> cxbsDisabled,
...
var
  ...
  AInvalidateBounds: TRect;
begin
  ANavigatorControl := Owner.NavigatorOwner.GetNavigatorControl;
  if ANavigatorControl.HandleAllocated and not Bounds.IsEmpty then
  begin
    AInvalidateBounds := Bounds;
    if Owner.NavigatorOwner3 <> nil then
      AInvalidateBounds.Offset(Owner.NavigatorOwner3.GetOrigin);
    cxInvalidateRect(ANavigatorControl.Handle, AInvalidateBounds, False);
  end;
end;
...
begin
  Result := (Self = Owner.FocusedButton) and
    ...
    Owner.NavigatorOwner.GetNavigatorFocused and
    Painter.SupportsNativeFocusRect;
end;
...
begin
  FText := AText;
  FBounds := ABounds;
  FViewParams := InfoPanel.GetViewParams;
  if FViewParams.TextColor = clDefault then
    FViewParams.TextColor := Painter.NavigatorInfoPanelTextColor;
  FTextRect := cxRectContent(Bounds, Painter.NavigatorInfoPanelContentOffsets(NavigatorViewInfo.ScaleFactor));
end;
...
begin
  ...
  begin
    ...
    try
      Draw(ABitmap.cxCanvas, ABitmap.ClientRect);
      ABitmap.MakeOpaque;
    ...
    Draw(NavigatorViewInfo.Canvas, Bounds);
end;

function TcxCustomNavigatorInfoPanelViewInfo.GetAdornerTargetElementControl: TWinControl;
begin
  Result := NavigatorViewInfo.NavigatorOwner.GetNavigatorControl;
end;
...
begin
  Result := InfoPanel.GetAdornerTargetElementVisible;
end;
...
procedure TcxCustomNavigatorInfoPanelViewInfo.Draw(ACanvas: TcxCanvas; const R: TRect);
begin
  Painter.DrawNavigatorInfoPanel(ACanvas, R, FViewParams);

  ACanvas.Brush.Style := bsClear;
  ACanvas.Font := FViewParams.Font;
  ACanvas.Font.Color := FViewParams.TextColor;
  ACanvas.DrawTexT(Text, TextRect, cxSingleLine or cxAlignCenter);
  ACanvas.Brush.Style := bsSolid;
end;

function TcxCustomNavigatorInfoPanelViewInfo.GetInfoPanel: TcxCustomNavigatorInfoPanel;
begin
  Result := FNavigatorViewInfo.NavigatorOwner2.GetNavigatorInfoPanel;
end;

function TcxCustomNavigatorInfoPanelViewInfo.GetIsPaintOnGlass: Boolean;
begin
  Result := dxIsPaintOnGlass(GetAdornerTargetElementControl);
end;

function TcxCustomNavigatorInfoPanelViewInfo.GetPainter: TcxCustomLookAndFeelPainter;
begin
  Result := NavigatorViewInfo.LookAndFeelPainter;
end;
...
function TdxNavigatorAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  if AChildID = 0 then
    Result := ViewInfo.NavigatorOwner.GetNavigatorBounds
  else
    if AChildID > ViewInfo.ButtonCount then
      Result := ViewInfo.InfoPanelViewInfo.Bounds
    else
      if AChildID > 0 then
        Result := ViewInfo.Buttons[AChildID - 1].Bounds;

  Result := cxRectSetOrigin(Result, ViewInfo.NavigatorOwner.GetNavigatorControl.ClientToScreen(Result.TopLeft));
end;

function TdxNavigatorAccessibilityHelper.ChildIsSimpleElement(AIndex: Integer): Boolean;
begin
  Result := True;
end;

procedure TdxNavigatorAccessibilityHelper.DoDefaultAction(AChildID: TcxAccessibleSimpleChildElementID);
begin
  if (AChildID > 0) and (AChildID <= ViewInfo.ButtonCount) then
    ViewInfo.NavigatorOwner.GetNavigatorButtons.ClickButton(ViewInfo.Buttons[AChildID - 1].Button.DefaultIndex);
end;

function TdxNavigatorAccessibilityHelper.GetChildCount: Integer;
begin
  Result := ViewInfo.ButtonCount;
  if ViewInfo.InfoPanel.Visible then
    Inc(Result);
end;

function TdxNavigatorAccessibilityHelper.GetDefaultActionDescription(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  if (AChildID > 0) and (AChildID <= ViewInfo.ButtonCount) then
    Result := 'Press';
end;

function TdxNavigatorAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := 'Navigator';
  if AChildID > ViewInfo.ButtonCount then
    Result := ViewInfo.InfoPanelViewInfo.Text
  else
    if AChildID > 0 then
      Result := ViewInfo.Buttons[AChildID - 1].Button.GetInternalHint;

end;

function TdxNavigatorAccessibilityHelper.GetOwnerObjectWindow: HWND;
begin
  Result := ViewInfo.NavigatorOwner.GetNavigatorControl.Handle;
end;

function TdxNavigatorAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
...
function TdxNavigatorAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxSTATE_SYSTEM_FOCUSABLE;
  if (AChildID > 0) and (AChildID <= ViewInfo.ButtonCount) and not ViewInfo.Buttons[AChildID - 1].Button.GetInternalEnabled then
    Result := Result or cxSTATE_SYSTEM_UNAVAILABLE;
end;

function TdxNavigatorAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := [aopFocus, aopLocation, aopDefaultAction];
end;

function TdxNavigatorAccessibilityHelper.IsExtended: Boolean;
begin
  Result := True;
end;

function TdxNavigatorAccessibilityHelper.GetViewInfo: TcxNavigatorViewInfo;
begin
  Result := TcxNavigatorViewInfo(OwnerObject);
end;
...
begin
  ...
  Supports(ANavigator, IcxNavigatorOwner3, FNavigatorOwner3);
...
begin
  ...
  AMinWidth := AWidth;
  CalculateSize(AMinWidth, AHeight, AInfoPanelText, AButtonIndent, AInfoPanelWidth);
  if UseFullControlWidth then
    AWidthDelta := AWidth - AMinWidth
  else
    AWidth := AMinWidth;
  CalculateBounds(AInfoPanelText, AInfoPanelWidth, AWidthDelta, AHeight, AButtonIndent);
end;
...
function TcxNavigatorViewInfo.GetButtonViewInfoByButton(AButton: TcxNavigatorButton): TcxNavigatorButtonViewInfo;
var
  I: Integer;
begin
  Result := nil;
...
begin
  CheckCalculate;
  ABounds := GetBackgroundBounds;
  PaintBackground(Canvas, ABounds);
  for I := 0 to ButtonCount - 1 do
    PaintButton(I);
...
begin
  CheckCalculate;
  ASelected := FIsSelected;
  if not ASelected then
  begin
    ANavigatorControl := NavigatorOwner.GetNavigatorControl;
    ASelected := (csDesigning in ANavigatorControl.ComponentState) and ANavigatorControl.Enabled;
  end;
  LookAndFeelPainter.DrawNavigatorBorder(Canvas, ABounds, ASelected);
  if LookAndFeelPainter.LookAndFeelStyle in [lfsUltraFlat, lfsOffice11] then
    Canvas.IntersectClipRect(cxRectInflate(ABounds, -1));
end;
...
begin
  ...
  begin
    ...
    AInvalidateBounds := InfoPanelViewInfo.Bounds;
    if NavigatorOwner3 <> nil then
      AInvalidateBounds.Offset(NavigatorOwner3.GetOrigin);
    Windows.InvalidateRect(ANavigatorControl.Handle, @AInvalidateBounds, False);
  end;
end;
...
procedure TcxNavigatorViewInfo.CalculateBounds(const AInfoPanelText: string; AInfoPanelWidth, AWidthDelta, AHeight, AButtonIndent: Integer);
var
...
begin
  ...
  ANavigatorBounds := NavigatorOwner.GetNavigatorBounds;
  AButtonBounds := cxRectBounds(ANavigatorBounds.Left, ANavigatorBounds.Top, 0, AHeight);
  ...
  begin
    ...
    if AWidthDelta > 0 then
    begin
      AButtonBounds.Width := AButtonBounds.Width + 1;
      Dec(AWidthDelta);
    end;
...
procedure TcxNavigatorViewInfo.PaintBackground(ACanvas: TcxCanvas; const ABounds: TRect);
begin
  if FIsInplace then
    ACanvas.FillRect(ABounds, LookAndFeelPainter.DefaultControlColor)
  else
  begin
    cxDrawTransparentControlBackground(NavigatorOwner.GetNavigatorControl, ACanvas, ABounds);
    if not LookAndFeelPainter.SupportsEditorBorders then
      LookAndFeelPainter.DrawScaledEditorBackground(ACanvas, ABounds, IsInplace, False,
        LookAndFeelPainter.DefaultControlColor, cbsNormal, ScaleFactor, nil);
  end;
end;
...
function TcxNavigatorViewInfo.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin
  if FIAccessibilityHelper = nil then
    FIAccessibilityHelper := TdxNavigatorAccessibilityHelper.Create(Self);
...
function TcxNavigatorViewInfo.GetLookAndFeelPainter: TcxCustomLookAndFeelPainter;
begin
  Result := NavigatorOwner.GetNavigatorLookAndFeel.GetAvailablePainter(totButton);
end;
...
function TcxNavigatorViewInfo.GetBackgroundBounds: TRect;
begin
  if IsInplace or LookAndFeelPainter.SupportsEditorBorders then
    Result := NavigatorOwner.GetNavigatorBounds
  else
    Result := NavigatorOwner.GetNavigatorControl.ClientRect;
end;
...
begin
  ...
  begin
    ...
    AMargins := LookAndFeelPainter.NavigatorInfoPanelContentOffsets(ScaleFactor);
    if AInfoPanel.Width > 0 then
      Result := cxSize(AInfoPanel.Width, TdxTextMeasurer.TextLineHeight(AFont) + cxMarginsHeight(AMargins))
    else
    begin
      for I := 1 to Length(AText) do
      begin
        if dxCharInSet(AText[I], ['0'..'9']) then
          AText[I] := '0';
      end;
      Result := cxSizeAddMargins(TdxTextMeasurer.TextSizeTO(AFont, AText), AMargins);
      Inc(Result.cx, 2 * TdxTextMeasurer.TextIndent(AFont));
    end;
  end
  else
    Result := cxNullSize;
...
begin
  ...
  Result := cxSizeAddMargins(Result, APainter.NavigatorScaledButtonGlyphPadding(ScaleFactor));
  Result := cxSizeMax(Result, APainter.NavigatorScaledButtonMinSize(ScaleFactor));
end;
...
var
  ...
  AOffset: TPoint;
begin
  if not LookAndFeelPainter.IsButtonHotTrack then
    Exit;
  ANavigatorControl := NavigatorOwner.GetNavigatorControl;
  ...
  begin
    ...
    if NavigatorOwner3 <> nil then
    begin
      AOffset := NavigatorOwner3.GetOrigin;
      R1.Offset(AOffset);
      R2.Offset(AOffset);
    end;
...
begin
  ...
  CalculateBounds(AInfoPanelText, AInfoPanelWidth, 0, FNavigatorSize.cy, AButtonIndent);
  CalculateButtons;
  FIsDirty := False;
end;
...
begin
  Result := Max(Max(FNavigatorSize.cy,
    LookAndFeelPainter.NavigatorScaledButtonMinSize(ScaleFactor).cy),
    dxGetSystemMetrics(SM_CYHSCROLL, ScaleFactor));
  if (InfoPanel <> nil) and InfoPanel.Visible then
    Result := Max(Result, TdxTextMeasurer.TextLineHeight(GetInfoPanelFont));
  if FIsDirty then
    ValidateButtonsViewInfos;
...
begin
  ...
  begin
    ...
    ANavigatorWidth := GetWidth;
    if Control.UseRightToLeftAlignment then
    begin
      if AOwnerControl.IsSizeGripVisible and cxRectIsEmpty(Result) then
      begin
        Result.Left := AOwnerControl.GetSizeGripBounds.Right;
        Result.Right := AOwnerControl.ClientBounds.Right;
      end
      else
      begin
        Result.Left := Result.Right;
        Result.Right := Result.Left + ANavigatorWidth;
      end;
    end
    else
    begin
      if AOwnerControl.IsSizeGripVisible and cxRectIsEmpty(Result) then
      begin
        Result.Right := AOwnerControl.GetSizeGripBounds.Left;
        Result.Left := AOwnerControl.ClientBounds.Left;
      end
      else
      begin
        Result.Right := Result.Left;
        Result.Left := Result.Right - ANavigatorWidth;
      end;
    end;
  end
  else
  begin
    ...
    if not AOwnerControl.IsPopupScrollBars and AOwnerControl.VScrollBarVisible and not AOwnerControl.HScrollBarVisible then
      Inc(Result.Right, AOwnerControl.GetVScrollBarAreaWidth);
  end;
end;
...
begin
  ...
  FViewInfo.UseFullControlWidth := GetUseFullWidth;
  FViewInfo.Paint;
end;
...
begin
  ...
  if (Button = mbLeft) and not IsDesigning then
...
procedure TcxCustomNavigator.Notification(AComponent: TComponent; Operation: TOperation);
begin
  inherited Notification(AComponent, Operation);
  if not IsDestroying and (Operation = opRemove) and (AComponent = CustomButtons.Images) then
    CustomButtons.Images := nil;
end;
...
function TcxCustomNavigator.GetUseFullWidth: Boolean;
begin
  Result := Align in [alTop, alBottom, alClient];
end;
...
procedure TcxCustomNavigator.WMGetObject(var Message: TMessage);
var
  AObjectID: Cardinal;
begin
...
function TcxCustomNavigator.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin
...
begin
  ...
  if (ANavigatorControl <> nil) and ((ADefaultIndex <> NBDI_DELETE) or not ConfirmDelete or
      ((dxMessageDlg(cxGetResourceString(@cxNavigator_DeleteRecordQuestion),
...
function TcxNavigatorControlInfoPanel.IsInplaceNavigator: Boolean;
begin
  Result := False;
end;
