...
type
  ...
  protected
    ...
    class var FPrevNextTimeZone: Integer;
    class var FPrevTimeZone: Integer;
    class var FTimeZoneDeltaBias: Double;
    // first day of week
    class var FSystemStartOfWeek: TDay;
    // date formats
    class var FLongDateOrder: Integer;
    class var FLongDayMonthFormat: string;
    class var FDayMonthFormats: TStringList;
    class var FLockUpdateDayMonthFormat: Boolean;
    // date utils
    class procedure CutMonth(var S: string; AStartIndex: Integer);
    class procedure CutLeftYear(var S: string);
    class procedure CutRightYear(var S: string);
    class procedure CutYear(var S: string; AStartIndex: Integer);
    class procedure GetDayMonthFormats;
    class function GetLongDayMonthFormat: string;
    class function GetShortestFormat: string;
    // time zone info
    ...
    class procedure InitDayMonthFormats;
    class procedure FreeDayMonthFormats;
  public
    ...
    class function MonthToStrNominative(const ADate: TDateTime): string; virtual;
    ...
    class procedure LockUpdateDayMonthFormat(ALockUpdate: Boolean);
    class function LockedUpdateDayMonthFormat: Boolean;
  end;
  ...
  public
    ...
    class function LoadImages(AWidth, AHeight: Integer; const AResName: string;
      AHasPalette: Boolean = False; AHasTransparent: Boolean = True): TcxImageList;
    class function LoadSvgImages(AWidth, AHeight: Integer; const AResName: string;
      AHasPalette: Boolean = False; AHasTransparent: Boolean = True): TcxImageList;
  ...
  private
    function GetItem(AIndex: TdxListIndex): TObject;
  public
    destructor Destroy; override;
    procedure Clear; override;
    property Items[Index: TdxListIndex]: TObject read GetItem;
  end;
  ...
  private
    ...
    function GetItem(AIndex: TdxListIndex): TDateTime;
    procedure SetItem(AIndex: TdxListIndex; const AValue: TDateTime);
  protected
  ...
  public
    ...
    property Items[Index: TdxListIndex]: TDateTime read GetItem write SetItem; default;
...
function cxDaysToInteger(ADay: TDays): Integer; inline;
function cxIntegerToDays(AValue: Integer): TDays; inline;
...
function EventImages: TcxImageList;
function MenuImages: TcxImageList;
function TimeLinePatterns: TcxImageList;
function StartContinueArrow: TdxSkinImage;
function EndContinueArrow: TdxSkinImage;
...
uses
  CommCtrl, cxClasses, cxControls, cxLibraryConsts, dxCoreGraphics, cxFormats;

const
  dxThisUnitName = 'cxSchedulerUtils';
...
var
  ...
  FEventImages: TcxImageList;
  FMenuImages: TcxImageList;
  FTimeLinePatterns: TcxImageList;

  FStartContinueArrow: TdxSkinImage;
  FEndContinueArrow: TdxSkinImage;
  ...
  Shadow, ShadowRTL: TdxSkinImage;
  ModernMoreEvents: TdxSkinImage;


{$R cxScheduler.res}
{$R cxScheduler_svg.res}
...
function EventImages: TcxImageList;
begin
  if not Assigned(FEventImages) then
  begin
    if dxUseVectorIcons then
      FEventImages := TcxSchedulerPainterHelper.LoadSvgImages(17, 17, 'CXSCHEDULEREVENTICONS', True)
    else
      FEventImages := TcxSchedulerPainterHelper.LoadImages(17, 17, 'CXSCHEDULEREVENTICONS', True);
  end;
  Result := FEventImages;
end;
...
function MenuImages: TcxImageList;
begin
  if not Assigned(FMenuImages) then
  begin
    if dxUseVectorIcons then
      FMenuImages := TcxSchedulerPainterHelper.LoadSvgImages(16, 16, 'CXSCHEDULERMENUIMAGES', True)
    else
      FMenuImages := TcxSchedulerPainterHelper.LoadImages(16, 16, 'CXSCHEDULERMENUIMAGES', True);
  end;
  Result := FMenuImages;
end;
...
function TimeLinePatterns: TcxImageList;
begin
  if not Assigned(FTimeLinePatterns) then
  begin
    if dxUseVectorIcons then
      FTimeLinePatterns := TcxSchedulerPainterHelper.LoadSvgImages(12, 12, 'CXTIMELINEPATTERNS', True)
    else
      FTimeLinePatterns := TcxSchedulerPainterHelper.LoadImages(12, 12, 'CXTIMELINEPATTERNS', True);
  end;
  Result := FTimeLinePatterns;
end;

function StartContinueArrow: TdxSkinImage;
begin
  if not Assigned(FStartContinueArrow) then
  begin
    FStartContinueArrow := TdxSkinImage.Create(nil);
    if dxUseVectorIcons then
      FStartContinueArrow.Texture.LoadFromResource(hInstance, 'STARTCONTINUEARROW', 'SVG')
    else
      FStartContinueArrow.Texture.LoadFromResource(hInstance, 'STARTCONTINUEARROW', 'PNG');
     FStartContinueArrow.Stretch := smNoResize;
  end;
  Result := FStartContinueArrow;
end;

function EndContinueArrow: TdxSkinImage;
begin
  if not Assigned(FEndContinueArrow) then
  begin
    FEndContinueArrow := TdxSkinImage.Create(nil);
    if dxUseVectorIcons then
      FEndContinueArrow.Texture.LoadFromResource(hInstance, 'ENDCONTINUEARROW', 'SVG')
    else
      FEndContinueArrow.Texture.LoadFromResource(hInstance, 'ENDCONTINUEARROW', 'PNG');
     FEndContinueArrow.Stretch := smNoResize;
  end;
  Result := FEndContinueArrow;
end;
...
begin
  if (ATimeZone <> FPrevTimeZone) or (ANextTimeZone <> FPrevNextTimeZone) then
  begin
    FTimeZoneDeltaBias := ConvertToLocalTime(
      ConvertToGlobalTime(ADateTime, ATimeZone), ANextTimeZone) - ADateTime;
    FPrevTimeZone := ATimeZone;
    FPrevNextTimeZone := ANextTimeZone;
  end;
  Result := ADateTime;
  if (FTimeZoneDeltaBias < 0) and (Result < 1) then
    Result := Result + 1;
  Result := Result + FTimeZoneDeltaBias;
end;
...
class procedure TcxSchedulerDateTimeHelper.CutLeftYear(var S: string);
var
  F: Boolean;
  I, Pos: Integer;
begin
...
class procedure TcxSchedulerDateTimeHelper.CutRightYear(var S: string);
var
  F: Boolean;
  I, Pos: Integer;
begin
...
class procedure TcxSchedulerDateTimeHelper.CutYear(var S: string; AStartIndex: Integer);
var
  F: Boolean;
  I: Integer;
begin
  F := False;
  for I := AStartIndex + 4 to Length(S) do
    if Pos(S[I], '''"') > 0 then
      F := not F
    else
      if not F and dxCharInSet(UpCase(S[I]), ['D', 'M']) then
        break;
  Delete(S, AStartIndex, I - AStartIndex);
end;

class procedure TcxSchedulerDateTimeHelper.CutMonth(var S: string; AStartIndex: Integer);
  ...
  begin
    ...
    if Pos > 0 then
      Delete(S, I + 1, AStartIndex - I + 2);
  end;

begin
  if FLongDateOrder = 1 then
    CutRightMonth(S)
  else
    CutLeftMonth(S);
  S := Trim(S);
...
class function TcxSchedulerDateTimeHelper.GetShortestFormat: string;
var
  I: Integer;
begin
...
function DayMonthFormatsCompareStrings(List: TStringList; Index1, Index2: Integer): Integer;
var
  ALength1, ALength2: Integer;
begin
  ALength1 := Length(List.Strings[Index1]);
  ALength2 := Length(List.Strings[Index2]);
  Result := 0;
...
class procedure TcxSchedulerDateTimeHelper.GetDayMonthFormats;
var
  S: string;
  I: Integer;
begin
  S := FLongDayMonthFormat;
  FDayMonthFormats.Clear;
  FDayMonthFormats.Append(S);
  FDayMonthFormats.Append(GetShortestFormat);
  ...
  I := Pos('ddd', LowerCase(S));
  if (I > 0) and (Pos('m', LowerCase(S)) = 0) then
    FDayMonthFormats.Append('d');
  FDayMonthFormats.CustomSort(DayMonthFormatsCompareStrings)
end;

class function TcxSchedulerDateTimeHelper.DateToLongDateStr(ADate: TDateTime): string;
begin
...
begin
  ...
  begin
    ...
    if not cxGetDateFormat(ADate, Result, 0, FDayMonthFormats[AFormat]) then
...
class function TcxSchedulerDateTimeHelper.MonthToStrNominative(const ADate: TDateTime): string;
begin
  if cxGetDateFormat(ADate, Result, 0, 'dd MMMM') then
    Delete(Result, 1, Pos(' ', Result))
  else
    Result := dxFormatSettings.LongMonthNames[MonthOf(ADate)];
end;

class function TcxSchedulerDateTimeHelper.MonthYearToStr(
  AMonth, AYear: Integer): string;
begin
  if FLongDateOrder < 2 then
...
class procedure TcxSchedulerDateTimeHelper.FreeDayMonthFormats;
begin
  if FDayMonthFormats <> nil then
    FDayMonthFormats.Free;
end;
...
class function TcxSchedulerDateTimeHelper.LockedUpdateDayMonthFormat: Boolean;
begin
  Result := FLockUpdateDayMonthFormat;
end;

class procedure TcxSchedulerDateTimeHelper.LockUpdateDayMonthFormat(ALockUpdate: Boolean);
begin
  FLockUpdateDayMonthFormat := ALockUpdate;
end;

class function TcxSchedulerDateTimeHelper.StartOfWeek: Integer;
begin
  Result := Ord(FSystemStartOfWeek);
end;
...
begin
  if AIsPriorFormatUpdateRequired then
    UpdateDayMonthFormat;
  if InRange(AFormat, 0, FDayMonthFormats.Count - 1) then
    Result := FDayMonthFormats[AFormat]
  else
    Result := '';
end;
...
class function TcxSchedulerDateTimeHelper.GetLongDayMonthFormat: string;
var
  L, AYearPosition: Integer;
begin
  if cxFormatController.UseDelphiDateTimeFormats then
  ...
  if L < 2 then
    ...
    CutLeftYear(Result);
  AYearPosition := Pos('YYYY', UpperCase(Result));
  if AYearPosition > 0 then
    CutYear(Result, AYearPosition);
end;

class procedure TcxSchedulerDateTimeHelper.UpdateDayMonthFormat;
begin
  if LockedUpdateDayMonthFormat then
    Exit;
  FLongDayMonthFormat := GetLongDayMonthFormat;
  GetDayMonthFormats;
end;
...
begin
  ...
  GetFormatSettings;
  FLongDateOrder := StrToIntDef(GetLocaleStr(GetThreadLocale, LOCALE_ILDATE, '1'), 1);
  UpdateDayMonthFormat;
end;
...
begin
  FSystemStartOfWeek := dxGetStartOfWeek;
end;

class procedure TcxSchedulerDateTimeHelper.InitDayMonthFormats;
begin
  if FDayMonthFormats = nil then
    FDayMonthFormats := TStringList.Create;
end;
...
class procedure TcxSchedulerPainterHelper.DrawState(ACanvas: TcxCanvas; R: TRect; AState: Integer;
  ...
  begin
    ...
    SetBrushOrgEx(ACanvas.Handle, -ACanvas.BaseOrigin.X, -ACanvas.BaseOrigin.Y, @P);
...
begin
  TdxImageDrawer.DrawImage(ACanvas, ARect, nil, AImages, AIndex, True, nil, AScaleFactor);
end;
...
var
  ...
  S_and_L: Double;
begin
  AValue := TdxColorUtils.ColorToHSL(AEventCellColor);
  ...
  Result := TdxColorUtils.HSLToColor(AValue);
end;
...
class function TcxSchedulerPainterHelper.LoadImages(AWidth, AHeight: Integer;
  const AResName: string; AHasPalette: Boolean = False; AHasTransparent: Boolean = True): TcxImageList;
const
  ATransparent: array[Boolean] of DWORD = (CLR_NONE, CLR_DEFAULT);
begin
  Result := TcxImageList.CreateSize(AWidth, AHeight);
  if AHasPalette then
...
class function TcxSchedulerPainterHelper.LoadSvgImages(AWidth, AHeight: Integer; const AResName: string; AHasPalette,
  AHasTransparent: Boolean): TcxImageList;
var
...
begin
  ...
  if dxUseVectorIcons then
  begin
    AResourceStream := TResourceStream.Create(HInstance, AResName, 'SVG');
    try
...
class function TcxSchedulerPainterHelper.TextWidth(AFont: TFont; const AText: string): Integer;
begin
  Result := TdxTextMeasurer.TextWidthTO(AFont, AText);
end;
...
begin
  ...
  Shadow.MarginsScaled := True;

  ShadowRTL := TdxSkinImage.Create(nil);
  ShadowRTL.Assign(Shadow);
  ShadowRTL.Texture.Flip(True, False);

  ModernMoreEvents := TdxSkinImage.Create(nil);
...
begin
  ...
  FStartContinueArrow.Free;
  FEndContinueArrow.Free;
  FreeStateBrushes;
  MoreEventButtonGlyphs.Free;
  FreeAndNil(FMenuImages);
  FreeAndNil(FEventImages);
  FreeAndNil(FTimeLinePatterns);
end;
...
class procedure TcxSchedulerHelpersFactory.FreeCursors;
begin
  dxUnregisterCursor(crSchedulerCopyEvent);
  dxUnregisterCursor(crSchedulerMoveEvent);
  dxUnregisterCursor(crSchedulerHorzSplit);
  dxUnregisterCursor(crSchedulerVertSplit);
  dxUnregisterCursor(crSchedulerSplitAll);
  dxUnregisterCursor(crSchedulerVertResize);
  dxUnregisterCursor(crCalendarMirrorArrow);
end;

class procedure TcxSchedulerHelpersFactory.LoadCursors;
begin
  dxRegisterCursor(crSchedulerCopyEvent, HInstance, 'CXSDCOPYEVENT');
  dxRegisterCursor(crSchedulerMoveEvent, HInstance, 'CXSDMOVEEVENT');
  dxRegisterCursor(crSchedulerHorzSplit, HInstance, 'CXSDHORZSPLIT');
  dxRegisterCursor(crSchedulerVertSplit, HInstance, 'CXSDVERTSPLIT');
  dxRegisterCursor(crSchedulerSplitAll, HInstance, 'CXSDSPLITALL');
  dxRegisterCursor(crSchedulerVertResize, HInstance, 'CXSDVERTRESIZE');
  dxRegisterCursor(crCalendarMirrorArrow, HInstance, 'CXCALENDARMIRRORARROW');
end;
...
function TcxSchedulerObjectList.GetItem(AIndex: TdxListIndex): TObject;
begin
  Result := List[AIndex];
end;
...
function TcxSchedulerDateList.GetItem(AIndex: TdxListIndex): TDateTime;
begin
  Result := Integer(inherited Items[AIndex]);
end;

procedure TcxSchedulerDateList.SetItem(
  AIndex: TdxListIndex; const AValue: TDateTime);
begin
