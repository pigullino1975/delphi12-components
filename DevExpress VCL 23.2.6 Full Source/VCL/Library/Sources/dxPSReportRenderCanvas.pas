...
uses
  System.UITypes,
  Windows, SysUtils, Classes, Graphics, cxClasses, cxGraphics, cxGeometry, dxSmartImage,
  cxDrawTextUtils, dxPSFillPatterns, dxCore, ImgList, dxGDIPlusAPI, dxGDIPlusClasses, dxSVGImage, dxCoreGraphics;
...
type
  ...
  {$REGION 'Private Types'}
    ...
    { TSVGReportImageDrawer }

    TSVGReportImageDrawer = class(TAbstractGraphicDrawer)
    public
      procedure Paint(ACanvas: TCanvas; const ADestRect: TRect); override;
    end;
    ...
    public
      ...
      //
      property Bitmap: TcxBitmap read FBitmap;
    end;
    ...
    protected
      function GetGdipImage: GpImage; virtual;
    public
      procedure Initialize; override;
      procedure Export(AProc: TdxDrawGraphicHelperExportProc); override;
      procedure Paint(ACanvas: TCanvas; const ADestRect: TRect); override;
    ...
    TSvgImageDrawer = class(TSmartImageDrawer)
    strict private
      FRasterImage: TdxGPImageHandle;
    protected
      function GetGdipImage: GpImage; override;
    end;
  ...
  protected
    ...
    function GetActualBackgroundColor: TColor;
    function GetDrawerClass: TAbstractGraphicDrawerClass; virtual;
    function GetDrawerClassForSmartImage(AHandle: TdxSmartImageCustomHandle): TAbstractGraphicDrawerClass; virtual;
  ...
  public
    ...
    procedure Initialize(AGraphic: TGraphic;
      ABackgroundColor, ATransparentColor: TColor; AIsTransparent, APrinting: Boolean; const APalette: IdxColorPalette); overload;
  ...
  public
    ...
    procedure DrawGraphic(AHelper: TdxDrawGraphicHelper; const R: TRect); virtual; abstract; // for internal use
...
uses
  Math, Types, ActiveX, AxCtrls, dxPSUtl, dxPSCore, dxPSEdgePatterns,
  dxTypeHelpers;

const
  dxThisUnitName = 'dxPSReportRenderCanvas';
...
{ TdxDrawGraphicHelper.TSVGReportImageDrawer }

procedure TdxDrawGraphicHelper.TSVGReportImageDrawer.Paint(ACanvas: TCanvas; const ADestRect: TRect);
var
  AOldTransparent: Boolean;
begin
  AOldTransparent := Graphic.Transparent;
...
begin
  ...
  try
    if Printing then
    begin
      if IsWin10v2004OrLater or IsWinServer and IsWin10v1809OrLater then 
        PrintTransparentBitmap(ACanvas, ABitmap, ADestRect, ABitmap.Canvas.Handle)
      else
      begin
      ...
      PaintTransparentBitmap(ACanvas, ABitmap, ADestRect, ABitmap.Canvas.Handle);
  finally
...
var
  ...
  AMaskColor: TColor;
begin
  AMaskColor := Owner.GetActualBackgroundColor;
  ...
  cxStretchBlt(ABitmapDC, ASrcDC, cxRect(ABitmapSize), ASrcRect, SRCCOPY);
  if (ABitmapSize.cx = ADestRect.Width) and (ABitmapSize.cy = ADestRect.Height) then
    cxBitBlt(ADestDC, ABitmapDC, ADestRect, cxNullPoint, SRCAND)
  else
    cxStretchBlt(ADestDC, ABitmapDC, ADestRect, cxRect(ABitmapSize), SRCAND);
...
procedure TdxDrawGraphicHelper.TCustomBitmapDrawer.PaintTransparentBitmap(
  ACanvas: TCanvas; ABitmap: TBitmap; const ADestRect: TRect; AMemoryDC: HDC);
var
  ABitmapClientRect: TRect;
begin
  ABitmapClientRect := cxGetImageClientRect(ABitmap);
  cxStretchBlt(ACanvas.Handle, AMemoryDC, ADestRect, ABitmapClientRect, SRCINVERT);
  Mask(ACanvas.Handle, ADestRect, AMemoryDC, ABitmapClientRect);
  cxStretchBlt(ACanvas.Handle, AMemoryDC, ADestRect, ABitmapClientRect, SRCINVERT);
end;
...
procedure TdxDrawGraphicHelper.TCustomBitmapDrawer.PrintTransparentBitmap(
  ACanvas: TCanvas; ABitmap: TBitmap; const ADestRect: TRect; AMemoryDC: HDC);
var
  ADibHelper: TDIBHelper;
begin
  ...
  try
    ...
    begin
      ...
      Mask(ACanvas.Handle, ADestRect, AMemoryDC, cxGetImageClientRect(ABitmap));
      ADibHelper.Draw(ACanvas, ADestRect, SRCINVERT);
    end
    else
...
const
  PixelFormatMap: array[Boolean] of TPixelFormat = (pf24bit, pf32bit);
begin
  if FBitmap = nil then
  begin
    FBitmap := TcxBitmap.Create;
    FBitmap.HandleType := bmDIB;
    FBitmap.PixelFormat := PixelFormatMap[Printing];
  ...
  FBitmap.Canvas.Brush.Color := Owner.GetActualBackgroundColor;
...
begin
  TdxImageDrawer.DrawImage(Bitmap.Canvas, Bitmap.ClientRect, nil, Owner.ImageList, Owner.ImageIndex, ifmStretch, idmNormal, False, Owner.Palette);
end;
...
begin
  ...
  begin
    if TdxCustomSmartImageAccess(Graphic).Handle is TdxGPImageHandle then
      Exit(TdxGPImageHandle(TdxCustomSmartImageAccess(Graphic).Handle).Handle);
  end;
  Result := nil;
end;
...
{ TdxDrawGraphicHelper.TSvgImageDrawer }

function TdxDrawGraphicHelper.TSvgImageDrawer.GetGdipImage: GpImage;
var
  ASvgImageHandle: TdxSVGImageHandle;
  ASvgImageSize: TSize;
begin
  if FRasterImage = nil then
  begin
    ASvgImageHandle := TdxCustomSmartImageAccess(Graphic).Handle as TdxSVGImageHandle;
    ASvgImageSize := cxSizeScale(ASvgImageHandle.Size, PixelsNumerator, PixelsDenominator);
    FRasterImage := ASvgImageHandle.GetRasterizedImage(ASvgImageSize, nil);
  end;
  Result := FRasterImage.Handle;
end;
...
function TdxDrawGraphicHelper.GetActualBackgroundColor: TColor;
begin
...
begin
  ...
  else if Graphic is TdxSVGReportImage then
    Result := TSVGReportImageDrawer
  else if Graphic is TdxCustomSmartImage then
    Result := GetDrawerClassForSmartImage(TdxCustomSmartImageAccess(Graphic).Handle)
...
function TdxDrawGraphicHelper.GetDrawerClassForSmartImage(AHandle: TdxSmartImageCustomHandle): TAbstractGraphicDrawerClass;
begin
  if AHandle is TdxGPImageHandle then
    Result := TSmartImageDrawer
  else if AHandle is TdxSVGImageHandle then
    Result := TSvgImageDrawer
  else
    Result := TGenericDrawer;
end;
...
procedure TdxDrawGraphicHelper.Initialize(AGraphic: TGraphic;
  ABackgroundColor, ATransparentColor: TColor; AIsTransparent, APrinting: Boolean; const APalette: IdxColorPalette);
begin
...
const
  AlignFlags = CXTO_TOP or CXTO_CENTER_VERTICALLY or CXTO_BOTTOM or CXTO_DISTRIBUTE_VERTICALLY;
var
  R: TRect;
begin
  R := cxRect(0, 0, AWidth, MaxWord);
  FCanvas.Font := Font;
  FCanvas.CalculateTextRect(AText, R, (FFlags and not AlignFlags) or CXTO_CALCRECT, nil, 0, 0, 0, clDefault, ALineSpacing);
