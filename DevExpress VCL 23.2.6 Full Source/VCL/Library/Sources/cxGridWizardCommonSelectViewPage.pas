...
const
  dxThisUnitName = 'cxGridWizardCommonSelectViewPage';
...
const
  GroupNames: array [TcxGridWizardGridViewType] of Pointer = (
    ...
    @scxgwSelectViewPageGroupHeaderEMFViews
  );
...
var
  ...
  AIndex, AMasterViewType: TcxGridWizardGridViewType;
...
begin
  ...
  try
    ...
    if IsDetailViewCustomizing and
      (CustomizedGrid.ActiveLevel <> nil) and (CustomizedGrid.ActiveLevel.GridView <> nil) then
      AMasterViewType :=
        cxGridWizardGetGridViewGroup(TcxCustomGridViewClass(CustomizedGrid.ActiveLevel.GridView.ClassType))
    else
      AMasterViewType := gvtUnbound;
    for AIndex := Low(TcxGridWizardGridViewType) to High(TcxGridWizardGridViewType) do
      AGroups[AIndex].Visible := (AGroups[AIndex].ItemCount > 0) and
        (not IsMultiLevelStructure or (AIndex = gvtDB) or (AIndex = gvtEMF)) and
        ((AMasterViewType = gvtUnbound) or (AIndex = AMasterViewType));
  finally
