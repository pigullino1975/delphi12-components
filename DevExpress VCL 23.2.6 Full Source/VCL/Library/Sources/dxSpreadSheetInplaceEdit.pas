...
type
  ...
  { TcxSpreadSheetRichEditViewData }

  TcxSpreadSheetRichEditViewData = class(TcxCustomRichEditViewData)
  public
    function IsPaddingAllowedForInplace: Boolean; override;
    function IsPaddingAllowedForStandalone: Boolean; override;
  end;

  { TcxSpreadSheetRichEditProperties }

  TcxSpreadSheetRichEditProperties = class(TcxRichEditProperties)
  protected
...
uses
  StrUtils, SysUtils, Math, Graphics, cxGeometry, cxFormats, dxGDIPlusClasses, cxRichEditUtils,
  dxSpreadSheetCore,
...
const
  dxThisUnitName = 'dxSpreadSheetInplaceEdit';
...
var
  ACharPosition: Integer;
begin
  Result := ClientBounds;
  ACharPosition := Result.Left + cxRichEditGetCharPosition(
    InnerRich.Handle, TcxRichInnerEditAccess(InnerRich).RichVersion, SelStart).X;
  if IsRightToLeft then
    Result.Right := ACharPosition
  else
    Result.Left := ACharPosition;
end;
...
begin
  ...
  begin
    AText := GetAdjustedRichEditText(Self);
    ASelStart := SelStart;
...
begin
  ...
  begin
    ...
    if IsRightToLeft then
      AutoCompleteWindow.AlignHorz := pahRight;
...
begin
  ...
  begin
    ...
    ASearchTextLength := Length(FSearchText);
...
begin
  ...
  else if AObject is TdxSpreadSheetDefinedName then
    Result := TdxSpreadSheetDefinedName(AObject).Comment
  else
    Result := '';
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        AHintSize := cxSize(FHintWindow.CalcHintRect(AMonitorRect.Right - ARect.Right, AHint, nil));
...
procedure TdxSpreadSheetCustomInplaceInnerEdit.CMColorChanged(var Message: TMessage);
begin
  if HandleAllocated then
    inherited;
end;

procedure TdxSpreadSheetCustomInplaceInnerEdit.KeyDown(var Key: Word; Shift: TShiftState);
begin
  FIsDecimalSeparator := Key = VK_DECIMAL;
  inherited;
end;

procedure TdxSpreadSheetCustomInplaceInnerEdit.KeyUp(var Key: Word; Shift: TShiftState);
begin
  FIsDecimalSeparator := False;
  inherited;
end;

procedure TdxSpreadSheetCustomInplaceInnerEdit.WMChar(var Message: TWMChar);
begin
  if FIsDecimalSeparator then
    Message.CharCode := Ord(dxFormatSettings.DecimalSeparator);
  inherited;
end;
...
begin
  ...
  begin
    ...
    TdxImageDrawer.DrawUncachedImage(Canvas.Handle, AImageRect, AImageRect, nil,
...
begin
  ...
  cxTextOut(cxScreenCanvas.Handle, AHint, Result, CXTO_CALCRECT or GetDrawTextFlags);
...
begin
  ...
  try
    ...
    cxTextOut(cxPaintCanvas.Handle, Text, ATextRect, GetDrawTextFlags or CXTO_CENTER_VERTICALLY);
  finally
    cxPaintCanvas.EndPaint;
  end;
end;

function TdxSpreadSheetInplaceEditAutoCompleteHintWindow.GetDrawTextFlags: Integer;
begin
...
class function TcxSpreadSheetRichEditProperties.GetViewDataClass: TcxCustomEditViewDataClass;
begin
  Result := TcxSpreadSheetRichEditViewData;
end;

{ TcxSpreadSheetCustomRichEditViewData }

function TcxSpreadSheetRichEditViewData.IsPaddingAllowedForInplace: Boolean;
begin
  Result := False;
end;

function TcxSpreadSheetRichEditViewData.IsPaddingAllowedForStandalone: Boolean;
begin
  Result := False;
end;
