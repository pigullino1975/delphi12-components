...
uses
  System.UITypes,
  Types, Windows, Messages, Classes, Controls, Forms, Dialogs, Graphics, Menus, StdCtrls, SysUtils, Math,
  dxCore, dxCoreClasses, dxMessages, dxThemeManager, dxUxTheme, cxClasses, cxControls, MultiMon,
  dxTouch, cxGraphics, cxLookAndFeels, cxScrollBar, cxLookAndFeelPainters, cxGeometry, dxTypeHelpers, cxAccessibility,
  dxCoreGraphics;

var
  cxDefaultStyleReadOnlyAsDisabled: Boolean = True;  // for internal use
...
type
  ...
  TcxContainerStateItem = (csNormal, csActive, csDisabled, csHotTrack, csReadOnly);
...
type
  ...
  strict protected type
    ...
    TStyleColorPalette = class(TdxCustomColorPalette)
    strict private
      FBackgroundColor: TdxAlphaColor;
      procedure SetBackgroundColor(AValue: TdxAlphaColor);
    public
      constructor Create;
  ...
  strict protected class var
    FStyleColorPalette: TStyleColorPalette;
  private
    FBackgroundColor: TColor;
    function GetBorderState: TcxContainerBorderState;
  protected
    FCalculated: Boolean;
    ...
    procedure GetBackgroundStyleColorPalette([unsafe] var APalette: IdxColorPalette);
    procedure DrawBorder(ACanvas: TcxCanvas; R: TRect); virtual;
    procedure DrawNativeStyleBackground(ACanvas: TcxCanvas); virtual;
    procedure DrawNonNativeStyleBackground(ACanvas: TcxCanvas); virtual;
    function CalculateBackgroundColor(AStyle: TcxContainerStyle; AEnabled: Boolean; AContainer: TcxContainer = nil): TColor; virtual;
    function CalculateBorderColor(AStyle: TcxContainerStyle; AEnabled, AIsDesigning: Boolean; AContainer: TcxContainer = nil): TColor; virtual;
    function CalculateTextColor(AStyle: TcxContainerStyle; AEnabled: Boolean; AContainer: TcxContainer = nil): TColor; virtual;
    function GetPainterBackgroundColor(AContainer: TcxContainer): TColor; virtual;
    function GetContainerBorderStyle: TcxContainerBorderStyle; virtual;
    function GetSkinnedTextColor(AKind: TcxEditStateColorKind): TColor; virtual;
    function IsStyleValueAssigned(AStyle: TcxContainerStyle; AValue: TcxContainerStyleValue): Boolean; inline;
    procedure InternalPaint(ACanvas: TcxCanvas); virtual;
    function ShouldUseStyleBackgroundColor(AStyle: TcxContainerStyle): Boolean; virtual;
  public
    BorderColor: TColor;
    BorderRect: TRect;
    ...
    UseBorderedBackground: Boolean;
    UseSkinnedColors: Boolean;
    UseSkins: Boolean;


    StyleColorAssigned: Boolean; // for internal use
    UseStyleBackgroundColor: Boolean; // for internal use
    ...
    property BorderState: TcxContainerBorderState read GetBorderState;
  ...
  private
    ...
    function InternalGetReadOnly(var AReadOnly: Boolean): Boolean;
    ...
    procedure InternalSetFont(AFont: TFont);
  ...
  protected
    ...
    function UsePainterBorderSettings: Boolean; virtual;
    procedure UpdateFont;

    property ParentStyle: TcxContainerStyle read FParentStyle;
    property OnChanged: TNotifyEvent read FOnChanged write FOnChanged;
  public
    StyleData: TcxContainerStyleData; // for internal use
    function IsColorAssigned: Boolean; // for internal use
  ...
  private
    ...
    function GetStyleReadOnly: TcxContainerStyle;
    ...
    procedure SetStyleReadOnly(Value: TcxContainerStyle);
  public
    ...
    property StyleReadOnly: TcxContainerStyle read GetStyleReadOnly write SetStyleReadOnly;
  ...
  private
    class var FFormInactivationLocked: Boolean;
  private
    ...
    FOnActiveStyleChanged: TNotifyEvent;
    ...
    function GetStyleReadOnly: TcxContainerStyle;
    function GetVisibleFont: TFont;
    function GetVisibleFontColor: TColor;
    procedure SetFakeStyleController(Value: TcxStyleController);
    procedure SetInnerControl(Value: TWinControl);
    procedure SetOnGlass(AValue: Boolean);
  ...
  protected
    ...
    function GetDefaultStyleReadOnlyAsDisabled: Boolean;
    function GetContainerState(const P: TPoint; AMouseTracking: Boolean): TcxContainerState; virtual;
    function GetDefaultContainerState(const P: TPoint; AMouseTracking: Boolean): TcxContainerState;
    ...
    procedure AdjustInnerControlScrollBarBounds(AScrollBarKind: TScrollBarKind; var ABounds: TRect); virtual;
    procedure AdjustScrollBarPosition(const AScrollBar: IcxControlScrollBar); virtual;
    ...
    procedure ShortRefreshContainerOnWindowPosChanging; virtual;
    ...
    function GetDefaultSkinnedTextColor(AEnabled: Boolean): TColor; virtual;
    function GetEditStateColorKind: TcxEditStateColorKind; virtual;
    function GetInnerControlBounds(const AInnerControlsRegion: TRect; AInnerControl: TControl): TcxContainerInnerControlBounds; virtual;
    ...
    function GetDefaultActiveStyle: TcxContainerStyle;
    ...
    function HandleInnerControlGestures: Boolean; virtual;
    ...
    function NeedUpdateWindowRegion: Boolean; virtual;
    ...
    function UseBorderedBackground: Boolean; virtual;
    ...
    property VisibleFontColor: TColor read GetVisibleFontColor;
    property OnActiveStyleChanged: TNotifyEvent read FOnActiveStyleChanged write FOnActiveStyleChanged;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    class constructor Create;
    class procedure LockFormInactivation(AValue: Boolean);
  ...
  {$IFNDEF VCLGLASSPAINT}
    property OnGlass: Boolean read GetOnGlass write SetOnGlass;
  {$ENDIF}
  ...
  private
    ...
    procedure SetLookAndFeel(Value: TcxLookAndFeel);
    procedure ValidateItemBitmapSize(const AItemSize: TSize);
    procedure WMEraseBkgnd(var Message: TWMEraseBkgnd); message WM_ERASEBKGND;
  ...
  protected
    ...
    FBackgroundPadding: TdxPadding;
    FItemPadding: TdxPadding;
    // IdxVisualRefinementsListener
  ...
  {$ENDIF}
    ...
    procedure BufferedDrawItem(DC: HDC; AIndex: Integer; ARect: TRect; AState: TOwnerDrawState); virtual;
    procedure CalculatePadding; virtual;
    procedure DrawItem(Index: Integer; Rect: TRect; State: TOwnerDrawState); override;
    procedure DrawItemContent(AIndex: Integer; ARect: TRect; AState: TOwnerDrawState); virtual;
    procedure DrawItemFocusRect(const ABounds: TRect); virtual;
    procedure DrawItemText(const AText: string; ARect: TRect; AState: TOwnerDrawState); virtual;
    procedure DrawItemBackground(AIndex: Integer; const ARect: TRect; AState: TOwnerDrawState); virtual;
    function GetDefaultItemPadding: TdxPadding; virtual;
    function GetItemContentBounds(const ARect: TRect): TRect; virtual;
    function GetItemFocusRectBounds(const ARect: TRect): TRect; virtual;
    function GetItemNonTextWidth(AIndex: Integer): Integer; virtual;
    function IsBufferedItemPaint: Boolean; virtual;
    function SupportsListBoxSkinPadding: Boolean; virtual; 
    procedure UpdatePaintCache; virtual;

    procedure PrepareDrawItemCanvas(ACanvas: TCanvas; AIndex: Integer; AState: TOwnerDrawState);

    procedure DoAutoComplete(var Key: Char); virtual;
    function DoCustomDrawItem(AIndex: Integer; const ARect: TRect; AState: TOwnerDrawState): Boolean; virtual;
    ...
    function GetItem(AIndex: Integer): string; virtual;
    function GetPopupMenu: TPopupMenu; override;
    function GetRealStyle: TListBoxStyle; virtual;
    function GetRootContainer: TcxContainer; virtual;
    function IsEmbedded: Boolean; virtual;
    procedure KeyDown(var Key: Word; Shift: TShiftState); override;
    ...
    function SpecialProcessMouseClick(var AMessage: TWMMouse): Boolean; virtual;
    procedure UpdateControl;
    procedure UpdateItemsLayout; virtual;
    function HasContentVerticalPadding: Boolean; virtual;
    function UseCustomPadding: Boolean; virtual;
    procedure WndProc(var Message: TMessage); override;
  ...
  { TdxCustomInnerListBoxAccessibilityHelper }

  TdxCustomInnerListBoxAccessibilityHelper = class(TcxAccessibilityHelper) // for internal use
  strict private
    FScrollItemProvider: TdxScrollItemProvider;

    function GetListBox: TcxCustomInnerListBox;
  protected
    ...
    property ListBox: TcxCustomInnerListBox read GetListBox;
  public
  ...
  TdxCustomInnerListBoxAccessibilityHelperClass = class of TdxCustomInnerListBoxAccessibilityHelper; // for internal use
...
function ExecuteEmbeddedDialog(ADialog: TCommonDialog; AParentWnd: THandle): Boolean;
...
begin
  ...
  if Result and dxIsProcessDPIPerMonitorV2Aware then
  begin
...
const
  BorderStyle: array [TcxLookAndFeelKind] of TcxContainerBorderStyle = (cbsFlat, cbs3D, cbsUltraFlat, cbsOffice11);
begin
...
function IsMouseDownMessage(AMsg: WPARAM): Boolean;
begin
  Result := cxControls.IsMouseDownMessage(AMsg);
end;
...
begin
  ...
  try
    ...
    Result := ADialog.Execute(AParentWnd);
...
{ TcxContainerViewInfo.TStyleColorPalette }

constructor TcxContainerViewInfo.TStyleColorPalette.Create;
begin
  inherited Create;
  _AddRef;
end;

function TcxContainerViewInfo.TStyleColorPalette.GetFillColor(const ID: string): TdxAlphaColor;
begin
  Result := FBackgroundColor;
end;

function TcxContainerViewInfo.TStyleColorPalette.GetStrokeColor(const ID: string): TdxAlphaColor;
begin
  Result := TdxAlphaColors.Empty;
end;

procedure TcxContainerViewInfo.TStyleColorPalette.SetBackgroundColor(AValue: TdxAlphaColor);
begin
...
class constructor TcxContainerViewInfo.Create;
begin
  FStyleColorPalette := TStyleColorPalette.Create;
end;

class destructor TcxContainerViewInfo.Destroy;
begin
  FStyleColorPalette._Release;
  FStyleColorPalette := nil;
end;
...
begin
  if UseBorderedBackground then
    Painter.DrawContainerBorderedBackground(ACanvas, R, BorderStyle, BorderWidth, BorderColor, Edges, BorderState)
  else
...
begin
  Font := AStyle.Font;
  ALookAndFeel := AStyle.LookAndFeel;
  Painter := ALookAndFeel.Painter;
  UseSkins := ALookAndFeel.SkinPainter <> nil;
  UseSkinnedColors := UseSkins;
end;

function TcxContainerViewInfo.ShouldUseStyleBackgroundColor(AStyle: TcxContainerStyle): Boolean;
begin
  Result := False;
end;

procedure TcxContainerViewInfo.GetBackgroundStyleColorPalette([unsafe] var APalette: IdxColorPalette);
begin
  if UseStyleBackgroundColor then
  begin
    FStyleColorPalette.BackgroundColor := TdxAlphaColors.FromColor(BackgroundColor);
    APalette := FStyleColorPalette;
  end
  else
    APalette := nil;
end;

function TcxContainerViewInfo.GetPainterBackgroundColor(AContainer: TcxContainer): TColor;
begin
  Result := Painter.DefaultEditorBackgroundColorEx(AContainer.GetEditStateColorKind);
end;

function TcxContainerViewInfo.CalculateBackgroundColor(
  AStyle: TcxContainerStyle; AEnabled: Boolean; AContainer: TcxContainer = nil): TColor;
begin
  Result := clDefault;
  if UseSkinnedColors and (AContainer <> nil) then
  begin
    if not (AContainer.ParentColor or IsStyleValueAssigned(AStyle, csvColor)) then
      Result := GetPainterBackgroundColor(AContainer);
  end;

  if Result = clDefault then
  begin
    if NativeStyle and not (AEnabled or AStyle.IsValueAssigned(csvColor)) then
      Result := cxGetThemeColor(ThemedObjectType, NativePart, NativeState, TMT_FILLCOLOR);
  end;
...
function TcxContainerViewInfo.CalculateBorderColor(
  AStyle: TcxContainerStyle; AEnabled, AIsDesigning: Boolean; AContainer: TcxContainer = nil): TColor;
var
  AIsHighlightBorder: Boolean;
begin
  AIsHighlightBorder :=
    (csActive in ContainerState) or
    (csHotTrack in ContainerState) and AStyle.HotTrack or
    (AEnabled and AIsDesigning);

  if IsStyleValueAssigned(AStyle, csvBorderColor) then
    Result := AStyle.BorderColor
  else
    if UseSkinnedColors then
      Result := Painter.GetContainerBorderColor(AIsHighlightBorder)
    else
      Result := clDefault;

  if Result = clDefault then
  begin
    if AStyle.BorderStyle in [cbsUltraFlat, cbsOffice11] then
      Result := GetContainerBorderColor(AIsHighlightBorder, AStyle.BorderStyle = cbsOffice11)
    else
      Result := AStyle.BorderColor;
  end;
end;

function TcxContainerViewInfo.CalculateTextColor(AStyle: TcxContainerStyle; AEnabled: Boolean; AContainer: TcxContainer): TColor;
begin
  if UseSkinnedColors and (AContainer <> nil) and not IsStyleValueAssigned(AStyle, csvTextColor) then
  begin
    Result := AContainer.GetDefaultSkinnedTextColor(AEnabled);
    if Result = clDefault then
      Result := GetSkinnedTextColor(AContainer.GetEditStateColorKind);
  end
  else
    Result := AStyle.TextColor;
end;

function TcxContainerViewInfo.GetBorderState: TcxContainerBorderState;
begin
  if csDisabled in ContainerState then
    Result := cbsDisabled
  else
  begin
    Result := cbsNormal;
    if csActive in ContainerState then
      Result := cbsFocused;
    if csHotTrack in ContainerState then
      Result := TcxContainerBorderState(Ord(Result) + Ord(cbsHovered));
  end;
end;
...
function TcxContainerViewInfo.GetSkinnedTextColor(AKind: TcxEditStateColorKind): TColor;
begin
  Result := Painter.DefaultEditorTextColorEx(AKind);
end;

function TcxContainerViewInfo.IsStyleValueAssigned(AStyle: TcxContainerStyle; AValue: TcxContainerStyleValue): Boolean;
begin
  Result := AStyle.IsValueAssigned(AValue);
  if Result then
    Exit;

  if ((AStyle.State = TcxContainerStateItem.csDisabled) and not (AValue in [csvBorderColor, csvColor, csvTextColor])) or
    (AStyle.State <> TcxContainerStateItem.csDisabled) then
      Result := (AStyle.ParentStyle <> nil) and AStyle.ParentStyle.IsValueAssigned(AValue); 

  if not Result and (AStyle.State = TcxContainerStateItem.csNormal) and (AValue = csvColor) then
    Result := AStyle.IsColorAssigned;
end;

procedure TcxContainerViewInfo.DrawNativeStyleBackground(ACanvas: TcxCanvas);
...
begin
  ...
  begin
    ...
    cxGetThemeColor(AThemedObjectType, APart, AState, TMT_BORDERCOLOR, AColor);
    InternalFillRect(ACanvas, Bounds, R, AColor);
  end;
end;

procedure TcxContainerViewInfo.DrawNonNativeStyleBackground(ACanvas: TcxCanvas);
var
  R: TRect;
begin
  R := BorderRect;
...
begin
  if NativeStyle then
    DrawNativeStyleBackground(ACanvas)
  else
    DrawNonNativeStyleBackground(ACanvas);
end;
...
function TcxContainerStyles.GetStyleReadOnly: TcxContainerStyle;
begin
  Result := FStyles[csReadOnly];
end;
...
procedure TcxContainerStyles.SetStyleReadOnly(Value: TcxContainerStyle);
begin
  FStyles[csReadOnly].Assign(Value);
end;
...
begin
  FIsDestroying := True;
  if (FParentStyle = nil) and (ActiveStyleController <> nil) then
    ActiveStyleController.RemoveListener(Self);
  FreeAndNil(FLookAndFeel);
  FreeAndNil(FVisibleFont);
  if not DirectAccessMode and (FParentStyle = nil) then
...
begin
  Result := (ParentStyle = nil) or ParentStyle.HasBorder;
end;
...
begin
  ...
  case AValue of
    ...
    csvReadOnly:
      Result := InternalGetReadOnly(ATempBool);
    csvShadow:
...
function TcxContainerStyle.DefaultReadOnly: Boolean;
begin
  Result := False;
end;
...
function TcxContainerStyle.IsColorAssigned: Boolean;
var
  AContainer: TcxContainer;
  AColor: TColor;
begin
  if DirectAccessMode then 
    Exit(False);
  Result := csvColor in AssignedValues;
  if Result then
    Exit;
  AContainer := Container;
  if AContainer <> nil then
  begin
    if AContainer.IsInplace then
      Exit(False);
    if (State = csNormal) and (ParentStyle = nil) then
    begin
      if AContainer.ParentColor and (AContainer.Parent <> nil) then
        Exit(True);
    end;
  end;
  Result := InternalGetColor(AColor);
  if not Result and (State <> csDisabled) and (ParentStyle <> nil) then
    Result := ParentStyle.IsColorAssigned;
end;
...
begin
  if FDirectAccessMode then
    Exit(StyleData.Color);
  if InternalGetColor(Result) then
    Exit;

  if ParentStyle = nil then
  begin
    AContainer := Container;
    if (AContainer <> nil) and not AContainer.IsInplace and AContainer.ParentColor and (AContainer.Parent <> nil) then
      Exit(TWinControlAccess(AContainer.Parent).Color);
  end;
...
begin
  Result := [csvEdges, csvFont, csvHotTrack, csvShadow, csvTransparentBorder, csvReadOnly];
end;
...
begin
  ...
  try
    ...
    begin
      ...
      if (Container <> nil) and Container.ParentFont then
        StyleData.Font.Assign(Container.Font)
      else
        RestoreFont(StyleData.Font);
  finally
...
function TcxContainerStyle.GetReadOnly: Boolean;
begin
  if DirectAccessMode then
  begin
    if csvReadOnly in FAssignedValues then
      Result := FReadOnly
    else
    ...
    if ParentStyle <> nil then
      Result := ParentStyle.ReadOnly
    else
      if not InternalGetReadOnly(Result) then
        Result := DefaultReadOnly;
end;
...
begin
  if UsePainterBorderSettings and (LookAndFeel.Painter <> nil) and
    not LookAndFeel.Painter.SupportsTransparentBorder then
    Exit(False);
...
function TcxContainerStyle.InternalGetReadOnly(var AReadOnly: Boolean): Boolean;
begin
  Result := csvReadOnly in FAssignedValues;
  if Result then
    AReadOnly := FReadOnly
  else
    if ActiveStyleController <> nil then
      Result := ActiveStyleController.Styles[State].InternalGetReadOnly(AReadOnly);
end;
...
procedure TcxContainerStyle.InternalSetFont(AFont: TFont);
begin
  LockFontAssignedValue(True);
  try
    Font := AFont;
...
function TcxContainerStyle.IsReadOnlyStored: Boolean;
begin
  Result := (csvReadOnly in FAssignedValues) and ((Container = nil) or
    Container.IsStylePropertyPublished('ReadOnly', State <> csNormal));
end;
...
procedure TcxContainerStyle.SetReadOnly(Value: Boolean);
begin
  if ParentStyle <> nil then
    ParentStyle.ReadOnly := Value
  else
  begin
    if (csvReadOnly in FAssignedValues) and (Value = FReadOnly) then
      Exit;
    FReadOnly := Value;
    Include(FAssignedValues, csvReadOnly);
    Changed;
  end;
end;
...
begin
  if not DirectAccessMode and (ParentStyle = nil) then
  begin
    StyleData.Font := dxCreateFontForDefaultDPI;
...
function TcxContainerStyle.UsePainterBorderSettings: Boolean;
begin
  Result := True;
end;
...
begin
  cxResetFont(AFont, dxGetScaleFactor(Owner));
end;
...
class constructor TcxContainer.Create;
begin
  FFormInactivationLocked := False;
end;

class procedure TcxContainer.LockFormInactivation(AValue: Boolean);
begin
  FFormInactivationLocked := AValue;
end;
...
procedure TcxContainer.ChangeScaleEx(M, D: Integer; isDpiChange: Boolean);
  ...
  begin
    Result := (M <> D) and (csvFont in Style.AssignedValues);
  end;
...
begin
  ...
  try
    if NeedFontScaling then
      dxScaleFont(Style.Font, M, D)
    else
      Style.UpdateFont;
  finally
...
function TcxContainer.GetContainerState(const P: TPoint; AMouseTracking: Boolean): TcxContainerState;
begin
  Result := GetDefaultContainerState(P, AMouseTracking);
end;

function TcxContainer.GetDefaultStyleReadOnlyAsDisabled: Boolean;
begin
  Result := cxDefaultStyleReadOnlyAsDisabled;
end;

function TcxContainer.GetDefaultContainerState(const P: TPoint; AMouseTracking: Boolean): TcxContainerState;
var
  AIsReadOnly: Boolean;
begin
  if Enabled then
  begin
    AIsReadOnly := Style.ReadOnly and IsReadOnly;
    if IsDesigning then
    begin
      if AIsReadOnly then
        Result := [csReadOnly]
      ...
      if GetDefaultStyleReadOnlyAsDisabled then
      begin
        if AIsReadOnly then
          Result := [csReadOnly]
        else
...
begin
  Result := inherited IsGestureTarget(AWnd) or
    HandleInnerControlGestures and (InnerControl.Handle = AWnd);
end;
...
begin
  if CanDisableAlignOnCreateInnerControl and TcxControlHelper.CanSetParent(Self, AParent) then
  begin
    DisableAlign; 
...
procedure TcxContainer.DestroyHandle;

  function GetFocusingParent: TWinControl;
  var
    AParent: TWinControl;
  begin
    AParent := Parent;
    ...
    begin
      if csFocusing in AParent.ControlState then
        Exit(AParent);
...
begin
  if (InnerControl <> nil) and InnerControl.Focused and (GetFocusingParent <> nil) then
    Windows.SetFocus(GetFocusingParent.Handle); 
  inherited;
end;
...
function TcxContainer.UseBorderedBackground: Boolean;
begin
  Result := True;
end;
...
procedure TcxContainer.CalculateViewInfo(const P: TPoint; AMouseTracking: Boolean);
  ...
  var
    ASelected: Boolean;
  begin
    ViewInfo.ContainerState := GetContainerState(P, AMouseTracking);
...
begin
  ...
  if not ViewInfo.NativeStyle and (GetContainerBorderWidth(ViewInfo.BorderStyle) < APrevBorderWidth) then
    CalculateContainerState;
  ...
  ViewInfo.Edges := AStyle.Edges;
  ViewInfo.Shadow := HasShadow;
  ViewInfo.BorderColor := ViewInfo.CalculateBorderColor(AStyle, Enabled, IsDesigning, Self);
  ViewInfo.BackgroundColor := ViewInfo.CalculateBackgroundColor(AStyle, Enabled, Self);
  ViewInfo.UseBorderedBackground := UseBorderedBackground;
  ViewInfo.StyleColorAssigned := AStyle.IsColorAssigned;
  ViewInfo.UseStyleBackgroundColor := ViewInfo.ShouldUseStyleBackgroundColor(AStyle);
end;
...
{$IFNDEF VCLGLASSPAINT}
  case Message.Msg of
    ...
    DXM_BUFFEREDPAINTONGLASS:
      ...
      try
        dxDrawWindowOnGlass(InnerControl.Handle);
      finally
        FInnerControlBufferedPaint := False;
      end;
  end;
...
procedure TcxContainer.ShortRefreshContainerOnWindowPosChanging;
begin
  if not FInternalWindowRegionSetting then
    ShortRefreshContainer(False);
end;
...
begin
  ...
  begin
    ...
    Result := FActiveStyleData.ActiveStyle;
    if Assigned(FOnActiveStyleChanged) then
      FOnActiveStyleChanged(Self);
  end;
end;
...
function TcxContainer.GetDefaultSkinnedTextColor(AEnabled: Boolean): TColor;
begin
  Result := clDefault;
end;
...
begin
  Result := GetDefaultActiveStyle;
end;

function TcxContainer.GetDefaultActiveStyle: TcxContainerStyle;
begin
  ...
  else if Style.ReadOnly and (csReadOnly in ViewInfo.ContainerState) then
    Result := FStyles.StyleReadOnly
  else
    Result := FStyles.Style;
end;
...
function TcxContainer.NeedUpdateWindowRegion: Boolean;
begin
...
begin
  if HandleAllocated and (Width > 0) and (Height > 0) and not FInternalWindowRegionSetting and NeedUpdateWindowRegion then
  begin
    ...
    begin
      ...
      if AVScrollbarVisible then
        ...
        if InnerControl is TcxCustomInnerListBox then
          if UseRightToLeftScrollBar then
            Result.Left := Result.Left - cxScrollBar.GetScrollBarSize.cx
          else
            Result.Right := Result.Right + cxScrollBar.GetScrollBarSize.cx;
...
begin
  Inc(FRefreshLockCount);
  try
    if not IsDestroying and HasInnerControlHandle then
    begin
      AVScrollbarVisible := IsScrollBarVisible(sbVertical, AScrollbarInfo);
      AHScrollbarVisible := IsScrollBarVisible(sbHorizontal, AScrollbarInfo);
      DoSetSize;
      if (AVScrollbarVisible <> IsScrollBarVisible(sbVertical, AScrollbarInfo)) or
         (AHScrollbarVisible <> IsScrollBarVisible(sbHorizontal, AScrollbarInfo)) then
      begin
        ViewInfo.Bounds := GetControlRect(Self);
        ViewInfo.UseBorderedBackground := UseBorderedBackground;
        ViewInfo.BorderRect := cxRectContent(ViewInfo.Bounds, GetBorderExtent);
...
procedure TcxContainer.AdjustInnerControlScrollBarBounds(AScrollBarKind: TScrollBarKind; var ABounds: TRect);
begin
end;

procedure TcxContainer.AdjustScrollBarPosition(const AScrollBar: IcxControlScrollBar);
var
...
begin
  ...
  begin
    ...
    R := cxRectBounds(R.Left, R.Top, R.Right, R.Bottom);
...
function TcxContainer.HandleInnerControlGestures: Boolean;
begin
  Result := False;
end;
...
function TcxContainer.GetOnGlass: Boolean;
begin
...
function TcxContainer.GetStyleReadOnly: TcxContainerStyle;
begin
  Result := FStyles.StyleReadOnly;
end;
...
begin
  ...
  Result.Color := VisibleFontColor;
end;

function TcxContainer.GetVisibleFontColor: TColor;
begin
  Result := ViewInfo.CalculateTextColor(ActiveStyle, Enabled, Self);
end;
...
procedure TcxContainer.SetOnGlass(AValue: Boolean);
begin
  if AValue then
...
procedure TcxContainer.SetStyleReadOnly(Value: TcxContainerStyle);
begin
...
begin
  ...
  if not IsDestroying then
    ShortRefreshContainerOnWindowPosChanging;
end;
...
begin
  ...
  {$IFDEF DELPHI110}
    if ParentFont then
      CMParentFontChanged(Message) 
    else
  {$ENDIF}
    begin
      Style.InternalSetFont(Font);
      inherited;
    end;
  end
  else
...
begin
  ...
  begin
    ...
    begin
    {$IFDEF DELPHI120} 
      if TCustomFormAccess(AParentForm).FormStyle <> fsMDIChild then
    {$ENDIF}
...
begin
  ...
  begin
    ...
    begin
      ...
      if FIsTopMost and ((AParentForm = nil) or (GetWindowLong(AParentForm.Handle, GWL_EXSTYLE) and WS_EX_TOPMOST <> 0)) then
...
procedure TcxCustomPopupWindow.CreateWindowHandle(
  const Params: TCreateParams);

  // Forms.TCustomForm.SetLayeredAttribs
  procedure SetLayeredAttributes;
  const
    AlphaMap: array [Boolean] of Integer = (0, LWA_ALPHA);
    ColorKeyMap: array [Boolean] of Integer = (0, LWA_COLORKEY);
  var
    AStyle: NativeInt;
  begin
    ...
    begin
      ...
      begin
        ...
        SetLayeredWindowAttributes(Handle, ColorToRGB(TransparentColorValue), AlphaBlendValue,
          AlphaMap[AlphaBlend] or ColorKeyMap[TransparentColor]);
      end
      else
      begin
  ...
  function CreateWindow(const P: TCreateParams): HWND;
  begin
    Result := CreateWindowEx(P.ExStyle, P.WinClassName, P.Caption, P.Style,
      P.X, P.Y, P.Width, P.Height, P.WndParent, 0, P.WindowClass.hInstance, P.Param);
  end;
...
begin
  AParams := Params;
  AParams.ExStyle := AParams.ExStyle and not WS_EX_LAYERED;
  WindowHandle := CreateWindow(AParams);
  if (WindowHandle = 0) and (GetLastError = 5) and (AParams.WndParent = Windows.GetDesktopWindow) then 
  begin
    AParams.WndParent := FSavedWndParent;
    CreationControl := Self;
    WindowHandle := CreateWindow(AParams);
    if WindowHandle <> 0 then
      Windows.SetParent(Handle, Windows.GetDesktopWindow);
  end;
...
begin
  if dxTryGetScaleFactorForControl(OwnerControl, AScaleFactor) then
  begin
    if Adjustable then
    begin
      ABounds := BoundsRect;
    ...
    ForceScaleForPPI(AScaleFactor.Apply(dxDefaultDPI), ATargetBounds);
  end;
end;
...
begin
  ...
  BorderStyle := bsNone;
  ControlStyle := ControlStyle + [csDoubleClicks, csOverrideStylePaint] - [csOpaque];
  ParentColor := False;
  ParentFont := True;
  TdxVisualRefinements.AddListener(Self);
  FItemBitmap := TcxBitmap.Create;
end;
...
begin
  ...
  FreeAndNil(FLookAndFeel);
  FreeAndNil(FCanvas);
  FreeAndNil(FItemBitmap);
  inherited Destroy;
end;

function TcxCustomInnerListBox.GetPainter: TcxCustomLookAndFeelPainter;
begin
...
function TcxCustomInnerListBox.GetItemWidth(AIndex: Integer): Integer;
begin
  Result := TdxTextMeasurer.TextSizeDT(Font, GetItem(AIndex)).cx + GetItemNonTextWidth(AIndex);
end;

function TcxCustomInnerListBox.GetItemNonTextWidth(AIndex: Integer): Integer;
begin
  if UseCustomPadding then
    Result := FItemPadding.Width + FBackgroundPadding.Width
  else
    Result := 0;
end;
...
begin
  if (csDestroying in ComponentState) or not UsecxScrollBars or
...
procedure TcxCustomInnerListBox.VisualRefinementsChanged;
begin
  UpdateControl;
end;
...
begin
  ...
  begin
    ...
    if not dxCharInSet(Key, [dxVKReturn, dxVKBack, dxVKEscape]) then
      Key := #0;
  end
  else
  begin
...
function TcxCustomInnerListBox.DoCustomDrawItem(AIndex: Integer; const ARect: TRect; AState: TOwnerDrawState): Boolean;
begin
...
function TcxCustomInnerListBox.GetDefaultItemHeight: Integer;
begin
...
function TcxCustomInnerListBox.GetDrawTextFlags: Cardinal;
begin
  Result := DrawTextBiDiModeFlags(DT_SINGLELINE or DT_VCENTER or DT_NOPREFIX or DT_NOCLIP);
end;

function TcxCustomInnerListBox.GetItem(AIndex: Integer): string;
begin
...
function TcxCustomInnerListBox.GetRealStyle: TListBoxStyle;
begin
  if (Style in [lbStandard, lbVirtual]) and (UseCustomPadding or (LookAndFeel.SkinPainter <> nil)) then
...
function TcxCustomInnerListBox.IsEmbedded: Boolean;
begin
  Result := False;
end;
...
begin
  Result := not Assigned(OnDrawItem) and Painter.SupportsNativeFocusRect(TdxPaintPartID.ListBox) and
    Focused
//    and MultiSelect;
end;
...
begin
  ...
  if IsBufferedItemPaint and (AScrollBarKind = sbHorizontal) then
    Invalidate;
end;

procedure TcxCustomInnerListBox.UpdateControl;
begin
  if HandleAllocated then
  begin
    CalculatePadding;
    if FRealStyle <> GetRealStyle then
      RecreateWnd
    else
      UpdateItemsLayout;
  end;
...
procedure TcxCustomInnerListBox.UpdateItemsLayout;
begin
  if FRealStyle = lbOwnerDrawFixed then 
    SendMessage(Handle, LB_SETITEMHEIGHT, 0, GetDefaultItemHeight);
  Invalidate;
end;

function TcxCustomInnerListBox.HasContentVerticalPadding: Boolean;
begin
  Result := (FBackgroundPadding.Top <> 0) or (FBackgroundPadding.Bottom <> 0);
end;

function TcxCustomInnerListBox.UseCustomPadding: Boolean;
begin
  Result := SupportsListBoxSkinPadding or
    (TdxVisualRefinements.UsePaddingForStandaloneEditors and not TdxVisualRefinements.Padding.IsZero);
end;
...
procedure TcxCustomInnerListBox.ValidateItemBitmapSize(const AItemSize: TSize);
begin
  if (FItemBitmap.Width < AItemSize.cx) or (FItemBitmap.Height < AItemSize.cy) then
    FItemBitmap.SetSize(AItemSize.cx, AItemSize.cy);
end;

procedure TcxCustomInnerListBox.EraseBackground(DC: HDC);
var
...
begin
  ...
  begin
    ...
    FillRectByColor(DC, R, Brush.Color);
  end;
end;

procedure TcxCustomInnerListBox.WMEraseBkgnd(var Message: TWMEraseBkgnd);
begin
  if not IsWin11OrLater and (FRealStyle = lbOwnerDrawFixed) and (Columns = 0) then
  begin
...
procedure TcxCustomInnerListBox.DoMouseDown(var Message: TWMMouse; Button: TMouseButton;
  Shift: TShiftState);
begin
...
function TcxCustomInnerListBox.SpecialProcessMouseClick(var AMessage: TWMMouse): Boolean;
begin
  Result := ItemAtPos(TPoint.Create(AMessage.XPos, AMessage.YPos), True) < 0;
  if Result then
  begin
    SendCancelMode(Self);
    ...
    begin
      ...
      DoMouseDown(AMessage, mbLeft, []);
    end
    else
    begin
      if csClickEvents in ControlStyle then
        DblClick;
      DoMouseDown(AMessage, mbLeft, [ssDouble]);
    end;
    AMessage.Result := 0;
  end
end;

procedure TcxCustomInnerListBox.WMLButtonDblClk(var Message: TWMLButtonDblClk);
begin
  if not SpecialProcessMouseClick(Message) then
    inherited;
end;
...
begin
  if (Container <> nil) and Container.IsPopupScrollBars then  
    Exit;
  inherited;
end;
...
procedure TcxCustomInnerListBox.CMFontChanged(var Message: TMessage);
begin
  inherited;
  UpdateItemsLayout;
end;
...
procedure TcxCustomInnerListBox.PrepareDrawItemCanvas(ACanvas: TCanvas; AIndex: Integer; AState: TOwnerDrawState);
begin
  ACanvas.Font := Font;
  ACanvas.Brush := Brush;
  if (AIndex >= 0) and (odSelected in AState) then
  begin
    ACanvas.Brush.Color := Painter.DefaultSelectionColor;
    if Painter.UseDefaultListBoxSelectionTextColor then
      ACanvas.Font.Color := Painter.DefaultListBoxSelectionTextColor;
  end;
end;
...
begin
  ...
  begin
    ...
    if IsBufferedItemPaint then
      BufferedDrawItem(hDC, Integer(itemID), rcItem, AItemState)
    else
    begin
      ACanvas := inherited Canvas;
      ACanvas.Handle := hDC;
      PrepareDrawItemCanvas(ACanvas, Integer(itemID), AItemState);
...
procedure TcxCustomInnerListBox.BufferedDrawItem(DC: HDC; AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
var
...
begin
  ...
  begin
    if FBackgroundPadding.Left <> 0 then
    begin
      R := ARect;
      R.Right := FBackgroundPadding.Left;
      FillRectByColor(DC, R, Brush.Color);
    end;
    if FBackgroundPadding.Right <> 0 then
    begin
      R := ARect;
      R.Left := R.Right - FBackgroundPadding.Right;
      FillRectByColor(DC, R, Brush.Color);
    end;
    Inc(ARect.Left, FBackgroundPadding.Left);
    Dec(ARect.Right, FBackgroundPadding.Right);
  end
  else
  begin
    if FBackgroundPadding.Right <> 0 then
    begin
      R := ARect;
      R.Right := FBackgroundPadding.Right;
      FillRectByColor(DC, R, Brush.Color);
    end;
    if FBackgroundPadding.Left <> 0 then
    begin
      R := ARect;
      R.Left := R.Right - FBackgroundPadding.Left;
      FillRectByColor(DC, R, Brush.Color);
    end;

    Inc(ARect.Left, FBackgroundPadding.Right);
    Dec(ARect.Right, FBackgroundPadding.Left);
  end;

  ValidateItemBitmapSize(ARect.Size);
  ASaveCanvas := FCanvas;
  FCanvas := FItemBitmap.cxCanvas;
  Canvas.Lock;
  try
    Canvas.WindowOrg := ARect.TopLeft;
    Canvas.FillRect(ARect, Brush.Color);
    PrepareDrawItemCanvas(Canvas.Canvas, AIndex, AState);
    if (AIndex >= 0) and (AIndex < Items.Count) then
      DrawItem(AIndex, ARect, AState);
    Canvas.WindowOrg := TPoint.Null;
    cxBitBlt(DC, Canvas.Handle, ARect, TPoint.Null, SRCCOPY);
  finally
    Canvas.Unlock;
    FCanvas := ASaveCanvas;
  end;
...
procedure TcxCustomInnerListBox.DrawItemContent(AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
begin
  Canvas.Brush.Style := bsClear;
  DrawItemText(GetItem(AIndex), ARect, AState);
  Canvas.Brush.Style := bsSolid;
end;

procedure TcxCustomInnerListBox.DrawItem(Index: Integer; Rect: TRect; State: TOwnerDrawState);
var
  AContentBounds: TRect;
begin
  if FItemDrawOffset <> 0 then
  begin
    Rect.Offset(FItemDrawOffset, 0);
    Rect.Left := FBackgroundPadding.Left;
  end;
  if not DoCustomDrawItem(Index, Rect, State) then
  begin
    DrawItemBackground(Index, Rect, State);
    AContentBounds := GetItemContentBounds(Rect);
    DrawItemContent(Index, AContentBounds, State);
  end;
  if (odFocused in State) and NeedDrawFocusRect then
    DrawItemFocusRect(GetItemFocusRectBounds(Rect));
end;

procedure TcxCustomInnerListBox.DrawItemFocusRect(const ABounds: TRect);
begin
  Painter.DrawFocusRect(Canvas, ABounds);
end;

procedure TcxCustomInnerListBox.DrawItemText(const AText: string; ARect: TRect; AState: TOwnerDrawState);
begin
  DrawText(FCanvas.Handle, AText, Length(AText), ARect, GetDrawTextFlags);
end;

procedure TcxCustomInnerListBox.DrawItemBackground(AIndex: Integer; const ARect: TRect; AState: TOwnerDrawState);
begin
  if not Container.Enabled then
   Include(AState, odDisabled); 
  Painter.DrawScaledListBoxItemBackground(FCanvas, ARect, AState, ScaleFactor);
end;

function TcxCustomInnerListBox.GetItemContentBounds(const ARect: TRect): TRect;
begin
  Result := ARect;
  FItemPadding.Deflate(Result, UseRightToLeftAlignment);
end;

function TcxCustomInnerListBox.GetItemFocusRectBounds(const ARect: TRect): TRect;
begin
  Result := ARect;
end;

function TcxCustomInnerListBox.IsBufferedItemPaint: Boolean;
begin
  Result := not Assigned(OnDrawItem);
end;

function TcxCustomInnerListBox.SupportsListBoxSkinPadding: Boolean;
begin
  Result := Container.LookAndFeelPainter.SupportsListBoxPadding;
end;

procedure TcxCustomInnerListBox.CNMeasureItem(var Message: TWMMeasureItem);
var
  AHeight: Integer;
  AMeasureItemStruct: PMeasureItemStruct;
begin
  AMeasureItemStruct := Message.MeasureItemStruct;
  with AMeasureItemStruct^ do
  begin
    itemHeight := GetDefaultItemHeight;
    case Style of
      lbOwnerDrawVariable:
        begin
          if itemID < Cardinal(Count) then
          begin
            AHeight := itemHeight;
...
procedure TcxCustomInnerListBox.CalculatePadding;
var
  AScrollbarInfo: TScrollBarInfo;
  AScrollBarPadding: TRect;
begin
  FBackgroundPadding := TdxPadding.Null;
  FItemPadding := GetDefaultItemPadding;
  if UseCustomPadding then
  begin
    ...
    begin
      if Columns = 0 then
      begin
        FBackgroundPadding.Right := Painter.GetListBoxBackgroundPadding(IsEmbedded, ScaleFactor).Right;
        AScrollBarPadding := Painter.GetListBoxScrollBarPadding(IsEmbedded, sbVertical, ScaleFactor);
        if not Container.IsPopupScrollBars then
        begin
          if Container.IsScrollBarVisible(sbVertical, AScrollBarInfo) then
            Inc(FBackgroundPadding.Right, AScrollBarPadding.Left)
          else
            if IsEmbedded then
              Dec(FBackgroundPadding.Right, AScrollBarPadding.Right)
            else
              FBackgroundPadding.Right := 0;
        end
        else
          Dec(FBackgroundPadding.Right, AScrollBarPadding.Right);
      end;
      ...
      begin
        Inc(FItemPadding.Left, ScaleFactor.Apply(TdxVisualRefinements.Padding.Left));
        Inc(FItemPadding.Right, ScaleFactor.Apply(TdxVisualRefinements.Padding.Right));
      end
      else
        TdxVisualRefinements.Padding.InflatePadding(FItemPadding, ScaleFactor);
  end;
end;

function TcxCustomInnerListBox.GetDefaultItemPadding: TdxPadding;
begin
  Result := Painter.GetListBoxItemPadding(ScaleFactor); 
end;

procedure TcxCustomInnerListBox.UpdatePaintCache;
begin
  if UseRightToLeftAlignment and (Columns = 0) and (ScrollWidth > ClientWidth) then
  begin

    Container.GetStandardScrollBarParameters(sbHorizontal, FScrollInfo);
    FItemDrawOffset := (FScrollInfo.nMax + 1) - ClientWidth - FScrollInfo.nTrackPos;
  end
  else
    FItemDrawOffset := 0;
end;
...
begin
  ...
  FRealStyle := GetRealStyle;
  with Params do
  begin
    if ExtendedSelect then
      ASelect := ExtendSelects[FMultiSelect]
    else
      ASelect := MultiSelects[FMultiSelect];
    Style := Style or (WS_HSCROLL or WS_VSCROLL or
      Data[Self.Style in [lbVirtual, lbVirtualOwnerDraw]] or LBS_NOTIFY) or
      Styles[FRealStyle] or Sorteds[Sorted] or ASelect or IntegralHeights[IntegralHeight] or
      MultiColumns[Columns <> 0] or BorderStyles[BorderStyle] or TabStops[TabWidth <> 0];
...
constructor TdxCustomInnerListBoxAccessibilityHelper.Create(AOwnerObject: TObject);
begin
  inherited;
  FScrollItemProvider := TdxScrollItemProvider.Create;
end;

destructor TdxCustomInnerListBoxAccessibilityHelper.Destroy;
begin
  FreeAndNil(FScrollItemProvider);
  inherited;
end;

function TdxCustomInnerListBoxAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  if AChildID = 0 then
    Result := ListBox.BoundsRect
  else
    Result := ListBox.ItemRect(AChildID - 1);

  Result := cxRectSetOrigin(Result, ListBox.ClientToScreen(Result.TopLeft));
end;

function TdxCustomInnerListBoxAccessibilityHelper.ChildIsSimpleElement(AIndex: Integer): Boolean;
begin
  Result := True;
end;

function TdxCustomInnerListBoxAccessibilityHelper.GetChildCount: Integer;
begin
  Result := ListBox.Items.Count;
end;

function TdxCustomInnerListBoxAccessibilityHelper.GetDefaultActionDescription(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  if AChildID = 0 then
    Result := ''
  else
...
function TdxCustomInnerListBoxAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  if AChildID > 0 then
    Result := ListBox.Items[AChildID - 1]
  else
    Result := '';
end;

function TdxCustomInnerListBoxAccessibilityHelper.GetOwnerObjectWindow: HWND;
begin
  Result := ListBox.Handle;
end;

function TdxCustomInnerListBoxAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  if AChildID = 0 then
...
function TdxCustomInnerListBoxAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxSTATE_SYSTEM_FOCUSABLE or cxSTATE_SYSTEM_SELECTABLE;
    if (AChildID > 0) and (ListBox.ItemIndex = AChildID - 1) then
      Result := Result or cxSTATE_SYSTEM_FOCUSED or cxSTATE_SYSTEM_SELECTED;
end;

function TdxCustomInnerListBoxAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  if AChildID = 0 then
    Result := [aopFocus, aopLocation]
  else
    Result := [aopFocus, aopLocation, aopDefaultAction, aopSelect];
end;

function TdxCustomInnerListBoxAccessibilityHelper.IsExtended: Boolean;
begin
  Result := True;
end;

function TdxCustomInnerListBoxAccessibilityHelper.IsSupportedPattern(APatternID: Integer; out AProvider: IInterface;
  AChildID: TcxAccessibleSimpleChildElementID = 0): Boolean;
begin
  if (AChildID > 0) and (APatternID = dxUIA_ScrollItemPatternId) then
  begin
    Result := True;
    AProvider := FScrollItemProvider;
  end
  else
    Result := inherited;
end;

function TdxCustomInnerListBoxAccessibilityHelper.GetListBox: TcxCustomInnerListBox;
begin
  Result := TcxCustomInnerListBox(OwnerObject);
end;
...
var
  ...
  APopupWindow: TcxCustomPopupWindow;
  I, J: Integer;
  APCWPStruct: Windows.PCWPStruct absolute LParam;
begin
  ...
  begin
    ...
    case message of
      WM_SETFOCUS:
        if not dxMessagesController.IsMessageLocked(CM_ENTER) and IsParentFormPlaced(wparam, hwnd) then
          dxPerformMessageByQueue(dxFindParentControl(hwnd), CM_ENTER);
      WM_KILLFOCUS:
        if not dxMessagesController.IsMessageLocked(CM_EXIT) and IsParentFormPlaced(wparam, hwnd) then
  ...
  if not cxPopupVisibleController.IsAnyVisible then
  begin
    if TcxContainer.FFormInactivationLocked and (APCWPStruct^.message = WM_NCACTIVATE) and (APCWPStruct^.wParam = 0) then
      dxMessagesController.BlockMessage(APCWPStruct^.hwnd);
    Exit;
  end;
  ...
  with APCWPStruct^ do
    case message of
      ...
      WM_POINTERACTIVATE, WM_GESTURE:  // #Ch T1085303
...
finalization
  ...
  RemoveHooks;
  dxFreeGlobalObject(FcxPopupVisibleController);
