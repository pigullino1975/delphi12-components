...
type
  ...
  strict private
    ...
    function GetDisplayTextCore: string;
...
uses
  ...
  dxSpreadSheetCoreFormulasParser, dxSpreadSheetCoreFormulasTokens, dxSpreadSheetCoreStrs, dxSpreadSheetCoreReferences;

const
  dxThisUnitName = 'dxSpreadSheetHyperlinks';
...
type
  ...
  protected
    ...
    FReference: TdxSpreadSheetHyperlinkDocumentReference;
    FSpreadSheet: TdxCustomSpreadSheet;

    procedure ClearReference;
    function TryConvertTokenToReference(AToken: TdxSpreadSheetFormulaToken): Boolean;
    //
    property Formula: TdxSpreadSheetFormula read FFormula;
    property Reference: TdxSpreadSheetHyperlinkDocumentReference read FReference;
  public
...
function TdxSpreadSheetHyperlinkDocumentReference.IsValid: Boolean;
begin
...
begin
  ...
  if not (havDisplayText in AssignedValues) then
...
begin
  ...
  if Name <> nil then
    ...
    if IsValid then
      Result := dxReferenceToString(Area, ASpreadSheet.OptionsView.R1C1Reference, OptionsMap[AView <> Sheet], Sheet.Caption)
    else
...
begin
  if (Sheet <> nil) and (TdxSpreadSheetAccess(ASpreadSheet).FSheets.IndexOf(Sheet) < 0) then
    Sheet := nil;
  if (Name <> nil) and (TdxSpreadSheetObjectListAccess(ASpreadSheet.DefinedNames).ItemList.IndexOf(Name) < 0) then
    Name := nil;
end;
...
begin
  ...
  FFormula := TdxSpreadSheetHyperlinkReferenceHelperFormula.Create(Self);
end;
...
begin
  AParser := TdxSpreadSheetFormulaReferencesParser.Create(FSpreadSheet);
  try
    AParser.ParseFormula(dxSpreadSheetFormulaIncludeEqualSymbol(Reference.Value), FFormula);
  finally
    AParser.Free;
  end;

  if (FFormula <> nil) and TryConvertTokenToReference(Formula.Tokens) then
...
function TdxSpreadSheetHyperlinkReferenceHelper.TryConvertTokenToReference(AToken: TdxSpreadSheetFormulaToken): Boolean;
begin
  if AToken is TdxSpreadSheetFormulaReferenceToken then
  begin
    ClearReference;
    Reference.Sheet := TdxSpreadSheetFormulaReferenceToken(AToken).View as TdxSpreadSheetCustomView;
    Reference.Area.Left := TdxSpreadSheetFormulaReferenceToken(AToken).ActualColumn;
    Reference.Area.Top := TdxSpreadSheetFormulaReferenceToken(AToken).ActualRow;
    if AToken is TdxSpreadSheetFormulaAreaReferenceToken then
    begin
      Reference.Area.Bottom := TdxSpreadSheetFormulaAreaReferenceToken(AToken).ActualRow2;
      Reference.Area.Right := TdxSpreadSheetFormulaAreaReferenceToken(AToken).ActualColumn2;
    end
    else
  ...
  begin
    ClearReference;
    Reference.Name := TdxSpreadSheetFormulaDefinedNameToken(AToken).DefinedName;
    Reference.Area := cxNullRect;
    Result := Reference.IsValid;
  end
  else

  if (AToken <> nil) and (AToken.ChildCount = 1) and (AToken.Next = nil) and (AToken.Prev = nil) then
    Result := TryConvertTokenToReference(AToken.FirstChild)
  else
    Result := False;
end;
...
begin
  Result := FOwner.FSpreadSheet.FormulaController;
end;
...
begin
  ...
  begin
    ...
    Result := TdxSpreadSheetHyperlinkDocumentReference(Data).IsValid;
  end;
end;
...
var
  ...
  AHyperlinkTextColor: TColor;
  ARow, AColumn: Integer;
  AView: TdxSpreadSheetTableView;
begin
  if not IsAreaCorrect then
    Exit;

  AView := TdxSpreadSheetTableView(View);
  if TdxSpreadSheetAccess(AView.SpreadSheet).State <> [] then
    Exit;

  AView.BeginUpdate;
  try
    ACell := AView.CreateCell(Area.Top, Area.Left);
    ADisplayText := GetDisplayTextCore;
    if DisplayTextAssigned or (ACell.AsString = '') and (ADisplayText <> '') then
      ACell.SetText(ADisplayText);

    AHyperlinkTextColor := TdxSpreadSheetAccess(AView.SpreadSheet).LookAndFeelPainter.DefaultHyperlinkTextColor;
    for ARow := Area.Top to Area.Bottom do
      for AColumn := Area.Left to Area.Right do
      begin
        AFont := TdxSpreadSheetFontAccess(AView.CreateCell(ARow, AColumn).Style.Font);
        AFont.ChangeFont(AFont.Name, AFont.Charset, AHyperlinkTextColor, AFont.Size, AFont.Pitch, AFont.Style + [fsUnderline]);
      end;
  finally
    AView.EndUpdate;
  end;
...
var
  ACell: TdxSpreadSheetCell;
begin
  if not DisplayTextAssigned then
  begin
    ACell := TdxSpreadSheetTableView(Owner.View).Cells[Area.Top, Area.Left];
    if (ACell <> nil) and not ACell.IsEmpty then
      Exit(ACell.AsString);
  end;
  Result := GetDisplayTextCore;
end;

function TdxSpreadSheetHyperlink.GetDisplayTextCore: string;
begin
