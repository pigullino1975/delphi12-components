...
unit dxRichEdit.Import.Html.TagBase; // for internal use
...
type
  ...
  public
    ...
    function GetEnumValue(const AStr: string; const ADefaultValue: T; ACaseSensitive: Boolean = False): T; overload;
  ...
  public
    constructor Create(AImporter: TdxCustomHtmlImporter); virtual;
    destructor Destroy; override;
    ...
    function DecodeStringContent(const AContentText: string; out ADecodeContentText: string): Boolean; virtual;
  ...
  TdxTagBaseClass = class of TdxTagBase;
  TdxNamespaceTagInfo = record
    Name: TdxHtmlTagNameID;
    TagClass: TdxTagBaseClass;
    constructor Create(AName: TdxHtmlTagNameID; ATagClass: TdxTagBaseClass);
  end;

  TdxNamespaceTagInfos = class(TDictionary<string, TdxNamespaceTagInfo>);
  TdxNamespaceTagTables = class(TObjectDictionary<string, TdxNamespaceTagInfos>);

  { TdxNamespaceTag }

  TdxNamespaceTag = class sealed
  strict private
    class var
      FNamespaceTable: TdxNamespaceTagTables;
    class constructor Initialize;
    class destructor Finalize;
    class procedure CreateNamespaceTable;
  strict private
    class function FindNamespaceTagInfo(AImporter: TdxCustomHtmlImporter; const AName: string;
      out ANamespaceTagInfo: TdxNamespaceTagInfo): Boolean;
  public
    class function Create(AImporter: TdxCustomHtmlImporter): TdxTagBase;
    class function NamespaceContains(AImporter: TdxCustomHtmlImporter; const ATagName: string): boolean;
    class procedure PopulateNamespaceTranslatorTable(const ANamespace: string; ANamespaceTagInfos: TdxNamespaceTagInfos);
  end;

implementation

uses
  RTLConsts, Contnrs, Character, Math, IOUtils,
  dxDocumentLayoutUnitConverter,
  dxRichEdit.Import.Html,
  dxCharacters, dxCoreClasses,
...
const
  dxThisUnitName = 'dxRichEdit.Import.Html.TagBase';

type
  TdxHtmlImporterAccess = class(TdxHtmlImporter);
...
function TdxTranslationTable<T>.GetEnumValue(const AStr: string; const ADefaultValue: T; ACaseSensitive: Boolean = False): T;
var
  AEntry: TdxTranslationTableEntry<T>;
  S: string;
  I: Integer;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxTagBase.Initialize', SysInit.HInstance);{$ENDIF}
  FBorderLineStyleTable := CreateBorderLineStyleTable;
  FAttributeTable := CreateAttributeTable;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxTagBase.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxTagBase.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxTagBase.Finalize', SysInit.HInstance);{$ENDIF}
  FBorderLineStyleTable.Free;
  FAttributeTable.Free;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxTagBase.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  ...
  begin
    ...
    if ((AAttribute.Value <> '') or (AAttribute.Name = 'NOWRAP') or (AAttribute.Name = 'NOSHADE')) and
        AttributeTable.TryGetValue(AAttribute.Name, ATranslator) then
      ATranslator(FImporter, Importer.DecodeStringContent(AAttribute.Value), Self);
  end;
end;
...
function TdxTagBase.DecodeStringContent(const AContentText: string; out ADecodeContentText: string): Boolean;
begin
  Result := False;
end;
...
begin
  TdxHtmlImporterAccess(Importer).ProcessIsEmptyLine;
...
begin
  ...
  begin
    ...
    begin
      ApplyCharacterProperties(Importer.PieceTable.Runs.Count - 2);
...
constructor TdxNamespaceTagInfo.Create(AName: TdxHtmlTagNameID; ATagClass: TdxTagBaseClass);
begin
  Name := AName;
  TagClass := ATagClass;
end;
...
class function TdxNamespaceTag.Create(AImporter: TdxCustomHtmlImporter): TdxTagBase;
var
  ATag: TdxTag;
  ANamespaceTagInfo: TdxNamespaceTagInfo;
begin
  ATag := TdxHtmlImporter(AImporter).Element as TdxTag;
  if not FindNamespaceTagInfo(AImporter, ATag.RawText, ANamespaceTagInfo) then
    ANamespaceTagInfo := TdxNamespaceTagInfo.Create(TdxHtmlTagNameID.Unknown, TdxTagBase);
  ATag.NameID := ANamespaceTagInfo.Name;
  Result := ANamespaceTagInfo.TagClass.Create(AImporter);
end;

class constructor TdxNamespaceTag.Initialize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxNamespaceTag.Initialize', SysInit.HInstance);{$ENDIF}
  CreateNamespaceTable;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxNamespaceTag.Initialize', SysInit.HInstance);{$ENDIF}
end;

class procedure TdxNamespaceTag.CreateNamespaceTable;
begin
  if FNamespaceTable = nil then
    FNamespaceTable := TdxNamespaceTagTables.Create([doOwnsValues]);
end;

class destructor TdxNamespaceTag.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxNamespaceTag.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FNamespaceTable);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxNamespaceTag.Finalize', SysInit.HInstance);{$ENDIF}
end;

class function TdxNamespaceTag.NamespaceContains(AImporter: TdxCustomHtmlImporter; const ATagName: string): boolean;
var
  ANamespaceTagInfo: TdxNamespaceTagInfo;
begin
  Result := FindNamespaceTagInfo(AImporter, ATagName, ANamespaceTagInfo);
end;

class function TdxNamespaceTag.FindNamespaceTagInfo(AImporter: TdxCustomHtmlImporter; const AName: string;
  out ANamespaceTagInfo: TdxNamespaceTagInfo): Boolean;
var
  ANamespaceAliasTag: TArray<string>;
  ANamespace: string;
  AHtmlImporter: TdxHtmlImporter absolute AImporter;
  ANamespaceTagInfos: TdxNamespaceTagInfos;
begin
  ANamespaceAliasTag := TdxStringHelper.Split(TdxStringHelper.Trim(AName, ['<', '>', ' ']), [':']);
  if AHtmlImporter.HtmlNamespaces.TryGetValue(UpperCase(ANamespaceAliasTag[0]), ANamespace) then
    Result := FNamespaceTable.TryGetValue(ANamespace, ANamespaceTagInfos) and
      ANamespaceTagInfos.TryGetValue(UpperCase(ANamespaceAliasTag[1]), ANamespaceTagInfo)
  else
    Result := False;
end;

class procedure TdxNamespaceTag.PopulateNamespaceTranslatorTable(const ANamespace: string;
  ANamespaceTagInfos: TdxNamespaceTagInfos);
begin
  if FNamespaceTable = nil then
    CreateNamespaceTable;
  FNamespaceTable.Add(ANamespace, ANamespaceTagInfos);
end;
