...
uses
  System.UITypes,
  Types, Variants, Windows, Classes, Graphics, Dialogs, Messages, Controls, Generics.Defaults, Generics.Collections,
  cxGeometry, dxCore, cxGraphics, cxControls, cxEdit, cxTextEdit, cxRichEdit, cxClasses, cxCustomCanvas,
  dxSpreadSheetCore, dxSpreadSheetGraphics, dxCoreClasses, dxSpreadSheetCoreStyles, dxSpreadSheetFormulas, dxGenerics;
...
type
  ...
  TdxSpreadSheetCustomTableViewDragAndDropObject = class(TdxSpreadSheetCustomCellBasedDragAndDropObject)
  strict private
  ...
  TdxSpreadSheetTableViewReferenceHighlighterItem = TdxSpreadSheetFormulaReferenceInfo;
  ...
  protected
    ...
    function GetCellAtAbsolutePoint(const P: TPoint; out ARowIndex, AColumnIndex: Integer): Boolean; overload; override;
  ...
  protected
    ...
    procedure AdjustRowAndColumnIndexes(P: TPoint; var ARowIndex, AColumnIndex: Integer); virtual;
    procedure BeforeBeginDragAndDrop; override;
  ...
  TdxSpreadSheetTableViewReferenceHighlighterAddAreaDragAndDropObject = class(
    TdxSpreadSheetTableViewReferenceHighlighterResizeDragAndDropObject)
  protected
    procedure AdjustRowAndColumnIndexes(P: TPoint; var ARowIndex, AColumnIndex: Integer); override;
    procedure BeforeBeginDragAndDrop; override;
    procedure BeginDragAndDrop; override;
  ...
  public
    ...
    //
    property Corner: TdxCorner read FCorner write FCorner;
  end;
...
uses
  SysUtils, Math,
  dxSpreadSheetUtils, dxSpreadSheetTypes, dxSpreadSheetClasses, dxSpreadSheetStrs, Forms,
  dxTypeHelpers, dxHashUtils, dxSpreadSheetCoreHelpers, cxLibraryConsts, dxSpreadSheetAutoFilling,
  dxSpreadSheetCoreFormulas, dxSpreadSheetCoreStrs, cxLookAndFeelPainters, dxSpreadSheetPopupMenu, dxMessageDialog;

const
  dxThisUnitName = 'dxSpreadSheetSelection';
...
function TdxSpreadSheetCustomTableViewDragAndDropObject.GetHitTest: TdxSpreadSheetTableViewHitTest;
begin
  Result := View.HitTest;
end;
...
begin
  ...
  FColorPalette := TdxColorList.Create;
...
begin
  ...
  begin
    ...
    begin
      Calculate(GetAdjustedRichEditText(TcxCustomRichEdit(AEdit)));
      HighlightReferences(TcxCustomRichEdit(AEdit));
    end
    else
      Calculate(VarToStr(AEdit.EditValue));
  end;
end;

procedure TdxSpreadSheetTableViewReferenceHighlighter.Calculate(const AFormulaText: string);
var
  AReference: TdxSpreadSheetFormulaReferenceInfo;
  AReferences: TdxSpreadSheetFormulaReferences;
  I: Integer;
begin
  References.Clear;
  if Cell <> nil then
  begin
    AReferences := TdxSpreadSheetFormulaReferences.Create(False);
    try
      AReferences.Initialize(Cell, AFormulaText);
      References.Capacity := AReferences.Count;
      for I := 0 to AReferences.Count - 1 do
      begin
        AReference := AReferences[I];
        AReference.Area := cxRectAdjust(AReference.Area);
        if dxSpreadSheetIsValidArea(AReference.Area) then
        begin
          AReference.DisplayBounds := GetAreaBounds(AReference.Area);
...
begin
  ...
  for I := 0 to References.Count - 1 do
    ReferenceColors.AddOrSetValue(References[I].Area, Colors[ReferenceColors.Keys.Count]);
end;
...
procedure TdxSpreadSheetTableViewReferenceHighlighter.HighlightReferences(AEdit: TcxCustomRichEdit);
  ...
  begin
    ...
    AEdit.SelLength := AItem.Length;
    AEdit.SelAttributes.Color := ReferenceColors.Items[AItem.Area];
  end;
...
begin
  ...
  try
    ...
    try
      AEdit.SelectAll;
      AEdit.SelAttributes.Color := TcxCustomRichEditAccess(AEdit).VisibleFontColor;
      for I := 0 to References.Count - 1 do
...
var
  ABuilder: TdxColorPaletteBuilder;
begin
  ABuilder := TdxColorPaletteBuilder.Create(SpreadSheet.LookAndFeel.Painter.SpreadSheetSelectionColor);
  try
    FColorPalette.Clear;
    ABuilder.Build(FColorPalette, dxSpreadSheetSelectionColors);
  finally
    ABuilder.Free;
  end;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      TdxSpreadSheetSelectionHelper.Draw(ACanvas, AReference.DisplayBounds,
        ReferenceColors.Items[AReference.Area], ABackgroundColor, [ssseCorners], dxAllCorners);
    end;
    for I := 0 to References.Count - 1 do
    begin
      AReference := References[I];
      TdxSpreadSheetSelectionHelper.Draw(ACanvas, AReference.DisplayBounds,
        ReferenceColors.Items[AReference.Area], ABackgroundColor, [ssseFrame, ssseBackground], []);
    end;
  finally
...
begin
  ...
  for I := 0 to References.Count - 1 do
  begin
    ARect := References[I].DisplayBounds;
...
begin
  ...
  InitializeReference;
  FContentRect := TdxSpreadSheetTableViewAccess(View).GetAbsoluteCellsArea;
  FCursor := HitTest.HitObject.GetCursor(HitTest);
  FPrevCellArea := Reference.Area;
  FBounds := Reference.DisplayBounds;
end;
...
function TdxSpreadSheetTableViewReferenceHighlighterCustomDragAndDropObject.GetCellAtAbsolutePoint(
  const P: TPoint; out ARowIndex, AColumnIndex: Integer): Boolean;
begin
  Result := inherited GetCellAtAbsolutePoint(P, ARowIndex, AColumnIndex);
  if Result then
...
begin
  ...
  if AForce or not cxRectIsEqual(AReference, Reference.Area) then
  begin
    ...
    AIndexDelta := Length(AReferenceAsString) - Reference.Length;
    ...
    AFormulaText := Copy(AFormulaText, 1, Reference.PositionInText - 1) + AReferenceAsString +
      Copy(AFormulaText, Reference.PositionInText + Reference.Length, MaxInt);
    Reference.Area := AReference;
    Reference.DisplayBounds := Highlighter.GetAreaBounds(AReference);
    Reference.Length := Length(AReferenceAsString);
    ...
    try
      ...
      if Edit is TcxCustomTextEdit then
        TcxCustomTextEdit(Edit).SelStart := Reference.PositionInText + Reference.Length - 1;
...
begin
  inherited BeginDragAndDrop;
  ACellRef := EnsureCellInArea(GetCellAtAbsolutePoint(CapturePoint), Reference.Area);
  FColumnDelta := Reference.Area.Left - ACellRef.X;
  FRowDelta := Reference.Area.Top - ACellRef.Y;
end;
...
begin
  ...
  ARowIndex := Min(ARowIndex, dxSpreadSheetMaxRowCount - dxSpreadSheetAreaHeight(Reference.Area));
  AColumnIndex := Min(AColumnIndex, dxSpreadSheetMaxColumnCount - dxSpreadSheetAreaWidth(Reference.Area));
end;
...
begin
  Accepted := True;
  ACellRef := GetCellAtAbsolutePoint(P);
  Inc(ACellRef.X, FColumnDelta);
  Inc(ACellRef.Y, FRowDelta);
  CheckRowAndColumnIndexes(ACellRef.Y, ACellRef.X);
  UpdateReference(cxRectSetOrigin(Reference.Area, ACellRef));
  inherited DragAndDrop(P, Accepted);
end;
...
procedure TdxSpreadSheetTableViewReferenceHighlighterResizeDragAndDropObject.AdjustRowAndColumnIndexes(
  P: TPoint; var ARowIndex, AColumnIndex: Integer);
...
begin
  inherited BeforeBeginDragAndDrop;
  FCellsArea := Reference.Area;
  FCorner := TdxCorner(HiWord(HitTest.HitObjectData));
  if IsRightToLeftLayout then
    FCorner := TdxRightToLeftLayoutConverter.ConvertCorner(FCorner);
end;
...
begin
  Accepted := True;
  ACellRef := GetCellAtAbsolutePoint(P);
  AdjustRowAndColumnIndexes(P, ACellRef.Y, ACellRef.X);
  CheckRowAndColumnIndexes(ACellRef.Y, ACellRef.X);
  FCellsArea := CalculateReference(ACellRef.Y, ACellRef.X);
...
begin
  inherited BeginDragAndDrop;
  UpdateReference(Reference.Area, True);
end;
...
begin
  ...
  begin
    ...
    if InRange(ACursorPos, AItem.PositionInText, AItem.PositionInText + AItem.Length - 1) then
  ...
  FReference.Area := Bounds(AColumn, ARow, 0, 0);
  FReference.DisplayBounds := Highlighter.GetAreaBounds(FReference.Area);
...
begin
  inherited BeginDragAndDrop;
  ACellRef := GetCellAtAbsolutePoint(ToAbsolutePoint(CurMousePos));
  ASelectedArea := GetSelectionArea;
  FCapturedCell := EnsureCellInArea(ACellRef, ASelectedArea);
  ClipboardArea.Initialize(View, ASelectedArea);
end;
...
begin
  ...
  begin
    if dxMessageDlg(cxGetResourceString(@sdxReplaceCellsDataConfirmation), mtConfirmation, [mbOK, mbCancel], 0) = mrCancel then
      Abort;    
  end;
end;
...
begin
  ...
  if Accepted then
  begin
    ADelta := cxPointOffset(GetCellAtAbsolutePoint(ToAbsolutePoint(P)), CapturedCell, False);
...
constructor TdxSpreadSheetTableViewSelectionFrameViewInfo.Create(AOwner: TObject);
begin
  inherited;
  FCorner := coBottomRight;
end;
...
begin
  ...
  begin
    ...
    if Result then
    begin
      ACornerBounds := TdxSpreadSheetSelectionHelper.GetCornerBounds(Bounds, FCorner);
      ACornerBounds := cxRectCenter(ACornerBounds, dxSpreadSheetCornetHitTestZoneSize, dxSpreadSheetCornetHitTestZoneSize);
      TdxSpreadSheetCustomHitTestAccess(AHitTest).HitObject := Self;
...
procedure TdxSpreadSheetTableViewSelectionFrameViewInfo.RightToLeftConversion(const AClientBounds: TRect);
begin
  inherited RightToLeftConversion(AClientBounds);
  FCorner := TdxRightToLeftLayoutConverter.ConvertCorner(FCorner);
end;
...
function TdxSpreadSheetTableViewPrintAreaViewInfo.InitHitTest(AHitTest: TdxSpreadSheetCustomHitTest): Boolean;
begin
  Result := TdxSpreadSheetSelectionHelper.IsInFrame(Bounds, AHitTest.ActualHitPoint);
end;
