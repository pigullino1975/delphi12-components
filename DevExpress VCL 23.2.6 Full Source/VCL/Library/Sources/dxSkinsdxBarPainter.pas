...
type
  ...
  private
    ...
    function GetBarSubItemArrow: TdxSkinElement;
    function GetBarSubItemArrowWidth(var AWidth: Integer): Boolean;
    ...
    function GetPopupMenuLinkArrow: TdxSkinElement;
    function GetPopupMenuLinkArrowWidth(var AWidth: Integer): Boolean;
  ...
  protected
    ...
    function DrawArrow(const ADrawParams: TdxBarButtonLikeControlDrawParams; AArrowRect: TRect;
      ARecalcArrowRect: Boolean; ABackButton: Boolean = False): Boolean;
    ...
    // Bar
      // Attributes
    function BarCaptionColor(ABarControl: TdxBarControl): TColor; override;
    ...
    property BarSubItemArrow: TdxSkinElement read GetBarSubItemArrow;
    ...
    property PopupMenuLinkArrow: TdxSkinElement read GetPopupMenuLinkArrow;
  ...
  public
    ...
    function IsFlatItemText: Boolean; override;
    function IsCustomSelectedTextColorExists(ABarItemControl: TdxBarItemControl): Boolean; override;
      // Sizes
    function GetDockContentOffsets(AScaleFactor: TdxScaleFactor): TRect; override;
    ...
    function GetGlyphColorPalette(ABarItemControl: TdxBarItemControl; APaintType: TdxBarPaintType; ASelected, ADowned,
      ADrawDowned, ADroppedDown, AIsSplit, ADrawImageEnabled: Boolean): IdxColorPalette; override;
    function GetState(AEnabled, ASelected, ADowned, ADrawDowned, ADroppedDown, AIsMenuItem,
      ADrawImageEnabled: Boolean): TdxSkinElementState;
    ...
    function RealButtonArrowWidth(ABarManager: TdxBarManager): Integer; override;
    function RealLargeButtonArrowWidth(ABarManager: TdxBarManager): Integer; override;
    ...
    class function GetDefaultTextSize(ACanvas: TcxCanvas; AScaleFactor: TdxScaleFactor; APainter: TdxBarPainter{ = nil}): Integer; override;
    ...
    // Edit
    function EditGetDisabledBkColor(ABarItemControl: TdxBarCustomEditControl): TColor; override;
    function EditGetDisabledTextColor: TColor; override;
    function EditGetEnabledBkColor(ABarItemControl: TdxBarCustomEditControl): TColor; override;
    function EditGetEnabledTextColor: TColor; override;
    ...
    function SubMenuControlArrowWidth(ATextSize: Integer; AScaleFactor: TdxScaleFactor): Integer; override;
    ...
    class function SubMenuControlGetItemIconRect(const AItemRect: TRect; AIconAreaSize: Integer; APainter: TdxBarPainter): TRect; override;
    class procedure SubMenuControlOffsetDetachCaptionRect(ABarSubMenuControl: TdxBarSubMenuControl; var R: TRect); override;
    class function SubMenuControlTextSize(ACanvas: TcxCanvas; AScaleFactor: TdxScaleFactor; APainter: TdxBarPainter{ = nil}): Integer; override;
      // Positions
    class function SubMenuControlGetItemTextIndent(const ADrawParams: TdxBarItemControlDrawParams; APainter: TdxBarPainter): Integer; override;
      // Draw
    ...
    // BarItemControl
      // Sizes
    procedure CorrectItemControlSmallHeight(const ADrawParams: TdxBarItemControlDrawParams; var ASmallHeight: Integer); override;
    ...
    function GetButtonHeight(AIconSize, ATextSize, AEditSize: Integer; AScaleFactor: TdxScaleFactor): Integer; override;
    function GetButtonWidth(AIconSize, ATextSize, AEditSize: Integer; AScaleFactor: TdxScaleFactor): Integer; override;
      // Draw
    ...
    procedure DrawButtonControlArrow(const ADrawParams: TdxBarButtonLikeControlDrawParams; ARect: TRect); override;
    ...
    class function EditControlCaptionRightIndentIsOpaque(const ADrawParams: TdxBarEditLikeControlDrawParams): Boolean; override;
      // Sizes
    function EditControlBorderOffsets(APaintType: TdxBarPaintType): TRect; override;
      // Draw
    ...
    class function EditControlCaptionSimpleIndent(const ADrawParams: TdxBarEditLikeControlDrawParams; APainter: TdxBarPainter): Integer; override;

    function GetCaptionOffsets: TRect; override;
    procedure GetEditTextParams(out AOffsets: TRect; out AHeightCorrection: Integer; APainter: TdxBarPainter); override;
    ...
    // InPlaceSubItemControl
      // Sizes
    function InPlaceSubItemGetArrowWidth(const ADrawParams: TdxBarInPlaceSubItemControlDrawParams): Integer; override;
    function InPlaceSubItemGetTextIndent: Integer; override;
      // Conditions
    ...
    procedure DropDownGalleryDrawFilterBand(DC: HDC; const R: TRect; AState: Integer); override;
...
uses
  Math, dxOffice11, dxBarSkinConsts, dxSkinsStrs, dxFading, dxDPIAwareUtils, dxTypeHelpers;

const
  dxThisUnitName = 'dxSkinsdxBarPainter';
...
function TdxBarSkinPainter.IsFlatItemText: Boolean;
begin
  Result := True;
end;
...
function TdxBarSkinPainter.GetDockContentOffsets(AScaleFactor: TdxScaleFactor): TRect;
begin
  Result := AScaleFactor.Apply(Dock.ContentOffset.Rect);
end;
...
var
  ASkinElement: TdxSkinElement;
begin
  R := ADrawParams.BarItemControl.ItemBounds;  // because QuickControlItem draw before
  ...
  begin
    ...
    begin
      ...
      if ADrawParams.Canvas.UseRightToLeftAlignment then
        Inc(R.Left, ADrawParams.ArrowSize.cx)
      else
        Dec(R.Right, ADrawParams.ArrowSize.cx);
    end
    else
      ASkinElement := PopupMenuLinkSelected;

    DrawSkinElement(ASkinElement, ADrawParams.Canvas.Handle, R, dxGetScaleFactor(ADrawParams.BarItemControl), 0,
      ButtonLikeControlGetState(ADrawParams), ADrawParams.Canvas.UseRightToLeftAlignment);
  end;
end;
...
function TdxBarSkinPainter.RealButtonArrowWidth(ABarManager: TdxBarManager): Integer;
var
  AWidth: Integer;
begin
  if GetBarSubItemArrowWidth(AWidth) then
    Result := AWidth
  else
    Result := inherited RealButtonArrowWidth(ABarManager);
end;

function TdxBarSkinPainter.RealLargeButtonArrowWidth(ABarManager: TdxBarManager): Integer;
var
  AWidth: Integer;
begin
  if GetPopupMenuLinkArrowWidth(AWidth) then
    Result := AWidth
  else
...
class function TdxBarSkinPainter.GetDefaultTextSize(ACanvas: TcxCanvas; AScaleFactor: TdxScaleFactor; APainter: TdxBarPainter{ = nil}): Integer;
var
  APadding: TRect;
begin
  if (APainter <> nil) and APainter.LookAndFeelPainter.ApplyEditorAdvancedMode then
  begin
    APadding := APainter.LookAndFeelPainter.GetEditorPadding(True, AScaleFactor);
    Result := TdxTextMeasurer.TextLineHeight(ACanvas.Handle) + APadding.Top + APadding.Bottom;
  end
  else
    Result := inherited GetDefaultTextSize(ACanvas, AScaleFactor, APainter);
end;
...
function TdxBarSkinPainter.EditGetDisabledBkColor(ABarItemControl: TdxBarCustomEditControl): TColor;
begin
...
function TdxBarSkinPainter.EditGetDisabledTextColor: TColor;
begin
...
function TdxBarSkinPainter.EditGetEnabledBkColor(ABarItemControl: TdxBarCustomEditControl): TColor;
begin
...
function TdxBarSkinPainter.EditGetEnabledTextColor: TColor;
begin
...
procedure TdxBarSkinPainter.DockControlFillBackground(ADockControl: TdxDockControl;
  ...
  begin
    ABitmap.Width := AWholeR.Width;
    ABitmap.Height := AWholeR.Height;
...
begin
  if GetSkinPainterData(ASkinInfo) then
    ASkinElement := ASkinInfo.DropDownGalleryBackground
  else
    ASkinElement := nil;
...
begin
  if GetSkinPainterData(ASkinInfo) then
    ASkinElement := ASkinInfo.DropDownGallerySizeGrips
  else
    ASkinElement := nil;
...
begin
  if GetSkinPainterData(ASkinInfo) then
    ASkinElement := ASkinInfo.DropDownGallerySizingPanel
  else
    ASkinElement := nil;
...
procedure TdxBarSkinPainter.DropDownGalleryDrawFilterBand(DC: HDC; const R: TRect; AState: Integer);
begin
...
begin
  if GetSkinPainterData(ASkinInfo) then
    ASkinElement := ASkinInfo.DropDownGalleryGroupHeader
  else
    ASkinElement := nil;
...
var
  ASkinInfo: TdxSkinInfo;
begin
  if GetSkinPainterData(ASkinInfo) and (ASkinInfo.StandaloneGalleryItem <> nil) then
  begin
    ACanvas.SaveClipRegion;
    try
      ACanvas.ExcludeClipRect(cxRectInflate(R, -2));
      DrawSkinElement(ASkinInfo.StandaloneGalleryItem, ACanvas.Handle, R, dxDefaultScaleFactor, 0,
        dxSkinElementCheckState(ASkinInfo.StandaloneGalleryItem, GetSkinElementState));
    finally
...
begin
  if GetSkinPainterData(ASkinInfo) then
    Result := ASkinInfo.DropDownGalleryGroupHeader.TextColor
  else
  ...
  if GetSkinPainterData(ASkinInfo) and not TdxBarSubItemControlAccess(ABarItemControl).GetCollapsed then
    ...
    begin
      ...
      if TdxBarSkinnedPainterAccess.GetPartState(ADrawParams, icpControl) <> DXBAR_NORMAL then
        ...
        AColor1 := clDefault;
      AColor1 := cxGetActualColor(AColor1, GetSkinElementTextColor(ASkinInfo.DropDownGalleryBackground, cxbsNormal));
      AColor2 := AColor1;
    end
    else
...
function TdxBarSkinPainter.SubMenuControlArrowWidth(ATextSize: Integer; AScaleFactor: TdxScaleFactor): Integer;
var
  AWidth: Integer;
begin
  if GetPopupMenuLinkArrowWidth(AWidth) then
    Result := AScaleFactor.Apply(AWidth)
  else
    Result := inherited SubMenuControlArrowWidth(ATextSize, AScaleFactor);
end;
...
begin
  ...
  begin
    APadding := AScaleFactor.Apply(PopupMenuExpandButton.ContentOffset.Rect);
    if not PopupMenuExpandButton.Glyph.Empty then
      AMarkSize := AScaleFactor.Apply(PopupMenuExpandButton.Glyph.Size.cy);
    AMarkSize := Max(AMarkSize, GetSkinElementSize(PopupMenuExpandButton, AScaleFactor).cy) + APadding.Height;
  end;
end;
...
class function TdxBarSkinPainter.SubMenuControlGetItemIconRect(const AItemRect: TRect; AIconAreaSize: Integer; APainter: TdxBarPainter): TRect;
var
  ASkinPainter: TdxBarSkinPainter;
begin
  ASkinPainter := Safe<TdxBarSkinPainter>.Cast(APainter);
  if (ASkinPainter <> nil) and ASkinPainter.LookAndFeelPainter.ApplyEditorAdvancedMode then
  begin
    Result := inherited SubMenuControlGetItemIconRect(cxRectInflate(AItemRect, -3, -3), AIconAreaSize - 6, APainter);
  end
  else
    Result := inherited SubMenuControlGetItemIconRect(AItemRect, AIconAreaSize, APainter);
end;
...
class function TdxBarSkinPainter.SubMenuControlTextSize(ACanvas: TcxCanvas; AScaleFactor: TdxScaleFactor; APainter: TdxBarPainter{ = nil}): Integer;
var
  APadding: TRect;
  ASkinPainter: TdxBarSkinPainter;
begin
  ...
  begin
    ...
    APadding := ASkinPainter.PopupMenuLinkSelected.ContentOffset.Rect;
...
class function TdxBarSkinPainter.SubMenuControlGetItemTextIndent(const ADrawParams: TdxBarItemControlDrawParams; APainter: TdxBarPainter): Integer;
begin
  Result := TdxBarFlatPainter.SubMenuControlGetItemTextIndent(ADrawParams, APainter);
end;
...
var
  ARect: TRect;
begin
  ARect.Init(ABeginGroupRect.Left + SubMenuBeginGroupIndent + TdxBarItemControlAccess(AControl).TextAreaOffset,
...
begin
  ...
  begin
    ...
    begin
      ...
      ABandRect.Left := ABandRect.Left - ASeparatorRect.Width;
    end
    else
    begin
      ...
      ABandRect.Right := ABandRect.Right + ASeparatorRect.Width;
    end;
...
begin
  cxPaintCanvas.BeginPaint(DC);
  try
    cxPaintCanvas.IntersectClipRect(MarkR);
...
procedure TdxBarSkinPainter.CorrectItemControlSmallHeight(const ADrawParams: TdxBarItemControlDrawParams; var ASmallHeight: Integer);
var
  ASkinInfo: TdxSkinInfo;
begin
  if GetSkinPainterData(ASkinInfo) and ASkinInfo.IsAdvanced then
    ASmallHeight := Max(ASmallHeight, ADrawParams.DefaultButtonSize.cy);
end;
...
function TdxBarSkinPainter.GetButtonHeight(AIconSize, ATextSize, AEditSize: Integer; AScaleFactor: TdxScaleFactor): Integer;
begin
  if LookAndFeelPainter.ApplyEditorAdvancedMode then
  begin
    Result := Max(AIconSize + GetButtonBorderHeight(AScaleFactor), ATextSize);
    Result := Max(Result, AEditSize);
  end
  else
    Result := inherited GetButtonHeight(AIconSize, ATextSize, AEditSize, AScaleFactor);
end;

function TdxBarSkinPainter.GetButtonWidth(AIconSize, ATextSize, AEditSize: Integer; AScaleFactor: TdxScaleFactor): Integer;
begin
  if LookAndFeelPainter.ApplyEditorAdvancedMode then
  begin
    Result := Max(AIconSize + GetButtonBorderWidth(AScaleFactor), ATextSize);
    Result := Max(Result, AEditSize);
  end
  else
    Result := inherited GetButtonWidth(AIconSize, ATextSize, AEditSize, AScaleFactor);
end;

procedure TdxBarSkinPainter.DrawAssociateControlArrow(const ADrawParams: TdxBarButtonLikeControlDrawParams; ARect: TRect);
begin
  if not DrawArrow(ADrawParams, ARect, True) then
    inherited DrawAssociateControlArrow(ADrawParams, ARect);
end;

procedure TdxBarSkinPainter.DrawBackButtonControlArrow(const ADrawParams: TdxBarButtonLikeControlDrawParams; ARect: TRect);
begin
  if not DrawArrow(ADrawParams, ARect, False, True) then
    inherited DrawBackButtonControlArrow(ADrawParams, ARect);
end;

procedure TdxBarSkinPainter.DrawButtonControlArrow(const ADrawParams: TdxBarButtonLikeControlDrawParams; ARect: TRect);
begin
  if not DrawArrow(ADrawParams, ARect, False) then
    inherited DrawButtonControlArrow(ADrawParams, ARect);
end;
...
begin
  ...
  begin
    if (GetSkinPainterData(ASkinPainterInfo) and ASkinPainterInfo.IsAdvanced) then
    begin
      if (ADrawParams.PaintType = ptMenu) or not ADrawParams.DroppedDown then
        DrawBackground(ADrawParams.BarItemControl, ADrawParams.Canvas.Handle, R1, ABrush,
...
function TdxBarSkinPainter.GetCaptionOffsets: TRect;
begin
  if IsAdvanced then
    Result := TRect.Create(0, 0, 0, 0)
  else
    Result := inherited GetCaptionOffsets;
end;
...
function TdxBarSkinPainter.EditControlBorderOffsets(APaintType: TdxBarPaintType): TRect;
begin
...
class function TdxBarSkinPainter.EditControlCaptionSimpleIndent(const ADrawParams: TdxBarEditLikeControlDrawParams; APainter: TdxBarPainter): Integer;
begin
  Result := TdxBarOffice11Painter.EditControlCaptionSimpleIndent(ADrawParams, APainter);
end;
...
function TdxBarSkinPainter.InPlaceSubItemGetArrowWidth(const ADrawParams: TdxBarInPlaceSubItemControlDrawParams): Integer;
var
  AWidth: Integer;
begin
  if GetPopupMenuLinkArrowWidth(AWidth) then
    Result := ADrawParams.ScaleFactor.Apply(AWidth)
  else
    Result := inherited InPlaceSubItemGetArrowWidth(ADrawParams);
end;

function TdxBarSkinPainter.InPlaceSubItemGetTextIndent: Integer;
begin
  if PopupMenuSideStrip <> nil then
    Result := PopupMenuSideStrip.ContentOffset.Rect.Left
  else
    Result := inherited InPlaceSubItemGetTextIndent;
end;
...
procedure TdxBarSkinPainter.InPlaceSubItemControlDrawArrow(const ADrawParams: TdxBarInPlaceSubItemControlDrawParams;
  ADirection: TcxArrowDirection; ARect: TRect);
begin
  if not DrawArrow(ADrawParams, ARect, True) then
    inherited InPlaceSubItemControlDrawArrow(ADrawParams, ADirection, ARect);
end;
...
begin
  ...
  FSkinPainter.DrawProgressBarPart(ADrawParams.Canvas, BarR, BarR, ADrawParams.PaintType = ptVert,
    dxGetScaleFactor(ADrawParams.BarItemControl), pbpBackground);
...
begin
  FSkinPainter.DrawProgressBarPart(ADrawParams.Canvas, R, R, ADrawParams.PaintType = ptVert,
    dxGetScaleFactor(ADrawParams.BarItemControl), pbpProgressChunk);
end;
...
function TdxBarSkinPainter.DrawArrow(const ADrawParams: TdxBarButtonLikeControlDrawParams; AArrowRect: TRect;
  ARecalcArrowRect: Boolean; ABackButton: Boolean = False): Boolean;
var
  AElement: TdxSkinElement;

  function GetArrowElement: Boolean;
  const
    Direction: array[TdxBarPaintType] of TcxArrowDirection = (adDown, adRight, adRight);
  begin
    AElement := nil;
    if Direction[ADrawParams.PaintType] = adRight then
      AElement := PopupMenuLinkArrow
    else  // adDown
      AElement := BarSubItemArrow;
    Result := AElement <> nil;
  end;
  ...
  begin
    Result := ARect;
    if (ADrawParams.PaintType = ptMenu) and GetPopupMenuLinkArrowWidth(AWidth) then
    begin
      Result.Left := Result.Right - ADrawParams.ScaleFactor.Apply(AWidth);
      if ADrawParams.UseRightToLeftAlignment then
        Result := TdxRightToLeftLayoutConverter.ConvertRect(Result, ARect);
    end
    else if (ADrawParams.PaintType in [ptVert, ptHorz]) and GetBarSubItemArrowWidth(AWidth) then
    begin
      if ADrawParams.PaintType = ptVert then
        Result.Top := Result.Bottom - ADrawParams.ScaleFactor.Apply(AWidth)
      else
        Result.Left := Result.Right - ADrawParams.ScaleFactor.Apply(AWidth);
      if ADrawParams.UseRightToLeftAlignment then
        Result := TdxRightToLeftLayoutConverter.ConvertRect(Result, ARect);
  ...
  function GetArrowState: TdxSkinElementState;
  var
    AIsSelected: Boolean;
    AIsPressed: Boolean;
  begin
    AIsSelected := ((ADrawParams.PaintType = ptMenu) and ADrawParams.DrawSelected) or
      ((ADrawParams.PaintType <> ptMenu) and ADrawParams.IsTextSelected);
    AIsPressed := ADrawParams.DrawDowned or ADrawParams.DroppedDown;
    if ADrawParams.Enabled then
    begin
      if AIsPressed then
        Result := esPressed
      else if AIsSelected then
        Result := esHot
      else
        Result := esNormal;
    end
...
begin
  Result := GetArrowElement;
  if Result then
  begin
    if ARecalcArrowRect then
      AArrowRect := GetArrowRect(AArrowRect);
    if not ABackButton then
      AElement.RightToLeftDependentDraw(ADrawParams.Canvas.Handle, AArrowRect, ADrawParams.ScaleFactor,
        ADrawParams.UseRightToLeftAlignment, 0, GetArrowState)
    else
      AElement.RightToLeftDependentDraw(ADrawParams.Canvas.Handle, AArrowRect, ADrawParams.ScaleFactor,
        not ADrawParams.UseRightToLeftAlignment, 0, GetArrowState)
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      if Canvas.UseRightToLeftAlignment then
        Inc(R.Left, TdxBarItemControlAccess(BarItemControl).FParts[bcpDropButton].Width)
      else
        Dec(R.Right, TdxBarItemControlAccess(BarItemControl).FParts[bcpDropButton].Width);
    end;
...
function TdxBarSkinPainter.GetGlyphColorPalette(ABarItemControl: TdxBarItemControl; APaintType: TdxBarPaintType;
  ASelected, ADowned, ADrawDowned, ADroppedDown, AIsSplit, ADrawImageEnabled: Boolean): IdxColorPalette;
var
  AElement: TdxSkinElement;
begin
  if APaintType = ptMenu then
  ...
  if AElement <> nil then
    Result := AElement.GetGlyphColorPalette(GetState(ABarItemControl.Enabled, ASelected, ADowned,
      ADrawDowned, ADroppedDown, TdxBarItemControlAccess(ABarItemControl).IsMenuItem, ADrawImageEnabled))
  else
    Result := nil;
end;

function TdxBarSkinPainter.GetState(AEnabled, ASelected, ADowned, ADrawDowned, ADroppedDown, AIsMenuItem,
  ADrawImageEnabled: Boolean): TdxSkinElementState;
const
...
begin
  if (not AEnabled) and (not ADrawImageEnabled) then
    Result := esDisabled
  else
...
function TdxBarSkinPainter.BarCaptionColor(ABarControl: TdxBarControl): TColor;
begin
  if FloatingBar <> nil then
...
function TdxBarSkinPainter.GetBarSubItemArrow: TdxSkinElement;
var
  ASkinPainterInfo: TdxSkinInfo;
begin
  Result := nil;
  if GetSkinPainterData(ASkinPainterInfo) then
    Result := ASkinPainterInfo.BarSubItemArrow;
end;

function TdxBarSkinPainter.GetBarSubItemArrowWidth(var AWidth: Integer): Boolean;
var
  APadding: TdxPadding;
begin
  Result := (BarSubItemArrow <> nil) and (not BarSubItemArrow.Image.Empty);
  if Result then
  begin
    APadding := BarSubItemArrow.ContentOffset.Rect;
    AWidth := BarSubItemArrow.Image.Size.cx + APadding.Width;
  end;
end;
...
procedure TdxBarSkinPainter.GetEditTextParams(out AOffsets: TRect; out AHeightCorrection: Integer;
  APainter: TdxBarPainter);
begin
  if IsAdvanced then
  begin
    AOffsets := GetCaptionOffsets;
    AHeightCorrection := 1;
  end
  else
    inherited GetEditTextParams(AOffsets, AHeightCorrection, APainter);
end;
...
function TdxBarSkinPainter.GetIsAdvanced: Boolean;
var
  ASkinPainterInfo: TdxSkinInfo;
begin
  Result := GetSkinPainterData(ASkinPainterInfo) and ASkinPainterInfo.IsAdvanced;
end;
...
function TdxBarSkinPainter.GetPopupMenuLinkArrow: TdxSkinElement;
var
  ASkinPainterInfo: TdxSkinInfo;
begin
  Result := nil;
  if GetSkinPainterData(ASkinPainterInfo) then
    Result := ASkinPainterInfo.PopupMenuLinkArrow;
end;

function TdxBarSkinPainter.GetPopupMenuLinkArrowWidth(var AWidth: Integer): Boolean;
var
  APadding: TdxPadding;
begin
  Result := (PopupMenuLinkArrow <> nil) and (not PopupMenuLinkArrow.Glyph.Empty);
  if Result then
  begin
    APadding := PopupMenuLinkArrow.ContentOffset.Rect;
    AWidth := PopupMenuLinkArrow.Glyph.Size.cx + APadding.Width;
  end;
end;
...
begin
  ...
  if ((Result = Bar[ABarControl.IsRealVertical]) or (Result = MainMenu[ABarControl.IsRealVertical])) and
    (ABarControl is TdxBarControl) and (TdxBarControlAccess(ABarControl).Bar.BorderStyle = bbsNone) then
    Result := Dock;
end;
...
begin
  ...
  begin
    R := ARect;
    if ARect.Width < cxMarginsWidth(AElement.Image.Margins.Margin) then
      R.Left := ARect.Left + ARect.Width - cxMarginsWidth(AElement.Image.Margins.Margin);
    ACanvas.SaveClipRegion;
    try
      ACanvas.IntersectClipRect(ARect);
...
procedure TdxBarSkinPainter.DrawFloatingBarCaptionButton(DC: HDC; ARect: TRect; AContentType: Integer;
  AState: TdxBarMarkState; AScaleFactor: TdxScaleFactor);
const
  MarkState2SkinState: array [TdxBarMarkState] of TdxSkinElementState = (esActive, esHot, esPressed);
begin
  DrawSkinElement(DockControlWindowButton, DC, ARect, AScaleFactor, 0, MarkState2SkinState[AState]);
  DrawSkinElement(DockControlWindowButtonGlyph, DC, ARect, AScaleFactor, AContentType);
end;
...
procedure TdxBarSkinPainter.InternalDrawDockedBarBackground(
  ...
  begin
    ...
    DrawSkinElementParentBackground(ACanvas.Handle, R, ABarControl, AElement);
    DrawSkinElement(AElement, ACanvas.Handle, R, dxGetScaleFactor(ABarControl), 0, esNormal, ABarControlAccess.UseRightToLeftAlignment);
  end;
...
begin
  ...
  begin
    ...
    try
      R1 := R;
      R1.Left := R1.Right - ADrawParams.BarItemControl.ItemBounds.Width;
      ADrawParams.Canvas.IntersectClipRect(R);
...
begin
  ...
  begin
    ...
    begin
      ...
      try
        R1 := R;
        Inc(R1.Right, ABarItem.FParts[bcpDropButton].Width);
...
begin
  ...
  begin
    ...
    AEnabled := ABarItemControl.Enabled;
    if not (AEnabled and AAllowFading and ButtonLikeDrawFadingElement(ABarItemControl, DC, R, IsSplit)) then
    begin
      if Selected or Down then
      begin
        AIsMenuItem := TdxBarItemControlAccess(ABarItemControl).IsMenuItem;
        AState := GetState(AEnabled, Selected, Down, DrawDowned, ADroppedDown, AIsMenuItem, False);
        if AIsMenuItem then
