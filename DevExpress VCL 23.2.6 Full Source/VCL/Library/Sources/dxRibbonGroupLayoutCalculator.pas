...
uses
  Types, Windows, Classes, Contnrs, dxBar, dxRibbon, cxGeometry, dxCore;
...
type
  ...
  private
    function GetItem(Index: TdxListIndex): TRLCItemControlInfo;
  public
    property Items[Index: TdxListIndex]: TRLCItemControlInfo read GetItem; default;
  end;
...
const
  dxThisUnitName = 'dxRibbonGroupLayoutCalculator';
...
begin
  Result := AItemControlViewInfo.IsMultiColumnItemControl(True, AMultiColumnItemControlViewInfo) or
    (AItemControlViewInfo.GetViewLevel in [ivlLargeIconWithText, ivlLargeControlOnly]) or
    Supports(AItemControlViewInfo, IdxBarCompositeItemControlViewInfo, ACompositeItemControlViewInfo)
end;
...
function TSequenceForWholeGroupHeightReducing.GetItem(Index: TdxListIndex): TRLCItemControlInfo;
begin
...
begin
  ...
  begin
    ...
    if Supports(AItemControlViewInfo, IdxBarCompositeItemControlViewInfo, ACompositeItemControlViewInfo) then
      ACompositeItemControlViewInfo.InitializeChildren;
...
begin
  ...
  begin
    ...
    AItemControlViewInfo.SetBounds(R);
    if Supports(AItemControlViewInfo, IdxBarCompositeItemControlViewInfo, ACompositeItemControlViewInfo) then
      ACompositeItemControlViewInfo.OffsetContent(AOffset);
...
begin
  ...
  AItemControlWidth := GetItemControlCurrentWidth(AItemControlViewInfo);
  ABounds := TRect.Create(X, 0, X + AItemControlWidth, GetGroupHeight);
  AItemControlViewInfo.SetBounds(ABounds);
  if Supports(AItemControlViewInfo, IdxBarCompositeItemControlViewInfo, ACompositeItemControlViewInfo)then
    ACompositeItemControlViewInfo.SetBounds(ABounds);
