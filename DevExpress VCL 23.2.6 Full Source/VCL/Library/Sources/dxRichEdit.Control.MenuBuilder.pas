...
uses
  ...
  cxLookAndFeels, dxCore, dxCoreClasses, cxGraphics, dxGDIPlusClasses, cxControls, dxBuiltInPopupMenu,
...
type
  ...
  protected
    ...
    function HasControlPopupMenu: Boolean;
  ...
  private
    procedure ControlPopupMenuItemClickHandler(Sender: TObject);
  ...
  protected
    ...
    procedure AddControlPopupMenuItems;
...
{$R 'dxRichEdit.Control.Images_svg.RES'}

uses
  RTLConsts, Math, Menus,
  dxTypeHelpers,
  dxSpellCheckerCore,
...
const
  dxThisUnitName = 'dxRichEdit.Control.MenuBuilder';

type
  TdxVCLControlAccess = class(TdxVCLControl);
      ...
      try
        ...
        try
          ...
          begin
            ...
            TdxImageDrawer.DrawBitmap(ADestBitmap.Canvas.Handle, AImage, ADestBitmap.ClientRect, ASrcPoint);
...
begin
  ...
  begin
    AResourceStream := TResourceStream.Create(HInstance, 'DXRECPMIMAGES_svg', RT_RCDATA);
    try
      AImages.AddImagesFromZipStream(AResourceStream);
    finally
...
function TdxRichEditCustomPopupMenu.HasControlPopupMenu: Boolean;
begin
  Result := (TdxVCLControlAccess(Control).PopupMenu <> nil) and
    (TdxVCLControlAccess(Control).PopupMenu is TPopupMenu);
end;
...
procedure TdxRichEditContentPopupMenu.AddControlPopupMenuItems;
begin
  TdxBuiltInPopupMenuAdapterHelper.AddMenu(Adapter, TPopupMenu(TdxVCLControlAccess(Control).PopupMenu), ControlPopupMenuItemClickHandler);
end;
...
procedure TdxRichEditContentPopupMenu.ControlPopupMenuItemClickHandler(
  Sender: TObject);
begin
...
begin
  AHasFieldMenu := not HasControlPopupMenu and AddFieldMenu;
  if AHasFieldMenu then
    Adapter.AddSeparator;
  AddSpellCheckerMenuItems;
  if HasControlPopupMenu then
  begin
    AddControlPopupMenuItems;
    Exit;
  end;
