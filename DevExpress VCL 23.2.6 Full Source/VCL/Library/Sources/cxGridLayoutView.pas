...
type
  ...
  public
    constructor Create(AContainer: TdxLayoutContainer; ARecordViewInfo: TcxCustomGridRecordViewInfo); override;
...

{
  ...
  TcxGridLayoutViewRecordExpandButtonPainter = class(TcxGridCustomLayoutRecordExpandButtonPainter)
  private
  ...
  TcxGridLayoutViewRecordPainter = class(TcxGridCustomLayoutRecordPainter)
  private
  ...
  TcxGridLayoutViewRecordExpandButtonViewInfo = class(TcxGridCustomLayoutRecordExpandButtonViewInfo)
  private
    function GetRecordViewInfo: TcxGridLayoutViewRecordViewInfo;
  protected
    function GetPainterClass: TcxCustomGridCellPainterClass; override;
  ...
  private
    ...
    function GetExpandButtonViewInfo: TcxGridLayoutViewRecordExpandButtonViewInfo;
    function GetGridView: TcxGridLayoutView;
  ...
  protected
    ...
    function GetExpandButtonAreaWidth: Integer; override;
    function GetExpandButtonOffset: Integer; override;
    ...
    //internal classes
    function GetExpandButtonViewInfoClass: TcxGridCustomLayoutViewRecordExpandButtonViewInfoClass; override;
  ...
  public
    ...
    property ExpandButtonViewInfo: TcxGridLayoutViewRecordExpandButtonViewInfo read GetExpandButtonViewInfo;
    property GridRecord: TcxGridLayoutViewRecord read GetGridRecord;
    property GridView: TcxGridLayoutView read GetGridView;
  ...
  public
    function GetMouseWheelMajorScrollBarKind: TScrollBarKind; override;
  ...
  { TcxGridLayoutViewCustomizationController }

  TcxGridLayoutViewCustomizationController = class(TcxGridLayoutCustomizationController)
  protected
  ...
  published
    ...
    property ItemFilteredItemsListShowFilteredItemsOnly;
  ...
  protected
    function GetDefaultHotTrack: Boolean; override;
  public
    constructor Create(AGridView: TcxCustomGridView); override;
    procedure Assign(Source: TPersistent); override;
  published
    ...
    property ItemHotTrack: Boolean read GetItemHotTrack write SetItemHotTrack default True;
  ...
  private
    ...
    procedure SetCellBorders(AValue: Boolean);
...
uses
  ...
  cxGridLayoutViewCarouselMode, cxGridLayoutViewCustomizationForm, Dialogs, dxDPIAwareUtils, dxCoreGraphics, RTLConsts;

const
  dxThisUnitName = 'cxGridLayoutView';
...
procedure TcxGridBaseLayoutItemEditViewInfo.GetDefaultValueParams(out AParams: TcxViewParams);
begin
  GridView.Styles.GetDataCellContentParams(GetGridRecord, Item.GridViewItem, AParams);
end;
...
function TcxGridBaseLayoutItemEditViewInfo.GetItem: TcxGridBaseLayoutItem;
begin
...
function TcxGridLayoutItemEditViewInfo.GetGridRecord: TcxCustomGridRecord;
begin
...
begin
  ...
  AItemViewInfo := ItemViewInfo.GridItemViewInfo;
  if AItemViewInfo <> nil then
  begin
    TdxVisualRefinements.HeaderPadding.InflatePadding(Result, AItemViewInfo.ScaleFactor);
    Inc(Result.Right, AItemViewInfo.GetExtraCaptionPadding);
  end;
end;
...
function TcxGridLayoutItemViewInfo.GetCurrentGridItemViewInfo: TcxGridViewLayoutItemDataCellViewInfo;
begin
...
constructor TcxGridLayoutContainerCloneViewInfo.Create(AContainer: TdxLayoutContainer;
  ARecordViewInfo: TcxCustomGridRecordViewInfo);
begin
  inherited Create(AContainer, ARecordViewInfo);
  ItemViewDataList.OnChanged := ItemCloneDataListChangedHandler;
end;
...
begin
  Result := (RecordViewInfo <> nil) and RecordViewInfo.CanShowGroupScrollBars;;
end;
...
function TcxGridLayoutContainerCloneViewInfo.GetCloneDataSize: Integer;
var
  I: Integer;
begin
  Result := 0;
...
begin
  Result := inherited IsActuallyVisible and (RecordViewInfo <> nil) and RecordViewInfo.Visible;
end;

procedure TcxGridLayoutContainerCloneViewInfo.LoadCloneData(AStream: TStream);
var
  I: Integer;
begin
  for I := 0 to ItemViewDataList.Count - 1 do
...
procedure TcxGridLayoutContainerCloneViewInfo.SaveCloneData(AStream: TStream);
var
  I: Integer;
begin
  for I := 0 to ItemViewDataList.Count - 1 do
...
procedure TcxGridLayoutContainerCloneViewInfo.RecalculateCloneDataList;
var
  I: Integer;
begin
  for I := 0 to ItemViewDataList.Count - 1 do
...
procedure TcxGridLayoutContainerCloneViewInfo.ItemCloneDataListChangedHandler(Sender: TObject);
begin
...
begin
  ViewInfo.LookAndFeelPainter.LayoutViewDrawScaledRecordExpandButton(
    Canvas, ViewInfo.Bounds, ViewInfo.ButtonState, ViewInfo.RecordViewInfo.IsExpanded, ScaleFactor);
end;
...
begin
  ViewInfo.LookAndFeelPainter.LayoutViewDrawScaledRecordCaption(Canvas, ViewInfo.Bounds, ViewInfo.TextBounds,
    ViewInfo.RecordViewInfo.RecordCaptionPosition, GetState, ViewInfo.ScaleFactor, ViewInfo.Params.Color, ViewInfo.Params.Bitmap);
end;
...
begin
  if CanDrawBackground then
    with ViewInfo do
      LookAndFeelPainter.LayoutViewDrawScaledRecordContent(Self.Canvas, LayoutAreaBounds,
        RecordCaptionPosition, GetState, ScaleFactor, LayoutAreaBorders);
end;
...
function TcxGridLayoutViewItemViewInfo.CalculateHotTracked: Boolean;
begin
  Result := CanDrawHotTracked and AllowDrawHotTracked and IsHotTracked;
end;

function TcxGridLayoutViewItemViewInfo.CanDrawSelectedWhenEditing: Boolean;
begin
  Result := True;
end;
...
function TcxGridLayoutViewItemViewInfo.CanDrawSelected: Boolean;
begin
...
function TcxGridLayoutViewItemViewInfo.HasBorders: Boolean;
begin
  Result := inherited HasBorders and GridView.OptionsView.CellBorders and not GridView.OptionsBehavior.ItemHotTrack;
end;
...
function TcxGridLayoutViewItemViewInfo.RecalculateOnStateChange: Boolean;
begin
...
begin
  Result := inherited GetVisible and RecordViewInfo.CaptionViewInfo.Visible;
end;

function TcxGridLayoutViewRecordExpandButtonViewInfo.GetRecordViewInfo: TcxGridLayoutViewRecordViewInfo;
begin
  Result := TcxGridLayoutViewRecordViewInfo(inherited RecordViewInfo);
end;
...
function TcxGridLayoutViewRecordViewInfo.GetExpandButtonViewInfo: TcxGridLayoutViewRecordExpandButtonViewInfo;
begin
  Result := TcxGridLayoutViewRecordExpandButtonViewInfo(inherited ExpandButtonViewInfo);
end;
...
begin
  Result := LookAndFeelPainter.LayoutViewRecordCaptionScaledTailSize(GetRecordCaptionPosition, ScaleFactor);
end;
...
function TcxGridLayoutViewRecordViewInfo.CanDrawHotTracked: Boolean;
begin
  Result := False;
end;
...
function TcxGridLayoutViewRecordViewInfo.GetExpandButtonViewInfoClass: TcxGridCustomLayoutViewRecordExpandButtonViewInfoClass;
begin
...
begin
  Result := inherited GetExpandButtonAreaWidth;
  dxAdjustToTouchableSize(Result, ScaleFactor);
end;

function TcxGridLayoutViewRecordViewInfo.GetExpandButtonOffset: Integer;
begin
  Result := ScaleFactor.Apply(cxGridLayoutViewRecordExpandButtonOffset);
end;
...
function TcxGridLayoutViewViewInfo.IsCellPartVisible(ACell: TcxGridTableDataCellViewInfo): Boolean;
var
  ALayoutCell: TcxGridLayoutViewItemViewInfo absolute ACell;
begin
  Result := inherited IsCellPartVisible(ACell) or not cxRectContain(ALayoutCell.VisibleBounds, ALayoutCell.EditBounds);
end;
...
begin
  ...
  AHeight := ScaleFactor.Apply(cxGridLayoutViewRecordExpandButtonOffset) * 2 + LookAndFeelPainter.ScaledGroupExpandButtonSize(ScaleFactor);
  dxAdjustToTouchableSize(AHeight, ScaleFactor);
  GridView.StylesAdapter.GetRecordCaptionParams(nil, AParams);
  AHeight := Max(AHeight, TdxTextMeasurer.TextLineHeight(AParams.Font) + ScaleFactor.Apply(cxGridCellTextOffset) * 2 + TdxVisualRefinements.Padding.ScaledHeight(ScaleFactor));
...
function TcxGridLayoutViewControllerHelper.GetMouseWheelMajorScrollBarKind: TScrollBarKind;
const
  ResultMap: array[Boolean] of TScrollBarKind = (sbVertical, sbHorizontal);
begin
...
function TcxGridLayoutViewCustomizationController.CreateForm: TForm;
begin
  Result := TcxGridLayoutViewCustomizationForm.Create(nil);
end;

function TcxGridLayoutViewCustomizationController.GetFormDefaultHeight: Integer;
begin
  Result := cxGridLayoutViewCustomizationFormDefaultHeight;
end;

function TcxGridLayoutViewCustomizationController.GetFormDefaultWidth: Integer;
begin
...
function TcxGridLayoutViewController.CreateCustomizationController: TcxCustomGridCustomizationController;
begin
  Result := TcxGridLayoutViewCustomizationController.Create(GridView);
end;

function TcxGridLayoutViewController.GetCustomizationFormClass: TcxCustomGridCustomizationFormClass;
begin
  Result := nil;
end;
...
function TcxGridLayoutViewOptionsBehavior.GetDefaultHotTrack: Boolean;
begin
  Result := True;
end;

function TcxGridLayoutViewOptionsBehavior.GetItemHotTrack: Boolean;
begin
  Result := HotTrack;
end;
...
procedure TcxGridLayoutViewOptionsBehavior.SetItemHotTrack(const Value: Boolean);
begin
  HotTrack := Value;
end;
...
begin
  ...
  begin
    ...
    CellBorders := ASource.CellBorders;
...
procedure TcxGridLayoutViewOptionsView.SetCellBorders(AValue: Boolean);
begin
...
procedure TcxGridLayoutViewOptionsView.SetItemPadding(const Value: TcxLayoutViewLookAndFeelItemPadding);
begin
...
begin
  Result := DrawRecordFocused(ARecord) and AItem.Focused and ((ACellViewInfo = nil) or
    TcxGridTableCellViewInfoAccess(ACellViewInfo).AllowDrawSelected);
end;
...
procedure TcxGridLayoutView.DoRefreshStylesCache;
begin
  inherited DoRefreshStylesCache;
...
procedure TcxGridLayoutView.UpdateContainerStyles(ARecord: TcxCustomGridRecord; var ALayoutLookAndFeel: TcxGridLayoutLookAndFeel);
  ...
  begin
    ...
    dxAssignFont(AOptions.Font, AParams.Font, dxGetScaleFactor(ALayoutLookAndFeel), dxGetScaleFactor(Control));
  end;
...
procedure TcxGridLayoutView.ViewModeChanged;
begin
  ViewInfoCache.Reset;
