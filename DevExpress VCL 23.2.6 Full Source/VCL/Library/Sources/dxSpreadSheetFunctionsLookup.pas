...
unit dxSpreadSheetFunctionsLookup; // for internal use
...
const
  dxThisUnitName = 'dxSpreadSheetFunctionsLookup';
...
begin
  if not (Sender.ExtractNumericParameter(ARow, AParams) and Sender.ExtractNumericParameter(AColumn, AParams, 1) and
          ...
          CheckParams(ARow, AColumn, AAbsNum) and Sender.ExtractParameterDef(ASheet, '', '', AParams, 4))
  then
    Exit;
  ...
  ASheetName := VarToStr(ASheet);
  Sender.AddValue(TdxSpreadSheetReferenceAsString.Build(
    TdxSpreadSheetReferenceAsString.BuildViewReference(ASheetName, dxBooleanToDefaultBoolean(Pos(' ', ASheetName) > 0)),
    ARowReference, AColumnReference, 0, 0, AA1RefStyle = 0));
end;

function dxExtractFirstChild(Sender, AResultStorage: TdxSpreadSheetFormulaResult; const AParams: TdxSpreadSheetFormulaToken;
...
begin
  ...
  if (Result is TdxSpreadSheetFormulaParenthesesToken) or (Result is TdxSpreadSheetFormulaFunctionToken) then
    dxSpreadSheetIsReferenceToken(Result, AResultStorage);

  if (Result is TdxSpreadSheetFormulaArrayToken) and (Result.Next <> nil) then
      AErrorCode := ecNA;
end;

function dxExtractArea(Sender, AResultStorage: TdxSpreadSheetFormulaResult; const AParams: TdxSpreadSheetFormulaToken;
  const AAreaNum: Integer; var AErrorCode: TdxSpreadSheetFormulaErrorCode): TdxSpreadSheetFormulaToken;
var
  I: Integer;
begin
  Result := dxExtractFirstChild(Sender, AResultStorage, AParams, AErrorCode);
...
var
  ...
  AResult, AFirstChildResult: TdxSpreadSheetFormulaResult;
  AIsList: Boolean;
begin
  Result := 0;
  AFirstChildResult := TdxSpreadSheetFormulaResult.Create(Sender.Owner);
  try
    AParam := dxExtractFirstChild(Sender, AFirstChildResult, AParams, AErrorCode);
    if AErrorCode = ecNone then
      if (AParam is TdxSpreadSheetFormulaArrayToken) or (AParam is TdxSpreadSheetFormulaCustomDefinedNameToken) then
      begin
    ...
    if not((AErrorCode <> ecNone) or (AParam is TdxSpreadSheetFormulaArrayToken) or (AParam is TdxSpreadSheetFormulaCustomDefinedNameToken)) then
    begin
        ...
        try
          ...
          begin
            AErrorCode := ErrorCodeMap[
              (AExpression is TdxSpreadSheetFormulaReferenceToken) or
              (AExpression is TdxSpreadSheetFormulaArrayToken) or
              (AExpression is TdxSpreadSheetFormulaCustomDefinedNameToken)];
          end;
        finally
          AResult.Free;
        end;
...
var
  ...
  var
    ...
    AValue: Variant;
  begin
    if Sender.ExtractNumericParameter(AValue, AParams) then
    begin
      AIndex := Int(AValue);
...
begin
  ...
  begin
    AParamResult := TdxSpreadSheetFormulaResult.Create(Sender.Owner);
    try
      AParam := dxExtractFirstChild(Sender, AParamResult, AParams, AErrorCode);
      if not dxSpreadSheetIsReferenceToken(AParam, AParamResult) then
        Sender.SetError(ecValue)
      else
        Sender.AddValue(TdxSpreadSheetFormulaReferenceToken(AParam).ActualColumn + 1);
    finally
      AParamResult.Free;
    end;
  end
...
begin
  ...
  begin
    ...
    try
      AParam := dxExtractFirstChild(Sender, AParamResult, AParams, AErrorCode);
      if dxSpreadSheetIsReferenceToken(AParam, AParamResult) or (AParam is TdxSpreadSheetFormulaArrayToken) then
      begin
...
begin
  ...
  begin
    ...
    AParamResult := TdxSpreadSheetFormulaResult.Create(Sender.Owner);
    try
      if dxSpreadSheetIsReferenceToken(AParam, AParamResult) and Supports(AParam.View, IdxSpreadSheetViewData, AViewData) then
      begin
        ACellData := AViewData.GetCellData(
          TdxSpreadSheetFormulaReferenceToken(AParam).ActualRow,
          TdxSpreadSheetFormulaReferenceToken(AParam).ActualColumn);
...
var
  ...
  AErrorCode: TdxSpreadSheetFormulaErrorCode;
  AFoundIndex: Integer;
  ARangeParamResult: TdxSpreadSheetFormulaResult;
begin
  ...
  ARangeParamResult := TdxSpreadSheetFormulaResult.Create(ARangeParam.Owner);
  try
    dxSpreadSheetIsReferenceToken(ARangeParam, ARangeParamResult);
...
procedure fnLookup(Sender: TdxSpreadSheetFormulaResult; const AParams: TdxSpreadSheetFormulaToken);
    ...
    var
      ...
      ATmpResult: TdxSpreadSheetFormulaResult;
    ...
    begin
      ...
      begin
        ...
        ATmpResult := TdxSpreadSheetFormulaResult.Create(AResultRange.Owner);
        try
          dxSpreadSheetIsReferenceToken(AResultRange, ATmpResult);
          ...
          begin
            ...
            begin
              if AResultRange is TdxSpreadSheetFormulaReferenceToken then
                Result := TdxSpreadSheetFormulaReferenceToken(AResultRange).ExtractVector(ALookupVectorLength, True)
              else
            ...
            begin
              if AResultRange is TdxSpreadSheetFormulaReferenceToken then
                Result := TdxSpreadSheetFormulaReferenceToken(AResultRange).ExtractVector(ALookupVectorLength, False)
              else
...
var
  ...
  ALookupRangeResult: TdxSpreadSheetFormulaResult;
...
begin
  ...
  ALookupRangeResult := TdxSpreadSheetFormulaResult.Create(ALookupRange.Owner);
  try
    dxSpreadSheetIsReferenceToken(ALookupRange, ALookupRangeResult);
...
procedure fnIndex(Sender: TdxSpreadSheetFormulaResult; const AParams: TdxSpreadSheetFormulaToken);
  ...
  var
    R: TRect;
  begin
    R.Left := TdxSpreadSheetFormulaReferenceToken(AArea).ActualColumn;
    R.Top := TdxSpreadSheetFormulaReferenceToken(AArea).ActualRow;
    if AArea is TdxSpreadSheetFormulaAreaReferenceToken then
    begin
      R.Right := TdxSpreadSheetFormulaAreaReferenceToken(AArea).ActualColumn2;
      R.Bottom := TdxSpreadSheetFormulaAreaReferenceToken(AArea).ActualRow2;
    end
    else
      R.BottomRight := R.TopLeft;
    if TdxSpreadSheetFormulaReferenceToken(AArea).Column.IsAllItems then
    begin
      R.Left := 0;
      R.Right := dxSpreadSheetMaxColumnIndex;
    end;
    if TdxSpreadSheetFormulaReferenceToken(AArea).Row.IsAllItems then
    begin
      R.Top := 0;
      R.Bottom := dxSpreadSheetMaxRowIndex;
    end;
...
var
  ...
  AResultStorage: TdxSpreadSheetFormulaResult;
...
begin
  ...
  if Sender.ExtractParameterDef(ARowNum, 0, AParams, 1) and
     Sender.ExtractParameterDef(AColumnNum, 0, AParams, 2) and
  ...
  begin
    ...
    if (ARowNum < 0) or (AColumnNum < 0) or (AAreaNum > AAreasCount) then
      ...
      begin
        AResultStorage := TdxSpreadSheetFormulaResult.Create(Sender.Owner);
        try
          AArea := dxExtractArea(Sender, AResultStorage, AParams, AAreaNum, AErrorCode);
          if AErrorCode = ecNone then
            if (ARowNum = 0) and (AColumnNum = 0) then
            begin
              AResultStorage.ExtractToken(AArea, AOwnership);
              if AOwnership = soOwned then
                Sender.AddTemporary(AArea)
              else
                Sender.Add(AArea);
            end
            else
            begin
              ...
              if AErrorCode = ecNone then
                ...
                begin
                  dxSpreadSheetIsReferenceToken(AArea, AResultStorage);
...
procedure fnIndirect(Sender: TdxSpreadSheetFormulaResult; const AParams: TdxSpreadSheetFormulaToken);
  ...
  function CalculateReferenceToken(const AReference: string; AR1C1Reference: Boolean; out AToken: TdxSpreadSheetFormulaToken): Boolean;
  var
    AFinishPos: Integer;
  ...
  begin
    ...
    try
      AStartPos := 1;
      AFinishPos := Length(AReference);
      AParser.FormatSettings.R1C1Reference := AR1C1Reference;
      AParser.UpdateFormula(Sender.Owner);
      AParser.Initialize(AReference, Sender.Owner.AnchorRow, Sender.Owner.AnchorColumn);
      AToken := AParser.GetNextToken(AStartPos, AFinishPos);
      Result := (AToken <> nil) and
        ((AToken is TdxSpreadSheetFormulaReferenceToken) or (AToken is TdxSpreadSheetFormulaCustomDefinedNameToken));
      if not Result and (AToken <> nil) then
...
begin
  AErrorCode := ecNone;
  ARefText := Sender.ExtractStringParameter(AParams);
  if Sender.Validate then
  begin
    if ARefText = '' then
      Sender.SetError(ecRefErr)
  ...
  begin
    ...
    if Sender.Validate and CalculateReferenceToken(ARefText, not Boolean(AIsA1Style), AResultToken) then
      Sender.AddTemporary(AResultToken)
    else
      Sender.SetError(ecRefErr);
  end;
end;
...
procedure fnMatch(Sender: TdxSpreadSheetFormulaResult; const AParams: TdxSpreadSheetFormulaToken);
  ...
  function ExtractLookupVector(out ALookupVector: TdxSpreadSheetVector): Boolean;
  var
    ADimension: TdxSpreadSheetFormulaTokenDimension;
    AErrorCode: TdxSpreadSheetFormulaErrorCode;
    ALookupToken: TdxSpreadSheetFormulaToken;
    ATmpStorage: TdxSpreadSheetFormulaResult;
  begin
    ALookupVector := nil;
    ...
    ATmpStorage := TdxSpreadSheetFormulaResult.Create(Sender.Owner);
    try
      dxSpreadSheetIsReferenceToken(ALookupToken, ATmpStorage);
      AErrorCode := ATmpStorage.ErrorCode;
      if AErrorCode = ecNone then
...
begin
  if not Sender.ExtractParameterDef(ALookupValue, 0, AParams) then
    Exit;
  if not Sender.ExtractNumericParameterDef(AMatchType, 1, 0, AParams, 2) then
    Exit;
  if not ExtractLookupVector(ALookupVector) then
  begin
    FreeAndNil(ALookupVector);
    Exit;
  end;
...
begin
  AResultStorage := TdxSpreadSheetFormulaResult.Create(Sender.Owner);
  try
    AArea := dxExtractArea(Sender, AResultStorage,AParams, 1, AErrorCode);
    if AErrorCode = ecNone then
      ADimension := AArea.GetDimension(AErrorCode);
    if AErrorCode <> ecNone then
      Sender.SetError(AErrorCode)
    ...
    begin
      ...
      begin
        if AArea is TdxSpreadSheetFormulaCustomDefinedNameToken then
        begin
          dxSpreadSheetCalculateExpression(AArea, AResultStorage);
          if TResultAccess(AResultStorage).LastItem is TdxSpreadSheetFormulaReferenceToken then
            AArea := TResultAccess(AResultStorage).LastItem
          else
            Sender.SetError(ecRefErr);
        end;
        if Sender.Validate then
        begin
          R.Left := TdxSpreadSheetFormulaReferenceToken(AArea).ActualColumn + AColumns;
          R.Top := TdxSpreadSheetFormulaReferenceToken(AArea).ActualRow + ARows;
  ...
  begin
    ...
    try
      ...
      if not dxSpreadSheetIsReferenceToken(AParam, AParamResult) then
        ...
        Sender.AddValue(TdxSpreadSheetFormulaReferenceToken(AParam).ActualRow + 1);
    finally
      AParamResult.Free;
    end;
  end
...
begin
  ...
  AParamResult := TdxSpreadSheetFormulaResult.Create(Sender.Owner);
  try
    dxSpreadSheetIsReferenceToken(TdxSpreadSheetFormulaToken(ASource), AParamResult);
...
begin
  ARepository.InternalAdd(@sfnAddress, fnAddress, fpiAddress, frkValue, 219, ftLookupAndReference, @sfnAddressDescription, 0, False);
  ARepository.InternalAdd(@sfnAreas, fnAreas, fpiAreas, frkValue, 75, ftLookupAndReference, @sfnAreasDescription, 0, False);
  ARepository.InternalAdd(@sfnChoose, fnChoose, fpiChoose, frkParamValue, 100, ftLookupAndReference, @sfnChooseDescription);
  ARepository.InternalAdd(@sfnColumn, fnColumn, fpiColumn, frkValue, 9, ftLookupAndReference, @sfnColumnDescription, 0, False);
  ARepository.InternalAdd(@sfnColumns, fnColumns, fpiColumns, frkValue, 77, ftLookupAndReference, @sfnColumnsDescription, 0, False);
  ARepository.InternalAdd(@sfnFormulaText, fnFormulaText, fpiFormulaText, frkValue, 255, ftLookupAndReference, @sfnFormulaTextDescription, 1, False);
  ARepository.InternalAdd(@sfnGetPivotData, nil, nil, frkValue, 358, ftLookupAndReference, @sfnGetPivotDataDescription);
  ARepository.InternalAdd(@sfnHLookup, fnHLookup, fpiHLookup, frkValue, 101, ftLookupAndReference, @sfnHLookupDescription, 0, False);
  ARepository.InternalAdd(@sfnHyperlink, nil, nil, frkValue, 359, ftLookupAndReference, @sfnHyperlinkDescription);
  ARepository.InternalAdd(@sfnLookup, fnLookup, fpiLookup, frkValue, 28, ftLookupAndReference, @sfnLookupDescription, 0, False);
  ARepository.InternalAdd(@sfnIndex, fnIndex, fpiIndex, frkValue, 29, ftLookupAndReference, @sfnIndexDescription, 0, False);
  ARepository.InternalAdd(@sfnIndirect, fnIndirect, fpiIndirect, frkValue, 148, ftLookupAndReference, @sfnIndirectDescription, 0, False);
  ARepository.InternalAdd(@sfnMatch, fnMatch, fpiMatch, frkValue, 64, ftLookupAndReference, @sfnMatchDescription, 0, False);
  ARepository.InternalAdd(@sfnOffset, fnOffset, fpiOffset, frkArray, 78, ftLookupAndReference, @sfnOffsetDescription, 0, False);
  ARepository.InternalAdd(@sfnRow, fnRow, fpiRow, frkValue, 8, ftLookupAndReference, @sfnRowDescription, 0, False);
  ARepository.InternalAdd(@sfnRows, fnRows, fpiRows, frkValue, 76, ftLookupAndReference, @sfnRowsDescription, 0, False);
  ARepository.InternalAdd(@sfnRTD, nil, nil, frkValue, 379, ftLookupAndReference, @sfnRTDDescription);
  ARepository.InternalAdd(@sfnTranspose, fnTranspose, fpiTranspose, frkArray, 83, ftLookupAndReference, @sfnTransposeDescription, 0, False);
  ARepository.InternalAdd(@sfnVLookup, fnVLookup, fpiVLookup, frkValue, 102, ftLookupAndReference, @sfnVLookupDescription, 0, False);
end;
