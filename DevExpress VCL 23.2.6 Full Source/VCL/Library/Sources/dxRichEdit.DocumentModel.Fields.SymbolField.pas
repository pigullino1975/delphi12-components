...
unit dxRichEdit.DocumentModel.Fields.SymbolField; // for internal use
...
const
  dxThisUnitName = 'dxRichEdit.DocumentModel.Fields.SymbolField';

{ TdxSymbolField }

class constructor TdxSymbolField.Initialize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxSymbolField.Initialize', SysInit.HInstance);{$ENDIF}
  FSwitchesWithArgument := CreateSwitchesWithArgument(['f', 's']);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxSymbolField.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxSymbolField.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxSymbolField.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FSwitchesWithArgument);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxSymbolField.Finalize', SysInit.HInstance);{$ENDIF}
end;
