...
uses
  ...
  dxCore, dxCoreGraphics, dxCultureInfo, cxFormats,
...
type
  ...
  public
    ...
    function GetFormatCode(const ALocaleID: Integer): string; overload; virtual; abstract;
    function GetFormatCode: string; overload;
  ...
  public
    constructor Create(const AFormatCode: string; AFormatCodeId: Integer = -1); overload;
    constructor Create(const AFormatCode: string; AFormatCodeId, ALocaleId: Integer); overload;
  ...
  public
    class function GenerateCurrencyFormat(const AFormatSettings: TdxFormatSettings;
...
uses
  dxSpreadSheetCoreStrs, dxSpreadSheetUtils, dxSpreadSheetNumberFormatParser, StrUtils, dxStringHelper;

const
  dxThisUnitName = 'dxSpreadSheetNumberFormat';
...
function TdxSpreadSheetCustomNumberFormat.GetFormatCode: string;
begin
  Result := GetFormatCode(dxGetInvariantLocaleID);
end;
...
begin
  Create(AFormatCode, AFormatCodeId, dxGetInvariantLocaleID);
end;

constructor TdxSpreadSheetNumberFormat.Create(const AFormatCode: string; AFormatCodeId, ALocaleId: Integer);
begin
  inherited Create;
  FFormat := TdxNumberFormatParser.Parse(AFormatCode, AFormatCodeId, ALocaleId);
  if FFormat = nil then
...
function TdxSpreadSheetNumberFormat.GetFormatCode(const ALocaleID: Integer): string;
var
  ABuilder: TdxNumberFormatCodeBuilder;
begin
  ABuilder := TdxNumberFormatCodeBuilder.Create(ALocaleID);
  try
    FFormat.BuildCode(ABuilder);
...
class function TdxSpreadSheetCurrencyFormatHelper.GenerateCurrencyFormat(const AFormatSettings: TdxFormatSettings;
  const AFormatString, APattern, APatternZero: string; ABlindCurrencySymbol: Boolean = False): string;
var
...
begin
  ...
  begin
    if (Length(ACurrencySymbol) = 1) and IsCurrencySymbolFollowsValue(AFormatSettings.NegCurrFormat) then
      ACurrencySymbol := '_' + ACurrencySymbol
    else
      ACurrencySymbol := '';
  end;
  Result := Format(AFormatString, [
    GetPositiveFormat(ACurrencySymbol, APattern,     AFormatSettings.CurrencyFormat, not ABlindCurrencySymbol),
    GetNegativeFormat(ACurrencySymbol, APattern,     AFormatSettings.NegCurrFormat,  not ABlindCurrencySymbol),
    GetPositiveFormat(ACurrencySymbol, APatternZero, AFormatSettings.CurrencyFormat, not ABlindCurrencySymbol)
  ]);
end;
