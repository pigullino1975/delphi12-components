...
type
  ...
  protected
    ...
    function IsCurrencyField: Boolean; override;
  ...
  protected
    ...
    function SkipLocateNotification: Boolean; override;
...
function TcxDBPivotGridDataController.SkipLocateNotification: Boolean;
begin
  Result := True;
end;
...
function TcxDBPivotGridFieldDataBinding.IsCurrencyField: Boolean;
begin
  if DBField is TFloatField then
    Result := TFloatField(DBField).Currency
  else
    if DBField is TBCDField then
      Result := TBCDField(DBField).Currency
    else
      if DBField is TAggregateField then
        Result := TAggregateField(Field).Currency
      else
        if DBField is TFMTBCDField then
          Result := TFMTBCDField(DBField).Currency
        else
          Result := inherited IsCurrencyField;
end;
