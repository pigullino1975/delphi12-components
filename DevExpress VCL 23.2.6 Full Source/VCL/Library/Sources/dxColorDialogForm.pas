...
type
  ...
  TdxFixedGalleryControlPainter = class(TdxGalleryControlPainter)
  public
  ...
  TdxColorDialogFormClass = class of TdxColorDialogForm;
  ...
  protected
    ...
    procedure UpdateGlyph(AItem: TdxGalleryControlItem);
...
var
  dxColorDialogFormClass: TdxColorDialogFormClass = TdxColorDialogForm;
...
const
  dxThisUnitName = 'dxColorDialogForm';
...
procedure TdxColorDialogForm.UpdateGlyph(AItem: TdxGalleryControlItem);
var
  AAlpha: Byte;
  ABitmap: TcxBitmap32;
  AColor: TColor;
...
begin
  ...
  try
    AColor := dxAlphaColorToColor(AItem.Tag, AAlpha);
    if AAlpha < MaxByte then
      cxDrawTransparencyCheckerboard(ABitmap.cxCanvas, ABitmap.ClientRect, cxTransparencyCheckerboardSizeSmall);
...
procedure TdxColorDialogForm.UpdateGlyphs;
var
  I: Integer;
begin
  gcPalette.BeginUpdate;
  try
    for I := 0 to gcPaletteGroup1.Items.Count - 1 do
      UpdateGlyph(gcPaletteGroup1.Items[I]);
    for I := 0 to gcPaletteGroup2.Items.Count - 1 do
      UpdateGlyph(gcPaletteGroup2.Items[I]);
  finally
    gcPalette.EndUpdate;
  end;
end;
...
{ TdxFixedGalleryControlPainter }

procedure TdxFixedGalleryControlPainter.DrawItemSelection(ACanvas: TcxCustomCanvas; AViewInfo: TdxGalleryItemViewInfo);
const
...
var
  ...
  AOffset: Integer;
begin
  ASelectionBounds := AViewInfo.Bounds;
  if LookAndFeelPainter.ApplyEditorAdvancedMode then
  begin
    AOffset := dxcItemGlyphFrameOffset * 2 - ScaleFactor.Apply(1);
    ASelectionBounds.Inflate(AOffset, AOffset);
  end;
  LookAndFeelPainter.DrawGalleryItemSelection(ACanvas, ASelectionBounds, AViewInfo.State);
end;

{ TdxGalleryControl }

function TdxGalleryControl.CreatePainter: TdxGalleryControlPainter;
begin
  Result := TdxFixedGalleryControlPainter.Create(Self);
end;
