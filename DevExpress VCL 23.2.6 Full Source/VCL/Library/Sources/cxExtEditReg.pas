...
uses
  ...
  dxMeasurementUnitEdit, dxTreeView, dxListView, dxListViewEditor, dxTreeViewControlEditor, dxCoreReg, ComCtrls;

const
  dxThisUnitName = 'cxExtEditReg';
...
type
  ...
  TdxTreeViewControlAccess = class(TdxTreeViewControl);
...
type
  ...
  { TdxTreeViewComponentEditor }

  TdxTreeViewComponentEditor = class(TdxComponentEditor)
  private
  ...
  { TdxListViewComponentEditor }

  TdxListViewComponentEditor = class(TdxComponentEditor)
  private
  ...
  { TdxListGroupImageIndexProperty }

  TdxListGroupImageIndexProperty = class(TImageIndexProperty)
  public
    function GetImages: TCustomImageList; override;
  end;

  { TdxListColumnImageIndexProperty }

  TdxListColumnImageIndexProperty = class(TImageIndexProperty)
  public
    function GetImages: TCustomImageList; override;
  end;

  { TdxListViewCollectionProperty }

  TdxListViewCollectionProperty = class(TClassProperty)
  public
    procedure ShowEditor(ATab: TdxListViewEditorTab);
    function GetAttributes: TPropertyAttributes; override;
  end;

  { TdxListColumnsProperty }

  TdxListColumnsProperty = class(TdxListViewCollectionProperty)
  public
    procedure Edit; override;
  end;

  { TdxListGroupsProperty }

  TdxListGroupsProperty = class(TdxListViewCollectionProperty)
  public
    procedure Edit; override;
  end;

  { TdxListItemsProperty }

  TdxListItemsProperty = class(TdxListViewCollectionProperty)
  public
    procedure Edit; override;
  end;

  { TdxTreeViewNodesProperty }

  TdxTreeViewNodesProperty = class(TClassProperty)
  public
    procedure Edit; override;
    function GetAttributes: TPropertyAttributes; override;
  end;
...
procedure TdxTreeViewComponentEditor.DoLinkTo(AObject: TObject);
var
  ATreeView: TTreeView;
begin
  if AObject is TcxTreeView then
    ATreeView := TcxTreeView(AObject).InnerTreeView
  else
    if AObject is TTreeView then
      ATreeView := TTreeView(AObject)
    else
      ATreeView := nil;
  if ATreeView <> nil then
  begin
    TdxTreeViewControlAccess(TreeView).AssignFromTreeView(ATreeView);
    Designer.Modified;
  end;
end;

function TdxTreeViewComponentEditor.GetLinkToTypeClass: TClass;
begin
  Result := TWinControl;
end;

function TdxTreeViewComponentEditor.GetProductName: string;
begin
  Result := cxEditorsLibraryProductName;
end;

function TdxTreeViewComponentEditor.GetTreeView: TdxCustomTreeView;
begin
  Result := Component as TdxCustomTreeView;
end;

procedure TdxTreeViewComponentEditor.InternalExecuteVerb(AIndex: Integer);
begin
  case AIndex of
    0: dxEditTreeViewControl(TreeView);
  else
    inherited InternalExecuteVerb(AIndex);
  end;
end;

function TdxTreeViewComponentEditor.InternalGetVerb(AIndex: Integer): string;
begin
  Result := 'Items Editor...';
end;

function TdxTreeViewComponentEditor.InternalGetVerbCount: Integer;
begin
  Result := 1;
end;

function TdxTreeViewComponentEditor.IsLinkable: Boolean;
begin
  Result := True;
end;

function TdxTreeViewComponentEditor.IsObjectLinkable(AObject: TObject): Boolean;
begin
  Result := (AObject is TcxTreeView) or
    (AObject is TTreeView);
end;

{ TdxListViewComponentEditor }

procedure TdxListViewComponentEditor.DoLinkTo(AObject: TObject);
var
  AListView: TListView;
begin
  if AObject is TcxListView then
    AListView := TcxListView(AObject).InnerListView
  else
    if AObject is TListView then
      AListView := TListView(AObject)
    else
      AListView := nil;
  if AListView <> nil then
  begin
    AssignFromVCLListView(AListView, ListView);
    Designer.Modified;
  end;
end;

function TdxListViewComponentEditor.GetLinkToTypeClass: TClass;
begin
  Result := TWinControl;
end;

function TdxListViewComponentEditor.GetListView: TdxListViewControl;
begin
  Result := Component as TdxListViewControl;
end;

function TdxListViewComponentEditor.GetProductName: string;
begin
  Result := cxEditorsLibraryProductName;
end;

procedure TdxListViewComponentEditor.InternalExecuteVerb(AIndex: Integer);
begin
  case AIndex of
    0: ShowListViewEditor(Designer, ListView, TdxListViewEditorTab.lvetItems);
    1: ShowListViewEditor(Designer, ListView, TdxListViewEditorTab.lvetColumns);
    2: ShowListViewEditor(Designer, ListView, TdxListViewEditorTab.lvetGroups);
  else
    inherited InternalExecuteVerb(AIndex);
  end;
end;

function TdxListViewComponentEditor.InternalGetVerb(AIndex: Integer): string;
begin
  case AIndex of
    0: Result := 'Items Editor...';
    1: Result := 'Columns Editor...';
    2: Result := 'Groups Editor...';
  else
    Result := InternalGetVerb(AIndex);
  end;
end;

function TdxListViewComponentEditor.InternalGetVerbCount: Integer;
begin
  Result := 3;
end;

function TdxListViewComponentEditor.IsLinkable: Boolean;
begin
  Result := True;
end;

function TdxListViewComponentEditor.IsObjectLinkable(AObject: TObject): Boolean;
begin
  Result := (AObject is TcxListView) or (AObject is TListView);
end;

{ TdxListGroupImageIndexProperty }

function TdxListGroupImageIndexProperty.GetImages: TCustomImageList;
begin
  if GetComponent(0) is TdxListGroup then
    Result := TdxListGroup(GetComponent(0)).ListView.ImageOptions.GroupHeaderImages
  else
    Result := nil;
end;
...
function TdxListColumnImageIndexProperty.GetImages: TCustomImageList;
begin
  if GetComponent(0) is TdxListColumn then
    Result := TdxListColumn(GetComponent(0)).ListView.ImageOptions.ColumnHeaderImages
  else
    Result := nil;
end;
...
function TdxListViewCollectionProperty.GetAttributes: TPropertyAttributes;
begin
  Result := [paDialog, paReadOnly, paVCL];
end;

procedure TdxListViewCollectionProperty.ShowEditor(ATab: TdxListViewEditorTab);
var
  AObj: TPersistent;
  AListView: TdxCustomListView;
begin
  AObj := GetComponent(0);
  while (AObj <> nil) and not (AObj is TComponent) do
    AObj := TPersistentAccess(AObj).GetOwner;
  if Safe.Cast(AObj, TdxCustomListView, AListView) then
    ShowListViewEditor(Designer, AListView, ATab);
end;

{ TdxListColumnsProperty }

procedure TdxListColumnsProperty.Edit;
begin
  ShowEditor(TdxListViewEditorTab.lvetColumns);
end;

{ TdxListGroupsProperty }

procedure TdxListGroupsProperty.Edit;
begin
  ShowEditor(TdxListViewEditorTab.lvetGroups);
end;

{ TdxListItemsProperty }

procedure TdxListItemsProperty.Edit;
begin
  ShowEditor(TdxListViewEditorTab.lvetItems);
end;

{ TdxTreeViewNodesProperty }

procedure TdxTreeViewNodesProperty.Edit;
var
  AObj: TPersistent;
...
begin
  ...
  while (AObj <> nil) and not (AObj is TComponent) do
    AObj := TPersistentAccess(AObj).GetOwner;
  if Safe.Cast(AObj, TdxCustomTreeView, ATreeView) then
    dxEditTreeViewControl(ATreeView);
end;

function TdxTreeViewNodesProperty.GetAttributes: TPropertyAttributes;
begin
  Result := [paDialog, paReadOnly, paVCL];
end;
...
begin
  ...
  RegisterComponents(cxEditorsLibraryProductPage, [TcxLabel, TcxProgressBar, TcxTrackBar, TdxZoomTrackBar,
    ...
    TcxCheckGroup, TcxRichEdit, TdxColorEdit, TdxRatingControl, TdxRangeTrackBar, TdxRangeControl, TdxTreeViewControl, TdxListViewControl]);
  RegisterNoIcon([TdxMeasurementUnitEdit, TdxInternalTreeView]);
  RegisterNoIcon([TdxListGroup, TdxListColumn]);
  ...
  RegisterComponents(cxEditorsUtilitiesProductPage, [TcxHintStyleController, TcxSpinButton,
    TcxMCListBox, TcxListView, TcxTreeView, TcxHeader, TcxSplitter, TdxTaskbarProgress, TdxUIAdornerManager, TdxTreeViewControl]);
  RegisterClasses([TdxBadge, TdxGuide]);
  RegisterNoIcon([TdxBadge, TdxGuide]);

  RegisterEditPropertiesIcon(TcxCheckComboBoxProperties, 'TcxCheckComboBox', Hinstance);
  RegisterEditPropertiesIcon(TcxCheckGroupProperties, 'TcxCheckGroup', Hinstance);
  RegisterEditPropertiesIcon(TcxColorComboBoxProperties, 'TcxColorComboBox', Hinstance);
  RegisterEditPropertiesIcon(TcxFontNameComboBoxProperties, 'TcxFontNameComboBox', Hinstance);
  RegisterEditPropertiesIcon(TcxLabelProperties, 'TcxLabel', Hinstance);
  RegisterEditPropertiesIcon(TcxProgressBarProperties, 'TcxProgressBar', Hinstance);
  RegisterEditPropertiesIcon(TdxRangeTrackBarProperties, 'TdxRangeTrackBar', Hinstance);
  RegisterEditPropertiesIcon(TdxRatingControlProperties, 'TdxRatingControl', Hinstance);
  RegisterEditPropertiesIcon(TcxRichEditProperties, 'TcxRichEdit', Hinstance);
  RegisterEditPropertiesIcon(TcxTrackBarProperties, 'TcxTrackBar', Hinstance);
  ...
  RegisterComponentEditor(TdxListViewControl, TdxListViewComponentEditor);
  RegisterPropertyEditor(TypeInfo(TcxImageIndex), TdxListGroup, 'TitleImage', TdxListGroupImageIndexProperty);
  RegisterPropertyEditor(TypeInfo(TcxImageIndex), TdxListColumn, 'ImageIndex', TdxListColumnImageIndexProperty);
  RegisterPropertyEditor(TypeInfo(TdxListColumns), TdxCustomListView, 'Columns', TdxListColumnsProperty);
  RegisterPropertyEditor(TypeInfo(TdxListGroups), TdxCustomListView, 'Groups', TdxListGroupsProperty);
  RegisterPropertyEditor(TypeInfo(TdxListItems), TdxCustomListView, 'Items', TdxListItemsProperty);
  RegisterComponentEditor(TdxTreeViewControl, TdxTreeViewComponentEditor);
  RegisterPropertyEditor(TypeInfo(TdxTreeViewNodes), TdxCustomTreeView, 'Items', TdxTreeViewNodesProperty);
end;
