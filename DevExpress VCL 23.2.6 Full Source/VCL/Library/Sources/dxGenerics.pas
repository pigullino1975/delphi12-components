...
uses
  System.UITypes,
  TypInfo, Types, SysUtils, Classes, Graphics, Generics.Defaults, Generics.Collections, Contnrs,
...
type
  ...
  TdxObjectIntegerDictionary = class(TDictionary<TObject, Integer>);
  TdxObjectDictionary = class(TObjectDictionary<TObject,TObject>);
  ...
  private
    ...
    procedure SetItem(Index: TdxListIndex; const Value: T);
  public
    ...
    property Items[Index: TdxListIndex]: T read GetItem write SetItem; default;
  end;
  ...
  public
    ...
    procedure Sort(ACompare: TdxListSortCompareDelegate); overload;
  ...
  public
    ...
    procedure AddRange(const AEnumerable: TEnumerable<T>); overload;
  ...
  TdxColorList = class(TdxOrdinalList<TColor>);
  ...
  TdxComparableFunc<T> = reference to function (const Arg1: T): Integer;
  ...
  public
    ...
    class function BinarySearch(AList: TList<T>; const APredicate: TdxComparableFunc<T>;
      AStartIndex, AEndIndex: Integer; out AIndex: Integer): Boolean; overload;
    class function BinarySearch(AList: TList<T>; const APredicate: TdxComparableFunc<T>; out AIndex: Integer): Boolean; overload;
  end;
  ...
  public
    ...
    class function BinarySearch(AList: TdxList<T>; const APredicate: TdxComparableFunc<T>;
      AStartIndex, AEndIndex: Integer; out AIndex: Integer): Boolean; overload;
    class function BinarySearch(AList: TdxList<T>; const APredicate: TdxComparableFunc<T>; out AIndex: Integer): Boolean; overload;
  ...
  public
    ...
    function BinarySearch(const APredicate: TdxComparableFunc<T>): Integer; overload;
    procedure Delete(AIndex: Integer);
  ...
  { TdxClassDictionary<T> }

  TdxClassDictionary<T> = class(TcxIUnknownObject)
  strict private
    FData: TDictionary<TClass, T>;
  ...
  public
    ...
    function TryGetValue(AClass: TClass; out AValue: T): Boolean; overload;
    function TryGetValue(AObject: TObject; out AValue: T): Boolean; overload;
...
uses
  Math, RTLConsts, dxHash;

const
  dxThisUnitName = 'dxGenerics';
...
function TdxList<T>.GetItem(Index: TdxListIndex): T;
begin
...
procedure TdxList<T>.SetItem(Index: TdxListIndex; const Value: T);
begin
...
procedure TdxOrdinalList.Sort(ACompare: TdxListSortCompareDelegate);
begin
  dxInternalQuickSortList2(@FList[0], Count, ACompare);
end;
...
procedure TdxOrdinalList<T>.AddRange(const AEnumerable: TEnumerable<T>);
var
  AValue: T;
begin
  for AValue in AEnumerable do
    Add(AValue);
end;
...
begin
  Result := NativeInt(inherited Pop);
end;
...
begin
  ...
  begin
    AMedian := AIndex + ((AHi - AIndex) shr 1);
    ACompareResult := APredicate.CompareTo(T(AList.List[AMedian]));
    if ACompareResult = 0 then
    begin
      AIndex := AMedian;
      Exit(True);
...
class function TdxAlgorithms<T>.BinarySearch(AList: TList<T>; const APredicate: TdxComparableFunc<T>;
  out AIndex: Integer): Boolean;
begin
  Result := BinarySearch(AList, APredicate, 0, AList.Count - 1, AIndex);
end;

class function TdxAlgorithms<T>.BinarySearch(AList: TList<T>; const APredicate: TdxComparableFunc<T>; AStartIndex,
...
begin
  ...
  begin
    AMedian := AIndex + (AHi - AIndex) shr 1;
    ACompareResult := APredicate(AList.List[AMedian]);
    if ACompareResult = 0 then
    begin
      AIndex := AMedian;
      Exit(True);
...
class function TdxAlgorithms1<T>.BinarySearch(AList: TdxList<T>; const APredicate: TdxComparableFunc<T>;
  out AIndex: Integer): Boolean;
begin
  Result := BinarySearch(AList, APredicate, 0, AList.Count - 1, AIndex);
end;

class function TdxAlgorithms1<T>.BinarySearch(AList: TdxList<T>; const APredicate: TdxComparableFunc<T>; AStartIndex,
  AEndIndex: Integer; out AIndex: Integer): Boolean;
var
  AHi, AMedian, ACompareResult: Integer;
begin
  ...
  begin
    AMedian := AIndex + (AHi - AIndex) shr 1;
    ACompareResult := APredicate(T(AList.List[AMedian]));
    if ACompareResult = 0 then
    begin
      AIndex := AMedian;
      Exit(True);
...
function TdxSortedList<T>.BinarySearch(const APredicate: TdxComparableFunc<T>): Integer;
begin
  if not TdxAlgorithms<T>.BinarySearch(FInnerList, APredicate, Result) then
    Result := not Result; 
end;
...
constructor TdxClassDictionary<T>.Create;
begin
  FData := TDictionary<TClass, T>.Create;
end;

destructor TdxClassDictionary<T>.Destroy;
begin
  FreeAndNil(FData);
  inherited Destroy;
end;

procedure TdxClassDictionary<T>.Add(const AClass: TClass; const AValue: T);
begin
  FData.Add(AClass, AValue);
end;

procedure TdxClassDictionary<T>.AddOrSetValue(const AClass: TClass; const AValue: T);
begin
  FData.AddOrSetValue(AClass, AValue);
end;

procedure TdxClassDictionary<T>.Remove(const AClass: TClass);
begin
  FData.Remove(AClass);
end;

function TdxClassDictionary<T>.TryGetValue(AObject: TObject; out AValue: T): Boolean;
begin
  if AObject <> nil then
...
function TdxClassDictionary<T>.TryGetValue(AClass: TClass; out AValue: T): Boolean;
begin
  repeat
...
function TdxClassDictionary<T>.GetCount: Integer;
begin
  Result := FData.Count;
end;

function TdxClassDictionary<T>.GetItem(AClass: TClass): T;
begin
