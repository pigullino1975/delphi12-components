...
uses
  ...
  cxClasses, dxHashUtils, dxSpreadSheetInplaceEdit, cxCustomCanvas;
...
type
  ...
  strict private
    ...
    function GetUseRightToLeftLayout: Boolean; inline;
  protected
  ...
  public
    ...
    property UseRightToLeftLayout: Boolean read GetUseRightToLeftLayout;
  end;
  ...
  protected
    ...
    function IsContainerFocused: Boolean;
    ...
    procedure DoRenameContainer(AContainer: TdxSpreadSheetContainer; const AName: string); virtual;
    procedure DoSelectArea(const AView: TdxSpreadSheetTableView; const AArea: TRect); virtual;
    procedure DoSelectContainer(AContainer: TdxSpreadSheetContainer); virtual;
...
const
  dxThisUnitName = 'dxSpreadSheetFormulaBar';
...
begin
  ...
  ButtonBox.Enabled := FormulaEdit.Enabled and not FormulaEdit.Properties.ReadOnly;
...
procedure TdxSpreadSheetAbstractFormulaBar.DrawOpaqueBackground(ACanvas: TcxCanvas; const ARect: TRect);
begin
  LookAndFeelPainter.DrawPanelContent(ACanvas, ARect);
end;
...
procedure TdxSpreadSheetFormulaBarInplaceEditScrollBarPainter.Paint(ACanvas: TcxCustomCanvas);
begin
  inherited;
  DrawExpandButton(ACanvas as TcxCanvas, ViewInfo.ExpandButtonRect, ViewInfo.ExpandButtonState, not cxRectIsEmpty(ViewInfo.ScrollBarBounds));
end;
...
var
  ADelta: Integer;
begin
  ADelta := P.X - FCapturePoint.X;
  if Control.IsRightToLeft then
    ADelta := -ADelta;
  Control.SeparatorPosition := Max(0, Min(FSeparatorPosition + ADelta, FSeparatorPositionMax));
  Control.Update;
end;
...
procedure TdxSpreadSheetFormulaBarViewInfo.Calculate(ABounds: TRect);
  ...
  function Place(var R: TRect; AWidth: Integer): TRect;
  begin
    if UseRightToLeftLayout then
    begin
      Result := cxRectSetRight(R, R.Right, AWidth);
      Result := cxRectSetHeight(Result, LineHeight);
      if AWidth > 0 then
        R.Right := Result.Left - IndentBetweenElements;
    end
    else
    begin
      Result := cxRectSetSize(R, AWidth, LineHeight);
      if AWidth > 0 then
        R.Left := Result.Right + IndentBetweenElements;
    end;
  end;

var
  AButtons: TcxEditButtons;
  AButtonWidth, AButtonBoxWidth: Integer;
  I: Integer;
begin
  ...
  begin
    FNameBoxRect := Place(ABounds, Bar.SeparatorPosition);
    FSeparatorRect := Place(ABounds, Painter.SpreadSheetFormulaBarGetScaledSeparatorSize(ScaleFactor));
    FSeparatorRect := cxRectInflate(SeparatorRect, IndentBetweenElements, 0);
  end
  else
  begin
    FNameBoxRect := Place(ABounds, 0);
    FSeparatorRect := Place(ABounds, 0);
  end;

  if Bar.OptionsView.ShowButtons then
  begin
    AButtonWidth := LineHeight;
    dxAdjustToTouchableSize(AButtonWidth, ScaleFactor);

    AButtons := Bar.ButtonBox.Properties.Buttons;
    AButtons.BeginUpdate;
    try
      for I := 0 to AButtons.Count - 1 do
        AButtons[I].Width := AButtonWidth;
    finally
      AButtons.EndUpdate;
    end;

    AButtonBoxWidth := AButtonWidth * 3;
    if Painter.SupportsEditorBorders then
      Inc(AButtonBoxWidth, 2 * Painter.BorderSize)
    else
      Inc(AButtonBoxWidth, Painter.GetEditorButtonsPadding(ScaleFactor).Left + Painter.GetEditorButtonsPadding(ScaleFactor).Right);

    FButtonBoxRect := Place(ABounds, AButtonBoxWidth);
  end;
...
function TdxSpreadSheetFormulaBarViewInfo.MeasureLineHeight: Integer;
  ...
  procedure AdjustLineHeight(ACanvas: TcxCanvas; AEdit: TcxCustomEditAccess; var AHeight: Integer);
  var
  ...
  begin
    ...
    try
      AHeight := Max(AHeight, AViewData.GetEditSize(ACanvas, AEdit.EditValue, ASizeProperties, AEdit.ViewInfo).cy);
    finally
      AViewData.Free;
    end;
  end;
...
begin
  Result := 0;
  AdjustLineHeight(cxScreenCanvas, TcxCustomEditAccess(Bar.NameBox), Result);
  AdjustLineHeight(cxScreenCanvas, TcxCustomEditAccess(Bar.ButtonBox), Result);
...
function TdxSpreadSheetFormulaBarViewInfo.GetUseRightToLeftLayout: Boolean;
begin
  Result := Bar.IsRightToLeft;
end;
...
procedure TdxSpreadSheetCustomFormulaBar.DoRenameContainer(AContainer: TdxSpreadSheetContainer; const AName: string);
begin
...
procedure TdxSpreadSheetCustomFormulaBar.DoSelectContainer(AContainer: TdxSpreadSheetContainer);
begin
...
begin
  ...
  try
    TdxSpreadSheetDefinedName(AObject).EnumReferences(
      ...
      begin
        if View is TdxSpreadSheetTableView then
  ...
  if ActiveSheet.Containers.FindByName(AText, AContainer) then
    DoSelectContainer(AContainer)
  else
  begin
    AContainer := GetFocusedContainer;
    if AContainer <> nil then
      DoRenameContainer(AContainer, AText)
    else
...
function TdxSpreadSheetCustomFormulaBar.IsContainerFocused: Boolean;
begin
  Result := GetFocusedContainer <> nil;
end;
...
begin
  FValidationErrorOccurs := False;
  if (EditingController <> nil) and not IsContainerFocused then
  begin
...
begin
  ...
  FormulaEdit.Properties.ReadOnly := IsContainerFocused;
...
function TdxSpreadSheetCustomFormulaBar.GetFocusedContainer: TdxSpreadSheetContainer;
var
  ASheet: TdxSpreadSheetTableView;
begin
  ASheet := ActiveSheet;
  if ASheet <> nil then
    Result := ASheet.Controller.FocusedContainer
  else
    Result := nil;
end;
