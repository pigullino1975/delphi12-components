...
uses
  ...
  dxSpreadSheetCore, dxSpreadSheetClasses, dxSpreadSheetPackedFileFormatCore, dxSpreadSheetFormatODSHelpers, cxClasses,
  ...
  dxSpreadSheetConditionalFormatting, dxGenerics;
...
type
  ...
  strict private
    FDisplayNameMap: TdxStringsDictionary;
  protected
  ...
  TdxSpreadSheetODSReaderCustomParser = class(TdxSpreadSheetCustomFilerSubTask)
  strict private
  ...
  strict private
    ...
    function ReadXML(const AFileName: AnsiString): TdxXMLDocument;
  public
  ...
  public
    constructor Create(ANode: TdxXMLNode; AViewParser: TdxSpreadSheetODSReaderTableViewParser; ABaseCell: TdxSpreadSheetCell = nil);
    destructor Destroy; override;
    procedure Execute; override;
    //
  ...
  { TdxSpreadSheetODSReaderMetadataFileParser }

  TdxSpreadSheetODSReaderMetadataFileParser = class(TdxSpreadSheetODSReaderCustomDocumentParser)
  strict private
    function GetMetadata: TdxSpreadSheetOptionsMetadata;
    function ReadDateTime(AParentNode: TdxXMLNode; const AValueName: AnsiString): TDateTime; inline;
    function ReadText(AParentNode: TdxXMLNode; const AValueName: AnsiString): string; inline;
  public
    procedure Execute; override;
    //
  ...
  public
    ...
    property ContainersZOrder: TList read FContainersZOrder;
...
const
  dxThisUnitName = 'dxSpreadSheetFormatODSReader';
...
begin
  inherited Create;
  FDisplayNameMap := TdxStringsDictionary.Create;
end;
...
begin
  ...
  begin
    ...
    if PackageReader.Exists(sdxODSMetadataFile) then
      ExecuteSubTask(TdxSpreadSheetODSReaderMetadataFileParser.Create(sdxODSMetadataFile, Self));
...
function TdxSpreadSheetODSReaderCustomDocumentParser.ReadXML(const AFileName: AnsiString): TdxXMLDocument;
var
  AStream: TMemoryStream;
begin
  AStream := Owner.ReadFile(AFileName);
  try
    Result := TdxXMLDocument.Create;
...
constructor TdxSpreadSheetODSReaderCustomContainerParser.Create(
  ANode: TdxXMLNode; AViewParser: TdxSpreadSheetODSReaderTableViewParser; ABaseCell: TdxSpreadSheetCell = nil);
begin
...
var
  ...
  AOffset: TPoint;
begin
  Container.Name := Node.Attributes.GetValueAsString(sdxODSAttrDrawName);
...
begin
  ...
  if Node.FindChild(sdxODSNodeText, ATextNode) or Node.FindChild([sdxODSAttrDrawTextBox, sdxODSNodeText], ATextNode) then
  begin
    ...
    try
      ...
      AHelper := TdxSpreadSheetODSReaderRichTextHelper.Create(Owner);
      try
...
constructor TdxSpreadSheetODSReaderRichTextHelper.Create(AOwner: TdxSpreadSheetODSReader);
begin
  inherited Create;
  Owner := AOwner;
...
begin
  if Owner <> nil then
    Result := Owner.CellStyles.DefaultStyle.Font
  else
    Result := nil;
end;
...
begin
  if (Owner <> nil) and Owner.Styles.TryGetValue(ANode.Attributes.GetValueAsString(sdxODSAttrTextStyleName), AStyleHandle) then
    Result := AStyleHandle.Font
  else
    Result := nil
end;
...
procedure TdxSpreadSheetODSReaderMetadataFileParser.Execute;
var
  ANode: TdxXMLNode;
begin
  if Node.FindChild(sdxODSNodeOfficeMeta, ANode) then
  begin
    Metadata.Created := ReadDateTime(ANode, sdxODSNodeCreationDate);
    Metadata.CreatedBy := ReadText(ANode, sdxODSNodeInitialCreator);
    Metadata.Description := ReadText(ANode, sdxODSNodeDescription);
    Metadata.Keywords := ReadText(ANode, sdxODSNodeKeywords);
    Metadata.LastModifiedBy := ReadText(ANode, sdxODSNodeCreator);
    Metadata.LastModified := ReadDateTime(ANode, sdxODSNodeModificationDate);
    Metadata.LastPrinted := ReadDateTime(ANode, sdxODSNodeMetaPrintDate);
    Metadata.Subject := ReadText(ANode, sdxODSNodeSubject);
    Metadata.Title := ReadText(ANode, sdxODSNodeTitle);
  end;
end;

function TdxSpreadSheetODSReaderMetadataFileParser.ReadDateTime(AParentNode: TdxXMLNode; const AValueName: AnsiString): TDateTime;
begin
  if AParentNode.FindChild(AValueName, AParentNode) and (AParentNode.TextAsString <> '') then
    Result := AParentNode.TextAsDateTime
  else
    Result := 0;
end;

function TdxSpreadSheetODSReaderMetadataFileParser.ReadText(AParentNode: TdxXMLNode; const AValueName: AnsiString): string;
begin
  if AParentNode.FindChild(AValueName, AParentNode) then
    Result := AParentNode.TextAsString
  else
    Result := '';
end;

function TdxSpreadSheetODSReaderMetadataFileParser.GetMetadata: TdxSpreadSheetOptionsMetadata;
begin
  Result := Owner.SpreadSheet.OptionsMetadata;
end;
...
begin
  ...
  FContainersZOrder := TList.Create;
end;
...
begin
  ...
  if Node.FindChild(sdxODSNodeTableShapes, ASubNode) then
    ASubNode.ForEach(ProcessDrawObject);
...
begin
  ...
  begin
    AContainer := ContainersZOrder.List[I];
    if AContainer <> nil then
    begin
      AContainer.Index := AIndex;
      Inc(AIndex);
    end;
  end;
end;
...
procedure TdxSpreadSheetODSReaderTableViewParser.ProcessDrawObject(ANode: TdxXMLNode; AUserData: Pointer);
var
...
begin
  ...
  begin
    if ANode.FindChild([sdxODSAttrDrawTextBox, sdxODSNodeText], ASubNode) then
  ...
  if AParserClass <> nil then
    ExecuteSubTask(AParserClass.Create(ANode, Self, TdxSpreadSheetCell(AUserData)));
end;
...
var
  ...
  ACellNext: TdxSpreadSheetCell;
begin
  ACell := ARow.FirstCell;
  while ACell <> nil do
  begin
    ACellNext := ACell.Next;
...
begin
  ...
  ANode.ForEach(TableViewParser.ProcessDrawObject, ACell);
end;
