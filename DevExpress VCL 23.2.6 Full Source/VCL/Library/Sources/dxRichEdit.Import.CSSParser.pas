...
unit dxRichEdit.Import.CSSParser; // for internal use
...
type
  ...
  strict private
    ...
    FRawProperties: TdxStringsDictionary;
    FCssFloat: TdxHtmlCssFloat;
    FPieceTable: TdxPieceTable;
    function GetHasRawProperties: Boolean;
    function GetRelativeProperties: PdxRelativeProperties; inline;
    function GetUnitConverter: TdxDocumentModelUnitConverter;
    function GetRawProperties: TdxStringsDictionary;
  protected
    function IsDefaultProperties: Boolean;
  ...
  public
    ...
    property HasRawProperties: Boolean read GetHasRawProperties;
    property RawProperties: TdxStringsDictionary read GetRawProperties;
...
const
  dxThisUnitName = 'dxRichEdit.Import.CSSParser';
...
begin
  ...
  FreeAndNil(FBordersProperties);
  FreeAndNil(FImageProperties);
  inherited Destroy;
end;

function TdxCssProperties.GetHasRawProperties: Boolean;
begin
  Result := (Self <> nil) and (FRawProperties <> nil) and (FRawProperties.Count > 0);
end;

function TdxCssProperties.GetRawProperties: TdxStringsDictionary;
begin
  if FRawProperties = nil then
    FRawProperties := TdxStringsDictionary.Create;
  Result := FRawProperties;
end;
...
begin
  ...
  if AValue.FRawProperties <> nil then
    for ARawItem in AValue.FRawProperties do
      RawProperties.Add(ARawItem.Key, ARawItem.Value)
  else
    FreeAndNil(FRawProperties);
end;
