...
uses
  ...
  dxCore, dxCoreClasses, dxCoreGraphics, cxGraphics, dxSpreadSheetNumberFormatCore;
...
type
  ...
  public
    ...
    procedure AddIfAbsent(const AKey: Char; AValue: Integer);
    procedure Append(const AKey: Char; AValue: Integer);
  end;
  ...
  strict private
    ...
    procedure GenerateDesignators;
  public
    constructor Create(const ALocaleID: Integer);
  ...
  { TdxNumberFormatCodeBuilder }

  TdxNumberFormatCodeBuilder = class(TdxNumberFormatCodeAbstractBuilder)
  strict private
    FBuilder: TStringBuilder;
    FLocalizer: TdxNumberFormatLocalizer;
  protected
    function GetFormatSettings: TFormatSettings; override;
  public
    constructor Create(const ALocaleID: Integer);
    destructor Destroy; override;
    procedure Append(const V: string); override;
    procedure AppendDesignator(ID: Integer); override;
    function MustBeEscaped(AChar: Char): Boolean; override;
  ...
  public
    constructor Create(const ALocaleID: Integer);
    destructor Destroy; override;
    class function Parse(const S: string; AFormatCodeID: Integer = -1): TdxNumberFormat; overload;
    class function Parse(const S: string; AFormatCodeID, ALocaleID: Integer): TdxNumberFormat; overload;
  end;
...
const
  dxThisUnitName = 'dxSpreadSheetNumberFormatParser';

type

  { TdxNumberFormatDesignatorFactory }

  TdxNumberFormatDesignatorFactory = class
  strict private type
    TProc = procedure (ADesignators: TdxNumberFormatDesignatorMap);
  strict private
    class var FFactory: TDictionary<Integer, TProc>;

    class procedure GenerateCzechDesignators(ADesignators: TdxNumberFormatDesignatorMap); static;
    class procedure GenerateDanishDesignators(ADesignators: TdxNumberFormatDesignatorMap); static;
    class procedure GenerateDutchDesignators(ADesignators: TdxNumberFormatDesignatorMap); static;
    class procedure GenerateFinnishDesignators(ADesignators: TdxNumberFormatDesignatorMap); static;
    class procedure GenerateFrenchDesignators(ADesignators: TdxNumberFormatDesignatorMap); static;
    class procedure GenerateGermanDesignators(ADesignators: TdxNumberFormatDesignatorMap); static;
    class procedure GenerateGreekDesignators(ADesignators: TdxNumberFormatDesignatorMap); static;
    class procedure GenerateHungarianDesignators(ADesignators: TdxNumberFormatDesignatorMap); static;
    class procedure GenerateInvariantDesignators(ADesignators: TdxNumberFormatDesignatorMap); static;
    class procedure GenerateItalianDesignators(ADesignators: TdxNumberFormatDesignatorMap); static;
    class procedure GeneratePolishDesignators(ADesignators: TdxNumberFormatDesignatorMap); static;
    class procedure GeneratePortugueseBrazilDesignators(ADesignators: TdxNumberFormatDesignatorMap); static;
    class procedure GeneratePortuguesePortugalDesignators(ADesignators: TdxNumberFormatDesignatorMap); static;
    class procedure GenerateRussianDesignators(ADesignators: TdxNumberFormatDesignatorMap); static;
    class procedure GenerateSpanishDesignators(ADesignators: TdxNumberFormatDesignatorMap); static;
    class procedure GenerateTurkishDesignators(ADesignators: TdxNumberFormatDesignatorMap); static;
    class procedure PopulateFactory;
  public
    class procedure Initialize(ALocaleID: Integer; ADesignators: TdxNumberFormatDesignatorMap);
    class procedure Finalize;
  end;

{ TdxNumberFormatDesignatorMap }

procedure TdxNumberFormatDesignatorMap.AddIfAbsent(const AKey: Char; AValue: Integer);
begin
  if not ContainsKey(AKey) then
    Add(AKey, AValue);
end;

procedure TdxNumberFormatDesignatorMap.Append(const AKey: Char; AValue: Integer);
var
  APrevValue: Integer;
begin
  if TryGetValue(AKey, APrevValue) then
    AddOrSetValue(AKey, APrevValue or AValue)
  else
    Add(AKey, AValue);
end;
...
constructor TdxNumberFormatLocalizer.Create(const ALocaleID: Integer);

  procedure CheckAddChar(ADesignator, ASymbolDesignator: Integer; ASymbol: Char; var AHasMatch: Boolean);
  begin
    if ADesignator and ASymbolDesignator <> 0 then
    begin
      if not FChars.ContainsKey(ASymbolDesignator) then
        FChars.Add(ASymbolDesignator, ASymbol);
      AHasMatch := True;
    end;
  end;
...
begin
  ...
  dxGetLocaleFormatSettings(LocaleID, FFormatSettings);
  ...
  begin
    ...
    CheckAddChar(ADesignator, TdxNumberFormatDesignator.AmPm, AKey, AHasMatch);
    CheckAddChar(ADesignator, TdxNumberFormatDesignator.Year, AKey, AHasMatch);
    CheckAddChar(ADesignator, TdxNumberFormatDesignator.InvariantYear, AKey, AHasMatch);
    CheckAddChar(ADesignator, TdxNumberFormatDesignator.Month, AKey, AHasMatch);
    CheckAddChar(ADesignator, TdxNumberFormatDesignator.Minute, AKey, AHasMatch);
    CheckAddChar(ADesignator, TdxNumberFormatDesignator.DateSeparator, AKey, AHasMatch);
    CheckAddChar(ADesignator, TdxNumberFormatDesignator.DecimalSeparator, AKey, AHasMatch);
    CheckAddChar(ADesignator, TdxNumberFormatDesignator.FractionOrDateSeparator, AKey, AHasMatch);
    CheckAddChar(ADesignator, TdxNumberFormatDesignator.GroupSeparator, AKey, AHasMatch);
    CheckAddChar(ADesignator, TdxNumberFormatDesignator.TimeSeparator, AKey, AHasMatch);
    CheckAddChar(ADesignator, TdxNumberFormatDesignator.Day, AKey, AHasMatch);
    CheckAddChar(ADesignator, TdxNumberFormatDesignator.Second, AKey, AHasMatch);
    CheckAddChar(ADesignator, TdxNumberFormatDesignator.General, AKey, AHasMatch);
    CheckAddChar(ADesignator, TdxNumberFormatDesignator.JapaneseEra, AKey, AHasMatch);
    CheckAddChar(ADesignator, TdxNumberFormatDesignator.DayOfWeek, AKey, AHasMatch);
    CheckAddChar(ADesignator, TdxNumberFormatDesignator.Hour, AKey, AHasMatch);
    if not AHasMatch then
...
procedure TdxNumberFormatLocalizer.GenerateDesignators;
var
  ADateSeparator: Char;
...
begin
  ...
  AGroupSeparator := FormatSettings.ThousandSeparator;
  ...
  FDesignators.Append(ADecimalSeparator, TdxNumberFormatDesignator.DecimalSeparator);
  FDesignators.Append(ADateSeparator, TdxNumberFormatDesignator.DateSeparator);
  FDesignators.Append(AGroupSeparator, TdxNumberFormatDesignator.GroupSeparator);
  FDesignators.Append(ATimeSeparator, TdxNumberFormatDesignator.TimeSeparator);
  FDesignators.Append('/', TdxNumberFormatDesignator.FractionOrDateSeparator);

  TdxNumberFormatDesignatorFactory.Initialize(LocaleID, FDesignators);

  FDesignators.AddIfAbsent('*', TdxNumberFormatDesignator.Asterisk);
  FDesignators.AddIfAbsent('@', TdxNumberFormatDesignator.At);
  FDesignators.AddIfAbsent('\', TdxNumberFormatDesignator.Backslash);
  FDesignators.AddIfAbsent('[', TdxNumberFormatDesignator.Bracket);
  FDesignators.AddIfAbsent('#', TdxNumberFormatDesignator.DigitEmpty);
  FDesignators.AddIfAbsent('?', TdxNumberFormatDesignator.DigitSpace);
  FDesignators.AddIfAbsent('0', TdxNumberFormatDesignator.DigitZero);
  FDesignators.AddIfAbsent(';', TdxNumberFormatDesignator.EndOfPart);
  FDesignators.AddIfAbsent('E', TdxNumberFormatDesignator.Exponent);
  FDesignators.AddIfAbsent('e', TdxNumberFormatDesignator.InvariantYear);
  FDesignators.AddIfAbsent('%', TdxNumberFormatDesignator.Percent);
  FDesignators.AddIfAbsent('"', TdxNumberFormatDesignator.Quote);
  FDesignators.AddIfAbsent('_', TdxNumberFormatDesignator.Underline);
  FDesignators.AddIfAbsent('b', TdxNumberFormatDesignator.ThaiYear);

  FDesignators.Append(dxLowerCase(Designators.General[1]), TdxNumberFormatDesignator.General);
  FDesignators.Append(dxUpperCase(Designators.General[1]), TdxNumberFormatDesignator.General);
end;

{ TdxNumberFormatCodeBuilder }

constructor TdxNumberFormatCodeBuilder.Create(const ALocaleID: Integer);
begin
  inherited Create;
  FBuilder := TStringBuilder.Create(256);
  FLocalizer := TdxNumberFormatLocalizer.Create(ALocaleID);
end;

destructor TdxNumberFormatCodeBuilder.Destroy;
begin
...
procedure TdxNumberFormatCodeBuilder.Append(const V: string);
begin
  FBuilder.Append(V);
end;

procedure TdxNumberFormatCodeBuilder.AppendDesignator(ID: Integer);
var
  AValue: Char;
begin
  if ID = TdxNumberFormatDesignator.Default then
    Exit;
  if ID = TdxNumberFormatDesignator.General then
    FBuilder.Append(FLocalizer.Designators.General)
  else
    if FLocalizer.Chars.TryGetValue(ID, AValue) then
      FBuilder.Append(AValue)
    else
      raise EInvalidArgument.CreateFmt('No one character is associated with %d designator', [ID]);
end;

function TdxNumberFormatCodeBuilder.MustBeEscaped(AChar: Char): Boolean;
begin
  AChar := dxLowerCase(AChar);
  Result := (Ord(AChar) = Ord('n')) or FLocalizer.Designators.ContainsKey(AChar);
end;

function TdxNumberFormatCodeBuilder.ToString: string;
begin
  Result := FBuilder.ToString;
end;

function TdxNumberFormatCodeBuilder.GetFormatSettings: TFormatSettings;
begin
  Result := FLocalizer.FormatSettings;
end;

{ TdxNumberFormatParser }

constructor TdxNumberFormatParser.Create(const ALocaleID: Integer);
begin
...
class function TdxNumberFormatParser.Parse(const S: string; AFormatCodeID: Integer): TdxNumberFormat;
begin
  Result := Parse(S, AFormatCodeID, dxGetInvariantLocaleID);
end;

class function TdxNumberFormatParser.Parse(const S: string; AFormatCodeID, ALocaleID: Integer): TdxNumberFormat;
begin
  if S = '' then
    Exit(TdxGeneralNumberFormat.Create);
  if AFormatCodeID = 14 then
    Exit(TdxDateTimeSystemShortDateNumberFormat.Create(nil));
  if AFormatCodeID = 22 then
    Exit(TdxDateTimeSystemFullDateTimeNumberFormat.Create(nil));

  with TdxNumberFormatParser.Create(ALocaleID) do
  try
    FFormatString := S;
    Result := ParseCore;
  finally
...
begin
  Result := SameText(Copy(FormatString, FCurrentIndex, Length(Localizer.Designators.General)), Localizer.Designators.General);
end;
...
begin
  ...
  ALowerSymbol := dxLowerCase(FCurrentSymbol);
  while ACurrentIndex <= Length(AFormatString) do
  begin
    if dxLowerCase(AFormatString[ACurrentIndex]) <> ALowerSymbol then
    begin
      Dec(ACurrentIndex);
...
begin
  ...
  if cxColorByName(S, AColor) then
    ...
    if StrUtils.StartsText(TdxNumberFormatElementColor.Prefix, S) then
    begin
      AIndex := StrToIntDef(Copy(S, Length(TdxNumberFormatElementColor.Prefix) + 1, MaxInt), 0) - 1;
      if InRange(AIndex, Low(dxExcelStandardColors), High(dxExcelStandardColors)) then
        Result := TdxNumberFormatElementColor.Create(dxExcelStandardColors[AIndex], AIndex);
    end
end;
...
begin
  ...
  if Localizer.Designators.TryGetValue(FCurrentSymbol, ADesignator) then
  begin
    ABlockLength := GetDateTimeBlockLength(S, 1);
    if ABlockLength = Length(S) then
    begin
      if ADesignator and TdxNumberFormatDesignator.Hour <> 0 then
        Result := TdxNumberFormatElementHours.Create(ABlockLength, True, False)
      else if ADesignator and TdxNumberFormatDesignator.Minute <> 0 then
        Result := TdxNumberFormatElementMinutes.Create(ABlockLength, True)
      else if ADesignator and TdxNumberFormatDesignator.Second <> 0 then
        Result := TdxNumberFormatElementSeconds.Create(ABlockLength, True);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    Inc(FCurrentIndex, 2);
    FElements.Add(TdxNumberFormatElementAmPm.CreateEx(dxIsLowerCase(S[1]), dxIsLowerCase(S[3])));
  end
  else
    Exit(False);
...
begin
  ...
  begin
    Inc(FCurrentIndex, Length(Localizer.Designators.General) - 1);
...
begin
  FElements.Add(TdxNumberFormatElementInvariantYear.Create(GetDateTimeBlockLength));
end;
...
class procedure TdxNumberFormatDesignatorFactory.Finalize;
begin
  FreeAndNil(FFactory);
end;

class procedure TdxNumberFormatDesignatorFactory.Initialize(ALocaleID: Integer; ADesignators: TdxNumberFormatDesignatorMap);
var
  AProc: TProc;
begin
  if FFactory = nil then
  begin
    FFactory := TDictionary<Integer, TProc>.Create;
    PopulateFactory;
  end;
  if FFactory.TryGetValue(ALocaleID, AProc) then
    AProc(ADesignators)
  else
    GenerateInvariantDesignators(ADesignators);
end;

class procedure TdxNumberFormatDesignatorFactory.GenerateCzechDesignators(ADesignators: TdxNumberFormatDesignatorMap);
begin
  ADesignators.General := 'Vęeobecný';
  ADesignators.Add('a', TdxNumberFormatDesignator.AmPm or TdxNumberFormatDesignator.DayOfWeek);
  ...
  ADesignators.Add('r', TdxNumberFormatDesignator.Year);
  ADesignators.Add('g', TdxNumberFormatDesignator.JapaneseEra)
end;

class procedure TdxNumberFormatDesignatorFactory.GenerateDanishDesignators(ADesignators: TdxNumberFormatDesignatorMap);
begin
  ADesignators.General := 'Standard';
  ADesignators.Add('a', TdxNumberFormatDesignator.AmPm or TdxNumberFormatDesignator.DayOfWeek);
  ADesignators.Add('d', TdxNumberFormatDesignator.Day);
  ADesignators.Add('t', TdxNumberFormatDesignator.Hour);
  ADesignators.Add('m', TdxNumberFormatDesignator.Minute or TdxNumberFormatDesignator.Month);
  ADesignators.Add('s', TdxNumberFormatDesignator.Second);
  ADesignators.Add('å', TdxNumberFormatDesignator.Year);
  ADesignators.Add('g', TdxNumberFormatDesignator.JapaneseEra)
end;

class procedure TdxNumberFormatDesignatorFactory.GenerateDutchDesignators(ADesignators: TdxNumberFormatDesignatorMap);
begin
  ADesignators.General := 'Standaard';
  ADesignators.Add('a', TdxNumberFormatDesignator.AmPm or TdxNumberFormatDesignator.DayOfWeek);
  ADesignators.Add('d', TdxNumberFormatDesignator.Day);
  ADesignators.Add('u', TdxNumberFormatDesignator.Hour);
  ADesignators.Add('m', TdxNumberFormatDesignator.Minute or TdxNumberFormatDesignator.Month);
  ADesignators.Add('s', TdxNumberFormatDesignator.Second);
  ADesignators.Add('j', TdxNumberFormatDesignator.Year);
  ADesignators.Add('g', TdxNumberFormatDesignator.JapaneseEra)
end;

class procedure TdxNumberFormatDesignatorFactory.GenerateFinnishDesignators(ADesignators: TdxNumberFormatDesignatorMap);
begin
  ADesignators.General := 'Yleinen';
  ADesignators.Add('a', TdxNumberFormatDesignator.AmPm or TdxNumberFormatDesignator.DayOfWeek);
  ADesignators.Add('p', TdxNumberFormatDesignator.Day);
  ADesignators.Add('t', TdxNumberFormatDesignator.Hour);
  ADesignators.Add('m', TdxNumberFormatDesignator.Minute);
  ADesignators.Add('k', TdxNumberFormatDesignator.Month);
  ADesignators.Add('s', TdxNumberFormatDesignator.Second);
  ADesignators.Add('v', TdxNumberFormatDesignator.Year);
  ADesignators.Add('g', TdxNumberFormatDesignator.JapaneseEra)
end;

class procedure TdxNumberFormatDesignatorFactory.GenerateFrenchDesignators(ADesignators: TdxNumberFormatDesignatorMap);
begin
  ADesignators.General := 'Standard';
  ADesignators.Add('o', TdxNumberFormatDesignator.DayOfWeek);
  ADesignators.Add('a', TdxNumberFormatDesignator.AmPm or TdxNumberFormatDesignator.Year);
  ADesignators.Add('j', TdxNumberFormatDesignator.Day);
  ADesignators.Add('h', TdxNumberFormatDesignator.Hour);
  ADesignators.Add('m', TdxNumberFormatDesignator.Minute or TdxNumberFormatDesignator.Month);
  ADesignators.Add('s', TdxNumberFormatDesignator.Second);
  ADesignators.Add('g', TdxNumberFormatDesignator.JapaneseEra)
...
class procedure TdxNumberFormatDesignatorFactory.GenerateGermanDesignators(ADesignators: TdxNumberFormatDesignatorMap);
begin
  ADesignators.General := 'Standard';
  ADesignators.Add('a', TdxNumberFormatDesignator.AmPm or TdxNumberFormatDesignator.DayOfWeek);
  ADesignators.Add('t', TdxNumberFormatDesignator.Day);
  ADesignators.Add('h', TdxNumberFormatDesignator.Hour);
  ADesignators.Add('m', TdxNumberFormatDesignator.Minute or TdxNumberFormatDesignator.Month);
  ADesignators.Add('s', TdxNumberFormatDesignator.Second);
  ADesignators.Add('j', TdxNumberFormatDesignator.Year);
...
class procedure TdxNumberFormatDesignatorFactory.GenerateGreekDesignators(ADesignators: TdxNumberFormatDesignatorMap);
begin
  ADesignators.General := 'Γενικός τύπος';
  ADesignators.Add('a', TdxNumberFormatDesignator.AmPm or TdxNumberFormatDesignator.DayOfWeek);
  ADesignators.Add('η', TdxNumberFormatDesignator.Day);
  ADesignators.Add('ω', TdxNumberFormatDesignator.Hour);
  ADesignators.Add('λ', TdxNumberFormatDesignator.Minute);
  ADesignators.Add('μ', TdxNumberFormatDesignator.Month);
  ADesignators.Add('δ', TdxNumberFormatDesignator.Second);
  ADesignators.Add('ε', TdxNumberFormatDesignator.Year);
  ADesignators.Add('g', TdxNumberFormatDesignator.JapaneseEra)
end;

class procedure TdxNumberFormatDesignatorFactory.GenerateHungarianDesignators(ADesignators: TdxNumberFormatDesignatorMap);
begin
  ADesignators.General := 'Normál';
  ADesignators.Add('a', TdxNumberFormatDesignator.AmPm or TdxNumberFormatDesignator.DayOfWeek);
  ADesignators.Add('n', TdxNumberFormatDesignator.Day);
  ADesignators.Add('ó', TdxNumberFormatDesignator.Hour);
  ADesignators.Add('p', TdxNumberFormatDesignator.Minute);
  ADesignators.Add('h', TdxNumberFormatDesignator.Month);
  ADesignators.Add('m', TdxNumberFormatDesignator.Second);
  ADesignators.Add('é', TdxNumberFormatDesignator.Year);
  ADesignators.Add('g', TdxNumberFormatDesignator.JapaneseEra)
end;

class procedure TdxNumberFormatDesignatorFactory.GenerateInvariantDesignators(ADesignators: TdxNumberFormatDesignatorMap);
begin
  ADesignators.General := 'General';
  ADesignators.Add('a', TdxNumberFormatDesignator.AmPm or TdxNumberFormatDesignator.DayOfWeek);
  ADesignators.Add('d', TdxNumberFormatDesignator.Day);
  ADesignators.Add('h', TdxNumberFormatDesignator.Hour);
  ADesignators.Add('m', TdxNumberFormatDesignator.Minute or TdxNumberFormatDesignator.Month);
...
class procedure TdxNumberFormatDesignatorFactory.GenerateItalianDesignators(ADesignators: TdxNumberFormatDesignatorMap);
begin
  ADesignators.General := 'Standard';
  ADesignators.Add('o', TdxNumberFormatDesignator.DayOfWeek);
  ADesignators.Add('a', TdxNumberFormatDesignator.AmPm or TdxNumberFormatDesignator.Year);
  ADesignators.Add('g', TdxNumberFormatDesignator.Day);
  ADesignators.Add('h', TdxNumberFormatDesignator.Hour);
  ADesignators.Add('m', TdxNumberFormatDesignator.Minute or TdxNumberFormatDesignator.Month);
  ADesignators.Add('s', TdxNumberFormatDesignator.Second);
  ADesignators.Add('x', TdxNumberFormatDesignator.JapaneseEra)
end;

class procedure TdxNumberFormatDesignatorFactory.GeneratePolishDesignators(ADesignators: TdxNumberFormatDesignatorMap);
begin
  // japaneseEra as hour
  ADesignators.General := 'Standardowy';
  ADesignators.Add('a', TdxNumberFormatDesignator.AmPm or TdxNumberFormatDesignator.DayOfWeek);
  ADesignators.Add('d', TdxNumberFormatDesignator.Day);
  ADesignators.Add('g', TdxNumberFormatDesignator.Hour or TdxNumberFormatDesignator.JapaneseEra);
  ADesignators.Add('m', TdxNumberFormatDesignator.Minute or TdxNumberFormatDesignator.Month);
  ADesignators.Add('s', TdxNumberFormatDesignator.Second);
  ADesignators.Add('r', TdxNumberFormatDesignator.Year)
end;

class procedure TdxNumberFormatDesignatorFactory.GeneratePortugueseBrazilDesignators(ADesignators: TdxNumberFormatDesignatorMap);
begin
  ADesignators.General := 'Geral';
  ADesignators.Add('o', TdxNumberFormatDesignator.DayOfWeek);
  ADesignators.Add('a', TdxNumberFormatDesignator.AmPm or TdxNumberFormatDesignator.Year);
  ADesignators.Add('d', TdxNumberFormatDesignator.Day);
  ADesignators.Add('h', TdxNumberFormatDesignator.Hour);
...
class procedure TdxNumberFormatDesignatorFactory.GeneratePortuguesePortugalDesignators(ADesignators: TdxNumberFormatDesignatorMap);
begin
  ADesignators.General := 'Estandar';
  ADesignators.Add('o', TdxNumberFormatDesignator.DayOfWeek);
  ADesignators.Add('a', TdxNumberFormatDesignator.AmPm or TdxNumberFormatDesignator.Year);
  ADesignators.Add('d', TdxNumberFormatDesignator.Day);
  ADesignators.Add('h', TdxNumberFormatDesignator.Hour);
...
class procedure TdxNumberFormatDesignatorFactory.GenerateRussianDesignators(ADesignators: TdxNumberFormatDesignatorMap);
begin
  ADesignators.General := 'Основной';
  ADesignators.Add('a', TdxNumberFormatDesignator.AmPm or TdxNumberFormatDesignator.DayOfWeek);
  ADesignators.Add('д', TdxNumberFormatDesignator.Day);
  ADesignators.Add('ч', TdxNumberFormatDesignator.Hour);
  ADesignators.Add('м', TdxNumberFormatDesignator.Minute or TdxNumberFormatDesignator.Month);
  ADesignators.Add('с', TdxNumberFormatDesignator.Second);
  ADesignators.Add('г', TdxNumberFormatDesignator.Year);
  ADesignators.Add('g', TdxNumberFormatDesignator.JapaneseEra)
end;

class procedure TdxNumberFormatDesignatorFactory.GenerateSpanishDesignators(ADesignators: TdxNumberFormatDesignatorMap);
begin
  ADesignators.General := 'Estándar';
  ADesignators.Add('o', TdxNumberFormatDesignator.DayOfWeek);
  ADesignators.Add('a', TdxNumberFormatDesignator.AmPm or TdxNumberFormatDesignator.Year);
  ADesignators.Add('d', TdxNumberFormatDesignator.Day);
  ADesignators.Add('h', TdxNumberFormatDesignator.Hour);
...
class procedure TdxNumberFormatDesignatorFactory.GenerateTurkishDesignators(ADesignators: TdxNumberFormatDesignatorMap);
begin
  // japaneseEra as day
  // dayOfWeek as month
  ADesignators.General := 'Genel';
  ADesignators.Add('a', TdxNumberFormatDesignator.AmPm or TdxNumberFormatDesignator.DayOfWeek or TdxNumberFormatDesignator.Month);
  ADesignators.Add('g', TdxNumberFormatDesignator.Day or TdxNumberFormatDesignator.JapaneseEra);
  ADesignators.Add('s', TdxNumberFormatDesignator.Hour);
  ADesignators.Add('d', TdxNumberFormatDesignator.Minute);
  ADesignators.Add('n', TdxNumberFormatDesignator.Second);
  ADesignators.Add('y', TdxNumberFormatDesignator.Year);
end;

class procedure TdxNumberFormatDesignatorFactory.PopulateFactory;
begin
  FFactory.Add(127, GenerateInvariantDesignators);

  FFactory.Add(1133, GenerateRussianDesignators); // "ba-RU"
  FFactory.Add(1059, GenerateRussianDesignators); // "be-BY"
  FFactory.Add(1087, GenerateRussianDesignators); // "kk-KZ"
  FFactory.Add(1049, GenerateRussianDesignators); // "ru-RU"
  FFactory.Add(1157, GenerateRussianDesignators); // "sah-RU"
  FFactory.Add(1092, GenerateRussianDesignators); // "tt-RU"
  FFactory.Add(2115, GenerateRussianDesignators); // "uz-Cyrl-UZ"
  FFactory.Add(1091, GenerateRussianDesignators); // "uz-Latn-UZ"

  FFactory.Add(1156, GenerateFrenchDesignators); // "gsw-FR"
  FFactory.Add(1150, GenerateFrenchDesignators); // "br-FR"
  FFactory.Add(1155, GenerateFrenchDesignators); // "co-FR"
  FFactory.Add(2060, GenerateFrenchDesignators); // "fr-BE"
  FFactory.Add(3084, GenerateFrenchDesignators); // "fr-CA"
  FFactory.Add(1036, GenerateFrenchDesignators); // "fr-FR"
  FFactory.Add(4108, GenerateFrenchDesignators); // "fr-CH"
  FFactory.Add(1154, GenerateFrenchDesignators); // "oc-FR"

  FFactory.Add(1069, GenerateSpanishDesignators); // "eu-ES"
  FFactory.Add(1027, GenerateSpanishDesignators); // "ca-ES"
  FFactory.Add(1110, GenerateSpanishDesignators); // "gl-ES"
  FFactory.Add(2058, GenerateSpanishDesignators); // "es-MX"
  FFactory.Add(3082, GenerateSpanishDesignators); // "es-ES"
  FFactory.Add(2051, GenerateSpanishDesignators); // "ca-ES-valencia"

  FFactory.Add(1046, GeneratePortugueseBrazilDesignators); // "pt-BR"

  FFactory.Add(2070, GeneratePortuguesePortugalDesignators); // "pt-PT"

  FFactory.Add(1029, GenerateCzechDesignators); // "cs-CZ"

  FFactory.Add(1030, GenerateDanishDesignators); // "da-DK"
  FFactory.Add(1044, GenerateDanishDesignators); // "nb-NO"
  FFactory.Add(2068, GenerateDanishDesignators); // "nn-NO"
  FFactory.Add(4155, GenerateDanishDesignators); // "smj-NO"
  FFactory.Add(5179, GenerateDanishDesignators); // "smj-SE"
  FFactory.Add(1083, GenerateDanishDesignators); // "se-NO"
  FFactory.Add(2107, GenerateDanishDesignators); // "se-SE"
  FFactory.Add(6203, GenerateDanishDesignators); // "sma-NO"
  FFactory.Add(7227, GenerateDanishDesignators); // "sma-SE"
  FFactory.Add(1053, GenerateDanishDesignators); // "sv-SE"

  FFactory.Add(2067, GenerateDutchDesignators); // "nl-BE"
  FFactory.Add(1043, GenerateDutchDesignators); // "nl-NL"
  FFactory.Add(1122, GenerateDutchDesignators); // "fy-NL"

  FFactory.Add(1035, GenerateFinnishDesignators); // "fi-FI"
  FFactory.Add(9275, GenerateFinnishDesignators); // "smn-FI"
  FFactory.Add(3131, GenerateFinnishDesignators); // "se-FI"
  FFactory.Add(8251, GenerateFinnishDesignators); // "sms-FI"
  FFactory.Add(2077, GenerateFinnishDesignators); // "sv-FI"

  FFactory.Add(3079, GenerateGermanDesignators); // "de-AT"
  FFactory.Add(1031, GenerateGermanDesignators); // "de-DE"
  FFactory.Add(5127, GenerateGermanDesignators); // "de-LI"
  FFactory.Add(2055, GenerateGermanDesignators); // "de-CH"
  FFactory.Add(2094, GenerateGermanDesignators); // "dsb-DE"
  FFactory.Add(1070, GenerateGermanDesignators); // "hsb-DE"

  FFactory.Add(1032, GenerateGreekDesignators); // "el-GR"

  FFactory.Add(1038, GenerateHungarianDesignators); // "hu-HU"

  FFactory.Add(1040, GenerateItalianDesignators); // "it-IT"
  FFactory.Add(2064, GenerateItalianDesignators); // "it-CH"

  FFactory.Add(1045, GeneratePolishDesignators); // "pl-PL"

  FFactory.Add(1055, GenerateTurkishDesignators); // "tr-TR"
end;
...
finalization
  ...
  TdxNumberFormatDesignatorFactory.Finalize;
