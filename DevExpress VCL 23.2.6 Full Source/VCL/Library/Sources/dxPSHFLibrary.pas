...
uses
  ...
  cxGraphics, dxPSGlbl, dxPSUtl, dxPSImgs, dxPSRes, dxPSCore, cxDrawTextUtils, dxGDIPlusClasses,
...
function dxHFFunctionLibrary: TdxHFFunctionLibrary;
...
const
  dxThisUnitName = 'dxPSHFLibrary';

var
  fdxHFFunctionLibrary: TdxHFFunctionLibrary = nil;

function dxHFFunctionLibrary: TdxHFFunctionLibrary;
begin
  if fdxHFFunctionLibrary = nil then
    fdxHFFunctionLibrary := TdxStandardHFFunctionLibrary.Create;
  Result := fdxHFFunctionLibrary;
end;
...
begin
  ...
  FGlyph.SourceDPI := USER_DEFAULT_SCREEN_DPI * 2;
end;
...
procedure TdxHFCustomFunction.SetGlyph(Value: TdxSmartGlyph);
begin
  Glyph.Assign(Value);
end;
...
begin
  inherited Create;
  dxLoadGlyphFromResource(Glyph, IDB_DXPSFUNCTION_PAGENUMBER, 'SVG');
end;
...
begin
  inherited Create;
  dxLoadGlyphFromResource(Glyph, IDB_DXPSFUNCTION_TOTALPAGES, 'SVG');
end;
...
begin
  inherited Create;
  dxLoadGlyphFromResource(Glyph, IDB_DXPSFUNCTION_PAGENUMBEROFPAGES, 'SVG');
end;
...
begin
  inherited Create;
  dxLoadGlyphFromResource(Glyph, IDB_DXPSFUNCTION_MACHINENAME, 'SVG');
end;
...
begin
  inherited Create;
  dxLoadGlyphFromResource(Glyph, IDB_DXPSFUNCTION_USERNAME, 'SVG');
end;
...
begin
  inherited Create;
  dxLoadGlyphFromResource(Glyph, IDB_DXPSFUNCTION_DATETIME, 'SVG');
end;
...
begin
  inherited Create;
  dxLoadGlyphFromResource(Glyph, IDB_DXPSFUNCTION_DATE, 'SVG');
end;
...
begin
  inherited Create;
  dxLoadGlyphFromResource(Glyph, IDB_DXPSFUNCTION_TIME, 'SVG');
end;
...
begin
  inherited Create;
  dxLoadGlyphFromResource(Glyph, IDB_DXPSFUNCTION_IMAGE, 'SVG');
end;
...
finalization
  ...
  FreeAndNil(fdxHFFunctionLibrary);
