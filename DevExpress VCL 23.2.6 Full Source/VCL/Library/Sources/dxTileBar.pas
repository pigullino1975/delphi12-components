...
uses
  System.UITypes,
  Windows, Messages, Forms, SysUtils, Classes, Controls, StdCtrls, Graphics, Math, ImgList, Dialogs, Types, Menus, MultiMon,
  ...
  cxLibraryConsts, dxHooks, dxAnimation, cxButtonEdit, cxEdit, cxEditConsts, dxMessages, cxContainer, cxCustomCanvas,
  dxCustomTileControl;
...
type
  ...
  protected
    ...
    procedure DrawContent(ACanvas: TcxCustomCanvas; const ADrawRect: TRect); override;
 ...
 { TdxTileBarItem }
  ...
  strict private
    function GetTileBar: TdxCustomTileBar;
  protected
    procedure DrawDefaultDropDownButtonArrow(ACanvas: TcxCustomCanvas;
      const ARect: TRect; ADirection: TcxArrowDirection; ABorderColor: TColor); overload; virtual;
    procedure DrawDropDownButton(ACanvas: TcxCustomCanvas; const ARect: TRect; ADirection: TcxArrowDirection;
      AState: TcxButtonState; AHighlightColor: TdxAlphaColor; AItem: TdxTileControlCustomItem); virtual;
  public
    property TileBar: TdxCustomTileBar read GetTileBar;
  end;
  ...
  protected
    ...
    procedure DoCustomPaint(ACanvas: TcxCustomCanvas); override;
  ...
  TdxTileBar = class(TdxCustomTileBar, IcxCustomCanvasSupport)
  published
    property Anchors;
    property AutoSize;
    property BiDiMode;
    ...
    property OnGroupRemoveWithLastItem;
...
begin
  Result := (UInt32(BarItem.GroupIndex) shl 16) or UInt32(BarItem.IndexInGroup);
end;
...
procedure TdxTileBarItemDropDownButtonViewInfo.DoDraw(ACanvas: TcxCustomCanvas);
const
  DirectionMap: array[TdxTileBarPosition] of TcxArrowDirection = (adDown, adUp, adRight, adLeft);
begin
  TdxTileBarPainter(TileBar.Painter).DrawDropDownButton(ACanvas, Bounds,
    DirectionMap[TileBar.OptionsView.ActualPosition], State,
...
procedure TdxTileBarItemViewInfo.DrawContent(ACanvas: TcxCustomCanvas; const ADrawRect: TRect);
begin
...
procedure TdxTileBarPainter.DrawDefaultDropDownButtonArrow(
  ACanvas: TcxCustomCanvas; const ARect: TRect; ADirection: TcxArrowDirection; ABorderColor: TColor);
var
...
begin
  R := GetArrowRect;
  CalculatePolygon(R);
  ACanvas.Polygon(APolygon, clWhite, ABorderColor);
  if ADirection in [adUp, adDown] then
    Inc(APolygon[0].X)
  else
    Inc(APolygon[0].Y);

  ACanvas.Line(APolygon[0], APolygon[2], clWhite);
end;

procedure TdxTileBarPainter.DrawDropDownButton(ACanvas: TcxCustomCanvas; const ARect: TRect;
  ADirection: TcxArrowDirection; AState: TcxButtonState; AHighlightColor: TdxAlphaColor; AItem: TdxTileControlCustomItem);
var
...
begin
  ...
  ACanvas.FillRect(cxRectInflate(R, 0, -TileBar.ScaleFactor.Apply(7)), ASplitLineColor, MaxByte);
end;
...
begin
  if TdxTileBarItemSize(AItemSizeIndex) = tbisRegular then
...
begin
  ...
  ADetail.BoundsRect := ViewInfo.DetailSiteArea;
end;
...
procedure TdxCustomTileBar.DoCustomPaint(ACanvas: TcxCustomCanvas);
begin
