...
uses
  ...
  dxMapControlTypes, dxMapControlCacheOptions, dxMapControlHttpRequest,
...
type
  ...
  TdxMapControlBingMapTileSource = class(TdxMapControlMultiScaleTileSource) // for internal use
  private
  ...
  private
    FConnectionProtocol: TdxMapControlConnectionProtocol;
    ...
    procedure SetConnectionProtocol(const Value: TdxMapControlConnectionProtocol);
  ...
  public
    ...
    function GetMapSizeInPixels(AZoomLevel: Double): TdxSizeDouble; override; // for internal use
    function IsDataValid: Boolean; // for internal use
    procedure PrepareData; override; // for internal use
 published
    property BingKey: string read GetBingKey write SetBingKey;
    property CacheOptions;
    property ConnectionProtocol: TdxMapControlConnectionProtocol read FConnectionProtocol write SetConnectionProtocol default cpDefault;
...
const
  dxThisUnitName = 'dxMapControlBingMapImageryDataProvider';
...
begin
  case AMapKind of
    bmkRoad: Result := 'RoadOnDemand'; 
    bmkArea: Result := 'Aerial';
  else//  bmkHybrid
    Result := 'AerialWithLabelsOnDemand'; 
  end;
end;
...
procedure TdxMapControlBingMapImageryDataProvider.SetConnectionProtocol(const Value: TdxMapControlConnectionProtocol);
begin
  if FConnectionProtocol <> Value then
  begin
    FConnectionProtocol := Value;
    FImageryDataService.ConnectionProtocol := FConnectionProtocol;
    FIsImageryDataDirty := True;
  end;
end;
