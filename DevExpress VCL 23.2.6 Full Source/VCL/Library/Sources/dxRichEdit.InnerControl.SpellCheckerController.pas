...
unit dxRichEdit.InnerControl.SpellCheckerController; // for internal use
...
type
  ...
  strict private
    ...
    FTextController: TdxRichEditTextController; 
  ...
  protected
    ...
    property TextController: TdxRichEditTextController read FTextController;
  public
...
uses
  RTLConsts, Contnrs, Math, Character,
...
const
  dxThisUnitName = 'dxRichEdit.InnerControl.SpellCheckerController';
...
type
  ...
  public
    ...
    function GetCharacter(const APos: TdxDocumentModelPosition): Char; inline;
    function IsNotWordSymbolAndNotSentenceSeparator(const ACh: Char): Boolean; inline;
    function IsDot(const ACh: Char): Boolean; inline;
    function IsSpace(const ACh: Char): Boolean; inline;
    function IsSpecialSentenceSeparator(const ACh: Char): Boolean; inline;
    function IsSimpleSentenceSeparator(const ACh: Char): Boolean; inline;
    function IsSentenceSeparator(const ACh: Char): Boolean; inline;
    function IsStartOfDocument(const APos: TdxDocumentModelPosition; AWordIterator: TdxSpellCheckerWordIterator): Boolean; inline;
    function IsEndOfDocument(const APos: TdxDocumentModelPosition; AWordIterator: TdxSpellCheckerWordIterator): Boolean; inline;
  end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxDocumentPosition.Initialize', SysInit.HInstance);{$ENDIF}
  FZero := TdxEmptyDocumentPosition.Create;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxDocumentPosition.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxDocumentPosition.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxDocumentPosition.Finalize', SysInit.HInstance);{$ENDIF}
  FZero := nil;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxDocumentPosition.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  ...
  APage := APages[AIndex];
  AResultLevel := UpdatePageAreaRecursive(APage, 0, ADetailsLevel);
...
begin
  ...
  AResultLevel := UpdateBoxRecursive(AFrom, ADetailsLevel);
  if AResultLevel >= ADetailsLevel then
    Exit(AResultLevel);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ATableRow := Row as TdxTableCellRow; 
...
begin
  ...
  if Safe.Cast(AValue, TdxExpandedDocumentLayoutPosition, AExpandedPosition) then
  begin
...
begin
  ...
  ATextBoxContent := ARun.Content as TdxTextBoxFloatingObjectContent;
...
begin
  Result := CharInSet(ACh, [TdxCharacters.ParagraphMark, TdxCharacters.ColumnBreak,
    TdxCharacters.PageBreak, TdxCharacters.SectionMark, TdxCharacters.LineBreak,
    TdxCharacters.FloatingObjectMark]) or (ACh = TdxCharacters.ObjectMark);
end;
...
begin
  ...
  begin
    ...
    if not GetCharacter(Result).IsLetter then
    begin
...
begin
  ...
  begin
    ...
    AStart := FindPrevLetterOrDigitPosition(Result);
    AFinish := FindLetterOrDigitPosition(Result);
    if not GetCharacter(ProcessPunctuation(AStart, AFinish)).IsLetter then
      Break
    else
...
begin
  ...
  AIsNextWordStartWithUpperCase := GetCharacter(AFinish).IsUpper;
...
var
  ...
  ACh: Char;
begin
  if not GetCharacter(AStart).IsLetter then
    Exit(TdxDocumentModelPosition.Null);
...
begin
  ...
  Result := GetCharacter(ANewPos).IsLetter;
end;
...
begin
  Result := APos;
  if GetCharacter(APos).IsLetter then
    FWordIterator.MoveToPrevChar(Result);
...
begin
  ...
  if GetCharacter(APrevPos).IsWhiteSpace then
  begin
...
begin
  ...
  if GetCharacter(APos).IsWhiteSpace then
  begin
    APrevPos := APos;
...
begin
  ...
  ACalculator := TdxSentencePositionCalculator.Create(GetWordIterator(AResult.PieceTable));
  try
...
begin
  ...
  begin
    if AWord[I].IsLetterOrDigit then
      Exit(True);
  end;
  Result := False;
end;
...
begin
  ...
  TdxCheckAsYouTypeTextController(FTextController).ResetSpellCheck(AStartRunIndex, AEndRunIndex);
end;

procedure TdxSpellCheckerController.ResetCore;
begin
  TdxCheckAsYouTypeTextController(FTextController).Reset;
end;
...
begin
  ...
  TdxCheckAsYouTypeTextController(FTextController).SetPieceTable(PieceTable);
end;
