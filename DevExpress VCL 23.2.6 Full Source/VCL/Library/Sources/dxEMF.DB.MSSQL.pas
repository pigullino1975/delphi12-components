      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if ALikeIndex > 0 then
            begin
              AConstantValue := TdxConstantValue.Create(AOperandString.Substring(0, ALikeIndex) + '%');
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxMSSQLConnectionProvider.Destroy', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FConnectionParameters);
  FreeAndNil(FDBTypes);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxMSSQLConnectionProvider.Destroy', SysInit.HInstance);{$ENDIF}
end;
...
begin
  ...
  ANames := TStringBuilder.Create;
  ASchemas := TdxStringsDictionary.Create;
  try
    for ATable in ATables do
    begin
...
begin
  AQuery := TdxQuery.Create(ASQL.SQLText + #10' select ' + IfThen(is2000, 'SCOPE_IDENTITY()', '@@Identity'),
    ASQL.Parameters, ASQL.ParameterNames, 0, 0);
  try
    ...
    try
      case ARoot.IdentityColumnType of
        TdxDBColumnType.Int32:
          begin
            Id32 := Result;
            AQuery.AppendParameter('seq', TdxOperandValue.Create(Id32));
          end;
        TdxDBColumnType.Int64:
          begin
            Id64 := Result;
            AQuery.AppendParameter('seq', TdxOperandValue.Create(Id64));
          end
        else
  ...
  try
    if Is2005 then
      AQuery := TdxQuery.Create(Format('select t.name, p.name from sys.objects t join sys.schemas p on p.schema_id = ' +
        't.schema_id where (t.type =''u'' %s) and ' +
        'CAST(case '+
        ...
        '  where '+
        '    major_id = t.object_id and minor_id = 0 and class = 1 and name = N''microsoft_database_tools_support'') '+
        '  is not null then 1 '+
        'else 0 '+
        'end AS bit) = 0',
