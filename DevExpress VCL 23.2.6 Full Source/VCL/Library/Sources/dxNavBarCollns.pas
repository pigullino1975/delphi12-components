...
type
  ...
  TdxNavBarGroup = class(TdxNavBarCustomItem, IdxNavigationItem, IdxAdornerTargetElement)
  private
    FAlign: TcxTopBottom;
  ...
  public
    ...
    property CaptionPanelIAccessibilityHelper: IdxNavBarAccessibilityHelper read GetCaptionPanelIAccessibilityHelper; // for internal use
    ...
    property LinkContainerIAccessibilityHelper: IdxNavBarAccessibilityHelper read GetLinkContainerIAccessibilityHelper; // for internal use
...
function GetActualNavBar(AGroup: TdxNavBarGroup): TdxCustomNavBar; overload;
begin
  Result := AGroup.GetParentComponent as TdxCustomNavBar;
  if Result <> nil then
    Result := TdxNavBarControllerAccess(Result.Controller).GetActualNavBar(AGroup);
end;

function GetActualNavBar(AItem: TdxNavBarItem): TdxCustomNavBar; overload;
begin
  Result := AItem.GetParentComponent as TdxCustomNavBar;
  if Result <> nil then
    Result := TdxNavBarControllerAccess(Result.Controller).GetActualNavBar;
end;
...
begin
  AViewInfo := TdxCustomNavBarAccess(GetActualNavBar(Item)).GetActualLinkViewInfoByLink(Self);
...
begin
  Result := GetActualNavBar(Item);
end;

function TdxNavBarItemLink.GetAdornerTargetElementVisible: Boolean;
var
  ANavBar: TdxCustomNavBarAccess;
begin
  ANavBar := TdxCustomNavBarAccess(GetActualNavBar(Item));
  Result := not ANavBar.ExpandAnimationController.Active and Item.Visible and (ANavBar.GetActualLinkViewInfoByLink(Self) <> nil);
end;
...
begin
  ...
  begin
    ...
    if IsAcrylicEnabled then
      dxGpFillRect(Canvas.Handle, ClientRect, Color)
    else
      inherited;
  if IsOnPopupControl then
...
function TdxNavBarGroupControl.IsAcrylicEnabled: Boolean;
begin
  Result := TdxCustomNavBarAccess(Parent).IsAcrylicEnabled;
end;

procedure TdxNavBarGroupControl.WMEraseBkgnd(var Message: TWMEraseBkgnd);
begin
  if IsAcrylicEnabled then
    Message.Result := 1
  else
...
procedure TdxNavBarGroupControl.WMPaint(var Message: TWMPaint);
begin
  if IsAcrylicEnabled then
    dxPaintWindowOnGlass(Handle, UseStyle)
  else
    inherited;
end;
...
begin
  ...
  begin
    if Group.AllowChildGroups and Group.IsLoading then
      FLoadedExpanded := AValue
    else
    begin
      FExpanded := AValue;
      if FExpanded then
...
begin
  ...
  OptionsExpansion.FLoadedExpanded := AStorage.ReadBool(ASection, 'Expanded', Expanded);
  ...
  begin
    ...
    for I := 0 to LinkCount - 1 do
      ...
      begin
        ...
        if AllowChildGroups then
          WriteInteger(ASection, 'Link' + IntToStr(I) + 'Position', Links[I].Position);
      end;
...
begin
  Result := TdxNavBarViewInfoAccess(GetActualNavBar(Self).ViewInfo).GetGroupAccessibilityHelperClass;
end;

function TdxNavBarGroup.GetCaptionPanelAccessibilityHelperClass: TdxNavBarCustomAccessibilityHelperClass;
begin
  Result := TdxNavBarViewInfoAccess(GetActualNavBar(Self).ViewInfo).GetGroupCaptionPanelAccessibilityHelperClass;
end;
...
begin
  if FControl <> nil then
    if csDestroying in FControl.ComponentState then
      ReleaseControl
    else
      FControl.Free;
end;
...
begin
  if FCaptionPanelIAccessibilityHelper = nil then
    FCaptionPanelIAccessibilityHelper := NavBarGetAccessibilityHelper(
      GetCaptionPanelAccessibilityHelperClass.Create(Self, Collection.ParentComponent as TdxCustomNavBar));
...
function TdxNavBarGroup.GetAdornerTargetElementBounds: TRect;
  ...
  var
    I: Integer;
    ANavBar: TdxCustomNavBarAccess;
    AActualGroup: TdxNavBarGroup;
  begin
    Result := nil;
    ANavBar := TdxCustomNavBarAccess(GetActualNavBar(Self));
    AActualGroup := ANavBar.GetActualGroup(AGroup);
    for I := 0 to ANavBar.ViewInfo.GroupCount - 1 do
    begin
      Result := ANavBar.ViewInfo.Groups[I].GetGroupViewInfoByGroup(AActualGroup);
...
begin
  ANavBar := TdxCustomNavBarAccess(GetActualNavBar(Self));
  Result := not ANavBar.ExpandAnimationController.Active and Visible and (ANavBar.GetActualGroupViewInfoByGroup(Self) <> nil);
end;
...
begin
  if not (Collection as TdxNavBarGroups).FIsRestoreExpandState and Value and (Parent <> nil) then
...
procedure TdxNavBarGroups.RestoreExpandStates;
var
  I: Integer;
begin
  FIsRestoreExpandState := True;
  for I := 0 to Count - 1 do
    Items[I].Expanded := Items[I].OptionsExpansion.FLoadedExpanded;
  FIsRestoreExpandState := False;
end;
