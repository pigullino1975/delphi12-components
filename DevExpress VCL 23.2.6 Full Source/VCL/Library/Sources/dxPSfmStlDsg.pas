...
constructor TdxfmPrintStylesDesignWindow.Create(AOwner: TComponent);
  ...
  function MakeTTag(AnAction: TEditAction): NativeInt;
  begin
    Result := NativeInt(AnAction);
  end;
...
begin
  with lbxStyles do
    if (FSaveDragIndex <> -1) and (FSaveDragIndex <> ItemIndex) and dxPSEngine.DialogsLookAndFeel.Painter.SupportsNativeFocusRect then
