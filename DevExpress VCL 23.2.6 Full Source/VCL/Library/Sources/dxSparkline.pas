...
uses
  ...
  cxEdit, cxEditConsts, cxLookAndFeels;
...
//    property DataBinding: TdxSparklineItemDataBinding read FDataBinding;
  ...
  protected
    ...
    function GetItemsDataBindingClass: TdxSparklineItemDataBindingClass; virtual;
...
begin
  if (FValueType = '') and (DataIndex >= 0) and (Properties.Owner <> nil) then
  begin
    AValueTypeClass := DataController.GetItemValueTypeClass(DataIndex);
    if AValueTypeClass <> nil then
      FValueType := AValueTypeClass.Caption;
  end;
  Result := FValueType;
end;
...
begin
  Result := Owner.GetItemsDataBindingClass.Create(Self, Owner, TcxStringValueType);
end;
...
begin
  Result := Owner.GetItemsDataBindingClass.Create(Self, Owner, TcxFloatValueType);
end;
...
function TdxSparklineProperties.GetItemsDataBindingClass: TdxSparklineItemDataBindingClass;
begin
...
begin
  if (AInfo is TcxDataChangedInfo) or (AInfo is TcxLayoutChangedInfo) or
    (AInfo is TcxUpdateRecordInfo) or (AInfo is TcxFindCriteriaChangedInfo) then
      DataChanged;
end;
...
begin
  ...
  AHeight := cxRectHeight(ACanvasRect);
  if (AWidth > 0) and (AHeight > 0) then
  begin
    Result.X := (ADiagramRange.X.Max - ADiagramRange.X.Min) / AWidth;
    Result.Y := (ADiagramRange.Y.Max - ADiagramRange.Y.Min) / AHeight;
  end
  else
    Result := dxPointF(0, 0);
end;
...
begin
  ...
  Result.cx := 100;
  Result.cy := TdxTextMeasurer.TextLineHeight(ACanvas.Handle) + GetEditContentSizeCorrection.cy;
end;

procedure TdxSparklineViewData.AdjustPadding(var APadding: TdxPadding);
begin
...
procedure TdxSparklineViewData.CalculateRange;
  ...
  procedure ValidateRange(var AValueRange: TdxValueRange);
  begin
    if AValueRange.Min = AValueRange.Max then
    begin
      AValueRange.Min := AValueRange.Min - 1;
      AValueRange.Max := AValueRange.Max + 1;
    end;
  end;
