...
type
  ...
  published
    ...
    property Container: TdxStatusBarContainerControl read FContainer write SetContainer; // for internal use
  end;
  ...
  protected
    ...
    class procedure DrawSizeGripCore(ACanvas: TcxCanvas; const ARect: TRect; AScaleFactor: TdxScaleFactor); virtual;
  ...
  protected
    class procedure DrawSizeGripCore(ACanvas: TcxCanvas; const ARect: TRect; AScaleFactor: TdxScaleFactor); override;
  ...
  protected
    ...
    procedure CalculatePanelsViewInfos(ABounds: TRect; AAutoWidthObject: TcxAutoWidthObject);
...
uses
  ActnList, StdActns, Types, Math, dxThemeConsts, dxUxTheme, dxOffice11, dxDPIAwareUtils, dxSkinsdxStatusBarPainter, dxCoreGraphics,
  dxSkinInfo, dxSkinsCore;

const
  dxThisUnitName = 'dxStatusBar';
...
procedure TdxStatusBarTextPanelStyle.DrawContent(ACanvas: TcxCanvas; R: TRect; APainter: TdxStatusBarPainterClass);
  ...
  begin
    Result := nil;
    if (StatusBarControl.PaintStyle = stpsUseLookAndFeel) and Assigned(StatusBarControl.LookAndFeel.SkinPainter) then
      if StatusBarControl.LookAndFeel.SkinPainter.GetPainterData(ASkinInfo) then
        Result := ASkinInfo.RibbonSmallButton.GetGlyphColorPalette(esNormal);
  end;

const
  AShowEndEllipsis: array[TdxStatusBarEllipsisType] of Integer = (0, cxShowEndEllipsis, cxShowPathEllipsis);
var
...
begin
  ...
  if IsImageAssigned(StatusBarControl.Images, ImageIndex) then
  begin
    AImageSize := dxGetImageSize(StatusBarControl.Images, ScaleFactor);
    ALeft := R.Left + 1;
    ATop := ((R.Top + R.Bottom) div 2) - AImageSize.cy div 2;
    R.Left := ALeft + AImageSize.cx + 1;
    if StatusBarControl.UseRightToLeftAlignment then
      ARealLeft := R.Right - (ALeft - R.Left) - AImageSize.cx
    else
      ARealLeft := ALeft;

    TdxImageDrawer.DrawImage(ACanvas,
      cxRectBounds(ARealLeft, ATop, AImageSize.cx, AImageSize.cy), nil,
      StatusBarControl.Images, ImageIndex, True, GetGlyphColorPalette, ScaleFactor);
  end;
...
begin
  ...
  FTimer := cxCreateTimer(Execute, 100);
end;
...
procedure TdxStatusBarViewInfo.CalculatePanelsViewInfos(ABounds: TRect; AAutoWidthObject: TcxAutoWidthObject);
var
  AIndex: Integer;
...
begin
  ...
  begin
    ...
    APanelViewInfo.Bounds := cxRectSetWidth(ABounds, AAutoWidthObject[AIndex].AutoWidth);
    ABounds.Left := APanelViewInfo.Bounds.Right;
    if AIndex + 1 < AAutoWidthObject.Count then
    begin
      APanelViewInfo.SeparatorRect := cxRectSetWidth(ABounds, AAutoWidthObject[AIndex + 1].AutoWidth);
      ABounds.Left := APanelViewInfo.SeparatorRect.Right;
    end;
    Inc(AIndex, 2);
  end;
  if (PanelCount > 0) and IsSizeGripInPanelArea and StatusBar.SizeGripAllocated then
  begin
    APanelViewInfo := PanelViewInfo[PanelCount - 1];
    ABounds := APanelViewInfo.Bounds;
    Inc(ABounds.Right, Painter.ContentExtents(StatusBar).Right);
    APanelViewInfo.Bounds := ABounds;
  end;
end;
...
begin
  Result := Max(Painter.GripAreaSize(StatusBar).cy, TdxTextMeasurer.TextLineHeight(PanelFont) +
...
const
  CornerMap: array [Boolean] of TdxCorner = (coBottomRight, coBottomLeft);
begin
  dxRotateSizeGrip(ACanvas, R, AStatusBar.ScaleFactor, GetSizeGripBackgroundColor(AStatusBar),
    CornerMap[AStatusBar.UseRightToLeftAlignment], DrawSizeGripCore);
end;
...
begin
  Result := R;
  if IsSizeGripInPanelArea(AStatusBar) and AStatusBar.SizeGripAllocated and
    cxRectIntersect(R, AStatusBar.ViewInfo.SizeGripAreaRect) then
  begin
    if AStatusBar.UseRightToLeftAlignment then
      Result.Left := AStatusBar.ViewInfo.SizeGripAreaRect.Right
    else
      Result.Right := AStatusBar.ViewInfo.SizeGripAreaRect.Left;
  end;
end;

class function TdxStatusBarPainter.GetSizeGripBackgroundColor(AStatusBar: TdxCustomStatusBar): TColor;
begin
  if (AStatusBar <> nil) and ((AStatusBar.PaintStyle = stpsXP) or
    ((AStatusBar.PaintStyle = stpsUseLookAndFeel) and AStatusBar.LookAndFeel.NativeStyle)) then
    Result := clNone
  else
    Result := AStatusBar.Color;
end;

class procedure TdxStatusBarPainter.DrawSizeGripCore(ACanvas: TcxCanvas; const ARect: TRect; AScaleFactor: TdxScaleFactor);
begin
  cxLookAndFeelPaintersManager.GetPainter(lfsStandard).DoDrawSizeGrip(ACanvas, ARect, AScaleFactor);
end;
...
class procedure TdxStatusBarOffice11Painter.DrawSizeGripCore(ACanvas: TcxCanvas; const ARect: TRect; AScaleFactor: TdxScaleFactor);
begin
...
class procedure TdxStatusBarXPPainter.DrawSizeGripCore(
  ACanvas: TcxCanvas; const ARect: TRect; AScaleFactor: TdxScaleFactor);
begin
  dxDrawThemeBackground(ACanvas.Handle, totStatus, AScaleFactor, ARect, SP_GRIPPER, 0);
end;

class function TdxStatusBarXPPainter.GripAreaSize(AStatusBar: TdxCustomStatusBar): TSize;
begin
  Result := GetThemeScaledPartSize(totStatus, AStatusBar.ScaleFactor, SP_GRIPPER, 0);
end;

class function TdxStatusBarXPPainter.SeparatorSize(AStatusBar: TdxCustomStatusBar): Integer;
begin
  Result := GetThemeScaledPartSize(totStatus, AStatusBar.ScaleFactor, SP_PANE, 0).cx;
end;
...
begin
  dxDrawThemeBackground(ACanvas.Handle, totStatus, AStatusBar.ScaleFactor, R, 0, 0);
  Inc(R.Top, TopBorderSize);
end;

class procedure TdxStatusBarXPPainter.DrawPanelSeparator(AStatusBar: TdxCustomStatusBar; ACanvas: TcxCanvas; const R: TRect);
begin
  dxDrawThemeBackground(ACanvas.Handle, totStatus, AStatusBar.ScaleFactor, R, SP_PANE, 0);
end;
