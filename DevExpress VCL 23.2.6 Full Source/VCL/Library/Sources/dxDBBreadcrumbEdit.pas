...
const
  dxThisUnitName = 'dxDBBreadcrumbEdit';
...
begin
  ...
  if Result = 0 then
    Result := CompareText(ANode1.Name, ANode2.Name);
end;

{ TdxDBBreadcrumbEditDataLink }

constructor TdxDBBreadcrumbEditDataLink.Create(AOwner: TdxDBBreadcrumbEditDataBinding);
begin
  inherited Create;
  FOwner := AOwner;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if C = 0 then
      begin
        while (I > 0) and VarEquals(TdxDBBreadcrumbEditNode(AList[I - 1]).ParentKeyValue, AParentKeyValue) do
          Dec(I);
        ANode := TdxDBBreadcrumbEditNode(AList[I]);
        Exit(True);
      end;
    end;
  end;
