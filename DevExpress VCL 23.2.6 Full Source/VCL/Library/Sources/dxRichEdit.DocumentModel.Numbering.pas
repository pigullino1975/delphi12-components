...
unit dxRichEdit.DocumentModel.Numbering; // for internal use
...
uses
  ...
  dxCoreClasses, dxCoreGraphics, dxCultureInfo, dxCore,
...
type
  ...
  private
    ...
    function GetItem(AIndex: Integer): TdxAbstractListLevel; inline;
  ...
  TdxNumberingListHelper = class sealed
  public type
  ...
  strict private
    function GetItem(Index: TdxListIndex): TdxNumberingList;
  public
    function First: TdxNumberingList;
    function Last: TdxNumberingList;

    property Items[Index: TdxListIndex]: TdxNumberingList read GetItem; default;
  end;
  ...
  public
    ...
    function GetListIndex(AStart, AEnd: TdxParagraphIndex): TdxNumberingListIndex;
    function CreateNewAbstractList(ASource: TdxAbstractNumberingList): TdxAbstractNumberingListIndex;
    function CreateNewList(ASource: TdxAbstractNumberingList): TdxNumberingListIndex;
  ...
  TdxDefaultNumberingListHelper = class sealed
  public type
...
const
  dxThisUnitName = 'dxRichEdit.DocumentModel.Numbering';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxAbstractNumberingListIdProvider.Initialize', SysInit.HInstance);{$ENDIF}
  FLock := TObject.Create;
  Randomize;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxAbstractNumberingListIdProvider.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxAbstractNumberingListIdProvider.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxAbstractNumberingListIdProvider.Finalize', SysInit.HInstance);{$ENDIF}
  FLock.Free;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxAbstractNumberingListIdProvider.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
function TdxNumberingListCollection.GetItem(Index: TdxListIndex): TdxNumberingList;
begin
