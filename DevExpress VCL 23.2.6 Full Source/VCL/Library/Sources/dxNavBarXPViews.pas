...
type
  ...
  public
    class function DrawObject(ACanvas: TCanvas; ARect: TRect; AObject: TdxThemedObjectType;
      iPartId, iStateId: Integer; AScaleFactor: TdxScaleFactor): Boolean;
    class function DrawText(ACanvas: TCanvas; ARect: TRect; ACaption: string; ADrawTextFlag: Integer;
  ...
  protected
    class procedure InternalDrawBackground(ACanvas: TCanvas; ARect: TRect; APicture: TPicture;
      ...
      AGradientMode: TdxBarStyleGradientMode; AScaleFactor: TdxScaleFactor); override;
  end;
  ...
  protected
    class procedure InternalDrawBackground(ACanvas: TCanvas; ARect: TRect; APicture: TPicture; AColor1, AColor2: TColor;
      AAlphaBlend1, AAlphaBlend2: Byte; AGradientMode: TdxBarStyleGradientMode; AScaleFactor: TdxScaleFactor); override;
  end;
  ...
  protected
    class procedure InternalDrawSelection(ACanvas: TCanvas; ARect: TRect;
      ABackColor: TColor; AState: TdxNavBarObjectStates; AScaleFactor: TdxScaleFactor); override;
  end;
  ...
  protected
    class procedure InternalDrawBottomButton(ACanvas: TCanvas; ARect: TRect;
      ...
      ABorderColor: TColor; AState: TdxNavBarObjectStates; AScaleFactor: TdxScaleFactor); override;
  ...
  protected
    ...
    class procedure InternalDrawBackground(ACanvas: TCanvas; ARect: TRect; APicture: TPicture; AEraseBackground: Boolean;
...
const
  dxThisUnitName = 'dxNavBarXPViews';
...
begin
  if dxXPExplorerBarGroupBorderColor <> clNone then
    ...
    if cxGetThemeColor(totExplorerBar, EBP_NORMALGROUPBACKGROUND, 0, TMT_BORDERCOLOR, AColor) then
      Result := AColor
    else
...
var
  AColor: TColor;
begin
  if cxGetThemeColor(totExplorerBar, EBP_NORMALGROUPBACKGROUND, 0, TMT_FILLCOLOR, AColor) then
    Result := AColor
  else
    if cxGetThemeColor(totExplorerBar, EBP_NORMALGROUPBACKGROUND, 0, TMT_GRADIENTCOLOR1, AColor) then
      Result := AColor
    else
...
begin
  if cxGetThemeColor(totExplorerBar, EBP_NORMALGROUPBACKGROUND, 0, TMT_FILLCOLOR, AColor) then
    ...
    if cxGetThemeColor(totExplorerBar, EBP_NORMALGROUPBACKGROUND, 0, TMT_GRADIENTCOLOR2, AColor) then
      Result := AColor
    else
...
begin
  ...
  if cxGetThemeColor(totExplorerBar, APart, 0, TMT_FILLCOLOR, AColor) then
    Result := AColor
  else
    if cxGetThemeColor(totExplorerBar, APart, 0, TMT_GRADIENTCOLOR1, AColor) then
      Result := AColor
    else
...
begin
  ...
  if cxGetThemeColor(totExplorerBar, APart, 0, TMT_FILLCOLOR, AColor) then
    ...
    if cxGetThemeColor(totExplorerBar, APart, 0, TMT_GRADIENTCOLOR2, AColor) then
      Result := AColor
    else
      Result := inherited CaptionBackColor;
end;
...
var
  AColor: TColor;
begin
  if cxGetThemeColor(totExplorerBar, EBP_HEADERBACKGROUND, -1, TMT_GRADIENTCOLOR1, AColor) then
    Result := AColor
  else
    Result := inherited BgBackColor;
end;
...
var
  AColor: TColor;
begin
  if cxGetThemeColor(totExplorerBar, EBP_HEADERBACKGROUND, -1, TMT_GRADIENTCOLOR2, AColor) then
    Result := AColor
  else
    Result := inherited BgBackColor2;
end;
...
begin
  if (sSpecial in AState) and (dxXPExplorerBarSpecialGroupHeader <> nil) then
    ...
    Result := dxXPExplorerBarNormalGroupHeader;
end;

class procedure TdxNavBarXPExplorerBarButtonPainter.InternalDrawButton(ACanvas: TCanvas;
...
begin
  ...
  if (OpenTheme(totExplorerBar) <> 0) and (ABitmap <> nil) then
  begin
...
class function TdxNavBarXPPainter.DrawObject(ACanvas: TCanvas; ARect: TRect;
  AObject: TdxThemedObjectType; iPartId, iStateId: Integer; AScaleFactor: TdxScaleFactor): Boolean;
var
  ATheme: TdxTheme;
begin
  ATheme := OpenTheme(AObject, AScaleFactor);
  Result := ATheme <> 0;
  if Result then
    DrawThemeBackground(ATheme, ACanvas.Handle, iPartId, iStateId, @ARect);
end;

class function TdxNavBarXPPainter.DrawText(ACanvas: TCanvas; ARect: TRect; ACaption: string;
  ADrawTextFlag: Integer; AObject: TdxThemedObjectType; iPartId, iStateId: Integer): Boolean;
var
  ATheme: TdxTheme;
begin
...
class procedure TdxNavBarXPBackgroundPainter.InternalDrawBackground(ACanvas: TCanvas;
  ...
  AScaleFactor: TdxScaleFactor);
begin
  if not TdxNavBarXPPainter.DrawObject(ACanvas, ARect, totTab, TABP_BODY, 0, AScaleFactor) then
    inherited;
end;

{ TdxNavBarXPGroupBackgroundPainter }

class procedure TdxNavBarXPGroupBackgroundPainter.InternalDrawBackground(ACanvas: TCanvas; ARect: TRect;
...
begin
  ...
  if not TdxNavBarXPPainter.DrawObject(ACanvas, ARect, totToolBar, TP_BUTTON, AStateID, AScaleFactor) then
    inherited;
end;
...
begin
  ...
  if not TdxNavBarXPPainter.DrawObject(ACanvas, ARect, totButton, BP_PUSHBUTTON, AStateID, AScaleFactor) then
    inherited;
end;

{ TdxNavBarXPScrollButtonsPainter }

class procedure TdxNavBarXPScrollButtonsPainter.InternalDrawBottomButton(ACanvas: TCanvas; ARect: TRect;
  AButtonPainterClass: TdxNavBarCustomButtonPainterClass; AColor1, AColor2: TColor;
  AAlphaBlend1, AAlphaBlend2: Byte; AGradientMode: TdxBarStyleGradientMode;
  ...
  begin
    if sPressed in AState then
      ...
      if sHotTracked in AState then
        ...
        AStateID := ABS_DOWNNORMAL;

    TdxNavBarXPPainter.DrawObject(ACanvas, ARect, totScrollBar, SBP_ARROWBTN, AStateID, AScaleFactor);
...
class procedure TdxNavBarXPScrollButtonsPainter.InternalDrawTopButton(ACanvas: TCanvas;
  ARect: TRect; AButtonPainterClass: TdxNavBarCustomButtonPainterClass; AColor1, AColor2: TColor;
  AAlphaBlend1, AAlphaBlend2: Byte; AGradientMode: TdxBarStyleGradientMode;
  ABorderColor: TColor; AState: TdxNavBarObjectStates; AScaleFactor: TdxScaleFactor);
var
  ATheme: TdxTheme;
