...
uses
  ...
  dxMessages, dxGDIPlusClasses, cxLookAndFeelPainters, dxSkinsCore, dxSkinInfo, dxCustomHint,
...
type
  ...
  TdxRichEditHintHelper = class(TcxCustomHintHelper)  //for internal use
  strict private
    FControl: TdxRichEditControlBase;
    FHintTarget: IcxHintableObject2;
    function GetHintObject(AHintableObject: IcxHintableObject2): TObject;
    procedure SetHintTarget(const Value: IcxHintableObject2);
  protected
    ...
    property HintTarget: IcxHintableObject2 read FHintTarget write SetHintTarget;
  public
    constructor Create(AControl: TdxRichEditControlBase);
  ...
  strict private
    ...
    FHintHelper: TdxRichEditHintHelper;
    FInnerControl: TdxInnerRichEditControl;
    FInsideResize: Boolean;
    FIsCaretDestroyed: Boolean;
  ...
  protected
    ...
    procedure ApplyFont(ACharacterProperties: TdxCharacterProperties; AFont: TFont); virtual;
    ...
    function GetScaleFactor: TdxScaleFactor; reintroduce;
    ...
    procedure DestroyCaret; virtual;
...
uses
  Dialogs, Math, ComObj, IOUtils, Menus,
  dxCore, dxDPIAwareUtils, dxTypeHelpers, cxScrollBar, dxThreading, dxMessageDialog,
  ...
  dxRichEdit.Utils.ToolTipService,
  dxRichEdit.Export.Formats;

const
  dxThisUnitName = 'dxRichEdit.Control.Core';
  dxInstantCaretTimerInterval = 1;
  dxSmoothCaretTimerInterval = 1000;
...
begin
  ...
  try
    ...
    ADCOrigin := Control.Canvas.DCOrigin;
    APaintOrg.Offset(
      ALayoutUnitConverter.PixelsToLayoutUnits(ADCOrigin.X),
      ALayoutUnitConverter.PixelsToLayoutUnits(ADCOrigin.Y));
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxRichEditControlBase.Initialize', SysInit.HInstance);{$ENDIF}
  FSystemCaretBitmap := CreateSystemCaretBitmap;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxRichEditControlBase.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxRichEditControlBase.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxRichEditControlBase.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FSystemCaretBitmap); 
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxRichEditControlBase.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  ...
  begin
    if Safe.Cast(MouseController, TdxRichEditMouseController, AMouseController) then
    begin
      if Safe.Cast(AMouseController.ActiveObject, TdxField, AField) and DocumentModel.ActivePieceTable.IsHyperlinkField(AField) then
        Exit(True);
    end;
  end;
  Result := False;
...
procedure TdxRichEditControlBase.FontChanged;
begin
...
begin
  ...
  if not Handled and (PopupMenu <> nil) and not (PopupMenu is TPopupMenu) then
  begin
  ...
  if not Handled and (Options.Behavior.ShowPopupMenuAllowed or (PopupMenu is TPopupMenu)) then
  begin
...
begin
  if IsUpdateLocked then
    Exit;
  if Caret.IsBlinking then
  begin
    Caret.IsHidden := not Caret.IsHidden;
    Painter.DrawCaret;
  end
  else
  begin
    FCaretTimer.Interval := dxSmoothCaretTimerInterval;
...
begin
  ...
  if not IsPopupMenuShown then
    FIsCaretDestroyed := not Windows.CreateCaret(Handle, FSystemCaretBitmap.Handle, 0, 0);
end;

procedure TdxRichEditControlBase.DestroyCaret;
begin
  if not FIsCaretDestroyed then
    FIsCaretDestroyed := Windows.DestroyCaret;
end;
...
begin
  ...
  if ShouldShowCaret and not Caret.IsBlinking then
    FCaretTimer.Interval := dxInstantCaretTimerInterval;
end;

procedure TdxRichEditControlBase.AddServices;
begin
  AddService(IdxToolTipService, TdxToolTipService.Create(Self));
end;
...
function TdxRichEditControlBase.GetFont: TFont;
begin
  Result := Font;
end;

function TdxRichEditControlBase.ShouldApplyForeColor: Boolean;
begin
  case Options.Behavior.ForeColorSource of
    TdxRichEditBaseValueSource.Control:
      Exit(True);
    TdxRichEditBaseValueSource.Document:
      Exit(False);
    else 
      Exit(False); 
  end;
...
function TdxRichEditControlBase.ShouldApplyFont: Boolean;
begin
  case Options.Behavior.FontSource of
    TdxRichEditBaseValueSource.Control:
      Exit(True);
    TdxRichEditBaseValueSource.Document:
      Exit(False);
...
procedure TdxRichEditControlBase.ApplyFont(ACharacterProperties: TdxCharacterProperties; AFont: TFont);
begin
  case Options.Behavior.FontSource of
    TdxRichEditBaseValueSource.Control:
      TdxCharacterPropertiesFontAssignmentHelper.AssignFont(ACharacterProperties, Font);
    TdxRichEditBaseValueSource.Auto:
      TdxCharacterPropertiesFontAssignmentHelper.AssignFont(ACharacterProperties, AFont);
  end;
end;
...
begin
  ...
  if InnerControl <> nil then
    InnerControl.ResetSpellChecker;
...
begin
  Result := dxMessageDlg(AMessage, mtWarning, [mbOK], 0);
end;

function TdxRichEditControlBase.ShowErrorMessage(const AMessage: string): TModalResult;
begin
  Result := dxMessageDlg(AMessage, mtError, [mbOK], 0);
end;
...
var
  ACaretBlinkTime: Cardinal;
begin
  if FCaretTimer.Enabled then
    FCaretTimer.Enabled := False;
  ACaretBlinkTime := GetCaretBlinkTime;
  Caret.IsBlinking := ACaretBlinkTime <> MAXDWORD;
  if Caret.IsBlinking then
    FCaretTimer.Interval := ACaretBlinkTime
  else
    FCaretTimer.Interval := dxInstantCaretTimerInterval;
...
begin
  case Options.HorizontalScrollbar.Visibility of
    ...
    begin
      ...
      Result := ActiveView.PageViewInfoGenerator.MaxPageWidth * ActiveView.PageViewInfoGenerator.ScaleFactor > ActiveView.PageViewInfoGenerator.ViewPortBounds.Width + 1;
    end;
  end;
end;
...
{ TdxRichEditHintHelper }

procedure TdxRichEditHintHelper.CorrectHintWindowRect(var ARect: TRect);
  ...
  begin
    ...
    if APrimaryMonitorDPI < 144 then
      Result := cxGetCursorSize
    else
    begin
      Result.cy := GetSystemMetrics(SM_CYCURSOR);
      Result.cx := GetSystemMetrics(SM_CXCURSOR);
      if GetIconInfo(GetCursor, AIconInfo) then
      begin
        if GetObject(AIconInfo.hbmMask, SizeOf(ABitmapInfo), @ABitmapInfo) > 0 then
        begin
          Result.cx := Abs(ABitmapInfo.bmWidth);
          Result.cy := Result.cx - Integer(AIconInfo.yHotspot);
        end;
...
begin
  ...
  if (FHintTarget <> nil) and FHintTarget.IsHintAtMousePos and GetCursorPos(AMousePos) then
  begin
    ACursorSize := GetCursorSize;
    if FControl.UseRightToLeftAlignment then
    begin
      ARect := cxRectSetTop(ARect, AMousePos.Y + ACursorSize.cy + 1);
      ARect := cxRectSetRight(ARect, AMousePos.X + ACursorSize.cx);
    end
    else
      ARect := cxRectSetOrigin(ARect, Point(AMousePos.X, AMousePos.Y + ACursorSize.cy + 1));
  end;
end;

constructor TdxRichEditHintHelper.Create(AControl: TdxRichEditControlBase);
begin
  inherited Create;
  FControl := AControl;
end;

procedure TdxRichEditHintHelper.MouseMove(X, Y: Integer);
begin
  HintTarget := FControl.InnerControl.GetService<IdxToolTipService>.CalculateToolTipInfo(Point(X, Y));
end;

function TdxRichEditHintHelper.GetHintControlBounds: TRect;
begin
  Result := FControl.Bounds;
end;

function TdxRichEditHintHelper.GetHintWinControl: TWinControl;
begin
  Result := FControl;
end;

function TdxRichEditHintHelper.GetOwnerWinControl: TWinControl;
begin
  Result := FControl;
end;

function TdxRichEditHintHelper.IsHintAtMousePos: Boolean;
begin
  Result := (FHintTarget <> nil) and FHintTarget.IsHintAtMousePos;
end;

function TdxRichEditHintHelper.PtInCaller(const P: TPoint): Boolean;
begin
  Result := PtInRect(FControl.ClientBounds, P) and
            (GetHintObject(FControl.InnerControl.GetService<IdxToolTipService>.CalculateToolTipInfo(P)) = GetHintObject(FHintTarget));
end;

function TdxRichEditHintHelper.GetHintObject(AHintableObject: IcxHintableObject2): TObject;
begin
  if AHintableObject = nil then
    Result := nil
  else
    Result := AHintableObject.GetHintObject;
end;

procedure TdxRichEditHintHelper.SetHintTarget(const Value: IcxHintableObject2);
begin
  if GetHintObject(FHintTarget) <> GetHintObject(Value) then
  begin
    FHintTarget := Value;
    if FHintTarget = nil then
      CancelHint
    else
      ActivateHint(FHintTarget);
  end;
end;
