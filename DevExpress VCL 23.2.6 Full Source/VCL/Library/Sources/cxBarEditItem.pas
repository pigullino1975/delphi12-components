...
uses
  ...
  dxBarAccessibility, cxAccessibility;
...
type
  ...
  private
    ...
    FCanUseItemIndexProvider: Boolean;
  ...
  {$HINTS ON}
    ...
    procedure UpdateItemIndexWhenComponentLoaded;
  protected
    ...
    function GetPropertiesItemIndexProvider: IcxValueByItemIndexProvider;
    ...
    property CanUseItemIndexProvider: Boolean read FCanUseItemIndexProvider;
...
begin
  ...
  for I := 0 to GetRegisteredEditProperties.Count - 1 do
    ...
    begin
      ABarItem := BarDesignController.AddInternalItem(AItemLinks, TdxBarButton, GetRegisteredEditProperties.Descriptions[I], OnButtonClick);
      LoadEditorComponentIcon(GetRegisteredEditProperties.Items[I].ClassName, ABarItem.Glyph, AMouseDPI);
    end;
end;
...
function TcxCustomBarEditItem.GetPropertiesItemIndexProvider: IcxValueByItemIndexProvider;
begin
  if not Supports(TObject(Properties), IcxValueByItemIndexProvider, Result) then
    Exit;
  if not Result.SupportsItemIndex then
    Result := nil;
end;
...
begin
  ...
  FCanUseItemIndexProvider := False;
end;
...
begin
  if not CanUseItemIndexProvider then
    Exit(False);
...
procedure TcxCustomBarEditItem.UpdateItemIndexWhenComponentLoaded;
var
  AProvider: IcxValueByItemIndexProvider;
begin
  ...
  FSavedItemCount := AProvider.ItemCount;
...
var
  AProvider: IcxValueByItemIndexProvider;
  AProviderItemCount: Integer;
  ANewValue: Variant;
begin
  if (csLoading in ComponentState) or CanUseItemIndexProvider then
    Exit;
  AProvider := GetPropertiesItemIndexProvider;
  if AProvider = nil then
  begin
  ...
  AProviderItemCount := AProvider.ItemCount;
  try
    if FItemIndex <> -1 then
    begin 
      if FSavedItemCount <> AProviderItemCount then
        ItemIndex := -1
      else
      ...
      begin
        case TcxComboBoxProperties(Properties).DropDownListStyle of
           TcxEditDropDownListStyle.lsFixedList,
           TcxEditDropDownListStyle.lsEditFixedList: EditValue := Null;             
        end;        
      end;           
  finally
    FSavedItemCount := AProviderItemCount;
  end;
end;
...
begin
  ...
  begin
    FItemIndex := AValue;
    if not ApplyItemIndexToControl and not (csLoading in ComponentState) then
      UpdateValues;
  end;
end;
...
begin
  inherited Loaded;
  UpdateItemIndexWhenComponentLoaded;
end;
...
begin
  ...
  if Supports(Parent, IdxVisualRefinementsTuner, AIntf) then
...
function TdxBarEditItemControlAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxROLE_SYSTEM_TEXT;
end;

function TdxBarEditItemControlAccessibilityHelper.GetValue(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := VarToStr(EditItem.EditValue);
end;

function TdxBarEditItemControlAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
