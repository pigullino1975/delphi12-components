...
type
  ...
  public
    ...
    class function TryStrToFloat(const S: string; out Value: Double; IgnoreSpecialConsts: Boolean = True): Boolean; overload; static;
    class function TryStrToFloat(const S: string; out Value: Double; const AFormatSettings: TFormatSettings; IgnoreSpecialConsts: Boolean = True): Boolean; overload; static;
...
function Trim(const ABuilder: TStringBuilder): string; overload;
...
function Trim(const ABuilder: TStringBuilder): string;
var
  S, F: Integer;
begin
  S := 0;
  ...
  while (S <= F) and (Ord(TStringBuilderAccess(ABuilder).FData[S{$IFDEF DELPHI103} + 1{$ENDIF}]) <= Ord(' ')) do
    Inc(S);
  while (S <  F) and (Ord(TStringBuilderAccess(ABuilder).FData[F{$IFDEF DELPHI103} + 1{$ENDIF}]) <= Ord(' ')) do
    Dec(F);

  if S > F then
    Result := EmptyStr
  else
    Result := ABuilder.ToString(S, F - S + 1);
end;
...
class function TdxStringHelper.TryStrToFloat(const S: string; out Value: Double;
  const AFormatSettings: TFormatSettings; IgnoreSpecialConsts: Boolean): Boolean;
begin
  Result := SysUtils.TryStrToFloat(S, Value, AFormatSettings);
  {$IFDEF DELPHI120}
  if Result and IgnoreSpecialConsts and
    ((Value = Extended.NaN) or (Value = Extended.PositiveInfinity) or (Value = Extended.NegativeInfinity)) then
    Result := False;
  {$ENDIF}
end;

class function TdxStringHelper.TryStrToFloat(const S: string; out Value: Double;
  IgnoreSpecialConsts: Boolean): Boolean;
begin
