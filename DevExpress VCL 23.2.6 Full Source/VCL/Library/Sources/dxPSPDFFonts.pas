...
uses
  Generics.Defaults, Generics.Collections, Windows, SysUtils, Classes, Graphics, cxClasses, dxCore, cxGraphics,
...
type
  ...
  public
    ...
    destructor Destroy; override;
    function CanEncodeText(C: Char): Boolean; override;
...
uses
  dxStringHelper, dxFontHelpers;

const
  dxThisUnitName = 'dxPSPDFFonts';
...
function dxPDFIsOpenTypeFont(AStream: TMemoryStream): Boolean;
const
  OpenTypeSignature: array[0..3] of byte = ($4F, $54, $54, $4F);
var
  AData: TBytes;
...
begin
  ...
  try
    AStream.Position := 0;
    SetLength(AData, 4);
    AStream.ReadBuffer(AData[0], 4);
    Result := CompareMem(AData, @OpenTypeSignature, 4);
  finally
    AStream.Position := ASavedPosition;
  end;
end;

function dxPDFTryGetEmbedFontData(AFont: TFont; AStream: TMemoryStream = nil): Boolean;
begin
  Result := (AFont.Charset <> SYMBOL_CHARSET) and dxPSGetFontData(AStream, AFont);
end;
...
begin
  ...
  try
    Result := dxPDFTryGetEmbedFontData(AFont, AStream) and not dxPDFIsOpenTypeFont(AStream);
  finally
    AStream.Free;
  end;
end;
...
function dxPDFCanEmbedFont(AFont: TFont): Boolean;
begin
  Result := dxPDFTryGetEmbedFontData(AFont, nil);
end;
...
function TdxPSPDFTrueTypeFont.CanEncodeText(C: Char): Boolean;
begin
  Result := True;
end;
...
function TdxPSPDFTrueTypeFont.EncodeText(const S: string): AnsiString;
  ...
  function CheckEscapeSequences(const AText: AnsiString): AnsiString;

    procedure EscapeChar(var ADestination: PAnsiChar; AChar: AnsiChar; var ACount: Integer);
    begin
      ADestination^ := '\';
      Inc(ADestination);
      ADestination^ := AChar;
  ...
  var
    I, AActualLength, ALength: Integer;
    ACurrentChar, ADestination: PAnsiChar;
  begin
    ACurrentChar := PAnsiChar(AText);
    ALength := Length(AText);
    SetLength(Result, ALength * 2);
    ADestination := PAnsiChar(Result);
    AActualLength := 0;
    for I := 0 to ALength - 1 do
    begin
      case ACurrentChar^ of
        #40: 
          EscapeChar(ADestination, '(', AActualLength);
        #41: 
          EscapeChar(ADestination, ')', AActualLength);
        #92: 
          EscapeChar(ADestination, '\', AActualLength);
        #10: 
          EscapeChar(ADestination, 'n', AActualLength);
        #13: 
          EscapeChar(ADestination, 'r', AActualLength);
        #12: 
          EscapeChar(ADestination, 'f', AActualLength);
        #8: 
          EscapeChar(ADestination, 'b', AActualLength);
        #9: 
          EscapeChar(ADestination, 't', AActualLength);
      else
        ADestination^ := ACurrentChar^;
        Inc(AActualLength);
      end;
      Inc(ADestination);
      Inc(ACurrentChar);
    end;
    SetLength(Result, AActualLength);
  end;

begin
  Result := '(' + CheckEscapeSequences(ConvertToAnsiString(S)) + ')';
end;
...
function TdxPSPDFCIDFont.CanEncodeText(C: Char): Boolean;
begin
  Result := TTFFile.GlyphIndex[Word(C)] <> 0;
end;
...
begin
  ...
  begin
    ...
    try
      FSymbol := ASymbol;
      cxInitLogFont(ALogFont, '');
      FAllFontsProcessed := dxEnumFontFamilies(DC, ALogFont, @EnumFontsProc, TRUETYPE_FONTTYPE, 0);
      Result := FindFontInfo(ASymbol, AInfo);
    finally
      ReleaseDC(0, DC);
    end;
