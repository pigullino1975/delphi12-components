...
unit dxRichEdit.Import.Rtf.DestinationPicture; // for internal use
...
const
  dxThisUnitName = 'dxRichEdit.Import.Rtf.DestinationPicture';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxPictureDestination.Initialize', SysInit.HInstance);{$ENDIF}
  ...
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxPictureDestination.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxPictureDestination.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxPictureDestination.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FRtfHundredthsOfMillimeterConverter);
  FreeAndNil(FRtfPixelsConverter);
  FreeAndNil(FKeywordHT);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxPictureDestination.Finalize', SysInit.HInstance);{$ENDIF}
end;
