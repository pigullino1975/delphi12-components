...
uses
  ...
  System.UITypes,
  dxCore, dxCoreClasses, dxCoreGraphics, dxActions, dxPrinting, cxGraphics, dxGallery,
  dxRichEdit.Types,
  dxRichEdit.Control,
...
type
  ...
  TdxRichEditControlCustomGalleryAction = class(TdxRichEditControlAction, IdxActionGalleryClient, IdxLocalizerListener)
  protected
    FLastMeasurementUnit: TdxMeasurementUnit;
    function GetRichEditMeasurementUnit: TdxMeasurementUnit;
  ...
  strict private
    ...
    function GetPaperDescription(const APaperKind: TdxPaperKind): string;
  ...
  TdxRichEditControlPageMarginsGallery = class(TdxRichEditControlCustomGalleryAction, IdxActionGalleryWithDescriptionTabbedText)
  protected type
  ...
  strict private
    ...
    function GetMarginsCaption(const APredefinedMarginsSet: TPredefinedMarginsSet): string;
...
uses
  Windows, 
  RTLConsts, Math, Variants, dxMeasurementUnits, dxPrintUtils, cxGeometry, cxControls, cxClasses,
...
const
  dxThisUnitName = 'dxRichEdit.Actions';
...
begin
  ...
  Visible := RichEditControl.Options.Behavior.Printing <> TdxDocumentCapability.Hidden;
end;

function TdxCustomRichEditControlPrintingAction.IsEnabled: Boolean;
begin
  Result := inherited IsEnabled and RichEditControl.InnerControl.Enabled and
    not (RichEditControl.Options.Behavior.Printing in [TdxDocumentCapability.Disabled, TdxDocumentCapability.Hidden]);
end;
...
constructor TdxRichEditControlCustomGalleryAction.Create(AOwner: TComponent);
begin
  FLastMeasurementUnit := TdxMeasurementUnit.Document;
  inherited Create(AOwner);
...
procedure TdxRichEditControlCustomGalleryAction.TranslationChanged;
begin
  BeginUpdate;
  try
...
function TdxRichEditControlCustomGalleryAction.FindGalleryGroup(const AName: string): TdxCustomGalleryGroup;
var
  AOwner: TComponent;
begin
  Result := nil;
...
function TdxRichEditControlCustomGalleryAction.FindGalleryGroupItem(AGroup: TdxCustomGalleryGroup;
...
function TdxRichEditControlCustomGalleryAction.GetRichEditMeasurementUnit: TdxMeasurementUnit;
begin
  if Assigned(RichEditControl) then
    Result := RichEditControl.MeasurementUnit
  else
    Result := TdxMeasurementUnit.Inch;
  if Result = TdxMeasurementUnit.Document then
    Result := TdxMeasurementUnit.Inch;
end;

procedure TdxRichEditControlCustomGalleryAction.UpdateGalleryContentResourceStrings;
begin
  // do nothing
end;
...
procedure TdxRichEditControlPaperSizeGallery.Assign(Source: TPersistent);
begin
  inherited;
  if Source is TdxRichEditControlPaperSizeGallery then
    FGalleryGroup := TdxRichEditControlPaperSizeGallery(Source).FGalleryGroup;
end;

procedure TdxRichEditControlPaperSizeGallery.DefineProperties(Filer: TFiler);
begin
  inherited;
  Filer.DefineProperty('GalleryGroup', ReadGalleryGroup, WriteGalleryGroup, FGalleryGroup <> nil);
end;

procedure TdxRichEditControlPaperSizeGallery.Notification(AComponent: TComponent; Operation: TOperation);
begin
  inherited;
...
begin
  ...
  if (RichEditControl <> nil) and (RichEditControl.MeasurementUnit <> FLastMeasurementUnit) then
  begin
    UpdateGalleryContentResourceStrings;
    FLastMeasurementUnit := RichEditControl.MeasurementUnit;
  end;
end;
...
begin
  ...
  begin
    ...
    AInfo.Add(APaperKind, '',
      ...
      GetPaperDescription(APaperKind), GetImageName(APaperKind));
  end;
end;
...
procedure TdxRichEditControlPaperSizeGallery.UpdateGalleryContentResourceStrings;

  procedure UpdateGalleryGroupItemResourceStrings(APaperKind: TdxPaperKind);
  var
    AItem: TdxCustomGalleryItem;
  begin
    AItem := FindGalleryGroupItem(FGalleryGroup, APaperKind);
    if AItem <> nil then
    begin
      AItem.Caption := cxGetResourceString(dxPaperKindNames[APaperKind]);
      AItem.Description := GetPaperDescription(APaperKind);
    end;
  end;
...
begin
  if FGalleryGroup <> nil then
    for I := 0 to Length(CDefaultPaperKindList) - 1 do
      UpdateGalleryGroupItemResourceStrings(CDefaultPaperKindList[I]);
end;

function TdxRichEditControlPaperSizeGallery.GetPaperDescription(const APaperKind: TdxPaperKind): string;
...
begin
  ...
  AUnitsName := TdxUnitAbbreviationDictionary.UnitAbbreviation[GetRichEditMeasurementUnit];
  AWidth := ConvertValue(APaper.Width, TdxMeasurementUnit.Millimeter, GetRichEditMeasurementUnit) / 10;
  AHeight := ConvertValue(APaper.Height, TdxMeasurementUnit.Millimeter, GetRichEditMeasurementUnit) / 10;
  Result := Format('%f %s x %f %s', [AWidth, AUnitsName, AHeight, AUnitsName]);
end;

procedure TdxRichEditControlPaperSizeGallery.ReadGalleryGroup(Reader: TReader);
begin
  FGalleryGroup := FindGalleryGroup(Reader.ReadIdent);
end;

procedure TdxRichEditControlPaperSizeGallery.WriteGalleryGroup(Writer: TWriter);
begin
...
procedure TdxRichEditControlPageMarginsGallery.Assign(Source: TPersistent);
begin
  inherited;
  if Source is TdxRichEditControlPageMarginsGallery then
    FGalleryGroup := TdxRichEditControlPageMarginsGallery(Source).FGalleryGroup;
end;

procedure TdxRichEditControlPageMarginsGallery.DefineProperties(Filer: TFiler);
begin
  inherited;
  Filer.DefineProperty('GalleryGroup', ReadGalleryGroup, WriteGalleryGroup, FGalleryGroup <> nil);
end;

procedure TdxRichEditControlPageMarginsGallery.Notification(AComponent: TComponent; Operation: TOperation);
begin
  inherited;
  if (Operation = opRemove) and (AComponent = FGalleryGroup) then
    FGalleryGroup := nil;
...
begin
  ...
  AInfo.Add(TPredefinedMarginsSet.pmsNormal, '', GetMarginsCaption(TPredefinedMarginsSet.pmsNormal),
    GetMarginsDescription(CPredefinedNormalMarginsSet), 'Pages\PageMarginsNormal.png');
  AInfo.Add(TPredefinedMarginsSet.pmsNarrow, '', GetMarginsCaption(TPredefinedMarginsSet.pmsNarrow),
    GetMarginsDescription(CPredefinedNarrowMarginsSet), 'Pages\PageMarginsNarrow.png');
  AInfo.Add(TPredefinedMarginsSet.pmsModerate, '', GetMarginsCaption(TPredefinedMarginsSet.pmsModerate),
    GetMarginsDescription(CPredefinedModerateMarginsSet), 'Pages\PageMarginsNormal.png');
  AInfo.Add(TPredefinedMarginsSet.pmsWide, '', GetMarginsCaption(TPredefinedMarginsSet.pmsWide),
...
procedure TdxRichEditControlPageMarginsGallery.UpdateGalleryContentResourceStrings;

  procedure UpdateGalleryGroupItemResourceStrings(const APredefinedMarginsSet: TPredefinedMarginsSet;
    const AMarginsSet: TMarginsSet);
  var
    AItem: TdxCustomGalleryItem;
  begin
    ...
    if AItem <> nil then
    begin
      AItem.Caption := GetMarginsCaption(APredefinedMarginsSet);
      AItem.Description := GetMarginsDescription(AMarginsSet);
    end;
  end;
...
begin
  if FGalleryGroup <> nil then
  begin
    UpdateGalleryGroupItemResourceStrings(TPredefinedMarginsSet.pmsNormal, CPredefinedNormalMarginsSet);
    UpdateGalleryGroupItemResourceStrings(TPredefinedMarginsSet.pmsNarrow, CPredefinedNarrowMarginsSet);
    UpdateGalleryGroupItemResourceStrings(TPredefinedMarginsSet.pmsModerate, CPredefinedModerateMarginsSet);
    UpdateGalleryGroupItemResourceStrings(TPredefinedMarginsSet.pmsWide, CPredefinedWideMarginsSet);
  end;
end;

function TdxRichEditControlPageMarginsGallery.GetMarginsCaption(const APredefinedMarginsSet: TPredefinedMarginsSet): string;
begin
  case APredefinedMarginsSet of
    TPredefinedMarginsSet.pmsNormal:
      Result := cxGetResourceString(@sdxRichEditCommandPageMarginsGalleryNormalMarginsCaption);
    TPredefinedMarginsSet.pmsNarrow:
      Result := cxGetResourceString(@sdxRichEditCommandPageMarginsGalleryNarrowMarginsCaption);
    TPredefinedMarginsSet.pmsModerate:
      Result := cxGetResourceString(@sdxRichEditCommandPageMarginsGalleryModerateMarginsCaption);
  else //TPredefinedMarginsSet.pmsWide:
    Result := cxGetResourceString(@sdxRichEditCommandPageMarginsGalleryWideMarginsCaption);
  end;
end;

function TdxRichEditControlPageMarginsGallery.GetMarginsDescription(const AMarginsSet: TMarginsSet): string;
var
  AUnitsName: string;
  AMeasurementUnit: TdxMeasurementUnit;
  ActualMargins: TMarginsSet;
  i: TMarginsPart;
begin
  AMeasurementUnit := GetRichEditMeasurementUnit;
  AUnitsName := TdxUnitAbbreviationDictionary.UnitAbbreviation[AMeasurementUnit];
  for I := Low(ActualMargins) to High(ActualMargins) do
    ActualMargins[I] := ConvertValue(AMarginsSet[I], TdxMeasurementUnit.Inch, AMeasurementUnit);

  Result :=
    Format('%s:'#9'%2.2f %s'#9'%s:'#9'%2.2f %s', [
      cxGetResourceString(@sdxRichEditCommandPageMarginsGalleryTopPartCaption),
      ActualMargins[TMarginsPart.mpTop], AUnitsName,
      cxGetResourceString(@sdxRichEditCommandPageMarginsGalleryBottomPartCaption),
      ActualMargins[TMarginsPart.mpBottom], AUnitsName]) +
    #13#10 +
    Format('%s:'#9'%2.2f %s'#9'%s:'#9'%2.2f %s', [
      cxGetResourceString(@sdxRichEditCommandPageMarginsGalleryLeftPartCaption),
      ActualMargins[TMarginsPart.mpLeft], AUnitsName,
      cxGetResourceString(@sdxRichEditCommandPageMarginsGalleryRightPartCaption),
      ActualMargins[TMarginsPart.mpRight], AUnitsName]);
end;

procedure TdxRichEditControlPageMarginsGallery.ReadGalleryGroup(Reader: TReader);
begin
  FGalleryGroup := FindGalleryGroup(Reader.ReadIdent);
end;

procedure TdxRichEditControlPageMarginsGallery.WriteGalleryGroup(Writer: TWriter);
begin
  Writer.WriteIdent(FGalleryGroup.Name);
end;
...
procedure TdxRichEditControlCustomStylesGalleryAction.DoUpdateState;
begin
  inherited;
  Enabled := (FGalleryGroup <> nil) and (FGalleryGroup.Items.Count > 0);
end;
...
begin
  ...
  try
    ...
    AGalleryGroupItem.Glyph.SourceDPI := cxGetCurrentDPI;
  finally
    ABitmap.Free;
  end;
...
begin 
  ...
  begin
    ...
    if AStyle.StyleType = TdxStyleType.ParagraphStyle then
    begin
      AParagraphStyle := TdxParagraphStyle(AStyle);
      for I := AModel.Selection.Interval.NormalizedStart.ParagraphIndex to AModel.Selection.Interval.NormalizedEnd.ParagraphIndex do
      begin
        AModel.ActivePieceTable.Paragraphs[I].ParagraphStyleIndex := AModel.ParagraphStyles.IndexOf(AStyle);
        AModel.ActivePieceTable.Paragraphs[I].ParagraphProperties.ResetUse(AParagraphStyle.ParagraphProperties.Info.Options.Value);
        for J := AModel.ActivePieceTable.Paragraphs[I].FirstRunIndex to AModel.ActivePieceTable.Paragraphs[I].LastRunIndex do
          AModel.ActivePieceTable.Runs[J].CharacterProperties.ResetUse(AParagraphStyle.CharacterProperties.Info.Options.Value);
      end;
    end
    else
    ...
    begin
      ...
      begin
        ...
        AModel.ActivePieceTable.Runs[I].CharacterProperties.ResetUse(TdxCharacterStyle(AStyle).CharacterProperties.Info.Options.Value);
      end;
    end;
  end;
end;
...
begin
  if (AStyleName <> '') and (RichEditControl.Document <> nil) then
  begin
