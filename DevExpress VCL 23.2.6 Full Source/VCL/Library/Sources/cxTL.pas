...
uses
  ...
  Forms, StdCtrls, Menus, Graphics, ImgList, ComCtrls, Contnrs, Math, MultiMon,
{$IFDEF DELPHI103}
...
{$ENDIF}
  ...
  dxScrollbarAnnotations, cxFindPanel, cxDataControllerSpreadSheetExpressionProvider,
  dxAnimation, dxTypeHelpers, dxScrollAnimation, dxCoreGraphics, dxStringHelper;
...
type
  ...
  IcxTreeListDesignTimeOperations2 = interface(IcxTreeListDesignTimeOperations)
  ['{B797D20A-D859-495D-9E0B-7DFB410AE627}']
    function CanCreateAllItems: Boolean;
  end;
  ...
  TcxTreeListChange = (tcStructure, tcData, tcLayout, tcImages, tcColumns, tcSortOrder, tcLoading,
                      tcSelection, tcFocusedNode, tcSummary, tcFilterCriteria, tcFindCriteria);
  ...
  private
    ...
    function GetErrorCode(AIndex: Integer): Integer;
  ...
  public
    ...
    property ErrorCodes[Index: Integer]: Integer read GetErrorCode;
  ...
  published
    ...
    property FilteredItemsListShowFilteredItemsOnly;
  ...
  protected
    ...
    //IcxDataControllerSpreadSheetExpressionItem
    function IcxDataControllerSpreadSheetExpressionItem.CanUseInExpression = IsVisibleForExpressionEditor;
    function IcxDataControllerSpreadSheetExpressionItem.GetReferenceName = GetReferenceNameForExpression;
    function GetReferenceNameForExpression: string;
    // column methods
  ...
  public
    function GetItemCaption(AItem: TcxFindPanelItemKind): string; override;
 ...
 { TcxTreeListColumnsCustomizationPopup }
  ...
  public
    ...
    function CanInsertNode(AIsAppend: Boolean = False): Boolean;
    ...
    procedure EndDragAndDrop(Accepted: Boolean); override;
    procedure EndUpdateSelection(ACancel: Boolean);
 ...
 { TcxTreeListHitTest }
  ...
  private
    ...
    function GetGlyphColorPalette: IdxColorPalette;
  ...
  public
    ...
    property GlyphColorPalette: IdxColorPalette read GetGlyphColorPalette;
  ...
  protected
    ...
    procedure InitializeWithBorders(ABand: TcxTreeListBand; const ATreeListBounds, ABandRect: TRect);
  public
    destructor Destroy; override;
    class function CustomDrawID: Integer; override;
  ...
  protected
    ...
    function GetCellBorderWidth: Integer; override;
    ...
    procedure GetFindTextInfo(out AStart: Integer; out ALength: Integer); override;
    ...
    function GetRealCellEditRect: TRect; override;
  ...
  public
    Bounds: TRect;
    Cells: TcxCustomControlCells; // for internal use
    ...
    ViewInfo: TcxTreeListViewInfo; // for internal use
  ...
  TcxTreeListColumnScrollAnimationHelper = class(TdxScrollAnimationHelper)
  strict private
  ...
  TcxTreeListNodePixelScrollData = record
    Node: TcxTreeListNode;
    ScrollOffset: Integer;
    constructor Create(ANode: TcxTreeListNode; AOffset: Integer);
  end;

  TcxTreeListNodeScrollAnimationHelper = class(TdxCustomScrollAnimationHelper<TcxTreeListNodePixelScrollData>)
  strict private
    FIsScrollingByRecord: Boolean;
    FOwner: TcxTreeListViewInfo;
  protected
    procedure AfterAnimationEnded(AIsRaiseEvent: Boolean); override;
    function CalculateAnimatedPosition(var APosition: Integer): TcxTreeListNodePixelScrollData; override;
    function CalculateScrollDelta(AValue: TcxTreeListNodePixelScrollData): Integer; override;
    function GetOwnerPosition: TcxTreeListNodePixelScrollData; override;
    function GetUnassignedTargetPosition: TcxTreeListNodePixelScrollData; override;
    function IsEqual(AValue1, AValue2: TcxTreeListNodePixelScrollData): Boolean; override;
    procedure BeforeAnimationStarted; override;
    procedure UpdateOwnerPosition(AValue: TcxTreeListNodePixelScrollData); override;
  ...
  private
    ...
    FColumnScrollAnimationHelper: TcxTreeListColumnScrollAnimationHelper;
    ...
    FFooterCellTextOffsets: TRect;
    FFooterLineHeight: Integer;
    FFooterPanelContentOffsets: TRect;
    FFooterVerticalSpaceBetweenItems: Integer;
    FFooterHorizontalSpaceBetweenItems: Integer;
    ...
    FMultiRows: Boolean;
    FNodeScrollAnimationHelper: TcxTreeListNodeScrollAnimationHelper;
    ...
    procedure DoSetHScrollPos(APosition: Integer);
    procedure DoSetLastNode(ANode: TcxTreeListNode; AAnimated: Boolean);
    function GetBands: TcxTreeListBands;
    ...
    function GetEditingController: TcxTreeListEditingController;
    function GetFilterBoxViewInfo: TcxTreeListFilterBoxViewInfo;
    ...
    procedure RefreshFooterOffsets;
    procedure SetCalculateInProcess(AValue: Boolean);
    procedure SetHScrollPosAnimated(APosition: Integer);
  ...
  protected
    ...
    function AddColumnHeader(AColumn: TcxTreeListColumn; ABandRowVisibleIndex, AColumnVisibleIndexInTheBandRow: Integer;
    ...
    // scroll animation
    procedure DestroyColumnScrollAnimation;
    procedure DestroyNodeScrollAnimation;
    function IsColumnScrollAnimationActive: Boolean;
    function IsScrollAnimationActive: Boolean;
    function IsScrollAnimationEnabled: Boolean;
    function IsNodeScrollAnimationActive: Boolean;
    procedure NodeScrollAnimated(ATopNode: TcxTreeListNode; ATopRecordPixelScrollOffset: Integer);
    procedure ScrollAnimationEnded;
    procedure ScrollAnimationStarted;
    procedure StopColumnScrollAnimation;
    ...
    procedure DoSetTopNodeWithOffset(ANode: TcxTreeListNode; APixelScrollNodeOffset: Integer; ARaiseEvent: Boolean);
    function GetContentBounds: TRect; virtual;
    function GetPixelScrollOffset(AFirstNode: TcxTreeListNode; AFirstNodeOffset: Integer; ALastNode: TcxTreeListNode; ALastNodeOffset: Integer): Integer;
    function GetRealContentBounds: TRect;
    ...
    function GetFooterLineHeight(AFont: TFont): Integer;
    ...
    function GetLastNodeByTopNode(ATopNode: TcxTreeListNode; ATopNodePixelScrollOffset: Integer): TcxTreeListNode;
    ...
    procedure SetTopNodeWithOffset(ANode: TcxTreeListNode; APixelScrollNodeOffset: Integer; AAnimated: Boolean);
    ...
    property FilterBoxViewInfo: TcxTreeListFilterBoxViewInfo read GetFilterBoxViewInfo;
    property FooterCellTextOffsets: TRect read FFooterCellTextOffsets;
    property FooterPanelContentOffsets: TRect read FFooterPanelContentOffsets;
  ...
  public
    ...
    property Cells: TcxCustomControlCells read FCells write FCells; // for internal use
    property CheckboxSize: TSize read GetCheckBoxSize;
    property CheckBoxViewInfo: TcxCustomEditViewInfo read FCheckBoxViewInfo;
    property CheckBoxViewData: TcxCustomEditViewData read FCheckBoxViewData; // for internal use
    ...
    property GridLineWidth: Integer read FGridLineWidth; // for internal use
    ...
    property HorzBackgroundCell: TcxTreeListBackgroundCellViewInfo read FHorzBackgroundCell; // for internal use
    ...
    property IsRightToLeftConverted: Boolean read FIsRightToLeftConverted;
    property LevelInfo[ALevel: Integer]: TcxTreeListLevelInfo read GetLevelInfo; // for internal use
    ...
    property NodeViewData[Index: Integer]: TcxTreeListNodeViewData read GetNodeViewData; // for internal use
    property OptionsView: TcxTreeListOptionsView read GetOptionsView;
    property OriginalViewInfo: TcxTreeListViewInfo read GetOriginalViewInfo; // for internal use
  ...
  protected
    ...
    function IsNextToLeftFixedBands: Boolean;
    function IsPreviousToRightFixedBands: Boolean;
    function IsLeftMostVisible: Boolean;
    function IsFirstRightFixed: Boolean;
    function IsRightMostVisible: Boolean;
    function IsLastLeftFixed: Boolean;
  ...
  protected
    ...
    procedure AssignBands(ASource: TcxTreeListBands; ARecreate: Boolean);
    procedure AssignColumnsWidth;
  ...
  TcxTreeListScrollbarAnnotationOptions = class(TcxControlScrollbarAnnotationOptions)
  published
    property Active;
    property CustomAnnotations;
    property ShowErrors;
    property ShowSearchResults;
    property ShowFocusedRow;
    property ShowSelectedRows;
  end;
  ...
  TcxTreeListDataFindCriteria = class(TcxDataFindCriteria)
  private
    function GetDataController: TcxTreeListDataController;
    function GetMatchNode(AMatchIndex: Integer): TcxTreeListNode;
    function GetMatchNodeVisibleIndex(AMatchIndex: Integer): Integer;
    function GetTreeList: TcxCustomTreeList;
  protected
    procedure DoBeforeChange(const AChange: TcxDataFindCriteriaChange); override;
    function GetFocusedIndex: Integer; override;
    function GetMatchCount: Integer; override;
    function GoToMatch(AMatchIndex: Integer): Boolean; override;
    procedure PopulateMatches; override;
    function UseMatches: Boolean; override;

    property DataController: TcxTreeListDataController read GetDataController;
    property NodeVisibleIndexes[AMatchIndex: Integer]: Integer read GetMatchNodeVisibleIndex;
    property TreeList: TcxCustomTreeList read GetTreeList;
  public
    function GetTextStartPositionByNode(ANode: TcxTreeListNode; AItemIndex: Integer;
      out AHighlightedText: string): Integer; virtual;

    property MatchNodes[AMatchIndex: Integer]: TcxTreeListNode read GetMatchNode;
  end;
  ...
  private
    ...
    procedure DoClearNodeValueProc(ANode: TcxTreeListNode; Data: Pointer);
    procedure DoFilterNodeProc(ANode: TcxTreeListNode; Data: Pointer);
    function GetFindCriteria: TcxTreeListDataFindCriteria;
    function GetHasEditData: Boolean;
    function GetRoot: TcxTreeListNode;
    function GetTreeList: TcxCustomTreeList;
    function GetValueDef(AIndex: Integer): TcxValueDef; overload;
    function GetValueDef(AField: TcxCustomDataField): TcxValueDef; overload;
    procedure FreeRecordProc(ANode: TcxTreeListNode; AData: Pointer);
    procedure InsertValueDef(AField: TcxCustomDataField); overload;
  ...
  protected
    ...
    procedure ClearFieldValueDef(AField: TcxCustomDataField); override;
    procedure ClearFieldValues(AField: TcxCustomDataField); override;
    procedure ClearNodeExpressionFieldValues(ANode: TcxTreeListNode); virtual;
    procedure ClearNodeValue(ANode: TcxTreeListNode; AField: TcxCustomDataField); virtual;
    procedure ClearValue(ARecordIndex: Integer; AField: TcxCustomDataField); override;
    ...
    procedure FieldExpressionChanged; override;
    ...
    function GetErrorCode(ARecordIndex, AItemIndex: Integer): Integer; override;
    ...
    function GetFilteringRecordCount(AUseFiltered: Boolean): Integer; override;
    function GetFilterDisplayText(ARecordIndex: Integer; AItemIndex: Integer): string; override;
    function GetFilterRecordIndexByFilteringRecordIndex(AUseFiltered: Boolean; AIndex: Integer): Integer; override;
    function GetFilterRecordIndexByNode(ANode: TcxTreeListNode): Integer; virtual;
    function GetInternalValue(ARecordIndex: Integer; AField: TcxCustomDataField): Variant; override;
    function GetNodeByFilterRecordIndex(ARecordIndex: Integer): TcxTreeListNode; virtual;
    function GetNodeByRecordIndex(ARecordIndex: Integer): TcxTreeListNode; virtual;
    function GetNodeErrorCode(ANode: TcxTreeListNode; AIndex: Integer): Integer; virtual;
    function GetNodeExpressionDisplayText(ANode: TcxTreeListNode; AIndex: Integer): string; virtual;
    function GetNodeExpressionValue(ANode: TcxTreeListNode; AIndex: Integer): Variant; virtual;
    function GetNodeStoredDisplayText(ANode: TcxTreeListNode; AIndex: Integer): string; virtual;
    function GetNodeStoredValue(ANode: TcxTreeListNode; AIndex: Integer): Variant; virtual;
    function GetRecordIndexByNode(ANode: TcxTreeListNode): Integer; virtual;
    function GetStoredDisplayText(ARecordIndex: Integer; AField: TcxCustomDataField): string; override;
    function GetStoredErrorCode(ARecordIndex: Integer; AField: TcxCustomDataField): Integer; override;
    function GetStoredValue(ARecordIndex: Integer; AField: TcxCustomDataField): Variant; override;
    function HasFilterEvent: Boolean; override;
    procedure InitFieldValueDef(AField: TcxCustomDataField); override;
    function IsHiddenByFilter(ANode: TcxTreeListNode): Boolean; virtual;
    function IsStoredValueEmpty(ARecordIndex: Integer; AField: TcxCustomDataField): Boolean; override;
    procedure ResetNodesFiltering; virtual;
    procedure SetNodeDisplayText(ANode: TcxTreeListNode; AIndex: Integer; const Value: string); virtual;
    procedure SetNodeStoredValue(ANode: TcxTreeListNode; AIndex: Integer; const Value: Variant); virtual;
    procedure SetStoredErrorCode(ARecordIndex: Integer; AField: TcxCustomDataField; ACode: Integer); override;
    procedure SetStoredValue(ARecordIndex: Integer; AField: TcxCustomDataField; const Value: Variant); override;
    // Bookmark
    ...
    procedure SetNodeData(ASource, ADest: TcxTreeListNode);
  ...
  public
    ...
    property FindCriteria: TcxTreeListDataFindCriteria read GetFindCriteria;
  ...
  TcxTreeListNodeCheckChangingEvent = procedure(
    Sender: TcxCustomTreeList; ANode: TcxTreeListNode; AState: TcxCheckBoxState; var Allow: Boolean) of object;
  ...
  protected
    ...
    function GetMaxRowIndex: Integer; override;
  ...
  TcxTreeListSearchEngineClass = class of TcxTreeListSearchEngine;

  { TcxTreeListSearchEngine }

  TcxTreeListSearchEngine = class // for internal use
  type
    TSearchStrategyClass = class of TCustomSearchStrategy;

    { Functions Match }

    TMatchFunc = TcxTreeListFindFunc;

    { TArguments }

    TArguments = record
      TreeList: TcxCustomTreeList;
      Data: Pointer; 
      Match: TMatchFunc; 
      StartNode: TcxTreeListNode;
      FinishNode: TcxTreeListNode;
      ExpandedOnly: Boolean;
      Forward: Boolean;
      IgnoreStartNode: Boolean;
      constructor Create(ATreeList: TcxCustomTreeList; AData: Pointer; AStartNode, AFinishNode: TcxTreeListNode;
        AExpandedOnly, AForward: Boolean; AMatch: TcxTreeListFindFunc; AIgnoreStartNode: Boolean);
    ...
    public
      ...
      constructor Create(AText: string; AColumn: TcxTreeListColumn; ACaseSensitive: Boolean; AMode: TcxTreeListFindMode; ALikeParams: TcxTreeListLikeParams);
      destructor Destroy; override;
      function Match(ANodeText: string): Boolean;
    end;
    ...
    TMatcher = class
    public
      class function Match(ANode: TcxTreeListNode; AData: Pointer): Boolean; static;
    end;

    { TCustomSearchStrategy }

    TCustomSearchStrategy = class abstract
    strict private
      FArguments: PArguments;
    protected
      property Arguments: PArguments read FArguments;
    public
      constructor Create(AArguments: PArguments); virtual;
      function Execute: TcxTreeListNode; virtual; abstract;
    end;

    { TTreeSearchStrategy }

    TTreeSearchStrategy = class(TCustomSearchStrategy)
    protected
      function IsFinishNode(ANode: TcxTreeListNode): Boolean; virtual;
      function LoadChildren(ANode: TcxTreeListNode; AExpandedOnly: Boolean): Boolean; virtual;
      function FindNext(ANode: TcxTreeListNode): TcxTreeListNode; virtual;
      function FindPrev(ANode: TcxTreeListNode): TcxTreeListNode;
      function GoToNext(ANode: TcxTreeListNode): TcxTreeListNode;
    public
      function Execute: TcxTreeListNode; override;
    end;

    { TSearchStrategyInExpandedNodes }

    TExpandedNodesSearchStrategy = class(TTreeSearchStrategy)
    protected
      function LoadChildren(ANode: TcxTreeListNode; AExpandedOnly: Boolean): Boolean; override;
    end;

    { TSiblingSearchStrategy }

    TSiblingSearchStrategy = class(TTreeSearchStrategy)
    public
      function Execute: TcxTreeListNode; override;
    end;

    { THybridSearchStrategy }

    THybridSearchStrategy = class(TCustomSearchStrategy)
    public
      function Execute: TcxTreeListNode; override;
    end;

    { TCachedSearchStrategy }

    TCachedSearchStrategy = class sealed(TTreeSearchStrategy)
    type
      TExpandedNodeBasedStrategy = class(TExpandedNodesSearchStrategy)
      protected
        Nodes: TList;
        function LoadChildren(ANode: TcxTreeListNode; AExpandedOnly: Boolean): Boolean; override;
      end;
    strict private
      ...
      FNodeLast: TcxTreeListNode;
      FIsLastNodeFound: Boolean;
      function InternalLoadChildren(ANode: TcxTreeListNode): Boolean;
    protected
      function FindNext(ANode: TcxTreeListNode): TcxTreeListNode; override;
      function IsFinishNode(ANode: TcxTreeListNode): Boolean; override;
    public
  ...
  strict private
    class procedure Lock(ATreeList: TcxCustomTreeList); static;
    class procedure Unlock(ATreeList: TcxCustomTreeList); static;
  protected
    class function Execute(AClass: TSearchStrategyClass; AArguments: PArguments): TcxTreeListNode; static;
  public
    class function GetTextMatchFunc(AArguments: TcxTreeListSearchEngine.PArguments): TcxTreeListSearchEngine.TMatchFunc; virtual;
    class function Search(AArguments: TcxTreeListSearchEngine.PArguments): TcxTreeListNode; virtual;
    class function FullSearch(AArguments: PArguments): TcxTreeListNode; static;
    class function HybridSearch(AArguments: PArguments): TcxTreeListNode; static;
    class function CachedSearch(AArguments: PArguments): TcxTreeListNode; static;
    class function SearchAtLevel(AArguments: TcxTreeListSearchEngine.PArguments): TcxTreeListNode; static;
    class function SearchAmongExpandedNodes(AArguments: TcxTreeListSearchEngine.PArguments): TcxTreeListNode; static;
  end;
  ...
  private
    ...
    FAppendToFocused: Boolean;
    FBands: TcxTreeListBands;
    FCancelKeyWatcher: TdxKeyPressWatcher;
    FChanges: TcxTreeListChanges;
    FChangesLocked: Integer;
    ...
    FSearchEngineClass: TcxTreeListSearchEngineClass;
    ...
    procedure AfterNodeFiltering;
    procedure DoUnselectNodeAfterFilteringProc(ANode: TcxTreeListNode; Data: Pointer);
    ...
    function GetChangesLocked: Boolean;
    function GetColumn(AIndex: Integer): TcxTreeListColumn;
    function GetColumnCount: Integer;
    ...
    function GetOptionsView: TcxTreeListOptionsView;
    function GetScrollbarAnnotations: TcxTreeListScrollbarAnnotationOptions;
    ...
    procedure SetChangesLocked(const AValue: Boolean);
    ...
    procedure SetFilter(AValue: TcxDataFilterCriteria);
    ...
    procedure SetScrollbarAnnotations(Value: TcxTreeListScrollbarAnnotationOptions);
    ...
    procedure DoAddAbsoluteItem(ANode: TcxTreeListNode; AData: Pointer);
    procedure DoChangedTimer(Sender: TObject);
    procedure InvalidateScrollbarAnnotations;
  ...
  protected
    ...
    procedure AssignData(ASource: TcxCustomTreeList; ARecreateBandsAndColumns: Boolean); virtual;
    ...
    procedure CheckStructureAbsoluteItems;
    procedure CheckStructure; virtual;
    procedure ClearCalculatedWidths;
    procedure ColumnExpressionChanged; virtual;
    ...
    function CreateScrollbarAnnotations: TcxControlScrollbarAnnotationOptions; override;
    ...
    procedure DoSetTopVisibleNode(ANode: TcxTreeListNode; ARaiseEvent: Boolean);
    ...
    procedure InternalDeselectNode(ANode: TcxTreeListNode);
    function IsRecordIndexBasedScrollbarAnnotationRendering: Boolean; override;
    // used classes
    ...
    function IsSortedColumnsSupportMultiThreading: Boolean; virtual;
    procedure LookAndFeelChanged(Sender: TcxLookAndFeel; AChangedValues: TcxLookAndFeelValues); override;
    ...
    function CanCreateAllItems: Boolean; virtual;
...
//    procedure DoSetNodeSelected(ANode: TcxTreeListNode; Value: Boolean; Shift: TShiftState = []); virtual;
    ...
    function GetCurrentScrollbarAnnotationRecordIndex: Integer; override;
    ...
    function IsScrollbarAnnotationVisible(AKind: Integer): Boolean; override;
    ...
    procedure PopulateScrollbarAnnotations(AKind: TdxScrollbarAnnotationKind; AList: TdxScrollbarAnnotationRowIndexList); override;
    procedure SetCurrentScrollbarAnnotationRecordIndex(AIndex: Integer); override;
    //
    ...
    class function GetSearchEngineClass: TcxTreeListSearchEngineClass; virtual;
    ...
    property AppendToFocused: Boolean read FAppendToFocused write FAppendToFocused;
    property AvailableContentWidth: Integer read GetAvailableContentWidth;
    property CancelKeyWatcher: TdxKeyPressWatcher read FCancelKeyWatcher;
    property ChangeDelayTimer: TcxTimer read FDelayTimer;
    property Changes: TcxTreeListChanges read FChanges write FChanges;
    property ChangesLocked: Boolean read GetChangesLocked write SetChangesLocked;
    ...
    property SearchEngineClass: TcxTreeListSearchEngineClass read FSearchEngineClass write FSearchEngineClass;
  ...
  public
    ...
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
    procedure AssignTreeList(Source: TcxCustomTreeList; ARecreateBandsAndColumns: Boolean); // for internal use
    ...
    // find
    function Find(AData: Pointer; AStart: TcxTreeListNode; AExpandedOnly, AForward: Boolean; AFilter: TcxTreeListFindFunc; AIgnoreStartNode: Boolean = False): TcxTreeListNode; virtual;
    ...
    property Filter: TcxDataFilterCriteria read GetFilter write SetFilter;
    property FindCriteria: TcxTreeListDataFindCriteria read GetFindCriteria;
    property FindPanel;
    ...
    property GridLineWidth: Integer read GetGridLineWidth; // for internal use
    property HitTest: TcxTreeListHitTest read GetHitTest;
    ...
    property ScrollbarAnnotations: TcxTreeListScrollbarAnnotationOptions read GetScrollbarAnnotations write SetScrollbarAnnotations;
...
uses
  Types, CommCtrl, RTLConsts, cxLike, cxFormats, dxThreading, dxDPIAwareUtils, cxFilterControlDialog,
...
const
  ...
  ColumnPropertiesName: array[0..11] of string =
    ('Visible', 'Index', 'LineCount', 'ColIndex', 'RowIndex',
     'BandIndex', 'Caption', 'SortOrder', 'SortIndex', 'RealMinWidth', 'RealWidth', 'Expression');
...
function cxExcludeBorders(const ARect: TRect; ABorders: TcxBorders; ABorderWidth: Integer): TRect;
begin
  Result := ARect;
  with Result do
  begin
    Inc(Left, Byte(bLeft in ABorders) * ABorderWidth);
    Inc(Top, Byte(bTop in ABorders) * ABorderWidth);
    Dec(Right, Byte(bRight in ABorders) * ABorderWidth);
    Dec(Bottom, Byte(bBottom in ABorders) * ABorderWidth);
  end;
end;
...
begin
  ...
  begin
    AColumn := TcxTreeListColumn(AColumns[I]);
    AField := Fields[AColumn.ItemIndex];
    if IsConversionNeededForCompare(AField) or AColumn.IsExpression then
    begin
      ...
      Result := Compare(AColumn.ValueDef,
...
function TcxTreeListDataController.GetFilterDataValue(ARecordIndex: Integer; AField: TcxCustomDataField): Variant;
begin
  Result := GetNodeByFilterRecordIndex(ARecordIndex).Values[AField.Index];
end;
...
begin
  if ANode.Handle = nil then
    ...
    if IsExpression(AIndex) then
      Result := GetNodeExpressionDisplayText(ANode, AIndex)
    else
      Result := GetNodeStoredDisplayText(ANode, AIndex);
end;
...
begin
  if ANode.Handle = nil then
    ...
    if IsExpression(AIndex) then
      Result := GetNodeExpressionValue(ANode, AIndex)
    else
      Result := GetNodeStoredValue(ANode, AIndex);
end;
...
begin
  if IsSortByDisplayTextNeeded(AField) then
    Result := GetNodeDisplayText(ANode, AField.Index)
  else
    Result := GetNodeValue(ANode, AField.Index);
end;
...
begin
  RemoveValueDef(TcxTreeListColumn(AItem).ValueDef);
  TreeList.Controller.CancelIncSearching;
...
begin
  if (ANode = nil) or IsExpression(AIndex) then Exit;
  if ANode.Handle = nil then
    ANode.InitializeHandle;
  if ANode.Handle = nil then Exit;
  if ValueDefs[AIndex] = nil then Exit;
  SetNodeStoredValue(ANode, AIndex, AValue);
  ClearNodeExpressionFieldValues(ANode);
  FIsValueChanged := True;
  if IsLoading then Exit;
  TreeList.AddChanges([tcSummary, tcData]);
  TreeList.DoNodeChanged(ANode, TreeList.Columns[AIndex]);
end;
...
procedure TcxTreeListDataController.ClearFieldValueDef(AField: TcxCustomDataField);
begin
  RemoveValueDef(GetValueDef(AField));
  TcxCustomDataHelper.SetTextStored(AField, not AField.IsInternal and TcxTreeListColumn(AField.Item).GetIsTextStored);
end;

procedure TcxTreeListDataController.ClearFieldValues(AField: TcxCustomDataField);
begin
  ForEachNode(DoClearNodeValueProc, AField);
end;

procedure TcxTreeListDataController.ClearNodeExpressionFieldValues(ANode: TcxTreeListNode);
begin
  ClearExpressionFieldValues(ANode.FAbsoluteIndex)
end;

procedure TcxTreeListDataController.ClearNodeValue(ANode: TcxTreeListNode; AField: TcxCustomDataField);
var
  AValueDef: TcxValueDef;
begin
  if ANode.Handle = nil then
    Exit;
  AValueDef := GetValueDef(AField);
  if AValueDef <> nil then
    TcxDataStorageHelper.ClearValue(ANode.Handle, AValueDef);
end;

procedure TcxTreeListDataController.ClearValue(ARecordIndex: Integer; AField: TcxCustomDataField);
begin
  if ARecordIndex >= 0 then
   ClearNodeValue(GetNodeByRecordIndex(ARecordIndex), AField);
end;
...
function TcxTreeListDataController.CreateFindCriteria: TcxDataFindCriteria;
begin
  Result := TcxTreeListDataFindCriteria.Create(Self);
end;
...
begin
  Result := TreeList.DoFilterNodeEvent(GetNodeByFilterRecordIndex(ARecordIndex));
end;

procedure TcxTreeListDataController.FieldExpressionChanged;
begin
  TreeList.ColumnExpressionChanged;
end;

procedure TcxTreeListDataController.FilterChanged;
begin
  TreeList.Controller.EditingController.HideEdit(True);
  TreeList.FilterChanged;
  SyncFocused(TreeList.FocusedNode);
end;
...
procedure TcxTreeListDataController.FindCriteriaChanged;
begin
  TreeList.FindCriteriaChanged;
end;
...
function TcxTreeListDataController.GetErrorCode(ARecordIndex, AItemIndex: Integer): Integer;
begin
...
var
  ANode: TcxTreeListNode;
begin
  ANode := TreeList.AbsoluteVisibleItems[Index];
  Result := GetRecordIndexByNode(ANode);
end;

function TcxTreeListDataController.GetFilteringRecordCount(AUseFiltered: Boolean): Integer;
begin
  if AUseFiltered then
    Result := inherited GetFilteringRecordCount(AUseFiltered)
  else
    Result := TreeList.AbsoluteCount;
end;

function TcxTreeListDataController.GetFilterDisplayText(ARecordIndex: Integer; AItemIndex: Integer): string;
begin
  Result := GetNodeByFilterRecordIndex(ARecordIndex).Texts[AItemIndex];
end;

function TcxTreeListDataController.GetFilterRecordIndexByFilteringRecordIndex(AUseFiltered: Boolean; AIndex: Integer): Integer;
var
  ARecordIndex: Integer;
  ANode: TcxTreeListNode;
begin
  ARecordIndex := inherited GetFilterRecordIndexByFilteringRecordIndex(AUseFiltered, AIndex);
  ANode := GetNodeByRecordIndex(ARecordIndex);
  Result := GetFilterRecordIndexByNode(ANode);
end;

function TcxTreeListDataController.GetFilterRecordIndexByNode(ANode: TcxTreeListNode): Integer;
begin
  Result := ANode.FAbsoluteIndex;
end;
...
function TcxTreeListDataController.GetNodeByFilterRecordIndex(ARecordIndex: Integer): TcxTreeListNode;
begin
  Result := GetNodeByRecordIndex(ARecordIndex);
end;

function TcxTreeListDataController.GetNodeByRecordIndex(ARecordIndex: Integer): TcxTreeListNode;
begin
  Result := TreeList.FAbsoluteItems[ARecordIndex];
end;

function TcxTreeListDataController.GetNodeErrorCode(ANode: TcxTreeListNode; AIndex: Integer): Integer;
begin
  if ANode.Handle = nil then
    Result := 0
  else
    Result := ErrorCodes[GetRecordIndexByNode(ANode), AIndex];
end;

function TcxTreeListDataController.GetNodeExpressionDisplayText(ANode: TcxTreeListNode; AIndex: Integer): string;
begin
  Result := GetExpressionFieldDisplayText(GetRecordIndexByNode(ANode), Fields[AIndex]);
end;

function TcxTreeListDataController.GetNodeExpressionValue(ANode: TcxTreeListNode; AIndex: Integer): Variant;
begin
  Result := GetExpressionFieldValue(GetRecordIndexByNode(ANode), Fields[AIndex]);
end;

function TcxTreeListDataController.GetNodeStoredDisplayText(ANode: TcxTreeListNode; AIndex: Integer): string;
var
  AValueDef: TcxValueDef;
begin
  AValueDef := GetValueDef(AIndex);
  if AValueDef <> nil then
    Result := TcxDataStorageHelper.GetDisplayText(AValueDef, ANode.Handle)
  else
    Result := '';
end;

function TcxTreeListDataController.GetNodeStoredValue(ANode: TcxTreeListNode; AIndex: Integer): Variant;
var
  AValueDef: TcxValueDef;
begin
  AValueDef := GetValueDef(AIndex);
  if AValueDef <> nil then
    Result := TcxDataStorageHelper.GetValue(AValueDef, ANode.Handle)
  else
    Result := Null;
end;
...
function TcxTreeListDataController.GetStoredDisplayText(ARecordIndex: Integer; AField: TcxCustomDataField): string;
begin
  Result := GetNodeStoredDisplayText(GetNodeByRecordIndex(ARecordIndex), AField.Index);
end;

function TcxTreeListDataController.GetStoredErrorCode(ARecordIndex: Integer; AField: TcxCustomDataField): Integer;
var
  AValueDef: TcxValueDef;
begin
  AValueDef := TcxCustomDataHelper.GetValueDef(AField);
  if AValueDef <> nil then
    Result := TcxDataStorageHelper.GetErrorCode(AValueDef, GetNodeByRecordIndex(ARecordIndex).Handle)
  else
    Result := 0;
end;

function TcxTreeListDataController.GetStoredValue(ARecordIndex: Integer; AField: TcxCustomDataField): Variant;
begin
  Result := GetNodeStoredValue(GetNodeByRecordIndex(ARecordIndex), AField.Index);
end;
...
procedure TcxTreeListDataController.InitFieldValueDef(AField: TcxCustomDataField);
begin
...
begin
  Result := not ANode.IsRoot and not DoGlobalFilterRecord(GetFilterRecordIndexByNode(ANode));
end;

function TcxTreeListDataController.IsStoredValueEmpty(ARecordIndex: Integer; AField: TcxCustomDataField): Boolean;
var
  AValueDef: TcxValueDef;
begin
  AValueDef := TcxCustomDataHelper.GetValueDef(AField);
  Result := (AValueDef = nil) or TcxDataStorageHelper.IsValueEmpty(AValueDef, GetNodeByRecordIndex(ARecordIndex).Handle);
end;
...
procedure TcxTreeListDataController.SetNodeDisplayText(ANode: TcxTreeListNode; AIndex: Integer; const Value: string);
var
  AValueDef: TcxValueDef;
begin
  AValueDef := GetValueDef(AIndex);
  if AValueDef <> nil then
    TcxDataStorageHelper.SetDisplayText(ANode.Handle, AValueDef, Value);
end;

procedure TcxTreeListDataController.SetNodeStoredValue(ANode: TcxTreeListNode; AIndex: Integer; const Value: Variant);
var
  AValueDef: TcxValueDef;
begin
  AValueDef := GetValueDef(AIndex);
  if AValueDef <> nil then
    TcxDataStorageHelper.SetValue(ANode.Handle, AValueDef, Value);
end;

procedure TcxTreeListDataController.SetStoredErrorCode(ARecordIndex: Integer; AField: TcxCustomDataField; ACode: Integer);
var
  AValueDef: TcxValueDef;
begin
  AValueDef := TcxCustomDataHelper.GetValueDef(AField);
  if AValueDef <> nil then
    TcxDataStorageHelper.SetErrorCode(GetNodeByRecordIndex(ARecordIndex).Handle, AValueDef, ACode);
end;

procedure TcxTreeListDataController.SetStoredValue(ARecordIndex: Integer; AField: TcxCustomDataField; const Value: Variant);
begin
  SetNodeStoredValue(GetNodeByRecordIndex(ARecordIndex), AField.Index, Value);
end;
...
procedure TcxTreeListDataController.SetNodeData(ASource, ADest: TcxTreeListNode);
var
  I: Integer;
begin
  TreeList.BeginUpdate;
  try
    for I := 0 to TreeList.ColumnCount - 1 do
      SetNodeValue(ADest, I, GetNodeStoredValue(ASource, I));
  finally
    TreeList.EndUpdate;
  end;
end;
...
procedure TcxTreeListDataController.DoClearNodeValueProc(ANode: TcxTreeListNode; Data: Pointer);
begin
  ClearNodeValue(ANode, Data);
end;

procedure TcxTreeListDataController.DoFilterNodeProc(ANode: TcxTreeListNode; Data: Pointer);
begin
  if ANode.Focused and TreeList.Controller.IsEditing then
    ANode.HiddenByFilter := False
  else
    ANode.HiddenByFilter := IsHiddenByFilter(ANode);
end;

function TcxTreeListDataController.GetFindCriteria: TcxTreeListDataFindCriteria;
begin
  Result := TcxTreeListDataFindCriteria(inherited FindCriteria);
end;
...
begin
  Result := GetValueDef(Fields[AIndex]);
end;

function TcxTreeListDataController.GetValueDef(AField: TcxCustomDataField): TcxValueDef;
begin
  Result := TcxCustomDataHelper.GetValueDef(AField);
end;
...
procedure TcxTreeListDataController.InsertValueDef(AField: TcxCustomDataField);
var
  AValueDef: TcxValueDef;
  AData: TcxTreeListValueDefData;
begin
  AValueDef := GetValueDef(AField);
  if AValueDef = nil then Exit;
  AData.Index := AValueDef.ValueDefs.Count;
  if not AField.IsInternal then
    TcxTreeListColumn(AField.Item).InitializeValueDef;
  AData.ValueDef := AValueDef;
...
procedure TcxTreeListDataController.RemoveValueDef(AValueDef: TcxValueDef);
var
  AData: TcxTreeListValueDefData;
begin
  AData.ValueDef := AValueDef;
...
begin
  ...
  ExpandTimer := TcxTimer.Create(nil);
...
var
  ANode, ANextFocusedNode: TcxTreeListNode;
  ANextTopNodePixelScrollOffset: Integer;
begin
  ...
  if TreeList.ViewInfo.IsRecordPixelScrollMode then
    if AGoForward then
      if ASetCursor then
      begin
        if TreeList.ViewInfo.IsScrollAnimationEnabled then
        begin
          TreeList.ViewInfo.UpdatePixelScrollTopNodeAndOffsetMaxValues;
          if FocusedNode.VisibleIndex >= TreeList.ViewInfo.FMaxPixelScrollTopNode.VisibleIndex then
            ANode := TreeList.ViewInfo.FMaxPixelScrollTopNode
          else
            ANode := GetNext(FocusedNode, True);
          ANextFocusedNode := TreeList.ViewInfo.GetLastNodeByTopNode(ANode, 0);
          TreeList.SetFocusedNode(ANextFocusedNode, AShift);
        end
        else
        begin
      ...
      if ASetCursor then
        if TreeList.ViewInfo.IsScrollAnimationEnabled then
        begin
          ANode := GetNext(FocusedNode, False);
          if ANode <> nil then
            TreeList.ViewInfo.GetPixelScrollTopNodeAndOffsetByBottomNode(ANode, ANextFocusedNode, ANextTopNodePixelScrollOffset)
          else
            ANextFocusedNode := FocusedNode;
          TreeList.SetFocusedNode(ANextFocusedNode, AShift);
        end
        else
        begin
  ...
  case AKey of
    ...
    VK_INSERT:
      begin
        ...
        begin
          TreeList.AppendToFocused := ssCtrl in Shift;
          try
            TreeList.InsertNode(FocusedNode, ssCtrl in Shift);
          finally
            TreeList.AppendToFocused := False;
          end;
        end;
...
var
  ...
  AMousePos, AEditorOrigin, ANewEditorOrigin, AScrollOffset: TPoint;
  AEditCellViewInfo: TcxTreeListEditCellViewInfo;
begin
  ...
  AScrollOffset := cxNullPoint;
  AEditorOrigin := cxNullPoint; 
  if Safe.Cast(HitTest.HitTestItem, TcxTreeListEditCellViewInfo, AEditCellViewInfo) then
    AEditorOrigin := AEditCellViewInfo.GetRealCellEditRect.TopLeft;
  AMousePos := cxPoint(X, Y);
  ...
  begin
    if Safe.Cast(HitTest.HitTestItem, TcxTreeListEditCellViewInfo, AEditCellViewInfo) then
    begin
      ANewEditorOrigin := AEditCellViewInfo.GetRealCellEditRect.TopLeft;
      AMousePos := cxPointOffset(AMousePos, ANewEditorOrigin.X - AEditorOrigin.X, ANewEditorOrigin.Y - AEditorOrigin.Y);
    end;
    ShowEdit(Shift, AMousePos.X, AMousePos.Y);
  end;
end;
...
var
  A110B5F442D7B6203160E21385B061403627F233F29736178: TcxTreeListNode;
begin
  if (FocusedNode = nil) and (FocusedNodeIndex >= 0) then
  begin
    A110B5F442D7B6203160E21385B061403627F233F29736178 := FindNearestFocusableNode(FocusedNodeIndex);
    if A110B5F442D7B6203160E21385B061403627F233F29736178 <> nil then
      A110B5F442D7B6203160E21385B061403627F233F29736178.Focused := True;
  end;
end;
...
function TcxTreeListController.CanInsertNode(AIsAppend: Boolean = False): Boolean;
begin
  Result := (not AIsAppend and TreeList.OptionsData.Inserting) or (AIsAppend and TreeList.OptionsData.Appending);
...
begin
  HitTest.Recalculate;
  if IsEditing and (FocusedNode <> nil) and FocusedNode.HasChildren and
    OptionsBehavior.ExpandOnDblClick and not GetAlwaysShowEditor then
      EditingController.HideEdit(True);
  EditingController.StopEditShowingTimer;
  inherited DblClick;
  HitTest.Recalculate;
...
begin
  ...
  try
    ...
    EndUpdateSelection(AKey in [VK_MENU, VK_SHIFT, VK_CONTROL]);
  end;
...
begin
  ...
  begin
    if (ARecordIndex <> 0) and ((EditingController.EditingItem <> AItem) or not TcxTreeListNode(ARecordIndex).Focused) then
    begin
...
begin
  AFocused := Focused;
  Focused := EditingControl.IsFocused;
  if Focused <> AFocused then
    ControlFocusChanged;
  if not Focused then
    HideHint;
end;
...
procedure TcxTreeListController.CheckKeyAutoRepeat(ACharCode: Word; ARepeatCount: Cardinal);
begin
  if IsNavigationKey(ACharCode) and (ARepeatCount > 1) then
  begin
    FIsNavigationKeyAutoRepeat := True;
    if TreeList.ViewInfo.IsNodeScrollAnimationActive then
      TreeList.ViewInfo.DestroyNodeScrollAnimation;
    if TreeList.ViewInfo.IsColumnScrollAnimationActive then
      TreeList.ViewInfo.DestroyColumnScrollAnimation;
  end;
end;

function TcxTreeListController.IsNavigationKey(ACharCode: Word): Boolean;
begin
  Result := ACharCode in [VK_NEXT, VK_PRIOR, VK_UP, VK_DOWN, VK_LEFT, VK_RIGHT];
end;

procedure TcxTreeListController.ResetKeyAutoRepeat(ACharCode: Word);
begin
...
begin
  inherited Create(AOwner);
  FSearchEngineClass := GetSearchEngineClass;
  ...
  FCancelKeyWatcher.Init(VK_ESCAPE);
end;
...
begin
  ...
  FreeAndNil(FAbsoluteVisibleItems);
  inherited Destroy;
end;
...
begin
  if Source is TcxCustomTreeList then
    AssignTreeList(TcxCustomTreeList(Source), True)
  else
    inherited Assign(Source);
end;

procedure TcxCustomTreeList.AssignTreeList(Source: TcxCustomTreeList; ARecreateBandsAndColumns: Boolean);
begin
  BeginUpdate;
  try
    DefaultRowHeight := TcxCustomTreeList(Source).DefaultRowHeight;
    AssignData(Source, ARecreateBandsAndColumns);
    AssignOptions(Source);
    DataController.Assign(Source.DataController);
...
begin
  ...
  BeginUpdate;
  try
    Bands.AssignColumnsWidth;
    for I := 0 to Bands.VisibleItemCount - 1 do
      if Bands.VisibleItems[I].Width <> 0 then
        Bands.VisibleItems[I].FWidth := Bands.VisibleItems[I].DisplayWidth;
  finally
    EndUpdate;
  end;
...
begin
   AArguments := TcxTreeListSearchEngine.TArguments.Create(Self, AData, AStart, nil, AExpandedOnly, AForward, AFilter, AIgnoreStartNode);
   Result := SearchEngineClass.Search(@AArguments);
end;
...
begin
  ATextInfo := TcxTreeListSearchEngine.TSearchTextInfo.Create(AText, AColumn, ACaseSensitive, AMode, ALikeParams);
  try
    AArguments := TcxTreeListSearchEngine.TArguments.Create(Self, ATextInfo, AStartNode, nil, AExpandedOnly, AForward, nil, AIgnoreStartNode);
    AArguments.Match := SearchEngineClass.GetTextMatchFunc(@AArguments);
    Result := SearchEngineClass.Search(@AArguments);
  finally
    ATextInfo.Free;
  end;
...
procedure TcxCustomTreeList.ScrollContent(ADirection: TcxDirection);
begin
  inherited ScrollContent(ADirection);
  UpdateScrollBars;
end;
...
function TcxCustomTreeList.GoToNextScrollbarAnnotation(AKinds: TdxScrollbarAnnotationKinds = dxAllScrollbarAnnotationKinds; AGoForward: Boolean = True; AGoOnCycle: Boolean = False): Boolean;
begin
  Result := (ScrollbarAnnotationHelper <> nil) and
    ScrollbarAnnotationHelper.GoToNext(AKinds, AGoForward, AGoOnCycle);
end;
...
function TcxCustomTreeList.IsScrollbarAnnotationVisible(AKind: Integer): Boolean;
begin
  case AKind of
    ...
    Result := inherited IsScrollbarAnnotationVisible(AKind);
  end;
end;
...
procedure TcxCustomTreeList.PopulateScrollbarAnnotations(AKind: TdxScrollbarAnnotationKind; AList: TdxScrollbarAnnotationRowIndexList);
var
  I: Integer;
begin
  case AKind of
    dxErrorScrollbarAnnotationID:
      for I := 0 to AbsoluteVisibleCount - 1 do
        if AbsoluteVisibleItems[I].HasErrors then
          AList.Add(I);
    dxFocusedRowScrollbarAnnotationID:
      AList.Add(Controller.FocusedNodeIndex);
    dxSelectedRowScrollbarAnnotationID:
      for I := 0 to SelectionCount - 1 do
        AList.Add(Selections[I].VisibleIndex)
  else
    inherited PopulateScrollbarAnnotations(AKind, AList);
  end;
end;

procedure TcxCustomTreeList.SetCurrentScrollbarAnnotationRecordIndex(AIndex: Integer);
begin
  SetFocusedVisibleNode(AbsoluteVisibleItems[AIndex]);
end;
...
begin
  ...
  begin
    ...
    Result.Bottom := TdxTextMeasurer.TextLineHeight(Font);
...
function TcxCustomTreeList.GetCurrentScrollbarAnnotationRecordIndex: Integer;
begin
  Result := Controller.FocusedNodeIndex;
end;
...
procedure TcxCustomTreeList.AfterNodeFiltering;
var
  AOldFocusedNode: TcxTreeListNode;
  ANodeToBeFocused: TcxTreeListNode;
begin
  BeginUpdate;
  try
    AOldFocusedNode := Controller.FocusedNode;
    ViewInfo.IsDirty := True;
    if OptionsSelection.MultiSelect then
    begin
      DataController.ForEachNode(DoUnselectNodeAfterFilteringProc, nil);
      if (AOldFocusedNode <> nil) and AOldFocusedNode.IsHidden then
        Controller.ChangeFocusedNode(Controller.FindNearestFocusableNode(AOldFocusedNode.VisibleIndex));
    end
    else
    begin
      if AOldFocusedNode = nil then
        ANodeToBeFocused := Controller.FindNearestFocusableNode(0)
      else if AOldFocusedNode.HiddenByFilter then
        ANodeToBeFocused := Controller.FindNearestFocusableNode(AOldFocusedNode.VisibleIndex)
      else
        ANodeToBeFocused := nil;
      if ANodeToBeFocused <> nil then
      begin
        FocusedNode := ANodeToBeFocused;
      end;
    end;
  finally
    CancelUpdate;
...
procedure TcxCustomTreeList.AssignData(ASource: TcxCustomTreeList; ARecreateBandsAndColumns: Boolean);
var
  I: Integer;
  AName: string;
  AColumn: TcxTreeListColumn;
begin
  BeginUpdate;
  try
    Bands.AssignBands(ASource.Bands, ARecreateBandsAndColumns);

    if ARecreateBandsAndColumns then
    begin
      while ContainerList.Count > 0 do
        TObject(ContainerList.Last).Free;
    ...
    begin
      if ContainerList.Count <> ASource.ContainerList.Count then
        raise EArgumentException.Create('ASource.ContainerList.Count must be equal to Self.ContainerList.Count');
    end;
...
begin
  Result := dxDefaultBooleanToBoolean(OptionsData.MultiThreadedSorting, dxDefaultMultiThreadedSorting) and
    dxCanUseMultiThreading and IsSortedColumnsSupportMultiThreading;
end;
...
var
  ASaveChanges: TcxTreeListChanges;
  AFindCriteriaChanges: TcxDataFindCriteriaChanges;
  AChangedAnnotationKinds: TdxScrollbarAnnotationKinds;
  ADataChanged, AStructureChanged, AFocusChanged, AFindCriteriaChanged: Boolean;
begin
  if (Changes = []) or ChangesLocked or IsLoading or IsDestroying or ViewInfo.CalculateInProcess then Exit;
  ChangeDelayTimer.Enabled := OptionsBehavior.ChangeDelay > 0;
  BeginUpdate;
  try
    ChangesLocked := True;
    try
      ...
      if (Changes * [tcFilterCriteria, tcSortOrder, tcData, tcStructure, tcFindCriteria] <> []) and
        ViewInfo.IsNodeScrollAnimationActive then
        ViewInfo.DestroyNodeScrollAnimation;
      if ([tcData, tcStructure] * Changes <> []) and (DataController.HasFiltering or DataController.Filtered) then
        CheckStructureAbsoluteItems;
      if tcColumns in Changes then
        Bands.RefreshInformation;
      if tcFindCriteria in Changes then
      begin
        Include(AChangedAnnotationKinds, dxSearchResultScrollbarAnnotationID);
        if FindCriteria.ChangesData then
          Changes := Changes + [tcData, tcFocusedNode];
      end;
      if tcFilterCriteria in Changes then
        Changes := Changes + [tcData, tcFocusedNode];
      if tcSortOrder in Changes then
        Include(FChanges, tcData);
      ADataChanged := tcData in Changes;
      if ADataChanged then
      begin
        DoFilterNodes;
        DoSortNodes;
        AChangedAnnotationKinds := dxAllScrollbarAnnotationKinds;
      end;
      AStructureChanged := (tcStructure in Changes) or (DataController.NodesCount <> FAbsoluteItems.Count);
      if AStructureChanged then
      begin
        CheckStructure;
        AChangedAnnotationKinds := dxAllScrollbarAnnotationKinds;
      end;
      if ADataChanged and DataController.HasFiltering then
        AfterNodeFiltering;
      if SelectionChanges * Changes <> [] then
      begin
        CheckSelectionAndFocused;
        AChangedAnnotationKinds := AChangedAnnotationKinds + [dxSelectedRowScrollbarAnnotationID, dxFocusedRowScrollbarAnnotationID];
      end;
      if tcSummary in Changes then
        Summary.Calculate;
      if tcImages in Changes then
        CheckLevelsInfo;
      AFocusChanged := tcFocusedNode in Changes;
      CheckEvents;
      AFindCriteriaChanged := tcFindCriteria in Changes;
      if Changes = ASaveChanges then
        Changes := ASaveChanges * SelectionChanges
      else
      begin
        Changes := Changes - [tcData..tcColumns, tcSummary..tcFindCriteria];
        if tcLoading in Changes then
          Changes := Changes - [tcStructure, tcSortOrder];
      end;
      FindCriteria.Update(ADataChanged or AStructureChanged, AFocusChanged);
      if AFindCriteriaChanged then
      begin
        AFindCriteriaChanges := FindCriteria.Changes;
        FindCriteria.ResetChanges;
        if Controller.IsFindPanelVisible then
          Controller.FindPanel.CriteriaChanged(AFindCriteriaChanges);
        FindCriteria.DoChanged(AFindCriteriaChanges);
      end
      else
        if (ADataChanged or AStructureChanged or AFocusChanged) and Controller.IsFindPanelVisible then
          Controller.FindPanel.UpdateViewInfo;
      RefreshScrollbarAnnotations(AChangedAnnotationKinds);
      ChangesLocked := False;
    except
      ChangesLocked := False;
      FChanges := [];
    ...
    begin
      CancelUpdate;
      if (ASaveChanges - [tcLoading]) <> [] then
        DoChanged();
    end;
  end;
end;
...
begin
  ...
  if not IsDestroying then
  begin
    if (tcFocusedNode in Changes) and ((Controller.PrevFocusedNode <> FocusedNode) or (FocusedNode = nil)) then
    begin
      if Controller.FocusedNode = nil then
        Controller.CheckFocusedNode;
      if (Controller.PrevFocusedNode <> FocusedNode) or ((FocusedNode = nil) and (Count = 0)) then
...
procedure TcxCustomTreeList.CheckStructureAbsoluteItems;
begin
  FAbsoluteItems.Count := 0;
  FAbsoluteItems.Capacity := Max(FAbsoluteItems.Capacity, DataController.NodesCount);
  DataController.ForEachNode(DoAddAbsoluteItem, nil);
end;
...
procedure TcxCustomTreeList.ColumnExpressionChanged;
begin
  BeginUpdate;
  try
    AddChanges([tcData, tcSummary]);
...
function TcxCustomTreeList.CreateExpressionProvider: TcxDataCustomExpressionProvider;
begin
  Result := TcxDataControllerSpreadSheetExpressionProvider.Create(DataController);
end;
...
function TcxCustomTreeList.CreateScrollbarAnnotations: TcxControlScrollbarAnnotationOptions;
begin
  Result := TcxTreeListScrollbarAnnotationOptions.Create(Self);
end;
...
begin
  ...
  FDelayTimer := TcxTimer.Create(nil);
...
begin
  if TcxTreeListColumn(AItem).StoreID = FNextStoreID - 1 then
    Dec(FNextStoreID);
  if TcxTreeListCellNavigator(Controller.Navigator).FCurrentNavigationColumn = AItem then
    TcxTreeListCellNavigator(Controller.Navigator).FCurrentNavigationColumn := nil;
...
begin
  ...
  begin
    FChanges := FChanges + [tcStructure, tcLayout];
    Exit;
  end;
...
procedure TcxCustomTreeList.DoSetTopVisibleNode(ANode: TcxTreeListNode; ARaiseEvent: Boolean);
begin
...
procedure TcxCustomTreeList.DoUnselectNodeAfterFilteringProc(
  ANode: TcxTreeListNode; Data: Pointer);
begin
  if ANode.HiddenByFilter and ANode.Selected then
    InternalDeselectNode(ANode);
end;
...
begin
  ...
  try
    ...
    AddChanges([tcFilterCriteria]);
  finally
    EndUpdate;
  end;
end;

procedure TcxCustomTreeList.FindCriteriaChanged;
begin
  BeginUpdate;
  try
    AddChanges([tcFindCriteria]);
  finally
    EndUpdate;
  end;
end;
...
function TcxCustomTreeList.GetDataPixelScrollSize: Integer;
begin
  Result := ViewInfo.FCacheVScrollSize;
end;

function TcxCustomTreeList.GetRecordPixelScrollPosition(ARecordIndex: Integer): Integer;
begin
  Result := AbsoluteVisibleItems[ARecordIndex].PixelScrollPosition;
end;

function TcxCustomTreeList.GetBordersScaleFactor: TdxScaleFactor;
begin
  if LookAndFeelPainter.TreeListScaleGridLines then
  begin
    if LookAndFeelPainter.TreeListUseDiscreteScalingForGridLines then
...
function TcxCustomTreeList.GetScrollableRecordCount: Integer;
begin
  Result := AbsoluteVisibleCount;
end;
...
function TcxCustomTreeList.IsRecordIndexBasedScrollbarAnnotationRendering: Boolean;
begin
  Result := not ViewInfo.IsPixelBasedScrollDataPos;
end;
...
function TcxCustomTreeList.GetUseLightBorders: Boolean;
var
...
begin
  ...
  Result := TdxVisualRefinements.LightBorders and (not OptionsView.ExtPaintStyle) and (not ALookAndFeel.NativeStyle) and ((ALookAndFeel.Kind = lfUltraFlat) or Assigned(ALookAndFeel.SkinPainter));
end;
...
class function TcxCustomTreeList.GetSearchEngineClass: TcxTreeListSearchEngineClass;
begin
  Result := TcxTreeListSearchEngine;
end;
...
begin
  ...
  for I := FAbsoluteItems.Count - 1 downto 0 do
    DataController.FreeNodeRecord(TcxTreeListNode(FAbsoluteItems.List[I]));
  for I := 0 to ContainerList.Count - 1 do
...
procedure TcxCustomTreeList.InternalDeselectNode(ANode: TcxTreeListNode);
begin
  if ANode = nil then Exit;
  if SelectionList.Remove(ANode) >= 0 then
    DoSelectionChanged;
end;
...
function TcxCustomTreeList.IsSortedColumnsSupportMultiThreading: Boolean;
var
  I: Integer;
begin
  Result := True;
  ...
  begin
    Result := DataController.IsItemSupportMultiThreading(SortedColumns[I].ItemIndex);
    if not Result then
      Exit;
  end;
end;
...
begin
  ...
  ViewInfo.SetDirty;
  InvalidateScrollbarAnnotations;
  ViewInfo.FDefaultCellHeight := 0;
...
begin
  ...
  begin
    if nsInternalDelete in ANode.State then
      FTopVisibleNode := nil
    else
    begin
...
begin
  ...
  AAnimated := ViewInfo.IsScrollAnimationEnabled and not Controller.EditingController.EditingItemSetting;
  if FTopVisibleNode = nil then
  begin
    FTopVisibleNode := ANode;
    DoTopRecordIndexChanged;
  ...
  if (ANode = FTopVisibleNode) then
   if ViewInfo.FPixelScrollNodeOffset = 0 then
     ...
     ViewInfo.SetTopNodeWithOffset(ANode, 0, AAnimated)
  else
  begin
    ViewInfo.Validate;
    ...
    begin
      if (FTopVisibleNode = nil) or (ANode.VisibleIndex <= FTopVisibleNode.VisibleIndex) then
        ...
        if AAnimated then
          ViewInfo.DoSetLastNode(ANode, True)
        else
...
function TcxCustomTreeList.CanCreateAllItems: Boolean;
begin
  Result := SupportCreateAllItems;
end;
...
begin
  case AButtonIndex of
    ...
    NBDI_APPEND:
      if Controller.CanInsertNode(True) then
      begin
        AppendToFocused := True;
        try
          InsertNode(FocusedNode, True);
        finally
          AppendToFocused := False;
        end;
      end;
    NBDI_DELETE:
...
function TcxCustomTreeList.GetOnFindCriteriaBeforeChange: TcxDataFindCriteriaBeforeChangeEvent;
begin
  Result := DataController.OnFindCriteriaBeforeChange;
end;

function TcxCustomTreeList.GetOnFindCriteriaChanged: TcxDataFindCriteriaChangedEvent;
begin
  Result := DataController.OnFindCriteriaChanged;
end;
...
function TcxCustomTreeList.GetChangesLocked: Boolean;
begin
  Result := FChangesLocked > 0;
end;
...
function TcxCustomTreeList.GetFindCriteria: TcxTreeListDataFindCriteria;
begin
  Result := DataController.FindCriteria;
end;
...
function TcxCustomTreeList.GetGridLineWidth: Integer;
begin
  Result := BordersScaleFactor.Apply(1);
end;
...
begin
  Result := FIsCancelOperation or CancelKeyWatcher.WasPressed;
...
function TcxCustomTreeList.GetScrollbarAnnotations: TcxTreeListScrollbarAnnotationOptions;
begin
  Result := TcxTreeListScrollbarAnnotationOptions(inherited ScrollbarAnnotations);
end;
...
procedure TcxCustomTreeList.SetChangesLocked(const AValue: Boolean);
begin
  if AValue then
    Inc(FChangesLocked)
  else
...
procedure TcxCustomTreeList.SetFilter(AValue: TcxDataFilterCriteria);
begin
  DataController.Filter := AValue;
end;
...
procedure TcxCustomTreeList.SetOnFindCriteriaBeforeChange(Value: TcxDataFindCriteriaBeforeChangeEvent);
begin
  DataController.OnFindCriteriaBeforeChange := Value;
end;

procedure TcxCustomTreeList.SetOnFindCriteriaChanged(Value: TcxDataFindCriteriaChangedEvent);
begin
  DataController.OnFindCriteriaChanged := Value;
end;
...
procedure TcxCustomTreeList.SetScrollbarAnnotations(Value: TcxTreeListScrollbarAnnotationOptions);
begin
  inherited ScrollbarAnnotations := Value;
end;
...
begin
  if ViewInfo.IsNodeScrollAnimationActive then
    ViewInfo.DestroyNodeScrollAnimation;
  DoSetTopVisibleNode(ANode, True);
end;
...
procedure TcxCustomTreeList.DoAddAbsoluteItem(ANode: TcxTreeListNode; AData: Pointer);
begin
  if ANode <> Root then
    ANode.FAbsoluteIndex := FAbsoluteItems.Add(ANode);
end;
...
procedure TcxCustomTreeList.InvalidateScrollbarAnnotations;
begin
 if ScrollbarAnnotationHelper <> nil then
   ScrollbarAnnotationHelper.InvalidateRenderers;
end;
...
procedure TcxCustomTreeList.WMKeyDown(var Message: TWMKeyDown);
begin
  Controller.CheckKeyAutoRepeat(Message.CharCode, Word(Message.KeyData));
  inherited;
end;

procedure TcxCustomTreeList.WMKeyUp(var Message: TWMKeyUp);
begin
...
begin
  Result := Painter.ScaledHeaderHeight(TdxTextMeasurer.TextLineHeight(Font), ScaleFactor);
end;
...
begin
  ...
  if not CellHasOrigin then
    ACanvas.Rectangle(BoundsRect, ViewParams, Borders, BorderColor, TreeList.ViewInfo.GridLineWidth)
  else
  begin
    AWindowOrg := ACanvas.WindowOrg;
    ACanvas.WindowOrg := cxNullPoint;
    ACanvas.Rectangle(cxRectOffset(BoundsRect, GetCellOrigin),
      ViewParams, Borders, BorderColor, TreeList.ViewInfo.GridLineWidth);
...
begin
  ...
  if not Transparent then
    Painter.DrawFooterCellContent(ACanvas, BoundsRect, AlignHorz, AlignVert, MultiLine,
      '', ViewParams.Font, ViewParams.TextColor, ViewParams.Color, DrawBackgroundHandler, cxBordersAll, ScaleFactor, TreeList.BordersScaleFactor);
  DrawText(ACanvas);
  Painter.DrawFooterCellBorder(ACanvas, BoundsRect, cxBordersAll, TreeList.BordersScaleFactor);
  if Selected and Painter.SupportsNativeFocusRect then
  begin
    ACanvas.DrawFocusRect(cxRectInflate(DisplayRect, -1 * TreeList.ViewInfo.GridLineWidth));
    ACanvas.DrawFocusRect(cxRectInflate(DisplayRect, -2 * TreeList.ViewInfo.GridLineWidth));
  end;
end;
...
begin
  ACanvas.Brush.Style := bsClear;
  ACanvas.DrawTexT(FText, GetTextBounds, AlignHorz, AlignVert, MultiLine, ShowEndEllipsis);
...
function TcxTreeListFooterCellViewInfo.GetTextBounds: TRect;
begin
  Result := BoundsRect;
  Result.Deflate(TreeList.ViewInfo.FooterCellTextOffsets);
end;
...
begin
  ...
  begin
    ...
    try
      ...
      FHintRect := R;
      R := cxRectSetHeight(R, TdxTextMeasurer.TextLineHeight(ViewParams.Font) + ScaleFactor.Apply(cxTextOffset) * 2);
      ACanvas.Font := ViewParams.Font;
      ACanvas.Brush.Style := bsClear;
      AWidth := 0;
      ACountInLine := 0;
...
begin
  FTextBounds := cxRectInflate(BoundsRect, -1 * TreeList.ViewInfo.GridLineWidth);
  FTextBounds.Deflate(Painter.HeaderContentOffsets(ScaleFactor));
  inherited DoCalculate;
end;
...
begin
  ...
  begin
    Painter.DrawScaledHeader(ACanvas, R, TextBounds, Neighbors, Borders,
      State, GetRealAlignHorz, GetAlignVert, GetMultiline, GetShowEndEllipsis, GetText, ACanvas.Font,
      ViewParams.TextColor, ViewParams.Color, BordersScaleFactor, DrawBackgroundHandler, IsLast, False, BordersScaleFactor.Apply(1));
  end;
end;
...
begin
  ...
  begin
    if ExtPaintStyle then
      Painter.DrawScaledIndicatorItemEx(ACanvas, DisplayRect, Kind, ViewParams.Color, ScaleFactor, IsRightToLeftConverted, DrawBackgroundHandler)
    else
      Painter.DrawScaledIndicatorItem(ACanvas, DisplayRect, Kind, ViewParams.Color, BordersScaleFactor, IsRightToLeftConverted, DrawBackgroundHandler, BordersScaleFactor.Apply(1));

    ACanvas.ExcludeClipRect(ClipRect);
  end
  else
    if Position = tlipFooter then
      Painter.DrawFooterPanel(ACanvas, DisplayRect, ViewParams, Borders, ScaleFactor, TreeList.BordersScaleFactor)
    else
    begin
...
begin
  ...
  if Selected and Painter.SupportsNativeFocusRect then
  begin
    ACanvas.DrawFocusRect(cxRectInflate(cxRectInflate(DisplayRect, BordersMargins), -1 * TreeList.ViewInfo.GridLineWidth));
    ACanvas.DrawFocusRect(cxRectInflate(cxRectInflate(DisplayRect, BordersMargins), -2 * TreeList.ViewInfo.GridLineWidth));
  end;
end;

procedure TcxTreeListHeaderCellViewInfo.DoDrawGlyph(ACanvas: TcxCanvas);

  procedure DoDrawGlyphCore(ACanvas: TcxCanvas);
  begin
    Glyph.StretchDraw(ACanvas.Handle, cxRectSetOrigin(cxRect(GetGlyphSize), FGlyphPosition), MaxByte, GlyphColorPalette);
  end;
...
function TcxTreeListHeaderCellViewInfo.GetGlyphColorPalette: IdxColorPalette;
begin
  Result := Painter.HeaderGlyphColorPalette(State);
end;
...
procedure TcxTreeListBandHeaderCellViewInfo.InitializeWithBorders(
  ABand: TcxTreeListBand; const ATreeListBounds, ABandRect: TRect);
begin
  Initialize(ABand.Caption);

  Include(FBorders, bRight);
  if TreeList.UseLightBorders then
  begin
    if not Assigned(ABand.ParentBand) then
      Exclude(FBorders, bTop);
    if (TreeList.OptionsView.FixedSeparatorWidth > 0) and ABand.IsLastLeftFixed then
      Exclude(FBorders, bRight);
    if (TreeList.OptionsView.FixedSeparatorWidth > 0) and ABand.IsFirstRightFixed then
      Exclude(FBorders, bLeft);
    if ABand.IsLeftMostVisible then
      Exclude(FBorders, bLeft);
    if ABand.IsRightMostVisible then
      if (TreeList.OptionsView.ColumnAutoWidth or (ABandRect.Right >= ATreeListBounds.Right)) and (not TreeList.ViewInfo.ForceIncludeRightmostBorder) then
        Exclude(FBorders, bRight);
    if ABand.IsNextToLeftFixedBands then
      Exclude(FBorders, bLeft);
    if ABand.IsPreviousToRightFixedBands then
      Exclude(FBorders, bRight);
  end;
end;
...
begin
  if CellTransparent then
    ACanvas.FrameRect(DisplayRect, BorderColor, TreeList.ViewInfo.GridLineWidth, Borders)
  else
  begin
    AWindowOrg := ACanvas.WindowOrg;
    ACanvas.WindowOrg := cxNullPoint;
  ...
  if Focused and OptionsView.FocusRect and (Node.ViewData <> nil) and not Node.ViewData.DrawFocusRect and
    (not TreeList.OptionsSelection.HideFocusRect or TreeList.IsFocused) and EditViewInfo.Painter.SupportsNativeFocusRect
  then
    ACanvas.DrawFocusRect(cxExcludeBorders(DisplayRect, Borders, TreeList.ViewInfo.GridLineWidth));
end;
...
function TcxTreeListEditCellViewInfo.GetCellBorderWidth: Integer;
begin
  Result := TreeList.ViewInfo.GridLineWidth;
end;
...
begin
  Result := GetRealCellEditRect;
  TreeList.Controller.EditingController.CheckMultilineEditBounds(Result);
  cxRectIntersect(Result, Result, TreeList.ViewInfo.ContentBounds);
end;
...
procedure TcxTreeListEditCellViewInfo.GetFindTextInfo(out AStart: Integer; out ALength: Integer);
var
  AText: string;
begin
  AStart := TreeList.FindCriteria.GetTextStartPositionByNode(Node, EditContainer.ItemIndex, AText);
...
function TcxTreeListEditCellViewInfo.GetRealCellEditRect: TRect;
begin
  Result := CellEditRect;
...
begin
  ...
  if (Node.ViewData.PreviewHeight > 0) and (TreeList.Preview.Place = tlppTop) then
    DY := TreeList.ViewInfo.GridLineWidth;
  X2 := ARect.Right;
  if IsRightToLeftConverted then
    X2 := ARect.Left;
  if ilHorz in Lines then
    FHorzTreeLine := cxRect(ACenter.X, ACenter.Y - TreeList.ViewInfo.GridLineWidth + DY, X2, ACenter.Y + DY);
  if [ilVertUp, ilVertDown] * Lines <> [] then
  begin
    FVertTreeLine := cxRect(ACenter.X, ARect.Top, ACenter.X + TreeList.ViewInfo.GridLineWidth, ARect.Bottom);
    if not (ilVertUp in Lines) then
      FVertTreeLine.Top := ACenter.Y - TreeList.ViewInfo.GridLineWidth;
    if not (ilVertDown in Lines) then
      FVertTreeLine.Bottom := ACenter.Y - TreeList.ViewInfo.GridLineWidth;
...
begin
  ...
  begin
    Images.Draw(ACanvas.Canvas, GlyphRect.Left, GlyphRect.Top, ImageIndex, dsTransparent, Images.ImageType, True);
    Images.Draw(ACanvas.Canvas, GlyphRect.Left, GlyphRect.Top, OverlayIndex, dsTransparent, Images.ImageType, True);
  end
  else
  begin
    AColorPalette := Painter.DefaultContentGlyphColorPalette(cxbsNormal);
    TdxImageDrawer.DrawImage(ACanvas, GlyphRect, nil, Images, ImageIndex, True, AColorPalette, ScaleFactor);
    TdxImageDrawer.DrawImage(ACanvas, GlyphRect, nil, Images, OverlayIndex, True, AColorPalette, ScaleFactor);
  end;
end;
...
function TcxTreeListFindPanelViewInfo.GetBordersScaleFactor: TdxScaleFactor;
begin
  Result := TcxCustomTreeList(Control).BordersScaleFactor;
end;
...
function TcxTreeListFilterBoxViewInfo.GetBordersScaleFactor: TdxScaleFactor;
begin
  Result := TcxCustomTreeList(Control).BordersScaleFactor;
end;
...
begin
  Result := Byte(GridLines in [tlglVert, tlglBoth]) * TreeList.ViewInfo.GridLineWidth;
end;
...
begin
  Result := Byte(GridLines in [tlglHorz, tlglBoth]) * TreeList.ViewInfo.GridLineWidth;
end;
...
function TcxTreeListBand.IsNextToLeftFixedBands: Boolean;
begin
  if Visible and (FixedKind <> tlbfLeft) then
  begin
    if not Assigned(ParentBand) then
    begin
      if VisibleRootIndex > 0 then
        Result := Bands.VisibleRootItems[VisibleRootIndex - 1].FixedKind = tlbfLeft
      else
        Result := False;
    end
    else
      Result := (ParentBand.ChildVisibleBands[0] = Self) and ParentBand.IsNextToLeftFixedBands;
  end
  else
    Result := False;
end;

function TcxTreeListBand.IsPreviousToRightFixedBands: Boolean;
begin
  if Visible and (FixedKind <> tlbfRight) then
  begin
    if not Assigned(ParentBand) then
    begin
      if VisibleRootIndex < Bands.VisibleRootItemCount - 1 then
        Result := Bands.VisibleRootItems[VisibleRootIndex + 1].FixedKind = tlbfRight
      else
        Result := False;
    end
    else
      Result := (ParentBand.ChildVisibleBands[ParentBand.ChildVisibleBands.Count - 1] = Self) and ParentBand.IsPreviousToRightFixedBands;
  end
  else
    Result := False;
end;
...
function TcxTreeListBand.IsLeftMostVisible: Boolean;
begin
  if Visible then
  begin
    if not Assigned(ParentBand) then
      Result := VisibleRootIndex = 0
    else
      Result := (ParentBand.ChildVisibleBands[0] = Self) and ParentBand.IsLeftMostVisible;
  end
  else
    Result := False;
end;

function TcxTreeListBand.IsFirstRightFixed: Boolean;
begin
  if Visible and (FixedKind = tlbfRight) then
  begin
    if not Assigned(ParentBand) then
    begin
      ...
      Result := (ParentBand.ChildVisibleBands[0] = Self) and ParentBand.IsFirstRightFixed;
  end
  else
    Result := False;
end;

function TcxTreeListBand.IsRightMostVisible: Boolean;
begin
  if Visible then
  begin
    if not Assigned(ParentBand) then
      Result := VisibleRootIndex = Bands.VisibleRootItemCount - 1
    else
      Result := (ParentBand.ChildVisibleBands[ParentBand.ChildVisibleBands.Count - 1] = Self) and ParentBand.IsRightMostVisible;
  end
  else
    Result := False;
end;

function TcxTreeListBand.IsLastLeftFixed: Boolean;
begin
  if Visible and (FixedKind = tlbfLeft) then
  begin
    if not Assigned(ParentBand) then
    begin
      ...
      Result := (ParentBand.ChildVisibleBands[ParentBand.ChildVisibleBands.Count - 1] = Self) and ParentBand.IsLastLeftFixed;
  end
  else
    Result := False;
end;
...
begin
  if ASource is TcxTreeListBands then
    AssignBands(TcxTreeListBands(ASource), True)
  else
    inherited Assign(ASource);
end;

procedure TcxTreeListBands.AssignBands(ASource: TcxTreeListBands;
  ARecreate: Boolean);
var
  I: Integer;
begin
  ...
  try
    ...
    begin
      if Count <> ASource.Count then
        raise EArgumentException.Create('ASource.Count must be equal to Self.Count');
    end;

    for I := 0 to ASource.Count - 1 do
      Items[I].Assign(ASource[I]);
...
procedure TcxTreeListFiltering.RunCustomizeDialog(AColumn: TcxTreeListColumn = nil);
begin
  inherited RunCustomizeDialog(AColumn);
end;
...
begin
  FChangeDelay := Max(0, AValue);
  if FChangeDelay > 0 then
    TreeList.ChangeDelayTimer.Interval := FChangeDelay
  else
    TreeList.ChangeDelayTimer.Enabled := False;
end;
...
function TcxTreeListFindPanel.GetItemCaption(AItem: TcxFindPanelItemKind): string;
begin
  case AItem of
    fpikFindButton:
    ...
    fpikNextButton:
      Result := cxGetResourceString(@scxTreeListFindPanelNextButtonCaption);
    fpikPreviousButton:
      Result := cxGetResourceString(@scxTreeListFindPanelPreviousButtonCaption);
    else
      Result := inherited GetItemCaption(AItem);
  end;
end;

procedure TcxTreeListFindPanel.FocusControlContent;
var
  AFocusedNode: TcxTreeListNode;
  ANode: TcxTreeListNode;
begin
  AFocusedNode := TreeList.FocusedNode;
  if Assigned(AFocusedNode) and (AFocusedNode.VisibleIndex = -1) then
  begin
    AFocusedNode.Focused := False;
    AFocusedNode := nil;
  end;
  if (AFocusedNode = nil)
      or (AFocusedNode.VisibleIndex < TreeList.TopVisibleNode.VisibleIndex)
      or (Assigned(TreeList.LastVisibleNode) and (AFocusedNode.VisibleIndex > TreeList.LastVisibleNode.VisibleIndex)) then
  begin
    ANode := TreeList.TopVisibleNode;
    if ANode <> nil then
    begin
      ANode.Focused := True;
      TreeList.MakeNodeVisible(ANode);
    end;
  end;
...
function TcxTreeListFindPanel.GetTreeList: TcxCustomTreeList;
begin
  Result := TcxCustomTreeList(EditingControl);
end;
...
begin
  ...
  if ((DestBand = nil) or DropInfo.Customizing) and CanRemove then
    ...
    if (DestBand <> nil) and (TreeList.OptionsCustomizing.StackedColumns or
      not DropInfo.InsertRow or (DestBand.BandRows.VisibleItemCount = 0))  then
    begin
...
procedure TcxTreeListCellNavigator.AppendRecord(
  var ARowIndex, ACellIndex: Integer);
begin
  if not TreeList.OptionsData.Appending or (TreeList.IsInserting and not TreeList.DataController.IsValueChanged) then
    Exit;
  ...
  try
    ...
    if TcxTreeListController(Controller).CanInsertNode(True) and TreeList.InsertNode(nil, True) then
    begin
...
function TcxTreeListDataFindCriteria.GetTextStartPositionByNode(ANode: TcxTreeListNode; AItemIndex: Integer;
...
begin
  ...
  ARecordIndex := DataController.GetFilterRecordIndexByNode(ANode);
  if not IsActive or (Items.IndexOf(AField) = -1) or not DoFilterRecord(ARecordIndex) then
    Exit;
  ADisplayText := ANode.Texts[AItemIndex];
  Result := GetTextStartPosition(ADisplayText, AHighlightedText);
end;

procedure TcxTreeListDataFindCriteria.DoBeforeChange(const AChange: TcxDataFindCriteriaChange);
begin
  DataController.CheckBrowseMode;
  inherited DoBeforeChange(AChange);
end;

function TcxTreeListDataFindCriteria.GetFocusedIndex: Integer;
begin
  Result := TreeList.Controller.FocusedNodeIndex;
end;

function TcxTreeListDataFindCriteria.GetMatchCount: Integer;
begin
  if not UseMatches then
    Result := TreeList.AbsoluteVisibleCount
  else
    Result := inherited GetMatchCount;
end;

function TcxTreeListDataFindCriteria.GoToMatch(AMatchIndex: Integer): Boolean;
var
  ANode: TcxTreeListNode;
begin
  ANode := MatchNodes[AMatchIndex];
  Result := ANode <> nil;
  if Result then
  begin
    TreeList.SetFocusedVisibleNode(ANode);
    Result := ANode.Focused;
  end;
end;

procedure TcxTreeListDataFindCriteria.PopulateMatches;
var
  ANode: TcxTreeListNode;
  AVisibleIndex, ARecordIndex: Integer;
begin
  for AVisibleIndex := 0 to TreeList.AbsoluteVisibleCount - 1 do
  begin
    ANode := TreeList.AbsoluteVisibleItems[AVisibleIndex];
    ARecordIndex := DataController.GetFilterRecordIndexByNode(ANode);
    if ANode.IsVisible and DoFilterRecord(ARecordIndex) then
      Matches.Add(AVisibleIndex);
  end;
end;

function TcxTreeListDataFindCriteria.UseMatches: Boolean;
begin
  Result := True;
end;

function TcxTreeListDataFindCriteria.GetDataController: TcxTreeListDataController;
begin
  Result := TcxTreeListDataController(inherited DataController);
end;

function TcxTreeListDataFindCriteria.GetMatchNode(AMatchIndex: Integer): TcxTreeListNode;
var
  AVisibleIndex: Integer;
begin
  AVisibleIndex := NodeVisibleIndexes[AMatchIndex];
...
function TcxTreeListDataFindCriteria.GetMatchNodeVisibleIndex(AMatchIndex: Integer): Integer;
begin
  Result := MatchRowIndexes[AMatchIndex];
end;

function TcxTreeListDataFindCriteria.GetTreeList: TcxCustomTreeList;
begin
...
var
  ANode: TcxTreeListNode;
begin
  TreeList.CancelKeyWatcher.Start;
  try
    ...
    TreeList.CancelKeyWatcher.Finish;
  end;
end;
...
begin
  Result := TreeList.CanUseMultiThreadedSorting;
end;
...
function TcxTreeListNode.HasErrors: Boolean;
var
  I: Integer;
...
begin
  Result := False;
  AData := TcxEditValidateInfo.Create;
  try
    for I := 0 to TreeList.ColumnCount - 1 do
    begin
      AValue := TreeList.Columns[I].DoOnGetDisplayText(Self, False, True);
      TreeList.Columns[I].DoValidateDrawValue(Self, AValue, AData);
      if AData.ErrorType <> eetNone then
      begin
        Result := True;
        Break;
      end;
...
begin
  if tcStructure in TreeList.Changes then
    TreeList.CheckStructureAbsoluteItems;
...
function TcxTreeListNode.GetErrorCode(AIndex: Integer): Integer;
begin
  Result := TreeList.DataController.GetNodeErrorCode(Self, AIndex);
end;
...
procedure TcxTreeListEditingController.EditKeyPress(Sender: TObject; var Key: Char);
begin
  if Assigned(TreeList.OnKeyPress) then
    TreeList.OnKeyPress(TreeList, Key);
  if Key = #27 then
    Key := #0;
  if Key <> #0 then
...
begin
  if FIsEditAutoHeight and (AutoHeight = eahEditor) then
    InflateRect(ABounds, TreeList.GridLineWidth, TreeList.GridLineWidth);
end;
...
function TcxTLPopupMenuImagesManager.GetImages(const AResourceName: string): TCustomImageList;
  ...
  begin
    ...
    begin
      AResourceStream := TResourceStream.Create(HInstance, AResourceName, RT_RCDATA);
      try
        Result.AddImagesFromZipStream(AResourceStream)
      finally
...
function TcxTreeListFooterPopupMenu.IsItemEnabled(AKind: TcxSummaryKind): Boolean;
  ...
  begin
    ...
    begin
      ...
      Result := ((AVarType in ANumberVarTypes) {$IFNDEF NONDB}or (AVarType = TcxFMTBcdValueType.GetVarType){$ENDIF}) and
...
begin
  if not ACanvas.RectVisible(cxRectInflate(GetRealBounds, ViewInfo.GridLineWidth)) and (GetRealBounds.Top <> 39) then
    Exit;
  ...
  try
    ...
    begin
      ...
      if ViewInfo.Painter.SupportsNativeFocusRect then
...
begin
  ...
  R := GetRealBounds;
  R.Left := 0;
  Node.TreeList.InvalidateRect(cxRectInflate(R, ViewInfo.GridLineWidth), False);
end;

{ TcxTreeListColumnScrollAnimationHelper }

constructor TcxTreeListColumnScrollAnimationHelper.Create(
  AOwner: TcxTreeListViewInfo);
begin
  inherited Create;
  FOwner := AOwner;
end;

procedure TcxTreeListColumnScrollAnimationHelper.AfterAnimationEnded(AIsRaiseEvent: Boolean);
begin
  inherited AfterAnimationEnded(AIsRaiseEvent);
  FOwner.ScrollAnimationEnded;
end;

procedure TcxTreeListColumnScrollAnimationHelper.BeforeAnimationStarted;
begin
  FOwner.ScrollAnimationStarted;
  inherited BeforeAnimationStarted;
end;

function TcxTreeListColumnScrollAnimationHelper.GetOwnerPosition: Integer;
begin
  Result := Owner.HScrollPos;
end;

procedure TcxTreeListColumnScrollAnimationHelper.PositionChanged;
begin
  Owner.TreeList.DoLeftPosChanged;
end;

procedure TcxTreeListColumnScrollAnimationHelper.UpdateOwnerPosition(AValue: Integer);
begin
  Owner.DoSetHScrollPos(AValue);
  Owner.TreeList.UpdateScrollBars;
end;

{ TcxTreeListNodePixelScrollData }

constructor TcxTreeListNodePixelScrollData.Create(ANode: TcxTreeListNode;
  AOffset: Integer);
begin
  Node := ANode;
...
{ TcxTreeListNodeScrollAnimationHelper }

function TcxTreeListNodeScrollAnimationHelper.CalculateScrollDelta(
  AValue: TcxTreeListNodePixelScrollData): Integer;
var
  AFirstNode, ALastNode: TcxTreeListNode;
begin
  AFirstNode := GetOwnerPosition.Node;
  ALastNode := AValue.Node;
  FIsScrollingByRecord := Abs(ALastNode.VisibleIndex - AFirstNode.VisibleIndex) > FOwner.GetPixelScrollContentSize;
  if FIsScrollingByRecord then
    Result := ALastNode.VisibleIndex - AFirstNode.VisibleIndex
  else
    Result := FOwner.GetPixelScrollOffset(GetOwnerPosition.Node, GetOwnerPosition.ScrollOffset,
      AValue.Node, AValue.ScrollOffset);
end;

constructor TcxTreeListNodeScrollAnimationHelper.Create(
  AOwner: TcxTreeListViewInfo);
begin
  inherited Create;
  FOwner := AOwner;
...
procedure TcxTreeListNodeScrollAnimationHelper.AfterAnimationEnded(AIsRaiseEvent: Boolean);
begin
  if AIsRaiseEvent and (StartPosition.Node <> GetOwnerPosition.Node) then
    FOwner.TreeList.DoTopRecordIndexChanged;
  FOwner.ScrollAnimationEnded;
end;

function TcxTreeListNodeScrollAnimationHelper.CalculateAnimatedPosition(var APosition: Integer): TcxTreeListNodePixelScrollData;
var
  AOverPan: Boolean;
  ADelta: Integer;
begin
  ...
  if FIsScrollingByRecord then
  begin
    Result.Node := FOwner.TreeList.AbsoluteVisibleItems[Result.Node.VisibleIndex + ADelta];
    if TargetPosition.Node = Result.Node then
      Result.ScrollOffset := TargetPosition.ScrollOffset
    else
      Result.ScrollOffset := 0;
  end
  else
    FOwner.CalculatePixelScrollInfo(Result.Node, Result.ScrollOffset,
      nil, -1, -ADelta, AOverPan);
end;

function TcxTreeListNodeScrollAnimationHelper.GetOwnerPosition: TcxTreeListNodePixelScrollData;
begin
  Result.Node := FOwner.TopNode;
  Result.ScrollOffset := FOwner.PixelScrollNodeOffset;
end;

function TcxTreeListNodeScrollAnimationHelper.GetUnassignedTargetPosition: TcxTreeListNodePixelScrollData;
begin
  Result.Node := nil;
  Result.ScrollOffset := MaxInt;
end;

function TcxTreeListNodeScrollAnimationHelper.IsEqual(AValue1, AValue2: TcxTreeListNodePixelScrollData): Boolean;
begin
  Result := (AValue1.Node = AValue2.Node) and (AValue1.ScrollOffset = AValue2.ScrollOffset);
end;

procedure TcxTreeListNodeScrollAnimationHelper.BeforeAnimationStarted;
begin
  FOwner.ScrollAnimationStarted;
  inherited BeforeAnimationStarted;
end;

procedure TcxTreeListNodeScrollAnimationHelper.UpdateOwnerPosition(AValue: TcxTreeListNodePixelScrollData);
begin
  FOwner.DoSetTopNodeWithOffset(AValue.Node, AValue.ScrollOffset, False);
  FOwner.TreeList.Repaint;
end;
...
procedure TcxTreeList.AssignData(ASource: TcxCustomTreeList; ARecreateBandsAndColumns: Boolean);
var
...
class function TcxTreeListSearchEngine.GetTextMatchFunc(AArguments: TcxTreeListSearchEngine.PArguments): TcxTreeListSearchEngine.TMatchFunc;
begin
  Result := TMatcher.Match;
end;

class function TcxTreeListSearchEngine.Search(AArguments: TcxTreeListSearchEngine.PArguments): TcxTreeListNode;
begin
  if AArguments.ExpandedOnly then
    Result := SearchAmongExpandedNodes(AArguments)
  else
    if (AArguments.StartNode = nil) and (AArguments.FinishNode = nil) then
      Result := CachedSearch(AArguments)
    else
      Result := FullSearch(AArguments);
end;

class function TcxTreeListSearchEngine.SearchAtLevel(AArguments: TcxTreeListSearchEngine.PArguments): TcxTreeListNode;
begin
  Result := Execute(TSiblingSearchStrategy, AArguments);
end;

class function TcxTreeListSearchEngine.SearchAmongExpandedNodes(AArguments: TcxTreeListSearchEngine.PArguments): TcxTreeListNode;
begin
  Result := Execute(TExpandedNodesSearchStrategy, AArguments);
end;

class function TcxTreeListSearchEngine.FullSearch(AArguments: PArguments): TcxTreeListNode;
begin
  Result := Execute(TTreeSearchStrategy, AArguments);
end;

class function TcxTreeListSearchEngine.HybridSearch(AArguments: PArguments): TcxTreeListNode;
begin
  Result := Execute(THybridSearchStrategy, AArguments);
end;

class function TcxTreeListSearchEngine.CachedSearch(AArguments: PArguments): TcxTreeListNode;
begin
  Result := Execute(TCachedSearchStrategy, AArguments);
end;

class procedure TcxTreeListSearchEngine.Lock(ATreeList: TcxCustomTreeList);
begin
  ATreeList.BeginUpdate;
end;

class procedure TcxTreeListSearchEngine.Unlock(ATreeList: TcxCustomTreeList);
begin
...
class function TcxTreeListSearchEngine.Execute(AClass: TSearchStrategyClass; AArguments: PArguments): TcxTreeListNode;
var
  AStrategy: TcxTreeListSearchEngine.TCustomSearchStrategy;
begin
  Lock(AArguments.TreeList);
  try
    AStrategy := AClass.Create(AArguments);
    try
      Result := AStrategy.Execute;
    finally
      AStrategy.Free;
    end;
  finally
    Unlock(AArguments.TreeList);
  end;
end;

{ TcxTreeListSearchEngine.TArguments }

constructor TcxTreeListSearchEngine.TArguments.Create(ATreeList: TcxCustomTreeList; AData: Pointer;
  AStartNode, AFinishNode: TcxTreeListNode; AExpandedOnly, AForward: Boolean;
  AMatch: TcxTreeListFindFunc; AIgnoreStartNode: Boolean);
begin
  TreeList := ATreeList;
  Data := AData;
  Match := AMatch;
  StartNode := AStartNode;
  FinishNode := AFinishNode;
  ExpandedOnly := AExpandedOnly;
  Forward := AForward;
  IgnoreStartNode := AIgnoreStartNode;
end;

function TcxTreeListSearchEngine.TArguments.IsDefault: Boolean;
begin
  Result := not ExpandedOnly and (StartNode = nil) and (FinishNode = nil);
end;

{ TcxTreeListSearchEngine.TSearchTextInfo }

constructor TcxTreeListSearchEngine.TSearchTextInfo.Create(AText: string;
  AColumn: TcxTreeListColumn; ACaseSensitive: Boolean;
  AMode: TcxTreeListFindMode; ALikeParams: TcxTreeListLikeParams);
begin
  inherited Create;
  Column := AColumn;
  CaseSensitive := ACaseSensitive;
  Mode := AMode;
  LikeParams := ALikeParams;
  if not CaseSensitive then
    Text := AnsiUpperCase(AText)
  else
    Text := AText;
  if Column = nil then
    Buffer := TdxStringBuilderManager.Get;
end;

destructor TcxTreeListSearchEngine.TSearchTextInfo.Destroy;
begin
  if Buffer <> nil then
    TdxStringBuilderManager.Release(Buffer);
  inherited Destroy;
end;

function TcxTreeListSearchEngine.TSearchTextInfo.Match(ANodeText: string): Boolean;
var
  ...
  case Mode of
    tlfmLike:
      begin
        ...
        Result := LikeStr(ANodeText, Text, APercent, AUnderLine);
      end;
    tlfmExact:
      begin
...
{ TcxTreeListSearchEngine.TMatcher }

class function TcxTreeListSearchEngine.TMatcher.Match(ANode: TcxTreeListNode; AData: Pointer): Boolean;
var
  I: Integer;
  ANodeText: string;
  ATextInfo: TSearchTextInfo;
begin
  ATextInfo := TSearchTextInfo(AData);
  if ATextInfo.Column <> nil then
    ANodeText := ANode.Texts[ATextInfo.Column.ItemIndex]
  else
  begin
    ...
    for I := 0 to ANode.ValueCount - 1 do
      ATextInfo.Buffer.Append(ANode.Texts[I]);
...
{ TcxTreeListSearchEngine.TCustomSearchStrategy }

constructor TcxTreeListSearchEngine.TCustomSearchStrategy.Create(AArguments: PArguments);
begin
  inherited Create;
  FArguments := AArguments;
end;

{ TcxTreeListSearchEngine.TTreeSearchStrategy }

function TcxTreeListSearchEngine.TTreeSearchStrategy.GoToNext(ANode: TcxTreeListNode): TcxTreeListNode;
begin
  if Arguments.Forward then
    Result := FindNext(ANode)
  else
    Result := FindPrev(ANode);
end;

function TcxTreeListSearchEngine.TTreeSearchStrategy.FindNext(ANode: TcxTreeListNode): TcxTreeListNode;
begin
  Result := ANode;
  if not LoadChildren(Result, Arguments.ExpandedOnly) then
    while Result <> nil do
    begin
...
function TcxTreeListSearchEngine.TTreeSearchStrategy.FindPrev(ANode: TcxTreeListNode): TcxTreeListNode;
begin
  Result := ANode;
  if Result.FPrev <> nil then
  begin
    Result := Result.FPrev;
    while LoadChildren(Result, Arguments.ExpandedOnly) do
      Result := Result.FLast;
  end
  else
  begin
    Result := Result.Parent;
    if Result = Arguments.TreeList.Root then
      Result := nil;
  end;
end;

function TcxTreeListSearchEngine.TTreeSearchStrategy.LoadChildren(ANode: TcxTreeListNode; AExpandedOnly: Boolean): Boolean;
begin
  if not AExpandedOnly and (ANode.Count = 0) and not ANode.Expanded and ANode.HasChildren then
    ANode.LoadChildren;
  Result := (ANode.Count <> 0) and (ANode.Expanded or not AExpandedOnly);
end;

function TcxTreeListSearchEngine.TTreeSearchStrategy.IsFinishNode(ANode: TcxTreeListNode): Boolean;
begin
  Result := Arguments.FinishNode = ANode;
end;

function TcxTreeListSearchEngine.TTreeSearchStrategy.Execute: TcxTreeListNode;

  function GetLastNode: TcxTreeListNode;
  begin
    Result := Arguments.TreeList.Root.FLast;
    if Result = nil then
      Exit;
    while Result <> nil do
    begin
      if LoadChildren(Result, False) and (Result.FLast <> nil) then
         Result := Result.FLast
      else
      begin
        if Result.FLast <> nil then
          Result := Result.FLast;
        Break;
      end;
    end;
...
var
  ACurrent, AStartNode: TcxTreeListNode;
  AMatch: Boolean;
begin
  Result := nil;
  AStartNode := Arguments.StartNode;
  ACurrent := AStartNode;
  if ACurrent = nil then
  begin
    if Arguments.Forward then
      ACurrent := Arguments.TreeList.Root.FFirst
    else
      ACurrent := GetLastNode;
  end;
  while ACurrent <> nil do
  begin
    AMatch := Arguments.Match(ACurrent, Arguments.Data);
    if AMatch and not (Arguments.IgnoreStartNode and (ACurrent = AStartNode)) then
    begin   
      Result := ACurrent;
      Break;
    end;
    if IsFinishNode(ACurrent) then
      Exit(nil);
    if AStartNode <> nil then
      AStartNode := nil;
    ACurrent := GoToNext(ACurrent);
  end;
end;

{ TcxTreeListSearchEngine.TExpandedNodesSearchStrategy }

function TcxTreeListSearchEngine.TExpandedNodesSearchStrategy.LoadChildren(ANode: TcxTreeListNode; AExpandedOnly: Boolean): Boolean;
begin
  Result := (ANode.FCount <> 0) and ANode.Expanded;
end;

{ TcxTreeListSearchEngine.THybridSearchStrategy }

function TcxTreeListSearchEngine.THybridSearchStrategy.Execute: TcxTreeListNode;
begin
  Result := TcxTreeListSearchEngine.Execute(TExpandedNodesSearchStrategy, Arguments);
  if (Result = nil) and not Arguments.ExpandedOnly then
    Result := TcxTreeListSearchEngine.Execute(TTreeSearchStrategy, Arguments);
end;

{ TcxTreeListSearchEngine.TSiblingSearchStrategy }

function TcxTreeListSearchEngine.TSiblingSearchStrategy.Execute: TcxTreeListNode;
begin
  Assert(Arguments.StartNode <> nil);
  LoadChildren(Arguments.StartNode, False);
  Result:= Arguments.StartNode.FFirst;
  while Result <> nil do
    if Arguments.Match(Result, Arguments.Data) then
      Exit
    else
      Result := Result.FNext;
end;
...
{ TcxTreeListSearchEngine.TCachedSearchStrategy }

constructor TcxTreeListSearchEngine.TCachedSearchStrategy.Create(
  AArguments: PArguments);
begin
  inherited Create(AArguments);
  FExpandedNodes := TList.Create;
  FExpandedNodes.Capacity := AArguments.TreeList.DataController.NodesCount div 4;
end;

destructor TcxTreeListSearchEngine.TCachedSearchStrategy.Destroy;
begin
  FreeAndNil(FExpandedNodes);
  inherited Destroy;
end;

function TcxTreeListSearchEngine.TCachedSearchStrategy.Execute: TcxTreeListNode;

   function SearchAmongExpandedNodes: TcxTreeListNode;
   var
    AStrategy: TExpandedNodeBasedStrategy;
   begin
     AStrategy := TExpandedNodeBasedStrategy.Create(Arguments);
     try
       AStrategy.Nodes := FExpandedNodes;
       try
         Result := AStrategy.Execute;
       finally
         AStrategy.Nodes := nil;
   ...
   function SearchAmongCollapsedNodes: TcxTreeListNode;
   var
     I: Integer;
     ANode: TcxTreeListNode;
     ASavedStartNode: TcxTreeListNode;
     ASavedIgnoreStartNode: Boolean;
   begin
     if FExpandedNodes.Count <= 0 then
       Exit(nil);
     Result := nil;
     ASavedIgnoreStartNode := Arguments.IgnoreStartNode;
     ASavedStartNode := Arguments.StartNode;
     Arguments.IgnoreStartNode := False;
     try       
       for I := 0 to FExpandedNodes.Count - 1 do
       begin
         ANode := TcxTreeListNode(FExpandedNodes.List[I]);
         if not InternalLoadChildren(ANode) then
           Continue;
         Arguments.StartNode := ANode.FFirst;        
         FNodeLast := ANode.FLast;
         FIsLastNodeFound := False;
         Result := inherited Execute;
         if Result <> nil then
           Break;
       end;
     finally
       Arguments.IgnoreStartNode := ASavedIgnoreStartNode;
       Arguments.StartNode := ASavedStartNode;
     end;
   end;

begin
  if not Arguments.Forward then
    Result := inherited Execute
  else
  begin
    Result := SearchAmongExpandedNodes;
    if (Result = nil) and not Arguments.ExpandedOnly then
      Result := SearchAmongCollapsedNodes;
  end;
end;

function TcxTreeListSearchEngine.TCachedSearchStrategy.InternalLoadChildren(
  ANode: TcxTreeListNode): Boolean;
begin
  Result := inherited LoadChildren(ANode, False);
end;

function TcxTreeListSearchEngine.TCachedSearchStrategy.FindNext(ANode: TcxTreeListNode): TcxTreeListNode;
begin
  Result := ANode;
  if Result = FNodeLast then
    FIsLastNodeFound := True;
  if not LoadChildren(Result, Arguments.ExpandedOnly) then
    while Result <> nil do
    begin
      if Result.FNext <> nil then
      ...
      if (Result = FNodeLast) or (ANode = FNodeLast) then
        Result := nil;
    end
  else
    Result := Result.FFirst;
...
function TcxTreeListSearchEngine.TCachedSearchStrategy.IsFinishNode(ANode: TcxTreeListNode): Boolean;
begin
  Result := FIsLastNodeFound and (ANode = FNodeLast);
  if not Result then
    Result := inherited IsFinishNode(ANode);
end;

{ TcxTreeListSearchEngine.TCacheCombinedFindStrategy.TExpandedNodeBasedStrategy }

function TcxTreeListSearchEngine.TCachedSearchStrategy.TExpandedNodeBasedStrategy.LoadChildren(
  ANode: TcxTreeListNode; AExpandedOnly: Boolean): Boolean;
begin
  Result := (ANode.FCount <> 0) and ANode.Expanded;
...
procedure TcxTreeListColumn.MakeVisible;
  ...
  begin
    if Abs(APosition) <= 2 then
      APosition := 0;
    if TreeList.ViewInfo.IsScrollAnimationEnabled and not TreeList.Controller.EditingController.EditingItemSetting then
      TreeList.ViewInfo.SetHScrollPosAnimated(APosition)
    else
...
begin
  ...
  if not HeaderCell.HasClipping then
  ...
  if TreeList.ViewInfo.IsColumnScrollAnimationActive and AWasFullyVisible then
    TreeList.ViewInfo.StopColumnScrollAnimation;
end;
...
begin
  Result := CanFocus and Options.Editing and
    TreeList.Options.OptionsData.Editing and (TreeList.FocusedNode <> nil) and
      not TreeList.FocusedNode.IsHidden and not IsExpression;
end;
...
begin
  ...
  begin
    AOffset := TreeList.LookAndFeelPainter.HeaderContentOffsets(ScaleFactor);
    Result := cxTextWidth(TreeList.Styles.GetColumnHeaderParams(Self).Font, Caption.Text) +
     (AOffset.Left + AOffset.Right) + cxHeaderTextOffset;
...
begin
  ...
  begin
    ...
    AFooterCellTextOffsets := TreeList.ViewInfo.FooterCellTextOffsets;
    AItemWidth := cxTextWidth(AViewParams.Font, AText) + AFooterCellTextOffsets.Left + AFooterCellTextOffsets.Right;
...
function TcxTreeListColumn.GetReferenceNameForExpression: string;
begin
  Result := Caption.Text;
end;
...
begin
  ...
  AValue := DataController.GetNodeValue(ANode, ItemIndex);
  AUseValue := (AProperties.GetEditValueSource(False) = evsValue) or
    (AProperties.GetEditValueSource(False) = evsText) and AProperties.IsValueFormattedByProperties or
    VarIsNull(AValue) and (not IsExpression or (DataController.GetNodeErrorCode(ANode, ItemIndex) = 0));
  if AUseValue then
...
begin
  ...
  try
    if AEditProp = PropertiesValue then
      ...
      AViewData := AEditProp.CreateViewData(TreeList.EditStyle, True, TreeList.ScaleFactor);
...
function TcxTreeListColumn.IsVisible: Boolean;
begin
  Result := Visible;
end;
...
begin
  ...
  try
    ...
    TreeList.DataController.UpdateExpressionItems;
  finally
    TreeList.EndUpdate;
  end;
end;
...
begin
  ...
  FColumnScrollAnimationHelper := TcxTreeListColumnScrollAnimationHelper.Create(Self);
  FNodeScrollAnimationHelper := TcxTreeListNodeScrollAnimationHelper.Create(Self);
end;
...
begin
  ...
  FreeAndNil(FNodeScrollAnimationHelper);
...
begin
  Result := (IsDirty or (TreeList.Changes - SelectionChanges <> [])) and
    not (CalculateInProcess or TreeList.IsLocked or TreeList.ChangesLocked);
  if Result then
...
begin
  ...
  Result.InitializeWithBorders(ABand, Bounds, R);
...
begin
  ...
  begin
    ...
    begin
      if (FixedSeparatorWidth = 0) and ABand.IsPreviousToRightFixedBands then
      begin
        Inc(ABounds.Right, GridLineWidth);
        Inc(AClipRect.Right, GridLineWidth);
      end;
    end;
  end;
  ...
  if APart = tlbpContent then
  begin
    if not TreeList.UseLightBorders then
      Result.FBorders := GridLines
    else
    begin
      Result.FBorders := GridLines - [bRight];
      if ABand.IsRightMostVisible then
        if (not TreeList.OptionsView.ColumnAutoWidth) and (ABounds.Right < Bounds.Right) then
          Result.FBorders := Result.FBorders + [bRight];
    end;
  end;
  if APart = tlbpHeader then
  begin
    ...
    begin
      Include(Result.FBorders, bRight);
      if ABand.IsRightMostVisible then
        if TreeList.OptionsView.ColumnAutoWidth or (ABounds.Right >= Bounds.Right) and (not TreeList.ViewInfo.ForceIncludeRightmostBorder) then
          Result.FBorders := Result.FBorders - [bRight];
    end;
  end;
...
function TcxTreeListViewInfo.AddColumnHeader(AColumn: TcxTreeListColumn; ABandRowVisibleIndex, AColumnVisibleIndexInTheBandRow: Integer;
  var ABounds: TRect; const AClipBounds: TRect): TcxTreeListColumnHeaderCellViewInfo;
var
  R: TRect;
  ABand: TcxTreeListBand;
...
begin
  ...
  begin
    ...
    ABandRow := AColumn.Position.Row;
    if (AColumnVisibleIndexInTheBandRow = 0) and ABand.IsLeftMostVisible then
      Exclude(Result.FBorders, bLeft);
    if (AColumnVisibleIndexInTheBandRow = ABandRow.VisibleItemCount - 1) and (FixedSeparatorWidth > 0) and ABand.IsLastLeftFixed then
      Exclude(Result.FBorders, bRight);
    if (AColumnVisibleIndexInTheBandRow = 0) and ABand.IsNextToLeftFixedBands then
      Exclude(Result.FBorders, bLeft);
    if (AColumnVisibleIndexInTheBandRow = ABandRow.VisibleItemCount - 1) and ABand.IsPreviousToRightFixedBands then
      Exclude(Result.FBorders, bRight);
    if (AColumnVisibleIndexInTheBandRow = 0) and (FixedSeparatorWidth > 0) and ABand.IsFirstRightFixed then
      Exclude(Result.FBorders, bLeft);
    if (AColumnVisibleIndexInTheBandRow = ABandRow.VisibleItemCount - 1) and ABand.IsRightMostVisible then
      if (TreeList.OptionsView.ColumnAutoWidth or (Result.BoundsRect.Right >= Bounds.Right)) and (not TreeList.ViewInfo.ForceIncludeRightmostBorder) then
        Exclude(Result.FBorders, bRight);
    if (ABandRowVisibleIndex = 0) and (not TreeList.OptionsView.Bands) then
      Exclude(Result.FBorders, bTop);
  end;
...
begin
  ...
  if UseLightBorders and (AClass = TcxTreeListEditCellViewInfo) then
  begin
    ABand := AColumn.Position.Band;
    if AColumn.HasIndent and (AViewData.IndentWidth = 0) then
      Exclude(Result.CellBorders, bLeft);
    if (FixedSeparatorWidth > 0) and (AColumn.Position.VisibleColIndex = AColumn.Position.Row.VisibleItemCount - 1) and ABand.IsLastLeftFixed then
      Exclude(Result.CellBorders, bRight);
    if (FixedSeparatorWidth > 0) and (AColumn.Position.VisibleColIndex = 0) and ABand.IsFirstRightFixed then
      Exclude(Result.CellBorders, bLeft);
    if (AColumn.Position.VisibleColIndex = AColumn.Position.Row.VisibleItemCount - 1) and ABand.IsRightMostVisible then
      if (TreeList.OptionsView.ColumnAutoWidth or (Result.BoundsRect.Right >= Bounds.Right)) and (not TreeList.ViewInfo.ForceIncludeRightmostBorder) then
        Exclude(Result.CellBorders, bRight);
    if (AColumn.Position.VisibleColIndex = 0) and ABand.IsNextToLeftFixedBands and (FixedSeparatorWidth <> 0)  then
      Exclude(Result.CellBorders, bLeft);
    if (AColumn.Position.VisibleColIndex = AColumn.Position.Row.VisibleItemCount - 1) and ABand.IsPreviousToRightFixedBands then
      Exclude(Result.CellBorders, bRight);
  end;
...
var
  ...
  AOffset: Integer;
begin
  R := cxRectSetTop(ABounds,
    AClipRect.Top + FooterPanelContentOffsets.Top + AColumn.Position.Row.LineOffset * (FooterLineHeight + FFooterVerticalSpaceBetweenItems) + 1 {border},
    FooterLineHeight + (AColumn.Position.LineCount - 1) * (FooterLineHeight + FFooterVerticalSpaceBetweenItems));
  AOffset := 0;
  if not IsPrinting then
  begin
    InflateRect(R, -FFooterHorizontalSpaceBetweenItems, 0);
    AOffset := FFooterVerticalSpaceBetweenItems;
  end;
  ...
  begin
    AddCell(TcxTreeListFooterMultiItemsCellViewInfo, R, AClipRect, ACell);
    ...
    for I := 0 to AItems.Count - 1 do
      ...
      begin
        AddCell(TcxTreeListFooterSingleCellViewInfo, R, AClipRect, ACell);
        TcxTreeListFooterSingleCellViewInfo(ACell).Initialize(ADataNode, AttachNode, AColumn, AItems[I]);
        OffsetRect(R, 0, FooterLineHeight + AOffset);
      end;
end;
...
begin
  ...
  begin
    ...
    if (I = 0) and not IsPrinting then
      Inc(ACellsClipRect.Left, GridLineWidth);
    if I = Bands.BottomItemCount - 1 then
      Dec(ACellsClipRect.Right, GridLineWidth);
    ...
    begin
      ...
      begin
        if ABounds.Right < Bounds.Right then
          Include(APart.FBorders, bRight);
      end;
    end;
    if not IsPrinting then
    ...
    begin
      ...
      if ((ADataNode = Root) and not AColumn.Options.Footer) or ((ADataNode <> Root) and not AColumn.Options.GroupFooter) then
        Continue;
      if cxRectIntersect(R, ACellsClipRect, cxRectSetYPos(AColumn.HeaderCell.DisplayRect, AClipRect.Top, AClipRect.Bottom)) then
      begin
...
begin
  ...
  begin
    case APosition of
      tlipColumns:
        begin
          if not TreeList.OptionsView.Bands then
            Exclude(Result.FBorders, bTop);
          Exclude(Result.FBorders, bLeft);
        end;
      tlipFooter:
        begin
          Exclude(Result.FBorders, bLeft);
          Exclude(Result.FBorders, bBottom);
        end;
      tlipContent:
        Exclude(Result.FBorders, bLeft);
      tlipBands:
...
begin
  ...
  begin
    ...
    if TreeList.UseLightBorders then
    begin
      if (FixedSeparatorWidth = 0) and ABand.IsFirstRightFixed then
      begin
        Inc(R.Left, GridLineWidth);
        Inc(AClipRect.Left, GridLineWidth);
      end;
    end;
    ...
    if ABand.ActuallyExpandable then
      Inc(R.Left, AViewData.IndentWidth + Byte(bLeft in GridLines) * GridLineWidth);
    if I = 0 then
...
begin
  ...
  Inc(AViewData.ContentBounds.Left, AViewData.IndentWidth + IndicatorWidth + Byte(bLeft in GridLines) * GridLineWidth);
  AViewData.Bounds.Right := Min(AViewData.Bounds.Right, Bounds.Right) + Byte(HScrollSize > cxRectWidth(ClientRect) * GridLineWidth);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if AViewData.IndentWidth <> 0 then
          Inc(R.Left, AViewData.IndentWidth + Byte(bLeft in GridLines) * GridLineWidth);
      end;
    end;
    ...
    if UseLightBorders then
    begin
      if (AColumn.Position.VisibleColIndex = 0) and ABand.IsFirstRightFixed and (FixedSeparatorWidth = 0) then
      begin
        Inc(R.Left, GridLineWidth);
        Inc(AClipRect.Left, GridLineWidth);
      end;
...
begin
  ...
  if (OptionsView.GroupFooters = tlgfAlwaysVisible) and
    ...
    Dec(AFooterLevel);
  repeat
    ABounds := cxRectSetTop(AViewData.Bounds, AViewData.Height - GridLineWidth +
      Byte(IsPrinting) * GridLineWidth, GroupFooterHeight + GridLineWidth - Byte(IsPrinting) * GridLineWidth);
...
begin
  ...
  begin
    Inc(ABounds.Left, AViewData.IndentWidth + Byte(bLeft in GridLines) * GridLineWidth);
    if Bands.VisibleLeftFixedCount = 0 then
...
begin
  ...
  Result.ContentBounds := cxRectSetTop(Result.ContentBounds, 0, Result.Height - Byte(bRight in GridLines) * GridLineWidth);
...
function TcxTreeListViewInfo.IsColumnScrollAnimationActive: Boolean;
begin
  Result := FColumnScrollAnimationHelper.IsAnimationActive;
end;

procedure TcxTreeListViewInfo.ScrollAnimationEnded;
begin
  if not IsScrollAnimationActive then
    EditingController.FinalizeCellAfterScrollAnimation;
end;

procedure TcxTreeListViewInfo.ScrollAnimationStarted;
begin
  if not IsScrollAnimationActive then
    EditingController.InitializeCellBeforeScrollAnimation;
end;

procedure TcxTreeListViewInfo.StopColumnScrollAnimation;
begin
  FColumnScrollAnimationHelper.StopAnimation(True);
end;

procedure TcxTreeListViewInfo.DestroyNodeScrollAnimation;
begin
  FNodeScrollAnimationHelper.StopAnimation(False);
end;

function TcxTreeListViewInfo.IsNodeScrollAnimationActive: Boolean;
begin
  Result := FNodeScrollAnimationHelper.IsAnimationActive;
end;

procedure TcxTreeListViewInfo.NodeScrollAnimated(ATopNode: TcxTreeListNode; ATopRecordPixelScrollOffset: Integer);
begin
  if ATopNode <> nil then
    FNodeScrollAnimationHelper.SetPosition(TcxTreeListNodePixelScrollData.Create(ATopNode, ATopRecordPixelScrollOffset));
end;

procedure TcxTreeListViewInfo.DestroyColumnScrollAnimation;
begin
  FColumnScrollAnimationHelper.StopAnimation(False);
end;

function TcxTreeListViewInfo.IsScrollAnimationActive: Boolean;
begin
  Result := IsColumnScrollAnimationActive or IsNodeScrollAnimationActive;
end;

function TcxTreeListViewInfo.IsScrollAnimationEnabled: Boolean;
begin
  Result := TreeList.IsScrollAnimationEnabled and IsRecordPixelScrollMode and
    not Controller.IsNavigationKeyAutoRepeat;
end;
...
begin
  ...
  begin
    ...
    if OptionsView.Footer and Columns[I].Options.Footer then
      FFooterLineHeight := Max(FFooterLineHeight, GetFooterLineHeight(Styles.GetColumnFooterParams(Columns[I], Root).Font));
  end;
  ...
  if FFooterLineHeight = 0 then
    FFooterLineHeight := GetFooterLineHeight(TreeList.Font);
  ...
  for I := 0 to TreeList.ColumnCount - 1 do
    if (TreeList.Columns[I].HeaderCell <> nil) and
      (TreeList.Columns[I].HeaderCell.Item = FDragItem) then
        Controller.DragItem := TreeList.Columns[I].HeaderCell;
  for I := 0 to TreeList.Bands.Count - 1 do
    if (TreeList.Bands[I].HeaderCell <> nil) and
      (TreeList.Bands[I].HeaderCell.Item = FDragItem) then
        Controller.DragItem := TreeList.Bands[I].HeaderCell;
end;

procedure TcxTreeListViewInfo.BeforeCalculate;
begin
  FGridLineWidth := TreeList.GridLineWidth;
...
begin
  FContentWidth := 0;
  ABounds := Bounds;
  Inc(ABounds.Top, PanelsHeight[fpTop]);
  ...
  for AKind := Low(TcxTreeListBandFixedKind) to High(TcxTreeListBandFixedKind) do
    FContentParts[AKind].Bottom := Bounds.Bottom - PanelsHeight[fpBottom];
...
var
  I, ABandRowVisibleIndex, AColumnVisibleIndexInTheBandRow, ATop: Integer;
  ABand: TcxTreeListBand;
  ARow: TcxTreeListBandRow;
  AColumn: TcxTreeListColumn;
...
begin
  FMultiRows := False;
  ATop := Bounds.Top + BandsHeight + PanelsHeight[fpTop];
  if IsIndicatorVisible then
  begin
  ...
  begin
    ...
    for ABandRowVisibleIndex := 0 to ABand.BandRows.VisibleItemCount - 1 do
    begin
      ARow := ABand.BandRows.VisibleItems[ABandRowVisibleIndex];
      ARowBounds := cxRectSetYPos(ABounds,
        RowOffset[ARow.LineOffset], RowOffset[ARow.LineOffset + ARow.LineCount]);
      for AColumnVisibleIndexInTheBandRow := 0 to ARow.VisibleItemCount - 1 do
      begin
        AColumn := ARow.VisibleItems[AColumnVisibleIndexInTheBandRow];
        ARowBounds.Bottom := RowOffset[ARow.LineOffset + AColumn.Position.LineCount];
        AddColumnHeader(AColumn, ABandRowVisibleIndex, AColumnVisibleIndexInTheBandRow, ARowBounds, AClipBounds);
      end;
    end;
  end;
end;
...
begin
  ...
  FilterBoxViewInfo.GetHitTest(AHitTest);
end;
...
begin
  AShift := Byte(Painter.HeaderBorders([nLeft..nBottom]) <> cxBordersAll) * GridLineWidth;
...
begin
  FHitTestCells.Clear;
  FDragItem := nil;
  if Controller.DragItem is TcxTreeListHeaderCellViewInfo then
  begin
    FDragItem := TcxTreeListHeaderCellViewInfo(Controller.DragItem).Item;
    Controller.DragItem := nil;
  end;
  FreeMem(FRows);
...
procedure TcxTreeListViewInfo.DoSetTopNodeWithOffset(ANode: TcxTreeListNode; APixelScrollNodeOffset: Integer; ARaiseEvent: Boolean);
begin
  ...
  begin
    ...
    if TreeList.TopVisibleNode <> ANode then
      TreeList.DoSetTopVisibleNode(ANode, ARaiseEvent)
    else
      TreeList.LayoutChanged;
  end;
end;
...
function TcxTreeListViewInfo.GetPixelScrollOffset(AFirstNode: TcxTreeListNode; AFirstNodeOffset: Integer;
  ALastNode: TcxTreeListNode; ALastNodeOffset: Integer): Integer;
var
  ANode: TcxTreeListNode;
  ASize: Integer;
  ASign: Integer;
begin
  Result := 0;
  if (AFirstNode.VisibleIndex > ALastNode.VisibleIndex) or
    (AFirstNode = ALastNode) and (AFirstNodeOffset < ALastNodeOffset) then
  begin
    ExchangePointers(AFirstNode, ALastNode);
    ExchangeLongWords(AFirstNodeOffset, ALastNodeOffset);
    ASign := -1;
  end
  else
    ASign := 1;
  ANode := AFirstNode;
  while ANode <> ALastNode do
  begin
    ASize := GetNodePixelScrollSize(ANode);
    Inc(Result, ASize);
    ANode := ANode.GetNextVisible;
  end;
  Inc(Result, AFirstNodeOffset - ALastNodeOffset);
...
begin
  ...
  begin
    FPrevFont := AFont;
    FPrevFontHeight := TdxTextMeasurer.TextLineHeight(AFont);
  end;
  Result := FPrevFontHeight;
end;

function TcxTreeListViewInfo.GetFooterLineHeight(AFont: TFont): Integer;
begin
  Result := GetFontHeight(AFont) + FooterCellTextOffsets.Top + FooterCellTextOffsets.Bottom;
  dxAdjustToTouchableSize(Result, ScaleFactor);
end;

function TcxTreeListViewInfo.GetForceIncludeRightmostBorder: Boolean;
begin
  Result := Assigned(TreeList.LookAndFeel) and (TreeList.LookAndFeel.ScrollbarMode = sbmHybrid) and
    Assigned(TreeList.VScrollBar) and TreeList.VScrollBar.Enabled;
end;
...
function TcxTreeListViewInfo.GetLastNodeByTopNode(ATopNode: TcxTreeListNode; ATopNodePixelScrollOffset: Integer): TcxTreeListNode;
var
  AOverPan: Boolean;
  AScrollContentSize: Integer;
begin
  AScrollContentSize := GetPixelScrollContentSize;
  if AScrollContentSize > VScrollSize then
    Result := TreeList.LastVisibleNode
  else
  begin
    Result := ATopNode;
    CalculatePixelScrollInfo(Result, ATopNodePixelScrollOffset, nil, -1,
      -AScrollContentSize, AOverPan);
  end;
end;
...
begin
  ...
  if Assigned(StateImages) and Assigned(ANode.ViewData) and (ANode.ViewData.StateImageIndent = nil) then
    Result := Result - LevelInfo[ANode.Level].Size.cx;
end;
...
begin
  ...
  begin
    if ABounds.Top = AClipRect.Top then
      Dec(AClipRect.Top, GridLineWidth);
    Dec(ABounds.Top, GridLineWidth);
  end;
  if bLeft in GridLines then
  begin
    if ABounds.Left = AClipRect.Left then
      Dec(AClipRect.Left, GridLineWidth);
    Dec(ABounds.Left, GridLineWidth);
  end;
end;
...
begin
  Result := (TreeList.OptionsBehavior.RecordScrollMode = rsmByPixel) or
    (TreeList.OptionsBehavior.RecordScrollMode = rsmDefault) and (cxIsTouchModeEnabled or TreeList.IsScrollAnimationEnabled);
end;

procedure TcxTreeListViewInfo.MakeVisible(ANode: TcxTreeListNode);
var
  APos: Integer;
  ATopNode, ANextTopNode: TcxTreeListNode;
  ATopNodePixelScrollOffset, ANextTopNodePixelScrollOffset: Integer;
  AWasFullyVisible: Boolean;
begin
  if IsScrollAnimationEnabled and not EditingController.EditingItemSetting then
  begin
    AWasFullyVisible := True;
    ATopNode := TreeList.TopVisibleNode;
    ATopNodePixelScrollOffset := TreeList.ViewInfo.PixelScrollNodeOffset;
    if ANode.VisibleIndex <= ATopNode.VisibleIndex then
    begin
      AWasFullyVisible := (ANode.VisibleIndex = ATopNode.VisibleIndex) and (ATopNodePixelScrollOffset = 0);
      SetTopNodeWithOffset(ANode, 0, True);
    end
    else
    begin
      GetPixelScrollTopNodeAndOffsetByBottomNode(ANode, ANextTopNode, ANextTopNodePixelScrollOffset);
      if (ANextTopNode.VisibleIndex > ATopNode.VisibleIndex) or
        (ANextTopNode.VisibleIndex = ATopNode.VisibleIndex) and (ANextTopNodePixelScrollOffset < ATopNodePixelScrollOffset) then
      begin
        DoSetLastNode(ANode, True);
        AWasFullyVisible := False;
      end;
    end;
    if IsNodeScrollAnimationActive and AWasFullyVisible then
      FNodeScrollAnimationHelper.StopAnimation(True);
  end
  else
    if IsRecordPixelScrollMode and
      ((ANode.ViewData = nil) or (ANode.ViewData.GetRealBounds.Bottom > ContentBounds.Bottom)) then
      DoSetLastNode(ANode, False)
    else
    begin
  ...
  EditingController.DoUpdateEdit;
end;
...
begin
  FreeAndNil(FCheckBoxViewData);
  FCheckBoxViewData := FCheckBoxProperties.CreateViewData(TreeList.EditStyle, True, ScaleFactor);
end;

procedure TcxTreeListViewInfo.RefreshFooterOffsets;
begin
  FFooterPanelContentOffsets := LookAndFeelPainter.FooterPanelContentOffsets(cxBordersAll, False, ScaleFactor, TreeList.BordersScaleFactor);
  FFooterCellTextOffsets := LookAndFeelPainter.FooterCellTextOffsets(cxBordersAll, ScaleFactor, TreeList.BordersScaleFactor);
  FFooterVerticalSpaceBetweenItems := (FooterPanelContentOffsets.Top + FooterPanelContentOffsets.Bottom) div 2;
  FFooterHorizontalSpaceBetweenItems := (FooterPanelContentOffsets.Left + FooterPanelContentOffsets.Right) div 2;
end;

procedure TcxTreeListViewInfo.SetTopNodeWithOffset(ANode: TcxTreeListNode; APixelScrollNodeOffset: Integer; AAnimated: Boolean);
begin
  if AAnimated then
    NodeScrollAnimated(ANode, APixelScrollNodeOffset)
  else
  begin
    if IsNodeScrollAnimationActive then
      DestroyNodeScrollAnimation;
    DoSetTopNodeWithOffset(ANode, APixelScrollNodeOffset, True);
  end;
end;
...
begin
  ...
  SetTopNodeWithOffset(ANode, APixelScrollNodeOffset, False);
  TreeList.Repaint;
  if AOverPan then
...
var
  AHScrollPos: Integer;
begin
  if IsColumnScrollAnimationActive and (AScrollCode in [scLineUp..scPageDown]) then
    AHScrollPos := FColumnScrollAnimationHelper.TargetPosition
  else
    AHScrollPos := HScrollPos;
  CheckScrollPosition(AScrollCode, AHScrollPos, HScrollPage, HScrollInc,
    HScrollSize + HScrollInc, AScrollPos);
  if IsScrollAnimationEnabled and (AScrollCode <> scTrack) then
    SetHScrollPosAnimated(AScrollPos)
  else
    HScrollPos := AScrollPos;
  AScrollPos := HScrollPos;
end;
...
var
  ...
  ANextTopNode, ATopNode, ALastNode: TcxTreeListNode;
  ANextTopNodePixelScrollOffset, ATopNodePixelScrollOffset: Integer;
begin
  if (TopNode = nil) or (Count = 0) then Exit;

  if IsScrollAnimationEnabled and (AScrollCode in [scLineUp..scPageDown]) then
  begin
    if IsNodeScrollAnimationActive then
    begin
      ATopNode := FNodeScrollAnimationHelper.TargetPosition.Node;
      ATopNodePixelScrollOffset := FNodeScrollAnimationHelper.TargetPosition.ScrollOffset;
    end
    else
    begin
      ATopNode := TopNode;
      ATopNodePixelScrollOffset := FPixelScrollNodeOffset;
    end;
    ANextTopNode := nil;
    ANextTopNodePixelScrollOffset := 0;
    case AScrollCode of
      scLineUp:
        if ATopNode.IsFirstVisible then
          ANextTopNode := ATopNode
        else
          ANextTopNode := ATopNode.GetPrevVisible;
      scPageUp:
        begin
          if ATopNode.IsFirstVisible then
            ANextTopNode := ATopNode
          ...
          begin
            GetPixelScrollTopNodeAndOffsetByBottomNode(ATopNode, ANextTopNode, ANextTopNodePixelScrollOffset);
            if (ANextTopNode = ATopNode) and (ATopNodePixelScrollOffset = 0) then
              ANextTopNode := ATopNode.GetPrevVisible
            else
              ANextTopNodePixelScrollOffset := 0;
          end;
      ...
      scLineDown:
        begin
          UpdatePixelScrollTopNodeAndOffsetMaxValues;
          if ATopNode = FMaxPixelScrollTopNode then
            if ATopNodePixelScrollOffset = FMaxPixelScrollTopNodeOffset then
              Exit
            else
            begin
              ANextTopNode := FMaxPixelScrollTopNode;
              ANextTopNodePixelScrollOffset := FMaxPixelScrollTopNodeOffset;
            end
          else
            ANextTopNode := ATopNode.GetNextVisible;
        end;
      scPageDown:
        begin
          UpdatePixelScrollTopNodeAndOffsetMaxValues;
          ...
          begin
            ALastNode := GetLastNodeByTopNode(ATopNode, ATopNodePixelScrollOffset);
            if ALastNode.VisibleIndex > FMaxPixelScrollTopNode.VisibleIndex then
            begin
              ANextTopNode := FMaxPixelScrollTopNode;
              ANextTopNodePixelScrollOffset := FMaxPixelScrollTopNodeOffset;
            end
            else
              if (ALastNode = ATopNode) and (ALastNode.GetNextVisible <> nil) then
                ANextTopNode := ALastNode.GetNextVisible
              else
                ANextTopNode := ALastNode;
          end;
        end;
    end;
    NodeScrollAnimated(ANextTopNode, ANextTopNodePixelScrollOffset);
...
begin
  APosition := Max(0, Min(APosition, AMax - APage));
end;

procedure TcxTreeListViewInfo.DoSetHScrollPos(APosition: Integer);
const
...
procedure TcxTreeListViewInfo.DoSetLastNode(ANode: TcxTreeListNode; AAnimated: Boolean);
var
...
begin
  ...
  SetTopNodeWithOffset(ATopNode, ATopNodePixelScrollOffset, AAnimated);
end;
...
begin
  Result := FooterHeight + PanelsHeight[fpBottom];
end;
...
function TcxTreeListViewInfo.GetController: TcxTreeListController;
begin
  Result := TreeList.Controller;
end;
...
function TcxTreeListViewInfo.GetEditingController: TcxTreeListEditingController;
begin
  Result := Controller.EditingController;
end;

function TcxTreeListViewInfo.GetFilterBoxViewInfo: TcxTreeListFilterBoxViewInfo;
begin
  Result := TcxTreeListFilterBoxViewInfo(inherited FilterBoxViewInfo);
end;
...
var
  AFooterLineCount: Integer;
begin
  AFooterLineCount := FooterLineCount;
  if AFooterLineCount > 0 then
  begin
    Result := FooterLineHeight + (AFooterLineCount - 1) * (FooterLineHeight + FFooterVerticalSpaceBetweenItems);
    if (Result = 0) or IsPrinting then
      Exit;
    Result := Result + FooterPanelContentOffsets.Top + FooterPanelContentOffsets.Bottom;
...
var
  AGroupFooterLineCount: Integer;
begin
  AGroupFooterLineCount := GroupFooterLineCount;
  if AGroupFooterLineCount > 0 then
  begin
    Result := FooterLineHeight + (AGroupFooterLineCount - 1) * (FooterLineHeight + FFooterVerticalSpaceBetweenItems);
    if (Result = 0) or IsPrinting then
      Exit;
    Result := Result + FooterPanelContentOffsets.Top + FooterPanelContentOffsets.Bottom;
    Inc(Result, GridLineWidth); 
...
begin
  Result := HeadersHeight + BandsHeight + PanelsHeight[fpTop];
end;
...
function TcxTreeListViewInfo.GetUseLightBorders: Boolean;
begin
  Result := TreeList.UseLightBorders;
end;
...
var
  I: Integer;
  ANode: TcxTreeListNode;
  AIsAnyNodeHeightChanged: Boolean;
begin
  if IsPixelBasedScrollDataPos then
  begin
    Result := 0;
    FIsScrollbarAnnotationsRenderingByRecordPixelScrollPosition := True;
    AIsAnyNodeHeightChanged := False;
    for I := 0 to TreeList.AbsoluteVisibleCount - 1 do
    begin
      ANode := TreeList.AbsoluteVisibleItems[I];
      AIsAnyNodeHeightChanged := AIsAnyNodeHeightChanged or (ANode.FPixelScrollPosition <> Result);
      ANode.FPixelScrollPosition := Result;
      Result := Result + GetNodePixelScrollSize(ANode);
    end;
    if AIsAnyNodeHeightChanged then
      TreeList.InvalidateScrollbarAnnotations;
  end
  else
  begin
    if FIsScrollbarAnnotationsRenderingByRecordPixelScrollPosition then
    begin
      FIsScrollbarAnnotationsRenderingByRecordPixelScrollPosition := False;
      for I := 0 to TreeList.AbsoluteVisibleCount - 1 do
        TreeList.AbsoluteVisibleItems[I].FPixelScrollPosition := 0;
      TreeList.InvalidateScrollbarAnnotations;
    end;
    Result := TreeList.AbsoluteVisibleCount;
  end;
...
procedure TcxTreeListViewInfo.SetHScrollPosAnimated(APosition: Integer);
begin
  FColumnScrollAnimationHelper.SetPosition(APosition);
end;

procedure TcxTreeListViewInfo.SetHScrollPos(APosition: Integer);
begin
  ValidateScrollPos(APosition, HScrollPage, HScrollSize);
  if APosition = HScrollPos then
    Exit;
  if IsColumnScrollAnimationActive then
    DestroyColumnScrollAnimation;
  DoSetHScrollPos(APosition);
...
begin
  ...
  begin
    ...
    if (ANode <> TopNode) or (AOffset <> FPixelScrollNodeOffset) then
      SetTopNodeWithOffset(ANode, AOffset, False);
  end
  else
...
procedure TcxTreeListConditionalFormattingProvider.DoOwnerBeginUpdate;
begin
  FOwner.BeginUpdate;
end;

procedure TcxTreeListConditionalFormattingProvider.DoOwnerEndUpdate;
begin
  FOwner.EndUpdate;
end;
...
function TcxTreeListConditionalFormattingProvider.GetItemDisplayName(AItem: TObject): string;
var
  AColumn: TcxTreeListColumn absolute AItem;
begin
  Result := AColumn.Caption.Text;
end;
...
function TcxTreeListConditionalFormattingProvider.GetMaxRowIndex: Integer;
begin
  Result := FOwner.AbsoluteCount - 1;
end;
...
function TcxTreeListConditionalFormattingProvider.IsItemVisible(AItem: TObject): Boolean;
var
  AColumn: TcxTreeListColumn absolute AItem;
begin
