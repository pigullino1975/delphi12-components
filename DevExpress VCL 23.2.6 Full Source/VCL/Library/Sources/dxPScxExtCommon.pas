...
uses
  ...
  dxCore, cxGeometry, cxLookAndFeelPainters, dxGDIPlusClasses, cxGraphics,
...
function FontImages: TcxImageList;
...
uses
  SysUtils, Controls, Variants, cxLabel, cxProgressBar, cxTrackBar, cxFontNameComboBox, dxRatingControl, dxDPIAwareUtils,
  cxExtEditUtils, {$IFDEF REGISTERCXSHELLCTRLS} cxShellComboBox, {$ENDIF}
  ComCtrls, Forms, cxContainer, cxRichEdit, dxPSRELnk, dxPSUtl, Math, cxDrawTextUtils, cxVariants;

const
  dxThisUnitName = 'dxPScxExtCommon';
...
begin
  ...
  begin
    FFontImages := TcxImageList.Create(nil);
    FFontImages.Width := cxFontNameComboBox.FTrueTypeFontGlyph.Width;
    FFontImages.Height := cxFontNameComboBox.FTrueTypeFontGlyph.Height;
    FFontImages.AllocBy := 2;
    FFontImages.Add(cxFontNameComboBox.FTrueTypeFontGlyph);
    FFontImages.Add(cxFontNameComboBox.FNonTrueTypeFontGlyph);
  end;
  Result := FFontImages;
end;
...
class function TdxPSProgressBarDataMap.GetValue(AProperties: TcxCustomEditProperties; const AValue: TcxEditValue): Double;
var
...
begin
  ...
  if not TryStrToFloat(AProgressBarProperties.InternalGetDisplayText(AValue, False, cxtsPosition), Result) then
    Result := 0;
end;
  ...
  begin
    ...
    try
      ...
      try
        ...
        AViewData := TcxCustomTrackBarViewData(AProperties.CreateViewData(AStyle, True, dxDefaultScaleFactor));
        try
...
class procedure TdxPSRatingControlDataMap.InitializeItem(
  ...
  begin
    ...
    try
      ...
      try
        AStyle.Assign(DefaultEditStyleController.Style);
        AViewData := TdxCustomRatingControlViewData(AProperties.CreateViewData(AStyle, True, dxDefaultScaleFactor));
        try
          AcxCanvas := TcxCanvas.Create(ACanvas);
          try
