...
uses
  ...
  dxCore, dxCoreClasses, cxClasses, dxCultureInfo, dxXMLDoc, dxGenerics;
...
type
  ...
  strict private
    ...
    function GetItem(Index: TdxListIndex): TdxSpellCheckerReplacement;
  public
    ...
    property Items[Index: TdxListIndex]: TdxSpellCheckerReplacement read GetItem; default;
  end;
  ...
  public
    // Helpers
    class procedure CheckFinish;
    class procedure CheckStart(AControl: TWinControl);
    class procedure DrawMisspellings(AControl: TWinControl; DC: HDC = 0);
    class procedure LayoutChanged(AControl: TWinControl);
    class procedure SelectionChanged(AControl: TWinControl);
    class procedure TextChanged(AControl: TWinControl);
    class procedure Undo;

    // General
    class procedure SetInstance(const AInstance: TObject);
    class procedure RegisterListener(AListener: TNotifyEvent);
    class procedure UnregisterListener(AListener: TNotifyEvent);
  ...
  strict private
    class var FAdapters: TdxClassDictionary<TdxSpellCheckerCreateAdapterProc>;
...
const
  dxThisUnitName = 'dxSpellCheckerCore';
...
function TdxSpellCheckerReplacementList.GetItem(Index: TdxListIndex): TdxSpellCheckerReplacement;
begin
...
class procedure TdxSpellCheckerInstance.CheckFinish;
begin
  if ISpellChecker <> nil then
    ISpellChecker.CheckFinish;
end;

class procedure TdxSpellCheckerInstance.CheckStart(AControl: TWinControl);
begin
  if ISpellChecker <> nil then
    ISpellChecker.CheckStart(AControl);
end;

class procedure TdxSpellCheckerInstance.DrawMisspellings(AControl: TWinControl; DC: HDC = 0);
begin
  if ISpellChecker <> nil then
    ISpellChecker.DrawMisspellings(AControl, DC);
end;

class procedure TdxSpellCheckerInstance.LayoutChanged(AControl: TWinControl);
begin
  if ISpellChecker <> nil then
    ISpellChecker.LayoutChanged(AControl);
end;

class procedure TdxSpellCheckerInstance.SelectionChanged(AControl: TWinControl);
begin
  if ISpellChecker <> nil then
    ISpellChecker.SelectionChanged(AControl);
end;

class procedure TdxSpellCheckerInstance.TextChanged(AControl: TWinControl);
begin
  if ISpellChecker <> nil then
    ISpellChecker.TextChanged(AControl);
end;

class procedure TdxSpellCheckerInstance.Undo;
begin
  if ISpellChecker <> nil then
    ISpellChecker.Undo;
end;

class procedure TdxSpellCheckerInstance.RegisterListener(AListener: TNotifyEvent);
begin
...
begin
  ...
  begin
    for I := 0 to FListeners.Count - 1 do
      FListeners[I](AInstance);
  end;
end;

class procedure TdxSpellCheckerInstance.UnregisterListener(AListener: TNotifyEvent);
begin
  if FListeners <> nil then
  begin
...
begin
  if FAdapters = nil then
    FAdapters := TdxClassDictionary<TdxSpellCheckerCreateAdapterProc>.Create;
...
begin
  Result := (FAdapters <> nil) and FAdapters.TryGetValue(AObject, AProc);
end;
