...
const
  dxThisUnitName = 'dxRichEdit.Api.Images';
...
begin
  ...
  begin
    Result := nil;
    if Safe.Cast(ARun, TdxFloatingObjectAnchorRun, AAnchorRun) then
    begin
      if Safe.Cast(AAnchorRun.Content, TdxPictureFloatingObjectContent, AContent) then
        Result := TdxNativeDocumentImage.Create(ADocument, ARunIndex);
    end;
  end;
end;
...
begin
  ...
  begin
    if Safe.Cast(ARun, TdxFloatingObjectAnchorRun, AAnchorRun) then
      Result := Safe.Cast(AAnchorRun.Content, TdxPictureFloatingObjectContent, AContent);
  end;
end;
...
begin
  Supports(FDocument.PieceTable.Runs[TdxNativeDocumentPosition(FRange.Start).Position.RunIndex], IdxPictureContainerRun, Result);
end;
...
begin
  Supports(FDocument.PieceTable.Runs[TdxNativeDocumentPosition(FRange.Start).Position.RunIndex], IdxRectangularScalableObject, Result);
end;

function TdxNativeDocumentImage.GetScaleX: Single;
begin
  Result := RectangleScalableObject.ScaleX * 0.01;
end;
...
begin
  Result := RectangleScalableObject.ScaleY * 0.01;
end;
