...
type
  ...
  public
    ...
    procedure DrawQuickAccessToolbarMarkTruncated(DC: HDC; const R: TRect; AState: Integer; APlace: TdxQATPosition); override;
    procedure DrawQuickAccessToolbarPopup(DC: HDC; const R: TRect); override;
    procedure DrawQuickAccessToolbarSmallButton(DC: HDC; const R: TRect; AState: Integer; APlace: TdxQATPosition); override;
...
const
  dxThisUnitName = 'dxRibbonSkins2019';
...
begin
  ...
  AHSL := TdxColorUtils.ColorToHSL(AColor);
  AHSL.S := 0;
  AColor := TdxColorUtils.HSLToColor(AHSL);
...
begin
  case APart of
    ...
    rtatpEditBackground:
      begin
        ...
        if AState = DXBAR_HOT then
          Result := TdxColorUtils.ChangeLightness(Result, 0.08);
      end;
    ...
    DXRIBBON_TAT_SMALLBUTTON:
      begin
        ...
        if AState = DXBAR_DISABLED then
          Result := TdxColorUtils.ChangeSaturation(Result, -1);
      end;
    ...
    DXBAR_BACKSTAGEVIEW_GALLERYCONTROL_GROUPHEADER:
      Result := TdxColorUtils.MultiplyLightness(GetPartColor(DXBAR_EDIT_BACKGROUND), 0.9);
  else
    Result := inherited;
  end;
end;
...
begin
  case APart of
    DXBAR_TOOLBAR, DXBAR_TOOLBARINPOPUP:
      Result := ScaleFactor.Apply(TRect.Create(9, 2, 9, 3));
    DXRIBBON_TAT_SMALLBUTTON:
      Result := ScaleFactor.Apply(TRect.Create(8, 4, 8, 4));
    DXBAR_BACKSTAGEVIEW_MENUBAR:
      Result := ScaleFactor.Apply(TRect.Create(0, 0, 0, 17));
    DXBAR_BACKSTAGEVIEW_MENUBAR_ITEM, DXBAR_BACKSTAGEVIEW_MENUBAR_TAB:
      Result := ScaleFactor.Apply(TRect.Create(20, 11, 20, 11));
...
procedure TdxCustomColorfulRibbon2019Skin.DrawQuickAccessToolbarSmallButtonDropButtonArrowPart(
  DC: HDC; const R: TRect; AState: Integer; APlace: TdxQATPosition);
begin
  DrawQuickAccessToolbarSmallButton(DC, R, AState, APlace);
end;
...
procedure TdxCustomColorfulRibbon2019Skin.DrawStatusBar(DC: HDC; const R: TRect; AIsLeftSideRaised, AIsRightSideRaised: Boolean);
begin
  DrawStatusBarCore(DC, R, GetTabGroupsAreaContentColor);
end;
...
begin
  case AState of
    DXBAR_HOT, DXBAR_HOTCHECK:
      Result := TdxColorUtils.ChangeLightness(GetMasterDarkColor, 0.08);
...
begin
  ...
  AValue := TdxColorUtils.ColorToHSL(GetTabGroupsAreaContentColor);
  AValue.S := Max(AValue.S - 0.035, 0);
  FHighlightContentColor := TdxColorUtils.HSLToColor(AValue.H, AValue.S, Max(AValue.L - 0.14, 0));
  FHighlightBorderColor := TdxColorUtils.HSLToColor(AValue.H, AValue.S, Max(AValue.L - 0.25, 0));
end;
...
begin
  case AState of
    rtsActiveHot, rtsFocused, rtsHot:
      FillRectByColor(DC, R, TdxColorUtils.ChangeLightness(GetTabGroupsAreaContentColor, 0.05));
  end;
end;
...
begin
  ...
  DrawTabAreaButtonCore(DC, R,
    dxColorToAlphaColor(TdxColorUtils.ChangeLightness(AColor, Map[cxbsPressed])),
    dxColorToAlphaColor(TdxColorUtils.ChangeLightness(AColor, Map[AState])));
end;
...
begin
  ...
  Result := TdxColorUtils.ChangeLightness(GetMasterColor, ADelta);
end;
...
begin
  if AState = rbisInactive then
    Result := TdxColorUtils.HSLToColor(TdxColorUtils.ColorToHSL(GetFormCaptionAreaColor).H, 0, 0.8)
  else
    Result := clWhite;
end;
...
begin
  ...
  if AState = cxbsDisabled then
    ABorderColor := TdxColorUtils.ChangeSaturation(ABorderColor, -1);
...
begin
  case AState of
    DXBAR_HOT, DXBAR_HOTCHECK:
      Result := TdxColorUtils.MultiplyLightness(GetMenuBackgroundColor, 2.3);
    DXBAR_PRESSED, DXBAR_CHECKED:
      Result := TdxColorUtils.MultiplyLightness(GetMenuBackgroundColor, 1.7);
  else
    Result := clNone;
  end;
end;
