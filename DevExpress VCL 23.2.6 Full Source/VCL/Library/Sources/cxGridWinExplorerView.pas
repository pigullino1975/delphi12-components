...
uses
  System.UITypes,
  Forms, StdCtrls, Graphics, Classes, Controls, SysUtils, Windows, cxClasses, cxGeometry, dxCore,
  cxControls, cxLookAndFeelPainters, cxCustomData, dxScrollAnimation, cxGridCustomView, cxGridCustomTableView,
...
type
  ...
  TcxGridWinExplorerContentScrollAnimationHelper = class(TdxScrollAnimationHelper)
  strict private
    FOwner: TcxGridWinExplorerViewController;
  protected
  ...
  public
    constructor Create(AOwner: TcxGridWinExplorerViewController);
  end;
  ...
  private
    FContentScrollAnimationHelper: TcxGridWinExplorerContentScrollAnimationHelper;
  ...
  protected
    ...
    procedure DataScroll(AScrollBarKind: TScrollBarKind;
      AScrollCode: TScrollCode; var AScrollPos: Integer); virtual;
    procedure DoSetContentScrollPos(AValue: Integer);
    ...
    procedure InternalSetScrollPos(AValue: Integer);
    function IsRecordFullyVisibleAtBottom(ARecordIndex: Integer): Boolean; override;
  ...
  protected
    ...
    function GetDefaultHotTrackSelection: Boolean; override;
  published
  ...
  public
    ...
    procedure GetDataCellParams(ARecord: TcxCustomGridRecord; AItem: TcxCustomGridTableItem; out AParams: TcxViewParams;
      ...
      AIgnoreHotTrack: Boolean = False); override;
...
uses
  Types, Math, Variants, RTLConsts, cxCheckBox, dxCoreGraphics, cxLookAndFeels, dxTypeHelpers;

const
  dxThisUnitName = 'cxGridWinExplorerView';
...
begin
  ...
  begin
    AValue := ViewInfo.GetValue;
    if VarIsOrdinal(AValue) then
      TdxImageDrawer.DrawImage(Canvas, ViewInfo.Bounds, nil, ViewInfo.ImageList, AValue, ifmProportionalStretch)
  end
  else
...
begin
  if ViewInfo.ButtonState <> cxbsNormal then
    ViewInfo.LookAndFeelPainter.WinExplorerViewDrawGroup(Canvas, ABounds, ViewInfo.ButtonState,
      ViewInfo.Params.Color, ViewInfo.Params.Bitmap);
end;
...
begin
  if ViewInfo.ButtonState <> cxbsNormal then
    ViewInfo.LookAndFeelPainter.WinExplorerViewDrawRecord(Canvas, ABounds, ViewInfo.ButtonState,
      ViewInfo.Params.Color, ViewInfo.Params.Bitmap);
end;
...
begin
  Result := inherited CanDrawCheckBox and ((ViewInfo.ButtonState <> cxbsNormal) or
...
function TcxGridWinExplorerViewExpandButtonViewInfo.GetHeight: Integer;
begin
...
function TcxGridWinExplorerViewExpandButtonViewInfo.GetWidth: Integer;
begin
...
begin
  Result := inherited GetTransparent or (not Editing and (RecordViewInfo.ButtonState <> cxbsNormal));
end;
...
begin
  CalculateParams;
  Result := TdxTextMeasurer.TextLineHeight(Params.Font) + cxTextOffset * 2 + TdxVisualRefinements.Padding.ScaledHeight(ScaleFactor);
end;
...
function TcxGridWinExplorerViewCustomRecordViewInfo.GetActualState: TcxGridCellState;
begin
  if Selected then
    Result := gcsPressed
  else
...
function TcxGridWinExplorerViewCustomRecordViewInfo.GetButtonState: TcxButtonState;
begin
  Result := inherited GetButtonState;
  if (Result = cxbsPressed) and not GridView.DrawRecordActive(GridRecord) then
    Result := cxbsDisabled;
end;

procedure TcxGridWinExplorerViewCustomRecordViewInfo.GetBackViewParams(var AParams: TcxViewParams);
begin
  GridView.Styles.GetDataCellParams(GridRecord, nil, AParams, False, nil, True, True);
end;
...
begin
  ...
  begin
    ...
    ABounds := cxRectSetLeft(ABounds, ABounds.Left + ScaleFactor.Apply(cxGridWinExplorerViewGroupExpandButtonIndent), ExpandButtonViewInfo.Width);
    ABounds := cxRectCenterVertically(ABounds, ExpandButtonViewInfo.Height);
...
begin
  CalculateParams;
  Result := Max(
    TdxTextMeasurer.TextLineHeight(Params.Font) + 2 * ScaleFactor.Apply(cxTextOffset),
...
function TcxGridWinExplorerViewRecordViewInfo.GetImageSize: TSize;
var
  AContentPadding: TdxPadding;
begin
  Result := OptionsDisplayMode.ImageSize.Value;
  if LookAndFeelPainter.SupportsImageEditorPadding then
  begin
    AContentPadding := LookAndFeelPainter.GetImageEditorPadding(True, ScaleFactor);
    Result.Width := Result.Width + AContentPadding.Width;
    Result.Height := Result.Height + AContentPadding.Height;
  end;
end;
...
var
  AContentPadding: TdxPadding;
begin
  Result := TextViewInfo.GetRequiredHeight + 2 * cxGridEditOffset;  
  if LookAndFeelPainter.SupportsEditorPadding(True) then
  begin
    AContentPadding := LookAndFeelPainter.GetEditorPadding(True, nil);
    Result := Result + AContentPadding.GetScaled(ScaleFactor).Height;
  end;
end;
...
function TcxGridWinExplorerViewRecordViewInfo.IsChildrenHotTracked: Boolean;
begin
  Result := inherited IsChildrenHotTracked or HasCheckBox and CheckBoxViewInfo.IsHotTracked or
    HasDescription and DescriptionViewInfo.IsHotTracked or HasText and TextViewInfo.IsHotTracked or
    HasImage and ImageViewInfo.IsHotTracked;
end;
...
procedure TcxGridWinExplorerViewRecordViewInfo.ResetChildrenState;
begin
  inherited ResetChildrenState;
  if HasCheckBox then
    CheckBoxViewInfo.ResetState;
  if HasDescription then
    DescriptionViewInfo.ResetState;
  if HasText then
    TextViewInfo.ResetState;
  if HasImage then
    ImageViewInfo.ResetState;
end;
...
function TcxGridWinExplorerViewControllerHelper.GetMouseWheelMajorScrollBarKind: TScrollBarKind;
begin
  Result := sbVertical;
end;
...
var
  APos: Integer;
begin
  if Controller.FContentScrollAnimationHelper.IsAnimationActive then
    APos := Controller.FContentScrollAnimationHelper.TargetPosition
  else
    APos := Controller.ContentScrollPos;
  case ADirection of
    dirLeft:
      Controller.DoSetContentScrollPos(APos - GetContentScrollStep);
    dirRight:
      Controller.DoSetContentScrollPos(APos + GetContentScrollStep);
    dirUp:
      Controller.ScrollRecords(False, 1);
...
function TcxGridWinExplorerViewControllerListModeHelper.GetMouseWheelMajorScrollBarKind: TScrollBarKind;
begin
  Result := sbHorizontal;
end;
...
{ TcxGridWinExplorerContentScrollAnimationHelper }

constructor TcxGridWinExplorerContentScrollAnimationHelper.Create(
  AOwner: TcxGridWinExplorerViewController);
begin
  inherited Create;
  FOwner := AOwner;
end;

procedure TcxGridWinExplorerContentScrollAnimationHelper.AfterAnimationEnded(AIsRaiseEvent: Boolean);
begin
  inherited AfterAnimationEnded(AIsRaiseEvent);
  FOwner.ScrollAnimationEnded;
end;

procedure TcxGridWinExplorerContentScrollAnimationHelper.BeforeAnimationStarted;
begin
  FOwner.ScrollAnimationStarted;
  inherited BeforeAnimationStarted;
end;

procedure TcxGridWinExplorerContentScrollAnimationHelper.CheckPosition(var AValue: Integer);
begin
  FOwner.Helper.AdjustContentScrollPos(AValue);
end;

function TcxGridWinExplorerContentScrollAnimationHelper.GetOwnerPosition: Integer;
begin
  Result := FOwner.ContentScrollPos;
end;

procedure TcxGridWinExplorerContentScrollAnimationHelper.UpdateOwnerPosition(AValue: Integer);
begin
  FOwner.InternalSetScrollPos(AValue);
end;
...
begin
  ...
  CreateHelper;
  FContentScrollAnimationHelper := TcxGridWinExplorerContentScrollAnimationHelper.Create(Self);
end;

destructor TcxGridWinExplorerViewController.Destroy;
begin
  FreeAndNil(FContentScrollAnimationHelper);
  DestroyHelper;
  inherited Destroy;
end;
...
procedure TcxGridWinExplorerViewController.DoSetContentScrollPos(AValue: Integer);
begin
  if IsScrollAnimationEnabled then
    FContentScrollAnimationHelper.SetPosition(AValue)
  else
    ContentScrollPos := AValue;
end;
...
function TcxGridWinExplorerViewController.GetMouseWheelMajorScrollBarKind: TScrollBarKind;
begin
  Result := Helper.GetMouseWheelMajorScrollBarKind;
end;
...
begin
  if (GridView.ActiveDisplayMode = dmList) and (ViewData.RecordCount > 0) then
  begin
    ATopIndex := ABottomRecordIndex - GetPageVisibleRecordCount(TopRecordIndex, 0);
...
procedure TcxGridWinExplorerViewController.InternalSetScrollPos(AValue: Integer);
var
  ADelta: Integer;
begin
  if FContentScrollPos <> AValue then
  begin
    ADelta := FContentScrollPos - AValue;
    FContentScrollPos := AValue;
...
function TcxGridWinExplorerViewController.IsScrollAnimationActive: Boolean;
begin
  Result := inherited IsScrollAnimationActive or FContentScrollAnimationHelper.IsAnimationActive;
end;
...
procedure TcxGridWinExplorerViewController.StopScrollAnimation;
begin
  inherited StopScrollAnimation;
  if FContentScrollAnimationHelper.IsAnimationActive then
    FContentScrollAnimationHelper.StopAnimation(False);
end;

procedure TcxGridWinExplorerViewController.DisplayModeChanged;
var
  AScrollbarAnnotationActive: Boolean;
begin
  BeginUpdate;
  try
    AScrollbarAnnotationActive := GridView.ScrollbarAnnotations.Active;
    GridView.ScrollbarAnnotations.Active := False;
    try
      RecreateHelper;
      ContentScrollPos := 0;
      ScrollBarPos := 0;
      ...
      GridView.ScrollbarAnnotations.Active := AScrollbarAnnotationActive;
    end;
  finally
    EndUpdate;
  end;
...
begin
  if FContentScrollAnimationHelper.IsAnimationActive then
    APos := FContentScrollAnimationHelper.TargetPosition
  else
    APos := ContentScrollPos;
  case AScrollCode of
    scLineUp:
      ScrollData(GetScrollBarLineUpDirection(AScrollBarKind));
    scLineDown:
      ScrollData(GetScrollBarLineDownDirection(AScrollBarKind));
    scPageUp:
      DoSetContentScrollPos(APos - Helper.GetScrollableAreaSize(AScrollBarKind));
    scPageDown:
      DoSetContentScrollPos(APos + Helper.GetScrollableAreaSize(AScrollBarKind));
    scTrack:
...
begin
  ...
  InternalSetScrollPos(Value);
end;

{ TcxGridWinExplorerViewOptionsBehavior }

function TcxGridWinExplorerViewOptionsBehavior.GetDefaultHotTrack: Boolean;
begin
  Result := True;
end;

function TcxGridWinExplorerViewOptionsBehavior.GetDefaultHotTrackSelection: Boolean;
begin
  Result := False;
end;
...
begin
  if not AIgnoreHotTrack and (AWinRecord <> nil) and (AWinRecord.ViewInfo.ButtonState = cxbsHot) then
...
begin
  ...
  FIsGroupParams := True;
  try
    if TcxGridWinExplorerViewGroupRecord(ARecord).ViewInfo.ButtonState <> cxbsNormal then
...
function TcxGridWinExplorerView.IsDataScrollbar(AKind: TScrollBarKind): Boolean;
begin
  Result := Controller.Helper.IsDataScrollBar(AKind);
end;
