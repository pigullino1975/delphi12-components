...
uses
  ...
  dxSpreadSheetCoreFormulasTokens, dxSpreadSheetCoreReferences;
...
const
  dxThisUnitName = 'dxSpreadSheetFormatXLSFormulas';

type
  TdxSpreadSheetFormulaReferenceTokenAccess = class(TdxSpreadSheetFormulaReferenceToken);
  TdxSpreadSheetFormulaAreaReferenceTokenAccess = class(TdxSpreadSheetFormulaAreaReferenceToken);
...
begin
  Result := AToken;
  if AToken is TdxSpreadSheetFormulaDefinedNameToken then
  try
    Tokens.Delete(Tokens.Count - 1);
    AddToken(TdxSpreadSheetFormulaUnknownFunctionToken.Create(TdxSpreadSheetFormulaDefinedNameToken(AToken).Value));
    Result := Tokens.Last;
  finally
    AToken.Free;
  end;
...
begin
  ...
  AddToken(TdxSpreadSheetFormulaAreaReferenceToken.Create(
    TdxSpreadSheetReference.Create(ARow, AbsRow),
    TdxSpreadSheetReference.Create(AColumn, AbsColumn),
    TdxSpreadSheetReference.Create(ARow2, AbsRow2),
    TdxSpreadSheetReference.Create(AColumn2, AbsColumn2)));
  Inc(FOffset, 8);
end;
...
begin
  ...
  DecodeAreaReference(ATokens^[1], ATokens^[3], ATokens^[2], ATokens^[4],
    ARow, AColumn, ARow2, AColumn2, AbsRow, AbsColumn, AbsRow2, AbsColumn2);
  AddToken(TdxSpreadSheetFormula3DAreaReferenceToken.Create(ALink, nil,
    TdxSpreadSheetReference.Create(ARow, AbsRow),
    TdxSpreadSheetReference.Create(AColumn, AbsColumn),
    TdxSpreadSheetReference.Create(ARow2, AbsRow2),
    TdxSpreadSheetReference.Create(AColumn2, AbsColumn2))
...
begin
  Read_Area3d;
  TdxSpreadSheetFormula3DAreaReferenceToken(Tokens.Last).IsError := True;
end;
...
begin
  ...
  if (AInfo <> nil) and Assigned(AInfo.ParamInfo) then
  begin
...
begin
  ...
  if AInfo = nil then
    AddToken(TdxSpreadSheetFormulaDefinedNameToken.Create(AName, Owner.SpreadSheet.DefinedNames))
  else
    AddToken(TdxSpreadSheetFormulaUnknownNameToken.Create(AName));
  Inc(FOffset, 4);
end;
...
begin
  ...
  AddToken(TdxSpreadSheetFormulaReferenceToken.Create(
    TdxSpreadSheetReference.Create(ARow, AAbsRow),
    TdxSpreadSheetReference.Create(AColumn, AAbsColumn)));
  Inc(FOffset, 4);
end;
...
begin
  ...
  ALink := TdxSpreadSheet3DReferenceLink.Create(Owner.GetSheet(ATokens^[0]));
  AddToken(TdxSpreadSheetFormula3DReferenceToken.Create(ALink,
    TdxSpreadSheetReference.Create(ARow, AAbsRow),
    TdxSpreadSheetReference.Create(AColumn, AAbsColumn)));
  Inc(FOffset, 6);
end;
...
begin
  Read_Ref3d;
  TdxSpreadSheetFormula3DReferenceToken(Tokens.Last).IsError := True;
end;
...
function TdxXLSFormulaWriter.IsInvalidFormula: Boolean;
   ...
   begin
     Result := (AToken is TdxSpreadSheetFormulaUnknownFunctionToken) or
       (AToken is TdxSpreadSheetFormulaUnknownNameToken);
...
begin
  Writers.Add(TdxSpreadSheetFormula3DAreaReferenceToken, WriteArea3DToken);
  Writers.Add(TdxSpreadSheetFormulaAreaReferenceToken, WriteAreaToken);
  ...
  Writers.Add(TdxSpreadSheetFormulaDefinedNameToken, WriteNameToken);
  ...
  Writers.Add(TdxSpreadSheetFormula3DReferenceToken, WriteRef3DToken);
//  Writers.Add(TdxSpreadSheetFormulaReferenceToken, WriteRefNToken);
  Writers.Add(TdxSpreadSheetFormulaReferenceToken, WriteRefToken);
...
begin
  AData := TdxSpreadSheet3DReferenceLink(TdxSpreadSheetFormula3DAreaReferenceToken(AToken).Link).Data;
  if TdxSpreadSheetFormula3DReferenceToken(AToken).IsError or TdxSpreadSheetInvalidObject.IsInvalid(AData) then
  begin
    AIndex := 0;
  ...
  EncodeReference(TdxSpreadSheetFormulaAreaReferenceTokenAccess(AToken).FRow,
    TdxSpreadSheetFormulaAreaReferenceTokenAccess(AToken).FColumn, ARow, AColumn);
  EncodeReference(TdxSpreadSheetFormulaAreaReferenceTokenAccess(AToken).FRow2,
    TdxSpreadSheetFormulaAreaReferenceTokenAccess(AToken).FColumn2, ARow2, AColumn2);
...
begin
  ...
  EncodeReference(TdxSpreadSheetFormulaAreaReferenceTokenAccess(AToken).FRow,
    TdxSpreadSheetFormulaAreaReferenceTokenAccess(AToken).FColumn, ARow, AColumn);
  if TdxSpreadSheetFormulaAreaReferenceTokenAccess(AToken).FColumn.IsAllItems and (AColumn and $FF = $FF) then
...
begin
  AName := TdxSpreadSheetFormulaDefinedNameToken(AToken).DefinedName;
...
begin
  AIndex := 0;
  ASheet := TdxSpreadSheetTableView(TdxSpreadSheet3DReferenceLink(TdxSpreadSheetFormula3DReferenceToken(AToken).Link).Data);
  if (ASheet <> nil) and not TdxSpreadSheetInvalidObject.IsInvalid(ASheet) then
    AIndex := ASheet.Index;
  Writer.WriteByte(ValidateToken(ATokens[TdxSpreadSheetFormula3DReferenceToken(AToken).IsError or
    TdxSpreadSheetInvalidObject.IsInvalid(ASheet)]));

//  Writer.WriteByte(ptgRef3dA);

  Writer.WriteWord(AIndex);
  EncodeReference(TdxSpreadSheetFormulaReferenceTokenAccess(AToken).FRow,
    TdxSpreadSheetFormulaReferenceTokenAccess(AToken).FColumn, ARow, AColumn);
  Writer.WriteWord(ARow);
  Writer.WriteWord(AColumn);
end;
...
begin
  if TdxSpreadSheetFormulaReferenceTokenAccess(AToken).IsError then
    Writer.WriteByte(ValidateToken(ATokens[TdxSpreadSheetFormulaReferenceTokenAccess(AToken).IsError]))
  else
    Writer.WriteByte(ATokens[TdxSpreadSheetFormulaReferenceTokenAccess(AToken).IsError]);
  EncodeReference(TdxSpreadSheetFormulaReferenceTokenAccess(AToken).FRow,
    TdxSpreadSheetFormulaReferenceTokenAccess(AToken).FColumn, ARow, AColumn);
  Writer.WriteWord(ARow);
  Writer.WriteWord(AColumn);
...
begin
  ...
  begin
    ...
    EncodeReferenceN(TdxSpreadSheetFormulaReferenceTokenAccess(AToken).FRow,
      TdxSpreadSheetFormulaReferenceTokenAccess(AToken).FColumn, ARow, AColumn);
