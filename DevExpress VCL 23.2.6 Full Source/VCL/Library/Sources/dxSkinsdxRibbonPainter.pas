...
type
  ...
  protected
    function CorrectTabGroupHeaderRect(const R: TRect; AIsInPopup: Boolean): TRect;
    function CorrectTabGroupRect(const R: TRect; AIsInPopup: Boolean): TRect;
    function CorrectTabHeaderRect(const R: TRect): TRect;
    function CorrectTabPanelRect(const R: TRect; AIsInPopup: Boolean): TRect;

    function DoGetPartColor(APart: Integer; AState: Integer = 0): TColor; override;
    procedure DrawArrow(DC: HDC; const R: TRect; AArrowDirection: TcxArrowDirection; AColor: TColor); override;
    ...
    function GetBorderSizeForBottom(AHasStatusBar: Boolean): Integer;
    ...
    function GetFormCornerColor(AActive: Boolean; ACorner: TdxCorner): TdxAlphaColor; override;
    function GetName: string; override;
    function GetPropertyColor(AColor: TdxSkinColor): TColor;
    function GetQATBackgroundElement(APlace: TdxQATPosition; AIsRibbonMinimized: Boolean): TdxSkinElement;
    ...
    function DrawElement(DC: HDC; const R: TRect; AState: Integer; AElement: TdxSkinElement; AImageIndex: Integer = 0;
      AUsePixelOffsetModeHalf: Boolean = True): Boolean; overload;

    procedure DrawColoredElement(DC: HDC; const R: TRect;
      AColor: TColor; AElement: TdxSkinElement; AState: TdxSkinElementState);
    procedure DrawFormBorder(DC: HDC; ASide: TcxBorder; const ABorderWidths, R: TRect; ARectangular, AActive: Boolean);
  ...
  public
    ...
    procedure DrawGalleryFilterBandBackground(DC: HDC; const R: TRect; AState: Integer); override;
    ...
    procedure DrawFormBorders(DC: HDC; const ABounds, ABordersWidth: TRect; AIsActive, AIsRectangular: Boolean); override;
    ...
    procedure DrawMenuMark(DC: HDC; const R: TRect; AState: Integer); override;
    ...
    procedure DrawQATButton(DC: HDC; const R: TRect; AState: Integer; APlace: TdxQATPosition); override;
    function GetSmallButtonColorPalette(AState: Integer): IdxColorPalette; override;

    // Status Bar
    procedure DrawStatusBar(DC: HDC; const R: TRect; AIsLeftSideRaised, AIsRightSideRaised: Boolean); override;
    procedure DrawStatusBarCore(DC: HDC; R: TRect; const AClipRect: TRect; AIsRaised: Boolean);
    procedure DrawStatusBarGripBackground(DC: HDC; const AStatusBarBounds, R: TRect); override;
    procedure DrawStatusBarPanel(DC: HDC; const AStatusBarBounds, R: TRect; AIsLowered: Boolean); override;
    ...
    function GetTabPanelHorizontalMargins: TRect; override;
    ...
    procedure DrawQuickAccessToolbar(DC: HDC; const R: TRect;
      APlace: TdxQATPosition; AHasApplicationButton, AIsActive, ADontUseAero, AIsRibbonMinimized: Boolean); override;
    procedure DrawQuickAccessToolbarArrowDown(DC: HDC; const R: TRect; AState: Integer; APlace: TdxQATPosition); override;
    procedure DrawQuickAccessToolbarDefaultGlyph(DC: HDC; const R: TRect; APlace: TdxQATPosition); override;
    procedure DrawQuickAccessToolbarGroupButton(DC: HDC; const R: TRect;
      APlace: TdxQATPosition; AIsActive: Boolean; AState: Integer); override;
    procedure DrawQuickAccessToolbarMarkArrow(DC: HDC; const R: TRect; AState: Integer; APlace: TdxQATPosition); override;
    procedure DrawQuickAccessToolbarPopup(DC: HDC; const R: TRect); override;
    procedure DrawQuickAccessToolbarSmallButtonDropButtonArrowPart(DC: HDC; const R: TRect; AState: Integer; APlace: TdxQATPosition); override;
    procedure DrawQuickAccessToolbarSmallButtonDropButtonMainPart(DC: HDC; const R: TRect; AState: Integer; APlace: TdxQATPosition); override;
    function GetQuickAccessToolbarColorPalette(AState: Integer; APlace: TdxQATPosition): IdxColorPalette; override;
    function GetQuickAccessToolbarLeftIndent(AHasApplicationButton, AUseAeroGlass: Boolean): Integer; override;
    function GetQuickAccessToolbarMarkButtonOffset(AHasApplicationButton: Boolean; ABelow: Boolean): Integer; override;
  ...
  protected
    procedure DrawBackstageViewBorders(DC: HDC; const R, ABordersWidth: TRect; AActive: Boolean); virtual;
  ...
  public
    ...
    procedure DrawBackstageViewMenuBackground(DC: HDC; const R: TRect); override;
    procedure DrawBackstageViewMenuHeader(DC: HDC; const R: TRect); override;
    procedure DrawFormBorders(DC: HDC; const ABounds, BordersWidth: TRect; AIsActive, AIsRectangular: Boolean); override;
    procedure DrawFormCaption(DC: HDC; const R: TRect); override;
    function GetPartContentOffsets(APart: Integer): TRect; override;
  end;
...
uses
  dxSkinsStrs, cxControls, dxShadowWindow, dxTypeHelpers;

const
  dxThisUnitName = 'dxSkinsdxRibbonPainter';
...
begin
  ...
  begin
    if (AElement.Image <> nil) and (AElement.Image.Margins <> nil) then
    begin
      Result := AElement.Image.Margins.Margin;
      if not cxRectIsNull(Result) then
        Exit;
    end;
    ...
    begin
      with AElement.Borders do
        Result.Init(Left.Thin, Top.Thin, Right.Thin, Bottom.Thin);
    end;
...
function DrawSkinElement(AElement: TdxSkinElement; ADC: HDC; const ARect: TRect; AScaleFactor: TdxScaleFactor; AImageIndex: Integer = 0; AState: TdxSkinElementState = esNormal;
...
begin
  ...
  begin
    ...
    try
      ...
      AElement.RightToLeftDependentDraw(cxPaintCanvas.Handle, ADrawRect, AScaleFactor, ARightToLeft, AImageIndex, AState);
    finally
...
begin
  ...
  begin
    ...
    ASize := cxSizeMax(ASize, ScaleFactor.Apply(AElement.MinSize.Size));
  end;
end;
...
function TdxSkinRibbonPainter.CorrectTabGroupRect(const R: TRect; AIsInPopup: Boolean): TRect;
var
  AContentOffset: TcxRect;
begin
  Result := R;
  if AIsInPopup and (SkinInfo.RibbonTabGroup <> nil) then
  begin
    AContentOffset := SkinInfo.RibbonTabGroup.ContentOffset;
    if AContentOffset.Left <> AContentOffset.Right then
    begin
      Dec(Result.Left, AContentOffset.Left - Min(AContentOffset.Left, AContentOffset.Right));
      Inc(Result.Right, AContentOffset.Right - Min(AContentOffset.Left, AContentOffset.Right));
    end;
  end;
end;

function TdxSkinRibbonPainter.CorrectTabGroupHeaderRect(const R: TRect; AIsInPopup: Boolean): TRect;
begin
  Result := CorrectTabPanelRect(CorrectTabGroupRect(R, AIsInPopup), AIsInPopup);
end;
...
function TdxSkinRibbonPainter.CorrectTabPanelRect(const R: TRect; AIsInPopup: Boolean): TRect;
begin
  Result := R;
...
function TdxSkinRibbonPainter.GetBorderSize(ASide: TcxBorder): TSize;
var
  AElement: TdxSkinElement;
begin
  ...
  if AElement = nil then
    ...
    if AElement.MinSize.IsEmpty then
      Result := AElement.Size
    else
      Result := AElement.MinSize.Size
end;

function TdxSkinRibbonPainter.GetBorderSizeForBottom(AHasStatusBar: Boolean): Integer;
begin
...
begin
  ...
  if UseSkins then
  begin
    if AHasAppButton then
      AProperty := SkinInfo.RibbonQATCustomizeButtonOutsizeQAT[qatpInCaption]
    else
      AProperty := SkinInfo.RibbonQATCustomizeButtonOutsizeQAT[qatpAbove];
...
begin
  ...
  begin
    ...
    Result := AElement.FindColor(APropertyStateName + dxRibbonColorSchemeAccentNames[ColorSchemeAccent], EmptyStr);
    if Result = clDefault then
      Result := AElement.FindColor(APropertyStateName, EmptyStr);
    if Result = clDefault then
...
function TdxSkinRibbonPainter.GetFormCornerColor(AActive: Boolean; ACorner: TdxCorner): TdxAlphaColor;
var
  ABorder: TcxBorder;
begin
  ...
  if ABorder = bBottom then
    AActive := AActive or (PaintData <> nil) and PaintData.IsStatusBarVisible;

  Result := dxSkinCalculateBorderColor(GetBorderSkinElement(ABorder, True), ABorder, Ord(not AActive));
end;
...
function TdxSkinRibbonPainter.GetQATBackgroundElement(APlace: TdxQATPosition; AIsRibbonMinimized: Boolean): TdxSkinElement;
begin
  if not UseSkins then
    Result := nil
  else
    if (APlace = qatpBelow) and AIsRibbonMinimized and (SkinInfo.RibbonQuickToolbarSingle <> nil) then
      Result := SkinInfo.RibbonQuickToolbarSingle
    else
      Result := SkinInfo.RibbonQuickToolbar[APlace];
end;
...
const
  QATOffsetDelta = 10;
var
  AQATBackgroundElement: TdxSkinElement;
begin
  if UseSkins then
  begin
    if AHasApplicationButton then
      AQATBackgroundElement := GetQATBackgroundElement(qatpInCaption, False)
    else
      AQATBackgroundElement := GetQATBackgroundElement(qatpAbove, False);
    Result := ScaleFactor.Apply(QATOffsetDelta) + GetApplicationButtonIndent(sdxRibbonAppButtonRightIndent) +
      ScaleFactor.Apply(SkinInfo.GetIntegerPropertyValue(AQATBackgroundElement, sdxRibbonQuickAccessToolbarOffset));
  end
  else
    Result := 0;
end;
...
begin
  if UseSkins and (SkinInfo.ButtonParts[cxbpButton] <> nil) then
    SkinInfo.ButtonParts[cxbpButton].Draw(DC, R, ScaleFactor, 0, ButtonState[AState = DXBAR_HOT])
  else
...
begin
  if not UseSkins or (SkinInfo.RibbonExtraPanePinButtonGlyph = nil) then
    ...
    SkinInfo.RibbonExtraPanePinButtonGlyph.RightToLeftDependentDraw(DC, R, ScaleFactor, UseRightToLeftAlignment, Ord(AChecked));
end;
...
begin
  ...
  begin
    SkinInfo.RibbonBackstageViewBackButton.RightToLeftDependentDraw(DC,
      cxRectCenter(R, ScaleFactor.Apply(SkinInfo.RibbonBackstageViewBackButton.MinSize.Size)),
      ScaleFactor, UseRightToLeftAlignment, 0, RibbonStateToSkinElementState(AState));
  end
  else
...
begin
  if not DrawElement(DC, R, AState, SkinInfo.RibbonBackstageViewMenuButton, GetActualColorSchemeAccentImageIndex,
    AState <> DXBAR_NORMAL) then
    ...
    case AState of
      DXBAR_CHECKED, DXBAR_HOTCHECK, DXBAR_PRESSED:
        ...
        begin
          ...
          SkinInfo.RibbonBackstageViewTabArrow.RightToLeftDependentDraw(
            DC, AArrowRect, dxDefaultScaleFactor, UseRightToLeftAlignment);
        end;
    end;
  end
  else
...
begin
  if UseSkins and (SkinInfo.StandaloneGalleryItem <> nil) then
    SkinInfo.StandaloneGalleryItem.Draw(DC, R, ScaleFactor, 0, RibbonStateToSkinElementState(AState))
  else
...
begin
  ...
  if AElement <> nil then
    AElement.Draw(DC, cxRectInflate(R, AElement.Image.Margins.Left, 0, AElement.Image.Margins.Right, 0), R, ScaleFactor);
end;
...
begin
  ...
  begin
    ...
    SkinInfo.RibbonButtonGroup.Draw(DC, ARect, R, ScaleFactor);
  end
  else
...
begin
  ...
  if Result then
    AElement.RightToLeftDependentDraw(DC, R, ScaleFactor, UseRightToLeftAlignment);
end;
...
begin
  Result := UseSkins and (AElement <> nil);
  if Result then
    AElement.RightToLeftDependentDraw(DC, R, ScaleFactor, UseRightToLeftAlignment, AImageIndex, AState);
end;

function TdxSkinRibbonPainter.DrawElement(DC: HDC; const R: TRect;
  AState: Integer; AElement: TdxSkinElement; AImageIndex: Integer = 0;
  AUsePixelOffsetModeHalf: Boolean = True): Boolean;
begin
  Result := UseSkins and (AElement <> nil);
  if Result then
    AElement.RightToLeftDependentDraw(DC, R, ScaleFactor,
      UseRightToLeftAlignment, AImageIndex, SkinElementCheckState(AElement, AState), AUsePixelOffsetModeHalf);
end;

procedure TdxSkinRibbonPainter.DrawProgressDiscreteBand(DC: HDC; const R: TRect);
var
  ABackgroundElement: TdxSkinElement;
  AProgressElement: TdxSkinElement;
  ARect: TRect;
begin
  if UseSkins then
    AProgressElement := SkinInfo.ProgressBarParts[False, pbpProgressChunk]
  else
    AProgressElement := nil;

  if AProgressElement <> nil then
  begin
    ARect := R;

    ABackgroundElement := SkinInfo.ProgressBarParts[False, pbpBackground];
    if ABackgroundElement <> nil then
    begin
      InflateRect(ARect, 0, 2);
      Dec(ARect.Bottom, ABackgroundElement.ContentOffset.Bottom);
      Inc(ARect.Top, ABackgroundElement.ContentOffset.Top);
    end;

    Inc(ARect.Right, AProgressElement.Image.Margins.Margin.Right);
    Dec(ARect.Left, AProgressElement.Image.Margins.Margin.Left);
    AProgressElement.Draw(DC, ARect, R, ScaleFactor);
  end
  else
...
begin
  ...
  begin
    ABackgroundElement := SkinInfo.ProgressBarParts[False, pbpBackground];
    if ABackgroundElement <> nil then
      ARect := cxRectContent(cxRectInflate(ARect, 2), ABackgroundElement.ContentOffset.Rect)
    else
      ARect := R;
...
begin
  if not DrawElement(DC, R, SkinInfo.ProgressBarParts[False, pbpBackground]) then
...
begin
  if not DrawElement(DC, R, SkinInfo.DropDownGalleryBackground) then
...
var
  ARect: TRect;
begin
  if UseSkins and (SkinInfo.DropDownGallerySizeGrips <> nil) then
  begin
    ARect := cxRectInflate(R, 0, -3, -2, -1);
    ARect.Left := ARect.Right - ARect.Height;
    if UseRightToLeftAlignment then
      ARect := TdxRightToLeftLayoutConverter.ConvertRect(ARect, R);
    SkinInfo.DropDownGallerySizeGrips.RightToLeftDependentDraw(DC, ARect, ScaleFactor, UseRightToLeftAlignment, 1);
  end
  else
...
begin
  if not DrawElement(DC, R, SkinInfo.DropDownGallerySizingPanel) then
...
begin
  if not DrawElement(DC, R, SkinInfo.DropDownGallerySizeGrips) then
...
begin
  ...
  begin
    ...
    if UseRightToLeftAlignment then
      ARect := TdxRightToLeftLayoutConverter.ConvertRect(ARect, R);
    SkinInfo.DropDownGallerySizeGrips.RightToLeftDependentDraw(DC, ARect, ScaleFactor, UseRightToLeftAlignment, 2);
  end
  else
...
procedure TdxSkinRibbonPainter.DrawGalleryFilterBandBackground(DC: HDC; const R: TRect; AState: Integer);
begin
  if not DrawElement(DC, R, AState, SkinInfo.DropDownGalleryFilterPanel) then
...
begin
  if not DrawElement(DC, R, SkinInfo.DropDownGalleryGroupHeader) then
...
begin
  ...
  begin
    ...
    try
      ...
      DrawElement(DC, ARect, AState, SkinInfo.StandaloneGalleryItem);
    finally
      cxPaintCanvas.EndPaint;
    end;
  end
...
begin
  ...
  begin
    ADrawRect := R;
    AElement.Draw(DC, ADrawRect, R, ScaleFactor, 0, RibbonStateToSkinElementState(AState));
  end
  else
...
begin
  ...
  if AElement <> nil then
    AElement.RightToLeftDependentDraw(DC, R, ScaleFactor, UseRightToLeftAlignment, 0, RibbonStateToSkinElementState(AState))
  else
...
begin
  ...
  begin
    ADrawRect := R;
    Dec(ADrawRect.Left, Max(AElement.Size.cx, AElement.Image.Margins.Left));
    AElement.RightToLeftDependentDraw(DC, R, ScaleFactor, UseRightToLeftAlignment, 0, RibbonStateToSkinElementState(AState));
  end
  else
...
procedure TdxSkinRibbonPainter.DrawFormBorders(DC: HDC; const ABounds, ABordersWidth: TRect; AIsActive, AIsRectangular: Boolean);
  ...
  var
    ...
    AElement: TdxSkinElement;
  begin
    AElement := GetBorderSkinElement(ASide, AIsRectangular);
    ...
    begin
      ...
      try
        ...
        AElement.Draw(ABitmap.Canvas.Handle, R, ScaleFactor, Integer(not AIsActive));
  ...
  procedure DrawBorder(ASide: TcxBorder; const ABounds, ABordersWidth: TRect);
  begin
    if (ASide = bBottom) and PaintData.IsStatusBarVisible then
      DrawStatusBarFormBorder(DC, ABordersWidth)
    else
      DrawFormBorder(DC, ASide, ABordersWidth, ABounds, AIsRectangular, AIsActive);
  end;

  procedure DrawBorders(const ABounds, ABordersWidth: TRect);
  var
    ASide: TcxBorder;
    R: TRect;
  begin
    for ASide := Low(TcxBorder) to High(TcxBorder) do
      DrawBorder(ASide, ABounds, ABordersWidth);
...
begin
  ...
  begin
    cxPaintCanvas.BeginPaint(DC);
    try
      cxPaintCanvas.ExcludeClipRect(cxRectContent(ABounds, ABordersWidth));
...
procedure TdxSkinRibbonPainter.DrawFormBorder(DC: HDC; ASide: TcxBorder; const ABorderWidths, R: TRect; ARectangular, AActive: Boolean);
...
var
  ...
  ASaveIndex: Integer;
begin
  AElement := GetBorderSkinElement(ASide, ARectangular);
  if (AElement <> nil) and not IsRectEmpty(R) then
  begin
    if UseRightToLeftAlignment then
      ASide := TdxRightToLeftLayoutConverter.ConvertBorder(ASide);

    ASaveIndex := SaveDC(DC);
    try
      ABorderRect := GetBorderBounds(ASide, ABorderWidths, R);
      if cxIntersectClipRect(DC, ABorderRect) then
      begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        R1 := cxRectSetTop(AData.GetBounds, AData.GetBounds.Bottom - ABorders.Bottom - AOffset, ABorders.Bottom);
        R2 := R1;
        Dec(R2.Top, SkinInfo.RibbonStatusBarBackground.Size.cy - ABorders.Bottom);
        if cxRectIntersect(AActualClipRect, R1, R2) then
        begin
          ASaveIndex := SaveDC(DC);
          try
            if cxIntersectClipRect(DC, AActualClipRect) then
              SkinInfo.RibbonStatusBarBackground.RightToLeftDependentDraw(DC, R2, ScaleFactor,
    ...
    DrawFormBorder(DC, bBottom,
      TRect.Create(ABorders.Left, 0, ABorders.Right, AOffset),
      AData.GetBounds, IsRectangularFormBottom(AData), True);
  end;
end;

procedure TdxSkinRibbonPainter.DrawStatusBarGripBackground(DC: HDC; const AStatusBarBounds, R: TRect);
begin
  DrawStatusBarCore(DC, AStatusBarBounds, R, True);
end;
...
const
  RibbonIconStateToSkinElementState: array[TdxRibbonBorderIconState] of TdxSkinElementState =
    (esNormal, esHot, esPressed, esActiveDisabled, esHot);
var
  AElement: TdxSkinElement;
  ARect: TRect;
begin
  ...
  if AElement <> nil then
  begin
    if not SkinInfo.IsAdvanced then
      ARect := cxRectInflate(ARect, -ScaleFactor.Apply(1));
    AElement.Draw(DC, ARect, ScaleFactor,
      Ord((PaintData.GetApplicationMenuState = ramsShownAsFullScreenFrame) and UseSkins),
      RibbonIconStateToSkinElementState[AState]);
  end
  else
    inherited DrawFormBorderIcon(DC, ARect, AIcon, AState);
end;
...
begin
  ...
  begin
    ...
    AImageIndex := Ord(not AIsActive);
    AState := StateMap[AIsActive];
    if FormPaintData.UseNavigationControlCompatibleBorders then
      DrawSkinElement(AElement, DC, R, ScaleFactor, AImageIndex, AState, True, UseRightToLeftAlignment)
    else
...
begin
  ...
  begin
    ...
    SkinInfo.RibbonTabPanelGroupButton.RightToLeftDependentDraw(
      DC, ARect, ScaleFactor, UseRightToLeftAlignment, 0, StateMap[AState]);
  end
  else
...
begin
  ...
  begin
    ...
    SkinInfo.PopupMenuSideStrip.Draw(DC, R1, R, ScaleFactor);
  end
  else
...
procedure TdxSkinRibbonPainter.DrawMenuMark(DC: HDC; const R: TRect; AState: Integer);
begin
  if SkinInfo.IsAdvanced then
    DrawMenuItem(DC, R, AState)
  else
    DrawSmallButton(DC, R, AState);

  if not DrawElement(DC, R, AState, SkinInfo.PopupMenuExpandButton) then
    inherited DrawMenuMark(DC, R, AState);
end;
...
begin
  if UseSkins and (SkinInfo.RibbonFormCaption <> nil) then
    DrawSkinElement(SkinInfo.RibbonFormCaption, DC, R, ScaleFactor, 0, esNormal, True, UseRightToLeftAlignment)
  else
...
procedure TdxSkinRibbonPainter.DrawQATButton(DC: HDC; const R: TRect; AState: Integer;
  APlace: TdxQATPosition);
var
  AElement: TdxSkinElement;
begin
  if UseSkins then
  begin
    AElement := SkinInfo.RibbonQuickToolbarButton[APlace];
    if AElement = nil then
      AElement := SkinInfo.RibbonSmallButton;
  end
  else
    AElement := nil;
  if AElement = nil then
    inherited DrawQATButton(DC, R, AState, APlace)
  else
  begin
    AElement.UseCache := True;
    AElement.Draw(DC, R, ScaleFactor, 0, dxSkinElementCheckState(AElement, RibbonStateToSkinElementState(AState)));
...
procedure TdxSkinRibbonPainter.DrawStatusBar(DC: HDC; const R: TRect; AIsLeftSideRaised, AIsRightSideRaised: Boolean);
var
  ABuffer: TcxBitmap;
  APrevLayout: Integer;
begin
  if UseSkins and (SkinInfo.RibbonStatusBarBackground <> nil) then
  begin
    APrevLayout := SetLayout(DC, LAYOUT_LTR);
    try
      DrawStatusBarCore(DC, R, R, False);
      if UseRightToLeftAlignment then
        ExchangeBooleans(AIsLeftSideRaised, AIsRightSideRaised);
      if AIsRightSideRaised then
        DrawStatusBarCore(DC, R, cxRectSetRight(R, R.Right, GetBorderSize(bRight).cx), True);
      if AIsLeftSideRaised then
      begin
        ABuffer := TcxBitmap.CreateSize(GetBorderSize(bLeft).cx, R.Height);
        try
          DrawStatusBarCore(ABuffer.cxCanvas.Handle, cxRectOffset(R, ABuffer.Width - R.Width, -R.Top), ABuffer.ClientRect, True);
          ABuffer.Flip(True, False);
          cxBitBlt(DC, ABuffer.cxCanvas.Handle, cxRectSetWidth(R, ABuffer.Width), cxNullPoint, SRCCOPY);
        finally
          ABuffer.Free;
        end;
      end;
    finally
      SetLayout(DC, APrevLayout);
    end;
  end
  else
    inherited;
...
procedure TdxSkinRibbonPainter.DrawStatusBarCore(DC: HDC; R: TRect; const AClipRect: TRect; AIsRaised: Boolean);
var
  ABorders: TRect;
  AElement: TdxSkinElement;
  AFormBounds: TRect;
...
begin
  ...
  if AElement <> nil then
  begin
    Inc(R.Bottom, GetBorderSizeForBottom(True));
    if AIsRaised and AElement.IsAlphaUsed then
      AElement.Draw(DC, R, AClipRect, ScaleFactor);
    AElement.Draw(DC, R, AClipRect, ScaleFactor, Ord(AIsRaised));

    if not UseRoundedWindowCorners or IsRectangularFormBottom(FormPaintData) then
    begin
      ASaveIndex := SaveDC(DC);
      try
        ...
        begin
          ...
          AFormBounds.Init(R.Left, R.Top, R.Right, R.Bottom + 1);
          DrawFormBorder(DC, bLeft, ABorders, AFormBounds, True, True);
          DrawFormBorder(DC, bRight, ABorders, AFormBounds, True, True);
        end;
      finally
        RestoreDC(DC, ASaveIndex);
      end;
...
begin
  ...
  begin
    ...
    SkinInfo.SizeGrip.RightToLeftDependentDraw(DC, R1, ScaleFactor, UseRightToLeftAlignment);
  end
  else
    inherited DrawStatusBarSizeGrip(DC, R);
end;

procedure TdxSkinRibbonPainter.DrawStatusBarPanel(DC: HDC; const AStatusBarBounds, R: TRect; AIsLowered: Boolean);
begin
  DrawStatusBarCore(DC, AStatusBarBounds, R, not AIsLowered);
end;
...
begin
  ...
  begin
    R1 := R;
    Inc(R1.Bottom, GetBorderSizeForBottom(True));
...
begin
  ...
  begin
    AElement.UseCache := True;
    AElement.RightToLeftDependentDraw(DC, CorrectTabGroupRect(R, AIsInPopup),
      ScaleFactor, UseRightToLeftAlignment, 0, StateMap[AState = DXBAR_HOT])
  end;
end;
...
begin
  ...
  begin
    SkinInfo.RibbonTabGroupHeader.RightToLeftDependentDraw(DC,
      CorrectTabGroupHeaderRect(R, AIsInPopup), ScaleFactor,
      UseRightToLeftAlignment, 0, StateMap[AState = DXBAR_HOT]);
  end
  else
...
var
  AElement: TdxSkinElement;
begin
  if AIsQATAtBottom and (not AIsInPopup) and (SkinInfo.RibbonTabPanelWithQATBelow <> nil) then
    AElement := SkinInfo.RibbonTabPanelWithQATBelow
  else
    AElement := SkinInfo.RibbonTabPanel;
  if UseSkins and (AElement <> nil) then
  begin
    AElement.UseCache := True;
    AElement.Draw(DC, CorrectTabPanelRect(R, AIsInPopup), ScaleFactor);
  end
  else
...
function TdxSkinRibbonPainter.GetTabPanelHorizontalMargins: TRect;
begin
  if SkinInfo.RibbonTabPanelHorizontalMargin <> nil then
    Result := SkinInfo.RibbonTabPanelHorizontalMargin.Value.Rect
  else
    Result := inherited GetTabPanelHorizontalMargins;
end;
...
begin
  if not DrawElement(DC, R, SkinInfo.RibbonQuickToolbarGlyphDefault) then
    inherited;
end;
...
var
  AElement: TdxSkinElement;
begin
  AElement := SkinInfo.RibbonQuickToolbarGlyphDefault;
...
procedure TdxSkinRibbonPainter.DrawQuickAccessToolbar(DC: HDC; const R: TRect;
  APlace: TdxQATPosition; AHasApplicationButton, AIsActive, ADontUseAero, AIsRibbonMinimized: Boolean);

  function IsAeroBackgroundUsed: Boolean;
  begin
    Result := (APlace = qatpInCaption) and IsCompositionEnabled and not (ADontUseAero or (APlace = qatpBelow));
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Dec(ARightIndent, GetQuickAccessToolbarMarkButtonOffset(AHasApplicationButton, APlace = qatpBelow));
      end;
...
var
  ...
  ARect: TRect;
begin
  if (APlace = qatpInRibbon) and SkinInfo.IsAdvanced then
    Exit;
  if APlace = qatpBelow then
    AElement := GetQATBackgroundElement(qatpBelow, AIsRibbonMinimized)
  else
    if AHasApplicationButton then
      AElement := GetQATBackgroundElement(qatpInCaption, AIsRibbonMinimized)
    else
      AElement := GetQATBackgroundElement(qatpAbove, AIsRibbonMinimized);
  if NeedDrawCustomQuickAccessToolBar(AElement) then
    inherited DrawQuickAccessToolbar(DC, R, APlace, AHasApplicationButton, AIsActive, ADontUseAero, AIsRibbonMinimized)
  else
    if not IsAeroBackgroundUsed then
    begin
      ARect := ValidateQATRect(R, SkinInfo.RibbonFormCaption);
      if (APlace = qatpBelow) or (ARect.Width >= ARect.Height) then
        DrawElement(DC, ARect, AElement);
    end;
end;

procedure TdxSkinRibbonPainter.DrawQuickAccessToolbarArrowDown(DC: HDC; const R: TRect; AState: Integer;
  APlace: TdxQATPosition);
const
  StateMap: array[0..8] of TdxSkinElementState =
    (esNormal, esDisabled, esHot, esActive, esNormal, esNormal, esNormal, esNormal, esNormal);
var
  AElement: TdxSkinElement;
begin
  if UseSkins then
  begin
    AElement := SkinInfo.RibbonQuickToolbarButtonArrow[APlace];
    if AElement = nil then
      AElement := SkinInfo.RibbonButtonArrow;
    if AElement <> nil then
      AElement.Draw(DC, R, ScaleFactor, 0, StateMap[AState])
    else
      inherited DrawQuickAccessToolbarArrowDown(DC, R, AState, APlace);
  end
  else
    inherited DrawQuickAccessToolbarArrowDown(DC, R, AState, APlace);
end;

procedure TdxSkinRibbonPainter.DrawQuickAccessToolbarDefaultGlyph(DC: HDC; const R: TRect; APlace: TdxQATPosition);
var
  AElement: TdxSkinElement;
begin
  AElement := SkinInfo.RibbonQuickToolbarButtonGlyph[APlace];
  if AElement = nil then
    AElement := SkinInfo.RibbonQuickToolbarButtonGlyphDefault;
  if not DrawElement(DC, R, AElement) then
    inherited DrawQuickAccessToolbarDefaultGlyph(DC, R, APlace)
end;

procedure TdxSkinRibbonPainter.DrawQuickAccessToolbarGroupButton(
  DC: HDC; const R: TRect; APlace: TdxQATPosition; AIsActive: Boolean; AState: Integer);
var
...
begin
  ...
  if (AElement = nil) or (ABackground = nil) then
    inherited DrawQuickAccessToolbarGroupButton(DC, R, APlace, AIsActive, AState)
  else
  begin
...
procedure TdxSkinRibbonPainter.DrawQuickAccessToolbarMarkArrow(DC: HDC; const R: TRect; AState: Integer;
  APlace: TdxQATPosition);
var
  AElement: TdxSkinElement;
begin
  AElement := SkinInfo.RibbonQuickToolbarGlyph[APlace];
  if AElement = nil then
      AElement := SkinInfo.RibbonQuickToolbarGlyphDefault;
  if not DrawElement(DC, R, AElement) then
    inherited DrawQuickAccessToolbarMarkArrow(DC, R, AState, APlace);
end;
...
procedure TdxSkinRibbonPainter.DrawQuickAccessToolbarSmallButtonDropButtonArrowPart(DC: HDC; const R: TRect;
  AState: Integer; APlace: TdxQATPosition);
begin
  if not DrawElement(DC, R, AState, SkinInfo.RibbonQuickToolbarButton[APlace]) then
    inherited DrawQuickAccessToolbarSmallButtonDropButtonArrowPart(DC, R, AState, APlace)
end;

procedure TdxSkinRibbonPainter.DrawQuickAccessToolbarSmallButtonDropButtonMainPart(DC: HDC; const R: TRect;
  AState: Integer; APlace: TdxQATPosition);
begin
  if not DrawElement(DC, R, AState, SkinInfo.RibbonQuickToolbarButton[APlace]) then
    inherited DrawQuickAccessToolbarSmallButtonDropButtonMainPart(DC, R, AState, APlace)
end;

function TdxSkinRibbonPainter.GetQuickAccessToolbarColorPalette(AState: Integer; APlace: TdxQATPosition): IdxColorPalette;
var
  AElement: TdxSkinElement;
begin
  AElement := nil;
  ...
  begin
    ...
    if AElement = nil then 
      if APlace = qatpBelow then
        AElement := SkinInfo.RibbonQuickToolbar[qatpBelow]
      else
        AElement := SkinInfo.RibbonQuickToolbar[qatpAbove];
  end;
  Result := GetColorPalette(AElement, AState);
  if Result = nil then
    Result := GetSmallButtonColorPalette(AState);
end;
...
procedure TdxSkinRibbonPainter.DrawArrow(DC: HDC; const R: TRect; AArrowDirection: TcxArrowDirection; AColor: TColor);
var
  AElement: TdxSkinElement;
  ARect: TRect;
  APadding: TdxPadding;
...
begin
  AElement := nil;
  AWidth := 0;
  if AArrowDirection in [adRight, adLeft] then
  begin
    AElement := SkinInfo.PopupMenuLinkArrow;
    if (AElement <> nil) and (not AElement.Glyph.Empty) then
    begin
      APadding := AElement.ContentOffset.Rect;
      AWidth := AElement.Glyph.Size.cx + APadding.Width;
    end;
  end
  else
  if AArrowDirection = adDown then
  begin
    AElement := SkinInfo.BarSubItemArrow;
    if (AElement <> nil) and (not AElement.Image.Empty) then
    begin
      APadding := AElement.ContentOffset.Rect;
      AWidth := AElement.Image.Size.cx + APadding.Width;
    end;
  end;

  if AWidth > 0 then
  begin
    ARect := TRect.CreateSize(R.Left - AWidth div 2, R.Top - AWidth div 2, AWidth, AWidth);
    AElement.RightToLeftDependentDraw(DC, ARect, ScaleFactor, AArrowDirection = adLeft, 0, esNormal)
  end
  else
  inherited
    DrawArrow(DC, R, AArrowDirection, AColor);
end;
  ...
  if UseSkins then
    case APart of
      ...
      DXBAR_BACKSTAGEVIEW_GALLERYCONTROL_GROUPHEADER_TEXTCOLOR:
        Result := GetElementTextColor(SkinInfo.StandaloneGalleryGroupHeader, AState);
      DXBAR_BACKSTAGEVIEW_GALLERYCONTROL_ITEMCAPTIONTEXTCOLOR:
        Result := GetElementTextColor(SkinInfo.StandaloneGalleryItem, AState);
      DXBAR_BACKSTAGEVIEW_GALLERYCONTROL_ITEMDESCRIPTIONTEXTCOLOR:
        Result := GetElementTextColor(SkinInfo.StandaloneGalleryItem, AState, sdxDescriptionTextColorPrefix);
      ...
      DXBAR_DROPDOWNGALLERYITEM_TEXTCOLOR:
        begin
          if AState = DXBAR_DISABLED then
            Result := SkinInfo.RibbonButtonDisabledTextColor
          else
            if AState <> DXBAR_NORMAL then
              Result := GetElementTextColor(SkinInfo.RibbonSmallButton, AState);
          if Result = clDefault then
            Result := GetElementTextColor(SkinInfo.DropDownGalleryBackground, AState);
        end;
      ...
      DXBAR_APPLICATIONMENUBUTTON:
        Result := GetElementTextColor(SkinInfo.ButtonParts[cxbpButton], AState);

      DXBAR_MENUITEMTEXT, DXBAR_MENUBUTTONITEMTEXT, DXBAR_MENUARROWRIGHT:
        if AState = DXBAR_DISABLED then
      ...
      DXBAR_MENUARROWSEPARATOR:
        Result := cxGetActualColor(SkinInfo.PopupMenuSplitButton2.TextColor, clNone);
      ...
      DXBAR_GALLERYGROUPHEADERTEXT, DXBAR_GALLERYFILTERBANDTEXT:
        Result := GetElementTextColor(SkinInfo.DropDownGalleryGroupHeader, AState);

      DXRIBBON_TAT_SMALLBUTTON:
        if not (AState in [DXBAR_NORMAL, DXBAR_DISABLED, DXBAR_ACTIVE]) then
          Result := GetElementTextColor(SkinInfo.RibbonSmallButton, AState);
    end;
  ...
  begin
    case APart of
      ...
      DXBAR_BACKSTAGEVIEW_GALLERYCONTROL_GROUPHEADER:
        AElement := SkinInfo.DropDownGalleryGroupHeader;
      DXBAR_APPLICATIONMENUBUTTON:
...
function TdxSkinRibbonPainter.GetPartSize(APart: Integer): Integer;
  ...
  function GetMarkArrowSize(AElement: TdxSkinElement): Integer;
  begin
    if (AElement <> nil) and (not AElement.Glyph.Empty) then
      Result := ScaleFactor.Apply(AElement.ContentOffset.Rect.Top + AElement.ContentOffset.Rect.Bottom + AElement.Glyph.Size.cy)
    else
      Result := 0;
  end;
...
begin
  ...
  begin
    case APart of
      rfspRibbonFormCaption:
        Result := GetElementSize(SkinInfo.RibbonFormCaption).cy;
      ...
      DXBAR_SEPARATOR_LINE:
        Result := 0;
      DXBAR_TOOLBAR, DXBAR_TOOLBARINPOPUP:
        Result := ScaleFactor.Apply(GetValue(SkinInfo.RibbonSpaceBetweenTabGroups));
      ...
      DXBAR_MARKARROW:
        Result := GetMarkArrowSize(SkinInfo.PopupMenuExpandButton);
    end;
  end;
end;
...
var
  ARibbonQuickToolbar: TdxSkinElement;
begin
  if not AUseAeroGlass and UseSkins then
  begin
    Result := -QATLeftDefaultOffset;
    if AHasApplicationButton then
      ARibbonQuickToolbar := SkinInfo.RibbonQuickToolbar[qatpInCaption]
    else
      ARibbonQuickToolbar := SkinInfo.RibbonQuickToolbar[qatpAbove];
    if ARibbonQuickToolbar <> nil then
      Inc(Result, ScaleFactor.Apply(ARibbonQuickToolbar.ContentOffset.Left));
...
begin
  ...
  begin
    if AHasApplicationButton then
      AOffsetProperty := SkinInfo.RibbonQATIndentBeforeCustomizeButton[qatpInCaption]
    else
      AOffsetProperty := SkinInfo.RibbonQATIndentBeforeCustomizeButton[qatpAbove];
  end
  else
    AOffsetProperty := nil;
...
begin
  if AHasApplicationButton then
    ...
    ARibbonQuickToolbar := SkinInfo.RibbonQuickToolbar[qatpAbove];
  if UseSkins and (ARibbonQuickToolbar <> nil) then
    Result := ScaleFactor.Apply(ARibbonQuickToolbar.ContentOffset.Right)
  else
...
begin
  ...
  begin
    ...
    Result.Bottom := GetBorderSizeForBottom(AHasStatusBar);
  end
  else
...
begin
  if UseSkins and (APart = DXBAR_APPLICATIONBUTTON) then
    Result.Init(GetApplicationButtonIndent(sdxRibbonAppButtonLeftIndent) + 1,
...
begin
  R1 := R;
  Dec(R1.Left, GetBorderSize(bLeft).cx);
  if UseRightToLeftAlignment then
...
begin
  R1 := R;
  Dec(R1.Left, GetBorderSize(bLeft).cx);
  Inc(R1.Top, GetBorderSizeForBottom(False) - 1); 
  if UseRightToLeftAlignment then
    R1 := TdxRightToLeftLayoutConverter.ConvertRect(R1, R);
...
procedure TdxSkinRibbon2013Painter.DrawBackstageViewBorders(DC: HDC; const R, ABordersWidth: TRect; AActive: Boolean);
var
  ASaveIndex: Integer;
  AThinBorders: TRect;
begin
  ...
  try
    ...
    FrameRectByColor(DC, R, GetBackstageViewBorderColor(AActive));
    FillRectByColor(DC, cxRectContent(R, AThinBorders), SkinInfo.RibbonBackstageView.Color);
    if UseRoundedWindowCorners then
      FWindowCornersPainter.Draw(DC, R, dxAllCorners, AActive, 4);
  finally
    RestoreDC(DC, ASaveIndex);
  end;
end;

procedure TdxSkinRibbon2013Painter.DrawFormBorders(DC: HDC;
  const ABounds, BordersWidth: TRect; AIsActive, AIsRectangular: Boolean);
var
  ABordersWidth: TRect;
  ASaveIndex: Integer;
begin
  ...
  begin
    ...
    try
      ...
      cxExcludeClipRect(DC, cxRectContent(ABounds, ABordersWidth));
      ABordersWidth.Top := PaintData.GetCaptionHeight;
      DrawBackstageViewBorders(DC, ABounds, ABordersWidth, AIsActive);
    finally
      RestoreDC(DC, ASaveIndex);
    end;
  end
...
begin
  if UseSkins and (PaintData.GetApplicationMenuState = ramsShownAsFullScreenFrame) then
    DrawBackstageViewBorders(DC, R, TRect.Create(0, R.Height, 0, 0), FormPaintData.GetIsActive)
  else
    inherited DrawFormCaption(DC, R);
end;
...
function TdxSkinRibbon2013Painter.GetBackstageViewBorderColor(AIsActive: Boolean): TColor;
  ...
  var
    ...
    AState: TdxSkinElementState;
  begin
    AState := RibbonFormBorderStates[AIsActive];
    AElement := SkinInfo.RibbonFormBottom[True];
    AColor1 := dxAlphaColorToColor(dxSkinCalculateBorderColor(AElement, bTop, 0, AState));
    AColor2 := dxAlphaColorToColor(dxSkinCalculateBorderColor(AElement, bBottom, 0, AState));
...
function TdxSkinRibbon2013Painter.GetFormCornerColor(AActive: Boolean; ACorner: TdxCorner): TdxAlphaColor;
begin
  if UseSkins and (PaintData.GetApplicationMenuState = ramsShownAsFullScreenFrame) then
    Result := dxColorToAlphaColor(GetBackstageViewBorderColor(AActive))
  else
    Result := inherited;
end;
...
begin
  ...
  begin
    ...
    SkinInfo.RibbonBackstageViewBackButton.RightToLeftDependentDraw(DC, AGlyphRect,
      ScaleFactor, UseRightToLeftAlignment, 0, RibbonStateToSkinElementState(AState));
  end
  else
    inherited;
...
begin
  if (AElement <> Element) or (ContextColor <> AContextColor) or (ImageCount <> AElement.ImageCount) or
    (R.Width <> Width) or (R.Height <> Height) then
  begin
    SetSize(R);
    Element := AElement;
...
procedure UnitInit;
begin
  TdxSkinsRibbonPainterManager.Register;
end;

procedure UnitDone;
begin
  TdxSkinsRibbonPainterManager.Unregister;
end;


initialization

  dxUnitsLoader.AddUnit(SysInit.HInstance, dxThisUnitName, UnitInit, UnitDone);

finalization

  dxUnitsLoader.RemoveUnit(SysInit.HInstance, dxThisUnitName, UnitDone);
