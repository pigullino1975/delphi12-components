...
uses
  ...
  ComCtrls, Dialogs, CommCtrl, Forms, Menus, ImgList, TypInfo, Registry, Themes, MultiMon,
  IniFiles, UITypes,
...
procedure dxLoadGlyphFromResource(ASmartImage: TdxSmartImage; const AResName: string; AResType: PChar);
...
{$IFDEF USEJPEGIMAGE}
  Jpeg,
{$ENDIF}
  dxTypeInfo, dxPSRes, dxPSImgs, cxShellControls, dxDPIAwareUtils, cxEdit, dxPrintUtils, cxDrawTextUtils, dxMessageDialog;

const
  dxThisUnitName = 'dxPSUtl';
...
function dxIsTrueTypeFont(AFont: TFont): Boolean;
  ...
  var
    DC: HDC;
    ASearchBuffer: TSearchBuffer;
  begin
    DC := GetDC(0);
    try
      try
        ASearchBuffer.FontName := AFont.Name;
        EnumFontFamilies(DC, nil, @EnumFontsProc, LPARAM(@ASearchBuffer));
        Result := ASearchBuffer.IsTrueType;
      except
        Result := False;
      end;
    finally
...
function DecodePageIndexes(const Source: string; out AOutput: TdxPSPageIndexes): Boolean;
var
  ABuffer: TIntegerDynArray;
  ABufferLength: Integer;
begin
  Result := dxDecodePageIndexes(Source, ABuffer);
  ABufferLength := Length(ABuffer);
  SetLength(AOutput, ABufferLength);
  cxCopyData(@ABuffer[0], @AOutput[0], ABufferLength * SizeOf(Integer));
  SetLength(ABuffer, 0);
end;
...
var
  ...
  ABufferLength: Integer;
begin
  ABufferLength := Length(ASource);
  SetLength(ABuffer, ABufferLength);
  cxCopyData(@ASource[0], @ABuffer[0], ABufferLength * SizeOf(Integer));
  Result := dxEncodePageIndexes(ABuffer);
  SetLength(ABuffer, 0);
end;
...
begin
  Result := Source;
  if Pos(cxEndEllipsis, Result) <> (Length(Result) - Length(cxEndEllipsis) + 1) then
...
procedure MessageError(const AMessage: string);
begin
  dxMessageDlg(AMessage, mtError, [mbOK], 0);
end;

procedure MessageWarning(const AMessage: string);
begin
  dxMessageDlg(AMessage, mtWarning, [mbOK], 0);
end;

function MessageQuestion(const AMessage: string): Boolean;
begin
  Result := dxMessageDlg(AMessage, mtConfirmation, [mbYes, mbNo], 0) = mrYes;
end;
...
begin
  Result := TdxTypeInfo.HasProperty(AClass, AName, ATypeKinds);
end;
...
procedure dxLoadGlyphFromResource(ASmartImage: TdxSmartImage; const AResName: string; AResType: PChar);
begin
  ASmartImage.LoadFromResource(hInstance, AResName, AResType);
end;
procedure dxLoadIconFromResource(AIcon: TIcon; const AResName: string);
var
  ABitmap: TBitmap;
...
begin
  ...
  try
    ...
    AImage.Style.BorderStyle := ebsNone;
    AImage.Style.Edges := [];
...
begin
  ...
  P := Pointer(AData);
  if not dxCanReadMemory(P, SizeOf(Pointer)) then Exit;

  P := PPointer(P)^;
  if not dxCanReadMemory(P, SizeOf(Pointer)) then Exit;

  SelfPtr := Pointer(NativeInt(P) + vmtSelfPtr);
  if not dxCanReadMemory(SelfPtr, SizeOf(Pointer)) then Exit;
...
initialization
  ...
  dxUnitsLoader.AddUnit(SysInit.HInstance, dxThisUnitName, InitializeUnit, nil);
