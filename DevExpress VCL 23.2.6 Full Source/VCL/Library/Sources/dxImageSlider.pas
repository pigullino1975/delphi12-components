...
uses
  ...
  cxGeometry, cxScrollBar, dxScrollAnimation, Generics.Defaults, Generics.Collections, Math;
...
type
  ...
  {$REGION 'private types'}
    ...
    TPreviewGestureHelper = class(TcxIUnknownObject, IdxGestureClient, IdxGestureClient2)
    private
      FSlider: TdxCustomImageSlider;
    public
      constructor Create(AOwner: TdxCustomImageSlider);
    ...
    TPreviewScrollAnimationHelper = class(TdxScrollAnimationHelper)
    private
      FOwner: TdxCustomImageSlider;
    protected
  ...
  private
    ...
    FScrollAnimationHelper: TPreviewScrollAnimationHelper;
    ...
    procedure CheckPreviewScrollPosition(var AValue: Integer);
    function CheckSplitterHitTest(X, Y: Integer): Boolean;
    procedure DoSetPreviewScrollPosition(AValue: Integer);
...
uses
  Types, cxLookAndFeelPainters, cxSplitter, dxDPIAwareUtils, dxCoreGraphics;

const
  dxThisUnitName = 'dxImageSlider';
...
begin
  Result := IndentsWidth + TdxTextMeasurer.TextLineHeight(ImageSlider.Font) + ImageSlider.ScaleFactor.Apply(cxTextOffset);
end;
...
begin
  ...
  FScrollAnimationHelper := TPreviewScrollAnimationHelper.Create(Self);
...
begin
  ...
  begin
    ...
    try
      ...
      TdxImageDrawer.DrawImage(ACanvas, ABounds, MultiFrameImage, nil, -1, AFitMode, idmNormal, True, nil, ScaleFactor,
...
var
  R: TRect;
begin
  R := cxRectSetBottom(AViewInfo.Bounds, AViewInfo.Bounds.Bottom, TdxTextMeasurer.TextLineHeight(Font) + ScaleFactor.Apply(cxTextOffset));
...
procedure TdxCustomImageSlider.DrawOpaqueBackground(ACanvas: TcxCanvas; const ARect: TRect);
begin
  ACanvas.FillRect(ARect, Color);
end;

function TdxCustomImageSlider.HasBackground: Boolean;
begin
  Result := True;
end;

function TdxCustomImageSlider.IsTransparentBackground: Boolean;
begin
  Result := Transparent;
end;
...
procedure TdxCustomImageSlider.UnInitScrollbars;
begin
  inherited UnInitScrollbars;
  FPreviewScrollBar.UnInitControl;
end;
...
begin
  ...
  Inc(APageBounds.Bottom, TdxTextMeasurer.TextLineHeight(Font) + ScaleFactor.Apply(cxTextOffset));
...
begin
  ...
  if Result then
  begin
    if FScrollAnimationHelper.IsAnimationActive then
      APos := FScrollAnimationHelper.TargetPosition
    else
      APos := PreviewScrollPosition;
    DoSetPreviewScrollPosition(APos - Math.Sign(WheelDelta) * dxImageSliderPreviewScrollStep);
  end;
end;
...
begin
  ...
  if FScrollAnimationHelper.IsAnimationActive then
  begin
    if ScrollCode in [scTrack, scPosition] then
  ...
  case ScrollCode of
    scLineUp, scLineDown:
      DoSetPreviewScrollPosition(APos + Sign[ScrollCode = scLineDown] * dxImageSliderPreviewScrollStep);
    scPageUp, scPageDown:
      DoSetPreviewScrollPosition(APos + Sign[ScrollCode = scPageDown] * APageSize);
    scTrack, scPosition:
      PreviewScrollPosition := ScrollPos;
    scTop:
      DoSetPreviewScrollPosition(0);
    scBottom:
      DoSetPreviewScrollPosition(AMax);
  end;
...
begin
  ...
  begin
    ...
    try
      ...
      if Animation <> nil then
        Animation.Draw(Canvas, ClientBounds)
      else
...
procedure TdxCustomImageSlider.CheckPreviewScrollPosition(var AValue: Integer);
var
  AMax, APageSize: Integer;
begin
  GetPreviewScrollBarInfo(APageSize, AMax);
...
procedure TdxCustomImageSlider.DoSetPreviewScrollPosition(AValue: Integer);
begin
  if IsScrollAnimationEnabled then
    FScrollAnimationHelper.SetPosition(AValue)
  else
    PreviewScrollPosition := AValue;
end;
...
begin
  ...
  begin
    ...
    AMax := PreviewOptions.ThumbnailSize.Height + TdxTextMeasurer.TextLineHeight(Font) + ScaleFactor.Apply(cxTextOffset);
  end
  else
  begin
...
var
  I, ADelta: Integer;
begin
  CheckPreviewScrollPosition(AValue);
...
begin
  ...
  if R.Top < R1.Top then
    DoSetPreviewScrollPosition(PreviewScrollPosition - (Abs(R.Top) + PreviewOptions.Indent))
  else
    if R.Bottom > R1.Bottom then
      DoSetPreviewScrollPosition(PreviewScrollPosition + (R.Bottom - R1.Bottom) + PreviewOptions.Indent)
    else
      if FScrollAnimationHelper.IsAnimationActive then
        FScrollAnimationHelper.StopAnimation(False);
...
function TdxCustomImageSlider.TPreviewGestureHelper.UseRightToLeftScrollbar: Boolean;
begin
  Result := FSlider.UseRightToLeftScrollBar;
end;

{ TdxCustomImageSlider.TPreviewScrollAnimationHelper }

procedure TdxCustomImageSlider.TPreviewScrollAnimationHelper.CheckPosition(
  var AValue: Integer);
begin
  FOwner.CheckPreviewScrollPosition(AValue);
end;

constructor TdxCustomImageSlider.TPreviewScrollAnimationHelper.Create(
  AOwner: TdxCustomImageSlider);
begin
  inherited Create;
  FOwner := AOwner;
end;

function TdxCustomImageSlider.TPreviewScrollAnimationHelper.GetOwnerPosition: Integer;
begin
  Result := FOwner.PreviewScrollPosition;
end;

procedure TdxCustomImageSlider.TPreviewScrollAnimationHelper.UpdateOwnerPosition(
  AValue: Integer);
begin
  FOwner.PreviewScrollPosition := AValue;
end;
