...
unit dxRichEdit.DocumentModel.Fields; // for internal use
...
type
  ...
  strict private
    class var
      FEmptySwitchesWithArgument: TdxStringBooleanDictionary;
  ...
  protected
    class function CreateSwitchesWithArgument(const ASwitches: array of string): TdxStringBooleanDictionary; overload; static;
    ...
    function GetSwitchesWithArguments: TdxStringBooleanDictionary; virtual;
    ...
    function GetShouldApplyFormating: Boolean; virtual;
    ...
    property ShouldApplyFormating: Boolean read GetShouldApplyFormating;
  public
    ...
    function GetNativeSwitches: TArray<string>; virtual;
...
uses
  RTLConsts, Contnrs, Character, Variants,
...
const
  dxThisUnitName = 'dxRichEdit.DocumentModel.Fields';
...
class destructor TdxCalculatedFieldBase.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxCalculatedFieldBase.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FEmptySwitchesWithArgument);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxCalculatedFieldBase.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
class function TdxCalculatedFieldBase.CreateSwitchesWithArgument(
  const ASwitches: array of string): TdxStringBooleanDictionary;
var
  ACount, I: Integer;
begin
...
class constructor TdxCalculatedFieldBase.Initialize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxCalculatedFieldBase.Initialize', SysInit.HInstance);{$ENDIF}
  FEmptySwitchesWithArgument := CreateSwitchesWithArgument;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxCalculatedFieldBase.Initialize', SysInit.HInstance);{$ENDIF}
end;
...
function TdxCalculatedFieldBase.GetShouldApplyFormating: Boolean;
begin
  Result := True;
end;

function TdxCalculatedFieldBase.GetSwitchesWithArguments: TdxStringBooleanDictionary;
begin
  Result := FEmptySwitchesWithArgument;
end;
...
begin
  if not ShouldApplyFormating then
    Exit;
...
function TdxCalculatedFieldBase.GetNativeSwitches: TArray<string>;
begin
  SetLength(Result, 0);
end;
...
var
  ...
  AIndex, AFirstRunIndex: Integer;
  ACurrentField: TdxField;
begin
  AFields := Fields;
  AFirstRunIndex := AField.FirstRunIndex;
  if not TdxAlgorithms1<TdxField>.BinarySearch(AFields.InnerList,
    function (const AField: TdxField): Integer
    begin
      Result := AField.LastRunIndex - AFirstRunIndex;
    end, AIndex) then
  begin
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxUpdateFieldOperationResult.Initialize', SysInit.HInstance);{$ENDIF}
  DisabledUpdate := TdxUpdateFieldOperationResult.Create(TdxUpdateFieldResult.FieldNotUpdated, True);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxUpdateFieldOperationResult.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxUpdateFieldOperationResult.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxUpdateFieldOperationResult.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(DisabledUpdate);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxUpdateFieldOperationResult.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  ...
  while GetCharacter(APos).IsWhiteSpace and (APos.RunIndex < AEndRunIndex) do
