...
const
  dxThisUnitName = 'dxRichEdit.Dialogs.Hyperlink';
...
begin
  ...
  try
    ...
    if AFileDialog.Execute(Handle) then
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxRichEditHyperlinkDialogForm.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FTargetFrames);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxRichEditHyperlinkDialogForm.Finalize', SysInit.HInstance);{$ENDIF}
end;

class constructor TdxRichEditHyperlinkDialogForm.Initialize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxRichEditHyperlinkDialogForm.Initialize', SysInit.HInstance);{$ENDIF}
  FTargetFrames := CreateTargetFrames;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxRichEditHyperlinkDialogForm.Initialize', SysInit.HInstance);{$ENDIF}
end;
