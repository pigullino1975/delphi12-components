...
uses
  ...
  dxLayoutControl, dxLayoutContainer, cxLookAndFeelPainters, dxTypeHelpers;
...
type
  ...
  private
    ...
    FPrevClosedState: Boolean;
    FPrevPoint: TPoint;
  protected
    procedure AssignSizingParameters;
    procedure RestoreSize; override;
    ...
    function GetMajorValue(const P: TPoint): Integer; overload; virtual; abstract;
    function GetMajorValue(const ASize: TSize): Integer; overload; virtual; abstract;
    function IsHorizontalSpecific: Boolean; virtual; abstract;
    procedure ReinitializeStartDragPoint; virtual; abstract;
  ...
  protected
    ...
    function IsHorizontalSpecific: Boolean; override;
    procedure ReinitializeStartDragPoint; override;
  ...
  protected
    ...
    procedure PaintDestinationArea(ABitmap: TcxAlphaBitmap; var ADrawingRect: TRect);
  ...
  { TdxLayoutDragAndDropState }

  TdxLayoutDragAndDropState = class
  strict private
    class var FDragAndDropObject: TdxLayoutDragAndDropObject;
    class var FSizingDragAndDropObject: TdxLayoutSizingDragAndDropObject;
    class var FUseWrapItemsOnMeasureMinWidth: Boolean;
  private
    class function GetController: TdxLayoutCustomDragAndDropController; static;
    class function GetDestinationContainer: TdxLayoutContainer; static;
  public
    class function CanDrop: Boolean; static;
    class procedure CorrectDropAreaPart(ADestinationItem: TdxCustomLayoutItem; var AAreaPart: TdxLayoutDropAreaPart); static;
    class function GetDragImageBackgroundColor(AViewInfo: TdxCustomLayoutItemViewInfo): TColor; static;
    class function GetDragImageFrameColor(AViewInfo: TdxCustomLayoutItemViewInfo): TColor; static;
    class function SourceItemIs(AItem: TdxCustomLayoutItem): Boolean; static;
    class function StateIn(AStateSet: TcxDragAndDropStates): Boolean; static;
    class procedure PaintDestinationArea(ABitmap: TcxAlphaBitmap; var ADrawingRect: TRect); static;
    class procedure PaintDraggedItem(ABitmap: TcxAlphaBitmap; AViewInfo: TdxCustomLayoutItemViewInfo); static;

    class property Controller: TdxLayoutCustomDragAndDropController read GetController;
    class property DestinationContainer: TdxLayoutContainer read GetDestinationContainer;
    class property DragAndDropObject: TdxLayoutDragAndDropObject read FDragAndDropObject write FDragAndDropObject;
    class property SizingDragAndDropObject: TdxLayoutSizingDragAndDropObject read FSizingDragAndDropObject write FSizingDragAndDropObject;
    class property UseWrapItemsOnMeasureMinWidth: Boolean read FUseWrapItemsOnMeasureMinWidth write FUseWrapItemsOnMeasureMinWidth;
  end;
...
const
  dxThisUnitName = 'dxLayoutDragAndDrop';
...
class function TdxLayoutDragAndDropState.CanDrop: Boolean;
begin
  if DragAndDropObject <> nil then
    Result := DragAndDropObject.CanDrop
  else
    Result := False;
end;

class procedure TdxLayoutDragAndDropState.CorrectDropAreaPart(ADestinationItem: TdxCustomLayoutItem;
  var AAreaPart: TdxLayoutDropAreaPart);
begin
  if Controller <> nil then
    Controller.CorrectDropAreaPart(ADestinationItem, AAreaPart);
end;

class function TdxLayoutDragAndDropState.GetController: TdxLayoutCustomDragAndDropController;
begin
  if DragAndDropObject <> nil then
    Result := DragAndDropObject.Controller
  else
    Result := nil;
end;

class function TdxLayoutDragAndDropState.GetDestinationContainer: TdxLayoutContainer;
begin
  if Controller <> nil then
    Result := Controller.DestinationContainer
  else
    Result := nil;
end;

class function TdxLayoutDragAndDropState.GetDragImageBackgroundColor(AViewInfo: TdxCustomLayoutItemViewInfo): TColor;
begin
  if Controller <> nil then
    Result := Controller.GetDragImageBackgroundColor(AViewInfo)
  else
    Result := clNone;
end;

class function TdxLayoutDragAndDropState.GetDragImageFrameColor(AViewInfo: TdxCustomLayoutItemViewInfo): TColor;
begin
  if Controller <> nil then
    Result := Controller.GetDragImageFrameColor(AViewInfo)
  else
    Result := clNone;
end;

class function TdxLayoutDragAndDropState.SourceItemIs(AItem: TdxCustomLayoutItem): Boolean;
begin
  if DragAndDropObject <> nil then
    Result := DragAndDropObject.SourceItem = AItem
  else
    Result := False;
end;

class function TdxLayoutDragAndDropState.StateIn(AStateSet: TcxDragAndDropStates): Boolean;
begin
  if DragAndDropObject <> nil then
    Result := DragAndDropObject.Control.DragAndDropState in AStateSet
  else
    Result := False;
end;

class procedure TdxLayoutDragAndDropState.PaintDestinationArea(ABitmap: TcxAlphaBitmap; var ADrawingRect: TRect);
begin
  if Controller <> nil then
    Controller.PaintDestinationArea(ABitmap, ADrawingRect);
end;

class procedure TdxLayoutDragAndDropState.PaintDraggedItem(ABitmap: TcxAlphaBitmap;
  AViewInfo: TdxCustomLayoutItemViewInfo);
begin
  if Controller <> nil then
    Controller.PaintDraggedItem(ABitmap, AViewInfo);
end;
...
begin
  inherited Create(AControl);
  TdxLayoutDragAndDropState.DragAndDropObject := Self;
  FController := CreateController;
end;

destructor TdxLayoutDragAndDropObject.Destroy;
begin
  TdxLayoutDragAndDropState.DragAndDropObject := nil;
...
begin
  inherited Create(AControl);
  TdxLayoutDragAndDropState.SizingDragAndDropObject := Self;
end;

destructor TdxLayoutSizingDragAndDropObject.Destroy;
begin
  TdxLayoutDragAndDropState.SizingDragAndDropObject := nil;
  inherited Destroy;
end;
...
begin
  ...
  if MarkerIndex in [0, 6, 7] then
    SourceItem.Width := FOriginalBounds.Width + XC * (FStartDragPoint.X - P.X);
  if MarkerIndex in [2, 3, 4] then
    SourceItem.Width := FOriginalBounds.Width + XC * (P.X - FStartDragPoint.X);
  if MarkerIndex in [0, 1, 2] then
    SourceItem.Height := FOriginalBounds.Height + YC * (FStartDragPoint.Y - P.Y);
  if MarkerIndex in [4, 5, 6] then
    SourceItem.Height := FOriginalBounds.Height + YC * (P.Y - FStartDragPoint.Y);
end;
...
begin
  ...
  begin
    SourceItem.Width := FOriginalSize.Width;
    SourceItem.Height := FOriginalSize.Height;
  end
  else
...
procedure TdxLayoutSplitterDragAndDropObject.DragAndDrop(const P: TPoint; var Accepted: Boolean);
  ...
  var
    ...
    AViewInfo: TdxCustomLayoutItemViewInfo;
  begin
    AOffset := TdxLayoutGroupViewInfoAccess(AGroup.ViewInfo).ItemOffset;
    Result := GetMajorValue(cxRectSize(AGroup.ViewInfo.ItemsAreaBounds));
    for I := 0 to AGroup.ViewInfo.ItemViewInfoCount - 1 do
    begin
      AViewInfo := AGroup.ViewInfo.ItemViewInfos[I];
      case FSizingStrategy of
        lssLeft:
          if (GetWorkAlign(AViewInfo) = ahClient) and (FLeftItem.ViewInfo <> AViewInfo) then
            Result := Result - GetItemMinSize(AViewInfo)
          else
            Result := Result - GetItemSize(AViewInfo);
        lssRight:
          if (GetWorkAlign(AViewInfo) = ahClient) and (FRightItem.ViewInfo <> AViewInfo) then
            Result := Result - GetItemMinSize(AViewInfo)
          else
            Result := Result - GetItemSize(AViewInfo);
      end;
  ...
  procedure CorrectDeltaForFreeSpace(var ADelta: Integer);
  var
    AFreeSpace: Integer;
    ANewSize: Integer;
    ALocalDelta: Integer;
  begin
    if not (FSizingStrategy in [lssLeft, lssRight]) then
      Exit;
    TdxLayoutDragAndDropState.UseWrapItemsOnMeasureMinWidth := (FSizingStrategy = lssRight) and (ADelta < 0);
    AFreeSpace := GetGroupFreeSpace(FParentItem);
    TdxLayoutDragAndDropState.UseWrapItemsOnMeasureMinWidth := False;

    case FSizingStrategy of
      lssLeft:
        begin
          ANewSize := GetNewLeftItemSize(ADelta);
          ALocalDelta := ANewSize - GetItemSize(FLeftItem.ViewInfo);
          if (AFreeSpace - ALocalDelta) < 0 then
            ADelta := ADelta - (ALocalDelta - AFreeSpace);
        end;
      lssRight:
        begin
          ANewSize := GetNewRightItemSize(ADelta);
          ALocalDelta := ANewSize - GetItemSize(FRightItem.ViewInfo);
          if (AFreeSpace - ALocalDelta) < 0 then
            ADelta := ADelta + (ALocalDelta - AFreeSpace);
        end;
    end;
  ...
  procedure CorrectDeltaForLeft(var ADelta: Integer);
  begin
  ...
  procedure CorrectDeltaForRight(var ADelta: Integer);
  begin
  ...
  function DoCanResize(var ANewSize: Integer; AItem: TdxCustomLayoutItem): Boolean;
  begin
    Result := (AItem = nil) or TdxLayoutSplitterItemAccess(SourceItem).DoCanResize(AItem, ANewSize);
  end;

  function CanResize(AItem: TdxCustomLayoutItem; ANewSize: Integer): Boolean;
  begin
    Result := True;
    if AItem <> nil then
      Result := (ANewSize >= GetItemMinSize(AItem.ViewInfo)) and (ANewSize <= GetItemMaxSize(AItem.ViewInfo));
  end;

var
  ADelta: Integer;
  ANewLeftSize, ANewRightSize: Integer;
begin
  if FPrevPoint <> P then
  begin
    FPrevPoint := P;
    ADelta := GetMajorValue(P) - GetMajorValue(FStartDragPoint);
    ...
    begin
      ...
      begin
        ...
        ReinitializeStartDragPoint;
        AssignSizingParameters;
      end;
      Exit;
    end;
    ...
    case FSizingStrategy of
      lssLeft:
        begin
          ANewLeftSize := GetNewLeftItemSize(ADelta);
          if DoCanResize(ANewLeftSize, FLeftItem) then
            SetItemSize(FLeftItem.ViewInfo, ANewLeftSize);
        end;
      lssRight:
        begin
          CorrectDeltaForFreeSpace(ADelta);
          if IsHorizontalSpecific then
          begin
            TdxLayoutDragAndDropState.UseWrapItemsOnMeasureMinWidth := True;
            CorrectDeltaForRight(ADelta);
            TdxLayoutDragAndDropState.UseWrapItemsOnMeasureMinWidth := False;
          end;
          ANewRightSize := GetNewRightItemSize(ADelta);
          if DoCanResize(ANewRightSize, FRightItem) then
            SetItemSize(FRightItem.ViewInfo, ANewRightSize);
        end;
      lssClient:
        begin
          CorrectDeltaForLeft(ADelta);
          CorrectDeltaForRight(ADelta);
          ANewLeftSize := GetNewLeftItemSize(ADelta);
          ANewRightSize := GetNewRightItemSize(ADelta);
          if DoCanResize(ANewLeftSize, FLeftItem) and CanResize(FLeftItem, ANewLeftSize) and
            DoCanResize(ANewRightSize, FRightItem) and CanResize(FRightItem, ANewRightSize) then
          begin
...
begin
  ...
  FPrevClosedState := TdxLayoutSplitterItemAccess(SourceItem).IsClosed;
  FPrevPoint := P;
  AssignSizingParameters;
end;

procedure TdxLayoutSplitterDragAndDropObject.AssignSizingParameters;
...
begin
  ...
  try
    for I := 0 to FParentItem.VisibleCount - 1 do
      SetItemSize(FParentItem.VisibleItems[I].ViewInfo, GetMajorValue(FOriginalSizes[I]));
    if FPrevClosedState then
...
function TdxLayoutHSplitterDragAndDropObject.IsHorizontalSpecific: Boolean;
begin
  Result := True;
end;
...
begin
  Result := AViewInfo.Bounds.Width;
end;

function TdxLayoutHSplitterDragAndDropObject.GetMajorValue(const P: TPoint): Integer;
begin
  Result := P.X;
end;

function TdxLayoutHSplitterDragAndDropObject.GetMajorValue(const ASize: TSize): Integer;
begin
  Result := ASize.Width;
end;

procedure TdxLayoutHSplitterDragAndDropObject.ReinitializeStartDragPoint;
begin
  FStartDragPoint.X := SourceItem.Viewinfo.Bounds.CenterPoint.X;
end;
...
function TdxLayoutVSplitterDragAndDropObject.IsHorizontalSpecific: Boolean;
begin
  Result := False;
end;
...
begin
  Result := AViewInfo.Bounds.Height;
end;

function TdxLayoutVSplitterDragAndDropObject.GetMajorValue(const P: TPoint): Integer;
begin
  Result := P.Y;
end;

function TdxLayoutVSplitterDragAndDropObject.GetMajorValue(const ASize: TSize): Integer;
begin
  Result := ASize.Height;
end;

procedure TdxLayoutVSplitterDragAndDropObject.ReinitializeStartDragPoint;
begin
  FStartDragPoint.Y := SourceItem.Viewinfo.Bounds.CenterPoint.Y;
end;
...
begin
  Result := Container.CanDragAndDrop and (Control.DragAndDropState = ddsNone) and not
    ((FMouseDownPos = cxInvalidPoint) or IsPointInDragDetectArea(FMouseDownPos, X, Y)) and
...
begin
  ...
  if Owner.FStartDragPoint <> cxInvalidPoint then
  ...
  if SourceItem.Container.CustomizeForm <> nil then
    TdxLayoutContainerAccess(SourceItem.Container).CustomizeFormPostUpdate([cfutAvailableItems, cfutVisibleItems]);
...
begin
  Result := (DestinationItem <> nil) and (DestinationGroup <> nil) and
    (not TdxLayoutGroupAccess(DestinationGroup).IsAvailable or TdxCustomLayoutItemAccess(SourceItem).CanRemove) and
    not TdxLayoutGroupAccess(DestinationGroup).IsLocked and SourceItem.CanMoveTo(DestinationGroup) and
    (TdxLayoutContainerAccess(Container).IsStandardCustomization or TdxLayoutGroupAccess(DestinationGroup).CanQuickCustomized);
end;
...
begin
  if DestinationGroup = nil then
    ...
    if  DestinationGroup.Parent = nil then
      Result := DestinationGroup.ViewInfo.SelectionArea
    else
      Result := DestinationGroup.Parent.ViewInfo.SelectionArea;
...
procedure TdxLayoutCustomDragAndDropController.PaintDestinationArea(ABitmap: TcxAlphaBitmap; var ADrawingRect: TRect);

  procedure DrawArrow(P: TPoint; AArrowPosition: TdxCorner);
  var
    ASign: TPoint;
  ...
  begin
    ...
    P := (DestinationContainer as IdxScaleFactor).Value.Revert(P);
    AScaleValue :=(DestinationContainer as IdxScaleFactor).Value.ApplyF(1.0);
    APoints[0] := P;
    ...
    try
      dxGPPaintCanvas.ScaleWorldTransform(AScaleValue, AScaleValue);
...
begin
  ...
  R := cxRectInflate(GetDestinationImageBounds, 7, 7); 
  if cxRectIsEmpty(ADrawingRect) then
    ADrawingRect := R
  else
    ADrawingRect.Union(R);
...
procedure TdxLayoutCustomDragAndDropController.PrepareDragImage;
  ...
  begin
    ...
    if AItem.Container.CustomizeForm <> nil then
      AItem.Container.CustomizeForm.BeginUpdate;
    ...
    try
      ...
      try
        ...
        FDragImage.SetBounds(0, 0, ADragImageSize.Width, ADragImageSize.Height);
      ...
      AItem.StopFloat;
      if AItem.Container.CustomizeForm <> nil then
        AItem.Container.CustomizeForm.CancelUpdate;
  ...
  begin
    ...
    try
      ...
      try
        ...
        FDragImage.SetBounds(0, 0, FDragDropInfo.SourceItemSize.Width, FDragDropInfo.SourceItemSize.Height);
...
begin
  if TdxCustomLayoutItemAccess(SourceItem).AllowFloatingDragImage then
