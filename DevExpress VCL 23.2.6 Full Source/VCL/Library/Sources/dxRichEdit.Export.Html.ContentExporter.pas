...
unit dxRichEdit.Export.Html.ContentExporter; // for internal use
...
type
  ...
  public
    constructor Create(const ADocumentModel: TdxDocumentModel; AUnitConverter: TdxDocumentModelUnitConverter; AOptions: TdxHtmlDocumentExporterOptions);
...
{$ENDREGION}
  ...
  protected
    ...
    function ConvertWhiteSpacesToOfficeSpace(const ARawText: string): string;
    procedure GenerateSpaces(ACount: Integer; AForceUseNonBreakingSpace: Boolean);
...
const
  dxThisUnitName = 'dxRichEdit.Export.Html.ContentExporter';
...
constructor TdxOfficeHtmlImageHelper.Create(const ADocumentModel: TdxDocumentModel;
  AUnitConverter: TdxDocumentModelUnitConverter; AOptions: TdxHtmlDocumentExporterOptions);
begin
  inherited Create;
...
procedure TdxOfficeHtmlImageHelper.ApplyImageProperties(AImageControl: TdxWebImageControl;
  ...
  function GetImageReference: TdxOfficeImageReference;
  var
    ASource: TdxOfficeImage;
  begin
    Result := nil;
    if FOptions.EmbedImages and FOptions.ScaleEmbeddedImages then
    begin
      ASource := AImage.Clone;
      ASource.Resize(FUnitConverter.ModelUnitsToPixels(AActualSize.Width, HorizontalResolution),
        FUnitConverter.ModelUnitsToPixels(AActualSize.Height, HorizontalResolution));
      ASource.ImageCodec := AImage.ImageCodec;
    end
    else
      ASource := AImage;
    try
      if ShouldConvertImage(ASource) and not AKeepImageSize then
        Result := GetHtmlImage(ASource, AActualSize, ACorrectWidth)
      else
        Result := FDocumentModel.CreateImage(ASource);
    finally
      if (ASource <> AImage) and (Result.Image <> ASource) then
        ASource.Free;
    end;
  end;
...
begin
  ...
  AForceWriteImageSize := ShouldConvertImage(AImage) and AKeepImageSize;
  if not ShouldConvertImage(AImage) then
  begin
    AUriImageWithoutSize := (AUriOfficeImage <> nil) and
      ((AUriOfficeImage.PixelTargetWidth = 0) and (AUriOfficeImage.PixelTargetHeight = 0));
    if not AUriImageWithoutSize then
      AForceWriteImageSize := True;
  end;
  AReference := GetImageReference;
  try
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxHtmlStyleHelper.Initialize', SysInit.HInstance);{$ENDIF}
  FSpecialCharactersReplaceTable := CreateSpecialCharacterReplaceTable;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxHtmlStyleHelper.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxHtmlStyleHelper.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxHtmlStyleHelper.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FSpecialCharactersReplaceTable);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxHtmlStyleHelper.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  Result := AParagraph.LastRunIndex - AParagraph.FirstRunIndex = 0;
end;
...
begin
  ...
  ExportCore(CreateHtmlExporterTextProperties(ARun.FontCacheIndex, AForeColor, ABackColor, False, False,
    TdxCharacterFormattingScript.Normal, ARun.AllCaps, ARun.DoubleFontSize), '<o:p>&nbsp;</o:p>');
end;
...
begin
  ...
  ABackgroundColor := ACell.GetActualBackgroundColor;
  if not TdxAlphaColors.IsEmpty(ABackgroundColor) then
...
begin
  ...
  FImageHelper := TdxOfficeHtmlImageHelper.Create(ADocumentModel, ADocumentModel.UnitConverter, AOptions);
...
begin
  ...
  begin
    TdxHtmlStyleRender.GetHtmlStyle(AFontInfo.Name, ATextProperties.DoubleFontSize / 2, TdxGraphicUnit.guPoint,
...
begin
  ALiteralControl := TdxHtmlLiteralControl.Create(ConvertWhiteSpacesToOfficeSpace(AText));
  FObjectsToDelete.Add(ALiteralControl);

  AParent.Controls.Add(ALiteralControl);
end;

function TdxHtmlExportHelper.ConvertWhiteSpacesToOfficeSpace(const ARawText: string): string;
var
  ASpaceIndex, ACurrentIndex, ACount, I, ALength: Integer;
begin
  ALength := Length(ARawText);
  if ALength = 0 then
    Exit(ARawText);

  ASpaceIndex := TdxStringHelper.IndexOf(ARawText, TdxCharacters.Space);
  if ASpaceIndex < 0 then
    Exit(ARawText);

  FText.Capacity := Max(FText.Capacity, ALength + Max(64, ALength div 20));

  ACurrentIndex := 0;
  while True do
  begin
    if ASpaceIndex > ACurrentIndex then
      FText.Append(TdxStringHelper.Substring(ARawText, ACurrentIndex, (ASpaceIndex - ACurrentIndex)));

    ACount := 0;
    for I := ASpaceIndex + 1 to ALength do
    begin
      if ARawText[I] = TdxCharacters.Space then
        Inc(ACount)
      else
        Break;
    end;
    GenerateSpaces(ACount, ASpaceIndex = 0); 

    ACurrentIndex := ASpaceIndex + ACount;
    if ACurrentIndex = ALength then
      Break;
    ASpaceIndex := TdxStringHelper.IndexOf(ARawText, TdxCharacters.Space, ACurrentIndex + 1);
    if ASpaceIndex < 0 then
    begin
      ACount := ALength - ACurrentIndex;
      if ACount > 0 then
        FText.Append(TdxStringHelper.Substring(ARawText, ACurrentIndex, ACount));
      Break;
    end;
  end;
...
procedure TdxHtmlExportHelper.GenerateSpaces(ACount: Integer; AForceUseNonBreakingSpace: Boolean);
begin
  if AForceUseNonBreakingSpace or (ACount > 1) then
  begin
    FText.Append('<span style=''mso-spacerun:yes''>');
    FText.Append(StringOfChar(TdxCharacters.NonBreakingSpace, ACount));
    FText.Append('</span>');
  end
  else
    FText.Append(TdxCharacters.Space);
end;
