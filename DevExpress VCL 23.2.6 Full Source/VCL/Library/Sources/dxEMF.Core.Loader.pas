...
{$ENDREGION}
  ...
  TdxQueryDataPool = class(TObjectDictionary<TdxEntityInfo, TdxQueryData>) // for internal use
  public
    constructor Create;
  ...
  TdxSelectStatementCollections = class sealed // for internal use
  public
...
const
  dxThisUnitName = 'dxEMF.Core.Loader';
...
begin
  ...
  begin
    ...
    SetTypeKind(AOriginal.TypeKind);
  end;
end;

function TdxSelectStatementResultMemberInfo.GetIsLoadable: Boolean;
begin
  Result := True;
end;
...
begin
  ...
  begin
    ...
    if AProperties[I].QueryInterface(IdxParentCriteria, AParentCriteria) = 0 then
      if AParentCriteria.ParentCriteria.QueryInterface(IdxMemberOperand, AMemberOperand) = 0 then
...
procedure TdxDataLoader.EndLoad;
begin
...
begin
  ...
  repeat
    ...
    if Result = nil then
    begin
      ANewObject := TdxEMFCustomSessionAccess(ASession).CreateEntity(EntityInfo);
...
begin
  ...
  begin
    if Safe.Cast(AProp as TdxCriteriaOperator, TdxOperandProperty, AMember) then
    begin
...
begin
  FProperties := TdxMemberPathCollection.Create;
    ...
    begin
      if ((not M.IsDelayed) or (M.ReferenceType <> nil)) then 
      begin
...
procedure TdxEMFSelectStatementResultCollection.Delete(AIndex: Integer);
begin
  FSelectStatementResult.Rows.Delete(AIndex);
end;
...
begin
  if FIndex >= 0 then
    Result := FSelectStatementResult.Rows.List[FIndex]
  else
    Result := nil;
end;
