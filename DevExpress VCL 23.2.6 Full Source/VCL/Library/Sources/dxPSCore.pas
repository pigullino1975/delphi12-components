...
uses
  ...
  dxBase, dxPSGlbl, dxPSSngltn, dxPSEngn, dxPSESys, dxPSForm, dxBkgnd, dxPrnPg, dxCoreGraphics,
...
type
  ...
  private
    function GetItem(Index: TdxListIndex): Integer;
    procedure SetItem(Index: TdxListIndex; Value: Integer);
  public
    function Add(AValue: Integer): Integer;
    property Items[Index: TdxListIndex]: Integer read GetItem write SetItem; default;
  end;
  ...
  private
    ...
    FInsufficientHeight: Boolean;
    FInsufficientWidth: Boolean;
  ...
  protected
    ...
    function UseReportLinkDelimiters: Boolean; virtual;
...
{$ENDIF}
  ...
  public
    ...
    procedure ValidateGraphicHelper(AGraphic: TGraphic; AImageList: TCustomImageList;
      AImageIndex: Integer; ABackgroundColor, ATransparentColor: TColor; AIsTransparent: Boolean; const APalette: IdxColorPalette);
  ...
  TdxPSReportRenderer = class(TObject) // for internal use
  private
    FBorderColor: TColor;
  ...
  public
    ...
    procedure DrawGraphicEx(ACanvas: TdxPSReportRenderCustomCanvas; R: TRect;
      ...
      AActualImageBuffering: TdxCellImageActualBuffering = cibAlways; ATransparentColor: TColor = clNone; const APalette: IdxColorPalette = nil); virtual;
  ...
  private
    ...
    procedure SetSignatureOptions(AValue: TdxPSPDFSignatureOptions);
  ...
  published
    ...
    property SignatureOptions: TdxPSPDFSignatureOptions read FSignatureOptions write SetSignatureOptions;
  ...
  TdxReportItem = class(TPersistent) // for internal use
  private
  ...
  protected
    ...
    property AdjustFont: Boolean read FAdjustFont write SetAdjustFont;
  ...
  protected
    ...
    function PrepareImage(out AGraphic: TGraphic; out AGraphicOwnership: TdxObjectOwnership): Boolean;
  ...
  public
    ...
    property Palette: IdxColorPalette read FPalette write FPalette;
  end;
  ...
  TdxReportLinkCapability = (rlcPageSetup, rlcHeaderFooter, rlcTitle, rlcFootnotes, rlcExportToPDF);
  ...
  protected
    ...
    function SupportsGapsBetweenCells: Boolean; virtual;
    procedure UpdatePrintDate; virtual;
  ...
  public
    ...
    function CanExportToPDF: Boolean;
...
var
  ...
  FReduceMemoryUse: Boolean = False; 
...
function dxPSCalculateImageBounds(const R: TRect; AWidth, AHeight: Integer; ADrawMode: TdxGraphicDrawMode): TRect;
...
uses
  Types, ComObj, Themes, UxTheme, Math, Variants, dxPSCPDsg,
 {$IFDEF USEJPEGIMAGE}
  Jpeg,
 {$ENDIF}
  TypInfo, Registry, Consts, CommCtrl, Dialogs, ShlObj, ShellAPI, dxPSRes,
  dxPSImgs, dxPSUtl, dxPSEvnt, dxPSEdgePatterns, dxfmDTFmt, dxfmPNFmt, dxPSfmTtl, dxPSfmFootnotes, dxfmChFN, dxPrnDev,
  dxPSPgsMnuBld, dxPSfmReportProperties, dxPSXplorerTreeView, dxPSCompsProvider, dxPSfmCompositionDsg, dxPSPrVwStd,
  dxPSPDFExport, dxPSHFLibrary, dxTypeHelpers, cxLookAndFeels, dxDPIAwareUtils, dxHash, dxMessageDialog;

const
  dxThisUnitName = 'dxPSCore';
...
type
  ...
  private
    function GetItem(Index: TdxListIndex): TdxReportLinkRegItem;
  protected
  ...
  public
    ...
    property Items[Index: TdxListIndex]: TdxReportLinkRegItem read GetItem;
  end;
  ...
  private
    function GetItem(Index: TdxListIndex): TBasedxReportLink;
  public
    property Items[Index: TdxListIndex]: TBasedxReportLink read GetItem; default;
  end;
  ...
  protected
    procedure AssignDelimitersFromReport;
  ...
  { TdxReportCellTextAdjustFontCache }

  TdxReportCellTextAdjustFontCache = class
  strict private
    FFontHeight: Integer;
    FFontProbe: TSize;
    FFormat: Cardinal;
    ...
    FTextHash: Cardinal;
  public
    procedure Update(ACanvas: TdxPSReportRenderCustomCanvas; AFont: TFont;
      const ASize: TSize; const AText: string; AMultiline: Boolean; AFormat: Cardinal);
    //
...
function dxPSCalculateImageBounds(const R: TRect;
  ...
  begin
    if AStretch or (AWidth > R.Width) or (AHeight > R.Height) then
    begin
...
function TdxPSReportLinkList.GetItem(Index: TdxListIndex): TBasedxReportLink;
begin
...
function TdxLinkList.GetItem(Index: TdxListIndex): TdxReportLinkRegItem;
begin
...
function TdxPageOverlayIndexes.GetItem(Index: TdxListIndex): Integer;
begin
  Result := Integer(inherited Items[Index]);
end;

procedure TdxPageOverlayIndexes.SetItem(Index: TdxListIndex; Value: Integer);
begin
...
begin
  ...
  if HasHeader then
    Inc(ContentBounds.Bottom, HeaderBounds.Height);
  if HasFooter then
    Inc(ContentBounds.Bottom, FooterBounds.Height);
  if HasRowHeader then
    Inc(ContentBounds.Right, RowHeaderBounds.Width);
  ...
  ContentBounds.Bottom := ContentBounds.Top + Min(AContentHeight, ContentBounds.Height);
end;
...
begin
  Result := 1;
  if not ReportLink.ShrinkToPageWidth and ReportLink.SupportsGapsBetweenCells then
  begin
    AEndIndex := {AStartIndex } -1;
    Result := 0;
    ATopSide := DetailBounds.Top;
    while (ATopSide < DetailBounds.Bottom) and (AEndIndex < ReportCells.Count) do
    begin
      Inc(Result);
      AStartIndex := AEndIndex + 1;
      repeat
        R := ReportCells.Cells[AStartIndex].AbsoluteRect;
        Inc(AStartIndex);
      until (AStartIndex = ReportCells.Count) or AreRectsIntersected(R, DetailBounds);
      Dec(AStartIndex);

      AEndIndex := AStartIndex;
      repeat
        R := ReportCells.Cells[AEndIndex].AbsoluteRect;
        Inc(AEndIndex);
      until (AEndIndex = ReportCells.Count) or not AreRectsIntersected(R, DetailBounds);
      Dec(AEndIndex);
      if AEndIndex <> ReportCells.Count - 1 then
        Dec(AEndIndex);
      AEndIndex := Max(AStartIndex, AEndIndex);
      ATopSide := R.Bottom;
    end;
  end;
end;
...
procedure TdxPSPageRenderInfo.CalculateOffsets;
  ...
  begin
    ...
    begin
      AOffset := Max(PaintSize.X - R.Width, 0) div 2;
  ...
  begin
    ...
    begin
      ...
      Inc(DataOffset.Y, MulDiv(Max(0, APaintSize - R.Height) div 2, 100, RenderInfo.ScaleFactor));
    end;
  end;
...
begin
  Result.X := MulDiv(PrinterPage.PaintRectLoMetric.Width, RenderInfo.UnitsPerInch, 254);
  Result.Y := MulDiv(PrinterPage.PaintRectLoMetric.Height, RenderInfo.UnitsPerInch, 254);
end;
...
procedure TdxPSPageDelimitersCalculator.Initialize;
begin
...
var
  ACalculator: TdxPSPageDelimitersCalculator;
begin
  ACalculator := Create(AReportRenderInfo);
  try
    ACalculator.Initialize;
    if AReportRenderInfo.UseReportLinkDelimiters then
      ACalculator.AssignDelimitersFromReport
    else
      ACalculator.CalculateDelimiters;
  finally
    ACalculator.Free;
  end;
end;

procedure TdxPSPageDelimitersCalculator.AssignDelimitersFromReport;
begin
  PageDelimitersList.Assign(ReportDelimitersList);
end;
...
function TdxPSPageDelimitersCalculator.ScaleValue(AValue: Integer; AScaleIn: Boolean = True): Integer;
begin
  if AScaleIn then
...
procedure TdxPSPageVerticalDelimitersCalculator.Initialize;
begin
...
begin
  ...
  Result := Max(0, Min(PrinterPage.PaintRectPixels.Height div 2, Result));
end;
...
begin
  FInsufficientHeight := False;
  FInsufficientWidth := False;
...
function TdxPSReportRenderInfo.UseReportLinkDelimiters: Boolean;
begin
  Result := False;
end;
...
function TdxPSReportRenderInfo.IsReportFitsToPages(APrinterPage: TdxPrinterPage): Boolean;
  ...
  function Check(ATargetValue, ACurrentValue: Integer; AInsufficientSpace: Boolean): Boolean;
  begin
    Result := (ATargetValue = 0) or (ACurrentValue <= ATargetValue) and not AInsufficientSpace;
  end;

begin
  Result :=
    Check(APrinterPage.FitToPagesVertically, PageRowCount, FInsufficientHeight) and
    Check(APrinterPage.FitToPagesHorizontally, PageColCount, FInsufficientWidth);
end;
...
begin
  ...
  begin
    if (FGraphicHelper.ImageList <> AImageList) or (FGraphicHelper.ImageIndex <> AImageIndex) then
      FGraphicHelper.Initialize(AImageList, AImageIndex, ABackgroundColor, ATransparentColor, AIsTransparent, Renderer.IsPrinting, APalette);
  end
  else
    if (FGraphicHelper.Graphic <> AGraphic) or not CheckRendererModeInfo then 
    begin
      FGraphicHelper.Initialize(AGraphic, ABackgroundColor, ATransparentColor, AIsTransparent, Renderer.IsPrinting, APalette);
      SaveModeInfo;
    end;
end;
...
begin
  ...
  begin
    ...
    SignatureOptions := AOptions.SignatureOptions;
...
procedure TdxPSPDFReportExportOptions.SetSignatureOptions(AValue: TdxPSPDFSignatureOptions);
begin
...
procedure TdxPSReportRenderer.DrawGraphicEx(ACanvas: TdxPSReportRenderCustomCanvas; R: TRect;
  ...
  AGraphic: TGraphic; AGraphicTransparent, ATransparent: Boolean;
  AColor, ABkColor: TColor; APattern: TdxPSFillPatternClass;
  AActualImageBuffering: TdxCellImageActualBuffering = cibAlways; ATransparentColor: TColor = clNone; const APalette: IdxColorPalette = nil);
...
begin
  ACanvas.SaveClipRgn;
  try
    if (R.Width > AClipRect.Width) or (R.Height > AClipRect.Height) then
    ...
    begin
      CachedGraphicInfo.ValidateGraphicHelper(AGraphic, AImageList, AImageIndex, ABkColor, ATransparentColor, IsImageTransparent(AGraphic), APalette);
...
begin
  ...
  begin
    ...
    if ReportLink.IsCurrentLink then
      dxHFFormatObject.TotalPages := ReportLink.GetPageCount;
...
procedure TdxPSReportRenderer.RenderPageContentDetails(const ADetailsPosition: TPoint; out AOffset: Integer);
  ...
  begin
    Result := cxNullPoint;
    if PageRenderInfo.HasRowHeader then
      Result.X := -PageRenderInfo.RowHeaderBounds.Width;
    if PageRenderInfo.HasHeader then
      Result.Y := -PageRenderInfo.HeaderBounds.Height
  end;
...
begin
  Canvas.OffsetWindowOrg(Point(0, -PageRenderInfo.HeaderBounds.Height - ADetailsOffset));
...
begin
  ...
  begin
    ...
    Indents := TdxReportCellText(Source).Indents;
...
var
  ACache: TdxReportCellTextAdjustFontCache;
  AFont: TFont;
  ARect: TRect;
  AText: string;
  ATextFormat: Cardinal;
  ATextParams: TcxTextParams;
begin
  ...
  begin
    ...
    try
      ACanvas.Font := Font;
      ATextParams := ACanvas.CalculateTextParams(ATextFormat);
      if ATextParams.PatternedText then
        Renderer.DrawTextEx(ACanvas, ARect, 0, AText, Font, ATextFormat, Indents.Rect)
      else
      begin
        if FAdjustFontCache = nil then
          FAdjustFontCache := TdxReportCellTextAdjustFontCache.Create;

        ACache := TdxReportCellTextAdjustFontCache(FAdjustFontCache);
        ACache.Update(ACanvas, Font, cxSize(cxPrepareRect(ARect, ATextParams, 0, 0)), AText, Multiline, ATextFormat);

        AFont := TFont.Create;
        try
          AFont.Assign(Font);
          AFont.Height := ACache.FontHeight;
          Renderer.DrawTextEx(ACanvas, ARect, 0, AText, AFont, ATextFormat, Indents.Rect, ACache.LineSpacing);
        finally
          AFont.Free;
        end;
      end;
...
var
  R: TRect;
begin
  R := Rect(0, 0, GetTextBounds(ACanvas).Width - cxMarginsWidth(Indents.Rect), 5);
...
begin
  ...
  AdjustFont := Indents.Left and dxFormatTextAdjustFont <> 0;
...
procedure TdxReportCellText.SetAdjustFont(AValue: Boolean);
begin
  if FAdjustFont <> AValue then
  begin
    FAdjustFont := AValue;
    FreeAndNil(FAdjustFontCache);
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        RowHeight := Max(ItemSize.cY, ItemsArea.Height div RowCount);
      end;
    end;
  end;
...
procedure TCustomdxReportCellImageContainer.DrawImage(ACanvas: TdxPSReportRenderCustomCanvas);
var
  AImage: TGraphic;
  AImageOwnership: TdxObjectOwnership;
  AImageAreaBounds: TRect;
  AImageBounds: TRect;
begin
  Renderer.CachedGraphicInfo.Clear;
  AImageBounds := GetImageBounds(ACanvas);
  AImageAreaBounds := GetImageAreaBounds(ACanvas);

  if PrepareImage(AImage, AImageOwnership) then
  try
    Renderer.DrawGraphicEx(ACanvas, AImageBounds, AImageAreaBounds, nil, -1, AImage, ImageTransparent,
      IsBackgroundBitmapDrawn or Transparent, Color, ContentBkColor, ContentPattern,
      ActualImageBuffering, ImageTransparentColor, Palette);
  finally
    if AImageOwnership <> ooReferenced then
      AImage.Free;
  end
  else
  begin
    Renderer.DrawGraphicEx(ACanvas, AImageBounds, AImageAreaBounds, ImageList, ImageIndex, Image,
      ImageTransparent, IsBackgroundBitmapDrawn or Transparent, Color, ContentBkColor, ContentPattern,
      ActualImageBuffering, ImageTransparentColor, Palette);

    if (ImageList <> nil) and (OverlayImageIndex <> -1) then
      Renderer.DrawGraphicEx(ACanvas, AImageBounds, AImageAreaBounds, ImageList, OverlayImageIndex, nil,
        True, True, clNone, clNone, ContentPattern, ActualImageBuffering, ImageTransparentColor, Palette);
  end;

  if FReduceMemoryUse then
...
function TCustomdxReportCellImageContainer.PrepareImage(
  out AGraphic: TGraphic; out AGraphicOwnership: TdxObjectOwnership): Boolean;

  function IsImageMustBeOpaque: Boolean;
  begin
    Result := IsTextBackgroundDrawn or IsImageBackgroundDrawn or not dxCanPrintTransparentImages(Renderer.Canvas);
  end;
...
var
  ABitmap: TcxBitmap;
begin
  if ImagePrepared then
  begin
    AGraphic := Image;
    AGraphicOwnership := ooReferenced;
    Result := True;
  end
  else
    if ImageListUsed then
    begin
      Result := (TcxImageList.GetPixelFormat(ImageList.Handle) = 32) and IsImageMustBeOpaque;
      if Result then
      begin
        ABitmap := TcxBitmap.CreateSize(ImageList.Width, ImageList.Height, pf24Bit);
        PrepareImageBackground(ABitmap.Canvas, ABitmap.ClientRect);
        TdxImageDrawer.DrawImage(ABitmap.Canvas, ABitmap.ClientRect, nil, ImageList, ImageIndex, ifmStretch, idmNormal, False, Palette);
        TdxImageDrawer.DrawImage(ABitmap.Canvas, ABitmap.ClientRect, nil, ImageList, OverlayImageIndex, ifmStretch, idmNormal, False, Palette);
        AGraphicOwnership := ooOwned;
        AGraphic := ABitmap;
      end;
    end
    else
    begin
      Result := Assigned(Image) and not (Image is TMetafile) and IsImageMustBeOpaque;
      if Result then
      begin
        ABitmap := TcxBitmap.CreateSize(dxGetImageSize(Image));
        PrepareImageBackground(ABitmap.Canvas, ABitmap.ClientRect);
        Image.Transparent := ImageTransparent;
        ABitmap.Canvas.StretchDraw(ABitmap.ClientRect, Image);
        AGraphicOwnership := ooOwned;
        AGraphic := ABitmap;
        if not FReduceMemoryUse then
        begin
          Image := ABitmap;
          ImagePrepared := True;
        end;
...
procedure TCustomdxReportCellImageContainer.SetImageIndex(const Value: Integer);
begin
  if Value <> -1 then
    Sleep(0);
  FImageIndex := Value;
...
begin
  ...
  begin
    ...
    Palette := TdxReportCellImage(Source).Palette;
  ...
  begin
    ...
    case ImageLayout of
      ...
      begin
        ...
        begin
          Inc(Result.Left, (Result.Width - W) div 2 + Renderer.UnitsPerPixel);
        ...
        case ImageLayout of
          ...
          ilImageCenterCenter:
            if not IsTextShiftedForHorizontallyCenteredImage then
            begin
              Inc(Result.Top, (Result.Height - H) div 2 + Renderer.UnitsPerPixel);
...
function TdxPSReportCellTreeLineViewInfo.CreateTreeLineRegion(
  ...
  begin
    if AIsVertical then
      Result := R.Height div (2 * LineThickness)
    else
      Result := R.Width div (2 * LineThickness);
  end;
...
function TBasedxReportLink.CanExportToPDF: Boolean;
begin
  Result := rlcExportToPDF in Capabilities;
end;
...
function TBasedxReportLink.SupportsGapsBetweenCells: Boolean;
begin
  Result := False;
end;

procedure TBasedxReportLink.UpdatePrintDate;
begin
  // do nothing
end;
...
begin
  ComponentPrinter.DoCustomDrawEntirePage(Self, ACanvas, ARealPageIndex, R,
    MulDiv(Denominator, R.Width, MulDiv(RenderInfo.PageSize.X, PixelsDenominator, PixelsNumerator)), Denominator);
end;
...
procedure TBasedxReportLink.Unprepare;
begin
  FPrepared := False;
end;
...
begin
  Result := RealPrinterPage.PaintRectPixels.Height - 1;
end;
...
begin
  Result := RealPrinterPage.PaintRectPixels.Width - 1;
end;
...
procedure TdxPSCompositionReportRenderRowInfo.CalculateContentInfo(
  ...
  begin
    Result := APageInfo.TitleHeight + APageInfo.ContentBounds.Height;
...
function TdxPSCompositionReportRenderRowInfo.CanCompose(AReportLink: TBasedxReportLink): Boolean;
  ...
  begin
    Result := AReportLink.RenderInfo.TitleHeight + Max(
      AReportLink.ReportCells.HeaderBoundsRect.Height,
      AReportLink.ReportCells.HeaderCornerBoundsRect.Height) +
      AReportLink.ReportCells.FooterBoundsRect.Height;
...
begin
  if (ReportLink <> nil) and not ReportLink.Activating and ReportLink.DataProviderPresent then
    ReportLink.RebuildReport;
end;
...
procedure TCustomdxComponentPrinter.DoStartPrint(AReportLink: TBasedxReportLink; FullPageCount: Integer);
begin
  AReportLink := CheckLink(AReportLink);
  if AReportLink <> nil then
  begin
...
begin
  ...
  begin
    ...
    MessageResult := dxMessageDlg(MessageText, mtWarning, Buttons, 0);
...
procedure TdxReportCellTextAdjustFontCache.Update(ACanvas: TdxPSReportRenderCustomCanvas; AFont: TFont;
  const ASize: TSize; const AText: string; AMultiline: Boolean; AFormat: Cardinal);
var
  AFontProbe: TSize;
  AHelper: TdxPSAdjustFontSizeHelper;
  ATextHash: Cardinal;
  AUpdateNeeded: Boolean;
begin
  ATextHash := 0;
  AFontProbe := cxNullSize;

  AUpdateNeeded := not cxSizeIsEqual(ASize, FSize) or (FMultiline <> AMultiline) or
    (AFormat <> FFormat) or (Length(AText) <> Length(FText));

  if not AUpdateNeeded then
  begin
    AFontProbe := ACanvas.TextSize(dxMeasurePattern);
    AUpdateNeeded := not cxSizeIsEqual(AFontProbe, FFontProbe);
  end;

  if not AUpdateNeeded then
  begin
    ATextHash := TdxStringHash.BobJenkins(AText);
    AUpdateNeeded := (ATextHash <> FTextHash) or (AText <> FText);
  end;

  if AUpdateNeeded then
  begin
    ...
    if ATextHash <> 0 then
      ...
      FTextHash := TdxStringHash.BobJenkins(AText);

    if cxSizeIsEmpty(AFontProbe) then
      FFontProbe := ACanvas.TextSize(dxMeasurePattern)
    else
      FFontProbe := AFontProbe;

    AHelper := TdxPSAdjustFontSizeHelper.Create(ACanvas);
    try
      AHelper.Font.Assign(AFont);
      AHelper.Calculate(ASize, AText, AMultiline, AFormat);

      FFontHeight := AHelper.Font.Height;
      FLineSpacing := AHelper.LineSpacing;
    finally
      AHelper.Free;
    end;
  end;
...
begin
  cxRegisterVclClasses([TBitmap, TMetafile, TIcon{$IFDEF USEJPEGIMAGE}, TJPEGImage{$ENDIF}]);
end;
