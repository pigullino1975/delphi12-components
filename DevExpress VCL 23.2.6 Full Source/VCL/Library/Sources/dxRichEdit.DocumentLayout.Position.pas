...
uses
  Classes, dxCoreClasses, cxClasses, Generics.Defaults, Generics.Collections,
...
type
  ...
  strict private
    ...
    procedure SetBox(const Value: TdxBox);
    ...
    procedure SetColumn(const Value: TdxColumn);
    procedure SetPageArea(const Value: TdxPageArea);
  protected
    ...
    function FindBoxIndex(ACollection: TdxBoxList; const AExactComparable: TdxBoxComparableFunc;
      const AIsIntersect: TdxIsIntersectedWithPrevBoxFunc;
      AStartIndex, AEndIndex: Integer): Integer; overload;
    function FindBoxIndex(ACollection: TdxBoxList; const AComparable, AExactComparable: TdxBoxComparableFunc;
      const AIsIntersect: TdxIsIntersectedWithPrevBoxFunc;
      AStartIndex, AEndIndex: Integer): Integer; overload;
  ...
  public
    ...
    property PageArea: TdxPageArea read FPageArea write SetPageArea;
    property Column: TdxColumn read FColumn write SetColumn;
    property Row: TdxRow read FRow write SetRow;
    property Box: TdxBox read FBox write SetBox;
  ...
  TdxTextBoxDocumentLayoutPosition = class(TdxDocumentLayoutPosition) // for internal use
  strict private
  ...
  TdxHeaderFooterDocumentLayoutPosition = class(TdxDocumentLayoutPosition) // for internal use
  strict private
    FPreferredPageIndex: Integer;
  ...
  protected
    ...
    function LookupPreferredPage(APages: TdxPageCollection): TdxPage;
    function IsPageMatch(APage: TdxPage): Boolean;
    function LookupPreferredPageBackward(APages: TdxPageCollection): TdxPage;
  public
...
const
  dxThisUnitName = 'dxRichEdit.DocumentLayout.Position';

type

  { TdxTableCellViewInfoAndLogPositionComparable }

  TdxTableCellViewInfoAndLogPositionComparable = record
  private
    FPieceTable: TdxPieceTable;
    FLogPosition: TdxDocumentLogPosition;
    function GetFirstPosition(ACell: TdxTableCellViewInfo): TdxDocumentLogPosition; inline;
    function GetLastPosition(ACell: TdxTableCellViewInfo): TdxDocumentLogPosition; inline;
  public
  ...
  TdxTableViewInfoAndLogPositionComparable = record
  private
    FPieceTable: TdxPieceTable;
    FLogPosition: TdxDocumentLogPosition;
  public
...
var
  AComparable: TdxExactPageComparable;
begin
  AComparable := TdxExactPageComparable.Create(PieceTable, LogPosition);
  Result := FindBoxIndex(AColumns, AComparable.ExactColumnAndLogPositionCompare, IsIntersectedWithPrevColumn, AStartIndex, AEndIndex);
end;
...
begin
  AComparable := TdxBoxLogPositionComparable.Create(PieceTable, LogPosition);
  TdxAlgorithms1<TdxBoxBase>.BinarySearch(ABoxes, AComparable.BoxAndLogPositionCompare, AStartIndex, AEndIndex, Result);
end;
...
begin
  AComparable := TdxBoxLogPositionComparable.Create(PieceTable, LogPosition);
  TdxAlgorithms1<TdxBoxBase>.BinarySearch(ACharacters, AComparable.BoxAndLogPositionCompare, AStartIndex, AEndIndex, Result);
end;
...
function TdxDocumentLayoutPosition.FindBoxIndex(ACollection: TdxBoxList; const AExactComparable: TdxBoxComparableFunc;
  const AIsIntersect: TdxIsIntersectedWithPrevBoxFunc;
...
begin
  AComparable := TdxBoxLogPositionComparable.Create(PieceTable, LogPosition);
  Result := FindBoxIndex(ACollection, AComparable.BoxStartAndLogPositionCompare, AExactComparable, AIsIntersect, AStartIndex, AEndIndex);
end;

function TdxDocumentLayoutPosition.FindBoxIndex(ACollection: TdxBoxList;
  const AComparable, AExactComparable: TdxBoxComparableFunc;
  const AIsIntersect: TdxIsIntersectedWithPrevBoxFunc;
  AStartIndex, AEndIndex: Integer): Integer;
var
  ABoxIndex: Integer;
...
begin
  ...
  if not TdxAlgorithms1<TdxTableCellViewInfo>.BinarySearch(ACells, AComparable1.CompareTo, AStartIndex, AEndIndex, ACellIndex) then
  begin
    ...
    if not TdxAlgorithms1<TdxTableCellViewInfo>.BinarySearch(ACells, AComparable2.CompareTo, AStartIndex, AEndIndex, ACellIndex) then
      Exit(nil); 
  end;
...
begin
  ...
  Result := FindBoxIndex(AAreas,
    AComparable.ExactPageAreaAndLogPositionCompare, IsIntersectedWithPrevArea,
...
begin
  AComparable := TdxBoxLogPositionComparable.Create(PieceTable, LogPosition);
  TdxAlgorithms1<TdxBoxBase>.BinarySearch(ARows, AComparable.BoxAndLogPositionCompare, AStartIndex, AEndIndex, Result);
...
begin
  AComparable := TdxExactPageComparable.Create(PieceTable, LogPosition);
  Result := FindBoxIndex(APages, AComparable.ExactPageAndLogPositionCompare, IsIntersectedWithPrevPage, AStartIndex, AEndIndex);
end;
...
procedure TdxDocumentLayoutPosition.SetBox(const Value: TdxBox);
begin
  if FBox <> Value then
  begin
    TdxBox.Release(FBox);
...
procedure TdxDocumentLayoutPosition.SetColumn(const Value: TdxColumn);
begin
  if FColumn <> Value then
  begin
    TdxReferencedObject.Release(FColumn);
    FColumn := Value;
    TdxReferencedObject.AddReference(FColumn);
  end;
end;
...
procedure TdxDocumentLayoutPosition.SetPageArea(const Value: TdxPageArea);
begin
  if FPageArea <> Value then
  begin
    TdxReferencedObject.Release(FPageArea);
    FPageArea := Value;
    TdxReferencedObject.AddReference(FPageArea);
  end;
end;
...
begin
  ...
  begin
    PageArea := LookupPageArea(APage, AStartIndex);
...
var
  ...
  AExactPageAndLogPositionComparable: TdxExactPageComparable;
  ABoxStartAndLogPositionComparable: TdxBoxLogPositionComparable;
begin
  ...
  try
    ...
    ABoxStartAndLogPositionComparable := TdxBoxLogPositionComparable.Create(FAnchorPieceTable, AAnchorLogPosition);
    AExactPageAndLogPositionComparable := TdxExactPageComparable.Create(FAnchorPieceTable, AAnchorLogPosition);
    Result := FindBoxIndex(APages, ABoxStartAndLogPositionComparable.BoxStartAndLogPositionCompare,
      AExactPageAndLogPositionComparable.ExactPageAndLogPositionCompare,
...
begin
  ...
  begin
    AFloatingObjectBox := AFloatingObjects.List[I];
    if AFloatingObjectBox.GetFloatingObjectRun = ARun then
      Exit(AFloatingObjectBox.DocumentLayout.Pages.First.Areas.First);
  end;
  Result := nil;
end;
