...
unit dxRichEdit.DocumentModel.Commands; // for internal use
...
const
  dxThisUnitName = 'dxRichEdit.DocumentModel.Commands';
...
var
  AIsExactly, AIsPreviousExactly: Boolean;
  I, AGridIntervalsCount, ATotalLength, APreviousTotalLength, APreviousDelta, AColumnIndex: Integer;
begin
  ...
  begin
    ...
    APreviousDelta := Abs(ADistanceToCell - APreviousTotalLength);
    AIsExactly := ADistanceToCell = ATotalLength;
    AIsPreviousExactly := APreviousDelta <= FDeltaBetweenColumnsUpdate;
    if ADistanceToCell <= ATotalLength then
    begin
      if (not AIsExactly and AIsPreviousExactly) and (APreviousTotalLength > 0) then
        AColumnIndex := I - 1
      else
        AColumnIndex := I;
      Result.ColumnIndex := AColumnIndex;
      Result.IsExactly := (AIsExactly) or ((AIsPreviousExactly and (APreviousTotalLength > 0)));
      Exit;
    end;
...
begin
  inherited Create(APieceTable);
  FOldOwnNumberingListIndex := NumberingListIndexListIndexNotSet;
end;
...
begin
  ...
  ANewValue := NumberingListIndexListIndexNotSet;
