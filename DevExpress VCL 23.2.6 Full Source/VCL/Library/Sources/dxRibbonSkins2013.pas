...
uses
  ...
  dxBar, dxBarSkin, dxBarSkinConsts, cxLookAndFeelPainters, dxRibbonSkins, dxRibbonSkins2010, dxCoreGraphics,
  cxLookAndFeels;
...
type
  ...
  public
    ...
    procedure DrawStatusBarCore(DC: HDC; const R: TRect; AColor: TColor);
...
const
  dxThisUnitName = 'dxRibbonSkins2013';

{$R dxRibbonSkins2013.res}

{ TdxCustomRibbon2013Skin }

procedure TdxCustomRibbon2013Skin.DrawStatusBarPanel(DC: HDC; const AStatusBarBounds, R: TRect; AIsLowered: Boolean);
var
  ASavedIndex: Integer;
begin
  ...
  try
    if cxIntersectClipRect(DC, R) then
      DrawStatusBar(DC, AStatusBarBounds, False, False);
  finally
  ...
  if LowColors then
    ...
    case AState of
      rabsNormal, rabsPressed:
        dxGpFillRect(DC, R, GetMenuBackgroundColor); 
      rabsHot:
        dxGpFillRect(DC, R, GetAccentColor(0.1)); 
    end;
end;
...
procedure TdxCustomRibbon2013Skin.DrawBackstageViewBackButton(DC: HDC; const R: TRect; AState: Integer);
  ...
  var
    ...
    ARect: TRect;
  begin
    AImage := TdxSmartImage.CreateSize(APart.Size.cx + 2 * SizeOfSmoothingZone, APart.Size.cy + 2 * SizeOfSmoothingZone, 0);
    try
...
begin
  if LowColors then
    ...
    AFont.Color := TdxColorUtils.ChangeLightness(AContextColor, -0.2);
end;
...
begin
  ...
  begin
    R1 := cxRectSetHeight(R, R.Height - GetPartSize(DXBAR_TABSGROUPSOVERLAPHEIGHT));
    dxGpFillRect(DC, R1, AContextColor);
    dxGpFillRect(DC, R1, clWhite, 200);
    DrawTab(DC, R, AState);
  end;
...
procedure TdxCustomRibbon2013Skin.DrawGalleryFilterBandBackground(DC: HDC; const R: TRect; AState: Integer);
begin
  if LowColors then
...
begin
  ...
  begin
    ...
    try
      ...
      FillRectByColor(DC, ABounds, GetFormBorderColor(AIsActive));
    finally
      RestoreDC(DC, ASaveIndex)
    end;
  end;
...
begin
  ...
  begin
    AData := FormPaintData;
    AIsActive := AData.GetIsActive;
    ACaptionColor := GetFormCaptionAreaColor(PaintData.GetApplicationMenuState, AIsActive);
    if AData.UseNavigationControlCompatibleBorders then
      FillRectByColor(DC, R, ACaptionColor)
    else
      DrawFrame(DC, R, ACaptionColor, GetFormBorderColor(AIsActive), BordersMap[AData.GetState = wsMinimized]);
  end;
end;
...
begin
  ...
  begin
    ...
    FillGradientRect(DC, cxRectSetBottom(R, R.Bottom, R.Height - ARibbonHeight),
...
begin
  case AState of
    DXBAR_HOT:
      Result := GetHighlightBorderColor;
    DXBAR_PRESSED:
      Result := TdxColorUtils.ChangeLightness(GetHighlightBorderColor, -0.1);
  else
...
procedure TdxCustomRibbon2013Skin.DrawStatusBar(DC: HDC; const R: TRect; AIsLeftSideRaised, AIsRightSideRaised: Boolean);
begin
  if LowColors then
    inherited
  else
    DrawStatusBarCore(DC, R, GetMenuBackgroundColor);
end;

procedure TdxCustomRibbon2013Skin.DrawStatusBarCore(DC: HDC; const R: TRect; AColor: TColor);
var
  ABorderColor: TColor;
  ABorders: TRect;
begin
  ABorders := GetWindowBordersWidth(True);
  ABorderColor := GetFormBorderColor((FormPaintData = nil) or FormPaintData.GetIsActive);
  FillRectByColor(DC, R, AColor);
  FillRectByColor(DC, cxRectSetWidth(R, ABorders.Left), ABorderColor);
  FillRectByColor(DC, cxRectSetRight(R, R.Right, ABorders.Right), ABorderColor);
end;

procedure TdxCustomRibbon2013Skin.DrawStatusBarGripBackground(DC: HDC; const AStatusBarBounds, R: TRect);
begin
  // do nothing
end;
...
begin
  if LowColors then
    ...
    begin
      ...
      if UseRightToLeftAlignment then
        R1 := TdxRightToLeftLayoutConverter.ConvertRect(R1, R);
      R1 := cxRectSetHeight(R1, R1.Height - 4);
      FillRectByColor(DC, R1, GetFrameColor);
    end;
end;
...
begin
  case APart of
    ...
    DXBAR_BACKSTAGEVIEW_MENUBAR:
      Result := ScaleFactor.Apply(TRect.Create(0, 18, 0, 0));
    DXBAR_BACKSTAGEVIEW_MENUBAR_ITEM:
      Result := ScaleFactor.Apply(TRect.Create(16, 0, 0, 0));
    DXBAR_BACKSTAGEVIEW_MENUBAR_TAB:
      Result := ScaleFactor.Apply(TRect.Create(21, 11, 11, 11));
    DXBAR_TOOLBAR:
      Result := TRect.Create(2, 2, 3, 2);
    DXBAR_RIBBONTABGROUP, DXBAR_RIBBONCONTEXTTABGROUP:
      Result := TRect.Create(0, 2, 2, 3);
  else
        ...
        DXBAR_ACTIVE, DXBAR_HOT, DXBAR_FOCUSED:
          begin
            ...
            if ColorSchemeAccent = rcsaYellow then
              Result := TdxColorUtils.ChangeLightness(Result, -0.2);
          end;
      else
        Result := GetPartColor(DXBAR_ITEMTEXT, AState);
      end;
    ...
    rtatpEditBackground:
      begin
        ...
        if not (AState in [DXBAR_NORMAL, DXBAR_DISABLED]) then
          Result := TdxColorUtils.ChangeLightness(Result, -0.082);
      end;
...
begin
  ...
  LoadElementParts(ABitmap, FBackstageViewBackButton, TRect.Create(0, 0, 40, 40), rbvpBackstageViewBackButton, cxNullRect, [0, 1], [0, 1]);
end;
...
begin
  Result := TdxColorUtils.ChangeLightness(GetAccentColor, ALightnessDelta);
end;
...
function TdxCustomRibbon2013Skin.GetFormCornerColor(AActive: Boolean; ACorner: TdxCorner): TdxAlphaColor;
begin
  Result := dxColorToAlphaColor(GetFormBorderColor(AActive));
end;
...
begin
  case AState of
    DXBAR_HOT, DXBAR_ACTIVEDISABLED:
      Result := TdxColorUtils.ChangeLightness(GetHighlightBorderColor, -0.3);
    DXBAR_PRESSED:
      Result := TdxColorUtils.ChangeLightness(GetHighlightBorderColor, -0.4);
    else
      Result := $777777;
  end;
end;
...
begin
  Result := TdxColorUtils.ChangeLightness(GetMenuBackgroundColor, ALightnessDelta);
end;
