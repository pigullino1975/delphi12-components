...
uses
  SysUtils, Types, Generics.Defaults, Generics.Collections, dxCore, dxCoreClasses, dxPDFBase, dxPDFTypes, dxPDFParser;
...
type
  ...
  TdxPDFCMapRecordList = class(TdxFastObjectList);
  ...
  strict private
    ...
    procedure PopulateTree(ACMapTable: TdxPDFCMapRecordList);
  public
    Data: TBytes;

    constructor Create(ACMapTable: TdxPDFCMapRecordList; AMaxByteCount: Integer);
    destructor Destroy; override;

    class function CreateCharacterMappingData(ACharMap: TdxPDFIntegerStringDictionary; const AFontName: string): TBytes;

    function GetStringData(const AData: TBytes; const AGlyphOffsets: TDoubleDynArray): TdxPDFStringCommandData;
  ...
  strict private
    ...
    procedure PopulateCMapTable(ACMapTable: TdxPDFCMapRecordList);
    procedure ReadChars(ACMapTable: TdxPDFCMapRecordList; ACount: Integer);
    procedure ReadRange(ACMapTable: TdxPDFCMapRecordList; var ACode, ALastCode: TBytes);
    procedure ReadRangeCodes(var ACode, ALastCode: TBytes);
    procedure ReadRanges(ACMapTable: TdxPDFCMapRecordList; ACount: Integer);
  strict protected
  ...
  strict private
    ...
    FMapping: TdxPDFIntegerStringDictionary;
  public
    constructor Create;
    destructor Destroy; override;

    procedure RegisterString(AToUnicode: TdxPDFIntegerStringDictionary);

    property HasAddedCharacters: Boolean read FHasAddedCharacters write FHasAddedCharacters;
    property Mapping: TdxPDFIntegerStringDictionary read FMapping;
  end;
  ...
  public
    ...
    class function Create(AGlyphRun: TdxPDFGlyphRun;
      AMapping: TdxPDFIntegerStringDictionary): TdxPDFGlyphMappingResult; overload; static;
...
uses
  RTLConsts, Classes, Variants, Math, dxStringHelper, dxPDFUtils, dxPDFFont, dxPDFFontUtils, dxFontFile;

const
  dxThisUnitName = 'dxPDFCharacterMapping';
...
constructor TdxPDFCharacterMapping.Create(ACMapTable: TdxPDFCMapRecordList; AMaxByteCount: Integer);
begin
  inherited Create;
...
class function TdxPDFCharacterMapping.CreateCharacterMappingData(ACharMap: TdxPDFIntegerStringDictionary;
...
function TdxPDFCharacterMapping.GetStringData(const AData: TBytes;
  const AGlyphOffsets: TDoubleDynArray): TdxPDFStringCommandData;
var
  ACodes: TdxPDFBytesList;
...
begin
  ...
  try
    ...
    begin
      ...
      TdxPDFUtils.AddData(AActualOffsets, AOffsets);
    end
    else
    begin
      SetLength(AOffsets, 0);
    ...
    SetLength(ACharCodes, ACodes.Count);
    for I := 0 to ACodes.Count - 1 do
      ACharCodes[I] := ACodes[I];

    SetLength(AStr, AResult.Count);
...
procedure TdxPDFCharacterMapping.PopulateTree(ACMapTable: TdxPDFCMapRecordList);
var
  ARecord: TdxPDFCMapRecord;
  I: Integer;
begin
  for I := 0 to ACMapTable.Count - 1 do
  begin
    ARecord := TdxPDFCMapRecord(ACMapTable[I]);
    FMaxCodeLength := Max(FMaxCodeLength, Length(ARecord.Code));
    FTree.AddCode(ARecord.Code, 0, ARecord.Value);
  end;
end;
...
procedure TdxPDFCMapStreamParser.ReadChars(ACMapTable: TdxPDFCMapRecordList; ACount: Integer);
var
  I: Integer;
  AValue: TdxPDFBase;
...
procedure TdxPDFCMapStreamParser.ReadRange(ACMapTable: TdxPDFCMapRecordList; var ACode, ALastCode: TBytes);
var
  I, AValue: Integer;
  ARange: TdxPDFBase;
  AUnicode: string;
...
procedure TdxPDFCMapStreamParser.ReadRanges(ACMapTable: TdxPDFCMapRecordList; ACount: Integer);
var
  I: Integer;
...
procedure TdxPDFCMapStreamParser.PopulateCMapTable(ACMapTable: TdxPDFCMapRecordList);
var
...
var
  ACMapTable: TdxPDFCMapRecordList;
begin
  ACMapTable := TdxPDFCMapRecordList.Create;
  try
...
begin
  ...
  FMapping := TdxPDFIntegerStringDictionary.Create;
end;
...
procedure TdxPDFCharacterCache.RegisterString(AToUnicode: TdxPDFIntegerStringDictionary);
var
  ACode: Integer;
  APair: TPair<Integer, string>;
begin
...
class function TdxPDFGlyphMappingResult.Create(AGlyphRun: TdxPDFGlyphRun;
  AMapping: TdxPDFIntegerStringDictionary): TdxPDFGlyphMappingResult;
begin
...
constructor TdxPDFToUnicodeMap.Create(const AData: TBytes);
begin
  inherited Create;
  FData := AData;
end;
