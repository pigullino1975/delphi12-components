...
uses
  Windows, ActiveX, Classes, ComCtrls, CommCtrl, ComObj, Controls, Dialogs, Themes,
  ...
  dxThemeManager, dxUxTheme, dxThemeConsts, cxEdit, cxImageList, Generics.Collections, Generics.Defaults,
  cxTextEdit, Clipbrd, StrUtils;
...
type
  ...
  protected
    ...
    function GetEnumerationObjectParentWindow: HWND; override;
  ...
  private
    function GetItem(Index: TdxListIndex): TWinControl;
  protected
  ...
  public
    ...
    //
    property Items[Index: TdxListIndex]: TWinControl read GetItem; default;
  end;
  ...
  TcxCustomInnerShellListView = class(TcxBaseInnerListView,
    IDropTarget, IcxShellDependedControls, IcxShellRoot)
  private
  ...
  protected
    ...
    property CurrentDropTarget: IDropTarget read FCurrentDropTarget write FCurrentDropTarget;
  ...
  public
    ...
    property DependedControls: TcxShellDependedControls read FDependedControls write FDependedControls; // for internal use
    ...
    property Root: TcxShellListRoot read GetRootField write SetRootField; // for internal use
  ...
  TdxCustomShellTreeItemProducer = class(TcxCustomItemProducer)
  strict private
    FShellItemInfo: TcxShellItemInfo;
    FOnDestroy: TNotifyEvent;
  protected
    function CreateFakeProducer: TdxCustomShellTreeItemProducer; virtual; abstract;
  ...
  TcxShellTreeItemProducer = class(TdxCustomShellTreeItemProducer)
  private
    FNode: TTreeNode;
    //
  ...
  public
    ...
    procedure ProcessItems(AIFolder: IShellFolder; APIDL: PItemIDList; ANode: TTreeNode;
  ...
  public
    ...
    property DependedControls: TcxShellDependedControls read FDependedControls; // for internal use
    property DragDropSettings:TcxDragDropSettings read FDragDropSettings write FDragDropSettings;
    property FolderCount: Integer read GetFolderCount;
    property Folders[AIndex: Integer]: TcxShellFolder read GetFolder;
    property ListView: TcxCustomInnerShellListView read FListView write SetListView; // for internal use
    property Options: TcxShellTreeViewOptions read FOptions write FOptions;
    property Root: TcxShellTreeRoot read GetRootField write SetRootField; // for internal use
  ...
  TdxShellControlGroups = class
  strict private class var
    FGroups: TObjectDictionary<Integer, TList<TWinControl>>;
    FLockCount: Integer;
    class function IsGroupExists(AIndex: Integer; out AGroup: TList<TWinControl>): Boolean;
  private
    class procedure Initialize; static;
    class procedure Finalize; static;
  protected
  public
    class procedure AddControl(AControl: TWinControl; AIndex: Integer);
    class procedure AbsolutePidlChanged(AControl: TWinControl; APidl: PItemIDList; AIndex: Integer);
    class procedure RemoveControl(AControl: TWinControl; AIndex: Integer);
    class procedure RootChanged(AControl: TWinControl; ARoot: TcxCustomShellRoot; AIndex: Integer);
  end;
...
function cxShellGetContextMenuCommandStrByShortCut(Key: Word; Shift: TShiftState): AnsiString; // for internal use only
procedure cxShellGetNotifyParams(Message: TMessage; out AEventID: Integer; out APidl1, APidl2: PItemIDList);
procedure cxShellInvokeContextMenuCommand(AShellFolder: IShellFolder; AItemPIDLList: TList;
  const ACommandStr: AnsiString; AParentWindow: HWND = 0); // for internal use only
function cxShellIsClipboardCommandContextMenuShortCut(Key: Word; Shift: TShiftState): Boolean; // for internal use only
...
function cxGetFileNameFromClipboard: string; // for internal use only
procedure cxPasteFileNameFromClipboard(AEditor: TcxCustomTextEdit); // for internal use only

implementation

uses
  Forms, Math, RTLConsts, cxGeometry, dxDPIAwareUtils, dxCoreClasses;

const
  dxThisUnitName = 'cxShellControls';
...
procedure cxShellInvokeContextMenuCommand(AShellFolder: IShellFolder; AItemPIDLList: TList; const ACommandStr: AnsiString; AParentWindow: HWND = 0);
var
  ...
  begin
    ...
    try
      ...
      begin
        ...
        AInvokeCommandInfo.hwnd := AParentWindow;
        ...
        begin
          EnableWindow(GetAncestor(AParentWindow, GA_ROOT), False);
          try
            AIContextMenu.InvokeCommand(AInvokeCommandInfo);
          finally
            EnableWindow(GetAncestor(AParentWindow, GA_ROOT), True);
          end;
        end;
      end;
    finally
...
function cxGetFileNameFromClipboard: string; // for internal use only
var
  AClipboardHandle: THandle;
  ABuffer: Pointer;
  ACharCount, AFilesCount: Integer;
begin
  Result := '';
  ...
  Clipboard.Open;
  try
    AClipboardHandle := Clipboard.GetAsHandle(CF_HDROP);
    if AClipboardHandle <> 0 then
    begin
      AFilesCount := DragQueryFile(AClipboardHandle, $FFFFFFFF, nil, 0);
      if AFilesCount > 0 then
      begin
        ACharCount := DragQueryFile(AClipboardHandle, 0, nil, 0);
        if ACharCount > 0 then
        begin
          Inc(ACharCount); 
          GetMem(ABuffer, ACharCount * SizeOf(Char));
          try
            if DragQueryFile(AClipboardHandle, 0, ABuffer, ACharCount) > 0 then
              Result := PChar(ABuffer);
          finally
            FreeMem(ABuffer);
          end;
        end;
...
procedure cxPasteFileNameFromClipboard(AEditor: TcxCustomTextEdit); // for internal use only
var
  ASourceString, ADestinationString: string;
  ALength: Integer;
begin
  if (AEditor.Properties.MaxLength > 0) and (Length(AEditor.Text) >= AEditor.Properties.MaxLength) then
    Exit;
  ASourceString := cxGetFileNameFromClipboard;
  if ASourceString <> '' then
  begin
    ALength := Length(ASourceString);
    ADestinationString := AEditor.Text;
    Delete(ADestinationString, AEditor.SelStart + 1, AEditor.SelLength);
    Insert(ASourceString, ADestinationString, AEditor.SelStart + 1);
    ALength := AEditor.SelStart + ALength;
    if AEditor.Properties.MaxLength > 0 then
    begin
      if ALength > AEditor.Properties.MaxLength then
        ALength := AEditor.Properties.MaxLength;
      if Length(ADestinationString) > AEditor.Properties.MaxLength then
        ADestinationString := LeftStr(ADestinationString, AEditor.Properties.MaxLength);
    end;
    AEditor.Text := ADestinationString;
    AEditor.SelStart := ALength;
  end;
end;
...
procedure TcxShellListViewOptions.DoAssign(Source: TcxShellOptions);
begin
...
begin
  if CanAllocateHandle(AControl) then
  begin
...
function TcxShellDependedControls.GetItem(Index: TdxListIndex): TWinControl;
begin
...
procedure TcxCustomInnerShellListView.AdvancedDrawItem(Sender: TCustomListView; Item: TListItem;
  ...
  begin
    ...
    if (Item.Selected or (cdsHot in State)) and not IsWinXP and IsXPManifestEnabled and AreVisualStylesAvailable([totMenu]) then
      dxDrawThemeBackground(ACanvas.Handle, totMenu, dxGetScaleFactor(Parent), ABounds, MENU_POPUPITEM, GetItemState)
  end;
  ...
  begin
    ...
    if IsWinVistaOrLater and not cxIsVCLThemesEnabled then
      TdxImageDrawer.DrawUncachedImage(ACanvas.Handle, ARect, AIconRect, nil, ItemProducer.FThumbnails,  AShellItem.ThumbnailIndex, idmNormal)
    else
  ...
  begin
    ...
    AFlags := DT_CENTER or DT_WORDBREAK or DT_NOPREFIX;
    if Item.Selected then
...
procedure TcxCustomInnerShellListView.GetDropTarget(pt: TPoint; out New: Boolean);
  ...
  begin
    Result := -1;
    P := dxMapWindowPoint(HWND_DESKTOP, Handle, pt);
...
function TcxShellListViewProducer.GetThumbnailIndex(AItem: TcxShellItemInfo): Integer;
  ...
  procedure AddThumbnailToImageList(AHBitmap: HBITMAP; const ARequiredSize: TSize);
  var
    ABitmap: TcxBitmap;
    AThumbnail: TBitmap;
    ARect: TRect;
...
function TcxShellListViewProducer.GetEnumerationObjectParentWindow: HWND;
begin
  Result := Owner.ParentWindow;
end;
 ...
 // if (FTopNode <> ANode) or (AVisibleCount > FMaxVisibleNodeCount) then
  begin
    ...
    try
      ...
      FItemsInfoGatherer.UpdateRequest(AItems);
    finally
      FItemsInfoGatherer.ProcessedItems.UnlockList;
      AItems.Free;
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      ATempPidl := GetPidlCopy(ATempPidl);
      try
        DependedControls.Navigate(ATempPIDL);
      finally
        DisposePidl(ATempPidl);
...
procedure TcxCustomInnerShellTreeView.GetDropTarget(out ANew: Boolean; APoint: TPoint);
  ...
  function InternalGetDropTarget(ANode: TTreeNode; out ADropTarget: IDropTarget): Boolean;
  var
    AItemProducer: TcxShellTreeItemProducer;
...
procedure TcxCustomInnerShellTreeView.ChangeScale(M, D: Integer{$IFDEF DELPHIBERLIN}; isDpiChange: Boolean{$ENDIF});
{$IFDEF DELPHI103}
var
  ASafeIndent: Integer;
{$ENDIF}
...
destructor TdxCustomShellTreeItemProducer.Destroy;
begin
...
function TdxCustomShellTreeItemProducer.CheckUpdates: Boolean;

  procedure MergeItems(AFakeProducer: TdxCustomShellTreeItemProducer);
  var
    I, J: Integer;
    AItem, ANewItem: TcxShellItemInfo;
    AFounded: Boolean;
    ANewItems: TdxFastList;
  begin
...
var
  AFakeProducer: TdxCustomShellTreeItemProducer;
begin
  ShowHourglassCursor;
  AFakeProducer := CreateFakeProducer;
  try
    LockWrite;
    try
...
procedure TdxCustomShellTreeItemProducer.CheckForSubitems(AItem: TcxShellItemInfo);
begin
...
procedure TdxCustomShellTreeItemProducer.DoSlowInitialization(AItem: TcxShellItemInfo);
begin
  InitializeItem(AItem);
end;

procedure TdxCustomShellTreeItemProducer.InitializeItem(AItem: TcxShellItemInfo);
begin
  inherited InitializeItem(AItem);
  CheckForSubitems(AItem);
end;

function TdxCustomShellTreeItemProducer.SlowInitializationDone(AItem: TcxShellItemInfo): Boolean;
begin
  Result := AItem.Updated and (AItem.IsSubitemsChecked or AItem.IsRemovable);
end;
...
function TcxShellTreeItemProducer.CreateFakeProducer: TdxCustomShellTreeItemProducer;
begin
  Result := TcxShellTreeItemProducer.Create(TreeView);
end;

function TcxShellTreeItemProducer.GetEnumerationObjectParentWindow: HWND;
begin
  Result := Owner.ParentWindow;
end;
...
{ TdxShellControlGroups }

class procedure TdxShellControlGroups.Finalize;
begin
  FreeAndNil(FGroups);
end;

class procedure TdxShellControlGroups.Initialize;
begin
  FGroups := TObjectDictionary<Integer, TList<TWinControl>>.Create([doOwnsValues]);
end;

class function TdxShellControlGroups.IsGroupExists(AIndex: Integer;
  out AGroup: TList<TWinControl>): Boolean;
begin
  Result := FGroups.TryGetValue(AIndex, AGroup);
end;

class procedure TdxShellControlGroups.RemoveControl(AControl: TWinControl;
  AIndex: Integer);
var
  AGroup: TList<TWinControl>;
begin
   if IsGroupExists(AIndex, AGroup) then
   begin
     AGroup.Remove(AControl);
     if AGroup.Count = 0 then
       FGroups.Remove(AIndex);
   end;
end;

class procedure TdxShellControlGroups.RootChanged(AControl: TWinControl;
  ARoot: TcxCustomShellRoot; AIndex: Integer);
var
  AGroup: TList<TWinControl>;
  I: Integer;
  ANotifiedControl: TWinControl;
begin
  if FLockCount = 0 then
  begin
    Inc(FLockCount);
    try
      if IsGroupExists(AIndex, AGroup) and (AGroup.Count > 1) then
      begin
        for I := 0 to AGroup.Count - 1 do
        begin
          ANotifiedControl := AGroup[I];
          if (ANotifiedControl <> AControl) and ANotifiedControl.HandleAllocated then
            SendMessage(ANotifiedControl.Handle, DSM_SYNCHRONIZEROOT, WPARAM(ARoot), 0);
        end;
      end;
    finally
      Dec(FLockCount);
...
class procedure TdxShellControlGroups.AddControl(AControl: TWinControl; AIndex: Integer);
var
  AGroup: TList<TWinControl>;
  AIcxShellRoot: IcxShellRoot;
begin
  if AIndex >= 0 then
  begin
    if not IsGroupExists(AIndex, AGroup) then
    begin
      AGroup := TList<TWinControl>.Create;
      FGroups.Add(AIndex, AGroup);
    end;
    AGroup.Add(AControl);
    if Supports(AControl, IcxShellRoot, AIcxShellRoot) then
      RootChanged(AControl, AIcxShellRoot.GetRoot, AIndex);
  end;
end;

class procedure TdxShellControlGroups.AbsolutePidlChanged(AControl: TWinControl; APidl: PItemIDList; AIndex: Integer);
var
  AGroup: TList<TWinControl>;
  I: Integer;
  ANotifiedControl: TWinControl;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        begin
          ANotifiedControl := AGroup[I];
          if (ANotifiedControl <> AControl) and CanAllocateHandle(ANotifiedControl) then
          begin
            ANotifiedControl.HandleNeeded;
            SendMessage(ANotifiedControl.Handle, DSM_DONAVIGATE, WPARAM(APidl), 0);
          end;
        end;
      end;
    finally
...
initialization
  ...
  NavigationLock := False;
  TdxShellControlGroups.Initialize;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.InitializationFinished(dxThisUnitName, SysInit.HInstance);{$ENDIF}
finalization
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.FinalizationStarted(dxThisUnitName, SysInit.HInstance);{$ENDIF}
  TdxShellControlGroups.Finalize;
