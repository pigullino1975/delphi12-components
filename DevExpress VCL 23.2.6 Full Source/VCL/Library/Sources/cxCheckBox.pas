...
uses
  ...
  cxDropDownEdit, cxVariants, cxFilterControlUtils, cxLookAndFeelPainters, dxFading, cxAccessibility;
...
type
  ...
  TcxCustomCheckBoxViewInfo = class(TcxCustomEditViewInfo)
  private
  ...
  protected
    ...
    class function IsOldStyleSupported: Boolean; virtual;
    class function GetContentOffset(AIsInplace, AIsCaptionVisible: Boolean; AItemAlignment: TLeftRight; AScaleFactor: TdxScaleFactor): TRect; virtual;
    class function GetTextGap(AIsInplace, ANativeStyle: Boolean; AScaleFactor: TdxScaleFactor): Integer; virtual;
    class function GetTextRect(const ABounds, ACheckBoxRect: TRect; AItemAlignment: TAlignment;
      AIsCaptionVisible, AIsInplace, ANativeStyle: Boolean; AFont: TFont; AScaleFactor: TdxScaleFactor): TRect;
  public
  ...
  private
    ...
    function GetBiDiModeDependedAlignment: TAlignment;
  ...
  protected
    ...
    property BiDiModeDependedAlignment: TAlignment read GetBiDiModeDependedAlignment;
  public
    ...
    function GetTextGap(AIsInplace, ANativeStyle: Boolean; AScaleFactor: TdxScaleFactor): Integer; virtual;
    function GetContentOffset(AIsCaptionVisible: Boolean): TRect; virtual;
...
//      write SetAlignmentVert stored IsAlignmentVertStored;
  ...
  { TdxCheckBoxAccessibilityHelper }

  TdxCheckBoxAccessibilityHelper = class(TdxEditAccessibilityHelper) // for internal use
  protected
  ...
  public
    ...
    property Style: TcxCheckBoxStyle read GetStyle write SetStyle; // for internal use
  ...
  published
    ...
    property ShowHint;
    property State;
    property Style; // for internal use
...
procedure DrawCheckBoxText(ACanvas: TcxCanvas; AText: string; AFont: TFont; ATextColor: TColor; ATextRect: TRect;
  ADrawTextFlags: Integer; AEnabled: Boolean; APaintOnGlass: Boolean = False);

implementation

uses
  Math, ActnList, dxThemeConsts, dxThemeManager, dxUxTheme, cxEditConsts, dxCoreGraphics,
  cxEditPaintUtils, cxDrawTextUtils, cxEditUtils, dxSmartImage;

const
  dxThisUnitName = 'cxCheckBox';
...
procedure DrawCheckBoxText(ACanvas: TcxCanvas; AText: string; AFont: TFont;
  ATextColor: TColor; ATextRect: TRect; ADrawTextFlags: Integer; AEnabled: Boolean; APaintOnGlass: Boolean = False);
begin
  if APaintOnGlass  then
    dxDrawTextOnGlass(ACanvas.Handle, AText, AFont, ATextRect, ATextColor, cxFlagsToDTFlags(ADrawTextFlags), 0, True)
  else
  begin
    ACanvas.Font := AFont;
  ...
  begin
    ...
    if AViewInfo.Alignment <> taCenter then
      DrawCheckBoxText(ACanvas, AViewInfo.Text, AViewInfo.Font, AViewInfo.TextColor,
        AViewInfo.TextRect, AViewInfo.DrawTextFlags, AViewInfo.IsTextEnabled, AViewInfo.PaintOnGlass);
  end;

  if (not IsRectEmpty(AViewInfo.FocusRect)) and AViewInfo.Painter.SupportsNativeFocusRect(TdxPaintPartID.CheckBox) then
...
procedure DrawScaledEditCheck(ACanvas: TcxCanvas; const ACheckRect: TRect;
  ...
  begin
    ...
    try
      ...
      TdxImageDrawer.DrawImage(ACanvas, ACheckRect, ABitmap, nil, -1, ifmStretch,
  ...
  begin
    ...
    cxIntersectClipRect(ACanvas.Handle, AContentRect);
...
begin
  ...
  begin
    ...
    begin
      AButtonState := CheckStateToButtonState[ACheckState];
      if ADrawBackground then
        ACanvas.FillRect(ACheckRect, ABackgroundColor);
      if AFocused and (AButtonState = cxbsNormal) then
        AButtonState := cxbsDefault;
      APainter.DrawScaledCheckButton(ACanvas, ACheckRect, AButtonState, AState, AScaleFactor);
    end
    else
    begin
      DrawCheckBoxBorder;
...
begin
  ...
  begin
    AStateCount := Integer(High(TcxCheckBoxState)) - Integer(Low(TcxCheckBoxState)) + 1;
...
function TcxCustomCheckBoxViewInfo.IsCaptionVisible: Boolean;
begin
  Result := (Alignment <> taCenter) and not IsInplace and (Text <> '');
end;
...
function TcxCustomCheckBoxViewInfo.SupportsBorders: Boolean;
begin
  Result := True;
end;

function TcxCustomCheckBoxViewInfo.CalculateTextRect: TRect;
begin
  Result := GetTextRect(ClientRect, CheckBoxRect, Alignment, IsCaptionVisible, IsInplace, NativeStyle, Font, ScaleFactor);
end;

class function TcxCustomCheckBoxViewInfo.IsOldStyleSupported: Boolean;
begin
  Result := True;
end;

class function TcxCustomCheckBoxViewInfo.GetContentOffset(AIsInplace, AIsCaptionVisible: Boolean; AItemAlignment: TLeftRight; AScaleFactor: TdxScaleFactor): TRect;
begin
  if AIsInplace then
    Result := Rect(1, 1, 1, 1)
  else
    if not AIsCaptionVisible then
      Result := Rect(0, 2, 0, 2)
    else
      case AItemAlignment of
        taLeftJustify:
          Result := Rect(0, 2, 2, 2);
        taRightJustify:
          Result := Rect(2, 2, 0, 2);
...
class function TcxCustomCheckBoxViewInfo.GetTextGap(AIsInplace, ANativeStyle: Boolean; AScaleFactor: TdxScaleFactor): Integer;
const
  ACheckTextGap: array [Boolean] of Integer = (5, 3);
begin
  Result := AScaleFactor.Apply(ACheckTextGap[ANativeStyle or AIsInplace]);
end;

class function TcxCustomCheckBoxViewInfo.GetTextRect(const ABounds, ACheckBoxRect: TRect; AItemAlignment: TAlignment;
  AIsCaptionVisible, AIsInplace, ANativeStyle: Boolean; AFont: TFont; AScaleFactor: TdxScaleFactor): TRect;
var
  AOffset: TRect;
begin
  Result := ABounds;
  AOffset := GetContentOffset(AIsInplace, AIsCaptionVisible, AItemAlignment, AScaleFactor);
  InflateRect(Result, -AOffset.Left, 0);

  if IsOldStyleSupported and not AIsInplace and not ANativeStyle and not Odd(cxRectHeight(Result) - TdxTextMeasurer.TextLineHeight(AFont)) then
    OffsetRect(Result, 0, -1);

  if cxRectWidth(ACheckBoxRect) < cxRectWidth(ABounds) then
    case AItemAlignment of
      taLeftJustify:
        Result.Left := ACheckBoxRect.Right + GetTextGap(AIsInplace, ANativeStyle, AScaleFactor);
      taRightJustify:
        Result.Right := ACheckBoxRect.Left - GetTextGap(AIsInplace, ANativeStyle, AScaleFactor);
    end
  else
...
function TcxCustomCheckBoxViewInfo.GetSkinnedTextColor(AKind: TcxEditStateColorKind): TColor;
begin
...
begin
  ...
  CalculateViewInfo(ACheckBoxViewInfo, AIsMouseEvent);
  ACheckBoxViewInfo.TextRect := cxRectContent(AViewInfo.ClientRect, EditContentParams.Offsets);
  ...
  if IsInplace and not (Properties.IsEmbeddedEdit or
    Properties.UseAlignmentWhenInplace or (ACheckBoxViewInfo.Text <> '')) then
      FAlignment := taCenter;
  ACheckBoxViewInfo.Alignment := BiDiModeDependedAlignment;
...
function TcxCustomCheckBoxViewData.GetTextGap(AIsInplace, ANativeStyle: Boolean; AScaleFactor: TdxScaleFactor): Integer;
begin
  Result := TcxCustomCheckBoxViewInfo.GetTextGap(AIsInplace, ANativeStyle, AScaleFactor);
end;

function TcxCustomCheckBoxViewData.GetContentOffset(AIsCaptionVisible: Boolean): TRect;
begin
  Result := TcxCustomCheckBoxViewInfo.GetContentOffset(IsInplace, AIsCaptionVisible, BiDiModeDependedAlignment, ScaleFactor);
end;

procedure TcxCustomCheckBoxViewData.AdjustPadding(var APadding: TdxPadding);
begin
...
begin
  ...
  ASize1 := GetScaledEditCheckSize(ACanvas, NativeStyle, Properties.Glyph, Properties.GlyphCount, Style.LookAndFeel.Painter,
    ScaleFactor);
  AOffset := GetContentOffset((AViewInfo as TcxCustomCheckBoxViewInfo).IsCaptionVisible);
  Result.cx := Result.cx + ASize1.cx + AOffset.Left + AOffset.Right;
  MinContentSize.cy := ASize1.cy + AOffset.Top + AOffset.Bottom;
end;

procedure TcxCustomCheckBoxViewData.Initialize;
begin
  inherited;
  FAlignment := Properties.Alignment;
end;
...
begin
  ...
  try
    ...
    if AText <> '' then
    begin
      ARect := Rect(0, 0, Max(AEditSizeProperties.Width - GetTextGap(IsInplace, NativeStyle, ScaleFactor), 1), 1);
      ...
      Result.cx := Result.cx + GetTextGap(IsInplace, NativeStyle, ScaleFactor);
      AOffset := GetContentOffset(True);
      Result.cy := Result.cy + AOffset.Top + AOffset.Bottom;
    end
    else
      Result := cxNullSize;
  finally
...
begin
  ...
  begin
    if IsInplace and Properties.IsEmbeddedEdit then
      ...
      AOffset := GetContentOffset(AViewInfo.IsCaptionVisible);

    case AViewInfo.Alignment of
      taLeftJustify:
        begin
          Result.Left := AAvailableArea.Left + AOffset.Left;
          Result.Right := Result.Left + ACheckSize.cx;
        end;
      taRightJustify:
        begin
          Result.Right := AAvailableArea.Right - AOffset.Right;
...
procedure TcxCustomCheckBoxViewData.CalculateFocusRect(AViewInfo: TcxCustomCheckBoxViewInfo);
  ...
  begin
    AViewInfo.FocusRect.Left := Max(AViewInfo.FocusRect.Left, AViewInfo.ClientRect.Left);
    AViewInfo.FocusRect.Right := Min(AViewInfo.FocusRect.Right, AViewInfo.ClientRect.Right);
    if AViewInfo.IsCaptionVisible then
    begin
      if AViewInfo.Alignment = taLeftJustify then
        AViewInfo.FocusRect.Left := Max(AViewInfo.FocusRect.Left, AViewInfo.TextRect.Left - 1)
      else
        AViewInfo.FocusRect.Right := Min(AViewInfo.FocusRect.Right, AViewInfo.TextRect.Right + 1);
    end;
    AViewInfo.FocusRect.Top := Max(AViewInfo.FocusRect.Top, AViewInfo.ClientRect.Top);
    AViewInfo.FocusRect.Bottom := Min(AViewInfo.FocusRect.Bottom, AViewInfo.ClientRect.Bottom);
  end;

  function CanShowFocusRect: Boolean;
  begin
    if AViewInfo.Alignment <> taCenter then
      Result := not IsInplace or Properties.IsEmbeddedEdit
    else
      Result := IsInplace and (epoShowFocusRectWhenInplace in PaintOptions);// or AViewInfo.UseSkins;
  end;

begin
  AViewInfo.FocusRect := cxEmptyRect;
  if Focused and CanShowFocusRect then
  begin
    if AViewInfo.Alignment = taCenter then
      AViewInfo.FocusRect := cxRectInflate(AViewInfo.ClientRect, -1)
    else
      if AViewInfo.Text <> '' then
      begin
        if Properties.FullFocusRect then
        begin
          AViewInfo.FocusRect := AViewInfo.ClientRect;
          case AViewInfo.Alignment of
            taLeftJustify:
              AViewInfo.FocusRect.Left := AViewInfo.TextRect.Left - 1;
            taRightJustify:
              AViewInfo.FocusRect.Right := AViewInfo.TextRect.Right + 1;
          end;
        end
        else
        begin
          AViewInfo.FocusRect := AViewInfo.TextRect;
          cxMeasureCanvas.Font := AViewInfo.Font;
          cxMeasureCanvas.TextExtent(AViewInfo.Text, AViewInfo.FocusRect, AViewInfo.DrawTextFlags);
          AViewInfo.FocusRect := cxRectInflate(AViewInfo.FocusRect, 1);
          if not AViewInfo.NativeStyle then
...
begin
  Result := AViewInfo.CalculateTextRect;
...
function TcxCustomCheckBoxViewData.GetBiDiModeDependedAlignment: TAlignment;
begin
  Result := Alignment;
...
procedure TdxCheckBoxAccessibilityHelper.DoDefaultAction(AChildID: TcxAccessibleSimpleChildElementID);
begin
  TcxCustomCheckBox(Edit).Checked := not TcxCustomCheckBox(Edit).Checked;
end;

function TdxCheckBoxAccessibilityHelper.GetDefaultActionDescription(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := 'To switch';
end;

function TdxCheckBoxAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxROLE_SYSTEM_CHECKBUTTON;
end;

function TdxCheckBoxAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := inherited;
  if TcxCustomCheckBox(Edit).Checked then
    Result := Result or cxSTATE_SYSTEM_CHECKED;
end;

function TdxCheckBoxAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
...
function TcxCustomCheckBox.IsTransparentBackground: Boolean;
begin
  Result := inherited or (ViewInfo <> nil) and ViewInfo.IsBackgroundTransparent; 
end;

function TcxCustomCheckBox.GetDefaultSkinnedTextColor(AEnabled: Boolean): TColor;
var
...
begin
  ...
  if IsInplace and (Key = dxSpace) and (ViewInfo.CheckBoxState in [ecsNormal, ecsHot]) then
  begin
    Toggle;
    Key := #0;
  end;
...
function TcxCustomCheckBox.GetAccessibilityHelperClass: TdxEditAccessibilityHelperClass;
begin
  Result := TdxCheckBoxAccessibilityHelper;
end;
...
begin
  ...
  if (Owner = nil) or not (csLoading in Owner.ComponentState) then
...
begin
  ...
  begin
    ...
    if HandleAllocated then
      NotifyWinEvent(dxUIA_ToggleToggleStatePropertyId, Handle, OBJID_CLIENT, CHILDID_SELF);
  end;
  ShortRefreshContainer(False);
end;
...
begin
  ACheckBoxProperties := AEditProperties as TcxCustomCheckBoxProperties;
...
procedure TcxCheckBoxActionLink.SetHint(const Value: string);
begin
...
procedure TcxCheckBoxFadingHelper.GetFadingImages(out AFadeOutImage, AFadeInImage: TcxBitmap);
  ...
  begin
    Result := not ViewInfo.IsBackgroundTransparent and
      (ViewInfo.BackgroundColor <> clNone) and ViewInfo.UseSkins;
  end;
