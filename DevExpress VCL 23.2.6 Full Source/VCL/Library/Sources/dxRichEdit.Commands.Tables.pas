...
type
  ...
  protected
    ...
    class function GetInsertTableRowCommandClass: TdxInsertTableRowCoreCommandBaseClass; virtual; abstract;
  ...
  protected
    class function GetInsertTableColumnCommandClass: TdxInsertTableColumnCoreCommandBaseClass; virtual; abstract;
...
const
  dxThisUnitName = 'dxRichEdit.Commands.Tables';

type
  { TdxInsertTableInnerCommand }

  TdxInsertTableInnerCommand = class(TdxRichEditMenuItemSimpleCommand)
  strict private
...
class function TdxInsertTableInnerCommand.Id: TdxRichEditCommandId;
begin
  Result := TdxRichEditCommandId.InsertTableInner;
end;

procedure TdxInsertTableInnerCommand.ExecuteCore;
var
  ALastTable: TdxTable;
  AStyleIndex: Integer;
begin
  ActivePieceTable.InsertTable(StartPosition, RowCount, ColumnCount, TdxTableAutoFitBehaviorType.FixedColumnWidth,
    MinInt, ColumnWidth, GetForceVisible, ActiveView.MatchHorizontalTableIndentsToTextEdge);
  ALastTable := ActivePieceTable.Tables.Last;
  AStyleIndex := DocumentModel.TableStyles.GetTableSimpleStyle;
  if AStyleIndex >= 0 then
    ALastTable.StyleIndex := AStyleIndex;
  ActivePieceTable.JoinTableWithPrevious(ALastTable);
end;

procedure TdxInsertTableInnerCommand.UpdateUIStateCore(const AState: IdxCommandUIState);
begin
...
procedure TdxInsertTableCoreCommand.InsertTable;
var
  ACommand: TdxInsertTableInnerCommand;
begin
  ACommand := TdxInsertTableInnerCommand.Create(RichEditControl);
  try
    ACommand.StartPosition := DocumentModel.Selection.&End;
    ACommand.RowCount := RowCount;
    ACommand.ColumnCount := ColumnCount;
    ACommand.ColumnWidth := GetColumnWidth;
    ACommand.Execute;
  finally
    ACommand.Free;
  end;
...
begin
  ...
  if FIsNotEmptySelectedCells then
    PerformModifyModelCore(Safe<TdxSelectedCellsCollection>.Cast(DocumentModel.Selection.SelectedCells));
end;
