...
uses
  System.UITypes,
  Windows, Messages, Types, Classes, Graphics, SysUtils, Forms, Controls{must be after Forms for D11}, ImgList,
...
type
  ...
  public
    ...
    function GetWindowPadding: TRect;
    function GetHeaderPadding: TRect;
    function GetDescriptionPadding: TRect;
    function GetSeparatorPadding: TRect;
    function GetFooterPadding: TRect;
  ...
  strict private
    ...
    procedure PrepareFont(ACanvas: TCanvas);
  protected
    procedure CorrectMinWidth(ACanvas: TCanvas; APainter: TdxScreenTipPainter; var AWidth: Integer);
    function GetAvailGlyphWidth: Integer;
    function GetBandPadding(APainter: TdxScreenTipPainter): TRect;
    function GetGlyphIndent: Integer;
    function GetText: string;
    function HasGlyph: Boolean;
    function HasText: Boolean;
    procedure Paint(ACanvas: TcxCanvas; APainter: TdxScreenTipPainter); virtual;
  ...
  public
    ...
    procedure Calculate(ACanvas: TCanvas; APainter: TdxScreenTipPainter; ALeft, ATop, AWidth: Integer); virtual;
    function GetHeight(ACanvas: TCanvas; const AWidth: Integer; APainter: TdxScreenTipPainter): Integer; virtual;
  ...
  { TdxScreenTipFooterBandViewInfo }

  TdxScreenTipFooterBandViewInfo = class(TdxScreenTipBandViewInfo)
  strict private
    FSeparatorBounds: TRect;
    FSeparatorLineBounds: TRect;
  protected
    function GetSeparatorPadding(APainter: TdxScreenTipPainter): TRect;
    procedure Paint(ACanvas: TcxCanvas; APainter: TdxScreenTipPainter); override;
  public
    procedure Calculate(ACanvas: TCanvas; APainter: TdxScreenTipPainter; ALeft, ATop, AWidth: Integer); override;
  end;
  ...
  protected
    ...
    function GetWindowPadding: TRect;
  public
    constructor Create(AScreenTip: TdxScreenTip; APainter: TdxScreenTipPainter; const AHintText: string; AShortCut: string = ''; AScaleFactor: TdxScaleFactor = nil);
    destructor Destroy; override;
  ...
  public
    constructor Create(AScreenTip: TdxScreenTip; APainter: TcxCustomLookAndFeelPainter;
      const AHintText: string; AShortCut: string = ''; AScaleFactor: TdxScaleFactor = nil);
  end;
...
function TdxScreenTipBand.GetScaleFactor: TdxScaleFactor;
begin
...
function TdxScreenTipFooterBand.GetScaleFactor: TdxScaleFactor;
begin
  Result := Repository.ScaleFactor;
end;
...
procedure TdxScreenTipBandViewInfo.Calculate(ACanvas: TCanvas; APainter: TdxScreenTipPainter; ALeft, ATop, AWidth: Integer);
var
  AHeight: Integer;
  APadding: TRect;
  AIsTextAlignRight: Boolean;
begin
  APadding := GetBandPadding(APainter);
  AHeight := GetHeight(ACanvas, AWidth, APainter);
  AIsTextAlignRight := Band.TextAlign = stbtaRight;

  if HasGlyph then
  begin
    FGlyphBounds.Left := ALeft +
      IfThen(AIsTextAlignRight, APadding.Left, AWidth - APadding.Right - FGlyphSize.Width) +
      IfThen(AIsTextAlignRight, 1, -1) * (GetAvailGlyphWidth - FGlyphSize.Width) div 2;

    FGlyphBounds.Top := ATop + APadding.Top;
    if FBand.BandType in [stbHeader, stbFooter] then
      FGlyphBounds.Top := FGlyphBounds.Top + ((AHeight - APadding.Top - APadding.Bottom) - FGlyphSize.Height) div 2;

    FGlyphBounds.Width := FGlyphSize.Width;
    FGlyphBounds.Height := FGlyphSize.Height;
  end;

  if HasText then
  begin
    FTextBounds.Left := ALeft + APadding.Left +
      IfThen(AIsTextAlignRight and HasGlyph, GetAvailGlyphWidth + GetGlyphIndent);

    FTextBounds.Top := ATop + APadding.Top +
      IfThen(IsTextCenter, ((AHeight - APadding.Top - APadding.Bottom) - FTextSize.Height) div 2);

    FTextBounds.Width := FTextSize.Width;
    FTextBounds.Height := FTextSize.Height;
  end;

  FBounds.InitSize(ALeft, ATop, AWidth, AHeight);
...
var
  ...
  begin
    ...
    begin
      ...
      if FGlyphSize.Width > AAvailWidth then
        FGlyphSize := cxSizeScale(FGlyphSize, AAvailWidth, FGlyphSize.Width);
    end
    else
  ...
  begin
    if HasText then
    begin
      AAvailWidth := AWidth - APadding.Left - IfThen(HasGlyph, GetAvailGlyphWidth + GetGlyphIndent) - APadding.Right;
...
begin
  ...
  if HasGlyph or HasText then
    Result := APadding.Top + Max(FGlyphSize.Height, FTextSize.Height) + APadding.Bottom;
end;
...
procedure TdxScreenTipBandViewInfo.CorrectMinWidth(ACanvas: TCanvas; APainter: TdxScreenTipPainter; var AWidth: Integer);
var
  AMinWidth: Integer;
  APadding: TRect;
begin
  AMinWidth := 0;
  if HasGlyph then
    AMinWidth := AMinWidth + GetAvailGlyphWidth;
  if HasText then
  begin
    APadding := GetBandPadding(APainter);
    AMinWidth := APadding.Left + AMinWidth + GetGlyphIndent + GetTextSize(ACanvas, 0).Width + APadding.Right;
  end;
...
const
  MSGuidelineDescriptionImageWidth = 108;
  MSGuidelineIconWidth = 16;
begin
  if not Band.GlyphFixedWidth then
    Result := dxGetImageSize(Band.Glyph, ScaleFactor).Width
  else
    if Band.BandType = stbDescription then
      Result := ScaleFactor.Apply(MSGuidelineDescriptionImageWidth)
    else
      Result := ScaleFactor.Apply(MSGuidelineIconWidth);
end;

function TdxScreenTipBandViewInfo.GetBandPadding(APainter: TdxScreenTipPainter): TRect;
begin
  case FBand.BandType of
    stbHeader: Result := ScaleFactor.Apply(APainter.GetHeaderPadding);
    stbDescription: Result := ScaleFactor.Apply(APainter.GetDescriptionPadding);
  else
    Result := ScaleFactor.Apply(APainter.GetFooterPadding);
  end;
end;

function TdxScreenTipBandViewInfo.GetGlyphIndent: Integer;
begin
  if HasGlyph then
    Result := 6
  else
...
begin
  ...
  if HasGlyph then
    TdxImageDrawer.DrawUncachedImage(ACanvas.Handle, GlyphBounds, GlyphBounds, Band.Glyph,
...
begin
  Result := (Band.BandType <> stbDescription) or not Band.GlyphFixedWidth
end;

procedure TdxScreenTipBandViewInfo.PrepareFont(ACanvas: TCanvas);
var
  AScreenTipRepositoryScaleFactor: TdxScaleFactor;
begin
  ACanvas.Font.Assign(Band.Font);
  AScreenTipRepositoryScaleFactor := Band.GetScaleFactor;
  if AScreenTipRepositoryScaleFactor.TargetDPI <> ScaleFactor.TargetDPI then
    ACanvas.Font.Height := ScaleFactor.Apply(ACanvas.Font.Height, AScreenTipRepositoryScaleFactor);
end;

{ TdxScreenTipViewInfoHelper }

constructor TdxScreenTipViewInfoHelper.Create(AScreenTip: TdxScreenTip;
  APainter: TdxScreenTipPainter; const AHintText: string; AShortCut: string = ''; AScaleFactor: TdxScaleFactor = nil);
begin
  inherited Create;
  FScreenTip := AScreenTip;
  FPainter := APainter;
...
var
  ATop, ABandsWidth: Integer;
  I: Integer;
  AWindowPadding: TRect;

  function GetBandsWidth: Integer;
  var
    I: Integer;
  begin
    Result := FScreenTip.GetWidth - AWindowPadding.Left - AWindowPadding.Right;
    for I := 0 to High(FBandViewInfos) do
      if FBandViewInfos[I].Band.BandType in [stbHeader, stbFooter] then
        FBandViewInfos[I].CorrectMinWidth(ACanvas, FPainter, Result);
  end;

begin
  DestroyBandViewInfos;
  CreateBandViewInfos;
  AWindowPadding := GetWindowPadding;
  ABandsWidth := GetBandsWidth;
  ATop := AWindowPadding.Top;
  for I := 0 to High(FBandViewInfos) do
  begin
    ...
    FBandViewInfos[I].Calculate(ACanvas, FPainter, AWindowPadding.Left, ATop, ABandsWidth);
    ATop := FBandViewInfos[I].Bounds.Bottom;
  end;
  BoundsRect.Init(0, 0, AWindowPadding.Left + ABandsWidth + AWindowPadding.Right, ATop + AWindowPadding.Bottom);
end;
...
begin
  if ABand.BandType = stbFooter then
    Result := TdxScreenTipFooterBandViewInfo.Create(ABand, FHintText, FShortCut, ScaleFactor)
  else
    Result := TdxScreenTipBandViewInfo.Create(ABand, FHintText, FShortCut, ScaleFactor);
end;
...
function TdxScreenTipViewInfoHelper.GetWindowPadding: TRect;
begin
  Result := ScaleFactor.Apply(FPainter.GetWindowPadding)
end;
...
constructor TdxScreenTipViewInfo.Create(AScreenTip: TdxScreenTip; APainter: TcxCustomLookAndFeelPainter;
  const AHintText: string; AShortCut: string = ''; AScaleFactor: TdxScaleFactor = nil);
begin
  FHint := AHintText;
  FScreenTip := AScreenTip;
...
begin
  FHelper := TdxScreenTipViewInfoHelper.Create(FScreenTip, FPainter, FHint, FShortCut, FScaleFactor);
end;
...
begin
  ...
  try
    inherited ShowHint(X, Y, ACaption, AHint, AMaxWidth);
    if AMaxWidth = 0 then
  ...
  begin
    ...
    begin
      if AScreenTip.UseHintAsHeader then
        AShortCut := '';
      Result := TdxScreenTipViewInfo.Create(AScreenTip, GetPainter, AHintText, AShortCut, ScaleFactor);
    end
    else
    begin
...
begin
  ...
  begin
    ...
    if not AbsolutePosition then
      UpdateScaleFactorFromPos(GetMouseCursorPos);
...
procedure TdxScreenTipStyle.ShowScreenTip(X, Y: Integer; AScreenTip: TdxScreenTip; const AHint: string = '');
var
  AHintWindow: TdxScreenTipWindow;
begin
  AHintWindow := (Owner as TcxCustomHintStyleController).HintWindow as TdxScreenTipWindow;
...
procedure TdxScreenTipStyle.DoShowHint(var AHintStr: string; var ACanShow: Boolean; var AHintInfo: THintInfo);
var
  AScreenTip: TdxScreenTip;
  AShortCut: string;
begin
...
function TdxScreenTipLinks.FindScreenTipByControl(AControl: TControl): TdxScreenTip;
var
  ALink: TdxCustomScreenTipLink;
begin
  Result := nil;
...
function TdxScreenTipActionLinks.FindScreenTipByAction(AAction: TBasicAction): TdxScreenTip;
var
  ALink: TdxCustomScreenTipLink;
begin
  Result := nil;
...
function TdxScreenTipActionLinks.GetItem(Index: Integer): TdxScreenTipActionLink;
begin
...
procedure TdxScreenTipActionLinks.SetItem(Index: Integer; Value: TdxScreenTipActionLink);
begin
  inherited SetItem(Index, Value);
end;
...
function TdxScreenTipPainter.GetDescriptionPadding: TRect;
begin
  Result := FPainter.ScreenTipGetDescriptionPadding;
end;
...
function TdxScreenTipPainter.GetFooterPadding: TRect;
begin
  Result := FPainter.ScreenTipGetFooterPadding;
end;

function TdxScreenTipPainter.GetHeaderPadding: TRect;
begin
  Result := FPainter.ScreenTipGetHeaderPadding;
end;
...
function TdxScreenTipPainter.GetSeparatorPadding: TRect;
begin
  Result := FPainter.ScreenTipGetSeparatorPadding;
end;
...
function TdxScreenTipPainter.GetWindowPadding: TRect;
begin
  Result := FPainter.ScreenTipGetWindowPadding;
end;
...
begin
  ...
  ASize.Width := ASize.Width + 1 + ScaleFactor.Apply(4);
  ASize.Height := ASize.Height + 1 + ScaleFactor.Apply(4);
end;
...
procedure TdxScreenTipFooterBandViewInfo.Calculate(ACanvas: TCanvas; APainter: TdxScreenTipPainter; ALeft, ATop, AWidth: Integer);
var
  ASeparatorPadding: TRect;
begin
  ASeparatorPadding := GetSeparatorPadding(APainter);
  FSeparatorBounds.InitSize(ALeft, ATop, AWidth, ASeparatorPadding.Top + APainter.GetFooterLineSize + ASeparatorPadding.Bottom);
  FSeparatorLineBounds := FSeparatorBounds;
  FSeparatorLineBounds.Deflate(ASeparatorPadding);

  inherited Calculate(ACanvas, APainter, ALeft, FSeparatorBounds.Bottom, AWidth);
end;

function TdxScreenTipFooterBandViewInfo.GetSeparatorPadding(APainter: TdxScreenTipPainter): TRect;
begin
  Result := ScaleFactor.Apply(APainter.GetSeparatorPadding);
end;

procedure TdxScreenTipFooterBandViewInfo.Paint(ACanvas: TcxCanvas; APainter: TdxScreenTipPainter);
begin
  APainter.DrawFooterLine(ACanvas, FSeparatorLineBounds);
  inherited Paint(ACanvas, APainter);
end;
