...
{                                                                    }
{********************************************************************}
unit dxRichEdit.Import.Doc.DocImporter; // for internal use
...
type
  ...
  strict private
    ...
    FDocObjectDispatcher: array[TdxDocObjectType] of TdxInsertDocObjectDelegate;
...
const
  dxThisUnitName = 'dxRichEdit.Import.Doc.DocImporter';
...
begin
  FDocObjectDispatcher[TdxDocObjectType.InlineImage] := InsertImage;
  FDocObjectDispatcher[TdxDocObjectType.PictureFloatingObject] := InsertPictureFloatingObject;
  FDocObjectDispatcher[TdxDocObjectType.TextBoxFloatingObject] := InsertTextBoxFloatingObject;
  FDocObjectDispatcher[TdxDocObjectType.TextRun] := InsertText;
  FDocObjectDispatcher[TdxDocObjectType.FieldBegin] := InsertFieldBegin;
  FDocObjectDispatcher[TdxDocObjectType.FieldSeparator] := InsertFieldSeparator;
  FDocObjectDispatcher[TdxDocObjectType.FieldEnd] := InsertFieldEnd;
  FDocObjectDispatcher[TdxDocObjectType.HyperlinkFieldData] := InsertHyperlinkFieldData;
  FDocObjectDispatcher[TdxDocObjectType.AutoNumberedFootnoteReference] := InsertFootNoteReference;
  FDocObjectDispatcher[TdxDocObjectType.AnnotationReference] := InsertAnnotationReference;
  FDocObjectDispatcher[TdxDocObjectType.EndnoteReference] := InsertEndnoteReference;
  FDocObjectDispatcher[TdxDocObjectType.NoteNumber] := InsertNoteNumber;
  FDocObjectDispatcher[TdxDocObjectType.TableCell] := InsertTableCell;
  FDocObjectDispatcher[TdxDocObjectType.TableRow] := InsertTableRow;
  FDocObjectDispatcher[TdxDocObjectType.Paragraph] := InsertParagraph;
  FDocObjectDispatcher[TdxDocObjectType.Section] := InsertSection;
  FDocObjectDispatcher[TdxDocObjectType.ExpectedFieldBegin] := InsertExpectedObjectAsText;
  FDocObjectDispatcher[TdxDocObjectType.ExpectedFieldSeparator] := InsertExpectedObjectAsText;
  FDocObjectDispatcher[TdxDocObjectType.ExpectedFieldEnd] := InsertExpectedObjectAsText;
end;
...
begin
  AAction := FDocObjectDispatcher[ADocObject.DocObjectType];
...
begin
  ...
  TdxTextRunBaseAccess(ALastRun).BeginUpdate;
  try
    ALastRun.DoubleFontSize := 2;
    ALastRun.Hidden := True;
  finally
    TdxTextRunBaseAccess(ALastRun).EndUpdate;
  end;
end;
...
begin
  ...
  ARun.Name := AFloatingObject.Name;
end;
