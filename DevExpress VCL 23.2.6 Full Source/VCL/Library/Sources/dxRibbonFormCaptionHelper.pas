...
unit dxRibbonFormCaptionHelper; // for internal use
...
uses
  System.UITypes,
  Types, Windows, Messages, SysUtils, Classes, Graphics, Controls, ExtCtrls, Forms, StrUtils,
  cxGeometry, dxCore, dxMessages, cxClasses, cxGraphics, cxControls, dxBar, dxRibbonSkins, dxCoreClasses,
  dxGDIPlusClasses, cxLookAndFeels;
...
type
  ...
  strict private
    ...
    function BorderIconToDrawIcon(AIcon: TdxRibbonBorderIcon): TdxRibbonBorderDrawIcon;
  ...
  protected
    ...
    procedure DrawCaption(ACanvas: TcxCanvas; const ACaption: TCaption);
    ...
    function GetHintText(ABorderIcon: TdxRibbonTrackedBorderIcon): string;
  ...
  public
    ...
    property DisplayOptionsMenu: TdxRibbonFormDisplayOptionsMenu read FDisplayOptionsMenu; // for internal use
    property FormCaptionDrawBounds: TRect read FFormCaptionDrawBounds;
    property FormPaintData: IdxRibbonFormPaintData read GetFormPaintData; // for internal use
    property RibbonHelper: IdxRibbonFormNonClientHelper read FRibbonHelper; // for internal use
...
function GetDefaultWindowBordersWidth(AWndHandle: THandle): TRect; overload;
function GetDefaultWindowBordersWidth(AWndHandle: THandle; AScaleFactor: TdxScaleFactor): TRect; overload;
...
const
  dxThisUnitName = 'dxRibbonFormCaptionHelper';

{$R dxRibbonFormCaptionHelper.res}
{$R dxRibbonFormCaptionHelper_svg.res}
...
function GetDefaultWindowBordersWidth(AWndHandle: THandle): TRect;
var
  AScaleFactor: TdxScaleFactor;
begin
  AScaleFactor := TdxScaleFactor.Create;
  try
    AScaleFactor.Assign(dxGetMonitorDPI(AWndHandle), dxDefaultDPI);
    Result := GetDefaultWindowBordersWidth(AWndHandle, AScaleFactor);
  finally
    AScaleFactor.Free;
  end;
end;
...
procedure TdxRibbonFormCaptionHelper.DrawCaption(ACanvas: TcxCanvas; const ACaption: TCaption);
var
...
begin
  ACanvas.Lock;
  try
    RibbonHelper.DrawRibbonFormCaption(ACanvas, FFormCaptionDrawBounds, GetWindowBordersWidth, ACaption);
    ...
    begin
      ...
      try
        ...
        begin
          AIconBounds := FBorderIconBounds[AIcon];
          if not cxRectIsEmpty(AIconBounds) then
            RibbonHelper.DrawRibbonFormBorderIcon(ACanvas, AIconBounds, BorderIconToDrawIcon(AIcon), GetBorderIconState(AIcon));
        end;
      finally
        ACanvas.RestoreClipRegion;
      end;

      RibbonHelper.DrawRibbonFormCorners(ACanvas, FFormCaptionDrawBounds, [coTopLeft, coTopRight]);
    end;
  finally
    ACanvas.Unlock;
  end;
...
begin
  Result := FormPaintData.GetWindowBordersWidth;
end;
...
begin
  ...
  try
    ...
    DrawCaption(FBitmap.cxCanvas, ACaption);
    if FormState <> wsMinimized then
    begin
...
begin
  AData := GetFormPaintData;
  RibbonHelper.DrawRibbonFormBorders(ACanvas, AData.GetBounds, AData.GetWindowBordersWidth);
end;

procedure TdxRibbonFormCaptionHelper.DrawWindowCaption(ACanvas: TcxCanvas; const ACaption: TCaption);
var
  ASaveIndex: Integer;
begin
  if Valid then
  begin
    ...
    begin
      ...
      try
        SelectClipRgn(ACanvas.Handle, FFormCaptionRegions[rfrClient]);
        DrawCaption(ACanvas, ACaption);
      finally
        RestoreDC(ACanvas.Handle, ASaveIndex);
      end;
      ExcludeCaptionRgn(ACanvas.Handle);
    end
    else
      if FormState = wsMinimized then
...
procedure TdxRibbonFormCaptionHelper.CalculateBorderIconsForCustomSkinning(AStyle: TdxRibbonStyle);
  ...
  begin
    ...
    Result := cxRectSetTop(GetClientRect,
      Max(AIndent, RibbonHelper.GetSkin.GetPartContentOffsets(rfspRibbonFormCaption).Top), AIconSize.cy);
...
begin
  ...
  RibbonHelper.AdjustRibbonFormBorderIconSize(BorderIconToDrawIcon(ABorderIcon), AIsToolWindow, ACaptionHeight, Result);
end;
...
begin
  if FormHandle = 0 then
    ...
    begin
      ...
      Result := cxGetClientRect(Control.Handle);
end;

function TdxRibbonFormCaptionHelper.BorderIconToDrawIcon(AIcon: TdxRibbonBorderIcon): TdxRibbonBorderDrawIcon;
begin
  case AIcon of
    rbiSystemMenu:
      Result := rbdiClose;
...
begin
  ...
  if (FormHandle <> 0) and
    ((FormState <> wsMaximized) or (Form is TdxCustomRibbonForm) and TdxCustomRibbonForm(Form).UseNavigationControlCompatibleBorders) and
    GetWindowRect(FormHandle, RW) then
  begin
...
begin
  ...
  B := GetWindowBordersWidth;
  RW.Bottom := RW.Top + GetWindowCaptionHeight + B.Top;
...
var
  ...
  DC: HDC;
begin
  if not Valid or UseAeroNCPaint(FormPaintData) or DisplayOptionsMenu.IsPopupShowing then
    Exit;
  if FormState = wsMinimized then
  begin
...
begin
  if Control.HandleAllocated and Valid then
  begin
    if NeedRepaint(GetMouseCursorPos, Control.Handle) then
    begin
      StopMouseTimer;
      HotBorderIcon := tbiNone;
    end;
...
begin
  ...
  AHintText := GetHintText(HotBorderIcon);
  Result := AHintText <> '';
end;
...
function TdxRibbonFormCaptionHelper.GetHintText(ABorderIcon: TdxRibbonTrackedBorderIcon): string;
begin
  Result := '';
  case ABorderIcon of
    tbiSystemMenu:
      Result := cxGetResourceString(@dxSBAR_RIBBONFORM_CLOSE);
    tbiHelp:
      Result := cxGetResourceString(@dxSBAR_RIBBONFORM_HELP);
    tbiDisplayOptions:
      Result := cxGetResourceString(@dxSBAR_RIBBONFORM_DISPLAYOPTIONS);

    tbiMaximize:
      if FormState = wsMaximized then
        Result := cxGetResourceString(@dxSBAR_RIBBONFORM_RESTOREDOWN)
      else
        Result := cxGetResourceString(@dxSBAR_RIBBONFORM_MAXIMIZE);

    tbiMinimize:
      if FormState = wsMinimized then
        Result := cxGetResourceString(@dxSBAR_RIBBONFORM_RESTOREUP)
      else
        Result := cxGetResourceString(@dxSBAR_RIBBONFORM_MINIMIZE);
  end;
end;
...
begin
  ...
  ARibbonPopupMenu.Ribbon := Helper.Control as TdxCustomRibbon;
end;
...
var
  AResKind: string;
begin
  AResKind := PChar(IfThen(dxUseVectorIcons, 'SVG', 'PNG'));
  FItemGlyphAutoHideRibbon := TdxSmartGlyph.Create;
  FItemGlyphAutoHideRibbon.LoadFromResource(HInstance, 'DXRIBBON_DISPLAYOPTIONS_AUTOHIDERIBBONGLYPH', PChar(AResKind));
  FItemGlyphShowTabs := TdxSmartGlyph.Create;
  FItemGlyphShowTabs.LoadFromResource(HInstance, 'DXRIBBON_DISPLAYOPTIONS_SHOWTABSGLYPH', PChar(AResKind));
  FItemGlyphShowTabsAndCommands := TdxSmartGlyph.Create;
  FItemGlyphShowTabsAndCommands.LoadFromResource(HInstance, 'DXRIBBON_DISPLAYOPTIONS_SHOWTABSANDCOMMANDSGLYPH', PChar(AResKind));
end;
...
begin
  Result := (AItem <> domiAutoHideRibbon) or (Helper.Form as TdxCustomRibbonForm).RibbonAutoHideMode.IsAvailable;
end;
...
begin
  Result := (Helper.Form as TdxCustomRibbonForm).RibbonAutoHideMode.Active;
end;

function TdxRibbonFormDisplayOptionsMenu.GetShowTabGroups: Boolean;
begin
  Result := (Helper.Control as TdxCustomRibbon).ShowTabGroups;
end;

procedure TdxRibbonFormDisplayOptionsMenu.SetAutoHideMode(AValue: Boolean);
begin
  (Helper.Form as TdxCustomRibbonForm).RibbonAutoHideMode.Active := AValue;
end;

procedure TdxRibbonFormDisplayOptionsMenu.SetShowTabGroups(AValue: Boolean);
begin
  (Helper.Control as TdxCustomRibbon).ShowTabGroups := AValue;
end;
