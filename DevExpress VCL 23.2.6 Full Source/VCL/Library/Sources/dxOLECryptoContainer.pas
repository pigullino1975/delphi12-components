...
const
  dxThisUnitName = 'dxOLECryptoContainer';
...
begin
  ...
  Result := TdxCipher.Decrypt(AKey, TdxByteArray.Resize(ABlock, AdjustSize(Length(ABlock), ABlockSize)), False);
end;
...
begin
  ...
  AVerifierHashCalculated := CalculateHash(CipherKey.GetProvider,
    TdxByteArray.Resize(TdxCipher.Decrypt(CipherKey, EncryptedVerifier, False), 16));
  AVerifierHashOriginal := TdxCipher.Decrypt(CipherKey, EncryptedVerifierHash, False);
