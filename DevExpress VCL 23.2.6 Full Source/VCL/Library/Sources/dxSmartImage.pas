...
type
  ...
  protected
    class function GetGraphicClassForRegistrationInVCL: TGraphicClass; virtual;
  ...
  public
    ...
    class function GetClipboardFormatId: Word; virtual;
  ...
  protected
    ...
    procedure PopulateImageDataFromHandle; virtual;
    ...
    function GetImageCodec: TdxSmartImageCodecClass; virtual;
    function GetTransparent: Boolean; override;
    function GetWidth: Integer; override;
    function ResizeCore(const ASize: TSize): Boolean;
  ...
  public
    ...
    constructor CreateFromBits(AWidth, AHeight: Integer; const ABits: PRGBQuad; AAlphaFormat: TAlphaFormat); overload; virtual;
    ...
    procedure StretchDraw(DC: HDC; const ADest: TRect;
      AAlpha: Byte = 255; APalette: IdxColorPalette = nil); overload;
    procedure StretchDraw(DC: HDC; const ADest: TdxRectF;
      AAlpha: Byte = 255; APalette: IdxColorPalette = nil); overload;
  ...
  strict private
    ...
    class var FRegisteredClasses: TList;
    class var FRegisterInVCL: Boolean;

    class function GetCount: Integer; static;
    class function GetItem(Index: Integer): TdxSmartImageCodecClass; static;
    class procedure SetRegisterInVCL(AValue: Boolean); static;
    class function SplitExtensions(const ExtList: string): TStringDynArray;
  protected
    class procedure CheckInitialized;
    class procedure DoRegisterInVCL(ACodec: TdxSmartImageCodecClass);
    class procedure DoUnregisterFromVCL(ACodec: TdxSmartImageCodecClass);
  public
    ...
    class function CanLoad(const AFileName: string): Boolean; overload;
    class function CanLoad(const AStream: TStream): Boolean; overload;
    class function GetOpenDialogFilter: string;
    class function GetSaveDialogFilter(AHandle: TdxSmartImageCustomHandle = nil): string;
    class function GetSupportedExtensions: string;
    ...
    class property RegisterInVCL: Boolean read FRegisterInVCL write SetRegisterInVCL;
  end;
...
uses
  SysUtils, Math, Clipbrd, dxHash, dxThreading, Consts, StrUtils, dxStringHelper, dxCoreClasses;

const
  dxThisUnitName = 'dxSmartImage';
...
begin
  ...
  try
    ...
    Draw(Result.Canvas.Handle, Rect(0, 0, ASize.cx, ASize.cy), ClientRect);
  except
    FreeAndNil(Result);
    raise;
  end;
...
begin
  Draw(DC, ADest.DeflateToTRect, ASource.DeflateToTRect, AAlpha, APalette);
end;
...
function TdxSmartImageEmptyHandle.Empty: Boolean;
begin
  Result := True;
end;
...
begin
  Result := UpperCase(TdxSmartImageCodecsRepository.GetDefaultExt(Self)) + dxSmartImageCodecDescriptionSuffix;
end;

class function TdxSmartImageCodec.Extensions: string;
begin
  Result := '';
end;

class function TdxSmartImageCodec.MimeType: string;
begin
  Result := '';
end;

class function TdxSmartImageCodec.GetGraphicClassForRegistrationInVCL: TGraphicClass;
begin
  Result := nil;
end;
...
class function TdxSmartImageCodec.GetClipboardFormatId: Word;
begin
  Result := 0;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if TdxColors.ArePremultiplied(FBits) then
          FBitsFormat := afPremultiplied
        else
...
constructor TdxCustomSmartImage.CreateFromBits(AWidth, AHeight: Integer; const ABits: PRGBQuad; AAlphaFormat: TAlphaFormat);
var
  AColors: TRGBColors;
  ACount: Integer;
begin
  ACount := AWidth * AHeight;
  SetLength(AColors, ACount);
  if ACount > 0 then
    Move(ABits^, AColors[0], ACount * SizeOf(TRGBQuad));
  CreateFromBits(AWidth, AHeight, AColors, AAlphaFormat);
end;
...
constructor TdxCustomSmartImage.CreateFromFile(const AFileName: string);
begin
  Create;
...
begin
  ...
  begin
    ...
    begin
      AStream := TdxHGlobalReadOnlyStream.Create(AData);
      try
...
begin
  ...
  begin
    ...
    try
      ...
      AData := dxStreamToHGlobal(AStream);
...
procedure TdxCustomSmartImage.StretchDraw(DC: HDC; const ADest: TdxRectF; AAlpha: Byte = 255; APalette: IdxColorPalette = nil);
begin
  StretchDraw(DC, ADest, ClientRect, AAlpha, APalette);
end;
  ...
  if ADest is TdxCustomSmartImage then
    ...
    if ADest is TMetafile then
      ...
      begin
        if Empty then
        begin
          if not TBitmap(ADest).Empty then
            ADest.Assign(nil);
        end
        else
        begin
...
function TdxCustomSmartImage.ResizeCore(const ASize: TSize): Boolean;
begin
  if HandleAllocated then
  begin
    ...
    Result := not cxSizeIsEqual(ASize, Handle.Size);
    if Result then
    begin
      Handle.Size := ASize;
      ImageData.FreeImageData;
  ...
  begin
    Result := not cxSizeIsEqual(ImageData.FNewSize, ASize);
    if Result then
...
class function TdxSmartImageCodecsRepository.CanLoad(const AFileName: string): Boolean;
begin
  Result := GetFormatByExt(ExtractFileExt(AFileName)) <> nil;
end;

class function TdxSmartImageCodecsRepository.CanLoad(const AStream: TStream): Boolean;
begin
  Result := GetFormatFromStream(AStream) <> nil;
end;

class function TdxSmartImageCodecsRepository.GetOpenDialogFilter: string;
begin
  Result := GetSaveDialogFilter(nil);
end;

class function TdxSmartImageCodecsRepository.GetSaveDialogFilter(AHandle: TdxSmartImageCustomHandle = nil): string;
var
  ACodec: TdxSmartImageCodecClass;
  ...
  I: Integer;
begin
  AExts := TdxStringBuilderManager.Get;
  AFilter := TdxStringBuilderManager.Get;
  try
    for I := 0 to TdxSmartImageCodecsRepository.Count - 1 do
    begin
      ACodec := TdxSmartImageCodecsRepository.Items[I];
      if (AHandle = nil) or ACodec.CanSaveImage(AHandle) then
      begin
        if ACodec.Description <> '' then
        begin
          AFilter.Append(ACodec.Description);
          AFilter.Append('|');
          AFilter.Append(ACodec.Extensions);
          AFilter.Append('|');
          AExts.Append(ACodec.Extensions);
        end;
      end;
    end;
    Result := Format('%0:s (%1:s)|%1:s|%2:s', [sAllFilter, AExts.ToString, AFilter.ToString]);
  finally
    TdxStringBuilderManager.Release(AFilter);
    TdxStringBuilderManager.Release(AExts);
  end;
end;

class function TdxSmartImageCodecsRepository.GetSupportedExtensions: string;
var
  AExts: TStringBuilder;
  I: Integer;
begin
  ...
  try
    for I := 0 to TdxSmartImageCodecsRepository.Count - 1 do
      AExts.Append(TdxSmartImageCodecsRepository.Items[I].Extensions);
    Result := AExts.ToString;
  finally
    TdxStringBuilderManager.Release(AExts);
  end;
end;

class function TdxSmartImageCodecsRepository.GetDefaultExt(ACodec: TdxSmartImageCodecClass): string;
var
  AExtensions: TStringDynArray;
begin
  AExtensions := SplitExtensions(ACodec.Extensions);
...
begin
  ...
  begin
    ...
    begin
      Result := Items[I];
      if Pos(E, Result.Extensions) > 0 then
        Exit;
    end;
  end;
  Result := nil;
...
begin
  ...
  AClipboardFormat := ACodec.GetClipboardFormatId;
  if AClipboardFormat <> 0 then
  begin
    if ACodec.ID <> dxImageUnknown then
      ImageClipboardFormats[ACodec.ID] := AClipboardFormat;
    if ACodec.GetGraphicClassForRegistrationInVCL <> nil then
      TPicture.RegisterClipboardFormat(AClipboardFormat, ACodec.GetGraphicClassForRegistrationInVCL);
  end;

  DoRegisterInVCL(ACodec);
...
begin
  ...
  begin
    ...
    DoUnregisterFromVCL(ACodec); 
  end;
end;

class procedure TdxSmartImageCodecsRepository.CheckInitialized;
begin
  ...
  {$IFDEF DXREGISTERPNGIMAGE}
    FRegisterInVCL := True;
  {$ELSE}
    FRegisterInVCL := False;
  {$ENDIF}
  end;
end;

class procedure TdxSmartImageCodecsRepository.DoRegisterInVCL(ACodec: TdxSmartImageCodecClass);
var
  AExtensions: TStringDynArray;
  AGraphicClass: TGraphicClass;
  AIsAlreadyRegistered: Boolean;
  I: Integer;
begin
  AGraphicClass := ACodec.GetGraphicClassForRegistrationInVCL;
  if AGraphicClass <> nil then
  begin
    if FRegisteredClasses = nil then
      FRegisteredClasses := TList.Create;
    AIsAlreadyRegistered := FRegisteredClasses.IndexOf(AGraphicClass) >= 0;
    if not AIsAlreadyRegistered then
      FRegisteredClasses.Add(AGraphicClass);

    if RegisterInVCL then
    begin
      AExtensions := SplitExtensions(ACodec.Extensions);
      for I := Low(AExtensions) to High(AExtensions) do
        TPicture.RegisterFileFormat(AExtensions[I], ACodec.Description, AGraphicClass);
    end
    else
      if not AIsAlreadyRegistered then
        TPicture.RegisterFileFormat('', '', AGraphicClass);
  end;
end;

class procedure TdxSmartImageCodecsRepository.DoUnregisterFromVCL(ACodec: TdxSmartImageCodecClass);
var
  AGraphicClass: TGraphicClass;
  I: Integer;
begin
  ...
  begin
    if FRegisteredClasses <> nil then
    begin
      FRegisteredClasses.Remove(AGraphicClass);
      if FRegisteredClasses.Count = 0 then
        FreeAndNil(FRegisteredClasses);
    end;
    TPicture.UnregisterGraphicClass(AGraphicClass);

    for I := 0 to Count - 1 do
    begin
      if Items[I].GetGraphicClassForRegistrationInVCL = AGraphicClass then
        DoRegisterInVCL(Items[I]);
    end;
  end;
end;
...
class procedure TdxSmartImageCodecsRepository.SetRegisterInVCL(AValue: Boolean);
var
  I: Integer;
begin
  if FRegisterInVCL <> AValue then
  begin
    if FRegisteredClasses <> nil then
    begin
      for I := 0 to FRegisteredClasses.Count - 1 do
        TPicture.UnregisterGraphicClass(FRegisteredClasses[I]);
      FRegisteredClasses.Clear;
    end;
    FRegisterInVCL := AValue;
    for I := 0 to Count - 1 do
      DoRegisterInVCL(Items[I]);
  end;
end;

class function TdxSmartImageCodecsRepository.SplitExtensions(const ExtList: string): TStringDynArray;
var
  I: Integer;
begin
  Result := SplitString(Copy(ExtList, 1, Length(ExtList) - 1), ';');
  for I := 0 to Length(Result) - 1 do
    Result[I] := Copy(Result[I], 3, MaxInt);
end;
