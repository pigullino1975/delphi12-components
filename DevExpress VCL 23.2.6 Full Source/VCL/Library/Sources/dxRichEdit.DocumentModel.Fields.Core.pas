...
uses
  Types, Classes, SysUtils, Generics.Defaults, Generics.Collections, Contnrs, RTLConsts, Rtti,
...
type
  ...
  TdxFieldRunInterval = class(TdxCloneable) // for internal use
  strict private
    FStart: TdxRunIndex;
    FEnd: TdxRunIndex;
  public
  ...
  TdxCalculatedFieldValue = class // for internal use
  strict private
  ...
  TdxCustomCalculatedField = class abstract(TcxIUnknownObject) // for internal use
  public
  ...
  TdxFieldCollectionBase = class // for internal use
  strict private
...
const
  dxThisUnitName = 'dxRichEdit.DocumentModel.Fields.Core';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxCalculatedFieldValue.Initialize', SysInit.HInstance);{$ENDIF}
  FNullValue := TdxCalculatedFieldValue.Create(nil);
  FInvalidValue := TdxCalculatedFieldValue.Create(nil);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxCalculatedFieldValue.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxCalculatedFieldValue.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxCalculatedFieldValue.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FNullValue);
  FreeAndNil(FInvalidValue);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxCalculatedFieldValue.Finalize', SysInit.HInstance);{$ENDIF}
end;
