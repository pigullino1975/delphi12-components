...
unit dxEMF.Utils.Evaluator; // for internal use
...
{$REGION 'Evaluation Cache types'}
      ...
      strict private
        ...
        FAllChunksObjects: TdxObjectIntegerDictionary;
      public
        constructor Create(AChunks: TList<TJoinEvaluationCacheChunk>; AAllChunksObjects: TdxObjectIntegerDictionary);

        property Chunks: TList<TJoinEvaluationCacheChunk> read FChunks;
        property AllChunksObjects: TdxObjectIntegerDictionary read FAllChunksObjects;
      end;
...
uses
  DateUtils,
  dxCore, dxTypeHelpers, dxStringHelper, dxHash,
  dxEMF.Strs;

const
  dxThisUnitName = 'dxEMF.Utils.Evaluator';
...
begin
  ...
  while APropertyPath.StartsWith('^.') do
  begin
    APropertyPath := APropertyPath.Substring(2);
    Dec(ACurrentLevel);
  end;
...
begin
  ...
  begin
    ...
    begin
      AList := TdxPlanAliasCriteriaInfo.Create(ANodesString.Split(['.']),
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxIsLogicalCriteriaChecker.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FInstance);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxIsLogicalCriteriaChecker.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  ...
  while FPropertyPath.StartsWith('^.') do
  begin
    Inc(FUpDepth);
    FPropertyPath := FPropertyPath.Substring(2);
  end;
...
begin
  ...
  begin
    ...
    if ADotPos < 0 then
    begin
      ARv.Add(AProp.Substring(ASubPropertyStart));
      Break;
    end
    else
    begin
...
constructor TdxJoinEvaluationContextCache.TJoinEvaluationCacheInfo.Create(AChunks: TList<TJoinEvaluationCacheChunk>;
  AAllChunksObjects: TdxObjectIntegerDictionary);
begin
  FChunks := AChunks;
...
var
  ...
  AChunkObjects: TdxObjectIntegerDictionary;
...
begin
  ...
  begin
    ...
    AChunkObjects := TdxObjectIntegerDictionary.Create;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxExpressionEvaluatorCoreBase.Initialize', SysInit.HInstance);{$ENDIF}
  FFnFunctions := TDictionary<TdxFnFuncKey, TFunc<TArray<TObject>, TObject>>.Create;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxExpressionEvaluatorCoreBase.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxExpressionEvaluatorCoreBase.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxExpressionEvaluatorCoreBase.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FFnFunctions);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxExpressionEvaluatorCoreBase.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  ...
  if AOpValue.IsNullOrEmpty then
    Exit(False);
...
begin
  if AValue.IsNullOrEmpty or not AValue.IsBoolean then
    Result.Reset
  else
...
begin
  ...
  begin
    ...
    begin
      ...
      if not AObj.IsNullOrEmpty then
        Exit(AObj);
    end;
    Exit(nil);
  end;
...
begin
  ...
  case AOperator.OperatorType of
    ...
    TdxUnaryOperatorType.Plus:
      begin
        if AOperand.IsNullOrEmpty then
          Exit(nil);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if ACandidate.IsNullOrEmpty then
              Continue;
          end;
