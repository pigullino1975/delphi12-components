...
const
  dxThisUnitName = 'dxRibbonMiniToolbar';
...
var
  ALineHeight: Integer;
begin
  ALineHeight := GetSizeForHeight(dsNone, TdxTextMeasurer.TextLineHeight(Font)).cy;
  Result := GetSizeForHeight(dsNone, 2 * ALineHeight + cxTextOffset, True);
  Result := cxSizeAddMargins(Result, Painter.GetToolbarContentOffsets(Bar, DockingStyle, ScaleFactor, False));
end;

function TdxRibbonMiniToolbarControl.NeedActivateOnPopup: Boolean;
begin
  Result := False;
end;
