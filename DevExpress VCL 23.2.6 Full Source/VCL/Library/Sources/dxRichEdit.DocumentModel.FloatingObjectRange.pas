...
type
  ...
  TdxTextBoxContentType = class(TdxContentTypeBase) // for internal use
  strict private
  ...
  TdxFloatingObjectContent = class abstract(TdxCustomObjectContent) // for internal use
  strict private
  ...
  TdxEmptyFloatingObjectContent = class(TdxFloatingObjectContent) // for internal use
  protected
  ...
  TdxTextBoxFloatingObjectContent = class(TdxFloatingObjectContent, IdxTextBoxPropertiesContainer) // for internal use
  strict private
  ...
  TdxExplicitFloatingObjectLocation = class(TInterfacedObject, IdxFloatingObjectLocation) // for internal use
  strict private
  ...
  TdxFloatingObject = class(TcxIUnknownObject, IdxFloatingObjectPropertiesContainer, IdxShapeContainer) // for internal use
  strict private
    FAnchorRun: TdxFloatingObjectAnchorRun;
...
const
  dxThisUnitName = 'dxRichEdit.DocumentModel.FloatingObjectRange';
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxFloatingObjectContent.Initialize', SysInit.HInstance);{$ENDIF}
  FEmpty := TdxEmptyFloatingObjectContent.Create;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxFloatingObjectContent.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxFloatingObjectContent.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxFloatingObjectContent.Finalize', SysInit.HInstance);{$ENDIF}
  FreeAndNil(FEmpty);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxFloatingObjectContent.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  ...
  try
    ATempDocumentModel.BeginUpdate;
    try
...
begin
  ...
  if Safe.Cast(FContent, TdxTextBoxFloatingObjectContent, ATextBoxContent) and (ATextBoxContent.TextBox <> nil) then
  begin
...
begin
  ...
  begin
    AHeaderFooter := PieceTable.ContentType as TdxSectionHeaderFooterBase;
