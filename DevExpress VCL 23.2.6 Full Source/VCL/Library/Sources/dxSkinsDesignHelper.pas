...
type
  ...
  TdxSkinsCheckListBox = class(CheckLst.TCheckListBox)
  strict private
  ...
  TCheckListBox = class(TdxSkinsCheckListBox);
  ...
  private
    ...
    procedure AddRunTimeSkinPackages(AProject: IOTAProject);
  ...
  TdxSkinsDesignHelperForm = class(TForm)
    ...
    procedure clbSkinsClickCheck(Sender: TObject);
  private
    FIsDefaultSettings: Boolean;
...
uses
  dxSkinsLookAndFeelPainter, dxSkinsStrs, dxDesignHelpers, dxSkinsReg, dxCore;

const
  dxThisUnitName = 'dxSkinsDesignHelper';
...
begin
  ...
  clbSkins.Hint := sdxSkinCheckListBoxHint;
  clbSkins.AllowBoldSelection := FileExists(dxSkinsProjectSettings.SkinsConfigFileName);
...
var
  ...
  I: Integer;
begin
  clbSkins.Items.BeginUpdate;
  try
    clbSkins.Items.Clear;
    for I := 0 to dxSkinsProjectSettings.UnitStateList.Count - 1 do
    begin
      AItem := dxSkinsProjectSettings.UnitStateList.Item[I];
      clbSkins.Items.AddObject(AItem.Name, TObject(AItem.Enabled));
      clbSkins.Checked[I] := AItem.Enabled;
    end;
  finally
    clbSkins.Items.EndUpdate;
  end;
...
begin
  ...
  for I := 0 to clbSkins.Count - 1 do
  begin
    Result := clbSkins.Checked[I] <> Boolean(clbSkins.Items.Objects[I]);
    if Result then
      Break;
  end;
...
begin
  ...
  clbSkins.Enabled := cbSkinsAutoFilling.Checked;
  cbShowNotifications.Enabled := cbSkinsAutoFilling.Checked;
  UpdateDefaultCheckBoxState;
end;

procedure TdxSkinsDesignHelperForm.clbSkinsClickCheck(Sender: TObject);
begin
  UpdateDefaultCheckBoxState;
end;
...
begin
  for I := 0 to clbSkins.Count - 1 do
    clbSkins.Checked[I] := TComponent(Sender).Tag = 1;
  UpdateDefaultCheckBoxState;
end;
...
function TdxSkinsProjectOptionsMenuExpert.CreateMenuItem(AParent: TMenuItem): TMenuItem;
  ...
  function AddToImageList(AImageList: TCustomImageList; const AGlyph: string): Integer;
  var
    ABitmap: TBitmap;
  begin
    ABitmap := TBitmap.Create;
    try
      ABitmap.LoadFromResourceName(HInstance, AGlyph);
      Result := AImageList.AddMasked(ABitmap, clFuchsia);
    finally
      ABitmap.Free;
    end;
  end;
...
begin
  ...
  {$IFDEF DELPHI110}
    if Supports(BorlandIDEServices, INTAServices90, AServices) then
    begin
      AImageList := TImageList.Create(nil);
      try
        AddToImageList(AImageList, sdxSkinsMenuItemGlyphResName);
        Result.ImageIndex := AServices.AddImages(AImageList);
      finally
        AImageList.Free;
      end;
    end
  ...
  {$ENDIF}
      if AParent.GetImageList <> nil then
        Result.ImageIndex := AddToImageList(AParent.GetImageList, sdxSkinsMenuItemGlyphResName)
      else
...
procedure TdxSkinsDesignHelper.BeforeCompile(const Project: IOTAProject; var Cancel: Boolean);
begin
  AddRunTimeSkinPackages(Project);
end;
...
begin
  AddRunTimeSkinPackages(ActiveProject);
end;
...
procedure TdxSkinsDesignHelper.AddRunTimeSkinPackages(AProject: IOTAProject);
begin
end;
