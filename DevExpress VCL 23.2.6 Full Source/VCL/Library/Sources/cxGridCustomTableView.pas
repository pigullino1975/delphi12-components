...
uses
  System.UITypes,
  Variants, Windows, Messages, Classes, Graphics, Controls, ImgList, Forms, StdCtrls, Dialogs, MultiMon, SysUtils,
...
type
  ...
  protected
    ...
    function CreateEditViewData(AProperties: TcxCustomEditProperties; ACellStyle: TcxEditStyle): TcxCustomEditViewData; overload;
...
uses
  Types, DateUtils, Math, TypInfo, RTLConsts, Clipbrd,
...
var
  I: Integer;
  A110B4C7A6A6F400506021A192117171C64511E16702D706A: TcxCustomGridRecord;
begin
  if NeedChangeLayoutOnSelectionChanged(AInfo) then
    ...
    begin
      A110B4C7A6A6F400506021A192117171C64511E16702D706A := ViewData.GetRecordByIndex(AInfo.RowIndexes[I]);
      if A110B4C7A6A6F400506021A192117171C64511E16702D706A <> nil then A110B4C7A6A6F400506021A192117171C64511E16702D706A.Invalidate;
    end;
...
begin
  ...
  for I := 0 to Count - 1 do
    if Items[I].Visible and not IsItemCalculatedSingly(Items[I]) then
    begin
      if Items[I].Alignment = fbaLeft then
        ALeft := ALeftMargin
      else
        ALeft := ARightMargin - Items[I].CalculateWidth;
      Items[I].Calculate(ALeft, GetTopBound(Items[I], ABounds));
      if Items[I].Alignment = fbaLeft then
...
var
  AWidth: Integer;
begin
  AWidth := Max(0, cxRectWidth(ABounds) - WidthLeftPart - WidthRightPart);
...
begin
  ...
  for I := Count - 1 downto 0 do
    if Items[I].Visible and (Items[I].Alignment = fbaRight) then
    begin
      if not AOffsetAssigned then
      begin
        AOffset := EditViewInfo.Bounds.Right + GetButtonsOffset - Items[I].Bounds.Left;
        AOffsetAssigned := True;
      end;
      Items[I].Offset(AOffset, 0);
    end;
end;
...
begin
  AEditSizeProperties.InitValues(MaxLineCount, -1, cxRectWidth(TextAreaBounds), nil);
...
function TcxCustomGridTableItem.CreateEditViewData(AProperties: TcxCustomEditProperties; ACellStyle: TcxEditStyle): TcxCustomEditViewData;
begin
  Result := AProperties.CreateViewData(ACellStyle, True, GetScaleFactor);
end;
...
begin
  ...
  try
    ...
    AEditSizeProperties := cxDefaultEditSizeProperties;
    AViewData := CreateEditViewData(GetProperties, AEditStyle); 
    try
...
begin
  for I := 0 to ViewInfo.Count - 1 do
    if (ViewInfo.Items[I] <> nil) and ViewInfo.Items[I].Calculated then
