...
uses
  ...
  dxSpreadSheetCoreStyles, dxSpreadSheetFormatXLSXReaderStyles, dxSpreadSheetFormatXLSXUtils;

const
  sdxXLSSummaryInformationStream = #5'SummaryInformation';
  sdxXLSWorkbookStream = 'Workbook';
...
type
  ...
  strict private
    ...
    procedure SetHandle(const Value: TdxSpreadSheetFontHandle);
  public
    Style: TFontStyles;
    Script: Word;
    Size: Integer;
    ...
    property Handle: TdxSpreadSheetFontHandle read FHandle write SetHandle;
  end;
  ...
  strict private
    ...
    procedure SetHandle(const Value: TdxSpreadSheetCellStyleHandle);
  public
    Font: Word;
    Format: Word;
    Flags: Word;
  ...
  TdxXLSWriterSSTBucketInfo = packed record
  ...
  TdxSpreadSheetXLSReaderThemeReader = class(TdxXLSXThemeReader)
  strict private
    FHelper: TdxSpreadSheetXLSReaderThemeHelper;
    FStream: TStream;
  protected
    function GetFileRelationships: TdxXLSXRelationships; override;
    function GetFileStream(const AFileName: string): TStream; override;
  public
    constructor Create(AStream: TStream; AHelper: TdxSpreadSheetXLSReaderThemeHelper);
  end;
...
//  brcSXEXT                   =   $00DC;              // External Source Information
  ...
  brcNameCmt                 =   $0894;              // Defined name comment
...
uses
  dxSpreadSheetFormatXLSFormulas, dxSpreadSheetFormatXLS, SysUtils, dxSpreadSheetFormulas,
  dxSpreadSheetFormatXLSReader, dxSpreadSheetCoreFormulasParser, dxHashUtils;

const
  dxThisUnitName = 'dxSpreadSheetFormatXLSTypes';
...
procedure TdxXLSCellStyle.SetHandle(const Value: TdxSpreadSheetCellStyleHandle);
begin
  dxChangeHandle(FHandle, Value);
end;

{ TdxXLSFont }

destructor TdxXLSFont.Destroy;
begin
  Handle := nil;
  inherited;
end;
...
procedure TdxXLSFont.SetHandle(const Value: TdxSpreadSheetFontHandle);
begin
  dxChangeHandle(FHandle, Value);
end;
...
begin
  FData := TdxXLSXReaderThemeData.Create;
...
function TdxSpreadSheetXLSReaderThemeHelper.GetColor(AColorIndex: Integer; ATintShade: SmallInt; var AColor: Integer): Boolean;
begin
  Result := (AColorIndex >= 0) and (AColorIndex < FData.Colors.Count);
  if Result then
  begin
    if AColorIndex = 0 then
      AColor := clWindow
    else if AColorIndex = 1 then
      AColor := clWindowText
    else
      AColor := Integer(FData.Colors[AColorIndex]);
...
begin
  ...
  begin
    with TdxSpreadSheetXLSReaderThemeReader.Create(AStream, Self) do
    try
      Execute;
    finally
      Free;
...
begin
  ...
  for I := 0 to Length(StandardColors) - 1 do
    FData.Colors.Add(Pointer(StandardColors[I]));
end;
...
{ TdxSpreadSheetXLSReaderThemeReader }

constructor TdxSpreadSheetXLSReaderThemeReader.Create(AStream: TStream; AHelper: TdxSpreadSheetXLSReaderThemeHelper);
begin
  FStream := AStream;
  FHelper := AHelper;
  inherited Create('theme/theme/theme1.xml', nil, AHelper.FData);
end;

function TdxSpreadSheetXLSReaderThemeReader.GetFileRelationships: TdxXLSXRelationships;
begin
  Result := TdxXLSXRelationships.Create;
end;

function TdxSpreadSheetXLSReaderThemeReader.GetFileStream(const AFileName: string): TStream;
var
...
begin
  ADataStream := TMemoryStream.Create;
  try
    ADataStream.CopyFrom(FStream, FStream.Size - FStream.Position);
    ADataStream.Position := 0;
    AZIPStream := TdxZIPStreamReader.Create(ADataStream);
    try
      Result := TMemoryStream.Create;
      if not AZIPStream.Extract(dxStringToAnsiString(AFileName), Result) then
        Result.Size := 0;
    finally
      AZIPStream.Free;
    end;
