...
type
  ...
  private
    ...
    function CompareRecordBuffers(ARecordBuffer1, ARecordBuffer2: TRecordBuffer; AIsCaseInsensitive: Boolean): Integer;
  ...
  public
    ...
    procedure SetValue(mField: TdxMemField; AIndex: Integer; ABuffer: TRecordBuffer);
    procedure SetHasValue(mField: TdxMemField; AIndex: Integer; AValue: AnsiChar); 
  ...
  private
    ...
    procedure DoSort(ASortedList, ASynchronizationLists: TList; AmdField: TdxMemField; ASortOptions: TdxSortOptions); overload;
    procedure DoSort(ASortedList, ASynchronizationLists: TList; ASortingInfos: TdxMemSortingInfos); overload;
    procedure MakeSort;
  ...
  protected
    ...
    function InternalGotoNearest(AValues: TList; AField: TField;
      const ABuffer: TRecordBuffer; ASortOptions: TdxSortOptions; ALocateOptions: TLocateOptions; out AIndex: Integer): Boolean; overload;
    function InternalGotoNearest(AValues: TList; ACompareInfos: TdxMemSortingInfos;
      const ARecordBuffer: TRecordBuffer): Integer; overload;
    function GotoNearest(const AValueBuffer: TRecordBuffer; ALocateOptions: TLocateOptions; out AIndex: Integer): Boolean; overload;
  ...
  public
    ...
    property SortedField: string read FSortedFieldNames write SetSortedFields stored False;
...
var
  A160B666C627866045B061407110E18146074120729736178: TFieldType;
begin
  A160B666C627866045B061407110E18146074120729736178 := GetFieldTypeByNo(AFieldTypeNo);
  case BaseFieldType(A160B666C627866045B061407110E18146074120729736178) of
...
function TdxCustomMemData.InternalLocate(const KeyFields: string; const KeyValues: Variant; Options: TLocateOptions): Integer;

  function CompareLocate_SortCaseSensitive: Boolean;
  begin
    Result := ((loCaseInsensitive in Options) and (soCaseInsensitive in SortOptions)) or
      (not (loCaseInsensitive in Options) and not (soCaseInsensitive in SortOptions));
  end;
  ...
  begin
    ...
    try
      ...
      if AIndex = nil then
      begin
        if not GotoNearest(ABuf, Options, Result) and not (loPartialKey in Options) then
          Result := -1;
      end
      else
      begin
...
function TdxCustomMemData.Locate(const KeyFields: string; const KeyValues: Variant; Options: TLocateOptions): Boolean;
var
  AIndex: Integer;
begin
...
begin
  case State of
    dsBrowse, dsOldValue, dsNewValue:
      if IsEmpty then
        ARecordBuffer := nil
      else
...
var
  mField: TdxMemField;
begin
  if (State in [dsOldValue]) and Modified and (self.State = dsEdit) then
  begin
...
function CompareRecordBuffersByFields(ARecordBuffer1, ARecordBuffer2: Pointer; ASortingInfos: TdxMemSortingInfos): Integer;
var
  I: Integer;
begin
  Result := 0;
  ...
  begin
    if ASortingInfos[I].MemField <> nil then
      Result := ASortingInfos[I].MemField.CompareRecordBuffers(ARecordBuffer1, ARecordBuffer2, soCaseInsensitive in ASortingInfos[I].Options);
    if Result <> 0 then
    begin
...
function CompareValuesByFields(ARecordBuffer1, ARecordBuffer2: Pointer): Integer;
begin
  Result := CompareRecordBuffersByFields(ARecordBuffer1, ARecordBuffer2, ACompareOptions);
end;
...
begin
  ...
  if FSortedField <> nil then
  begin
    if (FCurRec > 0) and (CompareRecordBuffersByFields(GetRecordBuffer(FCurRec), GetRecordBuffer(FCurRec - 1), FSortingInfos) = -1) then
      FGotoNearestMax := FCurRec - 1
    else
      if (FCurRec < FData.RecordCount - 1) and (CompareRecordBuffersByFields(GetRecordBuffer(FCurRec), GetRecordBuffer(FCurRec + 1), FSortingInfos) = 1) then
        FGotoNearestMin := FCurRec + 1;

    NewCurRec := InternalGotoNearest(FData.FValues, FSortingInfos, GetRecordBuffer(FCurRec));
    ...
    begin
      if FCurRec = 0 then
        ...
        ATestIndex := 0;
      if CompareRecordBuffersByFields(GetRecordBuffer(FCurRec), GetRecordBuffer(ATestIndex), FSortingInfos) = -1 then
    ...
    if NewCurRec = - 1 then
      NewCurRec := 0;
    if (fCurRec < NewCurRec) and (CompareRecordBuffersByFields(GetRecordBuffer(NewCurRec), GetRecordBuffer(FCurRec), FSortingInfos) = 1) then
...
begin
  ...
  if State = dsEdit then
  begin
    AIsMakeSort := AIsMakeSort and (CompareRecordBuffersByFields(FData.FValues[FCurRec], ActiveBuffer, FSortingInfos) <> 0);
...
procedure TdxCustomMemData.DoSort(ASortedList, ASynchronizationLists: TList; AmdField: TdxMemField; ASortOptions: TdxSortOptions);
var
...
procedure TdxCustomMemData.DoSort(ASortedList, ASynchronizationLists: TList; ASortingInfos: TdxMemSortingInfos);
...
function TdxCustomMemData.InternalGotoNearest(AValues: TList; ACompareInfos: TdxMemSortingInfos; const ARecordBuffer: TRecordBuffer): Integer;
  ...
  begin
    ...
    begin
      if ACompareInfos[I].MemField <> nil then 
        Result := ACompareInfos[I].MemField.CompareRecordBuffers(ARecordBuffer, ARecordBuffer2, soCaseInsensitive in ACompareInfos[I].Options);
      if Result <> 0 then
      begin
...
function TdxCustomMemData.GotoNearest(const AValueBuffer: TRecordBuffer; ALocateOptions: TLocateOptions; out AIndex: Integer): Boolean;
begin
  AIndex := -1;
  Result := False;
  ...
  if (FSortedField <> nil) and (FData.FValues.Count > 0) then
    Result := InternalGotoNearest(FData.FValues, FSortedField, AValueBuffer, FSortOptions, ALocateOptions, AIndex);
end;
...
function TdxMemFields.GetValue(mField: TdxMemField; AIndex: Integer): TRecordBuffer;
begin
  Result := mField.Values[AIndex];
end;

function TdxMemFields.GetHasValue(mField: TdxMemField; AIndex: Integer): AnsiChar;
begin
  Result := mField.GetHasValues(AIndex);
end;

procedure TdxMemFields.SetValue(mField: TdxMemField; AIndex: Integer; ABuffer: TRecordBuffer);
begin
...
procedure TdxMemFields.SetHasValue(mField: TdxMemField; AIndex: Integer; AValue: AnsiChar);
begin
...
function TdxIntegerList.GetItem(AIndex: TdxListIndex): TdxNativeInt;
begin
  Result := TdxNativeInt(inherited Items[AIndex]);
end;

procedure TdxIntegerList.SetItem(AIndex: TdxListIndex; AValue: TdxNativeInt);
begin
...
function TdxMemField.GetValueFromBuffer(ARecordBuffer: TRecordBuffer): TRecordBuffer;
begin
...
function TdxMemField.CompareRecordBuffers(ARecordBuffer1, ARecordBuffer2: TRecordBuffer; AIsCaseInsensitive: Boolean): Integer;
begin
  Result := DataSet.InternalCompareValues(GetValueFromBuffer(ARecordBuffer1), GetValueFromBuffer(ARecordBuffer2), Self, AIsCaseInsensitive);
end;
