...
type
  ...
  protected
    ...
    function DoFindRecordIndex: Integer; virtual;
    ...
    function FindRecordIndex: Integer;
  ...
  protected
    ...
    function GetCaseSensitiveSearchValue: Boolean; virtual;
  ...
  protected
    ...
    property IsTextChanging: Boolean read FIsTextChanging;
  public
    ...
    property Properties: TcxCustomLookupEditProperties read GetProperties write SetProperties;
  end;
...
begin
  ...
  try
    ...
    Properties.LookupGridInitLookAndFeel(Edit.PopupControlsLookAndFeel, Edit.ViewInfo.BackgroundColor, Edit.VisibleFont);
...
function TcxCustomLookupEditLookupData.FindRecordIndex: Integer;
begin
  if Edit.IsTextChanging then
    Result := Properties.FindByText(GetListIndex, Edit.Text, False)
  else
  begin
    if VarIsNull(Edit.EditValue) then
      Result := -1
    else
      Result := DoFindRecordIndex;
  end;
end;
...
function TcxCustomLookupEditLookupData.DoFindRecordIndex: Integer;
begin
  Result := Properties.FindByText(GetListIndex, Edit.Text, False);
end;
  ...
  begin
    if FVisible and Properties.GetIncrementalFiltering then
      if DataController.GetIncrementalFilterField = nil then
        ...
        if Edit.IsTextChanging and DataController.ChangeIncrementalFilterText(Edit.Text) then
          UpdateDropDownCount;
    ARecordIndex := -1;
    if not FVisible then // Properties.EditingStyle in [esEditList, esFixedList]
    begin
      ARecordIndex := FindRecordIndex;
      if ARecordIndex <> -1 then
      begin
        ASelected := True;
        DataController.ChangeFocusedRecordIndex(ARecordIndex);
...
begin
  Result := TcxCustomLookupEdit(Owner);
end;
...
function TcxCustomLookupEditProperties.GetCaseSensitiveSearchValue: Boolean;
begin
  Result := False;
end;
...
begin
  Result := DataController.FindRecordIndexByText(0, AItemIndex, AText, APartialCompare, False, True, GetCaseSensitiveSearchValue);
end;
...
begin
  ...
  begin
    if HasPopupWindow or LookupData.IsEmpty then
      ILookupData.CurrentKey := EditValue
    else
      LookupData.SyncSelected;
  end;
end;
