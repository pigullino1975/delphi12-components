...
{                                                                    }
{********************************************************************}
unit dxRichEdit.Import.Doc.OfficeArtContent; // for internal use
...
const
  dxThisUnitName = 'dxRichEdit.Import.Doc.OfficeArtContent';
...
function TdxOfficeArtProperties.HasFill: Boolean;
begin
  Result := not TdxAlphaColors.IsTransparentOrEmpty(Shape.FillColor);
end;

function TdxOfficeArtProperties.HasOutline: Boolean;
begin
  Result := not TdxAlphaColors.IsTransparentOrEmpty(Shape.OutlineColor);
end;
...
begin
  if (Shape <> nil) and Shape.UseFillColor and HasFill then
...
begin
  ...
  begin
    ALineStyleBooleanProperties.Line := HasOutline;
...
procedure TdxOfficeArtProperties.SetName(const AName: string);
begin
  FName := AName;
end;
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxOfficePropertiesFactory.Initialize', SysInit.HInstance);{$ENDIF}
...
//  AddProperty($0089, TdxDrawingShapeFontDirection);
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxOfficePropertiesFactory.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxOfficePropertiesFactory.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxOfficePropertiesFactory.Finalize', SysInit.HInstance);{$ENDIF}
  FPropertyTypes.Free;
  FPropertyIdentifiers.Free;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxOfficePropertiesFactory.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
procedure TdxDrawingShapeName.Execute(AOwner: TdxOfficeArtPropertiesBase);
var
  AProperties: TdxOfficeArtProperties;
begin
  inherited Execute(AOwner);
  AProperties := Safe<TdxOfficeArtProperties>.Cast(AOwner);
  if AProperties <> nil then
    AProperties.SetName(PChar(Data));
end;
...
procedure TdxOfficeArtContent.InitializeArtProperties(AArtProperties: TdxOfficeArtProperties;
  ARun: TdxFloatingObjectAnchorRun; AUnitConverter: TdxDocumentModelUnitConverter);
begin
  AArtProperties.SetName(ARun.Name);
...
begin
  ...
  AProperties.Add(TdxDrawingShapeName.Create);
end;
