...
unit dxSpreadSheetFunctionsText; // for internal use
...
uses
  ...
  dxStringHelper, dxSpreadSheetCoreReferences;

const
  dxThisUnitName = 'dxSpreadSheetFunctionsText';
...
var
  ...
  I: Integer;
begin
  AResult := Sender.ExtractStringParameter(AParams);
  ALength := Length(AResult);
  ABuffer := TdxStringBuilderManager.Get(ALength);
  try
    for I := 1 to Length(AResult) do
    begin
      if not dxCharInSet(AResult[I], [#0..#31]) then
        ABuffer.Append(AResult[I]);
    end;
    if ABuffer.Length <> ALength then
      Sender.AddValue(ABuffer.ToString)
    else
...
var
  AParamValue: string;
begin
  AParamValue := Sender.ExtractStringParameter(AParams);
  if AParamValue = '' then
    Sender.SetError(ecValue)
  else
    Sender.AddValue(Ord(AParamValue[1]));
end;
...
begin
  ...
  try
    ...
    begin
      ABuffer.Append(Sender.ExtractStringParameter(AItem));
      AItem := AItem.Next;
    end;
...
procedure fnExact(Sender: TdxSpreadSheetFormulaResult; const AParams: TdxSpreadSheetFormulaToken);
begin
  Sender.AddValue(AnsiCompareStr(
    Sender.ExtractStringParameter(AParams),
    Sender.ExtractStringParameter(AParams.Next)) = 0);
end;
...
begin
  ASubStr := Sender.ExtractStringParameter(AParams);
  if Sender.Validate then
  begin
    AStr := Sender.ExtractStringParameter(AParams.Next);
...
var
  ...
  AIntLength: Integer;
begin
  AStr := Sender.ExtractStringParameter(AParams);
...
var
  ...
  I: Integer;
begin
  AValue := dxSpreadSheetLowerCase(Sender.ExtractStringParameter(AParams));
  AValueLength := Length(AValue);
  if AValueLength > 0 then
  begin
    ABuffer := TdxStringBuilderManager.Get(AValueLength);
    try
      for I := 1 to AValueLength do
      begin
        if (I = 1) or (I > 1) and IsPriorCharNonLetter(AValue[I - 1]) then
          ABuffer.Append(UpCase(AValue[I]))
        else
...
begin
  ASubStr := Sender.ExtractStringParameter(AParams);
  S := Sender.ExtractStringParameter(AParams, 1);
...
begin
  if Sender.ExtractParameterDef(AParameter, 0, AParams) and Sender.ExtractParameterDef(AFormatText, '', AParams, 1) then
    ...
    begin
      AFormatText := dxSpreadSheetUnescapeString(dxSpreadSheetUnescapeString(AFormatText, dxStringMarkChar), dxStringMarkChar2);
...
var
  ...
  APos: Integer;
begin
  S := Trim(Sender.ExtractStringParameter(AParams));
  repeat
    APos := Pos('  ', S);
    if APos > 0 then
      Delete(S, APos, 1);
...
begin
  ARepository.InternalAdd(@sfnASC, nil, nil, frkValue, 214, ftText, @sfnASCDescription);
  ARepository.InternalAdd(@sfnBahtText, nil, nil, frkValue, 368, ftText, @sfnBahtTextDescription);
  ARepository.InternalAdd(@sfnChar, fnChar, fpiChar, frkValue, 111, ftText, @sfnCharDescription);
  ARepository.InternalAdd(@sfnClean, fnClean, fpiClean, frkValue, 162, ftText, @sfnCleanDescription);
  ARepository.InternalAdd(@sfnCode, fnCode, fpiCode, frkValue, 121, ftText, @sfnCodeDescription);
  ARepository.InternalAdd(@sfnConcatenate, fnConcatenate, fpiConcatenate, frkNonArrayValue, 336, ftText, @sfnConcatenateDescription);
  ARepository.InternalAdd(@sfnDBCS, nil, nil, frkValue, 215, ftText, @sfnDBCSDescription);
  ARepository.InternalAdd(@sfnDollar, fnDollar, fpiDollar, frkValue, 13, ftText, @sfnDollarDescription);
  ARepository.InternalAdd(@sfnExact, fnExact, fpiExact, frkValue, 117, ftText, @sfnExactDescription);
  ARepository.InternalAdd(@sfnFind, fnFind, fpiFind, frkValue, 124, ftText, @sfnFindDescription);
  ARepository.InternalAdd(@sfnFindB, nil, nil, frkValue, 205, ftText, @sfnFindBDescription);
  ARepository.InternalAdd(@sfnFixed, fnFixed, fpiFixed, frkValue, 14, ftText, @sfnFixedDescription);
  ARepository.InternalAdd(@sfnLeft, fnLeft, fpiLeft, frkValue, 115, ftText, @sfnLeftDescription);
  ARepository.InternalAdd(@sfnLeftB, nil, nil, frkValue, 208, ftText, @sfnLeftBDescription);
  ARepository.InternalAdd(@sfnLen, fnLen, fpiLen, frkValue, 32, ftText, @sfnLenDescription);
  ARepository.InternalAdd(@sfnLenB, nil, nil, frkValue, 211, ftText, @sfnLenBDescription);
  ARepository.InternalAdd(@sfnLower, fnLower, fpiLower, frkValue, 112, ftText, @sfnLowerDescription);
  ARepository.InternalAdd(@sfnMid, fnMid, fpiMid, frkValue, 31, ftText, @sfnMidDescription);
  ARepository.InternalAdd(@sfnMidB, nil, nil, frkValue, 210, ftText, @sfnMidBDescription);
  ARepository.InternalAdd(@sfnNumberValue, nil, nil, frkValue, 255, ftText, @sfnNumberValueDescription, 1);
  ARepository.InternalAdd(@sfnPhonetic, nil, nil, frkValue, 360, ftText, @sfnPhoneticDescription);
  ARepository.InternalAdd(@sfnProper, fnProper, fpiProper, frkValue, 114, ftText, @sfnProperDescription);
  ARepository.InternalAdd(@sfnReplace, fnReplace, fpiReplace, frkValue, 119, ftText, @sfnReplaceDescription);
  ARepository.InternalAdd(@sfnReplaceB, nil, nil, frkValue, 207, ftText, @sfnReplaceBDescription);
  ARepository.InternalAdd(@sfnRept, fnRept, fpiRept, frkValue, 30, ftText, @sfnReptDescription);
  ARepository.InternalAdd(@sfnRight, fnRight, fpiRight, frkValue, 116, ftText, @sfnRightDescription);
  ARepository.InternalAdd(@sfnRightB, nil, nil, frkValue, 209, ftText, @sfnRightBDescription);
  ARepository.InternalAdd(@sfnSearch, fnSearch, fpiSearch, frkValue, 82, ftText, @sfnSearchDescription);
  ARepository.InternalAdd(@sfnSearchB, nil, nil, frkValue, 206, ftText, @sfnSearchBDescription);
  ARepository.InternalAdd(@sfnSubstitute, fnSubstitute, fpiSubstitute, frkValue, 120, ftText, @sfnSubstituteDescription);
  ARepository.InternalAdd(@sfnT, fnT, fpiT, frkValue, 130, ftText, @sfnTDescription);
  ARepository.InternalAdd(@sfnText, fnText, fpiText, frkValue, 48, ftText, @sfnTextDescription);
  ARepository.InternalAdd(@sfnTrim, fnTrim, fpiTrim, frkValue, 118, ftText, @sfnTrimDescription);
  ARepository.InternalAdd(@sfnUniChar, nil, nil, frkValue, 255, ftText, @sfnUniCharDescription, 1);
  ARepository.InternalAdd(@sfnUniCode, nil, nil, frkValue, 255, ftText, @sfnUniCodeDescription, 1);
  ARepository.InternalAdd(@sfnUpper, fnUpper, fpiUpper, frkValue, 113, ftText, @sfnUpperDescription);
  ARepository.InternalAdd(@sfnValue, fnValue, fpiValue, frkValue, 33, ftText, @sfnValueDescription)
end;
