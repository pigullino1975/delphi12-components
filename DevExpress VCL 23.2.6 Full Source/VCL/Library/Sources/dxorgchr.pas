...
const
  ...
  OtContentBottomAdditionalPadding = 6;
...
type
  ...
  public
    ...
    function GetNodeColor(ACurrentColor: TColor; ASelected: Boolean): TColor; virtual;
  ...
  protected
    ...
    procedure DoDrawText(Handle: HDC; Text: string; var Rect: TRect; Flags: Integer); virtual;
    ...
    function GetNodeMinSize: TSize; virtual;
    function GetPaddingSize: TSize;
  ...
  protected
    ...
    function GetTreeSize: TSize; virtual;
...
uses
  Types, Math, cxScrollBar, dxDPIAwareUtils, dxScrollAnimation, cxClasses, dxTypeHelpers;

const
  dxThisUnitName = 'dxorgchr';
...
type
  TdxScrollAnimationHelperAccess = class(TdxScrollAnimationHelper);
...
function TdxOrgChartCustomPainter.GetNodeColor(ACurrentColor: TColor; ASelected: Boolean): TColor;
begin
  if ASelected then
    Result := cxGetActualColor(Chart.SelectedNodeColor, LookAndFeelPainter.DefaultSelectionColor)
  else
    Result := cxGetActualColor(ACurrentColor, GetBackgroundColor);
end;
...
function TdxOrgChartCustomCustomizeForm.CalculateFormPosition(AOrgChart: TdxCustomOrgChart): TRect;
    ...
    procedure AdjustRectangle(const AMonitorRect: TRect; var AFormRect: TRect);
    begin
      AFormRect.Width := Min(AFormRect.Width,  AMonitorRect.Width);
      AFormRect.Height := Min(AFormRect.Height, AMonitorRect.Height);
      AFormRect.SetLocation(
        Min(Max(AFormRect.Left, AMonitorRect.Left), AMonitorRect.Right - AFormRect.Width),
        Min(Max(AFormRect.Top, AMonitorRect.Top), AMonitorRect.Bottom - AFormRect.Height));
    end;

var
  ATreeSize: TSize;
begin
  ATreeSize := GetTreeSize;
  if ATreeSize = cxInvalidSize then
    Result.InitSize(Top, Left, Width, Height)
  else
    Result.InitSize(Top, Left, Width - ATreeSize.Width + AOrgChart.Width, Height - ATreeSize.Height + AOrgChart.Height);
  AdjustRectangle(Monitor.WorkareaRect, Result);
end;
...
function TdxOrgChartCustomCustomizeForm.GetTreeSize: TSize;
begin
...
begin
  ...
  try
    ...
    AIndentX := IfThen(not Rotated, IndentX, IndentY); 
    AIndentY := IfThen(not Rotated, IndentY, IndentX);
    IndentX := MulDiv(AIndentX, M, D);
    IndentY := MulDiv(AIndentY, M, D);
...
function TdxCustomOrgChart.IsScrollDataValid: Boolean;
begin
  if IsZoomed then
    Result := False
  else
    Result := inherited IsScrollDataValid;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (ocFocus in Options) and ANode.Selected and LookAndFeelPainter.SupportsNativeFocusRect then
...
begin
  Result.cx := FullWidth + GetPaddingSize.cx;
  Result.cy := FullHeight + GetPaddingSize.cy;
end;
...
function TdxCustomOrgChart.GetPaddingSize: TSize;
begin
  Result.cx := Padding.Left + Padding.Right;
  Result.cy := Padding.Top + Padding.Bottom;
end;
...
begin
  ...
  begin
    ...
    try
      ...
      TdxImageDrawer.DrawUncachedImage(ACanvas.Handle, AImageRect, AImageRect, nil, Images, Node.ImageIndex, idmNormal, ocImageFiltering in Options);
    finally
      AScaleFactor.Free;
    end;
  end;
...
function TdxCustomOrgChart.GetNodeMinSize: TSize;
begin
  Result := dxGetImageSize(Images, ScaleFactor);
  Result.cx := Max(Result.cx * 2, 16);
  Result.cy := Max(Result.cy * 2, 16);
end;
...
var
  ANodeMinSize: TSize;
begin
  ANodeMinSize := GetNodeMinSize;
  if Value < ANodeMinSize.cx then
    Value := ANodeMinSize.cx;
...
begin
  ANodeMinSize := GetNodeMinSize;
  if Value < ANodeMinSize.cy then
    Value := ANodeMinSize.cy;
...
begin
  if Rotated then
    Result := FRoot.ChildrenHeight + OtContentBottomAdditionalPadding
  else
...
begin
  Result := LeftPos - Padding.Left;
end;

function TdxCustomOrgChart.GetTopEdge: Integer;
begin
  Result := TopPos - Padding.Top;
end;
...
begin
  ...
  begin
    ...
    begin
      if Rotated then
        Dec(R.Left, OtContentBottomAdditionalPadding)
      else
        Dec(R.Top, OtContentBottomAdditionalPadding);

      Result := True;
      if HasButton(Value) then
      begin
        if Rotated then
          Inc(R.Right, OtContentBottomAdditionalPadding)
        else
          Inc(R.Bottom, OtContentBottomAdditionalPadding);
      end;
...
procedure TdxCustomOrgChart.HitTestsAt(X, Y: Integer);
  ...
  begin
    ...
    if (Y >= R.Top) and (Y < R.Bottom + OtContentBottomAdditionalPadding) then
    begin
...
procedure TdxCustomOrgChart.DoNavigate(ANavigateCode: TdxOcNavigate; AValue: Integer);
  ...
  procedure SetLeftTopEdge(ALeft, ATop: Integer);
  begin
    SetLeftTop(Point(ALeft, ATop), IsScrollAnimationEnabled);
  end;

  procedure  SetLeft(AValue: Integer);
  begin
    SetLeftTopEdge(AValue, TopEdge);
  end;

  procedure SetTop(AValue: Integer);
  begin
    SetLeftTopEdge(LeftEdge, AValue);
  end;

const
  dxOcNavigateRotated: array[ocnLineLeft..ocnSelectNextDown] of TdxOcNavigate =
    (ocnLineUp, ocnLineLeft, ocnLineDown, ocnLineRight,
    ocnPageUp, ocnPageLeft, ocnPageDown, ocnPageRight,
    ocnTop, ocnLeft, ocnBottom, ocnRight,
    ocnTopPosition, ocnLeftPosition,
    ocnSelectNextUp, ocnSelectNextLeft, ocnSelectNextDown, ocnSelectNextRight);
var
  ANextNode: TdxOcNode;
  ALeft, ATop: Integer;
begin
  if TdxScrollAnimationHelperAccess(HorzScrollAnimationHelper).IsAnimationActive then
    ALeft := TdxScrollAnimationHelperAccess(HorzScrollAnimationHelper).TargetPosition
  else
    ALeft := LeftEdge;

  if TdxScrollAnimationHelperAccess(VertScrollAnimationHelper).IsAnimationActive then
    ATop := TdxScrollAnimationHelperAccess(VertScrollAnimationHelper).TargetPosition
  else
    ATop := TopEdge;

  if Rotated then
    ANavigateCode := dxOcNavigateRotated[ANavigateCode];

  case ANavigateCode of
    ocnLineLeft:
      SetLeft(ALeft - OtScrollUnit);
    ocnLineUp:
      SetTop(ATop - OtScrollUnit);
    ocnLineRight:
      SetLeft(ALeft + OtScrollUnit);
    ocnLineDown:
      SetTop(ATop + OtScrollUnit);
    ocnPageLeft:
      SetLeft(ALeft - ClientBoundsWidth + OtScrollUnit);
    ocnPageUp:
      SetTop(ATop - ClientBoundsHeight + OtScrollUnit);
    ocnPageRight:
      SetLeft(ALeft + ClientBoundsWidth - OtScrollUnit);
    ocnPageDown:
      SetTop(ATop + ClientBoundsHeight - OtScrollUnit);
    ...
    ocnTop:
      SetTop(0);
    ocnRight:
      SetLeft(FullWidth - ClientBoundsWidth);
    ocnBottom:
      SetTop(FullHeight - ClientBoundsHeight);
    ocnLeftPosition:
      SetLeft(AValue);
    ocnTopPosition:
      SetTop(AValue);
...
procedure TdxCustomOrgChart.DoPaddingChange(Sender: TObject);
begin
  FSizeChanged := True;
  RecalcSizes;
  Repaint;
...
begin
  ...
  if Shift = [ssCtrl] then
    case Key of
      ...
      VK_HOME: SetLeftTop(cxNullPoint, IsScrollAnimationEnabled);
      VK_END: SetLeftTop(Point(FullWidth, FullHeight), IsScrollAnimationEnabled);
...
begin
  if IsZoomed then
    Result := Max(1, DoZoom(FLineWidth)) 
  else
    Result := FLineWidth;
end;
...
begin
  Inc(FZoom);
  APaddingSize := GetPaddingSize;
  if (ClientBoundsWidth - APaddingSize.cx) * FullHeight <= (ClientBoundsHeight - APaddingSize.cy) * FullWidth then
  begin
    FZoomLo := ClientBoundsWidth - APaddingSize.cx;
    FZoomHi := FullWidth + 1;
  end
  else
  begin
    FZoomLo := ClientBoundsHeight - APaddingSize.cy;
    FZoomHi := FullHeight;
  end;
  Dec(FZoom);
end;
...
begin
  ...
  begin
    ...
    ANodeMinSize := GetNodeMinSize;
    if DefaultNodeWidth < ANodeMinSize.cx then
      DefaultNodeWidth := ANodeMinSize.cx;
    if DefaultNodeHeight < ANodeMinSize.cy then
      DefaultNodeHeight := ANodeMinSize.cy;

    Node := GetFirstNode;
    while Node <> nil do
    begin
      if (Node.FWidth <> 0) and (Node.FWidth < ANodeMinSize.cx) then
        Node.Width := ANodeMinSize.cx;
      if (Node.FHeight <> 0) and (Node.FHeight < ANodeMinSize.cy) then
        Node.Height := ANodeMinSize.cy;
      Node := Node.GetNext;
    end;
  end;
  EndUpdate;
...
var
  ASize: TSize;
begin
  ASize := Owner.GetNodeMinSize;
  if (Value <> 0) and (Value < ASize.cx) then
    Value := ASize.cx;
...
begin
  ASize := Owner.GetNodeMinSize;
  if (Value <> 0) and (Value < ASize.cy) then
    Value := ASize.cy;
...
var
  AForm: TdxOrgChartCustomCustomizeForm;
begin
  Result := FList.Count > 0;
  if Result then
  begin
    AForm := GetActualClass.Create(nil);
    try
      Result := AForm.Execute(AOrgChart);
    finally
      AForm.Release;
    end;
  end;
