...
{                                                                    }
{********************************************************************}
unit dxRichEdit.Import.Doc.DocContentBuilder; // for internal use
...
const
  dxThisUnitName = 'dxRichEdit.Import.Doc.DocContentBuilder';
...
var
  ...
  AComparable: TdxTextRunBorderComparable;
begin
  AOffset := PieceTable.GetOffset(I);
  ALength := PieceTable.GetLength(I);
  AComparable := TdxTextRunBorderComparable.Create(AOffset);
  if not TdxAlgorithms1<TdxTextRunBorder>.BinarySearch(FTextRunBorders, AComparable.CompareTo, AIndex) then
  begin
...
begin
  ...
  AComparable := TdxTextRunBorderComparable.Create(AOffset);
  TdxAlgorithms1<TdxTextRunBorder>.BinarySearch(ABorders, AComparable.CompareTo, ALowIndex);
  AComparable := TdxTextRunBorderComparable.Create(AOffset + ALength);
  TdxAlgorithms1<TdxTextRunBorder>.BinarySearch(ABorders, AComparable.CompareTo, AHighIndex);
