...
uses
  Windows, Messages, Classes, Forms, Types, CommCtrl, ComCtrls, Controls,
  dxCore, cxGeometry, cxControls;
...
const
  dxThisUnitName = 'dxComCtrlsUtils';
...
procedure cxTreeViewSetSelection(ATreeView: TTreeView; AList: TList);
  ...
  procedure GetNodesToSelect(ANodes: TList);
  var
    ANode: TTreeNode;
    AIndex: Integer;
    ASelection: TList;
  begin
    ...
    try
      ASelection.Assign(AList, laCopy);
      ANode := ATreeView.Items.GetFirstNode;
      while ANode <> nil do
      begin
        AIndex := ASelection.IndexOf(ANode.Data);
        if AIndex > -1 then
        begin
          ANodes.Add(ANode);
          ASelection.Delete(AIndex);
  ...
  procedure GetNodeToSelect(ANodes: TList);
  var
    ANode: TTreeNode;
  begin
    ANode := ATreeView.Items.GetFirstNode;
    while ANode <> nil do
    begin
      if AList.List[0] = ANode.Data then
      begin
        ANodes.Add(ANode);
        Break;
      end;
...
begin
  ...
  try
    if AList.Count = 1 then
      GetNodeToSelect(ANodes)
    else
      GetNodesToSelect(ANodes);
    ATreeView.Select(ANodes);
  finally
    ANodes.Free;
  end;
end;
