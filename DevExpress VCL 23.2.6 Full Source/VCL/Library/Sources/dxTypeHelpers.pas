...
type
  ...
  public
    ...
    class function Create(const AValue : Integer): TSize; overload; static;
    ...
    function Scale(AScaleFactor: Integer): TSize; overload; inline;
  ...
  public
    ...
    function DistanceToSegment(const A, B: TPoint): Single; inline;
    procedure Invert;
  ...
  public
    ...
    class function Create(const AValue: Integer): TRect; overload; static;
    ...
    procedure Offset(const P: TPoint); overload; inline;
    procedure Offset(const S: TSize); overload; inline;
    procedure Offset(const R: TRect); overload; inline;
    ...
    procedure NegativeOffset(const R: TRect); overload; inline;
    ...
    procedure Deflate(const ADelta: Integer); overload; inline;
    procedure Deflate(const ADeltaLeft, ADeltaTop, ADeltaRight, ADeltaBottom: Integer); overload; inline;
    procedure Deflate(const DX, DY: Integer); overload; inline;
    procedure Deflate(const AOffsets: TRect); overload; inline;
    procedure Inflate(const ADelta: Integer); overload; inline;
    ...
    procedure Inflate(const AOffsets: TRect); overload; inline;
...
class function TdxSizeHelper.Create(const AValue: Integer): TSize;
begin
  Result.cx := AValue;
  Result.cy := AValue;
end;
...
procedure TdxSizeHelper.Init(const AValue : Integer);
begin
  cx := AValue;
  cy := AValue;
end;
...
function TdxSizeHelper.Scale(AScaleFactor: Integer): TSize;
begin
  Result.cx := cx * AScaleFactor;
  Result.cy := cy * AScaleFactor;
end;
...
function TdxPointHelper.DistanceToSegment(const A, B: TPoint): Single;
var
  T: Single;
begin
  if A.IsEqual(B) then
    Exit(Distance(A));
  T := ((X - A.X) * (B.X - A.X) + (Y - A.Y) * (B.Y - A.Y)) /
    (Sqr(B.X - A.X) + Sqr(B.Y - A.Y));
  T := EnsureRange(T, 0, 1);
  Result := Sqrt(Sqr(A.X - X + (B.X - A.X) * T) + Sqr(A.Y - Y + (B.Y - A.Y) * T));
end;
...
procedure TdxRectHelper.Init(const AValue: Integer);
begin
  Left := AValue;
  Top := AValue;
  Right := AValue;
...
procedure TdxRectHelper.Deflate(const ADelta: Integer);
begin
  Inc(Left, ADelta);
  Dec(Right, ADelta);
  Inc(Top, ADelta);
...
procedure TdxRectHelper.Deflate(const ADeltaLeft, ADeltaTop, ADeltaRight, ADeltaBottom: Integer);
begin
  Inc(Left, ADeltaLeft);
  Dec(Right, ADeltaRight);
  Inc(Top, ADeltaTop);
  Dec(Bottom, ADeltaBottom);
end;

procedure TdxRectHelper.Deflate(const DX, DY: Integer);
begin
  Inc(Left, DX);
  Dec(Right, DX);
  Inc(Top, DY);
...
procedure TdxRectHelper.Deflate(const AOffsets: TRect);
begin
  Inc(Left, AOffsets.Left);
  Dec(Right, AOffsets.Right);
  Inc(Top, AOffsets.Top);
  Dec(Bottom, AOffsets.Bottom);
end;

procedure TdxRectHelper.Inflate(const ADelta: Integer);
begin
  Dec(Left, ADelta);
  Inc(Right, ADelta);
  Dec(Top, ADelta);
...
procedure TdxRectHelper.Inflate(const AOffsets: TRect);
begin
  Dec(Left, AOffsets.Left);
  Inc(Right, AOffsets.Right);
  Dec(Top, AOffsets.Top);
  Inc(Bottom, AOffsets.Bottom);
end;
...
procedure TdxRectHelper.NegativeOffset(const R: TRect);
begin
  Dec(Left, R.Left);
  Dec(Right, R.Right);
  Dec(Top, R.Top);
...
procedure TdxRectHelper.Offset(const R: TRect);
begin
  Inc(Left, R.Left);
  Inc(Right, R.Right);
  Inc(Top, R.Top);
...
class function TdxRectHelper.Create(const AValue: Integer): TRect;
begin
  Result.Left := AValue;
  Result.Top := AValue;
  Result.Right := AValue;
