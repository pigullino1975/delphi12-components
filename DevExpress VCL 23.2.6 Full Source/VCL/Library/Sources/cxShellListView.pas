...
function GetPIDLDisplayName(APIDL: PItemIDList; AShowFullPath: Boolean = False; AIsForParsing: Boolean = False): string;
...
var
  dxShellForbidImmediateSpecialFolderInfoTableInitialization: Boolean = False;
  cxShellSpecialFolderInfoTable: array[CSIDL_DESKTOP..CSIDL_HISTORY] of
    TcxShellSpecialFolderInfoTableItem;

procedure dxShellPrepareShellSpecialFolderInfoTable;
...
const
  dxThisUnitName = 'cxShellListView';
...
var
  FIsPrepareShellSpecialFolderInfoTableInitialized: Boolean = False;
...
begin
  ...
  FIsPrepareShellSpecialFolderInfoTableInitialized := False;
end;
...
function GetPIDLDisplayName(APIDL: PItemIDList; AShowFullPath: Boolean = False; AIsForParsing: Boolean = False): string;
const
  ParsingFlagMap: array[Boolean] of Cardinal = (0, SHGDN_FORPARSING);
var
  AParentIFolder: IShellFolder;
  ATempPartPidlName: string;
  ATempPartPIDL: PItemIDList;
  I: Integer;
begin
  Result := '';
  ...
  begin
    ATempPartPIDL := ExtractParticularPidl(APIDL);
    try
      APIDL := GetNextItemID(APIDL);

      if AShowFullPath then
      begin
        ATempPartPidlName := dxGetPidlDisplayName(AParentIFolder, ATempPartPIDL, SHGDN_INFOLDER or ParsingFlagMap[AIsForParsing]);
        if ATempPartPidlName = '' then
          Exit;
        if Result <> '' then
          Result := Result + '\';
        Result := Result + ATempPartPidlName;
      end;

      if not Succeeded(AParentIFolder.BindToObject(ATempPartPIDL, nil, IID_IShellFolder, Pointer(AParentIFolder))) then
      begin
        Result := '';
        Exit;
      end;
    finally
      DisposePidl(ATempPartPIDL);
    end;
  end;

  if not AShowFullPath then
    Result := dxGetPidlDisplayName(AParentIFolder, APIDL, SHGDN_NORMAL or ParsingFlagMap[AIsForParsing]);
end;
...
function PathToAbsolutePIDL(APath: string; ARoot: TcxCustomShellRoot;
  ...
  begin
    Result := nil;
    APathType := CheckShellObjectPath(APath, GetPidlName(ARoot.Pidl).ToUpper, False);
    case APathType of
      sptIncorrect:
        Exit;
...
var
  ...
  ATempPIDL: PItemIDList;
begin
  if not FIsPrepareShellSpecialFolderInfoTableInitialized then
  begin
    ...
    for ACSIDL := CSIDL_DESKTOP to CSIDL_HISTORY do
      ...
      FIsPrepareShellSpecialFolderInfoTableInitialized := True;
  end;
end;
...
begin
  if not dxShellForbidImmediateSpecialFolderInfoTableInitialization then
    dxShellPrepareShellSpecialFolderInfoTable;
end;
