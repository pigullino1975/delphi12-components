...
var
  dxSpreadSheetPasteSpecialDialogClass: TdxSpreadSheetPasteSpecialDialogFormClass = TdxSpreadSheetPasteSpecialDialogForm; 
...
const
  dxThisUnitName = 'dxSpreadSheetPasteSpecialDialog';
...
procedure ShowPasteSpecialDialog(ASpreadSheet: TdxCustomSpreadSheet);
begin
  with dxSpreadSheetPasteSpecialDialogClass.Create(nil) do
  try
...
begin
  ...
  if not lgPasteOptions.Visible then
       lgPasteFormats.Height:= ScaleFactor.Apply(269);
...
begin
  if (AFormat <> nil) and (APasteOptions <> []) then
    TdxSpreadSheetTableViewAccess(Table).PasteFromClipboardCore(APasteOptions, AFormat.LoadFromClipboard);
end;
