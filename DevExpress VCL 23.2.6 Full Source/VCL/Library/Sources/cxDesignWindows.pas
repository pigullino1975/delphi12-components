...
type
  ...
  TcxDesignHelper = class(TObject, IUnknown, IcxDesignHelper, IcxDesignHelper2)
  private
...
const
  dxThisUnitName = 'cxDesignWindows';
...
begin
  ...
  try
    ...
    InitFormEditor;
    cxDialogsMetricsStore.InitDialog(Result);
    Show;
  except
    Free;
  end;
...
begin
  if (AListener <> nil) and Supports(AListener, IcxDesignSelectionChanged) and (FListeners.IndexOf(AListener) < 0) then
  begin
    if FWindow = nil then
    begin
...
function TcxDesignHelper.CanDeleteComponent(AOwner, AComponent: TComponent): Boolean;
begin
...
procedure TcxDesignHelper.ClearSelection(AOwner: TComponent);
var
  AList: TList;
begin
  AList := TList.Create;
...
begin
  ...
  begin
    if Supports(TObject(FListeners[I]), IcxDesignSelectionChanged, AIntf) then
    begin
...
procedure TcxDesignHelper.SelectionsChanged(Sender: TObject; const ASelection: TDesignerSelectionList);
var
  L: TList;
begin
  L := TList.Create;
