...
unit dxRichEdit.Import.Html.ImageTags; // for internal use
...
uses
  ...
  dxUriRecord,
  dxRichEdit.DocumentModel.Core, dxCore;

const
  dxThisUnitName = 'dxRichEdit.Import.Html.ImageTags';

type
  TdxUriOfficeImageAccess = class(TdxUriOfficeImage);
...
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorStarted(UnitName, 'TdxImageTag.Initialize', SysInit.HInstance);{$ENDIF}
  FAttributeTable := AddAttributes;
  FAlignments := CreateAllignmentsTable;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.ConstructorFinished(UnitName, 'TdxImageTag.Initialize', SysInit.HInstance);{$ENDIF}
end;

class destructor TdxImageTag.Finalize;
begin
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorStarted(UnitName, 'TdxImageTag.Finalize', SysInit.HInstance);{$ENDIF}
  FAttributeTable.Free;
  FAlignments.Free;
  {$IFDEF DX_INITIALIZATION_LOGGING}TdxUnitSectionsLogger.DestructorFinished(UnitName, 'TdxImageTag.Finalize', SysInit.HInstance);{$ENDIF}
end;
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        if (AImage.Image.Width > 0) and (AWidth.&Type <> TdxWidthUnitType.&Nil) then
          AScaleX := Importer.UnitConverter.ModelUnitsToTwips(AWidthInModelUnits) / AImage.Image.SizeInTwips.cx * 100;

        if (AImage.Image.Height > 0) and (AHeight.&Type <> TdxWidthUnitType.&Nil) then
        ...
        begin
          AScaleY := AScaleX;
          AHeightInModelUnits := Importer.UnitConverter.TwipsToModelUnits(AImage.SizeInTwips.cy);
          AHeightInModelUnits := Trunc(AHeightInModelUnits * AScaleY / 100.0);
        end
        else
          if (AWidth.&Type = TdxWidthUnitType.&Nil) and (AScaleY <> 100.0) then
          begin
            AScaleX := AScaleY;
            AWidthInModelUnits := Importer.UnitConverter.TwipsToModelUnits(AImage.SizeInTwips.cx);
            AWidthInModelUnits := Trunc(AWidthInModelUnits * AScaleX / 100.0);
          end;
      end;
