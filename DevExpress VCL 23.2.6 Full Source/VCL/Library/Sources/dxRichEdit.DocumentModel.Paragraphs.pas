...
unit dxRichEdit.DocumentModel.Paragraphs; // for internal use
...
type
  ...
  strict private
    function GetItem(Index: Integer): TdxIndexedTreeNodeBase; inline;
  public
  ...
  public
    ...
    function Last: TdxCustomParagraph; override; final;
    ...
    function FindItemByIndex(ARelativeIndex: Integer): TdxCustomParagraph; override; final;
  ...
  strict private
    function GetItem(Index: Integer): TdxCustomParagraph; inline;
    procedure SetItem(Index: Integer; const Value: TdxCustomParagraph);
  public
    function First: TdxCustomParagraph; reintroduce; inline;
    function Last: TdxCustomParagraph; reintroduce; inline;
  ...
  public
    ...
    function First: TdxCustomParagraph; inline;
    function Last: TdxCustomParagraph; inline;
...
const
  dxThisUnitName = 'dxRichEdit.DocumentModel.Paragraphs';
...
begin
  ...
  begin
    if TdxIndexedTreeNodeBase(Children.List[I]).FirstRelativeIndex > ARelativeIndex then
      Break;
    Inc(I);
  end;
  APrevChildren := Children.List[I - 1];
...
begin
  ...
  if (ANewChildIndex > MaxChildrenCount div 2) then
  begin
...
begin
  Assert(ChildrenCount > 0);
  Result := TdxIndexedTreeNodeBase(Children.List[ChildrenCount - 1]).Last;
end;
...
begin
  ...
  while AParentLevel <> nil do
  begin
    Inc(Result, AParentLevel.FirstRelativeIndex);
    AParentLevel := AParentLevel.Parent;
  end;
end;
...
begin
  ...
  begin
    ...
    while AParentLevel <> nil do
    begin
      Inc(Result, AParentLevel.RelativeLogPosition);
      AParentLevel := AParentLevel.Parent;
    end;
  end;
end;
...
begin
  Result := Items{$IFDEF RELEASE}.List{$ENDIF}[ARelativeIndex];
end;
...
begin
  if FItems.Count <= 0 then
    ...
    Result := TdxCustomParagraph(FItems.List[FItems.Count - 1]);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        AChild := ACurrentMiddleLevel.Children.List[I];
