...
uses
  ...
  dxPDFTypes, dxPDFUtils, dxPDFDocumentState;

const
  dxThisUnitName = 'dxPSdxPDFViewerLnk';
...
function TdxPDFPrinting.IsPrinting: Boolean;
begin
  Result := (GetRenderParameters as TdxPDFPrintParameters).IsPrinting;
end;
...
var
  ADPIFactor, APreviewFactor: Single;
  AScaledHalfWidth, AScaledHalfHeight: Integer;
  APageSize: TdxPointF;
begin
  Result := TdxPDFPrintParameters.Create(AParameters.DocumentState);
  Result.IsPrinting := AParameters.IsPrinting;
  Result.Canvas := AParameters.Canvas;
  Result.Rect := AParameters.Rect;
  Result.ZoomFactor := AParameters.ZoomFactor;
  ...
  APreviewFactor := AParameters.ZoomFactor / 100;
  ADPIFactor := AParameters.DPI / cxGetCurrentDPI;
  Result.ScaleFactor := AParameters.ScaleFactor * APreviewFactor * ADPIFactor;

  Result.Position := cxPointF(AParameters.ContentBounds.Left * ADPIFactor, AParameters.ContentBounds.Top * ADPIFactor);
  APageSize := TdxPDFViewerViewState.CalculatePageSize(APage.Size, (AParameters.DocumentState as TdxPDFDocumentState).RotationAngle);
  ARealBounds := cxRect(cxRectCenter(AParameters.ContentBounds), cxRectCenter(AParameters.ContentBounds));
  AScaledHalfWidth := Trunc(APageSize.X * AParameters.ScaleFactor / 2);
  AScaledHalfHeight := Trunc(APageSize.Y * AParameters.ScaleFactor / 2);
  ARealBounds := cxRectInflate(ARealBounds, AScaledHalfWidth, AScaledHalfHeight);
  ARealBounds := cxRectScale(ARealBounds, AParameters.DPI, cxGetCurrentDPI);
  ...
  Result.Position.X := Result.Position.X * APreviewFactor;
  Result.Position.Y := Result.Position.Y * APreviewFactor;

  ARealBounds.Left := Trunc(ARealBounds.Left * APreviewFactor);
  ARealBounds.Top := Trunc(ARealBounds.Top * APreviewFactor);
  ARealBounds.Right := Trunc(ARealBounds.Right * APreviewFactor);
  ARealBounds.Bottom := Trunc(ARealBounds.Bottom * APreviewFactor);
end;
...
begin
  case PrinterPage.ScaleMode of
    ...
    smAdjust:
      FScaleFactor := PrinterPage.ScaleFactor / 100 * 96 / 72;
  else
    FScaleFactor := 1;
  end;
end;
...
begin
  ...
  begin
    ...
    ARenderInfo.FPDFPage := TdxPDFPagesAccess(ADocument.Pages).List[FPageIndices[I] - 1];
...
function TdxPDFViewerReportLink.GetCapabilities: TdxReportLinkCapabilities;
begin
  Result := inherited GetCapabilities - [rlcExportToPDF];
end;
...
procedure TdxPDFViewerReportRenderer.RenderPageContent;
  ...
  procedure SetLogicalCoordinates(ACanvas: TcxCanvas);
  begin
    SetMapMode(ACanvas.Handle, 0);
    SetWindowExtEx(ACanvas.Handle, PageRenderInfo.PageSize.X, PageRenderInfo.PageSize.Y, nil);
    ScaleWindowExtEx(ACanvas.Handle, 1, 1, 1, 1, nil);
    SetViewPortExtEx(ACanvas.Handle, PageRenderInfo.PageSize.X, PageRenderInfo.PageSize.Y, nil);
    SetViewPortOrgEx(ACanvas.Handle, ViewPortRect.Left, ViewPortRect.Top, nil);
  end;
...
begin
  ...
  begin
    if not IsPrinting then
      ARenderCanvas.Canvas.IntersectClipRect(PageRenderInfo.ContentBounds);
    ARenderCanvas.Canvas.SaveState;
    try
      SetLogicalCoordinates(ARenderCanvas.Canvas);
      AParameters := TdxPDFPrintParameters.Create(TdxPDFDocumentAccess(Document).State);
      AParameters.IsPrinting := IsPrinting;
      AParameters.DPI := Canvas.PixelsPerInch;
      AParameters.ScaleFactor := PageRenderInfo.FScaleFactor;
      AParameters.ZoomFactor := ZoomFactor;
      ...
      try
        if NeedPrintAsImage or not IsPrinting then
      ...
      ARenderCanvas.Canvas.RestoreState;
    end;
  end;
end;
