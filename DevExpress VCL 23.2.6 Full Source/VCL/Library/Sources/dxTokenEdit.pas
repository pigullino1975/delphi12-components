...
uses
  ...
  cxListBox, cxDataUtils, cxAccessibility;
...
type
  ...
  { TdxTokenEditCustomTokenAccessibilityHelper }

  TdxTokenEditAccessibilityHelper = class;

  TdxTokenEditCustomTokenAccessibilityHelper = class(TdxEditChildAccessibilityHelper) // for internal use
  strict private
    function GetViewInfo: TdxTokenEditCustomTokenViewInfo;
  protected
    ...
    property ViewInfo: TdxTokenEditCustomTokenViewInfo read GetViewInfo;
  public
    function GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect; override;
  end;

  TdxTokenEditCustomTokenAccessibilityHelperClass = class of TdxTokenEditCustomTokenAccessibilityHelper;
  ...
  protected
    ...
    function GetAccessibilityHelperClass: TdxTokenEditCustomTokenAccessibilityHelperClass; virtual;
  ...
  { TdxTokenEditTokenAccessibilityHelper }

  TdxTokenEditTokenAccessibilityHelper = class(TdxTokenEditCustomTokenAccessibilityHelper) // for internal use
  strict private
    function GetViewInfo: TdxTokenEditTokenViewInfo;
  protected
    ...
    property ViewInfo: TdxTokenEditTokenViewInfo read GetViewInfo;
  public
    function GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect; override;
  end;
  ...
  protected
    ...
    function GetAccessibilityHelperClass: TdxTokenEditCustomTokenAccessibilityHelperClass; override;
  ...
  public
    ...
    //
    property Items[Index: Integer]: TdxTokenEditTokenViewInfo read GetItem write SetItem; default; // for internal use
  end;
  ...
  public
    ...
    property Properties: TdxTokenEditProperties read GetProperties;
    property Tokens: TdxTokenEditTokenViewInfoList read FTokens; // for internal use
  end;
  ...
  public
    ...
    property HitTest: TdxTokenEditHitTest read FHitTest; // for internal use
    property InnerEdit: IdxInnerTokenEdit read GetInnerEdit; // for internal use
    property ViewInfo: TdxTokenEditViewInfo read FViewInfo; // for internal use
  end;
  ...
  { TdxTokenEditAccessibilityHelper }

  TdxTokenEditAccessibilityHelper = class(TdxEditAccessibilityHelper)
  strict private
    function GetTokenEdit: TdxCustomTokenEdit;
    function GetTokenEditViewInfo: TdxTokenEditViewInfo;
  protected
    function GetChildCount: Integer; override;

    procedure DoCloseButtonClick(const AText: string; AToken: TdxTokenEditToken);
    procedure DoGlyphClick(const AText: string; AToken: TdxTokenEditToken);
    procedure DoTokenClick(const AText: string; AToken: TdxTokenEditToken);

    property TokenEdit: TdxCustomTokenEdit read GetTokenEdit;
    property TokenEditViewInfo: TdxTokenEditViewInfo read GetTokenEditViewInfo;
  end;
...
uses
  RTLConsts, cxEditConsts, StrUtils, dxDPIAwareUtils, dxStringHelper, cxLookAndFeels, dxTypeHelpers;

const
  dxThisUnitName = 'dxTokenEdit';
    ...
    try
      ...
      begin
        ...
        begin
          ...
          if not AError or AIsUserErrorDisplayValue then
            ATokens[I] := AToken;
        end;
...
begin
  ...
  if not AViewInfo.SupportsPadding and ((IsInplace and IsPaddingAllowedForInplace) or
    (TdxVisualRefinements.UsePaddingForStandaloneEditors and IsPaddingAllowedForStandalone)) then
  begin
    AActualPadding := TdxVisualRefinements.Padding;
    AdjustPadding(AActualPadding);
    AActualPadding.InflatePadding(Result, ScaleFactor, UseRightToLeftAlignment);
  end;
end;
...
function TdxTokenEditCustomTokenAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  Result := ViewInfo.Bounds;
  if Parent <> nil then
    Result := cxRectOffset(Result, Parent.GetScreenBounds(0).TopLeft)
end;

function TdxTokenEditCustomTokenAccessibilityHelper.GetDefaultActionDescription(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := 'Click';
end;

function TdxTokenEditCustomTokenAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := ViewInfo.Caption
end;

function TdxTokenEditCustomTokenAccessibilityHelper.GetParent: TcxAccessibilityHelper;
begin
  if ViewInfo.Owner.Edit <> nil then
    Result := TdxCustomTokenEdit(ViewInfo.Owner.Edit).IAccessibilityHelper.GetHelper
  else
    Result := nil;
end;

function TdxTokenEditCustomTokenAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxROLE_SYSTEM_SPLITBUTTON
end;

function TdxTokenEditCustomTokenAccessibilityHelper.GetState(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  Result := cxSTATE_SYSTEM_NORMAL;
  if ViewInfo.State = cxbsHot then
    Result := Result or cxSTATE_SYSTEM_HOTTRACKED;
end;

function TdxTokenEditCustomTokenAccessibilityHelper.GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID): TcxAccessibleObjectProperties;
begin
  Result := [aopFocus, aopLocation, aopDefaultAction];
end;

function TdxTokenEditCustomTokenAccessibilityHelper.IsExtended: Boolean;
begin
  Result := True;
end;

function TdxTokenEditCustomTokenAccessibilityHelper.GetViewInfo: TdxTokenEditCustomTokenViewInfo;
begin
  Result := TdxTokenEditCustomTokenViewInfo(OwnerObject);
end;
...
destructor TdxTokenEditCustomTokenViewInfo.Destroy;
begin
...
begin
  if not Painter.ApplyEditorAdvancedMode then
    Result := Painter.GetScaledTokenContentOffsets(Owner.IsInplace, ScaleFactor)
  else
    Result := Painter.GetEditorItemsPadding(ScaleFactor);
...
begin
  Result := cxSizeAddMargins(cxTextSize(Owner.Font, Caption), GetContentOffsets);
end;

procedure TdxTokenEditCustomTokenViewInfo.Offset(DX: Integer; DY: Integer);
begin
  OffsetRect(FBounds, DX, DY);
  OffsetRect(FCaptionBounds, DX, DY);
end;
...
function TdxTokenEditCustomTokenViewInfo.GetAccessibilityHelperClass: TdxTokenEditCustomTokenAccessibilityHelperClass;
begin
  Result := TdxTokenEditCustomTokenAccessibilityHelper;
end;
...
function TdxTokenEditCustomTokenViewInfo.GetIAccessibilityHelper: IcxAccessibilityHelper;
begin
...
function TdxTokenEditTokenAccessibilityHelper.GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID): TRect;
begin
  case AChildID of
    1:
      Result := ViewInfo.CloseGlyphBounds;
    2:
      Result := ViewInfo.GlyphBounds;
  else
    Result := ViewInfo.Bounds
  end;
  if Parent <> nil then
...
function TdxTokenEditTokenAccessibilityHelper.ChildIsSimpleElement(AIndex: Integer): Boolean;
begin
  Result := True;
end;

procedure TdxTokenEditTokenAccessibilityHelper.DoDefaultAction(AChildID: TcxAccessibleSimpleChildElementID);
begin
  if Parent <> nil then
  begin
    case AChildID of
      1:
        TdxTokenEditAccessibilityHelper(Parent).DoCloseButtonClick(ViewInfo.Text, ViewInfo.Item);
      2:
        TdxTokenEditAccessibilityHelper(Parent).DoGlyphClick(ViewInfo.Text, ViewInfo.Item);
    else
      TdxTokenEditAccessibilityHelper(Parent).DoTokenClick(ViewInfo.Text, ViewInfo.Item);
    end;
  end;
end;

function TdxTokenEditTokenAccessibilityHelper.GetChildCount: Integer;
begin
  Result := 2;
end;

function TdxTokenEditTokenAccessibilityHelper.GetName(AChildID: TcxAccessibleSimpleChildElementID): string;
begin
  Result := ViewInfo.Caption;
  if AChildID = 1 then
...
function TdxTokenEditTokenAccessibilityHelper.GetRole(AChildID: TcxAccessibleSimpleChildElementID): Integer;
begin
  if AChildID = 0 then
    Result := cxROLE_SYSTEM_SPLITBUTTON
  else
...
function TdxTokenEditTokenAccessibilityHelper.GetViewInfo: TdxTokenEditTokenViewInfo;
begin
  Result := TdxTokenEditTokenViewInfo(OwnerObject);
end;
...
procedure TdxTokenEditTokenViewInfo.Offset(DX: Integer; DY: Integer);
begin
  inherited Offset(DX, DY);
  OffsetRect(FCloseGlyphBounds, DX, DY);
  OffsetRect(FGlyphBounds, DX, DY);
end;
...
procedure TdxTokenEditTokenViewInfo.CalculateGlyphBounds(var ABounds: TRect; const ASize: TSize;
  APosition: TdxTokenEditElementPosition);
begin
  ABounds := cxRectCenterVertically(CaptionBounds, ASize.cy);
  if Painter.ApplyEditorAdvancedMode and Odd(CaptionBounds.Top + CaptionBounds.Bottom - ASize.cy) then 
    ABounds.Offset(0, 1); 
...
begin
  TdxImageDrawer.DrawImage(ACanvas, R, Item.Glyph, Images, Item.ImageIndex, ifmFit,
    DrawMode[State = cxbsDisabled], False, Painter.GetTokenColorPalette(State), ScaleFactor);
end;

function TdxTokenEditTokenViewInfo.GetAccessibilityHelperClass: TdxTokenEditCustomTokenAccessibilityHelperClass;
begin
  Result := TdxTokenEditTokenAccessibilityHelper;
end;
...
function TdxTokenEditViewInfo.GetPadding: Integer;
begin
...
procedure TdxTokenEditViewInfo.Offset(DX: Integer; DY: Integer);
var
  I: Integer;
begin
  inherited Offset(DX, DY);
  for I := 0 to Tokens.Count - 1 do
    Tokens[I].Offset(DX, DY);
  MoreTokensItem.Offset(DX, DY);
end;
...
begin
  ...
  begin
    ...
    Result.cy := TdxTextMeasurer.TextLineHeight(Font);
  end
  else
    Result := cxNullSize;
end;
...
procedure TdxTokenEditViewInfo.CalculateLayoutCore(const AContentRect: TRect; ARowHeight, AMaxRowCount, AMinItemSize: Integer);
  ...
  function IsNewRowNeeded(const AOffset: TPoint; const AItemWidth: Integer): Boolean;
  begin
    Result := (AOffset.X <> AContentRect.Left) and (AOffset.X + AItemWidth > AContentRect.Right) and (AMaxRowCount > 1);
  end;

  function IsInnerEditStretchNeeded(const AOffset: TPoint): Boolean;
  begin
    Result := (EditingTokenIndex = (Tokens.Count - 1)) or
      IsNewRowNeeded(AOffset, Max(Tokens[EditingTokenIndex + 1].MeasureSize.cx, InnerEditSize.cx));
  end;

  function CalculateItemBounds(var AOffset: TPoint; AItemWidth: Integer): TRect;
  begin
    if IsNewRowNeeded(AOffset, AItemWidth) then
    begin
  ...
  procedure StretchInnerEditToTheEndOfTheLine;
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if IsInnerEditStretchNeeded(AOffset) then
        StretchInnerEditToTheEndOfTheLine;
  ...
  begin
    ...
    begin
      ...
      begin
        InnerEditRect := CalculateItemBounds(AOffset, InnerEditSize.cx);
        InnerEditRect.Deflate(Painter.GetEditorItemsPadding(ScaleFactor));
      end;
      StretchInnerEditToTheEndOfTheLine;
    end;
  end;
...
begin
  Result := cxSizeAddMargins(AContentSize, cxMargins(ClientRect, ContentDisplayRect));
end;
...
begin
  ...
  begin
    Result := ClientRect;
    Result.Deflate(Painter.GetScaledTokenContentOffsets(IsInplace, ScaleFactor));
  end
  else
...
begin
  ...
  begin
    AInnerEditRect := InnerEditRect;
    Inc(AInnerEditRect.Bottom, Painter.GetEditorItemsPadding(ScaleFactor).Bottom);
    AContentRect := cxRectUnion(AContentRect, AInnerEditRect);
  end;
...
begin
  ...
  if (Edit <> nil) and not Edit.InnerControlMouseDown and
    not ((FocusedObject is TdxTokenEditViewInfo) and (EditingTokenIndex = - 1)) then
    StopEditing(False);
...
begin
  ...
  try
    ...
    if (AEditingTokenIndex = -1) or (AEditingTokenIndex <> Container.ActiveProperties.FirstInvalidTokenIndex) then
      AEditingTokenIndex := Container.ActiveProperties.FirstInvalidTokenIndex;
...
procedure TdxTokenEditInnerEdit.CMWantSpecialKey(var Message: TCMWantSpecialKey);
begin
...
begin
  ...
  if Result then
  begin
    APostToken := (ActiveProperties.FirstInvalidTokenIndex >= 0) and
      (Controller.EditingTokenIndex = ActiveProperties.FirstInvalidTokenIndex);
    if APostToken then
      Text := '';
    Controller.StopEditing(APostToken);
  end;
end;
...
begin
  Result := (FAutoCompleteWindow <> nil) and FAutoCompleteWindow.IsVisible;
end;
...
function TdxTokenEditAccessibilityHelper.GetChildCount: Integer;
var
  I: Integer;
begin
  inherited;
  for I := 0 to TokenEditViewInfo.Tokens.Count - 1 do
    AddAccessibilityChild(TokenEditViewInfo.Tokens.Items[I].IAccessibilityHelper);
  Result := AccessibilityChildren.Count;
end;

procedure TdxTokenEditAccessibilityHelper.DoCloseButtonClick(const AText: string; AToken: TdxTokenEditToken);
begin
  TokenEdit.DoTokenDelete(AText, AToken);
end;

procedure TdxTokenEditAccessibilityHelper.DoGlyphClick(const AText: string; AToken: TdxTokenEditToken);
begin
  TokenEdit.DoTokenGlyphClick(AText, AToken);
end;

procedure TdxTokenEditAccessibilityHelper.DoTokenClick(const AText: string; AToken: TdxTokenEditToken);
begin
  TokenEdit.DoTokenClick(AText, AToken);
end;

function TdxTokenEditAccessibilityHelper.GetTokenEdit: TdxCustomTokenEdit;
begin
  Result := TdxCustomTokenEdit(Edit);
end;

function TdxTokenEditAccessibilityHelper.GetTokenEditViewInfo: TdxTokenEditViewInfo;
begin
  Result := TdxTokenEditViewInfo(ViewInfo);
end;
...
procedure TdxCustomTokenEdit.ClearSelection;
var
...
begin 
  if SelLength = 0 then
    Exit;
  AText := VarToStr(InnerTextEdit.EditValue);
  Delete(AText, SelStart + 1, SelLength);
  APrevSelStart := SelStart;
  InnerTextEdit.EditValue := AText;
...
function TdxCustomTokenEdit.GetVScrollBarBounds: TRect;
begin
  Result := ClientBounds;
  Result.Top := ViewInfo.BorderRect.Top;
  Result.Bottom := ViewInfo.BorderRect.Bottom;
  if GetScrollbarMode = sbmClassic then begin
  ...
  try
    ...
    try
      ...
      try
        ...
        begin
          if (ATokensToAdd.Count = 1) and (ATokens[ATokenIndex] = ATokensToAdd[0]) and
            (ATokenIndex <> ActiveProperties.FirstInvalidTokenIndex) then
            Exit;
...
function TdxCustomTokenEdit.GetAccessibilityHelperClass: TdxEditAccessibilityHelperClass;
begin
  Result := TdxTokenEditAccessibilityHelper;
end;
