...
type
  ...
  protected
    ...
    procedure DoDrawTabCloseButton(ACanvas: TcxCanvas; const ARect: TRect;
      AState: TcxPCNavigatorButtonState); override;
    procedure DrawButtonFrameAndBackground(ACanvas: TcxCanvas; const ARect: TRect;
  ...
  public
    ...
    class function HasTabPressedState: Boolean; override;
  end;
  ...
  protected
    ...
    function GetTabVerticalCorrection(const AFullRect: TRect; ATabPosition: TcxTabPosition): TRect; override;
  ...
  protected
    ...
    function DirectionalGetFigureRegion(const R: TRect; APoints: array of TPoint;
      ALinePositions: array of TLinePosition; ATabPosition: TcxTabPosition; AForContent: Boolean): TcxRegion;
...
const
  ...
  cxPCArrowConversionA: array [nbTopLeft .. nbBottomRight, Boolean] of TcxPCArrow = (
    (aLeft, aTop),
    (aRight, aBottom)
  );
...
var
  ...
  OneNoteMainTabBorderColor: TColor;
  OneNoteTabBorderColor: TColor;
  OneNoteMainTabInnerBorderColor: TColor;
  OneNoteTabHotBorderColor: TColor;
  OneNoteTabInnerBorderColor1: TColor;
  OneNoteTabInnerBorderColor2: TColor;
...
begin
  ...
  dxDrawThemeBackground(DC, totTab, ScaleFactor, R, TABP_PANE, 0);
end;
  ...
  begin
    ...
    begin
      if IsWinSevenOrLater then
        Result := dxGetSystemMetrics(SM_CYHSCROLL, ScaleFactor)
      else
        if GetThemeScaledPartSize(totComboBox, ScaleFactor, CP_DROPDOWNBUTTON, ASize, CBXS_NORMAL) then
          Result := ASize.cx;
    end;
  end
  else
...
function TcxPCTabsPainter.GetTabVerticalCorrection(const AFullRect: TRect; ATabPosition: TcxTabPosition): TRect;
begin
  Result := GetExtendedRect(AFullRect, Rect(0, 0, 0, -1), ATabPosition);
end;
...
begin
  ...
  begin
    ...
    ATheme := OpenTheme(totButton, ScaleFactor);
    if ViewInfo.ParentBackground and
      IsThemeBackgroundPartiallyTransparent(ATheme, BP_PUSHBUTTON,
        BtnStateToXPBtnState(NavigatorBtnStateToLookAndFeelBtnState[AButtonInfo.State]))
    then
...
begin
  ...
  ATheme := OpenTheme(AThemeObjectType, ScaleFactor);
...
procedure TcxPCTabsPainter.PaintNativeTabBackground(DC: HDC; ATabVisibleIndex: Integer; const ABounds: TRect);
var
  APartId, AStateId: Integer;
begin
  GetTabNativePartAndState(ATabVisibleIndex, APartId, AStateId);
  dxDrawThemeBackground(DC, totTab, ScaleFactor, ABounds, APartId, AStateId);
end;
...
begin
  ...
  if AMainTab then
    R := GetTabVerticalCorrection(R, ATabPaintingPosition);
  if ATabPaintingPosition in [tpLeft, tpRight] then
  ...
  begin
    ...
    begin
      ...
      AFlipVertically := ATabViewInfo.PaintingPositionIndex in [1, 2, 6, 7, 5, 12];
    end
    else
    begin
      case ATabViewInfo.PaintingPositionIndex of
      ...
      AFlipVertically := ATabViewInfo.PaintingPositionIndex in [3, 6, 7, 5, 10, 11];
    end;
  end
  else
  begin
    ...
    AFlipVertically := ATabPaintingPosition in [tpLeft, tpBottom];
  end;
  ABitmap.Rotate(ARotationAngle, AFlipVertically);
end;
...
begin
  ...
  AMinSize := cxSize(AOffsets.Left + AOffsets.Right, AOffsets.Top + AOffsets.Bottom);
  ...
  if IsWinSevenOrLater and (ViewInfo.TabsViewInfo.ViewInfoCount > 0) then 
  begin
    ATabViewInfo := TabViewInfo[0];
    if not (ATabViewInfo.PaintingPositionIndex in [1, 2, 3, 5, 6, 8, 9]) then
      Inc(ARect.Bottom);

    if ViewInfo.UseRightToLeftAlignment and (ViewInfo.TabPosition in [tpTop, tpBottom]) then
    begin
      if not (ATabViewInfo.PaintingPositionIndex in [3, 4, 5, 6, 12]) then
        Inc(ARect.Right, 2);
    end
    else
      if not (ATabViewInfo.PaintingPositionIndex in [1, 2, 4, 5, 6, 10, 11]) then
        Inc(ARect.Right, 2);
  end;

  ATheme := OpenTheme(totTab, ScaleFactor);
  if ViewInfo.ParentBackground and (ACanvas.Handle = ViewInfo.Canvas.Handle) and
    IsThemeBackgroundPartiallyTransparent(ATheme, TABP_PANE, 0)
  then
    cxDrawThemeParentBackground(ViewInfo.IControl.GetControl, ACanvas, ARect)
  else
    ACanvas.FillRect(ARect, ViewInfo.Color);
...
begin
  DrawButtonFrameAndBackground(ACanvas, ARect, AState);
end;
...
procedure TcxPCStandardPainter.DrawButtonFrameAndBackground(ACanvas: TcxCanvas; const ARect: TRect;
  AState: TcxPCNavigatorButtonState);
var
  R: TRect;
begin
...
const
  TabAndNavigatorPosToGoDialogPos: array[TcxTabPosition, TcxPCNavigatorPosition] of TcxPCNavigatorPosition = (
...
begin
  ...
  AGoDialogPosition := TabAndNavigatorPosToGoDialogPos[ViewInfo.TabPosition, ViewInfo.NavigatorPosition];
...
begin
  ...
  begin
    ...
    ATabsContainerOffset := RotateRect(ATabsContainerOffset, ViewInfo.TabPosition);
    if ViewInfo.TabPosition in [tpTop, tpBottom] then
    begin
      if ViewInfo.NavigatorPosition in [npLeftTop, npLeftBottom] then
      begin
        Inc(Result.ExtendedTabsRect.Left, AButtonsWidth);
        Dec(Result.ExtendedTabsRect.Right, ATabsContainerOffset.Right);
      end
      else
      begin
        Dec(Result.ExtendedTabsRect.Right, AButtonsWidth);
        Inc(Result.ExtendedTabsRect.Left, ATabsContainerOffset.Left);
      end;
    end
    else
    begin
      if ViewInfo.NavigatorPosition in [npLeftTop, npRightTop] then
      begin
        Inc(Result.ExtendedTabsRect.Top, AButtonsWidth);
        Dec(Result.ExtendedTabsRect.Bottom, ATabsContainerOffset.Bottom);
      end
      else
      begin
        Dec(Result.ExtendedTabsRect.Bottom, AButtonsWidth);
        Inc(Result.ExtendedTabsRect.Top, ATabsContainerOffset.Top);
      end;
    end;
  end
  else
    Result.ExtendedTabsRect := cxRectContent(Result.ExtendedTabsRect, RotateRect(ATabsContainerOffset, ViewInfo.TabPosition));
  if ViewInfo.TabPosition in [tpTop, tpBottom] then
    Result.NormalRowWidth := (Result.ExtendedTabsRect.Right - Result.ExtendedTabsRect.Left)
  else
    Result.NormalRowWidth := (Result.ExtendedTabsRect.Bottom - Result.ExtendedTabsRect.Top);
  ...
  Result.MinDistanceBetweenTopOrLeftAndBottomOrRightExtendedTabsRects :=
...
begin
  if TabViewInfo[TabVisibleIndex].IsFocused and ViewInfo.IsFocused and ViewInfo.IControl.GetLookAndFeel.Painter.SupportsNativeFocusRect then
  begin
...
begin
  DrawButtonFrameAndBackground(ACanvas, AButtonInfo.Bounds, AButtonInfo.State);
end;
...
class function TcxPCStandardPainter.HasTabPressedState: Boolean;
const
  HasTabPressedStateMap: array [TcxPCStandardStyle] of boolean = (False, True,
    True);
begin
  Result := HasTabPressedStateMap[GetStandardStyle];
end;
...
begin
  if AButton = nbGoDialog then
    ...
    Result := cxPCArrowConversionA[AButton, ViewInfo.TabPosition in [tpLeft, tpRight]];
end;
...
begin
  ...
  if HasTabPressedState and IsAssignedImages and (InternalGetTextRotationAngle(ViewInfo) = raPlus90) then
  begin
    Inc(Result.Left);
    Dec(Result.Right);
  end;
...
begin
  if HasTabPressedState and IsTabBorderThick(ATabVisibleIndex) then
  begin
...
class function TcxPCExtraFlatPainter.HasTabPressedState: Boolean;
begin
  Result := False;
end;
...
function TcxPCSlantedPainter.InternalCalculateTabNormalHeight: Integer;
  ...
  begin
    Result := cxMarginsHeight(GetScaledDrawImageOffset(-1));
  end;

  function GetDrawTextTotalHOffset: Integer;
  begin
    Result := cxMarginsHeight(GetScaledDrawTextHOffset(-1));
  end;
...
function TcxPCSlantedPainter.DirectionalGetFigureRegion(const R: TRect;
  ...
  ATabPosition: TcxTabPosition; AForContent: Boolean): TcxRegion;
  ...
  begin
    ...
    if (ATabPosition = tpLeft) and (ALinePositions[0] = lpLB) then
    begin
      Inc(APoints[0].X);
      Inc(APoints[0].Y);
    end;
  ...
  begin
    ...
    for I := 0 to High(ALinePositions) do
      ALinePositions[I] := ALinePositionConversionTable[ATabPosition, ALinePositions[I]];
  end;
...
function TcxPCSlantedPainter.GetTabsLineRect(ATabIndexInterval: TcxPCIndexInterval; AFullRect: Boolean): TRect;
  ...
  procedure ExchangeTabs(var ALeftTabViewInfo, ARightTabViewInfo: TcxTabViewInfo);
  var
    AExchangeTabViewInfo: TcxTabViewInfo;
  begin
    AExchangeTabViewInfo := ALeftTabViewInfo;
    ALeftTabViewInfo := ARightTabViewInfo;
    ARightTabViewInfo := AExchangeTabViewInfo;
  end;
