...
type
  ...
  TdxPSTTFCharCacheListItem = class
  strict private
    FCharCode: Word;
    FGlyphIndex: Word;
  public
  ...
  { TdxPSTTFAbstractSection }

  TdxPSTTFAbstractSection = class abstract(TdxPSTTFCustomObject)
  strict private
    FExists: Boolean;
  ...
  protected
    ...
    procedure ReadData(Stream: TdxPSTTFStream); virtual; abstract;
    procedure ReadDataFromStream(AStream: TStream);
    procedure Reset; virtual;
    procedure WriteData(Stream: TdxPSTTFStream); virtual; abstract;
    //
  ...
  TdxPSTTFCustomSection = class(TdxPSTTFAbstractSection)
  strict private
    FVersion: Cardinal;
  protected
  ...
  TdxPSTTFBinarySection = class(TdxPSTTFAbstractSection)
  strict private
    FData: Pointer;
    FDataSize: Integer;
  protected
  ...
  public
    ...
    property UnitsPerEm: Cardinal read FUnitsPerEm;
  end;
  ...
  TdxPSTTFCMapSection = class(TdxPSTTFAbstractSection)
  strict private
  ...
  TdxPSTTFOS2Section = class(TdxPSTTFAbstractSection)
  strict private
    FAvgCharWidth: SmallInt;
    FBreakChar: Word;
    FCapHeight: SmallInt;
    FCodePageRange1: DWORD;
    FCodePageRange2: DWORD;
    FDefaultChar: Word;
    FFamilyClass: Byte;
    ...
    FUnicodeRange: array[0..3] of DWORD;
    FUpperOpticalPointSize: Word;
  ...
  TdxPSTTFHmtxSection = class(TdxPSTTFAbstractSection)
  strict private
  ...
  TdxPSTTFLocaSection = class(TdxPSTTFAbstractSection)
  strict private
    FOffsets: array of Integer;

    function GetGlyfSection: TdxPSTTFGlyfSection;
  ...
  public
    ...
    procedure Clear;
    //
    property GlyfSection: TdxPSTTFGlyfSection read GetGlyfSection;
  ...
  TdxPSTTFGlyfSection = class(TdxPSTTFAbstractSection)
  strict private
  ...
  strict private
    ...
    function GetSection(AIndex: Integer): TdxPSTTFAbstractSection;
    function GetSectionCount: Integer;
  protected
    function CalculateExistsSectionCount: Integer;
    function FindSection(const ATag: TdxPSTTFSectionTag): TdxPSTTFAbstractSection;
    function ReadSectionEntry(Stream: TdxPSTTFStream; ASection: TdxPSTTFAbstractSection; AEntriesCount, AHeaderOffset: Integer): Boolean;
  ...
  public
    constructor Create(AOwner: TdxPSTTFFile); override;
    destructor Destroy; override;
    procedure Register(ASection: TdxPSTTFAbstractSection);
    procedure Reset;
    //
    property ExistsSectionsCount: Integer read FExistsSectionsCount;
    property Section[Index: Integer]: TdxPSTTFAbstractSection read GetSection;
  ...
  strict private
    ...
    FGlyfSection: TdxPSTTFGlyfSection;
  ...
  public
    ...
    property GlyfSection: TdxPSTTFGlyfSection read FGlyfSection;
...
const
  dxThisUnitName = 'dxPSTrueTypeFont';
...
function TdxPSTTFStream.ReadByte: Byte;
begin
...
procedure TdxPSTTFStream.WriteByte(AValue: Byte);
begin
...
begin
  ...
  FGlyfSection := TdxPSTTFGlyfSection.Create(Self);
  ...
  TableDirectory.Register(FGlyfSection);
  TableDirectory.Register(TdxPSTTFPrepSection.Create(Self));
  TableDirectory.Register(TdxPSTTFCvtSection.Create(Self));
  TableDirectory.Register(TdxPSTTFFpgmSection.Create(Self));
...
var
  AMemStream: TMemoryStream;
  ASavedCharset: Integer;
  ASection: TdxPSTTFAbstractSection;
  I: Integer;
begin
  ...
  try
    ...
    AMemStream := TMemoryStream.Create;
    try
      dxPSGetFontData(AMemStream, AFont);
      AMemStream.Position := 0;
      try
        LoadFromStream(AMemStream);
      except
        TableDirectory.Reset;
...
begin
  ...
  GlyfSection.RemoveUnusedGlyphs(ACharList);
...
begin
  Result := 1000 / HeadSection.UnitsPerEm;
end;
...
function TdxPSTTFTableDirectory.FindSection(const ATag: TdxPSTTFSectionTag): TdxPSTTFAbstractSection;
var
  I: Integer;
begin
  Result := nil;
...
function TdxPSTTFTableDirectory.GetSection(AIndex: Integer): TdxPSTTFAbstractSection;
begin
  Result := TdxPSTTFAbstractSection(FSections.Items[AIndex]);
end;
...
function TdxPSTTFTableDirectory.ReadSectionEntry(Stream: TdxPSTTFStream;
  ASection: TdxPSTTFAbstractSection; AEntriesCount, AHeaderOffset: Integer): Boolean;
var
  I: Integer;
  ATableData: TdxPSTTFTableEntryData;
begin
...
procedure TdxPSTTFTableDirectory.SaveToStream(Stream: TdxPSTTFStream);
  ...
  begin
    for I := 0 to SectionCount - 1 do
      if Section[I].Exists then
      begin
        ATableData := Section[I].FTableEntryData;
...
begin
  ...
  begin
    ASection := Section[I];
    if ASection.Exists then
      ASection.SaveToStream(Stream);
  end;
end;

procedure TdxPSTTFTableDirectory.Register(ASection: TdxPSTTFAbstractSection);
begin
  FSections.Add(ASection);
end;

procedure TdxPSTTFTableDirectory.Reset;
var
  I: Integer;
begin
  for I := 0 to SectionCount - 1 do
...
function TdxPSTTFAbstractSection.GetSectionOffset: Integer;
begin
  Result := FTableEntryData.Offset;
end;

procedure TdxPSTTFAbstractSection.LoadFromStream(Stream: TdxPSTTFStream);
begin
...
procedure TdxPSTTFAbstractSection.ReadDataFromStream(AStream: TStream);
var
...
procedure TdxPSTTFAbstractSection.Reset;
begin
  ZeroMemory(@FTableEntryData, SizeOf(FTableEntryData));
  Exists := False;
end;

procedure TdxPSTTFAbstractSection.SaveToStream(Stream: TdxPSTTFStream);
begin
  Stream.WritePadding;
  WriteData(Stream);
end;

class function TdxPSTTFAbstractSection.Tag: TdxPSTTFSectionTag;
begin
  Result := 0;
end;
...
begin
  FreeBuffer;
  FDataSize := FTableEntryData.Length;
...
var
  I: Integer;
begin
  SetLength(FEntries, HHEASection.NumberOfHMetrics);
  for I := 0 to HHEASection.NumberOfHMetrics - 1 do
  begin
...
procedure TdxPSTTFCMapSection.ReadUnicodeTable(Stream: TdxPSTTFStream; AUnicodeTableOffset: Integer);
begin
...
begin
  ...
  for I := 0 to GlyfSection.GlyphInfoCount - 1 do
  begin
    AGlyphInfo := GlyfSection.GlyphInfo[I];
...
function TdxPSTTFLocaSection.GetGlyfSection: TdxPSTTFGlyfSection;
begin
  Result := Owner.GlyfSection;
end;
...
constructor TdxPSTTFGlyfSection.Create(AOwner: TdxPSTTFFile);
begin
  inherited Create(AOwner);
  FGlyphList := TcxObjectList.Create;
end;

destructor TdxPSTTFGlyfSection.Destroy;
begin
  FreeAndNil(FGlyphList);
  inherited Destroy;
end;

function TdxPSTTFGlyfSection.GetGlyphInfoByGlyphIndex(
...
function TdxPSTTFGlyfSection.GetLocaSection: TdxPSTTFLocaSection;
begin
  Result := Owner.LocaSection;
end;

function TdxPSTTFGlyfSection.GetGlyphInfo(Index: Integer): TdxPSTTFGlyphInfo;
begin
  Result := TdxPSTTFGlyphInfo(FGlyphList.Items[Index]);
end;

function TdxPSTTFGlyfSection.GetGlyphInfoCount: Integer;
begin
  Result := FGlyphList.Count;
end;

function TdxPSTTFGlyfSection.GetSize: Integer;
var
  I: Integer;
begin
  Result := 0;
...
class function TdxPSTTFGlyfSection.Tag: TdxPSTTFSectionTag;
begin
  Result := GetSectionTag('glyf');
end;

procedure TdxPSTTFGlyfSection.ReadData(Stream: TdxPSTTFStream);
var
  AGlyphInfo: TdxPSTTFGlyphInfo;
...
procedure TdxPSTTFGlyfSection.RemoveUnusedGlyphs(ACharList: TdxPSTTFCharCacheList);
...
procedure TdxPSTTFGlyfSection.WriteData(Stream: TdxPSTTFStream);
var
  I: Integer;
begin
  for I := 0 to GlyphInfoCount - 1 do
...
function TdxPSTTFOS2Section.GetSize: Integer;
begin
  Result := 23 * SizeOf(Word) + 2 * SizeOf(Byte) + SizeOf(Panose) + SizeOf(FUnicodeRange) + SizeOf(FVendor);
  if FVersion >= 1 then
    Inc(Result, 2 * SizeOf(DWORD));
  if FVersion >= 4 then
    Inc(Result, 5 * SizeOf(Word));
  if FVersion >= 5 then
...
begin
  FVersion := Stream.ReadWord;
  FAvgCharWidth := Stream.ReadWord;
  FWeightClass := Stream.ReadWord;
  FWidthClass := Stream.ReadWord;
  FFontType := Stream.ReadWord;
  FSubscriptXSize := Stream.ReadWord;
  FSubscriptYSize := Stream.ReadWord;
  FSubscriptXOffset := Stream.ReadWord;
  FSubscriptYOffset := Stream.ReadWord;
  FSuperscriptXSize := Stream.ReadWord;
  FSuperscriptYSize := Stream.ReadWord;
  FSuperscriptXOffset := Stream.ReadWord;
  FSuperscriptYOffset := Stream.ReadWord;
  FStrikeoutSize := Stream.ReadWord;
  FStrikeoutPosition := Stream.ReadWord;
  FFamilyClass := Stream.ReadByte;
  FFamilySubClass := Stream.ReadByte;

  ZeroMemory(@FPanose, SizeOf(FPanose));
  Stream.ReadRaw(FPanose, SizeOf(FPanose));

  Stream.ReadRaw(FUnicodeRange, SizeOf(FUnicodeRange));
  Stream.ReadRaw(FVendor[0], SizeOf(FVendor));

  FSelection := Stream.ReadWord;
  FFirstCharIndex := Stream.ReadWord;
  FLastCharIndex := Stream.ReadWord;

  FTypoAscender := Stream.ReadWord;
  FTypoDescender := Stream.ReadWord;
  FTypoLineGap := Stream.ReadWord;
  FWinAscent := Stream.ReadWord;
  FWinDescent := Stream.ReadWord;

  if FVersion >= 1 then
  begin
    FCodePageRange1 := Stream.ReadLong;
    FCodePageRange2 := Stream.ReadLong;
  end;

  if FVersion >= 4 then
  begin
    FHeight := Stream.ReadWord;
    FCapHeight := Stream.ReadWord;
    FDefaultChar := Stream.ReadWord;
    FBreakChar := Stream.ReadWord;
    FMaxContext := Stream.ReadWord;
  end;

  if FVersion >= 5 then
  begin
    FLowerOpticalPointSize := Stream.ReadWord;
    FUpperOpticalPointSize := Stream.ReadWord;
  end;
end;

procedure TdxPSTTFOS2Section.WriteData(Stream: TdxPSTTFStream);
begin
  Stream.WriteWord(FVersion);
  Stream.WriteWord(FAvgCharWidth);
  Stream.WriteWord(FWeightClass);
  Stream.WriteWord(FWidthClass);
  Stream.WriteWord(FFontType);
  Stream.WriteWord(FSubscriptXSize);
  Stream.WriteWord(FSubscriptYSize);
  Stream.WriteWord(FSubscriptXOffset);
  Stream.WriteWord(FSubscriptYOffset);
  Stream.WriteWord(FSuperscriptXSize);
  Stream.WriteWord(FSuperscriptYSize);
  Stream.WriteWord(FSuperscriptXOffset);
  Stream.WriteWord(FSuperscriptYOffset);
  Stream.WriteWord(FStrikeoutSize);
  Stream.WriteWord(FStrikeoutPosition);
  Stream.WriteByte(FFamilyClass);
  Stream.WriteByte(FFamilySubClass);

  Stream.WriteRaw(FPanose, SizeOf(FPanose));
  Stream.WriteRaw(FUnicodeRange, SizeOf(FUnicodeRange));
  Stream.WriteRaw(FVendor[0], SizeOf(FVendor));

  Stream.WriteWord(FSelection);
  Stream.WriteWord(FFirstCharIndex);
  Stream.WriteWord(FLastCharIndex);

  Stream.WriteWord(FTypoAscender);
  Stream.WriteWord(FTypoDescender);
  Stream.WriteWord(FTypoLineGap);
  Stream.WriteWord(FWinAscent);
  Stream.WriteWord(FWinDescent);

  if FVersion >= 1 then
  begin
    Stream.WriteLong(FCodePageRange1);
    Stream.WriteLong(FCodePageRange2);
  end;

  if FVersion >= 4 then
  begin
    Stream.WriteWord(FHeight);
    Stream.WriteWord(FCapHeight);
    Stream.WriteWord(FDefaultChar);
    Stream.WriteWord(FBreakChar);
    Stream.WriteWord(FMaxContext);
  end;

  if FVersion >= 5 then
  begin
    Stream.WriteWord(FLowerOpticalPointSize);
    Stream.WriteWord(FUpperOpticalPointSize);
  end;
end;
