...
type
  ...
  { TdxPScxControlAsGraphicProducer }

  TdxPScxControlAsGraphicProducer = class(TdxPSContainerCustomWinControlProducer)
  protected
    function CreateControlImage(AItem: TdxReportVisualItem): TGraphic; virtual; abstract;
  ...
  { TdxPScxControlAsMetafileProducer }

  TdxPScxControlAsMetafileProducer = class(TdxPScxControlAsGraphicProducer)
  protected
    function CreateControlImage(AItem: TdxReportVisualItem): TGraphic; override;
  end;

  { TdxPScxControlAsBitMapProducer }

  TdxPScxControlAsBitMapProducer = class(TdxPScxControlAsGraphicProducer)
  protected
    function CreateControlImage(AItem: TdxReportVisualItem): TGraphic; override;
  end;
  ...
  { TdxPSdxFormattedLabelProducer }

  TdxPSdxFormattedLabelProducer = class(TdxPScxCustomEditProducer)
  protected
    procedure InitializeItem(AnItem: TdxReportVisualItem); override;
    function ItemClass: TdxReportVisualItemClass; override;
    function ObjectExpandHeight: Boolean; override;
  ...
  public
    function Control: TdxCustomFormattedLabel; reintroduce; overload;
    class function ControlClass: TControlClass; override;
  end;
  ...
  { TdxPScxCustomNavigatorProducer }

  TdxPScxCustomNavigatorProducer = class(TdxPScxControlAsBitMapProducer)
  public
    class function ControlClass: TControlClass; override;
  end;
...
uses
  Types, Variants, SysUtils, dxThemeConsts, dxUxTheme, cxVariants, dxPSUtl, cxClasses, dxDPIAwareUtils,
  dxPScxEditorLnks, dxPScxListBoxLnk, cxGeometry, dxGDIPlusClasses, cxEditUtils, cxNavigator,
  dxCore;

const
  dxThisUnitName = 'dxPScxEditorProducers';
...
type
  ...
  TdxCustomFormattedLabelAccess = class(TdxCustomFormattedLabel);
...
class function TdxPScxControlAsGraphicProducer.ControlClass: TControlClass;
begin
  Result := TcxControl;
end;

procedure TdxPScxControlAsGraphicProducer.InitializeItem(AItem: TdxReportVisualItem);
begin
  inherited;
...
function TdxPScxControlAsGraphicProducer.ItemClass: TdxReportVisualItemClass;
begin
...
function TdxPScxControlAsMetafileProducer.CreateControlImage(
...
function TdxPScxControlAsBitMapProducer.CreateControlImage(
  AItem: TdxReportVisualItem): TGraphic;
begin
  Result := TdxReportCellDpiAwareGraphic(AItem).CreateImage(TcxBitMap);
  Result.Width := AItem.Width;
  Result.Height := AItem.Height;
  cxPaintToBitmap(Control, TcxBitMap(Result));
end;
...
begin
  Result := cxTextEditProperties_GetDisplayText(Properties, Control.EditValue, False);
end;
...
begin
  ...
  begin
    TdxSparklineImage(AnItem).ViewData := Control.Properties.CreateViewData(DefaultEditStyleController.Style, True, dxDefaultScaleFactor);
...
function TdxPSdxFormattedLabelProducer.Control: TdxCustomFormattedLabel;
begin
  Result := inherited Control as TdxCustomFormattedLabel;
end;

class function TdxPSdxFormattedLabelProducer.ControlClass: TControlClass;
begin
  Result := TdxCustomFormattedLabel;
end;

procedure TdxPSdxFormattedLabelProducer.InitializeItem(AnItem: TdxReportVisualItem);
const
...
begin
  inherited;
  if Properties is TdxFormattedLabelProperties then
  begin
    AFormattedLabelProperties := TdxFormattedLabelProperties(Properties);
    ...
    AItem.Text := VarToStr(Control.EditValue);
    AItem.Transparent := Control.Transparent;
    AItem.Indents.Rect := Rect(2, 2, 2, 2);
    AItem.MeasureTrailingSpaces := True;
  end;
end;

function TdxPSdxFormattedLabelProducer.ItemClass: TdxReportVisualItemClass;
begin
  Result := TdxReportCellFormattedLabel;
end;

function TdxPSdxFormattedLabelProducer.ObjectExpandHeight: Boolean;
begin
  Result := TdxCustomFormattedLabelAccess(Control).AutoSize;
end;

function TdxPSdxFormattedLabelProducer.ObjectShrinkHeight: Boolean;
begin
  Result := TdxCustomFormattedLabelAccess(Control).AutoSize;
end;

function TdxPSdxFormattedLabelProducer.ObjectExpandWidth: Boolean;
begin
  Result := TdxCustomFormattedLabelAccess(Control).AutoSize;
end;

function TdxPSdxFormattedLabelProducer.ObjectShrinkWidth: Boolean;
begin
  Result := TdxCustomFormattedLabelAccess(Control).AutoSize;
end;
...
begin
  Result := inherited CanProcessChild(AChildControl) and
    not (AChildControl is TcxCustomRadioGroupButton); 
end;
...
class function TdxPScxCustomNavigatorProducer.ControlClass: TControlClass;
begin
  Result := TcxCustomNavigator;
end;
...
begin
  ...
  begin
    ...
    try
      ...
      try
        ARect := BoundsRect;
        if (FitMode = ifmNormal) and ShowText then
        begin
          TdxCustomBarCodeSymbologyAccess(ASymbology).CalculateSize(Text, Font, ASize.cx, ASize.cy);
          ARect := cxRectSetSize(ARect, ASize);
        end;
        ...
        begin
          ...
          ARect := cxRectInflate(ARect, -ADifferenceSize + 1, ADifferenceSize + 1, -ADifferenceSize + 1, ADifferenceSize + 1);
        end;
...
begin
  ...
  TdxPSdxFormattedLabelProducer.Register;
  TdxPScxCustomNavigatorProducer.Register;
end;

procedure UnregisterAssistants;
begin
  TdxPScxCustomNavigatorProducer.Unregister;
  TdxPSdxFormattedLabelProducer.Unregister;
