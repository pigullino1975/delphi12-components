...
type
  ...
  strict private
    ...
    function InternalCorrectValue(AValue: Integer = 1): Integer;
  protected
...
const
  dxThisUnitName = 'dxSkinscxPCPainter';
...
var
  ACorrectValue: Integer;
begin
  ACorrectValue := InternalCorrectValue;
  case APosition of
    tpTop:
      ARect.Bottom := Indents[siDownGrow] + ACorrectValue;
    tpBottom:
      ARect.Top := -Indents[siDownGrowBottomRight] - ACorrectValue;
    tpLeft:
      ARect.Right := Indents[siDownGrow] + ACorrectValue;
    tpRight:
      ARect.Left := -Indents[siDownGrowBottomRight] - ACorrectValue;
  end;
end;
...
function TcxPCSkinPainter.InternalCorrectValue(AValue: Integer): Integer;
var
  ASkinPainterInfo: TdxSkinInfo;
begin
  Result := AValue;
  if GetSkinPainterData(ASkinPainterInfo) and ASkinPainterInfo.IsAdvanced then
    Result := Round(Result * ScaleFactor.Numerator / (ScaleFactor.Denominator + 0.01)); 
end;
...
function TcxPCSkinPainter.GetTabVerticalCorrection(const AFullRect: TRect; ATabPosition: TcxTabPosition): TRect;
begin
  Result := GetExtendedRect(AFullRect, Rect(0, 0, 0, -InternalCorrectValue), ATabPosition);
end;
...
begin
  ...
  if AFrameContent = nil then
    ...
    AFrameContent.Draw(ACanvas.Handle, GetPageFrameRect, ScaleFactor);
end;
...
begin
  AFrameContent := FrameContent;
  if AFrameContent <> nil then
    AFrameContent.Draw(DC, cxRectOffset(GetPageFrameRect, -ATab.Left, -ATab.Top), ScaleFactor)
  else
    inherited;
end;
...
begin
  ...
  begin
    ACorrectValue := InternalCorrectValue;
    Result := cxRectTransform(FrameContent.ContentOffset.Rect, Rect(ACorrectValue, ACorrectValue, ACorrectValue, ACorrectValue))
  end;
end;
...
begin
  ...
  begin
    ...
    if not ATabViewInfo.Enabled then
      Result := dxSkinColorSelect(True, ASkinInfo.TabTextColorDisabled, ASkinInfo.TabTextColor)
    else
      if ATabViewInfo.IsMainTab then
        Result := dxSkinColorSelect(ATabViewInfo.IsHotTrack, ASkinInfo.TabTextColorActiveHot, ASkinInfo.TabTextColorActive)
      else
        Result := dxSkinColorSelect(ATabViewInfo.IsHotTrack, ASkinInfo.TabTextColorHot, ASkinInfo.TabTextColor);
  end;
  ...
  begin
    ...
    try
      ...
      if FrameContent.Image.GradientBeginColor = clNone then
        FrameContentCache.DrawEx(ACanvas.Handle, FrameContent, ADrawRect, ViewInfo.ScaleFactor)
      else
        FrameContent.Draw(ACanvas.Handle, ADrawRect, ViewInfo.ScaleFactor);
    finally
...
begin
  if Header <> nil then
    Header.Draw(DC, ABounds, ScaleFactor, 0, GetTabState(ViewInfo.TabsViewInfo[ATabVisibleIndex]))
  else
...
begin
  ...
  begin
    ...
    FrameContent.Draw(ABitmap.Canvas.Handle, R, ScaleFactor);
  end;
end;
...
begin
  Result := 0;
  if GetSkinPainterData(ASkinPainterInfo) then
    Result := InternalCorrectValue(ASkinPainterInfo.PageControlIndents[Integer(AType)])
end;
