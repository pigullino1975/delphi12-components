...
uses
  Types, SysUtils, Classes, dxCore, dxGenerics, Variants, Generics.Defaults, Generics.Collections;
...
type
  ...
  public
    ...
    procedure Skip(ACount: Cardinal);
  end;
  ...
  public
    ...
    procedure WriteRaw(AStream: TMemoryStream); overload;
    procedure WriteRaw(const AData; ASize: TdxNativeInt); overload;
    procedure WriteRect(const AValue: TRect);
    procedure WriteShortInt(AValue: ShortInt);
    procedure WriteSingle(AValue: Single);
    procedure WriteString(const AValue: string); //note: this method work difference with unicode/ansi type of string
  ...
  TdxVariantSet = class(TdxHashSet<Variant>)
  public
...
function ReadDoubleFunc(AStream: TStream): Double;
procedure ReadDoubleProc(AStream: TStream; var Value: Double);
procedure WriteDoubleProc(AStream: TStream; const AValue: Double);
...
{$IFNDEF NONDB}
  function ReadBCD: Variant;
  var
    ABCD: TBCD;
  begin
    AStream.ReadBuffer(ABCD, SizeOf(TBCD));
...
{$IFNDEF NONDB}
  procedure WriteBCD(const Value: TBCD);
  begin
    WriteValue(TValueType(vaBCD));
    AStream.WriteBuffer(Value, SizeOf(TBCD));
  end;
{$ENDIF}
...
begin
  ...
  {$IFNDEF NONDB}
    ...
    else if VType = VarFMTBcd then
      WriteBCD(VarToBcd(AValue))
    else
  {$ENDIF}
    try
      WriteAnsiString(dxVariantToAnsiString(AValue));
...
procedure TcxReader.BeginChunk(out APosition: Int64);
begin
  APosition := ReadCardinal;
  Inc(APosition, Stream.Position);
end;

procedure TcxReader.EndChunk(const APosition: Int64);
begin
...
procedure TcxReader.Skip(ACount: Cardinal);
begin
  Stream.Seek(ACount, soCurrent);
end;
...
procedure TcxWriter.WriteRaw(AStream: TMemoryStream);
begin
  Stream.WriteBuffer(AStream.Memory^, AStream.Size);
end;

procedure TcxWriter.WriteRaw(const AData; ASize: TdxNativeInt);
begin
...
procedure TcxWriter.WriteString(const AValue: string);
begin
  WriteInteger(Length(AValue));
  if Length(AValue) > 0 then
    Stream.WriteBuffer(AValue[1], Length(AValue) * SizeOf(Char));
end;
...
var
  AComparer: IComparer<Variant>;
begin
  AComparer := TdxVariantListComparer.Create; 
  inherited Create(AComparer);
end;

{ TdxVariantSet }

constructor TdxVariantSet.Create(ACapacity: Integer = 0);
var
  AComparer: IEqualityComparer<Variant>;
begin
  AComparer := TdxVariantComparer.Create; 
