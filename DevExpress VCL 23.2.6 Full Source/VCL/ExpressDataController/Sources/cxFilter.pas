...
uses
  Types, SysUtils, Classes, Variants, Windows, cxClasses, ImgList, dxCore, dxCoreClasses, cxLike, dxCustomFunction;

type
  TcxFilterCriteriaDisplayStyle = (fcdsDefault, fcdsText, fcdsTokens);

const
  cxFilterCriteriaVersion1 = 1; //for internal use only. //#Old.
  cxFilterCriteriaVersion2 = 2; //for internal use only. //#Store ItemLink ID.
  cxFilterCriteriaVersion3 = 3; //for internal use only. //#Store ItemLink Name.
  cxFilterCriteriaVersion4 = 4; //for internal use only. //#Store ItemLink ID and Expression.
  cxFilterCriteriaVersion5 = 5; //for internal use only. //#Store ItemLink Name and Expression.
  cxFilterCriteriaVersion6 = 6; //for internal use only. //#Store CustomFunction.
  cxFilterCriteriaLastVersion = cxFilterCriteriaVersion6; //for internal use only

  dxFilterCriteriaDisplayStyle: TcxFilterCriteriaDisplayStyle = fcdsText;
...
type
  ...
  protected
    function GetFilterExpressionText(const AValue: Variant): string; virtual;
    function GetFilterExpressionValue(const AValue: Variant): string; virtual;
  ...
  public
    ...
    function IsUnary: Boolean; virtual;
  ...
  protected
    ...
    function GetLikeOperator: cxLike.TcxFilterLikeOperator;
  ...
  { TcxFilterCustomFunctionOperator }

  TcxFilterCustomFunctionOperator = class(TcxFilterOperator)
  private
    FCustomFunctionOperator: TdxCustomFunctionOperator;
    FFunctionName: string;
    FMaxOperandCount: Integer;
  ...
  protected
    function AddExpressionItem(AItemLink: TObject; const ACustomFunctionName: string;
      const AExpression: string; const ADisplayValue: string): TcxFilterCriteriaItem; overload;
    function AddItem(AItemLink: TObject; const ACustomFunctionName: string;
      const AValue: Variant; const ADisplayValue: string): TcxFilterCriteriaItem; overload;
  ...
  public
    ...
    function AddExpressionItem(AItemLink: TObject; AOperatorKind: TcxFilterOperatorKind;
      const AExpression: string; const ADisplayValue: string): TcxFilterCriteriaItem; overload;
  ...
  private
    ...
    procedure SetCustomFunctionName(const Value: string);
  protected
    procedure CheckDisplayValue;
    function CompareByDisplayValue: Boolean;
    function DoFilterData(AData: TObject): Boolean; override;
    procedure ExpressionChanged; virtual;
    function GetComparedValue(AData: TObject; out AHasError: Boolean): Variant; virtual;
    function GetDataValue(AData: TObject): Variant; virtual; abstract;
    function GetDisplayValue: string; virtual;
    function GetExpressionValue(AData: TObject; out AHasError: Boolean): Variant; virtual; abstract;
    function GetFieldCaption: string; virtual;
    function GetFieldName: string; virtual;
    function GetFilterExpressionValue(AIsCaption: Boolean): string; virtual; 
    function GetFilterOperatorClass: TcxFilterOperatorClass; overload; virtual;
    function GetFilterOperatorClass(AKind: TcxFilterOperatorKind): TcxFilterOperatorClass; overload;
    function GetItemLink: TObject; virtual;
    procedure SetItemLink(Value: TObject); virtual;
    function GetIsItemList: Boolean; override;
    function IsOperatorNull(AOperatorKind: TcxFilterOperatorKind): Boolean; overload;
    procedure Prepare;
    procedure PrepareValue;
    procedure RecreateOperator; virtual;
    procedure ReadData(AStream: TStream); override;
    function ReadExpression(AStream: TStream; AIsUnicode: Boolean): string; virtual;
    function SupportsMultiThreading: Boolean; virtual;
    procedure WriteData(AStream: TStream); override;
    procedure WriteExpression(AStream: TStream; const AExpression: string); virtual;
  public
    constructor Create(AOwner: TcxFilterCriteriaItemList; AItemLink: TObject; AOperatorKind: TcxFilterOperatorKind;
      const AValue: Variant; const ADisplayValue: string); virtual;
    destructor Destroy; override;

    function IsEmpty: Boolean; override;
    function IsOperatorNull: Boolean; overload; //for internal use only
    function ValueIsNull(const AValue: Variant): Boolean; virtual;

    property CustomFunctionName: string read FCustomFunctionName write SetCustomFunctionName;
  ...
  TcxGetFilterImagesEvent = procedure(Sender: TObject; AValueList: TcxFilterValueList;
    var AImages: TCustomImageList) of object;
  TcxGetFilterValuesEvent = procedure(Sender: TObject; AValueList: TcxFilterValueList) of object;
  TcxQueryCustomFunctions = procedure(Sender: TObject; const APropertyName: string; AFunctionNames: TStrings) of object;
  ...
  public
    ...
    function ComparePreparedValue(const APreparedValue: Variant; const APreparedDisplayText: string): Integer; // for internal use
  ...
  strict private
    ...
    FMRUSeparatorIndex: Integer;
    FSortByDisplayText: Boolean;
    FStartValueIndex: Integer;
  ...
  protected
    ...
    property ItemsList: TdxFastObjectList read FItems; // read-only
  public
    ...
    function IndexOf(AItem: TcxFilterValueItem): Integer; inline;
    ...
    property StartValueIndex: Integer read FStartValueIndex;
  end;
  ...
  protected
    ...
    function ConvertDateToStr(const AValue: Variant; AIsSQLTimeStamp: Boolean): string; virtual;
  ...
  public
    ...
    function AddExpressionItem(AParent: TcxFilterCriteriaItemList; AItemLink: TObject; AOperatorKind: TcxFilterOperatorKind;
      const AExpression: string; const ADisplayValue: string): TcxFilterCriteriaItem; overload; virtual;
    function AddExpressionItem(AParent: TcxFilterCriteriaItemList; AItemLink: TObject; const ACustomFunctionName: string;
      const AExpression: string; const ADisplayValue: string): TcxFilterCriteriaItem; overload; virtual;
    function AddItem(AParent: TcxFilterCriteriaItemList; AItemLink: TObject; AOperatorKind: TcxFilterOperatorKind;
      const AValue: Variant; const ADisplayValue: string): TcxFilterCriteriaItem; overload; virtual;
    function AddItem(AParent: TcxFilterCriteriaItemList; AItemLink: TObject; const ACustomFunctionName: string;
      const AValue: Variant; const ADisplayValue: string): TcxFilterCriteriaItem; overload; virtual;
...
function IsOperatorSupportedForExpression(AOperator: TcxFilterOperatorKind): Boolean; //for internal use only
...
uses
  ...
  cxVariants, cxFilterConsts, cxDateUtils, StrUtils;

const
  dxThisUnitName = 'cxFilter';
...
function IsOperatorSupportedForExpression(AOperator: TcxFilterOperatorKind): Boolean;
begin
  Result := AOperator in [foEqual..foNotLike, foContains..foEndsWith];
end;

function VersionContainsItemLinkNames(AVersion: Integer): Boolean;
begin
  Result := (AVersion = cxFilterCriteriaVersion3) or (AVersion >= cxFilterCriteriaVersion5);
end;

function VersionContainsExpressions(AVersion: Integer): Boolean;
begin
  Result := AVersion >= cxFilterCriteriaVersion4;
end;

function VersionContainsCustomFunction(AVersion: Integer): Boolean;
begin
  Result := AVersion >= cxFilterCriteriaVersion6;
end;
{ TcxFilterOperator }
...
function TcxFilterOperator.IsUnary: Boolean;
begin
  Result := False;
end;

function TcxFilterOperator.GetFilterExpressionText(const AValue: Variant): string;
begin
  Result := GetFilterExpressionValue(AValue);
end;

function TcxFilterOperator.GetFilterExpressionValue(const AValue: Variant): string;
var
  AVarType: Integer;
begin
  ...
  begin
    ...
    begin
      ...
      if (AVarType = varDate) or (AVarType = VarSQLTimeStamp) then
        Result := '''' + CriteriaItem.Criteria.ConvertDateToStr(AValue, AVarType = VarSQLTimeStamp) + ''''
      else
...
function TcxFilterEqualOperator.GetFilterExpressionText(const AValue: Variant): string;
...
begin
  ...
  APos := 1;
  Result := '(' + ExtractFieldName(AFieldNames, APos) + ' = ' + GetFilterExpressionValue(AValue[0]) + ')';
  for I := VarArrayLowBound(AValue, 1) + 1 to VarArrayHighBound(AValue, 1) do
    Result := Result + ' AND ' +
      '(' + ExtractFieldName(AFieldNames, APos) + ' = ' + GetFilterExpressionValue(AValue[I]) + ')';
end;
...
function TcxFilterNotEqualOperator.GetFilterExpressionText(const AValue: Variant): string;
begin
  Result := 'NOT (' + inherited GetFilterExpressionText(AValue) + ')';
end;
...
function TcxFilterCustomLikeOperator.IsDescription: Boolean;
begin
  Result := True;
end;
...
function TcxFilterCustomLikeOperator.GetFilterExpressionText(const AValue: Variant): string;
begin
  Result := CriteriaItem.GetFieldName + ' ' + GetExpressionOperatorText + ' ' + GetFilterExpressionValue(PreparePatternString(AValue));
end;
...
function TcxFilterLikeOperator.GetLikeOperator: cxLike.TcxFilterLikeOperator;
var
  S: string;
begin
  ...
  begin
    ...
    Result := LikeOperatorByPattern(S, CriteriaItem.Criteria.PercentWildcard);
  end
  else
    Result := floLike;
end;
...
function TcxFilterNotLikeOperator.GetFilterExpressionText(const AValue: Variant): string;
begin
  Result := ' NOT (' + inherited GetFilterExpressionText(AValue) + ')';
end;
...
function TcxFilterNotContainsOperator.GetFilterExpressionText(const AValue: Variant): string;
begin
  Result := inherited GetFilterExpressionText(AValue);
...
function TcxFilterBetweenOperator.GetFilterExpressionText(const AValue: Variant): string;
var
  AFieldName: string;
begin
  AFieldName := CriteriaItem.GetFieldName;
  Result := '(' + AFieldName + ' >= ' + GetFilterExpressionValue(AValue[0]) + ') AND (' +
    AFieldName + ' <= ' + GetFilterExpressionValue(AValue[1]) + ')';
end;
...
begin
  ...
  if Result then
    DisplayValue := GetFilterExpressionValue(AValue[0]) + ' AND ' + GetFilterExpressionValue(AValue[1]);
end;
...
function TcxFilterNotBetweenOperator.GetFilterExpressionText(const AValue: Variant): string;
begin
  Result := 'NOT (' + inherited GetFilterExpressionText(AValue) + ')';
end;
...
function TcxFilterInListOperator.GetFilterExpressionText(const AValue: Variant): string;
var
  AFieldName: string;
  I: Integer;
begin
  AFieldName := CriteriaItem.GetFieldName;
  Result := '(' + AFieldName + ' = ' + GetFilterExpressionValue(AValue[0]) + ')';
  for I := VarArrayLowBound(AValue, 1) + 1 to VarArrayHighBound(AValue, 1) do
    Result := Result + ' OR ' + '(' + AFieldName + ' = ' + GetFilterExpressionValue(AValue[I]) + ')';
end;
...
begin
  ...
  if Result then
  begin
    DisplayValue := '(' + GetFilterExpressionValue(AValue[0]);
    for I := VarArrayLowBound(AValue, 1) + 1 to VarArrayHighBound(AValue, 1) do
      DisplayValue := DisplayValue + ', ' + GetFilterExpressionValue(AValue[I]);
...
function TcxFilterNotInListOperator.GetFilterExpressionText(const AValue: Variant): string;
begin
  Result := 'NOT (' + inherited GetFilterExpressionText(AValue) + ')';
end;
...
function TcxFilterNullOperator.IsUnary: Boolean;
begin
  Result := True;
end;
...
function TcxFilterDateOperator.IsUnary: Boolean;
begin
  Result := True;
end;

function TcxFilterDateOperator.GetFilterExpressionText(const AValue: Variant): string;
var
  AFieldName: string;
begin
  AFieldName := CriteriaItem.GetFieldName;
  if Date1 <> cxFilterNullDate then
    Result := '(' + AFieldName + ' >= ' + GetFilterExpressionValue(Date1) + ')'
  else
    Result := '';
  ...
  begin
    if Result <> '' then
      Result := Result + ' AND ';
    Result := Result + '(' + AFieldName + ' < ' + GetFilterExpressionValue(Date2) + ')';
  end;
end;
...
function TcxFilterCriteriaItemList.AddExpressionItem(AItemLink: TObject; AOperatorKind: TcxFilterOperatorKind;
  const AExpression: string; const ADisplayValue: string): TcxFilterCriteriaItem;
begin
  if Criteria.IsExpressionsSupported then
  begin
    Result := AddItem(AItemLink, AOperatorKind, Null, ADisplayValue);
...
function TcxFilterCriteriaItemList.AddExpressionItem(AItemLink: TObject; const ACustomFunctionName, AExpression,
  ADisplayValue: string): TcxFilterCriteriaItem;
begin
  if Criteria.IsExpressionsSupported then
  begin
    Result := AddItem(AItemLink, ACustomFunctionName, Null, ADisplayValue);
    Result.Expression := AExpression;
  end
  else
    Result := nil;
...
function TcxFilterCriteriaItemList.AddItem(AItemLink: TObject; const ACustomFunctionName: string; const AValue: Variant;
...
constructor TcxFilterCriteriaItem.Create(AOwner: TcxFilterCriteriaItemList; AItemLink: TObject;
...
function TcxFilterCriteriaItem.IsOperatorNull: Boolean;
begin
  Result := IsOperatorNull(OperatorKind);
end;
...
begin
  if IsOperatorNull and (FDisplayValue = '') then
...
procedure TcxFilterCriteriaItem.ExpressionChanged;
begin
  RecreateOperator;
  Changed;
end;

function TcxFilterCriteriaItem.GetComparedValue(AData: TObject; out AHasError: Boolean): Variant;
begin
  if Expression <> '' then
  begin
    Result := GetExpressionValue(AData, AHasError);
    if not AHasError then
      Result := Criteria.PrepareValue(Result);
  end
  else
  begin
...
function TcxFilterCriteriaItem.GetFilterExpressionValue(AIsCaption: Boolean): string;
begin
  if AIsCaption then
    Result := GetDisplayValue
  else
    if Expression = '' then
      Result := Operator.GetFilterExpressionValue(Value)
    else
      Result := '';
end;
...
begin
  Result := GetFilterOperatorClass(OperatorKind);
end;

function TcxFilterCriteriaItem.GetFilterOperatorClass(AKind: TcxFilterOperatorKind): TcxFilterOperatorClass;
const
  ...
  ANullOperatorClasses: array[Boolean] of TcxFilterOperatorClass = (TcxFilterNullOperator, TcxFilterNotNullOperator);
  AExpressionOperatorClasses: array[Boolean] of TcxFilterOperatorClass = (TcxFilterEqualOperator, TcxFilterNotEqualOperator);
begin
  if IsOperatorNull(AKind) then
    Result := ANullOperatorClasses[AKind in [foNotEqual, foNotLike, foNotContains]]
  else
    if (Expression <> '') and not IsOperatorSupportedForExpression(AKind) then
      Result := AExpressionOperatorClasses[OperatorKind in [foNotBetween, foNotInList]]
    else
      Result := AOperatorClasses[AKind];
end;
...
function TcxFilterCriteriaItem.IsOperatorNull(AOperatorKind: TcxFilterOperatorKind): Boolean;
begin
  Result := (AOperatorKind in [foEqual, foNotEqual, foLike, foNotLike, foContains..foEndsWith]) and
    ValueIsNull(Value) and (Expression = '');
end;
...
begin
  ...
  try
    ...
    if VersionContainsItemLinkNames(Criteria.LoadedVersion) then
      if AdxStream.IsUnicode then
    ...
    if VersionContainsExpressions(Criteria.LoadedVersion) then
      Expression := ReadExpression(AStream, AdxStream.IsUnicode)
    else
      Expression := '';
    if VersionContainsCustomFunction(Criteria.LoadedVersion) then
      CustomFunctionName := ReadWideStringFunc(AStream)
    else
      CustomFunctionName := '';
  finally
    AdxStream.Free;
...
function TcxFilterCriteriaItem.ReadExpression(AStream: TStream; AIsUnicode: Boolean): string;
begin
  if AIsUnicode then
    Result := ReadWideStringFunc(AStream)
  else
    Result := dxAnsiStringToString(ReadAnsiStringFunc(AStream));
end;
...
begin
  ...
  if VersionContainsItemLinkNames(Criteria.SavedVersion) then
    if Criteria.IsUnicode then
  ...
  if VersionContainsExpressions(Criteria.SavedVersion) then
    WriteExpression(AStream, Expression);
  if VersionContainsCustomFunction(Criteria.SavedVersion) then
    WriteWideStringProc(AStream, CustomFunctionName);
end;

procedure TcxFilterCriteriaItem.WriteExpression(AStream: TStream; const AExpression: string);
begin
  if Criteria.IsUnicode then
    WriteWideStringProc(AStream, AExpression)
  else
    WriteAnsiStringProc(AStream, dxStringToAnsiString(AExpression));
end;
...
var
  AHasError: Boolean;
  AValue, ADataValue, AComparedValue: Variant;
begin
  try
    ADataValue := GetDataValue(AData);
    AValue := Criteria.PrepareValue(ADataValue);
    // for Null special compare
    if (fcoIgnoreNull in Criteria.Options) and not IsOperatorNull and ValueIsNull(AValue) then
      Result := False
    else
    begin
      AComparedValue := GetComparedValue(AData, AHasError);
      Result := not AHasError and &Operator.CompareValues(AValue, AComparedValue);
    end;
  except
    on EVariantError do
...
procedure TcxFilterCriteriaItem.SetCustomFunctionName(const Value: string);
begin
  if FCustomFunctionName <> Value then
  begin
    FCustomFunctionName := Value;
    RecreateOperator;
    (FOperator as TcxFilterCustomFunctionOperator).FunctionName := Value;
    Changed;
  end;
end;
...
procedure TcxFilterCriteriaItem.SetExpression(const AValue: string);
begin
  if Criteria.IsExpressionsSupported and not AnsiSameText(AValue, Expression) then
  begin
    FExpression := AValue;
    ExpressionChanged;
  end;
...
procedure TcxFilterValueList.Add(AKind: TcxFilterValueItemKind;
  const AValue: Variant; const ADisplayText: string; ANoSorting: Boolean);
begin
...
begin
  if (AIndex <= FMRUSeparatorIndex) then
    Dec(FMRUSeparatorIndex);
  if (AIndex <= FStartValueIndex) and (FStartValueIndex > 0) then
    Dec(FStartValueIndex);
  FItems.Delete(AIndex);
end;

function TcxFilterValueList.Find(const AValue: Variant; const ADisplayText: string; var AIndex: Integer): Boolean;
var
...
begin
  ...
  if FMRUSeparatorIndex <> -1 then
  begin
    for I := 0 to FMRUSeparatorIndex - 1 do
...
function TcxFilterValueList.FindItemByKind(AKind: TcxFilterValueItemKind; const AValue: Variant): Integer;
var
...
begin
  ...
  begin
    for I := 0 to Count - 1 do
    begin
      if TcxFilterValueItem(FItems.List[I]).Kind = AKind then
        Exit(I);
    end;
  end
  else
    for I := 0 to Count - 1 do
    begin
      AItem := TcxFilterValueItem(FItems.List[I]);
      if (AItem.Kind = AKind) and VarEquals(AItem.Value, AValue) then
        Exit(I);
    end;
...
begin
  if ACriteriaItem = nil then
    ...
    if ACriteriaItem.ValueIsNull(ACriteriaItem.Value) and (ACriteriaItem.OperatorKind in [foEqual, foNotEqual]) then
    begin
      ...
      if not ((ACriteriaItem.OperatorKind = foEqual) and Find(ACriteriaItem.Value, ACriteriaItem.DisplayValue, Result)) then
        Result := FindItemByKind(fviCustom);
end;

function TcxFilterValueList.IndexOf(AItem: TcxFilterValueItem): Integer;
begin
  Result := ItemsList.IndexOf(AItem);
end;
...
procedure TcxFilterValueList.Add(AKind: TcxFilterValueItemKind;
  const AValue: Variant; const ADisplayText: string; ANoSorting: Boolean; AUniqueOnly: Boolean);

  procedure InsertItem(AIndex: Integer; AKind: TcxFilterValueItemKind; const AValue: Variant; const ADisplayText: string);
  begin
    FItems.Insert(AIndex, CreateItem(AKind, AValue, ADisplayText));
    if AIndex <= FMRUSeparatorIndex then
      Inc(FMRUSeparatorIndex);
    if AIndex <= FStartValueIndex then
    begin
      if AKind = fviValue then
        FStartValueIndex := AIndex
      else
        Inc(FStartValueIndex);
    end;
  end;
...
begin
  ...
  begin
    AIndex := FMRUSeparatorIndex;
    if AIndex = -1 then // first MRU item
    begin
      InsertItem(0, fviMRUSeparator, Null, '');
      FMRUSeparatorIndex := 0;
      AIndex := 0;
    end;
  end
  else
    if AKind <> fviValue then
      AIndex := FStartValueIndex
    else
      if ANoSorting then
      begin
        AIndex := Count;
  ...
  if AIndex <> -1 then
    InsertItem(AIndex, AKind, AValue, ADisplayText);
end;

function TcxFilterValueList.CompareItem(AIndex: Integer;
  const APreparedValue: Variant; const APreparedDisplayText: string): Integer;
begin
...
begin
  ...
  FVersion := cxFilterCriteriaVersion2;
  FIsUnicode := True;
  InitializeCriticalSectionAndSpinCount(FCriticalSection, 512);
end;
...
begin
  ...
  begin
    BeginUpdate;
    try
      CompareByDisplayValues := TcxFilterCriteria(Source).CompareByDisplayValues;
  ...
  try
    ...
    try
      ...
      if AIgnoreItemNames then
        ASource.Version := Min(ASource.Version, cxFilterCriteriaVersion4);
...
function TcxFilterCriteria.AddExpressionItem(AParent: TcxFilterCriteriaItemList; AItemLink: TObject;
  AOperatorKind: TcxFilterOperatorKind; const AExpression: string; const ADisplayValue: string): TcxFilterCriteriaItem;
begin
  if AParent = nil then
    AParent := Root;
  Result := AParent.AddExpressionItem(AItemLink, AOperatorKind, AExpression, ADisplayValue);
end;

function TcxFilterCriteria.AddExpressionItem(AParent: TcxFilterCriteriaItemList; AItemLink: TObject;
  const ACustomFunctionName, AExpression, ADisplayValue: string): TcxFilterCriteriaItem;
begin
  if AParent = nil then
    AParent := Root;
  Result := AParent.AddExpressionItem(AItemLink, ACustomFunctionName, AExpression, ADisplayValue);
end;

function TcxFilterCriteria.AddItem(AParent: TcxFilterCriteriaItemList; AItemLink: TObject;
  AOperatorKind: TcxFilterOperatorKind; const AValue: Variant; const ADisplayValue: string): TcxFilterCriteriaItem;
begin
  if AParent = nil then
    AParent := Root;
...
function TcxFilterCriteria.AddItem(AParent: TcxFilterCriteriaItemList; AItemLink: TObject;
  const ACustomFunctionName: string; const AValue: Variant; const ADisplayValue: string): TcxFilterCriteriaItem;
begin
  if AParent = nil then
    AParent := Root;
  Result := AParent.AddItem(AItemLink, ACustomFunctionName, AValue, ADisplayValue);
end;
...
begin
  if AIgnoreItemNames then
    AMaxVersion := cxFilterCriteriaVersion4
  else
    AMaxVersion := cxFilterCriteriaLastVersion;
...
function TcxFilterCriteria.FindItemByItemLink(AItemLink: TObject): TcxFilterCriteriaItem;
  ...
  begin
    ...
    begin
      if Result <> nil then
        Break;
      AItem := TcxCustomFilterCriteriaItem(ABranch.FItems.List[I]);
      if AItem.IsItemList then
        FindInBranch(TcxFilterCriteriaItemList(AItem))
      else
        if TcxFilterCriteriaItem(AItem).FItemLink = AItemLink then // TODO: !!!ItemLink
        begin
          Result := TcxFilterCriteriaItem(AItem);
          Break;
        end;
    end;
  end;
...
begin
  case AVersionID of
    -1: Result := cxFilterCriteriaVersion2;
    -2: Result := cxFilterCriteriaVersion3;
    -3: Result := cxFilterCriteriaVersion4;
    -4: Result := cxFilterCriteriaVersion5;
    -5: Result := cxFilterCriteriaVersion6;
  else
    Result := cxFilterCriteriaVersion1;
  end;
end;
...
begin
  case AVersion of
    cxFilterCriteriaVersion2: Result := -1;
    cxFilterCriteriaVersion3: Result := -2;
    cxFilterCriteriaVersion4: Result := -3;
    cxFilterCriteriaVersion5: Result := -4;
    cxFilterCriteriaVersion6: Result := -5;
  else
    Result := 0;  // undefined
  end;
end;
...
function TcxFilterCriteria.ConvertDateToStr(const AValue: Variant; AIsSQLTimeStamp: Boolean): string;
begin
  if DateTimeFormat <> '' then
    Result := FormatDateTime(DateTimeFormat, AValue)
  else
    if AIsSQLTimeStamp then
      Result := VarToStr(AValue)
    else
...
begin
  if not AIsCaption and AItem.Operator.IsExpression then
    Result := AItem.Operator.GetFilterExpressionText(AItem.Value)
  else
  begin
...
function TcxFilterCriteria.GetItemExpressionValue(AItem: TcxFilterCriteriaItem;
  AIsCaption: Boolean): string;
begin
  Result := AItem.GetFilterExpressionValue(AIsCaption);
end;
...
function TcxFilterCriteria.IsExpressionsSupported: Boolean;
begin
  Result := True;
end;
...
begin
  Result := VersionContainsItemLinkNames(Version);
end;
...
begin
  if StoreItemLinkNames <> Value then
    if Value then
      Version := cxFilterCriteriaLastVersion
    else
      Version := cxFilterCriteriaVersion4;
end;
...
function TcxFilterCustomFunctionOperator.CompareValues(const AValue1, AValue2: Variant): Boolean;
var
...
begin
  ...
  begin
    ACount := VarArrayHighBound(AValue2, 1) - VarArrayLowBound(AValue2, 1) + 1;
    SetLength(AValues, ACount + 1);
    AValues[0] := AValue1;
    for I := 0 to ACount - 1 do
      AValues[I + 1] := AValue2[I];
    Result := FCustomFunctionOperator.Evaluate(AValues);
  end
  else
    Result := FCustomFunctionOperator.Evaluate([AValue1, AValue2]);
end;

function TcxFilterCustomFunctionOperator.DisplayText: string;
begin
  Result := TdxCustomFunctionOperatorFactory.GetCustomFunctionOperatorName(FCustomFunctionOperator);
end;

function TcxFilterCustomFunctionOperator.FilterText: string;
begin
  Result := FFunctionName;
end;

function TcxFilterCustomFunctionOperator.IsUnary: Boolean;
begin
  Result := FMaxOperandCount < 2;
end;

procedure TcxFilterCustomFunctionOperator.PrepareDisplayValue(var DisplayValue: string);
begin
  if FMaxOperandCount < 2 then
    DisplayValue := ''
  else
    if FMaxOperandCount > 2 then
      DisplayValue := VarListArrayToStr(StrToVarListArray(DisplayValue))
    else
      inherited PrepareDisplayValue(DisplayValue);
end;

procedure TcxFilterCustomFunctionOperator.SetFunctionName(const Value: string);
begin
  FFunctionName := Value;
  FCustomFunctionOperator := TdxCustomFunctionOperatorFactory.GetCustomFunctionOperator(FFunctionName);
  if FCustomFunctionOperator is TdxCustomFunctionOperatorBrowsable then
    FMaxOperandCount := TdxCustomFunctionOperatorBrowsable(FCustomFunctionOperator).MaxOperandCount
  else
    FMaxOperandCount := 1;
end;
