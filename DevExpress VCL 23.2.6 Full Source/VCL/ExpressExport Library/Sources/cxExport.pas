...
{$ENDIF}
  ...
  dxCore, dxGdiPlusClasses, cxGraphics, cxClasses, cxGeometry, cxExportStrs, dxSpreadSheet, cxEdit, dxGenerics,
...
type
  ...
  ['{442A08A8-CDDA-4FD6-8E15-9D8BD34554F6}']
    ...
    function TryGetSharedImageHandle(AImageList: TObject; AImageIndex: Integer; const AImageSize: TSize; var AHandle: TObject): Boolean;
  ...
  public
    class function Provider(AExportType: Integer; AStream: TStream): TcxCustomExportProvider;
  ...
  strict private
    function GetItem(AIndex: TdxListIndex): TdxNativeInt; inline;
    procedure SetItem(AIndex: TdxListIndex; AValue: TdxNativeInt); inline;
  public
  ...
  TdxExportAxis = class
  private
    FDictionary: TdxIntegersDictionary;
    FIsDirty: Boolean;
    FValues: TList;
  ...
  protected
    ...
    property Dictionary: TdxIntegersDictionary read FDictionary;
...
function cxGetDisplayFormat(AEditProperties: TcxCustomEditProperties; AField: TObject = nil): string;
function cxGetDisplayFormatType(AEditProperties: TcxCustomEditProperties; AField: TObject = nil): TcxValueDisplayFormatType;
function cxGetFieldDisplayFormat(AField: TObject): string;
...
uses
  ...
  dxSpreadSheetCore, cxRichEdit, dxThreading, dxFormattedLabel;

const
  dxThisUnitName = 'cxExport';
...
function cxGetDisplayFormatType(AEditProperties: TcxCustomEditProperties; AField: TObject = nil): TcxValueDisplayFormatType;
begin
  if (AEditProperties is TcxTimeEditProperties) or (AEditProperties is TcxDateEditProperties)
  {$IFNDEF NONDB} or (AField is TDateTimeField) or (AField is TSQLTimeStampField){$ENDIF}
  then
    Result := vdftDateTime
  else
...
begin
  Result := IsRTFSupported and ((AProperties is TcxRichEditProperties) and not TcxRichEditProperties(AProperties).MemoMode or
    (AProperties is TdxFormattedLabelProperties));
end;
...
class function TcxExport.Provider(AExportType: Integer; AStream: TStream): TcxCustomExportProvider;
begin
  Result := GetExportClassByType(AExportType).Create(AStream);
end;
...
constructor TcxCustomExportProvider.Create(AStream: TStream);
begin
  FStream := AStream;
end;
...
function TcxExportIntList.GetItem(AIndex: TdxListIndex): TdxNativeInt;
begin
  Result := TdxNativeInt(List[AIndex]);
end;

procedure TcxExportIntList.SetItem(AIndex: TdxListIndex; AValue: TdxNativeInt);
begin
...
begin
  FDictionary := TdxIntegersDictionary.Create;
