...
unit dxMapControlShapeFileLoader; // for internal use
...
type
  ...
  public
    ...
    procedure LoadFromStream(AShpStream, ADbfStream: TStream; ACpgStream: TStream = nil);
  end;

implementation

const
  dxThisUnitName = 'dxMapControlShapeFileLoader';
...
procedure TdxMapControlShapeFileLoader.LoadFromStream(AShpStream,
  ADbfStream: TStream; ACpgStream: TStream = nil);
const
  CP_UTF8 = 65001;
var
  ...
  ACodePage: TStringStream;
  AEncoding: TEncoding;
begin
  AEncoding := nil;
  if ACpgStream <> nil then
  begin
    ACodePage := TStringStream.Create;
    ACodePage.LoadFromStream(ACpgStream);
    if SameText(ACodePage.DataString, Trim('utf-8')) then
  ...
  try
    ...
    ADbfFileInfo.LoadFromStream(ADbfStream, AEncoding);
