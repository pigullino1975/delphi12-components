...
type
  TdxMapControlOpenStreetMapTileSource = class(TdxMapControlMultiScaleTileSource) // for internal use
  private
    FUrlTemplate: string;
...
const
  dxThisUnitName = 'dxMapControlOpenStreetMapImageryDataProvider';
...
begin
  ...
  Result := StringReplace(Result, '[z]', IntToStr(AZoomLevel), []);
  Result := StringReplace(Result, '[x]', IntToStr(ATilePositionX), []);
  Result := StringReplace(Result, '[y]', IntToStr(ATilePositionY), []);
end;
