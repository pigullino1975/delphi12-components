...
type
  ...
  TdxMapItemStylesHelper = class // for internal use
  private
    FIsCacheValid: Boolean;
    FMaxBorderWidth: Integer;
    FMaxSizeFont: TFont;
  ...
  public
    ...
    function GetMaxBorderWidth: Integer;
  ...
  TdxCustomMapItemViewInfo = class(TdxMapControlElementViewInfo) // for internal use
  protected
  ...
  TdxMapItemViewInfo = class(TdxCustomMapItemViewInfo) // for internal use
  private
    FDrawBounds: TRect;
  ...
  TdxMapItemTextViewInfo = class(TdxCustomMapItemViewInfo) // for internal use
  private
 ...
 //   FAllowHotTrack: Boolean;
  ...
  public
    ...
    function GetStyle(AState: TdxMapControlElementState): TdxMapItemStyle; // for internal use
    procedure InvalidateStylesInfo; // for internal use
    procedure InvalidateTitleInfo; // for internal use
    function IsPointer: Boolean; virtual; // for internal use
    procedure SetParentComponent(Value: TComponent); override; // for internal use

    property Attributes: TdxMapItemAttributes read FAttributes;
    property Selected: Boolean read GetSelected write SetSelected;
    property ViewInfo: TdxMapItemViewInfo read FViewInfo; // for internal use
  published
  ...
  TdxMapDotViewInfo = class(TdxMapItemViewInfo) // for internal use
  private
  ...
  public
    constructor Create(AOwner: TComponent); override;
    function IsPointer: Boolean; override; // for internal use
  published
    property Location;
  ...
  TdxMapRectangleViewInfo = class(TdxMapItemViewInfo) // for internal use
  private
  ...
  TdxMapEllipseViewInfo = class(TdxMapRectangleViewInfo) // for internal use
  protected
    procedure DoPaint; override;
    function PtInElement(const APoint: TPoint): Boolean; override;
  end;
  ...
  public
    ...
    property OnChanged: TNotifyEvent read FOnChanged write FOnChanged; // for internal use
  end;

  TdxMapCustomMultiPointItemViewInfo = class(TdxMapItemViewInfo) // for internal use
  protected
    FMapBounds: TdxRectDouble;
  ...
  TdxMapPolylineViewInfo = class(TdxMapCustomMultiPointItemViewInfo) // for internal use
  private
  ...
  TdxMapCustomPolygonViewInfo = class(TdxMapCustomMultiPointItemViewInfo) // for internal use
  private
    FMapArea: Double;
    FCheckIntersections: Boolean;
  ...
  TdxMapPolygonViewInfo = class(TdxMapCustomPolygonViewInfo) // for internal use
  protected
    function CreatePath: TdxGpPath;
    procedure DoPaint; override;
  ...
  TdxMapPathSegmentPolygonViewInfo = class(TdxMapCustomPolygonViewInfo) // for internal use
  private
  ...
  TdxMapPathSegmentPolylineViewInfo = class(TdxMapPolylineViewInfo) // for internal use
  private
    FSegment: TdxMapPathSegment;
  protected
    function GetHitTestIndex: Integer; override;
  ...
  public
    ...
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
    property ViewInfo: TdxMapCustomMultiPointItemViewInfo read FViewInfo; // for internal use
  published
    property GeoPoints: TdxMapControlGeoPointCollection read FGeoPoints write SetGeoPoints;
    property Visible: Boolean read FVisible write SetVisible default True;
  end;
  ...
  TdxMapPathCustomViewInfo<T: TdxMapCustomMultiPointItemViewInfo> = class(TdxMapItemViewInfo) // for internal use
  private
  ...
  TdxMapPathPolygonViewInfo = class(TdxMapPathCustomViewInfo<TdxMapPathSegmentPolygonViewInfo>) // for internal use
  private
  ...
  TdxMapPathPolylineViewInfo = class(TdxMapPathCustomViewInfo<TdxMapPathSegmentPolylineViewInfo>) // for internal use
  protected
    procedure DoPaint; override;
    function PtInElement(const APoint: TPoint): Boolean; override;
  public
  ...
  TdxMapPointerViewInfo = class(TdxMapItemViewInfo) // for internal use
  private
  ...
  TdxMapPushpinViewInfo = class(TdxMapPointerViewInfo) // for internal use
  protected
  ...
  TdxMapCustomElementViewInfo = class(TdxMapPointerViewInfo) // for internal use
  protected
    procedure DoCalculateCachedBounds; override;
    procedure DoPaint; override;
  end;
  ...
  TdxMapItemsZoomHelper = class // for internal use
  public
...
var
  I: Integer;
  A160B4669734277151858051506120D3D60773E07626E2E7B: TdxMapItemStyle;
begin
  A160B4669734277151858051506120D3D60773E07626E2E7B := Style;
  if FCheckIntersections then
    for I := 0 to High(FVisiblePolylines) do
      dxGPPaintCanvas.Polyline(FVisiblePolylines[I], A160B4669734277151858051506120D3D60773E07626E2E7B.BorderColor, ScaleFactor.Apply(A160B4669734277151858051506120D3D60773E07626E2E7B.BorderWidth))
  else
    if Length(FScreenPoints) > 0 then
      dxGPPaintCanvas.Polyline(FScreenPoints, A160B4669734277151858051506120D3D60773E07626E2E7B.BorderColor, ScaleFactor.Apply(A160B4669734277151858051506120D3D60773E07626E2E7B.BorderWidth));
end;
...
begin
  ...
  begin
    if IsImageAssigned(MapPointer.Image) then
      FRelativeImageRect := cxRect(dxGetImageSize(MapPointer.Image, ScaleFactor))
    else
...
begin
  ...
  begin
    ...
    try
      if APath <> nil then
      begin
        AGpCanvas := TdxGPGraphics.Create(cxMeasureCanvas.Handle);
        try
...
begin
  AStyle := Style;
  dxGPPaintCanvas.Ellipse(DrawBounds, AStyle.BorderColor, AStyle.Color, ScaleFactor.Apply(AStyle.BorderWidth));
end;
...
function TdxMapItemStylesHelper.GetMaxBorderWidth: Integer;
begin
  CheckCache;
  Result := FMaxBorderWidth;
end;
...
procedure TdxMapItemStylesHelper.CheckCache;
var
  I: TdxMapControlElementState;
  AMaxFontSize, AFontSize: Integer;
begin
  if not FIsCacheValid then
  begin
    FMaxSizeFont := nil;
    AMaxFontSize := 0;
    FMaxBorderWidth := 0;
    for I := Low(TdxMapControlElementState) to High(TdxMapControlElementState) do
      ...
      begin
        AFontSize := cxTextSize(FStyles[I].Font, 'Wg').cx;
        if AFontSize > AMaxFontSize then
        begin
          AMaxFontSize := AFontSize;
          FMaxSizeFont := FStyles[I].Font;
        end;
        if FStyles[I].BorderWidth > FMaxBorderWidth then
          FMaxBorderWidth := FStyles[I].BorderWidth;
      end;
    FIsCacheValid := True;
  end;
end;
...
var
  ABorderWidth, AExternalBorderWidth: Integer;
begin
  ...
  begin
    ...
    FDrawBounds := Bounds;
    ABorderWidth := ScaleFactor.Apply(GetMaxBorderWidth);
    AExternalBorderWidth := Ceil(ABorderWidth/2);
    Bounds := cxRectInflate(Bounds, AExternalBorderWidth);
...
function TdxMapItemViewInfo.GetMaxBorderWidth: Integer;
begin
  if not FIsStylesValid then
    UpdateStyles;
  Result := FStyles.GetMaxBorderWidth;
end;
...
begin
  AStyle := Style;
  ABorderWidth := ScaleFactor.Apply(AStyle.BorderWidth);
  case MapDot.ShapeKind  of
    mcskCircle:
      dxGPPaintCanvas.Ellipse(DrawBounds, AStyle.BorderColor, AStyle.Color, ABorderWidth)
  else // mcskRectangle
     dxGPPaintCanvas.Rectangle(DrawBounds, AStyle.BorderColor, AStyle.Color, ABorderWidth);
  end;
end;
