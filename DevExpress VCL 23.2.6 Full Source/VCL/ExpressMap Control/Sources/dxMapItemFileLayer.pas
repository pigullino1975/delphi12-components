...
type
  ...
  public
    ...
    procedure LoadShapeFileFromStream(AShpStream, ADbfStream: TStream); overload;
    procedure LoadShapeFileFromStream(AShpStream, ADbfStream, ACpgStream: TStream); overload;
  published
...
const
  dxThisUnitName = 'dxMapItemFileLayer';
...
var
  ...
  ADbfStream, AShapeStream, ACpgStream: TMemoryStream;
  AFileExt: string;
  AShapeFileName, ADbfFileName, ACpgFileName: string;
begin
  Active := False;
  if AFileName <> '' then
    FFileName := AFileName;
  ...
  begin
    ...
    begin
      ...
      ACpgFileName := ChangeFileExt(FFileName, '.cpg');
      ...
      begin
        ...
        if FileExists(ACpgFileName) then
          ACpgStream := TMemoryStream.Create
        else
          ACpgStream := nil;
        try
          AShapeStream.LoadFromFile(AShapeFileName);
          if ACpgStream <> nil then
            ACpgStream.LoadFromFile(ACpgFileName);
          ADbfStream.LoadFromFile(ADbfFileName);
          LoadShapeFileFromStream(AShapeStream, ADbfStream, ACpgStream);
        finally
          if ACpgStream <> nil then
            ACpgStream.Free;
          ADbfStream.Free;
...
begin
  LoadShapeFileFromStream(AShpStream, ADbfStream, nil);
end;

procedure TdxMapItemFileLayer.LoadShapeFileFromStream(AShpStream, ADbfStream, ACpgStream: TStream);
begin
  FileType := miftShape;
  try
    with TdxMapControlShapeFileLoader.Create(Self) do
    try
      LoadFromStream(AShpStream, ADbfStream, ACpgStream);
    finally
      Free;
    end;
    FActive := True;
