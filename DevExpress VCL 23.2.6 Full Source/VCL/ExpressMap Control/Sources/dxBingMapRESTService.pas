...
uses
  ...
  dxMapControlHttpRequest, dxBingMapRESTServiceStrs, dxMapControlTypes, dxRESTService;

type
  TdxBingMapRESTServiceParams = class // for internal use
  private
    FKey: string;
  ...
  TdxBingMapRESTService = class(TdxRESTService) // for internal use
  private
  ...
  TdxBingMapDataXmlParser = class // for internal use
  public
...
const
  dxThisUnitName = 'dxBingMapRESTService';
...
function TdxBingMapRESTService.GetBaseUrl: string;
var
  AUriSchemeName: string;
begin
  if dxMapControlGetActualConnectionProtocol(ConnectionProtocol) = cpHTTPS then
    AUriSchemeName := 'https'
  else
    AUriSchemeName := 'http';
  Result := AUriSchemeName + '://' + GetAddress;
end;
