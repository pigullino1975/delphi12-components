...
type
  ...
  private
    ...
    FLockCount: Integer;
    FLockSelectionChangedEventCount: Integer;
    FSelectionChangedEventOccurs: Boolean;
  ...
  protected
    ...
    function IsSelectionChangedEventLocked: Boolean;
  ...
  public
    ...
    property HitTest: TdxMapControlHitTest read GetHitTest; // for internal use
...
begin
  ...
  FColor := dxacDefault;
  FCoordinateFont := dxCreateFontForDefaultDPI(16);
  ...
  FScaleFont := dxCreateFontForDefaultDPI(12);
...
begin
  if not IsDestroying then
    if not IsSelectionChangedEventLocked then
    begin
      FSelectionChangedEventOccurs := False;
      if Assigned(FOnSelectionChanged) then
        FOnSelectionChanged(Self);
    end
    else
      FSelectionChangedEventOccurs := True;
end;
...
procedure TdxCustomMapControl.BeginSelection;
begin
  Inc(FLockSelectionChangedEventCount);
end;

procedure TdxCustomMapControl.EndSelection;
begin
  Dec(FLockSelectionChangedEventCount);
  if not IsSelectionChangedEventLocked and FSelectionChangedEventOccurs then
    DoSelectionChanged;
end;

function TdxCustomMapControl.IsSelectionChangedEventLocked: Boolean;
begin
  Result := FLockSelectionChangedEventCount > 0;
end;
