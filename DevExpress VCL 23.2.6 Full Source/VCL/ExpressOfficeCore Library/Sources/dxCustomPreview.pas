...
procedure TdxCustomPreviewLayoutCalculator.CalculateZoomFactorForPagesPreviewZoomMode(
  ...
  begin
    ...
    Result := Max(Trunc(ZoomFactor * cxRectWidth(GetPagesArea) / Result), FPreview.MinZoomFactor);
  end;

begin
  ZoomFactor := Min(
    CalcZoomFactorX(AFirstPageIndex, ALastPageIndex, FPreview.PageXCount),
    CalcZoomFactorY(GetPagesArea, AFirstPageIndex, FPreview.PageYCount));
end;
