...
uses
  Types, Windows, Messages, WinSpool, Classes, Contnrs, dxCore, dxMeasurementUnits;
...
type
  ...
  TdxPageOrientationSubset = (posAll, posPortrait, posLandscape);
...
function dxDecodePageIndexes(const Source: string; out AOutput: TIntegerDynArray): Boolean;
function dxEncodePageIndexes(const ASource: TIntegerDynArray): string;
...
procedure dxSplitString(const ASource, ASeparator: string; AStrings: TStrings);
var
  P, L: Integer;
  Buffer: string;
begin
...
function dxDecodePageIndexes(const Source: string; out AOutput: TIntegerDynArray): Boolean;
  ...
  function ProcessRange(const S: string; out AArray: TIntegerDynArray): Boolean;
  var
...
var
  ...
  ASubRange: TIntegerDynArray;
begin
  SetLength(AOutput, 0);
  try
    ABuffer := StringReplace(Source, ' ', '', [rfReplaceAll, rfIgnoreCase]);
    if Length(ABuffer) > 0 then
    begin
      AStrings := TStringList.Create;
      try
        dxSplitString(ABuffer, cPageSeparator, AStrings);
        ...
        begin
          ...
          begin
            ...
            if ProcessRange(S, ASubRange) then
            begin
              L := Length(AOutput);
              SetLength(AOutput, L + Length(ASubRange));
              for J := L to L + Length(ASubRange) - 1 do
                AOutput[J] := ASubRange[J - L];
            end
            else
              Abort
        end;
...
class procedure TdxPrintingDefaults.SetPrinterName(const Value: string);
  ...
  begin
    SendMessageTimeout(HWND_BROADCAST, WM_SETTINGCHANGE, 0,
      ALParam, SMTO_NORMAL, 1000, @ANeeded);
  end;
