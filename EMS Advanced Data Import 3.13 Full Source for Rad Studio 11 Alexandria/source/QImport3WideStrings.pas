...
type
  ...
  TStringsDefined2 = set of (
    {$IFNDEF VCL7}sdNameValueSeparator,{$ENDIF}
    {$IFNDEF VCL9}sdLineBreak,{$ENDIF}
    sdStrictDelimiter);
...
begin
  ...
  begin
    ...
    try
      ...
      FDefined2 := TWideStrings(Source).FDefined2;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        repeat
          P := CharNextW(P);
        until not (not FStrictDelimiter and (P^ in [WideChar(#1)..WideChar(' ')]));
      end;
    end;
  finally
    EndUpdate;
...
begin
  if not (sdStrictDelimiter in FDefined2) then
...
begin
  if (FStrictDelimiter <> Value) or not (sdStrictDelimiter in FDefined2) then
  begin
    Include(FDefined2, sdStrictDelimiter);
...
begin
  if not (sdNameValueSeparator in {$IFDEF VCL7}FDefined{$ELSE}FDefined2{$ENDIF}) then
    NameValueSeparator := '=';
...
begin
  if (FNameValueSeparator <> Value) or not (sdNameValueSeparator in {$IFDEF VCL7}FDefined{$ELSE}FDefined2{$ENDIF}) then
  begin
    Include({$IFDEF VCL7}FDefined{$ELSE}FDefined2{$ENDIF}, sdNameValueSeparator);
