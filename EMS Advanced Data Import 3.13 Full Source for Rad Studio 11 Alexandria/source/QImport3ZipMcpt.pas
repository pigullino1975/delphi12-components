...
type
  ...
  private
    ...
    SZipfile, SUnZipfile : qiString;
    FUzWithoutPath,FUzOverwrite, BGetSize :Boolean;
    buf : pointer;
    FUnzAllComprSize : Integer;
   procedure OpenZipfile;
   procedure OpenUnZipfile;
   procedure SetUnZipfile(Value: qiString);
   procedure SetZipfile(Value: qiString);
   function  GetUnzAllComprSize: integer;
   function  do_extract_currentfile(DestDir: qiString) : integer;
  public
   ...
   function UnzipAllTo(DestDir: qiString): Boolean;
   property Zipfile : qiString Read SZipfile write SetZipfile;
   property UnZipfile : qiString Read SUnZipfile write SetUnZipfile;
...
FZipFile:=zipOpen(PqiChar(SZipfile),0);
end;
...
FZipFile:=zipOpen(PqiChar(SZipfile),1);
end;
procedure TMiniZip.OpenUnZipfile;
begin
if (FUnZFile<>nil) {or not fileexists(string(SUnZipfile))} then exit;
FUnZFile:=unzOpen(PqiChar(SUnZipfile));
end;

procedure TMiniZip.SetUnZipfile(Value: qiString);
begin
...
procedure TMiniZip.SetZipfile(Value: qiString);
begin
...
OpenUnZipfile;
if unzLocateFile(FUnZFile,PAnsiChar(FileNameInZip),2) <> UNZ_OK then exit;
Result:=do_extract_currentfile(qiString(DestDir))=UNZ_OK;
end;

function TMiniZip.do_extract_currentfile(DestDir: qiString) : int_t;
var
...
begin
 ...
 write_filename:=AnsiString(suerar(DestDir+qiString(write_filename),['/','\']));
...
function TMiniZip.UnzipAllTo(DestDir: qiString) : Boolean;
