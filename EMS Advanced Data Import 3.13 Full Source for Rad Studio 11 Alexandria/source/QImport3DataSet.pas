...
type
  ...
  protected
    ...
    function IsNeedFileNameCheck: Boolean; override;
...
function TQImport3DataSet.IsNeedFileNameCheck: Boolean;
begin
  Result := False;
end;
...
procedure TQImport3DataSet.CheckProperties;
var
  i: integer;
begin
  ...
  begin
    ...
    if not Assigned(Source.FindField(Map.Values[Map.Names[i]])) then
      raise EQImportError.CreateFmt(QImportLoadStr(QIE_SourceFieldNotFound), [Map.Values[Map.Names[i]]]);
  end;
end;
