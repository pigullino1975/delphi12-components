...
{ $Id: QImport3ZDEFLATE.pas 48826 2017-02-17 13:22:26Z gmv $ }
...
function deflateInit_(strm : z_streamp;
                      ...
                      stream_size : int_t) : int_t;


function deflateInit (var strm : z_stream; level : int_t) : int_t;
...
{EXPORT}
function deflate (var strm : z_stream; flush : int_t) : int_t;
...
function deflateEnd (var strm : z_stream) : int_t;
...
{EXPORT}
function deflateReset (var strm : z_stream) : int_t;
...
{EXPORT}
function deflateParams (var strm : z_stream; level : int_t; strategy : int_t) : int_t;
...
type
  compress_func = function(var s : deflate_state; flush : int_t) : block_state;

{local}
procedure fill_window(var s : deflate_state); forward;
{local}
function deflate_stored(var s : deflate_state; flush : int_t) : block_state; far; forward;
{local}
function deflate_fast(var s : deflate_state; flush : int_t) : block_state; far; forward;
{local}
function deflate_slow(var s : deflate_state; flush : int_t) : block_state; far; forward;
{local}
...
function deflate (var strm : z_stream; flush : int_t) : int_t;
var
  old_flush : int_t; { value of flush param for previous deflate call }
  s : deflate_state_ptr;
var
  header : uInt;
  level_flags : uInt;
    ...
    begin
      ...
      begin  { FULL_FLUSH or SYNC_FLUSH }
        ...
        begin
          ...
          s^.head^[s^.hash_size-1] := ZNIL;
          zmemzero(pBytef(s^.head), unsigned_t(s^.hash_size-1)*sizeof(s^.head^[0]));
        end;
      end;
...
{ ========================================================================= }
function deflateCopy (dest, source : z_streamp) : int_t;
{$ifndef MAXSEG_64K}
var
...
{local}
function read_buf(strm : z_streamp; buf : pBytef; size : unsigned_t) : int_t;
var
  len : unsigned_t;
begin
  len := strm^.avail_in;
...
label
  nextstep;
var
  chain_length : unsigned_t;    { max hash chain length }
  {register} scan : pBytef;   { current string }
  {register} match : pBytef;  { matched string }
  {register} len : int_t;       { length of current match }
  best_len : int_t;             { best match length so far }
  nice_match : int_t;           { stop if match long enough }
  limit : IPos;
...
{$else} { UNALIGNED_OK }
  ...
  {$IFDEF NoRangeCheck} {$R+} {$UNDEF NoRangeCheck} {$ENDIF}
        ...
        len := MAX_MATCH - int_t(ptr2int(strend) - ptr2int(scan));
        scan := strend;
        Dec(scan, MAX_MATCH);
...
var
  {register} n, m : unsigned_t;
  {register} p : pPosf;
  more : unsigned_t;    { Amount of free space at the end of the window. }
  wsize : uInt;
begin
   wsize := s.w_size;
   repeat
     more := unsigned_t(s.window_size -ulg(s.lookahead) -ulg(s.strstart));

     { Deal with !@#$% 64K limit: }
     if (more = 0) and (s.strstart = 0) and (s.lookahead = 0) then
       more := wsize
     else
     if (more = unsigned_t(-1)) then begin
       ...
       if (s.strstart >= wsize+ {MAX_DIST}(wsize-MIN_LOOKAHEAD)) then begin
         zmemcpy( pBytef(s.window), pBytef(@(s.window^[wsize])),
                 unsigned_t(wsize));
         if wsize> s.match_start then  //# T.F 03.02 added
          s.match_start:=0 else
...
begin
  if (s.block_start >= Long(0)) then
    _tr_flush_block(s, pcharf(@s.window^[unsigned_t(s.block_start)]),
...
{local}
function deflate_stored(var s : deflate_state; flush : int_t) : block_state;
...
{local}
function deflate_fast(var s : deflate_state; flush : int_t) : block_state;
var
...
{local}
function deflate_slow(var s : deflate_state; flush : int_t) : block_state;
var
