...
type
  ...
  public
    ...
    procedure GetFieldNames(var FieldNames, FieldTypesString: TStringArray; var FieldTypes: TFieldTypes; var FieldSizes: TFieldSizes);
  ...
  published
    ...
    property SourceType: TQImportAccessSourceType read FSourceType write FSourceType default isTable;
...
function BuildConnectionString(const Version: TQImportAccessVersion; const UserName, Password: qiString): qiString;
begin
  if Version = avAccess2007 then
    Result := ConnectionString2007
  else
  ...
  begin
    if Version = avAccess2007 then
      Result := Result + Format(PasswordString2007, [Password])
    else
...
begin
  ...
  try
    ...
    Connection.ConnectionString := Format(BuildConnectionString(Version, '', ''), [AccessFile]);
    try
...
function TADO_QImport3Access.CreateDataSet: TDataSet;
var
  query: TADOQuery;
  connStr: string;
begin
  ...
  query.ConnectionString := Format(connStr, [FileName]);
  ...
  if SourceType = isSQL then
    ...
    query.SQL.Text := Format(SelectFromTable, [TableName]);
  Result := query;
end;
...
procedure TADO_QImport3Access.CheckProperties;
begin
  inherited CheckProperties;
  if (SourceType = isTable) and (Length(TableName) = 0) then
    raise EQImportError.Create(QImportLoadStr(QIE_NoTableName));
end;
...
procedure TADO_QImport3Access.GetFieldNames(var FieldNames, FieldTypesString: TStringArray; var FieldTypes: TFieldTypes; var FieldSizes: TFieldSizes);

  function FieldTypeToStr(ADataType: TFieldType; const AFieldSize: Integer): string;
  begin
    case ADataType of
      ftBlob,
      {$IFDEF VCL10}
...
var
  ...
  fd: TFieldDef;
begin
  if (FSourceType = isSQL) and (FSQL.Text = '') then
    Exit;
