...
type
  ...
  public
    ...
    function FindBetweenExcept(Start, Finish: integer; ExceptIndex: integer; var Index: integer): boolean;
    function FindLeftAndRight(Position: integer; var Left, Right: integer): boolean;
    function FindByPosition(Position: integer): boolean;
    property Items[Index: integer]: TViewArrow read GetItem write SetItem; default;
  end;
  ...
  public
    ...
    property OnDeleteArrow: TDeleteArrowEvent read FOnDeleteArrow write FOnDeleteArrow;
    property OnMoveArrow: TMoveArrowEvent read FOnMoveArrow write FOnMoveArrow;
    property OnIntersectArrows: TIntersectArrowsEvent read FOnIntersectArrows write FOnIntersectArrows;
    property Import: TQImport3ASCII read FImport write FImport;
    property Charset: TQICharsetType read FCharset write FCharset;
    property OnDragDrop;
    property OnDragOver;
    property OnResize;
    property Lines: TqiStrings read FLines;
  end;
...
function TViewArrows.FindBetweenExcept(Start, Finish: integer; ExceptIndex: integer; var Index: integer): boolean;
var
  i: integer;
begin
  Result := False;
...
function TViewArrows.FindLeftAndRight(Position: integer; var Left, Right: integer): boolean;
var
  i: integer;
begin
  Result := False;
  ...
  for i := 0 to Count - 1 do
  begin
    if (Items[i].Position < Position) and ((Left = -1) or (Items[i].Position > Items[Left].Position)) then
      Left := i
    else
      if (Items[i].Position > Position) and ((Right = -1) or (Items[i].Position < Items[Right].Position)) then
        Right := i;
  end;
...
begin
  ...
  if FStep > 0 then
    ASize := (Width * 2 - 1) div FStep
  else
    ASize := 0;

  for i := 1 to ASize do
  begin
    if (i = FOffset) or ((i + FOffset - 1) mod 5 = 0) then
      HDivisor := 1
    else
      HDivisor := 2;
    case FAlign of
      raBottom : DrawVertTrack(i * FStep, 2, (Height - 1) div HDivisor);
      raTop    : DrawVertTrack(i * FStep, Height - 3, Height - (Height - 1) div HDivisor);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      if FRightArrow.Position < FViewer.ClientWidth then
        R.Right := FRightArrow.Position
      else
        R.Right := FViewer.ClientWidth;
    end
    else
      R.Right := 0;
    ...
    begin
      if FLeftArrow.Position < FViewer.ClientWidth then
        R.Left := FLeftArrow.Position
      else
        R.Left := FViewer.ClientWidth;
    end
    else
      R.Left := 0;
    ...
    begin
      if (R.Right > FVisibleRect.Right) and (R.Left = FVisibleRect.Left) then
      begin
        InvertRect(FViewer.Canvas.Handle, Rect(R.Left, R.Top, R.Right - FVisibleRect.Right, R.Bottom));
        FInverted := True;
      end
      else
        if (R.Left < FVisibleRect.Left) and (R.Right = FVisibleRect.Right) then
        begin
    ...
    begin
      if (R.Right > FVisibleRect.Right) and (R.Left = FVisibleRect.Left) then
      begin
        InvertRect(FViewer.Canvas.Handle, Rect(FVisibleRect.Right, R.Top, R.Right, R.Bottom));
        FInverted := True;
      end
      else
        if (R.Right < FVisibleRect.Right) and (R.Left = FVisibleRect.Left) then
        begin
          InvertRect(FViewer.Canvas.Handle, Rect(R.Right, R.Top, FVisibleRect.Right, R.Bottom));
          FInverted := True;
        end
        else
          if (R.Left < FVisibleRect.Left) and (R.Right = FVisibleRect.Right) then
          begin
            InvertRect(FViewer.Canvas.Handle, Rect(R.Left, R.Top, FVisibleRect.Left, R.Bottom));
            FInverted := True;
          end
          else
            if (R.Left > FVisibleRect.Left) and (R.Right = FVisibleRect.Right) then
            begin
              InvertRect(FViewer.Canvas.Handle, Rect(FVisibleRect.Left, R.Top, R.Left, R.Bottom));
              FInverted := True;
            end;
    end;
...
begin
  ...
  fileStream := TEncodedReadStream.Create(AFileName, FCharset);
  try
    FLines.Clear;
    FMaxLineLength := 0;
    i := 0;
    while not fileStream.GetEof do
    begin
      if (FLineCount > 0) and (i >= LineCount) then Break;
      str := fileStream.Readln;
      ReplaceTabs(str);
      FLines.Add(str);
      l := Length(str);
...
begin
  ...
  if FArrows.FindBetweenExcept(L, L, -1, Al) and FArrows.FindBetweenExcept(R, R, -1, Ar) then
  begin
...
begin
  if FSelection.Exists and (FCharWidth > 0) then
  begin
...
begin
  ...
  begin
    ...
    begin
{$IFDEF QI_UNICODE}
      Canvas.TextOutW(FRealLeft + FCharWidth, t, FLines[i] + ' ');
{$ELSE}
...
{$ENDIF}
  ...
  begin
    ...
    if FCharWidth = 0 then
      ...
      ScrollInfo.nMax := FMaxLineLength - 1;
  ...
  begin
    ...
    if FCharHeight = 0 then
      ...
      ScrollInfo.nMax := GetFullHeight div FCharHeight - 1;
...
{$ENDIF}
    ...
    DrawRulers;
    DrawLines;
    if not Assigned(FActiveArrow) then
      DrawArrows;
  end;
end;

procedure TQImport3TXTViewer.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
...
begin
  ...
  begin
    ...
    begin
      if FSelection.Exists and ((FArrows[Index] = FSelection.LeftArrow) or (FArrows[Index] = FSelection.RightArrow)) then
        FSelection.SetSelection(nil, nil);
      DrawArrow(Index);
    ...
    begin
      ...
      if ((L - FRealLeft) div FCharWidth) < 1 then
        Exit;
      Arrow := FArrows.Add;
      Arrow.Position := GetNewArrowPosition(X);
      DrawArrow(Arrow.Index);
      if FSelection.Exists and ((Arrow.Position > FSelection.LeftArrow.Position) and (Arrow.Position < FSelection.RightArrow.Position)) then
        FSelection.SetSelection(nil, nil);
    end;
  end
  else
...
begin
  ...
  begin
    ...
    if (ssLeft in Shift) and Assigned(FActiveArrow) then
    begin
      ...
      if Assigned(FOnMoveArrow) then
        FOnMoveArrow(Self, (OP - FRealLeft) div FCharWidth - 1, (NP - FRealLeft) div FCharWidth - 1);
      if not FArrows.FindBetweenExcept(NP, NP, FActiveArrow.Index, Index) then
      begin
        DrawArrow(FActiveArrow.Index);
        if (FActiveArrow = FSelection.LeftArrow) or (FActiveArrow = FSelection.RightArrow) then
        begin
          FSelection.Update;
        end;
      end
...
procedure TQImport3TXTViewer.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  L, Index: integer;
begin
  inherited;
  ...
  begin
    if X - FRealLeft >= FCharWidth then
    begin
      L := GetNewArrowPosition(X);
...
begin
  Result := FCharHeight * FLines.Count;
end;
...
begin
  ...
  if FCharWidth = 0 then
    ...
    OldScroll.nMax := FMaxLineLength - 1;
  NewScroll := OldScroll;
  case Msg.ScrollCode of
    ...
    SB_PAGELEFT:
      ...
      begin
        if Msg.ScrollCode = SB_LINELEFT then
          Step := 1
        else
          Step := 10;
    ...
    SB_PAGERIGHT:
      ...
      begin
        if Msg.ScrollCode = SB_LINERIGHT then
          Step := 1
        else
          Step := 10;
    ...
    SB_THUMBTRACK:
      ...
      begin
        FRealLeft := FRealLeft - (NewScroll.nTrackPos - NewScroll.nPos) * FCharWidth;
        ScrollBy((NewScroll.nPos - NewScroll.nTrackPos) * FCharWidth, 0);
        HorzScroll(NewScroll.nPos - NewScroll.nTrackPos);
        for i := 0 to FArrows.Count - 1 do
          FArrows[i].Position := FArrows[i].Position - (NewScroll.nTrackPos - NewScroll.nPos) * FCharWidth;
...
begin
  ...
  case Msg.ScrollCode of
    ...
    SB_PAGEUP:
      if NewScroll.nPos > NewScroll.nMin then
      begin
        if Msg.ScrollCode = SB_LINEUP then
          Step := 1
        else
          Step := 10;
        if NewScroll.nPos <= NewScroll.nMin + Step then
    ...
    SB_PAGEDOWN:
      if NewScroll.nPos < NewScroll.nMax then
      begin
        if Msg.ScrollCode = SB_LINEDOWN then
          Step := 1
        else
          Step := 10;
        if NewScroll.nPos >= NewScroll.nMax - Step then
    ...
    SB_THUMBTRACK:
      if NewScroll.nPos <> NewScroll.nTrackPos then
      begin
        FRealTop := FRealTop - (NewScroll.nTrackPos - NewScroll.nPos) * FCharHeight;
...
begin
  ...
  begin
    if (X mod FCharWidth) > (FCharWidth / 2) then
      ...
      Result := (X div FCharWidth) * FCharWidth;
  end
end;
