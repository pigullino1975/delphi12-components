...
function zipOpen (const pathname : PqiChar;  append : int_t) : zipFile; {ZEXPORT}
{ Create a zipfile.
...
function zipCloseFileInZip (afile : zipFile): int_t; {ZEXPORT}
{ Close the current file in the zipfile }

function zipClose (afile : zipFile; const global_comment : PAnsiChar): int_t; {ZEXPORT}
{ Close the zipfile }
...
type 
  curfile_info = record
    ...
    method : int_t;                { compression method of file currenty wr.}
...
function write_datablock(fout : FILEptr; ll : linkedlist_data_ptr) : int_t;
var
  ldi : linkedlist_datablock_internal_ptr;
begin
  ldi := ll^.first_block;
...
function ziplocal_putValue (afile : FILEptr; x : uLong; nbByte : int_t) : int_t;
var
...
begin
  ...
  zi^.ci.central_header := PAnsiChar( ALLOC( uInt(zi^.ci.size_centralheader)) );
...
function zipWriteInFileInZip (afile : zipFile; const buf : voidp; len : unsigned_t) : int_t; {ZEXPORT}
var
  zi : zip_internal_ptr;
  err : int_t;
var
...
function zipClose (afile : zipFile;
                   const global_comment : PAnsiChar) : int_t; {ZEXPORT}
var
  zi : zip_internal_ptr;
  err : int_t;
