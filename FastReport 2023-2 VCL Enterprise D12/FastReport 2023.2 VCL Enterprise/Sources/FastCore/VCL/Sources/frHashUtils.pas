...
function frMD5String(szString: AnsiString): AnsiString;
function frMD5File(szFilename: String): AnsiString;
function frMD5Stream(Stream: TStream): AnsiString;
procedure frMD5Buf(Buf: Pointer; const Len: Integer; Digest: Pointer);
function frPrintMD5(md5Digest: Puchar): AnsiString;

//ElfHash
function frElfHash(const S: string): Integer; overload;
function frElfHash(P: PChar; ALength: Integer): Integer; overload;

//Base64
function frBase64Encode(const S: AnsiString): AnsiString;
function frBase64Decode(const S: AnsiString): AnsiString;
function frBase64URLDecode(const S: AnsiString): AnsiString;
...
{$IFDEF FPC}
  base64,
{$ENDIF}
  SysUtils{$IFDEF DELPHI12}, AnsiStrings{$ENDIF};
...
const
  FBase64Charset = AnsiString('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/');
...
// context.
procedure TfrMD5.Update(chInput: Puchar; nInputLen: uint4);
var
...
function TfrMD5.Digest: Puchar;
begin
  Result := Puchar(@m_Digest);
end;

// MD5 basic transformation. Transforms state based on block.
procedure TfrMD5.Transform(block: Puchar);
var
  a, b, c, d: uint4;
...
// a multiple of 4.
procedure TfrMD5.Encode(dest: Puchar; src: Puint4; nLength: uint4);
var
  j: uint4;
  tmp: Puchar;
  tmp2: Puint4;
...
// a multiple of 4.
procedure TfrMD5.Decode(dest: Puint4; src: Puchar; nLength: uint4);
var
  j: uint4;
  tmp2: Puchar;
  tmp: Puint4;
...
function TfrMD5.RotateLeft(x: uint4; n: uint4): uint4;
begin
  Result := (x shl n) or (x shr (32 - n));
end;

procedure TfrMD5.FF(var a: uint4; b: uint4; c: uint4; d: uint4; x: uint4; s: uint4; ac: uint4);
begin
...
procedure TfrMD5.GG(var a: uint4; b: uint4; c: uint4; d: uint4; x: uint4; s: uint4; ac: uint4);
begin
...
procedure TfrMD5.HH(var a: uint4; b: uint4; c: uint4; d: uint4; x: uint4; s: uint4; ac: uint4);
begin
...
procedure TfrMD5.II(var a: uint4; b: uint4; c: uint4; d: uint4; x: uint4; s: uint4; ac: uint4);
begin
...
constructor TfrDataHashMap.Create;
begin
  ListKeys := TStringList.Create;
  ListValues := TList.Create;
end;

destructor TfrDataHashMap.Destroy;
begin
  Clear;
  ListKeys.Free;
  ListValues.Free;
...
function TfrDataHashMap.Count: Integer;
begin
  Result := ListKeys.Count;
end;

procedure TfrDataHashMap.Clear;
begin
  ListKeys.Clear();
  ListValues.Clear();
end;

procedure TfrDataHashMap.AddPicture(md5s: String);
begin
  AddPicture(md5s, -1);
end;

procedure TfrDataHashMap.AddPicture(md5s: String; i1: Integer);
var
  i: Integer;
begin
  i := FindPicture(md5s);
...
function TfrDataHashMap.FindPicture(md5s: String): Integer;
begin
  Result := ListKeys.IndexOf(md5s);
end;

function TfrDataHashMap.GetData(ind: Integer): Integer;
begin
  Result := -1;
...
function frElfHash(const S: string): Integer;
begin
  Result := frElfHash(PChar(S), Length(S));
end;

function frElfHash(P: PChar; ALength: Integer): Integer;
var
  I: Integer;
  AValue: Integer;
begin
  ...
  begin
    ...
    AValue := Result and $F0000000;
    if AValue <> 0 then
      Result := Result xor (AValue shr 24);
    Result := Result and not AValue;
    Inc(P);
  end;
end;
...
{$IFNDEF FPC}
  ...
  begin
    ...
    begin
      ...
      case F mod 3 of
        0 : begin
              P^ := FBase64Charset[C shr 2 + 1];
              Inc(P);
              R := (C and 3) shl 4;
            end;
        1 : begin
              P^ := FBase64Charset[C shr 4 + R + 1];
              Inc(P);
              R := (C and $0F) shl 2;
            end;
        2 : begin
              P^ := FBase64Charset[C shr 6 + R + 1];
              Inc(P);
              P^ := FBase64Charset[C and $3F + 1];
              Inc(P);
            end;
      end;
    end;
...
begin
  ...
  buf  := StringReplace(buf, '%', '=', [rfReplaceAll, rfIgnoreCase]);
  Result := frBase64Decode(AnsiString(buf));
end;
