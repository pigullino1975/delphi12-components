...
{$IFDEF FPC}
uses 
  Classes, Controls, Forms, StdCtrls, frCore, frTypes;
...
uses 
  Windows, Messages, Classes, Controls, Forms, StdCtrls, frxRichEdit, frCore;
...
begin
  ...
  SetWindowLongW(FControl.Handle, GWL_WNDPROC, frInteger(ObjectInstance));
...
begin
  ...
  SetWindowLongW(FControl.Handle, GWL_WNDPROC, frInteger(PrevWin32Proc));
...
begin
  ...
  SetWindowLongW(HWindow, GWL_WNDPROC, frInteger(ObjectInstance));
  if  (GetWindowLongW(HWindow, GWL_STYLE) and WS_CHILD <> 0)
  and (GetWindowLongW(HWindow, GWL_ID) = 0) then
    SetWindowLongW(HWindow, GWL_ID, frInteger(HWindow));
  SetProp(HWindow, MakeIntAtom(ControlAtom), TfrHandle(CreationControl));
  SetProp(HWindow, MakeIntAtom(WindowAtom), TfrHandle(CreationControl));
  CreationControl := nil;
