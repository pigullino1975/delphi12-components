...
begin
  ...
  Result := 0;
  Move(Pointer(frInteger(@Base) + Offset)^, Result, N);
end;

procedure SetNBytesAt(var Base; Offset, N, Value: Integer);
begin
  Assert(SizeOf(Value) >= N);
  Move(Value, Pointer(frInteger(@Base) + Offset)^, N);
end;
...
begin
  ...
  begin
    ...
    Move(Pointer(frInteger(@Data) + Offset)^, FChunk[Used], n);
...
begin
  ...
  for i := 0 to 7 do
    Move(FState[i, 0], Pointer(frInteger(@Digest) + i*8)^, 8);
end;
