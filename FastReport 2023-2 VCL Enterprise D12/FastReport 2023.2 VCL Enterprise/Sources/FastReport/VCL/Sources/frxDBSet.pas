...
type
  TfrxDataSetOption = (dsoStringAsDisplayText, dsoStringValueAsDisplayText);
...
uses frxUtils, frxRes, frUnicodeUtils;
...
{$ENDIF}
    begin
      if ((dsoStringValueAsDisplayText in DataSetOptions) and (dsoStringAsDisplayText in DataSetOptions) and
         Assigned(TField(Fields.Objects[i]).OnGetText) and (TField(Fields.Objects[i]) is TStringField)) then
        v := TField(Fields.Objects[i]).DisplayText
      else
...
{$IFDEF DELPHI10}
        if Field is TWideMemoField then
          TWideStrings(Obj).LoadFromStream(BlobStream)
        { special case for #597976 }
        else if (TBlobField(Field).BlobType
          in [ftWideString, ftWideMemo, ftFixedWideChar]) then
          TWideStrings(Obj).Text := Field.AsWideString
        else
{$ENDIF}
        begin
