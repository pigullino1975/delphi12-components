...
begin
  ...
  Encrypt(Data, FTail);
  if Data <> nil then Data := Pointer(frInteger(Data) + FTail);
  Dec(Size, FTail);
  Initialize(FBlockId + 1);
...
begin
  if Cached then
  begin
    FStream := TFileStream.Create(frGetTempFile, fmCreate);
    FRecords := TListCache.Create(frGetTempFile, frGetTempFile);
