...
uses
  Types, SysUtils, Classes, Controls, Variants, 
{$IFDEF FPC}
  LazHelper,
{$ELSE}
  WideStrings,
{$ENDIF}
  frCore, frCoreClasses, frUnicodeUtils, fs_iinterpreter, frxClass, frxCtrls, frxPreviewPages, frxEngine, frxDMPClass,
  frxVariables, fs_iformsrtti, frPictureGraphics;
...
begin
  ...
  begin
    ...
    AddEnum('TfrxCustomSignatureGraphicType', 'gtNo, gtImported, gtName');
...
begin
  ...
  begin
    if MethodName = 'GETMACROLIST' then
      Result := frInteger(TfrxPostProcessor(Instance).GetMacroList(Caller.Params[0]))
  ...
  begin
    if MethodName = 'ASBRUSHFILL' then
      Result := frInteger(TfrxCustomFill(Instance) as TfrxBrushFill)
    else if MethodName = 'ASGRADIENTFILL' then
      Result := frInteger(TfrxCustomFill(Instance) as TfrxGradientFill)
    else if MethodName = 'ASGLASSFILL' then
      Result := frInteger(TfrxCustomFill(Instance) as TfrxGlassFill)
  end
  else if ClassType = TWideStrings then
  begin
    _TStrings := TWideStrings(Instance);
    if MethodName = 'CREATE' then
      Result := frInteger(TWideStringList.Create)
    else if MethodName = 'ADD' then
      Result := _TStrings.Add(Caller.Params[0])
    else if MethodName = 'ADDOBJECT' then
      Result := _TStrings.AddObject(Caller.Params[0], TObject(frInteger(Caller.Params[1])))
      ...
      _TStrings.LoadFromStream(TStream(frInteger(Caller.Params[0])))
    else if MethodName = 'SAVETOFILE' then
      _TStrings.SaveToFile(Caller.Params[0])
    else if MethodName = 'SAVETOSTREAM' then
      _TStrings.SaveToStream(TStream(frInteger(Caller.Params[0])))
    else if MethodName = 'OBJECTS.GET' then
      Result := frInteger(_TStrings.Objects[Caller.Params[0]])
    else if MethodName = 'OBJECTS.SET' then
      _TStrings.Objects[Caller.Params[0]] := TObject(frInteger(Caller.Params[1]))
  ...
  begin
    ...
    if MethodName = 'OPEN' then
      ...
      _TfrxDataSet.GetFieldList(TStrings(frInteger(Caller.Params[0])))
  ...
  begin
    if MethodName = 'CLEAR' then
      ...
      Result := frInteger(TfrxComponent(Instance).FindObject(Caller.Params[0]))
    else if MethodName = 'LOADFROMSTREAM' then
      TfrxComponent(Instance).LoadFromStream(TStream(frInteger(Caller.Params[0])))
    else if MethodName = 'SAVETOSTREAM' then
      TfrxComponent(Instance).SaveToStream(TStream(frInteger(Caller.Params[0])), Caller.Params[1])
  ...
  begin
    if MethodName = 'LOADFROMFILE' then
      ...
      TfrxPictureView(Instance).LoadPictureFromStream(TStream(frxInteger(Caller.Params[0])))
  end
  ...
  begin
    if MethodName = 'CREATE' then
      Result := frInteger(TfrxVariables(Instance).Create)
  ...
  begin
    if MethodName = 'CREATE' then
      Result := frInteger(TfrxArray(Instance).Create)
    else if MethodName = 'VARIABLES.GET' then
  ...
  begin
    if MethodName = 'CALC' then
      ...
      Result := frInteger(TfrxReport(Instance).GetDataset(Caller.Params[0]))
    else if MethodName = 'LOADFROMFILE' then
  ...
  begin
    if MethodName = 'ADDANCHOR' then
      ...
      TfrxCustomEngine(Instance).ShowBand(TfrxBand(frInteger(Caller.Params[0])))
    else if MethodName = 'STOPREPORT' then
      TfrxCustomEngine(Instance).StopReport
    else if MethodName = 'GETPOSTPROCESSOR'  then
      Result := frInteger(TfrxPreviewPages(TfrxCustomEngine(Instance).PreviewPages).PostProcessor)
    else if MethodName = 'PROCESSOBJECT'  then
      TfrxCustomEngine(Instance).ProcessObject(TfrxView(frInteger(Caller.Params[0])));
  end
  ...
  begin
      if MethodName = 'CREATE' then
        Result := frInteger(TfrxStyles(Instance).Create(TfrxReport(frInteger(Caller.Params[0]))))
      else if MethodName = 'ITEMS.GET' then
        Result := frInteger(TfrxStyles(Instance).Items[Caller.Params[0]])
      else if MethodName = 'ADD' then
        Result := frInteger(TfrxStyles(Instance).Add)
      else if MethodName = 'FIND' then
        Result := frInteger(TfrxStyles(Instance).Find(Caller.Params[0]))
      else if MethodName = 'APPLY' then
        TfrxStyles(Instance).Apply
      else if MethodName = 'GETLIST' then
        TfrxStyles(Instance).GetList(TStrings(frInteger(Caller.Params[0])))
      else if MethodName = 'LOADFROMFILE' then
        TfrxStyles(Instance).LoadFromFile(Caller.Params[0])
      else if MethodName = 'LOADFROMSTREAM' then
        TfrxStyles(Instance).LoadFromStream(TStream(frInteger(Caller.Params[0])))
      else if MethodName = 'SAVETOFILE' then
        TfrxStyles(Instance).SaveToFile(Caller.Params[0])
      else if MethodName = 'SAVETOSTREAM' then
        TfrxStyles(Instance).SaveToStream(TStream(frInteger(Caller.Params[0])))
  end
  else if ClassType = TfrxBand then
  begin
    if MethodName = 'ALIGNCHILDREN' then TfrxBand(frInteger(Instance)).AlignChildren(True)
  end
  else if ClassType = TfrxReportPage then
  begin
    if MethodName = 'ALIGNCHILDREN' then TfrxReportPage(frInteger(Instance)).AlignChildren(True)
  end
  else if ClassType = TfrxPreviewPages then
  begin
    _PreviewPages := TfrxPreviewPages(Instance);
    if MethodName = 'PAGE.GET' then
      Result := frInteger(_PreviewPages.Page[Caller.Params[0]])
    else if MethodName = 'MODIFYOBJECT' then
      _PreviewPages.ModifyObject(TfrxComponent(frInteger(Caller.Params[0])));
  end
...
begin
  ...
  begin
    if PropName = 'OBJECTS' then
      Result := frInteger(TfrxComponent(Instance).Objects)
    else if PropName = 'ALLOBJECTS' then
      Result := frInteger(TfrxComponent(Instance).AllObjects)
    else if PropName = 'PARENT' then
      Result := frInteger(TfrxComponent(Instance).Parent)
    else if PropName = 'PAGE' then
      Result := frInteger(TfrxComponent(Instance).Page)
  ...
  begin
    if PropName = 'TEXT' then
      ...
      Result := frInteger(TfrxMemoView(Instance).Memo)
  ...
  begin
    if PropName = 'TERMINATED' then
      ...
      Result := frInteger(TfrxReport(Instance).Variables)
    else if PropName = 'STYLES' then
      Result := frInteger(TfrxReport(Instance).Styles)
    else if PropName = 'DRILLSTATE' then
      Result := frInteger(TfrxReport(Instance).DrillState)
  end
...
begin
  ...
  begin
    if PropName = 'PARENT' then
      TfrxComponent(Instance).Parent := TfrxComponent(frInteger(Value))
  end
  else if ClassType = TfrxView then
  begin
    if PropName = 'TAGSTR' then
  ...
  begin
    ...
    begin
      ...
      TfrxMemoView(Instance).Memo.Assign(TStrings(frInteger(Value)));
  end
  else if ClassType = TfrxDialogPage then
  begin
    if PropName = 'MODALRESULT' then
