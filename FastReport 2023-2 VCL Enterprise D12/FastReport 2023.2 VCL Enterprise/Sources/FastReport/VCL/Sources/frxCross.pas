...
uses
 ...
 {$ENDIF}
  ...
  Types, SysUtils, Classes, Controls, Graphics, Forms,
  frxClass, Variants, frCore;
...
{$ENDIF}
  ...
  public
    ...
    procedure ApplyStyle(Style: TfrxStyles); reintroduce; // todo change name
...
{$ENDIF}
  frxCrossRTTI, frxDsgnIntf, frXML, frxUtils, frxXMLSerializer, frxRes,
  frxDMPClass, frxVariables, frUnicodeUtils, Math;
...
begin
  ...
  begin
    v := VarArrayOf([frInteger(Memo), Row, Column, Cell, GetRowIndexes(Row),
...
begin
  ...
  begin
    v := VarArrayOf([frInteger(Memo), Header.GetIndexes, Header.GetValues, Header.Value]);
    if Report <> nil then
