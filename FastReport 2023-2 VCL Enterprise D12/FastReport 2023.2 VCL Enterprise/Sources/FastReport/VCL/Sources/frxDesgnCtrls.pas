...
uses
  ...
  frPictureCache, frxDPIAwareBaseControls,
{$IFDEF FPC}
...
{$ENDIF}
  ...
  procedure frxAssignToolbarSize(AToolBar: TToolBar; AImgList: TImageList; APPI: Integer);
{$IFDEF FPC}
//  procedure Register;
{$ENDIF}
...
uses
  ...
  frUtils, frxUtils, frXML, frxRes, frxBaseForm, frDPIAwareUtils;
...
begin
  ...
  if (ssDouble in Shift) and (Button = mbLeft) then
    FGuides.Add(frFloatToStr(Position))
...
var
  ...
  begin
    ...
    aRoot := TfrXMLItem.Create;
    wr := TfrXMLWriter.Create(ss);
    try
...
var
  ...
  var
    ...
    i: Integer;
  begin
    rd := TfrXMLReader.Create(AReader.Stream);
...
procedure frxAssignToolbarSize(AToolBar: TToolBar; AImgList: TImageList; APPI: Integer);
begin
  AToolBar.ButtonWidth := AImgList.Width + 7;
  AToolBar.ButtonHeight := AImgList.Height + 7;
{$IFDEF FPC}
  AToolBar.Height := AToolBar.ButtonHeight;
  AToolBar.Width := MulDiv(AToolBar.Width, APPI, frx_DefaultPPI);
{$ENDIF}
end;
