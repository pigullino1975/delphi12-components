...
uses
  ...
  StdCtrls, ExtCtrls, frCore, frxClass, frxProgress, Buttons, ComCtrls
{$IFDEF DELPHI6}
, Variants
{$ENDIF};
...
uses frxUtils, frxprinter, Printers,
  {$IFNDEF Linux}Winspool, {$ENDIF}frxExportTxtPrn, frxFileUtils, frxres;
...
begin
  ...
  begin
    if Report.FileName <> '' then
      FileName := ChangeFileExt(frGetTemporaryFolder + ExtractFileName(Report.FileName), frxGet(8326))
    else
      FileName := ChangeFileExt(frGetTempFile, frxGet(8326))
  end;
  CurrentPage := 0;
  FirstPage := True;
  ClearLastPage;
...
begin
{$IFDEF Linux}
  Result := frGetTempFileName(Path, FileName);
{$ELSE}
{$IFDEF DELPHI12}
