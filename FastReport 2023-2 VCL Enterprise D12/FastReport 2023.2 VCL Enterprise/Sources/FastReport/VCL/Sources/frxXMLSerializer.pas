...
uses
  ...
  {$ENDIF}
  SysUtils, Classes, Graphics, Controls, Forms, Dialogs, Variants,
  TypInfo, frXML, frUnicodeUtils, frxClass, frxVariables, frCore;
...
{ TfrxXMLSerializer is the XML analogue of the Delphi component streaming -
  ...
  public
    ...
    procedure ReadRootComponent(Root: TfrxComponent; XMLItem: TfrXMLItem = nil);
...
begin
  ...
  begin
    ...
    if Ref <> nil then
      SetOrdProp(Item.Obj, Item.PropInfo, frInteger(Ref));
  end;
...
var
  ...
  function OldLazToNewAllValue(const Buffer: string): String;
  var
    AnsiBuf: AnsiString;
  begin
    SetLength(AnsiBuf, Length(Buffer) div 2);
    HexToBin(PChar(Buffer), PAnsiChar(AnsiBuf), Length(AnsiBuf));
    Delete(AnsiBuf, 1, 8);
    Result := UTF8Decode(AnsiBuf);
  end;
...
{$ENDIF}
            ...
            begin
              TWideStrings(Obj1).Text := frxXMLToStr(AValue);
              continue;
            end
          ...
          begin
            if (Obj1.ClassName = 'TfrxVariable') then
            begin
              if (AValue = '0556616C75650600') then
                AValue := '0556616C7565060764656661756C74';
              AName := 'Value';
              AValue := OldLazToNewAllValue(AValue);
            end
            else
            begin
        ...
        if (p <> nil) and (p.SetProc <> nil) then
          case p.PropType^.Kind of
            ...
            tkInt64:
              SetInt64Prop(Obj1, p, StrToInt64(String(AValue)));
  ...
  var
    ...
    begin
      if Ancestor <> nil then
        Result := Value = GetInt64Prop(Ancestor, PropList[i])
      else
        Result := Value = PropList[i].Default;
    end;

  begin
    Value := GetInt64Prop(Obj, PropList[i]);
...
procedure TfrxXMLSerializer.ReadRootComponent(Root: TfrxComponent;
  XMLItem: TfrXMLItem = nil);
var
  XMLDoc: TfrXMLDocument;
  CompList: TList;
  SaveParent: TfrxComponent;

  procedure DoRead(Item: TfrXMLItem; Owner: TfrxComponent);
  var
    i: Integer;
    c: TfrxComponent;
    IsAncestor: Boolean;
...
procedure TfrxXMLSerializer.WriteRootComponent(Root: TfrxComponent;
  SaveChildren: Boolean = True; XMLItem: TfrXMLItem = nil);
var
  XMLDoc: TfrXMLDocument;

  procedure DoWrite(Item: TfrXMLItem; ARoot: TfrxComponent);
  var
    i: Integer;
...
var
  ...
  RootItem: TfrXMLItem;
begin
  rd := TfrXMLReader.Create(FStream);
  RootItem := TfrXMLItem.Create;
