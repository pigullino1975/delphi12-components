...
type
  ...
  private
    FCustomExportDialogClass: TfrxBaseExportDialogClass;
  protected
    procedure AfterFinish; override;
  public
    function ShowModal: TModalResult; override;
    class function ExportDialogClass: TfrxBaseExportDialogClass; virtual;
    property CustomExportDialogClass: TfrxBaseExportDialogClass read FCustomExportDialogClass write FCustomExportDialogClass;
  end;
...
var
  LDialog: TfrxBaseExportDialog;
  LDialogClass: TfrxBaseExportDialogClass;
begin
  if not Assigned(Stream) then
  begin
    LDialogClass := FCustomExportDialogClass;
    if not Assigned(LDialogClass) then
      LDialogClass := ExportDialogClass;
    LDialog := TfrxBaseExportDialog(LDialogClass.NewInstance);
    LDialog.Create(nil);
    try
      LDialog.InitControlsFromFilter(Self);
      Result := LDialog.ShowModal;
      if Result = mrOk then
        LDialog.InitFilterFromDialog(Self);
    finally
      LDialog.Free;
    end;
  end
