...
uses
  ...
  Types, Forms, Graphics, StdCtrls, Dialogs, RichEdit, Menus, ComCtrls, frCore,
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  frTypes;
...
uses
  Printers, ComStrs, OleConst, OleDlg, OleCtnrs, Registry, Math,
  frThreading;
...
{$IFDEF DELPHI12}
      if IsRTFHeader then
            SendMessage(RichEdit.Handle, WM_USER + 97 {EM_SETTEXTEX}, frInteger(@SetText),
...
{$ENDIF}
  try
    with EditStream do
    begin
      dwCookie := frInteger(Pointer(@StreamInfo));
...
begin
  ...
  begin
    ...
    FLResult := 0;
    if DoPerform then
      frThreadSynchronize(DoPerformSynchMessage)
    else
      frThreadSynchronize(DoSendSynchMessage);
    Result := FLResult;
  end
  else
    if DoPerform then
...
{$ENDIF}
  ...
  if RichEditVersion > 1 then
    Result := Perform(EM_GETTEXTLENGTHEX, frInteger(@TextLen), 0)
  else
