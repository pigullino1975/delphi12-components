...
{$ELSE}
  ...
  {$IFDEF LCLGTK2} CairoCanvas, {$ENDIF}
{$ENDIF}
  Messages, Types, SysUtils, Classes, Graphics, Controls, Forms,
  Dialogs, Printers, Variants,
  frxClass, frXML, frPictureCache, frBaseGraphicsTypes, frxVariables, frCore;
...
type
  ...
  public
    ...
    procedure DeleteItems(From: TfrXMLItem); override;
    ...
    procedure ShiftItems(From: TfrXMLItem; NewTop: Integer); override;
    function GetCurPosition: TfrXMLItem; override;
  end;
  ...
  private
    ...
    FPagesItem: TfrXMLItem;  { shortcut to XMLDoc.Root.FindName('previewpages') }
    FPictureCache: TfrxPictureCache;
    FSourcePages: TList;      { list of source pages }
    FTempStream: TStream;
    FXMLDoc: TfrXMLDocument; { parsed FP3 document }
    FXMLSize: Integer;
...
//    FMouseInView: Boolean;    { used in the ObjectOver to deteterminate mouseleave/mouseenter events }
    ...
    procedure AfterLoad;
    procedure ApplyStyle(AComponent: TfrxComponent);
    procedure BeforeSave;
    procedure ClearSourcePages;
    function CurXMLPage: TfrXMLItem;
  ...
  protected
    ...
    procedure ClearPageCache; override;
    function GetCount: Integer; override;
  ...
  public
    ...
    function FindAnchor(const Text: String): TfrXMLItem;
    function FindAnchorRoot: TfrXMLItem;
...
function TfrxOutline.Root: TfrXMLItem;
begin
  Result := TfrxPreviewPages(PreviewPages).FXMLDoc.Root.FindItem('outline');
end;

procedure TfrxOutline.DeleteItems(From: TfrXMLItem);
var
  nCount: Integer;
begin
...
procedure TfrxOutline.ShiftItems(From: TfrXMLItem; NewTop: Integer);
var
  i, TopY, CorrY: Integer;

  procedure EnumItems(Item: TfrXMLItem);
  var
    i: Integer;
  begin
...
function TfrxOutline.GetCurPosition: TfrXMLItem;
begin
  if Count = 0 then
    Result := nil else
...
begin
  ...
  if i <> -1 then
    Result := FSourceNames.Objects[frInteger(FNames.Objects[i])];
end;
...
begin
  ...
  i := FNames.IndexOf(Name);
  if i <> -1 then
    Result := FSourceNames[frInteger(FNames.Objects[i])];
end;
...
var
  PagesRoot, OutlineRoot: TfrXMLItem;
  p: TfrXMLItem;
  i: Integer;
begin
  if FFirstPageIndex = -1 then
...
function TfrxPreviewPages.CurXMLPage: TfrXMLItem;
begin
...
function TfrxPreviewPages.FindAnchor(const Text: String): TfrXMLItem;
var
  AnchorRoot, Item: TfrXMLItem;
  i: Integer;
begin
...
function TfrxPreviewPages.FindAnchorRoot: TfrXMLItem;
begin
...
var
  ...
  CurPageItem: TfrXMLItem;

  procedure DoAdd(c: TfrxComponent; Item: TfrXMLItem);
  var
    i: Integer;
...
begin
  ...
  xi.Prop[PropPaperWidth] := frFloatToStr(Width);
  xi.Prop[PropPaperHeight] := frFloatToStr(Height);
end;
...
begin
  ...
  xi.Prop[PropPaperSize] := '256';
  xi.Prop[PropPaperWidth] := frFloatToStr(SourcePage.PaperWidth);
  xi.Prop[PropPaperHeight] := frFloatToStr(SourcePage.PaperHeight);
...
var
  ...
  procedure CorrectX(xi: TfrXMLItem);
  var
    X: Extended;
  begin
    if xi.Prop['l'] <> '' then
  ...
  procedure CorrectY(xi: TfrXMLItem);
  var
    Y: Extended;
  begin
    if xi.Prop['t'] <> '' then
...
{ load the picturecache }
  ...
  i := FXMLDoc.Root.Find('styles');
  if i > 0 then
  begin
    xi := FXMLDoc.Root.Items[i];
    Styles.LoadFromXMLItem(xi, FXMLDoc.OldVersion);
    xi.Free;
  end;
end;

procedure TfrxPreviewPages.ApplyStyle(AComponent: TfrxComponent);
begin
  if (AComponent is TfrxView) and (TfrxView(AComponent).Style <> '') then
    TfrxView(AComponent).ApplyStyle(FindStyle(TfrxView(AComponent).Style));
end;
...
//  xi := FXMLDoc.Root.FindItem('picturecache');
  ...
  if IsStylesActive then
  begin
    xi := FXMLDoc.Root.FindItem('styles');
    Styles.SaveToXMLItem(xi);
  end;
end;
...
var
  ...
  procedure DoObjects(Item: TfrXMLItem; Owner: TfrxComponent);
  var
    i: Integer;
    c, c0: TfrxComponent;
...
var
  ...
  xi: TfrXMLItem;
  xs: TfrxXMLSerializer;
  AnchorRootSource, AnchorRootDest: TfrXMLItem;
...
var
  ...
  PrintedBins: TList;
  LPrinter, LGlobalPrinter: TfrxCustomPrinter;
  ...
  var
    ...
    begin
      ...
      if not PagePrinted or (orient <> LastOrientation) then
        LPrinter.SetPrintParams(Report.PrintOptions.PrintOnSheet,
      ...
      begin
        pieceX := SheetWidth * (LPrinter.DPI.X / 25.4);
        pieceY := SheetHeight * (LPrinter.DPI.Y / 25.4);
      end
      else
      begin
        pieceX := SheetHeight * (LPrinter.DPI.X / 25.4);
        pieceY := SheetWidth * (LPrinter.DPI.Y / 25.4);
      end;

      //marginY := 0;
      printedY := 0;
      offsY := -LPrinter.TopMargin * LPrinter.DPI.Y / 25.4;
      ...
      begin
        //marginX := 0;
        printedX := 0;
        offsX := -LPrinter.LeftMargin * LPrinter.DPI.X / 25.4;
        SplitAddX := 0;

        for x := 1 to countX do
        begin
          SavedDCState := SaveDC(LPrinter.Canvas.Handle);
          OldRgn := CreateRectRgn(0, 0, 10000, 10000);
          GetClipRgn(LPrinter.Canvas.Handle, OldRgn);
          OutRgn :=  CreateRectRgn(Round(SplitAddX + LPrinter.LeftMargin * LPrinter.DPI.X / 25.4),
                        Round(SplitAddY + LPrinter.TopMargin * LPrinter.DPI.Y / 25.4),
                        Round(Page.PaperWidth * fr01cm * LPrinter.DPI.X / 96 - LPrinter.RightMargin * LPrinter.DPI.X / 25.4) - 1,
                        Round(Page.PaperHeight * fr01cm * LPrinter.DPI.Y / 96 - LPrinter.BottomMargin * LPrinter.DPI.Y / 25.4) - 1);
          SelectClipRgn(LPrinter.Canvas.Handle, OutRgn);
          {$IFNDEF FPC}
          SetMetaRgn(LPrinter.Canvas.Handle);
          {$ENDIF}
          try
            LPrinter.BeginPage;
            DrawPage(Index, LPrinter.Canvas, LPrinter.DPI.X / 96, LPrinter.DPI.Y / 96,
              offsX, offsY);
...
{$ENDIF}
            ...
            { restore regions }
            SelectClipRgn(LPrinter.Canvas.Handle, OldRgn);
            DeleteObject(OutRgn);
            DeleteObject(OldRgn);
            RestoreDC(LPrinter.Canvas.Handle, SavedDCState);
          end;
          SplitAddX := SplicingLineSz * LPrinter.DPI.X / 25.4;
          marginX := LPrinter.LeftMargin * LPrinter.DPI.X / 25.4;
          printedX := printedX + (pieceX - marginX - LPrinter.RightMargin * LPrinter.DPI.X / 25.4) -
            SplitAddX;
          offsX := -printedX;
        end;
        marginY := LPrinter.TopMargin * LPrinter.DPI.Y / 25.4;
        SplitAddY := SplicingLineSz * LPrinter.DPI.Y / 25.4;
        printedY := printedY + (pieceY - marginY - LPrinter.BottomMargin * LPrinter.DPI.Y / 25.4) -
          SplitAddY;
        offsY := -printedY;
      end;
    ...
    begin
      ...
      begin
        if (not PagePrinted) or
          ...
          LPrinter.SetPrintParams(Page.PaperSize, Page.PaperWidth, Page.PaperHeight,
        ...
        begin
          ...
          SheetWidth := LPrinter.PaperWidth;
          SheetHeight := LPrinter.PaperHeight;
        end;

      if not PagePrinted then
        LPrinter.BeginDoc;
      if not LPrinter.Printing then
        begin
          LPrinter.Abort;
          Result := False;
          Exit;
      ...
      DrawPage(Index, LPrinter.Canvas, LPrinter.DPI.X / 96 * ZoomX, LPrinter.DPI.Y / 96 * ZoomY,
        -LPrinter.LeftMargin * LPrinter.DPI.X / 25.4,
        -LPrinter.TopMargin * LPrinter.DPI.Y / 25.4);
...
{$ENDIF}
    ...
    begin
      ...
      { start the cycle }
      pieceX := Page.PaperWidth * (LPrinter.DPI.X / 25.4);
      pieceY := Page.PaperHeight * (LPrinter.DPI.Y / 25.4);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            DrawPage(GetPageIndex(Index), LPrinter.Canvas, LPrinter.DPI.X / 96, LPrinter.DPI.Y / 96,
              offsX, offsY);
...
{$ENDIF}
    ...
    if Report.ReportOptions.Name <> '' then
      LPrinter.Title := Report.ReportOptions.Name else
      LPrinter.Title := Report.FileName;
...
{$ENDIF}
    ...
    if Report.PrintOptions.Collate then
      ...
      begin
        ...
        if NeedFinishDuplex or (LPrinter.IsDuplexActive and (MaxCount mod 2 = 1)
          and (ACopyNo < Report.PrintOptions.Copies))
        then
        begin
          NeedFinishDuplex := False;
          LPrinter.BeginPage;
...
{$ELSE}
  LPrinter := nil;
  SavePrintOptions := TfrxPrintOptions.Create;
  if Report.EngineOptions.EnableThreadSafe then
    frxPrinters.LockPrinterChange;
  try
    SavePrintOptions.Assign(Report.PrintOptions);
    DuplexMode := Report.PrintOptions.Duplex;
    Report.SelectPrinter;
    // temprary disabled. Need to move dialog to ThreadSynchronize
    if Report.PrintOptions.ShowDialog and not Report.EngineOptions.EnableThreadSafe then
      with TfrxPrintDialog.Create(Application) do
      try
        AReport := Report;
        ADuplexMode := DuplexMode;
    ...
    if Report.EngineOptions.EnableThreadSafe then
    begin
      LGlobalPrinter := frxPrinters.GetPrinterByName(Report.PrintOptions.Printer);
      if LGlobalPrinter = nil then
        LGlobalPrinter := frxPrinters.Printer;
      LPrinter := LGlobalPrinter.CreateCopy;
    end;
    if LPrinter = nil then
      LPrinter := frxPrinters.Printer;
    if Report.PrintOptions.PrnOutFileName <> '' then
      LPrinter.FileName := Report.PrintOptions.PrnOutFileName;

    if Report.PrintOptions.PrintMode <> pmDefault then
    begin
      if Report.PrintOptions.PrintOnSheet <> 256 then
        LPrinter.SetViewParams(Report.PrintOptions.PrintOnSheet, 0, 0, poPortrait)
      else
        LPrinter.SetViewParams(Report.PrintOptions.PrintOnSheet, LPrinter.PaperWidth,
                                          LPrinter.PaperHeight, poPortrait);

      if LPrinter.PaperWidth <> 0 then
        SheetWidth := LPrinter.PaperWidth
      else
        SheetWidth := LPrinter.DefPaperWidth;
      if LPrinter.PaperHeight <> 0 then
        SheetHeight := LPrinter.PaperHeight
      else
        SheetHeight := LPrinter.DefPaperHeight;
      SplicingLineSz := Report.PrintOptions.SplicingLine;
    end;
    ...
    if Assigned(LPrinter) and LPrinter.IsCopy then
    begin
      frxPrinters.UnlockPrinterChange;
      LPrinter.Free;
    end;
    SavePrintOptions.Free;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      try
        Params := VarArrayOf([frInteger(FLastObjectOver),
          frInteger(ModObjList), EventParams.Refresh]);
