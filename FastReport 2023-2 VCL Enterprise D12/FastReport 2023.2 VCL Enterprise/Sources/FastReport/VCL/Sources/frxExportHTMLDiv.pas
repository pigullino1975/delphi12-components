...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  frxExportHelpers, frxExportBaseDialog, frxVectorCanvas, frCSSStyle;
...
uses
  ...
  frxRes,
  Math,
  frXML, // for access to TfrxPreviewPages.FindAnchor's results
  frxPreviewPages,
...
begin
  ra := Round(a);
  Result := frIfStr(not AllowNegative and (ra <= 0), '0', IntToStr(ra) + 'px');
end;
...
begin
  Result := frIfStr(fsBold in Font.Style, 'bold ') + frIfStr(fsItalic in Font.Style,
    'italic ') + IntToStr(Font.Size) + 'pt ' + Font.Name
end;

function GetTextDecoration(s: TFontStyles): string;
begin
  Result := frIfStr(fsUnderline in s, 'underline ') + frIfStr(fsStrikeOut in s,
    'line-through')
end;
...
function TfrxHTMLDivExport.CreateFrameDiv(Obj: TfrxView): TfrxHTMLItem;
  ...
  var
    L, T, r, B: string;
  begin
    L := frIfStr(ftLeft in Frame.Typ, GetFrameLineStyle(Frame.LeftLine));
    T := frIfStr(ftTop in Frame.Typ, GetFrameLineStyle(Frame.TopLine));
    r := frIfStr(ftRight in Frame.Typ, GetFrameLineStyle(Frame.RightLine));
    B := frIfStr(ftBottom in Frame.Typ, GetFrameLineStyle(Frame.BottomLine));
...
begin
  ...
  Correction := frIfInt(Obj is TfrxShapeView, 2);
