...
uses 
  Windows, SysUtils, Types, Classes, Graphics, Forms, Printers, frCore
  {$IFDEF FPC}
...
type
  ...
  public
    ...
    function CreateCopy: TfrxCustomPrinter;
    procedure Assign(APrinter: TfrxCustomPrinter);
    ...
    property Handle: TfrHandle read FHandle;
  ...
  public
    ...
    function GetPrinterByName(const AName: string): TfrxCustomPrinter;
    procedure LockPrinterChange;
    procedure UnlockPrinterChange;
...
{$ELSE}
  WinSpool,
{$ENDIF}
  frxUtils, frxRes, frThreading, frUnicodeUtils;
...
function TfrxCustomPrinter.CreateCopy: TfrxCustomPrinter;
begin
  Result := TfrxCustomPrinter(NewInstance);
  try
    Result.Create(FName, FPort);
    Result.Assign(Self);
    Result.Init;
    Result.FIsCopy := True;
  except
...
procedure TfrxCustomPrinter.Assign(APrinter: TfrxCustomPrinter);
begin
  Bin := APrinter.Bin;
  FileName := APrinter.FileName;
  Title := APrinter.Title;
end;
...
function TfrxPrinters.GetPrinterByName(const AName: string): TfrxCustomPrinter;
var
  LIndex: Integer;
begin
  Result := nil;
  LIndex := frxPrinters.IndexOf(AName);
  if LIndex <> -1 then
    Result := frxPrinters[LIndex];
end;
...
begin
  ...
  try
    ...
    for i := 0 to FPrinterList.Count - 1 do
      if AnsiCompareText(TfrxCustomPrinter(FPrinterList[i]).Name, AName) = 0 then
      begin
        Result := i;
        break;
      end;
...
procedure TfrxPrinters.Lock;
begin
  FCriticalSection.Enter;
end;

procedure TfrxPrinters.LockPrinterChange;
begin
  Lock;
  try
...
begin
  // do not change singleton state
  if FChangeLockCount > 0 then Exit;
  if Value <> -1 then
    FPrinterIndex := Value
  else
...
procedure TfrxPrinters.Unlock;
begin
  FCriticalSection.Leave;
end;

procedure TfrxPrinters.UnlockPrinterChange;
begin
  Lock;
  try
...
begin
  ...
  begin
    ...
    M.Data := nil;
    frThreadSynchronize(TThreadMethod(M));
  end;
  Result := FPrinters;
end;
