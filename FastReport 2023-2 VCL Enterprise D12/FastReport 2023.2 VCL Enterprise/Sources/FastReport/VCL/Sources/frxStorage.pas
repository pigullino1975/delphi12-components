...
begin
  ...
  Result := Pointer(frInteger(b.Data) + (Index mod FBlock) * ItemSize);
end;
...
begin
  ...
  begin
    ...
    Move(Pointer(frInteger(@Buffer) + p)^, FChunk[FUsed], n);
...
begin
  ...
  begin
    ...
    Write(Pointer(frInteger(@Buffer) + n)^, Count - n);
  end
  else
  begin
    n := 0;
    ...
    while n + 3 <= Count do
    begin
      Encode(PInteger(frInteger(@Buffer) + n)^, 3);
      Inc(n, 3);
    end;

    if n < Count then
    begin
      FCache := PInteger(frInteger(@Buffer) + n)^ and BitMask(Count - n);
      FUsed := Count - n;
    end;
  end;
