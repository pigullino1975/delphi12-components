...
{$ENDIF}
  ...
  frxAnaliticGeometry, frxPolygonTemplate, frxBaseForm, frTypes;
...
uses
  ...
  frCore, frxCustomEditors, frxRes, frxDsgnIntf, frxMapLayer, frxMapShapeTags, frUtils, frxUtils,
  frxEditPicture, frDPIAwareUtils;
...
type
  ...
  private
    function GetPair(Index: TfrListInteger): TCanvasMapPair;
    procedure SetPair(Index: TfrListInteger; const Value: TCanvasMapPair);
  public
    procedure RecalcCanvas;
    function CalcFocusDoubleRect: TDoubleRect;
    function GetSegment(Index: TfrListInteger): TSegment;
    procedure CanvasShift(dX, dY: Extended);
    function CanvasPoints: TPointArray;

    property Items[Index: TfrListInteger]: TCanvasMapPair read GetPair write SetPair; default;
  end;
...
begin
  inherited;
  frxImages.MainButtonImages.GetBitmap(101, PointMenuSpeedButton.Glyph);
  frxImages.MainButtonImages.GetBitmap(101, RectMenuSpeedButton.Glyph);
  frxImages.MainButtonImages.GetBitmap(101, PictureMenuSpeedButton.Glyph);
  frxImages.MainButtonImages.GetBitmap(101, TemplateMenuSpeedButton.Glyph);
end;
...
function TPairList.GetPair(Index: TfrListInteger): TCanvasMapPair;
begin
  if      Index < 0 then
    Index := Index + Count
...
function TPairList.GetSegment(Index: TfrListInteger): TSegment;
begin
...
procedure TPairList.SetPair(Index: TfrListInteger; const Value: TCanvasMapPair);
begin
  if Index < 0 then
    Index := Index + Count;
...
procedure TLegendFigure.Draw;
  ...
  begin
    Result := frIfInt(Value > 127, 0, 255);
  end;
var
  Bitmap: TBitmap;
  Rect: TRect;
