...
uses
  ...
  frxDBSet, fs_idbrtti, Variants, frCore;
...
begin
  ...
  begin
    if MethodName = 'INDEXOF' then
      ...
      Result := frInteger(TfrxParams(Instance).Items[Caller.Params[0]])
  end
  ...
  begin
    if MethodName = 'LAST' then
      ...
      Result := frInteger(TfrxCustomDataset(Instance).GetDataSet.FieldByName(Caller.Params[0]))
  end
  else if ClassType = TfrxCustomQuery then
  begin
    if MethodName = 'PARAMBYNAME' then
      Result := frInteger(TfrxCustomQuery(Instance).ParamByName(Caller.Params[0]))
  end
end;
...
begin
  ...
  begin
    if PropName = 'DATASET' then
      Result := frInteger(TfrxCustomDataset(Instance).DataSet)
    else if PropName = 'ACTIVE' then
      Result := TfrxCustomDataset(Instance).Active
    else if PropName = 'FIELDS' then
      Result := frInteger(TfrxCustomDataset(Instance).Fields)
  end
