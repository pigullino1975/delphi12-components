...
uses
  Windows, Classes, Types, fs_iinterpreter, frxIBXComponents, fs_iibxrtti, Variants, frCore;
...
begin
  ...
  begin
    if PropName = 'DATABASE' then
      Result := frInteger(TfrxIBXDatabase(Instance).Database)
  end
  else if ClassType = TfrxIBXTable then
  begin
    if PropName = 'TABLE' then
      Result := frInteger(TfrxIBXTable(Instance).Table)
  end
  else if ClassType = TfrxIBXQuery then
  begin
    if PropName = 'QUERY' then
      Result := frInteger(TfrxIBXQuery(Instance).Query)
  end
end;
