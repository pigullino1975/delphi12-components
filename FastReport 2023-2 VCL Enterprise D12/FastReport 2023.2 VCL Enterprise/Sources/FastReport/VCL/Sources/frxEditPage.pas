...
uses
  Printers, frxPrinter, frxClass, frUtils, frxUtils, frxDesgn, frxRes;
...
begin
  ...
  begin
    ...
    WidthE.Text := frFloatToStr(mmToUnits(p.PaperWidth));
    HeightE.Text := frFloatToStr(mmToUnits(p.PaperHeight, False));
    PortraitRB.Checked := p.Orientation = poPortrait;
    LandscapeRB.Checked := p.Orientation = poLandscape;

    MarginLeftE.Text := frFloatToStr(mmToUnits(p.LeftMargin));
    MarginRightE.Text := frFloatToStr(mmToUnits(p.RightMargin));
    MarginTopE.Text := frFloatToStr(mmToUnits(p.TopMargin, False));
    MarginBottomE.Text := frFloatToStr(mmToUnits(p.BottomMargin, False));
    ...
    ColumnWidthE.Text := frFloatToStr(mmToUnits(p.ColumnWidth));
    for i := 0 to p.ColumnPositions.Count - 1 do
      ColumnPositionsM.Lines.Add(frFloatToStr(mmToUnits(frxStrToFloat(p.ColumnPositions[i]))));
  ...
  begin
    ...
    begin
      ...
      if Trim(HeightE.Text) = '' then
        HeightE.Text := frFloatToStr(mmToUnits(p.PaperHeight));
      ...
      for i := 0 to ColumnPositionsM.Lines.Count - 1 do
        p.ColumnPositions.Add(frFloatToStr(UnitsTomm(frxStrToFloat(ColumnPositionsM.Lines[i]))));
...
begin
  ...
  begin
    ...
    WidthE.Text := frFloatToStr(mmToUnits(PaperWidth));
    HeightE.Text := frFloatToStr(mmToUnits(PaperHeight, False));
  end;
...
begin
  ...
  begin
    ...
    ColumnWidthE.Text := frFloatToStr(mmToUnits(w));

    with ColumnPositionsM.Lines do
    begin
      Clear;
      while Count < n do
        Add(frFloatToStr(mmToUnits(Count * w)));
    end;
  end;
end;
