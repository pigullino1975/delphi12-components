...
uses
  ...
  frGzip, frHashUtils, WinSock, frxBaseTransportConnection, frxBaseSocketIOHandler;
...
uses
  StrUtils,
  frxFileUtils, frUtils, frxUtils, frxOpenSSL, frxHTTPProtocol, frxHTTPSProtocol
{$IFDEF DELPHI12}
  ,AnsiStrings
{$ENDIF};
...
begin
  ...
  SessionId := frIfStr(s1 <> '', String(s1));
    ...
    begin
      ...
      begin
        ...
        if Assigned(aToDataStream) and (Errors.Count = 0) then
          ...
          try
            frDecompressStream(FRawDataStream, aToDataStream)
          except
