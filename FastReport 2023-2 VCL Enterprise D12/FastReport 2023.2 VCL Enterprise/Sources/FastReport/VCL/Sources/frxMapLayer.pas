...
{$IFNDEF FPC}
  Windows,
{$ELSE}
  LCLType, LCLIntf, LCLProc, LazHelper, frLazGraphicClasses,
{$ENDIF}
...
uses
  ...
  Types, Variants, frUtils, frxUtils, frxRes, frxMapLayerTags {Editor},
  frPictureGraphics, frBaseGraphicsTypes,
...
begin
  case LabelKind of
    ...
    begin
      ...
      Result := Result + frIfStr((Result <> '') and (stValue <> ''), #13#10) + stValue;
    end;
  end;
end;
...
begin
  with FShapes[iRecord] do
    Result := frIfStr(IsValueEmpty, '', Format(ValueFormat, [Value]));
end;
...
begin
  case FOperation of
    opAverage:
      Result := frIfReal(FCount > 0, FValue / FCount);
    opMin, opMax, opSum:
      Result := frIfReal(FCount > 0, FValue);
  else // opCount:
    Result := FCount;
  end;
end;
...
var
  i: integer;
begin
  Result := frIfReal(Count > 0, OperationCalculator[0].Get);
