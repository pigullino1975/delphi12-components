...
function TfrxAPDFExport.WriteDataStream(Res, Src: TStream; const Ext: string;
  ...
  begin
    ...
    try
      ...
      Data.Size := 0;
      frDeflateStream(Temp, Data, gzFastest);
    finally
      Temp.Free;
    end;
  end;
...
begin
  ...
  begin
    if Report.FileName <> '' then
      FileName := ChangeFileExt(frGetTemporaryFolder + ExtractFileName(Report.FileName), frxGet(8708))
    else
      FileName := ChangeFileExt(frGetTempFile, frxGet(8708))
  end;
...
begin
  ...
  Writer := TfrXMLWriter.Create(Stream);
