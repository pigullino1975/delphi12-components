...
{$ENDIF}
  ...
  fs_iinterpreter, Printers,
{$IFNDEF FPC}
  frxWatchForm, frxBreakPointsForm,
{$ELSE}
  frxLazBWForm,
{$ENDIF}
  frPictureCache, frxSearchDForm, Variants, frxDPIAwareBaseControls,
{$IFDEF DELPHI17}
  System.Actions,
{$ENDIF}
{$IFDEF DELPHI9}
...
{$ENDIF}
  ...
  TfrxDesignerForm = class(TfrxCustomDesigner)
    ...
    DockBottom: {$IFDEF FPC}TfrxControlBar{$ELSE}TControlBar{$ENDIF};
    DockTop: {$IFDEF FPC}TfrxControlBar{$ELSE}TControlBar{$ENDIF};
...
{$ENDIF}
    ...
    StylesDockTB: TfrxDockToolBar;
    StylesPan: TfrxTBPanel;
    StyleCB: TfrxComboBox;
    StyleSheetPan: TfrxTBPanel;
    StyleSheetCB: TfrxComboBox;
    StyleBookTB: TToolButton;
    StyleSheetAddTB: TToolButton;
    StyleSheetDeleteTB: TToolButton;
    StyleModeTB: TToolButton;
    StyleAddTB: TToolButton;
    StyleDeleteTB: TToolButton;
    SepStyle2: TToolButton;
    SepStyle3: TToolButton;
    SepStyle1: TToolButton;
    StyleToolBarMI: TMenuItem;
    StyleToolBarCmd: TAction;
    StyleModePopup: TPopupMenu;
    StyleBookDesignMI: TMenuItem;
    StyleBookPreviewMI: TMenuItem;
    StyleAddNewCmd: TAction;
    StyleDeleteCmd: TAction;
    StyleSheetAddCmd: TAction;
    StyleSheetDeleteCmd: TAction;
    StyleEditModeCmd: TAction;
{$IFNDEF FPC}
    WatchMI: TMenuItem;
...
{$ENDIF}
    ...
    procedure StyleToolBarCmdExecute(Sender: TObject);
    procedure StyleBookDesignMIClick(Sender: TObject);
    procedure StyleEditModeCmdExecute(Sender: TObject);
    procedure StyleAddNewCmdExecute(Sender: TObject);
    procedure StyleDeleteCmdExecute(Sender: TObject);
    procedure StyleSheetCBChange(Sender: TObject);
    procedure StyleSheetAddCmdExecute(Sender: TObject);
    procedure StyleSheetDeleteCmdExecute(Sender: TObject);
  private
...
{$ENDIF}
    ...
    function ChangeStyleSheetName(var ANewText: string; const AOldText: string; const AItemIndex: Integer): Boolean;
...
uses
  ...
  frxDsgnIntf, frUtils, frxUtils, frxPopupForm, frxDesgnWorkspace1,
  frxDesgnEditors, frxEditOptions, frxEditReport, frxEditPage, frAbout,
  fs_itools, frXML, frxEditReportData, frxEditVar, frxEditExpr,
  frxEditHighlight, frxEditStyle, frxNewItem, frxEditFrame, frxEditFill,
  frxStdWizard, frxIOTransportIntf,
  frxEditTabOrder, frxCodeUtils, frxRes, frxDMPClass,
  frxEvaluateForm, frxSearchDialog, frxConnEditor, fs_xml, frxVariables,
  frxRegistredEditorsDialog, frxIOTransportDialog, frDPIAwareUtils, frxPlatformServices;
...
begin
  ...
  if Value and FEditStyles then
    Report.Styles.UpdateFromObjects(SelectedObjects);
  FModifiedBy := nil;
end;
...
{$ENDIF}
  ...
  StyleSheetCB.EditableItems := True;
  StyleSheetCB.OnChangeItem := ChangeStyleSheetName;
end;
...
{$IFDEF DELPHI10}
  procedure SetToolBarGradient(AToolBar: TToolBar);
  begin
    AToolBar.DrawingStyle := ComCtrls.dsGradient;
    AToolBar.GradientStartColor := frxDesignerComp.GradientStart;
    AToolBar.GradientEndColor := frxDesignerComp.GradientEnd;
  end;

  procedure SetControlBarGradient(AControlBar: TControlBar);
  begin
    AControlBar.DrawingStyle := ExtCtrls.dsGradient;
    AControlBar.GradientStartColor := frxDesignerComp.GradientStart;
    AControlBar.GradientEndColor := frxDesignerComp.GradientEnd;
  end;
{$ENDIF}
begin
  MainImages := frxImages.MainButtonImages;
  DisabledImages := frxImages.DisabledButtonImages;
  ObjectImages := frxImages.ObjectImages;
  ...
  StylesDockTB.Images := MainImages;
  StylesDockTB.DisabledImages := DisabledImages;
...
{$IFDEF DELPHI10}
  ...
  begin
    SetToolBarGradient(StandardTB);
    SetToolBarGradient(StylesDockTB);
    SetToolBarGradient(TextTB);
    SetToolBarGradient(FrameTB);
    SetToolBarGradient(AlignTB);
    SetToolBarGradient(ExtraToolsTB);
    SetToolBarGradient(ObjectsTB1);
    SetControlBarGradient(DockTop);
    SetControlBarGradient(DockBottom);
  end;
{$ENDIF}
{$IFDEF DELPHI11}
   ...
   StylesDockTB.Transparent := False;
...
{$ENDIF}
    begin
      ...
      begin
        frxImages.SetObjectImages(Item.ButtonBmp);
        Item.ButtonImageIndex := frxImages.ObjectImages.Count - 1;
      end;
      if Item.ClassRef = nil then
        CreateButton(i, Item);
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (Item.ButtonBmp <> nil) and (Item.ButtonImageIndex = -1) then
        begin
          frxImages.SetButtonImages(Item.ButtonBmp);
          Item.ButtonImageIndex := frxImages.MainButtonImages.Count - 1;
        end;
...
{$ENDIF}
    ...
    if (c is TfrxView) and not (c is TfrxDMPMemoView) then
          if TfrxView(c).Style = '' then
            StyleCB.Text := StyleCB.Items[0] else
            StyleCB.Text := TfrxView(c).Style;
...
var
 ...
 begin
  inherited;
  {$IFNDEF FPC}
  DockTop.RowSize := frxImages.MainButtonImages.Height + 12;
  {$ELSE}
  DockTop.Height:= frxImages.MainButtonImages.Height + 12;
  PanelTB3.Align := alNone;
  PanelTB2.Align := alNone;
  PanelTB1.Align := alNone;
  StyleSheetPan.Align := alNone;
  StylesPan.Align := alNone;
  {$ENDIF}
  ReadButtonImages;
  ...
  StylesPan.AutoSize := False;

  frxAssignToolbarSize(StandardTB, frxImages.MainButtonImages, ANewPPI);
  frxAssignToolbarSize(AlignTB, frxImages.MainButtonImages, ANewPPI);
  frxAssignToolbarSize(ExtraToolsTB, frxImages.MainButtonImages, ANewPPI);
  frxAssignToolbarSize(CodeTB, frxImages.MainButtonImages, ANewPPI);
  frxAssignToolbarSize(ObjectsTB1, frxImages.ObjectImages, ANewPPI);
  frxAssignToolbarSize(StylesDockTB, frxImages.ObjectImages, ANewPPI);
  frxAssignToolbarSize(TextTB, frxImages.ObjectImages, ANewPPI);
  frxAssignToolbarSize(FrameTB, frxImages.ObjectImages, ANewPPI);
  ObjectsTB1.Width := frxImages.ObjectImages.Width + 11;
...
{$IFDEF FPC}
 ...
 {$ENDIF}
  ...
  StylesDockTB.Width := StylesDockTB.Width + Round(ofs * aNewPPI / frx_DefaultPPI);
  ...
  TextTB.Left := StylesDockTB.Left + StylesDockTB.Width;
  TextTB.Width := TextAlignBottomB.Left + TextAlignBottomB.Width;
  StylesDockTB.Left := 0;
  StylesDockTB.Top := TextTB.Top;
...
begin
  ...
  begin
    if not DeleteHGuidesCmd.Checked then
      frxImages.SetSpeedButtonGlyph(FGuidesBtn, 138)
    else if not DeleteVGuidesCmd.Checked then
      frxImages.SetSpeedButtonGlyph(FGuidesBtn, 137)
    else
      frxImages.SetSpeedButtonGlyph(FGuidesBtn, 139);
  end
  else
    frxImages.SetSpeedButtonGlyph(FGuidesBtn, 136);
end;
...
procedure TfrxDesignerForm.UpdateStyleMode;
var
  I: Integer;
begin
  for I := 0 to StyleModePopup.Items.Count - 1 do
    StyleModePopup.Items[I].Checked := TfrxStyleBookMode(Byte(StyleModePopup.Items[I].Tag)) in Report.Styles.StyleBookMode;
  StyleSheetAddTB.Enabled := (Report.Styles.StyleBookMode <> []) and FEditStyles;
  StyleSheetDeleteTB.Enabled := StyleSheetAddTB.Enabled;
  StyleSheetCB.Enabled := Report.Styles.StyleBookMode <> [];
  StyleAddTB.Enabled := FEditStyles;
  StyleDeleteTB.Enabled := FEditStyles;
end;
...
begin
  ...
  Report.Styles.GetStyleSheetList(StyleSheetCB.Items);

  LIndex := StyleSheetCB.Items.IndexOf(Report.Styles.ActiveStyleSheet);
  if LIndex > 0 then
...
function TfrxDesignerForm.ChangeStyleSheetName(var ANewText: string;
  const AOldText: string; const AItemIndex: Integer): Boolean;
begin
  Result := False;
  if AItemIndex <= 0 then Exit;
  Result := Report.Styles.RenameStyleSheet(AOldText, ANewText);
end;
...
procedure TfrxDesignerForm.ClearLastActiveObject;
begin
  if Assigned(FWorkspace) then
    FWorkspace.ClearLastView;
end;
...
{$IFNDEF FPC}
  ...
  RestoreToolbars([StandardTB, AlignTB, StylesDockTB, TextTB, FrameTB, ExtraToolsTB]);
{$ENDIF}
  if not IsPreviewDesigner then
  begin
...
{$IFNDEF FPC}
    SaveToolbars([StandardTB, TextTB, FrameTB, AlignTB, ExtraToolsTB, StylesDockTB]);
{$ENDIF}
...
begin
  ...
  begin
    ...
    case TComponent(Sender).Tag of
      ...
      33: if c is TfrxView then
            if StyleCB.ItemIndex = 0 then
              TfrxView(c).Style := '' else
              TfrxView(c).Style := StyleCB.Text;
    ...
    if TComponent(Sender).Tag in [0..5, 20..29, 32, 44] then
      if (c is TfrxView) and not FEditStyles then
      begin
        TfrxView(c).Style := '';
  ...
  begin
    ...
    if TComponent(Sender).Tag in [24, 25, 26, 34..44] then
      UpdateControls;
  end;
end;
...
begin
  ...
  if not StatusBar.HandleAllocated then
    exit;
  {$ENDIF}
  ImgLst := frxImages.MainButtonImages;
...
begin
  ...
  StyleToolBarCmd.Checked := StylesDockTB.Visible;
...
var
  ...
  procedure CopyMenuItem(MFrom, MTo: TMenuItem);
  begin
    MTo.Caption := MFrom.Caption;
    MTo.Tag := MFrom.Tag;
    MTo.Checked := MFrom.Checked;
    MTo.Bitmap := MFrom.Bitmap;
  end;

  procedure CopyAll(MFrom, MTo: TMenuItem);
  var
    j: Integer;
    m2: TMenuItem;
  begin
    if MFrom.Count > 0 then
    begin
      m.OnClick := nil;
      for j := 0 to MFrom.Count - 1 do
      begin
        m2 := TMenuItem.Create(PagePopup);
        CopyMenuItem(MFrom[j], m2);
        m2.OnClick := OnComponentMenuClick;
        MTo.Add(m2);
        CopyAll(MFrom[j], m2)
      end;
    end
...
begin
  ...
  i := LGuides.IndexOf(frFloatToStr(Position));
  case Action of
    rpAdd:
      if i < 0 then
        LGuides.Add(frFloatToStr(Position));
    rpDelete:
      if i >= 0 then
        LGuides.Delete(i);
  end;
...
procedure TfrxDesignerForm.DeleteObject(vComp: TfrxComponent);
begin
  ClearLastActiveObject;
  FWorkspace.RemoveObjectsFromLits(vComp.Objects);
  inherited;
end;
...
procedure TfrxDesignerForm.StyleAddNewCmdExecute(Sender: TObject);
var
  LItem: TfrxStyleItem;
begin
  if FEditStyles then
  begin
    LItem := Report.Styles.Add;
    LItem.CreateUniqueName;
    if (SelectedObjects.Count > 0) and (TObject(SelectedObjects[0]) is TfrxView) then
     TfrxView(SelectedObjects[0]).AssignToStyle(LItem);
    UpdateStyles;
    StyleCB.ItemIndex := StyleCB.Items.IndexOf(LItem.Name);
    ToolButtonClick(StyleCB);
    Modified := True;
  end;
end;

procedure TfrxDesignerForm.StyleBookDesignMIClick(Sender: TObject);
var
  LStyleBookMode: TfrxStyleBookModes;
begin
  TMenuItem(Sender).Checked := not TMenuItem(Sender).Checked;
  LStyleBookMode := Report.Styles.StyleBookMode;
  ...
  if Report.Styles.StyleBookMode <> LStyleBookMode then
  begin
    Report.Styles.StyleBookMode := LStyleBookMode;
    Modified := True;
  end;
  UpdateStyleMode;
end;

procedure TfrxDesignerForm.StyleDeleteCmdExecute(Sender: TObject);
var
  LItem: TCollectionItem;
begin
  if FEditStyles and (StyleCB.ItemIndex > 0) then
  begin
    LItem := Report.Styles.Find(StyleCB.Items[StyleCB.ItemIndex]);
    LItem.Free;
    StyleCB.Items.Delete(StyleCB.ItemIndex);
    StyleCB.ItemIndex := 0;
    ToolButtonClick(StyleCB);
    Modified := True;
  end;
...
procedure TfrxDesignerForm.StyleEditModeCmdExecute(Sender: TObject);
begin
  FEditStyles := StyleModeTB.Down;
  UpdateStyleMode;
end;

procedure TfrxDesignerForm.StyleSheetAddCmdExecute(Sender: TObject);
var
  LSheetName: string;
begin
  LSheetName := Report.Styles.AddStyleSheet;
  Report.Styles.ActiveStyleSheet := LSheetName;
  UpdateStyles;
  StyleSheetCBChange(StyleSheetCB);
  Modified := True;
end;

procedure TfrxDesignerForm.StyleSheetCBChange(Sender: TObject);
begin
  if StyleSheetCB.ItemIndex < 0 then Exit;
  if StyleSheetCB.ItemIndex = 0 then
    Report.Styles.ActiveStyleSheet := ''
  else
    Report.Styles.ActiveStyleSheet := StyleSheetCB.Items[StyleSheetCB.ItemIndex];
  Report.Styles.Apply;
  Modified := True;
end;

procedure TfrxDesignerForm.StyleSheetDeleteCmdExecute(Sender: TObject);
begin
  if StyleSheetCB.ItemIndex <= 0 then Exit;
  Report.Styles.DeleteStyleSheet(StyleSheetCB.Items[StyleSheetCB.ItemIndex]);
  UpdateStyles;
  StyleSheetCB.ItemIndex := 0;
  StyleSheetCBChange(StyleSheetCB);
  Modified := True;
...
procedure TfrxDesignerForm.StyleToolBarCmdExecute(Sender: TObject);
begin
  StyleToolBarCmd.Checked := not StyleToolBarCmd.Checked;
  StylesDockTB.Visible := StyleToolBarCmd.Checked;
end;
...
begin
  ...
  StyleToolBarCmd.Caption := frxGet(2386);
...
{$ENDIF}
  ...
  StyleAddNewCmd.Hint := frxGet(2490);
  StyleDeleteCmd.Hint := frxGet(2491);
  StyleSheetAddCmd.Hint := frxGet(2492);
  StyleSheetDeleteCmd.Hint := frxGet(2493);
  StyleBookTB.Hint := frxGet(2494);
  StyleEditModeCmd.Hint := frxGet(2495);
...
var
  ...
  I: Integer;
  ARegistredLanguages: TStringDynArray;
// todo: refactor
//   reg: TRegistry;
begin
...
//    reg.Free;
//  end;
  ARegistredLanguages := frxResources.RegistredLanguages;
  if Length(ARegistredLanguages) > 1 then
  begin
    AMenuItem := TMenuItem.Create(ViewMenu);
    AMenuItem.Caption := '-';
    ViewMenu.Add(AMenuItem);
    AMenuItem := TMenuItem.Create(ViewMenu);
    AMenuItem.Caption := frxGet(2475);
    ViewMenu.Add(AMenuItem);
    for I := 0 to Length(ARegistredLanguages) - 1 do
    begin
      ASubMenuItem := TMenuItem.Create(AMenuItem);
      ASubMenuItem.Caption := ARegistredLanguages[I];
      ASubMenuItem.RadioItem := True;
      ASubMenuItem.OnClick := LangSelectClick;
      if SameText(ASubMenuItem.Caption, frxResources.Language) then
        ASubMenuItem.Checked := True;
...
begin
  ...
  frxResources.Language := TMenuItem(Sender).Caption;
// todo: refactor
//  m := Sender as TMenuItem;
//  m.Checked := True;
//  frxResources.LoadFromFile(frGetAppPath + m.Caption + '.frc');
//  Localize;
//  reg := TRegistry.Create;
//  try
//    reg.RootKey := HKEY_CURRENT_USER;
...
begin
  if Page <> nil then
    ...
    begin
      ...
      if Scale < 0.3 then
        Scale := 0.3;
      ScaleCB.Text := frFloatToStr(Round(Scale * 100)) + '%';
      ScrollBox.SetFocus;
    end;
end;
