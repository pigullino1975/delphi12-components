...
{$IFDEF DELPHI6}, Variants {$ENDIF}, frHashUtils, frUnicodeUtils;
  ...
  try
    try
      ...
      begin
        ...
        begin
          Socket.SendText(frBase64Encode(AnsiString(FAuth)) + #13#10);
          FAuth := FPassword;
          AddLogOut('****');
        end
...
//    {$ENDIF}
            ...
            bound := boundary + UpperCase(String(Copy(frmd5String(AnsiString(DateTimeToStr(Now))), 1, 14)));
...
{$ENDIF}
  ...
  if ws1 <> '' then ws1 := '=?UTF-8?B?' + {$IFDEF DELPHI12}WideString(frBase64Encode(UTF8Encode(ws1))){$ELSE}frBase64Encode(UTF8Encode(ws1)){$ENDIF} + '?=';
...
begin
  ...
  begin
{$IFNDEF DELPHI12}
    Result := '=?UTF-8?B?' + frBase64Encode(UTF8Encode(TranslateString(Str))) + '?=';
{$ELSE}
    Result := '=?UTF-8?B?' + frBase64Encode(UTF8Encode(Str)) + '?=';
{$ENDIF}
  end
  else
    Result := '';
