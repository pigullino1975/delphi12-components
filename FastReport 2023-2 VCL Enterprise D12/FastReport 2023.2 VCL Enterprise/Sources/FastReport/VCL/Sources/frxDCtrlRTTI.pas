...
uses
  ...
  fs_iinterpreter, fs_iformsrtti, frxDCtrl, frxClassRTTI, Variants, frCore;
...
begin
  ...
  begin
    if MethodName = 'PAGES.GET' then
      Result := frInteger(TfrxPageControl(Instance).PageControl.Pages[Caller.Params[0]])
  end;
...
begin
  ...
  begin
    if PropName = 'PAGECONTROL' then
      Result := frInteger(TfrxPageControl(TfrxTabSheet(Instance).Parent))
...
begin
  ...
  begin
    if PropName = 'PAGECONTROL' then
      TfrxTabSheet(Instance).Parent := TfrxPageControl(frInteger(Value))
  end;
end;
