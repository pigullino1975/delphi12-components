...
uses
  Windows, Types, Classes, fs_iinterpreter, frxADOComponents, fs_iadortti, Variants, frCore;
...
begin
  ...
  begin
    if PropName = 'DATABASE' then
      Result := frInteger(TfrxADODatabase(Instance).Database)
  end
  else if ClassType = TfrxADOTable then
  begin
    if PropName = 'TABLE' then
      Result := frInteger(TfrxADOTable(Instance).Table)
  end
  else if ClassType = TfrxADOQuery then
  begin
    if PropName = 'QUERY' then
      Result := frInteger(TfrxADOQuery(Instance).Query)
  end
end;
