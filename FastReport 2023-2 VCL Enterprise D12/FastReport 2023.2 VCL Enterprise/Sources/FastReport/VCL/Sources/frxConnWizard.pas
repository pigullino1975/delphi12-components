...
uses
  ...
  frxClass, frxSynMemo, frxCustomDB, frxBaseForm, frCore
  {$IFDEF FPC}
  , LResources, LCLType
  {$ENDIF}
{$IFDEF DELPHI6}
  ...
  begin
    ...
    begin
      for i := 0 to ConnCB.Items.Count - 1 do
        if frxObjects[frInteger(ConnCB.Items.Objects[i])].ClassRef = FDatabase.ClassType then
        begin
          ConnCB.ItemIndex := i;
          break;
        end;
...
begin
  ...
  ClassRef := frxObjects[frInteger(ConnCB.Items.Objects[ConnCB.ItemIndex])].ClassRef;
...
begin
  ...
  ClassRef := frxObjects[frInteger(ConnCB1.Items.Objects[ConnCB1.ItemIndex])].ClassRef;
...
begin
  ...
  ClassRef := frxObjects[frInteger(ConnCB2.Items.Objects[ConnCB2.ItemIndex])].ClassRef;
...
begin
  inherited;
  Toolbar1.Images := frxImages.MainButtonImages;
{$IFDEF FPC}
