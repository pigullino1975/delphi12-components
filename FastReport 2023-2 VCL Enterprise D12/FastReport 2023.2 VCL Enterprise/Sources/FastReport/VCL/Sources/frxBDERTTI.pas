...
begin
  ...
  begin
    if PropName = 'DATABASE' then
      Result := frInteger(TfrxBDEDatabase(Instance).Database)
  end
  else if ClassType = TfrxBDETable then
  begin
    if PropName = 'TABLE' then
      Result := frInteger(TfrxBDETable(Instance).Table)
  end
  else if ClassType = TfrxBDEQuery then
  begin
    if PropName = 'QUERY' then
      Result := frInteger(TfrxBDEQuery(Instance).Query)
  end
end;
