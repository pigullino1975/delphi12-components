...
uses
  frCore, frxClass, frxDMPClass, Variants;
...
uses SysUtils, Controls, Classes, Graphics, frxDCtrl, frxDBSet, TypInfo,
     frxCross, frxRich, frUnicodeUtils, frxADOComponents;
        ...
        if not (PropList[i]^.PropType^.Kind = tkMethod) then
          if PropList[i]^.Name <> 'Name' then
            if (PropList[i]^.PropType^.Kind = tkClass) then
              begin
                APersistent := TPersistent(GetObjectProp(AClass1, frAnsiToStr(PropList[i]^.Name), TPersistent));
                if PropList[i]^.Name <> 'FillGap' then
                  if (APersistent <> nil) then
                    APersistent.Assign(TPersistent(GetObjectProp(AClass2, frAnsiToStr(PropList[i]^.Name), TPersistent)));
              end
            else
            if (PropList[i]^.PropType^.Kind = tkEnumeration) then
              begin
                PropClassTypeData := GetTypeData(PropList[i]^.PropType^);
                j := GetEnumValue(PropClassTypeData^.BaseType^, GetPropValue(AClass2, frAnsiToStr(PropList[i]^.Name)));
                SetOrdProp(AClass1, frAnsiToStr(PropList[i]^.Name), j);
              end
            else
              begin
                SetPropValue(AClass1, frAnsiToStr(PropList[i]^.Name), GetPropValue(AClass2,
                  frAnsiToStr(PropList[i]^.Name)));
              end;
    finally
...
begin
  ...
  Memo.Memo.Assign(RichS.RichEdit.Lines);
