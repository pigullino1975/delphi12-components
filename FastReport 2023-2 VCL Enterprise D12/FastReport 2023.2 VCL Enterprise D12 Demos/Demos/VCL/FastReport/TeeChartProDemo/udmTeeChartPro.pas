...
uses
  ...
  frxDBSet, Db, frxClass, ADODB,frxJSON, frxTransportHTTP,DateUtils, frCoreClasses;

type
  TdmTeeChartPro = class(TDataModule)
    Customers: TADOTable;
...
procedure TdmTeeChartPro.ConnectToBase(ADOConnection: TADOConnection; FileName: String);
begin
...
{$IFNDEF CPUX64}
  ADOConnection.ConnectionString := 'Provider=Microsoft.Jet.OLEDB.4.0;Data Source=' +
    ExtractFilePath(Application.ExeName) + '..\..\Data\' + FileName;
{$ELSE}
  ADOConnection.ConnectionString := 'Provider=Microsoft.ACE.OLEDB.12.0;Data Source=' +
    ExtractFilePath(Application.ExeName) + '..\..\Data\' + FileName;
{$ENDIF}
  ADOConnection.Open;
end;

procedure TdmTeeChartPro.DataModuleCreate(Sender: TObject);
begin
  ConnectToBase(ADOConnection1, 'demo.mdb');
  ConnectToBase(ADOConnection2, 'Northwind.mdb');
  ConnectToBase(ADOConnection3, 'DatabaseOrg.mdb');
  NameCandles := 'GAS_USD';
...
procedure TdmTeeChartPro.JSON_DSGetValue(const VarName: string;
  var Value: Variant);
var
  Item: TfrxJSON;
  Time : string;
