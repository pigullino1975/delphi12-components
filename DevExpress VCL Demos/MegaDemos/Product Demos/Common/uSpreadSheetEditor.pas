...
uses
  ...
  dxRibbonCustomizationForm, dxRibbonSkins, cxFontNameComboBox, cxDropDownEdit,
  ...
  Vcl.ImgList, cxImageList, dxSpreadSheet, uDocumentEditor,
  dxCloudStorage, dxSpreadSheetCoreFormulas, dxPSGlbl, dxPSUtl, dxPSEngn, dxPrnPg, dxBkgnd, dxWrap, dxPrnDev,
  dxPSCompsProvider, dxPSFillPatterns, dxPSEdgePatterns, dxPSPDFExportCore, dxPSPDFExport, cxDrawTextUtils, dxPSPrVwStd,
  dxPSPrVwAdv, dxPSPrVwRibbon, dxPScxPageControlProducer, dxPSdxPDFViewerLnk, dxPScxExtComCtrlsLnk, dxPSTVLnk,
  dxPSdxLCLnk, dxPScxEditorLnks, dxPSTextLnk, dxPSdxSpreadSheetLnk, dxPScxEditorProducers, dxPScxExtEditorProducers,
  dxPSCore, dxPSBaseGridLnk, dxPSdxSpreadSheetLnkCore, dxPSRichEditControlLnk,
  dxRibbonMarginsGallery;

type
  { TSpreadSheetEditor }

  TSpreadSheetEditor = class(TDocumentEditor)
    dxSpreadSheet1: TdxSpreadSheet;
    dxComponentPrinter1: TdxComponentPrinter;
    dxComponentPrinter1Link1: TdxSpreadSheetReportLnk;
    dxRibbon1: TdxRibbon;
  ...
  private
    function GetFormat: TdxSpreadSheetCustomFormatClass;
  protected
...
{ TfmSpreadSheetEditor }

function TSpreadSheetEditor.GetFormat: TdxSpreadSheetCustomFormatClass;
begin
  dxSpreadSheetFormatsRepository.Find(FileExtension, Result);
end;

function TSpreadSheetEditor.HasChanges: Boolean;
var
  AFormat: TdxSpreadSheetCustomFormatClass;
begin
  AFormat := GetFormat;
  Result := (dxSpreadSheet1.ActiveSheetAsTable.History.UndoActionCount > 0) and
    (AFormat <> nil) and (AFormat.GetWriter <> nil);
end;

procedure TSpreadSheetEditor.DoSave(AStream: TStream);
begin
  dxSpreadSheet1.SaveToStream(AStream, GetFormat);
end;

procedure TSpreadSheetEditor.Load(AStream: TStream);
begin
  dxSpreadSheet1.LoadFromStream(AStream);
end;

procedure RegisterDocumentEditor;
var
  I: Integer;
begin
  for I := 0 to dxSpreadSheetFormatsRepository.Count - 1 do
    DocumentEditorFactory.RegisterEditor(dxSpreadSheetFormatsRepository.Exts[I], TSpreadSheetEditor);
end;

procedure UnregisterDocumentEditor;
var
  I: Integer;
begin
  for I := 0 to dxSpreadSheetFormatsRepository.Count - 1 do
    DocumentEditorFactory.UnRegisterEditor(dxSpreadSheetFormatsRepository.Exts[I]);
end;
