object dxCustomDemoFrame: TdxCustomDemoFrame
  Left = 0
  Top = 0
  Width = 640
  Height = 480
  Align = alClient
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  ParentFont = False
  TabOrder = 0
  object lcFrame: TdxLayoutControl
    Left = 0
    Top = 0
    Width = 451
    Height = 305
    Align = alClient
    TabOrder = 0
    object lcControlContent: TdxLayoutControl
      Left = 22
      Top = 28
      Width = 407
      Height = 217
      TabOrder = 0
      object lcControlContentGroup_Root: TdxLayoutGroup
        AlignHorz = ahClient
        AlignVert = avClient
        Hidden = True
        ShowBorder = False
        Index = -1
      end
      object lgContent: TdxLayoutGroup
        Parent = lcControlContentGroup_Root
        AlignHorz = ahClient
        AlignVert = avClient
        CaptionOptions.Text = 'New Group'
        CaptionOptions.Visible = False
        ShowBorder = False
        Index = 0
      end
    end
    object lcFrameGroup_Root: TdxLayoutGroup
      AlignHorz = ahClient
      AlignVert = avClient
      CaptionOptions.Visible = False
      Hidden = True
      ItemIndex = 1
      ShowBorder = False
      Index = -1
    end
    object liDescription: TdxLayoutLabeledItem
      Parent = lcFrameGroup_Root
      AlignHorz = ahClient
      AlignVert = avBottom
      CaptionOptions.Glyph.SourceDPI = 96
      CaptionOptions.Glyph.Data = {
        
        326832683476313068325632347A222F3E0D0A3C2F7376673E0D0A}
      CaptionOptions.Text = 'Label'
      CaptionOptions.WordWrap = True
      Index = 1
    end
    object lgParentContent: TdxLayoutGroup
      Parent = lcFrameGroup_Root
      AlignHorz = ahClient
      AlignVert = avClient
      CaptionOptions.Visible = False
      Index = 0
    end
    object liParentContent: TdxLayoutItem
      Parent = lgParentContent
      AlignHorz = ahClient
      AlignVert = avClient
      CaptionOptions.Visible = False
      Control = lcControlContent
      ControlOptions.OriginalHeight = 250
      ControlOptions.OriginalWidth = 299
      ControlOptions.ShowBorder = False
      Index = 0
    end
  end
  object ActionList1: TActionList
    Left = 400
    Top = 24
  end
  object amAdorners: TdxUIAdornerManager
    OnActiveChanged = amAdornersActiveChanged
    Left = 64
    Top = 32
  end
end
