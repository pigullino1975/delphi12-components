...
type
  ...
  private
    ...
    FIcons: TObjectDictionary<string, TdxSmartGlyph>;
...
begin
  ...
  FIcons := TObjectDictionary<string, TdxSmartGlyph>.Create([doOwnsValues]);
...
function TWeatherIconCache.GetIcon(const AIconHRef: string): TdxSmartGlyph;
begin
  Result := nil;
  if AIconHRef = '' then
    Exit;
  ...
  try
    ...
    begin
      Result := TdxSmartGlyph.CreateSize(1, 1);
