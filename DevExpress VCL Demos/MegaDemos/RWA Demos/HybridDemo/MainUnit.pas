...
type
  ...
  TMainForm = class(TdxForm, IdxLocalizerListener)
    ...
    CalloutPopupMain: TdxCalloutPopup;
    ...
    procedure ptcProductsResize(Sender: TObject);
  private
...
begin
  CalloutPopupMain.Popup(tbMain, TdxTileBarItemAccess(tbiLanguages).ViewInfo.Bounds);
end;
...
begin
  ...
  CalloutPopupMain.BorderColor := tbiLanguages.Style.GradientBeginColor;
  CalloutPopupMain.Color := CalloutPopupMain.BorderColor;
...
procedure TMainForm.ptcProductsResize(Sender: TObject);
begin
  TdxTileControl(Sender).Height := ScaleFactor.Apply(96);
end;
